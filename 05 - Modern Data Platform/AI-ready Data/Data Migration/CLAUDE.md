...
## Fases de la Migración (estructura física — modelo Mainframe Modernization)

Igual que `Mainframe Modernization` materializa su lifecycle en carpetas `Fase N - .../` (con Specialists internos o marcadas `(externo)`) más un `Enablement/` propio, Data Migration materializa el lifecycle DataOps en **8 fases físicas** + `Enablement/` bajo este sub-offering. Cada fase es un contenedor orquestador: el delivery lo ejecuta el SME canónico de `Solutioning/` vía `[INVOKE]`, y la validación se hace contra el `Reference Data Lab` (Enablement de este sub-offering). Las fases marcadas `(externo)` las gobierna otro offering/SME.

```
Data Migration/
├── CLAUDE.md                              ← este archivo (L3)
├── Enablement/
│   └── Training - Reference Data Lab/     ← genera data estates de referencia + answer key
...
├── Fase 6 - Curate & Gold/
├── Fase 7 - Reconcile & DQ Certification/
└── Fase 8 - Cutover & Decommission (externo)/
```

> El **Enablement** (`Enablement/Training - Reference Data Lab/`) provee data estates de referencia + answer key que ejercitan las Fases 4-7 sin IP de cliente. Seeds canónicos: `seed-sap-to-bigquery-medallion/` y `seed-sap-banking-crm-to-bigquery-medallion/`. Reutilizable por otros sub-offerings del domain vía referencia.

| Fase | Mapea a (DataOps) | Objetivo | Gate de salida | Ejecuta (`[INVOKE]`) |
|------|-------------------|----------|----------------|----------------------|
| **1 — Discover & Source Profiling** | DISCOVER | Inventario del data estate, volumetría, dependencias, clasificación de tablas por riesgo | Source Profile + mapa de dependencias + clasificación firmados | Data & ML SME · Data Architect |
| **2 — Target Design & Data Contracts** | DESIGN | Modelo medallion target (Bronze/Silver/Gold) + ADRs (plataforma, patrón) + data contracts | Target schema + ADR-MDP-MIG + contracts versionados | Data Architect · Cloud sub-SME |
| **3 — Mapping & Transformation Rules** | DESIGN→BUILD | Lineage columna-a-columna + reglas de transformación (DATS, CURR/TCURX, ALPHA, dedup, entity resolution) + diseño de DQ | Mapping doc + reglas + plan DQ aprobados | Data & ML SME · Specialist - Legacy Datastore Migration |
| **4 — Ingest & Bronze** | BUILD | Extracción + landing raw 1:1 (carga inicial + CDC) en Bronze | Bronze poblado = conteo fuente · CDC con lag bajo control | Specialist - Legacy Datastore Migration · Interoperability (CDC) |
| **5 — Conform & Silver** | BUILD→TEST | Tipado, conformado, dedup, FK validadas/cuarentena, MDM/entity resolution → Silver | DQ tests verdes · cuarentena documentada | Data & ML SME · Data Architect |
| **6 — Curate & Gold** | BUILD→RELEASE | Modelo dimensional + data products + semantic layer → Gold | Gold reconciliado vs Silver · contracts publicados | Data & ML SME · Industry SME (BIAN/Insurance/Retail) |
| **7 — Reconcile & DQ Certification** | TEST | Reconciliación (conteos + sumas por moneda) + certificación DQ + readiness de cutover | Reporte de reconciliación firmado (< tolerancia acordada) | Specialist - Equivalence Testing · Cybersecurity Data Security (PII) |
| **8 — Cutover & Decommission** `(externo)` | RELEASE→OPERATE | Cutover a target + drain + decommission del legacy con retención regulatoria | Cutover ejecutado · legacy archivado/retenido (CNBV 10a · LFPDPPP) | Owner del sistema fuente · AMS Reinvention (07) · `[DEPENDS-ON: 07]` |

`[INVARIANTE]` No se avanza de Fase 7 a Fase 8 sin reporte de reconciliación firmado dentro de tolerancia — la trampa de decimales por moneda (TCURX) y los huérfanos referenciales se cazan aquí, no después del cutover.

`[GAP]` Si una fase requiere un agente de ejecución ★ DC dedicado (no solo un SME GenAI), declararlo como `[GAP — crear o asignar]`. Hoy las fases delegan a SMEs GenAI + al Reference Data Lab; no hay specialists ★ DC propios bajo Data Migration (a diferencia de Mainframe, donde RE/Transpilation sí lo son).
...
- **Test Data Management para pre-PROD**: los entornos DEV/QA/UAT necesitan datos representativos del data product migrado para que los consumers puedan desarrollar y testear contra el nuevo endpoint. `Specialist - Test Data Management` produce subsets anonimizados y de referencia para estos entornos — sin PII real en pre-PROD.
...
- ADR-MDP-DPF-003: Estrategia de Test Data Management para pre-PROD (subconjunto anonimizado del real · datos de referencia generados · datos de referencia estáticos) — `Specialist - Test Data Management` ejecuta.
...
2. **Factory para datos de no-producción en programa de migración masivo**: en un programa donde se migran 50+ data marts en paralelo, los equipos de consumers necesitan datos en DEV/QA para desarrollar contra el nuevo endpoint antes de que el PROD esté listo. `Specialist - Test Data Management` produce subsets anonimizados o de referencia por data product — el factory los cataloga y distribuye a los entornos pre-PROD con el mismo schema del data product PROD.
