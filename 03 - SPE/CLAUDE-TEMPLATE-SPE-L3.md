...
**Lo que NO hago**: ejecuto delivery técnico end-to-end del componente. Delego al SME canónico de `SME/` vía `[INVOKE]` siguiendo §13 de DC Universal Rules. Mi rol es gobernar el lifecycle específico de este sub-offering, mantener el component catalog, y validar gates.
...
|-------------|------------------------------|------------------------------------|
| **{Solution 1}** | {microservicio · módulo IaC · pipeline · ...} | `SME/{ruta}/` |
| **{Solution 2}** | ... | `[GAP — crear o asignar SME]` si no existe SME canónico aún |

**Regla**: si un solution L4 no tiene SME canónico en `SME/`, declararlo explícitamente como `[GAP — crear o asignar]` y abrir CR en `delivery-playbook` con owner del gap. **No improvisar** delivery sin SME asignado — el sub-offering no puede comprometer ese solution hasta resolverlo.
