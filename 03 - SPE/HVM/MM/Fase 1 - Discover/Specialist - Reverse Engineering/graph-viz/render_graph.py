#!/usr/bin/env python3
"""
render_graph.py — Reverse Engineering · visualizador de grafo de dependencias.

Renderiza CUALQUIER `dependency-graph.json` (esquema compartido) como una
visualizacion HTML force-directed self-contained (OFFLINE: D3 inline + logo
Accenture base64). Sirve para grafos SINTETICOS (Training - Synthetic Codebase Lab)
o RECONSTRUIDOS por RE desde codigo real. Computa fan-in/out, alcanzabilidad,
SCCs (Tarjan) y clasificacion de acceso; color/cluster/capas se derivan de los datos.

Uso:
    python render_graph.py --graph <ruta/dependency-graph.json> [--out <salida.html>]

Lee opcionalmente, junto al grafo: copybook-usage.json y copybook-glossary.json.
Paleta Accenture (chrome). Dominios en escala categorica restringida.
"""
import json
import os
...
<div class="layout">
  <aside>
    ...
    <h3>Copybook coupling layer</h3>
    <select id="cpy"><option value="">— none —</option></select>
    <div class="note" id="cpy-note">The hidden hairball: programs that share a
      copybook are coupled by data even if they never call each other.</div>
    <h3>Search program</h3>
    <input type="text" id="search" placeholder="e.g. DEPB0144 / UDMSIIWR">
    <h3>Domains</h3>
...
<div class="backdrop" id="backdrop">
  <div class="modal">
    ...
    <div class="sub">Dependency graph of a mainframe system · <b>each node is a program</b>, each edge a call</div>
    <p><b>Why does it look like a cloud?</b> A real mainframe system is <b>not</b> a clean tree of boxes and arrows. It's a <i>hairball</i>: hundreds of programs where everything touches everything through shared utilities and common data structures. That density is exactly where the pain —and the risk— of a migration project lives. This view exposes the real topology: hubs, cycles, communities and couplings.</p>
    <div class="grid">
      <b id="m-nodes"></b><span>programs (graph nodes)</span>
      <b id="m-edges"></b><span>call dependencies (edges)</span>
      <b id="m-hub"></b><span>highest fan-in hub (called by hundreds)</span>
      <b id="m-cyc"></b><span>cycles — circular dependencies between programs</span>
      <b id="m-dead"></b><span>unreachable nodes — likely dead code</span>
      <b id="m-cpy"></b><span>shared copybooks — data coupling</span>
      <b id="m-acc"></b><span>inquiry / update transactions</span>
    </div>
    ...
    <p style="color:var(--muted);font-size:12px"><b>Takeaway:</b> planning a Strangler Fig by looking only at the call graph <b>fails</b>, because data coupling (copybooks) is invisible there. Navigate with <kbd>scroll</kbd> to zoom and drag to pan.</p>
...
CPY = _load("copybook-usage.json")        # opcional: capa de acoplamiento
GLO = _load("copybook-glossary.json")     # opcional: significado de copybooks
...
# alcanzabilidad desde entry points (WFL/ONLINE); si no existen, raíces sin fan-in
roots = [n for n in nodes if nodes[n].get("layer") in ("WFL", "ONLINE")]
if not roots:
...
LAYER_ORDER = ["WFL", "ONLINE", "BL", "DA", "UTIL"]
