CREATE PROCEDURE "informix".sp_bloqueactas(pNumCte CHAR(20), pEjecutivo CHAR(8))
RETURNING
CHAR(6) 	AS 	cCodRet,
CHAR(6) 	AS 	cCodRet2,
CHAR(6) 	AS 	cCodRet3
	...
	DEFINE iStatusCred	    INTEGER;
	DEFINE dFechaHoy	    DATE;
	DEFINE iEntro		    INTEGER;
	DEFINE iEntro2		    INTEGER;
...
BEGIN
    ON EXCEPTION SET iSqlErr, iIsamErr
		IF iSqlErr != 0 THEN
			LET cCodRet = iSqlErr;
			RETURN TRIM(cCodRet), TRIM(cCodRet2), TRIM(cCodRet3);
		END IF;
    END EXCEPTION;

	--SET DEBUG FILE TO "/home/sysifx/armandomorales/sp_bloqueactas.out";
	--TRACE ON;
	
	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	
	IF NVL(pNumCte,'') = '' AND NVL(pEjecutivo,'') = '' THEN
		LET cCodRet = '000001';
		RETURN TRIM(cCodRet), TRIM(cCodRet2), TRIM(cCodRet3);
	END IF;
	...
	WHERE empresa = '001';
		...
		--SE CONSULTA AL CLIENTE PARA OBTENER EL ESTATUS DE SU CUENTA, LA CUAL SI ES ACTIVA SE BLOQUEARA	
		FOREACH
			SELECT cuenta, status_cta
			INTO cCuenta, cStatusCta
			FROM bdicheq:"informix".sc_maechq   
			WHERE num_cte = pNumCte
			
			IF cStatusCta = '1' THEN
				EXECUTE PROCEDURE bdicheq:"informix".bloqueo_cta('001',TRIM(cCuenta), '0','09',3, dFechaHoy, pEjecutivo,'', '11','S','12','Z' )
				INTO cCodRet2, cClave;
		...
		----SE CONSULTA AL CLIENTE PARA OBTENER EL ESTATUS DE SU CREDITO, EL CUAL SI ES ACTIVA SE BLOQUEARÁ
		FOREACH
			SELECT num_credito, id_unidad_prod
			INTO cNumCred, iStatusCred
			FROM bdicred:"informix".sd_maecred
			WHERE numcte = pNumCte
			
			IF iStatusCred IS NULL THEN
				EXECUTE PROCEDURE bdicred:"informix".sp_bloqueocuenta('001', TRIM(cNumCred), 3, '09', pEjecutivo, 1)
				INTO cCodRet3, cMensajeCod;
		...
		IF iEntro = 0 AND iEntro2 = 0 THEN
			LET cCodRet = "000002";
		END IF
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se crea procedimiento el cual bloquea las cuentas que esten activas o los creditos que esten activos.', 
