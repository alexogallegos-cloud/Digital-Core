CREATE PROCEDURE "informix".consultacreditobc(pEmpresa      CHAR(3), psucursal char(4), pNumTarjeta char(16))
RETURNING CHAR(6)       AS cod_ret,
          CHAR(255)      AS mensaje,
          CHAR(52)      AS nombre,
          INTEGER AS pago_minimo,
          INTEGER AS saldo_actual,
          INTEGER AS pag_no_gen_int,
		  INTEGER AS monto_retenido,
		  CHAR(16) AS Folio;
-----------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------
...
DEFINE dNombreCliente        CHAR(52); 
DEFINE dPagoNoGenInteres     DECIMAL(18,2);
DEFINE dFolioOperacion		 CHAR(16);
...
LET dnombrecliente = '';
LET dFolioOperacion = '0000000000000000';

--SET DEBUG FILE TO '/tmp/consultacreditobc.out'; --- MODIFICAR RUTA DEL ARCHIVO
--TRACE ON;

BEGIN
   ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
      IF iSqlErr != 0 THEN
         LET cCodRet     =  '000999'; --iSqlErr;
         LET cMensajeRet = 'Falla del Sistema :'|| iSqlErr;
        RETURN cCodRet, cMensajeRet, dNombreCliente,dPagoMinimo, dSdoTotalLiq , dPagoNoGenInteres,dSdoRetenido,dFolioOperacion ;
      END IF;
   END EXCEPTION;

   IF ( NVL(pEmpresa,'') = '') OR (NVL(pNumTarjeta,'') = '') OR ( NVL(psucursal,'') = '')THEN
      let cCodRet = '000001' ;
      let cMensajeRet = 'No hay información para realizar la consulta'; 
	  RETURN cCodRet, cMensajeRet, dNombreCliente,dPagoMinimo, dSdoTotalLiq , dPagoNoGenInteres, dSdoRetenido,dFolioOperacion ;
   ELSE
      LET cEmpresa= TRIM(pEmpresa);
	  LET cNumTarjeta = TRIM(pNumTarjeta);
   END IF;
   ...
   if nvl(cNumCredito,'') <> '' then 
      execute PROCEDURE "informix".sp_consultasaldocorte(pEmpresa,cNumCredito, 0) into cCodRet, dPagoNoGenInteres;   
      if cCodRet = '00000' then  
         Select trim(nombre1) || ' '|| trim(nombre2)
           into dNombreCliente
           from bdinteg:si_cliente
           where numcte = (select numcte from bdicred:sd_maecred where empresa = pEmpresa and num_credito = cNumCredito );
         LET dNombreCliente = nvl(dNombreCliente,'');
         EXECUTE PROCEDURE informix.sp_consulta_saldos_general( pEmpresa, cNumCredito) into   
            cCodRet, cMensajeRet, cNumCredito, cCodTipCred,dtFechaOrigen, dtFechaProxPago,
         ...
         if cCodRet::integer < 0 then
		    ...
		    LET cCodRet = '000001' ;
            LET cMensajeRet = 'No hay información para realizar la consulta';
         elif cCodRet <> '000000' then
		    let cCodRet = '000005' ;
            let cMensajeRet = 'Ocurrió un error al realizar el calculo';
         end if;	   
      else         
         let cMensajeRet = 'Ocurrió un error al realizar el calculo' ||cCodRet;
		 let cCodRet = '000005' ;
      end if;
   else 
      let cCodRet = '000003' ;
      let cMensajeRet = 'El numero de Tarjeta no Existe';
   end if;          
   if cCodRet = '000000' then 
     let dFolioOperacion = 'c'||substr(pNumTarjeta, length(pNumTarjeta)-1,1)|| to_char(current, '%d%m%Y%H%M%S');  
   end if;	 
...
END PROCEDURE
DOCUMENT
'Se realiza procedimiento para obtener los saldos reciclador ',
'generales del crédito',
'AUTOR : Faviola Martínez',
'FECHA : Agosto 2013',
'BD    : BDICRED';

CREATE PROCEDURE "informix".pagocreditobc(pEmpresa      CHAR(3), psucursal char(4), pNumTarjeta char(16), pMonto money, pUsuario char(8), pTransaccion char(4) )
RETURNING CHAR(6)       AS cod_ret,
          CHAR(255)     AS mensaje,
          CHAR(16)      AS folio,
          CHAR(9)       AS boleto_ini,
          CHAR(9)       AS boleto_fin;
-----------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------
...
DEFINE cFolioOper       CHAR(16);
DEFINE cBoleto_ini      CHAR(9);
DEFINE cBoleto_fin      CHAR(9);
DEFINE p_referencia     CHAR(40);

DEFINE v_Remanente      MONEY(14,2) ;
DEFINE v_IntMoraCob     MONEY(14,2) ;
DEFINE v_IntVencCob     MONEY(14,2) ;
DEFINE v_CapVencCob     MONEY(14,2) ;
DEFINE v_IntVigCob      MONEY(14,2) ;
DEFINE v_CapVigCob      MONEY(14,2) ;
DEFINE v_Impuesto       MONEY(14,2) ;
DEFINE v_Comision       MONEY(14,2) ;
DEFINE v_Seguro         MONEY(14,2) ;
...
LET cErrorInfo           = '';
LET cCodRet              = '000000';
LET cMensajeRet          = 'Se realizó el pago correctamente.';

LET cNumCredito          = '';


LET cFolioOper       = '000000000';
LET cBoleto_ini      = '000000000';
LET cBoleto_fin      = '000000000';
LET p_referencia     = '';
...
BEGIN
   ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
      IF iSqlErr != 0 THEN
         LET cCodRet     =  '000999'; --iSqlErr;
         LET cMensajeRet = 'Falla del Sistema :'|| iSqlErr;
         RETURN cCodRet, cMensajeRet, cFolioOper ,cBoleto_ini , cBoleto_fin;
      END IF;
   END EXCEPTION;


   IF ( NVL(pEmpresa,'') = '') OR (NVL(pNumTarjeta,'') = '') OR (pMonto<=0) OR (NVL(psucursal,'') = '') or
       (NVL(pUsuario,'') = '') OR (NVL(pTransaccion,'') <> '5010')
        THEN
      let cCodRet = '000001' ;
      let cMensajeRet = 'No hay información para realizar el pago'; 
	  RETURN cCodRet, cMensajeRet, cFolioOper ,cBoleto_ini , cBoleto_fin;
   END IF;

   --SET DEBUG FILE TO '/informix/pagocreditobc.out'; --- MODIFICAR RUTA DEL ARCHIVO
   --TRACE ON;
   SET ISOLATION TO dirty READ;
  
--SET DEBUG FILE TO 'sp_consulta_saldos_general.out'; --- MODIFICAR RUTA DEL ARCHIVO
--TRACE ON;
   ...
   if nvl(cNumCredito,'') <> '' then 
      LET cFolioOper = substr(pNumTarjeta, length(pNumTarjeta)-2,2)|| to_char(current, '%d%m%Y%H%M%S');  
      LET p_referencia = 'Reciclador:'||to_char(current, '%H%M%S');   
      execute PROCEDURE "informix".principalrefer( pEmpresa,
                           cNumCredito,
                           1, 
                           pNumTarjeta,
                           pUsuario,
                           ...
                           0, 
                           pMonto,
                           p_referencia) into  cCodRet, v_Remanente, v_IntMoraCob, 
						                       v_IntVencCob, v_CapVencCob, v_IntVigCob, 
											   v_CapVigCob, v_Impuesto,v_Comision, v_Seguro;    
      if cCodRet::integer < 0 then 
         /*SELECT descripcion
           INTO cMensajeRet
         ...
         LET cMensajeRet = 'Falla del Sistema :'||cCodRet ;		   		     
		 LET cCodRet     =  '000999'; --iSqlErr;
      elif cCodRet <> '000' then	              
	     LET cFolioOper ='000000';
         LET cMensajeRet = 'Crédito Bloqueado '||cCodRet ;
		 LET cCodRet     =  '000301'; --iSqlErr;
      end if;
   else 
      let cCodRet = '000008' ;
      let cMensajeRet = 'El numero de Tarjeta no Existe';	
...
END PROCEDURE
DOCUMENT
'Se realiza procedimiento para aplicar pago los saldos reciclador ',
'generales del crédito',
'AUTOR : Faviola Martínez',
'FECHA : Agosto 2013',
'BD    : BDICRED';

CREATE PROCEDURE "informix".revpagocredbc(pEmpresa      CHAR(3), psucursal char(4), pUsuario char(16), pFolio CHAR(16) )
RETURNING CHAR(6)       AS cod_ret,
          CHAR(255)      AS mensaje;
-----------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------
...
LET cErrorInfo           = '';
LET cCodRet              = '000000';
LET cMensajeRet          = 'Se realizó el reverso correctamente.';
...
BEGIN
   ...
   IF ( NVL(pEmpresa,'') = '') or ( NVL(psucursal,'') = '') or  ( NVL(pUsuario,'') = '') or (NVL(pFolio,'') = '')  THEN
      let cCodRet = '000001' ;
      let cMensajeRet = 'No hay información para realizar el reverso'; 
	  RETURN cCodRet,cMensajeRet; 
   END IF;

--SET DEBUG FILE TO '/informix/revpagocredbc.out'; --- MODIFICAR RUTA DEL ARCHIVO
--TRACE ON;

   SET ISOLATION TO dirty READ;
   select limit 1 reversado
	 into vlReversado
	 from bdicred:sd_movdia 
	where empresa = pEmpresa
	  and folio_suc = pFolio;
		
   if vlReversado ='S' then 
      LET cCodRet= '000004';
      LET cMensajeRet= 'El folio '||pFolio || ' ya fue reversado' ;	  	
   elif vlReversado = 'N' then 
      execute PROCEDURE "informix".reversion(pEmpresa,psucursal,pUsuario,pFolio,0) 
	               into cCodRet;       
      
      if cCodRet = '431' then	     
	     LET cCodRet = lpad(trim(cCodRet),6,'0' );
         LET cMensajeRet     = 'Pago no es el último reversa en orden';
	  elif cCodRet::integer < 0 then          
         LET cMensajeRet     = 'Falla de Sistema '||cCodRet;	    
		 LET cCodRet = '000999';
      elif cCodRet <> '000' then
	     LET cCodRet = '000006';
         LET cMensajeRet     = 'Error al aplicar reverso '||cCodRet;
      end if;		      
   elif  nvl(vlReversado,'') = '' then 
      let cCodRet = '000005';
      let cMensajeRet = 'No existe folio a reversar';
   end if;		   
   LET cCodRet = lpad(trim(cCodRet),6,'0');  
   RETURN cCodRet,cMensajeRet; 
END
END PROCEDURE
DOCUMENT
'Se realiza sp para aplicar reverso de pagos  reciclador ',
'generales del crédito',
'AUTOR : Faviola Martínez',
'FECHA : Agosto 2013',
'BD    : BDICRED';

CREATE PROCEDURE "informix".sp_mon_buro_coninfoctestatus (pTiposolicitud CHAR(2), pNum_solicitud CHAR(20))
RETURNING
CHAR (5) 	AS Cod_ret,
CHAR (104)	AS Nombre,
CHAR (13)	AS RFC,
...
CHAR (40)	AS Status_solicitud;
----Declaración de variables----
DEFINE cCod_ret			 CHAR (5);  --- Código de retorno
DEFINE iSql_err			 INTEGER;   --- Error de informix
DEFINE cNom_cte			 CHAR (104);DEFINE cRfc				 CHAR (13); --- RFC del Cliente
DEFINE cNum_cte			 CHAR (20); --- Número de cliente
DEFINE cSucursal		 CHAR (4);  --- Número de Sucursal
DEFINE cNum_solicitud	 CHAR (20); --- Número de solicitud
DEFINE cNum_producto	 CHAR (45);  --- Código de producto
DEFINE cStatus_solicitud CHAR (40);  --- Estatus de solicitud
DEFINE cStatus			 CHAR (2); 	--- status de solicitud para validación de Estatus
DEFINE VSQL              CHAR(5000); ----Cadena que contiene la consulta a BD.
...
BEGIN
	...
	ON EXCEPTION SET iSql_err 
		IF iSql_err <> 0 THEN
			LET cCod_ret = iSql_err; ---- Error de informix (no controlado.)
		END IF;
		RETURN TRIM(cCod_ret), NVL(cNom_cte,''), NVL(cRfc,''), NVL(cNum_cte,''), NVL(cSucursal,''), NVL(cNum_solicitud,''), NVL(cNum_producto,''), NVL(cStatus_solicitud,'');
	END EXCEPTION
	
	--SET DEBUG FILE TO '/dbexportb/marioolivo/sp_mon_buro_coninfoctestatus.out';
	--TRACE ON;
	...
	IF NVL(ptiposolicitud,'') = '' AND NVL(pNum_solicitud,'') = '' THEN
		LET cCod_ret = '00001'; ----- Parametros Incorrectos.
		RETURN TRIM(cCod_ret), TRIM(NVL(cNom_cte,'')), NVL(cRfc,''), NVL(cNum_cte,''), NVL(cSucursal,''), NVL(cNum_solicitud,''), NVL(cNum_producto,''), NVL(cStatus_solicitud,'');
	END IF;
	
	
		LET VSQL =  ' ';
		LET VSQL = '  SELECT a.numcte, b.rfc, TRIM (b.nombre1)||" "|| TRIM(b.nombre2)||" "|| TRIM(b.apell_materno)||" "|| TRIM(b.apell_paterno), a.sucursal, a.num_solicitud, a.num_producto||"-"||d.nombre_prod, ';
				
		IF pTiposolicitud = '01' THEN
			LET VSQL = TRIM(VSQL) || ' a.status_solicitud||"-"||TRIM(c.descripcion), a.status_solicitud FROM bdisolic:"informix".ss_solicitudes a LEFT OUTER JOIN bdinteg:"informix".si_cliente b ON (b.numcte = a.numcte) LEFT OUTER JOIN bdisolic:"informix".ss_status_sol c ON(c.status_solicitud = a.status_solicitud) LEFT OUTER JOIN "informix".sd_definicion d ON (d.num_producto = a.num_producto) WHERE a.num_solicitud = "'||TRIM(pNum_solicitud)||'"  ';
		ELIF pTiposolicitud = '02' THEN
			LET VSQL = TRIM(VSQL) || ' a.status||"-"||TRIM(c.descripcion), a.status FROM "informix".sd_bitacora_aumlincred a LEFT OUTER JOIN bdinteg:"informix".si_cliente b ON (b.numcte = a.numcte) LEFT OUTER JOIN bdicred:"informix".sd_status_aumlincred c ON(c.status = a.status) LEFT OUTER JOIN "informix".sd_definicion d ON (d.num_producto = a.num_producto) WHERE a.fecha_status IN (select MAX(fecha_status) from "informix".sd_bitacora_aumlincred where num_solicitud = a.num_solicitud) AND  a.num_solicitud = "'||TRIM(pNum_solicitud)||'"    ';
		END IF;
	
		PREPARE xsql FROM TRIM(VSQL); 
		DECLARE xcur CURSOR FOR xsql; 
		OPEN xcur;		
			FETCH  xcur INTO cNum_cte, cRfc, cNom_cte, cSucursal, cNum_solicitud, cNum_producto, cStatus_solicitud, cStatus; 
		
		IF DBINFO('sqlca.sqlerrd2') = 0 THEN
				LET cCod_ret = '00002'; ---- No encontró información en la consulta.
				RETURN TRIM(cCod_ret), TRIM(NVL(cNom_cte,'')), NVL(cRfc,''), NVL(cNum_cte,''), NVL(cSucursal,''), NVL(cNum_solicitud,''), NVL(cNum_producto,''), NVL(cStatus_solicitud,'');
			END IF	
			
			WHILE  SQLCODE= 0 --Si encuentra registros el cursor
				RETURN TRIM(cCod_ret), TRIM(NVL(cNom_cte,'')), NVL(cRfc,''), NVL(cNum_cte,''), NVL(cSucursal,''), NVL(cNum_solicitud,''), NVL(cNum_producto,''), NVL(cStatus_solicitud,'') WITH RESUME;
...
END PROCEDURE
DOCUMENT
'DESCRIPCIÓN: RETORNA LA INFORMACIÓN DEL CLIENTE DANDO EL NUMERO DE SOLICITUD PARA LLENAR EN PANTALLA Y VERIFICA EL STATUS DEPENDIENDO SI ES DE SOLILICITUD DE CRÉDITO O INCREMENTO DE LÍNEA',
'FECHA DE CREACIÓN: 16-ABRIL-2013',
'BASE DE DATOS: BDICRED',
'CREADOR: MARIO GAMALIEL OLIVO URIAS',
'VERSION: 20130416.1511',
'DESCRIPCIÓN: SE ELIMINA VALIDACIÓN PARA PODER TRABAJAR CON TODOS LOS ESTATUS DE SOLICITUD Y ',
'             SE EJECUTA LA CONSULTA EN FORMA DINAMINA PARA UN MEJOR DESEMPEÑO',
'FECHA DE CREACIÓN: 15-MAYO-2013',
'BASE DE DATOS: BDICRED',
'MODIFICA: CARLOS OCHOA VALENZUELA',
'VERSION: 20130515.1539';

CREATE PROCEDURE "informix".sp_mon_buro_conscaterrores()

--RETORNOS-
RETURNING
CHAR(6)   AS cod_ret,
CHAR(100) AS descripcion_grupo,
CHAR(2)   AS cve_grupo;
...
BEGIN
	...
	ON EXCEPTION SET iSql_err 
		IF iSql_err <> 0 THEN
			...
			RETURN cCodret, TRIM(NVL(cDescripcion, '')),TRIM(NVL(cCve_grupo,''));
		END IF;
	END EXCEPTION;
		
	--SET DEBUG FILE TO '/dbexportb/carlos/buro/descripcion_grupo.out';
	--TRACE ON;
	...
	FOREACH
		...
		SELECT descripcion_grupo, cve_grupo 
		INTO cDescripcion, cCve_grupo
		FROM bdiburo:"informix".br_mon_buro_grupoerror
		WHERE cve_grupo = cve_grupo 
	
		RETURN cCodret, TRIM(cDescripcion),TRIM(cCve_grupo) WITH RESUME;
...
END PROCEDURE
DOCUMENT
'DESCRIPCIÓN: CONSULTA TODAS LAS DESCRIPCIONES DE GRUPOS DE LA TABLA br_mon_buro_grupoerror',
'             DE LA BASE DE DATOS BDIBURO.',
'FECHA DE CREACIÓN: 08 DE ABRIL DE 2013',
'BASE DE DATOS: BDICRED',
'CREADOR: CARLOS OCHOA VALENZUELA',
'VERSION: 2013';

CREATE PROCEDURE "informix".sp_consultaporanalistamc(pFechaInicial CHAR(10), pFechaFinal CHAR(10), pProducto CHAR(4))
RETURNING
	CHAR(6)  AS cCodRet,
