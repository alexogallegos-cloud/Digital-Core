...
CREATE PROCEDURE "informix".sp_verificastatusclientescoppel(pUsuario CHAR(8), pIdFuncion CHAR(10))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_verificastatusclientescoppel.out';
		--TRACE ON;
...
'DESCRIPCION: SPL encargado verificar el status de la consulta que obtiene el detalle de clientes Coppel.',
'BD: bdicnweb';
...
CREATE PROCEDURE "informix".sp_verificastatusinfprospectos(pUsuario CHAR(8), pIdFuncion CHAR(10))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_verificastatusinfprospectos.out';
		--TRACE ON;
...
'MODULO: Consultas',
'FUNCIONALIDAD: IdentificaciÃ³n de Prospectos',
'DESCRIPCION: SPL encargado verificar el status de la consulta que obtiene el detalle de prospectos.',
'BD: bdicnweb';
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: ActivaciÃ³n/DesactivaciÃ³n de Productos',
'DESCRIPCION: SPL que realiza la consulta de productos.',
'AUTOR: L. Montserrat LeÃ³n Amador',
'FECHA: 30/04/2020',
'DESCRIPCION: Se modifica SPL para eliminar llamado de la tabla si_ptf.',
'BD: bdicnweb';
		...
		FOREACH WITH HOLD 
			...
			IF NVL(iExisteFolio,0) > 0 THEN
				...
				IF (cCodTrans = '0001' AND cStatus = '01') OR (cCodTrans = '0010' AND cStatus = '01') OR (cCodTrans = '0036' AND cStatus IN ('12','01')) THEN
					...
					IF NVL(cDenominacion4, '') <> '' THEN
						IF dCantidadIng4 IS NOT NULL THEN
							LET dTotalBilletes = dTotalBilletes + (dCantidadIng4 * cDenominacion4);
						END IF;
					END IF;

					IF NVL(cDenominacion5, '') <> '' THEN
						IF dCantidadIng5 IS NOT NULL THEN
							LET dTotalBilletes = dTotalBilletes + (dCantidadIng5 * cDenominacion5);
						END IF;
					END IF;

					IF NVL(cDenominacion6, '') <> '' THEN
						IF dCantidadIng6 IS NOT NULL THEN
							LET dTotalBilletes = dTotalBilletes + (dCantidadIng6 * cDenominacion6);
						END IF;
					END IF;

					IF NVL(cDenominacion7, '') <> '' THEN
						IF dCantidadIng7 IS NOT NULL THEN
							LET dTotalBilletes = dTotalBilletes + (dCantidadIng7 * cDenominacion7);
						END IF;
					END IF;

					IF NVL(cDenominacion8, '') <> '' THEN
						IF dCantidadIng8 IS NOT NULL THEN
					...
					IF NVL(cDenominacion11, '') <> '' THEN
						IF dCantidadIng11 IS NOT NULL THEN
							LET dTotalMorralla = dTotalMorralla + (dCantidadIng11 * cDenominacion11);
						END IF;
					END IF;

					IF NVL(cDenominacion12, '') <> '' THEN
						IF dCantidadIng12 IS NOT NULL THEN
							LET dTotalMorralla = dTotalMorralla + (dCantidadIng12 * cDenominacion12);
						END IF;
					END IF;

					IF NVL(cDenominacion13, '') <> '' THEN
						IF dCantidadIng13 IS NOT NULL THEN
							LET dTotalMorralla = dTotalMorralla + (dCantidadIng13 * cDenominacion13);
						END IF;
					END IF;

					IF NVL(cDenominacion14, '') <> '' THEN
						IF dCantidadIng14 IS NOT NULL THEN
							LET dTotalMorralla = dTotalMorralla + (dCantidadIng14 * cDenominacion14);
						END IF;
					END IF;

					IF NVL(cDenominacion15, '') <> '' THEN
						IF dCantidadIng15 IS NOT NULL THEN
							LET dTotalMorralla = dTotalMorralla + (dCantidadIng15 * cDenominacion15);
						END IF;
					END IF;
...
DOCUMENT 'AUTOR: Oscar Flores Conde',
'FECHA: 11/02/2015',
'DESCRIPCION: Ejecuta en envio masivo de dotacion de caja general',
'AUTOR: L. Montserrat León Amador',
'FECHA: 30/03/2020',
'DESCRIPCION: Se modifica SPL eliminar notificación de correo electronico y obtener el número de dotaciones enviadas satisfactoriamente.',
'AUTOR: L. Montserrat León Amador',
'FECHA: 02/04/2020',
'DESCRIPCION: Se modifica SPL para corregir validación en denominaciones y aplicar commit cada 1000 registros procesados.',
'AUTOR: Rodolfo Conde Flores',
'FECHA: 13/04/2020',
'DESCRIPCION: Se modifica SPL para contabilizar de registros a procesar, de los proceados y no procesados.',
'AUTOR: L. Montserrat León Amador',
'FECHA: 16/04/2020',
'DESCRIPCION: Se modifica SPL para modificar validación en cantidades de la posición 8 a la 15 (morralla corresponde a la cantidad_7).',
'BD: bdicnweb';
...
'AUTOR: L. Montserrat León Amador',
'FECHA: 30/03/2020',
'DESCRIPCION: Se modifica SPL eliminar notificación de correo electronico y obtener el número de reverso enviados satisfactoriamente.',
'AUTOR: L. Montserrat León Amador',
'FECHA: 02/04/2020',
'DESCRIPCION: Se modifica SPL para aplicar commit cada 1000 registros procesados.',
'BD: bdicnweb';
...
CREATE PROCEDURE "informix".sp_enviardotacioncaja(pUsuario CHAR(8), pIdFuncion CHAR(10), pFolioOperacion CHAR(8), pStatus CHAR(2),
	...
	DEFINE fCantEnv_4 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_4 CHAR(18);
	DEFINE fCantEnv_5 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_5 CHAR(18);
	DEFINE fCantEnv_6 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_6 CHAR(18);
	DEFINE fCantEnv_7 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_7 CHAR(18);
	DEFINE fCantEnv_8 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_8 CHAR(18);
	DEFINE fCantEnv_9 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_9 CHAR(18);
	DEFINE fCantEnv_10 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_10 CHAR(18);
	DEFINE fCantEnv_11 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_11 CHAR(18);
	DEFINE fCantEnv_12 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_12 CHAR(18);
	DEFINE fCantEnv_13 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_13 CHAR(18);
	DEFINE fCantEnv_14 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_14 CHAR(18);
	DEFINE fCantEnv_15 FLOAT(8); --DECIMAL(10,2); 
	DEFINE cDenEnv_15 CHAR(18);
	...
	BEGIN
		...
		IF (pTotalDotEnviada = mTotalDotSolicitada) AND ((mTotalDotSolicitada < mSaldoDisponible) OR (mTotalDotSolicitada = mSaldoDisponible)) THEN
			...
			IF pStatus = '01' OR pStatus = '12' THEN
				...
				END IF;
				IF cDenEnv_4 = '-1' OR cDenEnv_4 = 'M' THEN
					LET fCantEnv_4 = fCantEnv_4 - NVL(pTotalEnviada4,0);
				ELSE 	
					LET fCantEnv_4 = fCantEnv_4 - NVL(pCantidadEnviada4,0);
				END IF;
				IF cDenEnv_5 = '-1' OR cDenEnv_5 = 'M' THEN
					LET fCantEnv_5 = fCantEnv_5 - NVL(pTotalEnviada5,0);
				ELSE 	
					LET fCantEnv_5 = fCantEnv_5 - NVL(pCantidadEnviada5,0);
				END IF;
				IF cDenEnv_6 = '-1' OR cDenEnv_6 = 'M' THEN
					LET fCantEnv_6 = fCantEnv_6 - NVL(pTotalEnviada6,0);
				ELSE 	
					LET fCantEnv_6 = fCantEnv_6 - NVL(pCantidadEnviada6,0);
				END IF;							
				IF cDenEnv_7 = '-1' OR cDenEnv_7 = 'M' THEN
				...
				END IF;
				IF cDenEnv_10 = '-1' OR cDenEnv_10 = 'M' THEN
					LET fCantEnv_10 = fCantEnv_10 - NVL(pMontoMorrallaEnv,0);
				END IF;
				IF cDenEnv_11 = '-1' OR cDenEnv_11 = 'M' THEN
					LET fCantEnv_11 = fCantEnv_11 - NVL(pMontoMorrallaEnv,0);
				END IF;	
				IF cDenEnv_12 = '-1' OR cDenEnv_12 = 'M' THEN
					LET fCantEnv_12 = fCantEnv_12 - NVL(pMontoMorrallaEnv,0);
				END IF;
				IF cDenEnv_13 = '-1' OR cDenEnv_13 = 'M' THEN
					LET fCantEnv_13 = fCantEnv_13 - NVL(pMontoMorrallaEnv,0);
				END IF;
				IF cDenEnv_14 = '-1' OR cDenEnv_14 = 'M' THEN
					LET fCantEnv_14 = fCantEnv_14 - NVL(pMontoMorrallaEnv,0);
				END IF;
				IF cDenEnv_15 = '-1' OR cDenEnv_15 = 'M' THEN
					LET fCantEnv_15 = fCantEnv_15 - NVL(pMontoMorrallaEnv,0);
				END IF;		
...
DOCUMENT 'AUTOR: L. Montserrat León Amador',
'FECHA: 06/02/2015',
'DESCRIPCION: SPL, que hace la actualización de los montos y de las cantidades diponibles, Envío Dotaciones Caja General',
'AUTOR: L. Montserrat León Amador',
'FECHA: 30/03/2020',
'DESCRIPCION: Se modifica SPL para cambio de tipo de datos de variables DECIMAL a FLOAT con respecto a tabla ss_operaciones.',
'BD: bdicnweb';
...
CREATE PROCEDURE "informix".sp_verificastatusdotaciones(pUsuario CHAR(8), pIdFuncion CHAR(10))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_verificastatusdotaciones.out';
		--TRACE ON;
...
'DESCRIPCION: SPL encargado verificar el status de la consulta que obtiene el detalle de las dotaciones.',
'BD: bdicnweb';
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Muestreo de Estados de Cuenta por Mesiversario', 
'DESCRIPCION: Spl encargado de consultar registros de cuentas mesiversario.',
'BD: bdicnweb',
'---------------------------------------------------------------------------',
'AUTOR: TASF S.A. DE C.V. - Julio MartÃ­nez Rugerio', 
'DESCRIPCION: Modificacion - Realizar consulta distintas numero de cuenta.',
'AUTOR: L. Montserrat LeÃ³n Amador',
'FECHA: 08/06/2020',
'DESCRIPCION: Se agrega validaciÃ³n de parÃ¡metros de entrada requeridos y se cambia campo de ordenamiento para recuperar los montos mÃ¡s altos (sdo_actual).';
...
CREATE PROCEDURE "informix".sp_dic_modactualizastatusalerta(pUsuario CHAR(8),pIdFuncion CHAR(10),pNumcte CHAR(20),pSucursal CHAR(4),pStatus CHAR(1),pFechaInsert DATE)
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_dic_modactualizastatusalerta.out';
		--TRACE ON;
...
CREATE PROCEDURE "informix".sp_seg_validacoordinador(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdEjecucion CHAR(1), pIdCoordinador CHAR(10), pPassword CHAR(50))
    RETURNING CHAR(5) AS codRet;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_seg_validacoordinador.out';
		--TRACE ON;
		...
		IF pIdEjecucion = '1' THEN
			...
			IF (SELECT COUNT(id_usuario) FROM bdicnweb:"informix".sw_seg_valida_usuarios WHERE id_usuario = pIdCoordinador AND pass_encrip = '0') = 1 THEN 
				LET cCodRet = '01148'; --PARA AUTENTICARSE POR CONTRASEÑA, ES NECESARIO SOLICITE AL ÁREA DE SEGURIDAD SU ENCRIPTACIÓN
				RETURN cCodRet;
			END IF;
		...
		ELIF pIdEjecucion = '2' THEN
			...
			IF (SELECT COUNT(id_usuario) FROM bdicnweb:"informix".sw_seg_valida_usuarios WHERE id_usuario = pIdCoordinador) = 1 THEN 	
				...
				FROM bdicnweb:"informix".sw_seg_valida_usuarios WHERE id_usuario = pIdCoordinador AND password_usuario = pPassword;
...
'AUTOR: L. Montserrat Leon Amador',
'FECHA: 06/05/2020',
'DESCRIPCION: SPL clon del procedimiento sp_sw_validacoordinador para la implementación en la funcionalidad Encriptación de Contraseñas.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consultareportepagoscre (pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaIni CHAR(10), pFechaFin CHAR(10), pStatus CHAR(1), pFolio CHAR(16), 
								pAplicacion SMALLINT, pConsulta SMALLINT, pProducto CHAR (4), pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
				CHAR(3)     As reg,
				CHAR(1)     As Secuencia,
				CHAR(30)    As DescrSecuencia,
				CHAR(20)    AS numCredito,
				CHAR(104)   AS nomcte,
				MONEY(18,2) AS importe,
				MONEY(18,2) AS capVigente,
				MONEY(18,2) AS capTransitorio,
				MONEY(18,2) AS capVencido,
				MONEY(18,2) AS capVdoNoexigible,
				MONEY(18,2) AS CapitalTotal,
				MONEY(18,2) AS intiVgente,
				MONEY(18,2) AS ivaIntvigente,
				MONEY(18,2) AS interesVencido,
				MONEY(18,2) AS ivaInteresvencido,
				MONEY(18,2) AS interesMoraBase,
				MONEY(18,2) AS interesMoraCopete,
				MONEY(18,2) AS ivaIntmoratorio,
				DATE        AS fechaMovimiento,
				CHAR(2)     AS codigoPago,
				CHAR(50)    AS descripCod_pago,
				CHAR(16)    AS folioGrupo,
				CHAR(16)    AS folio,
				CHAR(8)     AS usuario,
				CHAR(4)     AS sucursal,
				CHAR(50)    AS descripReverso,
				CHAR(4)     AS codigoProducto,
				CHAR(50)    AS descripcionPago,
				CHAR(4)     AS transaccion,
				MONEY(18,2) AS interesMoratorio,
				MONEY(18,2) AS ivaIntMorabase,
				MONEY(18,2) AS ivaIntMoracopete;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cReg, cSecuencia,cConcepto,cNumCredito, cNombreCte, m_ImportePago, mCapVigente, mCapTransitorio, mCapVencido, mCapVdoNoExigible, mCapitalTotal,
		...
		--SET DEBUG FILE TO '/tmp/sp_consultareportepagoscre.out';
		--TRACE ON;
		
		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pStatus = ''  OR pRegistros IS NULL OR pRecuperacion IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cReg, cSecuencia,cConcepto,cNumCredito, cNombreCte, m_ImportePago, mCapVigente, mCapTransitorio, mCapVencido, mCapVdoNoExigible, mCapitalTotal,
				mIntVigente, mIvaIntVigente, mInteresVencido, mIvaIntVencido, mIntMoraBase, mIntMoraCopete, mIvaIntMoratorio, dt_FechaMov, cCodigo, cDescripcionpag, 
				cfoliogrupo, cFolio, cUsuario, cSucursal, cDescRev, cCodProducto, c_DescriPago, cTransaccion, mIntMoratorio, mIvaIntMoraBase,mIvaIntMoraCopete;
		...
		FOREACH	EXECUTE PROCEDURE bdicred:'informix'.sp_consultarreportepagosspl(pFechaIni, pFechaFin, pStatus, pFolio, pAplicacion, pConsulta, pProducto)
			...
			ELIF iCodRetSp = 000003 THEN
				LET cCodRet = '00155';			--LA FECHA FINAL ES MAYOR A LA FECHA ACTUAL
				...
				IF iCodRetSp = 0 THEN
					IF iRegistros >= pRegistros THEN
						IF iRecuperacion < pRecuperacion THEN
							LET iRecuperacion = iRecuperacion + 1;
							RETURN cCodret, cReg, cSecuencia,cConcepto,cNumCredito, cNombreCte, m_ImportePago, mCapVigente, mCapTransitorio, mCapVencido, mCapVdoNoExigible, 
								mCapitalTotal, mIntVigente, mIvaIntVigente, mInteresVencido, mIvaIntVencido, mIntMoraBase, mIntMoraCopete, mIvaIntMoratorio, dt_FechaMov, cCodigo,
								cDescripcionpag, cfoliogrupo, cFolio, cUsuario, cSucursal, cDescRev, cCodProducto, c_DescriPago, cTransaccion, mIntMoratorio, mIvaIntMoraBase,
								mIvaIntMoraCopete WITH RESUME;
		...
		IF iNoRegs = 0 AND pRegistros = 0 THEN
			LET cCodRet = '00017';
			RETURN cCodret, cReg, cSecuencia,cConcepto,cNumCredito, cNombreCte, m_ImportePago, mCapVigente, mCapTransitorio, mCapVencido, mCapVdoNoExigible, mCapitalTotal, 
				mIntVigente, mIvaIntVigente, mInteresVencido, mIvaIntVencido, mIntMoraBase, mIntMoraCopete, mIvaIntMoratorio, dt_FechaMov, cCodigo,	cDescripcionpag, cfoliogrupo,
				cFolio, cUsuario, cSucursal, cDescRev, cCodProducto, c_DescriPago, cTransaccion, mIntMoratorio, mIvaIntMoraBase, mIvaIntMoraCopete;
...
'FECHA: 01/08/2014',
'DESCRIPCION: Genera Reporte de Pago a Cuenta.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_catalogotransaccionabonocap(pUsuario CHAR(10),pIdFuncion CHAR(10),pNaturaleza CHAR(1),pRegistros INTEGER, pRecuperacion INTEGER)
      RETURNING CHAR(5) AS codRet,
                CHAR(4) AS Clave_transaccion,
                CHAR(50) AS Desc_transaccion;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_catalogotransaccionabonocap.out';   
		--TRACE ON;
		
		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;

        IF  pUsuario = '' OR pIdFuncion = '' OR (pNaturaleza = '' OR pNaturaleza <> 'A') THEN
			LET cCodRet = '00003';
			RETURN  cCodRet,cClave_transaccion,cDesc_transaccion;
        END IF;
        ...
        FOREACH
                SELECT SKIP pRegistros FIRST pRecuperacion transacc, descripcion
                INTO cClave_transaccion, cDesc_transaccion
                FROM bdicheq:"informix".sc_tranval tv, bdinteg:"informix".si_transacc tr
                WHERE tv.empresa = cEmpresa
                AND tv.empresa = tr.empresa
                AND transacc = numero
		...
		IF iRecuperacion = 0 AND pRegistros = 0 THEN
			LET cCodRet = '01096'; --NO EXISTE INFORMACIÃâN PARA EL TIPO DE REPORTE SELECCIONADO, VERIFIQUE
			RETURN cCodRet,cClave_transaccion,cDesc_transaccion;
...
'FECHA 20/09/2016',
'MODULO: DEBIITO',
'FUNCIONALIDAD: DEPOSITO INDIVIDUAL',
'DESCRIPCION: SE MODIFICA PARA AGREGAR PARAMETROS DE PAGINACION Y DESPLEGAR TODAS LAS TRANSACCIONES EN LA FUNCIONALIDAD.',
'BD: bdicnweb',
...
'MODULO: DEBIITO',
'FUNCIONALIDAD: DEPOSITO INDIVIDUAL',
'DESCRIPCION: SE AGREGA CONTROL DE PAGINADO.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_mc_eliminasolicatenusuariomc_prueba(pUsuario CHAR(8), pIdFuncion CHAR(10))
RETURNING
        CHAR(5) AS COD_RET,
...
BEGIN
   ...
   ON EXCEPTION SET iSqlErr
	...
	SET DEBUG FILE TO "/tmp/mfinis/sp_mc_eliminasolicatenusuariomc.out";
	TRACE ON;
...
CREATE PROCEDURE "informix".sp_dic_registrositespcte(pUsuario CHAR(8), pIdFuncion CHAR(10), pCteNvo CHAR(20),   --CLIENTE NUEVO
																		...
																		pOrigMat CHAR(4),						--EMPRESA CON QUIEN SE HIZO MATCH EMPLEADOS, EXEMPLEADO,COPPEL, BANCOPPEL 
	...
	BEGIN
		...
		-- SET DEBUG FILE TO "/tmp/mfinis/sp_dic_registrositespcte.out";
		-- TRACE ON;
		
		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pCteNvo = '' OR pSitNvo = '' OR pCausaNvo IS NULL OR pCteMat = '' OR pOrigMat = '' OR pSucursal = '' OR pOperador = '' OR pFechaDictaINI = '' OR pFechaDictaFIN = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cMensaje;
		END IF;
		...
		EXECUTE PROCEDURE bdinteg:"informix".sp_dicta_registrositespcte(pCteNvo,pSitNvo,pCausaNvo,pCteMat,pSitMat,pCausaMat,pOrigMat,pSucursal,pOperador,pOrigen,pActivo,pBandera,pFechaDictaINI,pFechaDictaFIN,pNombreCoincidencia,pFechaNacimiento)
		INTO cCodRetSp, cMensaje;
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bdinteg:"informix".sp_dicta_registrositespcte';
		ELIF iCodRetSp = 1 OR iCodRetSp = 2 THEN
			LET cCodRet = '00003';
		ELIF iCodRetSp > 0 THEN
			LET cCodRet = CAST (iCodRetSp AS CHAR(5));
		END IF;
...
'DESCRIPCION: SPL que realiza el dictamen de de clientes',
'BD: bdicnweb',
'AUTOR: Johnattan Esquivel SÃ¡nchez',
'FECHA: 19/08/2020',
'MODULO: CLIENTES',
'FUNCIONALIDAD: MONITOR ALERTAS COMPARACION DE HUELLAS',
'DESCRIPCION: SPL que realiza ajuste para retornar codigos de errores controlados',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_reportepagosctasmasivocre(pIdUsuario CHAR(8), pIdFuncion CHAR(10), pIdFuncionPadre CHAR(10), pFechaInicio DATE, pFechaFin DATE, 
        ...
        DEFINE cCmd2 	 CHAR(5000);
        DEFINE cCmd3 	 CHAR(5000);
        DEFINE cCmd4 	 CHAR(5000);
        DEFINE cUser 	 CHAR(8);
		...
		DEFINE iContador  		INTEGER; --Contador para transaccion cada 1000 registros en cursor
        ...
        BEGIN
			...
			--SET DEBUG FILE TO '/tmp/mfinis/sp_reportepagosctasmasivocre.out';
			--TRACE ON;
			...
			LET cTmpTable = "DROP TABLE IF EXISTS tmpcapitalescta_"||iPid;
			...
			WHILE 1=1	
				...
				SET ISOLATION TO DIRTY READ;
				FOREACH	EXECUTE PROCEDURE bdicnweb:"informix".sp_consultareportepagoscre (pIdUsuario, pIdFuncionPadre, pFechaInicio, pFechaFin, 'N','','2','2','',iRegistrosRec,iRecuperacion)
					INTO cCodRetSp, cReg, cSecuencia,cConcepto,cNumCredito, cNombreCte, m_ImportePago, mCapVigente, mCapTransitorio, mCapVencido, mCapVdoNoExigible, mCapitalTotal,
							mIntVigente, mIvaIntVigente, mInteresVencido, mIvaIntVencido, mIntMoraBase, mIntMoraCopete, mIvaIntMoratorio, dt_FechaMov, cCodigo, cDescripcionpag, 
							cFolioGrupo, cFolio, cUsuario, cSucursal, cDescRev, cCodProducto, c_DescriPago, cTransaccion, mIntMoratorio, mIvaIntMoraBase,mIvaIntMoraCopete
			...
			WHILE 1=1	
				...
				SET ISOLATION TO DIRTY READ;
				FOREACH	EXECUTE PROCEDURE bdicnweb:"informix".sp_consultareportepagoscre (pIdUsuario, pIdFuncionPadre, pFechaInicio, pFechaFin, 'N','','2','4','',iRegistrosRec,iRecuperacion)
					INTO cCodRetSp, cReg, cSecuencia,cConcepto,cNumCredito, cNombreCte, m_ImportePago, mCapVigente, mCapTransitorio, mCapVencido, mCapVdoNoExigible, mCapitalTotal,
							mIntVigente, mIvaIntVigente, mInteresVencido, mIvaIntVencido, mIntMoraBase, mIntMoraCopete, mIvaIntMoratorio, dt_FechaMov, cCodigo, cDescripcionpag, 
							cFolioGrupo, cFolio, cUsuario, cSucursal, cDescRev, cCodProducto, c_DescriPago, cTransaccion, mIntMoratorio, mIvaIntMoraBase,mIvaIntMoraCopete
		...
		LET cCmd1=""||TRIM(cCmd1)||"   	   SELECT cm.id_registro, lote, (SELECT folio_grupo FROM bdicnweb:sw_tr_totales_masivo WHERE id_lote = cm.lote AND id_funcion = cm.id_funcion) AS folio_grupo, TRIM(NVL(cm.numcte, '')) AS numcte, NVL(trim(trim(trim(c.nombre1)||' '||TRIM(c.nombre2))||' '||trim(apell_paterno)||' '||TRIM(apell_materno)), '') AS nombre_cte, cm.cuenta, NVL(suc.sucursal||' '||TRIM(suc.nombre), '') AS sucursal, NVL(sp.num_producto||' '||sp.nombre_prod, '') AS producto, UPPER(NVL(sme.descripcion, '')) AS estatus_cuenta, NVL(TO_CHAR(bp.fecha_pago, '%d/%m/%Y'), '') AS fecha_ult_mov, cm.monto_importe, TRIM(NVL(cm.transaccion, '')) AS codigo_pago, TRIM(NVL(UPPER(cm.descripcion1), '')) AS concepto_pago, NVL(cm.resultado, '') AS resultado, NVL(cm.codret_proceso, '') AS codret_proceso, NVL(cm.motivo_rechazo, '') AS motivo_rechazo, nvl(trim(cm.folio), '') AS folio, TO_CHAR(DATE(cm.fecha_proceso), '%d/%m/%Y') AS fecha_aplicacion, TO_CHAR(DATE(cm.fecha_carga), '%d/%m/%Y') AS fecha_operacion, bp.saldo_ante_pago, bp.saldo_post_pago, UPPER(NVL(cm.descripcion2, '')) AS comentario, cm.usuario ";
		LET cCmd1=""||TRIM(cCmd1)||"   	   FROM ((((((bdicnweb:sw_tr_cargamasiva_pago cm LEFT JOIN bdinteg:si_cliente c ON c.numcte = cm.numcte) LEFT JOIN bdicred:sd_maecred mc ON mc.num_credito = cm.cuenta) LEFT JOIN bdinteg:si_sucursales suc ON suc.sucursal = mc.sucursal) LEFT JOIN bdicred:sd_definicion sp ON sp.num_producto = mc.num_producto) LEFT JOIN bdicred:sd_tipocartera sme ON sme.status_cred = mc.status_cred) LEFT JOIN bdinteg:si_transacc st ON st.sistema = '06' AND st.numero = cm.transaccion) LEFT JOIN bdicred:sd_bitacora_pagos bp ON bp.num_credito = cm.cuenta and bp.folio = cm.folio WHERE id_funcion = 'CPC102' AND DATE(cm.fecha_carga) BETWEEN '"||pFechaInicio||"' AND '"||pFechaFin||"'";
		LET cCmd1=""||TRIM(cCmd1)||"   	   UNION ";
		LET cCmd1=""||TRIM(cCmd1)||"   	   SELECT cm.id_registro, lote, (SELECT folio_grupo FROM bdicnweb:sw_tr_totales_masivo WHERE id_lote = cm.lote AND id_funcion = cm.id_funcion) AS folio_grupo, TRIM(NVL(cm.numcte, '')) AS numcte, NVL(trim(trim(trim(c.nombre1)||' '||TRIM(c.nombre2))||' '||trim(apell_paterno)||' '||TRIM(apell_materno)), '') AS nombre_cte, cm.cuenta, NVL(suc.sucursal||' '||TRIM(suc.nombre), '') AS sucursal, NVL(sp.num_producto||' '||sp.nombre_prod, '') AS producto, UPPER(NVL(sme.descripcion, '')) AS estatus_cuenta, nvl(TO_CHAR(bp.fecha_pago, '%d/%m/%Y'), '') AS fecha_ult_mov, cm.monto_importe, TRIM(NVL(cm.transaccion, '')) AS codigo_pago, TRIM(NVL(UPPER(cm.descripcion1), '')) AS concepto_pago, NVL(cm.resultado, '') AS resultado, NVL(cm.codret_proceso, '') AS codret_proceso, NVL(cm.motivo_rechazo, '') AS motivo_rechazo, nvl(trim(cm.folio), '') AS folio, to_char(DATE(cm.fecha_proceso), '%d/%m/%Y') AS fecha_aplicacion, TO_CHAR(DATE(cm.fecha_carga), '%d/%m/%Y') AS fecha_operacion, bp.saldo_ante_pago, bp.saldo_post_pago, UPPER(NVL(cm.descripcion2, '')) AS comentario, cm.usuario";
		LET cCmd1=""||TRIM(cCmd1)||"   	   FROM ((((((bdicnweb:sw_tr_cargamasiva_pago_hist cm LEFT JOIN bdinteg:si_cliente c ON c.numcte = cm.numcte) LEFT JOIN bdicred:sd_maecred mc ON mc.num_credito = cm.cuenta) LEFT JOIN bdinteg:si_sucursales suc ON suc.sucursal = mc.sucursal) LEFT JOIN bdicred:sd_definicion sp ON sp.num_producto = mc.num_producto) LEFT JOIN bdicred:sd_tipocartera sme ON sme.status_cred = mc.status_cred) LEFT JOIN bdinteg:si_transacc st ON st.sistema = '06' AND st.numero = cm.transaccion) LEFT JOIN bdicred:sd_bitacora_pagos bp ON bp.num_credito = cm.cuenta AND bp.folio = cm.folio WHERE id_funcion = 'CPC102' AND DATE(cm.fecha_carga) BETWEEN '"||pFechaInicio||"' AND '"||pFechaFin||"'";
		...
		LET cCmd1 = "SELECT ";
		LET cCmd1=""||TRIM(cCmd1)||" tmp1.lote, tmp1.folio_grupo, tmp1.numcte, tmp1.nombre_cte, tmp1.cuenta , tmp2.cReg ,tmp2.mImportePagoCondona,tmp2.mCapVigSdoAnterior,";
		...
		LET cCmd1=""||TRIM(cCmd1)||" tmp2.mIvaIntMoraSdoAnterior,tmp2.mIvaIntMoraImpAplicado,tmp2.mIvaIntMoraSdoNuevo,tmp1.sucursal,TRIM(tmp1.producto),TRIM(tmp1.estatus_cuenta),tmp1.fecha_ult_mov,tmp1.codigo_pago,";
		LET cCmd1=""||TRIM(cCmd1)||" tmp1.concepto_pago,TRIM(tmp1.resultado),tmp1.codret_proceso,tmp1.motivo_rechazo,tmp1.folio,tmp1.fecha_aplicacion,tmp1.fecha_operacion,TRIM(tmp1.comentario), tmp1.usuario";
		LET cCmd1=""||TRIM(cCmd1)||" FROM ( ";
		LET cCmd1=""||TRIM(cCmd1)||"   	   SELECT cm.id_registro, lote, (SELECT folio_grupo FROM bdicnweb:sw_tr_totales_masivo WHERE id_lote = cm.lote AND id_funcion = cm.id_funcion) AS folio_grupo, TRIM(NVL(cm.numcte, '')) AS numcte, NVL(trim(trim(trim(c.nombre1)||' '||TRIM(c.nombre2))||' '||trim(apell_paterno)||' '||TRIM(apell_materno)), '') AS nombre_cte, cm.cuenta, NVL(suc.sucursal||' '||TRIM(suc.nombre), '') AS sucursal, NVL(sp.num_producto||' '||sp.nombre_prod, '') AS producto, UPPER(NVL(sme.descripcion, '')) AS estatus_cuenta, NVL(TO_CHAR(bp.fecha_pago, '%d/%m/%Y'), '') AS fecha_ult_mov, cm.monto_importe, TRIM(NVL(cm.transaccion, '')) AS codigo_pago, TRIM(NVL(UPPER(cm.descripcion1), '')) AS concepto_pago, NVL(cm.resultado, '') AS resultado, NVL(cm.codret_proceso, '') AS codret_proceso, NVL(cm.motivo_rechazo, '') AS motivo_rechazo, nvl(trim(cm.folio), '') AS folio, TO_CHAR(DATE(cm.fecha_proceso), '%d/%m/%Y') AS fecha_aplicacion, TO_CHAR(DATE(cm.fecha_carga), '%d/%m/%Y') AS fecha_operacion, bp.saldo_ante_pago, bp.saldo_post_pago, UPPER(NVL(cm.descripcion2, '')) AS comentario, cm.usuario ";
		LET cCmd1=""||TRIM(cCmd1)||"   	   FROM ((((((bdicnweb:sw_tr_cargamasiva_pago cm LEFT JOIN bdinteg:si_cliente c ON c.numcte = cm.numcte) LEFT JOIN bdicred:sd_maecred mc ON mc.num_credito = cm.cuenta) LEFT JOIN bdinteg:si_sucursales suc ON suc.sucursal = mc.sucursal) LEFT JOIN bdicred:sd_definicion sp ON sp.num_producto = mc.num_producto) LEFT JOIN bdicred:sd_tipocartera sme ON sme.status_cred = mc.status_cred) LEFT JOIN bdinteg:si_transacc st ON st.sistema = '06' AND st.numero = cm.transaccion) LEFT JOIN bdicred:sd_bitacora_pagos bp ON bp.num_credito = cm.cuenta and bp.folio = cm.folio WHERE id_funcion = 'CPC102' AND DATE(cm.fecha_carga) BETWEEN '"||pFechaInicio||"' AND '"||pFechaFin||"'";
		LET cCmd1=""||TRIM(cCmd1)||"   	   UNION ";
...
DOCUMENT "AUTOR: Oscar Flores Conde",
"FECHA: 09/08/2013",
"DESCRIPCION: GeneraciÃÂ³n del reporte de los pagos a cuentas masivo de la aplicaciÃÂ³n CNWEB",
"FECHA: 15/01/2014",
...
"AUTOR: L. Montserrat LeÃ³n Amador",
"FECHA: 19/01/2017",
"DESCRIPCION: Se eliminan las declaraciones DROP PROCEDURE;SET EXPLAIN ON;SET PDQPRIORITY 0;SET OPTIMIZATION HIGH; al momento de crear la tabla temporal tmpcapitalescta_.",
"AUTOR: L. Montserrat LeÃ³n Amador",
"FECHA: 06/08/2020",
"DESCRIPCION: Se trabaja exception -958.",
"BD: bdicnweb";

CREATE PROCEDURE "informix".sp_cnsif_consultasaldoapoyo(pUsuario CHAR(8),pIdFuncion CHAR(10),pNumCredito CHAR(20),pNumCte CHAR(20))
	RETURNING CHAR(5) AS codret,
		DECIMAL(14,2) AS saldo_apoyo;
	...
	DEFINE cMensaje CHAR(100);
	DEFINE cEmpresa CHAR(3);
	DEFINE dSaldoApoyo DECIMAL(14,2);
	DEFINE iNoRegistros INTEGER;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,dSaldoApoyo;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cnsif_consultasaldoapoyo.out';
		--TRACE ON;
		...
		EXECUTE PROCEDURE bdicred:"informix".sp_consulta_sdo_apoyo(pNumCredito,pNumCte) 
		INTO cCodRetSp,dSaldoApoyo,cMensaje;
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION cCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bdicred:sp_consulta_sdo_apoyo';
		ELIF iCodRetSp = 1 THEN
			LET cCodRet = '01202'; --EL CREDITO NO EXISTE
		ELIF iCodRetSp = 2 THEN
			LET cCodRet = '01203'; --EL CREDITO NO ESTA EN EL PROGRAMA DE APOYO
		ELIF iCodRetSp = 3 THEN
			LET cCodRet = '01204'; --EL PRODUCTO NO ESTA EN EL PROGRAMA DE APOYO
		ELIF iCodRetSp = 4 THEN
			LET cCodRet = '01205'; --EL CLIENTE NO ESTA EN EL PROGRAMA DE APOYO
...
'FECHA: 09/07/2020',
'MODULO: CONSULTAS',
'FUNCIONALIDAD: CONSULTA GENERALIZADA DE CUENTAS DE CRÉDITO',
'DESCRIPCION: SPL encargado de obtener el saldo del programa de apoyo de tdc.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_cnsif_validaplanapoyo(pUsuario CHAR(8),pIdFuncion CHAR(10),pNumCte CHAR(20))
	RETURNING CHAR(5) AS codret,
		CHAR(1) AS plan_apoyo;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cPlanApoyo;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cnsif_validaplanapoyo.out';
		--TRACE ON;
		...
		FROM bdicred:"informix".sd_diferir
		WHERE numcte = pNumCte 
		AND canal_baja IS NULL OR canal_baja = '';
...
'MODULO: CONSULTAS',
'FUNCIONALIDAD: CONSULTA GENERALIZADA DE CUENTAS DE CRÉDITO',
'DESCRIPCION: SPL encargado de consultar si el cliente presenta el plan de apoyo activo.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_seg_comboperfilappmovil (pUsuario CHAR(8), pIdFuncion CHAR(10))
	RETURNING CHAR(5)		AS codigo_retorno,
			  SMALLINT		AS codParam,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
				LET cCodRet = iSqlErr;
				RETURN cCodRet, cCodParam, cDescripcion;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_seg_comboperfilappmovil.out';
		--TRACE ON;
		...
		FOREACH
			...
			SELECT cod_param, descripcion
			INTO cCodParam, cDescripcion
			FROM bdinteg:"informix".si_param_movil
			WHERE valor = 'amov'
			
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet, cCodParam, cDescripcion WITH RESUME;
...
'FECHA: 18/05/2020',
'MODULO: Seguridad',
'FUNCIONALIDAD: REPORTERÃA ALTA MÃVIL', 
'DESCRIPCION: SPL encargado de llenar el combo Perfil',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_seg_comboreporteappmovil (pUsuario CHAR(8), pIdFuncion CHAR(10))
	RETURNING CHAR(5)		AS codigo_retorno,
			  CHAR(1)		AS clave_reporte,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
				LET cCodRet = iSqlErr;
				RETURN cCodRet, cClaveReporte, cDescripcion;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_seg_comboreporteappmovil.out';
		--TRACE ON;
		...
		FOREACH
			...
			SELECT clave_reporte, descripcion 
			INTO cClaveReporte, cDescripcion
			FROM bdicnweb:"informix".sw_rptappmovil
			WHERE status = '1'
			
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet, cClaveReporte, cDescripcion WITH RESUME;
...
'MODULO: Seguridad',
'FUNCIONALIDAD: REPORTERÃA ALTA MÃVIL', 
'DESCRIPCION: SPL encargado de llenar el combo Reporte App Movil',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_seg_generareportesappmovil(pUsuario CHAR(8), pIdFuncion CHAR(10), pReporte CHAR(1), pInicio DATE, pFin DATE, pEjecutivo CHAR(20), pRuta CHAR(100))
	RETURNING CHAR(5)	AS codigo_retorno,
	...
	DEFINE cPerfil CHAR(20);
	DEFINE cIp CHAR(15);
	DEFINE cFecha_alta CHAR(10);
	DEFINE cUltimo_acceso CHAR(10);
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_seg_generareportesappmovil.out';
		--TRACE ON;
		...
		-- Se validan los parametros de entrada.
		IF NVL(pUsuario,'') = '' OR NVL(pIdFuncion,'') = '' OR NVL(pReporte,'') = '' OR NVL(pRuta,'') = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet,cNombreArchivo;
		END IF;
		
		-- Se validan los parametros de entrada.
		IF pReporte = 1 AND (pInicio IS NULL OR pFin IS NULL ) THEN
			LET cCodRet = '00003';
			RETURN cCodRet,cNombreArchivo;
		END IF;
		...
		DELETE FROM bdicnweb:"informix".sw_rpt_consultappmovil
		...
		IF pReporte = 1 THEN
			-- SE DEFINE NOMENCLATURA DEL REPORTE POR ACCIONES
			LET pRuta = TRIM(pRuta) || '/';
			LET cReporte = 'REPORTE_MOVIL_ACCIONES_' || TO_CHAR(CURRENT, '%Y%m%d');
			LET cRutaGral = TRIM(pRuta)||TRIM(cReporte)||'.txt';
			LET cNombreArchivo = TRIM(cReporte)||'.txt';
			FOREACH
				EXECUTE PROCEDURE bdinteg:"informix".sp_reporte_accesos_amov(pEjecutivo, pInicio, pFin)
				INTO cCodRetSp, cEjecutivo, cNombre, cFecha_alta, cHora,  cIp, cAccion
				...
				ELIF cCodRetSp = '00000' THEN
					...
					INSERT INTO bdicnweb:"informix".sw_rpt_consultappmovil(consecutivo, ejecutivo, nombre, ip, fecha_alta, hora, accion, usuario_insert) 
					VALUES(iTotalRegistros, cEjecutivo, cNombre, cIp, cFecha_alta, cHora, cAccion, pUsuario);
		...
		ELIF pReporte = 3 THEN
			-- SE DEFINE NOMENCLATURA DEL REPORTE POR USUARIOS
			LET pRuta = TRIM(pRuta) || '/';
			LET cReporte = 'REPORTE_MOVIL_USUARIOS_' || TO_CHAR(CURRENT, '%Y%m%d');
			LET cRutaGral = TRIM(pRuta)||TRIM(cReporte)||'.txt';
			LET cNombreArchivo = TRIM(cReporte)||'.txt';
			FOREACH
				EXECUTE PROCEDURE bdinteg:"informix".sp_reporte_usuarios_amov(pEjecutivo)
				INTO cCodRetSp, cEjecutivo, cNombre, cPerfil, cIp, cFecha_alta, cUltimo_acceso
				...
				ELIF cCodRetSp = '00000' THEN
					...
					INSERT INTO bdicnweb:"informix".sw_rpt_consultappmovil(consecutivo, ejecutivo, nombre, perfil, ip, fecha_alta, ultimo_acceso, usuario_insert) 
					VALUES(iTotalRegistros, cEjecutivo, cNombre, cPerfil, cIp, cFecha_alta, cUltimo_acceso, pUsuario);
		...
		ELIF pReporte = 2 THEN
			-- SE DEFINE NOMENCLATURA DEL REPORTE POR PERFILES
			LET pRuta = TRIM(pRuta) || '/';
			LET cReporte = 'REPORTE_MOVIL_PERFILES_' || TO_CHAR(CURRENT, '%Y%m%d');
			LET cRutaGral = TRIM(pRuta)||TRIM(cReporte)||'.txt';
			LET cNombreArchivo = TRIM(cReporte)||'.txt';
			FOREACH
				EXECUTE PROCEDURE bdinteg:"informix".sp_perfiles_amov()
				INTO cCodRetSp, cPerfil, cDescripcion, cFecha_alta
				
				INSERT INTO bdicnweb:"informix".sw_rpt_consultappmovil(consecutivo, perfil, fecha_alta, descripcion, usuario_insert) 
				VALUES(iTotalRegistros, cPerfil, cFecha_alta, cDescripcion, pUsuario);
		...
		BEGIN WORK;
			...
			IF pReporte = 1 THEN
				-- SE DEFINE NOMENCLATURA DEL REPORTE POR ACCIONES
				LET cCmd1 = "SELECT 'NO_EMPLEADO','NOMBRE','FECHA','HORA','IP','ACCION' FROM systables WHERE tabid = 1";
				
				IF pEjecutivo <> '' THEN
					LET cCmd1 = "" || TRIM(cCmd1)|| " UNION ALL SELECT ejecutivo, nombre, fecha_alta, hora, ip, accion FROM bdicnweb:""informix"".sw_rpt_consultappmovil ";
					LET cCmd1 = "" || TRIM(cCmd1)|| " WHERE usuario_insert = '"|| pUsuario ||"' AND ejecutivo = '"|| pEjecutivo ||"' AND fecha_alta BETWEEN '"||pInicio||"' AND '"||pFin||"' ";
				ELSE
					LET cCmd1 = "" || TRIM(cCmd1)|| " UNION ALL SELECT ejecutivo, nombre, fecha_alta, hora, ip, accion FROM bdicnweb:""informix"".sw_rpt_consultappmovil ";
					LET cCmd1 = "" || TRIM(cCmd1)|| " WHERE usuario_insert = '"|| pUsuario ||"' AND fecha_alta BETWEEN '"||pInicio||"' AND '"||pFin||"' ";
				END IF;
					
			ELIF pReporte = 3 THEN
				-- SE DEFINE NOMENCLATURA DEL REPORTE POR USUARIOS
				LET cCmd1 = "SELECT 'NO_EMPLEADO','NOMBRE','PERFIL','IP','FECHA_ALTA','ULTIMO_ACCESO' FROM systables WHERE tabid = 1";
				IF pEjecutivo <> '' THEN
					LET cCmd1 = "" || TRIM(cCmd1)|| " UNION ALL SELECT ejecutivo, nombre, perfil, ip, fecha_alta, ultimo_acceso FROM bdicnweb:""informix"".sw_rpt_consultappmovil ";
					LET cCmd1 = "" || TRIM(cCmd1)|| " WHERE usuario_insert = '"|| pUsuario ||"' AND ejecutivo = '"|| pEjecutivo ||"' ";
				ELSE
					LET cCmd1 = "" || TRIM(cCmd1)|| " UNION ALL SELECT ejecutivo, nombre, perfil, ip, fecha_alta, ultimo_acceso FROM bdicnweb:""informix"".sw_rpt_consultappmovil ";
					LET cCmd1 = "" || TRIM(cCmd1)|| " WHERE usuario_insert = '"|| pUsuario ||"' ";
				END IF;
			ELIF pReporte = 2 THEN
				LET cCmd1 = "SELECT 'PERFIL','FUNCIONALIDAD','FECHA_ALTA' FROM systables WHERE tabid = 1";
				LET cCmd1 = "" || TRIM(cCmd1)|| " UNION ALL SELECT perfil, descripcion, fecha_alta FROM bdicnweb:""informix"".sw_rpt_consultappmovil ";
				LET cCmd1 = "" || TRIM(cCmd1)|| " WHERE usuario_insert = '"|| pUsuario ||"' ";
			END IF;
...
'MODULO: Seguridad',
'FUNCIONALIDAD: REPORTERÃA ALTA MÃVIL', 
'DESCRIPCION: SPL encargado de Generar Reporte App Movil',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_sw_consultainfocatarula(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumProducto CHAR(4))
		RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		IF pUsuario = '' OR pIdFuncion = '' OR pNumProducto = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cNumProducto, cRangoPromedio, cTasasCete, cGatNominal, cGatReal;
		END IF;
...
CREATE PROCEDURE "informix".sp_fc_bitacorahuellas(pUsuario CHAR(8),pIdFuncion CHAR(10),pNumCteCorr CHAR(20),pNumCteInc CHAR(20),pMensaje CHAR(2),pUsEjecuta CHAR(8))
	RETURNING CHAR(5) AS codret;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_fc_bitacorahuellas.out';
		--TRACE ON;
		...
		EXECUTE PROCEDURE bdinteg:"informix".sp_bithuellasfusion(pNumCteCorr,pNumCteInc,pMensaje,pUsEjecuta) 
		INTO cCodRetSp;
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION cCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bdinteg:sp_bithuellasfusion';
		END IF;
...
'DESCRIPCION: SPL encargado de realizar el registro en la bitÃ¡cora de huellas.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_fc_bloqueactas(pUsuario CHAR(8),pIdFuncion CHAR(10),pCteTitular CHAR(20),pUsEjecuta CHAR(8))
	RETURNING CHAR(5) AS codret,
		CHAR(6) AS codret_sp,
		CHAR(6) AS codret_sp2,
		CHAR(6) AS codret_sp3;
	
	DEFINE cCodRet CHAR(5);
	DEFINE iSqlErr INTEGER;
	DEFINE cCodRetSp CHAR(6);
	DEFINE cCodRetSp2 CHAR(6);
	DEFINE cCodRetSp3 CHAR(6);
	DEFINE iCodRetSp INTEGER;
	DEFINE cDescCodRetSp CHAR(100);
	DEFINE cEmpresa CHAR(3);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cCodRetSp,cCodRetSp2,cCodRetSp3;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_fc_bloqueactas.out';
		--TRACE ON;
		...
		EXECUTE PROCEDURE bdinteg:"informix".sp_bloqueactas(pCteTitular,pUsEjecuta) 
		INTO cCodRetSp,cCodRetSp2,cCodRetSp3;
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION cCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bdinteg:sp_bloqueactas';
		END IF;
...
'MODULO: CLIENTES',
'FUNCIONALIDAD: FUSION MANUAL DE CLIENTES',
'DESCRIPCION: SPL encargado de bloquear las cuentas que esten activas o los creditos que esten activos.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_fc_conscliente(pUsuario CHAR(8),pIdFuncion CHAR(10),pNumCte CHAR(20))
	RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cApellPaterno,cApellMaterno,cNombre,cNombre2,cSucursal,cRFC,cStatusCte,dFechaAlta,dFechaNac,cSexo;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_fc_conscliente.out';
		--TRACE ON;
		...
		EXECUTE PROCEDURE bdinteg:"informix".sp_fuscte_conscte(pNumCte) 
		INTO cCodRetSp,cApellPaterno,cApellMaterno,cNombre,cNombre2,cSucursal,cRFC,cStatusCte,dFechaAlta,dFechaNac,cSexo;
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION cCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bdinteg:sp_fuscte_conscte';
		ELIF iCodRetSp = 100 THEN
			LET cCodRet = '01150'; --NO. DE CLIENTE INVÃLIDO
		ELIF iCodRetSp = 200 THEN
			LET cCodRet = '01151'; --LONGITUD INVÃLIDA, CORRESPONDIENTE AL NO. DE CLIENTE
		ELIF iCodRetSp = 300 THEN
			LET cCodRet = '01152'; --NO. DE CLIENTE NO EXISTE
		ELIF iCodRetSp = 400 THEN
			LET cCodRet = '01153'; --NO. DE CLIENTE CANCELADO
		ELIF iCodRetSp = 500 THEN
			LET cCodRet = '01154'; --SIN DATOS PERSONALES DEL CLIENTE []
		ELIF iCodRetSp = 600 THEN
			LET cCodRet = '01155'; --CLIENTE FUSIONADO []
		ELIF iCodRetSp = 700 THEN
			LET cCodRet = '01156'; --CLIENTE [] ES UN CLIENTE MORAL
		END IF;
...
'MODULO: CLIENTES',
'FUNCIONALIDAD: FUSION MANUAL DE CLIENTES',
'DESCRIPCION: SPL encargado de consultar los datos del cliente.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_fc_consctesdesb(pUsuario CHAR(8),pIdFuncion CHAR(10),pNumCte CHAR(20), pAnalista CHAR(20))
	RETURNING CHAR(5) AS codret,
		CHAR(26) AS apell_paterno, 
		CHAR(26) AS apell_materno, 
		CHAR(26) AS nombre, 
		...
		DATE AS fecha_nac, 
		CHAR(1) AS sexo,
		CHAR(26) AS apell_paterno_ctei, 
		CHAR(26) AS apell_materno_ctei, 
		CHAR(26) AS nombre_ctei, 
		CHAR(26) AS nombre2_ctei, 
		CHAR(4) AS sucursal_ctei, 
		CHAR(13) AS rfc_ctei, 
		CHAR(2) AS status_cte_ctei, 
		DATE AS fecha_alta_ctei,  
		DATE AS fecha_nac_ctei, 
		CHAR(1) AS sexo_ctei,
		CHAR(20) AS numcte_ctei;
	...
	DEFINE dFechaNac DATE;
	DEFINE cSexo CHAR(1);
	DEFINE cApellPaterno_CteI CHAR(26);
	DEFINE cApellMaterno_CteI CHAR(26);
	DEFINE cNombre_CteI CHAR(26);
	DEFINE cNombre2_CteI CHAR(26);
	DEFINE cSucursal_CteI CHAR(4);
	DEFINE cRFC_CteI CHAR(13);
	DEFINE cStatusCte_CteI CHAR(2);
	DEFINE dFechaAlta_CteI DATE;
	DEFINE dFechaNac_CteI DATE;
	DEFINE cSexo_CteI CHAR(1);
	DEFINE cNumCte_CteI CHAR(20);
	DEFINE iNoRegistros INTEGER;
	...
	LET dFechaNac = '';
	LET cSexo = '';
	LET cApellPaterno_CteI = '';
	LET cApellMaterno_CteI = '';
	LET cNombre_CteI = '';
	LET cNombre2_CteI = '';
	LET cSucursal_CteI = '';
	LET cRFC_CteI = '';
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cApellPaterno,cApellMaterno,cNombre,cNombre2,cSucursal,cRFC,cStatusCte,dFechaAlta,dFechaNac,cSexo,
			cApellPaterno_CteI,cApellMaterno_CteI,cNombre_CteI,cNombre2_CteI,cSucursal_CteI,cRFC_CteI,cStatusCte_CteI,dFechaAlta_CteI,dFechaNac_CteI,cSexo_CteI,cNumCte_CteI;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_fc_consctesdesb.out';
		--TRACE ON;
		...
		EXECUTE PROCEDURE bdinteg:"informix".sp_desbctasfus_conscte(pNumCte,pAnalista) 
		INTO cCodRetSp,cApellPaterno,cApellMaterno,cNombre,cNombre2,cSucursal,cRFC,cStatusCte,dFechaAlta,dFechaNac,cSexo,
		cApellPaterno_CteI,cApellMaterno_CteI,cNombre_CteI,cNombre2_CteI,cSucursal_CteI,cRFC_CteI,cStatusCte_CteI,dFechaAlta_CteI,dFechaNac_CteI,cSexo_CteI,cNumCte_CteI;
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION cCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bdinteg:sp_desbctasfus_conscte';
		ELIF iCodRetSp = 1 THEN
			LET cCodRet = '01180'; --NO SE ENCUENTRAN DATOS CON EL NO. DE ANALISTA A CONSULTAR
		ELIF iCodRetSp = 100 THEN
			LET cCodRet = '01150'; --NO. DE CLIENTE INVÃLIDO
		ELIF iCodRetSp = 200 THEN
			LET cCodRet = '01151'; --LONGITUD INVÃLIDA, CORRESPONDIENTE AL NO. DE CLIENTE
		ELIF iCodRetSp = 300 THEN
			LET cCodRet = '01181'; --NO SE ENCUENTRAN DATOS CON EL NO. CLIENTE A LOCALIZAR
		ELIF iCodRetSp = 400 THEN
			LET cCodRet = '01182'; --NO. DE CLIENTE CANCELADO
		ELIF iCodRetSp = 500 THEN
			LET cCodRet = '01183'; --SIN DATOS PERSONALES DEL CLIENTE
		ELIF iCodRetSp = 700 THEN
			LET cCodRet = '01184'; --EL NO. DE CLIENTE ES UN CLIENTE MORAL
		ELIF iCodRetSp <> 0 THEN
			LET cCodRet = '01185'; --EL NO. DE CLIENTE NO EXISTE
		END IF;
...
'DESCRIPCION: SPL encargado de consultar los datos personales del cliente.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_fc_conslocalctesnom(pUsuario CHAR(8),pIdFuncion CHAR(10),
pNombre1 CHAR(26),pNombre2 CHAR(26),pApPaterno CHAR(26),pApMaterno CHAR(26),pFecha DATE,
pRegistros INTEGER,pRecuperacion INTEGER)
	RETURNING CHAR(5) AS codret,
		CHAR(20) AS numcte, 
		CHAR(26) AS nombre1, 
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cNumcte,cNombre1,cNombre2,cApell_paterno,cApell_materno,cRfc,dFecha_nac;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_fc_conslocalctesnom.out';
		--TRACE ON;
		...
		END IF;
		LET pApPaterno = TRIM(pApPaterno)||'%';
		IF pApMaterno <> '' THEN
			LET pApMaterno = TRIM(pApMaterno)||'%';
		END IF;
		...
		FOREACH
			SELECT {+INDEX (bdinteg:si_cliente inx_si_cliente3)} {+INDEX (bdinteg:si_ctepf 221_466)} --LIMIT 100 
			SKIP pRegistros FIRST pRecuperacion
			cte.numcte,cte.nombre1,cte.nombre2,cte.apell_paterno,cte.apell_materno,cte.rfc,ctepf.fecha_nac 
			INTO cNumcte,cNombre1,cNombre2,cApell_paterno,cApell_materno,cRfc,dFecha_nac
			FROM bdinteg:"informix".si_cliente cte INNER JOIN bdinteg:"informix".si_ctepf ctepf ON cte.numcte = ctepf.numcte
			WHERE cte.nombre1 LIKE pNombre1
			AND cte.nombre2 LIKE (CASE WHEN pNombre2 <> '' THEN pNombre2 ELSE cte.nombre2 END)
			AND cte.apell_paterno LIKE pApPaterno
			AND cte.apell_materno LIKE (CASE WHEN pApMaterno <> '' THEN pApMaterno ELSE cte.apell_materno END)
			AND ctepf.fecha_nac = (CASE WHEN pFecha IS NOT NULL THEN pFecha ELSE ctepf.fecha_nac END)
			ORDER BY cte.apell_paterno ASC
			
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet,cNumcte,cNombre1,cNombre2,cApell_paterno,cApell_materno,cRfc,dFecha_nac WITH RESUME;
		END FOREACH;
		
		IF pRegistros = 0 AND iRecuperacion = 0 THEN
			LET cCodRet = '01176'; --NO SE ENCONTRÃ NINGÃN CLIENTE CON LOS DATOS CAPTURADOS
			RETURN cCodRet,cNumcte,cNombre1,cNombre2,cApell_paterno,cApell_materno,cRfc,dFecha_nac;
		ELIF pRegistros > 0 AND iRecuperacion = 0 THEN
			LET cCodRet = '1001';
			RETURN cCodRet,cNumcte,cNombre1,cNombre2,cApell_paterno,cApell_materno,cRfc,dFecha_nac;
...
'MODULO: CLIENTES',
'FUNCIONALIDAD: FUSION MANUAL DE CLIENTES',
'DESCRIPCION: SPL encargado de realizar la busqueda por nombre y obtener el detalle de la consulta local de clientes.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_fc_conslocalctesrs(pUsuario CHAR(8),pIdFuncion CHAR(10),
pRazonSoc CHAR(60),pRegistros INTEGER,pRecuperacion INTEGER)
	RETURNING CHAR(5) AS codret,
		CHAR(20) AS numcte, 
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cNumcte,cRazon_social,cRfc;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_fc_conslocalctesrs.out';
		--TRACE ON;
		...
		FOREACH
			SELECT  
			SKIP pRegistros FIRST pRecuperacion
			numcte,razon_social,rfc 
			INTO cNumcte,cRazon_social,cRfc
			FROM bdinteg:"informix".si_cliente
			WHERE razon_social LIKE pRazonSoc
			ORDER BY razon_social ASC
			
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet,cNumcte,cRazon_social,cRfc WITH RESUME;
		END FOREACH;
...
'MODULO: CLIENTES',
'FUNCIONALIDAD: FUSION MANUAL DE CLIENTES',
'DESCRIPCION: SPL encargado de realizar la busqueda por razon social y obtener el detalle de la consulta local de clientes.',
'BD: bdicnweb';
