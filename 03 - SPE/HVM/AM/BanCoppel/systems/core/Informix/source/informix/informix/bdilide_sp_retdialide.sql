CREATE PROCEDURE "informix".sp_retdialide(pEmpresa CHAR(3), dFecha DATE, cUsuario CHAR(10))
RETURNING VARCHAR(4), VARCHAR(80), CHAR(11);
    ...
    DEFINE vcStatusAcumMen      INTEGER;
    DEFINE vcStatusExtOpeDeb    INTEGER;
    DEFINE vcStatusExtOpeCrd    INTEGER;
    ...
    LET vdUltimoDiaMes    = '';
    LET vcStatusAcumMen	  = 0;
    LET vcStatusExtOpeDeb = 0;
    LET vcStatusExtOpeCrd = 0;
    LET vsdo_cta          = 0.00;
    LET vsdo_ret          = 0.00;
    LET vsdo_cong         = 0.00;
    ...
    ON EXCEPTION SET SQL_ERR, ISAM_ERR, ERROR_INFO
        SET DEBUG FILE TO "/resplogifx/conciliachq/sp_retdialide.err";
        TRACE ON;
    ...
    IF dFecha = vdUltimoDiaMes THEN
        ...
        IF vcStatusAcumMen <= 0 THEN
            RETURN "333", '', ''; --- "33333" No se ejecuto el proceso anterior o que fallo en la ejecucion
        END IF;
    ELIF dFecha <> vdUltimoDiaMes THEN
        /* ##############################################################################################################################
        -- // Si No es fin de mes entonces vamos a validar que los procesos extoptar_d y extoptar_c se hayan ejecutado anteriormete.
        SELECT COUNT(*)
          INTO vcStatusExtOpeDeb
          FROM bdilide:sl_procesos
         WHERE fech_proceso = dFecha
           AND proceso = "extoptar_d";
           
        SELECT COUNT(*)
          INTO vcStatusExtOpeCrd
          FROM bdilide:sl_procesos
         WHERE fech_proceso = dFecha
           AND proceso = "extoptar_c";

        IF ( vcStatusExtOpeDeb <= 0 OR vcStatusExtOpeCrd <= 0 ) THEN
            RETURN "222", '', ''; --- "22222" No se ha ejecutado los procesos de extracciï¿½n de operaciones de debito y credito
        END IF;
        ############################################################################################################################## */
    END IF;
    ...
    FOREACH WITH HOLD
        ...
        IF vmImporteCargo > 0 THEN
            ...
            IF TRIM(vcCodRetTemp) = "000" THEN
                ...
                VALUES
                (vcAnioMes, cNumcte, viConsecutivo, vcRfc, vcRefRet, cCuenta, dFecha, vmImporteCargo, cUsuario, CURRENT::DATE);
