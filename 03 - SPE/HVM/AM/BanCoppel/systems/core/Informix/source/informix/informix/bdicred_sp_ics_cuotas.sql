CREATE PROCEDURE "informix".sp_ics_cuotas(p_num_ejecucion INTEGER) 
	...
	--Variables interfaz
		...
		DEFINE v_contador_amortiza							INTEGER;
		DEFINE fecha_inicio									DATE;
		---Variable para columna v_capital_status
		DEFINE v_capital_status								CHAR(2);
		DEFINE v_contador_cuotas							INTEGER;
  ...
  --------Iniciar las nuevas variables
		...
		LET v_contador_cuotas								= NULL;
        LET v_transaccion									= 0;
		LET v_proceso 										='';                      

	--SET DEBUG FILE TO "/resplogifx/info_ics/cuotas_V2.out";
    --TRACE ON; 
	...
	BEGIN
		...
		ON EXCEPTION IN (-535)
			  ...
			  LET v_transaccion = 1;
			  INSERT INTO "informix".ics_control_errores(num_credito, numcte, num_producto,descripcion_error, proceso, fecha_insert)
			  VALUES( v_account_number, v_num_cliente, v_acc_customer_id, 'ERROR -535 Coutas',v_proceso, CURRENT);
			    COMMIT WORK;
				BEGIN WORK;
		...
		INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (iContador1, horaActual, 'IN_CICLO_CUOTAS'); 

---------------------------
		BEGIN WORK;
			
			LET v_transaccion = 1;
			LET v_proceso = 'Creditos Revolventes Cuotas';
			...
			FOREACH WITH HOLD
				...
				FROM bdicred:ics_obligacion
					WHERE account_number = v_account_number; */--Se comenta ya que se sacara el valor delin de cada couta
					--Last_payment_date
				...
				FROM sd_amortiza_credito 
					...
					--LET delinquency = /*v_cap_trans +*/ v_cap_vdo_exig + dIntVdo + dIntMoratorio + dIvaIntVdo + dIvaIntMoratorio + v_com_pend + v_iva_com;
						LET v_delinquency = v_capital_debe_2 + v_interes_debe_2 + v_iva_debe + v_interes_mora + v_iva_interes_mora;					--Principal_delinquent
			...
			--Se a̱ade para que solo tome en cuentes los estatus 1
			--Se a̱ade condicion para que tome solo las cuotas con capital estatus 3 y se tenga una cuota minima registrada por cada obligacion
				...
				AND fecha_cuota  <= add_months(c_fecha_ejecucion, 1);
				...
				SELECT COUNT(*) INTO v_contador_cuotas FROM ics_cuotas WHERE account_number = v_account_number;
			
				--Se a̱ade condicion para que tome solo las cuotas con capital estatus 3 y se tenga una cuota minima registrada por cada obligacion
				IF v_contador_cuotas = 0 OR v_contador_cuotas IS NULL OR v_contador_cuotas = '' THEN
					...
					IF fecha_inicio IS NOT NULL THEN 
						...
						FOREACH WITH HOLD
							...
							LET v_paid_interest_delinquent = 0.0;
							LET v_principal_delinquency = 0.0; --ajustado
							LET v_paid_delinquency = 0.0;
							LET v_interest_delinquent = 0.0; --ajustado
							LET v_update_date = 0.0;
							LET v_last_entrance_date = 0.0;
							LET v_state_cause_id = 'UPDATE';
							LET v_condicion_adeudo = 0.0;
							LET v_paid_principal_delinquency = 0.0;
...
--							Last_payment_date
						...
						LET v_delinquency = '0';					--Principal_delinquent
				...
				--"""Se realiza consulta con estatus 3
				SELECT COUNT(*) INTO v_contador_cuotas FROM ics_cuotas WHERE account_number = v_account_number;
...
--							Last_payment_date
			...
			---======================== Se agregan validaciones a la tabla sd__amortiza_credito_apoyo2021
					SELECT MIN(fecha_cuota)
						INTO fecha_inicio
					FROM sd_amortiza_credito_apoyo2021 
					WHERE num_credito = v_account_number
						AND capital_status in (2,6,7) ;
					...
					--Se obtiene la sucursal
						...
						--sum(((NVL(mora_provi_ordi,0) + NVL(mora_provi_cope,0) + NVL(mora_sdo_ordi,0) - NVL(mora_sdo_ordi_pag,0) + NVL(mora_sdo_cope,0) - NVL(mora_sdo_cope_pag,0)) * 0.16 )) v_iva_interes_mora
							INTO v_capital_debe_2, v_interes_debe_2, v_iva_debe, v_interes_mora--, v_v_iva_interes_mora
						FROM sd_amortiza_credito_apoyo2021 a
						WHERE a.empresa   = '001'
							AND a.num_credito = v_account_number
						AND capital_status= v_capital_status AND fecha_cuota = v_due_date and num_pago = v_num_pago;
					    --
...
--							Last_payment_date
			...
			----========= Concluye la incluicion de la tabla sd_amortiza_credito_apoyo2021
		...
		SELECT DBINFO("utc_to_datetime", sh_curtime) 
			INTO horaActual 
		FROM sysmaster:sysshmvals;
		INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (iContador1, horaActual, 'FIN_CICLO_CUOTA');
