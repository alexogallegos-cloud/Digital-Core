CREATE PROCEDURE "informix".pasecont1(pempresa   CHAR(3),
                          fecha_pase DATE,
                          pusuario   CHAR(8))
...
CREATE PROCEDURE "informix".cont_int_mora(interes_moraord7   DECIMAL(14,2), 
                               interes_moracop7   DECIMAL(14,2),
                               interes_moraord2   DECIMAL(14,2), 
                               interes_moracop2   DECIMAL(14,2),
                               p_fecha_proceso    DATE,
                               v_evento_cap       SMALLINT)
       RETURNING CHAR(5);
   ...
   DEFINE v7_provi_ordi     DECIMAL(14,2);
   DEFINE v7_provi_cope     DECIMAL(14,2);
   DEFINE v2_provi_ordi     DECIMAL(14,2);
   DEFINE v2_provi_cope     DECIMAL(14,2);
   DEFINE v7_estado_proc    CHAR(1);
   DEFINE v2_estado_proc    CHAR(1);
   ...
   IF p_fecha_proceso = g_ult_dia_mes OR v_evento_cap <> 0 THEN
      LET v7_provi_ordi     = 0;
      LET v7_provi_cope     = 0;
      SELECT   SUM(provi_mora_ordi), SUM(provi_mora_cope)
      INTO   v7_provi_ordi,     v7_provi_cope
      FROM sd_detmora
      WHERE num_credito = g_num_credito  
        AND empresa = g_empresa
        AND identifi_rec = "P" ;

      IF v7_estado_proc IS NULL THEN
         LET interes_moraord7 = 0;
         LET interes_moracop7 = 0;
      ELSE
         LET interes_moraord7 = v7_provi_ordi;
         LET interes_moracop7 = v7_provi_cope;
      END IF; 

      LET v2_provi_ordi     = 0;
      LET v2_provi_cope     = 0;
      --SELECT estado_prov,   SUM(provi_mora_ordi), SUM(provi_mora_cope)
      --INTO   v2_estado_proc, v2_provi_ordi,     v2_provi_cope
      --FROM sd_detmora
      ...
      IF v2_estado_proc IS NULL THEN
         LET interes_moraord2 = 0;
         LET interes_moracop2 = 0;
      ELSE
         LET interes_moraord2 = v2_provi_ordi;
         LET interes_moracop2 = v2_provi_cope;
      END IF; 
   ...
   -- #########################################################   
   -- ###   Provision Diaria o Mensual de Moratorios      #####
   -- #########################################################    
 
   -- Provision de intereses depues del vencimiento 
   -- y penales en status 7
   IF g_monto_vencido > 0 AND g_es_fisica = "S" THEN
      LET wcodigo_ref = 12; -- 6620 provision moras en 7 pf 
   END IF;
         IF g_monto_vencido > 0 AND g_es_fisica = "N" THEN
            LET wcodigo_ref = 23;  -- 6619 provision moras en 7 pm
   END IF;
   
   LET v_monto = 0;
   LET v_monto = interes_moraord7  + interes_moracop7;
   IF v_monto > 0 THEN
   ...
   -- Provision de intereses depues del vencimiento y 
   -- penales en status 2
   IF g_mto_venc_trasp > 0 AND g_es_fisica = "S" THEN
      LET wcodigo_ref = 26; -- provision moras en 2 pf  6620
   END IF;
   IF g_mto_venc_trasp > 0 AND g_es_fisica = "N" THEN
      LET wcodigo_ref = 27;  -- provision moras en 2 pm  6619
   END IF;
   
   LET v_monto = 0;
   LET v_monto = interes_moraord2 + interes_moracop2;
   IF v_monto > 0 THEN
...
DOCUMENT 
" Se modifica el programa para no contabilizar el calculo de intereses",
" moratorios ",
...
CREATE PROCEDURE "informix".traspaso_int(p_fecha_proceso DATE)
   ...
   DEFINE GLOBAL g_valor40            CHAR(50)       DEFAULT " ";
   --DIAS TRASPASO REVOLVENTES. INT. MENSUAL
   ...
   DEFINE v_mto_mora_trasp   MONEY(14,2);
   ...
   DEFINE v_dias_trasp_int   SMALLINT;
   DEFINE v_tp_linea         CHAR(1);

   ON EXCEPTION SET sql_err, isam_err, error_info
      LET v_codret = sql_err;
      SET DEBUG FILE TO "Traspaso_Int.err";
      TRACE sql_err||" * "||isam_err|| " * "||error_info;
   ...
   LET v_hora_c2        = v_hora_c1;
   LET v_folio_suc      = TRIM(v_usuario) || TRIM(v_hora_c2);
   LET v_dias_trasp_int = 0;
   LET v_tp_linea       = " ";


   SELECT tp_linea
   INTO v_tp_linea
   FROM lineas:sl_ctepro
   WHERE num_linea = g_num_linea and
         numcte    = g_numcte    and
         --num_gpo   = v_num_gpo   and
         producto  = g_num_producto;


   --int = 3 mensual
   IF g_tipo_calculo  = "04" THEN                 -- revolvente c/capitaliza
      LET v_dias_trasp_int   = g_valor40;
   ELSE
      IF v_tp_linea = "R" THEN                    -- revolvente s/capitaliza
         LET v_dias_trasp_int   = g_valor40;
      ELSE                                        -- no revolvente
         IF g_period_pago_int = "3" THEN          -- periodicidad mensual
            LET v_dias_trasp_int   = g_valor38;
         ELSE                                     -- periodicidad no mensual
            LET v_dias_trasp_int   = g_valor39;
         END IF;
      END IF;
   END IF;
   ...
   --######################################################
   --###     Funcion del Cierre de Credito Traspasos   ####
   --######################################################
   FOREACH WITH HOLD
      ...
      AND  ((fecha_cuota        >  g_fecha_ant   AND
            ...
            (p_fecha_proceso   -    fecha_cuota >= v_dias_trasp_int))
      ...
      --===================================================================
      --=== TRASPASO DE INTERES A VENCIDO NO TRASPASADO status de 1 a 7 ===
      --===================================================================
      IF gi_status_cuota = "1" THEN

         -- ##########################################################
         -- #### TRASPASO A CARTERA VENCIDA POR EFECTO DE TENER
         -- #### CUOTA(S) CON STATUS 2 en sd_paginter PASA DE 1 A 2
         -- ##########################################################
         ...
         IF v_cartera_asoc  = 0 THEN   --traspaso normal de interes de 1 a 7
            LET v_transacc_suc     = "3i17";

            --###########################################################
            --###### ACTUALIZA EL STATUS DE LA CUOTA sd_paginter  ######
            --###########################################################
            UPDATE sd_paginter
            SET  status_cuota     = "7"
            ...
            SET sdo_no_exig      = sdo_no_exig      - v_monto,
                ...
                mto_venc_int     = mto_venc_int     + v_monto
            WHERE num_credito    = g_num_credito
            AND empresa          = g_empresa;
            ...
            -- #################################################################
            -- ### GENERACION DE MOVIMIENTO CONTABLE TRASPASO INTERES        ###
            -- #################################################################
            IF (gi_cuota_rec = "6") OR (gi_cuota_rec = "8") THEN
               LET v_codigo_ref = 5;  -- traspaso int. prop/reno a vdo.transito
                                      -- 6661
            ELSE
               LET v_codigo_ref = 4;   -- traspaso int. vig. a vdo.transitorio
                                       -- 6661
            END IF;
         ...
         ELSE  -- traspaso de interes cartera asociada de 1 a 2
               -- con bandera ya prendida con anterioridad o
               -- bandera prendida en proc  campo_trab3 = "E"
            ...
            IF v_cartera_asoc = 1 AND g_campo_trab3  = "E" THEN
               ...
               -- ####################################################
               -- ###  MOVIMIENTO CONTABLE TRASPASO INTERES        ###
               -- ####################################################
               IF (gi_cuota_rec = "6") OR (gi_cuota_rec = "8") THEN
                  IF g_es_fisica = "S" THEN
                     LET v_codigo_ref = 30;  -- traspaso int. prop/reno
                                             -- a vdo.exigible p.f.
                                             -- 6605
                  ELSE
                     LET v_codigo_ref = 31;  -- traspaso int. prop/reno
                                             -- a vdo.exigible p.m.
                                             -- 6606
                  END IF;
               ELSE
                  IF g_es_fisica = "S" THEN
                     LET v_codigo_ref = 24;  -- traspaso int. vig.
                                             -- a vdo.exigible p.f.
                                             -- 6605
                  ELSE
                     LET v_codigo_ref = 25;  -- traspaso int. vig.
                                             -- a vdo.exigible p.m.
                                             -- 6606
                  END IF;
               END IF;
            ...
            IF v_cartera_asoc = 1 AND g_campo_trab3  != "E" THEN
               ...
               -- ### asociada por traspaso de interes               ###
               -- ######################################################
               UPDATE sd_maecred
               SET campo_trab3 = "E"
               WHERE num_credito    = g_num_credito
               ...
               LET g_campo_trab3  = "E";
               -- ########################################################
               -- #### traspaso de CAPITAL por efecto de la cartera asociada
               -- ########################################################
               CALL TraCap_por_Int(p_fecha_proceso)
               RETURNING v_codret;
      ...
      IF gi_status_cuota = "7" THEN
         IF v_cartera_asoc = 0 THEN
            IF p_fecha_proceso - gi_fecha_cuota  >= v_dias_trasp_int THEN
               ...
               IF (gi_cuota_rec = "6") OR (gi_cuota_rec = "8") THEN
                  LET v_codigo_ref = 7 ;
                                       --traspaso int pro/ren vdo.transitorio a
                                       --vencido contabilizado 6662
               ELSE
                  LET v_codigo_ref = 6;
                                       --traspaso int vdo.transitorio a vencido
                                       --  6662
               END IF ;
               ...
               --############################################################
               LET vt_cont_cuotas  = 0;
               SELECT COUNT(*), SUM(monto_cuota-monto_real_pag)
               INTO vt_cont_cuotas, vt_monto_asocia
               FROM sd_paginter
               WHERE num_credito = gi_num_credito  AND
                     empresa     = g_empresa       AND
                     status_cuota = "7";
               ...
               ELSE
                  ...
                  --###########################################################
                  UPDATE sd_maesdos
                  SET mto_venc_int     = mto_venc_int - vt_monto_asocia,
                      mto_venc_tra_int = mto_venc_tra_int + vt_monto_asocia
                  WHERE num_credito    = g_num_credito
                  AND empresa          = g_empresa;

                  LET g_mto_venc_int     = g_mto_venc_int - vt_monto_asocia;
                  LET g_mto_venc_tra_int = g_mto_venc_tra_int+vt_monto_asocia;
                  ...
                  -- #########################################################
                  IF (gi_cuota_rec = "6") OR (gi_cuota_rec = "8") THEN
                     LET v_codigo_ref = 7; --tras int pro/ren vdo.transitorio a
                                           --vencido contabilizado 6664
                  ELSE
                     LET v_codigo_ref = 6; --tras int vdo.transitorio a vencido
                                           --6662
                  END IF ;

                  LET v_monto         = 0;
                  LET v_monto         = vt_monto_asocia;
                  IF v_monto != 0 THEN
                     CALL genmov(g_empresa,gi_num_credito, g_num_producto,
                                 v_codigo_ref,     g_codigo_fun,
                                 g_fecha_hoy,      v_monto,     v_folio_suc,
   ...
   END FOREACH
      ...
      IF v_nocuotas_en_7 = 1 and vt_reg_x_cred >= 1 THEN
         -- ######################################################
         -- ### Traspaso de una sola cuota de interes
         -- ######################################################
         UPDATE sd_maecred
         SET campo_trab3 = "E"
         WHERE num_credito    = g_num_credito
...
DOCUMENT
"Spl para el traspaso a cartera vencida de los intereses y efecto de ",
"cartera asociada ",
...
CREATE PROCEDURE "informix".traspa_antic(p_fecha_proceso DATE)
       RETURNING CHAR(5), CHAR(20);
   ...
   DEFINE GLOBAL  gt_num_credito      CHAR(20)       DEFAULT " ";
   DEFINE GLOBAL  gt_fecha_trasp      DATE           DEFAULT " ";
   DEFINE GLOBAL  gt_status_trasp     CHAR(1)        DEFAULT " ";
   DEFINE GLOBAL  gt_ejecutivo        CHAR(8)        DEFAULT " ";
   DEFINE GLOBAL  gt_causa            CHAR(40)       DEFAULT " ";

   DEFINE GLOBAL g_codigo_fun         CHAR(3)        DEFAULT "034";

   DEFINE v_codret           CHAR(5);
   DEFINE v_proceso          CHAR(3);
   ...
   ON EXCEPTION SET sql_err, isam_err, error_info
      LET v_codret = sql_err;
      SET DEBUG FILE TO "Traspa_Antic.err";
      TRACE sql_err||" * "||isam_err|| " * "||error_info;
      RETURN v_codret, g_num_credito;
   END EXCEPTION;
   ...
   SELECT num_credito,      fecha_trasp,
          status_trasp,     ejecutivo,
          causa
   INTO   gt_num_credito,   gt_fecha_trasp,
          gt_status_trasp,  gt_ejecutivo,
          gt_causa
   FROM   sd_marcatrasp
   WHERE  num_credito = g_num_credito AND 
          empresa     = g_empresa     AND
          fecha_trasp = p_fecha_proceso;


    IF gt_num_credito IS NULL THEN
      LET v_codret  = "300" ;
      RETURN v_codret, g_num_credito;
   END IF;
   ...
   -- TRASPASO A CARTERA VENCIDA DE INTERES CON ASOCIADA
   CALL TraInt_por_Cap(p_fecha_proceso)
   RETURNING v_codret;

   IF (v_codret <> "000") THEN
      LET v_proceso = "310";
      INSERT INTO sd_bitacora
      VALUES (g_empresa,g_num_credito, v_codret, v_proceso, p_fecha_proceso);
   END IF;

   -- TRASPASO A CARTERA VENCIDA DE CAPITAL CON ASOCIADA
   CALL TraCap_por_Int(p_fecha_proceso)
   RETURNING v_codret;
   ...
   UPDATE sd_marcatrasp
   SET status_trasp = "A"
   WHERE  num_credito = gt_num_credito AND
          empresa     = g_empresa      AND
          fecha_trasp = p_fecha_proceso;
...
END PROCEDURE
DOCUMENT
"Esta funcion realiza el traspaso anticipado a cartera vencida",
"AUTOR : Jose Cruz Narvaez",
"FECHA : 2/Mayo/2001",
"Ver.  : 1.0",
"BD.   : bdicred",
