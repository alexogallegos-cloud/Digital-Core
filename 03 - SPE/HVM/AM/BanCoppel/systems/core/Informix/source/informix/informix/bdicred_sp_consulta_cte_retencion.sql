CREATE PROCEDURE "informix".sp_consulta_cte_retencion(pNumCte CHAR(9))			 
RETURNING 
	CHAR(5) AS codRet,
	CHAR(9) AS numCte,
	CHAR(12) AS numCredito,
	CHAR(16) AS folio,
	CHAR(1) AS semaforo;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr, iIsamErr
			IF iSqlErr <> 0 THEN
				LET cCodRet = iSqlErr;				
				RETURN cCodRet,cNumCte,cNumCredito,cFolio,cSemaforo;
			END IF;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/sp_consulta_cte_retencion.out';
		--TRACE ON;
		...
		SELECT sdcteret.numcte, sdcteret.num_credito,sdcteret.folio,sdcteret.semaforo 
		INTO cNumCte,cNumCredito, cFolio, cSemaforo
		FROM bdicred:sd_ctes_retencion sdcteret
		WHERE sdcteret.numcte = pNumCte;
		
		IF dbinfo('sqlca.sqlerrd2') = 0 THEN
			LET cCodRet = '00002';
		END IF;
		
		RETURN cCodRet,NVL(cNumCte,''),NVL(cNumCredito,''),NVL(cFolio,''),NVL(cSemaforo,'');
	END;
END PROCEDURE
DOCUMENT
'AUTOR: Alejandro Cordova Ramirez',
'DESCRIPCION: Realiza una consulta y retornar la informaciÃ³n de la tabla sd_ctes_retencion', 
'Codigo de retorno 00001 indica que se ha enviado parametros de entrada invalido', 
'Codigo de retorno 00002 indica que no se encontraron datos en la tabla sd_ctes_retencion ',
