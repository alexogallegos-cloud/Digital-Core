CREATE PROCEDURE "informix".sp_compara_tel_ref(pEmpresa CHAR(3), pNumCteTitular CHAR(20),pNumSolicitud CHAR(20), 
pSecuencia CHAR(20), pTelefonoCasa CHAR(13), pTelefonoCel CHAR(13),pTelefonoOfi CHAR(13), pEjecucion CHAR(1))
RETURNING CHAR(5);
...
DEFINE iRegistrosRef	INTEGER;

--SET DEBUG FILE TO '/tmp/sp_compara_tel_ref.out';
--TRACE ON;
		...
		IF iRegistros <> 0 THEN
			IF TRIM(NVL(pSecuencia,'')) <> '' AND CAST(TRIM(NVL(pSecuencia,'0')) AS INTEGER) > 0 THEN
				...
				IF EXISTS (SELECT a.numcte FROM "informix".si_refclientes AS a, "informix".si_refdirecciones AS b WHERE a.numcte = b.numcte AND a.secuencia = b.secuencia
						AND a.numcte_banco=b.numcte_banco AND a.empresa=pEmpresa AND a.numcte = pNumCteTitular AND a.num_solicitud = cNumSolicitud	AND a.secuencia = CAST(pSecuencia AS INTEGER)) THEN
					LET iExistsReg = 1;
				END IF;
			...
			ELIF iValidar = 2 THEN
				...
				ELIF CAST(TRIM(NVL(pEjecucion,'0')) AS INTEGER) = 2 THEN
					...
					ELSE
						LET iValidar= 0;
                        IF EXISTS (SELECT numcte FROM si_solicitud_movil WHERE numcte=pNumCteTitular 
                                    AND (num_tdc_coppel=pNumSolicitud OR num_tdc_bcoppel=pNumSolicitud)   
                                    AND folio_procesado='0')THEN
                        
                            DELETE FROM si_refdirecciones WHERE numcte=pNumCteTitular
                              AND secuencia IN (SELECT secuencia FROM si_refclientes 
                                 WHERE numcte=pNumCteTitular AND num_solicitud=pNumSolicitud);
                                    
                            DELETE FROM si_refclientes WHERE numcte=pNumCteTitular AND num_solicitud=pNumSolicitud;
                            LET cCodret = '00000';
                        ELSE
                            LET cCodret = '00003';
                        END IF;
				...
				IF TRIM(NVL(cSecuencia,'')) <> ''  AND CAST(TRIM(NVL(cSecuencia,'0')) AS INTEGER) > 0 THEN
					...
					SELECT count(numcte) INTO iRegistros FROM bdisolic:"informix".ss_refpersonales WHERE numcte =pNumCteTitular AND num_solicitud = cNumSolicitud AND telefono_ref IN (cTelefonoRef1,cTelefonoRef2,cTelefonoRef3) AND numcte_ref like 'R3%';
					SELECT count(numcte) INTO iRegistrosRef FROM bdisolic:"informix".ss_refpersonales WHERE numcte =pNumCteTitular AND num_solicitud = cNumSolicitud AND telefono_ref IN (cTelefonoRef1,cTelefonoRef2,cTelefonoRef3) AND numcte_ref IN ('R1','R2');

					IF NVL(iRegistros,0) >= 1 AND iRegistrosRef = 0 THEN
						LET cCodret = '00000';
						RETURN cCodret;	
					END IF;						
...
DOCUMENT
'AUTOR         : Felipe Urias',
'DESCRIPCION   : se realizo este Sp para la validacion de los telefonos de las referencias, se realizan comparasiones entre los numeros',
'de las referencias de una solicitud',
