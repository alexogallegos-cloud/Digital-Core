CREATE PROCEDURE "informix".cons_expediente_huella(pEmpresa CHAR(3),
												  pCliente CHAR(20),
												  pNum_regs SMALLINT)                
	RETURNING CHAR(5) 	AS Codret,
			  CHAR(20) 	AS cCuenta,
			  CHAR(40) 	AS cProd_nombre,
			  CHAR(4) 	AS cCod_docto,
			  DATE 		AS dFecha_alta,
			  CHAR(3) 	AS cCod_grupo,
			  CHAR(30)	AS cDescrip_gpo,
			  CHAR(35) 	AS cDescrip_docto,
			  CHAR(30) 	AS cDescrip2,
			  CHAR(1) 	AS cMulti_img,
			  SMALLINT 	AS siSecuencia,
			  CHAR(1) 	AS cIma_esnula;
...
-- SET DEBUG FILE TO "/home/tmp/oscar/cons_expediente_huella.out";
-- TRACE ON;
...
BEGIN
   ...
   END EXCEPTION;
    ...
    FOREACH
        ...
        SELECT  DISTINCT ex.cuenta,ex.producto || ' ' || ex.prod_nombre,
       ...
       EXECUTE PROCEDURE bdidigital@coppelimg_crx:"informix".cons_imgnula(pCliente,pEmpresa,cCod_docto,siSecuencia) 
...
'SP creado a partir de cons_expediente2. Consulta el expediente de documentos de identificacion del cliente.',
'Base de Datos: Bdidigital',
'---------------------------------';

CREATE PROCEDURE "informix".cons_expediente2_prueba(pEmpresa CHAR(3),
                pCliente    CHAR(20),
                pNum_regs   SMALLINT)
            RETURNING
...
-- ****************************************************************************
    ...
    FOREACH
        ...
        SELECT  ex.cuenta,ex.producto || ' ' || ex.prod_nombre,
                ex.cod_docto,ex.fecha_alta,gd.cod_grupo,
                gd.descripcion,td.descripcion,td.multi_imagen,
                ex.secuencia,nvl(ex.descrip2," "), case when img1.cliente is null then "0" else '1' end
        INTO    cCuenta,cProd_nombre,cCod_docto,dFecha_alta,
                cCod_grupo,cDescrip_gpo,cDescrip_docto,
                cMulti_img,siSecuencia,cDescrip2, cIma_esnula
--        FROM    bdidigital@coppelimg_app:"informix".dg_expediente ex,
--                bdidigital@coppelimg_app:"informix".dg_grupodocto gd,
--                bdidigital@coppelimg_app:"informix".dg_tipodocumento td
        FROM    bdidigital@coppelimg_crx:"informix".dg_expediente ex
                join bdidigital@coppelimg_crx:"informix".dg_tipodocumento td on (ex.cod_docto  = td.cod_docto)
                join bdidigital@coppelimg_crx:"informix".dg_grupodocto gd on (td.cod_grupo   = gd.cod_grupo)
                left outer join bdidigital@coppelimg_crx:"informix".dg_expediente_img1 img1 on (ex.cliente = img1.cliente and  td.cod_docto = img1.cod_docto and ex.secuencia = img1.secuencia and imagen is null)
        WHERE   --ex.cod_docto       = td.cod_docto
...
--       EXECUTE PROCEDURE bdidigital@coppelimg_app:"informix".cons_imgnula(pEmpresa,pCliente,cCod_docto,siSecuencia)
--       INTO cCodret2,cIma_esnula;
