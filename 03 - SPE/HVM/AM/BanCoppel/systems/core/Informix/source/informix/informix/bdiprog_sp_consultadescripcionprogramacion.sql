CREATE PROCEDURE "informix".sp_consultadescripcionprogramacion(p_NumCte CHAR(20), p_Tipo SMALLINT, p_Descripcion  CHAR(20))
RETURNING
     CHAR(6), ---cod_ret
     CHAR(10), ---clave pago de programacion
     CHAR(20), ---descricion
     CHAR(2); ---clave de pago
...
--SET DEBUG FILE TO "/tmp/consultadescripcionprogramacion.out";
--TRACE ON;

    LET  v_cod_ret   = "00000";

    IF p_Tipo = '1' THEN
        SELECT {INDEX (bdiprog:pp_pagoprog idxpp_num_cte)} COUNT(*) INTO Iexiste FROM  bdiprog: pp_pagoprog WHERE num_cte = p_NumCte and cve_estado<>'04';
        IF Iexiste>0 THEN
                FOREACH
                        SELECT  {INDEX (bdiprog:pp_pagoprog idxpp_num_cte)} 
                        cve_pagoprog, descripcion, cve_pago
                        INTO v_CvePagoProg, v_Descripcion, v_CvePago
                        FROM  bdiprog:pp_pagoprog
                        WHERE num_cte = p_NumCte
                        and cve_estado<>'04'
                        ORDER BY descripcion 

                        RETURN v_cod_ret, v_CvePagoProg, v_Descripcion, v_CvePago  WITH RESUME;
...
'DESCRIPCION: Consulta la clave y descripcion de la  programacion  de un cliente',
'EJECUTADO O LLAMADO POR: PLPAGPRO.EXE',
'FECHA : DICIEMBRE/2008',
'VERSION: 200812',
'BD    : bdiprog',
'SOLICITO: José Angel López Adams',
'CAMBIO: Se modifica para que en la consulta Tipo 2,  ademas de la descripcion de la programacion, se valide tambien que pertenezca al cliente recibido como parametro',
'AUTOR: José Angel López A',
'FECHA: 02/DIC/2009',
'SOLICITO: José Angel López Adams';
