create procedure "informix".pasecheqhist(pempresa char(3),
                                         pfecha_proceso date)
       returning char(5);
...
select precio_venta into vtpcambval
   ...
   where empresa = pempresa and divisa = vdivisa_cambio and
         fecha_tpcambio = pfecha_proceso and
         clase_tpcambio = "O";
if vtpcambval is null then
   select precio_venta into vtpcambval
      from bdinteg:si_histdiv
      where empresa = pempresa and divisa = vdivisa_cambio and
            fecha_tc = pfecha_proceso
...
foreach
   select md.sucursal,md.producto,divisa,transacc,
     ...
     from sc_movmes md,sc_maechq mc,outer sc_auxcont ac,sc_producto pr,
	  bdinteg:si_transacc tr, bdinteg:si_cliente cl,
          bdinteg:si_tipper tp
     where md.empresa = pempresa and md.fech_alt = pfecha_proceso and 
	...
	from sc_movmes md,sc_maechq ma,sc_producto pr,
  ...
  if vcodret = "000" then
     call pasecont(pempresa,pfecha_proceso) returning vcodret;
  end if
  return vcodret;
end
end procedure;