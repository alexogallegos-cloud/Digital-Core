CREATE PROCEDURE "informix".sp_buscalistainterna(pRfc CHAR(13), pNumCte CHAR (20))
RETURNING 
VARCHAR(3)      AS 	CodRetorno;
   ...
   SET ISOLATION TO DIRTY READ;
	...
	IF ((pRfc=="") AND (pNumCte=="")) THEN 
		LET P_COD_RET='021';
	ELSE
		IF (pRfc=="" OR pRfc IS NULL) THEN
			IF EXISTS (SELECT rfc FROM tbl_listainterna WHERE numcte= pNumCte) THEN
				LET P_COD_RET='021';
			END IF;
		ELSE
			IF EXISTS(SELECT rfc FROM tbl_listainterna WHERE rfc= pRfc) THEN
				LET P_COD_RET='021';
			END IF;
		END IF;
	END IF;
	...
	END PROCEDURE
	DOCUMENT
	'DESCRIPCION: Busca si existe el RFC o el número de cliente dentro de la tabla tbl_listainterna',
	'AUTOR : Nevarez Peinado Josè de Jesùs',
	'FECHA : 15 Julio 2010',
	'VERSION: 20100715',
	'BD: bdiauditor',
	'SISTEMA : PreLavado';

CREATE PROCEDURE "informix".sp_permorales_listanegra( pRfc CHAR(12),
						      pRazonSoc CHAR(150)
                                                       )
RETURNING CHAR(5) AS codRet,
	...
	DEFINE rfcint CHAR(12);
	DEFINE razonsocial CHAR(150);
	...
	--Descomponemos el parametro de entrada rfc para obtener fechaConstitucion
	LET	pAnio = SUBSTR(pRfc,4,2);
	LET pMes = SUBSTR(pRfc,6,2);
	LET pDia = SUBSTR(pRfc,8,2);
    ...
    --SET DEBUG FILE TO '/RESPALDOS/listasnegras/sp_permorales_listanegra.out';
    --TRACE ON;
	...
	IF (pRazonSoc IS NULL OR pRazonSoc= '' ) THEN
		LET cCodRet = '110';
		LET mensaje = 'Parametro pRazonSocial vacio';
       RETURN cCodRet,mensaje,listaNegra,tipoLista;
    END IF;
    
    
    --- VALIDA EN LISTAS NEGRA INTERNA
    SELECT {+AVOID_FULL(bdinteg:si_mant_listainterna)} trim(rfc),trim(razon_social)
    INTO  rfcint,razonsocial
    FROM bdinteg:si_mant_listainterna
    WHERE rfc = pRfc
	AND razon_social = pRazonSoc;
	 
   --- VALIDA EN LISTAS NEGRAS EXTERNA
   
	SELECT {+AVOID_FULL(bdiauditor:tblpld_worldcheck_compara)} first 1 trim(first_name),trim(last_name),SUBSTR(dob,3,2),
	SUBSTR(dob,6,2),SUBSTR(dob,9,2)
	INTO firstname,lastname,anio,mes,dia
	FROM bdiauditor:worldcheck_compara
      	WHERE last_name = pRazonSoc
		AND SUBSTR(dob,10,10) <> '00';
		--trim(last_name) = (trim(pRazonSoc)||' '||trim(pRazonSoc))
		--and (trim(first_name)=trim(pRazonSoc)||' '||trim(pRazonSoc));


    --- VALIDA SI SE ENCUENTRE DADO DE ALTA EN LAS LISTAS NEGRAS
    IF (rfcint = pRfc and razonsocial = pRazonSoc) THEN
		LET listaNegra = 1;
        LET cCodRet = '113';
		LET tipoLista = 1;
		LET mensaje = 'Match RazonSoc y RFC (PM) Lista Interna';
		RETURN cCodRet,mensaje,listaNegra,tipoLista;
    END IF;
	
	--- VALIDA SI SE ENCUENTRE DADO DE ALTA EN LAS LISTAS EXTERNA
    IF (lastname = pRazonSoc and anio = pAnio and pMes = mes and pDia = dia) THEN
		LET listaNegra = 1;
        LET cCodRet = '213';
		LET tipoLista = 2;
		LET mensaje = 'Match Razon Social y Fecha Const (PM) Lista Externa';
		RETURN cCodRet,mensaje,listaNegra,tipoLista;
    END IF;
	
	--- VALIDA SI SE ENCUENTRE DADO DE ALTA EN LAS LISTAS EXTERNA
    IF (lastname = pRazonSoc ) THEN
		LET listaNegra = 1;
        LET cCodRet = '214';
		LET tipoLista = 2;
		LET mensaje = 'Match Razon Social (PM) Lista Externa';
		RETURN cCodRet,mensaje,listaNegra,tipoLista;
    END IF;
...
CREATE PROCEDURE "informix".sp_obtenerdatosrecursospld()
--DATOS A REGRESAR---
RETURNING CHAR(6)  AS cCod_ret,
          CHAR(30)   AS cProveedorRecursos,
		  CHAR(1) As cStatus;
	...
	DEFINE iSqlErr  INTEGER;
	DEFINE iProveedorRecursos INTEGER;	
    DEFINE cProveedorRecursos  char(30);
	DEFINE cStatus  CHAR(1);

	ON EXCEPTION SET iSqlErr
		IF iSqlErr <> 0 THEN
			LET cCod_ret = iSqlErr;				
			RETURN cCod_ret,cProveedorRecursos,cStatus;	
		END IF;
	END EXCEPTION;
	
	--SET DEBUG FILE TO "/home/sysifx/sp_obtenerdatosrecursospld.out";
	--TRACE ON;
	...
	-- ****************************************************************************
	LET iSqlErr				= 0;
	LET iProveedorRecursos	= 0;
    LET cCod_ret     	  	= '000000';	
    LET cProveedorRecursos   = '';	
	LET cStatus   = '';
	...
	BEGIN
		FOREACH
			SELECT descripcion,status INTO cProveedorRecursos,cStatus FROM tbl_proveedor_recursos	
			RETURN cCod_ret,cProveedorRecursos,cStatus WITH RESUME;	
		END FOREACH;
	END;
END PROCEDURE
DOCUMENT
...
'--Fecha: 26/09/2022.',
'--Solicita:', 
'--Descripcion: Se crea procedimiento almacenado para obtener la informaciÃ³n',
'-- 			   del catÃ¡logo de proveedor de recursos.',
'--BD: bdiauditor.',
'-- --------------------------------------------------------------------------';

CREATE PROCEDURE "informix".sp_pld_chqc_crg_txt_genarch(pUsuario CHAR(8), pIdFuncion CHAR(10), pPeriodo CHAR(20), pReporte CHAR(30), pRutaDescarga CHAR(100))
    RETURNING CHAR(5) AS codRet,
	...
	BEGIN
		ON EXCEPTION SET iSqlErr, iIsamErr, cDescErr
			IF iSqlErr <> 0 THEN
				...
				AND procedimiento = 'sp_pld_chqc_crg_txt_genarch' AND movimiento = '2' AND nombre_archivo = cNombreArchivo AND estatus <> 'S';
				...
				LET cDesCodRet = 'ERROR NO CONTROLADO: sp_pld_chqc_crg_txt_genarch '|| DATE(CURRENT)||' '||iSqlErr||' '||cDescErr;
				...
				WHERE nombre_archivo = cNombreArchivo AND procedimiento = 'sp_pld_chqc_crg_txt_genarch'
				AND usuario_insert = pUsuario AND fecha_hora_inicio = dFechaHoraInicio;
				
				LET cCodRet = '01038'; --OCURRIÓ UN ERROR DURANTE LA GENERACIÓN DEL ARCHIVO DE TEXTO. INTENTE NUEVAMENTE
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_pld_chqc_crg_txt_genarch.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pPeriodo = '' OR pReporte = '' OR pRutaDescarga = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cNombreArchivo;
		END IF;
		...
		VALUES(null,TRIM(cNombreArchivo),'I','sp_pld_chqc_crg_txt_genarch','','',pUsuario,dFechaHoraInicio,null);
				
		SET ISOLATION TO DIRTY READ;	
		SET LOCK MODE TO WAIT 3;	
		
		-- SE REGISTRA PROCESO
		IF EXISTS (SELECT 1 FROM bdiauditor:"informix".tblpld_bit_ejec WHERE reporte = pReporte AND periodo = pPeriodo AND procedimiento = 'sp_pld_chqc_crg_txt_genarch') THEN
			...
			VALUES(pReporte,pPeriodo,pUsuario,dFechaHoraInicio,null,'sp_pld_chqc_crg_txt_genarch','2',TRIM(cNombreArchivo),'R');
		...
		ELSE
			...
			INSERT INTO bdiauditor:"informix".tblpld_bit_ejec(reporte,periodo,usuario,fecha_hora_inicio,fecha_hora_fin,procedimiento,movimiento,nombre_archivo,estatus)
			VALUES(pReporte,pPeriodo,pUsuario,dFechaHoraInicio,null,'sp_pld_chqc_crg_txt_genarch','2',TRIM(cNombreArchivo),'P');
		...
		WHERE reporte = pReporte AND periodo = pPeriodo	AND usuario = usuario AND fecha_hora_inicio = dFechaHoraInicio 
		AND procedimiento = 'sp_pld_chqc_crg_txt_genarch' AND nombre_archivo = cNombreArchivo;
		...
		BEGIN WORK;
			...
			LET cCmd1 ="";
			LET cCmd1 ="SELECT TRIM(fecha),TRIM(numero_cliente),TRIM(numero_cuenta),TRIM(nombre_razon),TO_CHAR(monto_usd),TO_CHAR(monto_mxn)";
			LET cCmd1 =""||TRIM(cCmd1)||" FROM bdiauditor:""informix"".tblpld_chqc_txt";
			LET cCmd1 =""||TRIM(cCmd1)||" ORDER BY fecha ASC";
			...
			LET cSql = '';
			LET cSql = TRIM(cRutaInformix)||'dbaccess bdiauditor '||TRIM(pRutaDescarga)||'query.sql';
			SYSTEM TRIM(cSql);
		...
		LET cCodRetSp = '000000';
		LET cDesCodRet = 'LA GENERACIÓN DEL REPORTE '||TRIM(pReporte)||' CONCLUYÓ EXITOSAMENTE. EL ARCHIVO GENERADO '||TRIM(cNombreArchivo)||' ESTÁ LISTO PARA SER DESCARGADO';
...
'DESCRIPCION: Spl encargado de consultar la información de la tabla destino (bdiauditor:"informix".tblpld_chqc_txt), para realizar la generación de los archivos txt.',
'BD: bdiauditor',
'AUTOR: Veronica Sanchez Tlacomulco',
'FECHA: 10/02/2023',
'DESCRIPCION: Se modifica ruta de la carpeta bin para ambiente productivo';
