CREATE PROCEDURE "informix".sp_repcredrev (pEmpresa CHAR(3), pFecha DATE )
...
DEFINE	iMesesAtraso28	INTEGER;
DEFINE	iMesesAtraso29	INTEGER;
DEFINE	iMesesAtraso30	INTEGER;
DEFINE	iMesesAtraso31	INTEGER;
DEFINE	iFlagCob	INTEGER;
DEFINE	iImpagos	INTEGER;
DEFINE	dTasaInt	DECIMAL(18,2);
DEFINE	dProbInc	DECIMAL(18,2);
DEFINE	dSevPerd	DECIMAL(18,2);
DEFINE	dPagoReal	DECIMAL(18,2);
DEFINE	dPagoMin	DECIMAL(18,2);
DEFINE	iMesesApert	DECIMAL(18,2);
...
define dtFechaHoy DATE;
	...
	--SET DEBUG FILE TO "/informix/jesus/correo/sp_repcredrev.out";
	--TRACE ON;
---INICIALIZACIONES
LET iSqlErr				= 0;
LET iIsamErr			= 0;
...
LET	dOtrosAdeudos	= 0;
LET	iMesesAtraso	= 0;
LET	iMesesAtraso28	= 0;
LET	iMesesAtraso29	= 0;
LET	iMesesAtraso30	= 0;
LET	iMesesAtraso31	= 0;
LET	iFlagCob	= 0;
LET	iImpagos	= 0;
LET	dTasaInt	= 0;
LET	dProbInc	= 0;
...
BEGIN
	...
	LET dTFechaSD = MONTHADD(mdy(month(dtFechaHoy),01,YEAR(dtFechaHoy)), - 1);
	--GENERA EL NOMBRE DEL ARCHIVO
	LET cNombreArchivo = TRIM('Infpattitcrdrev2')||TO_CHAR(dtFechaHoy,'%d%m%y')|| '.txt';
	LET cNombreArchivo1 = TRIM('Infpattitcrdrev2_aux')||TO_CHAR(dtFechaHoy,'%d%m%y')|| '.txt';
	
    
FOREACH WITH HOLD  
	SELECT a.numcte,TRIM(NVL(cte.nombre1,' ')) || ' ' || TRIM(NVL(cte.nombre2,' ')) || ' ' || TRIM(NVL(cte.apell_paterno,' ')) || ' ' || TRIM(NVL(cte.apell_materno,' ')),
	TODAY,a.num_credito, (CASE WHEN a.status_cred IN ('AA','E1') AND NVL(monto_vencido + mto_venc_trasp,0) = 0  THEN  1 ELSE 2 END), "TC",
    saldo_cierre,sdo_int_anticip,sdo_cap_insoluto,monto_vencido+mto_venc_trasp,int_tra_no_exig,sdo_moratorio+sdo_contab_mora,
	...
	b.mto_fin_ven_trasp,1,impagos_consecutivos,NVL(tasa_interes,0),
	probabilidad_incumplimiento,severidad_perdida,pagos_realizados, pago_minimo,
	meses_antiguedad
	INTO 
	cNumCte, cNombreCte,pFecha,cNumCred,  cSitCont,  tpCred,dSaldoTotal ,dSaldoRev ,dCapVig, dCapVenc ,dIntVenc ,dIntMor ,dComPend, dSeguros, dOtrosAdeudos,iMesesAtraso ,iFlagCob ,iImpagos ,dTasaInt ,dProbInc ,dSevPerd,dPagoReal, dPagoMin, iMesesApert
	
	FROM "informix".sd_maecredcont a, sd_maesdoscont b , "informix".sd_hist_reserva c ,
	...
	AND c.fecha_cierre = dtFechaFinMes
...
LET cConsulta = TRIM(NVL(cNumCte,''))||'|'|| TRIM(NVL(cNombreCte,''))||'|'||pFecha||'|'|| TRIM(NVL(cNumCred,''))||'|'||  TRIM(NVL(cSitCont,''))||'|'||  TRIM(NVL(tpCred,''))||'|'||
			NVL(dSaldoTotal,0)||'|'|| NVL(dSaldoRev,0)||'|'|| NVL(dCapVig,0)||'|'|| NVL(dCapVenc,0)||'|'|| NVL(dIntVenc,0)||'|'|| NVL(dIntMor,0)||'|'|| NVL(dComPend,0)||'|'|| NVL(dSeguros,0)||'|'|| NVL(dOtrosAdeudos,0)||'|'||
			NVL(iMesesAtraso,0)||'|'|| NVL(iFlagCob,0)||'|'|| NVL(iImpagos,0)||'|'|| NVL(dTasaInt,0)||'|'|| NVL(dProbInc,0)||'|'|| NVL(dSevPerd,0)||'|'|| NVL(dPagoReal,0)||'|'|| NVL(dPagoMin,0)||'|'|| NVL(iMesesApert,0);
   ...
   IF iContador2  > 0  THEN 	
		---se ejecuta para ponerle el encabezado 
		LET cEncabezado = 'echo "Identificar del cliente'||'|'||'Nombre del acreditado'||'|'||'Periodo'||'|'||'Identificador de Crédito'||'|'||'Situación Contable'||'|'||'Tipo de Crédito'||'|'||'Saldo Total'||'|'||'Saldo Revolvente'||'|'||'Capital Vigente'||'|'||'Capital Vencido'||'|'||'Interes Ordinario'||'|'||'Interes Moratorio'||'|'||'Comisiones'||'|'||'Seguros'||'|'||'Otros Adeudos'||'|'||'Meses de Atraso'||'|'||'Cobranza'||'|'||'Impagos Consecutivos'||'|'||'Tasa  de Interes'||'|'||'Probabilidad de Incumplimiento'||'|'||'Severidad de Perdida'||'|'||'Pago Realizado'||'|'||'Pago Minimo'||'|'||'Meses de Apertura'||'|'|| '" > '||TRIM(cruta)|| cNombreArchivo;  
		SYSTEM cEncabezado;
