CREATE PROCEDURE "informix".sp_limpia_telefono ( p_tel lvarchar )
RETURNING lvarchar;

 DEFINE s_tel lvarchar;
 DEFINE s_tel2 lvarchar;
 DEFINE s_tel_total lvarchar;
 DEFINE s_tel_tam int;
 DEFINE s_long_tel int;
 ...
 IF (NVL(s_tel,'') <> '') THEN
    LET s_tel_tam = LENGTH(s_tel);
    --Ciclo de AnÃ¡lisis
    WHILE (s_long_tel <= s_tel_tam)
			LET s_tel2 = SUBSTR(s_tel,s_long_tel,1);
            LET s_ascii = ASCII(s_tel2);

			IF s_ascii NOT BETWEEN 48 AND 57 THEN
                LET s_tel2 = '';
            END IF;

                LET s_long_tel = ( s_long_tel + 1);
                LET s_tel_total = s_tel_total || s_tel2;
