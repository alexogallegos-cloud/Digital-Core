CREATE PROCEDURE "informix".sp_generaredoctaeje_factelect( pEmpresa char(3) )
    RETURNING CHAR(5);
	...
	DEFINE vAniomes_cfdi            VARCHAR(6);
	DEFINE vCuenta_cfdi             VARCHAR(20);
	DEFINE vFechaini_cfdi           DATE;
	DEFINE vFechafin_cfdi           DATE;
	DEFINE vSdo_mes_ant_cfdi        MONEY(14,2);
	DEFINE vTotretiros_cfdi         MONEY(16,2);
	DEFINE vTotdepositos_cfdi       MONEY(16,2);
	DEFINE vSdo_actual_cfdi         MONEY(14,2);
	DEFINE vProducto_cfdi           VARCHAR(4);	
	...
	LET vAniomes_cfdi        = "";        
	LET vCuenta_cfdi         = "";    
	LET vSdo_mes_ant_cfdi    = 0.00;    
	LET vTotretiros_cfdi     = 0.00;    
	LET vTotdepositos_cfdi   = 0.00;    
	LET vSdo_actual_cfdi     = 0.00;    
	LET vProducto_cfdi       = "";
	LET vProdcfdi     = 0;
    ...
    ON EXCEPTION SET vsqlerr, iIsamErr, cErrorInfo
        IF vsqlerr != 0 THEN
            SET DEBUG FILE TO "/resplogifx/conciliachq/edoctacfd/sp_generaredoctaeje_factelect.err";
            TRACE ON;
            LET vcodret = vsqlerr;
            ...
            LET vcSql = 'echo "UPDATE bdicheq:sc_contproc_edocta_factelect '||
                        ...
                        'AND  status_proc = '''||'I'||''' '||
                        'AND tipo_proc  = '''||'D'||''';" > /tmp/contproc_edocta_fe.sql';
            SYSTEM vcSql;
            LET vcStmt = 'dbaccess bdicheq /tmp/contproc_edocta_fe.sql';
            SYSTEM vcStmt;
            RETURN vcodRet;
        END IF;
    END EXCEPTION;
    ...
    --SET DEBUG FILE TO "/RESPALDOSNEW/rsv/cfdi_4.0/produccion/sp_generaredoctaeje_factelect.txt";
    --TRACE ON;
    ...
    -- // obtener la fecha en que se ejecuto por ultima vez.
    ...
    IF vexiste_genedoctaeje is null OR vexiste_genedoctaeje = '' THEN
        LET vcSql = 'echo " INSERT INTO bdicheq:sc_contproc_edocta_factelect (empresa, proceso, fecha, tipo_proc, status_proc, ejecutivo, hora_inicio, hora_fin, cod_ret, mensaje) '||
                    'VALUES('''||pempresa||''', '''||'GENERA EDO CTA FE'||''', '''||vfecha_hoy||''', '''||'D'||''', '''||'I'||''', USER,'||
                    '(SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas WHERE empresa = '''||pempresa||'''),'||
                    'NULL,'''||vcodret||''', '''||vErrorInfo||''');" > /tmp/contproc_edocta_fe.sql';
        SYSTEM vcSql;
        LET vcStmt = 'dbaccess bdicheq /tmp/contproc_edocta_fe.sql';
        SYSTEM vcStmt;
    ELSE
        LET vcSql = 'echo " UPDATE bdicheq:sc_contproc_edocta_factelect '||
                    ...
                    'hora_fin = NULL '||
                    'WHERE fecha = '''||vfecha_hoy||''' AND status_proc = '''||'C'||''' AND  tipo_proc = '''||'D'||''';" > /tmp/contproc_edocta_fe.sql';
        SYSTEM vcSql;
        LET vcStmt = 'dbaccess bdicheq /tmp/contproc_edocta_fe.sql';
        SYSTEM vcStmt;
    END IF;
	
	--INICIALIZA LA TABLA QUE CONTIENE LA INFORMACION A PROCESAR
	TRUNCATE TABLE sc_maehis_cfdi_cap;
	
	FOREACH WITH HOLD
	
			SELECT mae.aniomes,      mae.cuenta,         mae.fechaini,     mae.fechafin,   mae.sdo_mes_ant, 
				   mae.totretiros,   mae.totdepositos,   mae.sdo_actual,   mae.producto 
		    INTO   vAniomes_cfdi,    vCuenta_cfdi,       vFechaini_cfdi,   vFechafin_cfdi, vSdo_mes_ant_cfdi,        
	               vTotretiros_cfdi, vTotdepositos_cfdi, vSdo_actual_cfdi, vProducto_cfdi  
			FROM   sc_maehis AS mae
			WHERE  mae.aniomes = mae.aniomes
			AND    mae.cuenta NOT IN (SELECT {+INDEX(sc_encabezado_edocta_factelect idx_encabezadofechacuenta_fe)}
								      ...
								      WHERE  fecha_emision = mae.fechafin 
                                      AND    num_cuenta    = mae.cuenta)
			AND    mae.cuenta NOT IN (SELECT fe.cuenta 
							          FROM   sc_maehis_factelect fe 
							          WHERE  mae.empresa  = fe.empresa 
							          AND    mae.cuenta   = fe.cuenta
							          AND    mae.fechafin = fe.fechafin)	
            AND    mae.fechafin BETWEEN vultejec  AND vfecha_hoy
			AND    mae.producto NOT IN('9901')									  
			
			
			--SE INSERTAN REGISTROS A LA TABLA DE CONTROL 
			INSERT INTO sc_maehis_cfdi_cap VALUES(vAniomes_cfdi,vCuenta_cfdi,vFechaini_cfdi,vFechafin_cfdi,vSdo_mes_ant_cfdi,vTotretiros_cfdi,vTotdepositos_cfdi,vSdo_actual_cfdi,vProducto_cfdi);
    END FOREACH;
	   
    -- // INVOCA PROCESO PARA ACTUALIZAR PARAMETROS DE NUMEROS DE RANGOS
    EXECUTE PROCEDURE "informix".sp_generaredoctaeje_factelect_param( pempresa, vfecha_hoy, vultejec )
    INTO vcodretparam;
    ...
    WHILE ( vprocesocomp1 = 0 OR vprocesocomp2 = 0 OR vprocesocomp3 = 0 OR vprocesocomp4 = 0 OR vprocesocomp5 = 0 )
        SET ISOLATION TO DIRTY READ;
        SELECT COUNT(*)
          INTO vprocesocomp1
          FROM sc_contproc
         WHERE empresa = pempresa
           AND proceso = 'genedoctafedcomp1'
           AND fecha = vfecha_ant;
    ...
    IF LPAD(DAY(vfecha_hoy),2,'0') = '03' THEN
        ...
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_encabezado_edocta_'||vfecha||'.txt'||
                ' SELECT a.* FROM sc_encabezado_edocta_factelect as a WHERE a.fecha_emision = '''|| vfecha_ant ||'''  UNION ALL '||
                ' SELECT b.* FROM sc_encabezado_edocta_factelect as b WHERE b.fecha_emision = '''|| vult_mes ||''' '||
                ' and b.num_cuenta in (select cuenta  from bditransfer:tf_resumen_edocta where periodo_fin = '''|| vult_mes ||''' ) '||
                ' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_encabezado2_edocta_'||vfecha||'.txt'||  --
                ' SELECT a.* FROM sc_encabezado2_edocta_factelect as a WHERE a.fecha_emision = '''|| vfecha_ant ||'''  UNION ALL '||
                ' SELECT b.* FROM sc_encabezado2_edocta_factelect as b WHERE b.fecha_emision = '''|| vult_mes ||''' '||
                ' and b.num_cuenta in (select cuenta  from bditransfer:tf_resumen_edocta where periodo_fin = '''|| vult_mes ||''' ) '||
                ' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_detalle_edocta_'||vfecha||'.txt'||
                ' SELECT a.* FROM sc_detalle_edocta_factelect as a WHERE a.fecha_emision = '''|| vfecha_ant ||'''  UNION ALL '||
                ' SELECT b.* FROM sc_detalle_edocta_factelect as b WHERE b.fecha_emision = '''|| vult_mes ||''' '||
                ' and b.num_cuenta in (select cuenta  from bditransfer:tf_resumen_edocta where periodo_fin = '''|| vult_mes ||''' ) '||
                ' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_piepagina_edocta_'||vfecha||'.txt'||
                ' SELECT a.* FROM sc_piepagina_edocta_factelect as a WHERE a.fecha_emision = '''|| vfecha_ant ||'''  UNION ALL '||
                ' SELECT b.* FROM sc_piepagina_edocta_factelect as b WHERE b.fecha_emision = '''|| vult_mes ||''' '||
                ' and b.num_cuenta in (select cuenta  from bditransfer:tf_resumen_edocta where periodo_fin = '''|| vult_mes ||''' ) '||
                ' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_mensajes_edocta_'||vfecha||'.txt'||
                ' SELECT a.* FROM sc_mensajes_edocta_factelect as a WHERE a.fecha_emision = '''|| vfecha_ant ||'''  UNION ALL '||
                ' SELECT b.* FROM sc_mensajes_edocta_factelect as b WHERE b.fecha_emision = '''|| vult_mes ||''' '||
                ' and b.num_cuenta in (select cuenta  from bditransfer:tf_resumen_edocta where periodo_fin = '''|| vult_mes ||''' ) '||
                ' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_grafica_edocta_'||vfecha||'.txt'||
                ' SELECT a.* FROM sc_grafica_fe as a WHERE a.fecha_emision = '''|| vfecha_ant ||'''  UNION ALL '||
                ' SELECT b.* FROM sc_grafica_fe as b WHERE b.fecha_emision = '''|| vult_mes ||''' '||
                ' and b.num_cuenta in (select cuenta  from bditransfer:tf_resumen_edocta where periodo_fin = '''|| vult_mes ||''' ) '||
                ' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_aclaraciones_edocta_'||vfecha||'.txt'||
                ' SELECT a.* FROM sc_aclaraciones_edocta_factelect as a WHERE a.fecha_emision = '''|| vfecha_ant ||'''  UNION ALL '||
                ' SELECT b.* FROM sc_aclaraciones_edocta_factelect as b WHERE b.fecha_emision = '''|| vult_mes ||''' '||
                ' and b.num_cuenta in (select cuenta  from bditransfer:tf_resumen_edocta where periodo_fin = '''|| vult_mes ||''' ) '||
                ' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = '';
        LET vsql = '/usr/bin/gzip -9 '||TRIM(vruta_descarga)||'sc_encabezado_edocta_'||vfecha||'.txt';
        SYSTEM vsql;
       
        LET vsql = '';
        LET vsql = '/usr/bin/gzip -9 '|| TRIM(vruta_descarga) ||'sc_encabezado2_edocta_'||vfecha||'.txt';
        SYSTEM vsql;
       
        LET vsql = '';
        LET vsql = '/usr/bin/gzip -9 '||TRIM(vruta_descarga)||'sc_detalle_edocta_'||vfecha||'.txt';
        SYSTEM vsql;
       
        LET vsql = '';
        LET vsql = '/usr/bin/gzip -9 '||TRIM(vruta_descarga)||'sc_piepagina_edocta_'||vfecha||'.txt';
        SYSTEM vsql;
       
        LET vsql = '';
        LET vsql = '/usr/bin/gzip -9 '||TRIM(vruta_descarga)||'sc_mensajes_edocta_'||vfecha||'.txt';
        SYSTEM vsql;
       
        LET vsql = '';
        LET vsql = '/usr/bin/gzip -9 '||TRIM(vruta_descarga)||'sc_grafica_edocta_'||vfecha||'.txt';
        SYSTEM vsql;
       
        LET vsql = '';
        LET vsql = '/usr/bin/gzip -9 '||TRIM(vruta_descarga)||'sc_aclaraciones_edocta_'||vfecha||'.txt';
        SYSTEM vsql;
    ELSE
        -- // GENERA EL ARCHIVO DE LA TABLA sc_encabezado_edocta_factelect
        LET vsql = '';
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_encabezado_edocta_'||vfecha||'.txt'||
                   ' SELECT * FROM sc_encabezado_edocta_factelect WHERE fecha_emision = '''|| vfecha_ant ||''' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = '';
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_encabezado2_edocta_'||vfecha||'.txt'||
                   ' SELECT * FROM sc_encabezado2_edocta_factelect WHERE fecha_emision = '''|| vfecha_ant ||''' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = '';
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_detalle_edocta_'||vfecha||'.txt'||
                   ' SELECT * FROM sc_detalle_edocta_factelect WHERE fecha_emision = '''|| vfecha_ant ||''' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = '';
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_piepagina_edocta_'||vfecha||'.txt'||
                   ' SELECT * FROM sc_piepagina_edocta_factelect WHERE fecha_emision = '''|| vfecha_ant ||''' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = '';
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_mensajes_edocta_'||vfecha||'.txt'||
                   ' SELECT * FROM sc_mensajes_edocta_factelect WHERE fecha_emision = '''|| vfecha_ant ||''' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = '';
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_grafica_edocta_'||vfecha||'.txt'||
                   ' SELECT * FROM sc_grafica_fe WHERE fecha_emision = '''|| vfecha_ant ||''' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
        ...
        LET vsql = '';
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(vruta_descarga)||'sc_aclaraciones_edocta_'||vfecha||'.txt'||
                   ' SELECT * FROM sc_aclaraciones_edocta_factelect WHERE fecha_emision = '''|| vfecha_ant ||''' ORDER BY num_cuenta" > '|| TRIM(vruta_descarga) ||'dskrga_fed.sql';
        SYSTEM vsql;
