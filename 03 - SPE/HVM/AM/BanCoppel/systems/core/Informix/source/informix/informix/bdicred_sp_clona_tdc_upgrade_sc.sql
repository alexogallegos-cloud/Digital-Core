CREATE PROCEDURE "informix".sp_clona_tdc_upgrade_sc(pEmpresa CHAR(3),P_EJECUTIVO CHAR(10) , pProducto CHAR(4), pCredito CHAR(20) ,pTarjeta CHAR(20),pTarjetaOro CHAR(20))
...
LET cSitEspCredCSG 			= '';
--Nueva Solicitud de Crédito Oro
LET scod_ret				= '';
LET cSolOro     			= '';
LET vFolio                  = '';
--LET vHoy                    = DATE(1);
...
--SET DEBUG FILE TO '/informix/Malena/sp_clona_tdcoro.out';
--TRACE ON;
...
IF SUBSTR(pTarjeta,1,6) <> SUBSTR(pTarjetaOro,1,6) THEN
	-- CONSULTAMOS EL SALDO GENERAL DEL CREDITO.
	EXECUTE PROCEDURE bdicred:"informix".sp_consulta_saldos_general(TRIM(pEmpresa), TRIM(pCredito))
	...
	IF cCodRetCSG = '000003' THEN -- Numero de crédito no existe
		LET cCodRet  = '000002';
	...
	END IF
	   ...
	   SELECT status_cred
		...
		IF dcSdoRetenidoCSG =0 AND CstatusSol IN ('AA' ,'E1') AND (dcCapTransCSG + dcCapVdoExigCSG) = 0 THEN

			CALL bdisolic:"informix".asigna_numsol(pEmpresa, pProducto)
		...
		SELECT fecha_hoy
		 ...
		 call monthadd(V_FECHA_APERT,12) returning V_FECHA_VENC;
			...
			EXECUTE PROCEDURE bdicred:"informix".sp_obtiene_tasa_int_diferenciadas(pEmpresa, pCredito, pProducto) INTO cCodRetTDif, V_TASA_INTERES, V_TASA_MORA;
			...
			*/				-- RQM 10 1224
			...
			--- Genera cuenta Clabe
			EXECUTE PROCEDURE bdicred:"informix".sp_gen_clabe_interbancaria (pEmpresa,cSolOro,pProducto)
			 ...
			 AND    SOL.EMPRESA         = pEmpresa;
				...
				SELECT a.num_producto, a.divisa, c.monto_otorgado, b.sucursal
		...
		-- *********************************************************
		   ...
		   IF DAY(V_FECHA_APERT) > vDiaCorte AND DAY(V_FECHA_APERT) <= DAY(vFechaT) THEN
		   --Se considera la fecha_cuota que devuelve mas 1 mes, ya que la consulta es fecha_cuota > = a la fecha_hoy menos 1 mes.
				INSERT INTO bdicred:"informix".sd_amortiza_credito(empresa,num_credito,fecha_cuota,tipo_cuota,capital_mto_cuota,
			...
			/* AAME 20160829 RQI 27 122 SE REALIZARÁN EN PROCESO NOCTURNO EL CLONADO DE TABLAS
			...
			INSERT INTO "informix".sd_maesdoscont(fecha,empresa,num_credito, fecha_ult_mov,sdo_int_anticip,sdo_int_ant_dev,sdo_intereses,
			...
			provi_venc_anticip,cap_tras_no_venci,mto_venc_int,mto_venc_tra_int,mto_finan_vdo,mto_reser_int,
			mto_fin_ven_trasp,mto_fin_vig_trasp,int_tra_no_exig,sdo_trab4
			FROM "informix".sd_maesdoscont WHERE  empresa= pEmpresa and num_credito =pCredito;

			--CLONADO DE TABLA DE SD_MAESDOSHIST
			INSERT INTO "informix".sd_maesdoshist(fecha,
			...
			provi_venc_anticip,cap_tras_no_venci,mto_venc_int,mto_venc_tra_int,mto_finan_vdo,mto_reser_int,
			mto_fin_ven_trasp,mto_fin_vig_trasp,int_tra_no_exig,sdo_trab4
			FROM "informix".sd_maesdoshist WHERE  empresa= pEmpresa and num_credito =pCredito;

			--CLONADO DE TABLA DE SD_HIST_RESERVA
			INSERT INTO "informix".sd_hist_reserva(empresa, fecha_corte,	num_credito, fecha_cierre,grado_riesgo,
			...
			-- SE GENERA EL FOLIO
			CALL bdicheq:"informix".sp_generafolionomina(P_EJECUTIVO) RETURNING cCodRet, vFolio;

			-----------------------------------------
			--- PROCESO DE LIQUIDACION DE CREDITO CLASICA---
			-----------------------------------------
			CALL bdicred:"informix".sp_liquida_cred_upgrade (pEmpresa,pCredito,vFolio, dcTotalLiqCSG) RETURNING vCodRet;
			...
			ELSE
				...
				UPDATE "informix".sd_maecred SET credito_externo = pCredito WHERE num_credito = cSolOro;

				-- Actualizacion de tarjeta de clasica por la cuenta de credito Oro
				UPDATE bdicred:"informix".sd_tarjeta  SET num_credito = cSolOro, prodtarjeta=pProducto, secuencia=1 WHERE num_tarjeta = pTarjetaOro;
				UPDATE intercard:"informix".tarjetacuenta  SET numcuenta = cSolOro WHERE numtarjeta = pTarjetaOro;
				-- Actualiza producto de la tarjeta nueva en intercard INI
				...
				if (NVL(Scodproducto,'') <> '') then
					UPDATE intercard:"informix".tarjeta SET codproductotarjeta = Scodproducto WHERE numtarjeta = pTarjetaOro;
				end if;
				...
				FOREACH WITH HOLD 
					...
					UPDATE bdicred:"informix".sd_tarjeta  SET num_credito = cSolOro, prodtarjeta=pProducto, secuencia=iSecuencia WHERE num_tarjeta = cNumtarjadi;
					UPDATE intercard:"informix".tarjetacuenta  SET numcuenta = cSolOro WHERE numtarjeta = cNumtarjadi;
				...
				UPDATE bdicred:"informix".sd_credito_upgrade  SET numero_credito_upgrade = cSolOro, numerotarjeta_upgrade= pTarjetaOro, Resultado='1'
				WHERE numerotarjeta = pTarjeta;
			END IF;
...
ELSE 
	-- Actualización de credito en bitacora de upgrade cuando se trata de una reposición de tarjeta personalizada
		UPDATE bdicred:"informix".sd_credito_upgrade SET numerotarjeta_upgrade= pTarjetaOro, Resultado='1'
		WHERE numerotarjeta = pTarjeta;
END IF;
	RETURN cCodRet, cMensajeRet;
...
END PROCEDURE
DOCUMENT
'Se realiza procedimiento para liquidar el credito de TDC clasica y crear el credito para TDC ORO que se ejecutará',
'desde el de Reposición de Tarjeta',
'AUTOR : Maria Elena Angulo Aispuro',
'FECHA : 22/02/2016',
'BD    : BDICRED';

CREATE PROCEDURE "informix".sp_consulta_saldos_general_mora(pEmpresa      CHAR(3),
                                                       pNumCredito   CHAR(20))
...
LET dMoraBase         = 0.00;
LET dMoraCopete       = 0.00;
LET cSucursal         = '';
LET dIvamoraBase      = 0.00;
LET dIvamoraCopete    = 0.00;
LET dIva              = 0.00;
LET cTipoCred	 	  = '';	
LET cCodCred		  = '';	
...
LET dTasaInteres          = 0.00;
LET dTasaMoratorios       = 0.00;
LET dMontoSBC             = 0.00;
LET dCapVig               = 0.00;
LET dCapTrans             = 0.00;
LET dCapVdoExig           = 0.00;
LET dCapVdoNoExig         = 0.00;
LET dSdoActCap        	  = 0.00;
LET dIntVig               = 0.00;
LET dIntVdo               = 0.00;
LET dIntMoratorio     	  = 0.00;
LET dIntMes           	  = 0.00;
LET dSdoActInt       	  = 0.00;
LET dIvaIntVig     	 	  = 0.00;
LET dIvaIntVdo      	  = 0.00;
LET dIvaIntMoratorio	  = 0.00;
LET dIvaIntMes        	  = 0.00;
LET dSdoActIvaInt         = 0.00;
LET dComPend              = 0.00;
LET dIvaCom               = 0.00;
LET dSdoRetenido          = 0.00;
LET dSdoTotalLiq          = 0.00;
LET dIntDevengado         = 0.00;
LET dIvaIntDevengado      = 0.00;
LET dLineaDisponible      = 0.00;
LET dPagosVdos            = 0.00;
LET cDescStatusCred       = '';
LET iIdUnidadProd         = 0;
LET cDescBloqueoCta       = '';
...
BEGIN
	...
	ON EXCEPTION SET iSqlErr
		IF iSqlErr <> 0 THEN
		  LET cCodRet     = iSqlErr;
		  LET cMensajeRet = 'OCURRIÓ ERROR AL CONSULTAR LOS SALDOS';
		RETURN TRIM(cCodRet), TRIM(cMensajeRet), TRIM(NVL(cNumCredito,'')), TRIM(NVL(cCodTipCred,'')),NVL(dtFechaOrigen,DATE(1)), NVL(dtFechaProxPago,DATE(1)),NVL(dPagoMinimo,0.00), NVL(dtFechaUltPago,DATE(1)),
			NVL(iPlazo,0), NVL(iPagosRealizados,0), NVL(dLineaOtorgada,0.00), NVL(dTasaInteres,0.00), NVL(dTasaMoratorios,0.00), NVL(dMontoSBC,0.00), NVL(dCapVig,0.00), NVL(dCapTrans,0.00),
			NVL(dCapVdoExig,0.00), NVL(dCapVdoNoExig,0.00), NVL(dSdoActCap,0.00), NVL(dIntVig,0.00), NVL(dIntVdo,0.00), NVL(dIntMoratorio,0.00), NVL(dIntMes,0.00), NVL(dSdoActInt,0.00), NVL(dIvaIntVig,0.00), 
			NVL(dIvaIntVdo,0.00), NVL(dIvaIntMoratorio,0.00), NVL(dIvaIntMes,0.00), NVL(dSdoActIvaInt,0.00), NVL(dComPend,0.00), NVL(dIvaCom,0.00), NVL(dSdoRetenido,0.00), NVL(dSdoTotalLiq,0.00),
			NVL(dIntDevengado,0.00),NVL(dIvaIntDevengado,0.00), NVL(dLineaDisponible,0.00), NVL(dPagosVdos,0.00), TRIM(NVL(cDescStatusCred,'')), NVL(iIdUnidadProd,0), TRIM(NVL(cDescBloqueoCta,'')), 
			NVL(cCodCaract2,''), NVL(TRIM(cDescCausaBloqueoCta),''), NVL(cSitCte,''), NVL(iCausaCte,0), NVL(TRIM(cDescSitEspCte),''), NVL(cSitCred,''), NVL(iCausaCred,0), 
			TRIM(NVL(cDescSitEspCred,'')),NVL(dMoraBase,0.00),NVL(dMoraCopete,0.00),NVL(dIvamoraBase,0.00),NVL(dIvaMoraCopete,0.00);
		END IF;
	END EXCEPTION;

	--SET DEBUG FILE TO '/respaldosbd/isarai/sp_consulta_saldos_general_mora.out';
	--TRACE ON;
	...
	--SE EJECUTA EL PROCEDIMIENTO PARA CONSULTAR LOS SALDOS DEL NUMERO DE CREDITO
	EXECUTE PROCEDURE "informix".sp_consulta_saldos_general(pEmpresa, pNumCredito)
		INTO cCodRet2,cMsjRet,cNumCredito,cCodTipCred,dtFechaOrigen,dtFechaProxPago,dPagoMinimo,dtFechaUltPago,iPlazo,iPagosRealizados,dLineaOtorgada,dTasaInteres,dTasaMoratorios,
			dMontoSBC,dCapVig,dCapTrans,dCapVdoExig,dCapVdoNoExig,dSdoActCap,dIntVig,dIntVdo,dIntMoratorio,dIntMes,dSdoActInt,dIvaIntVig,dIvaIntVdo,dIvaIntMoratorio,dIvaIntMes,dSdoActIvaInt,
			dComPend,dIvaCom,dSdoRetenido,dSdoTotalLiq,dIntDevengado,dIvaIntDevengado,dLineaDisponible,dPagosVdos,cDescStatusCred,iIdUnidadProd,cDescBloqueoCta,cCodCaract2,cDescCausaBloqueoCta,
			cSitCte,iCausaCte,cDescSitEspCte,cSitCred, iCausaCred,cDescSitEspCred;
	
	--ERROR EN LA CONSULTA AL PROCEDIMIENTO SP_CONSULTA_SALDOS_GENERAL
	IF cCodRet2::INTEGER <> 0 THEN
		LET cCodRet = '000002';
		LET cMensajeRet = 'ERROR EN LA CONSULTA AL PROCEDIMIENTO SP_CONSULTA_SALDOS_GENERAL';
		RETURN TRIM(cCodRet), TRIM(cMensajeRet), TRIM(NVL(cNumCredito,'')), TRIM(NVL(cCodTipCred,'')),NVL(dtFechaOrigen,DATE(1)), NVL(dtFechaProxPago,DATE(1)),NVL(dPagoMinimo,0.00), NVL(dtFechaUltPago,DATE(1)),
			NVL(iPlazo,0), NVL(iPagosRealizados,0), NVL(dLineaOtorgada,0.00), NVL(dTasaInteres,0.00), NVL(dTasaMoratorios,0.00), NVL(dMontoSBC,0.00), NVL(dCapVig,0.00), NVL(dCapTrans,0.00),
			NVL(dCapVdoExig,0.00), NVL(dCapVdoNoExig,0.00), NVL(dSdoActCap,0.00), NVL(dIntVig,0.00), NVL(dIntVdo,0.00), NVL(dIntMoratorio,0.00), NVL(dIntMes,0.00), NVL(dSdoActInt,0.00), NVL(dIvaIntVig,0.00), 
			NVL(dIvaIntVdo,0.00), NVL(dIvaIntMoratorio,0.00), NVL(dIvaIntMes,0.00), NVL(dSdoActIvaInt,0.00), NVL(dComPend,0.00), NVL(dIvaCom,0.00), NVL(dSdoRetenido,0.00), NVL(dSdoTotalLiq,0.00),
	...
	--OBTIENE EL CODIGO DE PRODUCTO DE CREDITO (TDC,PRESTAMO PERSONAL,CREDINOMINA,REESTRUCTURA,ETC).
	LET cTipoCred = SUBSTR(pNumCredito,1,4);		
		IF cTipoCred = '6000' THEN				
			LET cTipoCred = '6001';
		END IF;	
	
	--SELECCIONA EL TIPO DE PRODUCTO: TDC,PRESTAMO PERSONAL,CREDINOMINA O REESTRUCTURA
	SELECT cod_prod 
	INTO cCodCred 
	FROM "informix".sd_tipprod 
	WHERE abrevia_prod = cTipoCred 
	AND empresa=pEmpresa;

	--EN CASO QUE EL CREDITO NO SEA TDC, SE BUSCARA EN LA SD_AMORTIZA_CREDITOCRD
	IF cCodCred = 'P' OR cCodCred = 'R' THEN 
	
		SELECT SUM(NVL(mora_provi_ordi,0) + NVL(mora_sdo_ordi,0) - NVL(mora_sdo_ordi_pag,0)),
		SUM(NVL(mora_provi_cope,0) + NVL(mora_sdo_cope,0) - NVL(mora_sdo_cope_pag,0))
		INTO dMoraBase , dMoraCopete
		FROM "informix".sd_amortiza_creditocrd
		WHERE empresa = '001'
		AND num_credito = TRIM(pNumCredito)
		AND capital_status IN ('2','7','6');
		
		--OBTIENE LA SUCURSAL DEL CREDITO		
		SELECT sucursal 
		INTO cSucursal 
	...
	WHERE empresa = '001'	
	AND sucursal = TRIM(cSucursal);				
	
	--CALCULO DEL IVA DE INTERES MORA E IVA MORA COPETE
	LET dIvamoraBase = dMoraBase * dIva;
	LET dIvamoraCopete = dMoraCopete * dIva;
...
END PROCEDURE				
DOCUMENT
'DESCRIPCIÓN: SE REALIZA PROCEDIMIENTO PARA OBTENER LOS SALDOS Y PARA CALCULAR EL SALDO MORATORIO BASE Y SALDO MORATORIO COPETE DE UN CREDITO.',
'FECHA DE CREACIÓN: 10 DE ENERO DE 2014',
'BASE DE DATOS: BDICRED',
'CREADOR: ISARAI BOJORQUEZ AGUIRRE',
'VERSION: 20140110.1018';
