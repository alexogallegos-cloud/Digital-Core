CREATE PROCEDURE "informix".sp_comparacaracteresbts(pNombreBan CHAR(160), pNombreBTS CHAR(160))
    ...
    --SET DEBUG FILE TO "/respaldosbd/Pedro/1536/sp_ComparaCaracteresBTS.out";
    --TRACE ON;
...
BEGIN
	...
	LET pNombreBTS= UPPER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(pNombreBTS, 'Á', 'A'), 'É','E'), 'Í', 'I'), 'Ó', 'O'),'Ú','U'),'á', 'A'), 'é','E'), 'í', 'I'), 'ó', 'O'),'ú','U'),' ',''));
	 ...
	 FOR i = 1 TO LENGTH(pNombreBTS)
		LET cCarBan = SUBSTR(pNombreBan,i,1);
		LET cCarBTS = SUBSTR(pNombreBTS,i,1);
...
END PROCEDURE
DOCUMENT
'Compara caracter por caracter el nombre capturado en Bancoppel contra el que envía BTS',
...
CREATE PROCEDURE "informix".sp_comparadesfasamientobanco(pNombreBan CHAR(40), pNombreBTS CHAR(40))
   ...
   --SET DEBUG FILE TO "/respaldosbd/Pedro/1536/sp_ComparaDesfasamientoBanco.out";
  -- TRACE ON;
...
BEGIN
	...
	--VALIDAR PRIMER NOMBRE CARACTER POR CARACTER
	FOR i = 1 TO LENGTH(pNombreBTS) 
	    IF i > LENGTH(pNombreBan) THEN
		    EXIT FOR;
		END IF;
		LET cCarBan = SUBSTR(pNombreBan,iContador,1); 
		LET cCarBTS = SUBSTR(pNombreBTS,i,1);         
		...
		ELSE
		    LET cCarBan = SUBSTR(pNombreBan,iContador + 1,1);
			IF cCarBan = cCarBTS THEN
			    LET iCoicidencia = iCoicidencia + 1;
...
END PROCEDURE
DOCUMENT
'Compara el nombre capturado en Bancoppel contra el que envía BTS con desfasamiento en Banco',
'para obtener el número de coincidencias',
'AUTOR : Dulce Ramirez',
'FECHA : 25/Noviembre/2010',
'Ver.  : 1.1',
'BD    : bdisac',
'VER   : 1.1''AUTOR: 95689966, Pedro Jimenez Guzman',
'FOLIO: 1536-ElimEspBlanBTSWU',
'DESCRIPCION: Se elimina los espacios a la hora de comparar los nombres caracter por caracter ',
'FECHA: 10-02-2016 ',
'SUSTENTO: Se define con Jaime González en el requerimiento',
