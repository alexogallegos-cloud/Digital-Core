CREATE PROCEDURE "informix".sp_ins_serv_solic_hist (pempresa char(3),pnumcte char(20), pcuenta char(20), pproducto char(4), pfecha_inicio date, pfecha_fin date, puser_modif char(20)) 
    ...
    BEGIN
        ...
        --FCP 04-04-2018
        --Se consulta el máximo de veces a enviar historico de estados de cuenta.
        SELECT
            to_number(valor)
		...
		IF (v_cuenta IS NULL OR v_cuenta = '' ) THEN
			...
			LET v_sCodRet = '002'; --Cuenta No Tiene Activo Servicio
			RETURN v_sCodRet;
		
		ELIF (v_hist_env = 'S' ) THEN
			
			LET v_sCodRet = '003'; --Ya se envío la historia de Edos. Cta. al Cte.
                RETURN v_sCodRet;
       
        --FCP 04-04-2018
        --Modificación para contemplar el conteo de veces en que se envia el histórico.
		...
		--Préstamo Personal Y Reestructura
        ...
        ELIF ( v_origen_cuenta <> 0 AND v_origen_cuenta = 2 )
            ...
            FOREACH cursor1 WITH HOLD FOR
                    ...
                    INTO v_fecha_corte
					FROM
                        bdicred@pld_tcp:sd_encabezado2_edocta --(Producción)
					--FROM 
