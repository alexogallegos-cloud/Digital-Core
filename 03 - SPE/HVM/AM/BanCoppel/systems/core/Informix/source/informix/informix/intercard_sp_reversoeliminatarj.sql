CREATE PROCEDURE "informix".sp_reversoeliminatarj(pEmpresa CHAR(3),pNumTarjeta CHAR(20))
...
--SET DEBUG FILE TO '/respaldosbd/felipe/Sps/sp_reversoeliminatarj.out';
--TRACE ON;
...
BEGIN
	...
	IF NVL(pEmpresa ,'') <> '' AND NVL(pNumTarjeta ,'') <> ''  THEN
		...
		IF NVL(cTarjeta,'') <> '' THEN
			...
			DELETE FROM intercard:"informix".bitacoracambiosstatustarjeta
			WHERE tarjeta = pNumTarjeta
			AND CAST(fechahora AS DATE) = dFecha;
		ELSE
			LET cCodRet = '00002';
		END IF;
...
'AUTOR : 94972834',
'FECHA : 12/05/2014',
'SUSTENTO: Eliminación de tarjeta.pdf',
'SOLICITA: Rodolfo Gomez',
'00001: Falta parametro de entrada obligatorio',
'00002: No se encontro el numero de tarjeta',
'647:   No es posible realizar la cancelacion||  de la tarjeta //',
'BD: INTERCARD';

CREATE PROCEDURE "informix".sp_status_tar_transfer(pEmpresa CHAR(3),pNumTarjeta CHAR(20))
	RETURNING CHAR(5)  AS CodRet,
	     	  CHAR(10)  AS Status;
			 
DEFINE cCodRet  	 CHAR(5);
DEFINE cStatusIntTar CHAR(15);
DEFINE cStatusDebTar CHAR(1);
...
--SET DEBUG FILE TO '/respaldosbd/felipe/Sps/sp_status_tar_transfer.out';
--TRACE ON;
...
BEGIN
	...
	IF NVL(pEmpresa ,'') <> '' AND NVL(pNumTarjeta ,'') <> ''  THEN
		...
		IF NVL(cStatusIntTar,'') <> '' AND NVL(cStatusDebTar,'') <> '' THEN
			IF TRIM(cStatusDebTar) = 'A' THEN
				IF TRIM(cStatusIntTar) = 'BLO' OR TRIM(cStatusIntTar) = 'BLT' OR TRIM(cStatusIntTar) = 'NOA' THEN
					LET cCodRet = '642';
				ELSE
					LET cStatusTar  = 'ACTIVA';
				END IF;
			ELIF TRIM(cStatusDebTar) = 'C' THEN
...
'AUTOR : 94972834',
'FECHA : 08/05/2014',
'SUSTENTO: Eliminación de tarjeta.pdf, Reposicion_Tarjeta.pdf',
'SOLICITA: Rodolfo Gomez',
'00001: Falta parametro de entrada obligatorio',
'00002: Falta un estatus',
'642:   No es posible cancelar la tarjeta derivado',
'Nota: para completar el mensaje de error 642 se usa el codigo 643',
'BD: INTERCARD';
