CREATE PROCEDURE "informix".sp_confirmapinoffline 
(
 numtarjeta char (16), trj_stinicio char (01), trj_stfin char (01), sucursal char (04), ip_pc char (15), 
 ejecutivo char (08), secuencia char (06),  tagresponse char (11), TLVResponse char (255)
)

 returning varchar (5) as cod_ret, varchar (70) as men_ret;

	--  Variables de Errores y datos de SP:
	define  isqlerr         integer;
	define  isam_err        integer;
	define  cod_isam		integer;
	define  error_info      varchar(70);
	define  p_cod_ret       varchar(5);
	define  p_mensaje       varchar(70);

	define cno_tarjeta 			varchar(16);
	define cedoinicial_tarjeta 	varchar(1);
	define cedofinal_tarjeta 	varchar(1);
	define csucursal_ofi	 	varchar(4);
	define cipsucursal_ofi	 	varchar(15);
	define cnumejecutivo		varchar(8);
	define dtfechahora		 	datetime year to fraction (5);
	define c_secuencia		varchar(6);
	define c_transrc		varchar(4);
	define existe           integer;

	let cno_tarjeta 			= numtarjeta;
	let cedoinicial_tarjeta 	= trj_stinicio;
	let cedofinal_tarjeta 		= trj_stfin;
	let csucursal_ofi	 		= sucursal;
	let cipsucursal_ofi	 		= ip_pc;
	let cnumejecutivo			= ejecutivo;
	let dtfechahora		 		= CURRENT;
	let c_secuencia		 		= secuencia;
	let c_transrc		 		= '0004';
	let	p_cod_ret				= '00000';
	let p_mensaje				= 'Ejecucion SP exitosa.';
	let isqlerr 				= 0;
	let isam_err 				= 0;
	let cod_isam 				= 0;
	let error_info				= '';
...
BEGIN
    ...
    ON EXCEPTION IN (-201) 
		...
		let cod_isam = isam_err;
		let p_mensaje = error_info;
		set debug file to '/RESPALDOSNEW/sp_confirmapinoffline.out';
		TRACE ON;
   ...
   END EXCEPTION;
	...
	--set debug file to '/informix/OneClick/sp_confirmapinoffline_ramoncito.out';
	--trace on;

	if 
		cno_tarjeta 		is null or cno_tarjeta 			= '' or 
		cedoinicial_tarjeta is null or cedoinicial_tarjeta 	= '' or 
		cedofinal_tarjeta 	is null or cedofinal_tarjeta 	= '' or 
		csucursal_ofi 		is null or csucursal_ofi 		= '' or 
		cipsucursal_ofi 	is null or cipsucursal_ofi 		= '' or 
		cnumejecutivo 		is null or cnumejecutivo 		= ''
		then
			let	p_cod_ret				= '00001';
			let p_mensaje				= 'Parametro de entrada nulo o vacio. Validar.';
			RETURN p_cod_ret, p_mensaje;
		else
			--begin work;
				...
				Select count (cno_tarjeta) 
				into existe
				from "informix".bit_pinoffline 
				where fechahora_insert = current 
				and numtarjeta = cno_tarjeta;
				
				IF existe = 0 THEN
				insert into "informix".bit_pinoffline (numtarjeta, tarjeta_edoinicial, tarjeta_edofinal, sucursal, ip_pc, ejecutivo, fechahora_insert)
					values (cno_tarjeta, cedoinicial_tarjeta, cedofinal_tarjeta, csucursal_ofi, cipsucursal_ofi, cnumejecutivo, current);
				END IF;
				
				--	ATMs:
				insert into "informix".bitacorapinoffline (transaccionorigen, numtarjeta, fechageneracion, estatusscripting, secuenciaorig, respuestatlv, tag_9f5b) 
					values (c_transrc, numtarjeta, current, cedofinal_tarjeta , c_secuencia, trim(TLVResponse), trim(tagresponse));
				
				if cedofinal_tarjeta = '1' then --DESPERTADO
					update hsmcard set pin_offline =cedofinal_tarjeta, last_script_send = null where card_no=numtarjeta;
                    else
						if cedofinal_tarjeta = '2' then --ERROR
							update hsmcard set pin_offline =cedofinal_tarjeta WHERE card_no=numtarjeta;		
                        end if;
                end if;
...
DOCUMENT
'AUTOR: LGMR',
'Proyecto: RQI 13 839 Actualizacion sp_confirmapinoffline para evitar errores de sintaxis.',
'Fecha: 2022/04/26',
'Version: 20220426.01',
'BD: intercard';
