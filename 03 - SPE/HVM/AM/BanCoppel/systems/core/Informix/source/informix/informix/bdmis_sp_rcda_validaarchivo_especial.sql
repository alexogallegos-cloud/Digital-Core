CREATE PROCEDURE "informix".sp_rcda_validaarchivo_especial(pfecha DATE)
...
BEGIN
  ...
  -- trace on;
   ...
   DELETE FROM mi_rcda_cifrascontrol WHERE archivo = vnombrearchivo AND fecha = pfecha;
   FOREACH cursor1 WITH HOLD for
	   ...
	   FROM mi_rcda_infocoppel_cargaespecial
	   
	   IF (SELECT COUNT(*) FROM mi_rcda_cifrascontrol WHERE archivo = vnombrearchivo AND fecha = pfecha ) = 0 THEN
		
			INSERT INTO mi_rcda_cifrascontrol (archivo, fecha, Total_registros) VALUES ( vnombrearchivo, pfecha,1);
			
			if vclave =' ' THEN
			
				UPDATE mi_rcda_cifrascontrol set Total_Solicitud = Total_Solicitud + 1 WHERE archivo = vnombrearchivo AND fecha = pfecha;				
				
				execute PROCEDURE "informix".sp_rcda_esnumerico(vnum_solicitud)
				into vnumero ;
				IF	vnumero = 'F' THEN				
				
					UPDATE mi_rcda_cifrascontrol set Total_Solicitud_err = Total_Solicitud_err + 1 WHERE archivo = vnombrearchivo AND fecha = pfecha;
					CONTINUE FOREACH;
	...
	--nombre_cliente, ap_paterno, ap_materno, fecha_nacimiento
				...
				END IF		
						...
						UPDATE mi_rcda_cifrascontrol set total_cambio_solicitud = total_cambio_solicitud + 1 WHERE archivo = vnombrearchivo AND fecha = pfecha;				
						
						execute PROCEDURE "informix".sp_rcda_esnumerico(vnum_solicitud)
						into vnumero ;
						IF	vnumero = 'F' THEN				
						
							UPDATE mi_rcda_cifrascontrol set total_cambio_solicitud_err = total_cambio_solicitud_err + 1 WHERE archivo = vnombrearchivo AND fecha = pfecha;
							CONTINUE FOREACH;
						...
						UPDATE mi_rcda_cifrascontrol set total_alta = total_alta + 1 WHERE archivo = vnombrearchivo AND fecha = pfecha;				
						
						execute PROCEDURE "informix".sp_rcda_esnumerico(vnum_solicitud)
						into vnumero ;
						IF	vnumero = 'F' THEN				
						
							UPDATE mi_rcda_cifrascontrol set total_alta_err = total_alta_err + 1 WHERE archivo = vnombrearchivo AND fecha = pfecha;
							CONTINUE FOREACH;
						...
						UPDATE mi_rcda_cifrascontrol set total_compra = total_compra + 1 WHERE archivo = vnombrearchivo AND fecha = pfecha;				
						
						IF	vnum_solicitud <> ' ' THEN				
						
							UPDATE mi_rcda_cifrascontrol set total_compra_err = total_compra_err + 1 WHERE archivo = vnombrearchivo AND fecha = pfecha;
							CONTINUE FOREACH;
						...
						UPDATE mi_rcda_cifrascontrol set total_club = total_club + 1 WHERE archivo = vnombrearchivo AND fecha = pfecha;				
						
						IF	vnum_solicitud <> ' ' THEN				
						
							UPDATE mi_rcda_cifrascontrol set total_club_err = total_club_err + 1 WHERE archivo = vnombrearchivo AND fecha = pfecha;
							CONTINUE FOREACH;
		...
		ELSE
			...
			UPDATE mi_rcda_cifrascontrol set Total_registros = Total_registros + 1 WHERE archivo = vnombrearchivo AND fecha = pfecha;
