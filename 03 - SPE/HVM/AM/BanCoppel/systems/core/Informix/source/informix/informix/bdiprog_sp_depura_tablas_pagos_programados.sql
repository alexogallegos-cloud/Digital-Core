CREATE PROCEDURE "informix".sp_depura_tablas_pagos_programados()
...
DEFINE vcodRet 		      VARCHAR(5); 	  -- CODIGO DE RETORNO
DEFINE iSqlErr      	  INTEGER;		  -- ERROR DE INFORMIX
DEFINE iCont			  SMALLINT;       -- CONTADOR DE INTRUCCIONES
DEFINE iMaxCommit		  SMALLINT;       -- NUMERO MAXIMO DE COMMITS
DEFINE dFechaAnioAntes	  DATE;			  -- FECHA DE UN AÃO ATRAS
DEFINE cEstadoActivo      CHAR(2);        -- VARIABLE A UTILIZAR PARA EL ESTADO ACTIVO
DEFINE cEstadoCancelado   CHAR(2);        -- VARIABLE A UTILIZAR PARA EL ESTADO CANCELADO
DEFINE cEstadoFinalizado  CHAR(2);        -- VARIABLE A UTILIZAR PARA EL ESTADO FINALIZADO
DEFINE cEstadoAplicado    CHAR(2);        -- VARIABLE A UTILIZAR PARA EL ESTADO APLICADO
DEFINE cEstadoRechazado   CHAR(2);        -- VARIABLE A UTILIZAR PARA EL ESTADO RECHAZADO

-- LAS SIGUIENTES VARIABLES CORRESPONDEN A LAS COLUMNAS DE LAS TABLAS DE sc_portamovtos y sc_portamovtos_error
DEFINE cReferencia1		CHAR(18);
DEFINE cEmpresa			CHAR(30);
DEFINE cCliente			CHAR(20);
DEFINE cCuentaCargo		CHAR(20);
DEFINE cBancoRef		CHAR(3);
DEFINE cCuentaRef		CHAR(20);
DEFINE cTarjetaRef		CHAR(20);
DEFINE mMontoEnviar		MONEY(16,2);
DEFINE mMontoNomina		MONEY(16,2);
DEFINE cTransaccion		CHAR(4);
DEFINE dFechaEnvio		DATE;
DEFINE dFechaRecibido	DATE;
DEFINE cEstatus			CHAR(2);
DEFINE cFolioSuc		CHAR(16);
DEFINE cUserInsert		CHAR(8);
DEFINE dFechaInsert		DATE;
DEFINE iNumSerial       INTEGER;
DEFINE cError   		CHAR(4);

-- LAS SIGUIENTES VARIABLES CORRESPONDEN A LAS COLUMNAS DE LAS TABLAS DE pp_pagoprog
DEFINE cCvePagoprog      CHAR(10);
DEFINE cNumCte           CHAR(20);
DEFINE cDescripcion      CHAR(20);
DEFINE cCvePago          CHAR(2);
DEFINE cCveCuentaOri     CHAR(2);
DEFINE cCuentaOrigen     CHAR(20);
DEFINE cCveCuentaDest    CHAR(2);
DEFINE cCuentaDestino    CHAR(20);
DEFINE cBancoDestino     CHAR(3);
DEFINE vReferencia1      CHAR(40);
DEFINE cReferencia2      CHAR(20);
DEFINE cConvenio         CHAR(5);
DEFINE mImporte          MONEY(16,2);
DEFINE cRefCobranza      CHAR(40);
DEFINE mImporteIva       MONEY(16,2);
DEFINE iTipoSpei         INTEGER;
DEFINE cConcepto         CHAR(60);
DEFINE dFechaInicio      DATE;
DEFINE cCveFinal         CHAR(2);
DEFINE iNoRepeticiones   INTEGER;
DEFINE dFechaFin         DATE;
DEFINE cCvePrograma      CHAR(2);
DEFINE cTipoDiaria       CHAR(2);
DEFINE iCadaxDias        INTEGER;
DEFINE iCadaxSemanas     INTEGER;
DEFINE cDiasSemana       CHAR(7);
DEFINE cTipoMensual      CHAR(2);
DEFINE iDiaxDelMes       INTEGER;
DEFINE iCadaxMeses       INTEGER;
DEFINE cCveOcurre        CHAR(2);
DEFINE cCveDia           CHAR(2);
DEFINE cCveCanal         CHAR(2);
DEFINE cCveNotifica      CHAR(2);
DEFINE cBenEmail         CHAR(100);
DEFINE cBenCveCompania   CHAR(2);
DEFINE cBenCelular       CHAR(10);
DEFINE cCveNotificaEmi   CHAR(2);
DEFINE cEmiEmail         CHAR(100);
DEFINE cEmiCveCompania   CHAR(2);
DEFINE cEmiCelular       CHAR(10);
DEFINE cMensaje          CHAR(100);
DEFINE cCveEstado        CHAR(2);
DEFINE cUserCancela      CHAR(8);
DEFINE dFechaCancela     DATE;
DEFINE cCanalCancela     CHAR(2);

-- LAS SIGUIENTES VARIABLES CORRESPONDEN A LAS COLUMNAS DE LAS TABLAS DE pp_pagospend
DEFINE iConsecutivo      INTEGER;
DEFINE dFechaProg 		 DATE;
DEFINE cEstado 			 CHAR(2);
DEFINE dFechaAplic 	     DATE;
DEFINE vFolioSuc         CHAR(16);
DEFINE vUserInsert 		 CHAR(8);
DEFINE dFechadInsert	 DATE;
DEFINE vUserCancela 	 CHAR(8);
DEFINE dFechadCancela	 DATE;
DEFINE vCanalCancela     CHAR(2);
DEFINE cCveRechazo 	     CHAR(5);

--  LAS SIGUIENTES VARIABLES CORRESPONDEN A LOS VALORES MAXIMOS DE LA COLUMNA cve_pagoprog EN LA TABLA pp_pagoprog
DEFINE cCvePago01        CHAR(10);
DEFINE cCvePago02        CHAR(10);
DEFINE cCvePago03        CHAR(10);
DEFINE cCvePago04        CHAR(10);
DEFINE cCvePago05        CHAR(10);
DEFINE cCvePago06        CHAR(10);
DEFINE cCvePago07        CHAR(10);
DEFINE cCvePago08        CHAR(10);
...
LET iCont 				= 0;
LET iMaxCommit			= 100;
LET dFechaAnioAntes     = TODAY-370;
LET cEstadoActivo       ='01';
LET cEstadoCancelado    ='02';
LET cEstadoFinalizado   ='04';
LET cEstadoAplicado     ='05';
LET cEstadoRechazado    ='06';
...
LET cCuentaRef			= '';
LET cTarjetaRef		    = '';
LET mMontoEnviar		= 0.00;
LET mMontoNomina		= 0.00;
LET cTransaccion		= '';
LET dFechaEnvio			= null;
LET dFechaRecibido		= null;
LET cEstatus			= '';
LET cFolioSuc			= '';
LET cUserInsert		    = '';
...
LET mImporte 			= 0.00;
LET cRefCobranza        = '';
LET mImporteIva 		= 0.00;
LET iTipoSpei           = 0;
LET cConcepto           = '';
LET dFechaInicio 		= null;
LET cCveFinal           = '';
LET iNoRepeticiones     = 0;
LET dFechaFin 		    = null;
LET cCvePrograma        = '';
LET cTipoDiaria         = '';
LET iCadaxDias          = 0;
...
LET dFechaProg 		    = null;
LET cEstado 			= '';
LET dFechaAplic 	    = null;
LET vFolioSuc        	= '';
LET vUserInsert 		= '';
LET dFechadInsert	 	= null;
LET vUserCancela 	 	= '';
LET dFechadCancela	 	= null;
LET vCanalCancela     	= '';
LET cCveRechazo 	    = '';		
...
BEGIN 
	...
	--INICIA EL PROCESO DE DEPURACION
	

	-- INICIA EL PROCESO DE DEPURACION PARA LA TABLA sc_portamovtos	
	LET iCont = 0;
	BEGIN WORK;
	FOREACH WITH HOLD
		--VALIDAR QUE EXISTAN DATOS PARA DEPURAR EN LA TABLA DE sc_portamovtos PARA EL DIA DE HOY.
		SELECT {+AVOID_FULL (bdicheq: "informix".sc_portamovtos)} referencia1, empresa, cliente, cuenta_cargo, banco_ref, cuenta_ref, tarjeta_ref, monto_enviar, monto_nomina,transaccion, fecha_envio, fecha_recibido, estatus, folio_suc, user_insert, fecha_insert 
		INTO cReferencia1, cEmpresa, cCliente, cCuentaCargo, cBancoRef, cCuentaRef, cTarjetaRef, mMontoEnviar, mMontoNomina, cTransaccion, dFechaEnvio, dFechaRecibido, cEstatus, cFolioSuc, cUserInsert, dFechaInsert
		FROM bdicheq: "informix".sc_portamovtos
		WHERE estatus <> cEstadoActivo
		AND fecha_insert < dFechaAnioAntes

		--SE RESPALDAN LOS DATOS A DEPURAR DE LA TABLA sc_portamovtos.
		INSERT INTO bdicheq: "informix".sc_portamovtos_his(referencia1, empresa, cliente, cuenta_cargo, banco_ref, cuenta_ref, tarjeta_ref, monto_enviar, monto_nomina,transaccion, fecha_envio, fecha_recibido, estatus, folio_suc, user_insert, fecha_insert)
		VALUES (cReferencia1, cEmpresa, cCliente, cCuentaCargo, cBancoRef, cCuentaRef, cTarjetaRef, mMontoEnviar, mMontoNomina, cTransaccion, dFechaEnvio, dFechaRecibido, cEstatus, cFolioSuc, cUserInsert, dFechaInsert);

		--SE DEPURAN LOS MOVIMIENTOS DE PORTABILIDAD MENORES A UN MES.
		DELETE FROM bdicheq: "informix".sc_portamovtos
		WHERE referencia1 = cReferencia1;
	...
	-- INICIA EL PROCESO DE DEPURACION PARA LA TABLA sc_portamovtos_error
	FOREACH WITH HOLD
		--VALIDAR QUE EXISTAN DATOS EN LA TABLA DE sc_portamovtos_error PARA EL DIA DE HOY.
		SELECT {+AVOID_FULL (bdicheq: "informix".sc_portamovtos_error)} num_serial, referencia1, empresa, cliente, cuenta_cargo, banco_ref, cuenta_ref, tarjeta_ref, monto_enviar, monto_nomina,transaccion, fecha_envio, fecha_recibido, estatus, folio_suc, user_insert, fecha_insert, error
		INTO iNumSerial, cReferencia1, cEmpresa, cCliente, cCuentaCargo, cBancoRef, cCuentaRef, cTarjetaRef, mMontoEnviar, mMontoNomina, cTransaccion, dFechaEnvio, dFechaRecibido, cEstatus, cFolioSuc, cUserInsert, dFechaInsert, cError
		FROM bdicheq: "informix".sc_portamovtos_error
		WHERE fecha_insert < dFechaAnioAntes
		
		--SE RESPALDAN LOS DATOS A DEPURAR DE LA TABLA sc_portamovtos_error.
		INSERT INTO bdicheq: "informix".sc_portamovtos_error_his(num_serial, referencia1, empresa, cliente, cuenta_cargo, banco_ref, cuenta_ref, tarjeta_ref, monto_enviar, monto_nomina, transaccion, fecha_envio, fecha_recibido, estatus, folio_suc, user_insert, fecha_insert, error )
		VALUES (iNumSerial, cReferencia1, cEmpresa, cCliente, cCuentaCargo, cBancoRef, cCuentaRef, cTarjetaRef, mMontoEnviar, mMontoNomina, cTransaccion, dFechaEnvio, dFechaRecibido, cEstatus, cFolioSuc, cUserInsert, dFechaInsert, cError);

		--SE DEPURAN LOS MOVIMIENTOS DE PORTABILIDAD QUE TUVIERON ERRORES MENORES A UN MES.
		DELETE FROM bdicheq: "informix".sc_portamovtos_error
		WHERE referencia1 = cReferencia1; 
	...
	-- INICIA EL PROCESO DE DEPURACION PARA LA TABLA pp_pagospend

	-- OBTIENE EL VALOR MAXIMO DE CADA CLAVE DE PAGO PROGRAMADO 
	SELECT NVL(MAX(cve_pagoprog),'0') INTO cCvePago01 FROM "informix".pp_pagoprog  WHERE cve_pagoprog LIKE '01%';
	SELECT NVL(MAX(cve_pagoprog),'0') INTO cCvePago02 FROM "informix".pp_pagoprog  WHERE cve_pagoprog LIKE '02%';
	SELECT NVL(MAX(cve_pagoprog),'0') INTO cCvePago03 FROM "informix".pp_pagoprog  WHERE cve_pagoprog LIKE '03%';
	SELECT NVL(MAX(cve_pagoprog),'0') INTO cCvePago04 FROM "informix".pp_pagoprog  WHERE cve_pagoprog LIKE '04%';
	SELECT NVL(MAX(cve_pagoprog),'0') INTO cCvePago05 FROM "informix".pp_pagoprog  WHERE cve_pagoprog LIKE '05%';
	SELECT NVL(MAX(cve_pagoprog),'0') INTO cCvePago06 FROM "informix".pp_pagoprog  WHERE cve_pagoprog LIKE '06%';
	SELECT NVL(MAX(cve_pagoprog),'0') INTO cCvePago07 FROM "informix".pp_pagoprog  WHERE cve_pagoprog LIKE '07%';
	SELECT NVL(MAX(cve_pagoprog),'0') INTO cCvePago08 FROM "informix".pp_pagoprog  WHERE cve_pagoprog LIKE '08%';

	-- INICIA EL PROCESO DE DEPURACION PARA LA TABLA pp_pagoprog
	FOREACH WITH HOLD
		--VALIDAR QUE EXISTAN DATOS EN LA TABLA pp_pagoprog PARA EL DIA DE HOY.
		SELECT {+AVOID_FULL (bdiprog: "informix".pp_pagoprog)} cve_pagoprog, num_cte, descripcion, cve_pago, cve_cuenta_ori, cuenta_origen, cve_cuenta_dest, cuenta_destino, banco_destino, referencia1, referencia2, 
			convenio, importe, ref_cobranza, importe_iva, tipo_spei, concepto, fecha_inicio, cve_final, no_repeticiones, fecha_fin, cve_programa, tipo_diaria, cada_x_dias, 
			cada_x_semanas, dias_semana, tipo_mensual, dia_x_del_mes, cada_x_meses, cve_ocurre, cve_dia, cve_canal, cve_notifica, ben_email, ben_cve_compania, ben_celular, 
			cve_notifica_emi, emi_email, emi_cve_compania, emi_celular, mensaje, cve_estado, user_insert, fecha_insert, user_cancela, fecha_cancela, canal_cancela
		INTO cCvePagoprog, cNumCte, cDescripcion, cCvePago, cCveCuentaOri, cCuentaOrigen, cCveCuentaDest, cCuentaDestino, cBancoDestino, vReferencia1, cReferencia2, 
			cConvenio, mImporte, cRefCobranza, mImporteIva, iTipoSpei, cConcepto, dFechaInicio, cCveFinal, iNoRepeticiones, dFechaFin, cCvePrograma, cTipoDiaria, iCadaxDias, 
			iCadaxSemanas, cDiasSemana, cTipoMensual, iDiaxDelMes, iCadaxMeses, cCveOcurre, cCveDia, cCveCanal, cCveNotifica, cBenEmail, cBenCveCompania, cBenCelular, 
			cCveNotificaEmi, cEmiEmail, cEmiCveCompania, cEmiCelular, cMensaje, cCveEstado, cUserInsert, dFechaInsert, cUserCancela, dFechaCancela, cCanalCancela
		FROM "informix".pp_pagoprog 
	    WHERE cve_estado IN(cEstadoCancelado,cEstadoFinalizado,cEstadoAplicado,cEstadoRechazado)  
	    AND fecha_insert < dFechaAnioAntes
	    AND cve_pagoprog NOT IN (cCvePago01,cCvePago02,cCvePago03,cCvePago04,cCvePago05,cCvePago06,cCvePago07,cCvePago08)
	    
		FOREACH WITH HOLD
			--VALIDAR QUE EXISTAN DATOS EN LA TABLA pp_pagospend PARA EL DIA DE HOY.
			...
			INTO cCvePagoprog, iConsecutivo, dFechaProg, cEstado, dFechaAplic, vFolioSuc, vUserInsert, dFechadInsert, vUserCancela, dFechadCancela, vCanalCancela, cCveRechazo
			FROM "informix".pp_pagospend 
	        WHERE cve_pagoprog = cCvePagoprog

			--SE RESPALDAN LOS DATOS A DEPURAR DE LA TABLA pp_pagospend.
			INSERT INTO "informix".pp_pagospend_his(cve_pagoprog, consecutivo, fecha_prog, estado, fecha_aplic, folio_suc, user_insert, fecha_insert, user_cancela, fecha_cancela, canal_cancela, cve_rechazo)
			VALUES (cCvePagoprog, iConsecutivo, dFechaProg, cEstado, dFechaAplic, vFolioSuc, vUserInsert, dFechadInsert, vUserCancela, dFechadCancela, vCanalCancela, cCveRechazo);

		    --SE DEPURAN LOS PAGOS PENDIENTES CON ESTADOs: CANCELADO, FINALIZADO, APLICADO Y RECHAZADO QUE TIENEN MAS DE DE UN AÃO.
		    DELETE FROM "informix".pp_pagospend
			WHERE cve_pagoprog = cCvePagoprog
		...
		--SE RESPALDAN LOS DATOS A DEPURAR DE LA TABLA pp_pagoprog.
    	INSERT INTO "informix".pp_pagoprog_his(cve_pagoprog, num_cte, descripcion, cve_pago, cve_cuenta_ori, cuenta_origen, cve_cuenta_dest, cuenta_destino, banco_destino, referencia1, referencia2, 
		    convenio, importe, ref_cobranza, importe_iva, tipo_spei, concepto, fecha_inicio, cve_final, no_repeticiones, fecha_fin, cve_programa, tipo_diaria, cada_x_dias, 
		    cada_x_semanas, dias_semana, tipo_mensual, dia_x_del_mes, cada_x_meses, cve_ocurre, cve_dia, cve_canal, cve_notifica, ben_email, ben_cve_compania, ben_celular, 
		    cve_notifica_emi, emi_email, emi_cve_compania, emi_celular, mensaje, cve_estado, user_insert, fecha_insert, user_cancela, fecha_cancela, canal_cancela)
    	VALUES(cCvePagoprog, cNumCte, cDescripcion, cCvePago, cCveCuentaOri, cCuentaOrigen, cCveCuentaDest, cCuentaDestino, cBancoDestino, vReferencia1, cReferencia2, 
			cConvenio, mImporte, cRefCobranza, mImporteIva, iTipoSpei, cConcepto, dFechaInicio, cCveFinal, iNoRepeticiones, dFechaFin, cCvePrograma, cTipoDiaria, iCadaxDias, 
			iCadaxSemanas, cDiasSemana, cTipoMensual, iDiaxDelMes, iCadaxMeses, cCveOcurre, cCveDia, cCveCanal, cCveNotifica, cBenEmail, cBenCveCompania, cBenCelular, 
			cCveNotificaEmi, cEmiEmail, cEmiCveCompania, cEmiCelular, cMensaje, cCveEstado, cUserInsert, dFechaInsert, cUserCancela, dFechaCancela, cCanalCancela);
		
		--SE DEPURAN LOS PAGOS PROGRAMADOS CON ESTADOS: CANCELADO, FINALIZADO, APLICADO Y RECHAZADO QUE TIENEN MAS DE DE UN AÃO.
		DELETE FROM	"informix".pp_pagoprog
		WHERE cve_pagoprog = cCvePagoprog;
...
END;
END PROCEDURE
DOCUMENT 'AUTOR: Luis Enrique Orozco Cosme',
'FECHA: 04/08/2023',
'VERSION:1.0.0',
'BD: bdiprog',
'DESCRIPCION: REALIZA LA DEPURACION DE LAS TABLAS sc_portamovtos, sc_portamovtos_error, pp_pagoprog Y pp_pagospend',
'TIENE COMO OBJETIVO MANTENER LOS REGISTROS CON ESTATUS ACTIVOS. LOS REGISTROS CON ESTATUS RECHAZADO, FINALIZADO, CANCELADO Y APLICADO SOLO SE MANTIENEN POR 1 AÃO EN LA TABLA';
