CREATE PROCEDURE "informix".sp_buscaqueda_folio_csuac(opcion INTEGER, pFolioCSUAC CHAR(11),pNumCliente CHAR(20),pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
		DATE 		AS FechaCaptura,
		CHAR(11) 	AS FolioCsuac,
		INTEGER  	AS FkyReglaNegocio,
		CHAR(625) 	AS Predictamen,
		MONEY(16,2) AS ImporteReclamado,
		CHAR(26) 	AS Nombre1,
		CHAR(26) 	AS Nombre2,
		CHAR(26) 	AS ApellPaterno,
		CHAR(26) 	AS ApellMaterno,
		INTEGER  	AS Pky_producto,
		CHAR(255)	AS DescripcionProducto,
		CHAR(20) 	AS NumeroCuenta,
		CHAR(16) 	AS NumeroTarjeta,
		INTEGER  	AS FkyOrigenEvento,
		CHAR(50) 	AS DescripcionEvento,
		INTEGER  	AS PkyEstatusAclaracion,
		CHAR(255)	AS DescripcionEstatus,
		CHAR(20) 	AS NumCliente,
		CHAR(5)						 AS NumSucursal,
		CHAR(40)                     AS DescripcionSuc,
		INTEGER                      AS RangoImporte,
		INTEGER                      AS OrigenEvent,
		CHAR(50)                     AS DescripcionOrgEvent,
		DATETIME YEAR TO FRACTION(5) AS FechaInicio,
		CHAR(30)                     AS FolioSuc,
		DATETIME YEAR TO FRACTION(5) AS FechaHora,
		CHAR(13)                     AS Telefono;
	...
	DEFINE iFkyReglaNegocio INTEGER;
	DEFINE cPredictamen CHAR(625);
	DEFINE mImporteReclamado money(16,2);
	...
	DEFINE cDescripcionProducto CHAR(255);
	DEFINE cNumeroCuenta CHAR(20);
	DEFINE cNumeroTarjeta CHAR(16);

	DEFINE iFkyOrigenEvento INTEGER;
	DEFINE cDescripcionEvento CHAR(50);

	DEFINE iPkyEstatusAclaracion INTEGER;
	DEFINE cDescripcionEstatus CHAR(255);
	DEFINE cNumCliente CHAR(20);

	DEFINE cNumSucursal CHAR(5);
	DEFINE iPkyRangoImporte INTEGER;
	DEFINE iPkyOrigenEvent INTEGER;
	DEFINE cDescripcionOrgEvent CHAR(50);
	DEFINE dFechaInicio DATETIME YEAR TO FRACTION(5);
	DEFINE cDescripcionSuc CHAR(40);
	...
	LET dFechaCaptura = '';
	LET cFolioCsuac = '';
	LET iFkyReglaNegocio = 0;
	LET cPredictamen = '';
	LET mImporteReclamado = 0.00;
	...
	LET iFkyOrigenEvento = 0;
	LET cDescripcionEvento = '';

	LET iPkyEstatusAclaracion = 0;
	LET cDescripcionEstatus = '';
	LET cNumCliente = '';

	LET cNumSucursal = '';
	LET iPkyOrigenEvent = 0;
	LET cDescripcionOrgEvent = '';
	LET dFechaInicio = '';
	LET cFolioSuc = '';
	LET dFechaHora = '';

	LET iPkyRangoImporte = 0;
	LET cDescripcionSuc = '';
	LET cTelefono = '';
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,dFechaCaptura,cFolioCsuac,iFkyReglaNegocio,cPredictamen,mImporteReclamado,cNombre1,cNombre2,cApellPaterno,cApellMaterno,
			iPkyProducto,cDescripcionProducto,cNumeroCuenta,cNumeroTarjeta,iFkyOrigenEvento,cDescripcionEvento,iPkyEstatusAclaracion,cDescripcionEstatus,cNumCliente,
			cNumSucursal,cDescripcionSuc,iPkyRangoImporte,iPkyOrigenEvent,cDescripcionOrgEvent,dFechaInicio,cFolioSuc,dFechaHora,cTelefono;
		END EXCEPTION;

		--SET DEBUG FILE TO '/tmp/mfinis/sp_buscaqueda_folio_csuac.out';
		--TRACE ON;

		IF opcion = '' OR pRegistros IS NULL OR pRecuperacion IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet,dFechaCaptura,cFolioCsuac,iFkyReglaNegocio,cPredictamen,mImporteReclamado,cNombre1,cNombre2,cApellPaterno,cApellMaterno,
			iPkyProducto,cDescripcionProducto,cNumeroCuenta,cNumeroTarjeta,iFkyOrigenEvento,cDescripcionEvento,iPkyEstatusAclaracion,cDescripcionEstatus,cNumCliente,
			cNumSucursal,cDescripcionSuc,iPkyRangoImporte,iPkyOrigenEvent,cDescripcionOrgEvent,dFechaInicio,cFolioSuc,dFechaHora,cTelefono;
		...
		SELECT resp_estimada
		INTO iPkyRangoImporte
		FROM acl_rango_importe
		WHERE fky_id_regla = 714 and (1800 between rango_menor and rango_mayor);

		IF opcion = 1 THEN

			LET cNumSucursal = '';

			SELECT acl.fechacaptura,acl.folio_csuac,acl.fky_regla_negocio,acl.predictamen,acl.importeoriginal,
				   clt.nombre1,clt.nombre2,clt.apell_paterno,clt.apell_materno,
				   prod.pky_producto,prod.descripcion AS descripcion_producto,prod.numero_cuenta,prod.numero_tarjeta,
				   org.fky_origen_evento,org.descripcion,
				   st.pky_estatus_aclaracion,st.descripcion AS descripcion_estatus,acl.num_cliente,
				   mov.num_sucursal,
				   mov.ref_comercio,
				   event.pky_origen_evento, event.descripcion AS descripcion_even,
				   acl.fechainicio,
				   mov.folio_suc, mov.fechahora,
				   prod.telefonotransfer
			INTO dFechaCaptura,cFolioCsuac,iFkyReglaNegocio,cPredictamen,mImporteReclamado,
				 cNombre1,cNombre2,cApellPaterno,cApellMaterno,
				 iPkyProducto,cDescripcionProducto,cNumeroCuenta,cNumeroTarjeta,
				 iFkyOrigenEvento,cDescripcionEvento,
				 iPkyEstatusAclaracion,cDescripcionEstatus,cNumCliente,
				 cNumSucursal,
				 cDescripcionSuc,
				 iPkyOrigenEvent,cDescripcionOrgEvent,
				 dFechaInicio,
				 cFolioSuc,dFechaHora,
				 cTelefono
			FROM bdiaclaracion:"informix".acl_aclaracion AS acl
			INNER JOIN bdinteg:"informix".si_cliente AS clt ON clt.numcte = acl.num_cliente
			INNER JOIN bdiaclaracion:"informix".acl_producto AS prod ON prod.pky_producto = acl.fky_producto
			INNER JOIN bdiaclaracion:"informix".acl_tipo_evento AS org ON acl.fky_tipo_evento = org.pky_tipo_evento
            INNER JOIN bdiaclaracion:"informix".acl_origen_evento AS event ON event.pky_origen_evento = org.fky_origen_evento
			INNER JOIN bdiaclaracion:"informix".acl_estatus_aclaracion AS st ON st.pky_estatus_aclaracion = acl.fky_estatus_aclaracion
			INNER JOIN bdiaclaracion:"informix".acl_movimiento AS mov ON mov.folio_csuac = acl.folio_csuac
            AND mov.fky_padre IS NULL AND mov.folio_suc IS NOT NULL AND mov.duplicado = 0
		...
		ELIF opcion = 2 THEN
			...
			FOREACH
				...
				SELECT SKIP pRegistros FIRST pRecuperacion acl.fechacaptura,acl.folio_csuac,acl.fky_regla_negocio,acl.predictamen,acl.importeoriginal,
				   clt.nombre1,clt.nombre2,clt.apell_paterno,clt.apell_materno,
				   prod.pky_producto,prod.descripcion AS descripcion_producto,prod.numero_cuenta,prod.numero_tarjeta,
				   org.fky_origen_evento,org.descripcion,
				   st.pky_estatus_aclaracion,st.descripcion AS descripcion_estatus,acl.num_cliente,
				...
				INNER JOIN bdiaclaracion:"informix".acl_movimiento AS mov ON mov.folio_csuac = acl.folio_csuac
				AND mov.fky_padre IS NULL AND mov.folio_suc IS NOT NULL AND mov.duplicado = 0
				WHERE acl.num_cliente = pNumCliente AND acl.folio_csuac IS NOT NULL
				ORDER BY 1 DESC
				...
				RETURN cCodRet,dFechaCaptura,cFolioCsuac,iFkyReglaNegocio,cPredictamen,mImporteReclamado,cNombre1,cNombre2,cApellPaterno,cApellMaterno,
				iPkyProducto,cDescripcionProducto,cNumeroCuenta,cNumeroTarjeta,iFkyOrigenEvento,cDescripcionEvento,iPkyEstatusAclaracion,cDescripcionEstatus,cNumCliente,
				cNumSucursal,cDescripcionSuc,iPkyRangoImporte,iPkyOrigenEvent,cDescripcionOrgEvent,dFechaInicio,cFolioSuc,dFechaHora,cTelefono WITH RESUME;
...
'DESCRIPCION: Busqueda de cliente y por folo csuac.',
'AUTOR: Rodolfo Conde Flores',
'FECHA: 01/11/2018',
'DESCRIPCION: Se realiza ajuste para consultar y enviar numero de sucursal y ref_comercio de la tabla acl_movimiento',
'BD: bdiaclaracion';
