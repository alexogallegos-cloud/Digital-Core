CREATE PROCEDURE "informix".sp_con_constiporeporteprocesosuc(pUsuario CHAR(8), pIdFuncion CHAR(10))
			RETURNING CHAR(5) AS codret,
			INTEGER AS id_reporte, 
			CHAR(30) AS desc_reporte					
		
			DEFINE cCodRet CHAR(5);
			DEFINE iSqlErr INTEGER;
			DEFINE iReporte INTEGER;
			DEFINE cDescReporte CHAR(30);
			...
			BEGIN	
				ON EXCEPTION SET iSqlErr
						LET cCodRet = iSqlErr;
						RETURN cCodRet, iReporte, cDescReporte;
				END EXCEPTION;
			
				--SET DEBUG FILE TO '/tmp/mfinis/sp_con_constiporeporteprocesosuc.out';
				--TRACE ON;
				...
				FOREACH
						SELECT id_reporte, desc_reporte
								INTO iReporte, cDescReporte
								FROM bdicnweb:sw_con_tiporeporteprocesosuc			
						LET iNoRegistros = iNoRegistros + 1;		
						RETURN cCodRet, iReporte, UPPER(TRIM(cDescReporte)) WITH RESUME;		
				END FOREACH;
...
'DESCRIPCION: SPL que consulta Reporte Procesos Sucursal para llenado de combo',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_consultacajageneral(pUsuario CHAR(8), pIdFuncion CHAR(10), pPlaza CHAR(3))
		RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cCodProveedor, cDescripcion;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ope_consultacajageneral.out';
		--TRACE ON;
		...
		SELECT cod_proveedor, descripcion
			INTO cCodProveedor, cDescripcion
...
'DESCRIPCION: SPL que consulta las cajas generales de las sucursales.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_consultaentidadfederativa(pUsuario CHAR(8), pIdFuncion CHAR(10), pPais CHAR(3), pEstado CHAR(2))
		RETURNING CHAR(5) AS codret,		
		CHAR(30) AS nombre;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ope_consultaentidadfederativa.out';
		--TRACE ON;
...
'MODULO: OPERACION',
'FUNCIONALIDAD: Reportes Convenios SAC',
'DESCRIPCION: SPL que realiza la consulta de las entidades federaticas.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_gc_altamodificacionbines(pUsuario CHAR(8), pIdFuncion CHAR(10), pBin CHAR(6), pTipoOperacion CHAR(1), pIdBanco CHAR(2), pCveBanco CHAR(3), pNombreBanco CHAR(25), pCredDeb CHAR(1))
		RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		ON EXCEPTION IN (-268) -- EL BIN QUE DESEA INSERTAR YA EXISTE
			LET cCodRet = '00755';
			RETURN cCodRet, iNoRegistros;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_gc_altamodificacionbines.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pBin = '' OR pTipoOperacion = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, iNoRegistros;
		END IF;
		...
		IF pTipoOperacion NOT IN ('0', '1', '2') THEN
			LET cCodRet = '00044';
			RETURN cCodRet, iNoRegistros;
		END IF;
		
		IF LENGTH(pBin) < 6 THEN
			LET cCodRet = '00485';
			RETURN cCodRet, iNoRegistros;
		END IF;
		
		IF pTipoOperacion IN ('1', '2') THEN
			IF pIdBanco = '' OR pCveBanco = '' OR pNombreBanco = '' OR pCredDeb = '' THEN
				LET cCodRet = '00003';
				RETURN cCodRet, iNoRegistros;
			END IF;
			
			IF LENGTH(pIdBanco) < 2 OR LENGTH(pCveBanco) < 3 THEN
				LET cCodRet = '00485';
				RETURN cCodRet, iNoRegistros;
			END IF;
			
			IF LOWER(pCredDeb) NOT IN ('c', 'd') THEN
				LET cCodRet = '00044';
				RETURN cCodRet, iNoRegistros;
			END IF;
		END IF;
		
		IF pTipoOperacion = '0' THEN -- BAJA DEL BIN
			
			SET LOCK MODE TO WAIT 3;
			
				SELECT bin, id_bco, creditodebito, banco_prosa, cve_banco
				INTO pBin, pIdBanco, pCredDeb, pNombreBanco, pCveBanco
				FROM bdicheq:"informix".sc_bines
				WHERE bin = pBin;
			
			
			INSERT INTO "informix".sc_bines_hist(bin_banco, status_operacion, id_bco, creditodebito, banco_prosa, cve_banco, fecha_operacion, usuario)
			VALUES (pBin, pTipoOperacion, pIdBanco, LOWER(pCredDeb), pNombreBanco, pCveBanco, dFechaActual, pUsuario);

			DELETE FROM bdicheq:"informix".sc_bines WHERE bin = pBin;
			LET iNoRegistros = DBINFO('sqlca.sqlerrd2');
			
				
		ELIF pTipoOperacion = '1' THEN -- ALTA DEL BIN
		
			SET LOCK MODE TO WAIT 3;
			
			INSERT INTO bdicheq:"informix".sc_bines(bin, id_bco, creditodebito, banco_prosa, cve_banco, user_insert, fecha_insert) 
			VALUES(pBin, pIdBanco, LOWER(pCredDeb), pNombreBanco, pCveBanco, pUsuario, DATE(dFechaActual));
			
			INSERT INTO "informix".sc_bines_hist(bin_banco,status_operacion,id_bco,creditodebito,banco_prosa,cve_banco,fecha_operacion,usuario)
			VALUES (pBin,pTipoOperacion,pIdBanco,LOWER(pCredDeb),pNombreBanco,pCveBanco,DATE(dFechaActual),pUsuario);
			
			
			LET iNoRegistros = DBINFO('sqlca.sqlerrd2');
			
		ELIF pTipoOperacion = '2' THEN -- MODIFICACIÓN DEL BIN
		
			SET LOCK MODE TO WAIT 3;
			
     		SELECT bin, id_bco, creditodebito, banco_prosa, cve_banco
			INTO cBin, cIdBco, cCreditoDebito, cBancoProsa, cClaveBanco
			FROM bdicheq:"informix".sc_bines
			WHERE bin = pBin;

		    UPDATE bdicheq:"informix".sc_bines
			SET id_bco = pIdBanco
				, creditodebito = LOWER(pCredDeb)
				, banco_prosa = pNombreBanco
				, cve_banco = pCveBanco
			WHERE bin = pBin;
			
			INSERT INTO "informix".sc_bines_hist(bin_banco, status_operacion, id_bco, creditodebito, banco_prosa, cve_banco, fecha_operacion, usuario)
			VALUES (cBin, pTipoOperacion, cIdBco, LOWER(cCreditoDebito), cBancoProsa, cClaveBanco, dFechaActual, pUsuario);
			RETURN cCodRet, iNoRegistros;
...
'DESCRIPCION: SP que da de alta, modifica o elimina un bin de banco y lo registra en bitacora para su posterior consulta',
'BD: bdicnweb',
'AUTOR: Guadalupe Hernández Pérez',
'FECHA: 30/05/2016',
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Catalogo de bines',
'DESCRIPCION: Se realiza una modificacion al momento de insercion en la bitacora del historico solo se vea reaflijado en el mantenimiento de un bin de banco',
'BD: bdicnweb',
'AUTOR: Sandra Cano',
'FECHA: 03/06/2016',
'DESCRIPCION: Se realiza modificacion al SPL para guardar el historico el valor del registro antes de ser actualizado',
'BD: bdicnweb',
'AUTOR: Sandra Cano',
'FECHA: 08/06/2016',
'DESCRIPCION: Se realiza modificacion al SPL para guardar el historico el alta del BIN',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_gc_consultabines(pUsuario CHAR(8), pIdFuncion CHAR(10), pTipoOperacion CHAR(1), pBin CHAR(6), pPeriodoDel DATE, pPeriodoAl DATE, pCreditoDebito CHAR(1), pClaveBanco CHAR(3), pIdentificador CHAR(1), pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
					CHAR(6) AS bin_banco,
					CHAR(2) AS id_banco,
					CHAR(1) AS creditodebito,
					CHAR(25) AS nombre_banco,
					CHAR(3) AS clave_banco,
					CHAR(20) AS status_operacion,
	...
	DEFINE iSqlErr INTEGER;
	DEFINE iNoRegistros INTEGER;
	DEFINE cBinBanco CHAR(6);
	DEFINE cIdBanco CHAR(2);
	DEFINE cCreditoDebito CHAR(1);
	DEFINE cNombreBanco CHAR(25);
	DEFINE cClaveBanco CHAR(3);
	DEFINE cStatusOperacion CHAR(20);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cBinBanco, cIdBanco, cCreditoDebito, cNombreBanco, cClaveBanco, cStatusOperacion, dFechaOperacion, cEjecutivo, cNombreEjecutivo;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_gc_consultabines.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pTipoOperacion = '' OR pRegistros IS NULL OR pRecuperacion IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cBinBanco, cIdBanco, cCreditoDebito, cNombreBanco, cClaveBanco, cStatusOperacion, dFechaOperacion, cEjecutivo, cNombreEjecutivo;
		END IF;
		...
		IF pTipoOperacion = '2' AND pIdentificador = '1' THEN
			IF pRegistros IS NULL OR pRecuperacion IS NULL OR pPeriodoDel IS NULL OR pPeriodoAl IS NULL THEN
				LET cCodRet = '00003';
				RETURN cCodRet, cBinBanco, cIdBanco, cCreditoDebito, cNombreBanco, cClaveBanco, cStatusOperacion, dFechaOperacion, cEjecutivo, cNombreEjecutivo;
			END IF;
		...
		IF pTipoOperacion = '1' THEN
			...
			SELECT a.bin, a.id_bco, UPPER(a.creditodebito) AS creditodebito, UPPER(a.banco_prosa) AS nombre_banco, a.cve_banco
			INTO cBinBanco, cIdBanco, cCreditoDebito, cNombreBanco, cClaveBanco
			FROM bdicheq:"informix".sc_bines a
			WHERE a.bin = pBin;
		...
		ELIF pTipoOperacion = '2' THEN
			...
			IF  pIdentificador = '0' THEN -- 0 = sin fechas 
			FOREACH
				SELECT SKIP pRegistros FIRST pRecuperacion *					
						INTO cBinBanco, cIdBanco, cCreditoDebito, cNombreBanco, cClaveBanco, cStatusOperacion, dFechaOperacion, cEjecutivo, cNombreEjecutivo
					FROM(          
						SELECT c.bin, c.id_bco, UPPER(c.creditodebito) AS creditodebito, UPPER(c.banco_prosa) AS nombre_banco
							, c.cve_bAnco
							, 'REG. ACTUAL' AS estatus, DATE(c.fecha_insert) AS fecha
					...
					UNION
						...
						FROM "informix".sc_bines_hist a LEFT JOIN bdinteg:"informix".si_ejecut b ON b.ejecutivo = a.usuario
					...
					LET iNoRegistros = iNoRegistros + 1;
					RETURN cCodRet, cBinBanco, cIdBanco, cCreditoDebito, cNombreBanco, cClaveBanco, cStatusOperacion, dFechaOperacion, cEjecutivo, cNombreEjecutivo WITH RESUME;
			END FOREACH;
			
			ELIF pIdentificador = '1' THEN -- CON FECHAS 
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Catalogo de bines',
'DESCRIPCION: Consulta de datos de bines',
'BD: bdicnweb',
'AUTOR: Guadalupe Angélica Hernández Pérez',
'FECHA: 18/04/2016',
'DESCRIPCION: Se realizo una modificacion a la consulta por medio del periodo de fechas permitidas y otra consulta sin el periodo de fechas permitidas de datos de bines.',
'BD: bdicnweb',
'AUTOR: Sandra Cano',
'FECHA: 08/06/2016',
'DESCRIPCION: Se realiza modificacion al SPL, para mostrar el valor Registro Actual en el estatus, cuando se consulta sobre la tabla principal de bines.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_gc_obtiene_catalogos_usuario(pUsuario CHAR(8), pIdFuncion CHAR(10), pTipoOperacion SMALLINT, pIdCatalogo SMALLINT)
		RETURNING CHAR(5) AS codret,
	...
	DEFINE cDescCatalogo CHAR(50);
	DEFINE iIdABC INTEGER;
	DEFINE cDescTipoOperacionABC CHAR(15);
	DEFINE iNoRegistros INTEGER;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, iIdCatalogo, cDescCatalogo;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_gc_obtiene_catalogos_usuario.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pTipoOperacion IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet, iIdCatalogo, cDescCatalogo;
		END IF;
		...
		IF pTipoOperacion = 1 THEN
			FOREACH SELECT a.id_catalogo, b.descripcion
					INTO iIdCatalogo, cDescCatalogo
					FROM bdicnweb:'informix'.sw_gc_catalogos_funcionalidades a, bdicnweb:'informix'.sw_gc_catalogos b
					WHERE id_funcion IN (
										SELECT a.id_funcion
										FROM bdinteg:'informix'.si_seg_usuarios_funciones a, bdinteg:'informix'.si_seg_funciones b
										WHERE id_usuario = pUsuario
											AND a.id_funcion[1, 3] =  'GEC'
											AND a.status = '1'
											AND b.id_funcion = a.id_funcion
											AND b.id_submodulo = 31)
						AND b.id_catalogo = a.id_catalogo
					GROUP BY a.id_catalogo, b.descripcion
					
					RETURN cCodRet, iIdCatalogo, cDescCatalogo WITH RESUME;
					LET iNoRegistros = iNoRegistros + 1;
		...
		ELIF pTipoOperacion = 2 THEN -- consulta de los tipos de operaciones abc que puede hacer
		
			IF pIdCatalogo IS NULL THEN
				LET cCodRet = '00003';
				RETURN cCodRet, iIdCatalogo, cDescCatalogo;
			END IF;
		
			FOREACH SELECT c.id_tipo_abc, c.desc_tipo_abc
					INTO iIdABC, cDescTipoOperacionABC
					FROM bdicnweb:"informix".sw_gc_catalogos_funcionalidades a, bdinteg:'informix'.si_seg_usuarios_funciones b, "informix".sw_gc_catalogos_operacionesabc c
					WHERE a.id_catalogo = pIdCatalogo
						AND b.id_usuario = pUsuario
						AND b.status = '1'
						AND b.id_funcion = a.id_funcion
						AND c.id_tipo_abc = a.tipo_operacion
					ORDER BY c.id_tipo_abc
					
					RETURN cCodRet, iIdABC, cDescTipoOperacionABC WITH RESUME;
					LET iNoRegistros = iNoRegistros + 1;
...
'FECHA: 22/02/2015',
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Gestor de catalogos',
'DESCRIPCION: Consulta los catalogos a los que tiene permiso el usuario',
'BD: bdicnweb';
