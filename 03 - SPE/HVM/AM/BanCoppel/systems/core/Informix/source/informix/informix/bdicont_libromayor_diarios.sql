CREATE PROCEDURE "informix".libromayor_diarios(v_empresa CHAR(4), 
                                        v_fechainicio DATE,
                                        v_fechafin DATE,
                                        v_ccmayor CHAR(10),
                                        v_ccsub CHAR(10),
...
RETURNING VARCHAR(5), VARCHAR(255);
	...
	--SET DEBUG FILE TO "/tmp/libromayaux_diarios.out";
	--TRACE ON;
...
-- Inicio Proceso Saldos
       ...
       WHERE ccmayor = v_ccmayor
        ...
        FOREACH
            SELECT sucursal,ciudad,moneda,mes_dia_min,mes_dia_max
		...
		FOREACH 
			...
			IF tmovimientos = 0 THEN
				...
				VALUES(v_empresa, v_cuenta, v_ccmayor, v_ccsub, v_ccsubsub,v_ccssubsub, v_ccsssubsub, v_sector, tciudad, tsucursal_sdo,
					tmoneda_sdo, v_fechainicio, '', 0, 0,'', '', 0, 0, 0, 'SIN MOVIMIENTOS', v_fechafin, '', vusuario, v_idreporte);
			END IF
