CREATE PROCEDURE "informix".sp_cilocconsultaalertas()
		RETURNING   CHAR(5) as Codigo,	--codret
					CHAR(40) as Descripcion,	--desc_estatus
					CHAR(4) as Tipo;
					
	DEFINE cCodRet 			CHAR(5);
	DEFINE iCont            INTEGER;
	DEFINE iSqlErr 			INTEGER;
	DEFINE cDesc_estatus    CHAR(40);
	DEFINE cTipo            CHAR(4);
	...
	LET cTipo='';
	--------------------------------------------------------------------------
	--SET DEBUG FILE TO '/home/sysifx/Malena/sp_CiLocConsultaAlertas.out';
	--TRACE ON;
	--------------------------------------------------------------------------
	...
	BEGIN
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			LET cDesc_estatus='Error de Informix';
			RETURN cCodRet,cDesc_estatus,cTipo;
		END EXCEPTION;		
		
	--Se realiza consulta a la tabla cb_estatus_loc para obtener las situaciones de alarma.	
	set isolation to dirty read; -- Lectura de tablas bloqueadas.
	
		FOREACH   
			SELECT NVL(desc_estatus,''),NVL(tipo,'')
			INTO  cDesc_estatus,cTipo
			FROM bdicobranza:cb_estatus_loc
			LET icont=icont+1;
            RETURN cCodRet,cDesc_estatus,cTipo WITH RESUME;

		END FOREACH;		
		
        IF icont == 0 THEN 
			LET cCodret='00001';
            LET cDesc_estatus='No hay Informacion en la tabla';			
            RETURN cCodret,cDesc_estatus,cTipo WITH RESUME;
        END IF;
...
'DESCRIPCION : Devuelve un listado de las situaciones de alarma de la tabla cb_estatus_loc',
'FECHA       : 13 de Agosto de 2010',
'VERSION     : 20100813.1007',
'BD          : BDICOBRANZA';

CREATE PROCEDURE "informix".sp_cilocconsultamarcas()
		RETURNING   CHAR(5) as Codigo,	--codret
					CHAR(40) as Descripcion, --desc_marca
					CHAR(4) as Tipo; --Tipo de marca
					
	DEFINE cCodRet 			CHAR(5);
	DEFINE iCont            INTEGER;
	DEFINE iSqlErr 			INTEGER;
	DEFINE cDesc_marca		CHAR(40);
	DEFINE cTipo			CHAR(4);
	...
	LET cTipo='';
	--------------------------------------------------------------------------
	--SET DEBUG FILE TO '/home/sysifx/Malena/sp_CiLocConsultaMarcas.out';
	--TRACE ON;
	--------------------------------------------------------------------------
	...
	BEGIN
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			LET cDesc_marca='Error de Informix';
			RETURN cCodRet,cDesc_marca,cTipo;
		END EXCEPTION;		
		
	--Se realiza consulta a la tabla cb_tipo_marca para obtener los tipos de marcas.	
	
		FOREACH   
			SELECT NVL(desc_marca,''),NVL(tipo,'') 
			INTO  cDesc_marca,cTipo
			FROM bdicobranza:cb_tipo_marca
			LET icont=icont+1;
            RETURN cCodRet,cDesc_marca,cTipo WITH RESUME;

		END FOREACH;		
		
		IF icont == 0 THEN 
			LET cCodret='00001';   
			LET cDesc_marca='No hay Informacion en la tabla';
            RETURN cCodret,cDesc_marca,cTipo WITH RESUME;
        END IF;
...
DOCUMENT
'AUTOR       : Maria Elena Angulo Aispuro',
'DESCRIPCION : Devuelve un listado de los tipos de marcas de la tabla cb_tipo_marca',
'FECHA       : 12 de Agosto de 2010',
'VERSION     : 20100812.1019',
'BD          : BDICOBRANZA';

CREATE PROCEDURE "informix".sp_cilocconsultasituacionesmarcas()
		RETURNING   CHAR(5) as Codigo,	--codret
					CHAR(40) as Descripcion, --desc_estatus
					CHAR(4) as Tipo; --Tipo
	...
	LET cTipo='';
	--------------------------------------------------------------------------
	--SET DEBUG FILE TO '/home/sysifx/Malena/sp_CiLocConsultaSituacionesMarcas.out';
	--TRACE ON;
	--------------------------------------------------------------------------
	...
	--Se realiza consulta a la tabla cb_estatus_loc para obtener las situaciones de marca.	
...
DOCUMENT
'AUTOR       : Maria Elena Angulo Aispuro',
'DESCRIPCION : Devuelve un listado de las situaciones de marca de la tabla cb_estatus_loc',
'FECHA       : 12 de Agosto de 2010',
'VERSION     : 20100812.1130',
'BD          : BDICOBRANZA';
