CREATE PROCEDURE "informix".sp_rep_campanias_man_sms(pFecha date, pTipoEjec char(1))
...
DEFINE dFechaCorteAnterior DATE;
DEFINE cNumCredito         CHAR(20);
DEFINE cTelefono_cel       like bdinteg:si_telefonos_actual.telefono;
DEFINE cGrupo              like bdisolic:ss_resum_scor_fin.grupo;
DEFINE cApell_paterno      like bdinteg:si_cliente.apell_paterno;
DEFINE cNombre             CHAR(52);
DEFINE vMora_actual        like bdicred:sd_maesdos.mto_fin_ven_trasp;
DEFINE v_Mora_anterior     like bdicred:sd_maesdos.mto_fin_ven_trasp;
DEFINE vEvaluacion_bc      integer;
DEFINE vEvaluacion_prop    integer;
DEFINE vEvaluacion_fico    integer;
DEFINE vMora_anterior      like bdicred:sd_maesdoshist.mto_fin_ven_trasp;

DEFINE vvcodigo_retorno    CHAR(5);
...
LET dFechaCorteAnterior  = DATE(1);
LET cNumCredito          = '';
LET vEvaluacion_bc       = 0;
LET vEvaluacion_prop     = 0;
LET vEvaluacion_fico     = 0;
LET vMora_anterior       = 0;
LET vMora_actual         = 0;
...
LET vAnio                = '';    
LET vproceso             = '0038';
LET dFechaParam          = DATE(1);
LET cTipoEjec            = '';
LET cCodRet_2            = '000000';
-- SET DEBUG FILE TO '/informix/macf/sp_rep_campanias_man_sms.trc';
-- TRACE ON;
 ...
 ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
    IF iSqlErr != 0 THEN
      LET cCodRet     = iSqlErr;
      LET cMensajeRet = 'Ocurrió un error '||' - ' || cNumCredito || ' ' ||cErrorInfo;
      CALL "informix".sp_inserta_bitacora_cob('001', vproceso, cCodRet, cMensajeRet, '02')RETURNING cCodRet_2;
 ...
 CALL "informix".sp_inserta_bitacora_cob('001', vproceso, cCodRet, '', '01')RETURNING cCodRet_2;

 LET dFechaParam = pFecha;
 LET cTipoEjec = pTipoEjec; 

 TRUNCATE cb_camps_manuales_sms;
 ...
 SET ISOLATION TO dirty READ;
  ...
  --let dtFechaHoy = mdy(7,21,2016);
  IF cTipoEjec = 'M' then
     let dFechaCorte = dFechaParam; 
  ELSE
    let dFechaCorte = mdy(month(dtFechaHoy),20, year(dtFechaHoy));
  END IF;
  
  let dFechaCorteAnterior = dFechaCorte -1 units month;
  
  FOREACH WITH HOLD
  select a.num_credito, 
 	       nvl(c.telefono,''),
         nvl(e.grupo,''),
         trim(b.apell_paterno) APELL_PATERNO,
         trim(b.nombre1) || ' ' || trim(b.nombre2) NOMBRE,
         d.mto_fin_ven_trasp MORA_ACTUAL
    into cNumCredito, cTelefono_cel, cGrupo, cApell_paterno, cNombre, vMora_actual
    from bdicred:sd_maecred a left outer join bdinteg:si_telefonos_actual c on(a.numcte = c.numcte and c.tipo_tel = 2 and c.status_tel = 'A')
     left outer join bdisolic:ss_resum_scor_fin e on(a.empresa = e.empresa and a.num_credito = e.num_solicitud),  
     bdinteg:si_cliente b, bdicred:sd_maesdoshist d
   where d.fecha = dFechaCorte
     and a.empresa = '001'
     and a.empresa = d.empresa 
     and a.num_credito = d.num_credito
     and a.numcte = b.numcte
     and d.sdo_cap_insoluto > 0
    
   
        
         select evaluacion into vEvaluacion_bc
           from bdisolic:ss_resumen_scoring
          where empresa = cEmpresa
            and num_solicitud = cNumCredito
            and seccion = 1;
         
         select evaluacion into vEvaluacion_prop
           from bdisolic:ss_resumen_scoring
          where empresa = cEmpresa
            and num_solicitud = cNumCredito
            and seccion = 2;
            
         select evaluacion into vEvaluacion_fico
           from bdisolic:ss_resumen_scoring
          where empresa = cEmpresa
            and num_solicitud = cNumCredito
            and seccion = 3;

          select mto_fin_ven_trasp into v_Mora_anterior
          from bdicred:sd_maesdoshist
          where fecha = dFechaCorteAnterior
           and empresa = '001'
           and num_credito = cNumCredito;
        
          CALL bdicred:"informix".sp_consulta_saldocortemin(cEmpresa, cNumCredito, 3) RETURNING vvcodigo_retorno, dPagoMinimo;
          
         BEGIN;
            
            INSERT INTO cb_camps_manuales_sms(empresa, num_credito, telefono_cel, grupo, bc_score, score_propietario, fico_score, apell_paterno, nombre, mora_anterior, mora_actual, pagomin, fecha_corte) 
            VALUES(cEmpresa, cNumCredito, cTelefono_cel, cGrupo, vEvaluacion_bc, vEvaluacion_prop, vEvaluacion_fico, cApell_paterno, cNombre, v_Mora_anterior, vMora_actual, dPagoMinimo, dFechaCorte);
            
         COMMIT;  
     
     END FOREACH;

    let vDia = lpad(day(dFechaCorte),2,'0');
    let vMes = lpad(month(dFechaCorte),2,'0');
    let vAnio = lpad(year(dFechaCorte),4,'0');

     LET cNombre_archivo = trim('rep_camp_man_sms_' || trim(vdia) || trim(vMes) || trim(vAnio) || '.txt');
     
     let cSql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO /resplogifx/archivoscartera/' || trim(cNombre_archivo) ||
          ' DELIMITER ' || '''|'''  || ' SELECT ' || 
          ' num_credito,' ||
          ' telefono_cel,' ||
          ' grupo,' ||
          ' bc_score,' ||
          ' round(score_propietario,0) score_propietario,' ||
          ' fico_score,' ||
          ' apell_paterno,' ||
          ' nombre,' ||
          ' mora_anterior,' ||
          ' mora_actual,' ||
          ' pagomin ' ||
          ' FROM cb_camps_manuales_sms;"' ||
          ' > /resplogifx/archivoscartera/QueryRepCampaniaManual.sql';
     system trim(cSql);         
  
      let cSql='';
      let cSql = 'dbaccess bdicobranza /resplogifx/archivoscartera/QueryRepCampaniaManual.sql';
      system trim(cSql); 

      let cSql='';
      let cSql =  'gzip '|| '/resplogifx/archivoscartera/' ||TRIM(cNombre_archivo);
      system trim(cSql);

 CALL "informix".sp_inserta_bitacora_cob('001', vproceso, cCodRet, cMensajeRet, '03') RETURNING cCodRet_2;
...
CREATE PROCEDURE "informix".sp_cat_consulta_ultimo_convenio(pEmpresa   CHAR(3),
                                                            pNumCte    CHAR(20))
RETURNING   CHAR(6)         AS cod_ret,    
            CHAR(1)         AS origen,
            CHAR(4)         AS sucursal,
            DATE            AS fecha_ult_convenio,
            DECIMAL(14,2)   AS importe,
            CHAR(2)         AS cte_cumplido,
            DECIMAL(14,2)   AS monto_abonado,
            CHAR(8)         AS empleado_registro_convenio;
...
DEFINE dtFechaConpac    DATE; 
DEFINE dImporte         DECIMAL(14,2);
DEFINE cCumplido        CHAR(2);
DEFINE dMontoAbonado    DECIMAL(14,2);
DEFINE cEmpleadoCap     CHAR(8);
...
--SET DEBUG FILE TO "/home/sysifx/sp_cat _consulta_ultimo_convenio";
--TRACE ON;
...
BEGIN
    ...
    ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
          ...
          RETURN cCod_ret, NVL(cOrigen,""), NVL(cSucursal,""), NVL(dtFechaConpac,DATE(1)),
                 NVL(dImporte,0), NVL(cCumplido,""), NVL(dMontoAbonado,0), NVL(cEmpleadoCap,"");
    END EXCEPTION;
    ...
    SELECT {+INDEX(cb_compac_his idx_compachis1)} first 1 a.sucursal, a.fecha_compac, DECODE(a.origen,1,"C",2,"S",3,"A",""), a.importe,
           DECODE(TRIM(a.flag_pago),"0","NO","1","SI","NO"), a.imp_pagado, a.empleado_captura
      INTO cSucursal, dtFechaConpac, cOrigen, dImporte, 
           cCumplido, dMontoAbonado, cEmpleadoCap 
      FROM bdicobranza:cb_compac_his a
     WHERE a.empresa    = pEmpresa
       AND a.numcliente = pNumcte
       AND a.fecha_compac IN (SELECT {+INDEX(cb_compac_his idx_compachis1)} MAX (c.fecha_compac)
                              FROM bdicobranza:cb_compac_his c
                             WHERE c.empresa    = a.empresa
                               AND c.numcliente = a.numcliente);


-- Valida si se encontraron registros de convenios para el cliente
    IF DBINFO("sqlca.sqlerrd2") = 0 THEN
        -- LET cCod_ret = "101008";      --fmj nov 16
         RETURN cCod_ret, NVL(cOrigen,""), NVL(cSucursal,""), NVL(dtFechaConpac,DATE(1)),
                NVL(dImporte,0), NVL(cCumplido,""), NVL(dMontoAbonado,0), NVL(cEmpleadoCap,"");
    END IF;
...
END PROCEDURE
DOCUMENT
"Descripción: Procedimiento que obtiene los datos del último convenio realizado al crédito de un cliente.",
"BD: bdicobranza",
