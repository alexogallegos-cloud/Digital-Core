CREATE PROCEDURE "informix".sp_rcda_obtcompras_pba()
...
--variables de proceso
	...
	DEFINE	vnombre					CHAR (80);
	

BEGIN
   ON EXCEPTION SET SQL_ERR, ISAM_ERR, ERROR_INFO
      LET cod_ret    = SQL_ERR;
      LET mensaje  = ERROR_INFO || ' sp_rcda_obtcompras en paso ' || vpaso;
 ...
 -- paso de productividad de sucursales a acumulado mensual 
	...
	SELECT suc.num_sucursal, meta.producto, meta.metanum ,meta.aniomes
	...
	let vpaso = 8;	 
	INSERT INTO mi_rcda_apert_paso (fecha,sucursal,tpo_reg,ejecutivo,nombre,producto,numero,meta)	 
	select 	{+INDEX(bdmis:mi_acumps_mes idx_acumps_mes)}
			apert.fecha ,mes.sucursal,2,mes.ejecutivo,mes.nombre,mes.producto, SUM(NVL(mes.num_comp_mismomes,0)),
	nvl((((nvl((select metanum from tmp_metas_prod pd where  pd.aniomes = year(apert.fecha) || lpad (month(apert.fecha),2,'0') AND
												pd.num_sucursal = mes.sucursal and pd.producto = mes.producto ),0) * 24) / 30) * (select case
												when fecha_insert < mdy(1,1,year(apert.fecha))+1 
												then apert.fecha-mdy(1,1,year(apert.fecha))+1
												else apert.fecha-fecha_insert end
												from bdinteg:si_ejecut eej where eej.ejecutivo =mes.ejecutivo)) ,0)as meta
	--((nvl(mes.copentrmeta,0) * 24) / 30) * day (apert.fecha) 
	from mi_acumps_mes mes, mi_rcda_apert_paso apert
...
merge into mi_his_productividad a
	USING ( SELECT	fecha,sucursal,tpo_reg,ejecutivo,nombre,producto,sum(numero)as numero,sum(meta)as meta 
		--INTO	dfecha,vsucursal,vtpo_reg,vpromotor,vnombre, vproducto, vnumero, vmeta
		FROM mi_rcda_apert_paso
		WHERE  producto = '6111' and tpo_reg = 1 group by 1,2,3,4,5,6 ) b
	...
	USING ( SELECT	fecha,sucursal,tpo_reg,ejecutivo,nombre,producto,sum(numero)as numero,sum(meta)as meta
		--INTO	dfecha,vsucursal,vtpo_reg,vpromotor,vnombre, vproducto, vnumero, vmeta
			FROM mi_rcda_apert_paso
			WHERE  producto = '6111' and tpo_reg = 2 group by 1,2,3,4,5,6) b
	on a.fecha = b.fecha AND a.sucursal = b.sucursal AND a.tpo_reg = b.tpo_reg AND a.ejecutivo = b.ejecutivo AND a.producto = b.producto
	and a.nombre=b.nombre
	WHEN NOT MATCHED THEN
	insert (a.fecha,a.sucursal,a.tpo_reg,a.ejecutivo,a.nombre,a.producto,a.num_comp_mismomes,a.meta_comp_mismomes) 
