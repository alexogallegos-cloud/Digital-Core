CREATE PROCEDURE "informix".sp_insert_ex_empleados_adn () 
RETURNING CHAR(6) AS cCodRet;
    ...
    DEFINE tNum_credito     CHAR(20);
    DEFINE cNum_credito     INTEGER;
    DEFINE tFechaDoc        CHAR(6);
...
BEGIN
    ...
    -- SET DEBUG FILE TO '/home/e90317801/job/sp_insert_ex_empleados_adn.out';
    -- TRACE ON;

    -- BORRA LA TABLA DE TEMPORAL DE EX-EMPLEADOS CON CREDITO ADN
	DROP TABLE IF EXISTS ex_empleados_adn_temp;

	-- CREA LA TABLA DE TEMPORAL DE EX-EMPLEADOS CON CREDITO ADN
	CREATE TABLE ex_empleados_adn_temp ( num_credito CHAR(20) );

    UPDATE STATISTICS MEDIUM FOR TABLE ex_empleados_adn_temp;

    SELECT  TO_CHAR(fecha_hoy, '%d%m%y') 
    INTO    tFechaDoc
    FROM    bdicred:sd_fechas;

    -- GUARDA EL QUERY DEL LOAD EN EL ARCHIVO   *.SQL
	LET cSql = "";
	LET cSql = 'echo "LOAD FROM \"' || TRIM(cRuta) || 'BAJAS' || TRIM(tFechaDoc) || '.csv\" DELIMITER \",\" INSERT INTO  bdicred:\"informix\".ex_empleados_adn_temp(num_credito) " > '|| TRIM(cRuta) || 'Ejecuta_InsertaExEmpleados.sql';
	SYSTEM cSql;	

    -- EJECUTA LAS INSTRUCCIONES QUE ESTAN DENTRO DEL ARCHIVO  *.SQL
	LET cSql = "";
	LET cSql = 'dbaccess bdicred ' || TRIM(cRuta) || 'Ejecuta_InsertaExEmpleados.sql';
	SYSTEM cSql;

    -- RECORRE LA TABLA TEMPORAL ex_empleados_adn_temp
    FOREACH WITH HOLD

        SELECT  num_credito 
        INTO    tNum_credito 
        FROM    "informix".ex_empleados_adn_temp

        -- SI NO EXISTE EL REGISTRO DE ex_empleados_adn_temp, PROCEDE A INSERTARLO EN LA TABLA sd_exempleados_adn
        SELECT COUNT(num_credito) INTO cNum_credito FROM "informix".sd_exempleados_adn WHERE num_credito = tNum_credito;
        IF (cNum_credito = 0) THEN

            INSERT INTO "informix".sd_exempleados_adn (num_credito) VALUES (tNum_credito);
            
            SELECT COUNT(num_credito) INTO cNum_credito FROM "informix".sd_maecred WHERE num_credito = tNum_credito;
            IF (cNum_credito > 0) THEN
                BEGIN WORK;
                UPDATE  "informix".sd_maecred
                SET     id_unidad_prod = 3
                WHERE   num_credito = tNum_credito;
                COMMIT WORK;
            END IF;
    ...
    -- ELIMINA EL ARCHIVO QUE CONTIENE LA CARGA
    LET cSql = "";
	LET cSQL = "rm " ||TRIM(cRuta)||'Ejecuta_InsertaExEmpleados.sql';		
	SYSTEM cSql; 

    -- ELIMINA EL ARCHIVO QUE CONTIENE LA CARGA
    LET cSql = "";
	LET cSQL = "rm " || TRIM(cRuta) || 'BAJAS' || TRIM(tFechaDoc) || '.csv';		
	SYSTEM cSql; 

    -- ELIMINA EL ARCHIVO QUE CONTIENE LA CARGA
    LET cSql = "";
	LET cSQL = "rm " || TRIM(cRuta) || 'BAJAS' || TRIM(tFechaDoc) || '.zip';		
	SYSTEM cSql; 

    DROP TABLE "informix".ex_empleados_adn_temp;

	RETURN cCodRet; 
END
END PROCEDURE
DOCUMENT
'DESCRIPCION: Carga de ex-empleados a la tabla sd_exempleados_adn', 
'AUTOR: Joshua Antonio Lopez Dominguez',
'FECHA: 09/Julio/2024',
'BD: bdicred';

CREATE PROCEDURE "informix".conciliatc(pEmpresa     CHAR(3),
			    pTarjeta     CHAR(20),
			    pSucursal    CHAR(4),
			    pUsuario     CHAR(8),
			    pTpMov       CHAR(1),
   ...
   ON EXCEPTION SET sql_err, isam_err, error_info
      LET cod_ret = sql_err;
      RETURN cod_ret, vBandera;
   END EXCEPTION;

 --SET DEBUG FILE TO "/tmp/mfinis/ROMAN_/conciliatc.out";
 --TRACE ON;
 ...
 --  SELECT fecha_hoy INTO vHoy FROM sd_fechas WHERE empresa = pEmpresa;
-- ****************************************************************************
-- *                        PROGRAMA PRINCIPAL                                *
-- ****************************************************************************
	...
	SELECT count(num_credito),numero_credito_upgrade 
	INTO iBanUpgrade, cNumCredUpgrade 
	...
	IF iBanUpgrade > 0 THEN
		LET vNumCred = cNumCredUpgrade;
	END IF;

	   IF vNumCred IS NULL THEN
		LET cod_ret = "008";
		LET vBandera = "E";
		RETURN cod_ret, vBandera;
	   END IF; 
   ...
   SELECT status_cred
    ...
    WHERE empresa = pEmpresa
	 ...
	 IF cStatus IN ("FF","FC","CV") THEN
		LET cod_ret = "008";
		LET vBandera = "E";
		RETURN cod_ret, vBandera;	 
	 END IF;	 
	   
    SELECT fecha_proceso INTO vHoy
     FROM sd_maecredanexo
    WHERE empresa = pEmpresa
      AND num_credito = vNumCred;

	   IF vHoy IS NULL THEN
		LET cod_ret = "008";
		LET vBandera = "E";
		RETURN cod_ret, vBandera;
	   END IF

   IF pFormaAplica = "A" THEN -- Aplica Movimiento por no existir registrado
	IF pTpMov = "C" THEN
		LET vTranResp = pTran;
		SELECT NVL(tranlibprot,"0000")
		  INTO pTran
		  FROM bdinteg:si_transacc
		 WHERE empresa = pEmpresa
		   AND numero = pTran
		   AND sistema = "06";
		IF pTran = "0000" OR pTran = " " THEN
			LET pTran = vTranResp;
		END IF

		EXECUTE PROCEDURE cargo_cred
			(pEmpresa, vNumCred, pSucursal, pUsuario, pTran,
			 pMonto, pFolio, pTarjeta, 0, 0, vHoy, pReferencia,
			 pRfcComer, pRef23)
		INTO cod_ret;
		IF cod_ret <> "000" THEN
			LET vBandera = "E";
	...
	ELIF pTpMov = "A" THEN
            ...
            IF (pTran <> "6814") THEN
                IF (pTran <> "6883") THEN
                  LET pTran = "6813"; END IF;
                END IF;
			CALL principalrefer( pEmpresa,vNumCred, "1", pTarjeta,pUsuario, pSucursal,pFolio, pTran,0.0,pMonto,pRef23)					        			   
				RETURNING cod_ret, vMtoPaso, vMtoPaso, vMtoPaso,
					  vMtoPaso, vMtoPaso, vMtoPaso,
				  vMtoPaso, vMtoPaso,vMtoPaso;
					IF cod_ret <> "000" THEN
							LET vBandera = "E";
					ELSE
...
--		END IF
   ...
   ELIF pFormaAplica = "X" THEN -- Rev. y Aplica Movto por dif con el existente
                EXECUTE PROCEDURE abono_cred
                        (pEmpresa, vNumCred, pSucursal, pUsuario, pTran,
                         pMonto, pFolio, pTarjeta, 0, 0, vHoy, pReferencia,
                         "R", pRfcComer, pRef23)
...
--        	UPDATE sd_maesdos SET sdo_retenido = sdo_retenido + pMonto
--         	 WHERE empresa = pEmpresa
--           	   AND num_credito = vNumCred;
   ...
   ELIF pFormaAplica = "B" THEN -- Solo Concilia el Movimiento
	LET vBandera = "C";
	IF pTpMov = "C" THEN
		LET vTranResp = pTran;
                SELECT NVL(tranlibprot,"0000")
		...
		IF pTran in ("6800","6871","6872","6873",
								 "6802","6857","6858","6859",
								 "6804","6874","6875","6876",
								 "6850","6890","6891","6892","6893","6894","6895") THEN
			RETURN cod_ret, vBandera;
		END IF
...
END PROCEDURE
DOCUMENT
'Esta funcion realiza la concilicion de los movtos de tarjeta recibidos por ',
'por el sistema de credito a travez de POS y ATM',
...
-- ModificaciÃ³n para paso 2 de Alta Ãnica con migraciÃ³n de catÃ¡logos.
------------------------------------------------------------------------------------
-- Frank Gaxiola Gaxiola
-- 06-11-2009
-- Se aplica filtro a la consulta a la tabla si_ingresos por tipo de ingresosea igual a "T"
------------------------------------------------------------------------------------
-- MACF 20241003. Se agrega registro en Bitacora y limit 1 en consulta a se_ctessitespcte
...
BEGIN
    ...
    ON EXCEPTION SET SQL_ERR, ISAM_ERR, ERROR_INFO
		...
		LET cMensajeRet = '';
		LET cMensajeRet = SQL_ERR || ' ' || ERROR_INFO || ' Numcte: ' || trim(cNumCliente);
		CALL bdicobranza:"informix".sp_inserta_bitacora_cob(cEmpresa, cProceso, cCod_ret, trim(cMensajeRet) || ' Err Proc Directorio altas y cambios', '02') 
		     RETURNING cCod_retBit;
        RETURN P_COD_RET;
    END EXCEPTION;
...
--  trace on;
    CALL bdicobranza:"informix".sp_inserta_bitacora_cob(cEmpresa, cProceso, cCod_ret, 'Inicia Proceso Directorio altas y cambios', '02') RETURNING cCod_retBit;       
...
--    create unique index inx_clientes_alta on clientes_alta (numcte) using btree;
    ...
    FOREACH   -- Datos del Cliente
        SELECT
            ...
            nvl(ctepf.fecha_nac,date(1)) AS anionac,    -- aÃ±o de nacimiento
...
--            rpad(TRIM(puest.descripcion),30,' ') as puesto, -- descripcion puesto
            ...
            rpad(TRIM(tipoidentif.descripcion),40,' ') as tipoidentificacion, -- tipo de identificaciÃ³n
            rpad(TRIM(ctepf.numidentifi),30,' ') as numidentificacion, --numero de identificaciÃ³n
...
--            LEFT OUTER JOIN bdinteg:si_catciudades ciudad ON (ciudad.numerociudad = dir.numerociudad)
        ...
        SELECT limit 1 nvl(ctessit.situacion,' '), nvl(ctessit.causa,' '), nvl(ctessit.fechamovto,' ')
...
--                    nvl ( replace ( replace( cNomMunicipioTrab , '|' , ' ' ), '\' , ' ' ), ' ' ),
    ...
    CALL bdicobranza:"informix".sp_inserta_bitacora_cob(cEmpresa, cProceso, cCod_ret, 'Finaliza Proceso Directorio altas y cambios', '02') RETURNING cCod_retBit;
    RETURN P_COD_RET;
