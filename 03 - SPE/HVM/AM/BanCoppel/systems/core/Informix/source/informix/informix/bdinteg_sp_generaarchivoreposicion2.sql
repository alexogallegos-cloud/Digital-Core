CREATE PROCEDURE "informix".sp_generaarchivoreposicion2(p_Empresa CHAR(3), pFechaAct DATE)
RETURNING
     CHAR(6); ---cod_ret

--DEFINICION DE VARIABLES
--CLIENTE
DEFINE vClave CHAR(2); --DEFAULT 'R'
...
LET vflagentregotarjeta = 'A'; --DEFAULT A
LET vefectuo = 0;
LET vtiendafolio = 0;
...
BEGIN
	ON EXCEPTION
		SET iSqlErr
		Set debug file to '/RESPALDOS/sp_GeneraArchivoReposicion2.out';
		trace on;
	...
	IF pFechaAct <> mdy(1,1,1900) OR pFechaAct IS NOT NULL THEN 
		...
		ELSE
			...
			FOREACH
				...
				WHERE secuencia >= 1 AND tipocliente = 'T' AND fecha = pFechaAct
				...
				IF vnumcte <> '' AND vcliente_ref <> '' THEN
					...
					IF EXISTS (SELECT 1 FROM bdinteg:"informix".si_archivoscoppelhistorial WHERE empresa = p_Empresa AND tipomovto <> 'TO' OR tipomovto = 'TO') THEN
						IF EXISTS (SELECT 1 FROM bdinteg:"informix".si_archivoscoppeldiario WHERE empresa = p_Empresa AND tipomovto <> 'TO' OR tipomovto = 'TO') THEN
							LET iSecuencia = (SELECT MAX(secuencia) + 1  FROM bdinteg:"informix".si_archivoscoppeldiario WHERE empresa = p_Empresa AND tipomovto <> 'TO' OR tipomovto = 'TO');
						ELSE
					...
					LET vsSQL = vsSQL|| "|" ||TRIM(vclavereferencia1)||"|"||TRIM(vclientereferencia2)||"|"||TRIM(vnombreunoreferencia2)||"|"||TRIM(vnombredosreferencia2)||"|"||TRIM(vapellidopaternoreferencia2)||"|"||TRIM(vapellidomaternoreferencia2)||"|" 
							...
							||vtiporeposicion||"|"||NVL(vreposicion, 0)|| "|" ||TRIM(vflagentregotarjeta)|| "|" || NVL(vefectuo, 0)|| "|" || NVL(vtiendafolio, 0) || "|" ||TRIM(vfolio)||"|"||TRIM(NVL(cfechaaltacte, '1900/01/01'))||"|"   
							...
							||vempleadoautorizo|| "|" ||TRIM(vtipo)|| "|"||TRIM(NVL(cfechamovto, '1900/01/01'))||"|"||TRIM(vnumerosolicituddecredito)|| "|" || TRIM(NVL(vnumcte, ''))|| "|" ||vtiendafolioanterior|| "|" ||vfolioanterior||"|" 
...
END;
--*************************************************************************
--| Procedimiento   : "informix".sp_GeneraArchivoReposicion
--| Version         : 1.0
--| Creado por      : Mohamed Carreón
--| Fecha creacion  : Diciembre de 2008
--| Descripcion 	: Realiza la extraccion de datos para archivos de reposicion.
--*************************************************************************
...
--| Descripción     : Se modifica tipo de consulta para obtener los datos de las reposiciones del titular.
--*************************************************************************
--| Modificado por  : Adrian Lara
