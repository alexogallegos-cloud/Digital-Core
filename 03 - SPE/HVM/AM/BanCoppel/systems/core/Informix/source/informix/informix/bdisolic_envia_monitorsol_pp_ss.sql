CREATE PROCEDURE "informix".envia_monitorsol_pp_ss
													(pEmpresa     CHAR(3),
													 pSucursal    CHAR(20),
...
END EXCEPTION;
	...
	--SET DEBUG FILE TO "/informix/gpe/envia_monitorsol_pp_ss.out";
	--TRACE ON;
		...
		IF NVL(pEstatus,"")  <> "RS" THEN
			...
			FOREACH WITH HOLD
				...
				SELECT {+INDEX("informix".ss_solicitudes idx_ss_solicitudes2)} skip pSolicitudes LIMIT pLimit				
					...
					b.apell_paterno, b.apell_materno, b.Nombre1, b.Nombre2, b.razon_social, b.rfc, c.causa_solicitud
				INTO cNumSol, cNumCte, cStatus, cProducto, mLinea, dFechaSol,
					cApellidoPaterno, cApellidoMaterno, cNombre1, cNombre2, cRazonSocial, cRfc, cCausasolicitud
				FROM "informix".ss_solicitudes a
				...
				INTO cStDesc, cComentario, mIngreso, dFechaAut, cComentario2,
					cProdDesc, cDivisa,	cCuenta_eje,iFrecuencia,iDiaPago,cCausasolicitud
