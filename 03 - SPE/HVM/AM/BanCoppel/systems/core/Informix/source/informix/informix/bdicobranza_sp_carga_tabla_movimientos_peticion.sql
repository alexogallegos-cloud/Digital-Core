CREATE PROCEDURE "informix".sp_carga_tabla_movimientos_peticion(pfecha DATE)
...
___________________________________________________________________________________________________________________________________________________________________________
	...
	MODIFICADO: Carlos Valenzuela
	FECHA: 23-03-2017.
	DESCRIPCION MODIFICACION: Se optimiza el spl y se reemplaza el load por el dbload
...
DEFINE v_banderaCon				INTEGER;

/*DEFINE vCliente					CHAR(9);
DEFINE Vtipomovimiento			INTEGER;
DEFINE Vtipologica				INTEGER;
DEFINE Vtipocobranza			CHAR(1);
DEFINE Vtienda					CHAR(20);
DEFINE Vkeyx					VARCHAR(10);
...
LET vRutasql 				= "/ifxsif01/scripts/";
LET v_contador				= 0;
LET v_banderaCon			= 0;

/*LET vCliente				= '';
LET Vtipomovimiento			= 0;
LET Vtipologica				= 0;
LET Vtipocobranza			= '';
LET Vtienda					= '';
LET Vkeyx					= '';
...
--SET DEBUG FILE TO "/RESPALDOS/INFOSAT/ALDO/sp_carga_tabla_movimientos.out ";
	--TRACE ON;
...
BEGIN
    ...
    -- DEPURADO DE DUPLICADOS
	FOREACH WITH HOLD
    	SELECT {+AVOID_FULL(bdicobranza:cb_cat_movimientos)} cvemovimiento, tipomovimiento, horainicio, horafin, cliente,
        ...
        GROUP BY cliente,tipomovimiento,tipologica,tipocobranza,tienda,keyx,cvemovimiento,horainicio,horafin,tipoclientecampana,cuenta,
            importe,tipoconvenio,plazo,cobranzacat,sucursal,empresa,usteddebe,usteddebia,vencido,tipotelefono,numext,
            telefonooriginal,telefonoreconstruido,finllamada,contacto,tipored,aclaracion,fechahorallamada,horainiciollamada,
            horafinllamada,pkwhere,duracionefectiva,cat,ip,numempleado,observaciones,fechacartera,carrier,numerociudad,numeroestado
        HAVING count(*) > 1 
		...
		delete bdicobranza:cb_cat_movimientos
			WHERE DATE(horainicio) = pfecha
	...
	LET cCadena = 'cat ' || TRIM(vRutasql) || '206_3_40_carga_movimientos_cat_peticion.sql | sed "s/vnom_archivo/' || TRIM(VNomArch) || '/g" > ' || TRIM(vRuta) || 'catmovimientosctbcpl_peticion.sql';
	SYSTEM SUBSTR(cCadena,1,LENGTH(cCadena));

	--CREAMOS EL ARCHIVO CON LA CADENA A EJECUTAR
	LET cCadena = "";
	LET cCadena = 'dbload -d bdicobranza -c ' || TRIM(vRuta) || 'catmovimientosctbcpl_peticion.sql -l ' || TRIM(vRuta) || 'catmovimientosctbcpl_peticion.log -n 1000 -k';
	SYSTEM SUBSTR(cCadena,1,LENGTH(cCadena));

	--BORRA EL ARCHIVO
	LET cCadena = "";
	LET cCadena = 'rm -f ' || TRIM(vRuta) || 'catmovimientosctbcpl_peticion.sql';
	SYSTEM SUBSTR(cCadena,1,LENGTH(cCadena));
...
-------------------------------------------------------------------------------------------------------------------------------------
	...
	FOREACH WITH HOLD
            SELECT {+AVOID_FULL(bdicobranza:cb_cat_movimientos_peticion)}
                cvemovimiento, tipomovimiento, horainicio, horafin, cliente,
                tipologica, tipocobranza, tipoclientecampana, cuenta, tienda,
                importe, tipoconvenio, plazo, cobranzacat, sucursal,
                empresa, usteddebe, usteddebia, vencido, tipotelefono,
...
--	CALL bdicobranza:"informix".sp_actualiza_contacto_exitoso() RETURNING cCod_ret, cMensaje;
	...
	--BORRA EL LOG DE LA CARGA
	LET cCadena = "";
	LET cCadena = 'rm -f ' || TRIM(vRuta) || 'catmovimientosctbcpl_peticion.log';
	SYSTEM SUBSTR(cCadena,1,LENGTH(cCadena));
