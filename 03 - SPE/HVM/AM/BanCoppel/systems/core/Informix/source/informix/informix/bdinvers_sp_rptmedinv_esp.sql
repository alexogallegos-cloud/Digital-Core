CREATE PROCEDURE "informix".sp_rptmedinv_esp(pempresa CHAR(3), vfecha DATE)
RETURNING CHAR(5);
  ...
  /*VARIABLES DE REPORTE movhisINV */
    ...
    -- SET DEBUG FILE TO "/informix/resplogifx/conciliachq/sp_rptmedinv.out";
     ...
     LET vsql = 'echo "set isolation to dirty read; unload to /resplogifx/conciliachq/apediainv'||TO_CHAR(vfecha,'%m%d%y')||'.txt '||
                'select * from apediainv order by sucursal, inversion;" > /resplogifx/conciliachq/apediainvtmpesp.sql';
     SYSTEM vsql;
     LET vsql = '';
    
     LET vstmt = "/ifxsif01/bin/dbaccess bdinvers /resplogifx/conciliachq/apediainvtmpesp.sql"; 
     SYSTEM vstmt;
     LET vstmt = '';
     ...
     LET vsql = 'echo "set isolation to dirty read; unload to /resplogifx/conciliachq/movcontinv'||TO_CHAR(vfecha,'%m%d%y')||'.txt '||
                'select * from movcontinv order by moneda, producto,transaccion;" > /resplogifx/conciliachq/movcontinvtmpesp.sql';
     SYSTEM vsql;
     LET vsql = '';
    
     LET vstmt = "/ifxsif01/bin/dbaccess bdinvers /resplogifx/conciliachq/movcontinvtmpesp.sql"; 
     SYSTEM vstmt;
     LET vstmt = '';
     ...
     LET vsql = 'echo "set isolation to dirty read; unload to /resplogifx/conciliachq/movhisinv'||TO_CHAR(vfecha,'%m%d%y')||'.txt '||
               ' select * from movhisinv order by folio_tran,transaccion,importe;" > /resplogifx/conciliachq/movhisinvtmpesp.sql';
     SYSTEM vsql;
     LET vsql = '';
    
     LET vstmt = "/ifxsif01/bin/dbaccess bdinvers /resplogifx/conciliachq/movhisinvtmpesp.sql"; 
     SYSTEM vstmt;
     LET vstmt = '';
    ...
    LET vsql = 'echo "set isolation to dirty read; unload to /resplogifx/conciliachq/movrevinv'||TO_CHAR(vfecha,'%m%d%y')||'.txt '||
               ' select * from movrevinv order by folio_tran,transaccion,importe;" > /resplogifx/conciliachq/movrevinvtmpesp.sql';
    SYSTEM vsql;
    LET vsql = '';
    
    LET vstmt = "/ifxsif01/bin/dbaccess bdinvers /resplogifx/conciliachq/movrevinvtmpesp.sql"; 
    SYSTEM vstmt;
    LET vstmt = '';
	...
	( vsucursal,vcod_inst_inversion,vnum_certificado,vsecuencia,vcliente,vnom_cliente,vcapital,vinteres,vretencion,
     ...
     LET vsql = 'echo "set isolation to dirty read; unload to /resplogifx/conciliachq/sdodiainv'||TO_CHAR(vfecha,'%m%d%y')||'.txt '||
               ' select * from sdodiainv order by sucursal,producto,cliente;" > /resplogifx/conciliachq/sdodiainvtmpesp.sql';
     SYSTEM vsql;
     LET vsql = '';
    
     LET vstmt = "/ifxsif01/bin/dbaccess bdinvers /resplogifx/conciliachq/sdodiainvtmpesp.sql"; 
     SYSTEM vstmt;
     LET vstmt = '';
