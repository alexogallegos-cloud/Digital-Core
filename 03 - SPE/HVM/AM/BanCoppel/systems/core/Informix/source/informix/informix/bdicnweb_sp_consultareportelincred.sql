CREATE PROCEDURE "informix".sp_consultareportelincred(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaInicio DATE, pFechaFin DATE, pTipo CHAR(1), pStatus1 CHAR(1), pStatus2 CHAR(1), pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
			CHAR(20) AS num_credito,
			DECIMAL(18,2) AS monto_anterior,
			DECIMAL(18,2) AS monto_nuevo,
			DECIMAL(18,2) AS monto_incremento,
			CHAR(20) AS numero_cliente,
			CHAR(104) AS nombre_cliente,
			DATE AS fecha_alta,
			DATETIME HOUR TO SECOND AS hora_alta,
			CHAR(10) AS tipo_ejecucion,
	...
	DEFINE dMontoAnterior DECIMAL(18,2);
	DEFINE dMontoNuevo DECIMAL(18,2);
    DEFINE dMontoIncremento DECIMAL(18,2);
    DEFINE cNumCliente CHAR(20);
    DEFINE cNombreCliente CHAR(104);
    DEFINE dFechaAlta DATE;
    DEFINE dHoraAlta DATETIME HOUR TO SECOND;
    DEFINE cTipoEjecucion CHAR(1);
    DEFINE cEstatusCredito CHAR(1);
	...
	LET iRecuperacion = 0;
	LET cNumCredito = '';
	LET dMontoAnterior = NULL;
	LET dMontoNuevo = NULL;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
				LET cCodRet = iSqlErr;
				RETURN cCodRet, cNumCredito, dMontoAnterior, dMontoNuevo, dMontoIncremento, cNumCliente, cNombreCliente,
				dFechaAlta, dHoraAlta, cTipoEjecucion, cEstatusCredito;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultareportelincred.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pFechaInicio = '' OR pFechaFin = '' OR pTipo = '' OR pRegistros IS NULL OR pRecuperacion IS NULL THEN
				LET cCodRet = '00003';
				RETURN cCodRet, cNumCredito, dMontoAnterior, dMontoNuevo, dMontoIncremento, cNumCliente, cNombreCliente,
				dFechaAlta, dHoraAlta, cTipoEjecucion, cEstatusCredito;
		END IF;
		...
		FOREACH EXECUTE PROCEDURE bdicred:"informix".sp_consultafechalincred(pFechaInicio, pFechaFin, pTipo, pStatus1, pStatus2)
				INTO cCodRetSp, cNumCredito, dMontoAnterior, dMontoNuevo, dMontoIncremento, cNumCliente, cNombreCliente,
				dFechaAlta, dHoraAlta, cTipoEjecucion, cEstatusCredito
		
				LET iCodRetSp = cCodRetSp::INTEGER;
				IF iCodRetSp < 0 THEN
						RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_consultafechalincred';
				END IF;
						
					IF iRegistros >= pRegistros THEN
							IF iRecuperacion < pRecuperacion THEN
								--IF dMontoNuevo < dMontoAnterior THEN 
									RETURN cCodRet, cNumCredito, NVL(dMontoAnterior, 0), NVL(dMontoNuevo, 0), NVL(dMontoIncremento, 0), cNumCliente, cNombreCliente,
												dFechaAlta, dHoraAlta, DECODE(cTipoEjecucion, 'M', 'MANUAL', 'A', 'AUTOMATICO', ''), cEstatusCredito WITH RESUME;
...
'FECHA: 05/04/2014',
'DESCRIPCION: Consulta los datos para el llenado del reporte por periodo de fechas segÃÂÃÂºn su modificaciÃÂÃÂ³n de la linea de crÃÂÃÂ©dito',
'BD: bdicnweb',
'----------------------------------------------------------------------------------',
'Modificacion: Se modifica para agregar filtro de disminucion de Linea',
'Fecha de modificacion: 20/09/2016',
'Modificacion: Johnattan Esquivel Sanchez',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_rem_consultasacappqryi(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumRem CHAR(12), pRCode CHAR(4)) 
    RETURNING CHAR(5) AS codRet,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			IF iSqlErr <> 0 THEN
				LET cCodRet = iSqlErr;
				RETURN cCodRet,cTxn_status,cUnirefnum,cCode_Company,cChanneldid,cLocationunit,cNnumber,cTypecode_Branch,cCountrycode_Branch,cStatecode_Branch,cTerminalid,
				...
				cCountryCode_a,cStateCode,cZipCode,cTypeCode_i,cNumber,cExpirationDate,cIssuerCountryCode,cIssuerStateCode,cDateOfBirth,cUsuarioInsert,cFechaInser;
			END IF;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_rem_consultasacappqryi.out';
		--TRACE ON;
		...
		ELSE
			...
			IF NVL(cFolio_suc,'') <> '' AND SUBSTR(cFolio_suc,1,7) <> 'sys_apz' THEN
				...
				SELECT r_typecode_i,r_number,r_address_b,r_zipcode_b,r_city_b,r_statecode_b,r_countrycode_b,r_homephonenum
				INTO cR_typecode_i,cR_number,cR_address_b,cR_zipcode_b,cR_city_b,cR_statecode_b,cR_countrycode_b,cR_homephonenum
...
CREATE PROCEDURE "informix".sp_rem_consultasacappqryi_2(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumRem CHAR(12), pRCode CHAR(4)) 
    RETURNING CHAR(5) AS codRet,
		CHAR(1) AS  cTxn_status,
		CHAR(16) AS cUnirefnum,
		CHAR(3) AS  cCode_Company,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			IF iSqlErr <> 0 THEN
				...
				UPDATE "informix".sw_verifica_estatusrem
				SET status = 'E', error_proceso = 'S', error = cCodRet WHERE usuario_insert_proc = pUsuario;
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_rem_consultasacappqryi_2.out';
		--TRACE ON;
		
		-- SE LIMPIA TABLA POR USUARIO
		DELETE FROM "informix".sw_verifica_estatusrem WHERE DATE(fechahora_insert_proc) < DATE(CURRENT);
		DELETE FROM "informix".sw_verifica_estatusrem WHERE usuario_insert_proc = pUsuario;
		
		-- SE INSERTA A TABLA PARA EL MONITOREO DEL STATUS
		INSERT INTO "informix".sw_verifica_estatusrem(status,error_proceso,error,unirefnum,r_code,usuario_insert_proc,fechahora_insert_proc)
		VALUES('I','',cCodRet,pNumRem,pRCode,pUsuario,CURRENT);
		...
		ELSE
			...
			SELECT fecha_hoy INTO dFecha_hoy FROM bdisac:"informix".sac_fechas;
			...
			IF DATE(cFechaInser) = dFecha_hoy THEN
				
				FOREACH
					SELECT 
					FIRST 1
					folio_suc, id_sucursal, forma_pago
			...
			ELIF DATE(cFechaInser) < dFecha_hoy THEN
			...
			ELIF DATE(cFechaInser) > dFecha_hoy THEN
				LET cCodRet = '00975'; --LA FECHA DE PAGO ES INVÃÂLIDA
			END IF;
			...
			IF NVL(cFolio_suc,'') <> '' AND SUBSTR(cFolio_suc,1,7) <> 'sys_apz' THEN
				...
				SELECT r_firstname_b,r_middlename_b,r_lastname_b,r_mommaidenna_b,r_address_b,r_city_b,r_statecode_b,r_countrycode_b,r_zipcode_b,r_currencycod_d,
				typecodeci,numberci,adress,zipcode,city,statecodeadr,countrycodeadr,homephonenum,dateofbirth,issuercc,contrycode
				INTO cFirstName_b,cMiddleName_b,cLastName_b,cMotherMaidenName_b,cAddress_b,cCity_b,cStateCode_b,cCountryCode_b,cZipCode_b,cCurrencyCode_d,
				cR_typecode_i,cR_number,cR_address_b,cR_zipcode_b,cR_city_b,cR_statecode_b,cR_countrycode_b,cR_homephonenum,cDateBirth,cIssuercc,cContryCode 								
				FROM bdisac:"informix".sac_app_payi
				WHERE unirefnum = pNumRem
				AND r_code = '0000'
				AND fecha=(SELECT max(fecha) FROM bdisac:"informix".sac_app_payi WHERE unirefnum = pNumRem AND r_code = '0000') --nmr2519
                AND processtime=(SELECT max(processtime) FROM bdisac:"informix".sac_app_payi WHERE unirefnum = pNumRem AND r_code = '0000'); --nmr2519
		...
		UPDATE "informix".sw_verifica_estatusrem
		SET status = 'T',error_proceso = 'N',error = cCodRet,
		txn_status = cTxn_status,unirefnum = pNumRem,code = cCode_Company,channeldid = cChanneldid,locationunit = cLocationunit,
		nnumber = cNnumber,typecode = cTypecode_Branch,countrycode = cCountrycode_Branch,statecode = cStatecode_Branch,terminalid = cTerminalid,
		processdate = cProcessdate_Qry,processtime = cProcesstime_Qry,r_operacion = cCode_Operacion,r_code = pRCode,r_message = cMensCode,
		r_code_d = cCode_d,r_message_d = cMensajeD,r_processdate = cProcessDate,r_processtime = cProcessTime,r_rule = cRule,
		r_value = cValue,r_globtracknum = cGlobalTrackingNumber,r_ordstatuscode = cOrderStatusCode,r_ordstatusdate = cOrderStatusDate,r_ordstatustime = cOrderStatusTime,
		r_uniquerefnum = cUniqueReferenceNumber,r_codesalecom = cCodesalecom,r_countrycode = cCountryCode,r_statecodesale = cStateCodeSale,r_saledate = cSaleDate,
		r_saletime = cSaleTime,r_countrycode_o = cCountryCode_o,r_currencycode = cCurrencyCode,r_servicecode = cServiceCode,r_countrycode_d = cCountryCode_d,
		r_currencycod_d = cCurrencyCode_d,r_delimethodcod = cDeliveryMethodCode,r_playnwcode = cPayNetworkCode,r_paysubnwcode = cPaySubNetworkCode,r_branchnumber = cBranchNumber,
		r_accounttcod = cAccountTypeCode,r_accountnumber = cAccountNumber,r_originamount = cOriginAmount,r_destinamount = cDestinationAmount,r_rexchangerate = cRetailExchangeRate,
		r_wholesalerate = cWholesaleExchangeRate,r_deexhangerate = cDestinExchangeRate,r_servfeeamount = cServiceFeeAmount,r_discountamoun = cDiscountAmount,r_typecode = cTypeCode,
		r_accountnum = cAccountNumber_c,r_biccode = cBicCode,r_refnumber = cReferenceNumber,r_customernum = cCustomerNumber,r_firstname = cFirstName,
		r_middlename = cMiddleName,r_lastname = cLastName,r_mommaidenname = cMotherMaidenName,r_address = cAddress,r_city = cCity,
		r_countrycode_a = cCountryCode_a,r_statecode = cStateCode,r_zipcode = cZipCode,r_typecode_i = cTypeCode_i,r_number = cNumber,
		r_expirdate = cExpirationDate,r_isscontrycode = cIssuerCountryCode,r_issstatecode = cIssuerStateCode,r_dateofbirth = cDateOfBirth,
		r_customernum_b = cCustomerNumber_b,
		r_firstname_b = cFirstName_b,r_middlename_b = cMiddleName_b,r_lastname_b = cLastName_b,r_mommaidenna_b = cMotherMaidenName_b,r_firstname_f = cFirstName_f,
		r_middlename_f = cMiddleName_f,r_lastname_f = cLastName_f,r_mommaidenna_f = cMotherMaidenName_f,r_address_b = cAddress_b,r_city_b = cCity_b,
		r_countrycode_b = cCountryCode_b,r_statecode_b = cStateCode_b,r_zipcode_b = cZipCode_b,r_email = cEmail,r_homephonenum = cHomePhoneNumber,
		r_workphonenum = cWorkPhoneNumber,r_number_cl = cNumber_cl,r_receiveemail = cReceiveEmail,r_receivesms = cReceiveSMS,r_typecode_ib = cTypeCode_ib,
		r_number_ib = cNumber_ib,r_expirdate_ib = cExpirationDate_ib,r_issconcode_ib = cIssuerCountryCode_ib,r_issstacode_ib = cIssuerStateCode_ib,r_reastypecode = cReasonTypeCode,
		r_refortransfer = cReasonForTransfer,r_sourceoffunds = cSourceOfFunds,r_securphrase = cSecurityPhrase,r_feemessage = cFreeMessage,user_insert = cUsuarioInsert,fecha = cFechaInser,
		description_osc = cDescription_osc,folio_sucursal = cFolio_suc,id_sucursal = cId_sucursal,desc_sucursal = cDesc_sucursal,
		typecodeci = cR_typecode_i,numberci = cR_number,adress = cR_address_b,zipcode = cR_zipcode_b,city = cR_city_b,statecodeadr = cR_statecode_b,
		countrycodeadr = cR_countrycode_b,homephonenum = cR_homephonenum,forma_pago = cForma_pago,date_birth = cDateBirth,iss_uercc = cIssuercc,contry_code = cContryCode
		WHERE usuario_insert_proc = pUsuario;
...
'FECHA 21/05/2018',
'DESCRIPCION: Se realiza clon de spl sp_rem_consultasacappqryi.',
'Se actualiza la tabla para la consulta de los campos correspondientes al panel datos de pago.',
'Se actualizan campos para recuperar informaciÃÂ³n correspondiente al panel direcciÃÂ³n del beneficiario.',
'Se implementa mÃÂ©todo de optimizaciÃÂ³n para controlar el timeout del proceso.',
'AUTOR: L. Montserrat LeÃÂ³n Amador',
'FECHA 30/07/2018',
'DESCRIPCION: Se actualiza filtro r_uniquerefnum por unirefnum a la tabla sac_app_payi.',
'Se realiza cambio de nomenclatura a la tabla sp_rem_statusappqryi2 por sw_verifica_estatusrem.',
'BD: bdicnweb';
