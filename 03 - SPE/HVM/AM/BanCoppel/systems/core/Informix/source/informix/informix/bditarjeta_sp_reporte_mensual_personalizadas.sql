CREATE PROCEDURE "informix".sp_reporte_mensual_personalizadas()
    ...
    /*SET DEBUG FILE TO "/ifxsif01/_reportes/sp_reportes_mensual_personalizada.out";
    TRACE ON;*/
    ...
    LET 	DescRetorno = 'EjecuciÃ³n de proceso exitosa.';
    LET     viSqlErr = 0;
    LET 	viSamErr = 0;
...
BEGIN
	...
	ON EXCEPTION
        ...
        LET vsql = '';
        LET vsql = ' rm -f /RESPALDOSNEW/suc_tipo_tarjeta_15.unl';
        SYSTEM vsql;
    ...
    /*Se verifica el tipo periodo de ejecucion del Reporte*/	
	...
	IF (vPeriodoAnterior = v_ultimo_Periodo) THEN -- El proceso ya se ejecutÃ³ para el periodo
    ...
    LET vsql  = '';
    LET vsql  = 'echo "UNLOAD TO /RESPALDOSNEW/suc_tipo_tarjeta_15.unl ' ||
	...
	---CreaciÃ³n de Ã­ndices en periodo, sucursal e imagen.
	...
	--IntegraciÃ³n de Registros de AsignaciÃ³n de Tarjetas a Estructura rpt_stock_venta_tp
    ...
    --8) Descarga de Reposiciones de Tarjetas (que fueron sustituciÃ³n)
	--Se busca primero las tarjetas que corresponden a reposiciones
	    ...
	    CREATE INDEX "informix".idx_numtarjetasustituta 
	...
	--9) IntegraciÃ³n de Registros de Reposicion de Tarjetas a Estructura Base
    ...
    where inv.periodo = (select rep.periodo from tt_reposicion_asignacion_15 rep
        ...
        CREATE INDEX "informix".idx_tt_venta_15_periodo
            ON "informix".tt_venta_15(periodo,sucursal,imagen) ONLINE;
    ...
    UPDATE STATISTICS MEDIUM FOR TABLE "informix".rpt_stock_venta_tp;  
		...
		let vsql = ''; 	   
	    let vsql = 'echo "Periodo|TipoTarjeta|Sucursal|Nombre de Sucursal|Imagen|Nombre de la Imagen|Venta|Asignacion|Reposicion">/RESPALDOSNEW/TDpersonalizada_'|| day(vfecha_hoy)||LPAD (MONTH(vfecha_hoy),2,"0")||year(vfecha_hoy)||'.unl';
		system vsql;
		let vsql = '';
		let vsql = '';
		let vsql=  'echo "UNLOAD TO /RESPALDOSNEW/TDpersonalizada.unl select periodo,clave_tipotarjeta,clave_sucursal,nombre_sucursal,id_diseno,descripcion_diseno,venta,asignacion,reposicion from rpt_stock_venta_tp where clave_tipotarjeta = 15 and periodo = ' 
					|| vPeriodoAnterior || ';">/RESPALDOSNEW/tdpersonalizada.sql'; 
		system vsql;
		let vsql ='';
		let vsql= 'chmod 777 /RESPALDOSNEW/tdpersonalizada.sql';
		system vsql;
		let vsql ='';
		let vsql= 'dbaccess bditarjeta /RESPALDOSNEW/tdpersonalizada.sql';
		system vsql;
		let vsql = '';
		let vsql ='rm /RESPALDOSNEW/tdpersonalizada.sql';
		system vsql;
		let vsql ='';
		let vsql = "sed 's/|$//g' /RESPALDOSNEW/TDpersonalizada.unl >>/RESPALDOSNEW/TDpersonalizada_"|| day(vfecha_hoy)||LPAD (MONTH(vfecha_hoy),2,"0")||year(vfecha_hoy)||".unl";
		system vsql;
		let vsql ='rm /RESPALDOSNEW/TDpersonalizada.unl';
		system vsql;
...
CREATE PROCEDURE "informix".sp_bf_info_tarjetas (
    pFechaInicio DATETIME YEAR TO FRACTION(5), 
    pFechaTermino DATETIME YEAR TO FRACTION(5), 
    pNumeroTarjeta CHAR(16), 
    pSecuencia CHAR(7)
)    
    RETURNING  VARCHAR(16) as  NumTarjeta, CHAR(7) as SecuenciaOrigen,
        VARCHAR(16) as SecuenciaExtendida, DECIMAL(19,4) as MontoIntercard,
        DATETIME YEAR TO FRACTION(5) as FechaTransaccion, CHAR(16) as NumReferenciaIntercard,
        CHAR(13) as NumeroCuenta, CHAR(3) as EstatusTarjeta,
        ...
        DEFINE NumTarjeta VARCHAR(16);
        DEFINE SecuenciaOrigen CHAR(7);
        DEFINE SecuenciaExtendida CHAR(16);
        DEFINE MontoIntercard DECIMAL(19,4);
        DEFINE FechaTransaccion DATETIME YEAR TO FRACTION(5);
        
        DEFINE NumReferenciaIntercard CHAR(16);
        DEFINE NumeroCuenta CHAR(13);
        DEFINE EstatusTarjeta CHAR(3);
        DEFINE Referencia23_325 CHAR(23);
    ...
    BEGIN
        ...
        LET NumTarjeta = '';
        LET SecuenciaOrigen = '';
        LET SecuenciaExtendida = '';
        LET MontoIntercard = 0;
        LET FechaTransaccion = CAST('1900-01-01 12:00:00' as DATETIME YEAR TO FRACTION(5));        
        LET NumReferenciaIntercard = '';
        LET NumeroCuenta = '';
        LET EstatusTarjeta = '';
        LET Referencia23_325 = '';
        
        --SET DEBUG FILE TO RUTA_DESTINO||"sp_bf_info_tarjetas.out";
        --TRACE ON;
        ...
        IF (pFechaInicio IS NULL OR pFechaTermino IS NULL) THEN
            ...
            RETURN NumTarjeta,SecuenciaOrigen, SecuenciaExtendida, MontoIntercard,	
                        FechaTransaccion, NumReferenciaIntercard,	NumeroCuenta, EstatusTarjeta, Referencia23_325;
        END IF;
        ...
        SELECT LIMIT 1
            mov.numtarjeta , mov.secuencia , mov.secuenciaextendida ,NVL (mov.monto ,0 ) as monto,
            mov.fechahorainauth ,mov.referencia ,tjtcta.numcuenta,
            tjt.codstatustarjeta , mov.referencia23_325 as referencia23_325
            
            INTO NumTarjeta,SecuenciaOrigen, SecuenciaExtendida, MontoIntercard, 
                FechaTransaccion, NumReferenciaIntercard,NumeroCuenta, 
                    EstatusTarjeta, Referencia23_325
            FROM                
                bditarjeta:tbl_bf_movs_cnc_sorteo mov,
                    intercard:tarjetacuenta tjtcta,                     
                        intercard:tarjeta tjt
            WHERE  mov.fechahorainauth >= pFechaInicio
                AND mov.fechahorainauth <= pFechaTermino
            AND mov.numtarjeta = pNumeroTarjeta
                AND mov.secuencia = pSecuencia
                    AND mov.numtarjeta = tjt.numtarjeta                    
                    AND mov.numtarjeta = tjtcta.numtarjeta
                    AND mov.prodind = '02' 
                    AND mov.codigoiso = '00'
                    AND mov.movreversado = 'F'
                    AND mov.formato = '0200'
                    AND mov.esnacional = 'V'
                    AND mov.movconciliado IN ('P' ,'V')
                    AND mov.transaccionorigen = '1234' ---Unicamente esta considerada la transaccionalidad con eglobal.
                    AND mov.tipotransaccionposdigitada IN ('', 'AV', 'CE', 'HO', 'ND', 'TG' ) --Cadena de caracteres registradas hasta octubre 2018
                AND (mov.monto >= 250 );              

        RETURN 	
            NumTarjeta,SecuenciaOrigen, SecuenciaExtendida, MontoIntercard,	
                        FechaTransaccion, NumReferenciaIntercard, NumeroCuenta, EstatusTarjeta, Referencia23_325;
...
'Fecha de modificacion: 18.diciembre.2019',
'Descripcion: Consulta para obtener el registro de la tabla movimiento de acuerdo a la tarjeta y secuencia',
'Base de datos: bditarjeta'
;

CREATE PROCEDURE "informix".sp_bf_buscar_estado_pob(
		psNumtarjeta CHAR(16),
		piconsecutivo INTEGER
	)
RETURNING CHAR(5) AS Retorno, CHAR(1) AS encontrado;
	...
	DEFINE vsnumcliente1	CHAR(13);
	DEFINE vscodpostal 		CHAR(5);
	...
	BEGIN
		ON EXCEPTION SET viCodigo
            ...
            RETURN vssqlerr, NVL(vsEncontrado,'');
        END EXCEPTION;
	
        --SET DEBUG FILE TO '/resplogifx/sp_bf_buscar_estado_pob.out';
        --TRACE ON;
		...
		LET vsnumcliente1 = vsnumcliente;
		
						
		IF ( (vsnumcliente1 IS NULL) OR ( TRIM (vsnumcliente1) = ''))  THEN			
			LET vssqlerr = '00001';
			LET vsEncontrado = 'F';
		ELSE
		    LET vssqlerr = '00000';
		...
		IF (vsEncontrado = 'V') THEN
            SELECT zona.municipiozona, cd.nombreciudad, edo.nombre, dir.cod_postal 
                INTO vsmunicipiozona, vsnombreciudad, vsnomedo, vscodpostal
            ...
            WHERE
                dir.numcte = vsnumcliente1 AND 
                dir.tipo_dir = '1'; -- EL TIPO DE DIRECCION PUEDES SER 1, 2, 3
					
                LET vspoblacion = TRIM(NVL(vsmunicipiozona,'')) || ' ' ||TRIM (NVL(vsnombreciudad,''));
                LET vspoblacion = LPAD( TRIM(vspoblacion), 30, ' ');
                LET vsnomedo = LPAD( TRIM(vsnomedo), 30, ' ');                

				UPDATE bditarjeta:"informix".tbl_bf_sorteo_sat
                    SET estado = nvl(vsnomedo,''),
                        poblacion = nvl(vspoblacion,''),
                        codpostal = nvl(vscodpostal,'')
