CREATE PROCEDURE "informix".sp_tdcoro_web(pEmpresa VARCHAR(3), pSucursal VARCHAR(4),pBusquedaSuc VARCHAR(20),pOpcion INTEGER, pSecuencia INTEGER,pNumCliente VARCHAR(20))
...
BEGIN
	...
	--SET DEBUG FILE TO "/tmp/tdcoro.out";
	--TRACE ON;
		...
		IF  TRIM(pNumCliente) != "" AND pSecuencia != 0 THEN
			...
			IF v_salario < 9000 THEN
				...
				IF (TRIM(cFlagStatusSol) IS NULL OR TRIM(cFlagStatusSol) = '' OR TRIM(cFlagStatusSol) = '1') THEN
					IF DBINFO("sqlca.sqlerrd2") = 0  THEN
						FOREACH
							SELECT LIMIT 1 flag_oro, TO_CHAR(fecha_insert, '%Y-%m-%d')
							INTO cFlagStatusSol, cFchSoli
							FROM bdisolic: "informix".ss_solicitudes_tdcoro
							where numcte = pNumCliente
							AND numero_solicitud = pBusquedaSuc
							AND linea_teorica <> 0
							ORDER BY fecha_insert DESC
						END FOREACH;
					    
					    IF DBINFO("sqlca.sqlerrd2") = 0  THEN
							SELECT linea_teorica
							INTO cLinea
							FROM bdisolic: "informix".ss_solicitudes_tdcoro
							where numcte = pNumCliente
							AND numero_solicitud = pBusquedaSuc;
	...
	IF  cCodRet <> "00000" OR pOpcion <> 1 THEN
		RETURN TRIM(NVL(cCodRet,'')), TRIM(NVL(cNumeroSucursal,'')), TRIM(NVL(cNombreSucursal,'')), TRIM(NVL(cDireccionSucursal,'')),
		TRIM(NVL(cColoniaSucursal,'')), TRIM(NVL(cEstadoSucursal,''));
	END IF
...
DOCUMENT
'Folio: 736',
'RQM 10 1339 - Tarjetas de Crï¿½?Â©dito Oro nominadas_ innominadas',
'Crear procedimiento para Tarjetas de Credito Oro',
'Autor: 94206041 Jesus Rosario Lopez Castro',
'BD: bdicred',
'Fecha: 24-03-2021',

'Modifica: Victor Vazquez',
'FECHA: 14/08/2023',
'Descripcion: Se agrega validacion en la tabla bdisolic:ss_solicitudes_tdcoro',
'BD: bdisolic';
