create procedure "informix".sp_transfer_genarch_svaincomming (psfechahoy date)
returning
...
begin
   on exception set sql_err, isam_err, error_info
		set debug file to "/resplogifx/sp_transfer_genarch_svaincomming1.out";
		trace on;
...
--set debug file to "/informix/HomeInformix/rrm/sp_transfer_genarch_svaincomming.out";
--trace on;
...
select count(consecutivo) 
	into vicontador 
	from Bditransfer:"informix".tf_sva_incoming
		where 	status_envio = 'P' and
				fecha_proceso = vdfechahoy;

-- Arma Nombre del archivo 
let vsnombrearchivo = trim(vsprefijo_archivo)||lpad(day(vdfechahoy),2,'0')||lpad(month(vdfechahoy),2,'0')||year(vdfechahoy)||'_1.csv';
...
if vicontador = 0 then 
	let vsheader= '0,000000,'||year(vdfechasistema)||lpad(month(vdfechasistema),2,'0')||lpad(day(vdfechasistema),2,'0')||replace(EXTEND(vdfechasistema, HOUR TO MINUTE),':','');
else
	let vsheader= '0,'||vscontador||','||year(vdfechasistema)||lpad(month(vdfechasistema),2,'0')||lpad(day(vdfechasistema),2,'0')||replace(EXTEND(vdfechasistema, HOUR TO MINUTE),':','');
end if;
...
if vicontador > 0 then 
	...
	let vssql = '';
	let vssql = '';
	--let vssql = 'dbaccess bditransfer '||trim(vsrep_aix)||'descargasvaincomming.sql';  -- Desarrollo
	let vssql = '/ifxsif01/bin/dbaccess bditransfer '||trim(vsrep_aix)||'descargasvaincomming.sql';  -- Producción
	system vssql;
	-- Borra archivo del comando que genero el archivo 
	let vssql = '';
	let vssql = '';
	--let vssql = 'rm '||trim(vsrep_aix)||'descargasvaincomming.sql';    -- Desarrollo
	let vssql = '/usr/bin/rm '||trim(vsrep_aix)||'descargasvaincomming.sql';   -- Produccion
	system vssql;

	-- Borra el ultimo caracter y lo pone en otro archivo 

	let vssql = '';
	let vssql = '';
	--let vssql = 'cat '||trim(vsrep_aix)||'tfsvaincoming.unl | sed ''s/.$//'' >> '||trim(vsrep_aix)||'tfsvaincoming1.unl';   --- Desarrollo
	let vssql = '/usr/bin/cat '||trim(vsrep_aix)||'tfsvaincoming.unl | sed ''s/.$//'' >> '||trim(vsrep_aix)||'tfsvaincoming1.unl';  -- ¨Producción
	system vssql;

	-- Concatena archivo con encabezado con el archivo generado por la descarga 
	let vssql ='';
	let vssql =''; 
	--let vssql = "sed 's/|$//g' " || TRIM(vsrep_aix) || TRIM ('tfsvaincoming1.unl') || " >> " || TRIM(vsrep_aix) ||  TRIM (vsnombrearchivo);  -- Desarrollo
	let vssql = "/usr/bin/sed 's/|$//g' " || TRIM(vsrep_aix) || TRIM ('tfsvaincoming1.unl') || " >> " || TRIM(vsrep_aix) ||  TRIM (vsnombrearchivo);  -- Producción
	system vssql;

	-- Borra archivos de la descarga
	let vssql ='';
	let vssql ='';
	--let vssql = 'rm '||trim(vsrep_aix)||'tfsvaincoming.unl';         -- Desarrollo 
	let vssql = '/usr/bin/rm '||trim(vsrep_aix)||'tfsvaincoming.unl';   -- Produccion
	system vssql;

	let vssql ='';
	let vssql ='';
	--let vssql = 'rm '||trim(vsrep_aix)||'tfsvaincoming1.unl';    ---Desarrollo
	let vssql = '/usr/bin/rm '||trim(vsrep_aix)||'tfsvaincoming1.unl';  -- Produccion
	system vssql;
	...
	set isolation to dirty read;
		...
		foreach cusor1 with hold for
			...
			update Bditransfer:"informix".tf_sva_incoming
				set status_envio = 'V', 
					nombre_archivo_envio = vsnombrearchivo
				where status_envio = 'P' and
					 fecha_proceso <= vdfechahoy and 
...
'Descripcion: Proceso para el marcado de los resgitros con el nomvre del archivo con el cual van a ser enviados y el estatus de envio y generacion del archivo',
'Fecha: 2014/09/26',
...
'Proyecto: Integracion de Transfer',
'Solicito: Jose Luis Puebla Salinas',
'Descripcion: Se le ponen instrucciones para comandos a nivel producción',
'Fecha: 2015/01/27',
'Version: 20150127.18:45',
'BD: bditransfer';
