CREATE PROCEDURE "informix".actual_sdos_dia()
       RETURNING CHAR(5), CHAR(20);


   DEFINE GLOBAL g_num_credito          CHAR(20)      DEFAULT " ";
   DEFINE GLOBAL g_sdo_intereses        MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_sdo_dia_ant_int      MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_sdo_mes_ant_int      MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_sdo_acum_mes_int     MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_provision_normal     MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_dias_acum_int        SMALLINT      DEFAULT 0;
   DEFINE GLOBAL g_sdo_moratorio        MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_sdo_dia_ant_mor      MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_sdo_mes_ant_mor      MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_sdo_contab_mora      MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_dias_acum_mora       SMALLINT      DEFAULT 0;
   DEFINE GLOBAL g_sdo_cap_insoluto     MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_sdo_dia_ant_cap      MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_sdo_mes_ant_cap      MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_sdo_acum_mes_cap     MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_dias_acum_cap        SMALLINT      DEFAULT 0;
   DEFINE GLOBAL g_sdo_acum_vencido     MONEY(14,2)   DEFAULT 0;
   DEFINE GLOBAL g_dias_acum_intper     SMALLINT      DEFAULT 0;
   ...
   DEFINE p_fecha_proceso             DATE;


   ON EXCEPTION SET sql_err, isam_err, error_info
      LET v_codret = sql_err;
      SET DEBUG FILE TO "Actual_Sdos_Dia.err";
   ...
   FOREACH
      ...
      IF p_fecha_proceso != g_ult_dia_mes THEN 
         LET g_sdo_dia_ant_int    = g_sdo_intereses;
         LET g_sdo_intereses      = 0;
         LET g_sdo_dia_ant_mor    = g_sdo_moratorio;
         LET g_sdo_contab_mora    = 0;
         LET g_sdo_dia_ant_cap    = g_sdo_cap_insoluto;

         UPDATE sd_maesdos
         SET sdo_dia_ant_int      = g_sdo_dia_ant_int,
             sdo_intereses        = g_sdo_intereses,
             provision_normal     = g_provision_normal,
             sdo_dia_ant_mor      = g_sdo_dia_ant_mor,
             sdo_contab_mora      = g_sdo_contab_mora,
             sdo_dia_ant_cap      = g_sdo_dia_ant_cap ,
             provi_venc_normal    = g_provi_venc_nor,
             provi_venc_anticip   = g_provi_venc_ant 
         WHERE num_credito = g_num_credito
         AND empresa = g_empresa;
      END IF;


      IF p_fecha_proceso = g_ult_hab_mes THEN 
   
         LET g_sdo_dia_ant_int    = g_sdo_intereses;
         LET g_sdo_intereses      = 0;
         LET g_provision_normal   = 0;
         LET g_sdo_mes_ant_int    = g_sdo_acum_mes_int;
         LET g_sdo_acum_mes_int   = 0;
         LET g_dias_acum_int      = 0;

         LET g_sdo_dia_ant_mor    = g_sdo_moratorio;
         LET g_sdo_mes_ant_mor    = g_sdo_moratorio;
         LET g_sdo_contab_mora    = 0;
         LET g_dias_acum_mora     = 0;

         LET g_sdo_mes_ant_cap    = g_sdo_cap_insoluto;
         LET g_sdo_dia_ant_cap    = g_sdo_cap_insoluto;
         LET g_provi_venc_nor     = 0;
         LET g_provi_venc_ant     = 0;
         ...
         SET sdo_dia_ant_int      = g_sdo_dia_ant_int,
             sdo_intereses        = g_sdo_intereses,
             provision_normal     = g_provision_normal,
             sdo_mes_ant_int      = g_sdo_mes_ant_int,
             sdo_acum_mes_int     = g_sdo_acum_mes_int,
             dias_acum_int        = g_dias_acum_int,
             sdo_dia_ant_mor      = g_sdo_dia_ant_mor,
             sdo_mes_ant_mor      = g_sdo_mes_ant_mor,
             sdo_contab_mora      = g_sdo_contab_mora,
             dias_acum_mora       = g_dias_acum_mora,
             sdo_dia_ant_cap      = g_sdo_dia_ant_cap,
             sdo_mes_ant_cap      = g_sdo_mes_ant_cap,
             provi_venc_normal    = g_provi_venc_nor,
             provi_venc_anticip   = g_provi_venc_ant 
         WHERE num_credito = g_num_credito
         AND empresa = g_empresa;
      ...
      IF v_evento_int <> 0 THEN
         LET g_sdo_acum_mes_cap = 0;
         LET g_sdo_acum_vencido = 0;
         LET g_dias_acum_cap    = 0;
      
         LET g_sdo_acum_intper  = 0;
         LET g_dias_acum_intper = 0;
  
         UPDATE sd_maesdos
         SET sdo_acum_mes_cap  = g_sdo_acum_mes_cap ,
             sdo_acum_vencido  = g_sdo_acum_vencido ,
             dias_acum_cap     = g_dias_acum_cap    ,
             sdo_acum_intper   = g_sdo_acum_intper,
             dias_acum_intper  = g_dias_acum_intper
         WHERE num_credito = g_num_credito
         AND empresa = g_empresa;
      END IF;
   ...
   --CALL Audita_Rev_Tasa() RETURNING v_codret;
...
END PROCEDURE
DOCUMENT
"Realiza la actualizacion de saldos dia anterior se saco del cierre ",
"Por razones de reportes posteriores ",
"AUTOR : Jose Cruz Narvaez",
"Mod.  : Raul Mendoza D'nes ",
"      : 14/Agosto/2001",
"BD.:  : bdicred",
"Mod   : Raul Mendoza D'nes",
"      : 7/Septiembre/2001",
"      : Actual saldos para cierre de primer dia de mes cuando es inhabil",
"ACT   : Se manda para actualizar en GE el 4/dic/2001",
"Mod   : Sergio Fabricio Ruiz Jimenez",
"      : 11/Julio/2002",
"      : Revision De Tasas Al Inicio de Dia Para Actualizacion ",
"ACT   : A La Siguiente Fecha de Pago De Interes. Para Actualizacion en GE el 12/Jul/2002";

CREATE PROCEDURE "informix".cal_tradicion()
        RETURNING CHAR(5),       CHAR(20),
   ...
   DEFINE v_cap_pro_reno       DECIMAL(14,2);
   DEFINE base_calculo_pr      DECIMAL(14,2);
   ...
   ON EXCEPTION SET sql_err, isam_err, error_info
      ...
      LET base_calculo      = 0;
      SET DEBUG FILE TO "Cal_Tradicion.err";
      TRACE sql_err||" * "||isam_err|| " * "||error_info;
      RETURN v_codret,      g_num_credito,
          interes_diario,   interes_proreno, 
          interes_moraord7, interes_moracop7,
          interes_moraord2, interes_moracop2, base_calculo;
   END EXCEPTION;


   LET v_codret          = "000";
   LET v_cap_pro_reno    = 0;
   LET base_calculo_pr   = 0;
   LET interes_proreno   = 0;
   LET interes_moraord7  = 0;
   LET interes_moracop7  = 0;
   LET interes_moraord2  = 0;
   ...
   WHERE num_credito = g_num_credito AND
         empresa = g_empresa AND
         status_cuota IN ("1") AND
         cuota_rec IN ("6","8");
        

   IF v_cap_pro_reno IS NOT NULL AND v_cap_pro_reno > 0 THEN 
      LET base_calculo_pr =  v_cap_pro_reno;
      LET interes_proreno =  (base_calculo_pr * (g_tasa_interes / 100)) 
                              / 360;
   ELSE
      LET v_cap_pro_reno    = 0;
   END IF;

   LET base_calculo    = g_sdo_capital;
   LET interes_diario  = (base_calculo * (g_tasa_interes /100)) / 360;

   LET base_calculo_mo  = g_monto_vencido;
   LET interes_moraord7 = (base_calculo_mo * (g_tasa_interes /100))/360;
   LET base_calculo_mc  = g_monto_vencido;
   LET interes_moracop7 = (base_calculo_mc * (g_tasa_morato/100))/360;

   LET base_calculo_mo  = 0 ;
   LET base_calculo_mc  = 0 ;
   LET base_calculo_mo  = g_mto_venc_trasp;
   LET interes_moraord2 = (base_calculo_mo * (g_tasa_interes /100))/360;
   LET base_calculo_mc  = g_mto_venc_trasp;
...
END PROCEDURE
DOCUMENT
"Funcion que realiza el calculo de intereses para el tipo de calculo tradicional",
"Autor : Jose Cruz Narvaez",
"Fecha : 2/Mayo/2001",
"BD    : bdicred",
"Ver   : 1.0",
