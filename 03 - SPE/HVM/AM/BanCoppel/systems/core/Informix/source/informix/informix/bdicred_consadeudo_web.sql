CREATE PROCEDURE "informix".consadeudo_web(f_num_credito CHAR(20),
                                         f_empresa     CHAR(3))
RETURNING CHAR(5),      -- Codigo Retorno
         ...
         MONEY(14,2),   -- Total vencido
         CHAR(20),      -- Numero de Cliete
         DATE,          -- fecha proximo pago;
         MONEY(14,2);   -- Monto Proximo Pago
...
DEFINE r_TotVenc     MONEY(14,2);
DEFINE r_NumCte      CHAR(20);
DEFINE r_FecProxPag  DATE;
DEFINE r_MontoProxPag MONEY(14,2);
      ...
      LET v_codret = sqlerr;
        RETURN v_codret, r_nomcte, r_producto, r_nomprod, r_divisa, r_nomdiv,
	       ...
	       r_comis , r_stcred, c_num_cta,  r_numcuot, r_adeudovenc,
               r_capinsol, r_TotVenc, r_numCte, r_FecProxPag, r_MontoProxPag;
 END EXCEPTION;
...
SELECT {+ INDEX (si_divisas 192_418)} nombre1, nombre2, apell_paterno, apell_materno, razon_social,
       a.num_producto, nombre_prod, a.divisa, c.descripcion, a.status_cred||
       " " ||TRIM(e.descripcion) stcred, a.numcte
  INTO v_nom1, v_nom2, v_appa, v_apma, v_rs, r_producto, r_nomprod, r_divisa,
       r_nomdiv, r_stcred, r_numcte
  FROM sd_maecred a, bdinteg:si_cliente b, bdinteg:si_divisas c,
       sd_definicion d, sd_tipocartera e
 WHERE num_credito = f_num_credito
   ...
   SELECT sdo_exig_int, monto_vencido + mto_venc_trasp, sdo_moratorio,
     ...
     INTO r_intven, r_capven, r_intmor, v_sdocap, r_capinsol
     FROM sd_maesdos
    WHERE num_credito = f_num_credito
    AND empresa = f_empresa;

   -- Determina si la cuota vigente de capital ya es exigible
   SELECT {+ INDEX (sd_pagocapit 185_530)} fecha_cuota, saldo_cuota - monto_real_pag
   ...
   SELECT {+ INDEX (sd_paginter 186_542)} fecha_cuota, monto_cuota - monto_real_pag
  ...
  SELECT COUNT(1) INTO v_desem FROM sd_detminis
   WHERE num_credito = f_num_credito
     AND empresa = f_empresa
  ...
  END IF
   ...
   SELECT {+ INDEX (sd_detcomi inx_detcomi)} SUM(monto_com) INTO r_comis
   ...
   SELECT {+ INDEX (sd_detcomi inx_detcomi)} SUM(monto_com) INTO r_seguro
     FROM sd_detcomi a, sd_tpcomis b
    WHERE b.cod_comis = a.cod_comis
      AND b.empresa = a.empresa
      AND b.comi_o_seg = "2"
  ...
  -- Determina el numero de cuotas vencidas y el monto vencido
   ...
   WHERE
      empresa = f_empresa
   AND
      num_credito = f_num_credito
   AND
     status_cuota in ("2", "7");

   LET r_adeudovenc = r_intven + r_capven;

   SELECT MIN {+ INDEX (sd_pagocapit 185_530)} (fecha_cuota)
     INTO wfecha
     FROM sd_pagocapit
    WHERE num_credito = f_num_credito
      AND empresa = f_empresa
   ...
   IF (wfecha IS NOT NULL) THEN
      SELECT {+ INDEX (sd_pagocapit 185_530)}
         a.saldo_cuota - a.monto_real_pag,
      ...
      where
         a.empresa = f_empresa
      AND
         a.num_credito = f_num_credito
      AND
         a.fecha_cuota = wfecha
      AND
         b.empresa = a.empresa
      AND
         b.num_credito = a.num_credito
      AND
         b.fecha_cuota = a.fecha_cuota;
      ...
      LET r_MontoProxPag = winteres + wcapital;
      LET r_FecProxPag = wfecha;
   END IF;
