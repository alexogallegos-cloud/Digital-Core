CREATE PROCEDURE "informix".spsdpagotarcredpropia(pEmpresa char(3),
                                                pSucursal char(4),
                                                pUsuario char(8),
                                                pTransCargo char(4),
                                                pTransAbono char(4),
    ...
    -- Fecha     	: 26/05/2008
	-- Modificacion : 07/07/2015 Cambio del spl princial a principalrefer
	-- Modificacion : 03/04/2025 Cambio para no permitir los pagos de productos '7800','6900','8900'
...
on exception in (-535)
    ...
    COMMIT WORK;
    BEGIN WORK;
    --set debug file to '/tmp/spsdpagotarcredpropia.out';
    --trace on;
...
--Set debug file to '/home/e10000646/AJUSTES_SPS_APP/Base_de_Datos/informix/bdicred/SP/spsdpagotarcredpropia.out';				
--trace on;
--BEGIN
	...
	--SE MANDA A LLAMAR EL SPL. principalrefer a sugerencia del Ã¡rea de crÃ©dito en lugar del sp principal
    ...
    --IF vcodret <> '000' THEN
        ...
        EXECUTE PROCEDURE bdicred:reversion(pEmpresa, pSucursal, pUsuario, pFolioSuc, 'A') INTO vcodretRev;
        IF vcodretRev = '000' THEN
            LET vcodretRev = '001';
        END IF;
		
		EXECUTE PROCEDURE bdicheq:reversion(pEmpresa, pSucursal, pUsuario, pFolioSuc, 'A') INTO vcodretRev;
        IF vcodretRev = '000' THEN
            LET vcodretRev = '001';
        END IF;
		if vtransaccion = 1 then
