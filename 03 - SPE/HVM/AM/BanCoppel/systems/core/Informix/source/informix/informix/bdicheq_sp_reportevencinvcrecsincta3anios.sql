CREATE PROCEDURE "informix".sp_reportevencinvcrecsincta3anios( pSucursal CHAR(4), pRegistro INTEGER )
RETURNING CHAR(5)     AS RETORNO,			
    ...
    ON EXCEPTION SET vSqlError, vIsamError, vDescError
        SET debug file to "/tmp/sp_reportevencinvcrecsincta3anios.err";
        trace on;
        IF vSqlError <> 0 THEN
            ...
            RETURN cCodRet, cMensaje, cSucursal, cPromotor, cNumCte, cNombreCte, cCuenta, dFecha_apertura, 
                   dFecha_vencimiento, cTelefono1, cTelefono2, cTelefono3, cExtension, cMail WITH RESUME;
        END IF;
    END EXCEPTION;
    
    --- SET debug file to "/tmp/sp_reportevencinvcrecsincta3anios.out";
    --- trace on;
    ...
    ELSE
        ...
        FOREACH WITH HOLD
            SELECT sucursal, promotor, TRIM(numcte), TRIM(nombre_cte), TRIM(numcta), NVL(fecha_apertura,' '), NVL(fecha_vencimiento,' '), 
                   TRIM(NVL(telefono1,' ')), TRIM(NVL(telefono2,' ')), TRIM(NVL(telefono3,' ')), TRIM(NVL(extension,' ')), TRIM(NVL(correo_elect,' '))
              INTO cSucursal, cPromotor, cNumCte, cNombreCte, cCuenta, dFecha_apertura, dFecha_vencimiento,
                   cTelefono1, cTelefono2, cTelefono3, cExtension, cMail
              FROM bdicheq:sc_invcrecsincta3anios
