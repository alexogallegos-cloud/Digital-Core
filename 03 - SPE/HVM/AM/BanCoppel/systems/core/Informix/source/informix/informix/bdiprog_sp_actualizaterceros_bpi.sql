CREATE PROCEDURE "informix".sp_actualizaterceros_bpi(pNumCte char(20), pCuenta char (20), pCveBanco char(3), pClaveCuenta char (2), pAlias char(20),
		 pNombreTitular char(60), pRfc char(13),pCelular char (10), pCompaniaCel char (2),pCorreoE char (40))
returning char(5), char(60);


--Realizo: Javier A. Chávez Trujillo
--Solicito: Mauricio Leon
--Actividad: Actualizacion cuentas terceros BPI
-- Fecha: 03/12/2008
...
--Se toma el valor del estado para comparar y poder hacer la actualizacion
				...
				SELECT cve_estado INTO v_sCveEstado FROM bdiprog:pp_ctasterceros WHERE num_cte = pNumCte AND cuenta = pCuenta AND
					...
					IF v_sCveEstado = '01' THEN
						UPDATE bdiprog:pp_ctasterceros SET
							nombre = pNombreTitular, descrip_cta = pAlias, rfc =  pRfc, no_celular = pCelular,
							cve_compania = pCompaniaCel, direc_correo = pCorreoE WHERE num_cte = pNumCte and cuenta = pCuenta
							and cve_banco = pCveBanco AND cve_cuenta = pClaveCuenta;
