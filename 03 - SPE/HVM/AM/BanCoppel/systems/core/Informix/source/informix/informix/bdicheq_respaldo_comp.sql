create procedure "informix".respaldo_comp(pempresa char(3), ptipo_resp char(1))
returning char(5);
    ...
    on exception set vsqlerr, visamerr, vdescerr
        set debug file to "/tmp/respaldo_comp.err";
        trace on;
        if vsqlerr <> 0 then
            let vcodret = vsqlerr;
            ...
            LET vsql = 'echo "UPDATE bdinteg:sx_contproc '||
                       ...
                       'AND sistema   = '''||vsistema||''';" > /tmp/horasrespacie2.sql';
            SYSTEM vsql;
            LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /tmp/horasrespacie2.sql';
            SYSTEM vstmt;
            return vcodret;
        end if;
    end exception;
    ...
    --- set debug file to "/tmp/respaldo_comp.out";
    --- trace on;
    ...
    if ptipo_resp = "A" or ptipo_resp = "a" then
        ...
        let vproceso = "respacie2";
    else
        let vruta = "/cheques/d_cierre/";
        let vproceso = "respdcie2";
    end if
    ...
    if vexiste2 = 0 then
        LET vsql = 'echo " INSERT INTO bdinteg:sx_contproc VALUES '||
                   ...
                   '(SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas), NULL, NULL);" > /tmp/horasrespacie2.sql';
        SYSTEM vsql;
        LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /tmp/horasrespacie2.sql';
        SYSTEM vstmt;
    else
    ...
    foreach
        ...
        let vsql2 = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||vnomtabla||'SELECT * FROM '||vtabla||'; " > querychq2.sql';
        let vsql2 = vsql2;
        SYSTEM vsql2;
        LET vsql2 = "/ifxsif01/bin/dbaccess bdicheq querychq2.sql";
        SYSTEM vsql2;
        let vtabla = '';
    end foreach
