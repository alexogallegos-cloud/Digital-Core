CREATE PROCEDURE "informix".sp_constipooperacionesinternet(pUsuario CHAR(8), pIdfuncion CHAR(10))
	RETURNING CHAR(5) AS codret, 
		CHAR(4) AS idOperacion,
		CHAR(50) AS desc_oper;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodret = iSqlErr;
			RETURN cCodret, cIdOperacion, cConcepto;
		END EXCEPTION;
		...
		SELECT COUNT(*)
		INTO iExiste
		FROM bdinteg:si_bpioperaciones
		WHERE mostrar  = 't';
		...
		LET cIdOperacion = '9999';
		LET cConcepto = 'TODAS LAS OPERACIONES';
		RETURN cCodret, cIdOperacion, cConcepto WITH RESUME;
		
		LET cIdOperacion = '9998';
		LET cConcepto = 'POR CUENTA';
		RETURN cCodret, cIdOperacion, cConcepto WITH RESUME;
		
		FOREACH SELECT id_oper, desc_oper
			INTO cIdOperacion, cConcepto
			FROM bdinteg:si_bpioperaciones
			WHERE mostrar  = 't'
...
"FECHA: 04/11/2013",
"DESCRIPCION: Llena el combo Tipo de consulta";

CREATE PROCEDURE "informix".sp_consulta_reportebpioperacionesinternet(pUsuario CHAR(8), pIdFuncion CHAR(10), 
							pTipo INTEGER, pIdOperacion CHAR(5), pDtFechaIni DATE, pDtFechaFinal DATE, pCtaOrigen CHAR(20),
							pRegistros INTEGER, pRecuperacion INTEGER)
	RETURNING CHAR(5) AS codRetorno,
		CHAR(30) AS descrip,
		DATE AS fechaAplicacion,
		CHAR(20) AS ctaOrigen,
		CHAR(20) AS ctaDestino,
		MONEY(14,2) AS monto,
		CHAR(25) AS folioSuc,
		DATETIME YEAR TO FRACTION AS fechaOperacion,
		CHAR(25) AS refPago;
		...
		DEFINE iSqlErr INTEGER;
		DEFINE cDescrip CHAR(30);
		DEFINE dFechaAplicacion DATE;
		DEFINE cCtaOrigen CHAR(20);
		DEFINE cCtaDestino CHAR(20);
		DEFINE mMonto MONEY(14,2);
		DEFINE cFolioSuc CHAR(25);
		DEFINE dFechaOperacion DATETIME YEAR TO FRACTION;
		DEFINE cRefPago CHAR(25);
	...
	BEGIN
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cDescrip, dFechaAplicacion, cCtaOrigen, cCtaDestino, mMonto, cFolioSuc, dFechaOperacion, cRefPago;
		END EXCEPTION;	
		
		IF pTipo = 2 AND pCtaOrigen = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cDescrip, dFechaAplicacion, cCtaOrigen, cCtaDestino, mMonto,cFolioSuc, dFechaOperacion, cRefPago;
		END IF;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pTipo = '' OR pIdOperacion = '' OR pDtFechaIni = '' OR pDtFechaFinal = ''
						 OR  pRegistros = '' OR pRecuperacion = '' THEN 
			LET cCodRet = '00003';
			RETURN cCodRet, cDescrip, dFechaAplicacion, cCtaOrigen, cCtaDestino, mMonto,cFolioSuc, dFechaOperacion, cRefPago;
		END IF;
		...
		SET ISOLATION TO DIRTY READ;
		FOREACH EXECUTE PROCEDURE bdinteg:sp_consulta_bpioperaciones(pTipo,pIdOperacion, pDtFechaIni, pDtFechaFinal, pCtaOrigen) INTO cCodRetSp,cMensaje, cDescrip, dFechaAplicacion, cCtaOrigen, cCtaDestino, mMonto,cFolioSuc, dFechaOperacion, cRefPago
			...
			ELIF cCodRetSp = '00000' THEN
				IF iRegistros >= pRegistros THEN
					IF iRecuperacion < pRecuperacion THEN
						...
						RETURN cCodRet, cDescrip, dFechaAplicacion, cCtaOrigen, cCtaDestino, mMonto,cFolioSuc, dFechaOperacion, cRefPago WITH RESUME;
	...
	END;
END PROCEDURE
DOCUMENT "AUTOR: Fernando MartÃ­n Esparza Brenis",
"FECHA: 24/10/2013",
"DESCRIPCION: Recupera los registros para el reporte de operaciones de internet: SOCWEB";
