CREATE PROCEDURE "informix".sp_consultacuentasnominacliente_bpi(pEmpresa CHAR(3), pCliente CHAR(20), pNumRegistros SMALLINT)
RETURNING
	CHAR(5),
	CHAR(20),
	CHAR(40),
	...
	-- DESCRIPCIÃN	: Se modifica el SP para que muestre todas las cuentas de captaciÃ³n del cliente (No solo 10).
	...
	ON EXCEPTION SET cSqlErr
		IF cSqlErr <> 0 THEN
			let cCodRet = cSqlErr;
			RETURN cCodRet,cNumCuenta,cDesProducto,cCuentaCLABE,cNumProducto;
		END IF;
	END EXCEPTION;

	--SET DEBUG FILE TO "/sp_consultacuentasnominacliente_bpi.out";
	--TRACE ON; 
	...
	IF (SELECT COUNT(numcte) FROM bdinteg:"informix".si_cliente WHERE numcte = pCliente) > 0 THEN
		IF (SELECT COUNT(num_cte) FROM bdicheq:"informix".sc_maechq WHERE num_cte = pCliente AND status_cta = '1') > 0 THEN
				...
				FOREACH
					SELECT SKIP pNumRegistros FIRST 10 ma.cuenta,ma.cuenta_clabe,ma.producto,pr.nombre
					INTO cNumCuenta,cCuentaCLABE,cNumProducto,cDesProducto
					FROM bdicheq:"informix".sc_maechq AS ma
					FULL OUTER JOIN  bdicheq:"informix".sc_producto AS pr ON pr.producto=ma.producto
					WHERE ma.num_cte=pCliente
					AND ma.status_cta = '1'
					AND ma.producto IN((SELECT {+INDEX (sc_producportab, idx_prod_act)} producto FROM bdicheq:"informix".sc_producportab WHERE producto = ma.producto AND  activo = 1))
					ORDER BY ma.producto
					
					RETURN cCodRet,cNumCuenta,cDesProducto,cCuentaCLABE,cNumProducto WITH RESUME;
				END FOREACH;
		ELSE
			LET cCodRet = '00001';
		END IF;
...
'Fecha: 16/03/2016',
'ModificaciÃ³n: se clona sp para efectuar modificaciones acerca las cuentas que se repiten para portabilidad',
'BD: bdicheq',
'Se agrega la utilizaciÃ³n de Ã­ndices para reducir los costos',
