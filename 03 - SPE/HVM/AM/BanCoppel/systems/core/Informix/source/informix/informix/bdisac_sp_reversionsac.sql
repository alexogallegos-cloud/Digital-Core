CREATE PROCEDURE "informix".sp_reversionsac(pempresa CHAR(3),psucursal CHAR(4),pusuario  CHAR(8), pfolio    CHAR(16))
 RETURNING CHAR(5);

   DEFINE sql_err             		INTEGER;
   DEFINE cCodRet             		CHAR(5);
   DEFINE cNocontrol		  		CHAR(12);
   DEFINE cTransaccionUtilizada  	CHAR(4);
   ...
   --	2012.01.23 I
	...
	--	SET DEBUG FILE TO "/informix/tmp/sp_reversionsac.out";
	--	TRACE ON;
...
BEGIN
   ...
   LET cConvenio = '';
	...
	FROM bdicheq:sc_fechas;
        ...
        --Obtiene el número de control para verificar si en un EnvioYa
        Select {+INDEX (bdisac:sac_movimientos idxsac_mov114)} TRIM(referencia1), numcategoria, numconvenio 
        Into cNocontrol, cCategoria, cConvenio
        From bdisac:sac_movimientos
        Where folio_suc = pfolio;

        LET cont_exist = 0;

        SELECT COUNT (no_control) into cont_exist
        From bdisac:sac_enviosdineroya 
        Where no_control = cNocontrol
        And estatus not in ('00','05');

		IF cont_exist > 0 THEN

			--Obtiene el estatus del envío
			SELECT {+INDEX (bdisac:sac_enviosdineroya idxsac_envdinya13_1)} estatus 
			INTO cEstatus
			FROM bdisac:sac_enviosdineroya
			Where no_control = cNocontrol and estatus is not null;

			-- Verifica que este activo y se estareversando el Envío
			IF cEstatus = '01' AND cCategoria = '07' AND cConvenio = '001' THEN

					--Cancela EnvioYa y reguistra datos de reverso	
					Update {+INDEX (bdisac:sac_enviosdineroya idxsac_envdinya13_1)} bdisac:sac_enviosdineroya
					Set estatus = '05',
					fecha_cance = wfechoy,
					hora_cance = current hour to second,
					usua_cance = pusuario,
					suc_cance = psucursal
					Where no_control = cNocontrol and estatus is not null;
					--Activa el registro en el listado de movimientos
					UPDATE bdisac:sac_movimientos
					SET status_cancelado = 'S'
					Where numcategoria = '07'
					AND numconvenio = '001'
					AND referencia1 = cNocontrol;

			-- Verifica que este cancelado y se quiere reversar la cancelación
			ELIF cEstatus = '02' and  cCategoria = '07' AND cConvenio = '003' THEN
					--Activa el envío para su pago, se realiza el reverso de la cancelación o del pago.
					Update {+INDEX (bdisac:sac_enviosdineroya idxsac_envdinya13_1)} bdisac:sac_enviosdineroya
					Set estatus = '01',
					fecha_cance = wfechoy,
					hora_cance = current hour to second,
					...
					AND numconvenio = '003'
					AND referencia1 = cNocontrol;
			-- Verifica que este pagado
			ELIF cEstatus = '04' and  cCategoria = '07' AND cConvenio = '002' THEN
			--Activa el envío para su pago, se realiza el reverso de la cancelación o del pago.
...
'DESCRIPCION: Se genera proceso independiente para generar la reversion de DineroYa ahora Ordenes de Pagos', 
'Ademas de actualizar el movimientos del convenio que es el reverso.',
'FECHA: NOVIEMBRE 2009',
'BD: BDISAC',
'VERSION: 20100121.1309',
'MODIFICO: FRG',
'DESCRIPCION: Se modifico el IF EXIST',
