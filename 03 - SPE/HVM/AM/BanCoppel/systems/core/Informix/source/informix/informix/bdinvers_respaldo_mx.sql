create procedure "informix".respaldo_mx(pempresa char(3),
                                     ptipo_resp char(1))
returning char(5);
...
begin
   ...
   select fecha_hoy into vfecha_hoy
      from sv_fechas where empresa = pempresa;

   SELECT COUNT(*) INTO vEjecutado
   ...
   IF vEjecutado IS NULL OR vEjecutado = 0 THEN
       ...
       VALUES
         (pEmpresa, 'RespInv', vfecha_hoy, '03', 'I',
   ...
   foreach
      ...
      select 1 into vexiste
         from syscolumns
         where tabid = vtablaid and colname = "empresa";
      if vexiste is null then
         let vsql = 'echo "UNLOAD TO ' || TRIM(vnomtabla) ||
                    ' SELECT * FROM '||TRIM(vtabla) || '"' || ' > query.sql';
      else
         let vsql = 'echo "UNLOAD TO ' || TRIM(vnomtabla) ||
                    ' SELECT * FROM '||TRIM(vtabla) || ' where empresa = '||
                    pempresa || '"' || ' > query.sql';
      end if
      let vsql = vsql;
      SYSTEM vsql;
   ...
   update sv_contproc
      ...
      where empresa = pempresa and proceso = vproceso;
