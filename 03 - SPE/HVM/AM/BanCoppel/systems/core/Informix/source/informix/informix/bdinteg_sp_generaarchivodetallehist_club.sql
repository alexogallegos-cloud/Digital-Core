CREATE PROCEDURE "informix".sp_generaarchivodetallehist_club(pFecha DATE)
--DATOS A REGRESAR--
...
LET iIsamErr = 0;
LET vErrorInfo = '';
LET vProceso = 'sp_generaarchivodetallehist';
LET cSql = '';
LET cRuta = '';
LET cFechaArchivo = '';
LET cNumCte = '';
...
--SET DEBUG FILE TO "/informix/IrisA/sp_generaarchivodetallehist_club.out";
--TRACE ON;
	...
	ON EXCEPTION SET iSqlErr, iIsamErr, vErrorInfo
		IF iSqlErr <> 0 OR iIsamErr <> 0 THEN
			...
			INSERT INTO "informix".si_club_bitacoradomiciliacion(proceso, codigoretorno, mensajeretorno, fec_paquete)
				VALUES(vProceso, cCodret, vMensajeRet, pFecha);
			RETURN cCodret;
		END IF;
	END EXCEPTION;
	...
	IF TRIM(NVL(pFecha,'')) <> '' THEN
		...
		IF TRIM(NVL(cRuta,'')) <> '' THEN
			...
			FOREACH
				...
				WHERE fecha_alta = pFecha AND aceptada = '1'
				ORDER BY suc_alta
			...
			FOREACH
				...
				WHERE fech_alt = pFecha AND transacc IN('1303','1363','1393') AND empresa = '001' AND cancelad <> 'S'
				ORDER BY sucursal

				SELECT referencia1, referencia2 INTO cNumCteCoppel, cRecibo
				FROM bdisac:"informix".sac_movimientoshistorial
				WHERE fecha_pago = pFecha AND id_sucursal = cSucursal AND folio_suc = cFolioSuc;
			...
			FOREACH
				...
				FROM "informix".si_club_proteccion 
				WHERE fecha_cambio = pFecha AND suc_cambio = cSucursal AND tipo_mov = 'C'
				ORDER BY suc_cambio
			...
			FOREACH
				...
				LEFT JOIN "informix".si_club_beneficiario b ON (a.numcte = b.numcte AND b.secuencia = '2')
				LEFT JOIN "informix".si_club_beneficiario d ON (a.numcte = d.numcte AND d.secuencia = '3')
				WHERE a.fecha_modificacion = pFecha AND a.tipo_mov = 'C' AND a.secuencia = '1'
