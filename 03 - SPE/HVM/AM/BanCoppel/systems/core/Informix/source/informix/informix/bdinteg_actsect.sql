CREATE PROCEDURE "informix".actsect();

DEFINE wsector       LIKE si_sector.sector;
DEFINE wdescripcion  LIKE si_catalog.nombre;
DEFINE vdescripcion  LIKE si_sector.descripcion;
DEFINE wrowid  varchar(30);

BEGIN

   FOREACH c1 FOR SELECT sector, nombre
                  INTO wsector, wdescripcion
                  FROM si_catalog
                  WHERE sector <> '00'
      ...
      BEGIN
         ...
         FROM si_sector
         WHERE sector = wsector;
      END;
      
      UPDATE si_catalog SET nombre = vdescripcion
      WHERE current of C1;
