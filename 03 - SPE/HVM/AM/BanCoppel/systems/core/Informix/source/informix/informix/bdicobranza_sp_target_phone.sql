CREATE PROCEDURE "informix".sp_target_phone()
	...
	--se define la variable de numero de campania y el numero de producto para insertarlos a la tabla.
  ...
  DEFINE cccMensaje       CHAR(150);
	...
	--se inicializa la variable de numero de campania y el numero de producto para insertarlos a la tabla.
	LET sNumCampania    = 0;
	LET cNumProducto  	= '';
 	LET cccCod_ret = '';
 	LET cccMensaje = '';
 	
	--SET DEBUG FILE TO '/home/sysifx/SPs PAYAN/"informix".sp_target_phone.out';
	--TRACE ON;
	...
	BEGIN
		...
		DELETE bdicobranza:"informix".cb_ctes_mensajes WHERE num_campania = 5;
		
		---OBTENGO EL NUMERO DE PRODUCTO DE LA CAMPAÑA 11
		...
		FOREACH
			...
			AND b.monto_financiado BETWEEN 100 AND 65000
			AND a.num_credito NOT IN (SELECT aa.num_credito
						FROM bdicred:sd_maecredanexo aa
						WHERE aa.empresa  = vempresa
						AND aa.fecha_ult_pago > vtFechaCorte AND fecha_ult_pago <= vtFechaLimite)*/


			--SELECT LIMIT 40000 a.numcte,                                                         --- cliente
			SELECT LIMIT 200 a.numcte,                                                         --- PRUEBA MACF
			...
			--   TRIM(b.telefono2) Telefono_Celular                                     --- t_celular
			   tel1.telefono
			INTO vnumcte, vnum_credito, vciudad, vestado, vnombre,
			   vsexo, vestado_civil, vcelular
			FROM bdinteg:"informix".si_cliente a
			   ,bdinteg:"informix".si_ctepf aa
			   ,bdinteg:"informix".si_direcciones_actual b   --cambiar si_direcciones por si_direcciones_actual
			   ,bdicred:"informix".sd_maecred d
			   ,bdicred:"informix".sd_maesdos f
			   ,bdisolic:"informix".ss_refpersonales i
			   ,bdinteg:"informix".si_catciudades j
			   ,bdinteg:si_telefonos tel1
			WHERE a.empresa = d.empresa
				...
				AND d.empresa = vempresa
				AND i.numcte_ref = 'R1'
				and tel1.numcte = a.numcte and tel1.tipo_tel = 2 
				and tel1.secuencia = (select max(secuencia) from bdinteg:si_telefonos where numcte = a.numcte and tipo_tel = 2)
				AND b.numerociudad = j.numerociudad
				--AND b.telefono2 <> ''
				AND tel1.telefono <> ''
				AND f.monto_financiado BETWEEN 100 AND 65000
				AND d.status_cred IN ('AA','E1')
				AND (f.monto_vencido + f.mto_venc_trasp) = 0
				AND d.numcte NOT IN (SELECT numcliente FROM bdicobranza:"informix".cb_compac) -- que no tengan compromiso

			LET vcelular = SUBSTR(vcelular,LENGTH(vcelular)-9,10);
			...
			IF (vinstruccion = 1) THEN				
				...
				INSERT INTO bdicobranza:"informix".cb_ctes_mensajes(empresa,num_campania,producto,fecha_ejecucion,cliente,credito,ciudad,estado,nombre,t_celular,pago_venc,situacion,causa) 
				VALUES(vempresa,sNumCampania,cNumProducto,TODAY,vnumcte,vnum_credito,vciudad,vestado,vnombre,vcelular,0,NVL(vsituacion, ''),NVL(vcausa, 0));
		...
		END FOREACH;
			...
			CALL bdicobranza:sp_actualiza_saldos_admin()
			RETURNING cccCod_ret, cccMensaje;

    IF cccCod_ret <> '000000' THEN LET cCod_ret = cccCod_ret; LET cMensaje = 'Error al ejecutar: sp_actualiza_saldos_admin.'; END IF;  
		RETURN cCod_ret, cMensaje;
...
'Descripcion: Se obtiene el Numero de Campaña y en Numero de Producto y se inserta en la tabla cb_ctes_mensajes',
'Fecha: 2011/05/11',
