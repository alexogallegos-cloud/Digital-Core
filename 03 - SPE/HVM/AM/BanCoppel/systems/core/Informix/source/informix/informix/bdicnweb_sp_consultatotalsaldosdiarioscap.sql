CREATE PROCEDURE "informix".sp_consultatotalsaldosdiarioscap(pUsuario CHAR(8), pIdFuncion CHAR(10), pCuenta CHAR(20), pFechaInicial DATE, pFechaFinal DATE)
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultatotalsaldosdiarioscap.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pCuenta = '' OR pFechaInicial IS NULL OR pFechaFinal IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet, iRegistros;
		END IF;
		...
		LET iRegistros = (pFechaFinal - pFechaInicial) + 1;
		RETURN cCodRet, iRegistros;
...
"FECHA: 12/12/2013",
"DESCRIPCION: Realiza la consulta del total de saldos historicos diarios de una cuenta",
"BD: bdicnweb";

CREATE PROCEDURE "informix".sp_dinya_obtienetotalesordenpagosac(pUsuario CHAR(8), pIdFuncion CHAR(10), pConvenio CHAR(5), pFechaIni DATE, pFechaFin DATE,
	 pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codigoRetorno,
			CHAR(10) AS fechaEnvio,
			CHAR(20) AS estatus,
			INTEGER AS totalEnvios,
			MONEY(16,2) AS totalImporteEnviado;
	...
	BEGIN
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cFechaEnvio, cEstatus, iTotalEnvios, mTotalImporteEnviado;
		END EXCEPTION;	
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_dinya_obtienetotalesordenpagosac.out';
		--TRACE ON;
		...
		FOREACH EXECUTE PROCEDURE bdisac:sp_dinya_obtienetotales(pConvenio, pFechaIni, pFechaFin) INTO cCodRetSp, cFechaEnvio, cEstatus, iTotalEnvios, mTotalImporteEnviado
			...
			ELIF cCodRetSp = '00000' THEN
				IF iRegistros >= pRegistros THEN
					IF iRecuperacion < pRecuperacion THEN
						...
						RETURN cCodRet, cFechaEnvio, cEstatus, iTotalEnvios, mTotalImporteEnviado WITH RESUME;
	...
	END;
END PROCEDURE
DOCUMENT 'AUTOR: Esaprza Brenis Fernando Martin';
