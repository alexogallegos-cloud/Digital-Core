CREATE PROCEDURE "informix".respaldarees(pEmpresa    CHAR(3),
                          tp_Respaldo CHAR(1))
RETURNing char(5);
...
-- **************************************************************************
   ON EXCEPTION SET sql_err, isam_err, error_info
      SET DEBUG FILE TO "/pisa/pisabanco/pisa_resp/reestructura/respalda.err";
      TRACE ON;
...
-- Procesa Informacion
-- ***************************************************************************
  --SET DEBUG FILE TO "/pisa/pisabanco/pisa_resp/reestructura/respaldarees.out";
  --TRACE ON;
   ...
   SELECT COUNT(*)
    ...
    IF v_numreg = 0 THEN
      ...
      INSERT INTO sd_contproc VALUES
         (pEmpresa, 'RespaldaRe', vfecha, 'C', USER, CURRENT, CURRENT,
	...
	VALUES
	 (pEmpresa, 'RespaldaRe', vfecha, '06', 'C',
   ...
   ON EXCEPTION IN (-668) SET sql_err
      SET DEBUG FILE TO TRIM(v_tabla) || " respaldarees.err";
      TRACE ON;
       LET CodRet = "000";
       LET wdir = wdir;
   ...
   FOREACH
      ...
      SYSTEM v_sql;
      LET v_sql = "dbaccess bdicred querycrd.sql ";
      SYSTEM v_sql;
