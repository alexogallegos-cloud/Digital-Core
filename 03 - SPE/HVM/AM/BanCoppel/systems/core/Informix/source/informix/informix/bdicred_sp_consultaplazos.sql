CREATE PROCEDURE "informix".sp_consultaplazos(pNumPromocion SMALLINT)
	RETURNING CHAR(6) AS CodRetorno, INTEGER AS Plazo, INTEGER AS RegAct, INTEGER AS Tasa;	--DSB20140610
...
'-- Modificación..: Se modifica para que retorne la tasa de interes y se muestre en la dll de Credisoluciones',
'-- Sustento......: Analisis incidencias credisoluciones.doc',
'-- Solicita......: Faviola Martinez',
'-- BD............: Bdicred';

CREATE PROCEDURE "informix".sp_reg_dep_cobranza(pNum_Empleado 	 CHAR (8),
												pFecha 			 DATE,
											    pSucursal 		 CHAR(4), 
											    pProducto 		 CHAR(40), 
											    pNum_Cte 		 CHAR(20),
											    ...
											    pMonto 			 DECIMAL(18,2), 												
											    pSaldo_Anterior  DECIMAL(18,2), 
												pSaldo_Actual 	 DECIMAL(18,2),
											    pStatus_Anterior CHAR(60),											     
											    pStatus_Actual 	 CHAR(60), 
											    pFolio_Suc 		 CHAR(16), 
											    pReversion 		 CHAR(1))
...
--SET DEBUG FILE TO '/home/tmp/leonardo/sp_reg_dep_cobranza.out';
--TRACE ON;
...
BEGIN
	...
	IF NVL(pNum_Empleado, '') = '' OR NVL(pFecha, '') = '' OR NVL(pSucursal, '') = '' OR NVL(pProducto, '') = '' OR
	   NVL(pNum_Cte, '') = '' OR NVL(pNum_Credito, '') = '' OR NVL(pMonto, '') = '' OR NVL(pSaldo_Anterior, '') = '' OR
	   NVL(pSaldo_Actual, '') = '' OR NVL(pStatus_Anterior, '') = '' OR NVL(pStatus_Actual, '') = '' OR
	   NVL(pFolio_Suc, '') = '' OR NVL(pReversion, '') = '' THEN
		LET cCodRet = '00001';
	ELSE
		INSERT INTO bdicred:"informix".sd_depositos_cobranza (num_empleado,fecha_deposito,sucursal,producto,num_cte,num_credito,monto_deposito,saldo_anterior,saldo_actual,status_cred_anterior,status_cred_actual,folio_suc,reversado) 
		VALUES (pNum_Empleado,pFecha,pSucursal,pProducto,pNum_Cte,pNum_Credito,pMonto,pSaldo_Anterior,pSaldo_Actual,pStatus_Anterior,pStatus_Actual,pFolio_Suc,pReversion);
	END IF;
	RETURN cCodRet;
END;
END PROCEDURE
...
'AUTOR :95594213 ',
'FECHA :27-01-2014 ',
'MODIFICACIÓN:Se crea sp para guardar en la tabla sd_depositos_cobranza',
'SUSTENTO:RQM_09-338_Deposito_personal_cobranzav3.1.pdf',
'SOLICITA: Rodolfo Gomez',
'BD: bdicred';

CREATE PROCEDURE "informix".sp_consultas_cac_centralpbaaga(pEmpresa          CHAR(3),
                                                     pSucursal         CHAR(4),
                                                     pFechaInicial     DATE,
                                                     pFechaFinal       DATE,
                                                     pNumSol           CHAR(20),
