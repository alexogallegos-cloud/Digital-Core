CREATE PROCEDURE "informix".sp_ingresos (
...
--Variables para caracteres especiales
DEFINE iCont           INTEGER;
DEFINE iCodigo         INTEGER;
DEFINE cCaracter       CHAR(1);
DEFINE cnombre_empresa_fin        CHAR(60);
...
LET cnombre_empresa_fin = '';
LET iCont           =0;
LET iCodigo         =0;
LET cbandera        =0;


--Variables para validación de caracteres especiales
LET cnombre_empresa_fin = '';
LET iCont           	= 0;
LET iCodigo         	= 0;
LET cbandera        	= 0;

--SET debug FILE TO '/pisa/pisabanco/sp_ingresos.out';
--TRACE ON;
...
BEGIN
    ...
    END IF
		...
		-- Valida si el nombre de la empresa contiene caracteres especiales 18/02/2025
		FOR iCont = 1 TO LENGTH(pNombreEmpresa)
		LOOP
			LET iCodigo = ASCII(SUBSTR(pNombreEmpresa, iCont, 1));

			IF NOT((iCodigo >= 97 AND iCodigo <= 122) OR   -- a-z
				   (iCodigo >= 65 AND iCodigo <= 90) OR    -- A-Z
				   (iCodigo >= 48 AND iCodigo <= 57) OR    -- 0-9
				   (iCodigo = 32) OR                       -- Espacio
				   (iCodigo = 241) OR                      -- ñ
				   (iCodigo = 209)) THEN                   -- Ñ
				LET cbandera = 1;  
				EXIT FOR; -- Salir por caracter especial
			END IF;
		END LOOP;
		...
		IF cbandera = 1 THEN
			...
			LOOP
				LET iCodigo = ASCII(SUBSTR(pNombreEmpresa, iCont, 1));
				LET cCaracter = SUBSTR(pNombreEmpresa, iCont, 1);

				-- Valida si es letra, número o espacio
				IF (iCodigo >= 97 AND iCodigo <= 122) OR (iCodigo >= 65 AND iCodigo <= 90) OR 
				   (iCodigo = 241) OR (iCodigo >= 48 AND iCodigo <= 57) OR (iCodigo = 32) THEN
					LET cnombre_empresa_fin = TRIM(cnombre_empresa_fin) || TRIM(cCaracter);
				ELSE
					LET cnombre_empresa_fin = TRIM(cnombre_empresa_fin) || ' ';
				END IF;
			END LOOP;
		END IF;
		...
		-- Valida si el nombre de la empresa contiene caracteres especiales 18/05/2025

    IF pTipoOperacion = 'A' THEN --Alta
        ...
        INSERT INTO si_ingresos ( empresa, numcte, sec_ingreso, tipo_ingreso, nombre_empresa, puesto, puesto_esp, antiguedad,
        ...
        VALUES ( pEmpresa, pNumeroCliente, iSecuencia, pTipoIngreso, cnombre_empresa_fin, pPuesto, pPuestoEsp, pAntiguedad,
    ...
    ELIF pTipoOperacion = 'C' THEN --Actualizacion
        ...
        IF pSecuencia = 0 THEN
            ...
            IF NOT EXISTS ( SELECT numcte
                            FROM si_ingresos
                            WHERE empresa = pEmpresa AND numcte = pNumeroCliente AND sec_ingreso = iSecuencia ) THEN
            ...
            ELSE
                LET vcodret = '400';  --Secuencia invalida
            END IF

        ELSE

            UPDATE si_ingresos
            SET empresa = pEmpresa, numcte = pNumeroCliente, sec_ingreso = pSecuencia, tipo_ingreso = pTipoIngreso, nombre_empresa = cnombre_empresa_fin,
                puesto = pPuesto, puesto_esp = pPuestoEsp, antiguedad = pAntiguedad, nombre_depto = pNombreDepto, jefe_inmediato = pJefeInmediato,
                ingreso_mensual = pIngresosMensuales, user_insert = pUsuarioInserta, fecha_insert = pFechaInserta, clavepuesto = pClavePuesto,
                claveopcionpuesto = pClaveOpcionPuesto, clavesubopcionpuesto = pClaveSubOpcionPuesto, sis_cotiza = pSisCotiza,
                num_emp_lab = pNumEmpLab, periosidad = pPeriosidad, tipo_ingreso_ext = pTipoIngresoExt
            WHERE numcte = pNumeroCliente AND sec_ingreso = pSecuencia AND tipo_ingreso = pTipoIngreso;

        END IF

    ELSE
        LET vcodret = '200'; --Tipo de operacion incorrecta
    END IF


-- actualiza bitacora PLD

    IF (pClaveOpcionPuesto > 0 or pClaveSubOpcionPuesto > 0) THEN
        ...
        IF iSecuencia is not null and iSecuencia > 0 THEN
            UPDATE bdinteg:"informix".si_bitacoraapertura 
               SET id_act     = pClaveOpcionPuesto,
                   id_subact = pClaveSubOpcionPuesto
             where numcte = pNumeroCliente
               and id_secuencia = iSecuencia
               and id_pregunta = 6;
        END IF;
    END IF;
...
"Descripcion: Inserta datos en la tabla si_ingresos",
"Fecha      : 08/12/2008",
"Modifico   : Julio Cesar Polanco",
"Descripcion: Se modifica par añadir los campos nuevos de la tabla si_ingresos",
"             por Alta Unica Paso 4",
"Fecha      : 03/12/2009",
"Solicito   : Alfonso Velazquez", 
"Modifico   : Rodolfo Tortolero Varela",
"Descripcion: Se modifica para que tambien guarde los puestos en los campos puestos y puestos_esp  de la tabla si_ingresos",
"                        por Alta Unica Paso 4",
"Fecha      : 20/01/2010",
"Solicito   : Daniel Zambada";
