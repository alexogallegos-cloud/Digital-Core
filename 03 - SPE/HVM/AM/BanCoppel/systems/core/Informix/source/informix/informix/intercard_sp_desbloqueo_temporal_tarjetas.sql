CREATE PROCEDURE "informix".sp_desbloqueo_temporal_tarjetas (pdfecha date, psstatus char(3))
...
let vmaxnumregistros=5000;
		...
		FOREACH cursor1 WITH HOLD
          ...
          if ((vicontadorregistros > 0) or (vsflagentransaccion = 'V')) then
				commit work;
				update statistics medium for table intercard:"informix".tarjeta;      
				let vsflagentransaccion = 'F';
		   end if;
