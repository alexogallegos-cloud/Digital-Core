CREATE PROCEDURE "informix".sp_altactaspei(pNoCuentaClabe CHAR(18),pUsuario CHAR(8))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_altactaspei.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pNoCuentaClabe=''  THEN
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF;
		
		--EXISTE CUENTA CON STATUS ALTA
		IF EXISTS(SELECT vchrcuentaord FROM bdispei:tblclabebloqueo WHERE vchrcuentaord=pNoCuentaClabe AND chrstatus='A') THEN
			LET cCodRet = '00102';
			RETURN cCodRet;
		END IF;
		
		--EXISTE CUENTA CON STATUS BAJA
		IF EXISTS(SELECT vchrcuentaord FROM bdispei:tblclabebloqueo WHERE vchrcuentaord=pNoCuentaClabe) THEN
			
			UPDATE bdispei:tblclabebloqueo SET
			chrstatus='A',
			vchruser_mod=pUsuario,
			fecha_mod=CURRENT
			WHERE vchrcuentaord=pNoCuentaClabe;
			
			RETURN cCodRet;
			
		END IF;
		
		LET cBanco=SUBSTR(pNoCuentaClabe,0,3);
		
		IF EXISTS(SELECT banco FROM bdinteg:si_bancos WHERE banco=cBanco) THEN
			
			INSERT INTO bdispei:tblclabebloqueo(vchrcuentaord,chrstatus,vchruser_ins,dtfecha_ins, vchruser_mod, fecha_mod)VALUES(pNoCuentaClabe,'A',pUsuario,CURRENT,pUsuario,CURRENT);
		
		ELSE
			LET cCodRet = '00101';			RETURN cCodRet;
...
'DESCRIPCION: SP que actualiza o insert una cuenta clabe a la lista negra de CUENTAS CLABE',
'BD: bdispei';

CREATE PROCEDURE "informix".sp_bajactaspei(pNoCuentaClabe CHAR(18),pUsuario CHAR(8))
		RETURNING CHAR(5) AS codret;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_bajactaspei.out';
		--TRACE ON;
		...
		--EXISTE CUENTA CON STATUS CANCELADO
		IF EXISTS(SELECT vchrcuentaord FROM bdispei:tblclabebloqueo WHERE vchrcuentaord=pNoCuentaClabe AND chrstatus='C') THEN
			LET cCodRet = '00102';
			RETURN cCodRet;
		END IF;
...
CREATE PROCEDURE "informix".sp_consctectehist( pSucursal     CHAR(4),
    ...
    IF bTipo = 3 THEN
        FOREACH
            SELECT DISTINCT vchrNombreOrd, CASE WHEN 
                   ...
                   nvl(dtmhoracancela::char(20),''), nvl(vchrmotivodev,''), nvl(intcvecausadev,0), pago.chrusuarioprom, pago.dtfechacaptura 
              ...
              FROM tbldetranpago AS det
            -- INNER JOIN bdicheq:sc_maechq AS mto ON ( det.empresa = mto.empresa AND det.cuenta = mto.cuenta )
             INNER JOIN tblpago AS pago ON (pago.chrsentidopago = 'E' AND pago.vchrclaverastreo = det.clave_rastreo )
             INNER JOIN tblbanco AS bco ON ( pago.cvecesifbcodest = bco.cvecesif )
             WHERE det.sucursal = pSucursal
             --  AND mto.empresa = '001'
               AND pago.dtfechacaptura BETWEEN pFechaInicial AND pFechaFinal
