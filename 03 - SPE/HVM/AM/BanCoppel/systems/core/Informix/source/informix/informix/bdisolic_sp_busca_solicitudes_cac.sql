CREATE PROCEDURE "informix".sp_busca_solicitudes_cac(pEmpresa		CHAR(3),
...
--PASAR DE STATUS LC  STATUS AT
	...
	IF pBuscaSolic = 0 THEN
		FOREACH    -- SE TOMA SOLICITUD POR SOLICITUD
			SELECT a.num_solicitud, a.fecha_insert
			INTO cNumSolic, dtFechSolic
			FROM "informix".ss_solicitudes_cac a
			WHERE a.empresa = pEmpresa
			AND (
					  ( a.status = "LC"  AND (a.os = "N"   AND a.ejecutivo_autoriza IS NULL  OR a.ejecutivo_autoriza =  "")
						AND a.fecha_insert + 60 <= dfecha AND a.hora_insert < dtHoraAnterior)
					  OR
					  ( a.status = "LC"  AND (a.os = "S"   AND a.ejecutivo_autoriza IS NULL  OR a.ejecutivo_autoriza =  "")
						AND (a.fecha_insert + 61 UNITS DAY) < dfecha AND a.hora_insert < dtHoraActual)
				)
			AND a.num_solicitud NOT IN(SELECT num_solicitud FROM "informix".ss_sol_revision_cac  where num_solicitud = a.num_solicitud )
...
IF NVL(pDesplazar,0) IN (0,2,3) THEN -- CONSULTA INICIAL (0), PAGINA SIGUIENTE(2), CONSULTA GENERAL SIN PAGINACIÓN (3).
	...
	IF pNumSolicitud = "" OR pNumSolicitud IS NULL THEN
		...
		FOREACH
			SELECT a.status, a.num_solicitud, a.sucursal, a.numcte, a.fecha_insert, a.os, a.num_producto, a.ejecutivo_atiende
			INTO cStatus, cNumSolic, cSucursal, cNumCte, dtFechSolic, cTieneOS, cNum_Pdcto, cEjec_Atde
			FROM "informix".ss_solicitudes_cac a
			INNER JOIN bdisolic:ss_solicitudes b ON b.num_solicitud = a.num_solicitud AND b.status_solicitud = a.status			
			WHERE a.empresa = pEmpresa
			AND 	(
				   (a.status = "LC" )
				   OR
				   (a.status IN ("AT","RT") AND ((a.ejecutivo_autoriza IS NULL AND a.os = "S"
										AND (a.fecha_insert + 1 UNITS DAY) <= dfecha AND a.hora_insert < dtHoraAnterior)
										OR
										(a.ejecutivo_autoriza IS NULL AND a.os = "N"
										AND (a.fecha_insert + 1 UNITS DAY) <= dfecha AND a.hora_insert < dtHoraAnterior2)))
				)
			--AND num_solicitud NOT IN (SELECT num_solicitud  FROM "informix".ss_paginacion_solicitudes_cac WHERE ejecutivo =  pEjecutivo)
			ORDER BY a.OS,a.fecha_insert,a.hora_insert ASC
