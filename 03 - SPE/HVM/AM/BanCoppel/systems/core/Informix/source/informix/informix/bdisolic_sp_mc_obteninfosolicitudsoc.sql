CREATE PROCEDURE "informix".sp_mc_obteninfosolicitudsoc(pEmpresa CHAR(3),pNumSol CHAR(20))
RETURNING 
	...
	DECIMAL(18,2) AS AbonosVencidos,
...
ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo   
     ...
     RETURN cCodRet, cErrorInfo,cNumcte,cNombreCte,cRfc,cSucursal,
		cNumCteCop,dLineaCoppel,  cEficienciaPago,cMesesHist,cPuntualidad,dAbonosVen, cSituacion_credito,cCausa,cDescSitEsp,
		cNumSolicitud,dtFechaSol,dtFechaCambioStatus,
		dBCScore, dScoreProp,dResultadoTotal,NVL(cStatusSol,''),cCausaStatus,cComportamientoSic,
		dtFechaSolOs,cStatusOS,cSitEspOS,iCausaSitEspOS,cDescSitEspOS,cDescMotivoOS,  dtFechaResp,iSecuenciaOs,
		dtFechaOstel,cRespuestaOstel,cAtendio,
		dIngresoMensual,dIngresoLC,dCompromisosBanco,dCompromisosSIC,dCompromisosCoppel,dCMA,dTAB,	dLineaTeorica,NVL(dMontoSol,0),
		...
		iPar_prestamos,cEnvioCop,cNum_solicitud_ref,cStatusSol2, cCausaStatus2 ,dtFechaCambioStatus2,cPermCambio,NVL(dMontoAut,0),dLimitecreditopesos,cComportamientoSicCop,cCausaSol,cCausaSol2, 
		cCancelTemporalidad, cCancelTemporalidad2, cSexo; 		
END EXCEPTION;

    --SET DEBUG FILE TO '/RESPALDOSNEW/ulises/SOC_Precalif/Entrega_TASF/bdisolic/sp_mc_obteninfosolicitud.out';
	--TRACE ON;
...
SET LOCK MODE TO WAIT 3;
	...
	SELECT a.num_solicitud, a.numcte, a.tipo_solicitud, a.status_solicitud ,  a.monto_solicitado , a.monto_autorizado ,
	...
	IF NVL(cNumSolicitud,"") = "" THEN
		LET cCodret = '000002'; --
		LET cMensajeRet         = "No se encontro informaciÃ³n, verifique...";
		RETURN cCodRet, cMensajeRet,cNumcte,cNombreCte,cRfc,cSucursal,
		cNumCteCop,dLineaCoppel,  cEficienciaPago,cMesesHist,cPuntualidad,dAbonosVen, cSituacion_credito,cCausa,cDescSitEsp,
		cNumSolicitud,dtFechaSol,dtFechaCambioStatus,
		dBCScore, dScoreProp,dResultadoTotal,NVL(cStatusSol,''),cCausaStatus,cComportamientoSic,
	    ...
	    dCMA_cop,dTAB_cop,dCRA_cop,dMontoSol_cop,dLineaTeorica_cop,iPuntos_parcn,iPar_altoriesgo,iPar_celulares,iPar_prestamos,cEnvioCop,cNum_solicitud_ref,cStatusSol2, cCausaStatus2 ,dtFechaCambioStatus2,cPermCambio,NVL(dMontoAut,0),dLimitecreditopesos,cComportamientoSicCop,cCausaSol,cCausaSol2,
		cCancelTemporalidad, cCancelTemporalidad2, cSexo; 		
	END IF

	
	SELECT rfc,nombre1||' '||nombre2||' '||apell_paterno||' '||apell_materno,numcte_ref
	...
	--INI Datos secciÃ³n Orden de SupervisiÃ³n Calle
	SELECT a.fecha_solicitud, nvl(a.status, '') ,a.motivo_os,  c.situacionespecial, c.causasituacionespecial, a.fecha_respuesta, a.secuenciaos 
	INTO dtFechaSolOs,cStatusOS,cMotivoOS,cSitEspOS,iCausaSitEspOS, dtFechaResp, iSecuenciaOs
	...
	END IF;
	--FIN Datos secciÃ³n Orden de SupervisiÃ³n Calle		
	--INI Datos secciÃ³n Orden de SupervisiÃ³n TelefÃ³nica		
	...
	--FIN Datos secciÃ³n Orden de SupervisiÃ³n TelefÃ³nica	            
	
	
	--INI Datos secciÃ³n determinaciÃ³n lÃ­nea de crÃ©dito
	...
	(vencidoropa+vencidomuebles+vencidoprestamos+vencidototalaire+vencidototalafiliados+vencidototalreestructura),situacion_credito,causa,motivo_cc,
	(abonomensualprestamos + abonomensualmuebles + abonomensualropa),(abonomensualropa + abonomensualmuebles + abonomensualprestamos+ abonomensualaire + abonomensualafiliados + abonomensualreestructura),
	--datos bancoppel
	...
	INTO dLineaCoppel,cEficienciaPago,cMesesHist,cPuntualidad,
		dcVencidocoppel, cSituacion_credito,cCausa,cComportamientoSic,dCompromisosCoppel,dabonbase,
	...
	FOREACH WITH HOLD
		...
		INTO cCancelTemporalidad
		FROM "informix".ss_autorizacion 
		WHERE num_solicitud = pNumSol 
		AND status_solicitud= cStatusSol 
		AND causa_solicitud in('CR','CV','CVC','CPT','CEE','CCE','COA');
		--ORDER BY FECHA_HORA DESC
	...
	ELSE
		...
		END IF;
	END IF;
	--FIN Datos secciÃ³n determinaciÃ³n lÃ­nea de crÃ©dito
...
RETURN cCodRet, cMensajeRet,cNumcte,cNombreCte,cRfc,cSucursal,
		cNumCteCop,dLineaCoppel,  cEficienciaPago,cMesesHist,cPuntualidad,dAbonosVen, cSituacion_credito,cCausa,NVL(cDescSitEsp,""),
		cNumSolicitud,dtFechaSol,dtFechaCambioStatus,
		dBCScore, dScoreProp,dResultadoTotal,NVL(cStatusSol,''),cCausaStatus,cComportamientoSic,
		NVL(dtFechaSolOs,""),NVL(cStatusOS,""),NVL(cSitEspOS,""),NVL(iCausaSitEspOS,0),NVL(cDescSitEspOS,""),NVL(cDescMotivoOS,""), NVL(dtFechaResp,""),NVL(iSecuenciaOs,0) ,
		NVL(dtFechaOstel,""),NVL(cRespuestaOstel,""),NVL(cAtendio,""),
		dIngresoMensual,dIngresoLC,dCompromisosBanco,dCompromisosSIC,dCompromisosCoppel,dCMA,dTAB,	dLineaTeorica,NVL(dMontoSol,0),
		dMontoMaxSol,cNumProd,cDescNumProd,cNumProd2,cDescNumProd2,	
	    dCMA_cop,dTAB_cop,dCRA_cop,dMontoSol_cop,dLineaTeorica_cop,iPuntos_parcn,iPar_altoriesgo,iPar_celulares,iPar_prestamos,cEnvioCop,cNumSol2,cStatusSol2, cCausaStatus2,dtFechaCambioStatus2,cPermCambio,NVL(dMontoAut,0),dLimitecreditopesos,cComportamientoSicCop,cCausaSol,cCausaSol2,
		cCancelTemporalidad, cCancelTemporalidad2, cSexo;
