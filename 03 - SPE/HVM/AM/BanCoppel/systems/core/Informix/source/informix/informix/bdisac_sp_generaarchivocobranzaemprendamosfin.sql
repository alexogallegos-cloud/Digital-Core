CREATE PROCEDURE "informix".sp_generaarchivocobranzaemprendamosfin(pConvenio CHAR(5))
...
DEFINE vReferencia1					CHAR(10);
DEFINE cRutaArchEmprendamosFin			CHAR(100);
...
LET vNombreMes				= '';
	...
	--SET DEBUG FILE TO  '/informix/noe/sp_generaarchivocobranzaemprendamosfin.out';
	--TRACE ON;
	...
	BEGIN
		...
		LET cRutaArchEmprendamosFin = REPLACE(cRutaArchEmprendamosFin,'AAAA',cAnio);
		LET cRutaArchEmprendamosFin = REPLACE(cRutaArchEmprendamosFin,'MM',cMes);
		LET cRutaArchEmprendamosFin = REPLACE(cRutaArchEmprendamosFin,'DD',cDia);
		...
		--ENCABEZADO
		LET cStmt = 'echo "EMPRENDAMOSFIN ' || chr(13) || chr(13) || 'FECHA: ' || day(dFecha_Max_Procesada) || ' DE ' || trim(vNombreMes) || ' DE ' || year(dFecha_Max_Procesada) || chr(13) || '" >> ' || cRutaArchEmprendamosFin;
		SYSTEM cStmt;
		...
		FOREACH SELECT fecha_pago
				...
				, id_sucursal
				, case when forma_pago='1' then 'E' else 'C' end as f_pago
				, NVL(folio_suc,'')
				, NVL(referencia1,'')
				, NVL(importe_pago*100,0)
				, NVL(importe_comision_convenio*100,0)
				, NVL(iva_comision_convenio*100,0)
				INTO dFechaPago, vDiaPago, vMesPago, vAnioPago, vHoraPago, vSucursal, vFormaPago, vFolio, vReferencia1, vImporte_Pago, vImporte_Comision, vImporte_Iva
			...
			LET cStmt = 'echo "' || vTpoOperacion || ',' || vReferencia1 || ',' || LPAD(vImporte_Pago,9,0) || ',' || vAnioPago || vMesPago || vDiaPago || ',' || vHoraPago || ',' || vFolio || ',' || chr(13) || '" >> ' || cRutaArchEmprendamosFin;
			SYSTEM cStmt;
		...
		IF vNumero_Operaciones > 0 THEN
			--COMISION
			LET vTpoOperacion = 'C';
			LET cStmt = 'echo "' || vTpoOperacion || ',' || LPAD(iRelleno,10,0) || ',' || LPAD(vImporte_Total_Comision,9,0) || ',' || LPAD(iRelleno,8,0) || ',' || LPAD(iRelleno,6,0) || ',' || LPAD(iRelleno,16,0) || ',' || chr(13) || '" >> ' || cRutaArchEmprendamosFin;
			SYSTEM cStmt;
			
			--IVA
			LET vTpoOperacion = 'I';
			LET cStmt = 'echo "' || vTpoOperacion || ',' || LPAD(iRelleno,10,0) || ',' || LPAD(vImporte_Total_Iva,9,0) || ',' || LPAD(iRelleno,8,0) || ',' || LPAD(iRelleno,6,0) || ',' || LPAD(iRelleno,16,0) || ',' || chr(13) || '" >> ' || cRutaArchEmprendamosFin;
			SYSTEM cStmt;
			
			--TOTAL
			LET vTpoOperacion = 'T';
			LET cStmt = 'echo "' || vTpoOperacion || ',' || LPAD(vNumero_Operaciones,10,0) || ',' || LPAD(vImporte_Total - (vImporte_Total_Comision + vImporte_Total_Iva),9,0) || ',' || LPAD(iRelleno,8,0) || ',' || LPAD(iRelleno,6,0) || ',' || LPAD(iRelleno,16,0) || ',' || chr(13) || '" >> ' || cRutaArchEmprendamosFin;
			SYSTEM cStmt;
		ELSE
			LET cStmt = 'echo "" >> ' || cRutaArchEmprendamosFin;
			SYSTEM cStmt;
		END IF;
