CREATE PROCEDURE "informix".cierrechqcomp11_1( pEmpresa CHAR(3) )
RETURNING CHAR(5);
    ...
    LET vcUsuario      = USER;
    LET vcSistema      = '01';
    LET vcProceso      = 'cierrechqcomp11_1';
    LET viExiste       = 0;
    LET viExisteFin    = 0;
    LET vcExiste       = '';
	...
	LET v_c_vcontador  = 0;
    LET viTblcomp11    = 0;
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/altamasempnet/cierrechqcomp11_1.out";
    --- TRACE ON;
    
    BEGIN
    
    ON EXCEPTION SET viSqlErr, viIsamErr, vcDescErr
        SET DEBUG FILE TO "/resplogifx/conciliachq/altamasempnet/cierrechqcomp11_1.err";
        TRACE ON;
        IF viSqlErr <> 0 THEN
            ...
            LET vsql = 'echo "UPDATE bdinteg:sx_contproc '||
                       ...
                       'AND sistema   = '''||vcSistema||''';" > /tmp/horacierre11_1.sql';
            SYSTEM vsql;
            
            LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /tmp/horacierre11_1.sql';
            SYSTEM vstmt;
    ...
    IF viExiste = 0 THEN
        LET vsql = 'echo " INSERT INTO bdinteg:sx_contproc VALUES '||
                   ...
                   '(SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas), NULL, NULL);" > /tmp/horacierre11_1.sql';
        SYSTEM vsql;
        LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /tmp/horacierre11_1.sql';
        SYSTEM vstmt;
    ELSE
    ...
    FOREACH WITH HOLD
        SELECT SKIP 8000000 FIRST 8000000 
