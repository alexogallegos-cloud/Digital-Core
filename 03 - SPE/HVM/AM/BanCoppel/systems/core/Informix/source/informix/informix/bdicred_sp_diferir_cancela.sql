CREATE PROCEDURE "informix".sp_diferir_cancela(pcte CHAR(20), 
											pcel CHAR(10), 
											ptar CHAR(20), 
											pcanal SMALLINT)
    ...
    DEFINE vsqlerr, vcant, vcantr,vcantp, vsumavencidosr, vsumavencidosp INTEGER;
	...
	DEFINE fechaRetenido				DATE;
	DEFINE sdoRetenido			DECIMAL(14,2);
	DEFINE transsRet			CHAR(4);
	DEFINE cBanderaApoyo		CHAR(1);	
	DEFINE canalBajaDif			SMALLINT;
	DEFINE cteDiferir			CHAR(20);
	DEFINE rLog					SMALLINT;
	DEFINE auxApoyo				SMALLINT;
	DEFINE mesFecha				CHAR(3);
	DEFINE anoFecha				CHAR(4);
	DEFINE banderaEmail			SMALLINT;
	DEFINE banderaSMS			SMALLINT;
	DEFINE v_correo				CHAR (40);
	DEFINE auxCte				SMALLINT;
	DEFINE faltaCte				SMALLINT;
	DEFINE i					SMALLINT;
	DEFINE ExistApoyo	 		INTEGER; --CAX modificacion OTIS2023
	
	--SET DEBUG FILE TO "/ifxsif01/ciaguilar/Programa_apoyo_OTIS/sp_diferir_cancela_"||trim(pcel)||".out";
	--TRACE ON;
    ...
    BEGIN
		...
		IF LENGTH(pcte) < 9 THEN
			LET auxCte =  LENGTH(pcte);
			LET faltaCte = 9 - auxCte;
			
			IF faltaCte > 0 THEN
				FOR i = 1 to faltaCte
					LET pcte = '0'||TRIM(pcte);
				END FOR;
			END IF;
		END IF;
	
		/*IF pcanal = 19 THEN --CANAL 19 = MASIVO progrma apoyo 2020 y se comenta para registrar programa de apoyo OTIS 2023
			...
			WHERE numcte = pcte
				...
				AND secuencia in (SELECT MAX (secuencia) 
					FROM bdinteg:si_correos 
					WHERE numcte = pcte AND  status_correo = 'A'
					and valido = 1);
		
			IF LENGTH(v_correo) > 0 AND NOT v_correo is null THEN
				LET banderaEmail = 1;
			ELSE 
				SELECT telefono INTO pcel 
				FROM bdinteg:si_telefonos_actual 
				WHERE numcte=pcte  AND tipo_tel='2' AND status_tel='A';
				
					IF LENGTH(pcel) > 0 AND NOT pcel is null  THEN 
					   ...
					   SELECT COUNT(DISTINCT(numcte))  INTO vcant 
						...
						IF vcant > 1 THEN 
						   LET vcodret = "00008";
						   RETURN vcodret,'CLIENTE NO CUENTA CON CORREO Y NUMERO CELULAR ASOCIADO A MAS DE UN CLIENTE';

						--SI NO HAY NINGUN NUMERO DE CLIENTE ASOCIADO TERMINA EL PROCESO
						ELIF vcant < 1 THEN 
						   LET vcodret = "00009";
						   RETURN vcodret,'CLIENTE NO CUENTA CON CORREO Y NUMERO DE TELEFONO NO LO TIENE ASIGNADO';
		...
		EL*/IF LENGTH(pcel) > 0 AND NOT pcel is null  THEN -- Obtiene cliente por numero de celular
		...
		-- ASIGNA POR NUMERO DE CLIENTE PROPORCIONADO
		SELECT numcte INTO vCliente
		FROM bdinteg:si_cliente
		WHERE numcte=pcte;

		IF vCliente = ''  OR vCliente IS NULL THEN
			LET vcodret = "00005";
			RETURN vcodret,'CLIENTE NO EXISTE.';
		END IF;

	
		SELECT numcte,canal_baja
			INTO cteDiferir,canalBajaDif
		FROM bdicred:Sd_diferir 
		WHERE numcte = pcte;
				
		IF canalBajaDif IS NULL THEN
			LET canalBajaDif = 0;
		END IF;

		IF canalBajaDif <> 0 THEN
			LET vcodret = '00012';
			RETURN vcodret,'LA SOLICITUD DE CANCELACION YA SE REGISTRO PREVIAMENTE.';
		END IF;	
		
		IF cteDiferir IS NULL THEN
			LET cteDiferir = '';
		END IF;
		
		IF cteDiferir = '' THEN
			INSERT INTO "informix".sd_diferir VALUES(pcte, ptar,CURRENT,'',pcel,CURRENT,pcanal);
		END IF
				
		IF canalBajaDif = 0 AND cteDiferir <> '' THEN
			
			UPDATE bdicred:sd_diferir SET fecha_baja = CURRENT, canal_baja =  pcanal
			WHERE numcte = pcte;
		...
		FROM
			...
			FROM bdicred:Sd_maecred a
				join bdicred:sd_programa_apoyo b on (a.num_credito = b.num_credito)
			WHERE numcte = pcte AND b.bandera  = 'A'
			UNION ALL
			SELECT a.num_credito
			FROM bdicred:Sd_maecredcrd a
				join bdicred:sd_programa_apoyo_crd b on (a.num_credito = b.num_credito)
			WHERE numcte = pcte AND b.bandera  = 'A');
		...
		FOREACH 
			...
			SELECT a.num_credito, a.num_producto,a.sucursal,a.divisa,b.bandera
				INTO v_num_credito,v_producto,vSucursal,vDivisa,cBanderaApoyo
			FROM bdicred:Sd_maecred a
				join bdicred:sd_programa_apoyo b on (a.num_credito = b.num_credito)
			WHERE numcte = pcte AND b.bandera  = 'A'
			UNION ALL
			...
			IF v_producto in ('8100','6001','8500','6600','7000') THEN --- no se agregan 6400 9300 7800 para OTIS
			
				/*FOREACH WITH HOLD
				
					SELECT fecha,monto,transacc 
						INTO fechaRetenido, sdoRetenido,transsRet
					FROM BDICRED:sd_maeretenido 
					WHERE num_credito = v_num_credito 
						AND estatus = "R" 
						ORDER BY fecha
						
					LET mesFecha = DECODE( MONTH(fechaRetenido),
						"1","ENE","2","FEB","3","MAR",
						"4","ABR","5","MAY","6","JUN",
						"7","JUL","8","AGO","9","SEP",
						"10","OCT","11","NOV","12","DIC") ;
					LET anoFecha = YEAR (fechaRetenido);
					
					IF transsRet = '8369' THEN		--- transaccion de INTERES
					
						LET Folio = 'CAR_INT '||mesFecha||'/'||TRIM(anoFecha);
					
						CALL genmovcierre_movdia('001', v_num_credito, v_producto,12, "605", FechaHoy,  sdoRetenido, Folio, vSucursal, vDivisa, '8372',vPlaza) 
							RETURNING vcodret, v_mensaje;
						IF (vcodret <> "00000") THEN
							ROLLBACK WORK;
							LET v_mensaje = "CARGO INT PROGRAMA DE APOYO";
							CALL log_cierre ('001', v_num_credito, vcodret, FechaHoy, TRIM(v_mensaje) || "(GENMOV)") 
								RETURNING rLog;
							IF rLog > 0 THEN
								RETURN vcodret,'CARGO INT PROGRAMA DE APOYO';
							ELSE
								CONTINUE FOREACH;
							END IF
						END IF;
						
						UPDATE bdicred:sd_maeretenido SET estatus = 'S' 
						WHERE num_credito = v_num_credito
							AND fecha = fechaRetenido AND transacc = transsRet;
							
						UPDATE bdicred:Sd_maesdos SET sdo_retenido = sdo_retenido - sdoRetenido,
							sdo_capital = sdo_capital + sdoRetenido,
							sdo_cap_insoluto = sdo_cap_insoluto + sdoRetenido
						WHERE num_credito = v_num_credito;

					ELIF transsRet = '8370' THEN		--- transaccion de IVA
					
						LET Folio = 'CAR_IVA '||mesFecha||'/'||TRIM(anoFecha);
					
						CALL genmovcierre_movdia('001', v_num_credito, v_producto,13, "605", FechaHoy,  sdoRetenido, Folio, vSucursal, vDivisa, '8373',vPlaza) 
							RETURNING vcodret, v_mensaje;
						IF (vcodret <> "00000") THEN
							ROLLBACK WORK;
							LET v_mensaje = "CARGO IVA PROGRAMA DE APOYO";
							CALL log_cierre ('001', v_num_credito, vcodret, FechaHoy, TRIM(v_mensaje) || "(GENMOV)") 
								RETURNING rLog;
							IF rLog > 0 THEN
								RETURN vcodret,'CARGO IVA PROGRAMA DE APOYO';
							ELSE
								CONTINUE FOREACH;
							END IF
						END IF;
				...
				END FOREACH;*/
				SELECT COUNT(*) into ExistApoyo from  "informix".sd_programa_apoyo WHERE bandera = 'B' and num_credito = v_num_credito;
					
				IF (ExistApoyo = 0) THEN --CAX
					UPDATE bdicred:sd_programa_apoyo 
						SET bandera = 'B',fecha_solic_baja = CURRENT 
					WHERE num_credito = v_num_credito;	
				END IF;
				
			ELIF v_producto in ('6300','7600','7700','6800','9300','6011') THEN -- PRODUCTOS PARTICIPANTES PP Y REESCTRUCTURA TDC HURACAN OTIS GUERRERO 2023
			
				SELECT COUNT(*) into ExistApoyo from  "informix".sd_programa_apoyo_crd WHERE bandera = 'B' and num_credito = v_num_credito;
				
				IF (ExistApoyo = 0) THEN --CAX
					UPDATE bdicred:sd_programa_apoyo_crd 
					SET --bandera = 'A',
					fecha_solic_baja = CURRENT 
					WHERE num_credito = v_num_credito;
					
					UPDATE bdicred:sd_maecredcrd 
						SET campo_trab3 = ''
					WHERE num_credito = v_num_credito;	 -- pendiente validar				
				END IF;
			END IF;
		...
		IF banderaEmail = 1 THEN
			
			EXECUTE PROCEDURE bdimnsj:sp_registra_evento (1,'PROD_EMAIL','CAN_APOYO_EM','000000000','','',2,'','','','','','','','','','',v_correo,'',0,0,0,0,0,current,current)
			INTO cCodRetSp;
			
			IF cCodRetSp <> '00000' THEN
			   LET vcodret = "00010";
			   RETURN vcodret,'NO SE PUDO REALIZAR EL ENVIO SMS';					
			END IF;
			
		ELIF banderaSMS = 1 THEN		
		   
			EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento (2,'PROD_SMS','CAN_APOYO','000000000','','',2,'','','','','','','','','','','',pcel,0,0,0,0,0,current,current)
				INTO cCodRetSp;
				
			IF cCodRetSp <> '00000' THEN
			   LET vcodret = "00011";
			   RETURN vcodret,'NO SE PUDO REALIZAR EL ENVIO E-MAIL';					
			END IF;
				
		END IF;
			
		--- La baja del Plan de Apoyo para los afectados por el Huracan Otis ha sido exitosa, realiza tus pagos de forma habitual
		--- La baja del Plan de Apoyo por el Huracan Otis ha sido exitosa, realiza tus pagos de forma habitual
		--RETURN vcodret, 'CANCELACION RECIBIDA DE MANERA EXITOSA';
		RETURN vcodret, 'LA BAJA DEL PLAN DE APOYO POR EL HURACAN OTIS HA SIDO EXITOSA, REALIZA TUS PAGOS DE FORMA HABITUAL';

	END;
END PROCEDURE
DOCUMENT
'PROCESO PARA CANCELAR PROGRAMA DE APOYO POR CANAL',
'AUTOR : ISRAEL TRAVIESO DIAZ',
'FECHA : JUN/2020',
'BD    : BDICRED',
'MODIFICACION: Adecuacion para apoyo del huracan OTIS',
'AUTOR : CINTHIA AGUILAR',
'FECHA : 20 DE NOV 23'
;

CREATE PROCEDURE "informix".sp_geninsumos_calif_rees_pp()
...
DEFINE v_porcentaje_uso               	DECIMAL(18,6);	DEFINE v_ree_pp_anterior              	SMALLINT;
...
DEFINE v_fecha_corte_proc				DATE;			DEFINE v_fecha_corte_pp				DATE;
DEFINE v_dia_corte_pp					INTEGER;		DEFINE dt_ini_per_rep					DATE;
...
DEFINE v_num_credito_pp CHAR(20);
...
DEFINE v_pago_ini			DECIMAL(18,2);
DEFINE v_cuenta				CHAR(20);
DEFINE v_lim_cred_origen	DECIMAL(18,2);
DEFINE v_producto_orig		CHAR(4);
DEFINE v_monto_ult_disp		DECIMAL(18,2);
DEFINE v_mes_venc_orig		SMALLINT;
DEFINE v_tasa_interes_ree	DECIMAL(18,2);
...
LET v_porcentaje_pago             	= 0;	LET v_porcentaje_uso               	= 0;	LET v_ree_pp_anterior              = 0;
...
LET v_pago_ini=0;
LET v_cuenta='';
LET v_lim_cred_origen=0;
LET v_producto_orig	='';
LET v_monto_ult_disp=0;
LET v_mes_venc_orig	=0;
LET v_tasa_interes_ree=0;
...
BEGIN
	...
	--SET DEBUG FILE TO "/resplogifx/archivoscartera/sp_geninsumos_calif_reest_v6.out";
    --TRACE ON;
...
and num_credito in (SELECT num_credito FROM sd_maecredcontcrd    
					 WHERE fecha= pPeriodo
					   ...
					   AND num_credito > '' )
--AND num_credito in ('860000000007')	   --Filtro de cuentas para Pruebas
...
FROM bdiburo:br_variables_cc_cnr;
    ...
    FOREACH WITH HOLD
        ...
        a.tasa_interes, b.mto_fin_ven_trasp,c.fecha_ultimo_pago_h
        ...
        v_linea_autorizada,v_plazo_total,v_plazo_remanente,v_sdocap_insoluto_cierre, v_tasa_contractual,v_num_pagos_vencidos,v_fecha_ult_pago
        ...
        ON (a.empresa = e.empresa and a.num_credito = e.num_credito and  dia_corte <> 0)
        ...
        WHERE a.fecha= pPeriodo
		  AND a.fecha=b.fecha
          AND a.num_producto = '8600' 
		  AND a.num_credito > '' 
		  AND a.num_credito NOT IN (select num_credito from sd_insumos_calif_reest_pp where fecha_cierre = pPeriodo)
		  AND nvl(credito_externo,'')<>''
		  --Filtro de cuentas para Pruebas
      -- and a.num_credito in ('860000000007')
		...
		SELECT monto_otorgado INTO v_lim_cred_origen
		FROM sd_maesdoscontcrd WHERE num_credito=v_num_credito_ext and fecha in(select max(fecha) from sd_maesdoscontcrd where num_credito=v_num_credito_ext);
		
		select num_producto into v_producto_orig 
		from sd_maecredcrd where num_credito=v_num_credito_ext;
		
		IF v_producto_orig='6800' THEN
			SELECT sdo_nva_disp INTO v_monto_ult_disp 
			FROM sd_maesdoscrd_flex where num_credito=v_num_credito_ext and fecha_insert in
			(select max(fecha_insert) from sd_maesdoscrd_flex where num_credito=v_num_credito_ext);
		ELSE
			SELECT monto_otorgado INTO v_monto_ult_disp
			FROM sd_maesdoscrd where num_credito=v_num_credito_ext;
		END IF;
		
		SELECT mto_fin_ven_trasp INTO v_mes_venc_orig
		FROM sd_maesdoscrd where num_credito=v_num_credito_ext;
		...
		IF  v_num_producto = '6400' THEN
			LET v_cred_nomina =	1;
			LET v_ree_pp_anterior = 0;
		ELIF  v_num_producto = '8600' THEN
			LET v_cred_nomina =	0;			
			LET v_ree_pp_anterior = 1;
		ELSE 
			LET v_cred_nomina =	0;
			LET v_ree_pp_anterior = 0;
		END IF;	
            ...
            --FOREACH WITH HOLD
				...
				--IF v_status_fin_mes='BA' OR v_status_fin_mes='BT' OR v_status_fin_mes='VP' THEN
					...
					ELSE*/
						...
						--IF ((pPeriodo - v_fecha_apertura) /30.4) >=4 THEN
						IF v_antiguedad  = 0 THEN	
							SELECT nvl(atr,0) 
							INTO v_atr
							FROM sd_maesdoscontcrd 
							...
							IF v_atr=0 OR v_atr IS NULL THEN
								SELECT nvl(atr,0) 
								INTO v_atr
								FROM sd_maesdoscontcrd 
								WHERE num_credito =v_num_credito_ext
				...
				IF v_antiguedad  <= 2 THEN
					 ...
					 SELECT dia_corte--- mdy(month(pPeriodo),dia_corte,year(pPeriodo))
					 INTO v_dia_corte_pp --v_fecha_corte_pp
					 FROM sd_maecredanexocrd
					 WHERE empresa = v_empresa 
					 and num_Credito = v_num_credito_ext;
					 
					 IF v_dia_corte_pp =31 AND (MONTH(pPeriodo)=04 or MONTH(pPeriodo)=06 or MONTH(pPeriodo)=09 or MONTH(pPeriodo)=11) then
						LET v_dia_corte_pp=30;
					ELIF (v_dia_corte_pp =31 OR v_dia_corte_pp =30 OR v_dia_corte_pp =29) AND MONTH(pPeriodo)=02 then
						LET v_dia_corte_pp=28;
					END IF;
					
					LET v_fecha_corte_pp= mdy(month(pPeriodo),v_dia_corte_pp,year(pPeriodo));
				END IF;
					...
					--IF ((pPeriodo - v_fecha_apertura) /30.4) >=4 THEN
							...
							IF v_monto_exigible = 0 OR v_monto_exigible IS NULL THEN
								...
								INTO v_monto_exigible
								FROM sd_maesdoshistcrd
								WHERE num_credito =v_num_credito_ext and fecha=v_fecha_corte_pp; 
							END IF;

							--CAMPO 42 MONTO EXIGIBLE1		--pp
							...
							FROM sd_maesdoshistcrd
							WHERE num_credito =v_num_credito_ext and fecha=v_fecha_corte_pp - 1 units month; 	
							
							--CAMPO 43 MONTO EXIGIBLE2 		--pp
							--SELECT NVL(monto_financiado,0) + NVL(int_tra_no_exig,0) + NVL(mto_venc_int,0) + NVL(sdo_no_exig,0) + NVL(mto_finan_vdo,0) --Sin moratorios
							SELECT NVL(monto_financiado,0) + NVL(int_tra_no_exig,0) + NVL(mto_venc_int,0) + NVL(sdo_no_exig,0) + NVL(mto_finan_vdo,0)+ (sdo_contab_mora + sdo_moratorio)+  round((sdo_contab_mora + sdo_moratorio)*.16,2) --Con moratorios
							INTO v_monto_exigible2
							FROM sd_maesdoshistcrd
							WHERE num_credito =v_num_credito_ext and fecha=v_fecha_corte_pp - 2 units month; 	
							
							--CAMPO 44 MONTO EXIGIBLE3		--pp
							--SELECT NVL(monto_financiado,0) + NVL(int_tra_no_exig,0) + NVL(mto_venc_int,0) + NVL(sdo_no_exig,0) + NVL(mto_finan_vdo,0) --Sin moratorios
							SELECT NVL(monto_financiado,0) + NVL(int_tra_no_exig,0) + NVL(mto_venc_int,0) + NVL(sdo_no_exig,0) + NVL(mto_finan_vdo,0)+ (sdo_contab_mora + sdo_moratorio)+  round((sdo_contab_mora + sdo_moratorio)*.16,2) --Con moratorios
							INTO v_monto_exigible3
							FROM sd_maesdoshistcrd
							WHERE num_credito =v_num_credito_ext and fecha=v_fecha_corte_pp - 3 units month;  	
						ELIF v_antiguedad  = 1 THEN					
							  --CAMPO 41 MONTO EXIGIBLE		--REEST		
							SELECT NVL(monto_financiado,0) + NVL(int_tra_no_exig,0) + NVL(mto_venc_int,0) + NVL(sdo_no_exig,0) + NVL(mto_finan_vdo,0)
							INTO v_monto_exigible
						...
						IF v_antiguedad  <= 2 THEN
							...
							LET v_fecha_corte_pp=mdy(month(pPeriodo),v_dia_corte_pp,year(pPeriodo));
							LET dt_ini_per_rep= ((mdy(month(pPeriodo),v_dia_corte_pp,year(pPeriodo)) +1 units day) -1 units month);
							 /*SELECT mdy(month(pPeriodo),dia_corte,year(pPeriodo)), dia_corte,((mdy(month(pPeriodo),dia_corte,year(pPeriodo)) +1 units day) -1 units month)
							 INTO v_fecha_corte_pp,v_dia_corte_pp,dt_ini_per_rep
							 FROM sd_maecredanexocrd
							 WHERE empresa = v_empresa 
							 and num_Credito = v_num_credito_ext;*/
						END IF;
						...
						IF v_antiguedad  = 0 THEN	
							--SELECT SUM (CASE WHEN codigo_ref in (7,10,901,5,8) THEN monto ELSE 0 END) --Sin moratorios
							 SELECT SUM(monto)  --Monto pago completo
							  ...
							  FROM movs_pagos_ext
							 WHERE fecha_mov BETWEEN (dt_ini_per_rep - 1 units month) AND (v_fecha_corte_pp - 1 units month) --21/04 a 20/05
							   AND num_credito = v_num_credito_ext;
							...
							--SELECT SUM (CASE WHEN codigo_ref in (7,10,901,5,8) THEN monto ELSE 0 END) --Sin moratorios
							 SELECT SUM(monto)  --Monto pago completo
							  INTO v_pago_realizado2
							   FROM movs_pagos_ext
							  WHERE fecha_mov BETWEEN (dt_ini_per_rep - 2 units month) AND (v_fecha_corte_pp - 2 units month) --21/03 a 20/04
								AND num_credito = v_num_credito_ext;		
							...
							--SELECT SUM (CASE WHEN codigo_ref in (7,10,901,5,8) THEN monto ELSE 0 END) --Sin moratorios
							 SELECT SUM(monto)  --Monto pago completo
							   ...
							   FROM movs_pagos_ext
							  WHERE fecha_mov BETWEEN (dt_ini_per_rep - 3 units month) AND (v_fecha_corte_pp - 3 units month) --21/02 a 20/03
								AND num_credito = v_num_credito_ext;	
               ...
               -- and  month(a.fecha)||'-'||year(a.fecha) <> month(fecha_apertura)||'-'||year(fecha_apertura)               
                ...
                CREATE INDEX idx_INCUMp ON tmp_incumplimientos(fecha, status_cred) ONLINE;
		update statistics medium for table tmp_incumplimientos;
				...
				ELSE
					...
					SELECT NVL(sdo_cap_insoluto,0) ---+ NVL(monto_vencido,0) + NVL(mto_venc_trasp,0) + NVL(cap_tras_no_venci,0)+ NVL(sdo_no_exig,0)+NVL(sdo_acum_mes_int,0)  -- Se cambia el provision_normal x sdo_acum_mes_int, ya que se tenia un descueadre en 18 cuentas contra la balanza
					INTO v_saldo_cierre
					FROM sd_maesdoscontcrd
					WHERE num_credito=v_num_credito and fecha=mdy(month(pPeriodo), day(pPeriodo), year(pPeriodo));
				END IF;
			...
			----BLOQUE VARIABLES DE CIRCULO				
				...
				IF 	v_cred_consulta IS NULL THEN  --No se consulto a las SICs
					...
					IF nvl(v_atr,0) = 0 THEN
						LET v_bkatr = 10;
					ELIF nvl(v_atr,0) >= 1 THEN
						LET v_bkatr = 0;
					END IF;	
    ...
    END IF;*/
               ...
               select nvl(b.monto,0) Into v_gastos_originacion from sd_definicion a
				...
				LET v_tasa_interes_ree=v_tasa_contractual;
                ...
                --SELECT num_credito INTO v_num_credito_pp 
				--FROM sd_maecredcrd where credito_externo=v_num_credito;   
				
				SELECT credito_externo  INTO v_num_credito_pp 
				FROM sd_maecredcrd where num_credito=v_num_credito; 
				
				
                IF EXISTS (SELECT * FROM bdisolic:ss_Revision_determinacion WHERE num_solicitud = v_num_credito_pp) THEN  --VALIDAR SI AQUI ES credito_ree o credito pp
                    SELECT evalua_cc, bs_score, score_prop
                    INTO c_evalua_cc,n_scoreburo, n_scoreotor
                    FROM bdisolic:ss_Revision_determinacion 
                    WHERE num_solicitud = v_num_credito_pp;
					IF n_scoreotor IS NULL THEN
						...
						WHERE a.num_solicitud = v_num_credito_pp;
					END IF
                ELSE			
                    SELECT a.evalua_cc, b.evaluacion, c.evaluacion
                    INTO c_evalua_cc, n_scoreburo, n_scoreotor
				...
				SELECT limit 1 cuenta INTO v_cuenta 
				FROM BDICHEQ:SC_MAECHQ WHERE num_cte=v_num_cliente;
				
				SELECT SUM(nvl(monto_tot,0)) INTO v_pago_ini 
				FROM BDICHEQ:SC_MOVHIS WHERE fech_alt=v_fecha_apertura AND TRANSACC IN('0202','0204') AND cuenta=v_cuenta; 

				IF v_pago_ini=0 THEN
					select pago_inicial INTO v_pago_ini
					FROM sd_insumos_calif_reest_pp WHERE num_credito=v_num_credito AND fecha_cierre= pPeriodo -1 units month;
				END IF;
				...
				--se agrega las variables segÃÂºn cnbv
				select NVL(a.rfc,'')
					INTO v_rfc
               ...
               -- BEGIN WORK;
                    INSERT INTO sd_insumos_calif_reest_pp (fecha_cierre,
													alto,
                                                   ...
                                                   ree_pp_anterior,saldo_cierre,saldo_corte,saldo_exigible,saldo_no_exigible,
												   ...
												   antiguedad_bancos,sdo_actual_propio_ship,fecha_ult_pago,num_credito_pp,intereses_etapa1,intereses_etapa2, monto_condonado, pago_inicial,
												   lim_cred_origen,producto_orig,monto_ult_disp,mes_venc_orig,tasa_interes_ree,
												    Pct_Pgo0,Pct_pago1,Pct_pago2,Pct_pago3,Dias_Rem_Contractual,
													rfc,nombre_cte) --nvos cnbv
                                             VALUES(pPeriodo,
													v_alto,
                                                   ...
                                                   v_facturacion,v_fecha_apertura_format,v_fecha_apertura_cte_format,v_fecha_corte_format,v_fecha_venc_format,           
                                                   ...
                                                   v_ree_pp_anterior,nvl(v_saldo_cierre,0),nvl(v_saldo_corte,0),nvl(v_saldo_exigible,0),nvl(v_saldo_no_exigible,0),
												   ...
												   v_antiguedad_bancos,v_sdo_actual_propio_ship,v_fecha_ult_pago,v_num_credito_pp,v_intereses_etapa1,v_intereses_etapa2,nvl(v_condonaciones,0), nvl(v_pago_ini,0),
												   v_lim_cred_origen,v_producto_orig,v_monto_ult_disp,v_mes_venc_orig,v_tasa_interes_ree,
												   v_prom1,v_prom2,v_prom3,v_prom4,v_dias_rem_contractual,
												   v_rfc,c_nom_cte); --nvos cnbv
               -- COMMIT WORK;
...
SELECT DBINFO('utc_to_datetime', sh_curtime) INTO fin_proc 
  FROM sysmaster:sysshmvals;
    ...
    LET cCodRet     = "00000";
    LET cMensajeRet = "REPORTE DE REESTRUCTURAS PP OK "|| '8600';
