CREATE PROCEDURE "informix".situacion_pago_tienda_cjunk_precal(o_empresa CHAR(3),
...
-- Modifico: Viridiana Osobampo 
-- Descripcion: Se unifican cambios realizados para el proyecto Alta ÃÂnica con
--              la versiÃÂ³n del spl productivo.
--------------------------------------------------------------------------------
...
-- Modifico: Carolina Elizabeth Verdugo GastÃÂ©lum
-- Descripcion: Se agrega parÃÂ¡metro de entrada para validar sÃÂ­ es la primera o segunda 
-- ejecuciÃÂ³n del procedimiento para guardar en la tabla  ss_bitacora_precal.
--------------------------------------------------------------------------------
--****************************************************************************
...
DEFINE iValorMonetarioTotal         DECIMAL(10,2);      ---Autor: Angel Maya 14/05/2025
DEFINE iVencidoTotalizado           DECIMAL(10,2);      ---Autor: Angel Maya 14/05/2025
...
LET iValorMonetarioTotal        = 0;    ---Autor: Angel Maya 14/05/2025
LET iVencidoTotalizado          = 0;    ---Autor: Angel Maya 14/05/2025
...
--  ini jom Parametro referencia coppel
   ...
   END IF
	...
	If iVencidoAire is null then
	    
	    SELECT vencidototalaire, abonomensualaire, saldototalaire, vencidototalafiliados,
			   abonomensualafiliados, saldototalafiliados, vencidototalreestructura,
			   abonomensualreestructura, saldototalreestructura, scorepuntualidad, puntualidad
		  INTO iVencidoAire, iAbonoAire, iSaldoAire, iVencidoAfiliados, iAbonoAfiliados, iSaldoAfiliados,
			   iVencidoReestructura, iAbonoReestructura, iSaldoReestructura, iScorePuntualidad, cPuntualidad
		  FROM ss_resum_scor_fin sf
		 INNER JOIN bdisolic:ss_solicitudes sol ON sol.num_solicitud = sf.num_solicitud
		 WHERE numcte = o_num_cliente
		   AND sol.sucursal = LPAD(o_sucursal, 4, '0')
		   AND sol.num_producto = o_producto
		   AND sol.status_solicitud NOT IN ('AP','RT','AT','CM','CN','AN')
		   AND sol.fecha_hora = 
          ( select max(fecha_hora) from bdisolic:ss_solicitudes sol2 where numcte = o_num_cliente and sol2.sucursal = LPAD(o_sucursal, 4, '0')  and sol2.num_producto = o_producto and  
              sol2.status_solicitud not in ('AP','RT','AT','CM','CN','AN'));
   ...
   /*IF o_porcentaje >= 0 THEN
       IF o_porcentaje  < v_eva_min_sup OR o_porcentaje  >  v_eva_max_sup THEN
           ...
           LET vMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
							'en la pantalla "SelecciÃÂ³n de Productos". Para tramitar una solicitud de '||
							'crÃÂ©dito BanCoppel es necesario realizar puntualmente sus pagos '||
  ...
  -- IF ( v_avanza = 1 ) THEN
	...
	-- *****************************************************************
	  ...
	  --IF o_porcentaje >= v_eva_min_inf AND o_porcentaje < v_eva_min_sup THEN
           ...
           IF ( v_avanza = 1 ) THEN */
               ...
               LET vMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
								'en la pantalla "SelecciÃÂ³n de Productos". Para tramitar una solicitud de '||
								'crÃÂ©dito BanCoppel es necesario tener sus pagos al corriente en su Tienda '||
								'Coppel. Lo invito a ser mÃÂ¡s puntual en sus pagos en su tienda Coppel.';
			   LET scod_ret = "001";
   ...
   END IF	
    ...
    ---Autor: Angel Maya 14/05/2025
    ---Se suma cada uno de los seis montos vencidos, denominados o_vencidomuebles + o_vencidoropa + o_vencidoprestamos + iVencidoAire + iVencidoAfiliados + iVencidoReestructura, para obtener un total acumulado de cuentas vencidas (iVencidoTotalizado).
    LET iVencidoTotalizado = nvl(o_vencidomuebles,0) + nvl(o_vencidoropa,0) + nvl(o_vencidoprestamos,0) + nvl(iVencidoAire,0) + nvl(iVencidoAfiliados,0) + nvl(iVencidoReestructura,0);
    LET iValorMonetarioTotal = vMaxMtoUdi * vValorUdi; ---14/05/2025 calculando el valor monetario total (iValorMonetarioTotal) mediante la multiplicaciÃÂ³n de las variables existentes vValorUdi y vMaxMtoUdi, lo que establece el lÃÂ­mite permitido. 

    IF iVencidoTotalizado > iValorMonetarioTotal THEN      


        LET cCausaSol = "ACC";  
        LET vMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
						'en la pantalla "Seleccion de Productos". Para tramitar una solicitud de '||
						'credito BanCoppel es necesario tener sus pagos al corriente en su Tienda '||
						'Coppel. Lo invito a ser mas puntual en sus pagos en su tienda Coppel.';
        
        LET scod_ret = "001";

        -- Inserta Bitacora
        INSERT INTO bdisolic:ss_bitacora_precal(empresa,fecha,producto,sucursal,nombre,nombre_coppel,num_referencia,ejecutivo,porcentaje,situacion,meses_hist,
                    causa,motivo,tipo_rechazo,codret,mensaje,causa_solicitud, vencidototalmuebles, vencidototalropa, 
                    vencidoprestamos, saldomuebles, saldoropa, saldoprestamos, abonomuebles, abonoropa, 
                    abonoprestamos,fecha_ultima_compra,vencidototalaire,abonomensualaire,     
                    saldototalaire,vencidototalafiliados,abonomensualafiliados,saldototalafiliados,vencidototalreestructura,
                    abonomensualreestructura,saldototalreestructura,scorepuntualidad)
        VALUES     (o_empresa,CURRENT,o_producto,o_sucursal,o_nombre,o_nombre_coppel,o_num_referencia,o_ejecutivo,o_porcentaje,
                    o_situacion,o_meses_hist,o_causa,v_motivo,vTipoRech,scod_ret,vMensaje,cCausaSol, o_vencidomuebles, 
                    o_vencidoropa, o_vencidoprestamos, o_saldomuebles, o_saldoropa, o_saldoprestamos, o_abonomuebles, o_abonoropa,
                    o_abonoprestamos,o_ultimacompra,iVencidoAire,iAbonoAire, iSaldoAire,iVencidoAfiliados,iAbonoAfiliados,
                    iSaldoAfiliados,iVencidoReestructura,iAbonoReestructura,iSaldoReestructura,iScorePuntualidad);
        RETURN scod_ret, vMensaje;
    END IF
   ...
   IF (trim(v_paso_cliente) <> trim(o_num_referencia)) THEN
       ...
       IF NVL(o_meses_hist,0)<v_MesesHis THEN
       	   LET cCausaSol = "PAC";
           LET vMensaje = "Cliente con poca antigÃÂ¼edad en Coppel";
           LET scod_ret = "001";
