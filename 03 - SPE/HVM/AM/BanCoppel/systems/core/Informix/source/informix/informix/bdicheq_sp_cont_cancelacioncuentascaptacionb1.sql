CREATE PROCEDURE "informix".sp_cont_cancelacioncuentascaptacionb1(pBandera CHAR(1), pUsuario CHAR(8), pEmpresa CHAR(10), pNumeroCuenta CHAR(20),
	pNumcte CHAR(10), pTarjeta CHAR(20), pSolicitudes  SMALLINT, pOrigen CHAR(1), pMotivo CHAR(2), pPromotor CHAR(8), pSucursal CHAR(4), pIdFuncion CHAR(10), pRutaDescarga CHAR(100))
    RETURNING CHAR(5)  AS codret,
			  ...
			  CHAR(1)  AS cod_estatus,
			  CHAR(30) AS descripcion_estatus,
			  CHAR(10) AS fecha_ultimo_mov,
			  MONEY(14,2) AS saldo,
			  CHAR(20) AS folio_cancel,
	...
	DEFINE cDescripcionCancelacion     CHAR(40);
	DEFINE cNumero_cliente		CHAR(20);
	DEFINE cDescripcionMensaje  CHAR(80);
    ...
    DEFINE cCodigoEstatus		CHAR(1);     
    DEFINE cDescripcionEstatus	CHAR(30);     
    DEFINE cFechaUltimoMovimiento	CHAR(10);     
    DEFINE mSaldo				MONEY(14,2); 
	DEFINE cFolioCancel			CHAR(22);
	DEFINE cNombre_sucursal		CHAR(40);
	DEFINE cReporteGenerado		CHAR(100);
	...
	LET cFechaUltimoMovimiento  = '';
	LET mSaldo					= '';
	LET cFolioCancel			= '';
	LET cNombre_sucursal		= '';
	LET cReporteGenerado		= '';
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;			
			RETURN cCodRet, cNomCte1, cRazon_social,cEmpresa, dFecha_hoy, cCodRet2,cNomCte2,cApellPat,cApellMat,cRfc,cFechaNacimiento,
				cFechaAlta,cNombre_sucursal,cCuenta, cClaveCancelacion, cDescripcionCancelacion,cDescripcionMensaje, cNumero_cliente,cCodigoProducto,cNombreProducto,
				cFechaApertura,cCodigoEstatus,cDescripcionEstatus,cFechaUltimoMovimiento,mSaldo,cFolioCancel,cSucursal,cReporteGenerado;
		END EXCEPTION;
		
		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cap_cancelacioncuentascaptacionb1.out';
		--TRACE ON;
		...
		IF pBandera = '1' THEN
			FOREACH
				EXECUTE PROCEDURE sp_motivocancel('', 1) 
				INTO cCodRet, cDescripcionMensaje, cClaveCancelacion, cDescripcionCancelacion
				
				RETURN cCodRet, cNomCte1, cRazon_social,cEmpresa, dFecha_hoy, cCodRet2,cNomCte2,cApellPat,cApellMat,cRfc,cFechaNacimiento,
					cFechaAlta,cNombre_sucursal,cCuenta, cClaveCancelacion, cDescripcionCancelacion,cDescripcionMensaje, cNumero_cliente,cCodigoProducto,cNombreProducto,
					cFechaApertura,cCodigoEstatus,cDescripcionEstatus,cFechaUltimoMovimiento,mSaldo,cFolioCancel,cSucursal,cReporteGenerado WITH RESUME;
			END FOREACH
		...
		ELIF pBandera = '4' THEN
			...
			EXECUTE PROCEDURE bdinteg:"informix".sp_infoctecancel(pEmpresa, pNumcte, pNumeroCuenta, pTarjeta)
			INTO cCodRet,cCodRet2,cDescripcionMensaje,cNumero_cliente,cNomCte1,cNomCte2,cApellPat,cApellMat,cRfc,cFechaNacimiento,cFechaAlta,cSucursal,cCuenta;
			
			IF (cCodRet = '051' AND cCodRet2 = '321') THEN 
				LET cCodRet = '00022';				LET cCodRet2 = '00022';
			ELIF (cCodRet = '052' AND cCodRet2 = '322') THEN 
				LET cCodRet = '00009';				LET cCodRet2 = '00009';
			ELIF(cCodRet = '054' AND cCodRet2 = '324') THEN 
				LET cCodRet = '00029';				LET cCodRet2 = '00029';
			ELSE 
				SELECT nombre
				INTO cNombre_sucursal
				FROM bdinteg:"informix".si_sucursales
				WHERE empresa='001' AND sucursal = cSucursal;
			END IF;
		
		ELIF pBandera = '5' THEN
			
			FOREACH
				EXECUTE PROCEDURE sp_obtienectascancel(pEmpresa,pNumCte,pNumeroCuenta,pTarjeta,pSolicitudes,pOrigen)
				INTO cCodRet,cCodRet2,cDescripcionMensaje,cNumero_cliente,cCuenta,cCodigoProducto,cNombreProducto,cFechaApertura,cCodigoEstatus,
					cDescripcionEstatus,cFechaUltimoMovimiento,mSaldo
		...
		ELIF pBandera = '6' THEN 
			...
			EXECUTE PROCEDURE sp_cancelactachq(pEmpresa,pNumeroCuenta, pMotivo, pPromotor, pSucursal)
			INTO cCodRet,cCodRet2,cDescripcionMensaje,cFolioCancel;
		...
		ELIF pBandera = '7' THEN
			EXECUTE PROCEDURE "informix".sp_cap_genrepcancelacioncuentascaptacion(pPromotor, pIdFuncion,pRutaDescarga)
			INTO cCodRet, cReporteGenerado;
			
		END IF;

		IF pBandera NOT IN (1,5)  THEN
			RETURN cCodRet, cNomCte1, cRazon_social,cEmpresa, dFecha_hoy, cCodRet2,cNomCte2,cApellPat,cApellMat,cRfc,cFechaNacimiento,
				cFechaAlta,cNombre_sucursal,cCuenta, cClaveCancelacion, cDescripcionCancelacion,cDescripcionMensaje, cNumero_cliente,cCodigoProducto,cNombreProducto,
				cFechaApertura,cCodigoEstatus,cDescripcionEstatus,cFechaUltimoMovimiento,mSaldo,cFolioCancel,cSucursal,cReporteGenerado;
		END IF
...
'FECHA: 12/12/2022',
'MODULO: CONTABILIDAD',
'FUNCIONALIDAD: CancelaciÃ³n Cuentas CaptaciÃ³n',
