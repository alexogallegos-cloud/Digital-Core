CREATE PROCEDURE "informix".sp_calcsdo_ctasinactivas( pSdoConcentrado DECIMAL(18,2), pNumCte CHAR(20), pFechaConcentra DATE, pFechaHoy DATE )
RETURNING CHAR(5), DECIMAL(14,2), DECIMAL(14,2);
    ...
    ON EXCEPTION SET Sql_Err, Isam_Err, Desc_Err
        SET DEBUG FILE TO "/resplogifx/conciliachq/sp_calcsdo_ctasinactivas.err";
        TRACE ON;
        IF Sql_Err <> 0 THEN
            ...
            IF vEnTransacc = 1 THEN
                ROLLBACK WORK;
            END IF;
            RETURN vCodRet1, vIntereses, vISR;
        END IF;
    END EXCEPTION;
    
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/sp_calcsdo_ctasinactivas.out";
    --- TRACE ON;
    ...
    IF ( ( pNumCte is null OR pNumCte = '' ) OR
         ( pFechaHoy is null OR pFechaHoy = '' ) OR
         ( pSdoConcentrado is null OR pSdoConcentrado <= 0 ) OR
         ( pFechaConcentra is null OR pFechaConcentra = '' ) ) THEN
        LET vCodRet1 = '110';
        RETURN vCodRet1, vIntereses, vISR;
    END IF;
    ...
    LET vAnioMesIni = TO_CHAR(pFechaConcentra, '%Y%m');
    ...
    LET vAnioMesFin = TO_CHAR(pFechaHoy, '%Y%m');
    ...
    LET vDias = (pFechaHoy - pFechaConcentra) + 1;
    
    -- // CALCULA LOS INTERESES
    IF vDias > 0 AND vAjustexInf > 0.00 THEN
        LET vIntereses = (((pSdoConcentrado * vAjustexInf) / 365) * vDias);
    ELSE
        LET vIntereses = 0.00;
    END IF;
    ...
    IF vIntereses > 0.00 THEN
        ...
        -- // CALCULA LA BASE GRAVABLA PARA COBRO DE ISR
        LET vBaseGravable = pSdoConcentrado - vBaseExenta;
        ...
        IF vPorRetSuj <> 0 THEN
            ...
            IF vTipoPersona = 'F' THEN
                ...
                END IF;
            ELSE
                LET vISR = TRUNC( ( pSdoConcentrado * vTasa_ISR ), 2 );
            END IF;
        ELSE
            LET vISR = 0.00;
        END IF;
