CREATE PROCEDURE "informix".consfircredcaja(pEmpresa char(3), pNumeroCredito char(20), pNumeroCte char(20))
--DATOS A REGRESAR
RETURNING
...
LET vSecuencia = "";
	...
	--SET DEBUG FILE TO "/tmp/consfircredcaja.out"; 
	--TRACE ON;
...
FOREACH
        ...
        SELECT DISTINCT
                sd_tar.secuencia, si_cte.numcte, si_cte.apell_paterno, si_cte.apell_materno, si_cte.nombre1, si_cte.nombre2, si_cte.rfc,  si_pf.fecha_nac, sd_mae.num_producto
        INTO
                vSecuencia, vNumCte, vApell_Paterno, vApell_Materno, vNombre1, vNombre2, vRfc, vFechaNacimiento, vProductoCredito
        FROM
        ...
        WHERE
                ...
                sd_tar.status_tar != 'C' AND
                sd_tar.secuencia = (SELECT MAX(sd_tar.secuencia) FROM bdicred:"informix".sd_tarjeta AS sd_tar WHERE sd_tar.empresa = pEmpresa AND sd_tar.num_credito = pNumeroCredito AND sd_tar.numcte = vNumCte);
