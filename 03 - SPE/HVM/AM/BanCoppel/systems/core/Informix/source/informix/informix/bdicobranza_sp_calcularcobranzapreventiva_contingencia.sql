CREATE PROCEDURE "informix".sp_calcularcobranzapreventiva_contingencia()
...
truncate table CB_INFO_PREVENTIVA;
---------------------------------------------------------------------------------
...
select
    ...
    e.telefono2 Telefono_Celular,                                                               --Tel celular
    (b.sdo_cap_insoluto +
    (SELECT SUM(NVL(interes_debe,0) - NVL(interes_pagado,0))+
            SUM(NVL(iva_debe,0) - NVL(iva_pagado,0)) +
            SUM(NVL(mora_provi_ordi,0) + NVL(mora_provi_cope,0) + NVL(mora_sdo_ordi,0) -
    ...
    (SELECT SUM(NVL(iva_debe,0) - NVL(iva_pagado,0))
        FROM bdicred:sd_amortiza_credito
        WHERE empresa = '001'
        and num_credito = a.num_credito
        and capital_status IN ('2','7')) + (SELECT (sum(nvl(mora_provi_ordi,0)) +
        sum(nvl(mora_provi_cope,0))) * (1 + nvl( h.iva,0))
        FROM bdicred:sd_amortiza_credito
        WHERE empresa = '001'
        AND num_credito = a.num_credito) +
        b.monto_financiado  ) Pago_Minimo_Total,                                                -- Pago Minimo
    (SELECT count(v.empresa)
       FROM bdicred:sd_maecred t,
            bdicred:sd_maesdos u,
            bdicred:sd_amortiza_credito v
      WHERE t.empresa = u.empresa
        AND t.num_credito = u.num_credito
        AND t.empresa = v.empresa
        AND t.num_credito = v.num_credito
        AND u.monto_financiado >= 100
        AND v.capital_status in (7,2)
        AND t.empresa = a.empresa and t.num_credito = a.num_credito) as vencido                 -- vencido
...
join bdinteg:si_direcciones e on (a.numcte = e.numcte and tipo_dir = '1'
             and e.secuencia = ( select max(secuencia)
             FROM bdinteg:si_direcciones where tipo_dir = '1' and a.numcte = numcte))
join bdicred:sd_amortiza_credito v on (a.empresa = v.empresa and a.num_credito = v.num_credito)
...
left outer join bdinteg:si_sucursales h on (h.sucursal = a.sucursal and h.empresa = a.empresa)
WHERE a.empresa = '001'
  AND a.status_cred in ('BA')
  AND b.monto_financiado >= 100
  AND v.capital_status in (7,2)
---
--  AND (b.monto_vencido + b.mto_venc_trasp) >= 5000 and Monto_vencido >= 5000
