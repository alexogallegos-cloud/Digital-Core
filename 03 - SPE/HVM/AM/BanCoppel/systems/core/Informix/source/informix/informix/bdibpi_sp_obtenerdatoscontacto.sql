CREATE PROCEDURE "informix".sp_obtenerdatoscontacto(pNumCliente VARCHAR(9))
RETURNING CHAR (5), CHAR(100), CHAR(15), INT;
	...
	BEGIN
		ON EXCEPTION SET sql_err
		  IF sql_err <> 0 THEN
				...
				RETURN vCod_ret, vEmail, vCel, vCiaCel;
		  END IF ;
		END EXCEPTION ;
		...
		SELECT telefono, carrier INTO vCel, vCiaCel FROM bdinteg:"informix".si_telefonos_actual WHERE numcte = pNumCliente AND tipo_tel = '2';
		SELECT first 1 correo_elec INTO vEmail FROM bdinteg:"informix".si_correos WHERE numcte = pNumCliente AND tipo_correo = 1 AND status_correo = 'A';
					
		RETURN vCod_ret, NVL(vEmail, ''), NVL(vCel, ''), NVL(vCiaCel, 0);		
	END;
END PROCEDURE
DOCUMENT
