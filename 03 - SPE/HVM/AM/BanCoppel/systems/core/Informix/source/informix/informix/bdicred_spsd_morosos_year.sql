create procedure "informix".spsd_morosos_year(p_empresa char(3), p_fecha date)
       returning char(3),       date,          date,
                 ...
                 decimal(18,2),
                 varchar(60),   varchar(10),   decimal(18,2), integer;
   ...
   --###### Agroupa por fecha de apertura en r_groupby
   foreach select trim(a.num_credito), 
	   case
	   when incr_date(b.fecha_apertura, 12) >= r_fecha
	    then 1
	   when incr_date(b.fecha_apertura, 24) >= r_fecha 
	    then 2
	   when incr_date(b.fecha_apertura, 36) >= r_fecha
	    then 3
	   else
	    4
	   end 
           into   r_credito, r_groupby
           from   outer sd_pagocapit as a, sd_maecred as b, sd_marcpro as c
