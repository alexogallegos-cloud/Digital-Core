CREATE PROCEDURE "informix".sp_obtienechqpag(pFechaInicio CHAR(10), pFechaFin CHAR(10))
RETURNING CHAR(6),
          CHAR(4),
          CHAR(40),
          CHAR(20),
...
BEGIN
    ...
    IF TRIM(pFechaInicio) <> '' AND pFechaInicio IS NOT NULL OR TRIM(pFechaFin) <> '' AND pFechaFin IS NOT NULL THEN
       ...
       FOREACH 
            ...
            SELECT sh.sucursal, sc.nombre, sh.cuenta, sh.numchq, sh.monto, SUBSTRING(sh.fecha_alta FROM 4 FOR 2)||'/'||SUBSTRING(sh.fecha_alta FROM 1 FOR 2) ||'/'||SUBSTRING(sh.fecha_alta FROM 7 FOR 4)||' '||sh.hora_alta, sh.folio_suc, sh.transaccion, sch.num_cte, sh.secuencia 
