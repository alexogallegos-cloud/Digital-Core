CREATE PROCEDURE "informix".sp_reversioncrd_ofi(pEmpresa CHAR(3),
												pSucursal CHAR(4),
												pUsuario CHAR(8),
												pFolio CHAR(16),
												pTiporev CHAR(1))
	...
	--SET DEBUG FILE TO "/home/sysifx/sp_reversioncrd_ofi.out";
	--TRACE ON;
	...
	BEGIN
		...
		EXECUTE PROCEDURE "informix".reversioncrd(pEmpresa, pSucursal, pUsuario, pFolio, pTiporev) INTO cCodRet;
		IF cCodRet::INTEGER NOT IN(0,1) THEN
			LET cMensajeRet = 'Error al realizar la reversión del pago';
		ELSE
			LET cCodRet = '00000';
		END IF;
		RETURN cCodRet,cMensajeRet;
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se crea procedimiento para realizar la reversion de un pago de prestamo desde sucursal',
'AUTOR: Jesús Aguilar',
'FECHA: 24 de Junio de 2011',
'BD: BDICRED',
'VERSION: 20110624.1700';

CREATE PROCEDURE "informix".sp_consctaligada( pEmpresa char(3), pNumCredito char(20))
RETURNING 
	CHAR(5) AS Codigo_retorno,
	CHAR(80)  AS Mensaje_retorno,
...
--INICIALIZACION DE VARIABLES 	
   ...
   BEGIN
	...
	---SET DEBUG FILE TO "/home/sysifx/jesusm/sp_consctaligada.out";
	---TRACE ON;
   ...
   SET LOCK MODE TO WAIT 5;
	 ...
	 INNER JOIN bdicheq:"informix".sc_maechq b ON (b.cuenta = a.num_cta)
     WHERE a.empresa = pEmpresa
     AND a.num_credito = pNumCredito;
	 
	   IF DBINFO("sqlca.sqlerrd2") = 0 THEN
            LET cCodRet = '00362';
			LET cMensajeRet= 'No se exite cuenta ligada al prestamo';
       END IF;
	
	 RETURN cCodRet,cMensajeRet,cNumCta;
  
END
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se crea procedimiento para obtener la cuenta ligada al prestamo del cliente', 
'AUTOR: Jesús Aguilar',
'FECHA: 29 Abril 2011',
'BD: BDICRED',
'VERSION: 20110429.1700';

CREATE PROCEDURE "informix".sp_ofi_validacred(pEmpresa CHAR(3),pNumCredito CHAR(20))
RETURNING
	CHAR(5) AS COD_RET,
	CHAR(80)  AS MENSAJE_RETORNO,
...
BEGIN
	...
	---SET DEBUG FILE TO "/tmp/has/sp_ofi_validacred.out";
	---TRACE ON;
	IF NVL(pEmpresa,"") =  "" OR NVL(pNumCredito,"") = "" THEN
		LET cCodRet = '00361';
		LET cMensajeRet= 'PARAMETROS INVALIDOS';
	...
	LET cSiglasProd = SUBSTR(pNumCredito,1,2);
	--se obtiene el producto relacionado con el numero de crédito.
	SET ISOLATION TO DIRTY READ;
    SET LOCK MODE TO WAIT 3;

	SELECT TRIM(def.num_producto)
	INTO cProducto
	FROM "informix".sd_definicion def,
		 "informix".sd_tipprod tp
	WHERE siglas = cSiglasProd
	AND def.num_producto = tp.abrevia_prod
	AND tp.cod_prod IN ('P','R');

	LET iNRows = dbinfo("sqlca.sqlerrd2");

    IF iNRows = 0 THEN
        LET cCodRet = '00367';
		LET cMensajeRet= 'PRODUCTO INVALIDO PARA REALIZAR LA CONSULTA';
        RETURN cCodRet,cMensajeRet,'','','','';
    END IF
	...
	RETURN cCodRet,cMensajeRet,NVL(cProducto,''),NVL(cNomProductoCD,''),NVL(cNomCteCD,''),NVL(cNumCteCD,'');
END;
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se realiza procedimiento para validar si es posible realizar un pago de prestamo a plazo ',
'AUTOR: Mohamed Carreón, Jesús Aguilar',
'FECHA: 28 de Abril 2011',
'BD: BDICRED',
'VERSION: 20110428.1708';

CREATE PROCEDURE "informix".sp_consprestamocte( pEmpresa CHAR(3), pNumCliente CHAR(20),pRegistros SMALLINT)
RETURNING 
	CHAR(5) AS Codigo_retorno,
	CHAR(80)  AS Mensaje_retorno,
	CHAR(4) AS Producto,
	CHAR(20) AS Numero_prestamo,
...
--INICIALIZACION DE VARIABLES 	
	...
	LET cNumProd   		= "";
	LET cStatus   		= "";
	LET dtFechaAper		= MDY(1,1,1900);
	LET dtFechaVenc		= MDY(1,1,1900);
	LET iContador		= 0;
...
---VARIABLES PARA CACHAR LOS CAMPOS DEL PROCEDMIENTO DE sp_consulta_saldos_general
   ...
   BEGIN
	...
	---SET DEBUG FILE TO "/home/sysifx/jesusm/sp_consprestamocte.out";
	---TRACE ON;
   ...
   FOREACH 
   	EXECUTE PROCEDURE "informix".sp_consulta_datos_general(pEmpresa,pNumCliente,'','','','','')
		INTO cCodRetCD,cMensajeCD,cNumCredCD,cNumCteCD,cNomProductoCD,cNumTarjetaCD,cNomCteCD
	...
	END IF
	 ...
	 SELECT num_producto,fecha_apertura,fecha_vencim,status_cred
       INTO  cNumProd,dtFechaAper,dtFechaVenc,cStatus
     FROM "informix".sd_maecredcrd 
     WHERE empresa = pEmpresa
     AND num_credito = cNumCredCD;
	  --Sii no existe informacion, no es un prestamo
	  IF DBINFO("sqlca.sqlerrd2") = 0 THEN
         CONTINUE FOREACH;
       END IF;
	...
	EXECUTE PROCEDURE "informix".sp_consulta_saldos_general(pEmpresa,cNumCredCD)
	...
	END IF
	 ...
	 RETURN cCodRet,cMensajeRet,cNumProd,cNumCredCD,cNumCteCD,cNomCteCD,mCsg_sdo_act_total_cap,dtFechaVenc,dtFechaAper,cStatus WITH RESUME;

  END FOREACH;
END
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se crea procedimiento para obtener los prestamos con los que cuenta un cliente', 
'AUTOR: Jesús Aguilar',
'FECHA: 26 Abril 2011',
'BD: BDICRED',
'VERSION: 20110429.1700';
