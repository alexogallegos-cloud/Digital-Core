CREATE PROCEDURE "informix".sp_autenticacionafore(cNombre CHAR(52), cPaterno CHAR(26), cMaterno CHAR(26), cFechaNac CHAR(10), cSexo CHAR(1))		
...
--SET DEBUG FILE TO "/tmp/cristo/sp_autenticacionafore.out";
--TRACE ON;
...
BEGIN 
	...
	ON EXCEPTION SET sql_err, isam_err, error_info
		IF sql_err <> 0 THEN
			LET cCodRet = sql_err;
			RETURN cCodRet, cMensaje, cNumCte, cNombres, cApellidoP, cApellidoM, cFechaNacimiento, iGenero, cRFC;
		END IF;
	END EXCEPTION;
	...
	IF NVL(cNombre,'') = ''   OR cNombre = ''	
	OR NVL(cPaterno,'') = ''  OR cPaterno = ''
	OR NVL(cFechaNac,'') = '' OR cFechaNac = ''
	OR NVL(cSexo,'') = ''     OR cSexo = '' THEN
	
		LET cCodRet= '00001';		
		
	ELSE
		LET dFechaNac=TO_DATE(cFechaNac,'%Y-%m-%d')::DATE;		
		EXECUTE PROCEDURE bdinteg:"informix".sp_calcularfc('001', cPaterno, cMaterno, cNombre, '', dFechaNac) INTO cCodRet,cMensaje,cRFC;
		
		IF NVL(cRFC,'') <> '' OR cRFC IS NOT NULL THEN
		
			SELECT c.numcte,p.sexo INTO cNumCte,cGenero
			FROM si_cliente AS c INNER JOIN si_ctepf AS p
			ON c.empresa = p.empresa AND c.numcte = p.numcte
			WHERE c.rfc = cRFC;
			
			IF 	DBINFO('sqlca.sqlerrd2') = 0 THEN
				LET cCodRet= '00002';		
			ELSE
				IF (SELECT COUNT(*) FROM bdicheq:"informix".sc_maechq WHERE status_cta = 1 AND num_cte = cNumCte) > 0 THEN
					  
					LET cNombres = 'TRUE';
					LET cApellidoP = 'TRUE';
					LET cApellidoM = 'TRUE';
					LET cFechaNacimiento = 'TRUE'; 
					
					IF (cGenero = 'M' AND cSexo='1') Or (cGenero = 'F' AND cSexo='2') THEN
						LET iGenero = "TRUE";
					ELSE
						LET iGenero = "FALSE";
					END IF;
				...
				ELSE
					IF (SELECT COUNT(*) FROM bdicheq:"informix".sc_maechq WHERE num_cte = cNumCte) > 0 THEN
						LET cCodRet= '00007';
					ELSE
						LET cCodRet= '00006';
					END IF;
	...
	SELECT opcode_ds INTO cMensaje FROM bdisac:"informix".sac_ws_catmensajes WHERE agent_trans_type_code='BCPL_AUTAF' AND opcode = SUBSTR(cCodRet,2,4);	
	RETURN cCodRet, cMensaje, NVL(cNumCte,''), cNombres, cApellidoP, cApellidoM, cFechaNacimiento, iGenero, NVL(cRFC,'');
...
"FECHA : 09/09/2019",
"BD    : Bdinted ",
"Descripcion: sp de validaciÃ³n de datos",
