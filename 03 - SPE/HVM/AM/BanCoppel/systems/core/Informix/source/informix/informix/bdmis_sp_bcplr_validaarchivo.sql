CREATE PROCEDURE "informix".sp_bcplr_validaarchivo(vnombrearchivo		CHAR (35))
...
BEGIN
  ...
  -- trace on;
   ...
   DELETE FROM mi_rcda_cifrascontrol WHERE archivo = vnombrearchivo;
   FOREACH cursor1 WITH HOLD for
	   SELECT clave, num_solicitud, suc_tienda, promotor, cliente_cand_club, compraClub, fechageneracion, nombre_cliente, ap_paterno, ap_materno, fecha_inser,fecha_apertura, fecha_primercompra 
	   INTO vclave,	vnum_solicitud, vsuc_tienda, vpromotor, vcliente_cand_club, vCompraClub, vfechageneracion, vnombre_cliente, vap_paterno, vap_materno, vfecha_inser,vfecha_apertura, vfecha_primercompra 
	   FROM mi_rcda_infocoppel_paso
	   
	   IF (SELECT COUNT(*) FROM mi_rcda_cifrascontrol WHERE archivo = vnombrearchivo  ) = 0 THEN
		
			INSERT INTO mi_rcda_cifrascontrol (archivo, Total_registros) VALUES ( vnombrearchivo, 1);
			
			if vclave =' ' THEN
			
				UPDATE mi_rcda_cifrascontrol set Total_Solicitud = Total_Solicitud + 1 WHERE archivo = vnombrearchivo ;				
				
				execute PROCEDURE "informix".sp_rcda_esnumerico(vnum_solicitud)
				into vnumero ;
				IF	vnumero = 'F' THEN				
				
					UPDATE mi_rcda_cifrascontrol set Total_Solicitud_err = Total_Solicitud_err + 1 WHERE archivo = vnombrearchivo ;
					CONTINUE FOREACH;
	...
	--nombre_cliente, ap_paterno, ap_materno, fecha_nacimiento
				...
				*/
				...
				END IF		
						...
						UPDATE mi_rcda_cifrascontrol set total_cambio_solicitud = total_cambio_solicitud + 1 WHERE archivo = vnombrearchivo ;				
						
						execute PROCEDURE "informix".sp_rcda_esnumerico(vnum_solicitud)
						into vnumero ;
						IF	vnumero = 'F' THEN				
						
							UPDATE mi_rcda_cifrascontrol set total_cambio_solicitud_err = total_cambio_solicitud_err + 1 WHERE archivo = vnombrearchivo ;
							CONTINUE FOREACH;
						...
						UPDATE mi_rcda_cifrascontrol set total_alta = total_alta + 1 WHERE archivo = vnombrearchivo ;				
						
						execute PROCEDURE "informix".sp_rcda_esnumerico(vnum_solicitud)
						into vnumero ;
						IF	vnumero = 'F' THEN				
						
							UPDATE mi_rcda_cifrascontrol set total_alta_err = total_alta_err + 1 WHERE archivo = vnombrearchivo ;
							CONTINUE FOREACH;
						...
						UPDATE mi_rcda_cifrascontrol set total_compra = total_compra + 1 WHERE archivo = vnombrearchivo ;				
						
						IF	vnum_solicitud <> ' ' and vnum_solicitud <> '0'  THEN				
						
							UPDATE mi_rcda_cifrascontrol set total_compra_err = total_compra_err + 1 WHERE archivo = vnombrearchivo ;
							CONTINUE FOREACH;
						...
						UPDATE mi_rcda_cifrascontrol set total_club = total_club + 1 WHERE archivo = vnombrearchivo ;				
						
						IF	vnum_solicitud <> ' ' THEN				
						
							UPDATE mi_rcda_cifrascontrol set total_club_err = total_club_err + 1 WHERE archivo = vnombrearchivo ;
							CONTINUE FOREACH;
		...
		ELSE
			...
			UPDATE mi_rcda_cifrascontrol set Total_registros = Total_registros + 1 WHERE archivo = vnombrearchivo ;
