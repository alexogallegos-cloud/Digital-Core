CREATE PROCEDURE "informix".sp_dg_consproducto_acl(pCodigo CHAR(4))
	RETURNING CHAR(5) AS codret,
		CHAR(2) AS cod_sistema,
	...
	DEFINE cCodDef CHAR(3);
	DEFINE cNomProducto CHAR(50);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cCodSistema,cCodDef,cNomProducto;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_dg_consproducto_acl.out';
		--TRACE ON;
		...
		SELECT cod_sistema,cod_definicion,prod_nombre
		INTO cCodSistema,cCodDef,cNomProducto
		FROM bdidigital@coppelimg_tcp:dg_definicion 
		WHERE empresa = cEmpresa
		AND cod_producto = pCodigo
		AND cod_sistema = 'AC';
...
'FUNCIONALIDAD: ',
'DESCRIPCION: SPL encargado de consultar el detalle del producto.',
'BD: bdidigital';
