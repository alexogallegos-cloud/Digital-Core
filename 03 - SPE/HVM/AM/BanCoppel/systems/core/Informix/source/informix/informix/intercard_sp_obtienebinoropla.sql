CREATE PROCEDURE "informix".sp_obtienebinoropla(pProductoOro CHAR(4),pProductoPla CHAR(4))
	--DATOS A REGRESAR
	RETURNING 
	CHAR(5) AS cCodRet,	
	VARCHAR(70) AS cBines
--========== DEFINIR VARIABLES =======
	DEFINE iSqlErr SMALLINT;
	DEFINE iSamErr SMALLINT;
	DEFINE cErrorInfo CHAR(40);
	DEFINE cCodRet CHAR(5);	
	DEFINE cBines VARCHAR(70);
	DEFINE cBin CHAR(6);
...
BEGIN
	ON EXCEPTION SET iSqlErr, iSamErr, cErrorInfo
		LET cCodRet = iSqlErr;
		RETURN  cCodRet,cBines;
	END EXCEPTION;

	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;

	-- SET DEBUG FILE TO "/home/sysifx/respaldosbd/JudithMoreno/sp_obtieneBinOroPla.out";
	--TRACE ON;
	IF (NVL(pProductoOro,'')) = '' OR (NVL(pProductoPla,'')) = '' THEN
		LET cCodRet = '00001';		
	ELSE
		FOREACH 
			SELECT bin 
			...
			WHERE codprodcta IN (pProductoOro,pProductoPla)
			
			LET cBines = cBines || cBin || ",";
			
		END FOREACH;
	END IF;
	LET cBines = (SUBSTR(cBines,0,(length(cBines))-1));
	
	RETURN  cCodRet,cBines;
END;
END PROCEDURE
DOCUMENT
'Folio: 488 - Enmascarar el numero de tarjeta para el proyecto Control y registro de tarjetas en sucursal',
'Autor: 97893323 - Judith Moreno',
'BD: intercard',
'Solicita:	Abraham Narvaez',
'Fecha: 17/07/2018',
'Descripcion: Se crea procedimiento para obtener los bines de los productos oro (8000) y platinum (7100)';
