CREATE PROCEDURE "informix".sp_obtenerinforeversionpago(pFolio_grupo  CHAR(16))

RETURNING  CHAR(6), CHAR(50), CHAR(20), MONEY(14,2), CHAR(2), char(100), CHAR(100), CHAR(16), CHAR(1), CHAR(1);
...
DEFINE cCodigo_pago      CHAR(2);
DEFINE cDesc_Pago        CHAR(100);
DEFINE cResultado        CHAR(16);
DEFINE cFolio            CHAR(16);
DEFINE cEjecutar         CHAR(1);  
...
ON EXCEPTION SET iSqlErr
    IF iSqlErr != 0 THEN
        LET cCodRet= iSqlErr;
        RETURN cCodRet,cMensaje,cCredito,iImporte,cCodigo_pago,cDesc_Pago,cResultado,cFolio,cEjecutar,cReverso;
    END IF;
END EXCEPTION;

--SET DEBUG FILE TO "/respaldosbd/hectorb/sp_obtenerinforeversionpago.out";
--TRACE ON;
...
SET LOCK MODE TO WAIT 3;
	...
	FOREACH WITH HOLD 
		...
		SELECT num_credito, importe, cod_pag, desc_pag, resultado, folio, ejecutar, reverso
		INTO  cCredito, iImporte, cCodigo_pago, cDesc_Pago, cResultado, cFolio, cEjecutar, cReverso
		FROM "informix".sd_bitacora_pagos
		WHERE folio_grupo = pFolio_grupo

	
		
		RETURN cCodRet,cMensaje,cCredito,iImporte,cCodigo_pago,cDesc_Pago,cResultado,cFolio,cEjecutar,cReverso WITH RESUME;

	END FOREACH
	
	LET iReg = dbinfo("sqlca.sqlerrd2");
	
	IF iReg = 0 THEN
		--NO EXISTEN PAGOS REVERSADOS EN EL RANGO DE FECHA SOLICITADO	
		LET cCodRet = "000002";
		LET cMensaje = "No se encontraron regsitros del folio solicitado";
		RETURN cCodRet,cMensaje,cCredito,iImporte,cCodigo_pago,cDesc_Pago,cResultado,cFolio,cEjecutar,cReverso;
	END IF;
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: OBTIENE INFORMACION DE TODOS LOS REVERSOS DE PAGOS DEL FOLIO GRUPAL SOLICITADO', 
'AUTOR: HECTOR MANUEL BOJORQUEZ RUELAS',
'FECHA: MAYO 2011',
'VERSION: 20110523.1702',
'BD: BDICRED';

CREATE PROCEDURE "informix".sp_generareportepago
(
pOpcion CHAR(1), 
pFech_Ini CHAR(10), 
pFech_Fin CHAR(10)
...
RETURNING  
	...
	CHAR(3) AS COD_PAGO, 
	CHAR(100) AS DESC_PAGO, 
	CHAR(16) AS FOLIO_MOV, 
	DATE AS FECHA_APLIC,
	MONEY(14,2) AS SDO_ANT_REV,
	MONEY(14,2) AS SDO_POST_REV;

--OPCION N = CONSULTA DE PAGOS
--OPCION S = CONSULTA DE REVERSOS
...
DEFINE mSaldo_ante_rev   MONEY(14,2);
DEFINE mSaldo_post_rev   MONEY(14,2); 
...
LET mSaldo_ante_rev   = 0.0;
LET mSaldo_post_rev   = 0.0;
...
BEGIN
    ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
       IF iSqlErr != 0 THEN
          ...
          RETURN cCodRet,cMensajeRet,cFolio_Grupo,cCredito,mImporte,cCodigo_pago,cDesc_Pago,cFolio,dFecha_Aplic,mSaldo_ante_rev,mSaldo_post_rev;
       END IF;
    END EXCEPTION;

--SET DEBUG FILE TO "/respaldosbd/hectorb/sp_generareportepago.out";
--TRACE ON;

SET ISOLATION TO DIRTY READ;
SET LOCK MODE TO WAIT 3;

	IF NVL(pOpcion,"") = "" OR pOpcion NOT IN ("N","S") THEN
		LET cCodRet= "000001";
		LET cMensajeRet = "EL PARAMETRO OPCION NO ES VALIDO";
		RETURN cCodRet,cMensajeRet,cFolio_Grupo,cCredito,mImporte,cCodigo_pago,cDesc_Pago,cFolio,dFecha_Aplic,mSaldo_ante_rev,mSaldo_post_rev;
	END IF
	...
	IF NVL(pFech_Ini,DATE(1)) =  DATE(1) THEN
		LET pFech_Ini = dtFechaHoy;
	END IF;
	
	IF NVL(pFech_Fin,DATE(1)) =  DATE(1) THEN
		LET pFech_Fin = dtFechaHoy;
	END IF
	
	--LET pFech_Ini =  SUBSTR(pFech_Ini,4,2)||"/"||SUBSTR(pFech_Ini,1,2)||"/"||SUBSTR(pFech_Ini,7,4);
	--LET pFech_Fin =  SUBSTR(pFech_Fin,4,2)||"/"||SUBSTR(pFech_Fin,1,2)||"/"||SUBSTR(pFech_Fin,7,4);
	
	
    -- valida que la fecha ini no sea mayor que la fecha fin
    IF pFech_Ini > pFech_Fin THEN
		LET cCodRet= '000003';
	...
	IF pFech_Fin > dtFechaHoy THEN
		LET cCodRet= '000004';
		LET cMensajeRet = "LA FECHA FINAL ES MAYOR A LA FECHA ACTUAL";
		RETURN cCodRet,cMensajeRet,cFolio_Grupo,cCredito,mImporte,cCodigo_pago,cDesc_Pago,cFolio,dFecha_Aplic,mSaldo_ante_rev,mSaldo_post_rev;
	END IF;

	IF pOpcion = 'N' THEN -- OPCION 1 = CONSULTA DE PAGOS
		FOREACH WITH HOLD 
			SELECT folio_grupo, num_credito, importe, cod_pag, desc_pag, folio, fecha_pago
			INTO cFolio_Grupo, cCredito, mImporte, cCodigo_pago, cDesc_Pago, cFolio, dFecha_Aplic
			FROM "informix".sd_bitacora_pagos
			WHERE reverso =  'N' 
			AND fecha_pago BETWEEN pFech_Ini::DATE AND pFech_Fin::DATE
		
			RETURN cCodRet,cMensajeRet,cFolio_Grupo,cCredito,mImporte,cCodigo_pago,cDesc_Pago,cFolio,dFecha_Aplic,mSaldo_ante_rev,mSaldo_post_rev WITH RESUME;
			
		END FOREACH
	ELIF pOpcion = 'S' THEN --OPCION 2 = CONSULTA DE REVERSOS
		FOREACH WITH HOLD 		
			SELECT folio_grupo, num_credito, importe, cod_pag, desc_pag, folio, fecha_reverso, saldo_ante_rev, saldo_post_rev
			INTO cFolio_Grupo, cCredito, mImporte, cCodigo_pago, cDesc_Pago, cFolio, dFecha_Aplic, mSaldo_ante_rev, mSaldo_post_rev
			FROM "informix".sd_bitacora_pagos
			WHERE reverso = 'S'
			AND fecha_reverso BETWEEN pFech_Ini::DATE AND pFech_Fin::DATE
	...
	IF iReg = 0 THEN
		LET cCodRet = "000002";
		LET cMensajeRet = "NO EXISTE INFORMACION EN EL RANGO DE FECHA SOLICITADO";
		RETURN cCodRet,cMensajeRet,cFolio_Grupo,cCredito,mImporte,cCodigo_pago,cDesc_Pago,cFolio,dFecha_Aplic,mSaldo_ante_rev,mSaldo_post_rev;
	END IF;
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: OBTIENE INFORMACION PARA GENERACION DE RPORTE DE PAGOS MASIVOS', 
'AUTOR: HECTOR MANUEL BOJORQUEZ RUELAS',
'FECHA: MAYO 2011',
'VERSION: 20110516.1902',
'BD: BDICRED';
