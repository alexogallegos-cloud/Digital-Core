CREATE PROCEDURE "informix".sp_renuevapagares_comp( pempresa CHAR(3) )
RETURNING CHAR(5), CHAR(5), CHAR(50), INTEGER;
    ...
    DEFINE vmonto_abono     MONEY(14,2);
    DEFINE vmonto_cargo     MONEY(14,2);
    ...
    LET vsdodisp       = 0.00;
    LET vmontoret      = 0.00;
    LET vmonto_abono   = 0.00;
    LET vmonto_cargo   = 0.00;
    
    BEGIN

    ON EXCEPTION SET vsqlerr, visamerr, vdescerr
        SET debug file to "/resplogifx/conciliachq/sp_renuevapagares_comp.err";
        TRACE ON;
        IF vsqlerr <> 0 THEN
    ...
    --- SET debug file to "/resplogifx/conciliachq/sp_renuevapagares_comp.out";
    --- TRACE ON;
    
    SET ISOLATION TO DIRTY READ;
    SET LOCK MODE TO WAIT 5;
    
    IF EXISTS( SELECT dbsname, tabname FROM sysmaster:systabnames WHERE partnum > 0 AND tabname = 'sv_pagaresrenovados_comp') THEN
        DROP TABLE "informix".sv_pagaresrenovados_comp;
    END IF;
    
    CREATE TABLE "informix".sv_pagaresrenovados_comp
      (
    ...
    CREATE INDEX "informix".idx_pagrenov2 ON "informix".sv_pagaresrenovados_comp(cuenta) USING BTREE;
    UPDATE STATISTICS MEDIUM FOR TABLE sv_pagaresrenovados_comp;
    
    IF EXISTS( SELECT dbsname, tabname FROM sysmaster:systabnames WHERE partnum > 0 AND tabname = 'pagaresxrenovar_comp') THEN
        DROP TABLE "informix".pagaresxrenovar_comp;
    END IF;
    
    CREATE TABLE "informix".pagaresxrenovar_comp
      (
        cuenta      char(20)    not null,
        fecha_venc  date        not null,
        cta_cheques char(20)    not null,
        monto       money(18,2) not null,
        mto_abonar  money(18,2) not null,
        mto_cargar  money(18,2) not null
      )
    EXTENT SIZE 32 NEXT SIZE 32 LOCK MODE ROW;
    CREATE INDEX "informix".idx_pagxrenov2 ON "informix".pagaresxrenovar_comp(cuenta) USING BTREE;
      
    LET vsql = 'echo "LOAD FROM /resplogifx/conciliachq/pagares_a_renovar_2.unl DELIMITER ''","'' INSERT INTO pagaresxrenovar_comp" > /resplogifx/conciliachq/pagaresxrenov2.sql';
    SYSTEM vsql;
    LET vsql = '';
    
    LET vstmt = '/ifxsif01/bin/dbaccess bdinvers /resplogifx/conciliachq/pagaresxrenov2.sql';
    SYSTEM vstmt;
    LET vstmt = '';
    
    UPDATE STATISTICS MEDIUM FOR TABLE pagaresxrenovar_comp;
    ...
    FOREACH WITH HOLD
        SELECT pag.cuenta, pag.fecha_venc, pag.cta_cheques, pag.monto, pag.mto_abonar, pag.mto_cargar,
          ...
          INTO vcuenta, vfecha_venc, vcta_cheques, vmonto, vmonto_abono, vmonto_cargo,
        ...
        CALL bdicheq:abono_ref( pempresa,       -- empresa
                                vsuc_chq,       -- sucursal
                                vgusuario,      -- usuario
                                '0242',         -- transaccion
                                '0000',         -- transacc suc
                                ...
                                0,              -- cheque
                                vmonto_abono,   -- monto
                                vmonto_abono,   -- monto firme
                                0,              -- monto sbc
                                0,              -- monto rem
                                0,              -- dias ret
                                '01',           -- divisa
                                'COMPLEMENTO INTERESES PAGARE',  -- referencia
                                ' ',            -- num tarjeta
                                vgusuario )     -- autoriza
        RETURNING vcodret_abono;
        
        IF vcodret_abono = '000' THEN
            CALL bdicheq:cargo_ref( pempresa,       -- empresa
                                    vsuc_chq,       -- sucursal
                                    vgusuario,      -- usuario
                                    '0252',         -- transaccion
                                    '0000',         -- transacc suc
                                    ...
                                    vmonto_cargo,   -- monto
                                    '01',           -- divisa
                                    'COMPLEMENTO ISR PAGARE',  -- referencia
                                    ' ',            -- num tarjeta
                                    vgusuario )     -- autoriza
            ...
            IF vcodret_cargo = '000' THEN
                ...
                INSERT INTO sv_pagaresrenovados_comp VALUES( vctanva, vsecnva, vplazo_nva, vfecha_nva, vtotal, vtasa_nva, vintnet_nva, visr_nvo );
            ELSE
                ROLLBACK WORK;
                LET ven_transacc = 0;
                CONTINUE FOREACH;
