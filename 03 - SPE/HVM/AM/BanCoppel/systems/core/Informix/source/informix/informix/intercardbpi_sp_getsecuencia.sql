CREATE PROCEDURE "informix".sp_getsecuencia( piTipoMovimiento INT)
RETURNING CHAR(6) ;
...
BEGIN
    ...
    IF ( piTipoMovimiento = 22 ) THEN  --PAGOINTERBANCARIO
        LET vsNomSecuencia = 'SecuenciaPagoInterbancario' ;
    ELIF ( (piTipoMovimiento = 20 ) OR ( piTipoMovimiento = 21 ) ) THEN  --CASHBACK  - CASHADVANCE
        LET vsNomSecuencia = 'SecComCashBack' ;
	ELIF ( piTipoMovimiento = 23 ) THEN  --PAGOINTERBANCARIO
		LET vsNomSecuencia = 'SecuenciaMoneyGram' ;
    ELSE
        LET vsNomSecuencia = 'SecuenciaComisionCon' ;
    END IF ;

    SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ ;
    SELECT NVL( secuencia, '1' ) INTO vsSecuencia  FROM Intercard:"informix".SecuenciaInterCard Where NombreSecuencia = vsNomSecuencia ;

    IF (  vsSecuencia  = '999999' ) THEN  -- RESETEA EL VALOR A UNO
        UPDATE Intercard:"informix".SecuenciaInterCard SET secuencia = '1'  Where nombresecuencia = vsNomSecuencia ;
    ELSE  --AUMENTA UNO A LA SECUENCIA
        UPDATE Intercard:"informix".SecuenciaInterCard SET secuencia = secuencia + 1 Where nombresecuencia = vsNomSecuencia ;
    END IF ;
...
'Descripcion: SE MODIFICO LA LOGICA PARA OBTENER LA SECUENCIA DE LAS TRANSACCIONES DE MONEYGRAM.',
'Fecha: 2012/10/01',
'Version: 20121001.1103',
'BD: Intercard';
