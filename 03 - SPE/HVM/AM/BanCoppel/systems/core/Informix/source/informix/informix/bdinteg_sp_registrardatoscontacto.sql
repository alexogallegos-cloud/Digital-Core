CREATE PROCEDURE "informix".sp_registrardatoscontacto(
    cEmpresa			CHAR(3),
    cNumCte				CHAR(20), 
...
RETURNING   CHAR(5) AS cCodRet;
    ...
    BEGIN
        ...
        IF cTelefonoCelular <> '' AND vCodRet = '00000' THEN
            --Telefono celular
            EXECUTE PROCEDURE sp_registra_telefonos(cEmpresa, cNumCte, cTelefonoCelular, 2, '',
                1, iCanal, cUserInsert) INTO vCodRet;
    
            IF vCodRet = '999' OR vCodRet = '000' THEN
                LET vCodRet = '00000';
            END IF;
        END IF;
    
        IF cTelefonoCasa <> '' AND vCodRet = '00000' THEN
            --Telefono de casa
            EXECUTE PROCEDURE sp_registra_telefonos(
                cEmpresa, cNumCte, cTelefonoCasa, 1, '',
                0, iCanal, cUserInsert) 
                INTO vCodRet;
    
            --'Realiza actualizaciÃ³n del telefono'
            IF vCodRet = '999' OR vCodRet = '000' THEN
                LET vCodRet = '00000';
            END IF;
        END IF;
...
'Descripcion: CreaciÃ³n de procedure para el guardado de los datos de telefono y correo electronico, Codigo de retorno 000 OK cualquier otro codigo es un error',
