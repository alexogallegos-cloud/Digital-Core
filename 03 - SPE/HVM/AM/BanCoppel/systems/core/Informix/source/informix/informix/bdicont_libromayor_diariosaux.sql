CREATE PROCEDURE "informix".libromayor_diariosaux(v_empresa CHAR(4), 
                                                   v_fechainicio DATE,
                                                   v_fechafin DATE,
                                                   v_ccmayor CHAR(10),
                                                   v_ccsub CHAR(10),
...
RETURNING VARCHAR(5), VARCHAR(255);
	...
	--SET DEBUG FILE TO "/home/informix/mmeses/libromayaux_diariosaux.out";
	--TRACE ON;
	...
	BEGIN  --INICIO PROGRAMA		
        ...
        ON EXCEPTION
            ...
            IF iSqlErr <> 0 THEN
                ...
                END IF
                IF vb_auxiliares = 'T' THEN
                    DROP TABLE bdicont:tmp_auxiliares;
                END IF
...
-- Inicio Proceso Saldos
        ...
        CREATE INDEX idx01tmp_saldos ON bdicont:tmp_saldos(ccmayor,ccsub,ccsubsub,ccssubsub,ccsssubsub,sector,auxiliar,sucursal,ciudad,mes_dia,moneda);
		...
		FOREACH
            SELECT sucursal,auxiliar,moneda, '001', sum(NVL(saldo_inicio_dia,0)),sum(NVL(saldo_fin_de_dia,0)),mes_dia
			INTO tsucursal_sdo,tauxiliar_sdo,tmoneda_sdo, tciudad_sdo, tsaldo_inicial, tsaldo_final, tmes_dia
			FROM bdicont:"informix".co_diasaux
            ...
            INSERT INTO bdicont:tmp_saldos (ccmayor,ccsub,ccsubsub,ccssubsub,ccsssubsub,sector,auxiliar,sucursal,moneda,ciudad,saldo_inicio_dia,saldo_fin_de_dia,mes_dia,region)
            VALUES (v_ccmayor,v_ccsub,v_ccsubsub,v_ccssubsub,v_ccsssubsub,v_sector,tauxiliar_sdo,tsucursal_sdo,tmoneda_sdo,'001',tsaldo_inicial,tsaldo_final,tmes_dia,'001');
        ...
        SELECT {+INDEX(tmp_saldos idx01tmp_saldos)} DISTINCT auxiliar
        FROM bdicont:tmp_saldos
        WHERE ccmayor = v_ccmayor
            AND ccsub = v_ccsub
            AND ccsubsub = v_ccsubsub
            ...
            AND mes_dia BETWEEN v_fechainicio AND v_fechafin
            AND moneda IS NOT NULL
        INTO TEMP tmp_auxiliares WITH NO LOG;

        CREATE INDEX idxtmp_auxiliares ON tmp_auxiliares (auxiliar);

        UPDATE STATISTICS MEDIUM FOR TABLE tmp_auxiliares;

        LET vb_auxiliares = 'T';

        CREATE TEMP TABLE bdicont:tmp_parametros (auxiliar CHAR(12),
                                                  sucursal CHAR(4),
                                                  ciudad CHAR(3),
                                                  moneda CHAR(2)) WITH NO LOG;

        CREATE INDEX idxtmp_parametros ON tmp_parametros (auxiliar,sucursal,ciudad,moneda);
        ...
        FOREACH
            ...
            FOREACH
                ...
                FOREACH
                    SELECT {+INDEX(tmp_auxiliar idxtmp_auxiliar)} auxiliar INTO tauxiliar_sdo  FROM bdicont:tmp_auxiliares ORDER BY auxiliar                                
                    FOREACH
                        SELECT {+INDEX(tmp_sucursales idxtmp_sucursales)} sucursal INTO tsucursal_sdo  FROM bdicont:tmp_sucursales ORDER BY sucursal

                        INSERT INTO tmp_parametros(auxiliar,sucursal,ciudad,moneda)
                        VALUES(tauxiliar_sdo,tsucursal_sdo,tciudad_sdo,tmoneda_sdo);

                    END FOREACH --auxiliares
        ...
        CREATE INDEX idx01tmp_minmaxfechasaldos ON bdicont:tmp_minmaxfechasaldos(ccmayor,ccsub,ccsubsub,ccssubsub,ccsssubsub,sector,auxiliar,sucursal,ciudad,moneda);
        ...
        FOREACH
            SELECT {+INDEX(tmp_parametros idxtmp_parametros)} auxiliar,sucursal,ciudad,moneda
            INTO tauxiliar_sdo,tsucursal_sdo,tciudad_sdo,tmoneda_sdo
            FROM bdicont:tmp_parametros
            WHERE auxiliar IS NOT NULL
                AND sucursal IS NOT NULL
                AND ciudad IS NOT NULL
                AND moneda IS NOT NULL
			GROUP BY auxiliar,sucursal,ciudad,moneda
            ORDER BY auxiliar,sucursal,ciudad,moneda
            ...
            IF (tmes_dia_min IS NOT NULL) THEN
                INSERT INTO bdicont:tmp_minmaxfechasaldos(ccmayor,ccsub,ccsubsub,ccssubsub,ccsssubsub,sector,auxiliar,sucursal,ciudad,moneda,mes_dia_min,mes_dia_max)
                VALUES(v_ccmayor,v_ccsub,v_ccsubsub,v_ccssubsub,v_ccsssubsub,v_sector,tauxiliar_sdo,tsucursal_sdo,tciudad_sdo,tmoneda_sdo,tmes_dia_min,tmes_dia_max);
            END IF                        
        END FOREACH;
        ...
        FOREACH
            SELECT auxiliar,sucursal,ciudad,moneda
            INTO tauxiliar_sdo,tsucursal_sdo,tciudad_sdo,tmoneda_sdo
        ...
        CREATE INDEX idx01tmp_saldosfinales ON bdicont:tmp_saldosfinales(auxiliar,sucursal,region,cuenta,moneda);
        ...
        FOREACH
            SELECT auxiliar,sucursal,ciudad,moneda,mes_dia_min,mes_dia_max
            INTO tauxiliar_sdo,tsucursal_sdo,tciudad_sdo,tmoneda_sdo,tmes_dia_min,tmes_dia_max
            FROM bdicont:tmp_minmaxfechasaldos
            WHERE 1 = 1
            ...
            INSERT INTO bdicont:tmp_saldosfinales(cuenta,auxiliar,sucursal,region,moneda,saldo_inicial,saldo_final)
            VALUES(v_cuenta,tauxiliar_sdo,tsucursal_sdo,tciudad,tmoneda_sdo,tsaldo_inicial,tsaldo_final);
        ...
        FOREACH WITH HOLD
            SELECT cuenta,auxiliar,sucursal,region,moneda,NVL(saldo_inicial,0),NVL(saldo_final,0)
            INTO tcuenta_sdo,tauxiliar_sdo,tsucursal_sdo,tciudad,tmoneda_sdo,tsaldo_inicial,tsaldo_final
			FROM bdicont:tmp_saldosfinales
			WHERE 1 = 1
            ORDER BY cuenta,auxiliar,sucursal,region,moneda
            ...
            VALUES(v_empresa, tcuenta_sdo, v_ccmayor, v_ccsub, v_ccsubsub,v_ccssubsub, v_ccsssubsub, v_sector, '001', tsucursal_sdo,
					tmoneda_sdo, v_fechainicio, vusuario, 0, 0,tauxiliar_sdo, '', tsaldo_inicial, 0, tsaldo_final,'', v_fechafin, '0000',v_idreporte);
...
-- Fin Proceso Saldos
		...
		FOREACH 
            SELECT auxiliar,sucursal,region,moneda 
            INTO tauxiliar_sdo,tsucursal_sdo,tciudad,tmoneda_sdo
			FROM bdicont:tmp_saldosfinales
			WHERE auxiliar IS NOT NULL
               AND sucursal IS NOT NULL
			...
			WHERE empresa = v_empresa
			  ...
			  AND nro_auxiliar = tauxiliar_sdo;
