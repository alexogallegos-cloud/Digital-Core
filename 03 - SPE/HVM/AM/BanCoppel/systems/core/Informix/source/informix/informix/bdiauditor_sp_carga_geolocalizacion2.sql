CREATE PROCEDURE "informix".sp_carga_geolocalizacion2()
...
BEGIN
    ...
    ON EXCEPTION SET SQL_ERR,ISAM_ERR,ERROR_INFO
		...
		END IF
		LET cod_ret  = SQL_ERR;
        LET vmensaje  = TRIM(ERROR_INFO)||' sp_carga_geolocalizacion2 en el paso '||vpaso;
		...
		VALUES (v_fecha_hoy,'ERROR EN EL PROCESO PARA GENERAR EL: '||TIPO_PLANTILLA_CTA,vconteo,vconteo,0,0,cod_ret||' '||vmensaje,'sp_carga_geolocalizacion2');
	...
	LET TIPO_PLANTILLA_CTA	 = 'CargaGeoLocalizacion2_'||TRIM(cFecha);
	...
	FOREACH WITH HOLD
		...
		WHERE fecha_oper >= v_fecha_menos_uno AND fecha_oper < v_fecha_hoy AND referencia_23 = ''
	...
	VALUES (v_fecha_hoy,vmensaje,vconteo,vconteo,0,0,'sp_carga_geolocalizacion2');
...
CREATE PROCEDURE "informix".sp_carga_geolocalizacion_net()
RETURNING CHAR(5) AS CodRet,
          CHAR(180) AS mensaje;
...
BEGIN
    ...
    ON EXCEPTION SET SQL_ERR,ISAM_ERR,ERROR_INFO
		...
		END IF
		LET cod_ret  = SQL_ERR;
        LET vmensaje  = TRIM(ERROR_INFO)||' sp_carga_geolocalizacion_net en el paso '||vpaso;
		INSERT INTO "informix".tbl_logextraccion_minds(fechaejecucion,proceso,numregistros,regnuevos,regactualizados,regbaja,errorproceso,rutina)
		VALUES (v_fecha_hoy,'ERROR EN EL PROCESO PARA GENERAR EL: '||TIPO_PLANTILLA_CTA,vconteo,vconteo,0,0,cod_ret||' '||vmensaje,'sp_carga_geolocalizacion_net');
        RETURN cod_ret, vmensaje;
    END EXCEPTION;
	
    --SET DEBUG FILE TO "/ifxsif01/c90307913/sp_carga_geolocalizacion_net.out";
    --TRACE ON;
	...
	LET cFecha = cAno||cMes||cDia;
	LET RUTA_DESTINO	 	 = '/RESPALDOSNEW/';
	LET TIPO_PLANTILLA_CTA	 = 'CargaGeoLocalizacion_net_'||TRIM(cFecha);
	...
	WHERE fecha_hr_oper >= v_fecha_menos_uno AND fecha_hr_oper < v_fecha_hoy
	...
	INSERT INTO "informix".tbl_logextraccion_minds(fechaejecucion,proceso,numregistros,regnuevos,regactualizados,regbaja,rutina)
	VALUES (v_fecha_hoy,vmensaje,vconteo,vconteo,0,0,'sp_carga_geolocalizacion_net');
