create procedure "informix".burofisicas_pba()
       returning char(5),
                 char(50);
...
begin;
CREATE INDEX idx_sepomex ON sepomex(d_codigo) ONLINE;
commit;

begin;
CREATE INDEX idx_sepomex1 ON sepomex(d_mnpio) ONLINE;
commit;

begin;
CREATE INDEX idx_sepomex2 ON sepomex(estado_abrev) ONLINE;
commit;

begin;
CREATE INDEX idx_sepomex3 ON sepomex(d_asenta) ONLINE;
commit;

UPDATE STATISTICS high FOR TABLE sepomex;
...
--Se crea tabla temporal con los crÃ©ditos a procesar TDC
    SELECT c.numcte, c.num_producto, c.credito_externo,num_credito,status_cred, c.fecha_apertura,c.campo_trab3
...
--and c.sucursal in ('0192','0249','0547','0157','0379','0179','0075')
    UNION ALL
    SELECT c.numcte, c.num_producto, mae.credito_externo,c.num_credito,mae.status_cred, mae.fecha_apertura,c.campo_trab3
      FROM bdicred:sd_maecredcont c
...
--Se anexa a tabla temporal con los crÃ©ditos a procesar REESTRUCTURAS
    ...
    SELECT c.numcte, c.num_producto, c.credito_externo,c.num_credito,c.status_cred, c.fecha_apertura,c.campo_trab3
...
--and c.sucursal in ('0192','0249','0547','0157','0379','0179','0075')
    ...
    INSERT INTO creditos
    SELECT c.numcte, c.num_producto, c.credito_externo,c.num_credito,mae.status_cred, mae.fecha_apertura,c.campo_trab3
      FROM bdicred:sd_maecredcontcrd c
...
--IPCB 06082014/Integra Cancelaciones aperturadas el mismo mes.
	INSERT INTO creditos
    SELECT a.numcte, a.num_producto,  '' credito_externo,b.num_credito, a.status_cred, a.fecha_apertura,a.campo_trab3
     ...
     --WHERE a.status_cred = 'FF' RQM 09 343-0
	...
	INSERT INTO creditos	   
	SELECT a.numcte, a.num_producto,  '' credito_externo,a.num_credito, status_cred, fecha_apertura,a.campo_trab3
...
-- crea temporal pago de reestructuras -- reemplazar con indicadores de credito
    ...
    foreach with hold
        select numcte, num_producto, credito_externo, num_credito, status_cred, fecha_apertura,campo_trab3
          into vnumcte,cNumProducto, cCredExterno,vnum_credito,vstatus_cred, vfecha_apertura,vidbaja
          from creditos 
         ...
         SELECT limit 1 Trim(f.nombrecalle)||' '||Trim(a.numeroextcalle)||' '||
          ...
          WHERE a.numcte=vnumcte 
            ...
            SELECT first 1 count(*), d_mnpio, d_codigo  INTO iCP, vlMnpioReportar, vlCodigoReportar
              ...
              FROM sepomex WHERE d_codigo = vcod_postal AND d_mnpio = vdelegacion AND estado_abrev = vestado
            ...
            IF nvl(iCP,0) <= 0 THEN
             SELECT first 1 count(*), d_mnpio, d_codigo INTO iCP, vlMnpioReportar, vlCodigoReportar
              ...
              FROM sepomex WHERE  d_mnpio = vdelegacion AND estado_abrev = vestado
             group by d_mnpio, d_codigo ;    
            ...
            IF nvl(iCP,0) <= 0 THEN
             SELECT first 1 count(*), d_mnpio, d_codigo  INTO iCP, vlMnpioReportar, vlCodigoReportar
              ...
              FROM sepomex WHERE  d_asenta = vcolonia AND estado_abrev = vestado
              group by d_mnpio, d_codigo ;  
            ...
            IF nvl(iCP,0) <= 0 THEN
             SELECT first 1 count(*), d_mnpio, d_codigo  INTO iCP, vlMnpioReportar, vlCodigoReportar
              ...
              FROM sepomex WHERE  d_codigo = vlCodigoPOstalZona AND estado_abrev = vestado
             group by d_mnpio, d_codigo ;   
...
-- Claves de observacion de cierre el saldo es CERO
        ...
        -- if (vstatus_cred = 'FF' or vstatus_cred = 'FC' or vstatus_cred = 'CV' ) then RQM 09 343-0
...
--IPCB Abr15- Se modifica la validación para integrar a la cinta los registros marcados con CSS en la concilia .
       if bmotivo = 1 then
           INSERT INTO bdiburo:br_burofisicas_concilia VALUES('001',cNumProducto,vnum_credito,'CSS',vfecha_hoy,
                    ...
                    tb_saldo_venc, tb_cuotas_ven, tb_mop,tb_clave_obs, vrea_cal_cuota,vstatus_cred,tb_monto_insoluto,tb_num_credito_ext);
       else
...
--IPCB 12mar14 - Se integran al insert vstatus_cred,cNumProducto
			insert into br_burofisicas_describe
			   values (tb_num_credito, tb_apell_paterno, tb_apell_materno, tb_nombre1, tb_nombre2, tb_fecha_nac, tb_rfc, tb_nacionalidad, tb_estado_civil, tb_sexo,
					   ...
					   tb_clave_usu, tb_nombre_usu, tb_responsabilidad, tb_tipo_cuenta, tb_tipo_producto, tb_clave_monetaria, tb_num_pagos, tb_frecpago, tb_monto_pagar,
					   tb_fecha_apertura, tb_fecha_ult_pago, tb_fecha_ult_compra, tb_fecha_cierre, tb_fecha_reporte, tb_credito_maximo, tb_saldo_actual, tb_monto_otorgado,
					   tb_saldo_venc, tb_cuotas_ven, tb_mop,tb_clave_obs, vrea_cal_cuota,tb_fecha_vencimiento,tb_monto_insoluto,tb_ultimo_pago,vstatus_cred,cNumProducto,tb_num_credito_ext);
--                       tb_saldo_venc, tb_cuotas_ven, tb_mop,tb_clave_obs, vrea_cal_cuota);
  ...
  BEGIN;  CREATE INDEX "informix".inxburoconcilia             ON "informix".br_burofisicas_concilia(num_producto) ONLINE;  COMMIT;
  BEGIN;  CREATE INDEX "informix".inxburoconcilia1            ON "informix".br_burofisicas_concilia(num_credito) ONLINE;  COMMIT;
  BEGIN;  CREATE INDEX "informix".inxburoconcilia2            ON "informix".br_burofisicas_concilia(motivo) ONLINE;  COMMIT;
  BEGIN;  CREATE INDEX "informix".inxburoconcilia3            ON "informix".br_burofisicas_concilia(fecha_cinta) ONLINE;  COMMIT;
