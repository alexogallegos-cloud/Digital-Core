create procedure "informix".edo_ctarang(pempresa char(3),
                                        cta char(20),
                                        num_movto smallint,
                                        fecha1 date,
                                        fecha2 date)
   returning char(5), date, char(40), integer, money(14,2),
             money(14,2), money(14,2), char(51), char(1),
             char(20), money(14,2), char(40), char(35), char(50);
   ...
   define v_moneda     char(35);
   define v_plaza      char(2);
   define v_divisa     char(2);
   define v_despro     char(50);
      ...
      on exception set sql_err
         if sql_err <> 0 then
            ...
            return v_cod_ret, v_fecha, v_transacc, v_docto, v_monto,
                   ...
                   v_cliente, v_saldo_disp, v_referencia, v_moneda, v_despro;
         end if
      end exception;
   ...
   select sdo_actual, sdo_mes_ant, envio_direcc, numcte, apell_paterno,
          ...
          (sdo_actual-sdo_retenido-sdo_cong), producto, plaza
   ...
   SELECT divisa, nombre INTO v_divisa, v_despro
      FROM sc_producto
      WHERE empresa = pempresa and producto = v_producto;

   SELECT descripcion INTO v_moneda FROM bdinteg:si_divisas
   ...
   foreach
      ...
      where mm.empresa = pempresa and cuenta = cta and cancelad <> "V" and
            fech_alt between fecha1 and fecha2 and
   ...
   foreach
      ...
      where md.empresa = pempresa and md.cuenta = cta and
            tr.empresa = md.empresa and tr.numero = md.transacc
            and cancelad != "V" and se_emite_edocta = "S" and
            fech_alt between fecha1 and fecha2
