create procedure "informix".chqcaj1(i_empresa    char(3),
                              i_tipmov           char(4),
			      i_tipo_docto       char(2),
			      i_usuario          char(8),
...
-- ############################################################################
   select codigo_mn into v_codigo_mn from bdinteg:si_param;
   ...
   select ejecutivo,nombre,sucursal into v_clave_cajero,v_nomeje,v_sucursal
   ...
   select plaza,iva into v_plaza,v_iva from bdinteg:si_sucursales
...
if i_tipmov = "ALTA" then
   if i_no_cheque is null or i_no_cheque = " " then
      -- Asignacion automatica de numero
   else
      -- Verifica que NO exista aun.
      select count(*) into v_existe
         from bditrans:st_maetrans
         where num_docto = i_no_cheque and tipo_docto = v_cve_c_caja;
      if v_existe > 0  then
         let o_codret = "007";
         return o_comision,o_iva,o_total_com,o_stts,o_fecha,o_codret,o_no_cheque;
      end if
   ...
   if i_no_cheque is null or i_no_cheque = " " then
      foreach
         execute procedure stnumdocto(i_sucursal,i_tipo_docto)
  	    into o_codret,v_num_act_suc,o_no_cheque
      end foreach;
   ...
   foreach
      execute procedure stmovdia(v_plaza,i_sucursal,i_usuario,
				i_fecha_horaexp,v_cve_en_transito,i_moneda,
				v_cve_c_caja,i_no_cheque,i_folio,"")
  	 into o_codret
   end foreach;
   if o_codret != "000" then
      return o_comision,o_iva,o_total_com,o_stts,o_fecha,o_codret,o_no_cheque;
  ...
  if i_cta_cgo_abono is null then
   ...
   insert into bditrans:st_maetrans
      values (v_cve_c_caja,i_no_cheque,i_sucursal,i_moneda,i_unidades_divisa,i_usuario,
	      i_fecha_horaexp," "," "," "," "," "," ",0,0,0,0," ",0,i_monto_mn,
	      i_beneficiario," "," "," "," ",i_nombre_comprador,i_telsolic,i_domsolic,
	      i_mto_efec,i_mto_efec_c_div,i_mto_efec_c_val,i_cta_cgo_abono,
	      i_mto_cgo_abono,i_tipo_comision,o_comision,i_porc_cob_com,0,o_total_com,
	      v_cve_en_transito," ");
...
if i_tipmov = "PAGO" then
   ...
   select rowid,moneda,unidades_divisa,monto,beneficiario,
      ...
      from bditrans:st_maetrans
      where tipo_docto = i_tipo_docto and
   ...
   if i_moneda != v_codigo_mn then
      ...
      if i_unidades_divisa = 0 or i_unidades_divisa is null or i_unidades_divisa = " " then
	 let o_codret = "034";
	 return o_comision,o_iva,o_total_com,o_stts,o_fecha,o_codret,o_no_cheque;
      end if
   ...
   update bditrans:st_maetrans
      set (sucursal_pagadora,cajero_paga,mto_pagado_mn,status_docto,fecha_hora_pago)
