CREATE PROCEDURE "informix".sp_recuperardatoscomplementarios(
    cEmpresa CHAR(3),
    cNumcte CHAR(20))

RETURNING   CHAR(5)  AS cCodRet,
            CHAR(2)  AS cEstadoCivil,
            CHAR(3)  AS cPais,
            CHAR(3)  AS cNacionalidad,
            CHAR(2)  AS cLugarNacimiento,
            CHAR(1)  AS cSexo,
            CHAR(2)  AS cCodigoIdentificacion,
            CHAR(30) AS cNumeroIdentificacion,
            CHAR(18) AS cNoFm23,
            CHAR(20) AS cPromocion,
    ...
    DEFINE vLugarNacimiento         CHAR(2);
    DEFINE vSexo                    CHAR(1);
    DEFINE vCondigoIdentificacion   CHAR(2);
    DEFINE vNumeroIdentificacion    CHAR(30);
    DEFINE vNoFm23                  CHAR(18);
    DEFINE vPromocion               CHAR(20);
    ...
    LET vPais = '';
    LET vNacionalidad = '';
    LET vLugarNacimiento = '';
    LET vSexo = '';
    LET vCondigoIdentificacion = '';
    LET vNumeroIdentificacion = '';
    LET vNoFm23 = '';
    LET vPromocion = '';
    LET vTipoPersona = '';
    ...
    BEGIN
        ...
        ON EXCEPTION
            ...
            IF iSqlErr <> 0 THEN
                    LET vCodRet = iSqlErr;
                    RETURN
                        vCodRet, vEstadoCivil, vPais, vNacionalidad, vLugarNacimiento, vSexo,
                        vCondigoIdentificacion, vNumeroIdentificacion, vNoFm23, vPromocion, vTipoPersona;
            END IF;
        END EXCEPTION;

        --SET DEBUG FILE TO "/tmp/anj/recuperarDatosComplementarios.sql";
        --TRACE ON;
        ...
        IF vEstadoCivil IS NULL OR len(vEstadoCivil) = 0 THEN
            SELECT numcte_pros
            ...
            ELSE
                SELECT
                    pf.estado_civil, pf.sexo, pf.nacionalidad, pf.lugar_nac, pf.no_fm3,
                    cte.string1, pf.codidentifi, pf.numidentifi, case when vTipoPersona = "" then cte.tipo_cliente else vTipoPersona end
                INTO
                    vEstadoCivil,
                    vSexo,
                    vNacionalidad,
                ...
                INNER JOIN bdiprospectos:"informix".pr_ctepf pf ON pf.numcte_pros = cte.numcte_pros
                WHERE cte.numcte_pros = vNumCteProspecto;
            END IF
        END IF
		
		IF vNacionalidad IS NOT NULL AND vNacionalidad <> '' THEN
			LET vNacionalidad = LPAD(TRIM(vNacionalidad), 3, '0');
		END IF

        RETURN
            vCodRet, NVL(vEstadoCivil, ''), NVL(vPais, ''), NVL(vNacionalidad, ''), NVL(vLugarNacimiento, ''), NVL(vSexo, ''),
            NVL(vCondigoIdentificacion, ''), NVL(vNumeroIdentificacion, ''), NVL(vNoFm23, ''), NVL(vPromocion, ''), NVL(vTipoPersona, '');
    END;
...
'Descripcion: CreaciÃ³n de procedure para la recuperaciÃ³n de datos complementarios con base en numero de cliente',
