...
'DESCRIPCION: SPL que actualiza  repositorio y dias de depuracion',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_actualizaprocesoconau(pUsuario CHAR(8), pIdFuncion CHAR(10), pNombreArchivo CHAR(30), pProceso CHAR(1))
		RETURNING CHAR(5) AS codret;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_actualizaprocesoconau.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pNombreArchivo = '' OR pProceso = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF;
		...
		EXECUTE PROCEDURE bditarjeta:'informix'.sp_concreing_actualizaproceso(pProceso, pNombreArchivo, pUsuario)
		INTO cCodRetSp;
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bditarjeta:sp_concreing_actualizaproceso';
		END IF;
...
'DESCRIPCION: Actualiza el archivo para ser procesado en la conciliaciÃ³n',
'BD: bdicnweb';
...
CREATE PROCEDURE "informix".sp_cargarchextemporaneosccl(pUsuario CHAR(8), pIdFuncion CHAR(10),pTipoOpcion INTEGER, pNombreArch CHAR(50))
	...
	BEGIN
		...
		FOREACH
			...
			LET iCodRetSp = cCodRetSp::INTEGER;
			IF iCodRetSp < 0 THEN
				RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bditarjeta:sp_concreing_consultaextemporaneo';
			END IF;
			
			IF pTipoOpcion NOT IN(1,2,3)  AND iCodRetSp = 1  THEN 
				LET cCodRet = '00044';
				RETURN cCodRet,cArchivoOrigen,cPrefijoArchivo,cDiasDesfase,cRepAIX;
			END IF;
			IF pTipoOpcion IN (2,3) AND (pNombreArch IS NULL OR pNombreArch = '')  AND iCodRetSp = 1  THEN 
				LET cCodRet = '00003';
				RETURN cCodRet,cArchivoOrigen,cPrefijoArchivo,cDiasDesfase,cRepAIX;
			END IF;
			IF pTipoOpcion = 2 AND iCodRetSp = 1  THEN 
				LET cCodRet = '00537';
				RETURN cCodRet,cArchivoOrigen,cPrefijoArchivo,cDiasDesfase,cRepAIX;
			END IF;
			...
			RETURN cCodRet,cArchivoOrigen,cPrefijoArchivo,cDiasDesfase,cRepAIX WITH RESUME;
		END FOREACH;
...
'DESCRIPCION: SPL que consulta el catalogo de operaciones para proceso de integracion de archivos extemporaneos ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_catbitacoraconciliacion(pUsuario CHAR(8), pIdFuncion CHAR(10))
		RETURNING CHAR(5) AS codret,
			INTEGER AS id_elemento,
			CHAR (35) AS nombre_elemento;

	DEFINE cCodRet CHAR(5);
	DEFINE iSqlErr INTEGER;
	DEFINE iElemente INTEGER;
	DEFINE cNombreElemento CHAR (35);
	DEFINE iNoRegistros INTEGER;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, iElemente, cNombreElemento;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_catbitacoraconciliacion.out';
		--TRACE ON;
		...
		FOREACH SELECT elemento, nombre_elemento 
				INTO iElemente, cNombreElemento
				FROM bditarjeta:"informix".td_elemento
			LET iNoRegistros = iNoRegistros + 1;
			RETURN cCodRet, iElemente, UPPER(cNombreElemento)  WITH RESUME;		
		END FOREACH;
...
'FECHA: 20/08/2015',
'MODULO: Conciliaciones ',
'FUNCIONALIDAD: Bitácora de Conciliación ',
'DESCRIPCION: SPL que consulta el detalle de combo para la consulta de Bitácora de Conciliación. ',
'BD: bdicnweb';
...
'DESCRIPCION: SPL para la consulta del catálogo de información en Devoluciones POS',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consadmincorresponsalccl(pUsuario CHAR(8), pIdFuncion CHAR(10),pArchivoOrigen CHAR(3), pFecha DATE, pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
	...
	DEFINE cTtranCentral		CHAR(4);
	DEFINE vFolio325			CHAR(16);
	DEFINE mMonto325			MONEY(16,6);
	DEFINE cEstatus				CHAR(1);
	DEFINE cTxnLiberacion		CHAR(4);
	DEFINE cCuentac				CHAR(19);
	DEFINE cCuentaa				CHAR(19);
	DEFINE cFolioSif			CHAR(16);
	DEFINE mMontoSif			MONEY(16,6);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,  cArchivoOrigen, cNomArchivo325, cNomArchivoCom, dFechaRegistro, dFecha, cProdTarjeta, cTarjeta, cCuenta, cTipoMov, cTtranCentral, vFolio325, mMonto325, cEstatus, cTxnLiberacion, cCuentac, cCuentaa, cFolioSif, mMontoSif, cUsuario;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consadmincorresponsalccl.out';
		--TRACE ON;
		...
		FOREACH   
			EXECUTE PROCEDURE intercard:'informix'.sp_consultaconadmincorr2(pArchivoOrigen, pFecha, pRegistros, pRecuperacion)
			INTO  cArchivoOrigen, cNomArchivo325, cNomArchivoCom, dFechaRegistro, dFecha, cProdTarjeta, cTarjeta, cCuenta, cTipoMov, cTtranCentral, vFolio325, mMonto325, cEstatus, cTxnLiberacion, cCuentac, cCuentaa, cFolioSif, mMontoSif, cUsuario
			...
			END IF;
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet,  UPPER (cArchivoOrigen), UPPER (cNomArchivo325), UPPER(cNomArchivoCom), dFechaRegistro, dFecha, cProdTarjeta, cTarjeta, cCuenta, UPPER (cTipoMov), cTtranCentral, vFolio325, mMonto325, UPPER(cEstatus), cTxnLiberacion, cCuentac, cCuentaa, cFolioSif, mMontoSif, cUsuario WITH RESUME;	
		END FOREACH;
...
'DESCRIPCION: Consulta el detalle de la administracion de corresponsales, obtiene informacion de tabla conadmin.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consadmincorresponsalccl_totales(pUsuario CHAR(8), pIdFuncion CHAR(10), pArchivoOrigen CHAR(3), pFecha DATE)	
		RETURNING CHAR(5) AS codret,		
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consadmincorresponsalccl_totales.out';
		--TRACE ON;
		...
		EXECUTE PROCEDURE intercard:'informix'.sp_consultaconadmincorr2_totales(pArchivoOrigen, pFecha)
		INTO cCodRetSp,iNumRegistros;
		
		IF cCodRetSp::INTEGER < 0 THEN 
			RAISE EXCEPTION cCodRetSp::INTEGER, 0, 'ERROR EN LA EJECUCIÓN DEL SP intercard:sp_consultaconadmincorr2_totales';
		END IF;
...
CREATE PROCEDURE "informix".sp_consadmincorresponsalesccl(pUsuario CHAR(8), pIdFuncion CHAR(10),pArchivoOrigen CHAR(3), pFecha DATE, pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
		...
		MONEY(18,2)	AS id_tipo_operacion;		
	...
	LET mMonto = 0.0;
	LET mComision = 0.0;
	LET mComisionIva = 0.0;
	LET mId_Tipo_Operacion = 0.0;
	LET iNoRegistros = 0;
	LET iRegistros = 0;
	LET iRecuperacion = 0;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cIdTerminal, dFechaMov, mMonto, mComision, mComisionIva, mId_Tipo_Operacion;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consadmincorresponsalesccl.out';
		--TRACE ON;
		...
		FOREACH 
			EXECUTE PROCEDURE intercard:'informix'.sp_concorresponsales2(pArchivoOrigen, pFecha, pRegistros, pRecuperacion)
			INTO cIdTerminal, dFechaMov, mMonto, mComision, mComisionIva, mId_Tipo_Operacion
			
			LET iCodRetSp = cCodRetSp::INTEGER;
			IF iCodRetSp < 0 THEN
				RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP intercard:sp_concorresponsales';
			END IF;
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet, cIdTerminal, dFechaMov, mMonto, mComision, mComisionIva, mId_Tipo_Operacion WITH RESUME;	
		END FOREACH;
...
'MODULO: CONCILIACIONES',
'FUNCIONALIDAD: CONCILIACIÓN ADMINISTRATIVA',
'DESCRIPCION: Consulta el detalle de administracion de corresponsales,  obtiene informacion de tabla conarchcomisiones',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consadmincorresponsalesccl_totales(pUsuario CHAR(8), pIdFuncion CHAR(10), pArchivoOrigen CHAR(3), pFecha DATE)	
		RETURNING CHAR(5) AS codret,		
			INTEGER AS num_registros;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consadmincorresponsalesccl_totales.out';
		--TRACE ON;
		...
		EXECUTE PROCEDURE intercard:'informix'.sp_concorresponsales2_totales(pArchivoOrigen, pFecha)
		INTO cCodRetSp,iNumRegistros;
		
		IF cCodRetSp::INTEGER < 0 THEN 
			RAISE EXCEPTION cCodRetSp::INTEGER, 0, 'ERROR EN LA EJECUCIÓN DEL SP intercard:sp_concorresponsales2_totales';
		END IF;
...
'MODULO: CONCILIACIONES',
'FUNCIONALIDAD: ',
'DESCRIPCION: Consulta los totales de administracion de corresponsales,  obtiene informacion de tabla conarchcomisiones',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consadminusuarioccl(pUsuario CHAR(8), pIdFuncion CHAR(10), pClaveUsuario CHAR (10))
		RETURNING CHAR(5) AS codret,
		CHAR(90) AS nombre_colaborador;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consadminusuarioccl.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pClaveUsuario = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cNombre;
		END IF;
		...
		EXECUTE PROCEDURE bditarjeta:'informix'.sp_concreing_consultausuario(pClaveUsuario)
		INTO cCodRetSp, cMensaje, cNombre;
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bditarjeta:sp_concreing_consultausuario';
...
'DESCRIPCION: SPL para la administracion de usuario y sus perfiles',
'BD: bdicnweb';
