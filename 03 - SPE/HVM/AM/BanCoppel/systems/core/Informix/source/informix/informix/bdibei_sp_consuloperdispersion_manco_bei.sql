CREATE PROCEDURE "informix".sp_consuloperdispersion_manco_bei(pIdOperacion INTEGER)
RETURNING CHAR(5), CHAR(10), CHAR(20), CHAR(17), CHAR(10), CHAR(20),
        CHAR(3), CHAR(4), INTEGER, INTEGER, INTEGER, INTEGER,INTEGER,    
        SMALLINT, SMALLINT,MONEY(14,2),INTEGER,INTEGER,INTEGER, DECIMAL(18,2),
...
BEGIN
    ...
    END EXCEPTION ;
     ...
     SELECT to_char(f_aplicacion,"%iY-%m-%d") as f_aplicacion, cuenta_origen, archivos.nombre_archivo, to_char(f_dispersion,"%iY-%m-%d") as f_dispersion,
			...
			cantidadEmpleados, archivos.concepto, tipoDispersion, cargoDispersion, totalSinComision,
    ...
    FROM "informix".bei_operacionesmancomunadasoperador AS operacionesmancomunadasoperador
        INNER JOIN "informix".bei_archivos AS archivos 
        ON operacionesmancomunadasoperador.nombre_archivo = archivos.nombre_archivo
    WHERE operacionesmancomunadasoperador.ID_OPERACION = pIdOperacion;
