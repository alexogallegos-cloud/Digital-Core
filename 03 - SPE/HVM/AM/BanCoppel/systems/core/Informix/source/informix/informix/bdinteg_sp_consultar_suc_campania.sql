CREATE PROCEDURE "informix".sp_consultar_suc_campania(pEmpresa CHAR(3),pIdCamp SMALLINT, pIdJerarquia SMALLINT)
...
--DECLARACION DE VARIABLES
	...
	--SET DEBUG FILE TO "/respaldosbd/isarai/sp_consultar_suc_campania.out";
	--SET DEBUG FILE TO '/informix/PRISCILLA/sp_consultar_suc_campania.out';
	--TRACE ON;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			...
			IF iSqlErr <> 0 THEN
				LET cCodRet = iSqlErr;			
				RETURN TRIM(NVL(cCodRet,'')),TRIM(NVL(cNumSuc,'')),TRIM(NVL(cNombre,'')),TRIM(NVL(cCiudad,'')),
						TRIM(NVL(cEstado,''));
			END IF;
		...
		IF TRIM(NVL(pEmpresa,'')) <> '' AND NVL(pIdCamp,0) <> 0 AND NVL(pIdJerarquia,0) <> 0 THEN
			FOREACH 
				SELECT DISTINCT DECODE(sucursal,'T','T',sucursal),idzONa
				INTO cNumSuc, cZONa
				...
				LET cNumSuc = TRIM(NVL(cNumSuc,''));
				LET cZONa = TRIM(NVL(cZONa,''));
				
				IF NVL(cNumSuc,'') = TRIM('T') THEN
					LET cCodRet = '000002';		
					LET cNumSuc = '';
					LET cNombre = '';
					LET cCiudad = '';
				...
				ELIF NVL(cNumSuc,'') = '' THEN
				
					FOREACH
						
						SELECT DISTINCT p.id_ptf,a.nombre,NVL(l.desc_municipio,''),d.nombre 
						INTO cNumSuc,cNombre,cCiudad,cEstado
						FROM "informix".si_ptf p
						INNER JOIN "informix".si_sucursales a ON (p.id_ptf = a.sucursal AND p.tipo = a.tipo )
						INNER JOIN "informix".si_estados b ON p.cve_estado = b.estado 
						LEFT OUTER JOIN bdinteg:si_localidades l ON ( l.id > 0 AND 
                                                          l.cp = l.cp AND
                                                          l.cve_estado = p.cve_estado AND 
                                                          l.cve_mun = p.cve_mun AND
                                                          l.cve_localidad_cnbv = p.cve_localidad AND 
                                                          l.cve_col = p.cve_col )
						INNER JOIN "informix".si_plazas d ON a.plaza = d.plaza
						WHERE d.plaza = TRIM(NVL(cZONa,''))
						AND a.empresa = TRIM(NVL(pEmpresa,''))
						AND p.tipo <> 'C'
						AND a.tpo_sucursal = 'S'
						ORDER BY p.id_ptf ASC

						/*SELECT DISTINCT a.sucursal,a.nombre,c.nombre,d.nombre 
						INTO cNumSuc,cNombre,cCiudad,cEstado
						FROM "informix".si_sucursales a 
						INNER JOIN "informix".si_estados b ON a.estado = b.estado 
						INNER JOIN "informix".si_ciudades c ON a.ciudad = c.ciudad AND a.estado = c.estado
						INNER JOIN "informix".si_plazas d ON a.plaza = d.plaza
						INNER JOIN "informix".si_ciudades e ON e.estado = b.estado
						WHERE d.plaza = TRIM(NVL(cZONa,''))
						AND a.empresa = TRIM(NVL(pEmpresa,''))
						AND a.tpo_sucursal = 'S'
				...
				ELIF NVL(cNumSuc,'') <> '' THEN
					...
					FOREACH
						...
						LEFT OUTER JOIN bdinteg:si_localidades l ON ( l.id > 0 AND 
                                                          ...
                                                          l.cve_col = p.cve_col )
						INNER JOIN "informix".si_plazas d ON a.plaza = d.plaza
						WHERE p.id_ptf = TRIM(NVL(cNumSuc,''))
						AND a.empresa = TRIM(NVL(pEmpresa,''))
						AND p.tipo <> 'C'
						AND a.tpo_sucursal = 'S'
						ORDER BY p.id_ptf ASC
		...
		ELSE
			LET cCodRet = '000001';
			LET cNumSuc = '';	
			RETURN TRIM(NVL(cCodRet,'')),TRIM(NVL(cNumSuc,'')),TRIM(NVL(cNombre,'')),TRIM(NVL(cCiudad,'')),TRIM(NVL(cEstado,''));
		END IF;
...
'DESCRIPCION: PROCEDIMIENTO PARA OBTENER LAS SUCURSALES DE UNA CAMPANIA',
