CREATE PROCEDURE "informix".sp_medicionreestructura(pMes CHAR(2), pAnio CHAR(4))
...
DATE         AS fecha,
CHAR(4)      AS sucursal,
INTEGER      AS ctes_candidatos,
INTEGER      AS ctes_reestructurados ;

------DECLARACION DE VARIABLES------
DEFINE iSql_err,isam_err    INTEGER;
DEFINE iDia_valido          INTEGER;
DEFINE cSucursal            CHAR(4);
DEFINE iCantidad1           INTEGER;
DEFINE iCantidad2           INTEGER;
DEFINE cCodret,vvcCod_ret   CHAR(6);
DEFINE dFechaComparacion    DATE;
DEFINE cVencidos,cProceso   CHAR(4);
DEFINE cAdeudoLimite        CHAR(10);
DEFINE dFechaSdoDiario      DATE;
DEFINE dFechaMaecrd         DATE;
DEFINE dFechaMaxMaecrd      DATE;
...
LET dFechaComparacion       = DATE(1);
LET cVencidos               = '';
LET cAdeudoLimite           = '';
LET dFechaSdoDiario         = DATE(1);
LET dFechaMaecrd            = DATE(1);
LET dFechaMaxMaecrd         = DATE(1);
LET v_empresa               = '001';
LET vvcCod_ret              = '';
LET cProceso                = '0081';
...
BEGIN
	...
	ON EXCEPTION SET iSql_err, isam_err 
		IF iSql_err <> 0 THEN
		   --SI HAY UN ERROR NO CONTROLADO BORRA LAS TABLAS
			IF EXISTS (SELECT tabname FROM sysmaster:systabnames WHERE tabname ='paso1' AND dbsname= "bdicred") THEN
				DROP TABLE paso1;
			END IF;	

			IF EXISTS (SELECT tabname FROM sysmaster:systabnames WHERE tabname ='res_candidatas' AND dbsname= "bdicred") THEN
				DROP TABLE res_candidatas;
			END IF;	
				
			IF EXISTS (	SELECT tabname FROM sysmaster:systabnames WHERE tabname ='res_realizadas' AND dbsname= "bdicred") THEN
				DROP TABLE res_realizadas;
			END IF;	
      LET cCodret = iSql_err;
      
      LET cMensaje = 'Error: ' || isam_err;
     CALL bdicobranza:"informix".sp_inserta_bitacora_cob(v_empresa, cProceso, cCodret, cMensaje, '02') RETURNING vvcCod_ret;
			
			RETURN TRIM(cCodret), NVL(dFechaComparacion, DATE(1)), NVL(cSucursal,''), NVL(iCantidad1,0), NVL(iCantidad2,0); 
		END IF;
	END EXCEPTION;
		
	--SET DEBUG FILE TO '/dbexportb/carlos/cobranza/sp_medicionreestructura.out';
	--SET DEBUG FILE TO '/ifxsif01/joel/Modificados/sp_medicionreestructura.trc';
	--TRACE ON;
	
  	CALL bdicobranza:"informix".sp_inserta_bitacora_cob(v_empresa, cProceso, cCodret, cMensaje, '01') RETURNING vvcCod_ret;
	...
	IF NVL(pMes,'') = '' OR NVL(pAnio,'') = '' THEN
		LET cCodret = '000001';  --NO SE PROPORCIONARON PARAMETROS PARA EL PROCEDIMIENTO
		LET cMensaje = 'Error: No se proporcionaron parametros.';
    CALL bdicobranza:"informix".sp_inserta_bitacora_cob(v_empresa, cProceso, cCodret, cMensaje, '02') RETURNING vvcCod_ret;
		RETURN TRIM(cCodret), NVL(dFechaComparacion, DATE(1)), NVL(cSucursal,''), NVL(iCantidad1,0), NVL(iCantidad2,0); 
	END IF;
		
	--**************************************************************************************
	--****************  SE VERIFICA QUE EL DIA QUE SE EJECUTANDO EL SP     *****************
	SELECT DAY(fecha_hoy)::INTEGER
	INTO iDia_valido
	FROM "informix".sd_fechas
	WHERE empresa = v_empresa;
	
	--LET iDia_valido = 20;  --- TEST MACF
	

		-- LET dFechaComparacion = MDY(TRIM(pMes),'20',TRIM(pAnio));     MODIF MACF		
    LET dFechaComparacion = MDY(pMes,'20',pAnio);
    
		--SI EXISTEN SE ELIMINAN TABLAS TEMPORALES.
		IF EXISTS (SELECT tabname FROM sysmaster:systabnames WHERE tabname ='paso1' AND dbsname= "bdicred") THEN
			DROP TABLE paso1;
		END IF;	
		...
		LET cVencidos = cVencidos::DECIMAL;
	
		
		--SELECCIONA LOS NUMEROS DE CREDITO CON SU MORATORIO
		SELECT {+AVOID_FULL (bdinteg:si_sucursales)} a.num_credito, ((a.sdo_moratorio + a.sdo_contab_mora) * (1 + s.iva)) mora
		FROM bdicred:"informix".sd_maesdoshist a , bdicred:"informix".sd_maecred b, bdinteg:si_sucursales s
		WHERE a.empresa = v_empresa
		AND a.num_credito = b.num_credito
		AND a.empresa = s.empresa
		AND b.sucursal = s.sucursal
		AND a.fecha = dFechaComparacion
		AND a.mto_fin_ven_trasp >= cVencidos
		INTO temp paso1 WITH NO LOG; --SE CREA TABLA TEMPORAL

		CREATE UNIQUE INDEX inx_paso1 on paso1 (num_credito); --SE CREA INDICE PARA LA TABLA TEMPORAL
		UPDATE STATISTICS LOW FOR TABLE paso1; --SE ORDENA LA TABLA TEMPORAL

		--SE OBTIENE POR SUCURSAL LA CANTIDAD DE CREDITOS QUE TIENE UN ADEUDO MAYOR O IGUAL A 2000
		--LA FECHA SERA SIEMPRE EL PRIMER DIA DEL MES EN CURSO
		LET dFechaSdoDiario = MDY(TRIM(pMes),'01',TRIM(pAnio));	
		LET cAdeudoLimite = cAdeudoLimite::DECIMAL;
		
		SELECT b.sucursal, COUNT(*) cantidad
		FROM paso1 a, "informix".sd_sdodiario b
		WHERE a.num_credito = b.num_credito
		AND b.fecha = dFechaSdoDiario
		AND (capvig20 + captrans20 + capvencnoexig20 + capvenexig20 +intvig20 + intvenc20 + ivaintvig20 + ivaintvenc20 + mora >= cAdeudoLimite)
		GROUP BY 1
		INTO temp res_candidatas WITH NO LOG;
		
		CREATE UNIQUE INDEX inx_res_candidatas on res_candidatas (sucursal);
		UPDATE STATISTICS LOW FOR TABLE res_candidatas;

		LET dFechaMaecrd = (dFechaComparacion + 1 UNITS DAY);
		LET dFechaMaxMaecrd = (dFechaComparacion + 1 UNITS MONTH);
		
		
		SELECT sucursal, COUNT(*) AS cantidad
		FROM "informix".sd_maecredcrd
		WHERE empresa = v_empresa 
		AND numcte = numcte --PARA ACTIVAR INDICE
		AND num_producto = '6011'
		AND fecha_apertura BETWEEN dFechaMaecrd AND dFechaMaxMaecrd
		GROUP BY sucursal 
		INTO temp res_realizadas WITH NO LOG;

		CREATE UNIQUE INDEX inx_res_realizadas on res_realizadas(sucursal);
		UPDATE STATISTICS LOW FOR TABLE res_realizadas;

		FOREACH
			SELECT a.sucursal, NVL(b.cantidad,0), NVL(c.cantidad,0)
			INTO cSucursal , iCantidad1 , iCantidad2
			FROM bdinteg:"informix".si_sucursales a
			LEFT OUTER JOIN res_candidatas b ON (a.sucursal = b.sucursal)
			LEFT OUTER JOIN res_realizadas c ON (a.sucursal = c.sucursal)
			WHERE a.tpo_sucursal = 'S' 
			ORDER BY a.sucursal
            
			
			INSERT INTO "informix".sd_medicion_reestructura (fecha, sucursal, ctes_candidatos, ctes_reestructurados)
			VALUES(dFechaComparacion, cSucursal, iCantidad1, iCantidad2 );
						
			--RETURN TRIM(cCodret), NVL(dFechaComparacion, DATE(1)) , NVL(cSucursal,''), NVL(iCantidad1,0), NVL(iCantidad2,0)  WITH RESUME; --REGRESA LOS VALORES QUE INSERTO EN LA TABLA   MODIF BY MACF
		END FOREACH;
				
		--YA SEA QUE ENCUENTRE INFORMACION O NO SE BORRAN LAS TABLAS TEMPORALES CREADAS
		IF DBINFO('sqlca.sqlerrd2') = 0 THEN
			LET cCodret = '000003'; --LA BUSQUEDA NO ARROJO INFORMACION
			DROP TABLE paso1; 
			DROP TABLE res_candidatas;
			DROP TABLE res_realizadas;
			
			LET cMensaje = 'Error: La busqueda no arrojo informacion.';
      CALL bdicobranza:"informix".sp_inserta_bitacora_cob(v_empresa, cProceso, cCodret, cMensaje, '02') RETURNING vvcCod_ret;
    
			RETURN TRIM(cCodret), DATE(1), NVL(cSucursal,''), NVL(iCantidad1,0), NVL(iCantidad2,0); 
		ELSE 
			UPDATE STATISTICS LOW FOR TABLE sd_medicion_reestructura;
		END IF	
	...
	IF cCodret <> '000000'THEN
		RETURN TRIM(cCodret), NVL(dFechaComparacion, DATE(1)), NVL(cSucursal,''), NVL(iCantidad1,0), NVL(iCantidad2,0);
  ELSE
    CALL bdicobranza:"informix".sp_inserta_bitacora_cob(v_empresa, cProceso, cCodret, cMensaje, '03')
...
END PROCEDURE
DOCUMENT
'DESCRIPCIÓN: GENERA UN RESUMEN DE CADA SUCURSAL RESPECTO A REESTRUCTURAS CADA FECHA DE CORTE (DIA 20 DE', '             CADA MES) Y LA INSERTA EN LA TABLA sd_medicion_reestructura DE LA BDICRED, REGRESANDO LOS', '             VALORES QUE INSERTO EN LA TABLA',
'FECHA DE CREACIÓN: 21-FEBRERO-2013',
'BASE DE DATOS: BDICRED',
'CREADOR: CARLOS OCHOA VALENZUELA',
'VERSION: 201321021535';

CREATE PROCEDURE "informix".sp_conssdoedos2(cEmpresa CHAR(3),CNumCredito CHAR(20),pultreg SMALLINT)
