	...
	BEGIN WORK;
		-->> FORECH para afectaciÃ³n de movimientos.
			...
			FOREACH WITH hold
				...
				WHERE folio_csuac = pFolioSuac AND 
                                      ( ( (procede IS NULL OR procede = 1) AND (exitoso IS NULL OR exitoso <> '1') AND NVL(fky_padre,0) = CASE WHEN ( pDictamen IN ('AA','AS')) THEN 0 ELSE NVL(fky_padre,0) END)
				OR  ( (procede IS NULL OR procede = 1) AND (exitoso IS NULL OR exitoso <> '1') AND fky_tipo_movimiento = 332 ) --> Abono de intereses
                                      )
...
'FECHA : 20/Abril/2025',
'VERSION: 1.0.0',
'BD    :  Se mejora el SELECT del 2o FOREACH';
