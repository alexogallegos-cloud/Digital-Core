CREATE PROCEDURE "informix".sp_reportebts_diario ( pdtPeriodo DATE )
...
BEGIN
	...
	ELSE 
		...
		FOREACH SELECT 
			...
			--(DECODE(MOV.Forma_Pago, '1', 'EFECTIVO', DECODE(MOV.Forma_Pago, '2', 'CARGO EN CUENTA', DECODE(MOV.Forma_Pago, '3', 'MIXTO', DECODE(MOV.Forma_Pago, '4', 'ABONO CTA', MOV.Forma_Pago))))) AS Forma_pago, 
			...
			INTO vdtDia, vsNum_confirmacion, viImporte, vsForma_pago, vsFolio_op, vsSucursal, vsCajero, vsNom_benef
			...
			RETURN	vdtDia, vsNum_confirmacion, viImporte, vsForma_pago, vsFolio_op, vsSucursal, vsCajero, vsNom_benef WITH RESUME;
