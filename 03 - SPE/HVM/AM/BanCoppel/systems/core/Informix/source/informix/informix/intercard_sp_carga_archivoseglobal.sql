CREATE PROCEDURE "informix".sp_carga_archivoseglobal(pNomArchivo char(25), pArchivoOrigen char(3), pModoConc integer, pNumReg integer)
        returning char(6);

--****************************************************************************************************
-- DESCRIPCION: CARGA LOS ARCHIVOS PROPORCIONADOS POR EGLOBAL, PROSA Y  TIENDAS COPPEL A LAS TABLAS CORRESPONDIENTES 
-- PARA SER PROCESADOS POR LA CONCILIACION
-- AUTOR : Mauricio Leon
-- FECHA : 01/07/2008
-- BD: Intercard
...
--Definicion de variables
    ...
    DEFINE vRegistroAtm char(326);
    DEFINE vRegistroPos char(326);
...
BEGIN
    ...
    END EXCEPTION;
     ...
     LET vsql = 'echo "LOAD FROM ''/tmp/conciliacion/' || TRIM(pNomArchivo) || ''' DELIMITER ' || '''|''' || ' INSERT INTO archivos_conciliacion" > /tmp/conciliacion/cargaarchivoseglobal.sql';
     SYSTEM vsql;

    LET vsql = '';

     LET vsql = 'dbaccess intercard /tmp/conciliacion/cargaarchivoseglobal.sql';
     SYSTEM vsql;

     SET LOCK MODE TO WAIT  ;
     SET ISOLATION TO DIRTY READ ;
     SELECT COUNT(registro) INTO vNumReg FROM archivos_conciliacion;

     IF ( vNumReg = pNumReg ) THEN

         IF ( pModoConc = 1 ) THEN --ATM

            DELETE FROM conciliacion_atm_in where archivoorigen = pArchivoOrigen;

			IF ( pArchivoOrigen = 'TMC' ) OR ( pArchivoOrigen = 'TMD' )THEN -- ATM
				
				LET vsMonto = '' ;
				
				FOREACH WITH HOLD
				SELECT Registro INTO vRegistroAtm FROM archivos_conciliacion
					
					LET vsMonto = '' ; --EGLOBAL 183
					LET vsMonto = TRIM(SUBSTRING (vRegistroAtm FROM 183 FOR 8 )) || '.' || TRIM(SUBSTRING (vRegistroAtm FROM 191 FOR 2));  --MONTO
					...
					(
						CURRENT,
						pArchivoOrigen,
						TRIM(SUBSTRING (vRegistroAtm FROM 10 FOR 6 )), --SECUENCIAAUTH 
						TRIM(SUBSTRING (vRegistroAtm FROM 25 FOR 16 )),  --NUMTARJETA
						TRIM(SUBSTRING (vRegistroAtm FROM 150 FOR 8 )),  --FECHA
						TRIM(SUBSTRING (vRegistroAtm FROM 159 FOR 8 )),  --HORA
						'F', --CONCILIADO
						TRIM(SUBSTRING (vRegistroAtm FROM 3 FOR 4 )),  --ADQUIRIENTE
						TRIM(SUBSTRING (vRegistroAtm FROM 48 FOR 20 )),  --NUMCUENTA
						TRIM(SUBSTRING (vRegistroAtm FROM 91 FOR 15 )),  --DESCRIPCION
						TRIM(SUBSTRING (vRegistroAtm FROM 70 FOR 19 )),  --INDICADORDEREVERSA
						TRIM(SUBSTRING (vRegistroAtm FROM 115 FOR 3 )),  --CODIGOISO
						TRIM(SUBSTRING (vRegistroAtm FROM 121 FOR 12 )),  --SECUENCIACAJERO
						TRIM(vsMonto),  --MONTO
						TRIM(SUBSTRING (vRegistroAtm FROM 136 FOR 14 )), -- NUMCAJERO     14  PRO 10
						--NUEVO
						TRIM(SUBSTRING (vRegistroAtm FROM 109 FOR 6 )), -- RESP
						TRIM(SUBSTRING (vRegistroAtm FROM 170 FOR 6 )), -- ORDEN
						TRIM(SUBSTRING (vRegistroAtm FROM 176 FOR 7 )), -- RED
						TRIM(SUBSTRING (vRegistroAtm FROM 193 FOR 7 )), -- DOLARES
						'', -- NUMAUTORIZACION
						'', -- CODPAIS
						'', -- MONTOORIGEN
						'', -- CODMONEDA
						--TRIM(SUBSTRING (vRegistroAtm FROM 200 FOR 10 )), -- MONTOSURCHARGE
						TRIM(SUBSTRING (vRegistroAtm FROM 200 FOR 8 )) || '.' || TRIM(SUBSTRING (vRegistroAtm FROM 208 FOR 2)),  --MONTOSURCHARGE
						0.0, -- DONATIVO
						'', -- EMPRESA
						'', -- COMPAÑIA
						--TRIM(SUBSTRING (vRegistroAtm FROM 210 FOR 10 )), -- MONTO_LOYALTYFEE
						TRIM(SUBSTRING (vRegistroAtm FROM 210 FOR 8 )) || '.' || TRIM(SUBSTRING (vRegistroAtm FROM 218 FOR 2)),  --MONTO_LOYALTYFEE
						0.0, -- MONTO_USOLINEA
						TRIM(SUBSTRING (vRegistroAtm FROM 220 FOR 20 )), -- NOMBANCOEMISOR
						TRIM(SUBSTRING (vRegistroAtm FROM 240 FOR 1 )) -- BANDERAADQUIRIENTE
					);
					
				END FOREACH;
			
			ELIF ( pArchivoOrigen = 'TMP' ) THEN --ATM  PROIOS
				...
				SELECT Registro INTO vRegistroAtm FROM archivos_conciliacion
					...
					--ATM  PROIOS  --prosa  181
					LET vsMonto = TRIM(SUBSTRING (vRegistroAtm FROM 181 FOR 10 )) ;  --MONTO
					...
					(
						...
						TRIM(SUBSTRING (vRegistroAtm FROM 121 FOR 12 )),  --SECUENCIACAJERO
						TRIM(vsMonto),  --MONTO
						TRIM(SUBSTRING (vRegistroAtm FROM 136 FOR 10 )), -- NUMCAJERO     10  EG 14
						--NUEVO
						TRIM(SUBSTRING (vRegistroAtm FROM 109 FOR 2 )), -- RESP
						TRIM(SUBSTRING (vRegistroAtm FROM 170 FOR 4 )), -- ORDEN
						TRIM(SUBSTRING (vRegistroAtm FROM 176 FOR 4 )), -- RED
						TRIM(SUBSTRING (vRegistroAtm FROM 192 FOR 7 )), -- DOLARES
						TRIM(SUBSTRING (vRegistroAtm FROM 201 FOR 6 )), -- NUMAUTORIZACION
						TRIM(SUBSTRING (vRegistroAtm FROM 209 FOR 2 )), -- CODPAIS
						TRIM(SUBSTRING (vRegistroAtm FROM 213 FOR 13 )), -- MONTOORIGEN
						TRIM(SUBSTRING (vRegistroAtm FROM 228 FOR 3 )), -- CODMONEDA
						TRIM(SUBSTRING (vRegistroAtm FROM 233 FOR 10 )), -- MONTOSURCHARGE
						TRIM(SUBSTRING (vRegistroAtm FROM 245 FOR 10 )), -- DONATIVO
						TRIM(SUBSTRING (vRegistroAtm FROM 257 FOR 4 )), -- EMPRESA
						TRIM(SUBSTRING (vRegistroAtm FROM 262 FOR 10 )), -- COMPAÑIA
						TRIM(SUBSTRING (vRegistroAtm FROM 273 FOR 10 )), -- MONTO_LOYALTYFEE
						TRIM(SUBSTRING (vRegistroAtm FROM 284 FOR 10 )), -- MONTO_USOLINEA
						'', -- NOMBANCOEMISOR
						'' -- BANDERAADQUIRIENTE
					);
        ...
        ELIF ( pModoConc = 2 ) THEN --POS

            DELETE FROM conciliacion_pos_in where archivoorigen = pArchivoOrigen;

            SET LOCK MODE TO WAIT  ;
            SET ISOLATION TO DIRTY READ ;
            FOREACH SELECT registro INTO vRegistroPos FROM archivos_conciliacion

                 INSERT INTO intercard:conciliacion_pos_in(fechaconciliacion, archivoorigen, secuenciaauth, numtarjeta, fecha, GiroNegocio, tipotransaccion, monto, montocashback, idcomercio, nomcomercio, reftransaccion, rfc, fuenteid, moneda, Divisa, MontoDivisa, FormadePago, conciliado)
                    VALUES (
                        CURRENT,
                        pArchivoOrigen,                        
                        TRIM(SUBSTRING (vRegistroPos FROM 210 FOR 6 )),  --SECUENCIAAUTH
                        TRIM(SUBSTRING (vRegistroPos FROM 5 FOR 16 )),  --NUMTARJETA
                        TRIM(SUBSTRING (vRegistroPos FROM 234 FOR 6 )), --FECHA
                        TRIM(SUBSTRING (vRegistroPos FROM 166 FOR 4 )), --GIRONEGOCIO
                        TRIM(SUBSTRING (vRegistroPos FROM 37 FOR 2 )),  --TIPOTRANSACCION
                        TRIM(SUBSTRING (vRegistroPos FROM 39 FOR 11 )) || '.' || TRIM(SUBSTRING (vRegistroPos FROM 50 FOR 2)), --MONTO
                        TRIM(SUBSTRING (vRegistroPos FROM 52 FOR 11 )) || '.' || TRIM(SUBSTRING (vRegistroPos FROM 63 FOR 2)),  --MONTOCASHBACK
                        TRIM(SUBSTRING (vRegistroPos FROM 95 FOR 9 )),  --IDCOMERCIO
                        TRIM(SUBSTRING (vRegistroPos FROM 104 FOR 30 )),  --NOMCOMERCIO
                        TRIM(SUBSTRING (vRegistroPos FROM 142 FOR 23 )),  --REFTRANSACCION
                        TRIM(SUBSTRING (vRegistroPos FROM 216 FOR 16 )),  --RFC
                        TRIM(SUBSTRING (vRegistroPos FROM 250 FOR 3 )),  --FUENTEID
                        '01', --MONEDA
                        TRIM(SUBSTRING (vRegistroPos FROM 309 FOR 3 )), --DIVISA
                        TRIM(SUBSTRING (vRegistroPos FROM 292 FOR 9 )) || '.' || TRIM(SUBSTRING (vRegistroPos FROM 301 FOR 2)),  --MONTODIVISA
                        TRIM(SUBSTRING (vRegistroPos FROM 78 FOR 1 )), --FORMADEPAGO
                        'F');  --CONCILIADO

             END FOREACH;

        ELIF ( pModoConc = 3 ) THEN --ATM OTROS ( STAT 06 )

			SET LOCK MODE TO WAIT  ;
			SET ISOLATION TO DIRTY READ ;
			FOREACH SELECT registro INTO vRegistroAtm FROM archivos_conciliacion
				...
				VALUES (
					...
					pArchivoOrigen,
					TRIM(pNomArchivo),
					TRIM(SUBSTRING (vRegistroAtm FROM 3 FOR 4 )), --EMISOR
					TRIM(SUBSTRING (vRegistroAtm FROM 25 FOR 12 )), --NUMCANERO
					TRIM(SUBSTRING (vRegistroAtm FROM 37 FOR 16 )), -- NUMTARJETA
					TRIM(SUBSTRING (vRegistroAtm FROM 60 FOR 20 )), --NUMCUENTA
					TRIM(SUBSTRING (vRegistroAtm FROM 82 FOR 19 )), --INDICADORDEREVERSA
					TRIM(SUBSTRING (vRegistroAtm FROM 103 FOR 15 )), --DESCRIPCION
					TRIM(SUBSTRING (vRegistroAtm FROM 121 FOR 6 )), --RESPUESTA
					TRIM(SUBSTRING (vRegistroAtm FROM 127 FOR 2 )), --CODIGOISO
					TRIM(SUBSTRING (vRegistroAtm FROM 133 FOR 12 )), --SECUENCIA
					TRIM(SUBSTRING (vRegistroAtm FROM 150 FOR 8 )), --FECHA
					TRIM(SUBSTRING (vRegistroAtm FROM 159 FOR 8 )), --HORA
					TRIM(SUBSTRING (vRegistroAtm FROM 170 FOR 6 )), --ORDEN
					TRIM(SUBSTRING (vRegistroAtm FROM 176 FOR 4 )), --RED 
					TRIM(SUBSTRING (vRegistroAtm FROM 181 FOR 10 )),  --MONTO
					TRIM(SUBSTRING (vRegistroAtm FROM 192 FOR 7 )),  --DOLARES
					--NUEVO
					TRIM(SUBSTRING (vRegistroAtm FROM 200 FOR 10 )),  --COMISIONSURCHARGE
					TRIM(SUBSTRING (vRegistroAtm FROM 211 FOR 10 )),  --DONATIVO
					TRIM(SUBSTRING (vRegistroAtm FROM 222 FOR 4 )),  --EMP
					TRIM(SUBSTRING (vRegistroAtm FROM 227 FOR 6 )),  --AUTORIZACION
					TRIM(SUBSTRING (vRegistroAtm FROM 234 FOR 10 )), --COMPAÑIA
					TRIM(SUBSTRING (vRegistroAtm FROM 245 FOR 10 )),  --COMISION_LOYALTYFEE
					TRIM(SUBSTRING (vRegistroAtm FROM 256 FOR 10 ))  --COMISION_USOLINEA
					);
    ...
    ELSE --DIFERENCIA ENTRE REGISTROS REPORTADOS 

        LET chrcodret = '002';

    END IF ;

    DELETE FROM archivos_conciliacion;
...
'Descripcion: CARGA LOS ARCHIVOS PROPORCIONADOS POR EGLOBAL, PROSA Y  TIENDAS COPPEL A LAS TABLAS CORRESPONDIENTES PARA SER PROCESADOS POR LA CONCILIACION.',
'Fecha: 2008/02/01',
'Version: 20080201.1025',
'BD: Intercard',
'',
