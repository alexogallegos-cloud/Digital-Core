CREATE PROCEDURE "informix".sp_ejecuta_reporte_increm_auto () 
	RETURNING
	CHAR(6) AS retornofinal,
	CHAR(6) AS Retorno01,
	CHAR(6) AS Retorno02,
	CHAR(6) AS Retorno03,
	CHAR(6) AS Retorno04;
	...
	DEFINE cCodRet01				CHAR(6);
	DEFINE cCodRet02				CHAR(6);
	DEFINE cCodRet03				CHAR(6);
	DEFINE cCodRet04                CHAR(6);
	DEFINE dFechaHoy                DATE;
	
	LET cCodRet ='000000';
	LET vsqlerr = 0;
	LET cCodRet01='000000';
	LET cCodRet02='000000';
	LET cCodRet03='000000';
	LET cCodRet04='000000';
	LET dFechaHoy=DATE(1);
	
	--SET DEBUG FILE TO "/RESPALDOS/noapli.out";
	--TRACE ON;	
	...
	EXECUTE PROCEDURE "informix".sp_rep_men_increm_auto_noapli(dFechaHoy,'1')
	INTO cCodRet01;
	
	EXECUTE PROCEDURE "informix".sp_rep_men_increm_auto_acepynoacep(dFechaHoy,'1')
	INTO cCodRet02;
			
	EXECUTE PROCEDURE "informix".sp_rep_men_increm_auto_apli(dFechaHoy,'1')
	INTO cCodRet03;				

	EXECUTE PROCEDURE "informix".sp_rep_men_increm_auto_cartven(dFechaHoy,'1')
	INTO cCodRet04;
	
	IF cCodRet01 <> '000000' THEN 
		LET cCodRet= cCodRet01;
	ELIF cCodRet02 <> '000000' THEN 
		LET cCodRet= cCodRet02;
	ELIF cCodRet03 <> '000000' THEN
		LET cCodRet= cCodRet03;
	ELIF cCodRet04 <> '000000' THEN
		LET cCodRet= cCodRet04;
	END IF;
	
	END
		RETURN cCodRet,cCodRet01,cCodRet02,cCodRet03,cCodRet04;	
END PROCEDURE
DOCUMENT
'DESCRIPCION: Realiza la ejecución de los procedimientos que generan la reportería de incrementos de línea en automatico',
'AUTOR: Maria Elena Angulo',
'FECHA: julio de 2013',
'BASE DE DATOS: bdicred';

CREATE PROCEDURE "informix".sp_rep_men_increm_auto_acepynoacep(pFecha DATE, pModo CHAR(1)) 
	RETURNING
	CHAR(6) AS retorno;
	...
	DEFINE dtFecha                  DATE;
	DEFINE iRegistrosAutorizados    INTEGER;
	DEFINE dAcumuladoAutorizado     DECIMAL(18,2);
	DEFINE iRegistrosNoAutorizados  INTEGER;
	DEFINE dAcumuladoNoAutorizado   DECIMAL(18,2);
	DEFINE cCodRet                  CHAR(6);
	...
	LET cSql                        = '';
	LET dtFecha                     = DATE(1);
	LET iRegistrosAutorizados       = 0;
	LET dAcumuladoAutorizado        = 0.00;
	LET iRegistrosNoAutorizados     = 0;
	LET dAcumuladoNoAutorizado      = 0.00;
	LET cCodRet                     = '000000';
	LET iSqlErr                     = 0;
	LET iAnio                       = 0;
	LET cMes                        = '';
	...
	--SET DEBUG FILE TO "/informix/jesus/accepynoacep.out";
	--TRACE ON;
	...
	BEGIN
		...
		--ACTUALIZA LAS ESTADÍSTICAS DE OPTIMIZACIÓN DE CONSULTA PARA LA TABLA DE TRABAJO "SD_REP_MEN_INC_AUTO_ACEP_NOACEP".
		UPDATE STATISTICS MEDIUM FOR TABLE "informix".sd_rep_men_inc_auto_acep_noacep;	
		...
		IF pModo = 1 THEN 
			...
			DELETE "informix".sd_rep_men_inc_auto_acep_noacep WHERE (MONTH(fecha)||YEAR(fecha)) <> (MONTH(dtFechaMonthAdd)||YEAR(dtFechaMonthAdd)) AND modalidad = pModo;		
		ELSE 
		...
		FOREACH WITH HOLD	
			SELECT SUM(CASE WHEN a.ajuste_de_cuota =  'S'  THEN 1 ELSE  0 END ) AS RegistrosAutorizados, 
				  SUM(CASE WHEN a.ajuste_de_cuota <> 'S' THEN 1 ELSE  0 END ) AS RegistrosNoAutorizados,
				  SUM(CASE WHEN a.ajuste_de_cuota =  'S'  THEN b.monto_otorgado ELSE 0 END) AS AcumLineaAutoriz,
				  SUM(CASE WHEN a.ajuste_de_cuota <> 'S'  THEN b.monto_otorgado ELSE 0 END) AS AcumLineaNoAutoriz,
				  c.fecha_apertura 
			INTO iRegistrosAutorizados, iRegistrosNoAutorizados, dAcumuladoAutorizado,dAcumuladoNoAutorizado, dtFecha	  
			FROM "informix".sd_maecred c 
				INNER JOIN "informix".sd_maesdos b ON (b.empresa = c.empresa AND b.num_credito = c.num_credito) 
				INNER JOIN bdisolic:"informix".ss_solicitudes a ON(a.empresa = c.empresa AND a.num_solicitud= c.num_credito ) 
			WHERE c.fecha_apertura  BETWEEN dtPrimerDiaMes AND dtUltimoDiaMes 
			GROUP BY  c.fecha_apertura
			ORDER BY c.fecha_apertura ASC
			
			IF iRegistrosAutorizados > 0 OR  iRegistrosNoAutorizados > 0 THEN
				IF NOT EXISTS (SELECT {+INDEX("informix".sd_rep_men_inc_auto_acep_noacep idx_rep_acep)}  fecha FROM "informix".sd_rep_men_inc_auto_acep_noacep WHERE fecha = dtFecha AND modalidad = pModo ) THEN 
					...
					INSERT INTO "informix".sd_rep_men_inc_auto_acep_noacep (fecha, reg_aut, acum_lin_Aut, reg_no_aut, acum_lin_no_Aut, modalidad) 
					VALUES (dtFecha, NVL(iRegistrosAutorizados,0),NVL(dAcumuladoAutorizado,0.00) ,iRegistrosNoAutorizados, NVL(dAcumuladoNoAutorizado,0.00),pModo);
				...
				ELSE
					...
					UPDATE "informix".sd_rep_men_inc_auto_acep_noacep SET reg_aut = (reg_aut + iRegistrosAutorizados), acum_lin_Aut = (acum_lin_Aut + dAcumuladoAutorizado), reg_no_aut = (reg_no_aut + iRegistrosNoAutorizados), acum_lin_no_Aut = (acum_lin_no_Aut + dAcumuladoNoAutorizado) WHERE fecha = dtFecha AND modalidad = pModo;
					--SE INICIALIZAN LAS VARIABLES.
					LET dtFecha = DATE(1);
					LET iRegistrosAutorizados = 0;
					LET dAcumuladoAutorizado =  0.00; 
				...
				END IF;
			END IF;	
			--SI YA SE CUMPLEN LAS TRANSACCIONES SE CIERRA LA TRANSACCION Y SE INICIALIZA EL CONTADOR.
		...
		--SE HACE UPDATE STATISTICS ANTES DE LA CONSULTA,PARA ACTUALIZAR LAS ESTADÍSTICAS DE OPTIMIZACIÓN DE CONSULTA PARA LA TABLA DE TRABAJO "SD_REP_MEN_INC_AUTO_ACEP_NOACEP".
		UPDATE STATISTICS MEDIUM FOR TABLE "informix".sd_rep_men_inc_auto_acep_noacep;	
		
		--VACIADO A ARCHIVO TXT.
		IF pModo= '1' THEN
			LET cSql = 'echo "Fecha|Registros Autorizados|Acumulado de Línea Autorizada|Registros NO Autorizados|Acumulado de Línea Autorizada" >' || TRIM(cRuta) || '/ReporteAceptacionyrechazohead_'||TRIM(cTitulo)||'.txt';
			LET cSql = TRIM(cSql);
			SYSTEM cSql;

			LET cSql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO "'||TRIM(cRuta)||'"/ReporteAceptacionyrechazobody_"'||TRIM(cTitulo)||'".txt   SELECT {+INDEX("informix".sd_rep_men_inc_auto_acep_noacep idx_rep_acep)} fecha, reg_aut, acum_lin_aut, reg_no_aut, acum_lin_no_aut FROM sd_rep_men_inc_auto_acep_noacep WHERE fecha = fecha AND modalidad = "'||TRIM(pModo)||'";">  "'||TRIM(cRuta)||'"/body.sql'; 
			SYSTEM cSql;  

			LET cSql= 'dbaccess bdicred  "'||TRIM(cRuta)||'"/body.sql';
			SYSTEM cSql; 
			
			LET cSql = '';
			LET cSql = 'cat '||TRIM(cRuta)||'/ReporteAceptacionyrechazohead_'||TRIM(cTitulo)||'.txt '||TRIM(cRuta)||'/ReporteAceptacionyrechazobody_'||TRIM(cTitulo)||'.txt > '||TRIM(cRuta)||'/ReporteAceptacionyrechazo_'||TRIM(cTitulo)||'.txt';		
			LET cSql = TRIM(cSql);
			SYSTEM cSql;
			
			--COMANDO REMOVE.
			LET cSql= 'rm "'||TRIM(cRuta)||'"/ReporteAceptacionyrechazohead_'||TRIM(cTitulo)||'.txt';		
			SYSTEM cSql; 
			
			LET cSql= 'rm "'||TRIM(cRuta)||'"/ReporteAceptacionyrechazobody_'||TRIM(cTitulo)||'.txt';		
			SYSTEM cSql; 
			
			LET cSql= 'rm "'||TRIM(cRuta)||'"/body.sql';		
			SYSTEM cSql; 

			--SE BORRA TODA LA INFORMACION DE LA TABLA DE TRABAJO POR EL FILTRO DE MODALIDAD.
			DELETE {+INDEX("informix".sd_rep_men_inc_auto_acep_noacep idx_rep_acep)} "informix".sd_rep_men_inc_auto_acep_noacep WHERE fecha = fecha AND modalidad = pModo;		
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: procedimiento que se puede ejecutar de modo mensual (1) o historico (2) para calculo de los incrementos de linea aceptados y no aceptados',
'AUTOR: Carlos Ochoa',
'FECHA: junio de 2013',
'BASE DE DATOS: bdicred';
