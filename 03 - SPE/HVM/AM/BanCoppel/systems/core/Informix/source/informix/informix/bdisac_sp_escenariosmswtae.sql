CREATE PROCEDURE "informix".sp_escenariosmswtae
	(
		pNumCategoria CHAR   (2),  pNumConvenio CHAR (3), pId_Sucursal CHAR (4), 
...
RETURNING 
   ...
   -- DeclaraciÃ³n de variables 
	...
	DEFINE sRespInteract	CHAR  (5);
	DEFINE sStatus			CHAR (40);
	DEFINE sNum_clave		CHAR (40);
	DEFINE sClv_aprobacion	CHAR (40);
	DEFINE iStatus			INTEGER;
	DEFINE iNum_clave		INTEGER;
	DEFINE iClv_aprobacion	INTEGER;
   ...
   BEGIN
	...
	--SET DEBUG FILE TO '/respaldosbd/antoniocebreros/69/sp_escenariosmswtae.out';
	--TRACE ON;	
	...
	SET LOCK MODE TO WAIT 3;  
		...
		SELECT TRIM(campo1), TRIM(campo2), TRIM(campo4), TRIM(campo21) 
		INTO sRespInteract, sStatus, sNum_clave, sClv_aprobacion
		FROM  bdisac: "informix".sac_msw_respuesta  
		WHERE numcategoria= pNumCategoria AND numconvenio = pNumConvenio AND folio_suc = pFolioSucursal AND num_trama = pNumTrama;
			
		IF pNumTrama = 1 THEN
			IF sRespInteract = '00000' THEN			
				LET iStatus = CAST(sStatus AS INT);
				LET iNum_clave = CAST(sNum_clave AS INT);
				LET iClv_aprobacion = CAST(sClv_aprobacion AS INT);
				
				SELECT LIMIT 1 
				CASE iStatus
					WHEN 1 THEN '00001' --LET cCodigoRespuesta = '00001';					
					WHEN 0 THEN 
					CASE iNum_clave
						WHEN 301 THEN 
							CASE iClv_aprobacion 
								WHEN 0 THEN '00002' --LET cCodigoRespuesta = '00002';
								ELSE        '00003' --LET cCodigoRespuesta = '00003';
							END
						WHEN 313 THEN '00004'  --LET cCodigoRespuesta = '00004';
						WHEN 304 THEN '00005'  --LET cCodigoRespuesta = '00005';
						WHEN 314 THEN '00006'  --LET cCodigoRespuesta = '00006';
						ELSE '00009'  		   --LET cCodigoRespuesta = '00009';								
					END 
				ELSE  '00009'                  --LET cCodigoRespuesta = '00009';
				END 	
				INTO cCodigoRespuesta
				FROM bdisac: "informix".sac_msw_respuesta;					
			ELSE
				LET cCodigoRespuesta = '00001';				
			END IF;
			LET cCodRet = '00000';		
		ELSE
			IF pNumTrama = 2 THEN
				IF sRespInteract = '00000' THEN			
					...
					SELECT LIMIT 1
					CASE iStatus
						WHEN 1 THEN '00006'--LET cCodigoRespuesta = '00006';						
						WHEN 0 THEN
							CASE iNum_clave
								WHEN 304 THEN '00006' --LET cCodigoRespuesta = '00006'; 
								WHEN 312 THEN '00007' --LET cCodigoRespuesta = '00007'; 
								WHEN 301 THEN '00008' --LET cCodigoRespuesta = '00008';
							ELSE '00010'		      --LET cCodigoRespuesta = '00010';
							END
						ELSE  '00010'		          --LET cCodigoRespuesta = '00010';
					END 
					INTO cCodigoRespuesta
...
'DESCRIPCION: SP harÃ¡ la validaciÃ³n de los escenarios de acuerdo a los campos desglosados y devolverÃ¡ un solo cÃ³digo de respuesta.',
'FOLIO: 1485 - ModificacionVtaTiempoAireDobleConsulta',
'FECHA : 22-05-2015',
'VERSION: 20150521.1435',
'BD: bdisac',
'-----------------------------------------------------------------------------------------------------------------------',
'MODIFICO: 96273763 - Antonio Cebreros Perez',
'DESCRIPCION: Se agrega escenario 6 de acuerdo a lo solicitado por el cliente, se realizan mejoras a las validaciones',
'FOLIO: 230142 - 69 - VtaTpoAireElectBCP',
'FECHA: 26-06-2016',
'VERSION: 20160701.1125',
'BD: bdisac',
'-----------------------------------------------------------------------------------------------------------------------';

CREATE PROCEDURE "informix".sp_tramaconsultatae( pNumCategoria CHAR (2), pNumConvenio CHAR (3), pFolioSucursal CHAR (16), pRef1 CHAR (40), pId_Sucursal CHAR (4), pFecha_Pago DATE, pNumTrama INTEGER, pTimeStamp CHAR (10), pIdTransaccion CHAR (9), pFirma CHAR (40))
RETURNING CHAR (5)   AS cCodRet, CHAR (172) AS cTrama;
   
   -- DeclaraciÃ³n de variables 
	DEFINE cCodRet 				CHAR (5);
	DEFINE cTrama				CHAR (172);
	DEFINE iSqlErr 				INTEGER;
	...
	DEFINE cNum_recargaserial 	CHAR (9);
   ...
   BEGIN
	...
	--SET DEBUG FILE TO '/informix/EPG/sp_tramaconsultatae.out';
	--TRACE ON;	
	...
	SET LOCK MODE TO WAIT 3;  
		...
		SELECT TRIM(valor) INTO cTipo_Operacion FROM  bdisac:"informix".sac_param  where cod_param = 91;
		...
		WHERE  numcategoria = pNumCategoria  AND  numconvenio = pNumConvenio AND id_sucursal = pId_Sucursal AND folio_suc = pFolioSucursal AND  referencia1 = pRef1 AND fecha_pago=pFecha_Pago;	
			IF DBINFO("sqlca.sqlerrd2") = 0 Or cRefer2 = '' Or cImporte= '' THEN
				...
				RETURN cCodRet, NVL(cTrama, '');
			END IF;
		--seleccionamos la num_clave de la compañia EPG
		...
		ELSE
			...
			END IF;
		END IF;    
		--SELECT TRIM(valor) INTO cNum_Clave FROM  bdisac:"informix".sac_param  where cod_param = 101;
		...
		SELECT campo8, campo22 INTO cNum_Folio, cNum_recargaserial FROM bdisac: "informix".sac_msw_respuesta where numcategoria= pNumCategoria and numconvenio = pNumConvenio  and folio_suc = pFolioSucursal and num_trama = 1;
	
	if  cNum_Folio is null and cNum_recargaserial is null then
		let cNum_recargaserial = '';
		let cNum_Folio = '';	
	end if;
	
	--Agrupa los datos para la generacion de la trama
	LET cTrama = cTrans_MotorS||cNum_Sucursal||cTipo_Operacion||cLlave||pTimeStamp||RPAD(trim(pIdTransaccion), 9, ' ')||lower(pFirma)||cNum_Clave||cNum_Seguridad||cNum_Version||cNum_Tienda||cClv_Area||cNum_Caja||cNum_Folio||cImporte||cNum_Cliente||cUser_Insert||cNum_Telefono||cNum_Origen||cNum_recargaserial;
