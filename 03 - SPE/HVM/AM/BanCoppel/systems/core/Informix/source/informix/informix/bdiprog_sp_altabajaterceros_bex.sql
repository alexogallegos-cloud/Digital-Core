CREATE PROCEDURE "informix".sp_altabajaterceros_bex(p_sTipoOperacion CHAR(2), p_sClaveCuenta CHAR(2), p_sNum_Cte CHAR(20), 
p_sCuenta CHAR(20), p_sCve_Banco CHAR(3), p_sDesc_cta CHAR(20), p_sNombre CHAR(60), p_sRfc CHAR(13), p_sEmail CHAR(40), 
p_sCve_Compania CHAR(2), p_sNo_Celular CHAR(10), p_sCanal_Alta CHAR(2), p_sCanal_Baja CHAR(2), p_sUser_Insert CHAR(8))
...
--Inicio del procedimiento
	...
	--SET DEBUG FILE TO "/informix/ireb/bdiprog/bex/sp_altabajaterceros.out";
	--TRACE ON;
...
--	SELECT producto INTO v_sProducEfecti FROM bdicheq:sc_producto WHERE nombre  = 'CUENTA EFECTIVA';
 --Se quita la validaciÃ³n por k esta mal hecha JGP - 15/05/2009---Solucionado el dia 25-mayo-2009
...
-- ValidaciÃ³n temporal, en lo que se libera a Nivel Nacional y en Internet
...
--
	...
	--	RETURN v_sCodRet, v_sMensajeRet;
	--END IF;
	--Se valida que la clave de la compaÃ±ia no este en blanco o en nulo

	IF (NVL(p_sCve_Compania,'') <> '') THEN
		--Se valida que exista la clave de la compaÃ±ia si no existe manda el codigo de retorno 08 con su descripcion
...
--			SELECT cuenta,producto INTO v_sCuenta,v_sProductoPerm  FROM bdicheq:sc_maechq WHERE cuenta = p_sCuenta AND status_cta = '1';
	...
	IF (p_sTipoOperacion = '01') THEN
		...
		IF EXISTS(SELECT num_cte,cuenta,cve_banco FROM bdiprog:"informix".pp_ctasterceros_bex WHERE num_cte = p_sNum_Cte AND
			cuenta = p_sCuenta AND cve_banco = p_sCve_Banco) THEN
			...
			SELECT cve_estado INTO v_sCveEstado FROM bdiprog:"informix".pp_ctasterceros_bex WHERE num_cte = p_sNum_Cte AND
			cuenta = p_sCuenta AND cve_banco = p_sCve_Banco;
			IF (v_sCveEstado = '02') THEN
				UPDATE bdiprog:"informix".pp_ctasterceros_bex SET descrip_cta = p_sDesc_cta,cve_cuenta = p_sClaveCuenta,
				...
				fecha_estado = current,user_insert = p_sUser_Insert,fecha_insert = current, hora_insert = current WHERE num_cte = p_sNum_Cte AND
				cuenta = p_sCuenta AND cve_banco = p_sCve_Banco ;
			...
			END IF;
		ELSE
			INSERT INTO bdiprog:"informix".pp_ctasterceros_bex(num_cte,cuenta,cve_banco,descrip_cta,cve_cuenta,nombre,rfc,direc_correo,cve_compania,cve_estado,no_celular,canal_alta,canal_baja,fecha_estado,user_insert,fecha_insert)
	...
	ELSE
		...
		IF EXISTS(SELECT cve_estado FROM bdiprog:"informix".pp_ctasterceros_bex WHERE num_cte = p_sNum_Cte AND cuenta = p_sCuenta AND
		cve_banco = p_sCve_Banco AND cve_cuenta = p_sClaveCuenta)THEN
			...
			IF v_sCveEstado = '01' THEN
				UPDATE bdiprog:"informix".pp_ctasterceros_bex SET cve_estado = '02',canal_baja = p_sCanal_Baja,fecha_estado = current,
