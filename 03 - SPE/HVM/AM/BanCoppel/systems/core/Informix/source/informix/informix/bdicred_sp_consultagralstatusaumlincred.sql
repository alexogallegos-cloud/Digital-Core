CREATE PROCEDURE "informix".sp_consultagralstatusaumlincred(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechainicial DATE, pFechaFinal DATE,
			pOrigen CHAR(2), pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
				  INTEGER AS tieneCausa,
				  CHAR(100)	AS descripcion,
				  INTEGER AS totalStatus,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, iTieneCausa, cDescripcion, iTotalStatus, dPorcentaje, iTotalGeneral;
		END EXCEPTION;
		ON EXCEPTION IN (-206)
		END EXCEPTION WITH RESUME;

		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultagralstatusaumlincred.out';
		--TRACE ON;

		IF pUsuario = '' OR pIdFuncion = '' OR pFechaInicial = '' OR pFechaFinal = '' OR pOrigen = '' OR pRegistros IS NULL OR pRecuperacion IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet, iTieneCausa, cDescripcion, iTotalStatus, dPorcentaje, iTotalGeneral;
		END IF;
		...
		FOREACH EXECUTE PROCEDURE bdicred:'informix'.sp_rep_gral_status2(pFechainicial, pFechaFinal, pOrigen, pRegistros, pRecuperacion)
			...
			ELSE
				...
				RETURN cCodRet, iTieneCausa, cDescripcion, iTotalStatus, dPorcentaje, iTotalGeneral WITH RESUME;
			END IF;
		END FOREACH;
...
'FECHA: 04/09/2014',
'DESCRIPCION: llena el grid Status',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consultarevisioncacaumlincred(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechainicial DATE, pFechaFinal DATE,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, dTieneCausa, cDescripcion, iTotalRegCentral, dPorcentajeCentral, iTotalCentral, 
				dTotPorCentral, iTotalRegSucursal, dPorcentajeSucursal, iTotalSucursal, dTotPorSucursal;
		END EXCEPTION;
		ON EXCEPTION IN (-206)
		END EXCEPTION WITH RESUME;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultarevisioncacaumlincred.out';
		--TRACE ON;
		...
		FOREACH EXECUTE PROCEDURE bdicred:'informix'.sp_cac_rep_revisioncac2(pFechainicial, pFechaFinal, pRegistros, pRecuperacion)
			INTO cCodRetSp, cMensajeRetorno, dTieneCausa, cDescripcion, iTotalRegCentral, dPorcentajeCentral, iTotalCentral, 
			dTotPorCentral, iTotalRegSucursal, dPorcentajeSucursal, iTotalSucursal, dTotPorSucursal
			
			LET iCodRetSp = cCodRetSp::INTEGER;
			IF iCodRetSp < 0 THEN
				RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_cac_rep_revisioncac2';
			ELIF iCodRetSp = 000002 THEN
			...
			ELSE
				LET iRecuperacion = iRecuperacion + 1;
				RETURN cCodRet, dTieneCausa, cDescripcion, iTotalRegCentral, dPorcentajeCentral, iTotalCentral, 
					dTotPorCentral, iTotalRegSucursal, dPorcentajeSucursal, iTotalSucursal, dTotPorSucursal WITH RESUME;
			END IF;
		END FOREACH;
		...
		ELIF iRecuperacion = 0 AND pRegistros > 0 THEN
			LET cCodRet = '01001';
			RETURN cCodRet, 0, '', 0, 0, 0, 0, 0, 0, 0, 0;
		END IF;
	END;
...
DOCUMENT 'AUTOR: Esparza Brenis Fernando Martin',
'FECHA: 04/09/2014',
'DESCRIPCION: llena el grid MC',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consultarevisioncentralaumlincred(pUsuario CHAR(8), pIdFuncion CHAR(10),pFechainicial DATE, pFechaFinal DATE,
			pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
				  INTEGER AS tieneCausa,
				  CHAR(100) AS descripcion,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, iTieneCausa, cDescripcion, iTotalRegCasosCac, dPorcentajeCasosCac, iTotCAC, 
				dPorCAC, iTotalRegCasosAuto, dDorcentajeCasosAuto, iTotAUTO, dPorAUTO;
		END EXCEPTION;
		ON EXCEPTION IN (-206)
		END EXCEPTION WITH RESUME;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultarevisioncentralaumlincred.out';
		--TRACE ON;
		...
		FOREACH EXECUTE PROCEDURE bdicred:'informix'.sp_cac_rep_revisioncentral2(pFechainicial, pFechaFinal, pRegistros, pRecuperacion)
			...
			ELSE
				LET iRecuperacion = iRecuperacion + 1;
				RETURN cCodRet, iTieneCausa, cDescripcion, iTotalRegCasosCac, dPorcentajeCasosCac, iTotCAC, 
					dPorCAC, iTotalRegCasosAuto, dDorcentajeCasosAuto, iTotAUTO, dPorAUTO WITH RESUME;
			END IF;
		END FOREACH;
...
DOCUMENT 'AUTOR: Esparza Brenis Fernando Martin',
'FECHA: 04/09/2014',
'DESCRIPCION: llena el grid central',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consultatotrevisioncentralaumlincred(pUsuario CHAR(8), pIdFuncion CHAR(10),pFechainicial DATE, pFechaFinal DATE)
		RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultatotrevisioncentralaumlincred.out';
		--TRACE ON;
		...
		FOREACH EXECUTE PROCEDURE bdicred:'informix'.sp_cac_rep_revisioncentral_total(pFechainicial, pFechaFinal)
			INTO cCodRetSp, cMensajeRetorno, iTotalRegistros
			
			LET iCodRetSp = cCodRetSp::INTEGER;
			IF iCodRetSp < 0 THEN
				RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_cac_rep_revisioncentral_total';
			ELIF iCodRetSp = 000002 THEN
				LET cCodRet = '00388'; --PARÁMETRO DE FECHA INVALIDO PARA REALIZAR  LA CONSULTA
				RETURN cCodRet, iTotalRegistros;
			ELIF iCodRetSp = 000001 THEN
...
DOCUMENT 'AUTOR: Esparza Brenis Fernando Martin',
'FECHA: 04/09/2014',
'DESCRIPCION: Consulta que obtiene el total de registros de información para hacer el llendo del grid central',
'BD: bdicnweb';
