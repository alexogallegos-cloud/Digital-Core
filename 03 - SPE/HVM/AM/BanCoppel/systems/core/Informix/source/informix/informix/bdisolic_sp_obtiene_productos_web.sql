CREATE PROCEDURE "informix".sp_obtiene_productos_web(pEmpresa CHAR(3), pSucursal CHAR(4), pEjecutivo CHAR(8), pPuesto_local CHAR(2), pNumcte CHAR(20), 	
...
--jom-26/11/2018			
	...
	--SET DEBUG FILE TO "/tmp/anj/sp_obtiene_productos.sql";
	--TRACE ON;
		...
		FOREACH
			SELECT valor INTO cValorA FROM bdinteg:"informix".si_param where cod_param in('467','468')				
			LET cValorAa = SUBSTR(cValorA,2,4);
			
			SELECT COUNT(NUMCTE) INTO sExiste FROM bdisitesp:"informix".se_ctessitespcte WHERE CAUSA IN( cValorAa,'109') AND situacion='P' AND NUMCTE= pNumcte; 
...
---RGH 18122018
		...
		ELSE
			...
			-- Obtiene los productos de captacion con los que actualmente cuenta el cliente, se modifica para que se inserte la fecha de alta de la cuenta de captaciÃÂ³n
			FOREACH
				...
				IF cStatus_sol = "AP" OR cStatus_sol = "RT" THEN
					IF cTpSolicitudAct IN ('T','P') THEN
						...
						IF cStatus_sol = "AP" and ProdActual <> '7800' THEN
							...
							--Calculamos los dÃ¯Â¿Â½as que tiene la solcitud en estatus AP.
...
-- EM 2017/05/25
            ...
            IF (inumpPrestamos >= iPrestamosActivos) THEN
				...
				INSERT INTO bdisolic:"informix".ss_bitacora_precal (empresa,fecha,producto,sucursal,nombre,ejecutivo,mensaje,causa_solicitud)
					VALUES (pEmpresa,today,'6300',pSucursal,cnomcte,pEjecutivo,'El Cliente cuenta con el tope mÃÂ¡ximo de PrÃÂ©stamos Personales permitidos','RFP',cFechaUltimoPago, cPrestamoAutorizado, iMontoAutorizado, iRePrestamo);
			END IF;
			--JMAH
		...
		IF NVL(cCteProsp,'') <> '' THEN
			...
			IF NVL(cClave,'') = 'R' THEN
				...
				FOREACH
					...
					IF (SELECT count(1) FROM bdisolic:"informix".ss_productos_ofrecer 
		...
		-- Valida que NO oferte productos de crÃÂ¨dito a empleados GC RQM 09 539
		IF pEmp = '1' THEN
			UPDATE bdisolic:"informix".ss_productos_ofrecer SET aplica = 'N' WHERE cliente = pNumcte AND sucursal = pSucursal AND ejecutivo = pEjecutivo AND producto_ofr in (select num_producto from bdicred:sd_definicion where oferta_emp = '0');
		END IF;
		...
		FOREACH
			...
			IF cprod_final  = '6400' THEN 
				...
				IF (SELECT count(1) FROM bdisolic:"informix".ss_producto_credcap WHERE num_producto = cprod_final AND meses_alta IS NOT NULL) > 0 THEN
			...
			If pAlta = '0' then 
				If pUso = 1 then
					IF cprod_final in (Select producto from ss_cat_prod_ofrecer where ejecucion = '2') then
						LET cOfertar ='N';
						CONTINUE FOREACH;

					END IF;
				elif pUso = 2 then
					IF cprod_final in (Select producto from ss_cat_prod_ofrecer where ejecucion = '1') then
						LET cOfertar ='N';
						CONTINUE FOREACH;
