CREATE PROCEDURE "informix".sp_indicadores_usabilidad_srvs(vDia date)
RETURNING CHAR(5), CHAR (40), CHAR (20000);

	DEFINE vMensaje	CHAR (20000);
	DEFINE codret CHAR(5);
	DEFINE msjResp CHAR(40);
	DEFINE iSqlErr 	INTEGER;
	DEFINE vIndicadorAnterior char(55);
	DEFINE vIndicadorActual char(55);
	DEFINE vConvenio CHAR(70);
	DEFINE vdia1 CHAR(10);
	DEFINE vdia2 CHAR(10);
	DEFINE vdia3 CHAR(10);
	DEFINE vdia4 CHAR(10);
	DEFINE vdia5 CHAR(10);
	DEFINE vdia6 CHAR(10);
	DEFINE vdia7 CHAR(10);
	DEFINE vTotal CHAR(20);
	DEFINE vHeaderFechas CHAR(205);
	
	--SET DEBUG FILE TO '/informix/noe/sp_indicadores_usabilidad_srvs.out';
	--TRACE ON;

	LET iSqlErr = 0;
	LET codret = '00000';
	LET msjResp = 'PROCESO EXITOSO';
	LET vIndicadorAnterior ='';
	LET vIndicadorActual ='';
	LET vConvenio='';
	LET vdia1='';
	LET vdia2='';
	LET vdia3='';
   ...
   BEGIN
	ON EXCEPTION SET iSqlErr
		IF iSqlErr <> 0 THEN
			LET codret = iSqlErr;
			LET msjResp = 'ERROR EN PROCESO';
			RETURN codret, msjResp, vMensaje;
		END IF;
	END EXCEPTION;
	...
	DROP TABLE IF EXISTS TB_FECHAS_TMP;
	DROP TABLE IF EXISTS TB_TOTALES_TMP;

    CREATE TEMP TABLE TB_FECHAS_TMP ( dia1 DATE, dia2 DATE, dia3 DATE, dia4 DATE, dia5 DATE, dia6 DATE, dia7 DATE) WITH NO LOG; 
    CREATE TEMP TABLE TB_TOTALES_TMP ( indicador CHAR(55), convenio CHAR(70), total INTEGER, dia_uno INTEGER, dia_dos INTEGER, dia_tres INTEGER, dia_cuatro INTEGER, dia_cinco INTEGER, dia_seis INTEGER, dia_siete INTEGER, periodo CHAR(23)) WITH NO LOG; 


	INSERT INTO TB_FECHAS_TMP
	SELECT 
	CASE WHEN WEEKDAY(vDia) BETWEEN 0 AND 4 THEN 
	    case when WEEKDAY(vDia)=5 then vDia-7 else ((vDia) - (WEEKDAY(vDia) + 9)) end
	ELSE ((vDia) - (WEEKDAY(vDia) + 2))
	END  dia1,
	CASE WHEN WEEKDAY(vDia) BETWEEN 0 AND 4 THEN 
	    case when WEEKDAY(vDia)=5 then vDia-7 else ((vDia) - (WEEKDAY(vDia) + 8)) end
	ELSE ((vDia) - (WEEKDAY(vDia) + 1))
	END  dia2,
	CASE WHEN WEEKDAY(vDia) BETWEEN 0 AND 4 THEN 
	    case when WEEKDAY(vDia)=5 then vDia-7 else ((vDia) - (WEEKDAY(vDia) + 7)) end
	ELSE ((vDia) - (WEEKDAY(vDia) + 0))
	END  dia3,
	CASE WHEN WEEKDAY(vDia) BETWEEN 0 AND 4 THEN 
	    case when WEEKDAY(vDia)=5 then vDia-7 else ((vDia) - (WEEKDAY(vDia) + 6)) end
	ELSE ((vDia) - (WEEKDAY(vDia) -1))
	END  dia4,
	CASE WHEN WEEKDAY(vDia) BETWEEN 0 AND 4 THEN 
	    case when WEEKDAY(vDia)=5 then vDia-7 else ((vDia) - (WEEKDAY(vDia) + 5)) end
	ELSE ((vDia) - (WEEKDAY(vDia) -2))
	END  dia5,
	CASE WHEN WEEKDAY(vDia) BETWEEN 0 AND 4 THEN 
	    case when WEEKDAY(vDia)=5 then vDia-7 else ((vDia) - (WEEKDAY(vDia) + 4)) end
	ELSE ((vDia) - (WEEKDAY(vDia) -3))
	END  dia6,
	CASE WHEN WEEKDAY(vDia) BETWEEN 0 AND 4 THEN 
	    case when WEEKDAY(vDia)=5 then vDia-7 else ((vDia) - (WEEKDAY(vDia) + 3)) end
	ELSE ((vDia) - (WEEKDAY(vDia) -4))
	END  dia7
	FROM sac_fechas;

	LET vMensaje ='<!DOCTYPE html> ' ||
		'<html> ' ||
		'<head> ' ||
		'<style> ' ||
		--'.footer {color: #e2e2e2;text-align: center;} ' || 
		--'.srvbcpl { border-collapse: collapse; margin: 25px 0; font-size: 0.9em; font-family: sans-serif; min-width: 400px; box-shadow: 0 0 20px rgba(0, 0, 0, 0.15);} ' ||
		'thead tr {background-color: #3b83bd; color: #ffffff; text-align: left;} ' ||
		'th, td {padding: 12px 15px;} ' ||
		'tbody tr {border-bottom: 1px solid #dddddd;} ' ||
		'tbody tr:nth-of-type(even) {background-color: #f3f3f3;} ' ||
		'tbody tr:last-of-type {border-bottom: 2px solid #3b83bd;} ' ||
		'tbody tr.active-row {font-weight: bold; color: #3b83bd;} ' ||
		'</style> ' ||
		'</head> ' ||
		'<body> ' ||
		'<h3>Indicadores de Usabilidad</h3> <b> Gerencia: </b>Sistemas Mantenimiento IV <br/> <b> Coordinaci&oacute;n: </b>Servicios y Remesas <br/> <b> Periodo: </b> ' || (select TO_CHAR(dia1, '%d/%m/%Y') from TB_FECHAS_TMP) || ' - ' || (select TO_CHAR(dia7, '%d/%m/%Y') from TB_FECHAS_TMP) || --'<br/>' ||
		...
		LET vHeaderFechas = '<thead> <tr> <th>Transaccion</th> <th>'|| (select TO_CHAR(dia1, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> <th>'|| (select TO_CHAR(dia2, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> <th>'|| (select TO_CHAR(dia3, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> <th>'|| (select TO_CHAR(dia4, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> <th>'|| (select TO_CHAR(dia5, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> <th>'|| (select TO_CHAR(dia6, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> <th>'|| (select TO_CHAR(dia7, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> <th>TOTAL</th> </tr> </thead>';
/*
	LET vMensaje = trim(vMensaje) || '<th>'|| (select TO_CHAR(dia1, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> ';
    LET vMensaje = trim(vMensaje) || '<th>'|| (select TO_CHAR(dia2, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> ';
    LET vMensaje = trim(vMensaje) || '<th>'|| (select TO_CHAR(dia3, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> ';
    LET vMensaje = trim(vMensaje) || '<th>'|| (select TO_CHAR(dia4, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> ';
    LET vMensaje = trim(vMensaje) || '<th>'|| (select TO_CHAR(dia5, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> ';
    LET vMensaje = trim(vMensaje) || '<th>'|| (select TO_CHAR(dia6, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> ';
    LET vMensaje = trim(vMensaje) || '<th>'|| (select TO_CHAR(dia7, '%d/%m/%Y') from TB_FECHAS_TMP) ||'</th> <th>TOTAL</th> </tr> </thead> <tbody> ';
*/

--CORRESPONSALES
    --20: Transacciones de Pago de Servicios
        INSERT INTO TB_TOTALES_TMP
        SELECT 
            '01 - Homologaci&oacute;n de Servicios (Corresponsales)', 
            C.nomconvenio AS convenio, 
            count(*) total,
            sum(case when fecha_pago= (select dia1 from TB_FECHAS_TMP) then 1 else 0 end) uno,
            sum(case when fecha_pago= (select dia2 from TB_FECHAS_TMP) then 1 else 0 end) dos,
            sum(case when fecha_pago= (select dia3 from TB_FECHAS_TMP) then 1 else 0 end) tres,
            sum(case when fecha_pago= (select dia4 from TB_FECHAS_TMP) then 1 else 0 end) cuatro,
            sum(case when fecha_pago= (select dia5 from TB_FECHAS_TMP) then 1 else 0 end) cinco,
            sum(case when fecha_pago= (select dia6 from TB_FECHAS_TMP) then 1 else 0 end) seis,
            sum(case when fecha_pago= (select dia7 from TB_FECHAS_TMP) then 1 else 0 end) siete,
            (select TO_CHAR(dia1, '%d/%m/%Y') from TB_FECHAS_TMP) || ' - ' || (select TO_CHAR(dia7, '%d/%m/%Y') from TB_FECHAS_TMP) AS periodo
        FROM sac_movimientoshistorial M, sac_convenios C
        WHERE (fecha_pago BETWEEN (select dia1 from TB_FECHAS_TMP) AND (select dia7 from TB_FECHAS_TMP)) 
        AND statusconvenio='A'
        AND status_cancelado = 'N' 
        AND id_sucursal = '9764'
        AND C.numcategoria || C.numconvenio not in('07004  ','07006  ','07007  ','07008  ','07009  ')
        --AND (C.numcategoria <> '07' and C.numconvenio not in('004','006','007','008','009'))
        AND C.numcategoria = M.numcategoria
        AND C.numconvenio = M.numconvenio
        GROUP BY 1,2;
...
--REMESAS Y SERVICIOS
    --21: Pago de Remesas
        INSERT INTO TB_TOTALES_TMP
        SELECT '02 - Pago de Remesas', --case when C.numconvenio='004' then 21 when C.numconvenio in('006','007','008') then 22 when C.numconvenio='009' then 23 end , 
            trim(C.nomconvenio) || ' - Ventanilla' AS convenio,
            count(*) total,
            sum(case when fecha_pago= (select dia1 from TB_FECHAS_TMP) then 1 else 0 end) uno,
            sum(case when fecha_pago= (select dia2 from TB_FECHAS_TMP) then 1 else 0 end) dos,
            sum(case when fecha_pago= (select dia3 from TB_FECHAS_TMP) then 1 else 0 end) tres,
        ...
        AND statusconvenio='A'
        AND status_cancelado = 'N' 
        AND id_sucursal not in('9250','9764')
        AND origen <>'CPL'
        AND (C.numcategoria ='07' AND C.numconvenio in('004','006','007','008','009'))
        AND C.numcategoria = M.numcategoria
        AND C.numconvenio = M.numconvenio
        GROUP BY 1,2;


----Remesas AutomÃ¡ticas
        INSERT INTO TB_TOTALES_TMP
        SELECT '03 - Remesas Autom&aacute;ticas', --case when C.numconvenio='004' then 24 when C.numconvenio in('006','007','008') then 25 when C.numconvenio='009' then 26 end , 
            trim(C.nomconvenio) || ' - Autom&aacute;ticas' AS convenio,
            count(*) total,
            sum(case when fecha_pago= (select dia1 from TB_FECHAS_TMP) then 1 else 0 end) uno,
            sum(case when fecha_pago= (select dia2 from TB_FECHAS_TMP) then 1 else 0 end) dos,
            sum(case when fecha_pago= (select dia3 from TB_FECHAS_TMP) then 1 else 0 end) tres,
...
--Remesas Coppel
        INSERT INTO TB_TOTALES_TMP
        SELECT '04 - Homologaci&oacute;n de Remesas (Pagadas en Coppel)', --case when C.numconvenio='004' then 27 when C.numconvenio in('006','007','008') then 28 when C.numconvenio='009' then 29 end , 
            trim(C.nomconvenio) || ' - CPL' AS convenio,
            count(*) total,
            sum(case when fecha_pago= (select dia1 from TB_FECHAS_TMP) then 1 else 0 end) uno,
            sum(case when fecha_pago= (select dia2 from TB_FECHAS_TMP) then 1 else 0 end) dos,
            sum(case when fecha_pago= (select dia3 from TB_FECHAS_TMP) then 1 else 0 end) tres,
    ...
    --25: Pago de Servicios
        INSERT INTO TB_TOTALES_TMP
        SELECT '05 - Pago de Servicios', C.nomconvenio AS convenio,
            count(*) total,
            sum(case when fecha_pago= (select dia1 from TB_FECHAS_TMP) then 1 else 0 end) uno,
            sum(case when fecha_pago= (select dia2 from TB_FECHAS_TMP) then 1 else 0 end) dos,
            sum(case when fecha_pago= (select dia3 from TB_FECHAS_TMP) then 1 else 0 end) tres,
    ...
    --26 LATINIA
    	...
    	--Obtiene registros
	    Select tipo_mensaje, fecha_hora_registro::date fecha FROM bdimnsj@lat_tcp:mnsjr_trx_online
	    WHERE fecha_hora_registro between EXTEND((select dia1 from TB_FECHAS_TMP), YEAR to SECOND)+0 UNITS HOUR+0 UNITS MINUTE+0 UNITS SECOND AND EXTEND((select dia7 from TB_FECHAS_TMP), YEAR to SECOND)+23 UNITS HOUR+59 UNITS MINUTE+59 UNITS SECOND
	    UNION ALL
	    select tipo_mensaje, fecha_hora_registro::date fecha FROM bdimnsj@lat_tcp:mnsjr_trx_online_his
	    WHERE fecha_hora_registro between EXTEND((select dia1 from TB_FECHAS_TMP), YEAR to SECOND)+0 UNITS HOUR+0 UNITS MINUTE+0 UNITS SECOND AND EXTEND((select dia7 from TB_FECHAS_TMP), YEAR to SECOND)+23 UNITS HOUR+59 UNITS MINUTE+59 UNITS SECOND
	    UNION ALL
	    select tipo_mensaje, fecha_hora_registro::date fecha FROM bdimnsj@lat_tcp:mnsjr_trx_batch
	    WHERE fecha_hora_registro between EXTEND((select dia1 from TB_FECHAS_TMP), YEAR to SECOND)+0 UNITS HOUR+0 UNITS MINUTE+0 UNITS SECOND AND EXTEND((select dia7 from TB_FECHAS_TMP), YEAR to SECOND)+23 UNITS HOUR+59 UNITS MINUTE+59 UNITS SECOND
	    UNION ALL
	    select tipo_mensaje, fecha_hora_registro::date fecha FROM bdimnsj@lat_tcp:mnsjr_trx_batch_his
	    WHERE fecha_hora_registro between EXTEND((select dia1 from TB_FECHAS_TMP), YEAR to SECOND)+0 UNITS HOUR+0 UNITS MINUTE+0 UNITS SECOND AND EXTEND((select dia7 from TB_FECHAS_TMP), YEAR to SECOND)+23 UNITS HOUR+59 UNITS MINUTE+59 UNITS SECOND
		INTO TEMP TMP_MENSAJES WITH NO LOG;

		--Inserta en Totales
		INSERT INTO TB_TOTALES_TMP
		SELECT * FROM (SELECT '06 - Latinia','Correos' convenio, sum(case when tipo_mensaje = '1' then 1 else 0 end) total,
		sum(case when tipo_mensaje = '1' and fecha= (select dia1 from TB_FECHAS_TMP) then 1 else 0 end) uno,
		sum(case when tipo_mensaje = '1' and fecha= (select dia2 from TB_FECHAS_TMP) then 1 else 0 end) dos,
		sum(case when tipo_mensaje = '1' and fecha= (select dia3 from TB_FECHAS_TMP) then 1 else 0 end) tres,
		sum(case when tipo_mensaje = '1' and fecha= (select dia4 from TB_FECHAS_TMP) then 1 else 0 end) cuatro,
		sum(case when tipo_mensaje = '1' and fecha= (select dia5 from TB_FECHAS_TMP) then 1 else 0 end) cinco,
		sum(case when tipo_mensaje = '1' and fecha= (select dia6 from TB_FECHAS_TMP) then 1 else 0 end) seis,
		sum(case when tipo_mensaje = '1' and fecha= (select dia7 from TB_FECHAS_TMP) then 1 else 0 end) siete,
		(select TO_CHAR(dia1, '%d/%m/%Y') from TB_FECHAS_TMP) || ' - ' || (select TO_CHAR(dia7, '%d/%m/%Y') from TB_FECHAS_TMP) periodo
		FROM TMP_MENSAJES
		UNION ALL
		SELECT '06 - Latinia','SMS' convenio, sum(case when tipo_mensaje = '2' then 1 else 0 end) total,
		sum(case when tipo_mensaje = '2' and fecha= (select dia1 from TB_FECHAS_TMP) then 1 else 0 end) uno,
		sum(case when tipo_mensaje = '2' and fecha= (select dia2 from TB_FECHAS_TMP) then 1 else 0 end) dos,
		sum(case when tipo_mensaje = '2' and fecha= (select dia3 from TB_FECHAS_TMP) then 1 else 0 end) tres,
		sum(case when tipo_mensaje = '2' and fecha= (select dia4 from TB_FECHAS_TMP) then 1 else 0 end) cuatro,
		sum(case when tipo_mensaje = '2' and fecha= (select dia5 from TB_FECHAS_TMP) then 1 else 0 end) cinco,
		sum(case when tipo_mensaje = '2' and fecha= (select dia6 from TB_FECHAS_TMP) then 1 else 0 end) seis,
		sum(case when tipo_mensaje = '2' and fecha= (select dia7 from TB_FECHAS_TMP) then 1 else 0 end) siete,
		(select TO_CHAR(dia1, '%d/%m/%Y') from TB_FECHAS_TMP) || ' - ' || (select TO_CHAR(dia7, '%d/%m/%Y') from TB_FECHAS_TMP) periodo
		FROM TMP_MENSAJES);
        --LET vMensaje = trim(vMensaje) || trim(vHeaderFechas) ;

        FOREACH SELECT indicador,convenio,dia_uno,dia_dos,dia_tres,dia_cuatro,dia_cinco,dia_seis,dia_siete,total
        		INTO vIndicadorActual, vConvenio, vdia1, vdia2, vdia3, vdia4, vdia5, vdia6, vdia7, vTotal
        		FROM TB_TOTALES_TMP order by 1,2 asc

        		IF vIndicadorActual <> vIndicadorAnterior THEN
        			IF vIndicadorAnterior <> '' THEN
        				LET vMensaje = trim(vMensaje) || '</tbody>';
        			END IF;

        			LET vMensaje = trim(vMensaje) || '<thead> <tr style=' || '''background-color:#87CEEB; color:black;''' || '> <th colspan=' || '''9''' || ' style=' || '''text-align:center !important;''' || '>' || substr(trim(vIndicadorActual),6,55) || '</th> </tr> </thead>';
        			LET vMensaje = trim(vMensaje) || trim(vHeaderFechas)  || '<tbody>';
        			LET vIndicadorAnterior=vIndicadorActual;

        		END IF;

        		LET vMensaje = trim(vMensaje) || ' <tr> ' ||
		    			'<td>' || trim(vConvenio) || '</td> ' ||
					    '<td>' || trim(vdia1) || '</td> ' ||
					    '<td>' || trim(vdia2) || '</td> ' ||
					    '<td>' || trim(vdia3) || '</td> ' ||
					    '<td>' || trim(vdia4) || '</td> ' ||
					    ...
					    '<td>' || trim(vdia6) || '</td> ' ||
					    '<td>' || trim(vdia7) || '</td> ' ||
					    '<td>' || trim(vTotal) || '</td> </tr> ';
    		
        END FOREACH;
	
  
	LET vMensaje = trim(vMensaje) || '</tbody> </table> <footer style=' || '''color: #e2e2e2;text-align: center;''' || '><p>Reporte generado de forma autom&aacute;tica - 964_INDICADORES_SERVREM_PRO. </p> </footer> </body> </html>';
...
'DESCRIPCION: Genera el reporte de Indicadores de Usabilidad',
'FOLIO: ',
'FECHA : 22-04-2021',
'VERSION: 20210217.1702',
'BD: bdisac';
