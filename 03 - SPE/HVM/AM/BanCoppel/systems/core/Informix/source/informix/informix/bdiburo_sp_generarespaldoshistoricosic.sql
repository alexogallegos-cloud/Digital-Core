CREATE PROCEDURE "informix".sp_generarespaldoshistoricosic(pNumCte        CHAR(20), 
                                                           pSolicitud     VARCHAR(25), 
                                                           pInstitucion   CHAR(2),
													       pBandFlujo     SMALLINT)

--EXECUTE PROCEDURE	sp_generarespaldoshistoricosic( '','','BC',1);
RETURNING CHAR(6)  AS COD_RET,
		  CHAR(80) AS MENSAJE_EJEC;

--DECLARACIÃ¯Â¿Â½N DE VARIABLES
...
BEGIN
	...
	END EXCEPTION;
	 ...
	 --SET DEBUG FILE TO "/informix/paulq/pba/sp_generarespaldoshistoricosic_today.out";
	 --TRACE ON;

	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;

	IF pBandFlujo = 1 THEN		-- FLUJO INDIVIDUAL

		-- SE VALIDA SI EL NUMERO DE CLIENTE SE RECIBE VACIO
		IF NVL(pNumCte, "") = "" THEN
			LET cCodRet = "000003";
			LET cMensajeRet = "EL NUMERO DE CLIENTE RECIBIDO ES INCORRECTO";
...
--IPCB Mayo2016 Reingenieria de Demonios. Se cambia el insert a la br_respuesta_hist
		 ...
		 INSERT INTO "informix".sb_regreso_hist(institucion,num_solicitud,regreso,status) 
...
--IPCB Mayo2016 Reingenieria de Demonios. 
		...
		DELETE FROM "informix".sb_regreso
			  WHERE institucion = pInstitucion 
			    AND num_solicitud = pSolicitud;
...
--IPCB Mayo2016 Reingenieria de Demonios. Se elimina de la br_respuesta y br_respuesta_aprocesar
	...
	ELSE
		LET cCodRet = "000001";
		LET cMensajeRet = "BANDERA INDICADORA DEL FLUJO ES INVALIDA";
	END IF;
...
END PROCEDURE 
DOCUMENT
'Realiza el respaldo de las tablas donde se registra la informaciÃ¯Â¿Â½n de las SIC',
'AUTOR : HÃ¯Â¿Â½ctor Manuel Bojorquez Ruelas',
...
--EXECUTE PROCEDURE	sp_generarespaldoshistoricosic('021558755','600548461760','BC',1);
RETURNING CHAR(6)  AS COD_RET,
		  CHAR(80) AS MENSAJE_EJEC;
...
'Folio: 529 - ',
'Fecha: 19-03-2019',
'Modificación: Se crea el procedimiento almacenado "sp_respuesta_buro" para coulsultar las respuesta de buro de credito y el estatus de las solicitudes de credito de la tabla ss_solicitudes',
'Solicita: ', 
'Base de datos: bdiburo';

CREATE PROCEDURE "informix".sp_regreso_respuesta(pInstitucion CHAR(2),pnum_solicitud VARCHAR(25), pInicio INTEGER, pSalto INTEGER)
RETURNING CHAR(250) AS cadena;

        DEFINE cCodRet CHAR(5);
		DEFINE iSqlErr INTEGER;
		DEFINE cCadena CHAR(250);
		DEFINE iPosIni INTEGER;		
		DEFINE iPosFin INTEGER;
		DEFINE iSecIni INTEGER;
		DEFINE iSecFin INTEGER;
		DEFINE iPosReal INTEGER;
		DEFINE cTramaAux CHAR(10000);	
		DEFINE cTrama CHAR(20000);
		DEFINE cSalida CHAR(250);
        ...
        BEGIN
				...
				--SET DEBUG file to '/RESPALDOS/ipcb/tasf/regreso_respuesta.out';
				--TRACE ON;
				
				IF pInicio < iLongitud THEN
					LET iSecIni = 1;
				ELSE
					LET iSecIni = FLOOR(pInicio / iLongitud)+1;
				END IF;
				
				LET iPosFin = pInicio + pSalto;
				
				IF iPosFin < iLongitud THEN
					LET iSecFin = 1;
				ELSE
					LET iSecFin = FLOOR(iPosFin / iLongitud)+1;
				END IF;
				
				IF iSecIni = iSecFin THEN				
					FOREACH
						SELECT TRIM(regreso) INTO cTramaAux  FROM "informix".br_respuesta WHERE institucion = pInstitucion AND num_solicitud = pnum_solicitud AND secuencia in (iSecIni)order by secuencia					
						
						IF LENGTH(cTramaAux)=iLongitud - 1 THEN
							--SET DEBUG file to '/RESPALDOS/ipcb/tasf/valida_respuesta.out';
							--TRACE ON;
							LET cTramaAux = TRIM(cTramaAux) || '-'; 
						END IF
						
						LET cTrama = cTramaAux;					
					END FOREACH;
					
					IF (iSecIni = 1) THEN
						LET cSalida = SUBSTR(TRIM(cTrama),pInicio,pInicio+pSalto);
					ELSE
						LET iPosReal = (iLongitud - ((iLongitud * iSecIni) - pInicio));
						LET cSalida = SUBSTR(cTrama,iPosReal,iPosReal+pSalto);						
					END IF
					
					RETURN cSalida;
				ELSE
					FOREACH
						SELECT TRIM(regreso) INTO cTramaAux  FROM "informix".br_respuesta WHERE institucion = pInstitucion AND num_solicitud = pnum_solicitud AND secuencia in (iSecIni, iSecFin)order by secuencia					
						...
						LET cTrama = TRIM(cTrama) || TRIM(cTramaAux);					
					END FOREACH;
										
					LET iPosReal = (18000 - ((iLongitud * iSecFin) - pInicio));
					LET cSalida = SUBSTR(cTrama,iPosReal,iPosReal+pSalto);						
...
'FECHA: 01/06/2016',
'MODULO: DEMONIO',
'FUNCIONALIDAD: Obtener Cadena de br_respuesta por secuencia',
'DESCRIPCION:Obtiene una cadena de acuerdo a los parametros de entrada',
'BD: bdiburo';

CREATE PROCEDURE "informix".actualizacionsolicitudes(pContrasena CHAR(8))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/procesoreenvioDemonio.out';
		--TRACE ON;
		
		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;

		FOREACH SELECT  num_solicitud INTO cNumSolicitud
						...
						AND envio matches '*007MX*' 
						AND institucion = 'BC' 						
						
				UPDATE bdiburo:br_traslado SET envio = REPLACE(envio,SUBSTR(envio,51,8),pContrasena) WHERE institucion = "BC" AND num_solicitud = TRIM(cNumSolicitud);				
...
'FECHA: 17/05/2016',
'MODULO: DEMONIO',
'FUNCIONALIDAD: ACTUALIZACION DE PASSWORD',
'DESCRIPCION:SPL que actualiza solicitudes con el nuevo password',
'BD: bdiburo';

CREATE PROCEDURE "informix".actualizacionsolicitudesprospector(pContrasena CHAR(8))
	RETURNING CHAR(5) AS codret;
	...
	BEGIN
		...
		FOREACH SELECT  num_solicitud INTO cNumSolicitud
						...
						(SELECT num_solicitud FROM bdisolic:ss_solicitudes 
						WHERE status_solicitud = 'BC') 
						AND envio NOT matches '*'||(SELECT TRIM(valor) FROM bdiburo:br_param WHERE cod_param = "155")||'*' 
						AND envio matches '*107MX*' 
						AND institucion = 'BC' 						
...
'FECHA: 03/06/2020',
'MODULO: DEMONIO',
'FUNCIONALIDAD: ACTUALIZACION DE PASSWORD PROSPECTOR',
'DESCRIPCION:SPL que actualiza solicitudes con el nuevo password',
'BD: bdiburo';
