CREATE PROCEDURE "informix".sp_reporteliquidacionseleccion(pConvenio CHAR(5))
...
DEFINE iImportePago		 MONEY(16,2);
...
LET iImportePago	= 0;
	...
	--SET DEBUG FILE TO '/RESPALDOS/sp_reporteliquidacionselecciones.out';
	--TRACE ON;
	...
	BEGIN
		ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
			IF iSqlErr <> 0 THEN
				...
				EXECUTE PROCEDURE "informix".sp_sac_GuardaMensajeerror (iSqlErr, iIsamErr, cInfoErr, "sp_reporteliquidacionselecciones");
			END IF;
		END EXCEPTION;
		...
		IF TO_CHAR(dfecha_Hoy,"%A") = "Sunday" THEN
			...
			WHILE dFechaAux <= dfecha_Hoy
				...
				FOREACH
					...
					IF iFormaPago = 1 THEN
						--Pago en efectivo
						LET mCobEfeAux = iImportePago;
						LET mCobCCAux = 0;
						LET iRecEfeAux = cFolio;
						LET iRecCCAux = '';
					ELSE
						--Pago cargo a cuenta
						LET mCobEfeAux = 0;
						LET mCobCCAux = iImportePago;
						LET iRecEfeAux = '';
						LET iRecCCAux = cFolio;
					END IF;
