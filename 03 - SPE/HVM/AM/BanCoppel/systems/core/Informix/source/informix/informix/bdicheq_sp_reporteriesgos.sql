CREATE PROCEDURE "informix".sp_reporteriesgos()

RETURNING char(5);

    DEFINE dfecha_hoy         DATE;
    DEFINE dfecha_ayer        DATE;
    DEFINE dfecha_antier      DATE;
    DEFINE cProducto          CHAR(4);
    DEFINE pcod_ret 		  CHAR(5);
    DEFINE pcod_ret2 		  CHAR(5);
...
SET OPTIMIZATION ALL_ROWS;
    ...
    ON EXCEPTION SET sql_err, isam_err, error_info
		...
		INSERT INTO sc_ctrlrepriesgos (proceso, status, codigo,  fecha_ejecucion)
		VALUES ('Reporte Riesgos', '0', pcod_ret, dfecha_hoy);
        RETURN pcod_ret;
    END EXCEPTION;	
	...
	-- Set Debug File To '/tmp/Antonio/sp_reporteriesgos.out';
	-- Trace On;
	...
	LET dfecha_antier = dfecha_ayer -1;

	IF EXISTS (SELECT {+INDEX(sc_ctrlrepriesgos idx_ctrrepriesgos14)} fecha_ejecucion 
                 FROM sc_ctrlrepriesgos 
                WHERE proceso = proceso 
                  AND fecha_ejecucion = dfecha_hoy ) THEN
		let pcod_ret = '110'; -- // YA SE EJECUTO PROCESO PARA ESTA FECHA
		RETURN pcod_ret;	
	END IF;

	SET ISOLATION TO DIRTY READ;
    
    SELECT cuenta, status_cta, producto, sucursal, sdo_dia_ant
      FROM sc_maechq
     WHERE status_cta <> '2'
      INTO TEMP tmp_maechq WITH NO LOG;
    CREATE INDEX idx_tmpmaechq ON tmp_maechq(producto) USING BTREE FILLFACTOR 99;
    UPDATE STATISTICS MEDIUM FOR TABLE tmp_maechq;
    
    SELECT cuenta, fecha_alta
      FROM sc_maenoc
     WHERE empresa = '001'
       AND cuenta IN(SELECT cuenta FROM tmp_maechq)
      INTO TEMP tmp_maenoc WITH NO LOG;
    CREATE INDEX idx_tmpmaenoc ON tmp_maenoc(cuenta) USING BTREE FILLFACTOR 99;
    UPDATE STATISTICS MEDIUM FOR TABLE tmp_maenoc;
	...
	FOREACH
		SELECT valor 
		  ...
		  FROM sc_param 
		 WHERE empresa = '001'
		   AND codparam LIKE 'RepRiesgos%'
		
		IF EXISTS ( SELECT fecha 
                     FROM bdinteg:si_feriado 
                    WHERE fecha = dfecha_ayer 
                      AND laborable = 'N' ) THEN
		
			CALL sp_generainforiesgos(dfecha_antier, dfecha_antier, cProducto) 
            RETURNING pcod_ret2;
			
			IF pcod_ret2 <> '00000' THEN
				let pcod_ret = '120'; -- // sp_generainforiesgos con problemas
				INSERT INTO sc_ctrlrepriesgos (proceso, status, codigo,  fecha_ejecucion)
				VALUES ('Reporte Riesgos', '0', pcod_ret, dfecha_hoy);
				RETURN pcod_ret;
			END IF;
			
			-- // Valida que antier es lunes
			IF weekday(dfecha_antier) = 0 THEN
				CALL sp_generareporte(cProducto) 
                RETURNING pcod_ret2;
			...
			CALL sp_generainforiesgos(dfecha_ayer, dfecha_antier, cProducto) 
            RETURNING pcod_ret2;
			...
			-- // Valida que ayer es lunes
			IF weekday(dfecha_ayer) = 0 THEN
				CALL sp_generareporte(cProducto) 
                RETURNING pcod_ret2;
				
				IF pcod_ret2 <> '00000' THEN
					let pcod_ret = '130'; -- // sp_generarepriesgos con problemas
					INSERT INTO sc_ctrlrepriesgos (proceso, status, codigo,  fecha_ejecucion)
					VALUES ('Reporte Riesgos', '0', pcod_ret, dfecha_hoy);
					RETURN pcod_ret;
				END IF;
			END IF;
		ELSE 
			CALL sp_generainforiesgos(dfecha_ayer, dfecha_ayer, cProducto) 
            RETURNING pcod_ret2;		
	...
	-- // Inserta un registro indicando que el proceso ya se corrio.
	INSERT INTO sc_ctrlrepriesgos (proceso, status, codigo,  fecha_ejecucion)
	VALUES ('Reporte Riesgos', '1', pcod_ret, dfecha_hoy);
...
'DESCRIPCION: Genera el reporte de riesgos, manda llamar sp_generainforiesgos y sp_generareporte.',
'Captacion',
'FECHA      : Marzo 2009',
'VERSION    : 20090312.1350',
'LLAMADO    : Se ejecuta al terminar el corte despues del cambio de fechas',
'**********************************************',
'MODIFICO   : ANTONIO BASTIDAS',
'DESCRIPCION: Optimizar las consultas accesando por indices nuevos para si_feriado, sc_ctrlrepriesgos y sc_param',
'FECHA      : Febrero 2010',
'VERSION    : 20100222.1136',
'BD         : BDICHEQ';
