CREATE PROCEDURE "informix".sp_aumento_disminucion_lincred_carga(o_empresa CHAR(3),Pusuario CHAR(20))
...
RETURNING CHAR(6),CHAR(80);
    ...
    ON EXCEPTION SET sql_err,isam_err,cMensaje
      LET cCodRet = sql_err;
--      TRUNCATE TABLE  bdicred:cambio_linea_masivo;
      RETURN cCodRet,cMensaje;
   END EXCEPTION;
   
--   SET DEBUG FILE TO "/RESPALDOSNEW/Israel/CAMBIO_LINEA.out";
--   TRACE ON;
    ...
    FOREACH WITH HOLD
        ...
        FROM bdicred:sd_cambio_linea_masivo
		WHERE bandera = 0
	
        begin work;

        Call bdicred:sp_actualiza_lincred_central_masivo(o_empresa, v_num_solicitud, v_monto_ahora, 'A','1', Pusuario) Returning cCodRet,cMensaje;

        IF (cCodRet = "000000") THEN
            update bdicred:sd_cambio_linea_masivo set bandera = 1 where num_solicitud = v_num_solicitud;
            commit work;
        ELSE
            rollback work;
        END iF;
...
'DESCRIPCION: Se creó el sp para modificar la linea de crédito en forma masiva.',
'Crédito',
'FECHA : Enero de 2009',
'VERSION: 200901',
'BD    : BDICRED',

'DESCRIPCION: Se quito la creacion de la tabla',
