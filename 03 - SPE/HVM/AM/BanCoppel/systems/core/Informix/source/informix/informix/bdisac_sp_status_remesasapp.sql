CREATE PROCEDURE "informix".sp_status_remesasapp()
--DATOS DE SALIDA							
RETURNING
	CHAR(5) AS CodigoRetorno, CHAR(100) AS Mensaje;
	
--DECLARACION DE VARIABLES
DEFINE iSqlErr           INTEGER;
DEFINE iIsamErr         	INTEGER;
DEFINE cCodRet        	CHAR(5);
DEFINE Mensaje_validacion CHAR(100);

--------VARIABLE REFERENCIA,FOLIOSUC Y STATUS---------
DEFINE creferencia1         CHAR(40);
DEFINE cStatus_cancelado    CHAR(1);
DEFINE cFolio_suc           CHAR(16);
DEFINE contador             INTEGER;

--------VARIABLE RUTA ARCHIVO .UNL-------------
DEFINE cRuta_documento           CHAR(50);

--------CADENA DE ARCHIVO .UNL------------
DEFINE cCadena               CHAR(1000);


--INICIALIZACION DE VARIABLES
LET cStatus_cancelado  ='S';
LET cRuta_documento = '/RESPALDOSNEW/remesa_aprizza/';
LET contador = 0;
...
--SET DEBUG FILE TO "/informix/JDFS/remesas.out";
--TRACE ON;
	...
	--CONTROL DE ERRORES 'INFORMIX' NO CONTROLADOS
		ON EXCEPTION SET iSqlErr, iIsamErr
			IF iSqlErr <> 0 THEN
				LET cCodRet = iSqlErr;
				LET Mensaje_validacion='Error no controlado';
				RETURN cCodRet,Mensaje_validacion;
			END IF;
		END EXCEPTION;


	   ON EXCEPTION IN(-668)
	   --DROP TABLE IF EXISTS temp_appriza_rem;
		  LET cCodRet ='00002';
		  LET Mensaje_validacion = 'Exito: el archivo no se encuentra en la ruta';
		  RETURN cCodRet, Mensaje_validacion;
	   END EXCEPTION WITH RESUME;



   --realizar la carga del archivo UNL--------
          LET cCadena = 'echo "DROP TABLE IF EXISTS temp_appriza_rem; \nCREATE TABLE "informix".temp_appriza_rem(referencia1  CHAR(40), folio_suc CHAR(16)); \nLOAD FROM ' || TRIM(cRuta_documento) || 'temp_appriza_rem.unl INSERT INTO "informix".temp_appriza_rem;">' || TRIM(cRuta_documento) || 'l_appriza_rem.sql';
		  SYSTEM cCadena;
		  LET cCadena = 'chmod 777 ' || TRIM(cRuta_documento) || 'l_appriza_rem.sql';
		  SYSTEM cCadena;
		  LET cCadena= 'dbaccess bdisac ' || TRIM(cRuta_documento) || 'l_appriza_rem.sql';
		  SYSTEM cCadena;
		  LET cCadena = 'rm -f ' || TRIM(cRuta_documento) || 'l_appriza_rem.sql';
          SYSTEM cCadena;

	 

 ---Validar si  el archivo  .UNL tiene informacion en caso de que no, elimina el archivo junto con la tabla temporal 

          select count(*)
          INTO contador
          from "informix".temp_appriza_rem;

         IF contador = 0 THEN
				

			  DROP TABLE IF EXISTS temp_appriza_rem;
			  LET cCadena = 'rm -f ' || TRIM(cRuta_documento) || 'temp_appriza_rem.unl';
		       SYSTEM cCadena;	
                 LET cCodRet = '00004'; 
                 LET Mensaje_validacion = 'EL ARCHIVO .UNL NO TIENE INFORMACIÃN';
                 RETURN cCodRet, Mensaje_validacion;
       ...
       --CAMBIAR ESTATUS DE LAS REFERENCIAS CARGADOS EN EL UNL

      FOREACH WITH HOLD

            SELECT referencia1,folio_suc
            INTO creferencia1,cfolio_suc
            FROM  bdisac:"informix".temp_appriza_rem

            UPDATE bdisac:"informix".sac_movimientoshistorial SET status_cancelado = cStatus_cancelado 
            WHERE referencia1 =creferencia1 and folio_suc=cfolio_suc;

		  LET cCodRet = '00000'; --Actualiza con exito.
		  LET Mensaje_validacion = 'Actualizado con exito';
           

       END FOREACH;

       ---ELIMINA LA TABLA TEMPORAL Y EL ARCHIVO.UNL 
...
END PROCEDURE
 DOCUMENT
'AUTOR: Jose Daniel Fonseca Salazar',
'Proyecto: Cambio de estatus de sobrantes Appriza para cobro de ventanilla',
'Solicito: Eduardo Pineda Guzman',
'Descripcion: Cambiar el Estatus de las remesas de appriza para que los usuarios puedan cobrar.',
'Fecha: 2023/07/03',
'Version: 20230703.0002',
'BD: BdiSac';
