CREATE PROCEDURE "informix".sp_actualiza_direcciones(pempresa CHAR(3))
    ...
    ON EXCEPTION SET sql_err, isam_err
        SET DEBUG FILE TO "/resplogifx/conciliachq/sp_actualiza_direcciones.err";
        TRACE ON;
        IF sql_err <> 0 THEN
            LET vcodret = sql_err;
    ...
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/sp_actualiza_direcciones.out";
    --- TRACE ON;
    ...
    FOREACH WITH HOLD
        SELECT numcte, tipo_dir, secuencia
          INTO vnumcte, vtipo_dir, vsecuencia_tmp
        ...
        IF vsecuencia <> vsecuencia_tmp THEN
        
            SELECT calle, colonia, entre_calles, pais, estado, ciudad, municipio, 
              ...
              INTO vcalle, vcolonia, ventre_calles, vpais, vestado, vciudad, vmunicipio, 
             ...
             WHERE numcte = vnumcte
               AND tipo_dir = vtipo_dir
               AND secuencia = vsecuencia_tmp;
               
               UPDATE {+INDEX(bdinteg:si_direcciones_actual idx_diract_ctetpo)}
                      bdinteg:si_direcciones_actual
                  SET secuencia        = vsecuencia_tmp,
                      calle            = vcalle, 
                      colonia          = vcolonia, 
                      entre_calles     = ventre_calles,
                      pais             = vpais,
                      estado           = vestado,
                      ciudad           = vciudad,
                      municipio        = vmunicipio,
                      cod_postal       = vcod_postal,
                      apart_postal     = vapart_postal, 
                      tipo_telef1      = vtipo_telef1,
                      telefono1        = vtelefono1,
                      tipo_telef2      = vtipo_telef2,
                      telefono2        = vtelefono2,
                      tipo_telef3      = vtipo_telef3,
                      telefono3        = vtelefono3,
                      extension        = vextension,
                      estado_inegi     = vestado_inegi,
                      municipio_inegi  = vmunicipio_inegi,
                      localidad_inegi  = vlocalidad_inegi,
                      numerociudad     = vnumerociudad,
                      numeroextcalle   = vnumeroextcalle,
                      numerointcalle   = vnumerointcalle,
                      departamento     = vdepartamento,
                      numerocalle      = vnumerocalle,
                      numerocolonia    = vnumerocolonia,
                      puntocardinal    = vpuntocardinal,
                      unidadhabitac    = vunidadhabitac,
                      manzana          = vmanzana,
                      otros            = votros,
                      andador          = vandador,
                      etapa            = vetapa,
                      ...
                      user_insert      = vuser_insert,
                      fecha_insert     = vfecha_insert,
                      ind_cofeteltel1  = vind_cofeteltel1,
                      ind_cofeteltel2  = vind_cofeteltel2,
                      ind_cofeteltel3  = vind_cofeteltel3
                WHERE numcte = vnumcte
                  AND tipo_dir = vtipo_dir;
        END IF;   
