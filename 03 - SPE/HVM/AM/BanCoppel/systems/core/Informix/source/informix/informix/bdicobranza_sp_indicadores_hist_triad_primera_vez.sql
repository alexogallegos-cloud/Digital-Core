CREATE PROCEDURE "informix".sp_indicadores_hist_triad_primera_vez(pTipoProceso CHAR(1))
RETURNING CHAR(6)  AS codigo_retorno,
          CHAR(80) AS mensaje_retorno;          

-- pTipoProceso = '1' Carga Plazo
-- pTipoProceso = '2' Carga Revolvente
...
DEFINE iMaxMoraHist			SMALLINT;
DEFINE iNumConveniosHist 	SMALLINT;
DEFINE iNumPagosHist		SMALLINT;
DEFINE dSdoMaxHist			DECIMAL(18,2);
DEFINE iPeorMora12m			INTEGER;
DEFINE dFechaPrimeraMora   	DATE;	
...
LET cErrorInfo	= '';
LET cCodRet		= '000000';
LET cMensajeRet	= 'INSERCION EXITOSA.';
...
--        LET cMensajeRet = cErrorInfo;
	...
	--SET DEBUG FILE TO '/descarga_info/aacano/triad_unica_vez/sp_indicadores_hist_triad_primera_vez.out';
	--TRACE ON;
...
IF pTipoProceso = '1' THEN	
	CALL bdicobranza:"informix".sp_inserta_bitacora_cob(cEmpresa, cProceso, '000000', 'Inicio Insercion de Cuentas Revolventes', '02') Returning cCodRetBitacora; 	
	
	FOREACH WITH HOLD			
		SELECT NVL(b.num_credito,0),NVL(b.max_mora_hist,0),NVL(b.num_convenios_hist,0),NVL(b.num_pagos_hist,0),NVL(b.saldo_maximo_hist,0),NVL(b.peor_mora_12m,0),NVL(b.fecha_primera_mora,0)
		INTO cNumCredito,iMaxMoraHist,iNumConveniosHist,iNumPagosHist,dSdoMaxHist,iPeorMora12m,dFechaPrimeraMora	
		FROM bdicred:sd_indicador_cred a
		JOIN bdicobranza:cb_triad_indicadores_unica_vez b ON b.num_credito=a.num_credito
		
		BEGIN;	
			UPDATE bdicred:sd_indicador_cred
			SET	max_mora_hist		=	CASE WHEN iMaxMoraHist>NVL(max_mora_hist,0) THEN iMaxMoraHist ELSE max_mora_hist END,
				num_convenios_hist 	=	NVL(num_convenios_hist,0)+iNumConveniosHist,
				num_pagos_hist		=	NVL(num_pagos_hist,0)+iNumPagosHist,
				saldo_maximo_hist	=	CASE WHEN dSdoMaxHist>NVL(saldo_maximo_hist,0) THEN dSdoMaxHist ELSE saldo_maximo_hist END,
				peor_mora_12m		=	CASE WHEN iPeorMora12m>NVL(peor_mora_12m,0) THEN iPeorMora12m ELSE peor_mora_12m END,
				fecha_primera_mora  =	CASE WHEN dFechaPrimeraMora>NVL(fecha_primera_mora,DATE(1)) THEN dFechaPrimeraMora ELSE fecha_primera_mora END
    ...
    LET cMensaje = trim(cMensaje) ||'    Total Cuentas Revolventes Insertadas : ' || iTotalCuentasInsertadas;
	CALL bdicobranza:"informix".sp_inserta_bitacora_cob(cEmpresa, cProceso, '000000', trim(cMensaje), '02') Returning cCodRetBitacora;
	CALL bdicobranza:"informix".sp_inserta_bitacora_cob(cEmpresa, cProceso, '000000', 'Fin Insercion de Plazo', '02') Returning cCodRetBitacora;
	LET cMensajeRet = trim(cMensajeRet) || ' ' || iTotalCuentasInsertadas || ' Cuentas Revolventes Insertadas . ';

-- PLAZO
ELIF pTipoProceso = '2' THEN	
	CALL bdicobranza:"informix".sp_inserta_bitacora_cob(cEmpresa, cProceso, '000000', 'Inicio de Insercion de Cuentas a Plazo', '02') Returning cCodRetBitacora;
	
	FOREACH WITH HOLD			
		SELECT NVL(b.num_credito,0),NVL(b.max_mora_hist,0),NVL(b.num_convenios_hist,0),NVL(b.num_pagos_hist,0),NVL(b.saldo_maximo_hist,0),NVL(b.peor_mora_12m,0),NVL(b.fecha_primera_mora,0)
		INTO cNumCredito,iMaxMoraHist,iNumConveniosHist,iNumPagosHist,dSdoMaxHist,iPeorMora12m,dFechaPrimeraMora	
		FROM bdicred:sd_indicador_cred_crd a
		JOIN bdicobranza:cb_triad_indicadores_unica_vez b ON b.num_credito=a.num_credito
		
		BEGIN;		
			UPDATE bdicred:sd_indicador_cred_crd
			SET	max_mora_hist		=	CASE WHEN iMaxMoraHist>NVL(max_mora_hist,0) THEN iMaxMoraHist ELSE max_mora_hist END,
				num_convenios_hist 	=	NVL(num_convenios_hist,0)+iNumConveniosHist,
				num_pagos_hist		=	NVL(num_pagos_hist,0)+iNumPagosHist,
				saldo_maximo_hist	=	CASE WHEN dSdoMaxHist>NVL(saldo_maximo_hist,0) THEN dSdoMaxHist ELSE saldo_maximo_hist END,
    ...
    LET cMensaje = trim(cMensaje) ||'    Total cuentas Plazo Insertadas : ' || iTotalCuentasInsertadas;
	CALL bdicobranza:"informix".sp_inserta_bitacora_cob(cEmpresa, cProceso, '000000', trim(cMensaje), '02') Returning cCodRetBitacora;
	CALL bdicobranza:"informix".sp_inserta_bitacora_cob(cEmpresa, cProceso, '000000', 'Fin Insercion de Cuentas a Plazo', '02') Returning cCodRetBitacora;
	LET cMensajeRet = trim(cMensajeRet) || ' ' || iTotalCuentasInsertadas || ' Cuentas Plazo insertadas. ';
ELSE
	LET cCodRet		= '000100';
...
END PROCEDURE
DOCUMENT 
'Proceso que inserta datos por una unica ocasiÃ³n en la tabla sd_indicador_cred y sd_indicador_cred_crd con informaciÃ³n historica',
'AUTOR : ',
