CREATE PROCEDURE "informix".sp_soc_combocc_atm2(pRegistros INTEGER, pRecuperacion INTEGER)
...
-- SET DEBUG FILE TO '/tmp/mfinis/sp_soc_combocc_atm2.out';
-- TRACE ON;
...
foreach
	...
	AND sucursal NOT IN (SELECT cc FROM bdisuc:"informix".ss_relacionccid)
...
'DESCRIPCION: Clon de SP productivo, Se agrega parametros de paginado',
'BD: bdisuc';
...
--   set debug file to "/pisa/pisabanco/pisa_ftes/pasecont.out";
--   trace on;
...
let pccssubsub = pccssubsub;
  ...
  -- let vw_ccsssubsub   = pccssubsub[5,6]; Sobra Mau
...
-- Verifica si la poliza se ha transmitido ya en base a la secuencia.
   ...
   if (psecuencia = 1) then 
		select COUNT(*) INTO variable from bdicont:co_poldet where usuario = psucursal 
                and fecha_captura = pfecha_hoy and moneda = pmoneda
                and fecha_valida = pfecha_valor;
      if (variable > 0) then
...
-- *****************************************************************
   ...
   if pindicador = "1" then
      execute procedure bdicont:auditapase(pfecha_hoy,pempresa,vw_usuario,pfecha_valor)
              into cod_ret;
   end if
...
'FECHA:       	08/10/2019',
'DESCRIPCIÓN: 	se agregan transacciones para realizar rollback en caso de algun error',
'BASE DE DATOS: bdisuc',
'FOLIO:628',
'Llamado desde:PaseATM.exe';

CREATE PROCEDURE "informix".sp_atms(psucursal  CHAR(256), pregistro SMALLINT) 
...
'AUTOR:Jesus Moreno', 
'FECHA:2019-09-27',
'DESCRIPCIIÓN: Se modifica procedimiento para que regrese los nombres de cajeros sin contemplar la plaza',
'SOLICITA: Gabriela Angulo';

CREATE PROCEDURE "informix".sp_faltsob01(pempresa CHAR(3),
		psucursal CHAR(4),
...
CREATE PROCEDURE "informix".sp_inserta_atm(pempresa CHAR(3),
                                           pcod_atm CHAR(4), 
                                           pdivisa  CHAR(2), 
                                           pcant_1  FLOAT,
                                           pcant_2  FLOAT,
...
SELECT COUNT(*) INTO variable FROM bdisuc:"informix".ss_atm WHERE cod_atm = pcod_atm;
IF (variable > 0) THEN
	
	SELECT COUNT(*) INTO variable1 FROM bdisuc:"informix".ss_atm_rec WHERE cod_atm = pcod_atm;
	IF (variable1 > 0) THEN
		SELECT  cantidad_1,cantidad_2,cantidad_3,cantidad_4,cantidad_5,cantidad_6,saldo_anterior,saldo_total 
		INTO vcant_1,vcant_2,vcant_3,vcant_4,vcant_5,vcant_6,vmonto_ant,vmonto
		FROM  ss_atm WHERE cod_atm = pcod_atm;
		
		UPDATE bdisuc:"informix".ss_atm_rec SET cantidad_1 = vcant_1,cantidad_2 = vcant_2,cantidad_3 = vcant_3, cantidad_4 = vcant_4,cantidad_5 = vcant_5,cantidad_6 = vcant_6,
		saldo_anterior = vmonto_ant,saldo_total = vmonto
...
ELSE
	...
	SELECT COUNT(*) INTO variable2 FROM bdisuc:"informix".ss_atm_rec WHERE cod_atm = pcod_atm;
	IF (variable2 > 0) THEN
		UPDATE bdisuc:"informix".ss_atm_rec SET cantidad_1 = vcant_1,cantidad_2 = vcant_2,cantidad_3 = vcant_3, cantidad_4 = vcant_4,cantidad_5 = vcant_5,cantidad_6 = vcant_6, saldo_anterior = vmonto_ant,saldo_total = vmonto
		WHERE empresa = pempresa AND cod_atm = pcod_atm;
	END IF;
...
CREATE PROCEDURE "informix".sp_monitor_atm01(pempresa      CHAR(3),
                                           pplaza        CHAR(3),
                                           psucursal     CHAR(4),
                                           pregistro     SMALLINT,
                                           pfinicio      DATE,
