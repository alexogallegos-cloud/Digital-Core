CREATE PROCEDURE "informix".sp_comisionxapertura_contable_fin(pempresa CHAR(3),pfecha DATE)
...
--    SET DEBUG FILE TO "/informix/mahr/sp_comisionxapertura_contable_fin.out";
--    TRACE ON;
...
-- **********************************************************************
   ...
   FOREACH WITH HOLD
	   ...
	   WHERE empresa = pempresa
		...
		AND proceso_comision IN ('','APERTURA')
		...
		EXECUTE PROCEDURE "informix".genmov	(pEmpresa,cNumCred,cProducto,97,'339',pfecha,mMonto2,cFolioSuc,cSucursal,cDivisa,'8073')
		INTO cod_ret, vMensaje;
		
		IF cod_ret::INTEGER = 0 THEN
			UPDATE "informix".sd_comision_x_apertura_contable
			SET monto_aplicado = CASE WHEN monto_aplicado = 4.16 THEN mMontoApli + mMonto2 ELSE monto_aplicado + mMonto2 END , 
