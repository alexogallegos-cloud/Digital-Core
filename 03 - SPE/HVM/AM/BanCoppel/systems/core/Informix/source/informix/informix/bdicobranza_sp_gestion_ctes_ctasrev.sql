CREATE PROCEDURE "informix".sp_gestion_ctes_ctasrev(pTipoEjecucion CHAR(01), pDiaEjecucion CHAR(02))
...
--SET DEBUG FILE TO "sp_gestion_ctes_ctasrev.out";
--TRACE ON;
...
LET cCodRet           	= '000000';
LET P_MENSAJE           = 'El proceso de GESTION CTES. CTAS. REV. se ejecuto correctamente.';
LET cMensaje			= '';
...
-----------------------------------------------------------------
	SELECT 
	  ...
	  FROM "informix".cb_vencidos_findemes_tdc ven
	 INNER JOIN bdicred:"informix".sd_maecred mae ON mae.num_credito = ven.num_credito
	 INNER JOIN bdicred:"informix".sd_maesdos mas ON mas.num_credito = ven.num_credito AND mas.mto_fin_ven_trasp >= 7
	 ...
	 WHERE ven.fecha = dFechaaProcesar
	   AND ven.campania = 'TC_VTAC7PV'
	   AND ven.procesar = '0'
	INTO TEMP ctes_ctasrev WITH NO LOG;

	CREATE INDEX ind_vencido_tmprev ON ctes_ctasrev(fecha);
	UPDATE STATISTICS MEDIUM FOR TABLE ctes_ctasrev;
	...
	FOREACH WITH HOLD
		SELECT fecha,campania,num_credito,numcte,procesar,fecha_vencido,status_cred,mto_venc_trasp,sdo_vencido,mto_fin_ven_trasp,sdo_cap_insoluto,
		  ...
		  INTO dFecha,cCampania,cNumCredito,cNumcte,cProcesar,dFechaVencido,cStatusCred,dSdoVencido,dSdoVencidoAct,sNumVencidos,dSdoInsoluto,dSdoTotalLiquidar
		  FROM ctes_ctasrev WHERE fecha = dFechaaProcesar
...
-----------------------------------------------------------------
	SELECT 
	  ...
	  FROM "informix".cb_vencidos_findemes_tdc ven
	 INNER JOIN bdicred:"informix".sd_maecred mae ON mae.num_credito = ven.num_credito
	 INNER JOIN bdicred:"informix".sd_maesdos mas ON mas.num_credito = ven.num_credito
	 INNER JOIN bdinteg:"informix".si_sucursales suc ON suc.empresa = mae.empresa AND suc.sucursal = mae.sucursal
	 WHERE ven.fecha = dFechaaProcesar
	   AND ven.campania = 'TC_VTACCON'
	   AND ven.procesar = '0'
	INTO TEMP ctes_ctasrev WITH NO LOG;
...
-----------------------------------------------------------------
	...
	SELECT 
	  ...
	  FROM "informix".cb_vencidos_findemes_tdc ven
	 INNER JOIN bdicred:"informix".sd_maecred mae ON mae.num_credito = ven.num_credito
	 INNER JOIN bdicred:"informix".sd_maesdos mas ON mas.num_credito = ven.num_credito AND mas.mto_fin_ven_trasp >= pVen
	 INNER JOIN bdinteg:"informix".si_sucursales suc ON suc.empresa = mae.empresa AND suc.sucursal = mae.sucursal
	 WHERE ven.fecha = dFechaaProcesar
	   AND ven.campania = 'TC_VTAC7PV'
...
-----------------------------------------------------------------
	SELECT 
	  ...
	  FROM "informix".cb_vencidos_findemes_tdc ven
	 INNER JOIN bdicred:"informix".sd_maecred mae ON mae.num_credito = ven.num_credito
	 INNER JOIN bdicred:"informix".sd_maesdos mas ON mas.num_credito = ven.num_credito AND mas.mto_fin_ven_trasp = 6
	 INNER JOIN bdinteg:"informix".si_sucursales suc ON suc.empresa = mae.empresa AND suc.sucursal = mae.sucursal
	 WHERE ven.fecha = dFechaaProcesar
	   AND ven.campania = 'TC_VTAC6PV'
	   AND ven.procesar = '0'
	INTO TEMP ctes_ctasrev WITH NO LOG;
...
-----------------------------------------------------------------
	SELECT 
	  ...
	  FROM "informix".cb_vencidos_findemes_tdc ven
	 INNER JOIN bdicred:"informix".sd_maecred mae ON mae.num_credito = ven.num_credito
	 INNER JOIN bdicred:"informix".sd_maesdos mas ON mas.num_credito = ven.num_credito AND mas.mto_fin_ven_trasp = 2
	 INNER JOIN bdinteg:"informix".si_sucursales suc ON suc.empresa = mae.empresa AND suc.sucursal = mae.sucursal
	 WHERE ven.fecha = dFechaaProcesar
	   AND ven.campania = 'TC_CAIDA2M'
	   AND ven.procesar = '0'
	INTO TEMP ctes_ctasrev WITH NO LOG;
...
-----------------------------------------------------------------
	SELECT 
	  ...
	  FROM "informix".cb_vencidos_findemes_tdc ven
	 INNER JOIN bdicred:"informix".sd_maecred mae ON mae.num_credito = ven.num_credito
	 INNER JOIN bdicred:"informix".sd_maesdos mas ON mas.num_credito = ven.num_credito AND mas.mto_fin_ven_trasp = 1
	 INNER JOIN bdinteg:"informix".si_sucursales suc ON suc.empresa = mae.empresa AND suc.sucursal = mae.sucursal
	 WHERE ven.fecha = dFechaaProcesar
	   AND ven.campania = 'TC_CAIDA1M'
	   AND ven.procesar = '0'
	INTO TEMP ctes_ctasrev WITH NO LOG;
