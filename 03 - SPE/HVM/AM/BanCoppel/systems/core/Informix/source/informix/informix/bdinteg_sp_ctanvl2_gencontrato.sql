CREATE PROCEDURE "informix".sp_ctanvl2_gencontrato( pNumCte CHAR(20),pNumCta CHAR(20) )
RETURNING CHAR(5)   AS codret,
          CHAR(100) AS prod_cap,
          CHAR(100) AS prod_nom,
          CHAR(100) AS prod_gen,
    ...
    ON EXCEPTION SET iSqlErr, iSamErr, cDesErr
        SET DEBUG FILE TO '/RESPALDOSNEW/DoctosCtaNvl2/sp_ctanvl2_gencontrato.err';
        TRACE ON;
        IF iSqlerr <> 0 THEN
            LET cCodRet  = iSqlErr;
            LET cCodRet2 = iSamErr;
            LET cCodRet3 = cDesErr;
            RETURN cCodRet, cProdCap, cProdNom, cProdGen, cNombreCte, cFecha, cSucursal, cProducto;
        END IF;
    END EXCEPTION;
    
    --- SET DEBUG FILE TO '/resplogifx/conciliachq/sp_ctanvl2_gencontrato.out';
    --- TRACE ON;
    ...
    SELECT TRIM(TRIM(cte.nombre1)||' '||TRIM(cte.nombre2))||' '||TRIM(cte.apell_paterno)||' '||TRIM(cte.apell_materno) 
     INTO cNombreCte
      FROM bdinteg:si_cliente AS cte, 
           bdinteg:si_sucursales AS suc
     WHERE cte.numcte = pNumCte
       AND suc.sucursal = cte.sucursal;
    
    SELECT pro.nombre, noc.fecha_alta, mae.sucursal
      INTO cProducto, dtFechaAlta, cmsucursal
      FROM bdicheq:sc_maechq AS mae, 
           bdicheq:sc_producto AS pro,
           bdicheq:sc_maenoc AS noc
     WHERE mae.cuenta = pNumCta 
       AND mae.producto = pro.producto
       AND noc.cuenta = mae.cuenta;
    
    SELECT suc.sucursal||' '||TRIM(suc.nombre)
      into  cSucursal
      FROM  bdinteg:si_sucursales AS suc
      WHERE sucursal = cmsucursal
      AND empresa = '001';


    -- // ASIGNACIï¿½N DE VALORES
    --- LET cFecha = '';
    LET cFecha = TO_CHAR(dtFechaAlta, '%d/%m/%Y');
...
'FECHA: 15/07/2020',
'DESCRIPCION: SPL encargado de consultar el detalle de la informacion que sera implementada para la generacion del contrato.',
'BD: bdinteg',
'AUTOR: Josï¿½ Alfredo Rosete Mazahua',
'FECHA: 27/03/2024',
'DESCRIPCION: Se modifica la sucursal para asignarle la de sc_maechq y mostrarla en el contrato de los documentos',
'BD: bdinteg';

CREATE PROCEDURE "informix".sp_ctanvl2_genportada( pNumCte CHAR(20), pNumCta CHAR(20) )
RETURNING CHAR(5)   AS codret,
          CHAR(40)  AS producto,
          CHAR(107) AS nombre_cte,
          ...
          CHAR(13)  AS rfc,
          CHAR(50)  AS sucursal,
          CHAR(30)  AS operacion,
          CHAR(30)  AS fecha_ope,
          CHAR(20)  AS folio_ope,
          CHAR(20)  AS cuenta,
          CHAR(20)  AS cta_clabe,
          CHAR(104) AS titular,
          CHAR(2)   AS autoriza_revoc,
          CHAR(100) AS reca,
          CHAR(4)   AS prod;
    ...
    ON EXCEPTION SET iSqlErr, iSamErr, cDesErr
        SET DEBUG FILE TO '/RESPALDOSNEW/DoctosCtaNvl2/sp_ctanvl2_genportada.err';
        TRACE ON;
        IF iSqlerr <> 0 THEN
            LET cCodRet  = iSqlErr;
            LET cCodRet2 = iSamErr;
            LET cCodRet3 = cDesErr;
            RETURN cCodRet, cProducto, cNombreCte, cFechaNac, cRfc, cSucursal, cOperacion, cFechaOpe, cFolioOpe, cCuenta, cCtaClabe, cTitular, cAutorizaRevoc, cReca, cProd;
        END IF;
    END EXCEPTION;
    
    --- SET DEBUG FILE TO '/resplogifx/conciliachq/sp_ctanvl2_genportada.out';
    --- TRACE ON;
    ...
    SELECT pro.nombre, mae.cuenta, mae.cuenta_clabe, pro.producto, noc.fecha_alta , mae.sucursal
      INTO cProducto, cCuenta, cCtaClabe, cProd, dtFechaAlta, cmsucursal
      FROM bdicheq:sc_maechq AS mae, 
           bdicheq:sc_producto AS pro,
           bdicheq:sc_maenoc AS noc
     WHERE mae.cuenta = pNumCta 
       ...
       SELECT TRIM(TRIM(cte.nombre1)||' '||TRIM(cte.nombre2))||' '||TRIM(cte.apell_paterno)||' '||TRIM(cte.apell_materno), cte.rfc, cpf.fecha_nac
      INTO cNombreCte, cRfc, cFechaNac
      FROM bdinteg:si_cliente AS cte, 
           bdinteg:si_ctepf AS cpf,
           bdinteg:si_sucursales AS suc
     WHERE cte.numcte = pNumCte
    ...
    LET cOperacion = 'Apertura de Cuenta';
    --- LET cFechaOpe = TO_CHAR(SYSDATE, 'DD/MM/YYYY HH:MI:SS am');
    --- LET cFechaOpe = TO_CHAR(CURRENT, '%d/%m/%Y %H:%M:%S');
    LET cFechaOpe = TO_CHAR(dtFechaAlta, '%d/%m/%Y')||' '||'12:00:00';
    LET cFolioOpe = 'P '||TRIM(cCuenta);
    LET cTitular = cNombreCte;
    LET cAutorizaRevoc = 'SI';
...
'FECHA: 13/10/2021',
'DESCRIPCION: Se actualizara la fecha de la operaciÃ³n para tomar la de BD.',
'BD: bdinteg',
'AUTOR: Jorge Ivan Camacho Sanchez',
'FECHA: 24/04/2023',
'DESCRIPCION: Se modifica la fecha de la operaciÃ³n para asignarle la fecha de alta de la cuenta',
'BD: bdinteg',
'AUTOR: JosÃ© Alfredo Rosete Mazahua',
'FECHA: 27/03/2024',
'DESCRIPCION: Se modifica la sucursal para asignarle la de sc_maechq y mostrarla en la portada de los documentos',
'BD: bdinteg';
