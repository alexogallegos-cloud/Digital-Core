CREATE PROCEDURE "informix".sp_dskrgainfoperativa( pempresa CHAR(3) )
RETURNING CHAR(5); 
...
*/    
    ...
    -- // ####################################################################   REPORTES DIARIOS   ####################################################################
    -- // EJECUCIÃN DE SP QUE GENERA LOS REPORTES DIARIOS DE PORTABILIDAD DE NOMINA  
...
/*    IF vpri_hab_mes = vfecha_hoy THEN    
	   ...
	   UPDATE STATISTICS MEDIUM FOR TABLE ctaspag2;
        ...
        create index idxtmp_maechq1 on tmp_ctas(num_cte, producto) online;
