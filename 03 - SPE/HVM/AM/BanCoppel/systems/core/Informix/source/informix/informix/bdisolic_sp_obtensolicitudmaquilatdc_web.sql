CREATE PROCEDURE "informix".sp_obtensolicitudmaquilatdc_web(pempresa CHAR(3),
		pnumcte CHAR(20),
...
BEGIN
	...
	--SET DEBUG FILE TO '/informix/Malena/sp_obtensolicitudmaquilatdc.out';
	--TRACE ON;
	...
	IF ptpoejecucion IN ('1','2') THEN
			...
			IF ctpoenvio = 'D' THEN
				...
				IF vcodret <> '000' THEN
					LET cMensajeRet = 'Error al obtener la información del domicilio del Cliente';					 
			...
			END IF;
				...
				IF cCodRetTDif <> '000000' THEN
					LET vcodret = SUBSTRING (cCodRetTDif FROM 2 FOR 5);
				...
				*/					--	RQM 10 1224						 
				...
				For i = 1 To Length(cnombre_embosado) STEP 1
					 --Recorre la cadena leída del embozado y se divide en 4 campos
