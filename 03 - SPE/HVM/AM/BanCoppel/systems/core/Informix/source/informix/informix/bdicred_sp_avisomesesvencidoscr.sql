CREATE PROCEDURE "informix".sp_avisomesesvencidoscr(cEmpresa CHAR(3), cNumCredito CHAR(20))
    RETURNING
    CHAR(5) AS retorno,
    CHAR(2) AS aviso;

    DEFINE iSqlErr              INTEGER;
    DEFINE cAviso               CHAR(2);
    DEFINE iMesesVencidos       INTEGER;
    ...
    BEGIN
        ...
        IF iMesesVencidos = 0 THEN
            LET cAviso = '00';
        ELIF iMesesVencidos = 1 THEN
            LET cAviso = '01';
        ELIF iMesesVencidos = 2 THEN
            LET cAviso = '02';
        ELIF iMesesVencidos = 3 OR iMesesVencidos = 4 THEN
            LET cAviso = '03';
        ELSE
            LET cAviso = '04';
        END IF;
...
'VERSION: 20090416.1700',
'BD     : bdicred';

CREATE PROCEDURE "informix".sp_calculo_reserva_cierre(pEmpresa CHAR(3),dtFechaUltMes DATE)
RETURNING 
          CHAR(6)  AS resultado,
          CHAR(80) AS mensaje;
...
--SET DEBUG FILE TO "sp_calculo_reserva_cierre.out";
--TRACE ON;
...
/* SE ELIMINA TEMPORALMENTE PARA EJECUCION DE UNICA VEZ      

-- Se obtiene la fecha hoy del sistema.
 SELECT a.fecha_hoy,ult_dia_mes, ult_hab_mes, prox_fecha, pri_hab_mes
   INTO dtFechaHoy, dtFechaUltMes, vult_hab_mes, vprox_fecha, vpri_hab_mes
   FROM bdicred:sd_fechas a
  WHERE a.empresa = pempresa;

--rss
let dtFechaHoy = mdy('03','31','2009');
let dtFechaUltMes = mdy('03','31','2009');
let vult_hab_mes = mdy('03','31','2009');
let vprox_fecha = mdy('04','01','2009');
let vpri_hab_mes = mdy('03','01','2009');

 SELECT status_proc
   INTO vstatus_proc
   FROM bdinteg:sx_contproc
  WHERE empresa = pempresa 
    AND proceso = "califcart_corte" 
    AND status_proc = "F" 
    AND sistema = "06"  
    AND fecha = MDY(MONTH(dtFechaHoy),20,YEAR(dtFechaHoy));

   IF vstatus_proc IS NULL THEN
      LET cCodRet= '000001';
      LET cMensajeRet= 'No se ha ejecutado el proceso de reserva para el corte';
      RETURN cCodRet, cMensajeRet;
   END IF;
   ...
   SELECT status_proc
     ...
     FROM bdinteg:sx_contproc
    WHERE empresa = pempresa    
      and proceso = "califcart_cierre" 
      and sistema = "06"        
      and fecha   = dtFechaHoy;

   IF vstatus_proc IS NULL THEN
      INSERT INTO bdicred:sd_contproc (empresa, proceso, fecha, status_proc,
				ejecutivo, hora_inicio, hora_fin, cod_ret, mensaje)
		       VALUES  (pempresa, "califcart_cierre", dtFechaHoy, "I",
      ...
      INSERT INTO bdinteg:sx_contproc  (empresa, proceso, fecha, sistema, status_proc,
		       ...
		       VALUES  (pempresa, "califcart_cierre", dtFechaHoy, "06", "I",
  ...
  ELSE
   ...
   END IF;
*/
--rss esta tabla ya no existirá
/*
    -- Elimina el Movimiento Generado de la Calificacion anterior
    TRUNCATE TABLE "informix".sd_hist_reserva_cierre;

    -- Elimina el Movimiento del Dia en Historico
    DELETE FROM informix.sd_hist_reserva_cierre
          WHERE empresa = pEmpresa 
            AND year(fecha_insert) = Year(dtFechaHoy) 
            AND month(fecha_insert) = Month(dtFechaHoy);

    UPDATE STATISTICS MEDIUM FOR TABLE "informix".sd_hist_reserva_cierre;
*/
-- Se obtienen los parametros correspondientes.

 LET dtFechaHoy = dtFechaUltMes;
...
FOREACH WITH HOLD
    ...
    SELECT a.num_credito, CASE WHEN b.num_periodos IS NULL THEN 0 ELSE b.num_periodos END, CASE WHEN b.pagos_realizados IS NULL THEN 0 ELSE b.pagos_realizados END, 
           ...
           b.probabilidad_incumplimiento, b.severidad_perdida, b.limite_credito, b.antecedente_buro,
           CASE WHEN 
           NVL(c.sdo_capital,0) + NVL(c.cap_tras_no_venci,0) + NVL(c.monto_vencido,0) + NVL(c.mto_venc_trasp,0) <= 0 THEN
           NVL(c.sdo_capital,0) + NVL(c.cap_tras_no_venci,0) + NVL(c.monto_vencido,0) + NVL(c.mto_venc_trasp,0) ELSE
           NVL(c.sdo_capital,0) + NVL(c.cap_tras_no_venci,0) + NVL(c.sdo_no_exig,0) + NVL(c.monto_vencido,0) + NVL(c.mto_venc_trasp,0)
           END,
           b.saldo_corte,
           d.dia_corte
...
--           left outer join bdicred:sd_hist_reserva b on a.empresa = b.empresa AND a.num_credito = b.num_credito AND b.fecha_corte = mdy(month(dtFechaHoy),20,year(dtFechaHoy))
           left outer join bdicred:sd_hist_reserva b on a.empresa = b.empresa AND a.num_credito = b.num_credito AND b.fecha_corte = mdy(month(dtFechaUltMes),20,year(dtFechaUltMes))
           join bdicred:sd_maesdoscont c on a.empresa = c.empresa  AND a.num_credito = c.num_credito and c.fecha = mdy(month(dtFechaUltMes),day(dtFechaUltMes),year(dtFechaUltMes))
--           join bdicred:sd_maesdoscont c on a.empresa = c.empresa  AND a.num_credito = c.num_credito and c.fecha = mdy(month(dtFechaHoy),day(dtFechaHoy),year(dtFechaHoy))
           join bdicred:sd_maecredanexo d on a.empresa = d.empresa AND a.num_credito = d.num_credito
     WHERE a.empresa = pEmpresa
            and a.fecha = mdy(month(dtFechaUltMes),day(dtFechaUltMes),year(dtFechaUltMes))
--            and a.fecha = mdy(month(dtFechaHoy),day(dtFechaHoy),year(dtFechaHoy))
--       and a.sucursal in ('0318') --rss
--       and a.sucursal in ('0002','0004','0192','0318') --rss
--        and a.num_credito='600009267441'
--and a.num_credito in ('600000356177','600000010279','600001188660','600001185765','600000006855','600002613963')
--     ORDER BY b.num_credito
     ...
     END IF;
/*
    -- Se obtiene el endeudamiento total a fin de mes.
    SELECT c.monto_otorgado, nvl(c.sdo_no_exig,"0"),  nvl(c.sdo_cap_insoluto,"0") + nvl(c.sdo_no_exig,"0"), c.sdo_capital + c.cap_tras_no_venci + c.sdo_no_exig + c.monto_vencido + c.mto_venc_trasp
      INTO dLinCredAut, vInteres_venc, vTotal, dEndeudamientoTotCierre 
      FROM bdicred:sd_maesdos c
     WHERE c.num_credito= cNumCredito
       AND c.empresa= pEmpresa;
*/

     -- Se hacen los cálculos de todas las variables a los créditos aperturados después del 20
     IF dMesesAntiguedad IS NULL THEN
        ...
        -- Se obtiene el límite de crédito
           SELECT monto_otorgado
            ...
            WHERE empresa = pempresa 
              AND fecha_ult_mov = dtFechaHoy
              AND num_credito = cNumCredito;
        -- Se obtiene los pagos realizados
           EXECUTE PROCEDURE bdicred:monthadd(dtFechaUltMes, -1) INTO dtFechaPeriodo;
--           EXECUTE PROCEDURE bdicred:monthadd(dtFechaHoy, -1) INTO dtFechaPeriodo;
       -- Se determina el endeudamiento total (saldo)
       -- Se calcula % de Pago 
       -- Se calcula % de Uso 
           IF dEndeudamientoTotCierre > 0 THEN
--              LET dPorPago = dPagoRealizado / dEndeudamientoTotCierre; 
              LET dPorPago = 0;
...
--                   IF iACT < dConsComPI THEN
           LET iACT = 0;
           LET iHIST = 0;
           LET dPI = (1/(1 + EXP(-(dConsPI + (dConsACT * iACT) + (dConsHIST * iHIST) + (dConsANT * iANT) + (dConsPORPAGO * dPorPago) + (dConsPORUSO * dPorUso))))) * 100;
--                   ELSE
--                      LET dPI = dPIdefaul;
--                   END IF;
...
--                   IF iACT < dMeses THEN
           LET dSP = dConsSPMenor * 100;
--                   ELSE
--                      LET dSP = dConsSPMayor;
--                   END IF;
     END IF;

--Si es negativo o nulo el saldo, entonces se iguala a cero
--     IF dEndeudamientoTotCierre IS NULL or dEndeudamientoTotCierre < 0 THEN 
     IF dEndeudamientoTotCierre IS NULL THEN 
        LET dEndeudamientoTotCierre= 0;
     END IF;

     -- Se determina el endeudamiento total (saldo)
     -- Se calcula % de Uso al cierre
     IF dEndeudamientoTotCierre > 0 THEN
        LET dPorUso  = dEndeudamientoTotCierre / dLimiteCredito; 
     ELSE
        LET dPorUso  = dPorcUsoMin;
     END IF
     -- Valida rangos para %USO 
     IF dPorUso  > dConsMaxPorUso THEN LET dPorUso = dConsMaxPorUso; ELIF dPorUso < dConsMinPorUso THEN LET dPorUso = dConsMinPorUso; END IF 

     -- Se calcula la exposición al momento del incumplimiento(EI), utilizando la siguiente ecuación:
     IF dPorUso = 0 THEN
        LET dMax= 1;
     ELSE
     ...
     IF dEndeudamientoTotCierre > 0 THEN
        LET dEI = dEndeudamientoTotCierre * dMax;
     ELSE 
        LET dEI = (dPorSaldoMin * dLimiteCredito) * dMax;
     END IF;

     LET dPI = dPI / 100;
     LET dSP = dSP / 100;
      -- Se calcula el porcentaje de reserva y monto 
     LET dPorcentajeReserva = dPI * dSP;

--		LET dPorcentajeReserva = dPorcentajeReserva;

  -- Se calcula el porcentaje de reserva y monto para los CLIENTES NUNCA. 
     IF (dEndeudamientoTotCorte < 0 AND dPagos >= 0) OR (dEndeudamientoTotCorte = 0 AND dPagos >= 0) THEN
         LET dPorcentajeReserva = 0;
--Identificación de Clientes Nunca
         SELECT count(*)
           INTO dPorPago
           FROM bdicred:sd_maesdoshist a
          WHERE a.empresa = pEmpresa
            and a.fecha <= mdy(month(dtFechaUltMes),dDiaCorte,year(dtFechaUltMes))
--            and a.fecha <= mdy(month(dtFechaHoy),dDiaCorte,year(dtFechaHoy))
            AND (a.sdo_capital+a.cap_tras_no_venci+a.monto_vencido+a.mto_venc_trasp)=0
            AND NVL((select sum(monto) FROM sd_movhis WHERE empresa='001' AND num_credito=a.num_credito
            AND codigo_fun in ('033','334','335','336','337') AND codigo_ref=1 AND reversado='N' and fecha_mov <= mdy(month(dtFechaUltMes),dDiaCorte,year(dtFechaUltMes))),0)=0
            AND a.num_credito = cNumCredito;
            IF dPorPago > 0 THEN
               LET iTipoCliente = 1;
--               LET dPorcentajeReserva = dPorUsoMinCtesNunca;
            END IF;   
/*
     ELIF dEndeudamientoTotCorte = 0 AND dPagos = 0 THEN
--Identificación de Clientes Nunca y nuevos créditos
         SELECT count(*)
           INTO dPorPago
           FROM bdicred:sd_maecredcont a, bdicred:sd_maesdoscont b
          WHERE a.empresa = pEmpresa
            AND a.empresa = b.empresa 
            AND a.num_credito = b.num_credito
            and a.fecha = b.fecha
            and a.fecha = mdy(month(dtFechaUltMes),day(dtFechaUltMes),year(dtFechaUltMes))
            AND (b.sdo_capital+b.cap_tras_no_venci+b.monto_vencido+b.mto_venc_trasp)=0
            AND NVL((select sum(monto) FROM sd_movhis WHERE empresa='001' AND num_credito=a.num_credito
            AND codigo_fun in ('033','334','335','336','337') AND codigo_ref=1 AND reversado='N'),0)=0
            AND a.num_credito = cNumCredito;
            IF dPorPago > 0 THEN
               LET dPorcentajeReserva = dPorUsoMinCtesNunca;
            END IF;   
...
--Calcula la reserva
     ...
     ELSE
        IF dEndeudamientoTotCierre = 0 AND dPorPago = 0 THEN
           LET dResCalificacion = dPorUsoMinCtesNunca * dLimiteCredito;
        ELSE
           LET dResCalificacion = 0;
           LET dPorcentajeReserva = 0;
        END IF;
...
--Tratamiento Clientes Nunca y nuevos créditos
    IF iTipoCliente = 1 THEN
       LET dPorcentajeReserva = dPorUsoMinCtesNunca;
       LET dResCalificacion = dPorcentajeReserva * dLimiteCredito;
       LET dEI = dLimiteCredito;
    END IF;

   IF dPorcentajeReserva = 0 THEN
       LET dResCalificacion = 0;
   END IF
      ...
      -- Se obtiene el grado de riesgo del crédito (Calificación) en base al % de riesgo a constituir
        SELECT a.grado_riesgo
         ...
         WHERE (round(dPorcentajeReserva * 100,2) >= a.porcentaje_min  AND round(dPorcentajeReserva * 100,2) <= a.porcentaje_max);
         
        IF cGradoRiesgo= 'A' THEN
           LET vNvoPeriodo= 0;
        ELIF cGradoRiesgo= 'B1' THEN
           LET vNvoPeriodo= 1;
        ELIF cGradoRiesgo= 'B2' THEN
           LET vNvoPeriodo= 2;
        ELIF cGradoRiesgo= 'C' THEN
           LET vNvoPeriodo= 3;
        ELIF cGradoRiesgo= 'D' THEN
           LET vNvoPeriodo= 4;
        ELIF cGradoRiesgo= 'E' THEN
           LET vNvoPeriodo= 5;
        END IF;
    
--if cGradoRiesgo is null then let dEndeudamientoTotCierre=dPorcentajeReserva; end if;

     -- Determina la Periodicidad del Crédito.
        IF UPPER(cPeriodicidad) = "S" THEN
            IF iCuotasVdas > 18 THEN
                LET iCuotasVdas = 18;
            END IF
       ELIF UPPER(cPeriodicidad) = "Q" THEN
          IF iCuotasVdas > 13 THEN
            LET iCuotasVdas = 13;
          END IF
       ELIF UPPER(cPeriodicidad) = "M" THEN
          IF iCuotasVdas > 9 THEN
            LET iCuotasVdas = 9;
          END IF;
          -- No se toman los intereses en cuenta para creditos con mas de 1 pago vencido
          IF iCuotasVdas > 1 THEN
...
*/
        IF dMesesAntiguedad IS NOT NULL THEN
         -- Se almacena la información correspondiente al calculo de la reservas preventivas.
            ...
            WHERE empresa = pEmpresa
              AND num_credito = cNumCredito
              AND fecha_corte = mdy(month(dtFechaHoy),20,year(dtFechaHoy));
        ELSE 
-- Valida duplicidades para créditos creados después del 20 (temporal para reprocesamiento en caso de truene)
            SELECT count(*) INTO sExisten FROM informix.sd_hist_reserva WHERE empresa = pEmpresa 
                AND fecha_corte = mdy(month(dtFechaUltMes),day(dtFechaUltMes),year(dtFechaUltMes)) AND num_credito = cNumCredito; 
-- Valida duplicidades para créditos creados después del 20 (temporal para reprocesamiento en caso de truene)

            IF sExisten = 0 OR sExisten IS NULL THEN 
...
*/
     -- Reservas por Riesgos Operativos (Clientes con mal Antecedentes en el circulo de Crédito)
      IF dEvaBuro = '1' THEN
        LET vImporteReservaBuroCC= dResCalificacion * 0.15;
      END IF;
...
*/
    ...
    -- Reservas por Intereses devengados sobre créditos vencidos.
     ...
     IF vStatusCred IN ('BT','E3') THEN
			IF vStatusCred = 'BT' THEN
               FOREACH 
                    SELECT FIRST 2 monto, codigo_ref
                    ...
                    ORDER BY fecha_mov DESC
                       ...
                       LET vIntCapitalizado = vIntCapitalizado + vmonto_capitalizado;
               END FOREACH;
			ELSE
               FOREACH 
                    SELECT FIRST 3 monto, codigo_ref
                      INTO vmonto_capitalizado, vcodigo_ref
                      FROM bdicred:sd_movhis
                     WHERE empresa = pEmpresa
                       AND num_credito = cNumCredito  
                       AND codigo_fun = '605' 
					   AND codigo_ref in (2,125,127)
                       AND fecha_mov >= date(0)
                       AND reversado = 'N'
                    ORDER BY fecha_mov DESC
					
						IF vcodigo_ref = 2 and iContInteres < 2 THEN
						   LET vtotal_capitalizado = vtotal_capitalizado + vmonto_capitalizado;
						   LET vIntCapitalizado = vIntCapitalizado + vmonto_capitalizado;
						   LET iContInteres = iContInteres + 1;
						ELIF vcodigo_ref <> 2 THEN
							LET vtotal_capitalizado = vtotal_capitalizado + vmonto_capitalizado;
						    LET vIntCapitalizado = vIntCapitalizado + vmonto_capitalizado;
						END IF;
               END FOREACH;
			...
			END IF;
               ...
               IF vtotal_capitalizado > 0 THEN
                    LET vtotal_capitalizado = vtotal_capitalizado * (1 - (dPorcentajeReserva / 100));
/* SE ELIMINA TEMPORALMENTE PARA EJECUCION DE UNICA VEZ      
                    EXECUTE PROCEDURE genmov_hist(pEmpresa,
                                                  cNumCredito,
                                                  vProducto,
...
*/
     ...
     -- Se almacena la información correspondiente al calculo de la reservas por intereses devengados de créditos vencidos
     UPDATE informix.sd_hist_reserva
        SET 
            reserva_int_cred_ven      = vtotal_capitalizado,
            interes_cred_ven          = vIntCapitalizado
...
CREATE PROCEDURE "informix".sp_calculo_reserva_corte_cnr(pEmpresa CHAR(3))
...
DEFINE cNumeroCreditoCalif		CHAR(20);
...
--SET DEBUG FILE TO "/RESPALDOSNEW/sp_calculo_reserva_corte_cnr.out";
--TRACE ON;
...
--IPCB 01/08/13: Se acortan los mensajes para no tener desfaces en la entrega de respuesta a CTRL-M
    --LET cMensajeRet              	= 'Error en la ejecuciÃÂ³n del proceso CALIFICACION DE CREDITOS NO REVOLVENTES  ' || cNumeroCredito;
...
LET cCodRet                  	= '000000';
--IPCB 01/08/13: Se acortan los mensajes para no tener desfaces en la entrega de respuesta a CTRL-M
--LET cMensajeRet              	= 'El proceso de CALIFICACION DE CREDITOS NO REVOLVENTES se realizÃÂ³ correctamente';
...
LET iNumSesion               	= 0;
LET cStatus_proc,cNumeroCredito,cStatusCred,cSucursal,cDivisa,iFrec_Pago,cCapitalStatus,cNumeroCreditoCalif           	= '','','','','','','','';
...
IF dMaxPI IS NULL THEN
   LET cCodRet= '000007';
   LET cMensajeRet= 'FALTA PARAMETRO PORCENTAJE MÃÂXIMO DE PROBABILIDAD DE INCUMPLIMIENTO';
   RETURN cCodRet, cMensajeRet;
END IF;
...
--       AND trim(a.campo_trab3)    <> 'BAJA' --No se califican los crÃÂ©ditos que sufren baja en la cartera 28/02/2014


SELECT d.num_credito into cNumeroCreditoCalif
FROM "informix".sd_hist_reserva_cnr d
WHERE d.empresa         = '001'
AND d.num_credito = cNumeroCredito
AND d.fecha_cierre = dtFechaHoy;  --MDY(MONTH(dtFechaHoy),LPAD(b.dia_corte,2,0),YEAR(dtFechaHoy))    ----HMBR  meter campo en  tabla  para el corte

if cNumeroCreditoCalif is null or cNumeroCreditoCalif = '' then let cNumeroCreditoCalif = '-1'; end if;
if cNumeroCreditoCalif != '-1' then continue foreach; end if;
...
--     END IF;
    ...
    ELIF cNumProducto = '6400' THEN 
        LET cTipoFactura = 'N';
    ELSE
        LET cMensajeRet = 'El crÃÂ©dito no tiene nÃÂºmero de producto  ' || cNumeroCredito;
        LET cCodRet = '900000';
        RETURN cCodRet,cMensajeRet;    
    END IF;
...
*/
-- Periodos de incumplimiento o dÃÂ­as de atraso (ATR)
...
--para credinomina mensual, los pagos son los dias 15 unicamente por lo que en fecha de apertura se debe de poner siempre 15 y mes y aÃÂ±o de apertura
...
--Pendiente de calcular ÃÂºnicamente para periodos semanales iMaxATR
...
--Reservas calificaciÃÂ³n de la cartera 
--    IF cNumProducto = '6300' THEN LET cCodigoFun = '090'; ELIF cNumProducto = '6400' THEN LET cCodigoFun = '110'; END IF;
-- Se elimina registro contable ya que se estÃÂ¡ generando nueva metodologia por parte de Riesgos (RQI 21 180 EliminaciÃÂ³n registro contable en CalificaciÃÂ³n TDC y PP) y este proceso se ejecutara en paralelo.
...
/* -- Se elimina registro contable ya que se estÃÂ¡ generando nueva metodologia por parte de Riesgos (RQI 21 180 EliminaciÃÂ³n registro contable en CalificaciÃÂ³n TDC y PP) y este proceso se ejecutara en paralelo.
    ...
    END IF;
*/ -- Se elimina registro contable ya que se estÃÂ¡ generando nueva metodologia por parte de Riesgos (RQI 21 180 EliminaciÃÂ³n registro contable en CalificaciÃÂ³n TDC y PP) y este proceso se ejecutara en paralelo.
--Reservas estimaciÃÂ³n preventiva 
...
-- Se elimina registro contable ya que se estÃÂ¡ generando nueva metodologia por parte de Riesgos (RQI 21 180 EliminaciÃÂ³n registro contable en CalificaciÃÂ³n TDC y PP) y este proceso se ejecutara en paralelo.
--    IF cStatusCred = 'BT' and dInteresVencido > 0 THEN  
       /*select codigo_fun into cCodigoFun 
       from bdicred:sd_producto_codigofun_cnr
