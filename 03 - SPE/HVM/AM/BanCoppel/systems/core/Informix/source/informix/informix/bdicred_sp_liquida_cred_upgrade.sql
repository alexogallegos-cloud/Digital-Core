CREATE PROCEDURE "informix".sp_liquida_cred_upgrade (P_EMPRESA CHAR(3) ,P_NUM_SOLICITUD  CHAR(20),P_FOLIO CHAR(20),pMonto DECIMAL(18,2))
...
--set debug file to "/home/sysifx/MarcoC/INFINITE/liquida_cred.out";
-- trace on;
...
LET dIntPeriodo      = 0;
 ...
 BEGIN
  ...
  --LEE LA INFORMACION DEL CREDITO
     SELECT num_producto, divisa
     INTO   vNumProducto, vdivisa
     FROM   sd_maecred
     WHERE  empresa     = P_EMPRESA AND
            num_credito = vNumCredOld;

     -- Respalda Credito a Liquidar MACM RQM 101584 TDC INFINITE
	LET g_NumCredito = vNumCredOld;
	LET g_Folio      = vFolio;
    ...
    SELECT sdo_capital + cap_tras_no_venci , monto_otorgado--Capitales
    INTO   vVigente,dMontoOtorgado
    FROM   sd_maesdos
    WHERE  empresa = P_EMPRESA AND
           NUM_credito = vNumCredOld;

		   	--movimiento por baja de la linea de credito por cambio de producto
   EXECUTE PROCEDURE genmov(p_empresa, vNumCredOld, vNumProducto, 2,
                            '008', vHoy, dMontoOtorgado, vFolio, vSucursal,
                            vdivisa, "0000"
...
IF pMonto > 0 THEN  
	--movimiento por salida de traspaso de saldo 
   EXECUTE PROCEDURE genmov(p_empresa, vNumCredOld, vNumProducto, 3,
                            '008', vHoy, pMonto, vFolio, vSucursal,
                            vdivisa, "0000"
                            ) INTO vCodRet, vMensaje;
...
/*
IF pMonto > 0 THEN  
   	--movimiento por baja de la linea vigente por cambio de producto
   EXECUTE PROCEDURE genmov(p_empresa, vNumCredOld, vNumProducto, 1,
                            '075', vHoy, pMonto, vFolio, vSucursal,
                            vdivisa, "0000"
                            ) INTO vCodRet, vMensaje;
...
END IF;*/
  ...
  -- SE LIQUIDA LA LINEA DE CREDITO COMO SALDADA NORMAL
		-- SE DEJAN COMO PAGADO LAS CUOTAS PENDIENTES DE CAPITAL
	...
	-- SE DEJAN LOS SALDOS DEL CREDITO EN 0
     UPDATE sd_maesdos
        SET  sdo_no_exig       = 0,
             sdo_exig_int      = 0,
             sdo_moratorio     = 0,
	...
	-- ACTUALIZA EL STATUS DEL CREDITO A CANCELADO NORMAL DE CENTRAL COMO SALDADO NORMAL
	-- JRVT 02/01/2024 SE COMENTA LA ACTUALIZACIÃN DEL CAMPO CRÃDITO EXTERNO.
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Procedimiento para liquidar el credito para el proceso de upgrade TDC Oro',
'AUTOR: Maria Elena Angulo Aispuro',
'FECHA: 15 Abril 2016',
'VERSION: 20160415.1645',
'BD: bdicred',
'MODIFICADO POR: Alejandro Cordova Ramirez',
'FECHA MODIFICACION: 11 Octubre 2021',
'DESCRIPCION DE MODIFICACION: Se realiza la modifiaciÃÂ³n para que se actualice el campo status_cred de la tabla sd_maecred con valor a MP cuando se cancele un crÃÂ©dito por upgrade',
