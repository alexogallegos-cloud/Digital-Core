CREATE PROCEDURE "informix".sp_consultacuentascliente_bis(
                                            pEmpresa char (3),
...
RETURNING
            ...
            char(20) AS numero_Cte, --numero de cliente
            char(26) AS ApellidoPat, --apellido paterno
            char(26) AS ApellidoMat, --apellido materno
            char(26) AS Nombre1, --nombre1
            char(26) AS Nombre2, --nombre2
            char(20) AS NumeroRefer, --numero de la referencia cliente
            char(13) AS NumeroTel, --numero de telefono
            char(20) AS NumeroCred, --numero de credito
            char(40) AS DescripCred, --descripcion del credito
            char(30) AS SituacEspec, --Situacion especial
            char(30) AS Causa, --Causa
            char(30) AS SaldoAct, --Saldo actual
            char(30) AS TipoConve, --Tipo convenio

            char(30) AS NumDiasVencid, --Numero de dias vencidos
            char(30) AS NunPagosVencid, --Numero de pagos vencidos
            char(30) AS PagoMinimVencid, --Pago minimo vencido
			char(1) AS EstatTarj,  --Estatus tarjeta
		    decimal(18,2) AS TotalLiquidar,--Total para liquidar
            char(30) AS ProxPagoVencer, --proximo pago por vencer
            char(30) AS FechaUltAbono, --fecha ultimo abono
            char(30) AS ImportUltimAbono, --importe ultimo abono
            char(30) AS FechaUltiConven, --fecha ultimo convenio
            char(30) AS ImportConvenio, --importe convenio
            char(3) AS CumplioConvenio, --cumplio convenio
            char(3) AS OrigenConvenio, --origen convenio
            decimal(14,2) AS MontoMinimNegociar; -- monto mÃ­nimo a negociar; 
...
-- ModificÃ³:
-- Viridiana Osobampo
-- Se modifica para retornar el monto mÃ­nimo a negociar para el convenio de pago,
-- ademÃ¡s de obtener los saldos del crÃ©dito mediante el llamado al spl 
-- sp_consulta_saldos_general en lugar de obtenerlos de forma directa a las tablas.

-- 21/08/2015
-- ModificÃ³: Viridiana Paredes R.
-- Se modifico para que regrese los siguientes campos status_tar y dTotal_liquidacion en el procedimiento.
-- Tambien se validara si el credito realizara convenios con la condicion del nuevo campo realizar_convenio.
-- base de datos: bdicobranza.
-- Folio: 1503


--SET DEBUG FILE TO '/home/sysifx/vlv/sp_consultacuentascliente.out';
--TRACE ON;
...
*/
            ...
            AND secuencia = (SELECT MAX(secuencia) FROM bdinteg:si_telefonos WHERE numcte = v_numcte AND tipo_tel = 1);
				...
				-- Valida si se tiene convenio con la condicion del nuevo campo DSB VP
						SELECT a.num_credito, a.num_producto
						INTO v_numcredito, v_num_producto
				...
				--Obtener el estatus de la tarjeta con el status_tar DSB VP
						SELECT status_tar 
						INTO cStatus_tar
						FROM bdicred:"informix".sd_tarjeta
						--WHERE  num_credito = v_numcredito
						--AND expiracion =(select max(expiracion) from bdicred:"informix".sd_tarjeta where num_credito = v_numcredito);
						WHERE empresa = pEmpresa 
						  AND num_credito = v_numcredito
                          AND secuencia = (SELECT max(secuencia) from bdicred:sd_tarjeta where num_credito = v_numcredito); -- 20200813 Macf
				
			
						EXECUTE PROCEDURE bdicred:"informix".sp_consulta_saldos_general(pEmpresa, v_numcredito) INTO
							cCod_ret,				cMensaje,				cNumCred,				cTipCred,				dtFecha_origen,
							dtfecha_prox_pago,		dPago_minimo,			dtFecha_ult_pago,		iPlazo,					iPagos_realizados,
							dLinea_otorgada,		dTasa_interes,			dTasa_mora,				dMonto_sbc,				dCap_vig,
							dCap_trans,				dCap_vdo_exig,			dCap_vdo_no_exig,		dSdo_act_total_cap,		dInt_vig,
							dInt_vdo,				dInt_moratorios,		dInt_mes,				dSdo_act_total_int,		dIva_int_vig,
							dIva_int_vdo,			dIva_int_moratorios,	dIva_int_mes,			dSdo_act_total_iva,		dCom_pend,
							dIva_com,				dSdo_retenido,			dTotal_liquidacion,/*CAMPO REQUERIDO*/
							dInt_devengado,			dIva_int_devengado,		dLinea_disponible,		dPagos_vdos,        	cDesc_status_cred,
							iId_bloqueo_cred,		cBloqueo_cta,			cId_causa_bloqueo_cred,	cCausa_bloqueo_cta,		cId_sit_esp_cte,
							iId_causa_esp_cte,		cSit_esp_cte,			cId_sit_esp_cred,		iId_causa_esp_cred,		cSit_esp_cred;
                       ...
                       LET v_fechaultabono      = dtFecha_ult_pago;                       
                        ...
                        EXECUTE PROCEDURE bdicobranza:sp_validavencidos_bis(pEmpresa, v_numcredito, pFecha)
                        ...
                        EXECUTE PROCEDURE "informix".sp_compac_monto_minimo_convenio_bis(pEmpresa, v_numcredito)
                        ...
                        RETURN v_codret, v_numcte, v_apaterno, v_amaterno, v_nombre1, v_nombre2, v_numcteref, v_telefono, v_numcredito, v_descripcioncred,
                                v_situacionespecial, v_causa, v_saldoactual, v_tipoconvenio,v_numdiasvencidos, v_numpagovencidos, v_pagominvencido,cStatus_tar,dTotal_liquidacion,
								v_proxpagoporvencer, v_fechaultabono, v_importeultabono,v_fechaultconvenio, v_importeconvenio, v_cumplioconvenio, v_origenconvenio,dMtoNegociar WITH RESUME;
            END FOREACH;
			--VP
	...
	-- Valida si se tiene convenio con la condicion del nuevo campo DSB VP
			...
			FROM bdicred:"informix".sd_maecred a, 
				 ...
				 bdicred:"informix".sd_maesdos c
			WHERE a.empresa = pEmpresa
			AND a.num_credito =  v_numcredito
			AND a.empresa = b.empresa 
			AND a.num_credito = c.num_credito 
            AND a.status_cred IN ('BA','BT','E1','E2','E3')
		    AND (c.monto_vencido + c.mto_venc_trasp) > 0
...
DOCUMENT
'Fecha: 2020-08-11',
'DescripciÃ³n: ModificaciÃ³n por error -284 en consulta status_tar de sd_tarjeta',
'Modifica: Marco A. Campos';
