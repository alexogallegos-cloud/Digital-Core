...
- sp_capabilities: 74,211 links · 11,360/12,812 SPs cubiertos (88.7%) · 61 L3 distintas · CTM_ENTRY=155 · CTM_HINT=18
...
# ── Paso 3: CTM entry points desde Excel (requiere source/controlm/*.xls) ────
python generators/load-ctm-to-brain.py
# Produce: ctm_jobs, batch_archetype=CTM_ENTRY para ~87 SPs matcheados por nombre de job
# PRESERVA: batch_archetype IN (CTM_ENTRY, CTM_HINT)
# SOBREESCRIBE: batch_archetype de contenido en SPs CTM confirmados

# ── Paso 3b: CTM entry points desde contenido real de .sh (alta confianza) ───
python generators/mark-ctm-entry-points.py
# Fuente: CTM brain.db::sh_sp_refs confidence='high' (execute procedure "informix".sp_name())
# Produce: batch_archetype=CTM_ENTRY para 68 SPs adicionales no capturados por nombre de job
# PRESERVA: batch_archetype IN (CTM_ENTRY, CTM_HINT)  — nunca sobreescribe marks existentes
# Total CTM_ENTRY post paso 3b: 155 SPs (87 Excel + 68 sh-content)
# Requiere: CTM brain.db en systems/integration/ControlM/digital-brain/ (rebuild CTM brain primero)
...
| `CTM_ENTRY` | `load-ctm-to-brain.py` (Excel) + `mark-ctm-entry-points.py` (sh-content) | No sobreescrito por pasos posteriores |
...
### Pipeline canónico de `business_name` — síntesis leída de código fuente (ADR-SPE-AM-010)

**Principio:** el extractor emite `business_name = ""`. El nombre se **infiere leyendo el código fuente completo del SP**, nunca se transcribe la fórmula ni se copia el comentario. Enriquecer, no transcribir. La síntesis LLM es la única fuente del campo.

**Fuente autoritativa = brain.db.** `rule_enrichment_log.method='llm_synthesis_source_read'` es el gold standard y sobreescribe el overlay `name-overrides-ai.json`. `build-brain.py` restaura desde el log (2 pasadas: llena vacías + override source_read), normaliza (elimina `(regulatorio:...)` redundante con la columna `reg`, fugas técnicas `bdixxx`/`sp_xxx`, capitaliza inicial).

**GOTCHA del portal:** `gen-rules-portal.py` lee `business-rules-v3.json`, NO brain.db. Ya hace overlay de brain.db en memoria, pero **siempre regenerar el portal tras cambiar brain.db**; `digital-brain/sync-brain-to-v3.py` materializa los nombres en v3.json si se necesita.

**Metodología de re-síntesis (scatter-gather):** exportar reglas a lotes de ~70-80 con su `source_file` (`source/informix/informix/{db}_{sp_name}.sql`); un agente por lote que LEE el SP completo con Read, NO sub-delega, y escribe un JSON `{"results":{id:name}}` alineado 1:1 a los IDs de entrada; colectar (dedup + guardia anti-genérico), aplicar como `source_read`, rebuild, sync, regenerar portal. Auditar con detectores (vacías, genéricos, tokens de código, plantillas, débiles) hasta 0 real; ojo con falsos positivos (`--` de comentario, "monto IVA", "(×1)" Luhn).

**LECCIÓN:** los nombres de fragmentos truncados tienen errores FACTUALES (código 439=crédito cancelado, ctemoral=persona moral, cteppes=PEP). Solo leer el fuente garantiza calidad. Detalle en memoria `methodology-source-read-synthesis`.

**Scripts eliminados (heurísticos, no recrear):** enrich-rules.py, enrich-capa-e.py, infer-rule-names.py, enrich-names-local.py, detect-shared-formulas.py, sync-brain-v15.py, swarm_r_exception_audit.py, swarm_u_abbrev_expansion.py.
