...
`Cheques / Cuentas` no puede cortar hasta que estos dominios estén disponibles como API:

| ID | Dominio | Llamadas | Estado |
|----|---------|---------|--------|
| D02 | Integración y Autenticación | 130 | API disponible como prereq |
| D08 | SPEI | 74 | API disponible como prereq |
| D09 | Mensajería | 72 | API disponible como prereq |
| D03 | Créditos | 45 | API disponible como prereq |

## Dominios bloqueados por `bdicheq`

Estos dominios no pueden migrar hasta que `bdicheq` complete el cutover:

| ID | Dominio | Llamadas | Impacto |
|----|---------|---------|---------|
| D01 | Canal Digital Web | 2,346 | No puede migrar hasta que `bdicheq` esté listo |
| D05 | Saldos y Cuentas | 331 | No puede migrar hasta que `bdicheq` esté listo |
| D03 | Créditos | 288 | No puede migrar hasta que `bdicheq` esté listo |
| D02 | Integración y Autenticación | 194 | No puede migrar hasta que `bdicheq` esté listo |
...
T=0 (22:00): INICIO VENTANA DE CUTOVER
  ...
  22:05  → Freeze de escrituras en Informix bdicheq
