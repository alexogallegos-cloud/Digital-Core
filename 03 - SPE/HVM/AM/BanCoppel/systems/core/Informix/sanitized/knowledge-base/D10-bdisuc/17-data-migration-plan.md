# D10 · Sucursales — Plan de Migración de Datos

> **Componente:** LegacyCore · SPE-AM-001 · DESIGN Phase → BUILD
> **Base de datos:** `bdisuc` → Target: Aurora PostgreSQL 15+
...
| Métrica | Valor |
|---------|-------|
| SPs a migrar (scope mínimo) | 37 en call graph |
...
FASE 1 — SNAPSHOT INICIAL (sin downtime)
  ├─ Debezium captura snapshot de tablas bdisuc
...
FASE 3 — CUTOVER (con ventana de mantenimiento ~2 horas)
  ├─ Freeze de escrituras en Informix bdisuc
...
## Configuración Debezium para `bdisuc`

```json
{
  "name": "debezium-bdisuc",
  ...
  "database.dbname": "bdisuc",
  "table.include.list": "[SME-PENDING — DBA: lista de tablas a capturar]",
  "decimal.handling.mode": "string",
  "time.precision.mode": "adaptive_time_microseconds",
  "database.serverTimezone": "America/Mexico_City",
  "topic.prefix": "legacycore.bdisuc",
  "schema.history.internal.kafka.topic": "schema-changes.bdisuc"
}
```

## Dependencias de datos — pre-requisitos

Para que los datos de `bdisuc` sean coherentes en Aurora, los siguientes dominios deben haber sido migrados primero (referencian sus datos):
...
SELECT MAX(fecha_insert), COUNT(*) FROM bdisuc:tabla_principal; -- Informix
