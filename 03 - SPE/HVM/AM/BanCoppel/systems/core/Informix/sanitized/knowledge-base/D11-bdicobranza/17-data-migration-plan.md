# D11 · Cobranza — Plan de Migración de Datos

> **Componente:** LegacyCore · SPE-AM-001 · DESIGN Phase → BUILD
> **Base de datos:** `bdicobranza` → Target: Aurora PostgreSQL 15+
...
| Métrica | Valor |
|---------|-------|
| SPs a migrar (scope mínimo) | 82 en call graph |
| Tablas estimadas (inferidas) | ~5 (confirmar con DBA IBM Informix — Etapa 2) |
...
FASE 1 — SNAPSHOT INICIAL (sin downtime)
  ├─ Debezium captura snapshot de tablas bdicobranza
...
FASE 3 — CUTOVER (con ventana de mantenimiento ~2 horas)
  ├─ Freeze de escrituras en Informix bdicobranza
...
## Configuración Debezium para `bdicobranza`

```json
{
  "name": "debezium-bdicobranza",
  ...
  "database.dbname": "bdicobranza",
  "table.include.list": "[SME-PENDING — DBA: lista de tablas a capturar]",
  "decimal.handling.mode": "string",
  "time.precision.mode": "adaptive_time_microseconds",
  "database.serverTimezone": "America/Mexico_City",
  "topic.prefix": "legacycore.bdicobranza",
  "schema.history.internal.kafka.topic": "schema-changes.bdicobranza"
}
```

## Dependencias de datos — pre-requisitos

Para que los datos de `bdicobranza` sean coherentes en Aurora, los siguientes dominios deben haber sido migrados primero (referencian sus datos):

| Dominio prereq | Nombre | Volumen de relación | Impacto si no está listo |
|---------------|--------|--------------------|-----------------------|
| D09 | Mensajería | 53 | Debe estar disponible como API antes de migrar `bdicobranza` |
...
SELECT MAX(fecha_insert), COUNT(*) FROM bdicobranza:tabla_principal; -- Informix
