# D04 · Cheques / Cuentas — Estrategia de Pruebas de Equivalencia (Golden Master)
...
Demostrar que el `CheckingAccountService (Ledger Primitives)` target produce **resultados idénticos** al SP Informix original bajo todas las condiciones relevantes de producción. Sin esta equivalencia demostrada, el cutover no puede proceder.

## Prioridad — D04 es Wave 4

**Wave 4** — prioridad alta. Es un dominio core que requiere equivalencia perfecta antes de cutover.

## TS-D04-01 · Golden Master del SP más crítico: `abono_ref`

Este SP tiene 520 callers y es el punto de entrada más crítico del dominio.
...
FROM bdicheq:sp_audit_log           -- [SME-PENDING: confirmar tabla de audit]
WHERE fecha_insert >= TODAY - 7
  AND proc_name = 'abono_ref'
ORDER BY exec_ts;
```
...
| TC-09 | Volumen de carga de producción | Carga real × 520 req/seg | Latencia dentro de SLA target |
...
## TS-D04-02 · Pruebas de procesos batch (ver 11-batch-processes.md)
...
## TS-D04-03 · Pruebas de integración cross-domain

Los SPs de `bdicheq` hacen llamadas cross-DB a otros dominios. En el target, estas se convierten en llamadas API.

| SP | Dependencia cross-DB | Prueba requerida |
|----|---------------------|-----------------|
| `sp_abono_sd` | `bdicheq` | Simular respuesta API de `bdicheq` con mock — verificar contrato |
| `sp_abono_sd` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_abono_sd` | `bdinvers` | Simular respuesta API de `bdinvers` con mock — verificar contrato |
| `sp_abono_sd_pbajlh` | `bdicheq` | Simular respuesta API de `bdicheq` con mock — verificar contrato |
| `sp_abono_sd_pbajlh` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_abono_sd_pbajlh` | `BDICHEQ` | Simular respuesta API de `BDICHEQ` con mock — verificar contrato |
| `sp_abono_sd_pbajlh` | `bdisolic` | Simular respuesta API de `bdisolic` con mock — verificar contrato |
| `sp_abonos_operaciones` | `sysmaster` | Simular respuesta API de `sysmaster` con mock — verificar contrato |
| `sp_abonos_operaciones_esp` | `sysmaster` | Simular respuesta API de `sysmaster` con mock — verificar contrato |
| `sp_act_cuentas_bloqueadas` | `bdicheq` | Simular respuesta API de `bdicheq` con mock — verificar contrato |
...
`bdicheq` contiene operaciones con `MONEY` e `INTEGER`. En el target PostgreSQL:
...
---
*Generado por: Specialist — Informix SPL Analysis · 2026-07-03 · Evidencia: source/LegacyCore/informix/bdicheq_*.sql (análisis estático de 70 archivos SQL) · Specialist — Informix SPL Analysis + QA Lead Equivalencia*
