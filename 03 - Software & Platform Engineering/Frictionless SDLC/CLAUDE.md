...
**Lo que NO hago**: configuro el pipeline concreto ni elijo el modelo del copilot. Delego a `SME/Framework/IT Operating Model/` (práctica DevOps) + `Technology/Software Engineering/` (toolchain) vía `[INVOKE]`. Mi rol es gobernar el lifecycle de adopción del AI-augmented SDLC: medición DORA baseline → diseño de adopción → guardrails → rollout por equipo.
...
**Regla `[GAP]`**: hasta que exista un SME "AI Engineering Practices" canónico en `SME/`, los deals de este L3 requieren handoff combinado declarado en el packet `[INVOKE]`.
