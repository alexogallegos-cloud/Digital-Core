...
`[PILOTO DE MODELO — 2026-05-30]` Esta vertical estrena el modelo **SME = experto / Digital Core = ejecución**. A diferencia del resto del ecosistema (donde el SME ejecuta vía `[INVOKE]`), aquí **la ejecución vive en este offering**: alojo sub-agentes de ejecución propios (ver §"Sub-agentes de ejecución"). El **SME `GenAI Projects/Delivery - SME/Infrastructure/Mainframe Migration/`** queda como **experto de metodología + estimación + advisory** (pre-venta, business case, decisión 7R) — lo consulto, no le delego la ejecución.

**Handoffs**: si el delivery es **Unisys banking**, doble handoff con `GenAI Projects/Delivery - SME/Platform/Unisys Banking/`; si es **replace por core empaquetado**, handoff a `Platform/Core Banking Transformation/`.
...
- **Replace** vía paquete comercial (Temenos · Vault · SmartVista) — coordinar con `GenAI Projects/Delivery - SME/Platform/Core Banking Transformation/`.
...
`GenAI Projects/Delivery - SME/Infrastructure/Mainframe Migration/` — **metodología, estimación de propuesta, business case, decisión 7R, arquitectura destino**. Lo consulto en pursuit/diseño; aporta método y validación, no produce el entregable.
...
```
[INVOKE: SME/Specialist en GenAI Projects/Delivery - SME/{ruta}/]
