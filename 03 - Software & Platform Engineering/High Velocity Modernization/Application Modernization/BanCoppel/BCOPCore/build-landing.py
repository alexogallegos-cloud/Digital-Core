...
cg = load("callgraph-data.json"); J = load("journeys-data.json")
BR = load("business-rules.json"); VI = load("vocabulary-inventory.json")
EV = load("evolution-data.json")
try:
...
open(BASE + "index-bcop.html", "w", encoding="utf-8").write(HTML)
