# D02 · Integración y Autenticación — Plan de Cutover

> **Componente:** BCOPCore · SPE-AM-001 · RELEASE Phase
> **Base de datos:** `bdinteg` → Aurora PostgreSQL
...
`Integración y Autenticación` no puede cortar hasta que estos dominios estén disponibles como API:

| ID | Dominio | Llamadas | Estado |
|----|---------|---------|--------|
| D04 | Cheques / Cuentas | 194 | API disponible como prereq |
| D03 | Créditos | 165 | API disponible como prereq |
| D09 | Mensajería | 79 | API disponible como prereq |
| D06 | Solicitudes | 51 | API disponible como prereq |

## Dominios bloqueados por `bdinteg`

Estos dominios no pueden migrar hasta que `bdinteg` complete el cutover:

| ID | Dominio | Llamadas | Impacto |
|----|---------|---------|---------|
| D01 | Canal Digital Web | 15,304 | No puede migrar hasta que `bdinteg` esté listo |
| D07 | Aclaraciones | 452 | No puede migrar hasta que `bdinteg` esté listo |
| D03 | Créditos | 226 | No puede migrar hasta que `bdinteg` esté listo |
| D05 | Saldos y Cuentas | 145 | No puede migrar hasta que `bdinteg` esté listo |
| D04 | Cheques / Cuentas | 130 | No puede migrar hasta que `bdinteg` esté listo |
...
T=0 (22:00): INICIO VENTANA DE CUTOVER
  ...
  22:05  → Freeze de escrituras en Informix bdinteg
