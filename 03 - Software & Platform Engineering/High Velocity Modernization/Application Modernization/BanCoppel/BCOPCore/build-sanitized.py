...
# ── procesar HTML ────────────────────────────────────────────────────────────
htmls = [f for f in glob.glob("*.html") if "-bcop" in f]
for f in htmls:
...
jsons = ["callgraph-data.json", "evolution-data.json", "flow-data.json", "journeys-data.json"]
for f in jsons:
    if os.path.exists(f):
        src = open(f, encoding="utf-8").read()
        open(os.path.join(OUT, f), "w", encoding="utf-8").write(sanitize_data(src))
