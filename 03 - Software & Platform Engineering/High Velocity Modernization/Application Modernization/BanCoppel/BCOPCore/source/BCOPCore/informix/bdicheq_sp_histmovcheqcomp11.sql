CREATE PROCEDURE "informix".sp_histmovcheqcomp11(pempresa CHAR(3))
RETURNING CHAR(5)  AS vcodret1, 
    ...
    ON EXCEPTION
        ...
        SET DEBUG FILE TO "/resplogifx/conciliachq/sp_histmovcheqcomp11.err";
        TRACE ON;
        IF sql_err <> 0 THEN
    ...
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/sp_histmovcheqcomp11.out";
    --- TRACE ON;
...
*/	 
    ...
    SELECT valor 
      ...
      FROM sc_param
     WHERE empresa = pempresa
       AND codparam = 'CtaIniRepHisChqComp5';
    
    LET vfecha = TO_CHAR(vfecha_ant, '%d%m%Y');
    
    -- // DESCARGA MOVIMIENTOS POS - BANCO
    LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO /resplogifx/conciliachq/histmovcheq_aplicados_POS11_'||vfecha||'.txt '||
			   ...
			   'AND mov.cuenta >= '''||vcuenta_ini||''' AND mov.cuenta < '''||vcuenta_fin||''';" > /resplogifx/conciliachq/histmovcheq_POS11.sql';
			   
    SYSTEM vsql;
    
    LET vstmt = "/ifxsif01/bin/dbaccess bdicheq /resplogifx/conciliachq/histmovcheq_POS11.sql"; 
    SYSTEM vstmt;
