CREATE PROCEDURE "informix".sp_consulta_oper_user_withnumcliente_bei(pNumCliente CHAR(9),pIdPerfil INTEGER,pNoReg INTEGER,pRegIni INTEGER)
...
--**************************************************************************************************************
      ...
      FROM bdibei:"informix".bei_usuario_perfil bup INNER JOIN bdibei:"informix".bei_usuario bus
           ON (bup.id_usuario = bus.id_usuario)
      WHERE bup.id_perfil = pIdPerfil AND bus.num_cliente = pNumCliente AND bus.id_tipo_usuario <> 1;
