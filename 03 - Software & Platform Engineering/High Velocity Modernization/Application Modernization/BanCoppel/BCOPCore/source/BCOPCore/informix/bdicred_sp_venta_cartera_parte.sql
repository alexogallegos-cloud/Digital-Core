CREATE PROCEDURE "informix".sp_venta_cartera_parte(pEmpresa char(3), pProducto char(4), pBaja char(10))
returning char (87);

--  Autor: Paul Ivan Quintero Varela.
--  Fecha: 05/03/2008.
--  Observaciones: Se creá¡µn sp_para el proceso de la Venta de la Cartera.


--  Autor: Paul Ivan Quintero Varela.
--  Fecha: 18/12/2008.
--  Observaciones: Se modifica procedimiento para la solució® ¤e la incidencia
--                 de la baja de cartera en relació® ¡l rubro de reservas.
...
--SET DEBUG FILE TO '/informix/PruebasEnero/duplic_tablas/sp_venta_cartera_v01.out';
--TRACE ON;
...
if pProducto = '6001'  then
		...
		and num_credito in (Select num_credito From bdicobranza:cb_rep_cart_quebrantar
									...
									) INTO temp selec_vta WITH NO LOG;
else
...
ForEach With hold
	...
	IF cNumProducto = '6001' THEN
		...
		-- Se Actualiza la fecha de proceso por estar bloqueados los crê¥©tos
...
-------------------------
	...
	ELIF cNumProducto = '6300' THEN --ELSE
...
-------------------------
	...
	-- Se Replica la informacion de los creditos (CREDINOMINA) por Vender a la tabla bdicred:sd_maecred_vendida.
        --INSERT INTO bdicred:sd_maecredcrd_vendida
		INSERT INTO bdicred:sd_maecredcrd_vend_total(fecha,num_credito,num_producto,numcte,status_cred,fecha_apertura,credito_externo)
        --SELECT CURRENT, * FROM bdicred:sd_maecredcrd
		SELECT current,num_credito,num_producto,numcte,status_cred,fecha_apertura,credito_externo FROM bdicred:sd_maecredcrd
     ...
     -- FMV 23May13  ajuste de indicador de buro por la venta de Cartera 6300
        ...
        WHERE a.empresa          = pEmpresa
          AND a.num_credito      = cNumCredito
          AND a.bandera_ministra in ('M','S','Q')
          AND b.empresa          = a.empresa
          AND b.num_credito      = a.num_credito
          AND c.empresa          = a.empresa
          AND c.num_producto     = a.num_producto
		...
		IF vMontoVencidoExigible_rees > 0 THEN
			...
			CALL GenMovcrd(pEmpresa, cNumCredito, cNumProducto, 1, "447", dFecha, vMontoVencidoExigible_rees, "CarVendida", vSucursal, cDivisa, "0000","","")
				 RETURNING CodRet, Mensaje;
		...
		IF vMontoVencidoNoExigible_rees > 0 THEN
			...
			CALL GenMovcrd(pEmpresa, cNumCredito, cNumProducto, 2, "447", dFecha, vMontoVencidoNoExigible_rees, "CarVendida", vSucursal, cDivisa, "0000","","")
				 RETURNING CodRet, Mensaje;
...
-------------------------
      ...
      --  IF cStatusCred = 'BT' THEN
        -- Baja del INTERES VENCIDO por cobrar sobre operaciones crediticias
			...
			IF vInteresVencido_rees > 0 THEN
				...
				CALL GenMovcrd(pEmpresa, cNumCredito, cNumProducto, 3, "447", dFecha, vInteresVencido_rees, "CarVendida", vSucursal, cDivisa, "0000","","")
					 RETURNING CodRet, Mensaje;
			...
			IF vIvaInteresVencido_rees > 0 THEN
					...
					CALL GenMovcrd(pEmpresa, cNumCredito, cNumProducto, 4, "447", dFecha, vIvaInteresVencido_rees, "CarVendida", vSucursal, cDivisa, "0000","","")
					 RETURNING CodRet, Mensaje;
      ...
      --  END IF;
		...
		---------------  Baja de intereses provisionados antes de la fecha de la provision de este mes
            ...
            if (month(dproxfechapago) = month(dfechaproceso) and year(dproxfechapago) = year(dfechaproceso)) then
                ...
                if nvl(vInteresVencido_ant,0) = 0 then
                    ...
                    CALL GenMovcrd(pEmpresa, cNumCredito, cNumProducto, 7, "447", dFecha, vInteresVencido_ant, "CarVendida", vSucursal, cDivisa, "0000","","")
                         RETURNING CodRet, Mensaje;
                ...
                if nvl(vIvaInteresVencido_ant,0) = 0 then
                    ...
                    CALL GenMovcrd(pEmpresa, cNumCredito, cNumProducto, 8, "447", dFecha, vIvaInteresVencido_ant, "CarVendida", vSucursal, cDivisa, "0000","","")
                         RETURNING CodRet, Mensaje;
		...
		IF vInteresVencido > 0 THEN
			CALL GenMovcrd(pEmpresa, cNumCredito, cNumProducto, 5, "447", dFecha, vInteresVencido, "CarVendida", vSucursal, cDivisa, "0000","","")
				 RETURNING CodRet, Mensaje;
		...
		IF vIvaInteresVencido > 0 THEN
			CALL GenMovcrd(pEmpresa, cNumCredito, cNumProducto, 6, "447", dFecha, vIvaInteresVencido, "CarVendida", vSucursal, cDivisa, "0000","","")
				 RETURNING CodRet, Mensaje;
		...
		--begin work;
		  update bdicred:sd_movdia
		   ...
		   where empresa = '001'
		     ...
		     and codigo_fun in ('444','445','446','447');
		-- commit work;
	...
	FOREACH --WITH HOLD
	  SELECT num_credito
	    into cNumCredito
	    FROM bdicred:sd_maecredcrd cr
		where empresa = '001'
		  and status_cred = 'CV'
		  and campo_trab3 ='BAJA'

		--begin work;
		  update bdicred:sd_movdiacrd
		     set reversado = 'S'
		   where empresa = '001'
		     and num_credito = cNumCredito
		     and fecha_mov = dFecha
