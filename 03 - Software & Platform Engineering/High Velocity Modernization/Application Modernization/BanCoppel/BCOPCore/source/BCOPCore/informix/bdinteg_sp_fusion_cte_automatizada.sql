CREATE PROCEDURE "informix".sp_fusion_cte_automatizada(pNumCteCorrecto CHAR(20), pNumCteIncorrecto CHAR(20), pCanal CHAR(5), pFecha DATE, pUsuario CHAR(20))
...
LET cClubCI				= '0';
	...
	--SET DEBUG FILE TO '/informix/ALAN/INCIDENCIAFUSION/sp_fusion_cte_automatizada.out';
	--TRACE ON;	
BEGIN
	ON EXCEPTION
		...
		IF iSqlErr <> 0 THEN	
			...
			VALUES('Manejo de excepciones', 'sp_fusion_cte_automatizada', cCte_Titular, cCte_Traspaso, cCodRet ||'|'|| TRIM(cEstado), CURRENT  , 'infoaut', CURRENT);
	...
	IF	NVL(pFecha, '') = '' OR  NVL(pCanal, '') = '' OR  NVL(pUsuario, '') = '' THEN
		...
		INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
		VALUES('Valida parametros entrada', 'sp_fusion_cte_automatizada', pNumCteCorrecto, pNumCteIncorrecto, '00003' ||'|'|| 'Parametros incorrectos', CURRENT  , 'infoaut', CURRENT);
	...
	ELSE
		...
		IF EXISTS(SELECT valor FROM bdinteg:si_param WHERE cod_param = '314') THEN  --Verifica si existe el parametro con el usuario correcto para realizar fusion
				...
				IF USER = TRIM(cUserEjecuta) THEN
					...
					ELSE
						...
						IF NVL(pNumCteCorrecto, '') = '' OR  NVL(pNumCteIncorrecto, '') = '' THEN				
							IF EXISTS(SELECT valor FROM si_param WHERE cod_param = '307') THEN  -- Se valida si existe el parametro de máximo de fusiones 
								
								SELECT NVL(valor::INTEGER, 0) 
								INTO iMaximoFusiones
								FROM si_param 
								WHERE cod_param = '307';
								
								IF iMaximoFusiones > 0 THEN   --Se valida si el valor del parametro es mayor que 0
									SELECT COUNT(*) 
									INTO iPorProcesar
									FROM bdinteg:"informix".si_fusionaut
									WHERE estatus = 0
									...
									IF iPorProcesar > 0 THEN
										FOREACH WITH HOLD
											SELECT LIMIT iMaximoFusiones cliente_tit, cliente_tras, canal, fecha_insert, estatus, cod_retorno, proceso, fecha_fusion 
											...
											IF cCte_Titular <> cCte_Traspaso THEN
												--Validar que el cliente correcto exista
												IF EXISTS(SELECT numcte FROM bdinteg:si_cliente WHERE numcte = cCte_Titular) THEN
													--Aqui consultamos los tipo de clientes
													SELECT tipo_cliente AS c_correcto, (SELECT tipo_cliente FROM si_cliente WHERE numcte=cCte_Traspaso) AS c_incorrecto 
													INTO iTipo_cte_correcto, iTipo_cte_incorrecto
													FROM si_cliente
													WHERE numcte = cCte_Titular;
													
													IF iTipo_cte_correcto = 2 and iTipo_cte_incorrecto = 1 THEN --Si el cliente correcto = prospecto y cliente incorrecto = titular
														
														LET cProceso = 'Fusion omitida: tipo cliente';
														
														UPDATE bdinteg:"informix".si_fusionaut
														SET cod_retorno = '00006', proceso = cProceso, estatus = 3, fecha_proceso = dFechaHoy::DATE 
														...
														INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
														VALUES(cProceso, '', cCte_Titular, cCte_Traspaso, '00006'||'|'||cProceso, CURRENT  , pUsuario, CURRENT);
													ELSE
														IF iTipo_cte_correcto = 1 and iTipo_cte_incorrecto = 1 THEN --Si ambos clientes son titulares
															
															LET cProceso = 'Fusion omitida: tipo cliente';
															
															UPDATE bdinteg:"informix".si_fusionaut
															SET cod_retorno = '00006', proceso = cProceso, estatus = 4, fecha_proceso = dFechaHoy::DATE
															WHERE cliente_tit = cCte_Titular 
															AND cliente_tras = cCte_Traspaso
															AND canal = cCanal
															AND fecha_insert = dFecha
															...
															IF NOT EXISTS (SELECT 1 FROM bdicnweb:"informix".sw_tr_clientesduplicados WHERE numcte_1 = cCte_Titular AND numcte_2 = cCte_Traspaso) THEN
																IF NOT EXISTS (SELECT 1 FROM bdicnweb:"informix".sw_tr_clientesduplicados WHERE numcte_1 = cCte_Traspaso AND numcte_2 = cCte_Titular) THEN
																	SELECT rfc, rfc_alterno INTO cRfc_1, cRfc_alterno_1 FROM bdinteg:si_cliente WHERE numcte = cCte_Titular;
																	SELECT rfc, rfc_alterno INTO cRfc_2, cRfc_alterno_2 FROM bdinteg:si_cliente WHERE numcte = cCte_Traspaso;
																	...
																	VALUES (cCte_Titular, cCte_Traspaso, DECODE(NVL(cRfc_alterno_1,''),'',cRfc_1,cRfc_alterno_1), DECODE(NVL(cRfc_alterno_2,''),'',cRfc_2,cRfc_alterno_2), '', '0', '', '', '', '', '4', 'infoaut', CURRENT);
																END IF;
															END IF;
														...
														ELSE
															...
															IF cCodRetSP = '00000' THEN		
																...
																IF cValidaNom = 0 OR (cCodRetSP = '00000' AND cCodRet = '00000') THEN 	
																	...
																	IF cGeneros = 1 THEN
																		...
																		SELECT sexo
																		INTO cSexoIncorrecto
																		FROM bdinteg:"informix".si_ctepf
																		WHERE numcte = cCte_Traspaso;
																		IF cSexoCorrecto <> cSexoIncorrecto THEN
																			LET cCodRet  = '00002';
																			LET cProceso = 'VALIDACION GENERO';
																		END IF;
																	END IF;	

																	IF cGeneros = 0 OR cCodRet = '00000' THEN -- Traspasamos 
																		...
																		ELSE
																			...
																			IF (NVL(iCtaTel_CC,0) + NVL(iCtaTel_CI,0)) = 2 THEN
																				LET cCodRet = '00018';
																			...
																			ELSE
																				...
																				ELSE
																					...
																					ELSE								
																						...
																						END IF;			
																					END IF;
																				END IF; --Validacion de TRANSFER
																			END IF;
																		END IF;
																	END IF;	--Validacion de generos
																END	IF;	
															ELSE
																LET cCodRet = cCodRetSP;
															...
															IF cCodRet = '00000' THEN
																UPDATE bdinteg:"informix".si_fusionaut 
																SET estatus = 1, cod_retorno = '00000', proceso = 'CLIENTE FUSIONADO', fecha_fusion = CURRENT, fecha_proceso = dFechaHoy::DATE  
																WHERE cliente_tit = cCte_Titular 
																AND cliente_tras = cCte_Traspaso
																AND canal = cCanal
																AND fecha_insert = dFecha
																...
																DELETE bdinteg:"informix".si_cliente WHERE numcte = cCte_Traspaso;
																DELETE bdinteg:"informix".si_ctepf WHERE numcte = cCte_Traspaso;
															ELSE
																UPDATE bdinteg:"informix".si_fusionaut 
																SET cod_retorno = cCodRet, proceso = cProceso, estatus = 2, fecha_proceso = dFechaHoy::DATE 
																WHERE cliente_tit = cCte_Titular 
																AND cliente_tras = cCte_Traspaso
																AND canal = cCanal
																AND fecha_insert = dFecha
												...
												ELSE
													...
													UPDATE bdinteg:"informix".si_fusionaut 
													SET cod_retorno = '00013', proceso = cProceso, estatus = 2, fecha_proceso = dFechaHoy::DATE 
													WHERE cliente_tit = cCte_Titular 
													AND cliente_tras = cCte_Traspaso
													AND canal = cCanal
													AND fecha_insert = dFecha
													...
													INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
													VALUES('Validacion cliente correcto', 'sp_fusion_cte_automatizada', cCte_Titular, cCte_Traspaso, '00013'||'|'||cProceso, CURRENT  , pUsuario, CURRENT);
												END IF;
											ELSE
										...
										--EJECUTA PROCESO DE FUSION DE INSTRUCCIONES ORIGINADAS EN FRAUDES Y SOC
										/*EXECUTE PROCEDURE bdinteg:sp_fusion_cte_automatizada_solic('', '', '4', dFechaHoy - 1, 'infoaut')
										INTO cCodRetSolic, cEstadoSolic;
										
										--IF cCodRetSolic::INTEGER <> 0 THEN
											INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
											VALUES('Fusion de instrucciones Fraudes/SOC', 'sp_fusion_cte_automatizada_solic', pNumCteCorrecto, pNumCteIncorrecto, cCodRetSolic||'|'|| cEstadoSolic, CURRENT  , pUsuario, CURRENT);
										--END IF;
									...
									ELSE
										...
										INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
										VALUES('Instrucciones de fusion por procesar', 'sp_fusion_cte_automatizada', pNumCteCorrecto, pNumCteIncorrecto, '00009' ||'|'||iPorProcesar ||':Instruciones por procesar', CURRENT, pUsuario, CURRENT);
										
										LET cEstado = 'PROCESO NO REALIZADO, NO EXISTEN FUSIONES POR PROCESAR';

									END IF;
								ELSE
									--El valor del parametro '307' (numero maximo de fusiones) valor no establecido o igual o menor que cero
									--LET cCodRet = '00004';

									INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
									VALUES('Parametro Maximo de fusiones diarias', 'bdinteg:si_param', pNumCteCorrecto, pNumCteIncorrecto, '00004' ||'|'|| 'MAX fusion  NULL o CERO', CURRENT  , pUsuario, CURRENT);
									
									LET cEstado = 'PROCESO NO REALIZADO, PARAMETRO MAXIMO DE FUSIONES ES NULO o CERO';
									
									--RETURN cCodRet;
								END IF;
							ELSE
								--Entra aquí por que el parametro '307' no existe
								--LET cCodRet = '00005';								
								INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
								VALUES('Valida parametro Maximo de fusiones diarias', 'bdinteg:si_param', pNumCteCorrecto, pNumCteIncorrecto, '00005' ||'|'|| 'Registro MAX fusion no existe', CURRENT  , pUsuario, CURRENT);
								
								LET cEstado = 'PROCESO NO REALIZADO, PARAMETRO MAXIMO DE FUSIONES NO EXISTE';
								
								--RETURN cCodRet;
							END IF;
							
							IF EXISTS (SELECT valor FROM bdinteg:si_param WHERE cod_param = '334') THEN -- Valida si existe el parametro de Fusion OFI/SOC
								
								SELECT NVL(valor::INTEGER, 0) 
								INTO iEjecFusionOFI_SOC
								FROM si_param 
								WHERE cod_param = '334';
								
								IF iEjecFusionOFI_SOC = 1 THEN
									--EJECUTA PROCESO DE FUSION DE INSTRUCCIONES ORIGINADAS EN FRAUDES Y SOC
									EXECUTE PROCEDURE bdinteg:sp_fusion_cte_automatizada_solic('', '', '4', dFechaHoy - 1, 'infoaut')
									INTO cCodRetSolic, cEstadoSolic;
									...
									--IF cCodRetSolic::INTEGER <> 0 THEN
										INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
										VALUES('Fusion de instrucciones OFI/SOC', 'sp_fusion_cte_automatizada_solic', pNumCteCorrecto, pNumCteIncorrecto, cCodRetSolic||'|'|| cEstadoSolic, CURRENT  , pUsuario, CURRENT);
									--END IF;
								...
								ELSE
									INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
									VALUES('Valida parametro fusion OFI/SOC', 'bdinteg:si_param', pNumCteCorrecto, pNumCteIncorrecto, '00017' ||'|'|| 'Parametro fusion  OFI/SOC NULL, CERO o <> UNO', CURRENT  , pUsuario, CURRENT);
									
									--LET cEstado = 'PROCESO NO REALIZADO, PARAMETRO FUSION  OFI/SOC NULL, CERO o <> UNO';									
								END IF;
							ELSE
								INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
								VALUES('Valida parametro fusion OFI/SOC', 'bdinteg:si_param', pNumCteCorrecto, pNumCteIncorrecto, '00016' ||'|'|| 'Parametro fusion OFI/SOC no existe', CURRENT  , pUsuario, CURRENT);
								
								--LET cEstado = 'PROCESO NO REALIZADO, PARAMETRO FUSION OFI/SOC NO EXISTE';
							END IF;
						ELSE  --Busqueda solo para clientes especificos
							...
							--Se valida que ambos clientes no sean la misma persona
							IF cCte_Titular <> cCte_Traspaso THEN
								...
								IF NVL(cCte_Titular, '' ) <> '' THEN
									IF EXISTS(SELECT numcte FROM bdinteg:si_cliente WHERE numcte = cCte_Titular) THEN
										...
										IF iTipo_cte_correcto = 2 and iTipo_cte_incorrecto = 1 THEN --Si el cliente correcto = prospecto y cliente incorrecto = titular
											LET cProceso = 'Fusion omitida: tipo cliente';
											LET cCodRet = '00006';
											LET cEstado = 'PROCESO NO REALIZADO, FUSION OMITIDA POR TIPOS DE CLIENTE';
										...
										ELSE
											...
											ELSE
																		...
																		ELSE								
																			...
																			ELSE
																				...
																				ELSE
																					...
																					END IF;
																				END IF;																
																			END IF;	--Traspaso cuentas CRED		
																		END IF;
																	END IF; --Validacion cuenta TRANSFER
												...
												END IF;											
											END IF;				
										END IF; --Fin de la consulta de tipo de cliente
									ELSE
										--Aqui retorno por que no existe el cliente titular
										LET cProceso = 'CLIENTE TITULAR NO EXISTE';									
									...
									--Actualiza estadistica de fusiones
								...
								ELSE								
									INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
									VALUES('Valida instruccion', 'sp_fusion_cte_automatizada', pNumCteCorrecto, pNumCteIncorrecto, '00012' ||'|'|| 'INSTRUCCION NO EXISTE O YA FUE EJECUTADA', CURRENT  , pUsuario, CURRENT);
			...
			ELSE
				INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
				VALUES('Valida parametro usuario', 'sp_fusion_cte_automatizada', pNumCteCorrecto, pNumCteIncorrecto, '00011' ||'|'|| 'Parametro nulo o vacio', CURRENT  , pUsuario, CURRENT);
			
				LET cEstado = 'PROCESO NO REALIZADO, PARAMETRO USUARIO PERMITIDIO ES NULO o VACIO';
			END IF;
		ELSE
			INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
			VALUES('Valida parametro usuario', 'sp_fusion_cte_automatizada', pNumCteCorrecto, pNumCteIncorrecto, '00010' ||'|'|| 'Parametro no existe', CURRENT  , pUsuario, CURRENT);
...
'FECHA :11/12/2013',
'MODIFICACIÓN:Se crea sp para la finalidad de fusionar los clientes de manera automatica.',
'SUSTENTO:RQM 61 071 Fusión Automática de clientes.doc (Pagina 4,5,6,7)',
'SOLICITA:Jaime González',
'AUTOR :José Cristóbal Hernández Fierro',
'FECHA :07/05/2014',
'MODIFICACIÓN:Se modifica el SP para limitar el numero maximo de fusiones diarias.',
'SUSTENTO:RQI 64 007 Definicion de máximo de fusiones automáticas diarias',
'SOLICITA:José Angel Lopez Adams',
'AUTOR : José Cristóbal Hernández Fierro',
'FECHA :17/06/2014',
'MODIFICACION:Se agrega la validacion para verificar el tipo de cliente',
'SUSTENTO: RQI 64 020 Validación de tipo de cliente en fusión automática',
'SOLICITA:José Angel Lopez Adams',
'AUTOR:José Cristóbal Hernández Fierro',
'FECHA: 14/07/2014',
'MODIFICACION: Se comento la validacion de tipo de cliente',
'SUSTENTO: RQI 64 028',
'SOLICITA: José Angel Lopez Adams',
'BD:bdinteg',
'MODIFICACION: 	Se modifica para que el retorno del procedimiento regrese valores distintos a 00000 sólo cuando los errores que pudieran',
				'generarse sean NO controlados. Cuando por alguna inconsistencia de datos o por ejecución de otro proceso la Fusion no pueda  llevarse',
				'a cabo, el SP regresara 00000 acompañado de un mensaje que indique la razon por la que no pudo efectuarse, esto para NO alertar a ',
				'CONTROL-M por situaciones contempladas por el proceso.',
				'Además de lo anterior se le agrega nueva funcionalidad para que al terminar las fusiones diarias haga un estadistico de resultados ',
				'y lo almacene en tablas. Este estadistico será diario y mensual, y podran distinguirse las fusiones manuales y automaticas.',
'SUSTENTO: RQI 64 032',
'FECHA: 17/07/2014',
'SOLICITA: José Angel Lopez Adams',
'MODIFICA: José Angel Lopez Adams',
'AUTOR:José Cristóbal Hernández Fierro',
'FECHA: 22/SEP/2014',
'MODIFICACION: Se valida que si los dos clientes a fusionar tienen Club de Proteccion: se cancela la fusión de estos',
'SUSTENTO: RQI 64 044',
'SOLICITA: José Ángel Lopez Adams',
'BD: bdinteg',
'********************************************************************************************************************',
'FECHA: 2014/11/21',
'MODIFICACION: Se modifica para que mande llama el SP sp_fusion_cte_automatizada_solic el cual procesa las instrucciones de fusion originadas en SOC/FRAUDES',
'SUSTENTO: RQI 64 045',
...
CREATE PROCEDURE "informix".sp_fusion_cte_automatizada_esp(pNumCteCorrecto CHAR(20), pNumCteIncorrecto CHAR(20), pCanal CHAR(5), pFecha DATE, pUsuario CHAR(20))
RETURNING CHAR(5) AS codretorno, CHAR(200) AS mensaje;
--Definicion de Variables
DEFINE iSqlErr				INTEGER;
DEFINE iIsamErr				INTEGER;
...
BEGIN
	ON EXCEPTION
		...
		IF iSqlErr <> 0 THEN	
			...
			INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
			VALUES('Manejo de excepciones', 'sp_fusion_cte_automatizada_esp', cCte_Titular, cCte_Traspaso, cCodRet ||'|'|| TRIM(cEstado), CURRENT  , 'infoaut', CURRENT);
	...
	IF	NVL(pFecha, '') = '' OR  NVL(pCanal, '') = '' OR  NVL(pUsuario, '') = '' THEN
		...
		INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
		VALUES('Valida parametros entrada', 'sp_fusion_cte_automatizada_esp', pNumCteCorrecto, pNumCteIncorrecto, '00003' ||'|'|| 'Parametros incorrectos', CURRENT  , 'infoaut', CURRENT);
		
		LET cEstado = 'PROCESO NO REALIZADO, PARAMETROS DE ENTRADA INCORRECTOS, FECHA: ('||NVL(pFecha,'')||'), CANAL:('||NVL(TRIM(pCanal),'')||'),: USUARIO: ('||NVL(TRIM(pUsuario),'')||')';
	ELSE
		--SELECT MONTH(MAX(fecha_final)), YEAR(MAX(fecha_final)) INTO iMesAnterior, iAnioAnterior FROM bdinteg:si_estadistica_fusiones_mens;
		...
		IF EXISTS(SELECT valor FROM bdinteg:si_param WHERE cod_param = '314') THEN  --Verifica si existe el parametro con el usuario correcto para realizar fusion
			...
			IF cUserEjecuta <> '' THEN
				IF USER = TRIM(cUserEjecuta) THEN
					...
					ELSE
						...
						IF NVL(pNumCteCorrecto, '') = '' OR  NVL(pNumCteIncorrecto, '') = '' THEN				
							IF EXISTS(SELECT valor FROM si_param WHERE cod_param = '307') THEN  -- Se valida si existe el parametro de mÃ¡ximo de fusiones 
								...
								IF iMaximoFusiones > 0 THEN   --Se valida si el valor del parametro es mayor que 0
									...
									IF iPorProcesar > 0 THEN
											...
											IF cCte_Titular <> cCte_Traspaso THEN
												...
												IF EXISTS(SELECT numcte FROM bdinteg:si_cliente WHERE numcte = cCte_Titular) THEN	
													...
													IF cCodRet = '00000' THEN
														...
														DELETE bdinteg:"informix".si_ctepf WHERE numcte = cCte_Traspaso;
													ELSE
														UPDATE bdinteg:"informix".si_fusionaut SET cod_retorno = cCodRet, proceso = cProceso, estatus = 2, fecha_proceso = dFechaHoy::DATE 
														WHERE cliente_tit = cCte_Titular 
															AND cliente_tras = cCte_Traspaso
															AND canal = cCanal
															AND fecha_insert = dFecha
												...
												ELSE	
													...
													INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
													VALUES('Validacion cliente correcto', 'sp_fusion_cte_automatizada_esp', cCte_Titular, cCte_Traspaso, '00013'||'|'||cProceso, CURRENT  , pUsuario, CURRENT);
												END IF;
											ELSE
												LET cProceso = 'FUSION OMITIDA: MISMOS CLIENTES';
												UPDATE bdinteg:"informix".si_fusionaut 
										...
										--Inicia la obtenciÃ³n de las estadisticas de resultados de la fusiÃ³n automatica diaria
										--Obtiene estadistica general
										EXECUTE PROCEDURE "informix".sp_get_estadisticafusion('0', '' , dFechaHoy, '', pFecha, pUsuario) 
										INTO cCodRetEst;
									...
									ELSE
										...
										INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
										VALUES('Instrucciones de fusion por procesar', 'sp_fusion_cte_automatizada_esp', pNumCteCorrecto, pNumCteIncorrecto, '00009' ||'|'||iPorProcesar ||':Instruciones por procesar', CURRENT, pUsuario, CURRENT);
								...
								END IF;
							ELSE
								--Entra aquÃ­ por que el parametro '307' no existe
								--LET cCodRet = '00005';
						...
						ELSE  --Busqueda solo para clientes especificos
							...
							--Para continuar se valida que los clientes NO sean la misma persona
							IF cCte_Titular <> cCte_Traspaso THEN
								IF NVL(cCte_Titular, '' ) <> '' THEN
									--Aqui consultamos los tipo de clientes
									IF EXISTS(SELECT numcte FROM bdinteg:si_cliente WHERE numcte = cCte_Titular) THEN	
								...
								ELSE								
									INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
									VALUES('Valida instruccion', 'sp_fusion_cte_automatizada_esp', pNumCteCorrecto, pNumCteIncorrecto, '00012' ||'|'|| 'INSTRUCCION NO EXISTE O YA FUE EJECUTADA', CURRENT  , pUsuario, CURRENT);
							...
							ELSE
								...
								INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
								VALUES('Validacion de numeros de cliente', 'sp_fusion_cte_automatizada_esp', cCte_Titular, cCte_Traspaso, '00015'||'|'||cProceso, CURRENT  , pUsuario, CURRENT);
							END IF;
						END IF;
					END IF;
				ELSE
			...
			ELSE
				INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
				VALUES('Valida parametro usuario', 'sp_fusion_cte_automatizada_esp', pNumCteCorrecto, pNumCteIncorrecto, '00011' ||'|'|| 'Parametro nulo o vacio', CURRENT  , pUsuario, CURRENT);
			
				LET cEstado = 'PROCESO NO REALIZADO, PARAMETRO USUARIO PERMITIDIO ES NULO o VACIO';
			END IF;
		ELSE
			INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
			VALUES('Valida parametro usuario', 'sp_fusion_cte_automatizada_esp', pNumCteCorrecto, pNumCteIncorrecto, '00010' ||'|'|| 'Parametro no existe', CURRENT  , pUsuario, CURRENT);
...
'FECHA: 2014-08-01',
'SOLICITA: Jose Angel Lopez Adams',
'DESCRIPCION: Se crea el SP para atender fusiones automaticas especiales',
'BASE DE DATOS: bdinteg',
'********************************************************************************************************************',
'FECHA: 2014/11/21',
'MODIFICACION: Se modifica para ejecutar apropiadamente el SP sp_get_estadisticafusion de acuerdo a la nueva forma de calcular las estadisticas de fusiones',
'SUSTENTO: RQI 64 045',
'MODIFICA: JosÃ© Ãngel Lopez Adams',
'BD: bdinteg',
'********************************************************************************************************************',
'FECHA: 2014/12/01',
'MODIFICACION: Se modifica consulta para validar si el cliente cuenta con Club de proteccion, esto para evitar error -284',
