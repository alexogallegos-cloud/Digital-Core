CREATE PROCEDURE "informix".sp_remesaswu_pld_vg(NombreProceso CHAR(3),FechaIni DATE, FechaFin DATE)
RETURNING
...
CHAR(80)		AS mensaje_respuesta;
	...
	--SET DEBUG FILE TO  '/INFORMIXTMP/HMLG/EXEC_sp_remesaswu_pld_vg.out';
	--TRACE ON;
    ...
    --SET DEBUG FILE TO  '/ifxsif01/yoselin/job/sp_vg.out';
	--TRACE ON;
    ...
    BEGIN
        ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
            IF iSqlErr <> 0 THEN
                LET cCodRet = iSqlErr;
				LET cMensaje = "ERROR";
                EXECUTE PROCEDURE "informix".sp_sac_guardamensajeerror(iSqlErr, iIsamErr, cInfoErr, "sp_remesaswu_pld_vg" || "Remesa:" || cNum_confirmacion);
		...
		ELSE			
			...
			FOREACH WITH HOLD
				...
				where empresa = '001'
				  and transacc in ('1153','1123','1554','1401')
			...
			BEGIN WORK;
			FOREACH WITH HOLD
				--select {+INDEX("informix".sac_servicios_wu idxsac_servicios_wur1)} NVL(fecha_pago,mdy(01,01,1900)),'VIG','NO',NVL(referencia1,''),NVL(importe_pago,0),NVL(monto_dolares,0),NVL(transacc_suc,''),
				select NVL(fecha_pago,mdy(01,01,1900)),'VIG','NO',NVL(referencia1,''),NVL(importe_pago,0),NVL(monto_dolares,0),NVL(transacc_suc,''),
			...
			IF iCuantosCheq <> iCuantosMovtos THEN
				...
				FOREACH WITH HOLD						
					...
					VALUES(dFecha_alta,'VIG','NO','',mMonto_total,0,cTransaccion,cFolio_sucursal,(today), 
				...
				ELSE IF (iCuantosPay <> iCuantosMovtos) AND (iCuantosSearch <> iCuantosMovtos) THEN				
					...
					FOREACH WITH HOLD						
						...
						hora_proceso,hora_remesa,colonia_ordenante,tipo_id_ordenante,ciudad_id_ordenante,ocupacion_beneficiario,name_benef_suc,
						num_id_benef_suc,fecha_envio_remesa)
						VALUES(dFecha_alta,'VIG','NO',cReferencia1,mMonto_total,0,cTransaccion,cFolio_sucursal,(today), 
					...
					ELSE IF (iCuantosPay <> iCuantosMovtos) THEN
						...
						FOREACH WITH HOLD					
							...
							hora_proceso,hora_remesa,colonia_ordenante,tipo_id_ordenante,ciudad_id_ordenante,ocupacion_beneficiario,name_benef_suc,
							num_id_benef_suc,fecha_envio_remesa)
							VALUES(dFecha_alta,'VIG','NO',vPt_Mtcn,mMonto_total,mMonto_dolares,cTransaccion,cFolio_sucursal,(today), 
						...
						ELSE IF (iCuantosSearch <> iCuantosMovtos) THEN							
							...
							FOREACH WITH HOLD
								...
								ocupacion_beneficiario,name_benef_suc,num_id_benef_suc,fecha_envio_remesa)
								  VALUES(dFecha_alta,'VIG','NO',vPt_Mtcn,mMonto_total,0,cTransaccion,cFolio_sucursal,(today), 
