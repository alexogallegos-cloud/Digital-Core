CREATE PROCEDURE "informix".sp_incrementos_prestamo_digital(pEmpresa CHAR(3))
RETURNING CHAR(6);
...
DEFINE dFechaAperturaTmp 			DATE;
DEFINE dFechaIncremento 			DATE;
DEFINE dFechaMovHis 			    DATE;
DEFINE sBanderaEjec					INTEGER;
DEFINE sAux							SMALLINT;
DEFINE cCredito					    CHAR(20);
DEFINE cCreditoTmp					CHAR(20);
DEFINE cCreditoUltimo			    CHAR(20);
DEFINE sSecuencia					SMALLINT;
DEFINE sNumIncrementos		        SMALLINT;
DEFINE cMobParam190				    CHAR(100);
DEFINE cMesesAntEvalParam191		CHAR(100);
DEFINE dFechaMob 			        DATE;
DEFINE dFechaParam191 			    DATE;
DEFINE dFechaIni 			        DATE;
DEFINE dFechaFin 			        DATE;
DEFINE dFechaParam199 			    DATE;
DEFINE cMesesObtePorcenParam202		CHAR(100);
DEFINE dFechaParam202 			    DATE;
DEFINE cBcScoreParam192             CHAR(100);
DEFINE dMontoDispuesto     		    DECIMAL(18,2);
DEFINE dMontoLinea    		        DECIMAL(18,2);
DEFINE dPorcenUtilizacion           DECIMAL(18,2);
DEFINE dUtilizacionCredParam193     DECIMAL(18,2);
DEFINE dBcScoreCliente              DECIMAL(18,0);
DEFINE dPorcenAumentoLinea          DECIMAL(2);
DEFINE dPorcenAumentoLinea2         DECIMAL(18,2);
DEFINE cPorcenAumentoLinea          CHAR(7);
DEFINE iPorcenAumentoParam194       INTEGER;
DEFINE iPorcenAumentoParam195       INTEGER;
DEFINE dTopeLineaParam196           DECIMAL(18,2);
DEFINE iNumMaxIncrementosParam197   INTEGER;
DEFINE dNuevoMontoLinea     		DECIMAL(18,2);
DEFINE dAumentoLineaDisponible      DECIMAL(18,2);
DEFINE cAceptoIncremento     		CHAR(9);
DEFINE cNumCte             		    CHAR(20);
DEFINE cNumCteTmp             		CHAR(20);
DEFINE dAumentoLinea                DECIMAL(18,2);
DEFINE iDiasVigOferta               INTEGER;
DEFINE cMesesUltIcrmtParam199		CHAR(100);
DEFINE dCapitalDebe     		    DECIMAL(14,2);
DEFINE cNumCel               		CHAR(13);
DEFINE cSucursal					CHAR(4);
DEFINE cSucursalTmp					CHAR(4);
DEFINE cDivisa						CHAR(2);
DEFINE cDivisaTmp					CHAR(2);
DEFINE cCodRetGenMov				CHAR(6);
DEFINE cErrorInfo                   CHAR(80);
DEFINE isam_err						INTEGER;
DEFINE error_info 					CHAR(80);
DEFINE cCodRetBit 					CHAR(6);
DEFINE cCodRetDep 					CHAR(6);
DEFINE cProceso 					CHAR(4);
DEFINE cMensajeRet					CHAR(125);
DEFINE cBcScoreMinParam152		    INTEGER;
DEFINE cBcScoreMaxParam153		    INTEGER;
...
LET dFechaAperturaTmp           = DATE(1);
LET dFechaIncremento            = DATE(1);
LET dFechaMovHis			    = DATE(1);	
LET cCredito 					= '';
LET cCreditoTmp 			    = '';
LET cCreditoUltimo 			    = '';
LET sBanderaEjec 			    = 0;
...
LET cMobParam190                = '';
LET cMesesAntEvalParam191       = '';
LET dFechaMob                 	= DATE(1);
LET dFechaParam191           	= DATE(1);
LET dFechaParam199           	= DATE(1);
LET cBcScoreParam192          	= '';
LET dMontoDispuesto           	= 0;
LET dMontoLinea              	= 0;
LET dPorcenUtilizacion   	 	= 0;
LET dUtilizacionCredParam193  	= 0;
LET dBcScoreCliente         	= 0;
LET dPorcenAumentoLinea  		= 0;
LET dPorcenAumentoLinea2  		= 0;
LET cPorcenAumentoLinea 		= '';
LET dTopeLineaParam196      	= 0;
LET iNumMaxIncrementosParam197  = 0;
LET dNuevoMontoLinea         	= 0;
LET dAumentoLineaDisponible  	= 0;
LET cAceptoIncremento         	= '';
LET cNumCte                     = '';
LET cNumCteTmp                  = '';
LET dAumentoLinea             	= 0;
LET iDiasVigOferta  			= 28;
LET cMesesUltIcrmtParam199 		= '';
LET dFechaIni                   = DATE(1);
LET dFechaFin                   = DATE(1);
LET cMesesObtePorcenParam202 	= '';
LET dFechaParam202           	= DATE(1);
LET dCapitalDebe  				= 0;
LET cNumCel             		= '';
LET cSucursal                   = '';
LET cSucursalTmp                = '';
LET cDivisa                     = '';
LET cDivisaTmp                  = '';
LET cCodRetGenMov				= '000000';	
LET cErrorInfo                  = '';
LET isam_err 					= 0;
LET error_info 					= "";
LET cCodRetBit 					= '000000';
LET cCodRetDep 					= '000000';
LET cProceso 					= '0117';
LET cMensajeRet 				= '';
LET cBcScoreMinParam152		    = 0; 
LET cBcScoreMaxParam153			= 0;
LET iContador     				= 1;
...
BEGIN
	ON EXCEPTION SET vsqlerr, isam_err, error_info
	   IF vsqlerr != 0 THEN
		  ...
		  EXECUTE PROCEDURE bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, cMensajeRet, '02') 
		  INTO cCodRetBit;
	...
	END EXCEPTION;
 	...
 	--SET DEBUG FILE TO "/informix/AA/Sp_s/pruebas_incremento/sp_incrementos_prestamo_digital1.out";
	--TRACE ON; 
...
-- ****************************************************************************	
	...
	EXECUTE PROCEDURE bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, "INICIA EL PROCESO DE EVALUCAION DE CLIENTES PARA INCREMENTO EN EL PRESTAMO DIGITAL", '02') 
	INTO cCodRetBit;

	--Obtenemos del dia de hoy, el primero y ultimo dia del mes para evaluacion
	SELECT fecha_hoy INTO dFechaHoy FROM bdicred:"informix".sd_fechas WHERE empresa = pEmpresa;
	LET dFechaSistema = CURRENT::DATE;
	
	IF  dFechaHoy = dFechaSistema THEN
		
		SELECT COUNT(*) INTO sBanderaEjec 
		FROM bdicred:"informix".sd_evalua_candidatos_incrementos_pd;
		
		--VALIDAMOS QUE LA EJECUCION ANTERIOR HAYA FINALIZADO 
		IF sBanderaEjec = 0 THEN 
			--INICIA PROCESO DE DEPURACION
			
			EXECUTE PROCEDURE bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRetDep, "INICIA EL PROCESO DE DEPURACION PARA INCREMENTO EN EL PRESTAMO DIGITAL", '02') 
			INTO cCodRetBit;
			
			EXECUTE PROCEDURE bdicred:"informix".sp_depura_proc_incrementos_pd()
			INTO cCodRetDep;
			
			IF cCodRetDep != '000000' THEN
				EXECUTE PROCEDURE bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRetDep, "ERROR EN LA DEPURACION - SP_INCREMENTOS_PRESTAMO_DIGITAL", '02') 
				INTO cCodRetBit;
			...
			END IF;
			EXECUTE PROCEDURE bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRetDep, "TERMINA EL PROCESO DE DEPURACION PARA INCREMENTO EN EL PRESTAMO DIGITAL", '02') 
			INTO cCodRetBit;
			--TERMINA PROCESO DE DEPURACION
		END IF;
			
		--Obtenemos todos los valores a usar de la tabla bdicred:"informix".sd_param
		SELECT valor INTO cBcScoreMinParam152 FROM bdicred:"informix".sd_param WHERE cod_param = '152';
		SELECT valor INTO cBcScoreMaxparam153 FROM bdicred:"informix".sd_param WHERE cod_param = '153';
		SELECT valor INTO iContadorParam154 FROM bdicred:"informix".sd_param WHERE cod_param = '154';
		SELECT valor INTO cMobParam190 FROM bdicred:"informix".sd_param WHERE cod_param = '190';
		SELECT valor INTO cMesesAntEvalParam191 FROM bdicred:"informix".sd_param WHERE cod_param = '191';
		SELECT valor INTO cBcScoreParam192 FROM bdicred:"informix".sd_param WHERE cod_param = '192';
		SELECT valor INTO dUtilizacionCredParam193 FROM bdicred:"informix".sd_param WHERE cod_param = '193';
		SELECT valor INTO iPorcenAumentoParam194 FROM bdicred:"informix".sd_param WHERE cod_param = '194';
		SELECT valor INTO iPorcenAumentoParam195 FROM bdicred:"informix".sd_param WHERE cod_param = '195';
		SELECT valor INTO dTopeLineaParam196 FROM bdicred:"informix".sd_param WHERE cod_param = '196';
		SELECT valor INTO iNumMaxIncrementosParam197 FROM bdicred:"informix".sd_param WHERE cod_param = '197';
		SELECT valor INTO cMesesUltIcrmtParam199 FROM bdicred:"informix".sd_param WHERE cod_param = '199';
		SELECT valor INTO cMesesObtePorcenParam202 FROM bdicred:"informix".sd_param WHERE cod_param = '202';
		
		
		--Validamos que existan los parametros
		IF NVL(cMobParam190,'') = '' OR NVL(cMesesAntEvalParam191,'') = '' OR 
		    NVL(cBcScoreParam192,'') = '' OR NVL(dUtilizacionCredParam193,'') = '' OR 
			NVL(iPorcenAumentoParam194,'') = '' OR NVL(iPorcenAumentoParam195,'') = '' OR 
			NVL(dTopeLineaParam196,'') = '' OR NVL(iNumMaxIncrementosParam197,'') = '' OR 
			NVL(cMesesUltIcrmtParam199,'') = '' OR NVL(cMesesObtePorcenParam202,'') = '' OR 
			NVL(cBcScoreMinParam152,'') = '' OR NVL(cBcScoreMaxparam153,'') = '' OR 
			NVL(iContadorParam154,'') = ''	
			THEN
			LET cCodRet= '000002';
			EXECUTE PROCEDURE bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, "UNO O VARIOS PARAMETROS NO EXISTEN - SP_INCREMENTOS_PRESTAMO_DIGITAL", '02') 
			INTO cCodRetBit;
			
		    RETURN cCodRet;
		END IF;

		--Obtenermos las fechas de acuerdo a los paramentros de fechas
		EXECUTE PROCEDURE bdicred:"informix".monthadd(CURRENT, -cMobParam190) 
		INTO dFechaMob;
		EXECUTE PROCEDURE bdicred:"informix".monthadd(CURRENT, -cMesesAntEvalParam191) 
		INTO dFechaParam191;
		EXECUTE PROCEDURE bdicred:"informix".monthadd(CURRENT, -cMesesUltIcrmtParam199) 
		INTO dFechaParam199;
		EXECUTE PROCEDURE bdicred:"informix".monthadd(CURRENT, -cMesesObtePorcenParam202) 
		INTO dFechaParam202;
		
		--Obtenemos el primero y ultimo dia del mes anterior para evaluacion
		LET dFechaIni =  MDY(MONTH(dFechaParam191), 1, YEAR(dFechaParam191));
		LET dFechaFin =  LAST_DAY(dFechaParam191);
		
		IF(MONTH(dFechaHoy) = 2) THEN 
			IF(DAY(LAST_DAY(dFechaHoy)) = 28) THEN
				LET iDiasVigOferta = iDiasVigOferta -1;
			END IF;
		END IF;
		
		LET dFechaFinVigOfer = dFechaHoy + DAY(iDiasVigOferta);
		LET dFechaActual = dFechaHoy;
		
		IF sBanderaEjec = 0 THEN
			
			DROP TABLE IF EXISTS bdicred:"informix".tmp_evalua_candidatos_incrementos_pd;
			
			SELECT 
			MAECRE.sucursal, MAECRE.divisa, MAECRE.num_credito, MAECRE.numcte, SDLP.fecha_otorga	
			...
			INNER JOIN bdicred:"informix".sd_linea_prestamo AS SDLP ON SDLP.num_credito = MAECRE.num_credito 	
            WHERE SDLP.fecha_otorga <= dFechaMob 
			AND MAECRE.num_producto = '6800' 
			AND MAECRE.status_cred = 'E1' 
			AND MAEDOS.mto_fin_ven_trasp = 0
			AND MAEDOS.sdo_cap_insoluto > 0
			INTO TEMP tmp_evalua_candidatos_incrementos_pd WITH NO LOG;
			
			FOREACH WITH HOLD 
				SELECT sucursal, divisa, num_credito, numcte, fecha_otorga 
				INTO cSucursalTmp, cDivisaTmp, cCreditoTmp, cNumCteTmp, dFechaAperturaTmp 
				FROM bdicred:"informix".tmp_evalua_candidatos_incrementos_pd
				
				IF iContador = 1 THEN       
					BEGIN WORK;
				END IF;
			
				INSERT INTO bdicred:"informix".sd_evalua_candidatos_incrementos_pd(
				sucursal, divisa, num_credito, numcte, fecha_otorga) VALUES( 
				cSucursalTmp, cDivisaTmp, cCreditoTmp, cNumCteTmp, dFechaAperturaTmp);
			...
			DROP TABLE bdicred:"informix".tmp_evalua_candidatos_incrementos_pd;
			
		END IF;
	
        FOREACH	WITH HOLD
			-- Validamos prestamos digitales que esten dentro del parametro del mob	
            	
			SELECT sucursal, divisa, num_credito, numcte, fecha_otorga 
			INTO cSucursal, cDivisa, cCredito, cNumCte, dFechaApertura FROM 
			sd_evalua_candidatos_incrementos_pd
				
				BEGIN WORK;
				
				IF sBanderaEjec > 0  THEN 
					
					SELECT COUNT(*) INTO sAux
					FROM bdicred:"informix".sd_maesdoscrd 
					WHERE num_credito = cCredito AND mto_fin_ven_trasp = 0
					AND sdo_cap_insoluto > 0;
					
					IF sAux = 0 THEN
						--SUS DATOS NO CONTINUAN IGUAL, YA NO LO EVALUAMOS
						DELETE FROM bdicred:"informix".sd_evalua_candidatos_incrementos_pd WHERE num_credito = cCredito;
				...
				--Validamos que tengan disposicion y que sea la mas actual
				SELECT 
				MAX(fecha_mov), MAX(monto) 
				INTO dFechaMovHis, dMontoDispuesto
				FROM bdicred:"informix".sd_movhiscrd
				WHERE num_credito = cCredito 
				AND ((codigo_fun = '002' AND codigo_ref = '66')
				OR (codigo_fun = '002' AND codigo_ref = '126') --app
				OR (codigo_fun = '002' AND codigo_ref = '120') --atm
				OR (codigo_fun = '002' AND codigo_ref = '122')) --whatsapp
				AND fecha_mov BETWEEN dFechaParam202 AND dFechaFin
				...
				--Validamos el estatus y obtenemos el saldo siempre y cuando el registro este dentro del mes a evaluar
				SELECT 
				capital_debe 
				INTO dCapitalDebe
				FROM bdicred:"informix".sd_amortiza_creditocrd 	
				WHERE num_credito = cCredito 
				AND fecha_cuota BETWEEN dFechaIni AND dFechaFin 
				...
				--Validamos que esten dentro del mes de evaluacion o que su saldo sea mayor a 0
				IF (dFechaMovHis BETWEEN dFechaIni AND dFechaFin) OR (dCapitalDebe > 0) THEN 
				--Validamos que su bc_score sea mayor igual al permtido
					SELECT 
					bc_score
					INTO dBcScoreCliente
					FROM bdicred:"informix".sd_adviser_mensual_pp
					WHERE num_credito = cCredito;
					
					IF NVL(dBcScoreCliente,0) < cBcScoreParam192 THEN 
						INSERT INTO bdicred:"informix".sd_excluidos_incrementos_prest_digital(numcte, num_credito, fecha_apertura, motivo_rechazo_proc, fecha_insert)  
						VALUES(cNumCte, cCredito, dFechaApertura, CONCAT (NVL(dBcScoreCliente,0) || ' ES EL BC_SCORE DEL CLIENTE Y EL MINIMO REQUERIDO ES DE: ', cBcScoreParam192), dFechaActual); 			
					...
					-- Buscamos los creditos que hayan recibido incrementos en la tabla historica
						SELECT 
						secuencia, fecha_incremento, numero_incrementos 
						INTO sSecuencia, dFechaIncremento, sNumIncrementos
						FROM bdicred:"informix".sd_incrementos_prestamos_digitales_hist 
						WHERE num_credito = cCredito 
						...
						IF dbinfo("sqlca.sqlerrd2") = 0 THEN				 
							--Para prestamos SIN incrementos, sera la primera secuencia y primer numero de incremento
							LET sSecuencia = 0;
							LET sNumIncrementos = 0;
						END IF;
													
						--Verificamos la fecha de incremento
						IF (dFechaIncremento IS NULL) OR (dFechaIncremento = '') THEN
							--Si no hay fecha de incremento colocamos una fecha que siempre sea anterior
							LET dFechaIncremento = DATE(1);
						END IF;
						
						IF dFechaIncremento <= dFechaParam199  THEN
							
							SELECT 
							monto_linea, acepto_incremento 
							INTO dMontoLinea, cAceptoIncremento
							FROM bdicred:"informix".sd_linea_prestamo 
							WHERE num_credito = cCredito;
						
							--Calcaulamos el porcentaje de utilazacion del prestamo
							LET dPorcenUtilizacion = (dMontoDispuesto/dMontoLinea) * 100;
					
							IF dPorcenUtilizacion > dUtilizacionCredParam193 THEN
								
								IF dBcScoreCliente >= cBcScoreMinParam152 AND dBcScoreCliente <= cBcScoreMaxparam153 THEN  
									LET dPorcenAumentoLinea = iPorcenAumentoParam194;
									LET cPorcenAumentoLinea = CONCAT (iPorcenAumentoParam194, '%');
								END IF;	

								IF dBcScoreCliente > cBcScoreMaxparam153 THEN                              
									LET dPorcenAumentoLinea = iPorcenAumentoParam195;
									LET cPorcenAumentoLinea = CONCAT (iPorcenAumentoParam195, '%');									
								END IF;
								
								--Hacemos el calculo para el nuevo monto de linea
								LET dNuevoMontoLinea = dMontoLinea* (1 + (dPorcenAumentoLinea/100));
								
								--Validamos que el nuevo monto de linea sea mayor al anterior
							    IF NVL(dNuevoMontoLinea,0) <= dMontoLinea THEN 
									INSERT INTO bdicred:"informix".sd_excluidos_incrementos_prest_digital(numcte, num_credito, fecha_apertura, motivo_rechazo_proc, fecha_insert)  
									VALUES(cNumCte, cCredito, dFechaApertura, CONCAT('$' || NVL(dNuevoMontoLinea,0) || ' ES EL NUEVO MONTO DE LINEA Y ES MENOR O IGUAL A LA LINEA ANTERIOR QUE ES DE : $', dMontoLinea), dFechaActual); 			
								...
								IF dNuevoMontoLinea > dTopeLineaParam196 THEN
										
									LET dAumentoLinea = dTopeLineaParam196 - dMontoLinea;
									LET dNuevoMontoLinea = dMontoLinea+ dAumentoLinea;
									LET dPorcenAumentoLinea2 = (100/dMontoLinea) * dAumentoLinea;
										
									LET cPorcenAumentoLinea = CONCAT (dPorcenAumentoLinea2, '%');
												
								END IF;
								 
									--Validamos que no sobrepase el limite de linea
								IF (dMontoLinea < dTopeLineaParam196) AND (dNuevoMontoLinea <= dTopeLineaParam196) THEN 
									-- Validamos que no sobrepase el numero de incrememto
									IF (sNumIncrementos < iNumMaxIncrementosParam197) THEN
									--Se cumplio todo, INSERTAMOS EL REGISTRO
										...
										IF cNumCel IS NULL THEN
											LET cNumCel = ''; 
										END IF;
										
										IF UPPER(TRIM(cAceptoIncremento)) = "ACEPTA" THEN
											--TIPO 1
											--Obtenemos el aumento para la nueva linea disponible
											LET dAumentoLineaDisponible = dNuevoMontoLinea - dMontoLinea;
											
											INSERT INTO bdicred:"informix".sd_incrementos_prestamos_digitales(empresa, numcte, num_credito, num_producto, telefono, secuencia, numero_incrementos, fecha_evaluacion, linea_prestamo_anterior, linea_prestamo_actual, porcentaje_incremento_aplicado, fecha_incremento, incremento_automatico, canal_notificacion_invitacion, id_atm, respuesta_incremento, fecha_autorizo_o_rechazo, oferta_incremento_linea, fecha_fin_vigencia_oferta, observacion_oferta, frec_recordatorio_oferta, fecha_prox_recordatorio, estatus_sms, cod_ret_sms)  
											VALUES('001', cNumCte, cCredito, '6800', cNumCel, sSecuencia + 1, sNumIncrementos + 1, dFechaActual, dMontoLinea, dNuevoMontoLinea, cPorcenAumentoLinea, dFechaActual, 'Si', 'N/A', 'N/A', cAceptoIncremento, dFechaApertura, 'N/A', NULL, 'N/A', 'N/A', NULL, 'Pendiente', '');
											
											--Actualizamos el nuevo monto del credito
											UPDATE bdicred:"informix".sd_linea_prestamo 
											SET monto_linea = dNuevoMontoLinea, linea_disponible = linea_disponible + dAumentoLineaDisponible, fecha_ult_mod = dFechaActual 
											WHERE num_credito = cCredito;
											
											EXECUTE PROCEDURE bdicred:"informix".genmovcrd(pEmpresa, cCredito,'6800',1,'008', dFechaActual,dNuevoMontoLinea-dMontoLinea,'Inc LineaCredito',cSucursal,cDivisa,'6696','INCREMENTO DE LINEA PRESTAMO DIGITAL','') 
											INTO cCodRetGenMov, cErrorInfo;
											
											DELETE FROM bdicred:"informix".sd_evalua_candidatos_incrementos_pd WHERE num_credito = cCredito;
										
										END IF;
										
										IF UPPER(TRIM(cAceptoIncremento)) = "NO ACEPTA" THEN
											--TIPO 2
											INSERT INTO bdicred:"informix".sd_incrementos_prestamos_digitales(empresa, numcte, num_credito, num_producto, telefono, secuencia, numero_incrementos, fecha_evaluacion, linea_prestamo_anterior, linea_prestamo_actual, porcentaje_incremento_aplicado, fecha_incremento, incremento_automatico, canal_notificacion_invitacion, id_atm, respuesta_incremento, fecha_autorizo_o_rechazo, oferta_incremento_linea, fecha_fin_vigencia_oferta, observacion_oferta, frec_recordatorio_oferta, fecha_prox_recordatorio, estatus_sms, cod_ret_sms) 
											VALUES('001', cNumCte, cCredito, '6800', cNumCel, sSecuencia + 1, sNumIncrementos, dFechaActual, dMontoLinea, dNuevoMontoLinea, cPorcenAumentoLinea, NULL, 'No', '', '', cAceptoIncremento, NULL, 'Activa', dFechaFinVigOfer, 'En espera de autorizacion.', '', NULL, 'Pendiente', '');
											
											DELETE FROM bdicred:"informix".sd_evalua_candidatos_incrementos_pd WHERE num_credito = cCredito;
											
										END IF;
										
										IF (cAceptoIncremento IS NULL) OR (cAceptoIncremento = '') OR UPPER(TRIM(cAceptoIncremento)) = "PENDIENTE" THEN
											--TIPO 3
											INSERT INTO bdicred:"informix".sd_incrementos_prestamos_digitales(empresa, numcte, num_credito, num_producto, telefono, secuencia, numero_incrementos, fecha_evaluacion, linea_prestamo_anterior, linea_prestamo_actual, porcentaje_incremento_aplicado, fecha_incremento, incremento_automatico, canal_notificacion_invitacion, id_atm, respuesta_incremento, fecha_autorizo_o_rechazo, oferta_incremento_linea, fecha_fin_vigencia_oferta, observacion_oferta, frec_recordatorio_oferta, fecha_prox_recordatorio, estatus_sms, cod_ret_sms) 
											VALUES('001', cNumCte, cCredito, '6800', cNumCel, sSecuencia + 1, sNumIncrementos, dFechaActual, dMontoLinea, dNuevoMontoLinea, cPorcenAumentoLinea, NULL, 'No', '', '', 'Pendiente', NULL, 'Activa', dFechaFinVigOfer, 'En espera de autorizacion.', '', NULL, 'Pendiente', '');
											
											IF (cAceptoIncremento IS NULL) OR (cAceptoIncremento = '') THEN
												UPDATE bdicred:"informix".sd_linea_prestamo 
												SET acepto_incremento = 'Pendiente' 
												WHERE num_credito = cCredito;
											END IF;
									...
									ELSE				
										INSERT INTO bdicred:"informix".sd_excluidos_incrementos_prest_digital(numcte, num_credito, fecha_apertura, motivo_rechazo_proc, fecha_insert)  
										VALUES(cNumCte, cCredito, dFechaApertura, CONCAT('CREDITO YA TIENE SUS ', iNumMaxIncrementosParam197 || ' INCREMENTOS PERMITIDOS, NO SE PUEDE OTORGAR UNO NUEVO'), dFechaActual);
								...
								ELSE				
									INSERT INTO bdicred:"informix".sd_excluidos_incrementos_prest_digital(numcte, num_credito, fecha_apertura, motivo_rechazo_proc, fecha_insert)  
									VALUES(cNumCte, cCredito, dFechaApertura, CONCAT('YA TIENE SU LINEA DE CREDITO DE $', dTopeLineaParam196 || ' PERMITIDA, NO SE PUEDE OTORGAR UNA NUEVA LINEA'), dFechaActual);
							...
							ELSE
								INSERT INTO bdicred:"informix".sd_excluidos_incrementos_prest_digital(numcte, num_credito, fecha_apertura, motivo_rechazo_proc, fecha_insert)  
								VALUES(cNumCte, cCredito, dFechaApertura, CONCAT('EL PORCENTAJE DE UTILIZACION DE LA LINEA DE CREDITO NO ES EL SUFICIENTE, EL PORCENTAJE DEBE SER MAYOR AL: ', dUtilizacionCredParam193 || '%'), dFechaActual);							
						...
						ELSE						
							INSERT INTO bdicred:"informix".sd_excluidos_incrementos_prest_digital(numcte, num_credito, fecha_apertura, motivo_rechazo_proc, fecha_insert)  
							VALUES(cNumCte, cCredito, dFechaApertura, CONCAT ('NO HA TRANSCURRIDO EL TIEMPO REQUERIDO PARA UN NUEVO INCREMENTO, LOS MESES DE ANTIGUEDAD DEBEN SER: ', cMesesUltIcrmtParam199), dFechaActual);					
				...
				ELSE		
					INSERT INTO bdicred:"informix".sd_excluidos_incrementos_prest_digital(numcte, num_credito, fecha_apertura, motivo_rechazo_proc, fecha_insert)  
					VALUES(cNumCte, cCredito, dFechaApertura, 'EL CREDITO NO TIENE DISPOSICIONES NI SALDO', dFechaActual);
		...
		IF dbinfo("sqlca.sqlerrd2") = 0 THEN 	
			EXECUTE PROCEDURE bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, "TERMINA EL PROCESO DE EVALUACION DE CLIENTES PARA INCREMENTO EN EL PRESTAMO DIGITAL, NO SE ENCONTRARON REGISTROS", '02') 
			INTO cCodRetBit;
		...
		EXECUTE PROCEDURE bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, "TERMINA EL PROCESO DE EVALUACION DE CLIENTES PARA INCREMENTO EN EL PRESTAMO DIGITAL", '02') 
		INTO cCodRetBit;
			
	ELSE
		LET cCodRet= '000001';
		EXECUTE PROCEDURE bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, "LA FECHA ACTUAL NO CORRESPONDE CON LA FECHA EN EL SERVIDOR CENTRAL - SP_INCREMENTOS_PRESTAMO_DIGITAL", '02') 
		INTO cCodRetBit;
	END IF;	
END
RETURN cCodRet;
...
DOCUMENT
'ANALIZA Y AUMENTA LA LINEA DE CREDITO DE CLIENTES DE PRESTAMO DIGITAL, PRODUCTO 6800 - RQM 10 1543',
'AUTOR : ALAN CASTRO PAREDES',
'FECHA : FEB/2024',
'BD    : BDICRED';

CREATE PROCEDURE "informix".executaedoctacrd_pp(pempresa CHAR(3), pNumProd CHAR(4), pfechahoy DATE)
...
DEFINE v_ejecutar_sp 	SMALLINT;
DEFINE v_cod_ret_proc	CHAR(6);
...
--Modificacion:  Se agrego como parametro de entrada el tipo de producto para poder generar tambiÃ©n el estado de cuenta de Prestamos Personales

--SET DEBUG FILE TO "/informix/Daniella/Sin_Movimientos/INC_25100_22022018/executaedoctacrd_pp.out";
--TRACE ON;
...
BEGIN
  ...
  ON EXCEPTION SET sql_err, iIsamErr, cErrorInfo
   ...
   END EXCEPTION;
	...
	SELECT 	cod_tipcred, nombre_prod 
	   INTO cTipCred, 	cNomProd
	FROM "informix".sd_definicion
	WHERE num_producto = pNumProd;
	 
	IF cEmpresa IS NULL OR cTipCred IS NULL THEN
		LET v_cod_ret = '000002';
		LET cMensajeRet = 'Parametros no validos';
		RETURN v_cod_ret, cMensajeRet;
	END IF;
	 
	 
	IF cTipCred = '06' THEN
	    LET v_nameProcess = "GeneraEdoCta" || cNomProd;
		LET cMensajeRet = "Tipo de producto no soportado";
		LET v_cod_ret = "111111";
		RETURN v_cod_ret, cMensajeRet;
	ELIF cTipCred = '05' THEN
	    --LET v_nameProcess = "GeneraEdoCta" || cNomProd;
		--Se agrega para que el identificador sea con la etiqueta mas el producto PIQV
		LET v_nameProcess = "GeneraEdoCta" || pNumProd;
	ELSE
	    LET v_cod_ret = '000002';
		LET cMensajeRet = 'Tipo de producto no valido';
	    RETURN v_cod_ret, cMensajeRet;
	END IF;
 
	LET cMensaje = 'Estados de Cuenta de '|| cNomProd || ' Sin generar';
	
	-- Inicializamos bandera de control para esta vuelta
	LET v_ejecutar_sp = 1; 

	SELECT status_proc, proceso
	  INTO vStProc, 	v_proceso
	FROM "informix".sd_contproc
	WHERE empresa = pEmpresa
	AND SUBSTR (proceso, 1, 12)  = "GeneraEdoCta" --adlm
	AND fecha = pfechahoy
	AND status_proc = 'I';

      --IF vStProc IS NOT NULL THEN
	IF vStProc = 'I' AND v_proceso <> ''  THEN
	    LET v_cod_ret = '000002';
		LET cMensajeRet='Hay un proceso por concluir: ' || TRIM(v_proceso);
	    --RETURN v_cod_ret, cMensajeRet; 
		LET v_ejecutar_sp = 0;
    END IF;
	--------------------------------------------------------
	...
	-------------------------------------------------------
	SELECT status_proc, cod_ret
      INTO vStProc, 	v_cod_ret_proc
	FROM "informix".sd_contproc
	WHERE empresa  = pEmpresa 
	...
	IF vStProc IS NULL THEN
        INSERT INTO "informix".sd_contproc 
					(
			...
			VALUES 
					(
					pEmpresa, 		v_nameProcess, 	pfechahoy, 	'I', 			USER,
		...
		INSERT INTO bdinteg:sx_contproc 
		    ...
		    VALUES 
					(
					pEmpresa, 	v_nameProcess, 	pfechahoy, 	'06', 		'I', 
					USER, 		CURRENT, 		NULL, 		'000'
					);
	END IF;
	
	IF vStProc = "C" AND v_cod_ret_proc = '000' THEN
		LET cMensajeRet = 'Proceso ya ejecutado con errores controlados';
		RETURN v_cod_ret, cMensajeRet;
	END IF;
	
	IF vStProc = "F" AND v_cod_ret_proc = '000' THEN
	    LET cMensajeRet = 'Proceso ya ejecutado';
        RETURN v_cod_ret, cMensajeRet;
	ELSE 
		LET v_ejecutar_sp = 1;
	END IF;
	
	IF v_ejecutar_sp = 1 THEN
        EXECUTE PROCEDURE executaedoctageneralcrd_pp (pempresa, pNumProd, pfechahoy) INTO v_cod_ret, cMensajeRet;
	END IF;
	...
	SELECT 	COUNT(*) 
		INTO v_cuantos
	FROM "informix".sd_valedoctacrd
	WHERE empresa     = pempresa
	AND fecha_proc  = pfechahoy
	AND producto	   = pNumProd
	AND tipo <> "0";
	   
	SELECT COUNT(*) INTO v_cuantos_pp
	FROM "informix".sd_encabezado_edoctacrd
	WHERE fecha_emision = pfechahoy
	AND num_producto = pNumProd
	AND num_credito NOT IN ('6300100','6800100','7600100','7700100','9100100','9300100');

    IF v_cuantos > 0 OR v_cod_ret <> '000' THEN
        UPDATE sd_contproc
           SET status_proc = "C", hora_fin = CURRENT, cod_ret = v_cod_ret, mensaje = v_cuantos || " " || cMensaje
        WHERE proceso     = v_nameProcess
        AND fecha       = pfechahoy
        AND empresa     = pEmpresa;

        UPDATE bdinteg:sx_contproc
           SET status_proc = "C", hora_fin = CURRENT, codret = v_cod_ret
        WHERE proceso  = v_nameProcess
        AND empresa = pEmpresa
        AND fecha    = pfechahoy;
    ELSE 
		--IF (v_cuantos = 0 AND v_cuantos_pp > 0) OR (v_cod_ret_proc = '000') THEN 
			UPDATE sd_contproc
				SET status_proc = "F", hora_fin = CURRENT, cod_ret = v_cod_ret, mensaje = "Proceso Concluido " || v_cuantos_pp
			WHERE proceso     = v_nameProcess
			AND empresa     = pEmpresa
			AND fecha       = pfechahoy;

			UPDATE bdinteg:sx_contproc
				SET status_proc = "F", hora_fin = CURRENT, codret = v_cod_ret
			WHERE proceso  = v_nameProcess
			AND empresa  = pEmpresa
			AND fecha    = pfechahoy;
		--END IF;
...
"con el proceso de generaciÃ³n de Edo. Cta.",
"reestructura",
"base de datos : bdicred",
"AUTOR : Jose de Jesus Almeida",
"FECHA : 20/Julio/2009";

CREATE PROCEDURE "informix".executaedoctageneralcrd_pp(pempresa CHAR(3),pNumProd CHAR(4), pfechahoy DATE)
--EXECUTE PROCEDURE executaedoctageneralcrd_pp('001','6300',MDY('09','19','2021'));
RETURNING CHAR(6), CHAR(80);

--Fecha: 13/11/2009
--Autor: Roque Enrique Solis
--Modificacion:  Se agrego como parametro de entrada el tipo de producto para poder generar tambien el estado de cuenta de Prestamos Personales
...
--SET DEBUG FILE TO "/ifxsif01/gpe/Pruebas_finales/executaedoctageneralcrd_pp.out";
--TRACE ON;
...
--- INICIO DE VARIABLES PARA IDENTIFICAR PRESTAMOS MAS VENCIDO Y/O MAS ANTIGUO
    ...
    */
	...
	FOREACH
	--RQM 10 1177 Se agregan nuevos codigos de mensajes para 2 productos nuevos
--adlm --clave de mensajes por producto
    	SELECT REPLACE(mensajes,'{0}',TRIM(v_linea_auxiliar::VARCHAR(21))) , num_producto
		 ...
		 WHERE clave >= case when pNumProd = '6300' then '200'
                             when pNumProd = '6400' then '300' 
							 when pNumProd = '7600' then '400'
							 when pNumProd = '7700' then '500'
							 when pNumProd = '6800' then '600'
							 when pNumProd = '9100' then '700'
							 when pNumProd = '9300' then '800'
						END
		   AND clave < case	 when pNumProd = '6300' then '300'
                             when pNumProd = '6400' then '400' 
							 when pNumProd = '7600' then '500'
							 when pNumProd = '7700' then '600'
							 when pNumProd = '6800' then '700'
							 when pNumProd = '9100' then '800'
							 when pNumProd = '9300' then '900'
						END
	  ORDER BY clave
--adlm --clave de mensajes por producto
	...
	--------------------------------------------------------
			...
			IF NOT EXISTS(SELECT num_credito FROM "informix".sd_encabezado_edoctacrd WHERE fecha_emision = pfechahoy AND num_credito = v_id_registro) THEN
			   INSERT INTO "informix".sd_encabezado_edoctacrd
						(fecha_emision,        num_credito,         numcte,
						 ...
						 observaciones,        insertos,			confirmacion,
						 nombre_producto, --adlm
						ind_tabla_amortizacion, --RQM 10 1379
			...
			IF NOT EXISTS(SELECT num_credito FROM "informix".sd_encabezado2_edoctacrd WHERE fecha_emision = pfechahoy AND num_credito = v_id_registro) THEN
				...
				INSERT INTO "informix".sd_encabezado2_edoctacrd
                        ...
                        comisiones_efec_cargadas, descuento,			  subtotal,				
						total,					  comisiones,			  iva_comisiones,
						linea_autorizada,		  fecha_ult_disposicion				
						)
				VALUES (
						pfechahoy,			v_id_registro,			0,
						0,                  0,                      "0",
...
-- AAME 20150430 RQM 10 550 Se contemplan los 2 nuevos productos de prestamo para los mensajes de estados de cuenta	

	IF pNumProd IN ('6300','7600','7700','6800','9100','9300') THEN-- adlm INC 25 100/ RQI 25 102 - Prestamo Flexible
		...
		FOREACH
		        ...
		        SELECT a.empresa,a.num_credito
				 ...
				 WHERE  b.fecha = pfechahoy
				   ...
				   --AND a.num_producto IN ('6300','7600','7700')--adlm
				    AND a.num_producto = pNumProd --adlm
				...
				IF v_cod_ret <> "000000" THEN
--adlm - se agrega numero de producto
					SELECT descripcion
					  INTO v_descripcion
...
--FMV 25sep13: Se adiciona validacion para el mes de Febrero 29 y Enero 1
  ...
  IF (DAY(pfechahoy) = 30 AND MONTH(pfechahoy) IN (4, 6, 9 ,11)) OR  --Meses que tienen 30 dias
    ...
    -- AAME 20150430 RQM 10 550 Se contemplan los 2 nuevos productos de prestamo para los mensajes de estados de cuenta	
	--RQM 10 1177 Se agregan 2 productos nuevos para considerarlos en la Generacion de Estados de Cuenta
	
	IF pNumProd in ('6300','7600','7700','6800','9100','9300') THEN
...
--adlm - se agrega numero de producto
  ...
  END IF; --IF DAY(pfechahoy) = 30 AND MONTH(pfechahoy) IN (4, 6, 9 ,11) THEN --Meses que tienen 30 dias
    ...
    DROP TABLE mensajes;
	--RQM 10 1177 Se agregan 2 productos nuevos para el calculo de la cve de cobranza
	IF DAY(pfechahoy) = 17 AND pNumProd in('6300','7600','7700','6800','9100','9300') THEN --adlm
	  EXECUTE PROCEDURE "informix".sp_actualizacvlcobranzacte(pempresa, pfechahoy, pNumProd)
	  INTO ccodret;
...
"con la generacion de los estados de cuenta reestructura",
"(ejecutando el procedimiento: generaestadosdecuentacrd ",
"base de datos : bdicred",
"AUTOR : Jose de Jesus Almeida",
"FECHA : 23/Julio/2009",
"MODIFICACION: Se actualiza procedimiento para anexar la ejecucion de un nuevo procedimiento que actualizara la clave de cobranza por cliente",
