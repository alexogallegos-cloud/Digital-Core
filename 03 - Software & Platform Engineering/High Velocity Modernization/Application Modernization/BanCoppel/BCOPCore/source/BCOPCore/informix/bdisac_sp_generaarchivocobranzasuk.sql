CREATE PROCEDURE "informix".sp_generaarchivocobranzasuk(pConvenio CHAR(5))
	...
	DEFINE cMes                     CHAR(2);
	DEFINE cAnio                    CHAR(4);
	DEFINE cRutaArchSuk          	CHAR(130);
	DEFINE dTotImporte				DECIMAL(11,2);
	DEFINE dImporte					DECIMAL(11,2);
	DEFINE dSaldoInicial			DECIMAL(11,0);
	DEFINE dSaldoFinal				DECIMAL(11,0);
	DEFINE dTotIva					DECIMAL(11,2);
	DEFINE dIva						DECIMAL(11,2);
	DEFINE dTotIvaAux				DECIMAL(11,0);
	DEFINE dTotComision				DECIMAL(11,2);
	DEFINE dComision				DECIMAL(11,2);
	DEFINE dTotComisionAux			DECIMAL(11,0);
	DEFINE cStmt                    CHAR(250);
	DEFINE cTotSaldoInicial			CHAR(11);
	DEFINE cTotSaldoFinal			CHAR(11);
	DEFINE cTotIva 					CHAR(11);
	DEFINE cTotComision				CHAR(11);
...
--	2014.03.04 FRG-i
	DEFINE cctaprestadora			CHAR(20);
	DEFINE cTotmovssac				DECIMAL(11,0);
	DEFINE dsumIva					DECIMAL(11,2);
	DEFINE dsumComision				DECIMAL(11,2);
	DEFINE dtotsumComIva			DECIMAL(11,2);
	DEFINE dsumPagoSac				DECIMAL(11,2);
	DEFINE cTotMovsSuk				DECIMAL(11,2);
	DEFINE cTotMovsSuk2				DECIMAL(11,0);
	DEFINE cMntoSpei				CHAR(11);
--	2014.03.04 FRG-f
	...
	LET dIva	              = 0.00;
	LET dTotIvaAux			  = 0;
	LET dTotComision	      = 0.00;
	LET dComision		      = 0.00;
	LET dTotComisionAux		  = 0;
	LET cStmt                 = "";
	LET cTotSaldoInicial	  = "";
	LET cTotSaldoFinal		  = "";
	LET	cTotIva				  = "";
	LET cTotComision		  = "";
...
--	2014.03.04 FRG-f
	...
	--	SET DEBUG FILE TO '/informix/frg/sp_generaarchivocobranzasuk.out';
	--	TRACE ON;
...
--	2014.03.04 FRG-i
		...
		LET dtotsumComIva = dsumComision + dsumIva;
...
--	2014.03.04 FRG-f
		...
		LET cRutaArchSuk = REPLACE(cRutaArchSuk,'DD',cDia);
		LET cRutaArchSuk = REPLACE(cRutaArchSuk,'MM',cMes);
		LET cRutaArchSuk = REPLACE(cRutaArchSuk,'AAAA',cAnio);
		
		FOREACH
				SELECT 
				   importe_pago,
				   iva_comision_convenio, 
				   importe_comision_convenio,
				   flag_confirmacion_central, 
				   flag_confirmacion_sucursal, 
				   folio_suc
				INTO  dImporte,dIva,dComision,iFlagCen,iFlagSuc,cFolio
				FROM bdisac:"informix".sac_movimientoshistorial
				WHERE numcategoria = cCategoria
				AND numconvenio = cConvenio
				AND fecha_pago > dFechaIni
...
--2014.06.02 FRG-f
						IF iCuantos = 0 THEN
							CONTINUE FOREACH;
						ELSE
							LET dTotImporte = dTotImporte + dImporte;
							LET dTotIva = dTotIva + dIva;
							LET dTotComision = dTotComision	+ dComision;
						END IF;
					ELSE
						LET dTotImporte = dTotImporte + dImporte;
						LET dTotIva = dTotIva + dIva;
		...
		IF NVL(dTotImporte,'0') > 0 THEN
-- Se genera el Primer Renglón del archivo de cobranza con el encabezado
--	2014.03.04 FRG-i
			--	LET dSaldoInicial = (dTotImporte + dTotIva + dTotComision) * 100; 
			select sdo_dia_ant into dSaldoInicial
			from bdicheq:sc_maechq where cuenta = cctaprestadora;
			let dSaldoInicial = dSaldoInicial * 100;
--	2014.03.04 FRG-f
			LET cTotSaldoInicial = dSaldoInicial;
			LET cTotSaldoInicial = LPAD(TRIM(cTotSaldoInicial),11,'0');
			
--	2014.03.04 FRG-i
			--	LET dSaldoFinal = dTotImporte * 100; 
			select sdo_actual into dSaldoFinal
			from bdicheq:sc_maechq where cuenta = cctaprestadora;
			--	LET cTotSaldoFinal = dSaldoFinal;
			LET dSaldoFinal = dSaldoFinal * 100;
--	2014.03.04 FRG-f
			LET cTotSaldoFinal = dSaldoFinal;
			LET cTotSaldoFinal = LPAD(TRIM(cTotSaldoFinal),11,'0');
			
			LET cStmt = 'echo "' || 'SI' || "|" || TRIM(cAnio) || TRIM(cMes) || TRIM(cDia) || "|" || cTotSaldoInicial || "|" || cTotSaldoFinal || "|" || '">> ' || cRutaArchSuk;
			SYSTEM cStmt;
			
			-- Se genera el Segundo Renglón del archivo de cobranza con el encabezado
			LET cReferencia = 'SPEI BMX ***5751';
			LET cStmt = "";
--	2014.03.04 FRG-i
			--	LET cStmt = 'echo "' || TRIM(cAnio) || TRIM(cMes) || TRIM(cDia)  || "|" || '00000000' || "|" || cTotSaldoInicial || "|" || 'S' || "|" || RPAD(cReferencia,20,' ') || "|" || '0000' || "|" ||'">> ' || cRutaArchSuk;	
			...
			let cTotMovsSuk = cTotmovssac-dtotsumComIva;
			if	cTotMovsSuk < 0
				then
					let cMntoSpei = '00000000000';
				else
					let cTotMovsSuk = cTotMovsSuk * 100;
					let cTotMovsSuk2 = cTotMovsSuk;
					let cMntoSpei = cTotMovsSuk2;
					let cMntoSpei = LPAD(TRIM(cMntoSpei),11,'0');
			end if;
			
			LET cStmt = 'echo "' || TRIM(cAnio) || TRIM(cMes) || TRIM(cDia)  || "|" || '00000000' || "|" || cMntoSpei || "|" || 'S' || "|" || RPAD(cReferencia,20,' ') || "|" || '0000' || "|" ||'">> ' || cRutaArchSuk;
--	2014.03.04 FRG-f			
			SYSTEM cStmt;
		
			-- Se genera del 3er. Renglón  en adelante del archivo de cobranza con los IMPORTES
			FOREACH
				SELECT fecha_pago,
				   referencia1,
				   importe_pago * 100,
				...
				LET cStmt = 'echo "' || TRIM(cAnioPago)|| TRIM(cMesPago) || TRIM(cDiaPago)||"|"|| LPAD(TRIM(cReferencia1),8,'0')|| "|" || LPAD(dImporte_Pago,11,'0')|| "|" || TRIM(cCargoAbono) || "|" || RPAD(cReferencia2,20,' ')|| "|" || TRIM(cSucursal)|| "|" || '">> ' || cRutaArchSuk;
				SYSTEM cStmt;
			END FOREACH;
			...
			-- Se genera el penúltimo Renglón del archivo de cobranza con la Comision	
			LET dTotComisionAux = dTotComision * 100;
			LET cTotComision = dTotComisionAux;
			LET cTotComision = LPAD(TRIM(cTotComision),11,'0');
			LET cReferencia = 'COMISION PAGO REF';
					
			LET cStmt = "";
			LET cStmt = 'echo "' || TRIM(cAnio) || TRIM(cMes) || TRIM(cDia)  || "|" || '00000000' || "|" || cTotComision || "|" || 'C' || "|" || RPAD(cReferencia,20,' ') || "|" || '0000' || "|" ||'">> ' || cRutaArchSuk;
			SYSTEM cStmt;
			
			-- Se genera el último Renglón del archivo de cobranza con el IVA	
			LET dTotIvaAux = dTotIva * 100;
			LET cTotIva = dTotIvaAux;
			LET cTotIva = LPAD(TRIM(cTotIva),11,'0');
			LET cReferencia = 'IVA COMIS PAGO REF';
					
			LET cStmt = "";
			LET cStmt = 'echo "' || TRIM(cAnio) || TRIM(cMes) || TRIM(cDia)  || "|" || '00000000' || "|" || cTotIva || "|" || 'I' || "|" || RPAD(cReferencia,20,' ') || "|" || '0000' || "|" ||'">> ' || cRutaArchSuk;
			SYSTEM cStmt;
		ELSE
			LET cStmt = 'echo "' || "" ||'">> ' || cRutaArchSuk;
			SYSTEM cStmt;
		END IF;		
...
DOCUMENT
'AUTOR : Obed Vega',
'DESCRIPCION: Genera el archivo de cobranza SUKARNE de acuerdo al Layout',
'EJECUTADO O LLAMADO POR: sp_genera_ArchivosCobranzaCentral()',
'FECHA : 29 de Junio 2013',
'VERSION: 20130729',
'AUTOR : FRG',
'DESCRIPCION: Se modifican cálculos de dalso inicial y final en archivo.',
'EJECUTADO O LLAMADO POR: sp_genera_ArchivosCobranzaCentral()',
'FECHA : 04 de Marzo 2014',
'VERSION: 20140304',
'AUTOR: FRG',
