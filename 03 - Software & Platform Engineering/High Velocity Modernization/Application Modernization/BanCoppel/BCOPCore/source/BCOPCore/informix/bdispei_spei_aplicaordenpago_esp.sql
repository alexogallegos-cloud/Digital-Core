CREATE PROCEDURE "informix".spei_aplicaordenpago_esp( pRegistros INTEGER ) 
RETURNING CHAR(5), INTEGER, INTEGER; 
    ...
    ON EXCEPTION SET iSqlErr, iIsamErr, cDescErr
        SET DEBUG FILE TO "/resplogifx/conciliachq/spei/spei_aplicaordenpago_esp.err";
        TRACE ON;
        IF iSqlErr <> 0 THEN
            ...
            UPDATE tblctrlproceso
               SET chrstatus = '0'
             WHERE intcveproceso = 8;
            RETURN cCodRet1, iContador1, iContador2;
        END IF;
    END EXCEPTION;
    
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/spei/spei_aplicaordenpago_esp.out";
    --- TRACE ON;
    ...
    IF cStatusProc = '0' THEN 
        ...
        FOREACH WITH HOLD
            ...
            EXECUTE FUNCTION "informix".syn_verify(TRIM(wcadena_val), TRIM(wvchrfirma), 20)
            INTO codretfirma;
