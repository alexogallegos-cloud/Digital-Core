CREATE PROCEDURE "informix".sp_int_vigente_int_vencido(P_EMPRESA        VARCHAR(3),
       P_FECHA_HOY      DATE,
...
DEFINE V_MTO_VENC_INT    LIKE SD_MAESDOS.MTO_VENC_INT;
DEFINE V_SDO_EXIG_INT    LIKE SD_MAESDOS.SDO_EXIG_INT;
DEFINE V_NVO_NO_EXIGIBLE DECIMAL(18,2);
DEFINE V_NVO_MTO_VENCIDO DECIMAL(18,2);
DEFINE V_NVO_SALDO_CUOTA DECIMAL(18,2);
...
BEGIN
  ...
  END EXCEPTION;
    ...
    /* STATUS_CUOTA:  1 a 7                          */
      ...
      FOREACH SELECT S.EMPRESA, S.NUM_CREDITO, S.SDO_NO_EXIG, S.MTO_VENC_INT, S.SDO_EXIG_INT
              INTO   V_EMPRESA, V_NUM_CREDITO, V_SDO_NO_EXIG, V_MTO_VENC_INT, V_SDO_EXIG_INT
              FROM   SD_MAESDOS S
              WHERE  EXISTS (SELECT '1'
                             FROM   SD_PAGINTER P1
        ...
        SELECT V_SDO_NO_EXIG    - SALDO_CUOTA
             , V_MTO_VENC_INT   + SALDO_CUOTA
             , V_SDO_EXIG_INT   + SALDO_CUOTA
        INTO   V_NVO_NO_EXIGIBLE, V_NVO_MTO_VENCIDO, V_NVO_SALDO_CUOTA
        FROM   VALORES 
        WHERE  EMPRESA     = V_EMPRESA
        AND    NUM_CREDITO = V_NUM_CREDITO;

        UPDATE SD_MAESDOS 
          SET SDO_NO_EXIG  = V_NVO_NO_EXIGIBLE
            , MTO_VENC_INT = V_NVO_MTO_VENCIDO
            , SDO_EXIG_INT = V_NVO_SALDO_CUOTA
      ...
      FOREACH SELECT P.EMPRESA, P.NUM_CREDITO, M.NUM_PRODUCTO,
              ...
              AND    M.NUM_CREDITO  = P.NUM_CREDITO
              AND    P.STATUS_CUOTA = '1'
              AND    P.FECHA_CUOTA <= P_FECHA_HOY       -- P.FECHA_CUOTA = P_FECHA_HOY 24JUN02
              AND    P.EMPRESA      = P_EMPRESA
              AND    SUBSTR(M.STATUS_CRED, 1, 1) IN ('A', 'B')
      ...
      IF P_COD_RET = '00000' THEN
         ...
         UPDATE SD_PAGINTER 
         SET    SD_PAGINTER.CUOTA_REC    = '1',
                SD_PAGINTER.STATUS_CUOTA = '7'
         WHERE  SD_PAGINTER.FECHA_CUOTA  <= P_FECHA_HOY
         AND    SD_PAGINTER.EMPRESA      = P_EMPRESA
         AND    SD_PAGINTER.STATUS_CUOTA = '1'
         AND    EXISTS (SELECT '1'
                        FROM   SD_MAECRED M
