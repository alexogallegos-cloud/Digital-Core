CREATE PROCEDURE "informix".sp_actualizacionctepmsnom2(pOperacion CHAR(1), pNumCliente CHAR(20), 
													  pEstatus CHAR(1), pCuentaCliente CHAR(20), 
													  pTipoEmpresa CHAR(1), pProducto CHAR(4))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_actualizacionctepmsnom2.out';
		--TRACE ON;
		
		IF pOperacion = '' OR pNumCliente = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cCodigo, cNombreCte, cNumeroCte, iTipoEmpresa, cAceptaProducto, cCuentaCte, cStatusAlta;
		END IF;
		...
		FOREACH			
			...
			IF cCodRetSp::INTEGER < 0 THEN 
				RAISE EXCEPTION cCodRetSp::INTEGER, 0, 'ERROR EN LA EJECUCION DEL SP bdicheq:sp_snom_altaservicioctepmnomina';
...
'DESCRIPCION: sp clon sin validaciÃ³n de ejecutivo sp_actualizacionctepmsnom',
'FECHA: 12/09/2022',
'BASE DE DATOS: bdicnweb';

CREATE PROCEDURE "informix".sp_altadministradorespmempnet2(pUsuario CHAR(8), pNumCliente CHAR(9), pCodIden CHAR(3), pNumIden CHAR(30), 
		...
		BEGIN
            ...
            --SET DEBUG FILE TO '/tmp/mfinis/sp_altadministradorespmempnet2.out';
            --TRACE ON;
            
            IF pUsuario = '' OR pNumCliente = '' THEN
				LET cCodRet = '00003';
				RETURN cCodRet,cNumCliente,cFolioToken;
            END IF;
			...
			IF pBandera = '1' THEN
				...
				FOREACH
					...
					IF cCodRetSp::INTEGER < 0 THEN
						RAISE EXCEPTION cCodRetSp::INTEGER, 0, 'ERROR EN LA EJECUCION DEL SP bdibei:sp_administradorespm';
...
END PROCEDURE 
DOCUMENT 'AUTOR: Johnattan Esquivel Sanchez',
'DESCRIPCION: sp clon sin validaciÃ³n de ejecutivo sp_altadministradorespmempnet',
'FECHA: 12/09/2022',
'BASE DE DATOS: bdicnweb';

CREATE PROCEDURE "informix".sp_altaservicioempnet2(pUsuario CHAR(8), pNumCliente CHAR(20), pCantTokens SMALLINT, pBandera CHAR(1))
		...
		BEGIN
            ...
            --SET DEBUG FILE TO '/tmp/mfinis/sp_altaservicioempnet2.out';
            --TRACE ON;
            
            IF pUsuario = '' OR pNumCliente = '' OR pCantTokens = '' THEN
				LET cCodRet = '00003';
				RETURN cCodRet;
            END IF;
			...
			IF pBandera = '1' THEN
				...
				FOREACH
					...
					IF cCodRetSp::INTEGER < 0 THEN
						RAISE EXCEPTION cCodRetSp::INTEGER, 0, 'ERROR EN LA EJECUCION DEL SP bdibei:sp_senet_altaservicioempresanet';
					ELIF cCodRetSp::INTEGER = 1 THEN
						LET cCodRet = '00003'; 
						RETURN cCodRet;
					ELIF cCodRetSp::INTEGER = 2 THEN
						LET cCodRet = '00475'; --EL TOTAL DE DISPOSITIVOS DE SEGURIDAD SOLICITADOS NO ESTA EN EL RANGO PERMITIDO
						RETURN cCodRet;
...
CREATE PROCEDURE "informix".sp_consdireccionctemoral2(pNumCliente CHAR(20), pTipoDirec INTEGER)
		RETURNING CHAR(5) AS codret,
		INT AS vsecuencia,
		CHAR(1) AS vtipo_dir,
		CHAR(40) AS vcalle,
...
--		 SET DEBUG FILE TO '/tmp/mfinis/sp_consdireccionctemoral2.out';
-- TRACE ON;
		
		IF pNumCliente = '' OR pTipoDirec IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet, vsecuencia, vtipo_dir, vcalle, vcolonia, ventre_calles, vpais, vestado,
                vciudad, vmunicipio, vcod_postal, vapart_postal, vtipo_telef1, vtelefono1, vtipo_telef2, vtelefono2,
                vtipo_telef3, vtelefono3, vextension, vestado_inegi, vmunicipio_inegi, vlocalidad_inegi, vnumerociudad, vnumeroextcalle,
...
END PROCEDURE
DOCUMENT 'AUTOR: Johnattan Esquivel Sanchez',
'DESCRIPCION: sp clon sin validaciÃ³n de ejecutivo sp_consdireccionctemoral2',
'FECHA: 12/09/2022',
'BASE DE DATOS: bdicnweb';
