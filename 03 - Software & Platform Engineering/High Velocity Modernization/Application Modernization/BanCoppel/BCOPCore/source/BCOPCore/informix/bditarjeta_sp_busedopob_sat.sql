CREATE PROCEDURE "informix".sp_busedopob_sat(
		psNumtarjeta CHAR(16), 	-- Numero de tarjeta 
		piconsecutivo integer -- Consecutivo de  de tabla para ser procesada
	)
...
RETURNING 
	...
	--SET DEBUG FILE TO '/informix/HomeInformix/rrm/sp_busedopob_sat.out';
	--TRACE ON;
	...
	LET vsEncontrado = '';
		...
		IF ( (vsnumcliente1 IS NULL) OR ( TRIM (vsnumcliente1) = ''))  THEN
			/*NO EXISTE EL CLIENTE ASOCIADO A LA TARJETA */
			LET vssqlerr = '00001';
			LET vsEncontrado = 'F';
		else
		    LET vssqlerr = '00000';
...
'Descripcion:BUSCA ESTADO Y POBLACION DE AQUELLOS QUE FUERON PREMIADOS ',
'Fecha: 2014/11/18',
'Version: 20141118.1130',
'BD: bditarjeta';

CREATE PROCEDURE "informix".sp_busmovint_sat(
		psNumtarjeta CHAR(16), 	-- Numero de tarjeta 
...
RETURNING 
	...
	LET vmmontoarchivo = 0;
		...
		SET LOCK MODE TO WAIT 3;
		set isolation to dirty read;
		SELECT   numtarjeta,secuencia, 	secuenciaextendida, (NVL(monto,0)),	fechahorainauth, referencia,cuenta,	statustarjeta, referencia23_325
			...
			FROM    Intercard:"informix".sat_buenfin 
				WHERE 
		...
		IF ( (vsNumtarjeta IS NULL) OR ( TRIM (vsNumtarjeta) = '')) THEN
			...
			LET vssqlerr = '00400';
			LET vsErrorActividad = 'NO EXISTE EL MOVIMIENTO EN VISTA';
...
CREATE PROCEDURE "informix".sp_carga_sorteosat ()

RETURNING 
			VARCHAR (5) AS CodRet1, 
			VARCHAR(250) AS Mensaje_Respuesta1;

--Definición de Variables de Error
DEFINE vicodigo1 			integer;
DEFINE vsCodRet1 			VARCHAR(5);
DEFINE vsMensaje_Respuesta1 	VARCHAR(250);


--Definición de Variables de datos archivo
DEFINE vscadena 			varchar(80);
...
DEFINE viincremento integer;
DEFINE vsrrn 			char(12);

BEGIN
	ON EXCEPTION SET viCodigo1   --cacha el error en caso de que exista y regresa un valor predeterminado

				LET vsCodRet1 = viCodigo1;
				RETURN 	vsCodRet1, 
						vsMensaje_Respuesta1;

	END EXCEPTION;

--SET DEBUG FILE TO '/informix/HomeInformix/rrm/sp_carga_sorteosat.out';
--TRACE ON;

-- Inicialización de variables
LET 	vicodigo1 = 0;
LET		vsCodRet1 = '00000';
LET		vsMensaje_Respuesta1 = 'PROCESO TERMINADO SATISFACTORIAMENTE';
...
--Inicializacion de bandera de ciclo
...
LET viincremento = 0;
	...
	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	select count(*) into vinumreg from Bditarjeta:"informix".td_carga_archivo_sat;
	
	if vinumreg > 2 then -- Para que ganrantice la existencia de por lo menos 3 registros Encabezado,detalle,Pie
				SET LOCK MODE TO WAIT 3;
				SET ISOLATION TO DIRTY READ;
				Select limit 1 TRIM(SUBSTR(registro, 33,4)) into vsperiodo from BdiTarjeta:"informix".td_carga_archivo_sat
					where registro like '00|%';
				--   Pasar los registros de la tabla de carga a la tabla de procesos
				FOREACH WITH HOLD 
					SELECT Registro	INTO vscadena
						FROM BdiTarjeta:"informix".td_carga_archivo_sat
					
						IF (vsFlagEnTransaccion = 'F') THEN 
						 BEGIN WORK;
						 LET vsFlagEnTransaccion = 'V';
						END IF;
						-- ##########################   Para complementar RRN ##################################
						LET viincremento = length(TRIM(SUBSTR(vscadena, 53,12)));
						
						if viincremento < 12 then 
								let vsrrn = TRIM(SUBSTR(vscadena, 53,12));
								
								For vicontador = viincremento  TO 11 STEP 1
										LET vsrrn = '0'||vsrrn;
								end for;
						elif viincremento = 12 then 
								let vsrrn = TRIM(SUBSTR(vscadena, 53,12));
						else
							let vsrrn = '000000000000';
						end if;
						
						-- #######################################################################################
						
						if ((TRIM(SUBSTR(vscadena, 1,2 )) = '01') and  (TRIM(SUBSTR(vscadena, 66,12)) <> '000000000000')) then 
							insert into bditarjeta:"informix".tb_sorteo_sat
										(
											periodofiscal,
											tporeg,
											banemisor,
								...
								values
										(
										vsperiodo,							-- Del encabezado
										TRIM(SUBSTR(vscadena, 1,2)), 		-- Del detalle tipo de registro
										TRIM(SUBSTR(vscadena, 4,4)),		-- Numero del Banco
										TRIM(SUBSTR(vscadena, 9,6)),		-- Fecha de la transaccion
										TRIM(SUBSTR(vscadena, 16,16)),		-- Numero de la tarjeta
										TRIM(SUBSTR(vscadena, 33,12)),		-- Monto de transaccion
										TRIM(SUBSTR(vscadena, 46,6)),		-- Secuencia de transaccion
										vsrrn,								-- Referencia de POS de transaccion
										TRIM(SUBSTR(vscadena, 66,12)),		-- Monto del Premio otorgado a la transaccion
										'V', 								-- Se coloca bandera para acortar busqueda 
										'Transaccion Premiada'				-- Leyenda temporal 
										);
						elif ((TRIM(SUBSTR(vscadena, 1,2 )) = '01') and  (TRIM(SUBSTR(vscadena, 66,12)) = '000000000000')) then 
							insert into bditarjeta:"informix".tb_sorteo_sat
										(
											periodofiscal,
											tporeg,
								...
								values
										...
										vsrrn,								-- Referencia de POS de transaccion
										TRIM(SUBSTR(vscadena, 66,12)),		-- Monto del Premio otorgado a la transaccion
										'F', 								-- Se coloca bandera para acortar busqueda 
										'Transaccion no premiada',			-- Leyenda temporal 
										'0000000',							-- se rellena con ceros o valires fijos porque no se van a buscar 
										'0000000000000000',
										0.0,
										CAST('1900-10-10 12:00:00' AS DATETIME YEAR TO FRACTION(5)),
										vsrrn,
										'NAB',
										'NAB',
										'0000',
		...
		LET		vsCodRet1 = '00001';
		LET		vsMensaje_Respuesta1 = 'No existen registros a ser procesados';
	end if;
	
	RETURN 	vsCodRet1, 
			NVL(vsMensaje_Respuesta1,'');
...
'Proyecto: Sorteo de SAT del Buen Fin',
'Solicito: Luis Antonio Gomez Santiago',
'Descripcion: PROCESO DE CARGA DE TABLA DE TEMPORAL A TABLA DE SORTEO SAT PARA SU PROCESAMIENTO',
'Fecha: 2013/11/20',
'Version: 20131112.1200',
'BD: bditarjeta',
'',
'Modifico: Ricardo Resendiz Martinez',
'Proyecto: Sorteo de SAT del Buen Fin',
'Solicito: Luis Antonio Gomez Santiago',
'Descripcion: PROCESO DE CARGA DE TABLA DE TEMPORAL A TABLA DE SORTEO SAT PARA SU PROCESAMIENTO COMPLEMENTADO RRN Y DISCRIMINANDO AQUELAS QUE NO FUERON PREMIADAS',
'Fecha: 2013/11/20',
'Version: 20131213.1300',
'BD: bditarjeta';

CREATE PROCEDURE "informix".sp_valida_sorteosat 
(
...
CREATE PROCEDURE "informix".sp_busmovdev_sat(
		psNumtarjeta CHAR(16), 	-- Numero de tarjeta 
		psSecuencia CHAR(6) -- Secuencia de autorizacion
	)
