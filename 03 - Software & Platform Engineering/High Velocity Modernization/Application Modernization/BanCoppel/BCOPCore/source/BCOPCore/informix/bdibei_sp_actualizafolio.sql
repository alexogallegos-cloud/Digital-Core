CREATE PROCEDURE "informix".sp_actualizafolio(pUsuario CHAR(8), pIdFuncion CHAR(10), pValor CHAR(3))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_actualizafolio.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pValor = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF;
		...
		LET pValor = pValor + 1;
		UPDATE bdibpi:"informix".bpi_parametros_contenido SET folio = pValor;
...
'FECHA: 11/11/2014',
'DESCRIPCION: Procedimiento que actualiza el folio para poder generar el folioSuc en SOE',
'BD: bdibei';
...
CREATE PROCEDURE "informix".sp_cambiostatus_token_bei(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumCliente CHAR(9), pStatusNuevo CHAR(3), pCanal CHAR(2), pNumSolicitud CHAR(10))					
	...
	BEGIN
		...
		WHERE numcte = pNumCliente AND id_status = 120 AND solicitud = pNumSolicitud; -- UNICOS VALORES id_status: 100/130/199


		-- Cambio de status a los tokens en tablas tkn_nseries, tkn_status_token 	
		IF pNumCliente <> '' AND pStatusNuevo <> '' AND pCanal <> '' THEN
			
			FOREACH 
				SELECT COUNT(*)
				INTO iTotalReg FROM bdibei:"informix".bei_tokensolicitud 
				WHERE numcte = pNumCliente
				
				IF iTotalReg > 0 THEN	
					
					-- Este spl ejecuta internamente el sp_set_estatus_tokenasociados_bei y actualiza status de tokens asociados al cliente
					EXECUTE PROCEDURE bdibei:"informix".sp_set_estatus_tokenasociados_bei(pNumSolicitud, pNumCliente, pStatusNuevo, pUsuario, pCanal)
					INTO cCodRetSp;
					
					LET iCodRetSp = cCodRetSp::INTEGER;
					IF iCodRetSp < 0 THEN
						RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_set_estatus_tokenasociados_bei'; -- '000002' 
					ELIF iCodRetSp = 00003 THEN
						LET cCodRet = '00413'; -- 'EL NÃMERO DE SOLICITUD DE TOKEN NO CORRESPONDE, POR FAVOR VERIFIQUE'
					ELIF iCodRetSp = 00002 THEN
						LET cCodRet = '00417'; -- 'NO SE ENCONTRO TOKEN CON EL ESTATUS INDICADO'
					ELIF iCodRetSp = 00001 THEN
						LET cCodRet = '00003'; -- 'FALTA ALGUN PARAMETRO DE ENTRADA'
					RETURN cCodRet; --
					END IF;
							
				ELSE
					LET cCodRet = '00022'; --'000003'; 'El NUMERO DE CLIENTE NO EXISTE'
					RETURN cCodRet;
				END IF;
			END FOREACH;
		...
		--Cambio de status a la solicitud en tablas bei_tokensolicitud, bei_stasolicitud, bei_solicitudtoken, bei_envios 
		IF pNumCliente <> '' AND pStatusNuevo <> '' THEN  

			FOREACH
				SELECT COUNT(*)
				INTO iTotalReg FROM bdibei:"informix".bei_solicitudtoken
				WHERE numcte = pNumCliente
				
				IF iTotalReg > 0 THEN
					--Este spl cambia el status en tablas bei_tokensolicitud,bei_stasolicitud ,bei_solicitudtoken,bei_envios 
					EXECUTE PROCEDURE bdibei:"informix".sp_set_solicitudstatus_admtoken_bei(pNumSolicitud, pNumCliente, pUsuario, sStatusViejo, pStatusNuevo)
					INTO cCodRetSp;
					
					LET iCodRetSp = cCodRetSp::INTEGER;
					IF iCodRetSp < 0 THEN
						RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_set_solicitudstatus_admtoken_bei'; -- '000004'
					ELIF iCodRetSp = 001 THEN
						LET cCodRet = '00053'; -- '001'; '00053','EL CLIENTE QUE DESEA CONSULTAR NO EXISTE'); Ã '00022','El NUMERO DE CLIENTE NO EXISTE'
					RETURN cCodRet; --
					END IF;
					
				ELSE
					LET cCodRet = '00022'; --'000000'; 'El NUMERO DE CLIENTE NO EXISTE'
					RETURN cCodRet;
				END IF;
			END FOREACH;
...
'FECHA: 03/12/2014',
'DESCRIPCION: Cambia status de 120 (enviado) a 130 (preactivado o recibido por el cliente) y que sea utilizado en SOE',
'BD: bdibei';

CREATE PROCEDURE "informix".sp_consultaparametro(pUsuario CHAR(8), pIdFuncion CHAR(10), pTabla CHAR(3), pSistemaBase CHAR(2), pCampo CHAR(15), pWhere CHAR(15), pParametro CHAR(15))
		RETURNING CHAR(5) AS codret,
				  CHAR(20) AS valor;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultaparametro.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pTabla = '' OR pSistemaBase = '' OR pCampo = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cValor;
		END IF;
		
		IF pSistemaBase = 00 THEN
			LET cBaseDatos = 'bdibpi'; --CASO ESPECIAL
		ELIF pSistemaBase = 01 THEN
			LET cBaseDatos = 'bdibi';
		ELIF pSistemaBase = 02 THEN
			LET cBaseDatos = 'bdibpi';
		ELIF pSistemaBase = 03 THEN
			LET cBaseDatos = 'bdicheq';
		ELIF pSistemaBase = 04 THEN
			LET cBaseDatos = 'bdicnweb';
		ELIF pSistemaBase = 05 THEN
			LET cBaseDatos = 'bdicobranza';
		ELIF pSistemaBase = 06 THEN
			LET cBaseDatos = 'bdicred';
		ELIF pSistemaBase = 07 THEN
			LET cBaseDatos = 'bdidigital';
		ELIF pSistemaBase = 08 THEN
			LET cBaseDatos = 'bdinteg';
		ELIF pSistemaBase = 09 THEN
			LET cBaseDatos = 'bdibei';
		END IF;
		
		IF pTabla = 000 THEN
			LET cTabla = 'bpi_parametros_contenido'; --CASO ESPECIAL
		ELIF pTabla = 001 THEN
			LET cTabla = 'soe_parametros';
		ELIF pTabla = 002 THEN
			LET cTabla = 'bpi_parametros_contenido';
		END IF;
		...
		IF pSistemaBase <> '00' AND cTabla <> '000' THEN
			IF pWhere = '' THEN
				LET cQuery = "SELECT" || " "||TRIM(pCampo) || " " ||"FROM" || " " ||TRIM(cBaseDatos) || ":"|| "'informix'."||TRIM(cTabla)||";";
				--||"'informix'."
			...
			ELSE
				LET cQuery = "SELECT" || " "||TRIM(pCampo) || " " ||"FROM" || " " ||TRIM(cBaseDatos) || ":" || "'informix'."||TRIM(cTabla)|| " " ||
				"WHERE" || " " || TRIM(pWhere) || " " ||"=" || " '" || TRIM(pParametro) ||"';";
		...
		ELSE --CASO ESPECIAL 
			...
			EXECUTE PROCEDURE bdibei:"informix".sp_actualizafolio(pUsuario, pIdFuncion, cValor)
			INTO cCodRetSp;
...
'FECHA: 07/11/2014',
'DESCRIPCION: Procedimiento para obtener algun parametro con o sin condiciÃ³n',
'PARAMETROS;',
'tabla;',
	'000 bpi_parametros_contenido Caso especial, Ejecuta sp_actualizafolio el cual actualiza el numero de folio,',
	'001 soe_parametros,',
	'002 bpi_parametros_contenido,',
'sistemaBase;',
	'00 bdibei, CASO ESPECIAL,',
	'01 bdibi, 02 bdibpi, 03 bdicheq, 04 bdicnweb,',
	'05 bdicobranza, 06 bdicred, 07 bdidigital, 08 bdinteg, 09 bdibei',
'campo: Nombre del campo a traer,',
'where: Nombre del campo a comparar,',
'parametro: Valor del parametro a comparar.',
'BD: bdibei';

CREATE PROCEDURE "informix".sp_soe_baja_admin(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumCte CHAR(9), pNumIdentif CHAR(30), pNumSerTkn CHAR(9), pIpUsuario CHAR(15), pSucursal CHAR(4))
        RETURNING CHAR(5) AS codret;
    ...
    DEFINE cMensajeRetorno CHAR(50);
    ...
    BEGIN
                ...
                --SET DEBUG FILE TO '/tmp/mfinis/sp_soe_baja_admin.out';
                --TRACE ON;

                IF pUsuario = '' OR pIdFuncion = '' OR pNumCte = '' OR pNumIdentif = '' OR  pNumSerTkn = '' THEN
                        LET cCodRet = '00003';
                        RETURN cCodRet;
                END IF;
                ...
                SELECT NVL(id_usuario,0) INTO iIdUsuario FROM
                bdibei:"informix".bei_servicio WHERE  num_cliente = pNumCte AND identificacion_admin = pNumIdentif;

                IF NOT EXISTS(SELECT NVL(id_status,0) FROM bdibei:"informix".bei_servicio WHERE num_cliente = pNumCte AND identificacion_admin = pNumIdentif AND id_status = 99)THEN
                        ...
                        AND identificacion_admin = pNumIdentif;
                END IF;

                IF NOT EXISTS(SELECT NVL(id_status,0) FROM bdibei:"informix".bei_usuario WHERE num_cliente = pNumCte AND id_usuario = iIdUsuario AND id_status = 99)THEN
						-- obtiene status actual para insertar a bei_cambiostusuario
						SELECT id_status INTO iIdStatusActualUsuario 
						...
						-- Se inserta registro de usuario que se da de baja
						INSERT INTO bdibei:"informix".bei_cambiostusuario (numcliente, id_statusanterior, id_statusactual, ipusuario, fecha_cambio, suc_cambio, usuario_cambio, id_usuario)
                        VALUES (pNumCte, iIdStatusActualUsuario, 99, pIpUsuario, CURRENT, pSucursal, pUsuario, iIdUsuario);
						
                END IF;

                IF NOT EXISTS(SELECT NVL(id_status_token,0) FROM bdibei:"informix".bei_token WHERE num_cliente = pNumCte AND id_usuario = iIdUsuario AND ns_token = pNumSerTkn AND id_status_token = 199)THEN

                        EXECUTE PROCEDURE bdibei:"informix".sp_soe_obtenertoken(pUsuario, pIdFuncion, pNumCte, pNumSerTkn)
                        INTO cCodRetSp, cNsToken, sIdStatus;

                        LET iCodRetSp = cCodRetSp::INTEGER;
                        IF iCodRetSp < 0 THEN
                                RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_soe_obtenertoken';
                        ELIF iCodRetSp =  00003 THEN
                                LET cCodRet = '00003'; -- FALTA ALGUN PARAMETRO DE ENTRADA
                                RETURN cCodRet;
                        ELIF iCodRetSp =  00408 THEN
                                ...
                                RETURN cCodRet;
                        ELIF iCodRetSp =  00409 THEN
                                LET cCodRet = '00409'; -- TOKEN CON ESTATUS CANCELADO
                                RETURN cCodRet;
                        END IF;

                        EXECUTE PROCEDURE "informix".sp_soe_set_solicitudstatus(pUsuario, pIdFuncion, pNumCte, pNumSerTkn, '199')
                        INTO cCodRetSp;

                        LET iCodRetSp = cCodRetSp::INTEGER;
                        IF iCodRetSp < 0 THEN
                                RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_soe_set_solicitudstatus';
                        ELIF iCodRetSp =  00003 THEN
                                LET cCodRet = '00003'; -- FALTA ALGUN PARAMETRO DE ENTRADA
                                RETURN cCodRet;
                        ELIF iCodRetSp =  00017 THEN
                                LET cCodRet = '00017'; -- NO SE OBTUVIERON RESULTADOS
                                RETURN cCodRet;
                        END IF;

                        EXECUTE PROCEDURE "informix".sp_soe_cancelartoken(pUsuario, pIdFuncion, pNumCte, pNumSerTkn, sIdStatus, '199', 18)
                        INTO cCodRetSp;

                        LET iCodRetSp = cCodRetSp::INTEGER;
                        IF iCodRetSp < 0 THEN
                                RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_soe_cancelartoken';
                        ELIF iCodRetSp =  00003 THEN
                                LET cCodRet = '00003'; -- FALTA ALGUN PARAMETRO DE ENTRADA
                                RETURN cCodRet;
                        ELIF iCodRetSp =  00394 THEN
                                LET cCodRet = '00394'; -- OCURRIO UN ERROR AL TRATAR DE CANCELAR EL TOKEN
                                RETURN cCodRet;
                        END IF;

                        EXECUTE PROCEDURE "informix".sp_soe_set_statustoken(pUsuario, pIdFuncion, pNumSerTkn, sIdStatus, '199', pUsuario, 18)
                        INTO cCodRetSp, cMensajeRetorno;

                        LET iCodRetSp = cCodRetSp::INTEGER;
                        IF iCodRetSp < 0 THEN
                                RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_soe_set_statustoken';
                        ELIF iCodRetSp =  00003 THEN
                                LET cCodRet = '00003'; -- FALTA ALGUN PARAMETRO DE ENTRADA
                                RETURN cCodRet;
                        /*ELIF iCodRetSp = 00189 THEN
                                LET cCodRet = '00417'; -- NO SE ENCONTRO TOKEN CON EL ESTATUS INDICADO
                                RETURN cCodRet;*/
                        END IF;
...
DOCUMENT 'AUTOR: Esparza Brenis Fernando MartÃ­n',
'FECHA: 13/11/2014',
'DESCRIPCION: Procedimiento que realiza la Re-asignaciÃ³n de Rol Administrador en SOE',
'AUTOR: L. Montserrat Leon Amador',
'FECHA: 23/12/2014',
'DESCRIPCION: Se implemento la ejecucion del SLP sp_soe_set_statustoken, el cual hace el guarda el estatus del token para SOE en SOC',
'FECHA: 08/01/2015',
'DESCRIPCION: Se implemento la insercion en la tabla bei_cambiostusuario para bitacoreo de usuario que se da de baja en rol administrador',
'BD: bdibei';
...
CREATE PROCEDURE "informix".sp_soe_cancelaserviciobei(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumCliente CHAR(20), pIdStatusNuevo SMALLINT, pIpUsuario CHAR(15), pSucursal CHAR(4), pIdStatusTokenNuevo SMALLINT, pCanal CHAR(2), pIdStatusProceso SMALLINT, pDescripcionEstatus CHAR(50))
        ...
        BEGIN
                ...
                IF pUsuario = '' OR pIdFuncion = '' OR pNumCliente = '' OR pIdStatusNuevo = '' OR pIpUsuario = '' OR pSucursal = '' 
                        OR pIdStatusTokenNuevo IS NULL OR pCanal = '' OR pIdStatusProceso = '' OR pDescripcionEstatus = '' THEN
                ...
                -- VALIDACIÃN DE QUE SE TENGA CONTRATADO EL SERVICIO
                IF EXISTS (SELECT num_cliente FROM bdibei:"informix".bei_contratacion WHERE num_cliente = pNumCliente) THEN
                        -- VALIDACIÃN DE QUE EL SERVICIO DE EMPRESANET NO ESTE CANCELADO
                        IF (SELECT status_contrato FROM bdibei:"informix".bei_contratacion WHERE num_cliente = pNumCliente) = 30 THEN
                                
                                -- ACTUALIZACION DEL ESTATUS
                                UPDATE bdibei:"informix".bei_contratacion
                                SET status_contrato = pIdStatusNuevo
                                WHERE num_cliente = pNumCliente;
                                
                        ELSE
								LET pIdStatusProceso = 10;
                                LET pDescripcionEstatus = 'CANCELA SERVICIO GENERAL';
								
								-- BITACOREO DEL ESTATUS DE LA CANCELACION DEL SERVICIO
								EXECUTE PROCEDURE bdibei:"informix".sp_soe_obtstatus_cancserv(pUsuario, pIdFuncion, LPAD(pIdStatusProceso, 2, '0'), pDescripcionEstatus, pUsuario, pNumCliente)
								INTO cCodRetSp;
								
								LET iCodRetSp = cCodRetSp::INTEGER;
								IF iCodRetSp < 0 THEN
										RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_soe_obtstatus_cancserv';
								ELIF iCodRetSp > 0 THEN
									RETURN cCodRetSp, pIdStatusProceso, pDescripcionEstatus, iIdUsuario, cNumSerieToken;
								END IF;
						
                                LET cCodRet = '00393'; -- EL SERVICIO DE EMPRESANET SE ENCUENTRA CANCELADO
                                RETURN cCodRet, pIdStatusProceso, pDescripcionEstatus, iIdUsuario, cNumSerieToken;
                        END IF;
                ELSE
						-- BITACOREO DEL ESTATUS DE LA CANCELACION DEL SERVICIO
...
--              -- OBTENCIÃN DEL ESTATUS ACTUAL DE LOS USUARIOS DEL SERVICIO
				LET pIdStatusProceso = 20;
				LET pDescripcionEstatus = 'CANCELA USUARIOS';
								
                SET ISOLATION TO DIRTY READ;
                FOREACH SELECT id_usuario, id_status INTO iIdUsuario, iIdStatusActualUsuario FROM bdibei:"informix".bei_usuario 
                        WHERE num_cliente = pNumCliente
                                AND id_status != 99
                
                        UPDATE bdibei:"informix".bei_usuario
                        SET id_status = pIdStatusNuevo,
                                f_status = CURRENT
                        WHERE num_cliente = pNumCliente
                                AND id_usuario = iIdUsuario;
                                
                        INSERT INTO bdibei:"informix".bei_cambiostusuario (numcliente, id_statusanterior, id_statusactual, ipusuario, fecha_cambio, suc_cambio, usuario_cambio, id_usuario)
                        VALUES (pNumCliente, iIdStatusActualUsuario, pIdStatusNuevo, pIpUsuario, CURRENT, pSucursal, pUsuario, iIdUsuario);
                        
                        IF (SELECT id_tipo_usuario FROM bdibei:"informix".bei_usuario WHERE id_usuario = iIdUsuario AND num_cliente = pNumCliente) = 1 THEN
                ...
                SET ISOLATION TO DIRTY READ;
                -- BUSQUEDA DE LOS NUMEROS DE SOLICITUDES DEL CLIENTE
                FOREACH SELECT solicitud, id_status INTO cSolicitud, iIdStatusSolicitud FROM bdibei:"informix".bei_solicitudtoken
                        WHERE id_status != 199 AND numcte = pNumCliente
                        
                        IF iIdStatusSolicitud BETWEEN 100 AND 180 THEN
                        
                                FOREACH SELECT b.ns_token, c.id_usuario
                                                INTO cNumSerieToken, iIdUsuario
                                                FROM bdibei:"informix".bei_solicitudtoken a INNER JOIN bdibei:"informix".bei_tokensolicitud b 
                                                                ON a.numcte = pNumCliente AND a.solicitud = cSolicitud 
                                                                        AND b.numcte = a.numcte AND b.solicitud = a.solicitud
                                                        LEFT JOIN bdibei:"informix".bei_token c 
                                                                ON c.num_cliente = a.numcte AND c.ns_token = b.ns_token and c.id_status_token != 199
                                        
                                        -- BUSQUEDA DEL ESTATUS DEL TOKEN
                                        SELECT id_status INTO iIdStatusToken
                                        FROM bdibpi:"informix".tkn_nseries
                                        WHERE ns_token = cNumSerieToken;
                                        
                                        -- EJECUCIÃN DE SPL sp_soe_set_solicitudstatus
                                        EXECUTE PROCEDURE bdibei:"informix".sp_soe_set_solicitudstatus(pUsuario, pIdFuncion, pNumCliente, cNumSerieToken, pIdStatusTokenNuevo)
                                        INTO cCodRetSp;
                                        ...
                                        IF iIdUsuario IS NOT NULL THEN -- EL TOKEN TIENE UN USUARIO ASOCIADO
                                                -- CANCELACIÃN DEL TOKEN
                                                EXECUTE PROCEDURE bdibei:"informix".sp_soe_cancelartoken(pUsuario, pIdFuncion, pNumCliente, cNumSerieToken, iIdStatusToken, pIdStatusTokenNuevo, pCanal)
                                                INTO cCodRetSp;
                                                ...
                                                RETURN cCodRet, pIdStatusProceso, pDescripcionEstatus, iIdUsuario, cNumSerieToken WITH RESUME;
                                                LET iRegUsuarios = iRegUsuarios + 1;
                                        END IF;
                                        
                                        EXECUTE PROCEDURE bdibei:sp_soe_set_statustoken(pUsuario, pIdFuncion, cNumSerieToken, iIdStatusToken, pIdStatusTokenNuevo, pUsuario, pCanal)
                                        INTO cCodRetSp, cMensajeRetorno;
                                ...
                                UPDATE bdibei:"informix".bei_solicitudtoken
                                SET id_status = pIdStatusTokenNuevo
                                WHERE numcte = pNumCliente
                                        AND solicitud = cSolicitud;
                                        
                                INSERT INTO bdibei:"informix".bei_stasolicitud(solicitud, anterior, actual, f_registro)
                                VALUES (cSolicitud, iIdStatusSolicitud, pIdStatusTokenNuevo, CURRENT);

                                /*la tabla bei_tkn_envios no existe
								IF (SELECT id_status FROM bdibei:"informix".bei_tkn_envios WHERE solicitud = cSolicitud AND num_cliente = pNumCliente) BETWEEN 100 AND 130 THEN
                                        
                                        UPDATE bdibei:"informix".bei_tkn_envios
                                        SET id_status = pIdStatusTokenNuevo
                                        WHERE num_cliente = pNumCliente
                                                AND solicitud = cSolicitud;
								...
								*/
                                ...
                                -- SE ACTUALIZA EL ESTATUS DE LA SOLICITUD EN BEI_ENVIOS
                                UPDATE bdibei:"informix".bei_envios
                                SET id_status = 199
                                WHERE numcte = pNumCliente and solicitud = cSolicitud;

                        END IF;
                        
                END FOREACH;
                
                LET pIdStatusProceso = 40;
                LET pDescripcionEstatus = 'CANCELA SOLICITUDES';
...
'FECHA: 29/10/2014',
'DESCRIPCION: Proceso de cancelaciÃ³n del servicio SOE (EmpresaNet)',
'FECHA: 08/01/2015',
'DESCRIPCION: Bitacoreo del proceso de cancelaciÃ³n',
'BD: bdibei';

CREATE PROCEDURE "informix".sp_soe_cancelatokenporactivar(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumCliente CHAR(20), pNumSerieToken CHAR(10), pIdNuevoStatusToken SMALLINT)
	RETURNING CHAR(5) AS codret;
	...
	LET cNumSerieToken = '';
	 ...
	 BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_soe_cancelatokenporactivar.out';
		--TRACE ON;
		
		---VALIDACION DE DATOS REQUERIDOS 
		IF pUsuario = '' OR pIdFuncion = '' OR pNumCliente = '' OR pNumSerieToken = '' OR pIdNuevoStatusToken IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF;
		...
		SELECT a.ns_token
		INTO cNumSerieToken
		FROM bdibei:"informix".bei_tokensolicitud a, bdibpi:"informix".tkn_nseries b
		...
		IF iExisteRegistro = 1 THEN 
			...
			WHERE numcte = pNumCliente
				AND ns_token = cNumSerieToken;
			
			--SE CANCELA TOKEN DE LA TABLA tkn_nseries 
			SET LOCK MODE TO WAIT 3;
...
'FECHA: 22/01/2015',
'DESCRIPCION: Procedimiento que cancela un token con estatus por activar en tablas bei_tokensolicitud y tkn_nseries',
'BD: bdibei';

CREATE PROCEDURE "informix".sp_soe_constknacancelar(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumCte CHAR(9), pNumSerTkn CHAR(9))
		RETURNING CHAR(5) AS codret,
				  CHAR(1) AS cTipoUsuario,
				  CHAR(20) AS descripcion,
				  INTEGER AS iIdUsuario;

	DEFINE cCodRet CHAR(5);
	DEFINE iSqlErr INTEGER;
	DEFINE cIdTipoUsuario CHAR(1);
	DEFINE cDescripcion CHAR(20);
	DEFINE cEsReplegal CHAR(1);
	DEFINE iIdUsuario INTEGER;
	DEFINE iIdStatusNSToken INTEGER;
	...
	LET cEsReplegal = '';
	LET iIdUsuario = 0;
	LET iIdStatusNSToken = 0;
	
	BEGIN
	
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cIdTipoUsuario, cDescripcion, iIdUsuario;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_soe_constknacancelar.out';
		--TRACE ON;
		...
		IF EXISTS(SELECT ns_token FROM bdibei:"informix".bei_tokensolicitud WHERE numcte = pNumCte AND ns_token = pNumSerTkn) THEN
			
			SET ISOLATION TO DIRTY READ;
			SELECT id_status
			INTO iIdStatusNSToken
			FROM bdibei:"informix".bei_tokensolicitud a
			WHERE a.numcte = pNumCte AND a.ns_token = pNumSerTkn;
			
			IF iIdStatusNSToken != 199 THEN
				-- BUSQEUDA DEL TIPO DE USUARIO AL QUE ESTA ASIGNADO EL TOKEN
				SELECT id_tipo_usuario, id_usuario
				INTO cIdTipoUsuario, iIdUsuario
				FROM bdibei:bei_usuario a
				WHERE a.id_usuario IN (SELECT id_usuario
									   FROM bdibei:bei_token
									   WHERE id_usuario in (SELECT id_usuario 
															FROM bdibei:bei_usuario WHERE num_cliente = a.num_cliente)
									   AND ns_token = pNumSerTkn)
					AND a.num_cliente = pNumCte;
					
				IF cIdTipoUsuario IS NULL THEN
					LET cCodRet = '00412'; --EL NÃMERO DE SERIE TOKEN NO SE ENCUENTRA ASIGNADO
				ELSE
					IF cIdTipoUsuario = '1' THEN
						LET cDescripcion = 'ADMINISTRADOR';
					ELIF cIdTipoUsuario = '2' THEN
						LET cDescripcion = 'OPERADOR';
					END IF;

					SELECT es_replegal INTO cEsReplegal FROM bdibei:"informix".bei_servicio  WHERE num_cliente = pNumCte AND id_usuario = iIdUsuario;

					IF cEsReplegal = '1' AND cIdTipoUsuario = '1' THEN
						LET cDescripcion = 'REPRESENTANTE LEGAL';
					END IF;		
				END IF;
			ELSE
				LET cCodRet = '00409'; --TOKEN CON ESTATUS CANCELADO
...
DOCUMENT 'AUTOR: Esparza Brenis Fernando Martin',
'FECHA: 11/11/2014',
'DESCRIPCION: Procedimiento que valida que el numero de serie token exista con un estatus difetente a 199 y regresa el Tipo de Usuario en SOE',
'BD: bdibei';
