CREATE PROCEDURE "informix".sp_rep_sac_reportedomiciliacion(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaInicial DATE, pFechaFinal DATE, pSucursal CHAR(4),pRutaDescarga CHAR(100), pTipoR CHAR(1))
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
            LET cCodRet = iSqlErr;
			UPDATE "informix".sw_sac_reportedomiciliaciongridRep
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_rep_sac_reportedomiciliacion.out';
		--TRACE ON;
		...
		DELETE FROM "informix".sw_sac_reportedomiciliaciongridRep
		WHERE usuario = pUsuario AND tipo_proceso = 'LECTURA'; 
 
		-- SE INSERTA A TABLA PARA EL MONITOREO DEL STATUS
		INSERT INTO "informix".sw_sac_reportedomiciliaciongridRep(usuario, nombre_archivo, status, bandera_det_error, error_proceso, tipo_proceso, error, total_registros) 
		VALUES(pUsuario,'','I','','','LECTURA','',0);
 
	    IF nvl(pSucursal,'') ='' THEN
		SELECT COUNT(*)
		INTO iNumRegistros
		FROM bdisac:sac_reportediariodomi_seg WHERE fecha_pago BETWEEN pFechaInicial AND pFechaFinal;
		ELSE
		SELECT COUNT(*)
		INTO iNumRegistros
		FROM bdisac:sac_reportediariodomi_seg WHERE fecha_pago BETWEEN pFechaInicial AND pFechaFinal AND sucursal_alta=pSucursal; 
		END IF;
		...
		LET cCmd1 ="";
        IF nvl(pSucursal,'') ='' THEN
        LET cCmd1 =""||TRIM(cCmd1)||"SELECT ' ',' ',' ',' ',' ',' ','REPORTE DOMICILIACION CLUB DE PROTECCIÓN FAMILIAR',' ',' ',' ',' ',' ',' ',' ',' ',' '  FROM systables WHERE tabid = 1  UNION ALL SELECT ' ','Periodo del: "|| LPAD(DAY(pFechaInicial),2,0)||'/'||LPAD(MONTH(pFechaInicial),2,0)||'/'||YEAR(pFechaInicial)||"  A: "|| LPAD(DAY(pFechaFinal),2,0)||'/'||LPAD(MONTH(pFechaFinal),2,0)||'/'||YEAR(pFechaFinal)||"',' ','Fecha del reporte:"||LPAD(DAY(dFechaHoy),2,0)||'/'||LPAD(MONTH(dFechaHoy),2,0)||'/'||YEAR(dFechaHoy)||' '||TO_CHAR(CURRENT, '%H:%M:%S')||"',' ','SUCURSAL: TODAS ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' '  FROM systables WHERE tabid = 1 UNION ALL";
		ELSE
		LET cCmd1 =""||TRIM(cCmd1)||"SELECT ' ',' ',' ',' ',' ',' ','REPORTE DOMICILIACION CLUB DE PROTECCIÓN FAMILIAR',' ',' ',' ',' ',' ',' ',' ',' ',' '  FROM systables WHERE tabid = 1  UNION ALL SELECT ' ','Periodo del: "|| LPAD(DAY(pFechaInicial),2,0)||'/'||LPAD(MONTH(pFechaInicial),2,0)||'/'||YEAR(pFechaInicial)||"  A: "|| LPAD(DAY(pFechaFinal),2,0)||'/'||LPAD(MONTH(pFechaFinal),2,0)||'/'||YEAR(pFechaFinal)||"',' ','Fecha del reporte:"||LPAD(DAY(dFechaHoy),2,0)||'/'||LPAD(MONTH(dFechaHoy),2,0)||'/'||YEAR(dFechaHoy)||' '||TO_CHAR(CURRENT, '%H:%M:%S')||"',' ','SUCURSAL: "||cSuc|| " ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' '  FROM systables WHERE tabid = 1 UNION ALL";
		END IF;   
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'FECHA PAGO','REFERENCIA PAGO','IMPORTE PAGO','COMISION PAGO','IVA PAGO','MESES PAGO','No. CLIENTE COPPEL','No. CLIENTE','No. POLIZA','MONTO MES','SUCURSAL ALTA','PROMOTOR','NOMBRE PROMOTOR','TIPO PLAN','FECHA ALTA','FECHA CAMBIO' FROM systables WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT  LPAD(DAY(fecha_pago),2,0)||'/'||LPAD(MONTH(fecha_pago),2,0)||'/'||YEAR(fecha_pago),''''||num_cliente::CHAR(20), importe_pago::CHAR(18), importe_comision_convenio::CHAR(18), iva_comision_convenio::CHAR(18),meses::CHAR(18),''''||numcte_coppel::CHAR(20),''''||numcte::CHAR(20),''''||num_poliza::CHAR(20),monto_mes::CHAR(23),''''||sucursal_alta::CHAR(18),''''||promotor::CHAR(18),nom_promotor::CHAR(45),tipo_plan::CHAR(18),LPAD(DAY(fecha_alta),2,0)||'/'||LPAD(MONTH(fecha_alta),2,0)||'/'||YEAR(fecha_alta),LPAD(DAY(fecha_cambio),2,0)||'/'||LPAD(MONTH(fecha_cambio),2,0)||'/'||YEAR(fecha_cambio)";
        LET cCmd1 =""||TRIM(cCmd1)||" FROM bdisac:""informix"".sac_reportediariodomi_seg";
		IF nvl(pSucursal,'') ='' THEN
        LET cCmd1 =""||TRIM(cCmd1)||" WHERE fecha_pago BETWEEN '"||cFecha1||"' AND '"||cFecha2||"'" ;
		ELSE
		LET cCmd1 =""||TRIM(cCmd1)||" WHERE fecha_pago BETWEEN '"||cFecha1||"' AND '"||cFecha2||"' AND sucursal_alta= '"||pSucursal||"'" ;
		END IF;
		...
		IF (pTipoR=1) THEN 
		LET cNombreArchivo = 'REP_DOMICILIACION_CPF_'||TRIM(cFechaHoraArchivo)||'.xlsx';
		ELSE 
		LET cNombreArchivo = 'REP_DOMICILIACION_CPF_'||TRIM(cFechaHoraArchivo)||'.csv';
		END IF;
