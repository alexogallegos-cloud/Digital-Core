CREATE PROCEDURE "informix".sp_japac_validadv(pReferencia CHAR(22))
	RETURNING 
    CHAR (5)  AS cCodRet,
...
DEFINE i 				INTEGER;
DEFINE cDigVer        	INTEGER;
DEFINE cReferencia		CHAR(31);
DEFINE iDVRef			INTEGER;
DEFINE iNum				INTEGER;
...
LET iModulo			= 0;
	...
	--SET DEBUG FILE TO '/home/sysifx/JesusBueno/1483/sp_validadv_japac.out';
	--TRACE ON;
...
BEGIN
	...
	IF TRIM(NVL(pReferencia,'')) = '' OR iLongDV < 22 THEN
		LET cCodRet = '00001';
		LET cDescripcion = 'Referencia incorrecta, favor de validar';
	ELSE
		
		LET cReferencia = SUBSTR(pReferencia,4,18);
		LET cDigVer=SUBSTR(pReferencia,22,1);
		LET iLongDV = LENGTH(TRIM(cReferencia));
		
		FOR i = 1 TO iLongDV 
			LET iNum = SUBSTRING (cReferencia FROM i FOR 1);
			IF(iMultiplo = 1) THEN
				LET iMultiplo = 2;
			ELSE
				LET iMultiplo = 1;
		...
		END FOR;
		LET iModulo = TRUNC((iSuma / 10));
		LET iModulo = iModulo * 10;
		LET iResta = iSuma - iModulo;
		
		IF iResta = 0 THEN	    
			LET iDVRef = 0;
		ELSE  
			LET iDVRef = 10 - iResta;
		END IF;
		
		IF iDVRef <> cDigVer THEN
			LET cCodRet = '00437';
...
'Fecha: 20/04/2015',
'DescripciÃ³n: Se crea procedimiento para validar la linea de captura de JAPAC',
'BD: bdisac';

CREATE PROCEDURE "informix".sp_obtienefoliocoppel_hs(Cajero char(8),Hora char(6))
RETURNING CHAR(5), CHAR(16);
	...
	--SET DEBUG FILE TO "/informix/EPG/sp_obtienefoliocoppel_hs.out";
	--TRACE ON;
	...
	BEGIN
		...
		BEGIN WORK;
			...
			UPDATE "informix".sac_recibo_coppel
		 	   SET valor = (CASE WHEN (SELECT valor FROM "informix".sac_recibo_coppel WHERE cod_param = 8889) <> (SELECT valor FROM "informix".sac_recibo_coppel WHERE cod_param = 8888) THEN (SELECT CAST (CAST(valor AS INTEGER) + 1 AS CHAR(10)) AS INTEGER FROM "informix".sac_recibo_coppel WHERE cod_param = 8888) ELSE '1' END)
			 WHERE cod_param = 8888;					

			SELECT (CASE WHEN valor = 1 THEN (SELECT valor FROM "informix".sac_recibo_coppel WHERE cod_param = 8889) ELSE CAST((valor:: INTEGER - 1) AS CHAR(10)) END)
			INTO cConsecutivo
			FROM "informix".sac_recibo_coppel
			WHERE cod_param = 8888;
			
			IF cConsecutivo IN (1,2,3,4,5,6,7,8,9) THEN
				LET cConsecutivo = '0' || cConsecutivo;
			END IF;
	

		
/*
            LET Cajero = lpad(Cajero,8,'00000000');
            LET Hora = lpad(Hora,6,'000000');
            LET cConsecutivo = lpad(cConsecutivo,2,'00');
*/


			LET cFolioSuc = REPLACE((Cajero||Hora||cConsecutivo),' ','0');
 ...
 'DESCRIPCION: Genera consecutivo para generar Folio_suc de Homologación de Servicios',
 'FECHA: 20150504',
 'Folio: 0000',
 'BD: bdisac';

CREATE PROCEDURE  "informix".sp_validadvcaminemos( pNumReferencia CHAR(10))
RETURNING CHAR(5) AS CodigoRetorno;

DEFINE      iCodRet                CHAR(5); 
DEFINE      iIndice                 INTEGER; 
DEFINE      iIndice2               INTEGER; 
DEFINE      iBandMultPorDos  INTEGER; 
DEFINE      iSuma                   INTEGER; 
DEFINE      iAux                     INTEGER; 
...
BEGIN
	...
	--SET DEBUG FILE TO '/respaldosbd/mario/sp_validar_digito_verificador.out';
	--TRACE ON;		
	SET ISOLATION TO DIRTY READ;
	 
	IF LENGTH(TRIM(pNumReferencia)) = 10 THEN 	
	
		LET iDigVerCapturado = SUBSTR(pNumReferencia,10,1)::INTEGER;			
		FOR iIndice = 1 TO 9
			IF iBandMultPorDos = 0 THEN
				LET iAux =  SUBSTR(pNumReferencia,iIndice,1)::INTEGER * 2;
				LET iBandMultPorDos = 1;
			ELIF iBandMultPorDos = 1 THEN
				LET iAux =  SUBSTR(pNumReferencia,iIndice,1)::INTEGER * 1;
				LET iBandMultPorDos = 0;
			END IF;			
		    IF LENGTH(iAux::CHAR(2))>1 THEN
				FOR iIndice2 =1 TO  LENGTH(iAux::CHAR(2))
					LET iAux2 = iAux2 + SUBSTR(iAux::CHAR(2),iIndice2,1)::INTEGER;
				END FOR;
				LET iAux = iAux2;
		    END IF;			
...
END PROCEDURE
DOCUMENT
' DESCRIPCION:	Se crea SP para validar digito verificador  de el numero de referencia de el pago de servicios caminemos ',  
' MODIFICO : Mario Gallardo Cardenas',			
' FECHA : 2013/05/28	',
'BD:  bdisac ';

CREATE PROCEDURE "informix".sp_valida_dv_sukarne(pReferencia CHAR(8))
...
DEFINE iSuma        INTEGER;
DEFINE iResiduo     INTEGER;
DEFINE cValorDV		CHAR(2);
DEFINE i			INTEGER;
DEFINE cNum			CHAR(6);
DEFINE iPeso		INTEGER;		 				
DEFINE cDVCal		CHAR(2);
...
--SET DEBUG FILE TO '/informix/tmp/sp_valida_dv_sukarne.out';
--TRACE ON;	
...
-----Se verifica que la longitud de referencia es de 8 caracteres------
	IF LENGTH(TRIM(pReferencia))= 8.
	THEN
	
			--SE asigna el valor de DV a cValorDV---	
			LET cValorDV = SUBSTRING(pReferencia FROM 7 FOR 2);
	 
			FOR i = 1 TO 6 --Ciclo para sacar registro por registro y multiplicarlo
			 
				LET cNum = SUBSTRING(pReferencia  FROM i FOR 1); ---SE asigna el valor de pReferencia a cNum hasta la posicion 6
				
				IF ASCII(cNum) >= 48 AND ASCII(cNum) <= 57 THEN --- Se valida que la referencia solo contenga numeros
					...
					LET iSuma = iSuma + (cNum::INTEGER * iPeso); 
				ELSE
			...
			LET iResiduo = MOD(iSuma,97);
			
			LET iValor = 99 - iResiduo;
			
			IF LENGTH(TRIM(iValor::CHAR(2))) = 1 THEN 
				LET cDVCal = '0'|| iValor :: CHAR(2);
			ELSE
			   LET cDVCal = iValor::CHAR(2);
			END IF;
				
			IF 	cDVCal = cValorDV THEN
				LET cCodRet = '00000';				
			END IF;	
	ELSE 
...
DOCUMENT
'DESCRIPCION: Se crea SP para verificar que el DV de pagos SuKarne sea valido',  
'AUTOR: Christian Echavarria',			
'FECHA: 23/Jul/2013',
'BD: bdisac';

CREATE PROCEDURE "informix".sp_valida_dv_solfi (pNumReferencia CHAR(9))
RETURNING CHAR(5) AS CodigoRetorno;
...
BEGIN
	...
	--	SET DEBUG FILE TO '/informix/leo/sp_valida_dv_solfi.out';
	--	TRACE ON;	
	
	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	 
	IF LENGTH(TRIM(pNumReferencia)) = 9 THEN 	
	
		LET iDigVerCapturado = SUBSTR(pNumReferencia,9,1)::INTEGER;			
		FOR iIndice = 1 TO 8
			IF iBandMultPorDos = 0 THEN
				LET iAux =  SUBSTR(pNumReferencia,iIndice,1)::INTEGER * 1;
				LET iBandMultPorDos = 1;
...
--				FOR iIndice2 =1 TO  LENGTH(iAux::CHAR(2))
--					LET iAux2 = iAux2 + SUBSTR(iAux::CHAR(2),iIndice2,1)::INTEGER;
					LET iAux2 = SUBSTR(iAux::CHAR(2),1,1)::INTEGER;
                    LET iAux3 = SUBSTR(iAux::CHAR(2),2,1)::INTEGER;
--				END FOR;

				LET iAux = iAux2 + iAux3;
...
'DESCRIPCIÓN: SP valida el digito verificador de la referencia SOLFI.',
'FOLIO:1426',
'FECHA:25/04/2014',
'VERSIÓN: 20140425.1622',
'BASE DE DATOS: bdisac';

CREATE PROCEDURE  "informix".sp_carnival_validadv(pNumReferencia CHAR(8))
RETURNING CHAR(5) AS CodigoRetorno;
...
BEGIN
	...
	--SET DEBUG FILE TO '/respaldosbd/josue/sp_carnival_validadv.out';
	--TRACE ON;	
	...
	ELSE
		IF LENGTH(TRIM(pNumReferencia)) <> 8 THEN
			LET iCodRet = '00002';			
		ELSE
			LET pNumReferencia = SUBSTR(pNumReferencia,3,6);			
			LET iDig_ver_cap = SUBSTR(pNumReferencia,6,1);
			LET pNumReferencia = SUBSTR(pNumReferencia,1,5);
			WHILE i <> 0
				LET iAux = SUBSTR(pNumReferencia,i,1);
				LET iAux2 = iMulti * iAux;
				LET cCadena = TRIM(cCadena)||iAux2;
					IF iMulti = 2 THEN LET iMulti = 1; ELSE
					IF iMulti = 1 THEN LET iMulti = 2; END IF; END IF;
				LET i = i - 1;
				LET iAux = 0;	
				LET iAux2 = 0;			
			END WHILE;
...
'Autor: 94912599 ',
'Fecha: 24/06/2014',
'Descripción: Se crea un procedimiento en central para validar el digito verificador para CLUB CARNIVAL',
'Sustento: RQM 10 489 PgsRef_CARNIVAL.doc',
'Solicita: Jaime Gonzalez',
'BD: bdisac';

CREATE PROCEDURE "informix".sp_valid_dv_stanhome (pNumReferencia CHAR(9))
RETURNING CHAR(5) AS CodigoRetorno;
...
BEGIN
	...
	--SET DEBUG FILE TO '/respaldosbd/resbdrigoberto/sp_valid_dv_stanhome.out';
	--TRACE ON;
	
	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	 
	IF LENGTH(pNumReferencia) = 9 THEN 	
		-- Se extrae el digito verificador de la referencia numerica
		LET iDigVerCapturado = SUBSTR(pNumReferencia,9,1)::INTEGER;		
		FOR iIndice = 1 TO 8
			IF iBandMultPorDos = 0 THEN
				--Multiplicar por 1 y almacenar el resultado
				LET iAux =  SUBSTR(pNumReferencia,iIndice,1)::INTEGER * 1;
				LET iBandMultPorDos = 1;
			ELSE
				--Multiplicar por 2 y almacenar el resultado
				LET iAux =  SUBSTR(pNumReferencia,iIndice,1)::INTEGER * 2;
				LET iBandMultPorDos = 0;
			END IF;	
			-- Si el resultado tiene una longitud de 2 digitos se deberan sumar	
		    IF LENGTH(iAux::CHAR(2)) > 1 THEN
				--extrayendo primer digito
				LET iAux2 = SUBSTR(iAux::CHAR(2),1,1)::INTEGER;
				--extrayendo segundo digito
				LET iAux3 = SUBSTR(iAux::CHAR(2),2,1)::INTEGER;
				--extrayendo sumatoria de ambos digitos
				LET iAux = iAux2 + iAux3;
			END IF;
			-- Almacenar sumatoria de la referencia
			LET iSuma = iSuma + iAux;
		END FOR;
        ...
        -- Obtener digito verificador calculado
		IF iResiduo = 0 THEN
			--Si el residuo es igual a 0, el digito verificador es 0
			LET iValorDigito = 0;
		ELSE
			-- Es el equivalente a restar la decena proxima y la sumatoria
			LET iValorDigito = 10 - iResiduo;		
		END IF;
		
		IF iValorDigito = iDigVerCapturado THEN
			--El digito verificador calculado y el digito verificador capturado es el mismo
			LET iCodRet = '00000';
		ELSE
			--Ambos digitos son diferentes
			LET iCodRet = '00109';
		END IF;
	ELSE
		-- La referencia no es de 9 posiciones
		LET iCodRet = '00002';
	END IF;	
	RETURN iCodRet;
END;
END PROCEDURE
DOCUMENT
'AUTOR: Rigoberto Gonzalez Ll',
'DESCRIPCIÓN: SP valida el digito verificador de la referencia StanHome',
'FOLIO:1455',
'FECHA:08/08/2014',
'VERSIÓN: 20140808.1322',
'BASE DE DATOS: bdisac';
