CREATE PROCEDURE "informix".sps_validapass_bei(
	pEmpresa CHAR(3),
    pIdUsuario CHAR(20)
)
   RETURNING CHAR(5),CHAR(50),CHAR(5), CHAR(26), CHAR(13), CHAR(13), DATE, DATE ;
	...
	BEGIN
		ON EXCEPTION SET sql_err
			IF sql_err <> 0 THEN
				...
				RETURN cCod_ret, cUsuario, cPass,  cNombre, cTelefono1, cTelefono2,  dFecha_constitucion, dFecha_actual;
			END IF
		END EXCEPTION;
        ...
        SELECT usuario.usuario_bei, usuario.pass1 , usuario.pass2, usuario.pass3, usuario.id_usuario,usuario.num_cliente
        INTO cUsuario,  cPass1, cPass2, cPass3, cIdusuario,eNumCliente
        ...
        IF(cUsuario IS NULL) THEN
            ...
            RETURN cCod_ret, NVL(cUsuario,''),NVL(cPass,''), cNombre, NVL(cTelefono1,''), NVL(cTelefono2,''),  dFecha_constitucion, dFecha_actual;
        END IF;

 		IF(cPass1 IS NULL ) THEN
        	LET cPass = "00001";
        END IF;
