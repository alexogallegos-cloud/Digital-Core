...
--SET DEBUG FILE TO '/informix/ENP/spHuellas/out/sp_ctehuella.out';
--TRACE ON;
...
IF pfuncion="A" THEN
   ...
   /*SELECT 1 INTO vexiste FROM si_cte_huella
    WHERE numcte = pnumcte; CIB20220428: se comentÃÂ³ el select debido a que retornaba dos datos*/
	
	SELECT LIMIT 1 1 INTO vexiste FROM si_cte_huella
    WHERE numcte = pnumcte; -- CIB20220428: se agregÃÂ³ LIMIT 1 esto para limitar el retorno de datos a solo 1  */

   IF vexiste = "1" THEN
      select max(secuencia) + 1 INTO vsigsec
      from   si_cte_huella
      where  numcte = pnumcte;
      /*RETURN vcodret,vsigsec; CIB20220428: se comentÃÂ³ el return debido a que terminaba el proceso sin agregar los datos en la tabla*/
   ELSE
      LET vsigsec = 1;
   END IF;
   BEGIN
      ...
      VALUES
         (pnumcte,vsigsec,"A",pmapad,pmapai,pejecutivo,psucursal,pfecha_alta::DATE,CURRENT);
   END;
   RETURN vcodret,vsigsec;
ELIF pfuncion = "C" then
     BEGIN
        UPDATE si_cte_huella SET estado = "I",usuario_camb = pautoriza,
               fecha_camb = pfecha_alta::DATE,
...
"VER   : 1.1",
"-----------------------------------------------------",
"Autor: 90231110 - Rolando JosuÃÂ© UrÃÂ­as GarcÃÂ­a",
"Fecha: 28/04/2022 - CIB20220428",
"ModificaciÃÂ³n: Se modificÃÂ³ el SELECT 1 INTO vexiste FROM si_cte_huella WHERE numcte = pnumcte ya que cuando se ejecutaba retornaba el error 284",
"..............debido a que se retornaban 2 datos y en la validaciÃÂ³n de IF vexiste = '1' THEN select max(secuencia) + 1 INTO vsigsec from   si_cte_huella",
"..............where numcte = pnumcte RETURN vcodret,vsigsec ELSE LET vsigsec = 1; END IF; a pesar que ya estaba retornando bien, el return terminaba la ejecuciÃÂ³n",
"..............sin haber agregado los datos a la tabla por lo que se comentÃÂ³ el RETURN vcodret,vsigsec",
"Sustento: Se definio por correo electronico el dÃÂ­a miercoles 27 de abril por Jaime Gonzales Prado",
"Solicita: Jaime Gonzales Prado",
"Folio: 1997",
"Proyecto: INC-SPCTEHUELLA284YNOINSERTA",
"BD: bdinteg";

CREATE PROCEDURE "informix".sp_generahuellalinea_outbound(
												pNumCte CHAR(20),
												pIP CHAR(15),
												pTipoMov CHAR(2),
												pEmpleado CHAR(8),
...
/*
SCRIPT DE PROCEDIMIENTO ALMACENADO "sp_generahuellalinea"
...
---SET DEBUG FILE TO "/informix/ENP/spHuellas/out/sp_generahuellalinea.out";
--TRACE ON;
...
-- INICIA LA VALIDACION SOLO EN SUCURSALES PILOTO SE EJECUTA EL SIGUIENTE BLOQUE DE CÃÂDIGO, SE CONSULTA LA INF. EN LA TABLA si_piloto_suc
	 IF EXISTS(SELECT sucursal FROM bdinteg:"informix".si_piloto_suc WHERE tipo_piloto = 1  AND sucursal = cSucursal ) THEN
		
		-- SE INSERTA TABLA si_huella_linea_dec cÃÂ³digo nuevo proyecto 10 huellas
		SELECT count(numcte) 
		INTO iHuellas_cap 
		FROM bdinteg:"informix".si_cte_huella_dec_actual 
		WHERE numcte = pNumCte
		AND secuencia = cSecuencia AND id_excepcion = 0;
		...
		IF (iHuellas_cap > 0)THEN
			...
			VALUES(pNumCte, cSecuencia, dFechaCons, cStatusCons, cSexo, cSucursal, dFecha_alta_prueba, pIP, pTipoMov,
						pEmpleado, pSensor, cStatuHuella, cRefCte, cTipoCte, pVerificacion, dFechaUltCon, iHuellas_cap,CURRENT);
						
		END IF; 
		
		-- *=== cÃÂ³digo nuevo proyecto 10 huellas fin
	 END IF; -- *=== FIN DE LA VALIDACION SOLO SUCURSALES QUE ESTAN EN LA TABLA si_piloto_suc
	ELSE
		LET cCodRet = '00001';
...
'BD: bdinteg',
'----------------------------------------------------------------------------',
'Para RQI 63 730 Comparacion en linea 10 huellas, se agrega validaciÃÂ³n para guardar en la tabla si_huella_linea_dec, si_huella_linea_dec_hist, si_huella_linea_dec_result_hist, dependiedo si la sucursal es piloto y esta registrada en la tabla si_piloto_suc',
'Modifico: Gabriel Romero Cuauhitzo',
'Fecha: 08/04/2022',
'BD: bdinteg';

CREATE PROCEDURE "informix".sp_numcte_pba(pId CHAR(12), pIdDocto CHAR(1))
