		...
		IF TRIM(pOpcion) ='1' THEN
			...
			ELSE
				...
				IF TRIM(cCodret)="000000" THEN
					IF  (SELECT count(*) FROM bdinteg:"informix".si_club_proteccion WHERE empresa= TRIM(pEmpresa) AND numcte= TRIM(pCliente) AND aceptada = '1') >0 THEN

						UPDATE bdinteg:"informix".si_club_proteccion SET numcte_coppel = TRIM(pCteCoppel), num_poliza = TRIM(pNumPoliza), tipo_plan = TRIM(pTipoPlan), suc_alta = TRIM(pSucAlta), ejecutivo = TRIM(pEjecutivo), tipo_pago = TRIM(pTipopago),
						num_tarjeta	= TRIM(pNumtarjeta),num_cta = TRIM(pNumcta), meses_pagar = pMesesPagar, monto_pagar = pMontoPagar, monto_mes = pMontoMes, monto_total = pMontoTotal, motivo_rechazo = '', tipo_mov = 'V', pago_mov = '0',
						foliooperacion = TRIM(pFolioOperacion), fecha_cambio = CURRENT, fecha_vencimiento = pFechaVencimiento, tipoplan_ant = pTipoPlanAnt
					...
					IF  TRIM(cCodret)="000000" AND  TRIM(NVL(pTipoPlan,''))<>'' THEN
						...
						ELSE
							...
							SET meses_pagar=pMesesPagar,monto_pagar=pMontoPagar,monto_mes=pMontoMes,
								...
								tipo_plan=TRIM(pTipoPlan),tipoplan_ant=pTipoPlanAnt, num_poliza=TRIM(pNumPolizaNueva)
							WHERE empresa= TRIM(pEmpresa)
							AND numcte= TRIM(pCliente)
							AND numcte_coppel=TRIM(pCteCoppel)
							AND num_poliza=TRIM(pNumPoliza)
						...
						END IF
					ELIF TRIM(cCodret)="000000"  THEN
							UPDATE bdinteg:"informix".si_club_proteccion
							SET num_poliza=TRIM(pNumPolizaNueva)
							WHERE empresa= TRIM(pEmpresa)
							AND numcte= TRIM(pCliente)
							AND numcte_coppel=TRIM(pCteCoppel)
							AND num_poliza=TRIM(pNumPoliza)
