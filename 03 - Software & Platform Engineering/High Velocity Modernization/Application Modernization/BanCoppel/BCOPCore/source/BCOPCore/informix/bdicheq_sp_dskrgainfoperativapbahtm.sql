CREATE PROCEDURE "informix".sp_dskrgainfoperativapbahtm( pempresa CHAR(3) )
RETURNING CHAR(5); 
    ...
    LET vfecha_ant         = '';
    LET vpri_hab_mes       = '';
    LET vproceso           = 'dskrgainfoperativa';
    LET vsistema           = '01';
    LET vexiste            = 0;
    LET vexistefin         = 0;
    LET vusuario           = user;
    ...
    ON EXCEPTION SET sql_err, isam_err, desc_err
        SET DEBUG FILE TO "/resplogifx/conciliachq/sp_dskrgainfoperativa.err";
        TRACE ON;
        IF sql_err <> 0 THEN
    ...
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/sp_dskrgainfoperativa.out";
    --- TRACE ON;
    ...
    if vexiste = 0 then
        ...
        LET vsql = 'echo " INSERT INTO bdinteg:sx_contproc VALUES '||
                   ...
                   '(SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas), NULL, NULL);" > /tmp/horasinfoper.sql';
        SYSTEM vsql;
        LET vsql = '';
    ...
    SELECT b.fecha_hoy, a.cuenta, a.status_cta, a.sucursal, a.sdo_dia_ant, a.imp_chq_sbg
      FROM sc_maechq a,
           sc_fechas b
    ...
    SELECT fech_alt, cuenta, sucursal, fech_hor, monto_tot
    ...
    INSERT INTO tmp_mov
    select vaniomes, mov.*, tra.descripcion,
    ...
    LET vsql = 'echo "SET ISOLATION TO DIRTY READ; '||
               ...
               'select a.*, NVL(b.archivo, '' '') archivo_vnd, NVL(c.archivo, '' '') archivo_atmd '||
               'from tmp_mov a '||
               'LEFT OUTER JOIN bditarjeta:td_conposvnd b ON(a.folio_suc = b.folio_mov) '||
			   'LEFT OUTER JOIN bditarjeta:td_conatmd c ON(a.folio_suc = c.folio_mov);" > /resplogifx/conciliachq/obt_movs_2402.sql';
    SYSTEM vsql;
    LET vsql = '';
    ...
    SELECT a.fech_alt, a.aniomes, a.num_serial, a.folio_suc, a.sucursal, a.usuario, a.fech_alt, a.fech_val, a.fech_hor,
           ...
           NVL(b.archivo, ' ') archivo_vnd, 
           NVL(c.archivo, ' ') archivo_atmd
      FROM tmp_mov a
      LEFT OUTER JOIN bditarjeta:td_conposvnd b ON(a.folio_suc = b.folio_mov)
      LEFT OUTER JOIN bditarjeta:td_conatmd c ON(a.folio_suc = c.folio_mov);
      
    -- // ####################################################################   REPORTES DIARIOS   ####################################################################
    
     -- // EJECUCIÓN DE SP QUE GENERA LOS REPORTES DIARIOS DE PORTABILIDAD DE NOMINA  
    ...
    IF vpri_hab_mes = vfecha_hoy THEN    
        ...
        SELECT b.fecha_ant, a.numcte, a.nombre, a.cuenta, a.saldo, a.prov, a.sdoprov
	   ...
	   UPDATE STATISTICS MEDIUM FOR TABLE ctaspag2;
        ...
        SELECT b.fecha_ant, a.numcte, a.nombre, a.fecha_alta, a.fecha_venc, a.cuenta, a.saldo, a.prov, a.sdoprov
        ...
        update statistics medium for table pagares_10335;
		  ...
		  -----este index esta mal
        select {+INDEX(bdicred:sd_maecredcrd idx_maecrex)} numcte, count(*) cuantos, "PRESTAMO PERSONAL" producto
        ...
        select {+INDEX(bdicred:sd_maecred idx_maecreda)} numcte, count(*) cuantos, "TARJETA DE CREDITO" producto
	...
	IF LPAD(DAY(vfecha_hoy),2,'0') = '02' THEN
		...
		-- // REPORTE IVR MENSUAL BANCOPPEL CADA DIA 2
        LET vaniomes = TO_CHAR(vfecha_hoy, '%m%d%y');	
        
        LET vsql = '';
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; '||
                   'UNLOAD TO /resplogifx/conciliachq/ivr_bitacora_'||vaniomes||'.txt '||
                   'select substr(fecha_oper,9,2)||''-''||substr(fecha_oper,6,2)||''-''||substr(fecha_oper,1,4)||'' ''||substr(fecha_oper,12,2)||'':''||substr(fecha_oper,15,2)||'':''||substr(fecha_oper,18,2), '||
				   'secuencia, operacion, num_tarjeta, numcte, telefono '|| 
				   'from bdinteg:si_bitacora_ivr '||
                   'where fecha_oper::date >= '''||vpri_mes_ant||''' '||
                   'and fecha_oper::date <= '''||vult_mes_ant||''' '||                               
                   'order by fecha_oper ASC;" > /resplogifx/conciliachq/rptivrmensual.sql';
        SYSTEM vsql; 
        LET vsql = '';
        
        LET vstmt = '';
		LET vstmt = "/ifxsif01/bin/dbaccess bdicheq /resplogifx/conciliachq/rptivrmensual.sql"; 
        SYSTEM vstmt;
        LET vstmt = '';
		LET vaniomes = '';
    ...
    IF LPAD(DAY(vfecha_hoy),2,'0') = '04' THEN
        ...
        LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO /resplogifx/conciliachq/intsisrs_cheques_'||vaniomes||'.txt '||
                   ...
                   'AND mae.fechaini is not null '||
                   'AND mae.fechafin BETWEEN '''||vpri_mes_ant||''' AND '''||vult_mes_ant||''' '||
    ...
    LET vsql = 'echo "SET ISOLATION TO DIRTY READ; '||
               ...
               'SELECT {+INDEX(sc_maehis maehis1)} cuenta, acum_sdo_pos, dia_sdo_pos, ROUND(acum_sdo_pos/dia_sdo_pos, 2), tasabruta, totintpag, totisrcobrado, fechafin '||
               'FROM sc_maehis '||
               'WHERE aniomes is not null '||
               'AND cuenta >= ''10000000000'' '||
               'AND fechaini < '''||vfecha_ant||''' '||
               'AND fechafin = '''||vfecha_ant||''' '||
