CREATE PROCEDURE "informix".sp_consreportessolicitudessupmc(pUsuario CHAR(8), pIdFuncion CHAR(10), pRegistros INTEGER, pRecuperacion INTEGER)
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consreportessolicitudessupmc.out';
		--TRACE ON;
...
'FECHA: 04/04/2019',
'MODULO: CRÉDITO',
'FUNCIONALIDAD: SOLICITUDES DE CRÉDITO EN SUPERVISIÓN MESA DE CONTROL',
'DESCRIPCION: SPL encargado de consultar el detalle de los reportes generados.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consreportessolicitudessupmc_totales(pUsuario CHAR(8), pIdFuncion CHAR(10))
	RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consreportessolicitudessupmc_totales.out';
		--TRACE ON;
...
CREATE PROCEDURE "informix".sp_rep_prod_genarchivo(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaInicial DATE, pFechaFinal DATE, pTipo CHAR(1), pStatus CHAR(1), pArchivo CHAR(50), pRutaDescarga CHAR(100))
    RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		FOREACH
			 EXECUTE PROCEDURE bdicred:"informix".sp_rep_prod_upgrade2(cEmpresa, pFechaInicial, pFechaFinal, pTipo, pStatus, pArchivo, iReg, iRec) 
		...
		SELECT COUNT(*)
		INTO iNumRegistros
		FROM bdicnweb:"informix".sw_cred_cambioproducto WHERE us_insert = pUsuario; 		
		...
		LET cCmd1 ="";
        LET cCmd1 ="SELECT 'num_credito','num_tarjeta','tipo_tarjeta','nombre_cliente','fecha_carga','resultado','marcaje','solicitud_plastico','motivo_error' FROM systables WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT numero_credito, num_tarjeta, tipo_tarjeta, nombre_embozado, LPAD(DAY(fecha),2,0)||'/'||LPAD(MONTH(fecha),2,0)||'/'||YEAR(fecha),resultado, marcaje,sol_plastico, UPPER(mensaje_error) ";
		...
		LET cNombreArchivo = 'REPORTE_CAMBIO_PROD_TDC_'||TRIM(cFechaHoraArchivo)||'.txt';
...
CREATE PROCEDURE "informix".sp_cp_validaconsecutivonomarchivo(pUsuario CHAR(8), pIdFuncion CHAR(10), pFecha CHAR(8), pConsecutivo CHAR(5), pNombreArchivo CHAR(35))
	RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		IF pNombreArchivo = 'CAMBIOPRODTDC' THEN
			
			LET cNomArcValido = 'CAMBIOPRODTDC_'||TRIM(pFecha)||'_';
			...
			--AAME 20190624 Se quita el casteo del query para consultar por Fecha a peticion de BD
			SELECT fecha_insert INTO dtFecha
			FROM bdicred:"informix".sd_credito_upgrade WHERE nombre_archivo = cNomArchivo;
			
			LET dtFecha = NVL(dtFecha,CURRENT);
			LET dFecha = NVL(dtFecha::DATE,CURRENT);
						
			DELETE FROM bdicnweb:"informix".sw_cp_nomarchivotdc
			WHERE fecha = dFecha
			AND nombre_archivo NOT IN (SELECT DISTINCT(nombre_archivo) FROM bdicred:"informix".sd_rep_detallearchivotdc WHERE fecha_insert = dFecha)
			AND nombre_archivo NOT IN (SELECT DISTINCT(nombre_archivo) FROM bdicred:"informix".sd_credito_upgrade WHERE fecha_insert = dtFecha); 
		...
		ELIF pNombreArchivo = 'CAMBIOPRODTDCREP' THEN
			
			LET cNomArcValido = 'CAMBIOPRODTDCREP_'||TRIM(pFecha)||'_';
			LET cNomArchivo = 'CAMBIOPRODTDCREP_'||TRIM(pFecha)||'_'||TRIM(pConsecutivo)||'.TXT';
			LET dFecha = SUBSTRING(TRIM(pFecha) FROM 5 FOR 2)||'/'||SUBSTRING(TRIM(pFecha) FROM 7 FOR 2) ||'/'||SUBSTRING(TRIM(pFecha) FROM 1 FOR 4); 
			...
			WHERE fecha = dFecha
			--AND nombre_archivo = TRIM(cNomArchivo);
			AND SUBSTR(nombre_archivo,1, 26) = TRIM(cNomArcValido);
...
CREATE PROCEDURE "informix".sp_consdetallesolsupervision_totales(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdConsulta CHAR(1), pNumCliente CHAR(20), pNumSolicitud CHAR(20), 
pFechaInicio DATE, pFechaFin DATE, pStatus CHAR(2), pProducto CHAR(4))
		RETURNING CHAR(5) AS codret,
			INTEGER AS num_registros;		
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consdetallesolsupervision_totales.out';
		--TRACE ON;
		...
		IF pIdConsulta = '1' THEN
			...
			EXECUTE PROCEDURE bdisolic:"informix".sp_busca_sol_supervision_totales('001', pUsuario, pNumSolicitud, pNumCliente, pFechaInicio, pFechaFin, pStatus, pProducto)
		...
		ELIF pIdConsulta = '2' THEN
			...
			EXECUTE PROCEDURE bdicred:"informix".sp_consulta_supervision_mc_totales('001', pNumSolicitud, pNumCliente, pFechaInicio, pFechaFin, pStatus, pProducto)
			INTO cCodRetSp, iNumRegistros;
...
'DESCRIPCION: SPL que se encarga de consultar el número total de registros del detalle de la solicitud supervisión.',
'AUTOR: L. Montserrat León Amador',
'FECHA: 05/07/2019',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consultahistoincrementosaumlincred(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumCliente CHAR(20), pNumSolicitud CHAR(20), pStatus CHAR(2))
	RETURNING CHAR(5) AS codret,
			CHAR(20) AS num_solicitud,
			CHAR(2) AS status,
			CHAR(10) AS origen,
			CHAR(10) AS fecha_status,
			MONEY(18, 2) AS linea_credito_actual,
			MONEY(18, 2) AS linea_credito_sugerida;
	...
	DEFINE cFechaStatus CHAR(10);
	DEFINE mLineaCreditoActual MONEY(18,2);
	DEFINE mLineaCreditoSugerida MONEY(18,2);
	...
	LET cOrigen = '';
	LET cFechaStatus = '';
	LET mLineaCreditoActual = 0;
	LET mLineaCreditoSugerida = 0;
	LET iNoRegistros = 0;
	LET cEmpresa = '001';
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cNumSolicitud, cStatus, cOrigen, cFechaStatus, mLineaCreditoActual, mLineaCreditoSugerida;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultahistoincrementosaumlincred.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pNumSolicitud = '' OR pNumCliente = '' OR pStatus = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cNumSolicitud, cStatus, cOrigen, cFechaStatus, mLineaCreditoActual, mLineaCreditoSugerida;
		END IF;
		
		-- VALIDACIÃ?N DE ACCESO A LA FUNCIONALIDAD
		EXECUTE PROCEDURE bdinteg:"informix".sp_cnsif_permisosejecutivo(pUsuario, pIdFuncion, pNumCliente, '06', '2') INTO cCodRet;
		IF cCodRet <> '00000' THEN
			RETURN cCodRet, cNumSolicitud, cStatus, cOrigen, cFechaStatus, mLineaCreditoActual, mLineaCreditoSugerida;
		END IF;
		...
		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;
		FOREACH EXECUTE PROCEDURE bdicred:"informix".sp_cac_histincremento(cEmpresa, pNumCliente, pNumSolicitud, pStatus)
				INTO cCodRetSp, cMensajeRetorno, cNumSolicitud, cStatus, cOrigen, cFechaStatus, mLineaCreditoActual, mLineaCreditoSugerida
				RETURN cCodRet, cNumSolicitud, cStatus, cOrigen, cFechaStatus, mLineaCreditoActual, mLineaCreditoSugerida WITH RESUME;
...
'FECHA: 24/03/2014',
'DESCRIPCION: Consulta el historial de los incrementos aplicados de una lÃ­nea de credito',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_genreportesmesacontrol(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdReporte CHAR(20),
pProducto CHAR(4), pDescProducto CHAR(40), pFechaInicio DATE, pFechaFin DATE, pStatusInicial CHAR(2), pStatusFinal CHAR(2), pArea CHAR(2),
pRutaDescarga CHAR(100), pIdPlantilla CHAR(10), pTituloPlantilla CHAR(60))
	...
	--LET cRutaInformix = '/informix/bin/';--pruebas gpe
	LET cRutaGral = '';
	LET cNombreReporte = '';
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_genreportesmesacontrol.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pIdReporte = '' OR pFechaInicio IS NULL OR pFechaFin IS NULL OR 
		pRutaDescarga = '' OR pIdPlantilla = '' OR pTituloPlantilla = '' THEN	
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF;
		
		-- REPORTE CAMBIO DE ESTATUS
		IF TRIM(pIdReporte) = 'CAMBIOESTATUS' THEN
			IF pArea = '' THEN
				LET cCodRet = '00003';
				RETURN cCodRet;
			END IF;
		...
		--LET cNombreReporte = TRIM(pIdReporte)||'_'||pUsuario||'_'||TO_CHAR(CURRENT,'%d%m%Y')||'.txt';
		...
		-- REPORTE OPERACIÃN MC
		IF TRIM(pIdReporte) = 'OPERACIONMC' THEN
			
			-- SE LIMPIA TABLA PRINCIPAL
			DELETE FROM bdicnweb:"informix".sw_consultaoperacionesmc WHERE usuario_insert = pUsuario;
			
			FOREACH 
			
				EXECUTE PROCEDURE bdicred:"informix".sp_consultaoperacionesmc(pProducto, pFechaInicio, pFechaFin)
				INTO cCodRetSp, cMensaje, cFecha, cNombreProducto, iSolicitudesMc, iNoAnalizadasATiempo, 
				...
				LET iCodRetSp = cCodRetSp::INTEGER;
				IF iCodRetSp < 0 THEN
					RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bdicred:"informix".sp_consultaoperacionesmc';
				...
				ELSE
					...
					INSERT INTO bdicnweb:"informix".sw_consultaoperacionesmc(fecha, nombre_producto, solicitudes_mc, no_analizadas_atiempo, 
					analizadas_atiempo, revaluadas, no_revaluadas, sigue_proceso, porcentaje_sp,
					rechazadas, porcentaje_rt, canceladas, porcentaje_cn, mixta, unica, usuario_insert, fecha_insert) 
					VALUES(cFecha, cNombreProducto, iSolicitudesMc, iNoAnalizadasATiempo, 
					iAnalizadasATiempo, iRevaluadas, iNoRevaluadas, iSigueProceso, dPorcentajeSp, 
					iRechazadas, dPorcentajeRt, iCanceladas, dPorcentajeCn, iMixta, iUnica, pUsuario, dFechaHoy);
			...
			LET cCmd1 ="";
			LET cCmd1 = "SELECT 'FECHA','SOLICITUD DE MC','NO ANALIZADAS EN TIEMPO','ANALIZADAS EN TIEMPO','REVALUADAS','NO REVALUADAS',";
			LET cCmd1 =""||TRIM(cCmd1)||"'SIGUE PROCESO','% SP','RECHAZADAS','% RT','CANCELADAS','% CN','MIXTAS','UNICAS' FROM systables WHERE tabid = 1";
			LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL SELECT * FROM (SELECT fecha, solicitudes_mc::CHAR(11), no_analizadas_atiempo::CHAR(11), analizadas_atiempo::CHAR(11), revaluadas::CHAR(11), no_revaluadas::CHAR(11),";
			LET cCmd1 =""||TRIM(cCmd1)||" sigue_proceso::CHAR(11), porcentaje_sp::CHAR(20), rechazadas::CHAR(11), porcentaje_rt::CHAR(20), canceladas::CHAR(11), porcentaje_cn::CHAR(20), mixta::CHAR(11), unica::CHAR(11)";
			LET cCmd1 =""||TRIM(cCmd1)||" FROM bdicnweb:""informix"".sw_consultaoperacionesmc";
			LET cCmd1 =""||TRIM(cCmd1)||" WHERE usuario_insert = '"|| pUsuario ||"'";	
			LET cCmd1 =" "||TRIM(cCmd1)||" ORDER BY id_registro ASC)";
			
		-- REPORTE CAMBIO DE ESTATUS
		ELIF TRIM(pIdReporte) = 'CAMBIOESTATUS' THEN 
			
			LET cCmd1 ="";
			LET cCmd1 = "SELECT 'SOLICITUD','CLIENTE','NOMBRE','SUCURSAL','FECHA SOLICITUD','ESTATUS INICIAL','MOTIVO RT/CM','FECHA CAMBIO','ESTATUS FINAL','MOTIVO RT/CM',";
			LET cCmd1 =""||TRIM(cCmd1)||"'JUSTIFICACIONES','ANALISTA','TIPO DE MOVIMIENTO' FROM systables WHERE tabid = 1";
			LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL SELECT * FROM (SELECT ''''||num_solicitud, ''''||num_cte, nom_cliente, sucursal,";
			LET cCmd1 =""||TRIM(cCmd1)||" NVL(TO_CHAR(fecha_sol, '%d/%m/%Y'), ''), sta_inicial, motivo, NVL(TO_CHAR(fecha_cambio, '%d/%m/%Y'), ''), sta_final, motivo2, REPLACE(UPPER(justificacion),',',';'), analista, tipo_movto";
			LET cCmd1 =""||TRIM(cCmd1)||" FROM bdicnweb:""informix"".sw_reportecambioestatusmc";
			LET cCmd1 =""||TRIM(cCmd1)||" WHERE usuario = '"|| pUsuario ||"'";	
			LET cCmd1 =" "||TRIM(cCmd1)||" ORDER BY id_serial ASC)";
			
		-- REPORTE POR ANALISTA MC
		ELIF TRIM(pIdReporte) = 'PORANALISTAMC' THEN
			
			-- SE LIMPIA TABLA PRINCIPAL
			DELETE FROM bdicnweb:"informix".sw_consultaporanalistamc WHERE usuario_insert = pUsuario;
			
			FOREACH
				
				EXECUTE PROCEDURE bdicred:"informix".sp_consultaporanalistamc(pFechaInicio, pFechaFin, pProducto)
				INTO cCodRetSp, cEjecutivo, cNombre, iNoAnalizadas, dPorcentajeAnalizadas, iNumReevaluadas, dPorcentajeRevaluadas, iNumRevaluadas, dPorcentajeNumRevaluadas, 
				iNumSiguenProceso, dPorcentajeSigueProc, iNumRechazadas, dPorcentajeRechazadas, iCanceladas, dPorcentajeCanceladas, iNumMixta, dPorcentajeMixta, iNumUnica, dPorcentajeUnica, cNombreProd
			
				LET iCodRetSp = cCodRetSp::INTEGER;
				IF iCodRetSp < 0 THEN
					RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bdicred:"informix".sp_consultaporanalistamc';
				ELSE 
					
					LET iRegistros = iRegistros + 1;
					INSERT INTO bdicnweb:"informix".sw_consultaporanalistamc(ejecutivo, nombre, no_analizadas, porc_analizadas, num_reevaluadas, porc_revaluadas, num_revaluadas, porc_num_revaluadas, 
					num_siguen_proceso, porc_sigue_proc, num_rechazadas, porc_rechazadas, canceladas, porc_canceladas, num_mixta, porc_mixta, num_unica, porc_unica, nombre_producto, usuario_insert, fecha_insert) 
					VALUES(cEjecutivo, cNombre, iNoAnalizadas, dPorcentajeAnalizadas, iNumReevaluadas, dPorcentajeRevaluadas, iNumRevaluadas, dPorcentajeNumRevaluadas, 
					iNumSiguenProceso, dPorcentajeSigueProc, iNumRechazadas, dPorcentajeRechazadas, iCanceladas, dPorcentajeCanceladas, iNumMixta, dPorcentajeMixta, iNumUnica, dPorcentajeUnica, cNombreProd, pUsuario, dFechaHoy);
			...
			LET cCmd1 ="";
			LET cCmd1 = "SELECT 'NO. EMPLEADO','NOMBRE','ANALIZADAS','% ANALIZADAS','REVALUADAS','% REVALUADAS','NO REVALUADAS','% NO REVALUADAS','SIGUE PROCESO','% SP',";
			LET cCmd1 =""||TRIM(cCmd1)||"'RECHAZADAS','% RT','CANCELADAS','% CN','MIXTAS','% MIXTAS','UNICAS','% UNICAS' FROM systables WHERE tabid = 1";
			LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL SELECT * FROM (SELECT ejecutivo, nombre, no_analizadas::CHAR(11), porc_analizadas::CHAR(16), num_reevaluadas::CHAR(11), porc_revaluadas::CHAR(16), num_revaluadas::CHAR(11), porc_num_revaluadas::CHAR(16),";
			LET cCmd1 =""||TRIM(cCmd1)||" num_siguen_proceso::CHAR(11), porc_sigue_proc::CHAR(16), num_rechazadas::CHAR(11), porc_rechazadas::CHAR(16), canceladas::CHAR(11), porc_canceladas::CHAR(16), num_mixta::CHAR(11), porc_mixta::CHAR(16), num_unica::CHAR(11), porc_unica::CHAR(16)";
			LET cCmd1 =""||TRIM(cCmd1)||" FROM bdicnweb:""informix"".sw_consultaporanalistamc";
			LET cCmd1 =""||TRIM(cCmd1)||" WHERE usuario_insert = '"|| pUsuario ||"'";	
			LET cCmd1 =" "||TRIM(cCmd1)||" ORDER BY id_registro ASC)";
			
		-- REPORTE DETALLE DE SOLICITUDES
		ELIF TRIM(pIdReporte) = 'DETALLESOLICITUDES' THEN
			
			LET cCmd1 ="";
			LET cCmd1 = "SELECT 'SOLICITUD','SUCURSAL','NOMBRE CLIENTE','FECHA SOLICITUD','FECHA CAMBIO STATUS BC','REVALUADA','REFERENCIA COPPEL','EFICIENCIA COPPEL','MESES COPPEL',";
			LET cCmd1 =""||TRIM(cCmd1)||"'VENCIDO COPPEL','VENCIDO COPPEL (UDIS)','PUNTUALIDAD COPPEL','BC SCORE (SCORING 1)','SCORE PROPIETARIO (SCORING 2)','RESULTADO DE REVISION MC','MOTIVO DE RECHADO/CANCELACION','JUSTIFICACIONES','ANALISTA','TIPO DE MOVIMIENTO' FROM systables WHERE tabid = 1";
			LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL SELECT * FROM (SELECT ''''||num_solicitud, sucursal, nombre_cte, NVL(TO_CHAR(fecha_sol, '%d/%m/%Y'), ''), NVL(TO_CHAR(fecha_cambio, '%d/%m/%Y'), ''), revaluada, referencia_coppel, ceficiencia_coppel::CHAR(20), meses_coppel::CHAR(6),";
			LET cCmd1 =""||TRIM(cCmd1)||" vencido_coppel::CHAR(20), vencido_coppel_udis::CHAR(11), puntualidad, scoring1::CHAR(11), scoring2::CHAR(11), desc_status, causa_solic, REPLACE(UPPER(comentario),',',';'), analista, tipo_movto";
			LET cCmd1 =""||TRIM(cCmd1)||" FROM bdicnweb:""informix"".sw_reportesolicitudmc";
			LET cCmd1 =" "||TRIM(cCmd1)||" WHERE usuario = '"|| pUsuario ||"')";	
			--LET cCmd1 =""||TRIM(cCmd1)||" WHERE usuario = '"|| pUsuario ||"'";	
			--LET cCmd1 =" "||TRIM(cCmd1)||" ORDER BY id_registro ASC)";
		...
		BEGIN WORK;
			...
			FOREACH
				...
				DELETE FROM bdicnweb:"informix".sw_ctrlgenreportesmc WHERE nombre_reporte = cNombreReporteHist;
		...
		DELETE FROM bdicnweb:"informix".sw_ctrlgenreportesmc WHERE nombre_reporte = cNombreReporte;
		INSERT INTO bdicnweb:"informix".sw_ctrlgenreportesmc(nombre_reporte,fecha_reporte,hr_reporte,usuario_insert)
...
'AUTOR: L. Montserrat LeÃ³n Amador',
'FECHA: 05/07/2019',
'DESCRIPCION: Se modifica SPL para cambiar el formato de los reportes generados de txt a csv.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_cnt_catagrupa(pUsuario CHAR(8), pIdFuncion CHAR(10), pSt CHAR(2))
    RETURNING CHAR(5) AS codRet,
		CHAR(1) AS id_agrupa,
		CHAR(35) AS desc_agrupa;
	...
	DEFINE iCodRetSp INTEGER;
	DEFINE cEmpresa CHAR(3);
	DEFINE cIdAgrupa CHAR(1);
	DEFINE cDescAgrupa CHAR(35);
	DEFINE iNumRegistros INTEGER;
	...
	BEGIN
		ON EXCEPTION SET iSqlErr
			IF iSqlErr <> 0 THEN
				LET cCodRet = iSqlErr;
				RETURN cCodRet, cIdAgrupa, cDescAgrupa;
			END IF;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cnt_catagrupa.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pSt = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cIdAgrupa, cDescAgrupa;
		END IF;
		...
		IF pSt = 'OS' THEN
			FOREACH
				SELECT id_agrupa, desc_agrupa
				INTO cIdAgrupa, cDescAgrupa
				FROM "informix".sw_cnt_agrupacion
				ORDER BY id_agrupa ASC
				
				LET iNumRegistros = iNumRegistros + 1;
				RETURN cCodRet, cIdAgrupa, cDescAgrupa WITH RESUME;
			END FOREACH;
		ELSE
			FOREACH
				SELECT id_agrupa, desc_agrupa
...
'FECHA 02/05/2019',
'MODULO: CONTRALORÍA',
'FUNCIONALIDAD: MONITOR DE SOLICITUDES DE ESTUDIO',
'DESCRIPCION: Spl encargado de consultar el detalle del catálogo agrupar por.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_cnt_catconvenio(pUsuario CHAR(8), pIdFuncion CHAR(10))
    RETURNING CHAR(5) AS codRet,
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cnt_catconvenio.out';
		--TRACE ON;
		...
		FOREACH
			...
			ORDER BY numcategoria,numconvenio ASC
...
'DESCRIPCION: Spl encargado de consultar el detalle del catálogo convenio.',
'BD: bdicnweb';
