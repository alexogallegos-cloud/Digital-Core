CREATE PROCEDURE "informix".sp_obtieneclavetarlote(Lotet CHAR(11))
   RETURNING CHAR(5),  CHAR(6), CHAR(3);
      
   DEFINE cCodRet             CHAR(5);
   DEFINE iSqlErr             INTEGER;
   DEFINE cNoLote             CHAR(11);
...
BEGIN
                  ON EXCEPTION SET iSqlErr
                      IF iSqlErr <> 0 THEN
                         ...
                         RETURN cCodRet, cNoLote, cClavetp;
                      END IF;
                  END EXCEPTION;
           ...
           SELECT numerolote, clave_tipotarjeta
		   INTO cNoLote, cClavetp
		   FROM intercard:lote
		   WHERE numerolote = Lotet;     

           IF cNoLote IS NULL THEN
              LET  cCodRet = '00001';
           END IF;
...
'FECHA: 30/04/2018',
'BD: bdinteg',
'Objetivo: Se crea procedimiento para obtener clave tipo tarjeta de un lote.';

CREATE PROCEDURE "informix".sp_validatelefono_web(cEmpresa CHAR(3), cTelefonoCasa CHAR(10), cTelefonoCelular CHAR(10), cTelefonoOficina CHAR(10))
...
BEGIN
	...
	--FECHA: 2009-02-18
	--FUNCIONALIDAD: Verfifca si el telefono proporcionado en el alta de la direccion es un telefono validado por la COFETEL
	----------------------------------------
	...
	IF cTelefonoCasa <> "" THEN
		...
		IF cTelefonoNirCasa IN ('55', '33', '81','56') THEN
			...
			IF (SELECT COUNT(1) FROM "informix".si_cattelefono WHERE nir = cNirCasa AND serie = cSerieCasa AND numeracion_inicial <= cNumeracionCasa AND numeracion_final >= cNumeracionCasa) > 0 THEN
				LET cValCasa = '1';
				LET cCodRet = '00000';
			END IF;
		ELSE
		...
		IF cBanValidaTelefonos = '1' THEN
			IF (SELECT COUNT(1) FROM "informix".si_tels_invalidos WHERE telefono = cTelefonoCasa) > 0 THEN
				LET cValCasa = '0';
				LET cCodRet = '00000';
			END IF;
		END IF;
	...
	IF cTelefonoCelular <> "" THEN
		...
		IF cTelefonoNirCel IN ('55', '33', '81','56') THEN
			...
			IF (SELECT COUNT(1) FROM "informix".si_cattelefono WHERE nir = cNirCelular  AND serie = cSerieCelular AND numeracion_inicial <= cNumeracionCelular AND numeracion_final >= cNumeracionCelular AND tipored = 'MOVIL') > 0 THEN
				LET cValCelular = '1';
				LET cCodRet = '00000';
			END IF;
		ELSE
		...
		-- BCPL 17/06/2014
		IF cBanValidaTelefonos = '1' THEN
			IF (SELECT COUNT(1) FROM "informix".si_tels_invalidos WHERE telefono = cTelefonoCelular) > 0 THEN
				LET cValCelular = '0';
				LET cCodRet = '00000';
			END IF;
		END IF;
	...
	IF cTelefonoOficina <> "" THEN
		...
		IF cTelefonoNirOfi IN ('55', '33', '81','56') THEN
			...
			IF(SELECT COUNT(1) FROM "informix".si_cattelefono WHERE nir = cNirOficina  AND serie = cSerieOficina AND numeracion_inicial <= cNumeracionOficina AND numeracion_final >= cNumeracionOficina) > 0 THEN
				LET cValOficina = '1';
				LET cCodRet = '00000';
			END IF;
		ELSE
