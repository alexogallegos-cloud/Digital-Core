CREATE PROCEDURE "informix".sp_rep_segmentos_trim_mens_pba1(pEmpresa CHAR(3))
RETURNING CHAR(6);
...
--SET DEBUG FILE TO "/informix/sp_rep_segmentos_trim_mens.out";
--TRACE ON;
...
BEGIN
    ...
    END IF;
	--CREATE temp TABLE creditostransaccion(
...
--monto_disp DECIMAL(18,2)) with no log;
	
	create index segix1 on creditostransaccion (fecha);
    create index segix2 on creditostransaccion (num_credito);
...
IF 	MONTH(dFecha) = '03' or MONTH(dFecha) = '06' or MONTH(dFecha) = '09' or MONTH(dFecha) = '12' THEN
    ...
    CALL bdicred:"informix".sp_inserta_bitacora(pempresa, pproceso, pCod_ret, '30-Consulta principal trimestre', '02')
    Returning cCod_RetIB;
	...
	from bdicred:sd_indicador_cred_hist ind, bdicred:"informix".sd_maesdoscont sdo
  ...
  --and ind.fecha <= mdy('08','31','2013')
	...
	DELETE FROM creditostransaccion WHERE num_credito in (select num_credito from sd_maecredcont where fecha between dFechatrim and dFecha and status_cred = 'CV');
	...
	create index segix3 on segmento_trim (segmento);
	update statistics medium for table segmento_trim;
	
	CALL bdicred:"informix".sp_inserta_bitacora(pempresa, pproceso, pCod_ret, '30-Inserta en temporal trimestre', '02')
    Returning cCod_RetIB;
	...
	LET cnomarchivo =  'rep_segmentos_trimestral'||to_char( dFechaHoy,'%m%d%Y')||'.txt';
	--Encabezado
	let cSql='';
   ...
   -- and ind.fecha = mdy('02','28','2014')
	and day(ind.fecha) > 20
	group by 1,2; --,3,4,5;
	update statistics medium for table creditostransaccion;

    DELETE FROM creditostransaccion WHERE num_credito in (select num_credito from sd_maecred where campo_trab3 = 'BAJA');
	DELETE FROM creditostransaccion WHERE num_credito in (select num_credito from sd_maecredcont where fecha = dFecha and status_cred = 'CV');
...
END IF;
    ...
    call bdicred:"informix".sp_rep_segmentos_trim_mens20(pempresa) Returning cCod_RetIB;
