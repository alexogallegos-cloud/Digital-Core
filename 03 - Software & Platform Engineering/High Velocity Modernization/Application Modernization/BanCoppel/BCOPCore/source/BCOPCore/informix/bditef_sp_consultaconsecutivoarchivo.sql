CREATE PROCEDURE "informix".sp_consultaconsecutivoarchivo( pSistema CHAR(20), 
                                                           pConsecutivo CHAR(3) )
RETURNING CHAR(5), INTEGER, DATE;
    ...
    --- SET DEBUG FILE TO "/tmp/sp_ConsultaConsecutivoArchivo.out";
    --- TRACE ON;
    ...
    IF pSistema IS NULL OR pSistema = ""  THEN
        LET cCodRet = "00001";
        RETURN cCodRet, iConsecutivo, dFechaHoy;
    END IF;
    ...
    IF pConsecutivo = 0 THEN
        -- // Elimina los registros que son de dias anteriores sobre ese mismo sistema.
        DELETE FROM bditef:"informix".cce_consecutivo_proc 
         WHERE fecha_proceso < dFechaHoy 
           AND sistema = pSistema;

        -- // Consulta el maximo consecutivo que tiene el proceso.
        SELECT MAX(consecutivo)
          INTO iConsecutivo
          FROM bditef:"informix".cce_consecutivo_proc	
         WHERE fecha_proceso = dFechaHoy
           AND sistema = pSistema;

        IF iConsecutivo = '' OR iConsecutivo IS NULL THEN
            LET iConsecutivo = 1;
        ELSE
            LET iConsecutivo = iConsecutivo +1;
        END IF;

        -- // Registra una bitacora del porceso.
        INSERT INTO bditef:"informix".cce_consecutivo_proc(sistema, consecutivo, fecha_proceso)
        VALUES(pSistema, iConsecutivo, dFechaHoy);

        RETURN cCodRet, iConsecutivo, dFechaHoy;
    ELSE
        -- // Elimina el ultimo consecutivo si no se va generar reporte.
        DELETE FROM bditef:"informix".cce_consecutivo_proc 
         WHERE fecha_proceso = dFechaHoy 
           AND sistema = pSistema
...
'DESCRIPCION: Obtiene el consecutivo de un proceso ejecutado el mismo dia, eliminando los registros anteriores.',
'EJECUTADO O LLAMADO POR: itchedev.exe ',
'FECHA : JULIO 2011',
'VERSION: 20110719.1251',
'BD    : bditef';

CREATE PROCEDURE "informix".sp_consarchivos_tef
(
psConsArchDetalle CHAR(1),
...
RETURNING CHAR(5), CHAR(20), CHAR(15), CHAR(18), CHAR(8), CHAR(30), CHAR(7), CHAR(8), CHAR(18), CHAR(40), CHAR(364), CHAR(40), CHAR(10), CHAR(40), CHAR(364), CHAR(20), CHAR(40), CHAR(18), CHAR(2), CHAR(20), CHAR(40), CHAR(18), CHAR(40),
CHAR(40), CHAR(15), CHAR(7), CHAR(40), CHAR(30), CHAR(2), CHAR(8), CHAR(1), CHAR(364), CHAR(30), CHAR(1), CHAR(7), CHAR(3), CHAR(15), CHAR(2), CHAR(16), CHAR(8), CHAR(10), CHAR(2);

--***********************************************************************************************************
-- DESCRIPCION: Realiza consulta para obtener detalle de los archivos enviados para CCEN y recibidos de CCEN.
...
DEFINE vsTotRegistros CHAR(15);
DEFINE vsImporte CHAR(18);
DEFINE vsFecPresentacion CHAR(8);
DEFINE vsDescripcion CHAR(30);
DEFINE vsTipoReg CHAR(2);
DEFINE vsNumSec CHAR(7);
DEFINE vsCodOper CHAR(2);
DEFINE vsCveBanco CHAR(3);
DEFINE vsCodDivisa CHAR(2);
DEFINE vsSentido CHAR(1);
DEFINE vsServicio CHAR(1);
DEFINE vsNumBloque CHAR(7);
DEFINE vsNumOper CHAR(7);
DEFINE vsImpOper CHAR(18);
DEFINE vsUsoFutCccen CHAR(40);
DEFINE vsUsoFutBanco CHAR(364);
DEFINE vsUserInsert CHAR(8);
DEFINE vsFechaInsert CHAR(10);
DEFINE vsCveRechazobl CHAR(2);
DEFINE vsModalidad CHAR(1);
DEFINE vsFechaTrans CHAR(8);
DEFINE vsBancoPres CHAR(3);
DEFINE vsBancoRec CHAR(3);
DEFINE vsTipoOper CHAR(2);
DEFINE vsFechaAplica CHAR(8);
DEFINE vsTipoCtaOrd CHAR(2);
DEFINE vsNumCtaOrd CHAR(20);
DEFINE vsNombreOrd CHAR(40);
DEFINE vsRfcOrd CHAR(18);
DEFINE vsTipoCtaRec CHAR(2);
DEFINE vsNumCtaRec CHAR(20);
DEFINE vsNombreRec CHAR(40);
DEFINE vsRfcRec CHAR(18);
DEFINE vsRefServicio CHAR(40);
DEFINE vsNomTitServ CHAR(40);
DEFINE vsImpIva CHAR(15);
DEFINE vsRefNumerica CHAR(7);
DEFINE vsRefLeyenda CHAR(40);
DEFINE vsClaveRastreo CHAR(30);
DEFINE vsMotivoDev CHAR(2);
DEFINE vsFechaPresIni CHAR(8);
DEFINE vsSolConfirmacion CHAR(1);
DEFINE vsRefConfirmacion CHAR(30);
DEFINE vsTasaTiieProm CHAR(7);
DEFINE vsDiasRetraso CHAR(3);
DEFINE vsImpTotInt CHAR(15);
DEFINE vsCveEstatus CHAR(2);
DEFINE vsFolioSuc CHAR(16);
DEFINE vsTipoCta CHAR(2);
DEFINE vsUsoFutCce CHAR(1);
...
LET vsMotivoDev = "";
LET vsFechaPresIni = "";
LET vsSolConfirmacion = "";
LET vsRefConfirmacion = "";
LET vsTasaTiieProm = "";
LET vsDiasRetraso = "";
LET vsImpTotInt = "";
LET vsCveEstatus = "";
...
--SET DEBUG FILE TO "/dbexport/sp_consarchivos_tef.sql";
--TRACE ON;
...
ON EXCEPTION SET viSqlErr   --Cacha el error en caso de que exista y regresa un valor predeterminado
	IF viSqlErr <> 0 THEN
		RETURN viSqlErr, vsNomArchivo, vsTotRegistros, vsImporte, vsFecPresentacion, vsDescripcion, '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '';
	END IF;
END EXCEPTION;

IF(psConsArchDetalle == 1)THEN
	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	FOREACH
		SELECT archtef.nombre_arch, archtef.tot_registros, sumtef.imp_operaciones, archtef.fecha_presentacion, archstatustef.descripcion
		INTO vsNomArchivo, vsTotRegistros, vsImporte, vsFecPresentacion, vsDescripcion
		FROM bditef:"informix".tef_cce_archivos AS archtef, bditef:"informix".tef_status_archcce AS archstatustef, bditef:"informix".tef_cce_sumario AS sumtef
		WHERE archtef.fecha_presentacion BETWEEN psFechaInicio AND psFechaFin
		AND archstatustef.cve_status = archtef.cve_status
		AND archtef.nombre_arch = sumtef.nombre_arch
        order by archtef.fecha_presentacion, sumtef.imp_operaciones
		let vsImporte=  "$" || round(vsImporte / 100, 2);
        LET vsCodRet = '00000';
		RETURN vsCodRet, vsNomArchivo, vsTotRegistros, vsImporte, vsFecPresentacion, vsDescripcion, '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '' WITH RESUME;
	END FOREACH;
ELIF(psConsArchDetalle == 2)THEN
	IF(psEncDetSum == 1)THEN
		SET LOCK MODE TO WAIT 3;
		SET ISOLATION TO DIRTY READ;
		SELECT FIRST 1 nombre_arch, fecha_presentacion, tpo_registro, num_secuencia, cod_operacion, cve_banco, sentido, servicio, num_bloque, cod_divisa, cve_rechazo_bl, modalidad, uso_futuro_ccen, uso_futuro_banco, user_insert, fecha_insert
		INTO vsNomArchivo, vsFecPresentacion, vsTipoReg, vsNumSec, vsCodOper, vsCveBanco, vsSentido, vsServicio, vsNumBloque, vsCodDivisa, vsCveRechazobl, vsModalidad, vsUsoFutCccen, vsUsoFutBanco, vsUserInsert, vsFechaInsert
		FROM bditef:"informix".tef_cce_encabezado
		WHERE nombre_arch = psNomArchivo;
		LET vsCodRet = '00000';
		RETURN vsCodRet, vsNomArchivo, vsFecPresentacion, vsTipoReg, vsNumSec, vsCodOper, vsCveBanco, vsSentido, vsServicio, vsNumBloque, vsCodDivisa, vsCveRechazobl, vsModalidad, vsUsoFutCccen, vsUsoFutBanco, vsUserInsert, vsFechaInsert, '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '';
	ELIF(psEncDetSum == 2)THEN
		SET LOCK MODE TO WAIT 3;
		SET ISOLATION TO DIRTY READ;
		FOREACH
			SELECT nombre_arch, fecha_presentacion, tipo_registro, num_secuencia, cod_operacion, cod_divisa, fecha_trans, banco_presentador, banco_receptor, importe, uso_futuro_ccen, tipo_operacion, fecha_aplica, tipo_cta_ord, num_cta_ord, nombre_ord, rfc_ord, tipo_cta_rec, num_cta_rec, nombre_rec, rfc_rec, ref_servicio, nombre_titular_serv, importe_iva, ref_numerica, ref_leyenda, clave_rastreo, motivo_dev, fecha_pres_ini, solicitud_confirmacion, uso_futuro_banco, ref_confirmacion, uso_futuro_cce, tasa_tiie_prom, dias_retraso, imp_tot_int, cve_status, folio_suc, user_insert, fecha_insert, tipo_cta
			INTO vsNomArchivo, vsFecPresentacion, vsTipoReg, vsNumSec, vsCodOper, vsCodDivisa, vsFechaTrans, vsBancoPres, vsBancoRec, vsImporte, vsUsoFutCccen, vsTipoOper, vsFechaAplica, vsTipoCtaOrd, vsNumCtaOrd, vsNombreOrd, vsRfcOrd, vsTipoCtaRec, vsNumCtaRec, vsNombreRec, vsRfcRec, vsRefServicio, vsNomTitServ, vsImpIva, vsRefNumerica, vsRefLeyenda, vsClaveRastreo, vsMotivoDev, vsFechaPresIni, vsSolConfirmacion, vsUsoFutBanco, vsRefConfirmacion, vsUsoFutCce, vsTasaTiieProm, vsDiasRetraso, vsImpTotInt, vsCveEstatus, vsFolioSuc, vsUserInsert, vsFechaInsert, vsTipoCta
			FROM bditef:"informix".tef_cce_detalle
			WHERE nombre_arch = psNomArchivo
            order by fecha_presentacion, importe
			let vsImporte=  "$" || round(vsImporte / 100, 2);
			LET vsCodRet = '00000';
			RETURN vsCodRet, vsNomArchivo, vsFecPresentacion, vsTipoReg, vsNumSec, vsCodOper, vsCodDivisa, vsFechaTrans, vsBancoPres, vsBancoRec, vsImporte, vsUsoFutCccen, vsTipoOper, vsFechaAplica, vsTipoCtaOrd, vsNumCtaOrd, vsNombreOrd, vsRfcOrd, vsTipoCtaRec, vsNumCtaRec, vsNombreRec, vsRfcRec, vsRefServicio, vsNomTitServ, vsImpIva, vsRefNumerica, vsRefLeyenda, vsClaveRastreo, vsMotivoDev, vsFechaPresIni, vsSolConfirmacion, vsUsoFutBanco, vsRefConfirmacion, vsUsoFutCccen, vsTasaTiieProm, vsDiasRetraso, vsImpTotInt, vsCveEstatus, vsFolioSuc, vsUserInsert, vsFechaInsert, vsTipoCta WITH RESUME;
		END FOREACH;
	ELIF(psEncDetSum == 3)THEN
		SET LOCK MODE TO WAIT 3;
		SET ISOLATION TO DIRTY READ;
		SELECT FIRST 1 nombre_arch, fecha_presentacion, tipo_registro, num_secuencia, cod_operacion, num_bloque, num_operaciones, imp_operaciones, uso_futuro_ccen, uso_futuro_banco, user_insert, fecha_insert
		INTO vsNomArchivo, vsFecPresentacion, vsTipoReg, vsNumSec, vsCodOper, vsNumBloque, vsNumOper, vsImpOper, vsUsoFutCccen, vsUsoFutBanco, vsUserInsert, vsFechaInsert
		FROM bditef:"informix".tef_cce_sumario
		WHERE nombre_arch = psNomArchivo;
		LET vsCodRet = '00000';
		RETURN vsCodRet, vsNomArchivo, vsFecPresentacion, vsTipoReg, vsNumSec, vsCodOper, vsNumBloque, vsNumOper, vsImpOper, vsUsoFutCccen, vsUsoFutBanco, vsUserInsert, vsFechaInsert, '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '';
	END IF;
END IF;
...
'Descripcion: Consulta archivos general y detallado.',
'Fecha: 2011/04/08',
'Version: 20110408.1800',
'BD: BdiTef';

CREATE PROCEDURE "informix".sp_consdevext_tef
(
...
DEFINE vsFechaHoy CHAR(10);
DEFINE vsValFecha CHAR(10);
...
--SET DEBUG FILE TO "/tmp/TEF/sp_consdevext_tef.sql";
--TRACE ON;
...
ON EXCEPTION SET viSqlErr   --Cacha el error en caso de que exista y regresa un valor predeterminado
	IF viSqlErr <> 0 THEN
		RETURN viSqlErr, vsFechaAbono, vsCuenta, vsReferencia, vsBanco, vsImporte, vsEstatus, vsConfirmado, vsNumSecuencia, vsClaveRastreo, vsCveStatus;
	END IF;
END EXCEPTION;

--Consulta solo las que cumplen las condiciones para ser devueltos.
IF(psConsultaDevuelve == 1)THEN 
	--Selecciona la fecha hoy.
	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	SELECT FIRST 1 fecha_hoy INTO vsFechaHoy FROM bdinteg:"informix".si_fechas;
	--Se obtiene la cantidad de dias naturales.
	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	SELECT FIRST 1 valor INTO vsDiasNat FROM bditef:"informix".tef_parametros WHERE cod_param = '81';
	--Se le restan N dias naturales a la fecha hoy.
	LET vsFechaHoy = vsFechaHoy::DATE - TRIM(vsDiasNat)::INTEGER;
	--Se guarda en una variable de tipo char el dia calculado.
	LET vsValFecha = SUBSTRING(vsFechaHoy FROM 7 FOR 4) || SUBSTRING(vsFechaHoy FROM 1 FOR 2) || SUBSTRING(vsFechaHoy FROM 4 FOR 2);
	--Verifica que sea fecha habil.
	EXECUTE PROCEDURE bditef:"informix".sp_valida_fecha(vsValFecha) INTO vsCodRet;
		--Mientras no sea dia habil, se suma un dia a la fecha hoy, y se actualiza 'vsValFecha' para ser verificada de nuevo.
		WHILE vsCodRet == "00002"
			LET vsFechaHoy = vsFechaHoy::DATE + 1;
			LET vsValFecha = SUBSTRING(vsFechaHoy FROM 7 FOR 4) || SUBSTRING(vsFechaHoy FROM 1 FOR 2) || SUBSTRING(vsFechaHoy FROM 4 FOR 2);
			EXECUTE PROCEDURE bditef:"informix".sp_valida_fecha(vsValFecha) INTO vsCodRet;
		END WHILE
	--Al ser una fecha habil, se toma la fecha parametro.
	LET vsValFecha = SUBSTRING(psFechaArch FROM 5 FOR 2) || "/" || SUBSTRING(psFechaArch FROM 7 FOR 2) || "/" || SUBSTRING(psFechaArch FROM 1 FOR 4);
	--Compara que la fecha parametro siempre sea mayor o igual a la fecha maxima habil(hasta N dias naturales o la posterior
	IF(vsValFecha::DATE >= vsFechaHoy::DATE)THEN
		SET LOCK MODE TO WAIT 3;
		SET ISOLATION TO DIRTY READ;
		FOREACH
			--SELECT ccedet.fecha_aplica, ccedet.num_cta_ord, ccedet.folio_suc, siban.descripcion, ccedet.importe, statp.descripcion, ccedet.solicitud_confirmacion, ccedet.num_secuencia, ccedet.clave_rastreo, ccedet.cve_status
			SELECT ccedet.fecha_aplica, ccedet.num_cta_rec, ccedet.folio_suc, siban.descripcion, ccedet.importe, statp.descripcion, ccedet.solicitud_confirmacion, ccedet.num_secuencia, ccedet.clave_rastreo, ccedet.cve_status
			INTO vsFechaAbono, vsCuenta, vsReferencia, vsBanco, vsImporte, vsEstatus, vsConfirmado, vsNumSecuencia, vsClaveRastreo, vsCveStatus
			FROM bditef:"informix".tef_cce_detalle AS ccedet, bdinteg:"informix".si_bancos AS siban, bditef:"informix".tef_status_pago AS statp
			WHERE ccedet.fecha_aplica = psFechaArch
			AND ccedet.cod_operacion = '60'
			AND ccedet.banco_receptor = '137'
            AND ccedet.banco_presentador = siban.banco
			AND ccedet.cve_status = statp.cve_status
			ORDER BY ccedet.cve_status
			LET vsCodRet = "00000";
			RETURN vsCodRet, vsFechaAbono, vsCuenta, vsReferencia, vsBanco, vsImporte, vsEstatus, vsConfirmado, vsNumSecuencia, vsClaveRastreo, vsCveStatus WITH RESUME;
		END FOREACH;
	ELSE
		LET vsCodRet = "00001";
	END IF;
ELIF(psConsultaDevuelve == 2)THEN --Actualiza registros seleccionados para su devolución.
	UPDATE bditef:"informix".tef_cce_detalle SET cve_status = '03', motivo_dev = psMotivoDev 
	WHERE cod_operacion = '60'
	AND num_secuencia = psNumSecuencia 
	AND fecha_aplica = psFechaArch
	--AND num_cta_ord = psNumCtaOrd
	AND num_cta_rec = psNumCtaRec
	AND folio_suc = psFolioSuc;
	--IF EXISTS(SELECT num_secuencia, fecha_aplica, num_cta_ord, folio_suc FROM bditef:"informix".tef_cce_detalle WHERE num_secuencia = psNumSecuencia AND fecha_aplica = psFechaArch AND num_cta_ord = psNumCtaOrd AND folio_suc = psFolioSuc AND cve_status = '01')THEN
	--	LET vsCodRet = "00003";
	--ELSE
	--	LET vsCodRet = "00000";
	--END IF;
	RETURN vsCodRet, '', '', '', '', '', '', '', '', '', '' WITH RESUME;
END IF;
...
'Proyecto: Transferencia Electronica de Fondos',
'Solicito: Jaime Gonzalez Prado',
'Descripcion: Consulta registros para realizar devoluciones extemporaneas.',
'Fecha: 2011/03/25',
'Version: 20110325.1800',
'BD: BdiTef';
