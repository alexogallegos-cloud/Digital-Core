	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			...
			IF ven_transacc = 1 THEN
				ROLLBACK WORK;		
			END IF;
			UPDATE "informix".sw_verificastatusrepxsist
			SET status = 'E', error_proceso = 'S', error = cCodRet WHERE usuario_insert = pUsuario;
			RETURN cCodRet, cRutaGral, iTotal;
		END EXCEPTION;
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ris_consultarepsistema2.out';
		--TRACE ON;
		
		-- SE LIMPIA TABLA POR USUARIO
		DELETE FROM "informix".sw_verificastatusrepxsist WHERE usuario_insert = TRIM(pUsuario);
		INSERT INTO "informix".sw_verificastatusrepxsist(usuario_insert,status,nombre_archivo,error_proceso,error) VALUES(pUsuario,'I','','',cCodRet);
		
		IF pUsuario = '' OR pIdFuncion = '' OR pBandera = '' OR pSistema = '' THEN
			LET cCodRet = '00003';
			UPDATE "informix".sw_verificastatusrepxsist
			SET status = 'E', error_proceso = 'S', error = cCodRet WHERE usuario_insert = pUsuario;
			RETURN cCodRet, cRutaGral, iTotal;
		...
		IF  pBandera = 1 THEN
			...
			SELECT COUNT(*)
			INTO iTotal
			FROM (SELECT A.transaccion, B.descripcion, A.secuencia 
			FROM bdinteg:"informix".si_prodtran AS A
			INNER JOIN bdinteg:"informix".si_transacc AS B ON B.empresa = A.empresa AND B.numero = A.transaccion
			WHERE (B.naturaleza = 'A' OR B.naturaleza = 'C') AND A.sistema = pSistema AND A.empresa = cEmpresa
			GROUP BY transaccion, descripcion, secuencia);

            IF NVL(iTotal,0) = 0 THEN
            	LET cCodRet = '00017';
            
                UPDATE "informix".sw_verificastatusrepxsist
                SET status = 'E', error_proceso = 'S' WHERE usuario_insert = pUsuario;
                
                RETURN cCodRet, cRutaGral, iTotal;
            END IF;
			
			UPDATE "informix".sw_verificastatusrepxsist
			SET status = 'T', error_proceso = 'N' WHERE usuario_insert = pUsuario;
		
			RETURN cCodRet, cRutaGral, iTotal;
			
		ELIF pBandera = 2 THEN
		
			DELETE FROM bdicnweb:"informix".sw_ristras_reporteporsistema WHERE usuario_insert = pUsuario;
			DELETE FROM bdicnweb:"informix".sw_ristras_reporteporsistemaproductos WHERE usuario_insert = pUsuario;
			...
			INSERT INTO bdicnweb:"informix".sw_ristras_reporteporsistema (usuario_insert, transaccion, descripcion, secuencia)
			SELECT pUsuario, A.transaccion, B.descripcion, A.secuencia
			FROM bdinteg:"informix".si_prodtran AS A
			INNER JOIN bdinteg:"informix".si_transacc AS B ON B.empresa = A.empresa AND B.numero = A.transaccion
			WHERE (B.naturaleza = 'A' OR B.naturaleza = 'C') AND A.sistema = pSistema AND B.sistema = pSistema AND A.empresa = cEmpresa
			GROUP BY transaccion, descripcion, secuencia
			ORDER BY transaccion, secuencia;
		
			LET cCmd1 = TRIM(cUsrBin)||"echo " || '"' || "INSERT INTO bdicnweb:""informix"".sw_ristras_reporteporsistemaproductos (usuario_insert, producto, descripcion) SELECT '" || pUsuario || "', " || TRIM(cCampo1) || ", " || TRIM(cCampo2) || " FROM " || TRIM(cBase) || ":""informix""." || TRIM(cTabla) || ";" || '"' || " > /tmp/mfinis/" || cFile;
			SYSTEM TRIM(cCmd1);
			
			LET cCmd1 = "";
			LET cCmd1 = TRIM(cIfxBin)||"dbaccess bdicnweb < " || TRIM(pRutaDescarga) || "/" || cFile;
			SYSTEM TRIM(cCmd1);
		
			LET cCmd1 = "";
			LET cCmd1 = TRIM(cUsrBin)||"rm -rf " || TRIM(pRutaDescarga) || "/" || TRIM(cFile);
			SYSTEM TRIM(cCmd1);
			
			FOREACH
		
				SELECT producto, descripcion
				INTO cProducto, cDescProd
				FROM bdicnweb:"informix".sw_ristras_reporteporsistemaproductos 
				WHERE usuario_insert = pUsuario
				ORDER BY producto ASC
			 
				LET iAux = iAux+1;
				LET cNumProducto = TRIM(cNumProducto) || "'" || TRIM(cProducto) || "',";
                LET cNumProducto =  cNumProducto;
				LET cProductoFinal = TRIM(cProductoFinal) || TRIM(cProducto) || ",";
                LET cProductoFinal = cProductoFinal;
				IF (iAux<=30)THEN							
				LET cDescProdFinal = TRIM(cDescProdFinal) ||TRIM(cProducto) ||" "||TRIM(NVL(cDescProd,"")) || " - CARGO,"  || TRIM(cProducto) ||" "|| TRIM(NVL(cDescProd,""))||" - ABONO,";
				END IF;
				IF (iAux>30 and iAux<50)THEN
				LET cDescProdFinal2 = TRIM(cDescProdFinal2) ||TRIM(cProducto) ||" "||TRIM(NVL(cDescProd,"")) || " - CARGO,"  || TRIM(cProducto) ||" "|| TRIM(NVL(cDescProd,""))||" - ABONO,";
				END IF;
				IF (iAux>50)THEN
				LET cDescProdFinal3 = TRIM(cDescProdFinal3) ||TRIM(cProducto) ||" "||TRIM(NVL(cDescProd,"")) || " - CARGO,"  || TRIM(cProducto) ||" "|| TRIM(NVL(cDescProd,""))||" - ABONO,";
				END IF;
               
			END FOREACH;
			
			LET iTamReg = LENGTH(TRIM(cNumProducto));
			LET cNumProducto = SUBSTR(TRIM(cNumProducto), 0, (iTamReg - 1));
			
		--	LET iTamReg = LENGTH(TRIM(cDescProdFinal));
		--	LET cDescProdFinal = SUBSTR(TRIM(cDescProdFinal), 0, (iTamReg - 1));
			...
			LET cDescProdFinal = SUBSTR(TRIM(cDescProdFinal), 0, (iTamReg - 1));
				...
				IF (iAux>30 and iAux<50)THEN
                   LET iTamReg = LENGTH(TRIM(cDescProdFinal));
                    LET cDescProdFinal = SUBSTR(TRIM(cDescProdFinal), 0, (iTamReg - 1));
				     LET iTamReg = LENGTH(TRIM(cDescProdFinal2));
                	LET cDescProdFinal2 = SUBSTR(TRIM(cDescProdFinal2), 0, (iTamReg - 1));
				...
				IF (iAux>50)THEN
                 LET iTamReg = LENGTH(TRIM(cDescProdFinal));
                	...
                	LET cDescProdFinal2 = SUBSTR(TRIM(cDescProdFinal2), 0, (iTamReg - 1));
				     ...
				     LET iTamReg = LENGTH(TRIM(cDescProdFinal3));
                    LET cDescProdFinal3 = SUBSTR(TRIM(cDescProdFinal3), 0, (iTamReg - 1));
                   
				END IF;



			UPDATE bdicnweb:"informix".sw_ristras_reporteporsistema SET des_productos = cDescProdFinal WHERE usuario_insert = pUsuario;
			
			
			
			FOREACH
		
				SELECT transaccion, descripcion, secuencia
				INTO cTransaccion, cDescripcion, iSecuencia
				FROM bdicnweb:"informix".sw_ristras_reporteporsistema 
				WHERE usuario_insert = pUsuario
				...
				FOREACH
					...
					SELECT  NVL((TRIM(c_ccmayor) || ' '  || TRIM(c_ccsub) || ' '  || TRIM(c_ccsubsub) || ' '  || TRIM(c_ccsssub) || ' '  || TRIM(c_ccssssub) || ' '  || TRIM(c_sector)), '') AS cuenta_cargo, 
					NVL((TRIM(a_ccmayor) || ' '  || TRIM(a_ccsub) || ' '  || TRIM(a_ccsubsub) || ' '  || TRIM(a_ccsssub) || ' '  || TRIM(a_ccssssub) || ' '  || TRIM(a_sector)), '') AS cuenta_abono
					INTO cCuentaCargo, cCuentaAbono
					FROM bdinteg:si_prodtran AS A 
					INNER JOIN bdinteg:si_transacc AS B ON B.empresa = A.empresa AND B.numero = A.transaccion AND A.sistema =B.sistema
					WHERE (B.naturaleza = 'A' or B.naturaleza = 'C') 
					AND A.sistema = pSistema AND A.empresa = cEmpresa AND A.transaccion = cTransaccion AND descripcion= cDescripcion AND A.secuencia = iSecuencia AND A.producto = cProducto;

					LET cCuentaCargo = NVL(cCuentaCargo,'');
					LET cCuentaAbono = NVL(cCuentaAbono,'');
					LET cCtaContableFinal = TRIM(cCtaContableFinal) || TRIM(cCuentaCargo) || "," || TRIM(cCuentaAbono) || ",";
					
					SELECT c_cuenta
					INTO cCtaContablePaso
					FROM bdicnweb:"informix".sw_ristras_reporteporsistema 
					WHERE usuario_insert = pUsuario AND transaccion = cTransaccion AND descripcion= cDescripcion AND secuencia = iSecuencia;
					
					LET cCtaContablePaso = NVL(cCtaContablePaso,'');
					
					UPDATE bdicnweb:"informix".sw_ristras_reporteporsistema SET c_cuenta = (TRIM(cCtaContablePaso) || TRIM(cCtaContableFinal)) 
					WHERE usuario_insert = pUsuario AND transaccion = cTransaccion AND descripcion= cDescripcion AND secuencia = iSecuencia;
				...
				END FOREACH;
					...
					LET cCtaContablePaso = NVL(cCtaContablePaso,'');
					LET iTamReg = LENGTH(TRIM(cCtaContablePaso));
				    LET cCtaContablePaso = SUBSTR(TRIM(cCtaContablePaso), 0, (iTamReg - 1));
			...
			LET cNombreRep = 'REP_TRANS_SISTEMA_'||TO_CHAR(CURRENT, '%d%m%Y%H%M%S')||'.csv';
            ...
            LET cDescProdFinal = cDescProdFinal;
			LET cCmd1 ="";			
            LET cCmd1 =""||TRIM(cCmd1)||"SELECT 'NO. TRANSACCION','DESCRIPCION TRANSACCION','SECUENCIA','";
            LET cCmd2 =" FROM systables WHERE tabid = 1 UNION ALL SELECT * FROM( SELECT transaccion, descripcion, secuencia::CHAR(11), c_cuenta FROM bdicnweb:""informix"".sw_ristras_reporteporsistema WHERE usuario_insert = '"|| pUsuario ||"' ORDER BY id_registro ASC)";
			...
			LET cSql = '';			
			SYSTEM TRIM('/usr/bin/echo "SET ISOLATION TO DIRTY READ; UNLOAD TO '||TRIM(cRutaGral)||' DELIMITER '''|| ",' "||TRIM(cCmd1)|| TRIM(cDescProdFinal) || TRIM(nvl(cDescProdFinal2,'')) || TRIM(nvl(cDescProdFinal3,'')) ||"'" || TRIM(cCmd2)  || ' " > '||TRIM(pRutaDescarga)||'query.sql');
			
            LET long = length(cSql);
			LET cSql = '';
			...
			LET cSql = '';
			LET cSql = '/usr/bin/rm -rf '||TRIM(cRutaGral)||'.tmp';
			SYSTEM TRIM(cSql);
		...
		UPDATE "informix".sw_verificastatusrepxsist
		SET status = 'T', error_proceso = 'N', nombre_archivo = cRutaGral WHERE usuario_insert = pUsuario;
...
'DESCRIPCION: SPL encargado de Consultar el reporte y los totales para el reporte',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_actualizacuentasdormidas(pUsuario CHAR(8), pIdFuncion CHAR(10), pMontoDep MONEY(10,2), pDiasInac SMALLINT, pDiasBloq SMALLINT)
	RETURNING CHAR(5) AS codret;
	...
	DEFINE mMontoDeposito MONEY(10,2);
	DEFINE iNumDiasInac	SMALLINT;
	DEFINE iNumDiasBloq	SMALLINT;
	DEFINE iTotal INTEGER;
	...
	BEGIN
		...
		IF pUsuario = '' OR pIdFuncion = '' OR pMontoDep IS NULL OR pDiasInac IS NULL OR pDiasBloq IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF;
		...
		END IF;
		 ...
		 --SET DEBUG FILE TO '/tmp/mfinis/sp_actualizacuentasdormidas.out';
		 --TRACE ON;
		...
		SELECT COUNT(*)
		INTO iTotal
		FROM bdicheq:"informix".sc_param_ctasdormidas;
		
		IF (iTotal > 0) THEN 
		
			SELECT monto_deposito,num_dias_inac,num_dias_bloq 
			INTO mMontoDeposito,iNumDiasInac,iNumDiasBloq 
			FROM bdicheq:"informix".sc_param_ctasdormidas;
			
			IF (mMontoDeposito = pMontoDep AND iNumDiasInac = pDiasInac AND iNumDiasBloq = pDiasBloq) THEN
				LET cCodRet = '99999';
			ELSE
				UPDATE bdicheq:"informix".sc_param_ctasdormidas SET  
				fecha_alta = CURRENT, 
				hora_alta = CURRENT HOUR TO SECOND, 
				monto_deposito = pMontoDep, 
				num_dias_inac = pDiasInac, 
				num_dias_bloq = pDiasBloq,
				usuario = pUsuario;
			END IF;
		ELSE
			INSERT INTO bdicheq:"informix".sc_param_ctasdormidas (fecha_alta,hora_alta,monto_deposito,num_dias_inac,num_dias_bloq,usuario)
			VALUES (CURRENT,CURRENT HOUR TO SECOND,pMontoDep,pDiasInac,pDiasBloq,pUsuario);
		END IF;
...
'FECHA: 08/04/2021',
'MODULO: OPERACIONES',
'FUNCIONALIDAD: BLOQUEO DE CUENTAS DORMIDAS',
'DESCRIPCION: INSERTA O ACTUALIZA LOS VALORES DE LA TABLA sc_param_ctasdormidas',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consultacuentasdormidas(pUsuario CHAR(8), pIdFuncion CHAR(10))
	RETURNING CHAR(5) AS codret, MONEY(10,2) AS Monto_deposito, SMALLINT AS NumDiasInac, SMALLINT AS iNumDiasBloq;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,mMontoDeposito,iNumDiasInac,iNumDiasBloq;
		END EXCEPTION;
		...
		-- SET DEBUG FILE TO '/tmp/mfinis/sp_consultacuentasdormidas.out';
		-- TRACE ON;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: BLOQUEO DE CUENTAS DORMIDAS',
'DESCRIPCION: CONSULTA LOS VALORES DE LA TABLA sc_param_ctasdormidas',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_genrepcuentasdormidas(pUsuario CHAR(8), pIdFuncion CHAR(10),pfechaDe DATE, pfechaHasta DATE,pRutaDescarga CHAR(50),pTipo CHAR(1))
    RETURNING CHAR(5) AS codret,
    ...
    BEGIN
	    ...
	    --SET DEBUG FILE TO '/tmp/mfinis/sp_ofi_genrepcuentasdormidas.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pRutaDescarga = '' OR pTipo='' THEN
			LET cCodRet = '00003';
			RETURN cCodRet,cNombreRepXls;
		END IF;
		...
		-- SE DEFINE NOMENCLATURA DEL REPORTE A GENERAR		
		IF (pTipo=1) THEN 
		LET cNombreRepXls = 'CuentasDormidas'||TO_CHAR(CURRENT, '%d%m%Y')||'_'||(TO_CHAR(CURRENT, '%H%M%S'))||'.xls';
		ELSE 
		LET cNombreRepXls = 'CuentasDormidas'||TO_CHAR(CURRENT, '%d%m%Y')||'_'||(TO_CHAR(CURRENT, '%H%M%S'))||'.txt';
		END IF;
		LET pRutaDescarga = TRIM(pRutaDescarga) || '/';
        ...
        SELECT COUNT(*)
		INTO iTotal
		FROM bdicheq:sc_histbloq AS a 
        INNER JOIN bdicheq:sc_maechq AS b ON a.cuenta = b.cuenta 
        INNER JOIN bdicheq:sc_maenoc AS d ON d.cuenta = a.cuenta 
        INNER JOIN bdinteg:si_cliente AS c ON c.numcte = b.num_cte 
        WHERE a.tipo_mov = 'B' AND a.motivo = '14'
        and a.fecha >=  pfechaDe  and a.fecha <=  pfechaHasta;
		
		IF (iTotal > 0) THEN 

		LET cCmd1 ="";
		LET cCmd1 =""||TRIM(cCmd1)||"SELECT * FROM( ";
		LET cCmd1 =""||TRIM(cCmd1)||"SELECT 'NUMERO DE CLIENTE','NUMERO DE CUENTA','NOMBRE DEL CLIENTE','FECHA DE LA OPERACION','MONTO DE LA OPERACION','SUCURSAL DE LA OPERACION',";
		LET cCmd1 =""||TRIM(cCmd1)||"'FECHA DE APERTURA DE LA CUENTA'";
		LET cCmd1 =""||TRIM(cCmd1)||" FROM systables WHERE tabid = 1 UNION ALL SELECT * FROM( ";
		LET cCmd1 =""||TRIM(cCmd1)||"SELECT b.num_cte::CHAR(20),a.cuenta::CHAR(20), TRIM (c.apell_paterno::CHAR(26)) ||' '||TRIM(c.apell_materno::CHAR(26))||' '||TRIM(c.nombre1::CHAR(26))||' '||TRIM(c.nombre2::CHAR(26)),TO_CHAR(a.fecha, '%d/%m/%Y'),a.importe::CHAR(12),b.sucursal::CHAR(4),";
		LET cCmd1 =""||TRIM(cCmd1)||"TO_CHAR(d.fecha_alta, '%d/%m/%Y')";
		LET cCmd1 =""||TRIM(cCmd1)||" FROM bdicheq:sc_histbloq AS a INNER JOIN bdicheq:sc_maechq AS b ON a.cuenta = b.cuenta INNER JOIN bdicheq:sc_maenoc AS d ON d.cuenta = a.cuenta INNER JOIN bdinteg:si_cliente AS c ON c.numcte = b.num_cte WHERE a.tipo_mov = 'B' AND a.motivo = '14'";
		LET cCmd1 =""||TRIM(cCmd1)||" and a.fecha >=  '"|| TO_CHAR(pfechaDe, '%m%d%Y') ||"' and a.fecha <= '"|| TO_CHAR(pfechaHasta, '%m%d%Y') ||"'))";
...
'FECHA: 05/02/2021',
'MODULO: OPERACIONES ',
'FUNCIONALIDAD: REPORTE DE CONSULTA CUENTAS DORMIDAS',
...
'DESCRIPCION: SPL que consulta la caja general para el catálogo de  atm´s  ',
'FECHA: 08/07/2021',
'MODIFICO: Daniel Reyes Guillen',
'DESCRIPCION: Se añade paginado y direccionamiento al sp productivo clonado  ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_cp_consultadetcuentastdcoro(pUsuario CHAR(8), pIdFuncion CHAR(10), pDireccionMac CHAR(12), pNombreArchivo CHAR(35),
pRegistros INTEGER, pRecuperacion INTEGER)
	...
	BEGIN
		...
		FOREACH
			...
			ORDER BY num_credito ASC ,tipo_tarjeta DESC,num_tarjeta ASC
			...
			-- RECUPERA DOMICILIO ENVÃO DEL TITULAR PARA HOMOLOGAR TARJETAS ADICIONALES
...
CREATE PROCEDURE "informix".sp_cp_consultadetcuentastdcoro_totales(pUsuario CHAR(8), pIdFuncion CHAR(10), pDireccionMac CHAR(12), pNombreArchivo CHAR(35))
	RETURNING CHAR(5) AS codret,
...
'MODULO: CREDITO',
'FUNCIONALIDAD: CAMBIO DE PRODUCTO DE TDC OPERACIÃ?N MASIVA',
'DESCRIPCION: SPL encargado de consultar el nÃºmero total de las cuentas a las cuales se les va a aplicar el cambio de producto.',
'AUTOR: L. Montserrat LeÃ³n Amador',
'FECHA: 30/04/2019',
'DESCRIPCION: Se modifica spl para agregar nuevas reglas de negocio RQM 10 682-4.',
'AUTOR: L. Montserrat LeÃ³n Amador',
