CREATE PROCEDURE "informix".sp_consultarespuestaspld(pIdPregunta SMALLINT, pNumCte CHAR(20))
	RETURNING CHAR(5), CHAR(255), CHAR(2), CHAR(3), CHAR(3), CHAR (60);

--Definicion de Variables
DEFINE iSqlErr INTEGER;
DEFINE cCodRet CHAR(5);
DEFINE cResAbt CHAR(255);
DEFINE cResDir CHAR(2);
DEFINE cResAct CHAR(3);
DEFINE cResSub CHAR(3);
DEFINE cDesPus CHAR(60);
...
--SET DEBUG FILE TO '/tmp/sp_ConsultaRespuestasPLD.out';
--TRACE ON;
...
BEGIN
	ON EXCEPTION
		...
		IF iSqlErr <> 0 THEN
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cResAbt, cResDir, cResAct, cResSub, cDesPus;
		END IF;
	END EXCEPTION;
	...
	IF pIdPregunta = 6 THEN
	
		SELECT nombre_empresa, claveopcionpuesto, clavesubopcionpuesto INTO cDesPus, cResAct, cResSub FROM bdinteg:"informix".si_ingresos
		WHERE numcte = pNumCte AND tipo_ingreso = 'T' 
		AND sec_ingreso = (SELECT MAX(sec_ingreso) FROM bdinteg:"informix".si_ingresos WHERE numcte = pNumCte AND tipo_ingreso = 'T');
		
		IF NVL(cDesPus, '') = '' OR NVL(cResAct, '') = '' THEN
			SELECT id_act, id_subact INTO cResAct, cResSub FROM bdinteg:"informix".si_bitacoraapertura 
			WHERE numcte = pNumCte AND id_pregunta = pIdPregunta
			AND id_secuencia = (SELECT MAX(id_secuencia) FROM bdinteg:"informix".si_bitacoraapertura WHERE numcte = pNumCte AND id_pregunta = pIdPregunta);
			
			IF NVL(cResAct, '') = '' OR NVL(cResSub, '') = '' THEN
				LET cResAct = '';
				LET cResSub = '';
				LET cCodRet = '00001';
			END IF;
	...
	ELIF pIdPregunta = 7 OR pIdPregunta = 8 THEN
		SELECT motivo_rechazo INTO cResAbt FROM bdinteg:"informix".si_bitacoraapertura 
		WHERE numcte = pNumCte AND id_pregunta = pIdPregunta
		AND id_secuencia = (SELECT MAX(id_secuencia) FROM bdinteg:"informix".si_bitacoraapertura WHERE numcte = pNumCte AND id_pregunta = pIdPregunta);
	...
	ELSE
		SELECT respuesta INTO cResDir FROM bdinteg:"informix".si_bitacoraapertura 
		WHERE numcte = pNumCte AND id_pregunta = pIdPregunta
		AND id_secuencia = (SELECT MAX(id_secuencia) FROM bdinteg:"informix".si_bitacoraapertura WHERE numcte = pNumCte AND id_pregunta = pIdPregunta);
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Consulta respuestas del PLD',
'AUTOR : Adrian Lara I.',
'FECHA : 01 de Julio de 2011',
'VERSION: 20110701.1240',
'BD: bdinteg';
