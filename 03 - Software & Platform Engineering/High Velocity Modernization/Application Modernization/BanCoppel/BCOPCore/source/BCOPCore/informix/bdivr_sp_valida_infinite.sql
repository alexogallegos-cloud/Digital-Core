CREATE PROCEDURE "informix".sp_valida_infinite(p_cTelefono CHAR(10), p_cCapturado CHAR(16))
RETURNING
	CHAR(5),	-- CÃDGIO DE RETORNO
	CHAR(9),	-- NÃMERO DE CLIENTE
	CHAR(4),	-- NÃMERO DE SUCURSAL
	...
	-- SET DEBUG FILE TO "/tmp/clizarraga/sp_valida_infinite.out"; -- MODIFICAR RUTA DEL ARCHIVO
	-- TRACE ON;
	...
	BEGIN
		ON EXCEPTION SET iSqlErr
			IF iSqlErr != 0 THEN
				LET cReCord = iSqlErr;
				RETURN cReCord, v_sNumcliente, vSucursal, vOpcionAcceso;
			END IF;
		END EXCEPTION;
		...
		IF (TRIM(NVL(p_cTelefono,''))  != '') THEN
			...
			IF (TRIM(NVL(p_cCapturado,'')) != '') THEN
				...
				IF v_sTipoConsulta = 9 THEN --opcion numero de cliente por la que accesa el cliente.	
					SELECT numcte
					INTO v_sNumcliente
					...
					IF (TRIM(vcNumCtaCte) IS NULL) OR (TRIM(vcNumCtaCte) = '')THEN
						SELECT LIMIT 1 num_credito,num_producto, sucursal
							INTO vcNumCtaCte, vcProducto, vSucursal
						FROM bdicred:"informix".sd_maecred 
						WHERE numcte = v_sNumcliente
						AND num_producto='5400';
						
						LET iResConsulta =  DBINFO("sqlca.sqlerrd2");
					
						IF iResConsulta > 0 THEN
							SELECT limit 1 tar.num_tarjeta, crd.sucursal
							INTO vTarjeta, vSucursal 
							FROM bdicred:"informix".sd_tarjeta tar, bdicred:"informix".sd_maecred crd
							WHERE tar.num_credito = tar.num_credito 
							AND crd.num_credito = vcNumCtaCte
							AND crd.num_producto ='5400'
				...
				ELIF (v_sTipoConsulta = 16) THEN 
                    ...
                    SELECT concat(trim(bin), trim(producto)) AS binsubbin 
					    INTO vBin
					FROM intercard:"informix".binproducto 
					WHERE idbinproducto = 176;

					SELECT tar.numcte, crd.num_producto, crd.sucursal-- Consulta cuenta infinite en productos del cliente
						INTO v_sNumcliente, vcProducto, vSucursal
					...
					AND substr(tar.num_tarjeta, 1 ,8) = vBin; 
			...
			/*   -------------		SE COMENTA EL CODIGO DONDE VALIDA EL CLIENTE ENROLADO (TODO DEBIDO A TARJETA INFINITE)		-----------------------------
				...
				*/
				...
				END IF;
					...
					-------------		SE COMENTA EL CODIGO DONDE VALIDA EL NUMERO DE TEL DEL CLIENTE (TODO DEBIDO A TARJETA INFINITE)		-----------------------------
			...
			IF cReCord = '00000' OR cReCord = '00020' THEN
				...
				VALUES (p_cTelefono, v_sNumcliente, current);
			ELSE
		    	...
		    	VALUES (CURRENT, vSecMax, 'INF', '', '', p_cTelefono, vOpcionAcceso, '');	
			END IF;
		END IF;
...
/*
Fecha: 11/05/2023
DescripciÃ³n: Se valida si el nÃºmero de cliente, crÃ©dito o tarjeat corresponde a un producto INFINITE.
Base de datos: BDIVR
*/;
