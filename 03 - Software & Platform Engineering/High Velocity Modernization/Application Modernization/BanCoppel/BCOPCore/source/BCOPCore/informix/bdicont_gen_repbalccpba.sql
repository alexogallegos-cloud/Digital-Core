CREATE PROCEDURE "informix".gen_repbalccpba(p_val char(2),p_ext char(2),
                            w_fecha char(10),v_empresa char(3),v_moneda char(2),v_usuario char(10))
...
--set debug file to "/pisa/pisabanco/pisa_ftes/contabilidad/gen_repbalcc_concu.out";
--trace on;
...
where usuario=v_usuario;
   ...
   -- BALANZA PREVIA
   if p_ext = "PR" then
      EXECUTE PROCEDURE gen_balprevcc(v_empresa,lv_fechac,v_usuario);
      EXECUTE PROCEDURE ctas_nuevascc(v_empresa,lv_fechac,v_usuario);
   end if
   ...
   if p_ext = "MA" then
      ...
      foreach
         select co_sdomes.empresa,co_sdomes.ccmayor,co_sdomes.ccsub,
                ...
                --co_sdomes.ciudad,
                co_sdomes.sucursal,co_sdomes.moneda,
   ...
   if p_ext = "PM" then
      foreach
         {select co_sdodias.empresa,co_sdodias.ccmayor,co_sdodias.ccsub,
                co_sdodias.ccsubsub,co_sdodias.ccssubsub,
                co_sdodias.ccsssubsub,co_sdodias.sector,
                co_sdodias.ciudad,co_sdodias.sucursal,co_sdodias.moneda,
                dias_acumulado,saldo_acumulado,saldo_acumulado/day(w_fecha),
                0,naturaleza_cta
         into
                c_empresa,c_ccmayor,c_ccsub,c_ccsubsub,c_ccssubsub,
                c_ccsssubsub,c_sector,c_ciudad,c_sucursal,c_moneda,
                c_cargos_dia,c_abonos_dia,c_saldo_anterior,
                c_saldo_actual,c_naturaleza_cta
         from co_sdodias,bdinteg:si_catalog
         where co_sdodias.empresa = bdinteg:si_catalog.empresa
         ...
         and co_sdomes.sucursal = c_sucursal;
   ...
   if p_ext = "PO" then
      foreach
         {select co_histsdodias.empresa,co_histsdodias.ccmayor,
                ...
                co_histsdodias.ccsubsub,co_histsdodias.ccssubsub,
                co_histsdodias.ccsssubsub,co_histsdodias.sector,
                co_histsdodias.ciudad,co_histsdodias.sucursal,
                co_histsdodias.moneda,
                dias_acumulado,saldo_acumulado,saldo_acumulado/day(w_fecha),
                0,naturaleza_cta
         into
         ...
         LET c_promedio_anual=(c_saldo_actual+c_saldo_anterior)/(month(w_fecha));}
