CREATE PROCEDURE "informix".arr_edocta1()
...
DEFINE vMotivo       CHAR(1);
 ...
 --SET DEBUG FILE TO "/tmp/arr_edocta1.out";
 --TRACE ON;
	   ...
	   FOREACH
		...
		IF NOT vAnioMes IS NULL THEN
                   IF EXISTS (SELECT aniomes 
		               ...
		               WHERE empresa = "001"
		                 AND cuenta = vCuenta
		                 AND aniomes = vAnioMes1) AND vAnioMes1 = "200712" THEN
		     DELETE FROM  sc_maehis
		      WHERE empresa = "001"
		        AND cuenta = vCuenta
		        AND aniomes = vAnioMes1;
                   END IF;
