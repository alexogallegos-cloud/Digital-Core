CREATE PROCEDURE "informix".ins_consulta_buro(pempresa CHAR(3),psucursal CHAR(3), pusuario CHAR(8), pInstitucion CHAR(2),pnum_solicitud CHAR(20))
...
DEFINE vfecha_bc_sic DATE;
	...
	DEFINE pcadena_es VARCHAR(201);

--SET DEBUG file to '/informix/marcov/ins_consulta_buro.out';
--TRACE ON;
...
LET pe16 = NULL; LET pe17 = NULL; LET pe18 = NULL; LET pe19 = " "; LET tltl = NULL;
...
LET tl42 = NULL; LET iqiq = NULL; LET iq00 = " "; LET iq01 = " "; LET iq02 = " ";
...
LET paso = "Fecha";
SELECT fecha_hoy INTO fecha FROM bdicred:"informix".sd_fechas;
...
IF verrorburo = 'ERRR' and vetiq = '20' AND vvalbloq = 'Y' THEN
		EXECUTE PROCEDURE bdisolic:"informix".sp_actualiza_status_sol (pempresa, 'sistema',pnum_solicitud, 'RT', 'RCB', 'RECHAZO POR CREDITO BLOQUEADO') INTO p_cod_ret;
END IF;
...
IF ( SUBSTR(etiqueta,1,2) = "PA" )	THEN
	INSERT INTO "informix".br_pa  VALUES (pInstitucion,pnum_cliente,respalda_papa,pa00,pa01,pa02,pa03,pa04,pa05,TO_DATE(pa06,"%d%m%Y"),pa07,pa08,pa09,pa10,pa11,TO_DATE(pa12,"%d%m%Y"),vFechaHoy);
...
TO_DATE(pe11,"%d%m%Y"),pe12,pe13,pe14,pe15,TO_DATE(pe16,"%d%m%Y"),TO_DATE(pe17,"%d%m%Y"), TO_DATE(pe18,"%d%m%Y"),pe19,vFechaHoy);
END IF;
LET pcadena = "";
IF entro = "S" THEN
...
LET pcadena = pcadena;
	...
	END IF;	
IF (SUBSTR(etiqueta,1,2) = "ES" AND SUBSTR(pcadena, item_valor + 22 + dif_long_es, 2) = "**" ) THEN
--DSB 06/06/2012
IF entro = "S" THEN LET tamfin = tamfin + item_cadena - regre -1; END IF
SELECT SUBSTR(regreso,tamfin,250) INTO pcadena	FROM "informix".sb_regreso
WHERE institucion = pInstitucion AND num_solicitud = pnum_solicitud;
--DSB 01/02/2012
--LET sEs01 = SUBSTR(pcadena,1,9); LET sEs02 = SUBSTR(pcadena,10,4);LET sEs03 = SUBSTR(pcadena,14,9); LET sEs04 = SUBSTR(pcadena,23,6);
...
--PRUEBA IPCB FICO
--SET DEBUG file to '/RESPALDOS/ipcb/pruebas/ins_consulta_buro_pam.out';   TRACE ON;
...
CALL "informix".ins_buro_credito_aumlincred(pInstitucion,pempresa,pnum_solicitud,pnum_cliente,fecha,vFechaHoy,pcadena,item_cadena,paso,'0') RETURNING s_regreso;
LET cTipoSol = '2';
ELSE
	...
	--IPCB Mayo2015 RQM 09 384-0 FICO SCORE
		...
		IF cTpsol = "C" THEN
			UPDATE bdisolic:"informix".ss_solicitudes SET envio_parametrico = "1" WHERE num_solicitud = csolicitud AND empresa = pempresa;
				-- SI EL VALOR DEL ENVIO PARAMETRICO ES DIFERENTE DE 2 (QUE NO A RECIBIDO RESPUESTA DE COPPEL) Y LA SOLICITUD ES COPPEL. ACTUALIZAMOS EL ESTATUS DE LA SOLICITUD A "EC". RQM 18 023
				IF cStatusSol <> 'EC' THEN
					EXECUTE PROCEDURE bdisolic:"informix".sp_actualiza_status_sol (pempresa,'sistema',csolicitud,'EC','','Solicitud enviada a Evaluación Coppel')
					INTO p_cod_ret;
				END IF;
		ELSE
