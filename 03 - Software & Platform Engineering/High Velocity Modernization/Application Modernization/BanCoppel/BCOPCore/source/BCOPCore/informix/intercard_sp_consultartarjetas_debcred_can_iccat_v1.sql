CREATE PROCEDURE "informix".sp_consultartarjetas_debcred_can_iccat_v1 (pempresa CHAR(3), pnumcte CHAR(9), pNumRegistros SMALLINT)
...
BEGIN
	ON EXCEPTION SET isql_err, isam_err, error_info
		IF isql_err <> 0 THEN
			...
			RETURN ccodret, cnomcliente, cnumtarjeta, ctipotar, cestatustar, cproductotar, cnombreproductotar, cnumcuenta, cstatuscuentadesc, ctitular, ccodestatus, cnumCteTitularCuenta, cnumCteTarjeta;
		END IF;
	END EXCEPTION;
	...
	-- Se llena tabla de paso con cuentas de d̩bito del cliente - Modificado Gabriel
	FOREACH WITH HOLD
    ...
    FOREACH WITH HOLD
		...
		IF cExisteCta = 0 THEN
			...
			FOREACH WITH HOLD
				SELECT cpay.numctebanco, cpay.codprodcta, cpay.desccodprodcta, 'Activo', 'C', cpay.numero_cuenta
