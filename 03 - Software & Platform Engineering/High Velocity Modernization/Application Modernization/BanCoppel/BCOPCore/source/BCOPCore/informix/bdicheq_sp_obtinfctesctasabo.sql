CREATE PROCEDURE "informix".sp_obtinfctesctasabo(pempresa CHAR(3))
...
RETURNING CHAR(5), CHAR(5), CHAR(50), INTEGER, INTEGER;
    ...
    ON EXCEPTION SET vsqlerr, visamerr
        SET debug file to "./sp_obtinfctesctasabo.err";
        TRACE ON;
        IF vsqlerr <> 0 THEN
    ...
    --- SET debug file to "./sp_obtinfctesctasabo.out";
    --- TRACE ON;
    
    SET ISOLATION TO DIRTY READ;
    SET LOCK MODE TO WAIT 3;
    
    CREATE TEMP TABLE sc_infctesctasabon
        (
            cuenta          CHAR(20), 
            numcte          CHAR(20), 
            nombre          CHAR(104),
    ...
    CREATE INDEX idx_infctectaabo ON sc_infctesctasabon(numcte) USING BTREE;
    UPDATE STATISTICS MEDIUM FOR TABLE sc_infctesctasabon;
    ...
    CREATE TEMP TABLE ctas_abonos ( cuenta CHAR(20) NOT NULL ) WITH NO LOG LOCK MODE ROW;
    CREATE INDEX idx_ctaabono ON ctas_abonos(cuenta) USING BTREE;
    UPDATE STATISTICS MEDIUM FOR TABLE ctas_abonos;
      
    INSERT INTO ctas_abonos
    SELECT {+INDEX(bdicheq:sc_movhis_old2 movhis1_old2)} cuenta
      FROM bdicheq:sc_movhis_old2
     WHERE empresa = pempresa
       AND cuenta BETWEEN vmincta AND vmaxcta
       AND fech_alt >= vconsmovhisold2
       AND fech_alt <  vconsmovhisold
       AND cancelad <> 'S'
       AND transacc = '0202'
       AND fech_alt BETWEEN '07/01/2009' AND '06/30/2010';
       
    UPDATE STATISTICS MEDIUM FOR TABLE ctas_abonos;
       
    INSERT INTO ctas_abonos
    SELECT {+INDEX(bdicheq:sc_movhis_old movhis1)} cuenta
      FROM bdicheq:sc_movhis_old
     WHERE empresa = pempresa
       AND cuenta BETWEEN vmincta AND vmaxcta
       AND fech_alt >= vconsmovhisold
       AND fech_alt <  vconsmovhis
       AND cancelad <> 'S'
       AND transacc = '0202'
       AND fech_alt BETWEEN '07/01/2009' AND '06/30/2010';
    ...
    INSERT INTO ctas_abonos
    SELECT {+INDEX(bdicheq:sc_movhis idx_movhisnew4)} cuenta
    ...
    SELECT UNIQUE cuenta
      FROM ctas_abonos
      INTO TEMP tmp_ctas_abonos WITH NO LOG;
    CREATE INDEX idx_ctasabonos ON tmp_ctas_abonos(cuenta) USING BTREE FILLFACTOR 99;
    UPDATE STATISTICS MEDIUM FOR TABLE tmp_ctas_abonos;
    ...
    FOREACH WITH HOLD
        ...
        FOREACH 
            SELECT mae.cuenta, mae.status_cta, mae.fecha_proceso, mae.fecultdep, noc.fecha_alta
              ...
              FROM bdicheq:sc_maechq mae,
                   bdicheq:sc_maenoc noc,
                   tmp_ctas_abonos tmpo
             WHERE mae.num_cte = vnumcte
               AND mae.cuenta = tmpo.cuenta
               AND mae.producto = '1100'
               AND mae.fecultdep <= '06/30/2010'
               AND noc.empresa = mae.empresa
               AND noc.cuenta = mae.cuenta 
            ...
            -- // Inserta datos en tabla sc_infctesctasabon
            INSERT INTO sc_infctesctasabon 
