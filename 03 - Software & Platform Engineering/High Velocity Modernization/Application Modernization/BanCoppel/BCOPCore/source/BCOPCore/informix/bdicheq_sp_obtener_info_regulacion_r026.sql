CREATE PROCEDURE "informix".sp_obtener_info_regulacion_r026(pEmpresa CHAR(3), pTipoCorresponsal CHAR(2))
    RETURNING CHAR(5) as vCodigoRetorno1, VARCHAR(80) as vCodigoRetorno2;
    ...
    --LET RUTA_ORIGEN = '/ifxsif01/_argoz/acum/';
    ...
    --SET DEBUG FILE TO RUTA_ORIGEN||'sp_obtener_info_regulacion_r026.out';
    --TRACE ON;
    
    BEGIN

        ON EXCEPTION SET SQL_ERR, ISAM_ERR, ERROR_INFO        
            SET DEBUG FILE TO RUTA_UNLOAD_RESPALDOS || "excepcion_sp_obtener_info_regulacion_r026.err.out";
            TRACE ON;            
            IF ( SQL_ERR <> 0 ) THEN
        ...
        FOREACH busq_tienda_ptf WITH HOLD FOR
            
            SELECT UNIQUE tienda
                INTO vTienda
            FROM bdicheq:"informix".sc_movs_corresp_mc
                WHERE corresponsal = vTipoCorresponsal
        
            SELECT 
                LIMIT 1 clave
                    INTO vCrPlazaCrTienda
            FROM intercard:"informix".tbl_paso_repaut_r026_establecimiento               
                WHERE corresp_mc <> ''
                    AND corresp_mc = vTienda
            AND periodo_afectacion = (
                                        SELECT 
                                            MAX(periodo_afectacion) 
                                        FROM intercard:"informix".tbl_paso_repaut_r026_establecimiento
                                            WHERE corresp_mc <> ''
                                                AND corresp_mc = vTienda
                                        );
                
             --- tipo: C => Corresponsal / S => Sucursal         
            SELECT 
                LIMIT 1 TRIM(cve_estado)
                    INTO vClaveEstado
            FROM bdinteg:si_ptf
                WHERE tipo = 'C'
                    AND id_ptf = vCrPlazaCrTienda;
                    
            BEGIN;
                UPDATE bdicheq:"informix".sc_movs_corresp_mc
                    SET clave_estado = NVL(vClaveEstado,'00')
                WHERE tienda = vTienda
...
---Fecha de creacion: 12 de febrero del 2020
---Base de datos: bdicheq
---Este proceso corresponde al job 750
----#2 ---Fecha de modificacion: 28 de abril del 2020
----Para consultar los registros de establecimiento se hacen en la base de datos en intercard,
----previamente fue descargada la información con el sp sp_obtener_info_repaut_establecimiento
----EXECUTE PROCEDURE "informix".sp_obtener_info_regulacion_r026 ('001', '2');
;
