Create procedure "informix".sp_proac_cancelcta(pNum_cte CHAR(20),pCuentaEje CHAR(20),pEmpresa CHAR(3),pSucursal CHAR(4),pUsuario CHAR(8), pFolio_suc CHAR(20))
returning char(5),char(20),char(50),Money (14,2);

-- Declaración de variables:
DEFINE iSqlerr,iExiste,iOpcion,cStatus,iCommit,iSecuencia	INTEGER;
DEFINE cCodret,cCodret2,RcCodret,RcCodret2				CHAR(5);
DEFINE cDescripcion   									CHAR(50);
DEFINE cFechaCanc			   							CHAR(10);
DEFINE cCuenta				 							CHAR(20);
DEFINE cTipo_Mov										Char(1);
DEFINE mSdoPROAC,RmNuevoSdo,RmSdoRebajado				MONEY (14,2);			
DEFINE cTransaccCargo, cTransaccAbono 					CHAR(4);
DEFINE RcTransacc										CHAR (4);
DEFINE RdFecha											DATE;
...
begin
   on exception set iSqlerr
      if iSqlerr <> 0 then
         let cCodret = iSqlerr;
         return cCodret,cCuenta,cDescripcion,mSdoPROAC;
      end if;
	end exception;
	--SET DEBUG FILE TO "/tmp/sp_PROAC_CancelCta.out";
	--TRACE ON;
	-- Asignación de variables:
	LET RcCodret = "";
	LET RcCodret2 = "";
	LET RmNuevoSdo = 0.00;
	LET RmSdoRebajado = 0.00;
	LET RcTransacc = "";
	LET RdFecha = '01/01/1950';
	LET iSqlerr = 0;
	LET cCodret = "00000";
	LET iExiste = 0;
	LET cFechaCanc = "";
	...
	--Consulta las transacciones de cargo y abono para el traspaso entre cuentas PROAC	
	Select valor Into cTransaccCargo
	From sc_param
	Where codparam = 'CARGOENTRECUENTAS';
	
	Select valor Into cTransaccAbono
	From sc_param
	Where codparam = 'ABONOENTRECUENTAS';
	--Valida el valor de la transaccion.	
	If 	NVL(cTransaccCargo,"") = "" Or NVL(cTransaccAbono,"") = ""  Then
		LET cCodret = "80000";
		LET cDescripcion = "Transacciones Nulas";
	End If;
	--Valida los parametros de entrada
	If Trim(pCuentaEje) = "" Or pCuentaEje Is Null Then
		LET cCodret = "80001";
		LET cDescripcion = "La Cuenta Eje es Nula";
		return cCodret,cCuenta,cDescripcion,mSdoPROAC;
	End If;
	If Trim(pNum_cte) = "" Or pNum_cte Is Null then
		LET cCodret = "80002";
		LET cDescripcion = "El Cliente es Nulo";
		return cCodret,cCuenta,cDescripcion,mSdoPROAC;
	End If;	
	If Trim(pEmpresa) = "" Or pEmpresa Is Null then
		LET cCodret = "80003";
		LET cDescripcion = "La empresa es Nula";
		return cCodret,cCuenta,cDescripcion,mSdoPROAC;
	End If;	
	If Trim(pSucursal) = "" Or pSucursal Is Null then
		LET cCodret = "80004";
		LET cDescripcion = "La sucursal es Nula";
		return cCodret,cCuenta,cDescripcion,mSdoPROAC;
	End If;	
	If Trim(pUsuario) = "" Or pUsuario Is Null then
		LET cCodret = "80005";
		LET cDescripcion = "El Usuario es Nulo";
		return cCodret,cCuenta,cDescripcion,mSdoPROAC;
	End If;	
	If Trim(pFolio_suc) = "" Or pFolio_suc Is Null then
		LET cCodret = "80006";
		LET cDescripcion = "El Folio es Nulo";
		return cCodret,cCuenta,cDescripcion,mSdoPROAC;
	End If;	
	--Realiza la consulta a la cuenta activa PROAC  según los parametros de entrada correspondiente.
	Select NVL(cuenta,""),status_cta Into cCuenta,cStatus
	From sc_proac 
	Where num_cte = pNum_cte 
	And cta_eje = pCuentaEje
	And status_cta in ('1','3') ;	
	--Valida la cuenta PROAC que no sea nula o espacio en blanco.
	If Trim(cCuenta) = "" Or cCuenta Is Null Then
		LET cCodret = "80007";
		LET cDescripcion = "No existe una cuenta activa para este cliente";
		return cCodret,cCuenta,cDescripcion,mSdoPROAC;
	End If;
	--Consulta el maestro de saldos , trae el  stautus y saldo, según la cuenta proac.
	Select nvl(sdo_actual,'0.00') Into mSdoPROAC
	From sc_maechq
	Where cuenta = cCuenta;	
	--Consulta la fecha_hoy
	Select fecha_hoy Into cFechaCanc
	From sc_fechas 
	Where empresa = '001';
	--Valida el status de la cuenta PROAC  si esta bloqueada. Se complementa con un trigger donde se va actualizar segun el status de la cuenta PROAC
	--Segun el status de la cuenta Eje, en caso de estar bloqueada se busca el tipo de bloqueo de la cuenta eje si no es un bloqueo total o un bloqueo de cargos continua.
	If cStatus = 3 Then
		LET cCodret = "80008";
		LET cCuenta = pCuentaEje;
		LET cDescripcion = "Imposible de Cancelar la Cuenta";	
		return cCodret,cCuenta,cDescripcion,mSdoPROAC;
	End if;
	 --Valida el si el saldo de la cuenta es cero
	If mSdoPROAC = '0.00' Then
	 Select Max(secuencia) Into iSecuencia
	 From sc_proac Where  cuenta  = cCuenta ;
			--Cancela la cuenta en la proac
			Update sc_proac 
			Set status_cta = '2',fecha_canc = cFechaCanc
			Where num_cte = pNum_cte 
			And cuenta = cCuenta
			And secuencia = iSecuencia;
			--Cancela la cuenta en el maestro de cheques
			Update sc_maechq
			Set status_cta = '2'
			Where num_cte = pNum_cte 
			And cuenta = cCuenta;
			LET cDescripcion = "Cancelada Correctamente";
	Else
	 Select Max(secuencia) Into iSecuencia
	 From sc_proac Where  cuenta  = cCuenta ;
	 ...
	 --Realiza el cargo del saldo de la cuenta PROAC
	 Call cargo_ref(pEmpresa,pSucursal,pUsuario,cTransaccCargo,'0000',pFolio_suc,cCuenta,0,mSdoPROAC,'01','Cargo x Traspaso','','') returning RcCodret,RcTransacc,RdFecha,RmNuevoSdo,RmSdoRebajado ;
	 
	 
	  If RcCodret = "000" Then
	  
		--Obtengo folio
        Call sp_generafolionomina (pUsuario) Returning cCodRet, cNumeroFolio;
		--Realiza el abono del saldo de a la cuenta eje
		Call abono_ref(pEmpresa,pSucursal,pUsuario,cTransaccAbono,'0250',cNumeroFolio,pCuentaEje,0,mSdoPROAC,mSdoPROAC,0,0,0,'01','Abono x Traspaso','0','') returning RcCodret2;
		If RcCodret2 = "000" Then
			--Cancela la cuenta en la proac		
			Update sc_proac 
			Set status_cta = '2',fecha_canc = cFechaCanc
			Where num_cte = pNum_cte 
			...
			LET cDescripcion = "Cancelada Correctamente";
		 Else
			call reversion (pEmpresa, pSucursal, pUsuario,pFolio_suc, "C") Returning cCodRet;
		End if;	
	...
	'DESCRIPCION : Genera la Cancelacion del cliente PROAC donde graba en las tablas sc_maechq, sc_maenoc, sc_proac',
	'FECHA       : Febrero de 2009',
	'VERSION     : 200902',
	'BD          : BDICHEQ';

CREATE PROCEDURE "informix".sp_primdiames_indicadores()
RETURNING
	CHAR(6),
	CHAR(80)
