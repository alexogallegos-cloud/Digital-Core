CREATE PROCEDURE "informix".sp_obtiene_productos_cjunk(	pEmpresa 		CHAR(3),	-- Cód. de empresa
														pSucursal 		CHAR(4),	-- Cód de Sucursal
														pEjecutivo 		CHAR(8),    -- Ejecutivo
														pPuesto_local 	CHAR(2),  	-- Puesto local del ejecutivo
														pNumcte 		CHAR(20), 	-- Núm. de Cliente
														pCoppel 		CHAR (1), 	-- Indica si el cte ya tiene un crédito coppel efectuado en tienda.
																					-- (1:Tiene crédito coppel, 0: No tiene crédito coppel)
														pPrecalCoppel 	CHAR(1), 	--Indica el resultado de la precalificación del cte en coppel
																					-- (1: Mala eficiencia, 0: Buena eficiencia)
														pPrecalBco 		CHAR(1),    -- Indica el resultado de la precalificación del cte en banco
																					-- (1:Mala eficiencia del cte en Banco, 0: Buena eficiencia del cte)
														pDigiDomicilio 	CHAR(1), 	-- Indica si el cte digitalizó un comprobante de domicilio.
														pIdentificacion CHAR(1),	-- Indica si el cte digitalizó una identificacion oficial
														pOfertaProdCred CHAR(1))	--Indica si oferta o no productos de credito)
	RETURNING 	CHAR(6), -- Código de retorno
				CHAR(4), -- Cód de producto a ofrecer al cte
				CHAR(1), -- Tipo de solicitud correspondiente al producto a ofrecer
				CHAR(40),-- Descripción del producto
				CHAR(2); -- Prioridad del producto al momento de mostrarse

	--  CONTROL DE CAMBIOS
	-------------------------------------------------------------------------------------
	-- Autor: Viridiana Osobampo.
	-- Modificación: Se modifica para contemplar el producto Préstamo Personal (6300).
	-- Petición: RQM 10 108 Préstamo Personal.
	-- Fecha: 06-09-2009.
	-------------------------------------------------------------------------------------
	-------------------------------------------------------------------------------------
	-- Autor: Viridiana Osobampo.
	-- Modificación: Se modifica para que devuelva un código de retorno cuando no existen
	--              productos a ofrecer para el cliente, y se agrega una condición por
	--              producto coppel para que los clientes que no digitalizaron comprobante
	--              de domicilio solo puedan solicitar tarjeta de crédito coppel cuando
	--              no tenga ya una en trámite o aperturada.
	-- Petición: RQM 10 108 Préstamo Personal.
	-- Fecha: 09-10-2009.
	-------------------------------------------------------------------------------------
	-- Autor: Viridiana Osobampo.
	-- Modificación: Se eliminan validaciones para códigos de producto y en su lugar
	...
	-------------------------------------------------------------------------------------
	-- Autor: Viridiana Osobampo.
	-- Modificación: Se modifica para optimizar la consulta a la tabla bdidigital:dg_expediente_img
	-- Petición: Alta Única Paso 4
	-- Fecha: 28-01-2010.
	-------------------------------------------------------------------------------------
	-- Autor: Viridiana Osobampo.
	-- Modificación: Se modifica para validar que los productos a ofrecer al cliente
	--               sean los productos que la sucursal está ofreciendo en ese momento.
	-- Petición: RQM 10 108 Préstamo Personal.
	-- Fecha: 16-02-2010.
	-------------------------------------------------------------------------------------
	-- Autor: Viridiana Osobampo.
	-- Modificación: Se modifica para que los productos a ofertar se validen por el puesto del ejecutivo,
	-- 				 mismo que se recibe como parámetro.
	-- Petición: Alta Única paso 04
	-- Fecha: 22-06-2010
	-------------------------------------------------------------------------------
	-- FECHA: 2011/06/08
	...
	--Modificación: Se modifica para no ofertar prestamo personal cuando ya se encuente una solicitud de rechazada del mismo
	--Petición: RQM 09 306
	--Fecha: 12-11-2012
	-------------------------------------------------------------------------------------
	--Modificó: Jesús Manuel Aguilar Heredia
	--Modificación: Se modifica para ofertar TDC a clientes que cuenten con un credito cancelado por peticion del cliente.
	--Petición: RQI 27 004
	--Fecha: 11-12-2012
	-------------------------------------------------------------------------------------
	--Modificó: Guadalupe Payan
	--Modificación: Se realiza homologación con fuentes modificados por bancoppel: se agrega validacion para no permitir levantar una nueva solicitud de Préstamo cuando el cliente cuente	--              con una solicitud de Préstamo personal o de TDC Bancoppel Visa en estatus ?RT?. Tambien se agrega validacion para no permitir levantar una nueva solicitud de Préstamo
	--              cuando el cliente cuente con mas de 5 Préstamos Personales Activos.
	...
	--Petición: Contrato-MttoAltaUnica_04.doc
	--Fecha: 15-02-2013
	-------------------------------------------------------------------------------------
	--Modificó: Clemente Angulo
	--Modificación: Se modifica para que se pueda ofertar una cuenta de nomina a un cliente que cuente con una cuenta de nomina cancelada.
	--Petición: Incidencias productivas AU Paso 5.5.doc
	--Fecha: 22-04-2013
	--Modificó: Carlos Aguirre Vega
	--Descripción: Se agrega status EC - "Evaluacion Coppel" en la consulta de productos
	--Peticion: RQM 18 023 - EstatusSolicitudCoppelEC
	--Fecha de modificación: 22-04-2013
	------------------------------------------------------------------------------------- 
	--Modificó: Carlos Aguirre Vega
	--Descripción: Se modifica el nombre de sp_obtiene_productos_cjunk_02 a sp_obtiene_productos_cjunk
	--Peticion: RQM 18 023 - EstatusSolicitudCoppelEC
	--Fecha de modificación: 07-05-2013
	-------------------------------------------------------------------------------------
	--Modificó: Eduardo Lopez
	--Modificación: Se modifica para que reciba un parametro mas y no se oferten productos de credito en base a la comparacion de huellas
	--Petición: RQI 63 033 Evaluación de Comparación de Huella OFI - version 1.1.doc
	--Fecha: 18-09-2013 
	------------------------------------------------------------------------------------- 
	...
	BEGIN
		...
		IF cDigiDom = '1' THEN
			...
			END FOREACH
		ELSE
			-- Obtiene los productos de captación con los que actualmente cuenta el cliente, se modifica para que se inserte la fecha de alta de la cuenta de captación
			FOREACH
			...
			-- Obtiene los productos de crédito con los que cuenta el cliente
			FOREACH
			...
			-- Verifica si el cliente presenta un crédito en reestructura
			FOREACH
				SELECT num_producto
				INTO ProdActual
			...
			-- FMV 22-FEB-11 Verifica si el cliente presenta de Crédito Activo, previo a la Solicitud
			FOREACH
            ...
            IF (inumpPrestamos >= iPrestamosActivos) THEN
				...
				INSERT INTO "informix".ss_bitacora_precal (empresa,fecha,producto,sucursal,nombre,ejecutivo,mensaje,causa_solicitud)
					VALUES (pEmpresa,today,'6300',pSucursal,cnomcte,pEjecutivo,'El Cliente cuenta con el tope máximo de Préstamos Personales permitidos','RFP');
			END IF;
			--JMAH
		...
		IF NVL(cCteProsp,'') <> '' THEN
			...
			IF NVL(cClave,'') = 'R' THEN
				...
				FOREACH
					...
					IF EXISTS(SELECT 1 FROM "informix".ss_productos_ofrecer 
