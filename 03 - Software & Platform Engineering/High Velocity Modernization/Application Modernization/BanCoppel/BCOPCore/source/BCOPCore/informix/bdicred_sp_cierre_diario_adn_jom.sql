CREATE PROCEDURE "informix".sp_cierre_diario_adn_jom(pEmpresa CHAR(3))
RETURNING
   CHAR(6)        AS Cod_Ret,
   CHAR(80)       AS Mens_Ret;
...
--  SET DEBUG FILE TO '/home/sysmonitor/sp_cierre_diario_adn.out';
--  TRACE ON;
-- *******************************************************
...
SELECT a.fecha_hoy, a.prox_fecha, a.ult_dia_mes, 
  ...
  FROM "informix".sd_fechas a--solo para puebas
...
END IF;
--*************************************************************************
--   FIN DE PROVISIÓN POR REGISTROS DE MOVTO EN FIN MES
--*************************************************************************
-- *******************************************************
--  PROVISIÓN                                            *
-- *******************************************************
        ...
        --ELSE
			...
			END IF; --IF cIdProc3 = "M" AND cStatusCred='BT' THEN
                ...
                UPDATE "informix".sd_maecred
                   SET status_cred = (CASE WHEN cStatusCred = "AA" and dTraspCap>0 THEN "BA" ELSE status_cred END),
                       fecha_pago_cap = dtFechaProxCuota,
                       fecha_pago_int = dtFechaProxCuota
                 WHERE num_credito = cNumCredito
                   AND empresa     = cEmpresa;
---Ini cas Validación solicitada por operaciones para que no cobre el int devengado
---de un dia cuando la reestructura llega a su Última mensualidad
                    IF dCapTrasNoVen = 0 AND dSdoCapital = 0 AND dSdoInt > 0 THEN
                       LET dSdoInt = 0;
                    END IF;
---Fin cas Validación solicitada por operaciones
END IF;
...
-- JOM 11/04/2013 Se cambia traspado a periodos INI
-- Se realiza el traspado en la mensualidad 4 para considerar 90 o más días en transitorio            
           /*SELECT COUNT(a.num_credito), nvl(min(fecha_cuota), date(1))
            ...
            IF  (cStatusCred='BA' AND iNumVdosaux > 3 and dFechacuotamin <> date(1)) 
...
-- RQM 07 085 calificación credisoluciones
