CREATE PROCEDURE "informix".sp_actsitcredreestructurado()
RETURNING CHAR(6)  AS COD_RET, 
	...
	LET sEntro             = 0;
		...
		--SET DEBUG FILE TO '/dbexportb/carlos/asignacion/reestructura.out';
		--TRACE ON;	
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
			IF iSqlErr <> 0 THEN
				UPDATE "informix".se_bitacoramasiva SET estatus_proc = '0' WHERE nombre_proceso = 'CRB';
				LET cCodRet = iSqlErr;
		...
		ELSE
			...
			IF cBorraAmbasFechas = '1' THEN
				...
				UPDATE "informix".se_bitacoramasiva  SET inicia_proc = NULL, termina_proc = NULL WHERE nombre_proceso = 'CRB';
			
				UPDATE "informix".se_bitacoramasiva SET inicia_proc = dtFecha_Hora  WHERE nombre_proceso = 'CRB';
			END IF;
			
			UPDATE "informix".se_bitacoramasiva SET estatus_proc = '1' WHERE nombre_proceso = 'CRB';
			
			
		END IF;	
		
	FOREACH WITH HOLD
		--SE SELECCIONAN LOS CLIENTES QUE TENGAN REGISTRADO EL PRODUCTO '6011' Y CUYO ESTATUS SEA DIFERENTE A 'FF'
		SELECT {+INDEX(bdicred:"informix".sd_maecredcrd idx_maecrex)} mae.numcte
		INTO cNumCte
		FROM bdicred:"informix".sd_maecredcrd mae
		LEFT OUTER JOIN "informix".se_sitespmasivo sit ON(sit.empresa = '001' AND sit.numcte = mae.numcte AND sit.fecha = sit.fecha AND sit.etapa3 = '1' )
		WHERE mae.empresa = '001'
		AND mae.numcte = mae.numcte
		AND mae.num_producto = '6011'
		AND mae.status_cred <> 'FF'
		...
		--SI EL CONTADOR ESTA EN 1 QUIERE DECIR QUE ES LA PRIMERA TRANSACCION POR LO CUAL SE ABRE EL BEGIN
			...
			IF sBandera > 0 THEN
				INSERT INTO "informix".se_sitespmasivo (empresa,numcte,situacion,causa, etapa1, etapa2, etapa3, etapa4, etapa5, procesado, fecha) VALUES ('001',cNumCte,cSituacion,sCausa,'0','0','1','0','0','P',CURRENT);
	...
	END FOREACH;
		...
		UPDATE "informix".se_bitacoramasiva SET termina_proc = dtFecha_Hora , estatus_proc = '2' WHERE nombre_proceso = 'CRB';
