CREATE PROCEDURE "informix".sp_consultacuentasdestinoservicios2_web(p_NumCte CHAR(20), p_CvePago CHAR(2), p_Registros SMALLINT, p_CveBanco CHAR(3))
RETURNING
     CHAR(5), ---cod_ret
	 CHAR(20), ---cuenta
	 CHAR(100), ---nombre
	 CHAR(50), ---banco
	 CHAR(2), ---compania celular
...
BEGIN
   ...
   ON EXCEPTION
    ...
    IF (p_NumCte <> "" AND p_NumCte IS NOT NULL) AND (p_CvePago <> "" AND p_CvePago IS NOT NULL)  THEN
		...
		LET vVariable = (SELECT COUNT(ct.cuenta) FROM bdiprog:"informix".pp_ctasterceros ct, pp_cuentapago cp WHERE ct.num_cte = p_NumCte AND ct.cve_cuenta = cp.cve_cuenta);
