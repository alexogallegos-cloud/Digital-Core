CREATE PROCEDURE "informix".sp_activaserviciosdomi (p_sCuenta CHAR(20), p_sRfc CHAR(18), p_sNumCte CHAR(20), p_sCveCanal CHAR(2), p_fImpMax DECIMAL(16,2), 
                                        p_sCveSucursal CHAR(4), p_iNumRechazos INTEGER, p_sCveStatus CHAR(2), p_sUserStatus CHAR(8), p_sCveCausa CHAR(2), 
	...
	BEGIN
		...
		--Valida parametros de entrada
		 IF NVL(p_sCuenta,'') = '' OR NVL(p_sRfc,'') = '' OR NVL(p_sNumCte,'') = '' OR NVL(p_sCveCanal,'') = '' OR NVL(p_fImpMax,'') = '' OR NVL(p_iNumRechazos,'') = '' 
		 OR NVL(p_sCveSucursal,'')  = '' OR NVL(p_sCveStatus,'') = '' OR NVL(p_sUserStatus,'') = '' OR NVL(p_sCveCausa,'') = '' OR NVL(p_sUserInsert,'') = '' THEN
			LET sCodret="00110"; --Problema con los parametros
			RETURN sCodret;
		END IF;
		
		--Valida si el cliente  tiene dado de alta   el servicio, que se envía en los parametros para darse de alta.
		IF EXISTS (SELECT 1 FROM dom_autorizaciones WHERE cuenta = p_sCuenta AND rfc = p_sRfc AND num_cte = p_sNumCte) THEN
		   LET sCodret='00120'; --El cliente ya esta dado de alta en el servicio de domiciliacion.
		   RETURN sCodret;
		END IF
		
		SELECT fecha_hoy INTO dFechaHoy FROM bdinteg:si_fechas WHERE empresa = '001';
		SELECT valor INTO v_sCveSucursal FROM bdidomi:dom_parametros WHERE cod_param = '07';
		
		--Almacena el sevicio a domiciliar al cliente.
		INSERT INTO dom_autorizaciones (cuenta, rfc, num_cte, cve_canal, imp_maximo, num_rechazos, cve_sucursal,
		cve_estatus, fecha_estatus, user_estatus, cve_causa, user_insert, fecha_insert)
		VALUES (p_sCuenta, p_sRfc,p_sNumCte, p_sCveCanal, p_fImpMax, p_iNumRechazos, v_sCveSucursal, p_sCveStatus,
    	dFechaHoy, p_sUserStatus, p_sCveCausa, p_sUserInsert, dFechaInsert);
...
'FECHA      : 13/08/2009',
'VERSION    : 20090813.1200, 20091007.0943',
'BD         : BDIDOMI',
'CASO DE USO: Caso de uso asociado PCU-Domciliacion\CU-0001-ActivarServiciosDomi-SPL',
'MODIFCADO  : Fabiola Corrales 07/Oct/2009. Se modifica para que tome la fecha estatus del sistema (si_fechas) en lugar del current del server.',
'MODIFCADO  : Fabiola Corrales 26/Oct/2009. Se modifica para que tome el parametro de la sucursal';

CREATE PROCEDURE "informix".sp_consultactasdomi(p_sNumCliente CHAR(11), p_sRegistros SMALLINT)

	RETURNING CHAR(5),  -- cod retorno
			  CHAR(20), -- cuenta
			  CHAR(5),  --Producto de las CUENTAs
			  CHAR(20), --Cuenta Clabe
			  CHAR(16); --Numero de Tarjeta
	...
	DEFINE v_sProd			CHAR(4);
	DEFINE v_sNumTar		CHAR(16);
	...
	--***************************************************************************************
	--Set debug file to '/tmp/sp_consultactasdomi.out';
	--trace on;
	--***************************************************************************************
	...
	--Manejo de errorres
        ON EXCEPTION SET sql_err
	        IF sql_err <> 0 THEN
	            ...
	            RETURN v_sCodRet, v_sCuenta, v_sProducto, v_sClabe, v_sNumTar WITH RESUME;
	        END IF;
	    END EXCEPTION;
		
		IF NVL(p_sNumCliente, '') = '' OR NVL(p_sRegistros, '') = '' THEN
			LET v_sCodRet = '00110';
			RETURN v_sCodRet,'', '', '', '';
		END IF
	
	   --Obtiene los productos validos para domiciliacion de servicios
	   SELECT valor INTO v_sValor FROM bdidomi:dom_parametros WHERE cod_param = v_sCodParam;

		WHILE v_iPosicion < LENGTH(TRIM(v_sValor))

	        LET v_sProd = SUBSTR(v_sValor, v_iPosicion,4); --Extrae de la cadena de los productos válidos producto por producto

			FOREACH--Obtiene las cuentas  que tiene el cliente
			   SELECT SKIP p_sRegistros a.cuenta, a.producto, a.cuenta_clabe, b.num_tarjeta
			   INTO v_sCuenta, v_sProducto, v_sClabe, v_sNumTar
			   FROM bdicheq:sc_maechq a, bdicheq:sc_tarjeta b
			   WHERE a.empresa = '001' AND a.num_cte = p_sNumCliente
			   AND b.numcte = p_sNumCliente			   
			   AND a.producto = v_sProd
			   AND a.cuenta = b.cuenta
			   ORDER BY cuenta
			   			   
			   RETURN v_sCodRet, v_sCuenta, v_sProducto, v_sClabe, v_sNumTar WITH RESUME;  --Resultados a regresar
			END FOREACH;
...
DOCUMENT
'AUTOR      : Cristian Valentina Aguilar',
'DESCRIPCION: Se encarga de Obtener las cuentas del cliente que son aptas para el servicio domiciliación. Realiza una consulta a bdidomi:parametros',
'			  para obtener los productos validos para domiciliacion, y realiza una consulta a bdicheq:sc_maechq donde obtiene las cuentas. ',
'FECHA      : 19/07/2009',
'VERSION    : 200907',
'BD         : BDIDOMI',
'CASO DE USO: Caso de uso asociado PCU-Domciliacion\CU-0003-ConsultarCtasDomi-SPL',
'MODIFCADO  : Se modifica para agregar el SKIP';

CREATE PROCEDURE "informix".sp_consultadevolucionesdomi(p_sFechaInicial CHAR(10), p_sFechaFinal CHAR (10), p_sNumCte CHAR(9), p_iSecuencia SMALLINT)

RETURNING	CHAR(5)  AS Codret,
			CHAR(8)  AS FechaAplica,
			CHAR(60) AS RazonSocial,
			CHAR(40) AS cRefLeyenda,
			CHAR(20) AS Cuenta,
			CHAR(15) AS Importe,
	...
	DEFINE sql_err  		INTEGER;
	DEFINE v_sCuenta 		CHAR(20);
	DEFINE v_sRfc			CHAR(18);
	DEFINE v_sRazonSocial 	CHAR(60);
	DEFINE v_sClabe			CHAR(20);
	DEFINE v_sImporte		CHAR(15);
	DEFINE v_sRefLeyenda  	CHAR(40);
	DEFINE v_sFechaAplica 	CHAR(8);
	DEFINE v_sImporteMax  	CHAR(19);
	DEFINE v_sMotivoDev		CHAR(60);
	...
	BEGIN
		...
		ON EXCEPTION SET sql_err
			IF sql_err <> 0 THEN
				LET v_sCodret = sql_err;
				RETURN v_sCodret, v_sFechaAplica, v_sRazonSocial, v_sRefLeyenda, v_sCuenta, v_sImporte, v_sImporteMax, v_sMotivoDev WITH RESUME;
			END IF;
		END EXCEPTION;

		IF NVL(p_sFechaInicial, '') = '' OR NVL(p_sFechaFinal, '') = '' OR NVL(p_sNumCte, '') = '' OR NVL(p_iSecuencia, '') = '' THEN
			LET v_sCodret = '00110';
			RETURN v_sCodret,'','','','','','','';
		END IF
		...
		FOREACH
			SELECT a.cuenta, a.rfc, b.razon_social, a.imp_maximo
			INTO v_sCuenta, v_sRfc, v_sRazonSocial, v_sImporteMax
			FROM bdidomi:dom_autorizaciones a, bdidomi:dom_cat_servicios b
			WHERE a.num_cte = p_sNumCte
			AND a.rfc = b.rfc
			ORDER BY a.cuenta

		    --SELECT num_tarjeta INTO v_sNumTarjeta FROM bdicheq:sc_tarjeta WHERE cuenta = v_sCuenta AND numcte = p_sNumCte;
			SELECT cuenta_clabe INTO v_sClabe FROM bdicheq:sc_maechq WHERE cuenta = v_sCuenta;

			--LET v_sNumTarjeta = LPAD(TRIM(v_sNumTarjeta),20,'0');
			LET v_sClabe = LPAD(TRIM(v_sClabe),20,'0');

			FOREACH
				SELECT SKIP p_iSecuencia a.importe, a.fecha_aplica, a.ref_leyenda, b.descripcion
				INTO v_sImporte, v_sFechaAplica, v_sRefLeyenda, v_sMotivoDev
				FROM bdidomi:dom_cce_detalle a
				INNER JOIN dom_cat_devoluciones b ON a.motivo_dev = b.motivo_dev
				WHERE a.fecha_presentacion BETWEEN p_sFechaInicial AND p_sFechaFinal
				AND a.rfc_ord = v_sRfc
				AND (a.num_cta_rec IN (SELECT LPAD(TRIM(num_tarjeta),20,'0') FROM bdicheq:sc_tarjeta WHERE cuenta = v_sCuenta AND numcte = p_sNumCte)
				OR a.num_cta_rec = v_sClabe)
				AND a.cve_estatus = "02" AND a.cod_operacion = "30"
...
DOCUMENT
'AUTOR      : Noel Gerardo',
'DESCRIPCION: Consulta la devoluciones',
'FECHA      : 20/07/2009',
'VERSION    : 20091009.1801',
'BD         : BDIDOMI',
'CASO DE USO: Caso de uso asociado PCU-Domciliacion\CU-0004-ConsultarDevolucionesDomi-SPL',
'MODIFICADO : Fabiola Corrales 09/Oct/2009. Se modifica para que tomo el SKIP';

CREATE PROCEDURE "informix".sp_consultaparamdomi(p_sCodParam CHAR(2))
RETURNING CHAR(5) AS retorno, CHAR(50) AS descripcion, CHAR(100) AS valor;
 ...
 --****************************************************************
 -- SET DEBUG FILE TO "/tmp/sp_consultaparamdomi.out";       	--* 
 -- TRACE ON;                                            		--*
 --****************************************************************
	...
	BEGIN
		...
		FOREACH
			...
			WHERE cod_param = NVL(p_sCodParam, cod_param)
			
			LET v_sCodRet = '00000';
		
			RETURN v_sCodRet, v_sDescripcion, v_sCveValor WITH RESUME;
		END FOREACH;
	END
END PROCEDURE
DOCUMENT
'AUTOR      : Noel Gerardo ',
'DESCRIPCION: Obtiene un parametro de la tabla dom_parametros',
'FECHA      : 07/ago/2009',
'VERSION    : 20090807.1200',
'BD         : BDIDOMI',
'CASO DE USO: Caso de uso asociado PCU-Domciliacion\CU-0006-ConsultarParamDomi-SPL';

CREATE PROCEDURE "informix".sp_consultarcatreversos(p_sClaveReverso CHAR(2), p_iSecuencia SMALLINT)
RETURNING CHAR(5) AS retorno, CHAR(2) AS clavereverso, CHAR(50) AS descripcion;

	DEFINE v_sCodRet 			CHAR(5);
	DEFINE v_sClaveReverso 		CHAR(2);
	DEFINE v_sDescripcion  		CHAR(50);	
	DEFINE sql_err 				INTEGER;

 --****************************************************************
 -- SET DEBUG FILE TO "/tmp/sp_consultarcatreversos.out";     	--* 
 -- TRACE ON;                                            		--*
 --****************************************************************
	...
	BEGIN
		...
		IF NVL(p_sClaveReverso, '') = '' THEN
			LET p_sClaveReverso = NULL;
		END IF

		IF NVL(p_iSecuencia, '') = '' THEN
			LET v_sCodRet = '00001'; --Error en los parametros de entrada
			RETURN v_sCodRet,'','';
		END IF
		
		FOREACH
			SELECT SKIP p_iSecuencia cve_reverso, descripcion
			INTO v_sClaveReverso, v_sDescripcion
			FROM bdidomi:dom_cat_reversos
			WHERE cve_reverso = NVL(p_sClaveReverso, cve_reverso)
			ORDER BY cve_reverso
			
			LET v_sCodRet = '00000';
		
			RETURN v_sCodRet, v_sClaveReverso, v_sDescripcion WITH RESUME;
		END FOREACH;
	END
END PROCEDURE
DOCUMENT
'AUTOR      : Fabiola Corrales ',
'DESCRIPCION: Realiza la consulta del catalogo de reversos',
'FECHA      : 09/Oct/2009',
'VERSION    : 20091009.1221',
'BD         : BDIDOMI',
'CASO DE USO: Caso de uso asociado PCU-Domciliacion\CU-0012-ConsultarCatReversos-SPL';

CREATE PROCEDURE "informix".sp_consultastatusdomi(p_cve_estatus CHAR(2))
RETURNING CHAR(5) AS retorno, CHAR(2) AS cve_estatus, CHAR(20) AS descripcion;

	DEFINE v_scve_estatus 		CHAR(2);
 ...
 --****************************************************************
  -- Debug del Procedure                                  		--*
 -- SET DEBUG FILE TO "/tmp/sp_consultastatusdomi.out";       	--* 
 -- TRACE ON;                                            		--*
 --****************************************************************
	...
	BEGIN
		...
		IF NVL(p_cve_estatus, '') = '' THEN
			LET p_cve_estatus = NULL;
		END IF

		FOREACH
            SELECT cve_estatus, descripcion
			INTO v_scve_estatus, v_sdescripcion
            FROM bdidomi:dom_cat_estatusaut
            WHERE cve_estatus = NVL(p_cve_estatus,cve_estatus)
			
			LET v_scodret = '00000';
            
			RETURN v_scodret,v_scve_estatus, v_sdescripcion WITH RESUME;
		END FOREACH;
	END
END PROCEDURE
DOCUMENT
'AUTOR      : Noel Gerardo',
'DESCRIPCION: Obtiene todos los estatus de la tabla dom_cat_estatusaut',
'FECHA      : 21/07/2009',
'VERSION    : 20090721.1200',
'BD         : BDIDOMI',
'CASO DE USO: Caso de uso asociado PCU-Domciliacion\CU-0008-ConsultarStatusDomi-SPL';

CREATE PROCEDURE "informix".sp_domi_consultardatoscliente(pCuenta CHAR(20),pNumcte CHAR(20),pNumTarjeta CHAR(20))
	RETURNING CHAR(5),CHAR(20), CHAR(200), CHAR(18),CHAR(60),CHAR(10),CHAR(2),CHAR(1),CHAR(1),CHAR(1);
...
Begin
    ...
    ON EXCEPTION SET cSqlerr
        IF cSqlerr <> 0 THEN
            Let cCodret = cSqlerr;
            RETURN cCodret,cNumCte,cNombreCte,cRFC,cRazon_social,cFechaNac,cTipper,cEsFisica,cServicioDomi,cAutorizadoCteDomi;
        END IF;
    END EXCEPTION;
	IF pCuenta = '' AND pNumTarjeta = '' AND pNumcte = '' THEN
		LET cCodret = '02700';
		RETURN cCodret,cNumCte,cNombreCte,cRFC,cRazon_social,cFechaNac,cTipper,cEsFisica,cServicioDomi,cAutorizadoCteDomi;
	END IF
	...
	IF  pCuenta  <> ''  THEN
        ...
        SELECT num_cte INTO cNumCte FROM bdicheq:sc_maechq 
	...
	IF pNumTarjeta <> '' THEN
		...
		SELECT numcte INTO cNumCte FROM bdicheq:sc_tarjeta 
		WHERE empresa = '001' 
	...
	SELECT TRIM(nombre1)||' ' || TRIM(nombre2 )||' ' || TRIM(apell_paterno) ||' ' || TRIM(apell_materno),rfc,razon_social,tpo_persona ,numcte
	INTO 	cNombreCte,cRFC,cRazon_social,cTipper,cNumCte
	...
	SELECT es_fisica INTO cEsFisica FROM bdinteg:si_tipper WHERE tpo_persona = cTipper;
	
	IF cEsFisica = 'S' THEN
		Select fecha_nac INTO cFechaNac FROM bdinteg:si_ctepf WHERE numcte = cNumCte;
	END IF;
	
	IF cEsFisica = 'N' THEN
		Select fecha_constitct INTO cFechaNac FROM bdinteg:si_ctepm WHERE numcte = cNumCte;
	END IF;
	
	SELECT COUNT(*) INTO iExiste FROM bdidomi:dom_cat_servicios WHERE num_cte = cNumCte;
		IF iExiste IS NULL OR iExiste = 0 THEN
			LET cServicioDomi = 'N';
		ELSE
			LET cServicioDomi = 'S';
		END IF;
	...
	SELECT COUNT(*) INTO iExiste FROM bdidomi:dom_autorizaciones WHERE num_cte = cNumCte AND cve_estatus = '01';
		IF iExiste IS NULL OR iExiste = 0 THEN
			LET cAutorizadoCteDomi = 'N';
		ELSE	
			LET cAutorizadoCteDomi = 'S';
		END IF;
		LET iExiste = 0;
...
'DESCRIPCION:Consulta la informacion del cliente, nombre, rfc, razon_social, fecha_nac, tipo de persona ',
'FECHA : 10 de Agosto de 2009',
'BD    : BDIDOMI',
'VERSION: 20090810.1845';
