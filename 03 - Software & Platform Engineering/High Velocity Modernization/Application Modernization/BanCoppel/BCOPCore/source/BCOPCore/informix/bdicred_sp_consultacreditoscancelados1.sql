CREATE PROCEDURE "informix".sp_consultacreditoscancelados1()
RETURNING CHAR(5);
    ...
    /* DEFINICION VARIABLES FOREACH */
    DEFINE num_credito_1        CHAR(12);
    DEFINE num_tarjeta_1        CHAR(20);
    DEFINE numcte_1             CHAR(9);
    DEFINE num_producto_1       CHAR(45);
    DEFINE fecha_apertura_1     CHAR(10);
    DEFINE fecha_nac_1          CHAR(10);
    DEFINE siglas_1             CHAR(4);
    DEFINE municipiozona_1      CHAR(40);
    DEFINE fecha_1              CHAR(10);
    DEFINE motivo_1             CHAR(50);
    DEFINE folio_cancelacion_1  CHAR(16);
    DEFINE folio_1              CHAR(16);
    DEFINE sucursal_1           CHAR(4);
    DEFINE semaforo_1           CHAR(1);
    DEFINE hora_ini_1           CHAR(10);
    DEFINE hora_fin_1           CHAR(10);

    LET cod_ret = "00000";
    LET cNombreArchivo = "celula_retencion_f1_"||LPAD(MONTH(today)-1,2,'0')||LPAD(YEAR(today),4,'0')||".csv";
    ...
    BEGIN
        ...
        SELECT RPAD(sdbitret.num_credito,12,' ') AS num_credito
              ,RPAD(NVL(sdtar.num_tarjeta,''),16,' ') AS num_tarjeta
              ,RPAD(sdbitret.numcte,9,' ') AS numcte
              ,RPAD((DECODE(NVL(sddef.num_producto,''),'','',sddef.num_producto||' '||NVL(sddef.nombre_prod,''))),45,' ') AS num_producto
              ,RPAD(NVL(sdmae.fecha_apertura,''),10,' ') AS fecha_apertura
              ,RPAD(NVL(sicte.fecha_nac,''),10,' ') AS fecha_nac
              ,RPAD(NVL(siest.siglas,''),4,' ') AS siglas
              ,RPAD(NVL(sicat.municipiozona,''),40,' ') AS municipiozona
              ,RPAD(sdbitret.fecha,10,' ') AS fecha
              ,RPAD(NVL(sdbitret.motivo,''),50,' ') AS motivo
              ,RPAD(NVL(sdcrecan.folio_cancelacion,''),16,' ') AS folio_cancelacion
              ,RPAD(NVL(sdcteret.folio,''),16,' ') AS folio
              ,RPAD(NVL(sdbitret.sucursal,''),4,' ') AS sucursal
              ,NVL(sdcteret.semaforo,' ') AS semaforo
              ,RPAD(NVL(sdbitret.hora_ini,''),10,' ') AS hora_ini
              ,RPAD(NVL(sdbitret.hora_fin,''),10,' ') AS hora_fin
          FROM bdicred:sd_bitacora_retencion sdbitret
          LEFT OUTER JOIN bdicred:sd_maecred sdmae ON (sdmae.numcte = sdbitret.numcte
           AND sdmae.num_credito = sdbitret.num_credito)
          LEFT OUTER JOIN bdicred:sd_ctes_retencion sdcteret ON (sdcteret.numcte = sdmae.numcte
           AND sdcteret.num_credito = sdmae.num_credito)
          LEFT OUTER JOIN bdicred:sd_definicion sddef ON (sddef.empresa = sdmae.empresa
           AND sddef.num_producto = sdmae.num_producto)
          LEFT OUTER JOIN bdicred:sd_tarjeta sdtar ON (sdtar.numcte = sdmae.numcte
           AND sdtar.empresa = sdmae.empresa
           AND sdtar.num_credito = sdmae.num_credito
           AND sdtar.tipo_tarjeta = 'T'
           AND sdtar.secuencia = (
               SELECT MAX(sdtarj.secuencia)
                 FROM bdicred:sd_tarjeta sdtarj
                WHERE sdtarj.numcte = sdmae.numcte
                  AND sdtarj.empresa = sdmae.empresa
                  AND sdtarj.num_credito = sdmae.num_credito
                  AND sdtarj.tipo_tarjeta = 'T'))
          LEFT OUTER JOIN bdicred:sd_maecred sdcre ON (sdcre.empresa = sdmae.empresa
           AND sdcre.numcte = sdmae.numcte
           AND sdcre.num_credito = sdtar.num_credito)
          LEFT OUTER JOIN bdicred:sd_cred_can sdcrecan ON (sdcrecan.num_credito = sdmae.num_credito
           AND sdcrecan.num_cte = sdmae.numcte
           AND sdcrecan.empresa = sdmae.empresa
           AND sdcrecan.fecha_can = sdbitret.fecha)
          LEFT OUTER JOIN bdinteg:si_ctepf sicte ON (sicte.numcte = sdmae.numcte
           AND sicte.empresa = sdmae.empresa)
          LEFT OUTER JOIN bdinteg:si_direcciones_actual sidiract ON (sidiract.numcte = sdmae.numcte
           AND sidiract.tipo_dir = 1)
          LEFT OUTER JOIN bdinteg:si_catzonas sicat ON (sicat.numerociudad = sidiract.numerociudad
           AND sicat.numerocolonia = sidiract.numerocolonia)
          LEFT OUTER JOIN bdinteg:si_ptf sisuc ON (sisuc.id_ptf = sdbitret.sucursal
           AND sisuc.tipo = 'S')
          LEFT OUTER JOIN bdinteg:si_estados siest ON ( siest.estado = sisuc.cve_estado)
          LEFT OUTER JOIN bdinteg:si_ciudades siciud ON (siciud.estado = sisuc.cve_estado
           AND siciud.ciudad = sisuc.cve_ciudad)
         WHERE sdbitret.fecha BETWEEN dF_inicioMes AND dF_finMes
           AND sdbitret.num_credito <> ''
           AND sdtar.num_tarjeta    <> ''
          INTO TEMP rep_temp;

        /* INICIA PROCESO DE CREACION DE ARCHIVO */

        LET cSQL = 'echo "num_credito|num_tarjeta|numcte|num_producto|fecha_apertura|fecha_nac|siglas|municipiozona'
                  ||'|fecha|motivo|folio_cancelacion|folio|sucursal|semaforo|hora_ini|hora_fin|" > '||TRIM(cRutaEnvio)||TRIM(cNombreArchivo);
        SYSTEM cSQL;

        FOREACH
        SELECT num_credito,num_tarjeta,numcte,num_producto,fecha_apertura,fecha_nac,siglas,municipiozona,fecha
              ,motivo,folio_cancelacion,folio,sucursal,semaforo,hora_ini,hora_fin
          INTO num_credito_1,num_tarjeta_1,numcte_1,num_producto_1,fecha_apertura_1,fecha_nac_1,siglas_1,municipiozona_1
              ,fecha_1,motivo_1,folio_cancelacion_1,folio_1,sucursal_1,semaforo_1,hora_ini_1,hora_fin_1
          FROM rep_temp
         ORDER BY fecha,numcte

            LET cSQL = 'echo "'||TRIM(NVL(num_credito_1,''))||'|'||TRIM(NVL(num_tarjeta_1,''))||'|'||TRIM(NVL(numcte_1,''))
                               ||'|'||TRIM(NVL(num_producto_1,''))||'|'||TRIM(NVL(fecha_apertura_1,''))||'|'||TRIM(NVL(fecha_nac_1,''))
                               ||'|'||TRIM(NVL(siglas_1,''))||'|'||TRIM(NVL(municipiozona_1,''))||'|'||TRIM(NVL(fecha_1,''))
                               ||'|'||TRIM(NVL(motivo_1,''))||'|'||TRIM(NVL(folio_cancelacion_1,''))||'|'||TRIM(NVL(folio_1,''))
                               ||'|'||TRIM(NVL(sucursal_1,''))||'|'||TRIM(NVL(semaforo_1,''))||'|'||TRIM(NVL(hora_ini_1,''))
                               ||'|'||TRIM(NVL(hora_fin_1,''))||'|'|| '" >> ' ||TRIM(cRutaEnvio)||TRIM(cNombreArchivo);
            SYSTEM cSQL;
...
'FECHA : 18/07/2022',
'SCRIPT DE PROCEDIMIENTO ALMACENADO "sp_consultacreditoscancelados"',
'SOLICITA: ',
'BD: bdicred',
'MODIFICADO: Se crea /resplogifx/archivoscartera/altaunica/envios/celula_retencion_f1_yyyymmdd.csv';

CREATE PROCEDURE "informix".reversion_exp(o_empresa CHAR(3),
                           O_SUCursal CHAR(4),
                           O_USUARIO  char(8),
                           o_folio    CHAR(16),
                           o_tiporev  CHAR(1))
...
SET LOCK MODE TO WAIT 3;
 ...
 --SET DEBUG FILE TO "/tmp/reversion2109.out";
 --TRACE ON;
...
SELECT {+INDEX(sd_fechas idx_sdfechas)} fecha_hoy
  ...
  WHERE empresa = "001";
   ...
   SELECT UNIQUE(cod_tipcred), num_credito
	...
	--AAME INC 27 136 31/10/2019 No realizar reverso si el crÃ©dito esta cancelado (FF)  
	SELECT status_cred 
	INTO cstatus_cred
   ...
   SELECT count(*)  INTO cant_transacc
    ...
    IF sp_num_credisolucion IS NULL THEN
	   IF cant_transacc = 0 THEN
		  -- LA TRANSACCIÃN A REVERSAR ES UN PAGO Y SE VALIDA QUE NO TENGA UNA OPERACIÃN POSTERIOR AL PAGO A REVERSAR.
		...
		ELSE
		  -- LA TRANSACCIÃN A REVERSAR ES UN CARGO Y SE VALIDA QUE DESPUES DE LA TRANSACCIÃN DE CARGO NO SE ENCUENTRE UN MOVIMIENTO DE PAGO
		  SELECT COUNT(*) INTO sp_disp_desp_pag
			FROM "informix".sd_movdia
		   WHERE empresa = '001'
			 and num_credito = vnum_credito
  ...
  -- Reversa el Movimiento de disposicion de tarjeta
   IF vCodTipCred = "03" THEN
		...
		--AAME INC 27 108 Se elimina IF EXITS a peticiÃ³n de Base de Datos
   ...
   --Bloque de modificaciÃ³n para tomar en cuenta reestructuras y prestamos personal para reversos
  IF (vCodTipCred IS NULL AND vnum_credito IS NULL) OR (sp_num_credisolucion IS NOT NULL) THEN
	...
	End if;
		...
		END IF
END IF;
--FIN DEL BLOQUE DE MODIFICACIÃN
...
CREATE PROCEDURE "informix".sp_ce_validacuentacaptacion (pNumCta char(20), pNumCteInst char(20))
        returning  char(5),char(20),char(20),char(120),integer ;


    ------------------------------------------------------------------------------>
    -- Objetivo:     Se creo el sp para regresar nombre y numero de cliente de una cuenta.
    -- Autor:        Faviola Martinez Juarez
    -- Fecha:        Junio 2014
    -- Validación de cuenta eje
	------------------------------------------------------------------------------>
    -- CONTROL DE CAMBIOS
	------------------------------------------------------------------------------>
    -- Petición:     RQM 20 069 - 3 - Adendum -Ajuste en la Carga Masiva de Créditos en Orión
    -- Autor:        98769022 Miguel Alejandro Sanchez Mojica
    -- Fecha:        23/06/2022 
    -- Modificación: Se modifico el sp para cuentas nostro y regresar tambien la razón social.
    -- 
    ------------------------------------------------------------------------------>
    -- Ejemplo de ejecucion:
    -- 
    -- EXECUTE FUNCTION bdicred:sp_ce_validacuentacaptacion ("27000000154","001637238"); -- cuenta eje
    -- EXECUTE FUNCTION bdicred:sp_ce_validacuentacaptacion ("27000000138","001637238"); -- cuenta Nostro
    ...
    BEGIN
        ON EXCEPTION SET cSQL_ERR
            ...
            RETURN cCodret,cDescripcion, cNumCte, cNombre,cCodNostro;
        END EXCEPTION;

        --> Validacion si es cuenta nostro
        SELECT COUNT(*) INTO cCodNostro FROM bdicred:sd_ce_ctas_nostro cn WHERE cn.cuenta_nostro = pNumCta; 

        FOREACH
            --> Extrae Información
            SELECT status_cta, mae.num_cte,
               NVL(TRIM(si_cliente.razon_social),"") ||' ' || NVL(TRIM(si_cliente.nombre1),"") ||' ' || NVL(TRIM(si_cliente.nombre2),"") ||' '|| NVL(TRIM(si_cliente.apell_paterno),"") ||' '|| NVL(TRIM(si_cliente.apell_materno),"")
            INTO
        ...
        --> Valida si los datos que trae la consulta son correctos
        IF cNumCte IS NULL OR cNumCte = '' THEN
            LET  cCodret= '11111';
            LET cDescripcion = 'Validar cuenta eje';
            LET cNumCte = '0';
        END IF;

        --> Validacion si es cuenta nostro y asigna el coódigo de retorno
        IF cCodNostro > 0 THEN
            LET cCodNostro = 1;
        END IF;

        --> Retorna Codigo de retorno, Status, Numero cliente institucion, nombre o razon social y el indicador si es cuenta nostro
        RETURN cCodret,cDescripcion, cNumCte, cNombre, cCodNostro WITH resume ; 
    END;
END PROCEDURE;