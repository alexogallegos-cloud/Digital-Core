CREATE PROCEDURE "informix".sp_reportenegocio_pba()
...
--  TRACE ON;
 ...
 ------------- control de errores------
    ...
    END EXCEPTION WITH RESUME; */
        ...
        IF EXISTS ( SELECT dbsname, tabname FROM  sysmaster:SysTabNames  WHERE tabid > 99 and tabname = 'pasoprincipal' AND dbsname= 'intercard') THEN
            DROP TABLE intercard:pasoprincipal;
        END IF;*/
		
		SET ISOLATION TO DIRTY READ ;
        IF EXISTS ( SELECT dbsname, tabname FROM  sysmaster:SysTabNames  WHERE tabid > 99 and tabname = 'paso2' AND dbsname= 'intercard') THEN
            DROP TABLE intercard:paso2;
        END IF;

		
		SET ISOLATION TO DIRTY READ ;
        IF EXISTS ( SELECT dbsname, tabname FROM  sysmaster:SysTabNames  WHERE tabid > 99 and tabname = 'paso3' AND dbsname= 'intercard') THEN
            DROP TABLE intercard:paso3;
        END IF;
		
		SET ISOLATION TO DIRTY READ ;
        IF EXISTS ( SELECT dbsname, tabname FROM  sysmaster:SysTabNames  WHERE tabid > 99 and tabname = 'paso_nego' AND dbsname= 'intercard') THEN
            DROP TABLE intercard:paso_nego;
        END IF;

			SET ISOLATION TO DIRTY READ ;
        IF EXISTS ( SELECT dbsname, tabname FROM  sysmaster:SysTabNames  WHERE tabid > 99 and tabname = 'paso_estab' AND dbsname= 'intercard') THEN
