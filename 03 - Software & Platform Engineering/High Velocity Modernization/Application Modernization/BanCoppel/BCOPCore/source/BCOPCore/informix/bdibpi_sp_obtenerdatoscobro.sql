CREATE PROCEDURE "informix".sp_obtenerdatoscobro(pNumCte CHAR(9), pCuenta CHAR(12), pMonto DECIMAL(12,2))
   RETURNING CHAR(5);
...
BEGIN
	...
	--SET DEBUG FILE TO "/informix/gaby/admtoken-lib-15/bdibpi/sp_obtenerDatosCobro.out";
	--TRACE ON;
		
	IF NVL(pNumCte, '') = '' OR NVL(pCuenta, '') = '' OR NVL(pMonto, '') = '' THEN
		LET cCodRet = '00001';
	ELSE
		
		SELECT sc.folio_suc, sc.fech_alt, si.tpo_persona
                    INTO pfolio_suc, vFecha,  cTipoPersona 
                    FROM bdicheq:"informix".sc_movdia as sc,  bdinteg:"informix".si_cliente as si 
                    WHERE sc.cuenta =pCuenta and si.numcte = pNumCte
                    AND transacc IN ('3006','3299') AND TRIM(cancelad) <> 'S';
	
	
		--Actualiza los datos del cobro del reenvio de token
		UPDATE bdibpi: "informix".tkn_solcobranza SET f_cobro=vFecha, cuenta=pCuenta, monto_tot=pMonto, t_persona=cTipoPersona,  folio_suc=pfolio_suc
		WHERE numcte = pNumCte AND id_status = '180';
...
'FECHA: 01/05/2016',
'BD: bdibpi',
'Objetivo: Se modifica sp para que incluya las transacciones para personas morales 3299';
