CREATE PROCEDURE "informix".sp_actualizainventarj(pEmpresa CHAR(3), pSucursal CHAR(4),pNumTarjeta CHAR(16),pLote INT, pOpcion CHAR(1))
	--DATOS A REGRESAR
	RETURNING  
	CHAR(5) AS cCodRet
--========== DEFINIR VARIABLES =======
...
BEGIN
	...
	-- SET DEBUG FILE TO "/respaldosbd/Judith/sp_actualizainventarj.out";
	--TRACE ON;
	IF pEmpresa = '' OR pSucursal = '' THEN
			LET cCodRet = '00001'; 
	ELSE
		IF pOpcion = '1' Then -- Modifica campo banderaregistro == 0
			UPDATE intercard:"informix".control_inventario SET tt_banderaregistro = '0' 
			WHERE tt_empresa = pEmpresa AND tt_sucursal = LPAD(pSucursal,5,'0');
		ELSE -- Modifica campo banderaregistro == 1
			IF pNumTarjeta = '' OR pLote = '' THEN
				LET cCodRet = '00001'; 
			ELSE
				UPDATE intercard:"informix".control_inventario SET tt_banderaregistro = '1' 
				WHERE tt_empresa = pEmpresa AND tt_numerolote = pLote AND tt_numerotarjeta = pNumTarjeta AND tt_sucursal = LPAD(pSucursal,5,'0');
...
'Autor: 98516451 - Jesus Mercado',
'BD: intercard',
'Solicita:	Abraham Narvaez',
'Fecha: 21/03/2018',
'Descripcion: Se crea procedimiento para modificar el campo banderaregistro de la tabla control_inventario de la base de datos intercard';

CREATE PROCEDURE  "informix".sp_consultarangotarjetasbanco(	iLote INTEGER,cTarjetaini CHAR(16),cTarjetafin CHAR(16),iCuantos SMALLINT)
RETURNING CHAR (5) AS codRet, CHAR(16) AS numTarjeta;
	...
	BEGIN
		...
		IF ( iLote = 0 AND cTarjetaini = '' AND cTarjetafin = '') THEN
		...
		ELSE
			...
			FOREACH
				...
				WHERE numerolote = iLote AND numtarjeta BETWEEN cTarjetaini AND cTarjetafin
...
END PROCEDURE
DOCUMENT
'Autor: Ismael Lizarraga',
'Folio: 144 - ControlRegistrTarjetasSucursal',
'Fecha: 07-12-2016',
'Modificación: Se crea procedimiento para consultar rango de tarjetas BanCoppel recibidas',
'Sustento: 144.1 RQM 06 220 Control y Registro de Tarjetas en Sucursal-Contrato',
'Solicita: Abraham Narvaez',
'Base de datos: Intercard';

CREATE PROCEDURE "informix".sp_consultatarjetasdan(cEmpresa CHAR(3), cTarjeta CHAR(16), cTipoTar CHAR(1), cTipoProceso CHAR(1))
...
DEFINE cEstatusTarjBanCoppel	CHAR(3);
DEFINE cEstatusTarjCoppel		CHAR(1);

--INICIALIZACION DE LAS VARIABLES
LET cCodRet         		= '00000';
LET iSqlErr         		= 0;
LET cEstatusTarjBanCoppel	= '';
LET cEstatusTarjCoppel		= '';
...
BEGIN
	...
	--SET DEBUG FILE TO '/home/sysifx/Alexis/144/sp_consultatarjetasdan.out';
	--TRACE ON;	
	
	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	
	IF NVL(cEmpresa,'') <> '' AND NVL(cTarjeta,'') <> '' AND NVL(cTipoTar,'') <> '' THEN
	  IF cTipoProceso = '1' THEN
	  
		IF cTipoTar = '1' OR cTipoTar = '2' THEN
		
			--------Tarjeta BanCoppel
			IF cTipoTar = '1' THEN
			
				SELECT codstatustarjeta
				INTO cEstatusTarjBanCoppel
				FROM intercard:"informix".tarjeta
				...
				IF dbinfo("sqlca.sqlerrd2") = 0 THEN
					LET cCodRet = '00004';	
				ELSE					
					IF cEstatusTarjBanCoppel = 'DAN' THEN
						LET cCodRet = '00000';
			...
			------Tarjeta Coppel
			IF cTipoTar = '2' THEN
				SELECT estatustarjeta
				INTO cEstatusTarjCoppel
		...
		IF cTipoTar = '1' OR cTipoTar = '2' THEN
			...
			IF cTipoTar = '1' THEN
				...
				IF dbinfo("sqlca.sqlerrd2") = 0 THEN
					LET cCodRet = '00004';	
				ELSE					
					IF cEstatusTarjBanCoppel = 'INA' THEN
						LET cCodRet = '00000';
...
DOCUMENT
'DESCRIPCION: Se realizar un procedimiento para la consulta de tarjetas Coppel y BanCoppel dañadas',
