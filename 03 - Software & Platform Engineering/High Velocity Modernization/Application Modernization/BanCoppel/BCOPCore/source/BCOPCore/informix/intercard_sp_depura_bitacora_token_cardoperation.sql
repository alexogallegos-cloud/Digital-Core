CREATE PROCEDURE "informix".sp_depura_bitacora_token_cardoperation()
...
LET filePath                         = '';
    ...
    BEGIN
		...
		--SET DEBUG FILE TO "/home/c90313380/sp_depura_bitacora_token_cardoperation.out";
        --TRACE ON;
        ...
        SELECT COUNT(*)
           INTO conteo
        FROM bitacora_token_cardoperation
            WHERE fecha_insert < fechaInicioMes;
            
        IF conteo = 0 THEN
            LET codigo_retorno = "00002";
        ELSE
            LET nombreArchivo ='Respaldo_bitacora_token_cardoperation_' || anio|| mes || primerDiaMes ;
            LET filePath='"/RESPALDOSNEW/Tokenizacion/file_temp/'|| nombreArchivo ||'.unl" ';

            LET query = 'SELECT * FROM bitacora_token_cardoperation WHERE fecha_insert < '||'"'|| fechaInicioMes ||'"';           
           ...
           DELETE FROM bitacora_token_cardoperation
             WHERE fecha_insert < fechaInicioMes;
