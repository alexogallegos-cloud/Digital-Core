CREATE PROCEDURE "informix".sp_dicta_consultalertacomph2(pModo SMALLINT,
														 pSkyp INTEGER,
														 pStatus CHAR (1),
														 pNumctenvo CHAR (20),
														 pUsuario CHAR (20),
...
RETURNING 	CHAR (6) 	AS Cod_Ret,
	...
	LET cDescripcionOrigen = '';
		...
		--SET DEBUG FILE TO '/tmp/ivan/sp_dic_consultalertacomph2.out';
		--TRACE ON;
...
BEGIN
		ON EXCEPTION SET iSqlErr
			IF iSqlErr <> 0 THEN
				LET cCodRet = iSqlErr;
				RETURN TRIM(cCodRet),NVL(cStatus,''),NVL(cSucursal,''),NVL(cNumcte,''),NVL(sMatches,0),NVL(dtFecha,DATE(1)),NVL(cOrigen,''), NVL(iTotalReg, 0),NVL(cHoraAlerta, '00:00'),NVL(cAnalistaFraudes,''), NVL(cDescripcionOrigen,'');
			END IF;
		END EXCEPTION;
		...
		IF pModo = 1 THEN
			...
			FOREACH
				...
				SELECT {+INDEX("informix".si_catorigenhuellas idx_si_catorigenhuellas)} SKIP pRegistros FIRST pRecuperacion TRIM(ba.numcte),TRIM(ba.status_alerta),TRIM(ba.sucursal), ba.num_huellas,ba.fecha_insert,ba.origen,LPAD(SUBSTR(ba.fecha_insert,12,2),2,'0'),LPAD(SUBSTR(ba.fecha_insert,15,2),2,'0'),ba.user_analista, co.desc_origen
				...
				INNER JOIN si_catorigenhuellas co ON ba.origen = co.cod_origen
				WHERE ba.user_analista = pUsuario
		...
		ELIF pModo = 2 THEN
			...
			IF pStatus = '5' OR pStatus = '3'THEN
				...
				SELECT {+INDEX("informix".si_catorigenhuellas idx_si_catorigenhuellas)}{+INDEX("informix".si_bitacora_comparaciones idxsi_bitacora_comparaciones_numcte)}{+INDEX("informix".si_catorigenhuellas 4282_9173)} first 1 TRIM(bc.status_alerta),TRIM(bc.sucursal),TRIM(bc.numcte),bc.num_huellas,bc.fecha_insert,bc.origen,LPAD(SUBSTR(bc.fecha_insert,12,2),2,'0'),LPAD(SUBSTR(bc.fecha_insert,15,2),2,'0'),bc.analista_fraudes, co.desc_origen
				--SELECT  {+AVOID_FULL(bdinteg:si_bitacora_comparaciones )}   first 1 TRIM(bc.status_alerta),TRIM(bc.sucursal),TRIM(bc.numcte),bc.num_huellas,bc.fecha_insert,bc.origen,LPAD(SUBSTR(bc.fecha_insert,12,2),2,'0'),LPAD(SUBSTR(bc.fecha_insert,15,2),2,'0'),bc.analista_fraudes, co.desc_origen
				INTO cStatus,cSucursal,cNumcte,sMatches,dtFecha,cOrigen,cHora,cMinuto,cAnalistaFraudes, cDescripcionOrigen
				FROM 'informix'.si_bitacora_comparaciones bc
				INNER JOIN si_catorigenhuellas co ON bc.origen = co.cod_origen
				WHERE bc.numcte = pNumctenvo
				AND bc.status_alerta = pStatus;
			...
			ELSE 
				...
				--SELECT {+AVOID_FULL(bdinteg:si_bitacora_comparaciones )}  first 1 TRIM(bc.status_alerta),TRIM(bc.sucursal),TRIM(bc.numcte),bc.num_huellas,bc.fecha_insert,bc.origen,LPAD(SUBSTR(bc.fecha_insert,12,2),2,'0'),LPAD(SUBSTR(bc.fecha_insert,15,2),2,'0'),bc.analista_fraudes, co.desc_origen
					...
					INNER JOIN si_catorigenhuellas co ON bc.origen = co.cod_origen
					WHERE bc.numcte = pNumctenvo
					AND bc.status_alerta <> '4' AND bc.status_alerta <> '5' 
                    AND bc.fecha_insert = (select max(fecha_insert)	FROM 'informix'.si_bitacora_comparaciones  where numcte = pNumctenvo); 
