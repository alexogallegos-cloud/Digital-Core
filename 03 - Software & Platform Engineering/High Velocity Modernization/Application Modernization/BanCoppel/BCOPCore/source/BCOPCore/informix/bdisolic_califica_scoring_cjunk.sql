CREATE PROCEDURE "informix".califica_scoring_cjunk( o_empresa       CHAR(3),
													o_numsol   	 	CHAR(20),
	...
	-- Descripcion: Se agrega cambio al estatus de RT a CN y causas RCB/RGC a CCB/CGC.
...
--IPCB junio2017//RECHAZO POR CREDITO BLOQUEADO RCB 
	...
	-- RQM 09 654 unificado anticipo de nÃÂ³mina y prestamo directo de nomina (BRM) --
    DEFINE iConteoMotor        INTEGER;
    DEFINE iConteoMotor_pp     INTEGER;
	
	-- *                        ASIGNACION DE VARIABLES                           *
	-- ****************************************************************************
    --SET DEBUG FILE TO "/informix/Oscar/Web/califica_scoring_cjunk_precal_opt"||TRIM(o_numsol)||".out";
	--TRACE ON;
...
--IPCB junio2017//RECHAZO POR CREDITO BLOQUEADO RCB 
	...
	LET vFechaSolCoppel		= "";
	LET vFechaSolBanco		= "";
	LET vConsAleat				 = 0;	LET vflagvig				 = 0;	LET vFalloSIC				 = 0;
   ...
   END EXCEPTION WITH RESUME;*/
	   ...
	   --SET DEBUG FILE TO '/informix/Fperaza/NuevosCambios/canales/traces/califica_scoring_cjunk'||o_numsol||'.out';
		...
		--SET DEBUG FILE TO '/ifxsif01/KSOV/Trace/califica_scoring_cjunk'||o_numsol||'.out';
		--TRACE ON;
		
		-- SET DEBUG FILE TO "/home/e90233448/califica_scoring_cjunk"||TRIM(o_numsol)||".out";
		 --TRACE ON;
		
		--SET DEBUG FILE TO "/home/e10000315/califica/1908/trace/califica_scoring_cjunk"||TRIM(o_numsol)||".out";
		--TRACE ON;
		
		--SET DEBUG FILE TO "/home/c90077639/Flujo_SIC/Spl_2/califica_scoring_cjunk"||TRIM(o_numsol)||".out";
		--TRACE ON;
		
		--SET DEBUG file to '/home/e99806151/califica_scoring_cjunk_SIC.out';  
	  -- TRACE ON;

	    /*KSOV--Se agrega parametro de tiempo*/
		SELECT VALOR
		INTO VTiempo
		...
		sol.numcte,sol.tipo_solicitud, sol.num_producto,sol.sucursal,mov.status,mov.folio_movil ,sol.status_solicitud,sol.fecha_insert,canal_sol-- Viridiana. Obtiene el producto
		...
		--IPCB 12Abr21 INI --Credito solicita inactivar validacion de segundo producto para meter Requerimiento. Con esto no se prende la bandera																																		  
		...
		IF exists (select * from bdisolic: ss_canales_solic where canal_solic = vCanalSol and flag_canal_dud = 'A') then 
			LET sFlagCanalWeb = 1;
			--validar que la solicitud venga por el canal digital(autosolicitudes) capturada con cobranza y no por el canal movil
			if vCanalSol = 2 and v_sucursal <> '8503' then 
				LET sFlagCanalWeb = 0;
			end IF 
		END IF;
	--ini jesus RM 09-279-2
		...
		END IF;
			...
			--RQI 21 282-7 SE COMENTAN POR CAMBIO DE LOGICA MARCADO MIXTAS Y UNICAS, POR LAPSO DE TIEMPO 			
			--KSOV
			/*		
			IF NVL(cNumSolicitud, '') = '' THEN
			...
			END IF;
			*/
		--RQI 21 282-7 SE COMENTAN POR CAMBIO DE LOGICA MARCADO MIXTAS Y UNICAS, POR LAPSO DE TIEMPO -KSOV
		...
		-- ********************************
		--RQI 21 282-7 SE COMENTAN POR CAMBIO DE LOGICA MARCADO MIXTAS Y UNICAS, POR LAPSO DE TIEMPO -KSOV EL UPDATE SE SUBE AL IF DE LA VALIDACION DE MARCAJE
			
		UPDATE "informix".ss_resum_scor_fin
		SET ingreso_mensual = o_ingreso,
		  tp_ingreso = o_tpingreso,
		  periodo_ingreso = o_periodicidad,
		  tipo_movimiento = cTipoMov,--RQM 09 279-2 /*SE COMENTA KSOV*/
		  num_solicitud_ref  = cNumSolicitud --RQM 09 279-2 
		  --grupo = ptipogrupo mahr-cnbv
		WHERE empresa = o_empresa
		AND num_solicitud = o_numsol;
      ...
      END IF;
		...
		IF  (sFlagCanalWeb = 1 and cStatusPrev  <> '' and vValidaSPTienda <> 'F' ) THEN		
...
-- Se agrega validacion para filtrar clientes "Z" FIN 
		...
		--Inicio: RQM 09 606 consulta sic aleatorio
		...
		--Validar si existe una solicitud previa que aun este vigente
		...
		IF cNumSolSIC IS NULL THEN  --Valida que no hay sol vigente, por lo que asigna la SIC en base al Aleatorio
			...
			IF cNumSolSIC IS NULL THEN --Valida que no tiene solicitud base, por lo que asigna la SIC en base al Aleatorio
				...
				IF scod_ret <> '00000' THEN
						RETURN scod_ret; -- ocurrio un error al ejecutar el  procedimiento sp_random_sics
				END IF;
				

				--Seleccionar la institucion en base al numero aleatorio
		...
		--Fin RQM 09 606 consulta sic aleatorio
	   ...
	   --JMAH Se realiza homologacion con ajustes paso 5
		...
		-- Validacion y filtrado de Producto, sucursal y Tipo de Solicitud  RQM 09 654 (BRM) --
		SELECT COUNT(numsucursal) INTO iConteoMotor
        FROM bdicred:"informix".sd_sucursales_motor WHERE producto = sNum_producto;

        SELECT COUNT(numsucursal) INTO iConteoMotor_pp 
        FROM bdicred:"informix".sd_sucursales_motor_pp WHERE producto = sNum_producto;
		
		IF sNum_producto = "7800" THEN
		   --se valida el canal solicitud para que no se inserte en la tabla ss_enviossolicitudesmotor_adn
			IF cCanalSol != "8" AND EXISTS (SELECT
					   numproducto
					   FROM bdicred:"informix".sd_productos_motor_pp
					   WHERE numproducto = sNum_producto)
					   AND iConteoMotor_pp > 0
					   AND EXISTS (SELECT
								   numsucursal
								   FROM bdicred:"informix".sd_sucursales_motor_pp
								   WHERE numsucursal = v_sucursal
								   AND producto = sNum_producto) THEN
			   
				INSERT INTO  bdisolic:"informix".ss_enviossolicitudesmotor_adn
				(empresa, num_solicitud, num_cte, status_consumo, fecha_insert, status_respuesta)
				VALUES  (o_empresa,o_numsol, v_cliente , 0, current, '');
		  
           --se valida el canal solicitud para que no se inserte en la tabla ss_enviossolicitudesmotor_adn		  
			ELIF cCanalSol != "8" AND EXISTS (SELECT
						 numproducto
						 FROM bdicred:"informix".sd_productos_motor_pp
						 WHERE numproducto = sNum_producto)
						 AND iConteoMotor_pp = 0 THEN
						 --AND NOT EXISTS (SELECT producto FROM bdicred:"informix".sd_sucursales_motor_pp
						 --WHERE producto = sNum_producto AND numsucursal = v_sucursal) THEN
			   
				INSERT INTO  bdisolic:"informix".ss_enviossolicitudesmotor_adn
				(empresa, num_solicitud, num_cte, status_consumo, fecha_insert, status_respuesta)
				VALUES  (o_empresa,o_numsol, v_cliente , 0, current, '');
				
			ELIF ( cOrigenSol = '1')  THEN
...
--IPCB junio2017//RECHAZO POR CREDITO BLOQUEADO RCB --se extrae el nuevo campo causa_rt, para validar los rechazos			
			...
			IF cNumSolSIC IS NULL THEN 
					...
					AND  fecha_sic IS NULL
					AND institucion = status_consul);
			--Inicio: RQM 09 606 bandera solicitud vigente
			ELSE
				Let vflagvig = 1;
				--Fin: RQM 09 606 bandera solicitud vigente
			END IF;
			--//***FIN_KSOV***//								
...
--IPCB junio2017 //RECHAZO POR CREDITO BLOQUEADO RCB
				...
				ELSE
					...
					VALUES(o_empresa,v_cliente,o_numsol,o_numsol,institucion_sic,v_hoy,NULL,vConsAleat,vflagvig,vFalloSIC);
					EXECUTE PROCEDURE bdiburo:"informix".burocred(o_empresa, "0000", USER, o_numsol, 0) INTO scod_ret;
				END IF;
			ELSE
				IF dtFechaSic IS NULL THEN
				 IF fgst_prosp <> 'F' AND sFlagCanalWeb <> 1 THEN	 --IPCB 04sep20, no inserta para 0 - 6500
					INSERT INTO "informix".ss_solicitudes_sic
						(empresa,numcte,num_solicitud,num_solicitud_sic,institucion,fecha_insert,fecha_sic,consul_aleatoria,flagvig,FalloSIC)
				...
				--	IF cNumSolSIC <> o_numsol THEN --RGH
        		 IF  fgst_prosp <> 'F' AND sFlagCanalWeb <> 1 THEN	--IPCB 04sep20, no inserta para 0 - 6500
					INSERT INTO "informix".ss_solicitudes_sic
						(empresa,numcte,num_solicitud,num_solicitud_sic,institucion,fecha_insert,fecha_sic,causa_rt,consul_aleatoria,flagvig,FalloSIC)
					VALUES(o_empresa,v_cliente,o_numsol,cNumSolSIC,cConsultaSic,v_hoy,dtFechaSic,ccausaRT,vConsAleat,vflagvig,vFalloSIC);
				--	END IF; 
				END IF;
								...
								IF ( cOrigenSol = '1')  THEN
									...
									IF sFlagCanalWeb = 1 THEN
										IF cflujo_cc = '1' THEN --fjpr
											...
											-------------------------------------------------------------------------------------------------------------
                                                        IF  v_tpsol = 'T' AND sNum_producto ='6001' THEN
											            ...
											            ------------------------------REEVALUACION-------------------------------------------------------------
											              IF cCalifica = 'X' AND v_tpsol = 'T' AND sNum_producto ='6001' THEN
					                                    ...
					                                    -----------------------------FIN REEVALUACION-------------------------------------------------------------
	                                             		    ...
	                                             		    WHERE A.empresa = o_empresa AND A.num_solicitud = o_numsol;															
															--INI**KSOV
													            ...
													            END IF;
					                                             IF cCalifica = '0' THEN
					                                                ...
					                                                END IF;
						                                                ...
						                                                IF cSolMixta <> '' THEN
						                                                   --SI LA SOLICITUD BANCO ES MIXTA LE CAMBIA PARAMETRICO 5
						                                                ...
						                                                ELSE	
						                                                   ...
						                                                   UPDATE bdisolic: "informix".ss_solicitudes SET envio_parametrico = '6' WHERE num_solicitud = o_numsol AND empresa = o_empresa AND envio_parametrico IS NULL;                     
									
						                                                END IF;
					   
					                                             ELSE
						                                            --SI NO ES BUEN COMPORTAMIENTO PASA DIRECTO A EVALUACION						                         
												                    IF NOT EXISTS(SELECT numproducto from bdicred:sd_productos_motor WHERE numproducto=sNum_producto) THEN
											...
											ELSE
												CALL bdiburo:"informix".ins_buro_credito('CC',o_empresa,o_numsol,v_cliente,v_hoy,v_hoy,'',0,'','0') RETURNING s_regreso;
													IF s_regreso = '0' THEN
														...
														IF EXISTS(SELECT numproducto from bdicred:sd_productos_motor WHERE numproducto=sNum_producto) THEN -- AND EXISTS(SELECT numsucursal from bdicred:sd_sucursales_motor WHERE numsucursal=v_sucursal) THEN
  													          -------------------------------------------------------------------------------------------------------------
															  -----------------------Enviar solicitudes banco a consulta coppel    ----------------------------------------
															  -------------------------------------------------------------------------------------------------------------
																		IF  v_tpsol = 'T' AND sNum_producto ='6001' THEN
																		...
																		-----------------------------FIN REEVALUACION-------------------------------------------------------------
																			...
																			IF dtFechaSic IS NOT NULL THEN	
																				...
																				END IF;
																				 ...
																				 ELSE
																					--VALIDACION PARA SI NO ES UN PRODUCTO DE MOTOR 						                         
																					IF NOT EXISTS(SELECT numproducto from bdicred:sd_productos_motor WHERE numproducto=sNum_producto) THEN
																						EXECUTE PROCEDURE "informix".califica_scoring2_cjunk(o_empresa, o_numsol) INTO scod_ret; --fjpr
																					END IF;
																				 END IF;	
																...
																--------------------------------Fin del envio solicitudes banco a consulta coppel----------------------------
																-------------------------------------------------------------------------------------------------------------
																--SE COMENTA ESTA PARTE PARA EVITAR QUE ESTAS SOLICITUDES SE VAYAN A MOTOR SIN PASAR POR EC			  
															  --INSERT INTO  "informix".ss_enviossolicitudesmotor
  													          --(empresa, num_solicitud, num_cte, status_consumo, fecha_insert, status_respuesta)
  													          --VALUES  (o_empresa,o_numsol, v_cliente , 0, current, '');
  													      ELSE																	
  													          EXECUTE PROCEDURE "informix".califica_scoring2_cjunk(o_empresa, o_numsol) INTO scod_ret; --fjpr		
										...
										END IF;
									ELSE
										--se evalua que la solicitud sea de Prestamo
										IF   flag_rt_rcb <> 1 THEN
										
											IF (v_tpsol = 'P') THEN
												
												IF  EXISTS (SELECT
															numproducto
															FROM bdicred:sd_productos_motor_pp
															WHERE numproducto = sNum_producto)
															AND iConteoMotor_pp > 0
															AND EXISTS (SELECT
																		numsucursal
													...
													INSERT INTO "informix".ss_enviossolicitudesmotor_pp
													(empresa, num_solicitud, num_cte, status_consumo, fecha_insert, status_respuesta)
													VALUES (o_empresa,o_numsol, v_cliente , 0, current, '');
											...
											ELSE	
												...
												-------------------------------------------------------------------------------------------------------------
													IF  v_tpsol = 'T' AND sNum_producto ='6001' THEN
														...
														SELECT B.num_solicitud_ref INTO cSolMixta
														FROM bdisolic:ss_solicitudes A LEFT OUTER JOIN bdisolic:"informix".ss_resum_scor_fin B
														...
														-- 09/07/2024
															...
															--SI ES SOLICITUD BANCO CON BUEN COMPORTAMIENTO ENTONCES ACTUALIZA EL ESTATUS A EC
																...
																--SI LA SOLICITUD BANCO ES MIXTA LE CAMBIA PARAMETRICO 5
																	...
																	SET envio_parametrico = '5'
																	WHERE num_solicitud = o_numsol
																	AND empresa = o_empresa
																	AND envio_parametrico IS NULL;
															...
															--VALIDACION PARA SI NO ES UN PRODUCTO DE MOTOR 						                         
																IF EXISTS (SELECT
																		   numproducto
																		   FROM bdicred:"informix".sd_productos_motor
																		   WHERE numproducto = sNum_producto)
																		   AND iConteoMotor > 0
																		   AND EXISTS (SELECT
																					   numsucursal
																					   FROM bdicred:"informix".sd_sucursales_motor
																					   WHERE numsucursal = v_sucursal
																					   and producto = sNum_producto) THEN
																...
																ELIF EXISTS (SELECT
																			 numproducto FROM bdicred:sd_productos_motor
																			 WHERE numproducto = sNum_producto) 
																			 AND iConteoMotor = 0 THEN
																			 --AND NOT EXISTS (SELECT producto FROM bdicred:"informix".sd_sucursales_motor
																			 --WHERE producto = sNum_producto AND numsucursal = v_sucursal) THEN
													...
													ELSE
														...
														IF NOT EXISTS  (SELECT
																		numproducto
																		FROM bdicred:sd_productos_motor
																		WHERE numproducto=sNum_producto) THEN
																		
															EXECUTE PROCEDURE "informix".califica_scoring2_cjunk(o_empresa, o_numsol)
															INTO scod_ret; --fjpr
...
-- Obtiene le numero de reestructuras que ha tenido el cliente
		...
		UPDATE bdisolic:"informix".ss_revision_determinacion
		SET edad = iEdadcte,
			escolaridad_descrip = vescolaridad_des,
			situacion_especial = vsituacion_especial,
			causa_sit_esp = vcausa_situacion, 
			descripcion_siesp = cDescSitEsp,
			num_reest_cte = sReestructCte,
			fecha_sol = vfecha_sol,
