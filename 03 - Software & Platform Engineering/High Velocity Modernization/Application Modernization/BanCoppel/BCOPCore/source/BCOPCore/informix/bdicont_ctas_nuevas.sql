CREATE PROCEDURE "informix".ctas_nuevas(pempresa char(3),w_fecha date,w_usuario char(10))
...
foreach
   select distinct d.empresa, d.ccmayor, d.ccsub  , d.ccsubsub, d.ccssubsub,
          d.ccsssubsub, d.sector,
          --d.ciudad,
          d.moneda, naturaleza_cta,
          tipo_cuenta
   into   nuempresa,nuccmayor,nuccsub,nuccsubsub,nuccssubsub,
          nuccsssubsub,nusector,
          --nuciudad,
          numoneda,nunat_cta,
          nutipo_cta
   ...
   if lv_cuantos = 0 then
      ...
      if w_saldo_inicio_dia = 0 and w_cargos_dia = 0
         ...
         let nuempresa=nuempresa;
         let nuccmayor=nuccmayor;
         let nuccsub=nuccsub;
         let nuccsubsub=nuccsubsub;
         let nuccssubsub=nuccssubsub;
         let nuccsssubsub=nuccsssubsub;
         let nusector=nusector;
         --let nuciudad=nuciudad;
         let numoneda=numoneda;
         ...
         values (nuempresa,nuccmayor,nuccsub,
                 ...
                 w_abonos_dia,w_saldo_fin_de_dia,nutipo_cta,nunat_cta," "," "," ",0,0,w_usuario);
      end if
   end if
end foreach
end procedure;