CREATE PROCEDURE "informix".sp_conciliachqcomp1( pempresa CHAR(3) )
RETURNING CHAR(5), CHAR(5), CHAR(50), INTEGER, INTEGER;
    ...
    --- SET DEBUG FILE TO "/home/c98789058/SPL_ACCENTURE/sp_conciliachqcomp1.out";
    --- TRACE ON;
   ...
   SELECT valor INTO vcuentaini
     FROM bdicheq:sc_param
    WHERE empresa = pempresa
      AND codparam = 'CtaIniConcilChqComp1';

   SELECT valor INTO vcuentafin
     FROM bdicheq:sc_param
    WHERE empresa = pempresa
   ...
   -- // TABLA TEMPORAL DE MOVIMIENTOS
		SELECT con.cuenta, con.transacc, con.monto_tot,
		TRIM(pro.c_ccmayor)||TRIM(pro.c_ccsub)||TRIM(pro.c_ccsubsub)||TRIM(pro.c_ccsssub)||TRIM(pro.c_ccssssub)||TRIM(pro.c_sector) AS cta_cargo,
		TRIM(pro.a_ccmayor)||TRIM(pro.a_ccsub)||TRIM(pro.a_ccsubsub)||TRIM(pro.a_ccsssub)||TRIM(pro.a_ccssssub)||TRIM(pro.a_sector) AS cta_abono
		FROM bdinteg:si_transacc tra
		INNER JOIN bdicheq:sc_movdia_concil con ON tra.numero = con.transacc
		AND con.fech_alt = vfecha_hoy
		AND con.cancelad != 'S'
		AND con.producto = '2000'
		AND con.cuenta >= vcuentaini
		AND con.cuenta < vcuentafin
		INNER JOIN bdinteg:si_prodtran pro ON tra.sistema = pro.sistema
		AND tra.numero = pro.transaccion
		AND tra.sistema = '01'
		AND tra.se_contabiliza = 'S'
		AND pro.producto = '2000'
    ...
    IF vfecha_actual = vpri_hab_mes THEN
        ...
        FOREACH cur_1 WITH HOLD FOR
			...
			--Se quita directiva, no es necesaria, busqueda con INDEX
            ...
            FOREACH cur_1_1 WITH HOLD FOR
                ...
                LET vmonto_tot    = 0.00;
            END FOREACH;
            --FIN DE FOREACH cur_1_1 WITH HOLD FOR
            ...
            LET vgenero       = '';
        END FOREACH;
		--FIN DE FOREACH cur_1 WITH HOLD FOR
    ...
    ELSE
        ...
        FOREACH cur_2 WITH HOLD FOR
            ...
            LET vgenero       = '';
        END FOREACH;
		--FIN DE FOREACH cur_2 WITH HOLD FOR
    END IF;
