CREATE PROCEDURE "informix".sp_bitacorasistema(pUsuario CHAR(50), pEjecucion CHAR(1), pIpUsuario CHAR(16), pKey CHAR(255))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_bitacorasistema.out';
		--TRACE ON;
		...
		FOREACH
			SELECT FIRST 1 eje.nombre, eje.perfil, eje.sucursal, suc.nombre AS nombre_sucursal
			INTO cNombreEmpleado, iPerfil, cNumSucursal, cNombreSucursal
			FROM bdinteg:"informix".si_ejecut AS eje 
			INNER JOIN bdinteg:"informix".si_sucursales AS suc ON suc.sucursal = eje.sucursal
			WHERE eje.ejecutivo = pUsuario
			AND eje.empresa = cEmpresa
		END FOREACH; 
		
		-- ENTRADA
		IF pEjecucion = 'E' THEN
			
			--LET cKey = 'general.aclaraciones.bitacora.sistema.usuarioEntroAlSistema';
			LET cMensaje = 'El usuario ingresó al sistema.';
	
		-- SALIDA
		ELIF pEjecucion = 'S' THEN
			
			--LET cKey = 'general.aclaraciones.bitacora.sistema.usuarioSalioDelSistema';
			LET cMensaje = 'El usuario salió del sistema.';
			
			DELETE FROM "informix".acl_sesion_usuario WHERE numero_empleado = pUsuario AND ip_usuario = pIpUsuario;
			
		END IF;
		
		INSERT INTO "informix".acl_sistema_bitacora(pky_bitacora,area,fecha,key,mensaje,nombre_empleado,nombre_sucursal,num_empleado,num_sucursal,perfil,fky_mensaje)
		VALUES(sistema_bitacora_seq.NEXTVAL,null,CURRENT,pKey,cMensaje,cNombreEmpleado,cNombreSucursal,pUsuario,cNumSucursal,iPerfil,null);
...
'DESCRIPCION: SPL encargado de registrar el ingreso y salida al Sistema de Aclaraciones.',
'BD: bdiaclaracion';
