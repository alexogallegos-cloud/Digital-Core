CREATE PROCEDURE "informix".cierrechqcomp12_1( pEmpresa CHAR(3) )
RETURNING CHAR(5);
    ...
    LET vcUsuario      = USER;
    LET vcSistema      = '01';
    LET vcProceso      = 'cierrechqcomp12_1';
    LET viExiste       = 0;
    LET viExisteFin    = 0;
    LET vcExiste       = '';
    ...
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/altamasempnet/cierrechqcomp12_1.out";
    --- TRACE ON;
    
    BEGIN
    
    ON EXCEPTION SET viSqlErr, viIsamErr, vcDescErr
        SET DEBUG FILE TO "/resplogifx/conciliachq/altamasempnet/cierrechqcomp12_1.err";
        TRACE ON;
        IF viSqlErr <> 0 THEN
            ...
            LET vsql = 'echo "UPDATE bdinteg:sx_contproc '||
                       ...
                       'AND sistema   = '''||vcSistema||''';" > /tmp/horacierre12_1.sql';
            SYSTEM vsql;
            
            LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /tmp/horacierre12_1.sql';
            SYSTEM vstmt;
    ...
    IF viExiste = 0 THEN
        LET vsql = 'echo " INSERT INTO bdinteg:sx_contproc VALUES '||
                   ...
                   '(SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas), NULL, NULL);" > /tmp/horacierre12_1.sql';
        SYSTEM vsql;
        LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /tmp/horacierre12_1.sql';
        SYSTEM vstmt;
    ELSE
