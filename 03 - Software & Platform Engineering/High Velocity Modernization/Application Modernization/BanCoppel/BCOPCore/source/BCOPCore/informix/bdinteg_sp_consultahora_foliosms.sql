CREATE PROCEDURE "informix".sp_consultahora_foliosms (
    pEmpresa CHAR(5),
    pnumcte CHAR(20),
    pTelefono CHAR(10),
    pCodigoDinamico CHAR(4),
    pTiempoVigencia INTEGER
)
RETURNING CHAR(5);

DEFINE iMinTrans INTEGER;
DEFINE sEstado CHAR(10);
DEFINE dTimeSMS DATETIME YEAR TO FRACTION(3);
...
END EXCEPTION;
    ...
    --SET DEBUG FILE TO "/home/sysifx/sp_consultahora_foliosms.out";
    --TRACE ON;
    ...
    IF pEmpresa IS NULL OR pnumcte IS NULL OR pTelefono IS NULL 
       OR pCodigoDinamico IS NULL OR pTiempoVigencia IS NULL THEN
        LET scod_ret = "00001"; --  ParÃ¡metros incorrectos
		RETURN scod_ret;
    END IF;

    -- 1 Buscar la fecha de generaciÃ³n del codigo
    SELECT LIMIT 1 fecha INTO dTimeSMS FROM bdinteg:"informix".si_bitsmstels WHERE numcte = pnumcte 
    AND telefono = pTelefono
    AND digito_ver = pCodigoDinamico
    AND DATE(fecha) = DATE(CURRENT); --  Solo cÃ³digos generados hoy
    --ORDER BY fecha DESC;

    -- 2 Verificar si se encontrÃ³ un cÃ³digo
    IF dTimeSMS IS NULL THEN 
        LET scod_ret = "00002";
        RETURN scod_ret; --  No hay cÃ³digo registrado hoy
    END IF;

    -- 3 Calcular minutos transcurridos desde que se generÃ³ el cÃ³digo
    LET iMinTrans = ((CURRENT - dTimeSMS)::INTERVAL MINUTE(8) TO MINUTE)::CHAR(10)::INT;
    ...
    -- 4 Verificar si el cÃ³digo sigue vigente o ha expirado
    IF iMinTrans >= pTiempoVigencia THEN 
		LET scod_ret = "00005"; --  CÃ³digo vencido
    ELSE 
        LET scod_ret = "00000";   --  CÃ³digo vÃ¡lido
    END IF;

    -- 5 Retornar el estado del cÃ³digo
    RETURN scod_ret;
END
END PROCEDURE
DOCUMENT
"Especificacion: Procedimiento Almacenado para la Vigencia parametrizable del cÃ³digo SMS",
"Base de Datos : bdinteg",
"AUTOR : 99806221- JUAN PABLO MARIN DEL RINCON",
"FECHA : 18/03/2025";
...
CREATE PROCEDURE "informix".sp_validafoliosms_repetido (
    pEmpresa CHAR(5),
    pNumeroCliente CHAR(20),
    pNumeroTelefono CHAR(10),
    pCodigoDinamico CHAR(4)
)
RETURNING CHAR(5);
...
END EXCEPTION;
    ...
    --SET DEBUG FILE TO "/home/sysifx/sp_validafoliosms_repetido.out";
    --TRACE ON;
    ...
    --  Validaciones de parÃ¡metros
    IF pEmpresa IS NULL OR pNumeroCliente IS NULL OR pNumeroTelefono IS NULL OR pCodigoDinamico IS NULL THEN
		LET scod_ret = '00001';
        RETURN scod_ret; --  ParÃ¡metros incorrectos
    END IF;

    -- 1 Verificar si ya existe el cÃ³digo dinÃ¡mico para este cliente hoy
	SELECT COUNT(*) INTO iExiste
    FROM bdinteg:"informix".si_bitsmstels
    WHERE numcte = pNumeroCliente
    AND telefono = pNumeroTelefono
    AND digito_ver = pCodigoDinamico
    AND fecha >= CAST(TODAY AS DATETIME YEAR TO DAY)
    AND fecha <  CAST((TODAY +1) AS DATETIME YEAR TO DAY);

    -- 2 Evaluar si ya se usÃ³ el cÃ³digo
    IF iExiste > 0 THEN
		LET scod_ret = '00005';
    END IF;
...
END PROCEDURE
DOCUMENT
"Autor :JUAN PABLO MARIN DEL RINCON 99806221",
"FECHA : 18/03/2025",
"BD    : Bdinteg",
"PROYECTO: RQM 10 1697-2 ADENDUM OBSERVACIONES DE AUDITORIA",
"Descripcion: Procedimiento Almacenado para evitar enviar codigos sms repetidos";
...
BEGIN
	...
	END EXCEPTION;
         ...
         --SET DEBUG FILE TO '/home/e10001492/sp_cliente_cartera_opt.out'; 
         --TRACE ON;   
...
CREATE PROCEDURE "informix".sp_procesar_cartera_externa(pEmpresaCartera INT, pProcesado CHAR(1), pStatus CHAR(3),pOpcion INT)
    RETURNING 
    CHAR(6) as sCodRet, 
    CHAR(100) as sObservaciones;
-- DEFINICION DE VARIABLES.
    DEFINE cCodRet          CHAR(6);
    ...
    DEFINE cNumcte          CHAR(20);
    DEFINE dFec_Nac         DATE;
    DEFINE iId              INT;
    DEFINE cCodRetExiste    CHAR(6);
    DEFINE cSucursal        CHAR(4);
    DEFINE cEjecutivo       CHAR(8);
    DEFINE cGerente         CHAR(8);
    DEFINE cSector          CHAR(2);
    DEFINE cEmail           CHAR(100);
    DEFINE cCodAlta         CHAR(6);
    DEFINE cSexo            CHAR(1);    
        
    
    
    DEFINE siNombre1         CHAR(26);
    DEFINE siNombre2         CHAR(26);
    DEFINE siAPaterno        CHAR(26);
    DEFINE siAMaterno        CHAR(26);
    DEFINE siFec_Nac         DATE;
    
    DEFINE cRfcCod          CHAR(26);
    DEFINE cRfcMsg          CHAR(100);
    DEFINE cRfcRfc          CHAR(13);
    DEFINE cRfcAlta      CHAR(13);
    
    DEFINE cRfc             CHAR(13);
    DEFINE bFec_Nac         DATE;
    DEFINE iSqlErr          INTEGER;
...
-- INICIALIZACION DE VARIABLES.
  ...
  BEGIN    
        ...
        END EXCEPTION;
         ...
         --SET DEBUG FILE TO '/home/e10001492/sp_procesar_cartera_externa.out'; 
         --TRACE ON;    
        ...
        IF NVL(pEmpresaCartera, '') = '' or NVL(pProcesado, '') = '' or NVL(pStatus, '') = ''  THEN
            LET cCodRet = '000001';
            RETURN cCodRet, 'Parametros No VÃ¡lidos';
        END IF;
                
        --RECORRE LA TABLA SEGUN PARAMETROS       
        FOREACH
            SELECT d.id,NVL(d.nombre1_origen,''),NVL(d.nombre2_origen,''),NVL(d.apellido1_origen,''),NVL(d.apellido2_origen,''),d.fn_origen,d.rfc_origen,
            n.sucursal_default,n.ejecutivo_default,n.sector_default,d.email_origen,n.gerente_default,d.Sexo_origen
            INTO iId,siNombre1,siNombre2,siAPaterno,siAMaterno,siFec_Nac,cRfc,cSucursal,cEjecutivo,cSector,cEmail,cGerente,cSexo
            FROM bdinteg:"informix".si_cliente_cartera_externa d inner join bdinteg:"informix".si_empresa_cartera n
                           on d.id_empresa_cartera = n.id
            where id_empresa_cartera=pEmpresaCartera and procesado=pProcesado and status=pStatus              
            --SE CALCULA EL RFC
            EXECUTE PROCEDURE bdinteg:sp_calcularfc('001',UPPER(siAPaterno),UPPER(siAMaterno),UPPER(siNombre1),UPPER(siNombre2),siFec_Nac) INTO cRfcCod,cRfcMsg,cRfcRfc;                        
            IF (cRfcCod='000000') THEN            
                UPDATE bdinteg:"informix".si_cliente_cartera_externa  SET rfc_calculado=TRIM(cRfcRfc),observaciones=cRfcMsg,status='1' WHERE id=iId;                                                                                          
            ELSE
                UPDATE bdinteg:"informix".si_cliente_cartera_externa  SET rfc_calculado=rfc_origen,observaciones='RFC Origen',status='1' WHERE id=iId;
                LET cRfcRfc =cRfc;
            END IF;         
               
            IF (NVL(cRfcRfc,'')='') THEN
                UPDATE bdinteg:"informix".si_cliente_cartera_externa  SET observaciones='RFC No Valido',status = '2' WHERE id=iId;
            ELSE
            
                SELECT MAX(numcte) into cNumcte from bdinteg:"informix".si_cliente where empresa='001' and rfc= TRIM(cRfcRfc);
                --EXECUTE PROCEDURE bdichec:"informix".consdatadic('001',TRIM(cRfcRfc)) INTO  cCodRetExiste,cNumcte,cAPaterno,cAMaterno,cNombre1,cNombre2,cRfcAlta,dFec_Nac;                            
                IF (NVL(cNumcte,'')<>'')THEN                                
                    --CLIENTE EXISTENTE
                    SELECT  nombre1,nombre2,apell_paterno,apell_materno into cNombre1,cNombre2,cAPaterno,cAMaterno from bdinteg:"informix".si_cliente where numcte=cNumcte; 
                    UPDATE bdinteg:"informix".si_cliente_cartera_externa  SET num_cliente=TRIM(cNumcte), bnombre1 = TRIM(cNombre1),bnombre2 = TRIM(cNombre2),bapaterno = TRIM(cAPaterno),bamaterno = TRIM(cAMaterno),observaciones='Cliente existente',status = '2' WHERE id=iId;                                
                ELSE
                    --DAR DE ALTA NUEVO
                    UPDATE bdinteg:"informix".si_cliente_cartera_externa  SET observaciones= 'Cliente por dar de alta',status='2' WHERE id=iId;                                  
                    EXECUTE PROCEDURE bdinteg:"informix".sp_cliente_cartera_opt('001','A','',cSucursal,cEjecutivo,'01','2',siAPaterno,siAMaterno,siNombre1,siNombre2,cRfcRfc,cSector,'000','','000','000','1','','','01','','','',siFec_Nac,'','','','','','',cSexo,'','','','',0,'',cEmail,'','0','','',0,'',0,'','','',cGerente,'','','','') INTO cCodAlta,cNumCte;
                    IF (TRIM(NVL(cNumCte,''))='') THEN
                        UPDATE bdinteg:"informix".si_cliente_cartera_externa  SET observaciones= 'Error al dar de alta. '|| cCodAlta,status='3' WHERE id=iId;  
                    ELSE
                        UPDATE bdinteg:"informix".si_cliente_cartera_externa  SET num_cliente=cNumCte,observaciones= 'Cliente Alta Correcto',status='3' WHERE id=iId;
                    END IF;                                
                END IF;
            END IF;
                 
            LET cCodRet=cRfcCod;
            LET cObservaciones=cRfcRfc;
        END FOREACH;                   
        RETURN cCodRet, cObservaciones WITH RESUME;
    END;
END PROCEDURE
DOCUMENT
'SCRIPT DE PROCEDIMIENTO ALMACENADO "sp_consulta_nocaut_sitesp"',
'Folio.........: Proyecto Externo',
'Autor.........: 99802161 - Narciso Ivan Cisneros Acosta',
'Fecha.........: 30/10/2025',
'Solicita......: Abraham Narvaez Jacinto',
'BD............: bdinteg';
