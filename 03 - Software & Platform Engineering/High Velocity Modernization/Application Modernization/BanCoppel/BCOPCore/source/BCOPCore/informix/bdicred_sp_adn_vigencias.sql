CREATE PROCEDURE "informix".sp_adn_vigencias (pEmpresa CHAR(3))
RETURNING CHAR(6);          -- Codigo de Retorno
	...
	DEFINE cCodRetRgEvt     CHAR(6);
	
	DEFINE diasSinDispo		INTEGER;
	DEFINE yearAnti			INTEGER;
	DEFINE dtFechaHoy 		DATE;
	DEFINE dtMaxDisp 	    DATE;
	DEFINE dtMaxVig 	    DATE;
	DEFINE cNumSol			CHAR(20);
	DEFINE saldoInsoluto	Decimal(16,2);

	DEFINE si_ejecuto	    INTEGER;
	...
	DEFINE dt1Aviso,
	       ...
	       dtFchAprt,
	       dtFchInsrt,
	       dtFchNvasPlticas DATE;

	DEFINE cNumCel          CHAR(20);
	DEFINE cNumCte          CHAR(20);
	DEFINE cCodSMS          CHAR(15);
	...
	LET dtMaxDisp		    = DATE(1);
	LET dtMaxVig		    = DATE(1);
	LET cNumSol				= "";
	LET saldoInsoluto		= 0.00;
    ...
    LET dt2Aviso            = DATE(1);
    LET dt3Aviso            = DATE(1);
    LET dt1AvisoVig         = DATE(1);
    LET dt2AvisoVig         = DATE(1);
    LET dtFchAprt           = DATE(1);
    LET dtFchInsrt          = DATE(1);
    LET dtFchNvasPlticas    = DATE(1);
	...
	/* ********************************************************************************************************
	NOTA: Para la tabla sd_adn_cn_vig_notif en la columna status_sms estos son los estatus y sus descripciones.
	'0 - Hubo algun error que impidio que se mandara el sms al cliente
	'1 - Se mando de forma exitosa el sms al cliente
	'2 - Se intento reenviar el sms pero no se logro por algun problema en la ejecucion del sp_registra_evento
	******************************************************************************************************** */
	...
	BEGIN
		...
		--SET DEBUG FILE TO "/home/e90317801/joel/sp_adn_vigencias.out";
		--TRACE ON;
		...
		--Cancelacion por vigencia de 90 dias sin disposicion.
		LET dt1Aviso = dtFechaHoy - i1Aviso UNITS DAY;
		LET dt2Aviso = dtFechaHoy - i2Aviso UNITS DAY;
        LET dtMaxDisp = dtFechaHoy - diasSinDispo units DAY;
		LET dt3Aviso = dtMaxDisp + i3Aviso UNITS DAY;
		
		
		--Cancelacion por vigencia de 5 aÃ±os.
		LET dtMaxVig = dtFechaHoy - yearAnti units YEAR;
		LET dt1AvisoVig = dtMaxVig + i1AvisoVig UNITS DAY;
		LET dt2AvisoVig = dtMaxVig + i2AvisoVig UNITS DAY;
		
		
		-- EVALUA SI TIENE MAS DE N DIAS SIN SU PRIMER DISPOSICION PARA SER CANCELADA POR NO USO
		FOREACH WITH HOLD
			SELECT mcrd.num_credito, solcta.movil_cuenta, mcrd.fecha_apertura, solcta.numcte
			INTO cNumSol, cNumCel, dtFchAprt, cNumCte
			FROM bdicred:"informix".sd_maecred mcrd
			LEFT JOIN bdicred:"informix".sd_comision_x_apertura_contable comapr ON mcrd.num_credito = comapr.num_credito
			LEFT JOIN bdisolic:"informix".ss_adn_solicitudcuenta solcta on mcrd.num_credito = solcta.num_solicitud
			WHERE mcrd.num_producto = 7800
			AND mcrd.status_cred IN ('E1','E2','E3')
			AND mcrd.fecha_apertura >= dtFchNvasPlticas
			AND comapr.num_credito IS NULL
	        
	        IF (dtFchAprt = dt1Aviso) THEN
	        
                EXECUTE PROCEDURE bdimnsj:sp_registra_evento ('2' , 'ADN_SMS','SM_ADN_R1_NC', cNumCte,'', '','2', diasSinDispo, '', '', '', '', '', '', '', '', '', '', cNumCel, 0, 0,0, 0, 0,current, current) INTO cCodRetRgEvt;
                --Aun no usas tu Anticipo de Nomina. Estas dentro del periodo de 30 dias para hacer tu primera disposicion y evitar su cancelacion. MÃ¡s detalles en bancoppel.com

                IF cCodRetRgEvt != '00000' THEN
                    INSERT INTO bdicred:"informix".sd_adn_cn_vig_notif (num_credito, status_sms, cod_sms, cod_ret, fch_insert, hora_insert)
        		    VALUES (cNumSol, '0', 'SM_ADN_R1_NC', cCodRetRgEvt, dtFechaHoy, EXTEND(current, YEAR to SECOND));
        		    
                    CONTINUE FOREACH;
                END IF;
	        
	        ELIF (dtFchAprt = dt2Aviso) THEN
	        
                EXECUTE PROCEDURE bdimnsj:sp_registra_evento ('2' , 'ADN_SMS','SM_ADN_R2_NC', cNumCte,'', '','2', '', '', '', '', '', '', '', '', '', '', '', cNumCel, 0, 0,0, 0, 0,current, current) INTO cCodRetRgEvt;
                --Aun no has usado tu Anticipo de Nomina. Haz tu primera disposicion y evita su cancelacion. MÃ¡s detalles en bancoppel.com

                IF cCodRetRgEvt != '00000' THEN
                    INSERT INTO bdicred:"informix".sd_adn_cn_vig_notif (num_credito, status_sms, cod_sms, cod_ret, fch_insert, hora_insert)
        		    VALUES (cNumSol, '0', 'SM_ADN_R2_NC', cCodRetRgEvt, dtFechaHoy, EXTEND(current, YEAR to SECOND));
        		    
                    CONTINUE FOREACH;
                END IF;
                
            ELIF (dtFchAprt = dt3Aviso) Then
            
                EXECUTE PROCEDURE bdimnsj:sp_registra_evento ('2' , 'ADN_SMS','SM_ADN_R3_NC', cNumCte,'', '','2', '', '', '', '', '', '', '', '', '', '', '', cNumCel, 0, 0,0, 0, 0,current, current) INTO cCodRetRgEvt;
                --Aun no usas tu Anticipo de Nomina. Â¡Aprovecha ahora! Tienes solo 3 dias para hacer tu primera disposicion y evitar su cancelacion.

                IF cCodRetRgEvt != '00000' THEN
                    INSERT INTO bdicred:"informix".sd_adn_cn_vig_notif (num_credito, status_sms, cod_sms, cod_ret, fch_insert, hora_insert)
        		    VALUES (cNumSol, '0', 'SM_ADN_R3_NC', cCodRetRgEvt, dtFechaHoy, EXTEND(current, YEAR to SECOND));
                    CONTINUE FOREACH;
                END IF;
	        
    		ELIF (dtFchAprt <= dtMaxDisp) THEN
    			--CANCELA EL CREDITO
    			CALL sp_adn_cancelacredito(pEmpresa, cNumSol) RETURNING cCodRetCanCred;
    				
    			IF cCodRetCanCred = '00000' THEN
    				--INSERT INTO BITACORA
    				INSERT INTO bdicred:"informix".sd_adn_cn_vig VALUES (cNumSol, dtFechaHoy);
    					
    				EXECUTE PROCEDURE bdimnsj:sp_registra_evento ('2' , 'ADN_SMS','SM_ADN_R4_NC', cNumCte,'', '','1', '', '', '', '', '', '', '', '', '', '', '', cNumCel, 0, 0,0, 0, 0,current, current) INTO cCodRetRgEvt;
    	            --Tu Anticipo de Nomina fue cancelado por falta de uso. Podras volver a contratar despues de 90 dias en la app o en sucursal BanCoppel
    	
    	            IF cCodRetRgEvt != '00000' THEN
    	                INSERT INTO bdicred:"informix".sd_adn_cn_vig_notif (num_credito, status_sms, cod_sms, cod_ret, fch_insert, hora_insert)
        		        VALUES (cNumSol, '0', 'SM_ADN_R4_NC', cCodRetRgEvt, dtFechaHoy, EXTEND(current, YEAR to SECOND));
		...
		-- EVALUA SI TIENE MAS DE N AÃOS DESDE SU PRIMER DISPOSICION PARA SER CANCELADA POR ANTIGÃ¼EDAD
		FOREACH WITH HOLD
	        SELECT mcrd.num_credito, msdos.sdo_cap_insoluto, solcta.movil_cuenta, comapr.fecha_insert, solcta.numcte
			INTO cNumSol, saldoInsoluto, cNumCel, dtFchInsrt, cNumCte
			FROM bdicred:"informix".sd_maecred mcrd
			INNER JOIN bdicred:"informix".sd_comision_x_apertura_contable comapr ON mcrd.num_credito = comapr.num_credito
			INNER JOIN  bdicred:"informix".sd_maesdos msdos ON mcrd.num_credito = msdos.num_credito
			LEFT JOIN bdisolic:"informix".ss_adn_solicitudcuenta solcta on mcrd.num_credito = solcta.num_solicitud
			WHERE mcrd.num_producto = 7800
			AND mcrd.id_unidad_prod IS NULL
			AND mcrd.status_cred IN ('E1','E2','E3')
			AND comapr.fecha_insert >= dtFchNvasPlticas
	        AND comapr.fecha_insert <= dt1AvisoVig
			
			IF (dtFchInsrt = dt1AvisoVig) THEN
                EXECUTE PROCEDURE bdimnsj:sp_registra_evento ('2' , 'ADN_SMS','SM_ADN_V30D', cNumCte,'', '','2', '', '', '', '', '', '', '', '', '', '', '', cNumCel, 0, 0,0, 0, 0,current, current) INTO cCodRetRgEvt;
                --Proximamente vence tu Anticipo de Nomina. Renueva tu contrato en sucursal BanCoppel y sigue disfrutando el beneficio.

                IF cCodRetRgEvt != '00000' THEN
                    INSERT INTO bdicred:"informix".sd_adn_cn_vig_notif (num_credito, status_sms, cod_sms, cod_ret, fch_insert, hora_insert)
        		    VALUES (cNumSol, '0', 'SM_ADN_V30D', cCodRetRgEvt, dtFechaHoy, EXTEND(current, YEAR to SECOND));
        		    
                    CONTINUE FOREACH;
                END IF;
                
			ELIF (dtFchInsrt = dt2AvisoVig) THEN
                EXECUTE PROCEDURE bdimnsj:sp_registra_evento ('2' , 'ADN_SMS','SM_ADN_V10D', cNumCte,'', '','2', '', '', '', '', '', '', '', '', '', '', '', cNumCel, 0, 0,0, 0, 0,current, current) INTO cCodRetRgEvt;
                --Tu Anticipo de Nomina esta por vencer. Sigue disfrutando de este beneficio al contratar de nuevo. Acude a tu sucursal de preferencia.

                IF cCodRetRgEvt != '00000' THEN
                    INSERT INTO bdicred:"informix".sd_adn_cn_vig_notif (num_credito, status_sms, cod_sms, cod_ret, fch_insert, hora_insert)
        		    VALUES (cNumSol, '0', 'SM_ADN_V10D', cCodRetRgEvt, dtFechaHoy, EXTEND(current, YEAR to SECOND));
        		    
                    CONTINUE FOREACH;
                END IF;
                
            ELIF (dtFchInsrt <= dtMaxVig) THEN
        		IF saldoInsoluto > 0 THEN
        		    UPDATE bdicred:"informix".sd_maecred
        			SET id_unidad_prod = 3
        		    WHERE num_credito = cNumSol;
        					
        	    ELIF saldoInsoluto = 0 THEN
        		    -- CANCELA EL CREDITO
        			CALL sp_adn_cancelacredito(pEmpresa, cNumSol) RETURNING cCodRetCanCred;
        			...
        			IF cCodRetCanCred = '00000' THEN
        				...
        				EXECUTE PROCEDURE bdimnsj:sp_registra_evento ('2' , 'ADN_SMS','SM_ADN_FINC', cNumCte,'', '','2', '', '', '', '', '', '', '', '', '', '', '', cNumCel, 0, 0,0, 0, 0,current, current) INTO cCodRetRgEvt;
        		        --La vigencia de tu credito de Anticipo de Nomina ha concluido. Renueva tu contrato en tu sucursal BanCoppel mas cercana y sigue disfrutando el beneficio.
        		
        		        IF cCodRetRgEvt != '00000' THEN
        		            INSERT INTO bdicred:"informix".sd_adn_cn_vig_notif (num_credito, status_sms, cod_sms, cod_ret, fch_insert, hora_insert)
        		            VALUES (cNumSol, '0', 'SM_ADN_FINC', cCodRetRgEvt, dtFechaHoy, EXTEND(current, YEAR to SECOND));
		...
		-- ENVIA LOS SMS QUE QUEDARON PEDIENTES POR ALGUN ERROR AL CORRER EL SP_REGISTRA_EVENTO
		FOREACH WITH HOLD
			SELECT notif.num_credito, notif.cod_sms, solcta.movil_cuenta, solcta.numcte
			INTO  cNumSol, cCodSMS, cNumCel, cNumCte
			FROM bdicred:"informix".sd_adn_cn_vig_notif notif
			INNER JOIN bdisolic:"informix".ss_adn_solicitudcuenta solcta on notif.num_credito = solcta.num_solicitud
			AND notif.status_sms = '0'
			
			IF (cCodSMS = 'SM_ADN_R1_NC') THEN
				EXECUTE PROCEDURE bdimnsj:sp_registra_evento ('2' , 'ADN_SMS',cCodSMS, cNumCte,'', '','2', diasSinDispo, '', '', '', '', '', '', '', '', '', '', cNumCel, 0, 0,0, 0, 0,current, current) INTO cCodRetRgEvt;
			ELIF (cCodSMS = 'SM_ADN_R4_NC') THEN
				EXECUTE PROCEDURE bdimnsj:sp_registra_evento ('2' , 'ADN_SMS',cCodSMS, cNumCte,'', '','1', '', '', '', '', '', '', '', '', '', '', '', cNumCel, 0, 0,0, 0, 0,current, current) INTO cCodRetRgEvt;
			ELSE
				EXECUTE PROCEDURE bdimnsj:sp_registra_evento ('2' , 'ADN_SMS',cCodSMS, cNumCte,'', '','2', '', '', '', '', '', '', '', '', '', '', '', cNumCel, 0, 0,0, 0, 0,current, current) INTO cCodRetRgEvt;
			END IF;

            IF cCodRetRgEvt = '00000' THEN
                UPDATE bdicred:"informix".sd_adn_cn_vig_notif SET status_sms = '1',hora_insert = EXTEND(current, YEAR to SECOND)
        		WHERE num_credito = cNumSol
				AND cod_sms = cCodSMS
			  	AND fch_insert = dtFechaHoy;
        	ELSE
        	   UPDATE bdicred:"informix".sd_adn_cn_vig_notif SET status_sms = '2', cod_ret = cCodRetRgEvt, hora_insert = EXTEND(current, YEAR to SECOND)
        	   WHERE num_credito = cNumSol
			   AND cod_sms = cCodSMS
			   AND fch_insert = dtFechaHoy;
			END IF;
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: 	Contiene la lÃ³gica para determinar si tiene N dias sin uso o N aÃ±os desde su primer disposicion para pasar a ser cancelada por vigencia.',
'FECHA : 		13/diciembre/2024',
'BD: 			bdicred',
'AUTOR :  		Luis Eduardo Martinez',
'DESCRIPCION: 	Se agregaron validaciones para mandar el sms de avisos antes de las cancelaciones de los creditos por vigencia y un ciclo de reintento por si fallo el primer sms.',
