CREATE PROCEDURE "informix".sp_histmovcheqcomp6(pempresa CHAR(3))
RETURNING CHAR(5)  AS vcodret1, 
    ...
    ON EXCEPTION
        ...
        SET DEBUG FILE TO "/resplogifx/conciliachq/sp_histmovcheqcomp6.err";
        TRACE ON;
        IF sql_err <> 0 THEN
    ...
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/sp_histmovcheqcomp6.out";
    --- TRACE ON;
    ...
    LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO /resplogifx/conciliachq/histmovcheq_aplicados_pte6_'||vfecha||'.txt '||
               ...
               'AND mov.cuenta >= '''||vcuenta_ini||''';" > /resplogifx/conciliachq/histmovcheqcomp6.sql';
    SYSTEM vsql;
    LET vstmt = "/ifxsif01/bin/dbaccess bdicheq /resplogifx/conciliachq/histmovcheqcomp6.sql"; 
    SYSTEM vstmt;
