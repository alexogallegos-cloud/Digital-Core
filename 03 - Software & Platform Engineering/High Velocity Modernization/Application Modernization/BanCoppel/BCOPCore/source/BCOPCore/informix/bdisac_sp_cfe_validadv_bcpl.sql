CREATE PROCEDURE "informix".sp_cfe_validadv_bcpl(pReferencia CHAR(30),pImporte CHAR(10))
		RETURNING 
		CHAR (5)  AS cCodRet,
	...
	DEFINE cFechaAnio       CHAR(2);
	DEFINE cFechaMes        CHAR(2);
    DEFINE cRefAnio         CHAR(2);  
    DEFINE cRefMes          CHAR(2);
    DEFINE cRefDia          CHAR(2);
...
/* 		SET DEBUG FILE TO '/informix/VIMA/cfe/sp_cfe_validadv_bcpl.out';
		TRACE ON; */
	...
	BEGIN
		...
		IF  LENGTH(TRIM(pReferencia)) < 30 THEN
			LET cCodRet = '00003';
		...
		ELSE
			...
			LET iLongDV      = LENGTH(TRIM(pReferencia)) -1 ; --Elimina espacios en blanco con TRIM, Calcula la longitud de la cadena -1
			LET cDigVerRef   = SUBSTR (pReferencia, LENGTH(TRIM(pReferencia)), 1); --Obtiene el registro ennla posiciÃ³n igual a la longitud de la cadena, en este caso '30' y solo obtiene un numero el cual seria el posicionado en 30
			LET cFechaRef    = SUBSTR(pReferencia,15,6);
			LET cImporte     = SUBSTR(pReferencia,21,9);
			LET pImporte     = SUBSTR(pImporte,1,9);
			LET cFechaFinal  = SUBSTR(cFechaHoy,3,6);
			
			
			--Se agrega validaciÃ³n de fecha en la linea de captura
			
			LET cFechaAnio = SUBSTR(cFechaHoy,3,2);
			LET cFechaMes  = SUBSTR(cFechaHoy,5,2);
			
			LET cRefAnio   = SUBSTR(pReferencia,15,2);
			LET cRefMes    = SUBSTR(pReferencia,17,2);
			LET cRefDia    = SUBSTR(pReferencia,19,2);			
			...
			IF pImporte = '' THEN
				...
				IF cFechaMes = '11' OR cFechaMes = '12' THEN 
					     
					     
					IF ((cFechaAnio <> cRefAnio) AND  (cFechaAnio + 1 <> cRefAnio))   THEN
					  LET cCodRet = '00003';
					  LET cDescripcion = 'Referencia incorrecta, favor de validar';
					  RETURN cCodRet,cDescripcion;
					END IF;
				...
				ELSE 
					LET iMultiplo  = 2;   --Se inicializa la variable iMultiplo con el valor 2, que se usarÃ¡ en el cÃ¡lculo del dÃ­gito verificador.
					
					FOR i = 1 TO iLongDV -- Se inicia un bucle FOR que recorre desde 1 hasta iLongDV.
						LET iNum = SUBSTRING (pReferencia FROM iLongDV FOR 1);   -- Se extrae un nÃºmero de pReferencia en la posiciÃ³n iLongDV y se guarda en iNum.
						LET iDigVer = iNum * iMultiplo; --Se multiplica iNum por iMultiplo y se guarda en iDigVer.
						
						LET iSuma = iSuma + iDigVer; --Se suma iDigVer a iSuma.
						LET iLongDV = iLongDV - 1; -- Se decrementa iLongDV en 1 para avanzar en la siguiente iteraciÃ³n del bucle.
						LET iMultiplo = iMultiplo + 1; --Se incrementa iMultiplo en 1 para el siguiente cÃ¡lculo.
						
						IF iMultiplo > 7 THEN
							LET iMultiplo  = 2; --Si iMultiplo es mayor que 7, se reinicia a 2. 
						END IF;
						
					END FOR;
					
					LET iModulo = MOD(iSuma,11);  --Se calcula el mÃ³dulo de iSuma dividido por 11 y se guarda en iModulo.
					IF iModulo = 10 THEN	    
						LET iModulo = 0; -- Si iModulo es 10, se ajusta a 0.
					END IF;
	...
	'Fecha: 15/04/2015',
	'Descripcion: Se crea procedimiento para validar la linea de captura de CFE',
	'Folio: 86141',
	'Modifica: Victor Manuel Hernandez Lopez',
	'Descripcion: Se agrega validacion para que no permita numeros diferentes a una fecha en la linea de captura',
	'Fecha: 15/08/2024',
	'BD: bdisac';

CREATE PROCEDURE "informix".sp_decodifica_linea_base_reimpresion(pCaptura CHAR(20),
	...
	-- ELABORO: 	L.S.C. JosÃ© Magdiel MartÃ­nez LÃ³pez
	-- FECHA:		09-04-2014
	-- PROYECTO: 	REIMPRESIÃN PAGOS GDF BPI
	...
	BEGIN
		...
		IF(LENGTH(TRIM(NVL(pCaptura,'')))==20) THEN
			IF(pCaptura[1,2] IN ('01','02','03','04','05','07','09','11','13','14'))THEN
			...
			ELIF(pCaptura[1,2] IN ('49'))THEN
				...
				LET cCodRet = cCodRet2;
				LET cRespuestaMostrar = '|Concepto='||TRIM(NVL(cDescripcionConcepto,''))||'|';
				LET cRespuestaDecodificada = '|Folio InfracciÃ³n='||TRIM(NVL(cFolio,''))||'|';
			...
			ELIF(pCaptura[1,2] IN ('54','55','57','58','59','60','61','62','63','64','65','66','67','68','69','70','71','72','73','74','75','76','77'))THEN
				...
				IF(pCaptura[1,2] == ('77')) THEN
					...
					LET cRespuestaDecodificada = '|' || TRIM(NVL(cReferencia,'')) || '|' || TRIM(NVL(cFolio,'')) || '|' || TRIM(NVL(cCantidad,'')) || '|' || TRIM(NVL(cPrecio,'')) || '|' || TRIM(NVL(cConcepto,'')) || '|';				
				ELSE
			...
			ELIF(pCaptura[1,2] IN ('84','85','86','87','20','21','22','23','24','25','26','27'))THEN
				...
				LET cLlevaDatosAdicionales = '1';
			ELIF(pCaptura[1,2] IN ('88','96','89','90','91','92','93','97','98'))THEN --- Se quita el concepto 94 y 95 ya que no son permitidos en el portal. BGV
				...
				LET cRespuestaDecodificada = '|RFC='||TRIM(NVL(cRFC,''))||'|Ejercicio='||TRIM(NVL(cEjercicioFiscal,''))||'|Mes='||TRIM(NVL(cMes,''))||'|Cuenta Predial='||TRIM(NVL(cPredial,''))||'|Tipo de OperaciÃ³n='||TRIM(NVL(cTipoOperacion,''))||'|Folio='||TRIM(NVL(cFolio,''))||'|TrÃ¡mite='||TRIM(NVL(cTramite,''))||'|Referencia='||TRIM(NVL(cReferencia,''))||'|Tipo de DeclaraciÃ³n='||TRIM(NVL(cTipoDeclaracion,''))||'|Vigencia='||TRIM(NVL(cVigencia,''))||'|';
...
'Fecha: 09-04-2014',
'ModificaciÃ³n: DECODIFICA LA LINEA BASE REIMPRESION Y LISTA LOS CAMPOS A MOSTRAR',
