CREATE PROCEDURE "informix".sp_clientevencido_bis_web(pEmpresa CHAR(3), pNumCliente CHAR(20))
    ...
    -- Modifico:
    -- Walberto Castro
    -- Se agrego la validacion de cuando los dÃ?Â­as vencidos sean mayor que 165 dÃ?Â­as se regrese el codigo de retorno 003.
    ...
    -- evaluando.
	--19-09-2012
	--Se depura codigo que no se utiliza y se agrega validaciÃ?Â³n para tomar en cuenta solo creditos con estatus BA y BT para realizar convenios.
	...
	--Set debug file to '/informix/sp_clientevencido_bis_web.out';
	--trace on;
    ...
    BEGIN
		...
		ELSE
			...
			FOREACH			
				...
				/*IF(SELECT empresa, sucursal, origen, empleado_captura, numcliente, numcuenta, plazo, importe, tipo_compac, activo, flag_pago, 
								efectuo_compac, nombre_efectuo, fecha_compac, fecha_insert, keyx, quien_convenio, nom_convenio, email, referenciacoppel, 
								imp_pagado, hora_insert  FROM "informix".cb_compac WHERE empresa= pEmpresa AND numcliente = pNumCliente) > 0 THEN */
								
				IF(SELECT count(numcliente) FROM "informix".cb_compac WHERE empresa= pEmpresa AND numcliente = pNumCliente) > 0 THEN
