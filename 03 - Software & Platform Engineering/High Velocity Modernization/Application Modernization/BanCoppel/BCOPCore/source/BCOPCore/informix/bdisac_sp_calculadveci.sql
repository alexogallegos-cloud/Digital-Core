CREATE PROCEDURE  "informix".sp_calculadveci(pNumReferencia CHAR(10))
RETURNING 
...
BEGIN
	...
	--SET DEBUG FILE TO '/respaldosbd/antoniocebreros/1483/migrado/calculadveci.out';
	--TRACE ON;		
	...
	IF LENGTH(TRIM(pNumReferencia)) = 10 THEN
		...
		FOR iI = 1 TO 9
			LET iValorDigito = SUBSTR(pNumReferencia,iI,1)::INTEGER;
			IF MOD(iI,2) = 1 THEN
				LET iNoPeso = 2;
			ELSE
				LET iNoPeso = 1;
			...
			IF iAux > 9 THEN
				...
				LET iAux = (cNum1::INTEGER) + (cNum2::INTEGER);
			END IF; 	
		...
		END IF;	
	ELSE
		--raise notice ''Referencia no es de 10 digitos''; 
		LET cCodRet = '00002';
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se crea rutina en central (SPL) desde una copia en sucursal(FN), atendiendo el folio 1483-MttoValRefPagServAVON, (procedimiento en central para validar el digito verificador para Pagos AVON)',
'Valida si el digito verificador capturado en pagos ECI es correcto',
