CREATE PROCEDURE "informix".reversion_web_new(
                           o_empresa CHAR(3),
                           O_SUCursal CHAR(4),
                           O_USUARIO  CHAR(8),
                           o_folio    CHAR(16),
...
SELECT {+INDEX(sd_fechas idx_sdfechas)} fecha_hoy
  ...
  WHERE empresa = "001";
   ...
   SELECT UNIQUE(cod_tipcred), num_credito
	...
	--AAME INC 27 136 31/10/2019 No realizar reverso si el crÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©dito esta cancelado (FF)  
	SELECT status_cred 
	INTO cstatus_cred
  ...
  IF (vCodTipCred IS NULL AND vnum_credito IS NULL) OR (sp_num_credisolucion IS NOT NULL) THEN
	...
	End if;
		...
		IF vExiste = 1 THEN
			...
			EXECUTE PROCEDURE "informix".reversioncrd_new(o_empresa, o_sucursal, O_USUARIO, o_folio,o_tiporev)
			   INTO v_codret;
			COMMIT WORK;
		   IF wBegin = "S" THEN
			 BEGIN WORK;
...
'BD          : bdicred',
'----------------------------------------------------------------------------',
'Descripcion : Se modifica la ejecucion del SP reversioncrd al nuevo SP reversioncrd_new',
'Modifico    : 90260234 - Luis Eduardo HernÃ¡ndez Granillo',
'Fecha       : 03/04/2024',
'BD          : bdicred';
...
RETURNING CHAR(5);
   ...
   -- // BORRA TABLAS HISTORICAS
    ...
    CREATE TABLE sc_encabezado_edocta_factelect 
      (
        ...
        gerente_suc     char(40),
	      ...
	      obj_impuesto    CHAR(2),
	      base	        MONEY(18,2)) 
    fragment by round robin in dbs_info06, dbsdatos07
    ...
    fragment by round robin in dbs_datos04, dbs_datos05, dbs_info05
