CREATE PROCEDURE "informix".pagnive_pagare(P_EMPRESA       VARCHAR(3)
...
BEGIN
  ...
  LET V_MTO_PERIODO = ROUND(((V_ELEVADO * P_MONTO) * (V_FACTOR - 1)) / (V_ELEVADO - 1),0);
...
--LET V_MTO_PERIODO = TRUNC(V_MTO_PERIODO);
  ...
  SELECT fecha_hoy + 1 UNITS MONTH,fecha_hoy + P_PLAZO UNITS MONTH
