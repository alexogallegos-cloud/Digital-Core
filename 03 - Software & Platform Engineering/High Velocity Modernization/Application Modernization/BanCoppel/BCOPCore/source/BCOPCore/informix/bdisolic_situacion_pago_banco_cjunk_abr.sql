CREATE PROCEDURE "informix".situacion_pago_banco_cjunk_abr(o_empresa CHAR(3),
...
RETURNING CHAR(5),      -- Retorno
          ...
          CHAR(2),      -- Tipo persona
          CHAR(20),     -- NÃºmero de cte de la referencia 1
          CHAR(120),    -- Nombre de la referencia 1
          CHAR(20),     -- NÃºmero de cte de la referencia 2
          ...
          CHAR(3),      -- Puesto
          CHAR(1),      -- Tiene crÃ©ditos
          CHAR(3),      -- ProfesiÃ³n
          CHAR(13),     -- TelÃ©fono de la referencia 1
          CHAR(13),     -- TelÃ©fono de la referencia 2
          CHAR(2),      -- Parentesco con la referencia 1
          CHAR(2),      -- Parentesco con la referencia
          CHAR(20);     -- NÃºmero de referencia del cte

		  -- CONTROL DE CAMBIOS:
--------------------------------------------------------------------------------
--Autor: Viridiana Osobampo
--ModificaciÃ³n: Se modifica para asignar valor al puesto en base a los nuevos 
--              catÃ¡logos definidos.
--PeticiÃ³n:	Alta Ã?nica
--Fecha de ModificaciÃ³n: 05-11-2009
--------------------------------------------------------------------------------
--Modificacion: Se modifica para que la consulta a la tabla ss_scoring_solic
-- solo busque los registros antes de alta Ãºnica
...
--               BanCoppel, de ser asÃ­ no se permite al cliente realizar una
--               solicitud de crÃ©dito dejando registro en la bitÃ¡cora de
--               precalificaciÃ³n.
--Fecha de ModificaciÃ³n:  12-03-2009
-- Proyecto: Caja Unica
--------------------------------------------------------------------------------
--Autor: Viridiana Osobampo
--ModificaciÃ³n: Se modifica para evaluar los crÃ©ditos que el cliente tiene en Bancoppel y 
...
--Fecha de ModificaciÃ³n: 08-09-2009
--------------------------------------------------------------------------------
--Autor: Viridiana Osobampo
--ModificaciÃ³n: Se modifica para que cuando el cliente no tiene el nÃºm. referencia 
--                como cliente coppel, en la bitÃ¡cora de precalificaciÃ³n se asigne
--                el numcte Bancoppel, el motivo serÃ¡ "B" y cambia la secuencia de 
--                parÃ¡metro para la consulta de saldo vencido en crÃ©ditos Bancoppel.
--PeticiÃ³n:	RQM 10 108 PrÃ©stamo Personal
--Fecha de ModificaciÃ³n: 25-09-2009
--------------------------------------------------------------------------------
--Autor: Viridiana Osobampo
--ModificaciÃ³n: Se agrega un mensaje en los valores de retorno, de modo que se 
--              indique el resultado obtenido en la ejecuciÃ³n del spl.
--PeticiÃ³n:	RQM 10 108 PrÃ©stamo Personal
--Fecha de ModificaciÃ³n: 04-11-2009
--------------------------------------------------------------------------------
--Autor: Viridiana Osobampo
--ModificaciÃ³n: Se modifica para eliminar validaciones directas al producto "6300"
--              para que el spl estÃ© parametrizado para todos los productos futuros.
-- PeticiÃ³n: RQM 10 108 PrÃ©stamo Personal
--Fecha de ModificaciÃ³n: 06-11-2009
--------------------------------------------------------------------------------
--Autor: Viridiana Osobampo
--ModificaciÃ³n: Se unifican los cambios realizados para el proyecto Alta Ã?nica y
--              PrÃ©stamo Personal BanCoppel en la versiÃ³n del spl productivo.
-- PeticiÃ³n: RQM 10 108 PrÃ©stamo Personal
--Fecha de ModificaciÃ³n: 07-01-2010
--------------------------------------------------------------------------------
--Autor: Viridiana Osobampo
--ModificaciÃ³n: Se optimiza consulta a la tabla bdicred:sd_hist_reserva.
-- PeticiÃ³n: RQM 10 108 PrÃ©stamo Personal
--Fecha de ModificaciÃ³n: 16-03-2010
--------------------------------------------------------------------------------
--Autor: Viridiana Osobampo
--ModificaciÃ³n: Se modifican los mensajes cuando existe motivo de rechazo por la 
--		     situaciÃ³n del crÃ©dito del cliente.
-- PeticiÃ³n: Alta Ã?nica paso 04
--Fecha de ModificaciÃ³n: 23-06-2010
--------------------------------------------------------------------------------
--Autor: JesÃºs Manuel Aguilar Heredia
--ModificaciÃ³n: Se modifican los mensajes cuando existe motivo de rechazo por la 
--		     situaciÃ³n del crÃ©dito del cliente., se modifica el tamaÃ±o de retorno del mensaje
-- PeticiÃ³n: Alta Ã?nica paso 04.5
--Fecha de ModificaciÃ³n: 09-12-2010
--------------------------------------------------------------------------------
--Autor: JesÃºs Manuel Aguilar Heredia
--ModificaciÃ³n: Se modifica para que cuando un credito restructurado, ya este liquidado, no se envie mensaje de credito con reestructura
-- PeticiÃ³n: RQM 09 240 "Solicitud de crÃ©dito posterior a una liquidaciÃ³n de una reestructura".
--Fecha de ModificaciÃ³n: 26-07-2011
--------------------------------------------------------------------------------
--Autor: Diego Guerra Atienzo
--ModificaciÃ³n: Se modifica para que evite enviar error -1207 cuando el cliente tiene varios prestamos personales
-- PeticiÃ³n: CorrecciÃ³n de Incidencia Productiva.
--Fecha de ModificaciÃ³n: 31-07-2012
--------------------------------------------------------------------------------
--Autor: Carolina Verdugo 
--ModificaciÃ³n: llamado a procedimiento sp_cosnsulta_act_riesgo.sql para verificar si  el cliemte 
--              cuenta con inestabilidad en la vivienda o actividad de riesgo
-- PeticiÃ³n: Cambios para optimizar el preoceso de alta unica
--Fecha de ModificaciÃ³n: 22-10-2015
--------------------------------------------------------------------------------
-- ****************************************************************************
...
DEFINE iact 						INTEGER;DEFINE iatr 						INTEGER;
LET scod_ret            = "000";
LET vsqlerr             = 0;
LET v_nroref            = 0;
LET v_paramref          = 0;
...
BEGIN
   ...
   SET DEBUG FILE TO "/DBA/mbucio/Vobos/10052021/situacion_pago_banco_cjunk_abraham.out";
   TRACE ON;	
...
--   END FOREACH
   ...
   IF pEjecucion = "0" THEN
       IF v_motivo = "1" THEN
         ...
         LET cCausaSol = "MRB";
           ...
           saldoropa, saldoprestamos, abonomuebles, abonoropa, abonoprestamos, fecha_ultima_compra, grupo,fechaultimopago,prestamoautorizado,montoautorizado,represtamo)
           VALUES
          ...
          iSaldoRopa,iSaldoPrestamos,iAbonoMuebles,iAbonoRopa,iAbonoPrestamos,iFechaUltimaCompra, vlGrupo,cFechaUltimoPago,cPrestamoAutorizado,iMontoAutorizado,iRePrestamo);
  ...
  -- Ini caja Unica. Viridiana
   ...
   IF sParamVencido IS NULL THEN
       LET scod_ret= '102';
       LET cMensaje = 'No Existe parÃ¡metro para evaluar el monto vencido del crÃ©dito';
       LET cCausaSol = "SPV";
       LET crechazo = '1';  
   END IF;
   ...
   IF (cParamReserva IS NULL) OR (cParamGradoRiesgo IS NULL) THEN
       LET scod_ret= '105';
       LET cMensaje = 'No Existe parÃ¡metro para evaluar el grado de riesgo del crÃ©dito';
       LET cCausaSol = "SPG";
       LET crechazo = '1'; 
   END IF;
       ...
       FOREACH
            SELECT num_credito, status_cred,fecha_apertura,credito_externo
             ...
             WHERE empresa = o_empresa
               ...
               SELECT NVL(SUM(NVL(monto_vencido,0) + NVL(mto_venc_trasp,0)),0), act
                 INTO dSdo_vencido,iact
               ...
               IF (NVL(dSdo_vencido,0) >= sParamVencido) THEN
                   LET scod_ret= '002';
                   --LET cMensaje = 'El cliente tiene crÃ©dito vencido en Bancoppel';
                   LET cMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
								   'en la pantalla "SelecciÃ³n de Productos". El sistema me indica que tiene '||
								   'vencido en su CrÃ©dito BanCoppel, le recuerdo que es importante ser '||
								   'puntual en sus pagos para conservar los beneficios de su CrÃ©dito.' ;
                   LET cCausaSol = "CVB";
                   LET crechazo = '1';
                   EXIT FOREACH;
               ELIF TRIM(cStatus_cred) = "CV" THEN
                   LET scod_ret= '003';
                   --LET cMensaje = 'El cliente tiene crÃ©dito vendido en Bancoppel';
                   LET cMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
								   'en la pantalla "SelecciÃ³n de Productos". No se le puede ofrecer una '||
								   'solicitud de crÃ©dito porque cuenta con un crÃ©dito vendido en '||
                                   'BanCoppel.';
                   LET cCausaSol = "COB";
                   LET crechazo = '1';
                   EXIT FOREACH;
               ELIF TRIM(cStatus_cred) = "FC" THEN			   
					IF NOT EXISTS (SELECT num_credito 
						...
						LET scod_ret= '004';
						--LET cMensaje = 'El cliente cuenta con crÃ©dito Reestructurado en Bancoppel';
						LET cMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
										'en la pantalla "SelecciÃ³n de Productos". No se le puede ofrecer una '||
										'solicitud de crÃ©dito porque cuenta con un crÃ©dito reestructurado.';
						LET cCausaSol = "CRB";
						LET crechazo = '1';
						EXIT FOREACH;   
					END IF;		   
                  
              -- IFRS Se contempla el nuevo estatus del crédito por etapa vigente 
               --ELIF TRIM(cStatus_cred) = 'AA' THEN
			   ELIF ((TRIM(cStatus_cred) = 'AA') OR (NVL(iact,-1) =0 and cStatus_cred= 'E1'))THEN
                ...
                LET dtMaxFechaCorte=mdy(month(dtMaxFechaCorte),'20',year(dtMaxFechaCorte));
                   ...
                   IF (cGrado_riesgo IN ('D','E')) OR (cGrado_riesgo = cParamGradoRiesgo AND dMto_reserva > cParamReserva) THEN
                       ...
                       LET cMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
										'en la pantalla "SelecciÃ³n de Productos". Le recuerdo que es importante '||
										'realizar puntualmente sus pagos mensuales de su crÃ©dito BanCoppel.';
                       LET cCausaSol = "MEB";
                       LET crechazo = '1';
                       EXIT FOREACH;
                   END IF;
    ...
    IF crechazo <> '1' THEN    
       ...
       FOREACH 
           ...
           IF v_producto <> '6011' THEN
               SELECT NVL(SUM(NVL(monto_vencido,0) + NVL(mto_venc_trasp,0)),0),atr
                 INTO dSdo_vencidocrd,iatr
               ...
               ELIF TRIM(cStatus_cred) = "CV" THEN
                  ...
                  LET cMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
                   ...
                   LET crechazo = '1';
                   EXIT FOREACH;
			   -- IFRS Se contempla el estatus del crédito por Etapa vigente   
               --ELIF TRIM(cStatus_cred) = 'AA' THEN
			   ELIF ((TRIM(cStatus_cred) = 'AA') OR (NVL(iatr,-1) =0 and cStatus_cred= 'E1'))THEN
	   ...
	   -- Valida que el cliente no tenga un prestamo flexible (digital) aun vigente (liquidado (FF) pero con vigencia)
		IF o_producto = '6800' THEN
			...
			IF vlCteLargo > 0 THEN -- existe al menos un credito flexible aun vivo.
				...
				LET cMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) ' ||
									'en la pantalla "SelecciÃ³n de Productos". No se le puede ofrecer una '||
									'solicitud de crÃ©dito porque cuenta con un prestamo digital activo.';			
			END IF;
		END IF;
...
END 
          ...
          -- Cliente  presenta ocupaciÃ³n de Riesgo o inestabilidad en la Vivienda
   ...
   IF NVL(cRiesgoViviendaCpl,'') = 1 OR NVL(cRiesgoViviendaBcpl,'') = 1  THEN
	...
	ELIF cActRiesgoCpl = 1 AND cActRiesgoBCpl = 0 THEN
		IF TRIM(cDescpRiesgo) = TRIM(cAbogadoLit) THEN 
			LET scod_ret= '110';
			LET cMensaje = 'Rechazo por ocupaciÃ³n de Riesgo Abogado Litigante';
			LET cCausaSol = "RDO";
		ELSE
			LET scod_ret= '108';
			LET cMensaje = 'Rechazo por ocupaciÃ³n de Riesgo en Coppel';
			LET cCausaSol = "RDO";
		END IF ;
			LET crechazo = '1';
	ELIF cActRiesgoCpl = 0 AND cActRiesgoBCpl = 1 THEN		
		IF TRIM(cDescpRiesgo) = TRIM(cPoliciaM) THEN
			LET scod_ret= '111';
			LET cMensaje = 'Rechazo por ocupaciÃ³n de Riesgo  Servicios de Seguridad Publica (Municipal)';
			LET cCausaSol = "RDO";
		ELSE 
			LET scod_ret= '109';
			LET cMensaje = 'Rechazo por ocupaciÃ³n de Riesgo en Bancoppel';
			LET cCausaSol = "RDO";
		END IF;
		LET crechazo = '1';
	ELIF cActRiesgoCpl = 1 AND  cActRiesgoBCpl =1 THEN	
		...
		ELSE
			LET scod_ret= '107';
			LET cMensaje = 'Rechazo por ocupaciÃ³n de Riesgo';
			LET cCausaSol = "RDO";
		END IF;
		LET crechazo = '1';
	END IF;
