CREATE PROCEDURE "informix".sp_cap_constipoproductogat(pUsuario CHAR(8), pIdFuncion CHAR(10))
	RETURNING CHAR(5) AS codret,
		SMALLINT AS id_producto, 
		CHAR(4) AS num_producto,
		CHAR(30) AS desc_producto;					

	DEFINE cCodRet CHAR(5);
	DEFINE iSqlErr INTEGER;
	DEFINE sIdProducto SMALLINT;
	...
	BEGIN	
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, sIdProducto, cNumProducto, cDescProducto;
		END EXCEPTION;
	
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cap_constipoproductogat.out';
		--TRACE ON;
		...
		FOREACH
			SELECT id_producto, num_producto, desc_producto
			INTO sIdProducto, cNumProducto, cDescProducto
			FROM "informix".sw_cap_tipoproductogat	
			
			LET iNoRegistros = iNoRegistros + 1;		
			RETURN cCodRet, sIdProducto, cNumProducto, UPPER(TRIM(cDescProducto)) WITH RESUME;
		END FOREACH;
...
'DESCRIPCION: SPL que realiza la consulta de los productos para el mantenimiento del GAT ',
'BD: bdicnweb';
...
'DESCRIPCION: SP que da de alta una sucursal para atm´s ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_consultamodcajagenatms(pUsuario CHAR(8), pIdFuncion CHAR(10), pSucursal CHAR(4))
		RETURNING CHAR(5) AS codret,
		CHAR(3) AS id_cajagen,
		CHAR(80) AS desc_cajagen;
	...
	DEFINE iCodRetSp 	INTEGER;
	DEFINE cMensaje 	CHAR(50);
	DEFINE cIdCajaGen 	CHAR(3);
	DEFINE cDescCajaGen CHAR(80);
	DEFINE iNoRegistros INTEGER;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cIdCajaGen, cDescCajaGen;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ope_consultamodcajagenatms.out';
		--TRACE ON;
		...
		SET ISOLATION TO DIRTY READ;		
		FOREACH
			EXECUTE PROCEDURE bdisuc:"informix".sp_soc_cgmod_atm(pSucursal)
			INTO cCodRetSp, cMensaje, cIdCajaGen, cDescCajaGen
		
			LET iCodRetSp = cCodRetSp::INTEGER;
			IF iCodRetSp < 0 THEN
				RAISE EXCEPTION iCodRetSp, 0, "ERROR EN LA EJECUCION DEL SP bdisuc.sp_soc_cgmod_atm";
			END IF;
			LET iNoRegistros = iNoRegistros + 1;
			RETURN cCodRet, cIdCajaGen, cDescCajaGen WITH RESUME;		
		END FOREACH;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Catalogo de ATM´S',
'DESCRIPCION: SPL que consulta la modificación la caja general de los atm´s ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_consultamodcentrocostoatms(pUsuario CHAR(8), pIdFuncion CHAR(10), pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
		CHAR(4) AS id_descCentroCosto;
	...
	BEGIN
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cIdCentroCosto;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ope_consultamodcentrocostoatms.out';
		--TRACE ON;
		...
		SET ISOLATION TO DIRTY READ;		
		FOREACH
			EXECUTE PROCEDURE bdisuc:"informix".sp_soc_combocc_mod_atm2(pRegistros, pRecuperacion)
			INTO cCodRetSp, cMensaje, cIdCentroCosto
		
			LET iCodRetSp = cCodRetSp::INTEGER;
			IF iCodRetSp < 0 THEN
				RAISE EXCEPTION iCodRetSp, 0, "ERROR EN LA EJECUCION DEL SP bdisuc.sp_soc_combocc_mod_atm";
			END IF;
			LET iNoRegistros = iNoRegistros + 1;
			RETURN cCodRet, cIdCentroCosto WITH RESUME;		
		END FOREACH;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Catalogo de ATM´S',
'DESCRIPCION: SPL que consulta la modificación de los centros de costo para los atm´s ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_consultamodidatms(pUsuario CHAR(8), pIdFuncion CHAR(10), pSucursal CHAR(4))
		RETURNING CHAR(5) AS codret,
		CHAR(6) AS id_desc_id_atm;		
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cDesIdAtm;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ope_consultamodidatms.out';
		--TRACE ON;
		...
		SET ISOLATION TO DIRTY READ;		
		FOREACH
			EXECUTE PROCEDURE bdisuc:"informix".sp_soc_idmod_atm(pSucursal)
			INTO cCodRetSp, cMensaje, cDesIdAtm
		
			LET iCodRetSp = cCodRetSp::INTEGER;
			IF iCodRetSp < 0 THEN
				RAISE EXCEPTION iCodRetSp, 0, "ERROR EN LA EJECUCION DEL SP bdisuc.sp_soc_idmod_atm";
			END IF;
			LET iNoRegistros = iNoRegistros + 1;
			RETURN cCodRet, cDesIdAtm WITH RESUME;		
		END FOREACH;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Catalogo de ATM´S',
'DESCRIPCION: SPL que consulta la modificación del id para atm´s ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_consultamovimientosatms(pUsuario CHAR(8), pIdFuncion CHAR(10))
		RETURNING CHAR(5) AS codret,
		CHAR(15) AS tipo_mov;	
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cTipoMov;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ope_consultamovimientosatms.out';
		--TRACE ON;
		...
		SET ISOLATION TO DIRTY READ;		
		FOREACH
			EXECUTE PROCEDURE bdisuc:"informix".sp_soc_combomov_atm()
			INTO cCodRetSp, cMensaje, cTipoMov
			
			LET iCodRetSp = cCodRetSp::INTEGER;
			IF iCodRetSp < 0 THEN
				RAISE EXCEPTION iCodRetSp, 0, "ERROR EN LA EJECUCION DEL SP bdisuc.sp_soc_combomov_atm";
			END IF;
			
			LET iNoRegistros = iNoRegistros + 1;
			RETURN cCodRet, cTipoMov WITH RESUME;		
		END FOREACH;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Catalogo de ATM´S',
'DESCRIPCION: SPL que consulta los movimientos de los atm´s ',
'BD: bdicnweb';
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Catalogo de ATM´S',
'DESCRIPCION: SPL que consulta la descripción del centro de costos para el catálogo de  atm´s ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_genreportemovimientosatms(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaInicio DATE, pFechaFin DATE, pTipoMov CHAR(15), pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5)  AS codret,
		CHAR(4)  AS centro_costo,
		CHAR(40) AS nombre_atms,
		CHAR(15) AS tipo_movimiento,
		DATE     AS fecha,
		CHAR(4)  AS proveedor, 
		CHAR(40) AS caja_gen, 
		CHAR(8)  AS usuario,
	...
	DEFINE cCodRetSp 		CHAR(6);
	DEFINE iCodRetSp 		INTEGER;
	DEFINE cCentroCosto 	CHAR(4);
	DEFINE cNombreAtms 		CHAR(40);
	DEFINE cTipoMovimiento 	CHAR(15);
	DEFINE dFecha 			DATE;
	DEFINE cProveedor 		CHAR(4); 
	DEFINE cCajaGen 		CHAR(40); 
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cCentroCosto, cNombreAtms, cTipoMovimiento, dFecha, cProveedor, cCajaGen, cUsuario, cNombreUsuario;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ope_genreportemovimientosatms.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pFechaInicio IS NULL  OR pFechaFin IS NULL OR pTipoMov = '' OR pRegistros IS NULL OR pRecuperacion IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cCentroCosto, cNombreAtms, cTipoMovimiento, dFecha, cProveedor, cCajaGen, cUsuario, cNombreUsuario;
		END IF;
		...
		SET ISOLATION TO DIRTY READ;		
		FOREACH
			EXECUTE PROCEDURE bdisuc:"informix".sp_soc_reporte_mov_atm2(pFechaInicio, pFechaFin, pTipoMov, pRegistros, pRecuperacion)
			INTO cCodRetSp, cMensaje, cCentroCosto, cNombreAtms, cTipoMovimiento, dFecha, cProveedor, cCajaGen, cUsuario, cNombreUsuario
			
			LET iCodRetSp = cCodRetSp::INTEGER;
			IF iCodRetSp < 0 THEN
				RAISE EXCEPTION iCodRetSp, 0, "ERROR EN LA EJECUCION DEL SP bdisuc.sp_soc_reporte_mov_atm2";
			END IF;
			
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet, cCentroCosto, UPPER(TRIM(cNombreAtms)), TRIM(UPPER(cTipoMovimiento)), dFecha, cProveedor, cCajaGen, cUsuario, TRIM(UPPER(cNombreUsuario)) WITH RESUME;		
		END FOREACH;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Catalogo de ATM´S',
'DESCRIPCION: SPL que genera el reporte de los movimientos de los atm´s ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_genreportemovimientosatms_totales(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaInicio DATE, pFechaFin DATE, pTipoMov CHAR(15))
		RETURNING CHAR(5) AS codret,				
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ope_genreportemovimientosatms_totales.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR  pFechaInicio IS NULL OR pFechaFin IS NULL OR pTipoMov = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, iNoRegistros;
		END IF;
		...
		SET ISOLATION TO DIRTY READ;
		EXECUTE PROCEDURE bdisuc:"informix".sp_soc_reporte_mov_atm2_totales(pFechaInicio, pFechaFin,pTipoMov)
		INTO cCodRetSp, iNoRegistros ;
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, "ERROR EN LA EJECUCION DEL SP bdisuc.sp_soc_reporte_mov_atm2_totales";
		END IF;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Catalogo de ATM´S',
'DESCRIPCION: SPL que genera los totales del reporte de los movimientos de los atm´s ',
'BD: bdicnweb';
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Catalogo de ATM´S',
'DESCRIPCION: SP que actualiza los registros de los atm´s ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_procesartratamientoarchivo_tef(pUsuario CHAR(8), pIdFuncion CHAR(10), pRuta CHAR(100), pNombreArchivo CHAR(50), pTipoOperacion SMALLINT)
                RETURNING CHAR(5) AS codret;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/ingrid/sp_procesartratamientoarchivo_tef.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pRuta = '' OR pNombreArchivo = '' OR pTipoOperacion IS NULL THEN
				LET cCodRet = '00003';
				RETURN cCodRet;
		END IF;
		...
		LET cRutaArchivo = TRIM(pRuta)||TRIM(pNombreArchivo);
		
		IF pTipoOperacion = 1 THEN
				SYSTEM "sed 's/\\/\\\\/g' "||TRIM(cRutaArchivo)||" > "||TRIM(cRutaArchivo)||".tmp";
		ELIF pTipoOperacion = 2 THEN
				--SYSTEM "sed 's/\\\\\\\\/\\\\/g' "||TRIM(cRutaArchivo)||" > "||TRIM(cRutaArchivo)||".tmp";
				SYSTEM "sed 's,\\\\,\\,g' "||TRIM(cRutaArchivo)||" > "||TRIM(cRutaArchivo)||".tmp";
				SYSTEM "tr -d '\r' < "||TRIM(cRutaArchivo)||".tmp"||" > "||TRIM(cRutaArchivo)||".tmp1";
				SYSTEM "sed "||"'s/$'""/`echo \\\r`/"" "||TRIM(cRutaArchivo)||".tmp1"||" > "||TRIM(cRutaArchivo)||".tmp2";
		END IF;
		
		-- Eliminamos el archivo original
		SYSTEM "rm -rf "||TRIM(cRutaArchivo);
		
		IF pTipoOperacion = 1 THEN
			-- Se renombra el archivo temporal por el nombre original
			SYSTEM "mv "||TRIM(cRutaArchivo)||".tmp  "||TRIM(cRutaArchivo);
		
		ELIF pTipoOperacion = 2 THEN
			 -- Eliminamos el archivo tmp
			SYSTEM "rm -rf "||TRIM(cRutaArchivo)||".tmp";
			 -- Eliminamos el archivo tmp1
			SYSTEM "rm -rf "||TRIM(cRutaArchivo)||".tmp1";
			 -- Se renombra el archivo temporal por el nombre original
			SYSTEM "mv "||TRIM(cRutaArchivo)||".tmp2  "||TRIM(cRutaArchivo);
...
'FECHA: 26/04/2015',
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Envio/RecepciÃ³n de archivos BanCoppel',
'DESCRIPCION: Realiza el tratado de los cacateres de slash (\) para poner doble en el archivo',
'MODIFICACION: Martha Salgado Mendoza',
'FECHA: 29/09/2016',
'DESCRIPCION: Modificacion para los carateres slash (\) para tipo de operacion 2 (recepcion) para poner solo un slash. Conversion de LF a CRLF',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_portanom_reporte(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaInicial DATE, pFechaFinal DATE, pTipoSolicitud INTEGER, pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
				CHAR(10) AS fecha_archivo,
				CHAR(30) AS folio_solicitud,
				CHAR(20) AS estatus_solicitud,
				CHAR(20) AS estatus_rechazo,
				CHAR(10) AS fecha_respuesta,
				CHAR(18) AS cuenta,
				CHAR(20) AS banco,
				CHAR(90) AS cliente;
	...
	DEFINE cEstatusSolicitud CHAR(20);
	DEFINE cEstatusRechazo CHAR(20);
	DEFINE cFechaRespuesta CHAR(10);
	DEFINE cCuenta CHAR(18);
	DEFINE cBanco CHAR(20);
	DEFINE cCliente CHAR(90);
	DEFINE iNoRegistros INTEGER;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cFechaArchivo, cFolioSolicitud, cEstatusSolicitud, cEstatusRechazo, cFechaRespuesta, cCuenta, cBanco, cCliente;
		END EXCEPTION;

		-- SET DEBUG FILE TO '/tmp/mfinis/sp_portanom_reporte.out';
		-- TRACE ON;

		IF pUsuario = '' OR pIdFuncion = '' OR pFechaInicial IS NULL OR pFechaFinal IS NULL OR pTipoSolicitud = '' OR pRegistros IS NULL OR pRecuperacion IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cFechaArchivo, cFolioSolicitud, cEstatusSolicitud, cEstatusRechazo, cFechaRespuesta, cCuenta, cBanco, cCliente;
		END IF;
		...
		FOREACH EXECUTE PROCEDURE bdicheq:"informix".sp_rptportab(pFechaInicial, pFechaFinal, pTipoSolicitud, pRegistros, pRecuperacion)
			INTO cCodRetSp, cFechaArchivo, cFolioSolicitud, cEstatusSolicitud, cEstatusRechazo, cFechaRespuesta, cCuenta, cBanco, cCliente

			LET iCodRetSp = cCodRetSp::INTEGER;
			IF iCodRetSp < 0 THEN
				RAISE EXCEPTION iCodRetSp, 0, "ERROR EN LA EJECUCION DEL SP bdicheq:sp_rptportab";
			...
			RETURN cCodRet, cFechaArchivo, cFolioSolicitud, UPPER(cEstatusSolicitud), UPPER(cEstatusRechazo), cFechaRespuesta, cCuenta, cBanco, cCliente WITH RESUME;
		END FOREACH;
...
'DESCRIPCION: Realiza la consulta de las solicitudes indicadas en el rango de fehcas y tipo de solicitud',
'AUTOR: L. Montserrat LeÃ³n Amador',
'FECHA: 30/09/2016',
'DESCRIPCION: Se modifica SPL para agregar la columna fecha envÃ­o o recepciÃ³n de respuesta.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_verificastatusconsaltasidbox(pUsuario CHAR(8), pIdFuncion CHAR(10))
	RETURNING CHAR(5) AS codret,
		CHAR(1) AS status,
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_verificastatusconsaltasidbox.out';
		--TRACE ON;
		...
		FROM bdicnweb:"informix".sw_con_statuscargaidbox WHERE usuario = TRIM(pUsuario);
...
'DESCRIPCION: SPL que consulta el Estatus del proceso de Consulta de Reporte Procesos Sucursal',
'BD: bdicnweb';
...
pRegistros INTEGER, pRecuperacion INTEGER)
	RETURNING CHAR(5) AS codret,
		...
		CHAR(50) AS perfil_anterior,
		CHAR(50) AS perfil_nuevo;		
	...
	DEFINE cSucursal CHAR(4);
	DEFINE cHoraCambio CHAR(5);
	DEFINE cUsAutoriza CHAR(8);
	DEFINE cUsRecibeCambio CHAR(8);
	DEFINE cPerfilAnt CHAR(50);
	DEFINE cPerfilNue CHAR(50);
	DEFINE cNomUsAutoriza CHAR(45);
	DEFINE cNomUsRecibeCambio CHAR(45);
	DEFINE iRecuperacion INTEGER;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,dFechaCambio,cSucursal,cHoraCambio,cUsAutoriza,cNomUsAutoriza,cUsRecibeCambio,cNomUsRecibeCambio,cPerfilAnt,cPerfilNue;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_aud_cambioperfilsuc.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = ''OR pSucursal = '' OR pFechaIni IS NULL OR pFechaFin IS NULL OR pRegistros IS NULL OR pRecuperacion IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet,dFechaCambio,cSucursal,cHoraCambio,cUsAutoriza,cNomUsAutoriza,cUsRecibeCambio,cNomUsRecibeCambio,cPerfilAnt,cPerfilNue;
		END IF;
		...
		FOREACH
			...
			fecha_cambio,sucursal,hora_cambio,user_autoriza,cod_usuario,nom_us_utoriza,nom_us_recibe_cambio,desc_perfil_ant,desc_perfil_nuevo
			INTO dFechaCambio,cSucursal,cHoraCambio,cUsAutoriza,cUsRecibeCambio,cNomUsAutoriza,cNomUsRecibeCambio,cPerfilAnt,cPerfilNue
			FROM 
				(SELECT ca.fecha_cambio,ca.sucursal,ca.hora_cambio,ca.user_autoriza,ca.cod_usuario,
					(SELECT nombre FROM bdinteg:"informix".si_ejecut WHERE empresa = cEmpresa AND ejecutivo = ca.user_autoriza) AS nom_us_utoriza,
					(SELECT nombre FROM bdinteg:"informix".si_ejecut WHERE empresa = cEmpresa AND ejecutivo = ca.cod_usuario) AS nom_us_recibe_cambio,
					(SELECT descripcion FROM bdinteg:"informix".si_perfiles_suc WHERE id = ca.perfil_ant) AS desc_perfil_ant,
					(SELECT descripcion FROM bdinteg:"informix".si_perfiles_suc WHERE id = ca.perfil_nuevo) AS desc_perfil_nuevo
				FROM bdinteg:"informix".si_cambio_perf AS ca
				WHERE sucursal = pSucursal AND fecha_cambio BETWEEN pFechaIni AND pFechaFin ORDER BY fecha_cambio, hora_cambio ASC)
			
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet,dFechaCambio,cSucursal,cHoraCambio,cUsAutoriza,UPPER(cNomUsAutoriza),cUsRecibeCambio,UPPER(cNomUsRecibeCambio),
			cPerfilAnt,cPerfilNue WITH RESUME;
...
'DESCRIPCION: SPL que se encarga de consultar el detalle de los ejecutivos que solicitan el cambio de perfil en sucursal.',
'BD: bdicnweb';
