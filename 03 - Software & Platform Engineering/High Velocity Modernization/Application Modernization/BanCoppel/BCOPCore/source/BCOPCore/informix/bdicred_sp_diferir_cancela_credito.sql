CREATE PROCEDURE "informix".sp_diferir_cancela_credito(pcte CHAR(20), 
											pcredito CHAR(20), 
											ptipo SMALLINT, 
											pcanal SMALLINT)
...
--  SET DEBUG FILE TO "/ifxsif01/Israel/sp_diferir_cancela_credito.out";
--  TRACE ON;
    ...
    BEGIN
		...
		IF (LENGTH(pcredito) = 0 OR pcredito is null) AND (LENGTH(pcte) = 0 OR pcte is null) AND (ptipo is null ) THEN
			LET vcodret = "00001";
...
--		END IF;	
		...
		IF cteDiferir = '' THEN
			INSERT INTO "informix".sd_diferir VALUES(pcte, '',CURRENT,'','',CURRENT,pcanal);
		END IF
		...
		IF ptipo = 1 THEN
			...
			SELECT a.num_producto,a.sucursal,a.divisa
				INTO v_producto,vSucursal,vDivisa
			FROM bdicred:Sd_maecred a
				join bdicred:sd_programa_apoyo2020 b on (a.num_credito = b.num_credito)
			WHERE a.num_credito = pcredito AND b.bandera  = 'A';
			...
			FOREACH WITH HOLD
				...
				IF transsRet = '8369' THEN		--- transaccion de INTERES
					...
					CALL genmovcierre_movdia('001', pcredito, v_producto,12, "605", FechaHoy,  sdoRetenido, Folio, vSucursal, vDivisa, '8372',vPlaza) 
						RETURNING vcodret, v_mensaje;
					IF (vcodret <> "00000") THEN
						ROLLBACK WORK;
						LET v_mensaje = "CARGO INT PROGRAMA DE APOYO";
						CALL log_cierre ('001', pcredito, vcodret, FechaHoy, TRIM(v_mensaje) || "(GENMOV)") 
							RETURNING rLog;
						IF rLog > 0 THEN
				...
				ELIF transsRet = '8370' THEN		--- transaccion de IVA
					...
					CALL genmovcierre_movdia('001', pcredito, v_producto,13, "605", FechaHoy,  sdoRetenido, Folio, vSucursal, vDivisa, '8373',vPlaza) 
						RETURNING vcodret, v_mensaje;
					IF (vcodret <> "00000") THEN
						ROLLBACK WORK;
			...
			UPDATE bdicred:sd_programa_apoyo2020 
		...
		ELIF ptipo = 2 THEN
			...
			IF v_producto IS NULL THEN
				LET vcodret = '00003';
				RETURN vcodret, 'NO SE ENCONTRO CREDITO CON APOYO';
			END IF;
			
			IF v_producto in ('6300','7600','7700','6800') THEN
				BEGIN;
	  ...
	  RETURN vcodret, 'CANCELACION EXITOSA';

	END;
END PROCEDURE
DOCUMENT
'PROCESO PARA CANCELAR PROGRAMA DE APOYO POR CREDITO',
