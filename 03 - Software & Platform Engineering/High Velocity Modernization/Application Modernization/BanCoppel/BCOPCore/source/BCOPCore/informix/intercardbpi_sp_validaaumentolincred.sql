...
DEFINE cmaesdos_numcred 		CHAR (20);
DEFINE cstatus_cred				CHAR (2);
DEFINE cnum_tarjeta				CHAR (20);
DEFINE ccodstatustarjeta		CHAR (3);
DEFINE caumlincred_numsolic 	CHAR (20);
DEFINE dmmaesdos_otorgado		DECIMAL(18,2);
DEFINE dmnuevalineacredito		DECIMAL(18,2);
DEFINE dmaumlincred_lincredant	DECIMAL(18,2);
DEFINE ccodproductotarjeta    	CHAR(3);
DEFINE ccodproductosegmento 	CHAR(3);
DEFINE cpermite_segmentacion	CHAR(1);
DEFINE dcmlimite_max			DECIMAL(19,4);
DEFINE vsql	            		CHAR(1150);
DEFINE rptfechaini				CHAR(6);
DEFINE rptfechafin				CHAR(6);
DEFINE icommit					INTEGER;

--		Set debug file to "/informix/mgap/sp_validaaumentolincred.out";
--		trace on;
...
-----------------------------------------------------------------------------------------------------------------*/
		...
		SET ISOLATION TO DIRTY READ;
				SELECT 
					{+ INDEX (bdicred:"informix".sd_maesdos idx_sd_maesdos) }
					{+ INDEX (bdicred:"informix".sd_tarjeta idxsdtarjetapba) }
					{+ INDEX (bdicred:"informix".sd_maecred maesta) }
					cPeriodo AS periodo, a.num_credito AS maesdos_numcred, f.status_cred AS status_cred, d.num_tarjeta AS num_tarjeta, 
					e.codstatustarjeta AS codstatustarjeta, b.num_solicitud AS aumlincred_numsolic, a.monto_otorgado AS maesdos_otorgado, 
					b.lincred_sugerida AS nuevalineacredito, b.lincred_actual AS aumlincred_lincredant, e.codproductotarjeta AS codproductotarjeta, 
					current AS fechahorainsert
				FROM 
					bdicred:sd_maesdos a, 
					bdicred:sd_bitacora_aumlincred b, 
					bdicred: sd_tarjeta d,
					intercard: tarjeta e,
					bdicred:sd_maecred f
				where 
					a.empresa = cEmpresa
					and a.num_credito = f.num_credito
					and d.empresa = cEmpresa
					and d.num_credito = b.num_solicitud
					and d.tipo_tarjeta in ('T', 'A')
					and d.status_tar = 'A'
					and f.status_cred IN ('AA','E1')
					and (a.monto_vencido + a.mto_venc_trasp) = 0
					and b.num_solicitud = a.num_credito
					and a.monto_otorgado = b.lincred_sugerida
					and b.fecha_status::date >= dFecha_Inicio   
					and b.fecha_status::date < dFecha_Fin       
					and b.status = 'AP'
					and e.numtarjeta = d.num_tarjeta
...
-----------------------------------------------------------------------------------------------------------------*/
		...
		FOREACH WITH HOLD
			SELECT 
				maesdos_numcred, status_cred, num_tarjeta, codstatustarjeta, aumlincred_numsolic, maesdos_otorgado, nuevalineacredito, 
				aumlincred_lincredant, codproductotarjeta
			INTO
				cmaesdos_numcred, cstatus_cred, cnum_tarjeta, ccodstatustarjeta, caumlincred_numsolic, dmmaesdos_otorgado, dmnuevalineacredito, 
				dmaumlincred_lincredant, ccodproductotarjeta
			FROM "informix".aumentoslineacredito
			WHERE periodo = cperiodo  

			select 
				{+ INDEX (intercard:"informix".segmentoproducto idx_sgmtoprod) }
				codproductotarjeta, codproductosegmento, limite_max, permite_segmentacion
				into ccodproductotarjeta, ccodproductosegmento, dcmlimite_max, cpermite_segmentacion
			from "informix".segmentoproducto
			where 
				codproductotarjeta = ccodproductotarjeta
				and tipo_producto = 'C';
			...
			BEGIN WORK;
				...
				if
					dmnuevalineacredito > dcmlimite_max and ccodstatustarjeta in ('ACT', 'BLO', 'BLT') 
					and cpermite_segmentacion = 'V'
					then
						update "informix".tarjeta set codproductotarjeta = ccodproductosegmento where numtarjeta = cnum_tarjeta;
						update "informix".bitacoracambiostarjeta set descvaloranterior = dmaumlincred_lincredant, descvalornuevo = dmnuevalineacredito where tarjeta = cnum_tarjeta and fechacambio ::date >= today;
...
/*-----------------------------------------------------------------------------------------------------------------
	Genera Reporte de intercard: tarjeta.segmentoproducto actualizadas:
-------------------------------------------------------------------------------------------------------------------*/

		let vsql = ''; 	   
		let vsql = 'echo "Num_Credito         |Num_Tarjeta         |LineaCredito_Anterior |LineaCredito_Nueva|CodProdSegmento_Anterior|CodProdSegmento_Actual|">/resplogifx/BIN426807-6000'||'_'||cPeriodo||'.unl';
		system vsql;
		let vsql = '';
		let vsql = '';
		let vsql=  'echo "UNLOAD TO /resplogifx/BIN426807-6000.unl SELECT cuenta, tarjeta, descvaloranterior, descvalornuevo, valoranterior, valornuevo from intercard: bitacoracambiostarjeta where fechacambio::date >= TODAY and tabla = ''"'||'Intercard:tarjeta'||'"'' and campo = ''"'||'codproductotarjeta'||'"'' and descvaloranterior is not null and descvalornuevo is not null and descvaloranterior <> '' and descvalornuevo <> '' order by cuenta;">/resplogifx/rptactsgmtoprod.sql';
		system vsql;
		let vsql ='';
		let vsql= 'dbaccess intercard /resplogifx/rptactsgmtoprod.sql';
		system vsql;
		let vsql = '';
		let vsql ='rm /resplogifx/rptactsgmtoprod.sql';
		system vsql;
		let vsql ='';
		let vsql = "sed 's/|$//g' /resplogifx/BIN426807-6000.unl >>/resplogifx/BIN426807-6000"||'_'||cPeriodo||".unl";
		system vsql;
		let vsql ='rm /resplogifx/BIN426807-6000.unl';
		system vsql;
...
'Proyecto: RQI 13 504 ActualizaciÃ³n de SP validaciÃ³n de aumento a la lÃ­nea de CrÃ©dito para segmentaciÃ³n de producto por actualizaciÃ³n de fechas de ejecuciÃ³n',
'Modificacion: Se corrige el calculo de fechas de consulta y generaciÃ³n del reporte para cuando su ejecuciÃ³n sea en los meses de Enero',
'Fecha: 2017/01/16',
'Version: 20170116.0800',
'BD: intercard';

CREATE PROCEDURE "informix".sp_ctes_tdd_presente( pNumeroMeses VARCHAR(2), pNumMesAnteriorSdo INTEGER )
...
BEGIN
    ...
    LET TIPO_PLANTILLA = 'TP_CAPTA';
    LET ID_PLANTILLA = '1';
    LET OCTUBRE = '10';
    
    --SET DEBUG FILE TO RUTA_DESTINO||"sp_ctes_tdd_presente.out";
    --TRACE ON;
    ...
    --Confirmar que la informacion es actual antes de ejecutar otro reporte trimestral.
    --Si existe la tabla se elimina. @TODO Validar si hay reporte.
    IF ( (SELECT COUNT(*) FROM intercard:systables WHERE tabname = 'info_reporte_trimestral') = 1 ) THEN
            DROP TABLE IF EXISTS info_reporte_trimestral;
    END IF;
            
    CREATE TABLE "informix".info_reporte_trimestral (
             secuencial SERIAL NOT NULL,
             plantilla VARCHAR(15) NULL,
             cliente VARCHAR(15),
    ...
    CREATE INDEX "informix".idx_info_reporte_trimestral
        ON "informix".info_reporte_trimestral (plantilla) ONLINE;

    UPDATE STATISTICS MEDIUM FOR TABLE "informix".info_reporte_trimestral;

    -->Paso 00. Validar si existe cambio de anio.
    SET ISOLATION TO dirty READ;
    ...
    WHERE fechahorainauth BETWEEN vFechaInicio AND vFechaFinal
        AND mov.numtarjeta = t.numtarjeta
        AND t.codstatustarjeta = 'ACT'
        AND t.titular IN ('T', 'A')
        AND codigoiso = '00'
        AND formato = '0200'
    ...
    */
    ...
    SELECT COUNT(*) conteo_total INTO vTotalRegistros FROM info_reporte_trimestral WHERE plantilla = ID_PLANTILLA;
        ...
        IF (vTotalRegistros = 0) THEN
            ...
            LET vsql = '';
            LET vsql = 'echo "UNLOAD TO "'||RUTA_DESTINO||TIPO_PLANTILLA||'"_00.unl SELECT * FROM info_reporte_trimestral WHERE plantilla = "'||ID_PLANTILLA||'";" >'||RUTA_DESTINO||'tdd_presente_capta.sql';
            SYSTEM vsql;
            
            LET vsql ='';
            LET vsql= 'dbaccess intercard '||RUTA_DESTINO||'tdd_presente_capta.sql';
            SYSTEM vsql;
            
            LET vsql = '';
            LET vsql ='rm '||RUTA_DESTINO||'tdd_presente_capta.sql';
            SYSTEM vsql;
        ...
        IF (vTotalRegistros >  0) THEN            
            ...
            WHILE (vTotalRegistros > 0 ) LOOP
                ...
                LET vsql = '';
                LET vsql = 'echo "UNLOAD TO "'||RUTA_DESTINO||TIPO_PLANTILLA||'"_'||vContadorArchivos||'.unl SELECT SKIP '||vNumInicioRegistros||' FIRST '||vRegistrosMaxPorArchivo||' correo_electronico, titular FROM info_reporte_trimestral WHERE plantilla = "'||ID_PLANTILLA||'";" >'||RUTA_DESTINO||'tdd_presente_capta.sql';
                SYSTEM vsql;
