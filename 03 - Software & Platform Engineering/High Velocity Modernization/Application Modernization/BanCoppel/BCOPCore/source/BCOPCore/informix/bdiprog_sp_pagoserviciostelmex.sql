CREATE PROCEDURE "informix".sp_pagoserviciostelmex
(
...
RETURNING CHAR(5), CHAR(250);
	...
	-- Actividad : Tener un procedimieto que permita el registro de las transacciones programadas
     --                      para un pago de servicios.
	-- Fecha     :15 de  Novimebre  de 2008
	--Modifico: Alejandro Osuna
	--Fecha: Enero 2009
	--Se valida el campo clave final para que sea 02 por default, se valida la clave de compañia de telefono, se valida el campo numero de telefono para que no acepte letras.
...
LET v_mResta = 0.00;
   ...
   --SET DEBUG FILE TO "/respaldosbd/antoniocebreros/153/sp_pagoserviciostelmex.out";
   --TRACE ON;
...
BEGIN
	...
	IF EXISTS (SELECT user_insert FROM bdiprog:pp_ctasterceros  WHERE  cuenta = p_scuenta_destino) THEN
		...
		IF EXISTS(SELECT user_insert  FROM bdiprog:pp_ctasterceros WHERE num_cte = p_snum_cte AND cuenta = p_scuenta_destino) THEN
			IF p_sbanco_destino <> '000' THEN
				IF vtransaccion = 1 THEN   ROLLBACK;  BEGIN WORK; 	ELSE  ROLLBACK; END IF;
				SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet  FROM bdiprog:pp_mensajes WHERE cve_mensaje = '216';
				RETURN v_sCodRet, v_sMensajeRet;
			END IF;
			SET ISOLATION TO DIRTY READ;	
	...
	ELSE
		...
		RETURN v_sCodRet, v_sMensajeRet;
	END IF;
	--Se varifica que la clave cuenta destino sea igual es '05'
			...
			IF EXISTS(SELECT nomconvenio FROM bdisac:sac_convenios WHERE numcategoria = v_sNumCategoria AND numconvenio = v_sNumConvenio AND statusconvenio = 'A') THEN
				...
				IF v_iLongitudRef1Tabla = v_iLongitudRef1 THEN
					...
					IF v_iflgcalculodv_ref1 = 1 THEN
						...
						ELSE
							IF vtransaccion = 1 THEN   ROLLBACK;  BEGIN WORK; 	ELSE   ROLLBACK; END IF;
							SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet FROM bdiprog:pp_mensajes WHERE cve_mensaje = '78';
							RETURN v_sCodRet, v_sMensajeRet;
						END IF;
					ELSE
					END IF;
	...
	--se toma el valor de flg_ref2 y se valida la referencia DOS
	...
	VALUES(v_spagoprog, '1', p_dfecha_inicio, '03', '', '',p_suser_insert , CURRENT::DATE, '', '', '', '00');
	IF vtransaccion = 1 THEN
		COMMIT WORK;
		BEGIN WORK;
	ELSE
