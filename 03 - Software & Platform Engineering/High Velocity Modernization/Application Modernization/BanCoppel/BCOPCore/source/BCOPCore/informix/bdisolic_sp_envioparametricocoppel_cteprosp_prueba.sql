CREATE PROCEDURE "informix".sp_envioparametricocoppel_cteprosp_prueba(p_Empresa CHAR(3), p_NumCte VARCHAR(20), p_NumSolicitud CHAR(20), pTipoCliente INTEGER)
...
--IPCB 13sep2016
	...
	DEFINE iCampoFuturo5                        INTEGER;
	DEFINE iCampoFuturo6						INTEGER;
	DEFINE iCampoFuturo7                        INTEGER;
...
BEGIN
	...
	--SET DEBUG FILE TO "/tmp/martin/sp_parametrico/NUEVA_MODIFCACION_PARAMETRICO/sp_envioparametricocoppel_cteprosp.out";
	--TRACE ON;
...
*/
			...
			LET vContador13 = (SELECT count(solicitud) FROM bdiburo:"informix".br_auditor WHERE solicitud = p_NumSolicitud);
		...
		*/
		...
		IF TRIM(cFolioMovil) <> '' THEN
			...
			IF LEN(NVL(TRIM(cGeoCli),'')) > 10 AND (NVL(cFLagDomAlta,'') = 'S' ) THEN
				...
				LET vContador19 = (SELECT count(id_ptf) FROM bdinteg:"informix".si_ptf  WHERE TRIM(latitud)||","||TRIM(longitud) = TRIM(cGeoCli));
...
--RQM 18 086 Modificaciones servicio Parametrico Coppel.pdf	 DSB-03/02/2016 FIN
		...
		LET v_Cadena3 = cVar01||"|"||cVar02||"|"||cVar03||"|"||cVar04||"|"||cVar05||"|"||iVar06||"|"||iVar07||"|"||iVar08
				...
				||"|"||cFlag_ProductoCoppel||"|"||iCampoFuturo2||"|"||iCampoFuturo3||"|"||iCampoFuturo5||"|"||iCampoFuturo5
				||"|"|| iCampoFuturo6||"|"|| iCampoFuturo7||"|"|| iCampoFuturo8||"|"|| iCampoFuturo9||"|"|| iCampoFuturo10;
	END IF;
	...
	IF pTipoCliente = 2 THEN
        ...
        */
		...
		ELSE
			--OBTENER LA SUCURSAL DEL CLIENTE NO SE ESPECIFICA, SE HIZO EL FILTRO POR EL NUMERO DE CLIENTE PROSPECTO,O ??¿ES CORRECTO?.
			SELECT sucursal INTO cSucursal FROM bdiprospectos:"informix".pr_cliente WHERE numcte_pros = p_NumSolicitud;
