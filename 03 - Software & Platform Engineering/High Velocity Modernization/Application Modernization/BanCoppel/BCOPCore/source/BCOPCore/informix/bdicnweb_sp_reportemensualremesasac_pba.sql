CREATE PROCEDURE "informix".sp_reportemensualremesasac_pba(pUsuario CHAR(8), pIdFuncion CHAR(10), pPeriodo DATE, pConvenio CHAR(5))
	RETURNING
...
CREATE PROCEDURE "informix".sp_catalogosufijospm(pUsuario CHAR(8), pIdFuncion CHAR(10))
					
		RETURNING CHAR(5) AS codret,
			CHAR(2) AS cod_sufijo,
			CHAR(60) AS desc_sufijo,
			CHAR(1) AS status;
			
		DEFINE cCodRet CHAR(5);
        DEFINE iSqlErr INTEGER;	
		DEFINE cCodSufijo CHAR(2);
		DEFINE cDescSufijo CHAR(60);
		...
		BEGIN
			...
			ON EXCEPTION SET iSqlErr
                LET cCodRet = iSqlErr;
				RETURN cCodRet, cCodSufijo, cDescSufijo, cStatus;
			END EXCEPTION;
            
            --SET DEBUG FILE TO '/tmp/mfinis/sp_catalogosufijospm.out';
            --TRACE ON;
			...
			FOREACH
				SELECT codigo, descripcion, status INTO cCodSufijo, cDescSufijo, cStatus
				FROM bdinteg:"informix".si_sufijos ORDER BY codigo::INTEGER
			
				RETURN cCodRet, NVL(cCodSufijo,''), NVL(UPPER(cDescSufijo),''), NVL(cStatus,'') WITH RESUME;
...
'FECHA: 07/05/2015',
'DESCRIPCION: SPL que hace la consulta a la tabla bdinteg:si_sufijos para obtener codigo sufijo, descripciÃ³n del sufijo y el status.',
'FUNCIONALIDAD: Mantenimiento CatÃ¡logo Sufijos', 
'MODULO: Clientes',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_conscodigosufijopm(pUsuario CHAR(8), pIdFuncion CHAR(10))
		...
		BEGIN
			...
			ON EXCEPTION SET iSqlErr
                LET cCodRet = iSqlErr;
				RETURN cCodRet, codSufijo;
			END EXCEPTION;
            
            --SET DEBUG FILE TO '/tmp/mfinis/sp_conscodigosufijopm.out';
            --TRACE ON;
			...
			-- ASIGNA CÃDIGO
			SELECT LPAD(MAX(codigo::INTEGER) + 1, 2, '0') AS codigo
			INTO codSufijo FROM bdinteg:"informix".si_sufijos;
...
DOCUMENT 'AUTOR: L. Montserrat LeÃ³n Amador',
'FECHA: 06/05/2015',
'DESCRIPCION: SPL que asigna un nuevo cÃ³digo de sufijo (secuencial) para ser agregado a la tabla bdinteg:si_sufijos.',
'FUNCIONALIDAD: Mantenimiento CatÃ¡logo Sufijos', 
'MODULO: Clientes',
'BD: bdicnweb';
