CREATE PROCEDURE "informix".sp_maxdelq0to11mos_motor(pEmpresa CHAR(4), pNumCte CHAR(20) )
    RETURNING
        CHAR(6) as cCodRet,
    ...
    DEFINE maxDelq0To11Mos CHAR(4); 
    DEFINE iLineasCollection INTEGER;
    DEFINE cMesesHistContMas7Ocu CHAR(4);
    DEFINE cFlagDerogatory CHAR(1);
    DEFINE cMesesDerogatory CHAR(4); 
    DEFINE iHistPagosMas7Count INTEGER;
    DEFINE iHistPagosMas6Count INTEGER;
    DEFINE cMesesHistContMas6Ocu CHAR(4);
    DEFINE cBandera120Dias CHAR(1);
    DEFINE cMeses120dias CHAR(4);
    DEFINE iHistPagosMas5Count INTEGER;
    DEFINE cMesesHistContMas5Ocu CHAR(4);
    DEFINE cMeses90dias CHAR(4);
    DEFINE cBandera90Dias CHAR(1);
    DEFINE iHistPagosMas3Count INTEGER;
    DEFINE cMesesHistContMas3Ocu CHAR(4);
    DEFINE cMeses30dias CHAR(4);
    DEFINE cBandera30Dias CHAR(1);
    DEFINE iHistPagosMas4Count INTEGER;
    DEFINE cMesesHistContMas4Ocu CHAR(4);
    DEFINE cMeses60dias CHAR(4);
    DEFINE cBandera60Dias CHAR(1);
    DEFINE iHistPagosMas2Count INTEGER;
    DEFINE cMesesHistContMas2Ocu CHAR(4);
    DEFINE cMeses29dias CHAR(4);
    DEFINE cBandera29Dias CHAR(1);
    DEFINE cBanderaUnknownDelinquency CHAR(1);
    DEFINE cMesesUnknownDelinquency CHAR(4);
    DEFINE cBanderaNeverDelinquent CHAR(1);
    DEFINE  cMesesNeverDelinquent CHAR(4);
    DEFINE cMesesFechaAperColl CHAR(4);
    DEFINE cMesesFechaAper CHAR(4);
    DEFINE cTl30_std CHAR(2);
    DEFINE cTl27_std CHAR(24);
    DEFINE cMesesFechaMasRecienteHistPagos CHAR(4);
    DEFINE cMesesFechaReporte CHAR(4);
    DEFINE cTl38_std CHAR(2);
    DEFINE cMesesFechaHistMorGrave  CHAR(4);
    DEFINE cTl26_std CHAR(2);
    DEFINE iTl35_std INTEGER;
    DEFINE iTl33_std INTEGER;
    DEFINE iTl34_std INTEGER;
    DEFINE cSaldoVecidoEstan CHAR(6);
    DEFINE iTl25 INTEGER;
    DEFINE iMesesHistCont INTEGER;
    DEFINE var_i INTEGER;
    DEFINE iMeses120dias INTEGER;
    ...
    --------------------
    LET cMesesFechaAperColl = '';
    LET cMesesFechaAper = '';
    LET iLineasValidas = 0;
    LET maxDelq0To11Mos = '99'; 
    LET iLineasCollection = 0;
    LET cMesesHistContMas7Ocu = 'MV0';
    LET cFlagDerogatory = 'F';
    LET cMesesDerogatory = '1200'; 
    LET iHistPagosMas7Count = 0;
    LET iHistPagosMas6Count = 0;
    LET cMesesHistContMas6Ocu = 'MV0';
    LET cBandera120Dias = 'F';
    LET cMeses120dias = '1200';
    LET iHistPagosMas5Count = 0;
    LET cMesesHistContMas5Ocu = 'MV0';
    LET cMeses90dias = '1200';
    LET cBandera90Dias = 'F';
    LET iHistPagosMas3Count = 0;
    LET cMesesHistContMas3Ocu = 'MV0';
    LET cMeses30dias = '1200';
    LET cBandera30Dias = 'F';
    LET iHistPagosMas4Count = 0;
    LET cMesesHistContMas4Ocu = 'MV0';
    LET cMeses60dias = '1200';
    LET cBandera60Dias = 'F';
    LET iHistPagosMas2Count = 0;
    LET cMesesHistContMas2Ocu = 'MV0';
    LET cMeses29dias = '1200';
    LET cBandera29Dias = 'F';
    LET cMeses29dias = '1200';
    LET cBanderaUnknownDelinquency = 'F';
    LET cMesesUnknownDelinquency = '1200';
    LET cBanderaNeverDelinquent = 'F';
    LET  cMesesNeverDelinquent = '1200';
    ...
    LET cMensaje_ret        = '';
     ...
     BEGIN
        ON EXCEPTION SET iSqlErr, iIsamErr
            IF iSqlErr != 0 THEN
                LET cCodRet = iSqlErr;
                RETURN  cCodRet, maxDelq0To11Mos;
            END IF;
	    END EXCEPTION;

        --SET debug file to '/home/sysifx/VeraMariscal/sp_maxDelq0To11Mos_motor.out';
        --TRACE ON;
        ...
        FOREACH --collection
            ...
            SELECT mesesfechaapertura_std
                INTO cMesesFechaAperColl
                FROM bdiburo:"informix".br_tl_estand 
                WHERE numcte_std = pNumCte
                AND bandera_tradeline = 'T'
        
            LET iLineasCollection = iLineasCollection + 1;

            IF cMesesFechaAperColl NOT IN ('MV1','MV7') THEN
                IF cMesesFechaAperColl::INTEGER < 11 THEN
                    IF maxDelq0To11Mos::INTEGER > 0 THEN
                        LET maxDelq0To11Mos = '0';
                    END IF;
                END IF;
            ELIF maxDelq0To11Mos::INTEGER < 8 THEN
                LET maxDelq0To11Mos = '8'; 
            END IF;
        END FOREACH;

        FOREACH -- trade line

            SELECT mesesfechaapertura_std, tl30_std, tl27_std, mesesfechamasrecientehistpagos_std, mesesfechareporte_std, tl38_std, mesesfechahistmorgrave_std, tl26_std, tl35_std, tl33_std, saldovecidoestan_std, tl25_std, tl34_std
                INTO cMesesFechaAper, cTl30_std, cTl27_std, cMesesFechaMasRecienteHistPagos, cMesesFechaReporte, cTl38_std, cMesesFechaHistMorGrave, cTl26_std, iTl35_std, iTl33_std, cSaldoVecidoEstan, iTl25, iTl34_std
                FROM bdiburo:"informix".br_tl_estand 
                WHERE numcte_std = pNumCte
                AND bandera_tradeline = 'F'

            LET iLineasValidas = iLineasValidas + 1;
            
            /* 150+ days delinquent */
            --do process CALCULATE MONTHS SINCE DEROGATORY:
            --Subproceso: SEARCH PAYMENT HISTORY - RECENT (GREATER OR EQUAL) using VALUE = '7'
            LET iMesesHistCont = -1;
            LET var_i = 0;

            LET cMesesFechaMasRecienteHistPagos = '';
            LET cTl27_std = replace(replace(replace(cTl27_std,'-','0'),'X','0'),'U','0');
            FOR var_i = 1 to LENGTH (cTl27_std) 
                IF SUBSTR(cTl27_std,var_i,1)  >= 7 OR SUBSTR(cTl27_std,var_i,1) = ''   THEN
                    LET iMesesHistCont = iMesesHistCont + 1;
                    IF SUBSTR(cTl27_std,var_i,1)  >= 7 THEN 
                        LET iHistPagosMas7Count = iHistPagosMas7Count + 1;
                        IF cMesesFechaMasRecienteHistPagos NOT IN ('MV7','MV1') THEN
                            LET cMesesHistContMas7Ocu = to_char (iMesesHistCont + cMesesFechaMasRecienteHistPagos::INTEGER);
                            IF cMesesFechaReporte NOT IN ('MV7','MV1') THEN
                                IF cMesesHistContMas7Ocu::INTEGER < cMesesFechaReporte::INTEGER THEN
                                    LET cMesesHistContMas7Ocu = cMesesFechaReporte;
                                END IF;
                            END IF;
                        END IF;
                    END IF;
                END IF;
            END FOR;
            --Subproceso: CALCULATE MONTHS SINCE DEROGATORY
            IF iHistPagosMas7Count >= 1 THEN
                LET cFlagDerogatory = 'T';
                IF cMesesHistContMas7Ocu <> 'MV0' THEN
                    IF cMesesHistContMas7Ocu::INTEGER < cMesesDerogatory::INTEGER THEN
                        LET cMesesDerogatory = cMesesHistContMas7Ocu;
                    END IF;
                END IF;
            END IF;
            IF cTl30_std IN ('CL', 'FD', 'FP', 'FR', 'GP', 'IM', 'LC', 'LO', 'NV', 'PC', 'UP', 'VR') THEN
                LET cFlagDerogatory = 'T';
                IF cMesesFechaReporte NOT IN ('MV7','MV1') THEN 
                    IF cMesesFechaReporte::INTEGER < cMesesDerogatory::INTEGER THEN
                        LET cMesesDerogatory = cMesesFechaReporte;
                    END IF;
                END IF;
            END IF;
            IF cTl26_std IN ('07', '96', '97', '99') THEN
                LET cFlagDerogatory = 'T';
                IF cMesesFechaReporte NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaReporte::INTEGER < cMesesDerogatory::INTEGER THEN
                        LET cMesesDerogatory = cMesesFechaReporte;
                ...
                END IF;
            END IF;
            IF cTl38_std IN ('07', '96', '97', '99') THEN
                LET cFlagDerogatory = 'T';
                IF cMesesFechaHistMorGrave NOT IN ('MV7','MV1')  THEN
                    IF (cMesesFechaHistMorGrave::INTEGER) < (cMesesDerogatory::INTEGER) THEN
                        LET cMesesDerogatory = cMesesFechaHistMorGrave;
                    END IF;
                END IF;
            END IF;
            IF cMesesDerogatory = '1200' THEN
                LET cMesesDerogatory = 'MV1';
            END IF;
            -------
            IF cFlagDerogatory = 'T' AND cMesesDerogatory <> 'MV1' THEN
                IF cMesesDerogatory::INTEGER <= 11 THEN
                    IF maxDelq0To11Mos::INTEGER > 1 THEN
                        LET maxDelq0To11Mos = '1'; 
                    END IF;
            END IF;
            -------

            /* 120 days delinquent */
            --do process CALCULATE MONTHS SINCE 120 DAYS
            --Subproceso: CALCULATE MONTHS SINCE 120 DAYS
            --Subproceso: SEARCH PAYMENT HISTORY - RECENT (EXACT) using VALUE = '6'
            LET iMesesHistCont = -1;
            LET var_i = 0;

            LET cTl27_std = replace(replace(replace(cTl27_std,'-','0'),'X','0'),'U','0');
            FOR var_i = 1 to LENGTH (cTl27_std) 
                IF SUBSTR(cTl27_std,var_i,1)  = 6 OR SUBSTR(cTl27_std,var_i,1) = ''   THEN
                    LET iMesesHistCont = iMesesHistCont + 1;
                    IF SUBSTR(cTl27_std,var_i,1)  >= 6 THEN
                        LET iHistPagosMas6Count = iHistPagosMas6Count + 1;
                        IF cMesesFechaMasRecienteHistPagos NOT IN ('MV7','MV1') THEN
                            LET cMesesHistContMas6Ocu = iMesesHistCont + cMesesFechaMasRecienteHistPagos::INTEGER;
                            IF cMesesHistContMas6Ocu::INTEGER < cMesesFechaReporte::INTEGER THEN
                                LET cMesesHistContMas6Ocu = cMesesFechaReporte;
                            END IF;
                        END IF
                    END IF;
                END IF;
            ...
            IF iHistPagosMas6Count >= 1 THEN
                LET cBandera120Dias = 'T';
                IF cMesesHistContMas6Ocu <> 'MV0' THEN
                    IF  cMesesHistContMas6Ocu::INTEGER < cMeses120dias::INTEGER THEN
                        LET cMeses120dias = cMesesHistContMas6Ocu;
                    END IF;
                END IF;
            END IF;
            IF cTl26_std = '06' THEN
                LET cBandera120Dias = 'T';
                IF cMesesFechaReporte NOT IN ('MV7','MV1') THEN 
                    IF  cMesesFechaReporte::INTEGER < iMeses120dias THEN
                        LET iMeses120dias = cMesesFechaReporte::INTEGER ;
                    END IF;
                END IF;
            END IF;
            IF cTl38_std = '06' THEN
                LET cBandera120Dias = 'T';
                IF cMesesFechaHistMorGrave NOT IN ('MV7','MV1') THEN 
                    IF cMesesFechaHistMorGrave::INTEGER < cMeses120dias::INTEGER THEN
                        LET cMeses120dias = cMesesFechaHistMorGrave ;
                    END IF;
                END IF;
            END IF;
            IF cMeses120dias = '1200' THEN
                LET cMeses120dias = 'MV1';
            END IF;
            -------

            ELIF cBandera120Dias = 'T' AND cMeses120dias = 'MV1' THEN
                IF cMeses120dias::INTEGER <= 11 THEN
                    IF maxDelq0To11Mos::INTEGER > 2 THEN
                        LET maxDelq0To11Mos = '2'; 
                    END IF;
                END IF;


            /* 90 days delinquent */
            --SUBPROCESO: CALCULATE MONTHS SINCE 90 DAYS
            --SUBPROCESO: SEARCH PAYMENT HISTORY - RECENT (EXACT) using VALUE = '5' 
            LET iMesesHistCont = -1;
            LET var_i = 0;

            LET cTl27_std = replace(replace(replace(cTl27_std,'-','0'),'X','0'),'U','0');
            FOR var_i = 1 to LENGTH (cTl27_std) 
                IF SUBSTR(cTl27_std,var_i,1)  = 5 OR SUBSTR(cTl27_std,var_i,1) = ''   THEN
                    LET iMesesHistCont = iMesesHistCont + 1;
                    IF SUBSTR(cTl27_std,var_i,1)  = 5 THEN
                        LET iHistPagosMas5Count = iHistPagosMas5Count + 1;
                        IF cMesesFechaMasRecienteHistPagos NOT IN ('MV7','MV1') THEN
                            LET cMesesHistContMas5Ocu = iMesesHistCont + cMesesFechaMasRecienteHistPagos::INTEGER;
                            IF cMesesHistContMas5Ocu::INTEGER < cMesesFechaReporte::INTEGER THEN
                                LET cMesesHistContMas5Ocu = cMesesFechaReporte;
                            END IF;
                        END IF
                    END IF;
                END IF;
            ...
            IF iHistPagosMas5Count >= 1 THEN
                LET cBandera90Dias = 'T';
                IF cMesesHistContMas5Ocu <> 'MV0' THEN
                    IF cMesesHistContMas5Ocu::INTEGER < cMeses90dias::INTEGER THEN
                        LET cMeses90dias = cMesesHistContMas5Ocu;
                    END IF;
                END IF;
            END IF;
            IF cTl26_std = '05' THEN
                LET cBandera90Dias = 'T';
                IF cMesesFechaReporte NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaReporte::INTEGER < cMeses90dias::INTEGER THEN
                        LET cMeses90dias = cMesesFechaReporte;
                    END IF;
                END IF;
            END IF;

            IF cTl38_std = '05' THEN
                LET cBandera90Dias = 'T';
                IF cMesesFechaHistMorGrave NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaHistMorGrave::INTEGER < cMeses90dias::INTEGER THEN
                        LET cMeses90dias = cMesesFechaHistMorGrave;
                    END IF;
                END IF;
            END IF;
            IF iTl35_std > 0 THEN
                LET cBandera90Dias = 'T';
                IF cMesesFechaAper NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaReporte::INTEGER < cMeses90dias::INTEGER THEN
                        LET cMeses90dias = cMesesFechaReporte; 
                    END IF;
                END IF;
            END IF;
            IF cMeses90dias = '1200' THEN 
                LET cMeses90dias = 'MV1';
            END IF;
            --------
            ELIF cBandera90Dias = 'T' AND cMeses90dias <> 'MV1' THEN
                IF cMeses90dias::INTEGER <= 11 THEN
                    IF maxDelq0To11Mos::INTEGER > 3 THEN
                        LET maxDelq0To11Mos= '3'; 
                    END IF;
                END IF;
            

            /* 60 days delinquent */
            --do process CALCULATE MONTHS SINCE 60 DAYS
            --SUBPROCESO: SEARCH PAYMENT HISTORY - RECENT (EXACT) using VALUE = '4' 
            LET iMesesHistCont = -1;
            LET var_i = 0;

            LET cTl27_std = replace(replace(replace(cTl27_std,'-','0'),'X','0'),'U','0');
            FOR var_i = 1 to LENGTH (cTl27_std) 
                IF SUBSTR(cTl27_std,var_i,1)  = 4 OR SUBSTR(cTl27_std,var_i,1) = ''   THEN
                    LET iMesesHistCont = iMesesHistCont + 1;
                    IF SUBSTR(cTl27_std,var_i,1)  = 4 THEN
                        LET iHistPagosMas4Count = iHistPagosMas4Count + 1;
                        IF cMesesFechaMasRecienteHistPagos NOT IN ('MV7','MV1') THEN
                            LET cMesesHistContMas4Ocu = iMesesHistCont + cMesesFechaMasRecienteHistPagos::INTEGER;
                            IF cMesesHistContMas4Ocu::INTEGER < cMesesFechaReporte::INTEGER THEN
                                LET cMesesHistContMas4Ocu = cMesesFechaReporte;
                            END IF;
                        END IF
                    END IF;
                END IF;
            END FOR;
            --------
            --SUBPROCESO: CALCULATE MONTHS SINCE 60 DAYS

            IF iMesesHistCont >= 1 THEN
                LET cBandera60Dias = 'T';
                IF cMesesHistContMas3Ocu <> 'MV0' THEN
                    IF cMesesHistContMas3Ocu::INTEGER < cMeses60dias::INTEGER THEN
                        LET cMeses60dias = cMesesHistContMas3Ocu;
                    END IF;
                END IF;
            END IF;
            IF cTl26_std = '04' THEN
                LET cBandera60Dias = 'T';
                IF cMesesFechaReporte NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaReporte::INTEGER < cMeses60dias::INTEGER THEN
                        LET cMeses60dias = cMesesFechaReporte; 
                    END IF;
                END IF;
            END IF;
            IF cTl38_std = '04' THEN
                LET cBandera60Dias = 'T';
                IF cMesesFechaHistMorGrave NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaHistMorGrave::INTEGER < cMeses60dias::INTEGER THEN
                        LET cMeses60dias = cMesesFechaHistMorGrave; 
                    END IF;
                END IF;
            END IF;
            if iTl34_std > 0 THEN
                LET cBandera60Dias = 'T';
                IF cMesesFechaAper NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaAper::INTEGER < cMeses60dias::INTEGER THEN
                        LET cMeses60dias = cMesesFechaAper; 
                    END IF;
                END IF;
            END IF;
            IF cMeses60dias = '1200' THEN
                LET cMeses60dias = 'MV1';
            END IF;
            ---------           

            ELIF cBandera60Dias = 'T' AND cMeses60dias <> 'MV1' THEN
                IF cMeses60dias::INTEGER <= 11 THEN
                    IF maxDelq0To11Mos::INTEGER > 4 THEN
                        LET maxDelq0To11Mos = '4';
                    END IF;
                END IF;

            /* 30 days delinquent */
            --do process CALCULATE MONTHS SINCE 30 DAYS
            --SUBPROCESO: SEARCH PAYMENT HISTORY - RECENT (EXACT) using VALUE = '3' 
            LET iMesesHistCont = -1;
            LET var_i = 0;
            ...
            FOR var_i = 1 to LENGTH (cTl27_std) 
                IF SUBSTR(cTl27_std,var_i,1)  = 3 OR SUBSTR(cTl27_std,var_i,1) = ''   THEN
                    ...
                    IF SUBSTR(cTl27_std,var_i,1) = 3 THEN
                        LET iHistPagosMas3Count = iHistPagosMas3Count + 1;
                        IF cMesesFechaMasRecienteHistPagos NOT IN ('MV7','MV1') THEN
                            LET cMesesHistContMas3Ocu = iMesesHistCont + cMesesFechaMasRecienteHistPagos::INTEGER;
                            IF cMesesHistContMas3Ocu::INTEGER < cMesesFechaReporte::INTEGER THEN
                                LET cMesesHistContMas3Ocu = cMesesFechaReporte;
                            END IF;
                        END IF
                    END IF;
                END IF;
            ...
            --SUBPORCESO CALCULATE MONTHS SINCE 30 DAYS
            IF iMesesHistCont >= 1 THEN
                LET cBandera30Dias = 'T';
                IF cMesesHistContMas3Ocu <> 'MV0' THEN
                    IF cMesesHistContMas3Ocu::INTEGER < cMeses30dias::INTEGER THEN
                        LET cMeses30dias = cMesesHistContMas3Ocu; 
                    END IF;
                END IF;
            END IF;
            IF cTl26_std = '03' THEN
                LET cBandera30Dias = 'T';
                IF cMesesFechaReporte NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaReporte::INTEGER < cMeses30dias::INTEGER THEN
                        LET cMeses30dias = cMesesFechaReporte; 
                    END IF;
                END IF;
            END IF;
            IF cTl38_std = '03' THEN
                LET cBandera30Dias = 'T';
                IF cMesesFechaHistMorGrave NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaHistMorGrave::INTEGER < cMeses30dias::INTEGER THEN
                        LET cMeses30dias = cMesesFechaHistMorGrave; 
                    END IF;
                END IF;
            END IF;
            if iTl33_std > 0 THEN
                LET cBandera30Dias = 'T';
                IF cMesesFechaAper NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaAper::INTEGER < cMeses30dias::INTEGER THEN
                        LET cMeses30dias = cMesesFechaAper; 
                    END IF;
                END IF;
            END IF;
            IF cMeses30dias = '1200' THEN
                LET cMeses30dias = 'MV1';
            END IF;
            ---------
            ELIF cBandera30Dias = 'T' AND cMeses30dias <> 'MV1' THEN
                IF cMeses30dias::INTEGER <= 11 THEN
                    IF maxDelq0To11Mos::INTEGER > 5 THEN
                        LET maxDelq0To11Mos= '5'; 
                    END IF;
                END IF;

            /* 1-29 days delinquent */
            --SUBPROCESO: SEARCH PAYMENT HISTORY - RECENT (EXACT) using VALUE = '2'
            LET iMesesHistCont = -1;
            LET var_i = 0;

            LET cTl27_std = replace(replace(replace(cTl27_std,'-','0'),'X','0'),'U','0');
            FOR var_i = 1 to LENGTH (cTl27_std) 
                IF SUBSTR(cTl27_std,var_i,1)  = 2 OR SUBSTR(cTl27_std,var_i,1) = ''   THEN
                    LET iMesesHistCont = iMesesHistCont + 1;
                    IF SUBSTR(cTl27_std,var_i,1) = 2 THEN
                        LET iHistPagosMas2Count = iHistPagosMas2Count + 1;
                        IF cMesesFechaMasRecienteHistPagos NOT IN ('MV7','MV1') THEN
                            LET cMesesHistContMas2Ocu = iMesesHistCont + cMesesFechaMasRecienteHistPagos::INTEGER;
                            IF cMesesHistContMas2Ocu::INTEGER < cMesesFechaReporte::INTEGER THEN
                                LET cMesesHistContMas2Ocu = cMesesFechaReporte;
                            END IF;
                        END IF
                    END IF;
                END IF;
            ...
            IF iHistPagosMas2Count >= 1 THEN
                LET cBandera29Dias = 'T';
                IF cMesesHistContMas2Ocu <> 'MV0' THEN
                    IF cMesesHistContMas2Ocu::INTEGER < cMeses29dias::INTEGER THEN
                        LET cMeses29dias = cMesesHistContMas2Ocu;
                    END IF;
                END IF;
            END IF;
            IF cTl26_std = '02' THEN
                LET cBandera29Dias = 'T';
                IF cMesesFechaReporte NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaReporte::INTEGER < cMeses29dias::INTEGER THEN
                        LET cMeses29dias = cMesesFechaReporte;
                    END IF;
                END IF;
            END IF;
            IF cTl38_std = '02' THEN
                LET cBandera29Dias = 'T';
                IF cMesesFechaHistMorGrave NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaHistMorGrave::INTEGER < cMeses30dias::INTEGER THEN
                        LET cMeses30dias = cMesesFechaHistMorGrave; 
            ...
            IF iTl33_std > 0 THEN
                LET cBandera29Dias = 'T';
                    IF cMesesFechaAper NOT IN ('MV7','MV1') THEN
                    IF cMesesFechaAper::INTEGER < cMeses30dias::INTEGER THEN
                        LET cMeses29dias  = cMesesFechaAper;
                    END IF;
                END IF;
            END IF;
            IF cMeses29dias = '1200' THEN
                LET cMeses29dias = 'MV1';
            END IF;
            --------
            ELIF cBandera29Dias = 'T' AND cMeses29dias <> 'MV1'THEN
                IF cMeses29dias::INTEGER <= 11 THEN
                    IF maxDelq0To11Mos::INTEGER > 6 THEN
                        LET maxDelq0To11Mos = '6';
                    END IF; 
                END IF;

            /* unknown delinquency */
                --do process CALCULATE MONTHS SINCE UNKNOWN DELINQUENCY

                IF  (cSaldoVecidoEstan <> 'MV21' AND cSaldoVecidoEstan::INTEGER > 0) OR iTl25 > 0 THEN
                    LET cBanderaUnknownDelinquency = 'T';
                    IF cMesesFechaReporte NOT IN ('MV7','MV1') THEN 
                        IF cMesesFechaReporte::INTEGER < cMesesUnknownDelinquency THEN
                            LET cMesesUnknownDelinquency = cMesesFechaReporte::INTEGER; 
                        END IF;

                    ELIF cMesesFechaAper NOT IN ('MV7','MV1') THEN
                        IF cMesesFechaAper::INTEGER < cMesesUnknownDelinquency::INTEGER THEN
                            LET cMesesUnknownDelinquency = cMesesFechaAper;
                        END IF;
                    END IF;
                END IF;
                IF cMesesUnknownDelinquency::INTEGER = 1200 THEN
                    LET cMesesUnknownDelinquency = 'MV1';
                END IF;

            ELIF cBanderaUnknownDelinquency = 'T' AND cMesesUnknownDelinquency <> 'MV1' THEN
                IF cMesesUnknownDelinquency::INTEGER <= 11 THEN
                    IF maxDelq0To11Mos::INTEGER > 7 THEN
                        LET maxDelq0To11Mos = '7'; 
                    END IF;
                END IF;

            /* delinquency not in last 12 months or undated) */
            ELIF ((cFlagDerogatory = 'T') AND (cMesesDerogatory::INTEGER >= 12 OR cMesesDerogatory = 'MV1'))  OR
                 ((cBandera120Dias = 'T') AND (cMeses120dias::INTEGER >= 12 OR cMeses120dias = 'MV1')) OR
                 ((cBandera90Dias = 'T') AND (cMeses90dias::INTEGER >= 12 OR cMeses90dias = 'MV1')) OR
                 ((cBandera60Dias = 'T') AND (cMeses60dias::INTEGER >= 12 OR cMeses60dias = 'MV1')) OR
                 ((cBandera30Dias = 'T') AND (cMeses30dias::INTEGER >= 12 OR cMeses30dias = 'MV1')) OR
                 ((cBandera29Dias = 'T') AND (cMeses29dias::INTEGER >= 12 OR cMeses29dias = 'MV1')) OR
                 ((cBanderaUnknownDelinquency = 'T') AND (cMesesUnknownDelinquency::INTEGER >= 12 OR cMesesUnknownDelinquency = 'MV1'))  THEN
                IF maxDelq0To11Mos::INTEGER > 8 THEN
                    LET maxDelq0To11Mos = '8'; 
                END IF;

            /* current and never delinquent (and NOT too new to rate) */
            --SUBPROCESO: CALCULATE MONTHS SINCE NEVER DELINQUENT
            LET  cBanderaNeverDelinquent = 'F';
    
            IF cTl26_std = '01' AND (cSaldoVecidoEstan IN ('MV18','MV21','MV9') OR cSaldoVecidoEstan = '0') AND cTl30_std NOT IN ( 'CL', 'FD','FP', 'FR', 'GP', 'IM', 'LC', 'LO', 'NV', 'PC', 'UP', 'VR') THEN
                LET cBanderaNeverDelinquent = 'T';
                IF cMesesFechaReporte NOT IN ('MV7','MV1') THEN 
                    IF cMesesFechaReporte::INTEGER < cMesesNeverDelinquent::INTEGER THEN
                        LET cMesesNeverDelinquent = cMesesFechaReporte;
                    END IF;
                END IF;
            END IF;
            IF cMesesNeverDelinquent = '1200' THEN
                LET cMesesNeverDelinquent = 'MV1';
            END IF;

            ELIF cBanderaNeverDelinquent = 'T' and cMesesFechaReporte NOT IN ('MV7','MV1') THEN
                IF cMesesFechaReporte::INTEGER >= 3 THEN
                    IF cMesesNeverDelinquent = 'MV1' THEN
                        IF cMesesNeverDelinquent <= 11 THEN
                            IF maxDelq0To11Mos::INTEGER > 9 THEN
                                LET maxDelq0To11Mos = '9';
                            END IF;
                        END IF;
                    END IF;
                END IF;    
            ...
            /* all else */
            ELIF maxDelq0To11Mos::INTEGER > 10 THEN
                LET maxDelq0To11Mos = 10; 
            END IF;
        END FOREACH;
        
        IF iLineasValidas = 0 THEN 
            LET maxDelq0To11Mos = 'MV0';
        END IF;
...
DOCUMENT 
'----------------------------------------------------------------------------',
'Descripcion : Se crea sp del armado de variable maxDelq0To11Mos necesaria para motor de evaluaciÃ³n',
'Modifico    : 90233448 Vera Mariscal',
'Fecha       : 01/07/2022',
'BD          : BDIBURO';
