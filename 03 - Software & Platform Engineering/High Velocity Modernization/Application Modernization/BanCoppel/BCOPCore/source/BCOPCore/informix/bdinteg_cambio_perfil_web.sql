CREATE PROCEDURE "informix".cambio_perfil_web(psucursal CHAR(4),pcod_usuario CHAR(8),ppass_ant CHAR(1),
                                          ppass_nuevo CHAR(1),pperf_ant INTEGER,pperf_nuevo INTEGER,
                                          puser_autoriza CHAR(8),pfecha_cambio DATE,phora_cambio CHAR (5))

                             RETURNING CHAR(5),CHAR(4),CHAR(8),
                                       CHAR(1),CHAR(1),INTEGER,INTEGER,
                                       CHAR(8),DATE,CHAR(5);
--DEFINICION DE VARIABLES									   
DEFINE vcodret          CHAR(5);
DEFINE vsqlerr          INTEGER;
DEFINE vsucursal        CHAR(4);
DEFINE vcod_usuario     CHAR(8);
DEFINE vpass_ant        CHAR(1);
DEFINE vpass_nuevo      CHAR(1);
DEFINE vperfil_ant      INTEGER;
DEFINE vperfil_nuevo    INTEGER;
DEFINE vuser_autoriza   CHAR(8);
DEFINE vfecha_cambio    DATE;
DEFINE vhora_cambio     CHAR(5);
...
BEGIN
    ON EXCEPTION SET vsqlerr
        IF vsqlerr <> 0 THEN
         LET vcodret = vsqlerr;
         RETURN vcodret, vsucursal, vcod_usuario, vpass_ant, vpass_nuevo, vperfil_ant, vperfil_nuevo, vuser_autoriza,
                vfecha_cambio,vhora_cambio;
        END IF;
    END EXCEPTION;
	...
	IF NVL(ppass_ant,"") = "" THEN
      LET ppass_ant = ppass_nuevo;
    END IF;	   
	IF NVL(pperf_ant,"") = "" THEN
      LET pperf_ant = pperf_nuevo;			   
	...
	INSERT INTO "informix".si_cambio_perf_web (sucursal,cod_usuario,password_ant,password_nuevo,perfil_ant,perfil_nuevo,user_autoriza,fecha_cambio,hora_cambio )
	VALUES (psucursal,pcod_usuario,ppass_ant,ppass_nuevo,pperf_ant,pperf_nuevo,puser_autoriza,vfecha_cambio,phora_cambio);			   

    RETURN vcodret, psucursal, pcod_usuario, ppass_ant, ppass_nuevo, pperf_ant, pperf_nuevo, puser_autoriza,
           pfecha_cambio,phora_cambio;
END
END PROCEDURE
DOCUMENT
"MODIFICO: JosÃ© LÃºis Polanco B.",
"FECHA: 03/Febrero/2011",
"Ver.: 20110204.1133",
"BD: bdinteg",
"DescripciÃ³n: Se modifica sp cambio_perfil para que valide cuando se realice un alta de usuario y este no tenga datos registrados",
"en la tabla si_cambio_perf, se asigne el valor de el campo password_nuevo a el campo password_ant y el valor de el campo",
"perfil_nuevo a el campo de perfil_ant solo en el caso de que estos estÃ©n vacÃ­os.";
