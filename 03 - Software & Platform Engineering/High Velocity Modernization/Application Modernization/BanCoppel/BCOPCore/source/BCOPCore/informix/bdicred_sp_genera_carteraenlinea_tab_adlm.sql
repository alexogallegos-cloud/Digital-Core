CREATE PROCEDURE "informix".sp_genera_carteraenlinea_tab_adlm(pEmpresa char(3), pServicio char(1)) 
...
--SET DEBUG FILE TO "/resplogifx/archivoscartera/cobranza/sp_genera_carteraenlinea_tab.out";
--TRACE ON;
...
BEGIN
    ...
    SELECT fecha_ant INTO dFecha_hoy FROM bdicred:"informix".sd_fechas WHERE empresa = pEmpresa; 
    IF dFecha_hoy IS NULL THEN
        LET dFecha_hoy = Today;
    END IF
      
	--ValidaciÃÂÃÂ³n de la empresa
    ...
    -- Elimina la informacion almacenada, generada el dia anterior.
        --Delete from bdicred:"informix".sd_sdos_cartera_linea;
    /*Truncate bdicred:"informix".sd_sdos_cartera_linea;   
    ...
    IF pServicio = '1' OR pServicio = '3' THEN      -- Obtiene informacion de Cartera de Tarjeta de Credito
        ...
        LET cSQL1 = ' echo " SET ISOLATION TO DIRTY READ; UNLOAD TO ' || TRIM(cruta) || 'creditos_tab2.txt' 
                || ' select {+INDEX(sd_maecred maesta)} empresa, numcte, num_credito , sucursal, status_cred, num_producto,'
                || ' fecha_apertura, tasa_interes, tasa_moratorios, (select max(b.fecha) from sd_maesdoshist b) fecha_his '
                || ' from bdicred:sd_maecred a'
				|| ' where a.empresa = ''001'' '
                || ' and a.status_cred in (''BT'',''BA'');  '
                ...
                || ' create unique index inx_creditossl_tab2 on creditossl_tab2(numcte, num_credito);'
...
/*LET cSQL2 = '">' || TRIM(cRuta) || 'Ejecuta_cart_linea_creatabla.sql';
        ...
        FOREACH                 
            SELECT a.numcte, a.num_credito, nvl(c.num_tarjeta,'0'), 0 Cuenta_eje, a.num_producto, b.sdo_capital, b.monto_vencido, 
                ...
                and capital_status in ('2','7')),0) interes_iva, b.mto_fin_ven_trasp::integer mora_actual, a.status_cred, d.fecha_ult_pago, 
                ...
                where a.empresa = empresa and a.num_credito = num_credito and capital_status in ('2','7'))) + 
                round((b.sdo_moratorio + b.sdo_contab_mora) * (1+ s.iva),2) + (select sum(interes_debe - interes_pagado) + 
                sum(iva_debe - iva_pagado) from bdicred:sd_amortiza_credito where a.empresa = empresa and a.num_credito = num_credito 
                and capital_status in ('2','7')),0) pago_una_mora, a.sucursal, a.fecha_apertura, b.monto_otorgado, a.tasa_interes, 
				...
				trunc((dfecha_hoy - a.fecha_apertura)/30) antiguedad, nvl(scr1.evaluacion,0), nvl(scr2.evaluacion,0), nvl(scr3.evaluacion,0), 
				nvl(scr4.evaluacion,0), nvl(mahis1.mto_fin_ven_trasp,0), nvl(mahis2.mto_fin_ven_trasp,0), nvl(mahis3.mto_fin_ven_trasp,0), 
				nvl(mahis4.mto_fin_ven_trasp,0), nvl(mahis5.mto_fin_ven_trasp,0), nvl(mahis6.mto_fin_ven_trasp,0), cel.telefono, 0.00
                INTO
                ...
                vmensualidad_act, sDiaCorte, vgrupo, vantiguedad, vbcscore, vscoreprop, vficoscore, vbhscore, vnovencidos1, vnovencidos2,
				vnovencidos3, vnovencidos4, vnovencidos5, vnovencidos6, vcelular, vivatrasp
            	from bdicred:creditossl_tab2 a       -- sd_maecred a   -- 
				...
				left outer join bdicred:sd_maesdoshist mahis1 on(a.num_credito=mahis1.num_credito and mahis1.fecha=add_months(a.fecha_his,-1))
				left outer join bdicred:sd_maesdoshist mahis2 on(a.num_credito=mahis2.num_credito and mahis2.fecha=add_months(a.fecha_his,-2))
				left outer join bdicred:sd_maesdoshist mahis3 on(a.num_credito=mahis3.num_credito and mahis3.fecha=add_months(a.fecha_his,-3))
				left outer join bdicred:sd_maesdoshist mahis4 on(a.num_credito=mahis4.num_credito and mahis4.fecha=add_months(a.fecha_his,-4))
				left outer join bdicred:sd_maesdoshist mahis5 on(a.num_credito=mahis5.num_credito and mahis5.fecha=add_months(a.fecha_his,-5))
				left outer join bdicred:sd_maesdoshist mahis6 on(a.num_credito=mahis6.num_credito and mahis6.fecha=add_months(a.fecha_his,-6))
            ...
            IF NOT EXISTS (SELECT fecha, num_credito FROM bdicred:"informix".sd_sdos_cartera_linea 
                ...
                SELECT
                    ...
                    where empresa = pEmpresa and num_credito = vcredito and fecha_cuota >= cfechavencto and capital_status in ('2','7');
                ...
                INSERT INTO bdicred:"informix".sd_sdos_cartera_linea 
					...
					bhscore, novencidos1, novencidos2, novencidos3, novencidos4, novencidos5, novencidos6, celular, iva_int_trasp)
    ...
    IF pServicio = '2' OR pServicio = '3' THEN      -- Obtiene informacion de Cartera de Prestamo Personal y Reestructura
        ...
        FOREACH
            SELECT a.numcte, a.num_credito, 0 num_tarjeta, (SELECT num_cta FROM bdicred:sd_ctascarg WHERE a.num_credito = num_credito 
                ...
                b.mto_fin_ven_trasp::integer mora_actual, (case when (a.status_cred = 'VP' and b.mto_fin_ven_trasp = 1 ) then 'BA'  
                when (a.status_cred = 'VP' and b.mto_fin_ven_trasp > 1) then 'BT' when (a.status_cred <> 'VP') then a.status_cred end) Status_Cred, 
                ...
                FROM bdicred:sd_amortiza_creditocrd amort WHERE a.empresa = amort.empresa AND a.num_credito = amort.num_credito
                AND amort.fecha_cuota = (SELECT min (fecha_cuota) FROM bdicred:sd_amortiza_creditocrd WHERE amort.empresa = empresa 
                AND amort.num_credito = num_credito AND capital_status IN ('2','7'))) Pago_una_mora, a.sucursal, a.fecha_apertura, b.monto_otorgado, 
                ...
                (Select min(fecha_cuota) from bdicred:sd_amortiza_creditocrd WHERE a.empresa = empresa and a.num_credito = num_credito 
                 and capital_status in ('2','7')) fecha_vencto, round(NVL(b.sdo_intereses,0) * (1+ s.iva),2),
				...
				(select resum.grupo from bdisolic:ss_resum_scor_fin resum where a.num_credito=resum.num_solicitud) grupo,
				trunc((dfecha_hoy - a.fecha_apertura)/30) antiguedad, case when a.num_producto <> '6011' then nvl(scr1.evaluacion,0) else nvl(sRe1.evaluacion,0) end, 
				...
				nvl(mhre5.mto_fin_ven_trasp,0), nvl(mhre6.mto_fin_ven_trasp,0),cel.telefono, 
				nvl((SELECT (nvl(interes_debe,0) - nvl(interes_pagado,0)) + (nvl(iva_debe,0) - nvl(iva_pagado,0)) FROM bdicred:sd_amortiza_creditocrd 
				...
				vgrupo, vantiguedad, vbcscore, vscoreprop, vficoscore, vbhscore, vnovencidos1, vnovencidos2, vnovencidos3, vnovencidos4, vnovencidos5,
				vnovencidos6, vcelular, vivatrasp
            ...
            IF NOT EXISTS (SELECT fecha, num_credito FROM bdicred:"informix".sd_sdos_cartera_linea 
                ...
                SELECT
                    ...
                    where empresa = pEmpresa and num_credito = vcredito and fecha_cuota >= cfechavencto and capital_status in ('1','2','7');
