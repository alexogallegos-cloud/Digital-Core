CREATE PROCEDURE "informix".sp_corresp_genera_reporte(pEmpresa  CHAR(3),
                                                                                                                pEjecutivo  CHAR(8),
                                                                                                                pTipoMov    SMALLINT,
                                                                                                                pPagMov     SMALLINT)

RETURNING   CHAR(6)             AS retorno,        
                            CHAR(100)       AS  mensje_ret,
                            CHAR(20)          AS cuenta,
                             CHAR(16)         AS folio,
                             DECIMAL(18,2)  AS monto,
                             VARCHAR(250)   AS referencia,
                             CHAR(16)             AS num_tarjeta,
                             CHAR(104)          AS  nombre_titular,
                             CHAR(20)             AS num_cte,
                             CHAR(1)                AS reversado,
                             SMALLINT              AS pagina
...
ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
          ...
          RETURN cCodRet, cMensajeRet,  NVL(cCuenta,""), NVL(cFolio,""),NVL(dMonto,""), NVL(cReferencia,""),NVL(cTarjeta,""), NVL(cTitular,""), NVL(cNumcte,""), NVL(cReversado,""), sPagina;
END EXCEPTION;

            --SET DEBUG FILE TO "/respaldosbd/viridiana/sp_corresp_genera_reporte.out";
            --TRACE ON;
	...
	IF NVL(pEmpresa, "") = "" OR NVL(pEjecutivo,"") = "" THEN
		LET cCodRet = "000001";
		LET cMensajeRet = "La empresa o ejecutivo no se ha indicado correctamente.";
		RETURN cCodRet, cMensajeRet,  NVL(cCuenta,""), NVL(cFolio,""),NVL(dMonto,""), NVL(cReferencia,""),NVL(cTarjeta,""), NVL(cTitular,""), NVL(cNumcte,""), NVL(cReversado,""), sPagina;
	END IF;
	...
	END IF;
        ...
        IF pTipoMov IN (0,1) THEN -- Obtiene los movimientos consultados por el ejecutivo en la fecha de hoy.
                    ...
                    FOREACH
                            ...
                            SELECT num_cuenta,
                              ...
                              WHERE empresa = pEmpresa
                                    ...
                                    AND pagina = (CASE WHEN NVL(pPagMov,0) <> 0 THEN pPagMov ELSE pagina END)
                                    AND pagina = (CASE WHEN NVL(pTipoMov,0) = 1 THEN  (select max(pagina) from bdicred:sd_movtos_corresponsal where -- el tipo movto 1 iindica que se obtienen los registros de la última pagina.
                                                                                                                                            empresa = pEmpresa and ejecutivo = pEjecutivo and fecha_insert = dtFechaHoy)
                                                                                                                                ELSE pagina END)

                                    RETURN cCodRet, cMensajeRet,  NVL(cCuenta,""), NVL(cFolio,""),NVL(dMonto,""), NVL(cReferencia,""),NVL(cTarjeta,""), NVL(cTitular,""), NVL(cNumcte,""), NVL(cReversado,""), sPagina WITH RESUME;                           

                    END FOREACH;
            

        ELIF pTipoMov = 2 THEN -- Elimina los movimientos consultados por el ejecutivo el día de hoy.

                    DELETE FROM bdicred:"informix".sd_movtos_corresponsal
                    WHERE empresa = pEmpresa
                          AND ejecutivo = pEjecutivo
                          AND fecha_insert = dtFechaHoy;
        ...
        ELSE
                LET cCodRet = "000003";
                LET cMensajeRet = "Indicador de operación incorrecto.";
                RETURN cCodRet, cMensajeRet,  NVL(cCuenta,""), NVL(cFolio,""),NVL(dMonto,""), NVL(cReferencia,""),NVL(cTarjeta,""), NVL(cTitular,""), NVL(cNumcte,""), NVL(cReversado,""), sPagina;
        END IF;

    IF DBINFO("sqlca.sqlerrd2") = 0 THEN
            LET cCodRet = "000004";
            LET cMensajeRet = "No existen movimientos consultados el día de hoy por el ejecutivo  proporcionado.";
            RETURN cCodRet, cMensajeRet, NVL(cCuenta,""), NVL(cFolio,""),NVL(dMonto,""), NVL(cReferencia,""),NVL(cTarjeta,""), NVL(cTitular,""), NVL(cNumcte,""), NVL(cReversado,""), sPagina;
    END IF;
...
END PROCEDURE
DOCUMENT
"Descripción: Procedimiento que obtiene los movimientos de pagos y depósitos realizados en corresponsal para cuentas de crédito que fueron consultados ",
"                        por un ejecutivo durante el día.",
"Base de datos: bdicred",
"Autor: Viridiana Osobampo A.",
"Fecha: 26-Abr-2011";

CREATE PROCEDURE "informix".aclaraciones_edoctacrd(cEmpresa CHAR(3), cNumCredito CHAR(20), dFechaEmision DATE,
                                                   sNumRegistros SMALLINT)

    RETURNING CHAR(5), DATE, CHAR(20), SMALLINT, SMALLINT, DATE, CHAR(11), DATE, CHAR(255), DECIMAL(14,2);
    ...
    DEFINE cFolioSucursal CHAR(11);
    DEFINE cFechaMovimiento DATE;
    DEFINE cDescripcion CHAR(255);
    DEFINE dcImporte DECIMAL(14,2);
    ...
    --SET DEBUG FILE TO "/tmp/aclaraciones_edoctacrd.out";
    --TRACE ON;

    BEGIN
        ON EXCEPTION SET sSqlErr
            LET cCodRet = sSqlErr;
            RETURN cCodRet, dFechaDeEmision, cNumeroCredito, sSecuencia, sNumLinea, cFechaAclaracion,
                   cFolioSucursal, cFechaMovimiento, cDescripcion, dcImporte;
        END EXCEPTION;

        --Generacion de las Aclaraciones del Estado de Cuenta
        FOREACH
            SELECT fecha_emision, num_credito, secuencia, nlinea, fecha_aclara, folio_suc, fecha_mov,
                   descripcion, importe
            INTO dFechaDeEmision, cNumeroCredito, sSecuencia, sNumLinea, cFechaAclaracion,
                 cFolioSucursal, cFechaMovimiento, cDescripcion, dcImporte
            ...
            RETURN cCodRet, dFechaDeEmision, cNumeroCredito, sSecuencia, sNumLinea, cFechaAclaracion,
                   cFolioSucursal, cFechaMovimiento, cDescripcion, dcImporte WITH RESUME;
        END FOREACH
    END;
END PROCEDURE
DOCUMENT
"Genera las Aclaraciones del Estado de Cuenta de Crédito Reestructurado",
"AUTOR: Iris Arias Zazueta",
"FECHA: 06/08/2009",
"BD: bdicred";

CREATE PROCEDURE "informix".clientes_edoctacrd(cEmpresa CHAR(3), cNumCredito CHAR(20), dFechaHoy DATE)

    RETURNING CHAR(5), DATE, CHAR(20), CHAR(20), CHAR(150), CHAR(200), CHAR(120), CHAR(120), CHAR(80),
			  CHAR(120), CHAR(150), CHAR(120), DATE, CHAR(10), CHAR(13), CHAR(58), CHAR(30), CHAR(30),
			  DECIMAL (18,2);
    ...
    DEFINE iUltimaDireccionCte INTEGER;
    DEFINE cNumCte CHAR(20);
    DEFINE cNombreCte CHAR(150);
    DEFINE cDireccionCn CHAR(120);
    DEFINE cDireccionCol CHAR(120);
    DEFINE cDireccionDel CHAR(120);
    DEFINE cEdoCd CHAR(80);
    DEFINE cNombreSucursal CHAR(120);
    DEFINE cGerenteSucursal CHAR(150);
    DEFINE cTelSucursal CHAR(120);
    DEFINE cCodPostal CHAR(10);
    DEFINE cRfc CHAR(13);
    DEFINE cRuta CHAR(58);
    DEFINE cEntreCalles CHAR(30);
    DEFINE cObservaciones CHAR(30);
    DEFINE dcSaldoActual DECIMAL (18,2);
    ...
    DEFINE iCentroZonas INTEGER;
    DEFINE iJefeGrupoZonas INTEGER;
    DEFINE iSupervisorZonas INTEGER;

    -- INICIALIZACION DE VARIABLES --
    LET sSqlEr = 0;
    LET cCodRet = '000';
    LET cSucursal = '';
    LET iUltimaDireccionCte = 0;
    LET cNumCte = '';
    LET cNombreCte = '';
    LET cDireccionCn = '';
    LET cDireccionCol = '';
    ...
    LET cNombreCalle = '';
    LET iCentroZonas = 0;
    LET iJefeGrupoZonas = 0;
    LET iSupervisorZonas = 0;
	
	--SET DEBUG FILE TO "/tmp/clientes_edoctacrd.out";
    --TRACE ON;

    BEGIN
        ON EXCEPTION SET sSqlEr
            LET cCodRet = sSqlEr;
            RETURN cCodRet, dFechaHoy, NVL(cNumCredito, ""), NVL(cNumCte, ""), NVL(cNombreCte, ""), NVL(cDireccionCn, ""),
				   NVL(cDireccionCol, ""), NVL(cDireccionDel, ""), NVL(cEdoCd, ""), NVL(cNombreSucursal, ""), NVL(cGerenteSucursal, ""),
				   NVL(cTelSucursal, ""), dFechaHoy, NVL(cCodPostal, ""), NVL(cRfc,""), NVL(cRuta, ""), NVL(cEntreCalles, ""),
                   NVL(cObservaciones, ""), dcSaldoActual;
        END EXCEPTION ;

        -- Generacion del Encabezado del Reporte de Movimientos
        SELECT numcte, sucursal INTO cNumCte, cSucursal
        FROM sd_maecredcrd
        ...
        SELECT TRIM(numeroextcalle) || " " || TRIM(numerointcalle), cod_postal, entre_calles, observaciones,
               numerociudad, numerocolonia, numerocalle, numeroextcalle, estado
        INTO cDireccionCn, cCodPostal, cEntreCalles, cObservaciones, sNumCiudad, iNumColonia, iNumCalle,
             cNumExtCalle, cEstado
        FROM bdinteg:si_direcciones_actual
        WHERE numcte  = cNumCte AND tipo_dir = '1';

        SELECT TRIM(nombrecalle) INTO cNombreCalle
        FROM bdinteg:si_catcalles
        WHERE numerocalle = iNumCalle;

        SELECT nombrezona, centro, jefegrupozona, supervisorzona
        INTO cDireccionCol, iCentroZonas, iJefeGrupoZonas, iSupervisorZonas
        FROM bdinteg:si_catzonas
        WHERE numerociudad = sNumCiudad AND numerocolonia = iNumColonia;

        SELECT nombreciudad INTO cDireccionDel
        FROM bdinteg:si_catciudades
        WHERE numerociudad = sNumCiudad;

        SELECT nombre INTO cEdoCd
        FROM bdinteg:si_estados
        WHERE estado = cEstado;

        SELECT nombre, gerente, telefono1
        INTO cNombreSucursal, cGerenteSucursal, cTelSucursal
        FROM bdinteg:si_sucursales
        WHERE empresa = cEmpresa AND sucursal = cSucursal;

        SELECT sdo_cap_insoluto INTO dcSaldoActual
        FROM sd_maesdoscrd
        WHERE num_credito = cNumCredito;

        LET cDireccionCn = cNombreCalle || cDireccionCn;
        LET cRuta = LPAD(sNumCiudad,4,'0')||"/"||
                    LPAD(iCentroZonas,6,'0')||"/"||
                    LPAD(iJefeGrupoZonas,8,'0')||"/"||
                    LPAD(iSupervisorZonas,8,'0')||"/"||
                    LPAD(iNumColonia,4,'0')||"/"||
                    LPAD(iNumCalle,6,'0')||"/"||
                    LPAD(TRIM(cNumExtCalle),5,'0');
...
END PROCEDURE
DOCUMENT
"Genera el Encabezado del Reporte de Movimientos de Crédito Reestructurado",
"FECHA: 04/09/2009",
"AUTOR: Iris Arias Zazueta",
"BD: bdicred";

CREATE PROCEDURE "informix".detalle_edoctacrd(cEmpresa CHAR(3), cNumCredito CHAR(20), dFechaEmision DATE,
                                              sNumRegistros SMALLINT)
...
-----------------------------------------
    ...
    BEGIN
        ON EXCEPTION SET sSqlErr
            LET cCodRet = sSqlErr;
            RETURN cCodRet, dFechaDeEmision, cNumeroCredito, sSecuencia, sNumLinea, dFechaMovimiento,
        ...
        FOREACH
            ...
            RETURN cCodRet, dFechaDeEmision, NVL(cNumeroCredito, ""), NVL(sSecuencia, 0), NVL(sNumLinea, 0),
                   NVL(dFechaMovimiento, ""), NVL(dConcepto, 0), NVL(dCargos, 0), NVL(dAbonos, 0)
              WITH RESUME;
        END FOREACH
    END;
END PROCEDURE
...
CREATE PROCEDURE "informix".encabezado2_edoctacrd(cEmpresa CHAR(3), cNumCredito CHAR(20), dFechaEmision DATE)

    RETURNING CHAR(5), DATE, CHAR(20), DECIMAL(14,2), DECIMAL(14,2), DECIMAL(14,2), CHAR(9), DECIMAL(14,2),
              DECIMAL(14,2), DECIMAL(14,2), DECIMAL(14,2), DECIMAL(14,2), DECIMAL(14,2), DECIMAL(14,2), DATE,
              DATE, DATE, DATE, CHAR(3), DECIMAL(14,2), DATE, DECIMAL(14,2), DECIMAL(14,2);
    ...
    DEFINE dFechaDeEmision DATE;
    DEFINE cNumeroCredito CHAR(20);
    DEFINE dcCapital DECIMAL(14,2);
    DEFINE dcInteres DECIMAL(14,2);
    DEFINE dcIvaInteres DECIMAL(14,2);
    DEFINE cNumeroPago CHAR(9);
    DEFINE dcMontoPago DECIMAL(14,2);
    DEFINE dcCapitalVencido DECIMAL(14,2);
    DEFINE dcInteresVencido DECIMAL(14,2);
    DEFINE dcIvaInteresVencido DECIMAL(14,2);
    DEFINE dcMoratorios DECIMAL(14,2);
    DEFINE dcIvaMoratorios DECIMAL(14,2);
    DEFINE dcPagoTotal DECIMAL(14,2);
    DEFINE dFechaLimite DATE;
    DEFINE dPeriodoInicial DATE;
    DEFINE dPeriodoFinal DATE;
    DEFINE dFechaCorte DATE;
    DEFINE cDiasPeriodo CHAR(3);
    DEFINE dcMontoCredito DECIMAL(14,2);
    DEFINE dFechaOtorgamiento DATE;
    DEFINE dinteresesefecpag DECIMAL(14,2);
    DEFINE dcomefeccargadas DECIMAL(14,2);
    ...
    LET cNumeroPago = '';
    LET dcMontoPago = 0;
    LET dcCapitalVencido = 0;
    LET dcInteresVencido = 0;
    LET dcIvaInteresVencido = 0;
    LET dcMoratorios = 0;
    LET dcIvaMoratorios = 0;
    LET dcPagoTotal = 0;
    LET dFechaLimite = '';
    LET dPeriodoInicial = '';
    LET dPeriodoFinal = '';
    LET dFechaCorte = '';
    LET cDiasPeriodo = '';
    LET dcMontoCredito = 0;
    LET dFechaOtorgamiento = '';
    LET dinteresesefecpag = 0;
    LET dcomefeccargadas = 0;

    --SET DEBUG FILE TO "/tmp/encabezado2_edoctacrd.out";
    --TRACE ON;
    ...
    BEGIN
        ...
        ON EXCEPTION SET sSqlErr
            LET cCodRet = sSqlErr;
            RETURN cCodRet, dFechaDeEmision, cNumeroCredito, dcCapital, dcInteres, dcIvaInteres, cNumeroPago,
                   dcMontoPago, dcCapitalVencido, dcInteresVencido, dcIvaInteresVencido, dcMoratorios,
                   dcIvaMoratorios, dcPagoTotal, dFechaLimite, dPeriodoInicial, dPeriodoFinal, dFechaCorte,
                   cDiasPeriodo, dcMontoCredito, dFechaOtorgamiento,dinteresesefecpag,dcomefeccargadas;
        END EXCEPTION ;

        -- Se genera Encabezado 2 del Estado de Cuenta
        SELECT fecha_emision, num_credito, capital_tc, interes_tc, iva_interes_tc, numero_pago_tc,
               monto_pago, capital_ven_tc, interes_ven_tc, iva_interes_ven_tc, moratorios_tc,
               iva_moratorios_tc, pago_total_tc, fecha_limite_tc, periodo_tc_ini, periodo_tc_fin,
               fecha_corte_tc, dias_periodo_tc, monto_credito_tc, fecha_otorgamiento_tc, intereses_efec_pag,
               comisiones_efec_cargadas
        INTO dFechaDeEmision, cNumeroCredito, dcCapital, dcInteres, dcIvaInteres, cNumeroPago,
             dcMontoPago, dcCapitalVencido, dcInteresVencido, dcIvaInteresVencido, dcMoratorios,
             dcIvaMoratorios, dcPagoTotal, dFechaLimite, dPeriodoInicial, dPeriodoFinal, dFechaCorte,
             cDiasPeriodo, dcMontoCredito, dFechaOtorgamiento ,dinteresesefecpag,dcomefeccargadas
...
END PROCEDURE
DOCUMENT
"Genera el Encabezado 2 del Estado de Cuenta de Crédito Reestructurado",
"AUTOR: Iris Arias Zazueta",
"FECHA: 05/08/2009",
"BD: bdicred";

CREATE PROCEDURE "informix".encabezado_edoctacrd(cEmpresa CHAR(3), cNumCredito CHAR(20), dFechaEmision DATE)

    RETURNING CHAR(5), DATE, CHAR(20),CHAR(20), CHAR(20), CHAR(150), CHAR(120), CHAR(120), CHAR(120), CHAR(80), CHAR(60),
			  CHAR(4), CHAR(120), CHAR(150), CHAR(13), CHAR(120), CHAR(10), CHAR(58), CHAR(30), CHAR(30);
    ...
    DEFINE cAnioValido CHAR(4);
    DEFINE cMesValido CHAR(2);
    ...
    --SET DEBUG FILE TO "/tmp/encabezado_edoctacrd.out";
	--TRACE ON;

	BEGIN
        ON EXCEPTION SET sSqlErr
            LET cCodRet = sSqlErr;
            RETURN cCodRet, dFechaDeEmision, cNumeroCredito,CNum_cta_efec, cNumCte, cNombreCte, cDireccionCn, cDireccionCol, cDireccionDel,
				   cEdoCd, cCveCobranza, cNumSucursal, cNombreSucursal, cGerenteSucursal, cRfc, cTelSucursal, cCodPostal,
				   cRuta, cEntreCalles, cObservaciones;
		END EXCEPTION;

        -- Generacion del Encabezado del Estado de Cuenta

        SELECT fecha_emision, num_credito, num_cta_efec,numcte, nombre_cte, direccion_cn, direccion_col, direccion_del,
			   edo_cd, cl_cobra, sucursal_numero, sucursal_nombre, sucursal_gerente, rfc, sucursal_tel, cp,
			   ruta, entre_calles, observaciones
        INTO dFechaDeEmision, cNumeroCredito,CNum_cta_efec, cNumCte, cNombreCte, cDireccionCn, cDireccionCol, cDireccionDel,
             cEdoCd, cCveCobranza, cNumSucursal, cNombreSucursal, cGerenteSucursal, cRfc, cTelSucursal, cCodPostal,
			 cRuta, cEntreCalles, cObservaciones
        ...
        RETURN cCodRet, dFechaDeEmision, NVL(cNumeroCredito, ""),NVL(CNum_cta_efec, ""), NVL(cNumCte, ""), NVL(cNombreCte, ""), NVL(cDireccionCn, ""),
			   NVL(cDireccionCol, ""), NVL(cDireccionDel, ""), NVL(cEdoCd, ""), NVL(cCveCobranza, ""), NVL(cNumSucursal, ""),
			   NVL(cNombreSucursal, ""), NVL(cGerenteSucursal, ""), NVL(cRfc, ""), NVL(cTelSucursal, ""), NVL(cCodPostal, ""),
			   NVL(cRuta, ""), NVL(cEntreCalles, ""), NVL(cObservaciones, "");
	END;
END PROCEDURE
DOCUMENT
"Genera el Encabezado del Estado de Cuenta de Crédito Reestructurado",
"FECHA: 05/08/2009",
"AUTOR: Iris Arias Zazueta",
"Se modifica el parametro que valida si se muestra el estado de cuenta",
"FECHA: 04/09/2009",
"BD: bdicred";
