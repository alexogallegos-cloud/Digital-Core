CREATE PROCEDURE "informix".sp_vta_excluyecte(pNumCredito char(21),TipoExcepcion char(1), TipoEjecucion Smallint )
returning 
          CHAR(6) as resultado,
          CHAR(100) as mensaje;
	...
	DEFINE  dtFechaHoy2    	DATE;
	
--	SET DEBUG FILE TO "/informix/aacano/sp_vta_excluyecte.out";
--    TRACE ON; 
	
	
--InicializaciÃ³n de variables
	LET cMensajeRet = 'El Proceso de marcar y desbloqueo de los crÃ©ditos a excluir se ejecutÃ³ correctamente';
	LET iCodRet     = 0;
	LET SCodRet     ='000000';
	...
	LET vexcluir    = '';
	LET bEsNumero   = 't';
	LET dtFechaHoy2  = DATE(1);
	
	IF (length(pNumCredito) > 20)  THEN
        LET SCodRet    = '000002';
        LET cMensajeRet   = 'Numero de crÃ©dito no vÃ¡lido, excede del nÃºmero de dÃ­gitos permitidos';
	...
	IF (nvl(pNumCredito,'') <> '')  THEN
		EXECUTE PROCEDURE bdinteg:val_num (pNumCredito) INTO bEsNumero;
			IF bEsNumero = 'f' THEN
				LET SCodRet    = '000003';
				LET cMensajeRet   = 'Numero de crÃ©dito no vÃ¡lido, contiene caracteres no numericos';
				RETURN SCodRet, cMensajeRet;
			END IF;	
    END IF;

	IF (nvl(TipoExcepcion,'') not in ('E','F','B',''))  THEN
			LET SCodRet    = '000004';
			LET cMensajeRet   = 'Motivo exclusiÃ³n no vÃ¡lido, solo permite "E", "F", "B" o ""';
			RETURN SCodRet, cMensajeRet;
    END IF;
	
	IF (nvl(TipoEjecucion,0) not in (0,1))  THEN
			LET SCodRet    = '000005';
			LET cMensajeRet   = 'Tipo EjecuciÃ³n no vÃ¡lido, solo permite 0 Ã³ 1';
			RETURN SCodRet, cMensajeRet;
    END IF;
	
	IF (nvl(pNumCredito,'') <> '' and nvl(TipoExcepcion,'') = '' )  THEN
			LET SCodRet    = '000006';
			LET cMensajeRet   = 'Falta capturar motivo exclusiÃ³n';
			RETURN SCodRet, cMensajeRet;
	ELIF (nvl(pNumCredito,'') = '' and nvl(TipoExcepcion,'') <>'' )  THEN	
			LET SCodRet    = '000007';
			LET cMensajeRet   = 'Falta capturar nÃºmero de crÃ©dito';
			RETURN SCodRet, cMensajeRet;
	ELIF (nvl(pNumCredito,'') = '' and nvl(TipoExcepcion,'') = '' and nvl(TipoEjecucion,0) = 1) THEN	
			LET SCodRet    = '000008';
			LET cMensajeRet   = 'Falta capturar nÃºmero de crÃ©dito y motivo exclusiÃ³n';
			RETURN SCodRet, cMensajeRet;
	ELIF (nvl(pNumCredito,'') <> '' and nvl(TipoExcepcion,'') <> '' and nvl(TipoEjecucion,0) = 0) THEN	
			LET SCodRet    = '000009';
			LET cMensajeRet   = 'Falta capturar Tipo Ejecucion igual a 1 para hacer insert en tabla excluidos';
			RETURN SCodRet, cMensajeRet;			
    END IF;
	...
	IF iCodRet != 0 THEN
		LET SCodRet = iCodRet;
		LET cMensajeRet = 'Error sp_vta_excluyecte en la ejecuciÃ³n del proceso de marcar y desbloqueo de los crÃ©ditos a excluir';
	END IF;
	RETURN SCodRet,cMensajeRet;
	END EXCEPTION;
	...
	select fecha_hoy into dtFechaHoy2 from bdicred:sd_fechas;
	
	If TipoEjecucion = 1 Then
	  insert into  bdicred:sd_ctes_excluidos_vta (num_credito, motivo )
	  values (pNumCredito, TipoExcepcion);
	End if;  
	
	FOREACH
		SELECT a.num_credito, a.motivo, b.fechareporte, b.producto
		INTO vnumcredito, vmotivo, vfecharep, vproducto
		FROM bdicred:sd_ctes_excluidos_vta a, bdicobranza:cb_rep_cart_quebrantar b
		...
		IF 	vmotivo = 'B' THEN
			UPDATE bdicobranza:cb_rep_cart_quebrantar
			SET fecha_baja = dtFechaHoy2
			WHERE num_credito = vnumcredito and FechaReporte =  dtFechaHoy;
		END IF;
		
			--Actualizando en la tabla maecred y maecredcrd los creditos que se desbloquearÃ¡n por excluirse de la venta de cartera
			IF  vproducto IN ('6001','8100','7000','8500') and vmotivo = 'E' then --se incluyen los produsctos 8100 y 7000
				UPDATE bdicred:sd_maecred
					SET id_unidad_prod = null 
					WHERE empresa = '001'
					AND num_credito = vnumcredito;
			ELIF vproducto IN ('6001','8100','7000','8500') and vmotivo = 'F' then 
				UPDATE bdicred:sd_maecred
					SET id_unidad_prod = null, cod_caract_2 = 'DEF' 
					WHERE empresa = '001'
					AND num_credito = vnumcredito;		
			ELIF vproducto IN ('6001','8100','7000','8500') and vmotivo = 'B' then 
				UPDATE bdicred:sd_maecred
					SET id_unidad_prod = 3, campo_trab3 = 'BAJA'
					WHERE empresa = '001'
					AND num_credito = vnumcredito;	
			--AAME RQM 10 393 20150623 Se contemplan los 2 productos nuevos de prestamo personal (7600,7700)
			ELIF vproducto in ('6011','6300','6400','7600','7700','6800') and vmotivo in ('E','F') then 			
				UPDATE bdicred:sd_maecredcrd
					SET id_origen = ''
					WHERE empresa = '001'
					AND num_credito = vnumcredito;
			--AAME RQM 10 393 20150623 Se contemplan los 2 productos nuevos de prestamo personal (7600,7700)
			ELIF vproducto in ('6011','6300','6400','7600','7700','6800') and vmotivo ='B'  then 			
				UPDATE bdicred:sd_maecredcrd
					SET id_origen = '', campo_trab3 = 'BAJA'
					WHERE empresa = '001'
					AND num_credito = vnumcredito;        
			END IF;
	END FOREACH;
...
END PROCEDURE
DOCUMENT
'Se realiza procedimiento para marcar y desbloquear los crÃ©ditos a excluir de la venta de cartera solicitados por el area de operaciones y crÃ©dito',
'AUTOR : Marco Antonio Valenzuela LeÃ³n',
'FECHA : 03/Abril/2013',
'BD    : bdicred',
'Version: 20130506.0957',
'ModificaciÃ³n Se agregaron validaciones a parÃ¡metros de entrada: nÃºmero de crÃ©dito, tipo de excepciÃ³n y tipo ejecuciÃ³n ',
'AUTOR : Marco Antonio Valenzuela LeÃ³n',
'FECHA : 08/Mayo/2013',
'Version: 20140430.1610',
'ModificaciÃ³n Se agregÃ³ producto 6400 - CredinÃ³mina',
'AUTOR : Marco Antonio Valenzuela LeÃ³n',
'FECHA : 30/Abr/2013',
'BD    : bdicred',
'ModificaciÃ³n Se agregÃ³ producto 8100 y 7000 - TDC Oro y Platino',
