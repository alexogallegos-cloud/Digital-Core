CREATE PROCEDURE "informix".sp_desbctasfus_obtenerdoctos_soc(pNumeroCliente CHAR(20), inicia  smallint,pnum_regs  smallint)
RETURNING CHAR(5), CHAR(4), CHAR(35), CHAR(20), SMALLINT, CHAR(10),CHAR(50),CHAR(4),CHAR(4),CHAR(40);
--DECLARACION DE VARIABLES
DEFINE cCodRet    		CHAR(5);
DEFINE cCodDocto  		CHAR(4);
DEFINE cDesDocto  		CHAR(35);
...
LET cDescProducto 		= '';
  ...
  --SET DEBUG FILE TO "/informix/VH/decli/sp_desbctasfus_obtenerdoctos.out";
  --TRACE ON;
...
BEGIN
  ...
  ON EXCEPTION SET iSqlErr
    IF iSqlErr <> 0 THEN
        LET cCodRet = iSqlErr;
        RETURN cCodRet, cCodDocto, cDesDocto, cCuenta, sSecuencia,dFecha,cDescrip,cCuentaValida,cProducto,cDescProducto;
    END IF;
  END EXCEPTION;
	...
	SET ISOLATION TO DIRTY READ;
		...
		FOREACH
			SELECT exp.cod_docto, exp.cuenta, exp.secuencia,docto.descripcion,exp.fecha_alta,exp.descrip2,exp.producto,exp.prod_nombre
			INTO cCodDocto, cCuenta, sSecuencia, cDesDocto,dFecha,cDescrip,cProducto,cDescProducto
			...
			GROUP BY exp.cod_docto, exp.cuenta, exp.secuencia,docto.descripcion,exp.fecha_alta,exp.descrip2,exp.producto,exp.prod_nombre ORDER BY cod_docto,secuencia
		
			SELECT FIRST 1 cliente_tras INTO cValidaNumCte FROM log_fusionclientes
			WHERE cliente_tit=pNumeroCliente and fecha_insert=(SELECT max(fecha_insert) from log_fusionclientes WHERE cliente_tit=pNumeroCliente);

			SELECT FIRST 1 cod_docto INTO cCuentaValida FROM si_fusexpediente
			WHERE cod_docto=cCodDocto AND secuencia=sSecuencia AND cuenta=cCuenta and cliente=cValidaNumCte;

			IF cCuentaValida = '' OR cCuentaValida IS NULL THEN
				LET cCuentaValida = '';
			END IF
			
			LET sContador = sContador +1;
			
				IF sContador > pnum_regs THEN
						CONTINUE FOREACH;
				END IF; 
			IF sContador>inicia THEN      
				RETURN cCodRet, cCodDocto, cDesDocto, cCuenta, sSecuencia,dFecha,cDescrip,cCuentaValida,cProducto,cDescProducto WITH RESUME;    
			END IF
		END FOREACH;    
    IF sContador=0 THEN
        LET cCodRet = "00100";
...
'FECHA: 16/05/2013',
'DESCRIPCION: PROCESO QUE SE ENCARGA DE OBTENER DOCUMENTOS DIGITALIZADOS QUE LE PERTENECEN A UN DETERMINADO CLIENTE',
'BD: bdinteg',
'ASUNTO:		Modificacion',
'ELABORÓ: 		Jose Ernesto Raygoza Villa',
'DESCRIPCIÓN: 	Se genera algunas variables en donde se les asigna la consulta que tienen la instruccion IF EXIST para validarlo directamente de la variable y no de la instuccion IF EXIST',
'FECHA: 		04/09/2013 DSB20130911',
'----------------------------------------------',
'AUTOR: L. Montserrat Leon Amador',
'FECHA: 07/05/2020',
'MODIFICACION: Se realiza clonacion de spl para agregar nuevos retornos (producto, prod_nombre).';

CREATE PROCEDURE "informix".sp_fustraspasotelefonos_soc(pClienteTitular CHAR(20), pClienteTraspasaCtas CHAR(20), pUsuario CHAR(8)) 
		...
		--******************************DEPURA TELEFONOS CLIENTE INCORRECTO ***************************************
			...
			FOREACH   
				...
				IF vcont=0 THEN
					...
					INSERT INTO 
					bdinteg:si_fustelefonos2(empresa, numcte, telefono, tipo_tel, status_tel, secuencia, extension, carrier, canal, contacto, cofetel, fecha_hora, user_insert, movil_fijo, status_stel, verificado, marcatel, fecha_actualiza,tel_confirmado,fech_confirmado)
				...
				ELSE
					...
					IF NOT EXISTS (SELECT {+AVOID_FULL (bdinteg:"informix".si_fustelefonos2)} telefono FROM bdinteg:si_fustelefonos2 WHERE numcte = pClienteTitular AND telefono = vtel AND tipo_tel = cTipo_tel ) THEN
						IF EXISTS (SELECT {+AVOID_FULL (bdinteg:"informix".si_fustelefonos2)} telefono FROM bdinteg:si_fustelefonos2 WHERE numcte = pClienteTitular  AND tipo_tel = cTipo_tel) THEN
							UPDATE 
							{+AVOID_FULL (bdinteg:"informix".si_fustelefonos2)}
							bdinteg:si_fustelefonos2 SET status_tel = 'C' 
                        ...
                        INSERT INTO 
						bdinteg:si_fustelefonos2(empresa, numcte, telefono, tipo_tel, status_tel, secuencia, extension, carrier, canal, contacto, cofetel, fecha_hora, user_insert, movil_fijo, status_stel, verificado, marcatel, fecha_actualiza,tel_confirmado,fech_confirmado)
                        SELECT empresa,pClienteTitular AS numcte,telefono,tipo_tel,'A',(SELECT {+AVOID_FULL (bdinteg:"informix".si_fustelefonos2)} NVL(MAX(secuencia),0)+1 FROM si_fustelefonos2 WHERE numcte = pClienteTitular ) AS secuencia, extension,carrier,canal,contacto,cofetel,fecha_hora,user_insert, movil_fijo, status_stel, verificado, marcatel, fecha_actualiza,tel_confirmado,fech_confirmado       
                        FROM bdinteg:si_telefonos
			...
			FOREACH   
				...
				IF vcont=0 THEN
					...
					IF NOT EXISTS (SELECT {+AVOID_FULL (bdinteg:"informix".si_fustelefonos2)} telefono FROM bdinteg:si_fustelefonos2 WHERE numcte = pClienteTitular AND telefono = vtel AND tipo_tel = cTipo_tel) THEN
						...
						SELECT empresa,numcte,telefono,tipo_tel,'A',(SELECT {+AVOID_FULL (bdinteg:"informix".si_fustelefonos2)} NVL(MAX(secuencia),0)+1 FROM si_fustelefonos2 WHERE numcte = pClienteTitular ) AS secuencia,extension,carrier,canal,contacto,cofetel,fecha_hora,user_insert, movil_fijo, status_stel, verificado, marcatel, fecha_actualiza,tel_confirmado,fech_confirmado    
						FROM bdinteg:si_telefonos
						WHERE numcte = pClienteTitular 
							AND secuencia=vsec;
					END IF;
				ELSE											
					...
					IF NOT EXISTS (SELECT {+AVOID_FULL (bdinteg:"informix".si_fustelefonos2)} telefono FROM bdinteg:si_fustelefonos2 WHERE numcte = pClienteTitular AND telefono = vtel AND tipo_tel = cTipo_tel  AND status_tel = cStatus_tel ) THEN
						IF EXISTS(SELECT {+AVOID_FULL (bdinteg:"informix".si_fustelefonos2)} telefono FROM bdinteg:si_fustelefonos2 WHERE numcte = pClienteTitular AND tipo_tel = cTipo_tel) THEN
							UPDATE 
							{+AVOID_FULL (bdinteg:"informix".si_fustelefonos2)}
							bdinteg:si_fustelefonos2 SET status_tel = 'C' 
		...
		/*	FOREACH   
			...
			*/
			...
			FOREACH 
				...
				INSERT INTO 
				bdinteg:si_telefonos(empresa, numcte, telefono, tipo_tel, status_tel, secuencia, extension, carrier, canal, contacto, cofetel, fecha_hora, user_insert, movil_fijo, status_stel, verificado, marcatel, fecha_actualiza,tel_confirmado,fech_confirmado)
				SELECT 
				{+AVOID_FULL (bdinteg:"informix".si_fustelefonos2)}
