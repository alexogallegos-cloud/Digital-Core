CREATE PROCEDURE "informix".sp_carga_intereses_capitalizados(pEmpresa CHAR(3))
RETURNING
          CHAR(6)   AS resultado,
          CHAR(100) AS mensaje;
...
DEFINE dMonto           DECIMAL(18,5);
DEFINE vsql                          char(200);
DEFINE cNumCredito                   CHAR(20);
DEFINE dfechacuota                   DATE;
DEFINE sCuentaReg   integer;
...
ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
   IF iSqlErr != 0 THEN
      ---COMMIT WORK;
      LET cCodRet= iSqlErr;
      LET cMensajeRet= 'Se generó un ERROR en la CARGA INTERESES VENCIDOS ' || cNumCredito;
...
--SET DEBUG FILE TO "sp_carga_intereses_capitalizados.out";
--TRACE ON;

LET cCodRet = '000';
LET cMensajeRet = 'El proceso de CARGA INTERESES VENCIDOS se realizó correctamente';
...
let sCuentaReg  =0;
/*
CREATE temp TABLE "informix".sd_intereses_vencidos ( 
    num_credito        	CHAR(20) NOT NULL,
    monto      	DECIMAL(18,2) NOT NULL
) with no log;

load from creditos_bt.txt insert into sd_intereses_vencidos; 

BEGIN;
  CREATE INDEX "informix".idx_tmp_credito ON "informix".sd_intereses_vencidos(num_credito) ONLINE;
COMMIT;



SELECT mae.num_credito,inv.monto
FROM bdicred:sd_maecred mae
join bdicred:sd_maesdoshist mas on mas.empresa=mae.empresa and mas.num_credito=mae.num_credito and mas.mto_venc_trasp > 0 and mas.fecha = mdy(month(today),20,year(today))
join bdicred:sd_intereses_vencidos inv on inv.num_credito = mae.num_credito
 WHERE mae.empresa = pEmpresa
   AND mae.num_credito > ''
   AND mae.status_cred  = 'BT'
*/

UPDATE statistics medium FOR TABLE bdicred:sd_intereses_vencidos; 

SELECT mae.num_credito,case when status_cred = 'BT' then inv.monto else 0 end monto
FROM bdicred:sd_maecred mae
join bdicred:sd_maesdoshist mas on mas.empresa=mae.empresa and mas.num_credito=mae.num_credito and mas.mto_venc_trasp > 0 and mas.fecha = mdy(month(today),20,year(today))
left outer join bdicred:sd_intereses_vencidos inv on inv.num_credito = mae.num_credito
 WHERE mae.empresa = '001'
   AND mae.status_cred  in ('AA','BT')
  INTO temp creditos_bt WITH NO LOG;

BEGIN;
CREATE INDEX indice1 ON creditos_bt(num_credito);
COMMIT;

UPDATE statistics medium FOR TABLE creditos_bt; 
...
-- Se obtienen los créditos calificados con corte al 20.
    SELECT num_credito,monto
      INTO cNumCredito,dMonto
      FROM creditos_bt

    IF dMonto IS NOT NULL THEN
        begin;
        UPDATE bdicred:sd_maesdos
           SET mto_venc_tra_int = dMonto
         WHERE empresa = pEmpresa
           AND num_credito = cNumCredito;
        commit;
        let sCuentaReg  =sCuentaReg + 1;
    END IF;

END FOREACH;

drop table creditos_bt;

LET cMensajeRet= 'El proceso de CARGA INTERESES VENCIDOS se realizó correctamente '|| sCuentaReg  ;
...
CREATE PROCEDURE "informix".proyecta_pba(pempresa        char(3),
...
END EXCEPTION;
    ...
    SET DEBUG FILE TO "proyecta_pba.trc";
    TRACE ON;
    ...
    INTO vcat
    FROM sd_param
    WHERE cod_param='321';  --FMV 6-DIC-11: Se cambia codigo para Cat con iva solo Reestructura
...
-- SE LE INCREMENTA 1 AL I.V.A
   LET vIvaMas = vIva + 1;
    ...
    SELECT valor  INTO vTasaMora FROM bdinteg:si_fechavalor
    WHERE tasa = vCodTasaMora  AND
   ...
   IF  apli_plan = 'S' THEN
       LET usuario = usuario;
       delete from sd_proyecta
       where num_solicitud = usuario;
       ...
       from bdicred:sd_definicioncrd where num_producto = producto;
       SELECT valor  INTO wtasa_interes FROM bdinteg:si_fechavalor
       WHERE tasa = vcod_tasa_base  AND
             fecha = (SELECT MAX(fecha)
                      FROM bdinteg:si_fechavalor
      ...
      SELECT valor INTO v_dias_cal_int
      FROM   bdicred:sd_param
      WHERE  cod_param = "24";

      select max(num_solicitud) into proyeccion
      from   sd_proyecta
      where  num_solicitud = usuario;
 ...
 --  execute PROCEDURE "informix".monthadd(wfecha_alta, vmaxmeses-2) 
 --  into cFechaMaxPlazoMax;
   

 --  LET cMontoMaxPlazoMax = round(vabono_fijo * (1 - pow(1 + vtasa_periodo, - vmaxmeses))/vtasa_periodo,2);    

 --  IF cMontoMaxPlazoMax > wmonto_linea THEN
 --      LET vValorPre = (wmonto_linea * vtasa_periodo);
 --  END IF;
...
/*       -- Mel
       IF apli_plan = 'S' THEN
       --   LET vIvaMas = vIva + 1;
          --LET vValorPre = round(vValorPre + ((capital+vmonto_int_par+wmonto_iva) * pow(1 + (vtasa_diario * vdia1),- ciclo)),2);
          LET vValorPre = round(vValorPre + ((capital+vmonto_int_par+wmonto_iva) * pow(1 + ((vtasa_diario + vIvaMas)  * vdia1),- ciclo)),2);
       ELSE
          LET vValorPre = 0;
       END IF;
*/
 ...
 /*  if wadicional != 0 then
      if vdia1 < 18 then
         let vabono_fijo = round((wmonto_linea -pagopropuesto) * (vtasa_periodo / (1 -(1/pow(1+vtasa_periodo,wplazo_linea-1)))),2);
      else
        let vabono_fijo = round((wmonto_linea - pagopropuesto) * (vtasa_periodo / (1 -(1/pow(1+vtasa_periodo,wplazo_linea)))),2);
      end if
      if vabono_fijo > pagopropuesto or ciclo = 1 then
         while vabono_fijo > pagopropuesto  or ciclo = 1
            let wfecha_cambio = vfecha_hoy;
            let wfecha_cambi1 = vfecha_primer;
            let vdia1 = wfecha_cambi1 - wfecha_cambio;
            let wmonto_linea = wmonto_linea - (pagopropuesto * .10);  --10;
            let wplazo_linea = wplazo_linea;
            if vdia1 < 18 then
               let vabono_fijo = round((wmonto_linea - pagopropuesto) * (vtasa_periodo / (1 -(1/pow(1+vtasa_periodo,wplazo_linea - 1)))),2);
            else
               let vabono_fijo = round((wmonto_linea - pagopropuesto) * (vtasa_periodo / (1 -(1/pow(1+vtasa_periodo,wplazo_linea)))),2);
            end if
            if vabono_fijo < ((wmonto_linea - pagopropuesto ) * (vtasa_periodo / (1 -(1/pow(1+vtasa_periodo,wplazo_linea - 1))))) and vdia1 < 18 then
              let vabono_fijo = vabono_fijo + .01;
            end if
            if vabono_fijo < ((wmonto_linea - pagopropuesto ) * (vtasa_periodo / (1 -(1/pow(1+vtasa_periodo,wplazo_linea))))) and vdia1 >= 18 then
               let vabono_fijo = vabono_fijo + .01;
            end if
            if vabono_fijo < pagopropuesto then
               let cicloseguro = 0;
               while cicloseguro < wplazo_linea
                   let ciclo = 0;
                   let wadicional = wmonto_linea;
                   let wfecha_cambio = vfecha_hoy;
                   let wfecha_cambi1 = vfecha_primer;
                   while ciclo < wplazo_linea
                       ...
                       LET vmonto_int_par = round(wadicional * vtasa_diario,2);
                       LET vmonto_int_par = round(vmonto_int_par * vdia1,2);
                       let wmonto_iva = vmonto_int_par  * vIva;
                  ...
                  let cicloseguro = ciclo;
                  if cicloseguro < wplazo_linea then
                     let wplazo_linea = wplazo_linea + 10;
                  end if
              end while
              if wadicional > 0 then
                 let ciclo = 1;
         ...
         let vabono_fijo = trunc( vabono_fijo,0);
     ELSE
        let cicloseguro = 0;
        while cicloseguro < wplazo_linea
             let ciclo = 0;
  ...
  -- let valorfinal = valortotal - wmonto_linea;
   ...
   while ciclo < wplazo_linea and wmonto_linea <> 0
       ...
       insert into sd_proyecta (empresa,num_solicitud,fecha_cuota,capital_cuota,
  ...
  /*    if ciclo=vmaxmeses and wmonto_linea>0 and BanderaCas<>'2' then
        ...
        let wmonto_linea = valortotal-valorfinal;
        let capital=0;
        let wplazo_linea = vmaxmeses;
        let vabono_fijo = pagopropuesto;
        let vfecha_hoy = vfecha_hoyAnt;
        let wfecha_alta = vfecha_primer;
        let BanderaCas='2';
            delete from sd_proyecta
            where num_solicitud = usuario;
  ...
  select count(*) into wplazo_v
      from sd_proyecta
      where num_solicitud = usuario ;
  let wmesespro = wplazo_v;

  select sum(capital_cuota) into capital
  from sd_proyecta
  where num_solicitud = usuario;
  let valorfinal = valortotal - capital;
  ...
  INTO vfecha_hoy
    ...
    foreach
      select fecha_cuota,capital_cuota ,interes_cuota,iva_cuota,plazo,plazo,tasa_interes
          into vfecha      ,vCapital,vInteres, vIva,vMesPro,vValorFin, v_tasa_interes
          from sd_proyecta
          where num_solicitud = trim(usuario)
