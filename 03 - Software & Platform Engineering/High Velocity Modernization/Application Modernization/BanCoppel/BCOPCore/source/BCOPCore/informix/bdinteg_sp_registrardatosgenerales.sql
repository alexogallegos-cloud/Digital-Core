CREATE PROCEDURE "informix".sp_registrardatosgenerales(
    cEmpresa              CHAR(3),
    cFuncion              CHAR(1),
    cNumcte               CHAR(20),
    cSucursal             CHAR(4),
    cEjecutivo            CHAR(8),
    iClaveOpcionPuesto    INTEGER,
    iClaveSubopcionPuesto INTEGER,
    cHabita_en            CHAR(20))

RETURNING   CHAR(5) AS cCodRet;

    DEFINE vSecuencia           SMALLINT;
    DEFINE vTipoIngreso         CHAR(1);
    DEFINE vNombreEmpresa       char(60);
    DEFINE vPuesto              char(3);
    DEFINE vAntiguedad          DECIMAL(4,2);
    DEFINE vNombreDepto         char(40);
    DEFINE vJefeInmediato       char(60);
    DEFINE vIngresosMensuales   money(14,2);
    DEFINE vFechaActual         DATE;
    DEFINE vPuestoEsp           CHAR(2);
    DEFINE vClavePuesto         INTEGER;
    DEFINE vSisCotiza           INTEGER;
    DEFINE vNumEmpLab           INTEGER;
    DEFINE vTipoIngresoExt      INTEGER;
    ...
    LET iSqlerr = 0;
    LET vSecuencia = 0;
    LET vTipoIngreso = 'T'; --El tipo de ingreso siempre debe de ser 'T'
    LET vNombreEmpresa = '';
    LET vPuesto = '';
    LET vAntiguedad = 0;
    LET vNombreDepto = '';
    LET vJefeInmediato = '';
    LET vIngresosMensuales = 0;
    LET vPuestoEsp = '';
    LET vClavePuesto = 0;
    LET vSisCotiza = 0;
    LET vNumEmpLab = 0;
    LET vTipoIngresoExt = 0;
    LET vPeriodicidad = 0;
    ...
    BEGIN
        ...
        --SET DEBUG FILE TO "/tmp/anj/registrarDatosGenerales.sql";
        --TRACE ON;
        ...
        IF vSecuencia IS NULL THEN
            LET vSecuencia = 1;
        ELSE
			-- RAEF: preguntamos si no existe un registro con esta secuencia y con estos datos.
			IF NOT EXISTS (SELECT sec_ingreso
                            FROM bdinteg:"informix".si_ingresos
                            WHERE empresa = cEmpresa AND numcte = cNumcte 
								AND sec_ingreso = vSecuencia AND  tipo_ingreso = vTipoIngreso
								AND ClaveOpcionPuesto = iClaveOpcionPuesto AND ClaveSubopcionPuesto = iClaveSubopcionPuesto) THEN
				
				--RAEF: Obten el resto de datos para realizar el alta del registro nuevo manteniendo datos del anterior.
				SELECT nombre_empresa, ingreso_mensual, tipo_ingreso_ext
				INTO vNombreEmpresa, vIngresosMensuales, vTipoIngresoExt
                FROM bdinteg:"informix".si_ingresos
                WHERE empresa = cEmpresa
				AND numcte = cNumcte
				AND sec_ingreso = vSecuencia;
				
				--Genera nueva secuencia
				LET vSecuencia = vSecuencia + 1;
			ELSE
				-- RAEF: Indica que es una actualizacion de datos
				LET cFuncion = 'C';
			END IF;
        END IF;
		
		IF cFuncion = 'A' THEN	-- RAEF: si es un alta
			EXECUTE PROCEDURE bdinteg:"informix".sp_ingresos(
				cFuncion, cEmpresa, cNumcte, vSecuencia, vTipoIngreso,
				vNombreEmpresa, vPuesto, vAntiguedad, vNombreDepto, vJefeInmediato,
				vIngresosMensuales, cEjecutivo, vFechaActual, vPuestoEsp, vClavePuesto,
				iClaveOpcionPuesto, iClaveSubopcionPuesto, vSisCotiza, vNumEmpLab, vTipoIngresoExt,
				vPeriodicidad) INTO vCodRet;
...
*/
		...
		-- RAEF: se garantiza que se actualice el dato en caso de ser diferente al que ya existe
		IF EXISTS (SELECT * FROM bdinteg:"informix".si_ctepf WHERE numcte = cNumcte ) THEN
							
			UPDATE bdinteg:"informix".si_ctepf
			SET habita_en = cHabita_en
			WHERE empresa = cEmpresa
			AND numcte = cNumcte;
					
			UPDATE bdinteg:"informix".si_ctepf
			SET profesion = iClaveOpcionPuesto
			WHERE empresa = cEmpresa
			AND numcte = cNumcte;
...
END PROCEDURE
DOCUMENT
'FOLIO: RQM 18 159 - 2 OptimizaciÃÂ³n de Clientes y ContrataciÃÂ³n de Productos',
'Descripcion: CreaciÃÂ³n de procedure para el registro de datos generales',
'AUTOR: 98021080 - Hiram Ramirez',
'Fecha: 17/11/2022',
'BDD: bdinteg';

CREATE PROCEDURE "informix".sp_actualiza_info_cliente_opt(cNumCte CHAR(9),cNumSolicitud CHAR(50), cActSexo CHAR(1), cActEdoCiv CHAR(1),cActHabDom CHAR (1), cEscolaridad CHAR(2))
...
LET cHabDom = "";
 ...
 BEGIN
	...
	END IF;
     ...
     --Se agrega validacion para confirmar el correcto guardado del campo escolaridad AEEC
    IF cEscolaridad <> "" THEN
        IF EXISTS(SELECT numcte FROM bdinteg:"informix".si_ctepf WHERE numcte = cNumCte) THEN           
          
		   UPDATE bdinteg:"informix".si_ctepf SET escolaridad = cEscolaridad
           WHERE numcte = cNumCte;
