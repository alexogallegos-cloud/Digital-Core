CREATE PROCEDURE "informix".sp_reporte_banco_mexico(pFecha DATE, pEmpresa char(3),pperiodos SMALLINT)
RETURNING CHAR(6)
...
-- Cuentas básicas para el público en general
        select count(*),nvl(sum(a.sdo_mes_ant),0)
        into deb1009,deb1009_m
        from bdicheq:sc_maenoc a
        INNER JOIN bdicheq:sc_maechq b on (b.cuenta=a.cuenta and b.producto IN ('1400','1700','1300'))
...
--Total de cuentas con TDD (al último día del trimestre) Fisicas
        ...
        from bdicheq:sc_maenoc a
        join bdicheq:sc_maechq b on (b.cuenta=a.cuenta and b.producto <> '1100')
        join bdicheq:sc_tarjeta c on (c.empresa=pEmpresa and c.cuenta=a.cuenta and c.secuencia=1 and c.tipo_tarjeta='T')
...
--Total de cuentas con TDD (al último día del trimestre) Morales
        ...
        join bdicheq:sc_maechq b on (b.cuenta=a.cuenta and b.producto <> '1100')
        join bdicheq:sc_tarjeta c on (c.empresa=pEmpresa and c.cuenta=a.cuenta and c.secuencia=1 and c.tipo_tarjeta='T'
        AND c.numcte IN  (select numcte from bdinteg:si_cliente where tpo_persona<>'01') )
        where a.fecha_alta <= dFechaFin;
...
--Cuentas abiertas con TDD (durante el trimestre)
        ...
        from bdicheq:sc_maenoc a
        join bdicheq:sc_maechq b on (a.cuenta=b.cuenta and b.producto <> '1100')
        join bdicheq:sc_tarjeta c on (c.empresa=pEmpresa and a.cuenta=c.cuenta  and c.secuencia=1 and c.tipo_tarjeta='T')
...
--canceladas con TDD (durante el trimestre) (fisicas)
        ...
        from bdicheq:sc_maenoc a
        join bdicheq:sc_maechq b on (a.cuenta=b.cuenta and b.producto <> '1100' and b.status_cta=2 
                                    and b.fec_ult_mov BETWEEN dFechaIni AND dFechaFin) 
        join bdicheq:sc_tarjeta c on (a.cuenta=c.cuenta and c.tipo_tarjeta='T' and c.secuencia=1)
...
--canceladas con TDD (durante el trimestre)
        ...
        from bdicheq:sc_maenoc a
        join bdicheq:sc_maechq b on (a.cuenta=b.cuenta and b.producto in ('1300','1700') and b.status_cta=2 
        and b.fec_ult_mov BETWEEN dFechaIni AND dFechaFin)
        join bdicheq:sc_tarjeta c on (a.cuenta=c.cuenta and c.tipo_tarjeta='T' and c.secuencia=1)
        where a.empresa=pEmpresa and a.cuenta=b.cuenta;
...
--Tarjetas de débito adicionales
        ...
        join bdicheq:sc_maenoc a on (a.fecha_alta < dFechaFin)
...
--Tarjetas de crédito adicionales
        ...
        SELECT count(*) 
        into cred1746  
        FROM bdicred:sd_tarjeta a, bdicred:sd_maecred b 
...
--Clientes que pagaron el saldo total (en el último mes o corte)
        SET ISOLATION TO DIRTY READ;
            ...
            and codigo_fun in (select cod_fun 
                ...
                where codigo>"")
            and codigo_ref in (7,8,9,10,907,908,909)
            and reversado = 'N'
            ) >= sdo_cap_insoluto
...
--Clientes que pagaron el mínimo (en el último mes o corte)
    ...
    IF vproceso=3 THEN
--Monto pagado por los clientes que liquidan el saldo total al último corte del trimestre (totaleros)
        SET ISOLATION TO DIRTY READ;
            select sum(monto) 
            into cred2402
            from bdicred:sd_movhis 
            ...
            and codigo_fun in (select cod_fun 
                from bdicred:sd_conceptospagomanual 
                where codigo>"")
            and codigo_ref in (7,8,9,10,901,907,908,909)
            and reversado = 'N'
            and num_credito in (
            select num_credito
            from bdicred:sd_maesdoshist a
...
--                AND fechahorainauth BETWEEN dFechaIni AND dFechaFin
                AND fechahorainauth  >= cFechaIni AND fechahorainauth <= cFechaFin 
                and numtarjeta in (select b.num_tarjeta from bdicheq:sc_maechq a 
                                   inner join bdicheq:sc_tarjeta b on (b.empresa='001' 
...
--                AND fechahorainauth BETWEEN dFechaIni AND dFechaFin
   ...
   IF vproceso=7 THEN
        ...
        select sum(case when descripcion matches 'RETIR*CHEQU*' or descripcion matches 'RETIR*MAESTR*' THEN monto ELSE 0 END),
...
--        where keyx>0 and fechaconciliacion BETWEEN dFechaIni AND dFechaFin
        ...
        SET ISOLATION TO DIRTY READ;
        FOREACH
               SELECT COUNT(distinct b.numcuenta),SUBSTR(a.numtarjeta,1,6) 
...
-------genera reporte de sis_pagos comisiones
    IF (select count(*) from bdicred:sd_sispagos_comisiones where fecha_fin=dFechaFin)=0 THEN
        ...
        set isolation to dirty read;
        select count(*),sum(monto_tot) INTO deb_cant_reposicion,deb_monto_reposicion
        ...
        set isolation to dirty read;
        select sum(case when ((codigo_fun='605' and codigo_ref in (2,125,127)) or (codigo_fun in ('033','334','335','336','337') and codigo_ref in (3,5,9,925,926)))
        ...
        from bdicred:sd_movhis a where empresa=pEmpresa 
        and fecha_mov BETWEEN dFechaIni AND dFechaFin
        and num_credito>0 and 
        ((codigo_fun='605' and codigo_ref in (2,125,127)) or 
         (codigo_fun in ('033','334','335','336','337') and codigo_ref in (3,5,9,925,926)) or
        ...
        select sum(monto_tot) into transfer_otrobanco_fisica
        from bdicheq:sc_movhis where empresa=pEmpresa and cuenta>0
        and fech_alt BETWEEN dFechaIni AND dFechaFin
        and transacc in ('3236','3237');

           BEGIN WORK;
                UPDATE bdicred:sd_sispagos_comisiones
