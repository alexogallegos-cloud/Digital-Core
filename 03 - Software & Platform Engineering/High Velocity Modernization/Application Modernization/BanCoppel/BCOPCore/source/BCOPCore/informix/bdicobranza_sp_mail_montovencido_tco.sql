CREATE PROCEDURE "informix".sp_mail_montovencido_tco(pempresa char (3))
returning 
VARCHAR(6)  AS codigo_retorno,
CHAR(80)    AS mensaje_retorno;

--    execute procedure "informix".sp_mail_montovencido_tco('001');
...
define i                integer;
define num              smallint;
define iCount_TCO_MORA1	integer;
define iCount_TCO_MORA2	integer;
define iCount_TCO_MORA3	integer;
define iCount_TCO_MORA4	integer;
define iCount_TCO_MORA5	integer;
DEFINE iCuentasProcesadas     integer;
--DEFINE iCuentasExcluidasXMail integer;
DEFINE iCuentasExcluidasXMail_TCO_MORA1 integer;
DEFINE iCuentasExcluidasXMail_TCO_MORA2 integer;
DEFINE iCuentasExcluidasXMail_TCO_MORA3 integer;
DEFINE iCuentasExcluidasXMail_TCO_MORA4 integer;
DEFINE iCuentasExcluidasXMail_TCO_MORA5 integer;
...
DEFINE iCuentasProcesadas_TCO_MORA1  integer;
DEFINE iCuentasProcesadas_TCO_MORA2  integer;
DEFINE iCuentasProcesadas_TCO_MORA3  integer;
DEFINE iCuentasProcesadas_TCO_MORA4  integer;
DEFINE iCuentasProcesadas_TCO_MORA5  integer;

let P_COD_RET   = '000000';
let P_MENSAJE   ='El proceso de las campañas EMAIL MORAS TDO se realizó correctamente.';
let cCod_ret    = '000000';
let cMensaje    = '';
let cproceso    = '0014';
let SQL_ERR		=0;
...
let i = 0;
LET num = 0;
let iCount_TCO_MORA1	  = 0;
let iCount_TCO_MORA2	  = 0;
let iCount_TCO_MORA3	  = 0;
let iCount_TCO_MORA4	  = 0;
let iCount_TCO_MORA5	  = 0;
let iCuentasProcesadas      = 0;
let iCuentasExcluidasXMail_TCO_MORA1  = 0;
let iCuentasExcluidasXMail_TCO_MORA2  = 0;
let iCuentasExcluidasXMail_TCO_MORA3  = 0;
let iCuentasExcluidasXMail_TCO_MORA4  = 0;
let iCuentasExcluidasXMail_TCO_MORA5  = 0;
let iCuentasExcluidasXSdosVencidos = 0;
let dFechaCarLinea = date(1);
let iOtrasExclusiones = 0;
let sCampana		  = 0;
let cNumProducto 	  = '';
let iCuentasProcesadas_TCO_MORA1 = 0;
let iCuentasProcesadas_TCO_MORA2 = 0;
let iCuentasProcesadas_TCO_MORA3 = 0;
let iCuentasProcesadas_TCO_MORA4 = 0;
let iCuentasProcesadas_TCO_MORA5 = 0;
...
--let vfecha_ant = mdy('02','22','2015');
	...
	insert into bdimnsj:mnsjr_trx_batch(tipo_mensaje,id_mensaje,cliente, fecha_hora_registro,importe1,fecha1,fecha2)
		select  1, 'TCO_MORA1',numcte,current,100,current,current
		from bdinteg:si_cliente
        where numcte in (select substr(valor,num,9) from bdicobranza:cb_param where cod_param in (57));
	insert into bdimnsj:mnsjr_trx_batch(tipo_mensaje,id_mensaje,cliente,fecha_hora_registro,importe1,fecha1,fecha2)
		select  1, 'TCO_MORA2',numcte,current,100,current,current
		from bdinteg:si_cliente
        where numcte in (select substr(valor,num,9) from bdicobranza:cb_param where cod_param in (57));
	insert into bdimnsj:mnsjr_trx_batch(tipo_mensaje,id_mensaje,cliente,fecha_hora_registro,importe1,fecha1,fecha2)
		select  1, 'TCO_MORA3',numcte,current,100,current,current
		from bdinteg:si_cliente
        where numcte in (select substr(valor,num,9) from bdicobranza:cb_param where cod_param in (57));
	insert into bdimnsj:mnsjr_trx_batch(tipo_mensaje,id_mensaje,cliente,fecha_hora_registro,importe1,fecha1,fecha2)
		select  1, 'TCO_MORA4',numcte,current,100,current,current
		from bdinteg:si_cliente
        where numcte in (select substr(valor,num,9) from bdicobranza:cb_param where cod_param in (57));
	insert into bdimnsj:mnsjr_trx_batch(tipo_mensaje,id_mensaje,cliente,fecha_hora_registro,importe1,fecha1,fecha2)
		select  1, 'TCO_MORA5',numcte,current,100,current,current
		from bdinteg:si_cliente
        where numcte in (select substr(valor,num,9) from bdicobranza:cb_param where cod_param in (57));
        let num = num + 10;
	end for
	...
	Foreach
		...
		select cl.fecha, cl.mto_venc_trasp + cl.monto_vencido, cl.interes_iva, cl.moratorio, cl.num_tarjeta,
            ...
            ,(cl.monto_vencido + cl.mto_venc_trasp + cl.moratorio + cl.interes_iva),
		...
		and cte.secuencia = (select max(secuencia) from bdinteg:si_correos 
            ...
            if (pmesvencido = 1) then
                let iCuentasProcesadas_TCO_MORA1 = iCuentasProcesadas_TCO_MORA1 + 1;

                if pmail is null or pmail = '' then 
                   let iCuentasExcluidasXMail_TCO_MORA1 = iCuentasExcluidasXMail_TCO_MORA1 + 1;
                   continue foreach; 
                end if;
                let iCount_TCO_MORA1	  = iCount_TCO_MORA1 + 1;
                let sCampana = 1021;
                call bdimnsj:"informix".sp_registra_evento (1, 'TCO_MORA1' , pnumcte, pnumcredito,vnumtarjeta, 2,
                                '','','','','',pmtofin,0,0,0,0, today, '' )RETURNING P_COD_RET;
            elif (pmesvencido = 2) then
                let iCuentasProcesadas_TCO_MORA2 = iCuentasProcesadas_TCO_MORA2 + 1;

                if pmail is null or pmail = '' then 
                   let iCuentasExcluidasXMail_TCO_MORA2 = iCuentasExcluidasXMail_TCO_MORA2 + 1;
                   continue foreach; 
                end if;
                let iCount_TCO_MORA2	  = iCount_TCO_MORA2 + 1;
                let sCampana = 1022;
                call bdimnsj:"informix".sp_registra_evento (1, 'TCO_MORA2' , pnumcte, pnumcredito,vnumtarjeta, 2,
                                '','','','','',pmtofin,0,0,0,0, today, '' )RETURNING P_COD_RET;
            elif (pmesvencido = 3) then
                let iCuentasProcesadas_TCO_MORA3 = iCuentasProcesadas_TCO_MORA3 + 1;

                if pmail is null or pmail = '' then 
                   let iCuentasExcluidasXMail_TCO_MORA3 = iCuentasExcluidasXMail_TCO_MORA3 + 1;
                   continue foreach; 
                end if;
                let iCount_TCO_MORA3	  = iCount_TCO_MORA3 + 1;
                let sCampana = 1023;
                call bdimnsj:"informix".sp_registra_evento (1, 'TCO_MORA3' , pnumcte, pnumcredito,vnumtarjeta, 2,
                                '','','','','',pmtofin,0,0,0,0, today, '' )RETURNING P_COD_RET;
            elif (pmesvencido = 4) then
                let iCuentasProcesadas_TCO_MORA4 = iCuentasProcesadas_TCO_MORA4 + 1;

                if pmail is null or pmail = '' then 
                   let iCuentasExcluidasXMail_TCO_MORA4 = iCuentasExcluidasXMail_TCO_MORA4 + 1;
                   continue foreach; 
                end if;
                let iCount_TCO_MORA4	  = iCount_TCO_MORA4 + 1;
                let sCampana = 1024;
                call bdimnsj:"informix".sp_registra_evento (1, 'TCO_MORA4' , pnumcte, pnumcredito,vnumtarjeta, 2,
                                '','','','','',0,0,0,0,0, today, '' )RETURNING P_COD_RET;
            elif (pmesvencido = 5) then
                let iCuentasProcesadas_TCO_MORA5 = iCuentasProcesadas_TCO_MORA5 + 1;

                if pmail is null or pmail = '' then 
                   let iCuentasExcluidasXMail_TCO_MORA5 = iCuentasExcluidasXMail_TCO_MORA5 + 1;
                   continue foreach; 
                end if;
                let iCount_TCO_MORA5	  = iCount_TCO_MORA5 + 1;
                let sCampana = 1025;
                call bdimnsj:"informix".sp_registra_evento (1, 'TCO_MORA5' , pnumcte, pnumcredito,vnumtarjeta, 2,
                                '','','','','',0,0,0,0,0, today, '' )RETURNING P_COD_RET;
            else
        ...
        CALL bdicobranza:"informix".sp_latinia_contador_cobranza('TCO_MORA1',iCuentasProcesadas_TCO_MORA1,iCuentasExcluidasXMail_TCO_MORA1) RETURNING P_COD_RET;

    	CALL bdicobranza:"informix".sp_latinia_contador_cobranza('TCO_MORA2',iCuentasProcesadas_TCO_MORA2,iCuentasExcluidasXMail_TCO_MORA2) RETURNING P_COD_RET;

    	CALL bdicobranza:"informix".sp_latinia_contador_cobranza('TCO_MORA3',iCuentasProcesadas_TCO_MORA3,iCuentasExcluidasXMail_TCO_MORA3) RETURNING P_COD_RET;

    	CALL bdicobranza:"informix".sp_latinia_contador_cobranza('TCO_MORA4',iCuentasProcesadas_TCO_MORA4,iCuentasExcluidasXMail_TCO_MORA4) RETURNING P_COD_RET;

    	CALL bdicobranza:"informix".sp_latinia_contador_cobranza('TCO_MORA5',iCuentasProcesadas_TCO_MORA5,iCuentasExcluidasXMail_TCO_MORA5) RETURNING P_COD_RET;

--Genera cifras de control
    if iCuentasProcesadas > 0 then
       let cMensaje = 'TOTAL Cuentas procesadas campaña TCO_MORAS : ' ||iCuentasProcesadas;
       let cMensaje = trim(cMensaje) ||'    EMAILs enviados MORA1: ' ||iCount_TCO_MORA1;
       CALL bdicobranza:"informix".sp_inserta_bitacora_cob(pempresa, cproceso, cCod_ret, trim(cMensaje), '02') RETURNING P_COD_RET;
       let cMensaje = 'EMAILs enviados MORA2: ' ||iCount_TCO_MORA2;
       let cMensaje = trim(cMensaje) ||'    EMAILs enviados MORA3: ' ||iCount_TCO_MORA3;
       CALL bdicobranza:"informix".sp_inserta_bitacora_cob(pempresa, cproceso, cCod_ret, trim(cMensaje), '02') RETURNING P_COD_RET;
       let cMensaje = 'EMAILs enviados MORA4: ' ||iCount_TCO_MORA4;
       let cMensaje = trim(cMensaje) ||'    EMAILs enviados MORA5: ' ||iCount_TCO_MORA5;
       CALL bdicobranza:"informix".sp_inserta_bitacora_cob(pempresa, cproceso, cCod_ret, trim(cMensaje), '02') RETURNING P_COD_RET;
       let cMensaje = '    Cuentas excluidas por error saldos vencidos : ' ||iCuentasExcluidasXSdosVencidos;
       let cMensaje = trim(cMensaje) ||'    Cuentas excluidas por error email : ' ||iCuentasExcluidasXMail_TCO_MORA1 + iCuentasExcluidasXMail_TCO_MORA2 + iCuentasExcluidasXMail_TCO_MORA3 + iCuentasExcluidasXMail_TCO_MORA4 + iCuentasExcluidasXMail_TCO_MORA5;
       CALL bdicobranza:"informix".sp_inserta_bitacora_cob(pempresa, cproceso, cCod_ret, trim(cMensaje), '02') RETURNING P_COD_RET;
