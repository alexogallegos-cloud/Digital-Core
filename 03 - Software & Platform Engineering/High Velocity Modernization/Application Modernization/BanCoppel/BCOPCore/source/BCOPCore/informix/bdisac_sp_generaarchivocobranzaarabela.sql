CREATE PROCEDURE "informix".sp_generaarchivocobranzaarabela(cId_Convenio CHAR(5))
   -- DEFINICION DE VARIABLES
	...
	LET dFecha_Hoy    = '01-01-1990';
    	...
    	--SET DEBUG FILE TO "/informix/noe/sp_generaarchivocobranzaarabela.out";
    	--TRACE ON;
    ...
    BEGIN
		...
		LET cRutaArchAra = REPLACE(cRutaArchAra,'AA',cAnio);
		LET cRutaArchAra = REPLACE(cRutaArchAra,'MM',cMes);
		LET cRutaArchAra = REPLACE(cRutaArchAra,'DD',cDia);
...
--2014.06.02 FRG-f
            ...
            LET cStmt = 'echo "' || cAnioPag || cMesPag || cDiaPag || LPAD(trim(cReferencia1), 8, '0') || LPAD(iImporte_Pago, 11, '0') || cFormaPago || --cDiaPag || cMesPag || cAnioPag ||  
						 LPAD(trim(cSucursal), 4, '0') || cConstante  || '" >> ' || cRutaArchAra;
            SYSTEM cStmt;
        END FOREACH;
        
		
		IF dbinfo("sqlca.sqlerrd2") = 0 THEN 
            LET cStmt = 'echo "' || "0"  || '" >> ' || cRutaArchAra;
            SYSTEM cStmt;                 
        END IF;  
...
DOCUMENT
'AUTOR : Dulce Ramirez',
'DESCRIPCION: Genera el archivo de cobranza Arabela de acuerdo al Layout',
...
CREATE PROCEDURE "informix".sp_generaarchivocobranzastanhome(pConvenio CHAR(5))
...
DEFINE cRutaArchStanHome		CHAR(100);
DEFINE cStmt				CHAR(250);
DEFINE cFolio				CHAR(16);
...
--	SET DEBUG FILE TO '/respaldosbd/hugovaz/sp_generaarchivocobranzastanhome.out';
--	TRACE ON;
	...
	BEGIN
		...
		INTO cRutaArchStanHome,cNombrelegalempresa
		...
		LET cRutaArchStanHome = REPLACE(cRutaArchStanHome,'mm',cMinutoRuta);
		LET cRutaArchStanHome = REPLACE(cRutaArchStanHome,'hh',cHoraRuta);
		LET cRutaArchStanHome = REPLACE(cRutaArchStanHome,'YYYY',cAnio);
		LET cRutaArchStanHome = REPLACE(cRutaArchStanHome,'MM',cMes);
		LET cRutaArchStanHome = REPLACE(cRutaArchStanHome,'DD',cDia);

		--IMPRIME EL ENCABEZADO DEL ARCHIVO
		LET cStmt='echo "' || cNombrelegalempresa || '" >> ' || cRutaArchStanHome;
			SYSTEM cStmt;
		LET cStmt='echo "' || 'FECHA: ' ||cDia||" "||TRIM(cNomes)||" "||cAnio|| '" >> ' || cRutaArchStanHome;
			SYSTEM cStmt;
		...
		FOREACH
			...
			SELECT fecha_pago,
				...
				LET cStmt = 'echo "' || cTpoOperacion || cDiaPago || cMesPago || cAnioPago || cHora || cMinuto || LPAD(TRIM(cSucursal), 5, '0') || LPAD(cFolio, 16, '0') || LPAD(TRIM(cReferencia1), 9, '0') || LPAD(iImporte_Pago, 14, '0') || '" >> ' || cRutaArchStanHome;
				SYSTEM cStmt;
		END FOREACH;
		...
		IF iSumaImporte_Comision <> 0 THEN
			LET cTpoOperacion = 'C';
			LET cStmt = 'echo "' || cTpoOperacion || cDiaPago || cMesPago || cAnioPago ||LPAD(TRIM(cHora), 2, '0') ||LPAD(TRIM(cMinuto), 2, '0') ||LPAD(TRIM(cSucursal), 5, '0') ||LPAD(TRIM(cFolio), 16, '0') || LPAD(TRIM(cReferencia1), 9, '0') || LPAD(iSumaImporte_Comision, 14, '0') || '" >> ' || cRutaArchStanHome;
			SYSTEM cStmt;
		END IF;
		...
		IF iSumaImporte_IVA_Comision <> 0 THEN
			LET cTpoOperacion = 'I';
			LET cStmt = 'echo "' || cTpoOperacion || cDiaPago || cMesPago || cAnioPago ||LPAD(TRIM(cHora), 2, '0') ||LPAD(TRIM(cMinuto), 2, '0') ||LPAD(TRIM(cSucursal), 5, '0')||LPAD(TRIM(cFolio), 16, '0') || LPAD(TRIM(cReferencia1), 9, '0') || LPAD(iSumaImporte_IVA_Comision, 14, '0') || '" >> ' || cRutaArchStanHome;
			SYSTEM cStmt;
		END IF;
		...
		IF iNumPagos <> 0 THEN
			...
			LET cStmt = 'echo "' || cTpoOperacion || cDiaPago || cMesPago || cAnioPago ||LPAD(TRIM(cHora), 2, '0') ||LPAD(TRIM(cMinuto), 2, '0')||LPAD(TRIM(cSucursal), 5, '0')||LPAD(TRIM(cFolio), 16, '0') || LPAD(TRIM(TO_CHAR(iNumPagos)), 9, '0')  ||LPAD(iTotal_Pago, 14, '0')|| '" >> ' || cRutaArchStanHome;
			SYSTEM cStmt;
		END IF;
		...
		IF dbinfo("sqlca.sqlerrd2") = 0 THEN
			LET cTpoOperacion = 'T';
			LET cStmt = 'echo "' || cTpoOperacion || LPAD(TRIM(cDia),2,'0') || LPAD(TRIM(cMes),2,'0') || LPAD(TRIM(cAnio),4,'0') ||LPAD(TRIM(cHora), 2, '0') ||LPAD(TRIM(cMinuto), 2, '0')||LPAD(TRIM(cSucursal), 5, '0')||LPAD(TRIM(cFolio), 16, '0')||LPAD(TRIM(cReferencia1), 9, '0') || LPAD(iTotal_Pago, 14, '0') || '" >> ' || cRutaArchStanHome;
			SYSTEM cStmt;
		END IF;
...
'DESCRIPCIÓN: SP que genera un archivo .txt donde se guardan las operacines de pagos STAN HOME.',
'FOLIO:1454',
'FECHA:12/08/2014',
'VERSIÓN: 201415081421',
