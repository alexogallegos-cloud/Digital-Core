CREATE PROCEDURE "informix".sps_insertarusuario_bei(pNumCliente VARCHAR(9), pUsuario VARCHAR(50),pIdentAdmin	CHAR(30),pIdUsuario INTEGER)
...
-- SOLICITO :
--***************************************************************************************************
-- MODIFICACIÓN:  Permite sobreescribir los nombres de usuario que ya tienen registro y no son reset. 
...
--***************************************************************************************************
	...
	--	SELECT num_cliente INTO vNumCliente FROM bdibei:"informix".bei_usuario WHERE usuario_bei = pUsuario;
...
--***************************************************************************************************
		...
		IF NVL(sIdUsuario, -1) == -1 THEN
			...
			INSERT INTO bdibei:"informix".bei_usuario(id_usuario,usuario_bei, num_cliente, id_status,id_tipo_usuario,f_registro)
			...
			INSERT INTO bdibei:"informix".bei_datos_usuario(id_usuario,nombre,activo)
			VALUES (sIdUsuario,sNombre,'t');

		ELSE
			LET iUsuario=iUsuario||sIdUsuario; --RESETUSUARIO + idusuario
			SELECT usuario_bei
			INTO vUsuario
			...
			IF NVL(vUsuario, iUsuario) <> iUsuario THEN --Si el nombre de usuario en BD no es "RESETUSUARIO + idusuario"
								
				IF  (SELECT COUNT(id_usuario) FROM bdibei:"informix".bei_datos_usuario WHERE id_usuario=sIdUsuario )>= 1 THEN
					----Nuevo---Solo hace el update omite el insert de datos por que ya lo tiene----------------------
