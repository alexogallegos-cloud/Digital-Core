CREATE PROCEDURE "informix".sp_consultainfosolicitud_web(pEmpresa CHAR(3), pNumSolicitudActual CHAR(20) ,pNumcte CHAR(20), pProducto CHAR (4), pPaginacion INTEGER)
...
BEGIN
	...
	--SET DEBUG FILE TO '/pisa/pisabanco/sp_consultainfosolicitud.out';
	--TRACE ON;
	...
	IF NOT EXISTS (SELECT valor_alfabetico FROM bdicobranza:"informix".cb_param_campania WHERE empresa = TRIM(pEmpresa) AND tipo_campania = 59 AND valor_alfabetico = TRIM(pProducto)) THEN
		LET cCodret = '00003'; --EL PRODUCTO DEL PARAMETRO NO ES UN PRODUCTO VÃLIDO
	END IF;
	...
	IF cCodret = '00000' THEN
		...
		IF DBINFO('sqlca.sqlerrd2') = 0 THEN
			LET cCodret = '00005'; --EL NÃMERO DE SOLICITUD NO TIENE REGISTRADAS PREGUNTAS/RESPUESTAS/INGRESO
