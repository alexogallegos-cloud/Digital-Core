CREATE PROCEDURE "informix".sp_cartera_total_ppyr()

RETURNING CHAR(6);
--Creado por: maria elizabeth anzures ibarguen
--28-12-2011    
--Proceso para la generaciÃ³n de archivo cartera total prestamo personal y reestructura
--Modificado por: Jorge Tirado Villa
--12-04-2017 
	--Se agrego los valores de los scores de originacion.
...
BEGIN
    ...
    ON EXCEPTION SET sql_err, isam_err, error_info
        ...
        RETURN cCod_ret;
	END EXCEPTION;
--SET DEBUG FILE TO "/aplicacion/Jorge/Adendum_Reporte_Cartera/tracer.out";
--TRACE ON;
  ...
  select crd.num_credito, fecha_ult_pago fecha_mov
	...
	FOREACH WITH HOLD
        select  a.num_producto,a.num_credito,NVL(a.credito_externo,'0'),a.numcte,a.sucursal,suc.nombre,b.monto_otorgado ,NVL(a.fecha_apertura,DATE(1))
			    ...
			    (CASE WHEN (a.status_cred NOT IN ('AA','BA','E1')) THEN (sdo_intereses + sdo_no_exig + int_tra_no_exig) ELSE 0 END) 
            into Vproducto , Vnum_credito,Vcreditoexterno  , Vnumcte,Vnum_sucursal,vnom_suucursal,vmonto_apertura,vfecha_apertura
                ...
                vmes_vencido,Vfechaultmov,  Vinteres,Vsaldovencido 				
            from CreditosCrd a
...
-- lee el ultimo correo valido
            ...
            select limit 1  nvl(a.ingreso_mensual,0) ,nvl(a.situacion_pago,0)  ,nvl(a.meses_historia,0), a.evalua_cc, a.num_solicitud	,DECODE(res2.flag2creditoicc,'1','EvaluaciÃ³n de Segundo producto de crÃ©dito en adelante','')	 flag2creditoicc		
            into Vingreso_mensual,Vficiencia, Vmeses_historia, Vhit, VcreditoConsulta,cFlag2Credito
            from scorfin a
			left outer join bdisolic:"informix".ss_revision_determinacion res2 on (res2.empresa ='001' and  res2.num_solicitud =a.num_credito)
...
/*
            if (vestatus in ('AA') OR (NVL(atr_aux,-1)=0 and vestatus= 'E1')) then
                let Vsaldo_cierre =  Vcapital_vigente;
        	elif (vestatus in ('BA') OR (NVL(atr_aux,-1) >0 and vestatus in ('E1','E2'))) then
                let Vsaldo_cierre =  vcapital_vigente + vcapital_transitorio;
            elif (vestatus in  ('BT','VP') OR (NVL(atr_aux,-1)>0  and vestatus in ('E3'))) then
...
------------Obtenemos los valores de scores de originacion
			--valida si es prestamo o reestructura
				--si es prestamo, tmar el valor de vnum_credito
			if Vproducto = '6300' then
				let v_selectcredito = Vnum_credito;
			--Si es reestructura, tomar el valor de vcreditoexterno
			else 
	...
	-------------------------BUSCAR ULTIMO MOVIMIENTO DEL CLIENTE-------------------------
            ...
            INSERT INTO sd_cartera_total_PPyR 
                ...
                cd_banco ,tel1 ,tel2 ,tel3 ,ext ,ref_coppel ,eficiencia ,meses_historia ,hit ,secc1 ,secc2, motivo, intereses,interesesvenc ,flag2creditoicc,
				bc_score , score_prop, fico, fico_extended, icc)
            VALUES
                ...
                Vcd_banco , Vtel1 ,Vtel2 ,Vtel3 ,Vext , Vref_coppel ,Vficiencia , Vmeses_historia ,Vhit ,Vsecc1 , Vsecc2, cMotivo, Vinteres,Vsaldovencido ,cFlag2Credito,
				dBcScore,dScoreProp, dFico, dFicoExtended, dIcc);
	...
	---------------------------------------GENERAR ARCHIVO------------------------------------------------------------------
	--ruta para prueba
	--let cruta = '/aplicacion/Jorge/Adendum_Reporte_Cartera/';
	---
