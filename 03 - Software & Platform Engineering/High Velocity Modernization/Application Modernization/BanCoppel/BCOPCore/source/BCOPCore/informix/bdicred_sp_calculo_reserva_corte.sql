CREATE PROCEDURE "informix".sp_calculo_reserva_corte(pEmpresa CHAR(3))
...
--SET DEBUG FILE TO "/RESPALDOS/ipcb/calif/sp_calculo_reserva_corte.out"; 
--TRACE ON;
...
--let dtFechaCalculo = mdy('10','24','2019');
--let dtFechaUltMes = mdy('10','31','2019');
--temporal solo para pruebas	 
...
-- Se obtienen los datos del crédito.
   ...
   SELECT a.num_credito, a.status_cred, a.sucursal, a.num_producto, 
    ...
    WHERE a.empresa          = pEmpresa               
      ...
      AND a.num_producto in( '7000','8500')
...
--			LET dtFechaCorte =  monthadd(dtFechaUltMes,+1) - 1 UNITS DAY;
			LET dtFechaCorte = mdy(month(dtFechaCalculo),1,year(dtFechaCalculo));
			LET dtFechaCorte =  monthadd(dtFechaCorte,+1) - 1 UNITS DAY;
		ELSE
...
--IPCB 26/09/2017 Se agrega validación para programa de apoyo 2017. Activa bandera si el crédito pertenece al programa de apoyo.	 
	IF EXISTS (select * from bdicred:sd_programa_apoyo2017 where num_credito =  sNumeroCredito and bandera = 'A') THEN
		LET cred_apoyo = 1;
	ELSE
		LET cred_apoyo = 0;
	END IF;
     ...
     IF (iHIST > 0 and dFechaAct >= dFechaPeriodoAnterior) then
		IF cred_apoyo = 1 THEN --IPCB 26/09/2017,  Si es programa de apoyo consulta la amortiza_credito_apoyo2017
		   SELECT 
			 ...
			 LEFT OUTER JOIN bdicred:sd_amortiza_credito_apoyo2017 amo on amo.empresa = b.empresa and amo.num_credito = b.num_credito and amo.fecha_cuota = dFechaPeriodoAnterior
			WHERE b.fecha       = dFechaPeriodoAnterior
			  AND b.empresa     = pEmpresa               
			  AND b.num_credito = sNumeroCredito;	
		 ELSE --ELIF cred_apoyo <> 1 then --IPCB 26/09/2017,  Si es programa de apoyo consulta la amortiza_credito
		   SELECT 
				  --NVL(b.monto_otorgado,0) limite_credito,
					(b.monto_financiado + (b.sdo_contab_mora + b.sdo_moratorio) * (select 1 + nvl(iva,0) from bdinteg:si_sucursales where empresa='001' and sucursal = v_sucursal) 
							+ amo.campo_trabajo1 + case when b.int_tra_no_exig - b.sdo_int_anticip >= 0 then  b.int_tra_no_exig - b.sdo_int_anticip else b.int_tra_no_exig end) pago_minimo,  -- pago_query,
