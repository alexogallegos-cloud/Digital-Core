CREATE PROCEDURE "informix".consasigtarjeta(pEmpresa CHAR(3), pNumeroCuenta CHAR(20), pNumeroCliente CHAR(20))
	...
	--SET DEBUG FILE TO "/respaldosbd/Daniela/consasigtarjeta.out";
	--TRACE ON;
	...
	BEGIN
		...
		where numcte = pNumeroCliente AND cuenta = pNumeroCuenta;
		
		IF NVL(vCantReg,0) > 0 THEN

			SELECT DISTINCT num_tarjeta
			  INTO vNumeroTarjeta
			  FROM bdicheq:"informix".sc_tarjeta AS sc_tar
			 WHERE sc_tar.empresa = pEmpresa
			   AND sc_tar.cuenta = pNumeroCuenta
			   AND sc_tar.numcte = pNumeroCliente
			   AND sc_tar.secuencia = (
							SELECT MAX(sc_tar.secuencia) 
							  FROM bdicheq:"informix".sc_tarjeta AS sc_tar 
							 WHERE sc_tar.empresa = pEmpresa 
							   AND sc_tar.cuenta = pNumeroCuenta 
							   AND sc_tar.numcte = pNumeroCliente);
