CREATE PROCEDURE "informix".paseinvhis(pempresa char(3), pfecha date)
       returning char(5);
...
LET vtpcambval =1;
foreach
     select {+INDEX(sv_instrum idx_sv_instrum_emp)}
	        md.sucursal,md.cod_instrum,moneda,transacc,
  ...
  insert into sv_contab
      select {+INDEX(aux_contab idx_aux_contab_emp)}
	  empresa,secuencia,sucursal,succta,ccmayor,ccsub,ccsubsub,
