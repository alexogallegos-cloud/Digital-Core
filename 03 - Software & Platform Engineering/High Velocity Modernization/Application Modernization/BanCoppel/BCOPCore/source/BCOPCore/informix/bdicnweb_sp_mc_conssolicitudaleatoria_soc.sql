CREATE PROCEDURE "informix".sp_mc_conssolicitudaleatoria_soc(pUsuario CHAR(8), pIdFuncion CHAR(10), pStatus CHAR(2), pEjecutivo CHAR(8), pEjecucion CHAR(1),pBandera CHAR(1))
    ...
    BEGIN
		...
		FOREACH
			...
			EXECUTE PROCEDURE bdisolic:"informix".sp_asigna_solicitud_soc(cEmpresa,pEjecutivo,pBandera)
...
CREATE PROCEDURE "informix".sp_cred_grabacomplementarias(pUsuario CHAR(10), pIdFuncion CHAR(10), pCuentas_Medios CHAR(1), pCancelacion_Inac CHAR(1), pCancelacion_Vig CHAR(1), pTiempo_Cancelar CHAR(1), 
			 ...
			 pidcta_concentradora CHAR(1), pcta_concentradora CHAR(20),pRangoini_gracia CHAR(2),pRangofin_gracia CHAR(2))
      RETURNING CHAR(5) AS codRet;
	...
	BEGIN
		...
		INSERT INTO bdicred:"informix".tmp_caracteristicas_complementarias(pcuentas_medios, pcancelacion_inac, pcancelacion_vig, ptiempo_cancelar, pseguro_vida, pcobro_mensualidad, penvio_mesa_control, pid_domiciliacion, pconciliador, phistorico_cred, pperiodo_gracia, pdias_gracia, pcapital_interes, pintereses, pcapital, pestado_cuenta, ptipoedocta, ptipofacturacion, pd_estadocuenta, pemision_estado_cuenta, prango_inicial, prango_final, pid_tipo_facturacion, pn_dias_facturacion, pdia_facturacion, prango_f_fecha_inic, prango_f_fecha_fin, pidcta_concentradora, pcta_concentradora, usuario_insert,prangoini_gracia,prangofin_gracia) 
		VALUES(pCuentas_Medios, pCancelacion_Inac, pCancelacion_Vig, pTiempo_Cancelar, pSeguro_Vida, pCobro_Mensualidad, pEnvio_Mesa_Control, pId_Domiciliacion, pConciliador, pHistorico_Cred, pPeriodo_Gracia, 
			 ...
			 pdia_facturacion, prango_f_fecha_inic, prango_f_fecha_fin, pidcta_concentradora, pcta_concentradora, pUsuario,prangoini_gracia,prangofin_gracia);
...
CREATE PROCEDURE "informix".sp_pm_obtiene_ultimas_img_digi_cte_soc(pUsuario CHAR(8), pIdFuncion CHAR(10),pEmpresa CHAR(3), pNumCte CHAR(10))
		RETURNING CHAR(5) AS codret,
				  CHAR(5) AS CodigoDoc,
				  SMALLINT AS SecuenciaMaxComprobante;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cCodigoDoc,sSecuenciaMaxComprobante;
		END EXCEPTION;

		--SET DEBUG FILE TO '/tmp/mfinis/sp_pm_obtiene_ultimas_img_digi_cte_soc.out';
		--TRACE ON;
		...
		SELECT COUNT(num_solicitud) INTO iSolMC FROM bdisolic:ss_solicitudes WHERE empresa =pEmpresa AND numcte = pNumCte AND status_solicitud ='MC';
		
		IF iSolMC >= 1 THEN
			SELECT MAX(num_solicitud) INTO cSolicitudMC FROM bdisolic:ss_solicitudes WHERE empresa =pEmpresa AND numcte = pNumCte AND status_solicitud ='MC';
		END IF;
        CALL bdidigital@coppelimg_app:sp_obtiene_ult_img_digi_comp_ingreso(pEmpresa,pNumCte, cSolicitudMC) 
		RETURNING cCodRet,cCodigoDoc,sSecuenciaMaxComprobante;
...
'DESCRIPCION: SPL encargado de ejecutar el sp productivo sp_obtiene_ultimas_img_digi_cte_soc',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_sac_catsucaralreporte(pRegistros INTEGER, pRecuperacion INTEGER)
	RETURNING 	CHAR(5) AS codret,
				CHAR(4) AS sucursal,
				CHAR(40) AS nombre,
				CHAR(3) AS plaza,
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_catsucaralreporte.out';
		--TRACE ON;
		...
		FOREACH SELECT 
				SKIP pRegistros FIRST pRecuperacion sucursal, nombre, plaza_cajagen, pais, estado
...
CREATE PROCEDURE "informix".sp_sac_consultareportes(pUsuario CHAR(8), pIdFuncion CHAR(10),pRegistros INTEGER, pRecuperacion INTEGER)
	RETURNING CHAR(5) AS codret,
	...
	DEFINE cTipo CHAR(60);
	DEFINE dFecha DATE;
	DEFINE cHora CHAR(20);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cNombre,cTipo,dFecha,cHora,cUsuario;
		END EXCEPTION;
	 
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_consultareportes.out';
		--TRACE ON;
		...
		nombre_reporte,tipo,fecha_reporte,hr_reporte,usuario_insert
		INTO cNombre,cTipo,dFecha,cHora,cUsuario
		FROM "informix".sw_ctrlgenreportesac
		WHERE usuario_insert = pUsuario
		ORDER BY hr_reporte ASC

		LET iTotal = iTotal+1;
		
		RETURN cCodRet,cNombre,cTipo,dFecha,cHora,cUsuario WITH RESUME;	
		END FOREACH;
...
'FECHA: 23/12/2021',
'MODULO: REPORTES',
'DESCRIPCION: SPL encargado de recuperar los reportes por tipo',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_sac_consultareportes_totales(pUsuario CHAR(8), pIdFuncion CHAR(10))
	RETURNING CHAR(5) AS codret,	
			  INTEGER AS total;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_consultareportes_totales.out';
		--TRACE ON;
...
'FECHA: 23/12/2021',
'MODULO: REPORTES',
'DESCRIPCION: SPL encargado de recuperar el total de reportes por tipo',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_sac_consultasucursalesfiltro(pFiltro CHAR(2), pRegistros INTEGER, pRecuperacion INTEGER)
	RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_consultasucursalesfiltro.out';
		--TRACE ON;
...
CREATE PROCEDURE "informix".sp_sac_montototal(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaInicio DATE, pFechaFinal DATE,pTransaccion1 CHAR(4), pTransaccion2 CHAR(4),pSucursal CHAR(4), pCategoria CHAR(2), pConvenio CHAR(5))
        RETURNING CHAR(5) AS codret,
		...
		ON EXCEPTION SET iSqlErr    
			LET cCodRet = iSqlErr;
			SET LOCK MODE TO WAIT 3;
			UPDATE bdicnweb:"informix".sw_verificasacmontototal
			SET status = 'E', error_proceso = 'S', error_code = TRIM(cCodRet) WHERE usuario = TRIM(pUsuario);
			RETURN cCodRet, iNoRegistros,mMonto;
		END EXCEPTION;          

		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_montototal.out';
		--TRACE ON;
		
		-- SE LIMPIA TABLA POR USUARIO
		SET LOCK MODE TO WAIT 3;
		DELETE FROM bdicnweb:"informix".sw_verificasacmontototal WHERE usuario = TRIM(pUsuario);
		
		-- SE INSERTA A TABLA PARA EL MONITOREO DEL STATUS
		SET LOCK MODE TO WAIT 3; 
		INSERT INTO bdicnweb:"informix".sw_verificasacmontototal(usuario,status,error_proceso,error_code)
		...
		DELETE FROM bdicnweb:"informix".sw_sac_montototal WHERE usuario = TRIM(pUsuario);
	 				
		SELECT c.total_operaciones, c.monto
		INTO iTotalOperacion, mMonto
		FROM
		(SELECT {+INDEX(bdicheq:"informix".sc_movhis  idx_movhisnew4)} SUM(a.monto_tot) as monto, COUNT(*) as total_operaciones, a.sucursal
		 FROM
		(SELECT *
		 FROM bdicheq:sc_movhis 
		 WHERE fech_alt  >= pFechaInicio AND fech_alt <= pFechaFinal
		 AND transacc IN (pTransaccion1,pTransaccion2)
		 AND cancelad <> 'S'
		 AND sucursal = pSucursal
		 UNION
		 SELECT  {+INDEX(bdicheq:"informix".sc_movhis  idx_movhisnew4_old)} {+INDEX(bdisac:"informix".sac_movimientoshistorial  idxsac_movhisfe)} *
		 ...
		 WHERE b.fecha_pago >= pFechaInicio AND b.fecha_pago <= pFechaFinal
		 AND b.numcategoria = pCategoria AND b.numconvenio = pConvenio
		 AND a.cancelad <>'S' AND b.status_cancelado <>'S'
		...
		INSERT INTO bdicnweb:"informix".sw_sac_montototal(usuario,total_operaciones, monto) 
		VALUES(pUsuario,iTotalOperacion, mMonto);
...
CREATE PROCEDURE "informix".sp_sac_reportedetalletransucursal(pUsuario CHAR(8), pIdFuncion CHAR(10),  pFechaIni DATE, pFechaFin DATE, pSucursal CHAR(4), pConvenio CHAR(5),pRutaDescarga CHAR(100),pIdPlantilla CHAR(25),pTituloPlantilla CHAR(255))
    RETURNING CHAR(5) AS codret,
	...
	LET cUsuario              = "";
	LET cFolioSuc             = "";
	LET cNumcategoria         = SUBSTRING(pConvenio FROM 1 FOR 2);
	LET cNumconvenio          = SUBSTRING(pConvenio FROM 3 FOR 3);
	LET cNomConvenio          = "";
	LET cReferencia1          = "";
	LET cReferencia2          = "";
	LET mImportePago         = 0;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_reportedetalletransucursal.out';
		--TRACE ON;
		
		IF pConvenio ='07004' THEN 
		LET cTipo ='REPORTE DETALLE POR SUCURSAL BTS';
		ELIF pConvenio ='07009' THEN 	
		LET cTipo ='REPORTE DETALLE POR SUCURSAL APPRIZA';		
		END IF;

		IF pUsuario = '' OR pIdFuncion = '' OR pRutaDescarga = ''  OR pFechaIni ='' OR pFechaFin='' OR pConvenio ='' OR  pIdPlantilla ='' OR pTituloPlantilla ='' THEN
			LET cCodRet = '00003';			
			EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento('1',TRIM(pIdMensaje),TRIM(pIdPlantilla),pUsuario,'','','1','no exitoso','','','','',cTipo,'','','','','','',1,0,0,0,0,CURRENT,'') INTO cCodRetSp;
	       RETURN cCodRet, cNombreArchivo;
    	END IF;
		...
		IF pConvenio ='07004' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacdetalletranssucbtstmp WHERE usuario = pUsuario;		
			
		ELIF pConvenio = '07009' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacdetalletranssucapptmp WHERE usuario = pUsuario;
		END IF;
		...
		FOREACH 
                ...
                WHERE b.fecha_pago >= pFechaIni
                AND b.fecha_pago <= pFechaFin
			   ...
			   INSERT INTO "informix".sw_cb_reportesacdetalletranssucbtstmp(usuario,cusuario,foliosuc, nom_convenio,referencia1,referencia2,imp_pago, com_conv, iva_com_conv,com_cte,iva_com_cte,formapago,cuentacarg,region) 
			   VALUES(pUsuario,cUsuario, cFolioSuc, cNomConvenio, cReferencia1, cReferencia2, mImportePago, mImpComisionConvenio, mIVAComisionConvenio, mImpComisionCte, mIVAComisionCte, cFormaPago,cCuentaCargo,cRegion);
			   	
			   ELIF pConvenio = '07009' THEN 
               
			   INSERT INTO "informix".sw_cb_reportesacdetalletranssucapptmp(usuario,cusuario, foliosuc, nom_convenio,referencia1,referencia2,imp_pago, com_conv, iva_com_conv,com_cte,iva_com_cte,formapago,cuentacarg,region) 
			   VALUES(pUsuario,cUsuario, cFolioSuc, cNomConvenio, cReferencia1, cReferencia2, mImportePago, mImpComisionConvenio, mIVAComisionConvenio, mImpComisionCte, mIVAComisionCte, cFormaPago,cCuentaCargo,cRegion);
			   END IF;
        ...
        END IF;
		IF pConvenio ='07004' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacdetalletranssucbtstmp WHERE usuario = pUsuario;
		ELIF pConvenio ='07009' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacdetalletranssucapptmp WHERE usuario = pUsuario;		
		END IF;		
		...
		LET cCmd1 ="";
		LET cCmd1 ="SELECT ' ',' ',' ',' ',' ',' ',' ',' ',' ',' ','FECHA:','"||TO_CHAR(dFechaHoy, '%d/%m/%Y') ||"' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ',' ',' ',' ',' ',' ',' ',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ',' ',' ','SISTEMA DE ADMINISTRACION DE CONVENIOS',' ',' ',' ',' ',' ',' ',' ' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		IF pSucursal <>'0000' THEN
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'CONVENIOS:','''"||pConvenio||"',' ','SUCURSAL:','''"||pSucursal||"',' ','REGION:','"||cRegion||"','RANGO DE FECHAS DEL:','"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','AL:','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"' FROM systables  WHERE tabid = 1 ";
		ELSE
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'CONVENIOS:','''"||pConvenio||"',' ','SUCURSAL:','TODAS',' ','REGION:','"||cRegion||"','RANGO DE FECHAS DEL:','"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','AL:','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"' FROM systables  WHERE tabid = 1 ";
		END IF;
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'USUARIO','SECUENCIA','CONVENIO','REFERENCIA 1','REFERENCIA 2','IMPORTE SUCURSAL','COMISION CONVENIO','IVA CONVENIO','COMISION CLIENTE','IVA CLIENTE','FORMA PAGO','CUENTA CARGO/PAGO' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        
     	IF pConvenio ='07004' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT cusuario::CHAR(8), foliosuc::CHAR(16),nom_convenio::CHAR(40),referencia1::CHAR(20),referencia2::CHAR(20),imp_pago::CHAR(20),com_conv::CHAR(20),iva_com_conv::CHAR(20),com_cte::CHAR(20),iva_com_cte::CHAR(20),formapago::CHAR(20),cuentacarg::CHAR(20) FROM ""informix"".sw_cb_reportesacdetalletranssucbtstmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio ='07009' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT cusuario::CHAR(8), foliosuc::CHAR(16),nom_convenio::CHAR(40),referencia1::CHAR(20),referencia2::CHAR(20),imp_pago::CHAR(20),com_conv::CHAR(20),iva_com_conv::CHAR(20),com_cte::CHAR(20),iva_com_cte::CHAR(20),formapago::CHAR(20),cuentacarg::CHAR(20) FROM ""informix"".sw_cb_reportesacdetalletranssucapptmp WHERE usuario ='"||pUsuario||"'"; 
		END IF;
		...
		IF pConvenio ='07004' AND pSucursal <>'0000' THEN 
		LET cNombreArchivo = 'REPORTE_DETALLE_SUCURSAL_REMESAS_BTS_'||TRIM(pSucursal)||'_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio ='07004' AND pSucursal ='0000' THEN 	
		LET cNombreArchivo = 'REPORTE_DETALLE_SUCURSAL_REMESAS_BTS_TODAS_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio ='07009' AND pSucursal <>'0000' THEN 
		LET cNombreArchivo = 'REPORTE_DETALLE_SUCURSAL_REMESAS_APPRIZA_'||TRIM(pSucursal)||'_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio ='07009' AND pSucursal ='0000' THEN		
		LET cNombreArchivo = 'REPORTE_DETALLE_SUCURSAL_REMESAS_APPRIZA_TODAS_'||TRIM(cFechaHoraArchivo)||'.csv';
		END IF;
        ...
        LET cRutaGral = TRIM(pRutaDescarga)||TRIM(cNombreArchivo);
				...
				IF pConvenio ='07004' AND pSucursal <>'0000' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_DETALLE_SUCURSAL_REMESAS_BTS_'||TRIM(pSucursal)||'%');
				ELIF pConvenio ='07004' AND pSucursal ='0000' THEN 	
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_DETALLE_SUCURSAL_REMESAS_BTS_TODAS_%');
				ELIF pConvenio ='07009' AND pSucursal <>'0000' THEN				
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_DETALLE_SUCURSAL_REMESAS_APPRIZA_'||TRIM(pSucursal)||'%');
				ELIF pConvenio ='07009' AND pSucursal ='0000' THEN 	
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_DETALLE_SUCURSAL_REMESAS_BTS_TODAS_%');
				END IF;
        ...
        -- SE ENVIA LA NOTIFICACIÓN DE CORREO ELECTRONICO
        EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento('1',TRIM(pIdMensaje),TRIM(pIdPlantilla),pUsuario,'','','1','con éxito','','','','',cTipo,'','','','','','',1,0,0,0,0,CURRENT,'') INTO cCodRetSp;
	    END IF;
		RETURN cCodRet, cNombreArchivo;
...
'FECHA: 11/11/2021',
'DESCRIPCION: SPL que genera los reportes de conciliación remesas',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_sac_reportesconciliacion(pUsuario CHAR(8), pIdFuncion CHAR(10),pFechaIni DATE, pFechaFin DATE, pConvenio CHAR(5),pRutaDescarga CHAR(100),pIdPlantilla CHAR(25),pTituloPlantilla CHAR(255))
    RETURNING CHAR(5) AS codret,
    CHAR(100) AS reporte_generado;
	...
	DEFINE mCargoPagoRemesa MONEY(16,2);
	DEFINE mSaldoCuentaBTSWU MONEY(16,2) ;
	DEFINE cFolioOperacion CHAR(16);
	DEFINE cClaveEnvio CHAR(11);
	DEFINE cSucursal CHAR(4);
	DEFINE cCajero CHAR(8);
	DEFINE cImportePag CHAR(20);
	DEFINE iTotal INTEGER;
	DEFINE dHoraHoy DATETIME HOUR TO SECOND;
	DEFINE cNombre CHAR(30);	
	DEFINE pIdMensaje CHAR(10);
	...
	DEFINE vsImportePago MONEY ;
	DEFINE vsImportePago_old MONEY ;
	...
	LET	dDia = NULL;
	LET	cFormaPago = '';
	LET	mCargoPagoRemesa = 0;
	LET	mSaldoCuentaBTSWU = 0;
	LET	cFolioOperacion = '';
	LET	cClaveEnvio = '';
	LET	cSucursal = '';
	LET	cCajero = '';
	...
	LET vsImportePago = 0.0;
	LET vsImportePago_old = 0.0;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_reportesconciliacion.out';
		--TRACE ON;
		
		IF pConvenio ='07006' THEN 
		LET cTipo ='REPORTE DE CONCILIACION WU';
		ELIF pConvenio ='07007' THEN 	
		LET cTipo ='REPORTE DE CONCILIACION OV';
		ELIF pConvenio = '07008' THEN 		
		LET cTipo='REPORTE DE CONCILIACION VIGO';
		ELIF pConvenio = '07004' THEN 
		LET cTipo='REPORTE DE CONCILIACION BTS';
		ELIF pConvenio = '07009' THEN 		
		LET cTipo='REPORTE DE CONCILIACION APPRIZA';
		END IF;
		...
		IF pConvenio ='07006' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacconciliacionwutmp WHERE usuario = pUsuario;

		ELIF pConvenio ='07007' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacconciliacionovtmp WHERE usuario = pUsuario;
		
		ELIF pConvenio = '07008' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacconciliacionvgtmp WHERE usuario = pUsuario;
		
		ELIF pConvenio = '07004' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacconciliacionbtstmp WHERE usuario = pUsuario;
		
		ELIF pConvenio = '07009' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacconciliacionapptmp WHERE usuario = pUsuario;
		END IF;
		
		DELETE FROM "informix".sw_ctrlgenreportesac WHERE usuario_insert = pUsuario AND fecha_reporte <= TODAY-1;
		...
		AND transacc_suc = cTransacc_Suc ) 
           INTO dDia, cFormaPago, mCargoPagoRemesa,  mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero, cImportePag
		...
		INSERT INTO "informix".sw_cb_reportesacconciliacionwutmp(usuario, dia, forma_pago, cargo_pago_remesa, saldo_cuenta_wu, folio_operacion, clave_envio,sucursal,cajero,importe_pago) 
		VALUES(pUsuario,dDia, cFormaPago, mCargoPagoRemesa,  mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero, cImportePag);
	
		ELIF pConvenio ='07007' THEN 
		
		INSERT INTO "informix".sw_cb_reportesacconciliacionovtmp(usuario, dia, forma_pago, cargo_pago_remesa, saldo_cuenta_wu, folio_operacion, clave_envio,sucursal,cajero,importe_pago) 
		VALUES(pUsuario,dDia, cFormaPago, mCargoPagoRemesa,  mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero, cImportePag);
		
		ELIF pConvenio = '07008' THEN 
		
		INSERT INTO "informix".sw_cb_reportesacconciliacionvgtmp(usuario, dia, forma_pago, cargo_pago_remesa, saldo_cuenta_wu, folio_operacion, clave_envio,sucursal,cajero,importe_pago) 
		VALUES(pUsuario,dDia, cFormaPago, mCargoPagoRemesa,  mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero, cImportePag);
		...
		--SE OBTIENE EL MONTO TOTAL TANTO DE CHEQUES + CHEQUES_OLD
		SELECT NVL(SUM(monto_tot), 0) INTO vsImportePago FROM bdicheq:sc_movhis 
		WHERE fech_alt BETWEEN pFechaIni AND pFechaFin
			AND transacc IN ('1140', '1110')
			AND folio_suc IN(SELECT folio_suc FROM bdisac:sac_movimientoshistorial 
							WHERE numcategoria='07' AND numconvenio='004' 
							AND fecha_pago BETWEEN pFechaIni AND pFechaFin
							AND status_cancelado = 'N');

		SELECT NVL(SUM(monto_tot), 0) INTO vsImportePago_old FROM bdicheq:sc_movhis_old 
		WHERE fech_alt BETWEEN pFechaIni AND pFechaFin
			AND transacc IN ('1140', '1110')
			AND folio_suc IN(SELECT folio_suc FROM bdisac:sac_movimientoshistorial 
							WHERE numcategoria='07' AND numconvenio= '004' 
		...
		LET cImportePag = CAST(vsImportePago + vsImportePago_old AS CHAR (20));
		--SE OBTIENEN TODO EL DETALLE
		FOREACH
		SELECT 	mvs_chq.fech_alt       AS dia,
				(DECODE(forma_pago, '1', 'EFECTIVO', DECODE(forma_pago, '2', 'CARGO EN CUENTA', DECODE(forma_pago, '3', 'MIXTO', DECODE(forma_pago, '4', 'ABONO EN CUENTA', forma_pago))))) AS Forma_pago,
				mvs_chq.monto_tot      AS cargo_pago_remesa,
				mvs_chq.sdo_cuenta     AS saldo_cuenta_bts,
				mvs_chq.folio_suc      AS folio_operacion,
				servicios.referencia1  AS clave_envio,
				mvs_chq.sucursal       AS sucursal,
				mvs_chq.usuario        AS cajero
		INTO dDia, cFormaPago, mCargoPagoRemesa,  mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero
		FROM bdisac:sac_movimientoshistorial servicios, bdicheq:sc_movhis mvs_chq
		WHERE   servicios.numcategoria = '07' AND servicios.numconvenio = '004'
			AND mvs_chq.fech_alt BETWEEN pFechaIni AND pFechaFin
			AND mvs_chq.fech_alt = servicios.fecha_pago
			AND mvs_chq.transacc IN ('1140', '1110')
			AND mvs_chq.empresa = '001'
			AND mvs_chq.cuenta > '00000000000'
			AND mvs_chq.cancelad <> 'S'
			AND servicios.status_cancelado = 'N'
			AND mvs_chq.folio_suc = servicios.folio_suc
			AND mvs_chq.monto_tot = servicios.importe_pago
		UNION ALL
		SELECT 	mvs_chq_old.fech_alt   AS dia_old,
				(DECODE(forma_pago, '1', 'EFECTIVO', DECODE(forma_pago, '2', 'CARGO EN CUENTA', DECODE(forma_pago, '3', 'MIXTO', DECODE(forma_pago, '4', 'ABONO EN CUENTA', forma_pago))))) AS Forma_pago,
				mvs_chq_old.monto_tot  AS cargo_pago_remesa,
				mvs_chq_old.sdo_cuenta AS saldo_cuenta_bts_old,
				mvs_chq_old.folio_suc  AS folio_operacion,
				servicios.referencia1  AS clave_envio,
				mvs_chq_old.sucursal   AS sucursal_old,
				mvs_chq_old.usuario    AS cajero_old
		FROM bdisac:sac_movimientoshistorial servicios,  bdicheq:sc_movhis_old mvs_chq_old
			WHERE   servicios.numcategoria = '07' AND servicios.numconvenio = '004'
			AND mvs_chq_old.fech_alt BETWEEN pFechaIni AND pFechaFin
			AND mvs_chq_old.fech_alt = servicios.fecha_pago
			AND mvs_chq_old.transacc IN ('1140', '1110')
			AND mvs_chq_old.empresa = '001'
			AND mvs_chq_old.cuenta > '00000000000'
			AND mvs_chq_old.cancelad <> 'S'
			AND servicios.status_cancelado = 'N'
			AND mvs_chq_old.folio_suc = servicios.folio_suc
			AND mvs_chq_old.monto_tot = servicios.importe_pago
			ORDER BY dia
		
		INSERT INTO "informix".sw_cb_reportesacconciliacionbtstmp(usuario, dia, forma_pago, cargo_pago_remesa, saldo_cuenta_wu, folio_operacion, clave_envio,sucursal,cajero,importe_pago) 
		VALUES(pUsuario,dDia, cFormaPago, mCargoPagoRemesa,  mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero, cImportePag);
		...
		INTO dDia, cFormaPago, mCargoPagoRemesa,  mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero
		FROM bdisac:sac_movimientoshistorial servicios, bdicheq:sc_movhis mvs_chq
		WHERE   servicios.numcategoria = '07' AND servicios.numconvenio = '009'
			AND mvs_chq.fech_alt BETWEEN pFechaIni AND pFechaFin
			AND mvs_chq.fech_alt = servicios.fecha_pago
			AND mvs_chq.transacc IN ('1355', '1325')
			AND mvs_chq.empresa = '001'
			AND mvs_chq.cuenta > '00000000000'
			AND mvs_chq.cancelad <> 'S'
			AND servicios.status_cancelado = 'N'
		...
		FROM bdisac:sac_movimientoshistorial servicios,  bdicheq:sc_movhis_old mvs_chq_old
			...
			AND mvs_chq_old.fech_alt BETWEEN pFechaIni AND pFechaFin
			AND mvs_chq_old.fech_alt = servicios.fecha_pago
			AND mvs_chq_old.transacc IN ('1355', '1325')
			AND mvs_chq_old.empresa = '001'
			AND mvs_chq_old.cuenta > '00000000000'
			AND mvs_chq_old.cancelad <> 'S'
			AND servicios.status_cancelado = 'N'
		...
		INSERT INTO "informix".sw_cb_reportesacconciliacionapptmp(usuario, dia, forma_pago, cargo_pago_remesa, saldo_cuenta_wu, folio_operacion, clave_envio,sucursal,cajero,importe_pago) 
		VALUES(pUsuario,dDia, cFormaPago, mCargoPagoRemesa,  mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero, cImportePag);
		...
		IF pConvenio ='07006' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacconciliacionwutmp WHERE usuario = pUsuario;
		ELIF pConvenio ='07007' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacconciliacionovtmp WHERE usuario = pUsuario;
		ELIF pConvenio = '07008' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacconciliacionvgtmp WHERE usuario = pUsuario;
		ELIF pConvenio = '07004' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacconciliacionbtstmp WHERE usuario = pUsuario;
		ELIF pConvenio = '07009' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacconciliacionapptmp WHERE usuario = pUsuario;
		END IF;		
        ...
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
			IF pConvenio ='07006' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','CONCILIACION DE PAGOS DE REMESAS WU',' ',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio ='07007' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','CONCILIACION DE PAGOS DE REMESAS OV ',' ',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '07008' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','CONCILIACION DE PAGOS DE REMESAS VIGO ',' ',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '07004' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','CONCILIACION DE PAGOS DE REMESAS BTS ',' ',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '07009' THEN 
            LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','CONCILIACION DE PAGOS DE REMESAS APPRIZA ',' ',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		END IF;	
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'CONVENIO:','''"||pConvenio||"','SUCURSAL:TODAS',' ','RANGO DE FECHAS DEL:','"||pFechaIni||"','AL:','"||pFechaFin||"' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'DIA','FORMA DE PAGO','CARGO PAGO REMESAS','SALDO CUENTA CONVENIO','FOLIO OPERACION','CLAVE ENVIO','NUMERO SUCURSAL','CAJERO' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        
     	IF pConvenio ='07006' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(dia),2,0)||'/'||LPAD(MONTH(dia),2,0)||'/'||YEAR(dia), forma_pago::CHAR(20),cargo_pago_remesa::CHAR(20),saldo_cuenta_wu::CHAR(20),folio_operacion::CHAR(20),clave_envio::CHAR(20),''''||sucursal::CHAR(4),cajero::CHAR(8) FROM ""informix"".sw_cb_reportesacconciliacionwutmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio ='07007' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(dia),2,0)||'/'||LPAD(MONTH(dia),2,0)||'/'||YEAR(dia), forma_pago::CHAR(20),cargo_pago_remesa::CHAR(20),saldo_cuenta_wu::CHAR(20),folio_operacion::CHAR(20),clave_envio::CHAR(20),''''||sucursal::CHAR(4),cajero::CHAR(8) FROM ""informix"".sw_cb_reportesacconciliacionovtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07008' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(dia),2,0)||'/'||LPAD(MONTH(dia),2,0)||'/'||YEAR(dia), forma_pago::CHAR(20),cargo_pago_remesa::CHAR(20),saldo_cuenta_wu::CHAR(20),folio_operacion::CHAR(20),clave_envio::CHAR(20),''''||sucursal::CHAR(4),cajero::CHAR(8) FROM ""informix"".sw_cb_reportesacconciliacionvgtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07004' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(dia),2,0)||'/'||LPAD(MONTH(dia),2,0)||'/'||YEAR(dia), forma_pago::CHAR(20),cargo_pago_remesa::CHAR(20),saldo_cuenta_wu::CHAR(20),folio_operacion::CHAR(20),clave_envio::CHAR(20),''''||sucursal::CHAR(4),cajero::CHAR(8) FROM ""informix"".sw_cb_reportesacconciliacionbtstmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07009' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(dia),2,0)||'/'||LPAD(MONTH(dia),2,0)||'/'||YEAR(dia), forma_pago::CHAR(20),cargo_pago_remesa::CHAR(20),saldo_cuenta_wu::CHAR(20),folio_operacion::CHAR(20),clave_envio::CHAR(20),''''||sucursal::CHAR(4),cajero::CHAR(8) FROM ""informix"".sw_cb_reportesacconciliacionapptmp WHERE usuario ='"||pUsuario||"'"; 
		END IF;
		...
		IF pConvenio ='07006' THEN 
		LET cNombreArchivo = 'REPORTE_CONCILIACION_WU_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo ='REPORTE DE CONCILIACION WU';
		ELIF pConvenio ='07007' THEN 
		LET cNombreArchivo = 'REPORTE_CONCILIACION_OV_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo ='REPORTE DE CONCILIACION OV';
		ELIF pConvenio = '07008' THEN 
		LET cNombreArchivo = 'REPORTE_CONCILIACION_VIGO_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='REPORTE DE CONCILIACION VIGO';
		ELIF pConvenio = '07004' THEN 
		LET cNombreArchivo = 'REPORTE_CONCILIACION_BTS_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='REPORTE DE CONCILIACION BTS';
		ELIF pConvenio = '07009' THEN 
		LET cNombreArchivo = 'REPORTE_CONCILIACION_APP_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='REPORTE DE CONCILIACION APPRIZA';
		END IF;
        ...
        LET cBanDetError = 't';
               ...
               END IF;
				IF pConvenio ='07006' THEN 				
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_CONCILIACION_WU_%');
				ELIF pConvenio ='07007' THEN 			
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_CONCILIACION_OV_%');
				ELIF pConvenio = '07008' THEN 				
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_CONCILIACION_VIGO_%');
				ELIF pConvenio = '07004' THEN 				
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_CONCILIACION_BTS_%');
				ELIF pConvenio = '07009' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_CONCILIACION_APP_%');
				END IF;
			    INSERT INTO "informix".sw_ctrlgenreportesac(nombre_reporte,fecha_reporte,hr_reporte,usuario_insert,tipo)
			    VALUES(TRIM(cNombreArchivo),dFechaHoy,dHoraHoy,pUsuario,cTipo);
...
CREATE PROCEDURE "informix".sp_sac_reportesedocuenta(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaIni DATE, pFechaFin DATE, pConvenio CHAR(5),pRutaDescarga CHAR(100),pIdPlantilla CHAR(25),pTituloPlantilla CHAR(255))
    RETURNING CHAR(5) AS codret,
    CHAR(100) AS reporte_generado;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_reportesedocuenta.out';
		--TRACE ON;
		
		IF pConvenio ='07006' THEN 
		LET cTipo ='ESTADO DE CUENTA WU';
		ELIF pConvenio ='07007' THEN 		
		LET cTipo ='ESTADO DE CUENTA OV';
		ELIF pConvenio = '07008' THEN 
		LET cTipo='ESTADO DE CUENTA VIGO';
		ELIF pConvenio = '07004' THEN 
		LET cTipo='ESTADO DE CUENTA BTS';
		ELIF pConvenio = '07009' THEN 		
		LET cTipo='ESTADO DE CUENTA APPRIZA';
		END IF;
		...
		IF pConvenio ='07006' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacedoctawutmp WHERE usuario = pUsuario;
	
		ELIF pConvenio ='07007' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacedoctaovtmp WHERE usuario = pUsuario;	
		
		ELIF pConvenio = '07008' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacedoctavgtmp WHERE usuario = pUsuario;
		
		ELIF pConvenio = '07004' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacedoctabtstmp WHERE usuario = pUsuario;

		ELIF pConvenio = '07009' THEN 
			-- SE LIMPIA TABLA POR USUARIO		 
			DELETE FROM "informix".sw_cb_reportesacedoctaapptmp WHERE usuario = pUsuario;
		END IF;
        ...
        WHILE(dFechaIni <= dFechaFin)
			...
			INSERT INTO "informix".sw_cb_reportesacedoctawutmp(usuario, fecha, saldo_inicial, total_abonos, monto_total_abonos, total_cargos, monto_total_cargos, saldo_final,ctaconcentradora,ctaclabe) 
			VALUES(pUsuario,cFechaIni, NVL(cSaldoInicial, '$0.00'), cTotalAbonos, NVL(cMontoTotalAbonos, '$0.00'), cTotalCargos, NVL(cMontoTotalCargos, '$0.00'), NVL(cSaldoFinal, '$0.00'), cCtaConcentradora, cCtaClabe);
	
			ELIF pConvenio ='07007' THEN 
			
			INSERT INTO "informix".sw_cb_reportesacedoctaovtmp(usuario, fecha, saldo_inicial, total_abonos, monto_total_abonos, total_cargos, monto_total_cargos, saldo_final,ctaconcentradora,ctaclabe) 
			VALUES(pUsuario,cFechaIni, NVL(cSaldoInicial, '$0.00'), cTotalAbonos, NVL(cMontoTotalAbonos, '$0.00'), cTotalCargos, NVL(cMontoTotalCargos, '$0.00'), NVL(cSaldoFinal, '$0.00'), cCtaConcentradora, cCtaClabe);
	
			ELIF pConvenio = '07008' THEN 
			
			INSERT INTO "informix".sw_cb_reportesacedoctavgtmp(usuario, fecha, saldo_inicial, total_abonos, monto_total_abonos, total_cargos, monto_total_cargos, saldo_final,ctaconcentradora,ctaclabe) 
			VALUES(pUsuario,cFechaIni, NVL(cSaldoInicial, '$0.00'), cTotalAbonos, NVL(cMontoTotalAbonos, '$0.00'), cTotalCargos, NVL(cMontoTotalCargos, '$0.00'), NVL(cSaldoFinal, '$0.00'), cCtaConcentradora, cCtaClabe);
		...
		--Se asignan a variables los parametros recibidos.
		LET vdFechaIni = pFechaIni;
		LET vdFechaFin = pFechaFin;
		...
		--	2014.01.10 FRG-i
		--	select valor into vsFechaParam from bdicheq:sc_param where codparam='fechcon_movhis';
		select valor into vsFechaParam from bdicheq:sc_param where empresa = '001' and codparam='fechcon_movhis';
		--	2014.01.10 FRG-f
		...
		WHILE(vdFechaIni <= vdFechaFin)
			...
			IF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "01" )THEN
				...
				IF( TRIM(SUBSTRING(vsFechaAnt FROM 9 FOR 2)) = "28" )THEN
					SELECT capvig28 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMesAnt;
				ELIF( TRIM(SUBSTRING(vsFechaAnt FROM 9 FOR 2)) = "29" )THEN
					SELECT capvig29 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMesAnt;
				ELIF( TRIM(SUBSTRING(vsFechaAnt FROM 9 FOR 2)) = "30" )THEN
					SELECT capvig30 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMesAnt;
				ELIF( TRIM(SUBSTRING(vsFechaAnt FROM 9 FOR 2)) = "31" )THEN
					SELECT capvig31 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMesAnt;
				END IF;
				...
				SET ISOLATION TO DIRTY READ;
		--	2013.01.10 FRG-i			
				if vsMes = "01" and TRIM(SUBSTRING(vsFechaAnt FROM 9 FOR 2)) = "31"
					then
						SELECT capvig31 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMesAnt;
		--	2013.01.10 FRG-f
					else
						SELECT capvig1 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				end if;
				--Respectivamente se obtendra el capital vigente de cada dia correspondiente al rango de fechas a procesar.
				SET LOCK MODE TO WAIT 3;
				SET ISOLATION TO DIRTY READ;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "02" )THEN
				IF vsMes <>"01" then
					SELECT capvig1 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
					SELECT capvig2 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				ELSE
		--	2013.01.10 FRG-i
				--	SELECT capvig31 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc_2011 WHERE cuenta = vsCtaConcentradora AND aniomes = '201112';
					SELECT capvig31 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMesAnt;
		--	2013.01.10 FRG-f
					SELECT capvig2 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				END IF;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "03" )THEN
				SELECT capvig2 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig3 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "04" )THEN
				SELECT capvig3 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig4 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "05" )THEN
				SELECT capvig4 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig5 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "06" )THEN
				SELECT capvig5 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig6 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "07" )THEN
				SELECT capvig6 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig7 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "08" )THEN
				SELECT capvig7 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig8 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "09" )THEN
				SELECT capvig8 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig9 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "10" )THEN
				SELECT capvig9 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig10 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "11" )THEN
				SELECT capvig10 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig11 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "12" )THEN
				SELECT capvig11 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig12 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "13" )THEN
				SELECT capvig12 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig13 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "14" )THEN
				SELECT capvig13 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig14 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "15" )THEN
				SELECT capvig14 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig15 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "16" )THEN
				SELECT capvig15 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig16 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "17" )THEN
				SELECT capvig16 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig17 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "18" )THEN
				SELECT capvig17 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig18 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "19" )THEN
				SELECT capvig18 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig19 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "20" )THEN
				SELECT capvig19 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig20 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "21" )THEN
				SELECT capvig20 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig21 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "22" )THEN
				SELECT capvig21 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig22 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "23" )THEN
				SELECT capvig22 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig23 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "24" )THEN
				SELECT capvig23 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig24 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "25" )THEN
					if vsMes <>"12" then
							SELECT capvig24 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
							SELECT capvig25 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
							else
							SELECT capvig24 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
							SELECT capvig24 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
						end if;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "26" )THEN
				if vsMes <>"12" then
							SELECT capvig25 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
							SELECT capvig26 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
						ELSE
							SELECT capvig24 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
							SELECT capvig26 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				END if;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "27" )THEN
				SELECT capvig26 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig27 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "28" )THEN
				SELECT capvig27 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig28 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "29" )THEN
				SELECT capvig28 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig29 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "30" )THEN
				SELECT capvig29 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig30 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			ELIF( TRIM(SUBSTRING(vsFechaIni FROM 4 FOR 2)) = "31" )THEN
				SELECT capvig30 INTO vsSaldoInicial FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
				SELECT capvig31 INTO vsSaldoFinal FROM bdicheq:"informix".sc_sdodiarioc WHERE cuenta = vsCtaConcentradora AND aniomes = vsAnioMes;
			END IF;
		...
		--	IF vsFechaIni >= vsFechaParam THEN
			...
			INSERT INTO "informix".sw_cb_reportesacedoctabtstmp(usuario, fecha, saldo_inicial, total_abonos, monto_total_abonos, total_cargos, monto_total_cargos, saldo_final,ctaconcentradora,ctaclabe) 
			VALUES(pUsuario,vsFechaIni, NVL(vsSaldoInicial, '$0.00'), vsTotalAbonos, NVL(vsMontoTotalAbonos, '$0.00'), vsTotalCargos, NVL(vsMontoTotalCargos, '$0.00'), NVL(vsSaldoFinal, '$0.00'), vsCtaConcentradora, vsCtaClabe);
		...
		SET LOCK MODE TO WAIT 3;
		SET ISOLATION TO DIRTY READ;
		SELECT cuenta_prestadora INTO vsCtaConcentradora FROM bdisac:"informix".sac_convenios WHERE numcategoria = '07' AND numconvenio = '009';
		SELECT numcuentaclabe INTO vsCtaClabe FROM bdisac:"informix".sac_convenios WHERE numcategoria = '07' AND numconvenio = '009';
		--	2014.01.10 FRG-i
		--	select valor into vsFechaParam from bdicheq:sc_param where codparam='fechcon_movhis';
		select valor into vsFechaParam from bdicheq:sc_param where empresa = '001' and codparam='fechcon_movhis';
		--	2014.01.10 FRG-f
		...
		--	IF vsFechaIni >= vsFechaParam THEN
			...
			INSERT INTO "informix".sw_cb_reportesacedoctaapptmp(usuario, fecha, saldo_inicial, total_abonos, monto_total_abonos, total_cargos, monto_total_cargos, saldo_final,ctaconcentradora,ctaclabe) 
			VALUES(pUsuario,vsFechaIni, NVL(vsSaldoInicial, '$0.00'), vsTotalAbonos, NVL(vsMontoTotalAbonos, '$0.00'), vsTotalCargos, NVL(vsMontoTotalCargos, '$0.00'), NVL(vsSaldoFinal, '$0.00'), vsCtaConcentradora, vsCtaClabe);
		...
		IF pConvenio ='07006' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacedoctawutmp WHERE usuario = pUsuario;
		ELIF pConvenio ='07007' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacedoctaovtmp WHERE usuario = pUsuario;
		ELIF pConvenio = '07008' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacedoctavgtmp WHERE usuario = pUsuario;
		ELIF pConvenio = '07004' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacedoctabtstmp WHERE usuario = pUsuario;			
		ELIF pConvenio = '07009' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacedoctaapptmp WHERE usuario = pUsuario;			
		END IF;		
        ...
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		IF pConvenio ='07006' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','ESTADO DE CUENTA WU',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio ='07007' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','ESTADO DE CUENTA OV ',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '07008' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','ESTADO DE CUENTA VIGO',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '07004' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','ESTADO DE CUENTA BTS',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '07009' THEN 
            LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','ESTADO DE CUENTA APPRIZA',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		END IF;		
		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'CONVENIO:','''"||pConvenio||"','SUCURSAL:TODAS','RANGO DE FECHAS DEL:','"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','AL:','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"' FROM systables  WHERE tabid = 1 ";
		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'CUENTA BANCOPPEL NO.:','''"||cCtaConcentradora||"',' ','CUENTA CLABE BANCOPPEL:','"||cCtaClabe||"',' ',' ' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'FECHA','SALDO INICIAL','TOTAL ABONOS','MONTO TOTAL ABONOS','TOTAL CARGOS','MONTO TOTAL CARGOS','SALDO FINAL' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        
     	IF pConvenio ='07006' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT fecha::CHAR(10), saldo_inicial::CHAR(20),total_abonos::CHAR(20),monto_total_abonos::CHAR(20),total_cargos::CHAR(20),monto_total_cargos::CHAR(20),saldo_final::CHAR(20) FROM ""informix"".sw_cb_reportesacedoctawutmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio ='07007' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT fecha::CHAR(10), saldo_inicial::CHAR(20),total_abonos::CHAR(20),monto_total_abonos::CHAR(20),total_cargos::CHAR(20),monto_total_cargos::CHAR(20),saldo_final::CHAR(20) FROM ""informix"".sw_cb_reportesacedoctaovtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07008' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT fecha::CHAR(10), saldo_inicial::CHAR(20),total_abonos::CHAR(20),monto_total_abonos::CHAR(20),total_cargos::CHAR(20),monto_total_cargos::CHAR(20),saldo_final::CHAR(20) FROM ""informix"".sw_cb_reportesacedoctavgtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07004' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT fecha::CHAR(10), saldo_inicial::CHAR(20),total_abonos::CHAR(20),monto_total_abonos::CHAR(20),total_cargos::CHAR(20),monto_total_cargos::CHAR(20),saldo_final::CHAR(20) FROM ""informix"".sw_cb_reportesacedoctabtstmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07009' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT fecha::CHAR(10), saldo_inicial::CHAR(20),total_abonos::CHAR(20),monto_total_abonos::CHAR(20),total_cargos::CHAR(20),monto_total_cargos::CHAR(20),saldo_final::CHAR(20) FROM ""informix"".sw_cb_reportesacedoctaapptmp WHERE usuario ='"||pUsuario||"'"; 
		END IF;
		...
		IF pConvenio ='07006' THEN 
		LET cNombreArchivo = 'ESTADO_CUENTA_WU_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo ='ESTADO DE CUENTA WU';
		ELIF pConvenio ='07007' THEN 
		LET cNombreArchivo = 'ESTADO_CUENTA_OV_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo ='ESTADO DE CUENTA OV';
		ELIF pConvenio = '07008' THEN 
		LET cNombreArchivo = 'ESTADO_CUENTA_VIGO_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='ESTADO DE CUENTA VIGO';
		ELIF pConvenio = '07004' THEN 
		LET cNombreArchivo = 'ESTADO_CUENTA_BTS_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='ESTADO DE CUENTA BTS';
		ELIF pConvenio = '07009' THEN 
		LET cNombreArchivo = 'ESTADO_CUENTA_APP_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='ESTADO DE CUENTA APPRIZA';
		END IF;
        ...
        LET cBanDetError = 't';
               ...
               END IF;
			    IF pConvenio ='07006' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('ESTADO_CUENTA_WU_%');
				ELIF pConvenio ='07007' THEN
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('ESTADO_CUENTA_OV_%');
				ELIF pConvenio = '07008' THEN
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('ESTADO_CUENTA_VIGO_%');
				ELIF pConvenio = '07004' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('ESTADO_CUENTA_BTS_%');
				ELIF pConvenio = '07009' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('ESTADO_CUENTA_APP_%');
				END IF;
			     INSERT INTO "informix".sw_ctrlgenreportesac(nombre_reporte,fecha_reporte,hr_reporte,usuario_insert,tipo)
			    VALUES(TRIM(cNombreArchivo),dFechaHoy,dHoraHoy,pUsuario,cTipo);
...
CREATE PROCEDURE "informix".sp_sac_reportesmensual(pUsuario CHAR(8), pIdFuncion CHAR(10), pPeriodo DATE, pConvenio CHAR(5),pRutaDescarga CHAR(100),pIdPlantilla CHAR(25),pTituloPlantilla CHAR(255))
    RETURNING CHAR(5) AS codret,
    CHAR(100) AS reporte_generado;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_reportesmensual.out';
		--TRACE ON;
		
		IF pConvenio ='07006' THEN 
		LET cTipo ='REPORTE MENSUAL REMESAS WU';
		ELIF pConvenio ='07007' THEN 
		LET cTipo ='REPORTE MENSUAL REMESAS OV';
		ELIF pConvenio = '07008' THEN 
		LET cTipo='REPORTE MENSUAL REMESAS VIGO';
		ELIF pConvenio = '07004' THEN 
		LET cTipo='REPORTE MENSUAL REMESAS BTS';
		ELIF pConvenio = '07009' THEN 
		LET cTipo='REPORTE MENSUAL REMESAS APPRIZA';
		END IF;
		...
		IF pConvenio ='07006' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacmensualwutmp WHERE usuario = pUsuario;
	
		ELIF pConvenio ='07007' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacmensualovtmp WHERE usuario = pUsuario;
	
		ELIF pConvenio = '07008' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacmensualvgtmp WHERE usuario = pUsuario;
		
		ELIF pConvenio = '07004' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacmensualbtstmp WHERE usuario = pUsuario;
			
		ELIF pConvenio = '07009' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacmensualapptmp WHERE usuario = pUsuario;
		END IF;
		...
		WHILE (dFechaIni < dFechaFin)
			...
			SELECT {+INDEX(bdisac:"informix".sac_movimientoshistorial  idxsac_movhisfe)} 
			...
			INSERT INTO "informix".sw_cb_reportesacmensualwutmp(usuario, dia, tot_oper, monto) 
			VALUES(pUsuario,iDia, iTotOperaciones, mMonto);
	
			ELIF pConvenio ='07007' THEN 
			
			INSERT INTO "informix".sw_cb_reportesacmensualovtmp(usuario, dia, tot_oper, monto) 
			VALUES(pUsuario,iDia, iTotOperaciones, mMonto);
	
			ELIF pConvenio = '07008' THEN 
						
			INSERT INTO "informix".sw_cb_reportesacmensualvgtmp(usuario, dia, tot_oper, monto) 
			VALUES(pUsuario,iDia, iTotOperaciones, mMonto);
		...
		WHILE (dFechaIni < dFechaFin)
			...
			SELECT {+INDEX(bdisac:"informix".sac_movimientoshistorial  idxsac_movhisfe)} 
			DAY(dFechaIni) AS DIA , NVL(COUNT(Importe_Pago), 0) AS NUM_MOV, NVL(SUM(Importe_Pago),0) AS MONTO
			INTO iDia, iTotOperaciones, mMonto
			...
			INSERT INTO "informix".sw_cb_reportesacmensualbtstmp(usuario, dia, tot_oper, monto) 
			VALUES(pUsuario,iDia, iTotOperaciones, mMonto);
		...
		WHILE (dFechaIni < dFechaFin)
			...
			INSERT INTO "informix".sw_cb_reportesacmensualapptmp(usuario, dia, tot_oper, monto) 
			VALUES(pUsuario,iDia, iTotOperaciones, mMonto);
		...
		IF pConvenio ='07006' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacmensualwutmp WHERE usuario = pUsuario;
		ELIF pConvenio ='07007' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacmensualovtmp WHERE usuario = pUsuario;
		ELIF pConvenio = '07008' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacmensualvgtmp WHERE usuario = pUsuario;
		ELIF pConvenio = '07004' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacmensualbtstmp WHERE usuario = pUsuario;
		ELIF pConvenio = '07009' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacmensualapptmp WHERE usuario = pUsuario;
		END IF;		
		...
		LET dFechaIni = MONTH(pPeriodo) || '/01/' || YEAR(pPeriodo);
		LET dFechaFin = dFechaIni + INTERVAL (1) MONTH TO MONTH;	
		LET dFechaFin = dFechaFin - INTERVAL (1) DAY TO DAY;	
        IF cCodRet='00000' THEN 
		--GENERACION DE REPORTE	
		LET cCmd1 ="";
		LET cCmd1 ="SELECT ' ',' ','SISTEMA DE ADMINISTRACION DE CONVENIOS ',' ',' ','FECHA:','"||TO_CHAR(dFechaHoy, '%d/%m/%Y') ||"' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		IF pConvenio ='07006' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','CONCILIACION DE PAGOS DE REMESAS WU MENSUAL',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio ='07007' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','CONCILIACION DE PAGOS DE REMESAS OV MENSUAL',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '07008' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','CONCILIACION DE PAGOS DE REMESAS VIGO MENSUAL',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '07004' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','CONCILIACION DE PAGOS DE REMESAS BTS MENSUAL',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '07009' THEN 
            LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','CONCILIACION DE PAGOS DE REMESAS APPRIZA MENSUAL',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		END IF;		
		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'CONVENIO:','''"||pConvenio||"','SUCURSAL:TODAS','RANGO DE FECHAS DEL:','"||TO_CHAR(dFechaIni, '%d/%m/%Y')||"','AL:','"||TO_CHAR(dFechaFin, '%d/%m/%Y')||"' FROM systables  WHERE tabid = 1 ";
		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'DIA','TOTAL DE OPERACIONES','MONTO TOTAL DEL DIA',' ',' ',' ',' ' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        
     	IF pConvenio ='07006' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT dia::CHAR(3), tot_oper::CHAR(20),monto::CHAR(20),' ',' ',' ',' ' FROM ""informix"".sw_cb_reportesacmensualwutmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio ='07007' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT dia::CHAR(3), tot_oper::CHAR(20),monto::CHAR(20),' ',' ',' ',' ' FROM ""informix"".sw_cb_reportesacmensualovtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07008' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT dia::CHAR(3), tot_oper::CHAR(20),monto::CHAR(20),' ',' ',' ',' ' FROM ""informix"".sw_cb_reportesacmensualvgtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07004' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT dia::CHAR(3), tot_oper::CHAR(20),monto::CHAR(20),' ',' ',' ',' ' FROM ""informix"".sw_cb_reportesacmensualbtstmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07009' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT dia::CHAR(3), tot_oper::CHAR(20),monto::CHAR(20),' ',' ',' ',' ' FROM ""informix"".sw_cb_reportesacmensualapptmp WHERE usuario ='"||pUsuario||"'"; 
		END IF;
		...
		IF pConvenio ='07006' THEN 
		LET cNombreArchivo = 'REPORTE_MENSUAL_REMESAS_WU_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo ='REPORTE MENSUAL REMESAS WU';
		ELIF pConvenio ='07007' THEN 
		LET cNombreArchivo = 'REPORTE_MENSUAL_REMESAS_OV_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo ='REPORTE MENSUAL REMESAS OV';
		ELIF pConvenio = '07008' THEN 
		LET cNombreArchivo = 'REPORTE_MENSUAL_REMESAS_VIGO_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='REPORTE MENSUAL REMESAS VIGO';
		ELIF pConvenio = '07004' THEN 
		LET cNombreArchivo = 'REPORTE_MENSUAL_REMESAS_BTS_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='REPORTE MENSUAL REMESAS BTS';
		ELIF pConvenio = '07009' THEN 
		LET cNombreArchivo = 'REPORTE_MENSUAL_REMESAS_APP_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='REPORTE MENSUAL REMESAS APPRIZA';
		END IF;
        ...
        LET cBanDetError = 't';
               ...
               END IF;
			    IF pConvenio ='07006' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_MENSUAL_REMESAS_WU_%');
				ELIF pConvenio ='07007' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_MENSUAL_REMESAS_OV_%');
				ELIF pConvenio = '07008' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_MENSUAL_REMESAS_VIGO_%');
				ELIF pConvenio = '07004' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_MENSUAL_REMESAS_BTS_%');
				ELIF pConvenio = '07009' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_MENSUAL_REMESAS_APP_%');
				END IF;
			     INSERT INTO "informix".sw_ctrlgenreportesac(nombre_reporte,fecha_reporte,hr_reporte,usuario_insert,tipo)
			    VALUES(TRIM(cNombreArchivo),dFechaHoy,dHoraHoy,pUsuario,cTipo);
...
CREATE PROCEDURE "informix".sp_sac_reportesmontototal(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaIni DATE, pFechaFin DATE,pTransaccion1 CHAR(4), pTransaccion2 CHAR(4), pSucursal CHAR(4), pCategoria CHAR(2), pConvenio CHAR(5),pRutaDescarga CHAR(100),pIdPlantilla CHAR(25),pTituloPlantilla CHAR(255))
   RETURNING CHAR(5) AS codret,
    CHAR(100) AS reporte_generado;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_reportesmontototal.out';
		--TRACE ON;
		
		IF pConvenio ='006' THEN 		
		LET cTipo ='MONTO TOTAL PAGADO WU';
		ELIF pConvenio ='007' THEN 
		LET cTipo ='MONTO TOTAL PAGADO OV';
		ELIF pConvenio = '008' THEN 
		LET cTipo='MONTO TOTAL PAGADO VIGO';
		ELIF pConvenio = '004' THEN 
		LET cTipo='MONTO TOTAL PAGADO BTS';
		ELIF pConvenio = '009' THEN 
		LET cTipo='MONTO TOTAL PAGADO APPRIZA';
		END IF;

		IF pUsuario = '' OR pIdFuncion = '' OR pRutaDescarga = ''  OR pFechaIni  ='' OR pFechaFin='' OR pTransaccion1 ='' OR pTransaccion2 ='' OR pCategoria ='' OR pConvenio ='' OR  pIdPlantilla ='' OR pTituloPlantilla ='' THEN
			LET cCodRet = '00003';					
			EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento('1',TRIM(pIdMensaje),TRIM(pIdPlantilla),pUsuario,'','','1','no exitoso','','','','',cTipo,'','','','','','',1,0,0,0,0,CURRENT,'') INTO cCodRetSp;
	       RETURN cCodRet, cNombreArchivo;
    	END IF;
		...
		IF pConvenio ='006' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacmtototwutmp WHERE usuario = pUsuario;
			
		ELIF pConvenio ='007' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacmtototovtmp WHERE usuario = pUsuario;
	
		ELIF pConvenio = '008' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacmtototvgtmp WHERE usuario = pUsuario;
			
		ELIF pConvenio = '004' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacmtototbtstmp WHERE usuario = pUsuario;
							
		ELIF pConvenio = '009' THEN 
			-- SE LIMPIA TABLA POR USUARIO	
			DELETE FROM "informix".sw_cb_reportesacmtototapptmp WHERE usuario = pUsuario;	
		END IF;
		...
		FOREACH 
				...
				(SELECT * FROM bdicheq:sc_movhis WHERE fech_alt  BETWEEN pFechaIni AND pFechaFin AND transacc IN (pTransaccion1,pTransaccion2) AND cancelad <> 'S'
				UNION
				SELECT * FROM bdicheq:sc_movhis_old WHERE fech_alt  BETWEEN pFechaIni AND pFechaFin AND transacc IN (pTransaccion1,pTransaccion2) AND cancelad <> 'S') AS a
				...
				AND a.monto_tot = b.importe_pago AND b.fecha_pago BETWEEN pFechaIni AND pFechaFin AND b.numcategoria = pCategoria AND b.numconvenio = pConvenio
				...
				IF pConvenio ='006' THEN			
					INSERT INTO bdicnweb:"informix".sw_cb_reportesacmtototwutmp(usuario, sucursal, caja_general, entidad, total_operacion, monto) 
					VALUES(pUsuario,cSucursal, cCajaGeneral, cEntidad, iTotalOperacion, mMonto);
				ELIF pConvenio ='007' THEN 
					INSERT INTO bdicnweb:"informix".sw_cb_reportesacmtototovtmp(usuario, sucursal, caja_general, entidad, total_operacion, monto) 
					VALUES(pUsuario,cSucursal, cCajaGeneral, cEntidad, iTotalOperacion, mMonto);
				ELIF pConvenio = '008' THEN 
					INSERT INTO bdicnweb:"informix".sw_cb_reportesacmtototvgtmp(usuario, sucursal, caja_general, entidad, total_operacion, monto) 
					VALUES(pUsuario,cSucursal, cCajaGeneral, cEntidad, iTotalOperacion, mMonto);
				ELIF pConvenio = '004' THEN 
					INSERT INTO bdicnweb:"informix".sw_cb_reportesacmtototbtstmp(usuario, sucursal, caja_general, entidad, total_operacion, monto) 
					VALUES(pUsuario,cSucursal, cCajaGeneral, cEntidad, iTotalOperacion, mMonto);
				ELIF pConvenio = '009' THEN 
					INSERT INTO bdicnweb:"informix".sw_cb_reportesacmtototapptmp(usuario, sucursal, caja_general, entidad, total_operacion, monto) 
					VALUES(pUsuario,cSucursal, cCajaGeneral, cEntidad, iTotalOperacion, mMonto);
				END IF;
		...
		ELSE
				...
				(SELECT *
				 ...
				 AND a.monto_tot = b.importe_pago
				 AND b.fecha_pago BETWEEN pFechaIni AND pFechaFin
				 AND b.numcategoria = pCategoria AND b.numconvenio = pConvenio
				 GROUP BY a.sucursal) c
				INNER JOIN bdinteg:"informix".si_sucursales d ON c.sucursal = d.sucursal
				INNER JOIN bdisuc:"informix".ss_proveedores e ON d.plaza_cajagen = e.plaza
				...
				IF pConvenio ='006' and cSucursal <>'' THEN			
					INSERT INTO bdicnweb:"informix".sw_cb_reportesacmtototwutmp(usuario, sucursal, caja_general, entidad, total_operacion, monto) 
					VALUES(pUsuario, cSucursal, cCajaGeneral, cEntidad, iTotalOperacion, mMonto);
				ELIF pConvenio ='007' and cSucursal <>'' THEN 
					INSERT INTO bdicnweb:"informix".sw_cb_reportesacmtototovtmp(usuario, sucursal, caja_general, entidad, total_operacion, monto) 
					VALUES(pUsuario, cSucursal, cCajaGeneral, cEntidad, iTotalOperacion, mMonto);
				ELIF pConvenio = '008' and cSucursal <>'' THEN 
					INSERT INTO bdicnweb:"informix".sw_cb_reportesacmtototvgtmp(usuario, sucursal, caja_general, entidad, total_operacion, monto) 
					VALUES(pUsuario, cSucursal, cCajaGeneral, cEntidad, iTotalOperacion, mMonto);
				ELIF pConvenio = '004' and cSucursal <>'' THEN 
					INSERT INTO bdicnweb:"informix".sw_cb_reportesacmtototbtstmp(usuario, sucursal, caja_general, entidad, total_operacion, monto) 
					VALUES(pUsuario, cSucursal, cCajaGeneral, cEntidad, iTotalOperacion, mMonto);
				ELIF pConvenio = '009' and cSucursal <>'' THEN 
					INSERT INTO bdicnweb:"informix".sw_cb_reportesacmtototapptmp(usuario, sucursal, caja_general, entidad, total_operacion, monto) 
					VALUES(pUsuario, cSucursal, cCajaGeneral, cEntidad, iTotalOperacion, mMonto);
				END IF;
		...
		IF pConvenio ='006' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacmtototwutmp WHERE usuario = pUsuario;
		ELIF pConvenio ='007' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacmtototovtmp WHERE usuario = pUsuario;
		ELIF pConvenio = '008' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacmtototvgtmp WHERE usuario = pUsuario;
		ELIF pConvenio = '004' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacmtototbtstmp WHERE usuario = pUsuario;
		ELIF pConvenio = '009' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacmtototapptmp WHERE usuario = pUsuario;
		END IF;		
        ...
        --LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		IF pConvenio ='006' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','REPORTE DEL TOTAL DE PAGOS POR CONCEPTO DE REMESAS WU',' ',' ',' ',' ' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio ='007' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','REPORTE DEL TOTAL DE PAGOS POR CONCEPTO DE REMESAS OV',' ',' ',' ',' ' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '008' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','REPORTE DEL TOTAL DE PAGOS POR CONCEPTO DE REMESAS VIGO',' ',' ',' ',' ' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '004' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','REPORTE DEL TOTAL DE PAGOS POR CONCEPTO DE REMESAS BTS',' ',' ',' ',' ' FROM systables  WHERE tabid = 1 ";
		ELIF pConvenio = '009' THEN 
            LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','REPORTE DEL TOTAL DE PAGOS POR CONCEPTO DE REMESAS APPRIZA',' ',' ',' ',' ' FROM systables  WHERE tabid = 1 ";
		END IF;	
		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'PERIODO DEL:','"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','AL:','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"','FECHA OPERACION:','"||TO_CHAR(dFechaHoy, '%d/%m/%Y') ||"','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";
		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";		
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'PERIODO DEL','PERIODO AL','SUCURSAL','CAJA GENERAL','ENTIDAD FEDERATIVA','No. TOTAL DE OPERACIONES','MONTO PAGADO' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        
     	IF pConvenio ='006' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT '"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"',REPLACE(sucursal,',',' ')::CHAR(44), caja_general::CHAR(45),entidad::CHAR(30),total_operacion::CHAR(20),monto::CHAR(30) FROM ""informix"".sw_cb_reportesacmtototwutmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio ='007' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT '"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"',REPLACE(sucursal,',',' ')::CHAR(44), caja_general::CHAR(45),entidad::CHAR(30),total_operacion::CHAR(20),monto::CHAR(30) FROM ""informix"".sw_cb_reportesacmtototovtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '008' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT '"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"',REPLACE(sucursal,',',' ')::CHAR(44), caja_general::CHAR(45),entidad::CHAR(30),total_operacion::CHAR(20),monto::CHAR(30) FROM ""informix"".sw_cb_reportesacmtototvgtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '004' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT '"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"',REPLACE(sucursal,',',' ')::CHAR(44), caja_general::CHAR(45),entidad::CHAR(30),total_operacion::CHAR(20),monto::CHAR(30) FROM ""informix"".sw_cb_reportesacmtototbtstmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '009' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT '"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"',REPLACE(sucursal,',',' ')::CHAR(44), caja_general::CHAR(45),entidad::CHAR(30),total_operacion::CHAR(20),monto::CHAR(30) FROM ""informix"".sw_cb_reportesacmtototapptmp WHERE usuario ='"||pUsuario||"'"; 
		END IF;
		...
		IF pConvenio ='006' AND pSucursal <>'0' THEN 
		LET cNombreArchivo = 'MONTO_TOTAL_PAGADO_WU_'||TRIM(pSucursal)||'_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio ='006' AND pSucursal ='0' THEN
		LET cNombreArchivo = 'MONTO_TOTAL_PAGADO_WU_TODAS_'||TRIM(cFechaHoraArchivo)||'.csv';		
		
		ELIF pConvenio ='007' AND pSucursal <>'0' THEN 
		LET cNombreArchivo = 'MONTO_TOTAL_PAGADO_OV_'||TRIM(pSucursal)||'_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio ='007' AND pSucursal ='0' THEN  
		LET cNombreArchivo = 'MONTO_TOTAL_PAGADO_OV_TODAS_'||TRIM(cFechaHoraArchivo)||'.csv';
		 
		ELIF pConvenio = '008' AND pSucursal <>'0' THEN 
		LET cNombreArchivo = 'MONTO_TOTAL_PAGADO_VIGO_'||TRIM(pSucursal)||'_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio = '008' AND pSucursal ='0' THEN 
		LET cNombreArchivo = 'MONTO_TOTAL_PAGADO_VIGO_TODAS_'||TRIM(cFechaHoraArchivo)||'.csv';
		 
		ELIF pConvenio = '004' AND pSucursal <>'0' THEN 
		LET cNombreArchivo = 'MONTO_TOTAL_PAGADO_BTS_'||TRIM(pSucursal)||'_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio = '004' AND pSucursal ='0' THEN 
		LET cNombreArchivo = 'MONTO_TOTAL_PAGADO_BTS_TODAS_'||TRIM(cFechaHoraArchivo)||'.csv';
		 
		ELIF pConvenio = '009' AND pSucursal <>'0' THEN 
		LET cNombreArchivo = 'MONTO_TOTAL_PAGADO_APP_'||TRIM(pSucursal)||'_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio = '009' AND pSucursal ='0' THEN 
		LET cNombreArchivo = 'MONTO_TOTAL_PAGADO_APP_TODAS_'||TRIM(cFechaHoraArchivo)||'.csv';
        ...
        LET cRutaGral = TRIM(pRutaDescarga)||TRIM(cNombreArchivo);
                ...
                BEGIN WORK;
                       LET ven_transacc = 1;
						...
						IF pConvenio ='006' AND pSucursal <>'0' THEN 						
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('MONTO_TOTAL_PAGADO_WU_'||TRIM(pSucursal)||'%'); 
						ELIF pConvenio ='006' AND pSucursal ='0' THEN
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('MONTO_TOTAL_PAGADO_WU_TODAS_%'); 
						ELIF pConvenio ='007' AND pSucursal <>'0' THEN 
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('MONTO_TOTAL_PAGADO_OV_'||TRIM(pSucursal)||'%'); 
						ELIF pConvenio ='007' AND pSucursal ='0' THEN  
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('MONTO_TOTAL_PAGADO_OV_TODAS_%'); 
						ELIF pConvenio = '008' AND pSucursal <>'0' THEN 
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('MONTO_TOTAL_PAGADO_VIGO_'||TRIM(pSucursal)||'%'); 
						ELIF pConvenio = '008' AND pSucursal ='0' THEN 
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('MONTO_TOTAL_PAGADO_VIGO_TODAS_%'); 
						ELIF pConvenio = '004' AND pSucursal <>'0' THEN 
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('MONTO_TOTAL_PAGADO_BTS_'||TRIM(pSucursal)||'%'); 
						ELIF pConvenio = '004' AND pSucursal ='0' THEN 
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('MONTO_TOTAL_PAGADO_BTS_TODAS_%'); 
						ELIF pConvenio = '009' AND pSucursal <>'0' THEN 
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('MONTO_TOTAL_PAGADO_APP_'||TRIM(pSucursal)||'%'); 
						ELIF pConvenio = '009' AND pSucursal ='0' THEN 
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('MONTO_TOTAL_PAGADO_APP_TODAS_%'); 
						END IF;
...
CREATE PROCEDURE "informix".sp_sac_reportesremnoconciliadas(pUsuario CHAR(8), pIdFuncion CHAR(10),pFechaIni DATE, pFechaFin DATE, pConvenio CHAR(5),pRutaDescarga CHAR(100),pIdPlantilla CHAR(25),pTituloPlantilla CHAR(255))
    RETURNING CHAR(5) AS codret,
    CHAR(100) AS reporte_generado;
	...
	DEFINE iCantidadPagosPAYC   INTEGER;
	DEFINE iCantidadPagosPAYI   INTEGER;
	...
	DEFINE cBanRefAbono 		CHAR(16);
	DEFINE cConfirmacionAbono	CHAR(11);
	DEFINE cExiste				CHAR(2);
	DEFINE dFechaMaxima 		DATETIME YEAR TO FRACTION(5);
	DEFINE iCountServicios		INTEGER;
	DEFINE iCount 				INTEGER;
	DEFINE cont_folsucsac		INTEGER;
	DEFINE cont_folsucchqs		INTEGER;
	DEFINE cont_folsucja		INTEGER;
	DEFINE cont_folsucab		INTEGER;
	DEFINE sumapgssac			INTEGER;
	--EPG 210514
	DEFINE cDiferencia_suc 		CHAR(16);
	DEFINE cDiferencia_ref 		CHAR(16);
	...
	DEFINE cRefnum CHAR(30);
	DEFINE cNnumber CHAR(15);
	DEFINE mR_destinamount MONEY(16,2);		
	DEFINE dFecha  DATETIME YEAR to FRACTION(3);
	DEFINE cEstatus CHAR(15);
	DEFINE cDato1 CHAR(50);
	DEFINE cDato2 CHAR(100);
	DEFINE cDato3 CHAR(150);
	DEFINE cNombreReporteHist CHAR(100);
	...
	LET iCantidadPagosPAYC = 0;
	LET iCantidadPagosPAYI = 0;
	LET cTransaccEfec      = "";
	LET cTransaccAbo       = "";
	--13-03-2013
	...
	LET cBanRefAbono 	   = "";
	LET cConfirmacionAbono = ""; 
	LET cExiste 		   = "NO";
	LET dFechaMaxima 	   = CURRENT;
	LET iCountServicios    = 0;
	LET iCount 			   = 0;
	LET cont_folsucsac	   = 0;
	LET cont_folsucchqs	   = 0;
	LET cont_folsucja	   = 0;
	LET cont_folsucab      = 0;
	LET sumapgssac		   = 0;
	--EPG 210514
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_reportesremnoconciliadas.out';
		--TRACE ON;
		
		IF pConvenio ='07006' THEN 		
		LET cTipo ='REPORTE REMESAS NO CONCILIADAS WU';
		ELIF pConvenio ='07007' THEN 
		LET cTipo ='REPORTE REMESAS NO CONCILIADAS OV';
		ELIF pConvenio = '07008' THEN 
		LET cTipo='REPORTE REMESAS NO CONCILIADAS VIGO';
		ELIF pConvenio = '07004' THEN 
		LET cTipo='REPORTE REMESAS NO CONCILIADAS BTS';
		ELIF pConvenio = '07009' THEN 
		LET cTipo='REPORTE REMESAS NO CONCILIADAS APPRIZA';
		END IF;
		...
		IF pConvenio ='07006' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacnoconcilwutmp WHERE usuario = pUsuario;
			DELETE FROM "informix".sw_cb_reportesacnoconcilrevwutmp WHERE usuario = pUsuario;

		ELIF pConvenio ='07007' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacnoconcilovtmp WHERE usuario = pUsuario;
			DELETE FROM "informix".sw_cb_reportesacnoconcilrevovtmp WHERE usuario = pUsuario;
				
		ELIF pConvenio = '07008' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacnoconcilvgtmp WHERE usuario = pUsuario;
			DELETE FROM "informix".sw_cb_reportesacnoconcilrevvgtmp WHERE usuario = pUsuario;

		ELIF pConvenio = '07004' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacnoconcilbtstmp WHERE usuario = pUsuario;
			DELETE FROM "informix".sw_cb_reportesacnoconcilrevbtstmp WHERE usuario = pUsuario;

		ELIF pConvenio = '07009' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacnoconcilapptmp WHERE usuario = pUsuario;
			...
			SET ISOLATION TO DIRTY READ;
				...
				WHILE (dFechaIni <= dFechaFin)
					 ...
					 FOREACH							 
						...
						INTO dFechaIni,iCantidadPagosServ,iCantidadPagos, iCantidadPagosREVI, cDiferencia
						...
						IF pConvenio = '07006' THEN 
						INSERT INTO "informix".sw_cb_reportesacnoconcilwutmp(usuario, fecha_ini, cantidad_pagos_serv, cantidad_pagos, cantidad_pagos_revi, diferencia) 
						VALUES(pUsuario,dFechaIni,iCantidadPagosServ,iCantidadPagos, iCantidadPagosREVI, cDiferencia);
				
						ELIF pConvenio ='07007' THEN 
									
						INSERT INTO "informix".sw_cb_reportesacnoconcilovtmp(usuario, fecha_ini, cantidad_pagos_serv, cantidad_pagos, cantidad_pagos_revi, diferencia) 
						VALUES(pUsuario,dFechaIni,iCantidadPagosServ,iCantidadPagos, iCantidadPagosREVI, cDiferencia);
					
						ELIF pConvenio = '07008' THEN 
						
						INSERT INTO "informix".sw_cb_reportesacnoconcilvgtmp(usuario, fecha_ini, cantidad_pagos_serv, cantidad_pagos, cantidad_pagos_revi, diferencia) 
						VALUES(pUsuario,dFechaIni,iCantidadPagosServ,iCantidadPagos, iCantidadPagosREVI, cDiferencia);
			...
			--Tomamos los valores de las fecha de los parametros
				...
				WHILE (dFechaIni <= dFechaFin)
					 ...
					 FOREACH							 
						...
						INTO dFechaIni,iCantidadPagosServ,iCantidadPagos,cDiferencia
						FROM bdisac:"informix".sac_wu_remesasnoconciliadas
						WHERE retfecha = dFechaIni
						and numcategoria = cCategoria
						and numconvenio = cConvenio
						...
						IF pConvenio = '07006' THEN 
						INSERT INTO "informix".sw_cb_reportesacnoconcilrevwutmp(usuario, fecha_ini, cantidad_pagos_serv, cantidad_pagos, diferencia) 
						VALUES(pUsuario,dFechaIni,iCantidadPagosServ,iCantidadPagos, cDiferencia);
						
						ELIF pConvenio ='07007' THEN 				
						INSERT INTO "informix".sw_cb_reportesacnoconcilrevovtmp(usuario, fecha_ini, cantidad_pagos_serv, cantidad_pagos, diferencia) 
						VALUES(pUsuario,dFechaIni,iCantidadPagosServ,iCantidadPagos, cDiferencia);
						
						ELIF pConvenio = '07008' THEN 
						INSERT INTO "informix".sw_cb_reportesacnoconcilrevvgtmp(usuario, fecha_ini, cantidad_pagos_serv, cantidad_pagos, diferencia) 
						VALUES(pUsuario,dFechaIni,iCantidadPagosServ,iCantidadPagos, cDiferencia);
		...
		ELIF pConvenio = '07004' THEN 
				...
				WHILE (dFechaIni <= dFechaFin)
					...
					SELECT {+INDEX(bdisac:sac_servicios_paso idx_btssrvpso1)}
					COUNT(folio_suc) --SERVICIOS
					...
					INTO iCantidadPagos
					FROM bdisac:"informix".sac_cheques_paso
					WHERE fech_alt = dFechaIni;

					SELECT COUNT(confirmation_nm) --abonos
					INTO iCantidadPagosPAYC
					FROM bdisac:"informix".sac_abono_paso
					WHERE fecha_insert = dFechaIni;

					SELECT COUNT(confirmation_nm) --caja
					INTO iCantidadPagosPAYI
					FROM bdisac:"informix".sac_btscaja_paso
					WHERE fecha_insert::DATE = dFechaIni;

					LET sumapgssac = iCantidadPagosPAYC + iCantidadPagosPAYI;
					LET iCantidadPagosPAYI = sumapgssac;

					IF iCantidadPagosServ = iCantidadPagos AND sumapgssac = iCantidadPagosServ
						THEN
							LET cDiferencia = "Sin Diferencia";
							INSERT INTO "informix".sw_cb_reportesacnoconcilbtstmp(usuario, fecha_ini, cantidad_pagos_serv, cantidad_pagos, cantidad_pagos_payi,cantidad_pagos_payc, diferencia) 
							VALUES(pUsuario,dFechaIni,iCantidadPagosServ,iCantidadPagos,iCantidadPagosPAYI,iCantidadPagosPAYC,cDiferencia);
						ELSE
							DELETE FROM bdisac:"informix".sac_conciliacionbts_paso;
							IF (iCantidadPagos > iCantidadPagosServ) 
								THEN
									INSERT INTO bdisac:"informix".sac_conciliacionbts_paso
									SELECT {+INDEX(bdisac:"informix".sac_cheques_paso idx_btschqspso2)} {+INDEX(bdisac:"informix".sac_cheques_paso idx_btssrvpso3)} cheq.folio_suc ,mov.referencia1
									FROM bdisac:"informix".sac_cheques_paso cheq, bdisac:"informix".sac_servicios_paso mov
									WHERE fech_alt = dFechaIni
									AND  cheq.fech_alt = mov.fecha_pago
									AND  cheq.folio_suc =  mov.folio_suc
									AND cheq.folio_suc NOT IN 
										(SELECT folio_suc
										FROM bdisac:"informix".sac_servicios_paso
										WHERE fecha_pago = dFechaIni
											AND status_cancelado = 'N');
							...
							IF (iCantidadPagosServ > iCantidadPagos) THEN
								...
								AND folio_suc NOT IN (SELECT folio_suc 
													...
													WHERE fech_alt = dFechaIni)
								AND folio_suc NOT IN(SELECT folio_suc 
												 FROM bdisac:"informix".sac_conciliacionbts_paso);
								--EPG 210514
								IF DBINFO('sqlca.sqlerrd2') <> 0 THEN
										LET iAcumulado = iAcumulado + 1;
								END IF;
							...
							--	IF (iCantidadPagosServ > iCantidadPagosPAYC + iCantidadPagosPAYI) THEN
							IF (iCantidadPagosServ > sumapgssac)  --	Sac Vs. Payi:
								THEN
									INSERT INTO bdisac:"informix".sac_conciliacionbts_paso
									SELECT folio_suc, referencia1
									FROM bdisac:"informix".sac_servicios_paso
									WHERE status_cancelado = 'N'
									...
									END IF;				
								ELSE
									IF (sumapgssac > iCantidadPagosServ) --	Payi vs. Sac:
										THEN
											INSERT INTO bdisac:"informix".sac_conciliacionbts_paso
											select bank_ref_nm, confirmation_nm
											from bdisac: sac_btscaja_paso
											where 
											fecha_insert = dFechaIni
											and trim (bank_ref_nm) not in
											(select folio_suc
											from bdisac:sac_servicios_paso 
											where 
							...
							IF (iCantidadPagosPAYC + iCantidadPagosPAYI  > iCantidadPagosServ ) THEN
								INSERT INTO bdisac:"informix".sac_conciliacionbts_paso
								SELECT bank_ref_nm, confirmation_nm
								FROM bdisac:"informix".sac_btscaja_paso
								WHERE fecha_insert = dFechaIni
						...
						IF (iCantidadPagosPAYC + iCantidadPagosPAYI  > iCantidadPagos) THEN
							INSERT INTO bdisac:"informix".sac_conciliacionbts_paso
							SELECT bank_ref_nm, confirmation_nm
							FROM bdisac:"informix".sac_btscaja_paso
							WHERE fecha_insert = dFechaIni
					...
					FOREACH
							...
							END IF;		
							  ...
							  SELECT count (*) 
									into cont_folsucsac
								FROM bdisac:"informix".sac_servicios_paso 
									WHERE fecha_insert = dFechaMaxima AND status_cancelado = 'N' AND referencia1 = cReferencia;
									IF cont_folsucsac > 0
										THEN 
											SELECT folio_suc 
											...
											SELECT count (*) into cont_folsucchqs
											FROM bdisac:"informix".sac_cheques_paso  WHERE folio_suc = cFolioSucServicios 
											AND fech_alt = dFechaIni;

											IF cont_folsucchqs > 0
												THEN 
													SELECT count (*) into cont_folsucja
													FROM bdisac:"informix".sac_btscaja_paso 
													WHERE bank_ref_nm= cFolioSucServicios AND confirmation_nm = cReferencia;
													IF cont_folsucja > 0
														THEN 
															SELECT bank_ref_nm,confirmation_nm 
															INTO cBanRefAbono, cConfirmacionAbono
															FROM bdisac:"informix".sac_abono_paso
															WHERE bank_ref_nm= cFolioSucServicios
															AND confirmation_nm = cReferencia;

															IF NVL(cBanRefAbono,'') <> '' AND NVL(cConfirmacionAbono,'') <> '' 
																THEN 
																	LET cExiste = "SI";
																ELSE 
																	LET cExiste = "SI";
															END IF;												
														ELSE	
															SELECT count (*) into cont_folsucab
															FROM bdisac:"informix".sac_abono_paso 
															WHERE bank_ref_nm= cFolioSucServicios AND confirmation_nm = cReferencia;
					...
					IF (iAcumulado = 0) THEN
						FOREACH
							SELECT  {+INDEX(bdisac:"informix".sac_cheques_paso idx_btschqspso2)} {+INDEX(bdisac:"informix".sac_cheques_paso idx_btssrvpso3)} cheq.folio_suc 
							  INTO cDiferencia_suc
							  FROM bdisac:"informix".sac_cheques_paso cheq, bdisac:"informix".sac_servicios_paso mov
							 WHERE fech_alt = dFechaIni
							   AND  cheq.fech_alt = mov.fecha_pago
						       AND  cheq.folio_suc <> mov.folio_suc
							   AND cheq.folio_suc NOT IN (SELECT folio_suc FROM bdisac:"informix".sac_servicios_paso
														   WHERE fecha_pago = dFechaIni
									                         AND status_cancelado = 'N')
						     GROUP BY cheq.folio_suc
						
						SELECT {+INDEX(bdicheq:"informix".sc_movhis  idx_movhisnew4)}  SUBSTR (referencia,17,27) INTO cDiferencia_ref
						  FROM bdicheq:"informix".sc_movhis  WHERE fech_alt = dFechaIni AND folio_suc = cDiferencia_suc GROUP BY referencia;
						
						LET cDiferencia = cDiferencia_ref ;
						INSERT INTO "informix".sw_cb_reportesacnoconcilbtstmp(usuario, fecha_ini, cantidad_pagos_serv, cantidad_pagos, cantidad_pagos_payi,cantidad_pagos_payc, diferencia) 
						VALUES(pUsuario,dFechaIni,iCantidadPagosServ,iCantidadPagos,iCantidadPagosPAYI,iCantidadPagosPAYC,cDiferencia);
						END FOREACH;						
					END IF;
				...
				--	2014.01.24 FRG-i	Se elimina el campo usuario en la tabla:
				DELETE FROM bdisac:"informix".sac_conciliacionrev_paso;
				...
				--	2014.01.24 FRG-f
					...
					IF ((iCantidadPagosServ = iCantidadPagos) AND (iCantidadPagosREVI = iCantidadPagosServ)) THEN
						LET cDiferencia = "Sin Diferencia";
						INSERT INTO "informix".sw_cb_reportesacnoconcilrevbtstmp(usuario, fecha_ini, cantidad_pagos_serv, cantidad_pagos, cantidad_pagos_payi,diferencia) 
						VALUES(pUsuario,dFechaIni,iCantidadPagosServ,iCantidadPagos,iCantidadPagosREVI,cDiferencia);
					ELSE
					    DELETE FROM bdisac:"informix".sac_conciliacionrev_paso;
				--	2014.01.24 FRG-i
				...
				--	2014.01.24 FRG-i
							  --	AND usuario = pUsuario;
				--	2014.01.24 FRG-f
								IF EXISTS (SELECT folio_suc FROM bdisac:"informix".sac_servicios_paso 
											WHERE fecha_insert = dFechaMaxima AND status_cancelado = 'N' 
											AND referencia1 = cReferencia)
				--	2014.01.24 FRG-i
											--	AND usuario = pUsuario)
				--	2014.01.24 FRG-i
				...
				--	2014.01.24 FRG-f
									...
									IF EXISTS(SELECT folio_suc FROM bdisac:"informix".sac_cheques_paso 
												WHERE folio_suc = cFolioSucServicios AND fech_alt = dFechaIni)
				--	2014.01.24 FRG-i
												--	AND usuario = pUsuario)
				--	2014.01.24 FRG-f
												THEN 
										
											IF EXISTS (SELECT bank_ref_nm,confirmation_nm FROM bdisac:"informix".sac_btscaja_paso 
														WHERE bank_ref_nm= cFolioSucServicios AND confirmation_nm = cReferencia)
														--	AND usuario = pUsuario)
														THEN
				...
				--	2014.01.24 FRG-f
											...
											ELIF EXISTS (SELECT bank_ref_nm,confirmation_nm FROM bdisac:"informix".sac_abono_paso 
														WHERE bank_ref_nm= cFolioSucServicios AND confirmation_nm = cReferencia)
														--	AND usuario = pUsuario )
														THEN
												LET cExiste = "SI";
		...
		ELIF pConvenio = '07009' THEN 
			...
			SET ISOLATION TO DIRTY READ;
				...
				WHILE (dFechaIni <= dFechaFin)
					 ...
					 FOREACH							 
						SELECT unirefnum, refnum, nnumber,r_destinamount, fecha,dato1,dato2,dato3, CASE estatus WHEN '1' THEN 'REVERSADO' ELSE '' END CASE 
						INTO cUnirefnum,cRefnum,cNnumber, mR_destinamount, dFecha, cDato1, cDato2,cDato3,cEstatus
						FROM bdisac:"informix".sac_app_remesasnoconciliadas
						WHERE DATE(fecha) = DATE (dFechaIni)
						and estatus IN ('0','1')
						
						INSERT INTO "informix".sw_cb_reportesacnoconcilapptmp(usuario, unirefnum, refnum, nnumber, r_destinamount,fecha,dato1,dato2,dato3,estatus) 
						VALUES(pUsuario,cUnirefnum,cRefnum,cNnumber, mR_destinamount, dFecha, cDato1, cDato2,cDato3,cEstatus);
		...
		IF pConvenio ='07006' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacnoconcilwutmp WHERE usuario = pUsuario;
			SELECT COUNT(*) INTO iTotal2 FROM "informix".sw_cb_reportesacnoconcilrevwutmp WHERE usuario = pUsuario;
		ELIF pConvenio ='07007' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacnoconcilovtmp WHERE usuario = pUsuario;
			SELECT COUNT(*) INTO iTotal2 FROM "informix".sw_cb_reportesacnoconcilrevovtmp WHERE usuario = pUsuario;
		ELIF pConvenio = '07008' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacnoconcilvgtmp WHERE usuario = pUsuario;
			SELECT COUNT(*) INTO iTotal2 FROM "informix".sw_cb_reportesacnoconcilrevvgtmp WHERE usuario = pUsuario;
		ELIF pConvenio = '07004' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacnoconcilbtstmp WHERE usuario = pUsuario;
			SELECT COUNT(*) INTO iTotal2 FROM "informix".sw_cb_reportesacnoconcilrevbtstmp WHERE usuario = pUsuario;
		ELIF pConvenio = '07009' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacnoconcilapptmp WHERE usuario = pUsuario;
		END IF;		
	
		
		IF (iTotal = 0 AND iTotal2 = 0) THEN			
			LET cCodRet ='00017';				
			EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento('1',TRIM(pIdMensaje),TRIM(pIdPlantilla),pUsuario,'','','1','no exitoso','','','','',cTipo,'','','','','','',1,0,0,0,0,CURRENT,'') INTO cCodRetSp;
		END IF;
        ...
        IF cCodRet='00000' THEN 
		--GENERACION DE REPORTE	
		IF pConvenio ='07009' THEN 
		LET cCmd1 ="";		
		LET cCmd1 ="SELECT ' ',' ',' ','SISTEMA DE ADMINISTRACION DE CONVENIOS ',' ',' ',' ','FECHA:','"||TO_CHAR(dFechaHoy, '%d/%m/%Y') ||"' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ',' ','REMESAS NO CONCILIADAS APPRIZA',' ',' ',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";	
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ',' ',' ',' ','RANGO DE FECHAS DEL:','"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','AL:','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";     
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'NO. REMESA','FOLIO SUC','SUCURSAL','MONTO','FECHA','DATO1','DATO2','DATO3','REVERSADO' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT unirefnum::CHAR(20),refnum::CHAR(30),nnumber::CHAR(15),r_destinamount::CHAR(20),LPAD(DAY(fecha),2,0)||'/'||LPAD(MONTH(fecha),2,0)||'/'||YEAR(fecha),NVL(dato1,'')::CHAR(50),NVL(dato2,'')::CHAR(100),NVL(dato3,'')::CHAR(150),estatus::CHAR(15) FROM ""informix"".sw_cb_reportesacnoconcilapptmp WHERE usuario ='"||pUsuario||"'"; 
		ELSE
        ...
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		IF pConvenio ='07006' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT  ' ','REMESAS NO CONCILIADAS WU',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";	
		ELIF pConvenio ='07007' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT  ' ','REMESAS NO CONCILIADAS OV',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";	
		ELIF pConvenio = '07008' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT  ' ','REMESAS NO CONCILIADAS VIGO',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";	
		ELIF pConvenio = '07004' THEN 
			LET cCmd1 =""||TRIM(cCmd1)||" SELECT  ' ','REMESAS NO CONCILIADAS BTS',' ','HORA:','"||TO_CHAR(dHoraHoy, '%H:%M:%S')||"' FROM systables  WHERE tabid = 1 ";	
		END IF;		
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT  ' ','RANGO DE FECHAS DEL:','"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','AL:','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";     
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'FECHA','SERVICIOS','CHEQUES','CAJA','DIFERENCIA' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        
     	IF pConvenio ='07006' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(fecha_ini),2,0)||'/'||LPAD(MONTH(fecha_ini),2,0)||'/'||YEAR(fecha_ini), cantidad_pagos_serv::CHAR(20),cantidad_pagos::CHAR(20),cantidad_pagos_revi::CHAR(20),diferencia::CHAR(20) FROM ""informix"".sw_cb_reportesacnoconcilwutmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio ='07007' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(fecha_ini),2,0)||'/'||LPAD(MONTH(fecha_ini),2,0)||'/'||YEAR(fecha_ini), cantidad_pagos_serv::CHAR(20),cantidad_pagos::CHAR(20),cantidad_pagos_revi::CHAR(20),diferencia::CHAR(20) FROM ""informix"".sw_cb_reportesacnoconcilovtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07008' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(fecha_ini),2,0)||'/'||LPAD(MONTH(fecha_ini),2,0)||'/'||YEAR(fecha_ini), cantidad_pagos_serv::CHAR(20),cantidad_pagos::CHAR(20),cantidad_pagos_revi::CHAR(20),diferencia::CHAR(20) FROM ""informix"".sw_cb_reportesacnoconcilvgtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07004' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(fecha_ini),2,0)||'/'||LPAD(MONTH(fecha_ini),2,0)||'/'||YEAR(fecha_ini), cantidad_pagos_serv::CHAR(20),cantidad_pagos::CHAR(20),cantidad_pagos_payi::CHAR(20),diferencia::CHAR(20) FROM ""informix"".sw_cb_reportesacnoconcilbtstmp WHERE usuario ='"||pUsuario||"'"; 
		
		END IF;
		
		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'FECHA','SERVICIOS','CHEQUES','CONVENIO','DIFERENCIA' FROM systables  WHERE tabid = 1 ";
		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		
		IF pConvenio ='07006' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(fecha_ini),2,0)||'/'||LPAD(MONTH(fecha_ini),2,0)||'/'||YEAR(fecha_ini), cantidad_pagos_serv::CHAR(20),cantidad_pagos::CHAR(20),'07006',diferencia::CHAR(20) FROM ""informix"".sw_cb_reportesacnoconcilrevwutmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio ='07007' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(fecha_ini),2,0)||'/'||LPAD(MONTH(fecha_ini),2,0)||'/'||YEAR(fecha_ini), cantidad_pagos_serv::CHAR(20),cantidad_pagos::CHAR(20),'07007',diferencia::CHAR(20) FROM ""informix"".sw_cb_reportesacnoconcilrevovtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07008' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(fecha_ini),2,0)||'/'||LPAD(MONTH(fecha_ini),2,0)||'/'||YEAR(fecha_ini), cantidad_pagos_serv::CHAR(20),cantidad_pagos::CHAR(20),'07008',diferencia::CHAR(20) FROM ""informix"".sw_cb_reportesacnoconcilrevvgtmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio = '07004' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT LPAD(DAY(fecha_ini),2,0)||'/'||LPAD(MONTH(fecha_ini),2,0)||'/'||YEAR(fecha_ini), cantidad_pagos_serv::CHAR(20),cantidad_pagos::CHAR(20),'07004',diferencia::CHAR(20) FROM ""informix"".sw_cb_reportesacnoconcilrevbtstmp WHERE usuario ='"||pUsuario||"'"; 
		...
		IF pConvenio ='07006' THEN 
		LET cNombreArchivo = 'REPORTE_REMESAS_NO_CONCILIADAS_WU_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo ='REPORTE REMESAS NO CONCILIADAS WU';
		ELIF pConvenio ='07007' THEN 
		LET cNombreArchivo = 'REPORTE_REMESAS_NO_CONCILIADAS_OV_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo ='REPORTE REMESAS NO CONCILIADAS OV';
		ELIF pConvenio = '07008' THEN 
		LET cNombreArchivo = 'REPORTE_REMESAS_NO_CONCILIADAS_VIGO_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='REPORTE REMESAS NO CONCILIADAS VIGO';
		ELIF pConvenio = '07004' THEN 
		LET cNombreArchivo = 'REPORTE_REMESAS_NO_CONCILIADAS_BTS_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='REPORTE REMESAS NO CONCILIADAS BTS';
		ELIF pConvenio = '07009' THEN 
		LET cNombreArchivo = 'REPORTE_REMESAS_NO_CONCILIADAS_APP_'||TRIM(cFechaHoraArchivo)||'.csv';
		LET cTipo='REPORTE REMESAS NO CONCILIADAS APPRIZA';
		END IF;
        ...
        LET cRutaGral = TRIM(pRutaDescarga)||TRIM(cNombreArchivo);
				...
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_REMESAS_NO_CONCILIADAS_WU_%'); 
				ELIF pConvenio ='07007' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_REMESAS_NO_CONCILIADAS_OV_%'); 
				ELIF pConvenio = '07008' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_REMESAS_NO_CONCILIADAS_VIGO_%'); 
				ELIF pConvenio = '07004' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_REMESAS_NO_CONCILIADAS_BTS_%'); 
				ELIF pConvenio = '07009' THEN 
				DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_REMESAS_NO_CONCILIADAS_APP_%'); 
				END IF;
			     INSERT INTO "informix".sw_ctrlgenreportesac(nombre_reporte,fecha_reporte,hr_reporte,usuario_insert,tipo)
			    VALUES(TRIM(cNombreArchivo),dFechaHoy,dHoraHoy,pUsuario,cTipo);
...
CREATE PROCEDURE "informix".sp_sac_sacreportetotalporconvenio(pUsuario CHAR(8), pIdFuncion CHAR(10),  pFechaIni DATE, pFechaFin DATE, pSucursal CHAR(4), pConvenio CHAR(5),pRutaDescarga CHAR(100),pIdPlantilla CHAR(25),pTituloPlantilla CHAR(255))
    RETURNING CHAR(5) AS codret,
    CHAR(100) AS reporte_generado;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_sacreportetotalporconvenio.out';
		--TRACE ON;
		
		IF pConvenio ='07004' THEN 	
		LET cTipo ='REPORTE TOTAL POR CONVENIO BTS';
		ELIF pConvenio ='07009' THEN 
		LET cTipo ='REPORTE TOTAL POR CONVENIO APPRIZA';		
		END IF;
		...
		IF pConvenio ='07004' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesactotalconvbtstmp WHERE usuario = pUsuario;
			DELETE FROM "informix".sw_cb_reportesacconciltotalconvbtstmp WHERE usuario = pUsuario;									
			
		ELIF pConvenio = '07009' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesactotalconvapptmp WHERE usuario = pUsuario;
			DELETE FROM "informix".sw_cb_reportesacconciltotalconvapptmp WHERE usuario = pUsuario;			
		END IF;
		...
		IF pConvenio = "00000" THEN      -- Todos los convenios
                IF pSucursal = "0000"  THEN   -- Todos los convenios y todas las sucursales
					SET ISOLATION TO DIRTY READ;
                    FOREACH
                        ...
                        IF cNumcategoria = "06" AND cNumconvenio="001" THEN
                            ...
                            FROM bdisac:sac_movimientoshistorial
                            WHERE fecha_pago::DATE  >= pFechaIni AND fecha_pago::DATE  <= pFechaFin AND status_cancelado <> 'S' AND
						...
						INSERT INTO "informix".sw_cb_reportesactotalconvbtstmp(usuario, num_pagos, nom_convenio, imp_pago, com_conv, iva_com_conv,com_cte,iva_com_cte,conf_central,conf_suc) 
						VALUES(pUsuario, iNumPagos, cNomConvenio,  mImportePago, mImpComisionConvenio, mIVAComisionConvenio, mImpComisionCte, mIVAComisionCte, iConfirmacionCentral,iConfirmacionSucursal);
							
						ELIF pConvenio = '07009' THEN 

						INSERT INTO "informix".sw_cb_reportesactotalconvapptmp(usuario, num_pagos, nom_convenio, imp_pago, com_conv, iva_com_conv,com_cte,iva_com_cte,conf_central,conf_suc) 
						VALUES(pUsuario, iNumPagos, cNomConvenio,  mImportePago, mImpComisionConvenio, mIVAComisionConvenio, mImpComisionCte, mIVAComisionCte, iConfirmacionCentral,iConfirmacionSucursal);
						END IF;
				...
				ELSE   --Todos los convenios y una sucursal
					...
					FOREACH
                        ...
                        IF cNumcategoria = "06" AND cNumconvenio="001" THEN
                            ...
                            FROM bdisac:sac_movimientoshistorial
                            WHERE fecha_pago::DATE  >= pFechaIni AND fecha_pago::DATE  <= pFechaFin AND status_cancelado <> 'S' AND
                            numcategoria = cNumcategoria AND numconvenio = cNumconvenio AND id_sucursal = pSucursal 
                            AND flag_confirmacion_central = 1
                            AND flag_confirmacion_sucursal = 1;
                        ELSE
                            ...
                            FROM bdisac:sac_movimientoshistorial
                            WHERE fecha_pago::DATE >= pFechaIni AND fecha_pago::DATE  <= pFechaFin AND numcategoria = cNumcategoria AND
                                numconvenio = cNumConvenio AND id_sucursal = pSucursal AND status_cancelado <> 'S'
                            AND flag_confirmacion_central = 1
                            AND flag_confirmacion_sucursal = 1;
                        END IF;
					...
					END IF;
				ELSE
					IF pSucursal = "0000"  THEN   -- Un convenio y todas las sucursales
					SET ISOLATION TO DIRTY READ;
...
--					WHERE numcategoria = cNumcategoria AND numconvenio = cNumconvenio;
        ...
        IF pConvenio = "" OR LENGTH(pConvenio) <> 5 THEN
            LET cCodRet = "00003";
            RETURN cCodRet, cNombreArchivo;
        ELSE		
			IF pFechaIni>=dFechaTabla THEN	
				...
				IF pConvenio = "00000" THEN -- Todos los convenios
					...
					FOREACH
						...
						SELECT nomconvenio, numcategoria || numconvenio, numconvenio, numcategoria, NVL(cuenta_contable,''),
						  ...
						  INTO cNomConvenio, pConvenio, cConv, cCateg, cCuenta_contable, cCuenta_cheques						 
						  FROM bdisac:"informix".sac_convenios
						...
						IF pConvenio NOT IN ('07001', '07002', '07003') THEN
							WHILE dFecha_pago <= pFechaFin
								IF pConvenio <> '08002' THEN
							...
							INSERT INTO "informix".sw_cb_reportesacconciltotalconvbtstmp(usuario, nom_convenio, fecha_pago, importe_archivo, cuenta_cheques, cargo_cuenta,cuenta_contable,importe_conta) 
							VALUES(pUsuario, cNomConvenio, dFecha_pago,  deImporte_archivo, cCuenta_cheques, mCargoCuenta, cCuenta_contable, deImporte_conta);
								
							ELIF pConvenio = '07009' THEN 

							INSERT INTO "informix".sw_cb_reportesacconciltotalconvapptmp(usuario, nom_convenio, fecha_pago, importe_archivo, cuenta_cheques, cargo_cuenta,cuenta_contable,importe_conta) 
							VALUES(pUsuario, cNomConvenio, dFecha_pago,  deImporte_archivo, cCuenta_cheques, mCargoCuenta, cCuenta_contable, deImporte_conta);
							END IF;
					...
					FOREACH
						...
						IF pConvenio NOT IN ('07001', '07002', '07003') THEN 
							WHILE dFecha_pago <= pFechaFin
								...
								-- Obtiene el Monto Total de los Movimientos de cheques 
									IF(pConvenio = "07004" OR pConvenio = "07006" OR pConvenio = "07007" OR pConvenio = "07008")THEN
		...
		--	2014.10.09 FRG-i	Se controlan totales por convenio Abonos Coppel y Club Protección.
				...
				ELSE-- Un Solo Convenio
					...
					SELECT nomconvenio, numcategoria || numconvenio, numconvenio, numcategoria, NVL(cuenta_contable,''),
						   NVL(cuenta_prestadora,''), NVL(proceso_automatico,0), NVL(trans_cen_abono_convenio,''), NVL(trans_cen_efectivo_cliente,''), NVL(trans_cen_cargo_cliente,'')
					INTO cNomConvenio, pConvenio, cConv, cCateg, cCuenta_contable,
		...
		IF pConvenio ='07004' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesactotalconvbtstmp WHERE usuario = pUsuario;
			SELECT COUNT(*) INTO iTotal2 FROM "informix".sw_cb_reportesacconciltotalconvbtstmp WHERE usuario = pUsuario;
		ELIF pConvenio ='07009' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesactotalconvapptmp WHERE usuario = pUsuario;
			SELECT COUNT(*) INTO iTotal2 FROM "informix".sw_cb_reportesacconciltotalconvapptmp WHERE usuario = pUsuario;			
		END IF;		
		...
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','SISTEMA DE ADMINISTRACION DE CONVENIOS',' ',' ',' ',' ',' ',' ' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ','REPORTE TOTAL DE COBRANZA - CONCILIACION DE PAGOS DE SERVICIOS REFERENCIADOS',' ',' ',' ',' ',' ',' ' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		IF pSucursal <>'0000' THEN
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'CONVENIOS:','''"||pConvenio||"','SUCURSAL:',' ','''"||pSucursal||"','RANGO DE FECHAS DEL:','"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','AL:','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"' FROM systables  WHERE tabid = 1 ";
		ELSE
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'CONVENIOS:','''"||pConvenio||"','SUCURSAL:','TODAS',' ','RANGO DE FECHAS DEL:','"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','AL:','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"' FROM systables  WHERE tabid = 1 ";
		END IF;
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'NUM. PAGOS','CONVENIO','IMPORTE PAGO','IMPORTE COMISION','IVA COMISION','IMPORTE COMISION CLIENTE','IVA COMISION CLIENTE','CONFIRMACION CENTRAL','CONFIRMACION SUCURSAL' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        
     	IF pConvenio ='07004' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT num_pagos::CHAR(20),nom_convenio::CHAR(40),imp_pago::CHAR(20),com_conv::CHAR(20),iva_com_conv::CHAR(20),com_cte::CHAR(20),iva_com_cte::CHAR(20),conf_central::CHAR(20),conf_suc::CHAR(20) FROM ""informix"".sw_cb_reportesactotalconvbtstmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio ='07009' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT num_pagos::CHAR(20),nom_convenio::CHAR(40),imp_pago::CHAR(20),com_conv::CHAR(20),iva_com_conv::CHAR(20),com_cte::CHAR(20),iva_com_cte::CHAR(20),conf_central::CHAR(20),conf_suc::CHAR(20) FROM ""informix"".sw_cb_reportesactotalconvapptmp WHERE usuario ='"||pUsuario||"'"; 
		END IF;
		 
		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'CONVENIO','FECHA PAGO','IMPORTE ARCHIVO','CUENTA CHEQUES','CARGO CUENTA','CUENTA CONTABLE','IMPORTE CONTA',' ',' ' FROM systables  WHERE tabid = 1 ";
		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		
		IF pConvenio ='07004' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT * FROM ( SELECT nom_convenio::CHAR(40),LPAD(DAY(fecha_pago),2,0)||'/'||LPAD(MONTH(fecha_pago),2,0)||'/'||YEAR(fecha_pago), importe_archivo::CHAR(25),cuenta_cheques::CHAR(30),cargo_cuenta::CHAR(25),cuenta_contable::CHAR(30),importe_conta::CHAR(25),' ',' ' FROM ""informix"".sw_cb_reportesacconciltotalconvbtstmp WHERE usuario ='"||pUsuario||"' ORDER BY fecha_pago)"; 
		ELIF pConvenio ='07009' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT * FROM ( SELECT nom_convenio::CHAR(40),LPAD(DAY(fecha_pago),2,0)||'/'||LPAD(MONTH(fecha_pago),2,0)||'/'||YEAR(fecha_pago), importe_archivo::CHAR(25),cuenta_cheques::CHAR(30),cargo_cuenta::CHAR(25),cuenta_contable::CHAR(30),importe_conta::CHAR(25),' ',' ' FROM ""informix"".sw_cb_reportesacconciltotalconvapptmp WHERE usuario ='"||pUsuario||"' ORDER BY fecha_pago)"; 
		...
		IF pConvenio ='07004' AND pSucursal <>'0000' THEN 
		LET cNombreArchivo = 'REPORTE_TOTAL_CONVENIO_BTS_'||TRIM(pSucursal)||'_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio ='07004' AND pSucursal ='0000' THEN
		LET cNombreArchivo = 'REPORTE_TOTAL_CONVENIO_BTS_TODAS_'||TRIM(cFechaHoraArchivo)||'.csv';		
		
		ELIF pConvenio ='07009' AND pSucursal <>'0000' THEN 
		LET cNombreArchivo = 'REPORTE_TOTAL_CONVENIO_APPRIZA_'||TRIM(pSucursal)||'_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio ='07009' AND pSucursal ='0000' THEN  
		LET cNombreArchivo = 'REPORTE_TOTAL_CONVENIO_APPRIZA_TODAS_'||TRIM(cFechaHoraArchivo)||'.csv';	
		END IF;
        ...
        LET cRutaGral = TRIM(pRutaDescarga)||TRIM(cNombreArchivo);
                ...
                BEGIN WORK;
                       LET ven_transacc = 1;
						...
						IF pConvenio ='07004' AND pSucursal <>'0000' THEN 						
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_TOTAL_CONVENIO_BTS_'||TRIM(pSucursal)||'%'); 
						ELIF pConvenio ='07004' AND pSucursal ='0000' THEN
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_TOTAL_CONVENIO_BTS_TODAS_%'); 
						ELIF pConvenio ='07009' AND pSucursal <>'0000' THEN 
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_TOTAL_CONVENIO_APPRIZA_'||TRIM(pSucursal)||'%'); 
						ELIF pConvenio ='07009' AND pSucursal ='0000' THEN  
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_TOTAL_CONVENIO_APPRIZA_TODAS_%'); 
						END IF;
					  INSERT INTO "informix".sw_ctrlgenreportesac(nombre_reporte,fecha_reporte,hr_reporte,usuario_insert,tipo)
					  VALUES(TRIM(cNombreArchivo),dFechaHoy,dHoraHoy,pUsuario,cTipo);
...
CREATE PROCEDURE "informix".sp_sac_sacreporteconciliacionconveniosucursal(pUsuario CHAR(8), pIdFuncion CHAR(10),  pFechaIni DATE, pFechaFin DATE, pSucursal CHAR(4), pConvenio CHAR(5),pRutaDescarga CHAR(100),pIdPlantilla CHAR(25),pTituloPlantilla CHAR(255))
    RETURNING CHAR(5) AS codret,
    CHAR(100) AS reporte_generado;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_sacreporteconciliacionconveniosucursal.out';
		--TRACE ON;
		
		IF pConvenio ='07004' THEN 
		LET cTipo ='REPORTE CONVENIO REMESAS BTS';
		ELIF pConvenio ='07009' THEN 
		LET cTipo ='REPORTE CONVENIO REMESAS APPRIZA';		
		END IF;

		IF pUsuario = '' OR pIdFuncion = '' OR pRutaDescarga = '' OR pConvenio ='' OR  pIdPlantilla ='' OR pTituloPlantilla ='' THEN
			LET cCodRet = '00003';			
			EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento('1',TRIM(pIdMensaje),TRIM(pIdPlantilla),pUsuario,'','','1','no exitoso','','','','',cTipo,'','','','','','',1,0,0,0,0,CURRENT,'') INTO cCodRetSp;
	       RETURN cCodRet, cNombreArchivo;
    	END IF;
		...
		IF pConvenio ='07004' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacconciliacionconvsucbtstmp WHERE usuario = pUsuario;		
			
		ELIF pConvenio = '07009' THEN 
			-- SE LIMPIA TABLA POR USUARIO
			DELETE FROM "informix".sw_cb_reportesacconciliacionconvsucapptmp WHERE usuario = pUsuario;
		END IF;
    ...
    IF pConvenio = "" OR  pSucursal = "" OR LENGTH(pConvenio) <> 5 OR LENGTH(pSucursal) <> 4 THEN
		 LET cCodRet = '00003';
	     EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento('1',TRIM(pIdMensaje),TRIM(pIdPlantilla),pUsuario,'','','1','no exitoso','','','','',cTipo,'','','','','','',1,0,0,0,0,CURRENT,'') INTO cCodRetSp;
	    
    ELSE
		IF (pFechaIni>=dFechaTabla) THEN --Nuevo Proceso utilizando la tabla sac_conciliaciontotalporconvenio
			IF pConvenio = "00000" THEN      -- Todos los convenios
				IF pSucursal = "0000"  THEN   -- Todos los convenios y todas las sucursales
					FOREACH
						...
						FOREACH
							...
							FROM bdisac:"informix".sac_conciliaciontotalporconvenio
							WHERE fecha_pago::DATE  >= pFechaIni
							AND fecha_pago::DATE  <= pFechaFin
							AND numcategoria = cNumcategoria
							AND numconvenio = cNumconvenio
							...
							INSERT INTO "informix".sw_cb_reportesacconciliacionconvsucbtstmp(usuario, sucursal, num_pagos, nom_convenio, imp_pago, com_conv, iva_com_conv,com_cte,iva_com_cte,conf_central,conf_suc) 
							VALUES(pUsuario,cIdSucursal, iNumPagos, cNomConvenio,  mImportePago, mImpComisionConvenio, mIVAComisionConvenio, mImpComisionCte, mIVAComisionCte, iConfirmacionCentral,iConfirmacionSucursal);
								
							ELIF pConvenio = '07009' THEN 

							INSERT INTO "informix".sw_cb_reportesacconciliacionconvsucapptmp(usuario, sucursal, num_pagos, nom_convenio, imp_pago, com_conv, iva_com_conv,com_cte,iva_com_cte,conf_central,conf_suc) 
							VALUES(pUsuario,cIdSucursal, iNumPagos, cNomConvenio,  mImportePago, mImpComisionConvenio, mIVAComisionConvenio, mImpComisionCte, mIVAComisionCte, iConfirmacionCentral,iConfirmacionSucursal);
							END IF;
					...
					IF pSucursal = "0000"  THEN   -- Todos los convenios y todas las sucursales
						FOREACH
							...
							FOREACH
								...
								WHERE b.fecha_pago::DATE  >= pFechaIni
								AND b.fecha_pago::DATE  <= pFechaFin
								AND a.numcategoria = b.numcategoria
								AND a.numconvenio = b.numconvenio
								AND b.numcategoria = cNumcategoria
								AND b.numconvenio = cNumconvenio
    ...
    END IF; 
		...
		IF pConvenio ='07004' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacconciliacionconvsucbtstmp WHERE usuario = pUsuario;
		ELIF pConvenio ='07009' THEN 
			SELECT COUNT(*) INTO iTotal FROM "informix".sw_cb_reportesacconciliacionconvsucapptmp WHERE usuario = pUsuario;		
		END IF;		
		...
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ',' ','SISTEMA DE ADMINISTRACION DE CONVENIOS',' ',' ',' ',' ',' ',' ' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT ' ',' ',' ','DETALLE DE COBRANZA POR SUCURSAL - CONCILIACION DE PAGOS DE PAGOS REFERENCIADOS',' ',' ',' ',' ',' ',' ' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
		IF pSucursal <>'0000' THEN
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'CONVENIOS:','''"||pConvenio||"',' ','SUCURSAL:',' ','''"||pSucursal||"','RANGO DE FECHAS DEL:','"||TO_CHAR(pFechaIni, '%d/%m/%Y')||"','AL:','"||TO_CHAR(pFechaFin, '%d/%m/%Y')||"' FROM systables  WHERE tabid = 1 ";
		ELSE
		...
		END IF;
		 LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT 'SUCURSAL','NUM. PAGOS','CONVENIO','IMPORTE PAGO','IMPORTE COMISION','IVA COMISION','IMPORTE COMISION CLIENTE','IVA COMISION CLIENTE','CONFIRMACION CENTRAL','CONFIRMACION SUCURSAL' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL ";
        
     	IF pConvenio ='07004' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT ''''||sucursal::CHAR(4), num_pagos::CHAR(20),nom_convenio::CHAR(40),imp_pago::CHAR(20),com_conv::CHAR(20),iva_com_conv::CHAR(20),com_cte::CHAR(20),iva_com_cte::CHAR(20),conf_central::CHAR(20),conf_suc::CHAR(20) FROM ""informix"".sw_cb_reportesacconciliacionconvsucbtstmp WHERE usuario ='"||pUsuario||"'"; 
		ELIF pConvenio ='07009' THEN 
		LET cCmd1 =""||TRIM(cCmd1)||" SELECT ''''||sucursal::CHAR(4), num_pagos::CHAR(20),nom_convenio::CHAR(40),imp_pago::CHAR(20),com_conv::CHAR(20),iva_com_conv::CHAR(20),com_cte::CHAR(20),iva_com_cte::CHAR(20),conf_central::CHAR(20),conf_suc::CHAR(20) FROM ""informix"".sw_cb_reportesacconciliacionconvsucapptmp WHERE usuario ='"||pUsuario||"'"; 
		END IF;
		...
		IF pConvenio ='07004' AND pSucursal <>'0000' THEN 
		LET cNombreArchivo = 'REPORTE_CONVENIO_REMESAS_BTS_'||TRIM(pSucursal)||'_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio ='07004' AND pSucursal ='0000' THEN
		LET cNombreArchivo = 'REPORTE_CONVENIO_REMESAS_BTS_TODAS_'||TRIM(cFechaHoraArchivo)||'.csv';		
		
		ELIF pConvenio ='07009' AND pSucursal <>'0000' THEN 
		LET cNombreArchivo = 'REPORTE_CONVENIO_REMESAS_APPRIZA_'||TRIM(pSucursal)||'_'||TRIM(cFechaHoraArchivo)||'.csv';
		ELIF pConvenio ='07009' AND pSucursal ='0000' THEN  
		LET cNombreArchivo = 'REPORTE_CONVENIO_REMESAS_APPRIZA_TODAS_'||TRIM(cFechaHoraArchivo)||'.csv';	
		END IF;
        ...
        LET cRutaGral = TRIM(pRutaDescarga)||TRIM(cNombreArchivo);
                ...
                BEGIN WORK;
                       LET ven_transacc = 1;
						...
						IF pConvenio ='07004' AND pSucursal <>'0000' THEN 						
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_CONVENIO_REMESAS_BTS_'||TRIM(pSucursal)||'%'); 
						ELIF pConvenio ='07004' AND pSucursal ='0000' THEN
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_CONVENIO_REMESAS_BTS_TODAS_%'); 
						ELIF pConvenio ='07009' AND pSucursal <>'0000' THEN 
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_CONVENIO_REMESAS_APPRIZA_'||TRIM(pSucursal)||'%'); 
						ELIF pConvenio ='07009' AND pSucursal ='0000' THEN  
						DELETE FROM "informix".sw_ctrlgenreportesac WHERE TRIM(tipo) = TRIM(cTipo) AND usuario_insert = pUsuario AND TRIM(nombre_reporte) LIKE TRIM('REPORTE_CONVENIO_REMESAS_APPRIZA_TODAS_%'); 
						END IF;
...
CREATE PROCEDURE "informix".sp_sac_verificastatussacmontototal(pUsuario CHAR(8), pIdFuncion CHAR(10))
	RETURNING CHAR(5) AS codret,
			  CHAR(1) AS status,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cStatus,cErrorProceso,cError,iTotalOperacion, mMonto;		
		END EXCEPTION;
	 
		--SET DEBUG FILE TO '/tmp/mfinis/sp_sac_verificastatussacmontototal.out';
		--TRACE ON;
		...
		FROM bdicnweb:"informix".sw_verificasacmontototal 
		...
		SELECT COUNT(*)
		INTO iNoRegistros
		FROM bdicnweb:"informix".sw_sac_montototal where usuario = pUsuario; 		
		
		
		SELECT total_operaciones,monto
		INTO iTotalOperacion, mMonto
		FROM bdicnweb:"informix".sw_sac_montototal where usuario = pUsuario; 	
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: REPORTES DE CONVENIOS SAC',
'DESCRIPCION: SPL que verifica el status de la ejecucion del spl sp_sac_montototal',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_pos_busquedabitacorapos(pUsuario CHAR(8), pIdFuncion CHAR(10), pEjecut CHAR(8),pFechaIni DATE,pFechaFin DATE,pRegistros INTEGER, pRecuperacion INTEGER)
                RETURNING CHAR(5) AS codret,
						  CHAR(8) AS empleado,
						  CHAR(45) AS nombre,
						  CHAR(50) AS movimiento,
						  DATE AS fecha;          
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cEmpleado,cNombre,cMovimiento,dFecha;
		END EXCEPTION;
			
		-- SET DEBUG FILE TO '/tmp/mfinis/sp_pos_busquedabitacorapos.out';
		-- TRACE ON;
		...
		IF NVL(pEjecut,'') <> '' AND NVL(pFechaIni,'') ='' AND NVL(pFechaFin,'') ='' THEN 
		
		FOREACH
			
			SELECT SKIP pRegistros FIRST pRecuperacion 
			usuario_modifica, nombre_usuario_modifica, accion,fecha_modifica
			INTO cEmpleado,cNombre, cMovimiento,dFecha
			FROM bdinteg:"informix".si_bitacora_pos	
			WHERE usuario_modifica =pEjecut
			ORDER BY fecha_modifica
			
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet,cEmpleado,cNombre,cMovimiento,dFecha WITH RESUME;
		...
		FOREACH
			...
			INTO cEmpleado,cNombre, cMovimiento,dFecha
			FROM bdinteg:"informix".si_bitacora_pos	
			WHERE fecha_modifica BETWEEN pFechaIni AND pFechaFin
			ORDER BY fecha_modifica
		...
		FOREACH
			...
			INTO cEmpleado,cNombre, cMovimiento,dFecha
			FROM bdinteg:"informix".si_bitacora_pos	
			WHERE usuario_modifica =pEjecut AND fecha_modifica BETWEEN pFechaIni AND pFechaFin
			ORDER BY fecha_modifica
...
'AUTOR: Daniel Reyes Guillen',
'FECHA: 25/11/2021',
'DESCRIPCION: Sp encargado de consultar datos de la tabla si_bitacora_pos',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_pos_busquedabitacorapos_total(pUsuario CHAR(8), pIdFuncion CHAR(10), pEjecut CHAR(8),pFechaIni DATE,pFechaFin DATE)
                RETURNING CHAR(5) AS codret,
						  INTEGER AS total;
		...
		-- SET DEBUG FILE TO '/tmp/mfinis/sp_pos_busquedabitacorapos_total.out';
		-- TRACE ON;
...
CREATE PROCEDURE "informix".sp_pos_busquedapos(pUsuario CHAR(8), pIdFuncion CHAR(10), pDescripLarg CHAR(152),pDescripCort CHAR(32),pRegistros INTEGER, pRecuperacion INTEGER)
                RETURNING CHAR(5) AS codret,
						  INTEGER AS folio,
						  CHAR(150) AS descrip_lar,
						  CHAR(30) AS descrip_cor;          
						  
        DEFINE cCodRet CHAR(5);
        DEFINE iSqlErr INTEGER;    
		DEFINE cDescripLarg CHAR(150);
		DEFINE cDescripCort CHAR(30);
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,iFolio,cDescripLarg,cDescripCort;
		END EXCEPTION;
			
		 --SET DEBUG FILE TO '/tmp/mfinis/sp_pos_busquedapos.out';
		 --TRACE ON;
		...
		IF NVL(pDescripLarg,'') <> '' AND pDescripCort ='' THEN 
 
        
		FOREACH
			
			SELECT SKIP pRegistros FIRST pRecuperacion 
			folio, descrip_larga, descrip_corta
			INTO iFolio,cDescripLarg, cDescripCort
			FROM bdinteg:"informix".si_descrip_pos	
			WHERE descrip_larga LIKE '%'||TRIM(pDescripLarg)||'%'
			ORDER BY folio
			
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet,iFolio,cDescripLarg,cDescripCort WITH RESUME;
		...
		IF NVL(pDescripCort,'') <> '' AND pDescripLarg='' THEN 
		...
		FOREACH
			...
			INTO iFolio,cDescripLarg, cDescripCort
			FROM bdinteg:"informix".si_descrip_pos	
			WHERE descrip_corta LIKE '%'||TRIM(pDescripCort)||'%' 
			ORDER BY folio
...
'AUTOR: Daniel Reyes Guillen',
'FECHA: 25/11/2021',
'DESCRIPCION: Sp encargado de consultar los datos POS de la tabla si_descrip_pos',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_pos_busquedapos_total(pUsuario CHAR(8), pIdFuncion CHAR(10), pDescripLarg CHAR(152),pDescripCort CHAR(32))
                RETURNING CHAR(5) AS codret,
						  INTEGER AS total;        
		...
		-- SET DEBUG FILE TO '/tmp/mfinis/sp_pos_busquedapos_total.out';
		-- TRACE ON;
...
'AUTOR: Daniel Reyes Guillen',
'FECHA: 25/11/2021',
'DESCRIPCION: Sp encargado de consultar el total los datos POS de la tabla si_descrip_pos',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_pos_consultapos(pUsuario CHAR(8), pIdFuncion CHAR(10), pRegistros INTEGER, pRecuperacion INTEGER)
                RETURNING CHAR(5) AS codret,
						  INTEGER AS folio,
						  CHAR(150) AS descrip_lar,
						  CHAR(30) AS descrip_cor;          
		...
		-- SET DEBUG FILE TO '/tmp/mfinis/sp_pos_consultapos.out';
		-- TRACE ON;
...
CREATE PROCEDURE "informix".sp_pos_consultapos_total(pUsuario CHAR(8), pIdFuncion CHAR(10))
                RETURNING CHAR(5) AS codret,
						  INTEGER AS total;       
		...
		-- SET DEBUG FILE TO '/tmp/mfinis/sp_pos_consultapos_total.out';
		-- TRACE ON;
		...
		SELECT COUNT(*) INTO iTotal FROM bdinteg:"informix".si_descrip_pos;
...
'AUTOR: Daniel Reyes Guillen',
'FECHA: 25/11/2021',
'DESCRIPCION: Sp encargado de consultar el total de filas de la tabla si_descrip_pos',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_pos_eliminapos(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdPosAct INT)
                RETURNING CHAR(5) AS codret;          
        ...
        BEGIN   
                ...
                -- SET DEBUG FILE TO '/tmp/mfinis/sp_pos_eliminapos.out';
                -- TRACE ON;
                
                IF pUsuario = '' OR pIdFuncion = '' OR pIdPosAct = '' THEN
                    LET cCodRet = '00003';
					RETURN cCodRet;
                END IF;
				...
				--NOMBRE DE USUARIO PARA BITACORA
				SELECT nombre INTO cNombre FROM bdinteg:"informix".si_ejecut where ejecutivo = pUsuario;
	
				DELETE FROM  bdinteg:"informix".si_descrip_pos WHERE folio = pIdPosAct;
				
				IF DBINFO('sqlca.sqlerrd2') > 0 THEN
					INSERT INTO  bdinteg:"informix".si_bitacora_pos (usuario_modifica,nombre_usuario_modifica,accion,fecha_modifica) 
					VAlUES (pUsuario,cNombre,'BAJA DE ESTABLECIMIENTO POS',CURRENT);	
				END IF;
...
'AUTOR: Daniel Reyes Guillen',
'FECHA: 25/11/2021',
'DESCRIPCION: Sp encargado de eliminar un POS',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_pos_genrepbitacora(pUsuario CHAR(8), pIdFuncion CHAR(10),pEjecut CHAR(8),pFechaIni DATE,pFechaFin DATE, pRutaDescarga CHAR(100),pBandera CHAR(1))
    RETURNING CHAR(5) AS codret,
    CHAR(100) AS reporte_generado;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
            LET cCodRet = iSqlErr;
			UPDATE {+INDEX (bdicnweb:sw_verificastatusrepposbitacora idx_sw_verificastatusrepposbitacora)}  "informix".sw_verificastatusrepposbitacora
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_pos_genrepbitacora.out';
		--TRACE ON;
        ...
        DELETE {+INDEX (bdicnweb:sw_pos_reportebitacorapostmp idx_sw_pos_reportebitacorapostmp)} FROM "informix".sw_pos_reportebitacorapostmp WHERE usuario = pUsuario;
		DELETE {+INDEX (bdicnweb:sw_verificastatusrepposbitacora idx_sw_verificastatusrepposbitacora)}  FROM "informix".sw_verificastatusrepposbitacora
		WHERE usuario_insert = pUsuario AND tipo_proceso = 'LECTURA'; 
 
		-- SE INSERTA A TABLA PARA EL MONITOREO DEL STATUS
		INSERT INTO "informix".sw_verificastatusrepposbitacora(usuario_insert, nombre_archivo, status,  error_proceso, tipo_proceso, error) 
		...
		INSERT INTO "informix".sw_pos_reportebitacorapostmp(usuario, usuario_modifica, nombre_usuario_modifica, accion, fecha_modifica) 
		VALUES(pUsuario,cEmpleado,cNombre,cMovimiento,dFecha);
		...
		SELECT {+INDEX (bdicnweb:sw_pos_reportebitacorapostmp idx_sw_pos_reportebitacorapostmp)} COUNT(*) INTO iTotal FROM "informix".sw_pos_reportebitacorapostmp WHERE usuario = pUsuario;
		...
		--GENERACION DE REPORTE	
		LET cCmd1 ="";
        LET cCmd1 ="SELECT 'NO. EMPLEADO','NOMBRE','MOVIMIENTO REALIZADO','FECHA' FROM systables  WHERE tabid = 1 ";
        LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL SELECT * FROM ( ";
        LET cCmd1 =""||TRIM(cCmd1)||" SELECT usuario_modifica::CHAR(20), nombre_usuario_modifica::CHAR(80),accion::CHAR(50),LPAD(DAY(fecha_modifica),2,0)||'/'||LPAD(MONTH(fecha_modifica),2,0)||'/'||YEAR(fecha_modifica) FROM ""informix"".sw_pos_reportebitacorapostmp WHERE usuario ='"||pUsuario||"' ORDER BY fecha_modifica)"; 
       			
		-- SE DEFINE NOMENCLATURA DEL REPORTE A GENERAR		
		
		IF pBandera = '1' THEN 
		LET cNombreArchivo = 'Reporte_Bitacora_Establecimientos_POS.txt';
		END IF;
		IF pBandera = '2' THEN 
		LET cNombreArchivo = 'Reporte_Bitacora_Establecimientos_POS.csv';
		END IF;
...
'AUTOR: Daniel Reyes Guillen',
'FECHA: 25/11/2021',
'DESCRIPCION: SPL que genera el reporte de la tabla si_bitacora_pos',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_pos_insertapos(pUsuario CHAR(8), pIdFuncion CHAR(10), pDescripLarg CHAR(150),pDescripCort CHAR(30),pIdPosAct INT)
                RETURNING CHAR(5) AS codret;          
        ...
        BEGIN   
                ...
                -- SET DEBUG FILE TO '/tmp/mfinis/sp_pos_insertapos.out';
                -- TRACE ON;
                
                IF pUsuario = '' OR pIdFuncion = '' OR pDescripLarg = '' OR pDescripCort = '' THEN
                    LET cCodRet = '00003';
					RETURN cCodRet;
                END IF;
				...
				SELECT COUNT(*) INTO iExiste FROM bdinteg:"informix".si_descrip_pos WHERE TRIM(descrip_larga)= TRIM(pDescripLarg) and TRIM(descrip_corta)=TRIM(pDescripCort);
				...
				SELECT nombre INTO cNombre FROM bdinteg:"informix".si_ejecut where ejecutivo = pUsuario;
					...
					IF NVL(pIdPosAct,'')='' THEN  
					--SE INSERTA EL POS
					INSERT INTO  bdinteg:"informix".si_descrip_pos (descrip_larga,descrip_corta) 
						VAlUES (UPPER(pDescripLarg),UPPER(pDescripCort));
					IF DBINFO('sqlca.sqlerrd2') > 0 THEN
					INSERT INTO  bdinteg:"informix".si_bitacora_pos (usuario_modifica,nombre_usuario_modifica,accion,fecha_modifica) 
						VAlUES (pUsuario,cNombre,'ALTA DE ESTABLECIMIENTO POS',CURRENT);
					END IF;
					
					ELIF NVL(pIdPosAct,'')<>'' THEN 
					--SE ACTUALIZA EL POS
					UPDATE  bdinteg:"informix".si_descrip_pos SET descrip_larga = UPPER(pDescripLarg), descrip_corta=UPPER(pDescripCort)
						WHERE folio = pIdPosAct;
						
					IF DBINFO('sqlca.sqlerrd2') > 0 THEN
					INSERT INTO  bdinteg:"informix".si_bitacora_pos (usuario_modifica,nombre_usuario_modifica,accion,fecha_modifica) 
						VAlUES (pUsuario,cNombre,'MODIFICACIÓN DE ESTABLECIMIENTO POS',CURRENT);	
					END IF;
...
'AUTOR: Daniel Reyes Guillen',
'FECHA: 25/11/2021',
'DESCRIPCION: Sp encargado de dar de alta o actualizar un POS',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_pos_recuperanombre(pUsuario CHAR(8), pIdFuncion CHAR(10), pEjecut CHAR(30))
                RETURNING CHAR(5) AS codret,
						  CHAR(45) AS nombre;          
        ...
        BEGIN   
                ...
                -- SET DEBUG FILE TO '/tmp/mfinis/sp_pos_recuperanombre.out';
                -- TRACE ON;
                
                IF pUsuario = '' OR pIdFuncion = '' OR pEjecut = '' THEN
                    LET cCodRet = '00003';
					RETURN cCodRet,cNombre;
                END IF;
				...
				SELECT nombre INTO cNombre FROM bdinteg:"informix".si_ejecut where ejecutivo = pEjecut;
...
'AUTOR: Daniel Reyes Guillen',
'FECHA: 25/11/2021',
'DESCRIPCION: Sp encargado consultar el nombre del ejecutivo',
'BD: bdicnweb';
