CREATE PROCEDURE "informix".sp_abono_sd(pCuenta_eje CHAR(20), pCuenta_sd CHAR(20), pTipoMov CHAR(1), pCanal CHAR(1), pFecOper DATE, 
    pHorOper CHAR(10), pMontoAbono MONEY(14,2), pTipoAbono INT)
    ...
    RETURNING   CHAR (5)    AS pcodRetorno,         --Codigo de retorno
				...
				INTEGER     AS pTipoAbono,          --Abono manual o inicial (1/3)
                CHAR(2)     AS pTipoApartado,       --1 con abano automatico y 2 Sin abono Automatico
                CHAR(10)    AS pDiaAboIni;          --Representa los numero de la semana 1-7 o del mes 1-30
    ...
    DEFINE vMontoInicial    MONEY(14,2);
    ...
    BEGIN
        ON EXCEPTION SET vsqlerr, iIsamErr, cErrorInfo
            IF  vsqlerr != 0 THEN
                ...
                RETURN  vCodRet,vCuenta_eje,vCuenta_sd,vFecha_oper,vHora_oper,vMontoAbono,vFolioOper,vNombre_sd,vIcono,vColor,
                        vMonto_meta,vFecha_meta,vMonto_acum,vMontAboAuto,vPeriodicidad,vFechUltAbo,vProxAboAut,vEst_sd,vCanal,pTipoAbono,vTipoApartado,vDiaAboIni;
            END IF;
        END EXCEPTION;
        ...
        LET pTipoAbono = NVL(pTipoAbono, 0);
        --SE VALIDA QUE EL TIPO DE MOVIMIENTO SEA ABONO
        IF NVL(vTipMov, 0) <>  1 OR  (pTipoAbono <> 1 AND pTipoAbono <> 3)  THEN
        ...
        --DATOS DEL APARTADO
        SELECT 
            NVL(monto_meta, 0), NVL(monto_acum, 0),  estatus,  nombre_sd,   NVL(periodo, 0), NVL(monto_ahor_auto, 0),tipo_apartado,
            icono, color,fecha_meta,periodicidad,ult_fech_abo_auto,prox_fech_abo_auto,canal,dia_del_cobro,monto_ini,fecha_proc
		INTO   
            vMonto_meta, vMonto_acum,vEst_sd, vNombre_sd, vPeriodo,mMtoNvoAuto, vTipoApartado,vIcono,
            vColor,vFecha_meta,vPeriodicidad,vFechUltAbo,vProxAboAut, vCanal,vDiaAboIni,vMontoInicial,vFecha_proc
        ...
        IF(pTipoAbono = "3") THEN
            IF vFecha_proc IS NOT NULL OR vTipoApartado = "1" THEN
                LET vCodRet = '00027'; --El apartado ya tiene un monto inicial o el tipo de apartado no permite abono inicial
                RETURN  vCodRet,vCuenta_eje,vCuenta_sd,vFecha_oper,vHora_oper,vMontoAbono,vFolioOper,vNombre_sd,vIcono,vColor,
                        vMonto_meta,vFecha_meta,vMonto_acum,vMontAboAuto,vPeriodicidad,vFechUltAbo,vProxAboAut,vEst_sd,vCanal,pTipoAbono,vTipoApartado,vDiaAboIni;
            END IF;

            LET vMontoInicial =  pMontoAbono;
        END IF;
        ...
        IF (vMontoAbono > v_ret10 AND pTipoAbono <> 3) OR ((vMontoAbono > v_ret10 OR vMontoAbono > 10000000) AND pTipoAbono = 3) OR (vMontoAbono < vMontoMin AND vTipoApartado = 1) OR (vMontoAbono <= 0 AND vTipoApartado = 2) THEN
            LET  vCodRet='00009'; --Saldo no disponible para realizar el abono.
            RETURN  vCodRet,vCuenta_eje,vCuenta_sd,vFecha_oper,vHora_oper,vMontoAbono,vFolioOper,vNombre_sd,vIcono,vColor,
                    vMonto_meta,vFecha_meta,vMonto_acum,vMontAboAuto,vPeriodicidad,vFechUltAbo,vProxAboAut,vEst_sd,vCanal,pTipoAbono,vTipoApartado,vDiaAboIni;
        END IF;
		
        IF (vTipoApartado = "1") THEN
            LET vMontoRest = NVL((vMonto_meta - vMonto_acum),0);
        ELSE
            LET vMontoRest = NVL((10000000 - vMonto_acum),0); --El monto maximo de un apartado tipo 2 es 10 millones
        END IF;
        ...
        LET vMonto_acum = vMonto_acum + vMontoAbono;

        IF (vTipoApartado = "1") THEN
            IF vPeriodo > 0 THEN
                LET mMtoNvoAuto = (vMonto_meta - vMonto_acum)/vPeriodo;
                LET vMontoRest = vMonto_meta - vMonto_acum;		
        ...
        SET monto_acum = vMonto_acum,
            monto_ahor_auto = mMtoNvoAuto, -- se quitara si el abono no cuenta como parte del dia del cobro del apartado
			periodo = vPeriodo,
            fecha_proc = vFecha_oper,
            monto_ini = vMontoInicial
        ...
        INSERT INTO "informix".sc_mov_sd VALUES (vCuenta_eje,vCuenta_sd,vFolioOper,1,1,vFecha_oper,vHora_oper,vMontoAbono,pTipoAbono);
        ...
        IF (vTipoApartado = "1" AND vMonto_meta = vMonto_acum AND vMonto_meta >= 1) THEN
            LET vEst_sd     = 3;
            LET vPeriodo    = 0;
            LET vProxAboAut = "";
        ...
        RETURN  vCodRet,vCuenta_eje,vCuenta_sd,vFecha_oper,vHora_oper,vMontoAbono,vFolioOper,vNombre_sd,vIcono,vColor,
            vMonto_meta,vFecha_meta,vMonto_acum,mMtoNvoAuto,vPeriodicidad,vFechUltAbo,vProxAboAut,vEst_sd,vCanal,pTipoAbono,vTipoApartado,vDiaAboIni;
    END; 
END PROCEDURE
DOCUMENT
