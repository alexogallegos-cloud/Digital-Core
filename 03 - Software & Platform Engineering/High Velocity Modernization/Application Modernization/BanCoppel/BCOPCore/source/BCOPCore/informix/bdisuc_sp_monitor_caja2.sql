CREATE PROCEDURE "informix".sp_monitor_caja2()
RETURNING CHAR(5);
...
DEFINE vt2_empresa       CHAR(3);
DEFINE vt2_sucursal      CHAR(4);
DEFINE vt2_fecha_operacion DATE;
DEFINE vt2_divisa        CHAR(2);
DEFINE vt2_procedencia   CHAR(4);
DEFINE vt2_cod_trans     CHAR(4);
DEFINE vt2_monto         MONEY(16,2);

DEFINE vt3_empresa       CHAR(3);
DEFINE vt3_sucursal      CHAR(4);
DEFINE vt3_fecha_env     DATE;
DEFINE vt3_monto         MONEY(16,2);
DEFINE vt3_folio_oper    CHAR(8);
DEFINE vt3_status        CHAR(2);
DEFINE vt3_folio_sucursal CHAR(16);
DEFINE vt3_cod_proveedor  CHAR(4);

DEFINE vt4_sucursal      CHAR(4);
DEFINE vt4_suc_abrio     CHAR(1);
DEFINE vt4_suc_cerro     CHAR(1);
DEFINE vt4_fecha_pase    DATE;
DEFINE vt4_usuario       CHAR(8);
...
LET vt3_folio_oper       = "";
LET vt3_status           = "";
LET vt3_folio_sucursal    = "";
LET vt3_cod_proveedor     = "";
...
--SET debug FILE TO 'sp_monitor_caja.out';
--TRACE ON;
...
WHERE bdinteg:si_fechas.empresa = sempresa;


   ---Ejecuta Cursor principal para revisar la relacion de cajas por sucursal
 FOREACH
      SELECT distinct ss_operaciones.empresa,ss_operaciones.sucursal,fecha_operacion,divisa,procedencia,
             ss_operaciones.cod_trans,ss_operaciones.monto

        INTO vt2_empresa, vt2_sucursal, vt2_fecha_operacion, vt2_divisa, vt2_procedencia,
             vt2_cod_trans, vt2_monto
      FROM ss_operaciones
      WHERE ss_operaciones.empresa = sempresa



      ORDER BY 3,2

           --Valida si la Sucursal esta Abierta Y/O Cerrada.
           ...
           SELECT sucursal,suc_abrio,suc_cerro,fecha_pase
           INTO vt4_sucursal, vt4_suc_abrio, vt4_suc_cerro, vt4_fecha_pase
           FROM ss_pase_sucursal
           WHERE ss_pase_sucursal.sucursal = vt2_sucursal
           AND ss_pase_sucursal.fecha_pase = svt_fecha_hoy;


          --Valida si existe la fecha de cada sucursal

          IF vt4_sucursal IS NULL AND vt4_fecha_pase IS NULL THEN

            --Da de Alta el Registro

            INSERT INTO ss_pase_sucursal
            VALUES(vt2_sucursal,"1","1",svt_fecha_hoy,"95736042");

            UPDATE ss_pase_sucursal
            SET(suc_cerro)=("1")
            WHERE fecha_pase < svt_fecha_hoy 
            AND sucursal = vt2_sucursal;
...
"Descripcion: Ejecuta Cursor principal de Actualizacion de folios para solicitud movil",
"Fecha      : 20/03/2015",
"Modifico   : ";

CREATE PROCEDURE "informix".sp_consul_dotacion2(eEmpresa    CHAR(3), 
                                               eTipo       CHAR(1), 
                                               eProveedor  CHAR(4),
                                               eSucursal   CHAR(4),
                                               eFecInicio  DATE,
...
SET LOCK MODE TO WAIT 3;
SET ISOLATION TO DIRTY READ;
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consul_dotacion2.out';
        --TRACE ON;
		IF eProveedor <> '' and eSucursal = '' THEN   --** Por proveedor
                ...
                END IF
           FOREACH
                  SELECT SKIP eRegistros FIRST eRecup b.sucursal,a.fecha_operacion,b.status,b.folio_oper,b.monto, a.usuario
...
SET LOCK MODE TO WAIT 3;
 ...
 --SET DEBUG FILE TO "/respaldosbd/felipe/Sps/sp_guarda_reclamo_bym.out";
 --TRACE ON;
	...
	IF TRIM(NVL(pOpcion,'')) = '1' OR TRIM(NVL(pOpcion,'')) = '2' THEN
		...
		ELIF TRIM(NVL(pOpcion,'')) = '2' THEN
			...
			IF  TRIM(NVL(pNumRecibo,'')) <> '' AND TRIM(NVL(pFechaOperacion,'')) <> '' AND TRIM(NVL(pTipoOperReclamo,'')) <> '' AND TRIM(NVL(pCodEmpReclamo,'')) <> '' AND TRIM(NVL(pDescEmpReclamo,'')) <> '' AND TRIM(NVL(pTipoRecibo,'')) <> ''  AND NVL(pIdTenedor,0) > 0  THEN
				...
				SET tipo_oper_reclamo = pTipoOperReclamo, fecha_operacion = pFechaOperacion,  cod_empresa_reclamo = pCodEmpReclamo, desc_empresa_reclamo = pDescEmpReclamo, desc_suc_reclamo = TRIM(NVL(pDescSucReclamo,'')), 
					num_suc_reclamo = TRIM(NVL(pNumSucReclamo,'')), domicilio_suc_reclamo = TRIM(NVL(pDomSucReclamo,'')), tel_suc_reclamo = TRIM(NVL(pTelSucReclamo,'')), tipo_recibo = pTipoRecibo, observaciones = TRIM(NVL(pObservaciones,''))
...
'FECHA: 03/02/2015',
'DESCRIPCION: Guarda los datos del reclamo en la tabla ss_recibo_bym_falsos',
'BD: bdisuc';

CREATE PROCEDURE "informix".reversion_sobrante(pempresa     CHAR(3),
											   psucursal    CHAR(4),
											   pusuario     CHAR(8),
											   pRef_sobra   CHAR(20),
											   ptiporev     CHAR(1),
	   ...
	   DEFINE wfechoy DATE;
	   DEFINE cEstatusSobrantre CHAR(1);
	   ...
	   --let wfechoy  = today;
		...
		--SET DEBUG FILE TO "/informix/reversion_sobrante.out";
		--TRACE ON;
		...
		SELECT estado_trans INTO cEstatusSobrantre FROM bdisuc:"informix".ss_sobrantescaja 
		WHERE empresa = pempresa AND referencia = pRef_sobra AND TO_CHAR(fech_hor_insert, '%Y-%m-%d')  = TO_CHAR(wfechoy, '%Y-%m-%d') ;

		
		IF DBINFO('sqlca.sqlerrd2') = 1 THEN
			IF cEstatusSobrantre = 'C' AND ptran = '19' THEN
				LET cod_ret = '01327';
				RETURN cod_ret;
			
		  else 
		
			IF cEstatusSobrantre = 'C' THEN
				LET cEstatusSobrantre = 'A';
			ELIF cEstatusSobrantre = 'A' THEN
				LET cEstatusSobrantre = 'R';
			END IF;
			UPDATE bdisuc:"informix".ss_sobrantescaja
			SET estado_trans= cEstatusSobrantre , gerente_autoriza = pusuario, fech_actualiza = CURRENT
			WHERE empresa = pempresa AND referencia = pRef_sobra;  
		end if   
	 end if 
	   RETURN cod_ret;
	END;
	...
	DOCUMENT
	'DSB 20/08/2015',
	'Folio: 1741 -  Eliminación de Sobrante con numero de referencia siempre y cuando exista',
	'Autor: 95142134 Mario Gallardo',
	'Fecha: 20/08/2015',
	'Modificación: Se agrega validacion para que se reversen operaciones 19 y 20 te tabla ss_sobrantescaja',
	...
	'DB:bdisuc',
	'Modifico: Ivan Castillo Montalvo se valida sobrante mediante referencia (unicamente trasnsacciones 19 y 20)',
	'Sustento: RQM 06-298 RQM Eliminación de Sobrante con numero de referencia siempre y cuando exista .pdf',
	'Fecha: 09/11/2015',
	'DB:bdisuc';
