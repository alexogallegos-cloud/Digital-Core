CREATE PROCEDURE "informix".sp_ics_maectrl_carga_estatus_inactiva()
...
--DefiniciÃÂ³n de variables
	...
	DEFINE SCRIPT_EJECUCION_UNLOAD 	VARCHAR(34);
	...
	DEFINE SCRIPT_EJECUCION_LOAD 	VARCHAR(33);
	DEFINE iContador  			    INTEGER;
	DEFINE v_num_credito            VARCHAR(14);
...
--InicializaciÃÂ³n de variables
	LET RUTA                        = '/RESPALDOSNEW/'; -- PROD /RESPALDOSNEW/ = DEV = /ifxsif01/
	LET vDia                        = '';
	LET vMes                        = '';
	LET vAnio                       = '';
	...
	--SET DEBUG FILE TO '/ifxsif01/sp_ics_maectrl_carga_estatus_inactiva.out';
    --TRACE ON;

	BEGIN 		
        ON EXCEPTION SET SQLERR, ISAM_ERR, ERROR_INFO
            
            --SET DEBUG FILE TO RUTA||"sp_ics_maectrl_carga_estatus_inactiva.err.out" WITH APPEND;
            --TRACE ON;
    ...
    END EXCEPTION;
		...
		INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (0, horaActual, 'INICIA RESPALDO CREDITOS INACTIVOS');
		
		-- Nombre de los archivos
	    LET NOMBRE_UNL_ARCHIVO = 'ics_maectrl_respaldo.unl';
		LET SCRIPT_EJECUCION_LOAD = 'ics_maectrl_respaldo_load.sql';
	    LET SCRIPT_EJECUCION_UNLOAD = 'ics_maectrl_respaldo_unload.sql';
	    

	    -- Elimina registros en tabla ics_maectrl_temp si es que existen
		DELETE FROM bdicred:"informix".ics_maectrl_temp;
		
		-- Consulta para obtener los creditos inactivos en ICS, validando la fecha y envia la salida al archivo ics_maectrl_respaldo_unload.sql
	    LET vExecuteSQL	= '';
        LET vExecuteSQL = 'echo "SET ISOLATION TO DIRTY READ; '||
        '   SET LOCK MODE TO WAIT 3; '||
        '       UNLOAD TO '||RUTA||NOMBRE_UNL_ARCHIVO||
        '   SELECT * '||        
        '    FROM bdicred:\"informix\".ics_maectrl' ||
        " WHERE NOT activo_ics and fecha_desactivado_ics <= today; "||        
         '" >'||RUTA||SCRIPT_EJECUCION_UNLOAD;            
        SYSTEM vExecuteSQL;
		
		
		-- EjecuciÃÂ³n de script ics_maectrl_respaldo_unload.sql
        LET vExecuteSQL   = '';
        LET vExecuteSQL   = 'dbaccess bdicred '||RUTA||SCRIPT_EJECUCION_UNLOAD;
        SYSTEM vExecuteSQL;
		
	   
	   -- Sentencia para leer el archivo ics_maectrl_respaldo.unl e insertar en la tabla ics_maectrl_temp
       LET vExecuteSQL = 'echo "LOAD FROM '|| TRIM(RUTA) || NOMBRE_UNL_ARCHIVO|| ' INSERT INTO ics_maectrl_temp;" > ' || TRIM(RUTA) || SCRIPT_EJECUCION_LOAD||' "LOAD FROM '|| TRIM(RUTA) || NOMBRE_UNL_ARCHIVO|| ' INSERT INTO ics_maectrl_historico;" > ' || TRIM(RUTA) || SCRIPT_EJECUCION_LOAD;
	   SYSTEM vExecuteSQL;
	   
	   
	   -- EjecuciÃÂ³n de script ics_maectrl_respaldo_load.sql' y carga datos a las tablas: ics_maectrl_temp, ics_maectrl_historico
	   LET vExecuteSQL = '';
	   LET vExecuteSQL = 'dbaccess bdicred '|| TRIM(RUTA) || SCRIPT_EJECUCION_LOAD;
	   SYSTEM vExecuteSQL;
	   
	   
	    -- Verificamos que existan creditos inactivos en ICS, si existen continuamos con el proceso de migraciÃÂ³n
	  	IF (SELECT COUNT(*) FROM bdicred:"informix".ics_maectrl_temp) > 0 THEN
	  	
	        BEGIN WORK;
		    FOREACH WITH HOLD SELECT num_credito  INTO v_num_credito FROM bdicred:"informix".ics_maectrl_temp
                    -- Realizamos doble verificaciÃÂ³n para saber si el crÃÂ©dito ya se encuentra migrado, si este se encuentra en la tabla ics_maectrl_historico, procedemos a borrar de la tabla ics_maectrl
					--IF exists(select 1 from ics_maectrl_historico where num_credito= v_num_credito and fecha_desactivado_ics <= today) THEN //JVG-25-08-2022 solo se comento la linea
						Delete from bdicred:ics_maectrl where num_credito in (v_num_credito);
						--Delete from bdicred:ics_maectrl_temp where num_credito in (v_num_credito); //JVG-25-08-2022 solo se comento la linea
					--END IF; //JVG-25-08-2022 solo se comento la linea
            ...
            ELSE
       	    --  Si no hay creditos a migrar, controlamos y mandamos mensaje 
			
			
                LET vCODIGO_RETORNO  = '00001';
                LET vMENSAJE_RETORNO = 'No existen creditos inactivos en ICS para migrar';
                
                SELECT DBINFO("utc_to_datetime", sh_curtime) 
			    INTO horaActual 
		        FROM sysmaster:sysshmvals;
		        INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (0, horaActual, 'FIN RESPALDO CREDITOS INACTIVOS');
	    ...
	    -- Elimina registros en tabla ics_maectrl_temp si es que existen
		DELETE FROM bdicred:"informix".ics_maectrl_temp;		--//JVG-25-08-2022 se paso este delete que estaba anteriormente en el ciclo
					
	   -- Eliminamos archivos generados ics_maectrl_respaldo.unl, ics_maectrl_respaldo_unload.sql, ics_maectrl_respaldo_load.sql
        LET vExecuteSQL = '';
        LET vExecuteSQL = 'rm -f ' ||RUTA||NOMBRE_UNL_ARCHIVO||'  '||RUTA||SCRIPT_EJECUCION_UNLOAD||'  '||RUTA||SCRIPT_EJECUCION_LOAD;
        SYSTEM vExecuteSQL;
...
'Fecha de creacion: 09/03/2022 ',
'Jorge Lara',
'Coordinacion de aclaraciones - Gerencia IV',
'Descripcion: Toma los creditos inactivos de ICS y los migra a una tabla historica'
;

CREATE PROCEDURE "informix".sp_ics_pagos_crd(p_num_ejecucion INTEGER) 
		returning 
			CHAR(5) 		AS 		v_cod_ret;
	...
	LET c_dia_semana                    = 0;
	 ...
	 --SET DEBUG FILE TO "/resplogifx/info_ics/pbas_iniciales/icspagos02.out"; 
     --TRACE ON;
	...
	BEGIN
		...
		--OBTIENE FECHA ACTUAL Y DIA DE LA SEMANA
			...
			SELECT DAY(fecha_hoy), WEEKDAY(fecha_hoy)
		...
		SELECT DBINFO("utc_to_datetime", sh_curtime) 
			INTO horaActual 
		FROM sysmaster:sysshmvals;
		INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (iContador1, horaActual, 'IN_CICLO_PAGOS_ACTUALIZADO_CRD');
		
		SELECT MAX(numero_ejecucion) INTO v_ejecucion_proceso FROM "informix".ics_control_ejecucion;
		
		BEGIN WORK;
			LET v_transaccion = 1;
			LET v_proceso = 'Creditos NO Revolventes Pagos';				
			...
			FOREACH WITH HOLD
				...
				SELECT mc.numcte, mc.num_producto, mc.num_credito, mc.sucursal,  mc.rfc, mc.fecha_apertura
					INTO v_num_cliente, v_acc_customer_id, v_account_number, v_cust_branch_id, v_identity_code, v_fecha_apertura
				...
				ELSE
						...
						IF v_activo_ics = 'f' and v_fecha_desactivado_ics = (v_fecha_ejecucion + 1) THEN
								...
								VALUES
									...
									0, 0, v_rate, 0, v_session_id, v_fecha_ejecucion	);
						
						
						ELSE	
							--Busca en la movdiacrd
							/*SELECT FIRST 1 fecha_mov
								INTO v_maxima_fecha
							FROM bdicred:sd_movhiscrd 
							WHERE empresa = '001' and codigo_fun in (SELECT cod_fun FROM sd_conceptospagomanualcrd) AND codigo_ref = 1
							AND num_credito= v_account_number and reversado = 'N' and fecha_mov = v_fecha_ejecucion;*/
							
								--IF v_maxima_fecha IS NOT NULL THEN --SI NO ES NULA ES POR EXISTIO PAGO
							
									SELECT SUM(monto)
										INTO v_payment_amount
									FROM sd_movhiscrd 
									WHERE codigo_fun in (SELECT cod_fun FROM sd_conceptospagomanualcrd) AND codigo_ref = 1
									AND num_credito= v_account_number and reversado = 'N' and fecha_mov = v_fecha_ejecucion;
									...
									FROM sd_conceptospagomanualcrd
									...
									LET v_payment_type = "PAGOCREDITO";
										...
										IF v_support_entity IS NOT NULL THEN
											...
											--LET v_payment_type = "PAGOCREDITO";--v_transacc_suc || ' ' ||v_cod_fun;
								...
								IF v_payment_amount > 0 /*and v_maxima_fecha IS NOT NULL*/ THEN
									...
									LET v_payment_date_char = TO_CHAR(v_fecha_ejecucion,'%d/%m/%Y');
									/*IF v_maxima_fecha = '' OR v_payment_date_char IS NULL THEN
										LET v_payment_date_char = '01/01/1900';
									END IF;*/
								...
								IF iContador1 >= 100000 THEN
									SELECT DBINFO("utc_to_datetime", sh_curtime) 
										INTO horaActual 
									FROM sysmaster:sysshmvals;
									INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (iContador1, horaActual, 'CICLO_PAGOS_ACTUALIZADO_CRD');
									LET iContador1 = 0;
								END IF; 
			...
			END FOREACH;
				...
				INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (iContador1, horaActual, 'FIN_CICLO_PAGOS_ACTUALIZADO_CRD');
				LET v_ejecucion_proceso = v_ejecucion_proceso + 1;
				
				IF p_num_ejecucion = '10' THEN
					INSERT INTO  "informix".ics_control_ejecucion(fecha_ejecucion, numero_ejecucion) VALUES(current, v_ejecucion_proceso);
				END IF;
...
CREATE PROCEDURE "informix".sp_ics_genera_layouts_hilos( p_num_ejecucion integer) 
	...
	DEFINE delinquency					DECIMAL(18,2);
	DEFINE principal_delinquent 		DECIMAL(18,2);
	DEFINE interest_delinquent 			DECIMAL(18,2);
	DEFINE collection_chargue 			DECIMAL(18,2);
	DEFINE collection_chargue_proyec	DECIMAL(18,2);
	...
	DEFINE v_tasa_moratoria				DECIMAL(9,6);
	DEFINE v_sdo_moratorio 				DECIMAL(18,2);
	DEFINE v_sdo_360 					DECIMAL(18,2);
	DEFINE v_moratorio_proyeccion 		DECIMAL(18,2);
	DEFINE v_iva_moratorio_proyeccion	DECIMAL(18,2);
	DEFINE v_moratorio_proyeccion_2		DECIMAL(18,2);
	...
	--Inicialización de Variables SP
	...
	LET v_balance							= 0.0; 
	LET delinquency						= 0.0; 
	LET principal_delinquent 			= 0.0; 
	LET interest_delinquent 			= 0.0; 
	LET collection_chargue 				= 0.0; 
	LET v_cap_trans 					= 0.0;
	...
	LET collection_chargue_proyec		= 0.0; 
	LET v_monto_vencido 				= 0.0;
	LET v_tasa_moratoria				= 0.0;
	LET v_sdo_moratorio 				= 0.0;
	LET v_sdo_360 						= 0.0;
	LET v_moratorio_proyeccion 			= 0.0;
	LET v_iva_moratorio_proyeccion		= 0.0;
	LET v_moratorio_proyeccion_2		= 0.0;
	LET v_dia							= NULL;
	LET v_dia_diferencial_20 			= NULL;
	LET v_dia_diferencial				= NULL;
	LET v_pagos_vencidos				= 0;
   ...
   -- TRACE ON; 
	...
	BEGIN
		...
		/*TRUNCATE TABLE "informix".ics_clientes_cred_crd;*/

		/*ALTER TABLE ics_clientes_cred_crd MODIFY (ics_consecutivo serial (1));
		
		DROP INDEX IF EXISTS informix.ics_clientes_idx_tipo_cred_crd ONLINE;
		DROP INDEX IF EXISTS informix.ics_clientes_idx_balance_cliente_cred_crd ONLINE;
		DROP INDEX IF EXISTS informix.ics_clientes_idx_consecutivo_cred_crd ONLINE;
		DROP INDEX IF EXISTS informix.ics_clientes_idx_rfc_cred_crd ONLINE;
		DROP INDEX IF EXISTS informix.ics_clientes_idx_cte_cred_crd ONLINE;*/
		...
		--ALTER SEQUENCE "informix".ics_consecutivo RESTART with 1; --Reinicia el valor de la secuencia a 1
		--update statistics medium for table "informix".ics_clientes_cred_crd;
		...
		/*SELECT DBINFO("utc_to_datetime", sh_curtime) 
			INTO horaActual 
		FROM sysmaster:sysshmvals;
		INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (1, horaActual, 'inicio ejecucion uni crd');*/
		
		LET v_dia = DAY(c_fecha_actual);
		LET v_dia_diferencial = WEEKDAY(c_fecha_actual);
		
		----Obtención de creditos revolventes
		BEGIN WORK;
			LET v_proceso ='Creditos Revolventes';

			 LET v_transaccion = 1 ;
			--IF iDia_corte = 18 OR iDia_corte = 20 OR v_ejecucion_proceso = '1'  THEN
			
			/*SELECT num_credito, NVL(sdo_cap_insoluto,0) sdo_cap_insoluto, NVL(sdo_retenido,0) sdo_retenido, NVL(mto_venc_trasp,0) mto_venc_trasp, NVL(monto_financiado,0) monto_financiado
					FROM bdicred:"informix".sd_maesdos
					WHERE sdo_cap_insoluto > 0
				INTO TEMP tmp_maesdos WITH NO LOG;*/
			SELECT valor_inicial, valor_final, fecha_ejecucion
			...
			WHERE numero_hilo = p_num_ejecucion and tipo_cred = '1';
				...
				FOREACH WITH HOLD
					...
					SELECT num_credito
						INTO v_num_credito
					FROM ics_clientes  
					WHERE tipo_cred = '1' AND num_credito BETWEEN v_valor_inicial AND v_valor_final
					
						SELECT /*num_credito,*/ NVL(sdo_cap_insoluto,0) sdo_cap_insoluto, NVL(sdo_retenido,0) sdo_retenido, NVL(mto_venc_trasp,0) mto_venc_trasp, NVL(monto_financiado,0) monto_financiado, monto_vencido
							INTO /*v_num_credito,*/ dSdoActCap, dSdoRetenido, v_cap_trans, v_cap_vdo_exig, v_monto_vencido
						FROM bdicred:"informix".sd_maesdos WHERE num_credito = v_num_credito;

					--IF dSdoActCap > 0 THEN --rev
					
						SELECT numcte, status_cred, sucursal, bandera_ministra, fecha_apertura, fecha_vencim, num_producto, tasa_interes, tasa_moratorios
							INTO v_numcte, v_status_cred, v_sucursal, v_bandera_ministra, v_fecha_apertura, v_fecha_vencim, v_num_producto, v_tasa_interes, v_tasa_moratoria
						FROM "informix".sd_maecred WHERE empresa='001' and num_credito = v_num_credito; --rev
						--evaluar foreach para info combinada de TEMP y MAESTRA
						
							LET v_numcte = TRIM(v_numcte);
							LET v_num_credito = TRIM(v_num_credito);
							LET v_tipo_cred = '1';							
							SELECT fecha_proceso into v_fecha_proceso from "informix".sd_maecredanexo where empresa='001' and num_credito = v_num_credito;
						
							SELECT rfc INTO v_identity_code FROM bdinteg:si_cliente WHERE numcte = v_numcte; 
							
							CALL bdicred:"informix".sp_obtener_pagomin(cEmpresa,v_num_credito)
								RETURNING codRet_sp_sdos, codRet_sp_sdos, v_comodin_decimal_sp, dIntVdo, dIntMoratorio,dIvaIntVdo, v_pagos_vencidos, 
							dIvaIntMoratorio, v_comodin_decimal_sp, v_comodin_decimal_sp, v_comodin_decimal_sp, v_comodin_decimal_sp;
							
							--=========== Se generan consultas para obtener el interes y moratorios ==========================
							...
							IF	v_dia = 20 AND  v_dia_diferencial <> 0 THEN
								
								LET v_dia_diferencial_20 = 7 - v_dia_diferencial;
									
								LET v_tasa_moratoria = (v_tasa_moratoria/100);
								LET v_sdo_moratorio = v_tasa_moratoria * v_monto_vencido;
								LET v_sdo_360 = (v_sdo_moratorio / 360);
								LET v_moratorio_proyeccion = (v_sdo_360 * v_dia_diferencial_20);
								--LET v_moratorio_proyeccion_2 = v_moratorio_proyeccion;
								LET v_iva_moratorio_proyeccion = (v_moratorio_proyeccion * 0.16);
							...
							ELSE
								...
								LET v_sdo_moratorio = v_tasa_moratoria * v_monto_vencido;
								LET v_sdo_360 = (v_sdo_moratorio / 360);
								LET v_moratorio_proyeccion = (v_sdo_360 * 7);
								--LET v_moratorio_proyeccion_2 = v_moratorio_proyeccion;
								LET v_iva_moratorio_proyeccion = (v_moratorio_proyeccion * 0.16);
								--LET v_moratorio_proyeccion = v_moratorio_proyeccion;
							END IF;
							
							SELECT sum(NVL(monto_int_iva,0))
								INTO vRetCs_acum
							FROM bdicred:"informix".sd_promocion_credito
							WHERE num_credito = v_num_credito AND status = 0;
							...
							LET v_balance = dSdoTotalLiq; --actualziado
											
					--De	linquency
							LET delinquency = /*v_cap_trans +*/ v_cap_vdo_exig + dIntVdo + dIntMoratorio + dIvaIntVdo + dIvaIntMoratorio + v_com_pend + v_iva_com;
						
							--Principal_delinquent
							LET principal_delinquent = /*v_cap_trans +*/ v_cap_vdo_exig;
							
							--Interest_ delinquent
							LET interest_delinquent =  dIntVdo + dIntMoratorio + dIvaIntVdo + dIvaIntMoratorio;
							
							--Collection_chargue
							LET collection_chargue = dIntMoratorio + dIvaIntMoratorio + v_moratorio_proyeccion + v_iva_moratorio_proyeccion; 
							
								--Se valida que la fecha proceso sea igual a la fecha de ejecución
								IF v_fecha_proceso = c_fecha_actual OR v_fecha_proceso = c_fecha_actual_2 AND v_status_cred in('E1','E2', 'E3', 'VP') /*and v_balance > 0*/ THEN
									--IF v_contador_inactivo = 0 OR v_contador_cte_nunca = 0 THEN
										
										UPDATE "informix".ics_clientes
											SET numcte=v_numcte,
												status_cred=v_status_cred,
												sucursal=v_sucursal,
												rfc=v_identity_code,
												bandera_ministra=v_bandera_ministra,
												fecha_apertura=v_fecha_apertura,
												fecha_vencim=v_fecha_vencim,
												fecha_ejecucion=c_fecha_actual,
												num_producto=v_num_producto,
												tasa_interes=v_tasa_interes,
												tipo_cred=v_tipo_cred,
												balance=v_balance,
												delinquency=delinquency,
												principal_delinquent=principal_delinquent,
												interest_delinquent=interest_delinquent,
												collection_chargue=collection_chargue,
												collection_chargue_proyec = v_moratorio_proyeccion,
												collection_chargue_proyec_iva = v_iva_moratorio_proyeccion,
												pagos_vencidos = v_pagos_vencidos --;--tiempos_inicial
										where num_credito = v_num_credito;
										
										
										/*INSERT INTO "informix".ics_clientes_cred(
											numcte, num_credito, status_cred, sucursal, bandera_ministra,rfc, 
											fecha_apertura, fecha_vencim, fecha_ejecucion, num_producto, tasa_interes, tipo_cred,
											balance, delinquency, principal_delinquent, interest_delinquent, collection_chargue)
										VALUES(
											v_numcte, v_num_credito, v_status_cred, v_sucursal, v_bandera_ministra, v_identity_code,
											v_fecha_apertura, v_fecha_vencim, c_fecha_actual, v_num_producto, v_tasa_interes, v_tipo_cred,
											balance, delinquency, principal_delinquent, interest_delinquent, collection_chargue);*/
									--END IF;
								ELSE
										DELETE FROM "informix".ics_clientes WHERE num_credito = v_num_credito;
								END IF;
												
						LET dIvaIntMoratorio = NULL;
		...
		--Obtención de Universo de Clientes de Creditos Fijos:
		BEGIN WORK;	
			LET v_proceso ='Creditos No revolventes Fijos';
			LET  v_transaccion = 1 ;

			/*SELECT num_credito, NVL(sdo_cap_insoluto,0) sdo_cap_insoluto, NVL(sdo_retenido,0) sdo_retenido, NVL(mto_venc_trasp,0) mto_venc_trasp, NVL(monto_financiado,0) monto_financiado, NVL(monto_otorgado,0) monto_otorgado, NVL(sdo_intereses,0) sdo_intereses
			FROM bdicred:"informix".sd_maesdoscrd
			WHERE sdo_cap_insoluto > 0
			into temp tmp_sd_maesdoscrd with no log;*/
			...
			FOREACH WITH HOLD 
				...
				/*SELECT num_credito, sdo_cap_insoluto, sdo_retenido, mto_venc_trasp, monto_financiado, monto_otorgado, sdo_intereses
						INTO v_num_credito, dSdoActCap, dSdoRetenido,  v_cap_trans, v_cap_vdo_exig, dLineaOtorgada, dIntDevengado
				FROM tmp_sd_maesdoscrd*/
					
					SELECT num_credito
						INTO v_num_credito
					FROM ics_clientes  
					WHERE tipo_cred = '2' AND num_credito BETWEEN v_valor_inicial AND v_valor_final
					
					SELECT /*num_credito,*/ NVL(sdo_cap_insoluto,0) sdo_cap_insoluto, NVL(sdo_retenido,0) sdo_retenido, NVL(mto_venc_trasp,0) mto_venc_trasp,
					NVL(monto_financiado,0) monto_financiado, NVL(monto_otorgado,0) monto_otorgado, NVL(sdo_intereses,0) sdo_intereses, monto_vencido
						INTO /*v_num_credito,*/ dSdoActCap, dSdoRetenido,  v_cap_trans, v_cap_vdo_exig, dLineaOtorgada, dIntDevengado, v_monto_vencido
						FROM bdicred:"informix".sd_maesdoscrd
					WHERE num_credito = v_num_credito;
					...
					CALL bdicred:"informix".sp_obtener_pagomin(cEmpresa,v_num_credito)
					RETURNING codRet_sp_sdos, codRet_sp_sdos, v_comodin_decimal_sp, dIntVdo, dIntMoratorio,dIvaIntVdo, v_pagos_vencidos, 
					dIvaIntMoratorio, v_comodin_decimal_sp, v_comodin_decimal_sp, dIntVig, dIvaIntVig;
					/*
								SELECT iva
									INTO dIvaSuc
					...
					CALL bdicred:"informix".monthadd(v_fecha_apertura, 1) RETURNING dtMesiversario;
				
					SELECT ind_comision, tran_comision
						INTO cind_comision, ctran_comision
					FROM  sd_definicion
					WHERE empresa='001' and num_producto = v_num_producto;
						...
						*/
						...
						WHERE empresa='001' and cod_comis=ctran_comision;
					...
					SELECT a.fecha_cuota, a.iva_fecha_pago
						INTO dtFechaCuota, dtIvaFechaPag
					FROM bdicred:"informix".sd_amortiza_creditocrd a
					WHERE a.empresa = cEmpresa AND a.num_credito = v_num_credito AND a.capital_status = '3';
					
					CALL bdicred:"informix".calc_iva_grav_pp(cEmpresa,v_num_credito,v_tasa_interes,dIvaSuc,c_fecha_actual, dtIvaFechaPag,dtFechaOrigen,dtFechaCuota,dIntDevengado)
					RETURNING codRet_sp_sdos,dIvaIntDevengado,codRet_sp_sdos;
					...
					SELECT fecha_proceso INTO v_fecha_proceso 
					from "informix".sd_maecredanexocrd where empresa='001' and num_credito = v_num_credito;
						...
						IF v_fecha_proceso = c_fecha_actual OR v_fecha_proceso = c_fecha_actual_2 AND v_status_cred in('E1','E2', 'E3', 'VP') /*and v_balance > 0*/ THEN
							...
							/*INSERT INTO "informix".ics_clientes_cred_crd(
									numcte, num_credito, status_cred, sucursal, bandera_ministra, rfc, 
									fecha_apertura, fecha_vencim, fecha_ejecucion, num_producto, tasa_interes, tipo_cred,
									balance, delinquency, principal_delinquent, interest_delinquent, collection_chargue)
								VALUES(
	...
	BEGIN WORK;
		...
		FOREACH WITH HOLD
			...
			SELECT num_credito INTO v_num_credito FROM ics_clientes where balance <= 0 and tipo_cred = '2' AND num_credito BETWEEN v_valor_inicial AND v_valor_final
			
			IF v_num_credito IS NOT NULL THEN
				DELETE FROM "informix".ics_clientes WHERE num_credito = v_num_credito;
			END IF;
	...
	BEGIN WORK;
		...
		FOREACH WITH HOLD
			...
			SELECT num_credito INTO v_num_credito FROM ics_clientes where balance <= 0 and tipo_cred = '1' AND num_credito BETWEEN v_valor_inicial AND v_valor_final
	...
	/*	CREATE INDEX informix.ics_clientes_idx_tipo_cred_crd    ON informix.ics_clientes_cred_crd(tipo_cred);
		CREATE INDEX informix.ics_clientes_idx_balance_cliente_cred_crd    ON informix.ics_clientes_cred_crd(balance);
		CREATE INDEX informix.ics_clientes_idx_consecutivo_cred_crd		    ON informix.ics_clientes_cred_crd(ics_consecutivo);
		CREATE INDEX informix.ics_clientes_idx_rfc_cred_crd		    ON informix.ics_clientes_cred_crd(rfc);
		CREATE INDEX informix.ics_clientes_idx_cte_cred_crd	    ON informix.ics_clientes_cred_crd(numcte)
		;
		...
		SELECT DBINFO("utc_to_datetime", sh_curtime) 
			INTO horaActual 
		FROM sysmaster:sysshmvals;
		INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (2, horaActual, 'Fin ejecucion crd');*/
...
'Area			:	Sistemas Administrativos y Perifericos',
					'Gerencia de Mtto y Soporte IV',
'Coordinador	:	Norberto Corona Berruecos',
'Analista    	:	Rey David Zavala',
'FECHA			: 	Marzo 2022',
'Requerimiento	:	RQM 09 596',
'VERSION		: 	1.0.0',
'Analista  MOD 	:	Rey David Zavala',
