CREATE PROCEDURE "informix".sp_selfechacontable(p_sEmpresa CHAR(3))
RETURNING CHAR(5) AS codigo, DATE AS fechacontable, DATETIME HOUR TO SECOND AS hora;
		  
	--DEFINICION DE VARIABLES
	DEFINE vCodret 			CHAR(5);
	DEFINE v_fechacontable	DATE;
	DEFINE iSqlErr          INTEGER;
	DEFINE v_sHora			DATETIME HOUR TO SECOND;
			
	--SET DEBUG FILE TO "/dbexportb/Fabiola/sp_selfechacontable.out"; 
	--TRACE ON;
	...
	BEGIN
		...
		LET vCodret = '000';
		LET v_fechacontable = MDY(01,01,YEAR(CURRENT));
		LET v_sHora = EXTEND(CURRENT,HOUR TO SECOND);

		SELECT fecha_hoy
		INTO v_fechacontable
		FROM bdicont:co_fechas WHERE empresa = p_sEmpresa;
		
		RETURN vCodret, v_fechacontable, v_sHora;
	END;
END PROCEDURE
DOCUMENT
'CREADO POR: VLADIMIR FÉLIX GÁLVEZ 8/JULIO/2009',
'Sp que obtiene la fecha del sistema de contabilidad';
