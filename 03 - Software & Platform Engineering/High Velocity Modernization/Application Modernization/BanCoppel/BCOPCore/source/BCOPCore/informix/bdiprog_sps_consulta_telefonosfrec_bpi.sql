CREATE PROCEDURE "informix".sps_consulta_telefonosfrec_bpi(p_NumCte CHAR(20), p_CveCuenta CHAR(2), p_Registros SMALLINT)
RETURNING
	 ...
	 CHAR(1) as activarBPI;
	 
--#############################################################################################################
-- SP clonado para la reingenierÃ­a, donde se agrega parÃ¡metro de salida para el tipo de caducidad
-- Bibiana Gaxiola Verdugo
-- 19/12/2012
-- Se agrega filtro para que no presente en la lista los telÃ©fonos frecuentes que no tienen digito verificador
-- Bibiana Gaxiola Verdugo
-- 05/03/2014
------------------
...
--SET DEBUG FILE TO "/informix/gaby/ArchivosOut/sps_consulta_telefonosfrec_bpi.out";
--TRACE ON;
	...
	IF (p_NumCte <> "" AND p_NumCte IS NOT NULL) AND (p_CveCuenta <> "" AND p_CveCuenta IS NOT NULL)  THEN
		...
		SELECT LIMIT 1 ct.cuenta INTO v_ExisteCuenta FROM bdiprog:"informix".pp_ctasterceros ct WHERE ct.num_cte = p_NumCte AND ct.cve_cuenta = p_CveCuenta;
		IF (v_ExisteCuenta IS NOT NULL) THEN
		
            IF TRIM(p_CveCuenta) = '05' THEN
				FOREACH
                    SELECT ct.cuenta, ct.descrip_cta, ct.digito_ver, ct.canal_alta, ct.fecha_insert, ct.hora_insert, NVL(ct.monto_maximo,0), ct.cve_caducidad, '1' AS activaBPI
                    INTO v_CtaDestino,v_DescCta, v_Digito, v_Canal, v_FechaInsert, v_HoraInsert, v_MontoMaximo, v_CveCaducidad, v_ActivaBPI
					...
					UNION
					SELECT bex.cuenta, bex.descrip_cta, bex.digito_ver, bex.canal_alta, bex.fecha_insert, bex.hora_insert, NVL(bex.monto_maximo,0), bex.cve_caducidad, '0' AS activaBPI
                    FROM bdiprog:"informix".pp_ctasterceros_bex bex
                    WHERE bex.num_cte = p_NumCte
                    AND bex.cve_banco = '201'
                    AND bex.cve_cuenta = p_CveCuenta                    
                    AND bex.cve_estado = '01'
					AND bex.digito_ver <> ''
					AND (current - ( YEAR(fecha_insert) || '-' || MONTH(fecha_insert) || '-' || DAY(fecha_insert) || ' ' || hora_insert)::DATETIME YEAR TO FRACTION) < '0 00:30:00'
					ORDER BY activaBPI ASC, ct.descrip_cta ASC, ct.cuenta
					...
					RETURN v_cod_ret, v_CtaDestino, v_DescCta, v_Digito, v_Inhabil, v_MontoMaximo, v_CveCaducidad, v_ActivaBPI WITH RESUME;
				END FOREACH;
