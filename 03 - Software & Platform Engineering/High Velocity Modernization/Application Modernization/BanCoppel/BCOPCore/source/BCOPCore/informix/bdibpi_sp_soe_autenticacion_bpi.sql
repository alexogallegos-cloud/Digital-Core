CREATE PROCEDURE "informix".sp_soe_autenticacion_bpi(pUsuario CHAR(8), pIdFuncion CHAR(10), pRfc CHAR(13))
	...
	BEGIN
		...
		--SET	DEBUG FILE TO '/informix/Berenice/sp_soe_autenticacion_bpi.out';
		--TRACE ON;
		...
		WHERE rfc = TRIM(pRfc) AND tpo_persona IN ('02', '04') AND tipo_cliente = '1';
		...
		WHERE c.rfc = TRIM(pRfc) AND c.tpo_persona IN ('02', '04') AND c.tipo_cliente = '1';
		...
		FOREACH SELECT bp.usuario_aut, 'XX' as nombre2 , 'XX' as apell_paterno, 'XX' as apell_materno, 
            bp.no_identificacion_oficial, bp.folio_contrato, 0,	bp.id_status, 
            bu.id_usuario, cr.correo_elec, tk.ns_token, bp.suc_registro, 1 -- bu.id_tipo_usuario
			...
			FROM bdinteg:si_bpiusuariospm bp
            INNER JOIN bdibpi:"informix".bpi_usuariopm bu ON bu.numcliente = bp.num_cliente AND bu.st_portal='activo'
            INNER JOIN bdinteg:"informix".si_bpitokenpm tk ON tk.num_cliente = bp.num_cliente
			LEFT OUTER JOIN bdinteg:"informix".si_correos cr ON cr.numcte = bp.num_cliente AND status_correo = 'A'
			WHERE bp.num_cliente = cNumCte
...
"FECHA: 07/03/2014",
"DESCRIPCION: Función que realiza la autenticación del cliente persona moral de BPI por medio de su RFC para el mosulo de SOE en CNWEB"
;
