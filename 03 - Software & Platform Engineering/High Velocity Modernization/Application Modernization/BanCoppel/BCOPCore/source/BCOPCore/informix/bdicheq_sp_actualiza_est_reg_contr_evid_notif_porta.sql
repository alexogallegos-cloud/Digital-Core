CREATE PROCEDURE "informix".sp_actualiza_est_reg_contr_evid_notif_porta(
	pCliente 			CHAR(20),				--Numero de cliente
	pFolio_solicitud 	CHAR(30),				--Folio de la solicitud
	pEstatus			CHAR(1))				--Estatus a cambiar
	...
	DEFINE cNumReg					INTEGER;		--Valida que exista el registro
	
	LET Sql_Err				= 0;
	LET cCodRet				= '00000';
	LET cMensajeRet			= 'Se actualizo el estatus del registro correctamente';
	LET cNumReg				= 0;
	...
	BEGIN
		...
		IF (pCliente IS NULL OR pCliente = '' OR 
			pFolio_solicitud IS NULL OR pFolio_solicitud = '' OR 
			pEstatus IS NULL OR pEstatus = '') THEN
			
			LET cCodRet = '00001';
			LET cMensajeRet = 'Campos requeridos vacios, valide';
			
			RETURN cCodRet,cMensajeRet;
		ELSE 
			--Valida que exista el registro en la tabla sc_control_evidencia_notif_portab
			SELECT COUNT(*) INTO cNumReg 
			FROM sc_control_evidencia_notif_portab 
			WHERE cliente = pCliente 
			AND folio_solicitud = pFolio_solicitud;
			
			--Si no existe el registro retornamos el error 00002
			IF(cNumReg == 0) THEN
				LET cCodRet = '00002';
				LET cMensajeRet = 'No existe el registro. Valide los datos';
				
				RETURN cCodRet,cMensajeRet;
			ELSE
				--Actualiza el estatus del registro y su fecha de modificacion
				UPDATE sc_control_evidencia_notif_portab 
				SET estatus = pEstatus, fecha_modificacion =  TODAY
				WHERE cliente = pCliente 
				AND folio_solicitud = pFolio_solicitud;
...
DOCUMENT 
'+----------------------------------------------------------------------------------------------------------------------+',
'+ Realizo: Ezequiel Moreno Paredes																						+',
'+ Fecha Creacion: 01/07/2023																							+',
'+ Proyecto: RQM 06 895  Evidencia aceptacion alta portabilidad															+',
'+ Funcion del SP: Cambia el estatus de un registro en la tabla sc_control_evidencia_notif_portab 						+',
'+----------------------------------------------------------------------------------------------------------------------+';
