CREATE PROCEDURE "informix".sp_valida_procede_fusion(pClienteTitular CHAR(20),pClienteTraspasaCtas CHAR(20),pUsuario CHAR(8))
	RETURNING CHAR(6),CHAR(80);
	
	DEFINE c_CodRet    CHAR(6);
	DEFINE cCodRet 	   CHAR(5);
	DEFINE i_SqlErr    INTEGER;
	DEFINE c_Mensaje   CHAR(80);
	DEFINE cProceso   CHAR(50);
	DEFINE c_detalle_mov2 CHAR(200);
	...
	DEFINE iContador1 INT;
	DEFINE iContador2 INT;
	
	LET c_CodRet = "000000";
	LET cCodRet = "";
	LET i_SqlErr = 0;
	LET c_Mensaje = "Los Clientes si pueden ser fusionados";
	LET cProceso = "FusionClientes";
	LET c_detalle_mov2 = "";
	LET iClubCI = 0; 
	LET iClubCC = 0;
	LET iCtaTel_CC = 0;
	...
	--SET DEBUG FILE TO "/informix/rmarquez/inc64023/sp_valida_procede_fusion.out";
	--TRACE ON;
	...
	BEGIN
		...
		ON EXCEPTION SET i_SqlErr
			IF i_SqlErr <> 0 THEN
				LET c_CodRet = i_SqlErr;
				...
				LET c_detalle_mov2=i_SqlErr||'|'||'sp_valida_procede_fusion';
				INSERT INTO log_fusionclientes(proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES (cProceso,'',pClienteTitular,pClienteTraspasaCtas,c_detalle_mov2,dtFechaInsercion,pUsuario,dtFechaInsercion::DATE);
				RETURN c_CodRet,c_Mensaje;
			END IF;
		END EXCEPTION;
			
		LET c_CodRet = '000000'; --Indica que si procede la fusiÃÂÃÂ³n. 																
		LET iClubCI = 0;
		LET iClubCC = 0;
		
		SELECT COUNT(*) INTO iContador1 FROM bdinteg:si_club_proteccion 
		WHERE numcte = pClienteTitular and empresa = '001' and aceptada IS NOT NULL and fecha_vencimiento>=today-90; 
		
		IF 	iContador1 >=1 THEN
			LET iClubCC = 1;
		END IF;		
		
		SELECT COUNT(*) INTO iContador2 FROM bdinteg:si_club_proteccion 
		WHERE numcte = pClienteTraspasaCtas and empresa = '001' and aceptada IS NOT NULL and fecha_vencimiento>=today-90;
		
		IF 	iContador2 >=1 THEN	
			LET iClubCI = 1;
		END IF;
		
		--Si los dos clientes cuentan con club de proteccion, la fusion de los clientes se cancela
		IF (iClubCI <> 0 AND iClubCC <> 0) THEN
			--Si ambos cuentan con club de protecciÃÂÃÂ³n
			LET cCodRet = '00014';
			LET cProceso  = 'si_club_proteccion';
			LET c_CodRet = '000010';
			LET c_Mensaje = "Clientes con club de protecciÃÂÃÂ³n, no pueden ser fusionados";
			...
			VALUES('CLUB DE PROTECCION', cProceso, pClienteTitular, pClienteTraspasaCtas, cCodRet ||'|'|| 'PROCESO NO REALIZADO, AMBOS CLIENTES TIENEN CLUB DE PROTECCION', CURRENT  , pUsuario, CURRENT);																		
			
		ELSE
			LET iCtaTel_CC = 0;
			LET iCtaTel_CI = 0;
			
			SELECT 1 INTO iCtaTel_CC FROM bdicheq:sc_cuenta_telefono WHERE num_cte = pClienteTitular;
			SELECT 1 INTO iCtaTel_CI FROM bdicheq:sc_cuenta_telefono WHERE num_cte = pClienteTraspasaCtas;
			
			IF (NVL(iCtaTel_CC,0) + NVL(iCtaTel_CI,0)) = 2 THEN
				LET cCodRet = '00018';
				LET cProceso  = 'sc_cuenta_telefono';
				LET c_CodRet = '000011';
				LET c_Mensaje = "Clientes con cuenta-telÃÂÃÂ©fono, no pueden ser fusionados";
				INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
				VALUES('CUENTA-TELEFONO', cProceso, pClienteTitular, pClienteTraspasaCtas, cCodRet ||'|'|| 'PROCESO NO REALIZADO, AMBOS CLIENTES TIENEN CUENTA-TELEFONO', CURRENT  , pUsuario, CURRENT);																		
				
			ELSE
				LET iCtaTransfer_CC = 0;
				LET iCtaTransfer_CI = 0;
				
				SELECT LIMIT 1 1 INTO iCtaTransfer_CC FROM bditransfer:tf_maecte WHERE numcte = pClienteTitular;
				SELECT LIMIT 1 1 INTO iCtaTransfer_CI FROM bditransfer:tf_maecte WHERE numcte = pClienteTraspasaCtas;
				
				IF (NVL(iCtaTransfer_CC,0) + NVL(iCtaTransfer_CI,0) = 2) THEN
					LET cCodRet = '00019';
					LET cProceso  = 'bditransfer:tf_maecte';	
					LET c_CodRet = '000012';
					LET c_Mensaje = "Clientes con cuenta transfer, no pueden ser fusionados";	
					INSERT INTO "informix".log_fusionclientes(proceso, tabla, cliente_tit, cliente_tras, detalle_mov, fecha_hora, user_insert, fecha_insert)
					VALUES('CUENTA TRANSFER', cProceso, pClienteTitular, pClienteTraspasaCtas, cCodRet ||'|'|| 'PROCESO NO REALIZADO, AMBOS CLIENTES TIENEN CUENTA TRANSFER', CURRENT  , pUsuario, CURRENT);																		
				END IF;
			END IF;
		END IF;	
	RETURN c_CodRet, c_Mensaje;				
...
END PROCEDURE
DOCUMENT
'AUTOR:Rocio Karina MÃÂ¡rquez Coronel',
'FECHA:07/ABRIL/2015',
'MODIFICACION: Se crea sp para validar en la fusiÃÂ³n si procede o no una fusiÃÂ³n. ValidarÃÂ¡ cuando los clientes a fusionar tienen club de protecciÃÂ³n, cuenta_telefono o cuenta transfer',
'SUSTENTO: INC 64023',
'SOLICITA: JosÃÂ© Angel Lopez Adams',
'---------------------',
'AUTOR: Jesus Daniel Guerrero Benitez',
'FECHA: 03/10/2024',
'MODIFICACION: Se modifica procedimiento almacenado para permitir realizar la fusion de clientes cuando solo un cliente tiene activo y vigente el club de proteccion';

CREATE PROCEDURE "informix".sp_actualiza_estado(p_tabla INTEGER, p_proceso INTEGER)
	...
	BEGIN
		ON EXCEPTION SET iSql_err
            IF iSql_err <> 0 THEN
				...
				RETURN cCodRet, iTot_act, dInicio, CURRENT;
		...
		BEGIN WORK;
			...
			IF p_tabla = 1 THEN --tabla si_direcciones
				IF p_proceso = 1 THEN -- ActualizaciÃ³n de estado
					FOREACH WITH HOLD
						SELECT numcte, secuencia, estado
						INTO sNumcte, iSecuencia, sEstado
						FROM bdinteg:"informix".tmp_act_edo_cte
						WHERE tabla = 'A'
							AND actualizado = 0
						
						LET cEstado = '0' || TRIM(sEstado);
						
						UPDATE bdinteg:"informix".si_direcciones
						SET estado = cEstado
						WHERE numcte = sNumcte 
							AND secuencia = iSecuencia;
						
						UPDATE bdinteg:"informix".tmp_act_edo_cte
						SET actualizado = 1
						WHERE numcte = sNumcte 
							AND secuencia = iSecuencia
							AND tabla = 'A';
						...
						END IF;
					END FOREACH;
				ELIF p_proceso = 2 THEN -- Reverso de la actualizaciÃ³n de estado
					FOREACH WITH HOLD
						SELECT numcte, secuencia, estado
						INTO sNumcte, iSecuencia, sEstado
						FROM bdinteg:"informix".tmp_act_edo_cte
					...
					END FOREACH;
				END IF;
			ELIF p_tabla = 2 THEN --tabla si_direcciones_actual
				IF p_proceso = 1 THEN -- ActualizaciÃ³n de estado
					FOREACH WITH HOLD
						SELECT numcte, secuencia, estado
						INTO sNumcte, iSecuencia, sEstado
...
"FUNCIONAMIENTO:SP para actualizar el ID de la eentidad federativa en las tablas si_direcciones y si_direcciones_actual",
"FECHA : 27-08-2024",
"BD    : bdiinteg";

CREATE PROCEDURE "informix".sp_seg_consultausuario(p_id_usuario CHAR(13))
    RETURNING CHAR(5) AS codret, 
              INTEGER AS id_nivel_consulta;

    DEFINE cCodRet CHAR(5);
    DEFINE iSqlErr INTEGER;
    DEFINE iNivel_consulta INTEGER;

    LET cCodRet = '00000'; 
    LET iSqlErr = 0;
    LET iNivel_consulta = 0;

    BEGIN
        ON EXCEPTION
            SET iSqlErr LET cCodRet = iSqlErr;
            RETURN cCodRet, iNivel_consulta;
        END EXCEPTION;

        --SET DEBUG FILE TO '/tmp/mfinis/Dann/sp_seg_consultaUsuario_debug';
        --TRACE ON;

        SET LOCK MODE TO WAIT 3;
        SET ISOLATION TO DIRTY READ;

        SELECT MIN(nivel_acceso) INTO iNivel_consulta
        FROM bdinteg:"informix".si_seg_nivel_acceso_modulo
...
DOCUMENT 
'AUTOR: HECTOR DANIEL JUAREZ PEREZ',
'FECHA:  24/04/2024',
'DESCRIPCION: Este SP nos devolverÃ¡ el nivel de acceso del usuario seleccionado.',
'DB: bdinteg';

CREATE PROCEDURE "informix".sp_seg_validaejecutivo(cNUMEMPLEADO CHAR(8),cTIPO_LOGIN CHAR (1),cCONTRASENA CHAR(88),cIP CHAR(16),cMAC CHAR(12))
    RETURNING CHAR(5),CHAR(8),CHAR(1),CHAR(45);
	
	DEFINE iexiste 				INT;
	DEFINE cCodRet 				CHAR(5);
	DEFINE iSql_err 			INT;
	DEFINE cNumero_empeleado	CHAR(8);
	...
	BEGIN
		...
		ON EXCEPTION SET iSql_err
            IF iSql_err <> 0 THEN
                LET cCodRet = iSql_err;
                RETURN 
				cCodRet,cNumero_empeleado,cUsu_estado,cNombre;
            END IF;
        END EXCEPTION;
		
		--SET DEBUG FILE TO "/informix/Ana/SOC/sp_seg_validaejecutivo.out";
		--TRACE ON;
        ...
        IF cTIPO_LOGIN='1' THEN
            ...
            END IF;
        END IF;
		IF (SELECT COUNT(ejecutivo) FROM si_ejecut where ejecutivo =cNUMEMPLEADO) = 0 THEN 
			LET cCodRet = "00002";
			RETURN 
				cCodRet,cNumero_empeleado,cUsu_estado,cNombre;
		END IF;	
		IF (SELECT COUNT(ejecutivo) FROM si_ejecut where ejecutivo =cNUMEMPLEADO) = 1 THEN
            IF (SELECT COUNT(ejecutivo) FROM si_macejecutivo WHERE ejecutivo  = cNUMEMPLEADO and mac = cMAC) = 1 THEN 
                SELECT EJ.ejecutivo,MA.status,EJ.nombre 
                INTO cNumero_empeleado,cUsu_estado,cNombre
                FROM si_ejecut EJ
                LEFT JOIN si_macejecutivo MA
                ON MA.ejecutivo = EJ.ejecutivo
                WHERE EJ.ejecutivo = cNUMEMPLEADO AND MA.mac=cMAC;
                IF cTIPO_LOGIN='1' THEN
					SELECT NVL(COUNT(id_usuario),0) INTO iexiste FROM si_seg_usuarios where id_usuario =cNUMEMPLEADO AND encriptacion=TRIM(cCONTRASENA);
                    ...
                    END IF;	 
                END IF;    
               RETURN cCodRet,cNumero_empeleado,cUsu_estado,cNombre;
            ELSE 
                LET cCodRet = "00006";
                RETURN 
                cCodRet,cNumero_empeleado,cUsu_estado,cNombre;
...
"FUNCIONAMIENTO:Este sp evaluara si existe el usuario dentro de la base de datos, verificando el numero de empleado, contraseï¿½a  la ip y la macaddress de la maquina",
"Autor : Victor H. Sanchez",
"FUNCIONAMIENTO:Mantenimiento para direccionar la contraseï¿½a a ï¿½a tabla de usuarios.",
"FECHA : 10-01-2012",
"AUTOR: L. Montserrat Leon Amador",
"FECHA: 06/05/2020",
"DESCRIPCION: SPL clon del procedimiento sp_cnsif_validaejecutivo2 para la implementaciï¿½n en la funcionalidad Encriptaciï¿½n de Contraseï¿½as.",
"AUTOR: JosÃ© Antonio RamÃ­rez Franco",
"FECHA : 06-09-2024",
"DESCRIPCION: Se cambia el tamaÃ±o del parametro cCONTRASENA de CHAR(50) a CHAR(88).",
"BD    : bdinteg",
"VER   : 1.0";
