CREATE PROCEDURE "informix".sps_user_manc_aut_bei2(
												pNumCliente CHAR(9),
												pIdUser integer,
												pNoReg integer,
												pRegIni integer,
   ...
   CHAR(5),   INTEGER,   INTEGER,    CHAR(9),    CHAR(50),     CHAR(150),     CHAR(15),
    SMALLINT,     CHAR(50),    SMALLINT,    SMALLINT,    CHAR(50),    INTEGER,INTEGER,INTEGER;
  ...
  BEGIN
   ...
   ON EXCEPTION SET sql_err
      IF sql_err <> 0 THEN
           ...
           RETURN cod_ret, iTotalReg,NVL(sIdUsuario,-1), sNumCliente,sUserNom,sNombre,sTelCelular,sCiaCel,sEmail,sIdStatus,sIdTipoUser,sUserNomAdmin,sIdAdminManco,sIdUserAdmin,sTipoMov ;
      END IF ;
   END EXCEPTION ;
...
--**************************************************************************************************************
     ...
     IF iTotalReg == 0 THEN
        ...
        --  RETURN cod_ret, iTotalReg,NVL(sIdUsuario,-1), sNumCliente,sUserNom,sNombre,sTelCelular,sCiaCel,sEmail,sIdStatus,sIdTipoUser,sUserNomAdmin,sIdAdminManco;
	  RETURN cod_ret, iTotalReg,NVL(sIdUsuario,-1), NVL(sNumCliente,''),NVL(sUserNom,''),NVL(sNombre,''),NVL(sTelCelular,''),NVL(sCiaCel,-1),NVL(sEmail,''),NVL(sIdStatus,-1),NVL(sIdTipoUser,-1),NVL(sUserNomAdmin,''),NVL(sIdAdminManco,-1),NVL(sIdUserAdmin,-1),NVL(sTipoMov,-1);
        END IF ;
...
--**************************************************************************************************************
    	IF NVL(pIdTipoMov,99) <> 99 THEN
          FOREACH
            ...
            usu.cia_cel,usu.e_mail,usu.id_status,usu.id_tipo_usuario,us.usuario_bei,usu.id_admin_manco,usu.id_usuario_admin,usu.tipo_mov
         ...
         --   RETURN cod_ret, iTotalReg,NVL(sIdUsuario,-1), sNumCliente,sUserNom,sNombre,sTelCelular,sCiaCel,sEmail,sIdStatus,sIdTipoUser,sUserNomAdmin,sIdAdminManco WITH RESUME;
          RETURN cod_ret, iTotalReg,NVL(sIdUsuario,-1), NVL(sNumCliente,''),NVL(sUserNom,''),NVL(sNombre,''),NVL(sTelCelular,''),NVL(sCiaCel,-1),NVL(sEmail,''),NVL(sIdStatus,-1),NVL(sIdTipoUser,-1),NVL(sUserNomAdmin,''),NVL(sIdAdminManco,-1),NVL(sIdUserAdmin,-1),NVL(sTipoMov,-1) WITH RESUME;
