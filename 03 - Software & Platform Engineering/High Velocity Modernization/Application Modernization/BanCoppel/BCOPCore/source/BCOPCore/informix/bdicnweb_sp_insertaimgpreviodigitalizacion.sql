CREATE PROCEDURE "informix".sp_insertaimgpreviodigitalizacion(pUsuario CHAR(8), pIdFuncion CHAR(10), pCliente CHAR(20), pCodDocto CHAR(4), pImgFormato CHAR(3), pSvrRemoto CHAR(30))
	...
	BEGIN
		...
		IF pUsuario = '' OR pIdFuncion = '' OR pCliente  = '' OR pCodDocto = '' OR pImgFormato = '' OR pUsuario = '' OR pSvrRemoto = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, sSecuencia;
		END IF;
		...
		LET cQuery = "EXECUTE PROCEDURE" || " "||TRIM(pSvrRemoto)||"'informix'.inserta_img_previo_soc('"||cEmpresa|| "','" ||TRIM(pCliente)||"','" ||pCodDocto|| "','" ||TRIM(pImgFormato)|| "','" ||pUsuario||"');";
...
'FECHA: 07/08/2014',
'DESCRIPCION:  insertar registro en dg_expediente_img sin la imagen',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_existeimagendigitalizacion(pUsuario CHAR(8), pIdFuncion CHAR(10), pCliente CHAR(20), pCodDocto CHAR(4), pSecuencia CHAR(6), pSvrRemoto CHAR(30))
		RETURNING CHAR(5) AS codret,
				  CHAR(1) AS esNula;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_existeimagendigitalizacion.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pCliente  = '' OR pCodDocto = '' OR pSecuencia = '' OR pSvrRemoto = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cEsNula;
		END IF;
...
CREATE PROCEDURE "informix".sp_consultadocsegundadefdigitalizacion2(pUsuario CHAR(8), pIdFuncion CHAR(10), pCodDefinicion CHAR(3), pProducto CHAR(4), pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
				SMALLINT AS secuencia,
	...
	--	SET DEBUG FILE TO '/informix/vamilan/sp_consultadocsegundadefdigitalizacion2.out';
    --    TRACE ON;
		...
		ELSE
			...
			FOREACH EXECUTE PROCEDURE bdidigital:"informix".sp_dgconsultadocumentossegundefinicion2(cEmpresa ,pCodDefinicion, pProducto)
				INTO cCodRetSp, sSecuencia, cCodigoDocumento, cDescripcionDocumento, cCodigoGrupo, cDescripcionGrupo, cMultiImagen, iImgTamanioMax, cImgFormato, sImgCompresion, 
						bImgLargo, bImgAncho, sImgDPI, cImgColores, cGenerico, cLigar
...
CREATE PROCEDURE "informix".sp_consultainformacionctamoral2(pUsuario CHAR(8), pIdFuncion CHAR(10), pCuenta CHAR(20))
                RETURNING CHAR(5) AS codret,
        ...
        BEGIN
				...
				-- SET DEBUG FILE TO '/informix/vamilan/sp_consultainformacionctamoral2.out';
              --  TRACE ON;
                ...
                FOREACH EXECUTE PROCEDURE bdicheq:"informix".sp_ctamec_consultarinfoctamoral2(cEmpresa, pCuenta)
...
CREATE PROCEDURE "informix".sp_guardacuentactemoral2(pUsuario CHAR(8), pIdfuncion CHAR(10), pSucursal CHAR(4), pProducto CHAR(4), pNumCte CHAR(20), 	pNumCot CHAR(2), pClaseCta CHAR(1), pRegFirmas CHAR(1), 
	...
	--	SET DEBUG FILE TO '/informix/vamilan/sp_guardacuentactemoral2.out';
     --   TRACE ON;
...
CREATE PROCEDURE "informix".sp_catbancos(pUsuario CHAR(8), pIdFuncion CHAR(10), pRegistros INTEGER, pRecuperacion INTEGER)
    RETURNING CHAR(5) AS codRet,
		CHAR(3) AS cve_banco,
	...
	BEGIN
		ON EXCEPTION SET iSqlErr
			IF iSqlErr <> 0 THEN
				LET cCodRet = iSqlErr;
				RETURN cCodRet, cCveBanco, cDescBanco;
			END IF;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_catbancos.out';
		--TRACE ON;
		...
		FOREACH
			SELECT SKIP pRegistros FIRST pRecuperacion banco, descripcion 
			INTO cCveBanco, cDescBanco
			FROM bdinteg:"informix".si_bancos 
			ORDER BY descripcion ASC
			
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet, TRIM(cCveBanco), UPPER(TRIM(cDescBanco)) WITH RESUME;		
		END FOREACH;
...
'FECHA 09/11/2017',
'MODULO: DÉBITO',
'FUNCIONALIDAD: CAPTURA DE CUENTAS DE CHEQUES SBC NO DESEADOS',
'DESCRIPCION: SPL encargado de consultar el detalle del catálogo de bancos recuperado de la tabla bdinteg:si_bancos.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consultanombrebanco(pUsuario CHAR(8), pIdFuncion CHAR(10), pCveBanco CHAR(3))
    RETURNING CHAR(5) AS codRet,
		CHAR(40) AS desc_banco;
	...
	BEGIN
		ON EXCEPTION SET iSqlErr
			IF iSqlErr <> 0 THEN
				LET cCodRet = iSqlErr;
				RETURN cCodRet, cDescBanco;
			END IF;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultanombrebanco.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pCveBanco = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cDescBanco;
		END IF;
		...
		INTO cDescBanco
		FROM bdinteg:"informix".si_bancos 
		WHERE banco = pCveBanco;
	
		IF NVL(cDescBanco,'') = '' THEN
			LET cCodRet = '01020'; --EL NÚMERO DE BANCO CAPTURADO NO EXISTE, VERIFIQUE
		END IF;
...
'MODULO: DÉBITO',
'FUNCIONALIDAD: CAPTURA DE CUENTAS DE CHEQUES SBC NO DESEADOS',
'DESCRIPCION: SPL encargado de consultar el nombre del banco.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_registractualizactaspld(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdEjecucion CHAR(1), pCveBanco CHAR(3), pNumCuenta CHAR(20))
    RETURNING CHAR(5) AS codRet,
		CHAR(1) AS existe;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_registractualizactaspld.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pIdEjecucion = '' OR pCveBanco = '' OR pNumCuenta = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cExiste;
		END IF;
		...
		IF pIdEjecucion = '1' THEN
			...
			IF EXISTS (SELECT banco FROM bdicheq:"informix".sc_cuentas_pld WHERE banco = pCveBanco AND numcuenta = pNumCuenta) THEN
				LET cExiste = '1';
			ELSE
				LET cExiste = '0';
			END IF;
		...
		ELIF pIdEjecucion = '2' THEN
			...
			INSERT INTO bdicheq:"informix".sc_cuentas_pld(banco,numcuenta,user_insert,fecha_alta) VALUES(pCveBanco,pNumCuenta,pUsuario,DATE(CURRENT));
			IF DBINFO('sqlca.sqlerrd2') = 0 THEN
		...
		ELIF pIdEjecucion = '3' THEN
			...
			DELETE FROM bdicheq:"informix".sc_cuentas_pld WHERE banco = pCveBanco AND numcuenta = pNumCuenta;
			IF DBINFO('sqlca.sqlerrd2') = 0 THEN
				LET cCodRet = '00236'; --ERROR AL PROCESAR LA SOLICITUD
			END IF;
...
'pIdEjecucion = 1 - Consultar si la clave del banco y el numero de cuenta se encuentran dados de alta en la tabla bdicheq:sc_cuentas_pld.',
'pIdEjecucion = 2 - Guardar un nuevo registro en la tabla bdicheq:sc_cuentas_pld.',
'pIdEjecucion = 3 - Eliminar el registro que contiene la clave del banco y el numero de cuenta ingresado, en la tabla bdicheq:sc_cuentas_pld.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_tr_genreportectesfusionados(pUsuario CHAR(8), pIdFuncion CHAR(10), 
	pFechaInicio DATE, pFechaFin DATE, pIdEmpleado CHAR(8), pRegistros INTEGER, pRecuperacion INTEGER)
		...
		RETURNING CHAR(5) AS codret,
			...
			CHAR(2) AS acta_nac_cte2,
			CHAR(2) AS correcto2,
			CHAR(4) AS sucurcal_cte1,
			CHAR(40) AS nombre_sucurcal_cte1,
			CHAR(4) AS sucurcal_cte2,
			CHAR(40) AS nombre_sucurcal_cte2;
	...
	DEFINE cActaNacCte1 CHAR(2);
	DEFINE cCorrecto1 CHAR(2);
	...
	DEFINE cActaNacCte2 CHAR(2);
	DEFINE cCorrecto2 CHAR(2);
	DEFINE cFusionara CHAR(2);
	DEFINE cEstatus CHAR(50);
	DEFINE cObservaciones CHAR(50);
	DEFINE iExiste1 INTEGER;
	DEFINE iExiste2 INTEGER;
	DEFINE iIdCliente INTEGER;
	DEFINE iRecuperacion INTEGER;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cNumAnalista,cNombreAnalista,dFechaValidacion,cIdCliente1,cNumCliente1,cPrimerNombreCte1,cSegundoNombreCte1,cApellPaternoCte1,cApellMaternoCte1,dFechaNacCte1,cGeneroCte1,cActaNacCte1,cCorrecto1,
			cIdCliente2,cNumCliente2,cPrimerNombreCte2,cSegundoNombreCte2,cApellPaternoCte2,cApellMaternoCte2,dFechaNacCte2,cGeneroCte2,cActaNacCte2,cCorrecto2,cSucursalCte1,cNombreSucursalCte1,cSucursalCte2,cNombreSucursalCte2;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_tr_genreportectesfusionados.out';
		--TRACE ON;
		...
		FOREACH 
			...
			SELECT SKIP pRegistros FIRST pRecuperacion a.user_asig, NVL(b.nombre, '') AS nombre_analista, a.fecha_dict, a.numcte_1, a.numcte_2, a.cte_correcto
			INTO cNumAnalista,cNombreAnalista,dFechaValidacion,cNumCte1,cNumCte2,cCteCorrecto
			...
			SELECT CASE WHEN COUNT(secuencia) > 0 THEN 1 ELSE 0 END
			INTO iExiste1
			...
			WHERE   expe.cod_docto       = td.cod_docto 
					...
					AND expe.cliente     = cNumCliente1
					AND gd.cod_grupo     = '001'
					AND expe.cod_docto   = '0939';
			
			IF iExiste1 = 1 THEN 
				LET cActaNacCte1 = 'SI';
			ELIF iExiste1 = 0 THEN 
				LET cActaNacCte1 = 'NO';
			END IF;
			...
			WHERE   expe.cod_docto       = td.cod_docto 
					AND td.cod_grupo    = gd.cod_grupo 
					-- AND expe.empresa     = '001'
					AND expe.cliente     = cNumCliente2
					AND gd.cod_grupo     = '001'
					AND expe.cod_docto   = '0939';
		
			IF iExiste2 = 1 THEN 
				LET cActaNacCte2 = 'SI';
			ELIF iExiste2 = 0 THEN 
				LET cActaNacCte2 = 'NO';
			END IF;
		
			LET iIdCliente = iIdCliente + 1;
			
			IF cCteCorrecto = '1' THEN
					LET cCorrecto1 = 'SI';
					LET cCorrecto2 = 'NO';
			ELIF cCteCorrecto = '2' THEN
					LET cCorrecto1 = 'NO';
					LET cCorrecto2 = 'SI';
			END IF;
			...
			RETURN cCodRet,cNumAnalista,cNombreAnalista,dFechaValidacion,cIdCliente1,cNumCliente1,cPrimerNombreCte1,cSegundoNombreCte1,cApellPaternoCte1,cApellMaternoCte1,dFechaNacCte1,cGeneroCte1,cActaNacCte1,cCorrecto1,
			cIdCliente2,cNumCliente2,cPrimerNombreCte2,cSegundoNombreCte2,cApellPaternoCte2,cApellMaternoCte2,dFechaNacCte2,cGeneroCte2,cActaNacCte2,cCorrecto2,cSucursalCte1,cNombreSucursalCte1,cSucursalCte2,cNombreSucursalCte2	WITH RESUME;
...
'DESCRIPCION: Consulta el detalle de los registros de fusion semiautomatizada a fusionar.',
'ModificÃ³: Johnattan Esquivel SÃ¡nchez',
'Fecha de modificaciÃ³n: 20/12/2016',
'ModificaciÃ³n: Se modifica para agregar la sucursal y el nombre de la sucursal',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_catalogoautoridad(pUsuario CHAR(8), pIdFuncion CHAR(10))
		RETURNING CHAR(5) AS codret,
			SMALLINT AS id_autoridad,
			CHAR(45) AS desc_autoridad;
			
	DEFINE cCodRet CHAR(5);
	DEFINE iSqlErr INTEGER;
	DEFINE cIdAutoridad SMALLINT;
	DEFINE cDescAutoridad CHAR(45);
	DEFINE iNoRegistros INTEGER;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cIdAutoridad, cDescAutoridad;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_catalogoautoridad.out';
		--TRACE ON;
		...
		FOREACH
			...
			INTO cIdAutoridad, cDescAutoridad
			FROM bdicnweb:"informix".sw_ca_autoridad
			ORDER BY id_autoridad ASC
			
			LET iNoRegistros = iNoRegistros + 1;
			RETURN cCodRet, cIdAutoridad, TRIM(UPPER(cDescAutoridad)) WITH RESUME;
...
'DESCRIPCION: SPL encargado de consultar el detalle del catÃ¡logo autoridad - archivos xml.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_catalogoestatus(pUsuario CHAR(8), pIdFuncion CHAR(10))
		RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cIdEstatus, cDescEstatus;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_catalogoestatus.out';
		--TRACE ON;
		...
		FOREACH
			...
			INTO cIdEstatus, cDescEstatus
			FROM bdicnweb:"informix".sw_ca_estatus
			ORDER BY id ASC
			
			LET iNoRegistros = iNoRegistros + 1;
			RETURN cCodRet, cIdEstatus, TRIM(UPPER(cDescEstatus)) WITH RESUME;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: CONSULTA DE ARCHIVOS XML',
'DESCRIPCION: SPL encargado de consultar el detalle del catÃ¡logo estatus - archivos xml.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_consultaarchivocaxml(pUsuario CHAR(8), pIdFuncion CHAR(10), pClaveOficio CHAR(100), pRegistros INTEGER, pRecuperacion INTEGER)
	RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cNombreOficio,cIdEstatus,cDescEstatus;
		END EXCEPTION;
	 
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_consultaarchivocaxml.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pClaveOficio = '' OR pRegistros IS NULL OR pRecuperacion IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet,cNombreOficio,cIdEstatus,cDescEstatus;
		END IF;		
		...
		FOREACH
			...
			SELECT SKIP pRegistros FIRST pRecuperacion nombre_oficio,id_estatus,desc_estatus
			INTO cNombreOficio,cIdEstatus,cDescEstatus
			FROM bdicnweb:"informix".sw_ca_bitacoraprocesoxml
			WHERE clave_oficio = pClaveOficio
			AND usuario_insert = pUsuario
			ORDER BY id_registro ASC
			
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet,TRIM(cNombreOficio),cIdEstatus,TRIM(UPPER(cDescEstatus)) WITH RESUME;
...
'DESCRIPCION: SPL encargado de consultar el listado de los archivos procesados y no procesados mediante la carga automÃ¡tica de archivos XML.',
'AUTOR: L. Montserrat LeÃ³n Amador',
'FECHA: 09/02/2018',
'DESCRIPCION: Se cancela la conversiÃ³n a mayÃºsculas para la recuperaciÃ³n del nombre del oficio y se agrega paginado.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_detallearchivosxmlnoprocesados(pUsuario CHAR(8), pIdFuncion CHAR(10),
pFechaInicio DATE, pFechaFin DATE, pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
			CHAR(100) AS nombre_oficio,
			CHAR(25) AS fecha_publicacion,
			CHAR(25) AS fecha_hora_procesamiento,
			CHAR(6) AS cod_error,
	...
	DEFINE cFechaHrProcesamiento DATETIME YEAR TO FRACTION(5);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cNombreOficio, dFechaPublicacion, cFechaHrProcesamiento, cCodError, cDescError;
		END EXCEPTION;

		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_detallearchivosxmlnoprocesados.out';
		--TRACE ON;
		...
		--SELECT COUNT(id_estatus) INTO iTotalNoProcesados
		--FROM bdicnweb:"informix".sw_ca_bitacoraprocesoxml WHERE id_estatus = 'N';
		--
		--IF NVL(iTotalNoProcesados,0) = 0 THEN
		--	LET cCodRet = '01030';
		--	RETURN cCodRet, cNombreOficio, dFechaPublicacion, cFechaHrProcesamiento, cCodError, cDescError;
		--END IF;
		...
		FOREACH
			...
			SELECT SKIP pRegistros FIRST pRecuperacion nombre_oficio, fecha_publicacion, fecha_hora_insert, cod_error, desc_error
			INTO cNombreOficio, dFechaPublicacion, cFechaHrProcesamiento, cCodError, cDescError
			FROM bdicnweb:"informix".sw_ca_bitacoraerroresxml
			WHERE fecha_insert BETWEEN pFechaInicio AND pFechaFin
			ORDER BY fecha_hora_insert ASC

			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet, UPPER(cNombreOficio), dFechaPublicacion, cFechaHrProcesamiento, cCodError, UPPER(cDescError) WITH RESUME;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: CONSULTA DE ARCHIVOS XML',
'DESCRIPCION: SPL encargado de consultar el detalle de los archivos xml no procesados.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_detallearchivosxmlnoprocesados_totales(pUsuario CHAR(8), pIdFuncion CHAR(10),
pFechaInicio DATE, pFechaFin DATE)
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_detallearchivosxmlnoprocesados_totales.out';
		--TRACE ON;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: CONSULTA DE ARCHIVOS XML',
'DESCRIPCION: SPL encargado de consultar el nÃºmero total de registros de los archivos xml no procesados.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_detallearchivosxmlprocesados_totales(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumCliente CHAR(20),
pIdEstatus CHAR(1), pFechaInicio DATE, pFechaFin DATE, pNumOficio CHAR(60), pNumExpediente CHAR(60), pIdAutoridad CHAR(2))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_detallearchivosxmlprocesados_totales.out';
		--TRACE ON;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: CONSULTA DE ARCHIVOS XML',
'DESCRIPCION: SPL encargado de consultar el nÃºmero total de registros de los archivos xml procesados.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_extraevalortagxml(pIdRow INTEGER, pEtiqueta VARCHAR(50))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_extraevalortagxml.out';
		--TRACE ON;
		...
		INTO cValor
		FROM bdicnweb:"informix".sw_ca_archivoxml_tmp
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: CARGA AUTOMÃTICA DE ARCHIVOS XML',
'DESCRIPCION: SPL encargado de buscar una etiqueta dentro de la cadena XML, para posteriormente regresar su valor.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_procesaencabezadoxml(pUsuario CHAR(8), pIdFuncion CHAR(10))
	RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_procesaencabezadoxml.out';
		--TRACE ON;
		...
		SELECT id_registro
		INTO iNumIndiceEnc
		FROM "informix".sw_ca_archivoxml_tmp
		WHERE TRIM(xmlfile_data) LIKE '<Cnbv_NumeroOficio>%';
		
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Cnbv_NumeroOficio') INTO cCodRetSp, cNumOficio;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Cnbv_NumeroExpediente') INTO cCodRetSp, cNumExpediente;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Cnbv_SolicitudSiara') INTO cCodRetSp, cSolSiara;
		-- Numero de folio
		SELECT id_registro
		INTO iNumIndiceEnc
		FROM "informix".sw_ca_archivoxml_tmp
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Cnbv_Folio') INTO cCodRetSp, iNumFolio;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Cnbv_OficioYear') INTO cCodRetSp, cAnioOficio;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Cnbv_AreaClave') INTO cCodRetSp, cIdArea;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Cnbv_AreaDescripcion') INTO cCodRetSp, cDescArea;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Cnbv_FechaPublicacion') INTO cCodRetSp, dFechaPublicacion;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Cnbv_DiasPlazo') INTO cCodRetSp, cDiazPlazo;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'AutoridadNombre') INTO cCodRetSp, cNomAutoridad;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'AutoridadEspecificaNombre') INTO cCodRetSp, cNomAutoridadEsp;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'NombreSolicitante') INTO cCodRetSp, cNomSolicitante;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Referencia') INTO cCodRetSp, cReferencia;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Referencia1') INTO cCodRetSp, cReferencia1;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'Referencia2') INTO cCodRetSp, cReferencia2;
		...
		EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iNumIndiceEnc, 'TieneAseguramiento') INTO cCodRetSp, cAseguramiento;
		
		-- Guardado de los datos
		INSERT INTO "informix".sw_ca_encabezado(num_oficio,num_expediente,solicitud_siara,folio,anio_oficio,id_area,desc_area,
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: CARGA AUTOMÃTICA DE ARCHIVOS XML',
'DESCRIPCION: SPL que se encarga de procesar el llenado del encabezado, de acuerdo al contenido del archivo xml cargado.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_procesasolicespecificaxml(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdExpediente INTEGER)
	RETURNING CHAR(5) AS codret;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_procesasolicespecificaxml.out';
		--TRACE ON;
		...
		FOREACH 
			...
			FROM bdicnweb:"informix".sw_ca_archivoxml_tmp
			WHERE REPLACE(REPLACE(TRIM(xmlfile_data),CHR(13) || CHR(10),''),CHR(9),'') LIKE '</SolicitudEspecifica>%'
			AND id_registro > iIdRowMinEspecifica;
			-- SOLICITUD ESPECIFICA
			...
			EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'SolicitudEspecificaId') INTO cCodRetSp, iIdSolicitudEspecifica;
			...
			IF NVL(iIdMinRowInstruccionesXConocer,0) <> 0 AND NVL(iIdMaxRowInstruccionesXConocer,0) <> 0 THEN
				...
				FOREACH 
					...
					EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'InstruccionesCuentasPorConocer') INTO cCodRetSp, cInstruccionesCuentasPorConocer;
					
					-- Guardado de los datos
					INSERT INTO bdicnweb:"informix".sw_ca_solicitudespecifica(id_expediente, id_solicitud_especifica, instrucciones_cuentas_x_conocer)
			...
			FOREACH
				...
				EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'PersonaId') INTO cCodRetSp, iPersonaId;
				...
				EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Caracter') INTO cCodRetSp, cCaracter;
				...
				EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Persona') INTO cCodRetSp, cDescTipoPersona;
				...
				EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Paterno') INTO cCodRetSp, cApellidoPaterno;
				...
				EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Materno') INTO cCodRetSp, cApellidoMaterno;
				...
				EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Nombre') INTO cCodRetSp, cNombre;
				...
				EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Rfc') INTO cCodRetSp, cRfc;
				...
				EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Relacion') INTO cCodRetSp, cRelacion;
				...
				EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Domicilio') INTO cCodRetSp, cDomicilio;
				...
				EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Complementarios') INTO cCodRetSp, cComplementarios;
				
				-- Guardado de los datos
				INSERT INTO bdicnweb:"informix".sw_ca_personassolicitud(id_expediente, id_solicitud_especifica, id_persona, caracter, des_tipo_persona, 
				...
				FOREACH
					...
					EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Entidad') INTO cCodRetSp, cEntidad;
					...
					EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Cuenta') INTO cCodRetSp, cCuenta;
					...
					EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'Instrucciones') INTO cCodRetSp, cInstrucciones;
					
					-- Guardado de los datos
					INSERT INTO bdicnweb:"informix".sw_ca_cuentasconocidas(id_expediente, id_solicitud_especifica, id_persona, entidad, cuenta, intrucciones)
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: CARGA AUTOMÃTICA DE ARCHIVOS XML',
'DESCRIPCION: SPL que se encarga de procesar el llenado de la solicitud especifica, de acuerdo al contenido del archivo xml cargado.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_procesasolicpartesxml(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdExpediente INTEGER)
	RETURNING CHAR(5) AS codret;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_procesasolicpartesxml.out';
		--TRACE ON;
		...
		FOREACH 
			...
			EXECUTE PROCEDURE "informix".sp_ca_extraevalortagxml(iIdRow, 'ParteId') INTO cCodRetSp, iParteId;
			...
			-- Guardado de los datos
			INSERT INTO bdicnweb:"informix".sw_ca_solicitudpartes(id_expediente, id_persona, caracter, des_tipo_persona, ap_paterno, ap_materno, nombre, rfc)
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: CARGA AUTOMÃTICA DE ARCHIVOS XML',
'DESCRIPCION: SPL que se encarga de procesar el llenado de la solicitud partes, de acuerdo al contenido del archivo xml cargado.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_verificastatuscaxml(pUsuario CHAR(8), pIdFuncion CHAR(10))
	RETURNING CHAR(5) AS codret,
	    CHAR(1) AS status,
	    CHAR(1) AS bandera_error,
	    CHAR(6) AS cod_error,
	    CHAR(250) AS desc_error,
		CHAR(100) AS clave_oficio;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			...
			RETURN cCodRet,cStatus,cBanDetError,cCodError,cDescError,cClaveOficio;
		END EXCEPTION;
	 
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_verificastatuscaxml.out';
		--TRACE ON;
		...
		SELECT status,bandera_error,cod_error,desc_error,clave_oficio
		INTO cStatus,cBanDetError,cCodError,cDescError,cClaveOficio
		FROM bdicnweb:"informix".sw_ca_statuscargaxml 
		WHERE usuario_insert = pUsuario;
		
		IF DBINFO('sqlca.sqlerrd2') = 0 THEN		
			RETURN cCodRet,'','','','',''; 
		ELSE 			
			RETURN cCodRet,cStatus,cBanDetError,cCodError,TRIM(cDescError),TRIM(UPPER(cClaveOficio));		
		END IF;	
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: CARGA AUTOMÃTICA DE ARCHIVOS XML', 
'DESCRIPCION: SPL encargado de verificar el status de la carga automÃ¡tica de los archivos XML.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_rem_catalogotipolimite(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdConsulta CHAR(1))
		RETURNING CHAR(5) AS codret,
			SMALLINT AS id_limite,
			CHAR(100) AS desc_limite;
			
	DEFINE cCodRet CHAR(5);
	DEFINE iSqlErr INTEGER;
	DEFINE cIdLimite SMALLINT;
	DEFINE cDescLimite CHAR(100);
	DEFINE iNoRegistros INTEGER;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_rem_catalogotipolimite.out';
		--TRACE ON;
		...
		IF pIdConsulta = '1' THEN
			...
			FOREACH
				...
				FROM bdicnweb:"informix".sw_rem_tipolimite
				WHERE grupo_limite = 'A'
				ORDER BY desc_limite ASC
				
				LET iNoRegistros = iNoRegistros + 1;
				RETURN cCodRet, cIdLimite, TRIM(UPPER(cDescLimite)) WITH RESUME;
...
CREATE PROCEDURE "informix".sp_ca_procesaarchivoxml(pUsuario CHAR(8), pIdFuncion CHAR(10))
	RETURNING CHAR(5) AS codret,
		CHAR(60) AS num_oficio,
		INTEGER AS id_oficio;		
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/informix/OMC/sp_ca_procesaarchivoxml.out';
		--TRACE ON;
		...
		-- 1. Procesamiento del encabezado del archivo
		EXECUTE PROCEDURE "informix".sp_ca_procesaencabezadoxml(pUsuario, pIdFuncion) INTO cCodRetSp, cNumOficio, iIdOficio;
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP "informix".sp_ca_procesaencabezadoxml';
		...
		EXECUTE PROCEDURE "informix".sp_ca_procesasolicpartesxml(pUsuario, pIdFuncion, iIdOficio) INTO cCodRetSp;
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP "informix".sp_ca_procesasolicpartesxml';
		ELIF iCodRetSp > 0 THEN
			LET cCodRet = cCodRetSp;
			RETURN cCodRet, cNumOficio, iIdOficio;
		END IF;
		...
		EXECUTE PROCEDURE "informix".sp_ca_procesasolicespecificaxml(pUsuario, pIdFuncion, iIdOficio) INTO cCodRetSp;
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP "informix".sp_ca_procesasolicespecificaxml';
		ELIF iCodRetSp > 0 THEN
			LET cCodRet = cCodRetSp;
			RETURN cCodRet, cNumOficio, iIdOficio;
		END IF;
		
		DELETE FROM "informix".sw_ca_archivoxml_tmp;
		--TRUNCATE TABLE "informix".sw_ca_archivoxml_tmp;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: CARGA AUTOMÃTICA DE ARCHIVOS XML',
'DESCRIPCION: SPL encargado de procesar el archivo XML cargado en la tabla sw_ca_archivoxml_tmp.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_validamontoremesabts_pba(p_cEmpresa CHAR(3), p_cNombre1 CHAR(40), p_cNombre2 CHAR(40), p_cApellidoPaterno CHAR(40), p_cApellidoMaterno CHAR(40), p_cFechaNacimiento CHAR(8), p_cFechaHoy CHAR(8), p_cMontoAPagar CHAR(20))
    RETURNING
    CHAR(5);
...
CREATE PROCEDURE "informix".sp_ca_actualizastatuscaxml(pUsuario CHAR(8), pIdFuncion CHAR(10), pClaveOficio CHAR(100))
	RETURNING CHAR(5) AS codret;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ca_actualizastatuscaxml.out';
		--TRACE ON;
		...
		UPDATE bdicnweb:"informix".sw_ca_statuscargaxml SET status = 'P' WHERE usuario_insert = pUsuario;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: CARGA AUTOMÃTICA DE ARCHIVOS XML', 
'DESCRIPCION: SPL encargado de actualizar el status de la carga automÃ¡tica de los archivos XML.',
'FECHA: 07/05/2018',
'DESCRIPCION: ModificaciÃ³n al SPL para actualizar estatus por usuario.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ca_ejecutacargaautomaticaxml(pUsuario CHAR(8), pIdFuncion CHAR(10), pRutaCarga CHAR(100), pNumIntentos SMALLINT)
	RETURNING CHAR(5) AS codret,
		CHAR(1) AS bandera_error;		
...
CREATE PROCEDURE "informix".sp_aud_cambioperfilsuc_totales(pUsuario CHAR(8), pIdFuncion CHAR(10), pSucursal CHAR(4), pFechaIni DATE, pFechaFin DATE)
	RETURNING CHAR(5) AS codret,
		INTEGER AS num_registros;		
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_aud_cambioperfilsuc_totales.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = ''OR pSucursal = '' OR pFechaIni IS NULL OR pFechaFin IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet,iNumRegistros;
		END IF;
		...
		SET ISOLATION TO DIRTY READ;
                SELECT COUNT(*) INTO iNumRegistros FROM bdinteg:"informix".si_cambio_perf WHERE sucursal = pSucursal AND fecha_cambio BETWEEN pFechaIni AND pFechaFin; 
...
'FECHA: 07/10/2016',
'MODULO: CONSULTAS',
'FUNCIONALIDAD: CONSULTA CAMBIO DE PERFIL EN SUCURSAL', 
'DESCRIPCION: SPL que se encarga de consultar el número total de solicitudes de cambio de perfil en sucursal.',
'BD: bdicnweb';
