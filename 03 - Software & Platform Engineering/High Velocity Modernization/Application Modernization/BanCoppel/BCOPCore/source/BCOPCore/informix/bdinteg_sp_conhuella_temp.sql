CREATE PROCEDURE "informix".sp_conhuella_temp( cEmpresa CHAR(3),
                                         cSucursal CHAR(4),
                                         cUsuario3 CHAR(8),
                                         cNumCte CHAR(20))
...
define cMapad  CHAR(942);
define cMapai  CHAR(942);
...
END IF;
   ...
   SELECT MAX(secuencia) INTO iSecuencia FROM si_huella_temp WHERE  numcte = cNumCte;
   SELECT dmapa,imapa  INTO cMapad,cMapai
   FROM   si_huella_temp
   WHERE  numcte = cNumCte
   AND status ="M" AND secuencia = iSecuencia;
   IF cMapad is null or cMapai is null THEN
      let cCodRet = "132";
      RETURN cCodRet,cMapad,cMapai;
   END IF
