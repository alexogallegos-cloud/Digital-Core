CREATE PROCEDURE "informix".situacion_pago_banco_cjunk_1_pbajlh(o_empresa CHAR(3),
...
--               BanCoppel, de ser asi no se permite al cliente realizar una
--               solicitud de credito dejando registro en la bitacora de
--               precalificacion.
--Fecha de Modificacon:  12-03-2009
-- Proyecto: Caja Unica
--------------------------------------------------------------------------------
--Autor: Viridiana Osobampo
...
--------------------------------------------------------------------------------
--Autor: Viridiana Osobampo
--Modificacion: Se modifica para que cuando el cliente no tiene el nÃ?ÃÂºm. referencia 
--                como cliente coppel, en la bitÃ?ÃÂ¡cora de precalificacion se asigne
--                el numcte Bancoppel, el motivo serÃ?ÃÂ¡ "B" y cambia la secuencia de 
--                parÃ?ÃÂ¡metro para la consulta de saldo vencido en creditos Bancoppel.
--Peticion:	RQM 10 108 PrÃ?ÃÂ©stamo Personal
...
--              para que el spl estÃ?ÃÂ© parametrizado para todos los productos futuros.
-- Peticion: RQM 10 108 PrÃ?ÃÂ©stamo Personal
--Fecha de Modificacion: 06-11-2009
--------------------------------------------------------------------------------
--Autor: Viridiana Osobampo
--Modificacion: Se unifican los cambios realizados para el proyecto Alta Ã??nica y
--              PrÃ?ÃÂ©stamo Personal BanCoppel en la versiÃ?ÃÂ³n del spl productivo.
-- Peticion: RQM 10 108 PrÃ?ÃÂ©stamo Personal
...
-- Peticion: Alta Ã??nica paso 04
--Fecha de Modificacion: 23-06-2010
--------------------------------------------------------------------------------
--Autor: JesÃ?ÃÂºs Manuel Aguilar Heredia
--Modificacion: Se modifican los mensajes cuando existe motivo de rechazo por la 
--		     situacion del credito del cliente., se modifica el tamaÃ?ÃÂ±o de retorno del mensaje
-- Peticion: Alta Ã??nica paso 04.5
...
LET iScorePuntualidad    = 0;	---Autor: Jonathan Medina(FINAL)	22/08/2021
...
BEGIN
   ...
   END EXCEPTION;
    ...
    --SET DEBUG FILE TO "/tmp/fernanda/situacion_pago_banco_cjunk.out";
    --TRACE ON;	
...
-- Se modifica para realizar una sola conuslta conuslta y traer la referencia de la ultima solicitud
     SELECT limit 1 nvl(a.numcte_ref, " "), nvl(nombre_ref," "), nvl(telefono_ref, " "), nvl(parentesco, " ")
       ...
       FROM "informix".ss_refpersonales a
...
-- 	*************************************
   ...
   IF v_motivo = "1" THEN
       ...
       INSERT INTO "informix".ss_bitacora_precal
       ...
       saldoropa, saldoprestamos, abonomuebles, abonoropa, abonoprestamos, fecha_ultima_compra, grupo,vencidototalaire,abonomensualaire,
       ...
       iSaldoRopa,iSaldoPrestamos,iAbonoMuebles,iAbonoRopa,iAbonoPrestamos,iFechaUltimaCompra, vlGrupo, iVencidoAire,iAbonoAire,
  ...
  -- Ini caja Unica. Viridiana
   ...
   IF crechazo <> 1 THEN
       FOREACH
			...
			UNION
				SELECT a.num_credito, 'CV',a.fecha_apertura,a.credito_externo
               ...
               IF (NVL(dSdo_vencido,0) >= sParamVencido) THEN
                   LET scod_ret= '002';
                   --LET cMensaje = 'El cliente tiene crÃ?ÃÂ©dito vencido en Bancoppel';
                   LET cMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
								   'en la pantalla "SelecciÃ?ÃÂ³n de Productos". El sistema me indica que tiene '||
								   'vencido en su CrÃ?ÃÂ©dito BanCoppel, le recuerdo que es importante ser '||
								   'puntual en sus pagos para conservar los beneficios de su CrÃ?ÃÂ©dito.' ;
                   LET cCausaSol = "CVB";
                   LET crechazo = '1';
                   EXIT FOREACH;
               ELIF TRIM(cStatus_cred) = "CV" THEN
                   LET scod_ret= '003';
                   --LET cMensaje = 'El cliente tiene crÃ?ÃÂ©dito vendido en Bancoppel';
                   LET cMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
								   'en la pantalla "SelecciÃ?ÃÂ³n de Productos". No se le puede ofrecer una '||
								   'solicitud de crÃ?ÃÂ©dito porque cuenta con un crÃ?ÃÂ©dito vendido en '||
                                   'BanCoppel.';
                   LET cCausaSol = "COB";
                   LET crechazo = '1';
                   EXIT FOREACH;
               ELIF TRIM(cStatus_cred) = "FC" THEN			   
					IF NOT EXISTS (SELECT num_credito 
						...
						LET scod_ret= '004';
						--LET cMensaje = 'El cliente cuenta con crÃ?ÃÂ©dito Reestructurado en Bancoppel';
						LET cMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
										'en la pantalla "SelecciÃ?ÃÂ³n de Productos". No se le puede ofrecer una '||
										'solicitud de crÃ?ÃÂ©dito porque cuenta con un crÃ?ÃÂ©dito reestructurado.';
						LET cCausaSol = "CRB";
						LET crechazo = '1';
						EXIT FOREACH;   
					END IF;		   
			   ...
			   ELIF (TRIM(cStatus_cred) in ('AA','E1') and dSdo_vencido = 0) THEN												
                ...
                LET dtMaxFechaCorte=mdy(month(dtMaxFechaCorte),'20',year(dtMaxFechaCorte));
                   ...
                   IF (cGrado_riesgo IN ('D','E')) OR (cGrado_riesgo = cParamGradoRiesgo AND dMto_reserva > cParamReserva) THEN
                       ...
                       LET cMensaje = 'Por el momento le podemos tramitar el (los) producto (s) presentado (s) '||
										'en la pantalla "SelecciÃ?ÃÂ³n de Productos". Le recuerdo que es importante '||
										'realizar puntualmente sus pagos mensuales de su crÃ?ÃÂ©dito BanCoppel.';
                       LET cCausaSol = "MEB";
                       LET crechazo = '1';
                       EXIT FOREACH;
                   END IF;
