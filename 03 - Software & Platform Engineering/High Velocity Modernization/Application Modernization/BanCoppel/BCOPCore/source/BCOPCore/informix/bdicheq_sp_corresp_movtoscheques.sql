CREATE PROCEDURE "informix".sp_corresp_movtoscheques      (pEmpresa  CHAR(3),
                                                           pEjecutivo   CHAR(8),
														   pFecha   DATE,
														   pFolioSuc    CHAR(16))
...
DEFINE cFech_param          DATE;    -- Variable para fecha de los historicos
...
LET cFech_param       = DATE(1);      -- Variable para fecha de los historicos
...
ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
      ...
      RETURN cCodRet,cMensajeRet,cNumCuenta,cFolioSuc,dMonto,cReferencia,cNumTarjeta,cNombreCte,cNumcte,cCancelad;
END EXCEPTION;

            
--set debug file to "/informix/VILLELA/sp_corresp_movtoscheques.out";
--trace on;
...
SET LOCK MODE TO WAIT 3;
	...
	-- // OBTIENE PARAMETROS DE FECHAS DE LOS HISTORICOS DE MOVIMIENTOS
     ...
     AND codparam = 'fechcon_movhis';
        ...
        LET dtFechaIni = dtFechaHoy - 90 UNITS DAY;    --  La consulta se realiza 90 dias atras.
        ...
        END IF;
                    ...
                    SELECT pagina, count(num_cuenta)
                     ...
                     WHERE empresa = pEmpresa
                           ...
                           AND pagina = (SELECT MAX(pagina) FROM bdicred:"informix".sd_movtos_corresponsal
                                                    WHERE empresa = pEmpresa
                                                           AND ejecutivo = pEjecutivo
                                                           AND fecha_insert = dtFechaHoy)
                    group by pagina; 
                ...
                IF pFecha = dtFechaHoy THEN
                                ...
                                RETURN cCodRet,cMensajeRet,cNumCuenta,cFolioSuc,dMonto,cReferencia,cNumTarjeta,cNombreCte,cNumcte,cCancelad WITH RESUME;
			   ...
			   ELIF pFecha >= cFech_param THEN   --  Compara la fecha de la transaccion con la  fecha  del inicio de la movhis
                            ...
                            FOREACH
                                            SELECT h.cuenta,
                                                  ...
                                                  INTO cNumCuenta,
                                                            ...
                                                            cNumcte,
                                                            cCancelad
                                                FROM bdicheq:"informix".sc_movhis h,
                                                            bdicheq:"informix".sc_maechq m,
                                                            bdinteg:"informix".si_cliente c
                                            WHERE h.empresa = m.empresa
                                                  AND h.cuenta = m.cuenta
                                                  AND m.empresa = c.empresa
                                                  AND m.num_cte = c.numcte
                                                  AND h.transacc = "0282"
                                                  AND h.fech_alt  = pFecha
                           ...
                           FOREACH
                                            SELECT          mov.cuenta,
                                                ...
                                                FROM     bdicheq:"informix".sc_movhis_old mov,
                                                         bdicheq:"informix".sc_maechq m,
                                                         bdinteg:"informix".si_cliente c
                                            WHERE     mov.empresa = m.empresa
                                                  AND mov.cuenta = m.cuenta
                                                  AND m.empresa = c.empresa
                                                  AND m.num_cte = c.numcte
                                                  AND mov.transacc = "0282"
                                                  AND mov.fech_alt  = pFecha
                                                  AND mov.folio_suc = pFolioSuc
...
END PROCEDURE
DOCUMENT
"Descripción: Procedimiento que obtiene los movimientos de pagos y depósitos realizados en corresponsal para cuentas de cheques. ",
"Base de datos: bdicheq",
"Autor: Viridiana Osobampo A.",
"Fecha: 20-Abr-2011";

CREATE PROCEDURE "informix".sp_corresp_movtostransprescheques (pEmpresa  CHAR(3),
                                                           pEjecutivo   CHAR(8),
														   pFecha   DATE,
														   pFolioSuc    CHAR(16))
...
--set debug file to "/informix/VILLELA/sp_corresp_movtostransprescheques.out";
--trace on;
...
END PROCEDURE
DOCUMENT
"Descripción: Procedimiento que obtiene los movimientos de pagos y depósitos de transferencias de prestamos Coppel. ",
"Autor: Villela Alex",
"Fecha: 10-Enero-2015";

CREATE PROCEDURE "informix".sp_obtenercuentascoppel(pStatus CHAR(1))
...
BEGIN 
	...
	ON EXCEPTION SET iSqlErr
		IF iSqlErr <> 0 THEN
			LET cCodRet = iSqlErr;
			RETURN TRIM(NVL(cCodRet,"")), TRIM(NVL(cNumCuenta,""));
		END IF;
	END EXCEPTION;
	...
	IF NVL(pStatus,"") = "" THEN
		LET cCodRet = "00001"; -- VALOR DEL PARAMETRO INCORRECTO
		RETURN TRIM(NVL(cCodRet,"")), TRIM(NVL(cNumCuenta,""));
	END IF;
	...
	FOREACH
		SELECT cuenta
		INTO cNumCuenta
		FROM "informix".sc_cuentascoppel
		WHERE status = pStatus
		
		RETURN TRIM(NVL(cCodRet,"")), TRIM(NVL(cNumCuenta,"")) WITH RESUME;
		
	END FOREACH;
	
	IF DBINFO("sqlca.sqlerrd2") = 0 THEN
		LET cCodRet = "00002"; -- NO SE ENCONTRARON CUENTAS
		RETURN TRIM(NVL(cCodRet,"")), TRIM(NVL(cNumCuenta,""));
	END IF;
END;
END PROCEDURE
...
"DESCRIPCION: Extrae las cuentas coppel de la tabla sc_cuentascoppel dependiendo el status.",
"FECHA: 04/09/2014",
"SUSTENTO: Se definio en el requerimiento RQM 06 285 Depósitos Referenciados de Coppel en BanCoppel",
"BD: BDICHEQ";

CREATE PROCEDURE "informix".sp_valida_cta(pEmpresa CHAR(3),pCuenta  CHAR(20),pTipo INTEGER)
	RETURNING
	CHAR(5) AS CodRet,
	CHAR(6) AS CodRet2,
	CHAR(20) AS Cuenta;
...
BEGIN
    ...
    ON EXCEPTION SET iSql_err,iIsamErr
        IF iSql_err <> 0 THEN
            ...
            RETURN cCodRet,cCodRet2,cCuenta;
        END IF;
    END EXCEPTION;  
	
	--SET DEBUG FILE TO "/respaldosbd/mario/sp_valida_cta.out";
	--TRACE ON;

	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO wait 3;
	
	IF NVL(pEmpresa,'') <> '' AND NVL(pCuenta,'') <> '' THEN		
		IF pTipo = 1 THEN
			SELECT a.producto,a.num_cte,a.status_cta INTO cProducto,cCliente,cStatus FROM bdicheq:"informix".sc_maechq a,bdicheq:"informix".sc_producto b WHERE a.empresa = pEmpresa AND a.cuenta = pCuenta AND a.producto = b.producto;
			IF DBINFO('sqlca.sqlerrd2') = 0 THEN
				LET cCodRet2 = '01168';
			ELSE
			...
			END IF;
		ELIF pTipo = 2 THEN
			SELECT num_cte INTO cCliente FROM bdicheq:"informix".sc_maechq WHERE empresa = pEmpresa AND cuenta = pCuenta;
			FOREACH EXECUTE PROCEDURE bdicheq:"informix".sp_consultactasinversion(pTipo, cCliente, pCuenta, 0) INTO cCodRet2, cCuenta, cNombre,cStatus
			END FOREACH;
		END IF;
	ELSE
		LET cCodRet2 = '00086';
...
"Fecha: 17/02/2015",
"Modificación: Se crea procedimiento para validar si existe cuenta de captacion y veriificar si cuenta con cuenta eje asociada a la cuenta de inversion",
