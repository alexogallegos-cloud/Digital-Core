CREATE PROCEDURE "informix".consnombrenumcte_transfer(pEmpresa CHAR(3),pNombre1 CHAR(26),pNombre2 CHAR(26),pPaterno CHAR(26),
pMaterno CHAR(26),pFechaNac DATE,pNo_Rfc CHAR(13),pSecuencia SMALLINT)
RETURNING 
CHAR(6) AS cod_ret,
CHAR(60) AS nombre_completo,
CHAR(20) AS numcteTransfer,
CHAR(13) AS rfc;
...
--SET DEBUG FILE TO '/respaldosbd/mario/consnombrenumcte_transfer.out';
--TRACE ON;
	...
	IF pNo_Rfc IS NOT NULL AND pNo_Rfc != "" THEN
		FOREACH
			...
			DISTINCT(nombre1),nombre2,apell_paterno,apell_materno,numcte_tf,rfc
			INTO v_nombre1,v_nombre2,v_paterno,v_materno,v_numcte,v_rfc
			FROM bditransfer:"informix".tf_maecte
			WHERE rfc = pno_rfc AND status_cta = '1'
			ORDER BY numcte_tf
...
"Fecha:05/08/2014",
"Modificación: Se crea SP para validar si existe el cliente en la tabla bditransfer: tf_maecte .",
"Sustento: modificaciones_busqueda_clientes_caja.doc",
"Solicita:  Berenice Mendez ",
"BD: bditransfer";

CREATE PROCEDURE "informix".sp_actualiza_ctetf( pCuentaTf CHAR(20), pTelefono CHAR(13), pNumCte CHAR(20))
...
--SET DEBUG FILE TO '/respaldosbd/felipe/Sps/sp_actualiza_ctetf.out';
--TRACE ON;
...
BEGIN
	...
	IF NVL(pCuentaTf ,'') <> '' AND NVL(pTelefono ,'') <> ''  AND NVL(pNumCte ,'') <> ''  THEN
...
'AUTOR : 94972834',
'FECHA : 12/05/2014',
'SUSTENTO: Asigna_Tarjeta.pdf',
'SOLICITA: Rodolfo Gomez',
'00001: Falta parametro de entrada obligatorio',
'00002: no se encotro cuenta',
'BD: bditransfer';

CREATE PROCEDURE "informix".sp_actualiza_reversatarjeta_tf(pEmpresa CHAR(3), pNumTarjeta CHAR(16), pStatusEnvio CHAR(1))
...
--SET DEBUG FILE TO '/home/tmp/leonardo/sp_actualiza_reversatarjeta_tf.out';
--TRACE ON;
...
BEGIN
	...
	IF NVL(pEmpresa,'') <> '' AND NVL(pNumTarjeta, '') <> '' AND NVL(pStatusEnvio,'') <> ''  THEN
		
		UPDATE bditransfer:"informix".tf_reversotarjeta SET status_envio = pStatusEnvio WHERE empresa = pEmpresa AND numtarjeta = pNumTarjeta;
...
'FECHA: 11/06/2014',
'MODIFICACIÓN: Se crea sp para Actualizar en la tabla tf_reversotarjeta el estatus_envio',
'SUSTENTO: AsignacionReposicionTarjeta_Anexo.doc',
'SOLICITA: Rodolfo Gomez',
'BD: bditransfer';

CREATE PROCEDURE "informix".sp_cons_errordes_tf(pEmpresa CHAR(3), pError INTEGER)
...
--SET DEBUG FILE TO '/home/tmp/leonardo/sp_cons_errordes_tf.out';
--TRACE ON;
...
BEGIN
	...
	IF NVL(pEmpresa,'') <> '' AND NVL(pError,0) <> 0 THEN
	
		SELECT id_error
		INTO iId_Error
		FROM bditransfer:"informix".tf_errordes
		WHERE id_error = pError
...
'AUTOR :95594213 Leonardo Plata',
'FECHA : 11/06/2014',
'MODIFICACIÓN: Se crea sp para verificar si existe el error que regresa el web services en la tabla  tf_errordes',
'SUSTENTO: AsignacionReposicionTarjeta_Anexo.doc',
'SOLICITA: Rodolfo Gomez',
'BD: bditransfer';

CREATE PROCEDURE  "informix".sp_consctatransfer ( pNumCte char(20), pUltReg smallint)

RETURNING char(5), 	--CodRet
		  char(3),	--CodRet2
		  char(20); --NumCta Transfer
		  
	DEFINE vcod_ret        	char(5);
	DEFINE vcod_ret2        char(3);
    DEFINE vcuentatf        char(20);
	...
	--SET DEBUG FILE TO "/home/sysifx/Gilberto/sp_consctatransfer.out";	
	--TRACE ON;	
	...
	BEGIN
		...
		ON EXCEPTION SET sql_err
			IF sql_err <> 0 THEN
				...
				RETURN vcod_ret, vcod_ret2, vcuentatf;
		...
		IF pNumCte <> "" THEN 
			...
			--SE BUSCA EL NUMERO CUENTA TRANSFER RELACIONANDO POR NUM CLIENTE BANCOPPEL
			FOREACH numCtaTf FOR
				SELECT cuenta_tf 
				INTO   vcuentatf
				FROM   bditransfer: "informix".tf_maecte 
				...
				RETURN vcod_ret, vcod_ret2, vcuentatf WITH RESUME;
			END FOREACH;
			
			IF vcuentatf = "" OR vcuentatf IS NULL THEN 
				--SI NO SE ENCUENTRA LA CUENTA POR RELACION NUM CLIENTE BANCOPPEL SE BUSCA POR NOMBRE
				
				SELECT a.numcte, NVL(a.apell_paterno,""), NVL(a.apell_materno,""), NVL(a.nombre1,""), NVL(a.nombre2,""), NVL(a.rfc,""), b.fecha_nac
				INTO   vnumcte, vapell_paterno, vapell_materno, vnombre1, vnombre2, vrfc, vfnac
				FROM   bdinteg: "informix".si_cliente a,
					   bdinteg: "informix".si_ctepf   b
				WHERE  b.numcte = a.numcte AND
					   a.numcte = pNumCte;
				...
				FOREACH numCtaTf FOR
						...
						INTO   vcuentatf
						FROM bditransfer: "informix".tf_maecte 
						WHERE apell_paterno = vapell_paterno AND
							  apell_materno = vapell_materno AND
							  nombre1 		= vnombre1 		 AND
							  nombre2 		= vnombre2		 AND
							  fecha_nac		= vfnac			 AND
							  rfc			= vrfc AND
							  status_cta = '1'
		...
		IF vcuentatf = "" OR vcuentatf IS NULL THEN
			LET vcod_ret2 = "001"; --NO SE PUDO ENCONTRAR LA CUENTA TRANSFER EN NINGUNO DE LOS DOS CASOS
			RETURN vcod_ret, vcod_ret2, vcuentatf;
		END IF
...
'Autor:951421354 Mario Gallardo',
'Fecha:05/08/2014',
'Modificación: Se modiica para buscar cuentas de el cliente en la tabla bditransfer: tf_maecte por numero de cliente transfer.',
'Sustento: modificaciones_busqueda_clientes_caja.doc',
'Solicita:  Berenice Mendez',
'BD: bditransfer';

CREATE PROCEDURE "informix".sp_error_trama_tf
(
	pTran 			SMALLINT,
	pReturnCode 	CHAR(20),
	pErrorDes		CHAR(256),
...
CREATE PROCEDURE "informix".sp_obtiene_datos_trama_tf(pID SMALLINT)
...
DOCUMENT
'Realiza una consulta con los parámetros de entrada',
'AUTOR : 95579737 - José Ernesto Raygoza Villa',
'FECHA : 22/Abril/2014',
'BD    : bditransfer';
