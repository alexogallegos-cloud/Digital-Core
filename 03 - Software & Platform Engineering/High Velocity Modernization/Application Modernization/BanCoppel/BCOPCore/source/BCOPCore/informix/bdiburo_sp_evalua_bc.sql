CREATE PROCEDURE "informix".sp_evalua_bc(pEmpresa CHAR(3))
...
DEFINE vFechaHoyAumlincred DATE;
DEFINE vFechaHoy    DATE;
DEFINE vFechaAnt    DATE;
DEFINE vProxFecha   DATE;
...
LET status            = "";
LET cTable            = "N";
LET vFechaHoyAumlincred = DATE(1);
LET vFechaHoy     = DATE(1);
LET vFechaAnt     = DATE(1);
LET vProxFecha    = DATE(1);
...
--SET DEBUG FILE TO 'sp_evalua_bc.out';
--TRACE ON;
-- regreso 10122024
...
--obtener la fecha
SELECT fecha_hoy 
  INTO vFechaHoyAumlincred
  FROM bdicred:"informix".sd_fechas_aumlincred
 WHERE empresa = pEmpresa;

/*
SELECT proceso,nvl(status_proc,'')
  INTO wproceso,status
  FROM bdinteg:"informix".sx_contproc
 WHERE proceso = pProceso
   AND (YEAR(fecha) = YEAR(vFechaHoy) AND MONTH (fecha) = MONTH(vFechaHoy))
   AND sistema = "06"
...
IF (nrows = 0 ) then 
	INSERT INTO "informix".sd_contproc
         VALUES (pempresa, pProceso, vFechaHoy, "I", USER, CURRENT, CURRENT, "  ", "Proceso Iniciado");



	INSERT INTO bdinteg:"informix".sx_contproc (empresa,proceso,fecha,sistema,status_proc,ejecutivo,hora_ini,hora_fin,codret)
	     VALUES (pEmpresa, pProceso, vFechaHoy, "06", "I", USER, CURRENT, CURRENT, "  ");
ELSE
	IF  (status = 'C') or (status = 'I') THEN
		UPDATE "informix".sd_contproc
         ...
         WHERE empresa      = pEmpresa
	       ...
	       AND YEAR(fecha)  = YEAR(vFechaHoy)
		   AND MONTH(fecha) = MONTH(vFechaHoy);
	...
	ELSE
		LET cCodRet = "090";
	    LET cMensajeRet = 'EL proceso ya ha sido ejecutado exitosamente en este mes';
	    RETURN cCodRet, cMensajeRet;
	END IF; 	
END IF;	
*/
--BEGIN WORK;
   ...
   EXECUTE PROCEDURE bdiburo:"informix".sp_archivo_respuesta_bc(pEmpresa,vFechaHoyAumlincred) 
...
*/
    ...
    END IF;
		...
		EXECUTE PROCEDURE bdiburo:"informix".sp_interpreta_respuesta_bc(pEmpresa,vFechaHoyAumlincred) 
                     INTO cCodRet, cMensajeRet;
...
*/
		...
		EXECUTE PROCEDURE bdiburo:"informix".sp_valida_respuesta_bc(pEmpresa,vFechaHoyAumlincred) INTO cCodRet, cMensajeRet;
...
--		COMMIT WORK;
    ...
    if YEAR(vFechaHoyAumlincred + 1 units month) > YEAR(vFechaHoyAumlincred) then
       let vFechaHoy = mdy(MONTH(vFechaHoyAumlincred + 1 units month),10,YEAR(vFechaHoyAumlincred + 1 units year));
       let vFechaAnt = mdy(MONTH(vFechaHoyAumlincred + 1 units month),9,YEAR(vFechaHoyAumlincred + 1 units year));
       let vProxFecha = mdy(MONTH(vFechaHoyAumlincred + 1 units month),11,YEAR(vFechaHoyAumlincred + 1 units year));
    else
       let vFechaHoy = mdy(MONTH(vFechaHoyAumlincred + 1 units month),10,YEAR(vFechaHoyAumlincred));
       let vFechaAnt = mdy(MONTH(vFechaHoyAumlincred + 1 units month),9,YEAR(vFechaHoyAumlincred));
       let vProxFecha = mdy(MONTH(vFechaHoyAumlincred + 1 units month),11,YEAR(vFechaHoyAumlincred));
    end if;

    UPDATE bdicred:sd_fechas_aumlincred
    SET
        fecha_hoy     = vFechaHoy,
        fecha_ant     = vFechaAnt,
        prox_fecha    = vProxFecha
    WHERE empresa=pEmpresa;

    EXECUTE PROCEDURE bdicred:"informix".sp_cat_genarch_aumlincred('001', vFechaHoyAumlincred) INTO cCodRet;

    IF cCodRet <> "000000" THEN
       LET cMensajeRet = 'OcurriÃ³ un error en la ejecuciÃ³n del sp_cat_genarch_aumlincred.';
       RETURN cCodRet, cMensajeRet;
    END IF;
/*
  UPDATE sd_contproc
...
CREATE PROCEDURE "informix".sp_guarda_autorizacion_grandata_pbajlh_01(
												  pProcess INT, 
												  pNumCte CHAR(9), 
												  pNumCel CHAR(10), 
												  pCanal INT, 
...
RETURNING CHAR(5) AS cCodRet, 
	...
	DEFINE cFechaAut DATE;
	
	LET cNumCel = '';
	LET cCodRet = '00000'; --EXECUCION EXITOSA
	

	

   	--SET DEBUG FILE TO '/informix/LuisBeltran/BDIBURO/sp_guarda_autorizacion_grandata.out';
   	--TRACE ON;
    ...
    BEGIN
		...
		IF (pProcess = 0) THEN
			IF (pNumCte IS NULL OR pNumCte = '' OR pNumCel IS NULL OR pNumCel= '' )THEN
					LET cCodRet = '00004';			ELSE
				IF EXISTS(SELECT 1 FROM br_cliente_aut WHERE num_cliente = pNumCte AND num_celular = pNumCel)THEN
							LET cNumCel = pNumCel; 
							UPDATE br_cliente_aut SET fecha_hora_aut = CURRENT  WHERE num_cliente = pNumCte AND num_celular = pNumCel;
						ELSE
							INSERT INTO br_cliente_aut( num_cliente,num_celular,id_canal,fecha_hora_aut,fecha_hora_consulta,secuencia_mensual ) 
						VALUES(pNumCte,pNumCel,pCanal,CURRENT,NULL,0);
						LET cNumCel = pNumCel;
					END IF;
			...
			END IF;
		ELIF (pProcess = 1) THEN
			IF (pNumCte IS NULL OR pNumCte = '' OR pNumCel IS NULL OR pNumCel= '' OR pSecuencia = 0)THEN
					LET cCodRet = '00004';			ELSE
					IF EXISTS(SELECT 1 FROM br_cliente_aut WHERE num_cliente = pNumCte AND num_celular = pNumCel)THEN
							LET cNumCel = pNumCel; 
							UPDATE br_cliente_aut SET fecha_hora_consulta = pFechaActualizacion ,secuencia_mensual=pSecuencia WHERE num_cliente = pNumCte AND num_celular = pNumCel;
						ELSE
							LET cCodRet = '00003';					END IF;
			END IF;
				
		ELIF (pProcess = 2) THEN
			IF (pNumCte IS NULL OR pNumCte = '' )THEN
					LET cCodRet = '00004';			ELSE
				SELECT  TRIM(NVL(num_celular, ''))  INTO cNumCel 
				FROM br_cliente_aut  
				WHERE num_cliente = pNumCte 
				AND fecha_hora_aut = (SELECT MAX(fecha_hora_aut) FROM br_cliente_aut WHERE num_cliente = pNumCte);
		...
		ELSE
			LET cCodRet = '00002'; --operacion no valida
...
'DESCRIPCION: Se encarga de guardar en tablas la autorizacion del cliente para consultar por medio de su numero de linea celular',
'informacion crediticia con el proveedor Grandata',
'FECHA : 26 - Julio - 2022',
'BD    : bdiburo';

CREATE PROCEDURE "informix".sp_guarda_autorizacion_grandata_pbajlh_02(
	pProcess INT, 
	pNumCte CHAR(9), 
	pNumCel CHAR(10), 
	pCanal INT, 
	pSecuencia INT8, 
	pFechaActualizacion DATETIME YEAR TO SECOND
)
RETURNING CHAR(5) AS cCodRet, CHAR(10) AS cNumCel;
	...
	IF (pProcess = 0) THEN
		IF (pNumCte IS NULL OR pNumCte = '' OR pNumCel IS NULL OR pNumCel= '' )THEN
			LET cCodRet = '00004';		
			ELIF EXISTS(SELECT 1 FROM "informix".br_cliente_aut WHERE num_cliente = pNumCte AND num_celular = pNumCel)THEN
			LET cNumCel = pNumCel;
			UPDATE "informix".br_cliente_aut SET fecha_hora_aut = CURRENT  WHERE num_cliente = pNumCte AND num_celular = pNumCel;
		ELSE
			INSERT INTO "informix".br_cliente_aut( num_cliente,num_celular,id_canal,fecha_hora_aut,fecha_hora_consulta,secuencia_mensual ) 
			VALUES(pNumCte,pNumCel,pCanal,CURRENT,NULL,0);
			LET cNumCel = pNumCel;
		END IF;
	ELIF (pProcess = 1) THEN
		...
		ELIF EXISTS(SELECT 1 FROM "informix".br_cliente_aut WHERE num_cliente = pNumCte AND num_celular = pNumCel)THEN
            LET cNumCel = pNumCel;
            UPDATE "informix".br_cliente_aut SET fecha_hora_consulta = CURRENT , secuencia_mensual=pSecuencia WHERE num_cliente = pNumCte AND num_celular = pNumCel;
        ELIF EXISTS(SELECT 1 FROM bdinteg:si_telefonos WHERE numcte = pNumCte and telefono = pNumCel and verificado = 'V' and tipo_tel = 2 and status_tel = 'A')then
            INSERT INTO "informix".br_cliente_aut( num_cliente,num_celular,id_canal,fecha_hora_aut,fecha_hora_consulta,secuencia_mensual )
            VALUES(pNumCte,pNumCel,999,CURRENT,CURRENT,pSecuencia);
            LET cNumCel = pNumCel;
		ELSE
            LET cCodRet = '00003';		
		END IF;
	ELIF (pProcess = 2) THEN
		...
		ELSE
			SELECT  TRIM(NVL(num_celular, '')) INTO cNumCel
			FROM "informix".br_cliente_aut
			WHERE num_cliente = pNumCte
			AND fecha_hora_aut = (SELECT MAX(fecha_hora_aut) FROM "informix".br_cliente_aut WHERE num_cliente = pNumCte);

			IF (cNumCel IS NULL OR cNumCel= '')THEN
				SELECT TRIM(NVL(telefono, '')) INTO cNumCel
				FROM bdinteg:si_telefonos 
				where numcte = pNumCte and verificado = 'V' and tipo_tel = 2 and status_tel = 'A' 
				and fecha_hora = (SELECT MAX(fecha_hora) FROM bdinteg:si_telefonos WHERE numcte = pNumCte and verificado = 'V' and tipo_tel = 2 and status_tel = 'A');
				
				IF (cNumCel IS NULL OR cNumCel= '')THEN 
					LET cCodRet = '00005';	
				ELSE
					INSERT INTO "informix".br_cliente_aut( num_cliente,num_celular,id_canal,fecha_hora_aut,fecha_hora_consulta,secuencia_mensual )
					VALUES(pNumCte,cNumCel,999,CURRENT,NULL,0);
				END IF; 		
			END IF;
		END IF;
	ELSE
...
'DESCRIPCION: Se modifica el SP para agregar la validaciÃ³n del cliente en la tabla bdinteg:informix.si_telefonos_actual para asÃ­ obtener el numero',
'de telefono cuando el cliente no exista en la br_cliente_aut',
'FECHA : 16 - Noviembre - 2023',
'BD    : bdiburo',
'AUTOR : Felipe Antonio Ruiz AcuÃ±a',
'DESCRIPCION: Se agrega inserciÃ³n a la tabla br_cliente_aut cuando se encuentra el registro en la tabla si_telefonos',
'FECHA : 18 - Octubre - 2024',
'BD    : bdiburo';

CREATE PROCEDURE "informix".sp_transpasamovimientosautorizaciongrandata()
RETURNING CHAR(5);
    DEFINE iSqlErr, iIsamErr  INTEGER;
    DEFINE cCodRet            CHAR(5);
    DEFINE cInfoErr           CHAR(100);
    DEFINE dFecha_hoy         DATE;
    DEFINE ndiasResp          INTEGER;
    DEFINE dFecha_mes         DATETIME YEAR TO SECOND;
	DEFINE dFecha_mes_fin	  DATETIME YEAR TO SECOND;
    ...
    SET LOCK MODE TO WAIT 3;
            ...
            BEGIN
                ...
                SELECT CAST(fecha_hoy - ndiasResp AS DATETIME YEAR TO SECOND)
                INTO dFecha_hoy
                FROM bdinteg: si_fechas
                Limit 1;

                Foreach cursor_dias with hold for select distinct(cast(fecha_hora_aut as date)) Into dFecha_mes
                        from bdiburo:"informix".br_cliente_aut where fecha_hora_aut < dFecha_hoy

					LET dFecha_mes_fin = cast((dFecha_mes::date + 1 units day)as DATETIME YEAR TO SECOND) - 1 units second;

                    BEGIN WORK;

                        MERGE INTO bdiburo:"informix".br_cliente_aut_historial j
                        USING ( select num_cliente,num_celular,id_canal,fecha_hora_aut,fecha_hora_consulta,secuencia_mensual
                        from bdiburo:"informix".br_cliente_aut where fecha_hora_aut BETWEEN dFecha_mes and dFecha_mes_fin ) i
                            ON  j.num_cliente  = i.num_cliente
                            AND  j.num_celular  = i.num_celular
                            AND  j.fecha_hora_aut = i.fecha_hora_aut
                        WHEN NOT MATCHED THEN
                            INSERT (j.num_cliente, j.num_celular, j.id_canal, j.fecha_hora_aut, j.fecha_hora_consulta, j.secuencia_mensual)
                            VALUES (i.num_cliente, i.num_celular, i.id_canal, i.fecha_hora_aut, i.fecha_hora_consulta, i.secuencia_mensual);

                        DELETE FROM br_cliente_aut WHERE fecha_hora_aut BETWEEN dFecha_mes and dFecha_mes_fin;
                    COMMIT WORK;
                End Foreach
...
DOCUMENT
'AUTOR : Luis Alberto Beltran Rodriguez',
'DESCRIPCION: Se encarga mover los registros autorizados a la tabla histporica',
'FECHA : Octubre de 2022',
'VERSION: 20221002',
'BD    : bdiburo',
'AUTOR : Gilberto Lopez Inzunza',
'DESCRIPCION: Se agrega variables para fecha inicio y fecha fin que seran utilizadas en el foreach',
'donde posteriormente se procesaran los registros por dia del mes tanto para insercion como borrado',
'FECHA : Enero de 2025',
'VERSION: 20250115',
'BD    : bdiburo',
'==========================================================================';

CREATE PROCEDURE "informix".sp_guarda_autorizacion_grandata_pbajlh(
	pProcess INT, 
	pNumCte CHAR(9), 
	pNumCel CHAR(10), 
	pCanal INT, 
...
'DESCRIPCION: Se modifica el SP para agregar la validaciÃÂ³n del cliente en la tabla bdinteg:informix.si_telefonos_actual para asÃÂ­ obtener el numero',
'de telefono cuando el cliente no exista en la br_cliente_aut',
'FECHA : 16 - Noviembre - 2023',
'BD    : bdiburo',
'AUTOR : Felipe Antonio Ruiz AcuÃÂ±a',
'DESCRIPCION: Se agrega inserciÃÂ³n a la tabla br_cliente_aut cuando se encuentra el registro en la tabla si_telefonos',
'FECHA : 18 - Octubre - 2024',
'BD    : bdiburo';

CREATE PROCEDURE "informix".sp_guarda_autorizacion_grandata(
        pProcess INT,
        pNumCte CHAR(9),
        pNumCel CHAR(10),
        pCanal INT,
