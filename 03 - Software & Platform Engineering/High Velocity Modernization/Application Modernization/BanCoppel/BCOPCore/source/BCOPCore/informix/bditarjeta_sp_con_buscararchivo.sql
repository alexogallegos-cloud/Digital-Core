CREATE PROCEDURE "informix".sp_con_buscararchivo(p_Ruta VARCHAR(100), p_NombreArchivo VARCHAR(50))
RETURNING
	CHAR(5), ---cod_ret
...
BEGIN
	...
	---SET DEBUG FILE TO "/tmp/has/sp_BuscarArchivo";
	---TRACE ON;
	...
	IF EXISTS (SELECT tabname FROM bditarjeta:"informix".systables WHERE tabname = 'con_tmp_busca_archivo') THEN	
		DROP TABLE con_tmp_busca_archivo;
	END IF
	
	--- CREAR LA TABLA DE TRABAJO
	CREATE TABLE  bditarjeta:"informix".con_tmp_busca_archivo
	(linea LVARCHAR(50));	
	...
	LET sCadSql = 'echo "LOAD FROM ' || TRIM(p_Ruta) || 'buscar.bus' || ' INSERT INTO  bditarjeta:con_tmp_busca_archivo" > '|| TRIM(p_Ruta) || 'EjecutaScripts_sp_con_BuscarArchivo.sql';
	SYSTEM sCadSql;	

	--- EJECUTA LAS INSTRUCCIONES QUE ESTAN DENTRO DEL ARCHIVO  *.SQL
	LET sCadSql = 'dbaccess bditarjeta ' || TRIM(p_Ruta) || 'EjecutaScripts_sp_con_BuscarArchivo.sql';
	SYSTEM sCadSql;	
	...
	FOREACH
		SELECT linea
		INTO sLinea
		FROM bditarjeta:"informix".con_tmp_busca_archivo
...
--## Descripcion: Se cambia de la bd bditef a la bditarjeta 29/07/2011
--##############################################################################
END PROCEDURE;