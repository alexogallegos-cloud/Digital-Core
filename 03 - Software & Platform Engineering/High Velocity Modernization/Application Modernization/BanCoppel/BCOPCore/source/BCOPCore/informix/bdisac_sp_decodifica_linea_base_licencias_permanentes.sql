CREATE PROCEDURE "informix".sp_decodifica_linea_base_licencias_permanentes(pCaptura CHAR(20), pImporte CHAR(20), pLlaveGDF INTEGER)
...
-- ELABORO: 	ING VICTOR HERNANDEZ
-- FECHA:		23-10-2024
-- PROYECTO: 	PAGOS GDF LICENCIAS PERMANENTES
-- DESCRIPCION:	DECODIFICA LA LINEA BASE CLAVE 03
...
--SET DEBUG FILE TO '/home/informix/VIMA/sp_decodifica_linea_base_licencias_permanentes.out';
--TRACE ON;
...
BEGIN
	ON EXCEPTION
		...
		IF iSqlErr <> 0 THEN
			LET  cCodRet = iSqlErr;
			RETURN cCodRet, cTipoLicencia, cPeriodo, cTipoReferencia, cDescripcionConcepto, cTramite;
		END IF;
	END EXCEPTION;
	...
	ELSE
		...
		--SE VALIDA QUE LA LICENCIA SEA DE TIPO A
		
		IF (pCaptura[3,3] == 'A') THEN
		...
		IF (pCaptura[4,4] == 'E') THEN
		 LET cDescripcionConcepto = 'Licencia A Automovilista Permanente Nueva (ExpediciÃ³n y ReposiciÃ³n)';
		 LET cTramite = 'ExpediciÃ³n';
		
		ELIF (pCaptura[4,4] == 'R') THEN
		 LET cDescripcionConcepto = 'Licencia A Automovilista y Motociclista Permanente (ReposiciÃ³n)';
		 LET cTramite = 'ReposiciÃ³n';
		END IF;

	
	    -- OBTIENE LA REFERENCIA DE LA POSICION 5 A LA 13 OMITIENDO LA LETRA X Y LOS NUMEROS CEROS A LA IZQUIERDA
		
		LET cTipoReferencia = pCaptura[5,13]; --OBTIENE LA REFERENCIA
		      
			  --SE DEBEN DESCARTAR LAS LETRAS X Y CEROS
			
			    LET diferente = 1;
			  --ENCUENTRA LA POSICION DE LA PRIMER LETRA DIFERENTE 
                 WHILE SUBSTRING(cTipoReferencia FROM diferente FOR 1) = 'X' OR SUBSTRING(cTipoReferencia FROM diferente FOR 1) = '0'
                 LET diferente = diferente + 1;
                 END WHILE;
			  --OBTIENE LA REFERECIA A PARTIR DE ESA POSICION
			      LET cTipoReferencia = SUBSTRING(cTipoReferencia FROM diferente);
	

		--OBTIENE EL PERIODO DEL CONCEPTO DE LA CLAVE 03
	     SELECT periodo
	     INTO cPeriodo
	     FROM bdisac:"informix".sac_periodoslicenciagdf
	     WHERE clave = pCaptura[1,2];
...
DOCUMENT
'DESCRIPCION: DECODIFICA LA LINEA BASE CLAVE 03.',
'AUTOR : Ing. Victor Manuel Hernandez L',
'FECHA : 23-10-2024',
'BD: bdisac',
'Modificacion: Se crea SP para decodificar codigo 03 de licencias permanentes, con subclaves AE y AR',
'Sustento: Licencias permanentes de GDF';
