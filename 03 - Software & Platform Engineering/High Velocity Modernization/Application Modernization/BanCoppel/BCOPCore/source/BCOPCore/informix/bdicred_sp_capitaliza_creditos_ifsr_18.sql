CREATE PROCEDURE "informix".sp_capitaliza_creditos_ifsr_18()
    RETURNING CHAR(5);
...
FOREACH WITH HOLD
	...
	select num_credito,monto,folio_suc,sucursal,plaza,num_producto
	INTO vNumCred, vMonto, Folio, vSucursal,vPlaza,NumProducto
	from bdicred:sd_movhis
	where num_credito matches '81*'
	and codigo_fun = '606'
	and codigo_ref = 7018
	and reversado = 'N'
	and fecha_mov = mdy('01','18','2022')
	and num_credito not in
	(select num_credito from bdicred:sd_movhis
	where num_credito matches '81*'
	and codigo_fun = '605'
	and codigo_ref = 125
	and reversado = 'N'
	...
	BEGIN WORK;
		-- interes
		CALL genmovcierre_movdia('001', vNumCred, NumProducto,125, "605", mdy(01,18,2022),  vMonto, Folio, vSucursal, '01', '',vPlaza) RETURNING CodRet, Mensaje;
		...
		update bdicred:sd_maesdoshist set sdo_capital = sdo_capital + vMonto,
							sdo_cap_insoluto = sdo_cap_insoluto + vMonto,
							mto_capitalizado = mto_capitalizado + vMonto					
							where empresa = '001' and fecha = mdy(01,18,2022) and num_credito = vNumCred;
							
		--actualiza saldos diarios
		UPDATE bdicred:sd_sdodiario set capvig18 = capvig18 + vMonto, 
				capvig19 = capvig19 + vMonto, 
				capvig20 = capvig20 + vMonto
...
FOREACH WITH HOLD
	...
	BEGIN WORK;
		-- iva
		CALL genmovcierre_movdia('001', vNumCred, NumProducto,126, "605", mdy(01,18,2022), vMonto, Folio, vSucursal, '01', '',vPlaza) RETURNING CodRet, Mensaje;
