create procedure "informix".gencomanv(pempresa char(3),
                                      pcuenta char(20))
       returning char(5);

define vsqlerr integer;
define vcodret char(5);
define vprox_fecha,vfecha_hoy date;
...
begin
   ...
   --SET DEBUG FILE TO "/tmp/gencomanv.out";
   --TRACE ON;
   ...
   select {+INDEX(sc_fechas idx_fechas1)} fecha_hoy,prox_fecha
   ...
   foreach
      select {+INDEX(sc_prodcomis idx_prodcomis), +INDEX(sc_comisiones idx_comision1)} pc.comision,forma_aplica,monto_aplica,factor_aplica,rangos,
         ...
         where co.empresa = pempresa and forma_cargo = "02" and 
      ...
      foreach
         select mc.cuenta,mn.fecha_alta into vcuenta,vfecha_alta
            from sc_maechq mc, sc_maenoc mn
            where mc.empresa = pempresa and mc.cuenta = pcuenta and 
                  mc.producto = vproducto and 
                  mc.empresa = mn.empresa and mc.cuenta = mn.cuenta
         if month(vfecha_alta) = month(vfecha_hoy) and
            year(vfecha_alta) <> year(vfecha_hoy)  then
            ...
            if vcalcula_com = "1" then
               ...
               if vmonto_com > 0 then
                  insert into sc_detcomis
                     values(pempresa,vcuenta,vcomision,vmonto_com,0,
                            vfecha_hoy,"","P"," ");
                  update sc_maechq
