CREATE PROCEDURE "informix".sp_mon_buro_reenviosrep_pba(pModo SMALLINT, pTipoSolicitud CHAR(1), pNumSolicitud CHAR(20), pNumCte CHAR(20),pEstatus CHAR(2), pFechaIni DATE, pFechaFin DATE)
	...
	--TRACE ON;
	 ...
	 IF pModo = 1 OR pModo = 2 THEN ---1 REENVIO DE SOLIC. O 2 REENVIO EXITOSO
		...
		FOREACH WITH HOLD
				...
				aut.status_solicitud ELSE auto1.status END) NOT IN ("BC","CC") OR (rep.reenvio_exit='0' AND rep.producto='6500' AND rep.estatus_fin NOT IN ('','BC')) THEN "SI" ELSE "NO" END) AS reenvio_exitoso, 
				rep.fecha_reenvio,rep.estatus_fin AS estatus_reenvio,(CASE WHEN rep.tipo_sol=1 THEN aut.status_solicitud ELSE auto1.status END)AS estatus_fin ,rep.cve_grupo, rep.cve_segmento, rep.cve_etiqueta, 
				...
				LEFT JOIN bdicred:"informix".sd_autorizacion_aumlincred auto1 ON (rep.numsolicitud=auto1.num_solicitud		
							AND bta.fecha_insert BETWEEN rep.fecha_sol AND auto1.fecha_insert
							AND auto1.status NOT IN ('PC','BC','CC') 
							AND auto1.status IN (SELECT MIN(status) 
												FROM bdicred:"informix".sd_autorizacion_aumlincred 
												WHERE empresa = "001"
												AND status IN ('AC','AT','RT')
												AND fecha_insert = auto1.fecha_insert
												AND num_solicitud=auto1.num_solicitud))
