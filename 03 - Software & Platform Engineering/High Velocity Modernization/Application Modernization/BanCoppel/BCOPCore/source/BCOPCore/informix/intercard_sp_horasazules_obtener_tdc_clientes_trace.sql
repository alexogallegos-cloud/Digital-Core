CREATE PROCEDURE "informix".sp_horasazules_obtener_tdc_clientes_trace()
    ...
    BEGIN 
        ...
        --Validar si la campaña está habilitada.
        SELECT valores1 
        ...
        --Validar el periodo definido para la Campaña de Horas Azules
		...
		------------------------------------------------------------------------------------------
				...
				Select  EXTEND(valores2, YEAR TO MINUTE) - 5 UNITS MINUTE   INTO vFechaInicial from intercard:tbl_inter_params_cmp_horas_azules   
                WHERE empresa = '001' AND  cond_busqueda = 'cmp_azul_hr_pvt_inicial';

	 	        SELECT                 
                    DBINFO('utc_to_datetime', Sh_Curtime)::DATETIME YEAR TO SECOND as tiempo_final 			
			        INTO 
					vFechaFinaltoday
                FROM sysmaster:"informix".sysshmvals;
        ...
        ------------------------------------------------------------------------------------------
         ...
         FOREACH curCreditosActivos WITH HOLD FOR 
            ...
            WHERE c.fecha_activa BETWEEN vFechaInicial AND vFechaFinal   
                AND   a.num_producto = '6001'  
				AND tipo_asignacion = 'A'     
            AND c.empresa = '001' 
			----------
		      LET vExisteRegistro = 0; 
		    
		    -- Valida que no exista un producto 6001 anterior 
			 Select count(*) INTO vExisteRegistro 
			  from bdicred:sd_maecred where  numcte = vNumeroCliente
			  AND num_producto = '6001'  AND empresa = '001'; 
        ...
        FOREACH curCreditosOro WITH HOLD FOR 
            ...
            WHERE c.fecha_activa BETWEEN vFechaInicial AND vFechaFinal
				...
				--Valida que no exista un producto 8100 anterior 
				  Select count(*) INTO vExisteRegistro 
			  from bdicred:sd_tarjeta where  numcte = vNumeroCliente
			  AND prodtarjeta = '8100' AND status_tar <> 'A' ; 
        ...
        FOREACH curBuscarInfoCte WITH HOLD FOR
            ...
            INSERT INTO intercard:"informix".tbl_cmp_horas_azules_bitacora (
                ...
                VALUES (vFechaAsignacionTarjeta, vFechaActivacionTarjeta, vNumCredito, vNumeroCliente, vNumProducto, vNumTarjeta,vNumTelefono, LOWER(vCorreoElect),
                    vPrimerNombre, vSegundoNombre, vApellidoPaterno, vApellidoMaterno,vFechaEjecucion);
        END FOREACH             
        ...
        WHERE empresa = '001' AND  cond_busqueda = 'cmp_azul_hr_pvt_final';

	    --Finalizacion del proceso de campaña.
...
'Implementación para campaña de horas azules en productos de tarjeta de credito.'
;

CREATE PROCEDURE "informix".sp_validar_cliente_bancoppel( pNumClienteCpl VARCHAR(20) )
    RETURNING VARCHAR (5) as rCodigoRetorno, VARCHAR(120) as rMensajeRespuesta, VARCHAR(20) as rNumClienteBcpl;
    ...
    DEFINE RUTA_ORIGEN VARCHAR(50);
    DEFINE vNumClienteBcpl VARCHAR(20);
    ...
    LET vMENSAJE_RETORNO = 'Cliente Coppel Asociado.';
    LET vNumClienteBcpl = '';
    
    
    ---SET DEBUG FILE TO RUTA_ORIGEN || "debug_sp_validar_cliente_bancoppel.out";
    ---TRACE ON;
    ...
    BEGIN 
        ...
        ON EXCEPTION SET SQLERR, ISAM_ERR, ERROR_INFO
            ...
            SET DEBUG FILE TO RUTA_ORIGEN || "excep_sp_validar_cliente_bancoppel.err.out" WITH APPEND;
            TRACE ON;
            
            IF ( SQLERR <> 0 ) THEN
                LET vCODIGO_RETORNO = SQLERR;
                LET vMENSAJE_RETORNO = vMENSAJE_RETORNO;
                RETURN vCODIGO_RETORNO, vMENSAJE_RETORNO, vNumClienteBcpl;
            END IF;
        ...
        IF ( pNumClienteCpl IS NULL OR LENGTH(pNumClienteCpl) <= 0 ) THEN
            LET vCODIGO_RETORNO = '00002';
            LET vMENSAJE_RETORNO = 'Falta parametro de Cliente Coppel.';
            RETURN vCODIGO_RETORNO, vMENSAJE_RETORNO, vNumClienteBcpl;
        END IF 
        ...
        LIMIT 1 numcte
            INTO vNumClienteBcpl
        FROM bdinteg:si_cliente 
            WHERE empresa = '001'
        AND numcte_ref = pNumClienteCpl;
        
        IF ( vNumClienteBcpl IS NULL OR LENGTH(vNumClienteBcpl) <= 0 ) THEN
            LET vCODIGO_RETORNO = '00001';
            LET vMENSAJE_RETORNO = 'Num. Cliente Coppel sin registro.';
        END IF
...
'Implementación para obtener el número de Cliente Bancoppel'
;

CREATE PROCEDURE "informix".sp_validar_cliente_coppel( pNumClienteBanCoppel VARCHAR(20) )
    RETURNING VARCHAR (5) as rCodigoRetorno, VARCHAR(120) as rMensajeRespuesta, VARCHAR(20) as rNumClienteCoppel;
    ...
    DEFINE vMENSAJE_RETORNO VARCHAR(120);
    DEFINE RUTA_ORIGEN VARCHAR(50);
    DEFINE vNumClienteCoppel VARCHAR(20);
    ...
    LET RUTA_ORIGEN = '/RESPALDOSNEW/';
    LET vCODIGO_RETORNO = '00000';
    LET vMENSAJE_RETORNO = 'Cliente Bancoppel Asociado.';
    LET vNumClienteCoppel = '';
    
    
    ---SET DEBUG FILE TO RUTA_ORIGEN || "debug_sp_validar_cliente_coppel.out";
    ---TRACE ON;
    ...
    BEGIN 
        ...
        ON EXCEPTION SET SQLERR, ISAM_ERR, ERROR_INFO
            ...
            SET DEBUG FILE TO RUTA_ORIGEN || "excep_sp_validar_cliente_coppel.err.out" WITH APPEND;
            TRACE ON;
            
            IF ( SQLERR <> 0 ) THEN
                LET vCODIGO_RETORNO = SQLERR;
                LET vMENSAJE_RETORNO = vMENSAJE_RETORNO;
                RETURN vCODIGO_RETORNO, vMENSAJE_RETORNO, vNumClienteCoppel;
            END IF;
        ...
        IF ( pNumClienteBanCoppel IS NULL OR LENGTH(pNumClienteBanCoppel) <= 0 ) THEN
            LET vCODIGO_RETORNO = '00002';
            LET vMENSAJE_RETORNO = 'Falta parametro de Cliente Bancoppel.';
            RETURN vCODIGO_RETORNO, vMENSAJE_RETORNO, vNumClienteCoppel;
        END IF 
        
        SELECT
            LIMIT 1 numcte_ref
                INTO vNumClienteCoppel
        FROM bdinteg:si_cliente 
            WHERE empresa = '001'
        AND numcte = pNumClienteBanCoppel;
        
        IF ( vNumClienteCoppel IS NULL OR LENGTH(vNumClienteCoppel) <= 0 ) THEN
            LET vCODIGO_RETORNO = '00001';
            LET vMENSAJE_RETORNO = 'Num. Cliente Bancoppel sin registro.';
        END IF
...
CREATE PROCEDURE "informix".sp_generartarjetas_traceon(v_indicadortipoproceso varchar(1))
...
/*20180125.JHAP.End*/
	--set debug file to "/RESPALDOSNEW/sp_generartarjetas.out";
	--trace on;
...
let v_ultimatarjetalote=0;
		 ...
		 -- set debug file to '/RESPALDOSNEW/sp_generartarjetas_traceOn_a.out';
		 --trace on;
...
begin
  ...
  on exception set  sql_err  , isam_err, error_info
		...
		set debug file to '/RESPALDOSNEW/sp_generartarjetas_traceOn_r.out';
	    trace on;
        ROLLBACK WORK;
  ...
  BEGIN WORK;
	...
	/* 20170626.FB.Begin */  
	    ...
	    SELECT  {+AVOID_FULL(solicitud_maquila)} distinct clave_sucursal INTO v_sucursal FROM "informix".solicitud_maquila order by clave_sucursal
     /* 20170626.FB.End */ 			
            foreach
                    ...
                    /*ORDER BY sm.clave_tipotarjeta , sm.tipomaquila ASC*//*20190716*/
	       ...
	       /*SELECT 'Tipo Envio: '  || v_tipoenvio INTO v_mensajeerr FROM parametros;*/
			 ...
			 /*JDSO-CuentaN2-20220103 End*/
                    ...
                    /*20160906.JHAP.Begin*/		
					IF v_tipoenvio <> 'D' AND v_esVirtual <> 'V' THEN  /*JDSO-CuentaN2-20220103 Begin Se agrego al if ---AND v_esVirtual <> 'V' ---*/
					/*20160906.JHAP.End*/
					...
					SELECT provedormaquila INTO v_IdProveedor FROM "informix".tipotarjeta WHERE clave_tipotarjeta = v_clavetipotarjeta;
					...
					/*Entra a Maquilas SIN Enbozar*/
						...
						*/
						...
						/*SELECT numerolote, cantidadtarjetassol INTO v_loteactual , v_solicitadaslote FROM "informix".lote WHERE clave_tipotarjeta = v_clavetipotarjeta AND clave_sucursal = v_sucursal AND tipoenvio = v_tipoenvio AND fechageneracion >= TODAY;*//*20190716*/
						--SELECT  max(numerolote), cantidadtarjetassol INTO v_loteactual , v_solicitadaslote FROM "informix".lote WHERE clave_tipotarjeta = v_clavetipotarjeta AND clave_sucursal = v_sucursal and tipomaquila = v_tipo AND tipoenvio = v_tipoenvio AND idProveedor = v_IdProveedor AND fechageneracion >= TODAY  group by numerolote;
                    ...
                    INTO v_soportacajeropropio, v_soportacajeroconvenio, v_soportacajerored, v_soportainternacional FROM "informix".productotarjeta
                    ...
                    IF v_tipoenvio <> 'D' THEN
                        ...
                        SELECT direccion_sucursal INTO v_direccionsucursal FROM "informix".sucursal where clave_sucursal= v_sucursal;
                    ELSE
                        /*se obtiene el domicilio del cliente*/
                        SELECT TRIM(direccion_calle1) || ', ' || TRIM(direccion_calle2) || ', ' || TRIM(direccion_colonia) || ', ' || TRIM(direccion_municipio) || ', ' || TRIM(direccion_estado) || ', ' || TRIM(direccion_cp) INTO v_direccionsucursal FROM "informix".solicitudtarjeta WHERE idsolicitud= v_idsolicitud;
                    END IF;
                                ...
                                IF v_tipo = 'E' AND v_idsolicitud > 0 THEN
                                        ...
                                        ELSE
                                             ...
                                             IF v_tipoenvio = 'D' THEN
                                                ...
                                                /*SELECT folioasignacionactivacion INTO v_numfolioasignacion FROM "informix".paraminventarios;*/
...
---------------------------------
                                ...
                                /*Se inserta la informaciÃÂ???ÃÂ??ÃÂ?ÃÂÃÂ³ecesaria en la tabla detalle_maquila para la generacion de los archivos para gemalto*/
    ...
    UPDATE "informix".paraminventarios SET consecutivoarchivomaquila=consecutivoarchivomaquila+v_contadorfinal;
...
--    set debug file to '/RESPALDOSNEW/sp_generatarjetasxmaquilar.out';
--    trace on;
