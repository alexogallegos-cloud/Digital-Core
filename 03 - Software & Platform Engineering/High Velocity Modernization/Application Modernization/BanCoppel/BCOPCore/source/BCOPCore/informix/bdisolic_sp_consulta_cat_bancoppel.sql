CREATE PROCEDURE "informix".sp_consulta_cat_bancoppel(pSolicitud CHAR(20), pCliente CHAR(20), 	
	...
	--Control de errores
		...
		ELSE 		
			FOREACH 
				...
				--SELECT {+index "informix".ss_solicitudes idx_numctempresa} TRIM(sol.num_solicitud) NUMSOL, 
				SELECT {+  AVOID_FULL("informix".ss_solicitudes) AVOID_FULL(bdinteg:"informix".si_direcciones_actual) INDEX(bdinteg:"informix".si_estados inx_estado) } TRIM(sol.num_solicitud) NUMSOL, 
				TRIM(dir.numcte), 
				TRIM(sol.status_solicitud), 
				dir.numerociudad, 
				dir.numerocolonia,
