CREATE PROCEDURE "informix".sp_repipab_direc_pte3( pFechaIni DATE, pFechaFin DATE ) 
RETURNING CHAR(5), CHAR(40), CHAR(40), CHAR(40);
    ...
    --- SET DEBUG FILE TO '/resplogifx/conciliachq/sp_repipab_direc_pte3.out';
    --- TRACE ON;
    
    BEGIN

    ON EXCEPTION SET SQL_ERR, ISAM_ERR, DESC_ERR
        SET DEBUG FILE TO '/resplogifx/conciliachq/sp_repipab_direc_pte3.err';
        TRACE ON;
    ...
    IF EXISTS (SELECT tabname FROM sysmaster:systabnames WHERE partnum > 0 AND tabname = 'si_infpertit_pte3') THEN
        DROP TABLE si_infpertit_pte3;
    END IF;
    
    CREATE TABLE si_infpertit_pte3 
      (
        cve_unica       CHAR(10),   
        persona         CHAR(1),    
        nombre          CHAR(100),  
    ...
    /* ###############################################################################################################
    IF EXISTS (SELECT tabname FROM sysmaster:systabnames WHERE partnum > 0 AND tabname = 'si_infpattit_pte3') THEN
        DROP TABLE si_infpattit_pte3;
    END IF;
    
    CREATE TABLE si_infpattit_pte3 
      (
        numcta		    CHAR(20),       
    ...
    /* ###############################################################################################################
    IF EXISTS (SELECT tabname FROM sysmaster:systabnames WHERE partnum > 0 AND tabname = 'si_ctaasotit_pte3') THEN
        DROP TABLE si_ctaasotit_pte3;
    END IF;
    
    CREATE TABLE si_ctaasotit_pte3
      (
        numcta          CHAR(20),    
        num_inversion   CHAR(20),     
        cve_unica       CHAR(10),     
    ...
    CREATE INDEX idx_ipab_ctaaso ON si_ctaasotit_pte3 (cve_unica) USING BTREE;
    UPDATE STATISTICS MEDIUM FOR TABLE si_ctaasotit_pte3;
    ############################################################################################################### */
    ...
    FOREACH WITH HOLD
        ...
        FOREACH
            ...
            INSERT INTO si_infpattit_pte3 VALUES
            ...
            INSERT INTO si_ctaasotit_pte3 VALUES 
        ...
        IF vcve_unica > 0 THEN
            INSERT INTO si_infpertit_pte3 VALUES
    ...
    CREATE INDEX idx_infpertit3_cve ON si_infpertit_pte3(cve_unica) USING BTREE FILLFACTOR 99;
    UPDATE STATISTICS MEDIUM FOR TABLE si_infpertit_pte3;
    
    -- // DESCARGA DE ARCHIVOS
    LET vsql = 'echo "UNLOAD TO /resplogifx/conciliachq/infpertit_3.txt '||
               'SELECT * FROM si_infpertit_pte3;" > /resplogifx/conciliachq/desc_ipab_31.sql';
    SYSTEM vsql;
    
    LET vstmt = 'dbaccess bdinteg /resplogifx/conciliachq/desc_ipab_31.sql';
    SYSTEM vstmt;
    
    /* ###############################################################################################################################################################
    LET vsql = 'echo "UNLOAD TO /resplogifx/conciliachq/infpattit_3.txt '||
               'SELECT * FROM si_infpattit_pte3;" > /resplogifx/conciliachq/desc_ipab_32.sql';
    SYSTEM vsql;
    
    LET vstmt = 'dbaccess bdinteg /resplogifx/conciliachq/desc_ipab_32.sql';
    SYSTEM vstmt;
    
    LET vsql = 'echo "UNLOAD TO /resplogifx/conciliachq/ctaasotit_3.txt '||
               'SELECT * FROM si_ctaasotit_pte3;" > /resplogifx/conciliachq/desc_ipab_33.sql';
    SYSTEM vsql;
    
    LET vstmt = 'dbaccess bdinteg /resplogifx/conciliachq/desc_ipab_33.sql';
    SYSTEM vstmt;
    ############################################################################################################################################################### */
