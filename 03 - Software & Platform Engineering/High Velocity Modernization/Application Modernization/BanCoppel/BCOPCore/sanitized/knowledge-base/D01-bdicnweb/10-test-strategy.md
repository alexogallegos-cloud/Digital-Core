...
Demostrar que el `ChannelOrchestrationService (API Gateway + BFF layer)` target produce **resultados idénticos** al SP Informix original bajo todas las condiciones relevantes de producción. Sin esta equivalencia demostrada, el cutover no puede proceder.

## Prioridad — D01 es ÚLTIMO

**Migración final** — todos los dominios ya están migrados cuando llega el turno de este. Las pruebas son de integración end-to-end.

## TS-D01-01 · Golden Master del SP más crítico: `sp_actualizamonitorprocesos`

Este SP tiene 48 callers y es el punto de entrada más crítico del dominio.
...
FROM bdicnweb:sp_audit_log           -- [SME-PENDING: confirmar tabla de audit]
WHERE fecha_insert >= TODAY - 7
  AND proc_name = 'sp_actualizamonitorprocesos'
ORDER BY exec_ts;
```
...
## TS-D01-02 · Pruebas de procesos batch (ver 11-batch-processes.md)
...
## TS-D01-03 · Pruebas de integración cross-domain

Los SPs de `bdicnweb` hacen llamadas cross-DB a otros dominios. En el target, estas se convierten en llamadas API.

| SP | Dependencia cross-DB | Prueba requerida |
|----|---------------------|-----------------|
| `sp_abm_canal_cobro` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_abm_canal_cobro` | `bdisuc` | Simular respuesta API de `bdisuc` con mock — verificar contrato |
| `sp_abm_canal_cobro` | `bdicnweb` | Simular respuesta API de `bdicnweb` con mock — verificar contrato |
| `sp_abm_canal_cobro` | `bdisac` | Simular respuesta API de `bdisac` con mock — verificar contrato |
| `sp_abm_canal_cobro` | `bdilide` | Simular respuesta API de `bdilide` con mock — verificar contrato |
| `sp_abm_canal_cobro` | `bdirst` | Simular respuesta API de `bdirst` con mock — verificar contrato |
| `sp_abono_ref` | `bdicheq` | Simular respuesta API de `bdicheq` con mock — verificar contrato |
| `sp_abono_ref` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_abono_ref_masivo` | `bdicheq` | Simular respuesta API de `bdicheq` con mock — verificar contrato |
| `sp_abono_ref_masivo` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_abono_ref_masivo` | `bdilide` | Simular respuesta API de `bdilide` con mock — verificar contrato |
| `sp_abono_ref_masivo` | `bdicnweb` | Simular respuesta API de `bdicnweb` con mock — verificar contrato |
| `sp_abonotransfersoc` | `bdicheq` | Simular respuesta API de `bdicheq` con mock — verificar contrato |
| `sp_ac_actualizactas` | `bdicheq` | Simular respuesta API de `bdicheq` con mock — verificar contrato |
| `sp_ac_actualizactas` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_ac_actualizactas` | `bdisuc` | Simular respuesta API de `bdisuc` con mock — verificar contrato |
| `sp_ac_actualizactas` | `bdicnweb` | Simular respuesta API de `bdicnweb` con mock — verificar contrato |
| `sp_ac_actualizactas` | `bdicred` | Simular respuesta API de `bdicred` con mock — verificar contrato |
| `sp_ac_actualizactas` | `intercard` | Simular respuesta API de `intercard` con mock — verificar contrato |
| `sp_ac_actualizactas` | `bdiburo` | Simular respuesta API de `bdiburo` con mock — verificar contrato |
| `sp_ac_actualizactas` | `bdisolic` | Simular respuesta API de `bdisolic` con mock — verificar contrato |
...
`bdicnweb` contiene operaciones con `MONEY` e `INTEGER`. En el target PostgreSQL:
...
---
*Generado por: Specialist — Informix SPL Analysis · 2026-07-03 · Evidencia: source/LegacyCore/informix/bdicnweb_*.sql (análisis estático de 57 archivos SQL) · Specialist — Informix SPL Analysis + QA Lead Equivalencia*
