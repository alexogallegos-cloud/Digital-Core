...
Demostrar que el `BalanceService (Read-Heavy Query Engine)` target produce **resultados idénticos** al SP Informix original bajo todas las condiciones relevantes de producción. Sin esta equivalencia demostrada, el cutover no puede proceder.

## Prioridad — D05 es Wave 3

**Wave 3** — prioridad media-alta. Los dominios de Wave 4+ dependen de la estabilidad de este dominio.

## TS-D05-01 · Golden Master del SP más crítico: `sp_app_valdigito`

Este SP tiene 157 callers y es el punto de entrada más crítico del dominio.
...
FROM bdisac:sp_audit_log           -- [SME-PENDING: confirmar tabla de audit]
WHERE fecha_insert >= TODAY - 7
  AND proc_name = 'sp_app_valdigito'
ORDER BY exec_ts;
```
...
| TC-09 | Volumen de carga de producción | Carga real × 157 req/seg | Latencia dentro de SLA target |
...
## TS-D05-02 · Pruebas de procesos batch (ver 11-batch-processes.md)
...
## TS-D05-03 · Pruebas de integración cross-domain

Los SPs de `bdisac` hacen llamadas cross-DB a otros dominios. En el target, estas se convierten en llamadas API.

| SP | Dependencia cross-DB | Prueba requerida |
|----|---------------------|-----------------|
| `sp_act_ine_bdrem` | `bdisac` | Simular respuesta API de `bdisac` con mock — verificar contrato |
| `sp_act_ine_bdrem` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_actualiza_cte_remesa` | `bdisac` | Simular respuesta API de `bdisac` con mock — verificar contrato |
| `sp_actualiza_cte_remesa` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_actualiza_datos` | `bdisac` | Simular respuesta API de `bdisac` con mock — verificar contrato |
| `sp_actualiza_datos` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_actualiza_datos` | `bdicred` | Simular respuesta API de `bdicred` con mock — verificar contrato |
| `sp_actualiza_datos` | `sysmaster` | Simular respuesta API de `sysmaster` con mock — verificar contrato |
| `sp_actualiza_estatus_cardif` | `bdisac` | Simular respuesta API de `bdisac` con mock — verificar contrato |
| `sp_actualiza_sac_bts_sdep` | `bdisac` | Simular respuesta API de `bdisac` con mock — verificar contrato |
| `sp_actualiza_sac_bts_sdep` | `bdicred` | Simular respuesta API de `bdicred` con mock — verificar contrato |
| `sp_actualiza_sac_bts_sdep` | `bdicheq` | Simular respuesta API de `bdicheq` con mock — verificar contrato |
...
`bdisac` contiene operaciones con `MONEY` e `INTEGER`. En el target PostgreSQL:
...
---
*Generado por: Specialist — Informix SPL Analysis · 2026-07-03 · Evidencia: source/LegacyCore/informix/bdisac_*.sql (análisis estático de 58 archivos SQL) · Specialist — Informix SPL Analysis + QA Lead Equivalencia*
