# D06 · Solicitudes — Estrategia de Pruebas de Equivalencia (Golden Master)
...
Demostrar que el `SolicitudService (Loan Origination System)` target produce **resultados idénticos** al SP Informix original bajo todas las condiciones relevantes de producción. Sin esta equivalencia demostrada, el cutover no puede proceder.

## Prioridad — D06 es Wave 3

**Wave 3** — prioridad media-alta. Los dominios de Wave 4+ dependen de la estabilidad de este dominio.

## TS-D06-01 · Golden Master del SP más crítico: `sp_asigna_solicitud_soc`

Este SP tiene 236 callers y es el punto de entrada más crítico del dominio.
...
FROM bdisolic:sp_audit_log           -- [SME-PENDING: confirmar tabla de audit]
WHERE fecha_insert >= TODAY - 7
  AND proc_name = 'sp_asigna_solicitud_soc'
ORDER BY exec_ts;
```
...
| TC-09 | Volumen de carga de producción | Carga real × 236 req/seg | Latencia dentro de SLA target |
...
## TS-D06-02 · Pruebas de procesos batch (ver 11-batch-processes.md)
...
## TS-D06-03 · Pruebas de integración cross-domain

Los SPs de `bdisolic` hacen llamadas cross-DB a otros dominios. En el target, estas se convierten en llamadas API.

| SP | Dependencia cross-DB | Prueba requerida |
|----|---------------------|-----------------|
| `sp_actualiza_estatus_sol_canal` | `bdisolic` | Simular respuesta API de `bdisolic` con mock — verificar contrato |
| `sp_actualiza_info_cac` | `bdisolic` | Simular respuesta API de `bdisolic` con mock — verificar contrato |
| `sp_actualiza_monto_lineas` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_actualiza_monto_lineas` | `bdicred` | Simular respuesta API de `bdicred` con mock — verificar contrato |
| `sp_actualiza_monto_lineas` | `bdisolic` | Simular respuesta API de `bdisolic` con mock — verificar contrato |
| `sp_actualiza_respuestagrupo_cteprosp` | `bdiprospectos` | Simular respuesta API de `bdiprospectos` con mock — verificar contrato |
| `sp_actualiza_respuestagrupo_cteprosp` | `bdisolic` | Simular respuesta API de `bdisolic` con mock — verificar contrato |
| `sp_actualiza_resumscorfin` | `bdisolic` | Simular respuesta API de `bdisolic` con mock — verificar contrato |
...
`bdisolic` contiene operaciones con `MONEY` e `INTEGER`. En el target PostgreSQL:
...
---
*Generado por: Specialist — Informix SPL Analysis · 2026-07-03 · Evidencia: source/LegacyCore/informix/bdisolic_*.sql (análisis estático de 70 archivos SQL) · Specialist — Informix SPL Analysis + QA Lead Equivalencia*
