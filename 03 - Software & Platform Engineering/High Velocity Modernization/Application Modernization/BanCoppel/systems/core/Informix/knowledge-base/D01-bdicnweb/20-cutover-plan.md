...
`Canal Digital Web` no puede cortar hasta que estos dominios estén disponibles como API:

| ID | Dominio | Llamadas | Estado |
|----|---------|---------|--------|
| D02 | Integración y Autenticación | 15,304 | API disponible como prereq |
| D03 | Créditos | 10,903 | API disponible como prereq |
| D10 | Sucursales | 6,094 | API disponible como prereq |
| D05 | Saldos y Cuentas | 3,464 | API disponible como prereq |

## Dominios bloqueados por `bdicnweb`

Estos dominios no pueden migrar hasta que `bdicnweb` complete el cutover:

| ID | Dominio | Llamadas | Impacto |
|----|---------|---------|---------|
| D12 | Contabilidad | 639 | No puede migrar hasta que `bdicnweb` esté listo |
| D02 | Integración y Autenticación | 35 | No puede migrar hasta que `bdicnweb` esté listo |
| D04 | Cheques / Cuentas | 31 | No puede migrar hasta que `bdicnweb` esté listo |
| D05 | Saldos y Cuentas | 29 | No puede migrar hasta que `bdicnweb` esté listo |
...
T=0 (22:00): INICIO VENTANA DE CUTOVER
  ...
  22:05  → Freeze de escrituras en Informix bdicnweb
