...
`Cobranza` no puede cortar hasta que estos dominios estén disponibles como API:

| ID | Dominio | Llamadas | Estado |
|----|---------|---------|--------|
| D03 | Créditos | 74 | API disponible como prereq |
| D02 | Integración y Autenticación | 57 | API disponible como prereq |
| D09 | Mensajería | 53 | API disponible como prereq |

## Dominios bloqueados por `bdicobranza`

Estos dominios no pueden migrar hasta que `bdicobranza` complete el cutover:

| ID | Dominio | Llamadas | Impacto |
|----|---------|---------|---------|
| D01 | Canal Digital Web | 177 | No puede migrar hasta que `bdicobranza` esté listo |
| D03 | Créditos | 152 | No puede migrar hasta que `bdicobranza` esté listo |
| D02 | Integración y Autenticación | 23 | No puede migrar hasta que `bdicobranza` esté listo |
| D06 | Solicitudes | 11 | No puede migrar hasta que `bdicobranza` esté listo |
...
T=0 (22:00): INICIO VENTANA DE CUTOVER
  ...
  22:05  → Freeze de escrituras en Informix bdicobranza
