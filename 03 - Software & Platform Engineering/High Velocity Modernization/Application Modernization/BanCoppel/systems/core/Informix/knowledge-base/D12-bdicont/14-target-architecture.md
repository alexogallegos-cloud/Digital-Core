...
El dominio `bdicont` es de tipo **accounting**. Su migración sigue este patrón porque Registro contable y conciliación. 168 SPs.

## Topología AWS target

```
┌────────────────────────────────────────────────────────────┐
│  D12 · Contabilidad — AWS Target                 │
│                                                             │
│  [ACL — API Gateway]                                        │
│       │  REST / gRPC / AsyncAPI                             │
│       ▼                                                     │
...
│  ┌────┴────────────────────────────┐                        │
│  ▼                                 ▼                        │
│  [Aurora PostgreSQL + S3 (archivado)]              [EventBridge]               │
│  (migrado de bdicont)          (reemplaza cross-DB calls)      │
│       │                                                     │
...
|--------------|--------|----------------|
| `sp_consultaparametrosgenerales2` | 16 callers | REST endpoint / evento |
| `sp_datosdiahoy_cod47_2` | 16 callers | REST endpoint / evento |
| `sp_ope_catalogomotivos2` | 16 callers | REST endpoint / evento |
| `sp_si_empresasb4` | 1 callers | REST endpoint / evento |
| `sp_cam_asigna_rev_firmasb3` | 0 callers | REST endpoint / evento |
...
|-------------------|---------|-----------------|------------|
| `bdicont` → `bdicnweb` | 639 | API interna / evento | [SME-PENDING] |
| `bdicont` → `bdinteg` | 43 | API interna / evento | [SME-PENDING] |

## Servicios AWS seleccionados

| Componente | Servicio AWS | Justificación |
|-----------|-------------|--------------|
| Cómputo | Lambda (idempotente) | Apropiado para tipo accounting |
| Base de datos | Aurora PostgreSQL + S3 (archivado) | Reemplaza instancia Informix bdicont |
...
El dominio `Contabilidad` no puede entrar a BUILD hasta que estén disponibles:
...
| SAT | [SME-PENDING — Cybersecurity + Industry Banking] |
