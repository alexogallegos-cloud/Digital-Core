# SP Profile: `sp_cilocgenerarptalertascte`
...
El SP `sp_cilocgenerarptalertascte` implementa la logica de genera reporte, alertas y cliente en el dominio Cobranza (base de datos `bdicobranza`). Comprende 1,952 lineas de codigo, 1 tablas consultadas. Delega logica a: `bdinteg`.
...
| Vista HTML interactiva | [../../portal/sp-detail/sp-detail-sp_cilocgenerarptalertascte.html](../../portal/sp-detail/sp-detail-sp_cilocgenerarptalertascte.html) |
...
```mermaid
flowchart TD
    A["Entrada: sp_cilocgenerarptalertascte"]
    N1{"ptipo = 2"}
    A --> N1
...
sequenceDiagram
    ...
    participant SP as sp_cilocgenerarptalertascte
    participant C1 as bdinteg
    CL->>SP: invoca sp_cilocgenerarptalertascte
    SP->>C1: delega a bdinteg
    C1-->>SP: resultado
    Note over SP,CL: vocab: genera, generar, ciloc, alertas
