# D03 · Créditos — Plan de Cutover

> **Componente:** Informix · SPE-AM-001 · RELEASE Phase
> **Base de datos:** `bdicred` → Aurora PostgreSQL
...
`Créditos` no puede cortar hasta que estos dominios estén disponibles como API:

| ID | Dominio | Llamadas | Estado |
|----|---------|---------|--------|
| D04 | Cheques / Cuentas | 288 | API disponible como prereq |
| D02 | Integración y Autenticación | 226 | API disponible como prereq |
| D06 | Solicitudes | 212 | API disponible como prereq |
| D11 | Cobranza | 152 | API disponible como prereq |
| D09 | Mensajería | 77 | API disponible como prereq |

## Dominios bloqueados por `bdicred`

Estos dominios no pueden migrar hasta que `bdicred` complete el cutover:

| ID | Dominio | Llamadas | Impacto |
|----|---------|---------|---------|
| D01 | Canal Digital Web | 10,903 | No puede migrar hasta que `bdicred` esté listo |
| D02 | Integración y Autenticación | 165 | No puede migrar hasta que `bdicred` esté listo |
| D07 | Aclaraciones | 139 | No puede migrar hasta que `bdicred` esté listo |
| D06 | Solicitudes | 89 | No puede migrar hasta que `bdicred` esté listo |
| D11 | Cobranza | 74 | No puede migrar hasta que `bdicred` esté listo |
...
T=0 (22:00): INICIO VENTANA DE CUTOVER
  ...
  22:05  → Freeze de escrituras en Informix bdicred
