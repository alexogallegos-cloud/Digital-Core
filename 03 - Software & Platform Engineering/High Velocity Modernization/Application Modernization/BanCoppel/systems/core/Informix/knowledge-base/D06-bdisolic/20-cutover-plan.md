# D06 · Solicitudes — Plan de Cutover

> **Componente:** BCOPCore · SPE-AM-001 · RELEASE Phase
> **Base de datos:** `bdisolic` → Aurora PostgreSQL
...
`Solicitudes` no puede cortar hasta que estos dominios estén disponibles como API:

| ID | Dominio | Llamadas | Estado |
|----|---------|---------|--------|
| D03 | Créditos | 89 | API disponible como prereq |
| D02 | Integración y Autenticación | 81 | API disponible como prereq |
| D09 | Mensajería | 23 | API disponible como prereq |

## Dominios bloqueados por `bdisolic`

Estos dominios no pueden migrar hasta que `bdisolic` complete el cutover:

| ID | Dominio | Llamadas | Impacto |
|----|---------|---------|---------|
| D01 | Canal Digital Web | 2,326 | No puede migrar hasta que `bdisolic` esté listo |
| D03 | Créditos | 212 | No puede migrar hasta que `bdisolic` esté listo |
| D02 | Integración y Autenticación | 51 | No puede migrar hasta que `bdisolic` esté listo |
| D09 | Mensajería | 3 | No puede migrar hasta que `bdisolic` esté listo |
...
T=0 (22:00): INICIO VENTANA DE CUTOVER
  ...
  22:05  → Freeze de escrituras en Informix bdisolic
