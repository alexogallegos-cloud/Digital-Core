...
`Aclaraciones` no puede cortar hasta que estos dominios estén disponibles como API:

| ID | Dominio | Llamadas | Estado |
|----|---------|---------|--------|
| D02 | Integración y Autenticación | 452 | API disponible como prereq |
| D04 | Cheques / Cuentas | 153 | API disponible como prereq |
| D03 | Créditos | 139 | API disponible como prereq |
| D09 | Mensajería | 34 | API disponible como prereq |

## Dominios bloqueados por `bdiaclaracion`

Estos dominios no pueden migrar hasta que `bdiaclaracion` complete el cutover:

| ID | Dominio | Llamadas | Impacto |
|----|---------|---------|---------|
| D09 | Mensajería | 1 | No puede migrar hasta que `bdiaclaracion` esté listo |
...
T=0 (22:00): INICIO VENTANA DE CUTOVER
  ...
  22:05  → Freeze de escrituras en Informix bdiaclaracion
