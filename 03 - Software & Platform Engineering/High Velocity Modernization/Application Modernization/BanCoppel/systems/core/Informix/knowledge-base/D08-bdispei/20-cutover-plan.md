...
`SPEI` no puede cortar hasta que estos dominios estén disponibles como API:

| ID | Dominio | Llamadas | Estado |
|----|---------|---------|--------|
| D04 | Cheques / Cuentas | 67 | API disponible como prereq |
| D09 | Mensajería | 24 | API disponible como prereq |
| D03 | Créditos | 9 | API disponible como prereq |
| D02 | Integración y Autenticación | 9 | API disponible como prereq |

## Dominios bloqueados por `bdispei`

Estos dominios no pueden migrar hasta que `bdispei` complete el cutover:

| ID | Dominio | Llamadas | Impacto |
|----|---------|---------|---------|
| D01 | Canal Digital Web | 84 | No puede migrar hasta que `bdispei` esté listo |
| D04 | Cheques / Cuentas | 74 | No puede migrar hasta que `bdispei` esté listo |
| D05 | Saldos y Cuentas | 4 | No puede migrar hasta que `bdispei` esté listo |
...
T=0 (22:00): INICIO VENTANA DE CUTOVER
  ...
  22:05  → Freeze de escrituras en Informix bdispei
