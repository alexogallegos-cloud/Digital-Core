# D05 · Saldos y Cuentas — Plan de Cutover

> **Componente:** BCOPCore · SPE-AM-001 · RELEASE Phase
> **Base de datos:** `bdisac` → Aurora PostgreSQL
...
`Saldos y Cuentas` no puede cortar hasta que estos dominios estén disponibles como API:

| ID | Dominio | Llamadas | Estado |
|----|---------|---------|--------|
| D04 | Cheques / Cuentas | 331 | API disponible como prereq |
| D02 | Integración y Autenticación | 145 | API disponible como prereq |
| D03 | Créditos | 49 | API disponible como prereq |
| D01 | Canal Digital Web | 29 | API disponible como prereq |

## Dominios bloqueados por `bdisac`

Estos dominios no pueden migrar hasta que `bdisac` complete el cutover:

| ID | Dominio | Llamadas | Impacto |
|----|---------|---------|---------|
| D01 | Canal Digital Web | 3,464 | No puede migrar hasta que `bdisac` esté listo |
| D02 | Integración y Autenticación | 31 | No puede migrar hasta que `bdisac` esté listo |
| D04 | Cheques / Cuentas | 28 | No puede migrar hasta que `bdisac` esté listo |
...
T=0 (22:00): INICIO VENTANA DE CUTOVER
  ...
  22:05  → Freeze de escrituras en Informix bdisac
