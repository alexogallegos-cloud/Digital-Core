...
json.dump(flows, open(BASE + "portal/data/flow-data.json", "w", encoding="utf-8"),
...
open(BASE + "knowledge-base/orchestrators-complexity-bcop.md", "w", encoding="utf-8").write("\n".join(L))

import os as _os
print(f"flow-data.json ({_os.path.getsize(BASE+'portal/data/flow-data.json'):,} bytes) · {n_parsed} journeys parseados")
print(f"knowledge-base/orchestrators-complexity-bcop.md · {len(crit)} orquestadores críticos (score≥60)")
