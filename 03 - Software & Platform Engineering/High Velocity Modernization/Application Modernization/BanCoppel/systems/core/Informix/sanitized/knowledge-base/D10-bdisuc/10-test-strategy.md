# D10 · Sucursales — Estrategia de Pruebas de Equivalencia (Golden Master)
...
Demostrar que el `BranchService (Physical Branch Operations)` target produce **resultados idénticos** al SP Informix original bajo todas las condiciones relevantes de producción. Sin esta equivalencia demostrada, el cutover no puede proceder.

## Prioridad — D10 es Wave 3

**Wave 3** — prioridad media-alta. Los dominios de Wave 4+ dependen de la estabilidad de este dominio.

## TS-D10-01 · Golden Master del SP más crítico: `sp_consulta_catdenominacion_bym`

Este SP tiene 374 callers y es el punto de entrada más crítico del dominio.
...
FROM bdisuc:sp_audit_log           -- [SME-PENDING: confirmar tabla de audit]
WHERE fecha_insert >= TODAY - 7
  AND proc_name = 'sp_consulta_catdenominacion_bym'
ORDER BY exec_ts;
```
...
| TC-09 | Volumen de carga de producción | Carga real × 374 req/seg | Latencia dentro de SLA target |
...
## TS-D10-02 · Pruebas de procesos batch (ver 11-batch-processes.md)
...
## TS-D10-03 · Pruebas de integración cross-domain

Los SPs de `bdisuc` hacen llamadas cross-DB a otros dominios. En el target, estas se convierten en llamadas API.

| SP | Dependencia cross-DB | Prueba requerida |
|----|---------------------|-----------------|
| `auditapase` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `consentradassalidas` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `consulta_saldos` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `consulta_saldos` | `bdisuc` | Simular respuesta API de `bdisuc` con mock — verificar contrato |
| `consultacajageneral` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `consultacajageneral` | `bdisuc` | Simular respuesta API de `bdisuc` con mock — verificar contrato |
| `consultacajageneral2` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `consultacajageneral2` | `bdisuc` | Simular respuesta API de `bdisuc` con mock — verificar contrato |
...
`bdisuc` contiene operaciones con `MONEY` e `INTEGER`. En el target PostgreSQL:
...
---
*Generado por: Specialist — Informix SPL Analysis · 2026-07-03 · Evidencia: source/LegacyCore/informix/bdisuc_*.sql (análisis estático de 70 archivos SQL) · Specialist — Informix SPL Analysis + QA Lead Equivalencia*
