# D10 · Sucursales — Arquitectura Target

> **Componente:** LegacyCore · SPE-AM-001 · DESIGN Phase
> **Base de datos:** `bdisuc` · Target: **AWS Aurora PostgreSQL**
...
El dominio `bdisuc` es de tipo **branch**. Su migración sigue este patrón porque Operaciones de sucursales físicas. 293 SPs.

## Topología AWS target

```
┌────────────────────────────────────────────────────────────┐
│  D10 · Sucursales — AWS Target                 │
│                                                             │
│  [ACL — API Gateway]                                        │
│       │  REST / gRPC / AsyncAPI                             │
│       ▼                                                     │
...
│  (migrado de bdisuc)          (reemplaza cross-DB calls)      │
│       │                                                     │
...
|--------------|--------|----------------|
| `sp_consultadatospiezas_bym3` | 381 callers | REST endpoint / evento |
| `sp_consutacat_dictamen_bym` | 378 callers | REST endpoint / evento |
| `sp_consultadatospiezas_bym3_totales` | 376 callers | REST endpoint / evento |
| `sp_consultadatospiezas_bym2` | 376 callers | REST endpoint / evento |
| `sp_consultacat_estatus_bym` | 375 callers | REST endpoint / evento |
...
|-------------------|---------|-----------------|------------|
| `bdisuc` → `bdinteg` | 27 | API interna / evento | [SME-PENDING] |
| `bdisuc` → `bdicont` | 11 | API interna / evento | [SME-PENDING] |
| `bdisuc` → `bdmis` | 1 | API interna / evento | [SME-PENDING] |
| `bdisuc` → `bdicheq` | 1 | API interna / evento | [SME-PENDING] |

## Servicios AWS seleccionados

| Componente | Servicio AWS | Justificación |
|-----------|-------------|--------------|
| Cómputo | Lambda | Apropiado para tipo branch |
| Base de datos | Aurora PostgreSQL | Reemplaza instancia Informix bdisuc |
...
El dominio `Sucursales` no puede entrar a BUILD hasta que estén disponibles:
