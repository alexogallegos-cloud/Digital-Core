...
Este documento define los **contratos de interfaz** que el microservicio target de `Cobranza` debe exponer para reemplazar las llamadas `CALL bdicobranza:sp_nombre()` del sistema Informix. Sin estos contratos, los 82 SPs de los dominios callers no pueden migrar.
...
### API-D11-01: `sp_inserta_bitacora_cob` → `POST /inserta-bitacora-cob`

| Atributo | Valor |
|----------|-------|
| SP origen | `bdicobranza:sp_inserta_bitacora_cob` |
| Fan-in (callers actuales) | 406 |
| Protocolo target | OpenAPI 3.1 (REST) |
| Callers que deben migrar | 406 SPs en otros dominios |

**[SME-PENDING — Core Banking Transformation + Domain Expert LegacyCore]**
```yaml
# OpenAPI 3.1 draft — completar con Domain Expert
paths:
  /inserta-bitacora-cob:
    post:
      summary: "[SME-PENDING] Descripción de negocio del endpoint"
      operationId: insertabitacoracob
      requestBody:
        required: true
        content:
          application/json:
...
### API-D11-02: `sp_cat_graba_telefono_adicional` → `POST /cat-graba-telefono-adicional`

| Atributo | Valor |
|----------|-------|
| SP origen | `bdicobranza:sp_cat_graba_telefono_adicional` |
| Fan-in (callers actuales) | 15 |
| Protocolo target | OpenAPI 3.1 (REST) |
| Callers que deben migrar | 15 SPs en otros dominios |

**[SME-PENDING — Core Banking Transformation + Domain Expert LegacyCore]**
```yaml
# OpenAPI 3.1 draft — completar con Domain Expert
paths:
  /cat-graba-telefono-adicional:
    post:
      summary: "[SME-PENDING] Descripción de negocio del endpoint"
      operationId: catgrabatelefonoadicional
      requestBody:
        required: true
        content:
          application/json:
...
### API-D11-03: `sp_cat_gen_info_admin` → `POST /cat-gen-info-admin`

| Atributo | Valor |
|----------|-------|
| SP origen | `bdicobranza:sp_cat_gen_info_admin` |
| Fan-in (callers actuales) | 7 |
| Protocolo target | OpenAPI 3.1 (REST) |
| Callers que deben migrar | 7 SPs en otros dominios |
...
### API-D11-04: `sp_consultacuentascliente_bis` → `POST /consultacuentascliente-bis`

| Atributo | Valor |
|----------|-------|
| SP origen | `bdicobranza:sp_consultacuentascliente_bis` |
| Fan-in (callers actuales) | 4 |
| Protocolo target | OpenAPI 3.1 (REST) |
| Callers que deben migrar | 4 SPs en otros dominios |

**[SME-PENDING — Core Banking Transformation + Domain Expert LegacyCore]**
```yaml
# OpenAPI 3.1 draft — completar con Domain Expert
paths:
  /consultacuentascliente-bis:
    post:
      summary: "[SME-PENDING] Descripción de negocio del endpoint"
      operationId: consultacuentasclientebis
      requestBody:
        required: true
        content:
          application/json:
...
### API-D11-05: `sp_consultacuentascliente_crd` → `POST /consultacuentascliente-crd`

| Atributo | Valor |
|----------|-------|
| SP origen | `bdicobranza:sp_consultacuentascliente_crd` |
| Fan-in (callers actuales) | 4 |
| Protocolo target | OpenAPI 3.1 (REST) |
| Callers que deben migrar | 4 SPs en otros dominios |
...
# OpenAPI 3.1 draft — completar con Domain Expert
paths:
  /consultacuentascliente-crd:
    post:
      summary: "[SME-PENDING] Descripción de negocio del endpoint"
      operationId: consultacuentasclientecrd
      requestBody:
        required: true
        content:
          application/json:
