# D05 · Saldos y Cuentas — Plan de Migración de Datos

> **Componente:** LegacyCore · SPE-AM-001 · DESIGN Phase → BUILD
> **Base de datos:** `bdisac` → Target: Aurora PostgreSQL 15+
...
| Métrica | Valor |
|---------|-------|
| SPs a migrar (scope mínimo) | 145 en call graph |
| Tablas estimadas (inferidas) | ~9 (confirmar con DBA IBM Informix — Etapa 2) |
...
FASE 1 — SNAPSHOT INICIAL (sin downtime)
  ├─ Debezium captura snapshot de tablas bdisac
...
FASE 3 — CUTOVER (con ventana de mantenimiento ~2 horas)
  ├─ Freeze de escrituras en Informix bdisac
...
## Configuración Debezium para `bdisac`

```json
{
  "name": "debezium-bdisac",
  ...
  "database.dbname": "bdisac",
  "table.include.list": "[SME-PENDING — DBA: lista de tablas a capturar]",
  "decimal.handling.mode": "string",
  "time.precision.mode": "adaptive_time_microseconds",
  "database.serverTimezone": "America/Mexico_City",
  "topic.prefix": "legacycore.bdisac",
  "schema.history.internal.kafka.topic": "schema-changes.bdisac"
}
```

## Dependencias de datos — pre-requisitos

Para que los datos de `bdisac` sean coherentes en Aurora, los siguientes dominios deben haber sido migrados primero (referencian sus datos):
...
SELECT MAX(fecha_insert), COUNT(*) FROM bdisac:tabla_principal; -- Informix
