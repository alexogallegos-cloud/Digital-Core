# D09 · Mensajería — Plan de Migración de Datos

> **Componente:** LegacyCore · SPE-AM-001 · DESIGN Phase → BUILD
> **Base de datos:** `bdimnsj` → Target: Aurora PostgreSQL 15+
...
| Métrica | Valor |
|---------|-------|
| SPs a migrar (scope mínimo) | 1 en call graph |
...
FASE 1 — SNAPSHOT INICIAL (sin downtime)
  ├─ Debezium captura snapshot de tablas bdimnsj
...
FASE 3 — CUTOVER (con ventana de mantenimiento ~2 horas)
  ├─ Freeze de escrituras en Informix bdimnsj
...
## Configuración Debezium para `bdimnsj`

```json
{
  "name": "debezium-bdimnsj",
  ...
  "database.dbname": "bdimnsj",
  "table.include.list": "[SME-PENDING — DBA: lista de tablas a capturar]",
  "decimal.handling.mode": "string",
  "time.precision.mode": "adaptive_time_microseconds",
  "database.serverTimezone": "America/Mexico_City",
  "topic.prefix": "legacycore.bdimnsj",
  "schema.history.internal.kafka.topic": "schema-changes.bdimnsj"
}
```

## Dependencias de datos — pre-requisitos

Para que los datos de `bdimnsj` sean coherentes en Aurora, los siguientes dominios deben haber sido migrados primero (referencian sus datos):

| Dominio prereq | Nombre | Volumen de relación | Impacto si no está listo |
|---------------|--------|--------------------|-----------------------|
| (sin prereqs de datos en dominios previos) | | | |
...
SELECT MAX(fecha_insert), COUNT(*) FROM bdimnsj:tabla_principal; -- Informix
