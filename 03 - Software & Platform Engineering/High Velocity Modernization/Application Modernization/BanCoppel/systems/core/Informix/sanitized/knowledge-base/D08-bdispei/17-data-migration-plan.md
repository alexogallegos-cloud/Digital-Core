# D08 · SPEI — Plan de Migración de Datos
...
| Métrica | Valor |
|---------|-------|
| SPs a migrar (scope mínimo) | 46 en call graph |
...
FASE 1 — SNAPSHOT INICIAL (sin downtime)
  ├─ Debezium captura snapshot de tablas bdispei
...
FASE 3 — CUTOVER (con ventana de mantenimiento ~2 horas)
  ├─ Freeze de escrituras en Informix bdispei
...
## Configuración Debezium para `bdispei`

```json
{
  "name": "debezium-bdispei",
  ...
  "database.dbname": "bdispei",
  "table.include.list": "[SME-PENDING — DBA: lista de tablas a capturar]",
  "decimal.handling.mode": "string",
  "time.precision.mode": "adaptive_time_microseconds",
  "database.serverTimezone": "America/Mexico_City",
  "topic.prefix": "legacycore.bdispei",
  "schema.history.internal.kafka.topic": "schema-changes.bdispei"
}
```

## Dependencias de datos — pre-requisitos

Para que los datos de `bdispei` sean coherentes en Aurora, los siguientes dominios deben haber sido migrados primero (referencian sus datos):

| Dominio prereq | Nombre | Volumen de relación | Impacto si no está listo |
|---------------|--------|--------------------|-----------------------|
| D09 | Mensajería | 24 | Debe estar disponible como API antes de migrar `bdispei` |
...
SELECT MAX(fecha_insert), COUNT(*) FROM bdispei:tabla_principal; -- Informix
