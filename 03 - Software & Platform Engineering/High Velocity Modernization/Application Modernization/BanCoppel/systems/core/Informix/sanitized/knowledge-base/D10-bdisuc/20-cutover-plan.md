# D10 · Sucursales — Plan de Cutover

> **Componente:** LegacyCore · SPE-AM-001 · RELEASE Phase
> **Base de datos:** `bdisuc` → Aurora PostgreSQL
...
`Sucursales` no puede cortar hasta que estos dominios estén disponibles como API:

| ID | Dominio | Llamadas | Estado |
|----|---------|---------|--------|
| D02 | Integración y Autenticación | 27 | API disponible como prereq |
| D12 | Contabilidad | 11 | API disponible como prereq |
| D04 | Cheques / Cuentas | 1 | API disponible como prereq |

## Dominios bloqueados por `bdisuc`

Estos dominios no pueden migrar hasta que `bdisuc` complete el cutover:

| ID | Dominio | Llamadas | Impacto |
|----|---------|---------|---------|
| D01 | Canal Digital Web | 6,094 | No puede migrar hasta que `bdisuc` esté listo |
| D02 | Integración y Autenticación | 7 | No puede migrar hasta que `bdisuc` esté listo |
| D04 | Cheques / Cuentas | 6 | No puede migrar hasta que `bdisuc` esté listo |
| D05 | Saldos y Cuentas | 6 | No puede migrar hasta que `bdisuc` esté listo |
...
T=0 (22:00): INICIO VENTANA DE CUTOVER
  ...
  22:05  → Freeze de escrituras en Informix bdisuc
