# D07 · Aclaraciones — Plan de Migración de Datos
...
FASE 1 — SNAPSHOT INICIAL (sin downtime)
  ├─ Debezium captura snapshot de tablas bdiaclaracion
...
FASE 3 — CUTOVER (con ventana de mantenimiento ~2 horas)
  ├─ Freeze de escrituras en Informix bdiaclaracion
...
## Configuración Debezium para `bdiaclaracion`

```json
{
  "name": "debezium-bdiaclaracion",
  ...
  "topic.prefix": "legacycore.bdiaclaracion",
  "schema.history.internal.kafka.topic": "schema-changes.bdiaclaracion"
}
```

## Dependencias de datos — pre-requisitos

Para que los datos de `bdiaclaracion` sean coherentes en Aurora, los siguientes dominios deben haber sido migrados primero (referencian sus datos):
...
SELECT MAX(fecha_insert), COUNT(*) FROM bdiaclaracion:tabla_principal; -- Informix
