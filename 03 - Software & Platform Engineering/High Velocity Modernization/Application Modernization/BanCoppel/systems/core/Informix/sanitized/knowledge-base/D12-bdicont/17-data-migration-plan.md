...
| Métrica | Valor |
|---------|-------|
| SPs a migrar (scope mínimo) | 19 en call graph |
...
FASE 1 — SNAPSHOT INICIAL (sin downtime)
  ├─ Debezium captura snapshot de tablas bdicont
...
FASE 3 — CUTOVER (con ventana de mantenimiento ~2 horas)
  ├─ Freeze de escrituras en Informix bdicont
...
## Configuración Debezium para `bdicont`

```json
{
  "name": "debezium-bdicont",
  ...
  "database.dbname": "bdicont",
  "table.include.list": "[SME-PENDING — DBA: lista de tablas a capturar]",
  "decimal.handling.mode": "string",
  "time.precision.mode": "adaptive_time_microseconds",
  "database.serverTimezone": "America/Mexico_City",
  "topic.prefix": "legacycore.bdicont",
  "schema.history.internal.kafka.topic": "schema-changes.bdicont"
}
```

## Dependencias de datos — pre-requisitos

Para que los datos de `bdicont` sean coherentes en Aurora, los siguientes dominios deben haber sido migrados primero (referencian sus datos):
...
SELECT MAX(fecha_insert), COUNT(*) FROM bdicont:tabla_principal; -- Informix
