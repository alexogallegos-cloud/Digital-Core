# D08 · SPEI — Estrategia de Pruebas de Equivalencia (Golden Master)
...
Demostrar que el `SPEIService (Payment Rail — ISO 20022)` target produce **resultados idénticos** al SP Informix original bajo todas las condiciones relevantes de producción. Sin esta equivalencia demostrada, el cutover no puede proceder.

## Prioridad — D08 es Wave 2

**Wave 2** — alta prioridad. Los dominios que dependen de `bdispei` no pueden migrar hasta que este dominio esté probado y certificado.

## TS-D08-01 · Golden Master del SP más crítico: `sp_abonoordpago`

Este SP tiene 1 callers y es el punto de entrada más crítico del dominio.
...
FROM bdispei:sp_audit_log           -- [SME-PENDING: confirmar tabla de audit]
WHERE fecha_insert >= TODAY - 7
  AND proc_name = 'sp_abonoordpago'
ORDER BY exec_ts;
```
...
## TS-D08-02 · Pruebas de procesos batch (ver 11-batch-processes.md)
...
## TS-D08-03 · Pruebas de integración cross-domain

Los SPs de `bdispei` hacen llamadas cross-DB a otros dominios. En el target, estas se convierten en llamadas API.

| SP | Dependencia cross-DB | Prueba requerida |
|----|---------------------|-----------------|
| `con_canc_audi` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `con_canc_audi` | `bdispei` | Simular respuesta API de `bdispei` con mock — verificar contrato |
| `consulta_bancos` | `paginterban` | Simular respuesta API de `paginterban` con mock — verificar contrato |
| `consulta_bancos` | `bdispeua` | Simular respuesta API de `bdispeua` con mock — verificar contrato |
| `graba_spei` | `terceros` | Simular respuesta API de `terceros` con mock — verificar contrato |
| `graba_spei` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_abonocanelapago` | `bdicheq` | Simular respuesta API de `bdicheq` con mock — verificar contrato |
| `sp_abonocanelapago` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
| `sp_abonoordpago` | `bdicheq` | Simular respuesta API de `bdicheq` con mock — verificar contrato |
| `sp_abonoordpago` | `bdinteg` | Simular respuesta API de `bdinteg` con mock — verificar contrato |
...
`bdispei` contiene operaciones con `MONEY` e `INTEGER`. En el target PostgreSQL:
...
---
*Generado por: Specialist — Informix SPL Analysis · 2026-07-03 · Evidencia: source/LegacyCore/informix/bdispei_*.sql (análisis estático de 70 archivos SQL) · Specialist — Informix SPL Analysis + QA Lead Equivalencia*
