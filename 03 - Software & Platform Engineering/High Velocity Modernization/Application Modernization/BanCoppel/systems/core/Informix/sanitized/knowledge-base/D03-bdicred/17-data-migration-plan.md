# D03 · Créditos — Plan de Migración de Datos

> **Componente:** LegacyCore · SPE-AM-001 · DESIGN Phase → BUILD
> **Base de datos:** `bdicred` → Target: Aurora PostgreSQL 15+
...
| Métrica | Valor |
|---------|-------|
| SPs a migrar (scope mínimo) | 380 en call graph |
| Tablas estimadas (inferidas) | ~25 (confirmar con DBA IBM Informix — Etapa 2) |
...
FASE 1 — SNAPSHOT INICIAL (sin downtime)
  ├─ Debezium captura snapshot de tablas bdicred
...
FASE 3 — CUTOVER (con ventana de mantenimiento ~2 horas)
  ├─ Freeze de escrituras en Informix bdicred
...
## Configuración Debezium para `bdicred`

```json
{
  "name": "debezium-bdicred",
  ...
  "database.dbname": "bdicred",
  "table.include.list": "[SME-PENDING — DBA: lista de tablas a capturar]",
  "decimal.handling.mode": "string",
  "time.precision.mode": "adaptive_time_microseconds",
  "database.serverTimezone": "America/Mexico_City",
  "topic.prefix": "legacycore.bdicred",
  "schema.history.internal.kafka.topic": "schema-changes.bdicred"
}
```

## Dependencias de datos — pre-requisitos

Para que los datos de `bdicred` sean coherentes en Aurora, los siguientes dominios deben haber sido migrados primero (referencian sus datos):

| Dominio prereq | Nombre | Volumen de relación | Impacto si no está listo |
|---------------|--------|--------------------|-----------------------|
| D06 | Solicitudes | 212 | Debe estar disponible como API antes de migrar `bdicred` |
| D11 | Cobranza | 152 | Debe estar disponible como API antes de migrar `bdicred` |
...
SELECT MAX(fecha_insert), COUNT(*) FROM bdicred:tabla_principal; -- Informix
