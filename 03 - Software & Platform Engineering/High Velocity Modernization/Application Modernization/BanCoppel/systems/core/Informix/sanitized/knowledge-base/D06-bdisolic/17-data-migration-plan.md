# D06 · Solicitudes — Plan de Migración de Datos

> **Componente:** LegacyCore · SPE-AM-001 · DESIGN Phase → BUILD
> **Base de datos:** `bdisolic` → Target: Aurora PostgreSQL 15+
...
| Métrica | Valor |
|---------|-------|
| SPs a migrar (scope mínimo) | 84 en call graph |
...
FASE 1 — SNAPSHOT INICIAL (sin downtime)
  ├─ Debezium captura snapshot de tablas bdisolic
...
FASE 3 — CUTOVER (con ventana de mantenimiento ~2 horas)
  ├─ Freeze de escrituras en Informix bdisolic
...
## Configuración Debezium para `bdisolic`

```json
{
  "name": "debezium-bdisolic",
  ...
  "database.dbname": "bdisolic",
  "table.include.list": "[SME-PENDING — DBA: lista de tablas a capturar]",
  "decimal.handling.mode": "string",
  "time.precision.mode": "adaptive_time_microseconds",
  "database.serverTimezone": "America/Mexico_City",
  "topic.prefix": "legacycore.bdisolic",
  "schema.history.internal.kafka.topic": "schema-changes.bdisolic"
}
```

## Dependencias de datos — pre-requisitos

Para que los datos de `bdisolic` sean coherentes en Aurora, los siguientes dominios deben haber sido migrados primero (referencian sus datos):

| Dominio prereq | Nombre | Volumen de relación | Impacto si no está listo |
|---------------|--------|--------------------|-----------------------|
| D09 | Mensajería | 23 | Debe estar disponible como API antes de migrar `bdisolic` |
...
SELECT MAX(fecha_insert), COUNT(*) FROM bdisolic:tabla_principal; -- Informix
