...
> **Componente:** LegacyCore · SPE-AM-001 · DESIGN Phase → BUILD
...
| Métrica | Valor |
|---------|-------|
| SPs a migrar (scope mínimo) | 220 en call graph |
| Tablas estimadas (inferidas) | ~14 (confirmar con DBA IBM Informix — Etapa 2) |
...
FASE 1 — SNAPSHOT INICIAL (sin downtime)
  ├─ Debezium captura snapshot de tablas bdinteg
...
FASE 3 — CUTOVER (con ventana de mantenimiento ~2 horas)
  ├─ Freeze de escrituras en Informix bdinteg
...
## Configuración Debezium para `bdinteg`

```json
{
  "name": "debezium-bdinteg",
  ...
  "database.dbname": "bdinteg",
  "table.include.list": "[SME-PENDING — DBA: lista de tablas a capturar]",
  "decimal.handling.mode": "string",
  "time.precision.mode": "adaptive_time_microseconds",
  "database.serverTimezone": "America/Mexico_City",
  "topic.prefix": "legacycore.bdinteg",
  "schema.history.internal.kafka.topic": "schema-changes.bdinteg"
}
```

## Dependencias de datos — pre-requisitos

Para que los datos de `bdinteg` sean coherentes en Aurora, los siguientes dominios deben haber sido migrados primero (referencian sus datos):

| Dominio prereq | Nombre | Volumen de relación | Impacto si no está listo |
|---------------|--------|--------------------|-----------------------|
| D04 | Cheques / Cuentas | 194 | Debe estar disponible como API antes de migrar `bdinteg` |
| D03 | Créditos | 165 | Debe estar disponible como API antes de migrar `bdinteg` |
| D09 | Mensajería | 79 | Debe estar disponible como API antes de migrar `bdinteg` |
| D06 | Solicitudes | 51 | Debe estar disponible como API antes de migrar `bdinteg` |
...
SELECT MAX(fecha_insert), COUNT(*) FROM bdinteg:tabla_principal; -- Informix
