CREATE PROCEDURE "informix".sp_marcagraba_tdc_oro(pempresa CHAR(3),TipoEjec CHAR(1), NvaLinea DECIMAL(18,2),pnumsolcred CHAR(20))
...
--SET DEBUG FILE TO '/home/e99805455/trace/sp_marcagraba_tdc_oro'||pnumsolcred||'.out';
--TRACE ON;
...
IF TRIM(NVL(TipoEjec,'')) = '1' THEN
     ...
     WHERE a.empresa = pempresa       
	   ...
	   --Obtener la linea minima para una TDC Oro RQM 10 679-2
	...
	--Unity-Apolo Se excluyen solicitudes de origen canal 9 Apolo para la oferta de oro suc 8802
	IF csucursal <> '8503' AND csucursal <> '8802' THEN
		--Validar si la linea del cliente es mayor o igual a 15000
		IF NvaLinea >=v_Linea_min_oro THEN 	
			...
			ELIF vingresomensual >= v_ingreso_min_oro THEN
				...
				where num_solicitud=pNumsolcred
				AND seccion=2;			
				-- INC 27 201 Se agrega consideracion para cuando la respuesta de bcscore sea negativa se toma como Hit sin Información, se agrega con evalua X para tomarlo como Hit sin informacion solo esta ocasión ya que así esta parametrizado en Oro por que este producto no tiene No Hit
				--evalua_cc  Parametrizacion correcta de Buro
				--X-- No hit
				--0 , bcscore > a 0 -- Hit con buenos antecedentes
				--0 , bcscore < a 0 -- Hit sin información
				-- > 0  -- Hit con malos antecedentes
				IF dbscore <= 0 THEN 
					LET v_respsic='X';
				END IF;			
				
				/*Se cambia la validacion de score por parametrico de score coppel -> 4*/
				...
				IF VNuevoStatus = 'AT' THEN
					--Se registran datos de determinacion para el producto TDCOro
					SELECT count(numero_solicitud) INTO iexiste FROM "informix".ss_solicitudes_tdcoro where numero_solicitud=pNumsolcred; 
					IF iexiste = 0 THEN
						INSERT INTO bdisolic:"informix".ss_solicitudes_tdcoro(empresa, numero_solicitud,numcte ,numero_solicitud_oro, confirma_oro , flag_oro, nombre_embosado, ingreso_lc, valor_cma,	valor_tab, linea_teorica, user_insert, fecha_insert)
						  VALUES(pempresa, pNumsolcred,cNumcte, '', '',1, '', 0, 0, 0, 0, USER, CURRENT);
				...
				ELSE
					--Se apaga el flag de oro por que ya no se cumple con las politicas de credito de puntajes
					SELECT count(numero_solicitud) INTO iexiste FROM "informix".ss_solicitudes_tdcoro where numero_solicitud=pNumsolcred; 
					IF iexiste = 1 THEN
			...
			END IF;
		ELSE
			--Se apaga el flag de oro por que ya no se cumple con la linea minima de producto para Oro
			SELECT count(numero_solicitud) INTO iexiste FROM "informix".ss_solicitudes_tdcoro where numero_solicitud=pNumsolcred; 		
			IF iexiste = 1 THEN
				UPDATE  bdisolic:"informix".ss_solicitudes_tdcoro SET flag_oro=0 WHERE numero_solicitud=pNumsolcred;
			END IF;	
...
ELIF TRIM(NVL(TipoEjec,'')) = '2' THEN
	...
	IF NVL(cSolOro,'') = '' THEN 	
		...
		-- *************************************
		IF LENGTH(cSolOro) = 12 and bdinteg:"informix".val_num(cSolOro) then  --- se valida que el numero de solicitud sea de 12 y todos sean numericos.
	...
	--RQM 10 679-2 Se agrega migracion de tabla ss_nuevo parametrico.
	...
	(pempresa, 'sistema',pnumsolcred, "CN", "CMP", "Cancelacion por migracion a producto ORO")
	INTO scod_ret;
...
ELIF TRIM(NVL(TipoEjec,'')) = '3' THEN	
	...
	--tabla de control para clonacion de la solicitud  en esta ejecucion pnumsolcred es la Solicitud TDC Oro
