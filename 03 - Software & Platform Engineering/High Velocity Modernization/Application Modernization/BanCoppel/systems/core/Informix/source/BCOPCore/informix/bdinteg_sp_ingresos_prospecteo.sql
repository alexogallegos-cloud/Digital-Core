CREATE PROCEDURE "informix".sp_ingresos_prospecteo(pNumSolic char(12))
...
'Folio: 529 - ',
'Fecha: 19-03-2019',
'Modificación: Se crea el procedimiento almacenado "sp_ingresos_prospecteo" para obtener los ingresos del cliente ',
'Solicita: ', 
'Base de datos: BDISOLIC';

CREATE PROCEDURE "informix".sp_ws_valida_cotel_exp (pUserName CHAR(8), pUserPass CHAR(8),pIdSession CHAR(30), pIpOrigen CHAR(16), pAgentTransTypeCode CHAR(10), pAgentCd CHAR(3), pCorreo CHAR(100), pTelefono CHAR(10), pNombre1 CHAR(26), pNombre2 CHAR(26), pApellido1 CHAR(26), pApellido2 CHAR(26), pFechaNacimiento CHAR(8))


	RETURNING 	CHAR(4)  AS cCodRet , 					-- Indica el cÃÂ³digo de retorno del procedimiento
				CHAR(120) AS cDescrMensaje,				-- DescripciÃÂ³n de la respuesta obtenida	
				CHAR(4)   AS cCodRetCorreo, 			-- CÃÂ³digo de retorno de validaciÃÂ³n de correo
			    CHAR(120) AS cDescrMensajeCorreo,		-- DescripciÃÂ³n de la respuesta obtenida en validaciÃÂ³n de correo
				CHAR(4)	  AS cCodRetTelefono,			-- CÃÂ³digo de retorno de validaciÃÂ³n de telÃÂ©fono
				CHAR(13)  AS cRFC,						-- RFC calculado para el cliente
				CHAR(120) AS cDescrMensajeTelefono,		-- DescripciÃÂ³n de la respuesta obtenida en la validaciÃÂ³n de TelÃÂ©fono	
				CHAR(20)  AS cNumCte;					-- NÃÂºmero de cliente asignado.
	...
	SET DEBUG FILE TO '/RESPALDOSNEW/mbucio/Vobos/16122019/sp_cotel.out';
	TRACE ON;		
	
BEGIN
		ON EXCEPTION SET iSqlErr
			IF iSqlErr <> 0 THEN
				LET cCodRet = REPLACE(iSqlErr,'-','');
				...
				--  CALCULAR RFC
					...
					IF TRIM(cCodRetRfc) ='00000' THEN							
							...
							IF TRIM(NVL(cCodRetCorreo,'')) IN ('0000', '0002' ) AND  TRIM(NVL(cCodRetTelefono,'')) IN('0000', '0003', '0004','0005','1166', '1169') THEN
								...
								SELECT numcte INTO cNumCte FROM bdinteg:"informix".si_cliente WHERE rfc = cRFC;
		...
		--CONSILTA DE MENSAJES
		SELECT opcode_ds INTO cDescrMensaje FROM  bdinteg:"informix".si_so_catmensajes WHERE opcode= cCodRet;

		RETURN cCodRet ,NVL(cDescrMensaje,''), cCodRetCorreo, cDescrMensajeCorreo, cCodRetTelefono, cRFC,  cDescrMensajeTelefono, cNumCte ;
	
	END;
	
END PROCEDURE
DOCUMENT
'DESCRIPCION: SP para validar telÃÂ©fono y correo electronico ' ,
'AUTOR: Selene Campos',   
