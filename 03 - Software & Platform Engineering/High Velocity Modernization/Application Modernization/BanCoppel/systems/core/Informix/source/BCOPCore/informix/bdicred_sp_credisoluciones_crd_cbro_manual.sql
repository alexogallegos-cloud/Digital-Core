CREATE PROCEDURE "informix".sp_credisoluciones_crd_cbro_manual(pempresa CHAR(3), pproducto CHAR(4))
...
LET count_exists	 = 0;
 ...
 --SET DEBUG FILE TO "/informix/sp_credisoluciones_cbro_manual.out";
 --TRACE ON;
	...
	END IF;
		...
		BEGIN WORK;
			...
			DELETE FROM "informix".sd_log_cobroaut
				  WHERE num_credito > '690000000000'
					AND fecha_proceso = dtFechaHoy
					AND proceso = 'CobroAutCs';
		COMMIT WORK;
    ...
    FOREACH WITH HOLD  --FMV 15JUL14: Se adiciona with hold, ya que solo cobraba 1 credisolucion en vencimiento.
		...
		AND a.num_credito IN ('690000249829','690000279206')
        group by 1,2
		


        SELECT a.num_credito,a.folio_suc,a.folio_movto, c.prox_fecha_pago, b.numcte
        INTO cCredito_promo,cfolio_suc_promo,cfolio_mov_promo,dtFechaMesiversario, cNumCte
        ...
        AND d.status_cred IN('AA','BA');
		
		--IF cNumCreditocrdAnt = cNumCreditocrd THEN -- Para que pueda cobrar mas de una mensualidad de un solo credito
		SELECT limit 1 folio INTO cfolio_rev FROM bdicred:sd_maecredrevcrd WHERE numcte = cNumCte; 
			
		DELETE FROM bdicred:sd_log_cobroaut WHERE num_credito = cNumCreditocrd AND fecha_proceso = dtFechaHoy AND proceso = 'CobroAutCs';
		DELETE FROM bdicred:sd_secpago WHERE folio_suc = cfolio_rev; -- empresa = pempresa and num_credito = cNumCreditocrd;
		DELETE FROM bdicred:sd_maecredrevcrd WHERE numcte = cNumCte; 
		DELETE FROM bdicred:sd_maesdosrevcrd WHERE folio = cfolio_rev;
		DELETE FROM bdicred:sd_amortiza_creditorevcrd WHERE folio = cfolio_rev;
		DELETE FROM bdicred:sd_maecredanexorevcrd WHERE num_credito= cNumCreditocrd AND folio = cfolio_rev;
		DELETE FROM bdicred:sd_promocion_credito_rev WHERE num_cte = cNumCte AND num_sol_prestamo = cNumCreditocrd;
		DELETE FROM bdicred:sd_maeretenido_rev WHERE empresa = pempresa AND num_credito = cCredito_promo;
		--END IF;
        ...
        IF ( cCredito_promo is not null ) THEN
            ...
            IF v_mto_pag_cs > 0 THEN
                ...
                IF v_iva_cs <> 0 THEN
					...
					SELECT count(monto) INTO count_exists FROM bdicred:sd_maeretenido WHERE empresa = '001'
					   AND num_credito = cCredito_promo AND folio_suc = cfolio_mov_promo AND estatus = 'R';
                    --IF EXISTS() THEN                      
            ...
            ELIF v_mto_pag_cs IS NULL OR v_mto_pag_cs = '' THEN
				...
				SELECT COUNT(*) INTO count_exists FROM "informix".sd_log_cobroaut WHERE num_credito = cNumCreditocrd
				   AND fecha_proceso = dtFechaHoy AND proceso = 'CobroAutCs' ;
                --IF EXISTS ( ) THEN
				IF count_exists > 0 THEN
					LET count_exists = 0;
					DELETE FROM "informix".sd_log_cobroaut
        ...
        IF vlStatusCred = 'FF' THEN
			...
			select count(monto) INTO count_exists FROM bdicred:sd_maeretenido WHERE empresa = '001' AND num_credito = cCredito_promo 
               AND folio_suc = cfolio_mov_promo AND estatus = 'R';
            --IF EXISTS() THEN
			IF count_exists > 0 THEN
				LET count_exists = 0;
