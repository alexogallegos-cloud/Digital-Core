CREATE PROCEDURE "informix".act_lineas(e_linea    CHAR(20),
                             e_numcte   CHAR(20),
                             e_producto CHAR(4),
                             e_monto    MONEY(14,2),
                             e_tpmov    CHAR(1),
...
DEFINE v_codref   SMALLINT;
DEFINE v_tplinea  CHAR(1);
DEFINE v_sucursal CHAR(4);
DEFINE v_divisa   CHAR(2);
DEFINE v_conta    CHAR(1);
...
SELECT (linea_prod - linea_util), tp_linea
  INTO v_disp, v_tplinea
  FROM lineas:sl_ctepro
 WHERE num_linea = e_linea
   AND numcte    = e_numcte
   AND producto  = e_producto;

SELECT contabiliza INTO v_conta
  FROM lineas:sl_catgrupos a, lineas:sl_grupos b
 WHERE num_linea = e_linea
   AND a.num_gpo = b.num_gpo;
...
IF v_disp = 0 AND e_tpmov = "C" THEN
        LET v_codret = "601";
        IF wbegin <> "S" THEN
                ROLLBACK WORK;
        END IF
...
IF v_disp < e_monto  AND e_tpmov = "C" THEN
        LET v_codret = "602";
        IF wbegin <> "S" THEN
                ROLLBACK WORK;
        END IF
...
IF v_tplinea = "D" AND e_tpmov = "A" THEN
        LET v_codret = "603";
        IF wbegin <> "S" THEN
                ROLLBACK WORK;
        END IF
...
SELECT sucursal INTO v_sucursal FROM bdinteg:si_cliente
 WHERE numcte = e_numcte
 AND empresa  = e_empresa;

SELECT divisa INTO v_divisa FROM sd_definicion
 WHERE num_producto = e_producto
 AND empresa        = e_empresa;

IF e_tpmov = "C" THEN

        UPDATE lineas:sl_ctepro SET linea_util = linea_util + e_monto
         WHERE num_linea = e_linea
           AND numcte    = e_numcte
           AND producto  = e_producto;

        UPDATE lineas:sl_ctegpo SET linea_util = linea_util + e_monto
         WHERE num_linea = e_linea
           AND numcte    = e_numcte;

        UPDATE lineas:sl_grupos SET linea_util = linea_util + e_monto
         WHERE num_linea = e_linea;
...
IF v_codret = "000" THEN
 IF v_conta = "S" THEN
  --EXECUTE PROCEDURE genmov(e_empresa,e_numcte, 0, e_producto,v_codref,"050", e_hoy,
                          -- e_monto, e_folio, v_sucursal, v_divisa, "0000")
      --INTO v_codret;
 END IF
END IF
...
DOCUMENT
"****************************************************************************",
"* Este spl se encarga actualizar las lineas de credito                     *",
"* Se prohibe la distribucion total o parcial de este programa sin la       *",
"* autorizacion de GRUPO PISA   Base de Datos: BDICRED                      *",
"* Realizado por : Sergio Ruiz el 03/05/2002                                *",
"* Modificado por: Sergio Ruiz el 21/06/2003                                *",
"****************************************************************************";
