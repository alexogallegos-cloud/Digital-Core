CREATE PROCEDURE "informix".sp_seg_conssisegusuarios(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumUsuario CHAR(10))
		RETURNING CHAR(5) AS codret,
			CHAR(10) AS id_usuario,
			CHAR(20) AS password,
			CHAR(1) AS pass_encrip,
			CHAR(88) AS encriptacion;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cId_usuario,cPassword,cPass_encrip,cEncriptacion;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_seg_conssisegusuarios.out';
		--TRACE ON;
		...
		SELECT FIRST 1 id_usuario,password,pass_encrip,encriptacion 
		INTO cId_usuario,cPassword,cPass_encrip,cEncriptacion
		FROM bdinteg:"informix".si_seg_usuarios
		...
		IF NVL(cPass_encrip,0) = 0 THEN
			LET cPass_encrip = '0';
		ELIF cPass_encrip = '' THEN 
			LET cPass_encrip = '0';
		END IF;
...
'FECHA: 27/04/2020',
'MODULO: SEGURIDAD',
'FUNCIONALIDAD: ENCRIPTACIÃN DE CONTRASEÃAS',
'DESCRIPCION: SPL encargado de consultar si el usuario existe en tabla si_seg_usuarios.',
'BD: bdicnweb',
'AUTOR: Veronica Sanchez',
'FECHA: 27/10/2020',
'DESCRIPCION: Se realiza ajuste a SP para tratamiento de valor nulo en campo cPass_encrip';

CREATE PROCEDURE "informix".sp_seg_actualizausuarios(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdTabla CHAR(1), pNumUsuario CHAR(10), pPassword CHAR(88), pPassEncriptado CHAR(88))
		RETURNING CHAR(5) AS codret;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_seg_actualizausuarios.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pIdTabla = '' OR pNumUsuario = '' OR pPassEncriptado = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF;
		...
		IF pIdTabla = '1' THEN
			--UPDATE bdinteg:"informix".si_seg_usuarios SET password = pPassword, pass_encrip = '1' WHERE id_usuario = pNumUsuario;
			UPDATE bdinteg:"informix".si_seg_usuarios SET pass_encrip = '1', encriptacion = pPassEncriptado WHERE id_usuario = pNumUsuario;
			
			IF DBINFO('sqlca.sqlerrd2') = 0 THEN
				LET cCodRet = '00283';
			END IF;
		ELIF pIdTabla = '2' THEN
			UPDATE bdicnweb:"informix".sw_seg_valida_usuarios SET password_usuario = pPassEncriptado, pass_encrip = '1' WHERE id_usuario = pNumUsuario;
...
'FECHA: 27/04/2020',
'MODULO: SEGURIDAD',
'FUNCIONALIDAD: ENCRIPTACIï¿½N DE CONTRASEï¿½AS',
'DESCRIPCION: SPL encargado de actualizar el password y el campo pass_encrip para los usuarios con password encriptado.',
"AUTOR: JosÃ© Antonio RamÃ­rez Franco",
"FECHA : 06-09-2024",
"DESCRIPCION: Se cambia el tamaÃ±o del parametro pPassword de CHAR(50) a CHAR(88) y pPassEncriptado de CHAR(50) a CHAR(88).",
'BD: bdicnweb';
