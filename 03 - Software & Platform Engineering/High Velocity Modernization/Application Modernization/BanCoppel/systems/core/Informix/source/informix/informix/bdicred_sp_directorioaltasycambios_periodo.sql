CREATE PROCEDURE "informix".sp_directorioaltasycambios_periodo()
returning char (5);
...
create table sd_directorioaltasycambios_periodo
        (
...
--  SELECT fecha_ant INTO fecha_movto FROM bdicred:sd_fechas;
    ...
    select trim(descripcion), trim(valor)
      into vanio, vmes
    ...
    where fecha_insert >= mdy(vmes,'01',vanio)
      and fecha_insert < date(mdy(vmes,'01',vanio)) + 1 units month
    into temp clientes_alta with no log;


   LET cNombreArchivo1 = 'DirectorioCtesBancoppel' || trim(vanio) || trim(vmes)  || '.txt';
   LET cNombreArchivo2 = 'CifrasControlDirectorioCtesBancoppel' || trim(vanio) || trim(vmes)  || '.txt';
...
--                                rpad(TRIM(puest.descripcion),30,' ') as puesto, -- descripcion puesto
                                ...
                                --rpad(TRIM(ctepf.email),60,' ') as email, -- correo electronico
								rpad(TRIM(co.correo_elec),60,' ') as email, -- correo electronico
...
--                                rpad(TRIM(edo.nombre),30,' ') as estado, -- descripcion del estado
                                ...
                                lpad(dir.numerociudad,2,'0') AS numciudad, -- numero cd
...
--                                rpad(TRIM(ciudad.nombreciudad),25,' ') AS municipio,  -- descripcion municipio
                                ...
                                lpad(dir.numerocalle,4,'0') AS numcalle,  -- numero de calle de cliente
...
--                                nvl(ing.ingreso_mensual,' ') AS ingresomensual,     -- ingreso mensual
                               ...
                               -- nvl(rpad(TRIM(dir.telefono1),13,' '),' ') AS tel_casa,   -- telefono casa
                                --nvl(rpad(TRIM(dir.telefono2),13,' '),' ') AS celular,    -- celular
...
--                                  cEscolaridad, cNumEstado, sNumCiudad , cNomCiudad, cNomMunicipio, iNumColonia, iNumCalle, cNumExterior,
                                  ...
                                  cTipoCasa, /*cTelefonoCasa, cCelular,*/dFechaInsert,cEmpleadoInsert 
                      from bdinteg:si_cliente cte 
                      INNER JOIN clientes_alta cte_paso ON (cte.numcte = cte_paso.numcte)
                      INNER JOIN bdinteg:si_direcciones dir ON (dir.numcte = cte.numcte and dir.tipo_dir = '1'
                                                               and dir.secuencia = ( SELECT MAX(secuencia)
                                                               FROM bdinteg:si_direcciones dir1
                                                               WHERE dir1.tipo_dir = '1' AND dir1.numcte = cte.numcte))
...
--                      AND mcr.status_cred in ('AA','BA','BT')
					...
					where numcte = cNumCliente 
						...
						and secuencia = (select max(secuencia) from bdinteg:si_telefonos_actual 
													where numcte = cNumCliente and tipo_tel = 1 and cofetel ='V');
					...
					where numcte = cNumCliente 
						...
						and secuencia = (select max(secuencia) from bdinteg:si_telefonos_actual 
													where numcte = cNumCliente and tipo_tel = 2 and cofetel ='V');
        ...
        -- Se obtiene la descripcion del elemento respondido  en la pregunta Tiempo de permanencia en la ocupacion actual
                    ...
                    --- Informacion del Trabajo
                     SELECT  
                               lpad(dir.numerociudad,2,'0') AS numciudad_trab, -- numero cd  trabajo
...
--                               rpad(TRIM(ciudad.nombreciudad),25,' ') AS municipio_trab,  -- descripcion municipio trabajo
                               ...
                               lpad(dir.numerocalle,4,'0') AS numcalle_trab,  -- numero de calle del trabajo
                               ...
                               rpad(TRIM(NVL(dir.puntocardinal,'')),1,' ') AS puntocardinal_trab,   -- punto cardinal trabajo
                               ...
                               --nvl(rpad(TRIM(dir.telefono3),13,' '),' ')  AS tel_trabajo_trab, -- telefono trabajo
                               --nvl(rpad(TRIM(dir.extension),5,' '),' ')  AS ext_trabajo_trab -- extension trabajo
--                      INTO sNumCiudadTrab, cNomCiudadTrab, cNomMunicipioTrab, iNumColoniaTrab, iNumCalleTrab,  cNumExteriorTrab,
                      ...
                      FROM  bdinteg:si_cliente cte
                      LEFT OUTER JOIN bdinteg:si_direcciones dir ON (dir.numcte = cNumCliente AND dir.tipo_dir = '2'
                                                                    AND dir.secuencia = (SELECT MAX(secuencia)
                                                                    FROM bdinteg:si_direcciones dir1
                                                                    WHERE dir1.tipo_dir = '2'
                                                                    AND dir1.numcte = cNumCliente))
--                      LEFT OUTER JOIN bdinteg:si_catciudades ciudad ON (ciudad.numerociudad = dir.numerociudad)
                      WHERE cte.numcte= cNumCliente;

					select  nvl(rpad(TRIM(telefono),13,' '),' ')  ,nvl(rpad(TRIM(extension),5,' '),' ')
						into  cTelefonoTrabajo, cExtensionTrabajo
					from bdinteg:si_telefonos_actual 
					where numcte = cNumCliente 
						and tipo_tel = 3 and cofetel ='V'
						and secuencia = (select max(secuencia) from bdinteg:si_telefonos_actual 
													where numcte = cNumCliente and tipo_tel = 3 and cofetel ='V');
           ...
           --- Se inserta la informacion en la tabla cb_directorioctesbancoppel .
            ...
            INSERT INTO sd_directorioaltasycambios_periodo (empresa, numctebancoppel,apell_paterno,apell_materno,nombre1,nombre2,fecha_nac,rfc,curp,sexo,estado_civil,apell_casada,
...
--                                                    ingreso_mensual,num_ciudad_trab,ciudad_trabajo,delegacion_municipio_trabajo,colonia_trabajo,calle_trabajo,numeroext_trab,
                                                    ...
                                                    monto_solicitado,clave_movimiento,num_sucursal_realizo_movimiento,nombre_empleado_realizo_movimiento,fecha_movimiento)
...
--                        nvl ( replace ( replace( cNomMunicipioTrab , '|' , ' ' ), '\' , ' ' ), ' ' ),
    ...
    update statistics medium for table bdicred:sd_directorioaltasycambios_periodo;
    ...
    VALUES ('001', iContadorRegistros, iContadorAltas,iContadorCambios,mdy('07','31','2007'));

    -- para Generar el archivo de Salida.
        LET cSql = 'echo "UNLOAD TO ' || '''DirectorioCtesBancoppelRegistros.unl''' || ' DELIMITER ' || '''|'''  ||
                   ' SELECT * FROM bdicred:sd_directorioaltasycambios_periodo d ' ||
                   ' " > DirectorioCtesBancoppelQuerys.sql';
              SYSTEM cSql;

              LET cSql = '';
              LET cSql = 'dbaccess bdicred DirectorioCtesBancoppelQuerys.sql';
              SYSTEM cSql;

              LET cSql = '';
              LET cSql = "sed 's/|$//g' DirectorioCtesBancoppelRegistros.unl > paso001.txt ";
              SYSTEM cSql;

              LET cSql = '';
              LET cSql = "sed 's/" || '"' ||  "//g' paso001.txt > " || trim(cNombreArchivo1);
              SYSTEM cSql;
              ...
              LET cSql = "rm DirectorioCtesBancoppelRegistros.unl DirectorioCtesBancoppelQuerys.sql paso001.txt";
              SYSTEM cSql;
    -- para Generar el archvio de Cifras.
             LET cSql = '';
             LET cSql = 'echo "UNLOAD TO ' || '''DirectorioCifrasControlRegistros.unl''' || ' DELIMITER ' || '''|'''  ||
                                 ' SELECT * FROM bdicred:sd_cifracontroldirectorioaltasycambios c ' ||
                                 ' WHERE c.fecha_generacion = mdy("07","31","2007") ' ||
                                 ' " > DirectorioCifrasControlQuerys.sql';

              SYSTEM cSql;

              LET cSql = '';
              LET cSql = 'dbaccess bdicred DirectorioCifrasControlQuerys.sql';
              SYSTEM cSql;


              LET cSql = '';
              LET cSql = "sed 's/|$//g' DirectorioCifrasControlRegistros.unl > " || trim(cNombreArchivo2);
              SYSTEM cSql;
              ...
              let cSql = '';
              LET cSql = "rm DirectorioCifrasControlRegistros.unl DirectorioCifrasControlQuerys.sql " || trim(cNombreArchivo1) || " " || trim(cNombreArchivo2);
              SYSTEM cSql;

   DROP TABLE bdicred:sd_directorioaltasycambios_periodo;
