CREATE PROCEDURE "informix".sp_consultas_cac_central_cte(pEmpresa        CHAR(3), 
                                              pNombre1          CHAR(30),
                                              pNombre2          CHAR(30),
                                              pApellidoP        CHAR(30),
                                              pApellidoM        CHAR(30),
...
RETURNING 
          CHAR(6) AS codigo_error,            -- Código de Retorno  
          CHAR(80) AS mensaje_error,          -- Mensaje de Retorno
          CHAR(20) AS num_solicitud,          -- Número de Solicitud
          CHAR(20) AS num_cliente,            -- Número de Cliente
          CHAR(104) AS nombre_cliente,        -- Nombre del Cliente
          CHAR(13) AS rfc,                     --rfc
          CHAR(4) AS sucursal,                -- Sucursal
          DATE AS fecha_solicitud,            -- Fecha Solicitud
          DATE AS fecha_cambio,               -- Fecha Cambio Estatus
          DECIMAL(18,2) AS importe_linea,     -- Importe de Linea
          DECIMAL(5,2) AS eficiencia,         -- Eficiencia
          INTEGER AS historial,               -- Historial
          DECIMAL(5,2) AS puntos_1a_seccion,  -- Puntos 1a Sección
          DECIMAL(5,2) AS puntos_2a_seccion,  -- Puntos 2da Sección
          CHAR(2) AS status,                  -- Estatus
          CHAR(511) AS observaciones,         -- Observaciones Anteriores
          DECIMAL(8,2) AS sum_secciones,      -- Suma de secciones
		  CHAR(3) AS causa_solicitud;         -- Causa de solicitud
		  
		  
--ACTIVIDAD: Se modifica para contemplar un retorno para la causa de solicitud.
--FECHA: 07/06/2010.
--AUTOR: Mohamed Carreón

--ACTIVIDAD: Se agrega la validación de la consulta a Buró y Circulo de Crédito de la solicitud homologandolo con el spl sp_consultas_cac_central.
--FECHA: 24/01/2011.
...
--SET DEBUG FILE TO 'sp_consultas_CAC_central_pba2.out';
--TRACE ON;
...
LET cMensajeRet= "Se realizó la consulta al central correctamente.";
 ...
 IF pApellidoP is null then
    let pApellidoP = '';
 END IF;

 IF pApellidoM is null then
    let pApellidoM = '';
 END IF;

 IF pNombre1 is null then
    let pNombre1 = '';
 END IF;

 IF pNombre2 is null then
    let pNombre2 = '';
 END IF;
 
  IF pNumCte IS NULL THEN 
     SELECT numcte,
            (CASE WHEN NVL(razon_social,'') = '' 
			 THEN TRIM(nombre1) || " " || TRIM(nombre2) || " " || TRIM(apell_paterno) || " " || TRIM(apell_materno) 
			 ELSE TRIM(razon_social) END),
            rfc
       INTO pNumCte,
            cNombreCte,
            cRFC
       FROM bdinteg:"informix".si_cliente
       where apell_paterno = pApellidoP 
         AND apell_materno = pApellidoM 
...
FOREACH 
    SELECT sol.num_solicitud,
     ...
     WHERE sol.numcte = pNumCte
      ...
      -- Para las solicitudes con estatus de BC y CC, se valida que su información crediticia se encuentre correctamente
      ...
      IF cStatusSol IN ('CC','BC') THEN
       ...
       SELECT 
              (CASE WHEN NVL(d.fecha_entrada,date(1)) >= NVL(esp.fecha_modif,date(1))  
                 THEN NVL(d.fecha_entrada,date(1))
                 ELSE NVL(esp.fecha_modif,date(1)) 
              END),
             (CASE WHEN NVL(d.fecha_entrada,date(1)) >= NVL(esp.fecha_modif,date(1)) 
                 THEN NVL(d.comentario,"")
                 ELSE NVL(esp.comentario,"")
              END),
			  d.causa_solicitud
        INTO dtFechaModificacion,
             cComentarioAut,
			 sCausa
        FROM bdisolic:"informix".ss_autorizacion d           
        FULL OUTER JOIN bdisolic:"informix".ss_autorizacion_especial AS esp ON (d.empresa= esp.empresa 
                                                                   AND d.num_solicitud= esp.num_solicitud
                                                                   AND d.status_solicitud= esp.status_nvo
                                                                   AND esp.secuencia= (SELECT NVL(MAX(esp_aux.secuencia),0) 
                                                                                         FROM bdisolic:"informix".ss_autorizacion_especial AS esp_aux
                                                                                        WHERE esp_aux.status_nvo= cStatusSol
                                                                                          AND esp_aux.empresa= pEmpresa
                                                                                          AND esp_aux.num_solicitud=cNumSolicitud
                                                                                          AND esp_aux.numcte = cNumCte
                                                                                          AND esp_aux.secuencia = esp_aux.secuencia)                                                                                          
                                                                   )
        WHERE d.empresa=pEmpresa
        AND d.num_solicitud = cNumSolicitud
        AND d.status_solicitud= cStatusSol
        AND d.ejecutivo_auto= d.ejecutivo_auto 
        AND d.revision_cac <> 1 
        AND d.rowid= (SELECT MAX(e.rowid) 
                                 FROM bdisolic:"informix".ss_autorizacion e 
                                WHERE e.empresa = d.empresa 
                                  AND e.num_solicitud= d.num_solicitud 
                                  AND e.status_solicitud= d.status_solicitud 
                                  AND d.revision_cac <> 1);
        ...
        SELECT ef.situacion_pago,         -- Situacion Pago
		 ...
		 --  IF (dSituacionPago IS NULL AND iMesesHistoria IS NULL)  AND cProducto <> '6011' THEN
                --CONTINUE FOREACH;
           --END IF;
...
END PROCEDURE
DOCUMENT
'Se realiza procedimiento para consultas cac optimizadas',
'Autor: Roque Solis',
'FECHA : 05/18/2009',
'Modificacion: Mohamed Carreon, se agrego la causa de la solicitud, Junio 2010',
...
'VERSION: 20120904.1300',
'BD BDICRED';

CREATE PROCEDURE "informix".sp_reporte_syspagos(pEmpresa CHAR(3))
...
SELECT (pri_dia_mes -3 units month)::DATE, (pri_dia_mes - 1 units day)::DATE
INTO dtFechaIni,dtFechaFIN
FROM bdicred:"informix".sd_fechas;

	IF NOT EXISTS (SELECT * from sd_reporte_sispagos_sec1 
					WHERE fecha_ini_periodo	 = dtFechaIni 
					AND  fecha_fin_periodo = dtFechaFin) THEN
		EXECUTE PROCEDURE "informix".sp_reporte_syspagos1 (dtFechaIni , dtFechaFin ) 
		INTO cCodRet, cMensajeRet;
	return cCodRet, cMensajeRet;
	END IF;
	
	IF NOT EXISTS (SELECT * from sd_reporte_sispagos_sec2 
					WHERE fecha_ini_periodo	 = dtFechaIni 
					AND  fecha_fin_periodo = dtFechaFin) THEN
		EXECUTE PROCEDURE "informix".sp_reporte_syspagos2 (dtFechaIni , dtFechaFin ) 
		INTO cCodRet, cMensajeRet;
	return cCodRet, cMensajeRet;
	END IF;
	
	IF NOT EXISTS (SELECT * from sd_reporte_sispagos_sec3 
					WHERE fecha_ini_periodo	 = dtFechaIni 
					AND  fecha_fin_periodo = dtFechaFin) THEN
		EXECUTE PROCEDURE "informix".sp_reporte_syspagos3 (dtFechaIni , dtFechaFin ) 
		INTO cCodRet, cMensajeRet;
	return cCodRet, cMensajeRet;
	END IF;
	IF NOT EXISTS (SELECT * from sd_reporte_sispagos_sec4 
					WHERE fecha_ini_periodo	 = dtFechaIni 
					AND  fecha_fin_periodo = dtFechaFin) THEN
		EXECUTE PROCEDURE "informix".sp_reporte_syspagos4 (dtFechaIni , dtFechaFin ) 
		INTO cCodRet, cMensajeRet;
	return cCodRet, cMensajeRet;
	END IF;
	IF NOT EXISTS (SELECT * from sd_reporte_sispagos_sec5 
					WHERE fecha_ini_periodo	 = dtFechaIni 
					AND  fecha_fin_periodo = dtFechaFin) THEN
		EXECUTE PROCEDURE "informix".sp_reporte_syspagos5 (dtFechaIni , dtFechaFin )
		INTO cCodRet, cMensajeRet;
	END IF;
	IF NOT EXISTS	(SELECT * from sd_reporte_sispagos_sec6 
					WHERE fecha_ini_periodo	 = dtFechaIni 
					AND  fecha_fin_periodo = dtFechaFin) THEN
		EXECUTE PROCEDURE "informix".sp_reporte_syspagos6 (dtFechaIni , dtFechaFin )
		INTO cCodRet, cMensajeRet;
		return cCodRet, cMensajeRet;
	END IF;
