CREATE PROCEDURE "informix".sp_cancelacion_ctas_nunca_meses(pEmpresa CHAR(3),pMesCancelaCrd CHAR(4))
--CREATE PROCEDURE "informix".sp_cancelacion_ctas_nunca_meses(pEmpresa CHAR(3),pMesCancelaCrd CHAR(4), pOpcion CHAR(4))
...
--SET DEBUG FILE TO "/home/e10001176/Cancel_Cta_Nun/Cancel_Cta_Nun.out";
--TRACE ON;
...
END EXCEPTION;
	...
	IF  pMesCancelaCrd > 0 THEN
		-- Calcula fechas de antiguedad creditos clientes nunca
		LET dFechaMes3_Cancel = monthadd(dFechaHoy, -pMesCancelaCrd);
	ELSE
		LET iSqlErr ='11111';
		LET cCodRet = iSqlErr;
		RETURN cCodRet;
...
--	IF pOpcion = '1' THEN
		...
		FOREACH WITH HOLD
			...
			SELECT crd.num_credito, crd.numcte, crd.num_producto, crd.fecha_apertura, dos.monto_otorgado, NVL(nun.num_credito,''), crd.Sucursal, crd.divisa, dos.sdo_cap_insoluto
				...
				LEFT OUTER JOIN bdicred:sd_cancela_creds_nunca nun ON (crd.num_credito = nun.num_credito )
			...
			SELECT max(fecha_mov) INTO dFechaAux FROM bdicred:"informix".sd_movhis_calif WHERE num_credito = cNumCredito;
			
			SELECT monto, codigo_fun INTO cMonto_1, cCod_fun_1
			  FROM bdicred:"informix".sd_movhis_calif WHERE empresa = pEmpresa AND fecha_mov = dFechaAux AND num_credito = cNumCredito AND codigo_fun = '091';
						
			SELECT monto, codigo_fun INTO cMonto_2, cCod_fun_2
			  FROM bdicred:"informix".sd_movhis_calif WHERE empresa = pEmpresa AND fecha_mov = dFechaAux AND num_credito = cNumCredito AND codigo_fun = '090';
			...
			IF cCod_retBit::INTEGER != 0 THEN -- Se agrega continuacion del contador en caso de que un credito venga bloqueado para que continue el proceso
		...
		END FOREACH;
/*	ELSE
		CALL bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, "Inicia cancelacion de creditos inactivos", '02') Returning cCod_retBit;
		LET iContWork = 1;
		FOREACH WITH HOLD
	----------------------------------------------------------------------------------------------------------
			SELECT {+avoid_full (bdicred:sd_indicador_cred)}
			...
			WHERE empresa = pEmpresa  
			   AND nvl(fecha_ultima_compra, date(1)) <= dFechMes1_sms_Inac 	AND nvl(fecha_ultimo_pago, date(1)) <= dFechMes1_sms_Inac
			   AND nvl(f_primer_compra, date(1)) <= dFechMes1_sms_Inac 		AND nvl(f_primer_disp, date(1)) <= dFechMes1_sms_Inac 
			   AND nvl(atm_disp_fecha, date(1)) <= dFechMes1_sms_Inac  		AND nvl(vnt_disp_fecha, date(1)) <= dFechMes1_sms_Inac 
			   AND nvl(pos_disp_fecha, date(1)) <= dFechMes1_sms_Inac		AND comportamiento = 3	
			...
			INTO cMonto_1, cCod_fun_1
			 FROM bdicred:sd_movhis_calif 
			 WHERE empresa = pEmpresa AND fecha_mov = dFechaAux AND num_credito = cNumCredito AND codigo_fun = '091';
						
			SELECT monto, codigo_fun 
			INTO cMonto_2, cCod_fun_2
			FROM bdicred:sd_movhis_calif 
			WHERE empresa = pEmpresa AND fecha_mov = dFechaAux AND num_credito = cNumCredito AND codigo_fun = '090';
	...
	CALL bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, "Finaliza proceso de CancelaciÃÂ³n manual de cuentas inactivas y nunca", '02') Returning cCod_retBit;
	RETURN cCodRet;

END
END PROCEDURE
DOCUMENT
'Procedimiento para realizar la cancelacion de creditos inactivos y nunca manual sin realizar movimientos desde su fecha de apertura',
'Fecha: Abril 2025';
