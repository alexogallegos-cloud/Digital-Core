CREATE PROCEDURE "informix".sp_continuaenviocoppel_motor(pEmpresa CHAR(3),pNumSol CHAR(20),pStatus CHAR(1))
	------------------------------------------------------------------------------------
...
LET cStatus = "RT";
LET cCausa_sol = "RDO";
LET cMensaje= "Rechazado por estar fuera de polÃ­ticas";
...
BEGIN
	...
	--SET DEBUG FILE TO "/home/sysifx/jesusm/sp_continuaenviocoppel.out";
	--TRACE ON;
...
--       Actualiza datos de resumen fin
	...
	END IF;
	---   
	--Se agrega la ejecuciÃ³n de ActualizaciÃ³n del estatus de la solicitud para las solicitudes de Credito Coppel 
	--para que se contemplen en la reporterÃ­a del Monitor de Buro.
...
------------------------SI ES SOLICITUD MIXTA CON PRODUCTO 6001 SE EVALUA------------------------------------
		...
		IF cSolMixta <> '' AND cEnvioparametrico = '5' THEN   
				...
				IF EXISTS(SELECT numproducto from bdicred:sd_productos_motor WHERE numproducto=cProdMixto) THEN 
					IF cCanal <>'4' THEN
							INSERT INTO  "informix".ss_enviossolicitudesmotor
							(empresa, num_solicitud, num_cte, status_consumo, fecha_insert, status_respuesta)
