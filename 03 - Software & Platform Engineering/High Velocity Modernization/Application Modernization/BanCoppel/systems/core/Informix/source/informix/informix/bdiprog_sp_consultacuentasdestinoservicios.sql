CREATE PROCEDURE "informix".sp_consultacuentasdestinoservicios(p_NumCte CHAR(20), p_CvePago CHAR(2), p_Registros SMALLINT, p_CveBanco CHAR(3))
RETURNING
     CHAR(6), ---cod_ret
	 CHAR(20), ---cuenta
	 CHAR(100), ---nombre
...
BEGIN
   ...
   ON EXCEPTION
	...
	SELECT cod_ret INTO v_cod_ret FROM  BDIPROG:PP_MENSAJES WHERE cve_mensaje = "00";
    ...
    IF (p_NumCte <> "" AND p_NumCte IS NOT NULL) AND (p_CvePago <> "" AND p_CvePago IS NOT NULL)  THEN
        IF EXISTS (SELECT ct.cuenta FROM bdiprog: pp_ctasterceros ct, pp_cuentapago cp WHERE ct.num_cte = p_NumCte AND ct.cve_cuenta = cp.cve_cuenta)  THEN
            ...
            END FOREACH;
        ELSE
            SELECT cod_ret INTO v_cod_ret FROM  BDIPROG:PP_MENSAJES WHERE cve_mensaje = "13";
            RETURN v_cod_ret, NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL;
        END IF
    ELSE
        SELECT cod_ret INTO v_cod_ret FROM  BDIPROG:PP_MENSAJES	WHERE cve_mensaje = "01";
        RETURN v_cod_ret, NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL;
    END IF
    
END;
--##############################################################################
--## Procedimiento   : sp_consultacuentasdestinoservicios
--## Version         : 1.0
--## Creado por      : Adrian Lara
--## Fecha creacion  : 30 de Septiembre de 2010
