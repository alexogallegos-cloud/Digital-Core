CREATE PROCEDURE "informix".sp_calculahistcredinom(cEmpresa CHAR(3),cFecha date)
												
--------------------------------------------------------------------
--DOCUMENTACIÓN
--Calcula el cargo y abonos, para Credinómina
--Realizó: Richar 
--Fecha: 25/02/2015
--------------------------------------------------------------------													
...
--cTipoProd = 4 Credi nomina
	...
	LET vCtaCapVigPrest ='13110203010032';
	LET vCtaCapTransPrest ='13110203030032';
	LET vCtaCapNoExigPrest ='13610203010232';
	LET vCtaCapExigPrest ='13610203010132';
	LET vCtaInteresVigPrest ='13110203020032';
	LET vCtaInteresVenPrest	='13610203020132';
	LET vCtaInteresVenDOrPrest ='77106102030132';
	LET vCtaIVAInteresPrest ='14020305110332';
	LET vCtaIVAInteresOrPrest ='78376102030132';
	...
	--SET DEBUG FILE TO "sp_calculahistcredinom.out";
	--TRACE ON;	
	...
	-- MANEJADOR DE ERRORES	
		...
		let v_iDia = DAY(cFecha);
			...
			select a.num_credito,
			   sum(case when trim(c_ccmayor)||trim(c_ccsub)||trim(c_ccsubsub)||trim(c_ccsssub)||trim(c_ccssssub)||trim(c_sector) = '13110203010032' then monto else 0 end) capital_vigente_cargo,
			   sum(case when trim(a_ccmayor)||trim(a_ccsub)||trim(a_ccsubsub)||trim(a_ccsssub)||trim(a_ccssssub)||trim(a_sector) = '13110203010032' then monto else 0 end) capital_vigente_abono,
			   sum(case when trim(c_ccmayor)||trim(c_ccsub)||trim(c_ccsubsub)||trim(c_ccsssub)||trim(c_ccssssub)||trim(c_sector) = '13110203030032' then monto else 0 end) capital_transitorio_cargo,
			   sum(case when trim(a_ccmayor)||trim(a_ccsub)||trim(a_ccsubsub)||trim(a_ccsssub)||trim(a_ccssssub)||trim(a_sector) = '13110203030032' then monto else 0 end) capital_transitorio_abono,
			   sum(case when trim(c_ccmayor)||trim(c_ccsub)||trim(c_ccsubsub)||trim(c_ccsssub)||trim(c_ccssssub)||trim(c_sector) = '13610203010132' then monto else 0 end) capital_exigible_cargo,
			   sum(case when trim(a_ccmayor)||trim(a_ccsub)||trim(a_ccsubsub)||trim(a_ccsssub)||trim(a_ccssssub)||trim(a_sector) = '13610203010132' then monto else 0 end) capital_exigible_abono,
			   sum(case when trim(c_ccmayor)||trim(c_ccsub)||trim(c_ccsubsub)||trim(c_ccsssub)||trim(c_ccssssub)||trim(c_sector) = '13610203010232' then monto else 0 end) capital_no_exigible_cargo,
			   sum(case when trim(a_ccmayor)||trim(a_ccsub)||trim(a_ccsubsub)||trim(a_ccsssub)||trim(a_ccssssub)||trim(a_sector) = '13610203010232' then monto else 0 end) capital_no_exigible_abono,
			   sum(case when trim(c_ccmayor)||trim(c_ccsub)||trim(c_ccsubsub)||trim(c_ccsssub)||trim(c_ccssssub)||trim(c_sector) = '13110203020032' then monto else 0 end) interes_vigente_cargo,
			   sum(case when trim(a_ccmayor)||trim(a_ccsub)||trim(a_ccsubsub)||trim(a_ccsssub)||trim(a_ccssssub)||trim(a_sector) = '13110203020032' then monto else 0 end) interes_vigente_abono,
			   sum(case when trim(c_ccmayor)||trim(c_ccsub)||trim(c_ccsubsub)||trim(c_ccsssub)||trim(c_ccssssub)||trim(c_sector) = '14020305110332' then monto else 0 end) interes_IVA_cargo,
			   sum(case when trim(a_ccmayor)||trim(a_ccsub)||trim(a_ccsubsub)||trim(a_ccsssub)||trim(a_ccssssub)||trim(a_sector) = '14020305110332' then monto else 0 end) interes_IVA_abono,
			   sum(case when trim(c_ccmayor)||trim(c_ccsub)||trim(c_ccsubsub)||trim(c_ccsssub)||trim(c_ccssssub)||trim(c_sector) = '77106102030132' then monto else 0 end) interes_ORDEN_cargo,
			   sum(case when trim(a_ccmayor)||trim(a_ccsub)||trim(a_ccsubsub)||trim(a_ccsssub)||trim(a_ccssssub)||trim(a_sector) = '77106102030132' then monto else 0 end) interes_ORDEN_abono,
			   sum(case when trim(c_ccmayor)||trim(c_ccsub)||trim(c_ccsubsub)||trim(c_ccsssub)||trim(c_ccssssub)||trim(c_sector) = '78376102030132' then monto else 0 end) IVA_ORDEN_cargo,
			   sum(case when trim(a_ccmayor)||trim(a_ccsub)||trim(a_ccsubsub)||trim(a_ccsssub)||trim(a_ccssssub)||trim(a_sector) = '78376102030132' then monto else 0 end) IVA_ORDEN_abono--,
			...
			update statistics high for table his_cuenta;
				...
				select sum(case when v_iDia=1 then d.capvig1
							...
							sum(case when v_iDia=1 then d.intvenc1
									when v_iDia=2 then d.intvenc2
									when v_iDia=3 then d.intvenc3
									when v_iDia=4 then d.intvenc4
									when v_iDia=5 then d.intvenc5
									when v_iDia=6 then d.intvenc6
									when v_iDia=7 then d.intvenc7
									when v_iDia=8 then d.intvenc8
									when v_iDia=9 then d.intvenc9
									when v_iDia=10 then d.intvenc10
									when v_iDia=11 then d.intvenc11
									when v_iDia=12 then d.intvenc12
									when v_iDia=13 then d.intvenc13
									when v_iDia=14 then d.intvenc14
									when v_iDia=15 then d.intvenc15
									when v_iDia=16 then d.intvenc16
									when v_iDia=17 then d.intvenc17
									when v_iDia=18 then d.intvenc18
									when v_iDia=19 then d.intvenc19
									when v_iDia=20 then d.intvenc20
									when v_iDia=21 then d.intvenc21
									when v_iDia=22 then d.intvenc22
									when v_iDia=23 then d.intvenc23
									when v_iDia=24 then d.intvenc24
									when v_iDia=25 then d.intvenc25
									when v_iDia=26 then d.intvenc26
									when v_iDia=27 then d.intvenc27
									when v_iDia=28 then d.intvenc28
									when v_iDia=29 then d.intvenc29
									when v_iDia=30 then d.intvenc30
									when v_iDia=31 then d.intvenc31
								ELSE 0 END
							) as interes_vencido,
							sum(case when v_iDia=1 then d.intvenc1
									when v_iDia=2 then d.intvenc2
							...
							sum(case when v_iDia=1 then d.ivaintvig1
									when v_iDia=2 then d.ivaintvig2 
									when v_iDia=3 then d.ivaintvig3 
									when v_iDia=4 then d.ivaintvig4 
									when v_iDia=5 then d.ivaintvig5 
									when v_iDia=6 then d.ivaintvig6 
									when v_iDia=7 then d.ivaintvig7 
									when v_iDia=8 then d.ivaintvig8 
									when v_iDia=9 then d.ivaintvig9 
									when v_iDia=10 then d.ivaintvig10 
									when v_iDia=11 then d.ivaintvig11 
									when v_iDia=12 then d.ivaintvig12 
									when v_iDia=13 then d.ivaintvig13 
									when v_iDia=14 then d.ivaintvig14 
									when v_iDia=15 then d.ivaintvig15 
									when v_iDia=16 then d.ivaintvig16 
									when v_iDia=17 then d.ivaintvig17 
									when v_iDia=18 then d.ivaintvig18 
									when v_iDia=19 then d.ivaintvig19 
									when v_iDia=20 then d.ivaintvig20 
									when v_iDia=21 then d.ivaintvig21 
									when v_iDia=22 then d.ivaintvig22 
									when v_iDia=23 then d.ivaintvig23 
									when v_iDia=24 then d.ivaintvig24 
									when v_iDia=25 then d.ivaintvig25 
									when v_iDia=26 then d.ivaintvig26 
									when v_iDia=27 then d.ivaintvig27 
									when v_iDia=28 then d.ivaintvig28
									when v_iDia=29 then d.ivaintvig29
									when v_iDia=30 then d.ivaintvig30
									when v_iDia=31 then d.ivaintvig31
								ELSE 0 END
							) as iva_interes,
							sum(case when v_iDia=1 then d.ivaintvenc1
									when v_iDia=2 then d.ivaintvenc2 
									when v_iDia=3 then d.ivaintvenc3 
									when v_iDia=4 then d.ivaintvenc4 
									when v_iDia=5 then d.ivaintvenc5 
									when v_iDia=6 then d.ivaintvenc6 
									when v_iDia=7 then d.ivaintvenc7 
									when v_iDia=8 then d.ivaintvenc8 
									when v_iDia=9 then d.ivaintvenc9 
									when v_iDia=10 then d.ivaintvenc10 
									when v_iDia=11 then d.ivaintvenc11 
									when v_iDia=12 then d.ivaintvenc12 
									when v_iDia=13 then d.ivaintvenc13 
									when v_iDia=14 then d.ivaintvenc14 
									when v_iDia=15 then d.ivaintvenc15 
									when v_iDia=16 then d.ivaintvenc16 
									when v_iDia=17 then d.ivaintvenc17 
									when v_iDia=18 then d.ivaintvenc18 
									when v_iDia=19 then d.ivaintvenc19 
									when v_iDia=20 then d.ivaintvenc20 
									when v_iDia=21 then d.ivaintvenc21 
									when v_iDia=22 then d.ivaintvenc22 
									when v_iDia=23 then d.ivaintvenc23 
									when v_iDia=24 then d.ivaintvenc24 
									when v_iDia=25 then d.ivaintvenc25 
									when v_iDia=26 then d.ivaintvenc26 
									when v_iDia=27 then d.ivaintvenc27 
									when v_iDia=28 then d.ivaintvenc28
									when v_iDia=29 then d.ivaintvenc29
									when v_iDia=30 then d.ivaintvenc30
									when v_iDia=31 then d.ivaintvenc31
								ELSE 0 END
							) as interes_vencido_orden
		...
		select NVL(Sum(Case When trim(d.c_ccmayor)||trim(d.c_ccsub)||trim(d.c_ccsubsub)||trim(d.c_ccsssub)||trim(d.c_ccssssub)||trim(d.c_sector)=vCtaCapVigPrest Then a.monto else 0 End),0) as CapVig,
			   ...
			   NVL(Sum(Case When trim(d.a_ccmayor)||trim(d.a_ccsub)||trim(d.a_ccsubsub)||trim(d.a_ccsssub)||trim(d.a_ccssssub)||trim(d.a_sector)=vCtaIVAInteresOrPrest Then a.monto else 0 End),0) as IVAInteresOr_a
				into  vSdoCapVigPrest,vSdoCapVigPrest_a,vSdoCapTransPrest,vSdoCapTransPrest_a,vSdoCapNoExigPrest,vSdoCapNoExigPrest_a,vSdoCapExigPrest,vSdoCapExigPrest_a,vSdoInteresVigPrest,vSdoInteresVigPrest_a,vSdoInteresVenPrest,vSdoInteresVenPrest_a,vSdoInteresVenDOrPrest,vSdoInteresVenDOrPrest_a,vSdoIVAInteresPrest,vSdoIVAInteresPrest_a,vSdoIVAInteresOrPrest,vSdoIVAInteresOrPrest_a					  
						...
						where a.empresa = '001' and fecha_mov = cFecha and reversado = 'N' and a.num_producto = '6400';
		...
		select NVL(sum(Case When trim(d.c_ccmayor)||trim(d.c_ccsub)||trim(d.c_ccsubsub)||trim(d.c_ccsssub)||trim(d.c_ccssssub)||trim(d.c_sector)=vCtaCapVigPrest Then 1 else 0 End),0) as CapVig,
			   ...
			   NVL(sum(Case When trim(d.a_ccmayor)||trim(d.a_ccsub)||trim(d.a_ccsubsub)||trim(d.a_ccsssub)||trim(d.a_ccssssub)||trim(d.a_sector)=vCtaIVAInteresOrPrest Then 1 else 0 End),0) as IVAInteresOr_a
				...
				--Calculamos CapVig
				if exists(select * from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaCapVigPrest) then
					delete from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaCapVigPrest;
				End if;
				
				select NVL(saldo_fin_de_dia,0)
				into vMontoFinDiaAnt
				from sd_histsdodias where mes_dia=(cFecha -1) and trim(producto)=vTipProd and trim(cc)=vCtaCapVigPrest;
				...
				VALUES('001', vTipProd, vCtaCapVigPrest, cFecha, vSdoCapVigPrest, vSdoCapVigPrest_a, vSdoCapVigPrestCnt, vSdoCapVigPrest_aCnt, NVL(vMontoFinDiaAnt,0), NVL(vMontoFinDiaAct,0));
				
				LET vMontoFinDiaAnt = 0;
				
				--Calculamos CapTrans
				if exists(select * from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaCapTransPrest) then
					delete from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaCapTransPrest;
				End if;
				
				select NVL(saldo_fin_de_dia,0)
				into vMontoFinDiaAnt
				from sd_histsdodias where mes_dia=(cFecha -1) and trim(producto)=vTipProd and trim(cc)=vCtaCapTransPrest;
				
				LET vMontoFinDiaAct= vSdoCapTrans; --(vMontoFinDiaAnt + vSdoCapTransPrest) - vSdoCapTransPrest_a;
				
				INSERT INTO informix.sd_histsdodias(empresa, producto, cc, mes_dia, cargos_dia, abonos_dia, nro_cargos_dia, nro_abonos_dia, saldo_inicio_dia, saldo_fin_de_dia)
				VALUES('001', vTipProd, vCtaCapTransPrest, cFecha, vSdoCapTransPrest, vSdoCapTransPrest_a, vSdoCapTransPrestCnt, vSdoCapTransPrest_aCnt, NVL(vMontoFinDiaAnt,0), NVL(vMontoFinDiaAct,0));
				
				LET vMontoFinDiaAnt = 0;
				
				--Calculamos CapNoExig
				if exists(select * from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaCapNoExigPrest) then
					delete from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaCapNoExigPrest;
				End if;
				
				select NVL(saldo_fin_de_dia,0)
				into vMontoFinDiaAnt
				from sd_histsdodias where mes_dia=(cFecha -1) and trim(producto)=vTipProd and trim(cc)=vCtaCapNoExigPrest;
				
				LET vMontoFinDiaAct= vSdoCapNoExig; --(vMontoFinDiaAnt + vSdoCapNoExigPrest) - vSdoCapNoExigPrest_a;
				
				INSERT INTO informix.sd_histsdodias(empresa, producto, cc, mes_dia, cargos_dia, abonos_dia, nro_cargos_dia, nro_abonos_dia, saldo_inicio_dia, saldo_fin_de_dia)
				VALUES('001', vTipProd, vCtaCapNoExigPrest, cFecha, vSdoCapNoExigPrest, vSdoCapNoExigPrest_a, vSdoCapNoExigPrestCnt, vSdoCapNoExigPrest_aCnt, NVL(vMontoFinDiaAnt,0), NVL(vMontoFinDiaAct,0));

				LET vMontoFinDiaAnt = 0;
				
								
				--Calculamos CapExig
				if exists(select * from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaCapExigPrest) then
					delete from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaCapExigPrest;
				End if;
				
				select NVL(saldo_fin_de_dia,0)
				into vMontoFinDiaAnt
				from sd_histsdodias where mes_dia=(cFecha -1) and trim(producto)=vTipProd and trim(cc)=vCtaCapExigPrest;
				
				LET vMontoFinDiaAct= vSdoCapExig; --(vMontoFinDiaAnt + vSdoCapExigPrest) - vSdoCapExigPrest_a;
				
				INSERT INTO informix.sd_histsdodias(empresa, producto, cc, mes_dia, cargos_dia, abonos_dia, nro_cargos_dia, nro_abonos_dia, saldo_inicio_dia, saldo_fin_de_dia)
				VALUES('001', vTipProd, vCtaCapExigPrest, cFecha, vSdoCapExigPrest, vSdoCapExigPrest_a, vSdoCapExigPrestCnt, vSdoCapExigPrest_aCnt, NVL(vMontoFinDiaAnt,0), NVL(vMontoFinDiaAct,0));

				LET vMontoFinDiaAnt = 0;
				
				--Calculamos InteresVig
				if exists(select * from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaInteresVigPrest) then
					delete from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaInteresVigPrest;
				End if;
				
				select NVL(saldo_fin_de_dia,0)
				into vMontoFinDiaAnt
				from sd_histsdodias where mes_dia=(cFecha -1) and trim(producto)=vTipProd and trim(cc)=vCtaInteresVigPrest;
				
				LET vMontoFinDiaAct= vSdoInteresVig; --(vMontoFinDiaAnt + vSdoInteresVigPrest) - vSdoInteresVigPrest_a;
				
				INSERT INTO informix.sd_histsdodias(empresa, producto, cc, mes_dia, cargos_dia, abonos_dia, nro_cargos_dia, nro_abonos_dia, saldo_inicio_dia, saldo_fin_de_dia)
				VALUES('001', vTipProd, vCtaInteresVigPrest, cFecha, vSdoInteresVigPrest, vSdoInteresVigPrest_a, vSdoInteresVigPrestCnt, vSdoInteresVigPrest_aCnt, NVL(vMontoFinDiaAnt,0), NVL(vMontoFinDiaAct,0));

				LET vMontoFinDiaAnt = 0;
				
				--Calculamos InteresVen
				if exists(select * from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaInteresVenPrest) then
					delete from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaInteresVenPrest;
				End if;
				
				select NVL(saldo_fin_de_dia,0)
				into vMontoFinDiaAnt
				from sd_histsdodias where mes_dia=(cFecha -1) and trim(producto)=vTipProd and trim(cc)=vCtaInteresVenPrest;
				
				LET vMontoFinDiaAct= vSdoInteresVen; -- (vMontoFinDiaAnt + vSdoInteresVenPrest) - vSdoInteresVenPrest_a;
				
				INSERT INTO informix.sd_histsdodias(empresa, producto, cc, mes_dia, cargos_dia, abonos_dia, nro_cargos_dia, nro_abonos_dia, saldo_inicio_dia, saldo_fin_de_dia)
				VALUES('001', vTipProd, vCtaInteresVenPrest, cFecha, vSdoInteresVenPrest, vSdoInteresVenPrest_a, vSdoInteresVenPrestCnt, vSdoInteresVenPrest_aCnt, NVL(vMontoFinDiaAnt,0), NVL(vMontoFinDiaAct,0));

				LET vMontoFinDiaAnt = 0;
				
				--Calculamos InteresVenDOr
				if exists(select * from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaInteresVenDOrPrest) then
					delete from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaInteresVenDOrPrest;
				End if;
				
				select NVL(saldo_fin_de_dia,0)
				into vMontoFinDiaAnt
				from sd_histsdodias where mes_dia=(cFecha -1) and trim(producto)=vTipProd and trim(cc)=vCtaInteresVenDOrPrest;
				
				LET vMontoFinDiaAct= vSdoInteresVenDOr; --(vMontoFinDiaAnt + vSdoInteresVenDOrPrest) - vSdoInteresVenDOrPrest_a;
				
				INSERT INTO informix.sd_histsdodias(empresa, producto, cc, mes_dia, cargos_dia, abonos_dia, nro_cargos_dia, nro_abonos_dia, saldo_inicio_dia, saldo_fin_de_dia)
				VALUES('001', vTipProd, vCtaInteresVenDOrPrest, cFecha, vSdoInteresVenDOrPrest, vSdoInteresVenDOrPrest_a, vSdoInteresVenDOrPrestCnt, vSdoInteresVenDOrPrest_aCnt, NVL(vMontoFinDiaAnt,0), NVL(vMontoFinDiaAct,0));

				LET vMontoFinDiaAnt = 0;
				
				--Calculamos IVAIntere
				if exists(select * from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaIVAInteresPrest) then
					delete from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaIVAInteresPrest;
				End if;
				
				select NVL(saldo_fin_de_dia,0)
				into vMontoFinDiaAnt
				from sd_histsdodias where mes_dia=(cFecha -1) and trim(producto)=vTipProd and trim(cc)=vCtaIVAInteresPrest;
				
				LET vMontoFinDiaAct= vSdoIVAInteres; --(vMontoFinDiaAnt + vSdoIVAInteresPrest) - vSdoIVAInteresPrest_a;
				
				INSERT INTO informix.sd_histsdodias(empresa, producto, cc, mes_dia, cargos_dia, abonos_dia, nro_cargos_dia, nro_abonos_dia, saldo_inicio_dia, saldo_fin_de_dia)
				VALUES('001', vTipProd, vCtaIVAInteresPrest, cFecha, vSdoIVAInteresPrest, vSdoIVAInteresPrest_a, vSdoIVAInteresPrestCnt, vSdoIVAInteresPrest_aCnt, NVL(vMontoFinDiaAnt,0), NVL(vMontoFinDiaAct,0));

				LET vMontoFinDiaAnt = 0;
				
				--Calculamos IVAIntere
				if exists(select * from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaIVAInteresOrPrest) then
					delete from sd_histsdodias where mes_dia=cFecha and trim(producto)=vTipProd and trim(cc)=vCtaIVAInteresOrPrest;
				End if;
				
				select NVL(saldo_fin_de_dia,0)
				into vMontoFinDiaAnt
				from sd_histsdodias where mes_dia=(cFecha -1) and trim(producto)=vTipProd and trim(cc)=vCtaIVAInteresOrPrest;
				
				LET vMontoFinDiaAct= vSdoIVAInteresOr; --(vMontoFinDiaAnt + vSdoIVAInteresOrPrest) - vSdoIVAInteresOrPrest_a;
				
				INSERT INTO informix.sd_histsdodias(empresa, producto, cc, mes_dia, cargos_dia, abonos_dia, nro_cargos_dia, nro_abonos_dia, saldo_inicio_dia, saldo_fin_de_dia)
				VALUES('001', vTipProd, vCtaIVAInteresOrPrest, cFecha, vSdoIVAInteresOrPrest, vSdoIVAInteresOrPrest_a, vSdoIVAInteresOrPrestCnt, vSdoIVAInteresOrPrest_aCnt, NVL(vMontoFinDiaAnt,0), NVL(vMontoFinDiaAct,0));
