CREATE PROCEDURE "informix".sp_extrae_info_obligaciones() 
	Returning char(7);
...
--SET DEBUG FILE TO "/RESPALDOSNEW/aclaraciones/sp_repaltaunicaidbox.out";
 --TRACE ON;
	...
	BEGIN WORK;
		...
		/* Para incluir los pagos vencidos que corresponde a la columma  overdue_payments se insertara en la posición 
		que corresponde a la columana disputed_amount(disputed_amount) dentro del layaust Temp_Account esta entre las columnas provisioned_amount y collection_chargue
		del script's para la generación del archivo*/
		
		let vsql=  'echo "UNLOAD TO /bitacoras/cobranza/Temp_Account.txt '||
		'SELECT customer_id, identity_code, acc_customer_id, product_id, account_number, cust_branch_id, account_status, '||' '||
		'work_type,reviewed,last_entrance_date,days_delinquent,balance,delinquency,original_balance,principal_balance, '||' '||
		'interest_balance,principal_delinquent,interest_delinquent,over_limit,credit_limit,insurance_amount,collection_chargue,'||' '||
		'overdue_payments,provisioned_amount,interest_rate,cycle_day,billing_day,payment_frequency,credit_score,behaviour_score,'||' '||
		'principal_delinquency_0,principal_delinquency_1,principal_delinquency_2,principal_delinquency_3,principal_delinquency_4,'||' '||
		'principal_delinquency_5,principal_delinquency_6,delinquency_0,delinquency_1,delinquency_2,delinquency_3,delinquency_4,'||' '||
		'delinquency_5,delinquency_6,due_date_0,due_date_1,due_date_2,due_date_3,due_date_4,due_date_5,due_date_6,opening_date,'||' '||
		' wite_off_date,payment_date,prescription_date, expiration_date,last_rest_date,return_id,user_defined1,qualification,session_id,update_online  FROM ics_obligacion;">/bitacoras/cobranza/Temp_Account.sql';
		system vsql;
		let vsql= 'dbaccess bdicred /bitacoras/cobranza/Temp_Account.sql';
		system vsql;
		let vsql ='rm  /bitacoras/cobranza/Temp_Account.sql';
...
CREATE PROCEDURE "informix".sp_mostrar_grid_upgrade(pEmpresa CHAR(3),pnumsolcred CHAR(20),pflagadicional CHAR(1),pNumcte CHAR(20))
...
DEFINE dcapvencido   DECIMAL(18,2); -- ValidaciÃ³n de estatus IFRS
...
LET cTarjeta  = '';
LET cstatus= '';
LET dcapvencido = 0; -- ValidaciÃ³n de estatus IFRS
...
ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
    IF iSqlErr != 0 THEN
      LET cCodRet     = iSqlErr;
      RETURN cCodRet, NVL(cNumcredito,''),NVL(cstatus,''), NVL(cTipotarjeta,''),NVL(cNombre,''),NVL(cNomEmbozado,''),NVL(cTarjeta,''),'';
    END IF;
END EXCEPTION;

--SET DEBUG FILE TO '/informix/paulq/sp_mostrar_grid_upgrade.out';
--TRACE ON;
...
INTO cNumcredito,cstatus
FROM bdicred:sd_maecred
WHERE num_credito = pnumsolcred;

IF TRIM(NVL(cNumcredito,'')) = '' THEN
  LET cCodRet = '000002';
  --'Cuenta incorrecta/No existe'
  RETURN cCodRet, NVL(cNumcredito,''),NVL(cstatus,''), NVL(cTipotarjeta,''),NVL(cNombre,''),NVL(cNomEmbozado,''),NVL(cTarjeta,''),NVL(cNumcte,'');
END IF;

SELECT (NVL(monto_vencido,0) + NVL(mto_venc_trasp,0)) INTO dcapvencido 
FROM bdicred:"informix".sd_maesdos
WHERE num_credito = cNumcredito;

IF TRIM(NVL(cstatus,'')) NOT IN ('AA','E1') AND dcapvencido > 0 THEN -- ValidaciÃ³n de estatus IFRS
  LET cCodRet = '000003';
  --'La cuenta se encuentra en atraso por lo que no es posible realizar el cambio de producto'
  RETURN cCodRet, NVL(cNumcredito,''),NVL(cstatus,''), NVL(cTipotarjeta,''),NVL(cNombre,''),NVL(cNomEmbozado,''),NVL(cTarjeta,''),NVL(cNumcte,'');
END IF;
	...
	IF TRIM(NVL(cNumcte,'')) = '' THEN
	  LET cCodRet = '000001';
	  --'Error en el nÃÂºmero de cliente';
	  RETURN cCodRet, NVL(cNumcredito,''),NVL(cstatus,''), NVL(cTipotarjeta,''),NVL(cNombre,''),NVL(cNomEmbozado,''),NVL(cTarjeta,''),NVL(cNumcte,'');
	END IF;
		...
		TRIM(nvl(a.apell_paterno,'')),
		TRIM(nvl(a.apell_materno,''))
		INTO cNombre,cNombre1,cNombre2,capell_paterno,capell_materno
		FROM bdinteg:"informix".si_cliente a
		WHERE a.numcte = pNumcte;

		IF cNombre2 = '' AND capell_materno <> '' THEN
			LET cNomEmbozado = TRIM(nvl(cNombre1,' ')) ||'  '|| TRIM(nvl(capell_paterno,' ')) ||' '|| SUBSTR(TRIM(nvl(capell_materno,' ')),1,1);
		ELIF cNombre2 = '' AND capell_materno = '' THEN
			LET cNomEmbozado = TRIM(nvl(cNombre1,' ')) ||'  '|| TRIM(nvl(capell_paterno,' '));
		ELSE
			IF capell_materno = '' THEN
				LET cNomEmbozado = SUBSTR (TRIM(nvl(cNombre1,' ')),1,1) ||' '|| TRIM(nvl(cNombre2,' '))  ||' '|| TRIM(nvl(capell_paterno,' '));
			ELSE
				LET cNomEmbozado = SUBSTR (TRIM(nvl(cNombre1,' ')),1,1) ||' '|| TRIM(nvl(cNombre2,' '))  ||' '|| TRIM(nvl(capell_paterno,' ')) ||' '|| SUBSTR(TRIM(nvl(capell_materno,' ')),1,1);
			END IF;
		END IF;
		LET nrows = nrows +1;
		RETURN cCodRet, NVL(cNumcredito,''),NVL(cstatus,''), NVL(cTipotarjeta,''),NVL(cNombre,''),NVL(cNomEmbozado,''),NVL(cTarjeta,''),NVL(pNumcte,'') ;
	END IF;

	FOREACH WITH HOLD
		SELECT a.num_credito, case when a.tipo_tarjeta = 'T' then 'TIT' when a.tipo_tarjeta = 'A' then 'ADI' else ' ' end, a.numcte ,a.num_tarjeta
		INTO cNumcredito, cTipotarjeta,cNumcte,cTarjeta
		FROM bdicred:"informix".sd_tarjeta a
		WHERE a.empresa = pEmpresa
		AND a.num_credito = pnumsolcred
		AND a.tipo_tarjeta  = CASE WHEN pflagadicional = '1' THEN  'T'  ELSE a.tipo_tarjeta END
		AND a.status_tar IN('A')
        --AND a.secuencia IN (select MAX(secuencia) FROM bdicred:sd_tarjeta WHERE num_credito = pnumsolcred)
		...
		END IF;
		LET nrows = nrows +1;
		RETURN cCodRet, NVL(cNumcredito,''),NVL(cstatus,''), NVL(cTipotarjeta,''),NVL(cNombre,''),NVL(cNomEmbozado,''),NVL(cTarjeta,''),NVL(cNumcte,'') WITH RESUME;
	END FOREACH;


	IF nrows  = 0 THEN
	   LET cCodRet= '000004';
	   --'No hay datos con la informaciÃÂ³n indicada'
	   RETURN cCodRet, NVL(cNumcredito,''),NVL(cstatus,''), NVL(cTipotarjeta,''),NVL(cNombre,''),NVL(cNomEmbozado,''),NVL(cTarjeta,''),NVL(cNumcte,'');
	END IF;
...
END PROCEDURE
DOCUMENT
'Se realiza procedimiento para consultar los datos a mostrar en el grid de selecciÃÂ³n del upgrade de un producto ',
'AUTOR : Maria Elena Angulo Aispuro',
'FECHA : 01/03/2016',
'BD    : BDICRED';

CREATE PROCEDURE "informix".sp_mostrar_grid_upgrade2(pEmpresa CHAR(3),pnumsolcred CHAR(20),pflagadicional CHAR(1),pNumcte CHAR(20),pRegistros INTEGER, pRecuperacion INTEGER)
RETURNING CHAR(6)         AS codigo_retorno,
		  CHAR(20)		  AS cNumcredito,
		  CHAR(2)		  AS StatusCred,
		  CHAR(3) 		  AS Tipo_Tarjeta,
...
--SET DEBUG FILE TO '/tmp/mfinis/sp_mostrar_grid_upgrade2.out';
--TRACE ON;
...
END IF;
	...
	IF NVL(pNumcte,'') <> '' THEN
		...
		IF TRIM(NVL(cNumcte,'')) = '' THEN
		  LET cCodRet = '000001';
		  --'Error en el nÃºmero de cliente';
		  RETURN cCodRet, NVL(cNumcredito,''),NVL(cstatus,''), NVL(cTipotarjeta,''),NVL(cNombre,''),NVL(cNomEmbozado,''),NVL(cTarjeta,''),NVL(cNumcte,'');
		END IF;
	...
	FOREACH WITH HOLD
		SELECT SKIP pRegistros FIRST pRecuperacion a.num_credito, case when a.tipo_tarjeta = 'T' then 'TIT' when a.tipo_tarjeta = 'A' then 'ADI' else ' ' end, a.numcte ,a.num_tarjeta
		INTO cNumcredito, cTipotarjeta,cNumcte,cTarjeta
		FROM bdicred:"informix".sd_tarjeta a
		WHERE a.empresa = pEmpresa
		AND a.num_credito = pnumsolcred
	...
	IF nrows  = 0 THEN
	   LET cCodRet= '000004';
	   --'No hay datos con la informaciÃ³n indicada'
	   RETURN cCodRet, NVL(cNumcredito,''),NVL(cstatus,''), NVL(cTipotarjeta,''),NVL(cNombre,''),NVL(cNomEmbozado,''),NVL(cTarjeta,''),NVL(cNumcte,'');
	END IF;
...
'FECHA 30/03/2017',
'DESCRIPCION: Se crea SPL clon para el tratado de la paginaciÃ³n.',
'BD: bdicred';
