CREATE PROCEDURE "informix".sp_histmovcheqcomp10(pempresa CHAR(3))
RETURNING CHAR(5)  AS vcodret1, 
    ...
    ON EXCEPTION
        ...
        SET DEBUG FILE TO "/resplogifx/conciliachq/sp_histmovcheqcomp10.err";
        TRACE ON;
        IF sql_err <> 0 THEN
    ...
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/sp_histmovcheqcomp10.out";
    --- TRACE ON;
...
*/	 
	...
	SELECT valor 
      ...
      FROM sc_param
     WHERE empresa = pempresa
	   AND codparam = 'CtaIniRepHisChqComp3';
	   
    SELECT valor 
      INTO vcuenta_fin
      FROM sc_param
     WHERE empresa = pempresa
       AND codparam = 'CtaIniRepHisChqComp4';
    
    LET vfecha = TO_CHAR(vfecha_ant, '%d%m%Y');
    
    -- // DESCARGA MOVIMIENTOS POS - BANCO
    LET vsql = 'echo "SET ISOLATION TO DIRTY READ; UNLOAD TO /resplogifx/conciliachq/histmovcheq_aplicados_POS10_'||vfecha||'.txt '||
			   ...
			   'AND mov.cuenta >= '''||vcuenta_ini||''' AND mov.cuenta < '''||vcuenta_fin||''';" > /resplogifx/conciliachq/histmovcheq_POS10.sql';
			   
    SYSTEM vsql;
    
    LET vstmt = "/ifxsif01/bin/dbaccess bdicheq /resplogifx/conciliachq/histmovcheq_POS10.sql"; 
    SYSTEM vstmt;
