CREATE PROCEDURE "informix".sp_alertacargospei_pba(pempresa CHAR(3))
RETURNING CHAR(5);
    ...
    LET vfecha_ant2 = (vfecha_ant - 4 UNITS DAY);
    LET vprox_fecha = (vfecha_hoy + 4 UNITS DAY);
    ...
    FOREACH WITH HOLD
        ...
        SELECT {+INDEX(bdicheq:sc_movdia idx_movdiamspei)}
         ...
         WHERE transacc = '0274'
           AND fech_val BETWEEN vfecha_hoy AND vprox_fecha
           AND cancelad <> 'S'
        ...
        IF vexiste_movdia = 0 THEN
            SELECT COUNT(*)
             ...
             WHERE fech_alt BETWEEN vfecha_ant2 AND vfecha_ant
               AND transacc = '0274'
               AND cancelad <> 'S'
               AND fech_val BETWEEN vfecha_ant2 AND vfecha_ant
               AND referencia = vclave_rastreo;
        END IF;
