CREATE PROCEDURE "informix".cobrauto(c_empresa CHAR(3),
			  						 c_fecha   DATE,
			  						 c_usuario CHAR(8))
...
DEFINE v_capvig     MONEY(14,2);
DEFINE v_capven     MONEY(14,2);
DEFINE v_intvig     MONEY(14,2);
DEFINE v_intven     MONEY(14,2);
DEFINE v_intmor     MONEY(14,2);
DEFINE v_seg        MONEY(14,2);
DEFINE v_comis      MONEY(14,2);
DEFINE v_credito    CHAR(20);
DEFINE v_cuenta     CHAR(20);
DEFINE v_contador   SMALLINT;
DEFINE v_contadors  CHAR(2);
DEFINE v_folio      CHAR(16);
DEFINE v_parcial    CHAR(1);
DEFINE v_val        VARCHAR(50);
DEFINE v_adeudo     MONEY(14,2);
DEFINE v_sdocta     MONEY(14,2);
DEFINE v_mtopago    MONEY(14,2);
DEFINE v_tranpag    CHAR(4);
DEFINE si_reg       SMALLINT;
...
-- ****************************************************************************
	...
	SELECT NVL(valor,"N") INTO v_parcial
	  FROM sd_param
	 WHERE cod_param = "73"
	   AND empresa = c_empresa;

	SELECT NVL(valor,"0000") INTO v_tranpag
	  FROM sd_param
	 WHERE cod_param = "74"
	   AND empresa = c_empresa;

	IF v_tranpag = "0000" THEN
		LET scod_ret = "100";
		RETURN scod_ret;
	END IF
	...
	FOREACH WITH HOLD 
		SELECT UNIQUE a.num_credito, b.num_cta,
	              (select sum((z.monto_cuota-z.monto_real_pag) + 
     				  (x.monto_cuota-x.monto_real_pag))
         	         from sd_pagocapit z, sd_paginter x, sd_fechas
         		Where z.num_credito = x.num_credito
         	          and z.fecha_Cuota = x.fecha_cuota
         		  and z.fecha_cuota <=fecha_hoy
         		  and z.num_credito = a.num_credito
         		  and z.status_cuota <> '5')
		  INTO v_credito, v_cuenta, v_adeudo
		  FROM sd_pagocapit a, sd_ctascarg b
		 WHERE fecha_cuota <= c_fecha
		   AND status_cuota NOT IN ("5", "8")
		   AND a.num_credito = b.num_credito 
		   AND NOT EXISTS (SELECT * FROM sd_cobrauto g 
				    WHERE g.num_credito = a.num_credito 
                                      AND codret ='000' 
				      AND fecha_aplica = c_fecha)

	
		SELECT COUNT(*) INTO si_reg
		  FROM sd_cobrauto
		 WHERE num_credito = v_credito
		   AND fecha_aplica = c_fecha; 

		IF si_reg > 0 THEN
			UPDATE sd_cobrauto SET codret = "000"
		 	 WHERE num_credito = v_credito
		   	   AND fecha_aplica = c_fecha; 
		ELSE
			INSERT INTO sd_cobrauto
		 	 VALUES (c_fecha, v_credito, " ", 0, "000");
		END IF	

		{EXECUTE PROCEDURE con_adeupagoofi(v_credito, c_empresa)
		   INTO scod_ret, v_val, v_val, v_val, v_val, v_val,
			v_capvig, v_intvig, v_capven, v_intven, v_intmor,
			v_seg, v_comis, v_val, v_val, v_val, v_val, v_val;


		LET v_adeudo = v_capvig + v_intvig + v_capven + v_intven +
			       v_intmor + v_seg + v_comis;}

		--RQM 09 704. Se agrega el llamado al SP de consulta de saldo con el tipo de calculo requerido para esta operacion. EEAP
        EXECUTE PROCEDURE bdicheq:sp_cons_sdodisp_x_tpcalculo(v_cuenta, null, null, null, null, null, null, null, 'T', 2) INTO cCodRetConsSdo,cMensajeRetConsSdo,v_sdocta;

		IF v_sdocta < 0 THEN
			UPDATE sd_cobrauto SET codret ="105"
			 WHERE num_credito = v_credito
			   AND fecha_aplica = c_fecha;
			CONTINUE FOREACH;
		END IF
		...
		IF v_contador = 100 then
			LET v_contador = 1;
		END IF
		LET v_contadors = v_contador;
		IF LENGTH(v_contadors) = 1 THEN 
			LET v_contadors = "0" || TRIM(v_contadors);
		END IF

		LET v_folio = "cobrauto" || 
			      SUBSTR(CURRENT HOUR TO FRACTION(2),1,2) ||
			      SUBSTR(CURRENT HOUR TO FRACTION(2),4,2) ||
			      SUBSTR(CURRENT HOUR TO FRACTION(2),7,2) ||
			      v_contadors;
					

		IF v_adeudo <= v_sdocta THEN

			EXECUTE PROCEDURE principal
				(c_empresa, v_credito, "2", v_adeudo, c_usuario,
				 "001", v_folio, "0000")
			   INTO v_val, v_val, v_val, v_val, v_val, v_val,
				v_val, v_val, v_val, scod_ret, v_val;
			LET v_mtopago = v_adeudo;
		ELSE
		   IF v_parcial = "S" THEN
			EXECUTE PROCEDURE principal
				(c_empresa, v_credito, "2", v_sdocta, c_usuario,
			    	 "001", v_folio, "0000")
			   INTO v_val, v_val, v_val, v_val, v_val, v_val,
				v_val, v_val, v_val, scod_ret, v_val;

			LET v_mtopago = v_sdocta;
		   ELSE
			UPDATE sd_cobrauto SET codret = "105"
			 WHERE num_credito = v_credito
	 	 	   AND fecha_aplica = c_fecha;
		...
		IF scod_ret = "000" THEN
			EXECUTE PROCEDURE bdicheq:cargo_ref
				(c_empresa,"001", c_usuario,v_tranpag, "0000",
				 v_folio, v_cuenta, 0, v_mtopago,"01"," ")
			   INTO scod_ret, v_val, v_val, v_val, v_val;
			
			IF scod_ret <> "000" THEN
				EXECUTE PROCEDURE reversion
					(c_empresa,"001",c_usuario, v_folio,
					 "1")
				   INTO scod_ret;
				IF scod_ret <> "000" THEN
					UPDATE sd_cobrauto SET codret = s_codret
			 		 WHERE num_credito = v_credito
		 	   		   AND fecha_aplica = c_fecha;
					CONTINUE FOREACH;
				END IF
		...
		UPDATE sd_cobrauto SET codret = scod_ret,
				       folio = v_folio,
				       monto_pago = v_mtopago
		 WHERE num_credito = v_credito
		   AND fecha_aplica = c_fecha;
...
CREATE PROCEDURE "informix".trasp_quiebra(x_empresa CHAR(3),
			       x_numcred CHAR(20),
			       x_folio   CHAR(16),
			       x_fecha   DATE,
			       x_mtopag  MONEY(14,2),
			       x_intvig  MONEY(14,2),
			       x_intvenc MONEY(14,2),
			       x_inttras MONEY(14,2),
			       x_mora    MONEY(14,2),
			       x_tpact   CHAR(1))
RETURNING CHAR(5), CHAR(50),MONEY(14,2),MONEY(14,2),MONEY(14,2),MONEY(14,2);
...
-- ****************************************************************************
   ...
   DEFINE vsqlerr             INTEGER;
   DEFINE s_mensaje           CHAR(50);
   DEFINE v_st                CHAR(1);
   DEFINE v_sdo               MONEY(14,2);
   DEFINE s_sdovencido        MONEY(14,2);
   DEFINE s_sdotrasp          MONEY(14,2);
   DEFINE s_sdodia            MONEY(14,2);
   DEFINE s_sdomora           MONEY(14,2);
   DEFINE v_fecha	      DATE;
   DEFINE v_sdo2	      MONEY(14,2);
   DEFINE v_prod 	      CHAR(4);
   DEFINE v_hoy 	      DATE;
   DEFINE v_sdorem            MONEY(14,2);
   DEFINE v_suc               CHAR(4);
   DEFINE v_divisa	      CHAR(2);
   DEFINE v_begin             CHAR(1);
   DEFINE v_cta		      CHAR(20);
   DEFINE v_rem		      MONEY(14,2);
   DEFINE v_inm		      MONEY(14,2);
   DEFINE v_ive		      MONEY(14,2);
   DEFINE v_cve		      MONEY(14,2);
   DEFINE v_ivi		      MONEY(14,2);
   DEFINE v_cvi		      MONEY(14,2);
   DEFINE v_imp		      MONEY(14,2);
   DEFINE v_com		      MONEY(14,2);
   DEFINE v_seg		      MONEY(14,2);
   DEFINE x_paso              MONEY(14,2);
...
LET s_codret     = "00000";
LET vsqlerr      = 0;
LET g_NumCredito = x_numcred;
LET g_Empresa    = x_empresa;
LET g_Folio      = x_folio;
LET s_mensaje    = "***************";
LET s_sdovencido = 0;
LET s_sdotrasp   = 0;
...
ON EXCEPTION SET vsqlerr
   IF vsqlerr != 0 THEN
      ...
      END IF 
      LET s_mensaje =" ";
      RETURN s_codret, s_mensaje, s_sdovencido, s_sdotrasp, s_sdodia,s_sdomora;
   END IF;
END EXCEPTION;
...
-- ****************************************************************************
	SELECT num_producto, sucursal, divisa
	  INTO v_prod, v_suc, v_divisa
	  FROM sd_maecred 
	 WHERE num_credito = x_numcred
	   AND empresa = x_empresa;


	IF x_tpact = "1" THEN -- Respaldo de credito antes de trat de quiebra
		BEGIN WORK;
		LET v_begin="S";
		EXECUTE PROCEDURE respaldacredito() INTO s_codret;
		IF s_codret <> "000" THEN
			ROLLBACK WORK;
			LET s_mensaje = "Error en Respaldo de Credito";
			RETURN s_codret, s_mensaje, s_sdovencido, s_sdotrasp,
			       s_sdodia,s_sdomora;
		ELSE 
			COMMIT WORK;
			LET s_mensaje = "Proceso Existoso";
			RETURN s_codret, s_mensaje, s_sdovencido, s_sdotrasp,
			       s_sdodia,s_sdomora;
		END IF
	END IF

	IF x_tpact = "2" THEN -- Aplica pago con Monto de Haberes y Bloque Ctas
		EXECUTE PROCEDURE principal(x_empresa, x_numcred, "1", 
					    x_mtopag, SUBSTR(x_folio,1,8), 
					    v_suc, x_folio, "0000")
		  INTO v_rem, v_inm, v_ive, v_cve, v_ivi, v_cvi, v_imp, v_com,
		        v_seg, s_codret, s_mensaje;

		IF s_codret <> "000" THEN
		   RETURN s_codret, s_mensaje, s_sdovencido, 
		          s_sdotrasp, s_sdodia,s_sdomora;
		END IF

		----RQM 09 704. Se realiza modificacion a la formula del saldo disponible se le agrega el saldo sbc OACM 
		BEGIN WORK;
		LET v_begin="S";
		FOREACH WITH HOLD 
			SELECT cuenta, sdo_actual - sdo_retenido - sdo_cong - saldo_sbc
			  INTO v_cta, v_sdo
			  FROM bdicheq:sc_maechq a, sd_maecred b
			 WHERE b.num_credito = x_numcred
			   AND b.empresa = x_empresa
			   AND a.num_cte = b.numcte

			IF v_sdo > 0 THEN 
				EXECUTE PROCEDURE 
				    bdicheq:bloqueo_cta(x_empresa, v_cta, v_sdo,
						"10", v_hoy, 
					        SUBSTR(x_folio,1,8)," ")
				  INTO s_codret, s_mensaje;
				IF s_codret <> "000" THEN
				   ROLLBACK WORK;
			           RETURN s_codret, s_mensaje, s_sdovencido, 
				   s_sdotrasp, s_sdodia,s_sdomora;
	...
	IF x_tpact = "3" THEN -- Calcula Aduedo de acuerdo a fecha quiebra
		--Determina Vigente al momento de la Quiebra
                SELECT sdo_capital * (tasa_interes/100) / 360 *
                        (((a.fecha_cuota + 1) - x_fecha) * -1)
                     - (SELECT sum(monto_real_pag)
                          FROM sd_paginter x
                         WHERE x.num_credito = x_numcred
                           AND x.empresa = x_empresa
                           AND x.fecha_cuota >= x_fecha)
                 INTO s_sdodia
                 FROM sd_pagocapit a, sd_maecred c, sd_maesdos d
                WHERE a.num_credito = x_numcred
                  AND a.empresa = x_empresa
                  AND a.fecha_cuota =  (SELECT max(fecha_cuota)
                                  FROM sd_pagocapit s, sd_fechas d
                                 WHERE s.num_credito = x_numcred
                                           AND s.empresa = x_empresa
                                   AND fecha_cuota <= x_fecha)
                ...
                IF s_sdodia < 0 or s_sdodia IS NULL THEN
                        LET s_sdodia = 0;
               END IF
			 ...
			 WHERE num_credito = x_numcred
			  ...
			  IF v_st = "7" THEN
				LET s_sdovencido = v_sdo;
			  ELSE
				LET s_sdotrasp   = v_sdo;
			  END IF

		END FOREACH

		SELECT status_cuota INTO v_st
		  FROM sd_paginter 
		 WHERE num_credito = x_numcred
		   AND empresa = x_empresa
		   AND fecha_cuota = (SELECT MIN(fecha_cuota) FROM sd_paginter
					 ...
					 AND fecha_cuota > x_fecha);
		IF v_st = "7" THEN
			LET s_sdovencido = s_sdovencido + s_sdodia;
			LET s_sdodia = 0;
		ELIF v_st = "2" THEN
			LET s_sdotrasp = s_sdotrasp + s_sdodia;
			LET s_sdodia = 0;
		END IF

		SELECT NVL(SUM(sdo_mora_ordi),0) INTO s_sdomora 
		  FROM sd_detmora 
		 WHERE num_credito = x_numcred
		   AND empresa = x_empresa
	 	   AND fecha_cuota <= x_fecha;
	...
	IF x_tpact = "4" THEN -- Realiza el Ajuste  a Saldos 
		BEGIN WORK;
		LET v_begin="S";
		IF x_intvenc = 0 AND x_inttras > 0 THEN
		   SELECT SUM(monto_financiado) INTO v_sdorem
		     FROM sd_paginter
		    WHERE num_credito = x_numcred
		      AND empresa = x_empresa
		   ...
		   LET x_paso = x_inttras;

                   FOREACH WITH HOLD
                        SELECT fecha_cuota, monto_cuota, monto_real_pag
                          INTO v_fecha, v_sdo, v_sdo2
                          FROM sd_paginter
                         WHERE num_credito = x_numcred
                           AND empresa = x_empresa
                           AND status_cuota = "2"
                        ...
                        IF v_sdo > x_paso THEN
                                UPDATE sd_paginter SET monto_cuota =
                                                       x_paso
                                 WHERE num_credito = x_numcred
                                   AND empresa = x_empresa
                                   AND fecha_cuota = v_fecha;
				 LET x_paso = 0;
                        ELIF v_sdo <= x_paso THEN
                                LET x_paso = x_paso - v_sdo;
                        ELIF x_paso = 0 THEN
                                LET v_sdorem = v_sdorem + v_sdo;
                                UPDATE sd_paginter SET monto_cuota = 0,
                                                       status_cuota ="1"
                                 WHERE num_credito = x_numcred
                                   AND empresa = x_empresa
                        ...
                        END IF
                        IF v_sdorem > 0 THEN
                           EXECUTE PROCEDURE genmov(x_empresa, x_numcred,
                                                    v_prod, "1", "571",
                                                    v_hoy, v_sdorem,
                                                    x_folio, v_suc,
                                                    v_divisa, "0000")
		...
		IF x_intvig = 0 AND x_intvenc > 0 THEN
		   SELECT SUM(monto_financiado) INTO v_sdorem
		     FROM sd_paginter
		    WHERE num_credito = x_numcred
		      AND empresa = x_empresa
		...
		END IF 
		IF x_intvig > 0 THEN
			SELECT fecha_cuota, monto_cuota, monto_financiado
			  INTO v_fecha, v_sdo, v_sdo2
			  FROM sd_paginter
			 WHERE num_credito = x_numcred
			   AND empresa = x_empresa
			   AND fecha_cuota = (SELECT MIN(fecha_cuota)
						FROM sd_paginter
					       WHERE num_credito = x_numcred
						 AND status_cuota = "1");
			IF x_intvig < v_sdo2 THEN
			   LET v_sdorem = v_sdo2 - x_intvig;
                           EXECUTE PROCEDURE genmov(x_empresa, x_numcred,
                                                    v_prod, "1", "571",
                                                    v_hoy, v_sdorem,
                                                    x_folio, v_suc,
			    ...
			    END IF
			END IF
			UPDATE sd_paginter SET monto_cuota = x_intvig
			 WHERE num_credito = x_numcred
			   AND empresa = x_empresa
			   AND fecha_cuota = v_fecha;
		...
		UPDATE sd_maesdos SET mto_venc_int = x_intvenc,
				      mto_venc_tra_int = x_inttras,
				      sdo_exig_int = x_intvenc + x_inttras,
				      sdo_no_exig = x_intvig
		 WHERE num_credito = x_numcred
		   AND empresa = x_empresa;
...
END
	RETURN s_codret, s_mensaje, s_sdovencido, s_sdotrasp, s_sdodia,
	       s_sdomora;

END PROCEDURE
Document
'DESCRIPCION DE MODIFICACION:  Se realiza modificacion a la formula del saldo disponible se le agrega el saldo sbc OACM', 
'AUTOR: OSIEL ALFREDO CAMACHO MENDOZA ',
'FECHA: 09 de JULIO DEL 2025',
'VERSION: 20250709',
'BD: BDICRED';

CREATE PROCEDURE "informix".sp_cobro_automatico_tc(pempresa CHAR(3), pturno CHAR(1))
...
DEFINE v_codigo_retorno				CHAR(4);
DEFINE v_mensaje_retorno			CHAR(80);
DEFINE v_pago_minimo				DECIMAL(18,2);
DEFINE v_intvdo						DECIMAL(18,2);
DEFINE v_intmoratorio				DECIMAL(18,2);
DEFINE v_ivaintvdo					DECIMAL(18,2);
DEFINE v_pagosvdos					DECIMAL(18,2);
DEFINE v_ivaintmoratorio			DECIMAL(18,2);
DEFINE v_intmes						DECIMAL(18,2);
DEFINE v_ivaintmes					DECIMAL(18,2);
DEFINE v_intvig 					DECIMAL(18,2);
DEFINE v_ivaintvig					DECIMAL(18,2);
DEFINE g_campo_trab3				CHAR(10);
DEFINE vlFechaBaja					DATE;
DEFINE dtfechaUltDia				DATE;
DEFINE v_flag_aplica_pago			CHAR(1);
DEFINE v_existeM1                   INTEGER;


--------------------------- bdicred:sd_definicion -----------------------------------
DEFINE v_dia_cuota 					DECIMAL(2,0); 
DEFINE v_dias_cobro_aut				DECIMAL(2,0); 

----------------------------- bdinteg:si_feriado  -----------------------------------
DEFINE v_id_dia_laborable			CHAR(1); 
DEFINE v_flag_dia_laborable			CHAR(1); 

----------------------- Datos General --------------------------------
DEFINE g_Empresa            CHAR(3);
DEFINE g_NumCred            CHAR(20);
DEFINE g_Folio              CHAR(16);
DEFINE g_CuentaCap          CHAR(20);
DEFINE g_SdoCta             DECIMAL(14,2);
DEFINE g_StatusCtaCap       CHAR(1);

DEFINE v_status_proceso		CHAR(2);
DEFINE g_status_cred 		CHAR(2);
DEFINE g_prox_fecha_pago 	DATE;
DEFINE g_monto_vencido 		DECIMAL(18,2);
DEFINE g_mto_venc_trasp		DECIMAL(18,2);
DEFINE g_monto_financiado 	DECIMAL(18,2);
DEFINE g_sdo_actual_disp 	DECIMAL(18,2);
...
LET g_status_cred 		= '';
LET v_status_proceso 		= '';
LET g_prox_fecha_pago 	= '';
LET g_monto_vencido 	= 0;
LET g_mto_venc_trasp 	= 0;
LET g_monto_financiado 	= 0;
LET g_sdo_actual_disp 	= 0;
-------------------------------------------------------------------------------------
...
LET cMensajeRet           = "";
LET cMensaje           	  = "Se realiza el Proceso correctamente";
...
LET v_referencia		  = '';
LET v_pago_minimo 		  = 0;
LET v_id_dia_laborable	  = '';
LET v_flag_dia_laborable  = '0';
LET v_flag_aplica_pago	  = '0';
LET vcprocesoM1           ='';
LET v_existeM1            = 0;

-- : TURNO DE EJECUCION DEL PROCESO (M = Matutino, N = Nocturno)
LET vcproceso = 'CobroautoTC'|| '-' || trim(pturno);

--SET DEBUG FILE TO "/resplogifx/archivoscredito/sp_cobro_automatico_tc.out";
--TRACE ON;

    set isolation to dirty read;
    SET LOCK MODE TO WAIT 3;

    select fecha_hoy , last_day(fecha_hoy)
      into dtFechaHoy, dtfechaUltDia
...
END EXCEPTION;
	...
	SELECT laborable
	  INTO v_id_dia_laborable
	  FROM bdinteg:si_feriado 
	 WHERE fecha = dtFechaHoy;
	 
	IF v_id_dia_laborable IS NULL OR v_id_dia_laborable = '' OR v_id_dia_laborable = 'S' THEN
			LET v_flag_dia_laborable = '1';
	END IF;
	 
	SELECT dia_cuota, dias_cobro_aut
	  INTO v_dia_cuota, v_dias_cobro_aut
	  FROM bdicred:sd_definicion
	 WHERE empresa = pempresa 
	   AND num_producto = '8500';
		
	IF MONTH(dtfechaUltDia) = 2 THEN 
			LET v_dias_cobro_aut = DAY(dtfechaUltDia);
	END IF;
		
	IF DAY(dtFechaHoy) >= v_dia_cuota AND DAY(dtFechaHoy) < v_dias_cobro_aut THEN
			LET v_flag_aplica_pago = '0';
		ELSE
			LET v_flag_aplica_pago = '1';
	END IF;		
  ...
  SELECT status_proc 
   ...
   WHERE fecha= dtFechaHoy 
	...
	IF v_flag_dia_laborable = '1' THEN

		LET g_Empresa = pempresa;
		
			DROP TABLE IF EXISTS temp_cobro_aut_TC;
			
			CREATE TABLE temp_cobro_aut_TC (
											num_credito char(20), 
											status_cred char (2),
											campo_trab3 char(10), 
											monto_vencido decimal(18,2), 
											mto_venc_trasp decimal(18,2), 
											monto_financiado decimal(18,2), 
											num_cta char(20),
											sdo_actual decimal(18,2),
											sdo_retenido decimal(18,2),
											sdo_actual_disp decimal(18,2)
											); 
		
			IF v_flag_aplica_pago = 0 THEN
						INSERT INTO temp_cobro_aut_TC 
						SELECT {+INDEX (sd_maecred)} 
								a.num_credito,a.status_cred,a.campo_trab3,
								b.monto_vencido,b.mto_venc_trasp,b.monto_financiado,
								d.num_cta,
								e.sdo_actual,e.sdo_retenido, e.sdo_actual - (e.sdo_retenido + e.sdo_cong + e.imp_sbg_ccc + e.saldo_sbc) AS sdo_actual_disp
						FROM 	bdicred:"informix".sd_maecred a, 
								bdicred:"informix".sd_maesdos b, 
								bdicred:"informix".sd_maecredanexo c, 
								bdicred:"informix".sd_ctascarg d, 
						...
						WHERE 	a.empresa       = '001'
							...
							AND e.sdo_actual - e.sdo_retenido > 0;
				ELSE 
						INSERT INTO temp_cobro_aut_TC 
						SELECT {+INDEX (sd_maecred)} 
								a.num_credito,a.status_cred,a.campo_trab3,
						...
						FROM 	bdicred:"informix".sd_maecred a, 
								...
								bdicheq:"informix".sc_maechq e
						WHERE 	a.empresa       = '001'
							AND a.status_cred   NOT IN ('FF','FC','CV','FI')
							AND b.empresa       = a.empresa
							AND b.num_credito   = a.num_credito
							AND c.num_credito   = b.num_credito
							AND c.empresa       = b.empresa
			...
			FOREACH WITH HOLD
				...
				SELECT num_credito, status_cred, campo_trab3, monto_vencido, mto_venc_trasp, monto_financiado, num_cta, sdo_actual_disp
				INTO g_NumCred, g_status_cred, g_campo_trab3, g_monto_vencido, g_mto_venc_trasp, g_monto_financiado, g_CuentaCap, g_sdo_actual_disp
				FROM temp_cobro_aut_TC
				
					--SET DEBUG FILE TO "/informix/resplogifx/ggguadarrama/sp_cobro_automatico_TC_"||LPAD(TRIM(g_NumCred),12,'0')|| ".out";
					--TRACE ON;
					...
					EXECUTE PROCEDURE  bdicred:"informix".sp_obtener_pagomin(g_Empresa,g_NumCred)
					INTO 	v_codigo_retorno,v_mensaje_retorno,v_pago_minimo,v_intvdo,v_intmoratorio,v_ivaintvdo,v_pagosvdos,
							v_ivaintmoratorio,v_intmes,v_ivaintmes,v_intvig,v_ivaintvig;
					
					IF v_flag_aplica_pago = 0 THEN
						LET g_monto_financiado = g_monto_financiado - g_monto_vencido - g_mto_venc_trasp;
						LET v_pago_minimo = v_pago_minimo - g_monto_financiado;
					END IF
					
					IF v_pago_minimo > 0 THEN
						LET v_referencia = 'CRG. CTA. ';
									
						-- Se obtiene el saldo de la cuenta identificada.
						CALL bdicheq:"informix".cons_saldo(g_CuentaCap) RETURNING cCodRetAux,g_SdoCta,g_StatusCtaCap;
														
						SELECT SUBSTR(g_NumCred,3,10)||SUBSTR(CURRENT,12,2)||SUBSTR(CURRENT,15,2)||SUBSTR(CURRENT,18,2)
						INTO g_Folio
						FROM dual;
										
						IF g_SdoCta <= 0 THEN
								
								LET cCodRetAux = '00400'; --- FONDOS INSUFICIENTES
								LET cCodRetCtrl = '00000'; --- SIN AFECTACION EN CREDITO
								LET cMensajeRet = "fondos insuficientes";
							ELSE
									
								IF g_SdoCta < v_pago_minimo THEN
									LET v_pago_minimo = g_SdoCta;
								END IF
										
								EXECUTE PROCEDURE bdicheq:"informix".cargo_ref(g_Empresa, '9290', 'informix', '0425','0000', g_Folio, g_CuentaCap, 0,  v_pago_minimo, '01', LPAD(TRIM(g_NumCred),12,'0')||' '||v_referencia, '', 'informix')
								INTO cCodRetCtrl, ChaAux, ChaAux, DecAux, DecAux;
								LET cCodRetAux = '00'||cCodRetCtrl;
											
								IF cCodRetAux = '00000' THEN  
										EXECUTE PROCEDURE "informix".principal(g_Empresa, g_NumCred, 2, v_pago_minimo, 'informix', '9290', g_Folio, '8024')
										INTO cCodRetCtrl,DecAux,DecAux,DecAux,DecAux, DecAux, DecAux, DecAux, DecAux, DecAux;
										LET cCodRetCtrl = '00'||cCodRetCtrl;
										LET cMensajeRet = "Se realiza el pago correctamente";
											IF cCodRetCtrl <> '00000' THEN
												EXECUTE PROCEDURE bdicheq:"informix".reversion ('001','9290','informix', g_Folio,"A")
												INTO cCodRetAux;
												LET cMensajeRet = "Se realiza reverso correctamente";
												LET cCodRetAux = '00'||cCodRetAux;
											END IF
									ELSE 
										/*
										EXECUTE PROCEDURE bdicheq:"informix".reversion ('001','9290','informix', g_Folio,"A")
										...
										LET cMensajeRet = "Se realiza reverso correctamente";
										LET cCodRetAux = '00'||cCodRetAux;
										LET cCodRetCtrl = cCodRetAux;
										*/
										EXECUTE PROCEDURE bdicred:sp_ce_consultacodret (cCodRetAux,'01') 
										INTO cCodRetCtrl, ChaAux1, ChaAux2;
										LET cMensajeRet = ChaAux2;
						...
						--BEGIN WORK;
							...
							VALUES ('06','CobroAutTC',dtFechaHoy,current,'informix',g_NumCred,g_CuentaCap,dSeAplicoReverso,g_Folio,v_pago_minimo,cCodRetCtrl,cCodRetAux,cMensajeRet);
						--COMMIT WORK;
							LET cCodRetAux = '';
							LET cCodRetCtrl = '';
							LET cMensajeRet = '';
		...
		DROP TABLE temp_cobro_aut_TC;
	
	ELSE 
		IF v_flag_dia_laborable = '0' THEN
			LET cMensaje= "Dia no laborable NO se aplica proceso";	
		END IF;
	END IF	
