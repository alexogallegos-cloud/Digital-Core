CREATE PROCEDURE "informix".sp_reporteliquidacioncrediavan(pConvenio CHAR(5))
...
LET cConvenio	= SUBSTRING(pConvenio FROM 3 FOR 3);  --012
LET dFechaAux	= '';
LET dfecha_Hoy	= '';
LET dFechaIni	= '';
LET dPriDiaMes	= '';
...
--	SET DEBUG FILE TO '/respaldosbd/hugovaz/sp_reporteliquidacioncrediavan.out';
--	TRACE ON;
	...
	BEGIN
		ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
			IF iSqlErr <> 0 THEN
				...
				EXECUTE PROCEDURE "informix".sp_sac_GuardaMensajeerror (iSqlErr, iIsamErr, cInfoErr, "sp_reporteliquidacioncrediavan");
			END IF;
		END EXCEPTION;
