CREATE PROCEDURE "informix".sp_ctanvl2_regtelefonos(pNumCte CHAR(20),pTelefono CHAR(13),pTipoTel SMALLINT,pCarrier SMALLINT)
RETURNING CHAR(5) AS codret;
    ...
    DEFINE cTelAnterior CHAR(13);
    DEFINE iExisteCel INTEGER;
    DEFINE iExisteCarrier INTEGER;
    DEFINE cCodRetValTel CHAR(5);
    DEFINE cValTelCasa CHAR(1);
    DEFINE cValTelCelular CHAR(1);
    DEFINE cValTelOficina CHAR(1);
    DEFINE cCofetel CHAR(1);
    DEFINE sLimite SMALLINT;
    DEFINE sCoincideNumFijo SMALLINT;
    DEFINE iValidaDias INTEGER;
    DEFINE iDiasDifer INTEGER;
    DEFINE iTelefonoVal INTEGER;
    DEFINE iTelefonoNoVal INTEGER;
    DEFINE iSecuenciaTel INTEGER;
    DEFINE dFechaInsert DATE;
    DEFINE sMaxSecuencia SMALLINT;
    DEFINE cCodRetVal CHAR(6);
    DEFINE cDescripcion CHAR(10);
    DEFINE iEsFijo INTEGER;
    DEFINE sRows SMALLINT;
    DEFINE cNumCteDif CHAR(20);
    ...
    --- SET DEBUG FILE TO '/tmp/mfinis/sp_ctanvl2_regtelefonos.out';
    --- TRACE ON;
    ...
    IF pNumCte   IS NULL OR pNumCte   = '' OR 
       ...
       pTipoTel  IS NULL OR pTipoTel  = '' OR 
       pCarrier  IS NULL OR pCarrier  = '' THEN
        LET cCodRet = '110';
        RETURN cCodRet;
    END IF;
	
	--BANDERA DE INICO DE PROCESO (3) PARA DEPURACION  
	...
	EXECUTE PROCEDURE bdinteg:sp_registra_telefonos(cEmpresa, pNumCte, pTelefono, pTipoTel, pExtension, pCarrier, pCanal, 'informix')
	INTO cCodRet;
	...
	--BANDERA FINAL DEL PROCESO (3) PARA LA DEPURACION
...
'FECHA: 25/06/2020',
'BD: bdinteg',
'DESCRIPCION: SPL encargado de realizar la validacion y registro de los telÃ©fonos del cliente';
...
CREATE PROCEDURE "informix".sp_ctanvl2_regcorreos(pNumCte CHAR(20),pCorreoElec CHAR(100),pTipoCorreo SMALLINT)
	...
	DEFINE iCodRetSp INTEGER; 
	DEFINE iExisteCte INTEGER;
	DEFINE iCorreoInv INTEGER;
	DEFINE iExisteCorreo INTEGER;
	DEFINE sSecuencia SMALLINT;
	DEFINE cCorreoAnt VARCHAR(100);
	DEFINE iTotCorreoAnt INTEGER;
	DEFINE dFechaInsert DATE;
	DEFINE cEmpresa CHAR(3);
	DEFINE cExiste CHAR(1);
	DEFINE pCanal SMALLINT;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ctanvl2_regcorreos.out';
		--TRACE ON;
		...
		--VALIDACION DE CAMPOS REQUERIDOS
		IF pNumCte IS NULL OR pNumCte = '' OR pCorreoElec IS NULL OR pCorreoElec = '' OR 
		pTipoCorreo IS NULL OR pTipoCorreo = '' THEN
			LET cCodRet = '110';
			RETURN cCodRet;
		END IF;
		
		--BANDERA DE INICO DE PROCESO (4) PARA DEPURACION  
	    LET vFechInicio = CURRENT YEAR TO FRACTION(5);
	    UPDATE bdinteg:si_ctanvl2_ctrl
	    SET    proceso        =  4,
	           estatus        = 'I',
		...
		IF vExisteCorreo = 0 THEN		
			EXECUTE PROCEDURE bdinteg:sp_registra_correos(cEmpresa, pNumCte, pCorreoElec, pTipoCorreo, pCanal, 'informix')
			INTO cCodRet;
		ELSE
			LET cCodRet = '000';
		END IF;
		...
		--BANDERA FINAL DEL PROCESO (4) PARA LA DEPURACION
	    LET vFechFin = CURRENT YEAR TO FRACTION(5);
	    UPDATE bdinteg:si_ctanvl2_ctrl
	    SET    estatus     = 'F',
	      	   fechora_fin = vFechFin	  
	...
	END;
END PROCEDURE
DOCUMENT 'AUTOR: Veronica SÃÂ¡nchez Tlacomulco',
'FECHA: 22/06/2020',
'DESCRIPCION: SPL encargado de validar y registrar el correo electrÃÂ³nico del cliente',
'BD: bdinteg';
