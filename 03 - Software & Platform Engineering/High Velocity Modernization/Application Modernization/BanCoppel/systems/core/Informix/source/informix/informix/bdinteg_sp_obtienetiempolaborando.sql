CREATE PROCEDURE "informix".sp_obtienetiempolaborando (	cnumcte CHAR(20), cStatusSolicitud CHAR (5))
...
DEFINE cFechaLaborando CHAR(10);
...
LET cFechaLaborando = '';
	...
	--SET DEBUG FILE TO "/tmp/sp_ObtieneTiempoLaborando.out";
	--TRACE ON;
	...
	BEGIN
		...
		IF cnumcte IS NOT NULL OR cnumcte <> "" THEN
			...
			IF EXISTS(SELECT num_solicitud,fecha_insert
				...
				IF cDescripElemento = "10 años o más" THEN
					LET cDiasSolicitudFinal = iDiasSolicitud -3650;
				END IF;
				
				IF cDescripElemento = "Entre 8 años y 9 años 11 meses" THEN
					LET cDiasSolicitudFinal = (iDiasSolicitud -3615);
				END IF;
				
				IF cDescripElemento = "Entre 5 años y 7 años 11 meses" THEN
					LET cDiasSolicitudFinal = (iDiasSolicitud -2885);
				END IF;
				
				IF cDescripElemento = "Entre 4 y 4 años 11 meses" THEN
					LET cDiasSolicitudFinal = (iDiasSolicitud -1790);
				END IF;
				
				IF cDescripElemento = "Entre 3 y 3 años 11 meses" THEN
					LET cDiasSolicitudFinal = (iDiasSolicitud -1425);
				END IF;
				...
				IF cDescripElemento = "Entre 1 y 1 año 11 meses" THEN
					LET cDiasSolicitudFinal = (iDiasSolicitud -695);
				END IF;
				
				IF cDescripElemento = "Menos de 1 año" THEN
					LET cDiasSolicitudFinal = (iDiasSolicitud -364);
				END IF;
				
				LET cFechaLaborando = CAST(cDiasSolicitudFinal::INTEGER AS date);
			ELSE
				RETURN '00002',('01/01/2005');
			END IF;
		ELSE
			RETURN '00001',('01/01/2005'); -- CUANDO EL CLIENTE NO EXISTE CON TODAS LAS CARACTERISTICAS
		END IF;
		RETURN cCodRet, NVL(cFechaLaborando,'01/01/2005');
	END;
	--*************************************************************************
--| Procedimiento   : sp_ObtieneTiempoLaborando
--| Versión         : 1.0
--| Creado por      : Martha Aguirre
--| Fecha creacion  : Noviembre de 2008
--| Descripción     : Obtiene el tiempo que tiene laborando el cliente
