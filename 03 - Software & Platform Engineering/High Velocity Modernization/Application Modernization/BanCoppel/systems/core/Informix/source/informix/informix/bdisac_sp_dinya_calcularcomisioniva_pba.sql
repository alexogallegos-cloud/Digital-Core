CREATE PROCEDURE "informix".sp_dinya_calcularcomisioniva_pba
	(pIdConvenio CHAR(5),
	pImporte MONEY (16,2),
...
CREATE PROCEDURE "informix".sp_sac_eliminamovsbtspayi(fechmin DATE, fechmax DATE)
    RETURNING CHAR(5), char(6);  --CÃ³digos de retorno

DEFINE cCodRet       CHAR(5);
DEFINE vbank_ref_nm    CHAR (16);
DEFINE vtotregshist  CHAR (6);
DEFINE iSqlErr       integer;
DEFINE cont_borra  integer;

LET cCodRet        = '00000';
LET vbank_ref_nm     = '0000000000000000';
LET vtotregshist   = '000000';
LET iSqlErr        = 0;
LET cont_borra     = 0;

 --SET DEBUG FILE TO "/informix/cristol/sp_sac_eliminamovsbtspayi.out";
 --TRACE ON;
...
BEGIN
   ...
   ON EXCEPTION SET iSqlErr
	...
	SELECT count (*)
		...
		WHERE
		fecha_insert between fechmin and fechmax;
	...
	END IF;
		begin work;
		DELETE FROM bdisac:"informix".sac_bts_payi
...
DOCUMENT
'AUTOR : CJL',
'DESCRIPCION: Elimina registros de tabla bdisac:"informix".sac_bts_payi por medio de cursor, con fechas como parametro de entrada.',
'EJECUTADO O LLAMADO POR: Proceso especial (se ejecuta por script en casos especiales).',
'FECHA : Abril/2013',
'VERSION: 20130408',
'BD    : bdisac';

CREATE PROCEDURE "informix".sp_sac_eliminamovsbtsqryi(fechmin DATE, fechmax DATE)
    RETURNING CHAR(5), char(10);  --CÃ³digos de retorno

DEFINE cCodRet  CHAR(5);
DEFINE vfecha_insert DATETIME YEAR to FRACTION(5);
DEFINE vtotregshist  CHAR (10);
DEFINE iSqlErr       integer;
DEFINE iContBorra  integer;

LET cCodRet        = '00000';
LET vfecha_insert     = current;
LET vtotregshist   = '0000000000';
LET iSqlErr        = 0;
LET iContBorra     = 0;

 --SET DEBUG FILE TO "/informix/RESPALDOS/sp_sac_eliminamovsbtsqryi.out";
 --TRACE ON;
...
BEGIN
   ...
   ON EXCEPTION SET iSqlErr
	...
	SELECT count({+INDEX ("informix".sac_bts_qryi confirmation_nm)} confirmation_nm) 
		into vtotregshist
	...
	FOREACH cursor_borra WITH HOLD FOR
		...
		DELETE FROM "informix".sac_bts_qryi WHERE CURRENT OF cursor_borra;
...
DOCUMENT
'AUTOR : CJL',
'DESCRIPCION: Elimina registros de tabla bdisac:"informix".sac_bts_qryi por medio de cursor, con fechas como parametro de entrada.',
'EJECUTADO O LLAMADO POR: Proceso especial (se ejecuta por script en casos especiales).',
'FECHA : Abril/2013',
'VERSION: 20130411',
'BD    : bdisac';
...
CREATE PROCEDURE "informix".sp_guardarespuestaqryi(pSucursal CHAR (4), 
	...
	--SET DEBUG FILE TO "/tmp/sp_GuardaRespuestaQryi.out";
    --TRACE ON;
	...
	-- Nueva validación por duplicidad de pagos. JGP. 26-Sep-11
		SELECT status_cancelado INTO cStatus FROM bdisac:sac_movimientos 
			...
			IF cStatus ='N' AND pTrans_Status_Cd = 'ONP' THEN -- Si encontró un intento de pago previo y no ha sido reversado
...
CREATE PROCEDURE "informix".sp_sac_conciliadeta_pba(pfecha DATE)
	returning 	char(5)  	as CodRetorno,
				CHAR(90)	as mensaje;
				
	
	--Elaboró: Alejandro Osuna Iza
	--Actividad: Extrae la información de la tabla de sac_eglobal_detalle y de la sc_movhis para el proceso de generacion del reporte de detalle de la concilacion.
	...
	SET DEBUG FILE TO "/resplogifx/archivoscartera/sp_sac_conciliadeta.out";
	TRACE ON;
		...
		IF EXISTS(SELECT conciliado FROM bdisac:sac_eglobal_archivos where fecha_archivo = pfecha) THEN
			...
			IF EXISTS(SELECT conciliado FROM bdisac:sac_eglobal_archivos where fecha_archivo = pfecha and estatus = '1') THEN
				...
				--se extrae la informacion para los movimeintos conciliados
                    FOREACH
                        --obtiene los datos de lso movimientos conciliados
						...
						ELSE
                            ...
                            AND movhis.Transacc IN (SELECT TRIM(NVL(Valor, '')) FROM BdiSac:Sac_Param WHERE Empresa = '001' AND Cod_Param IN ('33008') );					  
				...
				----obtiene los datos de lso movimientos no conciliados en ARchivo E-Global
				FOREACH
					SELECT unique 	TRIM(sac_det.cod_txn),TRIM(sac_det.codigo_registro),TRIM(sac_det.numero_tarjeta),TRIM(sac_det.num_referencia),
									TRIM(sac_det.fecha_txn),TRIM(sac_det.importe_txn),TRIM(sac_det.cod_actual_destino),TRIM(sac_det.nombre_negocio),
...
/*					IF vcod_razon = '91' THEN
						LET vcod_razonF = vcod_razon || '-' || vCod_razon91;
					END IF;
					IF vcod_razon = '92' THEN
						LET vcod_razonF = vcod_razon || '-' || vCod_razon92;
						--OBTIENE LA CUENTA DE LA TRAJETA
                        if pfecha >= vconsmovhis then
                            SELECT LIMIT 1 TRIM(movhis.cuenta) INTO vcuenta_cargo 
                            FROM bdicheq:sc_movhis AS movhis
                            WHERE  movhis.folio_suc = vsFolioSuc
                            AND movhis.Transacc IN (SELECT TRIM(NVL(Valor, '')) FROM BdiSac:Sac_Param WHERE Empresa = '001' AND Cod_Param IN ('33008') );
						else
                            SELECT LIMIT 1 TRIM(movhis.cuenta) INTO vcuenta_cargo 
                            FROM bdicheq:sc_movhis_old AS movhis
					...
					END IF;
					IF vcod_razon = '93' THEN
						LET vcod_razonF = vcod_razon || '-' || vCod_razon93;
					END IF;
					IF vcod_razon = '94' THEN
						LET vcod_razonF = vcod_razon || '-' || vCod_razon94;
					END IF;
*/					
					...
					Values('2',vcod_txn,vbanco_des,vnum_tarjeta,vnum_referencia,vfecha_txn,vimporte_txn,vcod_actual_des,vnombre_negocio,vcod_razonF,vind_deposito,
							vsucursal,vcuenta_cargo,vtransacc_suc,iimporte,vnum_refemov);
				END FOREACH
				----obtiene los datos de lso movimientos no conciliados en tabla de movimientos Historicos
                if pfecha >= vconsmovhis then
                    FOREACH
                        ...
                        /*--OBTIENE LA CUENTA DE LA TRAJETA
                        SELECT FIRST 1 NVL(Cuenta, '') INTO vcuenta_cargo FROM bdicheq:Sc_Tarjeta  
                        WHERE Cuenta IS NOT NULL
                        AND Num_Tarjeta = NVL(vnum_tarjeta, '');
                        */

                        LET iContadorBan = iContadorBan + 1;
                        LET vbanco_des = '';
                        --SELECT TRIM(valor) INTO vcod_transuc FROM bdisac:sac_param where cod_param = '33005';
                        SELECT cod_param INTO vcod_transuc FROM bdisac:sac_param where valor = vtransacc_suc;
                        IF vcod_transuc <> '33005' THEN
                            --LET vcuenta_cargo = '';
                        END IF;
                        LET vcod_txn = '';
                        LET vcod_registro  = '';
...
'Descripcion: Extrae la información de la tabla de sac_eglobal_detalle y de la sc_movhis para el proceso de generacion del reporte de detalle de la concilacion.',
'Fecha: 2010/03/16',
...
CREATE PROCEDURE "informix".sp_bts_confirmapayc(pAgent_trans_type_code CHAR(4),pUsuario CHAR(8),
												pAgent_cd CHAR(3),pConfirmation_nm CHAR(11),
												pProcess_type_code CHAR(4),pBank_ref_nm CHAR(20),
												pBank_concept1 CHAR(40),pAgnt_region_sd CHAR(15),
												pAgnt_branch_sd CHAR(15),pAgnt_state_cd CHAR(3),
												pAgnt_country_cd CHAR(3),pAgnt_user_name CHAR(20),
												pAgnt_terminal CHAR(15),pAgent_dt CHAR(8),
												pAgent_tm CHAR(6),pType_cd CHAR(3),
												pIssuer_cd CHAR(3),pIssuer_state_cd CHAR(3),
												pIssuer_country_cd CHAR(3),pIdentif_nm CHAR(20),
												pExpiration_dt CHAR(8),pBenef_dob_dt CHAR(8),
												pDir_remitente CHAR(80),pCd_remitente CHAR(40),
												pRem_state_cd CHAR(3),pRem_country_cd CHAR(3),
												pRem_zip_code CHAR(10),pRem_phone CHAR(15),
												pOpcode CHAR(4),pProcess_msg CHAR(50),
												pErr_param_full_name CHAR(80),pTrans_status_cd CHAR(3),
												pTrans_status_dt CHAR(3),pProcess_dt CHAR(8),
												pProcess_tm CHAR(6),pBank_ref_num CHAR(20),
												pPromotoion_cd CHAR(4),pUser_insert CHAR(8))
RETURNING CHAR(5)  AS CodRetorno,
		  CHAR(80) AS Desc_Error,
		  CHAR(11) As Confirmation_nm,
...
DEFINE cEstatusDep			CHAR(2);
DEFINE cEstatusDepN			CHAR(2);
...
LET cOpCode				 = '0000';
LET cDescError	 		 = '';
LET cNombreSPL   		 = 'sp_bts_confirmapayc';
...
LET iIntentos            = 0;
LET cCadena_ent			 =  TRIM(NVL(pConfirmation_nm,'NULL'))||'|'||TRIM(NVL(pAgnt_user_name,'NULL'))||'|'||TRIM(NVL(pProcess_dt,'NULL'))||'|'||TRIM(NVL(pProcess_tm,'NULL'));
LET cEstatusDep			 = '';
LET cEstatusDepN		 = '';
LET	cCnxn_status		 = 'C';

--SET DEBUG FILE TO '/tmp/RMBTS/sp_bts_confirmapayc.out';
--TRACE ON;
...
BEGIN
	ON EXCEPTION
		...
		IF iSqlErr <> 0 THEN
			LET cCodRet = iSqlErr;
			INSERT INTO bdisac:"informix".sac_bts_payc(cnxn_status,agent_trans_type_code,agent_cd,confirmation_nm,process_type_code,bank_ref_nm,bank_concept1,agnt_region_sd,agnt_branch_sd,agnt_state_cd,agnt_country_cd,agnt_user_name,agnt_terminal,agent_dt,agent_tm,type_cd,issuer_cd,issuer_state_cd,issuer_country_cd,identif_nm,expiration_dt,benef_dob_dt,dir_remitente,cd_remitente,rem_state_cd,rem_country_cd,rem_zip_code,rem_phone,opcode,process_msg,err_param_full_name,trans_status_cd,trans_status_dt,process_dt,process_tm,bank_ref_num,promotion_cd,user_insert,fecha_insert) 
			VALUES(cCnxn_status,pAgent_trans_type_code,pAgent_cd,pConfirmation_nm,pProcess_type_code,pBank_ref_nm,pBank_concept1,pAgnt_region_sd,pAgnt_branch_sd,pAgnt_state_cd,pAgnt_country_cd,pAgnt_user_name,pAgnt_terminal,pAgent_dt,pAgent_tm,pType_cd,pIssuer_cd,pIssuer_state_cd,pIssuer_country_cd,pIdentif_nm,pExpiration_dt,pBenef_dob_dt,pDir_remitente,pCd_remitente,pRem_state_cd,pRem_country_cd,pRem_zip_code,pRem_phone,pOpcode,pProcess_msg,pErr_param_full_name,pTrans_status_cd,pTrans_status_dt,pProcess_dt,pProcess_tm,pBank_ref_num,pPromotoion_cd,pUser_insert,CURRENT );

			EXECUTE PROCEDURE bdisac:"informix".sp_insertaerrorws(1,cNombreSPL, cCodRet, '',iSqlErr,iIsamErr, cCadena_ent,pUsuario,pAgent_dt,pAgent_tm)
			INTO cCodRet2;
	...
	VALUES (cNombreSPL,pAgent_dt,pAgent_tm,'0','',pUsuario,current::date,cHoraInsert);
	
	IF NVL(pAgent_trans_type_code,'') = '' THEN
		LET cCodRet = '9982';
	ELSE
		IF NVL(pAgent_cd,'') = '' THEN
			LET cCodRet = '9981';
		ELSE
			IF	NVL(pConfirmation_nm,'') = '' THEN
				LET cCodRet = '9980';
			ELSE
				SELECT NVL(1,'0'),estatus_sdep 
				INTO cValor,cEstatusDep
				FROM bdisac:"informix".sac_bts_sdep 
				WHERE estatus_sdep IN ('03','04')
				AND num_confirmacion = pConfirmation_nm;
				
				IF cValor = 0 OR cValor IS NULL THEN
					LET cCodRet = '9979';
				ELSE
					SELECT fecha_hoy 
						INTO dFechaHoy
					...
					IF pOpcode = '1100' OR pOpcode = '1101' OR pOpcode = '9054' THEN
						IF cEstatusDep = '03' THEN
							LET cEstatusDepN = '05';
						ELIF cEstatusDep = '04' THEN
							LET cEstatusDepN = '06';
						END IF;
						
						UPDATE bdisac:"informix".sac_bts_sdep 
						SET estatus_sdep = cEstatusDepN
						WHERE  agent_trans_type_code = 'SDEP' 
						AND    num_confirmacion = pConfirmation_nm and estatus_sdep = cEstatusDep;	
	...
	IF cCodRet = '0000' THEN
		LET cDescError = 'Confirmación PAYC exitosa';
		EXECUTE PROCEDURE bdisac:"informix".sp_insertaerrorws(2,cNombreSPL, LPAD(cCodRet,5,'0'), cDescError,'','', cCadena_ent,pUsuario,pAgent_dt,pAgent_tm)
		INTO cCodRet2;
		RETURN LPAD(cCodRet,5,'0'),cDescError,pConfirmation_nm,'1',cFechaInsert,cHoraInsert;
	ELSE
		INSERT INTO bdisac:"informix".sac_bts_payc(cnxn_status,agent_trans_type_code,agent_cd,confirmation_nm,process_type_code,bank_ref_nm,bank_concept1,agnt_region_sd,agnt_branch_sd,agnt_state_cd,agnt_country_cd,agnt_user_name,agnt_terminal,agent_dt,agent_tm,type_cd,issuer_cd,issuer_state_cd,issuer_country_cd,identif_nm,expiration_dt,benef_dob_dt,dir_remitente,cd_remitente,rem_state_cd,rem_country_cd,rem_zip_code,rem_phone,opcode,process_msg,err_param_full_name,trans_status_cd,trans_status_dt,process_dt,process_tm,bank_ref_num,promotion_cd,user_insert,fecha_insert)
		VALUES(cCnxn_status,pAgent_trans_type_code,pAgent_cd,pConfirmation_nm,pProcess_type_code,pBank_ref_nm,pBank_concept1,pAgnt_region_sd,pAgnt_branch_sd,pAgnt_state_cd,pAgnt_country_cd,pAgnt_user_name,pAgnt_terminal,pAgent_dt,pAgent_tm,pType_cd,pIssuer_cd,pIssuer_state_cd,pIssuer_country_cd,pIdentif_nm,pExpiration_dt,pBenef_dob_dt,pDir_remitente,pCd_remitente,pRem_state_cd,pRem_country_cd,pRem_zip_code,pRem_phone,pOpcode,pProcess_msg,pErr_param_full_name,pTrans_status_cd,pTrans_status_dt,pProcess_dt,pProcess_tm,pBank_ref_num,pPromotoion_cd,pUser_insert,current );
		...
		EXECUTE PROCEDURE bdisac:"informix".sp_insertaerrorws(1,cNombreSPL, LPAD(cCodRet,5,'0'), cDescError,'','', cCadena_ent,pUsuario,pAgent_dt,pAgent_tm)
		INTO cCodRet2;
		...
		RETURN LPAD(cCodRet,5,'0'),NVL(cDescError,''),'','0','','';
	END IF;
END;

END PROCEDURE
DOCUMENT
'DESCRIPCION:  Se crea Procedimiento Almacenado, para actualizar la tabla sac_bts_sdep a "05 (Pagada) o 06 (Rechazada)"',
'			  las remesas que han sido confirmadas por BTS sobre el abono en la cuenta del beneficiario.',
'AUTOR : Martín Eduardo Miranda',
'FECHA : 01 Noviembre 2012',
'VERSION: 20121101.0951',
'BD: bdisac';
