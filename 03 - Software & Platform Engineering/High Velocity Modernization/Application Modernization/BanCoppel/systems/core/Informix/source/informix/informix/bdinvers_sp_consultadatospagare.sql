CREATE PROCEDURE "informix".sp_consultadatospagare(pEmpresa CHAR(3),pCuenta CHAR(20))
	...
	BEGIN
		...
		ELSE
			...
			AND cuenta = pCuenta 
			AND secuencia = ( SELECT MAX(secuencia) FROM "informix".sv_maeinv WHERE empresa = pEmpresa AND cuenta = pCuenta );			
			
			
			FOREACH
				SELECT UNIQUE( inst_vento) INTO cVencimiento FROM "informix".sv_maeinstrucc WHERE empresa = pEmpresa  AND cuenta = pCuenta
			...
			END IF;
				...
				EXECUTE PROCEDURE "informix".consctesfirminv(pEmpresa,cCuenta,'','1')
				INTO cValRetorno2,cTipo,cNumcte2,cApell_pat,cApell_mat,cNombre1,cNombre2,cRFC,cFech_nac,sSecuencia,cParentesco,cProducto;
					...
					IF cValRetorno2 = "000" THEN
						...
						EXECUTE  PROCEDURE "informix".conprev1(pEmpresa, cSucursal, cCod_instrum, mCapital,'', sPlazo, '01', 0.00, 'S')
