CREATE PROCEDURE "informix".sp_mail_primerconsumo_tco()
returning 
...
--Abrham lopez lopez
--2016-05-16
--crea archivo con datos que se muestran en pantalla cat con cliente con mora 1 tco.
--execute PROCEDURE "informix".sp_mail_primerconsumo_tco();
...
define iCount_TCO_PRIMER INTEGER; 
define iCount_TCO_PRIMES INTEGER;
...
let P_MENSAJE	= 'El proceso de las campañas XX TDC Oro PRIMER CONSUMO se realizó correctamente.';
let vproceso	= '0016';
let cMensaje	= '';
let vmail 		= '';
...
let vapell_paterno = '';
let iCount_TCO_PRIMER = 0;
let iCount_TCO_PRIMES = 0;
let i = 0;
LET num = 0;
let vNumIniciudad	='';
let vEstadoSiglas	='';
...
BEGIN
  ...
  --Set debug file to '/informix/ALL/TCOro/sp_mail_primerconsumo.out';
  --trace on;
...
--temporal para pruebas
	...
	insert into bdimnsj:mnsjr_trx_batch(tipo_mensaje,id_mensaje,cliente, fecha_hora_registro,string1,importe1,fecha1,fecha2)
		select  1, 'TCO_PRIMER',numcte,current,apell_paterno,100,current,current
	...
	FOREACH
		...
		if (vfecha1 is null or vfecha2 is null) then
			...
			and t.tipo_tarjeta ='T'  and t.status_tar = 'A';   
				...
				LET iCount_TCO_PRIMER = iCount_TCO_PRIMER +1;
				call bdimnsj:"informix".sp_registra_evento (1, 'TCO_PRIMER' , vnumcte, vnumcredito,vnumtarjeta, 2,
							'','','','','',vimporte,0,0,0,0, today, '')RETURNING P_COD_RET;

				call "informix".sp_inserta_info_rep_envios ('001','EMAIL',1020, vnumcredito, vnumcte, cNumProducto, today, vmail, '','', 0) returning P_COD_RET;

		end if;
	END FOREACH

	IF iCount_TCO_PRIMER > 0 THEN
        CALL bdicobranza:"informix".sp_latinia_contador_cobranza('TCO_PRIMER',iCuentasProcesadas,iCuentasExcluidasXMail) RETURNING P_COD_RET;
	END IF;
	
--Genera cifras de control
    if iCuentasProcesadas > 0 then
       let cMensaje = 'TOTAL Cuentas procesadas campaña TCO_PRIMER : ' ||iCuentasProcesadas;
       let cMensaje = trim(cMensaje) ||'    EMAILs enviados TCO_PRIMER : ' ||iCount_TCO_PRIMER;
...
---------------------------------------------------------SMS-------------------------------------------------------	
	...
	from bdimnsj:mnsjr_trx_batch_his
		where id_mensaje ='TCO_PRIMES' and DATE(fecha_hora_registro) >= vpri_dia_mes;
...
if(vtotal < vvalor_numerico) then 
			...
			if cCel is null or cCel = '' then 
		       ...
		       continue foreach; 
				...
				IF iCausa IS NULL THEN LET iCausa = 0; END IF; 
					...
					LET iCount_TCO_PRIMES = iCount_TCO_PRIMES +1;
					call bdimnsj:"informix".sp_registra_evento (2, 'TCO_PRIMES' , vnumcte, vnumcredito,vnumtarjeta, 2,
							'','','','','',0,0,0,0,0, '', '')RETURNING P_COD_RET;
							
					let vcontador = vcontador + 1;			
					call "informix".sp_inserta_info_rep_envios ('001','SMS',23, vnumcredito, vnumcte, cNumProducto, today, cCel, '','', 0) returning P_COD_RET;
					end if; 
	...
	insert into bdimnsj:mnsjr_trx_batch(tipo_mensaje,id_mensaje,cliente,fecha_hora_registro,string1)
		select  2, 'TCO_PRIMES',numcte,current,apell_paterno
		from bdinteg:si_cliente
        where numcte in (select substr(valor,num,9) from bdicobranza:cb_param where cod_param = 57);
			let num = num + 10;
	end for
	...
	-------------------------------------------contadores------------------------------------	
		...
		IF iCount_TCO_PRIMES > 0 THEN
            CALL bdicobranza:"informix".sp_latinia_contador_cobranza('TCO_PRIMES',iCuentasProcesadas,iCuentasExcluidasXCel) RETURNING P_COD_RET;
		END IF;
		
--Genera cifras de control
	    if iCuentasProcesadas > 0 then
	       let cMensaje = 'TOTAL Cuentas procesadas campaña TCO_PRIMES : ' ||iCuentasProcesadas;
	       let cMensaje = trim(cMensaje) ||'    EMAILs enviados TCO_PRIMES : ' ||iCount_TCO_PRIMES;
	       CALL bdicobranza:"informix".sp_inserta_bitacora_cob('001', vproceso, P_COD_RET, trim(cMensaje), '02') RETURNING P_COD_RET;
