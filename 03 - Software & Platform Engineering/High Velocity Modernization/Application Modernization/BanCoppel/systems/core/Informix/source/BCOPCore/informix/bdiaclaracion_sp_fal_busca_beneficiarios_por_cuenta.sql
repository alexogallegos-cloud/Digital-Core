CREATE PROCEDURE "informix".sp_fal_busca_beneficiarios_por_cuenta(p_numeroCuenta CHAR(20) )
    ...
    DEFINE resultado_apellidoPat_cte         CHAR(30);
    DEFINE resultado_apellidoMat_cte         CHAR(30);
    DEFINE resultado_nombre1_cte         CHAR(30);
    DEFINE resultado_nombre2_cte         CHAR(30);
    ...
    BEGIN
        ...
        FOREACH
               SELECT cte.numcte  as numero_cliente, 
                    ...
                    cte.nombre1,
                    cte.nombre2
                    INTO resultado_numeroCliente, resultado_porc_bene,resultado_apellidoPat_cte,resultado_apellidoMat_cte,resultado_nombre1_cte,resultado_nombre2_cte
                FROM  bdinteg:si_cliente cte 
                LEFT OUTER JOIN bdicheq:"informix".sc_beneficiario bene ON (cte.numcte = bene.numcte)
                WHERE bene.cuenta  = p_numeroCuenta

                            SELECT FIRST  1 
                                        qc.cuenta as numero_cuenta,
                                        stc.pky_estatus_cuenta,
                                        --CASE
                                        ...
                                        --CASE
                                            ...
                                            --ELSE bene.porcentaje
                                        --END as porcentaje,
                                        --NVL(bene.porcentaje,0) as porcentaje,
                                        pr.producto,
                                        cte.apell_paterno,
                                        cte.apell_materno,
                                        cte.nombre1,
                          ...
                          group by numero_cuenta,  estatus ,  stc.descripcion,porcentaje ,pr.producto , cte.apell_paterno,cte.apell_materno, cte.nombre1,cte.nombre2, mot.descripcion, stc.pky_estatus_cuenta;


                        IF resultado_numeroCuenta = '' OR resultado_numeroCuenta IS NULL THEN
                           LET resultado_numeroCliente = resultado_numeroCliente;
                           LET resultado_numeroCuenta = TRIM(p_numeroCuenta) || resultado_numeroCliente;
                           LET resultado_estatus = '';
                           LET resultado_motivo = '';
                           LET resultado_porcentaje = resultado_porc_bene;
                           LET resultado_numeroProducto = resultado_numeroProducto;
                           LET resultado_apellidoPat = resultado_apellidoPat_cte;
                           LET resultado_apellidoMat = resultado_apellidoMat_cte;
                           LET resultado_nombre1 = resultado_nombre1_cte;
                           LET resultado_nombre2 = resultado_nombre2_cte;
                           LET resultado_descripcion_estatus = '';
                        END IF
...
CREATE PROCEDURE "informix".sp_fal_busca_documentos_faltantes(p_numeroCuentaFallecido CHAR(20), p_numeroCuentaBeneficiario CHAR(20), p_numeroSolicitud INTEGER)
	RETURNING INTEGER AS resultadoNumeroDocsFaltantes;

	-- Definicion de variables
	DEFINE res_numDocsFaltantes	INTEGER;
	DEFINE iSQLErr				INTEGER;

	-- Inicializacion de variables
	LET res_numDocsFaltantes = 0;
	...
	BEGIN
        ...
        WHEN 
            SUM(
            CASE WHEN ctrl_digic.inconsistencia IS NULL THEN 1
                ELSE ctrl_digic.inconsistencia
            END
            ) IS NULL THEN -1
            ELSE SUM(
            CASE WHEN ctrl_digic.inconsistencia IS NULL THEN 1
        ...
        END
		INTO res_numDocsFaltantes
		FROM fal_beneficiario bene
		INNER JOIN fal_control_tramite ctrl_tram_cta on bene.fky_control_tramite = ctrl_tram_cta.pky_control_tramite
		INNER JOIN fal_cat_tipo_beneficiario tipo_bene on bene.fky_tipo_beneficiario = tipo_bene.pky_tipo_beneficiario
		INNER JOIN fal_beneficiario_gpo_doc bene_gpo_doc on tipo_bene.pky_tipo_beneficiario = bene_gpo_doc.fky_tipo_beneficiario
		INNER JOIN fal_grupo_documento gpo_doc on bene_gpo_doc.fky_grupo_documento = gpo_doc.fky_grupo_documento
		INNER JOIN fal_cat_tipo_documento tipo_doc on gpo_doc.fky_tipo_documento = tipo_doc.pky_tipo_documento
		LEFT JOIN fal_control_digitaliza_doc ctrl_digic on gpo_doc.fky_tipo_documento = ctrl_digic.fky_tipo_documento 
			AND bene.pky_cuenta_beneficiario = ctrl_digic.cuenta_beneficiario
            AND bene.pky_cuenta_cliente_fallecido = ctrl_digic.cuenta_cliente_fallecido
			AND ctrl_tram_cta.fky_solicitud = ctrl_digic.fky_solicitud
		WHERE bene.pky_cuenta_beneficiario = p_numeroCuentaBeneficiario
        AND bene.pky_cuenta_cliente_fallecido = p_numeroCuentaFallecido
		AND ctrl_tram_cta.fky_solicitud = p_numeroSolicitud
		RETURN res_numDocsFaltantes WITH RESUME;
...
CREATE PROCEDURE "informix".sp_fal_busca_pagares_cliente(p_numeroCliente CHAR(20) )
    ...
    DEFINE resultado_numeroCuenta       CHAR(30);
    DEFINE resultado_numeroCuetaTmp            CHAR(30);
    DEFINE resultado_estatus                CHAR(30);
    DEFINE resultado_motivo                 CHAR(100);
    ...
    BEGIN
        ON EXCEPTION
            ...
            IF iSqlErr <> 0 THEN
                ...
                RETURN resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta ,resultado_estatus;
            END IF;
        END EXCEPTION;

        FOREACH
                SELECT qc.cuenta 
                    INTO resultado_numeroCuetaTmp
                FROM bdicheq:"informix".sc_maechq qc
                WHERE qc.producto not in (1100)--DEJAMOS FUERA LAS INVERSIONES
                AND qc.num_cte  = p_numeroCliente
             --   AND  qc.status_cta not in (6)   

        FOREACH
                             SELECT pr.producto , pr.descripcion , inv.cuenta , '' as tarjeta
                            INTO resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta, resultado_estatus
                            FROM bdinvers:"informix".sv_maeinv inv ,acl_tipo_producto pr
                            WHERE inv.cod_instrum = pr.producto
                            AND inv.status_cta = '1'
                            AND inv.cta_cheques = resultado_numeroCuetaTmp
                RETURN resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta ,resultado_estatus 
                WITH RESUME;
       END FOREACH;
       END FOREACH;
...
CREATE PROCEDURE "informix".sp_fal_busca_pagares_cliente_fallecido(p_numeroCliente CHAR(20) )
    ...
    DEFINE resultado_fechaVenc_Char            CHAR(30);
    DEFINE resultado_fechaVenc            DATE;
    DEFINE iSqlErr                        INTEGER;
    ...
    LET resultado_fechaVenc_Char = '';
    LET resultado_fechaVenc = DATE(1);
    ...
    BEGIN
        ...
        FOREACH
                ...
                SELECT pr.producto , pr.descripcion , inv.cuenta , inv.status_cta , inv.motivo , (capital+intereses-isr) as saldo_vemncimiento ,inv.cta_cheques as cuentaDeposito, inv.fecha_venc as fechaVenc
                ...
                FROM bdinvers:"informix".sv_maeinv inv ,acl_tipo_producto pr
                WHERE inv.status_cta = '1' 
                AND inv.cod_instrum = pr.producto
                AND inv.cta_cheques = resultado_numeroCuetaTmp
                RETURN resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta ,resultado_estatus, resultado_motivo,resultado_montoActual,resultado_cuentaDeposito,TO_CHAR(date(resultado_fechaVenc))
                WITH RESUME;
       END FOREACH;
       END FOREACH;
    END
...
CREATE PROCEDURE "informix".sp_fal_busca_producto_deb_cheq_cliente(p_sNumeroCliente CHAR(20), p_skip INT)
    ...
    DEFINE resultado_cuenta_eje         CHAR(30);
    DEFINE resultado_cuenta_inv         CHAR(30);
    ...
    BEGIN
        ...
        END EXCEPTION;
                FOREACH
                    SELECT  DISTINCT chq.producto as numeroProducto,
                                         pr.nombre AS nombreProducto,
                                         chq.cuenta AS cuentaProducto, 
                                        tar.num_tarjeta AS tarjetaProducto
                    ...
                    LEFT JOIN bdicheq:"informix".sc_producto pr ON (chq.producto = pr.producto ) 
                    LEFT OUTER  JOIN bdicheq:"informix".sc_tarjeta tar ON (chq.cuenta = tar.cuenta )
                    WHERE chq.num_cte = p_sNumeroCliente  
                    AND chq.status_cta not in (2) AND tar.status_tar = 'A'
...
CREATE PROCEDURE "informix".sp_fal_busca_producto_deb_cheq_cliente_1(p_sNumeroCliente CHAR(20), p_skip INT)
        ...
        END EXCEPTION;
                FOREACH
                    ...
                    SELECT  DISTINCT chq.producto as numeroProducto,
                        ...
                        LEFT JOIN bdicheq:"informix".sc_producto pr ON (chq.producto = pr.producto ) 
                        LEFT OUTER  JOIN bdicheq:"informix".sc_tarjeta tar ON (chq.cuenta = tar.cuenta )
                        LEFT JOIN bdicheq:"informix".sc_maeinstrucc mae ON (chq.cuenta = mae.cuenta ) 
                        INNER JOIN bdicheq:"informix".sc_maechq cd ON cd.cuenta = mae.cuentadep
                        INNER JOIN fal_control_tramite con ON con.cuenta_cliente_fallecido = chq.cuenta
                        WHERE chq.num_cte = p_sNumeroCliente 
...
CREATE PROCEDURE "informix".sp_fal_busca_producto_deb_cheq_cliente_2(p_sNumeroCliente CHAR(20), p_skip INT)
...
CREATE PROCEDURE "informix".sp_fal_busca_producto_deb_cheq_cliente_3(p_sNumeroCliente CHAR(20), p_skip INT)
...
CREATE PROCEDURE "informix".sp_fal_busca_producto_pcuenta_deb_cte_fallecido(p_sNumCliente CHAR(30), psCuentaBeneficiario CHAR(30))

     RETURNING  
                        CHAR(6) AS numeroProducto,
                        CHAR(60) AS nombreProducto, 
                        CHAR(60) AS cuentaProducto, 
                        CHAR(30) AS estatus, 
                        CHAR(100) AS motivo,
                        MONEY(18,2) AS montoCargo,
                        MONEY(18,2) AS montoCalculado,
                        MONEY(18,2) AS montoPorcentaje,
                        MONEY(18,2) AS montoOriginal,
                        MONEY(18,2) AS montoActual;

    --definicion de variables--     
    DEFINE resultado_numeroProducto     CHAR(6);
    DEFINE resultado_nombreProducto     CHAR(60);
    DEFINE resultado_cuentaProducto     CHAR(60);
    DEFINE resultado_estatus            CHAR(30);
    DEFINE resultado_motivo             CHAR(100);
    DEFINE resultado_montoCargo         MONEY(18,2); 
    DEFINE resultado_montoCalculado     MONEY(18,2);
    DEFINE resultado_montoPorcentaje    MONEY(18,2);
    DEFINE resultado_montoOriginal      MONEY(18,2);
    DEFINE resultado_montoActual        MONEY(18,2);
    ...
    LET resultado_estatus = '';
    LET resultado_motivo = '';
    LET resultado_montoCargo = 0.0;
    LET resultado_montoCalculado = 0.0;
    LET resultado_montoPorcentaje = 0.0;
    LET resultado_montoOriginal = 0.0;
    LET resultado_montoActual = 0.0;
    SET ISOLATION TO DIRTY READ;
    ...
    BEGIN
        ...
        ON EXCEPTION
            ...
            IF iSqlErr <> 0 THEN
                ...
                LET resultado_montoOriginal = 0.0;
                LET resultado_montoActual = 0.0;
                RETURN resultado_numeroProducto,resultado_nombreProducto, resultado_cuentaProducto, resultado_estatus,resultado_motivo,
                       resultado_montoCargo, resultado_montoCalculado, resultado_montoPorcentaje, resultado_montoOriginal, resultado_montoActual;
            END IF;
        END EXCEPTION;
        ...
        FOREACH
                ...
                SELECT DISTINCT qc.producto as numeroProducto, 
                                                ...
                                                CASE WHEN(ct.monto_cargo IS NULL) THEN 0 ELSE ct.monto_cargo END as cargo,
                                                CASE WHEN(ct.monto_calculado IS NULL) THEN 0 ELSE ct.monto_calculado END as calculado,
                                                CASE WHEN(ct.monto_porcentaje IS NULL) THEN 0 ELSE ct.monto_porcentaje END as porcentaje,
                                                CASE WHEN(ct.monto_original IS NULL) THEN 0 ELSE ct.monto_original END as original,
                                                CASE WHEN(ct.monto_actual IS NULL) THEN 0 ELSE ct.monto_actual END as actual
                INTO resultado_numeroProducto,resultado_nombreProducto,resultado_cuentaProducto, resultado_estatus,resultado_motivo,
resultado_montoCargo, resultado_montoCalculado, resultado_montoPorcentaje, resultado_montoOriginal, resultado_montoActual
                FROM bdicheq:"informix".sc_maechq qc
                LEFT JOIN bdicheq:"informix".sc_producto pr ON (qc.producto = pr.producto ) 
                --LEFT JOIN bdicheq:sc_mae_estatus mst ON mst.cod_estatus=qc.pky_estatus_cuenta 
                LEFT JOIN fal_cat_estatus_cuenta stc ON (qc.status_cta = stc.pky_estatus_cuenta)
                LEFT JOIN bdicheq:"informix".sc_bloqueo cb ON (qc.motivo = cb.codigo)
                LEFT JOIN bdicheq:"informix".sc_maeinstrucc mae ON (qc.cuenta = mae.cuenta )  
                LEFT JOIN fal_control_tramite ct ON (ct.cuenta_cliente_fallecido = qc.cuenta )  
                INNER JOIN acl_tipo_producto ap ON (qc.producto=ap.producto)
                WHERE qc. num_cte = p_sNumCliente and ct.cuenta_beneficiario=psCuentaBeneficiario
                AND ap.tipo_producto=2
            RETURN resultado_numeroProducto,resultado_nombreProducto,resultado_cuentaProducto, resultado_estatus, resultado_motivo, 
                   resultado_montoCargo, resultado_montoCalculado, resultado_montoPorcentaje, resultado_montoOriginal, resultado_montoActual
                   WITH RESUME;
        END FOREACH;
...
CREATE PROCEDURE "informix".sp_fal_cancelacion_cuenta_debito( pEmpresa  CHAR(3), pCuenta   CHAR(20), pMotivo   CHAR(2), pPromotor CHAR(8), pSucursal CHAR(4))
...
RETURNING CHAR(6)     AS codigoRetorno,
  ...
  DEFINE saldo_actual_cuenta MONEY(14,2);
  DEFINE iSqlErr INTEGER;
  DEFINE resultado_codigo_retorno CHAR(3);
  DEFINE resultado_codigo_descripcion CHAR(50);
    -- VARIABLES OBTENIDAS DEL SP DE CANCELACION DE CUENTAS
  DEFINE codeRet CHAR(5);
  DEFINE codeRet2 CHAR(5);
  DEFINE mensajeCodeRet CHAR(80);
  DEFINE folioCancelacion CHAR(22);
  DEFINE dFechaHoy DATE;
  DEFINE cod_ret_bloq char(3); 
  DEFINE resultado_estatus INTEGER;

  DEFINE resultado_num_cte CHAR(9);
  DEFINE resultado_codRet_can_token CHAR(5);

  DEFINE resultado_cta_ordenante CHAR(20);
  DEFINE resultado_msg_cancela_porta CHAR(100);
  DEFINE resultado_codRet_cancela_porta CHAR(5);

  LET codigoRetorno = '';
  LET mensajeRetorno = '';
  LET saldo_actual_cuenta = 0;
  LET iSqlErr = '';
  LET resultado_codigo_retorno = '';
  LET resultado_codigo_descripcion = '';

  LET codeRet = '';
  LET codeRet2 = '';
  LET mensajeCodeRet = '';
  LET folioCancelacion = '';
  LET dFechaHoy = TODAY;
  LET cod_ret_bloq = '';
  LET resultado_num_cte = '';
  LET resultado_codRet_can_token = '';
  LET resultado_msg_cancela_porta = '';
  LET resultado_codRet_cancela_porta = '';
  LET resultado_cta_ordenante = '';

    --SET DEBUG FILE TO "/home/rtechno/logSPFallecidos/cancelacionCuentaDebito_"||TRIM(pCuenta)||"_34.out"; 
    --TRACE ON;
    SET ISOLATION TO DIRTY READ;
    SET LOCK MODE TO WAIT 3;
  ...
  BEGIN   
    ...
    END IF
      ...
      IF TRIM(pEmpresa) = '' OR TRIM(pCuenta) = '' OR TRIM(pMotivo) = '' OR TRIM(pPromotor) = '' OR TRIM(pSucursal) = '' THEN
          LET codigoRetorno       = '00001';                       -- CODIGO DEFINIDO
          LET mensajeRetorno      = 'Información incompleta.';  -- MENSAJE RETORNO     
      ...
      END IF
        ...
        SELECT qc.sdo_actual, status_cta, num_cte      
        INTO saldo_actual_cuenta, resultado_estatus,resultado_num_cte
        FROM bdicheq:"informix".sc_maechq qc                                    
        WHERE qc.cuenta = pCuenta;

        
      IF saldo_actual_cuenta = 0 THEN         
        
        IF resultado_estatus != 1 THEN
          CALL bdicheq:"informix".bloqueo_cta(pEmpresa,pCuenta,0,'00',0,today,pPromotor,'4469','07','A','12','Z' )
          RETURNING codeRet,codeRet2;

          IF TRIM(codeRet) != '000' THEN
              LET codigoRetorno       = codeRet;
              LET mensajeRetorno      = 'Error en SP: bloqueo_cta : No se pudo activar la cuenta. Codigo: ' || codeRet2;                                        
              RETURN codigoRetorno, mensajeRetorno;
          END IF
        END IF
        
      -- CANCELACION DE SERVICIOS PORTABILIDAD
      
       FOREACH
        
        SELECT cuenta_abono 
       INTO resultado_cta_ordenante
       FROM bdicheq:"informix".sc_portabilidadnomina p
            INNER JOIN  bdicheq:"informix".sc_portaestatus s  ON p.estatus = s.estatus
            WHERE p.cliente=resultado_num_cte
        and s.descripcion = 'ACTIVO'

        CALL bdicheq:sp_portabcancela(resultado_num_cte,TRIM(resultado_cta_ordenante),'OFI',pSucursal,pPromotor)
        RETURNING resultado_codRet_cancela_porta, resultado_msg_cancela_porta;  
      ...
      -- CANCELACION DE SERVICIOS TOKEN
      CALL bdibpi:sp_cancelarservicio_bpi(pEmpresa,resultado_num_cte,pPromotor,'01')
      RETURNING resultado_codRet_can_token;

        -- CANCELACION DE LA CUENTA
      CALL bdicheq:"informix".sp_cancelactachq(pEmpresa,pCuenta,pMotivo,pPromotor,pSucursal)
      RETURNING codeRet,codeRet2,mensajeCodeRet,folioCancelacion;

      IF codeRet = '069' THEN
        SELECT descripcion,codigo_retorno
        INTO resultado_codigo_descripcion,resultado_codigo_retorno
        FROM bdinteg:"informix".si_codret
        Where codigo_retorno = codeRet;

        LET codigoRetorno = '069';
        LET mensajeRetorno = resultado_codigo_descripcion;
              
        RETURN codigoRetorno, mensajeRetorno;
    
      ELSE
        CALL bdicheq:"informix".bloqueo_cta('001',TRIM(pCuenta), '0', '04', 3, dFechaHoy, pPromotor, '', '11', 'S', '12', 'Z' )
        RETURNING cod_ret_bloq,codeRet2;

        LET codigoRetorno = codeRet;
        LET mensajeRetorno = 'Ocurrio un error en la cancelación de la cuenta.';
    ...
    ELSE
          ...
          LET codigoRetorno       = '00002';                       -- CODIGO DEFINIDO
          LET mensajeRetorno      = 'La cuenta tiene saldo. No se ha realizado la cancelación';   -- MENSAJE RETORNO
          RETURN codigoRetorno, mensajeRetorno;
...
CREATE PROCEDURE "informix".sp_fal_consulta_ciudades(p_skip INT, p_Estado CHAR(2))
    ...
    DEFINE resultado_pais           CHAR(4);
        ...
        DEFINE resultado_claveCiudad    CHAR(4);
        DEFINE resultado_nombreCiudad   CHAR(30);
        DEFINE iSqlErr                  INTEGER;
        
     -- Inicializacion de las variables.
    LET resultado_pais = '';
    LET resultado_numeroEstado = '';
    LET resultado_claveCiudad = '';
    ...
    BEGIN
        ...
        ON EXCEPTION
                ...
                IF iSqlErr <> 0 THEN
                    ...
                    LET resultado_claveCiudad = '';
                    LET resultado_nombreCiudad = '';
                    RETURN resultado_pais, resultado_numeroEstado, resultado_claveCiudad, resultado_nombreCiudad;
                END IF;
        END EXCEPTION;

        FOREACH       
            SELECT SKIP p_skip DISTINCT pais, estado, ciudad, nombre
              INTO resultado_pais, resultado_numeroEstado, resultado_claveCiudad, resultado_nombreCiudad
              FROM bdinteg:"informix".si_ciudades 
              WHERE estado = p_Estado
              ORDER BY pais asC, estado asC, nombre asC
              RETURN resultado_pais, resultado_numeroEstado, resultado_claveCiudad, resultado_nombreCiudad WITH RESUME;
        END FOREACH;
    END
END PROCEDURE
DOCUMENT
...
CREATE PROCEDURE "informix".sp_fal_liquidacion_cuenta_debito(p_idSolicitud INTEGER, p_cta_cliente CHAR(20), p_cta_beneficiario CHAR(20), p_usuario char(8))
  RETURNING CHAR(6) as codigoRetorno,
            CHAR(250) as mensajeRetorno,
  ...
  --DEFINE resultado_fecha_vencimiento            DATETIME;
  ...
  -- 4) NUMERO DE DOCUMENTOS DIGITALIZADOS DEL BENEFICIARIO
...
--  LET resultado_fecha_vencimiento           = DATE(1);
  ...
  --SET DEBUG FILE TO "/home/rtechno/logSPFallecidos/liquidacionCuentaDebito_"||p_idSolicitud||"_"||TRIM(p_cta_beneficiario)||"_34.out";
  --TRACE ON;
  SET ISOLATION TO DIRTY READ;
  SET LOCK MODE TO WAIT 3;
  ...
  BEGIN
    ...
    --- PARA FINES DE PRUEBAS CON DOCUMENTOS DIGITALIZADOS
    ...
    IF resultado_pky_control_tramite_cuenta != 0 THEN -- VALIDACION DE PROCESO DE CUENTA
      ...
      IF p_idSolicitud is null OR TRIM(p_cta_cliente) = '' OR TRIM(p_cta_beneficiario) = '' OR TRIM(p_usuario) = '' THEN
        ...
        VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Error liquidación: Parámetros incorrectos.',sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
      ...
      IF TRIM(resultado_estatus_cuenta_beneficiario) not in (1,4,5) THEN
        ...
        -- HISTORICO DE LA SOLICITUD
        INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
        VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Error liquidación: El estatus de la cuenta del beneficiario no es ACTIVA: ' || resultado_estatus_cuenta_beneficiario,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);

        RETURN codigoRetorno,mensajeRetorno,tipoAccion,cuentaBeneficiario,cuentaClienteFallecido,codigoRetornoCancelacion,mensajeRetornoCancelacion,nombreBeneficiario;
      END IF


      -- 3) SE VERIFICA QUE SE PUEDA TRAMITAR EL PAGO, SI LA DOCUMENTACION DEL CLIENTE FALLECIDO ESTA COMPLETA
      ...
      --==== DESCOMENTAR PARA FINES DE PRUEBAS SIN DIGITALIZAR DOCUMENTACION
      ...
      IF v_numero_documentos_digitalizados_fallecido < v_numero_documentos_necesarios_fallecido AND v_numero_documentos_digitalizados_fallecido != 0 THEN
        ...
        LET codigoRetorno       = '000004';                       -- CODIGO DEFINIDO
        ...
        LET nombreBeneficiario = resultado_nombreBeneficiario;
         ...
         -- HISTORICO DE LA SOLICITUD
        INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
        VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Error: Se tendrá un plazo de 30 dí­as para digitalizar, de lo contrario se cancelará el proceso.',sysdate,resultado_foliocsuac,'PROCESO',resultado_pky_usuario,p_usuario);

        RETURN codigoRetorno,mensajeRetorno,tipoAccion,cuentaBeneficiario,cuentaClienteFallecido,codigoRetornoCancelacion,mensajeRetornoCancelacion,nombreBeneficiario;

      END IF

      IF resultado_estatus_cuenta_cliente_fallecido = 3 AND resultado_motivo <> '04' THEN -- EN CASO DE PREBLOQUEO

        LET codigoRetorno       = '000045';
        LET mensajeRetorno      = 'Su folio se ha enviado a Área interna.';
        LET tipoAccion          = '0';
        LET cuentaBeneficiario  = resultado_num_cta_beneficiario;
        LET cuentaClienteFallecido = resultado_num_cta_cliente;
        ...
        -- HISTORICO DE LA SOLICITUD
        INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
        VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Error: El estatus de la cuenta del CLIENTE no es ACTIVA: ' || resultado_estatus_cuenta_beneficiario,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);

        CALL "informix".sp_fal_liquidacion_asignar_analista(resultado_fky_usuario_analista,p_idSolicitud, resultado_num_cta_cliente, p_cta_beneficiario, p_usuario,resultado_nombreBeneficiario,resultado_pky_control_tramite_cuenta)
      ...
      -- 5) CONSULTA DEL SALDO DE LA CUENTA DEL CLIENTE
        IF resultado_estatus_cuenta_cliente_fallecido = '6' THEN

          CALL "informix".sp_fal_liquidacion_asignar_analista(resultado_fky_usuario_analista,p_idSolicitud, resultado_num_cta_cliente, p_cta_beneficiario, p_usuario,resultado_nombreBeneficiario,resultado_pky_control_tramite_cuenta)
          RETURNING codigoRetorno,mensajeRetorno,tipoAccion,cuentaBeneficiario,cuentaClienteFallecido,codigoRetornoCancelacion,mensajeRetornoCancelacion,nombreBeneficiario;

          LET codigoRetorno       = '000003';
          LET mensajeRetorno      = 'Su folio se ha enviado a Área interna.';
          LET tipoAccion          = '0';
          LET cuentaBeneficiario  = resultado_num_cta_beneficiario;
          LET cuentaClienteFallecido = resultado_num_cta_cliente;
          ...
          INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
          VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Liquidación: Se ha asignado un analista por cuenta CONCENTRADA.',sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);

          RETURN codigoRetorno,mensajeRetorno,tipoAccion,cuentaBeneficiario,cuentaClienteFallecido,codigoRetornoCancelacion,mensajeRetornoCancelacion,nombreBeneficiario;

        END IF

        -- VALIDAR QUE SEA EL ULTIMO BENEFICIARIO
        -- CALCULO DE TOTAL DE BENEFICIARIOS
      ...
      -- 8) OBTENER EL MONTO PARA EL BENEFICIARIO
        IF monto_pago_bene = 0 THEN -- EL SALDO A LIQUIDAR ES 0
          ...
          IF resultado_estatus_cuenta_cliente_fallecido <> 2 THEN
            ...
            WHERE  pky_cuenta_cliente_fallecido = p_cta_cliente;

            update fal_control_tramite SET exitoso=1, fky_estatus_corporativo = 6 , fky_estatus_sucursal = 3, monto_cargo = monto_pago_bene, tramita_analisis = 1
            where pky_control_tramite = resultado_pky_control_tramite_cuenta;

            -- VALIDA SI SE PUEDE CANCELAR LA CUENTA:
            CALL "informix".sp_fal_cancelacion_cuenta_debito( p_Empresa, TRIM(resultado_num_cta_cliente),motivo_cancelacion_debito, p_usuario, TRIM(resultado_num_sucursal))
            ...
            IF cod_resp_cancelacion_debito = '069' THEN
              ...
              UPDATE fal_control_tramite SET fecha_cancelacion = sysdate, tramita_analisis = 1
              WHERE cuenta_cliente_fallecido = p_cta_cliente;

              update fal_control_tramite SET fky_estatus_corporativo = 6, fky_estatus_sucursal=3, tramita_analisis = 1
              where cuenta_cliente_fallecido = p_cta_cliente
              AND fky_estatus_corporativo=2
              AND fky_estatus_sucursal=2;

              -- SI SE PUDO REALIZAR LA CANCELACION DE LA CUENTA
              LET codigoRetorno       = '000005';                       -- CODIGO DEFINIDO
              ...
              INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
              VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Liquidación: La cuenta se ha procesado correctamente, la cuenta del cliente fallecido se ha cancelado. Cod: ' || cod_resp_cancelacion_debito || ' Respuesta: ' || msj_resp_cancelacion_debito,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
            ...
            ELSE
              ...
              INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
              VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Liquidación: La cuenta se ha procesado correctamente. Cod: ' || cod_resp_cancelacion_debito || ' Respuesta: ' || msj_resp_cancelacion_debito,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
          ...
          ELSE
            ...
            INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
            VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Liquidación: La cuenta se ha procesado correctamente, la cuenta del cliente fallecido se ha cancelado.',sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
        ...
        -- ACCIONES PARA LA REGLA DE NEGOCIO
        -- resultado_accion_cumple = 1 = 



        -- 10) VALIDACION SI ES ACCION AUTOMATICA
        -- AUTOMATICO CUANDO EL RANGO ES MENOR
        -- IF resultado_accion = 2 THEN -- FLUJO AUTOMATICO
        --IF resultado_rango_inferior = 1 THEN
        IF resultado_accion_cumple = 1 THEN
          -- LA ACCION ES AUTOMATICA
          -- SE REALIZA EL PAGO AL BENEFICIARIO
          -- 10.1 SE DESBLOQUEA LA CUENTA DEL CLIENTE PARA PODER REALIZAR EL CARGO
          -- EN CASO DE TENER MONTO BLOQUEADO
          IF resultado_estatus_cuenta_cliente_fallecido = 3 AND resultado_motivo = '04' AND resultado_saldo_congelado > 0 THEN -- EN CASO DE TENER MONTO BLOQUEADO
          ...
          IF resultado_estatus_cuenta_cliente_fallecido = 3 AND resultado_motivo = '04' THEN
            --------------------------------------------------------------------------
            ------ SP POR DEFINIR ---------------------------------------------------
            -- [bdicheq:"informix".bloqueo_cta]
            ...
            RETURNING codret_blqcta,menret_blqcta;
            --######################################################################################################################################
            -- ASIGNACION POR PRUEBA DE FLUJO
            ...
            IF TRIM(codret_blqcta) != '000' THEN
              ...
              CALL bdicheq:"informix".bloqueo_cta('001',TRIM(resultado_num_cta_cliente), '0', '04', 3, today, p_usuario, '', '11', 'S', '12', 'Z' )
              RETURNING codret_blqcta,menret_blqcta;
              -- SE REALIZA LA ASIGNACION DE ANALISTA
              CALL sp_fal_liquidacion_asignar_analista(resultado_fky_usuario_analista,p_idSolicitud, p_cta_cliente, p_cta_beneficiario, p_usuario,resultado_nombreBeneficiario,resultado_pky_control_tramite_cuenta)
              RETURNING codigoRetorno,mensajeRetorno,tipoAccion,cuentaBeneficiario,cuentaClienteFallecido,codigoRetornoCancelacion,mensajeRetornoCancelacion,nombreBeneficiario;
              ...
              INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
              VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Error liquidación: No se pudo activar la cuenta del cliente. La liquidación de recursos se hará en Central. Cod: ' || codret_blqcta || ' Respuesta: ' || menret_blqcta,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);

              RETURN codigoRetorno,mensajeRetorno,tipoAccion,cuentaBeneficiario,cuentaClienteFallecido,codigoRetornoCancelacion,mensajeRetornoCancelacion,nombreBeneficiario;

            END IF
          END IF
          -- VALIDACION SI NO SE PUDO ACTIVAR LA CUENTA

          -- 10.2 SE GENERA EL FOLIO SUC
          SELECT substr((sysdate HOUR TO SECOND),1,2) || substr((sysdate HOUR TO SECOND),4,2) || substr((sysdate HOUR TO SECOND),7,2)
          INTO p_fecha_folio
          ...
          LET num_tarjeta_cliente = (
            select 
              case
                when st.num_tarjeta is null then ''
                else st.num_tarjeta
              end
            --nvl(st.num_tarjeta, '')
          ...
          IF resultado_cargo_bandera != 1 THEN -- VALIDACION EN 0 DEL CARGO AL CLIENTE
            ...
            -- 10.5 VALIDACION DE LA EJECUCION DEL SP DE CARGO
            IF codret_cargo_ref = '000' THEN
              ...
              IF vcodret_abono = '000' THEN
                ...
                UPDATE fal_control_tramite SET exitoso = 1, monto_cargo = monto_pago_bene, fky_estatus_corporativo = 6 , fky_estatus_sucursal = 3, tramita_analisis = 1
                WHERE pky_control_tramite = resultado_pky_control_tramite_cuenta;
                -- SE BLOQUEA DE NUEVO LA CUENTA DEL CLIENTE
                CALL bdicheq:"informix".bloqueo_cta('001',TRIM(resultado_num_cta_cliente), '0', '04', 3, today, p_usuario, '', '11', 'S', '12', 'Z' )
                RETURNING codret_blqcta,menret_blqcta;
                ...
                IF TRIM(cod_resp_cancelacion_debito) = '069' THEN

                  -- SE ACTUALIZA LA FECHA DE CANCELACION, ESTATUS CORP, ESTATUS SUC, ESTATUS GENERAL
                  update fal_control_tramite SET fecha_cancelacion = sysdate
                  where cuenta_cliente_fallecido = p_cta_cliente;

                  update fal_control_tramite SET fky_estatus_corporativo = 6, fky_estatus_sucursal=3, tramita_analisis = 1, fecha_cancelacion = sysdate
                  where cuenta_cliente_fallecido = p_cta_cliente
                  AND fky_estatus_corporativo=2
                  AND fky_estatus_sucursal=2;
                  ...
                  INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
                  VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Liquidación: La cuenta se ha procesado correctamente. Se ha cancelado la cuenta del cliente fallecido. Cod: ' || cod_resp_cancelacion_debito || ' Respuesta: ' || msj_resp_cancelacion_debito,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
                ...
                ELSE
                  ...
                  -- SE ACTUALIZA LA FECHA DE CANCELACION, ESTATUS CORP, ESTATUS SUC, ESTATUS GENERAL
                  --update fal_control_tramite SET fky_estatus_corporativo = 2 , fky_estatus_sucursal = 2, tramita_analisis = 1
                  --where pky_control_tramite = resultado_pky_control_tramite_cuenta;
                  ...
                  INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
                  VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Liquidación: La cuenta se bloquea. Cod: ' || codret_blqcta || ' Respuesta: ' || menret_blqcta,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
              ...
              ELSE
                ...
                INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
                VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Error liquidación: No se pudo abonar a la cuenta del beneficiario. Cod: ' || vcodret_abono || '/' || codret_blqcta || ' Respuesta: ' || menret_blqcta,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
            ...
            ELSE
              ...
              IF resultado_saldo_congelado > 0 THEN
                   -- SE BLOQUEA DE NUEVO LA CUENTA DEL CLIENTE
                   CALL bdicheq:"informix".bloqueo_cta(p_Empresa, TRIM(resultado_num_cta_cliente), resultado_saldo_congelado, '04', 3, today, p_usuario, '', '07', 'A', '12', 'Z')
                   RETURNING codret_blqcta,menret_blqcta;
              ELSE
                  -- SE BLOQUEA DE NUEVO LA CUENTA DEL CLIENTE FALLECIDO
                  CALL bdicheq:"informix".bloqueo_cta('001',TRIM(resultado_num_cta_cliente), '0', '04', 3, today, p_usuario, '', '11', 'S', '12', 'Z' )
              ...
              INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
                VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Error liquidación: No se pudo realizar el cargo al cliente. Cod: ' || codigoRetorno || '/' || codret_blqcta || ' Respuesta: ' || menret_blqcta,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
          ...
          ELSE
            ...
            IF resultado_aplicado IS NULL THEN
              ...
              -- VALIDACION SI SE PUDO HACER EL ABONO AL BENEFICIARIO
                INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
                VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Liquidación: Abono. Cod: ' || vcodret_abono || '/' ,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);

              IF TRIM(vcodret_abono) = '000' THEN

                -- ACTUALIZA LA TABLA DE BENEFICIARIOS
                UPDATE fal_beneficiario  SET aplicado = 1, monto_aplicado = monto_pago_bene, fecha_tramite = sysdate
                WHERE  fky_control_tramite = resultado_pky_control_tramite_cuenta;
                ...
                INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
                VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Liquidación: La cuenta se ha procesado correctamente. Cod: ' || cod_resp_cancelacion_debito || '/' || codret_blqcta || '/' || codigoRetorno || ' Respuesta: ' || msj_resp_cancelacion_debito,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);

                RETURN codigoRetorno,mensajeRetorno,tipoAccion,cuentaBeneficiario,cuentaClienteFallecido,codigoRetornoCancelacion,mensajeRetornoCancelacion,nombreBeneficiario;

             ELSE --NO SE HACE EL ABONO
                ...
                INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
                VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Error liquidación: No se pudo abonar a la cuenta del beneficiario. Cod: ' || vcodret_abono || '/' || codret_blqcta || '/' || codigoRetorno || ' Respuesta: ' || menret_blqcta,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);

                RETURN codigoRetorno,mensajeRetorno,tipoAccion,cuentaBeneficiario,cuentaClienteFallecido,codigoRetornoCancelacion,mensajeRetornoCancelacion,nombreBeneficiario;

              END IF --VALIDA SI SE HIZO EL ABONO
            ...
            ELSE --SI YA SE HIZO EL CARGO Y EL ABONO
                 ...
                 RETURNING cod_resp_cancelacion_debito, msj_resp_cancelacion_debito;
                  ...
                  IF cod_resp_cancelacion_debito != '069' THEN
                    LET mensajeRetorno      = 'No se pudo cancelar la cuenta.';
                  ...
                  INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
                  VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Sucursal - Liquidación: La cuenta se ha procesado correctamente. Cod: ' || cod_resp_cancelacion_debito || '/' || codret_blqcta || '/' || codigoRetorno || ' Respuesta: ' || menret_blqcta,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
        ...
        ELSE
          ...
          IF resultado_fky_usuario_analista IS NULL THEN
            ...
            FROM acl_usuario au
              ...
              WHERE au.activo = 1
              AND app.fky_id_permiso in (select pky_id_permiso from acl_permiso WHERE nombre = 'FAL_TABLERO');
            ...
            IF resultado_asign_usuario IS NULL THEN
              ...
              INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
              VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Error liquidación: No hay analistas con los permisos requeridos. Cod: ' || '/' || codret_blqcta || '/' || codigoRetorno || ' Respuesta: ' || menret_blqcta,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
            ...
            ELSE
              ...
              IF resultado_asign_usuario_2 IS NULL THEN
                ...
                -- COMENTAR PARA PRUEBA DE ASIGNACION DE ANALISTA
                ...
                INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
                VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Liquidación: Se asignó un analista. Cod: ' || '/' || codret_blqcta || '/' || codigoRetorno || ' Respuesta: ' || menret_blqcta,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
          ...
          ELSE
            ...
            INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
            VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Liquidación: Analista ya asignado. Cod: ' || '/' || codret_blqcta || '/' || codigoRetorno || ' Respuesta: ' || menret_blqcta,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);
      ...
      ELSE
        ...
        INSERT INTO fal_historico_solicitud (pky_historico_solicitud,fky_solicitud,descripcion,fecha_hora,folio_csuac,accion_realizo,fky_usuario,numero_empleado)
        VALUES (FAL_HISTORICO_SOLICITUD_SEQ.nextval,p_idSolicitud,'Error: Se tendrá un plazo de 30 días para digitalizar, de lo contrario se cancelará el proceso. Cod: ' || codret_blqcta || '/' || codigoRetorno || ' Respuesta: ' || menret_blqcta,sysdate,resultado_foliocsuac,'LIQUIDACION',resultado_pky_usuario,p_usuario);

        RETURN codigoRetorno,mensajeRetorno,tipoAccion,cuentaBeneficiario,cuentaClienteFallecido,codigoRetornoCancelacion,mensajeRetornoCancelacion,nombreBeneficiario;
      END IF

    ELSE -- VALIDACION DE PROCESO DE CUENTA
    ...
    END IF -- VALIDACION DE PROCESO DE CUENTA
...
CREATE PROCEDURE "informix".sp_fal_marcajesitesp(pEmpresa CHAR(3), pTipoProceso SMALLINT,pNumcte CHAR(20), pUsuario CHAR(8))
    ...
    DEFINE cCodretBloqCred CHAR(6);
    DEFINE cCodretBloqDeb  CHAR(6);
    DEFINE cMensajeRetBloqCred CHAR(40);
    DEFINE cMensajeRetBloqDeb CHAR(40);
    ...
    DEFINE resultado_estatus_cuenta CHAR(1);
    ...
    LET cCodretBloqCred = '';
    LET cCodretBloqDeb = '';
    LET cMensajeRetBloqCred = '';
    LET cMensajeRetBloqDeb = '';
   ...
   --SET DEBUG FILE TO "/home/rtechno/logSPFallecidos/marcajeEspecial"||"_F43_"||""||TRIM(pNumcte)||""||"_34.out"; --> TRACE DESDE APP
   --TRACE ON;

        SET ISOLATION TO DIRTY READ;
        SET LOCK MODE TO WAIT 3;

        CALL bdisitesp:"informix".sp_marcajesitesp(pEmpresa,  pTipoProceso, pNumcte, pUsuario)
        RETURNING cCodret, cMensajeRet;
                    
        FOREACH WITH hold
            SELECT numeroProducto, nombreProducto, cuentaProducto, tarjetaProducto
            INTO resultado_numeroProducto, resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta
            FROM TABLE( FUNCTION sp_fal_busca_producto_cred_cliente(pNumcte, 0) )
            AS a(numeroProducto, nombreProducto, cuentaProducto, tarjetaProducto)
            --RETURN resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta WITH RESUME;

            --CALL DBMS_OUTPUT.PUT_LINE('---cuenta de credito bloqueada: ' || resultado_numeroCuenta);
            CALL bdicred:"informix".sp_bloqueocuenta('001', TRIM(resultado_numeroCuenta), 3, '04', pUsuario, 1)
            RETURNING cCodretBloqCred, cMensajeRetBloqCred;

            --LET dCodProc = ',' || TRIM(resultado_numeroCuenta) || '-' || TRIM(cCodret) || ',' || dCodProc;
        END FOREACH;

        FOREACH WITH hold
            SELECT numeroProducto, nombreProducto, cuentaProducto, tarjetaProducto
            INTO resultado_numeroProducto, resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta
            FROM TABLE( FUNCTION sp_fal_busca_producto_cred_cliente_1(pNumcte, 0) )
            AS a(numeroProducto, nombreProducto, cuentaProducto, tarjetaProducto)
            --RETURN resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta WITH RESUME;
        ...
        --call DBMS_OUTPUT.PUT_LINE('------BLOQUEO DE CUENTAS DEBITO------');
        FOREACH WITH hold
            SELECT numeroProducto, nombreProducto, cuentaProducto, tarjetaProducto
            INTO resultado_numeroProducto, resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta
            FROM TABLE( FUNCTION sp_fal_busca_producto_deb_cheq_cliente(pNumcte, 0) )
            AS a(numeroProducto, nombreProducto, cuentaProducto, tarjetaProducto)
            --RETURN resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta WITH RESUME;

            -- SE VERIFICA EL ESTATUS DE LA CUENTA ANTES DE BLOQUEARLA POR TIPO DE FALLECIMIENTO
            SELECT status_cta
            INTO resultado_estatus_cuenta
            FROM bdicheq:"informix".sc_maechq
            WHERE cuenta = resultado_numeroCuenta;

            IF resultado_estatus_cuenta != '3' THEN

                --CALL DBMS_OUTPUT.PUT_LINE('---cuenta de debito bloqueada: ' || resultado_numeroCuenta);
                CALL bdicheq:"informix".bloqueo_cta('001',TRIM(resultado_numeroCuenta), '0', '04', 3, dFechaHoy, pUsuario, '', '11', 'S', '12', 'Z' )
                RETURNING cCodretBloqDeb, cMensajeRetBloqDeb;
  ...
  FOREACH WITH hold
            SELECT numeroProducto, nombreProducto, cuentaProducto, tarjetaProducto
            INTO resultado_numeroProducto, resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta
            FROM TABLE( FUNCTION sp_fal_busca_producto_deb_cheq_cliente_1(pNumcte, 0) )
            AS a(numeroProducto, nombreProducto, cuentaProducto, tarjetaProducto)
            --RETURN resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta WITH RESUME;
        ...
        END FOREACH;    
          ...
          FOREACH WITH hold
            SELECT numeroProducto, nombreProducto, cuentaProducto, tarjetaProducto
            INTO resultado_numeroProducto, resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta
            FROM TABLE( FUNCTION sp_fal_busca_producto_deb_cheq_cliente_2(pNumcte, 0) )
            AS a(numeroProducto, nombreProducto, cuentaProducto, tarjetaProducto)
            --RETURN resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta WITH RESUME;
...
CREATE PROCEDURE "informix".sp_fal_obten_secuencia_folio()
    ...
    BEGIN
        ...
        SELECT SOLICITUD_CONSECUTIVO_SEQ.NEXTVAL
        INTO  secuenciaMax        
        FROM systables WHERE tabid = 1;
        RETURN  secuenciaMax;
...
CREATE PROCEDURE "informix".sp_fal_rep_pagare_vencimiento(p_estatus INTEGER, fechaIni DATE, fechaFin DATE) 

    RETURNING  

        CHAR(11) AS folioCsuac,
        MONEY AS montoReclamado,
        CHAR(120) AS usuarioAsignado, 
        CHAR (120) AS evento,
        CHAR (20) AS numCuenta,
        CHAR (30) AS estatusCorp,
        CHAR (30) AS estatusSucu,
        CHAR (30) AS estatusGene,
        CHAR (50) AS respuestaArea,
        CHAR (20) AS respuestaPredictamen,
        CHAR (9) AS numCliente,
        DATE AS fechaVencimiento;


        DEFINE resultado_folioCsuac CHAR (11);
        DEFINE resultado_montoReclamado MONEY;
        DEFINE resultado_usuarioAsignado CHAR(120);
        DEFINE resultado_evento CHAR (30);
        DEFINE resultado_numCuenta CHAR (20);
        DEFINE resultado_estatusCorp CHAR (30);
        DEFINE resultado_estatusSucu CHAR (30);
        DEFINE resultado_estatusGene CHAR (30);
        DEFINE resultado_respuestaArea CHAR (50);
        DEFINE resultado_respuestaPredictamen CHAR (20);
        DEFINE resultado_respuestanumCliente CHAR (9);
        DEFINE resultado_fechaVencimiento DATE;
        DEFINE enAreaExterna CHAR (1);
        DEFINE pkySolicitud INTEGER;
        DEFINE pkyControlTramite INTEGER;
        DEFINE countAreaExterna INTEGER;
        DEFINE countCorrectos INTEGER;
        DEFINE existePredictamen INTEGER;
        DEFINE iSqlErr INTEGER;

         -- Inicializacion de las variables.
        LET resultado_folioCsuac = '';
        LET resultado_montoReclamado = 0;
        LET resultado_usuarioAsignado = '';
        LET resultado_evento = '';
        LET resultado_numCuenta = '';
        LET resultado_estatusCorp = '';
        LET resultado_estatusSucu = '';
        LET resultado_estatusGene = '';
        LET resultado_respuestaArea = '';
        LET resultado_respuestaPredictamen = '';
        LET resultado_respuestanumCliente = '';
        LET resultado_fechaVencimiento = '';
        LET countAreaExterna = 0;
        LET countCorrectos = 0;
        LET existePredictamen = 0;
        ...
        BEGIN
            ON EXCEPTION
                ...
                IF iSqlErr <> 0 THEN
                    ...
                    LET resultado_respuestanumCliente = '';
                    LET resultado_fechaVencimiento = '';
                    RETURN resultado_folioCsuac, resultado_montoReclamado, resultado_usuarioAsignado, resultado_evento, resultado_numCuenta, resultado_estatusCorp, resultado_estatusSucu, resultado_estatusGene,
                           resultado_respuestaArea, resultado_respuestaPredictamen, resultado_respuestanumCliente, resultado_fechaVencimiento;
                END IF;
            END EXCEPTION;
                    ...
                    IF p_estatus = 0 THEN 
                        ...
                        select sol.folio_csuac, tra.monto_porcentaje,  (SELECT usu.nombre FROM acl_usuario usu WHERE sol.fky_usuario_analista = usu.pky_usuario) as usuario_asignado, eve.nombre as evento, tra.cuenta_cliente_fallecido, 
                        corp.nombre as estatus_corporativo, sucu.nombre as estatus_sucursal, gene.nombre, tra.area_externa, sol.num_cliente, tra.fecha_vencimiento_pagare, sol.pky_solicitud, tra.pky_control_tramite
                        into resultado_folioCsuac, resultado_montoReclamado, resultado_usuarioAsignado, resultado_evento, resultado_numCuenta, resultado_estatusCorp, resultado_estatusSucu, resultado_estatusGene,
                               enAreaExterna, resultado_respuestanumCliente, resultado_fechaVencimiento, pkySolicitud, pkyControlTramite
                        from fal_solicitud sol
                        inner join fal_control_tramite tra on tra.fky_solicitud = sol.pky_solicitud
                        left join fal_cat_evento eve on eve.pky_evento = sol.fky_evento
                        left join fal_cat_estatus_corporativo corp on corp.pky_estatus_corporativo = tra.fky_estatus_corporativo
                        left join fal_cat_estatus_sucursal sucu on sucu.pky_estatus_sucursal = tra.fky_estatus_sucursal
                        left join fal_cat_estatus_general gene on gene.pky_estatus_general = sol.fky_estatus_general
                        where tra.fky_tipo_tramite = 3
                        and sol.fky_estatus_general <> 1
                        and sol.fecha_ingreso BETWEEN fechaIni and fechaFin

                        select first 1 predictamen 
                        into existePredictamen
                        from fal_control_tramite
                        where fky_solicitud =  pkySolicitud
                        and predictamen is not null;

                            IF existePredictamen IS NOT NULL THEN 
                                LET resultado_respuestaPredictamen = 'Aceptado';
                            END IF

                            IF enAreaExterna = '1' THEN 
                                --Obtiene la cantidad de solicitudes a area externa de la cuenta
                                select count(*) 
                                into countAreaExterna
                                from fal_control_tramite tra
                                inner join fal_solicitud_area_externa ext on ext.fky_control_tramite = tra.pky_control_tramite
                                where cuenta_cliente_fallecido = resultado_numCuenta;


                                --Si se ha enviado solamente una vez a area externa, validar que 
                                IF countAreaExterna = 1 THEN

                                    select first 1 'En espera de autorizacion para envio a ' || ar.nombre 
                                    into resultado_respuestaArea
                                    from fal_solicitud_area_externa ex
                                    inner join acl_area ar on ar.pky_area = ex.fky_area
                                    where fky_solicitud = pkySolicitud
                                    and fky_control_tramite = pkyControlTramite;

                                ELSE --Validar si se dio el Vo.Bo de todas las areas a nivel cuenta beneficiario
                                    ...
                                    and ext.correcto = 1;
                                        ...
                                        IF countAreaExterna = countCorrectos THEN 

                                            LET resultado_respuestaArea = 'Con respuesta de todas las areas.';

                                        ELSE 
                                            FOREACH orden_cursor FOR
                                            select first 1 'Con respuesta de areas externas: ' || ar.nombre
                                            into resultado_respuestaArea
                                            from fal_control_tramite tra
                                            inner join fal_solicitud_area_externa ext on ext.fky_control_tramite = tra.pky_control_tramite
                                            left join acl_area ar on ar.pky_area = ext.fky_area
                                            where cuenta_cliente_fallecido = resultado_numCuenta
                                            and ext.correcto = 1
                                            order by ext.pky_solicitud_area_externa
                                            END FOREACH;   
                                        END IF                          
                                END IF
                    ...
                    FOREACH fal_cursor FOR
                        ...
                        left join fal_cat_estatus_general gene on gene.pky_estatus_general = sol.fky_estatus_general
                        where tra.fky_tipo_tramite = 3
                        and sol.fky_estatus_general = p_estatus
                        and sol.fecha_ingreso BETWEEN fechaIni and fechaFin
                    ...
                    FOREACH fal_cursor FOR
                        ...
                        left join fal_cat_estatus_sucursal sucu on sucu.pky_estatus_sucursal = tra.fky_estatus_sucursal
                        left join fal_cat_estatus_general gene on gene.pky_estatus_general = sol.fky_estatus_general
                        WHERE (SELECT count(*) FROM  fal_control_tramite tra WHERE tra.exitoso=1 AND tra.fky_solicitud = sol.pky_solicitud) >= 1
