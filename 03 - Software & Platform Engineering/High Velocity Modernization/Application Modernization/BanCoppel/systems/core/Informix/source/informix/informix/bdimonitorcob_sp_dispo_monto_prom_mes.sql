CREATE PROCEDURE "informix".sp_dispo_monto_prom_mes(p_anio INTEGER, p_mes INTEGER, p_num_credito CHAR(20), p_origen INTEGER)
       RETURNING CHAR(6), CHAR(150);

-- EL PARAMETRO p_origen
-- 1 = EL SUSARIO INDICA LA FECHA A EJECUTAR
-- 2 = LA EJECUCION SE REALIZA CON LA FECHA ACTUAL
       
DEFINE v_disposicion       CHAR(3);
...
DEFINE v_disposicion_prom  CHAR (3);
DEFINE v_contador          INTEGER;
DEFINE promedio            DECIMAL;
DEFINE dMontoAnual         DECIMAL(18,2);
DEFINE v_disposicion_anual CHAR(3);
...
BEGIN
    ...
    SELECT valor_alfabetico INTO v_disposicion FROM mc_configuracion WHERE empresa = vempresa
    AND grupo_parametro = 'INDICADORES' AND num_parametro = 1;

    SELECT valor_alfabetico INTO v_disposicion_prom FROM mc_configuracion WHERE empresa = vempresa
    AND grupo_parametro = 'INDICADORES' AND num_parametro = 2;

    SELECT valor_alfabetico INTO v_disposicion_anual FROM mc_configuracion WHERE empresa = vempresa
    AND grupo_parametro = 'INDICADORES' AND num_parametro = 3;

SET ISOLATION TO dirty READ;
FOREACH

    SELECT {+INDEX(bdimonitorcob:mc_masterestad mc_masterestad_uc1), +INDEX(bdicred:sd_movhis inx_movhis5)} a.num_credito, NVL(SUM (b.monto), 0), NVL(count (b.monto), 0) 
    ...
    AND b.fecha_mov BETWEEN vfch_inicio AND vfch_fin
    AND b.codigo_fun IN ('002')
    AND b.codigo_ref IN ('30','34','35','36','40','41','42','50')
    --AND b.fecha_mov between '2009-10-21' AND '2009-11-20'
   ...
   IF (p_mes = 01) THEN
      ...
      WHERE num_credito = v_num_credito
        AND id_conceptom = v_disposicion
        AND anio = p_anio;
 
        UPDATE {+INDEX{bdimonitorcob:mc_detestadmes idx_detestadmes)} mc_detestadmes
         SET ene = promedio, fecha_ejecucion = CURRENT
      WHERE num_credito = v_num_credito
        AND id_conceptom = v_disposicion_prom
        AND anio = p_anio;
   ...
   --actualizar el monto de la tabla mc_detestadanual si el monto del mes es mayor al monto anual para el concepto '200'
    ...
    WHERE anio = p_anio
    AND id_conceptoa = v_disposicion_anual 
