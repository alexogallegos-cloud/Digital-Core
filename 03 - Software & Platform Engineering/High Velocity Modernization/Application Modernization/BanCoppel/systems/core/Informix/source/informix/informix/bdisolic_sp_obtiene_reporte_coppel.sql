CREATE PROCEDURE "informix".sp_obtiene_reporte_coppel()
RETURNING CHAR(6);
...
LET cFechaHoy		= '';
LET cNomArch		= '';
LET cNomArch2		= 'Solicitudes_reevaluadas_DDMMAA';
LET cRutaArchivo	= '';
LET cRutaArchivo2	= '';
LET cMes			= '';
LET cDia			= '';
...
BEGIN
	...
	--SET DEBUG FILE TO "/home/sysifx/JesusTASF/sp_obtiene_reporte_coppel.out";
	--TRACE ON;
	...
	SELECT valor INTO cNomArch FROM bdisolic:"informix".ss_param WHERE secuencia = '456';
	...
	IF TRIM(NVL(cRuta,'')) != '' AND TRIM(NVL(cNomArch,'')) != '' THEN
		...
		LET cNomArch2 = REPLACE(cNomArch2,'DD',cDia);
		LET cNomArch2 = REPLACE(cNomArch2,'MM',cMes);
		LET cNomArch2 = REPLACE(cNomArch2,'AA',cAnio);
		LET cNomArch2 = TRIM(cNomArch2) || '.txt';
		
		
		LET cRutaArchivo = TRIM(cRuta) || TRIM(cNomArch);
		LET cRutaArchivo2 = TRIM(cRuta) || TRIM(cNomArch2);		
		...
		--Elimina archivo
		LET cSQL2 = 'rm -rf ' || TRIM(cRutaArchivo2);		
		SYSTEM cSQL2;	
		--Crea archivo vacio
		LET cSQL2 = 'touch ' || TRIM(cRutaArchivo2);
		SYSTEM cSQL2;

		FOREACH
			SELECT TRIM(a.numcte),TRIM(a.num_solicitud)
			INTO cNumCte,cNumSolicitud
			FROM ss_solicitudes AS a
			JOIN ss_solic_rt AS e ON a.num_solicitud = e.num_solicitud 
			WHERE  e.reevaluado <> '0' AND e.fecha_reevaluacion = TODAY
			...
			LET cSQL2 = 'echo "' || TRIM(cNumCte) || ' | ' || TRIM (cNumSolicitud) || '" >> ' || cRutaArchivo2;

				IF cSQL2 IS NULL THEN
					LET cCodRet = '000002';
					RETURN cCodRet;
					EXIT FOREACH;
				ELSE
		...
		FOREACH
			...
			WHERE f.status_solicitud = 'AT' AND a.num_producto = '6500' AND reevaluado = '1'
