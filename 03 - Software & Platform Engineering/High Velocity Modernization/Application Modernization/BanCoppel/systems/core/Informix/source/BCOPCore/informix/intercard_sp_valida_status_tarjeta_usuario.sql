CREATE PROCEDURE "informix".sp_valida_status_tarjeta_usuario(pusuario CHAR(8), ptarjeta CHAR(16))
RETURNING CHAR(5),CHAR(1);

--@comment: Declaracion variables para responder
DEFINE ccodret char(9);
DEFINE error_info CHAR(104);
...
BEGIN
	...
	--SET DEBUG FILE TO '/home/sysifx/Edgar/sp_valida_status_tarjeta_usuario.out';
	--TRACE ON;
	
	SELECT MAX(fechahora) INTO cFecha
	FROM intercard: bitacoracambiosstatustarjeta
	WHERE tarjeta = ptarjeta;
...
END PROCEDURE
DOCUMENT
'OBJETIVO: 	Valida el status de la tarjea por usuario',
'AUTOR:		Edgar alarcon ',
'FECHA : 	07/05/2018',
'SolicitÃ³:  Ivan Manjarrez',
'BD : 		INTERCARD';
