CREATE PROCEDURE "informix".sp_valida_dv_coppel_com
(
	  pNumRef CHAR (12)
)
...
DEFINE vRefPagada		INTEGER;

--SET DEBUG FILE TO '/informix/noe/sp_valida_dv_coppel_com.out';
--TRACE ON;
...
LET vRefPagada	= 0;
	...
	BEGIN
        ...
        LET pNumRef = pNumRef;
		LET pNumRef=LTRIM(pNumRef,'0');


		--SE AGREGA VALIDACION DE INICIO DE NUMERO DE REFERENCIA, SIEMPRE DEBE SER 816 (01SEP17)
		IF SUBSTR(TRIM(pNumRef),1,3) <> '816' THEN
			LET cCod_ret = '00082';
			RETURN cCod_ret;
		END IF;

    
    IF LEN(trim(pNumRef)) = 11 or LEN(trim(pNumRef)) = 12 THEN
    	
			FOR vPosicion = LEN(trim(pNumRef)) - 1  TO 1 LOOP ---LONGITUD 11 o 12
			...
			IF vResultado <> CAST(SUBSTR(pNumRef, LEN(trim(pNumRef)),1) AS INTEGER) THEN ---POSICION 11 o 12
				LET cCod_ret = '00109';
			END IF;
			...
			WHERE numcategoria = '09' AND numconvenio = '013' AND status_cancelado <> 'S' AND referencia1 = pNumRef;
			
			IF vRefPagada > 0 THEN
				LET cCod_ret = '01188';
				RETURN cCod_ret;
			END IF;
...
END PROCEDURE
DOCUMENT
'ACTUALIZA: Noe Medina Ramirez',
'DESCRIPCION: Se modifica para validar referencias tambien de 12 digitos',
'FECHA: 02/05/2019',
'----------------------------------------------------------------',
'ACTUALIZA: Jose Aaron Lopez Ruiz',
'DESCRIPCION: Se implementa validaciÃ³n de referencia ya pagada.',
'----------------------------------------------------------------',
'AUTOR: Noe Medina Ramirez',
'FOLIO: ',
'DESCRIPCION: Valida el digito verificador de Coppel.Com',
'FECHA: 03/03/2017',
'VERSION: 1.0000',
'BD:Bdisac';

CREATE PROCEDURE "informix".sp_app_obtieneinfoidentificacion(pTipoEjec CHAR(1), pCodTipoIdent CHAR(3))
RETURNING
	CHAR(6)  		AS  COD_RET,
...
LET cDescTipoIdentificacion       = "";
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/sp_app_obtieneinfoidentificacion.out';
		--TRACE ON;
...
END PROCEDURE
DOCUMENT
'DESCRIPCION : Procedimiento para Obtener el listado de Identificaciones Validas para Appriza',
'AUTOR : Noe Medina',
'FECHA : 25/Abril/2019',
'BD    : BDISAC',
'VERSION:20190425.1030';

CREATE PROCEDURE "informix".sp_ctrl_ind_ctes_servs
(tipoExec INTEGER, mesanio CHAR(6), pFechaInicial CHAR(10), pFechaFinal CHAR(10))
RETURNING CHAR(5), CHAR(80);
	--Variables de codigos de error y retorno
	DEFINE codReturn CHAR(5);
	...
	DEFINE fechaProceso DATE;
	--Variables para insert en bitacora y ejecucion de otros sp
	DEFINE proceso CHAR(50);
	DEFINE usuario CHAR(10);
	DEFINE spNombre CHAR(50);
	DEFINE spDescripcion CHAR(100);
	DEFINE sqlStatement CHAR(500);
	DEFINE vSP CHAR(100);
	
	LET codReturn = '00000';
	LET cMensaje = 'PROCESO EXITOSO';
	
	LET fechaInicio =  DATE(TODAY - 1 UNITS DAY);
	LET fechaFin = DATE(TODAY - 1 UNITS DAY);
	LET fechaProceso = DATE(TODAY);
	
	LET proceso = 'IND_CTES_SERVS';
	LET usuario = 'informix';
	LET vSP = 'sp_ctrl_ind_ctes_servs';
	

	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	
	--SET DEBUG FILE TO '/ifxsif01/menriquez/sp_ctrl_ind_ctes_servs.out';
	--TRACE ON;
	...
	BEGIN
		ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
			IF iSqlErr <> 0 THEN
				LET cMensaje = "ERROR";
                RETURN iSqlErr, cMensaje;
			END IF;
		END EXCEPTION;
		-- Tipo de ejecuciÃ³n 0, cuando se especifica un
		-- periodo y/o mes y/o el mes anterior
		-- cuando mesanio viene vacio
		IF tipoExec = 0 THEN
			FOREACH SELECT nombre_sp, descripcion_sp INTO spNombre, spDescripcion FROM tb_ctrl_ind_ctes_servs WHERE estatus_exec = 1 ORDER BY prioridad_sp ASC
				LET sqlStatement = cast('echo "EXECUTE PROCEDURE "informix".' || TRIM(spNombre) || '('''||mesanio||''','''||pFechaInicial||''','''||pFechaFinal||''');" > /ifxsif01/Control-M/sac_ctrl_ind_ctes_servs.sql' as CHAR(500));
				SYSTEM trim(sqlStatement);
				
				LET sqlStatement = cast('dbaccess bdisac /ifxsif01/Control-M/sac_ctrl_ind_ctes_servs.sql' as char(500));
                SYSTEM trim(sqlStatement);
				...
				VALUES(proceso, fechaProceso, 0, usuario, SYSDATE, 1, spNombre, spDescripcion);
			END FOREACH;
		END IF;
		
		-- Tipo de ejecuciÃ³n 1, se ejecutan los indicadores que deben ser diario
		IF tipoExec = 1 THEN
			FOREACH SELECT nombre_sp, descripcion_sp INTO spNombre, spDescripcion FROM tb_ctrl_ind_ctes_servs WHERE tipo_exec = 1 AND estatus_exec = 1 ORDER BY prioridad_sp ASC
				LET sqlStatement = cast('echo "EXECUTE PROCEDURE "informix".' || TRIM(spNombre) || '('''||mesanio||''','''||TO_CHAR(fechaInicio,'%m/%d/%Y')||''','''||TO_CHAR(fechaFin,'%m/%d/%Y')||''');" > /ifxsif01/Control-M/sac_ctrl_ind_ctes_servs.sql' as CHAR(500));
				SYSTEM trim(sqlStatement);
		...
		-- Tipo de ejecuciÃ³n 2, se ejecutan los indicadores de forma mensual
		IF tipoExec = 2 THEN
			FOREACH SELECT nombre_sp, descripcion_sp INTO spNombre, spDescripcion FROM tb_ctrl_ind_ctes_servs WHERE tipo_exec = 2 AND estatus_exec = 1 ORDER BY prioridad_sp ASC
				LET sqlStatement = cast('echo "EXECUTE PROCEDURE "informix".' || TRIM(spNombre) || '('''||mesanio||''','''||pFechaInicial||''','''||pFechaFinal||''');" > /ifxsif01/Control-M/sac_ctrl_ind_ctes_servs.sql' as CHAR(500));
				SYSTEM trim(sqlStatement);
			...
			END FOREACH;
		END IF;
		RETURN codReturn, cMensaje;
	END;
END PROCEDURE
DOCUMENT
'AUTOR: Mario Enriquez Gallegos ',
'DESCRIPCIÃN: SP de control para ejecutar SPs de indicadores',
'SUSTENTO: ',
'FECHA: 21/05/2019',
'VERSIÃN:  ',
...
CREATE PROCEDURE "informix".sp_estadisticas_altactes_aper_prods
...
RETURNING
    ...
    LET Vsp = 'sp_estadisticas_altactes_aper_prods';
    
    --LET cRutaArchivo = '/RESPALDOSNEW/';
	--LET cNombreArchivo = 'ALTACTES_PRODCREDCAPT_' || cMes || cAnio || '.cvs';
	LET codret = '00000';
	...
	--SET DEBUG FILE TO '/ifxsif01/menriquez/sp_estadisticas_altactes_aper_prods.out';
	--TRACE ON;
	...
	BEGIN
		...
		--OBTIENE ALTAS DE CLIENTES
		        DROP TABLE IF EXISTS CTE_HUELLA;

		        SELECT a.numcte, a.fecha_alta 
		        FROM bdinteg:si_cte_huella a, bdinteg:si_cliente b
		        WHERE a.fecha_alta between cPrimerDia and cUltimoDia
		        AND secuencia = 1 
		        AND a.numcte=b.numcte 
		        AND b.tipo_cliente='1' 
		        INTO temp CTE_HUELLA WITH NO LOG;

		        DROP TABLE IF EXISTS TOT_CTE_HUELLA;
		        SELECT fecha_alta fecha, COUNT(*) TOTAL FROM CTE_HUELLA
		        GROUP BY 1
		        INTO TEMP TOT_CTE_HUELLA WITH NO LOG;

		--CREDITO COPPEL
		        DROP TABLE IF EXISTS CTECREDITOSOL;

		        SELECT a.numcte, a.num_solicitud, a.num_producto, a.fecha_insert, b.fecha_salida, b.fecha_insert AS fecha_insert2
		        FROM bdisolic:ss_solicitudes a, bdisolic:ss_autorizacion b, cte_huella C
		        ...
		        AND a.fecha_insert = C.fecha_alta
		        INTO temp CTECREDITOSOL WITH NO LOG;
		        
		        DROP TABLE IF EXISTS TOT_CTECREDITOSOL;
		        SELECT fecha_insert fecha, COUNT(*) TOTAL FROM CTECREDITOSOL
		        GROUP BY 1
		        INTO TEMP TOT_CTECREDITOSOL WITH NO LOG;


		--CREDITO BANCOPPEL
		        DROP TABLE IF EXISTS CTECREDITOSOL2;
		        ...
		        AND a.num_solicitud=b.num_solicitud 
		        AND a.fecha_insert = C.fecha_alta
		        INTO temp CTECREDITOSOL2 WITH NO LOG;
		        
		        DROP TABLE IF EXISTS TOT_CTECREDITOSOL2;
		        SELECT fecha_insert fecha, COUNT(*) TOTAL FROM CTECREDITOSOL2
		        GROUP BY 1
		        INTO TEMP TOT_CTECREDITOSOL2 WITH NO LOG;

		--CUENTAS CAPTACION
		        DROP TABLE IF EXISTS CTECUENTAS;
		        SELECT DISTINCT b.num_cte, b.cuenta, a.fecha_alta 
		        FROM bdicheq:sc_maenoc a , bdicheq:sc_maechq b, cte_huella c
		        WHERE a.cuenta=b.cuenta AND b.num_cte=c.numcte AND a.fecha_alta=c.fecha_alta
		        INTO temp CTECUENTAS WITH NO LOG;

		        DROP TABLE IF EXISTS TOT_CTECUENTAS;
		        SELECT fecha_alta fecha, COUNT(*) TOTAL FROM CTECUENTAS
		        GROUP BY 1
		        INTO TEMP TOT_CTECUENTAS WITH NO LOG;


		INSERT INTO "informix".sac_estadisticas_altactes_aperprods
		SELECT AL.fecha, AL.total ctes_nvos, C1.total cred_coppel, C2.total cred_bncpl, C3.total ctas_capt, 'informix', current 
		FROM TOT_CTE_HUELLA AL, TOT_CTECREDITOSOL C1, TOT_CTECREDITOSOL2 C2, TOT_CTECUENTAS C3 
		WHERE AL.fecha=C1.fecha
		AND AL.fecha=C2.fecha
		AND AL.fecha=C3.fecha
		ORDER BY 1 ASC;

		--LET cSQL = 'echo "fecha|ctes_nvos|cred_coppel|cred_bncpl|ctas_capt|" >' || TRIM(cRutaArchivo)|| TRIM(cNombreArchivo);
		--SYSTEM cSQL;
		
		--LET cSQL = 'echo "UNLOAD TO '||TRIM(cRutaArchivo) || TRIM(cNombreArchivo) || ' DELIMITER '|| "'|'" || ' SELECT fecha, ctes_nvos, cred_coppel, cred_bncpl, ctas_capt FROM "informix".sac_estadisticas_altactes_aperprods WHERE fecha_insert=(SELECT MAX(fecha_insert) FROM "informix".sac_estadisticas_altactes_aperprods) ORDER BY 1 ASC;" >'||TRIM(cRutaArchivo)||'query_altactesprods.sql';
		--SYSTEM cSQL;

		--LET cSQL= "chmod 777 " || TRIM(cRutaArchivo) || 'query_altactesprods.sql';
		--SYSTEM cSQL;
		--LET cSQL= '';

		--LET cSQL='dbaccess bdisac '||TRIM(cRutaArchivo) || 'query_altactesprods.sql';
		--SYSTEM cSQL;

	--BORRAR ARCHIVO
		--LET cSQL = '' ;
		--LET cSQL = 'rm ' || TRIM(cRutaArchivo) || 'query_altactesprods.sql';
		--SYSTEM cSQL;						
...
CREATE PROCEDURE "informix".sp_obtienedatosusrl( pEmpresa char(3))
		
returning CHAR (5), CHAR(6), CHAR(4), CHAR(8), CHAR(6), CHAR(80);

	DEFINE sql_err			INTEGER;
	DEFINE cod_err			CHAR(5);
	DEFINE vRequest_time    CHAR(6);
	DEFINE vOpcode			CHAR(4);
	DEFINE vProcess_dt		CHAR(8);
	DEFINE vProcess_tm		CHAR(6);
	DEFINE vSession_id		CHAR(80);
...
--	TRACE ON;
 ...
 BEGIN
  ...
  END EXCEPTION;
	...
	SELECT request_time, opcode, process_dt, process_tm, session_id 
	INTO vRequest_time, vOpcode, vProcess_dt, vProcess_tm, vSession_id
	FROM "informix".sac_bts_usrl;
	
    RETURN cod_err, vRequest_time, vOpcode, vProcess_dt, vProcess_tm, vSession_id;
   END;
END PROCEDURE
DOCUMENT
"Autor :Marco Cardenas 97959456",
"FECHA : 07/05/2019",
"BD    : Bdisac",
"Descripcion: Se crea procedimiento encargado de obtener datos de la tabla sac_bts_usrl para mandar a servidor de postgresql";
