CREATE PROCEDURE "informix".sp_consultacuentascliente_ofi_bis_web(
                                            pEmpresa char (3),
...
END PROCEDURE
DOCUMENT
'Descripcion: Se agrego la ejecucion del nuevo sp_consultacuentascliente_crd, se agrego para que reciba los nuevos campos',
'que se agregaron al procedimiento sp_consultacuentascliente',
'Fecha: 21-Ago-2015',
'Base de datos: bdicobranza',
'Autor:Viridiana Paredes Romero';

CREATE PROCEDURE "informix".sp_consultacampaniasctrl_web(pEmpresa CHAR(3), pTipoCampania SMALLINT, pNumParam INTEGER, pGrupoParam CHAR(10))
RETURNING
	CHAR(5) AS COD_RET, ---cod_ret
...
BEGIN
    ...
    ELSE 
		FOREACH 
			...
			WHERE 
			tipo_campania = DECODE(pTipoCampania,0,tipo_campania,pTipoCampania)
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Genera una consulta con informacion standar para las aplicaciones de Cobranzas', 
