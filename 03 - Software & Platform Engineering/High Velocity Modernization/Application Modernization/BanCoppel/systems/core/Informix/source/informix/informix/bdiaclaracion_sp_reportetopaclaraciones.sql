CREATE PROCEDURE "informix".sp_reportetopaclaraciones(desde date, hasta date, top integer, estatus char(15) )
returning lvarchar as numero, lvarchar as nombre, integer as total;

    define res_total integer;
    define res_nombre lvarchar;
    define res_numero lvarchar;

--   SET DEBUG FILE TO "/informix/sp_reportetopaclaraciones.out";
--   TRACE ON;
    ...
    begin
        ...
        then
            foreach
                select first top sucursal.sucursal as numero, sucursal.nombre, count(*) as total
                into res_numero, res_nombre, res_total
                from acl_aclaracion, bdinteg:si_sucursales as sucursal, acl_estatus_aclaracion
                where sucursal.sucursal = acl_aclaracion.num_sucursal
                and acl_estatus_aclaracion.pky_estatus_aclaracion = acl_aclaracion.fky_estatus_aclaracion
                and acl_estatus_aclaracion.nombre = estatus
                and acl_aclaracion.fechacaptura
                    between desde and hasta
                group by sucursal.nombre, sucursal.sucursal
                order by total desc
                return trim(res_numero), trim(res_nombre), res_total
                with resume;
            end foreach;
        else
            foreach
