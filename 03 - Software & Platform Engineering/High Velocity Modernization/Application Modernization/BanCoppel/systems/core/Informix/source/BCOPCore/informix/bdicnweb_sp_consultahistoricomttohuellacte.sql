CREATE PROCEDURE "informix".sp_consultahistoricomttohuellacte(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumCliente CHAR(20), pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret, 
				DATE AS fecha_mantto,
				CHAR(10) AS hora, 
				CHAR(4) AS sucursal, 
				CHAR(8) AS empleado_ejecuto,
				CHAR(30) AS nombre_empleado_ejecuto, 
				CHAR(8) AS empleado_autorizo, 
				CHAR(30) AS nombre_empleado_autorizo, 
				CHAR(8) AS empleado_caja, 
				CHAR(30) AS nombre_empleado_caja;
				
	DEFINE cCodRet CHAR(5);
	DEFINE cCodRetSp CHAR(5);
	DEFINE iSqlErr INTEGER;
	DEFINE dFechaMantto DATE;
	DEFINE cFechaMantto CHAR(10);
	DEFINE cHora CHAR(10); 
	DEFINE cSucursal CHAR(4); 
	DEFINE cEmpEjecuto CHAR(8); 
	DEFINE cNomEmpEjecuto CHAR(30); 
	DEFINE cEmpAutorizo CHAR(8); 
	DEFINE cNomEmpAutorizo CHAR(30); 
	DEFINE cEmpCaja CHAR(8); 
	DEFINE cNomEmpCaja CHAR(30);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, dFechaMantto, cHora, cSucursal, cEmpEjecuto, 
					cNomEmpEjecuto, cEmpAutorizo, cNomEmpAutorizo, cEmpCaja, cNomEmpCaja;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultahistoricomttohuellacte.out';
		--TRACE ON;
		...
		FOREACH EXECUTE PROCEDURE bdinteg:"informix".sp_consultahistoricomttohuella(cEmpresa, pNumCliente)
			INTO cCodRetSp, cFechaMantto, cHora, cSucursal, cEmpEjecuto, cNomEmpEjecuto, cEmpAutorizo, cNomEmpAutorizo, cEmpCaja, cNomEmpCaja
			
			IF cCodRetSp::INTEGER < 0 THEN
				RAISE EXCEPTION cCodRetSp::INTEGER, 0, 'ERROR EN LA EJECUCIÃN DEL SP bdinteg:sp_consultahistoricomttohuella';
			END IF;
			
			LET dFechaMantto = MDY(SUBSTR(cFechaMantto, 6, 2), SUBSTR(cFechaMantto, 9, 2), SUBSTR(cFechaMantto, 1, 4));
			
			IF iRegistros >= pRegistros THEN
					IF iRecuperacion < pRecuperacion THEN
						RETURN cCodRet, dFechaMantto, TRIM(cHora), cSucursal, cEmpEjecuto, 
								cNomEmpEjecuto, cEmpAutorizo, cNomEmpAutorizo, cEmpCaja, cNomEmpCaja WITH RESUME;
...
'FECHA: 22/01/2014',
'DESCRIPCION: Consulta el historico de los mantenimientos que se le han ado a la huella de un cliente',
'BD: bdinteg';

CREATE PROCEDURE "informix".sp_consultatothistoricomttohuellacte(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumCliente CHAR(20))
		RETURNING CHAR(5) AS codret,
				INTEGER AS total;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultatothistoricomttohuellacte.out';
		--TRACE ON;
...
DOCUMENT 'AUTOR: Oscar Flores Conde',
'FECHA: 22/01/2014',
'DESCRIPCION: Consulta el total de los datos del historico de los mantenimientos que se le han ado a la huella de un cliente',
'BD: bdinteg';

CREATE PROCEDURE  "informix".sp_reporteconciliacionsac(pUsuario CHAR(8), pIdFuncion CHAR(10), pdtFechaIni DATE, pdtFechaFin DATE, pConvenio CHAR(5), pRegistros INTEGER, pRecuperacion INTEGER)
	RETURNING CHAR(5) AS  codigoRetorno, 
		DATE AS Dia,
		CHAR (20) AS Forma_Pago,
		MONEY(16,2) AS Cargo_Pago_Remesa,
		MONEY(16,2) AS Saldo_Cuenta_BTS_WU,
	...
	BEGIN
        ...
        ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, dDia, cFormaPago, mCargoPagoRemesa, mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero, cImportePag;	
        END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/bdicnweb/sac/sp_reporteconciliacionsac2.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pdtFechaIni = '' OR pdtFechaFin = '' OR pConvenio = '' OR pRegistros = '' OR pRecuperacion = ''  THEN 
			LET cCodRet = '00003';
			RETURN cCodRet, dDia, cFormaPago, mCargoPagoRemesa, mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero, cImportePag;
		END IF;
		...
		IF pConvenio = '07004' THEN
			SET ISOLATION TO DIRTY READ;
			FOREACH EXECUTE PROCEDURE bdisac:sp_reportebts_conciliacion( pdtFechaIni, pdtFechaFin)
			INTO dDia, cFormaPago, mCargoPagoRemesa, mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero, cImportePag
				IF iRegistros >=  pRegistros THEN
					IF  iRecuperacion < pRecuperacion THEN
						LET iRecuperacion = iRecuperacion + 1;
						RETURN cCodRet, dDia, cFormaPago, mCargoPagoRemesa, mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero, cImportePag WITH RESUME;
						LET iNoRegs = iNoRegs + 1;
					ELSE
						EXIT FOREACH;
					END IF;
		...
		ELIF pConvenio = '07006' OR pConvenio = '07007' OR pConvenio = '07008' THEN  -- CONVENIO 
			FOREACH EXECUTE PROCEDURE bdisac:sp_reportewu_conciliacion( pdtFechaIni, pdtFechaFin, pConvenio)
			INTO dDia, cFormaPago, mCargoPagoRemesa,  mSaldoCuentaBTSWU, cFolioOperacion, cClaveEnvio, cSucursal, cCajero, cImportePag
...
'Fecha: 2013/12/10',
'Descripcion: Convenio 07004 GENERA REPORTE DE CONCILIACION DE WU, Convenios 07006, 07007, 07008  GENERA REPORTE DE TBS DIARIO.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_reporteestadocuentasac(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaIni DATE, pFechaFin DATE, pConvenio CHAR(5), pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING
...
CREATE PROCEDURE "informix".sp_sw_ro_constipobloqcred(pIdUsuario CHAR(8), pIdFuncion CHAR(10), pOpcion CHAR(1))
        RETURNING CHAR(5) AS codRet,
        ...
        BEGIN
                ...
                END IF;
                -- VALIDACION DE ACCESO A LA FUNCIONALIDAD
                EXECUTE FUNCTION bdinteg:sp_cnsif_confirmaejecutivo(pIdUsuario, pIdFuncion) 
                INTO cCodRet;
                IF cCodRet <> '00000' THEN
                        RETURN cCodRet, cCodBloq, cDescripcion;
                END IF;
                ...
                IF pOpciON = '0' THEN
                        ...
                        FOREACH SELECT nvl(clave, 0), descripcion
                                ...
                                RETURN cCodRet, cCodBloq, UPPER(cDescripcion)
                                WITH resume;
                        END FOREACH;
                ELIF pOpciON = '1' THEN
                        SET ISOLATION TO DIRTY READ;
...
DOCUMENT 'AUTOR: Oscar Flores Conde',
'FECHA: --',
'DESCRIPCION: Consulta el catalogo de bloqueos para cuentas de credito:',
'pOcion = 0 - Consulta todo el catalogo',
'pOcion = 1 - Consulta el catalogo sin el concepto de -No tiene bloqueo-',
'FECHA: 20/03/2014',
'DESCRIPCION: Se actualiza el campo de salida descripcion pasando de 25 caracteres a 60',
'BD: bdicnweb';
