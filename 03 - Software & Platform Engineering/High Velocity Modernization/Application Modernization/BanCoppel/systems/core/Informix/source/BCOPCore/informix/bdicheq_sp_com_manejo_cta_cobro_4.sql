CREATE PROCEDURE "informix".sp_com_manejo_cta_cobro_4()
...
BEGIN
    ON EXCEPTION SET vSQLErr, vIsamErr, vErrorInfo
        IF  vSQLErr != 0 THEN
            SET DEBUG FILE TO '/resplogifx/conciliachq/sp_com_manejo_cta_cobro_4.err';
            TRACE ON;
            LET vCodRet     = vSQLErr;
    ...
    --SET DEBUG FILE TO "/resplogifx/conciliachq/sp_com_manejo_cta_cobro_4.out";
    --TRACE ON;
    ...
    CREATE INDEX idx_tmp_ctas_pendientes_4 ON tmp_ctas_pendientes ( cuenta )
        USING BTREE;
