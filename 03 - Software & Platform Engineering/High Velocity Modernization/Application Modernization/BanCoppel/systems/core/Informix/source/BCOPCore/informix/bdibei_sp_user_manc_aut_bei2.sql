CREATE PROCEDURE "informix".sp_user_manc_aut_bei2(
												pNumCliente CHAR(9),
												pIdUser integer,
												pNoReg integer,
												pRegIni integer,
												pIdTipoMov SMALLINT)
   returning
   CHAR(5),   INTEGER,   INTEGER,    CHAR(9),    CHAR(250),     CHAR(150),     CHAR(15),
...
--**************************************************************************************************************
    	IF NVL(pIdTipoMov,99) <> 99 THEN
         ...
         --   RETURN cod_ret, iTotalReg,NVL(sIdUsuario,-1), sNumCliente,sUserNom,sNombre,sTelCelular,sCiaCel,sEmail,sIdStatus,sIdTipoUser,sUserNomAdmin,sIdAdminManco WITH RESUME;
          RETURN cod_ret, iTotalReg,NVL(sIdUsuario,-1), NVL(sNumCliente,''),NVL(sUserNom,''),NVL(sNombre,''),NVL(sTelCelular,''),NVL(sCiaCel,-1),NVL(sEmail,''),NVL(sIdStatus,-1),NVL(sIdTipoUser,-1),NVL(sUserNomAdmin,''),NVL(sIdAdminManco,-1),NVL(sTipoMov,-1) WITH RESUME;
