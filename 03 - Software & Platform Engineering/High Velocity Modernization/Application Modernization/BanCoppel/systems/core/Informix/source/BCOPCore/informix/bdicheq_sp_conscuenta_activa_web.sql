CREATE PROCEDURE "informix".sp_conscuenta_activa_web(
                                        pEmpresa CHAR(3),
                                        pNumCte CHAR(20),
                                        pProducto CHAR(4)
									  )
...
-- MODIFICO: Jose Luis Pulido Zepeda
-- Descripcion del cambio: Se cambio la validacion de cuentas bloquedas con derecho a deposito, tambiÃÂ©n se cambio la validacion para verificar si
...
--                        las opciones 1 y 3 se anexa la descripciÃÂ³n de los bloqueos.
...
DEFINE iNumRegoOneclick			INTEGER;

LET cCodRet		= "00000";
LET iSqlErr		= 0;
LET cCuenta		= "";
LET iNumReg		= 0;
LET iNumRegoOneclick		= 0;
...
BEGIN
    ...
    --SET DEBUG FILE TO "/informix/AAeldobleA/sp_conscuenta_activa_web.out";
    --TRACE ON;
	...
	SELECT COUNT(numcte)INTO iNumRegoOneclick FROM bdicred:sd_pre_aprobados_trx WHERE numcte=pNumCte AND num_producto='6400';
	IF iNumRegoOneclick > 0 THEN
		  --SELECT trx.num_cta_captacion INTO cCuenta FROM bdicred:sd_pre_aprobados_trx trx WHERE numcte=pNumCte

			SELECT num_cta_captacion
			INTO cCuenta
			FROM bdicred:sd_pre_aprobados_trx WHERE numcte = pNumCte AND num_cta_captacion IN (
				SELECT a.cuenta
...
'Descripcion: Se metio validaciÃ³n para identificar si es flujo de one click, tome la cuenta de la tabla sd_pre_aprobados_trx',
'Fecha: 2024/09/02',
'Version: 20240902.1205';
...
returning char(5);
	...
	--Se agregan las cuentas para TAE. 17/09/2024
	if ptransacc IN ("3395", "3389") then
        if vc_ccmayor = "2402" then
            let vc_sector = "31";
        end if;	 
    end if;
