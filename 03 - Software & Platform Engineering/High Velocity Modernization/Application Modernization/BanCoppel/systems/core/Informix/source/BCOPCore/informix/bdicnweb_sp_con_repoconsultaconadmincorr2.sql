CREATE PROCEDURE "informix".sp_con_repoconsultaconadmincorr2(pUsuario CHAR(8), pIdFuncion CHAR(10), psArchivoOrigen CHAR(3), pFecha DATE, pRutaDescarga CHAR(100))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_con_repoconsultaconadmincorr2.out';
		--TRACE ON;

		IF pUsuario = '' OR pIdFuncion = '' OR  pRutaDescarga='' OR psArchivoOrigen = '' OR psArchivoOrigen IS NULL OR pFecha = '' OR pFecha IS NULL THEN
			LET cCodRet = '00003';
		...
		--ELIMINACIÃN DE REGISTROS GENERADOS MENORE A LA FECHA HOY (T-1)
				FOREACH
					SELECT nombre_reporte
					INTO cNombreArchivo
					FROM bdicnweb:"informix".sw_con_genreportesconciliacion
					...
					DELETE FROM bdicnweb:"informix".sw_con_genreportesconciliacion WHERE nombre_reporte = TRIM(cNombreArchivo);
				END FOREACH;
            ...
            LET cCmd1 ="";
            LET cCmd1 ="SELECT 'NOMBRE ARCHIVO 325','FECHA','TARJETA','CUENTA', 'TIPO MOV', 'TRAN CENTRAL', 'FOLIO 325', 'MONTO 325', 'ESTATUS', 'TXN LIBERACION', 'CUENTA C', 'CUENTA A', 'FOLIO SIF', 'MONTO SIF' FROM systables WHERE tabid = 1";
            LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL SELECT * FROM ( ";
            LET cCmd1 =""||TRIM(cCmd1)||" SELECT  nomarchivo325||'',TO_CHAR(fecha,'%d/%m/%Y'),''''||tarjeta||'',''''||cuenta||'',tipomov||'',tran_central||'', ''''||folio325||'', TO_CHAR(monto325::MONEY(18,2))||'',estatus||'',txnliberacion||'',''''||cuentac||'',''''||cuentaa||'', ''''||foliosif||'', TO_CHAR(montosif::MONEY(18,2))||'' FROM intercard:conadmin";
            LET cCmd1 =""||TRIM(cCmd1)||" WHERE archivoorigen = '"||psArchivoOrigen||"' AND '"||pFecha|| "' BETWEEN fecha AND fecha ORDER BY keyx ASC)"; 
           
			LET dFechaHoy = CURRENT;
			LET dHoraHoy = CURRENT;	

			-- SE DEFINE NOMBRE DEL REPORTE A GENERAR		
			LET nomFecha = TO_CHAR(dFechaHoy, '%d%m%Y')||"_"||TO_CHAR(dHoraHoy, '%H%M');
			LET cNombreArchivo = 'CONCILIACIONADMINISTRATIVA_'||TRIM(psArchivoOrigen)||'_'||TRIM(nomFecha)||'.xls';
        	...
        	LET cRutaGral = TRIM(pRutaDescarga)||'/'||TRIM(cNombreArchivo);
				...
				--INSERCIÃN A LA TABLA DE GENERACIÃN DE REPORTES DE CONCILIACIÃN
				INSERT INTO "informix".sw_con_genreportesconciliacion(id_registro, nombre_reporte, fecha_reporte, hr_reporte, usuario_insert, estatus) 
				VALUES(0, cNombreArchivo, dFechaHoy, dHoraHoy, pUsuario, '0');
...
'DESCRIPCION: Genera un reporte xls mediante consultando el detalle de la administracion de corresponsales, obtiene informacion de tabla conadmin',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_con_genrepoconrresponsales(pUsuario CHAR(8), pIdFuncion CHAR(10), psArchivoOrigen CHAR(3), pFecha DATE, pRutaDescarga CHAR(100))
    RETURNING CHAR(5) AS codret,
    CHAR(100) AS reporte_generado;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_con_genrepoconrresponsales.out';
		--TRACE ON;
		...
		--ELIMINACIÃN DE REGISTROS GENERADOS MENORE A LA FECHA HOY (T-1)
			...
			IF((psArchivoOrigen = 'ACI') OR (psArchivoOrigen = 'ACC') OR (psArchivoOrigen = 'ACT')) THEN
				--GENERACION DE REPORTE	
				LET pFecha = MDY(MONTH(pFecha), DAY(pFecha), YEAR(pFecha)) -1 UNITS DAY;

				LET cCmd1 ="";
        		LET cCmd1 ="SELECT 'ID TERMINAL','FECHA MOV','MONTO','COMISION','COMISION IVA','TIPO OPERACION' FROM systables WHERE tabid = 1 ";
        		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL SELECT * FROM ( ";
        		LET cCmd1 =""||TRIM(cCmd1)||" SELECT ''''||idterminal||'',TO_CHAR(fechamov, '%d-%m-%Y %H:%M:%S')||'',TO_CHAR(monto::MONEY(18,2))||'',comision||'',comisioniva||'', idtpooperacion||'' FROM intercard:conarchcomisiones";
				LET cCmd1 =""||TRIM(cCmd1)||" WHERE archivoorigen ='"||psArchivoOrigen||"' AND fechamov::DATE = '"||pFecha||"'::DATE  ORDER BY keyx ASC )"; 
				LET dFechaHoy = CURRENT;
				LET dHoraHoy = CURRENT;	
...
'DESCRIPCION: Genera un reporte xls. de Consulta de administracion de corresponsales, obtiene informacion de tabla conarchcomisiones',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_con_repoconsultaconadmin2(pUsuario CHAR(8), pIdFuncion CHAR(10), psArchivoOrigen CHAR(3), pFecha DATE, pRutaDescarga CHAR(100))
    RETURNING CHAR(5) AS codret,
    CHAR(100) AS reporte_generado;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_con_repoconsultaconadmin2.out';
		--TRACE ON;
		...
		--ELIMINACIÃN DE REGISTROS GENERADOS MENORE A LA FECHA HOY (T-1)
			...
			IF psArchivoOrigen = 'ADC' THEN
        		...
        		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL SELECT * FROM ( ";
        		LET cCmd1 =""||TRIM(cCmd1)||" SELECT ''''||idterminal||'',TO_CHAR(fechamov, '%d-%m-%Y %H:%M:%S')||'',TO_CHAR(monto::MONEY(18,2))||'',comision||'',comisioniva||'', idtpooperacion||'' FROM intercard:conarchcomisiones";
				LET cCmd1 =""||TRIM(cCmd1)||" WHERE fechamov::DATE = '"||pFecha||"' ORDER BY keyx ASC )"; 
            
            ELSE 
                IF psArchivoOrigen = 'TCD' THEN
                    LET vsnomarchivo = TRIM ('BCPLTCD_' || LPAD(day(pFecha),2,'0') || LPAD(Month(pFecha),2,'0') || year(pFecha) || '.TXT');
                ELSE
                    LET vsnomarchivo = TRIM ('BCPLTCC_' || LPAD(day(pFecha),2,'0') || LPAD(Month(pFecha),2,'0') || year(pFecha) || '.TXT');
                END IF;
        		...
        		LET cCmd1 ="SELECT 'NOMBRE ARCHIVO 325','FECHA','TARJETA','CUENTA', 'TIPO MOV', 'TRAN CENTRAL', 'FOLIO 325', 'MONTO 325', 'ESTATUS', 'TXN LIBERACION', 'CUENTA C', 'CUENTA A', 'FOLIO SIF', 'MONTO SIF' FROM systables WHERE tabid = 1 ";
        		LET cCmd1 =""||TRIM(cCmd1)||" UNION ALL SELECT * FROM ( ";
        		LET cCmd1 =""||TRIM(cCmd1)||" SELECT {+index (intercard:conadmin idx_conadmin4)} nomarchivo325||'',TO_CHAR(fecha,'%d/%m/%Y'),''''||tarjeta||'',''''||cuenta||'',tipomov||'',tran_central||'', ''''||folio325||'', TO_CHAR(monto325::MONEY(18,2))||'',estatus||'',txnliberacion||'',''''||cuentac||'',''''||cuentaa||'', ''''||foliosif||'', TO_CHAR(montosif::MONEY(18,2))||'' FROM intercard:conadmin";
				LET cCmd1 =""||TRIM(cCmd1)||" WHERE archivoorigen = '"||psArchivoOrigen||"' AND nomarchivo325 = '"||TRIM(vsnomarchivo)||"' ORDER BY keyx ASC )"; 
            END IF;
...
'MODULO: CONCILIACIONES  ',
'FUNCIONALIDAD: CONCILIACIÃN ADMINISTRATIVA',
'DESCRIPCION: Genera un reporte xls del detalle de administracion de interredes, obtiene informacion de tabla conadmin y conarchcomisiones',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consretiromasivocap(pUsuario CHAR(8), pIdFuncion CHAR(10), pLote INT, pRegistros INT, pRecuperacion INT)
	RETURNING CHAR(5) AS codret,
		INT AS id,
		CHAR(20) AS cuenta,
		CHAR(20) AS num_cte,
	...
	BEGIN
		...
		FROM
			...
			UNION
				SELECT lote
				FROM bdicnweb:"informix".sw_tr_cargamasiva_retiro_hist
			WHERE lote = pLote AND usuario = pUsuario);
		...
		--cambio, se quitÃ³ la directiva que forzaba hacer uso de Ã­ndex que no se utilizaba.
		--UPDATE {+INDEX (bdicnweb:sw_tr_cargamasiva_retiro idx_sw_tr_cargamasiva_retiro_reg)} bdicnweb:sw_tr_cargamasiva_retiro
		UPDATE bdicnweb:sw_tr_cargamasiva_retiro
		...
		--cambio, se quitÃ³ la directiva que forzaba hacer uso de Ã­ndex que no se utilizaba.
		FOREACH
			SELECT SKIP pRegistros FIRST pRecuperacion id_registro, cuenta, numcte, resultado, codret_proceso, motivo_rechazo, folio, transaccion, doc_cheque, monto_importe, TRIM(descripcion2) as referencia, status, monto1
			...
			FROM
				(SELECT --{+INDEX (bdicnweb: sw_tr_cargamasiva_retiro idx_sw_tr_cargamasiva_retiro_reg)} id_registro, cuenta, numcte, resultado, codret_proceso, motivo_rechazo, folio, transaccion, doc_cheque, monto_importe, descripcion2, status, monto1
				           id_registro, cuenta, numcte, resultado, codret_proceso, motivo_rechazo, folio, transaccion, doc_cheque, monto_importe, descripcion2, status, monto1
				FROM bdicnweb:"informix".sw_tr_cargamasiva_retiro
				WHERE lote = pLote AND usuario = pUsuario
				UNION
				SELECT {+INDEX (bdicnweb: sw_tr_cargamasiva_retiro_hist idx_sw_tr_cargamasiva_retiro_histlt)} 
				id_registro, cuenta, numcte, resultado, codret_proceso, motivo_rechazo, folio, transaccion, doc_cheque, monto_importe, descripcion2, status, monto1
				FROM bdicnweb:"informix".sw_tr_cargamasiva_retiro_hist
				WHERE lote = pLote AND usuario = pUsuario)
			ORDER BY id_registro --preguntar si se debe de quitar este ORDER
			
			--Se hace Trim en una variable , se quita la FunciÃ³n trim en el 'Where' del Select
			LET cCuenta = TRIM(cCuenta);
			
			-- Agregamos el numero de cliente
			IF cNumCliente IS NULL OR TRIM(cNumCliente) = '' THEN
				
				SELECT {+INDEX (bdicheq:sc_maechq idx_sc_maechq)} num_cte
				INTO cNumCliente
				FROM bdicheq:"informix".sc_maechq
				WHERE cuenta = cCuenta;
				--WHERE cuenta = TRIM(cCuenta);
				
				UPDATE {+INDEX (bdicnweb:sw_tr_cargamasiva_retiro idx_sw_tr_cargamasiva_retiro_reg)} bdicnweb:"informix".sw_tr_cargamasiva_retiro
				SET numcte = cNumCliente
				WHERE id_registro = iIdRegistro;
				
				IF dbinfo('sqlca.sqlerrd2') = 0 THEN
					UPDATE bdicnweb:"informix".sw_tr_cargamasiva_retiro_hist
					SET numcte = cNumCliente
					WHERE id_registro = iIdRegistro;
				END IF;
...
"Fecha de creacion: 06/11/2013",
"Descripcion: Consulta los registros cargados en la tabla masiva para RETIRO masivo de captacion",
"MODIFICACIÃN : Ivan LÃ³pez Escorza GMI 16-Mayo-2024";

CREATE PROCEDURE "informix".sp_aplicadevol_cod41_ccep(pUsuario CHAR(8), pIdFuncion CHAR(10), pImporteTotal DECIMAL(14,2), pDireccionMac CHAR(15))
		RETURNING CHAR(5) AS codret;		
	...
	DEFINE cDescripSp CHAR(50);
	DEFINE cMensaje_RET CHAR(80);
	DEFINE contRegDevol INTEGER;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_aplicadevol_cod41_ccep.out';
		--TRACE ON;

SET ISOLATION TO DIRTY READ;
SET LOCK MODE TO WAIT 3;
		
		IF pUsuario = '' OR pIdFuncion = '' OR  pImporteTotal IS NULL OR pDireccionMac = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF; 
		...
		LET contRegDevol = 0;
		FOREACH
				SELECT {+INDEX (bdicnweb:ccep_procesacod41detalle_tmp idx_ccep_procesacod41detalle_tmp_iddetallechq_usuario_direccionMac)} iddetallechq,bancoLibrado,descbancoLibrado,importe,cuentaReferencia,numCheque,CuentaDeposito,observaciones,motivoDevolucion,procesar 
				...
				IF cprocesar <> 'f' AND cprocesar <> 'p' THEN

				LET cCuentaReferencia = TO_CHAR(cCuentaReferencia::INT8);
				LET cNumCheque = TO_CHAR(cNumCheque::INT8);
					
					 -- generar un registro en la tabla de control
					 --para la consulta de devoluciones desde sucursal
					/*EXECUTE PROCEDURE bditef:ins_reg_devo(cEmpresa,TRIM(cBancoLibrado),LPAD(cCuentaReferencia,20,'0'), TRIM(cNumCheque), 
														   SUBSTR(cMotivoDevolucion,1,2), pUsuario , cFechaHoy)INTO cCodRetSp, cDescripSp;*/

					EXECUTE PROCEDURE bditef:ins_reg_devo2(cEmpresa,TRIM(cBancoLibrado),cCuentaReferencia,cNumCheque, 
														   SUBSTR(cMotivoDevolucion,1,2), pUsuario , cFechaHoy) INTO cCodRetSp, cDescripSp;
					...
					IF cTransacc = 'S' THEN
						...
						BEGIN;
					END IF;
					LET iCodRetSp = TRIM(cCodRetSp)::INTEGER;
					IF iCodRetSp < 0 THEN
						RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP ins_reg_devo';
					ELIF iCodRetSp = 190 THEN
						LET cCodRet = '00003';
					ELIF iCodRetSp = 195 THEN
						LET cCodRet = '00761';
					ELIF iCodRetSp = 196 THEN
						LET cCodRet = '00762';
					ELIF iCodRetSp = 191 THEN
						LET cCodRet = '00763';
					ELIF iCodRetSp = 192 THEN
						LET cCodRet = '00764';
					ELIF iCodRetSp = 193 THEN
						LET cCodRet = '00765';
					ELIF iCodRetSp = 198 THEN
						LET cCodRet = '00766';
					END IF;	

					
					IF cCodRetSp = 0 THEN
						UPDATE bdicnweb:ccep_procesacod41detalle_tmp
						SET (observaciones, procesar) = ('000 PROCESADO SATISFACTORIAMENTE','p')
						WHERE iddetallechq = idRowDetalle AND
						usuario = pUsuario AND
						direccionMac = pDireccionMac;
					ELSE	
						UPDATE bdicnweb:ccep_procesacod41detalle_tmp
						SET observaciones = cCodRet|| ' ' || cDescripSp
						WHERE iddetallechq = idRowDetalle AND
						usuario = pUsuario AND
						direccionMac = pDireccionMac;
					...
					LET contRegDevol = contRegDevol + 1;
				END IF;
		
		END FOREACH;

		
		IF contRegDevol = 0 THEN
			LET cCodRet = '00781';
			RETURN cCodRet;
		END IF;
		...
		LET cCodRetSp = '';
		LET cCodRet = '00000';
		EXECUTE PROCEDURE BDITEF:'informix'.sp_tef_grab_arch_cam(pUsuario,contRegDevol,pImporteTotal,40,'CPPC40','',1)INTO cCodRetSp,cMensaje_RET;		
		
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_tef_grab_arch_cam';
...
'DESCRIPCION: Realiza la actualizacion en tablas y grabado de la devolucion.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_consbloquearchivopresentado(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdConsulta CHAR(1))
		RETURNING CHAR(5) AS codret,
		INTEGER AS num_bloque;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, iNoBloque;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ope_consbloquearchivopresentado.out';
		--TRACE ON;
...
SET LOCK MODE TO WAIT 3;
		...
		IF NVL(dFecha,'') = '' THEN
			LET cCodRet = '00533'; --EL PARAMETRO FECHA_HOY NO SE ENCUENTRA EN LA TABLA SC_FECHA
			RETURN cCodRet,iNoBloque;
		END IF;
		
		-- Archivos presentados durante el dIa (PRE)
		IF pIdConsulta = '1' THEN
			
			-- yyyymmdd
			LET cFechaHoy = TO_CHAR(dFecha, "%Y")|| TO_CHAR(dFecha, "%m") || TO_CHAR(dFecha, "%d");
			
			
			SELECT MAX(num_bloque::INTEGER)
			INTO iNoBloque
			FROM bditef:"informix".cce_encabezado
			WHERE fecha_presenta = cFechaHoy
			AND SUBSTR(nombrearchivo, 1, 3) = 'PRE';			
		
			LET iNoBloque = NVL(iNoBloque,0);
			
			RETURN cCodRet,iNoBloque;	
			
		-- Archivos presentados con imagen
		ELIF pIdConsulta = '2' THEN
		
			-- ddmmyyyy
			LET cFechaHoy =  TO_CHAR(dFecha, "%d")|| TO_CHAR(dFecha, "%m") || TO_CHAR(dFecha, "%Y");
		
			SELECT COUNT(nombrearchivo)
			INTO iNoBloque
			FROM bditef:"informix".cce_gransumario
			WHERE SUBSTR(nombrearchivo, 1, 12) = 'PRE_' || cFechaHoy
			AND total_reg_ti <> '0';
		...
		-- Archivos presentados durante el dIa (REV)
		ELIF pIdConsulta = '3' THEN
			...
			LET iNoBloque = NVL(iNoBloque,0) + 1;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: Generador de Archivos', 
'DESCRIPCION: SPL que calcula el bloque correspondiente, segun los archivos presentados en la fecha consultada;',
'con pIdConsulta = 1 consulta el bloque de los archivos presentados durante el dIa (PRE),', 
'con pIdConsulta = 2 consulta el bloque de los archivos presentados con imagen,',
'con pIdConsulta = 3 consulta el bloque de los archivos presentados durante el dIa (REV).',
'BD: bdicnweb';
