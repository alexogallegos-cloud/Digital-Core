CREATE PROCEDURE "informix".sp_generainforiesgos(pFecha_rep date, pFecha_info date, pProducto char(4))
...
returning char(5);
    ...
    DEFINE dDia               DATE;
    DEFINE iCteTot            INTEGER;
    DEFINE mDocRet			  MONEY(16,2);
    DEFINE cNumSemana         CHAR(6);
    DEFINE cAnio              CHAR(4);
    DEFINE cSucursal          CHAR(4);
    DEFINE iCtaAct			  INTEGER;
	...
	-- Set Debug File To '/tmp/Antonio/sp_generainforiesgos.out';
	-- Trace On;
	...
	execute function bdinteg:fnNumeroSemana(pFecha_rep,4) 
    into cNumSemana, cAnio;
	
    Let cNumSemana = trim(cAnio) || lpad(trim(cNumSemana),2,'0');
	
    -- // SE INSERTA EL ENCABEZADO DE LA SEMANA
    IF dDia = 1 then
        Insert Into sc_tblinfdiariesgos(NumSuc, aniosemana, NumProducto)
        Select sucursal, cNumSemana, pProducto 
    ...
    UPDATE STATISTICS MEDIUM FOR TABLE sc_tblinfdiariesgos;
	
    CREATE TEMP TABLE tmptotsdo(
        sucursal1   CHAR(4), 
        sdoanterior MONEY (16,2)) WITH NO LOG LOCK MODE ROW;
    
    CREATE TEMP TABLE tmptotclientes(
        totclientes INTEGER, 
        sucursal3   CHAR(4)) WITH NO LOG LOCK MODE ROW;
    
    CREATE TEMP TABLE tmptotactiva(
        totactivas  INTEGER, 
        sucursal4   CHAR(4)) WITH NO LOG LOCK MODE ROW;

	If TRIM(pProducto) <> '3000' Then
		INSERT INTO tmptotsdo (sucursal1, sdoanterior)
		SELECT rie.numsuc AS sucursal1, NVL(Sum(mae.sdo_dia_ant), 0) AS sdoanterior
		  FROM sc_tblinfdiariesgos AS rie,
		 OUTER tmp_maechq AS mae 
		 WHERE rie.numproducto = pProducto
		   AND mae.sucursal = rie.numsuc
		   AND mae.producto = rie.numproducto
		 GROUP BY rie.numsuc;
        
        -- // TOTAL DE CLIENTES DE ALTA EN LA SUCURSAL
        INSERT INTO tmptotclientes (totclientes,sucursal3)
        Select Count (noc.cuenta) AS totclientes, mae.sucursal AS sucursal3	      
          From tmp_maenoc noc,
               tmp_maechq mae
         Where mae.producto = pProducto
           and noc.cuenta = mae.cuenta
           And noc.fecha_alta = pFecha_info 
         GROUP BY mae.sucursal;
                    
        -- // TOTAL DE CUENTAS CON SALDO MAYOR A 0
        INSERT INTO tmptotactiva (totactivas,sucursal4)
        Select Count(mae.cuenta) AS totactivas, mae.sucursal AS sucursal4			 
          FROM tmp_maechq mae
         WHERE mae.producto = pProducto
           AND mae.sdo_dia_ant > 0
         GROUP BY mae.sucursal;
			
		CREATE INDEX idx_tmptotsdo ON tmptotsdo(sucursal1) USING BTREE FILLFACTOR 99;
        CREATE INDEX idx_tmptotclientes ON tmptotclientes(sucursal3) USING BTREE FILLFACTOR 99;
        CREATE INDEX idx_tmptotactiva ON tmptotactiva(sucursal4) USING BTREE FILLFACTOR 99;
		
        UPDATE STATISTICS MEDIUM FOR TABLE tmptotsdo;
        UPDATE STATISTICS MEDIUM FOR TABLE tmptotclientes;
        UPDATE STATISTICS MEDIUM FOR TABLE tmptotactiva;
		
		FOREACH
            SELECT TS.sucursal1, NVL(TS.sdoanterior,'0.00'), NVL(TC.totclientes,0), NVL(TA.totactivas,0)
              INTO cSucursal, mSaldo, iCteTot, iCtaAct
              FROM tmptotsdo AS TS
              LEFT OUTER JOIN tmptotclientes AS TC ON (TS.sucursal1 = TC.sucursal3)
              LEFT OUTER JOIN tmptotactiva AS TA ON (TS.sucursal1 = TA.sucursal4)
             ORDER BY TS.sucursal1

            UPDATE sc_tblinfdiariesgos
               SET SaldoAcumuladoD1   = DECODE(dDia,1,mSaldo,SaldoAcumuladoD1),
                   TotalClientesD1    = DECODE(dDia,1,iCteTot,TotalClientesD1),		
                   TotalCuentasActiD1 = DECODE(dDia,1,iCtaAct,TotalCuentasActiD1),
                   Fechad1            = DECODE(dDia,1,pFecha_rep,Fechad1),
                   SaldoAcumuladoD2   = DECODE(dDia,2,mSaldo,SaldoAcumuladoD2),
                   TotalClientesD2    = DECODE(dDia,2,iCteTot,TotalClientesD2),		
                   TotalCuentasActiD2 = DECODE(dDia,2,iCtaAct,TotalCuentasActiD2),
                   Fechad2			   = DECODE(dDia,2,pFecha_rep,Fechad2),
                   SaldoAcumuladoD3   = DECODE(dDia,3,mSaldo,SaldoAcumuladoD3),
                   TotalClientesD3    = DECODE(dDia,3,iCteTot,TotalClientesD3),		
                   TotalCuentasActiD3 = DECODE(dDia,3,iCtaAct,TotalCuentasActiD3),
                   Fechad3			   = DECODE(dDia,3,pFecha_rep,Fechad3),
                   SaldoAcumuladoD4   = DECODE(dDia,4,mSaldo,SaldoAcumuladoD4),
                   TotalClientesD4    = DECODE(dDia,4,iCteTot,TotalClientesD4),		
                   TotalCuentasActiD4 = DECODE(dDia,4,iCtaAct,TotalCuentasActiD4),
                   Fechad4			   = DECODE(dDia,4,pFecha_rep,Fechad4),
                   SaldoAcumuladoD5   = DECODE(dDia,5,mSaldo,SaldoAcumuladoD5),
                   TotalClientesD5    = DECODE(dDia,5,iCteTot,TotalClientesD5),		
                   TotalCuentasActiD5 = DECODE(dDia,5,iCtaAct,TotalCuentasActiD5),
                   Fechad5			   = DECODE(dDia,5,pFecha_rep,Fechad5),
                   SaldoAcumuladoD6   = DECODE(dDia,6,mSaldo,SaldoAcumuladoD6),
                   TotalClientesD6    = DECODE(dDia,6,iCteTot,TotalClientesD6),		
                   TotalCuentasActiD6 = DECODE(dDia,6,iCtaAct,TotalCuentasActiD6),
                   Fechad6			   = DECODE(dDia,6,pFecha_rep,Fechad6),
                   SaldoAcumuladoD0   = DECODE(dDia,0,mSaldo,SaldoAcumuladoD0),
                   TotalClientesD0    = DECODE(dDia,0,iCteTot,TotalClientesD0),		
                   TotalCuentasActiD0 = DECODE(dDia,0,iCtaAct,TotalCuentasActiD0),
                   Fechad0			   = DECODE(dDia,0,pFecha_rep,Fechad0)
			 WHERE numsuc = cSucursal 
			   AND numproducto = pProducto;	
		END FOREACH;	
        
	ELSE
    
		INSERT INTO tmptotsdo (sucursal1, sdoanterior)
		SELECT rie.numsuc, NVL(SUM(mae.capital), 0)                          
		  FROM sc_tblinfdiariesgos AS rie,
		 OUTER bdinvers:sv_maeinv as mae
		 WHERE rie.numproducto = '3000'
		   AND mae.empresa = '001'
		   AND mae.cod_instrum = rie.numproducto
		   AND mae.sucursal = rie.numsuc
		 GROUP BY rie.numsuc;
        
        -- // TOTAL DE CLIENTES DADOS DE ALTA EN LA SUCURSAL DE LA FECHA 
		INSERT INTO tmptotclientes (totclientes, sucursal3)
		SELECT COUNT(cuenta), sucursal 		   
		  FROM bdinvers:sv_maeinv
		 WHERE empresa = '001'
		   AND fecha_alta = pFecha_info
    ...
    DROP TABLE tmptotsdo;
    DROP TABLE tmptotclientes;
    DROP TABLE tmptotactiva;
...
'DESCRIPCION: Genera la informacion para un dia y producto determinado para cada sucursal',
'Captacion',
'FECHA      : Marzo 2009',
'VERSION    : 20090312.1346',
'BD         : BDICHEQ',
'LLAMADO    : Se ejecuta del sp_reporteriesgos',
'**********************************************',
'MODIFICO   : ANTONIO BASTIDAS',
'DESCRIPCION: optimizar la duración del proceso y eliminación de ciclos para consultas pesadas',
'FECHA      : Noviembre 2009',
'VERSION    : 20091127.0902';
