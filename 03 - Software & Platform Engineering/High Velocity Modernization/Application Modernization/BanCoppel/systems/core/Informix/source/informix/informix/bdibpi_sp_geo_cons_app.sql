CREATE PROCEDURE "informix".sp_geo_cons_app( pFecha DATE ) RETURNING CHAR(5) AS cod_ret;
--****************************************************************************************************
...
BEGIN
	...
	--Inicia iteracion A-APP
		FOREACH WITH HOLD
			...
			FOREACH WITH HOLD
				SELECT   geo.folio, geo.folio_suc, latitud, longitud, version, referencia_23  INTO  vRef, vfolio, vLat, vLong, xNumserial, vRef23
				FROM  bi_geolocalizacion as geo 
				WHERE  geo.id_operacion =vid_operacion AND geo.fecha_oper = dFechaHoy
