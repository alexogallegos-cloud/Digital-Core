CREATE PROCEDURE "informix".sp_consulta_productos(p_num_producto CHAR(4), p_nombre_prod CHAR(40),pTipo_Ejecucion CHAR(1))

	RETURNING CHAR(5)  		 AS CodRet, --Codigo de retorno
			  VARCHAR(100) 	 AS Variable1,
			  VARCHAR(100) 	 AS Variable2,
			  VARCHAR(100) 	 AS Variable3,
			  VARCHAR(100) 	 AS Variable4,
			  VARCHAR(100) 	 AS Variable5,
			  VARCHAR(100) 	 AS Variable6,
			  VARCHAR(100) 	 AS Variable7,
			  VARCHAR(100) 	 AS Variable8;
	...
	DEFINE Variable1			   VARCHAR(100);
	DEFINE Variable2			   VARCHAR(100);
	DEFINE Variable3			   VARCHAR(100);
	DEFINE Variable4			   VARCHAR(100);
	DEFINE Variable5			   VARCHAR(100);
	DEFINE Variable6			   VARCHAR(100);
	DEFINE Variable7			   VARCHAR(100);
	DEFINE Variable8			   VARCHAR(100);
	...
	BEGIN
		...
		--SET DEBUG FILE TO "/informix/F3/SP/sp_consulta_productos.out";
		--TRACE ON;
		...
		IF (p_num_producto = '' AND p_nombre_prod = '') THEN
			LET cCodRet = '00001';
			RETURN cCodRet,'','','','','','','','';
		ELSE
			...
			WHERE  nombre_prod  = p_nombre_prod OR num_producto  = p_num_producto;

			IF v_ya_existe > 0 THEN
					SELECT num_producto,nombre_prod, plazo_min_cred, plazo_max_cred, monto_min_cred, monto_max_cred, edad_min, edad_max, monto_min_disp,(SELECT {+INDEX(sd_cattipopago pk_sd_sd_cattipopago)}a.valor FROM sd_cattipopago a, sd_definicion b WHERE SUBSTR(a.tipo_pago,0,1) = b.periodo_plazo AND num_producto = p_num_producto) AS periodo_plazo,
					comi_gasto_cobranza, cobro_comision_anual, comi_disposicion_efect,comi_aclaracion_No, comi_liquidacion_antic, cobro_comis_apertura, garantias,(SELECT a.tipo_garantia FROM sd_tipo_garantia a , sd_definicion b WHERE a.id_garantia = b.idgarantia AND num_producto  = p_num_producto) AS tipo_garantia, 
					...
					(SELECT a.id_estdocta FROM sd_tipo_edocta a, sd_definicion b WHERE a.id_estdocta = b.id_estadocuenta AND num_producto = p_num_producto) AS id_estadocuenta,emision_estado_cuenta,rango_inicial,rango_final,idcta_concentradora,cta_concentradora,familia, monto_max_disp, envia_os,rangoini_gracia,rangofin_gracia
					INTO   cNum_Producto,cNomb_Producto,cPlazo_Minimo,cPlazo_Maximo,cMonto_Min_Cred,cMonto_Max_Cred,cEdad_Minima,cEdad_Maxima,cMonto_Min_Disp,cPeriodo_Plazo,cComision_Gastos,cComision_Anualidad,cComision_Disposicion,cComision_Aclaracion,
					cComision_Liquidacion,cComision_Apertura,cGarantias,cTipo_Garantia,cPorcentaje_Aforo,cObligado_Solidario,cNum_Obligados,cCaptura_Obligatoria,cCuentas_Medios,cCancelacion_Inac,cCancelacion_Vig,cTiempo_Cancelar,cSeguro_Vida,cCobro_Mensualidad,cEnvio_Mesa_Control,cId_Domiciliacion,cConciliador,
					cHistorico_Cred,cPeriodo_Gracia,cDias_Gracia,cCapital_Interes,cIntereses,Ccapital,cEstado_Cuenta,cId_Edocta,cEmision_estado_cuenta,cRango_inicial,cRango_final,cIdcta_concentradora,cCta_concentradora, cFamilia, cMonto_max_disp, cEnvia_os,crangoini_gracia,crangofin_gracia
					FROM  "informix".sd_definicion
					WHERE nombre_prod  = p_nombre_prod OR num_producto  = p_num_producto;
					
					SELECT form_aplica
					INTO flag_tpcomis_ComGastos
					FROM sd_tpcomis a,sd_definicion b
					WHERE a.cod_comis = b.cod_comi_gasto_cobranza
					AND num_producto  = p_num_producto;
					
					IF flag_tpcomis_ComGastos = 1 THEN 
						SELECT a.monto INTO cValor_Comision_Gastos FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comi_gasto_cobranza AND num_producto = p_num_producto;
					ELIF flag_tpcomis_ComGastos = 2 THEN 
						SELECT a.apli_factor INTO cValor_Comision_Gastos FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comi_gasto_cobranza AND num_producto = p_num_producto;
					END IF;
					...
					IF flag_tpcomis_ComAnualidad = 1 THEN 
						SELECT a.monto INTO cValor_Comision_Anualid FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comision_anualidad AND num_producto = p_num_producto;
					ELIF flag_tpcomis_ComAnualidad = 2 THEN 
						SELECT a.apli_factor INTO cValor_Comision_Anualid FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comision_anualidad AND num_producto = p_num_producto;
					END IF;					
					...
					IF flag_tpcomis_ComDisposicion = 1 THEN 
						SELECT a.monto INTO cValor_Comision_Dispos FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comision_efectivo AND num_producto = p_num_producto;					
					ELIF flag_tpcomis_ComDisposicion = 2 THEN 
						SELECT a.apli_factor INTO cValor_Comision_Dispos FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comision_efectivo AND num_producto = p_num_producto;		
					END IF;								
					...
					IF flag_tpcomis_ComAclaracion = 1 THEN 
						SELECT a.monto INTO cValor_Comision_Aclar FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comi_aclaracion_no AND num_producto = p_num_producto;					
					ELIF flag_tpcomis_ComAclaracion = 2 THEN 
						SELECT a.apli_factor INTO cValor_Comision_Aclar FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comi_aclaracion_no AND num_producto = p_num_producto;		
					END IF;						
					...
					IF flag_tpcomis_ComLiquidacion = 1 THEN 
						SELECT a.monto INTO cValor_Comision_Liquid FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comi_liquidacion_antic AND num_producto = p_num_producto;					
					ELIF flag_tpcomis_ComLiquidacion = 2 THEN 
						SELECT a.apli_factor INTO cValor_Comision_Liquid FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comi_liquidacion_antic AND num_producto = p_num_producto;		
					END IF;						
					...
					IF flag_tpcomis_ComApertura = 1 THEN 
						SELECT a.monto INTO cValor_Comision_Aper FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comision_apertura AND num_producto = p_num_producto;					
					ELIF flag_tpcomis_ComApertura = 2 THEN 
						SELECT a.apli_factor INTO cValor_Comision_Aper FROM sd_tpcomis a,sd_definicion b WHERE a.cod_comis = b.cod_comision_apertura AND num_producto = p_num_producto;		
					END IF;						

					IF pTipo_Ejecucion = '1' THEN
						LET Variable1 = cNum_Producto::VARCHAR(100);
						LET Variable2 = cNomb_Producto::VARCHAR(100);
						LET Variable3 = cPlazo_Minimo::VARCHAR(100);
						LET Variable4 = cPlazo_Maximo::VARCHAR(100);
						LET Variable5 = cMonto_Min_Cred::VARCHAR(100);
						LET Variable6 = cMonto_Max_Cred::VARCHAR(100);
						LET Variable7 = cEdad_Minima::VARCHAR(100);
						LET Variable8 = cEdad_Maxima::VARCHAR(100);
					ELIF pTipo_Ejecucion = '2' THEN
						LET Variable1 = cMonto_Min_Disp::VARCHAR(100);
						LET Variable2 = cPeriodo_Plazo::VARCHAR(100);
						LET Variable3 = cComision_Gastos::VARCHAR(100);
						LET Variable4 = cValor_Comision_Gastos::VARCHAR(100);
						LET Variable5 = cComision_Anualidad::VARCHAR(100);
						LET Variable6 = cValor_Comision_Anualid::VARCHAR(100);
						LET Variable7 = cComision_Disposicion::VARCHAR(100);
						LET Variable8 = cValor_Comision_Dispos::VARCHAR(100);
					ELIF pTipo_Ejecucion = '3' THEN
						LET Variable1 = cComision_Aclaracion::VARCHAR(100);
						LET Variable2 = cValor_Comision_Aclar::VARCHAR(100);
						LET Variable3 = cComision_Liquidacion::VARCHAR(100);
						LET Variable4 = cValor_Comision_Liquid::VARCHAR(100);
						LET Variable5 = cComision_Apertura::VARCHAR(100);
						IF p_num_producto= '6400' AND cComision_Apertura = '1' THEN
							SELECT valor INTO cValor_Comision_Aper
							FROM bdicred:"informix".sd_param
							WHERE  cod_param = '040';
						END IF;
						LET Variable6 = cValor_Comision_Aper::VARCHAR(100);
						LET Variable7 = cGarantias::VARCHAR(100);
						LET Variable8 = cTipo_Garantia::VARCHAR(100);
					ELIF pTipo_Ejecucion = '4' THEN
						LET Variable1 = cPorcentaje_Aforo::VARCHAR(100);
						LET Variable2 = cObligado_Solidario::VARCHAR(100);
						LET Variable3 = cNum_Obligados::VARCHAR(100);
						LET Variable4 = cCaptura_Obligatoria::VARCHAR(100);
						LET Variable5 = cCuentas_Medios::VARCHAR(100);
						LET Variable6 = cCancelacion_Inac::VARCHAR(100);
						LET Variable7 = cCancelacion_Vig::VARCHAR(100);
						LET Variable8 = cTiempo_Cancelar::VARCHAR(100);
					ELIF pTipo_Ejecucion = '5' THEN
						LET Variable1 = cSeguro_Vida::VARCHAR(100);
						LET Variable2 = cCobro_Mensualidad::VARCHAR(100);
						LET Variable3 = cEnvio_Mesa_Control::VARCHAR(100);
						LET Variable4 = cId_Domiciliacion::VARCHAR(100);
						LET Variable5 = cConciliador::VARCHAR(100);
						LET Variable6 = cHistorico_Cred::VARCHAR(100);
						LET Variable7 = cPeriodo_Gracia::VARCHAR(100);
						LET Variable8 = cDias_Gracia::VARCHAR(100);
					ELIF pTipo_Ejecucion = '6' THEN
						LET Variable1 = cCapital_Interes::VARCHAR(100);
						LET Variable2 = cIntereses::VARCHAR(100);
						LET Variable3 = cEstado_Cuenta::VARCHAR(100);
						LET Variable4 = cId_Edocta::VARCHAR(100);
						LET Variable5 = cEmision_estado_cuenta::VARCHAR(100);
						LET Variable6 = cRango_inicial::VARCHAR(100);
						LET Variable7 = cRango_final::VARCHAR(100);
						LET Variable8 = cIdcta_concentradora::VARCHAR(100);
					ELIF pTipo_Ejecucion = '7' THEN
						LET Variable1 = cCta_concentradora::VARCHAR(100);
						LET Variable2 = cFamilia::VARCHAR(100);
						LET Variable3 = cMonto_max_disp::VARCHAR(100);
						LET Variable4 = Ccapital::VARCHAR(100);
						LET Variable5 = cEnvia_os::VARCHAR(100);
						LET Variable6 = flag_tpcomis_ComGastos::VARCHAR(100);
						LET Variable7 = flag_tpcomis_ComAnualidad::VARCHAR(100);
						LET Variable8 = flag_tpcomis_ComDisposicion::VARCHAR(100);
					ELIF pTipo_Ejecucion = '8' THEN
						LET Variable1 = flag_tpcomis_ComAclaracion::VARCHAR(100);
						LET Variable2 = flag_tpcomis_ComLiquidacion::VARCHAR(100);
						LET Variable3 = flag_tpcomis_ComApertura::VARCHAR(100);
						LET Variable4 = crangoini_gracia::VARCHAR(100);
						LET Variable5 = crangofin_gracia::VARCHAR(100);
						LET Variable6 = '';
						LET Variable7 = '';
						LET Variable8 = '';
					END IF;

					RETURN cCodRet,Variable1,Variable2,Variable3, Variable4, Variable5,Variable6,Variable7,Variable8;

					--Tipo Ejecucion 1 -- Retorna eL NÃÂÃÂÃÂÃÂ?ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ° Producto, Nombre del Producto,Plazo Minimo para Credito, Plazo Maxima para Credito
					--Tipo Ejecucion 2 -- Retorna el Linea Minima Credito,Linea Maxima Credito, Edad Minima ,Edad Maxima , Monto min a disponer credito, Frecuencia de pago, Bandera Comision gastos cobranza,Comision gastos cobranza
					--Tipo Ejecucion 3 -- Retorna el Bandera Comision anualidad,Valor Comision anualidad, Bandera Comision disp de efectivo, Valor Comision disp de efectivo, Bandera Comision aclara no procedente, Valor Comision aclara no procede, Bandera Comision liquidacion antic, Valor Comision liquidacion antic
					--Tipo Ejecucion 4 -- Retorna el Bandera Comision apertura, Valor Comision apertura, BanDera de Garantias, Tipos de garantia, porcentaje del aforo, Bandera Obligado Solidario, Bandera numero obligados, Bandera captura obligatoria
					--Tipo Ejecucion 5 -- Retorna el Checkbox cta medios acceso, Bandera cancelacion x inactividad, Bandera cancelacion x vigencia, Tiempo cancelar en meses, Bandera Seguro de Vida, Bandera Cobro Mensualidad, Checkbox envio MC, Checkbox Id_domiciliacion
					--Tipo Ejecucion 6 -- Retorna el Checkbox Conciliador, Checkbox Historicocred, Checkbox Periodogracia,DÃÂÃÂÃÂÃÂ?ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ­as de gracia,Bandera tpo condonacion Cap e Int, Bandera tpo condonacion Int, Bandera estado de cuenta,Id estado de cuenta
					--Tipo Ejecucion 7 -- Retorna el Emision Edocta, Rango periodo inicial,Rango periodo final, Bandera Cta Concentradora, Cuenta Concentradora,crangoini_gracia,crangofin_gracia
			ELSE
				LET cCodRet = '00002';
				RETURN cCodRet,'','','','','','','','';
			END IF;
	...
	END
END PROCEDURE
DOCUMENT 'AUTOR:DSB Ever Fierro Hernandez',
'FECHA: 20/08/2020',
'DESCRIPCION: Se crea nuevo procedimiento para consultar productos de Prestamo y Tarjetas de Credito.',
'SISTEMA. SOC',
'BD: bdicred';
