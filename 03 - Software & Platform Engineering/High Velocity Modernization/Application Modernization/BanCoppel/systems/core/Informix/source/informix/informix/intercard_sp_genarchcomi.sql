CREATE PROCEDURE "informix".sp_genarchcomi(psRuta CHAR(100),psTipoArchivoCom CHAR(3),psNomArchivoCom CHAR(21))
...
--SET DEBUG FILE TO '/informixuc7/perifericos/genarchcomi.txt';
--TRACE ON;
...
BEGIN 	
	...
	IF ( psTipoArchivoCom = 'ACI') THEN
			...
			IF ( vsSQL <> '' ) THEN 
				SYSTEM vsSQL ;
				LET vsSQL4 = '' ;
				LET vsSQL4="chmod 777 /home/sysconau/conciliacion/tcoppel/tmp_consiliacion.sql";
				SYSTEM vsSQL4;
				LET vsSQL4 = '' ;
				LET vsSQL4 = 'dbaccess intercard ' || TRIM(psRuta) ||'/tmp_consiliacion.sql';
				SYSTEM vsSQL4;
			END IF;
			...
			EXECUTE PROCEDURE intercard:sp_con_buscararchivo(TRIM(psRuta), TRIM(psNomArchivoCom))
			INTO vsCodRetorno2, v_BandBA;
			
			IF (vsCodRetorno2 <> '00000') THEN
				LET vsCodRetorno = '00001';
				LET vsMensajeError = '(' || vsCodRetorno || ') Error en sp_con_buscararchivo (' || psRuta || ', ' || psNomArchivoCom || ')';
				ELIF (vsCodRetorno2 = "00000" AND v_BandBA = "F") THEN
					LET vsCodRetorno = "00002";
					LET vsMensajeError = '(' || vsCodRetorno || ') No se encontro el archivo ' || psNomArchivoCom || ' en la ruta: ' || psRuta;
			...
			IF (vsCodRetorno = '00000') THEN --VALIDA SI EL ARCHIVO SE GENERO CORRECTAMENTE
				LET vsTransArchivoAIX = 'V';
				LET vsActividad = 'GEN. ARCH. COMISIONES' ;
				LET vsFlagFinExito = 'V';
				LET vsMensajeError = '';
			ELSE --ERROR EN EL LE GENERACION DEL ARCHIVO GAC
				LET vsTransArchivoAIX = 'F';
				LET vsActividad = 'GEN. ARCH. COMISIONES' ;
				LET vsFlagFinExito = 'F';
				LET vsMensajeError = 'ERROR EN LA CONCILIACION ADMINISTRATIVA (' || 00000 || ') ' || vsMensajeError ;
				LET viNumRegistrosArchivo = 0;
			END IF;
			
			EXECUTE PROCEDURE Intercard:sp_ObtFechaHoraServidor  (psRuta) INTO iSamErr, vdtFechaTransArchivo ;
			...
			IF (vsMensajeError = '') THEN
				LET vsMensajeError = 'ACI Y CONADMIN EXITOSOS';
			END IF;
    
	ELIF ( psTipoArchivoCom = 'ACC') THEN 
			...
			IF ( vsSQL <> '' ) THEN 
				SYSTEM vsSQL ;
				LET vsSQL4 = '' ;
				LET vsSQL4="chmod 777 /home/sysconau/conciliacion/tcoppel/tmp_conciliacion_corr.sql";
				SYSTEM vsSQL4;
				LET vsSQL4 = '' ;
				LET vsSQL4 = 'dbaccess intercard ' || TRIM(psRuta) ||'/tmp_conciliacion_corr.sql';
				SYSTEM vsSQL4;
			END IF;
			
			LET vsSQL4 = '' ;
			LET vsSQL4 =  "chmod 777 /home/sysconau/rem_signo2.sh ";
			SYSTEM vsSQL4;
			LET vsSQL4 = '' ;
			LET vsSQL4 =  "/home/sysconau/rem_signo2.sh";
			SYSTEM vsSQL4;
			...
			IF (vsCodRetorno = '00000') THEN --VALIDA SI EL ARCHIVO SE GENERO CORRECTAMENTE
				LET vsTransArchivoAIX = 'V';
				LET vsActividad = 'GEN. ARCH. COMISIONESCORR';
				LET vsFlagFinExito = 'V';
				LET vsMensajeError = '';
			ELSE --ERROR EN LA GENERACION DEL ARCHIVO GAC
				LET vsTransArchivoAIX = 'F';
				LET vsActividad = 'GEN. ARCH. COMISIONESCORR';
				LET vsFlagFinExito = 'F';
				LET vsMensajeError = 'ERROR EN LA CONCILIACION ADMINISTRATIVA CORRESPONSALES(' || 00000 || ') ' || vsMensajeError ;
				LET viNumRegistrosArchivo = 0;
			END IF;
			...
			IF (vsMensajeError = '') THEN
				LET vsMensajeError = 'ACC Y CONADMIN EXITOSOS';
			END IF;
	
	ELIF ( psTipoArchivoCom = 'ACT') THEN  --TRANSFERENCIA DE PRESTAMOS
			...
			IF ( vsSQL <> '' ) THEN 
				SYSTEM vsSQL ;
				LET vsSQL4 = '' ;
				LET vsSQL4="chmod 777 /home/sysconau/conciliacion/tcoppel/tmp_conciliacion_pres.sql";
				--SYSTEM vsSQL4;
				LET vsSQL4 = '' ;
				LET vsSQL4 = 'dbaccess intercard ' || TRIM(psRuta) ||'/tmp_conciliacion_pres.sql';
				SYSTEM vsSQL4;
			END IF;
			
			LET vsSQL4 = '' ;
			LET vsSQL4 =  "chmod 777 /home/sysconau/rem_signo3.sh ";
			SYSTEM vsSQL4;
			LET vsSQL4 = '' ;
			LET vsSQL4 =  "/home/sysconau/rem_signo3.sh";
			SYSTEM vsSQL4;
			...
			IF (vsCodRetorno = '00000') THEN --VALIDA SI EL ARCHIVO SE GENERO CORRECTAMENTE
				LET vsTransArchivoAIX = 'V';
				LET vsActividad = 'GEN. ARCH. COMISIONESPRES';
				LET vsFlagFinExito = 'V';
				LET vsMensajeError = '';
			ELSE --ERROR EN LA GENERACION DEL ARCHIVO GAC
				LET vsTransArchivoAIX = 'F';
				LET vsActividad = 'GEN. ARCH. COMISIONESPRES';
				LET vsFlagFinExito = 'F';
				LET vsMensajeError = 'ERROR EN LA CONCILIACION ADMINISTRATIVA TRANSFERENCIAS PRESTAMOS(' || 00000 || ') ' || vsMensajeError ;
				LET viNumRegistrosArchivo = 0;
			END IF;
			...
			IF (vsMensajeError = '') THEN
				LET vsMensajeError = 'ACT Y CONADMIN EXITOSOS';
			END IF;
...
'Descripcion: GENERA ARCHIVO DE COMISIONES DE TRANSFERENCIAS.',
'Fecha: 2011/06/10',
'Version: 20110611.0933',
'BD: Intercard';

CREATE PROCEDURE "informix".sp_tipo_conciliacion_pos2( 
...
--****************************************************************************************************
-- DESCRIPCION: ASIGNA EL TIPO DE CONCILIACION PARA POS -CONTINUACION
...
-- MODIFICADO : 23/04/2010 Casanova Edeza Hector Juan.  Se modifico el flujo para que contemple los nuevos archivos archivos de corresponsales de pagos(CCP), depositos(CCD) y se generen registros para central con la informacion del archivo 325 de corresponsales (se manejan de forma similar a los pagos interbancarios debido a que no estan registrados en la tabla de movimientos de intercard).
-- MODIFICADO : 27/05/2011 Alfonso Antonio Cruz Alvarez.  Se modifico el flujo para que contemple los nuevos archivos archivos de corresponsales de Transferencias de Depositos(TPD).
--***************************************************************************************************
...
DEFINE TC_CORRESPONSAL CHAR (4) ;
DEFINE TS_CORRESPONSAL CHAR (4) ;
...
DEFINE PREFIJO_SEC_I CHAR (1) ;
 ...
 --SET DEBUG FILE TO "/informixuc7/perifericos/sp_tipo_conciliacion_pos2jy.out";
 --TRACE ON;
...
LET TC_CORRESPONSAL = '0282' ;
LET TS_CORRESPONSAL = '0282' ;
...
BEGIN 
    ...
    IF ( piExisteReverzaForzada > 0 ) THEN  --SI EXISTE UN MOVIMIENTO ORIGINAL. IDENTIFICA SI EL MOVIMIENTO ES UNA REVERSA O FORZADA
        ...
        LET vsSecuencia = SUBSTRING ( LPAD ( TRIM ( psRFSecuencia ), 7, '0' ) FROM 2 FOR 7 ) ;

        EXECUTE PROCEDURE intercard:sp_GetTypeOfTransaction( '02', psRFFormato, vsRFCodTran, vmRFMonto, vmRFMontoCashBack ) INTO viTipodeTransaccion ;

        IF ( viTipodeTransaccion = 204 ) THEN  --REVERSA_POS
            LET vsDESCRIPCION1 = 'La transaccion coincide con una Reversa' ;
            LET vsTipoTran = '02' ;
            LET vsMovCodReversa = '1' ;
        ELIF ( viTipodeTransaccion = 205 ) THEN  --FORZADA_POS
            LET vsDESCRIPCION1 = 'La transaccion coincide con una Forzada' ;
            LET vsTipoTran = '01' ;
        ELIF ( viTipodeTransaccion = 212 ) THEN  --REVERSA_CASHBACK_POS
            LET vsDESCRIPCION1 = 'La transacción coincide con una Reversa Cash Back' ;
            LET vsTipoTran = '09' ;
        ELIF ( viTipodeTransaccion = 213 ) THEN  --REVERSA_REVERSA_CASHADVANCE_POS
            LET vsDESCRIPCION1 = 'La transacción coincide con una Reversa Cash Advance' ;
            LET vsTipoTran = '09' ;
        ELSE
    ...
    ELSE -- EL MOVIMINETO ES UNA COMPRA O UNA DEVOLUCION YA QUE NO EXISTE MOVIMIENTO QUE 
        ...
        IF ( ( psTipoTransaccion = '21' ) AND (psMovCodTran = '00' ) ) THEN
            LET vsDESCRIPCION1 = 'La transaccion es una DEVOLUCION que coincide con una Compra' ;
            LET vsTipoTran = '02' ;
        ELIF ( ( psTipoTransaccion = '01' ) AND (psMovCodTran = '00' ) ) THEN
            LET vsDESCRIPCION1 = 'La transaccion coincide con una Compra' ;
            LET vsTipoTran = '00' ;
        ELIF ( ( psTipoTransaccion = '01' OR psTipoTransaccion = '09' ) AND (psMovCodTran = '00' )  AND ( pmMonto > 0 ) AND ( pmMonto > pmMontoCashBack )  AND ( pmMontoCashBack > 0) ) THEN
            LET vsDESCRIPCION1 = 'La transaccion coincide con un Cash Back' ;
            LET vsTipoTran = '09' ;
        ELIF ( ( psTipoTransaccion = '01' OR  psTipoTransaccion = '09' ) AND (psMovCodTran = '00' ) AND ( pmMonto > 0 ) AND ( pmMonto = pmMontoCashBack ) ) THEN
            LET vsDESCRIPCION1 = 'La transaccion coincide con un Cash Advance' ;
            LET vsTipoTran = '09' ;
		...
		IF ( piExisteEnMovimiento=3) THEN --/// MARCA CON ERROR A LOS MOVIMIENTOS DE COMPRA QUE NO TIENEN NUMERO DE AUTORIZACIÓN INTERCARD
            LET EnLinea = 'E' ;
        END IF ;	
		 
    END IF ;            

IF ( piExisteEnMovimiento < 1 ) THEN  --NO SE ENCONTRO EL MOVIMIENTO ORIGINAL EN LA TABLA DE MOVIMIENTO
    
	--TIPO 32 
	IF (psArchivoOrigen = 'CCD') OR (psArchivoOrigen = 'CCP') OR (psArchivoOrigen = 'TPD') THEN --CORRESPONSALES COPPEL
	
		LET ESTADO1 = '32' ;
		
		IF (psArchivoOrigen = 'CCD') THEN
			LET vsDESCRIPCION1 = 'DEPOSITO A CUENTA CORRESPONSALES' ;
			LET TC_CORRESPONSAL = '0282' ;
			LET TS_CORRESPONSAL = '0282' ;
		ELIF (psArchivoOrigen = 'CCP') THEN ----falta que banco defina la transaccion y rista contable del ccp -credito-
			LET vsDESCRIPCION1 = 'Corresponsal Pago Credito' ;
			LET TC_CORRESPONSAL = '0803' ;
			LET TS_CORRESPONSAL = '0803' ;
		ELIF (psArchivoOrigen = 'TPD') THEN ---- Transferencias de Prestamos Debito
			LET vsDESCRIPCION1 = 'Transferencias Prestamos Debito' ;
			LET TC_CORRESPONSAL = '0000' ;
			LET TS_CORRESPONSAL = '0000' ;
		END IF;
		
		
		LET viGenRefTipoMovimiento = 22 ;        --genera la referencia para el registro de la comision a cobrar por el movimiento
		
		EXECUTE PROCEDURE intercard:sp_GetSecuencia (viGenRefTipoMovimiento) INTO vsNuevaSecuencia  ; 
		EXECUTE PROCEDURE intercard:sp_GeneraReferencia( viGenRefTipoMovimiento, psNombreComercio, psSecuenciaAuth ) INTO vsReferencia ;
		
		LET vsReferencia = TRIM(vsReferencia) || ' ' || TRIM(vsReferenciaTransaccion);
		LET vsDocumento = PREFIJO_SEC_EL || LPAD ( TRIM ( vsNuevaSecuencia ), 6, '0' ) ;
		--LET vsFolioSucursal = psUsuarioParametros || vsDocumento ;
		
        		
		LET vsFolioSucursal = psNumTarjeta; ---referencia corresponsales
		
		LET vsSecEnviadaCentral = vsDocumento ;
		LET vsTipoEnviadaCentral = 'C' ;
		
		 IF EnLinea = 'E' THEN
            LET vsTipoEnviadaCentral = 'E' ;
         END IF
		
		--GUARDA EL REGISTRO CORRESPONDIENTE A LA TRANSACCION ACTUAL
		EXECUTE PROCEDURE  intercard:sp_InsertarCentral ( psArchivoOrigen, vsTipoEnviadaCentral, TC_CORRESPONSAL,
		vsFolioSucursal, TS_CORRESPONSAL, psNumTarjeta, vsDocumento, pmMonto, psMoneda,
		vsReferencia, vsFolioOrig, vsDocumentoOrig, vsTransaccionOrig, vmMontoOrig, pmMontoCashBack , 
    ...
    -- TIPO 25
    ELIF ( psTipoTransaccion = '21' ) THEN 

        LET ESTADO1 = '25' ;

        EXECUTE PROCEDURE intercard:sp_GetSecuencia (0) INTO vsNuevaSecuencia ;

        LET viGenRefTipoMovimiento = 3 ;        --genera la referencia para el registro de la comision a cobrar por el movimiento
        EXECUTE PROCEDURE intercard:sp_GeneraReferencia( viGenRefTipoMovimiento, vsNuevaSecuencia, '' ) INTO vsReferencia ;
        
        LET vsDocumento = PREFIJO_SEC_FL || LPAD ( TRIM ( vsNuevaSecuencia ), 6, '0' ) ;
        LET vsFolioSucursal = PREFIJO_SEC_I || REPLACE (SUBSTRING (CURRENT::DATE FROM 1 FOR 5), '/', '' ) || REPLACE (SUBSTRING (CURRENT FROM 12 FOR 6), ':', '' ) || vsDocumento;
        ...
        --GUARDA EL REGISTRO CORRESPONDIENTE A LA TRANSACCION ACTUAL
        EXECUTE PROCEDURE  intercard:sp_InsertarCentral ( psArchivoOrigen, vsTipoEnviadaCentral, TC_DEVOLUCION_POS,
        vsFolioSucursal, TS_DEVOLUCION_POS, psNumTarjeta, vsDocumento, pmMonto, psMoneda,
        vsReferencia, vsFolioOrig, vsDocumentoOrig, vsTransaccionOrig, vmMontoOrig, pmMovMontoCashBack , 
        psRFC, vsReferenciaTransaccion, psDivisa, pmMontoDivisa, psFormadePago, vsNumCajero, psTipoTransInterEmpresa, 
        pmMontoComInterEmpresa, psConvenioInterEmpresa ) INTO vsREGISTROCENTRAL1 ;

        LET vsDESCRIPCION2 = 'La devolucion con secuencia: ' || PREFIJO_SEC_FL || vsNuevaSecuencia || ' y monto de: ' || pmMonto 
            || ' no se le encontro un movimiento original ' ;

            LET viStatusConciliacion  = 2 ; 
        
     -- TIPO 26
    ELIF ( ( psTipoTransaccion = '01' ) AND ( psCodTran = '00' ) ) THEN 

        LET ESTADO1 = '26' ;

        LET vsDESCRIPCION2 = 'Compra POS Nacional o Internacional';

        LET viGenRefTipoMovimiento = 2 ;        --genera la referencia para el registro de la comision a cobrar por el movimiento
		
		EXECUTE PROCEDURE intercard:sp_GetSecuencia (viGenRefTipoMovimiento) INTO vsNuevaSecuencia  ; 
        EXECUTE PROCEDURE intercard:sp_GeneraReferencia( viGenRefTipoMovimiento, psNombreComercio, vsNuevaSecuencia ) INTO vsReferencia ;
        ...
        --GUARDA EL REGISTRO CORRESPONDIENTE A LA TRANSACCION ACTUAL
        EXECUTE PROCEDURE  intercard:sp_InsertarCentral ( psArchivoOrigen, vsTipoEnviadaCentral, TC_COMPRA,
        vsFolioSucursal, TS_COMPRA, psNumTarjeta, vsDocumento, pmMonto, psMoneda,
        vsReferencia, vsFolioOrig, vsDocumentoOrig, vsTransaccionOrig, vmMontoOrig, pmMovMontoCashBack , 
        psRFC, vsReferenciaTransaccion, psDivisa, pmMontoDivisa, psFormadePago, vsNumCajero, psTipoTransInterEmpresa, 
        pmMontoComInterEmpresa, psConvenioInterEmpresa ) INTO vsREGISTROCENTRAL1 ;
        ...
        IF ( pmMovMontoComision > 0) THEN
            ...
            EXECUTE PROCEDURE intercard:sp_GetSecuencia (0) INTO vsNuevaSecuencia  ; -- CON 0 OBTIENE LA SECUENCIA PARA EL COBRO DE LA COMISION

	        LET vsDocumento = PREFIJO_SEC_FL || LPAD ( TRIM ( vsNuevaSecuencia ), 6, '0' ) ;
            LET vsFolioSucursal = PREFIJO_SEC_I || REPLACE (SUBSTRING (CURRENT::DATE FROM 1 FOR 5), '/', '' ) || REPLACE (SUBSTRING (CURRENT FROM 12 FOR 6), ':', '' ) || vsDocumento;		
			
            LET viGenRefTipoMovimiento = 10 ;            --genera la referencia para el registro de la comision a cobrar por el movimiento
            EXECUTE PROCEDURE intercard:sp_GeneraReferencia( viGenRefTipoMovimiento, psNumTarjeta, '' ) INTO vsReferenciaComision  ;
        ...
        END IF
            ...
            EXECUTE PROCEDURE  intercard:sp_InsertarCentral ( psArchivoOrigen, vsTipoEnviadaCentral, TC_CCOMPRA, 
            ...
            LET vsSecuenciaComision = PREFIJO_SEC_FL || LPAD ( TRIM ( vsNuevaSecuencia ), 6, '0' ) ; 
    ...
    -- TIPO 27
    ELIF ( ( psTipoTransaccion = '01'  OR psTipoTransaccion = '09' ) AND ( psCodTran = '09' ) AND ( pmMonto > 0) AND ( pmMonto > pmMontoCashBack ) ) THEN 

        LET ESTADO1 = '27' ;

        LET vsDESCRIPCION2 = 'Compra POS Nacional o Internacional + CASHBACK ';
        LET viTipodeTransaccion =  203 ; --COMPRA_POS
        ...
        --GUARDA EL REGISTRO CORRESPONDIENTE A LA TRANSACCION ACTUAL
        EXECUTE PROCEDURE  intercard:sp_InsertarCentral ( psArchivoOrigen, vsTipoEnviadaCentral, TC_COMPRA,
        vsFolioSucursal, TS_COMPRA, psNumTarjeta, vsDocumento, ( pmMonto - pmMontoCashBack ), psMoneda,
        vsReferencia, vsFolioOrig, vsDocumentoOrig, vsTransaccionOrig, vmMontoOrig, pmMontoCashBack , 
        psRFC, vsReferenciaTransaccion, psDivisa, pmMontoDivisa, psFormadePago, vsNumCajero, psTipoTransInterEmpresa, 
        pmMontoComInterEmpresa, psConvenioInterEmpresa ) INTO vsREGISTROCENTRAL1 ;
		...
		EXECUTE PROCEDURE intercard:sp_GetSecuencia (viGenRefTipoMovimiento) INTO vsNuevaSecuencia  ; -- obtienen la secuencia del cashback
		EXECUTE PROCEDURE intercard:sp_GeneraReferencia( viGenRefTipoMovimiento, 'CASH BACK', vsNuevaSecuencia ) INTO vsReferencia ;


        LET vsDocumento = PREFIJO_SEC_CASH_BACK || LPAD ( TRIM ( vsNuevaSecuencia ), 6, '0' ) ;
        LET vsFolioSucursal = PREFIJO_SEC_I || REPLACE (SUBSTRING (CURRENT::DATE FROM 1 FOR 5), '/', '' ) || REPLACE (SUBSTRING (CURRENT FROM 12 FOR 6), ':', '' ) || vsDocumento;
        ...
        --GUARDA EL REGISTRO CORRESPONDIENTE A LA TRANSACCION ACTUAL
        EXECUTE PROCEDURE  intercard:sp_InsertarCentral ( psArchivoOrigen, vsTipoEnviadaCentral, TC_CASHBACK,
    ...
    ELIF ( ( psTipoTransaccion = '01' OR psTipoTransaccion = '09' ) AND ( psCodTran = '09' ) AND ( pmMonto > 0) AND ( pmMonto = pmMontoCashBack ) ) THEN 
        ...
        LET vsDESCRIPCION2 = 'Compra POS Nacional o Internacional + CASHADVANCE';
        LET viGenRefTipoMovimiento = 21 ;        --genera la referencia para el registro de la comision a cobrar por el movimiento	
        EXECUTE PROCEDURE intercard:sp_GetSecuencia (viGenRefTipoMovimiento) INTO vsNuevaSecuencia  ; -- obtienen la secuencia del cashback
        EXECUTE PROCEDURE intercard:sp_GeneraReferencia( viGenRefTipoMovimiento, 'CASH ADVANCE', vsNuevaSecuencia ) INTO vsReferencia ;
        ...
        --GUARDA EL REGISTRO CORRESPONDIENTE A LA TRANSACCION ACTUAL
        EXECUTE PROCEDURE  intercard:sp_InsertarCentral ( psArchivoOrigen, vsTipoEnviadaCentral, TC_CASHADVANCE,
        ...
        IF ( pmMovMontoComCashBack > 0) THEN                
			...
			LET viGenRefTipoMovimiento = 21 ;            --genera la referencia para el registro de la comision a cobrar por el movimiento
			EXECUTE PROCEDURE intercard:sp_GetSecuencia (viGenRefTipoMovimiento) INTO vsNuevaSecuencia  ; -- obtienen la secuencia del cashback
            EXECUTE PROCEDURE intercard:sp_GeneraReferencia( viGenRefTipoMovimiento, 'COMISION CASH ADVANCE', vsNuevaSecuencia ) INTO vsReferencia ;
        ...
        END IF
            ...
            -- CUARDA EL REGISTRO CORRESPONDIENTE AL COBRO DE LA COMISION DE LA TRANSACCION ACTUAL
            EXECUTE PROCEDURE  intercard:sp_InsertarCentral ( psArchivoOrigen, vsTipoEnviadaCentral, TC_CCOMISIONCASHADVANCE, 
            vsFolioSucursal, TC_CCOMISIONCASHADVANCE, psNumTarjeta, vsDocumento, pmMovMontoComCashBack, psMovMoneda, 
            vsReferenciaComision , vsFolioOrig , vsDocumentoOrig, vsTransaccionOrig , vmMontoOrig, pmMontoCashBack , 
            psRFC , vsReferenciaTransaccion, psDivisa, pmMontoDivisa, psFormadePago, vsNumCajero, '', 
            0.0, '' ) INTO vsREGISTROCENTRAL2 ;                

            LET vsMovSecuenciaComCashBack =  vsDocumento ;
        END IF ;
    ...
    ELIF ( psTipoTransaccion = '02' )  THEN 
        ...
        LET vsDESCRIPCION1 = 'Aviso. La txn es un Retiro ATM Internacional' ;
        LET vsDESCRIPCION2 = ' se tratará como compra.' ;

		EXECUTE PROCEDURE intercard:sp_GetSecuencia (0) INTO vsNuevaSecuencia ;
        LET viGenRefTipoMovimiento = 2 ;        
        EXECUTE PROCEDURE intercard:sp_GeneraReferencia( viGenRefTipoMovimiento, vsNuevaSecuencia, '' ) INTO vsReferencia ;
    ...
    -- TIPO 30
    ELIF ( psTipoTransaccion = '20' )  THEN 

        LET ESTADO1 = '30' ;

        LET vsDESCRIPCION1 = 'Pago Interbancario' ;
        ...
        LET vsDocumento = PREFIJO_SEC_INTERBACARIO || LPAD ( TRIM ( vsNuevaSecuencia ), 6, '0' ) ;
        LET vsFolioSucursal = PREFIJO_SEC_I || REPLACE (SUBSTRING (CURRENT::DATE FROM 1 FOR 5), '/', '' ) || REPLACE (SUBSTRING (CURRENT FROM 12 FOR 6), ':', '' ) || vsDocumento;
        ...
        --GUARDA EL REGISTRO CORRESPONDIENTE A LA TRANSACCION ACTUAL
        EXECUTE PROCEDURE  intercard:sp_InsertarCentral ( psArchivoOrigen, vsTipoEnviadaCentral, TC_PAGO_INTERBANCARIO,
        vsFolioSucursal, TS_PAGO_INTERBANCARIO, psNumTarjeta, vsDocumento, pmMonto, psMoneda,
        vsReferencia, vsFolioOrig, vsDocumentoOrig, vsTransaccionOrig, vmMontoOrig, pmMontoCashBack , 
        psRFC, vsReferenciaTransaccion, psDivisa, pmMontoDivisa, psFormadePago, vsNumCajero, psTipoTransInterEmpresa, 
        pmMontoComInterEmpresa, psConvenioInterEmpresa ) INTO vsREGISTROCENTRAL1 ;
...
'Proyecto: Conciliacion Automatica',
'Solicito: Jose Luis Puebla',
'Descripcion: Se modificaron los codigos de transaccion para los registros de Tiendas Copppel -InterRedes- para que se diferencien de las de EGlobal',
'Fecha: 2009/11/05',
'Version: 2009115.1204',
'BD: Intercard',
'',
'AUTOR: Hector Juan Casanova Edeza',
'Proyecto: Conciliacion Automatica',
...
'Folio: ',
'Solicito: Jose Luis Puebla',
'Descripcion: Se modifico el flujo para que contemple los nuevos archivos archivos de corresponsales de Transferencias de Depositos(TPD).',
'Fecha: 27/05/2011',
'Version: 20110527.0925',
'BD: Intercard',
*/;
