CREATE PROCEDURE "informix".sp_altabajaterceros_bpi(p_sTipoOperacion CHAR(2), p_sClaveCuenta CHAR(2), p_sNum_Cte CHAR(20), 
p_sCuenta CHAR(20), p_sCve_Banco CHAR(3), p_sDesc_cta CHAR(20), p_sNombre CHAR(60), p_sRfc CHAR(13), p_sEmail CHAR(100), 
...
DEFINE vExisteCuenta INTEGER;
DEFINE v_iContador INTEGER;
DEFINE v_canal INTEGER;
DEFINE v_iCuentasEfectivas INTEGER;
DEFINE v_iCuentasEfectivasActivas INTEGER;
DEFINE v_cve_compania INTEGER;
DEFINE v_cve_estado_bex INTEGER;
DEFINE v_cve_estado INTEGER;
...
-- *******************************************************************************************
	...
	--Modific?Ã?: Berenice Noriega
	--De: BanCoppel/Mantenimiento III/Coordinaci?Ã?n Internet.
	--Fecha:09-Diciembre-2013
	--Modificaci?Ã?n: Se agregan el producto 2700 en la validaci?Ã?n "--Se valida que el cliente tenga por lo menos una cuenta efectiva"
	...
	-- Se agrega validaci?Ã?n para los tel?Ã?fonos frecuentes que son dados de alta en sucursal sin digito verificador,
	-- si ese es el caso, se permite la actualizaci?Ã?n del registro cuando el usuario intenta darlo de alta desde el portal ya con d?Ã?gito verificador.
	-- Bibiana Gaxiola Verdugo
	-- 06/03/2014
	...
	-- Se agrega validaci?Ã?n para n?Ã?meros moviles en SPEI, para que no se pueda agregar el mismo n?Ã?mero con bancos distinto
	-- Bibiana Gaxiola Verdugo
	-- 05/11/2014
	
	-- Se agrega validaciÃ?n de cuenta existente para retornar codigo '90000', 'la cuenta ya existe'. 
	-- Modifico: Jorge Bibriesca
	-- Fecha: 03/08/2017
	
	--Se optimiza spl de acuerdo a observaciones realizada por BD
	-- Modifico: Gabriela Aguilar
	-- Fecha: 26/12/2018
	
	
	--Se actualiza el campo p_sEmail de char(40) hacia char(100)
	-- Modifico: Gabriela Aguilar
	-- Fecha: 26/06/2020
	-- subido a QA 16/12/2025
...
LET v_cve_estado = 0;
LET v_cve_estado_bex = 0;
LET vClaveCuenta ='';
LET vValTD = "";	
LET vValTemp = "";	
--Inicio del procedimiento

	-- SET DEBUG FILE TO "/ifxsif01/JuanRivera/traces/sp_altabajaterceros_bpi.out";
	-- TRACE ON;
...
BEGIN
	...
	--Se valida que la clave de la compania no este en blanco o en nulo
	LET p_sCve_Compania =NVL(p_sCve_Compania,'');
	
	IF (p_sCve_Compania = '') THEN
		SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet FROM "informix".pp_mensajes where cve_mensaje = '08';
		RETURN v_sCodRet, v_sMensajeRet;
	END IF;
	--Se valida que exista la clave de la cuenta si no existe manda el codigo de retorno 03 con su descripcion
	SELECT COUNT(*) INTO v_iContador FROM  bdiprog:"informix".pp_tpcuenta WHERE cve_cuenta = p_sClaveCuenta;
	...
	SELECT COUNT(*) INTO v_iContador FROM bdinteg:"informix".si_cliente WHERE numcte = p_sNum_Cte;
	IF (v_iContador=0) THEN
 ...
 --Se quita la validaci?Ã?n por k esta mal hecha JGP - 15/05/2009---Solucionado el dia 25-mayo-2009
	SELECT count(*) INTO v_iCuentasEfectivas FROM bdicheq:"informix".sc_maechq WHERE num_cte = p_sNum_Cte
				AND producto in (select producto from bdicheq:"informix".sc_producto where producto in('2000','1300','1400','1600','1800','1500','1700','1900','2500','1200','1600','2200','2600','2700','2900'));
	
	IF(v_iCuentasEfectivas > 0) THEN
		SELECT count(*) INTO v_iCuentasEfectivasActivas FROM bdicheq:"informix".sc_maechq  	WHERE num_cte = p_sNum_Cte
	...
	IF (p_sClaveCuenta = '01') OR (p_sClaveCuenta = '04') THEN
        --IF p_sTipoOperacion = '01' THEN
            ...
            IF p_sTipoOperacion = '01' THEN
				SELECT count(*) INTO  v_iCuenta FROM bdicheq:"informix".sc_maechq WHERE num_cte = p_sNum_Cte AND producto in (select producto from bdicheq:"informix".sc_producto where producto in('2000','1300','1400','1600','1800','1500','1700','1900','2500','1200','1600','2200', '2600','2700','2900'));
					IF  (v_iCuenta>0) then
					Select count(*) INTO  v_iCuenta  FROM bdicheq:"informix".sc_maechq WHERE num_cte = p_sNum_Cte
								AND producto in (select producto from bdicheq:"informix".sc_producto where producto in('2000','1300','1400','1600','1800','1500','1700','1900','2500','1200','1600','2200', '2600','2700','2900'));
					ELSE
						SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet FROM bdiprog:"informix".pp_mensajes where cve_mensaje = '217';
						RETURN v_sCodRet, v_sMensajeRet;
...
-- Validaci?Ã?n temporal, en lo que se libera a Nivel Nacional y en Internet
...
--
	...
	SELECT COUNT(*) INTO v_iContador FROM bdinteg:"informix".si_bancos WHERE banco = p_sCve_Banco;
			IF (v_iContador = 0) THEN
	...
	IF (v_iCelular < 10 AND v_iCelular > 0) THEN
		SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet FROM "informix".pp_mensajes where cve_mensaje = '16';
		RETURN v_sCodRet, v_sMensajeRet;
	END IF;
	...
	END WHILE;
	IF (v_sValor = 'B') THEN
		SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet FROM "informix".pp_mensajes WHERE cve_mensaje = '207';
...
--		END IF;
	...
	IF (p_sClaveCuenta = '01') THEN
		...
		IF  v_sCuenta <> ''  THEN
			SELECT COUNT(*)
			INTO v_iContador
			FROM bdiprog:"informix".pp_producperm 
			WHERE producto = v_sProductoPerm AND permite_prog = 'S';
	...
	IF (p_sTipoOperacion = '01') THEN
		...
		SELECT COUNT(*)
		INTO v_iContador
		FROM bdiprog:"informix".pp_tpcanal 
		WHERE cve_canal = p_sCanal_Alta;
		...
		--Se valida que exista el numero de cliente, la cuenta y la clave del banco sino se agrega y se manda el codigo de retorno '00' con su descripcion
        	...
        	SELECT count (*) INTO v_iContador FROM bdiprog:"informix".pp_ctasterceros WHERE num_cte = p_sNum_Cte AND cuenta = p_sCuenta AND cve_banco = p_sCve_Banco;
			IF v_iContador>0 THEN
			...
			IF (v_sCveEstado = '02') THEN
               ...
               --- se valida el que el pClaveCuenta corresponda a  el pCuenta 
				...
				IF (p_CveCaducidad = '1') THEN -- Caducidad 48 horas (2 d?Ã?as)
                ...
                DELETE FROM bdiprog:"informix".pp_ctasterceros_bex WHERE num_cte = p_sNum_Cte AND cuenta = p_sCuenta AND descrip_cta = p_sDesc_cta;
                ...
                DELETE FROM bdiprog:"informix".pp_ctasterceros_bex WHERE num_cte = p_sNum_Cte AND cuenta = p_sCuenta AND descrip_cta = p_sDesc_cta;
				ELIF (p_CveCaducidad = '3') THEN -- Caducidad indefinida - 1 a?Ã?o
				...
				ELSE
					LET v_sCodRet = '00002';
					LET v_sMensajeRet = 'Caducidad inv?Ã?lida';
					RETURN v_sCodRet, v_sMensajeRet;
				END IF;
				
				SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet FROM bdiprog:"informix".pp_mensajes where cve_mensaje = '00';
			
			ELIF (v_sCveEstado = '01') THEN --- Para los tel?Ã?fonos frecuentes que son registrados desde sucursal y no tienen digito verificador
				
                IF (p_sClaveCuenta = '05' AND p_sCve_Banco = '201') THEN
					IF (p_CveCaducidad = '1') THEN -- Caducidad 48 horas (2 d?Ã?as)
						LET vFechaCaducidad = TODAY + 2 UNITS DAY;
					ELIF (p_CveCaducidad = '2') THEN -- Caducidad 6 meses
						--LET vFechaCaducidad = TODAY + 180 UNITS DAY;
					...
					user_insert = p_sUser_Insert,fecha_insert = current, hora_insert = current ,cve_caducidad = p_CveCaducidad, fecha_caducidad = vFechaCaducidad
					WHERE num_cte = p_sNum_Cte AND cuenta = p_sCuenta AND cve_banco = p_sCve_Banco AND cve_cuenta = p_sClaveCuenta;
				...
				END IF;
					...
					LET v_sCodRet = '90000'; 
					LET v_sMensajeRet = 'La cuenta ya existe'; 
					RETURN v_sCodRet, v_sMensajeRet;
		...
		ELSE
			...
			IF (p_sClaveCuenta = '07') THEN --- Para n?Ã?meros moviles en transferencias SPEI 
				-- Se valida que ya exista el n?Ã?mero movil para el cliente
				IF (SELECT count(cuenta) FROM bdiprog:"informix".pp_ctasterceros WHERE num_cte = p_sNum_Cte AND cuenta = p_sCuenta) = 1 THEN
					...
					--Si es el mismo movil para el cliente pero con diferente banco se env?Ã?a mensaje de que la cuenta ya existe
		...
		END IF;
	ELSE
IF (p_sTipoOperacion = '03') THEN
		
			--Se valida el canal
				SELECT COUNT(*) 
		INTO v_iContador
		FROM bdiprog:"informix".pp_tpcanal 
		WHERE cve_canal = p_sCanal_Baja;
		...
		END IF;
				...
				SELECT COUNT (*) INTO  v_iContador FROM  bdiprog:"informix".pp_ctasterceros_bex WHERE num_cte = p_sNum_Cte AND cuenta = p_sCuenta AND
		...
		IF v_iContador>0 THEN 
					...
					SELECT cve_estado INTO v_sCveEstado FROM bdiprog:"informix".pp_ctasterceros_bex WHERE num_cte = p_sNum_Cte AND cuenta = p_sCuenta AND
					cve_banco = p_sCve_Banco AND cve_cuenta = p_sClaveCuenta;
					IF v_sCveEstado = '01' THEN
						...
						SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet FROM bdiprog:"informix".pp_mensajes where cve_mensaje = '00';  
					ELSE --El registro ya esta Cancelado
		...
		ELSE
				...
				SELECT count (cve_canal) INTO v_canal FROM  bdiprog:"informix".pp_tpcanal WHERE cve_canal = p_sCanal_Baja;
				...
				SELECT count (cve_estado) INTO v_cve_estado FROM bdiprog:"informix".pp_ctasterceros WHERE num_cte = p_sNum_Cte AND cuenta = p_sCuenta AND
				cve_banco = p_sCve_Banco AND cve_cuenta = p_sClaveCuenta;
				 
				SELECT count(cve_estado) INTO v_cve_estado_bex FROM bdiprog:"informix".pp_ctasterceros_bex WHERE num_cte = p_sNum_Cte AND cuenta = p_sCuenta AND
				cve_banco = p_sCve_Banco AND cve_cuenta = p_sClaveCuenta;
				
	            IF ((v_cve_estado > 0 ) or (v_cve_estado_bex > 0) ) THEN
				...
				ELSE
					IF EXISTS(SELECT cuenta FROM bdiprog:pp_ctasterceros WHERE num_cte = p_sNum_Cte AND canal_alta = '18' AND cve_estado = '01' and cve_cuenta='01') THEN -- Validacion de cuentas dadas de alta en app
						FOREACH
							...
							AND cve_estado ='01'
								...
								SELECT num_tarjeta INTO vValTemp from bdicheq:sc_tarjeta where num_tarjeta = vValTD and cuenta = p_sCuenta;
									IF NVL(vValTemp,'') <> "" THEN 
									
										SELECT cve_estado INTO v_sCveEstado FROM bdiprog:"informix".pp_ctasterceros WHERE num_cte = p_sNum_Cte AND cuenta = vValTemp AND
										cve_banco = p_sCve_Banco AND cve_cuenta = p_sClaveCuenta;
										IF v_sCveEstado = '01' THEN
											UPDATE bdiprog:"informix".pp_ctasterceros SET cve_estado = '02',canal_baja = p_sCanal_Baja,fecha_estado = current,
											user_insert = p_sUser_Insert WHERE num_cte = p_sNum_Cte AND cuenta = vValTemp AND cve_banco = p_sCve_Banco ;
											SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet FROM bdiprog:"informix".pp_mensajes where cve_mensaje = '00';  
										ELSE --El registro ya esta Cancelado
											SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet FROM bdiprog:"informix".pp_mensajes where cve_mensaje = '10';
										END IF;
								...
								SELECT cuenta_clabe INTO vValTemp from bdicheq:sc_maechq where cuenta_clabe = vValTD and cuenta = p_sCuenta;
									IF NVL(vValTemp,'') <> "" THEN 
										SELECT cve_estado INTO v_sCveEstado FROM bdiprog:"informix".pp_ctasterceros WHERE num_cte = p_sNum_Cte AND cuenta = vValTemp AND
										cve_banco = p_sCve_Banco AND cve_cuenta = p_sClaveCuenta;
										IF v_sCveEstado = '01' THEN
