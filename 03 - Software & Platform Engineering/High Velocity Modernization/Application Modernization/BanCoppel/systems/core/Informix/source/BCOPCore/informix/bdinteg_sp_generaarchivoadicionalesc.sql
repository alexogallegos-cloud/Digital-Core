CREATE PROCEDURE "informix".sp_generaarchivoadicionalesc(pempresa CHAR(3), pFechaAct DATE)
RETURNING
     CHAR(6);
...
--Set debug file to '/tmp/sp_GeneraArchivoAdicionalesC.out';
--trace on;
		...
		ELSE	
			...
			FOREACH
				...
				IF vnumcte <> '' AND vcliente_ref <> '' THEN
					...
					LET vsSQL = vclave||"|"||vcaja||"|"||varea||"|"||TRIM(vcliente_ref)||"|"||vnombre1||"|"||vnombre2||"|"||vapell_paterno||"|"||vapell_materno||"|"
					...
					LET vsSQL = vsSQL|| "|" ||vclavereferencia1||"|"||vclientereferencia2||"|"||vnombreunoreferencia2||"|"||vnombredosreferencia2||"|"||vapellidopaternoreferencia2||"|"||vapellidomaternoreferencia2||"|" 
							...
							||vtiporeposicion||"|"||vreposicion|| "|" ||vflagentregotarjeta|| "|" || NVL(vefectuo, 0)|| "|" || NVL(vtiendafolio, 0) || "|" ||vfolio||"|"||TRIM(NVL(cfechaaltacte, '1900/01/01'))||"|"   
							||vflagnoreconocehuella|| "|"||vfoliotienda|| "|" ||vrfc||"|"||vcveburo|| "|" ||vfolioaut|| "|" ||vfolioconsulta|| "|" ||vfolioconcir|| "|" ||vnegocio||"|"||vsubnegocio|| "|"   
							||vempleadoautorizo|| "|" ||vtipo|| "|"||TRIM(NVL(cfechamovto, '1900/01/01'))||"|"||vnumerosolicituddecredito|| "|" ||vnumcte|| "|" ||vtiendafolioanterior||"|"||vfolioanterior||"|" 
