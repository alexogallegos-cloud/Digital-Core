CREATE PROCEDURE "informix".sp_reporte_ctes_dirty_behavior(pEmpresa CHAR(3))
...
--SET DEBUG FILE TO "sp_reporte_ctes_dirty_behavior.out";
--TRACE ON;
...
BEGIN
    ...
    SELECT trim(valor) INTO cParamNomArch FROM bdicred:sd_param WHERE cod_param = 105;
    ...
    LET cNomArchEjecSql = 'Reporte_exclusion_Ctes_Dirty.sql';
    ...
    LET cSQL = ' echo "SET ISOLATION TO DIRTY READ; UNLOAD TO ' || TRIM(cRutaArch) || TRIM(cNomArchivo1) || " DELIMITER " || '''|'''
        ...
        || ' candidato_buro FROM bdicred:sd_clientes_dirty_behavior WHERE month(fecha_reporte) = month( ' || '''' || dFechaHoy || '''' 
