CREATE PROCEDURE "informix".sp_reporteremesascomision_pbajj()
...
--OBTIENE LOS DIAS INICIO Y FIN DEL MES ANTERIOR
		...
		--LET cFechaIni=mdy(1,1,2019);
		--LET cFechaFin=mdy(1,31,2019);
...
-- IDENTIFICAMOS LOS NUMEROS DE FOLIO_SUC DE TODAS LAS REMESAS
	SELECT {+INDEX(sac_movimientoshistorial idxsac_movhis236 )} DISTINCT (folio_suc) 
...
-- OBTENEMOS LOS DATOS DE LA TABLA SAC_MOVIMIENTOSHISTORIAL
	SELECT {+INDEX(sac_movimientoshistorial idxsac_movhis236 )} sac.id_sucursal,sac.folio_suc,sac.numcategoria,sac.numconvenio,sac.forma_pago,sac.importe_pago,sac.importe_comision_convenio,sac.fecha_pago
...
-- OBTENEMOS LOS DATOS DE LA TABLA SC_MOVHIS_OLD
	SELECT {+INDEX( bdicheq:"informix".sc_movhis_old idx_movhisnew7_old )} tempsac.folio_suc 
...
---COMPLEMENTO DE SC_MOVHIS
	...
	SELECT {+INDEX( bdicheq:"informix".sc_movhis idx_movhisnew7 )} tempsac.folio_suc 
...
--SE LLENAN LOS DATOS DE SUCURSALES EN LA TABLA DEL REPORTE
	INSERT INTO "informix".sac_reportcom_rem (nombre_sucursal,id_sucursal)
		SELECT  DISTINCT REPLACE(REPLACE(REPLACE(REPLACE(S.nombre,"\","-"),"."," "),","," "),"/","-"),M.id_sucursal 
...
--INDICE PARA TABLA FISICA FINAL
	...
	FOREACH vCursor WITH HOLD FOR SELECT id_sucursal INTO vId_suc FROM "informix".sac_reportcom_rem 
		...
		SELECT a.id_sucursal, 
			    ...
			    sum(case when numconvenio='004' then 0 else 0 end) as comision_BTS, -- se elimina importe_comision_convenio para enviar comisiÃ³n CERO
