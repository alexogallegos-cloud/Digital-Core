CREATE PROCEDURE "informix".sp_status_sol_aud2(pempresa CHAR(3),psucursal CHAR(4),pfechafin CHAR(10),pfechaini CHAR(10),pstatus CHAR(2),pRegistros INTEGER,pRecuperacion INTEGER)
...
ON EXCEPTION SET iSqlerr
   IF iSqlerr != 0 THEN
      ...
      RETURN cCod_ret, NVL(cNumSol,""), NVL(cNumcte,""), NVL(dtFechaSol,DATE(1)), NVL(cNomCte,""),
...
--SET DEBUG FILE TO "/tmp/mfinis/sp_status_sol_aud2.out";
--TRACE ON;
-- **********************************************************************
-- *                        PROGRAMA PRINCIPAL
-- **********************************************************************
	...
	FOREACH WITH HOLD
		...
		AND a.status_solicitud = nvl(pStatus, a.status_solicitud)
		AND  a.sucursal = nvl(pSucursal, a.sucursal)
	...
	--Obtencion de telefonos
      ...
      -----------------------------MONTO AUTORIZADO--------------------------------
		...
		IF cStatus='AP' THEN
			SELECT LIMIT 1 NVL(eje.nombre,"")--Usuario que aperturo el crÃ©dito.
			INTO cNombreProm
		...
		RETURN cCod_ret, NVL(cNumSol,""), NVL(cNumcte,""), NVL(dtFechaSol,DATE(1)), NVL(cNomCte,""),
         NVL(cStatus,""), NVL(mMonto,0), NVL(mMonto_aut,0), NVL(dtFechaCam,DATE(1)),NVL(cNombreProm,""),NVL(cTelCasa,""),NVL(cTelCel,""),NVL(cTelOfi,""),NVL(cSucursal,"") WITH RESUME;	
        LET cTelCasa = "";
		LET cTelCel = "";
		LET cTelOfi = "";
		LET mMonto_aut=0;
...
'FECHA: 04/08/2017',
'DESCRIPCION: Se realiza spl clon para agregar los parÃ¡metros de paginado.';
