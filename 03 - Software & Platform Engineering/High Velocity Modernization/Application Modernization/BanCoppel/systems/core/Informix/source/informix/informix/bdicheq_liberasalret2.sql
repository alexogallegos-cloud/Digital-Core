Create Procedure "informix".liberasalret2(pempresa char(3), pejecutivo char(10))
Returning char(5);
    ...
    let vdescerr  = "";
    let vconproc  = 0;
    let vproceso  = "libsalretchqCCE";  -- <- Renombrado para evitar conflicto -971
    let vsistema  = "01";
    let vRetenido = 0;
    let vabierto  = "0";
    let vcomienza = -1;
    ...
    --set debug file to "/tmp/mfinis/liberatranret.out";
    --trace on;
    ...
    on exception set vsqlerr, visamerr, vdescerr
        ...
        if vsqlerr <> 0  then
            ...
            UPDATE bdinteg:sx_contproc SET ejecutivo = pejecutivo, status_proc = 'C', codret = vcodret,
    ...
    if vexiste = 0 then
        ...
        INSERT INTO bdinteg:sx_contproc VALUES (pEmpresa, vproceso, vfecha_hoy, vsistema, 'I', pejecutivo, 
        ((SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas)), NULL, NULL);

    else

	----------------hasta qui ambientado
        select count(*)   
        into vexistefin
        from bdinteg:sx_contproc  
        where empresa     = pempresa  
        ...
        if vexistefin = 0 then
            ...
            UPDATE bdinteg:sx_contproc SET ejecutivo = pejecutivo, status_proc = 'I', codret = vcodret,
            hora_fin = (SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas)
            WHERE empresa = pempresa
            AND proceso = vproceso
            AND fecha = vfecha_hoy
    ...
    foreach principal with hold for
        ...
        foreach with hold
            select /*rowid,*/ cuenta, transacc, dias_ret, monto, fecha_alta, cancelado, num_chq, monto_ori, folio_suc
    ...
    UPDATE bdinteg:sx_contproc SET ejecutivo = pejecutivo, status_proc = 'F', codret = vcodret,
    hora_fin = (SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas)
    WHERE empresa = pempresa
    AND proceso = vproceso
    AND fecha = vfecha_hoy
...
DOCUMENT 'AUTOR: Karlos GÃ³mez DÃ­az',
'FECHA: 08/07/2025',
'MODULO: CAMARAS',
'FUNCIONALIDAD: Liberacion Saldo Retenido',
'DESCRIPCION: Mantenimiento correctivo para evitar error -971 al registrar el proceso en tabla de control. Se renombrÃ³ la variable vproceso a "libsalretchqCCE"',
'BD: bdicheq';

create procedure "informix".cargo_ref_mib( pempresa    char(3),
                                       psucursal   char(4),
                                       pusuario    char(8),
                                       ptransacc   char(4),
                                       ptransuc    char(4),
...
returning char(5), char(4), date, money(14,2), money(14,2);
   ...
   -- // 05/06/2021
    ...
    -- // 05/06/2021
        ...
        -- // Valida que exista la transaccion de cargo y determina el tipo de transacciÃ³n
        select tipo_tran
          into vtiptran
          from bdinteg:si_transacc
         where empresa = pempresa
        ...
        if (vuser_limit is not null or vuser_limit <> '') then 
            -- // validaciÃ³n adicional para reconocimiento de canal 120612
        ...
        if ptransacc = '0223' then
            ...
            if ( vstatus = 0 or vstatus is null ) then
                ...
                if vcodretver = '000' then
                    ...
                    EXECUTE PROCEDURE sp_cons_sdodisp_x_tpcalculo('', vsdo_cuenta, vsdo_retenido, vsdo_cong, mSaldoSbc, vimp_chq_sbg, null, null, 'F', 1) INTO cCodRetConsSdo,cMensajeRetConsSdo,vsdo_disponible;
        ...
        -- // Obtiene saldo disponible de la cuenta despues de la transacciÃ³n de cargo
        --- select sdo_actual - (sdo_retenido + sdo_cong + imp_sbg_ccc), com_pendiente
        --- into vsdodisp, vcompend
        
        --RQM 09 704. Se agrega el llamado al SP de consulta de saldo con el tipo de calculo requerido para esta operacion. EEAP
        EXECUTE PROCEDURE sp_cons_sdodisp_x_tpcalculo(pcuenta, null, null, null, null, null, null, null, 'T', 1) INTO cCodRetConsSdo,cMensajeRetConsSdo,vsdodisp;
