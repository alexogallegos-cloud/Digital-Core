CREATE PROCEDURE "informix".sp_cargos_operaciones_esp(pempresa CHAR(3))
RETURNING CHAR(5), CHAR(5), INTEGER, INTEGER;
    ...
    DEFINE vexistectabloq   SMALLINT;
    ...
    LET vmontoret      = 0.00;
    LET vfecha_hoy     = '';
    LET vexistectabloq = 0;
    
    BEGIN

    ON EXCEPTION SET sql_err, isam_err
        SET DEBUG FILE TO "/resplogifx/conciliachq/sp_cargos_operaciones_esp.err";
        TRACE ON;
        IF sql_err <> 0 THEN
    ...
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/sp_cargos_operaciones_esp.out";
    --- TRACE ON;
    ...
    LET vsql = 'echo "LOAD FROM /resplogifx/conciliachq/cargos_operaciones_esp.unl DELIMITER ''","'' INSERT INTO ctasxcargar" > /resplogifx/conciliachq/cargos.sql';
    SYSTEM vsql;
    LET vsql = '';
    
    LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /resplogifx/conciliachq/cargos.sql';
    ---LET vstmt = 'dbaccess bdicheq /resplogifx/conciliachq/cargos.sql';
    SYSTEM vstmt;
    LET vstmt = '';
    ...
    FOREACH WITH HOLD
        ...
        IF vcodret_cargo = '000' THEN
            ...
            IF vexistectabloq > 0 THEN
                ...
                VALUES(pempresa, vcuenta, 'D', '00', null, 0.00, 'informix', vfecha_hoy, vhora, '1234', 'D', vfolio, vdescripcion);
            END IF;
        ...
        ELSE   
            ...
            IF vexistectabloq > 0 THEN
                ...
                INSERT INTO sc_ctabloqueohist (cuenta, clave, opcion)
                VALUES (vcuenta, '09', 3);
                
                INSERT INTO sc_histbloq (empresa, cuenta, tipo_mov, motivo, opcion, importe, usuario, fecha, hora, clave, status_blo, folio_suc, referencia)
                VALUES(pempresa, vcuenta, 'B', '09', 3, 0.00, 'informix', vfecha_hoy, vhora, '12345', 'B', vfolio, vdescripcion);
            ELSE
                INSERT INTO sc_ctabloqueo (cuenta, clave, opcion)
                VALUES (vcuenta, '09', 3);
