CREATE PROCEDURE "informix".cierrechqcomp12( pEmpresa CHAR(3) )
RETURNING CHAR(5);
    ...
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/altamasempnet/cierrechqcomp12.out";
    --- TRACE ON;
    
    BEGIN
    
    ON EXCEPTION SET viSqlErr, viIsamErr, vcDescErr
        SET DEBUG FILE TO "/resplogifx/conciliachq/altamasempnet/cierrechqcomp12.err";
        TRACE ON;
        IF viSqlErr <> 0 THEN
            ...
            LET vsql = 'echo "UPDATE bdinteg:sx_contproc '||
                       ...
                       'AND sistema   = '''||vcSistema||''';" > /tmp/horacierre12.sql';
            SYSTEM vsql;
            
            LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /tmp/horacierre12.sql';
            SYSTEM vstmt;
    ...
    IF viExiste = 0 THEN
        LET vsql = 'echo " INSERT INTO bdinteg:sx_contproc VALUES '||
                   ...
                   '(SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas), NULL, NULL);" > /tmp/horacierre12.sql';
        SYSTEM vsql;
        LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /tmp/horacierre12.sql';
        SYSTEM vstmt;
    ELSE
    ...
    FOREACH WITH HOLD
        SELECT SKIP 16000000 FIRST 8000000
