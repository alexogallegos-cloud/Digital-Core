CREATE PROCEDURE "informix".sp_descarga_arch_vartdc_bpajj(pfecha DATE)
...
CREATE PROCEDURE "informix".sp_descarga_arch_vartdc_pbajj(ptipoproducto CHAR(03), pfecha DATE)
       RETURNING CHAR(6) AS codigo, CHAR(200) AS mensaje;
...
CREATE PROCEDURE "informix".sp_burofisicas_cortos_pbajj()
RETURNING CHAR(6),
          CHAR(100);
...
into temp estados_sepomex with no log;
 ...
 ---CreaciÃ³n de indices por cada una de las validaciones de SEPOMEX
...
--RQM 09 325 rss --IPCB 10sep14- RQM 06 316//Incluir producto 7000 tarjeta platino
    ...
    IF vnum_producto IN ('6001','6600','7000','8100','8500') AND vStatusCred IN ('AA','E1') AND vmop != 'UR' THEN
...
create procedure "informix".burofisicas_pba_jj()
       returning char(5),
                 char(50);
   -- Modifs: 20200729
...
CREATE INDEX idx_sepomex ON sepomex(d_codigo,d_mnpio,estado_abrev,d_asenta) in dbs_movhis_idx5 online; -- 20200729 Macf
...
LET itempsepomex    = 1;
   ...
   let actualiza_esta = 0;
	...
	IF iCuenta_regs = 0 THEN
        ...
        --DROP INDEX "informix".idx_br_burofisicas_describe;  -- Ya No es necesario eliminar índice por que preexistira informacion RQI 21 220 MACF
...
--- Genera registro encabezado para Circulo
    ...
    --UPDATE STATISTICS MEDIUM FOR TABLE "informix".br_burofisicas_describe;  --Ya NO actualizar estadìsticas RQI 21 220
...
-- crea temporal pago de reestructuras -- reemplazar con indicadores de credito
    ...
    -- Segmento PE
		 ...
		 -- RQM 09 502
		 if cNumProducto in('6001','6600','7000','8100','8500') then
		    select a.num_tarjeta, a.status_tar
              into vnum_tarjeta, cStatus_tar		 
		      from bdicred:sd_tarjeta a
             where a.empresa = '001' and a.num_credito = vnum_credito
...
*/  --IPCB 10Nov20 FIN-Se mueve bloque de validacion  a donde se valida el vmonto	 		 
	...
	-- Mover este query que estaba más abajo para obtener el dato dmonto_autorizado antes, para ser usado en el monto_otorgado 
	-- si este es 1, '' ó nulo, Solo se mueve el query.
	...
	--Monto del Credito en la originacio IPCB
      ...
      END IF;
	  --Monto del Credito en la originacio IPCB
	  -- Mover este query que estaba más abajo para tener el dato de dmonto_autorizado, para ser usado en el límite de crédito si este es 1, '' ó nulo
...
DOCUMENT
'Fecha: 20201127',
'Modif: Optimizar proceso para reducir tiempo de ejecución',
'Autor: Marco A. Campos',
'Fecha: 20210122',
'Modificación: Agregar los productos Platinum(7000) y ORO(8100) en validación de estatus de credito anterior.',
'Autor: Marco A. Campos';

CREATE PROCEDURE "informix".burofisicas_cnr_pba()
       returning char(5),
                 char(50);
...
--- Genera registro encabezado para CÃ?Â­rculo
       let vheader = vencabezado1||vversion||vclave_usu||vnombre_usu||vciclo||vfecha_reporte||vuso_futuro||rpad(trim(vinf_adicional),98,"&");
...
--Se crea tabla temporal con los crÃ?Â©ditos a procesar
--IPCB Nov2019-Se modifica la extraccion de universo, excluyendo 6800 flexible, para generarlocorrectamente.
      ...
      SELECT a.numcte, a.num_producto, a.credito_externo,a.num_credito,NVL(a.status_cred,"E1") status_cred, b.monto_otorgado, b.fecha_ult_mov, 
						...
						,0 dispact, 1 sec_credito, date(1) fec_ultdisp_flex
       ...
       WHERE a.empresa = "001"
         ...
         AND a.num_producto NOT IN ( '6011','6900','6800')
...
--AAME 2021-02-03 RQM 10 1177 Se modifica para contemplar los 2 nuevos productos de prestamo (9100,9300)
--IPCB 07052018/Integra la extracciÃ?Â³n de FI
	INSERT INTO creditos_sel   
...
--Flex Vigentes con disposicion 
  ...
  SELECT a.numcte, a.num_producto, a.credito_externo,a.num_credito,NVL(a.status_cred,"E1") status_cred, lp.monto_linea monto_otorgado, b.fecha_ult_mov, 
         ...
         ,1 dispact, sec_credito,a.fecha_apertura fec_ultdisp_flex				
        FROM  bdicred:sd_maecredcontcrd a
...
--Flex SIN PRESTAMO DISPONIBLE
  ...
  SELECT a.numcte, a.num_producto, a.credito_externo,a.num_credito,"E1" status_cred, lp.monto_linea, b.fecha_ult_mov, 
	    ...
	    ,0 v_dispact, sec_credito,a.fecha_apertura					
        FROM bdicred:sd_maecredcrd a
...
--Flex CANCELADAS Y VENDIDAS
  ...
  SELECT a.numcte, a.num_producto, a.credito_externo,a.num_credito,status_cred, lp.monto_linea, b.fecha_ult_mov, 
                        lp.fecha_otorga, a.plazo,a.periodo_plazo, 'LR' tipo_contrato, a.campo_trab3
        ,1 v_dispact, sec_credito 	,a.fecha_apertura						
        FROM bdicred:sd_maecredcrd a
...
-- se crea temporal para pago maximo
  ...
  foreach with hold
      SELECT numcte, num_producto, credito_externo, num_credito,NVL(status_cred,"E1") status_cred, monto_otorgado, fecha_ult_mov, fecha_apertura, plazo, periodo_plazo, tipo_contrato,campo_trab3,dispact, sec_credito ,fec_ultdisp_flex
         into vnumcte,cNumProducto, cCredExterno,vnum_credito,vstatus_cred, vmonto_otorgado,vfecha_finiq,vpago_int,vnum_pagos,vfrecpago,tb_tipo_producto,vidbaja, v_dispact, vsec_credito,vfec_utdisp_flex
         from creditos_sel 
...
-- Agrega Delegacion o MunicipÃ?Â­o
...
-- INICIA SEGMENTO PE (EMPLEO DEL CLIENTE) --RQM 09 467_Version 14
  --Etiqueta PE -Nombre o Razo Social del empleador 
   ...
   IF cArma_PE = 'S' OR (vsegmento_pe_base = '' OR vsegmento_pe_base is NULL) THEN  --- VALIDACION CAMBIO DOM TRAB (Ini)
	...
	-- Agregar origen del domicilio De la razÃ?Â³n social (pais) 
	let vsegmento_pe = trim(vsegmento_pe)||'1802MX';
	let tb_origen_razon_soc  = 'MX';
...
--IPCB 26nov14: Agrega validacion para  saldo_act si <= 0 entonces monto_pagar	=0 para crÃ?Â©ditos con Baja	 
...
--Monto del CrÃ?Â©dito en la originacion RQM 09 467_Version 14
...
--IPCB 19mar14 - Se integran al insert vstatus_cred,cNumProducto
  ...
  BEGIN;
  CREATE INDEX "informix".idx_br_burofisicas_cnr          ON "informix".br_burofisicas_cnr(numreg) in dbs_movhis_idx3 ONLINE;
  COMMIT;
  --BEGIN;
