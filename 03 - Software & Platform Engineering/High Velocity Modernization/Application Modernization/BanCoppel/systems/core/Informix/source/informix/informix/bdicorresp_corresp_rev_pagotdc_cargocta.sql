CREATE PROCEDURE "informix".corresp_rev_pagotdc_cargocta( psucursal char(4), pusuario char(8), pfolio char(16) )
RETURNING CHAR(5);
    ...
    --- SET DEBUG FILE TO "./corresp_rev_pagotdc_cargocta.out";
    --- TRACE ON;
    ...
    ON EXCEPTION SET sql_err, isam_err
        IF (sql_err <> 0) THEN
            --- SET DEBUG FILE TO "./corresp_rev_pagotdc_cargocta.err";
            --- TRACE ON;
    ...
    ELSE
		...
		-- REVERSO DE LA PARTE DE CREDITO
		SELECT COUNT(*)
		INTO vreversado
        ...
        IF vreversado > 0 THEN
            LET cod_ret1 = '000';
        ELSE
            EXECUTE PROCEDURE bdicred:reversion('001', psucursal, pusuario, pfolio, 'A')
            INTO cod_ret1;
            IF cod_ret1 <> '000' THEN
                IF cod_ret1 = '431' THEN
                    LET cod_ret1 = '431';
                END IF;
