CREATE PROCEDURE "informix".sp_arch_cartera_pyr()
RETURNING CHAR(6);
--Creado por: maria elizabeth anzures ibarguen
--21-03-2012
--Proceso para la generaciÃÂÃÂ³n de archivo cartera reestructura y prestamo personal saldos e intereses
...
--SET DEBUG FILE TO "/informix/miguel/sp_arch_cartera_pyr.out";
--TRACE ON; 
...
--let pfechacorte = '02-21-2016';
	...
	select  nvl(a.num_producto,0),nvl(a.num_credito,0),nvl(a.credito_externo,0),nvl(cta.num_cta,0),nvl(tar.num_tarjeta,0), nvl(a.numcte,0),nvl(a.sucursal,0),nvl(s.ciudad,0),a.fecha_apertura,
			...
			,c.fecha_ult_pago, 
--			(CASE WHEN (a.status_cred IN ('AA','BA') or (a.status_cred in ('E1','E2') AND NVL(b.atr,-1)>= 0)) THEN (sdo_intereses + sdo_no_exig) ELSE 0 END),
			(CASE WHEN (int_tra_no_exig <= 0) THEN (sdo_intereses + sdo_no_exig) ELSE 0 END),
--			(CASE WHEN (a.status_cred NOT IN ('AA','BA') or (a.status_cred not in ('E1','E2') AND NVL(b.atr,-1)> 0)) THEN (sdo_intereses + sdo_no_exig + int_tra_no_exig) ELSE 0 END) --,today--,(b.sdo_exig_int + b.mto_venc_tra_int)
			(CASE WHEN (int_tra_no_exig > 0) THEN (sdo_intereses + sdo_no_exig + int_tra_no_exig) ELSE 0 END)
