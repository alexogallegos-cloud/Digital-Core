CREATE PROCEDURE "informix".sp_adn_reporteinfodisp(pEmpresa CHAR(3), pSolicitud CHAR (20))
...
--TRACE ON;
	...
	END IF;
		...
		IF iMontoMax = 0 THEN
		LET cCodRet  = "000002";		RETURN cCodRet ,NVL(iMontoMax,0),  NVL(iComisionActi,0),  NVL(iComisionDisp,0),  NVL(iComisionDisp2,0),  NVL(iMontoDisp1,0),  NVL(iMontoDisp2,0) , NVL(iMontoMin,0),  NVL(iComDisp,0),  NVL(mCatPromM,0.0),  NVL(mCatPromQ,0.0),  NVL(mCatPromS,0.0),  NVL(cfecha1,'')	,  NVL(cfecha2,'');
		END IF;
