CREATE PROCEDURE "informix".sp_actualiza_premio(pNumCte CHAR(9), pOperador CHAR(8), pSucursal CHAR(4))			 
RETURNING CHAR(5) AS codRet
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/sp_actualiza_premio.out';
		--TRACE ON;
		...
		SELECT numcte, premio, folio, estatus, usuario, sucursal, fecha
		INTO cnumcte, cpremio, cfolio, cestatus, cusuario, csucursal, cfecha
		FROM bdinteg:"informix".si_premios
		...
		IF dbinfo('sqlca.sqlerrd2') = 0 THEN
			...
			SELECT numcte
			INTO cnumcte
			FROM bdinteg:"informix".si_noganadores
			WHERE numcte = pNumCte;
			IF dbinfo('sqlca.sqlerrd2') = 0 THEN
				INSERT INTO bdinteg:si_noganadores(numcte, nombre_cte, usuario, sucursal, nombre_suc, fecha, hora)
				VALUES (pNumcte, nombrecte, pOperador, pSucursal, nombresuc, today, CAST(substr (extend(current, hour to minute), 0, 5) AS CHAR(5)));
			END IF;
		ELSE	
			UPDATE bdinteg:si_premios
			SET estatus = 1, usuario = pOperador, sucursal = pSucursal, fecha = today
			WHERE numcte = pNumCte;
			
			INSERT INTO bdinteg:si_premios_hist(numcte, nombre_cte, premio, folio, estatus, usuario, sucursal, fecha, fecha_hora, nombre_suc)
			VALUES (cnumcte, nombrecte, cpremio, cfolio, cestatus, pOperador, pSucursal, today, current, nombresuc);
...
CREATE PROCEDURE "informix".sp_conspremios(pNumCte CHAR(9))			 
RETURNING 
	CHAR(5) AS codRet,
	CHAR(9) AS numCte,
	DECIMAL(9,2) AS premio,  
	CHAR(20) AS folio,
	CHAR(1) AS cEstatus
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr, iIsamErr
			IF iSqlErr <> 0 THEN
				LET cCodRet = iSqlErr;				
				RETURN cCodRet,cNumCte,cPremio,cFolio,cEstatus;
			END IF;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/sp_conspremios.out';
		--TRACE ON;
		...
		SELECT numcte, premio, folio, estatus 
		INTO cNumCte,cPremio, cFolio, cEstatus
		FROM bdinteg:si_premios 
		WHERE numcte = pNumCte;
		...
		RETURN cCodRet,NVL(cNumCte,''),NVL(cPremio,''),NVL(cFolio,''),NVL(cEstatus,'');
	END;
END PROCEDURE
DOCUMENT
'Codigo de retorno 00001 indica que se ha enviado parametro de entrada invalido', 
'Codigo de retorno 00002 indica que no se encontraron datos en la tabla si_premios ',
'FECHA : 15/Abril/2022',
'BD    : BDINTEG',
'SOLICITO: Abraham Narvaez';
