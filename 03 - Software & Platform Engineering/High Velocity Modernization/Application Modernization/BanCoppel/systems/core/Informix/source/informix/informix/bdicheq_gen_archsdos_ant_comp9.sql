CREATE PROCEDURE "informix".gen_archsdos_ant_comp9()
RETURNING CHAR(5), CHAR(5), CHAR(50), INTEGER;
    ...
    ON EXCEPTION SET sql_err, isam_err, desc_err
        SET DEBUG FILE TO "/resplogifx/conciliachq/gen_archsdos_ant_comp9.err";
        TRACE ON;
        IF sql_err <> 0 THEN
            ...
            LET vsql = 'echo "UPDATE bdinteg:sx_contproc '||
                       ...
                       'AND sistema   = '''||vsistema||''';" > /tmp/horassdosdia9.sql';
            SYSTEM vsql;
            
            LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /tmp/horassdosdia9.sql';
            SYSTEM vstmt;
            RETURN vcodret1, vcodret2, vcodret3, vcontador1;
        END IF;
    END EXCEPTION;
    
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/gen_archsdos_ant_comp9.out";
    --- TRACE ON;
    ...
    IF vexiste = 0 THEN
        LET vsql = 'echo "INSERT INTO bdinteg:sx_contproc VALUES '||
                   ...
                   '(SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas), NULL, NULL);" > /tmp/horassdosdia9.sql';
        SYSTEM vsql;
	...
	SELECT valor
      ...
      FROM sc_param
     WHERE empresa = vempresa
       AND codparam = 'CtaIniActuaSdosComp8';
    ...
    SELECT cuenta, monto_tot--28 registros
      FROM sc_movdia
     WHERE empresa = vempresa
    ...
    IF vfecha_hoy = vpri_hab_mes THEN
        ...
        FOREACH WITH HOLD
            SELECT {+INDEX(sc_maechq idx_sc_maechq )} 
            ...
            -- // INSERTA EN TABLAS DE LA CONCILIACION DE SALDOS E INTERESES
