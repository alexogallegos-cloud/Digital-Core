...
-- Procesa Informacion
   ...
   foreach
      ...
      if vexiste = "1" then
         let vsql = 'echo "unload to paso '||'select * from '||trim(vtabla)||
      ...
      let vsql = "dbschema -q -d bdinvers -t "||trim(vtabla)||
                 " -p all tabla; sed /revoke/d tabla > tabla.sql";
      system vsql;

      -- Elimina la tabla por drop y crea el esquema de la misma.
      let vsql = 'echo "drop table bdinvers:'||TRIM(vtabla)||'" > query.sql';
      system vsql;

      let vsql = "dbaccess bdinvers query.sql ";
      system vsql;

      let vsql = "dbaccess bdinvers tabla 2>/dev/null > /dev/null";
      system vsql;
      ...
      let vsql = "echo " || '"' || "file 'paso' delimiter '|' "||
               vnumcols || "; insert into " ||
               TRIM(vtabla) || ";" || '"' || ' > carga';
      system vsql;

      let vsql = "dbload -d bdinvers -c carga -l er -n 100";
      system vsql;
      ...
      let vsql ='echo "update statistics medium for table bdinvers:'||
   ...
   if vcodret = "000" then
      select fecha_ant into vfecha_ant
         from sv_fechas
         where empresa = pempresa;
      delete from bdicheq:sc_movinver
         where empresa = pempresa and fecha_alta >= pfecha
               and procesado = "N";
      if ptipo_resp = "A" or ptipo_resp = "a" then
         update bdicheq:sx_contproc
            set fecha = vfecha_ant
            where sistema = "03"  and proceso <> "fecha";
