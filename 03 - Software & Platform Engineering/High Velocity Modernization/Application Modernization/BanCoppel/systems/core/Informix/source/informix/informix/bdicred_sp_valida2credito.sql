CREATE PROCEDURE "informix".sp_valida2credito (pEmpresa CHAR(3), pCliente CHAR(20), pNumSol CHAR(20) , pValidacion SMALLINT)

RETURNING CHAR(5),    -- Codigo de Retorno
           SMALLINT,  -- 0 flujo normal, 1 segundo credito
		   SMALLINT;  -- Valor ICC
...
BEGIN
    ON EXCEPTION SET iSqlErr, iIsamErr,cErrorInfo
       IF iSqlErr != 0 THEN
          LET cCodRet = iSqlErr;          
          RETURN cCodRet,1,NVL(iValorICC,0);		 
       END IF;
    END EXCEPTION;

	--SET DEBUG FILE TO "/informix/jesus/soltar/sp_valida2Credito.out";
	--TRACE ON;
	...
	IF pValidacion IN(0,2) THEN
		IF (SELECT COUNT(numcte)	
			FROM bdicred:"informix".sd_grupo_cliente 
			WHERE empresa = pEmpresa
			AND numcte  = pcliente) > 0 THEN

			--SELECT  MAX( fecha_insert ) INTO dtFecha
			--FROM bdicred: sd_ctes_behavior;				
				
				IF (SELECT COUNT(numcte)	
					FROM bdicred:"informix".sd_maecred a, sd_ctes_behavior b
					WHERE a.empresa = pEmpresa
					AND a.num_credito = b.num_credito
					AND a.numcte  = pcliente
					AND fecha_insert = fecha_insert
					AND UPPER (segmento) in ('CLEAN_THICK','CLEAN_THIN'))> 0 or 
					(SELECT COUNT(numcte)	
					FROM bdicred:"informix".sd_maecredcrd a, sd_ctes_behavior b
					WHERE a.empresa = pEmpresa
					AND a.num_credito = b.num_credito
					AND a.numcte  = pcliente
					AND fecha_insert = fecha_insert
					AND UPPER (segmento) in ('CLEAN_THICK','CLEAN_THIN')) > 0
					THEN				
						...
						WHERE empresa = pEmpresa
						AND numcte  = pcliente
						AND  tipo_tel=2 AND status_tel='A') > 0 THEN
			
							IF pValidacion =0 THEN					
								RETURN cCodRet,1,NVL(iValorICC,0);								
							END IF;
							
							IF pValidacion = 2 THEN --validación de telefonos
									...
									and fecha = (SELECT max(fecha) from bdinteg:"informix".si_bitsmstels c 
										...
										--se debe actualizar la solicitud en el nuevo campo de la ss_resum_scor_fin de que ya se validaron los telefonos
									...
									END IF;						
							END IF;					
						END IF;	--validacion telefonos actual	
			    END IF;	
		 END IF;	
	ELIF pValidacion = 1 THEN --validar ICC de buro
	
	 
		SELECT sc01::INTEGER 
			INTO iValorICC
		FROM bdiburo:"informix".br_sc a
		WHERE a.rowid = (SELECT MAX(b.rowid) FROM bdiburo:"informix".br_sc b WHERE b.num_cliente= pcliente AND sc00 = "004") 
		AND num_cliente=pcliente
		AND sc00 = "004";
		
		IF  NVL(iValorICC,0) > 0 THEN					
			RETURN cCodRet,1,NVL(iValorICC,0);
		ELSE
			RETURN cCodRet,0,NVL(iValorICC,0);
		END IF;			
...
END PROCEDURE
DOCUMENT    
'DESCRIPCION: Procedimiento para  validacion de segundo producto RQM 09 392', 
