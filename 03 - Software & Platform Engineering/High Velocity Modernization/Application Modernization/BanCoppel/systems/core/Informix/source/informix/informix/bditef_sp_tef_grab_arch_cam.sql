CREATE PROCEDURE "informix".sp_tef_grab_arch_cam(pEjecutivo CHAR(8),pNumRegistro INTEGER,pImporte DECIMAL(14,2),pCodOper INTEGER,pClave CHAR(10),pNumArchLimt SMALLINT,pModalidad INTEGER)
...
BEGIN
	...
	--SET DEBUG FILE TO "/respaldosbd/Guadalupe/sp_tef_grab_arch_cam.out";
	--TRACE ON;
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	--Comienza codigo
	IF NVL(pEjecutivo,' ') =' ' OR  NVL(pNumRegistro,0) = 0 OR NVL(pImporte,0.00) = 0.00 OR NVL(pClave,' ') =' ' OR NVL(pCodOper,0) = 0 OR NVL(pModalidad,0) NOT IN(1,2) THEN
		LET cCodRet = '000001';
		LET cMensajeRet = 'TODOS LOS PARAMETROS SON NECESESARIOS';
		RETURN cCodRet,cMensajeRet;
	END IF;
	
	IF pModalidad = 1 THEN
	
		SELECT MAX(numero_archivo)
		INTO iNumArchivo
		FROM "informix".cce_archivos_camara
		WHERE codigo_operacion = pCodOper
		AND fecha::DATE = TODAY
		AND clave_archivo = pClave;
		
		IF NVL(iNumArchivo,0)> 0 THEN
			LET iNumArchivo = iNumArchivo +1;
		ELSE 
			LET iNumArchivo = 1;			
		END IF;
		
		INSERT INTO "informix".cce_archivos_camara
		VALUES(pEjecutivo,iNumArchivo,pNumRegistro,pImporte,pCodOper,CURRENT,pClave);		
	...
	IF pModalidad = 2 THEN
		IF NVL(pNumArchLimt,0) = 0 THEN
			LET cCodRet = '000002';
			LET cMensajeRet = 'PARAMETRO INVALIDO';
			RETURN cCodRet,cMensajeRet;			
		END IF;
		
		SELECT MIN(numero_archivo)
		INTO iNumArchivo
		FROM "informix".cce_archivos_camara
		WHERE codigo_operacion = pCodOper
		AND fecha::DATE = TODAY
		...
		IF NVL(iNumArchivo,0) = 0 THEN
			LET iNumArchivo = pNumArchLimt;						
		ELIF NVL(iNumArchivo,0) > 1 THEN 
			LET iNumArchivo = iNumArchivo - 1;
		ELSE 			
			LET cCodRet = '000002';
			LET cMensajeRet = 'YA NO ES POSIBLE GENERAR OTRO ARCHIVO PARA ESTE DIA';
			RETURN cCodRet,cMensajeRet;			
		END IF;
...
END PROCEDURE
DOCUMENT
'Realiza: Se registran datos en la tabla cce_archivos_camara',
'AUTOR : Jose Raul Pacheco Ortiz',
'FECHA : 30/MARZO/2015',
'BD    : BDITEF';

CREATE PROCEDURE "informix".sp_cce_controlusuariosaut( pOperacion INTEGER,
                                                       pNumEjecut INTEGER,
...
RETURNING CHAR(5) AS CodigoRetorno;
	...
	-- Valida que el parametro pOperación solamente tome valor de 1 o 2. 
	IF pOperacion NOT IN(1,2) THEN
	    LET cCodRet = "00001";
		RETURN cCodRet;
    END IF;
		
	-- Valida que al menos traiga valor en un parametro.
	IF NVL(pNumEjecut,"") = "" OR NVL(pNombreEjecut,"") = "" OR NVL(pFecha,"") = "" THEN  
        LET cCodRet = "00001"; 
        RETURN cCodRet;
	END IF;
	
	-- Realiza validación de que exista ejecutivo y guarda nuevo ejecutivo
	IF pOperacion = 1 THEN 
		-- Verifica que exista ejecutivo 
		IF EXISTS (SELECT ejecutivo FROM "informix".cce_usuarios_revision WHERE ejecutivo = pNumEjecut 
					AND nombre = TRIM(pNombreEjecut)) THEN 
			LET cCodRet = "00002";
		ELSE
			INSERT INTO "informix".cce_usuarios_revision (ejecutivo, nombre, fecha_alta)
			VALUES (pNumEjecut, TRIM(pNombreEjecut), pFecha);
			LET cCodRet = "00000";
		END IF;
	END IF;
	
	-- Realiza validación y eliminación de ejecutivo
	IF pOperacion = 2 THEN
		-- Verifica si no existe ejecutivo
	    IF NOT EXISTS (SELECT ejecutivo FROM bditef:"informix".cce_usuarios_revision WHERE ejecutivo = pNumEjecut 
						AND nombre = TRIM(pNombreEjecut)) THEN
			LET cCodRet = "00003";
		ELSE		
			DELETE FROM "informix".cce_usuarios_revision WHERE ejecutivo = pNumEjecut AND nombre = TRIM(pNombreEjecut);
		END IF;
	END IF;
...
DOCUMENT
'AUTOR: Irma Ureta',
'FECHA CREACION: 29 de Diciembre del 2014',
'DESCRIPCION: Guarda y elimina ejecutivo.',
'VERSION: 20141221.1217',
'BD: BDITEF';

CREATE PROCEDURE "informix".sp_firma_ejec(pEjecutivo CHAR(8),pNombre CHAR(50),pFecha DATE,pTipoConsulta INTEGER)

--Variables que se retornan.
RETURNING
CHAR(6) AS cCodRet,
CHAR(8) AS NumEjecutivo,
CHAR(50) AS NobreEjec,
CHAR(50) AS EjecutivoElaboro,
CHAR(50) AS EjecutivoReviso,
CHAR(50) AS EjecutivoVoBo,
BLOB AS imagenFirma;
--Variables necesarias, declaramos.

DEFINE cCodRet 	CHAR(6);
DEFINE iSqlErr 	INTEGER;
DEFINE bImgFirma BLOB;
DEFINE cNombreEjec CHAR(50);
DEFINE cEjecEla		CHAR(50);
DEFINE cEjecRevi	CHAR(50);
DEFINE cEjecVoBo	CHAR(50);
DEFINE cNumEjec   CHAR(8);
...
LET cNumEjec = "";
	...
	--SET DEBUG FILE TO "/home/sysifx/Raul/sp_firma_ejec.txt";
	--TRACE ON;
--iniciar el cuerpo del SP
BEGIN
	ON EXCEPTION SET iSqlErr -- por si ocurre un error en el transcurso del SP de informix
		IF iSqlErr <> 0 THEN
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cNumEjec,cNombreEjec,cEjecEla,cEjecRevi,cEjecVoBo,bImgFirma;
		END IF;
	END EXCEPTION;
	
	IF  NVL(pTipoConsulta,0) NOT IN (1,2,3,4) THEN
			LET cCodRet = '000001';
		RETURN cCodRet,cNumEjec,cNombreEjec,cEjecEla,cEjecRevi,cEjecVoBo,bImgFirma;
	END IF; 	
	...
	IF pTipoConsulta = 2 THEN
		FOREACH
			...
			INTO cNumEjec,cNombreEjec
			FROM "informix".cce_usuarios_cedula_contable
			ORDER BY ejecutivo
			IF TRIM(NVL(cNombreEjec,""))= "" THEN
				LET cCodRet = '000002';
				RETURN cCodRet,cNumEjec,cNombreEjec,cEjecEla,cEjecRevi,cEjecVoBo,bImgFirma;
			END IF;
			RETURN cCodRet,cNumEjec,cNombreEjec,cEjecEla,cEjecRevi,cEjecVoBo,bImgFirma WITH RESUME;
		END FOREACH;
	END IF;
	IF pTipoConsulta = 3 THEN
		IF NVL(pFecha,'1900-01-01') <> '1900-01-01' THEN
		
			SELECT FIRST 1 ejecutivo_elaboro,ejecutivo_reviso,ejecutivo_vobo
			INTO cEjecEla,cEjecRevi,cEjecVoBo
			FROM "informix".cce_cedulacontable 
			WHERE fecha_elaboracion::DATE = pFecha;
			
		END IF;
		RETURN cCodRet,cNumEjec,cNombreEjec,cEjecEla,cEjecRevi,cEjecVoBo,bImgFirma;
	END IF;
	IF pTipoConsulta = 4 THEN
		IF  NVL(pNombre,'') <> '' THEN
			IF Exists(SELECT nombre FROM "informix".cce_usuarios_cedula_contable WHERE nombre = pNombre) THEN
				SELECT firma_img
				INTO bImgFirma
				FROM "informix".cce_usuarios_cedula_contable
...
DOCUMENT
'DESCRIPCION:Regresa la imagen de la firma de los ejecutivos, de la tabla cce_usuarios_cedula_contable, para llenar el comboBox.',
'Para la Pantalla Firmas del RQM.',
'AUTOR : Christian Rodríguez Guzmán',
'FECHA : 08/04/2015',
'Ver.  : 20150408.1648',
'BD    : BDITEF';

CREATE PROCEDURE "informix".sp_insert_cedula( pEjecElavoro VARCHAR(50), pEjecReviso VARCHAR(50), pEjecVobo VARCHAR(50), pNartCuent VARCHAR(50), pNartImporte MONEY)
...
BEGIN
	ON EXCEPTION SET iSqlErr -- por si ocurre un error en el transcurso del SP de informix
		...
		IF iSqlErr <> 0 THEN
			LET cCodRet = iSqlErr;
			RETURN cCodRet; --regresa estos valores.
		END IF;

	END EXCEPTION;
	
	IF NVL(pEjecElavoro,"") = "" OR NVL(pEjecReviso,"") = "" OR NVL(pEjecVobo,"") = "" OR NVL(pNartCuent,"") = "" THEN
		LET cCodRet = '000001';
		RETURN cCodRet;
		
	ELSE 
		
		INSERT INTO cce_cedulacontable(ejecutivo_elaboro, ejecutivo_reviso, ejecutivo_vobo, narrativa_dcta11020101, narrativa_importe, fecha_elaboracion)
		VALUES (pEjecElavoro, pEjecReviso, pEjecVobo, pNartCuent, NVL(pNartImporte,0.00), CURRENT);
		RETURN cCodRet;
...
END PROCEDURE  -- Create procedure
DOCUMENT
'DESCRIPCION:Regresa los campos que corresponden al día actual para la pagina 24 y 25.',
'AUTOR : Christian Rodríguez Guzmán',
'FECHA : 08/04/2015',
'Ver.  : 20150408.1234',
'BD    : BDITEF';

CREATE PROCEDURE "informix".sp_tef_act_rep_sicam(pEjecutivo CHAR(8),
...
BEGIN
	...
	ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
	   IF iSqlErr != 0 THEN
		  LET cCodRet = iSqlErr;
		  LET cMensajeRet = cErrorInfo;
		 	RETURN TRIM(cCodRet),TRIM(cMensajeRet),mImportCheqPre,mImportCheqRec,mImportCheqSald,mImportDomPres,mImportDomRec,mImportDomSald,mImportTefPres,mImportTefRec,mImportTefSald,mImportTotalPres,mImportTotalRec,mImportTotalSald;
	   END IF;
	END EXCEPTION;

	-- SET DEBUG FILE TO "/home/sysifx/Raul/sp_tef_act_rep_sicam.out";
	-- TRACE ON;
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	
	IF pModalidad NOT IN (1,2,3,4,5) THEN
	...
	--Se obtiene fecha de hoy.
	SELECT fecha_hoy 
	INTO dFechaHoy 
	FROM bdinteg:"informix".si_fechas; 
	
	IF pModalidad = 1 THEN --Modalidad para validar que no exista cedula generada en la tabla cce_cedulacontable.
		IF EXISTS(SELECT fecha_elaboracion FROM "informix".cce_cedulacontable WHERE  CAST(NVL(fecha_elaboracion,DATE(1)) AS DATE)  = dFechaHoy) THEN
			
			LET cCodRet ='000002';			LET cMensajeRet = 'La cédula contable del dia ya fue generada, no es posible capturar información';
		ELSE
			
		END IF;
		RETURN TRIM(cCodRet),TRIM(cMensajeRet),mImportCheqPre,mImportCheqRec,mImportCheqSald,mImportDomPres,mImportDomRec,mImportDomSald,mImportTefPres,mImportTefRec,mImportTefSald,mImportTotalPres,mImportTotalRec,mImportTotalSald;
		
	ELIF pModalidad = 2 THEN --Modalidad para validar si ya fue generada la cedula contable para el dia de hoy, si existe obtiene informacion de la misma. de lo contrario se retorna codigo de error informandolo.
		IF EXISTS(SELECT fecha FROM  "informix".cce_cedulacontable_sicam WHERE  CAST(NVL(fecha,DATE(1)) AS DATE)  = dFechaHoy) THEN
			...
			FROM "informix".cce_cedulacontable_sicam 
			WHERE CAST(fecha AS DATE)  = dFechaHoy;
			
			RETURN TRIM(cCodRet),TRIM(cMensajeRet),mImportCheqPre,mImportCheqRec,mImportCheqSald,mImportDomPres,mImportDomRec,mImportDomSald,mImportTefPres,mImportTefRec,mImportTefSald,mImportTotalPres,mImportTotalRec,mImportTotalSald;
		
		ELSE 			
			
			LET cCodRet ='000003';			LET cMensajeRet = 'No existe informacion de la cedula contable para el dia de hoy';
			RETURN TRIM(cCodRet),TRIM(cMensajeRet),mImportCheqPre,mImportCheqRec,mImportCheqSald,mImportDomPres,mImportDomRec,mImportDomSald,mImportTefPres,mImportTefRec,mImportTefSald,mImportTotalPres,mImportTotalRec,mImportTotalSald;
			
		END IF;
		
	ELIF pModalidad = 3  THEN --Modalidad para validar si ya existe cedula contable para el dia de hoy y en base a la respuesta realizar una accion.
		IF EXISTS(SELECT fecha FROM "informix".cce_cedulacontable_sicam WHERE  CAST(NVL(fecha,DATE(1)) AS DATE)  = dFechaHoy)THEN
			
			LET cCodRet ='000004';
			LET cMensajeRet = 'Ya existe información capturada del dia actual ¿Desea actualizar la información?';
			RETURN TRIM(cCodRet),TRIM(cMensajeRet),mImportCheqPre,mImportCheqRec,mImportCheqSald,mImportDomPres,mImportDomRec,mImportDomSald,mImportTefPres,mImportTefRec,mImportTefSald,mImportTotalPres,mImportTotalRec,mImportTotalSald;					
			
		END IF;
	
	ELIF pModalidad = 4  THEN --Modalidad para Actualizar la informacion de la cedula contable. 
		UPDATE "informix".cce_cedulacontable_sicam
		SET ejecutivo = NVL(pEjecutivo,0.00),importechequespresentado = NVL(pImpCheqPres,0.00),importechequesrecibidos = NVL(pImpCheqRec,0.00),importechequessaldo = NVL(pImpCheqSald,0.00),importedomipresentado = NVL(pImpDomPres,0.00),importedomirecibido = NVL(pImpDomRec,0.00),importedomisaldo= NVL(pImpDomSald,0.00),importetefpresentado = NVL(pImpTefPres,0.00),
		importetefrecibido = NVL(pImpTefRec,0.00),importetefsaldo =NVL(pImpTefSald,0.00),total_presentado = NVL(pTotalPres,0.00),total_recibido = NVL(pTotalRec,0.00),total_saldo =NVL(pTotalSald,0.00), fecha = CURRENT 
		WHERE CAST(fecha AS DATE)  = dFechaHoy;
				
		RETURN TRIM(cCodRet),TRIM(cMensajeRet),mImportCheqPre,mImportCheqRec,mImportCheqSald,mImportDomPres,mImportDomRec,mImportDomSald,mImportTefPres,mImportTefRec,mImportTefSald,mImportTotalPres,mImportTotalRec,mImportTotalSald;
			
	ELIF pModalidad = 5  THEN --Modalidad para Generar la informacion de la cedula contable.
		--Se genera cedula contable.
		INSERT INTO "informix".cce_cedulacontable_sicam(ejecutivo,importechequespresentado ,importechequesrecibidos,importechequessaldo,importedomipresentado,importedomirecibido,importedomisaldo,importetefpresentado,importetefrecibido,importetefsaldo,total_presentado,total_recibido,total_saldo,fecha)
			 VALUES (NVL(pEjecutivo,""),NVL(pImpCheqPres,0.00),NVL(pImpCheqRec,0.00),NVL(pImpCheqSald,0.00),NVL(pImpDomPres,0.00),NVL(pImpDomRec,0.00),NVL(pImpDomSald,0.00),NVL(pImpTefPres,0.00),NVL(pImpTefRec,0.00),NVL(pImpTefSald,0.00),NVL(pTotalPres,0.00),NVL(pTotalRec,0.00),NVL(pTotalSald,0.00),CURRENT);
...
END PROCEDURE
DOCUMENT
'Realiza: El procedimiento tiene 4 funcionamientos validacion de fecha la  cual es necesaria para saber si se mostrara la pantalla en la que se ejecute, consulta de datos a la tabla "informix".cce_cedulacontable_sicam, Insert Y update segun sea el caso de la fecha en latabla "informix".cce_cedulacontable_sicam ',
'AUTOR : Jose Raul Pacheco Ortiz',
'FECHA : 30/MARZO/2015',
'BD    : BDITEF';

CREATE PROCEDURE "informix".sp_tef_obt_arch_cam_recibyprest40y41(pCodOper INTEGER, p_clv_archivo CHAR(8),pModalidad INTEGER)
	...
	--SET DEBUG FILE TO "/home/sysifx/Raul/sp_tef_obt_arch_cam_recibyprest40y41.txt";
	--TRACE ON;
	...
	BEGIN
		...
		IF NVL(pModalidad,0) NOT IN(1,2,3) THEN 
			LET cCodRet = '000001';
			RETURN cCodRet, 0, 0, 0, 0;
		END IF;
		LET dtFechaHabilAnt = dtFechaHoy;
		
		EXECUTE PROCEDURE bditef:"informix".cal_habil_ant(dtFechaHoy)
		INTO cCodRetDevuelto, dtFechaDevuelta;
		LET dtFechaHabilAnt = dtFechaDevuelta;

		If dtFechaDevuelta >= dtFechaHoy or i > j THEN
			LET cCodRet = '000003'; --No se pudo calcular fecha Habila Anterior
			RETURN cCodRet, intNumArchivo, intCodOperacion, intNumRegistros, monImport;
		END IF;
		IF pCodOper = 40 THEN 
			LET dtFechaAux = dtFechaHabilAnt ;
		ELSE
			LET dtFechaAux = dtFechaHoy;
		END IF;
	
		IF pModalidad = 1 THEN --CODIGO 40 Presentados CLAVE CPPC40 cce_archivos_camara, Codigo 41 Resividos  Clave CDDPC41--14
			...
			IF NVL(pCodOper,0) = 40 THEN 
				LET chrFecha = (SUBSTRING(dtFechaAux from 4 for 2) || '' ||
								SUBSTRING(dtFechaAux from 1 for 2) || '' ||
								SUBSTRING(dtFechaAux from 7 for 4) );
				FOREACH
					SELECT folio, cod_operacion, num_operaciones, importe_total
					INTO intNumArchivo, intCodOperacion, intNumRegistros, monImport	
					FROM  cce_gransumario 
		...
		END IF;	
		IF pModalidad = 2 THEN-- codigo 41 clave CDDPC41 face recepcion codigo 40
			IF NVL(p_clv_archivo ,'') = '' OR NVL(pCodOper,0) NOT IN (41,40)THEN
				LET cCodRet = '000001';
				RETURN cCodRet, 0, 0, 0, 0;
			END IF;
		...
		IF pModalidad = 3 THEN  -- CODIGO 40 Propios cce_propios_det
			FOREACH
				SELECT (SUBSTRING(nombrearchivo FROM  (length(nombrearchivo)-1))::integer),COD_OPERACION,COUNT(nombrearchivo), SUM(c_importe)
				INTO intNumArchivo, intCodOperacion, intNumRegistros, monImport
				FROM bditef:"informix".cce_propios_det
				WHERE  fecha_entrada = dtFechaAux
				GROUP  BY  1,2
				ORDER  BY  1 DESC
				RETURN cCodRet, intNumArchivo, intCodOperacion, intNumRegistros, monImport WITH resume;
				LET intSumRegistros = intSumRegistros + intNumRegistros;
...
END PROCEDURE  -- Create procedure
DOCUMENT
'DESCRIPCION:Regresa los campos que corresponden al día habil anterior con la clavearchivo CPPC40, el primer Select',
'habla de la pagina 14, el segundo select habla de la pagina 15. Para utilizar el segundo insert no es necesario mandar valor a p_clv_archivo.',
'AUTOR : Christian Rodríguez Guzmán',
'FECHA : 26/03/2015',
'Ver.  : 20150326.1155',
'BD    : BDITEF';

CREATE PROCEDURE "informix".sp_tef_rep_lib_sif()
...
DEFINE decImporte2101 MONEY(16,2);
DEFINE intNumRegistrosTDC INTEGER;
DEFINE decImporteTDC MONEY(16,2);

DEFINE intSumRegistros INTEGER;
DEFINE decSumaImporte MONEY(16,2);
DEFINE iTotalRegistros INTEGER;
DEFINE dTotalImport MONEY(16,2);
DEFINE intSumRegistros2 INTEGER;
DEFINE decSumaImporte2 MONEY(16,2);
DEFINE intSumRegistros1 INTEGER;
DEFINE decSumaImporte1 MONEY(16,2);
--inicializaar variables
LET  cCodRet =	'000000';
LET iSqlErr =	0;
LET intNumRegistros2101 = 0;
LET decImporte2101 = 0;
LET intNumRegistrosTDC = 0;
LET decImporteTDC = 0;
LET iTotalRegistros = 0;
LET dTotalImport = 0;

LET intSumRegistros1 = 0;
LET decSumaImporte1 = 0;
LET intSumRegistros2 = 0;
LET decSumaImporte2 = 0;

--SET DEBUG FILE TO "/home/sysifx/Raul/traccedula.txt";
--TRACE ON;
--iniciar el cuerpo del SP
BEGIN
	ON EXCEPTION SET iSqlErr -- por si ocurre un error en el transcurso del SP de informix
		...
		IF iSqlErr <> 0 THEN
			LET cCodRet = iSqlErr;
			RETURN cCodRet, 0, 0, 0; --regresa estos valores.
		END IF;
	...
	SET ISOLATION TO DIRTY READ;
		...
		FOREACH
			...
			INTO intNumRegistros2101, decImporte2101
			FROM bdicheq:"informix".sc_movdia
			WHERE transacc = 3246 AND fech_alt = (today)
			RETURN cCodRet, 'Liberación 2101', NVL(intNumRegistros2101,0), NVL(decImporte2101,0) WITH RESUME;
			LET intSumRegistros1 = intSumRegistros1 + intNumRegistros2101;
			LET decSumaImporte1 = decSumaImporte1 + decImporte2101;
		END FOREACH;
		FOREACH
			SELECT COUNT(monto), NVL(SUM(monto),0)
			INTO intNumRegistrosTDC, decImporteTDC
			FROM bdicred:"informix".sd_movdia
			WHERE fecha_mov = (today) AND transacc_suc = 6246 AND codigo_fun = 336 AND codigo_ref = 1
			RETURN cCodRet, 'Liberación TDC', NVL(intNumRegistrosTDC,0), NVL(decImporteTDC,0) WITH RESUME;
			LET intSumRegistros2 = intSumRegistros2 + intNumRegistrosTDC;
			LET decSumaImporte2 = decSumaImporte2 + decImporteTDC;
		END FOREACH;
		LET iTotalRegistros =intSumRegistros1 + intSumRegistros2;
		LET dTotalImport = decSumaImporte1 + decSumaImporte2;
			RETURN cCodRet, 'Total Liberado', NVL(iTotalRegistros,0), NVL(dTotalImport,0);
...
CREATE PROCEDURE "informix".sp_tef_domi_genrep30y60(pCodOperacion CHAR(2), ptipoarchivo CHAR(1),pModalidad SMALLINT)
--Variables que se retornan.
RETURNING
CHAR(6) AS cCodRet,
INTEGER AS iCodigo,
INTEGER AS iNumRegistos,
MONEY(16,2) AS Importe;
--Variables necesarias, declaramos.
DEFINE cCodRet 	CHAR(6);
DEFINE iSqlErr 	INTEGER;
DEFINE iCodOper CHAR(2);
DEFINE iSumRegistros INTEGER;
...
--SET DEBUG FILE TO "/home/sysifx/Raul/30606131.sql";
--TRACE ON;
...
BEGIN
	...
	SELECT SUBSTRING (fecha_hoy FROM 7 FOR 4)||SUBSTRING (fecha_hoy FROM 1 FOR 2)|| SUBSTRING(fecha_hoy FROM 4 FOR 2), fecha_hoy
	INTO cFechaPres,dtFechaHoy
	FROM bdinteg:"informix".si_fechas;
	
	IF NVL(pCodOperacion,"") = ""  OR NVL(ptipoarchivo,"") = ""  OR NVL(pModalidad,0) NOT IN (1,2,3) THEN 
		LET cCodRet = '000001';
    ...
    If dtFechaDevuelta >= dtFechaHoy or i > j THEN
		LET cCodRet = '000003'; --No se pudo calcular fecha Habila Anterior
		RETURN cCodRet, iCodOper, NVL(iSumRegistros,0), NVL(mImporte,0);
    END IF;
	IF pCodOperacion = 30 OR pCodOperacion = 60 THEN
			LET dtFechaAux = dtFechaHabilAnt;
		ELSE
			LET dtFechaAux = dtFechaHabilAnt;
		END IF;
	IF pModalidad = 1 THEN ----Resive como parametros (30,E),(30,S),(31,E),(31,S)
		
		SELECT det.cod_operacion, COUNT(det.cod_operacion), SUM(det.importe::DECIMAL(16,2))/100
		INTO iCodOper, iSumRegistros, mImporte
		...
		WHERE cod_operacion = pCodOperacion
		AND det.fecha_insert = dtFechaAux
		AND SUBSTR(det.nombre_arch,1,1) = ptipoarchivo GROUP BY 1;            
		RETURN cCodRet, iCodOper, NVL(iSumRegistros,0), NVL(mImporte,0);                            
	END IF;         
	
	IF pModalidad = 2 THEN --Resive como parametro (60,E),(60,S)
	
		LET cFechaPresHabAnt =SUBSTRING (dtFechaAux FROM 7 FOR 4)||SUBSTRING (dtFechaAux FROM 1 FOR 2)|| SUBSTRING(dtFechaAux FROM 4 FOR 2);
		SELECT a.cod_operacion, COUNT(a.nombre_arch), SUM(a.importe::DECIMAL(16,2))/100
		INTO iCodOper, iSumRegistros, mImporte
		...
		WHERE  substr(a.nombre_arch,1,1) = ptipoarchivo
	...
	IF pModalidad = 3 THEN		--Codigo 61 E y S
		LET cFechaPresHabAnt =SUBSTRING (dtFechaAux FROM 7 FOR 4)||SUBSTRING (dtFechaAux FROM 1 FOR 2)|| SUBSTRING(dtFechaAux FROM 4 FOR 2);
		SELECT cod_operacion, COUNT(cod_operacion), SUM(importe::DECIMAL(16,2))/100
		INTO iCodOper, iSumRegistros, mImporte 
		FROM bditef:"informix".tef_cce_detalle 
		WHERE cod_operacion =  pCodOperacion
		AND substr(nombre_arch,1,1) = ptipoarchivo
		AND cve_status = '02'
...
END PROCEDURE  -- Create procedure                                                              
DOCUMENT                                                                                        		
'DESCRIPCION:Regresa el CodigoOPeracion, Numero de Registro y ',                                
'el Importe de la Fase Precentación TEF del codigo 30 presentado, de la pagina 16 del RQM.',       
'AUTOR : Christian Rodríguez Guzmán',                                                           
'FECHA : 14/04/2015',                                                                           
'Ver.  : 20150414.1705',
'BD    : BDITEF';
