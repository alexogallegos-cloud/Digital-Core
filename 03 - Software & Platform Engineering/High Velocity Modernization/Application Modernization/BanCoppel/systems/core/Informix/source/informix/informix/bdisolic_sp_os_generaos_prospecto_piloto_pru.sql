CREATE PROCEDURE "informix".sp_os_generaos_prospecto_piloto_pru(pEmpresa CHAR(3))
...
DEFINE scod_ret6        VARCHAR(6); --Para recuperar el cÃ?Â³digo de error de sp_actualiza_status_sol
...
LET cCodRetActEstProspTit = '000000';
		...
		Set debug file to "/respaldos/erfr/trace_sp_os_generaos_prospecto_piloto.out";
		trace on;
    ...
    BEGIN
        ON EXCEPTION SET SQL_ERR, ISAM_ERR, ERROR_INFO
           ...
           IF SQL_ERR = -1213 THEN
               LET P_MENSAJE = 'Error conversion carÃ?Â¡cter a nÃ?Âºmerico, probable num de casa cliente incorrecto.';
           END IF;
	       BEGIN WORK;
   ...
   SELECT numcte_pros, fecha_insert, sucursal, TRIM(NVL(nombre1,'')), TRIM(NVL(nombre2,'')), TRIM(NVL(apell_paterno,'')), TRIM(NVL(apell_materno,'')),
     ...
     where tipo_cliente = 3 --and numcte_pros = 'P000001526'
       -- and estado_os =0  SE QUITA FILTRO POR PETICION DE CLIENTE 06/03/2015  -  JOSUE ZAZUETA
	   AND status_numcte_pros in ('EE','CE')  ---SE AGREGA FILTRO PARA OBTENER SOLAMENTE CLIENTES PROSPECTOS EN "ESTUDIO DE SUPERVISION" PARA SU PROCESAMIENTO DE GENERACION DE ORDEN DE SUPERVISION.

	  --LET dtFechaHora = CAST (dtFechaHora AS DATE);
	...
	IF bdisolic:EsNumeroCasa(vauxcasa) = 0 THEN
	  ...
	  IF nvl(V_ciudadCoppel,0) =0  or nvl(V_ciudadCoppel,0) = 0
				...
				EXECUTE PROCEDURE bdiprospectos:"informix".sp_ctepr_actualizastatus("sistema",V_NumSolicitud,"CE", "", "")
				INTO cCodRetActEst;
				...
				EXECUTE PROCEDURE "informix".sp_os_actualizastatusprospectotitular(pEmpresa,V_NumSolicitud)
...
--		  begin work;
				...
				EXECUTE PROCEDURE bdiprospectos:"informix".sp_ctepr_actualizastatus("sistema",V_NumSolicitud,"OS", "", "")
				INTO cCodRetActEst;
			...
			IF TRIM(V_NombreZonaCoppel) = "EL RASCÃ?Â?'N (LA LOMA)" THEN
			   LET V_NombreZonaCoppel = "EL RASCON (LA LOMA)";
			END IF;
			
			IF TRIM(V_nombrezonatrabajocoppel) = "EL RASCÃ?Â?'N (LA LOMA)" THEN
			...
			END IF;
				...
				VALUES(
					iMax, pEmpresa, V_NumSolicitud, dtFechaSolicitud, V_FechaImpresion, V_FechaRespuesta, V_EstatusOs, V_Observacion1,
				...
				IF cStatusCteProsFinal = "OS" THEN
					...
					WHERE num_solicitud = V_NumSolicitud AND status = 'S';
