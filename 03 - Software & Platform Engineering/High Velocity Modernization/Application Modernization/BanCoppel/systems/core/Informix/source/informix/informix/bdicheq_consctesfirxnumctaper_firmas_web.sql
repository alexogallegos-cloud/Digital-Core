CREATE PROCEDURE "informix".consctesfirxnumctaper_firmas_web(pEmpresa CHAR(3), pNumeroCuenta CHAR(20), pNumeroCliente CHAR(20))
	-- DATOS A REGRESAR --
	RETURNING
	...
	CHAR(40),     -- Producto de la cuenta
	CHAR(2),     -- Parentesco
	char(20),    -- Descripcion Parentesco
	...
	DEFINE vProductoCuenta CHAR(40);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			IF iSqlErr <> 0 THEN
				...
				RETURN vCodRet, vNumCte, vApePat, vApeMat, vNombre1, vNombre2, vRFC, vNumTarj, Vexpiracion, 
						Vprodtarjeta, vLimTar, vStatTjt, vTipoCte, vFechaNac, vProductoCuenta, vParentesco, vDesc, vTipo_firma;
			END IF;
		END EXCEPTION;
		...
		FOREACH
			...
			SELECT DISTINCT si_cte.numcte, 
				...
				sc_fir.secuencia As tipo_cliente, si_pf.fecha_nac, sc_mcq.producto, sc_fir.parentesco, sc_fir.tipo_firma
			INTO
                
				vNumCte, vApePat, vApeMat, vNombre1, vNombre2, vRFC, vTipoCte, vFechaNac, Vprodtarjeta, vParentesco, vTipo_firma
			FROM
            ...
            WHERE parentesco = vParentesco;   
			end if

			-- OBTENER LA TARJETA DEL FIRMANTE --

			SELECT DISTINCT sc_tjt.expiracion, sc_prod.nombre, sc_tjt.num_tarjeta, sc_tjt.limite_aut, sc_tjt.status_tar
			INTO
				Vexpiracion, vProductoCuenta, vNumTarj, vLimTar, vStatTjt
			FROM
				bdicheq:"informix".sc_tarjeta AS sc_tjt,
				bdicheq:"informix".sc_producto AS sc_prod
			WHERE sc_tjt.empresa = pEmpresa 
			  AND sc_tjt.cuenta = pNumeroCuenta
			  AND sc_tjt.numcte = vNumCte
			  AND sc_prod.producto = Vprodtarjeta
          ...
          --  LET vFechaNac= vFechaNac;
           -- LET vFechaNac = YEAR(vFechaNac) || '-' || LPAD ( MONTH(vFechaNac-2), 2, '0') || '-' || LPAD ( DAY (vFechaNac-2), 2, '0');
			...
			RETURN vCodRet, vNumCte, vApePat, vApeMat, vNombre1, vNombre2, vRFC, vNumTarj, Vexpiracion, 
					Vprodtarjeta, vLimTar, vStatTjt, vTipoCte, vFechaNac, vProductoCuenta, vParentesco, vDesc, vTipo_firma WITH RESUME;
...
DOCUMENT
'DESCRIPCION: Se agrega filtro para obtener datos de tajertas unicamente Activas, con parentezcos y tipo de firmas',
'EJECUTADO O LLAMADO POR: AperTP Web',
