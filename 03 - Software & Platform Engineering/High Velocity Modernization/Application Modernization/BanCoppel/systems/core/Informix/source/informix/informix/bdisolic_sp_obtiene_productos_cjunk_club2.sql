CREATE PROCEDURE "informix".sp_obtiene_productos_cjunk_club2(	pEmpresa 		CHAR(3),	-- Empresa
	...
	--				no se permita ofertar cuentas de captacion, unicamente inversion creciente y pagare
	...
	--Fecha: 15/01/2019
	--Modificacion: Se modifica para validar que si el solicitante tiene producto 1300 con apertura de dos anios mas para poder ofrecer el producto 8500
	--BD: bdisolic"
	-------------------------------------------------------------------------------------
		...
		IF cDigiDom = '1' THEN
			FOREACH
				...
				AND s.num_producto NOT IN (	SELECT num_producto
											...
											WHERE empresa= pEmpresa
											AND numcte = pNumcte
											AND status_solicitud IN("EA","EE","AT","AP","CC","OA","OS","BC","ST","CE","LC","MC","EC","PA","IN"))--JMAH RQM 09279 / RQM 18 023 --IPCBjulio2021// Se integra validacion de status IN
			...
			END FOREACH
		ELSE
			-- Obtiene los productos de captacion con los que actualmente cuenta el cliente, se modifica para que se inserte la fecha de alta de la cuenta de captaciOon
			FOREACH
...
-- Validacion ife/ine  FIN
			...
			FOREACH
				...
				WHERE empresa= pEmpresa
				AND numcte= pNumcte
				AND status_solicitud IN ("BC","CC","ST","EA","EE","OA","OS","CE","AT","AP","RT","LC","MC","EC","PA", "IN") --JMA RQM 09279/RQM 18 023 --IPCBjulio2021// Se integra validacion de status IN
				GROUP BY 1,2,3,4
