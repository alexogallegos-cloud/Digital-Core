CREATE PROCEDURE "informix".sp_confirmasmscte_6dig(pTelCel CHAR(10), pNumCte CHAR(9), pSucursal CHAR(5))
...
BEGIN
	...
	IF (iExist>0) THEN
	--IF EXISTS (SELECT * FROM bdinteg:"informix".si_bitsmstelsms_bpi WHERE numcte=pNumCte AND telefono=pTelCel AND DATE(fecha)=DATE(current) and sucursal=pSucursal) THEN
		foreach
