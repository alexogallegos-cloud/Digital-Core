CREATE PROCEDURE "informix".sp_mindscredito_por_fechas(pfecha_ini DATE)
...
DEFINE TIPO_PLANTILLA_CREDITO	VARCHAR(50);
DEFINE fname					VARCHAR(200);

--VARIABLE LAYOUT CREDITO
...
DEFINE v_pagoliquidacredito		INTEGER;
...
DEFINE	v_codigo_fun 			CHAR(5);
DEFINE	v_codigo_ref 			CHAR(5);
DEFINE	v_reversado 			CHAR(5);
DEFINE v_idretailer				CHAR(19);
...
LET vcommit = 0;
LET vconteo = 0;
LET v_idtipooperacion = '9';
LET v_idinstrumentomonetario = '1';
LET v_pagoliquidacredito = '0';
...
BEGIN
    ...
    ON EXCEPTION SET SQL_ERR,ISAM_ERR,ERROR_INFO
		...
		END IF
		LET cod_ret  = SQL_ERR;
        LET vmensaje  = TRIM(ERROR_INFO)||' sp_mindscredito_diario en el paso '||vpaso;
		...
		VALUES (v_fecha_ant,'ERROR EN EL PROCESO PARA GENERAR EL: '||TIPO_PLANTILLA_CREDITO,vconteo,vconteo,0,0,cod_ret||' '||vmensaje,'sp_mindscredito_diario');
        RETURN cod_ret, vmensaje;
    END EXCEPTION;
	
	--SET DEBUG FILE TO '/home/c91705134/sp_mindscredito_fechas.out';
    --TRACE ON;
	...
	LET v_fecharegistro	= to_char(pfecha_ini, '%Y-%m-%d');
	...
	LET TIPO_PLANTILLA_CREDITO	 = 'CargaCredMindsF_'||TRIM(cFechaI);
	
	
	LET vpaso = 3;
	--CREACION TABLA TEMPORAL 
	SELECT * FROM "informix".tbl_credito_minds_fechas
	WHERE idregistro IS NULL
	INTO TEMP tmp_tbl_credito_minds_fechas 	WITH NO LOG;
	
	LET vpaso = 4;
	
	FOREACH WITH HOLD
		SELECT a.sucursal, TO_CHAR(a.fecha_mov, "%m") ||  TO_CHAR(a.fecha_mov, "%y") || a.secuencia,a.num_credito,a.num_producto,a.divisa,a.usuario,a.fecha_mov,a.monto,a.hora_mov,b.transacc,a.referencia23,a.folio_suc,a.nro_tarjeta,a.codigo_fun,a.codigo_ref,a.reversado
		INTO v_aliassucursal,v_numeroreferencia,v_nocuenta,v_clavesubproducto,v_clavemoneda,v_noempleado,temp_fecha,v_monto,temp_hora,v_idconcepto,v_ref23,vpfolio_suc,vnum_tarjeta,v_codigo_fun,v_codigo_ref,v_reversado
		FROM bdicred:sd_movhis a, bdiauditor:transacc_minds b
		WHERE a.fecha_mov = pfecha_ini
		and b.tipo ='CR' 
		and a.codigo_fun = b.codigo_fun 
		and a.codigo_ref = b.codigo_ref
		ORDER BY a.num_credito, a.secuencia
		--and transacc_suc IN
		--('6250','6806','4314','4315','6282','6283','6510','6800','6814','6817',
		--'6818','6819','6832','6833','6871','6872','6873','6877','6900','6952',
		--'6999','7041','7100','7139','7380','7381','7382','7383','7384','7385',
		--'7461','7462','7468','7469','7486','7488','7542','7561','7706','8025',
		--'8105','8112','8174','8197','8275','8286','8310','0282','6282','8105',
		--'8112','8104','0407','0402','0403','0404','0405','0406','0482')*/
		...
		SELECT fecha_vencim,numcte
		INTO temp_fechavencimiento,v_numcte
		...
		SELECT pais,idretailer
		INTO vpais,v_idretailer
		FROM intercard:movimiento
		WHERE numtarjeta = vnum_tarjeta
		AND secuenciaextendida = SUBSTR(vpfolio_suc, 2, 15);

	   IF (vpais IS NULL) OR (vpais = '') THEN
			LET vpais = 'N/A';
		END IF
	
		LET vpaso = 8;
		
		SELECT sdo_capital,monto_otorgado
		...
		INSERT INTO "informix".tmp_tbl_credito_minds_fechas (idregistro,aliassucursal,idtipooperacion,idinstrumentomonetario,numeroreferencia,nocuenta,clavesubproducto,clavemoneda,noempleado,fecha,
		monto,montomb,hora,pagoliquidacredito,saldo,fechavencimiento,fechaactualizacion,idestatuscargaminds,idconcepto,fecharegistro,memo,idplaza,idciudadsepomex,idestado,cp,paisgeo,pais_cred,idcomercio)
		VALUES (vconteo,v_aliassucursal,v_idtipooperacion,v_idinstrumentomonetario,v_numeroreferencia,v_nocuenta,v_clavesubproducto,v_clavemoneda,v_noempleado,v_fechaactualizacion,
		v_monto,v_monto,v_hora,v_pagoliquidacredito,v_saldo,v_fechavencimiento,v_fechaactualizacion,v_idestatuscargaminds,v_idconcepto,v_fecharegistro,v_limite,v_plaza,v_ciudad,v_estado,v_cp,v_paisgeo,vpais,v_idretailer);
	...
	FOREACH WITH HOLD
		SELECT a.sucursal,TO_CHAR(a.fecha_mov, "%m") ||  TO_CHAR(a.fecha_mov, "%y") ||a.secuencia,a.num_credito,a.num_producto,a.divisa,a.usuario,a.fecha_mov,
		a.monto,a.hora_mov,b.transacc,a.referencia23		
		INTO v_aliassucursal,v_numeroreferencia,v_nocuenta,v_clavesubproducto,v_clavemoneda,v_noempleado,
		temp_fecha,	v_monto,temp_hora,v_idconcepto,v_ref23
		FROM bdicred:sd_movhiscrd a, bdiauditor:transacc_minds b
		WHERE a.fecha_mov = v_fecha_ant 
		and b.tipo = 'PR' 
		and a.codigo_fun = b.codigo_fun 
		and a.codigo_ref = b.codigo_ref
		ORDER BY a.num_credito, a.secuencia
		--((codigo_fun = '021' and codigo_ref = '1') OR
		--(codigo_fun = '023' and codigo_ref = '1') OR
		--(codigo_fun = '027' and codigo_ref = '1') OR
		--(codigo_fun = '222' and codigo_ref = '1') OR
		--(codigo_fun = '225' and codigo_ref = '1') OR
		--(codigo_fun = '028' and codigo_ref = '1'))*/
		...
		INSERT INTO "informix".tmp_tbl_credito_minds_fechas (idregistro,aliassucursal,idtipooperacion,idinstrumentomonetario,numeroreferencia,nocuenta,clavesubproducto,clavemoneda,noempleado,fecha,
		monto,montomb,hora,pagoliquidacredito,saldo,fechavencimiento,fechaactualizacion,idestatuscargaminds,idconcepto,fecharegistro,idplaza,idciudadsepomex,idestado,cp,paisgeo,pais_cred)
		VALUES (vconteo,v_aliassucursal,v_idtipooperacion,v_idinstrumentomonetario,v_numeroreferencia,v_nocuenta,v_clavesubproducto,v_clavemoneda,v_noempleado,v_fechaactualizacion,
		v_monto,v_monto,v_hora,v_pagoliquidacredito,v_saldo,v_fechavencimiento,v_fechaactualizacion,v_idestatuscargaminds,v_idconcepto,v_fecharegistro,v_plaza,v_ciudad,v_estado,v_cp,v_paisgeo,vpais);
	...
	LET vpaso = 19;	
	LET fname = TRIM(RUTA_DESTINO||TIPO_PLANTILLA_CREDITO) || '.txt'; 
	
	LET vsql = 'CREATE EXTERNAL TABLE tmp_tbl_credito_minds_fechas_ext ' ||
               'SAMEAS tbl_credito_minds_fechas USING ( DATAFILES("DISK:' ||
               TRIM(fname) ||
               '") );';
	...
	INSERT INTO tmp_tbl_credito_minds_fechas_ext SELECT * FROM tmp_tbl_credito_minds_fechas;
	
	DROP TABLE tmp_tbl_credito_minds_fechas_ext;
	DROP TABLE tmp_tbl_credito_minds_fechas;
	
	LET vpaso = 21;
	LET cod_ret = '000000';
    LET vmensaje = 'PROCESO EXITOSO, SE HA GENERADO EL ARCHIVO: '||TRIM(TIPO_PLANTILLA_CREDITO);
	...
	--VALUES (v_fecha_ant,vmensaje,vconteo,vconteo,0,0,'sp_mindscredito_diario');
...
END PROCEDURE
DOCUMENT 
'AUTOR: Gilberto Francisco Naranjo Valles',
'FECHA: 10/09/2024',
'DESCRIPCION: Se agregan campos de geolocalizacion y campos de idcomercio, pais, v_idretailer',
'BD: bdiauditor';

CREATE PROCEDURE "informix".sp_mindscredito_diario()
RETURNING CHAR(6) AS cod_ret,
		  CHAR(80) AS mensaje;
...
BEGIN
	...
	--SET DEBUG FILE TO '/ifxsif01/c90307913/sp_mindscredito_diario.out';
    --TRACE ON;
	...
	LET RUTA_DESTINO	 	 = '/RESPALDOSNEW/';  
	LET TIPO_PLANTILLA_CREDITO	 = 'CargaCredMinds_'||TRIM(cFecha);
	
	LET vpaso = 3;
	---Borrado de la tabla de paso
	BEGIN;
		TRUNCATE TABLE "informix".tbl_credito_minds;
	COMMIT;
	...
	FOREACH WITH HOLD
		...
		INSERT INTO "informix".tbl_credito_minds (idregistro,aliassucursal,idtipooperacion,idinstrumentomonetario,numeroreferencia,nocuenta,clavesubproducto,clavemoneda,noempleado,fecha,
		monto,montomb,hora,pagoliquidacredito,saldo,fechavencimiento,fechaactualizacion,idestatuscargaminds,idconcepto,fecharegistro,memo,idplaza,idciudadsepomex,idestado,cp,paisgeo,pais_cred,idcomercio)
		VALUES (vconteo,v_aliassucursal,v_idtipooperacion,v_idinstrumentomonetario,v_numeroreferencia,v_nocuenta,v_clavesubproducto,v_clavemoneda,v_noempleado,v_fechaactualizacion,
		v_monto,v_monto,v_hora,v_pagoliquidacredito,v_saldo,v_fechavencimiento,v_fechaactualizacion,v_idestatuscargaminds,v_idconcepto,v_fecharegistro,v_limite,v_plaza,v_ciudad,v_estado,v_cp,v_paisgeo,vpais,v_idretailer);
	...
	FOREACH WITH HOLD
		...
		SELECT FIRST 1 fecha_vencim,numcte
		INTO temp_fechavencimiento,v_numcte
		FROM bdicred:sd_maecredcrd 
		WHERE num_credito = v_nocuenta;
	...
	LET vsql = 'echo "UNLOAD TO '||RUTA_DESTINO||TIPO_PLANTILLA_CREDITO||'.txt select * FROM bdiauditor:tbl_credito_minds;">'||RUTA_DESTINO||TIPO_PLANTILLA_CREDITO||'_01.sql'; 
	system vsql;
	LET vsql = '';
	
	LET vpaso = 20;
	--SE EJECUTA EL SCRIPT
	LET vsql = 'chmod 777 '||RUTA_DESTINO||TIPO_PLANTILLA_CREDITO||'_01.sql';
	system vsql;
	LET vsql = '';
	LET vsql = 'dbaccess bdiauditor '||RUTA_DESTINO||TIPO_PLANTILLA_CREDITO||'_01.sql';
	system vsql;
	
	LET vpaso = 21;
	--SE BORRA EL SCRIPT
	LET vsql = '';
	LET vsql = 'rm '||RUTA_DESTINO||TIPO_PLANTILLA_CREDITO||'_01.sql';
	system vsql;
	...
	INSERT INTO "informix".tbl_logextraccion_minds(fechaejecucion,proceso,numregistros,regnuevos,regactualizados,regbaja,rutina)
	VALUES (v_fecha_hoy,vmensaje,vconteo,vconteo,0,0,'sp_mindscredito_diario');
	...
	LET vsql = '';
	LET vsql = "wc -l " || RUTA_DESTINO || TIPO_PLANTILLA_CREDITO || ".txt |awk  'BEGIN {print "||'"update tbl_logextraccion_minds set regtxt="}' || '{print $1}{print "where fechaejecucion=' || "'\''" ||TO_CHAR(v_fecha_hoy,"%m%d%Y") || "'\''" || " and rutina='\''sp_mindscredito_diario'\'' and errorproceso is null and idlog =" || vmaxlogid || '"' || "}'" || ">" || RUTA_DESTINO || "update_cfcredtlogminds.sql";
	system vsql;
	
	-- EJECTUAR SCRIPT CIFRAS CONTROL TXT
	LET vsql = '';
	LET vsql = 'chmod 777 ' || RUTA_DESTINO || 'update_cfcredtlogminds.sql';
	system vsql;
	LET vsql = '';
	LET vsql = 'dbaccess bdiauditor ' || RUTA_DESTINO || 'update_cfcredtlogminds.sql';
	system vsql;
	
	-- BORRAR SCRIPT CIFRAS CONTROL TXT
	LET vsql = '';
	LET vsql = 'rm ' || RUTA_DESTINO || 'update_cfcredtlogminds.sql';
	system vsql;
...
'AUTOR: Gilberto Francisco Naranjo Valles',
'FECHA: 01/10/2024',
'DESCRIPCION: Se quito el campos de saldoafavor y se le asigno vpais=mex y vpais=N/A',
'BD: bdiauditor';

CREATE PROCEDURE "informix".sp_saldo_diario()
RETURNING CHAR(6) AS cod_ret,
		  CHAR(90) AS mensaje;
...
--VARIABLE LAYOUT SALDO MINDS
DEFINE v_cuenta					CHAR(20);
DEFINE v_sdo_prom_mesant		MONEY;
DEFINE v_fecha_emision			DATE;

--VARIABLES DE PASO
DEFINE v_fecha_hoy				DATE;
DEFINE v_fecha_ant				DATE;
DEFINE v_fecha_emision_formateada VARCHAR(10);
DEFINE cDia		  				CHAR(2);
DEFINE cMes		  				CHAR(2);
DEFINE cAno		  				CHAR(4);
DEFINE cFecha  					CHAR(8);
...
BEGIN
    ...
    ON EXCEPTION SET SQL_ERR,ISAM_ERR,ERROR_INFO
		...
		END IF
		LET cod_ret  = SQL_ERR;
        LET vmensaje  = TRIM(ERROR_INFO)||' sp_saldo_diario en el paso '||vpaso;
		INSERT INTO "informix".tbl_logextraccion_minds(fechaejecucion,proceso,numregistros,regnuevos,regactualizados,regbaja,errorproceso,rutina)
		VALUES (v_fecha_hoy,'ERROR EN EL PROCESO PARA GENERAR EL: '||TIPO_PLANTILLA_BANCA,vconteo,vconteo,0,0,cod_ret||' '||vmensaje,'sp_saldo_diario');        RETURN cod_ret, vmensaje;
    END EXCEPTION; --control de bitacora auditoria
	
	--SET DEBUG FILE TO '/ifxsif01/c91705134/sp_saldo_diario.out';
    --TRACE ON;
	...
	LET cFecha = cAno||cMes||cDia;
	LET RUTA_DESTINO	 	 = '/RESPALDOSNEW/';
	LET TIPO_PLANTILLA_BANCA = 'SaldoDiario_'||TRIM(cFecha);
	
	LET vpaso = 3;
	
	--Borrado de la tabla de paso
	BEGIN;
		TRUNCATE TABLE "informix".tbl_saldo_diario;   
	COMMIT;
	
	LET v_cuenta = '';
	LET v_sdo_prom_mesant = 0;
	LET v_fecha_emision_formateada = '';  
	
	LET vpaso = 4;
		
	FOREACH WITH HOLD
		SELECT mae.cuenta, mae.sdo_prom_mesant, TO_CHAR(seef.fecha_emision, "%Y-%m-%d") AS fecha_emision_formateada
		INTO v_cuenta, v_sdo_prom_mesant, v_fecha_emision_formateada
		from bdicheq:sc_maenoc mae
		inner join bdicheq:sc_encabezado_edocta_factelect seef
		on seef.num_cuenta = mae.cuenta
		where seef.fecha_emision = v_fecha_ant 
	...
	LET vpaso = 6;	
		...
		INSERT INTO "informix".tbl_saldo_diario (cuenta, sdo_prom_mesant, fecha_emision)
		VALUES (v_cuenta, v_sdo_prom_mesant, v_fecha_emision_formateada);
	...
	LET vsql = 'echo "UNLOAD TO '||RUTA_DESTINO||TIPO_PLANTILLA_BANCA||'.txt select * FROM bdiauditor:tbl_saldo_diario ORDER BY cuenta, sdo_prom_mesant, fecha_emision;">'||RUTA_DESTINO||TIPO_PLANTILLA_BANCA||'_01.sql'; 
	system vsql;
	LET vsql = '';
	...
	LET vpaso = 12;
	LET cod_ret = '00000';
    LET vmensaje = 'EXITO, SE HA GENERADO EL ARCHIVO: '||TRIM(TIPO_PLANTILLA_BANCA);
...
'FECHA: 22/10/2024',
'DESCRIPCION: Se actualiza el formato de fecha para la carga de MINDS',
'BD: bdiauditor';

CREATE PROCEDURE "informix".sp_mindsremesas_diario()
RETURNING CHAR(6) AS cod_ret,
		  CHAR(90) AS mensaje;
...
BEGIN
	...
	LET cFecha = cAno||cMes||cDia;
	LET RUTA_DESTINO	 	 = '/RESPALDOSNEW/'; 
	LET TIPO_PLANTILLA_REMESA = 'CargaRemesasMinds_'||TRIM(cFecha);
	
	
	LET vpaso = 3; --Borrado de la tabla de paso
		
	
	BEGIN;
		TRUNCATE TABLE "informix".tbl_remesas_pld;
	COMMIT;
	
	BEGIN;
		TRUNCATE TABLE "informix".tbl_remesas_pld2;
	COMMIT;
	...
	FOREACH WITH HOLD
		SELECT a.sucursal,TO_CHAR(a.fech_alt, "%m") ||  TO_CHAR(a.fech_alt, "%y") || a.num_serial,a.usuario,a.fech_alt,a.monto_tot,a.fech_hor,a.sdo_cuenta,a.cuenta,a.producto,b.naturaleza,a.transacc,a.referencia_23,a.suc_cuen,a.folio_suc
		...
		AND a.transacc IN ('1110','1121','1122','1123','1325','1170','1181','1182','1183','1385') -- se agrego la 1355
		
		LET vpaso = 5;
		
		INSERT INTO "informix".tbl_remesas_pld2 (aliassucursal,numeroreferencia,noempleado,fecha,monto,montomb,temp_hora,saldoinicial,nocuenta,clavesubproducto,naturaleza,idconcepto,referencia_23,cve_sucursal_apertura,folio_suc)
	...
	FOREACH WITH HOLD
		...
		INTO v_aliassucursal,v_numeroreferencia,v_noempleado,temp_fecha,v_monto,temp_hora,v_saldoinicial,v_nocuenta,v_clavesubproducto,v_naturaleza,v_idconcepto,v_ref23,v_num_suc,v_tipo_remesa,v_abono_cuenta,v_ben_fec_nac,v_ben_direccion,v_ben_nic,
		v_ben_estado,v_ben_pais,v_ben_ocupacion,v_ord_direccion,v_ord_ciudad,v_ben_tpid,v_ben_numid,v_ben_telefono,v_ord_estado,v_ord_pais,v_ord_telefono,v_ord_tipoid,v_ord_numid,v_fecha_envio,v_pais_origen,v_pais_destino,v_moneda_destino,v_ordenante_nombre1,
		...
		FROM bdiauditor:tbl_remesas_pld2 re, bdisac:sac_pld_remesas r
		WHERE re.folio_suc = r.folio_sucursal
		and r.fecha_remesa=v_fecha_ant -- se agrego
...
------------------------------------------------------------------------------------------------------------------		
		...
		INSERT INTO "informix".tbl_remesas_pld (idregistro,aliassucursal,idtipooperacion,idinstrumentomonetario,idconcepto,numeroreferencia,noempleado,fecha,monto,montomb,depositante,beneficiario,
	...
	LET vpaso = 16;	
	--SE PREPARA LA RUTA DEL SCRIPT PARA LA DESCARGA DE BANCA TRADICIONAL
	LET vsql = 'echo "UNLOAD TO '||RUTA_DESTINO||TIPO_PLANTILLA_REMESA||'.txt select * FROM bdiauditor:tbl_remesas_pld ORDER BY nocuenta, numeroreferencia;">'||RUTA_DESTINO||TIPO_PLANTILLA_REMESA||'_01.sql'; 
	system vsql;
	LET vsql = '';
	
	LET vpaso = 17;
	--SE EJECUTA EL SCRIPT
	LET vsql = 'chmod 777 '||RUTA_DESTINO||TIPO_PLANTILLA_REMESA||'_01.sql';
	system vsql;
	LET vsql = '';
	LET vsql = 'dbaccess bdiauditor '||RUTA_DESTINO||TIPO_PLANTILLA_REMESA||'_01.sql';
	system vsql;
	
	LET vpaso = 18;
	--SE BORRA EL SCRIPT
	LET vsql = '';
	LET vsql = 'rm '||RUTA_DESTINO||TIPO_PLANTILLA_REMESA||'_01.sql';
	system vsql;
	
	LET vpaso = 19;
	LET cod_ret = '00000';
    LET vmensaje = 'EXITO, SE HA GENERADO EL ARCHIVO: '||TRIM(TIPO_PLANTILLA_REMESA);
	...
	VALUES (v_fecha_hoy,vmensaje,vconteo,vconteo,0,0,'sp_mindsremesas_diario');
	
	
	LET vpaso = 20; -- CIFRAS CONTROL CONTEO LINEAS TXT
	...
	LET vsql = '';
	LET vsql = "wc -l " || RUTA_DESTINO || TIPO_PLANTILLA_REMESA || ".txt |awk  'BEGIN {print "||'"update tbl_logextraccion_minds set regtxt="}' || '{print $1}{print "where fechaejecucion=' || "'\''" ||TO_CHAR(v_fecha_hoy,"%m%d%Y") || "'\''" || " and rutina='\''sp_mindsremesas_diario'\'' and errorproceso is null and idlog=" || vmaxlogid || '"' || "}'" || ">" || RUTA_DESTINO || "update_cfbtlogminds.sql";
	system vsql;
	...
	system vsql;
		...
		LET vpaso = 21; --Borrado de la tabla de paso
