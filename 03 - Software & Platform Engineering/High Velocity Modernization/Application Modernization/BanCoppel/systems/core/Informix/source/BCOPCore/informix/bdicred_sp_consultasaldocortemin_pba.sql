CREATE PROCEDURE "informix".sp_consultasaldocortemin_pba(pEmpresa  CHAR (3),pNumCredito CHAR (20), pTipoConsulta SMALLINT)
...
--INI: 31/10/2018 GM2 JUAN OLIVARES: SE ANEXA pTipoConsulta 5 PARA USO DE BANCOPPEL EXPRESS.
		IF ( pTipoConsulta = 5 ) THEN -- Saldo actual
			SELECT sdo_cap_insoluto  + 
			NVL(ROUND((sdo_moratorio + sdo_contab_mora) * (1+ s.iva),2),0) +
			CASE WHEN a.status_cred = 'BT' THEN
            NVL((SELECT SUM(interes_debe - interes_pagado + iva_debe - iva_pagado)
                 FROM bdicred:sd_amortiza_credito 
                 WHERE a.empresa = empresa 
                 AND a.num_credito = num_credito 
                 AND capital_status = 2),0) ELSE 0 END, 0
        ...
        ELSE
			...
			DECODE( pTipoConsulta,1, NVL(sdo_cap_insoluto,0),0)  
            ELSE NVL(sdo_cap_insoluto,0) +  NVL(ROUND((sdo_moratorio + sdo_contab_mora) * (1+ s.iva),2),0)  +			
            CASE WHEN NVL(int_tra_no_exig,0) > 0 THEN
            NVL(int_tra_no_exig,0) - NVL((SELECT interes_debe FROM bdicred:sd_amortiza_credito WHERE a.num_credito = num_credito AND b.fecha = fecha_cuota),0) 
            ELSE 0
            END +
    ...
    --									  (NVL(int_tra_no_exig,0) -NVL(sdo_acum_mes_int,0)) else NVL(int_tra_no_exig,0) end + 
            ...
            (SELECT NVL (campo_trabajo1 ,0)
             ...
             WHERE a.empresa = empresa 
             AND a.num_credito = num_credito 
             AND b.fecha = fecha_cuota)end),0),

                   nvl(( case when (nvl(monto_financiado,0) < 0) then 0
                         else  Nvl(monto_financiado,0) +  Nvl(round((sdo_moratorio + sdo_contab_mora) * (1+ s.iva),2),0)  +			

                          case when NVL(int_tra_no_exig,0) > 0 
                              then  NVL(int_tra_no_exig,0) - NVL((select interes_debe from bdicred:sd_amortiza_credito where a.num_credito = num_credito and b.fecha = fecha_cuota),0) 
                              else 0
                         end +
    --							case when (NVL(int_tra_no_exig,0) -NVL(sdo_acum_mes_int,0) >=0) then  ---
    --									  (NVL(int_tra_no_exig,0) -NVL(sdo_acum_mes_int,0)) else NVL(int_tra_no_exig,0) end + 
    ...
    -- Se agrega condicion a la consulta de movtos para descartar los pagos anticipados por credisolución '082' AAME 27112017
              ...
              END IF
        END IF;
--FIN: 31/10/2018 GM2 JUAN OLIVARES: SE ANEXA pTipoConsulta 5 PARA USO DE BANCOPPEL EXPRESS.
...
'FECHA DE MODIFICACION: 29 DE OCTUBRE DE 2018',
'OBJETIVO:SE ANEXA pTipoConsulta 5 PARA USO DE BANCOPPEL EXPRESS ',
'BD: BDIBPI';

CREATE PROCEDURE "informix".sp_consultasaldocorte(pEmpresa  CHAR (3),pNumCredito CHAR (20), pTipoConsulta SMALLINT)
  RETURNING CHAR (5) AS CodRet, DECIMAL (14,2) AS saldototal;
...
CREATE PROCEDURE "informix".sp_cierre_credito_2019(pEmpresa CHAR(3))
RETURNING
   CHAR(6)        AS Cod_Ret,
   CHAR(80)       AS Mens_Ret;
...
--             * Se agrega la actualizaciÃ³n del campo mto_fin_ven_trasp
--             * Se agrega la actualizaciÃ³n del campo dias_acum_mora
--             * Se agrega la actualizaciÃ³n del campo dias_acum_cap
--             * Se modifica el calculo de el parÃ¡metro de la tasa
...
-- Modifico: Paul Ivan Quintero Varela
-- Fecha: 12/02/2010
-- Comentario: * Se agrega validaciÃ³n para que la inserciÃ³n del historico de saldos (tabla sd_maesdoshistcrd)
--               tambien se realice en su facturaciÃ³n.
--             * Se modifica para que la condiciÃ³n del traspaso a los 90 dias no este ligado a que sea realizada siempre
...
SET DEBUG FILE TO '/tmp/sp_cierre_credito';
TRACE ON;

-- *******************************************************
--  VALIDACIONES DE EJECUCIÃN DE PROCESO                 *
-- *******************************************************
SELECT a.empresa 
  INTO cEmpresa
...
IF NVL(cCodTipCred,"") = "" THEN 
     LET cCodRet     = "000002";
     LET cMensajeRet = "El tipo de crÃ©dito indicado no existe";
     RETURN cCodRet, cMensajeRet;
END IF;
...
-- *******************************************************
--  SELECCIÃN DE CRÃDITOS PARA PROCESAR                  *
-- *******************************************************
...
-- *******************************************************
--  IDENTIFICACIÃN DE PROCESOS POR REALIZAR              *
-- *******************************************************

          -- ValidaciÃ³n para identificar un (Cierre de Mes).
          IF dtFechaHoy = dtFechaFinMes THEN  
            LET cIdProc1 = "C";
          END IF;			

          -- Validaciones para dÃ­as inhabiles y cambios de mes. (FacturaciÃ³n)
         ...
         --FMV 9ene13 Validacion 4o. Proceso, Provision a fin de mes cuando el siguiente dia es inhabil              
          IF (cIdProc1 = "C" AND dtFechaHoyAux > DATE(dtFechaHoy+1) AND iDiaCorte = 2) THEN
             ...
             --LET iDiasInt = iDiasInt - 1;  --Se quita un dia ya que provisionarÃ¡ los q no tienen vencimiento el dia 2
          END IF;
...
-- *******************************************************
   ...
   --FMV 20-ENE-11  Calcula el interes sobre el saldo capital sin el monto de traspaso ya calculado en la facturacion
           ...
           END IF;      
                  ...
                  --Se actualiza la fecha del prÃ³ximo proceso
                  UPDATE "informix".sd_maecredanexocrd
                     SET fecha_proceso  = dtFechaProx
                   WHERE num_credito    = cNumCredito
                     AND empresa        = cEmpresa;
...
-- *******************************************************
--  PROVISIÃN POR FIN DE MES EN DIA INHABIL SIGUIENTE    *
-- *******************************************************
 ...
 LET dProvInt_inh = dSdoInt_inh;
	IF dTasaInter > 0 THEN   --FMV 26-AGO-14: Si la tasa de interes es mayor a cero, genero calculo de interes Real.
	  ...
	  IF cCodRet <> "000000" THEN
			LET cCodRet      = "000005";
			LET cMensajeRet  = "OcurriÃ³ un error al realizar calculo de iva de interes";
			ROLLBACK WORK;
			RETURN cCodRet,cMensajeRet;
	  END IF;
	END IF;  --FMV
...
---Se genera la provision de interes en fin de mes o Facturacion
                ...
                IF (cCodRet <> "000000") THEN
                       LET cCodRet      = "000006";
                       LET cMensajeRet = "OcurriÃ³ un error al registrar la provisiÃ³n de interes";
                       ROLLBACK WORK;
                       RETURN cCodRet,cMensajeRet;    
                END IF;
...
---Se genera la provision de iva de interes real en fin de mes o Facturacion
                        ...
                        IF (cCodRet <> "000000") THEN
                               LET cCodRet      = "000007";
                               LET cMensajeRet = "OcurriÃ³ un error al registrar la provisiÃ³n de iva de interes";
                               ROLLBACK WORK;
                               RETURN cCodRet,cMensajeRet;    
                        END IF;
                END IF;
---ini cas, Se agrega el movimiento aplicativo de interes gravable y exento
                ...
                IF dIntGrav_inh>0 THEN
                        ...
                        IF (cCodRet <> "000000") THEN
                               LET cCodRet      = "000007";
                               LET cMensajeRet = "OcurriÃ³ un error al registrar el InterÃ©s Gravado";
                               ROLLBACK WORK;
                               RETURN cCodRet,cMensajeRet;    
                        END IF;
                END IF;
                IF dIntExen_inh>0 THEN
                        ...
                        IF (cCodRet <> "000000") THEN
                               LET cCodRet      = "000007";
                               LET cMensajeRet = "OcurriÃ³ un error al registrar el InterÃ©s Exento";
                               ROLLBACK WORK;
                               RETURN cCodRet,cMensajeRet;    
                        END IF;
                END IF;
...
END IF;
-- **********************************************************************
--  FIN DE PROVISIÃN POR REGISTROS DE MOVTO EN FIN MES POR DIA INHABIL  *
-- **********************************************************************
...
-- FMV 25-ENE-2011: RealizarÃ¡ la facturaciÃ³n siempre y cuando la suma de Saldos vencidos sean mayores a cero
                     ...
                     where empresa             = cEmpresa
                       ...
                       IF dIvaPeriodo IS NULL THEN LET dIvaPeriodo=0; END IF;
                        ...
                        IF dIntPeriodo>0 THEN
                                ...
                                IF (cCodRet <> "000000") THEN
                                       LET cCodRet      = "000007";
                                       LET cMensajeRet = "OcurriÃ³ un error al registrar el InterÃ©s Periodo";
                                       ROLLBACK WORK;
                                       RETURN cCodRet,cMensajeRet;    
                                END IF;
                        END IF;
           ...
           IF dTraspCap>0 THEN
             ...
             --FMV 25Abr13: Actualiza indicador del 1er. vencido y dias de atraso
              ...
              -- Traspaso capital vigente a transitorio.
               ...
               IF (cCodRet <> "000000") THEN
                      LET cCodRet      = "000009";
                      LET cMensajeRet = "OcurriÃ³ un error al registrar traspaso capital vigente a transitorio";
                      ROLLBACK WORK;
                      RETURN cCodRet,cMensajeRet;  
                END IF;
                     ...
                     WHERE empresa        = cEmpresa 
                      ...
                      IF (dSdoCapital - dTraspCap)>0 THEN
                          ...
                          RETURNING cCodRet,cMensajeRet;
                            ...
                            IF (cCodRet <> "000000") THEN
                                   LET cCodRet      = "000010";
                                   LET cMensajeRet = "OcurriÃ³ un error al registrar traspaso capital vigente a vdo no exigible";
                                   ROLLBACK WORK;
                                   RETURN cCodRet,cMensajeRet;    
                            END IF;
                      END IF;
                      ...
                      IF dMntVencido>0 THEN
                          ...
                          RETURNING cCodRet,cMensajeRet;
                            ...
                            IF (cCodRet <> "000000") THEN
                                   LET cCodRet      = "000011";
                                   LET cMensajeRet = "OcurriÃ³ un error al registrar traspaso capital transitorio a exigible";
                                   ROLLBACK WORK;
                                   RETURN cCodRet,cMensajeRet;    
                            END IF;
                      END IF;
                      ...
                      IF dIntVdo>0 THEN
                          ...
                          RETURNING cCodRet,cMensajeRet;
                            ...
                            IF (cCodRet <> "000000") THEN
                                   LET cCodRet      = "000012";
                                   LET cMensajeRet = "OcurriÃ³ un error al registrar traspaso interes vigente a vdo";
                                   ROLLBACK WORK;
                                   RETURN cCodRet,cMensajeRet;    
                            END IF;
                      END IF;
                /*      -- ProvisiÃ³n de interes ordinario a provisiÃ³n de cartera vencida
                      ...
                      RETURNING cCodRet,cMensajeRet;
                        ...
                        IF (cCodRet <> "000000") THEN
                               LET cCodRet      = "000013";
                               LET cMensajeRet = "OcurriÃ³ un error al registrar provisiÃ³n de interes ordinario";
                               IF cBegin = "S" THEN
                                  ROLLBACK WORK;
                               END IF;
                               RETURN cCodRet,cMensajeRet;    
               ...
               WHERE num_credito   = cNumCredito
                ...
                /*      CALL "informix".genmovcrd(cEmpresa,cNumCredito, cNumProducto , 2, 
                  ...
                  RETURNING cCodRet,cMensajeRet;
                    ...
                    IF (cCodRet <> "000000") THEN
                           LET cCodRet      = "000014";
                           LET cMensajeRet = "OcurriÃ³ un error al registrar traspaso capital vencido no exigible a vencido exigible";
                           ROLLBACK WORK;
                           RETURN cCodRet,cMensajeRet;    
                    END IF;             
           END IF;
...
-- *******************************************************
-- RESPALDO DE INFORMACIÃN CONTABILIDAD A FIN DE MES     *
-- *******************************************************
     IF cIdProc1 = "C"  or day(dtFechaHoy)=20 THEN
           INSERT INTO "informix".sd_maesdoscontcrd
                SELECT dtFechaHoy, *
