CREATE PROCEDURE "informix".sp_conssdoticket(cEmpresa  CHAR (3),CNumCredito CHAR (20), dFechaHoy Date)

  RETURNING CHAR (5) AS CodRet,DECIMAL(14,2) AS SusCompras, DECIMAL(14,2) AS Disposiciones, DECIMAL(14,2) AS SusComisiones, DECIMAL(14,2)

AS Iva, DECIMAL (14,2) AS SusAbonos, DECIMAL(14,2) AS dmorapagados, DECIMAL (14,2) AS divamorapagados, DECIMAL (14,2)

AS interespagototaltc;
...
-- SET DEBUG FILE TO "/tmp/conssdoticket.out";
-- TRACE ON;
...
/*
   SELECT fecha_hoy INTO dFechaCentral FROM bdicred:sd_fechas;

        IF (dFechaHoy <= dFechaCentral <= dFechaHoy) then
            EXECUTE PROCEDURE sp_mes_siguiente(dFechaHoy,-1,DAY(dFechaHoy))
                INTO cCodRetOtro, dPeriodoAnterior, iDiasPeriodo;

            IF cCodRetOtro <> "000" AND cCodRet = "000" THEN
                LET cCodRet = cCodRetOtro;
            END IF;

            LET dPeriodoIni = dPeriodoAnterior + 1 UNITS DAY;
            LET dPeriodoFin = dFechaHoy;
        ELSE
            LET dPeriodoIni = dFechaHoy + 1 UNITS DAY;
            LET dPeriodoFin = dFechaCentral;
        END IF;

*/
/*
	SELECT interes_pago_total_tc
	INTO dintpagtaltc
	FROM bdicred:sd_encabezado2_edocta
	WHERE num_credito = CNumCredito AND
	fecha_emision  = (SELECT max(fecha_emision)
	FROM bdicred:sd_encabezado2_edocta
...
*/
    ...
    select sdo_cap_insoluto +
           int_tra_no_exig -
           case when NVl(sdo_int_anticip,0) > 0 then sdo_int_anticip else 0 end +
		   --case when  NVL(mto_venc_trasp,0) > 0 then sdo_int_anticip else 0 end +
