CREATE PROCEDURE "informix".sp_doctosfusionados(pNumCteTrasp CHAR(20))
	...
	CHAR(20), -- Cliente
	CHAR(20), -- Cuenta
	CHAR(4),  -- Codigo del Documento
	INTEGER,  -- Secuencia
	CHAR(10), -- Fecha Alta
	CHAR(35); -- Descripcion del documento
	...
	--SET DEBUG FILE TO "/respaldosbd/Daniela/sp_doctosfusionados.out";
	--TRACE ON;
	...
	BEGIN
		...
		ON EXCEPTION SET iSql_err
			...
			IF iSql_err <> 0 THEN
				...
				RETURN  cCodRet,cCliente,cCta,cCod_docto,iSecuencia,cFecha_alta,cDescripcion;
		...
		IF EXISTS (SELECT {+INDEX (bdinteg:si_fusexpediente pk_expcta)} cliente FROM bdinteg:"informix".si_fusexpediente where cliente = pNumCteTrasp) THEN
		
			FOREACH
			
				SELECT {+INDEX (bdinteg:si_fusexpediente pk_expcta)} cliente, cuenta, cod_docto, secuencia, fecha_alta
				  INTO cCliente, cCta, cCod_docto, iSecuencia, cFecha_alta
				  FROM bdinteg:"informix".si_fusexpediente
				 WHERE cliente = pNumCteTrasp
				 
				SELECT descripcion
				  INTO cDescripcion
				  FROM bdidigital@coppelimg_tcp:dg_tipodocumento
				 WHERE cod_docto = cCod_docto;
				 
				RETURN cCodRet,cCliente,cCta,cCod_docto,iSecuencia,cFecha_alta,cDescripcion WITH RESUME;
			
			END FOREACH;
		
		ELSE
		
			LET cCodRet = '00001'; -- El cliente no tiene documentos fusionados
...
DOCUMENT
'Consulta documentos fusionados del cliente traspasado',
'Autor :Daniela Ramírez',
'FECHA : 27/Septiembre/2011',
'BD: bdinteg';

CREATE PROCEDURE "informix".sp_desbctasfus_obtenerdoctos(pNumeroCliente CHAR(20), inicia  smallint,pnum_regs  smallint)
RETURNING CHAR(5), CHAR(4), CHAR(35), CHAR(20), SMALLINT, CHAR(10),CHAR(50),CHAR(4);
...
BEGIN
  ...
  ON EXCEPTION SET iSqlErr
    IF iSqlErr <> 0 THEN
        LET cCodRet = iSqlErr;
        RETURN cCodRet, cCodDocto, cDesDocto, cCuenta, sSecuencia,dFecha,cDescrip,cCuentaValida;
    END IF;
  END EXCEPTION;
	...
	SET ISOLATION TO DIRTY READ;
		...
		FOREACH
			...
			INTO cCodDocto, cCuenta, sSecuencia, cDesDocto,dFecha,cDescrip
			...
			GROUP BY exp.cod_docto, exp.cuenta, exp.secuencia,docto.descripcion,exp.fecha_alta,exp.descrip2 ORDER BY cod_docto,secuencia
			...
			IF sContador>inicia THEN      
				RETURN cCodRet, cCodDocto, cDesDocto, cCuenta, sSecuencia,dFecha,cDescrip,cCuentaValida WITH RESUME;    
			END IF
		END FOREACH;    
    IF sContador=0 THEN
        LET cCodRet = "00100";
