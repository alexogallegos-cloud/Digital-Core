create procedure "informix".sp_mf_consultatipoproducto(p_dtfecha_ini DATETIME YEAR TO FRACTION, p_dtfecha_final DATETIME YEAR TO FRACTION, p_ValorInicial INT8,p_vCodProd varchar(4))
RETURNING 
...
BEGIN
	...
	inner join  intercard:tarjeta tar  on  tar.numtarjeta = b.numtarjeta and tar.codproductotarjeta = p_vCodProd
	WHERE fechahorainauth BETWEEN p_dtfecha_ini AND p_dtfecha_final;
	...
	IF EXISTS(SELECT b.NumTarjeta FROM intercard:movimiento b 
			inner join  intercard:tarjeta tar  on  tar.numtarjeta = b.numtarjeta   and tar.codproductotarjeta = p_vCodProd
			WHERE fechahorainauth BETWEEN p_dtfecha_ini AND p_dtfecha_final) THEN
	ELSE
			RETURN "00003",NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL;
	END IF;
	...
	FOREACH
		SELECT 
		MetodoCaptura,CodigoISO,NumTarjeta,Monto,InfReceptor,FechaLocalTransaccion,FechaHoraInAuth,HoraLocalTransaccion,Motivo
		...
		FROM
			TABLE(
			MULTISET(SELECT {+INDEX(intercard:movimiento idx_fechahorainauth)} 
			SKIP iValorSalto FIRST iValorBloque  b.MetodoCaptura AS MetodoCaptura, b.CodigoISO AS CodigoISO
...
END;
--##############################################################################
--## Procedimiento   : spmf_tipoproducto
