CREATE PROCEDURE "informix".sp_registra_notificacion(
		p_folio char(11), p_cliente char(20), p_tel char(10), p_correo char(100), 
		p_ident_sms integer, p_ident_correo integer, p_canal char(10), 
...
DEFINE v_desc_bitacora VARCHAR(100);
...
BEGIN
	...
	/*Genera la variable Asunto dependiendo el tipo de notificación*/
	...
	/*Se insertan las variables de la notificación en la Base de Datos*/
	...
	/*Se registra en la bitácora el envío del correo*/
...
CREATE PROCEDURE "informix".sp_crea_folio_aclaracion(pCliente CHAR(9))
	...
	--Variables--
		...
		DEFINE c_fecha_ingreso  			DATETIME YEAR to FRACTION(5);
		...
		LET v_consecutivo  				= NULL;
		LET c_fecha_ingreso				= CURRENT;
		LET c_separador					= '-';
		LET c_prefijo					= 'ACL';
	...
	BEGIN
		ON EXCEPTION SET sql_err
			IF sql_err <> 0 THEN
			    ...
			    RETURN v_cod_ret, v_folio_aclaracion;
		    END IF;
		END EXCEPTION;
		...
		--Si el Numero de cliente no existe en los registros del banco
		IF v_existe_cliente IS NULL OR v_existe_cliente = 0 THEN
			LET v_cod_ret = '00001';
			RETURN v_cod_ret, v_folio_aclaracion;
		END IF;
		
		SELECT max(consecutivo_cliente)
			INTO v_consecutivo
		FROM "informix".acl_folio_aclaracion
		...
		LET v_folio_aclaracion = c_prefijo || c_separador || pCliente || c_separador || LPAD(v_consecutivo, 3, '0');
		
		INSERT INTO "informix".acl_folio_aclaracion(num_cliente, consecutivo_cliente, fecha_ingreso, folio_aclaracion) 
			VALUES(pCliente, v_consecutivo, c_fecha_ingreso, v_folio_aclaracion);
...
CREATE PROCEDURE "informix".sp_obten_cat_tipo_flujo()
	RETURNING
		INTEGER 			as id_tipo_flujo,
		CHAR(100)			as tipo_flujo;

	--Variables--
	DEFINE id_tipo_flujo		INTEGER;
	DEFINE tipo_flujo 			CHAR(100);
	...
	BEGIN
		...
		FOREACH
			SELECT pky_tipo_flujo, descripcion
				INTO id_tipo_flujo, tipo_flujo
			FROM "informix".acl_tipo_flujo
			WHERE activo=1 
				ORDER BY pky_tipo_flujo asc

			RETURN id_tipo_flujo, tipo_flujo WITH RESUME;
...
CREATE PROCEDURE "informix".sp_registra_comentario_cliente(
						pIdAclaracion INTEGER, pMensaje LVARCHAR)
	...
	BEGIN
		...
		IF ((pMensaje IS NULL) OR (pMensaje = '')) AND (pIdAclaracion IS NULL) THEN
			RETURN '00001'; --La invocaciÃ³n debe tener algÃºn valor
		END IF;
		
		IF (pMensaje IS NULL) OR (pMensaje = '') THEN
			RETURN '00002'; --Debe proporcionar el mensaje a almacenar
		END IF;
		
		IF (pIdAclaracion IS NULL) THEN
			RETURN '00003'; --Debe proporcionar pky_aclaracion
		END IF;
		...
		IF (v_pky_aclaracion IS NULL) THEN
			RETURN '00004'; --No existe la AclaraciÃ³n
		END IF;
		
		SELECT pky_resolucion 
			INTO c_accion_resolucion
		FROM acl_resolucion WHERE nombre = 'comentarioTransaccion';
		...
		INSERT INTO "informix".acl_entrada_bitacora 
			...
			VALUES(entrada_bitacora_seq.NEXTVAL, TRIM(pMensaje), c_fechahora, v_folio_csuac, c_accion_resolucion, pIdAclaracion, v_fky_area, 
				v_fky_estatus_aclaracion, v_fky_estatus_corp_analisis, v_fky_estatus_corp_general, c_usuario);
...
CREATE PROCEDURE "informix".sp_relaciona_folioacl_idacl(pFolioAcl CHAR(18), pIdAclaracion INTEGER)
	...
	DEFINE v_folio_aclaracion			CHAR(18);
	DEFINE v_pky_aclaracion				INTEGER;
	DEFINE v_fecha_folio				DATETIME YEAR to FRACTION(5);
	DEFINE v_fecha_aclaracion			DATE;
	DEFINE v_cliente_folio 				CHAR(9);
	...
	LET v_folio_aclaracion			= NULL;
	LET v_pky_aclaracion			= NULL;
	LET v_fecha_folio				= NULL;
	LET v_fecha_aclaracion			= NULL;
	LET v_cliente_folio				= NULL;
	...
	BEGIN
		...
		IF ((pFolioAcl IS NULL) OR (pFolioAcl = '')) AND (pIdAclaracion IS NULL) THEN
			RETURN '00001'; --La invocaciÃ³n debe tener algÃºn valor
		END IF;
		
		IF (pFolioAcl IS NULL) OR (pFolioAcl = '') THEN
			RETURN '00002'; --Debe proporcionar folio_aclaracion
		END IF;
		...
		SELECT folio_aclaracion, fecha_ingreso, num_cliente
			INTO v_folio_aclaracion, v_fecha_folio, v_cliente_folio
		FROM acl_folio_aclaracion
		WHERE folio_aclaracion = pFolioAcl;
		
		SELECT pky_aclaracion, fechacaptura, num_cliente
			INTO v_pky_aclaracion, v_fecha_aclaracion, v_cliente_aclaracion
		FROM acl_aclaracion
		WHERE pky_aclaracion = pIdAclaracion;
		
		IF ((v_folio_aclaracion IS NULL) OR (v_folio_aclaracion = '')) AND (v_pky_aclaracion IS NULL) THEN
			RETURN '00004'; --Valores de invocaciÃ³n incorrectos
		END IF;
		
		IF (v_folio_aclaracion IS NULL) OR (v_folio_aclaracion = '') THEN
			RETURN '00005'; --No existe el Folio de AclaraciÃ³n
		END IF;
		
		IF (v_pky_aclaracion IS NULL) THEN
			RETURN '00006'; --No existe la AclaraciÃ³n
		END IF;
		
		IF (v_cliente_folio <> v_cliente_aclaracion) THEN
			RETURN '00007'; --El cliente del folio con el de la aclaraciÃ³n no coinciden
		END IF;
		
		IF (DATE(v_fecha_folio) <> v_fecha_aclaracion) THEN
			RETURN '00008'; --Las fechas de los folios no coinciden
		END IF;
		
		IF (v_fecha_aclaracion <> c_fecha_actual) THEN
			RETURN '00009'; --La fecha de ingreso es diferente a la fecha actual
		END IF;
		
		INSERT INTO "informix".acl_folio_aclaracion_acl_aclaracion(folio_aclaracion, fky_aclaracion) 
			VALUES(pFolioAcl, pIdAclaracion );
...
CREATE PROCEDURE "informix".sp_buscarorigen_por_flujo(
                        pProducto INTEGER,
                        pTipoFlujo INTEGER)
						
	RETURNING
		CHAR(5)				AS cod_ret,
		INTEGER 			AS id_origen_evento,
		CHAR(50)			AS descripcion,
		CHAR(50)			AS nombre,
		INTEGER				AS porcentaje_maximo,
		SMALLINT			AS validacion_cargo_rec,
		SMALLINT			AS indicador_robo_identidad,
		INTEGER				AS id_tipo_producto;
	...
	DEFINE v_porcentaje_maximo  		INTEGER;
	DEFINE v_acepta_cargos_recurrentes	SMALLINT;
	DEFINE v_indicador_robo_identidad	SMALLINT;
	DEFINE v_pky_tipo_producto 			INTEGER;

	LET v_cod_ret 						= "00000";
	LET v_pky_origen_evento 			= NULL;
	LET v_descripcion  					= NULL;
	LET v_nombre  						= NULL;
	LET v_porcentaje_maximo  			= NULL;
	LET v_acepta_cargos_recurrentes		= NULL;
	LET v_indicador_robo_identidad		= NULL;
	...
	BEGIN
		ON EXCEPTION SET sql_err
			IF sql_err <> 0 THEN
				LET v_cod_ret = sql_err;
				RETURN v_cod_ret, v_pky_origen_evento, v_descripcion, v_nombre, v_porcentaje_maximo, v_acepta_cargos_recurrentes, 
						v_indicador_robo_identidad, v_pky_tipo_producto;
			END IF;
		END EXCEPTION;
	
		FOREACH
			
			SELECT DISTINCT oe.pky_origen_evento, oe.descripcion, oe.nombre, oe.porcentaje_maximo, 
					oe.validacion_cargo_recurrente, oe.indicador_robo_identidad, tp.pky_tipo_producto
				INTO v_pky_origen_evento, v_descripcion, v_nombre, v_porcentaje_maximo, v_acepta_cargos_recurrentes, 
					v_indicador_robo_identidad, v_pky_tipo_producto
			FROM acl_tipo_evento te
				INNER JOIN acl_origen_evento oe ON te.fky_origen_evento = oe.pky_origen_evento
				INNER JOIN acl_tipo_producto_origen_evento tpto ON oe.pky_origen_evento = tpto.fky_origen_evento
				INNER JOIN acl_tipo_producto tp ON tpto.fky_tipo_producto = tp.pky_tipo_producto
			WHERE te.activo = 1 AND oe.activo = 1 AND te.fky_tipo_flujo = pTipoFlujo AND oe.indicador_robo_identidad = 0
				AND tp.producto = pProducto
			ORDER BY oe.descripcion ASC
			
			RETURN
				v_cod_ret, v_pky_origen_evento, v_descripcion, v_nombre, v_porcentaje_maximo, v_acepta_cargos_recurrentes, 
					v_indicador_robo_identidad, v_pky_tipo_producto
			WITH RESUME;
		END FOREACH;
	END;
...
CREATE PROCEDURE "informix".sp_busca_acls_por_folioaclaracion(
                        pFolioAcl VARCHAR(18),
                        pCliente VARCHAR(9))
	...
	DEFINE v_pky_aclaracion				INTEGER;
	DEFINE v_cliente_folio_acl			VARCHAR(9);
	
	LET v_cod_ret 						= "00000";
	LET v_folio_csuac 					= NULL;
	LET v_pky_aclaracion	 			= NULL;
	LET v_cliente_folio_acl				= NULL;
	...
	BEGIN
		ON EXCEPTION SET sql_err
			IF sql_err <> 0 THEN
				...
				RETURN v_cod_ret, v_pky_aclaracion, v_folio_csuac;
			END IF;
		END EXCEPTION;
		
		IF ((pFolioAcl IS NULL) OR (pFolioAcl = '')) AND ((pCliente IS NULL) OR (pCliente = '')) THEN
			RETURN '00001', v_pky_aclaracion, v_folio_csuac; --La invocaciÃ³n debe tener algÃºn valor
		END IF;
		
		IF (pFolioAcl IS NULL) OR (pFolioAcl = '') THEN
			RETURN '00002', v_pky_aclaracion, v_folio_csuac; --Debe proporcionar folio_aclaracion
		END IF;
		
		IF (pCliente IS NULL) OR (pCliente = '') THEN
			RETURN '00003', v_pky_aclaracion, v_folio_csuac; --Debe proporcionar el cliente
		END IF;
		
		LET pCliente = LPAD(TRIM(pCliente), 9, '0');
		
		FOREACH
			SELECT acl.pky_aclaracion, acl.folio_csuac 
				INTO v_pky_aclaracion, v_folio_csuac  	
			FROM acl_aclaracion acl
				INNER JOIN acl_folio_aclaracion_acl_aclaracion rfacl ON rfacl.fky_aclaracion = acl.pky_aclaracion
				INNER JOIN acl_folio_aclaracion facl ON facl.num_cliente = acl.num_cliente AND facl.folio_aclaracion = rfacl.folio_aclaracion
			WHERE 
				rfacl.folio_aclaracion = pFolioAcl AND facl.num_cliente = pCliente

			RETURN
				v_cod_ret, v_pky_aclaracion, v_folio_csuac
			WITH RESUME;
		END FOREACH;
	END;
END PROCEDURE
