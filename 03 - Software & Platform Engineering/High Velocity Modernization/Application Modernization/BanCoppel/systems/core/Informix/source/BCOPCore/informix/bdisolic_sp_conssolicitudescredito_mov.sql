CREATE PROCEDURE "informix".sp_conssolicitudescredito_mov(pTipo INTEGER, pEmpresa CHAR(3), pSucursal CHAR(20), pSolicitudes SMALLINT,
													  pNumCte CHAR(20),pStatus_solicitud CHAR(5),pNum_producto CHAR(4),pTpo SMALLINT,
...
RETURNING
	...
	--SET DEBUG FILE TO "/informix/jesus/sp_conssolicitudescredito.out";
	--TRACE ON;
		...
		ELSE
            IF pTipo = 0 THEN
				...
				IF iConsultaSP = 1 THEN
					FOREACH
						EXECUTE PROCEDURE "informix".envia_monitorsol_cjunk_ss_mov(
                       ...
                       /*IF (s_status = 'OS') or (s_status = 'RT') then-- OR  s_status<>'OA'
			...
			IF iConsultaSP = 2 OR vCantReg1 < 11 THEN 					
					...
					FOREACH
						EXECUTE PROCEDURE "informix".envia_monitorsol_pp_ss_mov(
...
--****************************
            ELIF pTipo = 4 THEN
				...
				IF iConsultaSP = 1 THEN
					FOREACH
						EXECUTE PROCEDURE "informix".envia_monitorsol_cjunk_ss_mov(
										  pEmpresa,pSucursal,pCantRegPres,pNumCte,'OA',pNum_producto,3,pFechaIni,pFechaFin)
									INTO  cValRetorno,s_numsol,s_numcte,s_nombre,s_rfc,s_fechasol,s_fechaaut,s_Producto,
										  s_ProdDes,s_linea,s_status,s_stdesc,s_comentario,s_diacorte,s_divisa,s_ingreso,
										  cCausaSol,vDescCausaSol,vdias_vigencia
