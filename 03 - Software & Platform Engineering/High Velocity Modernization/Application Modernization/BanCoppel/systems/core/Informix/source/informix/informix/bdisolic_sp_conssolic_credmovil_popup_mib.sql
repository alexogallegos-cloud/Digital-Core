CREATE PROCEDURE "informix".sp_conssolic_credmovil_popup_mib(pTipo 				INTEGER,
														pEmpresa 			CHAR(3),
														pSucursal 			CHAR(20),
														pSolicitudes 		SMALLINT,
														pNumCte 			CHAR(20),
														pStatus_solicitud 	CHAR(5),
														pNum_producto 		CHAR(4),
														pTpo 				SMALLINT,
														pEjecucion 			INTEGER,
														pLimit 				INTEGER,
														pConsultaSP 		INTEGER,
														pCantRegPres 		INTEGER,
														pFechaIni 			DATE,
														pFechaFin 			DATE)
RETURNING
	...
	INTEGER 	AS Factor_piso;	 --	factor piso. Identificadores: 1.- TDC Coppel, 2.- Cr̩dito Motos Coppel, 3.-Ambos.
	...
	--SET DEBUG FILE TO "/home/sysifx/Brando/sp_conssolic_credmovil_popup.out";
	--TRACE ON;
	...
	BEGIN
		...
		IF NVL(pNumCte,'') = '' THEN
			...
			RETURN  cValRetorno,s_numsol,s_numcte,s_nombre,s_rfc,s_fechasol,s_fechaaut,s_Producto,s_ProdDes,s_linea,
					...
					iejecucion,s_Limit,v_CausaSitEsp,iEsCtaCap,iConsultaSP,vCantRegPres,cSitEsp,cCuenta_eje,iFrecuencia,iDiaPago,NVL(cTelCasa,''),NVL(cTelOficina,''), NVL(cFactor_piso,'');
		END IF;
            ...
            IF pTipo = 0 THEN
				...
				IF 0 = 1 THEN
					FOREACH
						...
						IF s_Producto = "6500" THEN
							...
							SELECT factor_piso INTO cFactor_piso FROM "informix".ss_solicitudes 
							WHERE empresa = pEmpresa AND num_solicitud = s_numsol AND status_solicitud = s_status;
						END IF;
						...
						/*RETURN  cValRetorno,s_numsol,s_numcte,s_nombre,s_rfc,s_fechasol,s_fechaaut,s_Producto,s_ProdDes,s_linea,
								s_status,s_stdesc,s_comentario,s_diacorte,s_divisa,s_ingreso,cCausaSol,vDescCausaSol,vdias_vigencia,
								iejecucion,s_Limit,v_CausaSitEsp,iEsCtaCap,iConsultaSP,vCantReg,cSitEsp,cCuenta_eje,iFrecuencia,iDiaPago,NVL(cTelCasa,''),NVL(cTelOficina,''), NVL(cFactor_piso,'') WITH RESUME;
					*/
					
						RETURN  NVL(cValRetorno,''),NVL(s_numsol,''),NVL(s_numcte,''),NVL(s_nombre,''),NVL(s_rfc,''),NVL(s_fechasol,''),
						NVL(s_fechaaut,''),NVL(s_Producto,''),NVL(s_ProdDes,''),NVL(s_linea,''),
						NVL(s_status,''),NVL(s_stdesc,''),NVL(s_comentario,''),NVL(s_diacorte,''),NVL(s_divisa,''),NVL(s_ingreso,''),
						NVL(cCausaSol,''),NVL(vDescCausaSol,''),NVL(vdias_vigencia,''),
						NVL(iejecucion,''),NVL(s_Limit,''),NVL(v_CausaSitEsp,''),NVL(iEsCtaCap,''),NVL(iConsultaSP,''),NVL(vCantReg,''),
						NVL(cSitEsp,''),NVL(cCuenta_eje,''),NVL(iFrecuencia,''),NVL(iDiaPago,''),NVL(cTelCasa,''),NVL(cTelOficina,''), 
						NVL(cFactor_piso,'') WITH RESUME;
					END FOREACH;
				ELSE
					LET vCantReg1 = 12;
				END IF;
				...
				IF iConsultaSP = 2 OR vCantReg1 < 11 THEN
					...
					FOREACH
						...
						RETURN  cValRetorno,s_numsol,s_numcte,s_nombre,s_rfc,s_fechasol,s_fechaaut,s_Producto,s_ProdDes,s_linea,
								s_status,s_stdesc,s_comentario,s_diacorte,s_divisa,s_ingreso,cCausaSol,vDescCausaSol,vdias_vigencia,
								iejecucion,s_Limit,v_CausaSitEsp,iEsCtaCap,iConsultaSP,vCantReg2,cSitEsp,cCuenta_eje,iFrecuencia,iDiaPago,NVL(cTelCasa,''),NVL(cTelOficina,''), NVL(cFactor_piso,'') WITH RESUME;
					END FOREACH;	
				ELSE
					LET vCantReg2 = 12;
				END IF;
...
--aqui
			...
			ELIF pTipo = 1 THEN
				...
				IF iConsultaSP = 1 THEN
					FOREACH
						--EXECUTE PROCEDURE "informix".envia_monitorsol_cjunk_ss(
						EXECUTE PROCEDURE "informix".envia_monitorsol_cjunk_ss_popup(
				...
				IF iConsultaSP = 2 OR vCantReg1 < 11 THEN
					...
					FOREACH
						--EXECUTE PROCEDURE "informix".envia_monitorsol_pp_ss_2(
						  EXECUTE PROCEDURE "informix".envia_monitorsol_pp_popup(
										  pEmpresa,pSucursal,sinicio,pNumCte,pNum_producto,pStatus_solicitud,11,pFechaIni,pFechaFin)
									INTO  cValRetorno,s_Mensaje_Retorno,s_numsol,s_numcte,s_nombre,s_rfc,s_fechasol,
										  s_fechaaut,s_Producto,s_ProdDes,s_linea,s_status,s_stdesc,s_comentario,s_diacorte,
										  s_divisa,s_ingreso,iEsCtaCap,cCuenta_eje,iFrecuencia,iDiaPago,vdias_vigencia,cCausaSol,vDescCausaSol
					...
					FOREACH
						...
						*/
						...
						RETURN  cValRetorno,s_numsol,s_numcte,s_nombre,s_rfc,s_fechasol,s_fechaaut,s_Producto,s_ProdDes,s_linea,s_status,
								...
								s_Limit,v_CausaSitEsp,iEsCtaCap,iConsultaSP,vCantRegPres,cSitEsp,cCuenta_eje,iFrecuencia,iDiaPago,NVL(cTelCasa,''),NVL(cTelOficina,''),NVL(cFactor_piso,'') WITH RESUME;
					END FOREACH;
				END IF;
					...
					IF 0 = 1 THEN
						FOREACH
							...
							LET vCantReg = vCantReg + 1;
								...
								RETURN  cValRetorno,s_numsol,s_numcte,s_nombre,s_rfc,s_fechasol,s_fechaaut,s_Producto,s_ProdDes,s_linea,
										s_status,s_stdesc,s_comentario,s_diacorte,s_divisa,s_ingreso,cCausaSol,vDescCausaSol,vdias_vigencia,
										iejecucion,s_Limit,v_CausaSitEsp,iEsCtaCap,iConsultaSP,vCantRegPres,cSitEsp,cCuenta_eje,iFrecuencia,iDiaPago,NVL(cTelCasa,''),NVL(cTelOficina,''), NVL(cFactor_piso,'') WITH RESUME;
