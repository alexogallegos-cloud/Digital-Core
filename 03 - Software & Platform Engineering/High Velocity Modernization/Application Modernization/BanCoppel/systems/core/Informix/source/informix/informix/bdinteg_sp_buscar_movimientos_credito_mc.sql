CREATE PROCEDURE "informix".sp_buscar_movimientos_credito_mc(p_sNumeroCuenta CHAR(30), p_sFechaInicial DATE, p_sFechaFinal DATE, p_sMonto MONEY(14,2), p_skip INT, p_sTarjeta CHAR(30), ids_transacciones LVARCHAR, p_sNumeroEmpresa CHAR(3))
	...
	--SET DEBUG FILE TO "/informix/sp_buscar_movimientos_credito_mc.out";
	--TRACE ON;
	...
	BEGIN
		...
		END EXCEPTION;
			...
			FOREACH       				
				...
				INTO resultado_horaMovimiento,resultado_fechaMovimiento, resultado_monto, resultado_folioSuc, resultado_reversado, resultado_infreceptor, p_secuenciaextendida
				FROM intercard:movimiento
					LEFT JOIN intercard:tarjetacuenta ON (intercard:tarjetacuenta.numtarjeta = intercard:movimiento.numtarjeta)
				WHERE intercard:tarjetacuenta.numcuenta = p_sNumeroCuenta
					...
					AND intercard:movimiento.numtarjeta = p_sTarjeta
					AND intercard:movimiento.formato = '0200'
					AND UPPER(movreversado) = 'F'
					AND intercard:movimiento.codigoiso = '00'
                    AND intercard:movimiento.metodocaptura IN ('01', '05')
					ORDER BY folio_suc ASC, fechahoraacentral ASC
					
					LET resultado_claveTipo = '6830';
				    LET resultado_tipo = 'COMPRA EN COMERCIO';
				    --LET resultado_fechaMovimiento = resultado_horaMovimiento;
				
				-- Obtener referencia23
				SELECT DISTINCT referencia23_325 INTO resultado_referencia23
				FROM intercard:movimiento mov
				LEFT JOIN bditarjeta:td_movimientos_conciliacion ON (bditarjeta:td_movimientos_conciliacion.secuencia_extendida = mov.secuenciaextendida and bditarjeta:td_movimientos_conciliacion.numtarjeta = mov.numtarjeta)
				WHERE mov.numtarjeta = p_sTarjeta
				AND bditarjeta:td_movimientos_conciliacion.archivo_origen in (SELECT valor FROM  bdinteg:si_param_tdc_coppelmc WHERE codigo_parametro = 3) --codigo_parametro 3 = VNC
				...
				LET res_fechaMovimiento_ret = resultado_fechaMovimiento;
				LET res_horaMovimiento_ret = resultado_horaMovimiento;
				
				RETURN resultado_fechaMovimiento, resultado_horaMovimiento, resultado_monto, resultado_folioSuc, resultado_sucursal, resultado_nombre, resultado_claveTipo, resultado_tipo, 
				resultado_referencia23, resultado_reversado, resultado_refComercio, res_fechaMovimiento_ret, res_horaMovimiento_ret WITH RESUME;
			END FOREACH;
		
	END;
END PROCEDURE
DOCUMENT
'Sp para busqueda de movimientos de Credito Coppel Masterd Card',
