CREATE PROCEDURE "informix".sp_montos_manc_aut_bei2(
												pNumCliente char(9),
												pIdUser integer,
												pNoReg integer,
												pRegIni integer)
  ...
  BEGIN
   ...
   ON EXCEPTION SET sql_err
      IF sql_err <> 0 THEN
            let cod_ret = sql_err;
            RETURN cod_ret, iTotalReg,iIdMancomunidad,sUserNom,sTipoOperacion;
      END IF ;
   END EXCEPTION ;
...
--**************************************************************************************************************
          ...
          FOREACH
            ...
            Select SKIP pRegIni FIRST pNoReg  distinct mt.id_admin_manco, replace(replace(replace (trim(dtus.nombre),' ','<>' ),'><','' ),'<>',' ' ), mm.restricc
            Into   iIdMancomunidad, sUserNom, sTipoOperacion
            ...
            Inner Join "informix".bei_datos_usuario dtus On(dtus.id_usuario = mt.id_usuario_admin)
           ...
           RETURN cod_ret, iTotalReg,iIdMancomunidad,SUBSTRING(sUserNom FROM 0 FOR 50),sTipoOperacion WITH RESUME;
          END FOREACH;
