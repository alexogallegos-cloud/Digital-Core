CREATE PROCEDURE "informix".sp_pago_diferido_edocta (pEmpresa CHAR(3), pNumCredito CHAR(20), pFechaEmision DATE, pNumRegistros SMALLINT)
RETURNING CHAR(6)       AS codigo_retorno,
          DATE          AS fecha_emision,
		  CHAR(20)      AS num_credito,
		  CHAR(50) AS concepto,
		  DECIMAL(18,2) AS tasa,
		  DECIMAL(18,2) AS saldo_pendiente,
		  CHAR(5)  AS num_cuota_plazo,
		  DECIMAL(18,2) AS monto_prox_pago,
          DATE          AS fecha_prox_cargo,
          INTEGER       AS id_credisol;
...
DEFINE dtFechaEmision   DATE;
DEFINE vNumCredito      VARCHAR(20,1);
DEFINE vConcepto        VARCHAR(50,1);
DEFINE dTasa            DECIMAL(18,2);
DEFINE dSdoPendiente    DECIMAL(18,2);
DEFINE vNumCuota        VARCHAR(5,1);
DEFINE dMontoProxPago   DECIMAL(18,2);
DEFINE dtProxFechaPago  DATE;
...
ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
    IF iSqlErr != 0 THEN
      LET cCodRet     = iSqlErr;
      RETURN NVL(cCodRet,''), NVL(dtFechaEmision,DATE(1)), NVL(vNumCredito,''), NVL(vConcepto,''), NVL(dTasa,0), NVL(dSdoPendiente,0), NVL(vNumCuota,''), NVL(dMontoProxPago,0), NVL(dtProxFechaPago,DATE(1)), NVL(iSecuencia,0);
    END IF;
END EXCEPTION;
...
FOREACH WITH HOLD
	...
	SELECT fecha_emision, num_credito, concepto, tasa, saldo_pendiente, numero_cuotas::integer||'/'||plazo::integer,
		   monto_prox_pago, prox_fecha_pago
	  INTO dtFechaEmision, vNumCredito, vConcepto, dTasa, dSdoPendiente, vNumCuota, dMontoProxPago, dtProxFechaPago
	...
	RETURN	NVL(cCodRet,''), NVL(dtFechaEmision,DATE(1)), NVL(vNumCredito,''), 
			NVL(vConcepto,''), NVL(dTasa,0), NVL(dSdoPendiente,0), NVL(vNumCuota,''), 
			NVL(dMontoProxPago,0), NVL(dtProxFechaPago,DATE(1)), NVL(iSecuencia,0) 
	WITH RESUME;
...
END PROCEDURE
DOCUMENT
'Se realiza procedimiento el esquema del pago diferido del estado de cuenta TDC',
'del crédito consultado',
'AUTOR : Paul Ivan Quintero Varela',
'FECHA : 08/07/2015',
'BD    : BDICRED',
'MODIFICO: EDGAR ALARCÓN',
'FECHA:13/07/2015',
'DESCRIPCION: SE MODIFICO PARA RETORNAR PAGOS DIFERIDOS PAGINADOS';
