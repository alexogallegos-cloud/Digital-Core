CREATE PROCEDURE "informix".sp_os_solicita_web(pempresa CHAR(3), pnum_solicitud CHAR(20), pusuario CHAR(8))
RETURNING  VARCHAR(5);
...
--	AND fecha_solicitud > DATE(1) --VALIDAR
    ...
    IF (SELECT tipo_solicitud FROM "informix".ss_solicitudes WHERE num_solicitud=pnum_solicitud AND status_solicitud='OA')="C" THEN
        ...
        IF (SELECT COUNT(num_solicitud) FROM "informix".ss_solicitud_os WHERE secuenciaos=iSecOS AND num_solicitud in
			...
			and num_solicitud <> pnum_solicitud and status_solicitud not in ('RT','AT','AP','CN'))) > 0
           THEN 
		   ...
		   -- ANALIZAR LA POSIBILIDAD DE REENVIAR LAS SOLIC RECHAZADAS, IDENTIFICANDO CUALES PROCEDEN.
		    IF vStatusSol NOT IN  ('CC', 'OA') THEN
		        IF vStatusSol IN ('EE', 'OS') THEN
		            LET P_COD_RET = '00001';  -- YA HAY UNA OS EN PROCESO ACTUALMENTE
		        ELSE
					IF vStatusSol IN ("AT","AP") AND pnum_solicitud = cSolCoppel THEN
					    --  IF NOT EXISTS (SELECT COUNT(num_solicitud) FROM "informix".ss_os_solautdirecta WHERE num_solicitud = pnum_solicitud) THEN
						IF (SELECT COUNT(num_solicitud) FROM "informix".ss_os_solautdirecta WHERE num_solicitud = pnum_solicitud) = 0 THEN
							LET P_COD_RET = '00002';  --Estatus actual no valido para solicitar OS
						ELSE
		    ...
		    IF iCuantos >0 THEN
		        LET P_COD_RET = '00003';  --YA HAY UNA SOLICITUD DE OS EN ESPERA DE SER PROCESADA.
		       --SI HUBIESE ALGUNA EN ESTATUS 'S',  SI ESTA EN S ES PORQUE NO HA LLEGADO A LA HORA DEL CRON, O PORQUE
		       --POR ALGUN MOTIVO ESTA TRONANDO LA INTEGRACION DE DATOS. PARA QUE METER OTRA???
		    END IF;
					...
					IF (NVL(dFecha_insert,'01/01/1900')) >= (NVL(dFecha_resp,'01/01/1900')) THEN
						...
						IF (cOservaciones <> cOservaciones_Act) OR (cEntre_calles <> cEntre_calles_Act)
						 ...
						 OR (cColonia <> cColonia_Act) OR (cCalle <> cCalle_Act) OR (cNumeroextcalle <> cNumeroextcalle_Act) OR (cNumerointcalle <>cNumerointcalle_Act) THEN
						 	LET P_COD_RET = '00000';
						ELSE 
							LET P_COD_RET = '00004';							LET iBand = 1;
							EXIT;						
						END IF;
					ELSE
						LET P_COD_RET = '00004';						LET iBand = 1;
						...
						IF vStatusSol = 'OA' THEN --RQM 09 474 -2
							IF (SELECT COUNT(numcte) FROM "informix".ss_solicitudes WHERE	numcte = cNumcte AND num_solicitud = pnum_solicitud) >0  THEN							
									...
									IF V_telefonocelular <> "" OR  V_telefonocelular IS NOT NULL THEN
										...
										IF CodRet_regev = '000000' THEN LET P_COD_RET = '00000'; END IF;	
									END IF;
							END IF;
						END IF;
				    END IF;
			...
			END IF;
			IF iSolEnviadasOS > 1  THEN
				LET P_COD_RET = '00000';  -- ESTATUS ACTUAL NO VALIDO PARA SOLICITAR OS
			END IF;
	END WHILE;
