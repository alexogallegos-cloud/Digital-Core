CREATE PROCEDURE "informix".insertar_udi(pvalor decimal(14,6))
RETURNING CHAR(5)        AS codigo_retorno,
   ...
   DEFINE vValorAnt	      DECIMAL(14,6); 
   DEFINE vValorDif1      DECIMAL(14,6);
   DEFINE vValorDif2      DECIMAL(14,6);
...
BEGIN
 ...
 --SET DEBUG FILE TO '/informix/Cony/valida_udi.out';
 --TRACE ON;
...
-- ****************************************************************************
      ...
      -- ******************************************
        SELECT fecha_hoy, fecha_ant INTO vFecha, vFechaAnt 
      ...
      --consultar valor de ayer
      SELECT  precio_compra INTO vValorAnt
      ...
      IF vValorAnt IS NULL THEN
        LET cod_ret = "002";
        LET cMensajeRet = 'No existe el valor de la Udi de ayer';
		RETURN cod_ret, cMensajeRet;
	  END IF;

      LET vValorDif1= vValorAnt - (NVL(vValorAnt,0) *  (10/100));
      LET vValorDif2= vValorAnt + (NVL(vValorAnt,0) *  (10/100));

      
      IF pvalor BETWEEN vValorDif1 AND vValorDif2 THEN        
          ...
          IF EXISTS(
             SELECT precio_compra 
                ...
                AND hora_tpcambio=(SELECT MAX(hora_tpcambio)
                                  ...
                                  AND fecha_tpcambio = vFecha
                                  AND clase_tpcambio = vClaseUdi)
                AND clase_tpcambio = vClaseUdi) THEN
                LET cod_ret = "001";
                LET cMensajeRet = 'Ya Existe el valor de la Udi para el dia de hoy';
                RETURN cod_ret, cMensajeRet;
          ELSE
                INSERT INTO bdinteg:"informix".si_tpcambio(empresa, divisa, fecha_tpcambio, hora_tpcambio, clase_tpcambio, precio_compra, pc_arriba, pc_abajo, precio_venta, pv_abajo, pv_arriba, variacion_cpa, variacion_vta, tipo_cpa_div_dll, tipo_cpa_mn_dll, tipo_cpa_mn_div, tipo_vta_div_dll, tipo_vta_mn_dll, tipo_vta_mn_div, precio_base, var_cpa_base, var_vta_base, ref_clase_tc, user_insert, fecha_insert)
                VALUES('001', '09', (EXTEND(MDY(MONTH(vFecha),DAY(vFecha), YEAR(vFecha)), YEAR to SECOND)),EXTEND(MDY(1,1,1970), YEAR to SECOND)+0 UNITS HOUR+0 UNITS MINUTE+0 UNITS SECOND, 'O', pvalor, 0.000000, 0.000000, pvalor, 0.000000, round(pvalor,2), 0, 0, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, NULL, NULL, NULL, NULL, 'informix', EXTEND(MDY(MONTH(today), DAY(today), YEAR(today)), YEAR to SECOND));
             	RETURN cod_ret, cMensajeRet;
          END IF;	
      ELSE
            LET cod_ret = "003";
            LET cMensajeRet = 'El valor introducido no es valido';
            RETURN cod_ret, cMensajeRet;
      END IF;
