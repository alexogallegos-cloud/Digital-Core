...
'FECHA: 27/01/2014',
'DESCRIPCION: Procedimiento para revaluar la solicitud de banco cuando se realiza la unificacion del cliente',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_cr_consultasemaforo(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumCte CHAR(20), pNumCred CHAR(20))
                RETURNING CHAR(5) AS codret,
						  CHAR(2) AS semaforo;            
                                
        DEFINE cCodRet CHAR(5);
        DEFINE iSqlErr INTEGER;       
		DEFINE cSemaforo CHAR(2);
		DEFINE cProd CHAR(4);
        ...
        BEGIN   
                ON EXCEPTION SET iSqlErr
                        LET cCodRet = iSqlErr;
                        RETURN cCodRet,cSemaforo;
                END EXCEPTION;
                
                -- SET DEBUG FILE TO '/tmp/mfinis/sp_cr_consultasemaforo.out';
                -- TRACE ON;
                
                IF pUsuario = '' OR pIdFuncion = '' OR pNumCte = '' OR pNumCred = '' THEN
                        LET cCodRet = '00003';
						RETURN cCodRet,cSemaforo;
                END IF;
				...
				SELECT FIRST 1 num_producto INTO cprod FROM bdicred:"informix".sd_maecred WHERE numcte= TRIM(pNumCte) and num_credito=TRIM(pNumCred);
				
				IF nvl(cprod,'') ='' THEN 
				
					LET cSemaforo ='-1';
				
				ELIF cprod IN ('6001','8100','7000','8500','5400') THEN 
				
				  --SE COMPRUEBA EL VALOR DEL SEMAFORO
                SELECT FIRST 1 semaforo INTO cSemaforo  FROM  bdicred:"informix".sd_ctes_retencion WHERE numcte= TRIM(pNumCte) and num_credito=TRIM(pNumCred);
					...
					IF cSemaforo NOT IN ('1','3','5') THEN 
						LET cSemaforo ='5';
						RETURN cCodRet,cSemaforo;
					END IF;
				ELSE 
...
'FECHA: 29/10/2021',
'DESCRIPCION: Sp encargado de recuperar el valor semáforo de la tabla sd_ctes_retencion',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_pagomasivocre(pUsuario CHAR(8), pIdFuncion CHAR(10), pLote INT, pPlantilla CHAR(25), pTituloPlantilla CHAR(255))
	RETURNING CHAR(5) AS codret,
	...
	DEFINE cTransaccionPago	CHAR(4);
	...
	--SET DEBUG FILE TO '/ifxsif01/roman/ambientacion/TDC_INFINITE/Spl/sp_pagomasivocre.out';
	--TRACE ON;
		...
		-- Se genera el folio
		SELECT FIRST 1 'pagmas'||TO_CHAR(CURRENT, '%d%m%H%M%S')
		INTO cFolioGrupo
		FROM bdicnweb:sw_tr_cargamasiva_pago
		WHERE lote = pLote;
		...
		FOREACH WITH HOLD SELECT id_registro, cuenta, monto_importe, transaccion, descripcion1
			...
			---obtenemos numero de producto del credito
			FOREACH
			SELECT num_producto 
			INTO cProducto
			FROM bdicred:sd_maecred 
			WHERE num_credito = cCredito
			UNION
			SELECT num_producto
			...
			---obtenemos conceptos del pago
			IF cProducto in ('6001', '6600', '7000','8100','8500','5400') THEN
			SELECT transacc, cod_fun
			INTO cTransaccionPago, cCodFuncion
			FROM bdicred:sd_conceptospagomanual
			WHERE codigo = cTransaccion;
			ELSE
				SELECT transacc, cod_fun
				INTO cTransaccionPago, cCodFuncion
			...
			FOREACH EXECUTE PROCEDURE bdicred:"informix".sp_obtenertransaccionpagosmanuales(TRIM(cTransaccion), TRIM(cTransaccionPago), TRIM(cCodFuncion), cCredito, cProducto)
				INTO cCodRetSp, cCodPago, cDescPago, cTransaccion, cCodFuncion
			END FOREACH;
			
			IF cCodRetSp::INTEGER = 0  THEN

				EXECUTE PROCEDURE bdicred:"informix".sp_grabarpagosmasivos(cEmpresa, vUsuario, cFolioGrupo, cCredito, NVL(mImportePago,0), cTransaccion, cCodPago, cDescPago, TRIM(cConcepto))
		...
		BEGIN;
			INSERT INTO bdicnweb:sw_tr_cargamasiva_pago_hist(id_registro, id_funcion, fecha_carga, lote, archivo, usuario, fecha_proceso, status,
			...
			SELECT id_registro, id_funcion, fecha_carga, lote, archivo, usuario, fecha_proceso, status,
					resultado, codret_proceso, motivo_rechazo, cuenta, numcte, monto_importe, folio, transaccion,
					indicador1, indicador2, indicador3, descripcion1, descripcion2, monto1, monto2, monto3
			FROM bdicnweb:sw_tr_cargamasiva_pago WHERE lote = pLote AND status = 'S';
						
			DELETE FROM bdicnweb:sw_tr_cargamasiva_pago WHERE lote = pLote AND status = 'S';
		COMMIT;
		...
		-- NotificaciÃÂ³n de correo electronico
...
DOCUMENT "AUTOR: Oscar Flores Conde",
"FECHA: 09/08/2013",
"DESCRIPCION: Realiza el pago masivo de cuentas de crÃÂ©dito de la aplicaciÃÂ³n CNWEB";

CREATE PROCEDURE "informix".sp_sw_ro_consultaperiodosedoscta(pUsuario CHAR(8), pIdFuncion CHAR(10), pCuenta CHAR(20), pSistemaCuenta CHAR(2), pNumCliente CHAR(20), pFechaInicial DATE, pFechaFinal DATE, pTipoOperacion SMALLINT)
    RETURNING CHAR(5) AS codret,
        CHAR(6) AS periodo,
        DATE AS fechaInicio_Periodo,
        DATE AS fechaFin_Periodo;
    ...
    DEFINE iAnioMesFin INTEGER;
	DEFINE iFecha1Valida SMALLINT;
	DEFINE iFecha2Valida SMALLINT;
	DEFINE iSqlErr INTEGER;
    ...
    LET dFechaFin = NULL;
    LET iDiaCorte = 0;
    LET iAnioMesInicio = TO_CHAR(pFechaInicial, '%Y%m')::INTEGER;
    LET iAnioMesFin = TO_CHAR(pFechaFinal, '%Y%m')::INTEGER;
	LET iFecha1Valida = 0;
	LET iFecha2Valida = 0;
	LET iSqlErr = 0;
	LET cNumProducto = '';
    ...
    BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cPeriodo, dFechaInicio, dFechaFin;
		END EXCEPTION;
   
        --SET DEBUG FILE TO '/informix/VH/sp_sw_ro_consultaperiodosedoscta.out';
        --TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pCuenta = '' OR pNumCliente = '' OR pSistemaCuenta = '' OR 
			pFechaInicial IS NULL OR pFechaFinal IS NULL OR pTipoOperacion IS NULL THEN
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cPeriodo, dFechaInicio, dFechaFin;
		END IF;
		...
		ELIF pSistemaCuenta = '06' THEN
			...
			/*Fecha: 15-05-2018 
			  RQM 06 590
			  Modificacion: Se contempra productos Oro y Platino 
			  y se quita el limit para la obtencion del producto, se agrega TDC GP*/
		
			IF cNumProducto NOT IN ('6001','6600','6400','8100','7000','8500','5400') THEN
			...
			IF cNumProducto IN ('6001','6600','6400') THEN
                LET iDiaCorte='20';
            ELIF cNumProducto IN ('6300','6800', '7600', '7700') THEN
                LET iDiaCorte = SUBSTR(dFechaApertura,4,2);
            ELIF cNumProducto IN ('6011','8600') THEN
                LET iDiaCorte = SUBSTR(dFechaApertura,4,2);
                IF iDiaCorte>=3 AND iDiaCorte<17 THEN
                    LET iDiaCorte=2;
                ELIF iDiaCorte IN (1,2) OR iDiaCorte>=17 THEN
                    LET iDiaCorte=17;
                ELSE
                    LET iDiaCorte=0;
                END IF;
			ELIF cNumProducto IN ('7000','8100','8500','5400') THEN    
				SELECT dia_cuota 
				INTO iDiaCorte
				FROM bdicred:sd_definicion
				WHERE empresa ='001' AND num_producto = cNumProducto;
            END IF;
		END IF;

		IF pTipoOperacion = 1 THEN
			IF pSistemaCuenta = '01' THEN
				IF cTpoPersona <> '01' THEN
				LET dFechaInicio = pFechaInicial;

				LET dFechaFin = MDY(MONTH(pFechaFinal), DAY(LAST_DAY(pFechaFinal)), YEAR(pFechaFinal));
				...
				ELSE
					BEGIN
						ON EXCEPTION IN (-1206)
								IF iFecha1Valida = 0 THEN
									LET dFechaInicio = ADD_MONTHS(pFechaInicial, 1);
									LET dFechaInicio = MDY(MONTH(dFechaInicio), 1, YEAR(dFechaInicio));
								END IF;
								
								IF iFecha2Valida = 0 THEN
									LET dFechaFin = MDY(MONTH(dFechaFin), DAY(LAST_DAY(dFechaFin)), YEAR(dFechaFin));
								END IF;
						END EXCEPTION WITH RESUME;
						
						LET dFechaInicio = MDY(MONTH(pFechaInicial), iDiaCorte, YEAR(pFechaInicial));
						LET iFecha1Valida = 1;
						...
						IF iDiaCorte = 1 THEN
							...
							LET iFecha2Valida = 1;
						ELSE
							LET dFechaFin = ADD_MONTHS(pFechaFinal, 1); -- Aumento de un mes a la decha original
							LET dFechaFin = MDY(MONTH(dFechaFin), iDiaCorte - 1, YEAR(dFechaFin));
							LET iFecha2Valida = 1;
						END IF;
				...
				END IF;
			ELIF pSistemaCuenta = '06' THEN
				LET dFechaInicio = DATE(pFechaInicial - 1 UNITS MONTH); -- Resta de un mes a la fecha original
				LET dFechaInicio = MDY(MONTH(dFechaInicio), iDiaCorte + 1, YEAR(dFechaInicio));
				LET iFecha1Valida = 1;
				
				LET dFechaFin = MDY(MONTH(pFechaFinal), iDiaCorte, YEAR(pFechaFinal));
				LET iFecha2Valida = 1;
		...
		ELIF pTipoOperacion = 2 THEN
			IF pSistemaCuenta = '01' THEN
				...
				WHILE iAnioMesInicio <= iAnioMesFin
					ON EXCEPTION IN (-1206)
						IF iFecha1Valida = 0 THEN
							LET dFechaInicio = ADD_MONTHS(pFechaInicial, 1);
							LET dFechaInicio = MDY(MONTH(dFechaInicio), 1, YEAR(dFechaInicio));
					...
					IF iDiaCorte = 1 THEN
						LET dFechaFin = MDY(MONTH(pFechaInicial), DAY(LAST_DAY(pFechaInicial)), YEAR(pFechaInicial));
						LET iFecha2Valida = 1;
					ELSE
						LET dFechaFin = ADD_MONTHS(pFechaInicial, 1); -- Aumento de un mes a la decha original
						LET dFechaFin = MDY(MONTH(dFechaFin), iDiaCorte - 1, YEAR(dFechaFin));
						LET iFecha2Valida = 1;
					END IF;

					LET cPeriodo = TO_CHAR(pFechaInicial, '%Y%m');
					RETURN cCodRet, cPeriodo, dFechaInicio, dFechaFin WITH RESUME;

					LET pFechaInicial = ADD_MONTHS(pFechaInicial, 1);
					LET iAnioMesInicio = TO_CHAR(pFechaInicial, '%Y%m')::INTEGER;
					LET iFecha1Valida = 0;
					LET iFecha2Valida = 0;
				END WHILE;
			...
			ELIF pSistemaCuenta = '06' THEN
				WHILE iAnioMesInicio <= iAnioMesFin
					...
					LET dFechaFin = MDY(MONTH(pFechaInicial), iDiaCorte, YEAR(pFechaInicial));
					LET iFecha2Valida = 1;
...
DOCUMENT 'AUTOR: Oscar Flores Conde', 
'FECHA: 10/02/2014',
'DESCRIPCION: Consulta la fecha inicial y final de corte por cada periodo consultado para los estados de cuenta, por el momento solo abarca captacion',
'FECHA: 01/03/2021',
'AUTOR: Sandra Cano',
'DESCRIPCION:Se agrega validación a producto 6800 para obtener el día corte.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_verifica_estatus_conveniossac(pUsuario CHAR(8), pIdfuncion CHAR(10))
	RETURNING CHAR(5) AS codret,
		CHAR(1) AS status,
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_verifica_estatus_conveniossac.out';
		--TRACE ON;
        ...
        SELECT status, error_proceso, error, NVL(registros, 0) 
        INTO   cStatus, cErrorProceso, cError, iRegistros
        FROM "informix".sw_verificastatusconveniosac WHERE usuario_insert = pUsuario;
...
'FECHA: 06/11/2023',
'MODULO: OPERACIONES',
'FUNCIONALIDAD: VerificaciÃ³n de Estatus para los reportes de convenios',
'DESCRIPCION: SPL encargado verificar el status del proceso en la generaciÃ³n de reportes';
...
RETURNING CHAR(6) 		AS cod_ret,
	...
	--- SET DEBUG FILE TO '/respaldosbd/has/sp_cce_consultar_cheques60.out';
	--- TRACE ON;
    ...
    FOREACH WITH HOLD
        SELECT det.cvebanco, det.numcuenta, det.numcheque, mae.num_cte, doc.cuenta, doc.monto_ori, doc.sucursal
         ...
         WHERE mae.cuenta = doc.cuenta
           ...
           AND doc.numcuenta::INT8 = det.numcuenta::INT8
           AND doc.num_chq = det.numcheque::INTEGER
           AND doc.cancelado = 'T'
    ...
    FOREACH	WITH HOLD 
        SELECT ba.banco, ba.descripcion, doc.referencia, doc.num_chq, doc.monto_ori,
         ...
         WHERE doc.empresa = pEmpresa
           ...
           AND doc.numcuenta::INT8 = ccecr.numcuenta::INT8
...
END PROCEDURE 
DOCUMENT 
'DESCRIPCION: Proceso para obtener los datos de los cheques del código 60', 
'BD: bdicheq', 
...
pPesos MONEY(16,2), pUsd MONEY(16,2), pStatus SMALLINT, pLimitecte INTEGER)
		RETURNING CHAR(5) AS codret;
...
'FECHA: 18/12/2017',
'MODULO: OPERACIONES',
'FUNCIONALIDAD: MODIFICACIï¿½N DE Lï¿½MITE DE REMESAS',
'DESCRIPCION: SPL encargado de guardar/actualizar el valor del lï¿½mite en UDS y PESOS y el status, dependiendo del tipo de remesadora y estado o sucursal seleccionados.',
'AUTOR: ING. JOSÃ ANTONIO RAMÃREZ FRANCO',
'FECHA: 23/06/2023',
'DESCRIPCION: Se le aÃ±adio un nuevo campo llamado -limite cliente- como en parametro y en las operaciones Guardar/Actualizar',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_rem_conslimiteremesa_edo_suc(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdConsulta CHAR(1), pIdRemesadora SMALLINT, pValor CHAR(4))
		RETURNING CHAR(5) 	AS codret,
			MONEY(16,2) 	AS usd,
			MONEY(16,2) 	AS pesos,
			SMALLINT 		AS status,
	...
	BEGIN
		...
		IF pUsuario = '' OR pIdFuncion = '' OR pIdConsulta = '' OR pIdRemesadora IS NULL OR pValor = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, mUsd, mPesos, sStatus, iLimiteCte;
		END IF;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: MODIFICACIï¿½N DE Lï¿½MITE DE REMESAS',
'DESCRIPCION: SPL encargado de consultar el valor del lï¿½mite en UDS y PESOS y el status, dependiendo del tipo de remesadora y estado o sucursal seleccionados.',
'AUTOR: ING. JosÃ© Antonio RamÃ­rez Franco',
'FECHA: 22/06/2023',
...
CREATE PROCEDURE "informix".sp_rem_limitemaximomes_ope(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdConsulta CHAR(1),  pValor MONEY(16,2))
		RETURNING CHAR(5) 	AS codret,
			MONEY(16,2) 	AS pesos;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, mPesos;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_rem_limitemaximomes_ope.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pIdConsulta = ''  OR pValor = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, mPesos;
		END IF;
		...
		--CONSULTA DE LIMITE MONTO DE TODAS LAS REMESADORAS
		IF pIdConsulta = '1' THEN 
		
			SELECT pesos 
            INTO  mPesos
            FROM bdisac:"informix".sac_limite_monto
            WHERE abreviatura = 'TODAS_';
		...
		ELIF pIdConsulta = '2' THEN 
			...
			UPDATE bdisac:"informix".sac_limite_monto SET pesos = pValor
            WHERE abreviatura = 'TODAS_';
...
'FECHA: 20/10/2023',
'FUNCIONALIDAD: MODIFICACION DE LIMITE DE REMESAS',
'DESCRIPCION: SP Encargado de Consultar/Actualizar el limite maximo al mes de todas las remesadoras',
'MODULO: OPERACIONES',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_rem_mostrarpaises_totales(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdRemesadora SMALLINT, pIdConsulta INTEGER, pRegistros INTEGER, pRecuperacion INTEGER)
    RETURNING CHAR(5)  AS codret,
              CHAR(3)  AS id_pais,
              CHAR(20) AS nombre_pais;

    --DECLARACÃON DE VARIABLES
    DEFINE cCodRet      CHAR(5);
	DEFINE iSqlErr      INTEGER;
	DEFINE iNoRegistros INTEGER;
    DEFINE cIdPais      CHAR(3);
    DEFINE cNombrePais  CHAR(20);
    ...
    BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_rem_mostrarpaises_totales.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pIdRemesadora = '' OR pIdConsulta = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cIdPais, cNombrePais;
		END IF;
        ...
        --CONSULTA TODOS LOS PAISES DE UNA REMESADORA
        IF pIdConsulta = '1' THEN
            IF pIdRemesadora <> '6' THEN 
                FOREACH 
                --OBTENEMOS TODOS LOS PAISES OMITIENDO LOS PERMITIDOS
                SELECT SKIP pRegistros FIRST pRecuperacion pais, nombre
                INTO cIdPais, cNombrePais  
                FROM bdinteg:"informix".si_paises
                WHERE pais NOT IN (SELECT id_pais FROM bdinteg:"informix".si_paises_remesadoras WHERE id_remesadora = pIdRemesadora)
                ORDER BY nombre ASC
                ...
                END FOREACH;
            --MOSTRAMOS TODOS LOS PAISES OMITIENDO LOS PERMITIDOS EN COMÃN DE LAS REMESADORAS BTS|APIZA|WU
            ELIF pIdRemesadora = '6' THEN 

                FOREACH
				    
                    SELECT SKIP pRegistros FIRST pRecuperacion p.pais, p.nombre
                    INTO cIdPais, cNombrePais 
                    FROM bdinteg:"informix".si_paises p
                    WHERE NOT EXISTS (
                    SELECT 1
                        FROM bdinteg:"informix".si_paises_remesadoras AS spr
                        INNER JOIN "informix".sw_rem_remesadora AS sr ON spr.id_remesadora = sr.id_remesadora
                        WHERE sr.id_remesadora IN (1, 2, 3) AND spr.id_pais = p.pais
                        GROUP BY spr.id_pais
                        HAVING COUNT(DISTINCT sr.id_remesadora) = 3
                    )
        ...
        --CONSULTA TODOS LOS PAISES EN COMÃN DE LAS REMESADORA PARA LA OPCIÃN TODAS
        ELIF pIdConsulta = '2' THEN
            FOREACH
                
				SELECT SKIP pRegistros FIRST pRecuperacion sp.pais, sp.nombre
				INTO cIdPais, cNombrePais
				FROM bdinteg:"informix".si_paises sp
				WHERE sp.pais NOT IN ( SELECT r.id_pais FROM bdinteg:"informix".si_paises_remesadoras r GROUP BY r.id_pais
    				                    HAVING COUNT(DISTINCT r.id_remesadora) = ( SELECT COUNT(*) FROM "informix".sw_rem_remesadora WHERE  grupo_remesadora IN ('A', 'B'))
				)
                ORDER BY sp.nombre
...
'FECHA: 28/08/2023',
'MODULO: OPERACIONES',
'FUNCIONALIDAD: PAÃSES PERMITIDOS',
'DESCRIPCION: SPL encargado de consultar todos los paises del catalogo de si_paises omitiendo aquellos que ya estan registrados en la tabla de si_paises_remesadoras (paises permitidos)',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_rem_mostrarpaisesremesadora(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdRemesadora SMALLINT, pRegistros INTEGER, pRecuperacion INTEGER)
    RETURNING CHAR(5)  AS codret,
              CHAR(3)  AS id_pais,
              CHAR(20) AS nombre_pais;
    ...
    DEFINE cIdPais      CHAR(3);
    DEFINE cNombrePais  CHAR(20);
    DEFINE iIdRemesa    SMALLINT;
    DEFINE cRemesadora  CHAR(100);

	DEFINE iIdRemesadora  SMALLINT;
    ...
    BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_rem_mostrarpaisesremesadora.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pIdRemesadora = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cIdPais, cNombrePais;
		END IF;
		...
		--MOSTRAR TODOS LOS PAISES PERMITIDOS DE UNA REMESADORA EN especifico	
		IF pIdRemesadora > 0 AND pIdRemesadora <> '6' THEN
			FOREACH 
		
				SELECT SKIP pRegistros FIRST pRecuperacion sp.pais, sp.nombre
				INTO cIdPais, cNombrePais
				FROM bdinteg:"informix".si_paises sp
                WHERE pais IN (SELECT id_pais FROM bdinteg:"informix".si_paises_remesadoras WHERE id_remesadora = pIdRemesadora)
                ORDER BY nombre ASC
				...
				RETURN cCodRet, cIdPais, cNombrePais WITH RESUME;
			END FOREACH;
		--MOSTRAMOS TODOS LOS PAISES PERMITIDOS EN COMÃN DE LAS REMESADORAS BTS|APIZA|WU
		ELIF pIdRemesadora = '6' THEN
			FOREACH 

				SELECT SKIP pRegistros FIRST pRecuperacion spr.id_pais, sp.nombre
				INTO cIdPais, cNombrePais
				FROM bdinteg:"informix".si_paises_remesadoras AS spr
				INNER JOIN "informix".sw_rem_remesadora AS sr ON spr.id_remesadora = sr.id_remesadora
				INNER JOIN bdinteg:"informix".si_paises AS sp ON spr.id_pais = sp.pais
				WHERE sr.id_remesadora IN (1, 2, 3)
				GROUP BY spr.id_pais, sp.nombre
				HAVING COUNT(DISTINCT sr.id_remesadora) = 3
				ORDER BY sp.nombre
		...
		--MOSTRAR TODOS LOS PAISES PERMITIDOS DE  TODAS LAS REMESADORAS
		ELIF pIdRemesadora = 0 THEN 
			...
			FOREACH 
				...
				INTO cIdPais, cNombrePais
				FROM bdinteg:"informix".si_paises sp
				WHERE sp.pais IN ( SELECT r.id_pais FROM bdinteg:"informix".si_paises_remesadoras r GROUP BY r.id_pais
    								HAVING COUNT(DISTINCT r.id_remesadora) = ( SELECT COUNT(*) FROM "informix".sw_rem_remesadora WHERE grupo_remesadora IN ('A','B'))
				)
				ORDER BY sp.nombre
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: PAÃSES PERMITIDOS',
'DESCRIPCION: SPL encargado de consultar todos los paises permitidos de una remesedora en especifico.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_rem_ope_paisesremesadoras(pUsuario CHAR(8), pIdFuncion CHAR(10), pIdRemesadora SMALLINT, pTramPaises CHAR(900), pIdConsulta INTEGER, pRegistros INTEGER)
    RETURNING CHAR(5)  AS codret;
    ...
    DEFINE cAccion      SMALLINT;   --1 agregar 2 borrar
    DEFINE cDescAccion   CHAR(100);
    DEFINE cRemesadora   CHAR(100);
    DEFINE iIdRemesadora SMALLINT;
	DEFINE icount 		 INTEGER;
	DEFINE cMensaje 	 CHAR(25);
	
	DEFINE iP1 INTEGER;
	DEFINE iContador INTEGER;
	DEFINE iPosRegistro INTEGER;
	DEFINE iLonRegistro INTEGER;
    DEFINE cRegistro CHAR(180);
    DEFINE cPais  CHAR(3);
    --INICIALIZACIÃN
    LET cCodRet = '00000';
	LET iSqlErr = 0;
    ...
    BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_rem_ope_paisesremesadoras.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pIdRemesadora = '' OR pIdConsulta = '' OR pTramPaises = '' OR pTramPaises IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF;
		...
		--ELIMINAR
		IF pIdConsulta = '1' AND pRegistros > 0 THEN
			
			IF pRegistros = 1 THEN
				LET pTramPaises = TRIM(pTramPaises) || ',';
			END IF;
			
			FOR iContador = 1 TO pRegistros
				IF iContador = 1 THEN
					LET iPosRegistro = iContador;
					LET iLonRegistro = (INSTR(pTramPaises,",",1,iContador)) - 1;
				ELIF iContador > 1 THEN
					LET iPosRegistro = INSTR(pTramPaises,",",1,iContador - 1) + 1;
					IF iContador > 1 AND iContador < pRegistros THEN 
						LET iLonRegistro = (INSTR(pTramPaises,",",1,iContador) - INSTR(pTramPaises,",",1,iContador - 1))-1;                               
					ELSE
						LET iLonRegistro = LENGTH(pTramPaises) - INSTR(pTramPaises,",",1,iContador - 1);
					END IF;
				END IF;
				LET cRegistro = SUBSTRING(pTramPaises FROM iPosRegistro FOR iLonRegistro);
				IF cRegistro <> '' THEN
					
					LET cPais = TRIM(cRegistro);
					IF pIdRemesadora > 0  AND pIdRemesadora <> '6' THEN
						-- Verificamos si el id_pais y el id_remesadora existen en la tabla
						SELECT COUNT(*) INTO icount
						FROM bdinteg:"informix".si_paises_remesadoras
						WHERE id_pais = cPais AND id_remesadora = pIdRemesadora;
        
						-- Si existe, eliminamos el registro
						IF icount > 0 THEN
							DELETE FROM bdinteg:"informix".si_paises_remesadoras WHERE id_pais = cPais AND id_remesadora = pIdRemesadora;
							LET cDescAccion = 'Eliminacion del pais permitido: ' || TRIM(cNombrePais) || ' a remesadora: ' || TRIM(cRemesadora);

							--INSERTAMOS EN BITACORA
							INSERT INTO bdicnweb:"informix".sw_bitacora_paises_remesadoras(hora_insert, user_insert, accion, des_accion, id_pais, id_remesadora)
							VALUES( dhora, pUsuario, '2', cDescAccion, cPais, pIdRemesadora);
						END IF;
					...
					ELIF pIdRemesadora = '6' THEN
						...
						FOREACH 
							...
							--ITERAMOS LAS REMESADORAS BTS | APIZA | WU
							SELECT id_remesadora, desc_remesadora
							INTO iIdRemesadora, cRemesadora
							FROM bdicnweb:"informix".sw_rem_remesadora
							WHERE grupo_remesadora IN ('A')

							--Verificamos que el nuevo pais a agregar no exista 
							SELECT COUNT(*) INTO icount
							FROM bdinteg:"informix".si_paises_remesadoras
							WHERE id_pais = cPais AND id_remesadora = iIdRemesadora;
					
							--Si existe lo Eliminamos
							IF icount > 0 THEN
								DELETE FROM bdinteg:"informix".si_paises_remesadoras WHERE id_pais = cPais AND id_remesadora = iIdRemesadora;
								LET cDescAccion = 'Eliminacion del pais permitido: ' || TRIM(cNombrePais) || ' a remesadora: ' || TRIM(cRemesadora); 
							
								--INSERTAMOS EN BITACORA
								INSERT INTO bdicnweb:"informix".sw_bitacora_paises_remesadoras(hora_insert, user_insert, accion, des_accion, id_pais, id_remesadora)
								VALUES(dhora, pUsuario, '2', cDescAccion, cPais, iIdRemesadora);
							END IF;
							LET iNoRegistros = iNoRegistros + 1;
						END FOREACH;
					ELIF pIdRemesadora = 0 THEN
						...
						FOREACH 
							...
							--ITERAMOS TODAS LAS REMESADORAS
							SELECT id_remesadora, desc_remesadora
							INTO iIdRemesadora, cRemesadora
							FROM bdicnweb:"informix".sw_rem_remesadora
			...
			--INSERTAR
		ELIF pIdConsulta = '2' AND pRegistros > 0 THEN
			...
			FOR iContador = 1 TO pRegistros
				...
				IF cRegistro <> '' THEN
					...
					-- Verificamos si el id_pais y el id_remesadora existen en la tabla
						...
						IF icount = 0 THEN
							INSERT INTO bdinteg:"informix".si_paises_remesadoras (id_pais, id_remesadora, fecha_insert)
							VALUES (cPais, pIdRemesadora, TODAY);
							...
							SELECT desc_remesadora
							INTO  cRemesadora
							FROM bdicnweb:"informix".sw_rem_remesadora
							WHERE id_remesadora = pIdRemesadora;
						
							LET cDescAccion = 'Insercion del pais permitido: ' || TRIM(cNombrePais) || ' a remesadora: ' || TRIM(cRemesadora);  
                
							--INSERTAMOS EN BITACORA
							INSERT INTO bdicnweb:"informix".sw_bitacora_paises_remesadoras(hora_insert, user_insert, accion, des_accion, id_pais, id_remesadora)
							VALUES(dhora, pUsuario, '1', cDescAccion, cPais, pIdRemesadora);
					...
					ELIF pIdRemesadora = '6' THEN
						...
						FOREACH 
							...
							--ITERAMOS LAS REMESADORAS APRIZA | WU | BTS
							SELECT id_remesadora, desc_remesadora
							INTO iIdRemesadora, cRemesadora
							FROM bdicnweb:"informix".sw_rem_remesadora
							WHERE grupo_remesadora IN ('A')
							...
							IF icount = 0 THEN
								INSERT INTO bdinteg:"informix".si_paises_remesadoras (id_pais, id_remesadora, fecha_insert)
								VALUES (cPais, iIdRemesadora, TODAY);
								...
								--INSERTAMOS EN BITACORA
								INSERT INTO bdicnweb:"informix".sw_bitacora_paises_remesadoras(hora_insert, user_insert, accion, des_accion, id_pais, id_remesadora)
								VALUES(dhora, pUsuario, '1', cDescAccion, cPais, iIdRemesadora);
							END IF;
							LET iNoRegistros = iNoRegistros + 1;
						END FOREACH;
...
'MODULO: OPERACIONES',
'FUNCIONALIDAD: PAÃSES PERMITIDOS',
'DESCRIPCION: SPL encargado de realizar la alta y baja de los paises permitidos segÃºn la remesadora y paÃ­s, asi como Registrarlo en la bitacora.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consultainfoctemoral(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumcte CHAR(20))
		RETURNING CHAR(5) 		AS codret,
				  CHAR(13) 		AS RFC,
				  ...
				  DATE          AS FECHAINS_PD,
				  CHAR(100)      AS EMAIL_PM,
				  CHAR(30)      AS FOLIO_MERCAN,
				  CHAR(30)      AS CD_FOLIOMERCA,
				  CHAR(50)      AS USUARIO_AUTORIZADO1,
				  CHAR(50)      AS USUARIO_AUTORIZADO2,
				  CHAR(12) 		AS FOL_CONTRATO1,
				  CHAR(12) 		AS FOL_CONTRATO2,
				  CHAR(2)       AS CODIGO_IDENTIF1,	
				  CHAR(2)       AS CODIGO_IDENTIF2,
				  CHAR(50)      AS IDENTIF_DESCRIPCION1,
				  CHAR(50)      AS IDENTIF_DESCRIPCION2,
				  CHAR(30)      AS NOIDENTIF_OFICIAL1,
				  CHAR(30)      AS NOIDENTIF_OFICIAL2,
				  INTEGER       AS NUM_TOKENS,
	...
	DEFINE iNumAdmon                    INTEGER;
	DEFINE cUsuarioAut1                 CHAR(50);
	DEFINE cUsuarioAut2                 CHAR(50);
	DEFINE cFolContrato1   				CHAR(12);
	DEFINE cFolContrato2   				CHAR(12);
	DEFINE cCodIndentif                 CHAR(2);
	DEFINE cDescripcion                 CHAR(50);
	DEFINE cCodIndentifusu1             CHAR(2);
	DEFINE cCodIndentifusu2             CHAR(2);
	DEFINE cDescUsu1                    CHAR(50);
	DEFINE cDescUsu2                    CHAR(50);
	DEFINE cNumIdentUsuar1              CHAR(30);
	DEFINE cNumIdentUsuar2              CHAR(30);
	DEFINE iContToken                   INTEGER;
	...
	DEFINE cPrmTpopersona               CHAR(5);
	DEFINE cValorTpopersona             CHAR(1);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			...
			RETURN cCodRet, TRIM(NVL(cRFC,'')),TRIM(NVL(cApellPaterContactoRepLeg,'')),TRIM(NVL(cApellMaterContactoRepLeg,'')),TRIM(NVL(cNomb1ContactoRepLeg,'')),
						...
						TRIM(NVL(cNomContacto,'')),TRIM(NVL(cSufijo,'')),TRIM(NVL(cDescSufi,'')),TRIM(NVL(cEscritura,'')),TRIM(NVL(cNombreNot,'')),TRIM(NVL(cNumNot,'')),TRIM(NVL(cCdNotarioct,'')),TRIM(NVL(cDesCdNot,'')),TRIM(NVL(cEscrituraPod,'')),TRIM(NVL(cNomNotariopd,'')),TRIM(NVL(cNumNotariopd,'')),
						TRIM(NVL(cCdNotariopd,'')),TRIM(NVL(cDesCdNotpd,'')),TRIM(NVL(cNombreSoc,'')),NVL(dFechaInspd,DATE(1)),TRIM(NVL(cEmailpm,'')),
						TRIM(NVL(cCdfoliomerct,'')),TRIM(NVL(cNumfoliomerct,'')),TRIM(NVL(cUsuarioAut1, '')),TRIM(NVL(cUsuarioAut2, '')),TRIM(NVL(cFolContrato1,'')),
						TRIM(NVL(cFolContrato2,'')),TRIM(NVL(cCodIndentifusu1,'')),TRIM(NVL(cCodIndentifusu2,'')),TRIM(NVL(cDescUsu1,'')),TRIM(NVL(cDescUsu2,'')),
						TRIM(NVL(cNumIdentUsuar1,'')),TRIM(NVL(cNumIdentUsuar2,'')),NVL(iContToken,0), TRIM(NVL(cTpoPersona,'')),TRIM(NVL(cNumcteapoder,'')),
						TRIM(NVL(cNomapoder,'')),TRIM(NVL(cDocConst,'')),TRIM(NVL(cSucursal,'')),NVL(dFechaAlta,DATE(1)),NVL(iNumAdmon,0),TRIM(NVL(cPoder,'')),
						TRIM(NVL(cAdmin,'')),TRIM(NVL(cOrg,'')),TRIM(NVL(cNombreSucursal,'')), TRIM(NVL(cValorTpopersona,'')), TRIM(NVL(cRazonSocial, '')),cCURP,cRFCAlt,TRIM(cRegimenFiscal);
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/Antonio/sp_consultainfoctemoral.out';
		--TRACE ON;
		...
		EXECUTE PROCEDURE bdinteg:"informix".sp_consultarctemoral_03(pNumcte)
		INTO cCodRetSp, cRFC, cApellPaterContactoRepLeg, cApellMaterContactoRepLeg, cNomb1ContactoRepLeg,
			            cNomb2ContactoRepLeg , cCalleFiscal, cNumExtCalleFiscal, cColFiscal,
						vNomCiudFiscal, cCodMunFiscal, cNomEstadoFiscal, cNumcte, cNomCorto,
						cPagInternet, cSatFea, cTelContacto, cGiro, cNomGiro,
						cActividadSoc, cDesActObj, cRespStatus, cApellPaterFirmantes,
						cApellMaterFirmantes, cNomb1Firmantes, cNomb2Firmantes, cDesPoder,
						cDesAdmin, cDesOrg, dFechaIns, dFechaCons, cNac, cDesNacion,
						cNomContacto, cSufijo, cDescSufi, cEscritura, cNombreNot, cNumNot, cCdNotarioct, cDesCdNot, cEscrituraPod, cNomNotariopd, cNumNotariopd,
						cCdNotariopd, cDesCdNotpd, cNombreSoc, dFechaInspd, cEmailpm,
						cCdfoliomerct, cNumfoliomerct, cUsuarioAut1, cUsuarioAut2, cFolContrato1,
						cFolContrato2, cCodIndentifusu1, cCodIndentifusu2, cDescUsu1, cDescUsu2,
						cNumIdentUsuar1, cNumIdentUsuar2, iContToken, cTpoPersona, cNumcteapoder,
						cNomapoder, cDocConst, cSucursal, dFechaAlta, iNumAdmon, cPoder,
						cAdmin, cOrg, cNombreSucursal, cValorTpopersona, cRazonSocial,cCURP,cRFCAlt,cRegimenFiscal;
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_consultarctemoral_03';
		...
		RETURN cCodRet, TRIM(NVL(cRFC,'')),TRIM(NVL(cApellPaterContactoRepLeg,'')),TRIM(NVL(cApellMaterContactoRepLeg,'')),TRIM(NVL(cNomb1ContactoRepLeg,'')),
					...
					TRIM(NVL(cNumIdentUsuar1,'')),TRIM(NVL(cNumIdentUsuar2,'')),NVL(iContToken,0), TRIM(NVL(cTpoPersona,'')),TRIM(NVL(cNumcteapoder,'')),
					TRIM(NVL(cNomapoder,'')),TRIM(NVL(cDocConst,'')),TRIM(NVL(cSucursal,'')),NVL(dFechaAlta,DATE(1)),NVL(iNumAdmon,0),TRIM(NVL(cPoder,'')),
					TRIM(NVL(cAdmin,'')),TRIM(NVL(cOrg,'')),TRIM(NVL(cNombreSucursal,'')), TRIM(NVL(cValorTpopersona,'')), TRIM(NVL(cRazonSocial, '')),cCURP,cRFCAlt,TRIM(TRIM(cRegimenFiscal)); 	
	END;
	
END PROCEDURE
DOCUMENT 'AUTOR: SAUL ORTIZ BAEZA',
'FECHA: 23/05/2014',
'DESCRIPCION: Procedimiento espejo al sp_consultarctemoral.sql que obtiene la informacion del cliente moral',
'ahora validando de manera general el tipo de persona. Y adaptado ahora para el caso de 2 administradores',
'AUTOR: L. Montserrat LeÃ³n Amador',
'FECHA: 14/10/2015',
'MODULO: CLIENTES',
'FUNCIONALIDAD: ModificaciÃ³n Persona Moral', 
'DESCRIPCION: Se realizÃ³ el cambio del codigo de retorno 00020 (EL TIPO DE PERSONA NO ES VALIDO)', 
'por: 00701 (TIPO DE PERSONA NO MORAL, VERIFIQUE).',
'AUTOR: M.D.S.Sandra Cano',
'FECHA: 03/10/2016',
'DESCRIPCION: Se actualiza para extraer correo electronico de la tabla si_correos, solo para Persona Moral',
'AUTOR: L. Montserrat LeÃ³n Amador',
'FECHA: 28/02/2017',
'DESCRIPCION: Se actualiza para que la extracciÃ³n de correo electronico sea asociada al apoderado legal.',
'ID REQUERIMIENTO TASF: CON-01-02-03-B-0448',
'AUTOR: Daniel Reyes Guillen',
'FECHA: 24/06/2021',
'DESCRIPCION: Se agrega consulta de CURP',
'AUTOR: JosÃ© Antonio RamÃ­rez Franco',
'FECHA: 29/09/2023',
'DESCRIPCION: Se agrega la consulta de Regimen fiscal',
'BD: bdicnweb';
...
CREATE PROCEDURE "informix".sp_guardainfoctemoral(pUsuario CHAR(8),										
	...
	BEGIN
		...
		END EXCEPTION WITH RESUME;
		 ...
		 --SET DEBUG FILE TO '/tmp/mfinis/sp_guardainfoctemoral.out';
		 --TRACE ON;
		...
		EXECUTE PROCEDURE bdicnweb:"informix".sp_guardactemoral(pUsuario, pIdFuncion, pfuncion, pnumcte, pstatuscte, psucursal, ptp_persona, ptp_cliente, prazon_social, prfc,               
											  pfechaalta, pnacionalidad, pnombrecorto, pnombrecontacto, ptelefonocontacto, psufijo, pgiro, pactividad_princ, ppaginainternet,pCURP,pRFCAlt,pRegimen)    
		...
		IF pcodpostal <> '' AND iregistro > 0 THEN
			UPDATE bdinteg:"informix".si_fiscal 
			SET cod_postal = pcodpostal WHERE empresa = '001' AND numcte = pnumcte;
		END IF;
...
'AUTOR: Daniel Reyes Guillen',
'FECHA: 25/06/2021',
'DESCRIPCION: Se aÃÂ±ade CURP',
'AUTOR: JosÃ© Antonio RamÃ­rez Franco',
'FECHA: 25/06/2021',
'DESCRIPCION: Se agrega el campo Regimen fiscal',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_regimenfiscales_totales (pUsuario CHAR(8), pIdFuncion CHAR(10))
RETURNING CHAR(5) AS codret,
          CHAR(3) AS c_regimenfiscal,
    ...
    BEGIN 
        ...
        ON EXCEPTION SET iSqlErr
		   IF iSqlErr != 0 THEN
			  LET cCodRet = iSqlErr;
               RETURN cCodRet, cRegimenFiscal, cDescripcion;
        	END IF;
		END EXCEPTION;
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_regimenfiscales_totales.out';
		--TRACE ON;
        ...
        FOREACH
            ...
            WHERE tipo = 'M'
            ORDER BY c_regimenfiscal ASC

            LET iNoRegistros = iNoRegistros + 1;
            RETURN cCodRet, cRegimenFiscal, cDescripcion WITH RESUME;
...
DOCUMENT 'AUTOR: JOSE ANTONIO RAMIREZ FRANCO',
'FECHA: 28/09/2023',
'FUNCIONALIDAD: CLIENTE PM',
'DESCRIPCION: Muestra todo el catalogo de los regimen fiscales de la tabla: bdinteg:si_regimen_fiscal de las personas morales',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consultainfoctemoral2(pNumcte CHAR(20))
		RETURNING CHAR(5) 		AS codret,
				  CHAR(13) 		AS RFC,
				  CHAR(26) 		AS APELL_PATER_REP_LEG,
				  CHAR(26) 		AS APELL_MATER_REP_LEG,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			...
			RETURN cCodRet, TRIM(NVL(cRFC,'')),TRIM(NVL(cApellPaterContactoRepLeg,'')),TRIM(NVL(cApellMaterContactoRepLeg,'')),TRIM(NVL(cNomb1ContactoRepLeg,'')),
						...
						TRIM(NVL(cNumIdentUsuar1,'')),TRIM(NVL(cNumIdentUsuar2,'')),NVL(iContToken,0), TRIM(NVL(cTpoPersona,'')),TRIM(NVL(cNumcteapoder,'')),
						TRIM(NVL(cNomapoder,'')),TRIM(NVL(cDocConst,'')),TRIM(NVL(cSucursal,'')),NVL(dFechaAlta,DATE(1)),NVL(iNumAdmon,0),TRIM(NVL(cPoder,'')),
						TRIM(NVL(cAdmin,'')),TRIM(NVL(cOrg,'')),TRIM(NVL(cNombreSucursal,'')), TRIM(NVL(cValorTpopersona,'')), TRIM(NVL(cRazonSocial, '')),cCURP,cRFCAlt, cRegimenFiscal;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/Antonio/sp_consultainfoctemoral2.out';
		--TRACE ON;
		...
		EXECUTE PROCEDURE bdinteg:"informix".sp_consultarctemoral_04(pNumcte)
		INTO cCodRetSp, cRFC, cApellPaterContactoRepLeg, cApellMaterContactoRepLeg, cNomb1ContactoRepLeg,
			            cNomb2ContactoRepLeg , cCalleFiscal, cNumExtCalleFiscal, cColFiscal,
						vNomCiudFiscal, cCodMunFiscal, cNomEstadoFiscal, cNumcte, cNomCorto,
						cPagInternet, cSatFea, cTelContacto, cGiro, cNomGiro,
		...
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP sp_consultarctemoral_04';
		ELIF iCodRetSp = 2 THEN
			LET cCodRet = '00017';
			RETURN cCodRet, TRIM(NVL(cRFC,'')),TRIM(NVL(cApellPaterContactoRepLeg,'')),TRIM(NVL(cApellMaterContactoRepLeg,'')),TRIM(NVL(cNomb1ContactoRepLeg,'')),
			            TRIM(NVL(cNomb2ContactoRepLeg,'')),TRIM(NVL(cCalleFiscal,'')),TRIM(NVL(cNumExtCalleFiscal,'')),TRIM(NVL(cColFiscal,'')),
...
'DESCRIPCION: sp clon sin validaciÃ³n de ejecutivo sp_consultainfoctemoral',
'FECHA: 12/09/2022',
'AUTOR: Jose Antonio RamÃ­rez Franco',
'DESCRIPCION: Se aÃ±ade el campo de Regimen fiscal, se modifica la longitud del nombre corto y razÃ³n social',
'FECHA: 17/10/2023',
'BASE DE DATOS: bdicnweb';

CREATE PROCEDURE "informix".sp_consultainfoctemoralrfc(pRFC CHAR(13))
		RETURNING CHAR(5) 		AS codret,
				  CHAR(13) 		AS rfc,
				  CHAR(26) 		AS APELL_PATER_REP_LEG,
				  CHAR(26) 		AS APELL_MATER_REP_LEG,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			...
			RETURN cCodRet, TRIM(NVL(cRFC,'')),TRIM(NVL(cApellPaterContactoRepLeg,'')),TRIM(NVL(cApellMaterContactoRepLeg,'')),TRIM(NVL(cNomb1ContactoRepLeg,'')),
						...
						TRIM(NVL(cNumIdentUsuar1,'')),TRIM(NVL(cNumIdentUsuar2,'')),NVL(iContToken,0), TRIM(NVL(cTpoPersona,'')),TRIM(NVL(cNumcteapoder,'')),
						TRIM(NVL(cNomapoder,'')),TRIM(NVL(cDocConst,'')),TRIM(NVL(cSucursal,'')),NVL(dFechaAlta,DATE(1)),NVL(iNumAdmon,0),TRIM(NVL(cPoder,'')),
						TRIM(NVL(cAdmin,'')),TRIM(NVL(cOrg,'')),TRIM(NVL(cNombreSucursal,'')), TRIM(NVL(cValorTpopersona,'')), TRIM(NVL(cRazonSocial, '')),cRegimenFiscal, cCURP,cRFCAlt;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/Antonio/sp_consultainfoctemoralrfc.out';
		--TRACE ON;
		...
		EXECUTE PROCEDURE bdinteg:"informix".sp_consultarctemoral_rfc(pRFC)
		INTO cCodRetSp, cRFC, cApellPaterContactoRepLeg, cApellMaterContactoRepLeg, cNomb1ContactoRepLeg,
			            cNomb2ContactoRepLeg , cCalleFiscal, cNumExtCalleFiscal, cColFiscal,
						vNomCiudFiscal, cCodMunFiscal, cNomEstadoFiscal, cNumcte, cNomCorto,
						cPagInternet, cSatFea, cTelContacto, cGiro, cNomGiro,
...
'DESCRIPCION: SP encargado de Consular la informaciÃ³n de cliente de tipo Persona Moral por medio de RFC',
'FECHA: 17/10/2023',
'BASE DE DATOS: bdicnweb';

CREATE PROCEDURE "informix".sp_consultainfocteprospecto2(pNumCte CHAR(20))
    RETURNING
		CHAR(5) AS cCodRet,
		CHAR(3) AS empresa,
		CHAR(20) AS numcte,
	...
	BEGIN
		...
		----SET DEBUG FILE TO '/tmp/mfinis/sp_consultainfocteprospecto2.out';
		----TRACE ON;
...
END PROCEDURE
DOCUMENT 'AUTOR: Johnattan Esquivel Sanchez',
'DESCRIPCION: sp clon sin validaciÃÂ³n de ejecutivo sp_consultainfocteprospecto',
'FECHA: 12/09/2022',
'BASE DE DATOS: bdicnweb';

CREATE PROCEDURE "informix".sp_consultainfocteprospectorfc(pRfc CHAR(13))
    RETURNING
		CHAR(5) AS cCodRet,
		CHAR(3) AS empresa,
		CHAR(20) AS numcte,
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/Antonio/sp_consultainfocteprospectorfc.out';
		--TRACE ON;
		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;
		...
		EXECUTE PROCEDURE bdinteg:"informix".consnumcte_prospectorfc('001', pRfc) 
		INTO cCodRetSp, vempresa, vnumcte, vstatus_cte, vsucursal, vejecutivo, vtpo_persona, vtipo_cliente, vapell_paterno, vapell_materno,
					vnombre1, vnombre2, vrazon_social, vrfc, vsector, vsegmento, vactividad_princ, vgrupo, vsubgrupo, vresidencia, vfecha_alta, 
					vapell_casada, vdistrito, vnumcte_ref, vstring1, vstring2, vnumeric1, vnumeric2, vmoney1, vdate1, vpuesto_ppes, vfamiliar_ppes, 
					vactividad_esp, vejecut_autoriza, vuser_insert, vfecha_insert, vrfc_alterno, vpfempresa, vpfnumcte, vpffecha_nac, vpflugar_nac,
		...
		FROM(SELECT tipo_dir, MAX(secuencia)
			FROM bdinteg:si_direcciones_actual
			WHERE numcte = vnumcte 	GROUP BY tipo_dir ORDER BY 1 DESC);
...
END PROCEDURE
DOCUMENT 'AUTOR: Jose Antonio RamÃ­rez Franco',
'DESCRIPCION: sp clon sin validaciÃ³n de ejecutivo sp_consultainfocteprospecto por medio de RFC',
'FECHA: 17/10/2023',
'BASE DE DATOS: bdicnweb';

CREATE PROCEDURE "informix".sp_generaportadactamec4(pNumCte CHAR(20), pNumCta CHAR(20))
                RETURNING CHAR(5)       AS      codret,                 --Codigo de retorno
                                                ...
                                                CHAR(314)       AS      razonSoc,               --RAZON SOCIAL
        ...
        DEFINE  cRazonSoc                CHAR(314);      --RAZON SOCIAL  
	...
	LET     cCuentaLigada   = "";
        ...
        BEGIN
                ...
                FOREACH EXECUTE PROCEDURE bdicheq:"informix".sp_ctamec_generarptportadaproducto2_2('001', pNumCte, pNumCta)
...
END PROCEDURE
DOCUMENT 'AUTOR: JosÃ© Antonio RamÃ­rez Franco',
'DESCRIPCION: sp clon sin validaciÃ³n de ejecutivo sp_generaportadactamec3 con razÃ³n social ampleada a 254 caracteres',
'FECHA: 11/12/2023',
'BASE DE DATOS: bdicnweb';
...
CREATE PROCEDURE "informix".sp_guardactemoral2(pUsuario 		 CHAR(8), 
	...
	BEGIN
		...
		EXECUTE PROCEDURE bdinteg:"informix".ctemoral2(cEmpresa, pfuncion, pnumcte, pstatuscte, psucursal, pUsuario, ptp_persona, ptp_cliente, prazon_social, prfc, pfechaalta,
...
'FECHA: 12/09/2022',
'AUTOR: JosÃ© Antonio RamÃ­rez Franco',
'DESCRIPCION: Se aÃ±ade el Regimen fiscal como parametro de entrada y se cambiaron la longitud de nombre corto y Razon social',
'FECHA: 12/09/2022',
'BASE DE DATOS: bdicnweb';
...
END PROCEDURE
DOCUMENT 'AUTOR: Johnattan Esquivel Sanchez',
'DESCRIPCION: sp clon sin validaciÃÂ³n de ejecutivo sp_guardainfoctemoral',
'FECHA: 12/09/2022',
'AUTOR: JosÃ© Antonio RamÃ­rez Franco',
'DESCRIPCION: Se aÃ±ande el campo de regimen fiscal a la alta de persona moral',
'FECHA: 12/09/2022',
'BASE DE DATOS: bdicnweb';

CREATE PROCEDURE "informix".sp_coppelmax_consultarpuntos(pNumcte CHAR(20), pFecha_inicial DATE, pFecha_final DATE, pNumProducto CHAR(4))
RETURNING
	VARCHAR(5,0)  AS codRetorno,
	INTEGER 	  AS Row_id,
	VARCHAR(30,0) AS fecha_movimiento,
	VARCHAR(40,0) AS descripcion,
	VARCHAR(1,0)  AS naturaleza,
	DECIMAL(16,2) AS monto,
	DECIMAL(16,2) AS beneficio_calculado,
	DECIMAL(16,2) AS puntos_totales,
	DECIMAL(16,2) AS saldo_actual;
DEFINE cCodRet     				CHAR(5);
DEFINE iSqlErr      			INTEGER;
DEFINE cRow_id					INTEGER;
DEFINE cFechaMovimiento			CHAR(30);
DEFINE cDescripcion				CHAR(40);
DEFINE cNaturaleza				CHAR(1);
DEFINE cMonto					DECIMAL(16,2);
DEFINE cBeneficio_calculado		DECIMAL(16,2);
DEFINE cPuntos_totales			DECIMAL(16,2);
DEFINE cSaldo_actual			DECIMAL(16,2);
DEFINE iRegistros				INTEGER;
LET cCodRet = '00000';
LET iSqlErr = 0;
LET cRow_id = 0;
LET cFechaMovimiento = '';
LET cDescripcion = '';
LET cNaturaleza = '';
LET cMonto = 0;
LET cBeneficio_calculado = 0;
LET cPuntos_totales = 0;
LET cSaldo_actual = 0;
LET iRegistros = 0;
	...
	BEGIN
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;			
			Return cCodRet, cRow_id, cFechaMovimiento, cDescripcion, cNaturaleza, cMonto, cBeneficio_calculado, cPuntos_totales, cSaldo_actual;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/Antonio/sp_coppelmax_consultarpuntos.out';
		--TRACE ON;
		
		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;
		IF pNumcte = '' OR pNumcte = 0 OR pFecha_inicial = '' OR pFecha_final = '' THEN
			LET cCodRet = '00003';
			Return cCodRet, cRow_id, cFechaMovimiento, cDescripcion, cNaturaleza, cMonto, cBeneficio_calculado, cPuntos_totales, cSaldo_actual;
		END IF;
		
		SELECT NVL(SUM(monto),'0'), NVL(SUM(beneficio_calculado),'0')
		INTO cSaldo_actual, cPuntos_totales
		FROM bdicred:"informix".sd_movs_monedero_plan_lealtad
		WHERE numcte = pNumcte AND fecha_mov::DATE BETWEEN pFecha_inicial AND pFecha_final 
		AND tipo_producto  = pNumProducto;
		
		FOREACH
			SELECT id, fecha_mov, tipo_mov, monto, beneficio_calculado
			INTO cRow_id, cFechaMovimiento, cDescripcion, cMonto, cBeneficio_calculado
			FROM bdicred:"informix".sd_movs_monedero_plan_lealtad
			WHERE numcte = pNumcte AND tipo_producto  = pNumProducto 
			AND fecha_mov::DATE BETWEEN pFecha_inicial AND pFecha_final
			
			LET cNaturaleza = cDescripcion;
			LET iRegistros = iRegistros + 1;
			RETURN cCodRet, cRow_id, cFechaMovimiento, cDescripcion, cNaturaleza, cMonto, cBeneficio_calculado, cPuntos_totales, cSaldo_actual WITH RESUME;
		END FOREACH;
...
'MODULO: Coppel Max',
"FUNCIONAMIENTO:SP secundario que realiza la consulta de puntos coppel max",
"FECHA : 06-03-2023",
"AUTOR : JosÃ© Antonio RamÃ­rez Franco",
"FUNCIONAMIENTO:Se aÃ±ade el parametro de producto para que realice la filtraciÃ³n de productos por numero de producto.",
"FECHA : 09-11-2023",
"BD    : bdicnweb";

CREATE PROCEDURE "informix".sp_coppelmax_modificarpuntos(pRow_id INTEGER, pPuntos_beneficio DECIMAL(16,2))
RETURNING
	VARCHAR(5,0)  AS codRetorno;
...
LET iSqlErr = 0;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_coppelmax_modificarpuntos.out';
		--TRACE ON;
		...
		UPDATE bdicred:"informix".sd_movs_monedero_plan_lealtad 
		SET beneficio_calculado = pPuntos_beneficio
		WHERE id = pRow_id;
...
"AUTOR : Eduardo Ãvila PÃ©rez Tagle",
'MODULO: Coppel Max',
"FUNCIONAMIENTO:SP secundario que realiza la modificaciÃ³n de puntos coppel max",
"FECHA : 06-03-2023",
"AUTOR : JosÃ© Antonio RamÃ­rez Franco",
"FUNCIONAMIENTO: Se elimina la creaciÃ³n del registro nuevo cuando se modifican puntos",
"FECHA : 09-11-2023",
"BD    : bdicnweb";

CREATE PROCEDURE "informix".sp_puntoscoppelmax(pBandera CHAR(1), 
												pSBandera CHAR(1),
												pNumcte CHAR(20), 
												pFecha_inicial DATE, 
												pFecha_final DATE, 
												pRow_id INTEGER, 
												pPuntos_beneficio DECIMAL(16,2), 
...
LET cSaldo_actual = 0;
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_puntoscoppelmax.out';
		--TRACE ON;
		...
		ELIF pBandera = '1' THEN
			IF pNumcte = '' THEN
				LET cCodRet = '00003';
				Return cCodRet, cRow_id, cFechaMovimiento, cDescripcion, cNaturaleza, cMonto, cBeneficio_calculado, cPuntos_totales, cSaldo_actual;
			ELIF pFecha_inicial = '' THEN
				LET cCodRet = '00003';
				Return cCodRet, cRow_id, cFechaMovimiento, cDescripcion, cNaturaleza, cMonto, cBeneficio_calculado, cPuntos_totales, cSaldo_actual;
			ELIF pFecha_final = '' THEN
				LET cCodRet = '00003';
				Return cCodRet, cRow_id, cFechaMovimiento, cDescripcion, cNaturaleza, cMonto, cBeneficio_calculado, cPuntos_totales, cSaldo_actual;
			END IF;
		...
		IF pBandera = '1' THEN
			FOREACH
				EXECUTE PROCEDURE "informix".sp_coppelmax_consultarpuntos(pNumcte, pFecha_inicial, pFecha_final, pNumProducto)
				INTO cCodRet, cRow_id, cFechaMovimiento, cDescripcion, cNaturaleza, cMonto, cBeneficio_calculado, cPuntos_totales, cSaldo_actual
				Return cCodRet, cRow_id, cFechaMovimiento, cDescripcion, cNaturaleza, cMonto, cBeneficio_calculado, cPuntos_totales, cSaldo_actual WITH RESUME;
			END FOREACH;
			
		ELIF pBandera = '2' THEN
			EXECUTE PROCEDURE "informix".sp_coppelmax_modificarpuntos(pRow_id, pPuntos_beneficio)
			INTO cCodRet;
			Return cCodRet, cRow_id, cFechaMovimiento, cDescripcion, cNaturaleza, cMonto, cBeneficio_calculado, cPuntos_totales, cSaldo_actual;
		END IF;
	END;
...
'MODULO: Coppel Max',
"FUNCIONAMIENTO:SP padre que realiza las consultas de los SP secundarios de puntos coppel max",
"FECHA : 06-03-2023",
"AUTOR : JosÃ© Antonio RamÃ­rez Franco",
"FECHA : 03-11-2023",
"FUNCIONAMIENTO: Se eliminan las Banderas 3 (Agregar puntos) y Bandera 4 (EliminaciÃ³n de puntos)",
"BD    : bdicnweb";

CREATE PROCEDURE "informix".sp_catalogoplazacaja(pUsuario CHAR(8), pIdFuncion CHAR(10), pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
			CHAR(3) AS id_plaza,
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cIdPlaza, cDescPlaza;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_catalogoplazacaja.out';
		--TRACE ON;
		...
		-- COMBOBOX PLAZA 
		FOREACH		
			SELECT SKIP pRegistros FIRST pRecuperacion DISTINCT prov.plaza, pla.descripcion 
			INTO cIdPlaza, cDescPlaza
			FROM bdisuc:'informix'.ss_proveedores prov LEFT JOIN bdinteg:'informix'.si_plazas_cajagen pla ON (prov.plaza = pla.codigo_plaza) 
			ORDER BY pla.descripcion ASC
			RETURN cCodRet, cIdPlaza, UPPER(cDescPlaza) WITH RESUME;
			LET iNoRegistros = iNoRegistros + 1;
		END FOREACH;

		 IF iNoRegistros = 0 AND pRegistros = 0 THEN
			LET cCodRet = '00017'; 
			RETURN cCodRet, cIdPlaza, UPPER(cDescPlaza);
		ELIF iNoRegistros = 0 AND pRegistros > 0 THEN
			LET cCodRet = '1001';
			RETURN cCodRet, cIdPlaza, UPPER(cDescPlaza);
		END IF;		
...
END PROCEDURE
DOCUMENT 'AUTOR: L. Montserrat  Amador',
'FECHA: 22/01/2015',
'DESCRIPCION: SPL, que hace la consulta para obtener el id y la descripcion de la plaza de proveedores, Consultas Entrada Salida Caja General',
'AUTOR: JosÃÂ© Antonio RamÃÂ­rez Franco',
'FECHA: 07/12/2023',
'DESCRIPCION: Se aÃÂ±ade el paginado a la consulta',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_cnsif_agrupar_movimientos(pUsuario CHAR(8), pIdFuncion CHAR(10), pSistemaCuenta CHAR(20), pFechaInicial DATE, 
														 pFechaFinal DATE, pNumCuenta CHAR(20), pClaveMov CHAR(50))
    RETURNING CHAR(5) AS codret;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cnsif_agrupar_movimientos.out';
		--TRACE ON;
		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;
		...
		DELETE FROM "informix".sw_cons_movimientos2 WHERE usuario = pUsuario;

		FOREACH
			SELECT id_registro,fecha,hora,cve_transacc,desc_transacc,folio,periodo_inicial,monto,periodo_final,sis_cuenta,naturaleza,referencia,reversos,sucursal,cve_proc,desc_proc,saldo,num_tarjeta,reversados,usuario,referencia23
			...
			ORDER BY fecha DESC,hora DESC,folio,cve_transacc ASC

			--VALIDAMOS QUE SEAN CUENTA NOSTRO
			SELECT COUNT(cuenta_nostro) 
			INTO iclienteNostro
			FROM bdicred:sd_ce_ctas_nostro WHERE status = 1 AND TRIM(cuenta_nostro) = TRIM(pNumCuenta); --Se modifica la posición de valores en clausula where para que se respete el indice creado
			...
			INSERT INTO "informix".sw_cons_movimientos2(id_registro,fecha, numcuenta, hora, cve_transaccion, desc_transaccion, folio, periodo_inicial, monto, periodo_final, sistema_cuenta, naturaleza, referencia, reversos, sucursal, cve_procedencia, desc_procedencia, saldo, numero_tarjeta, reversados, usuario_mov, referencia23, ordenante, refe, concepto,usuario) 
			VALUES(iId_registro,dFecha,pNumCuenta, dHora,cCveTransacc,cDescTransacc,cFolio,dPeriodoInicial,mMonto,dPeriodoFinal,cSisCuenta,
			cNaturaleza,cReferencia,cReversos,cSucursal,cCveProc,cDescProc,mSaldo,cNumTarjeta,cReversados,cUsuario,cReferencia23, vOrdenante, dRef, vConcepto, pUsuario);
...
'DESCRIPCION MODIFICACION: SP encargado de agrupar los movimientos de las cuenta con informaciÃ³n de las cuentas NOSTRO en caso de encontrarse.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_mc_consultaproducto(pIdUsuario CHAR(8), pIdFuncion CHAR(10))
RETURNING CHAR(5) AS codret,
          CHAR(5) AS id_producto,
          CHAR(50) AS descripcion;


DEFINE cCodRet CHAR(5);
DEFINE iSqlErr INTEGER;
DEFINE cIdProducto CHAR(5);
DEFINE cDescripcion CHAR(50);
DEFINE iRegistros INTEGER;
...
BEGIN
  	...
  	ON EXCEPTION SET iSqlErr
		  LET cCodRet = iSqlErr;
			RETURN cCodRet, cIdProducto, cDescripcion;
		END EXCEPTION;
			
		--SET DEBUG FILE TO '/tmp/mfinis/sp_mc_consultaproducto.out';
		--TRACE ON;
  ...
  FOREACH
      ...
      INTO cIdProducto, cDescripcion
			FROM bdicobranza:cb_param_campania 
			where tipo_campania = 55 and valor_numerico = 0
      ORDER BY descripcion

     LET iRegistros = iRegistros + 1;
    RETURN cCodRet, cIdProducto, cDescripcion WITH RESUME;
    END FOREACH
...
DOCUMENT "AUTOR: JosÃ© Antonio RamÃ­rez Franco",
"FECHA: 05/12/2023",
"DESCRIPCION:Se encarga de devolver el catalogo de productos de la funcionalidad de Reportes Mesa de control",
"BD: bdicnweb";
