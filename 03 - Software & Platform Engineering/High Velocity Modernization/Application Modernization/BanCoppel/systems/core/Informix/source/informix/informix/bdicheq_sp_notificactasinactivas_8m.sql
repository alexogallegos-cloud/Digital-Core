CREATE PROCEDURE "informix".sp_notificactasinactivas_8m( pEmpresa char(3) )
RETURNING CHAR(5);
	...
	DEFINE vDiasInformada8m INTEGER;
    ...
    ON EXCEPTION SET Sql_Err, Isam_Err, Desc_Err
        SET DEBUG FILE TO "/resplogifx/conciliachq/sp_notificactasinactivas_8m.err";
        TRACE ON;
        IF Sql_Err <> 0 THEN
    ...
    ---SET DEBUG FILE TO "/resplogifx/conciliachq/sp_notificactasinactivas_8m.out";
    ---TRACE ON;
	...
	SELECT valor::INT
      INTO vDiasInformada8m
      FROM bdicheq:sc_param
     WHERE empresa = pEmpresa
       AND codparam = 'DiasIniCtaInformada8';
	   Let vDi8m = vDiasInformada8m; 
	...
	--VALIDA EL NUMERO DE DÃAS DE INACTIVIDAD DE LA CUENTA PARA ENVIO DE NOTIFICACIÃN 7M
	 ...
	 FOREACH WITH HOLD		   
		SELECT CASE WHEN fecultdep > fecultret THEN fecultdep
         ...
         WHERE mae.status_cta IN('1','4')
           ...
           AND ( ( ( vFechaHoy - mae.fecultdep ) = vDiasInformada8m ) OR ( ( vFechaHoy - mae.fecultret ) = vDiasInformada8m ) ) 
	  ...
	  BEGIN WORK;        
		...
		IF (vDiasSinTransacc == vDi8m) THEN
		
		LET vTermCta = TRIM(SUBSTR(vCuenta,8,4));
			      
			    ---- REALIZA LA NOTIFICACION MENDIANTE SMS
				EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento('2','CTAS_INACS','CTAS_INACS4',vNumCliente,'','','2',vTermCta,'','','','','','','','','','','',0,0,0,0,0,'','') ---- NotificaciÃ³n de SMS 2a 8m (976 DÃ­as) 
			    INTO vSp_CodRet; 

				---- REALIZA LA NOTIFICACION MENDIANTE CORREO ELECTRONICO			  
				EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento('1','CTAS_INAC','CTAS_INAC2',vNumCliente,'','','2',vTermCta,'120','','',TRIM(vDescProd),'','','','','','','',0,0,0,0,0,CURRENT,'') ----  NotificaciÃ³n de Email 2a 8m (976 DÃ­as)
                INTO vSp_CodRet;

			INSERT INTO bdicheq:control_ctas_8m 
