CREATE PROCEDURE "informix".sp_upd_status_reestruc(pempresa char(3)) 
RETURNING CHAR(5) AS CodigoRetorno
	...
	--SET DEBUG FILE TO  "sp_upd_status_reestruc.out"; 
    --TRACE ON;
	...
	BEGIN
		...
		IF v_status_cred_serv <> v_status_cred_cred AND v_status_serv_elec = 'A' THEN
			...
			IF (v_status_cred_cred IN ('BA','BT','VP','E1','E2','E3') AND cMtoVen > 0) THEN		--IF v_status_cred_cred IN ('BA','BT') THEN--IFRS 
			...
			IF (v_status_cred_cred IN ('VP','AA','E1') AND cMtoVen = 0) THEN --IFRS 
