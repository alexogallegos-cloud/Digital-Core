CREATE PROCEDURE "informix".sp_concreing_conciliaintercard(
	...
	ps_Txn_code 				CHAR(1),	--archivo_origen de BdiTarjeta:Td_Movimientos_Conciliacion
	ps_Indicador_fastfounds 	CHAR(1),	    --archivo_origen de BdiTarjeta:Td_Movimientos_Conciliacion
	ps_MSI 						VARCHAR(2),	--Indicador de MSI de BdiTarjeta:Td_Movimientos_Conciliacion
	ps_vspromoMSI 				VARCHAR(2)	--Indicador de MSI de BdiTarjeta:Td_Movimientos_Conciliacion
)
...
/* Proceso de actualizacion de montos en Bditarjeta:td_movimientos_conciliaciÃ³n*/
...
--  Giro de negocio gasolineras y limosinas

DEFINE vsMontoMax_girosNeg  	INTEGER;

--  Meses sin Intereses
DEFINE vsTipo_cnc_msi integer;


--SET DEBUG FILE TO '/informix/LVRQ/MSI/debug/TraceCONCILIAINTERCARD.txt';
--TRACE ON;
--SET DEBUG FILE TO '/RESPALDOSNEW/Pruebas_Conciliacion/Salidas_ejecuciones/debug_sp_concreing_conciliaintercard.out';
--TRACE ON;
...
END EXCEPTION;
	...
	/*SE COMENTA EL SET LOCK PARA NO TENER DUPLICIDAD*/
	...
	IF ( psIntegridad = 'V') THEN
		...
		( psCve_usuario, psNumtarjeta , psSecuencia325, psMonto325, psMontoCashBack325,ps_secuencia_ext_archivo,ps_ArchivoOriIST,ps_MSI,ps_vspromoMSI) -- Se agrega psmonto325 para validar montos
		INTO vsRetorno, vsSecuenciaorig, vsSecuencia_extendida, vmMontointercard, vmMontointercardCashback, vdFechatransaccion,
			...
			vsCodReversa, vsCodigoCentral, vscodgironeg,vsTipo_cnc_msi; --TFORZADAS
		...
		-- Estructura montos de fraccionadas para ir armando su acumulado correspondiente
			...
			(
				...
				ps_Txn_code	,				--BdiTarjeta:Td_Movimientos_Conciliacion
				ps_Indicador_fastfounds				--BdiTarjeta:Td_Movimientos_Conciliacion
			...
			if vsb_aplica = 'P' then
				...
				if viTipo_conciliacion = 5 then
						...
						--TRACE 'ENTRE AQUI COdigoNeg: '|| vscodgironeg;
						
						IF psArchivo_origen IN ('VIC', 'VID', 'MCC', 'MCD') AND vscodgironeg ='4121' THEN
						
							select valor into vsvalor from bditarjeta:"informix".td_param_conciliacion_concreing
								where codigo = '751';
							LET vsMontoMax_girosNeg = vsvalor;
							
						ELIF psArchivo_origen IN ('VIC', 'VID', 'MCC', 'MCD') AND vscodgironeg ='5542' THEN
						
							select valor into vsvalor from bditarjeta:"informix".td_param_conciliacion_concreing
								where codigo = '752';
							LET vsMontoMax_girosNeg = vsvalor;
						
						ELIF psArchivo_origen IN ('VIC', 'VID', 'MCC', 'MCD') THEN -- Porcentaje fijo para Internacionales
						...
						ELIF psArchivo_origen IN ('VNC', 'VND') THEN  -- Porcentaje en Base a Nacionales y giros
							...
							from bditarjeta:"informix".td_parametros_montosmayores
								where consecutivo > 1
								and codigo_negocio = vscodgironeg;
						...
						--TRACE 'este es mi giro de negocio :' || vscodgironeg;
						--TRACE 'Monto :' || vmMonto325;
						--TRACE 'monto2 :' || vmMontoCashback325;
						
						IF (vscodgironeg = 4121 AND (vmMonto325 + vmMontoCashback325) <= vsMontoMax_girosNeg )THEN
					
						let vsb_aplica = 'V';
						
						ELIF (vscodgironeg = 5542 AND (vmMonto325 + vmMontoCashback325) <= vsMontoMax_girosNeg )THEN
					
						let vsb_aplica = 'V';

						ELIF ((vmMonto325 + vmMontoCashback325) > vmmontototalmaximo)  then 
...
'Proyecto: Conciliacion Automatica',
'Solicito: Jose Luis Puebla',
'Descripcion: Se agregÃ³ consulta para actualizar el campo numtarjeta de td_movimientos_conciliacion .',
'Fecha: 2011/10/05',
...
'Descripcion: Se modificaciÃ³n llamado a sp_concreing_buscarmovimientointercard, por cambio de entrada de integro psmonto325',
'Fecha: 2013/05/30',
...
'Solicito: Luis Antonio Gomez',
'Descripcion: Se modificaciÃ³n llamado a sp_concreing_buscarmovimientointercard y sp_concreing_identificatipoconciliacion, por cambio de entrada al integrar monto cashBack',
'Fecha: 2013/07/30',
...
'BD: BdiTarjeta',
'',
'MODIFICACION: MarÃ­a Fernanda Ortiz Figueroa',
'Proyecto: Optimizacion SP Conciliacion Automatica',
'Solicito: Produccion y BD',
'Descripcion: Se crea Ã­ndice para mitigar sequential y se quitan directivas SET',
'Fecha: 2023/07/20',
'BD: bditarjeta';
