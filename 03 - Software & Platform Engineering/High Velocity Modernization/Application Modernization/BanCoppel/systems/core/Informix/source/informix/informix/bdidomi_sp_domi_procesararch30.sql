CREATE PROCEDURE "informix".sp_domi_procesararch30(pNom_Arch CHAR(20),pNom_Arch31 CHAR(20),pNom_Arch32 CHAR(20),pUsuario CHAR(8),vRango1 INTEGER,vRango2 INTEGER,pApuntador INTEGER)
	RETURNING CHAR(5);
	...
	DEFINE cursor1                              CHAR(50);
	DEFINE cursor2                              CHAR(50);
 ...
 BEGIN
	...
	--SET DEBUG FILE TO "/RESPALDOSNEW/depuraremesas/sp_domi_procesararch30"|| vRango1 ||".out";
    --TRACE ON;
	...
	LET vsNomProceso = 'TRUNCATE_DET_PASO1';
    LET vsDescripcionProceso = 'TRUNCATE TABLE DETALLE_PASO_1';
	...
	IF iExisteProc = 0 THEN  
		TRUNCATE TABLE bdidomi:dom_cce_detalle_paso_1;
	    EXECUTE PROCEDURE bdidomi:sp_domi_bitacora('A', CURRENT::DATE, TRIM(vsNomProceso), vsDescripcionProceso, '1', '00000', pUsuario, 'sp_domi_procesararch30', TRIM(pNom_Arch) , 
		YEAR(dFecha_hoy) || LPAD(MONTH (dFecha_hoy),2,'0') || LPAD(DAY (dFecha_hoy),2,'0'), '11' ) INTO cCodRet; 
	ELSE
		LET iExisteProc = 0;
	END IF;	
...
--********************************************************************************************************************************************
	...
	LET vsNomProceso = 'INSERT_DET_PASO1';
    LET vsDescripcionProceso = 'INSERT TABLE DETALLE_A_PASO_1';
	SELECT count(*) INTO iExisteProc FROM BdiDomi:Dom_Procesos WHERE Fecha_Proceso = dFecha_hoy AND TRIM(Cve_Proceso) = TRIM(vsNomProceso) AND estatus = '1';
	
	IF iExisteProc = 0 THEN  
		INSERT INTO bdidomi:dom_cce_detalle_paso_1 
		...
		UPDATE STATISTICS MEDIUM FOR TABLE bdidomi:dom_cce_detalle_paso_1;
		EXECUTE PROCEDURE bdidomi:sp_domi_bitacora('A', CURRENT::DATE, TRIM(vsNomProceso), vsDescripcionProceso, '1', '00000', pUsuario, 'sp_domi_procesararch30', TRIM(pNom_Arch) , 
		YEAR(dFecha_hoy) || LPAD(MONTH (dFecha_hoy),2,'0') || LPAD(DAY (dFecha_hoy),2,'0'), '11' ) INTO cCodRet;
	ELSE
		SELECT MAX(num_secuencia) INTO vNumSecuencia FROM bdidomi:dom_cce_detalle_paso_1 WHERE nombre_arch <> pNom_Arch;
		LET vNumSecuencia = vNumSecuencia + 1;
		--LET vNumSecuencia1 = vNumSecuencia1 + 200000;
		--LET iExisteProc = 0;
	END IF;	
  LET vCuenta = 0;	 
-- Inicio del ciclo de busqueda por archivo y sus registros.
  FOREACH cursor1 WITH HOLD FOR
	...
	FROM bdidomi: "informix".dom_cce_detalle_paso_1 Det
	WHERE  Det.nombre_arch =  pNom_Arch
	AND Det.tipo_registro = '02'
	AND Det.num_secuencia >= vRango1
	AND Det.num_secuencia <= vRango2
    ...
    LET vsDescripcionProceso = 'sp_domi_procesararch30' || ' ' || cClabeoTarjeta;
	...
	--	Valida que la clabe ni el importe se obtengan sin valores.
	 ...
	 LET vNumSecuencia = vNumSecuencia +1;
 	 ...
 	 /*SELECT COUNT(num_cta_rec) INTO iContadorRepetidas FROM bdidomi: "informix".dom_cce_detalle_paso_1
	 ...
	 IF iContadorRepetidas > 1 THEN
	 	...
	 	FOREACH cursor2 WITH HOLD FOR
			...
			INTO cSecuenciaAux
			FROM bdidomi: "informix".dom_cce_detalle_paso_1
			WHERE  nombre_arch =  pNom_Arch
			AND num_cta_rec = cClabeoTarjeta
			AND rfc_ord = cRFCOrdenante
			AND banco_presentador =  cBancoPresentador
			...
			INSERT INTO bdidomi: "informix".dom_cce_detalle_paso_1 (nombre_arch,fecha_presentacion,tipo_registro,num_secuencia,cod_operacion,cod_divisa,
			...
			UPDATE bdidomi: "informix".dom_cce_detalle_paso_1 SET cve_estatus = '02' WHERE CURRENT OF cursor2;
	 ...
	 SELECT tipo_registro,cod_divisa,importe,uso_futuro_ccen,tipo_operacion,fecha_aplica,tipo_cta_ord,
	  ...
	  FROM bdidomi: "informix".dom_cce_detalle_paso_1 WHERE nombre_arch = pNom_Arch AND cod_operacion = '30' AND num_secuencia = iSecuencia;																																																													 
	 ...
	 IF iExiste = 1 THEN
		...
		LET cSecuencia = LPAD (TRIM(TO_CHAR(vNumSecuencia)),7,'0');
		 ...
		 (pNom_Arch31,cFechaFormat,vTipo_registro,cSecuencia,'31', vCod_divisa,cFecha_trans,cBancoReceptor,cBancoPresentador,vImporte,vUso_futuro_ccen,																						  																						  
		  ...
		  UPDATE bdidomi: "informix".dom_cce_detalle_paso_1 SET cve_estatus = '02'
...
--				VALUES (cRFCOrdenante,cNombreTitular,'N','05','N',NULL,NULL,NULL,NULL,NULL,NULL,NULL,pUsuario,CURRENT::DATE);
		...
		ELSE
			...
			INSERT INTO bdidomi: "informix".dom_cce_detalle_paso_1 (nombre_arch,fecha_presentacion,tipo_registro,num_secuencia,cod_operacion,cod_divisa,
			 ...
			 (pNom_Arch32,cFechaFormat,vTipo_registro,cSecuencia,'32', vCod_divisa,cFecha_trans,cBancoReceptor,cBancoPresentador,vImporte,vUso_futuro_ccen,
			  ...
			  UPDATE bdidomi: "informix".dom_cce_detalle_paso_1 SET folio_suc =  cNumeroFolioCargo,cve_estatus = '01' --WHERE CURRENT OF cursor1;
			  --WHERE nombre_arch = pNom_Arch AND cod_operacion = '30' AND num_secuencia = iSecuencia;
			  WHERE CURRENT OF cursor1;
			  --Hago commit y vuelvo a iniciar
				LET vCuenta = vCuenta + 1;
  ...
  END FOREACH;
	...
	IF iExistePend >= 1 THEN 
		EXECUTE PROCEDURE bdidomi:sp_domi_procesararch30(pNom_Arch,pNom_Arch31,pNom_Arch32,pUsuario,vRango1,vRango2,pApuntador) INTO cCodRet;
	ELSE
		LET cCodRet = 0;
	END IF;							 
	IF cCodRet = 0 THEN
