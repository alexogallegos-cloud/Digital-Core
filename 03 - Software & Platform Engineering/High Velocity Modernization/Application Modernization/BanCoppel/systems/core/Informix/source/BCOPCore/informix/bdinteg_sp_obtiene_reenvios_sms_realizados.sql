CREATE PROCEDURE "informix".sp_obtiene_reenvios_sms_realizados( pEmpresa  CHAR(3),
                                                   pNumCte    CHAR(20),
                                                   pTelefono  CHAR(10) )
RETURNING CHAR(6)  AS rCodret,
          SMALLINT AS rTipoTel;

    -- #################################
    -- # 1. DECLARACIÃN DE VARIABLES
    -- #################################
    DEFINE sCodret   CHAR(6);    -- Almacena el cÃ³digo de retorno
    DEFINE iTotal    SMALLINT;   -- Contador para el total de reenvÃ­os encontrados
    DEFINE iSql_err  INTEGER;    -- Almacena el SQLCODE en caso de error
    DEFINE iSam_err  INTEGER;    -- Almacena el ISAMCODE en caso de error
    DEFINE sDesc_err CHAR(50);   -- Almacena la descripciÃ³n del error

    -- #################################
    -- # 2. INICIALIZACIÃN DE VARIABLES
    -- #################################
    -- Establecer valores iniciales para las variables de control y retorno.  
 
    LET sCodret   = '000000'; -- CÃ³digo de retorno inicial (asume Ã©xito)
    LET iTotal    = 0;        -- Inicializa el contador de reenvÃ­os a cero
    LET iSql_err  = 0;        -- Reinicia el SQLCODE de error
    LET iSam_err  = 0;        -- Reinicia el ISAMCODE de error
    LET sDesc_err = '';       -- Limpia la descripciÃ³n del error
    BEGIN
        -- #################################
        -- # 3. MANEJO DE EXCEPCIONES GENERAL
        -- #################################
        -- Este bloque ON EXCEPTION captura cualquier error SQL inesperado durante la ejecuciÃ³n.
        -- Retorna un cÃ³digo de error genÃ©rico '000001' para errores de sistema, junto con 0 reenvÃ­os.
        ON EXCEPTION SET iSql_err, iSam_err, sDesc_err
            --SET DEBUG FILE TO "/resplogifx/conciliachq/sp_consulta_telefonos.err";
            --TRACE ON;
            IF iSql_err <> 0 THEN
                LET sCodret = '000001';
                LET iSql_err = iSql_err;
                LET iSam_err = iSam_err;
                LET sDesc_err = sDesc_err;
                RETURN sCodret, iTotal;
            END IF;
        END EXCEPTION;
        ...
        -- #################################
        -- Configura el nivel de aislamiento y el modo de bloqueo para la transacciÃ³n.
        -- DIRTY READ permite leer datos no confirmados, optimizando el rendimiento para consultas.
        ...
        -- #################################
        -- Se valida que todos los parÃ¡metros esenciales (Empresa, NÃºmero de Cliente, Tipo de TelÃ©fono)
        -- contengan valores vÃ¡lidos y no estÃ©n nulos o vacÃ­os.
        IF (pEmpresa is null OR pEmpresa = '') OR
           (pNumCte is null OR pNumCte = '') OR
           (pTelefono is null OR pTelefono = 0) THEN
            LET sCodret = '000001'; --- DATOS INSUFICIENTES
            RETURN sCodret, iTotal;
        END IF;
        
        -- #################################
        -- # 6. CONSULTA DE REENVÃOS DE SMS
        -- #################################
        -- Obtiene la cantidad de reenvÃ­os de SMS realizados para el cliente y telÃ©fono
        -- especificados, Ãºnicamente para el dÃ­a actual y donde la acciÃ³n fue 'REENVIO SMS'.
        ...
        -- #################################
        -- # 7. RETORNO FINAL DE RESULTADOS
        -- #################################
        -- Retorna el cÃ³digo de Ã©xito ('000000') y el nÃºmero total de reenvÃ­os encontrados.
        -- Si no se encontraron reenvÃ­os, iTotal serÃ¡ 0.
        LET sCodret = '000000'; --- NO DE CLIENTE NO EXISTE
        RETURN sCodret, iTotal;
...
END PROCEDURE
DOCUMENT
'SCRIPT DE PROCEDIMIENTO ALMACENADO "sp_obtiene_reenvios_sms_realizados"',
'Folio.........: RQM 10 1697-2 Adendum: Reparos de AuditorÃ­a.',
'Autor.........:  Miguel Angel Blanco Arechiga',
'Descripcion...: Consulta el nÃºmero de reenvÃ­os de SMS realizados para un cliente especÃ­fico',
'Fecha.........: 03/03/2025',
