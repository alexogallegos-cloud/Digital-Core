CREATE PROCEDURE "informix".sp_trans_movs_unload_stat06( pFechaBusqInicial DATETIME YEAR to FRACTION(5), pFechaBusqFinal DATETIME YEAR to FRACTION(5) )
    ...
    DEFINE NOM_ARCH_REG_CNC_STAT VARCHAR(33);
    DEFINE NOM_ARCH_ERR_CNC_STAT VARCHAR(33);    
    ...
    LET NOM_ARCH_REG_CNC_STAT = '';
    LET NOM_ARCH_ERR_CNC_STAT = '';
    LET PREFIJO_ARCHIVO = 'transf_';
    LET vIndicadorProceso = '0';
    
    --SET DEBUG FILE TO RUTA_UNLOAD_RESPALDOS || "debug_sp_trans_movs_unload_stat06.out";                                                
    --TRACE ON;        
    ...
    BEGIN 		
        ...
        ON EXCEPTION SET SQLERR, ISAM_ERR, ERROR_INFO
            ...
            SET DEBUG FILE TO RUTA_UNLOAD_RESPALDOS || "excep_sp_trans_movs_unload_stat06.err.out" WITH APPEND;
            TRACE ON;
        ...
        IF ( vTotalRegistros = 0) THEN
            ...
            LET vMENSAJE_RETORNO = 'Sin informacion para procesar en el stat 06';
            RETURN vCODIGO_RETORNO, vMENSAJE_RETORNO;
        END IF
        
        
        LET NOMBRE_UNL_ARCHIVO = PREFIJO_ARCHIVO||'stat06_'||vAnio||'.unl';
        LET SCRIPT_EJECUCION = PREFIJO_ARCHIVO||'ejec_stat06_'||vAnio||'.sql';
        LET SCRIPT_UPD_STS = PREFIJO_ARCHIVO||'ejec_upd_sts_stat06_'||vAnio||'.sql';
        LET NOM_ARCH_REG_CNC_STAT = PREFIJO_ARCHIVO||'reg_stat06_'||vAnio||'.txt';
        LET NOM_ARCH_ERR_CNC_STAT = PREFIJO_ARCHIVO||'err_stat06_'||vAnio||'.log';           
        ...
        LET vExecuteSQL = '';
        LET vExecuteSQL = "echo "||'"'|| "FILE '"||RUTA_UNLOAD_RESPALDOS||NOMBRE_UNL_ARCHIVO|| "' delimiter '|' "|| '33'||                          
                          "; INSERT INTO conciliacion_atm_stat06_"||vAnio|| ";"||'"'||' > '||RUTA_UNLOAD_RESPALDOS||NOM_ARCH_REG_CNC_STAT;
        SYSTEM vExecuteSQL;        
        
        LET vExecuteSQL = '';
        LET vExecuteSQL = "dbload -d intercard -c "||RUTA_UNLOAD_RESPALDOS||NOM_ARCH_REG_CNC_STAT||" -l "||RUTA_UNLOAD_RESPALDOS||NOM_ARCH_ERR_CNC_STAT||" -n "||CONTADOR_TRANSACCIONES||" -r";
        SYSTEM vExecuteSQL;
        ...
        LET vExecuteSQL   = '';
        LET vExecuteSQL   = 'gzip -c '||RUTA_UNLOAD_RESPALDOS||NOMBRE_UNL_ARCHIVO||'>'||RUTA_UNLOAD_RESPALDOS||'stat06_bk_'||vMes||vAnio||'.gz';
        SYSTEM vExecuteSQL;
...
'Fecha de modificacion: 04 de junio del 2021',
'Armando Garcia Ortiz',
'Coordinacion de Tarjetas - Gerencia I',
'Descripcion: Componente principal para descargar informacion de la tabla conciliacion_atm_stat_06',
