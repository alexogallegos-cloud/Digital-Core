CREATE PROCEDURE "informix".sp_remesaswu_pld_ov(NombreProceso CHAR(3),FechaIni DATE, FechaFin DATE)
RETURNING
...
CHAR(80)		AS mensaje_respuesta;
	...
	--SET DEBUG FILE TO  '/INFORMIXTMP/HMLG/EXEC_sp_remesaswu_pld_ov.out';
	--TRACE ON;
	...
	/*				 
	Create temp table tmpsac_secuencia_search(
	pt_mtcn VARCHAR(16),
	fechahorainsercion DATETIME YEAR to SECOND);
	
	CREATE INDEX "informix".idxtmpsac_secuencia_searchps ON tmpsac_secuencia_search(pt_mtcn,fechahorainsercion);
	*/
	--SET DEBUG FILE TO  '/ifxsif01/yoselin/job/sp_ov.out';
	--TRACE ON;
    ...
    BEGIN
        ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
            IF iSqlErr <> 0 THEN
                LET cCodRet = iSqlErr;
				LET cMensaje = "ERROR";
                EXECUTE PROCEDURE "informix".sp_sac_guardamensajeerror(iSqlErr, iIsamErr, cInfoErr, "sp_remesaswu_pld_ov" || "Remesa:" || cNum_confirmacion);
		...
		ELSE			
			...
			FOREACH WITH HOLD
			  --select {+AVOID_FULL(bdicheq:"informix".sc_movhis )}					
				...
				where empresa = '001'
				  and transacc in ('1152','1122','1553','1400')
			...
			BEGIN WORK;
			FOREACH WITH HOLD
				--select {+INDEX("informix".sac_servicios_wu idxsac_servicios_wur1)} NVL(fecha_pago,mdy(01,01,1900)),'OVA','NO',NVL(referencia1,''),NVL(importe_pago,0),NVL(monto_dolares,0),NVL(transacc_suc,''),
				select NVL(fecha_pago,mdy(01,01,1900)),'OVA','NO',NVL(referencia1,''),NVL(importe_pago,0),NVL(monto_dolares,0),NVL(transacc_suc,''),
			...
			IF iCuantosCheq <> iCuantosMovtos THEN
				...
				FOREACH WITH HOLD						
				  ...
				  --where folio_suc not in (select {+INDEX("informix".sac_servicios_wu idxsac_servicios_wur1)} folio_suc from "informix".sac_servicios_wu)
					...
					VALUES(dFecha_alta,'OVA','NO','',mMonto_total,0,cTransaccion,cFolio_sucursal,(today), 
				...
				ELSE IF (iCuantosPay <> iCuantosMovtos) AND (iCuantosSearch <> iCuantosMovtos) THEN				
					...
					FOREACH WITH HOLD						
						...
						from "informix".sac_servicios_wu 
						where referencia1 not in (select mtcn from "informix".sac_datos_pay_wu)
						and referencia1 not in (select pt_mtcn from "informix".sac_datos_search)
						...
						hora_proceso,hora_remesa,colonia_ordenante,tipo_id_ordenante,ciudad_id_ordenante,ocupacion_beneficiario,name_benef_suc,
						num_id_benef_suc,fecha_envio_remesa)
						VALUES(dFecha_alta,'OVA','NO',cReferencia1,mMonto_total,0,cTransaccion,cFolio_sucursal,(today), 
					...
					ELSE IF (iCuantosPay <> iCuantosMovtos) THEN
						...
						FOREACH WITH HOLD					
							...
							where pt_mtcn not in (select mtcn from "informix".sac_datos_pay_wu)
							...
							hora_proceso,hora_remesa,colonia_ordenante,tipo_id_ordenante,ciudad_id_ordenante,ocupacion_beneficiario,name_benef_suc,
							num_id_benef_suc,fecha_envio_remesa)
							VALUES(dFecha_alta,'OVA','NO',vPt_Mtcn,mMonto_total,mMonto_dolares,cTransaccion,cFolio_sucursal,(today), 
							...
							FOREACH WITH HOLD
							  ...
							  --WHERE mtcn not in (select {+INDEX("informix".sac_datos_search idxsac_datos_searchp)} pt_mtcn from "informix".sac_datos_search)
								...
								ocupacion_beneficiario,name_benef_suc,num_id_benef_suc,fecha_envio_remesa)
								  VALUES(dFecha_alta,'OVA','NO',vPt_Mtcn,mMonto_total,0,cTransaccion,cFolio_sucursal,(today), 
