CREATE PROCEDURE "informix".sp_ctanvl2_generadocs_esp1( pEmpresa CHAR(3) )
RETURNING CHAR(5), CHAR(5), CHAR(50), INTEGER, INTEGER;
    ...
    ON EXCEPTION SET iSqlErr, iSamErr, iDesErr
        SET DEBUG FILE TO "/RESPALDOSNEW/DoctosCtaNvl2/sp_ctanvl2_generadocs_esp1.err";
        TRACE ON;
        IF iSqlErr <> 0 THEN
    ...
    --- SET DEBUG FILE TO "/RESPALDOSNEW/DoctosCtaNvl2/sp_ctanvl2_generadocs_esp1.out";
    --- TRACE ON;
    ...
    FOREACH WITH HOLD
        SELECT mae.num_cte, mae.cuenta
         ...
         WHERE mae.cuenta = noc.cuenta
           ...
           AND noc.fecha_alta IN('07/20/2023','07/22/2023')
         ORDER BY mae.cuenta
