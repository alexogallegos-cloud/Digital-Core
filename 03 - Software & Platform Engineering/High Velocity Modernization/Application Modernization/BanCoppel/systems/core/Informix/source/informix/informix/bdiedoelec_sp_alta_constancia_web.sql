CREATE PROCEDURE "informix".sp_alta_constancia_web (pempresa char(3),pnumcte char(20), pcuenta char(20), pstatus_serv_elec char(1), puser_modif char(20)) 
    ...
    --SET DEBUG FILE TO  "/informix/sp_alta_constancia.out"; 
    --TRACE ON;
    ...
    BEGIN
		...
		IF (v_cuenta IS NULL OR v_cuenta = '' ) THEN
			...
			LET v_sCodRet = '00002'; --Cuenta No VÃ¡lida
			RETURN v_sCodRet;
		...
		IF pstatus_serv_elec NOT IN ('A','I') THEN
			...
			LET v_sCodRet = '00003'; --Estatus de Servicio InvÃ¡lido
			RETURN v_sCodRet;
		...
		IF pempresa <> '001' THEN
			...
			LET v_sCodRet = '00004'; --Empresa InvÃ¡lida
			RETURN v_sCodRet;
