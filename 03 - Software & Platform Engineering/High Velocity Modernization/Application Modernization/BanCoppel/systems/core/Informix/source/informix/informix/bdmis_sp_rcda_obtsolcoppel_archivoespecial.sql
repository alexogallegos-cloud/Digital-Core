CREATE PROCEDURE "informix".sp_rcda_obtsolcoppel_archivoespecial(pfecha DATE)
...
BEGIN
   ON EXCEPTION SET SQL_ERR, ISAM_ERR, ERROR_INFO
      LET cod_ret    = SQL_ERR;
      LET mensaje  = ERROR_INFO || ' sp_rcda_obtsolcoppel_archivoespecial en paso ' || vpaso;
  ...
  DELETE FROM	mi_rcda_apert_paso;
   ...
   foreach cursor1 WITH HOLD for
		...
		where clave = ' ' and nombrearchivo = vnombrearchivo and fechageneracion = pfecha 
		group by 1,2,3
			...
			IF vtpo_reg = 1 THEN 
					...
					IF (SELECT COUNT(*) FROM mi_his_productividad WHERE fecha = pfecha AND sucursal = vsucursal AND tpo_reg = vtpo_reg AND ejecutivo = vpromotor AND producto = vproducto) = 0 THEN
							...
							INSERT INTO mi_his_productividad (fecha,sucursal,tpo_reg,ejecutivo,producto,copsoltdc,copsolmeta) 
								VALUES (pfecha,vsucursal,vtpo_reg,vpromotor, vproducto, vnumero, vmeta);
						ELSE	
