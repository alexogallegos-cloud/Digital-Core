CREATE PROCEDURE "informix".sp_consultabitacoracte(pEmpresa CHAR(3), pNumCte CHAR(20), pTipoProd CHAR(4))
--DATOS A REGRESAR--
...
--SET DEBUG FILE TO "/tmp/sp_consultabitacoracte.out";
--TRACE ON;
...
SET LOCK MODE TO WAIT 3;
	...
	IF pEmpresa = '' OR pEmpresa IS NULL OR pNumCte = '' OR pNumCte IS NULL OR pTipoProd = '' OR pTipoProd IS NULL THEN
		LET cCodRet = '00001'; --PARAMETRO DE ENTRADA VACIO.
		RETURN cCodRet, cExito;
	ELSE
		SELECT fecha_hoy INTO dtFecha FROM si_fechas;
		
		IF EXISTS(SELECT {+INDEX (si_encuesta_cte "idx_si_encuesta_cte")} numcte
		            FROM "informix".si_encuesta_cte 
			       WHERE empresa = pEmpresa
				     AND numcte = pNumCte
					 AND tipo_producto = pTipoProd
					 AND fecha_insert = dtFecha) THEN
			LET cExito = '0';
		ELSE
			LET cExito = '1';
		END IF;
...
CREATE PROCEDURE "informix".sp_genarchiadicionales(pempresa CHAR(3), pFechaAct DATE)
RETURNING
     CHAR(6);
