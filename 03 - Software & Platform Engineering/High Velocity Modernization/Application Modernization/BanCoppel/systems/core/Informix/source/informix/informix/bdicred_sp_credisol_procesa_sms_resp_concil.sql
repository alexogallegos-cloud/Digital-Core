CREATE PROCEDURE "informix".sp_credisol_procesa_sms_resp_concil(pEmpresa CHAR(3))
...
BEGIN
    ...
    --SET DEBUG FILE TO '/informix/mahr/sp_cred_proc_sms_resp_concil.out';
	--TRACE ON;
	...
	--=====================================================================================================================
	-- Identifica SMS de invitacion con respuesta SI del cliente y conciliacion liberada. (Conciliacion llego despues de respuesta del cliente.)
		

	LET cCodRet = '00000';
	LET cMensajeRet = '';
	CALL bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, 'INICIA CTES CON RESPUESTA SI Y COMPRA CONCILIADA SIN CREDISOL STATUS 0.', '02') Returning cCod_retIB;
		
	-- Identifica creditos por formalizar credisolucion. Respuesta SI de Cte y Compra conciliada
	FOREACH WITH HOLD
		SELECT ROWID,  num_credito, folio_compra_sms, tipo_sms,    envio_result_sms, plazo
		  INTO iRowID, cNumCredito, cFolioPromo, 	  sStatus_sms, cTipoMsgSms,		 sPlazo
		  FROM bdicred:sd_promocion_credito_sms
		 WHERE tipo_sms = '3' AND respuesta_cte_sms = 'S'
		...
		IF NVL(cNumCel,'') = '' THEN	-- Solo si no encuentra telefono activo
			SELECT first 1 a.telefono INTO cNumCel
			  FROM bdinteg:si_telefonos a INNER JOIN bdicred:sd_maecred b ON (a.numcte = b.numcte)
			 WHERE tipo_tel = 2 AND b.num_credito = cNumCredito;			
		END IF;
		   
		EXECUTE  PROCEDURE "informix".sp_credisol_contrata_x_sms (pEmpresa,2, cNumCel, cFolioPromo, 0, sPlazo)  INTO cCod_retIB;

	END FOREACH;

	LET cCodRet = '00000';
	LET cMensajeRet = '';
	CALL bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, 'FINALIZA CTES CON RESPUESTA SI Y COMPRA CONCILIADA SIN CREDISOL STATUS 0', '02') Returning cCod_retIB;
