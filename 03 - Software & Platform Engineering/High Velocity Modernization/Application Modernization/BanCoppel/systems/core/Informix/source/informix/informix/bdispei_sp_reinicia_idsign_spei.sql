CREATE PROCEDURE "informix".sp_reinicia_idsign_spei()
...
----Definicion de variables de evaluacion
	DEFINE vsval_spei INTEGER;
	DEFINE vsvalor VARCHAR(1);
	DEFINE vsfecha DATE;
	DEFINE vs_sec_a INTEGER;
	DEFINE vs_sec_b INTEGER;
-----------------------------------------------------****-----------------
----Inicalizacion de variables de control 
	...
	LET vMensaje = 'Proceso exitoso, se Limpio la secuencuia SPEI';
...
-----------------------------------------------------------
	...
	--SET DEBUG FILE TO RUTA||"/reinicio_spei.out";
	--TRACE ON;
	 ...
	 BEGIN
		...
		--SET DEBUG FILE TO RUTA || "reinicio_spei.err.out" WITH APPEND;
		--TRACE ON;
	...
	--------------------------------------------------Valida la bandera para poder ser ejecutado	
		SELECT vchrvalor 
		INTO vsval_spei
		FROM tblparametros 
		WHERE vchrcveparametro = 'BLOQUEO_A_USUARIOS';
		
				IF(vsval_spei = 0) THEN 
				
							LET vCodigoRetorno = '00001';
							LET vMensaje = 'Bandera arriba no se puede Limpiar la secuencuia SPEI';
				ELSE 
							SELECT valor
							INTO vsvalor
							FROM td_control_reinicio_spei;
						
					IF(vsvalor = 'F') THEN 
					
						UPDATE td_control_reinicio_spei SET fecha_modificacion = current,
						valor = 'V';
					----Restart sequence
								SELECT FIRST 1 seq_signmsgs.CURRVAL  INTO vs_sec_a
								FROM secsigns;
								
								UPDATE td_control_reinicio_spei SET val_anterior = vs_sec_a;
								
								ALTER SEQUENCE seq_signmsgs RESTART WITH 1;
								
								SELECT FIRST 1 seq_signmsgs.NEXTVAL INTO vs_sec_b
								FROM secsigns;
								
								UPDATE td_control_reinicio_spei SET val_nuevo = vs_sec_b ;
					 ...
					 ELIF(vsvalor = 'V') THEN 
						...
						FROM td_control_reinicio_spei;
							...
							IF(vsfecha > today -90) THEN 
								
								LET vCodigoRetorno = '00002';
								LET vMensaje = 'No se puede Limpiar la secuencuia SPEI ya que no se han cumplido 3 meses de antiguedad';
							
							ELSE 
									
								UPDATE td_control_reinicio_spei 
								set fecha_modificacion = current;
...
'Fecha de creacion: 19 de Enero del 2024',
'Base de datos: bdispei',
'RQI 34 010',
'Descripcion: Proceso de control para RESTART de SEQUENCIAL SPEI'
;

CREATE PROCEDURE "informix".spei_aplicaordenpago_mib2( pRegistros INTEGER, pOrigen CHAR(1) ) 
