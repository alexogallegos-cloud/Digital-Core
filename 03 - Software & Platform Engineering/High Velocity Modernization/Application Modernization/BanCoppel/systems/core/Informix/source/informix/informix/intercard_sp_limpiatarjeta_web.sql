CREATE PROCEDURE "informix".sp_limpiatarjeta_web( pcNumTarjeta CHAR(16),pcNumCredito CHAR(16))
...
BEGIN
	...
	--SET DEBUG FILE TO "/tmp/sp_limpiatarjeta.out"; -DSB20180619
	--TRACE ON;
	...
	--Se agrega validacion para solo eliminar de las bases de datos bdicred en caso de que la tarjeta no este asignada en intercard
	SELECT count(fechaasignacion) into iCont
	FROM intercard:"informix".tarjeta 
	WHERE fechaasignacion IS NOT NULL AND numtarjeta = pcNumTarjeta;
	...
	IF iCont < 1 THEN		
		...
		WHERE num_tarjeta = pcNumTarjeta AND empresa = '001';
	...
	-- Se trae la fecha actual para validar entre la apertura
		...
		From bdisolic:ss_autorizacion where empresa = '001' AND num_solicitud = pcNumCredito and status_solicitud = 'AP';
...
'Fecha.........: 22/04/2014',
'Modificacion..: Se crea procedimiento para validar si hay registros de la tarjeta en la tabla "sd_tarjeta" y eliminarlos.',
...
'Fecha.........: 19/06/2018',
'Modificacion..: Se modifica sp se comenta validacion si existe en  bdicred:"informix".sd_tarjeta ya que otro sp sp_validaAsignaTarjetaIncompleta de la BD:bdicred',
