CREATE PROCEDURE "informix".sp_obtiene_productos_cjunk_club_web_mx(pEmpresa 		CHAR(3),	-- Codigo de empresa
																...
																pCoppel 		CHAR(1), 	-- Indica si el cte ya tiene un creito coppel efectuado en tienda. 0
																							-- (1:Tiene credito coppel, 0: No tiene credito coppel)
																pPrecalCoppel 	CHAR(1), 	--Indica el resultado de la precalificacion del cte en coppel 1
	...
	RETURNING 	CHAR(6), -- Codigo de retorno
				CHAR(4), -- Codigo de producto a ofrecer al cte
	...
	BEGIN
		...
		--SET DEBUG FILE TO "/informix/mesg/sp_obtiene_productos_cjunk_club.out";
		--TRACE ON;
		...
		EXECUTE PROCEDURE bdinteg:"informix".consedadcte_web(pEmpresa, pNumcte)
		...
		ELSE
			...
			IF NOT EXISTS(SELECT perfilprom FROM  bdinteg:"informix".si_perfilproductos WHERE empresa = pEmpresa AND puesto = CAST(cPuesto AS INTEGER) AND activo = '1') THEN
				LET CodRet = '00012';
			END IF;
		END IF;
		...
		ELSE
            ...
            IF (inumpPrestamos >= iPrestamosActivos) THEN
				...
				INSERT INTO "informix".ss_bitacora_precal (empresa,fecha,producto,sucursal,nombre,ejecutivo,mensaje,causa_solicitud)
					VALUES (pEmpresa,today,'6300',pSucursal,cnomcte,pEjecutivo,'El Cliente cuenta con el tope mÃ?Â¡ximo de PrÃ?Â©stamos Personales permitidos','RFP');
			END IF;
			--JMAH
		...
		IF NVL(cCteProsp,'') <> '' THEN
			...
			AND secuencia = (SELECT MAX(secuencia) FROM "informix".ss_osclientesupervisar WHERE num_solicitud = cCteProsp);
			...
			IF NVL(cClave,'') = 'R' THEN
				...
				SELECT COUNT(*) 
					...
					AND sucursal = pSucursal 
					AND ejecutivo = pEjecutivo 
					AND producto_ofr = cProductoOfrecer;
			...
			FOREACH
				...
				IF cStatusSolic = "RT" THEN
					...
					IF iDiasRechazo <= 180 THEN
						UPDATE "informix".ss_productos_ofrecer SET aplica = 'N' WHERE cliente = pNumcte AND sucursal = pSucursal AND ejecutivo = pEjecutivo 
						AND producto_ofr = '6500';
					END IF
				END IF
			END FOREACH;
		...
		IF solApertPP = '1' THEN
			UPDATE "informix".ss_productos_ofrecer SET aplica = 'N' WHERE cliente = pNumcte AND sucursal = pSucursal AND ejecutivo = pEjecutivo AND tp_solicitud_ofr = 'P' AND producto_ofr IN ('6300','7600','7700');
		END IF;
		...
		FOREACH
			...
			IF cprod_final  = '6400' THEN 
				...
				SELECT COUNT(*) 
				INTO cCountExistente
				FROM "informix".ss_producto_credcap 
				WHERE num_producto = cprod_final 
				AND meses_alta IS NOT NULL;
			...
			IF cprod_final  = '7800' THEN --JMAH RQM 10 617
				   LET cOfertar ='N';
			       FOREACH
						EXECUTE PROCEDURE "informix".sp_adn_obtienectas_web('001', pNumcte)											
						INTO cVar1,cVar2
