CREATE PROCEDURE "informix".sp_com_manejo_cta_ident_4()
...
-- *****************************************************************************
    ...
    ON EXCEPTION SET vSQLErr, vIsamErr, vErrorInfo
        IF  vSQLErr != 0 THEN
            SET DEBUG FILE TO '/resplogifx/conciliachq/sp_com_manejo_cta_ident_4.err';
            TRACE ON;
            LET vCodRet     = vSQLErr;
    ...
    --SET DEBUG FILE TO '/resplogifx/conciliachq/sp_com_manejo_cta_ident_4.out';
    --TRACE ON;
        ...
        FOREACH WITH HOLD
           ...
           END IF;
            ...
            IF vFechaInicial < vFechConMovHis THEN
                ...
                WHERE mov.empresa  = vEmpresa
                AND mov.cuenta = vCuenta
                AND mov.fech_alt BETWEEN vFechaInicial 
                AND vFechaFinal
                AND mov.cancelad <> 'S'
                AND mov.transacc = "0273"
                AND mov.referencia LIKE "%NNNN%";
