CREATE PROCEDURE "informix".sc_datosriesgos(pempresa CHAR(3))
RETURNING CHAR(5);
--------------------------------------------------------------
...
FOREACH
	SELECT mae.num_cte,mae.cuenta,mae.sucursal,mae.plaza,mae.producto,mae.ultpagoint,fec.valor,cli.rfc,
		   ...
		   noc.acum_sdo_pos,noc.dias_acum_int,mae.sdo_actual,YEAR(cte.fecha_nac),MONTH(cte.fecha_nac),DAY(cte.fecha_nac)
	INTO vchrnumcte,vchrnumcuenta,vchrsucursal,vchrplaza,vchrproducto,vdteultpagoint,vdectasa,vchrrfc,
		 ...
		 vmnyacumsdo,vintdiasacum,vmnysdoactual,vintaniocte,vintmescte,vintdiacte 
	FROM sc_maechq mae
	...
	IF NOT EXISTS (SELECT COUNT(*) FROM sc_riesgoscap WHERE empresa = pempresa AND numcte = vchrnumcte
														AND cuenta = vchrnumcuenta AND fecha = vdtefecha ) THEN
		...
		VALUES ( pempresa,vchrnumcte,vchrnumcuenta,vchrsucursal,vchrplaza,vchrproducto,vdteultpagoint,vdectasa,vchrrfc,vchractividad,vchrresidencia,
				vchredocivil,vchrsexo,vchrhabitaen,vintanioshab,vmnyacumsdo,vintedad,vmnysdoprom,vmnysdoactual,vdtefecha );
	END IF;
