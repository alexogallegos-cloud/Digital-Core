CREATE PROCEDURE "informix".spei_aplicaordenpago( pRegistros INTEGER, pOrigen CHAR(1) ) 
RETURNING CHAR(5), INTEGER, INTEGER; 
    ...
    ---Variables para sustituir las cuentas de credito hipotecario por la cuenta concentradora del credito hipotecario
    DEFINE vNumCuentaConcentradora_credHip CHAR(11);
    DEFINE vInicioDigitos_CreditoHip CHAR(7);
    DEFINE vnumctadestino_iniciodigitos CHAR(7);
    DEFINE vCuentaBeneficiadora_credHip CHAR(18); -- cambio de 11 a 18 30 10 2025
    DEFINE vFechaOperacion DATETIME HOUR TO FRACTION(3);
    ...
    LET wcomision        = 0.00;
    LET wchrstatus       = '';
    LET vNumCuentaConcentradora_credHip    = '';
    LET vInicioDigitos_CreditoHip = '';
    LET vnumctadestino_iniciodigitos = '';
    LET vCuentaBeneficiadora_credHip = '';
    ...
    ON EXCEPTION SET iSqlErr, iIsamErr, cDescErr
        ...
        IF iSqlErr <> 0 THEN
			...
			ELIF pOrigen = 'T' THEN
                UPDATE tblctrlproceso SET chrstatus = '0' WHERE intcveproceso = 19;
            END IF;
	...
	ELIF pOrigen = 'T' THEN
        SELECT chrstatus INTO cStatusProc FROM tblctrlproceso WHERE intcveproceso = 19;
    END IF;
    ...
    IF cStatusProc = '0' THEN
		...
		ELIF pOrigen = 'T' THEN
            UPDATE tblctrlproceso SET chrstatus = '1' WHERE intcveproceso = 19;
        END IF;
		COMMIT WORK;
        ...
        FOREACH WITH HOLD
            SELECT {+INDEX(tblabono idx_tblabono_estatus)}
                   ...
                   vchrrefcobranza, fechalimpago, pagocomision, comision, folioplataforma, vchrfirma, chrestatusenvio, resfirm
              INTO cCveRastreo, cCuenta, mMonto, dFechaVal, cCtaBenef, cNumCte, cCtaBenefEmail, cTpoCtaBenefMsg, cTransacc, cFolioSuc,
                   ...
                   wvchrrefcobranza, wchrfechalimpago, wintpagocomision, wvcomision, wchrfolioplataf, wvchrfirma, wchrstatus, codretfirma
              FROM tblabono 
 ...
 */ -- SE COMENTA LA VALIDACION DE FIRMA 29 01 2026     
            ...
            IF codretfirma = 0 THEN
                ...
                IF vInicioDigitos_CreditoHip IS NOT NULL AND vInicioDigitos_CreditoHip != '' THEN
                    ...
                    LET vNumCtaDestino_inicioDigitos = substr(cCtaBenef,0,7);
                    
                    IF vInicioDigitos_CreditoHip = vNumCtaDestino_inicioDigitos THEN

                        LET vNumCuentaConcentradora_credHip = (SELECT valor FROM  bdicheq:sc_param WHERE codparam = 'cuentaconccredhip');

                        IF vNumCuentaConcentradora_credHip IS NOT NULL AND vNumCuentaConcentradora_credHip != '' THEN

                            LET vCuentaBeneficiadora_credHip = cCtaBenef;

                            LET cCuenta = vNumCuentaConcentradora_credHip;
                ...
                IF cCodRet5 = '000' THEN
                    ...
                    IF trim(cCuenta) = trim(vNumCuentaConcentradora_credHip) THEN
                    
                        LET vFechaOperacion = CURRENT HOUR TO FRACTION(3);

                        UPDATE bdicheq:sc_creditohipotecario 
                            SET status_envio = 'L', 
                                fecha = vFechaOperacion
                        WHERE cveRastreo = cCveRastreo 
                            AND monto = mMonto
                            AND clabe_referencia = vCuentaBeneficiadora_credHip
                    ...
                    /* ELIMINACION DE LA MODIFICACION TEMPORAL - DRP LATINIA 24/11/2023
					EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento_tmp_spei
                     ('1', 'SPEI_TRREC', 'SPEI_TRREC', cNumCte, '', '', '1', cCtaBenefMsg, cCtaBenefEmail, mMonto, cCveRastreo, cTpoCtaBenefMsg, '', '', '', '', '', '', '', 1, 0, 0, 0, 0, current, '')
                    INTO cCodRet6;*/
            ...
            ELSE
                ...
                IF vNumCuentaConcentradora_credHip = cCuenta THEN
                    ...
                    WHERE cveRastreo = cCveRastreo 
                        AND monto = mMonto
                        AND clabe_referencia = cCtaBenef
                        AND folio_suc = cFolioSuc;
                END IF;
...
'Autor: 90435114',
'Fecha: 24/09/2025',
'DescripciÃ³n: Se modifica el SP para liquidar el abono a la cuenta de credito hipotecario',
'se valida que el movimiento haya sido exitoso en bdicheq:sc:movdia',
'para que se actualice el status en la tabla bdicheq:sc_creditohipotecario',
'Sustento: DEF - Operaciones Hipotecario 2025 .docx',
'Solicita: Marcela Ozuna Reynosa',
'Autor Modificacion: MARIO GONZALEZ VAZQUEZ',
'Fecha: 29/01/2026',
'Descripcion: Se comenta en llamado a validacion de la firma y se modifica la consulta select a tblabono para agregar un nuevo campo que se asignara a la variable codretfirma que se le asignaria el resultado de la validacion',
'BD:BDISPEI';

CREATE PROCEDURE "informix".spei_reccancelacion( pvchrclaverastreo CHAR(30),       -- clave de rastreo
...
RETURNING CHAR(30); --- folio
    ...
    ON EXCEPTION SET vSqlErr, vIsamErr
        SET DEBUG FILE TO "/resplogifx/conciliachq/spei/spei_reccancelacion.err";
        TRACE ON;
        IF vSqlErr != 0 THEN
	...
	-- // El monto comision siempre llega a a 2 decimales 
	/* ################################
    IF wcomision = 0.00 THEN
	   LET vcomision = '0.0';
	ELSE
...
*/
	...
	IF codretfirma = 0 THEN
        ...
        IF pchartipopago IN('19', '20', '21', '22') THEN
           SELECT vchrconceptopago2, intcvetipoctaord, intcvetipoctabene, vchrnombrebenef, vchrcuentabenef, vchrnombreord,numcelord,numcelben,folioplataforma
             INTO pvchrConceptoPago, pvchrtpoctaord, pintTipoCtaBenef, pvchrNombreBenef, pvchrCuentaOrd, pvchrNombreOrd,pvchrCelOrd,pvchrCelBen,pvchridmjc    					   
        ...
        IF vcodret1 = '000' THEN
            ...
            IF pchartipopago IN('19', '20', '21', '22') THEN
               ...
               EXECUTE PROCEDURE spei_recerrorescodi('22', cVarDataErr,'b',pvchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,  
                                    pvchrclaverastreo,pintrefnumerica,pvchrCelOrd,pdigidord, intBancoOrd,pvchrtpoctaord,  
                                    pvchrcuentabenef,pvchrNombreOrd,pvchrCelBen,pdigidben,pintBancoDest,pintTipoCtaBenef,  
                                    pvchrCuentaOrd,pvchrNombreBenef,pnumseriecert) 
...
'Autor Modificacion: MARIO GONZALEZ VAZQUEZ',
'Fecha: 29/01/2026',
'Descripcion: Se comenta en llamado a validacion de la firma y se crea nuevo parametro al SP que contendra el resultado de la firma 0 correcto y 1 incorrecto y se asignara a la sig variable codretfirma = presfirm',
'BD:BDISPEI';

CREATE PROCEDURE "informix".spei_recdevolucion( pvchrclaverastreo CHAR(30),       -- clave de rastreo
                                                pvchrcuentabenef  CHAR(20),       -- numero de cuenta del beneficiario
                                                pmnyimporte       DECIMAL(17,2),  -- importe de la operaciÃ³n
                                                pintrefnumerica   CHAR(7),        -- referencia numÃ©rica
                                                pcausadevolucion  CHAR(2) ,       -- clave de devoluciï¿½??ï¿½?Â³n
                                                ptipo             CHAR(2),		  -- tipos de pago 19,20,21,22
												pcharfirma  	  CHAR(512), 	  -- firma a validar
												presfirm          INTEGER)   	  --resultado validacion firma 0 correcta 1 incorrecta 29 01 2026	  
...
RETURNING CHAR(30); -- folio
    ...
    --SET DEBUG FILE TO "/resplogifx/conciliachq/spei/spei_recdevolucion.out";
    --TRACE ON;
...
/* -- SE COMENTA LA VALIDACION DE FIRMA 29 01 2026
	--GENERA CADENA A VALIDAR
	LET wcadena_val = '|'||TRIM(pvchrclaverastreo)||'|'||TRIM(pvchrcuentabenef)||'|'||pmnyimporte||'|'||TRIM(pintrefnumerica)||'|'||TRIM(pcausadevolucion)||'|'||TRIM(ptipo)||'|';
...
*/ -- SE COMENTA LA VALIDACION DE FIRMA 29 01 2026
	...
	IF codretfirma = 0 THEN    
				IF ( ( pmnyimporte <= 0.00 ) OR 
					 ( LENGTH(TRIM(pvchrcuentabenef)) <> 16 AND LENGTH(TRIM(pvchrcuentabenef)) <> 11 AND LENGTH(TRIM(pvchrcuentabenef)) <> 18 AND LENGTH(TRIM(pvchrcuentabenef)) <> 10 ) ) THEN
					RETURN wfolio_suc;
				END IF;
				...
				IF wexiste is null OR wexiste = '' THEN
					RETURN wfolio_suc;
				END IF;
				}
				IF LENGTH(TRIM(pvchrcuentabenef)) = 11 THEN
				...
				IF vcodret1 = '000' THEN
					UPDATE bdispei:tblpago
					   SET chrestatusenvio = 'D',
						   intcvecausadev = pcausadevolucion
					 WHERE vchrclaverastreo = pvchrclaverastreo;      
				ELSE 
					LET wfolio_suc = '0';
				END IF;
	ELSE
			...
			--INSERT INTO tblintfallo (vchrcuentaord, dtfech_hor, vchrcuentabenef) VALUES(wcausa_dev,CURRENT, pvchrclaverastreo);
...
CREATE PROCEDURE "informix".spei_recextemporanea(pCveRastreo	CHAR(30),       -- clave de rastreo
										         pCtaOrdenante	CHAR(20),       -- numero de cuenta del ordenante
										         pMonto       	DECIMAL(17,2),  -- importe de la operaciÃ³n
										         pMontoInteres  DECIMAL(17,2),  -- importe de la comisiÃ³n del monto de la operacion
										         pRefNumerica 	CHAR(7),      	-- referencia numÃ³Â©rica
										         pConceptoPago	CHAR(210),      -- concepto de pago
										         pRefCobranza	CHAR(40), 		-- referencia cobranza	
												 pcharfirma	    CHAR(512),	    -- firma a validar
												 presfirm       INTEGER)		-- resultado validacion firma
RETURNING CHAR(30); -- folio origen
	...
	LET vnombre_cte		= '';
     ...
     --SET DEBUG FILE TO "/informix/Jess/spei_rec_extemporanea.out";
     --TRACE ON;

    BEGIN

    ON EXCEPTION SET vSqlErr, vIsamErr, vDescErr
        SET DEBUG FILE TO "/resplogifx/conciliachq/spei/spei_rec_extemporanea.out";
        TRACE ON;
        IF vSqlErr != 0 THEN
            LET vCodRet1    = vSqlErr;
            LET vCodRet2    = vIsamErr;
...
/* --SE COMENTA VALIDACION FIRMA 30 01 2026

	--GENERA CADENA A VALIDAR
	LET wcadena_val = '|'||TRIM(pCveRastreo)||'|'||TRIM(pCtaOrdenante)||'|'||pMonto||'|'||pMontoInteres||'|'||TRIM(pRefNumerica)||'|'
	||TRIM(pConceptoPago)||'|'||TRIM(pRefCobranza)||'|';
...
LET codretfirma = presfirm;
	...
	IF codretfirma = 0 THEN
					...
					--// OBTIENE LA TRANSACCION DEL ABONO X EL MONTO DE LA RECEPCION EXTEMPORANEA DEL SPEI
					SELECT vchrvalor
					INTO cTranAbono
					FROM "informix".tblparametros
					WHERE vchrcveparametro = "TRAN_ABO_DEV_EXT";
					
					--// OBTIENE LA TRANSACCION DEL ABONO X EL MONTO DEL INTERES DE LA RECEPCION EXTEMPORANEA DEL SPEI
					SELECT vchrvalor
					INTO cTranAbonoInt
					FROM "informix".tblparametros
					WHERE vchrcveparametro = "TRAN_ABO_INT_DEV_EXT";
					 
					IF cIndDisponible = '0' THEN
						LET cFolioOrigen = "0";
						RETURN cFolioOrigen;
					END IF;
					
					IF (pCveRastreo IS NULL OR pCveRastreo = "") OR (pCtaOrdenante IS NULL OR LENGTH(TRIM(pCtaOrdenante)) NOT IN (18,16,11,10)) OR
						(pMonto IS NULL OR pMonto <= 0.00 ) OR (pMontoInteres IS NULL OR pMontoInteres <= 0.00 ) OR (pRefNumerica IS NULL OR pRefNumerica = "") OR
						(pConceptoPago IS NULL OR pConceptoPago = "") OR (pRefCobranza IS NULL OR pRefCobranza = "") THEN
						LET cFolioOrigen = "0";
						RETURN cFolioOrigen; 
					END IF;
					
					IF LENGTH(TRIM(pCtaOrdenante)) = 11 THEN
						LET cCuenta = pCtaOrdenante;
						
						SELECT NVL(num_tarjeta, ' '), NVL(numcte, ' ')
						  INTO cNumTarjeta, cNumCte
						  FROM bdicheq:sc_tarjeta
						 WHERE empresa = "001"
						   AND cuenta = cCuenta
						   AND secuencia = ( SELECT MAX(secuencia)
						...
						IF ( cNumCte is null OR cNumCte = '' OR cNumCte = ' ' ) THEN
							...
							END IF;
						END IF;
					ELIF LENGTH(TRIM(pCtaOrdenante)) = 16 THEN 
						SELECT NVL(cuenta, ' '), NVL(numcte, ' ')
						  INTO cCuenta, cNumCte
						  FROM bdicheq:sc_tarjeta
						 WHERE empresa = "001"
						   AND num_tarjeta = pCtaOrdenante;
						...
						LET cNumTarjeta = pCtaOrdenante;
						
					ELIF LENGTH(TRIM(pCtaOrdenante)) = 18 THEN
						LET cCuenta = SUBSTR(pCtaOrdenante, 7, 11);
						...
						IF ( cNumCte is null OR cNumCte = '' OR cNumCte = ' ' ) THEN
							...
							END IF;
						END IF;
					ELIF LENGTH(TRIM(pCtaOrdenante)) = 10 THEN
						SELECT cuenta, num_cte
						  INTO cCuenta, cNumCte
						  FROM bdicheq:sc_cuenta_telefono
						 WHERE telefono = pCtaOrdenante;
						 
						LET cNumTarjeta = '';
						   
						IF cCuenta is null OR cCuenta = '' THEN
							SELECT cuenta_tf, num_tarjeta, numcte
							  INTO cCuenta, cNumTarjeta, cNumCte
							  FROM bditransfer:tf_maecte
							 WHERE telefono = pCtaOrdenante
							   AND status_cta = '1';
						END IF;
					...
					AND cuenta = cCuenta
					AND cancelad <> 'S'
					AND referencia = pCveRastreo
					AND transacc = cTranAbono;
					   
					IF iExisteMov > 0 THEN
					...
					EXECUTE PROCEDURE "informix".sp_obtfoliosuc(cUsuario)
					INTO vcodret1, iSerialFolio, cFolioOrigen;
					...
					--// ABONO POR EL MONTO DE LA RECEPCION EXTEMPORANEA DEL SPEI
					EXECUTE PROCEDURE bdicheq:"informix".abono_ref( "001", cSucursal, cUsuario, cTranAbono, cTranSuc, cFolioOrigen, cCuenta, 0, pMonto, pMonto, 0, 0, 0, cDivisa, pCveRastreo, cNumTarjeta, '')
					INTO vcodret1;
					...
					IF iCodRet = 0 THEN
						--// ABONO POR EL MONTO DEL INTERES DE LA RECEPCION EXTEMPORANEA DEL SPEI
						EXECUTE PROCEDURE bdicheq:"informix".abono_ref( "001", cSucursal, cUsuario, cTranAbonoInt, cTranSuc, cFolioOrigen, cCuenta, 0, pMontoInteres, pMontoInteres, 0, 0, 0, cDivisa, pCveRastreo, cNumTarjeta, '')
						INTO vcodret1;
					ELSE
						LET cFolioOrigen = '0';
					END IF;
	ELSE
			...
			INSERT INTO tblintfallo (vchrcuentaord, dtfech_hor, vchrcuentabenef) VALUES(pCtaOrdenante,CURRENT, pCtaOrdenante);
...
'Autor Modificacion: MARIO GONZALEZ VAZQUEZ',
'Fecha: 30/01/2026',
'Descripcion: Se comenta en llamado a validacion de la firma, se agrega un nuevo parametro para capturar resultado firma y se le asigna ese valor a la siguiente variable codretfirma = presfirm',
'BD:BDISPEI';
