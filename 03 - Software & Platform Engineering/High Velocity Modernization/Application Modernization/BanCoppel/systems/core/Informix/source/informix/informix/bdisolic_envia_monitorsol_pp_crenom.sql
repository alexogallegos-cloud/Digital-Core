CREATE PROCEDURE "informix".envia_monitorsol_pp_crenom
                            (
...
RETURNING 	CHAR(6)         AS CodRetorno,  	-- Codigo de retorno
			...
			INTEGER		    AS DiaPago;	 --dia de pago de nomina
...
-- Modificacion: Se modifica para que contemple obtener información de la nueva tabla donde se guarda información de las solicitudes del producto credinomina. 
-- Fecha: 2011/05/19
-- MODIFICO: Jesús Manuel Aguilar Heredia
-- Modificacion: Se le cambia el nombre para que pueda ser usado tanto el procedimiento productivo como la version creada para credinomina.
-- Fecha: 2011/09/13
-- MODIFICO: Diego Guerra Atienzo
-- Modificacion: Se modifican métodos de  búsqueda para eliminar error 24 en búsqueda por cliente
-- Fecha: 2012/02/23
-- ****************************************************************************
...
ON EXCEPTION SET isqlerr, iIsamErr, cErrorInfo
   IF isqlerr != 0 THEN
		...
		RETURN NVL(cCodRet,""), NVL(cMens_Ret,""), NVL(cNumSol,""), NVL(cNumCte,""), NVL(cNombre,""),
			...
			NVL(cCuenta_eje,""), NVL(iFrecuencia,0), NVL(iDiaPago,0);
   END IF;
END EXCEPTION;

--SET DEBUG FILE TO '/pisa/cas/envia_monitorsol_pp_crenom.out';
--TRACE ON;
    ...
    IF pNumcte IS NULL OR pNumcte = "" THEN
		FOREACH WITH HOLD
		 SELECT x.descripcion,
				...
				nom.cuenta, nom.Frecuencia_pgo,nom.dia_pago
		   INTO cStDesc,
				...
				cCuenta_eje,iFrecuencia,iDiaPago
		   FROM "informix".ss_solicitudes a
	 INNER JOIN bdinteg:"informix".si_cliente b          ON (a.numcte  = b.numcte  AND a.empresa = a.empresa)
	 INNER JOIN "informix".ss_resum_scor_fin c  ON (c.empresa = a.empresa AND c.num_solicitud = a.num_solicitud)
	 INNER JOIN "informix".ss_anexosol d        ON (d.empresa = a.empresa AND d.num_solicitud = a.num_solicitud)
	 INNER JOIN "informix".ss_status_sol x      ON (x.empresa = a.empresa AND x.status_solicitud= a.status_solicitud)
	 LEFT JOIN  "informix".ss_sol_nomina nom ON (nom.empresa = a.empresa AND nom.num_solicitud = a.num_solicitud)		
	 LEFT JOIN "informix".ss_autorizacion e    ON (e.empresa = a.empresa AND e.num_solicitud = a.num_solicitud
	 ...
	 INNER JOIN bdicred:"informix".sd_definicion f      ON (f.num_producto = a.num_producto AND f.empresa = a.empresa)
	  ...
	  ORDER BY b.Nombre1, b.Nombre2, b.apell_paterno, b.apell_materno, e.fecha_entrada
			...
			RETURN NVL(cCodRet,""), NVL(cMens_Ret,""), NVL(cNumSol,""), NVL(cNumCte,""), NVL(cNombre,""),
				   ...
				   NVL(cProdDesc,""), NVL(mLinea,0), NVL(cStatus,""), NVL(cStDesc,""),
				   NVL(cComentario,""), NVL(cDiaCorte,""), NVL(cDivisa,""), NVL(mIngreso,0), NVL(iCuentaCap,0),
				   NVL(cCuenta_eje,""), NVL(iFrecuencia,1), NVL(iDiaPago,0) WITH RESUME;
		END FOREACH;
