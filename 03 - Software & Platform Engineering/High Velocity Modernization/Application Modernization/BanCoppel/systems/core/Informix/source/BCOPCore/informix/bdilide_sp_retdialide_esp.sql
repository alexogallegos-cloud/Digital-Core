CREATE PROCEDURE "informix".sp_retdialide_esp(pEmpresa CHAR(3),
                                              dFecha date,
                                              cUsuario char(10))
...
RETURNING VARCHAR(4),VARCHAR(80), char(11);
    ...
    -- **************************************************************
    -- *	Realizo: Aymme Osuna Peraza                             *
    -- *	Actividad: Realizar redondeo en el cobro del IDE        *
    -- *	Solicito:Aymme Osuna                                    *
    -- *	Fecha: 06/NOV/2008                                      *
    -- *	Modifico: Anselmo Verdugo Gastelum                      *
    -- *	Fecha modificacion: 10/DIC/2008                         *
    -- *    Ultima Modificacion: Bancoppel DIC 2009                 *
    -- **************************************************************
    ...
    ON EXCEPTION SET SQL_ERR, ISAM_ERR, ERROR_INFO
        ...
        IF SQL_ERR < 0 THEN
            IF vcEsCargoDebito = 'S'  THEN
                ...
                ELSE
                    LET ERROR_INFO = 'NO SE PUDO HACER REVERSION DE ULTIMO CARGO PARA DEBITO';
                END IF;
    ...
    --- SET DEBUG FILE TO "sp_retdialide_esp.out";
    --- TRACE ON;
    ...
    IF dFecha = vdUltimoDiaMes THEN
        -- // Verificar si el proceso de "acu_men_op" se ejecuto antes para poder hacer la retencion.
        SELECT status
          INTO vcStatus2
        ...
        IF vcStatus2 = 0  or  vcStatus2 IS NULL THEN
            -- // "33333" indica que No se ejecuto el proceso anterior o que fallo en la ejecucion
            RETURN "333",'','';
        END IF;
    ELIF dFecha <> vdUltimoDiaMes THEN
        ...
        IF vcStatus2 = 0 or  vcStatus2 IS NULL THEN
            -- // "22222" indica que el proceso de extraccion de operaciones en efectivo 
            -- // con tarjeta de credito NO se ha ejecutado o la ejecucion fue insatisfactoria.
            RETURN "222",'','';
        END IF;
    END IF;
    ...
    SELECT {+INDEX(bdilide:sl_retlide idx_retcte)}
           'D',chq.num_cte,chq.cuenta,(chq.sdo_actual - chq.sdo_retenido - chq.sdo_cong - imp_chq_sbg - chq.saldo_sbc),
    ...
    FOREACH
        ...
        SELECT sdo_actual, sdo_retenido, sdo_cong, imp_chq_sbg, saldo_sbc
          INTO vsdo_cta , vsdo_ret, vsdo_cong, vimp_chq_sbg, mSaldoSbc
        ...
        IF mRecaudar >= mSaldo_ant THEN
            ...
            -- // Obtiene lo que esta pendiente a recaudar para que
            -- // se lo cobre de otra cuenta pero aun no aplica el cargo
        ...
        IF vmImporteCargo > 0 THEN
            -- // Se obtiene Sucursal donde se dio de alta la cuenta.
            SELECT sucursal
              INTO vcSucursal
              FROM bdicheq:sc_maechq
             WHERE empresa = pEmpresa
            ...
            -- // Obtener el numero de folio para efectuar el cargo
            ...
            CALL bdicheq:cargo_ref( vcEmpresa, vcSucursal, cUsuario, vcTransaccion, vcTransaccSuc, vcFolioSuc,	
                                    cCuenta, 0,	vmImporteCargo,	vcDivisa, '', vcNumTarjeta,	cUsuario)
            RETURNING vcCodRetTemp,	vtranret, vfechoy, vsdodisp, vmontoret;
            ...
            IF TRIM(vcCodRetTemp) = "000" THEN
                ...
                INSERT INTO bdilide:sl_detlide(aniomes,num_cte,consecutivo,rfc,ref_ret,cuenta_ret,
                                               fecha_ret,imp_recaudado,user_insert,fecha_insert)
                VALUES(vcAnioMes, cNumcte, viConsecutivo, vcRfc, vcRefRet,
                cCuenta, dFecha, vmImporteCargo, cUsuario, CURRENT::DATE);
            ...
            ELSE
                ...
                { *********************************************************
                --- LET P_MENSAJE = "ERROR AL EFECTUAR EL CARGO PARA CHEQUES.";
                ...
                --- RETURN  vcCodRetTemp, P_MENSAJE, cCuenta;
                ********************************************************** }
            END IF;
        END IF;
    ...
    FOREACH WITH HOLD
        ...
        -- // Ciclo para intentar recaudar en cada cuenta de debito del cliente
        FOREACH WITH HOLD
            ...
            IF vcTipo = "D" THEN
                -- // Obtiene el Saldo actual de la cuenta de debito
                --RQM 09 704. Se agrega el campo saldo_sbc en la consulta. EEAP.
                SELECT sdo_actual, sdo_retenido, sdo_cong, imp_chq_sbg, saldo_sbc, sucursal
                  INTO vsdo_cta , vsdo_ret, vsdo_cong, vimp_chq_sbg, mSaldoSbc, vcSucursal
                  FROM bdicheq:sc_maechq
                 WHERE empresa = pEmpresa
                   AND cuenta = cCuenta;
                --- WHERE cuenta = cCuenta and num_cte = cNumcte;
                ...
                ******************************************************** }
                ...
                -- // Obtener el n̼mero de folio para efectuar el cargo
                LET vcFolioSuc = "inform" || replace (substring (current FROM 12  FOR 8 ), ':', '')  || vcTransaccion;

                -- // Se manda llamar el cargo
                IF vmImporteCargo > 0  then

                    CALL bdicheq:cargo_ref(vcEmpresa, vcSucursal, cUsuario, vcTransaccion, vcTransaccSuc,
                                           vcFolioSuc, cCuenta, 0, vmImporteCargo, vcDivisa, '', vcNumTarjeta, cUsuario )
                    RETURNING vcCodRetTemp, vtranret, vfechoy, vsdodisp, vmontoret;
                    ...
                    ELSE
                        ...
                        --- ROLLBACK WORK;
                        --- DESHABILITAR PARA QUE NO PERMIA RETIROS EN EL PRODUCTO INVERSION CRECIENTE
                        --- UPDATE bdicheq:sc_producto SET per_retiros = 'U 0' WHERE producto = '1100';
                        --- RETURN vcCodRetTemp, P_MENSAJE, cCuenta;
                        ************************************************ }
                    END IF;
                END  IF;
            ...
            ELIF vcTipo = "C" THEN
                ...
                -- // Obtiene el Saldo actual de la cuenta de credito
                ...
                IF vmImporteCargo > 0  THEN
                    -- // Manda llamar el proceso de cargo
                    CALL bdicred:cargo_cred(vcEmpresa, cCuenta,vcSucursal, cUsuario, vcTransaccSucCred,
                                            vmImporteCargo,vcFolioSuc,vcNumTarjeta,0.00,0.00, '12172009',vcRefRet,'','' )
                    RETURNING vcCodRetTemp;
                    ...
                    ELSE
                        ...
                        { *************************************************
                        --- LET P_MENSAJE = "ERROR AL EFECTUAR EL CARGO PARA CREDITO";
                        --- ROLLBACK WORK;
                        -- // DESHABILITAR PARA QUE NO PERMIA RETIROS EN EL PRODUCTO INVERSION CRECIENTE
                        --- UPDATE bdicheq:sc_producto SET per_retiros = 'U 0' WHERE producto = '1100';
                        --- RETURN  vcCodRetTemp, P_MENSAJE, cCuenta;
    ...
    IF P_COD_RET = "000" THEN
        ...
        IF P_COD_RET = "000" THEN
            --- BEGIN WORK;
            -- // SE DESHABILITA LA LLAMADA A LA GENERACION DEL REPORTE
            --- CALL bdilide:sp_repRecaudacion(dFecha, cUsuario) RETURNING P_COD_RET;
            IF P_COD_RET = "000" THEN
                ...
                --- VALUES(pEmpresa, 'Redilide', dFecha, '23', 'F', cUsuario, current hour to fraction(3),
                ---        current hour to fraction(3), P_COD_RET);
                ************************************************************************************** }

                --- COMMIT WORK;
                LET P_MENSAJE = "PROCESO FINALIZO EXITOSAMENTE.";
            ELSE
                -- // PONER TODOS LOS PROCESOS ENVOLUCRADOS EN ESTADO CERO PARA INDICAR EL EXITO NO OBTENIDO
                UPDATE bdilide:sl_procesos
                   SET status = "0"
                 WHERE fech_proceso = dFecha
                   AND proceso  = "ret_dialde";
