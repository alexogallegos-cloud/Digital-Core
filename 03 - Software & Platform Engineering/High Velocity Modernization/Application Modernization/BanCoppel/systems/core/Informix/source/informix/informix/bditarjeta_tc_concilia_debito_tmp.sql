CREATE PROCEDURE "informix".tc_concilia_debito_tmp(pEmpresa CHAR(16), 
					       pArchivo CHAR(12))
...
BEGIN
 ...
 --SET DEBUG FILE TO "/pisa/pisabanco/pisa_ftes/Tichan/conciliacion/conciliacredito/tc_concilia_debito_tmp.out";
 --TRACE ON;
	...
	FOREACH WITH HOLD
			...
			SELECT 						a.cuenta,		b.sucursal,			b.usuario,
					...
					a.moneda,			a.referencia,	a.folio_original,	a.rfc_comer,
		           	a.referencia23,
		           	b.archivo,			a.consecutivo,	b.fecha,			"td_conposvnd",
		    ...
		    FROM td_conposvnd a, td_conciliaarchivos b
			...
			SELECT 						a.cuenta,		b.sucursal,			b.usuario,
					...
					a.moneda,			a.referencia,	a.folio_original,	a.rfc_comer,
		           	a.referencia23,
		           	b.archivo,			a.consecutivo,	b.fecha,			"td_conposvid",
		           	a.divisa	, a.monto_divisa,	a.num_cajero,	a.forma_pago
		    FROM td_conposvid a, td_conciliaarchivos b
			WHERE a.empresa = pEmpresa	AND a.bandera_proceso ="0"
			  AND b.empresa = a.empresa AND b.archivo = a.archivo
			  AND b.fecha   = a.fecha	AND b.archivo = pArchivo
			...
			SELECT 						a.cuenta,		b.sucursal,			b.usuario,
					...
					a.moneda,			a.referencia,	a.folio_original,	a.rfc_comer,
		           	a.referencia23,
		           	b.archivo,			a.consecutivo,	b.fecha,			"td_conatmd",
		           	a.divisa	, a.monto_divisa,	a.num_cajero,	a.forma_pago
		    FROM td_conatmd a, td_conciliaarchivos b
			WHERE a.empresa = pEmpresa	AND a.bandera_proceso ="0"
			  AND b.empresa = a.empresa AND b.archivo = a.archivo
			  AND b.fecha   = a.fecha	AND b.archivo = pArchivo
		   ...
		   EXECUTE PROCEDURE bdicheq:conciliadebito_tmp
		   		 (
			...
			--Compra en comercios nacionales VND
			IF v_tabla IN ('td_conposvnd') THEN
				LET v_transparencia = "";

			--Compras en comercios internacionales VID
			ELIF v_tabla IN ('td_conposvid')  THEN
				LET v_transparencia = TRIM(v_monto_divisa::CHAR(15))||' '|| v_divisa;

			--Retiros en Cajeros ATMC
			ELIF v_tabla IN ('td_conatmd')  THEN
			...
			IF TRIM(v_transparencia) <> "" THEN
				IF vBandera = 'C' THEN
					...
					SELECT cuenta INTO v_cuenta_cliente FROM bdicheq:sc_tarjeta
					WHERE empresa = pEmpresa AND num_tarjeta = v_cuenta;
				
				
					UPDATE bdicheq:sc_movhis SET referencia = TRIM(referencia) ||' '|| v_transparencia
					WHERE fech_alt > DATE(1) AND transacc = v_tran_central 
					AND empresa = pEmpresa AND cuenta = v_cuenta_cliente
					AND cancelad <> "S";
				
				ELIF vBandera = 'A' THEN
				
					UPDATE bdicheq:sc_movdia SET referencia = TRIM(referencia) ||' '|| v_transparencia
					WHERE folio_suc  = v_folio_mov AND empresa = pEmpresa;
			...
			-- VENTAS INTERNACIONALES DEBITO
			ELIF  v_tabla = "td_conposvid" THEN
				UPDATE td_conposvid	
				SET bandera_proceso = vBandera,cod_retorno = cod_ret,
					fecha_aplica = vFechaHoy
				WHERE empresa = pEmpresa	AND archivo = v_archivo
			...
			--	ATM
			--------------------------------------------------------
			-- VENTAS RETIROS DEBITO
			ELIF  v_tabla = "td_conatmd" THEN
				UPDATE td_conatmd	
				SET bandera_proceso = vBandera,cod_retorno = cod_ret,
					fecha_aplica = vFechaHoy
				WHERE empresa = pEmpresa	AND archivo = v_archivo
...
END PROCEDURE
DOCUMENT
'ESTA FUNCION BUSCA LOS ARCHIVOS PARA LA CONCILIACION DE TARJETA DE DEBITO ',
