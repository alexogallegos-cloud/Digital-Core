CREATE PROCEDURE "informix".sp_depura_bitacora_tokenizacion_otp()
...
LET filePath                         = '';
    ...
    BEGIN
        ...
        SELECT COUNT(*)
           INTO conteo
        FROM bitacora_tokenizacion_otp
            WHERE fecha_insert < fechaInicioMes;
            
        IF conteo = 0 THEN
            LET codigo_retorno = "00002";
        ELSE
            LET nombreArchivo ='Respaldo_bitacora_tokenizacion_otp_' || anio|| mes || primerDiaMes ;
            LET filePath='"/RESPALDOSNEW/Tokenizacion/file_temp/'|| nombreArchivo ||'.unl" ';

            LET query = 'SELECT * FROM bitacora_tokenizacion_otp WHERE fecha_insert < '||'"'|| fechaInicioMes ||'"';           
           ...
           DELETE FROM bitacora_tokenizacion_otp
             WHERE fecha_insert < fechaInicioMes;
