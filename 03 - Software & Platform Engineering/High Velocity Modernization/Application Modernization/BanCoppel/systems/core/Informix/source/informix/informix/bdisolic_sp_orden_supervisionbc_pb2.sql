CREATE PROCEDURE "informix".sp_orden_supervisionbc_pb2(lsecuencia INTEGER)
RETURNING 
char(5) as CodRet,
...
BEGIN
	...
	FOREACH
	Select nvl(num_solicitud,' '), nvl(estatusos,' '), nvl(observacion1,' '), nvl(observacion2,' '), nvl(tiendafolio,0),
	...
	INTO cNum_Solicitud, iEstatusos, cObservacion1,cObservacion2,iTiendaFolio, 
	...
	Where (nvl(clave, '') <> 'R' and nvl(clave, '') <> 'A') and secuencia > lsecuencia
	AND fechasolicitud > mdy('01','01','2018') AND secuencia < '8000000' -- Se tomaran solo las solicitudes de este a̱o y con secuencia menor a la que se tiene actualmente en el puente 
	order by secuencia
	...
	cCurp, cClaveidentificacion, cIdentificacion, cTelefonocelular, cTiposueldo, cNombrereferencia, 
	cApellidopaternoreferencia, cApellidomaternoreferencia, dTelefonoreferencia, cTelefonocelularreferencia, cEmpresa WITH RESUME;
	END FOREACH;
END
END PROCEDURE
