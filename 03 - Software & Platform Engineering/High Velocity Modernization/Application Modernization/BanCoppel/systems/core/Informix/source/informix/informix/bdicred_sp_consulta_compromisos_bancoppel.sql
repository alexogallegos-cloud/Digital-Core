CREATE PROCEDURE "informix".sp_consulta_compromisos_bancoppel(pEmpresa CHAR(4), pNumCte CHAR(9), pGen1 CHAR(50))
RETURNING 	CHAR(4) as codRet,
			CHAR(50) as mensaje,
			CHAR(4)  as num_producto,
			CHAR(20) as num_credito,
			DATE as fecha_ult_mov,
			DECIMAL(18,2) as sdo_int_anticip,
			DECIMAL(18,2) as sdo_intereses,
			DECIMAL(18,2) as sdo_moratorio,
			DECIMAL(18,2) as sdo_capital,
			DECIMAL(18,2) as sdo_cap_insoluto,
			DECIMAL(18,2) as sdo_acum_mes_cap,
			DECIMAL(18,2) as mto_capitalizado,
			DECIMAL(18,2) as cargos_mes_cap,
			DECIMAL(18,2) as abonos_mes_cap,
			DECIMAL(18,2) as monto_vencido,
			DECIMAL(18,2) as mto_venc_trasp,
			DECIMAL(18,2) as monto_financiado,
			DECIMAL(18,2) as sdo_acum_vencido,
			DECIMAL(18,2) as monto_otorgado,
			DECIMAL(18,2) as mto_venc_int,
			DECIMAL(18,2) as mto_venc_tra_int,
			DECIMAL(18,2) as mto_finan_vdo,
			INTEGER as atr,
			INTEGER as act,
			DATE as fecha_pago,
			CHAR(2) as status_credito,
			DECIMAL(18,2) as capital_mto_cuota,
			DATE as fecha_apertura,
			INTEGER as gen1,
			DECIMAL(18,2) as gen2,
			CHAR(20) as gen3;
	...
	DEFINE v_sdo_int_anticip DECIMAL(18,2);
	...
	DEFINE v_mto_capitalizado DECIMAL(18,2);
	...
	DEFINE v_sdo_acum_vencido DECIMAL(18,2);
	DEFINE v_monto_otorgado DECIMAL(18,2);
	DEFINE v_mto_venc_int DECIMAL(18,2);
	...
	DEFINE v_capital_mto_cuota DECIMAL(18,2);
	DEFINE v_fecha_apertura DATE;
	DEFINE gen1 INTEGER;
	DEFINE gen2 DECIMAL(18,2);
	DEFINE gen3 CHAR(20);
	
	DEFINE c_numcte CHAR (15);
	DEFINE v_no_reg INTEGER;
	...
	LET v_num_producto = 'X';
	LET v_num_credito = 'X';
	LET v_fecha_ult_mov = DATE(1);
	LET v_sdo_int_anticip = -1;
	LET v_sdo_intereses = -1;
	LET v_sdo_moratorio = -1;
	LET v_sdo_capital = -1;
	LET v_sdo_cap_insoluto = -1;
	LET v_sdo_acum_mes_cap = -1;
	LET v_mto_capitalizado = -1;
	LET v_cargos_mes_cap = -1;
	LET v_abonos_mes_cap = -1;
	LET v_monto_vencido = -1;
	LET v_mto_venc_trasp = -1;
	LET v_monto_financiado = -1;
	LET v_sdo_acum_vencido = -1;
	LET v_monto_otorgado = -1;
	LET v_mto_venc_int = -1;
	LET v_mto_venc_tra_int = -1;
	LET v_mto_finan_vdo = -1;
	LET v_atr = -1;
	LET v_act = -1;
	LET v_fecha_pago = DATE(1);
	LET v_status_credito = 'X';
	LET v_capital_mto_cuota = -1;
    ...
    ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
        IF iSqlErr != 0 THEN
            LET cCodRet = iSqlErr;
            LET cMensaje = 'Ocurrio error al consultar los saldos'||' - '||cErrorInfo;
            RETURN cCodRet, cMensaje, v_num_producto, v_num_credito, v_fecha_ult_mov, v_sdo_int_anticip, v_sdo_intereses, v_sdo_moratorio, v_sdo_capital, v_sdo_cap_insoluto, v_sdo_acum_mes_cap, v_mto_capitalizado, v_cargos_mes_cap, v_abonos_mes_cap, v_monto_vencido, v_mto_venc_trasp, v_monto_financiado, v_sdo_acum_vencido, v_monto_otorgado, v_mto_venc_int, v_mto_venc_tra_int, v_mto_finan_vdo, v_atr, v_act, v_fecha_pago, v_status_credito, v_capital_mto_cuota, v_fecha_apertura, gen1, gen2, gen3;
        END IF;
    END EXCEPTION;
	
	--SET debug file to '/home/e10001925/RQM-09716/sp_consulta_compromisos_bancoppel-' ||TRIM(pNumCte)|| '.out';
	--trace on;
	...
	IF (NVL(c_numcte, '') = '')THEN
		SELECT numcte
		INTO c_numcte
		FROM bdinteg:"informix".si_cliente
		WHERE numcte_ref = pNumCte;
	END IF;
	
	IF (NVL(c_numcte, '') = '')THEN
		LET cCodRet = '0002';
		LET cMensaje = 'NO SE ENCONTRO NO. DE CLIENTE';
		RETURN cCodRet, cMensaje, v_num_producto, v_num_credito, v_fecha_ult_mov, v_sdo_int_anticip, v_sdo_intereses, v_sdo_moratorio, v_sdo_capital, v_sdo_cap_insoluto, v_sdo_acum_mes_cap, v_mto_capitalizado, v_cargos_mes_cap, v_abonos_mes_cap, v_monto_vencido, v_mto_venc_trasp, v_monto_financiado, v_sdo_acum_vencido, v_monto_otorgado, v_mto_venc_int, v_mto_venc_tra_int, v_mto_finan_vdo, v_atr, v_act, v_fecha_pago, v_status_credito, v_capital_mto_cuota, v_fecha_apertura, gen1, gen2, gen3;
	END IF;
	
	FOREACH WITH HOLD 
		SELECT num_credito, num_producto, status_cred, fecha_apertura 
		INTO v_num_credito, v_num_producto, v_status_credito, v_fecha_apertura 
		FROM "informix".sd_maecred 
		WHERE numcte = c_numcte AND  
		status_cred NOT IN ("FF","FM","FR","FE","CC","FC","CV")
			...
			SELECT NVL(capital_mto_cuota,-1)
			INTO v_capital_mto_cuota
			FROM "informix".sd_amortiza_credito 
			WHERE empresa = pEmpresa AND
			num_credito = v_num_credito AND
			fecha_cuota = (SELECT MAX(fecha_cuota)
							FROM "informix".sd_amortiza_credito 
							WHERE empresa = pEmpresa AND
							num_credito = v_num_credito);
			
			SELECT 	NVL(fecha_ult_mov, DATE(1)),
					NVL(sdo_int_anticip,-1),
					NVL(sdo_intereses,-1),
					NVL(sdo_moratorio,-1),
					NVL(sdo_capital,-1),
					NVL(sdo_cap_insoluto,-1),
					NVL(sdo_acum_mes_cap,-1),
					NVL(mto_capitalizado,-1),
					NVL(cargos_mes_cap,-1),
					NVL(abonos_mes_cap,-1),
					NVL(monto_vencido,-1),
					NVL(mto_venc_trasp,-1),
					NVL(monto_financiado,-1),
					NVL(sdo_acum_vencido,-1),
					NVL(monto_otorgado,-1),
					NVL(mto_venc_int,-1),
					NVL(mto_venc_tra_int,-1),
					NVL(mto_finan_vdo,-1),
					-1,
					NVL(act, -1)
			INTO
		...
		RETURN cCodRet, cMensaje, v_num_producto, v_num_credito, v_fecha_ult_mov, v_sdo_int_anticip, v_sdo_intereses, v_sdo_moratorio, v_sdo_capital, v_sdo_cap_insoluto, v_sdo_acum_mes_cap, v_mto_capitalizado, v_cargos_mes_cap, v_abonos_mes_cap, v_monto_vencido, v_mto_venc_trasp, v_monto_financiado, v_sdo_acum_vencido, v_monto_otorgado, v_mto_venc_int, v_mto_venc_tra_int, v_mto_finan_vdo, v_atr, v_act, v_fecha_pago, v_status_credito, v_capital_mto_cuota, v_fecha_apertura, gen1, gen2, gen3 WITH RESUME;
		
	END FOREACH;
	
	LET v_no_reg = DBINFO("sqlca.sqlerrd2");
	...
	LET v_no_reg = v_no_reg + DBINFO("sqlca.sqlerrd2");
	
	IF v_no_reg = 0 THEN
		LET cCodRet = '0003';
		LET cMensaje = 'NO SE ENCONTRO INFORMACION';
		RETURN cCodRet, cMensaje, v_num_producto, v_num_credito, v_fecha_ult_mov, v_sdo_int_anticip, v_sdo_intereses, v_sdo_moratorio, v_sdo_capital, v_sdo_cap_insoluto, v_sdo_acum_mes_cap, v_mto_capitalizado, v_cargos_mes_cap, v_abonos_mes_cap, v_monto_vencido, v_mto_venc_trasp, v_monto_financiado, v_sdo_acum_vencido, v_monto_otorgado, v_mto_venc_int, v_mto_venc_tra_int, v_mto_finan_vdo, v_atr, v_act, v_fecha_pago, v_status_credito, v_capital_mto_cuota, v_fecha_apertura, gen1, gen2, gen3;
	END IF;
	END;
END PROCEDURE
DOCUMENT
'PROYECTO: RQM 09 716 Capacidad de Pago PP Coppel(Compromisos Banco)',
'DESCRIPCION: CONSULTAR PRODUCTOS Y SALDOS ACTIVOS DEL CLIENTE BANCOPPEL.',
'AUTOR: LUIS ANGEL GARCIA GAYOSSO',
'BD: BDICRED',
'FECHA: 00/10/2025',
'SOLICITA: MARIA NEREYDA DE LA O BELTRAN';

CREATE PROCEDURE "informix".sp_consultarctesincrementolincred_web(pEmpresa CHAR(3),
																pNumCte CHAR(20),
																pRFC CHAR(13),
																pNumTarjeta CHAR(20),
...
--SET DEBUG FILE TO "/informix/jesus/sp_consultarctesincrementolincred.out";
--TRACE ON ;
       ...
       IF (NVL(pNumTarjeta,"") <> "" OR NVL(pRFC,"") <> "" OR NVL(pNumCte,"") <> "") THEN
				...
				*/
                ...
                IF (cNumCte IS NULL OR cNumCte = '') THEN
                    LET cCodret = '00453';
                    LET cMensaje = 'No existe cliente con los parÃ¡metros especificados';
                    LET cIsCtePros = 'N';
                    LET cNumCte = '';
                    LET cNombre = '';
                    LET cRFC = '';
...
DOCUMENT 
'Realiza la selecciÃ³n de Clientes prospectos por diferentes filtros de bÃºsqueda',
...
CREATE PROCEDURE "informix".sp_consultaoperacionesmc(pProducto CHAR(4), pFechainicial CHAR(10), pFechafinal CHAR(10))
...
DEFINE iSolicMC               INTEGER;
DEFINE iNoAnali               INTEGER;
DEFINE iAnali                 INTEGER;
DEFINE iRevalu                INTEGER;
DEFINE iNoRevalu              INTEGER;
DEFINE iSiguePro              INTEGER;
DEFINE dPorcenSP              DECIMAL (18,2);
DEFINE iRecha                 INTEGER;
DEFINE iPorceRT               DECIMAL (18,2);
DEFINE iCancel                INTEGER;
DEFINE dPorcenCN              DECIMAL (18,2);
DEFINE iMIXTA				  INTEGER;
DEFINE iUNICA				  INTEGER;
DEFINE iTotalMix				  INTEGER;
DEFINE iTotalUni				  INTEGER;
...
LET iAnalizadas            = 0;
LET iRevaluadas            = 0;
LET iNoRevaluadas          = 0; --SIEMPRE EN 0 A PETICION DEL CLIENTE
LET iSigueProceso          = 0; --SIEMPRE EN 0 A PETICION DEL CLIENTE
LET dPorcentajeSP          = 0.00;
LET iRechazadas            = 0;
LET dPorcentajeRT          = 0.00;
LET iCanceladas            = 0;
LET dPorcentajeCN          = 0.00;
LET iSolicMC               = 0;
LET iNoAnali               = 0;
LET iAnali                 = 0;
LET iRevalu                = 0;
LET iNoRevalu              = 0; --SIEMPRE EN 0 A PETICION DEL CLIENTE
LET iSiguePro              = 0; --SIEMPRE EN 0 A PETICION DEL CLIENTE
LET dPorcenSP              = 0.00;
LET iRecha                 = 0;
LET iPorceRT               = 0.00;
LET iCancel                = 0;
...
BEGIN
	...
	ON EXCEPTION SET iSql_err , iIsamErr, cErrorInfo
		IF iSql_err <> 0 THEN
			LET cCodret = iSql_err;
			LET cMensaje = cErrorInfo;
			RETURN TRIM(cCodret), TRIM(cMensaje), NVL(cFecha, '1900-01-01'), NVL(cNombreProducto, ''), NVL(iSolicitudesMC, 0), NVL(iNoAnalizadas, 0), NVL(iAnalizadas, 0), NVL(iRevaluadas, 0), NVL(iAnalizadas-iRevaluadas, 0), NVL(iSigueProceso, 0), NVL(dPorcentajeSP, 0.00), NVL(iRechazadas, 0), NVL(dPorcentajeRT, 0.00), NVL(iCanceladas, 0), NVL(dPorcentajeCN, 0),iMIXTA,iUNICA;
		END IF;
	END EXCEPTION;
	
	
	--SET DEBUG FILE TO '/dbexportb/carlos/sp_consultaoperacionesmc.out';
	--TRACE ON;
	...
	IF NVL(pFechainicial, '') = '' OR NVL(pFechafinal, '') = '' OR LENGTH(pFechainicial) < 10 OR  LENGTH(pFechafinal) < 10 THEN
		LET cCodret  = '000001';
		LET cMensaje = 'DEBE PROPORCIONAR LOS PARÁMETROS CORRECTAMENTE';
		RETURN TRIM(cCodret), TRIM(cMensaje), NVL(cFecha, '1900-01-01'), NVL(cNombreProducto, ''), NVL(iSolicitudesMC, 0), NVL(iNoAnalizadas, 0), NVL(iAnalizadas, 0), NVL(iRevaluadas, 0), NVL(iAnalizadas-iRevaluadas, 0), NVL(iSigueProceso, 0), NVL(dPorcentajeSP, 0.00), NVL(iRechazadas, 0), NVL(dPorcentajeRT, 0.00), NVL(iCanceladas, 0), NVL(dPorcentajeCN, 0),iMIXTA,iUNICA;
	END IF;
	
	--OBTIENE LA DESCRIPCION DEL NÚMERO DE PRODUCTO
		IF TRIM(NVL(pProducto, '')) <> '' THEN
			SELECT nombre_prod INTO cNombreProducto FROM bdicred:sd_definicion WHERE num_producto  = pProducto ;
		END IF;
---SELECCIONA LAS FECHAS DENTRO DEL RANGO
 FOREACH WITH HOLD
		
		SELECT DISTINCT(fecha_insert),
...
--		COUNT(fecha_insert),
		...
		SUM(NVL( CASE WHEN  status_fin IN('CN','CM')  THEN 1 END,0)),
		SUM(NVL( CASE WHEN revalua = "S" THEN 1 END,0)),
		SUM(NVL( CASE WHEN revalua = "N" THEN 1 END,0))
			INTO cFecha,iSolicitudesMC,iMIXTA,iUNICA,iNoAnalizadas,iAnalizadas,iSigueProceso,iRechazadas,iCanceladas,iRevaluadas,iNoRevaluadas
		...
		--HACE EL CALCULO DE PORCENTAJES USANDO DATOS PREVIAMENTE OBTENIDOS
		IF iAnalizadas <= 0 THEN
			LET dPorcentajeSP = 0 ;
			LET dPorcentajeRT = 0 ;
			LET dPorcentajeCN = 0 ;
		ELSE 
			LET dPorcentajeSP = ROUND((iSigueProceso/iAnalizadas) * 100);
			LET dPorcentajeRT = ROUND((iRechazadas/iAnalizadas) * 100);
			LET dPorcentajeCN = ROUND((iCanceladas/iAnalizadas) * 100);
		END IF ;
		
		--LET iRevaluadas   = 0; --SIEMPRE EN 0 A PETICION DEL CLIENTE
		--LET iNoRevaluadas = 0; --SIEMPRE EN 0 A PETICION DEL CLIENTE
		LET iSolicMC      = iSolicMC + iSolicitudesMC;
		LET iNoAnali      = iNoAnali + iNoAnalizadas;
		LET iAnali        = iAnali + iAnalizadas;
		LET iTotalMix 	  = iTotalMix+ iMIXTA;
		LET iTotalUni 	  = iTotalUni+ iUNICA;
		LET iRevalu       = iRevalu + iRevaluadas;
		LET iNoRevalu     = iNoRevalu + (iAnalizadas-iRevaluadas); --SIEMPRE EN 0 A PETICION DEL CLIENTE
		LET iSiguePro     = iSiguePro + iSigueProceso; --SIEMPRE EN 0 A PETICION DEL CLIENTE
		LET dPorcenSP     = dPorcenSP + dPorcentajeSP;
		LET iRecha        = iRecha + iRechazadas;
		LET iPorceRT      = iPorceRT + dPorcentajeRT;
		LET iCancel       = iCancel + iCanceladas;
		LET dPorcenCN     = dPorcenCN + dPorcentajeCN;
		RETURN TRIM(cCodret), TRIM(cMensaje), SUBSTR(cFecha,4,2)||'/'||SUBSTR(cFecha,0,2)||'/'||SUBSTR(cFecha,7,4) , TRIM(cNombreProducto), iSolicitudesMC, iNoAnalizadas, iAnalizadas, iRevaluadas, iAnalizadas-iRevaluadas, iSigueProceso, dPorcentajeSP, iRechazadas, dPorcentajeRT, iCanceladas, dPorcentajeCN,iMIXTA,iUNICA WITH RESUME;
	END FOREACH
	...
	IF DBINFO('sqlca.sqlerrd2') = 0 THEN
		...
		RETURN TRIM(cCodret), TRIM(cMensaje), NVL(cFecha, DATE(1)) , NVL(cNombreProducto, ''), NVL(iSolicitudesMC, 0), NVL(iNoAnalizadas, 0), NVL(iAnalizadas, 0), NVL(iRevaluadas, 0), NVL(iAnalizadas-iRevaluadas, 0), NVL(iSigueProceso, 0), NVL(dPorcentajeSP, 0.00), NVL(iRechazadas, 0), NVL(dPorcentajeRT, 0.00), NVL(iCanceladas, 0), NVL(dPorcentajeCN, 0),iMIXTA,iUNICA;
	END IF	 
		
	
        LET cFecha                 = 'TOTAL';
		LET iSolicitudesMC         = iSolicMC ;
		LET iNoAnalizadas          = iNoAnali ;
		LET iAnalizadas            = iAnali ;
		LET iRevaluadas            = iRevalu ;
		LET iNoRevaluadas          = iNoRevalu ; --SIEMPRE EN 0 A PETICION DEL CLIENTE
		LET iSigueProceso          = iSiguePro ; --SIEMPRE EN 0 A PETICION DEL CLIENTE
		LET iRechazadas            = iRecha ;
		LET iCanceladas            = iCancel ;
		IF iAnalizadas <= 0 THEN
			LET dPorcentajeSP = 0 ;
			LET dPorcentajeRT = 0 ;
			LET dPorcentajeCN = 0 ;
		ELSE 
			...
			LET dPorcentajeCN = ROUND((iCanceladas/iAnalizadas) * 100);
		END IF ;
		RETURN TRIM(cCodret), TRIM(cMensaje), NVL(cFecha, '') , TRIM(cNombreProducto),iSolicitudesMC, iNoAnalizadas, iAnalizadas, iRevaluadas, iAnalizadas-iRevaluadas, iSigueProceso, dPorcentajeSP, iRechazadas, dPorcentajeRT, iCanceladas, dPorcentajeCN,iTotalMix,iTotalUni;
			
  END;
 END PROCEDURE
DOCUMENT
'DESCRIPCIÓN: PROCEDURE QUE CONTABILIZA Y GENERA UNA RELACIÓN DE DISTINTOS TIPOS DE SOLICITUDES DENTRO DE UN RANGO DE FECHA DADO', 
'FECHA DE CREACIÓN: 14 de Diciembre del 2012',
'BASE DE DATOS: BDICRED',
'CREADOR: Carlos Ochoa Valenzuela',
'VERSION: 20121214.1700',
'FECHA DE MODIFICACION: 23 de ABRIL del 2013',
'BASE DE DATOS: BDICRED',
'CREADOR: Jesus Aguilar',
'VERSION: 20130423.1700';
