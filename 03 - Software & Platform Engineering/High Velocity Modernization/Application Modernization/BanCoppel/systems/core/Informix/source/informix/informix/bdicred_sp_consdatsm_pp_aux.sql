CREATE PROCEDURE "informix".sp_consdatsm_pp_aux (pNumCteBco CHAR(20))
...
BEGIN
	ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
	   IF iSqlErr != 0 THEN
		  ...
		  RETURN cCodRet, vFechaTL37,vSaldoMorHistAltaTL36;
	   END IF;
	END EXCEPTION;

   -- SET debug file to '/home/sysifx/VeraMariscal/sp_consdatsm_pp_aux.out';
    --TRACE ON; 
    --SET debug file to '/informix/VeraMariscal/MuestrasPP/sp_consdatsm_pp_aux.out';
    --TRACE ON; 

    SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;

    --let vFechaTL37 = vFechaTL37;

    SELECT FIRST 1 MAX(TL37) 
				INTO vFechaTL37 
				FROM BDIBURO: "informix".BR_TL 
				WHERE NUM_CLIENTE = pNumCteBco  
...
DOCUMENT
'----------------------------------------------------------------------------',
'Descripcion : Se crea sp auxiliar para armado de variables necesarias para motor de evaluacion prestamos personales',
'Modifico    : Vera Mariscal',
'Fecha       : 05/10/2023',
'BD          : BDICRED',
'----------------------------------------------------------------------------';

create procedure "informix".sp_geninsumos_calif_oyp()
	...
	DEFINE cta_credsol 				CHAR (20);		DEFINE cred_ini,cred_fin  		 CHAR(20);	     
...
----CJAC CAMPOS ADICIONALES 
	...
	DEFINE v_int_etapa3    DECIMAL(18,2);
	...
	LET c_gpo_originacion	    = ''; 		LET n_gveces_1				= 0;		
	...
	LET d_pago_minimo_plazo_a   =0;			LET cta_credsol				=''; 		
...
--INICIALIZACION DE CAMPOS ADICIONALES
	...
	LET v_verificacion_num_anios = 0;
...
--SET DEBUG FILE TO "/RESPALDOS/Optimizacion_junio2023/sp_geninsumos_calif_oyp.out";
--TRACE ON;
...
--Reproceso
--LET dt_ini_per_proc = mdy('12','01','2018');
--LET dt_cierre_proc = mdy('12','31','2018');
--Reproceso
...
--modificaciones
	...
	LET pNombreArchivo = "Insumos_Calif_OYP_Lectura_"||TRIM(mes)||TRIM(anio)||".unl";
	
	LET comandoNombre = "/usr/bin/rm -rf "||TRIM(pRutaArchivo)||TRIM(pNombreArchivo);
		SYSTEM TRIM(comandoNombre);
	
	LET pNombreArchivo = "Insumos_Calif_OYP_Lectura_"||TRIM(mes)||TRIM(anio)||".unl.gz";
	...
	DROP TABLE IF EXISTS tempdatosoyp;
	CREATE TABLE bdicred:"informix".tempdatosoyp(
							num_credito CHAR(20),
	...
	CREATE INDEX "informix".idx_tempoyp ON bdicred:"informix".tempdatosoyp(num_credito);
	update statistics medium for table bdicred:"informix".tempdatosoyp;
	...
	LET sqlArchivoLeer = TRIM(sqlArchivoLeer)||" INSERT INTO bdicred:tempdatosoyp(num_credito,catcontrato,relacion,"; 
...
inner join sd_maecred d on (a.num_Credito = d.num_credito and d.fecha_apertura <= dt_cierre_proc) 
left join tempdatosoyp t ON  a.num_credito = t.num_credito
where (a.fecha = dt_cierre_proc AND a.fecha >= d.fecha_apertura) AND a.empresa = '001'
--and a.num_credito in ('810000422347')
and a.num_credito not in (select num_credito from sd_insumos_calif 
							where fecha_cierre = dt_cierre_proc)
and a.num_producto in ('7000','8100','5400')
...
and fecha_mov <= dt_cierre_proc        --mdy ('11','30','2018')
and num_credito in (select num_Credito from univ_ctas_calif)--actual
and num_credito not in (select num_Credito from tempdatosoyp)--cambio
and codigo_fun in (select cod_fun from sd_conceptospagomanual)
--and codigo_ref in (7,10,901,8,5,1)  --,6640) iva_interes_vencido
and codigo_ref in (7,8,5,923,925,926,907,908,909,10,901,1,2,6616,6617,6652,6709) 
and reversado = 'N'
...
LET val_t3 = 1;
begin;
CREATE INDEX idx_movs_comis ON movs_comis(num_credito,codigo_fun,codigo_ref) ONLINE;
commit;
...
--LET n_resc 			 = 0;
    ...
    FOREACH WITH HOLD
		SELECT  num_credito, numcte, num_producto,
				...
				tipo_producto,
				pago_realizado,pago_realizado1,pago_realizado2,pago_cierre,pago_cierre1,pago_cierre2,
          ...
          INTO  c_num_credito, c_numcte,c_producto, 
				...
				c_nombre_prod,
				d_pago_realizado_1,d_pago_realizado_2,d_pago_realizado_3,v_pago_cierre_1,v_pago_cierre_2,v_pago_cierre_3,
	...
	*/
	...
	END IF;
		...
		LET v_comaclara =0;
			...
			EXECUTE PROCEDURE sp_mes_siguiente(mdy(month(dt_cierre_proc), '18', year(dt_cierre_proc)),-1,DAY(mdy(month(dt_cierre_proc), '18', year(dt_cierre_proc))))
			...
			FROM sd_maesdoshist WHERE num_credito=c_num_credito AND fecha=mdy(month(dt_cierre_proc), '18', year(dt_cierre_proc));  ---'20210420'
	...
	LET d_saldo_cierre_credisol	= 0;		LET d_saldo_corte			= 0;		LET d_saldo_corte_credisol	= 0;		
	LET d_saldo_corte1			= 0;		LET d_saldo_corte2			= 0;		LET d_saldo_corte3			= 0;		
	LET n_scoreburo				= 0;		LET n_scoreotor				= 0;		
	LET n_sin_consulta			= 0;		
	LET c_status_corte			= '';		--LET c_nombre_prod			= '';			--se comenta por optimizacion 
...
/*		SELECT min(mdy(month(fecha_apertura),day(fecha_apertura),year(fecha_apertura))) INTO dt_ap_cte 
		...
		LEFT JOIN sd_maesdoshist c ON ((c.fecha = (dt_corte - 1 UNITS MONTH) AND c.fecha >= dt_apertura) AND c.empresa = b.empresa AND b.num_Credito = c.num_credito) --Mayo
		LEFT JOIN sd_maesdoshist d ON ((d.fecha = (dt_corte - 2 UNITS MONTH) AND d.fecha >= dt_apertura) AND d.empresa = b.empresa AND b.num_Credito = d.num_credito) --Abril
		LEFT JOIN sd_maesdoshist e ON ((e.fecha = (dt_corte - 3 UNITS MONTH) AND e.fecha >= dt_apertura) AND e.empresa = b.empresa AND b.num_Credito = e.num_credito) --Marzo 
		LEFT JOIN sd_maesdoshist f ON ((f.fecha = (dt_corte - 4 UNITS MONTH) AND f.fecha >= dt_apertura) AND f.empresa = b.empresa AND b.num_Credito = f.num_credito) --Febrero
		WHERE (b.fecha = dt_corte AND b.fecha >= dt_apertura)  AND b.empresa = '001' AND b.num_credito = c_num_credito;
...
--montos que no esten en el reporte 
		...
		END IF;*/
			...
			Select status_cred_18 INTO c_status_corte
			FROM sd_sdodiario
			WHERE num_credito=c_num_credito
		...
		WHILE (bandera_sdos <= 4) LOOP
			...
			--IF bandera_sdos < 3 or (bandera_sdos >= 3 AND (d_saldo_corte2 is null or d_saldo_corte2 = '')) THEN	
				...
				FROM sd_maecred a 				
				JOIN sd_maesdoshist b ON ( a.empresa = b.empresa and b.fecha = v_fec_sdo and  b.fecha  >= dt_apertura
		...
		IF c_producto IN('7000', '8100','5400') THEN
...
--Query  3		
		IF EXISTS (SELECT * FROM bdisolic:ss_Revision_determinacion WHERE num_solicitud = v_cred_externo) THEN
			...
			FROM bdisolic:ss_Revision_determinacion 
			WHERE num_solicitud = v_cred_externo;
			IF n_scoreotor IS NULL THEN
				...
				WHERE a.num_solicitud = v_cred_externo;
			END IF;
		ELSE			
			SELECT a.evalua_cc,a.grupo,b.evaluacion,c.evaluacion, a.situacion_pago
			INTO c_evalua_cc,c_gpo_originacion,n_scoreburo,n_scoreotor,v_eficiencia 
...
--Query 4
        ...
        LET v_capital_cierre=v_capital_cierre-v_intereses_ordinarios;
...
--Query 6
		...
		SELECT SUM(monto) INTO v_comremf
		 ...
		 LET v_comtotal= nvl(d_comision_disp,0) + nvl(v_comaclara,0) + nvl(v_comremf,0);
			...
			--toma el valor de movs_pagos, en base a el universo de creditos total, para siempre buscar el pago_realizado0
		...
		/* --se comenta por que se tomara el valor del reporte sustituyendo los valores, solo el primer pago realizado siempre se harÃ  la busqueda
		   SELECT SUM (CASE WHEN codigo_ref = 1 THEN monto ELSE 0 END) Pago_completo_realizado_1
				INTO d_pago_realizado_1
		   FROM movs_pagos
		  WHERE fecha_mov BETWEEN (dt_ini_per_rep - 1 units month) AND (dt_corte - 1 units month) --21/04 a 20/05
...
--HIST
		...
		*/
		...
		FROM sd_maecredcont
		WHERE fecha BETWEEN (dt_corte - 6 UNITS MONTH) AND dt_corte 
		AND num_Credito = c_num_credito
		AND (status_cred IN ('BA','BT') OR (status_cred='E1' and n_impagos_consec=1) OR (status_cred='E2' AND n_impagos_consec in(2,3)) OR (status_cred='E3' AND n_impagos_consec>=4));
		*/
		/*SELECT count(*) INTO n_imp_hist_6m_corte  --NUEVO CAMPO HIST_Corte
		...
		AND fecha BETWEEN (mdy(month(dt_cierre_proc), '18', year(dt_cierre_proc)) - 5 UNITS MONTH) AND mdy(month(dt_cierre_proc), '18', year(dt_cierre_proc));
	...
	SELECT SUM(c.sdo_cap_insoluto)+ SUM(c.sdo_intereses)+ SUM(c.sdo_no_exig) --CJAC Cambio a definiciones a saldo_cierre_credisol
		  ...
		  INNER JOIN sd_maesdoscontcrd c on c.fecha = dt_cierre_proc and c.empresa = a.empresa and c.num_credito = a.num_sol_prestamo
			   WHERE a.empresa = '001'
				...
				FOREACH WITH HOLD
					SELECT a.num_sol_prestamo, dia_corte
					  INTO cta_credsol , dia_corte_credsol
					  FROM sd_promocion_credito a
				INNER JOIN sd_maecredcontcrd b on b.fecha = dt_cierre_proc and b.empresa = a.empresa and b.num_credito = a.num_sol_prestamo and b.status_cred IN('AA','E1')
				INNER JOIN sd_maecredanexocrd  c on a.empresa  = c.empresa  and c.num_credito = a.num_sol_prestamo 
					...
					IF dia_corte_credsol >= 21 THEN --Si dia corte > = 21 se trae la fecha de corte de un mes previo
						 IF month(dt_ini_per_proc - 1 units day) = 2  AND dia_corte_credsol = 30 THEN
				 ...
				 SELECT NVL(sdo_cap_insoluto,0) 
				  ...
				  WHERE fecha = dt_corte_credsol
				   ...
				   LET d_saldo_corte_credisol = d_saldo_corte_credisol + NVL(d_saldo_corte_credisol_a,0);				   
					...
					LET v_saldopci= ((sum_capvig1+sum_capvig2)/abs(v_dias_periodo_tc));
					
					LET v_saldopci_total=v_saldopci_total+v_saldopci;
					...
					IF v_dia_corte_credisol='31' THEN 
						...
						INTO v_interesespci
						FROM sd_SDODIARIOCRD 
						WHERE num_credito=cta_credsol and fecha = mdy(month(dt_corte_credsol), '01', year(dt_corte_credsol));
					END IF;
				...
				inner join sd_tpcomis b
				on a.cod_comision_apertura=b.cod_comis
				where num_producto=c_producto;*/--optimizacion
				...
				LET v_form_sdoapagarxri=((d_saldo_corte+d_saldo_corte_credisol) * (v_tasa_interes));
		...
		IF d_monto_exigido > 0 AND v_verificacion_num_anios > 0 THEN 
			---LET v_numero_anios = -((logn(1-((d_sdo_corte_cred_t+d_saldo_corte_credisol)*(v_tasa_interes))/(NVL(d_pago_minimo,0)*12)))/(logn(1+(v_tasa_interes))));
			---LET v_numero_anios = -((logn(1-((d_sdo_corte_cred_t+d_saldo_corte_credisol)*(v_tasa_efectiva))/(NVL(d_pago_minimo,0)*12)))/(logn(1+(v_tasa_efectiva))));
			
			--LET v_numero_anios = -((logn(1-((d_saldo_corte+d_saldo_corte_credisol)*(v_tasa_efectiva))/(NVL(d_pago_minimo,0)*12)))/(logn(1+(v_tasa_efectiva))));  --CAC CAMBIO DE VARIABLE
		...
		SELECT NVL((CASE WHEN (NVL(sdo_cap_insoluto,0) < 0) THEN DECODE( 2,1, NVL(sdo_cap_insoluto,0),0)  
		 ...
		 -- BEGIN;
			...
			v_sdo_corte_msi,v_sdo_corte_msi_1,v_sdo_corte_msi_2,v_sdo_corte_msi_3,v_sdo_corte_msi_4,NVL(v_saldo_rev,0),NVL(v_interes_rev,0),v_saldopmsi,v_saldopci,v_interesespci,
			v_Promedio_MSI_contratados,v_Promedio_MSI_amort,v_Saldo_prom_MSI,v_MSI_hist,v_MSI_act,v_saldo_cierre_msi,v_intereses_etapa1,v_intereses_etapa2,v_cod_postal,v_rfc,v_curp,
...
SELECT DBINFO('utc_to_datetime', sh_curtime) INTO fin_proc 
  FROM sysmaster:sysshmvals;
LET cMensajeRet = 'Proceso Insumos Calificacion TDC Oro y Platino Ok';
