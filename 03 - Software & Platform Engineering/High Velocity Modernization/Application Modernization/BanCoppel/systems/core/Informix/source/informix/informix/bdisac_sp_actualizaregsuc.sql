CREATE PROCEDURE "informix".sp_actualizaregsuc (cSucursal CHAR(4), dFechaAct DATE, dFechaHoy DATE)
    RETURNING CHAR(5);
    ...
    -- debug flag
    -- SET DEBUG FILE TO "/tmp/sp_actualizaregistrosucursal.out";
    -- TRACE ON;
    ...
    BEGIN
        ...
        IF NOT EXISTS (SELECT numsucursal FROM sac_actualizacionsucursales WHERE numsucursal = cSucursal) THEN
            INSERT INTO sac_actualizacionsucursales VALUES (cSucursal, dFechaAct, dFechaHoy, 1);
        ELSE
            UPDATE sac_actualizacionsucursales SET fechaantactualizacion = dFechaAct, fechaultactualizacion = dFechaHoy, flagconfirmado = 1
            WHERE numsucursal = cSucursal;
        END IF;
        RETURN cCodRet;
    END;
END PROCEDURE
DOCUMENT
'AUTOR : Jesus Montoya',
'DESCRIPCION: Se encarga de gueardar o actualizar aquellos registros de sucursales a los que se les replicaron convenios',
'FECHA : Septiembre de 2008',
'VERSION: 20080830',
'BD    : bdisac';

CREATE PROCEDURE "informix".sp_actualizastatusconvenio(cStatus CHAR(1), cNumConvenio CHAR(3), cNumCategoria CHAR(2))
