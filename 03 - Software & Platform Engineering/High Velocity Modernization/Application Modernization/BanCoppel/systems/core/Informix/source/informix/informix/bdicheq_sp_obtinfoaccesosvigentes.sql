CREATE PROCEDURE "informix".sp_obtinfoaccesosvigentes( pFechaApli CHAR(7))
...
--SET DEBUG FILE TO "/tmp/Josue/sp_obtinfoaccesosvigentes.out";
--TRACE ON;
...
BEGIN
   ...
   ON EXCEPTION SET iSql_Err, iSam_Err
      IF iSql_Err <> 0 OR iSam_Err <> 0 THEN
		 ...
		 IF EXISTS(SELECT tabname FROM sysmaster:"informix".systabnames 
			WHERE tabname = 'tmpobtinfoaccesosvigentes' AND dbsname= 'bdicheq') THEN
			DROP TABLE bdicheq:"informix".tmpobtinfoaccesosvigentes;
		 END IF;
		 ...
		 END IF;
         RETURN cCodRet, cEmpleado, cNombre, iNumPuesto, cNombrePuesto, iCentro, cPerfil, cOperacion, cEstatus, cRutaPC, cUser, cPass, cIP, cRutaPCServer, cNombreArchivo;
      END IF;
   END EXCEPTION;
	...
	IF EXISTS(SELECT tabname FROM sysmaster:"informix".systabnames 
				WHERE tabname = 'tmpobtinfoaccesosvigentes' AND dbsname= 'bdicheq') THEN
		DROP TABLE tmpobtinfoaccesosvigentes;
	END IF;
	IF EXISTS (SELECT dbsname, tabname FROM sysmaster:"informix".systabnames  WHERE tabname = 'raw_resultadomtos') THEN
		DROP TABLE bdicheq:"informix".raw_resultadomtos;
	END IF;
	IF EXISTS (SELECT dbsname, tabname FROM sysmaster:"informix".systabnames  WHERE tabname = 'raw_intermedia') THEN
		DROP TABLE bdicheq:"informix".raw_intermedia;
	END IF;
	IF EXISTS (SELECT dbsname, tabname FROM sysmaster:"informix".systabnames  WHERE tabname = 'raw_secuencia') THEN
		DROP TABLE bdicheq:"informix".raw_secuencia;
	END IF;
	--se crean las tablas temporales
	CREATE TABLE bdicheq:"informix".tmpobtinfoaccesosvigentes
		( 
			Empleado		CHAR(10), 
			Nombre			CHAR(90), 
			NumPuesto		INTEGER,
	...
	CREATE RAW TABLE bdicheq:"informix".raw_intermedia
	(usuario					CHAR(10),
	operacion					CHAR(7),
	fecha						CHAR(24),
	secuencia					CHAR(10));
	UPDATE STATISTICS MEDIUM FOR TABLE bdicheq:"informix".raw_intermedia;
	
	CREATE RAW TABLE bdicheq:"informix".raw_secuencia
	(usuario					CHAR(10),
	secuencia					CHAR(10));
	UPDATE STATISTICS MEDIUM FOR TABLE bdicheq:"informix".raw_secuencia;
	...
	LET cNombreArchivo = "inf_corresp_"||TRIM(cMes)||TRIM(cAnio)||"_1.txt";
	
	WHILE (iContador < 3) 
		IF iContador = 1 THEN
			--se toman los movimientos de corresponsales de sc_movhis
			INSERT INTO bdicheq:"informix".raw_intermedia (usuario,operacion,fecha,secuencia)
			SELECT  usuario,CASE cancelad WHEN 'S' THEN 'Reverso' ELSE 'Captura' END, MAX(fech_alt || ' ' || fech_hor),MAX(num_serial)
			FROM sc_movhis
			JOIN sc_corresponsal ON emp= usuario 
			WHERE transacc='0282'
			AND fech_alt BETWEEN  dFechaIni AND dFechaFin
			GROUP BY 1,2;
		ELSE
			--se borra tabla intermedia
			DELETE FROM bdicheq:"informix".raw_intermedia;
			--se toman los movimientos de corresponsales de sd_movhis
			INSERT INTO bdicheq:"informix".raw_intermedia (usuario,operacion,fecha,secuencia)
			SELECT  usuario,CASE reversado WHEN 'S' THEN 'Reverso' ELSE 'Captura' END,MAX(fecha_mov || ' ' || hora_mov),MAX(secuencia) 
			FROM bdicred:"informix".sd_movhis
			JOIN sc_corresponsal ON emp= usuario 
			WHERE transacc_suc='6282'
			AND codigo_fun='700'
			AND codigo_ref=1 
			AND fecha_mov BETWEEN dFechaIni AND dFechaFin 
			GROUP BY 1,2;
		END IF
		--se toman los num_serial maximas
		INSERT INTO bdicheq:"informix".raw_secuencia (usuario,secuencia)
		SELECT usuario, MAX(secuencia)
		FROM bdicheq:"informix".raw_intermedia
		GROUP BY 1;
		
		--se insertan los movimientos maximos
		INSERT INTO bdicheq:"informix".raw_resultadomtos (usuario,operacion,fecha)
		SELECT usuario,operacion,fecha
		FROM bdicheq:"informix".raw_intermedia a
		WHERE secuencia = (SELECT secuencia FROM raw_secuencia b WHERE a.usuario = b.usuario);
		
		LET iContador = iContador + 1;
	END WHILE;
	
	--Se consultan accesos vigentes
	INSERT INTO bdicheq:"informix".tmpobtinfoaccesosvigentes (Empleado, Nombre, NumPuesto, NombrePuesto, Centro, Perfil, Operacion, Estatus)
	SELECT DISTINCT emp, TRIM(nombre)||' '||TRIM(appaterno)||' '||TRIM(apmaterno), NVL(nopuesto,"0"), TRIM(puesto), NVL(centro,"0"),
			TRIM(perfilcorresponsal), NVL(operacion,'Ninguna'), TRIM(estatus)
	FROM bdicheq:"informix".sc_corresponsal a
	LEFT OUTER JOIN bdicheq:"informix".raw_resultadomtos b 
	ON emp = usuario AND fecha= (SELECT MAX(fecha) 
								FROM raw_resultadomtos c 
								WHERE b.usuario= c.usuario);
	...
	let cSql3 = " SELECT 'emp', 'nombre',  'numpuesto', 'nombrepuesto', 'centro', 'perfilcorresponsal', 'operacion', 'estatus'  FROM bdicheq:'informix'.sc_fechas WHERE empresa = '001'"||
				" UNION ALL"||
				" SELECT TRIM(empleado::char(10)), TRIM(nombre::char(92)), TRIM(numpuesto::char(10)), TRIM(nombrepuesto::char(30)), TRIM(centro::char(10)), TRIM(perfil::char(10)), TRIM(operacion::char(10)), TRIM(estatus::char(20))  FROM bdicheq:'informix'.tmpobtinfoaccesosvigentes";
	...
	DROP TABLE bdicheq:"informix".raw_intermedia;
	DROP TABLE bdicheq:"informix".raw_secuencia;
	RETURN cCodRet, TRIM(cEmpleado), TRIM(cNombre), NVL(iNumPuesto,0) , TRIM(cNombrePuesto),NVL(iCentro, 0), TRIM(cPerfil), TRIM(cOperacion), TRIM(cEstatus), TRIM(cRutaPC), TRIM(cUser), TRIM(cPass), TRIM(cIP), cRutaPCServer, cNombreArchivo;
END;
END PROCEDURE
DOCUMENT
'AUTOR: Abigail Vasavilbazo Cañedo',
'FECHA: Julio del 2011',
'DESCRIPCION: Consulta los accesos vigentes y genera archivo txt con resultado',
'VERSION: 20110803.1051',
'BD: BDICHEQ',
'MODIFICO: Roberto Aguilar Pozos',
'FECHA: 18 de Agosto del 2011',
'MODIFICACION: Se modifica para que devuelva la operacion correcta',
'VERSION: 20110818.0350',
'MODIFICO: Roberto Aguilar Pozos',
'FECHA: 31 de Agosto del 2011',
'MODIFICACION: Se cambia la logica de la consulta para mejorar el tiempo de ejecucion',
'VERSION: 20110831.1755',
'MODIFICO: Victor Hugo Nuñez',
'FECHA: 28 de Octubre del 2011',
'MODIFICACION: Se agregaron modificaciones para poder validar tambien por secuencia maxima y numserial',
'VERSION: 20111028.1211';
