CREATE PROCEDURE "informix".sp_pantalla_correccion (pPlaza CHAR(30),pSobrante MONEY(16,2),pFaltante MONEY(16,2),pComen CHAR(300))
...
BEGIN
	...
	--SET DEBUG FILE TO '/respaldosbd/obed/sp_pantalla_correccion.out';
	--TRACE ON;	
		
		IF pSobrante > 0.00  AND pFaltante > 0.00 THEN
			IF LENGTH(TRIM(pComen)) > 0 THEN
				UPDATE  bdisuc:"informix".ss_arqueo_panamericano
				SET faltantes = pFaltante, sobrantes = pSobrante, comentarios = pComen
				WHERE plaza = TRIM(pPlaza);
			ELSE
				UPDATE  bdisuc:"informix".ss_arqueo_panamericano
				SET faltantes = pFaltante, sobrantes = pSobrante
				WHERE plaza = TRIM(pPlaza);
			END IF;
		ELIF pSobrante > 0.00 THEN
			IF LENGTH(TRIM(pComen)) > 0 THEN
				UPDATE  bdisuc:"informix".ss_arqueo_panamericano
				SET sobrantes = pSobrante, comentarios = pComen
				WHERE plaza = TRIM(pPlaza);
			ELSE
				UPDATE  bdisuc:"informix".ss_arqueo_panamericano
				SET sobrantes = pSobrante
				WHERE plaza = TRIM(pPlaza);
			END IF;
		ELIF pFaltante > 0.00 THEN
			IF LENGTH(TRIM(pComen)) > 0 THEN
				UPDATE  bdisuc:"informix".ss_arqueo_panamericano
				SET faltantes = pFaltante, comentarios = pComen
				WHERE plaza = TRIM(pPlaza);
			ELSE
				UPDATE  bdisuc:"informix".ss_arqueo_panamericano
				SET faltantes = pFaltante
				WHERE plaza = TRIM(pPlaza);
			END IF;
		END IF;
	RETURN cCodRet;
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se crea procedimiento para guardar los sobrantes y faltantes en el arqueo Pan Americano',
'AUTOR : Obed Vega',
'FECHA : 12/Jul/2013',
'VERSION: 1.0',
'BD: bdisuc';
