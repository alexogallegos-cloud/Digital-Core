CREATE PROCEDURE "informix".sp_concreing_conciliacionautomatica 
(
    ...
    DEFINE vsTransaccionCashBack VARCHAR (16); ---  Para contemplar proceso de Cash Back RRM
    ...
    DEFINE vssecuencia_ext_archivo CHAR(15);    -- TFORZADAS
    ...
    LET vsTransaccionCashBack = ''; -- InicializaciÃÂ³n de Variable de para transacciones Cash Back
    LET vsRep_Aix = '';
    LET vsRep_Win = '';
...
BEGIN
    ON EXCEPTION SET viSQLerr
        ...
        UPDATE BdiTarjeta:"informix".Td_Param_Conciliacion_ConcReing 
            SET Valor = 'F', 
                Fecha_Modificacion = (SELECT DBINFO('utc_to_datetime', Sh_Curtime)::DATETIME YEAR TO FRACTION(5) 
        ...
        EXECUTE PROCEDURE BdiTarjeta:"informix".Sp_ConcReing_GuardaBitacora (viElemento, '(' || vsCodRet || ') ' || vsMensaje_Respuesta, psCve_usuario) INTO vsCodRet2;
   ...
   -- SET DEBUG FILE TO "/informix/LVRQ/MSI/debug/concreing_conciliacion.out";
    --TRACE ON;
	...
	IF (EXISTS (SELECT Descripcion FROM BdiTarjeta:"informix".Td_Param_Conciliacion_ConcReing WHERE Codigo = '001' AND Descripcion = 'CONCILIACION EN EJECUCION' AND TRIM(Valor) = 'V') ) THEN --VALIDA QUE NO EXISTA UNA CONCILIACION EN EJECUCION (CONCREING)
        LET vsCodRet = '00001'; --CONCILIACION EN EJECUCIÃ?N --CONCREIN
			LET vsMensaje_Respuesta = 'CONCILIACION EN EJECUCIÃN (CONCREIN).';
    ...
    ELIF (NOT EXISTS (SELECT Archivo_Origen FROM BdiTarjeta:"informix".Td_Archivo_Origen WHERE Horario_Ejecucion_Hoy = piHorario OR Horario_Ejecucion_Ext = piHorario)) THEN --VALIDA QUE EL CRON/EJECUCION ESTE CONTEMPLADAPARA ALGUNO DE LOS ARCHIVOS
	...
	ELSE --OK
		...
		UPDATE BdiTarjeta:"informix".Td_Param_Conciliacion_ConcReing
            SET Valor = 'V',  Fecha_Modificacion = current
		WHERE Codigo = '001'
            AND Descripcion = 'CONCILIACION EN EJECUCION'
            AND TRIM(Valor) = 'F';
		...
		WHILE (vsFlag_Ciclo_BusrcarArch = 'V')  --CICLO DE BUSQUEDA DE ARCHIVOS PENDIENTES.
			...
			SELECT FIRST 1
				...
				Prefijo_Archivo, Sistema, Conciliacion_Inter, Conciliacion_SIF, Conciliacion_Admin, conciliacion_admin_atm,
			...
			INTO 
				...
				vsPrefijo_Archivo, vsSistema, vsConciliacion_Inter, vsConciliacion_SIF, vsConciliacion_Admin, vsConciliacionAdminAtm,
			...
			FROM TABLE( MULTISET 
				(SELECT Horario_Ejecucion_Hoy, orden_proceso, 
						'V' AS Flag_ArchPendiente, ArchCon.NombreArchivo, ArchCon.Archivo_Origen, ArchCon.Fecha_Archivo, ArchCon.Carga, 
						ArchOri.Prefijo_Archivo, ArchOri.Sistema, ArchOri.Conciliacion_Inter, ArchOri.Conciliacion_SIF, ArchOri.Conciliacion_Admin, ArchOri.conciliacion_admin_atm,
						ArchOri.Borra_Archivo_Fisico, ArchOri.Transaccion_Compra, ArchOri.Transaccion_Liberacion, ArchOri.Transaccion_Forzada, 
						ArchOri.Transaccion_Abono, ArchOri.Rep_Aix, ArchOri.Rep_Win, 
						ArchOri.Archivo_Companero, ArchOri.Archivo_Report_Comisiones, ArchOri.Tipo_LayOut
					...
					WHERE 	ArchCon.Proceso = 'P'
							AND ArchCon.Fecha_Archivo = (dtFecha_Hoy_Integral::DATE - ArchOri.Dias_Desfase)::DATE AND Horario_Ejecucion_Hoy <= piHorario
			...
			IF (NVL(vsFlag_ArchPendiente, 'F') <> 'V') THEN --NO ENCONTRO MOVIMIENTO NORMAL 
				...
				INTO 
						...
						vsArchivo_Companero, vsArchivo_Report_Comisiones, viTipo_LayOut
				FROM TABLE( MULTISET 
					(SELECT Horario_Ejecucion_Ext, orden_proceso, 
					'V' AS Flag_ArchPendiente, ArchCon.NombreArchivo, ArchCon.Archivo_Origen, ArchCon.Fecha_Archivo, ArchCon.Carga, 
					ArchOri.Prefijo_Archivo, ArchOri.Sistema, ArchOri.Conciliacion_Inter, ArchOri.Conciliacion_SIF, ArchOri.Conciliacion_Admin, ArchOri.conciliacion_admin_atm,
					ArchOri.Borra_Archivo_Fisico, ArchOri.Transaccion_Compra, ArchOri.Transaccion_Liberacion, ArchOri.Transaccion_Forzada, 
					ArchOri.Transaccion_Abono, ArchOri.Rep_Aix, ArchOri.Rep_Win, 
			...
			--OBTIENE LAS TRANSACCIONES ADICIONALES DE LOS ARCHIVOS TRAN-ADIC ABONO MONEYGRAM (VID)
			SELECT FIRST 1 TRIM(NVL(Valor, '0000'))
                INTO vsTransaccionMoneyGram  
			FROM BdiTarjeta:"informix".Td_Param_Conciliacion_ConcReing
			WHERE Codigo BETWEEN 501 AND 600
                AND Descripcion MATCHES 'TRAN-ADIC*' 
                AND Descripcion MATCHES '*(' || vsArchivo_Origen || ')';
			
            
			------- Agregar para transacciÃÂ³n cash back--------------
			
            
			SELECT FIRST 1 TRIM(NVL(Valor, '0000'))
                INTO vsTransaccionCashBack
			FROM BdiTarjeta:"informix".Td_Param_Conciliacion_ConcReing
			WHERE Codigo BETWEEN 601 AND 700
                AND Descripcion MATCHES 'TRAN-ADIC*' 
                AND Descripcion MATCHES '*(' || vsArchivo_Origen || ')';
            
            
            LET vsTransaccionMoneyGram = NVL(vsTransaccionMoneyGram, '0000');
			LET vsTransaccionCashBack = NVL(vsTransaccionCashBack, '0000000000000000');
			...
			IF (NVL(vsFlag_ArchPendiente, 'F') = 'V') THEN -- EXISTEN ARCHIVOS PENDIENTE POR PROCESAR
				...
				IF ((vsSistema IN ('C', 'A')) AND (NOT EXISTS (SELECT Status_Proc FROM BdiCred:"informix".Sd_ContProc WHERE Proceso = 'Trasl_Dia' AND Fecha = (TODAY-1) AND Cod_Ret = '000' AND Status_Proc = 'F')) ) THEN --VALIDA ESTATUS DEL PASE DE MOVIMIENTOS HITORICOS DE CREDITO
                ...
                ELIF ((vsSistema IN ('D', 'A')) AND (NOT EXISTS (SELECT Fecha FROM BdiCheq:"informix".sc_ContProc WHERE Proceso = 'pasomovshist' AND Fecha = (TODAY-1))) ) THEN --VALIDA ESTATUS DEL PASE DE MOVIMIENTOS HITORICOS DE CHEQUES
					
                    LET vsCodRet = '00007'; --NO SE HA REALIZADO EL PASE DE MOVIMIENTOS HITORICOS DE DEBITO
					LET vsMensaje_Respuesta = 'ARCHIVO (' || vsNombreArchivo || ') NO SE HA REALIZADO EL PASE DE MOVIMIENTOS HISTÃRICOS DE DÃBITO.';
                ...
                ELSE
					...
					IF (vsCarga <> 'V') THEN --VALIDA QUE LA CARGA DEL ARCHIVO NO FUE REALIZADA PREVIAMENTE
						...
						EXECUTE PROCEDURE BdiTarjeta:"informix".Sp_ConcReing_CargaArchivos ( vsRep_Aix, vsNombreArchivo, vsArchivo_Origen, viTipo_LayOut, vsSistema, vsRep_Aix)
					...
					END IF; --Cierre del IF (vsCarga <> 'V') THEN --VALIDA QUE LA CARGA DEL ARCHIVO NO FUE REALIZADA PREVIAMENTE
					...
					IF (vsCodRet = '00000') THEN --VALIDA SI EL ARCHIVO SE CARGO A LA TABLA DE PASO.
						...
						IF (vsCarga <> 'V') THEN --VALIDA QUE LA CARGA DEL ARCHIVO NO FUE REALIZADA PREVIAMENTE
							...
							EXECUTE PROCEDURE BdiTarjeta:"informix".sp_concreing_obtenerregistroarchivo (vsNombreArchivo, vsArchivo_Origen, viTipo_LayOut, psCve_Usuario )
							...
							WHERE NombreArchivo = vsNombreArchivo 
                                AND Archivo_Origen = vsArchivo_Origen 
                                AND Fecha_Archivo = vdtFecha_Archivo;
						END IF; --Cierre del segundo IF IF (vsCarga <> 'V') THEN --VALIDA QUE LA CARGA DEL ARCHIVO NO FUE REALIZADA PREVIAMENTE
						...
						IF (vsCodRet = '00000') THEN
							...
							UPDATE BdiTarjeta:"informix".Td_Archivos_Conciliacion
							SET Fecha_Hora_Ini_Concilia_Reg = (SELECT DBINFO('utc_to_datetime', Sh_Curtime)::DATETIME YEAR TO FRACTION(5) FROM SysMaster:"informix".Sysshmvals)
							WHERE NombreArchivo = vsNombreArchivo 
                                AND Archivo_Origen = vsArchivo_Origen 
                                AND Fecha_Archivo = vdtFecha_Archivo;
                                
							--INICIALIZA LAS VARIABLES DE CONADMIN
							LET vsNombreArchivo_Comi = '';
							LET vsNombreArchivoCompanero = '';
							LET vsRuta_Archivo_Comisiones = '';
                            
							--OBTENER EL NOMBRE DE ARCHIVO DE COMISIONES /CONCILIACION ADMINISTRATIVA
							IF ((vsConciliacion_Admin = 'V') AND (TRIM(vsArchivo_Report_Comisiones) <> '')) THEN --VALIDA SI TIENE RELACIONADO ARCHIVO DE COMISIONES
								--concitarjMMDDAAAA.txt
								--BCPL***_DDMMAAAA.txt
                                
                                
								--OBTIENE EL NOMBRE DEL ARCHIVO DE COMISIONES CORRESPONDIENTE AL TIPO Y FECHA DEL ARCHIVO
								SELECT FIRST 1 (TRIM(Prefijo_Archivo) || SUBSTR(vsNombreArchivo,11,2) || SUBSTR(vsNombreArchivo,9,2) || SUBSTR(vsNombreArchivo,13,4) || '.txt' ), TRIM(Rep_AIX) 
                                    INTO vsNombreArchivo_Comi, vsRuta_Archivo_Comisiones
								FROM BdiTarjeta:"informix".Td_Archivo_Origen 
								WHERE Archivo_Origen = vsArchivo_Report_Comisiones; --(ACC, ACI, ACT)
                                
								IF (vsArchivo_Companero <> '') THEN --SI TIENE ARCHIVO COMPANERO BUSCA SU NOMBRE   ---(TCC, TCD, CCP, CCD)
									--BCPL***_DDMMAAAA.txt

									--OBTIENE EL NOMBRE DEL ARCHIVO COMPANERO DEL ARCHIVO CORRESPONDIENTE AL TIPO Y FECHA DEL ARCHIVO
									SELECT FIRST 1 (TRIM(Prefijo_Archivo) || SUBSTR(vsNombreArchivo,9,2) || SUBSTR(vsNombreArchivo,11,2) || SUBSTR(vsNombreArchivo,13,4) || '.txt' ) 
									INTO vsNombreArchivoCompanero
									FROM BdiTarjeta:"informix".Td_Archivo_Origen 
									WHERE Archivo_Origen = vsArchivo_Companero --ORIGINAL ---(TCC, TCD, CCP, CCD, TPD)
									AND Archivo_Report_Comisiones = vsArchivo_Report_Comisiones; --(ACC, ACI, ACT)
								END IF;
                                
							END IF; --Cierre de IF vsConciliacion_Admin = 'V') AND (TRIM(vsArchivo_Report_Comisiones) <> ''
							...
							WHILE (vsFlag_Ciclo_BusrcarReg = 'V')  --CICLO DE BUSQUEDA DE REGISTROS PENDIENTES.
								...
								IF (vsFlag_Ciclo_BusrcarReg = 'V') THEN --VALIDA SI EXISTE REGISTRO PARA PROCESAR
									...
									IF (vsSistema IN ('C', 'D')) THEN --TODOS LOS REGISTROS SON DEL MISMO SISTEMA
										LET vsSistema_Registro = vsSistema;
									ELIF (vsSistema = 'A') THEN --VALIDA EL TIPO DE SISTEMA "A" PARA ARCHIVOS QUE TIENEN REGISTROS DE CRED Y DEB MEZCLADOS --- (ATMO, TNC)
									...
									--VALIDA LA INTEGRIDAD DE LOS REGISTROS INDIVIDUALES --07/2013 Se integra nuevo campo de validacion vsmontocashback325
									EXECUTE PROCEDURE BdiTarjeta:"informix".Sp_ConcReing_ValidaIntegridad ( vsArchivo_Origen, viConsecutivo, vsNumTarjeta, vsTipotransaccion325, vsMonto325, vsMontoCashBack325, vsIdcomercio325, vsNomcomercio325, vsReferencia23_325, vsSecuencia325, vsDivisa325, vsRfc325, vsBinDebito, vsBinCredito, vsSistema)
									...
									IF vsArchivo_Origen = 'PNC' THEN
										LET vsb_aplica = 'V';
									END IF;
                                    
									--Bloque TRANSFER 
                                    
                                    -- Para realizar insertardo de Registro cuando transacciones de cajeros transfer #######################                                    
									LET vsfechaopetransfer = SUBSTR(YEAR(vsFechaTransaccion::date),3,2) || LPAD(MONTH(vsFechaTransaccion::date),2,'0')|| LPAD(DAY(vsFechaTransaccion::date),2,'0');
                                    
									IF (vsCodRet = '00000' AND vsArchivo_Origen IN ('TMD', 'TMC') AND vsestransfer = 'V' ) THEN
                                    
										EXECUTE PROCEDURE bditransfer:"informix".sp_transfer_in_tfincapture (vsNombreArchivo, vsArchivo_Origen, vsNumTarjeta, viTipo_Conciliacion,	'', '',	vsSecuencia_Extendida, 
                                                                LPAD(vsMonto325,12,'0'), vsIdcomercio325, vsfechaopetransfer, SUBSTR(vsInfReceptor,1,26), vsReferencia23_325,'','') 
                                            INTO vsCodRet, vsMensaje_Respuesta;

									END IF;
                                    
									-- Se indica que para los archivos de PNC la bandera de aplicacion es V
									
									--VALIDA SI AL REGISTRO LE CORRESPONDE CONCILIACION INTERCARD y Fue validado Por proceso de transacciones forzadas
									IF ((vsConciliacion_SIF = 'V') AND (vsCodRet = '00000') and (vsb_aplica = 'V')) THEN
                                    
										LET vsMensaje_Respuesta = 'CONCILIACION SIF.';
										
                                        -- Para retomar valores si los campos de monto325 y montocashback325 cambiaron por no ser enviados asi en archivo325
										IF (viActualizacion = 1) THEN
                                            
                                            
											SELECT LIMIT 1 Monto325, montocashback325
                                                INTO vsMonto325,vsMontoCashBack325
                                            FROM BdiTarjeta:"informix".Td_Movimientos_Conciliacion
                                                WHERE 	NombreArchivo = vsNombreArchivo 
                                                    AND Archivo_Origen = vsArchivo_Origen 
										...
										EXECUTE PROCEDURE BdiTarjeta:"informix".Sp_ConcReing_ConSif (
											psCve_usuario, 
											vsArchivo_Origen, 
											dtFecha_Hoy_Integral, 
											vsSistema_Registro, 
											vsTransaccion_Compra,
											vsTransaccion_Liberacion, 
											vsTransaccion_Forzada, 
											vsTransaccion_Abono, 
											vsTransaccionMoneyGram,
											vsTransaccionCashBack, --- Para transacciones Cash Back
											viTipo_Conciliacion, 
											vsNumTarjeta, 
											vsNumCuenta, --SOLO ATM TRAE CUENTA
											vsTipoTransaccion325, 
											vsFolio_Mov,  --REQUIERE CONCILIACION INTERCARD
											((vsMonto325::MONEY)/100), 
											((vsMontoCashBack325::MONEY)/100), -- Para integracion de CashBack
											'01', --FIJO de tipo char 
											vsNomcomercio325, 
											vsRfc325, 
											vsReferencia23_325, 
											vsDivisa325, 
											((vsMonto_Divisa325::MONEY)/100), 
											vsIdTerminal, --REQUIERE CONCILIACION INTERCARD
											vsTipo_Mov, --REQUIERE CONCILIACION INTERCARD
											viConsecutivo, 
											vsNombreArchivo, 
											vsSecuencia_Extendida,  	-- Secuencia Extendida  POR INTEGRACION DE TRANSFER
											vsestransfer,				-- Bandera que indica el tipo de registro
											vsfechaopetransfer,			-- Fecha de la operacion en transfer,
											vsIdcomercio325,			-- Id de comercio
											vsCuenta)					-- Numero de cuenta relacionada al plastico 									) 
										INTO vsCodRet, vsMensaje_Respuesta, viElemento, vsTransaccion_Aplica, viTipo_Conciliacion,vsBandera_Proceso;
                                        
									END IF; --Cierre ((vsConciliacion_SIF = 'V') AND (vsCodRet = '00000') and (vsb_aplica = 'V'))
                                    ...
                                    IF ( (vsConciliacion_Admin = 'V') AND (vsCodRet = '00000') ) THEN
										...
										--##########################################################   TRANSACCIONES DE CORRESPONSALES      ##########################################################################											
											...
											IF vsTipoTransaccion325  = '20' and vsSistema = 'D'  and vsArchivo_Origen = 'TCD' then 
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '350';
											elif  vsTipoTransaccion325  = '20' and vsSistema = 'C'  and vsArchivo_Origen = 'TCC' then 
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '351';
											elif vsTipoTransaccion325  = '20' and vsSistema = 'D'  and vsArchivo_Origen = 'CCD' then
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '352';
											elif vsTipoTransaccion325  = '20' and vsSistema = 'C'  and vsArchivo_Origen = 'CCP' then
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '353';
											-- Transferencias
											elif vsTipoTransaccion325  = '20' and vsSistema = 'D'  and vsArchivo_Origen = 'TPD' then  -- Transaccion de transferencia Prestamo
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '354';
											elif vsTipoTransaccion325  = '21' and vsSistema = 'D'  and vsArchivo_Origen = 'TPD' then  -- Transaccion de transferencia Gastos Funerarios
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '363';
											elif vsTipoTransaccion325  = '22' and vsSistema = 'D'  and vsArchivo_Origen = 'TPD' then  -- Transaccion de transferencia Finiquito del Empleado
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '364';
											elif vsTipoTransaccion325  = '23' and vsSistema = 'D'  and vsArchivo_Origen = 'TPD' then  -- Transaccion de transferencia Total de Fondo de Ahorro
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '365';
											elif vsTipoTransaccion325  = '24' and vsSistema = 'D'  and vsArchivo_Origen = 'TPD' then  -- Transaccion de transferencia Reporto de Utilidades
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '366';
											elif vsTipoTransaccion325  = '25' and vsSistema = 'D'  and vsArchivo_Origen = 'TPD' then  -- Transaccion de transferencia Pago Seguro de Vida
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '367';
											elif vsTipoTransaccion325  = '26' and vsSistema = 'D'  and vsArchivo_Origen = 'TPD' then  -- Transaccion de transferencia Abonos a Creditos Coppel
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '370';
												
											-- Corresponsales	
											elif vsTipoTransaccion325  = '30' and vsSistema = 'D'  and vsArchivo_Origen = 'CCD' then  --Consulta de Saldo Efectiva
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '355'; 
											elif vsTipoTransaccion325  = '30' and vsSistema = 'C'  and vsArchivo_Origen = 'CCP' then  --Consulta de Saldo TDC BCPL
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '356'; 
											elif vsTipoTransaccion325  = '40' and vsSistema = 'D'  and vsArchivo_Origen = 'CCD' then  --Retiro de Efectiva
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '357'; 
											elif vsTipoTransaccion325  = '40' and vsSistema = 'C'  and vsArchivo_Origen = 'CCP' then  --Retiro de TDC BCPL
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '358'; 
											elif vsTipoTransaccion325  = '50' and vsSistema = 'D'  and vsArchivo_Origen = 'CCD' then  --Transferencia entre cta. efectivas
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '359'; 
											elif vsTipoTransaccion325  = '60' and vsSistema = 'C'  and vsArchivo_Origen = 'CCP' then  --Pago de TDC BCPL con CTA. Efectiva
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '360'; 
											elif vsTipoTransaccion325  = '70' and vsSistema = 'C'  and vsArchivo_Origen = 'CCP' then  --Pago de TDC Otro con CTA. Efectiva
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '361'; 
											elif vsTipoTransaccion325  = '80' and vsSistema = 'C'  and vsArchivo_Origen = 'CCP' then  --Pago de TDC Otro con Efectivo
												select valor into vsTransaccion_Aplica from Bditarjeta:"informix".td_param_conciliacion_concreing where codigo = '362'; 
											ELSE
											...
											LET vsTipo_Mov = 'C'; --COPPEL FIJO   --- Verificar por nuevas funcionalidades
										...
										--TRACE 'MIRA AQUI'|| vsTransaccion_Aplica;
										
										
                                        
										EXECUTE PROCEDURE BdiTarjeta:"informix".Sp_ConcReing_ConAdmin 
										( 
											vsSistema_Registro, 
											vdtFecha_Archivo, --FECHA DEL ARCHIVO PARA MANEJO CORRECTO DE EXTEMPORANEOS
											vsBandera_Proceso, 
									...
									IF (vsCodRet <> '00000') THEN --VALIDA QUE TODOS LOS PROCESOS PARA EL REGISTRO SEAN CORRECTOS
										...
										EXECUTE PROCEDURE BdiTarjeta:"informix".Sp_ConcReing_GuardaBitacora (viElemento, '(' || vsCodRet || ') [' || vsNombreArchivo || ']' || vsMensaje_Respuesta, psCve_usuario) INTO vsCodRet2;
									...
									--DEVOLUCIONES POS
									IF (vsTipoTransaccion325 = '21')  THEN --REGISTRA LAS DEVOLUCIONES PREVIAMENTE CLASIFICADAS EN LA TABLA DE Td_DevolucionesPOS  -- SE INCLUYEN LAS TIPO 0
										
                                        EXECUTE PROCEDURE BdiTarjeta:"informix".sp_ConcReing_Clasifica_DevPos ( 
															vsNombreArchivo, 
															vsArchivo_Origen, 
															vdtFecha_Archivo, 
															vsSistema_Registro, 
															...
															vsSecuencia325, 
															vsMonto325,
														    vsMontoCashBack325, --Agregando datos de Cash Back
															vsNomcomercio325, 
															vsReferencia23_325, 
															vmMontoIntercard, 
															vmMontoCashBack,  -- Agregando datos de cash Back
															viTipo_Conciliacion ) 
										INTO vsCodRet2, vsMensaje_Respuesta, viElemento;
                                        
										IF (vsCodRet2 <> '00000') THEN --VALIDA QUE EL REGISTRO DE LAS DEVOLUCIONES FUE CORRECTO
											EXECUTE PROCEDURE BdiTarjeta:"informix".Sp_ConcReing_GuardaBitacora (viElemento, '(' || vsCodRet || ') [' || vsNombreArchivo || ']' || vsMensaje_Respuesta, psCve_usuario) INTO vsCodRet2;
										END IF;
                                        
									END IF; -- Cierre (vsTipoTransaccion325 = '21')
							...
							LET vsMensaje_Respuesta = 'ACTUALIZA LA HORA DE FIN DE LA CONCILIACIÃN DE LOS REGISTROS Y TOTALES.';
							...
							UPDATE BdiTarjeta:"informix".Td_Archivos_Conciliacion
							SET Fecha_Hora_Fin_Concilia_Reg = (SELECT DBINFO('utc_to_datetime', Sh_Curtime)::DATETIME YEAR TO FRACTION(5) FROM SysMaster:"informix".Sysshmvals) 
							WHERE NombreArchivo = vsNombreArchivo 
                                AND Archivo_Origen = vsArchivo_Origen 
                                AND Fecha_Archivo = vdtFecha_Archivo;
				...
				--VALIDA SI TIENE ARCHIVO COMPANERO Y ESTE ESTA CONCILIADO CORRECTAMENTE
                --VALIDA SI EL ARCHIVO REQUIERE CONCILIACION ADMINISTRATIVA/GENERACION DE ARCHIVO DE COMISIONES
				IF (
                    ( (vsCodRet = '00000')
                        AND (vsConciliacion_Admin = 'V') 
                        AND (TRIM(vsArchivo_Report_Comisiones /*ACI,ACC,ACT*/) <> '') )
                        AND (
                            (TRIM(vsArchivo_Companero) = '') -- ARCHIVOS QUE NO TIENEN ARCHIVO COMPANERO ---(TPD)
                                OR (
                                    (TRIM(vsArchivo_Companero) <> '') --ARCHIVOS QUE TIENEN ARCHIVO COMPANERO ---(TCC, TCD, CCP, CCD)
                                        AND (
                                        EXISTS (
                                            SELECT NombreArchivo FROM BdiTarjeta:"informix".Td_Archivos_Conciliacion 
                                            WHERE NombreArchivo = vsNombreArchivoCompanero 
                                            AND Proceso = 'T' AND ConAdmin <> 'V')
                                        ) --BUSCA EL ARCHIVO CONCILIADO CORRECTAMENTE EN LA TD_ARCHIVOS_CONCILIACION
                                )
                            )
                ) THEN
						--OBTIENE LAS COMISIONES PARA DEBITO 
						
						
                        
						--- OBTENER EL PARAMETRO DEL VALOR DE LA COMISION DE TARJETA COPPEL DEBITO EN MN
						SELECT FIRST 1 TRIM(NVL(Valor, 0.00))
						INTO vmComisionDeb
						FROM BdiTarjeta:"informix".Td_Param_Conciliacion_ConcReing 
						WHERE Codigo BETWEEN 301 AND 350
                            AND Descripcion MATCHES 'COMISION*' 
                            AND Descripcion MATCHES '*(' || vsArchivo_Report_Comisiones || '-D)';
                        
						--OBTIENE LAS COMISIONES PARA CREDITO 
						...
						WHERE Codigo BETWEEN 301 AND 350
                            AND Descripcion MATCHES 'COMISION*' 
                            AND Descripcion MATCHES '*(' || vsArchivo_Report_Comisiones || '-C)';
                        
						--OBTIENE LOS IVAS DE LAS COMISIONES
						
						
						--- OBTENER EL PARAMETRO DEL VALOR DEL IVA DE TARJETA COPPEL DEBITO 
						SELECT FIRST 1 TRIM(NVL(Valor, 0.00))
						INTO vmIVAComisionDeb
						FROM BdiTarjeta:"informix".Td_Param_Conciliacion_ConcReing 
						WHERE Codigo BETWEEN 301 AND 350
						AND Descripcion MATCHES 'IVA COMISION*' 
						AND Descripcion MATCHES '*(' || vsArchivo_Report_Comisiones || '-D)';
						...
						SELECT FIRST 1 TRIM(NVL(Valor, 0.00))
						INTO vmIVAComisionCred
						FROM BdiTarjeta:"informix".Td_Param_Conciliacion_ConcReing 
						WHERE Codigo BETWEEN 301 AND 350
						AND Descripcion MATCHES 'IVA COMISION*' 
						AND Descripcion MATCHES '*(' || vsArchivo_Report_Comisiones || '-C)';
                        
						--CONCILIACION ADMINISTRATIVA -- GENERACION DE ARCHIVO DE COMISIONES
						LET vsMensaje_Respuesta = 'GENERACIÃN DE ARCHIVO DE COMISIONES.';
						
                        --EXECUTE PROCEDURE

						--TRACE 'despues porfiii '  || txn_CoppelBot;
						
						EXECUTE PROCEDURE BdiTarjeta:"informix".Sp_ConcReing_GenArCom
						( 
							vsRuta_Archivo_Comisiones, 
							NVL(DECODE(vsSistema, 'D', vsNombreArchivo, vsNombreArchivoCompanero), ''), 
							NVL(DECODE(vsSistema, 'C', vsNombreArchivo, vsNombreArchivoCompanero), ''), 
							NVL(DECODE(vsSistema, 'D', vsArchivo_Origen, vsArchivo_Companero), ''), 
							NVL(DECODE(vsSistema, 'C', vsArchivo_Origen, vsArchivo_Companero), ''), 
							vsArchivo_Report_Comisiones, 
							vsNombreArchivo_Comi, 
							vsBinDebito, --NO SE TOMA MAS EN CUENTA EL BIN DE DEBITO, SE CONCULTA INTERNAMENTE POR LA EXISTENCIA DE MAS DE UN BIN DE CREDITO.
							vsBinCredito, 
							psCve_Usuario, 
							NVL(vmComisionDeb, 0.00), 
							NVL(vmComisionCred, 0.00), 
							NVL(vmIVAComisionDeb, 0.00), 
							NVL(vmIVAComisionCred, 0.00),
							txn_CoppelBot,
							vsNomArch_ComiBot
						)
						INTO vsCodRet, vsMensaje_Respuesta, viElemento;
					...
					--ACTUALIZA LA HORA DE FIN DE PROCESO DEL ARCHIVO
					UPDATE BdiTarjeta:"informix".Td_Archivos_Conciliacion
                        SET Fecha_Hora_Gen_ConAdmin = (SELECT DBINFO('utc_to_datetime', Sh_Curtime)::DATETIME YEAR TO FRACTION(5) FROM SysMaster:"informix".Sysshmvals), 
                        ConAdmin = DECODE(vsCodRet, '00000', 'V'/*OK*/, 'F'/*ERROR*/) 
					WHERE NombreArchivo IN (vsNombreArchivo, vsNombreArchivoCompanero)
                        AND Archivo_Origen IN (vsArchivo_Origen, vsArchivo_Companero)
                        AND Fecha_Archivo = vdtFecha_Archivo;
				END IF;
			...
			IF ( EXISTS (SELECT Descripcion FROM BdiTarjeta:"informix".Td_Param_Conciliacion_ConcReing WHERE Codigo = '002' AND Descripcion = 'PARO DE EMERGENCIA DE CONCILIACION' AND TRIM(Valor) = 'V') ) THEN --VALIDA QUE SI EXISTE UNA ORDEN DE DETENER LA CONCILIACION (CONCREIN)
		...
		IF (EXISTS (SELECT NombreArchivo FROM BdiTarjeta:"informix".Td_Archivos_Conciliacion WHERE Proceso = 'X')) THEN
	...
	IF (vsCodRet <> '00000') THEN
		...
		EXECUTE PROCEDURE BdiTarjeta:"informix".Sp_ConcReing_GuardaBitacora (viElemento, '(' || vsCodRet || ') ' || vsMensaje_Respuesta, psCve_usuario) INTO vsCodRet2;
	ELSE
		LET vsMensaje_Respuesta = 'CONCILIACIÃN FINALIZADA.' 
...
'Proyecto: Reingenieria Conciliacion',
'Solicito: Jose Luis Puebla',
'Descripcion: SE AGREGO EL CAMPO Tipo_LayOut A LA TABLA  td_Archivos_concilicion.',
'Fecha: 2011/06/24',
'Version: 20110624.1000',
'BD: BdiTarjeta',
'+++++MODIFICACION: Hector Juan Casanova Edeza',
'Proyecto: Reingenieria Conciliacion',
'Solicito: Jose Luis Puebla',
'Descripcion: SE AGREGARON CAMPOS A LA TABLA  td_Archivos_concilicion CON AL FINALIDAD DE PARAMETRIZAR LOS ARCHIVOS A PROCESAR.',
'Fecha: 2011/06/29',
'Version: 20110629.1530',
'BD: BdiTarjeta',
'+++++MODIFICACION: Hector Juan Casanova Edeza',
'Proyecto: Reingenieria Conciliacion',
'Solicito: Jose Luis Puebla',
'Descripcion: SE AGREGA UNCAMPO A LA TABLA  td_Movimientos_concilicion CON AL FINALIDAD DE MARCAR LOS REGISTROS PROCESADOS.',
'Fecha: 2011/07/20',
'Version: 20110720.1231',
'BD: BdiTarjeta',
'+++++MODIFICACION: Hector Juan Casanova Edeza',
'Proyecto: Reingenieria Conciliacion',
'Solicito: Jose Luis Puebla',
'Descripcion: SE AGREGA UNA VALIDACION PARA QUE LOS REGISTROS CON EL CICLO DE CONCILIACION INCOMPLETO OMITAN REPETIR EL PASO DE CONCILIACION INTERCARD.',
'Fecha: 2011/10/07',
'Version: 20111007.1702',
'BD: BdiTarjeta',
'+++++MODIFICACION: ARTURO MENDEZ CARDENAS',
'Proyecto: ReingenieriaConciliacionAutomatica',
'Solicito: Luis Gomez Santiago',
'Descripcion: SE AGREGA EL NUEVO BIN 416916 DEBC .',
'Fecha: 2011/04/17',
'Version: 20110417.1542',
'BD: BdiTarjeta',
'+++++MODIFICACION: Hector Juan Casanova Edeza',
'Proyecto: Reingenieria Conciliacion - BINES',
'Solicito: Luis Gomez Santiago',
...
'Fecha: 2012/04/20',
'Version: 20120420.0944',
'BD: BdiTarjeta',
'+++++MODIFICACION: Hector Juan Casanova Edeza',
'Proyecto: Reingenieria Conciliacion - DEVOLUCIONES',
'Solicito: Luis Gomez Santiago',
'Descripcion: SE AGREGA EL PROCEDIMIENTO DE CLASIFICACION DE DEVOLUCIONES QUE PORPORCIONA COMPATIVILIDAD CON EL SISTEMA DE CONCILIACION ANTERIOR.',
'Fecha: 2012/05/07',
'Version: 20120507.1104',
'BD: BdiTarjeta',
'+++++MODIFICACION: Hector Juan Casanova Edeza',
'Proyecto: Reingenieria Conciliacion - DEVOLUCIONES',
'Solicito: Luis Gomez Santiago',
'Descripcion: SE INCLUYEN LAS DEVOLUCIONES DE TIPO 0 AL PROCEDIMIENTO CLASIFICACION DE DEVOLUCIONES.',
'Fecha: 2012/05/21',
'Version: 20120521.0953',
'BD: BdiTarjeta',
'+++++MODIFICACION: Hector Juan Casanova Edeza',
'Proyecto: Reingenieria Conciliacion ',
'Solicito: Luis Gomez Santiago',
'Descripcion: SE AJUSTA LA LOGICA PARA EL CASO DE REGISTROS DE CORRESPONSALES, PARA QUE REGISTRE LA TRANSACCION APLICA CORRESPONDIENTE.',
'Fecha: 2012/07/20',
'Version: 20120720.1910',
'BD: BdiTarjeta',
'+++++MODIFICACION: Hector Juan Casanova Edeza',
'Proyecto: Reingenieria Conciliacion ',
'Solicito: Luis Gomez Santiago',
'Descripcion: SE AGREGA LA OBTENCION DE LOS CAMPOS ISO323 Y MovRev325 PARA LAS CONCILIACION DE ATM.',
'Fecha: 2012/07/27',
'Version: 20120727.1516',
'BD: BdiTarjeta',
'+++++MODIFICACION: Hector Juan Casanova Edeza',
'Proyecto: Reingenieria Conciliacion',
'Solicito: Jose Luis Puebla',
'Descripcion: SE AGREGA EL PARAMETRO PARA MANDAR LA TRANSACCION DE CARGO A LA CONSIF.',
'Fecha: 2012/07/31',
...
'Proyecto: Reingenieria Conciliacion',
'Solicito: Jose Luis Puebla',
'Descripcion: SE MODIFICA LA LOGICA PARA OBTENER TENER LOS PARAMETROS PARA LAS TRANSACCIONES ADICIONALES DE LOS ARCHIVOS Y MANDARLOS AL SP_CONCREING_CONSIF.',
'Fecha: 2012/10/01',
'Version: 20121001.1118',
'BD: BdiTarjeta',
'+++++MODIFICACION: Hector Juan Casanova Edeza',
'Proyecto: Reingenieria Conciliacion',
'Solicito: Jose Luis Puebla',
...
'Fecha: 2012/10/12',
'Version: 20121012.1100',
'BD: BdiTarjeta',
'+++++MODIFICACION: Ricardo ResÃ©ndiz Martinez',
'Proyecto: Reingenieria Conciliacion',
'Solicito: Luis Antonio Gomez',
'Descripcion: Modificacion al proceso de conciliacion y a los llamos internos de spl por la introducciÃ³n del Monto Cash Back',
'Fecha: 2013/08/02',
'Version: 20120802.1305',
'BD: BdiTarjeta',
'+++++MODIFICACION: Ricardo ResÃ©ndiz Martinez',
'Proyecto: Reingenieria Conciliacion',
'Solicito: Luis Antonio Gomez',
'Descripcion: Modificacion al proceso de conciliacion para integraciÃ³n de MasterCard',
'Fecha: 2014/04/09',
'Version: 20140409.1750',
'BD: BdiTarjeta',
'+++++MODIFICACION: Ricardo ResÃ©ndiz Martinez',
'Proyecto: Transfer',
'Solicito: Jose Luis Puebla',
'Descripcion: Modificacion al proceso de conciliacion para integraciÃ³n de Transfer',
'Fecha: 2014/04/09',
'Version: 20140409.1750',
'BD: BdiTarjeta',
'+++++MODIFICACION: L.I.A. Ricardo ResÃ©ndiz Martinez',
'Proyecto: Agregar nuevas transacciones de Corresponsales',
'Solicito: Jose Luis Puebla',
'Descripcion: Se agrega ciclo para identificar el tipo de movimiento en DEbito o credito de archivos de corresponsales y poder asi mandar la transaccion correcta ',
'Fecha: 2015/10/01',
'Version: 20151001.1200',
'BD: BdiTarjeta',
'+++++MODIFICACION: L.I.A. Ricardo ResÃ©ndiz Martinez',
'Proyecto: Se agrega flujo para transacciones Forzadas',
'Solicito: Jose Luis Puebla',
'Descripcion: Se agrega ciclo para identificar el tipo de movimiento en DEbito o credito de archivos de corresponsales y poder asi mandar la transaccion correcta ',
'Fecha: 2015/11/23',
'Version: 20151123.1900',
'+++++MODIFICACION: L.I.A. Ricardo ResÃ©ndiz Martinez',
'Proyecto: Se agrega flujo para transacciones Forzadas',
'Solicito: Jose Luis Puebla',
'Descripcion: Se agregan nuevas transacciones de transferencias',
'Fecha: 2016/11/09',
'Version: 20161109.1300',
'BD: BdiTarjeta',
'+++++MODIFICACION: Miguel Lopez / Fernanda Ortiz',
'Proyecto: Optimizacion Conciliacion Automatica',
'Solicito: Base de Datos con atenciÃ³n a reporte de ProducciÃ³n',
'Descripcion: Se mejora commit paraa fin de resolver bloques grande de informacion duranrte el proceso',
'Fecha: 2023/11/14',
'Version: 20231114.1300',
'BD: BdiTarjeta';
