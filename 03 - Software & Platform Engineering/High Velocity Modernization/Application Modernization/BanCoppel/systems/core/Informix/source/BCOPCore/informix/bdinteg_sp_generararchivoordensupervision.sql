CREATE PROCEDURE "informix".sp_generararchivoordensupervision(pempresa CHAR(3), pFechaAct DATE)
                                         RETURNING CHAR(6); 
...
LET vaceptadosupervisadorechazado = ''; --DEFAULT P
			...
			FOREACH				
				...
				IF vnumerosolicituddecredito <> '' THEN --OR vnumcte <> '' THEN 
					...
					IF (SELECT COUNT(num_solicitud) FROM bdisolic:"informix".ss_solicitud_os WHERE num_solicitud = vnumerosolicituddecredito) > 1 THEN
						...
						--LET vfolioanterior = vtiendafolioanterior || vfolioanterior;
					END IF;
					
					--LET vfolio = vtiendafolio || vfolio;
					...
					IF vnumcte <> '' THEN 
						...
						IF 	vestadocivil = 'C' THEN
							...
							WHERE numcte = vnumcte AND numcte_banco <> "" AND secuencia = iSecuencia;
							...
							FROM bdinteg:"informix".si_refdirecciones 	
							WHERE numcte = vnumcte AND tipo_dir = '1' AND secuencia = iSecuencia;
						...
						ELSE
							...
							AND secuenciaos = (SELECT NVL(MAX(secuenciaos), '') FROM bdisolic:"informix".ss_solicitud_os WHERE num_solicitud = vnumerosolicituddecredito);
						END IF; 							
...
--***********************************************************************
--| Procedimiento   : "informix".sp_GenerarArchivoOrdenSupervision
--| Versión         : 1.0
...
--| Descripción     : Se modifica el tipo de consulta para las solicitudes enviadas a OS, se obtiene las respues y se construyen los folios.
--************************************************************************
END PROCEDURE;