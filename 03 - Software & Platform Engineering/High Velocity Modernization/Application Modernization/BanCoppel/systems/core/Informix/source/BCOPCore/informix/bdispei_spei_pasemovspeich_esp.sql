CREATE PROCEDURE "informix".spei_pasemovspeich_esp( pfechaejecuta DATE )
returning char(5),char(100);
    ...
    ON EXCEPTION SET vsql_err, visam_err
		SET DEBUG FILE TO "/resplogifx/conciliachq/spei_pasemovspeich_esp.err";
		TRACE ON;
        IF vsql_err <> 0 THEN
    ...
    --- SET debug file to '/resplogifx/conciliachq/spei_pasemovspeich_esp.out';
    --- trace on;
    ...
    IF (vestadoctrl IS NULL) OR (vestadoctrl = '') OR (vestadoctrl = '0') THEN
        ...
        FOREACH WITH HOLD
            SELECT intfoliopago, intpkpago, intcvecausadev, intpkpaqueteenv, mnyimporte, cvecesifbcoord, cvecesifbcodest, chrestatusenvio, 
