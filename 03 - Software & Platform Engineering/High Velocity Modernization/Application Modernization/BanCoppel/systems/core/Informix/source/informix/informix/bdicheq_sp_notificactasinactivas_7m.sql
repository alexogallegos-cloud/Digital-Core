CREATE PROCEDURE "informix".sp_notificactasinactivas_7m( pEmpresa char(3) )
RETURNING CHAR(5);
	...
	DEFINE vDiasInformada7m INTEGER;
    ...
    ON EXCEPTION SET Sql_Err, Isam_Err, Desc_Err
        SET DEBUG FILE TO "/resplogifx/conciliachq/sp_notificactasinactivas_7m.err";
        TRACE ON;
        IF Sql_Err <> 0 THEN
    ...
    ---SET DEBUG FILE TO "/resplogifx/conciliachq/sp_notificactasinactivas_7m.out";
    ---TRACE ON;
	...
	SELECT valor::INT
      INTO vDiasInformada7m
      FROM bdicheq:sc_param
     WHERE empresa = pEmpresa
       AND codparam = 'DiasIniCtaInformada7';
	   Let vDi7m = vDiasInformada7m;
	 ...
	 FOREACH WITH HOLD
		SELECT CASE WHEN fecultdep > fecultret THEN fecultdep
         ...
         WHERE mae.status_cta IN('1','4')
           ...
           AND ( ( ( vFechaHoy - mae.fecultdep ) = vDiasInformada7m ) OR ( ( vFechaHoy - mae.fecultret ) = vDiasInformada7m ) ) 
	  ...
	  BEGIN WORK;        
		...
		IF (vDiasSinTransacc == vDi7m) THEN
		
		LET vTermCta = TRIM(SUBSTR(vCuenta,8,4));
			      
			    ---- REALIZA LA NOTIFICACION MENDIANTE SMS
				EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento('2','CTAS_INACS','CTAS_INACS3',vNumCliente,'','','2',vTermCta,'','','','','','','','','','','',0,0,0,0,0,'','') ---- NotificaciÃ³n de SMS 2a 7m (946 DÃ­as)  
			    INTO vSp_CodRet; 

				---- REALIZA LA NOTIFICACION MENDIANTE CORREO ELECTRONICO			  
			    EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento('1','CTAS_INAC','CTAS_INAC2',vNumCliente,'','','2',vTermCta,'150','','',TRIM(vDescProd),'','','','','','','',0,0,0,0,0,CURRENT,'') ----  NotificaciÃ³n de Email 2a 7m (946 DÃ­as)
                INTO vSp_CodRet;

			INSERT INTO bdicheq:control_ctas_7m 
