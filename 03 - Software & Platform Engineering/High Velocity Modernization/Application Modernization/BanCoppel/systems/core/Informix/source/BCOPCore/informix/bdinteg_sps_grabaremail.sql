CREATE PROCEDURE "informix".sps_grabaremail (pEmpresa CHAR(3), pNumCte CHAR(20), pEMail CHAR(100))
   ...
   --TRACE ON;
    ...
    BEGIN
		...
		IF (pEMail <> '') THEN
            ...
            EXECUTE PROCEDURE bdinteg:"informix".sp_registra_correos(pEmpresa,pNumCte,pEMail,v_TipoCorreo,v_Canal,v_UserInsert)
                            INTO v_codret1;
			END IF;				
        END IF;
        RETURN sCodRet, cErrorInfo;
...
CREATE PROCEDURE "informix".sp_consultaorigenpoliza_clubfam
(
   pEmpresa CHAR(3),
   pNumCte CHAR(20),
   pTipoCte INTEGER
...
RETURNING CHAR(6) AS cCodRet,
		  CHAR(1) AS cOrigenPoliza,
		  CHAR(20)  AS cNumPoliza;
...
BEGIN
    ...
    ON EXCEPTION SET iSql_err
        IF iSql_err <> 0 THEN
           ...
           RETURN cCodRet, cOrigenPol,cNumPoliza;
        END IF;

    END EXCEPTION;

     --SET DEBUG FILE TO "/respaldosbd/sp_consultaorigenpoliza_clubfam.out";
     --TRACE ON;

    SET ISOLATION TO DIRTY READ;
    SET LOCK MODE TO WAIT 3;
	
--	UPDATE STATISTICS MEDIUM FOR TABLE "informix".si_relacion_ctebcplcpl;
	...
	IF NVL(pEmpresa,'') <> '' AND NVL(pNumCte,'') <> '' AND NVL(pTipoCte,0) <> 0  THEN
		...
		IF pTipoCte = 1 OR cOrigenPol <> 'N' THEN
			...
			SELECT  TRIM(NVL(numcte,'')),TRIM(NVL(num_poliza,''))
			INTO cNumCte,cNumPoliza
...
DOCUMENT
"Folio:87.2",
"Proyecto: RQM 10 745 Protección Familiar Coppel en Bancoppel",
"Autor:92597688 -Yadira Morales Zazueta",
"Fecha:20/09/2016",
"Descripción: Se crea Clon de SP para validar si una póliza del Club de Proteccion familar nació en BanCoppel",
"Sustento: RQM 10 745 Protección Familiar Coppel en Bancoppel.pdf",
