CREATE PROCEDURE "informix".sp_valida_huellaine(pOcr char(20)) 
                                                        
    RETURNING CHAR(5);

    DEFINE cCodRet CHAR(5);    
    DEFINE iSqlErr INTEGER;
    DEFINE ILongOcr INTEGER;
    DEFINE iBitIne INTEGER;
    ...
    --SET DEBUG FILE TO '/ifxsif01/LIP/sp_valida_huellaine.out';
    --TRACE ON;
...
BEGIN
    ...
    IF ILongOcr >= 13 THEN
        ...
        SELECT {+INDEX (si_bitacora_huella_ine, idx_si_bitacora_huella_ine)} COUNT(ocr_ife)
        INTO iBitIne
        FROM "informix".si_bitacora_huella_ine 
        WHERE ocr_ife = pOcr;
    ...
    ELSE 
        ...
        SELECT {+INDEX (si_bitacora_huella_ine, idx_si_bitacora_huella_ine_cic)} COUNT(ocr_ife)
        INTO iBitIne
        FROM "informix".si_bitacora_huella_ine 
        WHERE cic IS NOT NULL AND cic <> ''
		AND cic = pOcr;
...
"FECHA : 16072018",
"BD    : Bdinteg",
"Descripcion: prodecimeinto que verifica si el cliente ya fue validado ante el ine ";
