CREATE PROCEDURE "informix".sp_a_procesar_guias()
returning varchar(5)
...
-- Fecha: 11-08-2010
-- Modificó: Javier Calderón
-- Modificacón: Se modifico para que el archivo creado agregue comillas dobles al valor de los campos excepto los últimos 4
-- Fecha: 11-01-2011
...
DEFINE vsNombreDeArchivo CHAR(255);
...
BEGIN
	...
	--SET DEBUG FILE TO  "/home/informix/ivonne/procesar_guias.out";
    --TRACE ON;
	...
	SELECT date(valor) INTO  vFechaFinal FROM tkn_parametros WHERE id_param = '21';
	...
	IF ( vFechaHoy = vFechaFinal)	THEN
		...
		SELECT COUNT(*), valor INTO vCont, vNombreGuia FROM tkn_parametros WHERE id_param = '19' GROUP BY valor;
		SELECT date(valor) INTO  vFechaInicial FROM tkn_parametros WHERE id_param = '41';
		...
		SELECT LIMIT 1 cte_destino INTO vNumCte FROM  tkn_guias WHERE substring(f_registro::varchar(23) from 1 for 10) BETWEEN vFechaIniArchivo AND vFechaFinArchivo;
		LET vCondicion = " between '"||vFechaIniArchivo||"' AND '" ||vFechaFinArchivo||"'";

		IF (vNumCte	 <> '' OR vNumCte IS NOT NULL) THEN
			IF (vCont <> 0) THEN
				LET vNombreGuia = 'GUIA' || TRIM(vFechaArchivo2) || '.txt';
				UPDATE tkn_parametros SET valor = vNombreGuia WHERE id_param = '19';
			ELSE
				LET vNombreGuia = 'GUIA' || TRIM(vFechaArchivo2) || '.txt';
				INSERT INTO tkn_parametros(id_param,valor,descripcion,f_inicio,f_fin) VALUES('19',vNombreGuia,'Nombre del archivo de guia','2010-01-01 00:00:00','9999-01-01 00:00:00');
			END IF;

			LET vsSQL1 = 'echo "UNLOAD TO ' || TRIM(vDirectorio) ||  TRIM(vsArchTemp)|| ' DELIMITER ' || '''*''';
			LET vsSQL2 = " SELECT  '70','5.0','S','EM6BCUL616', SUBSTR(peso,2,5), flg_retorno, contenido, comentario, replace(factura,'-',' '),comentario, 'BANCOPPEL',cte_destino,'NOR||||1' FROM bdibpi:tkn_guias WHERE substring(f_registro::varchar(23) from 1 for 10)" ||vCondicion;
			LET vsSQL3 = '" >' || TRIM(vDirectorio) || 'guias.sql';
			...
			IF ( vsSQL <> '' ) THEN
				SYSTEM vsSQL ;
				LET vsSQL4 = '' ;
				LET vsSQL4 = 'dbaccess bdibpi ' || TRIM(vDirectorio) || 'guias.sql';
				SYSTEM vsSQL4 ;
				
								
				LET vsSQL = '' ;
				LET vsSQL =  "sed  's/\*/\|/g;s/\NOR||||1|/\NOR||||1/g' "||  TRIM(vDirectorio) || TRIM (vsArchTemp) || " > " || TRIM(vDirectorio) ||  TRIM (vNombreGuia);
				SYSTEM vsSQL ;
				...
				LET vsSQL = '' ;
				LET vsSQL = 'rm ' || TRIM(vDirectorio) || 'guias.sql';
				SYSTEM vsSQL;
				
											
				LET cod_ret = '00000';
				SELECT DBINFO('utc_to_datetime', sh_curtime)::DATETIME YEAR TO SECOND INTO vBatch FROM sysmaster:sysshmvals;
				INSERT INTO tkn_batch (id_proceso,f_proceso,descripcion,f_registro) VALUES (21,vBatch,'Creacion de nuevo archivo',vBatch);
			END IF;
	...
	END IF;
		...
		UPDATE tkn_parametros SET valor = 'GUIA'||vAnio||vMes||vDia||'.txt'  WHERE id_param = '19';
		UPDATE tkn_parametros SET valor = vFechaNueva::varchar(10) WHERE id_param = '21';
		UPDATE tkn_parametros SET valor = vFechaHoy::varchar(10) WHERE id_param = '41';
