CREATE PROCEDURE "informix".sp_exportarcatalogociudades(pCatalogo CHAR(1), pFechaAct DATE, pSeparador CHAR(1),pEjecucion CHAR(1))
...
LET cMensaje      = '';
LET cCadena       = '';
LET vNomArch      = 'si_catciudades_';
LET vNomArchAux   = 'si_catciudades_Aux';
LET vPath         = '';
 ...
 --SET DEBUG FILE TO "/ifxsif01/macf/sp_ExportarCatalogoCiudades.out";
 --TRACE ON;
...
IF (pCatalogo = 1) THEN
    if pEjecucion = 'A' then
        /*LET cCadena = 'echo "unload to ' ||trim(vPath) || trim(vNomArchAux) || ' DELIMITER ''' || pSeparador || ''' SELECT numerociudad, nombreciudad,'
                  || 'inicialciudad, numeroestado, inicialestado, salariominimo, '
                  || 'ivaciudad, TO_CHAR(antiguedadciudad,'|| '''%Y/%m/%d'')' || ', gerentezona, regioncobranzas, unificaciudadescobranzas, gerentecobranzas, '
                  || 'regionestadodecuenta, tipo_ciudad, numerociudadcoppel,'
                  || 'nombreciudadcoppel FROM BDINTEG:si_catciudades" >' ||trim(vPath) || 'corre_si_catciudades.sql';*/
        LET cCadena = 'echo "unload to ' ||trim(vPath) || trim(vNomArchAux) || ' DELIMITER ''' || pSeparador || ''' SELECT numerociudad, limpia_cadena(nombreciudad), '
                  || 'case when nvl(inicialciudad,''' || ''') <> ''' || ''' then limpia_cadena(inicialciudad) else inicialciudad end,'
                  || 'numeroestado, inicialestado, salariominimo, '
                  || 'ivaciudad, TO_CHAR(antiguedadciudad,'|| '''%Y/%m/%d'')' || ', gerentezona, regioncobranzas, unificaciudadescobranzas, gerentecobranzas, '
                  || 'regionestadodecuenta, tipo_ciudad, numerociudadcoppel,'
                  || 'CASE WHEN nvl(nombreciudadcoppel,''' || ''') <> ''' || ''' THEN limpia_cadena(nombreciudadcoppel)||chr(13) ELSE ''' || '''||chr(32)||chr(13) END '
                  || 'FROM BDINTEG:si_catciudades" >' ||trim(vPath) || 'corre_si_catciudades.sql';                  

                  
        System TRIM(cCadena);
        let cCadena = 'dbaccess bdinteg ' || trim(vPath) || 'corre_si_catciudades.sql';
        System TRIM(cCadena);

        LET cCadena = "sed 's/"||pSeparador ||"$//g' "|| trim(vPath) || trim(vNomArchAux) || " >> " ||  trim(vPath) || trim(vNomArch);
        SYSTEM cCadena;
        
        let cCadena = 'rm ' || trim(vPath) || 'corre_si_catciudades.sql';
        System cCadena;

        let cCadena = 'rm ' || trim(vPath) || trim(vNomArchAux );
        System cCadena; 
    else
    /* LET cCadena = 'echo "unload to ' || '''/tmp/' || SUBSTR(vNomArch,1,LENGTH(vNomArch)) || '''' || ' DELIMITER ''' || pSeparador || ''' SELECT numerociudad, nombreciudad,'
                  || 'inicialciudad, numeroestado, inicialestado, salariominimo, '
                  || 'ivaciudad, TO_CHAR(antiguedadciudad,'|| '''%Y/%m/%d'')' || ', gerentezona, regioncobranzas, unificaciudadescobranzas, gerentecobranzas, '
                  || 'regionestadodecuenta, tipo_ciudad, numerociudadcoppel,'
                  || 'nombreciudadcoppel FROM BDINTEG:si_catciudades" > /tmp/corre_si_catciudades.sql';

    System cCadena;

    let cCadena = 'dbaccess bdinteg /tmp/corre_si_catciudades.sql';

    System cCadena;*/

    let cCadena = 'rm /tmp/corre_si_catciudades.sql';

    --System cCadena;    
    end if;
ELIF (pCatalogo = 0) THEN
     if pEjecucion = 'A' then
      /*  LET cCadena = 'echo "unload to ' || SUBSTR(vPath,1,LENGTH(vPath)) || SUBSTR(vNomArch,1,LENGTH(vNomArch)) || ' DELIMITER ''' || pSeparador || ''' SELECT numerociudad, nombreciudad,'
                  || 'inicialciudad, numeroestado, inicialestado, salariominimo, '
                  || 'ivaciudad, TO_CHAR(antiguedadciudad,'|| '''%Y/%m/%d'')' || ', gerentezona, regioncobranzas, unificaciudadescobranzas, gerentecobranzas, '
                  || 'regionestadodecuenta, tipo_ciudad, numerociudadcoppel,'
                  || 'nombreciudadcoppel FROM BDINTEG:si_catciudades" >' || SUBSTR(vPath,1,LENGTH(vPath)) || 'corre_si_catciudades.sql';
        System cCadena;
        let cCadena = 'dbaccess bdinteg ' || SUBSTR(vPath,1,LENGTH(vPath)) || 'corre_si_catciudades.sql';
        System cCadena; */
        let cCadena = 'rm ' || SUBSTR(vPath,1,LENGTH(vPath)) || 'corre_si_catciudades.sql';
        --System cCadena;
