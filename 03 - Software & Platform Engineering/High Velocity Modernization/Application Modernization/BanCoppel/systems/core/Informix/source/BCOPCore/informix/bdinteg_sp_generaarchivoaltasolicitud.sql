CREATE PROCEDURE "informix".sp_generaarchivoaltasolicitud(pempresa CHAR(3), pFechaAct DATE)
RETURNING CHAR(6);
...
LET vTipoProducto = '11000'; --DEFAULT 11000
...
--Set debug file to '/tmp/sp_GeneraArchivoAltaSolicitud.out';
--trace on;
...
BEGIN
	...
	IF pFechaAct <> mdy(1,1,1900) OR pFechaAct IS NOT NULL THEN	
		...
		ELSE
			...
			FOREACH
				...
				WHERE num_producto = '6500' AND fecha_insert = pFechaAct AND status_solicitud <> 'PC'
				...
				IF vnumerosolicituddecredito <> '' OR vnumcte <> '' THEN 
					...
					dir.edificio, dir.entrada, TRIM(NVL(tel1.telefono1,0)), TRIM(NVL(tel2.telefono2,0)), dir.tipo_dir 
					...
					WHERE grupo = 6 AND seccion = 2 AND tpo_persona = '01' AND num_solicitud = vnumerosolicituddecredito;
					...
					--LET vfolio = TRIM(TRIM(vtiendafolio2)||TRIM(vfolio2));
					...
					LET vsSQL = vsSQL ||"|"||NVL(vnumerodependientes, 0)||"|"||NVL(vpersonastrabajan, 0)||"|"||NVL(vlimitecredito, 0)||"|"||NVL(vingresomensual, 0)||"|"||TRIM(vsituacionespecial)||"|"||vcausasituacionespecial||"|"
	...
	IF EXISTS (SELECT empresa, secuencia, sucursal, trama, tipomovto, fecha_insert FROM bdinteg:"informix".si_archivoscoppeldiario WHERE empresa = pempresa AND tipomovto = '' AND fecha_insert = pFechaAct) THEN
		LET vCodRetorno = '000000';
	ELSE
