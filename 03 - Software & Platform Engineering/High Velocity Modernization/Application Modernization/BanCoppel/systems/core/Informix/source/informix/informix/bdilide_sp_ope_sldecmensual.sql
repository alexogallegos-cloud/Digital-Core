CREATE PROCEDURE "informix".sp_ope_sldecmensual(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaProceso DATE, pTipoDecl CHAR(1), pFechaPresentacion DATE, pNumFolio CHAR(16))
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			...
			WHERE usuario_insert = pUsuario AND nombre_procedimiento = 'sp_ope_sldecmensual' AND fecha_fin IS NULL; 
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ope_sldecmensual.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pFechaProceso IS NULL OR pTipoDecl = '' THEN
			LET cCodRet = '00003';
		...
		VALUES(pUsuario, 'sp_ope_sldecmensual', CURRENT, null);

		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;
		
		
		EXECUTE PROCEDURE bdilide:"informix".spsldecmensual2(pFechaProceso, pUsuario, pTipoDecl, pFechaPresentacion, pNumFolio)
		INTO cCodRetSp, cMensaje;
		
		--EXECUTE PROCEDURE bdilide:"informix".spsldecmensual(pFechaProceso, pUsuario, pTipoDecl, pFechaPresentacion, pNumFolio)
		--INTO cCodRetSp, cMensaje;
		
		LET iCodRetSp = cCodRetSp::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, "ERROR EN LA EJECUCION DEL SP spsldecmensual";
		ELIF iCodRetSp = 1 THEN
			LET cCodRet = '01222'; -- EL REPORTE SE GENERO CON ERRORES, NO GENERAR XML
