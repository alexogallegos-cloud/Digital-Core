CREATE PROCEDURE "informix".sp_generareportecreditoclasico_reproceso(dFechaIn DATETIME year to fraction(5),dFechaFin DATETIME year to fraction(5),cTrimestre cHAR(5),iMes INTEGER)
returning
char (5),
char(50);
...
--Set debug file to "/informix/pruebasconciliacion/sp_generareportecreditoclasico.sql";
--trace on;
...
LET v_consulta = '';
	...
	begin work;
		CREATE INDEX idx_tmpmovimientostrim_01 ON bdireports:tmpmovimientostrim (ProdInd,CodigoIso) ONLINE;
	commit work;
	begin work;
		CREATE INDEX idx_tmpmovimientostrim_02 ON bdireports:tmpmovimientostrim (CodigoIso) ONLINE;
	commit work;
...
--Esta informacion solo se corre si es final de trimestre
		...
		LET dNumCuentasCredito = 2271808;
		...
		LET dNumTarjetas = 2295001 ;
		
		LET dNumTarjetas = dNumTarjetas + dNumTarjetasMod ;
		
		--select count(numtarjeta) INTO dNumTarjetas
		--from  intercard:tarjeta  where  codproductotarjeta in('001','002','003','004')
		--and fechaasignacion <= '2013-03-01 23:59:59.9' --dFechaFin 
		--and ((codstatustarjeta in ('CAN','INA','DES','FAL','EXT','DAN','ROB')
        --and fechaultmodif::date >= '2013-01-01' and fechaultmodif::date <= '2013-03-31') OR   --Agregar los periodos de inicio o fin
		--codstatustarjeta not in ('CAN','INA','DES','FAL','EXT','DAN','ROB'));
		...
		/*SET ISOLATION TO DIRTY READ;
		SELECT SUM(monto) INTO mCargoPorFinTotal FROM bdicred:sd_movhis WHERE empresa= '001' and fecha_mov between dFechaIn and dFechaFin
		and num_credito > 0 and codigo_fun='605' and codigo_ref='2' and reversado = 'N';*/
		...
		SELECT {+INDEX(bdicred:sd_movhis inx_movhis)} SUM(NVL(monto,0)) INTO mPagosTotal1 FROM bdicred:sd_movhis WHERE empresa= '001' and fecha_mov between dFechaIn and dFechaFin
		and num_credito > 0 and codigo_fun in ('334','335') and codigo_ref in ('7','8','10') and reversado = 'N';
		
		SELECT {+INDEX(bdicred:sd_movhis inx_movhis)} SUM(NVL(monto,0)) INTO mPagosTotal2 FROM bdicred:sd_movhis WHERE empresa= '001' and fecha_mov between dFechaIn and dFechaFin
		and num_credito > 0 and codigo_fun = '033' and codigo_ref in ('7','8','10','901') and reversado = 'N';
		
		SELECT {+INDEX(bdicred:sd_movhis inx_movhis)} SUM(NVL(monto,0)) INTO mPagosTotal3 FROM bdicred:sd_movhis WHERE empresa= '001' and fecha_mov between dFechaIn and dFechaFin
		and num_credito > 0 and codigo_fun = '336' and codigo_ref in ('7','8','10','901') and reversado = 'N';
		
		SELECT {+INDEX(bdicred:sd_movhis inx_movhis)} SUM(NVL(monto,0)) INTO mPagosTotal4 FROM bdicred:sd_movhis WHERE empresa= '001' and fecha_mov between dFechaIn and dFechaFin
		and num_credito > 0 and codigo_fun ='337' and codigo_ref in ('10','901') and reversado = 'N';

		SELECT {+INDEX(bdicred:sd_movhis inx_movhis)} SUM(NVL(monto,0)) INTO mPagosTotal5 FROM bdicred:sd_movhis WHERE empresa= '001' and fecha_mov between dFechaIn and dFechaFin
		and num_credito > 0 and codigo_fun ='342' and codigo_ref='1' and reversado = 'N';

		LET mPagosTotal = mPagosTotal1 + mPagosTotal2 + mPagosTotal3 + mPagosTotal4 + mPagosTotal5 ; 
		*/
		LET mPagosTotal = 4876989351.96 ;
		----Debitos Miscelaneos 
		/*
		SET ISOLATION TO DIRTY READ;
