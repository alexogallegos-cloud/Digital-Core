CREATE PROCEDURE "informix".sps_consulta_referenciasfrec_bpi(pNumCte CHAR(20), pTipoCta CHAR(2), pCveBanco CHAR(3),pReg SMALLINT)
	RETURNING CHAR(5) as codret,CHAR(20) as  alias,CHAR(20) as referencia,CHAR(1) as inhabil, MONEY(16,2) as monto, CHAR(1) as caducidad, CHAR(1) AS activarBPI;

	-- *************************************************
	-- Consulta las referencias frecuentes de pagos de servicios y tiene parÃ?Â¡metro de salida para la clave de caducidad
	-- Bibiana Gaxiola Verdugo.
	-- 19/12/2012
	-- Se agrega validaciÃ³n de registros antes del FOREACH
	-- 19/12/2018
	-----
	-- Se agrega validación de monto 00
	-- Gabreial Aguilar
	-- 11/09/2020
	-- ************************************************* 
...
--AsiganaciÃ?Â³n de valores a las variables
LET v_CodRet	 	='00000';
LET v_Alias		 	='';
LET v_Referencia 	='';
LET v_Cont		 	=0;
...
--SET DEBUG FILE TO "/informix/gaby/ArchivosOut/sps_consulta_referenciasfrec_bpi.out";
--TRACE ON;
...
BEGIN 
	...
	SELECT COUNT(*) 
	INTO v_Cont
	FROM (SELECT cuenta FROM bdiprog:"informix".pp_ctasterceros WHERE num_cte= pNumCte  AND cve_cuenta= pTipoCta AND cve_banco= pCveBanco AND cve_estado='01' UNION ALL
	SELECT cuenta FROM bdiprog:"informix".pp_ctasterceros_bex WHERE num_cte= pNumCte AND cve_cuenta= pTipoCta AND cve_banco= pCveBanco AND cve_estado='01');	 
	
	IF v_Cont > 0 THEN		
		FOREACH
			SELECT SKIP pReg FIRST 10 descrip_cta,cuenta, canal_alta, fecha_insert, hora_insert , NVL(monto_maximo,0), cve_caducidad, "1" AS bandAct 
			INTO v_Alias, v_Referencia, v_Canal, v_FechaInsert, v_HoraInsert, v_MontoMaximo, v_CveCaducidad, v_BandActiva
			...
			AND cve_estado= '01'
			UNION ALL
			SELECT 	descrip_cta,cuenta, canal_alta, fecha_insert, hora_insert , NVL(monto_maximo,0), cve_caducidad, "0" AS bandAct 
			FROM bdiprog:"informix".pp_ctasterceros_bex
			WHERE num_cte= pNumCte
			AND cve_cuenta= pTipoCta
			AND cve_banco= pCveBanco
			AND cve_estado= '01' 
			ORDER BY bandAct ASC, fecha_insert DESC, hora_insert DESC, descrip_cta ASC
				...
				LET v_Inhabil = '';  -- Si el canal es de internet, devolvera solo los registros que tengan 30 minutos o mas transcurridos despues de su alta	 
				...
				LET v_Cont = 1;
				RETURN v_CodRet,v_Alias,v_Referencia,v_Inhabil,v_MontoMaximo, v_CveCaducidad, v_BandActiva WITH RESUME;
...
'Fecha: 28/12/2018',
'Modificacion: Se agrega parametro v_BandActiva para validar Cuentas Frecuentes dadas de Alta por BancaExpress',
