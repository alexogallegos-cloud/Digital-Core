CREATE PROCEDURE "informix".sp_consultarinforeportecac( p_Empresa CHAR(3), p_Area CHAR(2), p_FechaIni CHAR(10), p_FechaFin CHAR(10), p_StaIni CHAR(2), p_StaFin CHAR (2), pProducto CHAR(4) )

-- CONTROL DE CAMBIOS:
-------------------------------------------------------------------------------------------------
-- ModificÃ³: Viridiana Sobampo Aguilar
-- DescripciÃ³n: Se modifica para obtener la cantidad y porcentaje respectivo de las solicitudes
--              que se encuentran en estatus Catalogo en estudio (CE)
-- Fecha:  14-Sep-2010
-- PeticiÃ³n: RQM 09-154-2
-------------------------------------------------------------------------------------------------

RETURNING
	CHAR(5) AS cod_ret,
	INTEGER AS SolAnalizadas,
	INTEGER AS SolRechazadas,
	DECIMAL(10,2) AS PorcRechazadas,
	INTEGER AS SolEE,
	DECIMAL(10,2) AS PorcEE,
	INTEGER AS SolAutorizadas,
	DECIMAL(10,2) AS PorcAT,
	INTEGER AS SolCE,
	DECIMAL(10,2) AS PorcCE,
	INTEGER AS SolEnProceso,
	DECIMAL(10,2) AS PorcEnProceso,
	CHAR(20) AS NumSolicitud,
	CHAR(20) AS cNumCte,
	CHAR(104) AS NomCliente,
	CHAR(4) AS cSucursal,
	CHAR(2) AS StaInicial,
	CHAR(2) AS StaFinal,
	DATE AS FechaSol,
	DATE AS FechaCambio,
	VARCHAR(3) AS Motivo,
	VARCHAR(250) AS Justificacion,
	CHAR(45) AS Analista,
	CHAR(10) AS Tipo_movto,
	CHAR(50) AS Producto,
	VARCHAR(3) AS Motivo2;
...
DEFINE SolAnalizadas		INTEGER;
DEFINE SolRechazadas		INTEGER;
DEFINE PorcRechazadas		DECIMAL(10,2);
DEFINE SolEE				INTEGER;
DEFINE PorcEE				DECIMAL(10,2);
DEFINE SolAutorizadas		INTEGER;
DEFINE PorcAT				DECIMAL(10,2);
DEFINE SolEnProceso			INTEGER;
DEFINE PorcEnProceso		DECIMAL(10,2);
DEFINE cNumCte				CHAR(20);
DEFINE NombreCte			CHAR(104);
DEFINE NumSolicitud			CHAR(20);
DEFINE cSucursal				CHAR(4);
DEFINE FechaSolicitud		DATE;
DEFINE FechaCambio			DATE;

DEFINE SolCE                INTEGER; 
DEFINE PorcCE               DECIMAL(10,2);       
DEFINE vMotivo          	VARCHAR(3);
DEFINE vComentario         	VARCHAR(250);
DEFINE cNombreAnalista      CHAR(45);
DEFINE cAutorizo      		CHAR(8);
DEFINE cNombreProducto 		CHAR(50);
DEFINE cNumProd 		CHAR(4);
DEFINE cTipoMovto 		CHAR(10);
DEFINE vMotivoIni		CHAR(3);
DEFINE cStatusAnt		CHAR(2);
DEFINE iSecuencia		INTEGER;
DEFINE cStatNvo			CHAR(2);

---INICIALIZACIONES
LET v_cod_ret = '00000';
LET SolAnalizadas			= 0;
LET SolRechazadas			= 0;
LET PorcRechazadas			= 0.0;
LET SolEE					= 0;
LET PorcEE					= 0.0;
LET SolAutorizadas			= 0;
LET PorcAT					= 0.0;
LET SolEnProceso			= 0;
LET PorcEnProceso			= 0.0;
LET cNumCte					= "";
LET NombreCte				= "";
LET NumSolicitud			= "";
LET cSucursal				= "";
LET FechaSolicitud			= MDY(1,1,1900);
LET FechaCambio				= MDY(1,1,1900);
LET SolCE              		= 0; 
LET PorcCE             		= 0;
LET vMotivo	     			= "";
LET vComentario				= "";
...
LET cStatNvo 		= '';
 ...
 BEGIN
	...
	ON EXCEPTION
		...
		RETURN v_cod_ret, 0, 0, 0.00, 0, 0.00, 0, 0.00, 0, 0.00, 0, 0.00, "", "","", "", "", "", DATE(1), DATE(1), "", "", "", "","","";
	END EXCEPTION;
	
	 --SET DEBUG FILE TO "/informix/gpe/sp_consultarinforeportecac.out";
	 --TRACE ON;
	...
	IF NVL(p_Empresa,"") = "" OR NVL(p_Area,"") = "" THEN
		LET v_cod_ret = "00001";
		RETURN v_cod_ret, 0, 0, 0.00, 0, 0.00, 0, 0.00, 0, 0.00, 0, 0.00, "", "", "", "", "", "", DATE(1), DATE(1), "", "", "", "","","";
	END IF
	
	--- SI ES AREA DE CENTRO DE ATENCION A CREDITO O MESA DE CONTROL
	IF p_Area = "01" OR p_Area = "02" THEN
		
		-- VALIDAMOS LOS PARAMETROS 
		IF NVL(p_Empresa,"") = "" OR NVL(p_FechaIni,"") = "" OR NVL(p_FechaFin,"") = "" THEN
			LET v_cod_ret = "00001";
			RETURN v_cod_ret, 0, 0, 0.00, 0, 0.00, 0, 0.00, 0, 0.00, 0, 0.00, "", "","", "", "", "", DATE(1), DATE(1), "", "", "", "","","";
		END IF
		
		FOREACH
			SELECT solicitudes_analizadas, solicitudes_rechazadas, porcentaje_rechazadas, solicitudes_ee, porcentaje_ee,
                               solicitudes_autorizadas, porcentaje_at, solicitudes_ce, porcentaje_ce,solicitudes_en_proceso, 
                               porcentaje_en_proceso, fecha
			  INTO SolAnalizadas, SolRechazadas, PorcRechazadas, SolEE, PorcEE, SolAutorizadas, PorcAT, SolCE, PorcCE,
                               SolEnProceso, PorcEnProceso, FechaSolicitud
			  FROM bdicred:"informix".sd_cifras_operaciones 
			 WHERE empresa = p_Empresa 
			   AND area = p_Area
			   AND fecha BETWEEN p_FechaIni AND p_FechaFin
		
			RETURN v_cod_ret, NVL(SolAnalizadas,0), NVL(SolRechazadas,0), NVL(PorcRechazadas,0), NVL(SolEE,0), NVL(PorcEE,0), 
				   NVL(SolAutorizadas,0), NVL(PorcAT,0), NVL(SolCE,0),NVL(PorcCE,0),NVL(SolEnProceso,0), NVL(PorcEnProceso,0),
					"", "", "", "", "", "", NVL(FechaSolicitud,''), "", NVL(vMotivo,''), NVL(vComentario,''), NVL(cNombreAnalista,''),cTipoMovto, NVL(cNombreProducto,"") ,NVL(vMotivoIni,'') WITH RESUME;
		END FOREACH
		
	ELIF p_Area = "CS" THEN -- REPORTE CAMBIO DE ESTATUS
	
		IF NVL(p_StaFin,"") = "" OR NVL(p_FechaIni,"") = "" OR NVL(p_FechaFin,"") = "" THEN
			LET v_cod_ret = "00002";
			RETURN v_cod_ret, 0, 0, 0.00, 0, 0.00, 0, 0.00, 0, 0.00, 0, 0.00, "", "", "", "", "", "", DATE(1), DATE(1), "", "", "", "","","";
		END IF
		...
		IF NVL(pProducto,"") <> "" THEN
			...
			SELECT valor_alfabetico INTO cNombreProducto FROM bdicobranza:"informix".cb_param_campania 
			WHERE empresa = p_Empresa AND tipo_campania = 55 AND descripcion = pProducto AND grupo_parametro = 'MCTRLINEA';
		END IF 
		
		-- CONSULTAMOS TODOS LOS REPORTES QUE CAMBIARON DE ESTATUS EN UN PERIODO DE FECHAS DETERMINADO.
		FOREACH
			SELECT num_solicitud,fecha_modif, status_nvo, causa_solicitud,
			--CASE WHEN TRIM(status_nvo) IN("RT","CM") THEN causa_solicitud ELSE "" END AS MotivoRTCM,
			comentario, usuario_modif,DECODE(tipo_movimiento,'M','MIXTO','U','UNICO','UNICO'), t2.status_ant ,t2.secuencia 
			INTO  NumSolicitud, FechaCambio, cStatNvo, vMotivo, vComentario, cAutorizo,cTipoMovto, cStatusAnt, iSecuencia
			FROM bdisolic:"informix".ss_autorizacion_especial t2 				 
			WHERE t2.status_nvo = p_StaFin	         
			  AND t2.secuencia = (SELECT MAX(secuencia) FROM bdisolic:"informix".ss_autorizacion_especial aut2 WHERE aut2.num_solicitud = t2.num_solicitud )			  
			  AND t2.fecha_modif >= p_FechaIni::DATE
			  AND t2.fecha_modif <= p_FechaFin::DATE
			  
			IF cStatNvo <> "RT" AND cStatNvo <> "CM" THEN
				LET vMotivo = '';
			END IF;
			
	  
			 IF (p_StaFin = "EE" AND cStatusAnt = "OA") OR (p_StaFin IN("RT","CM") AND cStatusAnt = "MC" )  THEN
				CONTINUE FOREACH;
			  END IF;
			  
			 /* IF cStatusAnt IN("RT","CM")  THEN
				  SELECT LIMIT 1 CASE WHEN TRIM(status_nvo) IN("RT","CM") THEN causa_solicitud ELSE "" END AS MotivoRTCM
						INTO  vMotivoIni
				FROM bdisolic:"informix".ss_autorizacion_especial t2 				 
				WHERE t2.status_nvo = cStatusAnt	         
				  AND t2.secuencia < iSecuencia;
			  ELSE
				LET vMotivoIni= "";
			  END IF;*/
			  SELECT numcte, sucursal, fecha_insert,num_producto
				INTO cNumCte,cSucursal,FechaSolicitud,cNumProd
			  FROM bdisolic:"informix".ss_solicitudes 
			  WHERE empresa = '001'
			  AND num_solicitud = NumSolicitud;
			  --AND status_solicitud = p_StaFin;--GJEV para que se tomen todos los estatus
			  
			  IF NVL(cNumCte,"") = "" THEN
				CONTINUE FOREACH;
			  END IF;	  
			  IF NVL(pProducto,"") <> "" AND NVL(pProducto,"") <> cNumProd THEN
				CONTINUE FOREACH;
			  END IF;	
			  
			  
			  SELECT  trim(nombre1) || " " || trim(nombre2) || " " || trim(apell_paterno) || " " || trim(apell_materno)::CHAR(104)
				INTO NombreCte
			...
			-- CONSULTAMOS EL NOMBRE DEL ANALISTA QUE ESTA PROCESANDO LA SOLICITUD.
			SELECT nombre INTO cNombreAnalista FROM bdinteg:"informix".si_ejecut WHERE ejecutivo = TRIM(cAutorizo);
			
			RETURN v_cod_ret, 0, 0, 0.00, 0, 0.00, 0, 0.00, 0, 0.00, 0, 0.00,
					NVL(NumSolicitud,''), NVL(cNumCte,""), NVL(NombreCte,''),  NVL(cSucursal,''), NVL(cStatusAnt,''), NVL(p_StaFin,''), NVL(FechaSolicitud,''), NVL(FechaCambio,''), NVL(vMotivo,''), 
					NVL(vComentario,''), NVL(cNombreAnalista,'SISTEMA'),cTipoMovto, NVL(cNombreProducto,""), NVL(vMotivoIni,'') WITH RESUME;
		END FOREACH
...
END;
--##############################################################################
--## Procedimiento   : sp_consultarinforeportecac
--## Version         : 1.0
...
'Descripcion: Agregamos el param pProducto y validamos que no muestre el nombre repetido en el reporte de cambio de estatus.',
'Fecha: 11/Diciembre/2012',
'BD: bdicred';

CREATE PROCEDURE "informix".sp_ofi_consultasdos_exp(pEmpresa CHAR(3),pNumCredito CHAR(20),pSucursal CHAR(4))
...
BEGIN
    ...
    ---SELECT {VOID_FULL(bdicred:"informix".sd_movhiscrd)} NVL(SUM(CASE WHEN codigo_ref = 1 AND fecha_mov > dFecha_MesIver THEN monto ELSE 0 END),0), --MENOS SUS ABONOS
...
CREATE PROCEDURE "informix".sp_ofi_consultasdos2(pEmpresa CHAR(3),pNumCredito CHAR(20),pSucursal CHAR(4))
RETURNING CHAR(5) AS Cod_Ret, 
	CHAR(80)  	  AS Mensaje_retorno,
	CHAR(20) 	  AS Num_credito,
	CHAR(4) 	  AS Nun_Producto,
