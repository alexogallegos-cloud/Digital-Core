CREATE PROCEDURE "informix".sp_consultaciudades_coppel(pEstado CHAR(2),pNumCiudad CHAR(3))
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultaciudades_coppel.out';
		--TRACE ON;
		
		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;
  	  
		IF pEstado IS NULL OR pEstado = '' OR pNumCiudad IS NULL OR pNumCiudad = '' THEN
			LET cCodRet = '000001';
			LET cMensajeRet = 'El parÃ¡metro nÃºmero de estado esta vacÃ­o';
			RETURN cCodRet, cMensajeRet,NVL(cCiudad,''),NVL(cNombre,'');
		END IF;
		
		SELECT {+INDEX(si_ciduades ix_2363)} ciudad, nombre
		INTO cCiudad,cNombre
		FROM bdinteg:si_ciudades 
		WHERE pais ='001'	
		AND estado= pEstado
		AND ciudad_coppel=pNumCiudad; 
...
'FECHA: 11/08/2020',
'DESCRIPCION: Se realiza procedimiento para la descripcion de la ciudad del cliente coppel',
'BD:bdinteg';
