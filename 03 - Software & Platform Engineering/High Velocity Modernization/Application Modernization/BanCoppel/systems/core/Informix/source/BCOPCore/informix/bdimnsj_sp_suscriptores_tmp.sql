CREATE PROCEDURE "informix".sp_suscriptores_tmp ( )
...
--	SET DEBUG FILE TO "/informix/rene/MNSJ/sp_suscriptores.OUT";
--	TRACE ON;
...
BEGIN
	...
	IF (vsCodRetorno='00000') THEN
		...
		FOREACH
			...
			select  limit 1000 cte.numcte,cte.apell_paterno,cte.apell_materno,cte.nombre1,cte.nombre2,pf.sexo
			...
			where cte.status_cte = 'AL' AND cte.tipo_cliente ='1'
			-- SE MODIFICA TEMPORALMENTE PARA SOLO CARGAR CIERTOS USUARIOS
			--and cte.numcte in (select numcte from bdinteg:"informix".si_telefonos_actual)			
			and cte.numcte in ('000001544','000004129','000005551','000017993','000021597','000204284','000001010','000116502','003367107','010349236')

			--Nombre del archivo
			LET vsNombreArchivo = 'SUSCP' ||'_'|| NVL(vsDia,'01') || NVL(vsMes,'01') || NVL(vsAnio,'01') ||'_'|| vArch ||'.laebd';

			
			IF vsnumcte <> '' THEN
				EXECUTE PROCEDURE bdinteg:"informix".sp_consulta_telefonos("001",vsnumcte,3,"0") 
