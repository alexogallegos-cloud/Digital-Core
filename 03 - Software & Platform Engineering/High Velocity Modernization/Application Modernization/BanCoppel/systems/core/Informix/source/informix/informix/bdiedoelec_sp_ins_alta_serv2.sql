CREATE PROCEDURE "informix".sp_ins_alta_serv2(pempresa char(3),pnumcte char(20), pcuenta char(20), pproducto char(4), pstatus_serv_elec char(1), pstatus_serv_imp char(1), puser_modif char(20)) 
	...
	DEFINE itipo_cuenta			INTEGER;
	DEFINE v_numcte_cta			INTEGER;
	DEFINE cConsProd  			CHAR(2);

   -- SET DEBUG FILE TO  "/informix/Acuellar/sp_ins_alta_serv.out"; 
   -- TRACE ON;
    ...
    BEGIN
		...
		LET cConsProd = SUBSTR(pcuenta,1,2);
		SELECT (CASE WHEN valor like '%'||cConsProd||'%' THEN 1 ELSE 0 END) INTO itipo_cuenta FROM bdicred:"informix".sd_param WHERE cod_param IN('059');
		
		IF SUBSTR(pcuenta,1,1) = '1' OR SUBSTR(pcuenta,1,1) = '2' THEN
		
			SELECT a.numcte, b.status_cta
			  INTO v_numcte_cap,v_status_cta
			  FROM bdinteg:"informix".si_cliente a, bdicheq:"informix".sc_maechq b 
			 WHERE a.numcte = b.num_cte
			   AND a.numcte = pnumcte
			   AND b.cuenta = pcuenta
			   AND b.producto = pproducto;
				  
		ELIF (itipo_cuenta) = 1 THEN--DSB 23/01/2019
			SELECT a.numcte, b.status_cred 
			  INTO v_numcte_cred, v_status_cred
			  FROM bdinteg:"informix".si_cliente a, bdicred:"informix".sd_maecred b
		...
		ELIF SUBSTR(pcuenta,1,2) IN ('61','63','76','77') THEN	 
			...
			SELECT a.numcte, b.status_cred 
			  ...
			  FROM bdinteg:"informix".si_cliente a, bdicred:"informix".sd_maecredcrd b
			  WHERE a.numcte = b.numcte
				AND a.numcte = pnumcte
				AND b.num_credito = pcuenta
				AND b.num_producto = pproducto;
		...
		SELECT count(*) INTO v_numcte_cta FROM "informix".edelec_alta_serv WHERE numcte = pnumcte AND cuenta = pcuenta AND producto = pproducto;
		
		IF (v_numcte_cta < 1 AND v_dia_corte > 0) or (v_numcte_cta is null  AND v_dia_corte > 0) THEN
						
					INSERT INTO "informix".edelec_alta_serv (empresa,numcte,cuenta,producto,status_serv_elec,status_serv_imp,status_cta,status_cred,fecha_alta_servicio,	 
		...
		ELIF v_numcte_cta > 0 THEN
					...
					IF pstatus_serv_elec = 'I' THEN -- Cancelar Servicio
						UPDATE "informix".edelec_alta_serv SET status_serv_elec = pstatus_serv_elec, status_serv_imp = pstatus_serv_imp, fecha_ultima_mod = TODAY, 
...
END PROCEDURE
DOCUMENT
"DESCRIPCION: Reaaliza consulta de Cliente para regresar la informaciÃ³n de sus cuentas de crÃ©ditos 7000 y 8100 y todas las futuras tarjetas de credito",
"Folio: 531",
...
CREATE PROCEDURE "informix".sp_obtenerctas_cfdi(pEmpresa CHAR(3), 
									 pNumCte CHAR(20),
									 pCuenta CHAR(20),
									 pTarjeta CHAR(20),
									 pTpo SMALLINT,
...
BEGIN
	...
	--SET DEBUG FILE TO '/home/tmp/jairo/sp_obtenerctas_cfdi.out';
	--TRACE ON;
			...
			IF iEjecucion = 0 THEN
				IF pNumCte <> '' OR iSistema = 1 THEN
					...
					FOREACH
						...
						--AND mc.cuenta = CASE WHEN pCuenta = "" THEN mc.cuenta ELSE pCuenta END
				...
				IF iEjecucion in (0,1,2) THEN
					...
					-- *********************************************************************
						SELECT skip sTipo LIMIT iLimit
							...
							fecha_apertura,fecha_vencim, mc.status_cred
						...
						AND pr.num_producto = '6001'
						AND mc.status_cred = tc.status_cred 
						AND mc.status_cred IN ('AA','BA','BT','E1','E2','E3')	--IFRS 
						ORDER BY 1
...
"Sustento: 12 231 Edo Cta Emisión Consulta Disponibilización y Respaldo ",
"Fecha: 27/05/2014",
"Solicitante: Rodolfo Gómez Hernandez",
"BD:bdiedoelec";
