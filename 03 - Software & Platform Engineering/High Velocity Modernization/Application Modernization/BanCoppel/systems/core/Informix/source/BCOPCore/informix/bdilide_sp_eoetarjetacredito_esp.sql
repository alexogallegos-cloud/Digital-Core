CREATE PROCEDURE "informix".sp_eoetarjetacredito_esp( pEmpresa CHAR(3), pFechaProceso DATE, pCveUsuario CHAR(8) )
...
RETURNING CHAR(4);
    ...
    END  EXCEPTION;
     ...
     ---SET DEBUG FILE TO "/tmp/sp_EOETarjetasCredito_esp.out";
     ---TRACE ON;
    ...
    FOREACH WITH HOLD
        -- // Obtener los parametros para extraer las operaciones de crédito
        SELECT valor 
          INTO vcTransaccSuc 
        ...
        FOREACH WITH HOLD
            ...
            SELECT {+INDEX(bdicred:sd_movhis16122009 inx_movhis_16)}
                   movhis.num_credito, movhis.fecha_mov,SUM( nvl(movhis.monto, 0)), nvl(movhis.nro_tarjeta,'')
              INTO vcNumCta, vdFechaMov, vmImpTotDep,vmNumTarjeta
              FROM bdicred:sd_movhis16122009 movhis
             WHERE movhis.empresa = pEmpresa
               AND movhis.num_credito = vcNumCta 
               AND movhis.fecha_mov  = pFechaProceso  
               AND movhis.transacc_suc = vcTransaccSuc  
               AND movhis.codigo_fun = vcCodigoFun 
               AND movhis.codigo_ref = viCodigoRef
               AND movhis.reversado <> 'S'
             GROUP BY movhis.num_credito, movhis.fecha_mov, movhis.nro_tarjeta;
            ...
            SELECT {+INDEX(bdicred:sd_movhis16122009 inx_movhis_16)}
                   MAX(secuencia)  
              INTO viNumSerial 
              FROM bdicred:sd_movhis16122009 
            ...
            SELECT {+INDEX(bdicred:sd_movhis16122009 inx_movhis4_16)}
                   sucursal 
              INTO vSucursal 
              FROM bdicred:sd_movhis16122009 
            ...
            --- IF NOT EXISTS ( SELECT {+INDEX(sl_exentos idx_exentos)} rfc FROM bdilide:sl_exentos WHERE num_cte = vcNumCte AND status = "1") THEN
                IF (vcStatusExento IS NULL) OR ( vcStatusExento = '0') THEN
                    ...
                    VALUES(vcAnioMes,vcNumCte,viNumSerial,vcRfc,'','C',vcNumCta,vdFechaMov,vmImpTotDep,vmImpIde,pCveUsuario, CURRENT ::DATE, vSucursal);
                END IF ;
            ELSE
                LET vcAnioMes = vcAnioMes;
                LET vcNumCte = vcNumCte;
                LET viNumSerial = viNumSerial;
                INSERT INTO bdilide:sl_movefec(aniomes,num_cte,num_serial,rfc,ref_ret,tipo_cta,num_cta,fecha_mov,imp_tot_dep,imp_ide,user_insert,fecha_insert, sucursal)
                VALUES(vcAnioMes,vcNumCte,viNumSerial,vcRfc,'','C',vcNumCta,vdFechaMov,vmImpTotDep,vmImpIde,pCveUsuario, CURRENT ::DATE, vSucursal);
            END IF;
