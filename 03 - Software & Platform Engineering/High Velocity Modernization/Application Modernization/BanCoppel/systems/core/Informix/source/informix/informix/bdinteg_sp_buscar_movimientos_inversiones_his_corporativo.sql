CREATE PROCEDURE "informix".sp_buscar_movimientos_inversiones_his_corporativo(p_sNumeroCuenta CHAR(30), p_sFechaInicial DATE, p_sFechaFinal DATE, p_numeroCliente CHAR(20), p_skip INT, p_sEmpresa CHAR(4))

     RETURNING	DATE AS fechaMovimiento, DATETIME HOUR to FRACTION(3) AS horaMovimiento , money(16,2) AS monto, 
                CHAR(30) AS folioSuc, CHAR(40) AS nombreSucursal, 
                CHAR(40) AS tipo, CHAR(1) AS reversado, CHAR(10) AS id, CHAR(20) AS cuenta,CHAR(1) AS naturaleza;
	...
	BEGIN
        ...
        ON EXCEPTION
                ...
                IF iSqlErr <> 0 THEN
                    ...
                    LET resultado_cuenta = '';
                    LET resultado_naturaleza = '';
                    RETURN resultado_fechaMovimiento, resultado_horaMovimiento, resultado_monto, resultado_folioSuc, resultado_nombreSucursal, resultado_tipo, resultado_reversado, resultado_id, resultado_cuenta,resultado_naturaleza;
                END IF;
        END EXCEPTION;

                IF (p_sNumeroCuenta IS NOT NULL AND p_sNumeroCuenta <> '') THEN
                    FOREACH       
                        SELECT SKIP p_skip DISTINCT fech_alt, fech_hor, monto_tot, folio_suc, bdinteg:si_sucursales.nombre, bdinteg:si_transacc.descripcion, cancelad, transacc, cuenta
                          ...
                          FROM bdinvers:sv_movhis
                            LEFT JOIN bdinteg:si_sucursales ON (bdinteg:si_sucursales.sucursal = bdinvers:sv_movhis.sucursal AND bdinteg:si_sucursales.empresa = p_sEmpresa) 
                            LEFT JOIN bdinteg:si_transacc ON (bdinteg:si_transacc.numero = bdinvers:sv_movhis.transacc AND bdinteg:si_transacc.empresa = p_sEmpresa)
                          WHERE fech_alt <= p_sFechaFinal 
                            AND fech_alt >= p_sFechaInicial 
                            AND cuenta = p_sNumeroCuenta
                            AND bdinvers:sv_movhis.empresa = p_sEmpresa
                ...
                ELSE
                    FOREACH       
                        SELECT SKIP p_skip DISTINCT fech_alt, fech_hor, monto_tot, folio_suc, bdinteg:si_sucursales.nombre, bdinteg:si_transacc.descripcion, cancelad, transacc, bdinvers:sv_maeinv.cuenta
                          INTO resultado_fechaMovimiento, resultado_horaMovimiento, resultado_monto, resultado_folioSuc, resultado_nombreSucursal, resultado_tipo, resultado_reversado, resultado_id, resultado_cuenta,resultado_naturaleza
                          FROM bdinvers:sv_movhis
                            LEFT JOIN bdinteg:si_sucursales ON (bdinteg:si_sucursales.sucursal = bdinvers:sv_movhis.sucursal AND bdinteg:si_sucursales.empresa = p_sEmpresa) 
                            LEFT JOIN bdinteg:si_transacc ON (bdinteg:si_transacc.numero = bdinvers:sv_movhis.transacc AND bdinteg:si_transacc.empresa = p_sEmpresa)
                            LEFT JOIN bdinvers:sv_maeinv ON (bdinvers:sv_movhis.cuenta = bdinvers:sv_maeinv.cuenta AND bdinvers:sv_maeinv.empresa = p_sEmpresa)
                          WHERE fech_alt <= p_sFechaFinal 
                            AND fech_alt >= p_sFechaInicial 
