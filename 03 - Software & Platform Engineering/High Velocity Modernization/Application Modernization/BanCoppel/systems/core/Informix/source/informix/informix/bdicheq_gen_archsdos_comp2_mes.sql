CREATE PROCEDURE "informix".gen_archsdos_comp2_mes()
RETURNING CHAR(5), CHAR(5), CHAR(50), INTEGER;
    ...
    ON EXCEPTION SET sql_err, isam_err, desc_err
        SET DEBUG FILE TO "/resplogifx/conciliachq/gen_archsdos_comp2_mes.err";
        TRACE ON;
        IF sql_err <> 0 THEN
            ...
            LET vsql = 'echo "UPDATE bdinteg:sx_contproc '||
                       ...
                       'AND sistema   = '''||vsistema||''';" > /tmp/horassdosmes2.sql';
            SYSTEM vsql;
            LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /tmp/horassdosmes2.sql';
            SYSTEM vstmt;
            RETURN vcodret1, vcodret2, vcodret3, vcontador1;
        END IF;
    END EXCEPTION;
    
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/gen_archsdos_comp2_mes.out";
    --- TRACE ON;
    ...
    IF vexiste = 0 THEN
        LET vsql = 'echo "INSERT INTO bdinteg:sx_contproc VALUES '||
                   ...
                   '(SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas), NULL, NULL);" > /tmp/horassdosmes2.sql';
        SYSTEM vsql;
