create procedure "informix".conprev1(pempresa char(3),
                         psucursal  char(4),
			 pinstrumento    char(4),
			 pcapital        money(14,2),
...
-- **************************************************************************
   ...
   define v_ofertartasa    char(60);
   define cod_tasaprom char(3);
   define v_id_prom integer;
...
-- **************************************************************************
   ...
   if pnum_autorizac != "0000000000000" and pnum_autorizac <> " " then
	 ...
	 let val = v_tasa + v_sobretasa;
      ...
      call sp_obtenertasa_prom(psucursal, pplazo, pcapital, pNumcte, pCtaInversion, pSecuencia)
     ...
     IF cod_tasaprom != '000' THEN
          -- // Realiza calculo de intereses de acuerdo a la tasa del plazo
           call "informix".calc_tasa(pempresa,v_tasa_instrum,ptp_persona,pcapital)
           returning v_codret,val;
     ELSE
		let v_codret = '000';
     END IF
