CREATE PROCEDURE "informix".sp_valida_sorteo_credito ( pFecha date )
RETURNING
	CHAR(5) AS COD_RET,
...
DEFINE iIndicador       SMALLINT;
DEFINE cCuentasVR       SMALLINT;
DEFINE cCuentasVen      SMALLINT;
...
--	SET DEBUG FILE TO "//informix/sp_valida_sorteo_credito.out";
--	TRACE ON;
	
    FOREACH WITH HOLD
        SELECT num_cliente
          INTO cCliente
          FROM bdinteg:si_sorteo_efectivo_temp
         WHERE fecha_info = pFecha
           and cred_valid = 99 --Clientes no evaluados --<> 2
         
             LET iIndicador = 1;
             LET cCuentasVR = 0;
             LET cCuentasVen = 0;
         -- Valida que el cliente no cuenta con creditos vendidos o reestructutados

             select count(*) 
               into cCuentasVR
               from bdicred:sd_maecred
              where numcte = cCliente
                and status_cred in ('CV','FC');

             if (cCuentasVR = 0) then
                 select count(*) 
                   into cCuentasVR     
             ...
             if (cCuentasVR > 0) THEN
                let iIndicador = 2;
             ELSE
         -- Valida que el cliente no cuenta con creditos vencidos
		 --IFRS Se contemplan los nuevos estatus de crÃ©dito por Etapas de Vencido
                select count(*) 
                   into cCuentasVen
                   from bdicred:sd_maecred a
				   inner join bdicred:sd_maesdos b ON (a.num_credito = b.num_credito)
                  where a.numcte = cCliente
				  and a.status_cred in ('BA','BT','E1','E2','E3') 
				  and (b.monto_vencido + b.mto_venc_trasp) > 0;
                    --and status_cred in ('BA','BT');

                 if (cCuentasVen = 0) then
                     select count(*) 
                       into cCuentasVen     
                       from bdicred:sd_maecredcrd a
					   inner join bdicred:sd_maesdoscrd b ON (a.num_credito = b.num_credito)
                      where a.numcte = cCliente
             ...
             begin work;
                 ...
                 UPDATE bdinteg:si_sorteo_efectivo_temp 
                    SET cred_valid = iIndicador
                  WHERE fecha_info = pFecha
                    AND num_cliente = cCliente;
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Valida creditos de clientes proximos a participar en el sorteo efectivo 2017', 
'AUTOR: Juan Olivares',
'FECHA: 27 de Septiembre del 2017',
'VERSION: 1.1',
'BD: bdicred';

CREATE PROCEDURE "informix".sp_validaciones_folioafore(pEmpresa CHAR(3),pNumCte CHAR(20),pTipoTarj CHAR(1),pNumTarj CHAR(16),pNumCuenta CHAR(20))
...
BEGIN
	...
	IF pTipoTarj = 'C' THEN
		...
		ELIF NVL(cStatus,'') NOT IN ('AA','BA','BT','E1','E2','E3') THEN
			LET iProcesa = 0;
			LET cCodRet = '000002'; --LA CUENTA DE CREDITO NO ESTA ACTIVA
	...
	ELIF pTipoTarj = 'D' THEN
		...
		ELIF NVL(cStatus,'') <> '1' THEN
			LET iProcesa = 0;
			LET cCodRet = '000002'; --LA CUENTA DE CAPTACION NO ESTA ACTIVA
...
'DESCRIPCIÃN: SP que valida que exista y sea correcta la CURP del cliente, ademas de validar que la cuenta de crÃ©dito o captaciÃ³n',
'sea la correcta. TambiÃ©n valida que la tarjeta no este vencida',
'FOLIO: 1629',
'FECHA: 28/07/2014',
