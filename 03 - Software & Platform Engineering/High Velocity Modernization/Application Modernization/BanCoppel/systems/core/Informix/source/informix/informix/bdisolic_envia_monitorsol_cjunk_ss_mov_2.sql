CREATE PROCEDURE  "informix".envia_monitorsol_cjunk_ss_mov_2(o_empresa CHAR(3), 
										  o_sucursal CHAR(20), 
...
-- COMENTARIOS: Se modifica para que muestre las solicitud de CrÃ©dito 
--              BanCoppel y las de CrÃ©dito Coppel
-------------------------------------------------------------------------------
-- FECHA: 2010/02/09
...
-- COMENTARIOS: Se modifica para realizar la homologacion del procedimiento con la versiÃ³n envia_monitorsol.
...
DEFINE dFechaEntradaInicio DATE;
DEFINE dFechaSalidaFinal DATE;
DEFINE cMINIMOCOMPARAR   CHAR(20);
-- ****************************************************************************
...
LET dFechaEntradaInicio = "";
LET dFechaSalidaFinal = "";
...
--    SET ISOLATION TO COMMITTED READ LAST COMMITTED;
   ...
   -- Carga la Fecha del Dia
    SELECT fecha_hoy
	 ...
	 --SELECT MIN (folio_movil) INTO cMINIMOCOMPARAR from "informix".ss_solicitudes_movil;
	 LET pFechaIni=NVL(pFechaIni,'');
	LET pFechaFin=NVL(pFechaFin,'');
	
	IF (pFechaIni = '') or (pFechaIni is null) THEN
	LET dFechaEntradaInicio=vfecha_hoy -10;
	ELSE
	LET dFechaEntradaInicio=pFechaIni;
	END IF;
	
	IF (pFechaFin = '') or (pFechaFin is null) THEN
	LET dFechaSalidaFinal=vfecha_hoy ;
	ELSE
	LET dFechaSalidaFinal=pFechaFin;
	END IF;
	IF o_Tipo = 1 THEN
		...
		FOREACH 
			...
			INNER JOIN  "informix".ss_solicitudes a ON ( a.num_solicitud = e.num_solicitud
														AND a.empresa = e.empresa
														AND a.status_solicitud IN ('AT','RT','CC','BC','OS','EE','OA','EA','CE','ST','LC','MC','EC','PA','IN'))
														
			INNER JOIN "informix".ss_autorizacion c on (c.empresa = a.empresa
                                                           ...
                                                           (SELECT MAX(fecha_entrada) FROM "informix".ss_autorizacion h
                                                            WHERE h.empresa           = c.empresa
									   	                    AND   h.num_solicitud     = c.num_solicitud
											                AND   h.status_solicitud  = c.status_solicitud)
                                                            AND   c.fecha_entrada >= dFechaEntradaInicio
															AND   c.fecha_entrada <= dFechaSalidaFinal )
			WHERE a.sucursal = o_sucursal
			ORDER BY a.num_solicitud
			
			SELECT b.apell_paterno, b.apell_materno, b.Nombre1, b.Nombre2, 
					b.razon_social, b.rfc
			INTO apellidopaterno, apellidomaterno, nombre1, nombre2, r_social, s_rfc
			FROM bdinteg:"informix".si_cliente b 
			WHERE b.numcte = s_numcte;
