CREATE PROCEDURE "informix".sd_datosriesgoscredprueba(pempresa CHAR(3),pfechahoy DATE,pfechaprox DATE)
RETURNING CHAR(5);
--------------------------------------------------------------
...
FOREACH
	...
	WHERE mae.status_cred="AA" AND mae.empresa=pempresa
	...
	IF pfechahoy IS NOT NULL AND vdtefechavenc IS NOT NULL THEN
		LET vintdiasmora = ( pfechahoy - vdtefechavenc ) / 30;
	ELSE
		LET vintdiasmora = 0;
	END IF;
	...
	DELETE FROM sd_riesgoscred WHERE empresa = pempresa AND numcte = vchrnumcte 
								AND numcredito = vchrnumcredito AND fecha = pfechahoy;
	...
	VALUES ( pempresa,vchrnumcte,vchrnumcredito,vchrproducto,vdtefechavenc,vdecmontootor,vdecsdolindisp,vdecsdodisp,
			...
			vchrhabitaen,vintanioshab,vintedad,vintdiasmora,pfechahoy,vdteprimermov,vdteultmov );
