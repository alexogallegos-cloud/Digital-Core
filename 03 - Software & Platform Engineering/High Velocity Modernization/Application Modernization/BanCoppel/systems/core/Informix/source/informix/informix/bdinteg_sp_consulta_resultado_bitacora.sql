CREATE PROCEDURE "informix".sp_consulta_resultado_bitacora(Pnumcte CHAR(20))
	RETURNING CHAR(5);
	
	DEFINE iSqlErr int;
	DEFINE CcodRet CHAR(5);
	DEFINE sResultado CHAR(20);
	DEFINE sCausa CHAR(50);
	DEFINE iBitIfe INTEGER;
...
BEGIN
	...
	-- buscar si existe el cte si_bitacora_ife
	...
	IF iBitIfe > 0 THEN
			...
			SELECT resultado, causa_rechazo 
			INTO sResultado, sCausa
			FROM bdinteg:"informix".si_bitacora_ife WHERE numcte = Pnumcte
			and fecha = (SELECT MAX(fecha) FROM bdinteg:"informix".si_bitacora_ife WHERE numcte =Pnumcte);
			
			
			IF (TRIM(sResultado)= 'False' OR TRIM(sResultado) = 'Falso') AND (TRIM(sCausa)='Menor cantidad de campos en OK') THEN
				
				LET CcodRet='00001'; --Validaciones IdBox falsas
...
"Se crea procedimiento para consultar el resultado de la bitacora ife",
"Folio: 460 050918",
"Bdinteg";

CREATE PROCEDURE "informix".sp_consulta_estatuscta(pTipo CHAR(1), pNumcte CHAR(20))
	...
	--SET DEBUG FILE TO '/home/sysifx/viridiana/sp_consulta_estatuscta.out';
	--TRACE ON;
...
BEGIN
	...
	IF pTipo= "1" THEN  --Consultar las cuentas de crÃ©dito
	
			
			SELECT COUNT(*) INTO iExiste FROM bdicred:"informix".sd_maecred WHERE numcte= pNumcte AND cod_caract_2 ='91';
			...
			ELSE
				...
				FOREACH
					...
					SELECT num_credito INTO cNuMCuenta FROM bdicred:"informix".sd_maecred WHERE numcte= pNumcte AND cod_caract_2 ='91'
					
					LET cCodRet = '00000';
					
					RETURN cCodRet, cNuMCuenta WITH RESUME;
				
				END FOREACH;
			END IF;
			
	ELIF  pTipo= "2"  THEN  --Consultar las cuentas de dÃ©bito
			
			SELECT COUNT(*) INTO iExiste FROM bdicheq:"informix".sc_maechq  WHERE num_cte=pNumcte AND motivo ='91';
			...
			ELSE
				...
				FOREACH
					...
					SELECT cuenta INTO cNuMCuenta FROM bdicheq:"informix".sc_maechq  WHERE num_cte=pNumcte AND motivo ='91'
...
DOCUMENT
"Selene Campos",
"Se crea procedimiento para consultar las cuentas bloqueadas del cliente con codigo 91",
"Folio: 588",
"Bdinteg";

CREATE PROCEDURE "informix".sp_bitacora_huella_ine (Pnumcte CHAR(20),PoCR_ife CHAR(20),Psucursal CHAR (4),Pejecutivo CHAR (8))
	...
	DEFINE CoCR_ife			CHAR(20);
	DEFINE Ccic				CHAR(20);
	DEFINE ILongOcr			CHAR(20);
	DEFINE Iexiste			INTEGER;
...
BEGIN
	...
	LET ILongOcr = LENGTH(PoCR_ife); 
	
	IF ILongOcr >= 13 THEN
		
		SELECT COUNT(numcte) INTO Iexiste FROM "informix".si_bitacora_huella_ine WHERE numcte = Pnumcte;
		
		IF  Iexiste > 0 THEN
				
				SELECT numcte,ocr_ife,sucursal,ejecutivo,fecha_insert,cic 
				INTO cnumcte,CoCR_ife,csucursal,cejecutivo,dfecha_insert,Ccic
				FROM  "informix".si_bitacora_huella_ine WHERE numcte = Pnumcte;
				
				INSERT INTO "informix".si_bitacora_huella_ine_hist(numcte,OCR_ife,sucursal,ejecutivo,fecha_insert,cic)
				VALUES(cnumcte,CoCR_ife,csucursal,cejecutivo,dfecha_insert,'' );
				
				
				UPDATE  "informix".si_bitacora_huella_ine SET ocr_ife=PoCR_ife,sucursal=Psucursal,ejecutivo=Pejecutivo,fecha_insert=CURRENT, cic = '' 
				WHERE  numcte = Pnumcte;
		ELSE
			INSERT INTO "informix".si_bitacora_huella_ine VALUES(Pnumcte,PoCR_ife,Psucursal,Pejecutivo,current,'');
	...
	ELSE
		...
		IF Iexiste > 0 THEN
				...
				INSERT INTO "informix".si_bitacora_huella_ine_hist(numcte,OCR_ife,sucursal,ejecutivo,fecha_insert,cic)
				VALUES(cnumcte,'',csucursal,cejecutivo,dfecha_insert,Ccic );
				
				
				UPDATE  "informix".si_bitacora_huella_ine SET ocr_ife='' ,sucursal=Psucursal,ejecutivo=Pejecutivo,fecha_insert=CURRENT, cic =PoCR_ife
				WHERE  numcte = Pnumcte;
		ELSE
			INSERT INTO "informix".si_bitacora_huella_ine VALUES(Pnumcte,'',Psucursal,Pejecutivo,current,PoCR_ife);
...
"FECHA : 01/11/2018",
"BD    : Bdinteg",
"Descripcion: Se crea procedimiento para clientes que ya fueron validados ante el ine y el resultado fue correcto ";

CREATE PROCEDURE "informix".sp_bitacora_cancelaciones(pCurp CHAR(20),pNombreproceso CHAR(50),pSucursal CHAR(4),pEjecutivo CHAR(8))

	RETURNING CHAR(5);
	
	DEFINE iSqlErr INTEGER;
	DEFINE cCodRet CHAR(5);
	DEFINE ILongCurp INTEGER;
...
--	TRACE ON;
	...
	SET LOCK MODE TO WAIT 3;
		...
		LET ILongCurp = LENGTH(pCurp); 
		
		IF ILongCurp >= 18 THEN
		
			INSERT INTO "informix".si_bitacora_cancelaciones(curp,nombreproceso,sucursal,ejecutivo,fecha_insert,numcte)
			VALUES(pCurp,pNombreproceso,pSucursal,pEjecutivo,current,"");
		
		ELSE
		
			INSERT INTO "informix".si_bitacora_cancelaciones(curp,nombreproceso,sucursal,ejecutivo,fecha_insert,numcte)
			VALUES("",pNombreproceso,pSucursal,pEjecutivo,current,pCurp);
...
"FECHA : 30/05/2018",
"BD    : Bdinteg",
"Descripcion: Se crea procedimiento encargado de guardar registro de los procesos cancelados en el alta del cliente";

CREATE PROCEDURE "informix".sp_bitacora_autorizaciones(Pnumcte CHAR(20),Pombreproceso CHAR(50),Psucursal CHAR(4),Pejecutivo CHAR(8),Pgerente CHAR(8))
...
--	TRACE ON;
	...
	SET LOCK MODE TO WAIT 3;
		...
		INSERT INTO "informix".si_bitacora_autorizaciones(numcte,nombreproceso,sucursal,ejecutivo,gerente,fecha_insert) 
			VALUES(Pnumcte,Pombreproceso,Psucursal,Pejecutivo,Pgerente,current);
...
'FECHA : 30/05/2018',
'BD    : Bdinteg',
'Descripcion: Se crea procedimiento  encargado de registrar las autorizaciones generadas por el gerente';

CREATE PROCEDURE "informix".sp_actualiza_estadoine(Pnumcte CHAR(20))
	RETURNING CHAR(5);
...
BEGIN
	...
	-- buscar si existe el cte si_consulta_ine_pendiente
	SELECT COUNT(numcte)
	INTO iInePend 
	FROM "informix".si_consulta_ine_pendiente 
	WHERE numcte  = Pnumcte;
	
	IF iInePend > 0 THEN
	
		DELETE FROM  "informix".si_consulta_ine_pendiente WHERE numcte = Pnumcte;
...
"Se crea procedimiento para actualizar el estatus de la consulta pendiente ine",
"Folio: 460 0509180",
"Bdinteg";

CREATE PROCEDURE "informix".sp_valida_persona_autorizadora (pNumEmp CHAR(20))
	RETURNING 	CHAR (5);			

	DEFINE cCodRet  CHAR (5);
	DEFINE iSqlErr	INTEGER;
	DEFINE iPerAut  INTEGER;

	LET cCodRet  	= '00000';
	LET iSqlErr     = 0;
	LET iPerAut 	= 0;

	--SET DEBUG FILE TO '/home/sysifx/viridiana/SP_VALIDA_PERSONA_AUTORIZADORA.out';
	--TRACE ON;		
BEGIN
		...
		LET pNumEmp = TRIM(pNumEmp);
		
		SELECT COUNT(num_empleado) 
		INTO iPerAut
		FROM "informix".si_personas_autorizadas 
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se crea procedimiento encargado de validar si el empleado tiene permisos para autorizar el alta del cliente' ,
'AUTOR:VIRIDIANA PAREDES ROMERO ',   
'FECHA DE CREACION: 14/06/2018',
'FOLIO: 420',
'BD: BDINTEG';

CREATE PROCEDURE "informix".sp_valida_huellaine_cte(pNumCte CHAR(20))
...
BEGIN
	...
	ELSE		
		...
		LET cAnio = YEAR(CURRENT);
		
		SELECT COUNT(ocr_ife) INTO iExiste 
...
'Fecha: 03/07/2018',
'ModificaciÃ³n: Se crea procedimiento almacenado para validar si el cliente ya esta validado ante el INE',
