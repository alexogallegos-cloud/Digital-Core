CREATE PROCEDURE "informix".sp_rcda_club_proteccion_pba()
...
BEGIN	
   ON EXCEPTION SET SQL_ERR, ISAM_ERR, ERROR_INFO
	  ...
	  select fecha_ant,'F',cod_ret, mensaje  from bdmis:mi_fechas where empresa = '001';
...
--calculo de acumulado
	...
	SELECT {+INDEX(mi_metasprod idx_mi_metasprod),INDEX(mi_sucursalesinfo idx_mi_sucursalesinfo)} 
       suc.num_sucursal, meta.producto, meta.metanum ,meta.aniomes
	...
	let vpaso = 8;	 
	INSERT INTO mi_rcda_apert_paso (fecha,sucursal,tpo_reg,ejecutivo,nombre,producto,ctescand,numero,meta)	 
	select apert.fecha ,mes.sucursal,2,mes.ejecutivo,mes.nombre,mes.producto, SUM(NVL(mes.clubncandidatos,0)), sum(nvl(mes.clubncompraron,0)),
