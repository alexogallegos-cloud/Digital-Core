CREATE PROCEDURE "informix".sp_consultacuentasdestino_statuscta_bpi(p_NumCte CHAR(20), p_CvePago CHAR(2), p_Registros SMALLINT)
RETURNING
     CHAR(6), ---cod_ret
	 CHAR(20), ---cuenta
	 CHAR(100), ---nombre
	 ...
	 CHAR(5); --Retorno del sp sc_cons_status_cta
...
--                    END IF;
					...
					IF(p_CvePago<>"03")THEN
						EXECUTE PROCEDURE bdicheq:"informix".sc_cons_status_cta('001',TRIM(v_CtaDestino)) INTO vCodRetStatus, vStatusCta, vProductoCta;
					END IF;
					
                    RETURN v_cod_ret, v_CtaDestino,v_Nombre,v_Banco,v_CompCel,v_NumCel,v_CorreoE,v_CveCuenta,v_DescCta,v_Rfc, v_MontoMaximo, NVL(vStatusCta,"")||NVL(vProductoCta,"")  WITH RESUME;
                END FOREACH;
            END IF;
        ELSE
...
END;
--##############################################################################
--## Procedimiento   : CLONACION DEL sp_ConsultaCuentasDestino
--## Creado por      : ING ALFONSO CRUZ
--## Fecha creacion  : 05/10/2012
--## Descripcion     : SE AGREGA PARÁMETRO DE SALIDA: RETORNO DEL SP sc_cons_status_cta
--##############################################################################
END PROCEDURE;