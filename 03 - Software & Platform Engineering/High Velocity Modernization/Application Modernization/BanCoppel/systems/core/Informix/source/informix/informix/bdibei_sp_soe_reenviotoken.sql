CREATE PROCEDURE "informix".sp_soe_reenviotoken(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumSolicitud CHAR(10))
		RETURNING CHAR(5) AS codret;
				
	DEFINE cCodRet 			CHAR(5);
	DEFINE iSqlErr 			INTEGER;
	DEFINE cNumCte 			CHAR(9);
	DEFINE cNsToken 		CHAR(9);	
	DEFINE cFolioSucursal   CHAR(16);
	DEFINE cSucursal        CHAR(5);	
	DEFINE dMonto			DECIMAL(12,2);	
	
	LET cCodRet = '00000';
	LET iSqlErr = 0;
	LET cNumCte = '';
	LET cNsToken = '';
	LET cFolioSucursal = '';
	LET cSucursal = '';	
	LET dMonto = 0.00;
	
	BEGIN
	
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_soe_reenviotoken.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pNumSolicitud = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF;

		-- VALIDACION DE ACCESO A LA FUNCIONALIDAD
		EXECUTE PROCEDURE bdinteg:"informix".sp_cnsif_confirmaejecutivo(pUsuario, pIdFuncion) INTO cCodRet;
		IF cCodRet <> '00000' THEN
			RETURN cCodRet;
		END IF; 
		IF EXISTS(SELECT solicitud FROM bdibei:"informix".bei_solicitudtoken WHERE solicitud = pNumSolicitud)THEN	
		
			SELECT numcte, folio_suc, sucursal INTO cNumCte, cFolioSucursal, cSucursal FROM bdibei:"informix".bei_solicitudtoken
			WHERE solicitud =  pNumSolicitud;
			
			UPDATE bdibei:"informix".bei_solicitudtoken SET id_status = '180' 
			WHERE solicitud = pNumSolicitud AND numcte = cNumCte;
			--SE INSERTA EL REGISTRO DEL CAMBIO DE ESTATUS DE LA SOLICITUD
			INSERT INTO bdibei:"informix".bei_stasolicitud(solicitud, anterior, actual, f_registro) 
				VALUES(pNumSolicitud, '170', '180', CURRENT); 
			
			FOREACH SELECT numcte, ns_token INTO cNumCte, cNsToken FROM bdibei:"informix".bei_tokensolicitud WHERE solicitud = pNumSolicitud
				DELETE bdibei:"informix".bei_tokensolicitud WHERE solicitud = pNumSolicitud AND numcte = cNumCte AND ns_token =  cNsToken;
			END FOREACH;
			
			IF cCodRet = '00000' THEN
			
				IF TRIM(SUBSTR(cFolioSucursal,1,8)) = 'SINCOMIS' THEN
					LET dMonto = 0.00;
				END IF				

				-- Inserta el registro del cobro del reenvio
			INSERT INTO bdibpi: "informix".tkn_solcobranza 
			(solicitud, Numcte, id_status, f_solicitud, folio_suc, f_cobro, cuenta, monto_tot, T_Persona)
			VALUES (pNumSolicitud, cNumCte, '180', CURRENT, cFolioSucursal, '', '', '', '');
			
			END IF
			
			RETURN cCodRet;
		ELSE
			LET cCodRet = '00089'; --EL NUMERO DE SOLICITUD NO EXISTE
			RETURN cCodRet;
		END IF;
	END;
END PROCEDURE
DOCUMENT 'AUTOR: Esparza Brenis Fernando MartÃ­n',
'FECHA: 30/10/2014',
'DESCRIPCION: Reenvia el Token SOE',
'BD:bdibei',
'AUTOR: 95419888 Elmer LÃ³pez Valenzuela',
'FECHA: 28/08/2015',
'DESCRIPCION: Se modifica para agregar un insert al final para el registro de la conciliaciÃ³n',
'BD:bdibei';

CREATE PROCEDURE "informix".sp_danado_paquete_bei( 
										pNumGuia char(30), 
										pNumSolicitud char(10),
										pUsuario char(8),
										pComentario char(200),
										pCanal char(2))
	RETURNING CHAR(5);
	
	--Elaboró: Moisés Soriano
	--Descripción: Procedimiento que cambia el estatus de los paquetes de PM a dañados.
	--Fecha: 19-10-2015
	--Solilcitó: José de Jesus Nevarez
	
	--DECLARACION DE VARIABLES

	DEFINE vsCodRet  		CHAR(5);
	DEFINE vNumCliente      CHAR(9);
	DEFINE vStatusAnt       CHAR(4);
	DEFINE vSqlErr          INTEGER;
	DEFINE vStatusAct       SMALLINT;
	DEFINE vStatusSolAnt    SMALLINT;
	DEFINE vNumSToken		CHAR(9);
	DEFINE vTotTokens		INT;
	DEFINE vTotAux			INT;
	
	--SET DEBUG FILE TO "/home/sysifx/moises/bdibei/sp_danado_paquete_bei.out";
	--TRACE ON;

	--Asignacion de variables
	LET vsCodRet = '00000';
	LET vSqlErr = 0;
	LET vNumCliente='';
	LET vStatusAct=0;
	LET vStatusSolAnt=0;
	LET vNumSToken = '';
	LET vTotTokens = 0;
	LET vTotAux = 0;
	
	BEGIN
		ON EXCEPTION SET vSqlErr
	      IF vSqlErr <> 0 THEN
	            LET vsCodRet = vSqlErr;
				RETURN vsCodRet;
	      END IF ;
		END EXCEPTION ;
		
		SET LOCK MODE TO WAIT 3;
		SET ISOLATION DIRTY READ ;
		
		SELECT 	numcte  
			INTO vNumCliente 
			FROM bdibei:"informix".bei_envios 
			WHERE solicitud=TRIM(pNumSolicitud)
			AND num_guia=TRIM(pNumGuia);
		
		SELECT COUNT(ns_token) 
			INTO vTotTokens 
			FROM bdibei:"informix".bei_tokensolicitud 
			WHERE solicitud=TRIM(pNumSolicitud);	
			
		
		IF(vNumCliente=='' OR vNumCliente IS NULL) THEN
			LET vsCodRet = '00100';		
		ELSE
			FOREACH	
				SELECT ns_token INTO 
				vNumSToken FROM bdibei:"informix".bei_tokensolicitud 
				WHERE solicitud=TRIM(pNumSolicitud)
				
				SELECT id_status INTO vStatusAnt FROM bdibpi:"informix".tkn_nseries WHERE ns_token=vNumSToken;
			
				IF (vStatusAnt=='' OR vStatusAnt IS NULL) THEN
					LET vsCodRet = '00200';				
				ELSE

					--Actualiza el estatus del token  a extraviado	
					UPDATE bdibpi:"informix".tkn_nseries SET id_status=176 WHERE ns_token=trim(vNumSToken);
					
					--Registra el cambio de estatus del token de estatus anterior a extraviado
					INSERT INTO bdibpi:"informix".tkn_status_token(ns_token,actual,anterior,f_cambio_status,usr_cambio_status,canal) 
																		VALUES(vNumSToken,176,vStatusAnt,CURRENT,pUsuario,pCanal);
					
					LET vTotAux = vTotAux + 1;
					IF(vTotTokens = vTotAux) THEN
							
						--Se obtiene el estatus anterior de las sol
						SELECT id_status 
						INTO vStatusSolAnt
						FROM bdibei:"informix".bei_solicitudtoken WHERE solicitud=pNumSolicitud AND numcte=vNumCliente;
						
						--Registra el cambio de estatus de la solicitud de estatus anterior a 100
						INSERT INTO bdibei:"informix".bei_stasolicitud VALUES(pNumSolicitud,vStatusSolAnt,100,CURRENT);
						
						--Actualiza el estatus de solicitud a 176(dañado) y graba comentarios
						UPDATE bdibei:"informix".bei_envios SET id_status=176,comentarios=pComentario 
						WHERE solicitud=pNumsolicitud 
						AND num_guia=pNumGuia;
						
						--Actualiza el estatus de solicitud a 100
							UPDATE bdibei:"informix".bei_solicitudtoken SET id_status=100 
							WHERE solicitud=TRIM(pNumSolicitud)
							AND	   numcte=TRIM(vNumCliente);
						
						--Elimina los registros correspondientes a los tokens asignados a la solicitud
						DELETE FROM bdibei:"informix".bei_tokensolicitud WHERE solicitud = TRIM(pNumSolicitud);
					
					END IF;
				END IF;
			END FOREACH;
		END IF;
		RETURN vsCodRet;	
	END
END PROCEDURE;