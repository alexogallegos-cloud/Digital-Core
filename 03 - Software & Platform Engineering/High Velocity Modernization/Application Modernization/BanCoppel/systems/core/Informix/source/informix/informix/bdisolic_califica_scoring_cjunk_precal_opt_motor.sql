CREATE PROCEDURE "informix".califica_scoring_cjunk_precal_opt_motor( o_empresa       CHAR(3),
													o_numsol   	 	CHAR(20),
	...
	-- Descripcion: Se aÃ±ade cambio al estatus de RT a CN y causas RCB/RGC a CCB/CGC.
...
--IPCB junio2017//RECHAZO POR CREDITO BLOQUEADO RCB 
	...
	-- ****************************************************************************
    --SET DEBUG FILE TO "/tmp/califica_scoring_cjunk_precal_opt.out";
	--TRACE ON;
...
--IPCB junio2017//RECHAZO POR CREDITO BLOQUEADO RCB 
	...
	--RQI 21 246  OriginaciÃ³n de solicitudes 24 x 7 INI
	...
	END IF;
	--RQI 21 246  OriginaciÃ³n de solicitudes 24 x 7 FIN
   ...
   END EXCEPTION WITH RESUME;*/
	   ...
	   --SET DEBUG FILE TO '/informix/Fperaza/NuevosCambios/canales/traces/califica_scoring_cjunk'||o_numsol||'.out';
	    ...
	    --SET DEBUG FILE TO '/ifxsif01/KSOV/Trace/califica_scoring_cjunk_precal_opt_motor'||o_numsol||'.out';
		--TRACE ON;
		...
		--IPCB 12Abr21 INI --Credito solicita inactivar validaciÃ³n de segundo producto para meter Requerimiento. Con esto no se prende la bandera																																		  
	...
	--ini jesus RM 09-279-2
		...
		--RQI 21 282-7 SE COMENTAN POR CAMBIO DE LOGICA MARCADO MIXTAS Y UNICAS, POR LAPSO DE TIEMPO -KSOV EL UPDATE SE SUBE AL IF DE LA VALIDACIÃN DE MARCAJE
      ...
      END IF;
        ...
        IF ((sNumPregSol != sNumGpos) OR (select count(*) from bdisolic:ss_detalle_scoring det where det.empresa = o_empresa and det.seccion = '2' and det.num_solicitud = o_numsol )  != sNumGpos )  THEN
				...
				IF NVL(cTicket,"") <> "" THEN   -- CON EL TICKET SE VALIDA QUE LA HUELLA NO CORRESPONDA A EMPLEADOS 
					...
					--RQM 10 1177 Se contemplan productos de prestamos y linea de credito diferentes de nomina
						...
						--IF sNum_producto NOT IN ( '6400','7800','8500') or (sNum_producto = '6400' and cEmpresa <> 0) THEN
                            ...
                            --Se aÃ±ade cambio al estatus de RT a CN y causas RCB/RGC a CCB/CGC.
...
-- Se toma el parametro PUNTUALIDAD PARA RECHAZAR CON EFP (EFICIENCIA FUERA DE POLÃTICAS
   SELECT valor
   INTO cPuntualidadZ
...
--	Se sustituye causa RDO por subcausas para reflejarse en el arbol de solicitudes RQM 04 469
		IF (v_puntualidad = cPuntualidadZ) THEN 
		   LET cCausa_sol = "EFP"; 
		   LET vMensaje='Eficiencia Fuera de PolÃ­ticas'; 	
					...
					IF  Flag_bitacora <> 1 THEN	
						...
						--JMAH Se realiza homologacion con ajustes paso 5
							...
							ELSE
								IF ( cOrigenSol = '1')  THEN
									--RQM 09 554
									SELECT numcte INTO cNumCte FROM bdisolic:"informix".ss_solicitudes WHERE num_solicitud = o_numsol;
									IF cCanal = 4 THEN
										IF cflujo_cc = '1' THEN --fjpr
											IF EXISTS (SELECT institucion FROM bdiburo:"informix".br_traslado WHERE numcte = v_cliente AND institucion = 'BC') THEN
												...
												IF EXISTS(SELECT numproducto from bdicred:sd_productos_motor WHERE numproducto=sNum_producto) THEN
													...
													VALUES  (o_empresa,o_numsol, cNumCte , 0, current, '');
												ELSE
