CREATE PROCEDURE "informix".sp_registra_evento_leo (
...
BEGIN
   ON EXCEPTION SET vsqlerr
      ...
      END IF;
   END EXCEPTION;
	SET DEBUG FILE TO "/tmp/sp_registra_evento3.out";
	TRACE ON;
...
--VALIDA QUE SI NO CONTIENE COMO PARAMETRO DE ENTRADA EL NUMERO DE CLIENTE, BÃ?Ã?Ã?Ã?Ã?Ã?Ã?ÃÂ¡SQUE POR TARJETA O CUENTA.
	...
	--SYNDEIN
		...
		pcorreo_alterno || "','"  || pcelular_alterno || "','" || pImporte1 || "','" || pImporte2 || "','" || pImporte3 || "','" || pImporte4 || "','" || pImporte5 || "'," || pfecha1Aux || "," || pfecha2Aux || ")";
