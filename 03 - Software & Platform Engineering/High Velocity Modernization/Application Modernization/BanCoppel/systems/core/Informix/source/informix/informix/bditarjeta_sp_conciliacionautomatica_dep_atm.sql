CREATE PROCEDURE "informix".sp_conciliacionautomatica_dep_atm(psCve_Usuario VARCHAR(10) , piHorario INTEGER)
RETURNING VARCHAR (5) AS CodRet, VARCHAR (150) AS Mensaje_Respuesta ;
	...
	BEGIN
		...
		ON EXCEPTION SET viSQLerr
			...
			UPDATE bditarjeta:"informix".td_param_conciliacion_dep_atm 
			SET Valor = 'F',
				Fecha_Modificacion = vdFechaDeHoy
			WHERE Codigo = '001' 
			AND Descripcion = 'CONCILIACION ATM DEPOSITADORES EN EJECUCION' 
			AND Valor = 'V';
			...
			EXECUTE PROCEDURE bditarjeta:"informix".sp_guardabitacora_dep_atm (viElemento, '(' || vsCodRet || ') ' || vsMensaje_Respuesta, psCve_usuario)
			INTO vsCodRet2;
		...
		--SET DEBUG FILE TO "/informix/MFOF/TRACE_CONAUTO_MC.sql";
		--TRACE ON;
		...
		(
			SELECT Descripcion 
			FROM bditarjeta:"informix".td_param_conciliacion_dep_atm 
            WHERE Codigo = '001' 
			AND Descripcion = 'CONCILIACION ATM DEPOSITADORES EN EJECUCION' 
			AND TRIM(Valor) = 'V'
		);
		...
		ELSE --OK
			...
			WHILE ( vsFlag_Ciclo_BusrcarArch = 'V' )  -- CICLO DE BUSQUEDA DE ARCHIVOS PENDIENTES.
				...
				FROM bditarjeta:"informix".td_archivos_conciliacion_dep_atm AS ArchCon 
				...
				IF (NVL(vsFlag_ArchPendiente, 'F') = 'V') THEN 
					-- EXISTEN ARCHIVOS PENDIENTE POR PROCESAR --IF(2)
					...
					ELIF ( (vsSistema IN ('A')) AND 
							...
							--VALIDA ESTATUS DEL PASE DE MOVIMIENTOS HITORICOS DE CHEQUES
					...
					*/
					...
					IF ( vsSistema = 'A' ) THEN
						...
						UPDATE bditarjeta:"informix".td_archivos_conciliacion_dep_atm
						...
						IF ( vsCarga <> 'V' ) THEN 
							-- VALIDA QUE LA CARGA DEL ARCHIVO NO FUE REALIZADA PREVIAMENTE -- IF (2.1.1)
							
							-- TRACE 'SI ENTRE :) 5';
										
							-- CARGA EL ARCHIVO A LA TABLA DE PASO
							EXECUTE PROCEDURE bditarjeta:"informix".sp_cargaarchivos_dep_atm ( vsRep_Aix, vsNombreArchivo, vsArchivo_Origen, viTipo_LayOut, vsSistema, vsRep_Aix )
						...
						IF ( vsCodRet = '00000' ) THEN 
							...
							IF ( vsCarga <> 'V' ) THEN 
								-- VALIDA QUE LA CARGA DEL ARCHIVO NO FUE REALIZADA PREVIAMENTE IF(2.1.2.A)
								
								-- TRACE 'SI ENTRE :) 8';
								
								-- CARGA LA INFORMACION SIGNIFICATIVA DE LOS REGISTROS A LA TABLA Td_Movimientos_Conciliacion_mc
								EXECUTE PROCEDURE bditarjeta:"informix".sp_obtenerregistroarchivo_dep_atm (vsNombreArchivo, vsArchivo_Origen, viTipo_LayOut, psCve_Usuario )
							...
							IF ( vsConadmin = '' ) THEN 
								...
								-- TRACE 'SI ENTRE :) 8';
								-- CARGA LA INFORMACION SIGNIFICATIVA DE LOS REGISTROS A LA TABLA Td_Movimientos_Conciliacion_mc
								-- El SP fue modificado de tal manera que solo modificarÃ¡ el el status, ya que no se requiere hacer el proceso de conciliaciÃ³n
								-- puesto que el archivo que genera el proceso no se requiere, y por ende el procesamiento que lo genera tampoco, solo la carga de la informaciÃ³n
								EXECUTE PROCEDURE bditarjeta:"informix".sp_concreing_dep_atm (psCve_Usuario, piHorario)
								INTO vsCodRet, vsMensaje_Respuesta;
								
								-- TRACE 'SI ENTRE :) 8.1'|| vsCodRet;
								-- ACTUALIZA LA HORA DE FIN DE LA CARGAR DE LA TABLA Td_Movimientos_Conciliacion_mc
								/*
								LET vsTotaltxn = 0;
								LET vsTotalMonto = 0.0;
																
								SELECT COUNT(*),SUM(monto_deposito) 
								INTO vsTotaltxn, vsTotalMonto
								FROM bdiTarjeta:dep_atm_stat06_vs_movhis
								WHERE NombreArchivo = vsNombreArchivo;

								LET vsTotalMonto = NVL(vsTotalMonto,0.0);
								*/
				...
				IF (vsCodRet <> '00000') THEN
					...
					EXECUTE PROCEDURE bditarjeta:"informix".sp_guardabitacora_dep_atm (viElemento, '(' || vsCodRet || ') ' || vsMensaje_Respuesta, psCve_usuario) INTO vsCodRet2;
				...
				IF ( EXISTS 
					...
					LET vsMensaje_Respuesta = 'PARO DE EMERGENCIA DE CNC ATM DEPOSITADORES.';

					-- LIBERA LA BANDERA DE PARO DE EMERGENCIA DE CONCILIACION
					UPDATE bditarjeta:"informix".td_param_conciliacion_dep_atm
					SET Valor = 'F', Fecha_Modificacion = vdDiaHora
					WHERE Codigo = '002' 
					AND Descripcion = 'PARO DE EMERGENCIA DE CNC ATM DEPOSITADORES'
					AND TRIM(Valor) = 'V';
			...
			IF (EXISTS 
				(
					...
					FROM bditarjeta:"informix".td_archivos_conciliacion_dep_atm 
					WHERE Proceso = 'X'
				)
			) THEN
