CREATE PROCEDURE "informix".sp_mitigacioncte()
RETURNING CHAR(6);
...
DEFINE cFechaLibro			CHAR(20);
DEFINE cFechaCredCorriente	CHAR(20);
DEFINE iExiste				INTEGER;
DEFINE cScoreClean 			CHAR(4);
DEFINE cScoreDirty			CHAR(4);
...
BEGIN
	...
	--SET DEBUG FILE TO "/home/sysifx/Oscar/736/sp_mitigacioncte.out";
	--TRACE ON;
	...
	--Obtener las fechas
	SELECT fecha_hoy, ADD_MONTHS(fecha_hoy,-12), ADD_MONTHS(fecha_hoy,-3)
	INTO cFechaHoy, cFechaLibro, cFechaCredCorriente
	FROM bdicred:"informix".sd_fechas 
	WHERE empresa = cEmpresa;
		
	FOREACH WITH HOLD
	--Inicia primera validaciÃ³n
	-- Obtiene los creditos e informaciÃ³n de clientes para producto Tarjeta de CrÃ©dito Bancoppel Visa (6001) con limite de credito entre 15,000 y 50,000
        
		SELECT cte.numcte,mc.num_credito,cte.apell_paterno,cte.apell_materno,cte.nombre1,cte.nombre2
		INTO cNumCte, cNum_Credito, cAPaterno, cAMaterno, cNombre1, cNombre2
		FROM bdicred:"informix".sd_maecred mc, bdinteg:"informix".si_cliente cte, 
			bdicred:"informix".sd_tarjeta tarj 
		WHERE mc.numcte = cte.numcte
		AND cte.numcte = tarj.numcte
		AND cte.empresa = cEmpresa
		AND mc.num_producto = '6001'
		AND mc.status_cred = 'E1'
		AND tarj.limite_aut BETWEEN 15000 AND 50000
		GROUP BY cte.numcte,mc.num_credito,cte.apell_paterno,cte.apell_materno,cte.nombre1,cte.nombre2
		
		IF DBINFO("sqlca.sqlerrd2") = 0 THEN
			LET cCodRet = '000001'; --No se encotraron datos
            RETURN cCodRet;
		ELSE
			--Valida el Behavior Score de 490 puntos Clean y 448 Dirty
			SELECT db.score, cb.score	
			INTO cScoreDirty, cScoreClean
			FROM bdicred:"informix".sd_clientes_dirty_behavior db, bdicred:"informix".sd_clientes_clean_behavior cb
			WHERE db.num_credito = cNum_Credito
			AND cb.num_credito = db.num_credito;
			
			IF cScoreClean >= '490' AND cScoreDirty >= '448' THEN
				--MÃ¡xima morosidad en los Ãºltimos 12 meses de 1 mes vencido
				SELECT COUNT(*)
				INTO iExiste
				...
				AND capital_fecha_pago >= cFechaLibro;
				
				IF iExiste <= 1 THEN
					--CrÃ©ditos al corriente en los Ãºltimos 3 meses
					LET iExiste = 0;
					SELECT COUNT(*)
					INTO iExiste
					FROM bdicred:"informix".sd_amortiza_credito
					...
					and num_credito = cNum_Credito
					and capital_status in (2,7)
					AND capital_fecha_pago >= cFechaCredCorriente;
					
					IF iExiste = 0 THEN
						LET iExiste = 0;
						--Consulta que tenga movimientos en los ultimos 12 meses de la fecha actual
						SELECT COUNT(*) INTO iExiste FROM (
						SELECT num_credito
						FROM bdicred:"informix".sd_movdia 
						WHERE empresa = cEmpresa 
						AND num_credito = cNum_Credito
						AND fecha_mov <= cFechaLibro 
						UNION 
						SELECT num_credito
						...
						UNION
						SELECT num_credito
						FROM bdicred:"informix".sd_movhis_old 
						WHERE empresa = cEmpresa
						AND num_credito = cNum_Credito		
						AND fecha_mov <= cFechaLibro );
						
						IF iExiste > 0 THEN
							--Inicia segunda validaciÃ³n
							--Verifica que no tenga pagos fijos vijentes
							LET iExiste = 0;
							...
							IF iExiste = 0 THEN	
								...
								IF iExiste = 0 THEN
									INSERT INTO "informix".sd_ctesamigrar (empresa, numcte, num_credito, apell_paterno,	apell_materno, nombre1, nombre2, sucursal, nomsuc, domsuc, flagstatussol, status, fecha_insert)
									VALUES (cEmpresa, cNumCte, cNum_Credito, cAPaterno, cAMaterno, cNombre1, cNombre2, '', '', '', 0, '', cFechaHoy);	
									
									IF DBINFO("sqlca.sqlerrd2") = 0 THEN
										LET cCodRet = '000002'; --No se insertaron datos
										RETURN cCodRet;									
									END IF;
								END IF;									
							END IF;
...
END PROCEDURE
DOCUMENT
'Se crea SP para insercion de clientes con Tarjeta de CrÃ©dito Bancoppel Visa que cumplan con los criterios', 
'de riesgos y operativos para ser considerados en la migraciÃ³n de Tarjeta de CrÃ©dito Bancoppel Visa',
'a Tarjeta de CrÃ©dito Bancoppel Oro.',
