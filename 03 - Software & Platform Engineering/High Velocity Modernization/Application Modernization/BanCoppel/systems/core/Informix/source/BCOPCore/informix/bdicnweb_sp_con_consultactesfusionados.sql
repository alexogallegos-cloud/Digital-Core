CREATE PROCEDURE "informix".sp_con_consultactesfusionados(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechIni DATE, pFechFin DATE , pRegistros INTEGER, pRecuperacion INTEGER)
		...
		BEGIN
				...
				FOREACH
						EXECUTE PROCEDURE bdinteg:"informix".sp_ctes_fusionados(pFechIni, pFechFin)
						INTO cCodRetSp, cFecha, iFusionados, iNoFusionados, iTotal
		
						LET iCodRetSp = cCodRetSp::INTEGER;
						IF iCodRetSp < 0 THEN
						RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bdinteg:sp_ctes_fusionados';		
						END IF;
			
						LET iTotalFusionados = iTotalFusionados + iFusionados;
						LET iTotalNoFusionados = iTotalNoFusionados + iNoFusionados;
						LET iResultadoTotal = iResultadoTotal + iTotal;	
				END FOREACH
				--OBTIENE DETALLES DE LA CONSULTA
				FOREACH
						EXECUTE PROCEDURE bdinteg:"informix".sp_ctes_fusionados2(pFechIni, pFechFin, pRegistros, pRecuperacion)
						INTO cCodRetSp, cFecha, iFusionados, iNoFusionados, iTotal
		
						LET iCodRetSp = cCodRetSp::INTEGER;
						IF iCodRetSp < 0 THEN
						RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bdinteg:sp_ctes_fusionados2';		
						END IF;									
		
						LET iNoRegistros = iNoRegistros + 1;
		
						RETURN cCodRet, cFecha, iFusionados, iNoFusionados, iTotal, iTotalFusionados, iTotalNoFusionados, iResultadoTotal WITH RESUME;
...
'DESCRIPCION: SPL que consulta clientes fusionados',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_con_consultaltas_idbox(pUsuario CHAR(8), pIdFuncion CHAR(10), pFecIni DATE, pFecFin DATE, pRegistros INTEGER, pRecuperacion INTEGER)	
RETURNING 		CHAR(5) AS codret,
				CHAR(5) AS sucursal, 
				INTEGER AS altas_total,
				INTEGER AS total_idb,
				INTEGER AS resultado_altas_total,
				INTEGER AS resultado_total_idb;	
		...
		DEFINE iNoRegistros INTEGER;
		DEFINE iResultadoAltasTotal INTEGER;
		DEFINE iResultadoTotIdb 	INTEGER;
		DEFINE dFechaActual DATE;
	        DEFINE iAnioActual INTEGER;
	        DEFINE iAnioIngreso INTEGER;
		...
		LET iTotIdb = 0;
		LET iNoRegistros = 0;
		LET iResultadoAltasTotal = 0;
		LET iResultadoTotIdb = 0;
		
	BEGIN	
			ON EXCEPTION SET iSqlErr
					LET cCodRet = iSqlErr;
					RETURN cCodRet, cSucursal, iAltasTotal, iTotIdb,iResultadoAltasTotal,iResultadoTotIdb;
			END EXCEPTION;	
	
-- SET DEBUG FILE TO '/tmp/ALAN/RQI_IDbox/basededatos/bdicnweb/sp/sp_con_consultaltas_idbox.out';
--TRACE ON;
			SET LOCK MODE TO WAIT 3;
			SET ISOLATION TO DIRTY READ;
...
------------  
			...
			LET iResultadoAltasTotal =  iAltasTotal;
			LET iResultadoTotIdb =  iTotIdb;			
			...
			FOREACH 
					...
					RETURN cCodRet, cSucursal, iAltasTotal, iTotIdb,iResultadoAltasTotal,iResultadoTotIdb WITH RESUME;	
			END FOREACH;
...
'MODULO: Consultas ',
'FUNCIONALIDAD: Reporte Procesos Sucursal',
'DESCRIPCION: SP que consulta altas idbox de Reporte Procesos Sucursal',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_con_consultausokiosko(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechaInicio DATE, pFechaFin DATE, pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
		CHAR(4)     AS sucursal,	
		INTEGER     AS saldos,
		INTEGER     AS mov,
		INTEGER     AS estadoscta,
		INTEGER     AS total_saldos, 
		INTEGER     AS total_mov,
        INTEGER     AS total_estadoscta;
        
		
	DEFINE cCodRet 			    CHAR(5);
	DEFINE iSqlErr 			    INTEGER;
	DEFINE cSucursal 		    CHAR(4);
	DEFINE dConsSaldos 		    INTEGER;
	DEFINE dConsMov 		    INTEGER;
	DEFINE dConsEstadoCta 	    INTEGER;
	DEFINE iNoRegistros 	    INTEGER;
	DEFINE iTotalSaldos		    INTEGER;
    DEFINE iTotalMovimiento	    INTEGER;
    DEFINE iTotalEstadoCuenta	INTEGER;
	...
	LET dConsSaldos 			= 0;
	LET dConsMov 	 			= 0;
	LET dConsEstadoCta 	 		= 0;
	LET iNoRegistros	 		= 0;
	LET iTotalSaldos 			= 0;
	LET iTotalMovimiento 		= 0;
	LET iTotalEstadoCuenta 		= 0;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cSucursal, dConsSaldos, dConsMov, dConsEstadoCta, iTotalSaldos, iTotalMovimiento, iTotalEstadoCuenta;
		END EXCEPTION;
				
		--SET DEBUG FILE TO "/informix/masv/sp_con_consultausokiosko.out";
		--TRACE ON;
		SET LOCK MODE TO WAIT 3;
		SET ISOLATION TO DIRTY READ;
		...
		----- VALIDACION DE QUE ESTA USANDO EL ANIO EN CURSO(NO SE PUEDE USAR OTRO ANIO)
			...
			IF (dFechaActual - pFechaInicio) > 366 THEN 
			LET cCodRet= '00996';
			RETURN cCodRet, cSucursal, dConsSaldos, dConsMov, dConsEstadoCta, iTotalSaldos, iTotalMovimiento, iTotalEstadoCuenta;
			END IF;
...
------------  
		IF pFechaInicio = pFechaFin THEN 
			...
			SELECT SUM(cons_saldos), SUM(cons_movimientos), SUM(cons_edocta)
			INTO iTotalSaldos, iTotalMovimiento, iTotalEstadoCuenta
			FROM bdinteg:"informix".si_indicadores_kiosko				
			WHERE fecha_proceso = pFechaInicio;			
		
			FOREACH 
				SELECT SKIP pRegistros FIRST pRecuperacion sucursal, SUM(cons_saldos), SUM(cons_movimientos), SUM(cons_edocta)
				INTO cSucursal, dConsSaldos, dConsMov, dConsEstadoCta
				FROM bdinteg:"informix".si_indicadores_kiosko
				WHERE fecha_proceso = pFechaInicio
				GROUP BY 1
				ORDER BY 1
				...
				LET iNoRegistros = iNoRegistros + 1;
				RETURN cCodRet, cSucursal, dConsSaldos, dConsMov, dConsEstadoCta, iTotalSaldos, iTotalMovimiento,iTotalEstadoCuenta WITH RESUME;
...
'MODULO: CONSULTAS ',
'FUNCIONALIDAD: REPORTE PROCESOS SUCURSAL',
'DESCRIPCION:SPL que consulta el uso del kiosco en el Reporte Procesos Sucursal.',
'BD: bdicnweb',
'AUTOR: Guadalupe Angelica Hernandez Perez',
'FECHA: 22/08/2016',
'DESCRIPCION:Realizo una modificaciÃ³n al orden de las consultas entre saldos y movimientos.',
'BD: bdicnweb';
...
CREATE PROCEDURE "informix".sp_con_consultaverificasms_totales(pUsuario CHAR(8), pIdFuncion CHAR(10), pFechIni DATE, pFechFin DATE)
		...
		BEGIN	
				...
				LET iCodRetSp = cCodRetSp::INTEGER;
				IF iCodRetSp < 0 THEN
				RAISE EXCEPTION iCodRetSp, 0, 'ERROR EN LA EJECUCION DEL SP bdinteg:sp_verifica_sms2_totales';		
				END IF;
...
'MODULO: CONSULTAS',
'FUNCIONALIDAD: Reporte Procesos Sucursal',
'DESCRIPCION: SPL que consulta totales de la verificaciÃ?Â?Ã?Â³n de sms del Reporte Procesos Sucursal',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_consultametascump(pUsuario CHAR(8), pIdFuncion CHAR(10), pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
				INTEGER AS parametro,
				CHAR(60) AS descripcion,
				DECIMAL(16,2) AS valor;
	...
	DEFINE iNoRegistros INTEGER;
	DEFINE iParametro INTEGER;
	DEFINE dValor DECIMAL(16,2);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, iParametro, cDescripcion, dValor;
		END EXCEPTION;
		
		-- SET DEBUG FILE TO '/tmp/mfinis/sp_ope_consultametascump.out';
		-- TRACE ON;
		...
		SET ISOLATION TO DIRTY READ;
		FOREACH		
			SELECT SKIP pRegistros FIRST pRecuperacion parametro, descripcion, valor
			INTO iParametro, cDescripcion, dValor
			FROM bdmis:"informix".mi_paramcump
			ORDER BY 1
			
			
			LET iNoRegistros = iNoRegistros + 1;
			RETURN cCodRet, iParametro, cDescripcion, dValor WITH RESUME;
		END FOREACH;
...
'FECHA: 18/05/2017',
'MODULO: Operaciones',
'FUNCIONALIDAD: Mantenimiento Metas Cumplimiento',
'DESCRIPCION: Procedimiento que obtiene la informacion para el llenado del grid de la funcionalidad',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_consultametascump_totales(pUsuario CHAR(8), pIdFuncion CHAR(10))
		RETURNING CHAR(5) AS codret,
	...
	BEGIN
		...
		-- SET DEBUG FILE TO '/tmp/mfinis/sp_ope_consultametascump_totales.out';
		-- TRACE ON;
...
'MODULO: Operaciones',
'FUNCIONALIDAD: Mantenimiento Metas Cumplimiento',
'DESCRIPCION: Procedimiento que obtiene la informacion para el total de registros a recuperar',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_ope_consultametasproductos(pUsuario CHAR(8), pIdFuncion CHAR(10), pAnioMes CHAR(10), pTipoSucursal INTEGER, pRegistros INTEGER, pRecuperacion INTEGER)
		RETURNING CHAR(5) AS codret,
				CHAR(4) AS producto,
				CHAR(40) AS descripcion,
				DECIMAL(16,2) AS meta;
	...
	DEFINE cDesProducto CHAR(40);
	DEFINE mMeta DECIMAL(16,2);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cProducto, cDesProducto, mMeta;
		END EXCEPTION;
		
		-- SET DEBUG FILE TO '/tmp/mfinis/sp_ope_consultametasproductos.out';
		-- TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pAnioMes = '' OR pTipoSucursal IS NULL OR pRegistros IS NULL OR pRecuperacion IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet, cProducto, cDesProducto, mMeta;
		END IF;
		...
		FOREACH		
				SELECT SKIP pRegistros FIRST pRecuperacion a.producto, b.nombre, a.metanum 
				INTO cProducto, cDesProducto, mMeta
				...
				LET iNoRegistros = iNoRegistros + 1;
				RETURN cCodRet, cProducto, cDesProducto, mMeta WITH RESUME;
...
'DESCRIPCION: Procedimiento que obtiene la informacion para el llenado del grid metas producto',
'BD: bdicnweb';
