CREATE PROCEDURE "informix".sp_asigsegabccred(P_ACCION SMALLINT, P_NUM_PROD CHAR(3), P_NUM_PROD_NEW CHAR(3), P_TIPO_CTA CHAR(20),
												P_NOM_CTA CHAR(20), P_MTOMIN DECIMAL(18,2), P_MTOMAX DECIMAL(18,2))														
                                                
RETURNING	CHAR(6)        AS Cod_ret,			
			CHAR(80)       AS Mensaje,		
			CHAR(3)        AS Num_prodcto,		
			CHAR(20)       AS Tipo_cta,		
			CHAR(20)       AS Nom_cta,		
			DECIMAL(18,2)  AS Mto_min,	
			DECIMAL(18,2)  AS Mto_max;			
...
BEGIN
	...
	--SET DEBUG FILE TO "/tmp/sp_AsigSegProdctoTDC.out";
	--TRACE ON;
	...
	IF NVL(P_ACCION, 0) = 0 THEN
		LET cCod_ret = "000001";
		LET cMensaje = "Parametro P_ACCION nulo o en cero.";
		RETURN	NVL(TRIM(cCod_ret), ''),
				NVL(TRIM(cMensaje), ''),
				NVL(TRIM(cNum_prodcto), ''),
				NVL(TRIM(cTipo_cta), ''),
	...
	-- VALIDANDO SI LA ORDEN DE ACCION ESTA CORRECTA
	IF 	P_ACCION <= 4 THEN
		IF 	P_ACCION = 1 THEN --consultas
		
			FOREACH
			
			  SELECT	TRIM(num_prod), TRIM(tipo_cta), TRIM(nombre), NVL(limite_min,0), NVL(limite_max,0)
				INTO	cNum_prodcto, cTipo_cta, cNom_cta, mMto_min, mMto_max  
				FROM	"informix".sd_segmentos
			   WHERE	empresa = cEmpresa
			ORDER BY	limite_min ASC
				...
				RETURN	NVL(TRIM(cCod_ret), ''),
						...
						NVL(TRIM(cNom_cta), ''),
						NVL(mMto_min, 0.0),
						NVL(mMto_max, 0.0) WITH RESUME;			  
			END FOREACH;
			--VALIDAR EL RETORNO DE DATOS
		...
		ELIF 	P_ACCION = 2 THEN --modificaciones	
				IF ( TRIM(NVL(P_NUM_PROD,'')) = "") OR ( TRIM(NVL(P_TIPO_CTA,'')) = "") OR ( TRIM(NVL(P_NOM_CTA,'')) = "")
					OR ( NVL(P_MTOMIN,-1) <= -1) OR ( NVL(P_MTOMAX,-1) <= -1) OR ( TRIM(NVL(P_NUM_PROD_NEW,'')) = "") THEN					
					LET cCod_ret = "000003";
					LET cMensaje = "Hay parametros en blanco.";						
				ELSE
					IF 	( P_MTOMIN < P_MTOMAX )  THEN					
						IF 	P_NUM_PROD <> P_NUM_PROD_NEW  THEN
							--VALIDAR DISPONIBILIDAD P_NUM_PROD_NEW
							SELECT TRIM(num_prod)
							  INTO cNum_prodcto
							  FROM "informix".sd_segmentos
							 WHERE num_prod= TRIM(P_NUM_PROD_NEW)
							   AND empresa = cEmpresa;
							 
								IF TRIM(NVL(cNum_prodcto,'')) = ''  THEN
									--P_NUM_PROD_NEW ESTA DISPONIBLE
									UPDATE	"informix".sd_segmentos
									...
									IF ( dbinfo('sqlca.sqlerrd2') = 0 ) THEN
										LET cCod_ret = '000004';
										LET cMensaje = "No fue posible modificar los datos.";					
									END IF;
								ELSE
									LET cCod_ret = '000005';
									LET cMensaje = "Producto no disponible.";
								END IF
						ELSE
							--P_NUM_PROD SIN CAMBIOS
							UPDATE	"informix".sd_segmentos
					...
					ELSE
						LET cCod_ret = '000007';
						LET cMensaje = "El monto mínimo debe ser menor al monto máximo.";
					END IF;								
				END IF;
		
		ELIF 	P_ACCION = 3 THEN --es insertion				
				IF ( TRIM(NVL(P_NUM_PROD,'')) = "") OR ( TRIM(NVL(P_TIPO_CTA,'')) = "") OR ( TRIM(NVL(P_NOM_CTA,'')) = "")
					OR ( NVL(P_MTOMIN,-1) <= -1) OR ( NVL(P_MTOMAX,-1) <= -1) THEN
					
					LET cCod_ret = "000008";
					LET cMensaje = "Hay parametros incorrectos.";						
				ELSE
					...
					IF 	TRIM(NVL(cNom_cta,'')) = ''  THEN
						IF 	( P_MTOMIN < P_MTOMAX )  THEN
							INSERT INTO	"informix".sd_segmentos
										(empresa,
										num_prod,
										tipo_cta,
										nombre,
								 ...
								 VALUES ( cEmpresa,
										...
										IF ( dbinfo('sqlca.sqlerrd2') = 0 ) THEN
											LET cCod_ret = '000009';
											LET cMensaje = "Problemas al guardar los datos.";					
										END IF;
						ELSE
							LET cCod_ret = '000010';
							LET cMensaje = "El monto mínimo debe ser menor al monto máximo.";
					...
					END IF;			
				END IF;					
		ELIF 	P_ACCION = 4 THEN --es un borrado
				IF  TRIM(NVL(P_NUM_PROD,'')) = ""  THEN				
					LET cCod_ret = "000012";
					LET cMensaje = "Parametro NUM_PROD en blanco";					
				ELSE
				
					DELETE	 FROM	"informix".sd_segmentos
							WHERE	num_prod= TRIM(P_NUM_PROD)
							  AND	empresa = cEmpresa;

					IF ( dbinfo('sqlca.sqlerrd2') = 0 ) THEN
							LET cCod_ret = '000013';							LET cMensaje = "No fue posible borrar datos";					
					END IF;					
				END IF;					
		END IF;
	ELSE	
...
END PROCEDURE
DOCUMENT
'DESCRIPCIÓN: PROCEDURE QUE DARA MANTENIMIENTO A LOS SEGMENTOS DE CREDITO',
'FECHA DE CREACION: 06-12-2012',
'BASE DE DATOS: BDICRED',
'AUTOR: RIGOBERTO GONZALEZ',
'VERSION: 20121206.1359';
