CREATE PROCEDURE "informix".sp_estadisticas_sac_correos_minuto
( fechaInicio date, fechaFin date )

RETURNING
    CHAR(5) AS codigo_respuesta,
	CHAR(80) AS mensaje_respuesta;
	
	
	DEFINE iSqlErr INT;
    DEFINE iIsamErr INT;
    DEFINE cInfoErr CHAR;
	
	DEFINE vFecha_minuto char(10);
    DEFINE vMaximo_minuto int;
    DEFINE vHora_maximo char(5);
	
	DEFINE cCodRet CHAR(5);
	DEFINE cMensaje CHAR(80);
	DEFINE usuario CHAR(9);
    DEFINE Vsp CHAR(100);
    DEFINE Vid_sp INTEGER;
    DEFINE Vperiodo CHAR(25);
	DEFINE vFechaInsert DATETIME YEAR to FRACTION(3);
	DEFINE vfechaInicio CHAR(23);
	DEFINE vfechaFin CHAR(23);
	
	LET cCodRet = "00000";
	LET cMensaje = 'PROCESO EXITOSO';
    LET usuario = 'informix';
    LET Vsp = 'sp_estadisticas_sac_correos_minuto';
    LET Vid_sp = '3';
    LET Vperiodo = cast((fechaInicio || ' a ' || fechaFin) as char(25));
	LET vFechaInsert = current;
	LET vfechaInicio = cast(year(fechaInicio) as char(4)) || '-' || TO_CHAR(fechaInicio, '%m') || '-' || TO_CHAR(fechaInicio, '%d') || ' 09:59:59.000';
	LET vfechaFin = cast(year(fechaFin) as char(4)) || '-' || TO_CHAR(fechaFin, '%m') || '-' || TO_CHAR(fechaFin, '%d') || ' 20:00:59.000';

	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;

	--SET DEBUG FILE TO '/informix/tmp/sp_estadisticas_sac_correos_minuto.out';
	--TRACE ON;

	BEGIN
		ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
			IF iSqlErr <> 0 THEN
                LET cCodRet = iSqlErr;
				LET cMensaje = "ERROR";
				INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022
				RETURN cCodRet, cMensaje;
			END IF;
		END EXCEPTION;
    
        DROP TABLE IF EXISTS tmp_correos;
		DROP TABLE IF EXISTS TB_MAXIMO_INDIVIDUAL;
		
		--SE TRAEN TODOS LOS REGISTROS DEL PERIODO
		
			select {+INDEX (bdinteg:"informix".si_correos.idx_si_correos8)} 
				fecha_hora, valida_correo, fecha_valida
			from bdinteg:"informix".si_correos 
			where fecha_hora > vfechaInicio 
			  and fecha_hora < vfechaFin 
			INTO TEMP tmp_correos with no log;

			
		--SE AGRUPAN POR TIPO DE CODIGO Y FECHA, SACANDO EL PROMEDIO DE VALIDACIONES POR MINUTO	
		
			insert into "informix".sac_estadisticas_correos_minuto (fecha, total, minutos_dif, cod_valid, cod_notvalid, cod_null, fecha_valida_null, promedio_minuto, user_insert, fecha_insert, sp, periodo)
					select cast(fecha_hora AS DATETIME YEAR to FRACTION(3))::date AS fecha
						,count(*) as Total
						, count(distinct(substr(fecha_hora, 12, 5))) as minutos_dif
						, sum(case when valida_correo='200' then 1 when valida_correo='210' then 1 when valida_correo='220' then 1 else 0 end) as cod_valid
						, sum(case when valida_correo='300' then 1 when valida_correo='400' then 1 when valida_correo='500' then 1 else 0 end) as cod_notvalid
						, sum(case when valida_correo is null then 1 else 0 end) as cod_NULL
						, sum(case when fecha_valida is null then 1 else 0 end) as fecha_valida_NULL
						, cast(
								(
									sum(case when valida_correo='200' then 1 when valida_correo='210' then 1 when valida_correo='220' then 1 else 0 end)
									+
									sum(case when valida_correo='300' then 1 when valida_correo='400' then 1 when valida_correo='500' then 1 else 0 end)
									)
							 / count(distinct(substr(fecha_hora, 12, 5))) as decimal(8,2)) as promXMinuto
						, usuario
						, vFechaInsert
						, trim(Vsp)
						, Vperiodo
					from tmp_correos
					where substr(fecha_hora, 12, 5) between '10:00' and '20:00'
						group by 1
						order by 1 asc;
		
		
		--SE OBTIENE EL NUMERO MAXIMO DE VALIDACIONES POR MINUTO Y EL MINUTO EN QUE OCURRIÃ
		
				FOREACH SELECT fecha INTO vFecha_minuto from "informix".sac_estadisticas_correos_minuto where fecha_insert = vFechaInsert
					
					DROP TABLE IF EXISTS TB_MAXIMO_INDIVIDUAL;
					
					CREATE TEMP TABLE TB_MAXIMO_INDIVIDUAL (fecha date, maximo int, minuto char(5)) WITH NO LOG; 	
					
						insert into TB_MAXIMO_INDIVIDUAL (fecha, maximo, minuto)
							select 
									EXTEND(cast(fecha_hora AS DATETIME YEAR to FRACTION(3)), YEAR TO DAY)::date AS FECHA
									, count(substr(fecha_hora, 12, 5)) as altasXMinuto
									, substr(fecha_hora, 12, 5) as minuto
							FROM tmp_correos 
							where substr(fecha_hora, 12, 5) between '10:00' and '20:00'
								and substr(fecha_hora, 1, 10) = EXTEND(mdy(month(vFecha_minuto),day(vFecha_minuto),year(vFecha_minuto)), YEAR TO DAY)
							group by 1,3 
							having count(substr(fecha_hora, 12, 5)) = 
							(SELECT MAX(A.altasXMinutos) 
							FROM (select cast(fecha_hora AS DATETIME YEAR to FRACTION(3))::date AS fecha_valida, substr(fecha_hora, 12, 5) as minuto, count(substr(fecha_hora, 12, 5)) as altasXMinutos
									from tmp_correos 
									where substr(fecha_hora, 12, 5) between '10:00' and '20:00'
										and substr(fecha_hora, 1, 10) = EXTEND(mdy(month(vFecha_minuto),day(vFecha_minuto),year(vFecha_minuto)), YEAR TO DAY)
									group by 1,2  
								) AS A ) order by minuto desc;

						LET vMaximo_minuto=(select first 1 maximo from TB_MAXIMO_INDIVIDUAL where fecha= date(vFecha_minuto));
						LET vHora_maximo=(select first 1 minuto from TB_MAXIMO_INDIVIDUAL where fecha= date(vFecha_minuto));
													
					UPDATE "informix".sac_estadisticas_correos_minuto SET maximo_validaciones=vMaximo_minuto, hora_maximo_validaciones=vHora_maximo WHERE fecha_insert = vFechaInsert and fecha= date(vFecha_minuto);
					
				END FOREACH;

				INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022
						
		RETURN cCodRet, cMensaje;
	END;

END PROCEDURE
DOCUMENT
'AUTOR: Noe Medina Ramirez ',
'DESCRIPCIÃN: Genera la informaciÃ³n para los Reportes Periodicos de VALIDACION DE CORREOS POR MINUTO.',
'SUSTENTO: ',
'EJECUTADO O LLAMADO POR: ',
'FECHA: 09/09/2016 ',
'ACTUALIZACION:',
'FECHA: 30/11/2016 ',
'SE QUITA EL TRUNCATE DE LA TABLA DE ESTADISTICAS',
'ACTUALIZACION:',
'FECHA: 05/12/2016 ',
'SE CAMBIA EL FORMATO DE FECHAS A MDY',
'VERSIÃN:  ',
'Solicita: Jaime GonzÃ¡lez',
'BD: bdisac';

CREATE PROCEDURE "informix".sp_estadisticas_sac_mnsj_batch_online
( fechaInicio date, fechaFin date )

RETURNING
	CHAR(5) AS codigo_respuesta,
	CHAR(80) AS mensaje_respuesta;

	DEFINE cCodRet CHAR(5);
	DEFINE cMensaje CHAR(80);
	
	DEFINE iSqlErr INT;
    DEFINE iIsamErr INT;
    DEFINE cInfoErr CHAR;
    DEFINE usuario CHAR(9);
    DEFINE Vsp CHAR(100);
    DEFINE Vid_sp INTEGER;
    DEFINE Vperiodo CHAR(25);
    
    DEFINE Vmes INTEGER;
    DEFINE Vanio INTEGER;
    DEFINE Vmesanio CHAR(25);
    DEFINE Vtotal INTEGER;
    DEFINE Vminutos_diferentes INTEGER;
    DEFINE Vpromedio_minuto DECIMAL(18,2);

    DEFINE VtotalB INTEGER;
    DEFINE VtotalO INTEGER;
    DEFINE VminutosDiferentesB INTEGER;
    DEFINE VminutosDiferentesO INTEGER;


--INICIALIZACION DE VARIABLES

	LET cCodRet = "00000";
	LET cMensaje = 'PROCESO EXITOSO';
	
    LET usuario='informix';
    LET Vsp ='sp_estadisticas_sac_mnsj_batch_online';
    LET Vid_sp = '12';
    LET Vperiodo = fechaInicio || ' a ' || fechaFin;

	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;

--	SET DEBUG FILE TO '/informix/tmp/sp_estadisticas_sac_mnsj_batch_online.out';
--	TRACE ON;

	BEGIN
		ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
			IF iSqlErr <> 0 THEN
                LET cCodRet = iSqlErr;
				LET cMensaje = "ERROR";
				INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022
				RETURN cCodRet, cMensaje;
			END IF;
		END EXCEPTION;
	 
	DROP TABLE IF EXISTS TMP_MNSJR_TEMPORAL;
	DROP TABLE IF EXISTS TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE;
		
	CREATE TEMP TABLE TMP_MNSJR_TEMPORAL (fecha_hora_registro DATETIME YEAR to SECOND NOT NULL, tipo_mensaje CHAR(1) NOT NULL, tabla char(1)) WITH NO LOG; 
	CREATE TEMP TABLE TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE ( 
																mes                       	INTEGER,
																anio                      	INTEGER,
																mesanio                   	VARCHAR(25),
																tipo1_b                   	INTEGER DEFAULT 0,
																minutos_diferentes_tipo1_b	INTEGER DEFAULT 0,
																promedio_minuto_tipo1_b   	DECIMAL(15,5) DEFAULT 0.00000,
																tipo2_b                   	INTEGER DEFAULT 0,
																minutos_diferentes_tipo2_b	INTEGER DEFAULT 0,
																promedio_minuto_tipo2_b   	DECIMAL(15,5) DEFAULT 0.00000,
																total_b                   	INTEGER DEFAULT 0,
																minutos_diferentes_b      	INTEGER DEFAULT 0,
																promedio_minuto_b         	DECIMAL(15,5) DEFAULT 0.00000,
																tipo1_o                   	INTEGER DEFAULT 0,
																minutos_diferentes_tipo1_o	INTEGER DEFAULT 0,
																promedio_minuto_tipo1_o   	DECIMAL(15,5) DEFAULT 0.00000,
																tipo2_o                   	INTEGER DEFAULT 0,
																minutos_diferentes_tipo2_o	INTEGER DEFAULT 0,
																promedio_minuto_tipo2_o   	DECIMAL(15,5) DEFAULT 0.00000,
																total_o                   	INTEGER DEFAULT 0,
																minutos_diferentes_o      	INTEGER DEFAULT 0,
																promedio_minuto_o         	DECIMAL(15,5) DEFAULT 0.00000,
																
																mensajes_tipo1_total            	INTEGER DEFAULT 0,
																minutos_diferentes_tipo1_total  	INTEGER DEFAULT 0,
																promedio_minuto_tipo1_total     	DECIMAL(15,5) DEFAULT 0.00000,
																
																mensajes_tipo2_total            	INTEGER DEFAULT 0,
																minutos_diferentes_tipo2_total  	INTEGER DEFAULT 0,
																promedio_minuto_tipo2_total     	DECIMAL(15,5) DEFAULT 0.00000,
																
																mensajes_total            	INTEGER DEFAULT 0,
																minutos_diferentes_total  	INTEGER DEFAULT 0,
																promedio_minuto_total     	DECIMAL(15,5) DEFAULT 0.00000,
																user_insert               	VARCHAR(25),
																fecha_insert              	DATETIME YEAR to FRACTION(3),
																sp                        	VARCHAR(50),
																periodo                   	VARCHAR(25) 
																) WITH NO LOG;
	
	INSERT INTO TMP_MNSJR_TEMPORAL(fecha_hora_registro, tipo_mensaje, tabla)
		SELECT fecha_hora_registro, tipo_mensaje,'B'
		FROM bdimnsj:"informix".mnsjr_trx_batch
		WHERE date(fecha_hora_registro) >= mdy(month(fechaInicio),day(fechaInicio),year(fechaInicio))
		AND date(fecha_hora_registro) <= mdy(month(fechaFin),day(fechaFin),year(fechaFin));
		
	INSERT INTO TMP_MNSJR_TEMPORAL(fecha_hora_registro, tipo_mensaje, tabla)
		SELECT fecha_hora_registro, tipo_mensaje,'B'
		FROM bdimnsj:"informix".mnsjr_trx_batch_his
		WHERE date(fecha_hora_registro) >= mdy(month(fechaInicio),day(fechaInicio),year(fechaInicio))
		AND date(fecha_hora_registro) <= mdy(month(fechaFin),day(fechaFin),year(fechaFin));
		
	INSERT INTO TMP_MNSJR_TEMPORAL(fecha_hora_registro, tipo_mensaje, tabla)
		SELECT fecha_hora_registro, tipo_mensaje,'O'
		FROM bdimnsj:"informix".mnsjr_trx_online
		WHERE date(fecha_hora_registro) >= mdy(month(fechaInicio),day(fechaInicio),year(fechaInicio))
		AND date(fecha_hora_registro) <= mdy(month(fechaFin),day(fechaFin),year(fechaFin));
		
	INSERT INTO TMP_MNSJR_TEMPORAL(fecha_hora_registro, tipo_mensaje, tabla)
		SELECT fecha_hora_registro, tipo_mensaje,'O'
		FROM bdimnsj:"informix".mnsjr_trx_online_his
		WHERE date(fecha_hora_registro) >= mdy(month(fechaInicio),day(fechaInicio),year(fechaInicio))
		AND date(fecha_hora_registro) <= mdy(month(fechaFin),day(fechaFin),year(fechaFin));

	--INSERTA TODAS LAS FECHAS Y TOTALES
			insert into TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE (mes, anio, mesanio, mensajes_total, minutos_diferentes_total, promedio_minuto_total, user_insert, fecha_insert, sp, periodo)
						select MONTH(fecha_hora_registro) as mes, year(fecha_hora_registro) as anio, 
								DECODE(MONTH(fecha_hora_registro),1,'Enero',2,'Febrero',3,'Marzo',4,'Abril',5,'Mayo',6,'Junio',7,'Julio',8,'Agosto',9,'Septiembre',10,'Octubre',11,'Noviembre',12,'Diciembre') || ' ' || year(fecha_hora_registro) as mesanio 
								, count(*) as total_mensajes
								, count(distinct(substr(fecha_hora_registro,1,16))) AS minutosDiferentes --distingue por MINUTO, NO POR SEGUNDO
								, cast(count(*) / count(distinct(substr(fecha_hora_registro,1,16))) as decimal(18,2)) as promPorMinuto
								, usuario
								, current
								, trim(Vsp)
								, trim(Vperiodo)
						from TMP_MNSJR_TEMPORAL 
						where tipo_mensaje in('1','2')
						group by 2,1,3 order by 2,1,3 asc;
						
						/*
						mensajes_total ,minutos_diferentes_total, promedio_minuto_total
						*/
		--TIPO 1 BATCH
				FOREACH select MONTH(fecha_hora_registro) as mes
								, year(fecha_hora_registro) as anio 
								, count(*) as total_mensajes
								, count(distinct(substr(fecha_hora_registro,1,16))) AS minutosDiferentes --distingue por MINUTO, NO POR SEGUNDO
								, cast(count(*) / count(distinct(substr(fecha_hora_registro,1,16))) as decimal(18,2)) as promPorMinuto
						INTO Vmes, Vanio, Vtotal, Vminutos_diferentes, Vpromedio_minuto
						from TMP_MNSJR_TEMPORAL 
						where tipo_mensaje in('1') and tabla='B'
						group by 2,1 order by 2,1 asc

						IF (SELECT COUNT(*) FROM TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE WHERE mes= Vmes and anio=Vanio) > 0 THEN
							UPDATE TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE 
							SET 
								  tipo1_b = Vtotal
								, minutos_diferentes_tipo1_b = Vminutos_diferentes
								, promedio_minuto_tipo1_b = Vpromedio_minuto
							WHERE mes= Vmes and anio=Vanio;
						END IF
				END FOREACH;
		
		--TIPO 2 BATCH		
				FOREACH select MONTH(fecha_hora_registro) as mes
								, year(fecha_hora_registro) as anio 
								, count(*) as total_mensajes
								, count(distinct(substr(fecha_hora_registro,1,16))) AS minutosDiferentes --distingue por MINUTO, NO POR SEGUNDO
								, cast(count(*) / count(distinct(substr(fecha_hora_registro,1,16))) as decimal(18,2)) as promPorMinuto
						INTO Vmes, Vanio, Vtotal, Vminutos_diferentes, Vpromedio_minuto
						from TMP_MNSJR_TEMPORAL 
						where tipo_mensaje in('2') and tabla='B'
						group by 2,1 order by 2,1 asc

						IF (SELECT COUNT(*) FROM TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE WHERE mes= Vmes and anio=Vanio) > 0 THEN
							UPDATE TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE 
							SET 
								  tipo2_b = Vtotal
								, minutos_diferentes_tipo2_b = Vminutos_diferentes
								, promedio_minuto_tipo2_b = Vpromedio_minuto
							WHERE mes= Vmes and anio=Vanio;
						END IF
				END FOREACH;
				
				
		--TOTAL (1,2) BATCH		
				FOREACH select MONTH(fecha_hora_registro) as mes
								, year(fecha_hora_registro) as anio 
								, count(*) as total_mensajes
								, count(distinct(substr(fecha_hora_registro,1,16))) AS minutosDiferentes --distingue por MINUTO, NO POR SEGUNDO
								, cast(count(*) / count(distinct(substr(fecha_hora_registro,1,16))) as decimal(18,2)) as promPorMinuto
						INTO Vmes, Vanio, Vtotal, Vminutos_diferentes, Vpromedio_minuto
						from TMP_MNSJR_TEMPORAL 
						where tipo_mensaje in('1','2') and tabla='B'
						group by 2,1 order by 2,1 asc

						IF (SELECT COUNT(*) FROM TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE WHERE mes= Vmes and anio=Vanio) > 0 THEN
							UPDATE TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE 
							SET 
								  total_b = Vtotal
								, minutos_diferentes_b = Vminutos_diferentes
								, promedio_minuto_b = Vpromedio_minuto
							WHERE mes= Vmes and anio=Vanio;
						END IF
				END FOREACH;
				
				
				
		--TIPO 1 ONLINE
				FOREACH select MONTH(fecha_hora_registro) as mes
								, year(fecha_hora_registro) as anio 
								, count(*) as total_mensajes
								, count(distinct(substr(fecha_hora_registro,1,16))) AS minutosDiferentes --distingue por MINUTO, NO POR SEGUNDO
								, cast(count(*) / count(distinct(substr(fecha_hora_registro,1,16))) as decimal(18,2)) as promPorMinuto
						INTO Vmes, Vanio, Vtotal, Vminutos_diferentes, Vpromedio_minuto
						from TMP_MNSJR_TEMPORAL 
						where tipo_mensaje in('1') and tabla='O'
						group by 2,1 order by 2,1 asc

						IF (SELECT COUNT(*) FROM TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE WHERE mes= Vmes and anio=Vanio) > 0 THEN
							UPDATE TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE 
							SET 
								  tipo1_o = Vtotal
								, minutos_diferentes_tipo1_o = Vminutos_diferentes
								, promedio_minuto_tipo1_o = Vpromedio_minuto
							WHERE mes= Vmes and anio=Vanio;
						END IF
				END FOREACH;
				
		--TIPO 2 ONLINE
				FOREACH select MONTH(fecha_hora_registro) as mes
								, year(fecha_hora_registro) as anio 
								, count(*) as total_mensajes
								, count(distinct(substr(fecha_hora_registro,1,16))) AS minutosDiferentes --distingue por MINUTO, NO POR SEGUNDO
								, cast(count(*) / count(distinct(substr(fecha_hora_registro,1,16))) as decimal(18,2)) as promPorMinuto
						INTO Vmes, Vanio, Vtotal, Vminutos_diferentes, Vpromedio_minuto
						from TMP_MNSJR_TEMPORAL 
						where tipo_mensaje in('2') and tabla='O'
						group by 2,1 order by 2,1 asc

						IF (SELECT COUNT(*) FROM TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE WHERE mes= Vmes and anio=Vanio) > 0 THEN
							UPDATE TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE 
							SET 
								  tipo2_o = Vtotal
								, minutos_diferentes_tipo2_o = Vminutos_diferentes
								, promedio_minuto_tipo2_o = Vpromedio_minuto
							WHERE mes= Vmes and anio=Vanio;
						END IF
				END FOREACH;
				
		--TOTAL ONLINE
				FOREACH select MONTH(fecha_hora_registro) as mes
								, year(fecha_hora_registro) as anio 
								, count(*) as total_mensajes
								, count(distinct(substr(fecha_hora_registro,1,16))) AS minutosDiferentes --distingue por MINUTO, NO POR SEGUNDO
								, cast(count(*) / count(distinct(substr(fecha_hora_registro,1,16))) as decimal(18,2)) as promPorMinuto
						INTO Vmes, Vanio, Vtotal, Vminutos_diferentes, Vpromedio_minuto
						from TMP_MNSJR_TEMPORAL 
						where tipo_mensaje in('1','2') and tabla='O'
						group by 2,1 order by 2,1 asc

						IF (SELECT COUNT(*) FROM TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE WHERE mes= Vmes and anio=Vanio) > 0 THEN
							UPDATE TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE 
							SET 
								  total_o = Vtotal
								, minutos_diferentes_o = Vminutos_diferentes
								, promedio_minuto_o = Vpromedio_minuto
							WHERE mes= Vmes and anio=Vanio;
						END IF
				END FOREACH;
				
		--TOTAL TIPO 1
				FOREACH select MONTH(fecha_hora_registro) as mes
								, year(fecha_hora_registro) as anio 
								, count(*) as total_mensajes
								, count(distinct(substr(fecha_hora_registro,1,16))) AS minutosDiferentes --distingue por MINUTO, NO POR SEGUNDO
								, cast(count(*) / count(distinct(substr(fecha_hora_registro,1,16))) as decimal(18,2)) as promPorMinuto
						INTO Vmes, Vanio, Vtotal, Vminutos_diferentes, Vpromedio_minuto
						from TMP_MNSJR_TEMPORAL 
						where tipo_mensaje in('1') and tabla in ('O','B')
						group by 2,1 order by 2,1 asc

						IF (SELECT COUNT(*) FROM TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE WHERE mes= Vmes and anio=Vanio) > 0 THEN
							UPDATE TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE 
							SET 
								  mensajes_tipo1_total = Vtotal
								, minutos_diferentes_tipo1_total = Vminutos_diferentes
								, promedio_minuto_tipo1_total = Vpromedio_minuto
							WHERE mes= Vmes and anio=Vanio;
						END IF
				END FOREACH;
				
		--TOTAL TIPO 1
				FOREACH select MONTH(fecha_hora_registro) as mes
								, year(fecha_hora_registro) as anio 
								, count(*) as total_mensajes
								, count(distinct(substr(fecha_hora_registro,1,16))) AS minutosDiferentes --distingue por MINUTO, NO POR SEGUNDO
								, cast(count(*) / count(distinct(substr(fecha_hora_registro,1,16))) as decimal(18,2)) as promPorMinuto
						INTO Vmes, Vanio, Vtotal, Vminutos_diferentes, Vpromedio_minuto
						from TMP_MNSJR_TEMPORAL 
						where tipo_mensaje in('2') and tabla in ('O','B')
						group by 2,1 order by 2,1 asc

						IF (SELECT COUNT(*) FROM TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE WHERE mes= Vmes and anio=Vanio) > 0 THEN
							UPDATE TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE 
							SET 
								  mensajes_tipo2_total = Vtotal
								, minutos_diferentes_tipo2_total = Vminutos_diferentes
								, promedio_minuto_tipo2_total = Vpromedio_minuto
							WHERE mes= Vmes and anio=Vanio;
						END IF
				END FOREACH;


			
			INSERT INTO "informix".sac_estadisticas_mnsj_batch_online 
				SELECT * FROM TMP_SAC_ESTADISTICAS_MNSJ_BATCH_ONLINE;

				INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022
				
		RETURN cCodRet, cMensaje;
                        
                  
	END;

END PROCEDURE
DOCUMENT
'AUTOR: Noe Medina Ramirez ',
'DESCRIPCIÃN: Genera la informaciÃ³n para los Reportes Periodicos de ENVIO DE MENSAJES (BATCH / ONLINE)',
'SUSTENTO: ',
'EJECUTADO O LLAMADO POR: ',
'FECHA: 21/10/2016 ',
'ACTUALIZACION:',
'FECHA: 30/11/2016 ',
'SE QUITA EL TRUNCATE DE LA TABLA DE ESTADISTICAS',
'ACTUALIZACION:',
'FECHA: 05/12/2016 ',
'SE CAMBIA EL FORMATO DE FECHAS A MDY',
'VERSIÃN:  ',
'Solicita: Jaime GonzÃ¡lez',
'BD: bdisac';

CREATE PROCEDURE "informix".sp_estadisticas_sac_pagoservicios
( fechaInicio date, fechaFin date )

RETURNING
	CHAR(5) AS codigo_respuesta,
	CHAR(80) AS mensaje_respuesta;

	DEFINE cCodRet CHAR(5);
	DEFINE cMensaje CHAR(80);
	DEFINE iSqlErr INT;
    DEFINE iIsamErr INT;
    DEFINE cInfoErr CHAR;
	DEFINE usuario CHAR(9);
    DEFINE Vsp CHAR(100);
    DEFINE Vid_sp INTEGER;
    DEFINE Vperiodo CHAR(25);
	
	DEFINE vdiasInicio INTEGER;
	DEFINE vdiasFin INTEGER;
	DEFINE vFechaInicioHIS DATE;
	DEFINE vFechaFinOLD DATE;
    
	
    LET usuario='informix';
    LET Vsp ='sp_estadisticas_sac_pagoservicios';
    LET Vid_sp = '2';
    LET Vperiodo = fechaInicio || ' a ' || fechaFin;
	
	LET vdiasInicio = (date(current) - fechaInicio);
	LET vdiasFin = (date(current) - fechaFin);
	
	LET vFechaInicioHIS = fechaInicio;
	LET vFechaFinOLD = fechaFin;
		
	LET cCodRet = "00000";
	LET cMensaje = 'PROCESO EXITOSO';

	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;

	--SET DEBUG FILE TO '/ifxsif01/Control-M/sp_estadisticas_sac_pagoservicios.out';
	--TRACE ON;

	BEGIN
		ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
			IF iSqlErr <> 0 THEN
                LET cCodRet = iSqlErr;
				LET cMensaje = "ERROR";
				INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022
				RETURN cCodRet, cMensaje;
			END IF;
		END EXCEPTION;
	
	
--VALIDA EL RAGO DE FECHAS
	IF fechaInicio > fechaFin  THEN
		LET cCodRet = "11111";
		LET cMensaje = "ERROR: LA FECHA DE INICIO NO PUEDE SER MAYOR A LA FECHA FINAL";
		RETURN cCodRet, cMensaje;
	END IF;
	
			
--SI LA FECHA INICIAL A CONSULTAR ES MAYOR A 91 DIAS PREVIOS Y LA FECHA FINAL ES MENOR A 92 DÃAS PREVIOS, SIGNIFICA QUE TIENE QUE BUSCAR EN DOS TABLAS
--ENTONCES ASIGNA LA FECHA FINAL QUE SE BUSCARA EN "HISTORIAL_OLD" Y LA FECHA INICIAL QUE SE BUSCARA EN "HISTORIAL"
	IF vdiasInicio > 91 and vdiasFin < 92 THEN
		LET vFechaFinOLD = (fechaInicio + (vdiasInicio - 92));
		LET vFechaInicioHIS = (vFechaFinOLD + 1);
	END IF;
	
	
--SI LAS FECHAS DE INICIO Y FIN, DE HISTORIAL Y OLD SON IGUALES RESPECTIVAMENTE, BUSCA SOLO UN RANGO DE FECHAS
	IF vFechaInicioHIS = fechaInicio AND vFechaFinOLD = fechaFin THEN
		
	--SI LA FECHA INICIAL ES MENOR A 92 DIAS PREVIOS BUSCARÃ EN "HISTORIAL", DE LO CONTRARIO BUSCARÃ EN "HISTORIAL_OLD"
		IF vdiasInicio < 92 THEN
				
		LET cMensaje = 'CORRECTO: DIAS BUSCADOS HIS: ' || (fechaFin - fechaInicio);
		
			insert into "informix".sac_estadisticas_pagoservicios 
			select 
				H.numcategoria
				, H.numconvenio
				, C.nomconvenio
				, MONTH(H.fecha_pago) as mes
				, year(H.fecha_pago) as anio
				, DECODE(MONTH(H.fecha_pago),1,'Enero',2,'Febrero',3,'Marzo',4,'Abril',5,'Mayo',6,'Junio'
											,7,'Julio',8,'Agosto',9,'Septiembre',10,'Octubre',11,'Noviembre',12,'Diciembre') || ' ' || year(H.fecha_pago) as mesAnio
				, count(distinct(day(H.fecha_pago))) as diasPorMes
				, cast(COUNT(*) as integer) as num_pagos
				, cast((COUNT(*) / count(distinct(day(H.fecha_pago)))) as decimal(18,0)) as promedio_pagos_dia
				, CAST(sum(H.importe_pago) AS DECIMAL(18,2)) as importe
				, cast(avg(H.importe_pago) as decimal(18,2)) as imprt_promedio
				, usuario
				, current
				, trim(Vsp)
				, Vperiodo
			from 
				bdisac:"informix".sac_movimientoshistorial H, 
				bdisac:"informix".sac_convenios C
			where H.status_cancelado <> 'S' 
				and H.fecha_pago between fechaInicio and fechaFin
				and C.statusconvenio='A'
				and H.numcategoria=C.numcategoria 
				and H.numconvenio=C.numconvenio 
			group by 5,4,1,2,3,6
			order by 5,4,1,2,3,6 asc;
		ELSE
		
		LET cMensaje = 'CORRECTO: DIAS BUSCADOS OLD:' || (fechaFin - fechaInicio);
		
			DROP TABLE IF EXISTS TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP;

			CREATE TEMP TABLE TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP(
																numcategoria              	CHAR(2) NOT NULL,
																numconvenio               	CHAR(5) NOT NULL,
																fecha_pago                	DATE NOT NULL,
																importe_pago              	MONEY
																) WITH NO LOG;
															
			EXECUTE IMMEDIATE  'CREATE INDEX idx_mov_pagoservicios_tmp ON TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP(numcategoria,numconvenio)';	
			
			INSERT INTO TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP
				SELECT {+INDEX (informix.sac_movimientoshistorial_old.idxsac_movhisfe_old)} numcategoria, numconvenio, fecha_pago, importe_pago FROM bdisac:"informix".sac_movimientoshistorial_old
					WHERE status_cancelado <> 'S' 
						and DATE(fecha_pago) >= mdy(month(fechaInicio),day(fechaInicio),year(fechaInicio))
						and DATE(fecha_pago) <= mdy(month(fechaFin),day(fechaFin),year(fechaFin));
						
			INSERT INTO "informix".sac_estadisticas_pagoservicios 
				SELECT {+INDEX (informix.TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP.idx_mov_pagoservicios_tmp)} 
					H.numcategoria
					, H.numconvenio
					, C.nomconvenio
					, MONTH(H.fecha_pago) as mes
					, year(H.fecha_pago) as anio
					, DECODE(MONTH(H.fecha_pago),1,'Enero',2,'Febrero',3,'Marzo'
											,4,'Abril',5,'Mayo',6,'Junio',7,'Julio'
											,8,'Agosto',9,'Septiembre',10,'Octubre'
											,11,'Noviembre',12,'Diciembre') || ' ' || year(H.fecha_pago) as mesAnio
					, count(distinct(day(H.fecha_pago))) as diasPorMes
					, cast(COUNT(*) as integer) as num_pagos
					, cast((COUNT(*) / count(distinct(day(fecha_pago)))) as decimal(18,0)) as promedio_pagos_dia
					, CAST(sum(H.importe_pago) AS DECIMAL(18,2)) as importe
					, cast(avg(H.importe_pago) as decimal(18,2)) as imprt_promedio
					, usuario
					, current
					, Vsp
					, Vperiodo
				from TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP H, "informix".sac_convenios C
				where H.numcategoria=C.numcategoria and H.numconvenio=C.numconvenio
				group by 5,4,1,2,3,6
				order by 5,4,1,2,3,6 asc;
          
			DROP TABLE IF EXISTS TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP;
		END IF
		
	ELSE
	--SI SE GENERAN DOS RANGOS DE FECHAS, CREA TABLA TEMPORAL Y ALMACENA LOS DATOS DE "HISTORIAL" E "HISTORIAL_OLD" PARA REALIZAR LOS CALCULOS
		
		LET cMensaje = 'CORRECTO: DIAS BUSCADOS HIS: ' || (fechaFin - vFechaInicioHIS) || ', OLD: ' || (vFechaFinOLD - fechaInicio);
		
		DROP TABLE IF EXISTS TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP;

				CREATE TEMP TABLE TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP(
																numcategoria              	CHAR(2) NOT NULL,
																numconvenio               	CHAR(5) NOT NULL,
																fecha_pago                	DATE NOT NULL,
																importe_pago              	MONEY
																) WITH NO LOG;
																
				EXECUTE IMMEDIATE  'CREATE INDEX idx_mov_pagoservicios_tmp ON TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP(numcategoria,numconvenio)';	
				
				INSERT INTO TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP
					SELECT {+INDEX (informix.sac_movimientoshistorial.idxsac_movhisfe)} numcategoria, numconvenio, fecha_pago, importe_pago FROM bdisac:"informix".sac_movimientoshistorial
						WHERE status_cancelado <> 'S' 
							and DATE(fecha_pago) >= mdy(month(vFechaInicioHIS),day(vFechaInicioHIS),year(vFechaInicioHIS))
							and DATE(fecha_pago) <= mdy(month(fechaFin),day(fechaFin),year(fechaFin));
				
				INSERT INTO TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP
					SELECT {+INDEX (informix.sac_movimientoshistorial_old.idxsac_movhisfe_old)} numcategoria, numconvenio, fecha_pago, importe_pago FROM bdisac:"informix".sac_movimientoshistorial_old
						WHERE status_cancelado <> 'S' 
							and DATE(fecha_pago) >= mdy(month(fechaInicio),day(fechaInicio),year(fechaInicio))
							and DATE(fecha_pago) <= mdy(month(vFechaFinOLD),day(vFechaFinOLD),year(vFechaFinOLD));
							
                INSERT INTO "informix".sac_estadisticas_pagoservicios 
					SELECT {+INDEX (informix.TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP.idx_mov_pagoservicios_tmp)} 
						H.numcategoria
						, H.numconvenio
						, C.nomconvenio
						, MONTH(H.fecha_pago) as mes
						, year(H.fecha_pago) as anio
						, DECODE(MONTH(H.fecha_pago),1,'Enero',2,'Febrero',3,'Marzo'
												,4,'Abril',5,'Mayo',6,'Junio',7,'Julio'
												,8,'Agosto',9,'Septiembre',10,'Octubre'
												,11,'Noviembre',12,'Diciembre') || ' ' || year(H.fecha_pago) as mesAnio
						, count(distinct(day(H.fecha_pago))) as diasPorMes
						, cast(COUNT(*) as integer) as num_pagos
						, cast((COUNT(*) / count(distinct(day(H.fecha_pago)))) as decimal(18,0)) as promedio_pagos_dia
						, CAST(sum(H.importe_pago) AS DECIMAL(18,2)) as importe
						, cast(avg(H.importe_pago) as decimal(18,2)) as imprt_promedio
						, usuario
						, current
						, Vsp
						, Vperiodo
					from TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP H, "informix".sac_convenios C
					where H.numcategoria=C.numcategoria and H.numconvenio=C.numconvenio
					group by 5,4,1,2,3,6
					order by 5,4,1,2,3,6 asc;
          
        DROP TABLE IF EXISTS TB_MOVIMIENTOS_PAGOSERVICIOS_TEMP;
		
	END IF;

	INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022

		RETURN cCodRet, cMensaje;
	END;

END PROCEDURE
DOCUMENT
'AUTOR: Noe Medina Ramirez ',
'DESCRIPCIÃN: Genera la informaciÃ³n para los Reportes Periodicos de Pago de Servicios',
'SUSTENTO: ',
'EJECUTADO O LLAMADO POR: ',
'FECHA: 09/09/2016 ',
'ACTUALIZACION:',
'FECHA: 30/11/2016 ',
'SE QUITA EL TRUNCATE DE LA TABLA DE ESTADISTICAS',
'ACTUALIZACION:',
'FECHA: 05/12/2016 ',
'SE CAMBIA EL FORMATO DE FECHAS A MDY',
'VERSIÃN:  ',
'Solicita: Jaime GonzÃ¡lez',
'BD: bdisac';

CREATE PROCEDURE "informix".sp_estadisticas_sac_remesas_estados
( fechaInicio date, fechaFin date )

RETURNING
	CHAR(5) AS codigo_respuesta,
	CHAR(80) AS mensaje_respuesta;
	
	DEFINE usuario CHAR(9);
    DEFINE Vsp CHAR(100);
    DEFINE Vid_sp INTEGER;
    DEFINE Vperiodo CHAR(25);
	
	DEFINE cCodRet CHAR(5);
	DEFINE cMensaje CHAR(80);
	
	DEFINE iSqlErr INT;
    DEFINE iIsamErr INT;
    DEFINE cInfoErr CHAR;
    
    DEFINE BTSestado char(30);
    DEFINE BTSnumPagos int;
    DEFINE BTSimportePagos decimal(18,2);
    DEFINE BTSpromedioImporte decimal(18,2);
    DEFINE BTSnumPagosH int;
    DEFINE BTSimportePagosH decimal(18,2);
	DEFINE BTmes INTEGER;
	DEFINE BTanio INTEGER;

    DEFINE WUestado char(30);
    DEFINE WUnumPagos int;
    DEFINE WUimportePagos decimal(18,2);
    DEFINE WUpromedioImporte decimal(18,2);
    DEFINE WUnumPagosH int;
    DEFINE WUimportePagosH decimal(18,2);
	DEFINE WUmes INTEGER;
	DEFINE WUanio INTEGER;
	
	DEFINE APPestado char(30);
    DEFINE APPnumPagos int;
    DEFINE APPimportePagos decimal(18,2);
    DEFINE APPpromedioImporte decimal(18,2);
    DEFINE APPnumPagosH int;
    DEFINE APPimportePagosH decimal(18,2);
	DEFINE APPmes INTEGER;
	DEFINE APPanio INTEGER;

	LET cCodRet = "00000";
	LET cMensaje = 'PROCESO EXITOSO';

    LET BTSestado='';
    LET BTSnumPagos=0;
    LET BTSimportePagos=0;
    LET BTSpromedioImporte=0;
    
    LET WUestado='';
    LET WUnumPagos=0;
    LET WUimportePagos=0;
    LET WUpromedioImporte=0;
	
	LET APPestado='';
    LET APPnumPagos=0;
    LET APPimportePagos=0;
    LET APPpromedioImporte=0;

    LET usuario='informix';
    LET Vsp ='sp_estadisticas_sac_remesas_estados';
    LET Vid_sp = '1';
    LET Vperiodo = cast((fechaInicio || ' a ' || fechaFin) as char(25));
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;

--	SET DEBUG FILE TO '/ifxsif01/Control-M/sp_estadisticas_sac_remesas.out';
--	TRACE ON;

	BEGIN
		ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
			IF iSqlErr <> 0 THEN
                LET cCodRet = iSqlErr;
				LET cMensaje = "ERROR";
				INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022
				RETURN cCodRet, cMensaje;
			END IF;
		END EXCEPTION;

        DROP TABLE IF EXISTS TB_REMESAS_UNION;
        DROP TABLE IF EXISTS TB_BTS;
        DROP TABLE IF EXISTS TB_WU;
		DROP TABLE IF EXISTS TB_MOVIMIENTOS_HISTORIAL;
		DROP TABLE IF EXISTS TB_APPRIZA;

                    
        CREATE TEMP TABLE TB_REMESAS_UNION (mes int, anio int, mesanio char(20), cve_estado char(2), estado char(30),cant_sucursales int, totalBTS int, totalWU int, totalAPP int, total int) WITH NO LOG; 
		
		CREATE TEMP TABLE TB_MOVIMIENTOS_HISTORIAL ( 
													id_sucursal               	CHAR(4) NOT NULL,
													numcategoria              	CHAR(2) NOT NULL,
													numconvenio               	CHAR(5) NOT NULL,
													referencia1               	CHAR(40) NOT NULL,
													referencia2               	CHAR(40) NOT NULL,
													forma_pago                	CHAR(1),
													importe_pago              	MONEY,
													importe_comision_convenio 	MONEY,
													iva_comision_convenio     	MONEY,
													importe_comision_cte      	MONEY,
													iva_comision_cte          	MONEY,
													cuenta_cargo              	CHAR(12),
													usuario                   	CHAR(8),
													folio_suc                 	CHAR(16) NOT NULL,
													transacc_suc              	CHAR(4),
													flag_confirmacion_central 	SMALLINT,
													flag_confirmacion_sucursal	SMALLINT,
													fecha_pago                	DATE NOT NULL,
													fecha_insert              	DATETIME YEAR to FRACTION(3),
													status_cancelado          	CHAR(1),
													origen                    	CHAR(4) DEFAULT '',
													sucursal_cpl              	CHAR(4) DEFAULT '',
													caja_cpl                  	CHAR(3) DEFAULT '',
													transaccion               	CHAR(5) DEFAULT '',
													hora                      	CHAR(6) DEFAULT '',
													folio_operacion           	CHAR(18) DEFAULT '',
													referencia3               	CHAR(40) DEFAULT '',
													referencia4               	CHAR(40) DEFAULT '') WITH NO LOG;
													
		insert into TB_MOVIMIENTOS_HISTORIAL
			select * from bdisac:"informix".sac_movimientoshistorial
				where status_cancelado <> 'S' 
				and date(fecha_pago) >= mdy(month(fechaInicio),day(fechaInicio),year(fechaInicio))
				and date(fecha_pago) <= mdy(month(fechaFin),day(fechaFin),year(fechaFin))
				and ((numcategoria='07' and numconvenio='004')
				OR (numcategoria='07' and numconvenio='006')
				OR (numcategoria='07' and numconvenio='007')
				OR (numcategoria='07' and numconvenio='008')
				OR (numcategoria='07' and numconvenio='009'))
				and id_sucursal not in ('9250')
				and forma_pago in('1','2','4','5');
		
		insert into TB_MOVIMIENTOS_HISTORIAL
			select * from bdisac:"informix".sac_movimientoshistorial_old
				where status_cancelado <> 'S' 
				and date(fecha_pago) >= mdy(month(fechaInicio),day(fechaInicio),year(fechaInicio))
				and date(fecha_pago) <= mdy(month(fechaFin),day(fechaFin),year(fechaFin))
				and ((numcategoria='07' and numconvenio='004')
				OR (numcategoria='07' and numconvenio='006')
				OR (numcategoria='07' and numconvenio='007')
				OR (numcategoria='07' and numconvenio='008')
				OR (numcategoria='07' and numconvenio='009'))
				and id_sucursal not in ('9250')
				and forma_pago in('1','2','4','5');
				
--INSERT TODAS LAS FECHAS DE REMESAS============================================================
		INSERT INTO TB_REMESAS_UNION 
			select month(date(M.fecha_pago)) as mes
                    , year(date(M.fecha_pago)) as anio
                    , DECODE(MONTH(date(M.fecha_pago)),1,'Enero',2,'Febrero',3,'Marzo',4,'Abril',5,'Mayo',6,'Junio',7,'Julio',8,'Agosto',9,'Septiembre',10,'Octubre',11,'Noviembre',12,'Diciembre') || ' ' || year(date(M.fecha_pago)) as mesanio
					, E.estado
                    , e.nombre,count(distinct(id_sucursal)),0,0,0,0
					from TB_MOVIMIENTOS_HISTORIAL M, bdinteg:"informix".si_sucursales S {+INDEX (idx_sucursal)}, bdinteg:"informix".si_estados E {+INDEX (idx_nombre_estado)}
					where M.id_sucursal=S.Sucursal
					and E.estado=S.estado
					group by 2,1,3,5,4
					order by 2,1,3,5,4 asc;


--PROCESA BTS===================================================================================
        CREATE TEMP TABLE TB_BTS (mes int, anio int, estado char(30), num_pagos int, importe_pagos decimal(18,2), promedio_importe decimal(18,2)) WITH NO LOG; 
                        insert into TB_BTS (mes, anio, estado, num_pagos, importe_pagos, promedio_importe) 
                            select month(date(M.fecha_pago)) as mes, year(date(M.fecha_pago)) as anio, e.nombre as Estado,
                                COUNT(*) as num_pagos, sum(importe_pago) as importe, cast(avg(importe_pago) as decimal(18,2)) as imprt_promedio
                                from TB_MOVIMIENTOS_HISTORIAL M, bdinteg:"informix".si_sucursales S {+INDEX (idx_sucursal)}, bdinteg:"informix".si_estados E {+INDEX (idx_nombre_estado)}
                                where status_cancelado <> 'S' 
                                and numcategoria='07' and numconvenio='004'
                                and id_sucursal not in ('9250')
                                and forma_pago in('1','2','4','5')
                                and M.id_sucursal=S.Sucursal
                                and E.estado=S.estado
                                group by 2,1,3
                                order by 2,1,3 asc;
				
				FOREACH SELECT mes, anio, estado, num_pagos, importe_pagos INTO BTmes, BTanio, BTSestado, BTSnumPagos, BTSimportePagos FROM TB_BTS
                        if (select count(*) from TB_REMESAS_UNION where estado= BTSestado) > 0 then
							UPDATE TB_REMESAS_UNION SET totalBTS=BTSnumPagos WHERE estado= BTSestado and mes= BTmes and anio= BTanio;
                        end if
                END FOREACH;
					

--PROCESA WU===================================================================================

        CREATE TEMP TABLE TB_WU (mes int, anio int, estado char(30), num_pagos int, importe_pagos decimal(18,2), promedio_importe decimal(18,2)) WITH NO LOG; 
                        insert into TB_WU (mes, anio, estado, num_pagos, importe_pagos, promedio_importe) 
                            select month(date(M.fecha_pago)) as mes, year(date(M.fecha_pago)) as anio, e.nombre as Estado,
                                COUNT(*) as num_pagos, sum(importe_pago) as importe, cast(avg(importe_pago) as decimal(18,2)) as imprt_promedio
                                from TB_MOVIMIENTOS_HISTORIAL M, bdinteg:"informix".si_sucursales S {+INDEX (idx_sucursal)}, bdinteg:"informix".si_estados E {+INDEX (idx_nombre_estado)}
                                where status_cancelado <> 'S' 
                                and ((numcategoria='07' and numconvenio='006')
                                OR (numcategoria='07' and numconvenio='007')
                                OR (numcategoria='07' and numconvenio='008'))
                                and id_sucursal not in ('9250')
                                and forma_pago in('1','2','4','5')
                                and M.id_sucursal=S.Sucursal
                                and E.estado=S.estado
                                group by 2,1,3
                                order by 2,1,3 asc;

					FOREACH SELECT mes, anio, estado, num_pagos, importe_pagos INTO WUmes, WUanio, WUestado, WUnumPagos, WUimportePagos FROM TB_WU
                        if (select count(*) from TB_REMESAS_UNION where estado= WUestado) > 0 then
							UPDATE TB_REMESAS_UNION SET totalWU=WUnumPagos WHERE estado= WUestado and mes= WUmes and anio= WUanio;
                        end if
					END FOREACH;

--PROCESA APPRIZA===================================================================================

        CREATE TEMP TABLE TB_APPRIZA (mes int, anio int, estado char(30), num_pagos int, importe_pagos decimal(18,2), promedio_importe decimal(18,2)) WITH NO LOG; 
                        insert into TB_APPRIZA (mes, anio, estado, num_pagos, importe_pagos, promedio_importe) 
                            select month(date(M.fecha_pago)) as mes, year(date(M.fecha_pago)) as anio, e.nombre as Estado,
                                COUNT(*) as num_pagos, sum(importe_pago) as importe, cast(avg(importe_pago) as decimal(18,2)) as imprt_promedio
                                from TB_MOVIMIENTOS_HISTORIAL M, bdinteg:"informix".si_sucursales S {+INDEX (idx_sucursal)}, bdinteg:"informix".si_estados E {+INDEX (idx_nombre_estado)}
                                where status_cancelado <> 'S' 
                                and numcategoria='07' and numconvenio='009'
                                and id_sucursal not in ('9250')
                                and forma_pago in('1','2','4','5')
                                and M.id_sucursal=S.Sucursal
                                and E.estado=S.estado
                                group by 2,1,3
                                order by 2,1,3 asc;

					FOREACH SELECT mes, anio, estado, num_pagos, importe_pagos INTO APPmes, APPanio, APPestado, APPnumPagos, APPimportePagos FROM TB_APPRIZA
                        if (select count(*) from TB_REMESAS_UNION where estado= APPestado) > 0 then
							UPDATE TB_REMESAS_UNION SET totalAPP=APPnumPagos WHERE estado= APPestado and mes= APPmes and anio= APPanio;
                        end if
					END FOREACH;					

										

        insert into bdisac:"informix".sac_estadisticas_remesas_estados
        SELECT mes, anio, mesanio, cve_estado, estado, cant_sucursales, totalBTS, totalWU, totalAPP,(totalBTS + totalWU + totalAPP),usuario,current, TRIM(Vsp), Vperiodo FROM TB_REMESAS_UNION;

        DROP TABLE IF EXISTS TB_REMESAS_UNION;
        DROP TABLE IF EXISTS TB_BTS;
        DROP TABLE IF EXISTS TB_WU;
		DROP TABLE IF EXISTS TB_MOVIMIENTOS_HISTORIAL;
		DROP TABLE IF EXISTS TB_APPRIZA;

		INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022

		RETURN cCodRet, cMensaje;
	END;

END PROCEDURE
DOCUMENT
'AUTOR: Noe Medina Ramirez ',
'DESCRIPCIÃN: Genera la informaciÃ³n para los Reportes Periodicos de Pago de Servicios',
'SUSTENTO: ',
'EJECUTADO O LLAMADO POR: ',
'FECHA: 09/09/2016 ',
'ACTUALIZACION:',
'FECHA: 30/11/2016 ',
'SE QUITA EL TRUNCATE DE LA TABLA DE ESTADISTICAS',
'ACTUALIZACION:',
'FECHA: 05/12/2016 ',
'SE CAMBIA EL FORMATO DE FECHAS A MDY',
'VERSIÃN:  ',
'Solicita: Jaime GonzÃ¡lez',
'BD: bdisac';

CREATE PROCEDURE "informix".sp_estadisticas_sac_remesas_minuto
( fechaInicio date, fechaFin date )

RETURNING
	CHAR(5) AS codigo_respuesta,
	CHAR(80) AS mensaje_respuesta;

	DEFINE cCodRet CHAR(5);
	DEFINE cMensaje CHAR(80);
	
    DEFINE usuario CHAR(9);
    DEFINE Vsp CHAR(100);
    DEFINE Vid_sp INTEGER;
    DEFINE Vperiodo CHAR(25);
    
    DEFINE Vfecha_insert DATE;
    DEFINE Vconteo int;
    DEFINE Vmin_dif int;
    DEFINE Vprom_min DECIMAL(18,2);
    
	DEFINE iSqlErr INT;
    DEFINE iIsamErr INT;
    DEFINE cInfoErr CHAR;

    DEFINE vFecha_alta char(10);
    DEFINE vMaximo_minuto int;
    DEFINE vHora_maximo char(5);
    

	
	LET cCodRet = "00000";
	LET cMensaje = 'PROCESO EXITOSO';
	
    LET usuario='informix';    
    LET Vsp ='sp_estadisticas_sac_remesas_minuto';
    LET Vid_sp = '5';
    LET Vperiodo = fechaInicio || ' a ' || fechaFin;


    LET Vfecha_insert='';
    LET Vconteo =0;
    LET Vmin_dif =0;
    LET Vprom_min =0;

	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;

--	SET DEBUG FILE TO '/ifxsif01/Control-M/sp_estadisticas_sac_remesas_minuto.out';
--	TRACE ON;

	BEGIN
		ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
			IF iSqlErr <> 0 THEN
                LET cCodRet = iSqlErr;
				LET cMensaje = "ERROR";

				INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022
				RETURN cCodRet, cMensaje;
			END IF;
		END EXCEPTION;
    		
		DROP TABLE IF EXISTS TB_SAC_REMESAS_MINUTO_TMP;
		
		TRUNCATE TABLE "informix".sac_estadisticas_temp_remesas_minuto;

			CREATE TEMP TABLE TB_SAC_REMESAS_MINUTO_TMP ( 
													fecha             	DATE,
													conteo_tot        	INTEGER,
													min_dif_tot       	INTEGER,
													prom_mint_tot     	DECIMAL(15,5),
													conteo_bts        	INTEGER,
													min_dif_bts       	INTEGER,
													prom_min_bts      	DECIMAL(15,5),
													conteo_wu         	INTEGER,
													min_dif_wu        	INTEGER,
													prom_min_wu       	DECIMAL(15,5),
													conteo_appriza      INTEGER,
													min_dif_appriza     INTEGER,
													prom_min_appriza    DECIMAL(15,5),
													num_max_pagos_tot 	INTEGER,
													hora_max_pagos_tot	VARCHAR(5),
													num_max_pagos_bts 	INTEGER,
													hora_max_pagos_bts	VARCHAR(5),
													num_max_pagos_wu  	INTEGER,
													hora_max_pagos_wu 	VARCHAR(5),
													num_max_pagos_appriza 	INTEGER,
													hora_max_pagos_appriza	VARCHAR(5),
													user_insert       	VARCHAR(25),
													fecha_insert      	DATETIME YEAR to FRACTION(3),
													sp                	VARCHAR(50),
													periodo           	VARCHAR(25) 
													) WITH NO LOG;
			
              insert into informix.sac_estadisticas_temp_remesas_minuto
              select {+INDEX (bdisac:"informix".sac_movimientos.idxsac_mov4)} fecha_pago, fecha_insert, numcategoria, numconvenio from bdisac:"informix".sac_movimientos 
              where status_cancelado <> 'S' and
			  ((numcategoria='07' and numconvenio='004') OR (numcategoria='07' and numconvenio='006') OR (numcategoria='07' and numconvenio='007')
			  OR (numcategoria='07' and numconvenio='008') OR (numcategoria='07' and numconvenio='009')) 
			  and date(fecha_pago) between fechaInicio and fechaFin 
			  and substr(fecha_insert, 12, 5) between '10:00' and '20:00';

              insert into informix.sac_estadisticas_temp_remesas_minuto
              select {+INDEX (bdisac:"informix".sac_movimientoshistorial.idxsac_movhis235)} fecha_pago, fecha_insert, numcategoria, numconvenio from bdisac:"informix".sac_movimientoshistorial
              where status_cancelado <> 'S' and
			  ((numcategoria='07' and numconvenio='004') OR (numcategoria='07' and numconvenio='006') OR (numcategoria='07' and numconvenio='007')
			  OR (numcategoria='07' and numconvenio='008') OR (numcategoria='07' and numconvenio='009')) 
			  and date(fecha_pago) between fechaInicio and fechaFin 
			  and substr(fecha_insert, 12, 5) between '10:00' and '20:00';

              insert into informix.sac_estadisticas_temp_remesas_minuto
              select {+INDEX (bdisac:"informix".sac_movimientoshistorial_old.idx_sac_movimientoshist3)} fecha_pago, fecha_insert, numcategoria, numconvenio from bdisac:"informix".sac_movimientoshistorial_old
              where status_cancelado <> 'S' and
			  ((numcategoria='07' and numconvenio='004') OR (numcategoria='07' and numconvenio='006') OR (numcategoria='07' and numconvenio='007')
			  OR (numcategoria='07' and numconvenio='008') OR (numcategoria='07' and numconvenio='009')) 
			  and date(fecha_pago) between fechaInicio and fechaFin 
			  and substr(fecha_insert, 12, 5) between '10:00' and '20:00';
           

       --BUSCA E INSERTA TODAS LAS FECHAS DEL PERIODO (MOVIMIENTOS)
			insert into TB_SAC_REMESAS_MINUTO_TMP (fecha, conteo_tot, min_dif_tot, prom_mint_tot, fecha_insert)
						select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, count(*) as total, count(distinct(substr(fecha_insert, 12, 5))) as minutos_dif
								, cast(count(*) / count(distinct(substr(fecha_insert, 12, 5))) as decimal(8,1)) as promXMinuto, current
						from informix.sac_estadisticas_temp_remesas_minuto 
						--where ((numcategoria='07' and numconvenio='004') OR (numcategoria='07' and numconvenio='006') OR (numcategoria='07' and numconvenio='007')
						--OR (numcategoria='07' and numconvenio='008') OR (numcategoria='07' and numconvenio='009')) 
						group by fecha_pago order by fecha_pago asc;

 
			FOREACH SELECT fecha INTO vFecha_alta from TB_SAC_REMESAS_MINUTO_TMP where fecha >= mdy(month(fechaInicio),day(fechaInicio),year(fechaInicio)) and fecha <= mdy(month(fechaFin),day(fechaFin),year(fechaFin))
						LET Vconteo=0;
						LET Vmin_dif=0;
						LET Vprom_min=0;
                                        
        --BUSCA, CUENTA Y ASIGNA PAGOS BTS
					if (select count(*) from (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, count(*) as conteoBTS, count(distinct(substr(fecha_insert, 12, 5))) as min_dif_bts, cast(count(*) / count(distinct(substr(fecha_insert, 12, 5))) as decimal(8,1)) as promXMinuto
												from informix.sac_estadisticas_temp_remesas_minuto where 
														numcategoria='07' and numconvenio='004' 
														and date(fecha_pago) =date(vFecha_alta) group by fecha_pago order by fecha_pago asc
											  ) as t1) > 0 then

						select fecha_pago, conteoBTS, min_dif_bts, promXMinuto  into Vfecha_insert, Vconteo, Vmin_dif, Vprom_min 
						from( select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, count(*) as conteoBTS, count(distinct(substr(fecha_insert, 12, 5))) as min_dif_bts, cast(count(*) / count(distinct(substr(fecha_insert, 12, 5))) as decimal(8,1)) as promXMinuto
												from informix.sac_estadisticas_temp_remesas_minuto where 
														numcategoria='07' and numconvenio='004' 
														and date(fecha_pago) =date(vFecha_alta) group by fecha_pago order by fecha_pago asc) as t2;
					end if;

					UPDATE TB_SAC_REMESAS_MINUTO_TMP SET conteo_bts= Vconteo, min_dif_bts= Vmin_dif, prom_min_bts= Vprom_min, user_insert=usuario WHERE date(fecha)= date(vFecha_alta);
					
						LET Vconteo=0;
						LET Vmin_dif=0;
						LET Vprom_min=0;

          --BUSCA, CUENTA Y ASIGNA PAGOS WESTERN UNION
						if (select count(*) from (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, count(*) as conteoBTS, count(distinct(substr(fecha_insert, 12, 5))) as min_dif_bts, cast(count(*) / count(distinct(substr(fecha_insert, 12, 5))) as decimal(8,1)) as promXMinuto
													from informix.sac_estadisticas_temp_remesas_minuto where 
															((numcategoria='07' and numconvenio='006') OR (numcategoria='07' and numconvenio='007')
															OR (numcategoria='07' and numconvenio='008')) 
															and date(fecha_pago) =date(vFecha_alta) group by fecha_pago order by fecha_pago asc
												  ) as t1) > 0 then

							select fecha_pago, conteoBTS, min_dif_bts, promXMinuto  into Vfecha_insert, Vconteo, Vmin_dif, Vprom_min 
							from( select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, count(*) as conteoBTS, count(distinct(substr(fecha_insert, 12, 5))) as min_dif_bts, cast(count(*) / count(distinct(substr(fecha_insert, 12, 5))) as decimal(8,1)) as promXMinuto
													from informix.sac_estadisticas_temp_remesas_minuto where 
															((numcategoria='07' and numconvenio='006') OR (numcategoria='07' and numconvenio='007')
															OR (numcategoria='07' and numconvenio='008')) 
															and date(fecha_pago) =date(vFecha_alta) group by fecha_pago order by fecha_pago asc) as t2;
						end if;

						UPDATE TB_SAC_REMESAS_MINUTO_TMP SET conteo_wu= Vconteo, min_dif_wu= Vmin_dif, prom_min_wu= Vprom_min, user_insert=usuario WHERE date(fecha)= date(vFecha_alta);
		   
						--LET vMaximo_minuto=0;
						--LET vHora_maximo='00:00';
						LET Vconteo=0;
						LET Vmin_dif=0;
						LET Vprom_min=0;
			
			
		  --BUSCA, CUENTA Y ASIGNA PAGOS APPRIZA
					if (select count(*) from (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, count(*) as conteoBTS, count(distinct(substr(fecha_insert, 12, 5))) as min_dif_bts, cast(count(*) / count(distinct(substr(fecha_insert, 12, 5))) as decimal(8,1)) as promXMinuto
												from informix.sac_estadisticas_temp_remesas_minuto where 
														numcategoria='07' and numconvenio='009' 
														and date(fecha_pago) =date(vFecha_alta) group by fecha_pago order by fecha_pago asc
											  ) as t1) > 0 then

						select fecha_pago, conteoBTS, min_dif_bts, promXMinuto  into Vfecha_insert, Vconteo, Vmin_dif, Vprom_min 
						from( select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, count(*) as conteoBTS, count(distinct(substr(fecha_insert, 12, 5))) as min_dif_bts, cast(count(*) / count(distinct(substr(fecha_insert, 12, 5))) as decimal(8,1)) as promXMinuto
												from informix.sac_estadisticas_temp_remesas_minuto where 
														numcategoria='07' and numconvenio='009' 
														and date(fecha_pago) =date(vFecha_alta) group by fecha_pago order by fecha_pago asc) as t2;
					end if;

					UPDATE TB_SAC_REMESAS_MINUTO_TMP SET conteo_appriza= Vconteo, min_dif_appriza= Vmin_dif, prom_min_appriza= Vprom_min, user_insert=usuario WHERE date(fecha)= date(vFecha_alta);
					
						LET Vconteo=0;
						LET Vmin_dif=0;
						LET Vprom_min=0;
						
						LET vMaximo_minuto=0;
						LET vHora_maximo='00:00';

           --BUSCA MAXIMO INDIVIDUAL (TODOS LOS PAGOS)
					  if (select count(*) from (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, count(substr(fecha_insert, 12, 5)) as altasXMinuto, substr(fecha_insert, 12, 5) as minuto
													from informix.sac_estadisticas_temp_remesas_minuto 
													where ((numcategoria='07' and numconvenio='004')
													OR (numcategoria='07' and numconvenio='006')
													OR (numcategoria='07' and numconvenio='007')
													OR (numcategoria='07' and numconvenio='008')
													OR (numcategoria='07' and numconvenio='009'))
													and date(fecha_pago) =date(vFecha_alta)
													group by 1,3 
													having count(substr(fecha_insert, 12, 5)) = 
														(
															SELECT MAX(A.altasXMinutos) 
																FROM (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, substr(fecha_insert, 12, 5) as minuto, count(substr(fecha_insert, 12, 5)) as altasXMinutos
																		from informix.sac_estadisticas_temp_remesas_minuto 
																		where ((numcategoria='07' and numconvenio='004')
																		OR (numcategoria='07' and numconvenio='006')
																		OR (numcategoria='07' and numconvenio='007')
																		OR (numcategoria='07' and numconvenio='008')
																		OR (numcategoria='07' and numconvenio='009'))
																		and date(fecha_pago) =date(vFecha_alta)
																		group by 1,2  
																	) AS A)
												)as t1) > 0 then
														
											select first 1 altasXMinuto, minuto into vMaximo_minuto, vHora_maximo from (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, altasXMinuto, minuto from (select fecha_pago, count(substr(fecha_insert, 12, 5)) as altasXMinuto, substr(fecha_insert, 12, 5) as minuto
													from informix.sac_estadisticas_temp_remesas_minuto 
													where ((numcategoria='07' and numconvenio='004')
													OR (numcategoria='07' and numconvenio='006')
													OR (numcategoria='07' and numconvenio='007')
													OR (numcategoria='07' and numconvenio='008')
													OR (numcategoria='07' and numconvenio='009'))
													and date(fecha_pago) =date(vFecha_alta)
													group by 1,3 
													having count(substr(fecha_insert, 12, 5)) = 
														(
															SELECT MAX(A.altasXMinutos) 
																FROM (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, substr(fecha_insert, 12, 5) as minuto, count(substr(fecha_insert, 12, 5)) as altasXMinutos
																		from informix.sac_estadisticas_temp_remesas_minuto 
																		where ((numcategoria='07' and numconvenio='004')
																		OR (numcategoria='07' and numconvenio='006')
																		OR (numcategoria='07' and numconvenio='007')
																		OR (numcategoria='07' and numconvenio='008')
																		OR (numcategoria='07' and numconvenio='009'))
																		and date(fecha_pago) =date(vFecha_alta)
																		group by 1,2  
																	) AS A)) as t3 order by 3 desc) as tb1;

						end if

						UPDATE TB_SAC_REMESAS_MINUTO_TMP SET num_max_pagos_tot=vMaximo_minuto, hora_max_pagos_tot=vHora_maximo WHERE fecha= vFecha_alta;
		 
							LET vMaximo_minuto=0;                                    
							LET vHora_maximo='00:00';
		   
		   --BUSCA MAXIMO INDIVIDUAL (BTS)
					  if (select count(*) from (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, count(substr(fecha_insert, 12, 5)) as altasXMinuto, substr(fecha_insert, 12, 5) as minuto
													from informix.sac_estadisticas_temp_remesas_minuto 
													where numcategoria='07' and numconvenio='004'
													and date(fecha_pago) =date(vFecha_alta)
													group by 1,3 
													having count(substr(fecha_insert, 12, 5)) = 
														(
															SELECT MAX(A.altasXMinutos) 
																FROM (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, substr(fecha_insert, 12, 5) as minuto, count(substr(fecha_insert, 12, 5)) as altasXMinutos
																		from informix.sac_estadisticas_temp_remesas_minuto 
																		where numcategoria='07' and numconvenio='004'
																		and date(fecha_pago) =date(vFecha_alta)
																		group by 1,2  
																	) AS A)
												)as t1) > 0 then
						 
							select first 1 altasXMinuto, minuto into vMaximo_minuto, vHora_maximo from (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, altasXMinuto, minuto from (select fecha_pago, count(substr(fecha_insert, 12, 5)) as altasXMinuto, substr(fecha_insert, 12, 5) as minuto
									from informix.sac_estadisticas_temp_remesas_minuto 
									where numcategoria='07' and numconvenio='004'
									and date(fecha_pago) =date(vFecha_alta)
									group by 1,3 
									having count(substr(fecha_insert, 12, 5)) = 
										(
											SELECT MAX(A.altasXMinutos) 
												FROM (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, substr(fecha_insert, 12, 5) as minuto, count(substr(fecha_insert, 12, 5)) as altasXMinutos
														from informix.sac_estadisticas_temp_remesas_minuto 
														where numcategoria='07' and numconvenio='004'
														and date(fecha_pago) =date(vFecha_alta)
														group by 1,2  
													) AS A)) as t5 order by 3 desc) as tb2;

						end if

						UPDATE TB_SAC_REMESAS_MINUTO_TMP SET num_max_pagos_bts=vMaximo_minuto, hora_max_pagos_bts=vHora_maximo WHERE fecha= vFecha_alta;

						LET vMaximo_minuto=0;                                    
						LET vHora_maximo='00:00';
          --BUSCA MAXIMO INDIVIDUAL (WESTERN UNION)
				  if (select count(*) from (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, count(substr(fecha_insert, 12, 5)) as altasXMinuto, substr(fecha_insert, 12, 5) as minuto
												from informix.sac_estadisticas_temp_remesas_minuto 
												where ((numcategoria='07' and numconvenio='006')
												OR (numcategoria='07' and numconvenio='007')
												OR (numcategoria='07' and numconvenio='008'))
												and date(fecha_pago) =date(vFecha_alta)
												group by 1,3 
												having count(substr(fecha_insert, 12, 5)) = 
													(
														SELECT MAX(A.altasXMinutos) 
															FROM (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, substr(fecha_insert, 12, 5) as minuto, count(substr(fecha_insert, 12, 5)) as altasXMinutos
																	from informix.sac_estadisticas_temp_remesas_minuto 
																	where ((numcategoria='07' and numconvenio='006')
																	OR (numcategoria='07' and numconvenio='007')
																	OR (numcategoria='07' and numconvenio='008'))
																	and date(fecha_pago) =date(vFecha_alta)
																	group by 1,2  
																) AS A)
											)as t1) > 0 then
					 
										select first 1 altasXMinuto, minuto into vMaximo_minuto, vHora_maximo from (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, altasXMinuto, minuto from (select fecha_pago, count(substr(fecha_insert, 12, 5)) as altasXMinuto, substr(fecha_insert, 12, 5) as minuto
												from informix.sac_estadisticas_temp_remesas_minuto 
												where ((numcategoria='07' and numconvenio='006')
												OR (numcategoria='07' and numconvenio='007')
												OR (numcategoria='07' and numconvenio='008'))
												and date(fecha_pago) =date(vFecha_alta)
												group by 1,3 
												having count(substr(fecha_insert, 12, 5)) = 
													(
														SELECT MAX(A.altasXMinutos) 
															FROM (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, substr(fecha_insert, 12, 5) as minuto, count(substr(fecha_insert, 12, 5)) as altasXMinutos
																	from informix.sac_estadisticas_temp_remesas_minuto 
																	where ((numcategoria='07' and numconvenio='006')
																	OR (numcategoria='07' and numconvenio='007')
																	OR (numcategoria='07' and numconvenio='008'))
																	and date(fecha_pago) =date(vFecha_alta)
																	group by 1,2  
																) AS A)) as t6 order by 3 desc) as tb3;
					end if

					UPDATE TB_SAC_REMESAS_MINUTO_TMP SET num_max_pagos_wu=vMaximo_minuto, hora_max_pagos_wu=vHora_maximo, sp=TRIM(Vsp), periodo=Vperiodo WHERE fecha= vFecha_alta;
						
						LET vMaximo_minuto=0;                                    
						LET vHora_maximo='00:00';
			
			--BUSCA MAXIMO INDIVIDUAL (APPRIZA)
					  if (select count(*) from (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, count(substr(fecha_insert, 12, 5)) as altasXMinuto, substr(fecha_insert, 12, 5) as minuto
													from informix.sac_estadisticas_temp_remesas_minuto 
													where numcategoria='07' and numconvenio='009'
													and date(fecha_pago) =date(vFecha_alta)
													group by 1,3 
													having count(substr(fecha_insert, 12, 5)) = 
														(
															SELECT MAX(A.altasXMinutos) 
																FROM (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, substr(fecha_insert, 12, 5) as minuto, count(substr(fecha_insert, 12, 5)) as altasXMinutos
																		from informix.sac_estadisticas_temp_remesas_minuto 
																		where numcategoria='07' and numconvenio='009'
																		and date(fecha_pago) =date(vFecha_alta)
																		group by 1,2  
																	) AS A)
												)as t1) > 0 then
						 
							select first 1 altasXMinuto, minuto into vMaximo_minuto, vHora_maximo from (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, altasXMinuto, minuto from (select fecha_pago, count(substr(fecha_insert, 12, 5)) as altasXMinuto, substr(fecha_insert, 12, 5) as minuto
									from informix.sac_estadisticas_temp_remesas_minuto 
									where numcategoria='07' and numconvenio='009'
									and date(fecha_pago) =date(vFecha_alta)
									group by 1,3 
									having count(substr(fecha_insert, 12, 5)) = 
										(
											SELECT MAX(A.altasXMinutos) 
												FROM (select {+INDEX (sac_estadisticas_temp_remesas_minuto."informix".idx_id_sucursal_minuto)} fecha_pago, substr(fecha_insert, 12, 5) as minuto, count(substr(fecha_insert, 12, 5)) as altasXMinutos
														from informix.sac_estadisticas_temp_remesas_minuto 
														where numcategoria='07' and numconvenio='009'
														and date(fecha_pago) =date(vFecha_alta)
														group by 1,2  
													) AS A)) as t5 order by 3 desc) as tb2;

						end if

						UPDATE TB_SAC_REMESAS_MINUTO_TMP SET num_max_pagos_appriza=vMaximo_minuto, hora_max_pagos_appriza=vHora_maximo WHERE fecha= vFecha_alta;

						
             END FOREACH;
					
					INSERT INTO "informix".sac_estadisticas_remesas_minuto
						SELECT * FROM TB_SAC_REMESAS_MINUTO_TMP;

				   DROP TABLE IF EXISTS TB_SAC_REMESAS_MINUTO_TMP;

				   INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022
				   
		RETURN cCodRet, cMensaje;
	END;
END PROCEDURE
DOCUMENT
'AUTOR: Noe Medina Ramirez ',
'DESCRIPCIÃN: Genera la informaciÃ³n para los Reportes Periodicos de Pago de REMESAS',
'SUSTENTO: ',
'EJECUTADO O LLAMADO POR: ',
'FECHA: 17/10/2016 ',
'ACTUALIZACION:',
'FECHA: 30/11/2016 ',
'SE QUITA EL TRUNCATE DE LA TABLA DE ESTADISTICAS',
'ACTUALIZACION:',
'FECHA: 05/12/2016 ',
'SE CAMBIA EL FORMATO DE FECHAS A MDY',
'VERSIÃN:  ',
'Solicita: Jaime GonzÃ¡lez',
'BD: bdisac';

CREATE PROCEDURE "informix".sp_estadisticas_sac_remesas_sucursal
( fechaInicio date, fechaFin date )

RETURNING
	CHAR(5) AS codigo_respuesta,
	CHAR(80) AS mensaje_respuesta;

	DEFINE cCodRet CHAR(5);
	DEFINE cMensaje CHAR(80);
	
    DEFINE usuario CHAR(9);
    DEFINE Vsp CHAR(100);
    DEFINE Vid_sp INTEGER;
    DEFINE Vperiodo CHAR(25);
    DEFINE VtotalPagos int;
    DEFINE Vimporte DECIMAL(18,2);
    DEFINE VimportePromedio DECIMAL(18,2);
    DEFINE Vsucursal char(4);
	DEFINE Vmes INTEGER;
	DEFINE Vanio INTEGER;
	DEFINE iSqlErr INTEGER;
    DEFINE iIsamErr INTEGER;
    DEFINE cInfoErr CHAR;

	LET cCodRet = "00000";
	LET cMensaje = 'PROCESO EXITOSO';
	
    LET usuario='informix';   
    LET Vsp ='sp_estadisticas_sac_remesas_sucursal';
    LET Vid_sp = '6';
    LET Vperiodo = fechaInicio || ' a ' || fechaFin; 
    LET Vsucursal='';
	
    
    LET VtotalPagos =0;
    LET Vimporte =0;
    LET VimportePromedio =0;

	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;

--	SET DEBUG FILE TO '/ifxsif01/Control-M/sp_estadisticas_sac_remesas_sucursal.out';
--	TRACE ON;

	BEGIN
		ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
			IF iSqlErr <> 0 THEN
                LET cCodRet = iSqlErr;
				LET cMensaje = "ERROR";
				INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022
				RETURN cCodRet, cMensaje;
			END IF;
		END EXCEPTION;

        DROP TABLE IF EXISTS TB_SAC_MOVIMIENTOS_SUCURSAL_TMP;
		DROP TABLE IF EXISTS TB_SAC_REMESAS_SUCURSAL_TMP;

			CREATE TEMP TABLE TB_SAC_MOVIMIENTOS_SUCURSAL_TMP ( 
                                                        id_sucursal               	CHAR(4),
                                                        numcategoria              	CHAR(2),
                                                        numconvenio               	CHAR(5),
                                                        referencia1               	CHAR(40),
                                                        referencia2               	CHAR(40),
                                                        forma_pago                	CHAR(1),
                                                        importe_pago              	MONEY,
                                                        importe_comision_convenio 	MONEY,
                                                        iva_comision_convenio     	MONEY,
                                                        importe_comision_cte      	MONEY,
                                                        iva_comision_cte          	MONEY,
                                                        cuenta_cargo              	CHAR(12),
                                                        usuario                   	CHAR(8),
                                                        folio_suc                 	CHAR(16),
                                                        transacc_suc              	CHAR(4),
                                                        flag_confirmacion_central 	SMALLINT,
                                                        flag_confirmacion_sucursal	SMALLINT,
                                                        fecha_pago                	DATE,
                                                        fecha_insert              	DATETIME YEAR to FRACTION(3),
                                                        status_cancelado          	CHAR(1),
                                                        origen                    	CHAR(4) DEFAULT '',
                                                        sucursal_cpl              	CHAR(4) DEFAULT '',
                                                        caja_cpl                  	CHAR(3) DEFAULT '',
                                                        transaccion               	CHAR(5) DEFAULT '',
                                                        hora                      	CHAR(6) DEFAULT '',
                                                        folio_operacion           	CHAR(18) DEFAULT '',
                                                        referencia3               	CHAR(40) DEFAULT '',
                                                        referencia4               	CHAR(40) DEFAULT ''
                                                        ) WITH NO LOG; 
			EXECUTE IMMEDIATE  'CREATE  INDEX idx_id_sucursal ON TB_SAC_MOVIMIENTOS_SUCURSAL_TMP(id_sucursal,fecha_insert)';
			
			
			CREATE TEMP TABLE TB_SAC_REMESAS_SUCURSAL_TMP ( 
														mes 						INTEGER,
														anio 						INTEGER,
														mesanio 					VARCHAR(20),
														suc                 		VARCHAR(4),
														cve_estado 					CHAR(2),
														estados             		VARCHAR(30),
														num_pagos_tot       		INTEGER,
														importe_tot         		DECIMAL(15,5),
														importe_promedio_tot		DECIMAL(15,5),
														num_pagos_bts       		INTEGER,
														importe_bts         		DECIMAL(15,5),
														importe_promedio_bts		DECIMAL(15,5),
														num_pagos_wu        		INTEGER,
														importe_wu          		DECIMAL(15,5),
														importe_promedio_wu 		DECIMAL(15,5),
														num_pagos_appriza        	INTEGER,
														importe_appriza          	DECIMAL(15,5),
														importe_promedio_appriza	DECIMAL(15,5),
														user_insert         		VARCHAR(25),
														fecha_insert        		DATETIME YEAR to FRACTION(3),
														sp                  		VARCHAR(50),
														periodo             		VARCHAR(25) 
														) WITH NO LOG;
			
              INSERT INTO TB_SAC_MOVIMIENTOS_SUCURSAL_TMP
              SELECT * FROM bdisac:"informix".sac_movimientos
              WHERE status_cancelado <> 'S' 
			  AND DATE(fecha_pago) >= MDY(MONTH(fechaInicio),DAY(fechaInicio),YEAR(fechaInicio))
			  AND DATE(fecha_pago) <= MDY(MONTH(fechaFin),DAY(fechaFin),YEAR(fechaFin))
			  AND numcategoria='07' AND numconvenio IN ('004','006','007','008','009');

              INSERT INTO TB_SAC_MOVIMIENTOS_SUCURSAL_TMP
              SELECT * FROM bdisac:"informix".sac_movimientoshistorial
              WHERE status_cancelado <> 'S' 
			  AND DATE(fecha_pago) >= MDY(MONTH(fechaInicio),DAY(fechaInicio),YEAR(fechaInicio))
			  AND DATE(fecha_pago) <= MDY(MONTH(fechaFin),DAY(fechaFin),YEAR(fechaFin))
			  AND numcategoria='07' AND numconvenio IN ('004','006','007','008','009');

              INSERT INTO TB_SAC_MOVIMIENTOS_SUCURSAL_TMP
              SELECT * FROM bdisac:"informix".sac_movimientoshistorial_old
              WHERE status_cancelado <> 'S' 
			  AND DATE(fecha_pago) >= MDY(MONTH(fechaInicio),DAY(fechaInicio),YEAR(fechaInicio))
			  AND DATE(fecha_pago) <= MDY(MONTH(fechaFin),DAY(fechaFin),YEAR(fechaFin))
			  AND numcategoria='07' AND numconvenio IN ('004','006','007','008','009');
              
       --BUSCA E INSERTA TODAS LAS FECHAS DEL PERIODO (MOVIMIENTOS)
                 INSERT INTO TB_SAC_REMESAS_SUCURSAL_TMP (mes, anio, mesanio, suc, cve_estado, estados, num_pagos_tot, importe_tot, importe_promedio_tot, fecha_insert)
                                        SELECT MONTH(DATE(M.fecha_pago)) AS mes
												, YEAR(DATE(M.fecha_pago)) AS anio
												, DECODE(MONTH(DATE(M.fecha_pago)),1,'Enero',2,'Febrero',3,'Marzo',4,'Abril',5,'Mayo',6,'Junio',7,'Julio',8,'Agosto',9,'Septiembre',10,'Octubre',11,'Noviembre',12,'Diciembre') || ' ' || YEAR(DATE(M.fecha_pago)) AS mesanio
												, S.sucursal AS Suc
												, e.estado AS cve_edo
												, e.nombre AS estado
                                                , COUNT(*) AS num_pagos
												, SUM(M.importe_pago) AS importe
												, CAST(AVG(M.importe_pago) AS decimal(18,2)) AS imprt_promedio
												, CURRENT
                                                FROM TB_SAC_MOVIMIENTOS_SUCURSAL_TMP M {+INDEX (idx_id_sucursal)}, bdinteg:"informix".si_sucursales S {+INDEX (idx_sucursal)}, bdinteg:"informix".si_estados E {+INDEX (idx_nombre_estado)}
                                                WHERE M.forma_pago IN('1','2','4','5')
                                                --AND S.Sucursal not in('9250')
                                                AND M.id_sucursal=S.Sucursal
                                                AND E.estado=S.estado
                                                GROUP BY 2,1,3,5,4,6
												ORDER BY 2,1,3,5,4,6 ASC;
                 
				FOREACH SELECT mes, anio, suc INTO Vmes, Vanio, Vsucursal FROM TB_SAC_REMESAS_SUCURSAL_TMP GROUP BY 2,1,3
								LET VtotalPagos=0;
								LET Vimporte=0;
								LET VimportePromedio=0;
								
		--BUSCA, CUENTA Y ASIGNA PAGOS BTS
					IF (SELECT COUNT(*) FROM (
										  SELECT S.sucursal AS Suc, e.nombre AS estado,
												COUNT(*) AS num_pagos, SUM(M.importe_pago) AS importe, CAST(AVG(M.importe_pago) AS DECIMAL(18,2)) AS imprt_promedio
										  FROM TB_SAC_MOVIMIENTOS_SUCURSAL_TMP M {+INDEX (idx_id_sucursal)}, bdinteg:"informix".si_sucursales S {+INDEX (idx_sucursal)}, bdinteg:"informix".si_estados E {+INDEX (idx_nombre_estado)}
										  WHERE S.sucursal = TRIM(Vsucursal) AND MONTH(M.fecha_pago)= Vmes AND YEAR(M.fecha_pago)= Vanio
												AND M.numconvenio='004'
												AND M.forma_pago IN ('1','2','4','5')
												--AND S.Sucursal not in('9250')
												AND M.id_sucursal=S.Sucursal
												AND E.estado=S.estado
												GROUP BY 1,2
												ORDER BY suc ASC
											) AS t2) > 0 THEN

						SELECT num_pagos, importe, imprt_promedio INTO VtotalPagos, Vimporte, VimportePromedio 
									FROM ( SELECT S.sucursal AS Suc, e.nombre AS estado,
												COUNT(*) AS num_pagos, SUM(M.importe_pago) AS importe, CAST(AVG(M.importe_pago) AS DECIMAL(18,2)) AS imprt_promedio
										  FROM TB_SAC_MOVIMIENTOS_SUCURSAL_TMP M {+INDEX (idx_id_sucursal)}, bdinteg:"informix".si_sucursales S {+INDEX (idx_sucursal)}, bdinteg:"informix".si_estados E {+INDEX (idx_nombre_estado)}
										  WHERE S.sucursal = TRIM(Vsucursal) AND MONTH(M.fecha_pago)= Vmes AND YEAR(M.fecha_pago)= Vanio
												AND M.numconvenio='004'
												AND M.forma_pago IN('1','2','4','5')
												--AND S.Sucursal not in('9250')
												AND M.id_sucursal=S.Sucursal
												AND E.estado=S.estado
												GROUP BY 1,2
												ORDER BY suc ASC  
										  ) AS t3;
					END IF;

					UPDATE TB_SAC_REMESAS_SUCURSAL_TMP SET num_pagos_bts= VtotalPagos, importe_bts= Vimporte, importe_promedio_bts= VimportePromedio, user_insert=usuario WHERE suc= TRIM(Vsucursal) AND mes= Vmes AND anio= Vanio;

						LET VtotalPagos=0;
						LET Vimporte=0;
						LET VimportePromedio=0;

		--BUSCA, CUENTA Y ASIGNA PAGOS WESTERN UNION
					IF (SELECT COUNT(*) FROM (
										  SELECT S.sucursal AS Suc, e.nombre AS estado,
												COUNT(*) AS num_pagos, SUM(M.importe_pago) AS importe, CAST(AVG(M.importe_pago) AS DECIMAL(18,2)) AS imprt_promedio
										  FROM TB_SAC_MOVIMIENTOS_SUCURSAL_TMP M {+INDEX (idx_id_sucursal)}, bdinteg:"informix".si_sucursales S {+INDEX (idx_sucursal)}, bdinteg:"informix".si_estados E {+INDEX (idx_nombre_estado)}
										  WHERE S.sucursal = TRIM(Vsucursal) AND MONTH(M.fecha_pago)= Vmes AND YEAR(M.fecha_pago)= Vanio
												AND M.numconvenio IN ('006','007','008')
												AND M.forma_pago IN ('1','2','4','5')
												--AND S.Sucursal not in('9250')
												AND M.id_sucursal=S.Sucursal
												AND E.estado=S.estado
												GROUP BY 1,2
												ORDER BY suc ASC
											) AS t4) > 0 THEN

						SELECT num_pagos, importe, imprt_promedio INTO VtotalPagos, Vimporte, VimportePromedio 
									FROM ( SELECT S.sucursal AS Suc, e.nombre AS estado,
												COUNT(*) AS num_pagos, SUM(M.importe_pago) AS importe, CAST(AVG(M.importe_pago) AS DECIMAL(18,2)) AS imprt_promedio
										  FROM TB_SAC_MOVIMIENTOS_SUCURSAL_TMP M {+INDEX (idx_id_sucursal)}, bdinteg:"informix".si_sucursales S {+INDEX (idx_sucursal)}, bdinteg:"informix".si_estados E {+INDEX (idx_nombre_estado)}
										  WHERE S.sucursal = TRIM(Vsucursal) AND MONTH(M.fecha_pago)= Vmes AND YEAR(M.fecha_pago)= Vanio
												AND M.numconvenio IN ('006','007','008')
												AND M.forma_pago IN ('1','2','4','5')
												--AND S.Sucursal not in('9250')
												AND M.id_sucursal=S.Sucursal
												AND E.estado=S.estado
												GROUP BY 1,2
												ORDER BY suc ASC  
										  ) AS t5;
					end if;

					UPDATE TB_SAC_REMESAS_SUCURSAL_TMP SET num_pagos_wu= VtotalPagos, importe_wu= Vimporte, importe_promedio_wu= VimportePromedio, user_insert=usuario, sp=TRIM(Vsp), periodo=Vperiodo WHERE suc= TRIM(Vsucursal) AND mes= Vmes AND anio= Vanio;
					
						LET VtotalPagos=0;
						LET Vimporte=0;
						LET VimportePromedio=0;

		--BUSCA, CUENTA Y ASIGNA PAGOS APPRIZA
					IF (SELECT COUNT(*) FROM (
										  SELECT S.sucursal AS Suc, e.nombre AS estado,
												COUNT(*) AS num_pagos, SUM(M.importe_pago) AS importe, CAST(AVG(M.importe_pago) AS DECIMAL(18,2)) AS imprt_promedio
										  FROM TB_SAC_MOVIMIENTOS_SUCURSAL_TMP M {+INDEX (idx_id_sucursal)}, bdinteg:"informix".si_sucursales S {+INDEX (idx_sucursal)}, bdinteg:"informix".si_estados E {+INDEX (idx_nombre_estado)}
										  WHERE S.sucursal = TRIM(Vsucursal) AND MONTH(M.fecha_pago)= Vmes AND YEAR(M.fecha_pago)= Vanio
												AND M.numconvenio='009'
												AND M.forma_pago IN ('1','2','4','5')
												--AND S.Sucursal not in('9250')
												AND M.id_sucursal=S.Sucursal
												AND E.estado=S.estado
												GROUP BY 1,2
												ORDER BY suc ASC
											) AS t6) > 0 THEN

						SELECT num_pagos, importe, imprt_promedio INTO VtotalPagos, Vimporte, VimportePromedio 
									FROM ( SELECT S.sucursal AS Suc, e.nombre AS estado,
												COUNT(*) AS num_pagos, SUM(M.importe_pago) AS importe, CAST(AVG(M.importe_pago) AS DECIMAL(18,2)) AS imprt_promedio
										  FROM TB_SAC_MOVIMIENTOS_SUCURSAL_TMP M {+INDEX (idx_id_sucursal)}, bdinteg:"informix".si_sucursales S {+INDEX (idx_sucursal)}, bdinteg:"informix".si_estados E {+INDEX (idx_nombre_estado)}
										  WHERE S.sucursal = TRIM(Vsucursal) AND MONTH(M.fecha_pago)= Vmes AND YEAR(M.fecha_pago)= Vanio
												AND M.numconvenio='009'
												AND M.forma_pago IN ('1','2','4','5')
												--AND S.Sucursal not in('9250')
												AND M.id_sucursal=S.Sucursal
												AND E.estado=S.estado
												GROUP BY 1,2
												ORDER BY suc ASC  
										  ) AS t7;
					END IF;
					
					UPDATE TB_SAC_REMESAS_SUCURSAL_TMP SET num_pagos_appriza= VtotalPagos, importe_appriza= Vimporte, importe_promedio_appriza= VimportePromedio, user_insert=usuario, sp=TRIM(Vsp), periodo=Vperiodo WHERE suc= trim(Vsucursal) AND mes= Vmes AND anio= Vanio;

				END FOREACH;
						
				INSERT INTO "informix".sac_estadisticas_remesas_sucursal
					SELECT * FROM TB_SAC_REMESAS_SUCURSAL_TMP;

			   DROP TABLE IF EXISTS TB_SAC_MOVIMIENTOS_SUCURSAL_TMP;
			   DROP TABLE IF EXISTS TB_SAC_REMESAS_SUCURSAL_TMP;
			   
			   INSERT INTO "informix".tb_ejecucion_estadisticas_sac(fecha_hora_ejecucion, id_sp, resultado, periodo, usuario_insert, fecha_insert) 
                VALUES(current, Vid_sp, cCodRet, Vperiodo, usuario, current); -- Actualizado RALVAREZ 10/10/2022
			RETURN cCodRet, cMensaje;
	END;
END PROCEDURE
DOCUMENT
'AUTOR: Noe Medina Ramirez ',
'DESCRIPCIÃN: Genera la informaciÃ³n para los Reportes Periodicos de Pago de REMESAS POR SUCURSAL',
'SUSTENTO: ',
'EJECUTADO O LLAMADO POR: ',
'FECHA: 18/10/2016 ',
'ACTUALIZACION:',
'FECHA: 30/11/2016 ',
'SE QUITA EL TRUNCATE DE LA TABLA DE ESTADISTICAS',
'ACTUALIZACION:',
'FECHA: 05/12/2016 ',
'SE CAMBIA EL FORMATO DE FECHAS A MDY',
'VERSIÃN:  ',
'Solicita: Jaime GonzÃ¡lez',
'BD: bdisac';

CREATE PROCEDURE "informix".sp_dinya_insdatoenv
	(pNo_Control CHAR(12),
	 pUsua_envio CHAR(8),
	 pSuc_origen CHAR(4),
	 pImporte_total MONEY(16,2),
	 pImporte_pago MONEY(16,2),
	 pImporte_envio MONEY(16,2),
	 pComision MONEY(16,2),
	 pIva MONEY(16,2),
	 pNum_ident CHAR(25),
	 pNombre1Rem CHAR(26),
	 pNombre2Rem CHAR(26),
	 pApellido1Rem CHAR(26),
	 pApellido2Rem CHAR(26),
	 pTelefono_rem CHAR(20),
	 pDirec_rem	CHAR(80),
	 pNombre1Ben CHAR(26),
	 pNombre2Ben CHAR(26),
	 pApellido1Ben CHAR(26),
	 pApellido2Ben CHAR(26),
	 pTelefono_ben CHAR(20),
	 pDirec_ben CHAR(80),
	 pMensaje CHAR(100),
	 pForma_pago CHAR(1),
	 pCuentaCargo CHAR(20),
	 pEfectivo MONEY(16,2),
	 pCargoCta MONEY(16,2),
	 pFolioSuc CHAR(16),
	 pTransacc_suc CHAR(4)
	 )
RETURNING  CHAR(5),CHAR(5);

DEFINE cCodRet 			CHAR(5);
DEFINE cFijo			CHAR(5);
DEFINE iSqlErr			INTEGER;
DEFINE dFecha_envio		DATE;
DEFINE iSam_error		INTEGER;
DEFINE cMensaje			CHAR(50);
DEFINE mSdo_disponible  MONEY(16,2);
DEFINE vTipoProducto	INTEGER;

LET cCodRet 			= '00000';
LET cFijo				= '00000';
LET iSqlErr				= 0;
LET dFecha_envio		= '';
LET iSam_error			= 0;
LET cMensaje			= '';
LET mSdo_disponible 	= 0;
LET vTipoProducto		= 0;


BEGIN

	ON EXCEPTION SET iSqlErr,iSam_error,cMensaje
		IF iSqlErr != 0 THEN
			LET cCodRet= iSqlErr;
			INSERT INTO bdisac:"informix".sac_mensajeerror (sql_error, iSam_error, descripcion, origen_error, fecha, fecha_insert)
			VALUES (iSqlErr,iSam_error,cMensaje,'sp_DinYa_InsDatoEnv',dFecha_envio,CURRENT );
			RETURN cFijo,cCodRet;
		END IF;
	END EXCEPTION;
	
	--SET DEBUG FILE TO "/respaldosbd/felipe/sp_DinYa_InsDatoEnv.out";
	--TRACE ON;

	--SET ISOLATION TO CURSOR STABILITY;
    --SET ISOLATION TO COMMITTED READ LAST COMMITTED; 
    --SET ISOLATION COMMITTED READ;
    SET ISOLATION DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	
	IF pNo_Control = '' OR  pUsua_envio = '' OR  pSuc_origen = '' OR  pImporte_total = '' OR  pImporte_total <= '0.00' OR
	pImporte_pago = '' OR pImporte_pago = '0.00' OR pImporte_envio = '' OR pImporte_envio = '0.00' OR pComision = '' OR
	pIva = '' OR  pNombre1Rem = '' OR  pApellido1Rem = '' OR pTelefono_rem = '' OR  pNombre1Ben = '' OR pApellido1Ben = '' 
	OR  pTelefono_ben = '' OR pForma_pago = ''  OR pFolioSuc = '' OR pTransacc_suc = '' THEN

		LET cCodRet = '00001';
		RETURN cFijo,cCodRet;
	END IF;
  
	IF pCargoCta IS NULL OR pCargoCta = '' THEN
		LET pCargoCta = '0.00';
	END IF;

	SELECT fecha_hoy INTO dFecha_envio
	FROM bdisac:"informix".sac_fechas;


--BUSCA TIPOS DE PRODUCTO INVALIDOS PARA ENVIO DE DINERO---- Modifica: Noe Medina Fecha:05-06-2017-----------------------------
	LET vTipoProducto = (select producto from bdicheq:sc_maechq where cuenta=trim(pCuentaCargo));

	IF (select count(*) from bdisac:"informix".sac_param where cod_param='120' and valor like '%' || vTipoProducto || '%') > 0 THEN
		LET cCodRet = '00000';
		LET cFijo	= '00151';
		RETURN cFijo,cCodRet;
	END IF;
-------------------------------------------------------------------------------------------------------------------------------

	IF EXISTS (SELECT {+INDEX (bdisac:"informix".sac_enviosdineroya idxsac_envdinya13_1)} 1 FROM bdisac:"informix".sac_enviosdineroya WHERE no_control = pNo_Control and estatus is not null) THEN
		LET cCodRet = '00002';
		RETURN cFijo,cCodRet;
	END IF;
	
	INSERT INTO bdisac:"informix".sac_enviosdineroya
	(no_control, fecha_envio, estatus, hora_envio, usua_envio, suc_origen, importe_total,
	importe_pago, importe_envio, comision, iva,identificacion, num_ident,pri_nom_rem,
	seg_nom_rem, apell_pat_rem, apell_mat_rem, telefono_rem, direc_rem, pri_nom_ben,
	seg_nom_ben, apell_pat_ben, apell_mat_ben, telefono_ben, direc_ben, mensaje)
	VALUES
	(pNo_Control,dFecha_envio,'00',CURRENT HOUR TO SECOND,pUsua_envio,pSuc_origen,
	pImporte_total,pImporte_pago,pImporte_envio,pComision,pIva,'00',pNum_ident,
	pNombre1Rem,pNombre2Rem,pApellido1Rem,pApellido2Rem,pTelefono_rem,pDirec_rem,pNombre1Ben,
	pNombre2Ben,pApellido1Ben,pApellido2Ben, pTelefono_ben,pDirec_ben,pMensaje);
		
	CALL bdisac:"informix".sp_grabapagoservicio (pSuc_origen,'07','001', LPAD(TRIM(pNo_Control),12,'0'),SUBSTR(LPAD(TRIM(pNo_Control),12,'0'),12,1),
	pForma_pago, pImporte_pago,'0.00','0.00',pComision,pIva,pCuentaCargo,pUsua_envio,pFolioSuc, pTransacc_suc,dFecha_envio)
	RETURNING cCodRet;

	IF cCodRet = 0 THEN
		
		IF pForma_pago = '3' AND pEfectivo = 0  THEN
			LET cCodRet = '00022';
		ELIF pForma_pago = '2' OR pForma_pago = '3' THEN
			SELECT sdo_actual - (sdo_cong + sdo_retenido) AS sdo_disponible
			INTO mSdo_disponible
			FROM bdicheq:"informix".sc_maechq WHERE cuenta = pCuentaCargo;
					
			IF mSdo_disponible < pCargoCta THEN
				LET cCodRet = '00032';
			END IF;
		END IF;
			
		IF cCodRet = 0 THEN
			CALL bdisac:"informix".sp_dinya_InsertaEnvios (pNo_Control,pEfectivo,pCargoCta,pCuentaCargo,pSuc_origen,pFolioSuc)
			RETURNING cCodRet,pFolioSuc;
		END IF;
				

		IF cCodRet <> 0 THEN
			DELETE {+INDEX (bdisac:"informix".sac_enviosdineroya idxsac_envdinya13_1)} FROM bdisac:"informix".sac_enviosdineroya 
			WHERE no_control = pNo_Control and estatus is not null;
				
			DELETE FROM bdisac:"informix".sac_movimientos 
			WHERE id_sucursal = pSuc_origen
			AND numcategoria = '07'
			AND numconvenio = '001'
			AND referencia1 = pNo_Control;
		ELSE
			UPDATE {+INDEX (bdisac:"informix".sac_enviosdineroya idxsac_envdinya13_1)} bdisac:"informix".sac_enviosdineroya 
			SET estatus = '01'
			WHERE no_control = pNo_Control and estatus is not null;
		END IF;
	END IF;

	RETURN cFijo,cCodRet;
END
END PROCEDURE
Document
'DESCRIPCION: Consulta los envios que estan pendientes de realizar su pago',
'AUTOR: Antonio Bastidas',
'FECHA: 03 de Noviembre de 2009',
'BD: BDISAC',
'VERSION: 20091204.1225',
'DESCRIPCION: Se envía el parámetro de dFecha_envio al proceso de sp_grabapagoservicio',
'AUTOR: Antonio Bastidas',
'FECHA: 08 de abril de 2010',
'BD: BDISAC',
'VERSION: 20100804.1728',
'DESCRIPCION: Se valida que el efectivo no vaya en 0 cuando el tipo de pago sea mixto',
'y que cuando sea cargo a cuenta o mixto la cuenta cargo tenga saldo suficiente para realizar el cargo',
'AUTOR: Dulce Ramírez',
'FECHA: 12 de mayo de 2010',
'BD: BDISAC',
'VERSION: 20100512.1154',
'DESCRIPCION: Se retira la direccion del beneficiario como dato obligatorio,se agregan reglas de informix ',
'AUTOR: Felipe Urias',
'FECHA: 19 de octubre de 2011',
'BD: BDISAC',
'VERSION: 20111119.1230';

CREATE PROCEDURE "informix".sp_consulta_datoscteremesa( pNumCte	CHAR(20) )
	RETURNING CHAR(5)	AS cCodRet,
						Char(40)	AS cPrimerNombre,
						Char(40)	AS cSegundoNombre,
						Char(40)	AS cApellidoPaterno,
						Char(40)	AS cApellidoMaterno,
						Date	 	AS dFechaNacimiento,
						Char(3) 	AS cIdNacionalidad,
						Char(3) 	AS cIdPaisNacimiento,
						Char(2) 	AS cIdEstadoNacimiento,
						Char(1) 	AS cSexo,
						Char(2) 	AS cTipoIdentificacion,
						Char(30) 	AS cNoIdentificacion,
						Char(3) 	AS cIdPaisEmision,
						Date	 	AS dFechaVencimiento,
						Char(3) 	AS cIdOcupacion,
						Char(2) 	AS cTipoCte,
						Char(2) 	AS cIdEstado,
						Char(3) 	AS cIdCiudad,
						Char(5) 	AS cIdMunicipio,
						integer 	AS iNumColonia,
						integer 	AS iNumCalle,
						integer 	AS iNumeroCiudad,
						Char(10) 	AS cNumExterior,
						Char(10) 	AS cNumInterior,
						Char(10) 	AS cDepartamento,
						Char(5) 	AS cCodPostal,
						Char(13) 	AS cTelefono,
						Char(13) 	AS cTelefonoCelular,
						Char(3) 	AS cIdPaisDomExt,
						Char(100) 	AS cCorreoElectronico,
						integer 	AS iClavePuesto,
						integer 	AS iClaveSubPuesto;

		--Declaracion de variables
		DEFINE cCodRet 		    		CHAR(5);
		DEFINE iSqlErr					INTEGER;
		DEFINE cTipoCliente				CHAR(1);
		DEFINE cNumCte					CHAR(20);
		
		DEFINE cFecha 					DATE;
		DEFINE cPrimerNombre			Char(40);		
        DEFINE cSegundoNombre           Char(40);
		DEFINE cApellidoPaterno         Char(40);
		DEFINE cApellidoMaterno         Char(40);
		DEFINE dFechaNacimiento     	Date;
		DEFINE cIdNacionalidad          Char(3);
		DEFINE cIdPaisNacimiento		Char(3);
		
		DEFINE cIdEstadoNacimiento      Char(2);
		DEFINE cSexo					Char(1);
		DEFINE cTipoIdentificacion      Char(2);
		DEFINE cNoIdentificacion		Char(30);
		DEFINE cIdPaisEmision			Char(3);
		DEFINE dFechaVencimiento		Date;
		DEFINE cIdOcupacion             Char(3);
		DEFINE cTipoCte					Char(2);
		DEFINE cIdEstado				Char(2);
		DEFINE cIdCiudad				Char(3);
		DEFINE cIdMunicipio				Char(5);
		DEFINE iNumColonia				integer;
		DEFINE iNumCalle				integer;
		DEFINE iNumCiudad				integer;
		DEFINE cNumExterior             Char(10);
		DEFINE cNumInterior             Char(10);
		DEFINE cDepartamento			Char(10);
		DEFINE cCodPostal				Char(5);
		DEFINE cTelefono				Char(13);
		DEFINE cTelefonoCelular         Char(13);
		DEFINE cIdPaisDomExt			Char(3);
		DEFINE cCorreoElectronico       Char(100);
		DEFINE iClavePuesto				integer;
		DEFINE iClaveSubPuesto          integer;
        DEFINE cSec_ingreso             Char(5);
		
		--Inicializacion
		LET cCodRet 				= '00002'; --Codigo 00000 = Exitoso; 00001 = Cliente titular 00002 = No es titular o no existe
		LET iSqlErr					= 0;
		LET cTipoCliente			= '';
		LET dFechaNacimiento		= '01-01-1900';
		LET dFechaVencimiento		= '01-01-1900';
		
		LET cPrimerNombre			= '';
		LET cSegundoNombre			= '';
		LET cApellidoPaterno		= '';
		LET cApellidoMaterno		= '';
		
		LET cIdNacionalidad			= '';
		LET cIdPaisNacimiento		= '';
		LET cIdEstadoNacimiento 	= '';
		LET cSexo					= '';
		LET cTipoIdentificacion 	= '';
		LET cNoIdentificacion		= '';
		LET cIdPaisEmision			= '';
		
		LET cIdOcupacion			= '';
		LET cTipoCte				= '';
		LET cIdEstado				= '';
		LET cIdCiudad				= '';
		LET cIdMunicipio			= '';
		LET iNumColonia				= 0;
		LET iNumCalle				= 0;
		LET iNumCiudad				= 0;
		LET cNumExterior			= '';
		LET cNumInterior			= '';
		LET cDepartamento			= '';
		LET cCodPostal				= '';
		LET cTelefono				= '';
		LET cTelefonoCelular		= '';
		LET cIdPaisDomExt			= '';
		LET cCorreoElectronico		= '';
		
		LET iClavePuesto			= 0;
		LET iClaveSubPuesto			= 0;

        LET cSec_ingreso            = '';

BEGIN
	ON EXCEPTION SET iSqlErr
		IF iSqlErr <> 0 THEN
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cPrimerNombre, cSegundoNombre, cApellidoPaterno,  cApellidoMaterno, dFechaNacimiento, cIdNacionalidad, cIdPaisNacimiento, cIdEstadoNacimiento, 
			              cSexo, cTipoIdentificacion, cNoIdentificacion, cIdPaisEmision, dFechaVencimiento, cIdOcupacion, cTipoCte, cIdEstado, cIdCiudad, cIdMunicipio, iNumColonia, 
						  iNumCalle, iNumCiudad, cNumExterior, cNumInterior, cDepartamento, cCodPostal, cTelefono, cTelefonoCelular, cIdPaisDomExt, cCorreoElectronico, iClavePuesto, iClaveSubPuesto;
		END IF;
	END EXCEPTION;

	--SET DEBUG FILE TO '/informix/EPG/sp_consulta_datoscteremesa.out';
	--TRACE ON;

	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;

	IF NVL(pNumCte, '') = '' THEN
		LET cCodRet = '00002';
		--DATOS VACIOS, ERROR.
		LET iClavePuesto=5;
		LET iClaveSubPuesto=2;
		RETURN cCodRet, cPrimerNombre, cSegundoNombre, cApellidoPaterno,  cApellidoMaterno, dFechaNacimiento, cIdNacionalidad, cIdPaisNacimiento, cIdEstadoNacimiento, 
			   cSexo, cTipoIdentificacion, cNoIdentificacion, cIdPaisEmision, dFechaVencimiento, cIdOcupacion, cTipoCte, cIdEstado, cIdCiudad, cIdMunicipio, iNumColonia, 
			   iNumCalle, iNumCiudad, cNumExterior, cNumInterior, cDepartamento, cCodPostal, cTelefono, cTelefonoCelular, cIdPaisDomExt, cCorreoElectronico, iClavePuesto, iClaveSubPuesto;
	ELSE

		SELECT	nombre1, nombre2, apell_paterno, apell_materno
		INTO	cPrimerNombre, cSegundoNombre, cApellidoPaterno, cApellidoMaterno
		FROM	bdinteg: "informix".si_cliente WHERE numcte = pNumCte;
		
		SELECT	fecha_nac, nacionalidad, sexo, codidentifi, numidentifi, profesion, lugar_nac, id_pais
		INTO	dFechaNacimiento, cIdNacionalidad, cSexo, cTipoIdentificacion, cNoIdentificacion, cIdOcupacion, cIdEstadoNacimiento, cIdPaisNacimiento
		FROM	bdinteg: "informix".si_ctepf WHERE numcte = pNumCte;
		
		SELECT	fecha_vencimiento, tipo_cte, pais_emision
		INTO	dFechaVencimiento, cTipoCte, cIdPaisEmision
		FROM	bdisac: "informix".sac_cte_remesas WHERE numcte = pNumCte;
		
		SELECT	telefono
		INTO	cTelefono
		FROM	bdinteg: "informix".si_telefonos WHERE numcte = pNumCte AND tipo_tel=1 AND status_tel='A'
         AND secuencia  = (SELECT MAX(secuencia) FROM bdinteg: "informix".si_telefonos 
                            WHERE tipo_tel  = 1 AND status_tel='A'
                              AND numcte = pNumCte);
        
        SELECT	telefono
		INTO	cTelefonoCelular
		FROM	bdinteg: "informix".si_telefonos WHERE numcte = pNumCte AND tipo_tel=2 AND status_tel='A'
         AND secuencia  = (SELECT MAX(secuencia) FROM bdinteg: "informix".si_telefonos 
                            WHERE tipo_tel  = 2 AND status_tel='A'
                              AND numcte = pNumCte);
		
		SELECT	correo_elec
		  INTO	cCorreoElectronico
          FROM bdinteg:"informix".si_correos WHERE numcte = pNumCte and status_correo = 'A' 
           AND secuencia = (SELECT max(secuencia) FROM  bdinteg: "informix".si_correos 
                             WHERE numcte = pNumCte and status_correo = 'A');
		
		SELECT	estado, ciudad, municipio, numerocolonia, numerocalle, numerociudad, numeroextcalle, numerointcalle, departamento, cod_postal, pais
		  INTO	cIdEstado, cIdCiudad, cIdMunicipio, iNumColonia, iNumCalle, iNumCiudad, cNumExterior, cNumInterior, cDepartamento,cCodPostal, cIdPaisDomExt
		  FROM	bdinteg: "informix".si_direcciones_actual WHERE tipo_dir = 1 AND numcte = pNumCte;
		
        IF cIdEstado = '9' AND cIdMunicipio = '' THEN
            LET cIdMunicipio = cIdCiudad;
        END IF;

        IF cIdEstado = '09' AND cIdMunicipio = '' THEN
            LET cIdMunicipio = cIdCiudad;  
        END IF;

        --SELECT claveopcionpuesto, clavesubopcionpuesto
        -- INTO iClavePuesto,iClaveSubPuesto
        --  FROM bdinteg: "informix".si_ingresos 
        -- WHERE numcte = pNumCte  
        --   AND sec_ingreso = (SELECT MAX(sec_ingreso) FROM  bdinteg: "informix".si_ingresos 
        --                       WHERE numcte = pNumCte);
		
		LET cCodRet = '00000';
		LET iClavePuesto=7;
		LET iClaveSubPuesto=99;
	END IF;
    

	RETURN cCodRet, cPrimerNombre, cSegundoNombre, cApellidoPaterno,  cApellidoMaterno, dFechaNacimiento, cIdNacionalidad, cIdPaisNacimiento, cIdEstadoNacimiento, 
				  cSexo, cTipoIdentificacion, cNoIdentificacion, cIdPaisEmision, dFechaVencimiento, cIdOcupacion, cTipoCte, cIdEstado, cIdCiudad, cIdMunicipio, iNumColonia, 
				  iNumCalle, iNumCiudad, cNumExterior, cNumInterior, cDepartamento, cCodPostal, cTelefono, cTelefonoCelular, cIdPaisDomExt, cCorreoElectronico, iClavePuesto, iClaveSubPuesto;
				  
END
END PROCEDURE
DOCUMENT
'DescripciÃÆÃÆÃâÃÂ³n	: Se inserta campo numcte para utilizar en remesas',
'Autor			: Geovani Garcia Ochoa',
'FECHA			: 28/02/2017',
'BD				: bdisac',
'FOLIO		: 198 - RQM 10 784 B ASE DE DATOS PARA EL ALTA DE USUARIOS DE REMESAS',
'-------------------------------------------------------------------------------------------------';

CREATE PROCEDURE "informix".sp_confpagoservicio_hs (cSucursal CHAR (4), cCategoria CHAR (2), cConvenio CHAR(5), cReferencia1 CHAR (40), cReferencia2 CHAR(40), cReferencia3 CHAR(40), cReferencia4 CHAR(40), cFolio_suc CHAR (16))

    RETURNING
    CHAR(5), CHAR(200);

    -- Definicion de Variables
    DEFINE cCodRet      CHAR(5);
    DEFINE iSql_err     INTEGER;
	DEFINE iIsamErr	    INTEGER;
	DEFINE cDescripcion CHAR (200);
	DEFINE vcadena_ent  CHAR (100);
    DEFINE scont        INT8; 
    DEFINE scont2       INT8; 
    DEFINE wBegin       CHAR(1);
	DEFINE vCodRet      CHAR(5);
	DEFINE vCodRetInt	CHAR(5);
	DEFINE vRfc			CHAR(13);
	DeFINE cReferenciaC CHAR(40);
	
     LET scont	       = 0; 
     LET scont2	       = 0; 
     LET cCodRet       = "00000";
     LET iSql_err      = 0;
	 LET iIsamErr      = 0;
	 LET cDescripcion  = "ACTUALIZACION FLAG-SUCURSAL EXITOSA.";
	 LET vcadena_ent   = cSucursal||'|'||cCategoria||'|'||cConvenio||'|'||TRIM(cReferencia1)||'|'||TRIM(cReferencia2)||'|'||TRIM(cReferencia3)||'|'||TRIM(cReferencia4)||'|'||cFolio_suc;
	 LET vCodRet	   = '00000';
	 LET vCodRetInt	   = '00000';
	 LET vRfc 	       = '';

	--SET DEBUG FILE TO '/informix/JorgeRivas/evidencias/sp_confpagoservicio_hs.out';
	--TRACE ON;
	 

    BEGIN
        ON EXCEPTION SET iSql_err, iIsamErr, cDescripcion
           IF iSql_err <> 0 THEN
              LET cCodRet = iSql_err;

              ROLLBACK WORK;

              IF (wBegin = "S") THEN
                 BEGIN WORK;
              END IF;               

			   INSERT INTO "informix".sac_mensajeerror (sql_error, isam_error, descripcion, origen_error, fecha, fecha_insert)
				VALUES (cCodRet,iIsamErr,cDescripcion ||' '|| scont||'-'||scont2,'sp_confpagoservicio',today,CURRENT);

			   INSERT INTO "informix".sac_log_envios (proceso, folio_suc, importe, cod_ret, cadena_ent, fecha_insert, hora_insert)
						values ('sp_confpagoservicio', cFolio_suc, 0, cCodRet, vcadena_ent, CURRENT, CURRENT);	

               RETURN cCodRet, cDescripcion;

           END IF;
        END EXCEPTION;

       ON EXCEPTION IN (-535)
          LET wBegin = "S";
          ROLLBACK WORK;
          BEGIN WORK;
       END EXCEPTION WITH RESUME;
     SET ISOLATION COMMITTED READ;

		SET ISOLATION TO DIRTY READ;
	    SET LOCK MODE TO WAIT 3;

--	2014.01.07 FRG-i
	--SET ISOLATION TO DIRTY READ;
	--SET ISOLATION TO COMMITTED READ LAST COMMITTED;
	SET LOCK MODE TO WAIT 5;

    LET wBegin = "N";

            IF cSucursal <> "" AND cCategoria <> "" AND cConvenio <> "" AND TRIM(cReferencia1) <> ""  AND cFolio_suc <> "" THEN

                BEGIN WORK;

					UPDATE "informix".sac_movimientos
					   SET  flag_confirmacion_sucursal = 1, referencia3 = cReferencia3, referencia4 = cReferencia4
					 WHERE id_sucursal = cSucursal
					   AND numcategoria = cCategoria
					   AND  numconvenio = cConvenio
					   AND referencia1 = TRIM(cReferencia1)
					   AND folio_suc = cFolio_suc;

					LET scont  = dbinfo("sqlca.sqlerrd2");
					LET scont2 = dbinfo("sqlca.sqlerrd1"); 


					IF(scont = 0) THEN
					   --ROLLBACK WORK;

						LET cCodRet = "00002";
						LET cDescripcion = "No existe registro en Tabla bdisac:sac_movimientos";

						INSERT INTO "informix".sac_log_envios (proceso, folio_suc, importe, cod_ret, cadena_ent, fecha_insert, hora_insert)
							VALUES ('sp_confpagoservicio', cFolio_suc, 0, cCodRet, vcadena_ent, CURRENT, CURRENT);

						INSERT INTO "informix".sac_mensajeerror (sql_error, isam_error, descripcion, origen_error, fecha, fecha_insert)
							VALUES (cCodRet,iIsamErr,scont||' - '||scont2,'sp_confpagoservicio - dbinfo',today,CURRENT);

					   IF (wBegin = "S") THEN
						  BEGIN WORK;
					   END IF;
					END IF;
					
					IF(cCodRet = "00000") THEN
						   
						LET scont  = dbinfo("sqlca.sqlerrd2");
						LET scont2 = dbinfo("sqlca.sqlerrd1"); 


						IF(scont = 0) THEN
						   --ROLLBACK WORK;

							LET cCodRet = "00002";
							LET cDescripcion = "No existe registro en Tabla bdicheq:sc_movdia";

							INSERT INTO "informix".sac_log_envios (proceso, folio_suc, importe, cod_ret, cadena_ent, fecha_insert, hora_insert)
								VALUES ('sp_confpagoservicio', cFolio_suc, 0, cCodRet, vcadena_ent, CURRENT, CURRENT);

							INSERT INTO "informix".sac_mensajeerror (sql_error, isam_error, descripcion, origen_error, fecha, fecha_insert)
								VALUES (cCodRet,iIsamErr,scont||' - '||scont2,'sp_confpagoservicio - dbinfo',today,CURRENT);

						   IF (wBegin = "S") THEN
							  BEGIN WORK;
						   END IF;
						END IF;
					END IF;
					
					EXECUTE PROCEDURE "informix".sp_tae_notifications(cFolio_suc) INTO vCodRetInt;

				   IF(cCodRet <> "00000") THEN
					  ROLLBACK WORK;
				   ELSE
					  COMMIT WORK;
				   END IF;

				   IF (wBegin = "S") THEN
					 BEGIN WORK;
				   END IF;

            ELSE
                -- Indica que uno de los campos llave viene vacio
                LET cCodRet = "00001";
			    LET cDescripcion = "Uno de los campos llave esta vacio.";
			   INSERT INTO "informix".sac_log_envios (proceso, folio_suc, importe, cod_ret, cadena_ent, fecha_insert, hora_insert)
				values ('sp_confpagoservicio', cFolio_suc, 0, cCodRet, vcadena_ent, CURRENT, CURRENT);	
            END IF;

      RETURN cCodRet, cDescripcion;
    END;
END PROCEDURE
DOCUMENT
'AUTOR : JosÃ© Angel LÃ³pez Adams',
'DESCRIPCION: Se encarga de confirmar el  movimiento para indicar que todo se grabo bien en sucursal',
'EJECUTADO O LLAMADO POR: Caja.exe()',
'FECHA : Septiembre de 2008',
'VERSION: 20080930',
'MODIFICO : Antonio Bastidas',
'DESCRIPCION: Se redimenciona el tamaÃ±o de referencia1 y referencia2 a 20 caracteres',
'EJECUTADO O LLAMADO POR: Procesos - DineroYa',
'FECHA : Diciembre de 2009',
'VERSION: 20091208.1627',
'MODIFICO : FRG',
'DESCRIPCION: Se agrega una tabla-bitacora para guardar los datos de entrada al SP',
'EJECUTADO O LLAMADO POR: Procesos - EjecuciÃ³n ConfirmaciÃ³n de Pago de Servicios',
'FECHA : Septiembre de 2011',
'VERSION: 20110905.1820',
'BD: bdisac',
'MODIFICO: Eduardo LÃ³pez Cuevas',
'EJECUTADO O LLAMADO POR: caja.exe',
'FECHA: 20130809.1721',
'MODIFICO : FRG',
'DESCRIPCION: Se agrega instrucciÃ³n para evitar bloqueos en tabla.',
'EJECUTADO O LLAMADO POR: Procesos - EjecuciÃ³n ConfirmaciÃ³n de Pago de Servicios',
'FECHA : Enero 2014',
'BD: bdisac',
'EJECUTADO O LLAMADO POR: caja.exe',
'FECHA: 20140107.1340',
'BD: bdisac',
'MODIFICO : Jorge ROberto',
'DESCRIPCION: actualizamos la referencia de la sc_movdia para pagos de servicio',
'EJECUTADO O LLAMADO POR: Procesos - Pago de servicios TAECOPPEL',
'FECHA : 14/10/2020';

CREATE PROCEDURE "informix".sp_app_submitpayment_web
(
	ptxn_status			CHAR(1),
	punirefnum			CHAR(16),
	prefnum				CHAR(30),
	pcode				CHAR(3),
	pchanneldid			CHAR(3),
	plocationunit		CHAR(15),
	pnnumber			CHAR(15),
	ptypecode			CHAR(3),
	pcountrycode		CHAR(3),
	pstatecode			CHAR(3),
	pterminalid			CHAR(15),
	pprocessdate		CHAR(8),
	pprocesstime		CHAR(6),
	pcustomernumber		CHAR(20),
	pfirstname			CHAR(40),
	pmiddlename			CHAR(40),
	plastname			CHAR(40),
	pmommaidenname	 	CHAR(40),
	padress				CHAR(80),
	pcity				CHAR(40),
	pcountrycodeadr		CHAR(3),
	pstatecodeadr		CHAR(3),
	pzipcode			CHAR(10),
	pemail				CHAR(100),
	phomephonenum		CHAR(15),
	pnumbercel			CHAR(15),
	preceiveemail		CHAR(3),
	preceivesms			CHAR(3),
	ptypecodeci			CHAR(3),
	pnumberci			CHAR(20),
	pexpirationdate		CHAR(8),
	pissuercc			CHAR(3),
	pdateofbirth		CHAR(8),
	pcontrycode			CHAR(5),
	pr_operacion		CHAR(5),
	pr_code				CHAR(4),
	pr_message			CHAR(255),
	pr_code_d			CHAR(4),
	pr_message_d		CHAR(255),
	pr_processdate		CHAR(8),
	pr_processtime		CHAR(6),
	pr_rule				CHAR(3),
	pr_value			CHAR(3),
	pr_globtracknum		CHAR(20),
	pr_ordstatuscode	CHAR(3),
	pr_ordstatusdate	CHAR(8),
	pr_ordstatustime	CHAR(6),
	pr_uniquerefnum		CHAR(16),
	pr_codesalecom		CHAR(3),
	pr_countrycode		CHAR(3),
	pr_statecodesale	CHAR(3),
	pr_saledate			CHAR(8),
	pr_saletime			CHAR(6),
	pr_countrycode_o	CHAR(3),
	pr_currencycode		CHAR(3),
	pr_servicecode		CHAR(3),
	pr_countrycode_d	CHAR(3),
	pr_currencycod_d	CHAR(3),
	pr_delimethodcod	CHAR(3),
	pr_playnwcode		CHAR(3),
	pr_paysubnwcode		CHAR(15),
	pr_branchnumber		CHAR(15),
	pr_accounttcod		CHAR(3),
	pr_accountnumber	CHAR(30),
	pr_originamount		CHAR(20),
	pr_destinamount		CHAR(20),
	pr_rexchangerate	CHAR(21),
	pr_wholesalerate	CHAR(21),
	pr_deexhangerate	CHAR(21),
	pr_servfeeamount	CHAR(20),
	pr_discountamoun	CHAR(20),
	pr_typecode			CHAR(3),
	pr_accountnum		CHAR(30),
	pr_biccode			CHAR(11),
	pr_refnumber		CHAR(30),
	pr_customernum		CHAR(20),
	pr_firstname		CHAR(40),
	pr_middlename		CHAR(40),
	pr_lastname			CHAR(40),
	pr_mommaidenname 	CHAR(40),
	pr_address			CHAR(80),
	pr_city				CHAR(40),
	pr_countrycode_a	CHAR(3),
	pr_statecode		CHAR(3),
	pr_zipcode			CHAR(10),
	pr_typecode_i		CHAR(3),
	pr_number			CHAR(20),
	pr_expirdate		CHAR(8),
	pr_isscontrycode	CHAR(3),
	pr_issstatecode		CHAR(3),
	pr_dateofbirth		CHAR(8),
	pr_customernum_b 	CHAR(20),
	pr_firstname_b		CHAR(40),
	pr_middlename_b		CHAR(40),
	pr_lastname_b		CHAR(40),
	pr_mommaidenna_b 	CHAR(40),
	pr_firstname_f		CHAR(40),
	pr_middlename_f		CHAR(40),
	pr_lastname_f		CHAR(40),
	pr_mommaidenna_f 	CHAR(40),
	pr_address_b		CHAR(80),
	pr_city_b			CHAR(40),
	pr_countrycode_b	CHAR(3),
	pr_statecode_b		CHAR(3),
	pr_zipcode_b		CHAR(10),
	pr_email			CHAR(100),
	pr_homephonenum 	CHAR(15),
	pr_workphonenum		CHAR(15),
	pr_number_cl		CHAR(15),
	pr_receiveemail		CHAR(3),
	pr_receivesms		CHAR(3),
	pr_typecode_ib		CHAR(3),
	pr_number_ib		CHAR(20),
	pr_expirdate_ib		CHAR(8),
	pr_issconcode_ib	CHAR(3),
	pr_issstacode_ib	CHAR(3),
	pr_reastypecode		CHAR(3),
	pr_refortransfer	CHAR(40),
	pr_sourceoffunds	CHAR(40),
	pr_securphrase		CHAR(40),
	pr_feemessage		CHAR(255),
	puser_insert		CHAR(8),
	pfecha				DATE,
	pNumCte			    CHAR(20)
)
RETURNING CHAR (5)	 AS cCodRet, 
		  CHAR (255) AS cr_Message, --1.2
		  CHAR (255) AS cr_Message_Detail; --1.3.2
		  
	  
--Declaracion de variables 
		DEFINE cCodRet 		    	CHAR(5);
		DEFINE iSqlErr				INTEGER;
		DEFINE imenscode			INTEGER;
		DEFINE cr_Message			CHAR (255);
		DEFINE cr_Message_Detail	CHAR (255);
		
		DEFINE crsp_CodRet			CHAR(5);
		DEFINE crsp_Message			CHAR (255);
		DEFINE crsp_Message_Detail	CHAR (255);
		DEFINE c_Mess_D				CHAR (255);
		DEFINE vfec_nac				DATE;
		
		DEFINE vCodRet				CHAR(5);
		DEFINE vcuenta				INTEGER;
		DEFINE vCategoria			CHAR(2);
		DEFINE vConvenio			CHAR(5);
		
		
-----------------------------------------------------------------------------------------------------------------------------------
		LET cCodRet 			= '00000'; --Codigo 00000 = OK; 00001 = No hubo datos
		LET iSqlErr				= 0;
		LET imenscode			= 0;
        LET cr_Message			= pr_message;
		LET cr_Message_Detail	= pr_message_d;
		
		LET crsp_CodRet			= '';
		LET crsp_Message		= '';
		LET crsp_Message_Detail	= '';
		LET c_Mess_D			= pr_message_d;
		
		LET vcuenta				= 0;
		LET vCodRet				= '00000';
		LET vCategoria			= '07';
		LET vConvenio			= '009';
		
BEGIN
	ON EXCEPTION SET iSqlErr
		IF iSqlErr <> 0 THEN
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cr_Message, cr_Message_Detail;
		END IF;
	END EXCEPTION;
	
	--SET DEBUG FILE TO '/informix/ENP/sp_app_submitpayment.out';
	--TRACE ON;	
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;  
		
		IF NVL(punirefnum, '') = '' OR NVL(pnnumber, '') = '' OR NVL(puser_insert, '') = '' OR NVL(pfecha, '') = '' OR NVL(prefnum, '') = '' THEN
			 LET cCodRet = '00001';
			 --DATOS VACIOS, ERROR.
			 RETURN cCodRet, NVL(cr_Message, ''),  NVL(cr_Message_Detail, '');

		ELSE
		
			-- verifica si los mensajes de regreso estan en el catalado para regresarlos en espanol 
			EXECUTE PROCEDURE BDISAC: "informix".sp_app_mensajes ('PAYI', pr_code, pr_code_d)
			INTO crsp_CodRet, crsp_Message, crsp_Message_Detail;	
			
			IF DBINFO("sqlca.sqlerrd2") = 0 THEN
				-- no trajo resultados del llamado al sp
				LET cCodRet = '00001';
			ELSE
				IF crsp_CodRet = '00000' THEN
					LET cr_Message = crsp_Message;
					LET imenscode = 1;
				ELSE
					IF crsp_CodRet = '00001' THEN
						LET cr_Message = crsp_Message;
					ELSE
						IF crsp_CodRet = '00002' THEN
							LET imenscode = 1;
						END IF;
					END IF;
				END IF;
			END IF;
			IF imenscode = 1 then
				IF pr_code_d = "D001" THEN
					LET imenscode = 20;
				ELIF  pr_code_d = "D002" THEN
					LET imenscode = 28;
				ELIF  pr_code_d = "D003" THEN
					LET imenscode = 19;
				ELIF  pr_code_d = "D004" THEN
					LET imenscode = 20;
				ELIF  pr_code_d = "D005" THEN
					LET imenscode = 20;
				END IF;
				-- corta parametro en mensaje ingles "Required Parameter: {0} Parameter Key"
				LET c_Mess_D = SUBSTR(c_Mess_D, imenscode);
				LET c_Mess_D = REPLACE(c_Mess_D, " Parameter Key","");
				-- concatenar parametro con el mensaje en espanol "El parametro requerido: {0} parametro clave."
				LET crsp_Message_Detail = REPLACE(crsp_Message_Detail,"{0}", trim(c_Mess_D));
				-- asigna mensaje que devera retornar
				LET cr_Message_Detail = crsp_Message_Detail;
			END IF;
			--Inserta registro
			IF (cCodRet = '00000')THEN
								
				INSERT INTO bdisac: "informix".sac_app_payi (txn_status,unirefnum,refnum,code,channeldid,locationunit,nnumber,typecode,countrycode,statecode,terminalid,processdate,processtime,customernumber,firstname,middlename,lastname,mommaidenname,adress,city,countrycodeadr,statecodeadr,zipcode,email,homephonenum,numbercel,receiveemail,receivesms,typecodeci,numberci,expirationdate,issuercc,dateofbirth,contrycode,r_operacion,r_code,r_message,r_code_d,r_message_d,r_processdate,r_processtime,r_rule,r_value,r_globtracknum,r_ordstatuscode,r_ordstatusdate,r_ordstatustime,r_uniquerefnum,r_codesalecom,r_countrycode,r_statecodesale,r_saledate,r_saletime,r_countrycode_o,r_currencycode,r_servicecode,r_countrycode_d,r_currencycod_d,r_delimethodcod,r_playnwcode,r_paysubnwcode,r_branchnumber,r_accounttcod,r_accountnumber,r_originamount,r_destinamount,r_rexchangerate,r_wholesalerate,r_deexhangerate,r_servfeeamount,r_discountamoun,r_typecode,r_accountnum,r_biccode,r_refnumber,r_customernum,r_firstname,r_middlename,r_lastname,r_mommaidenname,r_address,r_city,r_countrycode_a,r_statecode,r_zipcode,r_typecode_i,r_number,r_expirdate,r_isscontrycode,r_issstatecode,r_dateofbirth,r_customernum_b,r_firstname_b,r_middlename_b,r_lastname_b,r_mommaidenna_b,r_firstname_f,r_middlename_f,r_lastname_f,r_mommaidenna_f,r_address_b,r_city_b,r_countrycode_b,r_statecode_b,r_zipcode_b,r_email,r_homephonenum,r_workphonenum,r_number_cl,r_receiveemail,r_receivesms,r_typecode_ib,r_number_ib,r_expirdate_ib,r_issconcode_ib,r_issstacode_ib,r_reastypecode,r_refortransfer,r_sourceoffunds,r_securphrase,r_feemessage,user_insert,fecha,numcte) 
				VALUES (ptxn_status, punirefnum, prefnum, pcode, pchanneldid, plocationunit, pnnumber, ptypecode, pcountrycode, pstatecode, pterminalid, pprocessdate, pprocesstime, pcustomernumber, pfirstname, pmiddlename, plastname, pmommaidenname, padress, pcity, pcountrycodeadr, pstatecodeadr, pzipcode, pemail, phomephonenum, pnumbercel, preceiveemail, preceivesms, ptypecodeci, pnumberci, pexpirationdate, pissuercc, pdateofbirth, pcontrycode, pr_operacion, pr_code, cr_Message, pr_code_d, cr_Message_Detail,  pr_processdate, pr_processtime, pr_rule, pr_value, pr_globtracknum, pr_ordstatuscode, pr_ordstatusdate, pr_ordstatustime, pr_uniquerefnum, pr_codesalecom, pr_countrycode, pr_statecodesale, pr_saledate, pr_saletime, pr_countrycode_o, pr_currencycode, pr_servicecode, pr_countrycode_d, pr_currencycod_d, pr_delimethodcod, pr_playnwcode, pr_paysubnwcode, pr_branchnumber, pr_accounttcod, pr_accountnumber, pr_originamount, pr_destinamount, pr_rexchangerate, pr_wholesalerate, pr_deexhangerate, pr_servfeeamount, pr_discountamoun, pr_typecode, pr_accountnum, pr_biccode, pr_refnumber, pr_customernum, pr_firstname, pr_middlename, pr_lastname, pr_mommaidenname, pr_address, pr_city, pr_countrycode_a, pr_statecode, pr_zipcode, pr_typecode_i, pr_number, pr_expirdate, pr_isscontrycode, pr_issstatecode, pr_dateofbirth, pr_customernum_b, pr_firstname_b, pr_middlename_b, pr_lastname_b, pr_mommaidenna_b, pr_firstname_f, pr_middlename_f, pr_lastname_f, pr_mommaidenna_f, pr_address_b, pr_city_b, pr_countrycode_b, pr_statecode_b, pr_zipcode_b, pr_email, pr_homephonenum, pr_workphonenum, pr_number_cl, pr_receiveemail, pr_receivesms, pr_typecode_ib, pr_number_ib, pr_expirdate_ib, pr_issconcode_ib, pr_issstacode_ib, pr_reastypecode, pr_refortransfer, pr_sourceoffunds, pr_securphrase, pr_feemessage, puser_insert, CURRENT,pNumCte);
				IF DBINFO("sqlca.sqlerrd2") = 0 THEN
					LET cCodRet = '00001';
				--La ejecucion de este procedimiento se paso al procedimiento 'bdisac:sp_pago_appiza_web'
				--ELSE
				--	LET vfec_nac = MDY(SUBSTRING(pdateofbirth FROM 5 FOR 2), SUBSTRING(pdateofbirth FROM 7 FOR 2), SUBSTRING(pdateofbirth FROM 1 FOR 4));
				--	EXECUTE PROCEDURE bdisac:"informix".sp_actualizaremesa(vCategoria, vConvenio, punirefnum, pfirstname, pmiddlename, plastname, pmommaidenname, vfec_nac, pr_currencycode, pr_originamount)
				--	INTO vCodRet, vcuenta;
				END IF;
	            ---------------------------------------------------------------------------------
				IF
					pnnumber <> '' or pnnumber IS NOT NULL THEN
					IF EXISTS (SELECT * FROM bdinteg:"informix".si_sucursales WHERE sucursal NOT IN ('9250', '9251', '9764','5011','5003') AND sucursal = pnnumber) THEN
						EXECUTE PROCEDURE bdinteg: "informix".sp_inserta_msjafore(pNumcte,'',pnnumber,'') INTO cCodRet;
						IF 
							cCodRet <> '00000' THEN
							INSERT INTO bdisac:"informix".sac_procesos_jobs(proceso, fecha_proceso, status, user_insert, fecha_insert, numero_ejecuciones, nombre_sp, descripcion)
							VALUES ('sp_pago_wu_web', CURRENT, '0', 'informix', CURRENT, NULL, 'sp_inserta_msjafore', 'Codigo retorno: '|| cCodRet);
						END IF;
					END IF;
				END IF;
				----------------------------------------------------------------------------------
			END IF;

			RETURN cCodRet, NVL(cr_Message, ''),  NVL(cr_Message_Detail, '') ;
			
		END IF;
	END;
END PROCEDURE
DOCUMENT
'AUTOR : 95992243 - Trinidad Hernandez',
'DESCRIPCION: Servira para insertar en la tabla sac_app_payi',
'FOLIO: 1543 - PagosApprizaDLL',
'FECHA : 22/03/2016',
'VERSION: 20160318.0921',	
'BD: bdisac',
'-----------------------------------------------------------------------------------------------------------------------',
'Descripcion: Se insertan campo numcte para Trabajar en remesas',
'Autor      : Geovani Garcia Ochoa',
'FECHA DE CREACION    : 28/02/2017',
'BD         : bdisac ',
'FOLIO: 198 - RQM 10 784 B ASE DE DATOS PARA EL ALTA DE USUARIOS DE REMESAS',
'-----------------------------------------------------------------------------------------------------',
'Descripcion: Se quita llamado a sp_actualizaremesa, este se paso a sp_pago_appiza_web',
'Autor      : Aaron Lopez Ruiz',
'FECHA DE CREACION    : 15/08/2019',
'BD         : bdisac ',
'FOLIO: RQI 62 663 Remesas Web',
'-----------------------------------------------------------------------------------------------------';

CREATE PROCEDURE "informix".sp_app_submitpayment_web
(
	ptxn_status			        CHAR(1),
	punirefnum			        CHAR(16),
	prefnum				        CHAR(30),
	pcode				        CHAR(3),
	pchanneldid			        CHAR(3),
	plocationunit		        CHAR(15),
	pnnumber			        CHAR(15),
	ptypecode			        CHAR(3),
	pcountrycode		        CHAR(3),
	pstatecode			        CHAR(3),
	pterminalid			        CHAR(15),
	pprocessdate		        CHAR(8),
	pprocesstime		        CHAR(6),
	pcustomernumber		        CHAR(20),
	pfirstname			        CHAR(40),
	pmiddlename			        CHAR(40),
	plastname			        CHAR(40),
	pmommaidenname	 	        CHAR(40),
	padress				        CHAR(80),
	pcity				        CHAR(40),
	pcountrycodeadr		        CHAR(3),
	pstatecodeadr		        CHAR(3),
	pzipcode			        CHAR(10),
	pemail				        CHAR(100),
	phomephonenum		        CHAR(15),
	pnumbercel			        CHAR(15),
	preceiveemail		        CHAR(3),
	preceivesms			        CHAR(3),
	ptypecodeci			        CHAR(3),
	pnumberci			        CHAR(20),
	pexpirationdate		        CHAR(8),
	pissuercc			        CHAR(3),
	pdateofbirth		        CHAR(8),
	pcontrycode			        CHAR(5),
	pr_operacion		        CHAR(5),
	pr_code				        CHAR(4),
	pr_message			        CHAR(255),
	pr_code_d			        CHAR(4),
	pr_message_d		        CHAR(255),
	pr_processdate		        CHAR(8),
	pr_processtime		        CHAR(6),
	pr_rule				        CHAR(3),
	pr_value			        CHAR(3),
	pr_globtracknum		        CHAR(20),
	pr_ordstatuscode	        CHAR(3),
	pr_ordstatusdate	        CHAR(8),
	pr_ordstatustime	        CHAR(6),
	pr_uniquerefnum		        CHAR(16),
	pr_codesalecom		        CHAR(3),
	pr_countrycode		        CHAR(3),
	pr_statecodesale	        CHAR(3),
	pr_saledate			        CHAR(8),
	pr_saletime			        CHAR(6),
	pr_countrycode_o	        CHAR(3),
	pr_currencycode		        CHAR(3),
	pr_servicecode		        CHAR(3),
	pr_countrycode_d	        CHAR(3),
	pr_currencycod_d	        CHAR(3),
	pr_delimethodcod	        CHAR(3),
	pr_playnwcode		        CHAR(3),
	pr_paysubnwcode		        CHAR(15),
	pr_branchnumber		        CHAR(15),
	pr_accounttcod		        CHAR(3),
	pr_accountnumber	        CHAR(30),
	pr_originamount		        CHAR(20),
	pr_destinamount		        CHAR(20),
	pr_rexchangerate	        CHAR(21),
	pr_wholesalerate	        CHAR(21),
	pr_deexhangerate	        CHAR(21),
	pr_servfeeamount	        CHAR(20),
	pr_discountamoun	        CHAR(20),
	pr_typecode			        CHAR(3),
	pr_accountnum		        CHAR(30),
	pr_biccode			        CHAR(11),
	pr_refnumber		        CHAR(30),
	pr_customernum		        CHAR(20),
	pr_firstname		        CHAR(40),
	pr_middlename		        CHAR(40),
	pr_lastname			        CHAR(40),
	pr_mommaidenname 	        CHAR(40),
	pr_address			        CHAR(80),
	pr_city				        CHAR(40),
	pr_countrycode_a	        CHAR(3),
	pr_statecode		        CHAR(3),
	pr_zipcode			        CHAR(10),
	pr_typecode_i		        CHAR(3),
	pr_number			        CHAR(20),
	pr_expirdate		        CHAR(8),
	pr_isscontrycode	        CHAR(3),
	pr_issstatecode		        CHAR(3),
	pr_dateofbirth		        CHAR(8),
	pr_customernum_b 	        CHAR(20),
	pr_firstname_b		        CHAR(40),
	pr_middlename_b		        CHAR(40),
	pr_lastname_b		        CHAR(40),
	pr_mommaidenna_b 	        CHAR(40),
	pr_firstname_f		        CHAR(40),
	pr_middlename_f		        CHAR(40),
	pr_lastname_f		        CHAR(40),
	pr_mommaidenna_f 	        CHAR(40),
	pr_address_b		        CHAR(80),
	pr_city_b			        CHAR(40),
	pr_countrycode_b	        CHAR(3),
	pr_statecode_b		        CHAR(3),
	pr_zipcode_b		        CHAR(10),
	pr_email			        CHAR(100),
	pr_homephonenum 	        CHAR(15),
	pr_workphonenum		        CHAR(15),
	pr_number_cl		        CHAR(15),
	pr_receiveemail		        CHAR(3),
	pr_receivesms		        CHAR(3),
	pr_typecode_ib		        CHAR(3),
	pr_number_ib		        CHAR(20),
	pr_expirdate_ib		        CHAR(8),
	pr_issconcode_ib	        CHAR(3),
	pr_issstacode_ib	        CHAR(3),
	pr_reastypecode		        CHAR(3),
	pr_refortransfer	        CHAR(40),
	pr_sourceoffunds	        CHAR(40),
	pr_securphrase		        CHAR(40),
	pr_feemessage		        CHAR(255),
	puser_insert		        CHAR(8),
	pfecha				        DATE,
	pNumCte			            CHAR(20),
    pr_occupationcode		    CHAR(3),
    pr_nationalitycode		    CHAR(3),
    pr_relationshiptosendercode	CHAR(3),
    pr_reasonfortransfercode	CHAR(3),
    pr_reasonfortransfer		CHAR(100)
)
RETURNING CHAR (5)	 AS cCodRet, 
		  CHAR (255) AS cr_Message, --1.2
		  CHAR (255) AS cr_Message_Detail; --1.3.2
		  
	  
--Declaracion de variables 
		DEFINE cCodRet 		    	CHAR(5);
		DEFINE iSqlErr				INTEGER;
		DEFINE imenscode			INTEGER;
		DEFINE cr_Message			CHAR (255);
		DEFINE cr_Message_Detail	CHAR (255);
		
		DEFINE crsp_CodRet			CHAR(5);
		DEFINE crsp_Message			CHAR (255);
		DEFINE crsp_Message_Detail	CHAR (255);
		DEFINE c_Mess_D				CHAR (255);
		DEFINE vfec_nac				DATE;
		
		DEFINE vCodRet				CHAR(5);
		DEFINE vcuenta				INTEGER;
		DEFINE vCategoria			CHAR(2);
		DEFINE vConvenio			CHAR(5);
		
        --EPG 22/04/2021
        DEFINE refeferencia_remesa  CHAR(12);
		DEFINE tipo_remesa          INTEGER;
        --EPG 22/04/2021
-----------------------------------------------------------------------------------------------------------------------------------
		LET cCodRet 			= '00000'; --Codigo 00000 = OK; 00001 = No hubo datos
		LET iSqlErr				= 0;
		LET imenscode			= 0;
        LET cr_Message			= pr_message;
		LET cr_Message_Detail	= pr_message_d;
		
		LET crsp_CodRet			= '';
		LET crsp_Message		= '';
		LET crsp_Message_Detail	= '';
		LET c_Mess_D			= pr_message_d;
		
		LET vcuenta				= 0;
		LET vCodRet				= '00000';
		LET vCategoria			= '07';
		LET vConvenio			= '009';
        
        --EPG 22/04/2021
        LET refeferencia_remesa = '';
        LET tipo_remesa         = 0;
        --EPG 22/04/2021
		
BEGIN
	ON EXCEPTION SET iSqlErr
		IF iSqlErr <> 0 THEN
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cr_Message, cr_Message_Detail;
		END IF;
	END EXCEPTION;
	
	--SET DEBUG FILE TO '/informix/ENP/sp_app_submitpayment.out';
	--TRACE ON;	
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;  
		
		IF NVL(punirefnum, '') = '' OR NVL(pnnumber, '') = '' OR NVL(puser_insert, '') = '' OR NVL(pfecha, '') = '' OR NVL(prefnum, '') = '' THEN
			 LET cCodRet = '00001';
			 --DATOS VACIOS, ERROR.
			 RETURN cCodRet, NVL(cr_Message, ''),  NVL(cr_Message_Detail, '');

		ELSE
		
			-- verifica si los mensajes de regreso estan en el catalado para regresarlos en espanol 
			EXECUTE PROCEDURE BDISAC: "informix".sp_app_mensajes ('PAYI', pr_code, pr_code_d)
			INTO crsp_CodRet, crsp_Message, crsp_Message_Detail;	
			
			IF DBINFO("sqlca.sqlerrd2") = 0 THEN
				-- no trajo resultados del llamado al sp
				LET cCodRet = '00001';
			ELSE
				IF crsp_CodRet = '00000' THEN
					LET cr_Message = crsp_Message;
					LET imenscode = 1;
				ELSE
					IF crsp_CodRet = '00001' THEN
						LET cr_Message = crsp_Message;
					ELSE
						IF crsp_CodRet = '00002' THEN
							LET imenscode = 1;
						END IF;
					END IF;
				END IF;
			END IF;
			IF imenscode = 1 then
				IF pr_code_d = "D001" THEN
					LET imenscode = 20;
				ELIF  pr_code_d = "D002" THEN
					LET imenscode = 28;
				ELIF  pr_code_d = "D003" THEN
					LET imenscode = 19;
				ELIF  pr_code_d = "D004" THEN
					LET imenscode = 20;
				ELIF  pr_code_d = "D005" THEN
					LET imenscode = 20;
				END IF;
				-- corta parametro en mensaje ingles "Required Parameter: {0} Parameter Key"
				LET c_Mess_D = SUBSTR(c_Mess_D, imenscode);
				LET c_Mess_D = REPLACE(c_Mess_D, " Parameter Key","");
				-- concatenar parametro con el mensaje en espanol "El parametro requerido: {0} parametro clave."
				LET crsp_Message_Detail = REPLACE(crsp_Message_Detail,"{0}", trim(c_Mess_D));
				-- asigna mensaje que devera retornar
				LET cr_Message_Detail = crsp_Message_Detail;
			END IF;
			
			--Inserta registro
			IF (cCodRet = '00000')THEN
								
				---EPG 22/04/2021
                ---Valido si la remesa es Money Gram 
                SELECT referencia1 INTO refeferencia_remesa FROM bdisac:"informix".sac_movimientos WHERE numcategoria = vCategoria and numconvenio = vConvenio and folio_suc = prefnum;
                LET tipo_remesa = LENGTH(refeferencia_remesa);
                IF tipo_remesa = 8 THEN --Si es Money Gram guardo la referencia de Appriza 
                   UPDATE bdisac:"informix".sac_movimientos SET referencia4 = pr_uniquerefnum WHERE referencia1 = punirefnum and folio_suc = prefnum;
                END IF;
                ---EPG 22/04/2021

                INSERT INTO bdisac: "informix".sac_app_payi (txn_status,unirefnum,refnum,code,channeldid,locationunit,nnumber,typecode,countrycode,statecode,terminalid,processdate,processtime,customernumber,firstname,middlename,lastname,mommaidenname,adress,city,countrycodeadr,statecodeadr,zipcode,email,homephonenum,numbercel,receiveemail,receivesms,typecodeci,numberci,expirationdate,issuercc,dateofbirth,contrycode,r_operacion,r_code,r_message,r_code_d,r_message_d,r_processdate,r_processtime,r_rule,r_value,r_globtracknum,r_ordstatuscode,r_ordstatusdate,r_ordstatustime,r_uniquerefnum,r_codesalecom,r_countrycode,r_statecodesale,r_saledate,r_saletime,r_countrycode_o,r_currencycode,r_servicecode,r_countrycode_d,r_currencycod_d,r_delimethodcod,r_playnwcode,r_paysubnwcode,r_branchnumber,r_accounttcod,r_accountnumber,r_originamount,r_destinamount,r_rexchangerate,r_wholesalerate,r_deexhangerate,r_servfeeamount,r_discountamoun,r_typecode,r_accountnum,r_biccode,r_refnumber,r_customernum,r_firstname,r_middlename,r_lastname,r_mommaidenname,r_address,r_city,r_countrycode_a,r_statecode,r_zipcode,r_typecode_i,r_number,r_expirdate,r_isscontrycode,r_issstatecode,r_dateofbirth,r_customernum_b,r_firstname_b,r_middlename_b,r_lastname_b,r_mommaidenna_b,r_firstname_f,r_middlename_f,r_lastname_f,r_mommaidenna_f,r_address_b,r_city_b,r_countrycode_b,r_statecode_b,r_zipcode_b,r_email,r_homephonenum,r_workphonenum,r_number_cl,r_receiveemail,r_receivesms,r_typecode_ib,r_number_ib,r_expirdate_ib,r_issconcode_ib,r_issstacode_ib,r_reastypecode,r_refortransfer,r_sourceoffunds,r_securphrase,r_feemessage,user_insert,fecha,numcte, r_occupationcode, r_nationalitycode, r_relationshiptosendercode, r_reasonfortransfercode, r_reasonfortransfer) 
				VALUES (ptxn_status, punirefnum, prefnum, pcode, pchanneldid, plocationunit, pnnumber, ptypecode, pcountrycode, pstatecode, pterminalid, pprocessdate, pprocesstime, pcustomernumber, pfirstname, pmiddlename, plastname, pmommaidenname, padress, pcity, pcountrycodeadr, pstatecodeadr, pzipcode, pemail, phomephonenum, pnumbercel, preceiveemail, preceivesms, ptypecodeci, pnumberci, pexpirationdate, pissuercc, pdateofbirth, pcontrycode, pr_operacion, pr_code, cr_Message, pr_code_d, cr_Message_Detail,  pr_processdate, pr_processtime, pr_rule, pr_value, pr_globtracknum, pr_ordstatuscode, pr_ordstatusdate, pr_ordstatustime, pr_uniquerefnum, pr_codesalecom, pr_countrycode, pr_statecodesale, pr_saledate, pr_saletime, pr_countrycode_o, pr_currencycode, pr_servicecode, pr_countrycode_d, pr_currencycod_d, pr_delimethodcod, pr_playnwcode, pr_paysubnwcode, pr_branchnumber, pr_accounttcod, pr_accountnumber, pr_originamount, pr_destinamount, pr_rexchangerate, pr_wholesalerate, pr_deexhangerate, pr_servfeeamount, pr_discountamoun, pr_typecode, pr_accountnum, pr_biccode, pr_refnumber, pr_customernum, pr_firstname, pr_middlename, pr_lastname, pr_mommaidenname, pr_address, pr_city, pr_countrycode_a, pr_statecode, pr_zipcode, pr_typecode_i, pr_number, pr_expirdate, pr_isscontrycode, pr_issstatecode, pr_dateofbirth, pr_customernum_b, pr_firstname_b, pr_middlename_b, pr_lastname_b, pr_mommaidenna_b, pr_firstname_f, pr_middlename_f, pr_lastname_f, pr_mommaidenna_f, pr_address_b, pr_city_b, pr_countrycode_b, pr_statecode_b, pr_zipcode_b, pr_email, pr_homephonenum, pr_workphonenum, pr_number_cl, pr_receiveemail, pr_receivesms, pr_typecode_ib, pr_number_ib, pr_expirdate_ib, pr_issconcode_ib, pr_issstacode_ib, pr_reastypecode, pr_refortransfer, pr_sourceoffunds, pr_securphrase, pr_feemessage, puser_insert, CURRENT,pNumCte, pr_occupationcode, pr_nationalitycode, pr_relationshiptosendercode, pr_reasonfortransfercode, pr_reasonfortransfer);

				IF DBINFO("sqlca.sqlerrd2") = 0 THEN
					LET cCodRet = '00001';
				--La ejecucion de este procedimiento se paso al procedimiento 'bdisac:sp_pago_appiza_web'
				--ELSE
				--	LET vfec_nac = MDY(SUBSTRING(pdateofbirth FROM 5 FOR 2), SUBSTRING(pdateofbirth FROM 7 FOR 2), SUBSTRING(pdateofbirth FROM 1 FOR 4));
				--	EXECUTE PROCEDURE bdisac:"informix".sp_actualizaremesa(vCategoria, vConvenio, punirefnum, pfirstname, pmiddlename, plastname, pmommaidenname, vfec_nac, pr_currencycode, pr_originamount)
				--	INTO vCodRet, vcuenta;
				END IF;

            ---------------------------------------------------------------------------------
            IF
                pnnumber <> '' or pnnumber IS NOT NULL THEN
                IF EXISTS (SELECT * FROM bdinteg:"informix".si_sucursales WHERE sucursal NOT IN ('9250', '9251', '9764','5011','5003') AND sucursal = pnnumber) THEN
                    EXECUTE PROCEDURE bdinteg: "informix".sp_inserta_msjafore(pNumcte,'',pnnumber,'') INTO cCodRet;
                    IF 
                        cCodRet <> '00000' THEN
                        INSERT INTO bdisac:"informix".sac_procesos_jobs(proceso, fecha_proceso, status, user_insert, fecha_insert, numero_ejecuciones, nombre_sp, descripcion)
                        VALUES ('sp_pago_wu_web', CURRENT, '0', 'informix', CURRENT, NULL, 'sp_inserta_msjafore', 'Codigo retorno: '|| cCodRet);
                    END IF;
                END IF;
            END IF;
            ----------------------------------------------------------------------------------
			END IF;

			RETURN cCodRet, NVL(cr_Message, ''),  NVL(cr_Message_Detail, '') ;
			
		END IF;

END;
END PROCEDURE
DOCUMENT
'AUTOR : 95992243 - Trinidad Hernandez',
'DESCRIPCION: Servira para insertar en la tabla sac_app_payi',
'FOLIO: 1543 - PagosApprizaDLL',
'FECHA : 22/03/2016',
'VERSION: 20160318.0921',	
'BD: bdisac',
'-----------------------------------------------------------------------------------------------------------------------',
'Descripcion: Se insertan campo numcte para Trabajar en remesas',
'Autor      : Geovani Garcia Ochoa',
'FECHA DE CREACION    : 28/02/2017',
'BD         : bdisac ',
'FOLIO: 198 - RQM 10 784 B ASE DE DATOS PARA EL ALTA DE USUARIOS DE REMESAS',
'-----------------------------------------------------------------------------------------------------',
'Descripcion: Se quita llamado a sp_actualizaremesa, este se paso a sp_pago_appiza_web',
'Autor      : Aaron Lopez Ruiz',
'FECHA DE CREACION    : 15/08/2019',
'BD         : bdisac ',
'FOLIO: RQI 62 663 Remesas Web',
'-----------------------------------------------------------------------------------------------------',
'Descripcion: Se agregan campos requeridos para Appriza Pay - Money Gram',
'Autor      : Eduardo Pineda GuzmÃ¡n',
'FECHA DE CREACION    : 22/04/2021',
'BD         : bdisac ',
'FOLIO: RQM 10 1389 - Money Gram',
'-----------------------------------------------------------------------------------------------------------------------';

CREATE PROCEDURE "informix".sp_guardarespuestapayi2
(
    pSucursal 		            CHAR (4), 
    pTxn_Status					CHAR(1), 
    pConfirmation_nm 			CHAR (11), 
    pBank_Ref_Num			 	CHAR(20), 
    pUser_name 					CHAR(20), 
    pTerminal 					CHAR(15), 
    pAgent_Dt 					CHAR(8), 
    pAgent_Tm 					CHAR(6), 
    pR_First_Name				CHAR(40), 
    pR_Middle_Name 				CHAR(40), 
    pR_Last_Name 				CHAR(40), 
    pR_Mother_M_Name 			CHAR(40),
    pR_Type_Cd 					CHAR(3), 
    pR_Issuer_Cd 				CHAR(3), 
    pR_Issuer_State_Cd			CHAR(3), 
    pR_Issuer_Country_Cd		CHAR(3), 
    pR_Identif_Type				CHAR(5),
    pR_Identif_Nm 				CHAR(20), 
    pR_Expiration_Dt			CHAR(8),
    pR_Fecha_Nac 				CHAR(8),
    pR_Nacionalidad 			CHAR(50),
    pR_pais_nac 				CHAR(20),	
    pR_Nom_Calle 				CHAR(50),
    pR_Num_Ext 					CHAR(5),
    pR_Num_Int 					CHAR(5),
    pR_Depto 					CHAR(10),
    pR_Colonia					CHAR(80),
    pR_Cp						CHAR(5),
    pR_Mncpo_Delg 				CHAR(50),
    pR_Ciudad					CHAR(50),
    pR_Estado 					CHAR(50),
    pR_Telefono 				CHAR(15),
    pTipo_Pago 					CHAR(1),
    pOpCode 					CHAR(4), 
    pProcess_Msg		 		CHAR(255), 	
    pError_Param_Full_Name		CHAR(255), 
    pTrans_Status_Cd			CHAR(3), 
    pTrans_Status_Dt 			CHAR(8),
    pProcess_Dt 				CHAR(8), 
    pProcess_Tm 				CHAR(6), 
    pUsuario 					CHAR(8),
    pNumCte						CHAR(20)
)


	--DATOS A REGRESAR---
    RETURNING
    CHAR(5);   -- Codigo de Retorno
	
	 --DEFINICION DE VARIABLES--
    DEFINE sql_err                INT;
    DEFINE cCodRet                CHAR(5);
	DEFINE cAgent_Trans_Type_Code CHAR(4);
	DEFINE cAgent_Cd              CHAR(3);
	DEFINE cRegion_Sd             CHAR(15);
	DEFINE cBranch_Sd             CHAR(15);
	DEFINE cState_Cd              CHAR(3);
	DEFINE cCountry_Cd            CHAR(3);
	DEFINE vfec_nac				  DATE;
	DEFINE vCodRet				CHAR(5);
	DEFINE vcuenta				INTEGER;
	DEFINE p_moneda_origen 		CHAR(3);
	DEFINE p_importe_origen 	MONEY;
	DEFINE vCategoria			CHAR(2);
	DEFINE vConvenio			CHAR(5);
	
	
	
        --INICIALIZACION DE VARIABLES--
    LET sql_err                = 0;
    LET cCodRet                = '00000';
	LET cAgent_Trans_Type_Code = 'PAYI';
	LET cAgent_Cd              = '';
	LET cRegion_Sd             = '';
	LET cBranch_Sd             = '';
	LET cState_Cd              = '';
	LET cCountry_Cd            = '';
	LET vcuenta					= 0;
	LET vCodRet					= '00000';
	LET p_moneda_origen			= '';
	LET p_importe_origen		= 0;
	LET vCategoria				= '07';
	LET vConvenio				= '004';
	
	
	--SET DEBUG FILE TO "/respaldosbd/Dulce/sp_GuardaRespuestaPayi2.out";
    --TRACE ON;

    
	--SET DEBUG FILE TO "/ifxsif01/ENP/sp_GuardaRespuestaPayi2.out";
    --TRACE ON;

    
	
	BEGIN

    ON EXCEPTION SET sql_err
        IF sql_err <> 0 THEN
            LET cCodRet = sql_err;
            RETURN cCodRet;
        END IF;
    END EXCEPTION;
	
	IF pSucursal = "" OR  pSucursal IS NULL OR pTxn_Status = "" OR pTxn_Status IS NULL OR pConfirmation_nm = "" OR pConfirmation_nm IS NULL 
	    OR pBank_Ref_Num = "" OR pBank_Ref_Num IS NULL OR pUser_name = "" OR pUser_name IS NULL OR pTerminal = "" OR pTerminal IS NULL 
		OR pAgent_Dt = "" OR pAgent_Dt IS NULL OR pAgent_Tm = "" OR pAgent_Tm IS NULL 
		OR pR_First_Name = "" OR pR_First_Name IS NULL OR pR_Last_Name = "" OR pR_Last_Name IS NULL 
		OR pR_Type_Cd = "" OR pR_Type_Cd IS NULL OR pR_Issuer_Cd = "" OR pR_Issuer_Cd IS NULL 
		OR pR_Issuer_State_Cd = "" OR pR_Issuer_State_Cd IS NULL OR pR_Issuer_Country_Cd = "" OR pR_Issuer_Country_Cd IS NULL 
		OR pR_Identif_Nm = "" OR pR_Identif_Nm IS NULL OR pR_Expiration_Dt = "" OR pR_Expiration_Dt IS NULL 
		OR pUsuario = "" OR pUsuario IS NULL OR pR_pais_nac = "" OR pR_pais_nac IS NULL THEN
		LET cCodRet = "00001";
		RETURN cCodRet;
	END IF;
	
	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	
	EXECUTE PROCEDURE bdisac:"informix".sp_consultasucursal (pSucursal) 
	INTO cCodRet, cAgent_Cd, cRegion_Sd, cBranch_Sd, cState_Cd, cCountry_Cd;
	IF cCodRet = "00000" THEN
		INSERT INTO "informix".sac_bts_payi (txn_status, agent_trans_type_code, agent_cd, confirmation_nm, bank_ref_nm, region_sd, branch_sd, state_cd, 
		        country_cd, user_name, terminal, agent_dt, agent_tm, r_first_name, r_middle_name, r_last_name, r_mother_m_name, r_type_cd, 
				r_issuer_cd, r_issuer_state_cd, r_issuer_country_cd, r_identif_type, r_identif_nm, r_expiration_dt, r_fecha_nac, r_nacionalidad, r_pais_nac,
				r_nom_calle, r_num_ext, r_num_int, r_depto, r_colonia, r_cp, r_mncpo_deleg, r_ciudad, r_estado, r_telefono, tipo_pago, sucursal,
				opcode, process_msg, error_param_full_name, trans_status_cd, trans_status_dt, process_dt, process_tm, user_insert, fecha_insert,numcte)
		VALUES(pTxn_Status, cAgent_Trans_Type_Code, cAgent_Cd, pConfirmation_Nm, pBank_Ref_Num, cRegion_Sd, cBranch_Sd, cState_Cd,
         		cCountry_Cd, pUser_Name, pTerminal, pAgent_Dt, pAgent_Tm, pR_First_Name, pR_Middle_Name, pR_Last_Name, pR_Mother_M_Name, pR_Type_Cd,  
				pR_Issuer_Cd, pR_Issuer_State_Cd, pR_Issuer_Country_Cd, pR_Identif_Type, pR_Identif_Nm, pR_Expiration_Dt, pR_Fecha_Nac, pR_Nacionalidad, pR_pais_nac,
				pR_Nom_Calle, pR_Num_Ext, pR_Num_Int, pR_Depto, pR_Colonia, pR_Cp, pR_Mncpo_Delg, pR_Ciudad, pR_Estado, pR_Telefono, pTipo_Pago, pSucursal, 
				pOpCode, pProcess_Msg, pError_Param_Full_Name, pTrans_Status_Cd, pTrans_Status_Dt, pProcess_Dt, pProcess_Tm, pUsuario, CURRENT,pNumCte);
				
		--Busco datos de query
		EXECUTE PROCEDURE bdisac:"informix".sp_obtieneremadic(vCategoria, vConvenio, pConfirmation_Nm)
		INTO vCodRet, p_moneda_origen, p_importe_origen;
		
		--Actualizo tabla de datos para limites de remesas mensuales
		LET vfec_nac = MDY(SUBSTRING(pR_Fecha_Nac FROM 5 FOR 2), SUBSTRING(pR_Fecha_Nac FROM 7 FOR 2), SUBSTRING(pR_Fecha_Nac FROM 1 FOR 4));
		EXECUTE PROCEDURE bdisac:"informix".sp_actualizaremesa(vCategoria, vConvenio, pConfirmation_Nm, pR_First_Name, pR_Middle_Name, pR_Last_Name, pR_Mother_M_Name, vfec_nac, p_moneda_origen, p_importe_origen)
		INTO vCodRet, vcuenta;

            ---------------------------------------------------------------------------------
        IF
            pSucursal <> '' or pSucursal IS NOT NULL THEN
            IF EXISTS (SELECT * FROM bdinteg:"informix".si_sucursales WHERE sucursal NOT IN ('9250', '9251', '9764','5011','5003') AND sucursal = pSucursal) THEN
                EXECUTE PROCEDURE bdinteg:"informix".sp_inserta_msjafore(pNumcte,'',pSucursal,'') INTO cCodRet;
                IF 
                    cCodRet <> '00000' THEN
                    INSERT INTO bdisac:"informix".sac_procesos_jobs(proceso, fecha_proceso, status, user_insert, fecha_insert, numero_ejecuciones, nombre_sp, descripcion)
                    VALUES ('sp_pago_wu_web', CURRENT, '0', 'informix', CURRENT, NULL, 'sp_inserta_msjafore', 'Codigo retorno: '|| cCodRet);
                END IF;
            END IF;
        END IF;
        ----------------------------------------------------------------------------------
       
		
	END IF;
	
	RETURN cCodRet;
END
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se crea SP para guardar los datos de envio y recepciÃÂ³n del mensaje PAYI de BTS',
'AUTOR : Dulce Ramirez',
'FECHA : 05/Enero/2011',
'Ver.  : 1.1',
'BD    : bdisac',
'VER   : 1.1',
'MODIFICO: Felipe Urias',
'FECHA : 12/Abril/2012',
'DESCRIPCION: se agrego el campo r_pais_nac como parametro a guardar en tabla',
'MODIFICO: FRG',
'FECHA : 09/Mayo/2012',
'DESCRIPCION: se clona el SP con nombre sp_guardarespuestapayi2.sql',
'para no afectar el flujo actual en Prod. al guardar el campo r_pais_nac',
'****************************************************************************************************',
'DescripciÃÂ³n: Se inserta campo numcte para utilizar en remesas',
'Autor      : Geovani Garcia Ochoa',
'FECHA DE CREACION    : 28/02/2017',
'BD         : bdisac',
'FOLIO		: 198 - RQM 10 784 B ASE DE DATOS PARA EL ALTA DE USUARIOS DE REMESAS',
'-------------------------------------------------------------------------------------------------';

CREATE PROCEDURE "informix".sp_soldespagoskyonline(pFolioSuc char(16)) 
	--RETORNOS
	RETURNING
	CHAR(5) AS cCodigoRet, CHAR(850) AS cTrama; 
	
	--Definicion de Variables
	DEFINE cCodigoRet  	CHAR(5);
	DEFINE cTrama    	CHAR(850);
	DEFINE iSqlErr 		INTEGER;
	DEFINE cEnte_id 			CHAR(3);
	DEFINE cNumero_cuenta 	    CHAR(12);
	DEFINE cFecha_depo_banco    CHAR(10);
	DEFINE cImporte_transaccion CHAR(13);
	DEFINE cAutorizacion 	CHAR(10);
	DEFINE cMpel_id 		CHAR(15);
	DEFINE cUsoFuturo1 		CHAR(256);
	DEFINE cUsoFuturo2 		CHAR(256);
	DEFINE cUsoFuturo3 		CHAR(256);
	DEFINE cFolio_pago 		CHAR(10);
	DEFINE cTrancinterac    CHAR(5);
	DEFINE cTrancservice    CHAR(5);
	
	--Inicializacion de variables
	LET cCodigoRet = '00000';
	LET cTrama = '';
	LET iSqlErr = 0;
	LET cEnte_id ='';
	LET cNumero_cuenta ='';
	LET cFecha_depo_banco ='';
	LET cImporte_transaccion ='';
	LET cAutorizacion =''; 	
	LET cMpel_id ='';		
	LET cUsoFuturo1 	='';	
	LET cUsoFuturo2 	=''; 		
	LET cUsoFuturo3 	='';
	LET cFolio_pago 	=''; 
	LET cTrancinterac 	='';
	LET cTrancservice   ='';
	
	--SET DEBUG FILE TO '/home/sysifx/JesusAlbertoLI';
	--TRACE ON;
	
	BEGIN 
		ON EXCEPTION SET iSqlErr
			IF iSqlErr <> 0 THEN
				LET cCodigoRet = iSqlErr;
				RETURN TRIM(NVL(cCodigoRet,"")),TRIM( NVL(cTrama,""));
			END IF;
		END EXCEPTION;
		
		SET LOCK MODE TO WAIT 3;
		SET ISOLATION TO dirty READ;
		
		
		--Validamos parÃ¡metros para que no sean nulos
		IF NVL(pFolioSuc,'') = '' THEN
			 LET cCodigoRet = '00001';			 RETURN TRIM(NVL(cCodigoRet,"")),TRIM( NVL(cTrama,""));
        ELSE
		
		if (SELECT COUNT(*) FROM "informix".sac_sky_wsgpago  WHERE folio_suc = pFolioSuc AND id_respuesta ='000'  ) = 0 then 
			 LET cCodigoRet = '00002';			 RETURN TRIM(NVL(cCodigoRet,"")),TRIM( NVL(cTrama,""));
		END IF;
		SELECT trans_interact
		INTO cTrancinterac		
		FROM bdisac: "informix".sac_intrfz_serv  
		WHERE numcategoria = '06' and numconvenio='001' and num_trama = '2';
		
		
		SELECT trans_servicio
		INTO cTrancservice		
		FROM bdisac: "informix".sac_intrfz_serv  
		WHERE numcategoria = '06' and numconvenio='001' and num_trama = '2';
		
		
		SELECT valor 
		INTO cEnte_id 
		FROM bdisac: "informix".sac_param 
		WHERE cod_param = '114';
		
		SELECT referencia1 
		INTO cNumero_cuenta 
		FROM bdisac: "informix".sac_movimientos 
		WHERE folio_suc = pFolioSuc;
		
		--Fecha actual del sistema
		LET cfecha_depo_banco =
		SUBSTR(CURRENT, 9,  2)     ||'/'|| -- DD 
		SUBSTR(CURRENT, 6,  2)     ||'/'|| -- MM  
		SUBSTR(CURRENT, 1,  4)     ||' '|| -- AAAA   
					'';
		
		SELECT importe_pago 
		INTO cImporte_transaccion 
		FROM bdisac: "informix".sac_movimientos 
		WHERE folio_suc = pFolioSuc;
		
		LET cFolio_pago = SUBSTR(pFolioSuc, 7,  10);
		
		SELECT autorizacion
		INTO cAutorizacion
		FROM bdisac: "informix".sac_sky_wsgpago 
		WHERE folio_suc = pFolioSuc;
		
		SELECT mpel_id 
		INTO cMpel_id
		FROM bdisac: "informix".sac_sky_wsgpago 
		WHERE folio_suc = pFolioSuc;
		
		--concatenar todas la variables en cTrama
LET cTrama = NVL(cTrancinterac,'') || NVL(cTrancservice,'') || NVL(cEnte_id,'') || NVL(cNumero_cuenta,'') || NVL(cFecha_depo_banco,'') || NVL(SUBSTR(cImporte_transaccion,2,12),'0.00') || NVL(cFolio_pago,'') || NVL(cAutorizacion,'') || NVL(cMpel_id,'') || NVL(cUsoFuturo1,'') || NVL(cUsoFuturo2,'') || NVL(cUsoFuturo3,'');	
 
	END IF;
	
	RETURN TRIM(NVL(cCodigoRet,'')),NVL(cTrama,'');
	
	END;
END PROCEDURE;