CREATE PROCEDURE "informix".sp_consultatotalhistorialstatusmc(pUsuario CHAR(8), pIdFuncion CHAR(10),pEmpresa CHAR(3),pNumSol CHAR(20)) 
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultatotalhistorialstatusmc.out';
		--TRACE ON;
		
		IF NVL(pUsuario, '') = '' OR NVL(pIdFuncion, '') = '' OR NVL(pEmpresa, '') = '' OR NVL(pNumSol, '') = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet, iNoRegs;
		END IF;
		...
		FOREACH
			EXECUTE PROCEDURE bdisolic:'informix'.sp_mc_obtenhistorialstatus(pEmpresa, pNumSol) 
...
END PROCEDURE
DOCUMENT 
'DESCRIPCION: Sp Intermedio bdisolic:sp_mc_obtenhistorialstatus para la obtencion del total de registros del historial de los status de la solicitud',
'AUTOR : Oscar Flores Conde',
'FECHA : 14/01/2014',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_consultatotreporteoperacionesmc(pUsuario CHAR(8), pIdFuncion CHAR(10), pProducto CHAR(4), pFechaInicial DATE, pFechaFinal DATE)
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_consultatotreporteoperacionesmc.out';
		--TRACE ON;
