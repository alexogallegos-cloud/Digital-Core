CREATE PROCEDURE "informix".sp_depurar_telefonos_incorrectos()
	...
	DEFINE iExiste_tmp_si_telefonos_actual SMALLINT;
	
	DEFINE iError			INTEGER;
	
	--INICIALIZACION DE VARIABLES
	LET vCodRet 				= '000000';
	LET cEstado 				= 'PROCESO DE DEPURACION DE TELEFONOS INCORRECTOS EXITOSO';
	LET cDescErr				= '';
	LET iEnTransaccion 			= 0;
	LET cProceso 				= 'sp_depurar_telefonos_incorrectos';
	...
	LET iExiste_tmp_si_telefonos_actual = 0;

	LET iError			= 0;
	
	--SET DEBUG FILE TO "/tmp/josea/64136/sp_depurar_telefonos_incorrectos.out";
	--TRACE ON;
	...
	BEGIN
		ON EXCEPTION SET iNomErr, iIsamErr, cDescErr
			IF iNomErr <> 0 THEN
				...
				LET cEstado = 'PROCESO DE DEPURACION DE TELEFONOS INCORRECTOS GENERADO CON ERRORES';
				INSERT INTO bdinteg: si_log_depuracion_telefonos (fecha, telefono, proceso, evento, cod_error, mensaje, user_insert, fecha_insert)
				VALUES (CURRENT, cTelDepura, cProceso, cEvento, vCodRet, cDescErr, USER, (SELECT DBINFO('utc_to_datetime',sh_curtime)FROM sysmaster:"informix".sysshmvals));
		...
		IF EXISTS (SELECT 1 FROM bdinteg:si_param WHERE cod_param = 385) THEN
			...
			IF NVL(cCantidadTels,'0')::INTEGER > 0 THEN			
				...
				SET ISOLATION TO DIRTY READ;
				SET LOCK MODE TO WAIT 3;
				IF EXISTS(SELECT {+INDEX(bdinteg:si_telefonos_incorrectos idx_si_telefonos_incorrectos_01)} 1 FROM bdinteg:si_telefonos_incorrectos WHERE estatus = '0')THEN
												
					SET ISOLATION TO DIRTY READ;
					SET LOCK MODE TO WAIT 3;
					
					LET cEvento = 'OBTIENE NUMEROS INCORRECTOS';
					FOREACH WITH HOLD
						SELECT LIMIT iCantidadTels id, telefono
						INTO iId, cTelDepura
						FROM bdinteg:si_telefonos_incorrectos
						WHERE estatus = '0'
						ORDER BY id ASC
						...
						BEGIN
							ON EXCEPTION SET iNomErr, iIsamErr, cDescErr
								...
								IF iNomErr <> 0 THEN
									SET ISOLATION TO DIRTY READ;
									SET LOCK MODE TO WAIT 3;
									UPDATE si_telefonos_incorrectos
									SET estatus = '2', cod_retorno = iNomErr, proceso = cDescErr, fecha_proceso = CURRENT::DATE, fecha_hora_proceso = (SELECT DBINFO('utc_to_datetime',sh_curtime)FROM sysmaster:"informix".sysshmvals)
									WHERE id = iId;
										
									INSERT INTO bdinteg: si_log_depuracion_telefonos (fecha, telefono, proceso, evento, cod_error, mensaje, user_insert, fecha_insert)
									VALUES (CURRENT, cTelDepura, cProceso, cEvento, iNomErr::CHAR(6), cDescErr, USER, (SELECT DBINFO('utc_to_datetime',sh_curtime)FROM sysmaster:"informix".sysshmvals));
							...
							IF EXISTS(SELECT 1 FROM si_telefonos_actual WHERE telefono = cTelDepura ) THEN							
								
								LET cEvento = 'INICIA DEPURACION DE NUMERO TELEFONICO';
								FOREACH 
									...
									LET cEvento = 'ELIMINA NUMERO TELEFONICO';
									DELETE {+INDEX(bdinteg:si_telefonos_actual  5950_2841)} 
									FROM si_telefonos_actual WHERE numcte = cNumCteDepurar AND secuencia = iSecuencia;											
									
									LET cEvento = 'CANCELA NUMERO TELEFONICO';
									UPDATE bdinteg:si_telefonos
									SET status_tel = "C"
									WHERE numcte = cNumCteDepurar AND secuencia = iSecuencia;
									
									LET cEvento = 'sp_valrevtelefonos: MARCA CLIENTE PARA ACTUALIZAR TELEFONOS';
									...
									IF TRIM(vCodRetRev) <> '000' THEN
										LET cEvento = 'REVERSO CONTROLADO';
										ROLLBACK WORK;
										LET iEnTransaccion = 0;

										SET ISOLATION TO DIRTY READ;
										UPDATE si_telefonos_incorrectos
										SET estatus = '2', cod_retorno = vCodRetRev, proceso = 'sp_valrevtelefonos', fecha_proceso = CURRENT::DATE, fecha_hora_proceso = (SELECT DBINFO('utc_to_datetime',sh_curtime)FROM sysmaster:"informix".sysshmvals)
										WHERE id = iId;

										INSERT INTO bdinteg: si_log_depuracion_telefonos (fecha, telefono, proceso, evento, cod_error, mensaje, user_insert, fecha_insert)
										VALUES (CURRENT, cTelDepura, cProceso, cEvento, vCodRetRev, 'sp_valrevtelefonos', USER, (SELECT DBINFO('utc_to_datetime',sh_curtime)FROM sysmaster:"informix".sysshmvals));
								...
								IF iError = 0 THEN
									...
									UPDATE si_telefonos_incorrectos
									SET estatus = '1', cod_retorno = '000000', proceso = 'TELEFONO DEPURADO', fecha_proceso = CURRENT::DATE, fecha_depuracion = CURRENT::DATE, fecha_hora_proceso = (SELECT DBINFO('utc_to_datetime',sh_curtime)FROM sysmaster:"informix".sysshmvals)
									WHERE id = iId;
								END IF;
							ELSE
								LET cEvento = 'ACTUALIZA INSTRUCCION DE DEPURACION';
								SET ISOLATION TO DIRTY READ;
								SET LOCK MODE TO WAIT 5;
								UPDATE si_telefonos_incorrectos
								SET estatus = '2', cod_retorno = '000006', proceso = 'TELEFONO NO DEPURADO: NO EXISTE EN BD', 
									fecha_proceso = CURRENT::DATE, fecha_hora_proceso = (SELECT DBINFO('utc_to_datetime',sh_curtime)FROM sysmaster:"informix".sysshmvals)
								WHERE id = iId;
							END IF;
...
DOCUMENT
'FECHA: 19/01/2016',
'RQI 64 136 DEPURACION DE TELEFONOS INCORRECTOS',
'DESCRIPCION: PROCEDIMIENTO PARA DEPURAR LOS NUMEROS DE TELEFONOS INCORRECTOS, TRANSACCION POR NUMERO TELEFONICO',
'BD: BDINTEG';

CREATE PROCEDURE "informix".sp_datoscte_ivr(pEmpresa CHAR(3),pNumCte CHAR(20))
...
CREATE PROCEDURE "informix".sp_insbitformatoimpresion(pEmpresa CHAR(3),pNumCte CHAR(20),pNumTel CHAR(13),pEjecutivo CHAR(8),pSucursal CHAR(4))
	RETURNING CHAR(6) AS CodRet;
...
CREATE PROCEDURE "informix".sp_insbitllamadatelcte(pEmpresa char(3),pNumCte char(20),pNumTel char(13),pEjecutivo char(8),pSucursal char(4),pFolio char(4))
