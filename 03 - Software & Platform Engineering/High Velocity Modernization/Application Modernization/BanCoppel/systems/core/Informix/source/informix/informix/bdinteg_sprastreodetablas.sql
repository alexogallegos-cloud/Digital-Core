CREATE PROCEDURE "informix".sprastreodetablas(pRegistros INTEGER)
RETURNING
CHAR(5) AS CodRet,
CHAR(3) AS Id,
CHAR(50) AS NombreTabla;

	DEFINE iSqlErr INTEGER;
	DEFINE iIsamlErr INTEGER;
	DEFINE cCodRet CHAR(5);
	DEFINE cId CHAR(3);
	DEFINE cNombreTabla CHAR(50);
...
BEGIN
	ON EXCEPTION SET iSqlErr,iIsamlErr
	    IF iSqlErr <> 0 THEN
	        LET cCodRet = iSqlErr;
			RETURN cCodRet ,cId ,cNombreTabla;
	    END IF;
	END EXCEPTION;

	 --SET DEBUG FILE TO '/respaldosbd/mario/sprastreodetablas.out';
	 --TRACE ON;
	...
	FOREACH
		SELECT  nom_tabla INTO cNombreTabla FROM bdinteg:"informix".si_catgenerico 
		
      	    LET iConteo = iConteo+1;
      	    IF iConteo <= pRegistros THEN
	           CONTINUE FOREACH;
      	    END IF;			
			
			RETURN cCodRet ,cId ,cNombreTabla WITH RESUME;
	END FOREACH;
...
END PROCEDURE
DOCUMENT
'Folio: 1750 - HomologacionTablasLogin ',
'Autor: 95142134 Mario Gallardo',
'Fecha: 08/09/2015',
'Modificación:Se crea procedimiento para obtener todos los registros de la tabla si_catgenerico',
'Sustento: 6944- AT RQI 24 042 Homologacion de tablas-20150727_0157209 .pdf',
'BD: bdinteg ',
'Solicita: Cutberto Gonzales',
'*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*',
'Petición: 66 - RQI 24 023 Homologacion de tablas',
'Autor: 96591293 Christian Rodríguez Guzmán',
'Fecha: 03/06/2016',
'Modificación: Se modifica procedimiento para quitar el campo id de la consulta ya que elimino de la tabla si_catgenerico. ',
'              NOTA: Se sigue utilizando la variable cId en el retorno, ya que a petición de Bancoppel no se modificara ',
'                    el componente LOGIN, esto es para no afectar los parametros de retorno que el aplicativo espera. ',
'BD: bdinteg',
'Solicita: SOSIMO DE JESUS VERDUGO RAMOS';
