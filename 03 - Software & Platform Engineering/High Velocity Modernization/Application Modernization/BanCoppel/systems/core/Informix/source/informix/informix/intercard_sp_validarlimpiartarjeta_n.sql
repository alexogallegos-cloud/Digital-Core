...
--DSB PAY INICIO
LET cBinTar = SUBSTR(pcNumTarjetaNuevo,1,6);
...
--DSB PAY FIN
	...
	IF piOpcion = 1 THEN										--DSB20140318
		...
		--Validacion para verificar si la tarjeta anterior esta completamente cancelada.
			...
			IF EXISTS(SELECT numtarjeta FROM intercard:tarjeta_indicadores WHERE numtarjeta = pcNumTarjetaNuevo) THEN 
    			DELETE FROM intercard:tarjeta_indicadores WHERE numtarjeta = pcNumTarjetaNuevo;
			END IF;
						--Se comenta este bloque de codigo ya que la afectaciÃ³n de estas tablas se debe realizar por medio de interact rollback
	...
	ELSE 													--DSB20140318 {
		IF piOpcion = 2 THEN
		--Se agrega validaciÃ³n para solo eliminar de las bases de datos bdicred o bdicheq en caso de que la tarjeta no este asignada en intercard
			IF EXISTS(SELECT fechaasignacion FROM "informix".tarjeta WHERE numtarjeta = pcNumTarjetaNuevo AND fechaasignacion IS NULL) THEN
				...
				ELSE
					IF EXISTS (SELECT num_tarjeta FROM bdicred:"informix".sd_tarjeta WHERE empresa = '001' AND num_tarjeta = pcNumTarjetaNuevo) THEN
					-- AAME RQI 27 221 Se agrega actualizacion de estatus de de tarjeta anterior
						UPDATE bdicred:"informix".sd_tarjeta SET status_tar ='A' WHERE num_tarjeta =pcNumTarjetaAnterior;
...
'Folio.........: 1399-INC_TarjetasIncompletasEnReposiciÃ³n',
'Autor.........: 95526749 - JesÃºs Horacio LÃ³pez GonzÃ¡lez',
'Fecha.........: 18/03/2014 - DSB20140318',
'ModificaciÃ³n..: Se clona sp_validarlimpiartarjeta y se agrega parametro para limpiar la tabla sc_tarjeta o sd_tarjeta si ocurre un problema',
'                al momento de realizar el alta de la tarjeta mediante el Interact',
'Sustento......: INC-24-123 Suc. ReposiciÃ³n incompleta v1.0.pdf',
'Solicita......: Cutberto Gonzalez',
'BD............: INTERCARD',
'Descripcion: Se agrega validacion para eliminar registro de la tabla tarjeta_indicadores.',
'Base de Datos : intercard',
'Autor : Victor Vazquez',
'Fecha : 06/Febrero/2024';

CREATE PROCEDURE "informix".sp_consultatarjetabin(pEmpresa CHAR(3), pLote INTEGER, pTarjetaini CHAR(16), pTarjetafin CHAR(16))
