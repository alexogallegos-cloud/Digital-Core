CREATE PROCEDURE "informix".sp_genrepremesasbts_especial(vFechaInicial DATE, vFechaFinal DATE)

RETURNING CHAR(5) AS CodRetorno, CHAR(200) AS Mensaje;

--****************************************************************************************************
-- DESCRIPCION: REPORTE DE REMESAS BTS EXTRAORDINARIO
...
/*DSB ALFONSO CRUZ*/
-- 2011.12.06 I Se asiga la fecha del día tipo DATE:
...
-- 2011.12.06 I No se consideran porque ya va de entrada el rango de fechas.
-- 2011.12.06 I Se asiga la fecha del día tipo DATE:
...
-- 2011.12.06 I Se definen valores en base a las fechas de entrada:
...
-- 2011.12.06 F
	...
	IF (vsCodRetorno='00000') THEN
		...
		FOREACH
			...
			SELECT 
				{+INDEX (bdisac:sac_movimientoshistorial idxsac_movhisfe)}
...
-- 2011.12.05 I Se asignan rangos de fecha a la entrada del SP:
		    --	mov.fecha_pago BETWEEN DATE(vdFechaPri) AND DATE(vdFechaFin) AND
			mov.fecha_pago BETWEEN vFechaInicial AND vFechaFinal AND
-- 2011.12.05 F
...
-- 2011.12.05 I Se modifica la obtención de registros de sac_bts_qryi para omitir caracteres raros:
--			SELECT FIRST 1 
			--	{+INDEX (bdisac:bdisac:sac_bts_qryi idx_btsqryi)}
			--	s_first_name,
			--	s_middle_name,
			--	s_last_name, 
			--	s_mother_m_name,
