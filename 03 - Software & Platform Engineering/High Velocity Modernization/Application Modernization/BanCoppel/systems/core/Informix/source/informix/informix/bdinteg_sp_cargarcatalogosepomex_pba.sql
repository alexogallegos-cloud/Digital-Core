CREATE PROCEDURE "informix".sp_cargarcatalogosepomex_pba()
...
SET DEBUG FILE TO "sp_cargarcatalogosepomex_pba.out";    
TRACE ON; 
...
IF (cRuta is not null) and (cNombre is not null) and (cRuta <> '') and (cNombre <> '') then
    ...
    LET cNombre2 = SUBSTR(cNombre,1,LENGTH(cNombre)) || v_dia || v_mes || v_anio || '_res.txt';
    --LET cNombre = trim(cNombre) || v_dia || v_mes || v_anio || '.txt';
    LET cNombre = SUBSTR(cNombre,1,LENGTH(cNombre)) || v_dia || v_mes || v_anio || '.txt';
    ...
    IF EXISTS (SELECT VALOR FROM tmp_valida_catsepomex where valor = trim(cRuta) || trim(cNombre)) THEN
        ...
        FOREACH
        SELECT d_codigo, d_asenta, d_tipo_asenta, d_mnpio, d_estado, d_ciudad, c_estado
        into v_d_codigo, v_d_asenta, v_d_tipo_asenta, v_d_mnpio, v_d_estado, v_d_ciudad, v_c_estado
        FROM bdinteg:tmp_si_catsepomex
			--- VALIDA BANDERA DE EXISTENCIA DE DATOS DE LA TABLA DE CATALOGO DE SEPOMEX
			IF v_Band1 = "1" THEN
				LET v_Status = 2;
			ELSE
				--- VALIDA QUE EXISTA EN EL CATALOGO DE SEPOMEX 
				IF EXISTS (SELECT d_codigo FROM bdinteg:si_catsepomex WHERE d_codigo = v_d_codigo 
						AND TRIM(d_asenta) = TRIM(v_d_asenta) AND TRIM(d_tipo_asenta) = TRIM(v_d_tipo_asenta)
						AND TRIM(d_estado) = TRIM(v_d_estado) AND TRIM(d_ciudad) = TRIM (v_d_ciudad)
						) THEN
					LET v_Status = 1;
				...
				ELSE
					...
					IF EXISTS(SELECT d_codigo FROM bdinteg:si_catsepomex 
							WHERE TRIM(d_asenta) = TRIM(v_d_asenta) AND TRIM(d_tipo_asenta) = TRIM(v_d_tipo_asenta)
							AND TRIM(d_estado) = TRIM(v_d_estado) AND TRIM(d_ciudad) = TRIM (v_d_ciudad)
							) THEN
						LET v_Status = 3;
            ...
            INSERT INTO bdinteg:si_catsepomex (d_codigo,d_asenta,d_tipo_asenta,d_mnpio,d_estado,d_ciudad,c_estado,estatus,fecha_ejecucion)
            VALUES(v_d_codigo, v_d_asenta, v_d_tipo_asenta, v_d_mnpio, v_d_estado, v_d_ciudad, v_c_estado, v_Status, v_fecha);
