CREATE PROCEDURE "informix".sp_ht_obtener_tels
(
pUsuario CHAR(8)
)
RETURNING CHAR(6),
    ...
    --- SET DEBUG FILE TO '/informix/moha/sp_ht_obtener_tels.out';
    --- TRACE ON;
    ...
    SELECT fecha_hoy, SUBSTR(CURRENT::DATETIME HOUR TO SECOND,1,2) || SUBSTR(CURRENT::DATETIME HOUR TO SECOND,4,2)
      INTO cFechaHoy, cHoraMinHoy
      FROM "informix".si_fechas
     WHERE empresa = "001";
    
	IF EXISTS(SELECT status FROM "informix".si_ht_controlproc WHERE fecha = MDY(MONTH(cFechaHoy),20,YEAR(cFechaHoy)) AND status = 0) THEN
		--// CREACION DEL FOLIO
		LET cFolioArchivo =  "137" || SUBSTR(YEAR(cFechaHoy),3,2) || LPAD(MONTH(cFechaHoy),2,"0") || LPAD(DAY(cFechaHoy),2,"0") || cHoraMinHoy;
		...
		FOREACH 
			...
			IF iBandera = 0 THEN
				...
				IF iExiste = 0 THEN
					INSERT INTO "informix".si_ht_detalle_ctrl_tels
					(folio_archivo,num_cte,fecha_cte,nombre1,nombre2,apell_paterno,apell_materno,nombre_edo,nombre_ciudad,telefono,tipo_tel,carrier,tipo_prod)
					VALUES
					(cFolioArchivo, cCte, dtFechaCte, cNombre1, cNombre2, cApellPaterno, cApellMaterno, cNomEstado, cNomCiudad, cTelefono, iTipoTel, iCarrier, "C");
				END IF
			END IF
		END FOREACH
		...
		LET iTotRegsPend = NVL(iTotRegsPend,0);
		...
		LET iTotTelsAmbos = NVL(iTotTelsAmbos,0);
		...
		LET iTotTelsFijo = NVL(iTotTelsFijo,0);
		...
		LET iTotTelsCel = NVL(iTotTelsCel,0);
		
		-- SE GENERA EL RESUMEN DEL ARCHIVO
		INSERT INTO "informix".si_ht_resumen_ctrl_tels
		(folio_archivo,status,tot_reg,tot_ctes,tels_enviados,tels_validos,tels_invalidos,tels_pend,tels_duplicados,tot_telsambos,tot_telsfijo,tot_telscel,tot_telsfijoval,tot_telsfijoinval,tot_telscelval,tot_telscelinval,usuario)
		VALUES 
		(cFolioArchivo, 0, iTotRegs, iTotCtes, 0, 0, 0, 0, iNumTelsDup, iTotTelsAmbos,  iTotTelsFijo, iTotTelsCel, 0, 0, 0, 0, pUsuario);
		
		-- SE ACTUALIZA LA TABLA DE RESUMEN
		UPDATE "informix".si_ht_resumen_ctrl_tels
		SET tot_ctes = iTotCtes, tels_pend = iTotRegsPend, tot_telsambos = iTotTelsAmbos, tot_telsfijo = iTotTelsFijo, tot_telscel = iTotTelsCel
		WHERE folio_archivo = cFolioArchivo;
		
		-- SE INSERTA REGISTRO EN LA TABLA DE CONTROL DE PROCESOS PARA DEJAR EVIDENCIA DE QUE YA SE GENERO
		UPDATE "informix".si_ht_controlproc
		SET status = 1
		WHERE fecha = MDY(MONTH(cFechaHoy),20,YEAR(cFechaHoy));
	END IF
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Proceso para obtener los telefonos de los clientes de captación y de crédito y validar el número con el proceso de marcatel y',
			  'validar si no existe en el catálogo de teléfONos inválidos',
'BD: bdicheq', 
