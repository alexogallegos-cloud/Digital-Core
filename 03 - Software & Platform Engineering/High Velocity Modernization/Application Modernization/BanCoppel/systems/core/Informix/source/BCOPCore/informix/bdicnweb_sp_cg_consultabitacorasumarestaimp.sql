CREATE PROCEDURE "informix".sp_cg_consultabitacorasumarestaimp(pUsuario CHAR(8), pIdFuncion CHAR(10), pAnio INTEGER, pMes INTEGER, pRegistros INTEGER, pRecuperacion INTEGER)
RETURNING CHAR(5)   AS  codret,
	  ...
	  INTEGER  AS cant_m_100,			
	  INTEGER  AS cant_m_50,			
	  INTEGER  AS cant_m_20,			
	  INTEGER  AS cant_m_10,			
	  INTEGER  AS cant_m_5,			
	  INTEGER  AS cant_m_2,			
	  INTEGER  AS cant_m_1,			
	  INTEGER  AS cant_m_05,			
	  INTEGER  AS cant_m_02,			
	  INTEGER  AS cant_m_01,			
	  INTEGER  AS cant_m_005;			
...
DEFINE cDescripcion 	 CHAR(1);
DEFINE dSaldo 			 MONEY(19,2);
DEFINE iCantidad_1000	 INTEGER;
DEFINE iCantidad_500	 INTEGER;
DEFINE iCantidad_200	 INTEGER;
DEFINE iCantidad_100	 INTEGER;
DEFINE iCantidad_50		 INTEGER;
DEFINE iCantidad_20		 INTEGER;
DEFINE iCant_m_100		 INTEGER;
DEFINE iCant_m_50		 INTEGER;
DEFINE iCant_m_20		 INTEGER;
DEFINE iCant_m_10		 INTEGER;
DEFINE iCant_m_5		 INTEGER;
DEFINE iCant_m_2		 INTEGER;
DEFINE iCant_m_1		 INTEGER;
DEFINE iCant_m_05		 INTEGER;
DEFINE iCant_m_02		 INTEGER;
DEFINE iCant_m_01		 INTEGER;
DEFINE iCant_m_005		 INTEGER;
...
LET cUsuario 		    ='';
LET cNombre			    ='';
LET dFechaModificacion  =0;
LET cDescripcion 	    ='';
LET dSaldo 			    =0.0;
LET iCantidad_1000	    =0;
LET iCantidad_500	    =0;
LET iCantidad_200	    =0;
LET iCantidad_100	    =0;
LET iCantidad_50		=0; 
...
BEGIN
	...
	ON EXCEPTION SET iSqlErr
		LET cCodRet = iSqlErr;
		RETURN cCodRet,cEmpresa, cCaja_General,cUsuario, cNombre, dFechaModificacion, cDescripcion, dSaldo, iCantidad_1000, iCantidad_500, iCantidad_200, iCantidad_100, iCantidad_50, 
		iCantidad_20, iCant_m_100, iCant_m_50, iCant_m_20, iCant_m_10, iCant_m_5,iCant_m_2, iCant_m_1,iCant_m_05, iCant_m_02, iCant_m_01,iCant_m_005;
	END EXCEPTION;
	
	--SET DEBUG FILE TO '/tmp/mfinis/sp_cg_consultabitacorasumarestaimp.out';
	--TRACE ON;
	
	IF pUsuario = '' OR pIdFuncion = '' OR pAnio IS NULL OR pMes IS NULL OR pRegistros IS NULL OR pRecuperacion IS NULL THEN
		LET cCodRet = '00003';
		RETURN cCodRet,cEmpresa, cCaja_General,cUsuario, cNombre, dFechaModificacion, cDescripcion, dSaldo, iCantidad_1000, iCantidad_500, iCantidad_200, iCantidad_100, iCantidad_50, 
		iCantidad_20, iCant_m_100, iCant_m_50, iCant_m_20, iCant_m_10, iCant_m_5,iCant_m_2, iCant_m_1,iCant_m_05, iCant_m_02, iCant_m_01,iCant_m_005;
	END IF;
	...
	FOREACH		
		SELECT SKIP pRegistros FIRST pRecuperacion empresa, a.caja_general||''||b.descripcion, usuario, nombre, fecha_modificacion, a.descripcion, saldo, cantidad_1000, cantidad_500, cantidad_200, cantidad_100, cantidad_50, cantidad_20, cant_m_100, cant_m_50, cant_m_20, cant_m_10, cant_m_5, cant_m_2, cant_m_1, cant_m_05, cant_m_02, cant_m_01, cant_m_005 
		INTO cEmpresa, cCaja_General,cUsuario, cNombre, dFechaModificacion, cDescripcion, dSaldo, iCantidad_1000, iCantidad_500, iCantidad_200, iCantidad_100, iCantidad_50, 
		iCantidad_20, iCant_m_100, iCant_m_50, iCant_m_20, iCant_m_10, iCant_m_5,iCant_m_2, iCant_m_1,iCant_m_05, iCant_m_02, iCant_m_01,iCant_m_005
		FROM bdisuc:"informix".ss_bitacora_importe a INNER JOIN bdisuc:'informix'.ss_proveedores b ON b.cod_proveedor=a.caja_general
		WHERE YEAR(fecha_modificacion) =pAnio 
		AND MONTH(fecha_modificacion)=pMes 
		ORDER BY id_serial ASC	
		
		LET iNoRegistros = iNoRegistros + 1;
		
		RETURN cCodRet,cEmpresa, cCaja_General,cUsuario, cNombre, dFechaModificacion, cDescripcion, dSaldo, iCantidad_1000, iCantidad_500, iCantidad_200, iCantidad_100, iCantidad_50, 
		iCantidad_20, iCant_m_100, iCant_m_50, iCant_m_20, iCant_m_10, iCant_m_5,iCant_m_2, iCant_m_1,iCant_m_05, iCant_m_02, iCant_m_01,iCant_m_005 WITH RESUME;
...
'MODULO: CAJA GENERAL',
'FUNCIONALIDAD: : Aumento Resta Saldos Caja General ',
'DESCRIPCION: SP que consulta informaciÃ³n de la tabla historica bdisuc:ss_bitacora_importe, utilizada para guardar los registros modificados de la tabla bdisuc:ss_cajageneral',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_cg_consultaranios(pUsuario CHAR(8), pIdFuncion CHAR(10), pAnioInicial INTEGER)
		RETURNING CHAR(5) AS codret,
		CHAR(4)   AS cve,
		CHAR(4)   AS anio;         
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cCve,cAnio;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cg_consultaranios.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = ''   OR pAnioInicial IS NULL THEN
			LET cCodRet = '00003';
				RETURN cCodRet,cCve,cAnio;
		END IF;
		...
		SELECT COUNT(*) INTO valida FROM systables WHERE tabname = 'criterio_busqueda_anio_tmp';
		
		IF (valida > 0) THEN
			DROP TABLE bdicnweb:"informix".criterio_busqueda_anio_tmp;
		END IF;
		
		CREATE TABLE bdicnweb:"informix".criterio_busqueda_anio_tmp(
		...
		IF (pAnioInicial = iAnioActual) THEN
		
				INSERT INTO bdicnweb:"informix".criterio_busqueda_anio_tmp(anio) VALUES (pAnioInicial);
		
		ELSE
				LET iAnioCount =pAnioInicial;
				
				WHILE iAnioCount <  iAnioActual + 1
					INSERT INTO bdicnweb:"informix".criterio_busqueda_anio_tmp(anio) VALUES (iAnioCount);
				LET iAnioCount = iAnioCount + 1;
				END WHILE;
		...
		FOREACH 
			SELECT anio 
			INTO cAnio
			FROM bdicnweb:"informix".criterio_busqueda_anio_tmp
			
			LET iRecuperacion = iRecuperacion + 1;
			RETURN cCodRet,cAnio,cAnio  WITH RESUME;           
		END FOREACH;
...
'FECHA: 04/09/2018',
'MODULO: CAJA GENERAL',
'FUNCIONALIDAD: Aumento Resta de Importe Caja Gral ',
'DESCRIPCION: SP que consulta los años apartir del anio inicial al año actual  ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_cg_consultarmesesanio(pUsuario CHAR(8), pIdFuncion CHAR(10), pAnio INTEGER)
		RETURNING CHAR(5) AS codret,
		CHAR(2) AS id_mes,
		CHAR(20)   AS descripcion_mes;         
		
	
	DEFINE cCodRet CHAR(5);
	DEFINE iSqlErr INTEGER;
		
	DEFINE iIdMes INTEGER; 	
	DEFINE cDescripcionMes CHAR(20);      
	DEFINE iRecuperacion INTEGER;
	DEFINE iAnioActual INTEGER;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, iIdMes,cDescripcionMes;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cg_consultarmesesanio.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = ''  OR pAnio IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet, iIdMes,cDescripcionMes;
		END IF;
		...
		IF (pAnio  <  iAnioActual) THEN
		
				FOREACH 
					SELECT id, mes_descripcion 
					INTO iIdMes,cDescripcionMes
					FROM bdicnweb:"informix".sw_cat_mesesanio
			
					LET iRecuperacion = iRecuperacion + 1;
					RETURN cCodRet, iIdMes,cDescripcionMes  WITH RESUME;           
				END FOREACH;	
...
'FECHA: 04/09/2018',
'MODULO: CAJA GENERAL',
'DESCRIPCION: SP que consulta los meses del año que transcurrieron del parametro año ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_cg_consultaroperacionsumarestaimp(pUsuario CHAR(8), pIdFuncion CHAR(10))
		RETURNING CHAR(5) AS codret,
		CHAR(1)   AS clave,    
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cg_consultaroperacionsumarestaimp.out';
		--TRACE ON;
		...
		FOREACH SELECT clave,operacion
				INTO cClave,cDescripcion
				FROM bdicnweb:"informix".sw_cg_operacionsumarestaimportes
...
'MODULO: CAJA GENERAL',
'FUNCIONALIDAD: Aumento Resta de Importe Caja Gral ',
'DESCRIPCION: SP que consulta el catalogo operaciones de caja gral ',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_cg_consultasaldoactualsumarestaimp(pUsuario CHAR(8), pIdFuncion CHAR(10), pCajaGeneral  CHAR(4))
	RETURNING CHAR(5) AS codret,
		DECIMAL(14,2) AS pdeno_1000,
		DECIMAL(14,2) AS pdeno_500,
		DECIMAL(14,2) AS pdeno_200,
		DECIMAL(14,2) AS pdeno_100,
		DECIMAL(14,2) AS pdeno_50,
		DECIMAL(14,2) AS pdeno_20,
		MONEY(19,2) AS ptotal_billete, 
		DECIMAL(14,2) AS pdeno_mon_100,
		DECIMAL(14,2) AS pdeno_mon_50,
		DECIMAL(14,2) AS pdeno_mon_20,
		DECIMAL(14,2) AS pdeno_mon_10,
		DECIMAL(14,2) AS pdeno_mon_5,
		DECIMAL(14,2) AS pdeno_mon_2,
		DECIMAL(14,2) AS pdeno_mon_1,
		DECIMAL(14,2) AS pdeno_mon_0_50, 
		DECIMAL(14,2) AS pdeno_mon_0_20,
		DECIMAL(14,2) AS pdeno_mon_0_10,
		DECIMAL(14,2) AS pdeno_mon_0_05,
		MONEY(19,2) AS ptotal_morralla,
		MONEY(19,2) AS pmonto_total;
		...
		DEFINE dTotal_billete	DECIMAL(18,2); 
		...
		DEFINE dTotal_morralla  DECIMAL(18,2);
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
				LET cCodRet = iSqlErr;
				RETURN cCodRet,fCantidad_1000,fCantidad_500,fCantidad_200,fCantidad_100,fCantidad_50, fCantidad_20, dTotal_billete, fCantidad_mon_100, fCantidad_mon_50, fCantidad_mon_20, fCantidad_mon_10, 
					fCantidad_mon_5, fCantidad_mon_2,fCantidad_mon_1, fCantidad_mon_0_50, fCantidad_mon_0_20, fCantidad_mon_0_10, fCantidad_mon_0_05, dTotal_morralla,dMonto_total;
		END EXCEPTION;

		--SET DEBUG FILE TO '/tmp/mfinis/sp_cg_consultasaldoactualsumarestaimp.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = ''  OR pCajaGeneral='' THEN
			LET cCodRet = '00003';
			  RETURN cCodRet,fCantidad_1000,fCantidad_500,fCantidad_200,fCantidad_100,fCantidad_50, fCantidad_20, dTotal_billete, fCantidad_mon_100, fCantidad_mon_50, fCantidad_mon_20, fCantidad_mon_10, 
					fCantidad_mon_5, fCantidad_mon_2,fCantidad_mon_1, fCantidad_mon_0_50, fCantidad_mon_0_20, fCantidad_mon_0_10, fCantidad_mon_0_05, dTotal_morralla,dMonto_total;
		END IF;
		...
		SELECT COUNT(*) INTO valida FROM bdisuc:"informix".ss_cajageneral WHERE cod_proveedor = pCajaGeneral;
		
		IF ( valida < 1 ) THEN
				LET cCodRet = '00877'; -- La caja general número  no existe. Por favor verifique
				
				 RETURN cCodRet,fCantidad_1000,fCantidad_500,fCantidad_200,fCantidad_100,fCantidad_50, fCantidad_20, dTotal_billete, fCantidad_mon_100, fCantidad_mon_50, fCantidad_mon_20, fCantidad_mon_10, 
					fCantidad_mon_5, fCantidad_mon_2,fCantidad_mon_1, fCantidad_mon_0_50, fCantidad_mon_0_20, fCantidad_mon_0_10, fCantidad_mon_0_05, dTotal_morralla,dMonto_total;
		 END IF;
		 
		 EXECUTE PROCEDURE bdisuc:"informix".sp_saldoactual_cajageneral(cEmpresa, pCajaGeneral) INTO cCodRet, fCantidad_1000,fCantidad_500,fCantidad_200,fCantidad_100,fCantidad_50, fCantidad_20, dTotal_billete, fCantidad_mon_100, fCantidad_mon_50, fCantidad_mon_20, fCantidad_mon_10, 
		 fCantidad_mon_5, fCantidad_mon_2,fCantidad_mon_1, fCantidad_mon_0_50, fCantidad_mon_0_20, fCantidad_mon_0_10, fCantidad_mon_0_05, dTotal_morralla,dMonto_total;
		
		LET iCodRetSp = cCodRet::INTEGER;
		IF iCodRetSp < 0 THEN
			RAISE EXCEPTION iCodRetSp, 0, "ERROR EN LA EJECUCION DEL SP bdisuc:'informix'.sp_saldoactual_cajageneral";
		ELIF iCodRetSp = 1 THEN
			LET cCodRet = '00017'; 
		END IF;
		
		 RETURN cCodRet,NVL(fCantidad_1000,0),NVL(fCantidad_500,0),NVL(fCantidad_200,0),NVL(fCantidad_100,0), NVL(fCantidad_50,0), NVL(fCantidad_20,0), NVL(dTotal_billete,0), NVL(fCantidad_mon_100,0), NVL(fCantidad_mon_50,0), NVL(fCantidad_mon_20,0), NVL(fCantidad_mon_10,0), 
				NVL(fCantidad_mon_5,0), NVL(fCantidad_mon_2,0),NVL(fCantidad_mon_1,0), NVL(fCantidad_mon_0_50,0), NVL(fCantidad_mon_0_20,0), NVL(fCantidad_mon_0_10,0), NVL(fCantidad_mon_0_05,0), NVL(dTotal_morralla,0),NVL(dMonto_total,0);
...
'FUNCIONALIDAD: Aumento Resta Saldos Caja General',
'MODULO: Caja General',
'DESCRIPCION: SPL intermedio que realiza la consulta del catalogo caja general, Aumento Resta de Saldos Caja General',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_cargacuentasbc(pUsuario CHAR(8), pIdFuncion CHAR(10), pArchivo CHAR(150))
	RETURNING CHAR(5) AS codret;

	DEFINE cFechaCarga DATETIME YEAR TO FRACTION(3);
	DEFINE pArchivoTemp CHAR(150);
	DEFINE pArchivoTemp2 CHAR(150);
	...
	DEFINE bEncontrado BOOLEAN;
	DEFINE iPosicion INT;
	DEFINE cAux CHAR(150);
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cargacuentasbc.out';
		--TRACE ON;
		...
		LET cAux = TRIM(pArchivo);
		
		---Se obtiene nombre de archivo
		WHILE( bEncontrado == 'f')	
			LET iPosicion = CHARINDEX('/',TRIM(cAux));
			IF iPosicion > 0 THEN 
				LET cAux = REPLACE(SUBSTRING(TRIM(cAux) FROM iPosicion FOR 1),'/','_') || SUBSTRING(TRIM(cAux) FROM iPosicion + 1 FOR  LENGTH(TRIM(cAux))-iPosicion);
				LET bEncontrado = 'f';
			ELSE
				LET bEncontrado = 't';
				LET cAux = SUBSTRING(TRIM(cAux) FROM 2 FOR LENGTH(TRIM(cAux))-1);
			END IF
		END WHILE;
		
		LET cAux = SUBSTRING(TRIM(cAux) FROM LENGTH(TRIM(cAux))-7 FOR 4);
				
		---Se eliminan registros cargados previamente en el archivo para reescribirlos
		EXECUTE FUNCTION bdicnweb:"informix".sp_eliminadesactualizadasbc(pUsuario, pIdFuncion, TRIM(cAux), '1') INTO cCodRet;
		IF cCodRet <> '00000' THEN
			RETURN cCodRet;
		END IF;
		
		LET pArchivoTemp2 = TRIM(pArchivo)||".tmp2";
		
		-- Se elimina encabezado del archivo (primera linea)
		LET cCmd1 = "sed '1d' "||TRIM(pArchivo)||" > "||TRIM(pArchivoTemp2);
		SYSTEM TRIM(cCmd1);		
		...
		LET cCmd2 = '"\r$", ""); print }';
		LET cCmd3 = "' "||TRIM(pArchivoTemp2)||" > "||TRIM(pArchivoTemp);
		SYSTEM TRIM(cCmd1)||TRIM(cCmd2)||TRIM(cCmd3);
		
		LET cCmd3 = TRIM(cUsrBin)||"mv "||TRIM(pArchivo)||" "||TRIM(pArchivo)||".ori";
		SYSTEM TRIM(cCmd3);		
		...
		--_AAME 14032019 INC 27 122 Se corrige la ruta de ejecución por la productiva.
		...
		LET cCmd1 = TRIM(cUsrBin)||"rm -rf "||TRIM(pArchivoTemp)||" "||TRIM(pArchivoTemp2)||" "||TRIM(pArchivo);
		SYSTEM TRIM(cCmd1);
...
"DESCRIPCIÃ?N: Carga de archivos de cuentas desactualizadas BC por medio de un LOAD",
"             pasando como argumento el archivo a cargar, los campos son dinamicos y se encuentran en la tabla sw_tr_info_tablas",
'BD: bdicnweb';
