CREATE PROCEDURE "informix".sp_confpgserv_dina(cSucursal CHAR (4), cCategoria CHAR (2), cConvenio CHAR(5), cReferencia1 CHAR (40), cReferencia2 CHAR(40),cFolio_suc CHAR (16))
RETURNING CHAR(5), CHAR(200);
	...
	LET vcadena_ent   = cSucursal||'|'||cCategoria||'|'||cConvenio||'|'||cReferencia1||'|'||cReferencia2||'|'||cFolio_suc;
	
	LET TotReg = 0;

	--	SET DEBUG FILE TO '/respaldosbd/mario/sp_confpgserv_din.out';
	--	TRACE ON; 
        ...
        ON EXCEPTION SET iSql_err, iIsamErr, cDescripcion
           IF iSql_err <> 0 THEN
			   ...
			   INSERT INTO bdisac:"informix".sac_mensajeerror (sql_error, isam_error, descripcion, origen_error, fecha, fecha_insert)
				VALUES (cCodRet,iIsamErr,cDescripcion,'sp_confpgserv_din',today,CURRENT);
			   INSERT INTO bdisac:"informix".sac_log_envios (proceso, folio_suc, importe, cod_ret, cadena_ent, fecha_insert, hora_insert)
						VALUES ('sp_confpgserv_din', cFolio_suc, 0, cCodRet, vcadena_ent, CURRENT, CURRENT);	
...
'Fecha:09/01/2014',
'Modificación: Se crea copia de el  proceso central sp_confpagoservicio para ejecutarse de manera dinámica con el arma_envios.',
'Sustento: RQI 62 064-Reingeniería_PagoServicios.doc -  (Pagina 2 a 36)',
'Solicita: Jaime Gonzalez',
'BD:bdisac';

CREATE PROCEDURE "informix".sp_confpagocoppel (cSucursal CHAR (4), iRecibo INT8)
RETURNING CHAR(5);
    ...
    BEGIN
        ...
        ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
                IF iSqlErr <> 0 THEN
                    LET cCodRet = iSqlErr;
                    EXECUTE PROCEDURE sp_sac_GuardaMensajeerror (iSqlErr, iIsamErr, cInfoErr, "sp_ConfPagoCoppel");
                    RETURN cCodRet;
                END IF;
        END EXCEPTION;
...
SET ISOLATION COMMITTED READ;
            ...
            IF cSucursal <> "" AND iRecibo <> 0 THEN
                ...
                IF iRegistros > 0 THEN
                    UPDATE bdisac:sac_movimientosdetalle
                    SET status_coppel = 1
                    WHERE sucursal = cSucursal
                    AND recibo = iRecibo;
                ELSE
                    LET cCodRet = '00001';  --No existen el folio y el recibo
                END IF;
            ELSE
                LET cCodRet = "00002";
            END IF;
...
'DESCRIPCION: Se encarga de confirmar registro de aquel movimiento generado para coppel',
'EJECUTADO O LLAMADO POR: Caja.exe()',
'FECHA : Septiembre de 2008',
'VERSION: 20080930',
'BD    : bdisac';

CREATE PROCEDURE "informix".sp_obtieneparametro(siParametro INTEGER)
...
--INICIALIZACION DE VARIABLES
    ...
    --SET DEBUG FILE TO "/informix/alex/sp_RecibeValor.out";
    --TRACE ON;
 ...
 BEGIN
    ...
    ON EXCEPTION SET iSqlErr
        IF iSqlErr <> 0 THEN
            ...
            IF iSqlErr = '-245' THEN
                let cCodret = cValor;
                LET cValordesc = 'La tabla sac_convenios esta bloqueada.';
            END IF;
            IF iSqlErr = '-674' THEN
                let cCodret = cValor;
                LET cValordesc = 'No se encuentra el sp_verificaconvenio.';
            END IF;
...
SET LOCK MODE TO WAIT 3;
    ...
    IF CAST (siParametro AS INTEGER)= 0 THEN
        LET cCodret = "00001";
    ELSE
        SELECT valor
        INTO cValor
        FROM bdisac:sac_param
        WHERE cod_param = siParametro;

        --****  Agregado  20/04/2016  **********************************************************************
        -- Si cValor pertenece a BTS (30802,30803,30804) y a WU (30401,30402,30403,30405)
        -- se verifica el status en la sac_convenios para verificar si el servicio esta Activo o Inactivo.
        --**************************************************************************************************
        
        IF cValor IN ('30802','30803') THEN
            CALL sp_verificaconvenio(cValor) returning cCodret2, cValordesc;
         
            IF cCodret2 = '00504' THEN
                RETURN cCodret2, cValordesc;
            END IF;
        END IF;
...
DOCUMENT
    ...
    'DESCRIPCION: Obtiene el valor del parametro.',
    'EJECUTADO O LLAMADO POR: caja.exe',
    'FECHA : noviembre de 2008',
    'VERSION: 20081106',
    'BD    : bdisac';

CREATE PROCEDURE "informix".sp_sac_wu_validanombre(pPrimerNomRem CHAR(40), pApePatRem CHAR(40), pPrimerNomBenef CHAR(40), pApePatBenef CHAR(40), pMonto CHAR(40), pNumReferencia CHAR(10), pFolioSuc CHAR(16))
RETURNING
CHAR(5)      As CodRet,  
...
BEGIN
    ...
    ON EXCEPTION SET iSql_err
        IF iSql_err <> 0 THEN
            LET cCodRet = iSql_err;
            RETURN cCodRet, dPorcentaje,cTipoBenef,cPrimerNomRem ,cApePatRem ,cPrimerNomBenef ,cApePatBenef,cMonto  ,cNew_mtcn,cMontoTransferKey ;
        END IF;
    END EXCEPTION;
	...
	IF iBandera = 1 THEN			
		FOREACH 	
			SELECT a.emisor_nombre1, a.emisor_appaterno, a.benef_nombre1,a.benef_appaterno,b.money_transfer_key_rp,a.monto_total_destino,a.foreign_rs_refnum_rq,b.new_mtcn  
			  INTO cPrimerNomRem, cApePatRem, cPrimerNomBenef, cApePatBenef, cMontoTransferKey, cMonto,cForeign_Rs_Refnum_Rq,cNew_mtcn  			  
		   ...
		   EXECUTE PROCEDURE "informix".sp_validanombenefbts(pPrimerNomBenef, '', pApePatBenef, '', cPrimerNomBenef, '', cApePatBenef, '')
	...
	ELIF NVL(iNum_coincidencias ,'') = '' OR iBandera = 0 THEN 
		FOREACH 	
			SELECT emisor_nombre1,emisor_appaterno,benef_nombre1,benef_appaterno,money_transfer_key,monto_total_destino,foreign_rs_refnum_rq,new_mtcn  
			  INTO cPrimerNomRem, cApePatRem, cPrimerNomBenef, cApePatBenef, cMontoTransferKey, cMonto,cForeign_Rs_Refnum_Rq,cNew_mtcn  
			  FROM "informix".sac_wu_search 
			 WHERE mtcn = pNumReferencia 
			   AND fecha_insert = (SELECT MAX(fecha_insert) FROM "informix".sac_wu_search WHERE mtcn = pNumReferencia)
	...
	ELSE
		...
		IF iCantidad > 1 THEN
			...
			LET cTipoBenef         = '';
		ELSE
			SELECT emisor_nametype,emisor_nombre1,emisor_appaterno,benef_nombre1,benef_appaterno, new_mtcn, monto_total_destino, num_coincidencias 
