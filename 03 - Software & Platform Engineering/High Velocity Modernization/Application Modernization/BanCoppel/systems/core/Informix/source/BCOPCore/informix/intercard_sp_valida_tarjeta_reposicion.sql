CREATE PROCEDURE "informix".sp_valida_tarjeta_reposicion(pNumEmp char(9), pNumCuenta varchar(13))
   RETURNING char(5);
...
-- ***************************************************************************
   	...
   	LET cod_ret  = '00000'; --Cliente no cuenta con solicitud de reposicion activa
	LET cestatus_sol = '';
...
BEGIN
	...
	LET pNumEmp = TRIM(NVL(pNumEmp,''));
	LET pNumCuenta = TRIM(NVL(pNumCuenta,''));
    
	SELECT e.estatusproceso 
	INTO cestatus_sol
	FROM "informix".solicitudtarjeta e
	WHERE idsolicitud = (
		SELECT MAX(i.idsolicitud) 
		FROM "informix".solicitudtarjeta i 
		WHERE numcliente=pNumEmp 
		AND numcuenta=pNumCuenta
		GROUP BY i.numcliente,i.numcuenta);
		
	IF UPPER(NVL(cestatus_sol,'')) = 'F' THEN
		LET cod_ret='00001'; --indica que el cliente tiene una solicitud en proceso
	END IF;
...
END PROCEDURE
DOCUMENT
'Folio: 451.1 - RQM 10 802 - CancelaciÃ³n de Tarjetas DÃ©bito y CrÃ©dito ClÃ¡sica, Oro y Platino a travÃ©s del CAT.',
'Autor: Arturo Astorga',
'BD: intercard',
'Fecha: 25/03/2019',
'SolicitÃ³: jose luis polanco',
'Descripcion: Descripcion: indica si el cliente tiene en proceso una solicitud de reposicion.';
