CREATE PROCEDURE "informix".totcomppba1(o_empresa   CHAR(3),
				    o_usuario   CHAR(8),
				    o_sucursal  CHAR(4),
...
SET DEBUG FILE TO "/tmp/totcompcred";
trace on;
...
FOREACH
        SELECT divisa, usuario, COUNT(*), SUM(monto)
          INTO v_divisa, v_usuario, v_movto_firme, v_monto_firme
          FROM sd_movdia
         WHERE ((codigo_fun IN ("033", "333") and codigo_ref = 1) or (codigo_fun = "336" and codigo_ref = 20))
	   ...
	   AND sucursal = o_sucursal
        ...
        INSERT INTO sd_totcomp VALUES(o_empresa, v_divisa,
                                      v_fecha, v_usuario,
                                      v_movto_firme, v_monto_firme,
                                      v_movto_cargo, v_monto_cargo);
        LET v_existe = "1";
END FOREACH
FOREACH
        SELECT divisa, usuario, COUNT(*), SUM(monto)
          INTO v_divisa, v_usuario, v_movto_cargo, v_monto_cargo
          FROM sd_movdia
	   ...
	   AND sucursal = o_sucursal
        ...
        IF v_existe = "1" THEN
            UPDATE sd_totcomp SET no_movcgo = v_movto_cargo,
                                  monto_cgo = v_monto_Cargo
...
FOREACH SELECT a.*, b.descripcion
          INTO v_empresa, v_divisa, v_fecha,
               v_usuario, v_movto_firme, v_monto_firme,
               v_movto_cargo, v_monto_cargo,
               v_descripcion
          FROM sd_totcomp a, bdinteg:si_divisas b
          WHERE a.divisa = b.divisa
          AND   a.usuario = o_usuario
          AND   a.empresa = o_empresa

          LET v_ciclo = v_ciclo + 1;
          IF v_ciclo <= o_num_total  THEN
                CONTINUE FOREACH;
          END IF
        RETURN v_codret, v_divisa, v_monto_cargo, v_monto_firme,
               v_monto_sbc, v_monto_rem, v_descripcion, v_movto_cargo,
