CREATE PROCEDURE "informix".sp_consultafoliocontratobpi_web(pEmpresa CHAR(3), pNumCte CHAR(20))
...
BEGIN
    ...
    --IF EXISTS (SELECT {+ INDEX (si_bpiusuarios idx_bpi)} numcte FROM bdinteg:"informix".si_bpiusuarios WHERE numcte = pNumCte and empresa = pEmpresa) THEN
	SELECT {+ INDEX (si_bpiusuarios idx_bpi)} numcte INTO iCuantos
	FROM bdinteg:"informix".si_bpiusuarios WHERE numcte = pNumCte and empresa = pEmpresa;
	
	IF(iCuantos > 0) THEN
	
        SELECT {+ INDEX (si_bpiusuarios idx_bpi)} folio_contrato INTO vFolio FROM bdinteg:"informix".si_bpiusuarios WHERE numcte = pNumCte AND empresa = pEmpresa;
        LET vCodRet = '00000';

    ELSE
         LET vCodRet = '00001'; --El cliente no tiene servicio de banca por internet
    END IF;
