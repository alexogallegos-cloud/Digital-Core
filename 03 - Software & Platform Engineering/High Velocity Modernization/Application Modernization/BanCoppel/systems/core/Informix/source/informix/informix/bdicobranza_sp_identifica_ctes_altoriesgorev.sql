CREATE PROCEDURE "informix".sp_identifica_ctes_altoriesgorev()
returning VARCHAR(06),
          VARCHAR(80);
-----------------------------------------------------------------------
--  EXECUTE PROCEDURE "informix".sp_identifica_ctes_altoriesgorev();
...
DEFINE iTotalExcluidasTDC		INTEGER;
DEFINE iTotalCuentasVencidasTDC	INTEGER;
...
BEGIN 
   ON EXCEPTION SET SQL_ERR, ISAM_ERR, ERROR_INFO
     ...
     RETURNING P_COD_RET;
     LET P_COD_RET = SQL_ERR;
     DROP TABLE IF EXISTS ctes_altoriesgorev;
     RETURN P_COD_RET,P_MENSAJE;
   END EXCEPTION;

--SET DEBUG FILE TO "sp_identifica_ctes_altoriesgorev.out";
--TRACE ON;

LET cProceso            		= '2014';
LET P_COD_RET           		= '000000';
LET cCodRet           			= '000000';
LET P_MENSAJE           		= 'El proceso de IDENTIFICA CTES. ALTO RIESGO TDC se ejecuto correctamente.';
...
LET iTotalExcluidasTDC			= 0;
LET iTotalCuentasVencidasTDC 	= 0;
...
TRUNCATE TABLE "informix".cb_vencidos_findemes_tdc DROP STORAGE;
...
SELECT 
  ...
  FROM bdicred:"informix".sd_maecredcont mae
 INNER JOIN bdicred:"informix".sd_maesdoscont mas ON mas.fecha = mae.fecha AND mas.empresa = mae.empresa AND mas.num_credito = mae.num_credito
 INNER JOIN bdicred:"informix".sd_maecredanexo maa ON maa.empresa = mae.empresa AND maa.num_credito = mae.num_credito
 ...
 WHERE mae.fecha = dFechaProceso2
   ...
   AND (mas.mto_venc_trasp+mas.monto_vencido) > 0
INTO TEMP ctes_altoriesgorev WITH NO LOG;

CREATE INDEX ind_fecha_tmprev ON ctes_altoriesgorev(fecha);

UPDATE STATISTICS MEDIUM FOR TABLE ctes_altoriesgorev;
...
FOREACH WITH HOLD
	SELECT fecha,num_credito,numcte,num_producto,fecha_apertura,mto_venc_trasp,mto_fin_ven_trasp,sdo_cap_insoluto,
	  ...
	  FROM ctes_altoriesgorev WHERE fecha = dFechaProceso2

	LET	iTotalCuentasVencidasTDC = iTotalCuentasVencidasTDC + 1;

	IF sNumVencidos >= 7 THEN LET cCampania = ''; LET cCampania = 'TC_VTAC7PV';
	ELIF sNumVencidos IN (3,4,5,6) THEN
		IF (dFechaUltPago IS NULL OR dFechaUltPago = '') THEN
			LET cCampania = '';
			LET cCampania = 'TC_VTACCON';
		ELIF (sNumVencidos = 6) AND (dFechaUltPago IS NOT NULL OR dFechaUltPago <> '') THEN
			IF NOT EXISTS (SELECT num_credito FROM "informix".cb_vencidos_findemes_tdc WHERE fecha = dFecha AND campania = 'TC_VTACCON' AND num_credito = cNumCredito) THEN --VALIDAR QUE NO ESTE EN LA CAMPANA PP_VTACCON
				LET cCampania = '';
				LET cCampania = 'TC_VTAC6PV';
			ELSE
				LET	iTotalExcluidasTDC = iTotalExcluidasTDC + 1;
				CONTINUE FOREACH;
			END IF;
		ELSE
			LET	iTotalExcluidasTDC = iTotalExcluidasTDC + 1;
	...
	ELIF (sNumVencidos = 1) AND (dFechaUltPago IS NULL OR dFechaUltPago = '') THEN 
		LET cCampania = '';
...
UPDATE STATISTICS MEDIUM FOR TABLE "informix".cb_vencidos_findemes_tdc;

DROP TABLE ctes_altoriesgorev;

LET cMensaje = 'TOTAL Cuentas vencidas procesadas TDC : ' ||iTotalCuentasVencidasTDC;
CALL "informix".sp_inserta_bitacora_cob(cEmpresa, cProceso, cCodRet, trim(cMensaje), '02') RETURNING P_COD_RET;
LET cMensaje = "";
LET cMensaje = 'Cuentas excluidas TDC : ' ||iTotalExcluidasTDC;
LET cMensaje = trim(cMensaje) ||'    Cuentas insertadas TDC : ' ||iTotalInsertadas;
CALL "informix".sp_inserta_bitacora_cob(cEmpresa, cProceso, cCodRet, trim(cMensaje), '02') RETURNING P_COD_RET;
LET cMensaje = "";
