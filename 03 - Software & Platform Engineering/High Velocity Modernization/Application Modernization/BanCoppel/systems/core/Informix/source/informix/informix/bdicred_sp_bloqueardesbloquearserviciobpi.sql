CREATE PROCEDURE  "informix".sp_bloqueardesbloquearserviciobpi(pEmpresa CHAR(3), pNumCte CHAR(20), pStatus CHAR(2), pSucursal CHAR(4), pNumEmp CHAR(9), pIP  CHAR(15))
    ...
    ------------------------------------------------------------------------------------------------------------------------------------
	-- Modifica: Jessica GutÃ­errez
    --Actividad: Para la cancelacion, se modifica la consulta para que no consulte las solicitudes que ya estan canceladas.
    -- Fecha: 7-11-2013
  	------------------------------------------------------------------------------------------------------------------------------------
  ...
  -- SET DEBUG FILE TO "/informix/JuanRivera/Tracessp_BloquearDebloquearServicioBPI.out";
  -- TRACE ON;
...
BEGIN
    ...
    IF vCodRet = '000'  THEN
        ...
        SELECT id_status, servicio, folio_contrato INTO vIdStatus,vServicio, vFolio FROM bdinteg:si_bpiusuarios WHERE  empresa = pEmpresa AND numcte = pNumCte;
		
				--Se aÃ±ade validacion de folio nulo, si el cliente no cuenta con folio de activacion y tiene un status valido, se le asigna un folio nuevo.
			IF vFolio ='' OR vFolio IS NULL THEN
				LET i = 0;
				WHILE i <= 1
					EXECUTE FUNCTION bdinteg:"informix".sp_genera_folioactivacion_bpi() INTO vCodRet, vFolio;
						--validacion de duplicado generado.
				...
				END WHILE;
					--Se valida si el cliente cuenta con un folio sin encriptar, si no, se encripta.
			ELIF LENGTH(vFolio) = 12 THEN
			
				EXECUTE FUNCTION bdibpi:"informix".sp_encripta_folio_contrato(pNumCte);
			
			END IF;
				
        UPDATE bdinteg:si_bpiusuarios SET id_status = pStatus, f_status = CURRENT WHERE numcte = pNumCte;

		INSERT INTO bdinteg:si_cambiostcte (numcliente, id_statusanterior, id_statusactual, ipusuario, fecha_cambio, suc_cambio, usuario_cambio)
		VALUES (pNumCte, vIdStatus, pStatus, pIP, CURRENT, pSucursal, pNumEmp);
		
		--Cancelacion de servicio bpi.
        IF pStatus = '99' AND  vServicio = 2 THEN

            FOREACH
                SELECT solicitud, id_status, ns_token   INTO vSolicitud, vIdStaSol, vToken
                FROM bdibpi:bpi_tokensolicitud  WHERE empresa = TRIM(pEmpresa)  AND numcte = TRIM(pNumCte) AND id_status not in (199)
				
                EXECUTE PROCEDURE bdibpi:sp_cancelartoken(pEmpresa, pNumCte, vToken, pNumEmp, vIdStaSol, '199', '01') INTO vCodRet;
				
                EXECUTE PROCEDURE bdibpi:sp_set_solicitudstatus_admtoken(vSolicitud, pNumCte, pNumEmp, vIdStaSol, '199') INTO vCodRet;

            END FOREACH;       

        END IF;

        SELECT mensaje INTO vMensValid FROM bdinteg:si_bpicatmensajes  WHERE proceso = '2'
        AND operacion = '2'
        AND status_servicio = pStatus;

     ELSE
            LET vCodRet =   '001';
            LET vMensValid = 'No se pudo grabar el registro, favor de intentar mÃ¡s tarde';
     END IF;
...
END PROCEDURE
DOCUMENT
'Bloquea y desbloquea usuarios para la Banca por Internet y guarda registros en la si_cambiostct los cambios de status',
'Autor :' ,
'ModificÃ³ : Dulce RamÃ­rez',
'FECHA : 25/Mayo/2009',
'Ver.  : 1.0',
'BD    : bdinteg',
'VER   : 1.0',
'ModificÃ³: Berenice Noriega',
'Para actualizar la bdibpi:tkn_envios y bpi_tokensolicitud',
...
--    ELSE
      ...
      --      LET cAnio= YEAR(pPeriodo-1::DATE);
      --  Else
      --      LET cAnio= YEAR(pPeriodo::DATE);
      --  END IF;
--    END IF;

--LET  cNombre_Archivo= 'ReporteIvaInteresReal_' ||LPAD(TRIM( DAY(pPeriodo::DATE)::CHAR (2)),2,'0') || LPAD(TRIM(MONTH(pPeriodo - 1 units month )::CHAR(2)),2,'0') || cAnio || '.xls';
LET  cNombre_Archivo= '/resplogifx/archivoscartera/respaldo/ReporteIvaInteresReal_' || cMes || cAnio || '.txt';


-- Verifica que no exista previamente la tabla temporal si existe la elimina
  IF EXISTS( SELECT dbsname, tabname FROM sysmaster:systabnames  WHERE tabname = 'temp_reporteivainteresreal' ) THEN
        DROP TABLE temp_ReporteIvaInteresReal;
  END IF;

-- Se crea la tabla temporal donde se almacenara la informacion del reporte
    CREATE TABLE temp_ReporteIvaInteresReal
    (
        Fecha_de_Registro           Date,
        Numero_de_Credito           Char(20),
        Interes_Nominal             Decimal(18,2),
        Interes_Exento              Decimal(18,2),
        Interes_Gravado             Decimal(18,2),
        Provision_Interes           Decimal(18,2),
        Provision_Iva               Decimal(18,2),
        Capitalizacion_Interes      Decimal(18,2),
        Capitalizacion_Iva          Decimal(18,2)
    );

    SELECT empresa, num_credito, fecha_mov, reversado, codigo_fun, codigo_ref, monto
	  FROM bdicred:sd_movhis
	 WHERE empresa = '001'
	   AND (MONTH(fecha_mov)= cMes AND YEAR(fecha_mov)= cAnio)
	   AND num_credito > 0
	   AND ((codigo_fun = '605' AND codigo_ref  in (2,125,127) AND reversado   = 'N') OR
	   	    (codigo_fun = '606' AND codigo_ref IN (1,7018,10,11))  OR
			(codigo_fun = '340' AND codigo_ref = 20))
	  INTO temp tmp_movtos_hist WITH NO LOG;
	  
	  CREATE INDEX inx_tmp_movtos_hist on "informix".tmp_movtos_hist (fecha_mov,num_credito);
      UPDATE STATISTICS MEDIUM FOR TABLE "informix".tmp_movtos_hist;  
	  
	-- Se Realiza una seleccion de los creditos aperturados con los estatus ('AA','BA','BT')
   INSERT INTO temp_ReporteIvaInteresReal
		SELECT DISTINCT(a.fecha_mov),
			   a.num_credito,
			  SUM((CASE WHEN a.codigo_fun = '605' AND a.codigo_ref = 2  THEN NVL(monto,0) else 0 END)) AS INTERES_NOMINAL,
			  SUM((CASE WHEN a.codigo_fun = '606' AND a.codigo_ref = 11 THEN NVL(monto,0) else 0 END)) AS INTERES_EXENTO,
			  SUM((CASE WHEN a.codigo_fun = '606' AND a.codigo_ref = 10 THEN NVL(monto,0) else 0 END)) AS INTERES_GRAVADO,
			  SUM((CASE WHEN a.codigo_fun = '606' AND a.codigo_ref = 1  THEN NVL(monto,0) else 0 END)) AS PROVISION_INTERES, 
			  SUM((CASE WHEN a.codigo_fun = '340' AND a.codigo_ref = 20 THEN NVL(monto,0) else 0 END)) AS PROVISION_IVA,
			  SUM((CASE WHEN a.codigo_fun = '605' AND a.codigo_ref = 2  THEN NVL(monto,0) else 0 END)) AS CAPITALIZACION_INTERES,
			  SUM((CASE WHEN a.codigo_fun = '605' AND a.codigo_ref = 3  THEN NVL(monto,0) else 0 END)) AS CAPITALIZACION_IVA
		 FROM tmp_movtos_hist a
	 GROUP BY 1,2;	  
	  
        --Se genera archivo con la informacion del reporte delimitado por el caracter PIPE "|"
        let cSql = '';
        let cSql = 'echo "UNLOAD TO ' || '''/resplogifx/archivoscartera/respaldo/ReporteIvaInteresRealRegistros.unl''' || ' DELIMITER ' || '''|'''|| 
                   ' select * from bdicred:temp_ReporteIvaInteresReal;'|| 
                   ' " > /resplogifx/archivoscartera/respaldo/ReporteIvaInteresRealQuerys.sql';
        SYSTEM cSql;

          LET cSql = '';
          LET cSql = 'dbaccess bdicred /resplogifx/archivoscartera/respaldo/ReporteIvaInteresRealQuerys.sql';
          SYSTEM cSql;

          LET cSql = "sed 's/|$//g' /resplogifx/archivoscartera/respaldo/ReporteIvaInteresRealRegistros.unl > " || cNombre_Archivo;
          SYSTEM cSql;

          LET cSql = '';
          LET cSql = "gzip  " || cNombre_Archivo;
          SYSTEM cSql;

          LET cSql = '';
          LET cSQL = 'rm /resplogifx/archivoscartera/respaldo/ReporteIvaInteresRealQuerys.sql /resplogifx/archivoscartera/respaldo/ReporteIvaInteresRealRegistros.unl';
          SYSTEM cSql;
          LET cSql = '';
...
" Observación: Se crea sp para obtener el reporte de Iva de Interes Real",
" Fecha de Creación: 18-10-08",
" Base de Datos: bdicred ";

CREATE PROCEDURE "informix".sp_credisol_envia_sms_cancela(pEmpresa CHAR(3))
RETURNING CHAR(5);       -- Codigo de Retorno
	...
	DEFINE cResp_Cte_sms		CHAR(1); -- Respuesta del cliente para contratacion por SMS
...
BEGIN
    ...
    --SET DEBUG FILE TO '/informix/mahr/sp_credisol_envia_sms_cancela.out';
	--TRACE ON;
	...
	--=====================================================================================================================
	-- Cancelacion de invitaciones no autorizadas por el cliente, 7 dias despues de enviadas. 
	
	LET cProceso = '0091';
	LET cCodRet = '00000';
	LET cMensajeRet = '';
	CALL bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, 'INICIA CANCELACION DE SMS NO AUTORIZADOS POR EL CTE', '02') Returning cCod_retIB;
		
	-- Obtiene dias de vigencia para SMS. Despues se cancelan invitaciones.
	SELECT valor_numerico::SMALLINT INTO sDiasEspera_sms
	  FROM bdicred:sd_param_campania WHERE empresa = '001' AND tipo_campania = 2 AND grupo_parametro = 'PAGOSFIJOS' AND num_parametro = 10;

	LET dFechaCanc_sms = DATE(dtFechaHoy - sDiasEspera_sms units day);

	-- Identifica las invitaciones a cancelar.( num_promo = 10 corresponde a meses sin intereses)
	FOREACH WITH HOLD
        SELECT --{+INDEX (sd_promocion_credito_sms inx_smspfcred)}
			   sms.num_credito, crd.sucursal, sms.folio_compra_sms, sms.tipo_sms, envio_result_sms, respuesta_cte_sms
          INTO cNumCredito,     cSucursal,    cFolioComp_sms, 	    sStatus_sms,  cEnvio_r_sms,	    cResp_Cte_sms		
          FROM bdicred:"informix".sd_promocion_credito_sms sms JOIN bdicred:sd_maecred crd ON (sms.num_credito = crd.num_credito )
         WHERE (sms.tipo_sms = '0' OR sms.tipo_sms = '1' OR sms.tipo_sms = '2' OR sms.tipo_sms = '3')
		   AND fecha_invitacion <= dFechaCanc_sms  AND sms.num_promo != 10
	
		IF NVL(cResp_Cte_sms,'') = 'S' THEN		-- Si el cliente respondio SI, se mandara SMS de error y se cancela invitacion.
			-- Se marca sms pendiente de enviar
			UPDATE bdicred:sd_promocion_credito_sms SET tipo_sms = '5', envio_result_sms = '0', status_envio_r_sms = '0', fecha_cancela = dtFechaHoy
			 WHERE num_credito = cNumCredito AND folio_compra_sms = cFolioComp_sms;				 
				
		ELIF sStatus_sms = '0' THEN
		
			UPDATE sd_promocion_credito_sms SET tipo_sms = '8', fecha_env_sms_inv = CURRENT 
			 WHERE num_credito = cNumCredito AND folio_compra_sms = cFolioComp_sms AND tipo_sms = '0';	
			 
		ELSE									-- Se cancela invitacion sin enviar mensaje sms
		
			UPDATE bdicred:sd_promocion_credito_sms SET tipo_sms = '5', respuesta_cte_sms = 'N', fecha_resp_cte_sms = CURRENT, fecha_cancela = dtFechaHoy	
			 WHERE num_credito = cNumCredito AND folio_compra_sms = cFolioComp_sms;		
		END IF;
						
	END FOREACH;

	CALL bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, 'FINALIZA CANCELACION DE SMS NO AUTORIZADOS POR EL CTE', '02') Returning cCod_retIB;

	
	--=====================================================================================================================	
	-- Identifica los clientes marcados como prospectos para Pagos Fijos Saldos para el envio de invitaciones.

	LET cProceso = '0095';
	LET cCodRet = '00000';
	LET cMensajeRet = '';
	CALL bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, 'INICIA ENVIO DE INVITACIONES PAGOS FIJOS SMS SALDO', '02') Returning cCod_retIB;


	SELECT valor_numerico INTO sPFijosSaldoSMS FROM bdicred:sd_param_campania WHERE empresa = '001' AND tipo_campania = 2 AND grupo_parametro = 'PAGOSFIJOS' AND num_parametro = 14;
		
	IF sPFijosSaldoSMS = 1 THEN		
			
		EXECUTE PROCEDURE bdicred:sp_credisol_contrata_x_sms_sdos('001', 1, '', '') INTO cCodRet;

		IF cCodRet <> '00000' THEN		-- Error en proceso de envio de invitaciones.
			CALL bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, 'ERROR EN ENVIO DE INVITACIONES PAGOS FIJOS SMS SALDO - ' || cCodRet, '02') Returning cCod_retIB;
		END IF;
		
	END IF; 
			
	CALL bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, 'FINALIZA ENVIO DE INVITACIONES PAGOS FIJOS SMS SALDO', '02') Returning cCod_retIB;			
	...
	LET cProceso = '0092';	--	Envio de sms de resultado de proceso: Exitos o Erroneo
	LET cCodRet = '00000';
	LET cMensajeRet = '';
	CALL bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, 'INICIA ENVIO (BATCH) DE SMS CON RESULTADO DEL PROCESO.', '02') Returning cCod_retIB;
	...
	FOREACH WITH HOLD
		...
		SELECT first 1 a.telefono INTO cNumCel
		 ...
		 INNER JOIN bdicred:sd_maesdos d ON (b.num_credito = d.num_credito AND (d.monto_vencido + d.mto_venc_trasp) = 0)
		 WHERE tipo_tel = 2
		   --AND verificado = 'V'
		   AND status_tel = 'A'
		...
		IF sNumPromocion = 2 OR sNumPromocion = 5 OR sNumPromocion = 8 THEN			-- Pagos Fijos Compras
	   
			IF NVL(cTipoMsgSms, '') = '1' THEN			-- Se envia mensaje de contratacion correcta de pagos fijos.

				EXECUTE PROCEDURE "informix".sp_credisol_contrata_x_sms (pEmpresa, 4, cNumCel, cFolioPromo, 0) INTO cCod_retIB;
		
			ELIF NVL(cTipoMsgSms, '') = '0' THEN		-- Se envia mensaje de error. No se pudo realizar la contratacion
			
				EXECUTE PROCEDURE "informix".sp_credisol_contrata_x_sms (pEmpresa, 3, cNumCel, cFolioPromo, 0) INTO cCod_retIB;
				
			END IF;

		ELIF sNumPromocion = 3 OR sNumPromocion = 6 OR sNumPromocion = 9 THEN			-- Pagos Fijos Saldos
		
			IF NVL(cTipoMsgSms, '') = '1' THEN
			
				EXECUTE PROCEDURE bdicred:"informix".sp_credisol_contrata_x_sms_sdos(pEmpresa, 4, cNumCel, '', cFolioPromo) INTO cCod_retIB;
				
			ELIF NVL(cTipoMsgSms, '') = '0' THEN
		
				EXECUTE PROCEDURE bdicred:"informix".sp_credisol_contrata_x_sms_sdos(pEmpresa, 3, cNumCel, '', cFolioPromo) INTO cCod_retIB;
	...
	LET cCodRet = '00000';
	LET cMensajeRet = '';
	CALL bdicred:"informix".sp_inserta_bitacora(pEmpresa, cProceso, cCodRet, 'FINALIZA ENVIO (BATCH) DE SMS CON RESULTADO DEL PROCESO.', '02') Returning cCod_retIB;
	
	--=====================================================================================================================
	-- Proceso de envio de SMS para producto 6800 de los pagos realizados en el cobro automatico (transaccion 7506)
	
	FOREACH WITH HOLD
		SELECT num_credito, monto, numcte, status_cred
			INTO cNumCredito, vMonto, vNumcte, vStatusCred
		FROM sd_creditos_envio_sms
		
		SELECT NVL(a.telefono,'') 
		INTO cNumCel									
		FROM bdinteg:si_telefonos a
		WHERE a.tipo_tel = 2 AND a.verificado = 'V' AND a.status_tel = 'A' AND a.numcte = vNumCte; 
	
		-- -- Si telefono es vacÃ­o, se elimina de la tabla sd_creditos_envio_sms
		IF cNumCel is null or cNumCel = '' THEN
			BEGIN; DELETE sd_creditos_envio_sms WHERE num_credito = cNumCredito; COMMIT;
			CONTINUE FOREACH;
		END IF;
		...
		LET vFecha = DAY(dtFechaHoy) || '/' || MONTH(dtFechaHoy) || '/' || YEAR(dtFechaHoy);
	
		IF vStatusCred = 'FF' THEN
			----Envio de mensaje de Liquidacion del prestamo
			EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento ('2','CRED_SMS','PPF_SMS_FF','000000000','', '','2', vFecha, '', '', '', '', '', '', '', '', '', '',TRIM(cNumCel), vMonto, 0, 0, 0, 0, current, current) INTO cCodRet;
			IF cCodRet = '00000'  THEN
				--UPDATE sd_creditos_envio_sms SET status_envio_sms = 1 where num_credito = cNumCredito;
				BEGIN; DELETE sd_creditos_envio_sms WHERE num_credito = cNumCredito; COMMIT;
			END IF;
		ELSE
			--Envio de mensaje de pago realizado para prestamo flexible
			EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento ('2','CRED_SMS','PPF_SMS_CAUT','000000000','', '','2', vFecha, '', '', '', '', '', '', '', '', '', '',TRIM(cNumCel), vMonto, 0, 0, 0, 0, current, current) INTO cCodRet;
			IF cCodRet = '00000'  THEN
				BEGIN; DELETE sd_creditos_envio_sms WHERE num_credito = cNumCredito; COMMIT;
			END IF;
		END IF;
	...
	-- Se elimina informaciÃ³n de primera ejecuciÃ³n de la madrugada.
	BEGIN;
		DELETE FROM sd_creditos_envio_sms WHERE fecha_proceso=dtFechaHoy-3 AND proceso='CobroAutPP';
	COMMIT;
...
DOCUMENT
'AUTOR: Martha A Hdz 																																			',
'DESCRIPCION: SP que realiza dos procesos: 																														',
'	1.- Cancelacion de invitaciones no autorizadas por el cliente, 7 dias despues de enviadas. 																	',
'	2.- Envio de sms por medio de latinia de forma batch. SMS que no fue posible enviar durante el proceso por estar fuera del horario permitido: 8 am a 8 pm. 	',
'FECHA DE CREACION:  Julio 2018 																																',
'BD: bdicred																																					',
...
'			  6 Err en credisolucion  7 SMS con credisolucion																								   	',
