CREATE PROCEDURE "informix".sp_consultsolicitudes_cancelaciones( pfecha_reg date, cod_oper integer, pRegistros integer, pRecuperacion integer)
RETURNING CHAR(5),
 integer,
 CHAR(30),
 CHAR(10),
...
BEGIN
        ...
        --SET DEBUG FILE TO "/RESPALDOSNEW/rsv/portabilidad/TASF/bdicheq/V2/sp_consultsolicitudes_cancelaciones.out";
		--SET DEBUG FILE TO "/resplogifx/conciliachq/portabilidad/22E/sp_consultsolicitudes_cancelaciones.out";
        --TRACE ON;
        ...
        IF NVL(pfecha_reg, "") = "" OR NVL(cod_oper,"") = "" OR NVL(pRegistros,"") = "" OR NVL(pRecuperacion,"") = ""  THEN
            LET cCodRet = "00002";   
            RETURN cCodret, isolicitudes, cfolio_sol_can,cNumcte,cnombco_orden,cEstatus, iRegistros;
        END IF;
		...
		-- OBTIENE EL ULTIMO DIA HABIL PARA CECOBAN   
		EXECUTE PROCEDURE bdicheq:sp_porta_cal_ult_dia_hab("001",dtFechaFin)
	    INTO cCodret,v_idenFechultDiaHab; 
		
	
       	-- SOLO SI ES EL ULTIMO DIA HABIL DEL MES CONSIDERA MUESTA TAMBIEN LAS CANCELACIONES DE FORMA AUTOMTICA
	   	IF dtFechaHoy = v_idenFechultDiaHab THEN 

		   IF cod_oper = 22  THEN
				
					FOREACH 
						SELECT SKIP pRegistros FIRST pRecuperacion folio_cancelacion, num_cte, bco_ordenante
						INTO cfolio_sol_can, cNumcte, cbco_orden 
						FROM (
							SELECT ps.folio_cancelacion, ps.num_cte, ps.bco_ordenante
		...
		ELSE -- SI NO ES FIN DE MES, ENTONCES PROCESA  SOLO LAS PORTABILIDADES NORMALES 
		   ...
		   IF cod_oper = 22  THEN
					...
					FOREACH 
						SELECT SKIP pRegistros FIRST pRecuperacion ps.folio_cancelacion, ps.num_cte, ps.bco_ordenante
						INTO cfolio_sol_can, cNumcte, cbco_orden                       
						FROM "informix".sc_portacec_solicitud ps
						LEFT JOIN "informix".sc_portacec_bitacora_cancelaciones psc ON ps.folio_cancelacion = psc.folio_cancelacion
						WHERE ps.fecha_solca_portabilidad BETWEEN vfecant AND vfecha_reg             
