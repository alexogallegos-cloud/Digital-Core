CREATE PROCEDURE "informix".grabacom_tc
	(pTarjeta      CHAR(20),
	 pNumCredito    CHAR(20),
         pComSucursal   CHAR(4),
         pComUsuario    CHAR(8),
	 ...
	 pComBandera    CHAR(1),
	 pComMontoDlls  Money(14,2),
	 pEmpresa	CHAR(3),
	 pProducto      CHAR(4),
         pFechaHoy      DATE,
	 pDivisa        CHAR(2),
	 pTipoCambio    MONEY(14,2))
  ...
  -- ***********************************************************
  -- Asigna codigo de referencia de acuerdo a tipo de comision *
  -- ***********************************************************
  IF pComNumTran = "  " THEN
	LET CodigoRef = 5; -- Comision por Disposicion
  ELIF pcomNumTran = " " THEN
	LET CodigoRef = 6; -- Comision por Consulta
  ELSE
	LET cod_ret= "100";
        RETURN cod_ret;
  END IF
  ...
  -- ****************************************
  -- Aplica Comision de Acuerdo a Parametro *
  -- ****************************************
   IF pComBandera = "1" THEN -- No Comis en linea y No Comis Pendientes

   ELIF pComBandera = "2" THEN -- Comis en linea y No comis Pendientes

	EXECUTE PROCEDURE genmov_tc(pEmpresa, pNumCredito, pProducto,
				    CodigoRef, CodigoFun, pFechaHoy,
				    pComMonto, pComFolio, pComSucursal, 
				    pDivisa, pComNumTran, pTarjeta, 
				    pComReferencia, pTipoCambio, pComMontoDlls)
	INTO cod_ret, v_mensaje;
	

   ELSE -- Comis en Linea y si Comis Pendientes
...
END PROCEDURE
DOCUMENT
'Esta funcion realiza el cargo  por comision a una tarjeta de credito  ',
'AUTOR : iAntonio Ruiz Mtzo',
'FECHA : 2/1/2006',
'BD : bdicred ',
'CLIENTE : COPPEL';

CREATE PROCEDURE "informix".grabadisp_tc
	(pTarjeta    CHAR(20),
	 pNumCredito CHAR(20),
         pSucursal   CHAR(4),
         pUsuario    CHAR(8),
  ...
  -- ***********************************************************
  IF pBandera = "1" THEN
	LET CodigoRef = 5; -- Disposicion por Compra  
  ELIF pBandera = "2" THEN
	LET CodigoRef = 6; -- Disposicion Efectivo 
  ELSE
	LET cod_ret= "100";
        RETURN cod_ret;
  END IF
  ...
  -- **********************************
  -- Aplica Movimiento de Disposicion *
  -- **********************************

	EXECUTE PROCEDURE genmov_tc(pEmpresa, pNumCredito, pProducto,
				    CodigoRef, CodigoFun, pFechaHoy,
				    pMonto, pFolio, pSucursal, 
				    pDivisa, pNumTran, pTarjeta, 
				    pReferencia, pTipoCambio, pMontoDlls)
...
END PROCEDURE
DOCUMENT
'Esta funcion realiza el cargo por Disposicion  a una tarjeta de credito  ',
'AUTOR : iAntonio Ruiz Mtzo',
'FECHA : 2/1/2006',
'BD : bdicred ',
'CLIENTE : COPPEL';
