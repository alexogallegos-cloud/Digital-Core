CREATE PROCEDURE "informix".ins_consulta_buro2_motor_pp(pempresa CHAR(3),psucursal CHAR(3), pusuario CHAR(8), pInstitucion CHAR(2),pnum_solicitud VARCHAR(25))
...
--SET DEBUG FILE TO '/home/sysifx/MarcoC/ins_consulta_buro2_motor_pp'||pnum_solicitud||'.out';
--TRACE ON;
...
SELECT COUNT(solicitud)
	...
	FROM bdicred:"informix".sd_pre_aprobados_trx
...
IF iBanPreAprobado <> 0 THEN
	 ...
	 --v_tp_tarjeta
	 SELECT tipo_solicitud INTO  v_tp_tarjeta FROM  bdisolic:ss_solicitudes WHERE num_Solicitud =  pnum_solicitud;
	 
	 SELECT grupo INTO v_grupo FROM bdicred:sd_pre_aprobados_trx WHERE solicitud = pnum_solicitud ;
	 
	 IF v_tp_tarjeta = 'T' THEN
	   LET v_tp_tarjeta = 'T';
	 ELSE
	   LET v_tp_tarjeta = 'P';
	 END IF;	 
	 ...
	 SELECT min_score INTO v_min_score FROM bdisolic:"informix".ss_perfil_riesgo WHERE empresa = pempresa AND grupo = v_grupo AND hit_buro = cCalifica AND id_riesgo = 3 AND tp_solicitud = v_tp_tarjeta;
 
	  IF NOT EXISTS(SELECT * FROM bdisolic:"informix".ss_resumen_scoring WHERE num_solicitud = pnum_solicitud)THEN
	   INSERT INTO bdisolic:"informix".ss_resumen_scoring(empresa,num_solicitud,seccion,evaluacion) VALUES ('001',pnum_solicitud,2,v_min_score);
	 END IF;
	 
	  UPDATE bdisolic:"informix".ss_resum_scor_fin
      SET evalua_cc = cCalifica,
          motivo_cc = vMensaje,
    ...
    WHERE empresa = pempresa
	  ...
	  IF p_cod_ret = '000' THEN
		 IF cCalifica = '0' AND dCompromisos IS NOT NULL  THEN -- Que los compromisos sean <> a null
    ...
    EXECUTE PROCEDURE bdisolic:"informix".sp_actualiza_status_sol(pempresa, 'sistema',pnum_solicitud, 'AT','', 'Autorizada') INTO p_cod_ret;
		...
		ELSE
				IF cStatusSol <> "AP" THEN
	                ...
	                END IF;
					 IF cCalifica = '0' THEN
                      ...
                      IF cTpsol = 'T' AND cNumproducto = '6001'  THEN
                        ...
                        IF cTpsol = 'P' AND  EXISTS (SELECT numproducto FROM bdicred:"informix".sd_productos_motor_pp WHERE numproducto = cNumproducto) THEN
                      ...
                      END IF;
	                        ...
	                        --IF cEstatusSol = 'BC'  OR (cEstatusSol = 'CC' AND vFalloSIC = 1) AND cEnvioparametrico IS NOT NULL THEN --RQM 09 606 Se agrego la condiciÃ²n de estatus CC y falloSic
							IF ( cEstatusSol = 'BC'  OR cEstatusSol = 'CC' ) AND cEnvioparametrico IS NOT NULL THEN --RQM 09 606 Se agrego la condiciÃ²n de estatus CC y falloSi
					...
					ELSE
						...
						IF cNumproducto = cProductoME THEN							
...
DOCUMENT
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------',
'DESCRIPCION: Clon del procedimiento ins_consulta_buro2_motor, agregando las funcionalidades para prestamo pp en motor de evaluacion  ',
'AUTOR: Andres Godinez Hernandez - Kairos Digital Solutions',
'------------------------------------------------------------------------------------',
