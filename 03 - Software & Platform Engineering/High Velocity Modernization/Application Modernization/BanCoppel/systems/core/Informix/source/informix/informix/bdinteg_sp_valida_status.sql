CREATE PROCEDURE "informix".sp_valida_status(pFolio CHAR(12))

RETURNING CHAR(5) AS CodRet, CHAR(50) AS Status;

--DEFINE VARIABLES
DEFINE iSqlErr                 INTEGER;
DEFINE cCodRet                 CHAR(5);
DEFINE cStatus                 CHAR(5);
DEFINE cStatus1                CHAR(5);
DEFINE cStatus11               CHAR(5);
DEFINE cStatus2                CHAR(5);
DEFINE cStatus22               CHAR(5);
DEFINE cDescripcion            CHAR(50);

DEFINE cnum_tdc_coppel         CHAR(12);
DEFINE cstatus_tdc_coppel      CHAR(2);
DEFINE cnum_prestamo           CHAR(12);
DEFINE cstatus_prestamo        CHAR(2);
DEFINE cnum_tdc_bcoppel        CHAR(12);
DEFINE cstatus_tdc_bcoppel     CHAR(2);
DEFINE sfecha_insert           DATE;

DEFINE ssempresa               CHAR(3);
DEFINE ssnum_solicitud         CHAR(20);
DEFINE ssnumcte                CHAR(20);
DEFINE sstipo_solicitud        CHAR(1);
DEFINE ssstatus_solicitud      CHAR(2);
DEFINE ssstatus_solicitud2     CHAR(2);
DEFINE ssstatus_solicitud3     CHAR(2);
DEFINE ssnum_producto          CHAR(4);
DEFINE sstipo_prestamo         CHAR(2);
DEFINE ssmonto_solicitado      DECIMAL(18,2);

DEFINE sscodigo                CHAR(8);
DEFINE ssdescripcion           CHAR(256);
DEFINE ssdescripcion2          CHAR(256);
DEFINE ssdescripcion3          CHAR(256);

DEFINE vt_descripcion1         CHAR(50);
DEFINE vt_descripcion2         CHAR(50);
DEFINE vt_descripcion3         CHAR(50);

DEFINE vt_cuantosPA            INTEGER;
DEFINE vt_cuantosEN            INTEGER;
DEFINE vt_cuantosRT            INTEGER;
DEFINE vt_cuantos              INTEGER;
DEFINE vt_valida_prestamo      INTEGER;

DEFINE vt_valida               INTEGER;

DEFINE istatus_valua           INTEGER;
DEFINE cfolio           	   CHAR(12);
DEFINE vt_countSolMovil        INTEGER;

DEFINE iTipoFolio              INTEGER;
DEFINE ss_cuantos              INTEGER;


--INICIALIZA VARIABLES
LET iSqlErr		                = 0;
LET cCodRet		                = '00000';
LET cStatus		                = '';
LET cStatus1		            = '';
LET cStatus11		            = '';
LET cStatus2		            = '';
LET cStatus22		            = '';
LET cDescripcion                = '';

LET cnum_tdc_coppel             = "";
LET cstatus_tdc_coppel          = "";
LET cnum_prestamo               = "";
LET cstatus_prestamo            = "";
LET cnum_tdc_bcoppel            = "";
LET cstatus_tdc_bcoppel         = "";

LET ssempresa                   = "";
LET ssnum_solicitud             = ""; 
LET ssnumcte                    = "";
LET sstipo_solicitud            = "";
LET ssstatus_solicitud          = "";
LET ssstatus_solicitud2         = "";
LET ssstatus_solicitud3         = "";
LET ssnum_producto              = "";
LET sstipo_prestamo             = "";
LET ssmonto_solicitado          = 0;

LET sscodigo                    = "";
LET ssdescripcion               = "";
LET ssdescripcion2              = "";
LET ssdescripcion3              = "";

LET vt_descripcion1             = "";
LET vt_descripcion2             = "";
LET vt_descripcion3             = "";

LET vt_cuantosPA                = 0;
LET vt_cuantosEN                = 0;
LET vt_cuantosRT                = 0;
LET vt_valida                   = 0;
LET sfecha_insert               = "";
LET vt_cuantos                  = 0;
LET istatus_valua               = null;
LET cfolio               		= null;
LET vt_countSolMovil            = 0;

LET iTipoFolio                  = 0;
LET ss_cuantos                  = 0;



BEGIN
	-- ERRORES DE INFORMIX
	ON EXCEPTION SET iSqlErr
		IF iSqlErr <> 0 THEN
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cDescripcion;
		END IF;
	END EXCEPTION;

	--SET DEBUG FILE TO '/informix/ztr/sp_valida_status.out';
	--TRACE ON;

	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;


	--VALIDA PARAMETRO
	IF NVL(pFolio,'' ) = '' THEN
		LET cCodRet = '00001';
		LET cDescripcion="Ingresar folio de la solicitud o numero telefonico";
		RETURN cCodRet,cDescripcion;
	END IF;

	--VALIDA LONGITUD DE PARAMETRO
	IF NVL(LEN(TRIM(pFolio)),'' ) != 12 AND NVL(LEN(TRIM(pFolio)),'' ) != 10 THEN
		LET cCodRet = '00001';
		LET cDescripcion="Folio o numero de telefono NO Existe.";
		RETURN cCodRet,cDescripcion;
	END IF;

	IF LEN(TRIM(pFolio)) = 12 THEN
	
		LET iTipoFolio = TO_NUMBER(SUBSTRING(pFolio FROM 1 FOR 3));
		
		IF iTipoFolio >= 600 THEN
			SELECT COUNT(*) INTO ss_cuantos
			FROM bdisolic:ss_solicitudes
			WHERE bdisolic:ss_solicitudes.empresa = "001"
			AND bdisolic:ss_solicitudes.num_solicitud = pFolio;
			
			IF ss_cuantos = 0 THEN
				LET cCodRet = '00017';
				LET cDescripcion="Solicitud NO Existe.";
				RETURN cCodRet,cDescripcion;
			ELSE
				SELECT FIRST 1 status_solicitud INTO ssstatus_solicitud
				FROM bdisolic:ss_solicitudes
				WHERE bdisolic:ss_solicitudes.empresa = "001"
				AND bdisolic:ss_solicitudes.num_solicitud = pFolio;
				
				SELECT codigo, descripcion
				INTO sscodigo, ssdescripcion
				FROM bdinteg:si_status_solicitud
				WHERE bdinteg:si_status_solicitud.codigo = ssstatus_solicitud;
				
				LET ssdescripcion=TRIM(ssdescripcion)||' '||pFolio;
				
				RETURN cCodRet,ssdescripcion;
			END IF;
			
		ELSE
			SELECT COUNT(*) INTO vt_cuantos
			FROM si_solicitud_movil
			WHERE folio=pFolio;

			IF vt_cuantos = 0 THEN
				LET cCodRet = '00015';
				LET cDescripcion="Folio NO Existe.";
				RETURN cCodRet,cDescripcion;
			END IF;
		END IF;
		
	END IF;

	IF LEN(TRIM(pFolio)) = 10 THEN
		SELECT {+INDEX (bdinteg:si_solicitud_movil idx_telefono)} COUNT(*) INTO vt_cuantos
		FROM si_solicitud_movil
		WHERE telefono=pFolio;
  
		IF vt_cuantos = 0 THEN
			LET cCodRet = '00016';
			LET cDescripcion="Numero de telefono NO Existe.";
			RETURN cCodRet,cDescripcion;
		END IF;

		/*IF vt_cuantos > 1 THEN
			LET cCodRet = '00017';
			LET cDescripcion="Numero de telefono registrado mas de una vez.";
			RETURN cCodRet,cDescripcion,ssnum_solicitud;
		END IF;*/
	END IF;
	--Valida los datos de la solicitud.

	--Consulta los datos de la solicitud en si_solicitud_movil mediante el folio
    IF LEN(TRIM(pFolio)) = 12 THEN
		SELECT num_tdc_coppel,status_tdc_coppel,num_prestamo,status_prestamo,num_tdc_bcoppel,status_tdc_bcoppel,fecha_insert, status_valua, folio
		INTO cnum_tdc_coppel,cstatus_tdc_coppel,cnum_prestamo,cstatus_prestamo,cnum_tdc_bcoppel,cstatus_tdc_bcoppel,sfecha_insert, istatus_valua, cfolio
		FROM si_solicitud_movil
		WHERE folio=pFolio;
    END IF;

	--Consulta los datos de la solicitud en si_solicitud_movil mediante el telefono
    IF LEN(TRIM(pFolio)) = 10  THEN
		SELECT {+INDEX (bdinteg:si_solicitud_movil idx_telefono)} num_tdc_coppel,status_tdc_coppel,num_prestamo,status_prestamo,num_tdc_bcoppel,status_tdc_bcoppel,fecha_insert, status_valua, folio
		INTO cnum_tdc_coppel,cstatus_tdc_coppel,cnum_prestamo,cstatus_prestamo,cnum_tdc_bcoppel,cstatus_tdc_bcoppel,sfecha_insert, istatus_valua, cfolio
		FROM si_solicitud_movil
		WHERE telefono=pFolio
		and
		id = 
		(select max(id) from si_solicitud_movil WHERE telefono=pFolio);
		
    END IF;

	--En caso de que si_solicitud_movil.status_valua = 0 se envia mensaje de solicitud en proceso
    IF  (istatus_valua IS NOT NULL AND istatus_valua=0) THEN
		LET cCodRet = '00010';
		LET cDescripcion="Solicitud En Proceso.";
		RETURN cCodRet,cDescripcion;
    END IF;
	
	--En caso de que si_solicitud_movil.status_valua = 2 se envia mensaje de solicitud rechazada
    IF  (istatus_valua IS NOT NULL AND istatus_valua=2) THEN
		LET cCodRet = '00021';
		LET cDescripcion="Solicitud Rechazada por Temporalidad.";
		RETURN cCodRet,cDescripcion;
    END IF;

    --Valida si hay datos para comparar
    IF  (cnum_tdc_coppel IS NULL OR cnum_tdc_coppel="")
    AND (cnum_prestamo IS NULL OR cnum_prestamo ="")
    AND (cnum_tdc_bcoppel IS NULL OR cnum_tdc_bcoppel="") THEN

		SELECT COUNT(*)
		INTO vt_countSolMovil
		FROM bdisolic:ss_solicitudes_movil
		WHERE 
		bdisolic:ss_solicitudes_movil.folio_movil = cfolio 
		AND empresa = "001";

		--En caso de que si_solicitud_movil.status_valua = 1 y no exista registro en ss_solicitudes_movil se envia mensaje de solicitud en proceso
		IF (vt_countSolMovil = 0)
		THEN
			LET cCodRet = '00010';
			LET cDescripcion="Solicitud En Proceso.";
			RETURN cCodRet,cDescripcion;
		END IF;

		SELECT FIRST 1 num_solicitud, status
		INTO ssnum_solicitud, ssstatus_solicitud
		FROM bdisolic:ss_solicitudes_movil
		WHERE 
		bdisolic:ss_solicitudes_movil.folio_movil = cfolio 
		AND empresa = "001";

		--En caso de que si_solicitud_movil.status_valua = 1 y exista registro en ss_solicitudes_movil con estatus 3, se envia mensaje de solicitud rechazada
		IF (ssstatus_solicitud IS NOT NULL)
		AND (ssstatus_solicitud = 3)
		AND (ssnum_solicitud IS NOT NULL)
		AND (ssnum_solicitud="")
		THEN
			LET cCodRet = '00022';
			LET cDescripcion="Solicitud Rechazada por Precalificacion";
			RETURN cCodRet,cDescripcion;
		END IF;
    END IF;

    --VALIDA PARAMETRO 
    IF cnum_tdc_coppel IS NULL THEN
       --LET cCodRet = '00002';
       --LET cDescripcion="Solicitud no Definida Verifique.";
       --RETURN cCodRet,cDescripcion;

       LET ssstatus_solicitud = "";
    END IF;

    IF cnum_tdc_bcoppel IS NULL THEN 
       --LET cCodRet = '00003';
       --LET cDescripcion="Solicitud no Definida Verifique.";
       --RETURN cCodRet,cDescripcion;
	  -- LET ssstatus_solicitud = "";
    END IF;
	
	-- Valida que exista relacion entre tablas si_solicitud_movil y bdisolic:ss_solicitudes
	IF cnum_tdc_coppel != "" THEN
		  LET vt_valida_prestamo = 0;
		  
		  SELECT COUNT(*) INTO vt_valida_prestamo
		  FROM bdisolic:ss_solicitudes
		  WHERE bdisolic:ss_solicitudes.empresa = "001"
		  AND bdisolic:ss_solicitudes.num_solicitud = cnum_tdc_coppel;
			
		 IF vt_valida_prestamo = 0 THEN
			 LET cCodRet = '00020';
			 LET cDescripcion="Solicitud NO Encontrada Verifique.";
			 RETURN cCodRet,cDescripcion;
		 END IF;
	END IF;
	
	---Arma Cursos para saber el estatus de num_tdc_coppel
	IF cnum_tdc_coppel != "" THEN
		FOREACH
			
			SELECT empresa, num_solicitud, numcte, tipo_solicitud,
				 status_solicitud, num_producto, tipo_prestamo, monto_solicitado
			INTO ssempresa, ssnum_solicitud, ssnumcte, sstipo_solicitud,
			   ssstatus_solicitud, ssnum_producto, sstipo_prestamo, ssmonto_solicitado
			FROM bdisolic:ss_solicitudes
			WHERE bdisolic:ss_solicitudes.empresa = "001"
			AND bdisolic:ss_solicitudes.num_solicitud = cnum_tdc_coppel

			IF ssnum_producto IS NOT NULL AND ssnum_solicitud = "" THEN
				LET cCodRet = '00005';
				LET cDescripcion="Solicitud En Proceso.";
				LET cDescripcion=cDescripcion||' '||ssnum_solicitud;
				--LET cDescripcion="Solicitud Rechazada Verifique.";
				RETURN cCodRet,cDescripcion;
				EXIT FOREACH;
			END IF;
 
			---Extrae el estatus de la solicitud
			SELECT codigo, descripcion
			INTO sscodigo, ssdescripcion
			FROM bdinteg:si_status_solicitud
			WHERE bdinteg:si_status_solicitud.codigo = ssstatus_solicitud;
			
			IF ssstatus_solicitud = "AT" THEN
				LET cDescripcion = "Solicitud 1 Autorizada";
			END IF;
			
			IF sscodigo = 'BC' THEN
				LET cDescripcion = "Solicitud "|| TRIM("En Buro de Credito");
				--LET cDescripcion = TRIM(ssdescripcion)||' '||ssnum_solicitud;
			ELSE
				LET cDescripcion = "Solicitud "|| TRIM(ssdescripcion);
				--LET cDescripcion = TRIM(ssdescripcion)||' '||ssnum_solicitud;
			END IF;
		END FOREACH;
	END IF; 


	-- Valida que exista relacion entre tablas si_solicitud_movil y bdisolic:ss_solicitudes
	IF cnum_prestamo != "" THEN
		  LET vt_valida_prestamo = 0;
		  SELECT COUNT(*) INTO vt_valida_prestamo
		  FROM bdisolic:ss_solicitudes
		  WHERE bdisolic:ss_solicitudes.empresa = "001"
		  AND bdisolic:ss_solicitudes.num_solicitud = cnum_prestamo;
		
		 IF vt_valida_prestamo = 0 THEN
			 LET cCodRet = '00020';
			 LET cDescripcion="Solicitud NO Encontrada Verifique.";
			 LET cDescripcion=cDescripcion||' '||ssnum_solicitud;
			 RETURN cCodRet,cDescripcion;
		 END IF;

	END IF; 
	
    ---Arma Cursos para saber el estatus de num_prestamo
	FOREACH
		
		SELECT empresa, num_solicitud, numcte, tipo_solicitud,
			 status_solicitud, num_producto, tipo_prestamo, monto_solicitado
		INTO ssempresa, ssnum_solicitud, ssnumcte, sstipo_solicitud,
		   ssstatus_solicitud2, ssnum_producto, sstipo_prestamo, ssmonto_solicitado
		FROM bdisolic:ss_solicitudes
		WHERE bdisolic:ss_solicitudes.empresa = "001"
		AND bdisolic:ss_solicitudes.num_solicitud = cnum_prestamo

		IF ssnum_producto IS NOT NULL AND ssnum_solicitud = "" THEN
			LET cCodRet = '00006';
			--LET cDescripcion="Solicitud Rechazada Verifique....";
			LET cDescripcion="Solicitud En Proceso.";
			--LET cDescripcion=cDescripcion;
			LET cDescripcion=cDescripcion||' '||ssnum_solicitud;
			RETURN cCodRet,cDescripcion;
			EXIT FOREACH;
		END IF;

		---Extrae el estatus de la solicitud
		SELECT codigo, descripcion
		INTO sscodigo, ssdescripcion2
		FROM bdinteg:si_status_solicitud
		WHERE bdinteg:si_status_solicitud.codigo = ssstatus_solicitud2;
		
		IF ssdescripcion IS NOT NULL THEN
			LET cDescripcion = TRIM(ssdescripcion);
			--LET cDescripcion = TRIM(ssdescripcion)||' '||ssnum_solicitud;
		END IF;

		IF ssdescripcion IS NULL THEN
			LET cDescripcion = "NO HAY Relacion.";
			--LET cDescripcion = "NO HAY Relacion."||' '||ssnum_solicitud;
		END IF;
	END FOREACH;


	---Arma Cursos para saber el estatus de num_tdc_bcoppel
	IF cnum_tdc_bcoppel != "" AND (cnum_tdc_coppel IS NULL OR cnum_tdc_coppel = "")THEN
		FOREACH

			SELECT empresa, num_solicitud, numcte, tipo_solicitud,
				 status_solicitud, num_producto, tipo_prestamo, monto_solicitado
			INTO ssempresa, ssnum_solicitud, ssnumcte, sstipo_solicitud,
			   ssstatus_solicitud3, ssnum_producto, sstipo_prestamo, ssmonto_solicitado
			FROM bdisolic:ss_solicitudes
			WHERE bdisolic:ss_solicitudes.empresa = "001"
			AND bdisolic:ss_solicitudes.num_solicitud = cnum_tdc_bcoppel
			
			IF ssnum_producto IS NOT NULL AND ssnum_solicitud = "" THEN
				LET cCodRet = '00007';
				--LET cDescripcion="Solicitud Rechazada Verifique....";
				LET cDescripcion="Solicitud En Proceso.";
				--LET cDescripcion=cDescripcion||' '||ssnum_solicitud;
				RETURN cCodRet,cDescripcion;
				EXIT FOREACH;
			END IF;

			---Extrae el estatus de la solicitud
			SELECT codigo, descripcion
			INTO sscodigo, ssdescripcion3
			FROM bdinteg:si_status_solicitud
			WHERE bdinteg:si_status_solicitud.codigo = ssstatus_solicitud3;
			
			IF ssstatus_solicitud3 = "AT" THEN
				LET cDescripcion = "Solicitud 3 Autorizada";
			END IF;
			
			IF ssstatus_solicitud3 = "BC" THEN
				LET cDescripcion = "Solicitud "|| TRIM("En Buro de Credito");
				--LET cDescripcion = TRIM(ssdescripcion)||' '||ssnum_solicitud;
			ELSE
				LET cDescripcion ="Solicitud "|| TRIM(ssdescripcion3);
				--LET cDescripcion = TRIM(ssdescripcion)||' '||ssnum_solicitud;
			END IF;	
		END FOREACH;
	END IF; 
	

	IF ssstatus_solicitud = "PA" AND ssstatus_solicitud3 = "PA" THEN
		LET cDescripcion ="Solicitud "|| TRIM(ssdescripcion);
	ELSE
		IF ssstatus_solicitud != "PA" AND ssstatus_solicitud3 = "PA" THEN
			LET cDescripcion ="Solicitud "||TRIM(ssdescripcion3);
		END IF;
		IF ssstatus_solicitud = "PA" AND ssstatus_solicitud3 != "PA" THEN
			LET cDescripcion = "Solicitud "||TRIM(ssdescripcion);
		END IF;
		
		IF ssstatus_solicitud IN ("EC","MC","EE","OS","OA") AND ssstatus_solicitud3 IN ("EC","MC","EE","OS","OA") THEN
			LET cDescripcion = "Solicitud "||TRIM(ssdescripcion);
		END IF;

		IF ssstatus_solicitud = "RT" AND ssstatus_solicitud3 IN ("EC","MC","EE","OS","OA") THEN
			LET cDescripcion = "Solicitud "||TRIM(ssdescripcion3);
		END IF;

		IF ssstatus_solicitud IN ("EC","MC","EE","OS","OA") AND ssstatus_solicitud3 = "RT" THEN
			LET cDescripcion = "Solicitud "||TRIM(ssdescripcion);
		END IF;

		--RT RP
		IF ssstatus_solicitud IN ("RT") AND ssstatus_solicitud3 = "RT" THEN
			LET cDescripcion = "Solicitud "||TRIM(ssdescripcion);
		END IF;

		IF ssstatus_solicitud = "RP" AND ssstatus_solicitud3 IN ("EC","MC","EE","OS","OA") THEN
			LET cDescripcion = "Solicitud "||TRIM(ssdescripcion3);
		END IF;

		IF ssstatus_solicitud IN ("EC","MC","EE","OS","OA") AND ssstatus_solicitud3 = "RP" THEN
			LET cDescripcion = "Solicitud "||TRIM(ssdescripcion);
		END IF;

		IF ssstatus_solicitud IN ("RP") AND ssstatus_solicitud3 = "RP" THEN
			LET cDescripcion = " "||TRIM(ssdescripcion);
		END IF;
		--END RT RP
		
		IF sscodigo = "" THEN
			LET cDescripcion = "Solicitud "||TRIM("Rechazada por Precalificacion");
		END IF;

		IF ssstatus_solicitud = "PA" AND ssstatus_solicitud3 = "AT" THEN
			LET cDescripcion = TRIM(ssdescripcion);
		END IF;

		IF ssstatus_solicitud = "AT" AND ssstatus_solicitud3 = "AT" THEN
			LET cDescripcion = TRIM(ssdescripcion);
		END IF;

		IF ssstatus_solicitud = "AT" AND ssstatus_solicitud3 IN ("EC","MC","EE","OS","OA") THEN
			LET cDescripcion = TRIM(ssdescripcion3);
		END IF;
		IF ssstatus_solicitud IN ("EC","MC","EE","OS","OA") AND ssstatus_solicitud3 = "AT" THEN
			LET cDescripcion = TRIM(ssdescripcion);
		END IF;
		--if para compara el aspecto AP modificado 31/12/2019
		IF ssstatus_solicitud ="AP"  THEN
			LET cDescripcion = "Credito Asignado";
		END IF;
		-- fin de la modificacion
	END IF;

	IF ssstatus_solicitud2 != "" THEN
		LET cDescripcion ="Solicitud "|| TRIM(ssdescripcion2);
	END IF;
	   
	LET cDescripcion= TRIM(cDescripcion)||' '||ssnum_solicitud;
	RETURN cCodRet,cDescripcion;
	END
END PROCEDURE
DOCUMENT
"Spl para Validar los estatus de las solicitudes de captura movil ",
"base de datos: bdinteg",
"AUTOR : Sergio Fabricio Ruiz Jimenez",
"FECHA : 18/Marzo/2015",
"Ver.  : 1.0",
"Mod   : Modificacion para Rechazo de Solicitudes de captura movil ",
"base de datos: bdinteg",
"AUTOR : Sergio Fabricio Ruiz Jimenez",
"FECHA : 26/Marzo/2015",
"Mod   : Modificacion: se agrega validacion para buscar solicitud por medio de telefono ",
"base de datos: bdinteg",
"AUTOR : OMC",
"FECHA : 05/Diciembre/2016",
"Ver.  : 1.1",
"AUTOR : Eduardo Martinez Martinez",
"FECHA : 26/Julio/2022",
"Mod   : Modificacion: Al realizar una solicitud por medio del alta mÃ³vil y al consultarla aparece solo el mensaje de SOLICITUD y no arroja ninguna respuesta esto causa confusiÃ³n en los colaboradores y tardanza al dar una respuesta al cliente, se solicita modificar el mensaje de respuesta y que muestre el mensaje 'Rezachada por precalificacion' o 'Buro de credito'. Al realizar pruebas se detecta un error diferente al reportado donde el mensaje sale como âSolicitud Pre-autorizadaâ pero el numero de solicitud no corresponde a este mensaje, se realizan los ajustes para que muestre el mensaje correcto de la solicitud.",
"base de datos: bdinteg",
"AUTOR : Zahide Tellez Ramirez",
"FECHA : 02/Agosto/2022",
"Mod   : Modificacion: Al realizar pruebas se detecta un error en la respuesta del mensaje, se realizan las modificaciones para que se muestre el status correcto de cada solicitud que se consulte. ",
"base de datos: bdinteg",
"AUTOR : Zahide Tellez Ramirez",
"FECHA : 25/Agosto/2022",
"Mod   : Modificacion: Se realiza un ajuste a los casos con el status 2 y 3, actualmente solo muestra el mensaje 'Solicitud Rechazada Verifique ...', se consulta con Ariel y menciona que los mensajes a mostrar son 'Solicitud Rechazada por Temporalidad' y 'Solicitud Rechazada por PrecalificaciÃ³n'",
"base de datos: bdinteg";

CREATE PROCEDURE "informix".sp_autentica_usuariomovil(pEjecutivo CHAR(8),pPassword CHAR(20), pImei CHAR(40))

RETURNING CHAR(5) AS CodRet,CHAR(9258) AS ParamCatalagos;

--DEFINE VARIABLES
DEFINE	iSqlErr 		INTEGER;
DEFINE	cCodRet 		CHAR(5);
DEFINE 	cParamCat 		CHAR(9258);
DEFINE  cValor 			CHAR(1);
DEFINE  cValor2 		CHAR(4);
DEFINE  cValor3 		CHAR(5);
DEFINE  cDescripcion  	CHAR(480);
DEFINE  cDescripcion1 	CHAR(32);
DEFINE  cDescripcion2 	CHAR(32);
DEFINE  cDescripcion3 	CHAR(32);
DEFINE  cDescripciono 	CHAR(32);
DEFINE  cDescripcion4 	CHAR(280);
DEFINE  cDescripcion5 	CHAR(280);
DEFINE  cDescripcion6 	CHAR(480);
DEFINE  cDescripcion7 	CHAR(1);
DEFINE  cDescripcion8 	CHAR(1);
DEFINE  cCteCoppel    	CHAR(9);
DEFINE  cDomActCte 		CHAR(9);
DEFINE  cClave        	CHAR(1);
DEFINE  cDescripcion11  CHAR(15);
DEFINE  cEdoCivil		CHAR(80);
DEFINE  cHabita         CHAR(1);
DEFINE  cDescripcion12  CHAR(32);
DEFINE  cDescHabEN		CHAR(198);
DEFINE	iActId			INTEGER;
DEFINE  cDescripcion13	CHAR(56);
DEFINE  cActividad		CHAR(570);
DEFINE  iSubAct			INTEGER;
DEFINE  cDescripcion14	CHAR(66);
DEFINE  cSubActividad	CHAR(6565);
DEFINE 	iElemento		INTEGER;
DEFINE	cDescripcion15	CHAR(35);
DEFINE  cCompIng		CHAR(360);
DEFINE 	cDescripcion16	CHAR(25);
DEFINE	cEscolaridad	CHAR(156);
DEFINE  cSExo			CHAR(17);
DEFINE	cDescripcion18	CHAR(15);
DEFINE  cCarriers		CHAR(80);
DEFINE  cDescripcion19	CHAR(1);
DEFINE  cDescripcion20	CHAR(1);
DEFINE  cDescripcion21	CHAR(4);
DEFINE  cEjecutivo		CHAR(8);
DEFINE  cPassword 		CHAR(20);
DEFINE  cImei	 		CHAR(40);
DEFINE  cActivo 		CHAR(1);
DEFINE  cVersion 		CHAR(5);
DEFINE  cDescripcion22	CHAR(1);
DEFINE  cLimiteColonias	CHAR(2);
DEFINE  cLimiteLike		CHAR(2);
DEFINE  cLimiteTiempo	CHAR(8);

--INICIALIZA VARIABLES	
LET iSqlErr			= 0;
LET cCodRet			= '00000';
LET cParamCat 		= '';
LET cValor 			= '';
LET cValor2			= '';
LET cDescripcion 	= ''; 
LET cDescripcion1 	= '';
LET cDescripcion2 	= '';
LET cDescripcion3 	= '';
LET cDescripciono	= '';
LET cDescripcion4 	= '';
LET cDescripcion5 	= '';
LET cDescripcion6 	= '';
LET cDescripcion7 	= '';
LET cDescripcion8 	= '';
LET cCteCoppel 		= 'S;Si|N;No';
LET cDomActCte 		= 'S;Si|N;No';
LET cClave			= '';
LET cDescripcion11	= '';
LET cEdoCivil		= '';
LET cHabita 		= '';
LET cDescripcion12 	= '';
LET cDescHabEN 		= '';
LET iActId			= 0;
LET cDescripcion13	= '';
LET cActividad		= '';
LET iSubAct			= 0;
LET cDescripcion14	= '';
LET cSubActividad	= '';
LET iElemento		= 0;
LET cDescripcion15 	= '';
LET cCompIng		= '';
LET cDescripcion16 	= '';
LET cEscolaridad	= '';
LET cSexo			= 'M;Mujer |H;Hombre';
LET cDescripcion18 	= '';
LET cCarriers		= '';
LET cDescripcion19 	= '';
LET cDescripcion20	= '';
LET cDescripcion21	= '';
LET cEjecutivo		= '';
LET cPassword 		= '';
LET cImei	 		= '';	
LET cActivo 		= '';
LET cVersion		= '';
LET cValor3			= '';
LET CDescripcion22	= '';
LET cLimiteColonias	= '';
LET cLimiteLike		= '';
LET cLimiteTiempo	= '';

BEGIN
	-- ERRORES DE INFORMIX
	ON EXCEPTION SET iSqlErr
		IF iSqlErr <> 0 THEN
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cParamCat;	
		END IF;
	END EXCEPTION;

--SET DEBUG FILE TO '/informix/UAI/sp_autentica_usuariomovil.out';
--TRACE ON;

SET ISOLATION TO DIRTY READ;
SET LOCK MODE TO WAIT 3;

--VALIDA PARAMETROS
IF NVL(pEjecutivo,'') = '' OR NVL(pPassword,'') = '' OR NVL(pImei,'') = '' THEN
	
	LET cCodret = '00001';
	LET cParamCat='Falta introducir algun dato obligatorio';
	RETURN cCodRet,cParamCat;

END IF;
	
	--VALIDA SI EXISTE EL USUARIO
	
	--******************************************** 
	IF LEN(pImei) = 15 THEN
		SELECT {+INDEX (bdinteg:si_usuario_movil idx_usa_movil)} ejecutivo,password,imei,activo
		INTO cEjecutivo,cPassword,cImei,cActivo
		FROM "informix".si_usuario_movil
		WHERE ejecutivo = pEjecutivo and imei=pImei;
	ELSE
		SELECT {+INDEX (bdinteg:si_usuario_movil idx_usa_movil)} ejecutivo,password,device_id,activo
		INTO cEjecutivo,cPassword,cImei,cActivo
		FROM "informix".si_usuario_movil
		WHERE ejecutivo = pEjecutivo and device_id=pImei;
	END IF
	--********************************************
			
	IF cEjecutivo <> '' THEN
		IF  cPassword <> pPassword THEN
			LET cCodret = '00004';
			LET cParamCat='Password incorrecto';
			RETURN cCodRet,cParamCat;
		ELIF cImei <> pImei THEN
			LET cCodret = '00005';
			LET cParamCat='IMEI o Device ID incorrecto';
			RETURN cCodRet,cParamCat;
			
		ELIF cActivo <> '1' THEN
		
			LET cCodret = '00003'; -- USUARIO DADO DE BAJO
			LET cParamCat='Usuario no activo';
			RETURN cCodRet,cParamCat;
		END IF;	
	ELSE
		LET cCodret = '00002'; -- NO EXISTE USUARIO
		LET cParamCat='Usuario no autorizado';
		RETURN cCodRet,cParamCat;
	
	END IF;
	
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor_movil)} valor
			INTO cLimiteTiempo
			FROM "informix".si_param_movil 
			WHERE cod_param = 27;
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor_movil)} valor
			INTO cLimiteColonias
			FROM "informix".si_param_movil 
			WHERE cod_param = 25;
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor_movil)} valor
			INTO cLimiteLike
			FROM "informix".si_param_movil 
			WHERE cod_param = 26;

			SELECT {+INDEX (bdinteg:si_param_movil idx_valor_movil)} valor,descripcion 
			INTO cValor,cDescripcion
			FROM "informix".si_param_movil 
			WHERE cod_param = 1;
			
			LET cDescripcion1 = cValor||';'||cDescripcion;

			SELECT {+INDEX (bdinteg:si_param_movil idx_valor_movil)} valor,descripcion 
			INTO cValor,cDescripcion
			FROM "informix".si_param_movil 
			WHERE cod_param = 2;
			
			LET cDescripcion2 = cValor||';'||cDescripcion;
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor_movil)} valor,descripcion 
			INTO cValor,cDescripcion
			FROM "informix".si_param_movil 
			WHERE cod_param = 3;
			
			LET cDescripcion3 = cValor||';'||cDescripcion;
			
			LET cDescripcionO = '0,                              ';
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor_movil)} valor,descripcion 
			INTO cValor,cDescripcion
			FROM "informix".si_param_movil 
			WHERE cod_param = 4;
			
			LET cDescripcion4 = cValor||';'||cDescripcion;
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor_movil)} valor,descripcion 
			INTO cValor,cDescripcion
			FROM "informix".si_param_movil 
			WHERE cod_param = 5;
			
			LET cDescripcion5 = cValor||';'||cDescripcion;
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor_movil)} valor,descripcion 
			INTO cValor,cDescripcion
			FROM "informix".si_param_movil 
			WHERE cod_param = 6;
			
			LET cDescripcion6 = cValor||';'||cDescripcion;
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor2_movil)} valor
			INTO cValor
			FROM "informix".si_param_movil 
			WHERE cod_param = 7;
			
			LET cDescripcion7=cValor;
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor2_movil)} valor
			INTO cValor
			FROM "informix".si_param_movil 
			WHERE cod_param = 8;
			
			LET cDescripcion8=cValor;
			
			FOREACH
				
			    SELECT {+INDEX (bdinteg:si_edocivil idx_clave_movil)} clave,descripcion
				INTO cClave,cDescripcion
				FROM "informix".si_edocivil ORDER BY clave DESC
				
				LET cDescripcion11 =cClave||';'||cDescripcion;	
				LET cEdoCivil = cDescripcion11||'|'||cEdoCivil;
				
			END FOREACH;
			
			FOREACH
			    SELECT {+INDEX (bdinteg:si_habitaen idx_habital_movil)} habita_en, descripcion 
				INTO cHabita, cDescripcion
				FROM "informix".si_habitaen 
                                WHERE empresa = "001" ORDER BY habita_en DESC

				LET cDescripcion12 =cHabita||';'||cDescripcion;
				LET cDescHabEN = cDescripcion12||'|'||cDescHabEN;
				
			END FOREACH;
			
			FOREACH
			
				SELECT id_act,descrip 
				INTO iActId, cDescripcion
				FROM "informix".si_actsubact_movil 
				WHERE (id_subact=0 OR id_subact=99) 
				ORDER BY id_act DESC,id_subact DESC
				
				LET cDescripcion13 = iActId||';'||cDescripcion;
				LET cActividad = cDescripcion13||'|'||cActividad;
				
			END FOREACH;
			
			FOREACH			
							
			        SELECT  id_act,id_subact,descrip 
				INTO iActId,iSubAct,cDescripcion
				FROM "informix".si_actsubact_movil 
				WHERE id_subact<>0 
				AND id_act >=1 AND id_act<=5
				ORDER BY id_act DESC,id_subact DESC
				
				LET cDescripcion14 = iActId||';'||iSubAct||';'||cDescripcion;
				LET cSubActividad = cDescripcion14||'|'||cSubActividad;
					
			END FOREACH;
			
			FOREACH
							
				SELECT elemento,descripcion
				INTO iElemento,cDescripcion
				FROM "informix".si_scoring_movil 
				WHERE grupo=38 AND seccion=2 AND activa=1 ORDER BY elemento DESC
				
				LET cDescripcion15 = iElemento||';'||cDescripcion;
				LET cCompIng=cDescripcion15||'|'||cCompIng;
				
			END FOREACH;
			
			FOREACH
				
				SELECT elemento,descripcion 
				INTO iElemento,cDescripcion
				FROM "informix".si_scoring_movil 
				WHERE grupo=21 AND seccion=2 AND activa=1 ORDER BY elemento DESC
				
				LET cDescripcion16=iElemento||';'||cDescripcion;
				LET cEscolaridad=cDescripcion16||'|'||cEscolaridad;

			END FOREACH;
			
            FOREACH
				
				SELECT elemento,descripcion 
				INTO iElemento,cDescripcion
				FROM "informix".si_escolaridad_am 
				ORDER BY elemento DESC				
				
				LET cDescripcion16=iElemento||';'||cDescripcion;
				LET cEscolaridad=cDescripcion16||'|'||cEscolaridad;

			END FOREACH;

			FOREACH
			
				SELECT {+INDEX (bdinteg:si_carriers idx_carriers_movil)} cve_carrier,nombre_carrier
				INTO cClave,cDescripcion
				FROM "informix".si_carriers ORDER BY cve_carrier DESC
				
				LET cDescripcion18 = cClave||';'||cDescripcion;
				LET cCarriers = cDescripcion18||'|'||cCarriers;
				
			END FOREACH;
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor2_movil)} valor
			INTO cValor
			FROM "informix".si_param_movil
			WHERE cod_param = 9;
			
			LET cDescripcion19=cValor;
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor2_movil)} valor
			INTO cValor
			FROM "informix".si_param_movil
			WHERE cod_param = 10;
			
			LET cDescripcion20=cValor;
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor2_movil)} valor
			INTO cValor2
			FROM "informix".si_param_movil
			WHERE cod_param = 11;

			LET cDescripcion21=cValor2;

			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor2_movil)} valor
			INTO cValor3
			FROM "informix".si_param_movil
			WHERE cod_param = 12;

			LET cVersion=cValor3;
			
			SELECT {+INDEX (bdinteg:si_param_movil idx_valor2_movil)} valor
			INTO cValor
			FROM "informix".si_param_movil
			WHERE cod_param = 13;
			
			LET cDescripcion22=cValor;
			
			
			--CONCATENA LOS RESULATDOS DE LOS SCRIPTS PARA RETORNAR
			LET cParamCat = cDescripcion1||'|'||cDescripcion2||'|'||cDescripcion3||'|'||cDescripciono||'|'||cDescripcion4||'|'||cDescripcion5||'|'||cDescripcion6||'|'||cDescripcion7||'|'||cDescripcion8||'|'||cCteCoppel||'|'||cDomActCte||'|'||cEdoCivil||cDescHabEN||
			cActividad||cSubActividad||'|'||cCompIng||cEscolaridad||cSexo||'|'||cCarriers||cDescripcion19||'|'||cDescripcion20||'|'||cDescripcion21||'|'||cVersion||'|'||cDescripcion22||'|'||cLimiteColonias||'|'||cLimiteLike||'|'||cLimiteTiempo;

	RETURN cCodret,cParamCat;
END
END PROCEDURE
DOCUMENT
'AUTOR: 95689966, PEDRO JIMENEZ GUZMAN',
'FOLIO: 1484',
'DESCRIPCION: VALIDA SI EXISTE EL USUARIO EN LA TABLA SI_USUARIO_MOVIL VALIDANDO LOS PARAMETROS DE ENTRADA',
'FECHA: 10/02/2015',
'VERSION: 10022015.1144',
'SUSTENTO: SE DEFINIO CON JAIME GONZALEZ Y VICTOR HUGO SANCHEZ MENDOZA EN EL REQUERIMIENTO',
'RQI 61 116? Alta movil',
'20190212-EMM',
'SE AGREGAN LOS PARAMETROS CODIGO POSTAL, COLONIA Y TIEMPO LIMITE',
'FECHA: 21/07/2022',
'MODIFICADO: SE AGREGA VALIDACION MEDIANTE LA LONGITUD DEL PARAMETRO PIMEI, PARA IDENTIFICAR EL ID A VALIDAR (IMEI O DEVICE ID)',
'BD: BDINTEG';

CREATE PROCEDURE "informix".sp_cargarcatalogosepomex()
        RETURNING CHAR(5), CHAR(80);

--Elaboró: Bernardo Báez
--Fecha: 10-12-2009
--Se creo un proceso que permitirá la carga de archivos a la base de datos bdinteg:si_catSEPOMEX
--Este proceso registrará su actividad en la bitacora si_bitacora_dom
--la ruta y nombre de archivo se tomaran de la tabla si_param_dom para los codigos 6 y 7
--la fecha del proceso 
--se eliminaran los archivos anteriores de la ruta

--Modifico: Jesus Adilene Lara Armenta
--Fecha: 15-02-2010
--Se modifico para que al momento de cargar el archivo txt con los domicilios actuales, 
--si el registro tiene un valor NULL en el campo d_ciudad se inserte el valor contenido en el campo d_mnpio y
--se valide que el campo d_codigo tenga una longitud de 5 digitos y se agrega un 0 a la izquierda en los casos donde solo es de 4

--Modifico: Mohamed Carreón
--Fecha: 06-04-2010
--Se modifico conveniente al folio cmmi 1148 de SEPOMEX, 
--1.- Quitar el borrado de la tabla de catalogo de sepomex.
--2.- Insertar solo los registros nuevos.
--3.- Actualizar solo los codigos postales que cambiaron.

-- Modificó: Adrian Lara
-- Fecha: 21/06/2010
-- Se modifica la forma de reemplazar las minusculas por mayusculas, y la letra ñ para que se realice desde la consola

--Modificó: MACF
--Fecha: 20111124 - 20100803

DEFINE vCodRet                  CHAR(5);
DEFINE vMensaje                 CHAR(80);
--DEFINE vMensaje                 CHAR(200);  -- solo test MACF
DEFINE SQL_ERR                  INTEGER;
DEFINE ISAM_ERR                 INTEGER;
DEFINE ERROR_INFO               VARCHAR(80);
DEFINE cNombreProceso           CHAR(30);
DEFINE iRegistros               INTEGER;
DEFINE cCadena                  CHAR(2000);
DEFINE cEmpresa                 CHAR(3);
DEFINE iParamRuta               INTEGER;
DEFINE iParamNombre             INTEGER;
DEFINE cRuta                    CHAR(100);
DEFINE cNombre                  CHAR(100);
DEFINE cNombre2                 CHAR(100);
DEFINE cNombreBase              CHAR(100);
DEFINE cNombreBase2              CHAR(100);
DEFINE v_fecha                  DATE;
DEFINE v_dia                    CHAR(2);
DEFINE v_mes                    CHAR(2);
DEFINE v_anio                   CHAR(4);
DEFINE v_d_codigo           CHAR(5);
DEFINE v_d_asenta			CHAR(60);
DEFINE v_d_tipo_asenta		CHAR(25);		
DEFINE v_d_mnpio			CHAR(40);	
DEFINE v_d_estado			CHAR(60);
DEFINE v_d_ciudad			CHAR(100);
DEFINE v_c_estado			CHAR(2);
DEFINE v_paso               INTEGER;
DEFINE v_Status				INTEGER;
DEFINE v_Band1				CHAR(1);

DEFINE s_d_asenta			CHAR(60);
DEFINE s_d_tipo_asenta		CHAR(25);
DEFINE s_d_mnpio			CHAR(40);
DEFINE s_d_estado			CHAR(60);
DEFINE s_d_ciudad			CHAR(100);
DEFINE i_d_cp				INTEGER;
DEFINE s_c_estado			INTEGER;
DEFINE s_d_codigo           CHAR(5);
DEFINE iNumDuplic           INTEGER;
DEFINE s_d_tipo_asenta_unico CHAR(25);
DEFINE i_estado             INTEGER;

DEFINE v_d_asenta_2			CHAR(60);
DEFINE v_d_ciudad_2			CHAR(100);
DEFINE v_c_estado_2			CHAR(2);
DEFINE v_d_codigo_2         CHAR(5);
DEFINE v_d_codigo_1         CHAR(5);
DEFINE s_d_mnpio_2			CHAR(40);
DEFINE v_fecha_baja         DATE;

LET cNombreProceso  = 'CARGA DE CATALOGOS SEPOMEX';
LET vCodRet         = '11111';
LET vMensaje        = 'PROCESO INICIALIZADO';
LET iRegistros      = 0;
LET cCadena         = '';
LET cEmpresa        = '001';
LET iParamRuta      = 6;
LET iParamNombre    = 7;
LET cRuta           = '';
LET cNombre         = '';
LET v_fecha         = DATE(1);
LET v_Status		= 0;
LET v_Band1			= "0";

LET s_d_asenta			= "";
LET s_d_tipo_asenta		= "";
LET s_d_mnpio			= "";
LET s_d_estado			= "";
LET s_d_ciudad			= "";
LET i_d_cp				= 0;
LET s_c_estado			= 0;
LET s_d_codigo          = "";
LET iNumDuplic          = 0;
LET s_d_tipo_asenta_unico = "";
LET i_estado              = 0;
LET v_d_asenta_2		 = '';
LET v_d_ciudad_2		 = '';
LET v_c_estado_2		 = '';
LET v_d_codigo_2         = '';
LET s_d_mnpio_2			 = '';
LET v_d_codigo_1         = '';
LET v_c_estado			 = '';
LET v_d_estado           = '';
LET v_d_ciudad           = '';
LET v_d_mnpio            = '';
LET v_d_tipo_asenta      = '';
LET v_d_asenta           = '';
LET v_d_codigo           = '';
LET v_fecha_baja         = date(1);

  --SET DEBUG FILE TO "/ifxsif01/macf/sp_CargarCatalogoSEPOMEX.out";    
  --TRACE ON; 

SET ISOLATION TO DIRTY READ;
SET LOCK MODE TO WAIT 3;
---SET pdqpriority 20;

BEGIN

    ON EXCEPTION SET SQL_ERR, ISAM_ERR, ERROR_INFO
        LET vCodRet  = SQL_ERR;
        LET vMensaje  = ERROR_INFO;

        IF vCodRet = '-668' and v_paso = 1 then
            LET vCodRet = '00003';
            LET vMensaje = 'NO EXISTE LA RUTA QUE SE INDICA EN LOS PARAMETROS';
        ELIF vCodRet = '-668' and v_paso = 6 then
            LET vCodRet = '00004';
            LET vMensaje = 'LA ESTRUCTURA DE LOS DATOS DEL ARCHIVO ES INCORRECTA';
        END IF; 

		--LET vMensaje = trim(vMensaje) || 'Paso: ' || v_paso || ' ' || trim(v_d_codigo) || ' ' || trim(v_d_asenta) || ' ' || trim(v_d_mnpio) || ' ' || trim(v_d_ciudad) || ' ' || v_c_estado;  -- Solo test MACF
		LET vMensaje = trim(vMensaje) || 'Paso: ' || v_paso;
		
        --insertar control de procesos
        INSERT INTO bdinteg:si_bitacora_dom (proceso, cod_ret, mensaje, reg_insert, user_insert, fecha_insert, hora_insert) 
        VALUES(cNombreProceso, vCodRet, vMensaje, iRegistros ,user, v_fecha,
        (SELECT DBINFO('utc_to_datetime', sh_curtime)::DATETIME HOUR TO SECOND from sysmaster:sysshmvals));

        RETURN vCodRet, vMensaje;
    END EXCEPTION;

UPDATE STATISTICS MEDIUM FOR TABLE bdinteg:si_catsepomex;

SELECT fecha_hoy 
into v_fecha
from bdinteg:si_fechas
where empresa = cEmpresa;

 /*LET v_fecha = mdy('08','12','2022');  -- SOLO TEST MACF*/

INSERT INTO bdinteg:si_bitacora_dom (proceso, cod_ret, mensaje, reg_insert, user_insert, fecha_insert, hora_insert) 
        VALUES(cNombreProceso, vCodRet, vMensaje, iRegistros ,user, v_fecha,
        (SELECT DBINFO('utc_to_datetime', sh_curtime)::DATETIME HOUR TO SECOND from sysmaster:sysshmvals));

IF EXISTS(SELECT tabname FROM sysmaster:systabnames WHERE tabname = 'tmp_valida_catsepomex'  AND dbsname = 'bdinteg') THEN
        DROP TABLE tmp_valida_catsepomex;
END IF;

IF EXISTS(SELECT tabname FROM sysmaster:systabnames WHERE tabname = 'tmp_si_catsepomex'  AND dbsname = 'bdinteg') THEN
         DROP TABLE tmp_si_catsepomex;
END IF;

IF EXISTS(SELECT tabname FROM sysmaster:systabnames WHERE tabname = 'tmp_si_catsepomex_duplicados'  AND dbsname = 'bdinteg') THEN
          DROP TABLE tmp_si_catsepomex_duplicados;
END IF;


IF EXISTS(SELECT tabname FROM sysmaster:systabnames WHERE tabname = 'tmp_si_catsepomex_dup_07'  AND dbsname = 'bdinteg') THEN
          DROP TABLE tmp_si_catsepomex_dup_07;
END IF;

IF EXISTS(SELECT tabname FROM sysmaster:systabnames WHERE tabname = 'tmp_si_catsepomex_dup_04'  AND dbsname = 'bdinteg') THEN
          DROP TABLE tmp_si_catsepomex_dup_04;
END IF;

IF EXISTS(SELECT tabname FROM sysmaster:systabnames WHERE tabname = 'tmp_si_catsepomex_notexist'  AND dbsname = 'bdinteg') THEN
          DROP TABLE tmp_si_catsepomex_notexist;
END IF;




CREATE TABLE tmp_si_catsepomex
(
	d_codigo            CHAR(5),
	d_asenta            CHAR(60),
	d_tipo_asenta       CHAR(25),
	d_mnpio             CHAR(40),
	d_estado            CHAR(60),
	d_ciudad            CHAR(100),
	d_cp                INTEGER,
	c_estado            INTEGER
);

CREATE TABLE tmp_valida_catsepomex
(
	valor CHAR(200)
);

CREATE TABLE tmp_si_catsepomex_duplicados
(
	d_asenta            CHAR(60),
	d_tipo_asenta       CHAR(25),
	d_mnpio             CHAR(40),
	d_estado            CHAR(60),
	d_ciudad            CHAR(100),
	d_cp                INTEGER,
	c_estado            INTEGER
);


CREATE INDEX tmpSiCatsepomex ON tmp_si_catsepomex
(d_codigo) USING btree ;


CREATE INDEX tmpSiCatsepomexDup ON tmp_si_catsepomex_duplicados
(d_cp) USING btree ;















CREATE TABLE tmp_si_catsepomex_dup_07
(
	d_codigo            CHAR(5),
	d_asenta            CHAR(60),
	--d_tipo_asenta       CHAR(25),  -- habilitar MACF 2022-07-18
	d_mnpio             CHAR(40),
	d_estado            CHAR(60),
	d_ciudad            CHAR(100),
	c_estado            INTEGER,
	cantidad            integer
);


CREATE INDEX tmpSiCatsepomex_dup_07 ON tmp_si_catsepomex_dup_07
(d_codigo) USING btree ;
----- Tabla temp para quitar los duplicados MACF

CREATE TABLE tmp_si_catsepomex_dup_04
(
	d_asenta            CHAR(60),
	d_mnpio             CHAR(40),
	d_ciudad            CHAR(100),
	c_estado            INTEGER,
	cantidad            integer
);

CREATE INDEX tmpSiCatsepomex_dup_04 ON tmp_si_catsepomex_dup_04
(d_asenta,d_mnpio,d_ciudad,c_estado) USING btree ;










 
/*IF day(v_fecha) < 10 then
	LET v_dia = '0' || day(v_fecha);
ELSE
	LET v_dia = day(v_fecha);
END IF;

IF month(v_fecha) < 10 then
	LET v_mes = '0' || month(v_fecha);
ELSE
	LET v_mes = month(v_fecha);
END IF;

LET v_anio = year(v_fecha);*/

SELECT valor 
INTO cRuta
FROM bdinteg:si_param_dom 
where empresa = cEmpresa 
	and cod_param = iParamRuta;

SELECT valor 
INTO cNombre
FROM bdinteg:si_param_dom 
where empresa = cEmpresa 
	and cod_param = iParamNombre;

IF (cRuta is not null) and (cNombre is not null) and (cRuta <> '') and (cNombre <> '') then

    --LET cNombreBase = trim(cNombre) || '*.txt';
    LET cNombreBase = SUBSTR(cNombre,1,LENGTH(cNombre)) || '*.txt';
    --LET cNombreBase2 = trim(cNombre) || 'BASE2.txt';
    LET cNombreBase2 = SUBSTR(cNombre,1,LENGTH(cNombre)) || 'BASE2.txt';

	--LET cNombre2 = trim(cNombre) || v_dia || v_mes || v_anio || '_res.txt';
    --LET cNombre2 = SUBSTR(cNombre,1,LENGTH(cNombre)) || v_dia || v_mes || v_anio || '_res.txt';  -- 20210423 MACF
    --LET cNombre = trim(cNombre) || v_dia || v_mes || v_anio || '.txt';
    --LET cNombre = SUBSTR(cNombre,1,LENGTH(cNombre)) || v_dia || v_mes || v_anio || '.txt';  -- 20210423 MACF

	LET cNombre2 = SUBSTR(cNombre,1,LENGTH(cNombre)) || LPAD(day(v_fecha),2,0) ||  LPAD(month(v_fecha),2,0) || year(v_fecha) || '_res.txt'; -- 20210423 MACF
	LET cNombre  = SUBSTR(cNombre,1,LENGTH(cNombre)) || LPAD(day(v_fecha),2,0) ||  LPAD(month(v_fecha),2,0) || year(v_fecha) || '.txt';     -- 20210423 MACF
	
    LET v_paso = 1;
    --let cCadena = 'touch ' || trim(cRuta) || trim(cNombreBase2);
    let cCadena = 'touch ' || SUBSTR(cRuta,1,LENGTH(cRuta)) || SUBSTR(cNombreBase2,1,LENGTH(cNombreBase2)); 
    System SUBSTR(cCadena,1,LENGTH(cCadena));

    LET v_paso = 2;
    --let cCadena = 'ls ' || trim(cRuta) || trim(cNombreBase) || ' > /tmp/validaSEPOMEX.txt';
    let cCadena = 'ls ' || SUBSTR(cRuta,1,LENGTH(cRuta)) || SUBSTR(cNombreBase,1,LENGTH(cNombreBase)) || ' > /tmp/validaSEPOMEX.txt';
    System SUBSTR(cCadena,1,LENGTH(cCadena));  
	
    LET v_paso = 3;
    LET cCadena = 'echo "load from ''' || '/tmp/validaSEPOMEX.txt''' ||
    ' insert into bdinteg:tmp_valida_catsepomex" > /tmp/importa_si_catsepomex.sql';
    System SUBSTR(cCadena,1,LENGTH(cCadena)); 
	
    LET v_paso = 4;
    let cCadena = 'dbaccess bdinteg /tmp/importa_si_catsepomex.sql';
    System SUBSTR(cCadena,1,LENGTH(cCadena));

    IF EXISTS (SELECT VALOR FROM tmp_valida_catsepomex where valor = trim(cRuta) || trim(cNombre)) THEN
		--CONVIERTE EL ARCHIVO DE MINUSCULAS A MAYUSCULAS Y LO GUARDA EN UN  ARCHIVO NUEVO.
		LET v_paso = 41;
		--LET cCadena = '/usr/bin/cat ' || TRIM(cRuta) || TRIM(cNombre) || "|" || " tr '''[:lower:]''' '''[:upper:]''' > /tmp/" || TRIM(cNombre) ||'';
        LET cCadena = '/usr/bin/cat ' || SUBSTR(cRuta,1,LENGTH(cRuta)) || SUBSTR(cNombre,1,LENGTH(cNombre)) || "|" || " tr '''[:lower:]''' '''[:upper:]''' > /tmp/" || SUBSTR(cNombre,1,LENGTH(cNombre)) ||'';
		System SUBSTR(cCadena,1,LENGTH(cCadena));
		
		--QUITA LOS ACENTOS DE LAS LETRAS DEL NUEVO ARCHIVO Y LO GUARDA EN OTRO.
		LET v_paso = 42;
		--LET cCadena = '/usr/bin/cat /tmp/' || TRIM(cNombre) || 
		--"|" || " tr '''[Á]''' '''[A]''' " || "|" || " tr '''[É]''' '''[E]''' " || "|" || " tr '''[Í]''' '''[I]''' " || 
		--"|" || " tr '''[Ó]''' '''[O]''' " || "|" || " tr '''[Ú]''' '''[U]''' " || "|" || " tr '''[ñ]''' '''[Ñ]''' > /tmp/" || TRIM(cNombre2) ||'';
        LET cCadena = '/usr/bin/cat /tmp/' || SUBSTR(cNombre,1,LENGTH(cNombre)) || 
		"|" || " tr '''[Á]''' '''[A]''' " || "|" || " tr '''[É]''' '''[E]''' " || "|" || " tr '''[Í]''' '''[I]''' " || 
		"|" || " tr '''[Ó]''' '''[O]''' " || "|" || " tr '''[Ú]''' '''[U]''' " || "|" || " tr '''[ñ]''' '''[Ñ]''' " || "|" || " tr '''[Ü]''' '''[U]''' > /tmp/" || SUBSTR(cNombre2,1,LENGTH(cNombre2)) ||'';  --Agregar la Ü
		--"|" || " tr '''[Ó]''' '''[O]''' " || "|" || " tr '''[Ú]''' '''[U]''' " || "|" || " tr '''[ñ]''' '''[Ñ]''' > /tmp/" || SUBSTR(cNombre2,1,LENGTH(cNombre2)) ||'';
		System SUBSTR(cCadena,1,LENGTH(cCadena));
		
        LET v_paso = 5;
        
		--LET cCadena = 'echo "load from ''' || TRIM(cRuta) || TRIM(cNombre) || 
		--LET cCadena = 'echo "load from ''' || "/tmp/" || TRIM(cNombre2) || 
        --''' delimiter ''	'' insert into bdinteg:tmp_si_catsepomex" > /tmp/importa_si_catsepomex.sql';
        LET cCadena = 'echo "load from ''' || "/tmp/" || SUBSTR(cNombre2,1,LENGTH(cNombre2)) || 
        ''' delimiter ''	'' insert into bdinteg:tmp_si_catsepomex" > /tmp/importa_si_catsepomex.sql';
        System SUBSTR(cCadena,1,LENGTH(cCadena));
		
        LET v_paso = 6;
        let cCadena = 'dbaccess bdinteg /tmp/importa_si_catsepomex.sql';
        System SUBSTR(cCadena,1,LENGTH(cCadena));
        LET v_paso = 7;
        let cCadena = 'rm /tmp/importa_si_catsepomex.sql';
        System SUBSTR(cCadena,1,LENGTH(cCadena));
        LET v_paso = 8;
	
		--- /// Mohamed Carreón  >>>  Modificación 1.
        --- DELETE FROM bdinteg:si_catsepomex;
			
		--- LE PONE EL CERO A LA IZQUIERDA AL CODIGO POSTAL Y ACTUALIZA LA CIUDAD CON EL VALOR DEL MUNICIPIO EN CASO DE QUE NO LA TRAIGA
		UPDATE bdinteg:tmp_si_catsepomex
		SET d_codigo = LPAD(TRIM(d_codigo),5,"0"),
			d_ciudad = DECODE(d_ciudad,NULL,d_mnpio,d_ciudad);
		
		LET v_d_codigo = ""; LET s_d_asenta = "";  LET s_d_tipo_asenta = ""; LET s_d_mnpio = "";  LET s_d_estado = "";  LET s_d_ciudad = "";
		LET i_d_cp = 0 ;     LET s_c_estado = 0;   LET iNumDuplic = 0;
		
		
        LET v_paso = 81;		
		-- 2o BUSCAR LOS DUPLICADOS CONSIDERANDO LOS 7 CAMPOS (d_codigo, d_asenta, d_mnpio, d_estado, d_ciudad, d_cp, c_estado, cantidad)
		INSERT INTO tmp_si_catsepomex_dup_07
		SELECT d_codigo, d_asenta, d_mnpio, d_estado, d_ciudad, LPAD(c_estado,2,'0') c_estado, count(*) cantidad
          FROM bdinteg:tmp_si_catsepomex
         GROUP BY c_estado, d_estado, d_mnpio, d_ciudad, d_asenta, d_codigo;
        --HAVING COUNT(*)> 1;
		
		/*LET v_paso = 82;
		FOREACH WITH HOLD
			SELECT d_codigo, d_asenta, d_mnpio, d_ciudad, c_estado, d_estado, cantidad
			INTO v_d_codigo, s_d_asenta, s_d_mnpio, s_d_ciudad, v_c_estado, s_d_estado, iNumDuplic
			FROM tmp_si_catsepomex_dup_07
		    ORDER BY c_estado, d_mnpio, d_ciudad, d_asenta, d_codigo
		

				IF iNumDuplic = 2 THEN
				   SELECT limit 1 d_tipo_asenta INTO s_d_tipo_asenta_unico
			         FROM bdinteg:tmp_si_catsepomex 
			        WHERE d_codigo = v_d_codigo AND d_asenta = s_d_asenta AND d_mnpio = s_d_mnpio
		              AND d_ciudad = s_d_ciudad and c_estado = v_c_estado;
					  
					begin;
					   DELETE bdinteg:tmp_si_catsepomex WHERE d_codigo = v_d_codigo and d_asenta = s_d_asenta and d_mnpio = s_d_mnpio
					   and d_ciudad = s_d_ciudad and c_estado = v_c_estado;
					commit;
					
					begin;
					   INSERT INTO bdinteg:tmp_si_catsepomex(d_codigo, d_asenta, d_tipo_asenta, d_mnpio, d_estado, d_ciudad, d_cp, c_estado)
					   VALUES(v_d_codigo, s_d_asenta, s_d_tipo_asenta_unico, s_d_mnpio, s_d_estado, s_d_ciudad, 0, v_c_estado);
					commit;
			    ELIF iNumDuplic > 2 THEN
				   
					FOREACH WITH HOLD
					   SELECT d_codigo, d_asenta, d_ciudad, c_estado, d_mnpio
						 INTO v_d_codigo_2, v_d_asenta_2, v_d_ciudad_2, v_c_estado_2, s_d_mnpio_2
						 FROM bdinteg:tmp_si_catsepomex
						WHERE d_codigo = v_d_codigo 
						  AND d_asenta = s_d_asenta
						  AND d_ciudad = s_d_ciudad
						  AND c_estado = v_c_estado
						  AND d_mnpio = s_d_mnpio 
						  
						  IF s_d_asenta = v_d_asenta_2 AND s_d_ciudad = v_d_ciudad_2 AND v_c_estado = v_c_estado_2 AND  v_d_codigo = v_d_codigo_2 and s_d_mnpio = s_d_mnpio_2 THEN
							 begin;
							   DELETE bdinteg:tmp_si_catsepomex WHERE d_codigo = v_d_codigo and d_asenta = s_d_asenta and d_mnpio = s_d_mnpio
							 and d_ciudad = s_d_ciudad and  c_estado = v_c_estado;
							 commit;
						  END IF;	  
						  
						  
						LET v_d_asenta_2 = ''; 
						LET v_d_ciudad_2 = '';
						LET v_c_estado_2 = '';
						LET v_d_codigo_2 = '';
						LET s_d_mnpio_2  = '';
			        END FOREACH;
				
				END IF;
			
		
		END FOREACH;
		*/
		
		---- SE ELIMNA ESTE BLOQUE DE DEPURADO DE CAMPOS REPETIDOS 2022-06-29
		/*LET v_paso = 83;
		  {3o BUSCAR LOS REPETIDOS CONSIDERANDO LOS 4 CAMPOS}
		  {Hacer rutina de encontrar las otras colonias duplicadas. Campos: d_asenta, d_mnpio, c_estado, d_ciudad (solo con esos)}
		truncate tmp_si_catsepomex_dup_07;
		
	    LET v_paso = 84;
		INSERT INTO tmp_si_catsepomex_dup_04
		SELECT d_asenta, d_mnpio, d_ciudad, c_estado, count(*) cantidad
          FROM bdinteg:tmp_si_catsepomex
         GROUP BY 1,2,3,4 
        HAVING COUNT(*)> 1;
		
		LET v_paso = 85;
		FOREACH WITH HOLD
		    SELECT d_asenta, d_mnpio, d_ciudad, c_estado, cantidad
			INTO s_d_asenta, s_d_mnpio, s_d_ciudad, s_c_estado, iNumDuplic
			FROM tmp_si_catsepomex_dup_04
 
			SELECT limit 1 d_codigo, d_tipo_asenta, d_estado INTO v_d_codigo, s_d_tipo_asenta_unico, s_d_estado
			  FROM bdinteg:tmp_si_catsepomex 
			 WHERE d_asenta = s_d_asenta AND d_mnpio = s_d_mnpio
		       AND d_ciudad = s_d_ciudad and c_estado = s_c_estado;
		
		    IF iNumDuplic > 1 THEN
				begin;
				   DELETE bdinteg:tmp_si_catsepomex WHERE d_asenta = s_d_asenta AND d_mnpio = s_d_mnpio
				   AND d_ciudad = s_d_ciudad AND c_estado = s_c_estado;
				commit;
				
				begin;
				   INSERT INTO bdinteg:tmp_si_catsepomex(d_codigo, d_asenta, d_tipo_asenta, d_mnpio, d_estado, d_ciudad, d_cp, c_estado)
				   VALUES(v_d_codigo, s_d_asenta, s_d_tipo_asenta_unico, s_d_mnpio, s_d_estado, s_d_ciudad, 0, s_c_estado);
				commit;
			
            END IF;		
		    
			LET s_d_tipo_asenta_unico = "";
		
		END FOREACH
		*/
		
		/*
		--- /// Mohamed Carreón  >>>  SE AGREGO VALIDACION PARA BORRAR LOS REGISTROS DUPLICADOS Y DEJAR LA PRIMER COLONIA
		--- PONE LOS REGISTROS DUPLICADOS EN LA TABLA TEMPORAL
		INSERT INTO tmp_si_catsepomex_duplicados
		SELECT d_asenta,d_tipo_asenta,d_mnpio,d_estado,d_ciudad,d_cp,c_estado  
		FROM bdinteg:tmp_si_catsepomex 
		GROUP BY d_asenta,d_tipo_asenta,d_mnpio,d_estado,d_ciudad,d_cp,c_estado
		HAVING COUNT(d_asenta) > 1;
		
		--- CICLO PARA BARRER LOS REGSITROS QUE SE ENCONTRARON DUPLICADOS
		FOREACH
			SELECT d_asenta, d_tipo_asenta, d_mnpio, d_estado, d_ciudad, d_cp, c_estado
			INTO s_d_asenta, s_d_tipo_asenta, s_d_mnpio, s_d_estado, s_d_ciudad, i_d_cp, s_c_estado
			FROM tmp_si_catsepomex_duplicados
			
			SELECT MIN(d_codigo) 
			INTO s_d_codigo
			FROM tmp_si_catsepomex 
			WHERE d_asenta = s_d_asenta AND d_tipo_asenta = s_d_tipo_asenta AND d_mnpio = s_d_mnpio 
			AND d_estado = s_d_estado AND d_ciudad = s_d_ciudad AND d_cp = i_d_cp AND c_estado = s_c_estado;
			
			--- BORRAR LOS REGISTROS DUPLICADOS Y DEJA EL QUE TIENE EL CODIGO POSTAL MAS NUEVO
			DELETE bdinteg:tmp_si_catsepomex 
			WHERE d_asenta = s_d_asenta AND d_tipo_asenta = s_d_tipo_asenta AND d_mnpio = s_d_mnpio 
			AND d_estado = s_d_estado AND d_ciudad = s_d_ciudad AND d_cp = i_d_cp AND c_estado = s_c_estado
			AND d_codigo <> s_d_codigo;
		END FOREACH
		*/ 
		LET v_paso = 86;
		--- VALIDA SI LA TABLA DE CATALOGO DE SEPOMEX ESTA VACIA
		IF (SELECT COUNT(d_codigo) FROM bdinteg:si_catsepomex) = 0 THEN
			LET v_Band1 = "1";
		END IF
		
		LET v_paso = 87;
		LET v_d_codigo_2 = '';
		LET v_d_codigo_1 = '';
		
        FOREACH
		SELECT TRIM(d_codigo), TRIM(d_asenta), TRIM(d_mnpio), TRIM(d_ciudad), LPAD(c_estado,2,'0'), TRIM(d_estado)
		  into v_d_codigo, v_d_asenta, v_d_mnpio, v_d_ciudad, v_c_estado, v_d_estado
        --FROM bdinteg:tmp_si_catsepomex
		 FROM tmp_si_catsepomex_dup_07

			
			--- VALIDA BANDERA DE EXISTENCIA DE DATOS DE LA TABLA DE CATALOGO DE SEPOMEX
			IF v_Band1 = "1" THEN
				LET v_Status = 2;
			ELSE
			

					--- VALIDA QUE EXISTA EN EL CATALOGO DE SEPOMEX (con CP y todos sus demás datos)





					 SELECT d_codigo INTO v_d_codigo_1		
					   FROM bdinteg:si_catsepomex WHERE d_codigo = v_d_codigo
					   AND d_asenta = v_d_asenta AND d_mnpio = v_d_mnpio
					   AND c_estado = v_c_estado AND d_ciudad = v_d_ciudad AND nvl(fecha_baja,'') = ''; -- pend validar si se agrega la fecha_baja
						
						LET v_Status = 1;
						
						IF NVL(v_d_codigo_1,'') <> '' THEN
							UPDATE bdinteg:si_catsepomex
							SET estatus = v_Status --, fecha_ejecucion = v_fecha (ya no actualizar la fecha_ejecucion)
							WHERE d_codigo = v_d_codigo        
							AND d_asenta = v_d_asenta --AND d_tipo_asenta = v_d_tipo_asenta   -- 20210609 Ya no considerar el d_tipo_asenta
							AND c_estado = v_c_estado AND d_ciudad = v_d_ciudad AND d_mnpio = v_d_mnpio;


							LET v_d_codigo_1 = '';
							
                            CONTINUE FOREACH;
							
						ELSE

						    SELECT d_codigo, fecha_baja into v_d_codigo_2, v_fecha_baja
						      FROM bdinteg:si_catsepomex 
						     WHERE d_codigo = v_d_codigo 
							   AND d_asenta = v_d_asenta 
						       AND c_estado = v_c_estado AND d_ciudad = v_d_ciudad AND d_mnpio = v_d_mnpio 
						       AND nvl(fecha_baja,'') <> ''; 

						       LET v_Status = 5;	--- Se identificará con este estatus para que el sp_conciliarcolonias_sepomex la tome como una "actualización" y la reactive
                                                    --- ya que existira en si_catzonas con los campos nomzona_spmx, pobzona_spmx y mnpio_spmx vacías
						
							IF NVL(v_d_codigo_2,'') <> '' THEN
						
									UPDATE bdinteg:si_catsepomex
									SET estatus = v_Status, fecha_alta = v_fecha, fecha_baja = null
									WHERE d_codigo = v_d_codigo 
									AND d_asenta = v_d_asenta 
									AND c_estado = v_c_estado AND d_ciudad = v_d_ciudad AND d_mnpio = v_d_mnpio;
								
									LET v_d_codigo_2 = '';
								
									LET iRegistros = DBINFO("sqlca.sqlerrd2");
									
									IF iRegistros > 0 THEN 
									   
									   INSERT INTO bdinteg:si_catsepomex_his(d_codigo, d_asenta,d_mnpio, d_estado, d_ciudad, c_estado, estatus, fecha_baja, fecha_reactiva)
									   VALUES(v_d_codigo,v_d_asenta,v_d_mnpio,v_d_estado,v_d_ciudad,v_c_estado,v_Status,v_fecha_baja,v_fecha); 
										
									END IF;
									
									CONTINUE FOREACH;
							
						     ELSE 
							    /*
							    SELECT d_codigo, fecha_baja into v_d_codigo_3, v_fecha_baja
						          FROM bdinteg:si_catsepomex 
						         WHERE d_codigo = v_d_codigo  AND d_asenta = v_d_asenta AND d_mnpio = v_d_mnpio
						           AND c_estado = v_c_estado AND d_ciudad = v_d_ciudad 
						           AND nvl(fecha_baja,'') <> '';
							 
							 
							    IF NVL(v_d_codigo_3,'') <> '' AND NVL(v_fecha_baja,'') <> '' THEN
							 
							       INSERT INTO bdinteg:si_casepomex_his(d_codigo, d_asenta, d_tipo_asenta, d_mnpio, d_estado, d_ciudad, c_estado, estatus, fecha_ejecucion, fecha_baja)
								     VALUES(v_d_codigo_3, v_d_asenta, v_d_tipo_asenta, v_d_mnpio, v_d_estado, v_d_ciudad, v_c_estado, v_Status, v_fecha, v_fecha);

                                END IF;*/
							 
                                 LET v_Status = 2;
								 
						 
						     END IF;
						END IF;
						
					    --ELSE
						--- CUANDO CAMBIA EL CODIGO POSTAL (si es el mismo asentamiento)
						/*IF EXISTS(SELECT d_codigo FROM bdinteg:si_catsepomex 
								   WHERE d_asenta = v_d_asenta AND d_tipo_asenta = v_d_tipo_asenta   -- 20210609 Ya no considerar el d_tipo_asenta
								   AND d_estado = v_d_estado AND d_ciudad = v_d_ciudad 
								) THEN*/

							--LET v_Status = 3;
							
						

				--ELSE
							--- CUANDO ES UN CODIGO POSTAL NUEVO
							--LET v_Status = 2;
						---END IF				
					--END IF	
					
					
				/*ELSE
			    	
					--- VALIDA QUE EXISTA EN EL CATALOGO DE SEPOMEX (2) EDO =9
					IF EXISTS (SELECT d_codigo FROM bdinteg:si_catsepomex WHERE d_codigo = v_d_codigo 
							--AND d_asenta = v_d_asenta AND d_tipo_asenta = v_d_tipo_asenta   -- 20210609 Ya no considerar el d_tipo_asenta
							AND d_asenta = v_d_asenta 
							AND d_estado = v_d_estado AND d_ciudad = v_d_ciudad AND d_mnpio = v_d_mnpio
							) THEN
						LET v_Status = 1;
						
						UPDATE bdinteg:si_catsepomex
						SET estatus = v_Status , fecha_ejecucion = v_fecha
						WHERE d_codigo = v_d_codigo        
						AND d_asenta = v_d_asenta --AND d_tipo_asenta = v_d_tipo_asenta -- 20210609 Ya no considerar el d_tipo_asenta
						AND d_estado = v_d_estado AND d_ciudad = v_d_ciudad AND d_mnpio = v_d_mnpio;
						
						CONTINUE FOREACH;
					ELSE
						--- CUANDO CAMBIA EL CODIGO POSTAL (2)
						IF EXISTS(SELECT d_codigo FROM bdinteg:si_catsepomex 
								--WHERE d_asenta = v_d_asenta AND d_tipo_asenta = v_d_tipo_asenta  -- 20210609 Ya no considerar el d_tipo_asenta
								WHERE d_asenta = v_d_asenta 
								AND d_estado = v_d_estado AND d_ciudad = v_d_ciudad AND d_mnpio = v_d_mnpio
								) THEN
							LET v_Status = 3;
						
							UPDATE bdinteg:si_catsepomex
							SET estatus = v_Status, d_codigo = v_d_codigo, fecha_ejecucion = v_fecha
							WHERE d_asenta = v_d_asenta --AND d_tipo_asenta = v_d_tipo_asenta   -- 20210609 Ya no considerar el d_tipo_asenta
							AND d_estado = v_d_estado AND d_ciudad = v_d_ciudad AND d_mnpio = v_d_mnpio;
							
							CONTINUE FOREACH;
						ELSE
							--- CUANDO ES UN CODIGO POSTAL NUEVO (2)
							LET v_Status = 2;
						END IF				
					END IF	
					
				*/	
			END IF;
			
			LET i_estado = 0;
            let v_c_estado = lpad(trim(v_c_estado), 2, '0');
            LET iRegistros = iRegistros + 1;
            LET v_d_asenta = upper(v_d_asenta);
            LET v_d_tipo_asenta = upper(v_d_tipo_asenta);
            LET v_d_mnpio  = upper(v_d_mnpio);
            LET v_d_estado = upper(v_d_estado);
            LET v_d_ciudad = upper(v_d_ciudad);            
            LET v_d_codigo = lpad(trim(v_d_codigo),5,"0");
			
            INSERT INTO bdinteg:si_catsepomex (d_codigo,d_asenta,d_tipo_asenta,d_mnpio,d_estado,d_ciudad,c_estado,estatus,fecha_ejecucion,fecha_alta)
            VALUES(v_d_codigo, v_d_asenta, v_d_tipo_asenta, v_d_mnpio, v_d_estado, v_d_ciudad, v_c_estado, v_Status, v_fecha, v_fecha);
            LET iRegistros = iRegistros;
        END FOREACH;
		

		---BUSCAR cols que están en si_catsepomex y ya no en archivo que se está cargando, para darlas de baja  --METODO INDICADO POR PAM
		/*INSERT INTO tmp_si_catsepomex_notexist
		SELECT a.d_codigo, a.d_asenta, a.d_mnpio, a.d_ciudad, a.c_estado
		  FROM bdinteg:si_catsepomex a
		        left join tmp_si_catsepomex_dup_07 b on a.d_codigo = b.d_codigo and a.d_asenta = b.d_asenta and a.d_mnpio = b.d_mnpio
                                                        and a.d_ciudad = b.d_ciudad and a.c_estado = b.c_estado				
		  WHERE nvl(a.fecha_baja,'') = '';
		  --INTO temp tmp_si_catsepomex_notexist with no log;
		  
		FOREACH 
 		 select d_codigo, d_asenta, d_mnpio, d_ciudad, c_estado
		    into v_d_codigo, v_d_asenta, v_d_mnpio, v_d_ciudad, v_c_estado
		    from tmp_si_catsepomex_notexist
		
		    IF NVL(v_d_codigo,'') <> '' THEN
		       UPDATE bdinteg:si_catsepomex SET estatus = 4, fecha_baja = v_fecha
		       WHERE d_codigo = v_d_codigo AND d_asenta = v_d_asenta AND d_mnpio= v_d_mnpio AND d_ciudad = v_d_ciudad AND c_estado = v_c_estado;
		    END IF;
			
		END FOREACH;  */

		--PROBAR ahora barriendo la tabla, así si funciona
		IF v_Band1 <> "1" THEN
			FOREACH WITH HOLD
			   SELECT a.d_codigo, a.d_asenta, a.d_mnpio, a.d_ciudad, a.c_estado
				 into v_d_codigo, v_d_asenta, v_d_mnpio, v_d_ciudad, v_c_estado  
				 FROM bdinteg:si_catsepomex a
				 WHERE nvl(a.fecha_baja,'') = ''
			
				SELECT d_codigo into v_d_codigo_1
				  FROM tmp_si_catsepomex_dup_07		
				  WHERE d_codigo = v_d_codigo 
					and d_asenta = v_d_asenta 
					and d_mnpio = v_d_mnpio
					and d_ciudad = v_d_ciudad 
					and c_estado = v_c_estado;

				IF NVL(v_d_codigo_1,'') = '' THEN
				   UPDATE bdinteg:si_catsepomex SET estatus = 4, fecha_baja = v_fecha
				   WHERE d_codigo = v_d_codigo AND d_asenta = v_d_asenta AND d_mnpio= v_d_mnpio AND d_ciudad = v_d_ciudad AND c_estado = v_c_estado;
				END IF;
			
			
			END FOREACH;
		END IF;
		
		---  **************
        ---DROP TABLE bdinteg:tmp_si_catsepomex;
        LET vCodRet         = '00000';
        LET vMensaje        = 'PROCESO EXITOSO';

        LET v_paso = 9;
        --let cCadena = 'find /tmp/' || trim(cNombre) || ' -exec rm {} \;';
        let cCadena = 'find /tmp/' || SUBSTR(cNombre,1,LENGTH(cNombre)) || ' -exec rm {} \;';
        System SUBSTR(cCadena,1,LENGTH(cCadena));
		
        LET v_paso = 10;
		--let cCadena = 'find /tmp/' || trim(cNombre2) || ' -exec rm {} \;';
        let cCadena = 'find /tmp/' || SUBSTR(cNombre2,1,LENGTH(cNombre2)) || ' -exec rm {} \;';
        System SUBSTR(cCadena,1,LENGTH(cCadena));
		
    ELSE
        LET v_paso = 11;
        --let cCadena = 'rm ' || trim(cRuta) || trim(cNombreBase2);
        let cCadena = 'rm ' || SUBSTR(cRuta,1,LENGTH(cRuta)) || SUBSTR(cNombre2,1,LENGTH(cNombre2));
        System SUBSTR(cCadena,1,LENGTH(cCadena));
      
        LET vCodRet         = '00002';
        LET vMensaje        = 'NO EXISTE EL ARCHIVO EN LA RUTA QUE INDICADA EN LOS PARAMETROS';
    END IF;
ELSE
    LET vCodRet         = '00001';
    LET vMensaje        = 'FALTAN PARAMETROS EN TABLA bdinteg:SI_PARAM_DOM';
END IF

UPDATE STATISTICS MEDIUM FOR TABLE bdinteg:si_catsepomex;

INSERT INTO bdinteg:si_bitacora_dom (proceso, cod_ret, mensaje, reg_insert, user_insert, fecha_insert, hora_insert) 
        VALUES(cNombreProceso, vCodRet, vMensaje, iRegistros ,user, v_fecha,
        (SELECT DBINFO('utc_to_datetime', sh_curtime)::DATETIME HOUR TO SECOND from sysmaster:sysshmvals));

RETURN vCodRet, vMensaje;
END 
END PROCEDURE;