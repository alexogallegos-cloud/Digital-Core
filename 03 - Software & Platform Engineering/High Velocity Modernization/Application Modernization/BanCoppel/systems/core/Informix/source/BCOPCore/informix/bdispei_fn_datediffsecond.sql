CREATE FUNCTION "informix".fn_datediffsecond(pFecha1 DATETIME YEAR to SECOND, pFecha2 DATETIME YEAR to SECOND, pTipo INTEGER)
	RETURNING
            integer; -- Resp
...
BEGIN
	...
	IF (pTipo = 16) OR (pTipo = 18) THEN
		LET vDifSeg = ROUND(((vResultadof1 - vResultadof2 ) - 30 ),0);
	ELIF pTipo = 10 THEN
		LET vDifSeg = ROUND(((vResultadof1 - vResultadof2 ) - 5 ),0);
	ELSE
		LET vDifSeg=0;
	END IF;
