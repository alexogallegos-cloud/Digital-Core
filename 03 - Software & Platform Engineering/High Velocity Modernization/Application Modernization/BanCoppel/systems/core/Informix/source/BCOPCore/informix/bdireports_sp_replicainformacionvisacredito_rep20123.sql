CREATE PROCEDURE "informix".sp_replicainformacionvisacredito_rep20123(dFecha1 DATETIME year to fraction(5),dFecha2 DATETIME year to fraction(5),cTrimestre cHAR(5),iMes INTEGER)
...
-- área operativa.
--EXECUTE PROCEDURE "informix".sp_replicainformacionvisacredito_rep20122('2012-06-01 00:00:00.0','2012-07-01 00:00:00.0',20122,6);
...
--Esta informacion solo se corre si es final de trimestre
	...
	--Guarda en la base de datos
		...
		LET vCodRet = '001';
 		LET vVarDataErr = 'intercard:movimiento';
		SET ISOLATION TO DIRTY READ;
		FOREACH WITH HOLD 
       ...
       /*
		...
		LET vCodRet = '002';
 		LET vVarDataErr = 'bdicred:sd_maecred';
		Set isolation to dirty read;
		...
		update bdireports:rpt_creditoclasica
		set	num_ctasinter = NVL(dNumCuentasCredito,0.0)	
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
			  
        LET vCodRet = '003';
 		LET vVarDataErr = 'BdiReports:tmpmovimientostrim';		
		...
		update bdireports:rpt_creditoclasica
		set	numctas_ptovta = NVL(dNumTarjActPOS,0)			
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		...
		LET vCodRet = '004';
 		LET vVarDataErr = 'intercard:tarjeta';
		Set isolation to dirty read;
		...
		update bdireports:rpt_creditoclasica
		set	num_tar = NVL(dNumTarjetas,0.0)			
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		...
		update bdireports:rpt_creditoclasica
		set	numtran_noapro = NVL(dNumeroNoAprobadasTotal,0)
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		...
		LET vCodRet = '006';
 		LET vVarDataErr = 'bdicred:sd_pie_edocta';
		SET ISOLATION TO DIRTY READ;
		SELECT NVL(COUNT(*), 0)
		...
		update bdireports:rpt_creditoclasica
		set	est_ctasenviados = NVL(dNumeroEdoCuenta,0.0)
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;

		LET vCodRet = '009';
 		LET vVarDataErr = 'bdicred:sd_maesdoscont';
		...
		update bdireports:rpt_creditoclasica
		set	limcred_tot = NVL(dLimiteCredito,0.0)
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		...
		LET vCodRet = '010';
 		LET vVarDataErr = 'bdicred:sd_transfun';
		SET ISOLATION TO DIRTY READ;
		FOREACH WITH HOLD
...
/************************       JYDG    **************************/ 			
		    ...
		    LET vCodRet = '011';
 		    LET vVarDataErr = 'insert tmp_replica';
			SET LOCK MODE TO WAIT 3;
		...
		LET vCodRet = '012';
 		LET vVarDataErr = 'BdiReports:tmp_replica';
	    ...
	    update bdireports:rpt_creditoclasica
		set car_fin = NVL(mCargoPorFinTotal,0.0)
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		
		LET vCodRet = '013';
 		LET vVarDataErr = 'bdicont:co_histsdodias';
		...
		update bdireports:rpt_creditoclasica
		set car_pagatra = NVL(mCargoOtrosTotal,0.0)			
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		...
		update bdireports:rpt_creditoclasica
		set	pag_recidos = NVL(mPagosTotal,0.0)
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		...
		update bdireports:rpt_creditoclasica
		set deb_mis = NVL(mDebitosMiscTotal,0.0)
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		
		LET vCodRet = '016';
 		LET vVarDataErr = 'bdicred:sd_maecred mae';
		--Manuel Osuna V.
		...
		update bdireports:rpt_creditoclasica
		set	otros_cred = NVL(mCredOtros,0.0)		
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		...
		update bdireports:rpt_creditoclasica
		set	numctas_men30 = NVL(dSdonumctas_men30,0.0),
		    sdonumctas_men30 = NVL(dNumctas_men30,0)
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;	  
		...
		update bdireports:rpt_creditoclasica
		set	numctas_may30 = NVL(dSdonumctas_may30,0.0),
			sdonumctas_may30 = NVL(dNumctas_may30,0)
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		...
		update bdireports:rpt_creditoclasica
		set	numctas_may60 = NVL(dSdonumctas_may60,0.0),
			sdonumctas_may60 = NVL(dNumctas_may60,0)			
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		...
		update bdireports:rpt_creditoclasica
		set	numctas_may90 = NVL(dSdonumctas_may90,0.0),
			sdonumctas_may90 = NVL(dNumctas_may90,0)
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
		...
		update bdireports:rpt_creditoclasica
		set	numctas_may90 = NVL(dSdonumctas_may120,0.0),
			sdonumctas_may90 = NVL(dNumctas_may120,0)
		where ide_producto = cNumProducto and
		      tipo_producto =  'C' and
			  trimestre = cTrimestre;
