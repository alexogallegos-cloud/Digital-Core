CREATE PROCEDURE "informix".sp_ro_cargaarchivooficiosxml(pUsuario CHAR(8), pIdFuncion CHAR(10), pArchivoProcesar CHAR(255), pRutaCarga CHAR(100))
		RETURNING CHAR(5) AS codret;
		
	DEFINE cCodRet CHAR(5);
	DEFINE iSqlErr INTEGER;
	DEFINE cCmd CHAR(2000);
	DEFINE cArchivoTmp CHAR(255);
	DEFINE cScriptCarga CHAR(255);
	...
	LET cNombreArchivoTmp = 'scriptofi'||TO_CHAR(CURRENT, '%Y%m%d%H%M%S')||'.sql';
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_ro_cargaarchivooficiosxml.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pArchivoProcesar = '' OR pRutaCarga = '' THEN
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF;
		...
		LET cScriptCarga = "echo 'LOAD FROM "||TRIM(pRutaCarga)||TRIM(pArchivoProcesar)||" INSERT INTO bdicnweb:""informix"".oficios_xml_tmp(xmlfile_data);' > "||TRIM(pRutaCarga)||TRIM(cNombreArchivoTmp);
		SYSTEM TRIM(cScriptCarga);
		
		TRUNCATE bdicnweb:"informix".oficios_xml_tmp;		
		
		LET cCmd = TRIM(cRutaInformix)||'dbaccess bdicnweb < '||TRIM(pRutaCarga)||TRIM(cNombreArchivoTmp);
		SYSTEM TRIM(cCmd);
		
		UPDATE STATISTICS HIGH FOR TABLE bdicnweb:"informix".oficios_xml_tmp;
...
'DESCRIPCION: SPL encargado de hacer la limpieza del archivo xml, para que posteriormente sea cargado en la ruta especificada, en este caso: /tmp/mfinis/oficiosxml/.',
'AUTOR: Oscar Flores Conde',
'FECHA: 06/06/2016',
'DESCRIPCION: Se eliminan los tabuladores del archivo xml',
'BD: bdicnweb';
