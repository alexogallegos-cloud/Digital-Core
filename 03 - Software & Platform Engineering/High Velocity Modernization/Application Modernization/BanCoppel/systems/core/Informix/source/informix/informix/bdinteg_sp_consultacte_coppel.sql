CREATE PROCEDURE "informix".sp_consultacte_coppel (pEmpresa CHAR(3),pCteCoppel CHAR(20))
RETURNING CHAR(6)AS cCodRet, CHAR(20) AS cCteCoppel, CHAR(20) AS cCteBanco, DATE AS dFechaMvto, CHAR(20) AS cTipoRelacion, CHAR(45) AS cNomAnalista;

--DEFINICION DE VARIABLES
DEFINE cCodRet  CHAR(6);
DEFINE cCteCoppel CHAR(20);
DEFINE cCteBanco CHAR(20);
DEFINE dFechaMvto DATE;
DEFINE cTipoRelacion CHAR(20);
DEFINE sTipoRelCte SMALLINT;
...
LET cCteCoppel    ="";
LET cCteBanco     ="";
LET dFechaMvto    =DATE(1);
LET cTipoRelacion ="";
LET sTipoRelCte   =0;
LET cNomAnalista  ="";
LET cNumEmpleado  ="";
LET iSqlErr = 0;
--SET DEBUG FILE TO '/respaldosbd/Leslie/sp_consultacte_coppel.out';
    --TRACE ON;	
BEGIN
		...
		ON EXCEPTION SET iSqlErr
			IF iSqlErr <> 0 THEN
				LET cCodret = iSqlErr;
				RETURN cCodRet, cCteCoppel, cCteBanco, dFechaMvto, cTipoRelacion, cNomAnalista WITH RESUME;
			END IF;
		END EXCEPTION;
		...
		IF TRIM(NVL(pEmpresa,''))='' OR TRIM(NVL(pCteCoppel,'')) =''  THEN
			LET cCodret = '000001'; --Parámetros de entrada vacíos
			RETURN cCodRet, cCteCoppel, cCteBanco, dFechaMvto, cTipoRelacion, cNomAnalista;
		ELSE
			FOREACH
				SELECT cliente,numcte_banco, fecha_insert, tipo_relacion, numempleado 
				INTO cCteCoppel, cCteBanco,dFechaMvto,sTipoRelCte,cNumEmpleado
				FROM bdinteg:"informix".si_relacion_ctebcplcpl
				WHERE cliente=TRIM(NVL(pCteCoppel,'')) AND empresa=TRIM(NVL(pEmpresa,''))
				
				SELECT valor_alfabetico
				INTO   cTipoRelacion
				FROM bdicobranza:"informix".cb_param_campania
				WHERE grupo_parametro="TIPO_RELAC"
				AND valor_numerico=NVL(sTipoRelCte,0);
				
				IF NVL(sTipoRelCte,0)=3 THEN
					SELECT nombre
					INTO cNomAnalista
					FROM bdinteg:"informix".si_ejecut
					WHERE ejecutivo=TRIM(NVL(cNumEmpleado,''));
				ELSE 
					LET cNomAnalista="SISTEMAS";
				END IF
				
				RETURN cCodRet, TRIM(NVL(cCteCoppel,'')), TRIM(NVL(cCteBanco,'')), dFechaMvto, UPPER(cTipoRelacion), UPPER(TRIM(NVL(cNomAnalista,''))) WITH RESUME;
			END FOREACH;	
...
END PROCEDURE
DOCUMENT
"Descripción: Consulta los clientes banco con los que se ha relacionado un cliente Coppel",
"Autor : Leslie Rendón",
"FECHA : 08/01/2015",
"BD    : bdinteg";

CREATE PROCEDURE "informix".sp_consultadatoscte_bcpl(pFechaIni CHAR(10),pFechaFin CHAR(10))
...
BEGIN
		...
		ELSE
			...
			FOREACH WITH HOLD
				...
				SELECT a.fecha_insert,a.numcte_banco,a.tipo_relacion,a.definicion,a.tipo_re_ini, status
...
CREATE PROCEDURE "informix".sp_relacion_generarelacion (pNumCte CHAR(20), pNumCteRef CHAR(20),pNumEmpleado CHAR(10),pTipoRel CHAR(1),pOrigen CHAR(1))
	RETURNING
	CHAR(6)  AS COD_RET, 
...
CREATE PROCEDURE "informix".sp_graba_bitacora_registro_huellas(pcTipoMov CHAR(1), pcNumCte CHAR(20), pcSucursal CHAR(4), pcUsuario CHAR(8), pcIp CHAR(15), pcComponente CHAR(16), pcCodRet CHAR(6), pcStoredProc CHAR(50))
...
BEGIN
	...
	INSERT INTO "informix".si_bitacora_registro_huellas(tipomov, numcte, sucursal, usuario, ip, componente, codret, stored_proc, fecha, fecha_hora) 
	VALUES(pcTipoMov, pcNumCte, pcSucursal, pcUsuario, pcIp, pcComponente, pcCodRet, pcStoredProc, CURRENT, CURRENT);
...
"-- Fecha.........: 28/04/2015",
"-- Modificación..: Se crea procedimiento para guardar los registros en la bitacora si_bitacora_registro_huellas",
"-- Sustento......: RQI 64 086_Mantenimiento_comparacion_de_huellas_en_linea_v1.0",
"-- Solicita......: Jose Angel López Adams",
"-- BD............: bdinteg";

CREATE PROCEDURE "informix".sp_relacion_consultainfocte (pNumcte CHAR(20), pOpcion CHAR(1))
	RETURNING
	CHAR(6)  AS COD_RET, 
...
DEFINE dVencidoUdis       DECIMAL(14,2);
...
BEGIN
	...
	--SET DEBUG FILE TO "/informix/jesus/sp_relacion_consultainfocte.out";
	--TRACE ON;
	
	-- VALIDA LOS PARAMETROS DE ENTRADA
	IF NVL(pNumcte,"") = "" AND NVL(pOpcion,"") = "" THEN
		LET cCodRet = '000001';
		LET cMensajeRet = 'Parametros de entrada incompletos,verifique';
	ELSE--obtiene la información del cliente
	
		IF pOpcion = 0 THEN
			LET cNumcteBco = pNumcte;  
		ELIF pOpcion = 1 THEN
			SELECT LIMIT 1 numcte_banco
			INTO cNumcteBco
			FROM bdinteg:"informix".si_relacion_ctebcplcpl
		...
		IF NVL(cNumcteBco,'') <> '' THEN
			...
			ELSE	
				...
				*/
				...
				IF NVL(sTipoRel,0) = 0 THEN
						...
						LET cFechaRelacion 	= '';
				END IF;
			END IF;		CALL bdicred:"informix".determina_udi("001", TODAY) RETURNING cCodRetUDI, dcValorUDI;
			IF NVL(sTipoRel,0) <> 3 THEN --VALIDACION DEL TIPO DE RELACION.
					 --valida si el cliente cuenta con solicitudes de credito
				 IF EXISTS (SELECT num_solicitud FROM bdisolic:"informix".ss_solicitudes WHERE empresa = '001' 	AND        numcte = cNumcteBco ) THEN
					 ...
					 AND aux.numcte = cNumcteBco );		

						--obtiene la información de la ultima consulta a coppel de la solicitud obtenida anteriormente
					SELECT situacion_pago,meses_historia, puntualidad,situacion_credito,causa,(vencidoropa + vencidomuebles + vencidoprestamos),(abonomensualropa + abonomensualmuebles + abonomensualprestamos)
					INTO dSituacion_pago,sMeses_historia, cPuntualidad,cSituacion_credito,sCausa,dcVencidocoppel,dabonomes-- AAME RQM 09 333 Se agrega nuevo campo para anexar el Abono Mensual del cliente 
			...
			ELSE
				...
				SELECT eficiencia,meseshist,puntualidad,sitespecial,causa,(vdoropa+vdomuebles+vdoprestamos),nombrecop,fechanaccop,fecha_consulta,(abonomesropa + abonomesmuebles + abonomesprestamos)
				INTO dSituacion_pago,sMeses_historia, cPuntualidad,cSituacion_credito,sCausa,dcVencidocoppel,cNombre_coppel,cFechaNacCoppel,dtFechaCons,dabonomes -- AAME RQM 09 333 Se agrega nuevo campo para anexar el Abono Mensual del cliente 
				...
				END IF;		
			END IF;	--FIN DE VALIDACION DEL TIPO DE RELACION
		ELSE
			LET cCodRet = '000003';
			LET cMensajeRet = 'Número de cliente no existe, verifique';
		END IF;
	END IF;	--FIN DE VALIDACION DE LOS PARAMETROS DE ENTRADA
	LET dVencidoUdis = CASE WHEN dcValorUDI <= 0 THEN 0 ELSE dcVencidocoppel / dcValorUDI END;
		
	RETURN cCodRet,TRIM(cMensajeRet),cNumcteBco,NVL(cNombreCte,""),NVL(cFechaNac,""),NVL(sTipoRel,0),NVL(cDesTipoRel,""),NVL(cNomEmpleado,""),NVL(cNumRef,''),NVL(cNombre_coppel,""),	
        	NVL(cFechaNacCoppel,""),NVL(cFechaRelacion,""),NVL(dSituacion_pago,0),NVL(sMeses_historia,0), NVL(cPuntualidad,""),NVL(dVencidoUdis,0),
...
'Modificación',
'DESCRIPCION: se agrega opcion de iniciar la conculta con un numero de cliente coppel', 
'AUTOR: Felipe Urias',
'FECHA: 07  de Enero 2015',
'BD: BDINTEG',
'VERSION: 20150107.1326';

CREATE PROCEDURE "informix".spregresahuellac_pba( ptipo CHAR(1), pnumcte CHAR(20), pfechaini DATE, pfechafin DATE )
