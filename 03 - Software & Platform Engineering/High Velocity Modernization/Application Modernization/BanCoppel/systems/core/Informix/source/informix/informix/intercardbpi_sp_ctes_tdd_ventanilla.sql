CREATE PROCEDURE "informix".sp_ctes_tdd_ventanilla( pNumeroMeses VARCHAR(2), pNumMesAnteriorSdo INTEGER )
    
    RETURNING VARCHAR(6) as CODIGO_RETORNO, VARCHAR(80) as MENSAJE_RETORNO, 
        DATE as vFechaInicio, DATE as vFechaFinal, INTEGER as vTotRegTrimestral;
    ...
    DEFINE vTotRegTrimestral INTEGER;
...
BEGIN    
    ...
    LET TIPO_PLANTILLA = 'VENT_CAPTA';
    LET ID_PLANTILLA = '5';
    LET OCTUBRE = '10';
    
    --SET DEBUG FILE TO RUTA_DESTINO||"sp_ctes_tdd_ventanilla.out";
    --TRACE ON;
    ...
    FROM bditarjeta:td_parametro
        WHERE clave = 'SDO_TDD_VENTANILLA';
    ...
    /*
        {+AVOID_FULL (movimientohistorico)}
    Directiva utilizada para considerar los indices creados en la tabla especificada.
     "No full-table scan on the listed table"    
    */    
    SELECT
        {+AVOID_FULL (bdicheq:sc_movhis_old)}
        {+INDEX(bdicheq:sc_movhis_old idx_movhisnew6)}
    sucursal, cuenta, sdo_cuenta
    FROM bdicheq:sc_movhis_old
    WHERE fech_alt BETWEEN vFechaInicio AND vFechaFinal
    AND transacc IN ('0223', '0202') INTO TEMP tmp_mov_ventanilla_historica;

    
    SET ISOLATION TO DIRTY READ;
    SELECT 
        {+AVOID_FULL (bdicheq:sc_movhis)}
        {+INDEX(sc_movhis idx_movhisnew6)}
    sucursal, cuenta, sdo_cuenta
    FROM bdicheq:sc_movhis
    WHERE fech_alt BETWEEN vFechaInicio AND vFechaFinal
    AND transacc IN ('0223', '0202') INTO TEMP tmp_mov_ventanilla_actual;
    
    
    --Union de registros de movimientos.
    SELECT DISTINCT cuenta
    FROM tmp_mov_ventanilla_historica
		UNION ALL
    SELECT DISTINCT cuenta
    FROM tmp_mov_ventanilla_actual WHERE cuenta NOT IN (SELECT DISTINCT cuenta FROM tmp_mov_ventanilla_historica)
		INTO TEMP tmp_mov_trimestrales_ventanilla WITH NO LOG;
    ...
    WHERE mcheq.status_cta = 1
        AND mcheq.cuenta IN (SELECT cuenta FROM tmp_mov_trimestrales_ventanilla)
        --Se deben excluir los clientes de otras plantillas
        AND mcheq.num_cte NOT IN 
            (
                SELECT
    ...
    */
    ...
    --El siguiente conteo es el numero total entregado en el correo electronico despues de finalizar la ejecucion del job.
    SELECT COUNT(*) conteo_total INTO vTotRegTrimestral FROM info_reporte_trimestral;
        ...
        IF (vTotalRegistros = 0) THEN
            ...
            LET vsql = '';
            LET vsql = 'echo "UNLOAD TO "'||RUTA_DESTINO||TIPO_PLANTILLA||'"_00.unl SELECT * FROM info_reporte_trimestral WHERE plantilla = "'||ID_PLANTILLA||'";" > '||RUTA_DESTINO||'tdd_movs_ventanilla.sql';
            SYSTEM vsql;        
            
            LET vsql ='';
            LET vsql= 'dbaccess intercard '||RUTA_DESTINO||'tdd_movs_ventanilla.sql';
            SYSTEM vsql;
            
            LET vsql = '';
            LET vsql ='rm '||RUTA_DESTINO||'tdd_movs_ventanilla.sql';
            SYSTEM vsql;
            ...
            DROP TABLE IF EXISTS tmp_mov_ventanilla_historica;
            DROP TABLE IF EXISTS tmp_mov_ventanilla_actual;
            DROP TABLE IF EXISTS tmp_mov_trimestrales_ventanilla;
            DROP TABLE IF EXISTS tmp_clientes_maestra;
            DROP TABLE IF EXISTS tmp_clientes_promedio;
            DROP TABLE IF EXISTS tmp_info_clientes;
            ...
            ----------------------------------------------------------------------------------------------------------------------------------------------------
            RETURN 	CODIGO_RETORNO, MENSAJE_RETORNO, vFechaInicio, vFechaFinal, vTotRegTrimestral;
        END IF;
        ...
        IF (vTotalRegistros >  0) THEN            
            ...
            WHILE (vTotalRegistros > 0 ) LOOP
                ...
                LET vsql = '';
                LET vsql = 'echo "UNLOAD TO "'||RUTA_DESTINO||TIPO_PLANTILLA||'"_'||vContadorArchivos||'.unl SELECT SKIP '||vNumInicioRegistros||' FIRST '||vRegistrosMaxPorArchivo||' correo_electronico, titular FROM info_reporte_trimestral WHERE plantilla = "'||ID_PLANTILLA||'";" > '||RUTA_DESTINO||'tdd_movs_ventanilla.sql';
                SYSTEM vsql;
                ...
                --Linea que genera los numeros de linea de cada uno de los registros obtenidos | No se hace en base de datos para mejorar optimizaciÃÂ³n.
                LET vsql ='';
...
-- Modificado: 22.enero.2018 09:57:00am
