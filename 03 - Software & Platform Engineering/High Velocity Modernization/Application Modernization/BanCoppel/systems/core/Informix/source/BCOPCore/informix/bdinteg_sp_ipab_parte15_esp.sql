CREATE PROCEDURE "informix".sp_ipab_parte15_esp( pFechaIni DATE, pFechaFin DATE ) 
RETURNING CHAR(5), CHAR(5), CHAR(50);
    ...
    DEFINE cRfc, cTelefono, cTelefono2, cTelefono3, cTelefono4, cTelefono5, cTelefono6, cTelefono7, cRfcAlt CHAR(13);
    ...
    LET cRfc = ''; LET cTelefono = ''; LET cTelefono2 = ''; LET cTelefono3 = ''; LET cTelefono4 = ''; LET cTelefono5 = ''; LET cTelefono6 = ''; LET cTelefono7 = ''; LET cRfcAlt = '';
    LET cCurp = '';
    ...
    ON EXCEPTION SET iSqlErr, iSamErr, cDesErr
        SET DEBUG FILE TO '/resplogifx/conciliachq/ipab/sp_ipab_parte15_esp.err';
        TRACE ON;
        LET cCodRet  = iSqlErr;
    ...
    --- SET DEBUG FILE TO '/resplogifx/conciliachq/ipab/sp_ipab_parte15_esp.out';
    --- TRACE ON;
    ...
    -- // CREA TABLA INFORMACIï¿½N PERSONAL DE LOS TITULARES
    ...
    -- // CREA TABLA INFORMACIï¿½N PATRIMONIAL DE LOS TITULARES
    ...
    /* ###########################################################################################
    -- // CREA TABLA INFORMACIï¿½N CREDITICIA DE LOS TITULARES
    ...
    -- // OBTIENE EL VALOR DE LA UDI A LA FECHA DE LA PROYECCION
    /* 
    ...
    LET mValorUDI = 8.421934;
    */ 

    LET mValorUDI = 8.502028;
    ...
    FOREACH WITH HOLD
        ...
        SELECT TRIM(cte.rfc) AS rfc,  
               ...
               ctepf.fecha_nac,
               cte.rfc_alterno
          INTO cRfc, cPersona, cNombre, cApellido1, cApellido2, cCurp, cSucCte, dFechaNac, cRfcAlt
          FROM bdinteg:si_cliente cte
          LEFT OUTER JOIN bdinteg:si_ctepf ctepf ON (ctepf.numcte = cte.numcte)
         WHERE cte.numcte = cNumCliente;
         
        IF (cRfcAlt is not null OR cRfcAlt <> '' ) THEN
            LET cRfc = cRfcAlt;
        ELSE
            LET cRfc = cRfc;
        ENd IF;
        ...
        -- // OBTIENE DIRECCIï¿½N DEL CLIENTE
        ...
        LET cNombreCalle = REPLACE(cNombreCalle, '/', ' '); 
        LET cNombreCalle = REPLACE(cNombreCalle, 'â', ' '); 
        --- LET cNombreCalle = REPLACE(cNombreCalle, '\', ' '); 
        LET cNombreCalle = REPLACE(cNombreCalle, '|', ' '); 
        LET cNombreCalle = REPLACE(cNombreCalle, '*', ' ');
        LET cColonia = REPLACE(cColonia, '/', ' '); 
        LET cColonia = REPLACE(cColonia, 'â', ' '); 
        --- LET cColonia = REPLACE(cColonia, '\', ' '); 
        LET cColonia = REPLACE(cColonia, '|', ' '); 
        LET cColonia = REPLACE(cColonia, '*', ' ');
        LET cMunicipio = REPLACE(cMunicipio, '/', ' '); 
        LET cMunicipio = REPLACE(cMunicipio, 'â', ' '); 
        --- LET cMunicipio = REPLACE(cMunicipio, '\', ' '); 
        LET cMunicipio = REPLACE(cMunicipio, '|', ' '); 
        LET cMunicipio = REPLACE(cMunicipio, '*', ' ');
        LET cCodPostal2 = REPLACE(cCodPostal2, '/', ' '); 
        LET cCodPostal2 = REPLACE(cCodPostal2, 'â', ' '); 
        --- LET cCodPostal2 = REPLACE(cCodPostal2, '\', ' '); 
        LET cCodPostal2 = REPLACE(cCodPostal2, '|', ' '); 
        LET cCodPostal2 = REPLACE(cCodPostal2, '*', ' ');
        LET cNomCiudadCte = REPLACE(cNomCiudadCte, '/', ' '); 
        LET cNomCiudadCte = REPLACE(cNomCiudadCte, 'â', ' '); 
        --- LET cNomCiudadCte = REPLACE(cNomCiudadCte, '\', ' '); 
        LET cNomCiudadCte = REPLACE(cNomCiudadCte, '|', ' '); 
        LET cNomCiudadCte = REPLACE(cNomCiudadCte, '*', ' ');
        LET cEstado = REPLACE(cEstado, '/', ' '); 
        LET cEstado = REPLACE(cEstado, 'â', ' '); 
        --- LET cEstado = REPLACE(cEstado, '\', ' '); 
        LET cEstado = REPLACE(cEstado, '|', ' '); 
        LET cEstado = REPLACE(cEstado, '*', ' ');
        LET cPais = REPLACE(cPais, '/', ' '); 
        LET cPais = REPLACE(cPais, 'â', ' '); 
        --- LET cPais = REPLACE(cPais, '\', ' '); 
        LET cPais = REPLACE(cPais, '|', ' '); 
        LET cPais = REPLACE(cPais, '*', ' ');
        ...
        SELECT COUNT(*)
          INTO iExcluido
          FROM si_ipab_marcaje_excluidos --si_excluidosipab
         WHERE clave_unica = cNumCliente;
         
        IF cTipoPersona = 'M' THEN
            SELECT COUNT(*)
              INTO iSufijo
              FROM bdinteg:si_ipab_marcaje_fideicomiso
             WHERE no_fideicomiso IN( SELECT no_fideicomiso
                                        FROM bdinteg:si_ipab_marcaje_excluidos
                                       WHERE clave_unica = cNumCliente );
        END IF;
        
        IF   iExcluido > 0 AND iSufijo = 0 THEN
            LET iCauRev = 1;
        ...
        IF iContador2 >= 10000 THEN
            LET iContador2 = 0;
            LET cSql = 'echo "REGISTROS PROCESADOS PARTE 15: '||iContador1||'" > /resplogifx/conciliachq/ipab/regsprocpte15.txt';
            SYSTEM cSql;
        END IF;
        ...
        IF iContador3 >= 100000 THEN
            ...
            UPDATE STATISTICS MEDIUM FOR TABLE si_infpattit_temp ;
            UPDATE STATISTICS MEDIUM FOR TABLE si_ctaasotit_temp ;
            --- UPDATE STATISTICS MEDIUM FOR TABLE si_infcrdtit_temp RESOLUTION .7 DISTRIBUTIONS ONLY FORCE;
            --- UPDATE STATISTICS MEDIUM FOR TABLE si_crdasotit_temp RESOLUTION .7 DISTRIBUTIONS ONLY FORCE;
        END IF;
