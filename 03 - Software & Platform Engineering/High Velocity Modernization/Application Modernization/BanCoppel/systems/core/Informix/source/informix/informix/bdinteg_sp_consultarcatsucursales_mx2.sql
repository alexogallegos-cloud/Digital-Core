CREATE PROCEDURE "informix".sp_consultarcatsucursales_mx2(p_sEmpresa CHAR(3), p_sSucursal CHAR(4))
	...
	IF p_sSucursal = '' THEN
		...
		FOREACH
			SELECT {+AVOID_FULL(bdinteg:si_sucursales)}  
			       suc.empresa, ptf.id_ptf, suc.nombre,ptf.calle||' NUM '||ptf.num_ext AS direccion1, 'COL. '||loc.desc_colonia||' C.P. '||loc.cp AS direccion2, ptf.tel1, suc.gerente, suc.subger, ptf.tipo
	...
	ELSE
		FOREACH
			SELECT suc.empresa, ptf.id_ptf, suc.nombre,ptf.calle||' NUM '||ptf.num_ext as direccion1, 'COL. '||loc.desc_colonia||' C.P. '||loc.cp as direccion2, ptf.tel1, suc.gerente, suc.subger, ptf.tipo
			  INTO v_sEmpresa, v_sSucursal, v_sNombre, v_sDireccion1, v_sDireccion2, v_sTelefono1, v_sGerente, v_sSubgerente, v_sTipo_sucursal
			  FROM bdinteg:si_sucursales suc
			 INNER JOIN bdinteg:si_ptf ptf ON ( ptf.id_ptf = suc.sucursal AND ptf.tipo = suc.tipo )
			  LEFT OUTER JOIN bdinteg:si_localidades loc ON (loc.cve_estado = ptf.cve_estado AND loc.cve_mun = ptf.cve_mun AND loc.cve_col = ptf.cve_col )
			...
			RETURN v_sValRetorno, v_sEmpresa, v_sSucursal, v_sNombre, v_sDireccion1, v_sDireccion2, v_sTelefono1, v_sGerente, v_sSubgerente, v_sTipo_sucursal ;
		END FOREACH;
	END IF;
