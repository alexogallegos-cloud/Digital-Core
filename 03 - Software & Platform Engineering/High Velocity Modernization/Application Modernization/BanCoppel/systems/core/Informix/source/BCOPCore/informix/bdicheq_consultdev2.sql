create procedure "informix".consultdev2(pe_empresa char(3),
                             pe_sucursal     char(4),
                             pe_moneda       char(2),
                             pe_tipodev      char(1),
                             pe_secuencia    smallint)
   returning char(5),char(3), char(5),char(20),char(3), char(10),
             money(14,2), char(3),char(4),char(2),char(4);
...
foreach
   select codigo_bco,secuencia,numero_cta,producto,numero_cheque,
          importe,causa_dev,num_remesa,sucursal,moneda
      into vd_Codigo_Bco,vd_Secuencia, vd_Numero_Cta, vd_Producto,
           vd_Numero_Cheque, vd_Importe, vd_Causa_Dev,vd_Num_Remesa,
           vd_sucursal, vd_moneda
      from sc_devcam
      where empresa = pe_empresa and sucursal = pe_sucursal and
            moneda  = pe_moneda and (tipo_dev = pe_tipodev or
            pe_tipodev = "T")
      order by codigo_bco
   ...
   return vt_CodRet,vd_Codigo_Bco,vd_Secuencia, vd_Numero_Cta,
          ...
          vd_Causa_Dev,vd_sucursal,vd_moneda,vd_Num_Remesa with resume;
end foreach
