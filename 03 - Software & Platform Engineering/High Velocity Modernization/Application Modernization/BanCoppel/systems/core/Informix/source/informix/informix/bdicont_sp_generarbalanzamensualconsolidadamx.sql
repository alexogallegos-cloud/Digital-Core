CREATE PROCEDURE "informix".sp_generarbalanzamensualconsolidadamx(p_dFecha DATE)
RETURNING CHAR(5) AS retorno, VARCHAR(255) AS mensaje;
	...
	--SET DEBUG FILE TO "/tmp/sp_generarbalanzamensualconsolidadamx.out";
	--TRACE ON;
	...
	DELETE FROM bdicont:"informix".bi_balanza_mme
	...
	IF(p_dFecha <= v_dFechaContable) THEN
		...
		IF(v_cProcesoCierre IS NOT NULL) THEN
			IF(v_cProcesoCierre == "000") THEN
				...
				INSERT INTO bdicont:"informix".bi_balanza_mme (fecha_insert, fecha_balanza, ccmayor, ccsub, ccsubsub, ccssubsub, ccsssubsub, sector, tipo_cta, descripcion,
				...
				(SUM(CASE WHEN b.naturaleza_cta = "D" THEN a.saldo_inicio_mes ELSE 0 END) * v_tpc)::MONEY(32,2), 
				(SUM(CASE WHEN b.naturaleza_cta = "A" THEN a.saldo_inicio_mes ELSE 0 END) * v_tpc)::MONEY(32,2),
				(SUM(a.cargos_mes) * v_tpc)::MONEY(32,2), (SUM(a.abonos_mes) * v_tpc)::MONEY(32,2),
				(SUM(CASE WHEN b.naturaleza_cta = "D" THEN a.saldo_fin_de_mes ELSE 0 END) * v_tpc)::MONEY(32,2),
				(SUM(CASE WHEN b.naturaleza_cta = "A" THEN a.saldo_fin_de_mes ELSE 0 END) * v_tpc)::MONEY(32,2)
		...
		IF( v_sCod_Ret = "") THEN
			...
			FOREACH
				...
				FROM 	bdicont:"informix".bi_balanza_mme
				...
				SELECT COUNT(*) INTO w_cuantos FROM bdicont:"informix".bi_balanza_mme WHERE ccmayor = v_mayor AND ccsub = v_cero_s AND ccsubsub = v_cero_ss
				...
				IF bpccmayor > v_cero_may AND bpccsub > v_cero_s AND bpccsubsub > v_cero_ss AND bpccssubsub > v_cero_sss AND bpccsssubsub > v_cero_ssss THEN
					...
					SELECT COUNT(*) INTO w_cuantos FROM bdicont:"informix".bi_balanza_mme WHERE ccmayor = bpccmayor AND ccsub = bpccsub AND ccsubsub = bpccsubsub
					...
					SELECT COUNT(*) INTO w_cuantos FROM bdicont:"informix".bi_balanza_mme WHERE ccmayor = bpccmayor AND ccsub = bpccsub AND ccsubsub = v_cero_ss
					AND ccssubsub = v_cero_sss AND ccsssubsub = v_cero_ssss AND sector = v_cero_sect AND tipo_cta IS NOT NULL AND fecha_balanza = p_dFecha;
					...
					SELECT COUNT(*) INTO w_cuantos FROM bdicont:"informix".bi_balanza_mme WHERE ccmayor = bpccmayor AND ccsub = v_cero_s AND ccsubsub = v_cero_ss
					AND ccssubsub = v_cero_sss AND ccsssubsub = v_cero_ssss AND sector = v_cero_sect AND tipo_cta IS NOT NULL AND fecha_balanza = p_dFecha;
			...
			FOREACH
				...
				UPDATE bdicont:"informix".bi_balanza_mme
