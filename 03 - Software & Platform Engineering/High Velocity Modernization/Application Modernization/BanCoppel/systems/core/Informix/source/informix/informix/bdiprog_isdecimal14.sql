Create Function "informix".isdecimal14(pNumero Char(30))
Returning Smallint;
    ...
    Else
		...
		For i=1 To Length(Trim(pNumero))
			...
			If Substr(Trim(pNumero), i, 1) = '-' And i > 1 Then
				...
				Exit For;
			End If;
			-- // Valido que solo tenga un punto en caso de existir
			If Substr(Trim(pNumero), i, 1) = '.' Then
				Let sContPunto = sContPunto + 1;
				If sContPunto > 1 Then
					Let sResultado = 0;
					Exit For;
				End IF;
			End If;
		...
		-- // Valido que su valor maximo sea el Decimal 14.2 de ifx
		if sResultado = 1 then
			if pNumero > 999999999999.99 Or pNumero < -999999999999.99 then
				Let sResultado = 0;
			End If;			
		End If;
    End If;
