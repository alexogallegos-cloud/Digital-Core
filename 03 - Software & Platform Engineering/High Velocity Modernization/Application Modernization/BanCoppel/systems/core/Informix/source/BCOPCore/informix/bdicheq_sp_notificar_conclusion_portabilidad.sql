CREATE PROCEDURE "informix".sp_notificar_conclusion_portabilidad()
--DATOS A REGRESAR---
...
DEFINE cNumCtaClean 	CHAR(20);
DEFINE cFolio			CHAR(30);
DEFINE cEstatus			CHAR(2);
DEFINE cEstatusDescrip	CHAR(30);
DEFINE cEstatusDesSms	CHAR(30);
...
LET iSqlErr			= 0;
LET cTipoproc 	= '1';
LET cIdPlantilla = 'BEX_PNCSOL';
LET cIdMsj = 'PORTAL_BPI';
LET cTipoMsj 	= '1';
LET cEstatusDescrip = 'Aceptado';
LET cIdPlantillaPush = 'BEX_PPNCSOL';
--LET cIdMsjPush = 'BPI_PGOTCO';
LET cIdMsjPush = 'PNS_BEX';
LET cIdPlantillaSms = 'BEX_SPNCSOL';
...
BEGIN
	...
	-- SET DEBUG FILE TO '/INFORMIXDUMP/sp_notificar_conclusion_portabilidad.trc';
	-- TRACE ON;
    ...
    FOREACH
		SELECT
			"XXXXXXX"||substring(solicitud.cta_receptora from 14 for 4), substring(solicitud.cta_receptora from 7 for 11),
			solicitud.folio_solicitud,solicitud.num_cte,solicitud.estatus_portabilidad,
			tel.telefono,correo.correo_elec,TRIM(c.nombre1), TRIM(c.apell_paterno) 
		INTO cNumcta, cNumCtaClean, cFolio, cNumcte, cEstatus,cTelefono, cCorreoAlterno, cNombre1,cApellidoPat  
		FROM bdicheq:"informix".sc_portacec_solicitud solicitud
		INNER JOIN BDINTEG:"informix".si_telefonos_actual tel
			ON (tel.numcte = solicitud.num_cte)
		INNER JOIN BDINTEG:"informix".si_correos correo
			ON(solicitud.num_cte = correo.numcte)
		INNER JOIN BDINTEG:"informix".si_cliente c
			ON(solicitud.num_cte = c.numcte 
				AND c.empresa = correo.empresa)
		WHERE solicitud.clave_sentido = '2' 
			AND solicitud.sucursal = "5011"
			AND solicitud.estatus_portabilidad <> '2'
			AND LEN(solicitud.fecha_estatus_portabilidad)=8
			AND CAST(substring(solicitud.fecha_estatus_portabilidad from 5 for 2)||
				substring(solicitud.fecha_estatus_portabilidad from 7 for 2)||
				substring(solicitud.fecha_estatus_portabilidad from 3 for 2)
				AS DATE) > DATE(TODAY - 1 UNITS DAY)
			AND tel.tipo_tel = '2'
			AND correo.status_correo = 'A'
		ORDER BY solicitud.fecha_solicitud DESC
		

		IF NVL(cEstatus,"") = "1" THEN
			LET cEstatusDescrip = "Aceptado";	
			LET cEstatusDesSms = "aceptado";
			EXECUTE PROCEDURE bdiadminnomina:"informix".sp_sn_register_account(cNumcte,cNumCtaClean,'0','1','0','001','1','1','2','1','BCPLAP','1')
			INTO cCodRetInt;
		ELSE
			LET cEstatusDescrip = "Rechazado";
			LET cEstatusDesSms = "rechazado";
		END IF;
		--RASTREO DE INFO PARA ENVIO DE SMS
		EXECUTE PROCEDURE bdimnsj:sp_registra_evento(1,cIdMsjSms, cIdPlantillaSms,cNumcte, cNumcta,'', '1', cEstatusDesSms, '', '', '', '', '', '', '', '', '', '', cTelefono, 0, 0, 0, 0, 0,current,current)
		INTO cCodRetInt;
		--Envia EMAIL
		EXECUTE PROCEDURE bdimnsj:sp_registra_evento(1,cIdMsj, cIdPlantilla,cNumcte, cNumcta,'', '1', cNombre1, cApellidoPat, cFolio, cEstatusDescrip, '', '', '', '', '', '', cCorreoAlterno, '', 0, 0, 0, 0, 0,current,current)
		INTO cCodRetInt;
		--Envia PUSH
		EXECUTE PROCEDURE bdimnsj:sp_registra_evento(1,cIdMsjPush, cIdPlantillaPush,cNumcte, cNumcta,'', '1', cEstatusDesSms, '', '', '', '', '', '', '', '', '', '', '', 0, 0, 0, 0, 0,current,current)
		INTO cCodRetInt;
...
'MODIFICO :Arturo Astorga',
'DESCRIPCION:  Se realizan correcciones al procedure y agrega cIdMsjPush = PNS_BEX',
'FECHA : 12/05/2022',

'MODIFICO :Arturo Astorga',
'DESCRIPCION:  Se realiza correccion en el campo y el formato fecha del campo ',
' solicitud.fecha_estatus_portabilidad.',
'FECHA : 30/05/2022',
'BD: bdicheq',

'MODIFICO: 97879606 - Adrian Eduardo Lizarraga Cazares',
'PETICION: Iniciativa cuenta Nomina',
'DESCRIPCION: Se modifica para dar de alta las cuentas con beneficios de nomina al aceptarse las portabilidades de la app.',
'FECHA: 21/09/2022',
'BD: bdicheq';
