CREATE PROCEDURE "informix".sp_guardausuariopass(pusuario CHAR(60), ppass CHAR(60))
...
LET vcodret    = "00000";
	...
	--set debug file to "/tmp/sp_GuardaUsuarioPass.out";
	--trace on;

	--VALIDA QUE EXISTA EL USUARIO
	IF EXISTS (SELECT codparam  FROM Bdicheq:sc_param WHERE empresa = "001" AND codparam = 'NomUserEncrip') THEN
		UPDATE Bdicheq:sc_param SET valor= pUsuario WHERE empresa = "001" AND codparam = 'NomUserEncrip';
	ELSE
		INSERT INTO bdicheq:sc_param(empresa,codparam,descripcion,valor)
		VALUES ('001','NomUserEncrip', 'ELEMENTO PARA CONEXION SERVIDOR AIX', pUsuario );
	END IF;

	--VALIDA QUE EXISTA LA CONTRASEÑA
	IF EXISTS (SELECT codparam  FROM Bdicheq:sc_param WHERE empresa = "001" AND codparam = 'NomPassEncrip') THEN
		UPDATE Bdicheq:sc_param SET valor= pPass WHERE empresa = "001" AND codparam = 'NomPassEncrip';
	ELSE
		INSERT INTO bdicheq:sc_param(empresa,codparam,descripcion,valor)
		VALUES ('001','NomPassEncrip', 'ELEMENTO PARA CONEXION SERVIDOR AIX', pPass );
	END IF;	
...
'DESCRIPCION: GUARDA Y ACTUALIZA EL USUARIO Y PASS PARA LA CONEXION AL SERVIDOR AIX ',
'FECHA      : SEPTIEMBRE 2009',
'VERSION    : 20090908.1142',
'BD         : BDICHEQ';

CREATE PROCEDURE "informix".sp_nominabitacoranvosemp(pNombre_archivo CHAR(17), pFolio CHAR(2), pUsuario CHAR(10), pFecha DATE, pHora DATETIME HOUR TO FRACTION(3))
	RETURNING CHAR(5);
...
--SET debug FILE TO "/tmp/sp_NominaBitacoraNvosEmp.out";
--Trace ON;
...
Begin
	...
	IF bdiprog:isnumeric(pUsuario) <> '1' THEN
		--EL PARAMETRO USUARIO NO ES NUMERICO
		Let cCodret = '566';    
		RETURN NVL(cCodret,'');
	END IF;
	---VALIDAR SI SON NULLOS   pNombre_archivo CHAR(17), pFolio CHAR(2), pUsuario CHAR(10), pFecha DATE, pHora DATETIME HOUR TO FRACTION(3)
	IF TRIM(pNombre_archivo) = '' OR (pNombre_archivo IS NULL) OR TRIM(pFolio) = '' OR (pFolio IS NULL) OR
	   TRIM(pUsuario) = '' OR (pUsuario IS NULL) THEN
		--ERROR EN LOS PARAMETROS  SON NULLOS
		Let cCodret = '563';    
		RETURN NVL(cCodret,'');
	END IF;	
	--VALIDAR EL TAMAÑO de los parametros
	IF LENGTH(TRIM(pNombre_archivo)) <> 17 OR LENGTH(TRIM(pFolio)) <> 2 OR LENGTH(TRIM(pUsuario)) <> 8 THEN
		--EL TAMAÑO DEL ALGUN PARAMETRO NO ES EL CORRECTO
		Let cCodret = '567';    
        RETURN NVL(cCodret,'');
	END IF;	
	-------------------------------
	--Realizar el insert a la tabla sc_nominabitacoranvosemp 
	INSERT INTO sc_nominabitacoranvosemp (nombre_archivo, folio, usuario, fecha, hora) 
	VALUES (pNombre_archivo, pFolio, pUsuario, pFecha, pHora);
    RETURN NVL(cCodret,'');
END
END PROCEDURE
DOCUMENT
'AUTOR :César Valdéz Figueroa',
'DESCRIPCION: Este procedimiento almacena en la tabla sc_nominabitacoranvosemp los valores que son pasados por parametro, ',
'			  tabla la cual funciona como bitacora para los archivos recibidos',
'FECHA : Octubre de 2009',
'BD    : BDICHEQ',
'VERSION: 20091001.0700';

CREATE PROCEDURE "informix".sp_nominadetallenvosemp(
				pNombre_archivo CHAR(17),
				pNum_empleado CHAR(10),
				pApell_paterno CHAR(30),
				pApell_materno CHAR(20),
				pNombres CHAR(30),
				pCuenta_abono CHAR(20),
				pImporte CHAR(16),
				pConcepto INTEGER,
				pStatus CHAR(1),
				pContador INTEGER
...
--SET debug FILE TO "/tmp/sp_NominaDetalleNvosEmp.out";
--Trace ON;
...
Begin
	...
	IF TRIM(pNombre_archivo) = '' OR (pNombre_archivo IS null) OR TRIM(pNum_empleado) = '' OR (pNum_empleado IS null) OR 
	   TRIM(pApell_paterno) = '' OR (pApell_paterno IS null) OR TRIM(pNombres) = '' OR (pNombres IS null) OR TRIM(pCuenta_abono) = '' OR 
	   (pCuenta_abono IS null) OR (pImporte IS null) OR TRIM(pImporte) = '' OR (pConcepto IS null) OR TRIM(pStatus) = '' OR (pStatus IS null) 
	...
	--SE LIMPIAN LA TABLA TEMPORAL SOLO SI CONTADOR ES 1
	IF pContador = 1 THEN
		DELETE FROM BDICHEQ:sc_nominamovimientostemp WHERE nombre_archivo = pNombre_archivo;
	END IF;
	--SE VALIDA QUE SEAN ENTEROS LO QUE DEBEN SER
	IF bdiprog:isnumeric(pNum_empleado) <> '1' OR bdiprog:isnumeric(pCuenta_abono) <> '1' OR bdiprog:isnumeric(pConcepto) <> '1' OR
		bdiprog:isnumeric(pContador) <> '1'  OR bdiprog:isnumeric(pImporte) <> '1' THEN
		--Error Un valor No Es  Numerico En detalle
		LET cCodret = '187';
		RETURN cCodret;
	END IF;
	--Obtener el parametro del tamaño de la cuenta
	SELECT valor INTO cLongCta FROM bdicheq:sc_param WHERE empresa = "001" AND codparam = 'longcta';
	--VALIDAR EL TAMAÑO de los parametros
	IF LENGTH(TRIM(pNombre_archivo)) <> 17 OR LENGTH(TRIM(pCuenta_abono)) <> cLongCta THEN
		--EL TAMAÑO DEL ALGUN PARAMETRO NO ES EL CORRECTO
		Let cCodret = '567';    
        RETURN NVL(cCodret,'');
	END IF;	
	--HAcer el inser a la temporal para una siguiente validacion
	INSERT INTO sc_nominamovimientostemp (nombre_archivo, num_empleado, apell_paterno, apell_materno, nombres, cuenta_abono, importe, concepto, status)
	VALUES (pNombre_archivo,pNum_empleado ,pApell_paterno,pApell_materno,pNombres,pCuenta_abono,(pImporte),pConcepto,pStatus);
	RETURN NVL(cCodret,'');
END
END PROCEDURE
DOCUMENT
'AUTOR :César Valdéz Figueroa',
'DESCRIPCION:  Este procedimioento guarda los datos que se pasan por parametro en la tabla sc_nominamovimientostemp',
'              ademas de validar que no este vacios ni nullos',
'FECHA : Septiembre de 2009',
'BD    : BDICHEQ',
'VERSION: 20090929.0530';
