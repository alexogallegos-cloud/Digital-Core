CREATE PROCEDURE "informix".sp_consulta_prodaperturadoscte(pNumcte CHAR(20),pEjecucion SMALLINT)

	--RETORNOS-
	RETURNING
	CHAR(6)      AS codigo_ret,
	SMALLINT     AS Total_productos,
	SMALLINT     AS prod_debito,
	SMALLINT     AS prod_credito,
	CHAR(4)      AS codigo_producto,
	CHAR(40)     AS nombre_producto,
	CHAR(20)     AS cuenta,
	CHAR(2)      AS cod_estatus,
	CHAR(30)     AS desc_estatus;

	--DECLARACION DE VARIABLES--
	DEFINE iSql_err		    INTEGER; 
	DEFINE cCodret		    CHAR(6);
	DEFINE sTotalProd       SMALLINT;
	DEFINE sTotalDebito     SMALLINT;
	DEFINE sTotalCredito    SMALLINT;
	DEFINE cCodProd         CHAR(4);
	DEFINE cNomProd         CHAR(40);
	DEFINE cCodStatus       CHAR(2);
	DEFINE cEstatusProdCte  CHAR(30);
	...
	BEGIN
		...
		ON EXCEPTION SET iSql_err 
			IF iSql_err <> 0 THEN
				...
				RETURN TRIM(cCodret) , NVL(sTotalProd,0), NVL(sTotalDebito,0), NVL(sTotalCredito,0), TRIM(NVL(cCodProd,'')), TRIM(NVL(cNomProd,'')), TRIM(NVL(cCuenta,'')),TRIM(NVL(cCodStatus,'')), TRIM(NVL(cEstatusProdCte,''));
			END IF;
		END EXCEPTION;
			
		--SET DEBUG FILE TO '/dbexportb/carlos/like/sp_consulta_prodaperturadoscte.out';
		--TRACE ON;
		
		  SET ISOLATION TO DIRTY READ;
		  SET LOCK MODE TO WAIT 3;
		  
		--SE SACAN NVL's PARA EVITAR USARLO EN EL DECODE
		 IF NVL(pEjecucion,1) = 1 THEN 
			LET pEjecucion = 1;
		 END IF;
		 
		 IF pEjecucion NOT IN (1,2) THEN 
			LET cCodret = '000001'; --MODO DE EJECUCION NO VALIDO
		 ELIF NVL(pNumcte,'') = '' THEN
			LET cCodret = '000002'; --PARAMETRO VACIO
		 END IF;
		 
		 IF cCodret <> '000000' THEN  --SI EL CODIGO DE RETORNO ES DIFERENTE DE EXITO EN ESTE PUNTO DE LA EJECUCION ENTRA EN EL IF 
			RETURN TRIM(cCodret) , NVL(sTotalProd,0), NVL(sTotalDebito,0), NVL(sTotalCredito,0), TRIM(NVL(cCodProd,'')), TRIM(NVL(cNomProd,'')), TRIM(NVL(cCuenta,'')),TRIM(NVL(cCodStatus,'')), TRIM(NVL(cEstatusProdCte,''));
		 END IF;
		...
		-- REVISA SI TIENE CUENTAS DE CAPTACION
		SELECT COUNT(*) AS num_captacion
		INTO sTotalDebito
		FROM bdicheq:"informix".sc_maechq
		WHERE num_cte = pNumcte; 
		
		-- REVISA SI TIENE SOLICITUDES.
		SELECT COUNT(*) AS num_credito
		INTO sTotalCredito
		FROM bdisolic:"informix".ss_solicitudes
		WHERE empresa = '001'
		AND numcte =  pNumcte; 
		
		-- TOTAL DE PRODUCTOS CREADOS Y SOLICITADOS.
		LET sTotalProd = sTotalDebito + sTotalCredito;
		
		--EL CLIENTE NO ES VALIDO O NO TIENE NINGUNA CUENTA
		IF sTotalProd = 0 THEN
			LET cCodret = '000003'; 
			RETURN TRIM(cCodret) , NVL(sTotalProd,0), NVL(sTotalDebito,0), NVL(sTotalCredito,0), TRIM(NVL(cCodProd,'')), TRIM(NVL(cNomProd,'')), TRIM(NVL(cCuenta,'')),TRIM(NVL(cCodStatus,'')), TRIM(NVL(cEstatusProdCte,''));
		END IF;	
		
		IF pEjecucion = 2 THEN
		
			--SE INICIALIZA EL CONTADOR
			LET sTotalDebito = 0;
			LET sTotalCredito = 0;
			
			--SE CONSULTAN LOS PRODUCTOS DE DEBITO 
			FOREACH
				SELECT a.producto , b.nombre, a.status_cta, a.cuenta
				INTO cCodProd, cNomProd, cCodStatus , cCuenta
				FROM bdicheq:"informix".sc_maechq a , bdicheq:"informix".sc_producto b
				WHERE a.num_cte = pNumcte
				AND a.producto = b.producto
				ORDER BY 1,4
				
				LET sTotalDebito = sTotalDebito +1;
				
				--SE OBTIENE LA DESCRIPCION DE ESE CODIGO
				SELECT descripcion
				INTO cEstatusProdCte
				FROM bdicheq:"informix".sc_mae_estatus
				WHERE cod_estatus = TRIM(cCodStatus);

				RETURN TRIM(cCodret) , NVL(sTotalProd,0), NVL(sTotalDebito,0), NVL(sTotalCredito,0), TRIM(NVL(cCodProd,'')), TRIM(NVL(cNomProd,'')), TRIM(NVL(cCuenta,'')),TRIM(NVL(cCodStatus,'')), TRIM(NVL(cEstatusProdCte,'')) WITH RESUME;
			...
			FOREACH
				...
				--CONSULTA LAS SOLICITUDES DE CREDITO.
				SELECT {+INDEX(bdisolic:"informix".ss_solicitudes idx_numctesolic)} {+INDEX(bdicred:"informix".sd_definicion idx_sd_definifionempprd)} a.num_producto, b.nombre_prod, a.status_solicitud, a.num_solicitud
				INTO cCodProd, cNomProd, cCodStatus, cCuenta
				FROM bdisolic:"informix".ss_solicitudes a, bdicred:"informix".sd_definicion b
				WHERE a.numcte =  pNumcte
				AND a.num_producto = b.num_producto
				AND b.empresa = '001'
				ORDER BY 1,4
			
				LET sTotalCredito = sTotalCredito +1;
				
				--VALIDA SI LA SOLICITUD YA ESTA APERTURADA.
				IF TRIM(cCodStatus) = 'AP' THEN	
				
					IF cCodProd IN ('6300', '6011') THEN
					
						--CONSULTA EL ESTATUS DEL PRESTAMO PERSONAL O REESTRUCTURA DE TARJETA DE CREDITO.
						SELECT status_cred 
						INTO cStatusCred
					...
					ELSE
						...
						--CONSULTA EL ESTATUS DEL CREDITO.
						SELECT status_cred 
						INTO cStatusCred
					...
					--CONSULTA LA DESCRIPCION DEL ESTATUS DEL CREDITO.
					SELECT TRIM(descripcion) 
					INTO cEstatusProdCte
					FROM bdicred:"informix".sd_tipocartera
					WHERE status_cred = TRIM(cStatusCred);	
				ELSE
				
				--CONSULTA EL ESTATUS DE LA SOLICITUD.
					SELECT TRIM(descripcion) 
					INTO cEstatusProdCte
					FROM bdisolic:"informix".ss_status_sol
					WHERE empresa = '001' 
					AND status_solicitud = TRIM(cCodStatus);	
		...
		ELSE
			--SI LA EJECUCION ES 1, SOLO DEVOLVERA LOS RESULTADOS
			RETURN TRIM(cCodret) , NVL(sTotalProd,0), NVL(sTotalDebito,0), NVL(sTotalCredito,0), NVL(cCodProd,''), NVL(cNomProd,''), NVL(cCuenta,''),NVL(cCodStatus,''), NVL(cEstatusProdCte,'');
			
		END IF; 
	END;
END PROCEDURE
DOCUMENT
'DESCRIPCIÓN: consulta las cuentas aperturadas de captacion y ademas los créditos solicitados y/o aperturados de un cliente en especifico,',
'esto en base a dos modalidades: 1: Obtiene el total de cuentas y/o creditos. 2: Obtiene el detalle del total de las cuentas y/o creditos.',
'FECHA DE CREACIÓN: NOVIEMBRE DE 2013',
'BASE DE DATOS: BDINTEG',
'CREADOR: CARLOS OCHOA VALENZUELA',
'VERSION: 20131104.1830';

CREATE PROCEDURE "informix".sp_infosms_cte_coppel( pNumCte CHAR(9))
RETURNING CHAR(5) as CodRet, CHAR(9) as NumCte, CHAR(10) as TelCel, CHAR(4) as CodSMS, CHAR(2) as Conpania;
...
BEGIN
	ON EXCEPTION SET iSql_err
		...
		IF iSql_err <> 0 THEN
			LET cCodret = iSql_err;
			RETURN cCodret, sNumCte, sTelCel, sCodSMS,sCompania;
		END IF;
	END EXCEPTION;	
	
	--SET DEBUG FILE TO '/tmp/sp_infosms_cte.sql';
	--TRACE ON;
    ...
    FOREACH
        SELECT FIRST 1 a.numcte, a.telefono, b.digito_ver, LPAD(a.carrier,2,0)
          INTO sNumCte, sTelCel, sCodSMS, sCompania
            FROM bdinteg:"informix".si_telefonos_actual a
                LEFT JOIN bdinteg:"informix".si_bitsmstels b
	...
	RETURN cCodret, NVL(sNumCte,0), NVL(sTelCel,''), NVL(sCodSMS,''), NVL(sCompania,'');
...
'FECHA:	12/DIC/2015',
'DESCRIPCION: Se agrega el campo carrier para retornar la compania de celular',
'BD: BDINTEG';
