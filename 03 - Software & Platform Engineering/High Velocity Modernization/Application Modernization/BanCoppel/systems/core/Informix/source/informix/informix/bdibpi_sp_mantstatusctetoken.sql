CREATE PROCEDURE "informix".sp_mantstatusctetoken(pNumCliente VARCHAR(9))
	RETURNING char (5); 
...
BEGIN
   ...
   END EXCEPTION;
		...
		If EXISTS  (SELECT  usr.numcliente 			  
				    ...
				    GROUP BY 1
				HAVING COUNT(id_pregunta) = 0) THEN
			  			 			  
				UPDATE bdinteg:"informix".si_bpitoken SET id_status_token = '130' where num_cliente = pnumcliente AND id_status_token = '150';
				select ns_token INTO vns_token from bdibpi:bpi_tokensolicitud where numcte = pnumcliente;
				UPDATE bdibpi:"informix".tkn_nseries  SET id_status = '130'       where ns_token = vns_token AND id_status = '150';
				LET cod_ret = '000'; -- EL STATUS DEL TOKEN SE ACTUALIZO
				--LET vDesc_concepto = 'EL STATUS DEL TOKEN SE ACTUALIZO';
		ELSE 
			LET cod_ret = '001'; -- EL STATUS DEL TOKEN NO TIENE LAS CARACTERISTICAS PARA SER ACTUALZADO A 130
			--LET vDesc_concepto = 'EL STATUS DEL TOKEN ES ACTUALIZAR';
		END IF;
