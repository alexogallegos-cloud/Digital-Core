CREATE PROCEDURE "informix".sp_validadv_web(chrccc char(18))
...
select count(1) into intcontador from tblbancos where chrcodigo=chrbanco;
if intcontador<1 then
  let  intCodigoError = "100";
  return intCodigoError, strFuenteError;
end if;

select count(1) into intcontador from tblplazas where chrcodigo=chrplazatef;
if intcontador<1 then
  let intCodigoError = "100";
  return intCodigoError, strFuenteError;
end if; }
