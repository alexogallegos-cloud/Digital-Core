CREATE PROCEDURE "informix".gen_archsdos_des_comp5()
RETURNING CHAR(5), CHAR(5), CHAR(50), INTEGER;
    ...
    ON EXCEPTION SET vsqlerr, isam_err, desc_err
        SET DEBUG FILE TO "/resplogifx/conciliachq/gen_archsdos_des_comp5.err";
        TRACE ON;
        IF vsqlerr <> 0 THEN
            ...
            LET vsql = 'echo "UPDATE bdinteg:sx_contproc '||
                       ...
                       'AND sistema   = '''||vsistema||''';" > /tmp/horassdosdia5.sql';
            SYSTEM vsql;
            LET vstmt = '/ifxsif01/bin/dbaccess bdicheq /tmp/horassdosdia5.sql';
            SYSTEM vstmt;
            RETURN vcodret1, vcodret2, vcodret3, vcontador1;
        END IF;
    END EXCEPTION;
    
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/gen_archsdos_des_comp5.out";
    --- TRACE ON;
    ...
    IF vexiste = 0 THEN
        LET vsql = 'echo "INSERT INTO bdinteg:sx_contproc VALUES '||
                   ...
                   '(SELECT CURRENT + (fecha_hoy - CURRENT) FROM bdicheq:sc_fechas), NULL, NULL);" > /tmp/horassdosdia5.sql';
        SYSTEM vsql;
    ...
    FOREACH WITH HOLD
        SELECT UNIQUE chq.cuenta, chq.sucursal, chq.sdo_dia_ant, chq.imp_chq_sbg, noc.int_acum, noc.acum_sdo_int, noc.fecha_alta,
               chq.producto, chq.status_cta, chq.fecha_proceso, chq.fec_ult_mov, chq.sdo_actual, pro.pago_interes
          INTO vcuenta, vsucursal, vsdo_dia_ant, vimp_chq_sbg, vint_acum, vacum_sdo_int, vfecha_alta,
               vproducto, vstatus_cta, vfecha_proceso, vfec_ult_mov, vsdo_actual, vpago_interes
          FROM sc_maechq chq, 
               sc_maenoc noc,
               sc_movdia_concil mov,
               sc_producto pro
        ...
        IF vpago_interes = 'M' THEN
            LET vprovint = vprovint - vdesprov;
            LET vint_acum = ( vprovint - vpagoint );
        ELSE
