CREATE PROCEDURE "informix".sp_consulta_estatus_os2(pAgrupamiento CHAR(25), pNumSucursal CHAR(4), pStatusSolct CHAR(2), pArea CHAR(3), pRegInicio INTEGER,
												pRegFin INTEGER,pMatriz_impr SMALLINT, pFecha_Inicio CHAR(10), pFecha_Fin CHAR(10),pNumCte CHAR(9), pNumCred CHAR(12))
RETURNING CHAR(6), INT, SMALLINT, CHAR(40), CHAR(5), CHAR(20), DATE, CHAR(2), CHAR(170), DATE, CHAR(50), DATE, INT, CHAR(30),
...
--TRACE ON;
*/
--SET DEBUG FILE TO '/tmp/mfinis/sp_consulta_estatus_os2.out';
--TRACE ON;
	...
	ELSE
		...
		IF sRegistros > 0 THEN
		    ...
		    ELIF pAgrupamiento = 'STATUS' THEN
		        IF pArea = "OFI" THEN
					--Valida que el nÃºmero de la sucursal y el estatus de la solicitud sean vÃ¡lidos
          ...
          /*  -- 2012-01-09  MACF
					...
					*/
					------------------------------------------------------------------------------------------------------------------------------------
						---VALIDAR SI LA HORA ES DESPUÃS DE LAS 17 HRS. ( SOLAMENTE PARA OFI )
           ...
           END IF;
			...
			ELIF pAgrupamiento = 'REGIONES' THEN
                        -- Primero traemos las solicitudes que tienen asignada cualquier regiÃ³n.
		        FOREACH
            SELECT { + INDEX (ss_solicitud_env_os numero_solicitud)}   NVL(matriz_impr,0), NVL(numero_cobranza,0),
								NVL(nombre_sucursal,''), NVL(producto,''), NVL(SOL.num_solicitud,''),NVL(fecha_solicitud,''), NVL(SOL.status_solicitud,''),
								NVL(nombre_cliente,''), NVL(fecha_nacimiento,''), NVL(folio_os,''),NVL(fecha_os,''), NVL(dias_os,0), NVL(nombre_calle,''),
		        ...
		        END FOREACH;
                        -- Obtenemos las solicitudes que no tienen asignada regiÃ³n.
                FOREACH
    		          SELECT { + INDEX (ss_solicitud_env_os numero_solicitud)}   NVL(matriz_impr,0), NVL(numero_cobranza,0),
    								NVL(nombre_sucursal,''), NVL(producto,''), NVL(SOL.num_solicitud,''),NVL(fecha_solicitud,''), NVL(SOL.status_solicitud,''),
    								NVL(nombre_cliente,''), NVL(fecha_nacimiento,''), NVL(folio_os,''),NVL(fecha_os,''), NVL(dias_os,0), NVL(nombre_calle,''),
