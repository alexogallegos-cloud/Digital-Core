CREATE PROCEDURE "informix".gen_balprevreg(pempresa char(3), w_fecha date,v_usuario char(10))
...
if v_numregs > 0 then
   foreach
      ...
      sum(saldo_fin_de_dia),naturaleza_cta,bdinteg:si_catalog.nombre,tipo_cuenta
      into   sdempresa,sdccmayor,sdccsub,sdccsubsub,sdccssubsub,sdccsssubsub,
             sdsector,
             sdciudad,
             --sdsucursal,
             sdmoneda,sdcargos_dia,sdabonos_dia,
             sdsaldo_inicio_dia,sdsaldo_fin_de_dia,sdnaturaleza_cta,
             sdnombre_cta,sdtipo_cta
      ...
      GROUP BY 1,8,2,3,4,5,6,7,9,14,16,15
      ORDER BY 1,2,3,4,5,6,7,8,9,14,16,15

      {SELECT plaza
      INTO v_plaza
      FROM bdinteg:si_sucursales
      WHERE empresa = pempresa
        AND sucursal = trim(sdsucursal);
      ...
      if sdsaldo_inicio_dia = 0 and sdcargos_dia = 0
         ...
         values (sdempresa,sdccmayor,sdccsub,sdccsubsub,sdccssubsub,
                 sdccsssubsub,sdsector,sdciudad,
                 --sdsucursal,
                 "",sdmoneda,
                 w_fecha,sdsaldo_inicio_dia,sdcargos_dia,
                 sdabonos_dia,sdsaldo_fin_de_dia,sdtipo_cta,sdnaturaleza_cta,
                 sdnombre_cta," "," ",0,0,v_usuario);
      end if
   end foreach
{lse
   foreach
      ...
      co_detpol.sector,co_detpol.ciudad,co_detpol.moneda,
      0,0,0,0,naturaleza_cta,nombre
      into   sdempresa,sdccmayor,sdccsub,sdccsubsub,sdccssubsub,sdccsssubsub,
             sdsector,sdciudad,sdmoneda,sdcargos_dia,sdabonos_dia,
