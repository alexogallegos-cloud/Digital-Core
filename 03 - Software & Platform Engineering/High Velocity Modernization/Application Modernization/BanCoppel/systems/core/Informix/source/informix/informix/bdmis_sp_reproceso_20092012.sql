create procedure "informix".sp_reproceso_20092012()
...
begin	
   ...
   set isolation to dirty read;
	...
	--	EXTRACION DE MOVIMIENTOS DIARIOS DEL DIA
		...
		and prod.producto = chq.producto and mae.fecha_alta = '09/20/2012' and chq.status_cta <> "2"
	...
	-- INSERCION DE CUENTAS PROACTIVAS 
        ...
        where mae.fecha_alta = '09/20/2012' and mae.empresa = '001' and cta_eje = mae.cuenta and  status_cta = 1 and        
	...
	-- INSERCION DE CUENTAS DE PAGARES.
	    ...
	    where fecha_alta = '09/20/2012' --and status_cta = '1' // se elimina condición para generar correctamente la información
	...
	-- SOLICITUDES
	        ...
	        BEGIN WORK;           
			 select empresa, sucursal, user_insert, num_solicitud, '6001' as num_producto, numcte, '' as num_referencia,fecha_insert
                ...
                where fecha_insert = '09/20/2012' AND (num_producto not in ('6500') or user_insert = 'interact') AND  --today /*'09/20/2012' o '09/20/2012'fto */
                     numcte not in (SELECT rpt.numcte FROM mi_rptsolic rpt
                                     WHERE rpt.numcte = sol.numcte and rpt.sucursal = sol.sucursal and rpt.fecha_insert < '09/20/2012' )
                                 and num_solicitud = ( Select min(num_solicitud) from bdisolic:ss_solicitudes soli2  
                                 where fecha_insert = '09/20/2012' and soli2.numcte = sol.numcte 
            ...
            select empresa, sucursal, ejecutivo as user_insert, '' as num_solicitud, '6001' as num_producto,'' as numcte, num_referencia, fecha
                ...
                where fecha  = '09/20/2012' AND (producto not in ('6500') or ejecutivo = 'interact' )  AND --today /*'09/20/2012' o '09/20/2012'fto */
                      num_referencia not in (SELECT rpt.num_referencia FROM mi_rptsolic rpt
                                             WHERE rpt.num_referencia = precal.num_referencia and rpt.sucursal = precal.sucursal and rpt.fecha_insert < '09/20/2012' )
                and consecutivo = (SELECT min(consecutivo) FROM bdisolic:ss_bitacora_precal precali2 WHERE fecha  = '09/20/2012' AND precali2.num_referencia = precal.num_referencia
	...
	--
		...
		BEGIN WORK;
			select 'APERC' as tipo,empresa,sucursal,user_insert,num_producto, count(numcte) as num_ctasdia
				from bdmis:solicis
				where fecha_insert = '09/20/2012' and (numcte is not null or numcte <> '') and numcte > "000000000"
				group by 1,2,3,4,5
			union all
			select 'APERC' as tipo,empresa,sucursal,user_insert,num_producto,count(num_referencia) as num_ctasdia
				from bdmis:solicis
				where fecha_insert = '09/20/2012' and (num_referencia is not null or num_referencia <> '') and num_referencia > "0"
				group by 1,2,3,4,5
		...
		insert into mi_rptsolic(empresa, sucursal, ejecutivo, num_producto, numcte, num_referencia, fecha_insert )
		 ...
		 WHERE fecha_insert = '09/20/2012';
		 
	--tarjetas de credito entregadas 	

		insert into mi_tmpcierresuc(tipo,empresa,sucursal,ejecutivo,producto,num_ctasdia)
		select 'APERC',empresa,sucursal,ejecutivo,'6666',count(*)
		from bdicred:sd_maecred where fecha_apertura = '09/20/2012' and empresa = '001'
		group by 1,2,3,4; 
    ...
    --abonos y retiros de 		
	    ...
	    from table ( multiset(
						...
						from table ( multiset (
									 ...
									 where mov.empresa = '001' and mov.cuenta = mae.cuenta and mov.fech_alt = '09/20/2012' and mov.producto = mov.producto and mov.empresa = mae.empresa
		...
		from table (multiset(
				 select empresa,sucursal,usuario,
				  ...
				  from table( multiset(
								...
								where mov.fecha_mov = '09/20/2012' and mov.empresa = '001' and mov.reversado <> 'S' and mov.usuario <> 'interact'
		...
		set isolation to dirty read;
		select num_sucursal,
		(meta_acu_cap +  ((metasuc.meta_men_cap / 30.5) * day('09/20/2012')))::money / nvl (num,1) as metacapdia,     
		(meta_acu_col +  ((meta_men_col / 30.5) * day('09/20/2012')))::money   as metacoldia
			   from table ( multiset (
								   ...
								   from table ( multiset (
									...
									sum(case when tpo.aniomes[5,6] < MONTH('09/20/2012') then meta_monto_cap else 0 end) as meta_acu_cap,
									sum(case when tpo.aniomes[5,6] < MONTH('09/20/2012') then meta_monto_col else 0 end) as meta_acu_col,
									sum(case when tpo.aniomes[5,6] = MONTH('09/20/2012') then meta_monto_cap else 0 end) as meta_men_cap,
									sum(case when tpo.aniomes[5,6] = MONTH('09/20/2012') then meta_monto_col else 0 end) as meta_men_col
									from mi_sucursalesinfo li,mi_tiposuc tpo
									where tpo.id_tiposuc = tipo_suc and tpo.aniomes[1,4] = YEAR('09/20/2012')  and  tpo.aniomes[5,6] <= MONTH('09/20/2012')                            
									 group by 1,2
	    ...
	    num_retirocapta,monto_retirocapta,num_retirocoloca,monto_retirocoloca)
            select  tmp.empresa,tmp.sucursal,tmp.ejecutivo,si.nombre,tmp.producto,'09/20/2012',num_ctasdia,monto_ctasdia,
		...
		MP.ANIOMES = YEAR('09/20/2012') ||  LPAD( MONTH('09/20/2012'),2,'00') AND TS.ANIOMES = YEAR('09/20/2012') ||  LPAD(MONTH('09/20/2012'),2,'00') and  metaprod.num_sucursal = tmp.sucursal;
