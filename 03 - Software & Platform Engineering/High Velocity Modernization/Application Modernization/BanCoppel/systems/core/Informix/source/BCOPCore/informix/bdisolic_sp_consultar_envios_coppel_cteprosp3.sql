CREATE PROCEDURE "informix".sp_consultar_envios_coppel_cteprosp3(pTipoConsulta SMALLINT)
...
BEGIN
	...
	-- SET DEBUG FILE TO "/tmp/Guicho/sp_consultar_envios_coppel_cteprosp3.out";
	-- TRACE ON;
	...
	IF pTipoConsulta IN (1,3) THEN
		...
		FOREACH WITH HOLD
			...
			--WHERE envio_parametrico = "1" DSB20201026
			WHERE envio_parametrico = "8"
            --and sucursal > '0815' DSB20201026
			AND status_solicitud = "EC"
			--AND fecha_hora >= EXTEND(current, YEAR to SECOND)+0 UNITS HOUR-06 UNITS MINUTE+0 UNITS second
			AND fecha_hora < EXTEND(current, YEAR to SECOND)+0 UNITS HOUR-05 UNITS MINUTE+0 UNITS second
...
*/
	...
	IF pTipoConsulta IN (2,3) THEN
		...
		FOREACH WITH HOLD
				...
				SELECT empresa,numcte,numcte_pros
				INTO cEmpresa,cNumCte,cNumSolicitud
				FROM bdiprospectos:"informix".pr_cliente
				WHERE envio_parametrico = "4" AND status_numcte_pros = "EC" 
				ORDER BY fecha_hora DESC --AUM
