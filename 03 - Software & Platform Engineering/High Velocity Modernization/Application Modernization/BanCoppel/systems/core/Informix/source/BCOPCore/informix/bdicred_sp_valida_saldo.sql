CREATE PROCEDURE "informix".sp_valida_saldo(pempresa CHAR(3),pCuenta CHAR(20),pTipo CHAR(1))
RETURNING CHAR(5),       -- Codigo de Retorno
		  CHAR(1); --1--cuanta con saldo 0 --sin saldo
...
END EXCEPTION;
 ...
 --SET DEBUG FILE TO "/informix/jesus/sp_valida_saldo.out";
 --TRACE ON;
-- **********************************************************************
-- *                        PROGRAMA PRINCIPAL
-- **********************************************************************

    IF NVL(pCuenta, '') = '' OR  NVL(pTipo, '') = '' THEN
        LET  cCod_ret = '00001';		
		RETURN cCod_ret,'0';
	END IF;
   ...
   IF pTipo = 'D' THEN
		...
		IF EXISTS (SELECT cuenta FROM bdicheq:"informix".sc_maechq WHERE cuenta = pCuenta AND sdo_actual >0) THEN 
			LET cSaldo = "1";
		ELSE
			LET cSaldo = "0";
		END IF;
   ...
   ELSE 
		IF EXISTS ( SELECT num_credito FROM bdicred:"informix".sd_maesdos WHERE num_credito = pCuenta 	AND sdo_capital > 0 ) THEN		
			LET cSaldo = "1";
		ELSE
			LET cSaldo = "0";
		END IF;
   END IF;
	...
	RETURN cCod_ret,cSaldo;
END;
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se realiza procedimiento para validacion de saldos de la cuenta',
'AUTOR: Jes̼s Manuel Aguilar Heredia',
'BD: bdicred ',
'FECHA: noviembre 2014',
'VERSION: 20141111.1735';
