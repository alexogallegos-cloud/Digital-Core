CREATE PROCEDURE "informix".sp_aplicacargaarchcecoban(pUsuario CHAR(8), pIdFuncion CHAR(10), pNombreArchivo CHAR(22), pDireccionMac CHAR(12), pCodOperacion CHAR(5), pFecha DATE)
	...
	DEFINE cEncFechaPresenta CHAR(8);
	DEFINE cEncTipoArchivo CHAR(1);
	DEFINE cDetDatosCodOperacion CHAR(2);
	DEFINE cDetDatosNumCuenta CHAR(22);
	DEFINE cDetDatosNumCheque INTEGER;
	...
	DEFINE cSumTotalRegistros INTEGER;						
	DEFINE cSumImporte MONEY(19,2);
	DEFINE cSumTotalRegTruncImg INTEGER;
	DEFINE iIdSumario INTEGER;
	DEFINE cGranSumCodOperacion CHAR(2);						
	DEFINE cGranSumNumOperaciones INTEGER; 
	DEFINE cGranSumFecha CHAR(8);
	DEFINE cGranSumImporteTotal MONEY(19,2);
	DEFINE cGranSumTotalRegTruncImg INTEGER;
	DEFINE iIdGranSumario INTEGER;
	DEFINE mImporte MONEY(14,2);
	DEFINE cStatus CHAR(2);	
	DEFINE iSecuencia INTEGER;
	DEFINE cMiBanco CHAR(3);
	DEFINE iContImgF INT;
	DEFINE cStrImg1 CHAR(1);
	DEFINE iContImgT INT;
	DEFINE cStrImg2 CHAR(1);
	DEFINE cDev CHAR(1);
	DEFINE dFecha DATE;
	...
	LET cEncNumBloque = '';
	LET cEncFechaPresenta = '';
	LET cEncTipoArchivo = '';
	LET cDetDatosCodOperacion = '';
	LET cDetDatosNumCuenta = '';
	LET cDetDatosNumCheque = 0;
	...
	LET cSumTotalRegistros = 0;						
	LET cSumImporte = 0.00;
	LET cSumTotalRegTruncImg = 0;
	LET iIdSumario = 0;
	LET cGranSumCodOperacion = '';						
	LET cGranSumNumOperaciones = 0; 
	LET cGranSumFecha = '';
	LET cGranSumImporteTotal = 0.00;
	LET cGranSumTotalRegTruncImg = 0;
	LET iIdGranSumario = 0;
	LET mImporte = 0.00;
	LET cStatus = '';	
	LET iSecuencia = 0;
	LET cMiBanco = '';
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_aplicacargaarchcecoban.out';
		--TRACE ON;

		IF pUsuario = '' OR pIdFuncion = '' OR pNombreArchivo = '' OR pDireccionMac = '' OR pCodOperacion = '' OR pFecha IS NULL THEN
			LET cCodRet = '00003';
			RETURN cCodRet;
		END IF;
		...
		-- ENCABEZADO
		SELECT {+INDEX ( bdicnweb:"informix".sw_cr_procesaencabezado_tmp  idx_sw_cr_procesaencabezado_tmp) }
			 tipo_registro,num_secuencia,num_banco,sentido_transfer,plaza_cecoban,servicio_tei,diames_transfer,num_bloque,fecha_presenta,tipo_archivo 
		INTO cEncTipoRegistro,cEncNumSecuencia,cEncNumBanco,cEncSentidoTransfer,cEncPlazaCecoban,cEncServicioTei,cEncDiaMesTransfer,
		cEncNumBloque,cEncFechaPresenta,cEncTipoArchivo
		FROM bdicnweb:"informix".sw_cr_procesaencabezado_tmp 
		WHERE usuario = pUsuario AND direccion_mac = pDireccionMac AND fecha_insert = pFecha AND id_encabezado = 1;
		
		IF DBINFO('sqlca.sqlerrd2') = 0 THEN
			LET cCodRet = '00494'; --NO EXISTEN ARCHIVOS POR PROCESAR (NO HAY DATOS QUE PROCESAR, VERIFIQUE)
			RETURN cCodRet;
		END IF;
		
		LET iHayDatos = 0;
		SELECT COUNT(nombrearchivo) INTO iHayDatos
		FROM bditef:"informix".cce_propios_enc
		WHERE nombrearchivo = UPPER(TRIM(pNombreArchivo))
		AND fecha_entrada = pFecha
		AND num_bloque = cEncNumBloque;

		IF NVL(iHayDatos,0) = 0 THEN
			INSERT INTO bditef:"informix".cce_propios_enc 
			VALUES (UPPER(TRIM(pNombreArchivo)),
					pFecha,
					cEncTipoRegistro,
					cEncNumSecuencia,
					cEncNumBanco,
		...
		FOREACH
			...
			SELECT datos_cod_operacion,datos_num_cuenta,datos_num_cheque,datos_motivo_devol,
			...
			rfc_cte,curp_cte,tipo_ctadep,cuenta_deposito,nombre_cte,cta_alertamiento,folio_seguro,
			status_causa,id_detalle
			INTO cDetDatosCodOperacion,cDetDatosNumCuenta,cDetDatosNumCheque,cDetDatosMotivoDevol,
			...
			cDetCuentaDeposito,cDetNombreCte,cDetCtaAlertamiento,cDetFolioSeguro,
			cDetStatusCausa,iIdDetalle
			...
			LET mImporte = SUBSTR(cDetImporte,1,(LENGTH(cDetImporte)-2))||"."||SUBSTR(cDetImporte,(LENGTH(cDetImporte)-1),2);
			
			IF TRIM(cDetDatosCodOperacion) = '46' AND SUBSTR(TRIM(cDetDatosMotivoDevol),1,3) <> 'ERR' THEN
				
				--Obtiene el status 05 (pagado) del detalle
				SELECT status INTO cStatus	
				FROM bditef:"informix".cce_propios_det WHERE c_cuenta = TRIM(cDetDatosNumCuenta) 
				AND c_cheque = cDetDatosNumCheque AND fecha_presini = cDetFechaInicial;
				
				--Cheque cargado, abona monto por dev cod 47
				IF NVL(cStatus,'') = '05' THEN
				
					EXECUTE PROCEDURE bditef:"informix".abono_cta(cEmpresa,TRIM(cDetDatosNumCuenta),cDetDatosNumCheque,mImporte,'01',pUsuario)
					INTO cCodRetSp, cDescCodRetSp;
					
					IF SUBSTR(cCodRetSp,1,3) <> '000' THEN
					
						UPDATE bdicnweb:"informix".sw_cr_procesadetalle_tmp 
						SET datos_motivo_devol = 'ERR: ' ||TRIM(cCodRetSp)||' '||TRIM(cDescCodRetSp)
						WHERE usuario = pUsuario AND direccion_mac = pDireccionMac AND fecha_insert = pFecha 
						AND datos_nombre_archivo = TRIM(pNombreArchivo) AND id_detalle = iIdDetalle;
						
					ELSE
						
						--Actualiza el status del original
						UPDATE bditef:"informix".cce_propios_det SET mot_devol = TRIM(cDetMotivoDevol),	
						status = '07', fecha_proceso = pFecha, cod_ret = TRIM(cCodRetSp)
						WHERE c_cuenta = TRIM(cDetDatosNumCuenta) AND c_cheque = cDetDatosNumCheque 
						AND cod_operacion = '40' AND fecha_presini = TRIM(cDetFechaInicial);
						...
						UPDATE bdicheq:"informix".sc_contch SET estado = 'N', fecha_alta = pFecha, importe = mImporte
						WHERE cuenta = TRIM(cDetDatosNumCuenta) AND numero = cDetDatosNumCheque;
						...
						--Reversado
						UPDATE bdicnweb:"informix".sw_cr_procesadetalle_tmp SET status_causa = '11'
						WHERE usuario = pUsuario AND direccion_mac = pDireccionMac AND fecha_insert = pFecha 
						AND datos_nombre_archivo = TRIM(pNombreArchivo) AND id_detalle = iIdDetalle;
			...
			IF TRIM(pCodOperacion) = '47' THEN
				...
				SELECT NVL(secuencia,0) INTO iSecuencia FROM bditef:"informix".cce_propios_det
				WHERE c_cuenta = TRIM(cDetDatosNumCuenta) AND c_cheque = cDetDatosNumCheque AND status = '01';
				
				UPDATE bditef:"informix".cce_propios_det SET status = '04', fecha_proceso = pFecha
				WHERE c_cuenta = TRIM(cDetDatosNumCuenta) AND c_cheque = cDetDatosNumCheque
				AND secuencia = iSecuencia AND cod_operacion = '40' AND fecha_presini = cDetFechaInicial;
				...
				--Procesado
				UPDATE bdicnweb:"informix".sw_cr_procesadetalle_tmp SET status_causa = '02'
				WHERE usuario = pUsuario AND direccion_mac = pDireccionMac AND fecha_insert = pFecha 
				AND datos_nombre_archivo = TRIM(pNombreArchivo) AND id_detalle = iIdDetalle;
			
			END IF;
			
			--Marcar los registros donde subieron las imagenes previamente	
			IF TRIM(pCodOperacion) = '40' AND cDetTruncado = '0' THEN
				
				-- BANCO PROPIETARIO
				SELECT valor INTO cMiBanco FROM bdinteg:"informix".si_param WHERE empresa = cEmpresa AND cod_param = '5';
				
				SELECT COUNT(numcuenta) INTO iContImgF FROM bditef:"informix".cce_cheques_img
				WHERE empresa = cEmpresa AND cvebanco = cMiBanco
				AND numcuenta = TRIM(cDetDatosNumCuenta) AND numcheque = cDetDatosNumCheque
				AND lado_ft = 'F' AND fechapresenta = pFecha AND imagen IS NOT NULL;
				
				IF NVL(iContImgF,0) <> 0 THEN
					LET cStrImg1 = '3';
				ELSE
					LET cStrImg1 = '';
				END IF;
				
				SELECT COUNT(numcuenta) INTO iContImgT FROM bditef:"informix".cce_cheques_img
				WHERE empresa = cEmpresa AND cvebanco = cMiBanco
				AND numcuenta = TRIM(cDetDatosNumCuenta) AND numcheque = cDetDatosNumCheque
				AND lado_ft = 'T' AND fechapresenta = pFecha AND imagen IS NOT NULL;
				
				IF NVL(iContImgT,0) <> 0 THEN
					LET cStrImg2 = '3';
				ELSE
					LET cStrImg2 = '';
				END IF;
			...
			--Sentencia preparada para hacer el INSERT
			SELECT status_causa INTO cDetStatusCausa FROM bdicnweb:"informix".sw_cr_procesadetalle_tmp 
			WHERE usuario = pUsuario AND direccion_mac = pDireccionMac AND fecha_insert = pFecha 
			AND datos_nombre_archivo = TRIM(pNombreArchivo) AND id_detalle = iIdDetalle;
			
			IF SUBSTR(TRIM(cDetDatosMotivoDevol),1,3) = 'ERR' THEN
				LET cDev = '1';
			ELSE
				LET cDev = '0';
			END IF;
			
			IF NVL(cDetMotivoDevol,'') <> '00' OR TRIM(pCodOperacion) = '47' THEN
				LET dFecha = pFecha;
			ELSE
				LET dFecha = '';
			END IF;
			
			INSERT INTO bditef:"informix".cce_propios_det VALUES (UPPER(TRIM(pNombreArchivo)),pFecha,
			(SELECT NVL(MAX(secuencia),0)+1 FROM bditef:"informix".cce_propios_det WHERE nombrearchivo = UPPER(TRIM(pNombreArchivo)) AND fecha_entrada = pFecha),
			TRIM(cDetDatosNumCuenta),
			cDetDatosNumCheque,
			mImporte,
			cDetTipoRegistro,cDetNumSecuencia,cDetCodOperacion,cDetFechaTrasnfer,cDetBancoCedente,cDetBancoLibrado,
			cDetImporte,cDetLoteEntrada,cDetSecEntrada,cDetLoteSalida,cDetSecSalida,cDetTransaccion,cDetPlazaCompensa,
			...
			cDetMotivoDevol,cDetFechaInicial,cDetPlazaIntercam,cDetRfcCte,cDetCurpCte,cDetTipoCuentaDep,
			cDetCuentaDeposito,cDetNombreCte,cDetCtaAlertamiento,cDetFolioSeguro,
			TRIM(cDetStatusCausa),'',TRIM(cCodRetSp),dFecha,cStrImg1,cStrImg2,'N','',0,0,0,0,cDev);
			...
			UPDATE bdicnweb:"informix".sw_cr_procesadetalle_tmp 
			SET bandera_color = 'V'	WHERE id_detalle = iIdDetalle;
		...
		FOREACH
			...
			SELECT num_secuencia,cod_operacion,total_registros,importe,total_regtruncimg,id_sumario
			INTO cSumNumSecuencia,cSumCodOperacion,cSumTotalRegistros,cSumImporte,cSumTotalRegTruncImg,iIdSumario
			FROM bdicnweb:"informix".sw_cr_procesasumario_tmp 
			WHERE usuario = pUsuario AND direccion_mac = pDireccionMac AND fecha_insert = pFecha
			
			IF cSumImporte IS NULL OR NVL(cSumImporte,0) = 0 THEN
				LET cSumImporte = 0.00;
			END IF;
			
			LET iHayDatos = 0;
			SELECT COUNT(nombrearchivo) INTO iHayDatos 
			FROM bditef:"informix".cce_propios_sum
            WHERE nombrearchivo = UPPER(TRIM(pNombreArchivo))
            AND fecha_entrada = pFecha
            AND cod_operacion = TRIM(pCodOperacion)
            AND num_secuencia = TRIM(cSumNumSecuencia);
		
			IF NVL(iHayDatos,0) = 0 THEN
				INSERT INTO bditef:"informix".cce_propios_sum 
				VALUES (UPPER(TRIM(pNombreArchivo)),
						pFecha,
						TRIM(cSumCodOperacion),
						TRIM(cSumNumSecuencia),
						cSumTotalRegistros,
						cSumImporte,
						cSumTotalRegTruncImg);
		...
		-- GRAN SUMARIO
		SELECT cod_operacion,num_operaciones,fecha,importe_total,total_regtruncimg,id_gransumario
		INTO cGranSumCodOperacion,cGranSumNumOperaciones,cGranSumFecha,cGranSumImporteTotal,cGranSumTotalRegTruncImg,iIdGranSumario
		FROM bdicnweb:"informix".sw_cr_procesagransumario_tmp 
		WHERE usuario = pUsuario AND direccion_mac = pDireccionMac AND fecha_insert = pFecha;
	
		LET iHayDatos = 0;
		SELECT COUNT(nombrearchivo) INTO iHayDatos 
		FROM bditef:"informix".cce_propios_gsum
		WHERE nombrearchivo = UPPER(TRIM(pNombreArchivo))
		AND fecha_entrada = pFecha;
	
		IF NVL(iHayDatos,0) = 0 THEN
			INSERT INTO bditef:"informix".cce_propios_gsum 
			VALUES (UPPER(TRIM(pNombreArchivo)),
					pFecha,
					TRIM(cGranSumCodOperacion),
					cGranSumNumOperaciones,
					cGranSumImporteTotal,
					cGranSumTotalRegTruncImg,
					cGranSumFecha);
		...
		iIdDetalle,
		'','','0','03','M',
		'archivo de DATOS importado exitosamente',
		pUsuario,
		pFecha);
...
'DESCRIPCION: SPL encargado de hacer la carga manual del archivo de datos cecoban.',
'BD: bdicnweb';
