CREATE PROCEDURE "informix".sp_guardalog_catrh(pUsuario CHAR (8),pFecha_mod DATETIME YEAR TO SECOND,pIdcatalogo INTEGER, 
												pIdaccion INTEGER,pIdorigen INTEGER,pIddestino INTEGER,pDescripcion VARCHAR (254))
...
BEGIN
	...
	--SET DEBUG FILE TO "/respaldosbd/josue/sp_guardalog_catrh.out";
	--TRACE ON;
	
	IF (NVL(pUsuario,'')= '') OR (NVL(pFecha_mod,'') = '') OR (NVL(pIdcatalogo,0) = 0) OR (NVL(pIdaccion,0) = 0) OR (NVL(pDescripcion,'') = '') THEN
		LET cCod_ret	= '00001';
		LET cMensaje	= 'Par'||CHR(225) ||'metros incompletos';
	ELSE
		IF NVL(pIdaccion,0) NOT IN (1,2,3) THEN
			LET cCod_ret	= '00001';
			LET cMensaje	= 'Par'||CHR(225) ||'metros incompletos';
		...
		ELSE
				IF NVL(pIdaccion,0) = 3 AND ((NVL(pIddestino,0) = 0)) THEN
					LET cCod_ret	= '00001';
					LET cMensaje	= 'Par'||CHR(225) ||'metros incompletos';
				ELIF NVL(pIdaccion,0) IN(1,3) AND ((NVL(pIdorigen,0) = 0)) THEN
					LET cCod_ret	= '00001';
					LET cMensaje	= 'Par'||CHR(225) ||'metros incompletos';
				ELSE					
					IF NVL(pIdaccion,0) = 2 THEN	
						IF NVL(pIdcatalogo,0) = 1 THEN
							SELECT MAX(id_region) INTO iIdOrigen FROM si_catregion_rh;
						ELIF NVL(pIdcatalogo,0) = 2 THEN
							SELECT MAX(id_gerencia) INTO iIdOrigen FROM si_catgcb_rh;
						ELIF NVL(pIdcatalogo,0) = 3 THEN
							SELECT MAX(id_status) INTO iIdOrigen FROM si_catstatus_rh;
						/*JMMO*/	
						ELIF NVL(pIdcatalogo,0) = 4 THEN
							SELECT MAX(id_coordinacion) INTO iIdOrigen FROM si_catczb_rh;
						/*JMMO*/	
						END IF;			
					
						LET pDescripcion = pDescripcion ||" "|| iIdOrigen;
					END IF;
					
					INSERT INTO"informix".si_logcatalogos_rh (usuariomod,fechahora_mov,idcatalogo,idaccion,descripcion )
					VALUES(pUsuario,pFecha_mod,pIdcatalogo,pIdaccion,pDescripcion);	
				END IF;
		END IF;	
	END IF;	
	RETURN cCod_ret,NVL(cMensaje,'');
...
'AUTOR:94912599', 
'FECHA:26/10/2015',
'DESCRIPCIÓN:  Procedimiento  con el  que  se  guardará el  registro  de  las',  
'modificaciones  hechas Sobre  los  catálogos  GCB,  Región  y  Status.',
