CREATE PROCEDURE "informix".sp_actualiza_control_cobranza_automatica(
							pNumeroCliente CHAR(20), 							--Numero de cuenta de captacion a validar 
							pCuentaCaptacion CHAR(20), 							--Numero de sucursal de la operacion
							pEstatus INTEGER,									--Estatus del registro para la tabla de control
							pMontoPendientexPagar MONEY(14,2),					--Monto pendiente por pagar
							pMontoRetenido MONEY(14,2),                         --Monto retenido
							pMontoPendienteARetener MONEY(14,2))                --Monto pendiente por retener
RETURNING 	CHAR(5) as cCodRet, 
	...
	--Variables para la actualizacion en la tabla			
	DEFINE dFechaModificacion	DATETIME YEAR TO MINUTE;
	
	--Declaracion de archivo de debuggeo
	--SET DEBUG FILE TO "/c90314833/sp_actualiza_control_cobranza_automatica.out";
    --TRACE ON;
	
	---Inicializacion de variables	
	--Variables de retorno del SP 
	LET cCodRet				= '00000';
	LET cMensajeRet			= 'Registro actualizado correctamente';
	--Variables para el manejo de excepciones
	LET iSQLError           = 0;
	LET iISAMError          = 0;
	--Variables para la insercion en la tabla			
	LET dFechaModificacion	= CURRENT;
	...
	BEGIN
		...
		IF ( pCuentaCaptacion = '' OR pCuentaCaptacion IS NULL) OR
		(pNumeroCliente = '' OR pNumeroCliente IS NULL) OR
		(pMontoPendientexPagar = '' OR pMontoPendientexPagar IS NULL) OR
		(pEstatus = '' OR pEstatus IS NULL) OR
		(pMontoPendienteARetener = '' OR pMontoPendienteARetener IS NULL) OR
		(pMontoRetenido = '' OR pMontoRetenido IS NULL) THEN
		...
		IF pMontoPendientexPagar < 0 THEN		
			
			LET cCodRet = '00002';
			LET cMensajeRet = 'El monto pendiente por pagar no puede ser menor a 0';			
			RETURN cCodRet, cMensajeRet;
			
		END IF;
		
		IF pMontoPendienteARetener < 0 THEN		
			
			LET cCodRet = '00003';
			LET cMensajeRet = 'El monto pendiente por retener no puede ser menor a 0';			
			RETURN cCodRet, cMensajeRet;
			
		END IF;
		
		IF pMontoRetenido < 0 THEN		
			
			LET cCodRet = '00004';
			LET cMensajeRet = 'El monto retenido no puede ser menor a 0';			
			RETURN cCodRet, cMensajeRet;
			
		END IF;
		
		IF pEstatus NOT IN (1,2,3) THEN
			LET cCodRet = '00005';
			LET cMensajeRet = 'El estatus no puede ser diferente de 1, 2 o 3';			
			RETURN cCodRet, cMensajeRet;
		END IF;
		
		
		--Validacion de registro inexistente en la tabla de control
		IF(SELECT COUNT(*) FROM sc_control_cobranza_automatica WHERE numero_cliente = pNumeroCliente AND cuenta_captacion = pCuentaCaptacion) = 0 THEN
			
			LET cCodRet = '00006';
			LET cMensajeRet = 'No existe un registro en la tabla de control para el cliente y numero de cuenta proporcionados';			
			RETURN cCodRet, cMensajeRet;
			
		END IF;		
	
		--Actualizacion de la tabla con los datos proporcionados
		UPDATE sc_control_cobranza_automatica SET
		estatus = pEstatus,
		monto_pendiente_por_pagar = pMontoPendientexPagar,
		monto_retenido = pMontoRetenido,
		pendiente_a_retener = pMontoPendienteARetener,
		fecha_modificacion = dFechaModificacion
		WHERE numero_cliente = pNumeroCliente 
		AND cuenta_captacion = pCuentaCaptacion;
		
		--Retorno final del SP
		RETURN cCodRet, cMensajeRet; 
...
'FECHA :        01-10-2025',
'DESCRIPCION :  Este SPL tiene la finalidad de actualizar la tabla de sc_control_cobranza_automatica para los creditos que cuenten con una exigencia de pago',
