CREATE PROCEDURE "informix".sp_altaclientehuellatitular_web(p_empresa CHAR(3), p_NumCte VARCHAR(20), p_FlagAdic CHAR(1))
RETURNING
	CHAR(5), ---cod_ret
	INTEGER, ---Seguridad
	SMALLINT, ---Secuencia
...
BEGIN
	...
	--SET DEBUG FILE TO "/tmp/sp_AltaClienteHuellaTitular.out";
    --TRACE ON;
	...
	IF (p_Empresa IS NULL OR p_Empresa = '')  OR  (p_NumCte IS NULL OR p_NumCte = '') THEN
		LET v_cod_ret = '00001';
	ELSE
		IF EXISTS(SELECT 1 FROM bdinteg:"informix".si_adiccoppel WHERE empresa = p_Empresa AND sucursal = sucursal AND numcte = p_NumCte AND secuencia = 1) THEN
			SELECT numctecoppel
			INTO v_Cliente_Coppel
			FROM bdinteg:"informix".si_adiccoppel
			WHERE empresa = p_Empresa AND sucursal = sucursal 
			AND numcte = p_NumCte
			AND secuencia = 1;
			...
			SELECT secuencia, estado, dmapa, imapa, usuario, sucursal, YEAR(fecha_alta) || LPAD(MONTH(fecha_alta),2,'0') || LPAD(DAY(fecha_alta),2,'0')
					, usuario_camb, YEAR(fecha_camb) || LPAD(MONTH(fecha_camb),2,'0') || LPAD(DAY(fecha_camb),2,'0'), 2
			INTO  v_Secuencia, v_Estado, v_DMapa, v_IMapa, v_Usuario, v_Sucursal, v_Fecha_Alta, v_Usuario_Camb, v_Fecha_Camb, v_TipoSensor
			FROM bdinteg:"informix".si_cte_huella
			WHERE  numcte = p_NumCte
			AND secuencia IN (SELECT MAX(secuencia) FROM bdinteg:"informix".si_cte_huella WHERE numcte = p_NumCte);
...
END;
--##############################################################################
--## Procedimiento   : sp_AltaClienteHuellaTitular
