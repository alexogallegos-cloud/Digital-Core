CREATE PROCEDURE "informix".spei_devcodi(pchridmjc CHAR(20),
...
RETURNING CHAR(5), CHAR(30);
    ...
    --// spei_devcodi
    --// Version              1.0.0
    --// Objetivo:            Registro de una devoluciÃ³n CODI (operaciÃ³n interbancaria)
    ...
    --//          pchrbancobenf    : Clave CESIF del banco beneficiario de la orden.
    --//          pdecImporte      : Importe de la orden que se desea enviar.
    --//          pmnyIVA          : Importe del iva.
    --//          pvchrConceptoPago: Instrucciones o referencia del pago para el cliente o banco beneficiario.
    --//          pReferenciaBe       : Dato numerico que servira de referencia al beneficiario para indicar el concepto del pago.
	...
	--// Campos para el cargo y tblpago
	DEFINE pEmpresa  CHAR(3);
	DEFINE pchrSucursal CHAR(4);
	DEFINE pchrUsuario CHAR(8);
	DEFINE pchrTransuc  CHAR(4);
	DEFINE pmnyComision MONEY(14,2);
	DEFINE pmnyIvaComis MONEY(14,2);
	DEFINE pmnyIVA MONEY(14,2);
	DEFINE pvchrRefCobranza1 VARCHAR(40);
	DEFINE pindbenef CHAR(3);
	DEFINE ppagocomision INTEGER;
	DEFINE pcomision MONEY(14,2);
	DEFINE pnumseriecert CHAR(20);
	DEFINE pfolioplataforma CHAR(20);
	DEFINE vintexistedevolucion INTEGER;
	DEFINE vintexistedevolucioonhist INTEGER;
	DEFINE vLogCtaord INTEGER;
	DEFINE vLogCtabenf INTEGER;
	DEFINE vchrnum_cteord char(20);
	DEFINE vchrnum_ctebenf char(20);
    DEFINE vchrid CHAR(2);
	DEFINE vchridhist VARCHAR(2);
	DEFINE vdecauxmonto DECIMAL(12,2);
	DEFINE vdecauxmontohist DECIMAL(12,2);
    DEFINE vchrnombrebenaux CHAR(40);
	DEFINE vchrnombrebenauxhis CHAR(40);
	DEFINE vchrauxfechavalor DATETIME YEAR TO FRACTION(3);
	DEFINE vchrauxfechavalorhis DATETIME YEAR TO FRACTION(3); 
	DEFINE vauxserial INTEGER;
	DEFINE vauxhistserial INTEGER;
    DEFINE vchrNumCtaDestinoA CHAR(18);
	DEFINE vchrpNumCtaOrigenC CHAR(18);

	LET pchrSucursal = '5011';
    LET vtransaccion    = 0;
    LET cVarDataErr     = '';
	...
	LET pchrUsuario = 'transBPI';
	LET pchrTransuc = '0447';
	LET pmnyComision  = 0;
	LET pmnyIvaComis = 0;
	LET pmnyIVA = 0;
	LEt pvchrRefCobranza1 = '';
	LET pindbenef = '02';
	LET ppagocomision = 0;
	LET pcomision = 0 ;
	LET pnumseriecert  = pchraliasben;
	LET pfolioplataforma = pchridmjc;
	--LET pvchrRFCOrd= 'BEAP94040647'; ---PEDRI que se agregue a datos de entrada
	LET vintexistedevolucion = 0;
	LET vintexistedevolucioonhist = 0;
	LET vLogCtaord = LENGTH(pvchrCuentaOrd);
	LET vLogCtabenf = LENGTH(pvchrCtaBenef);
	LET vchrnum_cteord  = '';
	LET vchrnum_ctebenf = '';
	LET vtimestamp = '';
	LET vchrid = '';
	LET vchridhist = '';
	LET vdecauxmonto = 0;
	LET vdecauxmontohist = 0;
	LET vchrnombrebenaux='';
	LET vchrauxfechavalor='';
	LET vchrNumCtaDestinoA = pvchrCtaBenef;
	LET vchrpNumCtaOrigenC = pvchrCuentaOrd;
    LET intBancoOrd = '40137';
	
    BEGIN
    
    ON EXCEPTION SET vintcodret, vintcodret2, vchrcodret3
        IF vintcodret <> 0 THEN
		    SET DEBUG FILE TO "/resplogifx/conciliachq/spei/spei_devcodi.err";
            TRACE ON;
    ...
    -- SET DEBUG FILE TO '/informix/ash/CODI/spei_devcodi.out';
   --SET DEBUG FILE TO '/resplogifx/conciliachq/spei/spei_devcodi.out';
   --TRACE ON;
    ...
    LET vchrcodret = '000';
    LET vchrTarjeta = '';
	LET wimporte = pdecImporte;
	LET wrefnumerica = pReferenciaBe;
    
	--// Obtener RFC Ordenante 
	IF vLogCtaord = 11 THEN
		SELECT num_cte INTO vchrnum_cteord
		  FROM bdicheq:sc_maechq
		WHERE cuenta = pvchrCuentaOrd;
		LET pintTipoCtaOrd = '40';
	ELSE
		IF vLogCtaord = 18 THEN
			SELECT num_cte,cuenta,cuenta_clabe INTO vchrnum_cteord,pvchrCuentaOrd,vchrpNumCtaOrigenC
			  FROM bdicheq:sc_maechq
			WHERE cuenta_clabe = pvchrCuentaOrd;
			LET pintTipoCtaOrd = '40';
		END IF;	
	END IF;
	
	SELECT rfc  INTO pvchrRFCOrd
	  FROM bdinteg:si_cliente
	WHERE numcte  = vchrnum_cteord; 
	
	IF vLogCtaord  = 16 THEN
		LET pintTipoCtaOrd = '3';
	ELSE
		IF vLogCtaord = 10 THEN
			LET pintTipoCtaOrd = '10';
		END IF;
	END IF;
    ...
    IF pvchrNombreOrd IS NULL OR pvchrNombreOrd = '' OR
       ...
       pvchrConceptoPago IS NULL OR pvchrConceptoPago = '' THEN
        ...
        EXECUTE PROCEDURE spei_recerrorescodi(vchrcodret, 'D','o',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                              pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
                                              pvchrCtaBenef,pvchrNombreBenef,pchraliasord,pdigidord, intBancoOrd,pintTipoCtaOrd,
                                              vchrpNumCtaOrigenC,pvchrNombreOrd,pnumseriecert)
    ...
    IF pvchrNombreBenef IS NULL OR pvchrNombreBenef = '' OR
       ...
       pvchrCtaBenef IS NULL OR pvchrCtaBenef = ''  THEN
        ...
        LET vtimestamp  = dbinfo('utc_current') * 1000;
		LET wtimestamp = vtimestamp;
        EXECUTE PROCEDURE spei_recerrorescodi(vchrcodret, 'D','b',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                              pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
                                              pvchrCtaBenef,pvchrNombreBenef,pchraliasord,pdigidord, intBancoOrd,pintTipoCtaOrd,
                                              vchrpNumCtaOrigenC,pvchrNombreOrd,pnumseriecert)
        INTO wvchrcodretcodi;
    ...
    IF pintTipoCtaOrd = 3 THEN
        ...
        ELSE
			...
			LET vchrpNumCtaOrigenC=vchrTarjeta;
			IF SUBSTR(pvchrCuentaOrd, 1, 2) <> '80' THEN
                SELECT cuenta, sucursal
                  INTO pvchrCuentaOrd, vsuc_cta
                  FROM bdicheq:sc_maechq
                 WHERE cuenta = pvchrCuentaOrd
    ...
    IF pintTipoCtaOrd = 40 THEN
        IF SUBSTR(pvchrCuentaOrd, 1, 2) <> '80' THEN
            SELECT cuenta_clabe, sucursal
              INTO vchrCtaOrdClabe, vsuc_cta
              FROM bdicheq:sc_maechq
             WHERE cuenta = pvchrCuentaOrd
               AND status_cta NOT IN("2", "6");
        END IF;
        ...
        IF vchrCtaOrdClabe IS NOT NULL THEN
            ...
            ELIF LENGTH(vchrCtaOrdClabe) > 18 THEN
                LET vchrcodret = '016'; -- La cuenta debe ser de 18 digitos.
        ...
        ELIF LENGTH(pvchrCuentaOrd) = 11 THEN
            LET vchrcodret = '017'; -- La cuenta Ord. no permite 11 digitos.
    ...
    IF pintTipoCtaOrd = 10 THEN
        IF LENGTH(pvchrCuentaOrd) = 10 THEN
            ...
            LET vchrpNumCtaOrigenC = vchrTelefono;
            SELECT cuenta
              INTO pvchrCuentaOrd
        ...
        IF vchrTelefono is null OR vchrTelefono = '' OR pvchrCuentaOrd is null OR pvchrCuentaOrd = '' THEN
             LET vchrcodret = '018'; -- // La cuenta Ord. no existe.
    ...
    from tbl_histstsprocodi 
    WHERE vchridmjc = pchridmjc AND vchridtpa IN ('1','21','22','23','24','31','32');  
	
	--//Valida que se halla realizado un abono primero
    SELECT FIRST 1 vchridtpa,mnyimporte,vchrnomord,fcha_oper INTO vchrid,vdecauxmonto,vchrnombrebenaux,vchrauxfechavalor
	FROM tbl_stsprocodi
	WHERE vchridmjc = pchridmjc AND vchridtpa IN  ('1','21','22','23','24','31','32') AND num_serial = vauxserial;


	SELECT FIRST 1 vchridtpa,mnyimporte,vchrnomord,fcha_oper INTO vchridhist,vdecauxmontohist,vchrnombrebenauxhis,vchrauxfechavalorhis
	FROM tbl_histstsprocodi  
	...
	IF (vchrid IS NULL) AND (vchridhist IS NULL) THEN
        LET vchrcodret = '0445';
        LET cVarDataErr = 'No se puede devolver porque no se aplico el abono de la operacion original';
        LET vtimestamp  = dbinfo('utc_current') * 1000;
		LET wtimestamp = vtimestamp;
        EXECUTE PROCEDURE spei_recerrorescodi(vchrcodret, 'D','o',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                              pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
	...
	IF vchrnombrebenaux = '' OR vchrnombrebenaux IS NULL THEN
		LET pvchrNombreBenef = TRIM(vchrnombrebenauxhis);
		LET vchrauxfechavalor = date(vchrauxfechavalorhis);
	ELSE
		LET pvchrNombreBenef = TRIM(vchrnombrebenaux);
		LET vchrauxfechavalor = date(vchrauxfechavalor);
	END IF;
	

	--// Valida el monto original de la operaciÃ³n
	IF vdecauxmonto > wimporte AND  vdecauxmontohist >wimporte THEN
        LET vchrcodret = '0446';
        LET cVarDataErr = 'El monto no debe ser mayor al de la operacion original';
        LET vtimestamp  = dbinfo('utc_current') * 1000;
		LET wtimestamp = vtimestamp;
        EXECUTE PROCEDURE spei_recerrorescodi(vchrcodret, 'D','o',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                              pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
    ...
    IF vchrparametro IS NULL THEN
		LET intBancoOrd = 0; 
        LET vchrcodret = '011';
        LET cVarDataErr = 'Banco ord no encotrado ';
        LET vtimestamp  = dbinfo('utc_current') * 1000;
		LET wtimestamp = vtimestamp;
        EXECUTE PROCEDURE spei_recerrorescodi(vchrcodret, 'D','o',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                              pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
    ...
    --//Validar si es extemporanea solo para pruebas
	IF vchrauxFechaValor < date(vfecha_hoy) THEN 
	   LET pchartipopago = '18';
	END IF;
    ...
    IF pchrSucursal = '5003' THEN
        ...
        IF TRIM(vchrcodret) <> '000' THEN
            LET vtimestamp  = dbinfo('utc_current') * 1000;
			LET wtimestamp = vtimestamp;
            EXECUTE PROCEDURE spei_recerrorescodi(vchrcodret, 'D','i',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                                  pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
                                                  pvchrCtaBenef,pvchrNombreBenef,pchraliasord,pdigidord, intBancoOrd,pintTipoCtaOrd,
                                                  vchrpNumCtaOrigenC,pvchrNombreOrd,pnumseriecert)
            INTO wvchrcodretcodi;
    ...
    EXECUTE PROCEDURE spei_validaoperacion(pvchrCuentaOrd, pdecImporte, pchrSucursal)
    ...
    IF pintTipoCtaBenef = 40 THEN
		...
		IF LENGTH(pvchrCtaBenef) >= 16 AND LENGTH(pvchrCtaBenef) < 18 THEN
            ...
            IF vdigitoverifica = 0 THEN
                LET vchrcodret = '021'; -- La Cuenta Clabe del Benefciario es Invalida
                LET cVarDataErr = 'La Cuenta Clabe del Benefciario es Invalida ';
                LET vtimestamp  = dbinfo('utc_current') * 1000;
				LET wtimestamp = vtimestamp;
                EXECUTE PROCEDURE spei_recerrorescodi(vchrcodret, 'D','b',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                                  pchrCveRastreo,wrefnumerica,pchraliasord,pdigidord, intBancoOrd,pintTipoCtaOrd,
                                                  pvchrCuentaOrd,pvchrNombreOrd,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
        ...
        ELIF 
		*/
		---QUITAR DESPUES DE PRUEBAS recordar que l if de abajo no iba lo de length LENGTH(pvchrCtaBenef) <> 18 THEN iba despues de elif
		/*-- NO SE NECESITA VALIDAR LA CTA BENEFICIARIA
		IF 	LENGTH(pvchrCtaBenef) <> 18 THEN
            LET vchrcodret = '016'; -- La cuenta Benef debe ser de 18 digitos.
   ...
   { Para pruebas CODI no se valida la cuenta clabe 
   -- // Compara la clave del banco en la clabe del beneficiario, si es por cuenta CLABE.
    IF pintTipoCtaBenef = 40 THEN
        IF SUBSTR(pvchrCtaBenef, 1, 3) <> SUBSTRING(TRIM(pchrbancobenf::VARCHAR(10)) FROM LENGTH(TRIM(pchrbancobenf::VARCHAR(10))) -2) THEN
            LET vchrcodret = '025';
			...
			LET pdecImporte = pdecImporte;
			LET wimporte = wimporte;
			LET vtimestamp = vtimestamp;
			LET pchrCveRastreo = pchrCveRastreo;
			LET pReferenciaBe = pReferenciaBe;
			LET pchraliasord = pchraliasord;
			...
			LET pchraliasben = pchraliasben;
			LET pdigidben = pdigidben;
			LET pchrbancobenf = pchrbancobenf;
	...
	IF pchartipopago = '18' THEN 
	    --//Se obtiene la fecha de la operacion original	
        SELECT  fech_val INTO vchrFechaValor
	      FROM bdicheq:sc_movdia
        WHERE referencia  = pchrCveRastreo AND  transacc = '0446';
		
		IF vchrFechaValor = '' OR vchrFechaValor IS NULL THEN
		   SELECT  fech_val INTO vchrFechaValor
    ...
    SELECT cvecesif
      INTO vintCveCesif
      FROM tblbanco
     WHERE cvecesif = pchrbancobenf
	   AND chredobco = 'A'
       AND intindice >= 0;
       
    IF vintCveCesif IS NULL THEN
        LET vchrcodret = '028';
        LET cVarDataErr = 'Banco benf no Valido';
        LET vtimestamp  = dbinfo('utc_current') * 1000;
		LET wtimestamp = vtimestamp;
        EXECUTE PROCEDURE spei_recerrorescodi(vchrcodret, 'D','b',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                              pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
    ...
    ELSE
        IF vintCveCesif = intBancoOrd THEN
			LET vchrcodret = '029';
			LET cVarDataErr = 'La clave de banco ord y ben deben ser diferentes';
            LET vtimestamp  = dbinfo('utc_current') * 1000;
			LET wtimestamp = vtimestamp;
            EXECUTE PROCEDURE spei_recerrorescodi(vchrcodret, 'D','b',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                                  pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
    ...
    LET vsintLongCveRast = LENGTH(pchrCveRastreo);
    
    LET intpktblpago = vintfolioop;
    
    IF pReferenciaBe IS NULL or pReferenciaBe = 0 THEN
        LET pReferenciaBe = SUBSTR(LPAD(intpktblpago, 12, '0'), -7); 
    END IF;
	
	--//Validar que transaccion pertenece a esta devolucion
  	IF pchartipopago in('19', '20', '21', '22') THEN
	   LET vchrTranscargo = '0447';
	ELIF pchartipopago = ('17') THEN
	   LET vchrTranscargo = '0543'; --- Por lo mientras sera esta transacciÃ³n hasta que las den de alta pruebas
	ELIF  pchartipopago = ('18') THEN
	   LET vchrTranscargo = '0543'; --- Por lo mientras sera esta transacciÃ³n hasta que las den de alta pruebas
	END IF;
	
	--//Validar si no se ha realizado una devoluciÃ³n
	SELECT COUNT(*) INTO vintexistedevolucion
	FROM tblpago 
	WHERE  vchrclaverastreo = pchrCveRastreo
	AND intcvetipopago in('17','18');
	
	SELECT COUNT(*) INTO vintexistedevolucioonhist
	FROM tblhistpago 
	WHERE  vchrclaverastreo = pchrCveRastreo
	AND intcvetipopago in('17','18');
	
	IF (vintexistedevolucion > 0 )OR (vintexistedevolucioonhist > 0 ) THEN
		LET vchrcodret  = '444'; --Ya se aplico una devolucion
		IF vtransaccion = 1 then
                ROLLBACK WORK;
                BEGIN WORK;
            ELSE
                ROLLBACK WORK;
        END IF;
        LET cVarDataErr = 'Ya se aplico una devolucion';
        LET vtimestamp  = dbinfo('utc_current') * 1000;
		LET wtimestamp = vtimestamp;
        EXECUTE PROCEDURE spei_recerrorescodi(vchrcodret, 'D','o',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                              pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
    ...
    IF SUBSTR(pvchrCuentaOrd, 1, 2) <> '80' THEN
        EXECUTE PROCEDURE bdicheq:cargo_ref(pEmpresa,pchrSucursal,pchrUsuario,vchrTranscargo,pchrTransuc,pchrFolioSuc,pvchrCuentaOrd,0,pdecImporte,"01",pchrCveRastreo,vchrTarjeta,pchrUsuario)
        ...
        IF trim(vchrcodret) <> '000' THEN
            ...
            EXECUTE PROCEDURE spei_recerrorescodi(vcodret, 'D','o',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                                  pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
                                                  pvchrCtaBenef,pvchrNombreBenef,pchraliasord,pdigidord, intBancoOrd,pintTipoCtaOrd,
                                                  vchrpNumCtaOrigenC,pvchrNombreOrd,pnumseriecert)
            INTO wvchrcodretcodi;
        ...
        ELSE
            ...
            VALUES(pchrFolioSuc, pchrSucursal, pchrUsuario, pdtfechacaptura, vchrTranscargo, pEmpresa, pvchrCuentaOrd, pdecImporte, pchrCveRastreo);
        END IF;
    END IF;
    ...
    IF pmnyComision > 0 THEN
        IF SUBSTR(pvchrCuentaOrd, 1, 2) <> '80' THEN
            EXECUTE PROCEDURE bdicheq:cargo_ref(pEmpresa,pchrSucursal,pchrUsuario,vchrComis,pchrTransuc,pchrFolioSuc,pvchrCuentaOrd,0,pmnyComision,"01",pchrCveRastreo,vchrTarjeta,pchrUsuario)
            INTO vchrcodret, vchrtranret, dteFechacargo, vmnySdoDisp, vmnyMontoRet;
            ...
            ELSE
                ...
                INSERT INTO tbldetranpago(folio_suc,sucursal, usuario, fech_alt, transacc, empresa, cuenta, monto_tot, clave_rastreo)
                VALUES(pchrFolioSuc, pchrSucursal, pchrUsuario, pdtfechacaptura, vchrComis, pEmpresa, pvchrCuentaOrd, pmnyComision, pchrCveRastreo);
            END IF;
        END IF;
    END IF;
	...
	IF CURRENT HOUR TO fraction > '17:58:00' AND CURRENT HOUR TO fraction < '18:05:00' THEN
        LET vchrestatusenvio ='E'; /* CAMBIAR A 'E' esto solo es paraq pruebas PREGUNTAR A ALEJANDRO SI VA X  O  E ??*/
	...
	LET pchrRFCBenef = REPLACE(pchrRFCBenef, 'Ã', 'N');
	LET pchrRFCBenef = REPLACE(pchrRFCBenef, 'Ã½', 'X');
	LET pchrRFCBenef = REPLACE(pchrRFCBenef, 'Ã', 'X');
	LET pchrRFCBenef = REPLACE(pchrRFCBenef, 'Ã', 'A');
...
*/ --COMENTADO
    ...
    LET wmnyImporte = pdecImporte;
    LET wmnyIVA = pmnyIVA;

	LET wchrcadena_01 = '||'||vintCveCesif||'|'||'Bancoppel'||'|'||vchrFechaValor2||'|'||'|'||TRIM(pchrCveRastreo)||'|'||intBancoOrd||'|';
	...
	LET wchrcadena_03 = pintTipoCtaBenef||'|'||TRIM(pvchrNombreBenef)||'|'||TRIM(pvchrCtaBenef)||'|'||TRIM(pchrRFCBenef)||'||||||'||TRIM(pvchrConceptoPago)||'||||0|'||TRIM(pvchrRefCobranza1)||'|';
	LET wchrcadena_04 = pReferenciaBe||'||'||TRIM(chrtopologia)||'|'||''||TRIM(wmedioent)||'|'||'|'||'0'||'|'||wmnyIVA;
	
	IF pchartipopago in('17','18') THEN
       ...
       LET wchrcadena_05 = '|'||trim(wnumcelord)||'|'||wdigidord||'|'||trim(wnumcelben)||'|'||wdigidben||'|'||wpagocomision||'|'||wcomision||'|'||TRIM(wfolioplataforma)||'|'||TRIM(wnumseriecert)||'|'||TRIM(wfechalimpago)||'|'||wcomision||'||';
	...
	EXECUTE function bdispei:syn_sign(TRIM(wchrcadena_00), wvchrfirma, 21) /*CAMBIAR CUANDO SE  VAYA A PRODUCCION A 21 , pruebas*/
	INTO ret;
	...
	IF ret = 0 THEN 
		...
		VALUES (intpktblpago, pdecImporte, vchrestatusenvio, pvchrNombreOrd, pvchrCuentaOrd, pvchrRFCOrd, pintTipoCtaOrd, pvchrNombreBenef,
				pintTipoCtaBenef, pvchrCtaBenef, pchrRFCBenef, pmnyIVA, pReferenciaBe, pvchrConceptoPago, pvchrRefCobranza1,
				pchrUsuario, pchartipopago, 'E', vchrFechaValor, pchrCveRastreo, pchrFolioSuc, pdtfechacaptura, 
		...
		LET vtimestamp  = dbinfo('utc_current') * 1000;
        LET wtimestamp    = vtimestamp;
		EXECUTE PROCEDURE spei_recerrorescodi('61', 'D','o',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                                  pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
                                                  pvchrCtaBenef,pvchrNombreBenef,pchraliasord,pdigidord, intBancoOrd,pintTipoCtaOrd,
                                                  pvchrCuentaOrd,pvchrNombreOrd,pnumseriecert)
		INTO wvchrcodretcodi;

		--se agrega para firma de aviso 61_2, se realizara validacion.

		EXECUTE PROCEDURE spei_recerrorescodi('61_2', 'D','o',pchridmjc,pchrfchmjc,pvchrConceptoPago,wimporte,wtimestamp,
                                                  pchrCveRastreo,wrefnumerica,pchraliasben,pdigidben,pchrbancobenf,pintTipoCtaBenef,
                                                  pvchrCtaBenef,pvchrNombreBenef,pchraliasord,pdigidord, intBancoOrd,pintTipoCtaOrd,
                                                  pvchrCuentaOrd,pvchrNombreOrd,pnumseriecert)
					  INTO wvchrcodretcodi;
...
'FECHA MODIFICACION: 27/01/2026 LIBERACION',
'DESCRIPCION: SE AGREGA GENERACION DE FIRMA Y AVISO TIPO 61 2 Y SE CORRIGE EL REPLACE PARA CARACTERES ESPECIALES',
'DB: BDISPEI';

CREATE PROCEDURE "informix".spei_recerrorescodi (pvchrcodretc    CHAR(5),       -- Codigo de error   
                                                 pcVarDataErr    CHAR(100),     -- Descripcion del error
                                                 pchrparti       CHAR(2),       -- Participante 'o' Ã³  'b' ordenante/beneficiario   
                                                 ...
                                                 pvchrrefnum     CHAR(7),       -- Referencia NumÃÆÃÂ©rica  del beneficiario                                                
                                                 pvchrcelord     CHAR(10),      -- NÃÆÃÂºmero de celular  ordenante 
                                                 pvchrdiveord    CHAR(9),       -- Digito verificador 
                                                 ...
                                                 pvchrcelbenf    CHAR(20),      -- NÃÆÃÂºmero celular beneficiario
                                                 pvchrdivebenf   CHAR(9),       -- Digito verificador beneficiario
...
RETURNING char(5);
	...
	ON EXCEPTION SET sql_err, isam_err, desc_err
        SET DEBUG FILE TO "/resplogifx/conciliachq/spei/spei_recerrorescodi.err";
        TRACE ON;
		IF sql_err <> 0 THEN
            LET vcodret  = sql_err;
	...
	--- SET DEBUG FILE TO '/resplogifx/conciliachq/spei/spei_recerrorescodi.out';
    --- TRACE ON;
    ...
    LET vchrfchfinpro = CURRENT;
		...
		--//Se valida si no es una devoluciÃ³n de operaciÃ³n acreditada
	IF (pcVarDataErr  <>  'D') THEN
		...
		--//Errores por validaciones del comprador
		IF pvchrcodretc IN('001','004','010','011','012','014','015','016') AND pchrparti = 'o'  THEN
			LET vchridtpa = '2';
			LET vchrcode = '3';
		END IF;
		
		IF pvchrcodretc IN('017','018','022','023','024','026') AND pchrparti = 'o'  THEN
			LET vchridtpa = '2';
			LET vchrcode = '3';
		END IF;
		
		IF pvchrcodretc IN('027','030','036','037','17','019','020') AND pchrparti = 'o'  THEN
			LET vchridtpa = '2';
			LET vchrcode = '3';
		END IF;						

        IF pvchrcodretc IN('013','021','025','028','029') AND pchrparti = 'b'  THEN
			LET vchridtpa = '2';
			LET vchrcode = '3';
		END IF;
		
		--//Error por Cuenta bloqueada 
		IF pvchrcodretc IN ('200','24') AND pchrparti = 'o'THEN
			LET vchridtpa = '24';
			LET vchrcode = '3';
		END IF;
		--//Error por Cuenta sin fondos
		IF pvchrcodretc IN ('024') AND pchrparti = 'o'THEN
			LET vchridtpa = '24';
			LET vchrcode = '2';
		END IF;	

		--//Problemas con la cuenta,tarjeta,cuenta clabe,celular ordenante
		IF pvchrcodretc IN ('501','004','100','614','122','211','17') AND pchrparti = 'o'THEN
			LET vchridtpa = '2';
			LET vchrcode = '17';
		END IF;	


		--//Error de cancelaciÃ³n
		...
		--//Error en transferencia cuentas propias
	...
	ELSE--// Se realizan las validaciones correspondientes para una devoluciÃ³n de operaciÃ³n acreditada
		
		--//No existen errores ctaspropias o recretornocodi
		IF pvchrcodretc = '0' THEN
			LET vchridtpa = '6';
			LET vchrcode = '0';
		END IF;
		
		--//No existen errores devcodi
		IF pvchrcodretc = '61' THEN
			LET vchridtpa = '61';
			LET vchrcode = '0';
		END IF;
		
		--//No existen errores devcodi - validacion a 61 - 1 para spei_ctaspropiasdevcodi (en bdicheq) modificacion MGV SPEI (validacion)
		IF pvchrcodretc = '61_1' THEN
			LET vchridtpa = '61';
			LET vchrcode = '1';
		END IF;	

		--//No existen errores devcodi - validacion a 61 - 2 para spei_devcodi modificacion MGV SPEI (validacion)
		IF pvchrcodretc = '61_2' THEN
			LET vchridtpa = '61';
			LET vchrcode = '2';
		END IF;																											   							   	  
	
		--//Existen campos vienen vacios
		IF pvchrcodretc IN('012') AND pchrparti = 'o' THEN
			LET vchridtpa = '62';
			LET vchrcode = '0';
		END IF;
		
		IF pvchrcodretc IN('013') AND pchrparti = 'b' THEN
			LET vchridtpa = '63';
			LET vchrcode = '0';
		END IF;

		--//Problemas en devcodi
		IF pvchrcodretc IN ('011','004','014','015','001','013','010','022','023','026','030','0445','0446') AND pchrparti = 'o' THEN
			LET vchridtpa = '62';
			LET vchrcode = '0';
		END IF;	
		
		IF pvchrcodretc IN ('018','024') AND pchrparti = 'b' THEN
			LET vchridtpa = '63';
			LET vchrcode = '0';
		END IF;

		IF pvchrcodretc IN ('021','028','029') AND pchrparti = 'b' THEN
			LET vchridtpa = '62';
			LET vchrcode = '0';
		END IF;	

		--/Problemas con la cuenta,tarjeta,cuenta clabe,celular ordenante
		IF pvchrcodretc IN ('501','019','021','020','200','100','614','122','211','016','017','018','027') AND pchrparti = 'o'THEN
			LET vchridtpa = '62';
			LET vchrcode = '0';
		END IF;

		 --/Problemas con la cuenta,tarjeta,cuenta clabe,celular beneficiario
		IF pvchrcodretc IN ('020','025','016') AND pchrparti = 'b' THEN
			LET vchridtpa = '63';
			LET vchrcode = '0';
		END IF;
		...
		IF pvchrcodretc IN ('16','00') AND pchrparti = 'b' THEN
			LET vchridtpa = '32';			LET vchrcode = '3';
		END IF;
		IF pvchrcodretc ='02' AND pchrparti = 'o' THEN
			LET vchridtpa = '62';
			LET vchrcode = '0';
		...
		IF pvchrcodretc ='10' AND pchrparti = 'o' THEN
			LET vchridtpa = '63';
			LET vchrcode = '0';
		END IF;
		...
		IF pvchrcodretc ='17' AND pchrparti = 'b' THEN
			...
			LET vchrcode = '0';
		END IF;
		IF pvchrcodretc in('13','200') AND pchrparti = 'b' THEN
			LET vchridtpa = '63';
			LET vchrcode = '0';
		END IF;
		IF pvchrcodretc ='13' AND pchrparti = 'o' THEN
			LET vchridtpa = '62';
			LET vchrcode = '0';
		END IF;
		...
		IF pvchrcodretc in('00005','00006','444') AND pchrparti = 'o' THEN
			LET vchridtpa = '62';
			LET vchrcode = '0';
		END IF;
	...
	IF pvchrfchmjc IS NULL OR pvchrfchmjc ='' THEN
		LET pvchrfchmjc  = '-';
	END IF;
	...
	LET wchrcadena_03 = '|'||TRIM(pvchrnomord)||'|'||TRIM(pvchrcelbenf)||'|'||TRIM(pvchrdivebenf)||'|'||pvchrbancobenf||'|'||TRIM(pvchrtpoctabenf)||'|'|| TRIM(pvchrctabenf)||'|'|| TRIM(pvhrnombenf);
    ...
    INSERT INTO tbl_stsprocodi VALUES( 
        0,'N', vchridtpa, vchrcode, pvchridmjc, pvchrfchmjc, pvchrconcepto, pmnyimporte, vtimestamp, pvchrcveras, pvchrrefnum,             
        pvchrcelord, pvchrdiveord, pvchrbancoord, pvchrtpoctaord, pvchrctaord, pvchrnomord, pvchrcelbenf, pvchrdivebenf,   
        pvchrbancobenf, pvchrtpoctabenf, pvchrctabenf, pvhrnombenf, pnumseriecert,wvchrfirma,current);
...
'OBJETIVO: GENERAR Y GUARDAR LA ESTRUCTURA PARA EL AVISO DE PROCESAMIENTO DE LOS PAGOS CODI',
'MODIFICADO POR: ISRAEL FLORES GONZÃLEZ',
'FECHA DE MODIFICACIÃN: 25 DE SEPTIEMBRE DE 2019',
'OBJETIVO: GENERAR FIRMA EN MENSAJES DE COBRO',
'MODIFICADO POR: MARIO GONZALEZ VAZQUEZ',
'FECHA DE MODIFICACION: 27 ENERO 2026 LIBERACION',
'OBJETIVO: GENERAR FIRMA EN MENSAJES DE COBRO 61 1 Y 61 2',
'CoDi(R)',
'BD: BDISPEI';
