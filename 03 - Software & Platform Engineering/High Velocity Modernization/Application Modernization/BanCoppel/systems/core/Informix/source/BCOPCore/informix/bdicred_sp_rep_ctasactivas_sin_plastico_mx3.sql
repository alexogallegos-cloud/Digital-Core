CREATE PROCEDURE "informix".sp_rep_ctasactivas_sin_plastico_mx3(pEmpresa CHAR(3))
...
BEGIN
	...
	---select {+INDEX(bdicred:sd_maecred idx_sd_maecred4)} mae.num_credito,mae.fecha_apertura,mae.sucursal,ind.fecha_ultima_compra,sdo.sdo_cap_insoluto,sdo.monto_otorgado
	--select {+INDEX(bdicred:sd_maecred maesta)} mae.num_credito,mae.fecha_apertura,mae.sucursal,ind.fecha_ultima_compra,sdo.sdo_cap_insoluto,sdo.monto_otorgado
	---select {+MULTI_INDEX(bdicred:sd_maecred),+INDEX(bdicred:sd_maesdos idx_sd_maesdos} mae.num_credito,mae.fecha_apertura,mae.sucursal,ind.fecha_ultima_compra,sdo.sdo_cap_insoluto,sdo.monto_otorgado
	select {+AVOID_HASH(bdicred:sd_maesdos} mae.num_credito,mae.fecha_apertura,mae.sucursal,ind.fecha_ultima_compra,sdo.sdo_cap_insoluto,sdo.monto_otorgado
...
---	join bdicred:sd_indicador_cred ind on (mae.num_credito = ind.num_credito and mae.empresa = ind.empresa)
	join bdicred:sd_indicador_cred ind on (mae.num_credito = ind.num_credito)
---	join bdicred:sd_maesdos sdo on (mae.num_credito = sdo.num_credito and mae.empresa = sdo.empresa)
	join bdicred:sd_maesdos sdo on (mae.num_credito = sdo.num_credito)
	--where mae.empresa = '001'
	where mae.status_cred  = 'AA'
