CREATE PROCEDURE "informix".sp_inserta_atm_ofi_web(pempresa CHAR(3),
                                           pcod_atm CHAR(4), 
                                           pdivisa  CHAR(2), 
                                           pcant_1  FLOAT,
                                           pcant_2  FLOAT,
...
BEGIN
	...
	--25-09-2019
	--Se agrega el respaldo previo a las afectaciones operativas de la transacciÃ³n
	--Se agrega la transaccionalidad en el proceso.

	IF (SELECT COUNT(cod_atm) FROM bdisuc:"informix".ss_atm WHERE cod_atm = pcod_atm)> 0 THEN
...
'FECHA:2019-11-11',
'DESCRIPCION: Se modifica SP para realizar respaldo de la informacion en caso de que ocurra algun error restaurar los datos de la tabla de recuperacion',
'SOLICITA: Gabriela Angulo',
'Fecha 06-Ene-2020',
'Modifico:  Jesus Moreno',
'Se quita la actualizacion a la tabla ss_atm_rec y se agrega un delete a la misma, se renombre sp_inserta_atm a sp_inserta_atm_ofi para que solo se mande llamar desde el sistema OFI';

CREATE PROCEDURE "informix".sp_monitor_atm_admin_web(pempresa      CHAR(3),
                                           pplaza        CHAR(3),
                                           psucursal     CHAR(4),
                                           pregistro     SMALLINT,
                                           pfinicio      DATE,
	...
	RETURNING CHAR(5), CHAR(8),CHAR(4),
...
END PROCEDURE
DOCUMENT
'MODIFICÃ:    	Jesus Moreno',
'FECHA:       	14/10/2019',
'DESCRIPCIÃN: 	se modifica el tipo de dato de las variables DEFINE vcant_1,vcant_2,vcant_3,vcant_4,vcant_5 ,vcant_6',
'BASE DE DATOS: bdisuc',
'FOLIO:628',
'Llamado desde:MonitorAtm.exe',
'MODIFICÃ:    	Jesus Moreno',
'FECHA:       	06/01/2020',
'DESCRIPCIÃN: 	se renombra el sp de sp_monitor_atm01 a sp_monitor_atm_admin';

CREATE PROCEDURE "informix".sp_recepdota_atm_web(pempresa CHAR(3),
											psucursal CHAR(4),
...
BEGIN
	...
	--Modificado por: PAUL IVAN QUINTERO VARELA
	--25-09-2019
	--Se agrega el respaldo previo a las afectaciones operativas de la transacciÃÂ³n
	--Se agrega la transaccionalidad en el proceso.
	...
	ELSE
		...
		--SE VALIDA SI QUEDO UNA OPERACION INCONCLUSA PARA EL CAJERO ATM, DE SER ASI SE RESTAURA COMO ESTABA LA ULTIMA VEZ Y CONTINUA CONE EL FLUJO
		IF (SELECT cod_atm FROM  ss_atm_rec WHERE cod_atm = pSucursal) > 0 THEN
...
'AUTOR:Cristian Valentina Aguilar ', 
'FECHA:2019-11-11',
'DESCRIPCION: Se modifica SP para que no afecta la tabla ss_cajageneral y que valide si quedo una recepcion inconclusa en la tabla ss_atm, de ser asi se realiza reverso y se continua con el proceso normal',
'SOLICITA: Gabriela Angulo',
'FECHA: 06-ENE-2020',
'MODIFICO:  JESUS MORENO',
