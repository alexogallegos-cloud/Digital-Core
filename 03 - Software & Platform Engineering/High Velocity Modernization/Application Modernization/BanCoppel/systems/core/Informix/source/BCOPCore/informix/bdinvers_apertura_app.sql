CREATE PROCEDURE "informix".apertura_app( pempresa 	      char(3),
                                      pnumcte         char(20),
                                      psecuencia      smallint,
                                      pinstrumento    char(4),
                                      ppromotor       char(8),
...
returning char(5),char(20),char(5),date,money(14,2),money(14,2),
    ...
    -- SET DEBUG FILE TO "/informix/FAOC/Debug/Inv/apertura_app.out";
    -- TRACE ON;
   ...
   -- Verifica el capital de la inversion contra parametros establecidos
    ...
    if i_num_invers is null or i_num_invers = " " then
        --Apertura de pagare
        call "informix".sp_creacta_pag(pempresa,pnumcte,psecuencia,pinstrumento,i_num_invers,vlongcta,vdiferencia)
        returning v_codret,i_num_invers;
        IF v_codret <> '000' THEN
            RETURN v_codret,i_num_invers,vw_dias,pfecha_venc,v_imp_isr,v_rend_neto,v_tasa_bruta,v_tasa_isr,v_tasa_neta;
        END IF;
    end if;
    ...
    IF length(v_num_invers) = vlongcta and bdinteg:"informix".val_num(v_num_invers) then --*****se agraga validacion para longitud de la cuenta***
        ...
        UPDATE "informix".sv_maeinv 
            SET cod_instrum = pinstrumento,
                status_cta = "0",
                motivo = "  ",
                fec_ult_mov = v_fecha_hoy,
                fec_cancelac = " ",
                fec_reinversion = v_fecha_hoy,
                capital = pcapital,
                sdo_retenido = 0,
                sdo_cong = 0,
                plazo = wplazo,
                fecha_venc = pfecha_venc,
                opcion_retiro = popcion_ret,
                intereses = vw_tot_int,
                isr = vw_isr,
                tasa = val,
                sobretasa = ve_sobretasa,
                dia_sdo_pos = 0,
                acum_sdo_pos = 0,
                sdo_prom_mesant = 0,
                sdo_mes_ant = 0,
                ...
                sdo_ult_corte = 0,
                adicionado = pusuario,
                fecha_alta = v_fecha_hoy,
                fecha_val = " ",
                modificado = " ",
                fecha_mod = " ",
                cta_cheques = pcta_cheques,
                sucursal = psucursal,
                plaza = pplaza,
                promotor = ppromotor,
                tipo_banca = ptipo_banca,
                reg_firmas = preg_firmas,
                envio = penvio,
                direcc_envio = pdirecc_envio,
                cobraisr = pcobraisr,
                per_acred_int = pper_acred   
        WHERE cuenta = v_num_invers;
    else
        DELETE FROM "informix".sv_maeinv WHERE cuenta = v_num_invers AND num_cte = v_num_cte;
