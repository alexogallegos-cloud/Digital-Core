CREATE PROCEDURE "informix".sp_com_manejo_cta_ident_5()
...
-- *****************************************************************************
    ...
    ON EXCEPTION SET vSQLErr, vIsamErr, vErrorInfo
        IF  vSQLErr != 0 THEN
            SET DEBUG FILE TO '/resplogifx/conciliachq/sp_com_manejo_cta_ident_5.err';
            TRACE ON;
            LET vCodRet     = vSQLErr;
    ...
    --SET DEBUG FILE TO '/resplogifx/conciliachq/sp_com_manejo_cta_ident_5.out';
    --TRACE ON;
    ...
    AND noc.fecha_alta < vFechaInicial AND chq.cuenta = noc.cuenta
    ...
    /*     EXENCION POR PAGO TDC     */
    FOREACH WITH HOLD
        Select CC.cliente
        Into vNumCteCred
        From  bdicheq:sc_com_manejo_ctas_a_cobrar CC 
        Inner Join bdicred:sd_maecred CR on CC.cliente = CR.numcte
        Inner Join bdicheq:movs_cred_com_cta CM on CR.num_credito = CM.num_credito
        ...
        If vNumCteCred is null or vNumCteCred = '' Then
            Continue FOREACH;
        Else
            Delete From bdicheq:sc_com_manejo_ctas_a_cobrar Where cliente = vNumCteCred;
        End If;
