CREATE PROCEDURE "informix".sp_asignartarjetarepadicop(p_sempresa CHAR(3),p_scvesucursal CHAR(4), p_stipoAsig CHAR(1), p_sNum_Cte CHAR(9))
	RETURNING CHAR(5),CHAR(9);

--****************************************************************************************************
-- DESCRIPCION: Registra un numero de tarjeta de reposicion como asignada, actualiza el inventario, actualiza los envios e indica el numero de tarjeta de reposicion que se debe entregar al cliente.
...
--****************************************************************************************************
	...
	--SET DEBUG FILE TO "/tmp/conciliacion/tarrepor.txt";
	--TRACE ON;
...
IF EXISTS(SELECT empresa FROM bditarjcop:"informix".inventariotarcop WHERE empresa = p_sempresa)THEN
	...
	IF EXISTS(SELECT cvesucursal FROM bditarjcop:"informix".inventariotarcop WHERE cvesucursal = p_scvesucursal)THEN
		...
		IF (NVL(p_sempresa,'') <> '')  AND (NVL(p_scvesucursal,'') <> '') AND (NVL(p_stipoAsig,'') <> '') AND (NVL(p_sNum_Cte,'') <> '')THEN
			--Valida que el tipo dea signacion sea R o A.
			IF(p_stipoAsig = 'R') OR (p_stipoAsig = 'A')THEN
				---Se valida que existan tarjetas para aplicar en esa sucursal
				IF EXISTS (SELECT consumo FROM bditarjcop:"informix".inventariotarcop WHERE empresa = p_sempresa and cvesucursal = p_scvesucursal  AND tipotarjeta = 'R' and existencia <> 0) THEN

					IF EXISTS(SELECT numenvio,ulttarasignada,rangofin FROM bditarjcop:"informix".enviostarcop
						WHERE enviodisponible = 'A' AND empresa = p_sempresa AND cvesucursal = p_scvesucursal AND tipotarjeta = 'R') THEN
						...
						--El Sistema de Consumo Tarjetas coppel incrementa en uno el numero de tarjeta obtenido y registra el nuevo numero de tarjeta como el ultimo asignado del envio correspondiente de la sucursal.
						LET v_starjetaultima = (v_starjetaultima + 1)::INTEGER ;

						UPDATE bditarjcop:"informix".enviostarcop  SET ulttarasignada = v_starjetaultima  WHERE empresa = p_sempresa AND cvesucursal = p_scvesucursal AND numenvio = v_snum_envio AND tipotarjeta = 'R' AND v_starjetaultima BETWEEN rangoini AND rangofin;

						--El Sistema de Consumo Tarjetas coppel valida si el numero de la ultima tarjeta asignada es menor al rango final del envio de tarjetas de reposicion.
						IF v_irangofin <= v_starjetaultima THEN
							UPDATE bditarjcop:"informix".enviostarcop  SET EnvioDisponible = 'N' WHERE empresa = p_sempresa AND cvesucursal = p_scvesucursal AND numenvio = v_snum_envio AND tipotarjeta = 'R' AND v_starjetaultima BETWEEN rangoini AND rangofin;
						END IF;

						LET v_starjetaultima = LPAD(TRIM((v_starjetaultima)::CHAR(9)) ,9,'0') ;

						--Se realiza la insercion de tarjeta adicional o reposicion como "Pendiente".
						INSERT INTO bditarjcop:"informix".TarjetasRepoTarCop(NumCliente,Empresa,CveSucursal,NumTarjeta,NumEnvio,EstatusTarjeta,FechaAsignacion)
						VALUES(p_sNum_Cte,p_sempresa,p_scvesucursal,v_starjetaultima,v_snum_envio,'P',CURRENT);

						--El Sistema de Consumo Tarjetas coppel actualiza el consumo de tarjetas de reposicion incrementáolo en uno de la sucursal en donde correspondan la clave de la empresa y la clave de la sucursal
						--El Sistema de Consumo Tarjetas coppel actualiza la existencia de tarjetas de reposicion disminuyendo en uno de la sucursal en donde correspondan la clave de la empresa y la clave de la sucursal
						UPDATE bditarjcop:"informix".inventariotarcop SET consumo = consumo + 1, existencia = existencia - 1 WHERE empresa = p_sempresa AND cvesucursal = p_scvesucursal AND tipotarjeta = 'R';
					...
					--No existe el numero de tarjeta asignada proporcionada por el Sistema de Consumo Tarjetas Coppel.
					ELSE
						LET v_sCodRet = '00305';
						LET v_starjeta = '000000000';
					END IF;
				...
				END IF;
			ELSE
				--El tipo de asignacion no es valido.
				LET v_sCodRet = '00303';
				LET v_starjeta = '000000000';
			END IF;
		ELSE
...
CREATE PROCEDURE "informix".sp_calcularnumverificador (iNumeroCliente INTEGER)
RETURNING INTEGER AS RETORNO;

--****************************************************************************************************
-- DESCRIPCION: Calcula el numero verificador de la tarjeta numerada coppel
...
DEFINE iLongitudNumero    SMALLINT;
DEFINE iIniciaPosicion    SMALLINT;
DEFINE iTerminaPosicion   SMALLINT;
DEFINE iDeterminaDigito   SMALLINT;
DEFINE cDigitoVerificador CHAR(1);
DEFINE iResiduo           SMALLINT;
DEFINE sNumeroCliente     CHAR(8);
DEFINE viSqlError INTEGER;
DEFINE cNumDigito CHAR(9);

LET iLongitudNumero = 0;
LET iIniciaPosicion = 1;
LET iTerminaPosicion = 0;
LET iDeterminaDigito = 0;
...
END EXCEPTION;
	...
	LET sNumeroCliente = CAST (iNumeroCliente AS CHAR (8));
	LET iLongitudNumero = LENGTH (TRIM(sNumeroCliente));
	LET iIniciaPosicion = 1;
	LET iTerminaPosicion = 0;
	LET iDeterminaDigito = iLongitudNumero;
	LET cDigitoVerificador = ' ';

	WHILE (iIniciaPosicion <= iLongitudNumero)
		LET iTerminaPosicion = iTerminaPosicion + CAST (substr(sNumeroCliente,iIniciaPosicion,1) AS SMALLINT) * (iDeterminaDigito + 1);
		LET iIniciaPosicion = iIniciaPosicion + 1;
		LET iDeterminaDigito = iDeterminaDigito - 1;
	END WHILE;

	LET iResiduo = MOD(iTerminaPosicion,11);
	LET iDeterminaDigito = 11 - iResiduo;
	
	IF (iDeterminaDigito = 10) THEN
		LET cDigitoVerificador = '1';
	END IF;

	IF (iResiduo = 10 OR iResiduo = 0 OR iResiduo = 1) THEN
		LET cDigitoVerificador = '1';
	ELSE
		LET cDigitoVerificador = CAST (11-iResiduo AS CHAR(1));
	END IF;
	LET cNumDigito = TRIM(sNumeroCliente) || TRIM(cDigitoVerificador);	
	RETURN CAST(cNumDigito AS INTEGER);
...
CREATE PROCEDURE "informix".sp_confirmartarjetaasigcop(
psEmpresa CHAR(3),
psCveSucursal CHAR(4),
psTipoTarjeta CHAR(1),
psNumTarjeta CHAR(9),
psNumCteBanco CHAR(20)
)

RETURNING CHAR(5) AS codret;

--****************************************************************************************************
-- DESCRIPCION: Confirma la asignacion de tarjetas.
...
DEFINE vsNumCteCop CHAR(9);
DEFINE viSecuencia INTEGER;
DEFINE vsTipoCliente CHAR(1);
...
--set debug file to "/tmp/sp_ConfirmarTarjetaAsigCop.out";
--Trace on;
...
--Verifica si existe la empresa proporcionada.
IF EXISTS(SELECT empresa FROM bditarjcop:"informix".inventariotarcop WHERE empresa = psEmpresa)THEN
	--Verifica si la clave de sucursal proporcionada cuenta con un inventario de tarjetas.
	IF EXISTS(SELECT cvesucursal FROM bditarjcop:"informix".inventariotarcop WHERE cvesucursal = psCveSucursal)THEN
		--Si el tipo de tarjeta es numerada.
		IF(psTipoTarjeta = 'N') THEN
			--Verifica si existe el numero de tarjeta indicado.
			IF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasnumtarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta)THEN
				--Verifica que el numero de tarjeta proporcionado se encuentre con estatus "P".
				IF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasnumtarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta AND estatustarjeta = 'P')THEN
					--Marca como asignada el numero de tarjeta indicado.
					UPDATE bditarjcop:"informix".tarjetasnumtarcop SET estatustarjeta = 'A' WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta;
					--La operación se realizo de manera exitosa.
					LET vsCodRet = '00000';
				ELIF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasnumtarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta AND estatustarjeta = 'A')THEN
					--Numero de tarjeta marcada como asiganada anteriormente.
					LET vsCodRet = '01704';
				END IF;
			ELSE
				--No se localizo la tarjeta numerada proporcionada.
				LET vsCodRet = '01703';
			END IF;
		--Si el tipo de tarjeta es reposicion o adicional.
		ELIF(psTipoTarjeta = 'R') OR (psTipoTarjeta = 'A') THEN
			--Verifica si existe el numero de tarjeta indicado.
			IF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasrepotarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta)THEN
				--Verifica que el numero de tarjeta proporcionado se encuentre con estatus "P".
				IF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasrepotarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta AND estatustarjeta = 'P')THEN
					--Marca como asignada el numero de tarjeta indicado.
					UPDATE bditarjcop:"informix".tarjetasrepotarcop SET estatustarjeta = psTipoTarjeta WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta;
					--Obtiene numero de cliente coppel, obtiene la secuencia del numero de tarjeta repadic, valida si es titular o adicional e inserta registro en detallerepadic
					SET LOCK MODE TO WAIT 3;
					SET ISOLATION TO DIRTY READ;
					SELECT numcliente INTO vsNumCteCop FROM bditarjcop:"informix".tarjetasrepotarcop WHERE numtarjeta = psNumTarjeta;
					IF(psTipoTarjeta = 'R') THEN
						SELECT secuencia INTO viSecuencia FROM bdinteg:"informix".si_adiccoppel WHERE numctecoppel = vsNumCteCop AND numcte = psNumCteBanco;
						IF (viSecuencia = 1) THEN
							LET vsTipoCliente = 'T';
						ELIF (viSecuencia = 2) OR (viSecuencia = 3) THEN
							LET vsTipoCliente = 'A';
						END IF;
						SELECT MAX(secuencia) INTO viSecuencia FROM bditarjcop:"informix".detallerepadic WHERE numctecoppel = vsNumCteCop AND numctebanco = psNumCteBanco;
						LET viSecuencia = NVL(viSecuencia, 0) + 1;
					ELIF(psTipoTarjeta = 'A') THEN
						LET vsTipoCliente = 'A';
						SELECT MAX(secuencia) INTO viSecuencia FROM bdinteg:"informix".si_adiccoppel WHERE numctecoppel = vsNumCteCop;
						IF (viSecuencia = 2) THEN
							LET viSecuencia = 1;
						ELIF (viSecuencia = 3) THEN
							LET viSecuencia = 2;
						END IF;
					END IF;
					--Inserta en tabla nueva detallerepadic 2011.
					INSERT INTO bditarjcop:"informix".detallerepadic(numctebanco, numctecoppel, secuencia, numtarjeta, tipocliente, fecha, fechainsert) VALUES(psNumCteBanco, vsNumCteCop, viSecuencia, psNumTarjeta, vsTipoCliente, CURRENT::DATE, CURRENT);
					--La operación se realizo de manera exitosa.
					LET vsCodRet = '00000';
				ELIF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasrepotarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta AND estatustarjeta = 'R' OR estatustarjeta = 'A')THEN
					--Numero de tarjeta marcada como asignada anteriormente.
					LET vsCodRet = '01704';
				END IF;
			ELSE
				--No se localizo la tarjeta reposicion o adicional proporcionada.
				LET vsCodRet = '01703';
			END IF;
		--Si el tipo de tarjeta no es numerada, reposicion o adicional.
		ELIF(psTipoTarjeta <> 'N') AND (psTipoTarjeta <> 'R') AND (psTipoTarjeta <> 'A') THEN
			--El tipo de tarjeta no es valido.
			LET vsCodRet = '01702';
		END IF;
	ELSE
		--La sucursal no cuenta con un inventario de tarjetas.
...
CREATE PROCEDURE "informix".sp_confirmartarjetaextcop(
psEmpresa CHAR(3),
psCveSucursal CHAR(4),
psTipoTarjeta CHAR(1),
psNumTarjeta CHAR(9)
...
--****************************************************************************************************
-- DESCRIPCION: Confirma el extravio de tarjetas.
-- AUTOR : Rochin Rocha Edgar Ivan
-- FECHA : 18/12/2008
-- BD: bditarjcop
-- SISTEMA : Caja Unica
...
--set debug file to "/tmp/sp_ConfirmarTarjetaExtCop.out";
--Trace on;
	...
	IF EXISTS(SELECT cvesucursal FROM bditarjcop:"informix".inventariotarcop WHERE cvesucursal = psCveSucursal)THEN
		...
		IF(psTipoTarjeta = 'N') THEN
			...
			IF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasnumtarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta)THEN
				--Verifica que el numero de tarjeta proporcionado se encuentre con estatus "P".
				IF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasnumtarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta AND estatustarjeta = 'P')THEN
					--Marca como extraviada el numero de tarjeta indicado.
					UPDATE bditarjcop:"informix".tarjetasnumtarcop SET estatustarjeta = 'E' WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta;
					--La operación se realizo de manera exitosa.
					LET vsCodRet = '00000';
				ELIF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasnumtarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta AND estatustarjeta = 'E')THEN
		...
		ELIF(psTipoTarjeta = 'R') OR (psTipoTarjeta = 'A') THEN
			...
			IF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasrepotarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta)THEN
				...
				IF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasrepotarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta AND estatustarjeta = 'P')THEN
					--Marca como extraviada el numero de tarjeta indicado.
					UPDATE bditarjcop:"informix".tarjetasrepotarcop SET estatustarjeta = 'E' WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta;
					--La operación se realizo de manera exitosa.
					LET vsCodRet = '00000';
				ELIF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasrepotarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta AND estatustarjeta = 'E')THEN
					--Numero de tarjeta marcada como extraviada anteriormente.
					LET vsCodRet = '00404';
				ELIF EXISTS(SELECT numtarjeta FROM bditarjcop:"informix".tarjetasrepotarcop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND numtarjeta = psNumTarjeta AND estatustarjeta = 'R' OR estatustarjeta = 'A')THEN
					--Numero de tarjeta asignada anteriormente.
...
CREATE PROCEDURE "informix".sp_genreprecepciontarcop( psEmpleado CHAR(8), psEmpresa CHAR(3) )

RETURNING CHAR(5) AS CodRetorno, CHAR(200) AS Mensaje;

--****************************************************************************************************
-- DESCRIPCION: Genera un archivo que contiene lso datos de los envios recibidos en la sucursal el dia actual.
-- AUTOR : Casanova Edeza Hector Juan
-- FECHA : 30/01/2009
-- BD: bdiTarCop
-- SISTEMA : Inventario de Tarjetas Caja Unica.
...
BEGIN
  ...
  ON EXCEPTION SET viSqlError    --cacha el error en caso de que exista y regresa un valor predeterminado
		...
		INSERT INTO bditarjcop:"informix".BitacoraTarCop (Empleado, Fecha, Actividad, NomArchivo, CodRetorno) VALUES (psEmpleado, CURRENT, 'REP ENVIOS REC', vsNomArchivo, viSqlError);
	...
	--SET DEBUG FILE TO "/tmp/conciliacion/reptarcop.txt";
	--TRACE ON;
	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	...
	IF ( vsFlagSoloNumeros <> 'V' ) THEN --VALIDA QUE EL NUMERO DE EMPLEADO CONTENGA UNICAMENTE NUMEROS
		LET vsCodRetorno = '01004';
	ELIF (LENGTH (TRIM(psEmpleado)) < 8) THEN  -- ERROR DE LONGITUD DEL NUMERO DE EMPLEADO.
		LET vsCodRetorno = '01003';
	ELIF NOT EXISTS (SELECT Empresa FROM bditarjcop:"informix".InventarioTarCop WHERE Empresa = psEmpresa ) THEN  --NO EXISTE LA EMPRESA PROPORCIONADA
		LET vsCodRetorno = '01002';
	ELIF NOT EXISTS (SELECT Valor FROM bditarjcop:"informix".ParamInvTarCop WHERE Descripcion = "REPOSITORIO" AND Empresa = psEmpresa ) THEN --OBTIENE EL  REPOSITORIO DEL ARCHIVO DE INTERCAMBIO
		LET vsCodRetorno = '01001';
	ELSE
		LET vsNomArchivo = 'temporal.txt';
		LET vsNomArchivo2 = 'R' || REPLACE (SUBSTRING (CURRENT FROM 1 FOR 10), '-', '' ) || '.dat' ;
		
		SELECT FIRST 1 Valor INTO vsRepositorio FROM bditarjcop:"informix".ParamInvTarCop WHERE Descripcion = "REPOSITORIO" AND Empresa = psEmpresa ;
		...
		|| "(SELECT COUNT (CveSucursal) FROM bditarjcop:EnviosTarCop WHERE FechaRec::DATE = CURRENT::DATE)::INTEGER ||'|'||"
		|| "CURRENT||'|'||0||'|'||"
		|| "(SELECT COUNT (CveSucursal) FROM bditarjcop:EnviosTarCop WHERE FechaRec::DATE = CURRENT::DATE AND TipoTarjeta = 'N')::INTEGER ||'|'||"
		|| "(SELECT COUNT (CveSucursal) FROM bditarjcop:EnviosTarCop WHERE FechaRec::DATE = CURRENT::DATE AND TipoTarjeta = 'R')::INTEGER ||'|'||"
		|| "CURRENT ||'|'||'" || psEmpleado || "'||'|'||'00000'||'|'|| CURRENT) FROM bditarjcop:ParamInvTarCop WHERE Descripcion = 'REPOSITORIO' AND Empresa = '001' "
		|| "UNION SELECT (TRIM(Empresa)||'|'||TRIM(CveSucursal)||'|'||TRIM(TipoTarjeta)||'|'||NumEnvio||'|'||FechaSurt||'|'||CantidadRec||'|'||RangoIni::INTEGER||'|'|| RangoFin::INTEGER||'|'||FechaRec||'|'||"
		|| "EmpleadoRec||'|'||TRIM(NumGuia)||'|'||FechaGuia) FROM bditarjcop:EnviosTarCop WHERE FechaRec::DATE = CURRENT::DATE ";
		...
		IF ( vsSQL <> '' ) THEN 
			...
			LET vsCodRetorno = '00000';
		ELSE -- CONSULTA VACIA
			LET vsCodRetorno = '01000';
		END IF ;
		
	END IF ;
	
	--GUARDA  REGISTRO DE LA OPERACION EN LA BITACORA
	INSERT INTO bditarjcop:"informix".BitacoraTarCop (Empleado, Fecha, Actividad, NomArchivo, CodRetorno) VALUES (psEmpleado, CURRENT, 'REP ENVIOS REC', vsNomArchivo2, vsCodRetorno);
	
	SELECT mensaje INTO vsMensaje FROM bditarjcop:"informix".mensajestarcop WHERE codmensaje = vsCodRetorno;
...
CREATE PROCEDURE "informix".sp_guardarhistestadisticatarcop(
)
...
--SET DEBUG FILE TO "/tmp/sp_GuardarHistEstadisticaTarCop.out";
--TRACE ON;
...
LET vsFechaMesAnt = LPAD(viMes, 2, '0') || '/' || SUBSTRING(vdFecha FROM 1 FOR 4);

	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	FOREACH
--Obtiene el código de la empresa, la clave de la sucursal, el consumo de tarjetas de la sucursal, 
--el inventario de tarjetas físico de la sucursal, el tipo de tarjeta de todas las estadisticas existentes.
...
--Guarda el histórico de la estadística de la sucursal la cual consiste en: el código de la empresa, la clave de la sucursal, el consumo de tarjetas de la sucursal, el inventario de
--tarjetas físico de la sucursal, la fecha de la estadística el tipo de tarjeta de todas las estadisticas existentes con fecha del mes anterior.
	...
	END FOREACH
--Actualiza los datos de la estadística de la sucursal correspondiente de la siguiente manera: el consumo de tarjetas de la sucursal se regresa a 0 (cero)  de todas las estadisticas existentes.
		UPDATE bditarjcop:"informix".inventariotarcop SET consumo = 0;
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	FOREACH
--Obtiene el código de la empresa, la clave de la sucursal, el tipo de tarjeta, el numero del envio, la fecha del surtido, el folio del pedido especial, la cantidad de tarjetas recibidas, el rango inicial de las tarjetas, el rango final de 
--las tarjetas, la fecha de recepción del envio, el estatus del envio y el numero del empleado que recibio el envio de todos los envios con estatus "Terminado".
		...
		INTO vsEmpresa, vsCveSucursal, vsTipoTarjeta, viNumEnvio, vdFechaSurt, viCantidadRec, viRangoIni, viRangoFin, vdFechaRec, vsEnvioDisp, vsEmpleadoRec, vsNumGuia, vdFechaGuia, vsTipoSurtido
		FROM bditarjcop:"informix".enviostarcop
		WHERE enviodisponible = 'N'
--Guarda el histórico del envio de la sucursal cuyo estatus es "Terminado", el cual consiste en: el código de la empresa, la clave de la sucursal, el tipo de tarjeta, el numero del envio, la fecha del surtido, el folio del 
--pedido especial, la cantidad de tarjetas recibidas, el rango inicial de las tarjetas, el rango final de las tarjetas, la fecha de recepción del envio, el estatus del envio y el numero del empleado que recibio el envio.
		...
		VALUES(
		vsEmpresa, vsCveSucursal, vsTipoTarjeta, viNumEnvio, vdFechaSurt, viCantidadRec, viRangoIni, viRangoFin, vdFechaRec, vsEnvioDisp, vsEmpleadoRec, vsNumGuia, vdFechaGuia, vsTipoSurtido);
		
--Borra el envio con estatus "Terminado" que coincida con: el código de la empresa, la clave de la sucursal, el tipo de tarjeta, el numero del envio, la fecha del surtido, el folio del 
--pedido especial, la cantidad de tarjetas recibidas, el rango inicial de las tarjetas, el rango final de las tarjetas, la fecha de recepción del envio, el estatus del envio y el numero del empleado que recibio el envio.
		DELETE bditarjcop:"informix".enviostarcop WHERE empresa = vsEmpresa AND cvesucursal = vsCveSucursal AND tipotarjeta = vsTipoTarjeta
		AND numenvio = viNumEnvio AND fechasurt = vdFechaSurt AND cantidadrec = viCantidadRec AND rangoini = viRangoIni 
		AND rangofin = viRangoFin AND fecharec = vdFechaRec AND enviodisponible = vsEnvioDisp AND empleadorec = vsEmpleadoRec;
		--AND numguia = vsNumGuia AND fechaguia = vdFechaGuia AND tiposurtido = vdFechaGuia;
...
CREATE PROCEDURE "informix".sp_recibirlotetarcop(
psEmpresa CHAR(3),
psCveSucursal CHAR(4),
piNumEnvio INTEGER,
psTipoTarjeta CHAR(1),
psCantidadRec INTEGER,
piRangoIni INTEGER,
piRangoFin INTEGER,
...
--****************************************************************************************************
-- DESCRIPCION: Actualiza el inventario de la sucursal con el envio recibido.
-- AUTOR : Rochin Rocha Edgar Ivan
-- FECHA : 16/12/2008
-- BD: bditarjcop
-- SISTEMA : Caja Unica
...
DEFINE viSqlErr INTEGER;
DEFINE vsCodRet CHAR(5);
DEFINE viCantRango INTEGER;
DEFINE vdFechaActCU DATETIME YEAR TO FRACTION(1);
DEFINE vdFechaActual DATETIME YEAR TO FRACTION(1); 

LET viCantidadRec = 0;
LET viSqlErr = 0;
LET vsCodRet = '';
LET viCantRango = 0;
LET vdFechaActCU = CURRENT;
LET vdFechaActual = CURRENT;

--SET DEBUG FILE TO "/tmp/sp_RecibirLoteTarCop.sql";
--TRACE ON;
...
SELECT fechaactcu INTO vdFechaActCU FROM bditarjcop:"informix".sucursalescajaunica WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal;
--Obtiene fecha actual
SELECT LIMIT 1 CURRENT INTO vdFechaActual FROM bditarjcop:"informix".sucursalescajaunica;

--Verifica que la fecha de activacion de sucursal sea menor o igual al a fecha actual.
IF(vdFechaActCU <= vdFechaActual)THEN
--Verifica longitud de numero de empleado.
	IF(LENGTH (TRIM(psNumEmpleado)) < 8) THEN
		--La longitud del numero de empleado debe ser de 8 caracteres.
		LET vsCodRet = '00107';
	ELSE
		--Verifica si existe la empresa proporcionada.
		IF EXISTS(SELECT empresa FROM bditarjcop:"informix".inventariotarcop WHERE empresa = psEmpresa)THEN
			--Verifica si la clave de sucursal proporcionada cuenta con un inventario de tarjetas.
			IF EXISTS(SELECT cvesucursal FROM bditarjcop:"informix".inventariotarcop WHERE cvesucursal = psCveSucursal)THEN
				--Verifica que exista un inventario correpondiente a la sucursal y al tipo de tarjeta proporcionados por el sistema Caja Única de Sucursal.
				IF EXISTS(SELECT empresa, cvesucursal, consumo, existencia, tipotarjeta FROM bditarjcop:"informix".inventariotarcop
						  WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal AND tipotarjeta = psTipoTarjeta) THEN
					LET viCantRango = (piRangoFin - piRangoIni) + 1;
					--Valida que la cantidad del rango de tarjetas corresponda con la cantidad de tarjetas recibidas.
					IF(viCantRango = psCantidadRec)THEN
						...
						IF EXISTS(SELECT empresa, cvesucursal, tipotarjeta, numenvio, cantidadrec, rangoini, rangofin FROM bditarjcop:"informix".EnviosTarCop WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal
								  AND tipotarjeta = psTipoTarjeta AND numenvio = piNumEnvio AND cantidadrec = psCantidadRec AND rangoini = piRangoIni AND rangofin = piRangoFin) THEN
							--Verifica que el envio no se encuentre marcado como "Activo".
							IF NOT EXISTS(SELECT EnvioDisponible FROM bditarjcop:"informix".EnviosTarCop WHERE cvesucursal = psCveSucursal AND numenvio = piNumEnvio AND EnvioDisponible = 'A' AND tipotarjeta = psTipoTarjeta AND rangoini = piRangoIni AND rangofin = piRangoFin)THEN
										SELECT cantidadrec INTO viCantidadRec FROM bditarjcop:"informix".EnviosTarCop WHERE cvesucursal = psCveSucursal AND numenvio = piNumEnvio AND tipotarjeta = psTipoTarjeta AND rangoini = piRangoIni AND rangofin = piRangoFin;
										UPDATE bditarjcop:"informix".InventarioTarCop SET existencia = existencia + viCantidadRec WHERE tipotarjeta = psTipoTarjeta AND empresa = psEmpresa
										AND cvesucursal = psCveSucursal;
										UPDATE bditarjcop:"informix".EnviosTarCop SET EmpleadoRec = psNumEmpleado, EnvioDisponible = "A", FechaRec = CURRENT WHERE numenvio = piNumEnvio
										AND tipotarjeta = psTipoTarjeta AND empresa = psEmpresa AND cvesucursal = psCveSucursal AND rangoini = piRangoIni AND rangofin = piRangoFin;
							...
							END IF;
						ELSE
							--El envio no corresponde a la sucursal y por lo tanto no puede ser añadido al inventario de esta.
							LET vsCodRet = '00105';
						END IF;
					ELSE
...
CREATE PROCEDURE "informix".sp_registrarnuevolotetarjcop(
psTipoSurtido CHAR(2),
psEmpresa CHAR(3),
psCveSucursal CHAR (4),
psTipoTarjeta CHAR(1),
piNumEnvio INTEGER,
pdFechaSurt DATETIME YEAR TO FRACTION(5),
piCantidadRec INTEGER,
piRangoIni INTEGER,
piRangoFin INTEGER,
psNumGuia CHAR(25),
pdFechaGuia DATETIME YEAR TO FRACTION(5)
)

RETURNING CHAR(5) AS codret;

--****************************************************************************************************
-- DESCRIPCION: Registrar un envio de tarjetas generado por coppel.
-- AUTOR : Rochin Rocha Edgar Ivan
-- FECHA : 18/12/2008
-- BD: bditarjcop
-- SISTEMA : Caja Unica
...
DEFINE viSqlErr INTEGER;
DEFINE vsCodRet CHAR(5);
DEFINE viCantTar INTEGER;

LET viSqlErr = 0;
LET vsCodRet = '';
LET viCantTar = 0;

--SET DEBUG FILE TO "/tmp/sp_RegistrarNuevoLoteTarjCop.out"; 
--TRACE ON;
...
SET LOCK MODE TO WAIT 3;
SET ISOLATION TO DIRTY READ;
--El Sistema de registro Tarjetas coppel valida que la sucursal indicada por el Sistema de Captura de Surtido de tarjetas coppel se encuentra en operacion.
IF EXISTS(SELECT empresa, cvesucursal FROM bditarjcop:"informix".sucursalescajaunica WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal) THEN
	--Valida que la cantidad de tarjetas reportadas corresponda con la cantidad de tarjetas indicada por los rangos de tarjetas.
	LET viCantTar = (piRangoFin - piRangoIni) + 1;
	IF (viCantTar = piCantidadRec) THEN
		--Valida que la sucursal no cuente con un envio con el mismo numero de envio y tipo de tarjeta que los proporcionados por el Sistema Puente Coppel - BancCoppel.
		IF NOT EXISTS(SELECT cvesucursal, numenvio, tipotarjeta FROM bditarjcop:"informix".enviostarcop WHERE cvesucursal = psCveSucursal 
					  AND numenvio = piNumEnvio AND tipotarjeta = psTipoTarjeta AND rangoini = piRangoIni AND rangofin = piRangoFin) THEN
			--Verifica si el numero de guia y la fecha de guia estan en blanco o nulos.
			IF(((psNumGuia = "") OR (psNumGuia IS NULL)) AND ((pdFechaGuia = "") OR (pdFechaGuia IS NULL)))THEN
				LET psNumGuia = '';
				LET pdFechaGuia = '1900-01-01 00:00:00';
			ELIF((pdFechaGuia = "") OR (pdFechaGuia IS NULL))THEN
				LET psNumGuia = psNumGuia;
				LET pdFechaGuia = '1900-01-01 00:00:00';
			ELSE
				LET psNumGuia = psNumGuia;
				LET pdFechaGuia = pdFechaGuia;
			END IF;
				--Guarda los siguientes datos del envio
				INSERT INTO bditarjcop:"informix".enviostarcop(
				tiposurtido, empresa, cvesucursal, tipotarjeta, numenvio, fechasurt, cantidadrec, rangoini, rangofin, fecharec, enviodisponible, ulttarasignada, empleadorec, numguia, fechaguia)
				VALUES(
				psTipoSurtido, psEmpresa, psCveSucursal, psTipoTarjeta, piNumEnvio, pdFechaSurt, piCantidadRec, piRangoIni, piRangoFin, '1900-01-01 00:00:00', 'P',
				piRangoIni - 1, '', TRIM(psNumGuia), pdFechaGuia);
				LET vsCodRet = '00000';
		ELSE
			IF EXISTS(SELECT cvesucursal, numenvio, tipotarjeta FROM bditarjcop:"informix".enviostarcop WHERE cvesucursal = psCveSucursal 
					  AND numenvio = piNumEnvio AND tipotarjeta = psTipoTarjeta AND numguia = "" OR numguia IS NULL AND fechaguia = '1900-01-01 00:00:00' OR fechaguia IS NULL)THEN
				UPDATE bditarjcop:"informix".enviostarcop SET numguia = psNumGuia, fechaguia = pdFechaGuia WHERE empresa = psEmpresa AND cvesucursal = psCveSucursal 
					  AND numenvio = piNumEnvio AND tipotarjeta = psTipoTarjeta;
				LET vsCodRet = '00000';
			ELSE
				--El numero de envio y tipo de tarjeta con numero de guia y fecha indicado ya han sido asignados a esa sucursal.
				LET vsCodRet = '00502';
			END IF
		END IF;
	ELSE
		--Los montos de tarjetas reportados no corresponden.
		LET vsCodRet = '00501';
	END IF;
ELSE
	--La sucursal no esta en operacion y no se le pueden asignar envios.
	LET vsCodRet = '00500';
END IF;
...
CREATE PROCEDURE "informix".sp_au_guardarbitacora(psEmpleado VARCHAR(8), psActividad VARCHAR(20), psNomArchivo VARCHAR(20), psCodRetorno VARCHAR(5))
...
CREATE PROCEDURE "informix".sp_au_obtenerrepositorios(psEmpresa VARCHAR(3))

RETURNING VARCHAR(90) AS Ruta_AIX, VARCHAR(90) AS Ruta_WIN;

--****************************************************************************************************
-- DESCRIPCION: OBTIENE LAS RUTAS DE LOS REPOSITORIOS WINDOWS Y AIX PARA LA CARGA DE LOS ARCHIVOS DE SURTIDO
-- AUTOR : Casanova Edeza Hector Juan
-- FECHA : 26/12/2011
-- BD: bdiTarCop
-- SISTEMA : Inventario de Tarjetas Alta Unica.
...
BEGIN
  ...
  ON EXCEPTION SET viSqlError    --cacha el error en caso de que exista y regresa un valor predeterminado
	...
	SET ISOLATION TO DIRTY READ;
	--OBTIENE LA RUTA AIX
	SELECT FIRST 1 NVL(Valor, '') INTO vsRutaWIN FROM BdiTarjCop:"informix".ParamInvTarCop WHERE Descripcion = "REPOSITORIO WIN" AND Empresa = psEmpresa;
	
	SET LOCK MODE TO WAIT 3;
	SET ISOLATION TO DIRTY READ;
	--OBTIENE LA RUTA WINDOWS
	SELECT FIRST 1 NVL(Valor, '') INTO vsRutaAIX FROM BdiTarjCop:"informix".ParamInvTarCop WHERE Descripcion = "REPOSITORIO AIX" AND Empresa = psEmpresa;
	
	RETURN NVL(vsRutaAIX, ''), NVL(vsRutaWIN, '');
