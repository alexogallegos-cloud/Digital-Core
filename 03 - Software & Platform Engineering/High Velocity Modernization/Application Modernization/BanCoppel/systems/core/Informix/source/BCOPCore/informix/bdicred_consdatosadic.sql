CREATE PROCEDURE "informix".consdatosadic(pEmpresa char(3), pNumeroCredito char(20), pNumeroCte char(20)) 
--DATOS A REGRESAR
RETURNING
...
IF EXISTS(SELECT num_credito FROM bdicred:sd_tarjeta where num_credito = pNumeroCredito) THEN

	FOREACH
	SELECT DISTINCT 
		sd_tar.numcte, si_cte.apell_paterno, si_cte.apell_materno, si_cte.nombre1, si_cte.nombre2, si_cte.rfc, sd_tar.num_tarjeta,
		sd_tar.expiracion, sd_tar.limite_aut, sd_tar.status_tar, sd_tar.tipo_tarjeta, si_pf.fecha_nac, sd_mae.num_producto 
	INTO
		vNumCte,  vApell_Paterno, vApell_Materno, vNombre1, vNombre2, vRfc, vNumTarjeta, vExpiracion, vLimRetXmes, vStatusTarj, vTipoCte,
		vFechaNacimiento,  vProductoCredito
		
	FROM
		bdinteg:si_cliente si_cte,
		bdicred:sd_tarjeta sd_tar,
		bdicred:sd_maecred sd_mae, 
		bdinteg:si_ctepf si_pf
	WHERE
		sd_tar.empresa = pEmpresa AND sd_tar.numcte != pNumeroCte AND sd_tar.numcte = si_cte.numcte AND 
		si_cte.empresa = pEmpresa AND sd_mae.numcte= pNumeroCte AND sd_mae.empresa = pEmpresa AND 
		sd_tar.numcte = si_pf.numcte AND sd_tar.tipo_tarjeta = 'A' AND 
