CREATE PROCEDURE "informix".sp_relacioncta_cred(pEmpresa CHAR(3), pNumCliente CHAR(20), pNumCredisol CHAR(20),  pNumCta CHAR(16))
...
DEFINE inumcredisol         INTEGER; -- INC 56 165
...
LET inumcredisol 		= 0; -- INC 56 165


 --SET DEBUG FILE TO "/tmp/sp_relacioncta_cred.out";
 --TRACE ON;

    SET ISOLATION TO DIRTY READ;
    --SET PDQPRIORITY 10; HMD-INCIDENCIA-20220224

	IF NVL(pEmpresa,'') = '' or NVL(pNumCliente,'') = '' or  NVL(pNumCredisol,'') = '' or NVL(pNumCta,'') = '' THEN
		LET cCodRet = '000001';
		LET Mensaje   = "Parametros vacios, no se guardaran los datos";
	ELSE

		-- INC 56 165 06082019 AAME Se Valida si hay registro por credisolucion para eliminar y volver a insertar {
		-- Se modifica la validacion de [CLIENTE - CUENTA - CREDISOLUCION] A [CLIENTE - CREDISOLUCION]
		Select count(numcredisol)
		INTO inumcredisol
		from  bdicred: "informix".sd_verif_cuentas_crd
		where numcliente = pNumCliente and numcredisol = pNumCredisol;

		IF inumcredisol > 0 THEN
			DELETE FROM bdicred: "informix".sd_verif_cuentas_crd  WHERE numcliente = pNumCliente AND numcredisol = pNumCredisol;
		END IF;
		--If NOT  exists  (Select numcredisol from  bdicred: "informix".sd_verif_cuentas_crd  where numcliente = pNumCliente and numcta = pNumCta and numcredisol = pNumCredisol) Then
		-- } INC 56 165 06082019 AAME
		
			INSERT INTO bdicred: "informix".sd_verif_cuentas_crd (empresa,numcliente,numcta,numcredisol,fecha)
			VALUES (pEmpresa,pNumCliente,pNumCta,pNumCredisol,current);
			IF DBINFO("sqlca.sqlerrd2") = 0 THEN
				LET cCodRet = '000002';
				LET Mensaje   = "Fallo insercion de datos, no se guardaran los datos";
			ELSE
				LET cCodRet = "000000";
				LET Mensaje = "Se realizo el proceso exitosamente";
			END IF;
...
'Folio: 126',
'Descripcion: Se crea procedimiento para almacenar relaciones de las cuentas de clientes con las cuentas de credisoluciones. Se guardara el registro en la tabla sd_verif_cuentas_crd',
'Fecha: 11/11/2016',
'Version: 1',
'BD: bdicred',
'**********************************************************************************************************************************************************',
' MODIFICO: Maria Elena Angulo Aispuro ',
' RQ: INC 56 165 - Error -284 Informix- Pago anticipado de credisolucion con cargo a cuenta ',
' DESCRIPCION: Se Valida si hay registro por credisolucion para eliminar y volver a insertar. ',
'              Se modifica la validacion de [CLIENTE - CUENTA - CREDISOLUCION] A [CLIENTE - CREDISOLUCION]',
' FECHA: 06/08/2019 ',
' CC: 35126 ',
'***********************************************************************************************************************************************************';

CREATE PROCEDURE "informix".sp_relacioncta_cred_web(pEmpresa CHAR(3), pNumCliente CHAR(20), pNumCredisol CHAR(20),  pNumCta CHAR(16))
