CREATE PROCEDURE "informix".sp_carga_clientes_camp() 
RETURNING CHAR(6);
...
DEFINE iValidaSuc				INTEGER; -- CAX 202410 
...
BEGIN
  ...
  --SET DEBUG FILE TO '/ifxsif01/aame/sp_carga_clientes_camp.out';
  --TRACE ON;
	...
	IF NVL(cRuta,'') <> '' THEN
		...
		--	LET cDatosCtesCamp = TRIM(cDatosCtesCamp)||'_'||YEAR(dtCampAct)||LPAD(MONTH(dtCampAct),2,0)||LPAD(DAY(dtCampAct),2,0)||'.txt';      
		   ...
		   system '/usr/bin/sh ' || TRIM(cRuta)|| 'dbload_datosctes.sh';  
			...
			-- AAME 20190822 INC 27 131 Se valida si no encontro el archivo Ã³ no fue posible cargarlo
			...
			ELSE
				LET cCodRet = '000002'; 
				LET cCadena = '';
				LET cCadena = '/usr/bin/echo " EL ARCHIVO SE ENCUENTRA VACÃO, CON ERROR DE ESTRUCTURA Ã INEXISTENTE " > ' || TRIM(cRuta) || TRIM(cBitCamp);
				SYSTEM cCadena;						
			END IF;
		END IF;
		...
		IF cCodRet = '000000' THEN 
			...
			FOREACH WITH HOLD
				...
				-- CAX 2024 se agrega validacion para sucursal operativa
				SELECT count(*) INTO iValidaSuc FROM bdinteg:si_sucursales WHERE sucursal = vsucursal and tpo_sucursal = 'S';				

				IF iDatosCorrectos = 0 or iValidaSuc =0  THEN
...
'--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------',
'MODIFICACIÃN',
'RQ         : INC 27 127 CorrecciÃ³n de error en credisoluciones',
'FECHA      : 28 de Junio de 2019',
'DESCRIPCIÃN: Se agregÃ³ la validaciÃ³n de la sucursal de apertura del crÃ©dito tarjeta y nÃºmero de cliente para evitar credisoluciones con error por captura o lectura incorrecta',
