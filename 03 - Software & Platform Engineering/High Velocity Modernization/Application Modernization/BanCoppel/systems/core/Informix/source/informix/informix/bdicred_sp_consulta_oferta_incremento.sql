CREATE PROCEDURE "informix".sp_consulta_oferta_incremento(pEmpresa Char(3),pNumCliente Char(20), pNumTarjeta Char(16))

--DATOS A REGRESAR---												 
RETURNING CHAR(6) AS cCodRet,
          CHAR(20) AS cNumCredito,
          CHAR(8) AS cOferta_incremento_linea,
          DECIMAL(18,2) AS dLinea_prestamo_anterior,
          DECIMAL(18,2) AS dLinea_prestamo_actual;
	  
--DECLARACIONES.
DEFINE ccCodRet                  CHAR(6);
DEFINE ccNumCredito              CHAR(20);
DEFINE ccOferta_incremento_linea CHAR(8);
DEFINE cLinea_prestamo_anterior  DECIMAL(18,2);
DEFINE cLinea_prestamo_actual    DECIMAL(18,2);
DEFINE vFrecRecordatorio         INTEGER;
...
LET ccCodRet                  = "000001";
LET ccNumCredito              = '';
LET ccOferta_incremento_linea = '';
LET cLinea_prestamo_anterior  = 0;
LET cLinea_prestamo_actual    = 0;
LET vFrecRecordatorio         = 0;
LET iSecuencia                = 0;

BEGIN
    ON EXCEPTION SET iSqlErr	
		IF 	iSqlErr <> 0 THEN
			LET ccCodRet =  iSqlErr;
			RETURN ccCodRet,ccNumCredito,ccOferta_incremento_linea,cLinea_prestamo_anterior,cLinea_prestamo_actual;
		END IF;
	END EXCEPTION;
    ...
    SELECT CAST(TRIM(valor) AS INTEGER) INTO vFrecRecordatorio FROM bdicred:"informix".sd_param WHERE cod_param = '203';
	
		IF TRIM(pNumCliente) = '' AND TRIM(pNumTarjeta) = '' THEN  --CUANDO NO TRAE NUMERO DE CLIENTE NI NUMERO DE CREDITO
			LET ccCodRet = '000001';			
			RETURN ccCodRet,ccNumCredito,ccOferta_incremento_linea,cLinea_prestamo_anterior,cLinea_prestamo_actual;
		ELSE		    		    
		    IF TRIM(pNumCliente) <> ''  AND TRIM(pNumTarjeta) = ''  THEN 
		       
		       --OBTIENE SECUENCIA
		       SELECT {+INDEX(bdicred:"informix".sd_incrementos_prestamos_digitales idx_incrmto_pd)}
			   MAX(secuencia) INTO iSecuencia FROM bdicred:"informix".sd_incrementos_prestamos_digitales 
			   WHERE numcte = pNumCliente AND empresa = pEmpresa;
		       
		       --CONSULTA LA SI LA OFERTA DEL CLIENTE ESTA ACTIVA
			   SELECT FIRST 1 {+INDEX(bdicred:"informix".sd_incrementos_prestamos_digitales idx_incrmto_pd3)}
			   num_credito, oferta_incremento_linea, linea_prestamo_anterior, linea_prestamo_actual 
			   INTO ccNumCredito,ccOferta_incremento_linea,cLinea_prestamo_anterior,cLinea_prestamo_actual
               FROM bdicred:"informix".sd_incrementos_prestamos_digitales 
			   WHERE numcte = pNumCliente 
			   AND empresa= pEmpresa 
			   AND oferta_incremento_linea = 'Activa'
			   AND (fecha_prox_recordatorio IS NULL OR fecha_prox_recordatorio = TODAY)
               AND secuencia = iSecuencia;
             		 
               --SE VALIDA SI LA OFERTA ESTA ACTIVA	  	    	      
		       IF UPPER(TRIM(ccOferta_incremento_linea)) = 'ACTIVA' THEN		      
                 LET ccCodRet = '000000';
                 RETURN ccCodRet,ccNumCredito,ccOferta_incremento_linea,cLinea_prestamo_anterior,cLinea_prestamo_actual;
               ELSE
                 LET ccCodRet = '000002';
            ...
            ELSE
                IF TRIM(pNumCliente) = '' AND TRIM(pNumTarjeta) <> '' THEN
                   ...
                   --OBTIENE SECUENCIA                    
                    SELECT {+INDEX(bdicred:"informix".sd_incrementos_prestamos_digitales idx_incrmto_pd3)}
					MAX(IPD.secuencia) INTO iSecuencia
                    FROM bdicred:"informix".sd_incrementos_prestamos_digitales IPD
                    INNER JOIN bdicheq:"informix".sc_tarjeta TAR ON IPD.numcte = TAR.numcte AND TAR.status_tar='A'
                    WHERE TAR.num_tarjeta= pNumTarjeta AND TAR.empresa = pEmpresa;
                    
                    --CONSULTA LA SI LA OFERTA DEL CLIENTE ESTA ACTIVA
                    SELECT FIRST 1 {+INDEX(bdicred:"informix".sd_incrementos_prestamos_digitales idx_incrmto_pd3),
					+INDEX(bdicred:"informix".sd_incrementos_prestamos_digitales idx_incrmto_pd)}
					IPD.num_credito, IPD.oferta_incremento_linea, IPD.linea_prestamo_anterior, IPD.linea_prestamo_actual 
                    INTO ccNumCredito, ccOferta_incremento_linea,cLinea_prestamo_anterior,cLinea_prestamo_actual               
                    FROM bdicred:"informix".sd_incrementos_prestamos_digitales IPD
                    INNER JOIN bdicheq:"informix".sc_tarjeta TAR ON IPD.numcte=TAR.numcte AND TAR.status_tar='A'
                    WHERE TAR.num_tarjeta=pNumTarjeta AND TAR.empresa=pEmpresa AND IPD.oferta_incremento_linea='Activa'
                    AND (IPD.fecha_prox_recordatorio IS NULL OR IPD.fecha_prox_recordatorio = TODAY)
                    AND IPD.secuencia = iSecuencia;
                    ...
                    END IF;
                ELSE
                   IF  TRIM(pNumCliente) <> '' AND TRIM(pNumTarjeta) <> '' THEN --CUANDO TRAE EL NUMERO DE CLIENTE Y EL NUMERO TARJETA A VEZ
		               LET ccCodRet = '000003';
		               RETURN ccCodRet,ccNumCredito,ccOferta_incremento_linea,cLinea_prestamo_anterior,cLinea_prestamo_actual;
                   END IF;
                END IF;      
...
'Fecha: 11-06-2023',
'ModificaciÃ³n:  Se crea un SP para Consultar Oferta de incremento de linea.',
'Base de datos: bdicred,bdicheq',
'Solicito: ';
