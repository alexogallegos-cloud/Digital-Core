CREATE PROCEDURE "informix".sp_consultacodigoascii(pCodigo CHAR(4))
...
BEGIN
	ON EXCEPTION SET iSqlErr
		IF iSqlErr != 0 THEN
			LET cCodRet= iSqlErr;
			RETURN NVL(cCodRet,''), NVL(cCodigo,''), NVL(cDescripcion,'');
		END IF;
	END EXCEPTION;
	
--SET debug FILE TO "/home/Antonio/sp_consultacodigoascii.out";
--Trace ON;
	SET ISOLATION TO CURSOR STABILITY;
	SET LOCK MODE TO WAIT 3;
	-- Valida que almenos envies una consulta
	IF pCodigo IS NULL OR pCodigo = '' THEN
		LET cCodRet = '000001';
		RETURN NVL(cCodRet,''), NVL(cCodigo,''), NVL(cDescripcion,'');
	END IF;
	
	--Consulta el catalogo ascii
	SELECT id, caracter
	INTO cCodigo,cDescripcion
	FROM bdinteg:"informix".si_catalogoascii 
	WHERE id = pCodigo 
	OR caracter = pCodigo;
	
	--Devuelve el valor si es que la consulta anterior se efectuo
...
END PROCEDURE
Document
'DESCRIPCION: Consulta de catalogo de codigo ascii obteniendo su codigo o descripcion', 
'AUTOR: Antonio Bastidas',
'FECHA: 30/09/2011',
'VERSION: 20110930.1642',
'BD: bdinteg';
