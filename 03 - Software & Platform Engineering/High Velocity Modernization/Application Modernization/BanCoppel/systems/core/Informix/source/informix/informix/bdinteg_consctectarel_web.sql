CREATE PROCEDURE "informix".consctectarel_web(pempresa CHAR(3),pcliente CHAR(20),
                                          pnumeroregistro smallint)
										  
  RETURNING CHAR(5),CHAR(2),CHAR(35),CHAR(20),CHAR(2),CHAR(30),
            CHAR(2),CHAR(20),CHAR(60);

DEFINE vcodret CHAR(5);
DEFINE vsqlerr,visamerr INTEGER;

DEFINE vcsistema        CHAR(2);
DEFINE vcsistemades     CHAR(35);
DEFINE vccuenta         CHAR(20);
DEFINE vcrelacion       CHAR(2);
DEFINE vcrelaciondes    CHAR(30);
DEFINE vcparentesco     CHAR(2);
DEFINE vcparentescodes  CHAR(20);
DEFINE vnombrecliente   CHAR(60);
...
BEGIN
	ON EXCEPTION SET vsqlerr,visamerr
	   IF vsqlerr != 0 THEN
		  LET vcodret = vsqlerr;
		  RETURN vcodret,vcsistema,vcsistemades,vccuenta,vcrelacion,
				 vcrelaciondes,vcparentesco,vcparentescodes,vnombrecliente;
	   END IF;
	END EXCEPTION;
	...
	FOREACH
		SELECT {+INDEX(bdinteg:si_sistema idx_sistema)} cr.sistema,st.descripcion,cr.cuenta,cr.tipo_relacion,tr.descripcion,cr.parentesco,pt.descripcion
		INTO     vcsistema,vcsistemades,vccuenta,vcrelacion,vcrelaciondes,vcparentesco,vcparentescodes
		FROM si_cterelacionado cr
		   LEFT OUTER JOIN si_sistema st ON (trim(st.siglas) = cr.sistema)
		   LEFT OUTER JOIN si_tiporelacion tr ON (tr.empresa = pempresa and tr.tipo_relacion = cr.tipo_relacion)
		   LEFT OUTER JOIN si_parentesco pt ON (pt.empresa = pempresa and pt.parentesco = cr.parentesco)
		WHERE cr.empresa = pempresa and  cr.numcte = pcliente
		ORDER BY sistema

	  -- Extrae el Nombre del Cliente de la Cuenta
	    SELECT  trim(nvl(nombre1,""))||" "||trim(nvl(nombre2,""))||" "||
		  trim(nvl(apell_paterno,""))||" "||trim(nvl(apell_materno,""))||
		  " "||trim(nvl(razon_social,"")) as nombre
	    INTO  vnombrecliente
	    FROM  si_cliente as a , bdicheq:sc_maechq as b
	    WHERE a.numcte= b.num_cte
	    AND   b.cuenta = vccuenta;


	    LET vciclo = vciclo+1;
	    IF vciclo <= pnumeroregistro THEN
		   continue foreach;
	    END IF
	    RETURN  vcodret,vcsistema,vcsistemades,vccuenta,vcrelacion,
			  vcrelaciondes,vcparentesco,vcparentescodes,vnombrecliente
...
END PROCEDURE
DOCUMENT
"Consulta de cuentas relacionadas del cliente",
"Autor : Daniel Zambada S.A. de C.V.",
