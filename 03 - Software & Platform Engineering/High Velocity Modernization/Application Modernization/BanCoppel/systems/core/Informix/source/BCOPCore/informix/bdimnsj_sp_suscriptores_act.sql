CREATE PROCEDURE "informix".sp_suscriptores_act()
...
--****************************************************************************************************
-- DESCRIPCION: GeneraciÃÂ³n de actualizaciÃÂ³n de suscriptores.
...
-- y se procesen el dÃÂ­a siguiente, todos los correos que llegaron en la noche del dia anterior.
-- SOLICITA: Jaime Gonzalez Prado
-- AUTOR : Juan Francisco Ponce
-- FECHA : 10/01/2024
--****************************************************************************************************
...
DEFINE vsDia2             CHAR(2);
DEFINE vsMes2 		      CHAR(2);
DEFINE vsAnio2 		      CHAR(2);
DEFINE cMax 		      CHAR(10);
...
DEFINE vsfechacomp2		  CHAR(10);
DEFINE vistatus			  INTEGER;
DEFINE vcofetel			  CHAR(1);
DEFINE vsvalido			  CHAR(1);
...
--SET DEBUG FILE TO "/informix/ragomez/sp_suscriptores_act/20032024/sp_suscriptores_act.out";
--TRACE ON;
...
BEGIN
	...
	ON EXCEPTION SET viSqlError,isam_error,vsMensaje
		IF (viSqlError != 0) THEN
			...
			LET visam_error = isam_error;
				...
				VALUES (vsCodRetorno,visam_error, vsMensaje, 'sp_suscriptores_act', vdFechaHoy,CURRENT);		 
				
	             EXECUTE PROCEDURE  bdimnsj:"informix".sp_registra_evento_prod('1','PRO_ALERS','GRUPO_MANTO4','1','1','1','ERROR BD','10.36.197.51','sp_registra_evento_prod','FECHA NO PROCESADA: ' || date(current),'','','','','','','','','','1','1','1','1',date(current),date(current)) INTO vsCodRetorno2;
	...
	VALUES('ACT_SUSC',vdFechaHoy,vistatus,'informix',CURRENT);
	...
	LET vsDia2 =substr(CAST(pFecha2 AS DATETIME YEAR TO FRACTION(5)),9,2);
	LET vsMes2 =substr(CAST(pFecha2 AS DATETIME YEAR TO FRACTION(5)),6,2);
	LET vsAnio2 =substr(CAST(pFecha2 AS DATETIME YEAR TO FRACTION(5)),3,2);
	LET vsfechacomp2 = '20'||vsAnio2||'-'||vsMes2||'-'||vsDia2||'%';
	...
	IF (vsCodRetorno='00000') THEN
		...
		FOREACH
			select  cte.numcte,cte.apell_paterno,cte.apell_materno,cte.nombre1,cte.nombre2,pf.sexo
				...
				where cte.status_cte = 'AL' and co.status_correo ='A' and co.fecha_hora LIKE vsfechacomp and co.valido='1'
				--where cte.status_cte = 'AL' and co.status_correo ='A' and substr(co.fecha_hora,1,10)= vsfechacomp and co.valido='1'
			union
			select  cte.numcte,cte.apell_paterno,cte.apell_materno,cte.nombre1,cte.nombre2,pf.sexo
				from bdinteg:"informix".si_cliente cte  inner join
				bdinteg:"informix".si_ctepf as pf on cte.numcte = pf.numcte
				inner join bdinteg:"informix".si_telefonos_actual as tel on cte.numcte = tel.numcte
				where cte.status_cte = 'AL' and tel.tipo_tel = 2 and DATE(tel.fecha_hora)= pFecha 
				--where cte.status_cte = 'AL' and tel.tipo_tel = 2 and substr(tel.fecha_hora,1,10)= vsfechacomp 
			union --Nuevo codigo para recuperar los correos de ayer
			select  cte.numcte,cte.apell_paterno,cte.apell_materno,cte.nombre1,cte.nombre2,pf.sexo
				from bdinteg:"informix".si_cliente cte  inner join
				bdinteg:"informix".si_ctepf as pf on cte.numcte = pf.numcte
				inner join bdinteg:"informix".si_correos as co on cte.numcte = co.numcte
				where cte.status_cte = 'AL' and co.status_correo ='A' and co.fecha_hora LIKE vsfechacomp2
				--where cte.status_cte = 'AL' and co.status_correo ='A' and substr(co.fecha_hora,1,10)= vsfechacomp2 --(fecha de ayer)
				and DATE(co.fecha_valida)= pFecha and co.valido='1'
			--Nombre del archivo
...
--				INTO vsCodRet2,vscorreo,vstipocorreo,vsStatuscorreo;
				select first 1 correo_elec,valido into vscorreo,vsvalido from bdinteg:"informix".si_correos where numcte=vsnumcte and status_correo='A';
				--Se modifica para que solo se suban los mails validos.
				IF (vsvalido IS NULL OR vsvalido != '1') THEN -- Se verifica si el correo del cliente es invalido para actualizar al cliente
					LET vscorreo = '';
				END IF;
...
/*				IF vsCodRet2 <> '000' THEN
			...
			END IF;
				...
				IF vscorreo IS NOT NULL OR (vstelefono IS NOT NULL AND vcofetel = 'V') THEN
					...
					IF vscorreo IS NULL OR vscorreo = '' THEN
						...
						LET vsStmt5 =  'SETALL_USER_PROPS'||'|'||trim(cCompany)||'|'||trim(vsnumcte)||'|'|| 'celular' ||'='||trim(nvl(vstelefono,''));
						INSERT INTO bdimnsj:'informix'.mnsj_susc_paso (linea)
						VALUES (vsStmt5);
					...
					ELIF vstelefono IS NULL OR vstelefono = '' THEN
		...
		IF vsCodRetorno = '00000' THEN
			...
			UPDATE "informix".mnsj_procesos set status = '1' WHERE proceso = 'ACT_SUSC'
