CREATE PROCEDURE "informix".sp_consulparam_cajagensuc(pSucursal CHAR(4))
--DATOS A REGRESAR---												 
RETURNING CHAR(6) AS cCod_ret,
		  CHAR(20) AS cMontoMax, 
		  CHAR(20) AS cMontoMin;
	  
--DECLARACIONES.
DEFINE iSqlErr INTEGER;
DEFINE cCod_ret CHAR(6);
DEFINE cMontoMax CHAR(20);
DEFINE cMontoMin CHAR(20);
...
BEGIN
    ON EXCEPTION SET iSqlErr	
		IF 	iSqlErr <> 0 THEN
			...
			RETURN cCod_ret, cMontoMax, cMontoMin;
		END IF;
	END EXCEPTION;
	...
	-- SET DEBUG FILE TO "/home/sysifx/Alexis/97.out";
	-- TRACE ON;
	
	IF NVL(pSucursal,'') <> '' THEN
		SELECT mto_max_efect
		INTO cMontoMax
		FROM bdinteg:"informix".si_sucursales 
		WHERE sucursal = LPAD(pSucursal,4,'0');
		...
		IF NVL(cMontoMax,'') = '' OR NVL(cMontoMin,'') = '' THEN
			LET cCod_ret = '000002';
		ELSE
			LET cMontoMax = REPLACE(cMontoMax,'$','');
			LET cMontoMin = REPLACE(cMontoMin,'$','');
		END IF;
	ELSE
		LET cCod_ret = '000001';
	END IF;
	RETURN cCod_ret, NVL(cMontoMax,''), NVL(cMontoMin,'');
END;
END PROCEDURE
DOCUMENT
'Autor: 97247642 - Oscar Alexis Ibarra Verdugo',
'Folio: 97-FaltantesSobrantesPanAmericano',
'Fecha: 28-07-2016',
'Modificación: Se crea procedimiento para asignar el valor de monto maximo y monto minimo',
'Sustento: 5_RQI 14 193 - Transacciones de faltante y sobrante de Pan Americano.pdf	',
'Solicita: Abraham Narvaez Jacinto',
'Base de datos: bdisuc';

CREATE PROCEDURE "informix".sp_concensuc(pempresa CHAR(3),
		psucursal CHAR(4),
...
--SET debug file to "/tmp/concensuc.out";
--trace on;
