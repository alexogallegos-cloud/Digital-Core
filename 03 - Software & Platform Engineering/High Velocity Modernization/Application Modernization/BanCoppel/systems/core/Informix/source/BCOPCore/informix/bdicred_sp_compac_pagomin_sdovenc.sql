CREATE PROCEDURE "informix".sp_compac_pagomin_sdovenc(pEmpresa CHAR(3), pFechaIni DATE, pFechaFin DATE)
...
BEGIN
  ...
  LET dtFechaFin  = pFechaFin;
    ...
    LET cNombreArchivo= TRIM(cNombreArchivo2)|| TRIM(cArch_pagomin) || TRIM(cRandomId);
    ...
    LET cConsulta = 'SELECT sucursal,fecha_insert,cant_a_recup_pm,cant_recup_pm,Pct_PM_Recup,count_con_pagomin,count_sin_pagomin,' ||
			...
			LET cSql = '';
			LET cSql = 'echo "UNLOAD TO ' ||TRIM(cRuta)||TRIM(cNombreArchivo)||'.'||'txt'|| ' '||TRIM(cConsulta)||'" > '|| TRIM(cRuta) ||'query001.sql';
      ...
      LET cSql = 'echo "sucursal|Fecha|$ a Recup.PM|$ Recup.PM|%Cump.Recup PM|# de PM|# Sin PM|%Cump. #PM|$ a Recup.SV|$ Recup.SV|%Cump.Recup SV|# de Vencidos|# sin Vencidos|' ||
                      '%Cump. #Vencidos|%Cump.Recup.Cartera' || '" >> '|| TRIM(cRuta) || TRIM(cNombreArchivo) || '.txt';        	 
      SYSTEM TRIM(cSql);
