CREATE PROCEDURE "informix".sp_depura_sd_archivopagmin()
RETURNING CHAR(5) AS v_cod_ret;
-- ****************************************************************************
...
DEFINE	pfecha_registro 				DATE;
DEFINE  cnumcte 						char(20);
DEFINE  cnum_credito 					char(20);
DEFINE  isdo_cap_insoluto 				DECIMAL(18,2);
DEFINE  i_interes  						Decimal(18,2);
DEFINE  i_iva 							DECIMAL(18,2);
DEFINE  imonto_vencido_corte 			DECIMAL(18,2);
DEFINE  imto_fin_ven_trasp  			INTEGER;
DEFINE  imonto_otorgado  				DECIMAL(18,2);
DEFINE  isdo_trab4 						DECIMAL(18,2);
DEFINE	ifactor_porcentual 				DECIMAL (18,2);
DEFINE	itope_minimo 					DECIMAL (18,2);
DEFINE	ifactor_pago_min_linc 			DECIMAL (4,4);
DEFINE	ifac_pagm_suma_sdo 				DECIMAL (4,4);
DEFINE	ces_totalero 					CHAR(2);
DEFINE	isdo_nvo_pfsi 					DECIMAL(14,2);
DEFINE	isdo_hist_pfsi  				DECIMAL(14,2);
DEFINE	isdo_dia_pfsi  					DECIMAL(14,2);
DEFINE	isdo_capital  					MONEY(14,2);
DEFINE	icap_tras_no 					MONEY(14,2);
DEFINE	itotal_adeudo 					MONEY(14,2);
DEFINE	isdo_orig_pagmin 				MONEY(14,2);
DEFINE	imto_venc_trasp 				MONEY(14,2);
DEFINE	imonto_vencido  				DECIMAL(14,2);
DEFINE	imonto_financiado 				MONEY(14,2);
DEFINE	ical_iva      					DECIMAL(14,2);
DEFINE	ical_interes 					DECIMAL(14,2);
DEFINE	cstatus_cred 					CHAR (5);
-- ****************************************************************************
...
LET p_fecha				    = TODAY;
LET pFechaAnt               = TODAY;
LET pfecha_registro 		= TODAY;
LET cnumcte 				= "";
LET cnum_credito 			= "";
LET isdo_cap_insoluto 		= 0;
LET i_interes  				= 0;
LET i_iva 					= 0;
LET imonto_vencido_corte 	= 0;
LET imto_fin_ven_trasp  	= 0;
LET imonto_otorgado  		= 0;
LET isdo_trab4 				= 0;
LET ifactor_porcentual 		= 0;
LET itope_minimo 			= 0;
LET ifactor_pago_min_linc 	= 0;
LET ifac_pagm_suma_sdo 		= 0;
LET ces_totalero 			= "";
LET isdo_nvo_pfsi 			= 0;
LET isdo_hist_pfsi  		= 0;
LET isdo_dia_pfsi  			= 0;
LET isdo_capital  			= 0;
LET icap_tras_no 			= 0;
LET itotal_adeudo 			= 0;
LET isdo_orig_pagmin 		= 0;
LET imto_venc_trasp 		= 0;
LET imonto_vencido  		= 0;
LET imonto_financiado 		= 0;
LET ical_iva      			= 0;
LET ical_interes 			= 0;
LET cstatus_cred 			= "";
...
BEGIN
	...
	--SET LOCK MODE TO WAIT 3;
	--SET DEBUG FILE TO "/tmp/AA/GeneraArchivo.out";
	--TRACE ON;
	 --SET DEBUG FILE TO "/home/e90228206/jobs_inflacion/trace/sp_depura_sd_archivopagmin.out";
	--TRACE ON;
	
-- ****************************************************************************
-- *                 Depurar tabla sd_archivopagmin                           *
-- ****************************************************************************	

     SELECT fecha_hoy
            INTO p_fecha
        FROM bdicred:sd_fechas WHERE empresa = '001';
 --   LET p_fecha =   LPAD(TRIM(MONTH(p_fecha)::CHAR(2)),2,'0') '-' || || LPAD(TRIM(DAY(p_fecha  DAY)::CHAR(2)),2,'0') || '-' || YEAR(p_fecha);

  LET pFechaAnt = p_fecha - 2 UNITS YEAR;
   
	FOREACH WITH HOLD
	SELECT  {+index(bdicred:"informix".idx_sd_archivopagmin)}fecha_registro,numcte,num_credito,sdo_cap_insoluto,
		   interes,iva,monto_vencido_corte,mto_fin_ven_trasp,monto_otorgado,sdo_trab4,factor_porcentual,
		   tope_minimo,factor_pago_min_linc,fac_pagm_suma_sdo,es_totalero,sdo_nvo_pfsi,sdo_hist_pfsi,
		   sdo_dia_pfsi,sdo_capital,cap_tras_no,total_adeudo,sdo_orig_pagmin,
		   mto_venc_trasp,monto_vencido,monto_financiado,cal_iva,cal_interes,status_cred
	INTO   pfecha_registro,cnumcte,cnum_credito,isdo_cap_insoluto,i_interes,i_iva,imonto_vencido_corte,imto_fin_ven_trasp,
			imonto_otorgado,isdo_trab4,ifactor_porcentual,itope_minimo,ifactor_pago_min_linc,ifac_pagm_suma_sdo,ces_totalero,isdo_nvo_pfsi,
			isdo_hist_pfsi,isdo_dia_pfsi,isdo_capital,icap_tras_no,itotal_adeudo,isdo_orig_pagmin,imto_venc_trasp,imonto_vencido,
			imonto_financiado,ical_iva,ical_interes,cstatus_cred
	FROM  bdicred:"informix".sd_archivopagmin 
	WHERE fecha_registro <= pFechaAnt
	
	
	  INSERT INTO bdicred:"informix".sd_archivopagmin_hist(fecha_registro,numcte,num_credito,sdo_cap_insoluto,interes,
		   iva,monto_vencido_corte,mto_fin_ven_trasp,monto_otorgado,sdo_trab4,factor_porcentual,
		   tope_minimo,factor_pago_min_linc,fac_pagm_suma_sdo,es_totalero,sdo_nvo_pfsi,sdo_hist_pfsi,sdo_dia_pfsi,
		   sdo_capital,cap_tras_no,total_adeudo,sdo_orig_pagmin,
		   mto_venc_trasp,monto_vencido,monto_financiado,cal_iva,cal_interes,status_cred)								
		 VALUES(pfecha_registro,cnumcte,cnum_credito,isdo_cap_insoluto,i_interes,i_iva,imonto_vencido_corte,imto_fin_ven_trasp,
				imonto_otorgado,isdo_trab4,ifactor_porcentual,itope_minimo,ifactor_pago_min_linc,ifac_pagm_suma_sdo,ces_totalero,isdo_nvo_pfsi,
				isdo_hist_pfsi,isdo_dia_pfsi,isdo_capital,icap_tras_no,itotal_adeudo,isdo_orig_pagmin,imto_venc_trasp,imonto_vencido,
				imonto_financiado,ical_iva,ical_interes,cstatus_cred);
	...
	DELETE FROM bdicred:"informix".sd_archivopagmin 
	WHERE fecha_registro  <= pFechaAnt;
...
DOCUMENT
'****************************************************************************************************************',
'Procedimiento para Depurar tabla sd_archivopagmin ',
'AUTOR : Juan Pablo Tellez ',
'FECHA : 22/01/2025',
'BD: BDICRED',	   
'****************************************************************************************************************';
