CREATE PROCEDURE "informix".pasecajag_esp(pempresa   CHAR(3), pfecha_pase DATE, pusuario   CHAR(8))
 ...
 ***************************************************************************}
 ...
 --set debug file to "pasecajag.out";
 --TRACE ON;

   LET pusuario = "cajagral";
   LET vcod_ret = "000";
   LET vdate = current;
   LET vctrl_poliza = 0;
   LET wbegin = "S";
     ...
     FOREACH
      ...
      FOREACH
          SELECT sucursal,cod_trans,divisa,
				 ...
				 case 
				 when cod_trans = '0011' THEN 'DOTACION ENVIADA SUCURSAL'
				 when cod_trans = '0012' THEN 'CONCENTRACION ENVIADA SUCURSAL'
				 when cod_trans = '0013' THEN 'DOTACION RECIBIDA SUCURSAL'
				 when cod_trans = '0014' THEN 'CONCENTRACION RECIBIDA SUCURSAL'
				 else "MOVIMIENTOS DE CAJA GENERAL DEL DIA "||wfecha_hoy
				 end as descripcion 
          INTO   wsucursal,wtransacc,wdivisa,
                 wcmayor,wcsub1,wcsub2,wcsub3,wcsub4,wcsector,
                 wnro_auxiliar,wmonto,vcargo,vsucdest,wdescripcion_det
