CREATE PROCEDURE "informix".sp_validanumcuentabancoppel_web(cNumCuenta CHAR(20))
	...
	LET cCodRet = "00000";
	LET cMensaje = "El Numero de Tarjeta no es de BanCoppel";
	LET iSqlErr = 0;

	--SET DEBUG FILE TO "/tmp/domi/sp_ValidaNumCuentaBanCoppel.out";
	--TRACE ON;  
	BEGIN
		...
		IF TRIM(cNumCuenta) = "" OR cNumCuenta IS NULL THEN
			LET cCodRet = "00001";
			LET cMensaje = "Error en el parametro de entrada";
			RETURN cCodRet, cMensaje;
		END IF;

		IF (SELECT {INDEX (bdicheq:sc_tarjeta ix_tarjeta2)} 1 FROM bdicheq:sc_tarjeta where empresa='001' AND num_tarjeta = cNumCuenta)>0 THEN
			LET cCodRet = "00002";
			LET cMensaje = "El Numero de Tarjeta es de BanCoppel";
		END IF;
...
END PROCEDURE
DOCUMENT
"Valida si el n̼mero de tarjeta es de BanCoppel",
"AUTOR: Iris Arias Zazueta",
"FECHA: 19/04/2010",
"BD   : bdicheq";

CREATE PROCEDURE "informix".sp_valida_monto_tras_web(pcEmpresa CHAR(3), pcCuenta CHAR(20), pcTransuc CHAR(4), pmTotTrans MONEY)
	RETURNING CHAR(5) AS CodRetorno, CHAR(4) AS CodMensaje;
...
BEGIN
	...
	ELSE
		...
		IF (SELECT valor FROM bdicheq:"informix".sc_param WHERE codparam = 'montomaxtraspefectas')>0 THEN
