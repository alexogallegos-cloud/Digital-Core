CREATE PROCEDURE "informix".sp_obtienedatos_reevaluacion_web(pEmpresa CHAR(3),pDias INTEGER, pNumSol CHAR(20),pNumCte CHAR(20),pSucursal CHAR(4))
	...
	LET cSitEspecial 					= '';	--SITUACION ESPECIAL
	LET iCasuaSitEsp 					= 0;	--CAUSA DE SITUACION ESPECIAL
	LET iLineaCred                      = 0;	--LINEA DE CREDITO REAL
	LET iFlagLinCred                    = 0;	--FLAG LINEA DE CREDITO ESPECIAL
	LET iLinCredPesos                   = 0;	--LINEA DE CREDITO PESOS
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			...
			RETURN cCod_Ret,cFlagPagoIni,cPorcPagoIni, iMontoDispPagIni, cFlagPrestamo, cSitEspecial, iCasuaSitEsp,iLineaCred, iFlagLinCred,
				iLinCredPesos, sPuntFinal, iPuntRiesgo, sCiudadSuc;
		END EXCEPTION;
		...
		IF pEmpresa <> '' AND pNumCte <> '' AND pNumSol <> '' AND pSucursal <> '' THEN
			...
			IF iDiasDiff >= pDias OR pSucursal = '8503' THEN -- HU 24360
				--SE FILTRA PARA QUE LAS AUTOSOLICITUDES DE LA SUC 8503 SE ENVIEN A REEVALUAR
			...
			ELSE
				...
				SELECT flag_pagoini, porc_pagoini,flag_prestamo, monto_disp_pagoini
				INTO cFlagPagoIni,cPorcPagoIni, cFlagPrestamo,iMontoDispPagIni
				FROM "informix".ss_nuevo_parametrico 
				WHERE empresa = pEmpresa
				AND num_solicitud = pNumSol;
