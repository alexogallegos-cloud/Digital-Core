CREATE PROCEDURE "informix".sp_obtener_prospectos_aumlincred(pEmpresa CHAR(3))
...
DEFINE pNumProd       CHAR(4);
DEFINE sDiaRegistro	  SMALLINT;
...
----SET DEBUG FILE TO '/bitacoras/TRACERicado/sp_obtener_prospectos_aumlincred.out';
----TRACE ON;
...
SELECT valor::SMALLINT INTO sDiaRegistro
  FROM bdicred:"informix".sd_param 
 WHERE cod_param = '049' AND empresa   = pEmpresa;
    -- validacion de los parametros.
IF NVL(sDiaRegistro,"")  = "" THEN
    LET cCodRet     = "000001";
    LET cMensajeRet = "No existe día de registro para incrementos de línea";
    RETURN cCodRet, cMensajeRet;
END IF;
 
LET vFechaHoyAumlincred = MDY(MONTH(vFechaHoy),sDiaRegistro,YEAR(vFechaHoy));

-- Actualiza la fecha con el mes en que se ejecuta el proceso. Esto para efectos de cuando se deje de ejecutar el proceso de incrementos de 
-- línea en meses pasados, no tenga problema por tener la fecha desactualizada. Incidencia presentada en Diciembre 2015 cuando se dejó de ejecutar
-- el proceso por dos meses y no se actualizó la fecha de la tabla sd_fechas_aumlincred.
    UPDATE bdicred:sd_fechas_aumlincred
    SET
        fecha_hoy     = vFechaHoyAumlincred,
        fecha_ant     = vFechaHoyAumlincred - 1 units DAY,
        prox_fecha    = vFechaHoyAumlincred + 1 units DAY
    WHERE empresa=pEmpresa;
...
IF (nrows = 0) THEN
  LET cCodRet = "000090";
  LET cMensajeRet = 'Usuario no valido para ejecutar el proceso';
  RETURN cCodRet, cMensajeRet;
END IF;
*/

IF vFechaHoy < vFechaHoyAumlincred THEN 
  LET cCodRet = "999999";
...
--BEGIN WORK;
    ...
    EXECUTE PROCEDURE bdicred:"informix".sp_identificar_clientes(pEmpresa,vFechaHoyAumlincred) INTO cCodRet, cMensajeRet;

    IF cCodRet <> "000000" THEN

--        LET cCodRet = '000001';
--        LET cMensajeRet = 'Error en el SPL sp_identificar_clientes.';     
--        ROLLBACK WORK;
...
--	EXECUTE PROCEDURE bdicred:"informix".sp_calcularaumlincred(pEmpresa,vFechaHoyAumlincred) INTO cCodRet, cMensajeRet;

--    IF cCodRet <> "000000" THEN

--			LET cCodRet = '000002';
--			LET cMensajeRet = 'Error en el SPL sp_calcularaumlincred'; 
--			ROLLBACK WORK;
...
/*
     IF EXISTS( SELECT dbsname, tabname FROM sysmaster:systabnames WHERE tabname = 'tmp_prospecto' AND dbsname = "bdicred") THEN
           DROP TABLE tmp_prospecto;
     END IF;  

      CREATE TABLE "informix".tmp_prospecto
      (
        numcte        CHAR(20),
      ...
      INSERT INTO tmp_prospecto 
           SELECT numcte, num_solicitud 
             FROM "informix".sd_bitacora_aumlincred a
            WHERE a.status       = "BC"
              AND a.num_producto = pNumProd
              AND a.fecha_insert = vFechaHoyAumlincred
              AND a.empresa      = pEmpresa;

        CREATE UNIQUE INDEX indx_tmp_prospecto_cte ON "informix".tmp_prospecto(numcte);
        CREATE UNIQUE INDEX indx_tmp_prospecto_sol ON "informix".tmp_prospecto(num_solicitud);

        UPDATE STATISTICS MEDIUM FOR TABLE "informix".sd_bitacora_aumlincred;

--        DELETE FROM bdiburo:br_consulta_bc WHERE institucion = "BC" AND num_solicitud IN (SELECT num_solicitud FROM "informix".tmp_prospecto)
--        AND fecha_insert = vFechaHoyAumlincred;

        DELETE FROM bdiburo:br_consulta_bc  WHERE num_solicitud IN (SELECT num_solicitud FROM "informix".tmp_prospecto);
        DELETE FROM bdiburo:br_respuesta_bc WHERE num_solicitud IN (SELECT num_solicitud FROM "informix".tmp_prospecto);
        DELETE FROM bdiburo:br_cr_bc WHERE numcte IN (SELECT numcte FROM "informix".tmp_prospecto);
        DELETE FROM bdiburo:br_hi_bc WHERE numcte IN (SELECT numcte FROM "informix".tmp_prospecto);
        DELETE FROM bdiburo:br_hr_bc WHERE numcte IN (SELECT numcte FROM "informix".tmp_prospecto);
        DELETE FROM bdiburo:br_iq_bc WHERE numcte IN (SELECT numcte FROM "informix".tmp_prospecto);
        DELETE FROM bdiburo:br_pa_bc WHERE numcte IN (SELECT numcte FROM "informix".tmp_prospecto);
        DELETE FROM bdiburo:br_pe_bc WHERE numcte IN (SELECT numcte FROM "informix".tmp_prospecto);
        DELETE FROM bdiburo:br_pn_bc WHERE numcte IN (SELECT numcte FROM "informix".tmp_prospecto);
        DELETE FROM bdiburo:br_rs_bc WHERE numcte IN (SELECT numcte FROM "informix".tmp_prospecto);
        DELETE FROM bdiburo:br_sc_bc WHERE numcte IN (SELECT numcte FROM "informix".tmp_prospecto);
        DELETE FROM bdiburo:br_tl_bc WHERE numcte IN (SELECT numcte FROM "informix".tmp_prospecto);
*/

--	EXECUTE PROCEDURE bdiburo:"informix".sp_br_consulta_bc(pEmpresa,pNumProd,vFechaHoyAumlincred) INTO cCodRet, cMensajeRet;

--	IF cCodRet <> "000000" THEN

--                 LET cCodRet = '000003';
--       LET cMensajeRet = 'Error en el SPL sp_br_consulta_bc';
--                ROLLBACK WORK;
...
--    EXECUTE PROCEDURE bdiburo:"informix".sp_archivo_consulta_bc(pEmpresa,vFechaHoyAumlincred) INTO cCodRet, cMensajeRet;

--    IF cCodRet <> "000000" THEN

--        LET cCodRet = '000001';
--        LET cMensajeRet = 'Error en el SPL sp_archivo_consulta_bc';
--        ROLLBACK WORK;
...
--    EXECUTE PROCEDURE bdicred:"informix".sp_rep_aumlincred(pEmpresa) INTO cCodRet;
...
--	  ELSE
--          LET cMensajeRet = 'Error en el SPL sp_rep_aumlincred';
--          RETURN cCodRet, cMensajeRet;
--	  END IF;
--    END IF;
...
END PROCEDURE
DOCUMENT 
'Se realiza procedimiento para la obtencion de clientes',
'prospectos a un incremento de linea de crédito',
'AUTOR : Jesús Manuel Aguilar Heredia',
'FECHA : 28/JUNIO/2010',
'BD    : bdicred';

CREATE PROCEDURE "informix".sp_cierre_credito_paso(pEmpresa CHAR(3))
RETURNING
   CHAR(6)        AS Cod_Ret,
   CHAR(80)       AS Mens_Ret;
...
-- Fecha: 2010/01/20
-- Comentario: * Se agrega la actualización del campo capital_status_ant
--             * Se agrega la actualización del campo mto_fin_ven_trasp
--             * Se agrega la actualización del campo dias_acum_mora
--             * Se agrega la actualización del campo dias_acum_cap
--             * Se modifica el calculo de el parámetro de la tasa
...
-- Modifico: Paul Ivan Quintero Varela
-- Fecha: 12/02/2010
-- Comentario: * Se agrega validación para que la inserción del historico de saldos (tabla sd_maesdoshistcrd)
--               tambien se realice en su facturación.
--             * Se modifica para que la condición del traspaso a los 90 dias no este ligado a que sea realizada siempre
...
--SET DEBUG FILE TO 'sp_cierre_credito.out';
--TRACE ON;
...
-- *******************************************************
--  IDENTIFICACIÓN DE PROCESOS POR REALIZAR              *
-- *******************************************************

          -- Validación para identificar un (Cierre de Mes).
          IF dtFechaHoy = dtFechaFinMes THEN  
            LET cIdProc1 = "C";
          END IF;			

          -- Validaciones para días inhabiles y cambios de mes. (Facturación)
         ...
         --FMV 9ene13 Validacion 4o. Proceso, Provision a fin de mes cuando el siguiente dia es inhabil              
          IF (cIdProc1 = "C" AND dtFechaHoyAux > DATE(dtFechaHoy+1) AND iDiaCorte = 2) THEN
             ...
             --LET iDiasInt = iDiasInt - 1;  --Se quita un dia ya que provisionará los q no tienen vencimiento el dia 2
          END IF;
...
-- Venta de Cartera Reestructurada 
			IF ( v_marca_ayuda = '1' OR cStatusCred = 'CV' OR ( Campotrabajo3 = 'BAJA' AND cStatusCred <> 'CV') ) THEN 
                ...
                IF ( v_marca_ayuda = '1' OR ( Campotrabajo3 = 'BAJA' AND cStatusCred <> 'CV') ) THEN
...
-- *******************************************************
   ...
   --FMV 20-ENE-11  Calcula el interes sobre el saldo capital sin el monto de traspaso ya calculado en la facturacion
           ...
           END IF;      
                  ...
                  --Se actualiza la fecha del próximo proceso
                  UPDATE "informix".sd_maecredanexocrd
                     SET fecha_proceso  = dtFechaProx
                   WHERE num_credito    = cNumCredito
                     AND empresa        = cEmpresa;
...
-- *******************************************************
--  PROVISIÓN POR FIN DE MES EN DIA INHABIL SIGUIENTE    *
-- *******************************************************
...
-- LET dSdoInt = dSdoInt + dIntDiario;
 LET dProvInt_inh = dSdoInt_inh;
	IF dTasaInter > 0 THEN   --FMV 26-AGO-14: Si la tasa de interes es mayor a cero, genero calculo de interes Real.
	  ...
	  IF cCodRet <> "000000" THEN
			LET cCodRet      = "000005";
			LET cMensajeRet  = "Ocurrió un error al realizar calculo de iva de interes";
			ROLLBACK WORK;
			RETURN cCodRet,cMensajeRet;
	  END IF;
	END IF;  --FMV
...
---Se genera la provision de interes en fin de mes o Facturacion
                ...
                IF (cCodRet <> "000000") THEN
                       LET cCodRet      = "000006";
                       LET cMensajeRet = "Ocurrió un error al registrar la provisión de interes";
                       ROLLBACK WORK;
                       RETURN cCodRet,cMensajeRet;    
                END IF;
...
---Se genera la provision de iva de interes real en fin de mes o Facturacion
                        ...
                        IF (cCodRet <> "000000") THEN
                               LET cCodRet      = "000007";
                               LET cMensajeRet = "Ocurrió un error al registrar la provisión de iva de interes";
                               ROLLBACK WORK;
                               RETURN cCodRet,cMensajeRet;    
                        END IF;
                END IF;
---ini cas, Se agrega el movimiento aplicativo de interes gravable y exento
                ...
                IF dIntGrav_inh>0 THEN
                        ...
                        IF (cCodRet <> "000000") THEN
                               LET cCodRet      = "000007";
                               LET cMensajeRet = "Ocurrió un error al registrar el Interés Gravado";
                               ROLLBACK WORK;
                               RETURN cCodRet,cMensajeRet;    
                        END IF;
                END IF;
                IF dIntExen_inh>0 THEN
                        ...
                        IF (cCodRet <> "000000") THEN
                               LET cCodRet      = "000007";
                               LET cMensajeRet = "Ocurrió un error al registrar el Interés Exento";
                               ROLLBACK WORK;
                               RETURN cCodRet,cMensajeRet;    
                        END IF;
                END IF;
...
END IF;
-- **********************************************************************
--  FIN DE PROVISIÓN POR REGISTROS DE MOVTO EN FIN MES POR DIA INHABIL  *
-- **********************************************************************
...
-- FMV 25-ENE-2011: Realizará la facturación siempre y cuando la suma de Saldos vencidos sean mayores a cero
                     ...
                     where empresa             = cEmpresa
                       ...
                       IF dIvaPeriodo IS NULL THEN LET dIvaPeriodo=0; END IF;
                        ...
                        IF dIntPeriodo>0 THEN
                                ...
                                IF (cCodRet <> "000000") THEN
                                       LET cCodRet      = "000007";
                                       LET cMensajeRet = "Ocurrió un error al registrar el Interés Periodo";
                                       ROLLBACK WORK;
                                       RETURN cCodRet,cMensajeRet;    
                                END IF;
                        END IF;
           ...
           IF dTraspCap>0 THEN
             ...
             --FMV 25Abr13: Actualiza indicador del 1er. vencido y dias de atraso
              ...
              -- Traspaso capital vigente a transitorio.
               ...
               IF (cCodRet <> "000000") THEN
                      LET cCodRet      = "000009";
                      LET cMensajeRet = "Ocurrió un error al registrar traspaso capital vigente a transitorio";
                      ROLLBACK WORK;
                      RETURN cCodRet,cMensajeRet;  
                END IF;
                     ...
                     WHERE empresa        = cEmpresa 
                      ...
                      IF (dSdoCapital - dTraspCap)>0 THEN
                          ...
                          RETURNING cCodRet,cMensajeRet;
                            ...
                            IF (cCodRet <> "000000") THEN
                                   LET cCodRet      = "000010";
                                   LET cMensajeRet = "Ocurrió un error al registrar traspaso capital vigente a vdo no exigible";
                                   ROLLBACK WORK;
                                   RETURN cCodRet,cMensajeRet;    
                            END IF;
                      END IF;
                      ...
                      IF dMntVencido>0 THEN
                          ...
                          RETURNING cCodRet,cMensajeRet;
                            ...
                            IF (cCodRet <> "000000") THEN
                                   LET cCodRet      = "000011";
                                   LET cMensajeRet = "Ocurrió un error al registrar traspaso capital transitorio a exigible";
                                   ROLLBACK WORK;
                                   RETURN cCodRet,cMensajeRet;    
                            END IF;
                      END IF;
                      ...
                      IF dIntVdo>0 THEN
                          ...
                          RETURNING cCodRet,cMensajeRet;
                            ...
                            IF (cCodRet <> "000000") THEN
                                   LET cCodRet      = "000012";
                                   LET cMensajeRet = "Ocurrió un error al registrar traspaso interes vigente a vdo";
                                   ROLLBACK WORK;
                                   RETURN cCodRet,cMensajeRet;    
                            END IF;
                      END IF;
                /*      -- Provisión de interes ordinario a provisión de cartera vencida
                      ...
                      RETURNING cCodRet,cMensajeRet;
                        ...
                        IF (cCodRet <> "000000") THEN
                               LET cCodRet      = "000013";
                               LET cMensajeRet = "Ocurrió un error al registrar provisión de interes ordinario";
                               IF cBegin = "S" THEN
                                  ROLLBACK WORK;
                               END IF;
                               RETURN cCodRet,cMensajeRet;    
               ...
               WHERE num_credito   = cNumCredito
                ...
                /*      CALL "informix".genmovcrd(cEmpresa,cNumCredito, cNumProducto , 2, 
                  ...
                  RETURNING cCodRet,cMensajeRet;
                    ...
                    IF (cCodRet <> "000000") THEN
                           LET cCodRet      = "000014";
                           LET cMensajeRet = "Ocurrió un error al registrar traspaso capital vencido no exigible a vencido exigible";
                           ROLLBACK WORK;
                           RETURN cCodRet,cMensajeRet;    
                    END IF;             
           END IF;
...
-- *******************************************************
-- RESPALDO DE INFORMACIÓN CONTABILIDAD A FIN DE MES     *
-- *******************************************************
     IF cIdProc1 = "C"  or day(dtFechaHoy)=20 THEN
           INSERT INTO "informix".sd_maesdoscontcrd
                SELECT dtFechaHoy, *
