CREATE PROCEDURE "informix".sp_cobra_comision_cap_com3(p_empresa char(3))
    RETURNING   CHAR(5);
	...
	ON EXCEPTION SET vsqlerr, iIsamErr, cErrorInfo
	    IF  vsqlerr != 0 THEN
            SET DEBUG FILE TO "/resplogifx/conciliachq/sp_cobra_comision_cap_com3.err";
	 	    TRACE ON;
			LET vcodret    = vsqlerr;
	...
	--SET   DEBUG FILE TO '/resplogifx/conciliachq/sp_cobra_comision_cap_com3.txt';
	--SET   DEBUG FILE TO '/RESPALDOSNEW/opti/sp_cobra_comision_cap_com3.txt';
    --TRACE ON;
    ...
    CREATE TEMP TABLE tmp_portab_dts3 (
	cliente      CHAR(20),
	cuenta       CHAR(20),
	num_tarjeta  CHAR(20),
			...
			IF  v_valida_inv_cre = 0 THEN 
				...
				IF  v_valida_pagare =  0 THEN 
					...
					FOREACH WITH HOLD    ----------- PORTABILIDAD DE NOMINA 3
						...
						INSERT INTO tmp_portab_dts3 VALUES (v_num_cte_cargo,v_cta_mov,"",v_sdo_prom_car );
					...
					IF vportamovtot = 0 THEN     
					   ...
					   SELECT * FROM tmp_portab_dts3;
					   
					   DELETE FROM tmp_portab_dts3
					   WHERE cliente = v_num_cte_cargo;
					ELSE 
					   DELETE FROM tmp_portab_dts3
					   WHERE cliente = v_num_cte_cargo;
    ...
    --ACTUALIZA LA BANDERA DE FIN DE PROCESO COMPLEMENTO 3		
    UPDATE sc_contproc
    SET    fecha   = v_fecha_ant
    WHERE  empresa = p_empresa
