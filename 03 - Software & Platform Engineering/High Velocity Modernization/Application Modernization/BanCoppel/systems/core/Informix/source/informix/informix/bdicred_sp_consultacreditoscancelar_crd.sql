CREATE PROCEDURE "informix".sp_consultacreditoscancelar_crd(pEmpresa      CHAR(3), 
                                                        pNumCte       CHAR(20),
                                                        pNumCredito   CHAR(20))
--EXECUTE PROCEDURE sp_consultacreditoscancelar_crd('001','047418361','630090932298');
...
--SET DEBUG FILE TO '/ifxsif01/joel/sp_consultacreditoscancelar_crd.out';
--TRACE ON;
...
BEGIN 
	...
	ON EXCEPTION SET iSqlErr, iIsamErr, cErrorInfo
		IF iSqlErr != 0 THEN
		  ...
		  RETURN cCodRet, cMensajeRet, cNumCredito, cNumCte, NVL(cNomProducto,''), NVL(cNomCte,''), NVL(cStatus,'') ;
		END IF;
	END EXCEPTION;
	...
	-- VALIDACIÃÂÃÂN DE LOS PARAMETROS DE ENTRADA.
	IF TRIM(NVL(pEmpresa,'')) = '' OR TRIM(NVL(pNumCte,'')) = '' AND TRIM(NVL(pNumCredito,'')) = '' THEN
		EXECUTE PROCEDURE bdinteg:"informix".sp_desc_ret('06','590')
		INTO vCodRet, vMsjRetorno;
		
		LET cCodRet= '000001';
	    LET cMensajeRet = vMsjRetorno::CHAR(80);
		RETURN cCodRet, TRIM(NVL(cMensajeRet,'')), TRIM(NVL(cNumCredito,'')), TRIM(NVL(cNumCte,'')), TRIM(NVL(cNomProducto,'')), TRIM(NVL(cNomCte,'')), TRIM(NVL(cStatus,''));
	END IF;
	
/*
		IF (NVL(pNumCte,'')) <> '' THEN
				SELECT first 1 num_credito, numcte
...
*/		
		...
		IF NVL(cNumCredito,'') <> ''  THEN
			FOREACH
				...
				INTO cNumCredito, cNumCte, cNomProducto, cNomCte ,cStatus
	...
	--			  AND a.numcte = pNumCte
				  ...
				  AND a.num_producto IN ('6300','6800','7600','7700')
							
				LET iBandera = 1;
				
				RETURN cCodRet, TRIM(NVL(cMensajeRet,'')), TRIM(NVL(cNumCredito,'')), TRIM(NVL(cNumCte,'')), TRIM(NVL(cNomProducto,'')), TRIM(NVL(cNomCte,'')), TRIM(NVL(cStatus,'')) WITH RESUME;
...
--				  AND a.num_credito = cNumCredito
	...
	IF iBandera =  0 THEN
		...
		RETURN TRIM(cCodRet), TRIM(NVL(cMensajeRet,'')), TRIM(NVL(cNumCredito,'')), TRIM(NVL(cNumCte,'')), TRIM(NVL(cNomProducto,'')), TRIM(NVL(cNomCte,'')), TRIM(NVL(cStatus,''));
	END IF;
...
'DESCRIPCION: Se realiza procedimiento para realizar una consulta general para obtener la informaciÃÂÃÂ³n basica del cliente', 
'FECHA DE MODIFICACIÃÂÃÂN: 18 de Octubre del 2012',
'VERSION: 20121018.1726',
'BD: BDICRED',
'MODIFICÃÂÃÂ: Carlos Ochoa Valenzuela',
'DESCRIPCION: Se muestran todos los crÃÂÃÂ©ditos mientras no estÃÂÃÂ©n previamente cancelados y se agrega un nuevo return con el status del crÃÂÃÂ©dito', 
'FECHA DE MODIFICACIÃÂÃÂN: 11 de Diciembre del 2012',
