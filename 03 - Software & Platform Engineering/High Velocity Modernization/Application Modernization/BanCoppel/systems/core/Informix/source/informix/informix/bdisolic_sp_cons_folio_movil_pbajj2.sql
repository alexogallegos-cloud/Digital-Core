CREATE PROCEDURE "informix".sp_cons_folio_movil_pbajj2(pRFC CHAR(13), pfolio CHAR(12), pOCR CHAR(9), pApPat CHAR(30), pApMat CHAR(30), pNom1 CHAR(30), pNom2 CHAR(30), pFecNac CHAR(10))
		...
		IF (iExists <= 0) THEN
			...
			IF (iExistsA <= 0) THEN
				...
				IF (iExistsB <= 0 and pOCR <> '' )  THEN		
					...
					SELECT COUNT(*) INTO iExistsC FROM bdinteg:si_solicitud_movil WHERE ocr <> '' and  ocr matches pOCRconsulta AND folio_procesado='0' and status_valua='1';
...
--***********************NUEVOS CAMBIOS
        ...
        --***********************NUEVOS CAMBIOS
			...
			ELSE
                ...
                ELIF (iExistsC > 0 ) THEN
					...
					FIRST 1 folio from bdinteg:si_solicitud_movil WHERE ocr <> '' and ocr matches pOCRconsulta AND folio_procesado='0' 
					AND fecha_hora = (SELECT MAX(fecha_hora) FROM bdinteg:si_solicitud_movil WHERE ocr <> '' and ocr matches pOCRconsulta AND folio_procesado='0'));
        ...
        ELIF (iExistsD > 0) THEN
			...
			LET pFolio=(SELECT 
						{+INDEX (bdinteg:"informix".si_solicitud_movil idx_ocr)}
						FIRST 1 folio 
						from bdinteg:si_solicitud_movil WHERE ocr <> '' and ocr matches pOCRconsulta AND folio_procesado='0' AND status_valua='1'  
						AND fecha_hora = (SELECT MAX(fecha_hora) FROM bdinteg:si_solicitud_movil WHERE ocr <> '' and ocr matches pOCRconsulta AND folio_procesado='0' AND status_valua='1'));
