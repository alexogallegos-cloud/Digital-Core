CREATE PROCEDURE "informix".sp_directorioaltasycambios2(pfecha date)
returning char (5);
...
LET cNombreArchivo1= 'DirectorioCtesBancoppel' || LPAD(TRIM(DAY(pfecha::DATE)::CHAR(2)),2,'0') || LPAD(TRIM(MONTH(pfecha::DATE)::CHAR(2)),2,'0') ||YEAR(pfecha::DATE) || '.txt';
LET cNombreArchivo2= 'CifrasControlDirectorioCtesBancoppel' || LPAD(TRIM(DAY(pfecha::DATE)::CHAR(2)),2,'0') || LPAD(TRIM(MONTH(pfecha::DATE)::CHAR(2)),2,'0') ||YEAR(pfecha::DATE) || '.txt';
...
--  SELECT fecha_ant INTO fecha_movto FROM bdicred:sd_fechas;
    ...
    DELETE bdicred:sd_cifracontroldirectorioaltasycambios where empresa = '001' AND fecha_generacion = pfecha;


	
    select numcte,'A' Status
    from bdinteg:si_cliente
    where fecha_insert = pfecha - 1 UNITS DAY
...
--    create unique index inx_clientes_alta on clientes_alta (numcte) using btree;
    ...
    where tipo_dir = '1'
    and fecha_insert = pfecha - 1 UNITS DAY
...
--                                nvl(ing.ingreso_mensual,' ') AS ingresomensual,     -- ingreso mensual
                               ...
                               -- nvl(rpad(TRIM(dir.telefono2),13,' '),' ') AS celular,    -- celular
                                ...
                                --nvl(cte.numcte_ref,'') AS cliente_coppel
                            INTO  --cNumCredito ,
...
--                      AND mcr.status_cred in ('AA','BA','BT')
				--SE HOMOLOGA CON EL PRODUCTIVO QUE OBTIENE LOS TELEFONOS DE LA SI_TELEFONOS, por lo tanto se comenta la consulta a la si_telefonos_actual
				/*
...
--                      LEFT OUTER JOIN bdinteg:si_catciudades ciudad ON (ciudad.numerociudad = dir.numerociudad)
					...
					--SE COMENTA CONSULTA PARA HOMOLOGAR CON EL PRODUCTIVO Y OBTENER EL DATO DE LA SI_TELEFONOS 
...
--                        nvl ( replace ( replace( cNomMunicipioTrab , '|' , ' ' ), '\' , ' ' ), ' ' ),
                        ...
                        pfecha - 1 UNITS DAY ,cNumCte_Coppel );
    ...
    VALUES ('001', iContadorRegistros, iContadorAltas,iContadorCambios,pfecha);
    ...
    -- para Generar el archivo de Salida.
        LET cSql = 'echo "UNLOAD TO ' || '''/resplogifx/archivoscartera/DirectorioCtesBancoppelRegistros.unl''' || ' DELIMITER ' || '''|'''  ||
                   ' SELECT * FROM bdicred:sd_directorioaltasycambios ' ||
    ...
    -- para Generar el archvio de Cifras.
             ...
             LET cSql = 'echo "UNLOAD TO ' || '''/resplogifx/archivoscartera/DirectorioCifrasControlRegistros.unl''' || ' DELIMITER ' || '''|'''  ||
                                 ...
                                 ' WHERE c.fecha_generacion = '''|| pfecha::date  || ''' ' ||
