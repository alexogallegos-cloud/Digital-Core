CREATE PROCEDURE "informix".sp_consulta_sac_cte_mnsj_remesas( pMes	Smallint, pNum_fin_remesa	Smallint )
RETURNING CHAR (5)	 AS cCodRet,CHAR (1) As cStatus_mnsj; 
...
BEGIN
	ON EXCEPTION SET iSqlErr
		IF iSqlErr <> 0 THEN
			LET cCodRet = '00003';
			RETURN cCodRet,cStatus_mnsj;
		END IF;
	END EXCEPTION;
	
			
	
	--SET DEBUG FILE TO '/tmp/sp_consulta_sac_cte_mnsj_remesas.out";
	--TRACE ON;	
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;  
	
		IF ( NVL(pMes, '') = '' OR NVL(pNum_fin_remesa, '') = '' or pMes > 12 ) THEN
					LET cCodRet = '00002';
					--DATOS VACIOS, ERROR.
					RETURN cCodRet,cStatus_mnsj;
			ELSE
						...
						select status_mnsj 
						into cStatus_mnsj
						from bdisac: "informix".sac_cte_mnsj_remesas	
						where mes=pMes and num_fin_remesa = pNum_fin_remesa;
						LET cCodRet = '00000';
...
'DescripciÃ³n: Se inserta campo numcte para utilizar en remesas',
