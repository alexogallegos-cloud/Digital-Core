CREATE PROCEDURE "informix".sps_obtenerpreguntasusuario(pIdCliente VARCHAR(9), pLimite INT)
RETURNING CHAR (5), INT, CHAR(50);	
	...
	--SET DEBUG FILE TO "/home/sp_obtenerpreguntasusuario.out";
	--TRACE ON;
	...
	BEGIN
		...
		SELECT id_usuario INTO iIdUsuario FROM bdibpi:"informix".bpi_usuario WHERE numcliente = pIdCliente AND st_portal = 'activo'; -- ID_usuario
		...
		IF iIdRegistro < 5 THEN
			...
			END FOREACH;
			IF iOrdAle < 0 THEN
				IF ( (SELECT count(numcte) FROM bdinteg:si_bpiusuarios WHERE numcte = pIdCliente AND id_status='40' ) = 1 AND iIdCte = 0) THEN
		...
		FOREACH
			...
			EXECUTE PROCEDURE "informix".sp_random(iOrdAleAux, 100) INTO iOrdAle;
...
END PROCEDURE
DOCUMENT
'FOLIO.........: 1583-MttoBPI-Paquete3',
'AUTOR.........: Jose Ruben Lopez',
'FECHA.........: 06-10-2015',
'MODIFICACIÓN..: Se crea stored procedure, para obtener las preguntas por medio de id de cliente por motivo del nuevo proceso de HSM',
...
'FECHA.........: 09-02-2016',
'MODIFICACIÓN..: Se modifica stored procedure para saber si recibe id de cliente o numero de usuario',
