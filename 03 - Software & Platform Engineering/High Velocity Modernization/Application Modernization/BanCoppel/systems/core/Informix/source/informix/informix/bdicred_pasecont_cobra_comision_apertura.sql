CREATE PROCEDURE "informix".pasecont_cobra_comision_apertura(pempresa     CHAR(3),
                                     fecha_pase   DATE,
                                     pusuario     CHAR(8),
                                     pusuariopase CHAR(8),
                                     pproceso     CHAR(10))
 ...
 ***************************************************************************}
  ...
  --SET DEBUG FILE TO "/informix/paulq/pba/pasecont_cobra_comision.out";
  --TRACE ON;
...
--         END IF;
      ...
      ELSE
         SELECT c.regional, a.suc_origen, a.codigo_fun, a.codigo_ref,
           ...
           FROM sd_movdia a, sd_maecred b, bdinteg:si_plazas c, bdicred:sd_comision_x_apertura_contable d
          WHERE a.empresa = pempresa
            AND a.empresa = b.empresa
			...
			AND d.fecha_insert <= fecha_pase
			--AND d.aplica_cobro = '0'			
            group by 1,2,3,4,5,7,8
           INTO TEMP x WITH NO LOG;
      END IF
