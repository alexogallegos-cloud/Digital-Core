CREATE PROCEDURE "informix".sp_regordenctecte_bex_codi_exp1(pEmpresa  CHAR(3),
                                                 pchrSucursal CHAR(4),
                                                 pchrUsuario CHAR(8),
...
RETURNING CHAR(5), CHAR(30);
   ...
   -- SET DEBUG FILE TO '/informix/ash/CODI/sp_regordenctecte_bex_codi.out';
   --SET DEBUG FILE TO '/informix/Priscilla/sp_regordenctecte_bex_codi.out';
   --TRACE ON;
   ...
   -- // Compara la clave del banco en la clabe del beneficiario, si es por cuenta CLABE.
	...
	LET wchrcadena_03 = pintTipoCtaBenef||'|'||TRIM(pvchrNombreBenef)||'|'||TRIM(pvchrCtaBenef)||'|'||TRIM(pvchrRFCBenef)||'||'||pintTipoCtaBenef||'|'||TRIM(pvchrNombreBenef)||'|'||TRIM(pvchrCtaBenef)||'|'||TRIM(pvchrRFCBenef)||'|'||TRIM(pvchrConceptoPago)||'|'||TRIM(pvchrConceptoPago)||'|||0|'||TRIM(pvchrRefCobranza1)||'|';
...
CREATE PROCEDURE "informix".sp_regordenctecte_pp_exp1(pEmpresa  CHAR(3),
                                                 pchrSucursal CHAR(4),
                                                 pchrUsuario CHAR(8),
                                                 pintBancoDest INTEGER,
                                                 pmnyImporte MONEY(14,2),
...
RETURNING CHAR(5), CHAR(100), CHAR(30);
    ...
    ON EXCEPTION SET vintcodret, vintcodret2, vchrcodret3
        IF vintcodret <> 0 THEN
		    SET DEBUG FILE TO "/resplogifx/conciliachq/spei/sp_regordenctecte_pp.err";
            TRACE ON;
            LET vchrcodret = vintcodret;
            LET vchrcodret2 = vintcodret2;
            LET vcharcodret3 = vchrcodret3;
    ...
    --- DEBUG FLAG
    --- SET debug file to "/informix/sp_regordenctecte_pp.out";
    --- TRACE ON;
    ...
    IF pintTipoCtaOrd = 10 THEN
		...
		ELSE
			...
			IF vchrTelefono is null OR vchrTelefono = '' THEN  -- busca en cuentas transfer
				SELECT telefono
					INTO vchrTelefono
					FROM bditransfer:tf_maecte
				WHERE cuenta_tf = pvchrCuentaOrd
    ...
    IF pvchrConceptoPago LIKE 'PORTABILIDAD DE NOMINA%' THEN
        LET vchrCveRastreo = TO_CHAR(pdtfechacaptura, '%Y%m%d')||'40137'||'NNNN'||'COPL' || LPAD(vintfolioop||'', 8, '0') || vdigverif;
    ELSE
        LET vchrCveRastreo = TRIM(pchrSucursal) || TRIM(vsuc_cta) || UPPER(TRIM(pchrUsuario)) || LPAD(vintfolioop||'', 7, '0') || vdigverif;
    END IF;
    ...
    ELSE           
        ...
        INSERT INTO bdicheq:sc_movdia
        (num_serial, folio_suc, sucursal, usuario, fech_alt, fech_val, fech_hor, transacc, suc_cuen, producto, empresa, cuenta, causa_dev, num_cheq, monto_tot, 
         firme, en_sbc, remesas, dias_ret, cancelad, edo_cta, sdo_cuenta, transacc_suc, referencia, tasa_aplicada, num_tarjeta, usuautoriza, referencia_23, fech_oper)
		VALUES
        (0, pchrFolioSuc, pchrSucursal, pchrUsuario, pdtfechacaptura, vchrFechaValor, vhora, vchrTranscargo, pchrSucursal, vchrProdTrnf, pEmpresa, 
         pvchrCuentaOrd, '', 0, pmnyImporte, 0, 0, 0, 0, '', '', 0.00, pchrTransuc, vchrCveRastreo, 0, vchrTarjeta, pchrUsuario, '', dtFechaOperacion);
    ...
    IF pmnyComision > 0 THEN
        ...
        ELSE     
            ...
            (num_serial, folio_suc, sucursal, usuario, fech_alt, fech_val, fech_hor, transacc, suc_cuen, producto, empresa, cuenta, causa_dev, num_cheq, monto_tot, 
             firme, en_sbc, remesas, dias_ret, cancelad, edo_cta, sdo_cuenta, transacc_suc, referencia, tasa_aplicada, num_tarjeta, usuautoriza, referencia_23, fech_oper) 
			VALUES
            (0, pchrFolioSuc, pchrSucursal, pchrUsuario, pdtfechacaptura, vchrFechaValor, vhora, vchrComis, pchrSucursal, vchrProdTrnf, pEmpresa, 
             pvchrCuentaOrd, '', 0, pmnyComision, 0, 0, 0, 0, '', '', 0.00, pchrTransuc, vchrCveRastreo, 0, vchrTarjeta, pchrUsuario, '', dtFechaOperacion);
    ...
    IF pmnyIvaComis > 0 THEN
        ...
        ELSE
            ...
            (num_serial, folio_suc, sucursal, usuario, fech_alt, fech_val, fech_hor, transacc, suc_cuen, producto, empresa, cuenta, causa_dev, num_cheq, monto_tot, 
             firme, en_sbc, remesas, dias_ret, cancelad, edo_cta, sdo_cuenta, transacc_suc, referencia, tasa_aplicada, num_tarjeta, usuautoriza, referencia_23, fech_oper)
			VALUES
            (0, pchrFolioSuc, pchrSucursal, pchrUsuario, pdtfechacaptura, vchrFechaValor, vhora, vchrIvaComis, pchrSucursal, vchrProdTrnf, pEmpresa, 
             pvchrCuentaOrd, '', 0, pmnyIvaComis, 0, 0, 0, 0, '', '', 0.00, pchrTransuc, vchrCveRastreo, 0, vchrTarjeta, pchrUsuario, '', dtFechaOperacion);
    ...
    INTO vchrcodret, intpktblpago;
    
    IF (vchrcodret * 1) <> 0 THEN
	...
	IF ret = 0 THEN 
		...
		VALUES (intpktblpago, pmnyImporte, vchrestatusenvio, pvchrNombreOrd, pvchrCuentaOrd, pvchrRFCOrd, pintTipoCtaOrd, pvchrNombreBenef,
				...
				'', '', chrtopologia, '0', intBancoOrd, vintCveCesif, vchrTranscargo, vsintLongCveRast, wvchrfirma );
	END IF;
