CREATE PROCEDURE "informix".sp_generaarchivocobranzacontigo18hrs(pConvenio CHAR(5))
...
--SET DEBUG FILE TO '/informix/ENP/sp_generaarchivocobranzacontigo18hrs.out';
--TRACE ON;
...
SET LOCK MODE TO WAIT 3;
		...
		DROP TABLE IF EXISTS mov_contigo_1600;
		...
		LET cRutaArchContigo = REPLACE(cRutaArchContigo,'CONTIGO',cHora);
		...
		LET cStmt = 'rm -f ' || cRutaArchContigo;
		SYSTEM cStmt;
        ...
        SELECT * 
        FROM
            bdisac:"informix".sac_movimientos 
        WHERE
            numcategoria = cCategoria AND
            numconvenio = cConvenio
        INTO
            TEMP mov_contigo_1600 WITH NO LOG; 
        ...
        FROM
            mov_contigo_1600 
        WHERE
            extend (fecha_insert, HOUR TO SECOND) <= '18:00:00'
        ...
        DROP TABLE IF EXISTS temp_mov_contigo;
