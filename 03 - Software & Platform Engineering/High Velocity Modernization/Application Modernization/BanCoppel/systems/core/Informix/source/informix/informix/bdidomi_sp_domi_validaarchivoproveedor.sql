CREATE PROCEDURE "informix".sp_domi_validaarchivoproveedor(pNombreArchivo CHAR(20), pFechaEnvio DATE)
	RETURNING CHAR(5);
...
DEFINE  cCuentaAbonoD CHAR(20);
DEFINE  cCuenta 	CHAR(20);
DEFINE  cNum_Cte2 	CHAR(20);
DEFINE  cCuenta2 	CHAR(20);
DEFINE  cStatus_Cta	CHAR(1);
DEFINE  mImporteMaximo	MONEY(18,2);
DEFINE  mImporteOperacion	MONEY(18,2);
...
LET mImporteOperacion	= 0.00;


       --SET debug FILE TO "/tmp/Sp_Domi_ValidaArchivoProveedor.out";
       --Trace ON;
...
Begin
	...
	---Validar Datos de encabezado: No de cliente, no de cuenta( que exista, este activa y que pertenezca al cliente )
	--Se obtiene en numero de cliente y la clabe para validarlas
	SELECT num_cte, LPAD(cuenta_abono,20,'0') INTO cNum_Cte, cCuentaAbono FROM dom_cte_encabezado where nombre_arch = pNombreArchivo AND fecha_envio = pFechaEnvio;
	--VALIDA QUE EXISTE LA CUENTA ABONO
	IF NOT EXISTS (SELECT cuenta_clabe FROM bdicheq:sc_maechq WHERE empresa = '001' AND bdicheq:sc_maechq.cuenta_clabe = SUBSTR(cCuentaAbono,3,18)) THEN
		--La cuenta Abono no existe
		LET cCodret = '99901';
		RETURN cCodret;
	END IF;
	--SE OBTIENEN LA CUENTA Y EL NUMERO DE CUENTA QUE PERTENECEN A LA CUENTA ABONO
	SELECT num_cte,cuenta,status_cta INTO cNum_Cte2,cCuenta2,cStatus_Cta FROM bdicheq:sc_maechq WHERE empresa = '001' AND bdicheq:sc_maechq.cuenta = SUBSTR(cCuentaAbono,9,11);
	---VALIDAR QUE ESTE ACTIVA
--	IF (cStatus_Cta <> '1') THEN
	IF (cStatus_Cta = '2') THEN
		--La cuenta no tiene un estado activo
		LET cCodret = '99902';
		RETURN cCodret;
	END IF;
	--VALIDAR SI EXISTE LA CUENTA
	IF NOT EXISTS (SELECT cuenta_clabe FROM bdicheq:sc_maechq WHERE empresa = '001' AND bdicheq:sc_maechq.cuenta = cuenta) THEN
		--La cuenta Abono no existe
		LET cCodret = '99901';
		RETURN cCodret;
	END IF;

	--VALIDAR QUE LA CUENTA ABONO PERTENESCA AL CLIENTE
	IF (LPAD(TRIM(cNum_Cte2),20,'0') <> LPAD(TRIM(cNum_Cte),20,'0') ) THEN
		--La cuenta ABONO NO PERTENECE AL CLIENTE
		LET cCodret = '99903';
		RETURN cCodret;
	END IF;
	-----------------------------VALIDAR EL IMPORTE DE CADA REGISTRO----------------------------------------------------------------------------------------------------
	--- obtener el valor  del importe maximo por  operacion permitido por cecoban
	SELECT VALOR INTO mImporteMaximo FROM dom_parametros WHERE COD_PARAM = '10';
	FOREACH
		SELECT (imp_operacion / 100), LPAD(cuenta_abono,20,'0') INTO mImporteOperacion, cCuentaAbonoD FROM bdidomi:dom_cte_detalle where nombre_arch = pNombreArchivo AND fecha_envio = pFechaEnvio
		--validar que el importe de la operacion sea menor a importe  maximo permitido
		IF (mImporteOperacion > mImporteMaximo) THEN 
			--EL IMPORTE DE LA OPERACION ES MAYOR AL IMPORTE MAXIMO PERMITIDO POR CECOBAN
			LET cCodret = '99904';
			RETURN cCodret;
		END IF;
		--validar que la cuenta abono del encabezado sea igual a la de detalle
		IF (cCuentaAbono <> cCuentaAbonoD) THEN 
			--La Cuenta Abono No corresponde a la de encabezado
			LET cCodret = '99908';
			RETURN cCodret;
		END IF;
	END FOREACH;
...
'DESCRIPCION: Este procedimiento valida que el importe de las operaciones no sobre pase el importe maximo permitido por cecoban, ademas ',
'           : de validar que el numero de cliente, y la cuenta abono existan, este activa y pertenesta al cliente',
'FECHA : Agosto de 2009',
'BD    : BDIDOMI',
'VERSION: 20090818.1130';

CREATE PROCEDURE "informix".sp_domi_conciliacontable(pNomArchivo CHAR(20), pTpoProc INTEGER, pfecini Char(10), pfecfin Char(10))
RETURNING 
...
CREATE PROCEDURE "informix".sp_consultaserviciosdomi (pTipo CHAR(1),         --Tipo de consulta deseada
                                           pRfc CHAR (18),        --Rfc a buscar  
										   pNumRegistro SMALLINT) --Numero de Registro de donde emprezara a regresar
	...
	DEFINE sNombreCorto   			CHAR(20);
	DEFINE sCodGrupoAct    			CHAR(4);
	DEFINE sCodGrupoDes	   			CHAR(4);
	DEFINE sCodGrupoReact  			CHAR(4);	
	DEFINE sConvenio	   			CHAR(1);
	DEFINE sCveCanal	   			CHAR(2);
	DEFINE sPresentador    			CHAR(1);
	DEFINE sNumCte		   			CHAR(20);
	DEFINE sNumReintentos 			CHAR(20);
	DEFINE sComisioN	   			CHAR(20);
	DEFINE sComisioNDev   			CHAR(20);
	DEFINE sCuentaCargoComision 	CHAR(20);
	DEFINE sLayoutEspecial 		    CHAR(1);
	...
	LET sComisioN		   			= "";
	LET sComisioNDev      			= "";
	LET sCuentaCargoComision 		= "";
	LET sLayoutEspecial   			= "";

	--****************************************************************
	--SET DEBUG FILE TO "/tmp/sp_ConsultaServiciosDomi.out";
	--TRACE ON;  
	--****************************************************************
	...
	BEGIN 
		...
		ON EXCEPTION SET sql_err
			IF sql_err <> 0 THEN
				let sCodret = sql_err;
				RETURN sCodret, sRfc, sRazonSocial, sNombreCorto, sConvenio, sCveCanal, sPresentador, sNumCte, sNumReintentos, sComisioN,
				sComisioNDev, sCuentaCargoComision, sLayoutEspecial, sCodGrupoAct, sCodGrupoDes, sCodGrupoReact; --Regresa Resultados
			END IF;
		END EXCEPTION;
			
		IF NVL(pTipo,'') = '' OR (pTipo <> '1' AND pTipo <> '2' AND pTipo <> '3') OR NVL(pNumRegistro,'') = '' THEN
			LET sCodret= "00110";
			RETURN sCodret, sRfc, sRazonSocial,sNombreCorto,sConvenio,sCveCanal,sPresentador,sNumCte,sNumReintentos,sComisioN,sComisioNDev,sCuentaCargoComision,sLayoutEspecial,sCodGrupoAct,sCodGrupoDes,sCodGrupoReact; --Regresa Resultados
		END IF 

		IF pTipo = '1' THEN
		   FOREACH --Realiza una consulta donde obtiene los servicios a domiciliar 
				SELECT SKIP pNumRegistro rfc, razon_social, nombre_corto, convenio, cve_canal, presentador, num_cte, num_reintentos, comision,
				comision_dev, cuenta_cargo_comision, layout_especial, cod_grupo_act, cod_grupo_des, cod_grupo_react
				INTO sRfc, sRazonSocial, sNombreCorto, sConvenio, sCveCanal, sPresentador, sNumCte, sNumReintentos, sComisioN,
				sComisioNDev, sCuentaCargoComision, sLayoutEspecial, sCodGrupoAct, sCodGrupoDes, sCodGrupoReact
				...
				RETURN sCodret, sRfc, sRazonSocial, sNombreCorto, sConvenio, sCveCanal, sPresentador, sNumCte, sNumReintentos, sComisioN, 
				sComisioNDev, sCuentaCargoComision, sLayoutEspecial, sCodGrupoAct, sCodGrupoDes, sCodGrupoReact WITH RESUME; --Regresa Resultados
		   END FOREACH;
		ELIF pTipo = '2' THEN  --Opcion 2  Obtiene la razon social de un servicio proporcionando el RFC
			
			IF NVL(pRfc,'') = '' THEN --Valida que el RFC obtenido al inicio no sea nulo 
				LET sCodret= "00110";
				RETURN sCodret, sRfc, sRazonSocial, sNombreCorto, sConvenio, sCveCanal, sPresentador, sNumCte, sNumReintentos, sComisioN, sComisioNDev,
				sCuentaCargoComision, sLayoutEspecial, sCodGrupoAct, sCodGrupoDes, sCodGrupoReact WITH RESUME; --Regresa Resultados
			END IF;
			
			SELECT razon_social, cod_grupo_act, cod_grupo_des, cod_grupo_react
			INTO sRazonSocial, sCodGrupoAct, sCodGrupoDes, sCodGrupoReact
			FROM bdidomi:dom_cat_servicios
			WHERE rfc = pRfc ;			
			
			RETURN sCodret, sRfc, sRazonSocial, sNombreCorto, sConvenio, sCveCanal, sPresentador, sNumCte, sNumReintentos, sComisioN, sComisioNDev,
			sCuentaCargoComision, sLayoutEspecial, sCodGrupoAct, sCodGrupoDes, sCodGrupoReact; --Regresa Resultados
		ELIF pTipo = '3' THEN --Se agrega el pago de tarjetas de crédito
			FOREACH
				SELECT SKIP pNumRegistro rfc, razon_social, nombre_corto, convenio, cve_canal, presentador, num_cte, num_reintentos, comision,
				comision_dev, cuenta_cargo_comision, layout_especial, cod_grupo_act, cod_grupo_des, cod_grupo_react
				INTO sRfc, sRazonSocial, sNombreCorto, sConvenio, sCveCanal, sPresentador, sNumCte, sNumReintentos, sComisioN,
...
'DESCRIPCION: Se encarga de Obtener los servicios que se pueden domiciliar. Realiza una consulta a bdidomi:dom_cat_servicios o de obtener la razon social segun el rfc.',  	
'FECHA      : 2009/08/13',
'VERSION    : 20090813.0924, 20091007.0931',
'BD         : BDIDOMI',
'CASO DE USO: Caso de uso asociado PCU-Domciliacion\CU-0007-ConsultarServiciosDomi-SPL',
'MODIFICO	: Armida Pazos',
'DESCRIPCIÓN: Se agrega el pago de tarjetas de crédito',
'VERSIÓN	: 20100202.1550';

CREATE PROCEDURE "informix".sp_valferiadobanca(pEmpresa 	  		CHAR(3),
...
CREATE PROCEDURE "informix".sp_bancosdom(iRegOmitidos SMALLINT)
RETURNING
	CHAR (6) AS Retorno,	--Codigo de retorno
	CHAR (3) AS Clave,		--Tipo cuenta
	...
	--INICIALIZAR VARIABLES
		...
		BEGIN
			...
			ON EXCEPTION SET sql_err
				IF sql_err <> 0 THEN
					let cCodretorno = sql_err;
					RETURN cCodretorno, cClave, cDescripcion; --Regresa Resultados
				END IF;
			END EXCEPTION; 
					
			--SET DEBUG FILE TO '/tmp/sp_tiposcuentasdomi.out';
			--TRACE ON;
			
			SET ISOLATION TO DIRTY READ;
			FOREACH	
				SELECT SKIP iRegOmitidos banco, descripcion
				INTO cClave, cDescripcion
				FROM bdinteg:si_bancos
				WHERE flg_domi_r = 1 and banco <> '137'
				
				RETURN cCodretorno, cClave, cDescripcion WITH RESUME;
			END FOREACH;
		END;
END PROCEDURE
DOCUMENT
'Autor		: Armida Pazos',
'Descripción: Obtiene los bancos para pago domiciliado.',
'Fecha		: 2010/02/03',
'Versón		: 20100203.1230',
'BD         : bdidomi';

CREATE PROCEDURE "informix".sp_tarjetascreditodomi(cEmpresa CHAR(3), cNumcte CHAR(9), iRegistrosOmitidos SMALLINT)
	RETURNING	CHAR  (6) AS Retorno,--Codigo de Retorno
				CHAR (20) AS NumTarjeta, --Número de tarjeta
...
--Inicializar Variables
	...
	LET cStatusTarjeta				= '';
 	...
 	--SET debug FILE TO "/tmp/ sp_tarjetascreditodomi.out";
	--Trace ON;
	...
	BEGIN
			...
			ON EXCEPTION SET sql_err
				IF sql_err <> 0 THEN
					let cCodret = sql_err;
					RETURN cCodret, cNumTarjeta, cNumCredito; --Regresa Resultados
				END IF;
			END EXCEPTION;
			...
			FOREACH
				...
				INTO cNumTarjeta, cNumCredito, cStatusTarjeta
				FROM bdicred : "informix".sd_tarjeta
				WHERE numcte = cNumcte
				
				SELECT status_cred, NVL(id_unidad_prod, 0), num_producto 
				INTO cStatusCred, iIdUnidadProd, cNumProducto 
				...
				IF cStatusCred <> 'CV' AND iIdUnidadProd <> 2 AND iIdUnidadProd <> 4 AND cStatusTarjeta <> 'C' THEN
					LET iFlag = 1;
				END IF
				...
				IF EXISTS (SELECT cve_producto FROM bdidomi : "informix".dom_prod_permitidos_tc WHERE cve_producto = cNumProducto) THEN
					IF iFlag = 1 THEN
						...
						RETURN cCodret, cNumTarjeta, cNumCredito	
						WITH RESUME;
					END IF
			...
			IF iContador = 0 THEN
				LET cCodret = '000002'; --No hay tarjetas del cliente
				RETURN cCodret, cNumTarjeta, cNumCredito;
			END IF;
...
DOCUMENT
'Autor		: Armida Pazos',
'Descripción: Carga las tarjetas de crédito que pertenezcan al cliente.',
'Fecha		: 2010/02/02',
'Versón		: 20100219.1656',
'BD         : bdidomi';

CREATE PROCEDURE "informix".sp_tiposcuentasdomi(iRegOmitidos SMALLINT)
RETURNING
	CHAR (6) AS Retorno,	--Codigo de retorno
	CHAR (2) AS Clave,		--Tipo cuenta
	...
	--INICIALIZAR VARIABLES
		...
		BEGIN
			...
			SET ISOLATION TO DIRTY READ;
			FOREACH
				SELECT SKIP iRegOmitidos tipo_cta, descripcion
				INTO cClave, cDescripcion
				FROM bdidomi:"informix".dom_tipo_cta
				WHERE tipo_cta in ('03','40')
...
DOCUMENT
'Autor		: Armida Pazos',
'Descripción: Carga el tipo de cuentas para el pago domiciliado.',
'Fecha		: 2010/02/02',
'Versón		: 20100202.1850',
'BD         : bdidomi';
