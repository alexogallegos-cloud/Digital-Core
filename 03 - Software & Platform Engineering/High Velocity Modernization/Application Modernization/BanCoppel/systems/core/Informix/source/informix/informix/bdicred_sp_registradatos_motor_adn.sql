CREATE PROCEDURE "informix".sp_registradatos_motor_adn(
----Parametros de entrada
	...
	cProducto CHAR(4),     
	cNuevoStatus CHAR(2),   
	cCausaSolicitud CHAR(4),  
	vMensajeStatus CHAR(80),    
	cRazondeclinacion CHAR(100),
	ESTADO_CIVIL_VAR_INT CHAR(50),   
	PuntosESTADO_CIVIL_VAR_INT INTEGER,   
	cSegmento CHAR(1),       
	v_ingresomensual_lc DECIMAL(10,2),    
	pmonto_autorizado DECIMAL(14,2),    
	vAntiguedad            INTEGER,
	dIngresoAjustado DECIMAL(10,2),
	dMoraCoppel DECIMAL(10,2),
	dMoraBancoppel DECIMAL(10,2),
	dSaldoVencidoCoppel DECIMAL(10,2),
	dSaldoVencidoBanCoppel DECIMAL(10,2),
	dQuebranto DECIMAL(10,2),
	cEficiencia CHAR(50),
	cSituacionEspecial CHAR(50),
	iReestructura INTEGER,
	iFraudes INTEGER,
	iListaNegra INTEGER,
	cActividad CHAR(8),--
	iIdentFalsa INTEGER,
	iNoTramiteDiaTDC INTEGER,
	iNoTramiteDiaPP INTEGER,
	cModelo CHAR(50),
	cTipoSegmento CHAR(50),
	v_valor_2s INTEGER,
	dMaximoMonto DECIMAL(10,2),
	dmonto DECIMAL(10,2),
	dCuota DECIMAL(10,2),
	iEmpleado INTEGER,
	cTipoEmpleadoCode CHAR(50),
	cTipoEmpleadoName CHAR(50),
	--sp_registradatos_motor
	dValor_IngresoMensual INTEGER,   
	mIngreso_Mensual INTEGER,     
	out_Puntos_grupo_originacion INTEGER,     
	vVar_Grupo_Sol CHAR(50),      
	dValorVI_Genero_Edad INTEGER,       
	VI_Genero_Edad INTEGER,         
	dValorVI_Genero_Ocupacion INTEGER,  
	VI_Genero_Ocupacion INTEGER,    
	dValorVI_EdoCivil_Escolaridad INTEGER,  
	VI_EdoCivil_Escolaridad INTEGER,    
	dValorVI_Entidad_Localidad INTEGER,    
	iElemEntidadLocalidad INTEGER,       
	iPuntosEdoCivilTmpoEdoCivil INTEGER,
	iElemEdoCivilTmpoEdoCivil INTEGER,
	VI_TpResid_TmpResid INTEGER,              
	dValorVI_TpResid_TmpResid INTEGER,       
	VI_Edad_Escolaridad INTEGER,           
	dValorVI_Edad_Escolaridad INTEGER,       
	dPorcentajeMaxDisposicion DECIMAL(10,2),
	iPuntosTipoResidencia INTEGER,
	iElemTipoResidencia INTEGER,
	iPuntosTiempoResidencia INTEGER,
	iElemTiempoResidencia INTEGER,
	iPuntosEdad INTEGER,
	iElemEdad INTEGER,
	iPuntosEscolaridad INTEGER,
	iElemEscolaridad INTEGER,
	iPuntosGenero INTEGER,
	iElemGenero INTEGER,
	iPuntosConsultasFinancieras INTEGER,
	iElemConsultasFinancieras INTEGER,
	iElemOcupacion INTEGER,
	iPuntosOcupacion INTEGER,
	dPuntosPromIngresoUlt DECIMAL(10,2),
	dElemPromIngresoUlt DECIMAL(10,2),
	dPuntosContinuidadDepNom DECIMAL(10,2),
	dElemContinuidadDepNom INTEGER,
	dContinuidadDepNom VARCHAR(24),
	Origenout1 CHAR(50),--tasasiniva
	Origenout2 CHAR(50),
	Origenout3 CHAR(50),
	Origenout4 CHAR(50),
	Origenout5 INTEGER,
...
DEFINE cTipo_sol            CHAR(1);        --tipo de solicitud
DEFINE dMonto_Hipoteca MONEY;
DEFINE cRespSic CHAR(1);
DEFINE v_compromisos_sic_lc MONEY(14,2);
DEFINE cCodRetAdn           CHAR(6);
DEFINE iValidoAdn           SMALLINT;
...
LET dMonto_Hipoteca = 0;
LET cRespSic = 'X';
LET v_compromisos_sic_lc = 0;
...
--BD: BDICRED';
	...
	--SET debug file to '/home/e99805455/sp_registradatos_motor_adn'||trim(pNumSol)||'.out';
	--TRACE ON;
...
BEGIN
	...
	ON EXCEPTION SET vsqlerr, isam_err, error_info
		IF vsqlerr != 0 THEN
			...
			INSERT INTO bdisolic:"informix".ax_paso values ("bdicred:sp_registradatos_motor_adn", vsqlerr, CURRENT ||error_info||' sol '||TRIM(pNumSol));
			IF wbegin = 'S' THEN
				ROLLBACK WORK;
				BEGIN WORK;
			ELSE
	...
	LET cCausaSolicitud = TRIM(NVL(cCausaSolicitud,''));
	EXECUTE PROCEDURE bdinteg:"informix".sp_eliminaacentos(vMensajeStatus)
	INTO vMensajeStatus;
	
	----
	BEGIN WORK;

	INSERT INTO bdisolic:"informix".ss_certif_evaluacion_salida_adn(
		empresa_ss, numsol_ss, numctebanco_ss, producto_ss, estatus_solicitud_ss, 
		causasolicitud_ss, mensajestatus_ss, razondeclinacion_ss, elemestadocivil_ss, puntosestadocivil_ss,
		csegmento_ss, ingreso_mensual_ss, monto_autorizado_ss, antiguedad_ss, ingreso_ajustado_ss,
		mora_coppel_ss, mora_bancoppel_ss, saldo_vencido_coppel_ss, saldo_vencido_bancoppel_ss, quebranto_ss,
		eficiencia_ss, situacion_especial_ss, reestructuras_ss, fraudes_ss, lista_negra_ss, 
		actividad_ss, identificacion_falsa_ss, no_tramitedia_tdc_ss, no_tramitedia_pp_ss, modelo_ss,
		tipo_segmento_ss, score_ss, maximo_monto_ss, monto_ss, cuota_ss, 
		empleado_ss, tipo_empleado_code_ss, tipo_empleado_name_ss, puntos_ingresomensual_ss, elemingreso_mensual_ss,
		puntos_grupo_originacion_ss, grupo_originacion_ss, puntosgenero_edad_ss, elemgenero_edad_ss, puntosgenero_ocupacion_ss,
		elemgenero_ocupacion_ss, puntos_edocil_escolaridad_ss, elemedocivil_escolaridad_ss, puntos_entidad_localidad_ss, elementidad_localidad_ss,
		puntosedocivil_tmpoedocivil_ss, elemedocivil_tmpoedocivil_ss, elem_tpresid_tmpresid_ss, puntos_tpresid_tmpresid_ss, elem_edad_escolaridad_ss,
		puntosvi_edad_escolaridad_ss, porcentaje_maximodisposicion_ss, puntostiporesidencia_ss, elemtiporesidencia_ss, puntostiemporesidencia_ss,
		elemtiemporesidencia_ss, puntosedad_ss, elemedad_ss, puntosescolaridad_ss, elemescolaridad_ss, 
		puntosgenero_ss, elemgenero_ss, puntosconsultasfinancieras_ss, elemconsultasfinancieras_ss, elemocupacion_ss,
		puntosocupacion_ss, puntos_promedio_ingresom_ult4d_ss, elem_promedio_ingresom_ult4d_ss, puntos_continuidad_depositos_nomina_ss, elem_continuidad_depositos_nomina_ss,
		continuidad_depositos_nomina_ss, origenout1_ss, origenout2_ss, origenout3_ss, origenout4_ss,
	origenout5_ss, origenout6_ss, origenout7_ss, origenout8_ss, fecha_insert, edition_ss, editiondate_ss)
	values(
		pEmpresa, pNumSol, pNumCteBanco, cProducto, cNuevoStatus,   
	    cCausaSolicitud, vMensajeStatus, cRazondeclinacion, ESTADO_CIVIL_VAR_INT, PuntosESTADO_CIVIL_VAR_INT,
	    cSegmento, v_ingresomensual_lc, pmonto_autorizado, vAntiguedad, dIngresoAjustado,
	    dMoraCoppel, dMoraBancoppel, dSaldoVencidoCoppel, dSaldoVencidoBanCoppel, dQuebranto,
	    cEficiencia, cSituacionEspecial, iReestructura, iFraudes, iListaNegra,
	    cActividad, iIdentFalsa, iNoTramiteDiaTDC, iNoTramiteDiaPP, cModelo,
	    cTipoSegmento, v_valor_2s, dMaximoMonto, dmonto, dCuota,
	    iEmpleado, cTipoEmpleadoCode, cTipoEmpleadoName, dValor_IngresoMensual, mIngreso_Mensual,
	    out_Puntos_grupo_originacion, vVar_Grupo_Sol, dValorVI_Genero_Edad, VI_Genero_Edad, dValorVI_Genero_Ocupacion,
	    VI_Genero_Ocupacion, dValorVI_EdoCivil_Escolaridad, VI_EdoCivil_Escolaridad, dValorVI_Entidad_Localidad, iElemEntidadLocalidad,
		iPuntosEdoCivilTmpoEdoCivil, iElemEdoCivilTmpoEdoCivil, VI_TpResid_TmpResid, dValorVI_TpResid_TmpResid, VI_Edad_Escolaridad,
		dValorVI_Edad_Escolaridad, dPorcentajeMaxDisposicion, iPuntosTipoResidencia, iElemTipoResidencia, iPuntosTiempoResidencia,
		iElemTiempoResidencia, iPuntosEdad, iElemEdad, iPuntosEscolaridad, iElemEscolaridad,
		iPuntosGenero, iElemGenero, iPuntosConsultasFinancieras, iElemConsultasFinancieras, iElemOcupacion,
		iPuntosOcupacion, dPuntosPromIngresoUlt, dElemPromIngresoUlt, dPuntosContinuidadDepNom, dElemContinuidadDepNom,
		dContinuidadDepNom, Origenout1, Origenout2, Origenout3, Origenout4,
	    Origenout5, Origenout6, Origenout7, Origenout8, current, sEdition, cEditionDate
		);
	...
	SET tp_gen_planpago = 3 --vTipoHit para ADN simpre es 3  
	WHERE empresa = pEmpresa 
	AND num_solicitud = pNumSol;  				
	...
	IF pmonto_autorizado = 0 THEN --v_lineaban
		
		SELECT LIMIT 1 movil_cuenta   --cuenta_nomina,
				INTO cTelCel   --cCuenta
		...
		EXECUTE PROCEDURE bdimnsj:"informix".sp_registra_evento (1 , 'ADN_SMS' ,'ADN_SMS_3' , '000000000','', '','1', '', '', '', '', '', '', '', '', '', '', '', cTelCel, 0, 0,0, 0, 0, current, current) INTO scod_ret;	--Falta validar si se va a enviar el sms
					
		UPDATE bdisolic:"informix".ss_adn_solicitudcuenta
			SET num_solicitud = '', linea = pmonto_autorizado --v_lineaban		
		WHERE numcte = pNumCteBanco;
				
	END IF;
				
	UPDATE bdisolic:"informix".ss_adn_solicitudcuenta
		SET linea = pmonto_autorizado --v_lineaban
	WHERE empresa = pEmpresa
	AND numcte  =  pNumCteBanco;
	
	UPDATE bdisolic:"informix".ss_solicitudes
		SET monto_solicitado = pmonto_autorizado --acp
	WHERE empresa = pEmpresa 
	AND num_solicitud = pNumSol;
	...
	SET	ingreso_mensual_lc = v_ingresomensual_lc, 
	monto_hipoteca = dMonto_Hipoteca,
	evalua_cc = cRespSic,
	...
	insert into bdisolic:"informix".ss_detalle_modelo values(pEmpresa,pNumSol,'Entidad_&_Localidad',iElemEntidadLocalidad,current,user);
	--NUEVAS VARIABLES
	--insert into bdisolic:"informix".ss_detalle_modelo values(pEmpresa,pNumSol,'Entidad_&_Localidad',iElemEdoCivilTmpoEdoCivil,current,user);
	

	--Agregar elemtos y puntajes de la svariables nuevas en inser normales
	...
	where tipo_parametrico='2'
	and tp_solicitud=cTipo_sol
	and (grupo=68 and iElemEntidadLocalidad BETWEEN rango_min AND rango_max);
...
DOCUMENT
'----------------------------------------------------------------------------',
'Descripcion : Se crea procedimiento "sp_registradatos_motor_adn" para el registro de variables BRM ',
