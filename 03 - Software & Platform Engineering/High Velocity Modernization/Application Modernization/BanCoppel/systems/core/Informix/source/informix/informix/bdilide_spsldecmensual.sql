CREATE PROCEDURE "informix".spsldecmensual(p_dfechaproceso DATE,
								p_susuario CHAR(8),
								pTipoDecl CHAR(1),
	...
	--SET DEBUG FILE TO "/triad/entrada/lflores/spsldecmensual.out";
	--TRACE ON;
	...
	BEGIN
		...
		--END FOREACH
			...
			CREATE TABLE bdilide:"informix".tmp_ideRecaudacionescteMensual
			(
		...
		END FOREACH
			...
			--Obtiene la sumatoria del imp_recaudado, selecciona el num_cte y la fecha_ret donde el aniomes sea mayor o igual a enero y menor igual que el mes del parametro
		
			/* SE COMENTA YA NO SE UTILIZA LA RECAUDACIÓN DEL IMPUESTO
			SELECT 
			SUM(ROUND(imp_recaudado,0)) as imp_recaudado,num_cte,
			fecha_ret as fecha_ret, aniomes as aniomes
			FROM bdilide:"informix".sl_detlide
			WHERE aniomes < v_saniomes
			AND aniomes >= v_smesenero
			AND MONTH(fecha_ret) = v_imes AND YEAR(fecha_ret) = v_ianio
			GROUP BY num_cte, fecha_ret,aniomes
			ORDER BY fecha_ret
			INTO TEMP tmp_sl_detlideRecAnt WITH NO LOG;
			
			LET v_smensaje = 'SE GUARDO EN tmp_sl_detlideRecAnt';
			*/
			
			/*SELECT DISTINCT (a.num_cte) as cliente FROM bdilide:"informix".sl_retlide as a, bdilide:"informix".sl_detlide as b
			WHERE (a.aniomes <= v_saniomes and a.aniomes >= v_smesenero ) and a.num_cte = b.num_cte
			...
			*/
			...
			--SE CREA TABLA FISICA TEMPORAL
			CREATE TABLE bdilide:"informix".tmp_DatosClientesIde
			(
				tpo_persona char(2),
		...
		END FOREACH	
			...
			--SE CREA TABLA FISICA TEMPORAL
			CREATE TABLE bdilide:"informix".tmp_DireccionesClientesIde
			(
			nombrecalle char(30),
		...
		FOREACH 
			...
			/* SE COMENTA YA NO SE UTILIZA LA RECAUDACIÓN DEL IMPUESTO
			SELECT {+INDEX(bdilide:tmp_sl_detlideRecAnt idx_tmp_detlide )}
			ROUND(NVL(SUM(imp_recaudado),0) ,0)
			INTO v_mimp_remanente
			FROM tmp_sl_detlideRecAnt
			WHERE aniomes < v_saniomes
			AND aniomes >= v_smesenero
			AND num_cte = v_snum_cte
			AND fecha_ret = v_dfecha_ret
			GROUP BY num_cte, fecha_ret;
			*/
		...
		FOREACH
			...
			-- Selecciona las retenciones que se hayan recaudado en el mes y generado en meses anteriores
			
			SELECT num_cte, ROUND(NVL(SUM(imp_recaudado),0),0), fecha_ret
			INTO v_snum_cte, v_mimp_remanente, v_dfecha_ret
			FROM tmp_sl_detlideRecAnt
			/*WHERE aniomes < v_saniomes
			AND aniomes >= v_smesenero
			AND MONTH(fecha_ret) = v_imes AND YEAR(fecha_ret) = v_ianio
			...
			LET v_mimp_gravado = 0;
			LET v_mimp_arecaudar = 0;
			LET v_mimp_recaudado = 0;
			LET mRecPendiente = 0;

			IF NOT EXISTS (SELECT {+INDEX(bdilide: sl_menxml idx_sl_menxml04)} num_cte FROM bdilide:"informix".sl_menxml WHERE  fechaxml = v_saniomes AND fecha_ret = v_dfecha_ret AND tpo_persona IS NOT NULL AND num_cte = v_snum_cte ) THEN
				-- Selecciona los datos del cliente
				SELECT {+INDEX(bdilide:tmp_DatosClientesIde idx_tmp_si_cte)}  NVL(tpo_persona,'01'), apell_paterno, apell_materno, nombre1, nombre2, razon_social, NVL(rfc,''), NVL(curp,'')
				INTO v_stpo_persona, v_sapell_paterno, v_sapell_materno, v_snombre1, v_snombre2, v_srazon_social, v_srfc,v_scurp
				FROM tmp_DatosClientesIde
				WHERE cliente = v_snum_cte;
				...
				END IF
	   			...
	   			INSERT INTO bdilide:"informix".sl_menxml (num_cte, tpo_persona, rfc, curp, razon_soc, apell_pat, apell_mat, nombre1, nombre2,
				nom_calle, num_ext, num_int, nom_colonia, cod_post, fecha_ret, montoexcedente, impdeterminado, imprecaudado, recpendiente, 	remrecaudado, fechaxml)
				VALUES(v_snum_cte, v_stpo_persona, v_srfc, v_scurp, v_srazon_social, v_sapell_paterno, v_sapell_materno, v_snombre1, v_snombre2,v_snombrecalle, v_snumeroextcalle, v_snumerointcalle, v_snombrezona, v_scod_postal, v_dfecha_ret, v_mimp_gravado,
				v_mimp_arecaudar,v_mimp_recaudado, mRecPendiente , v_mimp_remanente, v_saniomes);
				
			END IF;
				LET nCont = nCont + 1;
				IF nCont = 30000 THEN	--DSB 01-11-2011
				    UPDATE STATISTICS MEDIUM FOR TABLE bdilide:"informix".sl_menxml;
					LET nCont= 0;
				END IF;
		END FOREACH
		...
		*/
		...
		IF v_imes = '01' THEN 
			...
			FROM bdilide:"informix".sl_menxml 
			--FROM bdilide:"informix".sl_menxml a
			--WHERE a.fecha_ret = (SELECT MAX(b.fecha_ret) FROM bdilide:"informix".sl_menxml b WHERE b.num_cte = a.num_cte AND  fechaxml = v_saniomes);
			--WHERE a.fecha_ret =fech_ret;
		...
		ELSE
			...
			GROUP BY num_cte
				...
				IF EXISTS(SELECT {+INDEX(bdilide: sl_menxml 116_375)} 1 FROM bdilide:"informix".sl_menxml WHERE num_cte = v_snum_cte AND fechaxml = v_saniomes) THEN
					...
					END FOREACH;
				----ELSE
				/*	IF NVL(v_msaldopenrec,0) > 0 THEN --Se agrega validacion solo insertar el registro en caso de que se tenga saldo pendiente por recaudar
						--Obtener los datos del cliente del ultimo registro que encuentre
						SELECT num_cte, tpo_persona, rfc, curp, razon_soc, apell_pat, apell_mat, nombre1, nombre2,nom_calle, num_ext, num_int, nom_colonia, cod_post, fecha_ret
						INTO v_snum_cte, v_stpo_persona, v_srfc, v_scurp, v_srazon_social, v_sapell_paterno, v_sapell_materno, v_snombre1, v_snombre2,v_snombrecalle, v_snumeroextcalle, v_snumerointcalle, v_snombrezona, v_scod_postal, v_dfecha_ret
						FROM bdilide:"informix".sl_menxml
						WHERE num_cte = v_snum_cte AND fecha_ret = (SELECT MAX(fecha_ret) FROM bdilide:"informix".sl_menxml WHERE num_cte = v_snum_cte);
						
						--Se obtiene la fecha de cierre del mes
						EXECUTE PROCEDURE bdicheq:"informix".sp_dia_primero_ultimo_mes_anio(v_imes,v_ianio) INTO v_scodret,v_dfecha_ret_pas, v_dfecha_ret;
						
						INSERT INTO bdilide:"informix".sl_menxml (num_cte,tpo_persona,rfc,curp,razon_soc,apell_pat,apell_mat,nombre1,nombre2,nom_calle,num_int,num_ext,
						nom_colonia,cod_post,fecha_ret,montoexcedente,impdeterminado,imprecaudado,recpendiente,remrecaudado,fechaxml,saldopenrec) 
						VALUES (v_snum_cte, NVL(v_stpo_persona,'01'), NVL(v_srfc,''), NVL(v_scurp,''), v_srazon_social, v_sapell_paterno, v_sapell_materno, v_snombre1,
						v_snombre2,v_snombrecalle, v_snumeroextcalle, v_snumerointcalle, v_snombrezona, v_scod_postal,v_dfecha_ret, 0,
						0,0, 0,0, v_saniomes, NVL(v_msaldopenrec,0));
					END IF;    Se documenta debido a que ya no se deben de arrastrar los clientes que no generaron excedente en el mes*/
				END IF;
		...
		-- Genera el total del entero    
		/*SELECT {+INDEX(bdilide:sl_enteros 102_23)}
		SUM(NVL(ROUND(monto,0),0))	
		INTO v_montoentero 
		FROM bdilide:"informix".sl_enteros WHERE MONTH(fech_entero) = v_imes
		AND YEAR(fech_entero) = v_ianio AND monto > 0;*/
