CREATE PROCEDURE "informix".validapolizanominarespaldo(p_empresa CHAR(3), p_usuario CHAR(8))
   ...
   DEFINE v_diasretroact			INTEGER;
   DEFINE v_diasretropermitidos			INTEGER;
...
--Debug del Procedure                                --*
--se intregan validaciones respecto a que el usuario --*
--cuente con un perfil para realizar la integración  --*
--de las polizas y ademas que la poliza no exceda de --*
--los dias retroactivos permitidos.                  --*
 ...
 ***************************************************************************}
   ...
   LET v_diasretroact			 = 0;
   LET v_diasretropermitidos	 = 0;
 ...
 FOREACH
   ...
   --validaciones
    ...
    FOREACH WITH HOLD
        ...
        IF v_usuario IS NULL OR trim(v_usuario) = "" THEN
           ...
           VALUES (v_usuario,
            ...
            --RETURN cod_ret,v_control_poliza;
        ELSE
			SELECT COUNT(*) INTO v_registros FROM bdinteg:si_ejecut a, bdinteg:si_perfil_ejecut b
			WHERE a.empresa = b.cod_emp 
			AND a.ejecutivo = b.ejecutivo
			AND a.empresa   = p_empresa
			AND a.ejecutivo = v_usuario
			AND b.perfil IN('701','703');
    ...
    --END FOREACH
		...
		LET v_diasretroact = v_fecha_captura - v_fecha_valida;
		
		SELECT dias_retroact INTO v_diasretropermitidos FROM bdicont:co_param;
		
		IF v_diasretroact > v_diasretropermitidos THEN
			LET cod_ret = "177";
			SELECT descripcion INTO tmensaje FROM bdinteg:si_codret WHERE codigo_retorno = cod_ret AND sistema = '07';
			INSERT INTO bdicont:tmpco_auditerr
				(usuario,
    ...
    ELSE
        ...
        FOREACH
            ...
            IF v_control_poliza = v_ctrl_poliza THEN
                ...
                FOREACH
                    ...
                    INSERT INTO bdicont:co_detpol
                         ...
                         VALUES (v_usuario, v_control_poliza,
                                ...
                                v_nro_auxiliar, v_naturaleza, v_monto,
                                v_descripcion_det, v_fecha_valida,
                                v_moneda, 0,  0, '', v_usuario, v_naturaleza,
                                v_ccosto_orig);
                END FOREACH;
                ...
                LET v_descripcion_det_c = 'ENCABEZADO DE LA POLIZA GENERADA';
