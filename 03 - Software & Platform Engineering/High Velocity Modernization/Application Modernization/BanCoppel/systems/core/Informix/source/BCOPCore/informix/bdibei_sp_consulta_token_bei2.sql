CREATE PROCEDURE "informix".sp_consulta_token_bei2(pNumCliente char(9))
	...
	END IF;
     SET LOCK MODE TO WAIT 4;
            ...
            AND btk.ns_token NOT IN(SELECT bs.ns_token
            						FROM bdibei:"informix".bei_servicio bs
...
--**************************************************************************************************************
          FOREACH
            ...
            SELECT  btk.id_usuario,btk.num_cliente,btk.ns_token,btk.id_status_token,replace(replace(replace (trim(dtus.nombre),' ','<>' ),'><','' ),'<>',' ' )
            INTO  sIdUsuario,sNumCliente,sNsToken, sIdStatusToken,sNomUser
            FROM bdibei:"informix".bei_token  btk
            LEFT JOIN  bdibei:"informix".bei_usuario us ON  us.id_usuario=btk.id_usuario
            Inner Join bdibei:"informix".bei_datos_usuario dtus On(dtus.id_usuario = btk.id_usuario)
            WHERE btk.num_cliente  = pNumCliente
            AND btk.ns_token NOT IN(SELECT bs.ns_token
            						FROM bdibei:"informix".bei_servicio bs
            						WHERE bs.num_cliente  = pNumCliente AND bs.ns_token IS NOT NULL)

           RETURN cod_ret, NVL(sIdUsuario,-1),NVL(sNumCliente,''),NVL(sNsToken,''), NVL(sIdStatusToken,-1),SUBSTRING(NVL(sNomUser,'') FROM 0 FOR 50) WITH RESUME;
         END FOREACH;
