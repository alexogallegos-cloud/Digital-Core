CREATE PROCEDURE "informix".sp_actualizaestatussolic(pEmpresa CHAR(3), pNumCte CHAR(20), pNumSolicitud CHAR(20), pStatus SMALLINT, pError CHAR(6))
	...
	LET vCodRet = '00000'; --ACTUALIZA STATUS.
...
--SET ISOLATION TO COMMITTED READ LAST COMMITTED;
	...
	--SET debug FILE TO '/tmp/sp_actualizaestatussolic.out';
	--TRACE ON;
	...
	BEGIN
		...
		IF EXISTS (SELECT numcte FROM bdinteg: "informix".si_clientescoppelporenviar WHERE empresa = pEmpresa and numcte = pNumCte AND num_solicitud = pNumSolicitud) THEN

			IF NVL(pError,'0') <> '0' AND NVL(pError,'') <> '' THEN
              IF pError= '-13' AND pStatus = '0' THEN
                LET pStatus = '2';
                UPDATE	bdinteg: "informix".si_clientescoppelporenviar
				SET		status = pStatus , error_devuelto = pError, fecha_actualiza = CURRENT
			...
			ELSE
			    UPDATE	bdinteg: "informix".si_clientescoppelporenviar
				SET		status = pStatus , fecha_actualiza = CURRENT
				WHERE	empresa = pEmpresa
				AND		numcte = pNumCte
				AND		num_solicitud = pNumSolicitud;
			END IF;
		...
		ELSE
			LET vCodRet = '00002'; --NO EXISTE REGISTRO EN LA TABLA.
		END IF;
...
'Fecha:05/11/2013',
'Modificación:Procedimiento que actualiza el status y error que regresa servicio, Se modifica para que no se actualice el campo error_devuelto a cero.',
'Sustento: Se definio por telefono con Rodolfo Gomez y Yadira Morales',
