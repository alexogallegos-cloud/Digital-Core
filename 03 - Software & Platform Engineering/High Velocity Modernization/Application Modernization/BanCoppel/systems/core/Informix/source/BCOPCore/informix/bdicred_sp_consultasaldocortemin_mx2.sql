CREATE PROCEDURE "informix".sp_consultasaldocortemin_mx2(pEmpresa  CHAR (3),pNumCredito CHAR (20), pTipoConsulta SMALLINT)
...
BEGIN
	...
	--SET DEBUG FILE TO '/tmp/bdicred/sp_consultasaldocortemin.out';
    --TRACE ON;	
...
--INI: 31/10/2018 GM2 JUAN OLIVARES: SE ANEXA pTipoConsulta 5 PARA USO DE BANCOPPEL EXPRESS.
		IF ( pTipoConsulta = 5 ) THEN -- Saldo actual
			...
			CASE WHEN a.status_cred IN ('BT','E2','E3') THEN
             ...
             AND b.fecha = fecha_cuota)end),0),
                   ...
                   nvl(( case when (nvl(monto_financiado,0) < 0) then 0
                          ...
                          case when NVL(int_tra_no_exig,0) > 0 
                              then  NVL(int_tra_no_exig,0) - NVL((select interes_debe from bdicred:"informix".sd_amortiza_credito where a.num_credito = num_credito and b.fecha = fecha_cuota),0) 
                              else 0
                         end +
    --							case when (NVL(int_tra_no_exig,0) -NVL(sdo_acum_mes_int,0) >=0) then  ---
    --									  (NVL(int_tra_no_exig,0) -NVL(sdo_acum_mes_int,0)) else NVL(int_tra_no_exig,0) end + 
    ...
    -- Se agrega condicion a la consulta de movtos para descartar los pagos anticipados por credisolución '082' AAME 27112017
              ...
              -- Obtiene pagos realizados historicos
                    ...
                    where empresa = pEmpresa
                      and num_credito = pNumCredito
                      and codigo_fun in (select {+INDEX(sd_conceptospagomanual idx_conceptospagomanual)} cod_fun from bdicred:"informix".sd_conceptospagomanual)
                      and codigo_ref = 1
                      and codigo_fun <> '082'
                      and reversado = 'N'
...
'OBJETIVO:Se modifica el indice a dos consultas y la variable NVL',
'VoBo: Juan Olivarez-GM2',
'VoBo: Alejandro Sanchez-GM1',
'BD: BDIBPI';

CREATE PROCEDURE "informix".sp_activa_insertos_fijoscrd(pEmpresa CHAR(3),dtFechaCalculo DATE)
RETURNING
          CHAR(5) AS resultado;
...
--SET DEBUG FILE TO "sp_activa_insertos_fijoscrd";
--TRACE ON;
...
-- Se obtienen los créditos que se tienen 1 pago vencido.
FOREACH WITH HOLD
  ...
  GROUP BY num_credito HAVING (COUNT(*)=1)
    ...
    IF (icontador >= 60000) THEN
        COMMIT WORK;
		LET cBanBegin  = 'N';
        UPDATE STATISTICS MEDIUM FOR TABLE "informix".sd_marcaje;
        LET icontador = 1;
        BEGIN WORK;
		LET cBanBegin= 'S';
    END IF;
...
END FOREACH;
  ...
  IF (icontador > 0) OR cBanBegin= 'S' THEN
    COMMIT WORK;
    LET cBanBegin  = 'N';
  END IF;
...
'la información del inserto correspondiente',
'a los definidos por el usuario operativo',
'para créditos reestructurados',
