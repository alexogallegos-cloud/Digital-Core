CREATE PROCEDURE "informix".sp_generareportepp(pNumSolicitud CHAR(20), pSucursal CHAR(5), cPlazo CHAR(2),iMontoTotal DECIMAL (16,2),
...
--	  SET DEBUG FILE TO "/tmp/sp_generareportepp.out";
	--TRACE ON;
			...
			IF NOT EXISTS ( SELECT num_solicitud FROM bdisolic:"informix".ss_solicitudes where num_solicitud = pNumSolicitud  AND num_producto = pProducto) THEN
					LET cCodret	='00120'; -- EL NÚMERO DE SOLICITUD PARA EL pProducto NO EXISTE
					RETURN cCodret,'','','','','','','','','','','','','','','','','','';
			END IF
			...
			IF pProducto = '6400' THEN				
					...
					IF pFrecuencia = 2 THEN
						...
						IF cCodRet::INTEGER <> 0  THEN	
							LET cCodRet    = "00140";	--Ocurrio un Error al obtener la fecha de pago del crédito para credinomina.
							RETURN cCodret,'','','','','','','','','','','','','','','','','','';
						END IF;	
						...
						END IF;	
						END IF;
						LET cDiasdePago = "Los días "|| iDiaPago|| " y " ||DAY(dtFechaT)||" de cada mes.";
					ELSE
						LET cDiasdePago = "Los días "|| iDiaCorte|| " de cada mes.";
					END IF;		
			...
			IF pTipo = 1 THEN 
					--VALOR DE LA COMISIÓN A COBRAR POR PAGO ANTICIPADO
					SELECT apli_factor 
					INTO cApli_factor 
					FROM bdicred:"informix".sd_tpcomis 
					WHERE cod_comis = '6903'; 
					
					--EN ESTE CICLO SE EJECUTA EL PROCEDIMIENTO PROYECTA PRÉSTAMO PARA OBTENER EL VALOR TOTAL A PAGAR
					FOREACH 
					...
					RETURN cCodret, cNumCte, cNombre,pNumSolicitud,cDiasPago,cCuenta,cIva,iMontoTotal,cInteresOrdinario,cPlazo,cInteresMoratorio,cPeriodoPlazo,cFecha,cDireccion,cCiudad,cCapacidad_pres,cApli_factor,iMontoTotalCredito,cDiasdePago   WITH RESUME;
			--SECCION PARA EL PAGARÉ 
			ELIF pTipo = 2 THEN
					--OBTIENE LA CIUDAD
					...
					RETURN cCodret, cNumCte, cNombre,pNumSolicitud,cDiasPago,cCuenta,cIva,iMontoTotal,cInteresOrdinario,cPlazo,cInteresMoratorio,cPeriodoPlazo,cFecha,cDireccion,cCiudad,cCapacidad_pres,cApli_factor,iMontoTotalCredito,cDiasdePago  WITH RESUME;
			--SECCION DE CODIGO PARA EL TIPO 3 (REIMPRESIÓN)
			ELIF pTipo = 3 THEN 
					SELECT num_cta
					INTO cCuenta
					...
					LET iMontoTotal			= "";
					LET cPlazo				= "";
					--SE OBTENIE LA FECHA, LA SUCURSAL Y LA DIVISA, CUANDO ES REIMPRESIÓN
					...
					END IF;
					LET iMontoTotalCredito= iMontoTotal + cMontoTotales;
					--CDIVISA RETORNARA EN EL CAMPO CIUDAD, YA QUE PARA LA REIMPRESIÓN DE CARATULA ES NECESARIO LA DIVISA Y NO ASI LA CIUDAD, Y CON EL OBJETIVO DE NO AGREGAR UN PARAMETRO D SALIDA MÁS SE REUTILIZARÁ EL CORRESPONDIENTE A LA CIUDAD.
					RETURN cCodret, cNumCte, cNombre,pNumSolicitud,cDiasPago,cCuenta,cIva,iMontoTotal,cInteresOrdinario,cPlazo,cInteresMoratorio,cPeriodoPlazo,cFecha,cDireccion,cDivisa,cCapacidad_pres,cApli_factor,iMontoTotalCredito,cDiasdePago   WITH RESUME;
			END IF;
	END;
END PROCEDURE
