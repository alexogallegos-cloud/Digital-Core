CREATE PROCEDURE "informix".sp_cap_conciliaciontraspasosctas(pFechaInicio DATE, pFechaFin DATE)
	...
	DEFINE vNoCteTmp			VARCHAR(10);
	DEFINE vNoCtaTmp			VARCHAR(20);
	DEFINE vTransaccTmp			VARCHAR(4);
	DEFINE vDesTransaccTmp		VARCHAR(50);
	DEFINE vDesProductoTmp		VARCHAR(50);
	DEFINE mMontoTmp			MONEY;
	
	DEFINE vNoCteAbono			VARCHAR(10);
    ...
    --SET DEBUG FILE TO '/ifxsif01/ilopez/SPL_SOFTEK/sp_cap_conciliaciontraspasosctas.out';
    --TRACE ON;

    SET ISOLATION TO DIRTY READ;
    SET LOCK MODE TO WAIT 3;
    
	select fecha_hoy-1 into pFechaInicio from bdicheq:sc_fechas;
	...
	--LET pFechaInicio = '07032024';
	
	
	--Borrado de registros de la tabla sc_concilia_trasp_cuentas para Re-Run
	let icontadorcommit=0;
	FOREACH WITH HOLD
		 SELECT fecha_alt, folio_suc 
		  INTO dfecha_alt, cfolio_suc 
		  FROM bdicheq:"informix".sc_concilia_trasp_cuentas 
		...
		delete from bdicheq:"informix".sc_concilia_trasp_cuentas 
		where fecha_alt=pFechaInicio
		and folio_suc = cfolio_suc;
		
		let icontadorcommit = icontadorcommit + 1;
	
		if (icontadorcommit >= 500) then
			commit work;
			let icontadorcommit = 0;
		end if;
	...
	--Borrado de registros de la tabla sc_difer_concilia_trasp_cuentas para Re-Run
	let icontadorcommit=0;
	FOREACH WITH HOLD
		 SELECT fecha_alt, folio_suc 
		  INTO dfecha_alt, cfolio_suc 
		  FROM bdicheq:"informix".sc_difer_concilia_trasp_cuentas 
		WHERE fecha_alt = pFechaInicio
		...
		delete from bdicheq:"informix".sc_difer_concilia_trasp_cuentas 
		where fecha_alt=pFechaInicio
		and folio_suc = cfolio_suc;
	...
	SELECT --{+INDEX(sc_movhis idx_cap_sc_movhis)}
		   folio_suc, transacc
	FROM bdicheq:"informix".sc_movhis
	WHERE 
	cancelad <> 'S'
	AND transacc IN ('0205', '0239')
	AND fech_alt=pFechaInicio
	AND ( folio_suc NOT LIKE 'informix%' AND folio_suc NOT LIKE 'agnt70ct%' )
	INTO TEMP tmp_movhis_concilia WITH NO LOG;

		let icontadorcommit=0;	
		UPDATE statistics medium FOR TABLE sc_concilia_trasp_cuentas;
		UPDATE statistics medium FOR TABLE sc_difer_concilia_trasp_cuentas;
		

    FOREACH WITH HOLD
         SELECT COUNT(folio_suc), folio_suc 
          INTO iDupla, vFolioSuc 
          FROM tmp_movhis_concilia
		  GROUP BY folio_suc
        ...
        FOREACH WITH HOLD
            SELECT c.num_cte, a.cuenta, a.transacc, b.descripcion, d.nombre, SUM(a.monto_tot), a.fech_alt 
              INTO vNoCteTmp, vNoCtaTmp, vTransaccTmp, vDesTransaccTmp, vDesProductoTmp, mMontoTmp, dFecha
             ...
             INNER JOIN bdicheq:"informix".sc_maechq AS c ON ( c.empresa = a.empresa AND c.cuenta = a.cuenta )
             INNER JOIN bdicheq:"informix".sc_producto AS d ON ( d.empresa = a.empresa AND d.producto = a.producto )
             WHERE folio_suc = vFolioSuc 
               AND a.transacc IN ('0205', '0239') 			   
             GROUP BY c.num_cte, a.cuenta, a.transacc, b.descripcion, d.nombre, a.monto_tot, a.fech_alt 
            
            IF vTransaccTmp = '0205' THEN
                LET vNoCteAbono			= NVL(vNoCteTmp,'');
                LET vNoCtaAbono			= NVL(vNoCtaTmp,'');
                LET vTransaccAbono		= NVL(vTransaccTmp,'');
                LET vDesTransaccAbono	= NVL(vDesTransaccTmp,'');
                LET vDesProductoAbono	= NVL(vDesProductoTmp,'');
                LET mMontoAbono			= NVL(mMontoTmp,0);
            ELSE
                LET vNoCteCargo			= NVL(vNoCteTmp,'');
                LET vNoCtaCargo			= NVL(vNoCtaTmp,'');
                LET vTransaccCargo		= NVL(vTransaccTmp,'');
                LET vDesTransaccCargo	= NVL(vDesTransaccTmp,'');
                LET vDesProductoCargo	= NVL(vDesProductoTmp,'');
                LET mMontoCargo			= NVL(mMontoTmp,0);
            END IF;
        END FOREACH;	
        ...
        IF vUltimoConciliado <> vFolioSuc THEN
            IF iDupla = 1 THEN
                INSERT INTO bdicheq:"informix".sc_difer_concilia_trasp_cuentas
                (fecha_alt, folio_suc, num_cte_cargo, cta_cargo, transacc_cargo, descrip_trans_cargo, producto_cargo, num_cte_abono, cta_abono, transacc_abono, descrip_trans_abono, product_abono, monto_cargo, monto_abono, diferencia) 
                VALUES
                (dFecha, vFolioSuc, vNoCteCargo, vNoCtaCargo, vTransaccCargo, vDesTransaccCargo, vDesProductoCargo, vNoCteAbono, vNoCtaAbono, vTransaccAbono, vDesTransaccAbono, vDesProductoAbono, mMontoCargo, mMontoAbono, mDiferencia);
                
                LET vUltimoConciliado = vFolioSuc;
				let icontadorcommit = icontadorcommit + 1;
            ELIF iDupla > 1 AND mDiferencia <> 0 THEN
				...
				let icontadorcommit = icontadorcommit + 1;
            ELSE
                INSERT INTO bdicheq:"informix".sc_concilia_trasp_cuentas
                (fecha_alt, folio_suc, num_cte_cargo, cta_cargo, transacc_cargo, descrip_trans_cargo, producto_cargo, num_cte_abono, cta_abono, transacc_abono, descrip_trans_abono, product_abono, monto_cargo, monto_abono) 
                VALUES
                (dFecha, vFolioSuc, vNoCteCargo, vNoCtaCargo, vTransaccCargo, vDesTransaccCargo, vDesProductoCargo, vNoCteAbono, vNoCtaAbono, vTransaccAbono, vDesTransaccAbono, vDesProductoAbono, mMontoCargo, mMontoAbono);
	...
	drop table tmp_movhis_concilia;
    RETURN cCodRet, cMensajeRet;
...
'FECHA: 21/08/2019',
'MODULO: CAPTACION',
'FUNCIONALIDAD: Conciliacion de los traspasos entre cuentas', 
'DESCRIPCION: SPL encargado de Conciliar los traspasos entre cuentas',
'BD: bdicheq',
'AUTOR: Carlos Abraham Sanchez Estrada - Softtek',
'FECHA: 16/10/2024',
'MODULO: CAPTACION',
'FUNCIONALIDAD: Conciliacion de los traspasos entre cuentas', 
'DESCRIPCION: Se realiza mantenimiento aplicando las mejores practicas al SP',
'BD: bdicheq';
