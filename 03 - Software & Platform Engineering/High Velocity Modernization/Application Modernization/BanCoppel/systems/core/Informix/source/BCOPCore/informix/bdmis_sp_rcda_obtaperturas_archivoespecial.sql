CREATE PROCEDURE "informix".sp_rcda_obtaperturas_archivoespecial(pfecha DATE )
...
BEGIN
   ...
   foreach cursor1 WITH HOLD for
		...
		where clave = 'A' --and nombrearchivo = vnombrearchivo and fechageneracion = pfecha 
		group by 1,2,3
   ...
   IF  (SELECT count(*) FROM mi_rcda_altas_bcplrcd WHERE nombrearchivo = vnombrearchivo ) = 0 THEN
		   ...
		   insert into mi_rcda_altas_bcplrcd (nombrearchivo, fecha, promotor, numcte,sucursal) 
		   SELECT '',fechamov,user_insert,numctecoppel,sucursal FROM bdinteg:si_adiccoppel where fechamov = pfecha;
 ...
 -- paso de productividad de sucursales a acumulado mensual
	...
	*/
	...
	foreach cursor1 WITH HOLD for
		...
		INTO	pfecha,vsucursal,vtpo_reg,vpromotor, vproducto, vnumero, vmeta
		FROM mi_rcda_apert_paso
		WHERE  producto = '6566'
		
			IF vtpo_reg = 1 THEN 
			
					IF (SELECT COUNT(*) FROM mi_his_productividad WHERE fecha = pfecha AND sucursal = vsucursal AND tpo_reg = vtpo_reg AND ejecutivo = vpromotor AND producto = vproducto) = 0 THEN
				
							INSERT INTO mi_his_productividad (fecha,sucursal,tpo_reg,ejecutivo,nombre,producto,copentrtdc, copentrmeta) 
								VALUES (pfecha,vsucursal,vtpo_reg,vpromotor,(select nombre from bdinteg:si_ejecut eje where eje.ejecutivo = vpromotor ), vproducto, vnumero, vmeta);
						ELSE	
							
							UPDATE mi_his_productividad SET copentrtdc = copentrtdc + vnumero
							WHERE fecha = pfecha AND sucursal = vsucursal AND tpo_reg = vtpo_reg AND ejecutivo = vpromotor AND producto = vproducto;
				...
				ELSE
					...
					IF (SELECT COUNT(*) FROM mi_his_productividad WHERE fecha = pfecha AND sucursal = vsucursal AND tpo_reg = vtpo_reg AND ejecutivo = vpromotor AND producto = vproducto) = 0 THEN
						...
						ELSE
							...
							DELETE FROM mi_his_productividad WHERE fecha = pfecha AND sucursal = vsucursal AND tpo_reg = vtpo_reg AND ejecutivo = vpromotor AND producto = vproducto ;
							INSERT INTO mi_his_productividad (fecha,sucursal,tpo_reg,ejecutivo,nombre,producto,copentrtdc, copentrmeta) 
								VALUES (pfecha,vsucursal,vtpo_reg,vpromotor,(select nombre from bdinteg:si_ejecut eje where eje.ejecutivo = vpromotor ) ,vproducto, vnumero, vmeta);
