CREATE PROCEDURE "informix".sp_intercard_calcular_periodos( pPeriodicidad CHAR(1), pNumeroDesfase SMALLINT )
    RETURNING CHAR (5) AS rCODIGO_RETORNO, CHAR(50) AS rMENSAJE_RESPUESTA,
                DATETIME YEAR TO FRACTION(5) as rFechaInicial, 
                    DATETIME YEAR TO FRACTION(5) as rFechaFinal, 
                    DATE as rFechaIntegralHoy;
    ...
    DEFINE vDesfaseInicial SMALLINT;
    DEFINE vDesfaseFinal SMALLINT;

    DEFINE PERIODICIDAD_DIARIA CHAR(1);
    DEFINE PERIODICIDAD_QUINCENAL CHAR(1);
    DEFINE PERIODICIDAD_MENSUAL CHAR(1);
    DEFINE PERIODICIDAD_SEMANAL CHAR(1);
    ...
    LET vDesfaseInicial =  0;
    LET vDesfaseFinal = 0;
    LET PERIODICIDAD_DIARIA = 'D';
    LET PERIODICIDAD_QUINCENAL = 'Q';
    LET PERIODICIDAD_MENSUAL = 'M';
    LET PERIODICIDAD_SEMANAL = 'S';
    ...
    BEGIN 
        ...
        ON EXCEPTION SET SQLERR, ISAM_ERR, ERROR_INFO
            ...
            SET DEBUG FILE TO RUTA_ORIGEN || "excepcion_sp_calcular_fechas_semanales.err.out"  WITH APPEND;
            TRACE ON;
        ...
        --SET DEBUG FILE TO RUTA_ORIGEN||"02_sp_intercard_calcular_periodos.out";
        --TRACE ON;
        
        SET ISOLATION TO DIRTY READ;
        SET LOCK MODE TO WAIT 3;        
        
        LET vDesfaseInicial =  pNumeroDesfase;
        LET vDesfaseFinal = 1;
        
        IF ( pPeriodicidad NOT IN (PERIODICIDAD_DIARIA, PERIODICIDAD_QUINCENAL, PERIODICIDAD_SEMANAL, PERIODICIDAD_MENSUAL) ) THEN
            LET CODIGO_RETORNO = '00020';
            LET MENSAJE_RESPUESTA = 'El periodo indicado es incorrecto.';
            RETURN CODIGO_RETORNO, MENSAJE_RESPUESTA, primer_dia_semana_hora, ultimo_dia_semana_hora, vFechaIntegral;
        END IF
        
        IF ( pPeriodicidad = PERIODICIDAD_DIARIA ) THEN
        
            SELECT fecha_hoy,
                    SUBSTRING ( ( EXTEND(fecha_hoy) - vDesfaseInicial UNITS DAY)  FROM 1 FOR 10 )||' '||HORA_MINUTO_SEG_INICIAL,
                        SUBSTRING( ( EXTEND(fecha_hoy) - vDesfaseFinal UNITS DAY)  FROM 1 FOR 10 )||' '||HORA_MINUTO_SEG_FINAL
                    INTO vFechaIntegral, primer_dia_semana_hora, ultimo_dia_semana_hora
            FROM bdinteg:si_fechas
                WHERE empresa = '001';
        
        END IF
        
        IF ( pPeriodicidad = PERIODICIDAD_QUINCENAL ) THEN 
            
            IF ( pNumeroDesfase NOT IN ( 1, 2 ) ) THEN 
                
                LET CODIGO_RETORNO = '00001';
                LET MENSAJE_RESPUESTA = 'Error al calcular los dias quincenales.';
                RETURN CODIGO_RETORNO, MENSAJE_RESPUESTA, primer_dia_semana_hora, ultimo_dia_semana_hora, vFechaIntegral;
            ...
            IF ( pNumeroDesfase = 1 ) THEN 
                SELECT fecha_hoy, 
                        SUBSTRING( ( EXTEND(pri_dia_mes) - 1 UNITS MONTH )  FROM 1 FOR 10 )||' '||HORA_MINUTO_SEG_INICIAL,
                        SUBSTRING( ( EXTEND(pri_dia_mes + 14 ) - 1 UNITS MONTH)  FROM 1 FOR 10 )||' '||HORA_MINUTO_SEG_FINAL
                    INTO vFechaIntegral, primer_dia_semana_hora, ultimo_dia_semana_hora
                FROM bdinteg:si_fechas
                    WHERE empresa = '001';
            END IF 
            
            IF ( pNumeroDesfase = 2 ) THEN 
             
                SELECT fecha_hoy, 
                        SUBSTRING( ( EXTEND(pri_dia_mes + 15 ) - 1 UNITS MONTH  )  FROM 1 FOR 10 )||' '||HORA_MINUTO_SEG_INICIAL,
                            SUBSTRING( ( EXTEND(pri_dia_mes) - 1 UNITS DAY )  FROM 1 FOR 10 )||' '||HORA_MINUTO_SEG_FINAL                 
                    INTO vFechaIntegral, primer_dia_semana_hora, ultimo_dia_semana_hora
                FROM bdinteg:si_fechas
                    WHERE empresa = '001';
        ...
        IF ( pPeriodicidad = PERIODICIDAD_MENSUAL ) THEN
        
            SELECT fecha_hoy,
                        SUBSTRING( ( EXTEND(pri_dia_mes) - vDesfaseInicial UNITS MONTH)  FROM 1 FOR 10 )||' '||HORA_MINUTO_SEG_INICIAL,
                            SUBSTRING ( ( EXTEND(pri_dia_mes) - vDesfaseFinal UNITS DAY)  FROM 1 FOR 10 )||' '||HORA_MINUTO_SEG_FINAL
                INTO vFechaIntegral, primer_dia_semana_hora, ultimo_dia_semana_hora
            FROM bdinteg:si_fechas
                WHERE empresa = '001';
        ...
        --Calcula el dia inicial (lunes) y final (domingo) de la semana anterior considerando la fecha de hoy (integral)
        -- pNumeroDesfase (1)
        IF ( pPeriodicidad = PERIODICIDAD_SEMANAL ) THEN
        
            LET vDesfaseInicial =  pNumeroDesfase * 7;
            
            SELECT empresa, fecha_hoy, WEEKDAY(fecha_hoy) as num_dia, fecha_hoy as fecha_integral
                FROM bdinteg:si_fechas            
            WHERE empresa = '001'
            ...
            IF ( vNumeroDia = DIA_LUNES ) THEN                
                ...
                LET primer_dia_semana_hora = extend (vFechaHoy - 0 units MONTH) - vDesfaseInicial units day;
                LET primer_dia_semana_hora= SUBSTRING(primer_dia_semana_hora FROM 1 FOR 10)||' '||HORA_MINUTO_SEG_INICIAL;
                
                LET ultimo_dia_semana_hora = extend(vFechaHoy - 0 units MONTH) - vDesfaseFinal units DAY;
                LET ultimo_dia_semana_hora = SUBSTRING(ultimo_dia_semana_hora FROM 1 FOR 10)||' '||HORA_MINUTO_SEG_FINAL;
...
---Creacion: 21 de julio del 2020
---Modificacion: 08 de diciembre del 2020
---Descripcion: Determina un rango entre dos fechas de acuerdo al parametro diario (D), quincenal (Q), semanal (S) o mensual (M) anterior 
---tomando en cuenta la fecha de hoy integral (bdinteg).

--Ejecucion: EXECUTE PROCEDURE "informix".sp_intercard_calcular_periodos('D', 1); --un dia anterior
--Ejecucion: EXECUTE PROCEDURE "informix".sp_intercard_calcular_periodos('Q', 1); --primera quincena del mes.
--Ejecucion: EXECUTE PROCEDURE "informix".sp_intercard_calcular_periodos('Q', 2); --segunda quincena del mes.
--Ejecucion: EXECUTE PROCEDURE "informix".sp_intercard_calcular_periodos('S', 1); --fecha de inicio lunes a fecha final domingo de la semana anterior
--Ejecucion: EXECUTE PROCEDURE "informix".sp_intercard_calcular_periodos('M', 1); --fecha de inicio primer día del mes anterior con fecha final del último día del mes anterior.
--Ejecucion: EXECUTE PROCEDURE "informix".sp_intercard_calcular_periodos('M', 6); --fecha de inicio primer día del mes 6 anterior con fecha final del último día del mes 6 anterior.
;

CREATE PROCEDURE "informix".sp_rpt_mc_corresp_retiros_efectivo ( pNomCorresponsal VARCHAR(7), pPeriodo CHAR(1), pNumeroDesfase SMALLINT )
    ...
    DEFINE SOLO_ARCHIVO_CASHBACK CHAR(1);
    ...
    LET vFechaEjecucion = '';
    LET SOLO_ARCHIVO_CASHBACK = '1';
    
    --SET DEBUG FILE TO RUTA_DESTINO|| 'sp_rpt_mc_corresp_retiros_efectivo.out';
    --TRACE ON;
	...
	BEGIN
        ...
        ON EXCEPTION SET SQLERR, ISAM_ERR, ERROR_INFO
            ...
            SET DEBUG FILE TO RUTA_DESTINO || "excep_sp_rpt_mc_corresp_retiros_efectivo.err.out" WITH APPEND;
            TRACE ON;
        ...
        IF ( vCodigoRetorno <> '00000' ) THEN
            LET vMensajeRetorno = 'Error al obtener el rango de fechas.';
            RETURN vCodigoRetorno, vMensajeRetorno;
        END IF
        ...
        EXECUTE PROCEDURE "informix".sp_rpt_mc_generar_archivos( pNomCorresponsal, vFechaEjecucion, SOLO_ARCHIVO_CASHBACK )
            INTO vCodigoRetorno, vMensajeRetorno;
...
'Descripcion: Componente principal ejecutado por los jobs: 692_01 y 692_02',
'Obtiene la transaccionalidad registrada mediante cashback de los corresponsales OXXO y 7Eleven'
;

CREATE PROCEDURE "informix".sp_rpt_mc_obtener_movs_dep_pag( pNombreCorresponsal VARCHAR(10), pFechaBusqInicial DATETIME YEAR to FRACTION(5),pFechaBusqFinal DATETIME YEAR to FRACTION(5) )
    ...
    DEFINE PREFIJO_ARCHIVO VARCHAR(14);
    DEFINE NOM_ARCH_REG_CORRESP VARCHAR(33);
    DEFINE NOM_ARCH_ERR_CORRESP VARCHAR(33);    
    DEFINE vExecuteSQL LVARCHAR(5000);    
    DEFINE vCondicionesTipoMov VARCHAR(250);    
    DEFINE vIdCorresponsal CHAR(2);    
    ...
    LET NOM_ARCH_REG_CORRESP = '';
    LET NOM_ARCH_ERR_CORRESP = '';
    LET PREFIJO_ARCHIVO = 'rptmc_dep_pag_';
    LET vIdCorresponsal = '';
    
    --SET DEBUG FILE TO RUTA_UNLOAD_RESPALDOS || "debug_sp_rpt_mc_obtener_movs_dep_pag.out";
    --TRACE ON;        
    ...
    BEGIN
        ...
        ON EXCEPTION SET SQLERR, ISAM_ERR, ERROR_INFO
            ...
            SET DEBUG FILE TO RUTA_UNLOAD_RESPALDOS || "excep_sp_rpt_mc_obtener_movs_dep_pag.err.out" WITH APPEND;
            TRACE ON;
        ...
        LET pNombreCorresponsal = UPPER(TRIM(pNombreCorresponsal));
        ...
        LET vCondicionesTipoMov = " AND transaccionorigen = '2345' AND metodocaptura = '01'  ";
        ...
        IF ( pNombreCorresponsal = CORRESP_OXXO ) THEN
            ...
            LET NOM_ARCH_REG_CORRESP = PREFIJO_ARCHIVO||'movs_'||CORRESP_OXXO||'.txt';
            LET NOM_ARCH_ERR_CORRESP = PREFIJO_ARCHIVO||'movs_'||CORRESP_OXXO||'.log';
            LET vIdCorresponsal = '02';            
        ...
        ELIF ( pNombreCorresponsal = CORRESP_SEVEN ) THEN
            ...
            LET NOM_ARCH_REG_CORRESP = PREFIJO_ARCHIVO||'movs_'||CORRESP_SEVEN||'.txt';
            LET NOM_ARCH_ERR_CORRESP = PREFIJO_ARCHIVO||'movs_'||CORRESP_SEVEN||'.log';            
            LET vIdCorresponsal = '03';
        END IF
        ...
        '  AND codigoiso = \"00\" ' ||
        '  AND codtran = \"28\" ' || vCondicionesTipoMov||        
        '  AND formato = \"0200\" ' ||
        '  AND movreversado = \"F\" '||
        '  AND prodind = \"02\" ' ||        
        '  AND codreversa = \"0\" ' ||        
        '  AND movconciliado = \"V\" '||
        '  AND idreceptor = "'||vIdCorresponsal||'"'||
        ...
        LET vExecuteSQL = "echo "||'"'|| "file '"||RUTA_UNLOAD_RESPALDOS||NOMBRE_UNL_ARCHIVO|| "' delimiter '|' "|| '5'||                          
                          "; INSERT INTO tbl_mc_movs_tipo_transaccional" || ";"||'"'||' > '||RUTA_UNLOAD_RESPALDOS||NOM_ARCH_REG_CORRESP;
        SYSTEM vExecuteSQL;        
        
        LET vExecuteSQL = '';
        LET vExecuteSQL = "dbload -d intercard -c "||RUTA_UNLOAD_RESPALDOS||NOM_ARCH_REG_CORRESP||" -l "||RUTA_UNLOAD_RESPALDOS||NOM_ARCH_ERR_CORRESP||" -n "||CONTADOR_TRANSACCIONES||" -k";
        SYSTEM vExecuteSQL;        
...
'Obtiene la transaccionalidad de los depósitos y pagos a tarjeta de crédito para los corresponsales OXXO y 7Eleven'
;
