CREATE PROCEDURE "informix".sp_generaredoctaeje_factelect_transfer(pEmpresa char(3))
RETURNING CHAR(5);
...
---NUEVAS VARIABLES
    ...
    END EXCEPTION;
     ...
     --SET DEBUG FILE TO "/informix/rsv//TRANSFER.TXT";
     ---TRACE ON;
		...
		FOREACH WITH HOLD 
			...
			IF v_Isr_valida > 0 THEN 
			     IF v_Isr_valida <> vRetencionIsr THEN 
			        ...
			        LET  vsdoprom = ROUND((vRetencionIsr / vValor_tasa_isr + .01 + vbase_exenta),2);
			     ELSE 
			        LET vBaseisr = vBaseisr; 
			        LET vsdoprom = vsdoprom; 
			     END IF;    
             END IF;
			...
			IF vRetencionIsr > 0 AND vBaseisr = 0  AND vValor_tasa_isr = 0 THEN  
			    SELECT promedio, dia_promedio, promedio - vbase_exenta, TRUNC((((tasa_isr / 100) * dia_promedio) / vaniobase),6)
				  INTO vsdoprom,vdias,vBaseisr,vValor_tasa_isr  
				  FROM bdicheq:sc_isr 
				 WHERE cuenta = vcuenta
				   AND tasa_isr IN (SELECT MAX(tasa_isr)  FROM bdicheq:sc_isr 
                                     WHERE cuenta = vcuenta); 
            END IF;		
