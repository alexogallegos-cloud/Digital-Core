CREATE PROCEDURE "informix".alta_sol_tc_cjunk_rodo2(o_empresa            CHAR(3),
...
-- Ini Caja Unica. Viridiana
	...
	set debug file to '/resplogifx/alta_sol_tc_cjunk.out_rodo';
	trace on; 
...
--INI MOVIL
	...
	INTO s_numsol
	FROM bdisolic:"informix".ss_solicitudes_movil a inner join bdisolic:ss_solicitudes b
         on a.num_solicitud=b.num_solicitud and b.status_solicitud<>'AP'
	WHERE a.empresa = o_empresa 
    AND a.numcte = o_num_cliente 
    AND a.status <> '3' and a.producto =o_producto;
   ...
   END IF;
	...
	-- Valida Solicitudes en Proceso
       ...
       END IF;
        ...
        -- *****************************************
       CALL asigna_numsol_rodo(o_empresa, o_producto)
    ...
    if length(s_numsol) = 12 and bdinteg:"informix".val_num(s_numsol) then  --- se valida que el numero de solicitud sea de 12 y todos sean numericos.
         EXECUTE PROCEDURE graba_sol_precalificada
                         ...
                         v_tpsol,  o_producto, o_ejecutivo, o_subProducto)
                    INTO scod_ret;
