CREATE PROCEDURE "informix".sp_cancelatarj_web(pempresa CHAR (3),ptarj CHAR(20),ptipo CHAR(1))
RETURNING CHAR(5);
...
CREATE PROCEDURE "informix".sp_consultafechavenc(vEmpresa char(3),vCuenta char(20),vTarjeta Char(16),iTipo INTEGER,iEjecucion INTEGER)
	RETURNING CHAR(6) AS CodRetorno,
			   DATE AS Fecha_Expiracion,
			   DATE As FEcha_Hoy,
			   INTEGER As Dias,
			   INTEGER AS Tipo;
...
DEFINE cProducto CHAR(2);
DEFINE cFechaAsignacion CHAR(10);
...
DEFINE dFechaAsig  datetime year to fraction(3);
...
LET cNumCred = '';
LET cStatusCred = '';
LET dFechaExp = DATE (1);
LET dFechaHoy = DATE (1);
LET dFechaAsignacion = DATE (1);
LET dFechaAsig = '1990-01-01 00:00:00.0';
LET cBinTar = '0';

--SET DEBUG FILE TO '/tmp/sp_consultafechavenc.out';
--TRACE ON;
...
BEGIN
	...
	IF NVL(vEmpresa,'') = '' THEN
		LET cCodRet = '00001';
		RETURN cCodRet,dFechaExp,dFechaHoy,iFecha,iTpo;
	ELSE
			
		IF iTipo = 1 THEN -- DEBITO
			-- Se consulta la fecha de expiracion de la tarjeta			
			IF  NVL(vCuenta,'') = '' THEN
				IF NVL(vTarjeta,'') = '' THEN
					LET cCodRet = '00001';
					RETURN cCodRet,dFechaExp,dFechaHoy,iFecha,iTpo;
				ELSE
					--Se consulta la Fecha Hoy
					SELECT fecha_hoy INTO dFechaHoy FROM bdicheq:"informix".sc_fechas WHERE empresa = vEmpresa;

					SELECT expiracion  INTO dFechaExp FROM bdicheq:"informix".sc_tarjeta WHERE empresa = vEmpresa AND num_tarjeta = vTarjeta;
			...
			ELSE
				...
				SELECT expiracion  INTO dFechaExp FROM bdicheq:"informix".sc_tarjeta WHERE empresa = vEmpresa AND num_tarjeta = vTarjeta AND cuenta = vCuenta; /*AND 
secuencia in (select MAX(secuencia)FROM bdicheq:"informix".sc_tarjeta WHERE empresa = vEmpresa AND cuenta = vCuenta);*/ --ANJ
			END IF;	
					
			LET iFecha = dFechaExp - dFechaHoy;

			IF (iFecha >= -90 AND iFecha < 0) THEN -- Se mandara un 2 cuando haya vencido la tarjeta
				LET iTpo = 2;
			ELIF (iFecha >= 0 AND iFecha <= 90)  THEN -- Se mandara un 1 cuando no haya vencido la tarjeta	
				LET iTpo = 1;
			ELIF iFecha  < -90  THEN  --- Se manda 4 cuando la fecha haya vencido hace mas de 90 dias
				LET iTpo = 4;
			END IF;
			
			IF iEjecucion = 1 THEN
			
				SELECT codstatustarjeta  INTO cCodStatusTarjeta FROM intercard:"informix".tarjeta WHERE numtarjeta = vTarjeta;
		...
		ELIF iTipo = 2 THEN -- CREDITO
			...
			IF iEjecucion = 0 THEN
				...
				IF  NVL(vCuenta,'') = '' THEN
					...
					ELSE
						--Se consulta la Fecha Hoy
						SELECT {+INDEX (sd_fechas idx_sdfechas)}fecha_hoy INTO dFechaHoy FROM bdicred:"informix".sd_fechas WHERE empresa = vEmpresa;
						
						SELECT expiracion  INTO dFechaExp FROM bdicred:"informix".sd_tarjeta WHERE empresa = vEmpresa AND num_tarjeta = vTarjeta;
					END IF;
				ELSE
					--Se consulta la Fecha Hoy
					SELECT {+INDEX (sd_fechas idx_sdfechas)}fecha_hoy INTO dFechaHoy FROM bdicred:"informix".sd_fechas WHERE empresa = vEmpresa;

					SELECT expiracion  INTO dFechaExp FROM bdicred:"informix".sd_tarjeta WHERE empresa = vEmpresa AND num_credito = vCuenta;
				END IF;
				...
				ELIF (iFecha > 0 AND iFecha <= 90)  THEN -- Se mandara un 1 cuando no haya vencido la tarjeta	
					LET iTpo = 1;
				ELIF iFecha = 0 THEN -- Se mandara un 3 al ticket
					LET iTpo = 3;
				ELIF iFecha  < -90  THEN  --- Se manda 4 cuando la fecha haya vencido hace mas de 90 dias
					LET iTpo = 4;
				END IF;
				
			ELIF iEjecucion = 2 THEN
				...
				IF cCodStatusTarjeta = "BLO" THEN 
					LET cCodRet = '00388';
					RETURN cCodRet,dFechaExp,dFechaHoy,iFecha,iTpo;
				ELIF cCodStatusTarjeta = "BLT" THEN
					LET cCodRet = '00389';
					RETURN cCodRet,dFechaExp,dFechaHoy,iFecha,iTpo;
				END IF;
				
		--DSB PAY INICIO
		LET cBinTar = SUBSTR(vTarjeta,1,6);

		IF TRIM(cBinTar) <> '514014' THEN
		--DSB PAY FIN

				
				SELECT num_credito INTO cNumCred FROM bdicred:"informix".sd_tarjeta WHERE num_tarjeta = vTarjeta;
				...
				IF cStatusCred IN ("BT","E1","E2","E3") AND cMtoVen > 0 THEN 
					LET cCodRet = '00390';
					RETURN cCodRet,dFechaExp,dFechaHoy,iFecha,iTpo;
			...
			END IF;				
				...
				SELECT fechaexp INTO cFechaExp FROM intercard:"informix".tarjeta WHERE numtarjeta = vTarjeta;
				
				LET cMes = SUBSTR(cFechaExp,3,5);
				LET cAnio = SUBSTR(cFechaExp,0,2);
				LET cAnio = "20"||cAnio;

				IF cMes = 01 OR cMes = 03 OR cMes = 05 OR cMes = 07 OR cMes = 08 OR cMes = 10 OR cMes = 12 THEN
					LET cDia = 31;
				ELIF cMes = 04 OR cMes = 06 OR cMes = 09 OR cMes = 11 THEN	
					LET cDia = 30;
				ELSE
				...
				LET cFechaHoy = cAnio||"-"||cMes||"-"||cDia;
				LET dFechaExp = TO_DATE(cFechaHoy,"%Y-%m-%d");
				LET iFecha = dFechaExp - dFechaHoy;
				
				LET iFecha = dFechaExp - dFechaHoy;
				
				IF iFecha < 0 THEN -- Se mandara un 2 cuando haya vencido la tarjeta
					LET iTpo = 2;
				ELIF (iFecha >= 0 AND iFecha <= 90)  THEN -- Se mandara un 1 cuando no haya vencido la tarjeta	
					LET iTpo = 1;
				END IF;
		...
		ELIF iTipo = 3 THEN
			...
			ELSE
				...
				SELECT fechaasignacion  INTO dFechaAsig FROM intercard:"informix".tarjeta WHERE numtarjeta = vTarjeta;
			END IF;
			
			LET cFechaAsignacion = SUBSTR(dFechaAsig,0,10);
			LEt dFechaExp = TO_DATE(cFechaAsignacion,"%Y-%m-%d");
			
			
			LET iFecha = dFechaExp - dFechaHoy;
			
			IF (iFecha >= 0 AND iFecha <= 30)  THEN -- Se mandara un 1 si esta entre los primeros dias de la renovacion
				LET iTpo = 1;
			ELIF iFecha > 31 THEN -- Se mandara un 2 si esta despues de los 30 dias de la renovacion
				LET iTpo = 2;
			END IF;
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se crea procedimiento para la consulta de la fecha de vencimiento de la tarjeta tanto de crédito como de captación,',
'y que calcule los dias que faltan para expirar o en su defecto los dias que lleva de expirada la misma.',
'AUTOR :  Martiín Miranda',
'FECHA : 24/11/2011',
'BD: bdicred';

CREATE PROCEDURE "informix".sp_validaasignatarjetaincompleta(pNumSolicitud VARCHAR(20), pEmpresa VARCHAR(3))
...
--SET DEBUG FILE TO  '/tmp/Guicho/sp_validaAsignaTarjetaIncompleta.out';
--TRACE ON;

    SET ISOLATION TO DIRTY READ;	
	SET LOCK MODE TO WAIT 3;
	
IF pNumSolicitud = "650000000000" THEN --TDC PAY
	LET cCodRet = '00000';ELSE--TDC PAY
	
	--VALIDAR PARÁMETROS VACÍOS O NULOS
	IF NVL(TRIM(pNumSolicitud),'') = '' OR NVL(TRIM(pEmpresa), '') = '' THEN
		LET cCodRet = '00001';
		RETURN cCodRet;
	END IF;
	
	select count(num_tarjeta) into iCont 
	FROM bdicred:"informix".sd_tarjeta 
	WHERE num_credito = pNumSolicitud  AND empresa = pEmpresa;
...
'Fecha............: 19/06/2018 ',
'Modificacion.........: Se crea sp para validar si existe tarjeta en la tabla bdicred:"informix".sd_tarjeta,',
'                       la asignacion de tarjetas incompletas',
