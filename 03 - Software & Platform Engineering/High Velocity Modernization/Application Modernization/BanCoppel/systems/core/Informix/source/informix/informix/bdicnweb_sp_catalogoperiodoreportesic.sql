CREATE PROCEDURE "informix".sp_catalogoperiodoreportesic(pUsuario CHAR(8), pIdFuncion CHAR(10), pNumProducto CHAR(6), pTipoReporte CHAR(1))
		RETURNING CHAR(5) AS codret, 
				  DATE AS pFecha,
				  CHAR(20) AS pDescripcionFecha;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN 	cCodRet, dFecha, cDescipFecha;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_catalogoperiodoreportesic.out';
		--TRACE ON;
		
		IF pUsuario = '' OR pIdFuncion = '' OR  pNumProducto= '' OR pTipoReporte = '' THEN
			LET cCodRet = '00003';
			RETURN 	cCodRet, dFecha, cDescipFecha;
		END IF;
		
		IF pTipoReporte NOT IN('C','D') THEN
			LET cCodRet = '00297';
			RETURN 	cCodRet, dFecha, cDescipFecha;
		END IF;
		...
		--C = RESPORTE CONSOLIDADO
		--D = REPORTE DETALLE DE DIFERENCIAS
		IF pTipoReporte = 'C' THEN
				FOREACH				
					SELECT fecha_proceso INTO dFecha FROM bdiburo:br_fechas_concil 
					WHERE num_producto = pNumProducto
					
					LET iMes = MONTH(dFecha);
					LET cDescipFecha = DECODE(iMes, 1,'ENERO',2,'FEBRERO',3, 'MARZO',4,'ABRIL',5,'MAYO',6,'JUNIO',7,'JULIO',8,'AGOSTO',9,'SEPTIEMBRE',10,'OCTUBRE',11,'NOVIEMBRE',12,'DICIEMBRE')
										||" "||YEAR(dFecha);
										
					RETURN 	cCodRet, dFecha, cDescipFecha WITH RESUME;				
				END FOREACH;
		...
		ELIF pTipoReporte = 'D' THEN
			FOREACH				
					SELECT fecha_proceso INTO dFecha FROM bdiburo:br_fechas_concil 
					WHERE num_producto = pNumProducto AND diferencia = 'D'
...
'FECHA: 02/06/2014',
'DESCRIPCION: Devuelve la fecha y descripcion del mes respecto al numero de producto',
'BD: bdicnweb';
