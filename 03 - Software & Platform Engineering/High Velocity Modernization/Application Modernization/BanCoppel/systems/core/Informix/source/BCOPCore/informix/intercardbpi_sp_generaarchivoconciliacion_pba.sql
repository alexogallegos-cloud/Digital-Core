CREATE PROCEDURE "informix".sp_generaarchivoconciliacion_pba(psNumEmpleado CHAR (8))
RETURNING
	CHAR(5); ---cod_ret
	...
	DEFINE v_MontoComision		MONEY(16,2);
	DEFINE v_MontoIva			MONEY(16,2);
	DEFINE v_TotReg				INTEGER;
	DEFINE v_ComisionTCD		DECIMAL(16,2);
	DEFINE v_ComisionTCC		DECIMAL(16,2);
	DEFINE v_IVATCD				DECIMAL(16,2);
	DEFINE v_IVATCC				DECIMAL(16,2);
...
BEGIN
	...
	LET vsSQL4 = '' ;
	LET vsSQL4 =  "/usr/bin/sed 's/?$//g' "||TRIM(vsRepositorio)||"/tmp_concitarj.txt >"||TRIM(vsRepositorio)||"/"||TRIM(vsArchivo);
	SYSTEM vsSQL4;
