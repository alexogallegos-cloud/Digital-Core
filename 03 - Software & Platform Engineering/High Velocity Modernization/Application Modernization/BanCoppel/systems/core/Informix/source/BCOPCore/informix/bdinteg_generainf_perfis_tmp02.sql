CREATE PROCEDURE "informix".generainf_perfis_tmp02( pFechaIni DATE, pFechaFin DATE ) 
RETURNING CHAR(5), CHAR(5), INTEGER;
    ...
    --- SET DEBUG FILE TO '/resplogifx/conciliachq/generainf_perfis_tmp02.out';
    --- TRACE ON;
    
    BEGIN

    ON EXCEPTION SET vsql_err, visam_err, vdesc_err
        SET DEBUG FILE TO '/resplogifx/conciliachq/generainf_perfis_tmp02.err';
        TRACE ON;
    ...
    SELECT numcte, id_pregunta, id_secuencia, id_act, id_subact
     ...
     WHERE numcte >= '003078925'
       AND numcte < '005881966'
       AND id_pregunta = 6
       AND id_secuencia > 0
    ...
    SELECT mae.num_cte AS numcte, mae.cuenta, mae.producto, mae.sucursal, mae.status_cta
     ...
     WHERE mae.empresa = noc.empresa
       ...
       AND mae.num_cte >= '003078925'
       AND mae.num_cte <  '005881966'
    ...
    SELECT num_cte AS numcte, cuenta, cod_instrum, sucursal, status_cta
     ...
     WHERE fecha_alta <= pFechaFin
       ...
       AND num_cte >= '003078925'
       AND num_cte <  '005881966'
    ...
    SELECT mae.numcte, mae.num_credito, mae.num_producto, mae.sucursal, mae.status_cred
     ...
     WHERE dos.num_credito = mae.num_credito
       ...
       AND mae.numcte >= '003078925'
       AND mae.numcte <  '005881966'
    ...
    FOREACH WITH HOLD
        ...
        IF vcontador2 >= 10000 THEN
            LET vcontador2 = 0;
            LET vstmt = '';
            LET vstmt = 'echo "CLIENTES PROCESADOS EN generainf_perfis_tmp02: '||vcontador||'" >> /resplogifx/conciliachq/generainf_perfis_tmp02.log';
            SYSTEM vstmt;
        END IF;
