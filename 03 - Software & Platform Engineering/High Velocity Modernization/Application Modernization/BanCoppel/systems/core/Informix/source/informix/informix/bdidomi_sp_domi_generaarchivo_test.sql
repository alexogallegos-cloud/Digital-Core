CREATE PROCEDURE "informix".sp_domi_generaarchivo_test(psNombreArchivo CHAR(20),psFechaPres CHAR(8),psId CHAR(2))
...
ON EXCEPTION IN(-668) SET viSqlErr	
	SET DEBUG FILE TO '/RESPALDOSNEW/trace/Tracesp_Domi_GeneraArchivo_testException.out';
    TRACE ON;
...
SET DEBUG FILE TO '/RESPALDOSNEW/trace/Tracesp_Domi_GeneraArchivo_test'  || SUBSTR(TRIM(psNombreArchivo) ,14,17) || '.out';
TRACE ON;	
...
IF EXISTS(SELECT nombre_arch FROM bdidomi:dom_cce_encabezado_paso WHERE nombre_arch = psNombreArchivo)THEN
        IF EXISTS(SELECT cod_param FROM bdidomi:dom_parametros WHERE cod_param = psId)THEN
			...
			LET vsArchTemp = cFechaArchivoOUT|| SUBSTR(TRIM(psNombreArchivo) ,14,17) ||'tmp1.txt';
            LET vsArchTemp1 = cFechaArchivoOUT|| SUBSTR(TRIM(psNombreArchivo) ,14,17) ||'tmp2.txt';
			...
			SYSTEM vsSQL1;
                ...
                IF ( vsSQL <> '' ) THEN
					...
					--LET vsSQL = '/ifxsif01/bin/dbaccess bdidomi ' || TRIM(vsRepositorio) || cFechaArchivoOUT||'.sql >> '||TRIM(vsRepositorio)||TRIM(cFechaArchivoOUT)||'.out 2>> ' || TRIM(vsRepositorio) || 'system_test.log';
					LET vsSQL = vsSQL;
					LET vsSQL = '/ifxsif01/bin/dbaccess bdidomi ' || TRIM(vsRepositorio) || cFechaArchivoOUT||'.sql >> '||TRIM(vsRepositorio)|| '/archivo' || SUBSTR(TRIM(psNombreArchivo) ,14,17) || '.out 2>&1';
					SYSTEM vsSQL;
