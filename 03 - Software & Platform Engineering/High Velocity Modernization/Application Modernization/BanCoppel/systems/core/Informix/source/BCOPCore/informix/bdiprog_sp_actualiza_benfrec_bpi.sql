CREATE PROCEDURE "informix".sp_actualiza_benfrec_bpi(pNumCte char(20), pCuenta char (20), pCveBanco char(3), pClaveCuenta char (2),
    pAlias char(20),pDomicilio char(80),pNumCel char(10), pCveCompaniaCel char(2),pEmail char(40), pMontoMax MONEY, pCveCaducidad char(1))
	...
	--SET DEBUG FILE TO "/home/informix/bibiana/sp_actualiza_benfrec_bpi.out";
	--TRACE ON;
	...
	BEGIN
		...
		IF EXISTS(SELECT cve_estado FROM bdiprog:"informix".pp_beneficiariosfrec_bpi WHERE num_cte = pNumCte AND cuenta = pCuenta AND
		...
		--Se toma el valor del estado para comparar y poder hacer la actualizacion
				...
				SELECT cve_estado INTO cCveEstado FROM bdiprog:"informix".pp_beneficiariosfrec_bpi WHERE num_cte = pNumCte AND cuenta = pCuenta AND
					...
					IF cCveEstado = '01' THEN
						IF pCveCaducidad IS NOT NULL THEN
							...
							IF (pCveCaducidad = '1') THEN -- Caducidad 48 horas (2 días)
								...
								UPDATE bdiprog:"informix".pp_beneficiariosfrec_bpi SET alias=pAlias, domicilio_ben=pDomicilio,no_celular=pNumcel,
								cve_compania=pCveCompaniaCel,email=pEmail,monto_maximo=pMontoMax,cve_caducidad=pCveCaducidad,fecha_caducidad=dFechaCaducidad,fecha_movtos= today
							...
							ELIF (pCveCaducidad = '2') THEN -- Caducidad 6 meses
								LET dFechaCaducidad = TODAY + 6 UNITS MONTH;
							...
							ELIF (pCveCaducidad = '4') THEN -- Caducidad Programada
								--LET dFechaCaducidad = TODAY + 1 UNITS YEAR;
								UPDATE bdiprog:"informix".pp_beneficiariosfrec_bpi SET alias=pAlias, domicilio_ben=pDomicilio,no_celular=pNumcel,
								cve_compania=pCveCompaniaCel,email=pEmail,monto_maximo=pMontoMax
								WHERE num_cte = pNumCte and cuenta = pCuenta and cve_banco = pCveBanco AND cve_cuenta = pClaveCuenta;

							ELSE
								LET cCodRet = '00007';
								LET cMensajeRet = 'Caducidad inválida';
								RETURN cCodRet, cMensajeRet;
							END IF;
						ELSE
							LET cCodRet = '00008';
							LET cMensajeRet = 'Clave de caducidad nula';
							RETURN cCodRet, cMensajeRet;
						END IF;

						SELECT cod_ret,desc_mensaje INTO cCodRet, cMensajeRet FROM bdiprog:"informix".pp_mensajes where cve_mensaje = '00';
					ELSE
						SELECT cod_ret,desc_mensaje INTO cCodRet, cMensajeRet FROM bdiprog:"informix".pp_mensajes where cve_mensaje = '10';
					END IF;
		ELSE
			SELECT cod_ret,desc_mensaje INTO cCodRet, cMensajeRet FROM bdiprog:"informix".pp_mensajes where cve_mensaje = '09';
		END IF ;
...
END PROCEDURE
Document
'DESCRIPCION:Actualiza beneficiarios frecuentes', 
'AUTOR: Jose Ruben Lopez',
'FECHA: 15 de enero de 2015',
'VERSION: 20141216.0900',
'BD: bdiprog';

CREATE PROCEDURE "informix".sp_consulta_benfrec_bpi( pNum_cliente CHAR(20),pClaveCuenta CHAR(2),pClaveBanco CHAR(3),pRegistros SMALLINT)
    RETURNING CHAR(5), CHAR(20), CHAR(3), CHAR(2), CHAR(20), CHAR(26), CHAR(26), CHAR(26), CHAR(26),CHAR(80),CHAR(40),CHAR(10),CHAR(2),CHAR(2),DATE,CHAR(2),MONEY(16,2),CHAR(1),DATE,DATE;
...
--**************************************************************************************
	...
	DEFINE cCveCuenta 		CHAR(2);
	DEFINE cAlias	  		CHAR(20);
	DEFINE cPrimerNombre	CHAR(26);
	DEFINE cSegundoNombre	CHAR(26);
	DEFINE cApellidoPat		CHAR(26);
	DEFINE cApellidoMat		CHAR(26);
	DEFINE cDomicilioBen	CHAR(80);
	DEFINE cEmail			CHAR(40);
	DEFINE cNoCelular		CHAR(10);
	DEFINE cCveCompania		CHAR(2);
	DEFINE cCveEstado		CHAR(2);
	DEFINE cCanalAlta		CHAR(2);
	DEFINE cCanalBaja		CHAR(2);
	DEFINE dFechaEstado		DATE;
	DEFINE cDigitoVer		CHAR(2);
	DEFINE mMontoMax		MONEY(16,2);
	DEFINE cCveCad			CHAR(1);
	DEFINE cFechaCad		DATE;
	DEFINE dFechaInsert     DATE;
	DEFINE iContadorRegistros 	INTEGER;
	DEFINE dFechaHoraInsert	DATETIME YEAR TO FRACTION;
	...
	--SET DEBUG FILE TO "/home/sysifx/sp_consulta_benfrec_bpi.out";
    --TRACE ON;
	...
	BEGIN
	   ...
	   ON EXCEPTION SET iSql_err
		  IF iSql_err <> 0 THEN
				...
				RETURN cCodRet,cNumCuenta,cCveBanco,cCveCuenta,cAlias,cPrimerNombre,cSegundoNombre,cApellidoPat,cApellidoMat,cDomicilioBen,cEmail,cNoCelular,cCveCompania,cCveEstado,dFechaEstado,cDigitoVer,mMontoMax,cCveCad,cFechaCad,dFechaInsert;
		  END IF ;
	   END EXCEPTION ;
	   
	   IF pNum_cliente<> ''THEN
			FOREACH
				SELECT cuenta,cve_banco,cve_cuenta,alias,primer_nombre,segundo_nombre,apellido_paterno,apellido_materno,domicilio_ben,email,no_celular,cve_compania,cve_estado,fecha_estado,digito_ver,monto_maximo,cve_caducidad,fecha_caducidad,fecha_insert,hora_insert 
				INTO cNumCuenta,cCveBanco,cCveCuenta,cAlias,cPrimerNombre,cSegundoNombre,cApellidoPat,cApellidoMat,cDomicilioBen,cEmail,cNoCelular,cCveCompania,cCveEstado,dFechaEstado,cDigitoVer,mMontoMax,cCveCad,cFechaCad,dFechaInsert,dHoraInsert
				FROM bdiprog:"informix".pp_beneficiariosfrec_bpi
				WHERE num_cte=pNum_cliente AND cve_banco=pClaveBanco AND cve_cuenta=pClaveCuenta AND cve_estado='01'
				ORDER BY alias
				
				LET dFechaHoraInsert = ( YEAR(dFechaInsert) || '-' || MONTH(dFechaInsert) || '-' || DAY(dFechaInsert) || ' ' || dHoraInsert)::DATETIME YEAR TO FRACTION;
				IF (current - dFechaHoraInsert) < '0 00:30:00' THEN
					CONTINUE FOREACH;
				END IF;
				
				LET iContadorRegistros=iContadorRegistros+1;
				IF iContadorRegistros <= pRegistros THEN -- Si el registro no es mayor al numero de registro recibido, no regresa nada y continua con el siguiente registro
								CONTINUE FOREACH;
				ELSE
					RETURN cCodRet,cNumCuenta,cCveBanco,cCveCuenta,cAlias,cPrimerNombre,cSegundoNombre,cApellidoPat,cApellidoMat,cDomicilioBen,cEmail,cNoCelular,cCveCompania,cCveEstado,dFechaEstado,cDigitoVer,mMontoMax,cCveCad,cFechaCad,dFechaInsert WITH RESUME;
				END IF;
			END FOREACH;
		ELSE
		...
		IF iContadorRegistros =0 THEN --SIN REGISTROS
			LET cCodRet='00002';
			RETURN cCodRet,cNumCuenta,cCveBanco,cCveCuenta,cAlias,cPrimerNombre,cSegundoNombre,cApellidoPat,cApellidoMat,cDomicilioBen,cEmail,cNoCelular,cCveCompania,cCveEstado,dFechaEstado,cDigitoVer,mMontoMax,cCveCad,cFechaCad,dFechaInsert;
		END IF;
...
END PROCEDURE
Document
'DESCRIPCION:obtienen los beneficiarios frecuentes habilitadas para las ordenes de pago', 
'AUTOR: Jose Ruben Lopez',
'FECHA: 15 de enero de 2015',
'VERSION: 20141216.0900',
'BD: bdiprog';

CREATE PROCEDURE "informix".sp_obtbeneficiariosfctesinhabiles( pNum_cliente CHAR(20),pClaveCuenta CHAR(2),pClaveBanco CHAR(3),pRegistros SMALLINT)
    RETURNING CHAR(5), CHAR(20), CHAR(3), CHAR(2), CHAR(20), CHAR(26), CHAR(26), CHAR(26), CHAR(26),CHAR(80),CHAR(40),CHAR(10),CHAR(2),CHAR(2),DATE,CHAR(2),MONEY(16,2),CHAR(1),DATE,DATE;
...
--**************************************************************************************
	...
	--SET DEBUG FILE TO "/home/informix/bibiana/sp_obtbeneficiariosfctesinhabiles.out";
    --TRACE ON;
	...
	BEGIN
	   ...
	   ON EXCEPTION SET iSql_err
		  IF iSql_err <> 0 THEN
				let cCodRet = iSql_err;
				RETURN cCodRet,cNumCuenta,cCveBanco,cCveCuenta,cAlias,cPrimerNombre,cSegundoNombre,cApellidoPat,cApellidoMat,cDomicilioBen,cEmail,cNoCelular,cCveCompania,cCveEstado,dFechaEstado,cDigitoVer,mMontoMax,cCveCad,dFechaCad,dFechaInsert;
		  END IF ;
	   END EXCEPTION ;
	   ...
	   IF pNum_cliente<> ''THEN
			FOREACH
				SELECT cuenta,cve_banco,cve_cuenta,alias,primer_nombre,segundo_nombre,apellido_paterno,apellido_materno,domicilio_ben,email,no_celular,cve_compania,cve_estado,fecha_estado,digito_ver,monto_maximo,cve_caducidad,fecha_caducidad,fecha_insert,hora_insert 
				INTO cNumCuenta,cCveBanco,cCveCuenta,cAlias,cPrimerNombre,cSegundoNombre,cApellidoPat,cApellidoMat,cDomicilioBen,cEmail,cNoCelular,cCveCompania,cCveEstado,dFechaEstado,cDigitoVer,mMontoMax,cCveCad,dFechaCad,dFechaInsert,dHoraInsert
				FROM bdiprog:"informix".pp_beneficiariosfrec_bpi
				WHERE num_cte=pNum_cliente AND cve_banco=pClaveBanco AND cve_cuenta=pClaveCuenta
				...
				IF (current - dFechaHoraInsert) < '0 00:30:00' THEN
							...
							IF iContadorRegistros <= pRegistros THEN -- Si el registro no es mayor al numero de registro recibido, no regresa nada y continua con el siguiente registro
								CONTINUE FOREACH;
							ELSE
								RETURN cCodRet,cNumCuenta,cCveBanco,cCveCuenta,cAlias,cPrimerNombre,cSegundoNombre,cApellidoPat,cApellidoMat,cDomicilioBen,cEmail,cNoCelular,cCveCompania,cCveEstado,dFechaEstado,cDigitoVer,mMontoMax,cCveCad,dFechaCad,dFechaInsert WITH RESUME;			
							END IF;			
				END IF;			
...
END PROCEDURE
Document
'DESCRIPCION:obtienen los beneficiarios frecuentes inhabiles para las ordenes de pago', 
'AUTOR: Jose Ruben Lopez',
'FECHA: 15 de enero de 2015',
'VERSION: 20141216.0900',
'BD: bdiprog';
