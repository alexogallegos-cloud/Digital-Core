...
Returning char(5);
	...
	LET v_origen_evento		= '';
	LET v_evento 			= '';
	LET v_estatus_general   = '';
	LET v_estatus_analisis  = '';
	LET v_numero_sucursal   = '';
	LET v_nombre_sucursal	= '';
	LET v_num_empleado		= '';
	LET v_modo_entrada	    = '';
	LET v_autorizacion		= '';
	LET v_estatus_cuenta      = '';
    LET v_producto_credito    = '';
    LET v_producto_debito     = '';
	LET v_tipo_producto		  = '';
	LET v_nombre_origen		  = '';
	LET v_num_cliente		  = '';
	LET p_folio_csuac		  = '';
	...
	SELECT tabid
			INTO v_temp_table
	FROM systables WHERE tabname ='acl_reporte_atm';
	
	IF v_temp_table IS NOT NULL THEN
		DROP TABLE "informix".acl_reporte_atm;
	END IF;
	...
	--SET DEBUG FILE TO "/resplogifx/RD_2/RQM_06_754/sp_reporte_2.out";
	...
	BEGIN	
		...
		CREATE  TABLE  "informix".acl_reporte_atm(
			folio_csuac	  		char(11),
			numero_tarjeta		varchar(20),
			importereclamado	numeric,
			fecha_mov 			date,
			hora_mov 			char(20),
			ref_comercio 		varchar(200),
			secuencia 			varchar(50),
			autorizacion		char(20),
			cuenta_destino 		char(20),
			nombre	 			varchar(100),
			telefono			char(20),
			num_cliente			char(20),
			...
			producto 			varchar(50),
			origen				varchar(20),
			origen_evento		varchar(100),
			evento 				varchar(100),
			estatus_general 	varchar(100),
			estatus_analisis	varchar(100),
	...
	into p_fechahoy, p_fechafinalmes
	...
	BEGIN WORK;
		...
		FOREACH WITH HOLD
			...
			SELECT {+AVOID_FULL(acl_folios_atm)} folio_csuac 
				into p_folio_csuac
			from acl_folios_atm where descargado = 0
	
		---
			IF p_folio_csuac IS NOT NULL THEN
            		SELECT FIRST 1
						a.folio_csuac,
						p.numero_tarjeta,
						a.importereclamado,
						date(mov.fechahora),	
						TO_CHAR(mov.fechahora, '%H:%M:%S'),
						mov.ref_comercio,
						'' as secuencia,
						mov.referencia as autorizacion,
						p.numero_cuenta as cuenta_destino,
						c.nombre1,
						c.nombre2,
						c.apell_paterno,
						c.apell_materno,
						...
						DECODE(a.tipo_movimiento,'V','Nacional','F','Internacional','',null,null,null) as origen,
						oe.descripcion as Origen_evento,
						e.descripcion as evento,
						cor.descripcion as estatus_general,
						cor2.descripcion as estatus_analisis,
						a.num_sucursal as numero_sucursal,
						suc.nombre as nombre_sucursal,
						a.num_empleado as num_empleado,
						a.modo_entrada,
						tp.tipo_producto
					INTO
						v_folio_csuac, v_numero_tarjeta, v_importereclamado, v_fecha_mov, v_hora_mov, v_ref_comercio,		
						v_secuencia, v_autorizacion, v_cuenta_destino, v_nombre1, v_nombre2, v_apell_paterno, v_apell_materno, v_telefono,			
						v_num_cliente, v_numero_cuenta, v_producto, v_origen, v_origen_evento, v_evento, v_estatus_general, v_estatus_analisis,
						v_numero_sucursal, v_nombre_sucursal, v_num_empleado, v_modo_entrada, v_tipo_producto		
								
					from acl_aclaracion a
						inner join acl_producto p on a.fky_producto=p.pky_producto
						inner join acl_movimiento mov on a.pky_aclaracion = mov.fky_aclaracion and mov.duplicado = 0
						...
						left outer join bdinteg:si_telefonos_actual tl on c.numcte=tl.numcte and tl.tipo_tel = '2'
						inner join acl_tipo_producto tp on p.fky_tipo_producto = tp.pky_tipo_producto
						inner join acl_origen_evento oe on e.fky_origen_evento = oe.pky_origen_evento
						inner join acl_estatus_corporativo cor on a.fky_estatus_corp_general = cor.pky_estatus_corporativo
						left outer join acl_estatus_corporativo cor2 on a.fky_estatus_corp_analisis = cor2.pky_estatus_corporativo
						left outer join bdinteg:si_sucursales suc on a.num_sucursal = suc.sucursal
					where a.folio_csuac = p_folio_csuac;
			--					
			--				
					LET v_nombre1 = trim(v_nombre1)||' '||trim(v_nombre2)||' '||trim(v_apell_paterno)||' '||trim(v_apell_materno);
					LET v_autorizacion = substr(v_autorizacion,11);
					LET v_nombre_sucursal = trim(v_nombre_sucursal)||'-'||trim(v_numero_sucursal);
					
					---Se obtiene el estatus de la cuenta si es credito
					IF(v_tipo_producto = '1' ) THEN 
			
						LET v_estatus_cuenta = (SELECT tc.descripcion FROM bdicred:sd_maecred mcrd
						RIGHT JOIN bdicred:sd_tipocartera tc ON tc.status_cred = mcrd.status_cred
						WHERE mcrd.numcte = v_num_cliente
						AND num_credito = v_numero_cuenta);
			
					END IF;
					----Se obtiene el estatus de la cuentas que correspondan a debito
					IF (v_tipo_producto = '2') THEN 
			
						LET v_estatus_cuenta = (SELECT mst.descripcion FROM bdicheq:sc_maechq mchq
						RIGHT JOIN bdicheq:sc_mae_estatus mst ON mst.cod_estatus=mchq.status_cta 
						WHERE mchq.num_cte = v_num_cliente
						AND mchq.cuenta = v_numero_cuenta);
			
					END IF;
					
					------Se inserta la informaciÃ³n del folio en la tabla de control para su posterior descarga
					INSERT INTO "informix".acl_reporte_atm(
						folio_csuac,
						numero_tarjeta,
						importereclamado,
						fecha_mov, 
			...
			UPDATE {+AVOID_FULL(acl_folios_atm)} acl_folios_atm SET descargado = 1 WHERE folio_csuac = p_folio_csuac;
	...
	COMMIT WORK;
			...
			--Generacion de Reporte Mensual descarga de informaciÃ³n
		let vsql = ' echo "FOLIO_CSUAC|NO_DE_PLÃSTICO|MONTO_RECLAMADO|FECHA|HORA_OPERACION|NUMERO_CAJERO|SECUENCIA|NUM_DE_AUTORIZACION|CUENTA_DESTINO|NOMBRE_DEL_CLIENTE|TELEFONO|NUMERO_DE_CLIENTE|NUMERO_DE_CUENTA|STATUS|PRODUCTO|ORIGEN|ORIGEN_EVENTO|EVENTO|ESTATUS_GENERAL|ESTATUS_ANALISIS|SUCURSAL|NUMERO_DE_EMPLEADO|CANAL_DE_RECEPCION">/resplogifx/repaclaraciones/ACL_ATM_'||LPAD (day(p_fechahoy),2,"0")||LPAD (MONTH(p_fechahoy),2,"0")||year(p_fechahoy)||'_'||TO_CHAR(current, '%H')||'_'||TO_CHAR(current, '%M')||'.txt';
		system vsql; 
		let vsql = '';
		let vsql=  'echo "UNLOAD TO reporte_atm.txt  select folio_csuac, numero_tarjeta, importereclamado, fecha_mov, hora_mov, ref_comercio, secuencia, autorizacion, cuenta_destino, nombre, telefono, num_cliente, numero_cuenta, estatus_cuenta, producto, origen, origen_evento, evento, estatus_general, estatus_analisis, sucursal, num_empleado, modo_entrada from acl_reporte_atm">reporte_atm.sql'; 
		system vsql;
		let vsql = '';
		let vsql= 'dbaccess bdiaclaracion reporte_atm.sql';
		system vsql;
		let vsql ='';
		let vsql ='rm  reporte_atm.sql';
		system vsql;
		let vsql ='';
		let vsql = "sed 's/|$//g' reporte_atm.txt >>/resplogifx/repaclaraciones/ACL_ATM_"||LPAD (day(p_fechahoy),2,"0")||LPAD (MONTH(p_fechahoy),2,"0")||year(p_fechahoy)||'_'||TO_CHAR(current, '%H')||'_'||TO_CHAR(current, '%M')||".txt";
		system vsql;
		let vsql ='rm  reporte_atm.txt';
		system vsql; 
		--	
		--let vcodret = '00000';					
		

		--Se limina la tabla de control que se creo
		 drop table "informix".acl_reporte_atm;
-----------------------------------------------------------------------------------
		---Se eliminan los registros que ya fueron descargados
		IF p_fechahoy = p_fechafinalmes THEN
			
			DELETE FROM acl_folios_atm WHERE descargado = 1;
...
DOCUMENT
'Sp para generaciÃ³n de Reporte solicitado por ATM',
'Genera la extraciÃ³n de informaciÃ³n correspondiente a las aclaraciones ingresadas durante el dia que correspondan a ATM',
'Es llamado desde desde la opcion 801 del menu de produccion',
'Se agrego un fist 1, debido a incidencia realcionada con extraer mas de un folio. - 05/05/2025',
'Aclaraciones',
