CREATE PROCEDURE "informix".sp_transfer_obtenernombresarchivos  (
...
--RETORNOS--
RETURNING 
VARCHAR(3) AS ArchivoOrigen, 
VARCHAR(45) AS NombreArchivo, 
DATE AS FechaArchivo, 
SMALLINT AS Dias_Desfase,
 VARCHAR(50) AS Ruta_Repositorio_AIX,
 VARCHAR(50) AS Ruta_Repositorio_WIN, 
 SMALLINT AS Tipo_LayOut,
 VARCHAR(2) AS B_Seguridad;

--DECLARACIONES--
DEFINE vsArchivo_Origen VARCHAR(3);
DEFINE vsNombreArchivo VARCHAR(45);
...
DEFINE vsb_Seguridad VARCHAR(2);
...
BEGIN
	...
	--set debug file to "/informix/HomeInformix/rrm/obtenernombre.out";
	--trace on;
	
	--if psArchivoOrigen in ('EE0','EE1', 'EE2', 'EE3', 'EE4', 'EE5')
	...
	FOREACH
		SELECT (CASE
					WHEN Archivo_Origen in ('EE0','EE1', 'EE2', 'EE3', 'EE4', 'EE5') then 'EEC'
					WHEN Archivo_Origen in ('DE0','DE1', 'DE2', 'DE3', 'DE4', 'DE5') then 'DEC'
					WHEN Archivo_Origen in ('RE0','RE1', 'RE2', 'RE3', 'RE4', 'RE5') then 'REC'
				else
						Archivo_Origen
				end ) as ArchivoOrigen,
		(TRIM(Prefijo_Archivo)
			--ddmmyy
			|| (CASE WHEN LOWER(TRIM(formato_fecha)) = "ddmmyy" THEN (LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0')||LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0')||(SUBSTR(YEAR(pdtFecha::DATE - Dias_Desfase),3,2))::VARCHAR(4))			
			--ddmmyyyy
			 WHEN LOWER(formato_fecha) = "ddmmyyyy" THEN (LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0')|| LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0')|| YEAR(pdtFecha::DATE - Dias_Desfase)::VARCHAR(4))
			--ddyymm
			 WHEN LOWER(TRIM(formato_fecha)) = "ddyymm" THEN  (LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0')||(SUBSTR(YEAR(pdtFecha::DATE - Dias_Desfase),3,2))::VARCHAR(4)||LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0'))  
			--ddyyyymm
			 WHEN LOWER(TRIM(formato_fecha)) = "ddyyyymm" THEN  (LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0')||YEAR(pdtFecha::DATE - Dias_Desfase)::VARCHAR(4)||LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0'))  
			--mmddyy
			WHEN LOWER(formato_fecha) = "mmddyy" THEN ( LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0') ||(LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0'))|| (SUBSTR(YEAR(pdtFecha::DATE - Dias_Desfase),3,2))::VARCHAR(4))
			--mmddyyyy
			WHEN LOWER(formato_fecha) = "mmddyyyy" THEN ( LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0') ||(LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0'))|| YEAR(pdtFecha::DATE - Dias_Desfase)::VARCHAR(4))
			--mmyydd
			WHEN LOWER(formato_fecha) = "mmyydd" THEN ( LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0')|| (SUBSTR(YEAR(pdtFecha::DATE - Dias_Desfase),3,2))::VARCHAR(4) ||(LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0')))
			--mmyyyydd
			WHEN LOWER(formato_fecha) = "mmyyyydd" THEN ( LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0') || YEAR(pdtFecha::DATE - Dias_Desfase)::VARCHAR(4)||(LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0')))
			--yymmdd
			WHEN LOWER(formato_fecha) = "yymmdd" THEN ( (SUBSTR(YEAR(pdtFecha::DATE - Dias_Desfase),3,2))::VARCHAR(4) || LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0')|| (LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0')))
			--yyddmm
			WHEN LOWER(formato_fecha) = "yyddmm" THEN ( (SUBSTR(YEAR(pdtFecha::DATE - Dias_Desfase),3,2))::VARCHAR(4) || (LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0'))||LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0'))
			--yyyymmdd
			WHEN LOWER(formato_fecha) = "yyyymmdd" THEN ( YEAR(pdtFecha::DATE - Dias_Desfase)::VARCHAR(4)||LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0') || (LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0')))
			--yyyyddmm
			WHEN LOWER(formato_fecha) = "yyyyddmm" THEN ( YEAR(pdtFecha::DATE - Dias_Desfase)::VARCHAR(4)|| (LPAD(DAY(pdtFecha - Dias_Desfase), 2, '0'))||LPAD(MONTH(pdtFecha - Dias_Desfase), 2, '0') )			 
			 END )) AS NombreArchivo, 
			 (pdtFecha::DATE - Dias_Desfase) AS Fecha_Archivo,  Dias_Desfase, Rep_Aix, Rep_Win, Tipo_LayOut,B_Seguridad
		INTO vsArchivo_Origen, vsNombreArchivo, dtFechaArchivo, viDias_Desfase, vsRep_Aix, vsRep_Win, viTipo_LayOut,vsb_Seguridad
		FROM "informix".tf_catarchivo_transfer
		...
		AND (((psArchivoOrigen <> '') AND (psArchivoOrigen = Archivo_Origen)) OR (psArchivoOrigen = '')) --SI EL PARAMETRO ESTA EN BLANCO REGRESA TODOS LOS REGISTROS, SI CONTIENE INFO REGRESA SOLO EL TIPO CORRESPONDIENTE.
			...
			RETURN vsArchivo_Origen, vsNombreArchivo, dtFechaArchivo, viDias_Desfase, vsRep_Aix, vsRep_Win, viTipo_LayOut,vsb_Seguridad WITH RESUME;
...
END PROCEDURE
DOCUMENT
'AUTOR: 95296042 - Selene Campos Concilión ',
'Proyecto: 1433- TransferenciaArchivosTransfer',
'Solicito: Martín Pineda Torres',
'Descripcion: Procedimiento para armar u obtener los nombres de los archivos a transferir de un servidor a otro (WIN-AIX)',
'Fecha: 22-Julio-2014',
'Version: 1.0',
'BD: bditransfer',
'------------------------------------------------------------------------------',
...
'Proyecto: 1433- TransferenciaArchivosTransfer',
'Solicita: Martín Pineda Torres',
'Descripcion: Se agrega una nueva columna formato_fecha a la tabla tf_catatrchivo_transfer para validar con que formato se requiere que se nombre el archivo en cuanto a fecha',
'Fecha: 20-Agosto-2014',
'Version: 1.1',
'BD: bditransfer',
'------------------------------------------------------------------------------',
'Modifica: LIA Ricardo Reséndiz Martínez',
'Proyecto: Transferencia Archivos Transfer Estados de cuenta',
'Solicita: Martín Pineda Torres',
'Descripcion: Se agrega esquema para contemplar archivos con desface de mes anterior con posibilidades de variacion',
'Fecha: 09-Abril-2015',
'Version: 1.2',
'BD: bditransfer';

CREATE PROCEDURE "informix".sp_busca_ctafondeo_tf
(
	pEmpresa 	CHAR(03),
	pNumCtaTf 	CHAR(20)
)
...
DOCUMENT
'Consulta la cuenta Banco, el nombre del producto mediante el número de la cuenta transfer',
