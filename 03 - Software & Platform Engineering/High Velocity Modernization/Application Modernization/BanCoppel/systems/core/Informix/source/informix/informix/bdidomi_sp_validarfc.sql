CREATE PROCEDURE "informix".sp_validarfc(p_sRfc CHAR(13),p_sTipo CHAR(1))
    RETURNING CHAR(5);
     --*************************************************
   ...
   -- SET DEBUG FILE TO "/tmp/sp_validaRFC.out";
    --TRACE ON;
	IF (p_sTipo = "M") OR (p_sTipo = "F") THEN
	ELSE
	END IF;
    --- Se Divide el RFC en partes
        
	IF p_sTipo = "M" THEN
		--PRimera parte solo debe de contener Caracteres y debe de tener un tamaño de 3 caracteres
		LET  v_sPrimerRFC = SUBSTR(p_sRfc,1,3);
		 --Segunda parte solo debe de contener digitos y debe de tener un tamaño de 6 caracteres
		LET v_sSegundoRFC = SUBSTR(p_sRfc,4,6);
        --TErcera parte solo debe de contener alfanumericos y debe de tener un tamaño de 3 caracteres
		LET v_sTercerRFC = SUBSTR(p_sRfc,10,3);
	ELSE
	...
	IF p_sTipo = "M" THEN
	--si es persona moral
		IF (v_sLongRFC = 12 ) AND (v_sLongPri = 3 ) AND (v_sLongSeg = 6 ) AND (v_sLongTer = 3 ) THEN
	    ELSE
	        LET v_sCodRet = '00004';
	        RETURN v_sCodRet;
	    END IF;
    ...
    while (v_iInicio <= v_iLongitud) and (v_sfinciclo = 'F')
        ...
        IF ((v_sCadenados >= 'A') and (v_sCadenados <= 'Z')) or ((v_sCadenados >= 'a') and (v_sCadenados <= 'z')) THEN
            LET p_svalor = 'A';
        ELSE
            LET p_svalor = 'B';
            LET v_sfinciclo = 'T';
    ...
    while (v_iInicio <= v_iLongitud) and (v_sfinciclo = 'F')
        LET v_sCadenados = substr(v_sCadena,v_iInicio,1);
        IF ((v_sCadenados >= 'A') and (v_sCadenados <= 'Z')) or ((v_sCadenados >= 'a') and (v_sCadenados <= 'z')) or ((v_sCadenados >= '0')  and (v_sCadenados <= '9'))  THEN
            LET p_svalor = 'A';
        ELSE
            LET p_svalor = 'B';
            LET v_sfinciclo = 'T';
