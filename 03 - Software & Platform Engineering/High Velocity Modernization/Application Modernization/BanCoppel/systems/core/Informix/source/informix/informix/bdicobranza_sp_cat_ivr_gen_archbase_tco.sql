CREATE PROCEDURE "informix".sp_cat_ivr_gen_archbase_tco(pempresa CHAR(3), 
						                                        pfechacorte DATE,
                                                    ptipocobranza char(1))
RETURNING CHAR(6);
-------------------------------------------------------------------------------------------------------------------------
-- Creado por: Abrham Lopez L: 16/05/2016. Proceso para la generación del archivo ivr_camp_preventiva para to
--  execute procedure "informix".sp_cat_ivr_gen_archbase_tco('001', '05-15-2016','P');
--------------------------------------------------------------------------------------------------------------------------
...
--SET DEBUG FILE TO "/home/informix/ALL/ivr_camp_preventiva.out";
--TRACE ON; 
...
/*        SELECT MAX(FECHA_INSERT) INTO vlFechaInsert 
	...
	SELECT TRIM(valor_alfabetico)   --Nombre en RQI cat_ivr_generacionTO_10MMAAAA nombre en plan de trabajo cat_ivr_generacion10_DDMMAAAA
        INTO cnombre
        FROM bdicobranza:cb_param_campania
        WHERE empresa = pempresa
        AND tipo_campania = 1
        AND grupo_parametro = 'ARCHIVOS'
        AND num_parametro = 70;
...
/*	select count(*) into vcount
	...
	IF (day(vfechacorte)=10) THEN
--		CALL bdicobranza:"informix".sp_latinia_contador_cobranza('IVR_PREV1',vcount) RETURNING cCod_ret;
		CALL bdicobranza:"informix".sp_latinia_contador_cobranza('IVR_PREV10',vcount,null) RETURNING cCod_ret;
	else
--		CALL bdicobranza:"informix".sp_latinia_contador_cobranza('IVR_PREV2',vcount) RETURNING cCod_ret;
		CALL bdicobranza:"informix".sp_latinia_contador_cobranza('IVR_PREV15',vcount,null) RETURNING cCod_ret;
	END IF;
