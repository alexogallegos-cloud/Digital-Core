CREATE PROCEDURE "informix".sp_pf_obten_movimientos(pEmpresa 		CHAR(3),
                                                	pSucursal   	CHAR(4), 
                                                	pEjecutivo  	CHAR(9),
                                                	pCanal     		SMALLINT,
													pCredito 		CHAR(20), 
	...
	DEFINE cTransacc				INTEGER;
	DEFINE iPlazoMinDisEffectivo	INTEGER;
	DEFINE iPlazoMinCompApp			INTEGER;
	DEFINE cSucursal				CHAR(4);
	DEFINE iTotalReg				INTEGER;
	DEFINE iTasa1					DECIMAL(18,2);
	DEFINE iTasa2					DECIMAL(18,2);
	DEFINE cNumProducto				CHAR(4);
	DEFINE cNumcte					CHAR(9);
	DEFINE cDisponCred				CHAR(1);
	DEFINE cEjecutado				SMALLINT;
	...
	LET sCiclo						= 0;
	LET cPagina						= '';
	LET iDisposicionEfectivoApp     = 7;  --VARIABLE PARA DISPOSICÃN EN EFECTIVO
    LET iComprasApp                 = 8;  --VARIABLE PARA COMPRAS APP
	LET iNumPromo           		= 0;
	LET cTransacc					= 0;
	LET iPlazoMinDisEffectivo		= 0;
	LET iPlazoMinCompApp			= 0;
	LET cSucursal					= '';
	LET iTotalReg					= '';
	LET iTasa1						= 0;
	LET iTasa2						= 0;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr,iIsamErr,cErrorInfo
			IF iSqlErr != 0 THEN
				...
				RETURN TRIM(cCodRet),TRIM(cMensajeRet), NVL(dtFechaMov, DATE(1)), NVL(dtHoraMov, CURRENT), TRIM(NVL(cNumCredito,'')),TRIM(NVL(vReferencia,'')),NVL(dmonto,0.00),TRIM(NVL(cFolioSuc,'')), cPagina;							
			END IF;
		END EXCEPTION;

		--SET DEBUG FILE TO "/home/syscybcred/sp_obten_movimientos_pf.out";
		--TRACE ON;
		...
		IF pSkip IS NULL OR pRecuperacion IS NULL THEN
			LET cCodRet = '00001';
			LET cMensajeRet = "ERROR EN LOS PARAMETROS";
			RETURN TRIM(cCodRet),TRIM(cMensajeRet),dtFechaMov,dtHoraMov,TRIM(NVL(cNumCredito,'')),TRIM(NVL(vReferencia,'')),NVL(dmonto,0.00),TRIM(NVL(cFolioSuc,'')),cPagina;
		END IF;
						

		IF pSkip = 0 THEN

			EXECUTE PROCEDURE "informix".sp_pf_validaciones_core(pEmpresa, pSucursal, pEjecutivo, pCanal, pCredito, 1)
			INTO cCodRet, cMensajeRet, cNumcte, cNumCredito, cNumProducto, dmontoMin, dtFechaCorte, vListPromos, vListDispo, vListCompra, vListTransacte, dtFechaHoy;
			...
			ELSE
				...
				LET dLstNumPromo       = vListPromos;
				LET dLstTransac        = vListTransacte;
				LET dLstTransacDispo   = vListDispo;
				LET dLstTransacCompras = vListCompra;
			END IF;	

			IF EXISTS (SELECT 1 FROM "informix".sd_movimientos_pf_tmp WHERE fecha < dtFechaHoy) THEN 
				DELETE FROM "informix".sd_movimientos_pf_tmp WHERE fecha < dtFechaHoy;
			END IF;

			DELETE FROM "informix".sd_movimientos_pf_tmp WHERE num_credito = cNumCredito AND num_promocion IN (iDisposicionEfectivoApp, iComprasApp);


			--obtener la linea de credito
			SELECT sdo_capital,  (monto_otorgado - (sdo_cap_insoluto + sdo_retenido)), sdo_cap_insoluto
			INTO dCsg_cap_vig, dCsg_linea_disp, dCsg_tot_liquidacion
			FROM bdicred:"informix".sd_maesdos WHERE num_credito = pCredito;

			LET iContador = 0;
			LET sCiclo = 0;

			--OBTENEMOS LOS MOVIMIENTOS DE COMPRAS A COMERCIO EN UN RANGO DE FECHAS ENTRE LA FECHA CORTE Y LA FECHA DE HOY DE LAS PROMOCIONES DISPOCION EN EFECTIVO APP y COMPRAS EN APP .
				FOREACH 
					SELECT fecha_mov,hora_mov,num_credito, referencia, monto, folio_suc,transacc_suc, sucursal
					INTO dtFechaMov,dtHoraMov,cNumCredito,vReferencia,dmonto,cFolioSuc,cTransacc,cSucursal
					FROM (
							SELECT a.fecha_mov,a.hora_mov,TRIM(a.num_credito) as num_credito, TRIM(SUBSTR(a.referencia, 17, 41)) as referencia, a.monto,a.folio_suc,a.transacc_suc,a.sucursal
							FROM bdicred:"informix".sd_movdia a
							WHERE a.transacc_suc IN dLstTransac 
								AND a.num_credito= pCredito
								AND a.folio_suc NOT IN (SELECT folio_movto FROM bdicred:sd_promocion_credito WHERE num_credito = pCredito AND status IN (2,0))
								AND a.monto >= dmontoMin
								AND a.reversado = 'N'
							UNION ALL
							SELECT b.fecha_mov,b.hora_mov,TRIM(b.num_credito) as num_credito,TRIM(SUBSTR(b.referencia, 17, 41)) as referencia, b.monto,b.folio_suc,b.transacc_suc,b.sucursal 
							FROM bdicred:"informix".sd_movhis b
							WHERE b.transacc_suc IN dLstTransac 
								AND b.monto >= dmontoMin
								AND b.num_credito= pCredito
								AND b.folio_suc NOT IN (SELECT folio_movto FROM bdicred:sd_promocion_credito WHERE num_credito = pCredito AND status IN (2,0))
								AND b.reversado = 'N'
					...
					LET dd_SaldoFinal_pp = 0.0;
					LET cMensajeRet	= 'PROCESO EXITOSO';
					LET c_CodigoRet_pp = '000000';

					IF NOT EXISTS(SELECT folio_suc FROM bdiaclaracion:"informix".acl_movimiento WHERE folio_suc=cFolioSuc) THEN

						IF dmonto > 0 THEN 
							IF cTransacc IN dLstTransacDispo THEN
								
								-- EJECUTAMOS EL SP DE TASA PREFERENCIALES PROMO 7
								FOREACH
									EXECUTE PROCEDURE "informix".sp_pf_consulta_tasa_plazo_preferenciales(2, pEmpresa, pCanal, cNumCredito, cNumProducto, idisposicionefectivoapp, dmonto, 1, dtFechaHoy)
									INTO c_CodigoRet_pp, iTasa1, iPlazoMinDisEffectivo
								END FOREACH

								IF c_CodigoRet_pp :: INTEGER > 0 THEN
									--LET cMensajeRet = 'NO EXISTE TASA DE LA PROMOCION';
									LET c_CodigoRet_pp = '';
									LET iTasa1 = 0;
									LET iPlazoMinDisEffectivo = 0;
								ELSE 
									-- SI ENCUENTRA TASA PREFERENCIAL EJECUTE EL sp_proyecta_prest_credisol
									LET c_CodigoRet_pp = '';
									LET iTasa1 = NVL(iTasa1, 0);
									LET iPlazoMinDisEffectivo = NVL(iPlazoMinDisEffectivo,0);

									FOREACH 
										EXECUTE PROCEDURE bdicred:"informix".sp_proyecta_prest_credisol(dmonto,iPlazoMinDisEffectivo::INTEGER,0,'6900',cSucursal,1,0,pCredito,null,1,iDisposicionEfectivoApp::INTEGER, '2', iTasa1)
									...
									IF '6900' = cTransacc THEN
										LET vReferencia = "RETIRO DE EFECTIVO VENTANILLA";
									END IF;

									--SOLO SE CONSIDEREA INTERES + IVA
									LET dd_SaldoFinal_pp = dd_SaldoFinal_pp - dmonto;
								
									IF dd_SaldoFinal_pp  < dCsg_linea_disp AND dCsg_tot_liquidacion >= dmonto AND c_CodigoRet_pp == '000000' THEN
										LET sCiclo = sCiclo + 1;
										INSERT INTO "informix".sd_movimientos_pf_tmp(fecha_mov,hora_mov,num_credito,referencia,monto,folio_suc,sucursal,num_promocion,num_linea) 
										VALUES (dtFechaMov,dtHoraMov,cNumCredito,vReferencia,dmonto,cFolioSuc,cSucursal, iDisposicionEfectivoApp,sCiclo);
									END IF;
								END IF;
							END IF;
							...
							IF cTransacc IN dLstTransacCompras THEN
								...
								-- EJECUTAMOS EL SP DE TASA PREFERENCIALES PROMO 8
								FOREACH
									EXECUTE PROCEDURE "informix".sp_pf_consulta_tasa_plazo_preferenciales(2, pEmpresa, pCanal, cNumCredito, cNumProducto, iComprasApp, dmonto, 1, dtFechaHoy)
									INTO c_CodigoRet_pp, iTasa2, iPlazoMinCompApp
								END FOREACH
								...
								ELSE 
									-- SI ENCUENTRA TASA PREFERENCIAL EJECUTE EL sp_proyecta_prest_credisol
									LET c_CodigoRet_pp = '';
									LET iTasa2 = NVL(iTasa2, 0);
									LET iPlazoMinCompApp = NVL(iPlazoMinCompApp,0);

									FOREACH                
										EXECUTE PROCEDURE bdicred:"informix".sp_proyecta_prest_credisol(dmonto,iPlazoMinCompApp::INTEGER,0,'6900',cSucursal,1,0,pCredito,null,1,iComprasApp::INTEGER, '2', iTasa2)
										INTO c_CodigoRet_pp, i_Periodo_pp, d_FechaCouta_pp, dd_SaldoInicial_pp, dd_Mensualidad_aux_pp, dd_Intereses_pp, dd_IvaInteres_pp, dd_Capital_pp, dd_SaldoFinal_aux_pp, s_DiasPeriodo_pp, d_FechaAper_pp, c_NumMesesPago_pp    
									...
									IF dd_SaldoFinal_pp  < dCsg_linea_disp AND dCsg_tot_liquidacion >= dmonto  AND c_CodigoRet_pp == '000000' THEN
										LET sCiclo = sCiclo + 1;
										INSERT INTO "informix".sd_movimientos_pf_tmp(fecha_mov,hora_mov,num_credito,referencia,monto,folio_suc,sucursal,num_promocion,num_linea) 
										VALUES (dtFechaMov,dtHoraMov,cNumCredito,vReferencia,dmonto,cFolioSuc,cSucursal, iComprasApp, sCiclo);
									END IF;
								END IF;
							END IF;
						END IF;
		...
		IF cEjecutado = 0 THEN

			EXECUTE PROCEDURE "informix".sp_pf_validaciones_core(pEmpresa, pSucursal, pEjecutivo, pCanal, pCredito, 0)
			INTO cCodRet, cMensajeRet, cNumcte, cNumCredito, cNumProducto, dmontoMin, dtFechaCorte, vListPromos, vListDispo, vListCompra, vListTransacte, dtFechaHoy;
			...
			ELSE
				...
				--Inicializamos las listas
				LET dLstNumPromo       = "LIST {" || iDisposicionEfectivoApp || "," || iComprasApp || "}";
		...
		FROM "informix".sd_movimientos_pf_tmp
		WHERE num_credito = pCredito
		AND   num_promocion IN dLstNumPromo;

		LET cPagina = "Registro " || sCiclo || " de " || iTotalReg;

		FOREACH 
			SELECT SKIP pSkip FIRST pRecuperacion fecha_mov, hora_mov,num_credito,referencia,monto,folio_suc,num_linea
				INTO dtFechaMov,dtHoraMov,cNumCredito,vReferencia,dmonto,cFolioSuc,iLinea
				FROM "informix".sd_movimientos_pf_tmp
				WHERE num_credito = pCredito
				AND   num_promocion IN dLstNumPromo
				ORDER BY num_linea
				...
				LET sCiclo = sCiclo + 1;
				LET cPagina = "Registro " || iLinea || " de " || iTotalReg;
				RETURN TRIM(cCodRet),TRIM(cMensajeRet),dtFechaMov,dtHoraMov,TRIM(cNumCredito),TRIM(vReferencia),dmonto,TRIM(NVL(cFolioSuc,'')), cPagina WITH RESUME;
		END FOREACH

		LET iNRows = sCiclo;

		--SE VALIDA QUE REGRESE INFORMACION EL PROCEDIMIENTO.		
		IF iNRows = 0 AND pSkip = 0 THEN
			LET cCodRet = "00004";			
		...
		ELIF iNRows = 0 AND pSkip > 0 THEN
			LET cCodRet = "01001";			
			LET cMensajeRet = "FIN DE LA PAGINACION";
			LET cNumCredito = "";
			LET cNumCredito = "";
			LET dtFechaMov = null;
			LET dtHoraMov = null;
...
'FECHA:05/06/2025',
'DESCRIPCION: Procedimiento que obtiene los movimientos de compras a comercio en un determinado rango de fechas entre la fecha corte y la fecha hoy',
'tomando en cuenta solo movimientos con promociones de DISPOCION EN EFECTIVO APP y COMPRAS EN APP para credisoluciones.',
'AUTOR: JosÃ© Antonio RamÃ­rez Franco',
'FECHA: 12/03/2026',
'MODIFICACION: se actualiza la validacion para unicamente contemplar los registros cuya linea disponible sea mayor al interes e iva',
'BD: bdicred';

CREATE PROCEDURE "informix".sp_pf_proyecta_movimiento(pEmpresa CHAR(3), pSucursal CHAR(4),pEjecutivo CHAR(8),pCanal SMALLINT,  pNum_Credito CHAR(20), pFolio_Movto CHAR(16))
	RETURNING
		CHAR(5) 						AS CodRet,
		CHAR(80)						AS MensajeRet,
		DECIMAL(18,2)					AS oTasa,
		INTEGER							AS oPlazo,
		DECIMAL(18,2)					AS oMontoADiferir,
		DECIMAL(18,2)					AS oMensualidad,
		DECIMAL(18,2)					AS oTotalAPagar,
		DATE                            AS oFechaPrimerPago;
	...
	DEFINE oMensualidad			DECIMAL(18, 2);	
	DEFINE oMontoADiferir       DECIMAL(18, 2);	
	DEFINE oTotalAPagar			DECIMAL(18, 2);	
	DEFINE oFechaPrimerPago     DATE;
	DEFINE dmontoMin			DECIMAL(18, 2);	
	DEFINE dtFechaHoy			DATE;
	DEFINE dtFechaCorte			DATE;
	DEFINE cNumCredito			CHAR(20);
    ...
    DEFINE dLstTransacOnce      LIST(VARCHAR(4) NOT NULL);
    DEFINE dLstTransacDoce      LIST(VARCHAR(4) NOT NULL);
	DEFINE dLstTransacMov     LIST(VARCHAR(4) NOT NULL);
	DEFINE cTransacc			CHAR(4);
	DEFINE iPlazoMinOnce		INTEGER;
	DEFINE iTasa1				DECIMAL(18,2);
    ...
    DEFINE c_NumMesesPago_pp            CHAR(3);
	DEFINE iContador                    INTEGER;
	DEFINE dPromoMovSelect				INTEGER;
	DEFINE iDisposicionEfectivoApp      SMALLINT;
	DEFINE iComprasApp                  SMALLINT;
	...
	LET oTasa				= 0.00;	
	LET oPlazo				= 0; 
	LET oMensualidad		= 0.00;
	LET oMontoADiferir      = 0.00;
	LET oTotalAPagar		= 0.00;	
	LET oFechaPrimerPago    = DATE(1);
	LET dmontoMin			= 0.00;	
	LET dtFechaHoy 			= '';
	...
	LET sCiclo				= 0;
    LET dLstTransac         = 'LIST{}';
	LET dLstTransacOnce     = 'LIST{}';
	LET dLstTransacDoce     = 'LIST{}';
	LET dLstTransacMov     = 'LIST{}';
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr,iIsamErr,cErrorInfo
			IF iSqlErr != 0 THEN
				LET cCodRet= iSqlErr;	
				LET cMensajeRet = TRIM(cErrorInfo);
				RETURN TRIM(cCodRet), TRIM(cMensajeRet), NVL(oTasa,0.00), NVL(oPlazo,0), NVL(oMontoADiferir,0.00),NVL(oMensualidad,0.00), NVL(oTotalAPagar,0.00),oFechaPrimerPago;							
			END IF;
		END EXCEPTION;

		--SET DEBUG FILE TO "/home/syscybcred/sp_pf_proyecta_movimiento.out";
		--TRACE ON;	
		...
		--VALIDACIONES CORE
		EXECUTE PROCEDURE "informix".sp_pf_validaciones_core(pEmpresa, pSucursal, pEjecutivo, pCanal, pNum_Credito, 1)
		INTO cCodRet, cMensajeRet, cNumcte, cNumCredito, cNumProducto, dmontoMin, dtFechaCorte, vListPromos, vListDispo, vListCompra, vListTransacte, dtFechaHoy;
			...
			INTO dCsg_cap_vig, dCsg_linea_disp, dCsg_tot_liquidacion
			FROM bdicred:"informix".sd_maesdos WHERE num_credito = pNum_Credito;
				...
				SELECT LIMIT 1  monto, transacc_suc
					...
					FROM (
							...
							WHERE a.transacc_suc IN dLstTransac 
								...
								AND a.folio_suc = pFolio_Movto
								AND a.folio_suc NOT IN (SELECT folio_movto FROM bdicred:sd_promocion_credito WHERE num_credito = pNum_Credito AND status IN (2,0))
								AND a.monto >= dmontoMin
								AND a.reversado = 'N'
							UNION ALL
							...
							WHERE b.transacc_suc IN dLstTransac 
								...
								AND b.folio_suc = pFolio_Movto
								AND b.folio_suc	 NOT IN (SELECT folio_movto FROM bdicred:sd_promocion_credito WHERE num_credito = pNum_Credito AND status IN (2,0))
								AND b.reversado = 'N'
								AND b.fecha_mov BETWEEN dtFechaCorte AND dtFechaHoy  
						) AS tb_mov;
						
			LET oMontoADiferir =  dmonto;		
			...
			FOREACH
				EXECUTE PROCEDURE "informix".sp_pf_consulta_tasa_plazo_preferenciales(0, pEmpresa, pCanal, pNum_Credito, cNumProducto, dPromoMovSelect, dmonto, 0, dtFechaHoy) INTO cCodRet, iTasa1, iPlazoMinOnce
					
					IF cCodRet :: INTEGER > 0 THEN
					   LET cCodRet = '00005';
					   LET cMensajeRet = 'NO EXISTE TASA DE LA PROMOCION';
					   RETURN TRIM(cCodRet),TRIM(cMensajeRet), NVL(iTasa1,0.00), NVL(iPlazoMinOnce,0), NVL(oMontoADiferir,0.00), NVL(dd_Mensualidad_pp,0.00), NVL(dd_SaldoFinal_pp,0.00),oFechaPrimerPago;
					END IF;
					...
					IF dmontoMin > 0 AND iPlazoMinOnce > 1 AND iTasa1 > 0 THEN 

						FOREACH 

							EXECUTE PROCEDURE bdicred:"informix".sp_proyecta_prest_credisol(dmonto,iPlazoMinOnce::INTEGER,0,'6900',pSucursal,1,0,pNum_Credito,null,1,dPromoMovSelect::INTEGER, '2', iTasa1)
							INTO c_CodigoRet_pp, i_Periodo_pp, d_FechaCouta_pp, dd_SaldoInicial_pp, dd_Mensualidad_aux_pp, dd_Intereses_pp, dd_IvaInteres_pp, dd_Capital_pp, dd_SaldoFinal_aux_pp, s_DiasPeriodo_pp, d_FechaAper_pp, c_NumMesesPago_pp    
							...
							IF iContador = 1 THEN
								LET dd_Mensualidad_pp = dd_Mensualidad_aux_pp;
								 CALL bdicred:"informix".sp_valfechabil(d_FechaCouta_pp,'+') RETURNING cCodRet, oFechaPrimerPago;
						...
						--SOLO SE CONSIDEREA INTERES + IVA
						LET dd_SaldoFinal_aux_pp = dd_SaldoFinal_pp - dmonto;
						IF  dd_SaldoFinal_aux_pp < dCsg_linea_disp AND c_CodigoRet_pp == '000000' THEN
							LET sCiclo = sCiclo + 1;
							LET cCodRet = '00000';
							RETURN TRIM(cCodRet),TRIM(cMensajeRet), NVL(iTasa1,0.00), NVL(iPlazoMinOnce,0), NVL(oMontoADiferir,0.00), NVL(dd_Mensualidad_pp,0.00), NVL(dd_SaldoFinal_pp,0.00),oFechaPrimerPago WITH RESUME;
			...
			IF sCiclo = 0 THEN  --vcp
				LET cCodRet = '00024';
				LET cMensajeRet = 'NO SE PUEDE PROYECTAR EL MOVIMENTO SELECCIONADO';
				RETURN TRIM(cCodRet),TRIM(cMensajeRet), NVL(iTasa1,0.00), NVL(iPlazoMinOnce,0), NVL(oMontoADiferir,0.00), NVL(dd_Mensualidad_pp,0.00), NVL(dd_SaldoFinal_pp,0.00),oFechaPrimerPago;
			END IF;
	END
END PROCEDURE
DOCUMENT
'FECHA:04-06-2025',
'DESCRIPCION: Proyecta mensualidades de pagos fijos.',
...
CREATE PROCEDURE "informix".sp_pf_val_prospecto(pEmpresa    CHAR(3),
                                                pSucursal   CHAR(4), 
                                                pEjecutivo  CHAR(8),
                                                pCanal      SMALLINT,
                                                pNumCredito CHAR(20),
...
RETURNING   CHAR(5)  AS cod_ret,
    ...
    DEFINE iDisposicionEfectivoApp      SMALLINT;
    DEFINE iComprasApp                  SMALLINT;
    DEFINE flg_linea_insuficiente_pf_comp    SMALLINT;
    DEFINE flg_linea_insuficiente_pf_disp    SMALLINT;
    DEFINE iPlazo                       SMALLINT;
    DEFINE iTasa1                       DECIMAL(10,2);
    DEFINE iTasa2                       DECIMAL(10,2);
    DEFINE dLstNumPromo                 LIST(SMALLINT NOT NULL);
    DEFINE dLstTransac                  LIST(VARCHAR(4) NOT NULL);
    DEFINE dLstTransacDispo             LIST(VARCHAR(4) NOT NULL);
    DEFINE dLstTransacCompras           LIST(VARCHAR(4) NOT NULL);
    ...
    DEFINE dDisposicion_min             DECIMAL(18,2);
    DEFINE dCompra_min                  DECIMAL(18,2);
    DEFINE dCsg_cap_vig                 DECIMAL(18,2);
    DEFINE dCsg_linea_disp              DECIMAL(18,2);
    DEFINE dCsg_tot_liquidacion         DECIMAL(18,2);
    ...
    LET dDisposicion_min            = 0.0;
    LET dCompra_min                 = 0.0;
    LET iDisposicionEfectivoApp     = 7;  --VARIABLE PARA DISPOSICÃN EN EFECTIVO
    LET iComprasApp                 = 8;  --VARIABLE PARA COMPRAS APP
    LET iPlazo                      = 0;
    LET flg_linea_insuficiente_pf_disp   = 0;
    LET flg_linea_insuficiente_pf_comp   = 0;
    LET dCsg_cap_vig                = 0.0;
    LET dCsg_linea_disp             = 0.0;
    LET dCsg_tot_liquidacion        = 0.0;
    LET iTasa1                      = 0.0;
    ...
    BEGIN
        ON EXCEPTION  SET iSqlErr
            IF iSqlErr <> 0  THEN
                LET  cCodRet  = iSqlErr;
                LET  cMensajeRet = 'OCURRIO UN ERROR EN EL PROCESO';
                RETURN cCodRet,cMensajeRet;
            END IF;
        END  EXCEPTION

        --SET DEBUG FILE TO "/home/syscybcred/sp_val_prospecto_pf.out";
        --TRACE ON;
        ...
        INTO cCodRet, cMensajeRet, cNumcte, cNumCredito, cNumProducto, dValorMinDiferir, dtFechaCorte, vListPromos, vListDispo, vListCompra, vListTransacte, dtFechaHoy;
        ...
        --SI SE INGRESA LA TARJETA TAMBIEN SE VALIDA
        IF NVL(pNumTarjeta,'') <> '' THEN

            IF LENGTH(pNumTarjeta) != 16 THEN
                LET cCodRet = '00001';
                LET cMensajeRet = 'LA LONGITUD DEL NUMERO DE TARJETA ES INCORRECTA';
                RETURN cCodRet,cMensajeRet;
            END IF;

            SELECT a.num_tarjeta, b.num_credito, b.num_producto, b.numcte
            INTO cNumTarjeta, cNumCredito, cNumProducto, cNumcte
        ...
        --MONTO DE DISPOSICIÃN 
        IF (iDisposicionEfectivoApp IN dLstNumPromo) THEN

            SELECT NVL(MIN(monto_min),0) INTO dDisposicion_min FROM(
                SELECT {+AVOID_FULL("informix".sd_movdia)} MIN ( a.monto) as monto_min
                FROM bdicred:"informix".sd_movdia a
                WHERE a.transacc_suc IN dLstTransacDispo
                AND a.num_credito= pNumCredito
                AND a.monto >= dValorMinDiferir 
                AND a.folio_suc NOT IN (SELECT b.folio_suc FROM bdiaclaracion:"informix".acl_aclaracion a
                                        INNER JOIN bdiaclaracion:"informix".acl_movimiento b ON a.pky_aclaracion = b.fky_aclaracion
                                        WHERE a.num_cliente = cNumcte AND fechacaptura BETWEEN dtFechaCorte AND dtFechaHoy)
                AND a.reversado = 'N'
                AND EXISTS (SELECT 1 from bdicred:"informix".sd_tasa_plazo_app t 
                    where t.num_promo = iDisposicionEfectivoApp
                    and t.num_producto = cNumProducto 
                    and t.plazo_activo = 1 and t.plazo >  1 
                    and a.monto BETWEEN t.monto_ini AND t.monto_fin )
            UNION ALL
                SELECT {+AVOID_FULL("informix".sd_movhis)} MIN ( b.monto)  as monto_min
                FROM bdicred:"informix".sd_movhis b
                WHERE b.transacc_suc IN dLstTransacDispo
                AND b.monto >= dValorMinDiferir
                AND b.num_credito= pNumCredito
                AND b.folio_suc NOT IN (SELECT folio_movto FROM bdicred:sd_promocion_credito WHERE num_credito = pNumCredito AND status IN (2,0))
                AND b.folio_suc NOT IN (SELECT b.folio_suc FROM bdiaclaracion:"informix".acl_aclaracion a
                                        INNER JOIN bdiaclaracion:"informix".acl_movimiento b ON a.pky_aclaracion = b.fky_aclaracion
                                        WHERE a.num_cliente = cNumcte AND fechacaptura BETWEEN dtFechaCorte AND dtFechaHoy)
                AND b.fecha_mov BETWEEN dtFechaCorte AND dtFechaHoy 
                AND b.reversado = 'N'
                AND EXISTS (SELECT 1 from bdicred:"informix".sd_tasa_plazo_app t 
                    ...
                    and t.num_producto = cNumProducto 
                    and t.plazo_activo = 1 and t.plazo >  1 
                    and b.monto BETWEEN t.monto_ini AND t.monto_fin )
                    ) AS tb_disposicion;
        END IF;

        -- MONTO DE COMPRAS 
        IF(iComprasApp IN dLstNumPromo) THEN

            SELECT NVL(MIN(monto_min),0) INTO dCompra_min from(
                SELECT {+AVOID_FULL("informix".sd_movdia)} 
                        MIN (a.monto) as monto_min
                FROM bdicred:"informix".sd_movdia a
                WHERE a.transacc_suc IN dLstTransacCompras
                AND a.num_credito= pNumCredito
                AND a.monto >= dValorMinDiferir
                AND a.folio_suc NOT IN (SELECT b.folio_suc FROM bdiaclaracion:"informix".acl_aclaracion a
                                        INNER JOIN bdiaclaracion:"informix".acl_movimiento b ON a.pky_aclaracion = b.fky_aclaracion
                ...
                AND a.reversado = 'N'
                AND EXISTS (SELECT 1 from bdicred:"informix".sd_tasa_plazo_app t 
                    where t.num_promo = iComprasApp
                    and t.num_producto = cNumProducto 
                    and t.plazo_activo = 1 and t.plazo >  1 
                    and a.monto BETWEEN t.monto_ini AND t.monto_fin )
            UNION ALL
                SELECT {+AVOID_FULL("informix".sd_movhis)} 
                        MIN (b.monto)  as monto_min
                FROM bdicred:"informix".sd_movhis b
                WHERE  b.transacc_suc IN dLstTransacCompras
                AND b.monto >= dValorMinDiferir
                AND b.num_credito= pNumCredito
                AND b.folio_suc NOT IN (SELECT folio_movto FROM bdicred:"informix".sd_promocion_credito WHERE num_credito = pNumCredito AND status IN (2,0))
                AND b.folio_suc NOT IN (SELECT b.folio_suc FROM bdiaclaracion:"informix".acl_aclaracion a
                                        INNER JOIN bdiaclaracion:"informix".acl_movimiento b ON a.pky_aclaracion = b.fky_aclaracion
                                        WHERE a.num_cliente = cNumcte AND fechacaptura BETWEEN dtFechaCorte AND dtFechaHoy)
                AND b.fecha_mov BETWEEN dtFechaCorte AND dtFechaHoy 
        ...
        IF dDisposicion_min = 0 AND dCompra_min = 0 THEN
            LET cCodRet = '02434';
            LET cMensajeRet = 'EL CLIENTE ES CANDIDATO PERO NO CUENTAN CON MOVIMIENTOS';
            RETURN cCodRet,cMensajeRet;
        END IF;
        ...
        SELECT sdo_capital,  (monto_otorgado - (sdo_cap_insoluto + sdo_retenido)), sdo_cap_insoluto
        INTO dCsg_cap_vig, dCsg_linea_disp, dCsg_tot_liquidacion
        FROM bdicred:"informix".sd_maesdos WHERE num_credito = pNumCredito; 

        LET iContador = 0;
		LET dd_SaldoFinal_pp = 0.0;
        LET c_CodigoRet_pp = '';

        IF dCompra_min > 0 THEN
            
            FOREACH
                EXECUTE PROCEDURE "informix".sp_pf_consulta_tasa_plazo_preferenciales(2, pEmpresa, pCanal, cNumCredito, cNumProducto, iComprasApp, dCompra_min, 1, dtFechaHoy)
                INTO c_CodigoRet_pp, iTasa2, iPlazo
            ...
            LET iPlazo = NVL(iPlazo,0);
            LET iTasa2 = NVL(iTasa2,0);
            LET c_CodigoRet_pp = '';

            IF iPlazo > 1 AND iTasa2 > 0 THEN 
                FOREACH
                    EXECUTE PROCEDURE bdicred:"informix".sp_proyecta_prest_credisol(dCompra_min,iPlazo::INTEGER,0,'6900',pSucursal,1,0,pNumCredito,null,1,iComprasApp::INTEGER,'2', iTasa2)                
                    INTO c_CodigoRet_pp, i_Periodo_pp, d_FechaCouta_pp, dd_SaldoInicial_pp, dd_Mensualidad_aux_pp, dd_Intereses_pp, dd_IvaInteres_pp, dd_Capital_pp, dd_SaldoFinal_aux_pp, s_DiasPeriodo_pp, d_FechaAper_pp, c_NumMesesPago_pp    

                    IF c_CodigoRet_pp != '000000' THEN
                        LET cCodRet = c_CodigoRet_pp;
                        LET cMensajeRet = 'ERROR AL EJECUTAR sp_proyecta_prest_credisol';
                        RETURN cCodRet,cMensajeRet;
                    END IF;
                ...
                --SOLO SE CONSIDEREA INTERES + IVA
				LET dd_SaldoFinal_pp = dd_SaldoFinal_pp - dCompra_min;
                
                IF NOT(dd_SaldoFinal_pp  < dCsg_linea_disp AND dCsg_tot_liquidacion >= dCompra_min) THEN
                    LET flg_linea_insuficiente_pf_comp = 1;
                END IF;
            END IF;
        ELSE
            LET flg_linea_insuficiente_pf_comp = 1;
        ...
        IF dDisposicion_min > 0 THEN

            -- EJECUTAMOS EL SP DE TASA PREFERENCIALES
            FOREACH
                EXECUTE PROCEDURE "informix".sp_pf_consulta_tasa_plazo_preferenciales(2, pEmpresa, pCanal, cNumCredito, cNumProducto, iDisposicionEfectivoApp, dDisposicion_min, 1, dtFechaHoy)
                INTO c_CodigoRet_pp, iTasa1, iPlazo
            ...
            IF iPlazo > 1 AND iTasa1 > 0 THEN 
                FOREACH
                    EXECUTE PROCEDURE bdicred:"informix".sp_proyecta_prest_credisol(dDisposicion_min,iPlazo::INTEGER,0,'6900',pSucursal,1,0,pNumCredito,null,1,iDisposicionEfectivoApp::INTEGER,'2', iTasa1)                 
                    INTO c_CodigoRet_pp, i_Periodo_pp, d_FechaCouta_pp, dd_SaldoInicial_pp, dd_Mensualidad_aux_pp, dd_Intereses_pp, dd_IvaInteres_pp, dd_Capital_pp, dd_SaldoFinal_aux_pp, s_DiasPeriodo_pp, d_FechaAper_pp, c_NumMesesPago_pp    
                ...
                END FOREACH;
                 ...
                 --SOLO SE CONSIDEREA INTERES + IVA
				LET dd_SaldoFinal_pp = dd_SaldoFinal_pp - dDisposicion_min;

                IF NOT(dd_SaldoFinal_pp  < dCsg_linea_disp AND dCsg_tot_liquidacion >= dDisposicion_min) THEN
                    LET flg_linea_insuficiente_pf_disp = 1;
                END IF;
            END IF;
        ELSE
            LET flg_linea_insuficiente_pf_disp = 1;
        ...
        if iTasa1 = 0 AND iTasa2 = 0 THEN
            LET cCodRet = '00005';
            LET cMensajeRet = 'NO EXISTE TASA DE LA PROMOCION';
            RETURN cCodRet,cMensajeRet;
        END IF;

        IF flg_linea_insuficiente_pf_comp = 1 AND  flg_linea_insuficiente_pf_disp = 1 THEN
            LET cCodRet = '02433';
            LET cMensajeRet = 'EL CLIENTE NO ES CANDIDATO PARA DIFERIR';
        END IF;
...
'FECHA: 29/05/2025',
'DESCRIPCIÃN: Proceso se encargado de validar que el nÃºmero de credito o tarjeta ingresado sea candidato a prospecto de diferir validando',
'             las promociones de DisposicÃ­Ã³n y de Compras en la app para las Credisoluciones.',
'AUTOR: JosÃ© Antonio RamÃ­rez Franco',
'FECHA: 12/03/2026',
'MODIFICACION: se actualiza la validacion para unicamente contemplar los registros cuya linea disponible sea mayor al interes e iva',
'BD: bdicred';
