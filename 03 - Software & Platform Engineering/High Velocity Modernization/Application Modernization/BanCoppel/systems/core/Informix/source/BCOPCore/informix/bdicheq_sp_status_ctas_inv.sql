CREATE PROCEDURE "informix".sp_status_ctas_inv(vfechacarga DATE)
	 RETURNING CHAR(5), CHAR(60);
		...
		--SET DEBUG FILE TO "/informix/ifg/sp_status_ctas_inv.out"; --Se genera log en un archivo .out
		--TRACE ON;
		...
		BEGIN
			...
			IF EXISTS (SELECT dbsname, tabname FROM sysmaster:systabnames WHERE tabname = 'sc_ctas_inv')
			THEN 	
						DROP TABLE "informix".sc_ctas_inv;
						LET error_info = 'Se borro tabla sc_ctas_inv';
			END IF;
			
			CREATE TABLE "informix".sc_ctas_inv(
													aniomes     CHAR(6),
													producto    CHAR(12),
													status_cta  CHAR(10),
													total decimal(17,0)
			...
			LET error_info = 'Se creo tabla sc_ctas_inv';
			/*Se insertan las inversiones cresientes*/
			SELECT vaniomes AS aniomes, producto, status_cta::CHAR(10) AS status_cta, count(*) AS total FROM bdicheq:"informix".sc_maechq WHERE empresa = '001'
							AND producto = '1100'
                            AND status_cta IN (1, 2) GROUP BY producto, status_cta ORDER BY 2,3
					INTO TEMP tbltmpctas_inv;
			LET error_info = 'Se cargaron inversiones cresientes';
			/*Se insertan subtotal inversiones cresientes*/
			INSERT INTO tbltmpctas_inv
				SELECT vaniomes, producto, 'total', sum(total) FROM tbltmpctas_inv
					WHERE producto = '1100'GROUP BY producto ORDER BY 2,3;
			LET error_info = 'Se cargaron total de inversiones cresientes';
			/*Se insertan los Pagares*/
			INSERT INTO tbltmpctas_inv
				SELECT  vaniomes, cod_instrum, status_cta, count(*) FROM bdinvers:"informix".sv_maeinv WHERE empresa = '001'
						AND cod_instrum = '3000'
                        GROUP BY cod_instrum, status_cta ORDER BY 2,3;
			LET error_info = 'Se cargaron pagares';
			/*Se insertan subtotal inversiones cresientes*/
			INSERT INTO tbltmpctas_inv
				SELECT vaniomes, producto, 'total', sum(total) FROM tbltmpctas_inv
					WHERE producto = '3000'GROUP BY producto ORDER BY 2,3;
			LET error_info = 'Se cargaron total de pagares';
			/*Se inserta Gran total*/
			INSERT INTO tbltmpctas_inv
				SELECT vaniomes, 'TOTAL', '', SUM(total) FROM tbltmpctas_inv
					WHERE status_cta = 'total'
					 GROUP BY aniomes ORDER BY 2,3;
			/*SE PASAN REGISTROS A LA TABLA FISICA*/
			INSERT INTO sc_ctas_inv
				SELECT * FROM tbltmpctas_inv; 
			/*SE ELIMINA TABLA TEMPORAL*/
			--DROP TABLE tbltmpctas_inv;
			--MANEJO DE ARCHIVOS
			LET vsArchTemporal = 'temporal6.txt';
			LET vsNomArchivo = 'sc_ctas_inv' || TRIM(vaniomes) || '.txt' ;

			--GENERA EL ARCHIVO DE INTERCAMBIO
			LET vsSQL1 = 'echo "UNLOAD TO /resplogifx/conciliachq/' || TRIM(vsArchTemporal) || ' DELIMITER ' || '''|''';

			LET vsSQL2 = " SELECT  producto, status_cta, total   FROM sc_ctas_inv;";

			LET vsSQL3 = '" > /resplogifx/conciliachq/control_reporte6.sql';
			...
			LET vsSQL = '' ;
			LET vsSQL = 'chmod 666 /resplogifx/conciliachq/control_reporte6.sql' ;
			SYSTEM vsSQL;
			LET vsSQL = '' ;
			LET vsSQL = 'dbaccess bdicheq /resplogifx/conciliachq/control_reporte6.sql';
			SYSTEM vsSQL;
			--Borra el archivo de control.
			LET vsSQL = '' ;
			LET vsSQL = 'rm /resplogifx/conciliachq/control_reporte6.sql';
			SYSTEM vsSQL ;
...
'		   de captacion a plazos',
'	       indicando sus estatus';
