CREATE PROCEDURE "informix".consulta_numctecr(cEmpresa CHAR(3), cNumCredito CHAR(20))

    RETURNING CHAR(5), CHAR(20), SMALLINT,char(4);
    ...
    --SET DEBUG FILE TO "/tmp/consulta_numctecr.out";
    --TRACE ON;

    BEGIN
        ON EXCEPTION SET iSqlErr
            LET cCodRet = iSqlErr;
            RETURN cCodRet, cNumCte, sDiaCorte,v_numproducto;
        END EXCEPTION;
		
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO wait 3;
	
		SELECT day(ult_dia_mes)
		INTO vFecha
		FROM "informix".sd_fechas
		WHERE empresa = '001';
		
		-- Obtiene Numero de Cliente para el Estado de Cuenta
        SELECT numcte,num_producto INTO cNumCte,v_numproducto
        ...
        if v_numproducto <> '6400' then
            SELECT dia_corte INTO sDiaCorte
            FROM "informix".sd_maecredanexocrd
            WHERE empresa = cEmpresa AND num_credito = cNumCredito;
			
			IF sDiaCorte > vFecha then
				LET sDiaCorte = vFecha;
			end if
        else
            SELECT day(fecha_apertura) INTO sDiaCorte
            FROM "informix".sd_maecredcrd
            WHERE empresa = cEmpresa AND num_credito = cNumCredito;
        end if    
...
END PROCEDURE
DOCUMENT
"Se Obtiene el Numero de Cliente para el Estado de Cuenta de Crédito Reestructurado",
"FECHA: 12/08/2009",
"AUTOR: Iris Arias Zazueta",
"Se agrega consulta para obtener el dia de corte",
"FECHA: 04/09/2009",
"BD: bdicred";
