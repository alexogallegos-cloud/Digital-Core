CREATE PROCEDURE "informix".sp_domi_procesarch_hilo3()
 returning CHAR(5) AS Codigo_Respuesta,CHAR (100) AS Mensaje_Respuesta;
	...
	BEGIN
		...
		SELECT FIRST 1 nombre_arch30,fecha_presentacion,nombre_arch31,nombre_arch32,rango1,rango2,user_insert,fecha_insert INTO v_nombre_arch30,vsFecha_Presentacion,vsNomArchivo31,vsNomArchivo32,valor1,valor2,v_user_insert,v_fecha_insert FROM bdidomi:dom_cce_control_hilos WHERE nombre_procesarch=TRIM('sp_domi_procesararch30_3');
		
		EXECUTE PROCEDURE "informix".sp_domi_procesararch30_2(TRIM(v_nombre_arch30),TRIM(vsNomArchivo31),TRIM(vsNomArchivo32),TRIM(v_user_insert),valor1,valor2,5000000) INTO v_cod_ret;
