CREATE PROCEDURE "informix".sp_cce_consultar_detallecheques_pba
(
pEmpresa    CHAR(3),
pBanco      CHAR(3),
pCuenta     CHAR(20),
...
--	TRACE ON;
	...
	ELSE
        FOREACH WITH HOLD
            SELECT {+INDEX(cce_cheques_det idx_cheques_det4)} compensacion, transaccion, codseguridad, digverpre, digverinter
            INTO cCompensacion, cTransacc, cCodSeguridad, cDigVerPre, cDigVerInter
            FROM bditef:cce_cheques_det
            WHERE empresa = pEmpresa 
            AND cvebanco = pBanco
            ---AND numcuenta::INT8 = pCuenta::INT8
            AND numcuenta = '677027551'
            ---AND numcheque::INTEGER = pNoCheque::INTEGER
            AND numcheque = '14756'
