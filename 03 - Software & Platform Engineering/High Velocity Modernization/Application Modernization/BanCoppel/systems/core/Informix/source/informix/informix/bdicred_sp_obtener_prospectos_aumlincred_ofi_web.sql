CREATE PROCEDURE "informix".sp_obtener_prospectos_aumlincred_ofi_web
(
	pEmpresa    CHAR(3),	
	pSucursal   CHAR(4),
	pNumCredito CHAR(20),
	...
	RETURNING CHAR(5)  AS codigo_retorno, CHAR(80) AS mensaje_retorno;           
...
BEGIN
	...
	IF (SELECT count(num_solicitud) FROM  "informix".sd_bitacora_aumlincred WHERE empresa = pEmpresa AND num_solicitud = pNumCredito AND numcte =  pNumcte AND fecha_insert =  pFechaHoyAumlincred ) = 0 THEN	
		...
		IF (SELECT count(ejecutivo) FROM "informix".sd_perfiles_cac_aumlincred where empresa = 	pEmpresa AND ejecutivo = pEjecutivo) > 0 THEN
			LET cOrigen = 'C';
		ELSE
			LET cOrigen = 'S';
		END IF
		...
		LET cCodRet = SUBSTRING(cCodRet FROM 2 FOR 5);
	END IF
