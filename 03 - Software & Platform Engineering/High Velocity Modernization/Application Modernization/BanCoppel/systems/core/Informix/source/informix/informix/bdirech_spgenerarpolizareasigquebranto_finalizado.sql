CREATE PROCEDURE "informix".spgenerarpolizareasigquebranto_finalizado(p_dFechaActual DATE, p_sUsuario CHAR(8))
	...
	--SET DEBUG FILE TO  "spgenerarpolizareasigquebranto_finalizado.out";
	--TRACE ON;
	...
	BEGIN
		...
		IF v_sCodRet = '00000' THEN
			...
			FOREACH							
				...
				AND b.contable = '0'
				AND idasignado IN (7)
				AND b.usuarioautoriza=p_sUsuario
				...
				LET v_iSecuencia = v_iSecuencia + 1;
					...
					EXECUTE PROCEDURE bdinteg:"informix".spobtenercuentascontables (v_sEmpresa, v_sTransacReasigFaltante, v_sSistema, 7)
					...
					--LET v_sAuxiliar = '';	la cuenta 9513 si requiere auxiliar				
			...
			IF v_iSecuencia > 0 THEN --VALIDA SI SE GENERARON MOVIMIENTOS PARA GENERAR POLIZA CONTABLE
				--IF NOT EXISTS (SELECT 1 FROM bdicont:"informix".co_errorpoliza) THEN    
					...
					IF TRIM(v_sCodRet) = '000' AND v_iControlPoliza <> 0 AND NOT EXISTS(SELECT 1 FROM bdicont:tmpco_auditerr WHERE usuario = p_sUsuario) THEN
						...
						AND fecharegistro = p_dFechaActual AND contable = '0' AND usuarioautoriza=p_sUsuario;
