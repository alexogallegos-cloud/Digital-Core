CREATE PROCEDURE "informix".sp_aforedispersionautomatica(pUsuario CHAR(8), pTipoarch CHAR(1))
Returning CHAR(5),CHAR(200);

 DEFINE vcodret 					CHAR(5);
 DEFINE vsqlerr						INTEGER;
 DEFINE cMensaje					CHAR(200);
 DEFINE cNombre_arch				CHAR(30);
 ...
 DEFINE cCodRetInterno				CHAR(5); --DSB 12/03/2014 
 DEFINE cIndicadorNomArch			CHAR(2); --DSB 12/03/2014 
...
BEGIN
    ...
    --SET DEBUG FILE TO "/tmp/sp_AforeDispersionAutomatica.out";
    --TRACE ON;

 LET vcodret = '00000';
 LET cMensaje = '';
 LET cNomProceso = '';  --DSB 12/03/2014
 LET cNombre_arch = '';
 LET cStatusProceso = '';
 LET dFechaActual = '';
 LET cFechaFormat = '';
 LET cHoraActual = Current HOUR TO SECOND;
 LET cCodRetInterno = '00000'; --DSB 12/03/2014 
 LET cIndicadorNomArch = '';   --DSB 12/03/2014
	...
	LET cFechaFormat = LPAD(DAY(dFechaActual),2,0) || LPAD(MONTH(dFechaActual),2,0) || YEAR(dFechaActual) ;
		...
		IF pTipoarch = 1 THEN   --DSB 12/03/2014 
			LET cNomProceso = 'AforeDiAut';
			LET cIndicadorNomArch = 'AC';
		ELIF pTipoarch = 2 THEN
			LET cNomProceso = 'AfoDiAutOB';
			LET cIndicadorNomArch = 'OB';
		ELIF NVL(pTipoarch,'') = '' OR pTipoarch NOT IN (1,2) THEN
			LET vcodret = '10035';
			CALL "informix".sp_Afore_MensajeRetorno (vcodret) RETURNING cCodRetInterno,cMensaje;
	...
	FOREACH WITH HOLD
		SELECT nombre_arch INTO cNombre_arch FROM bdiprog:pp_arch_afore 
		WHERE SUBSTR(nombre_arch,1,5) = 'PAGOS' 
		AND SUBSTR(nombre_arch,6,8) = cFechaFormat 
		AND status = '01'
		AND SUBSTR(nombre_arch,15,2) = TRIM(cIndicadorNomArch) --DSB 12/03/2014
		
		CALL "informix".sp_Afore_Dispersion ( cNombre_arch , pUsuario, pTipoarch) RETURNING vcodret, cMensaje; 
	...
	Return vcodret,cMensaje WITH RESUME;
END 
END PROCEDURE
DOCUMENT
'AUTOR      : Antonio Bastidas',
'DESCRIPCION: Se genera proceso que toma todos los archivos que se encuentren por dispersar para pagos de AFORE.',
'FECHA      : 12 de JUNIO de 2009',
'VERSION    : 20090612.1133',
'BD         : BDIPROG',
...
CREATE PROCEDURE "informix".sp_aforebuscararchivosprocesar(pTipo CHAR(1), pNombreArchivo CHAR(30), pFechaInicial DATE,pFechaFinal DATE,pEstatus CHAR(2))
	RETURNING CHAR(5),CHAR(30),CHAR(30),DATE,CHAR(2),INTEGER,CHAR(30),CHAR(32);
...
DEFINE cIdentificador CHAR(2); --DSB 25/03/2014
...
LET iContador = '0';
LET cEstado = '';
LET cIdentificador = '';  --DSB 25/03/2014

--SET DEBUG FILE TO "/tmp/sp_AforeBuscarArchivosProcesar.out";
--TRACE ON;
...
BEGIN
	...
	ON EXCEPTION SET iSqlErr
		...
		END IF;
	END EXCEPTION;
	--si me mandan un stado distinto a 19 funciona normal
	IF pEstatus <> 19 THEN
		----si me mandan defchas i gual a 1900-01-01 
		IF pFechaInicial = date(1) and pFechaFinal = date(1) THEN
			FOREACH WITH HOLD 
				Select  tp.descripcion,arc.nombre_arch,arc.fecha_generado,arc.status,arc.tipo
				Into cTipoArchivo,cNombreArchivo,dFechaGeneracion,cEstatus,cTipo
				...
				LET iNoMovimientos = '';
				LET cNombre = '';
				-- SI EL NOMBRE DEL ARCHIVO CONTIENE 'PAGOS'
				IF SUBSTR(cNombreArchivo,1,5) = 'PAGOS' THEN  --DSB 25/03/2014
					-- TOMAR EL IDENTIFICADOR AC / OB
					LET cIdentificador = SUBSTR(cNombreArchivo,15,2); --DSB 25/03/2014
				-- SI EL NOMBRE DEL ARCHIVO CONTIENE 'CONF' O 'CONT'
				ELIF SUBSTR(cNombreArchivo,1,4) = 'CONT' OR SUBSTR(cNombreArchivo,1,4) = 'CONF' THEN
					--TOMAR LOS DOS DIGITOS DESPUES DE LA POSICION 5
					LET cIdentificador = SUBSTR(cNombreArchivo,5,2); --DSB 25/03/2014
					-- SI cIdentificador ES DIFENTE DE 'OB' SE ASIGNA EL cIdentificador = 'AC'
					IF cIdentificador <> 'OB' THEN
						LET cIdentificador = 'AC';
					END IF;
				END IF;
				
				IF cIdentificador = 'AC' THEN  --DSB 25/03/2014
					---obtener el total de movimiento dependiendo del tipo de archivo
					IF cTipo = 'P' THEN --Archivo de Pagos
						select total_mov into iNoMovimientos from bdiprog:pp_sumario where pp_sumario.nombre_arch = cNombreArchivo;
					ELIF  cTipo = 'C'  THEN---Archivo de Confirmación   trim(cTipoArchivo)
						LET cNombre = cNombreArchivo;
						LET cNombre = 'PAGOS' || SUBSTR(cNombreArchivo,5,8) || '.ACOPPEL.'|| SUBSTR(cNombreArchivo,22,2);
						select no_mov into iNoMovimientos from bdiprog:pp_encabezado where nombre_arch = cNombre;
						
					ELIF  cTipo = 'T'  THEN--Archivo de Cifras de Control   trim(cTipoArchivo)
						LET cNombre = 'PAGOS' || SUBSTR(cNombreArchivo,5,8) || '.ACOPPEL.'|| SUBSTR(cNombreArchivo,22,2);
						FOREACH
							Select distinct(status) Into cEstado From bdiprog:pp_detalle Where bdiprog:pp_detalle.nombre_arch = cNombre Group by status
							LET iContador = iContador + 1;
						END FOREACH;
							LET iNoMovimientos = nvl(iContador,'0');
							LET iContador = 0;
					END IF;
				ELIF cIdentificador = 'OB' THEN  --DSB 25/03/2014   -- OTROS BANCOS
					IF cTipo = 'P' THEN
						SELECT total_mov INTO iNoMovimientos FROM "informix".pp_sumario WHERE nombre_arch = cNombreArchivo;
					ELIF  cTipo = 'C'  THEN
						LET cNombre = cNombreArchivo;
						LET cNombre = 'PAGOS' || SUBSTR(cNombreArchivo,7,8) || '.OBACOPPEL.'|| SUBSTR(cNombreArchivo,24,2);
						SELECT no_mov INTO iNoMovimientos FROM "informix".pp_encabezado WHERE nombre_arch = cNombre;
					ELIF  cTipo = 'T'  THEN
						LET cNombre = 'PAGOS' || SUBSTR(cNombreArchivo,7,8) || '.OBACOPPEL.'|| SUBSTR(cNombreArchivo,24,2);
						FOREACH
							SELECT DISTINCT(status) INTO cEstado FROM "informix".pp_detalle WHERE nombre_arch = cNombre GROUP BY status
							LET iContador = iContador + 1;
						END FOREACH;
						LET iNoMovimientos = NVL(iContador,'0');
						LET iContador = 0;
					END IF;
				END IF;
				--selec par aobtener la descripcion del stado-----
				select descripcion,status || '-'|| descripcion into cEstadoDescripcion,cEstadoNumDes from bdiprog:pp_status_afore where status = cEstatus;				RETURN cCodRet,cTipoArchivo,cNombreArchivo,dFechaGeneracion,cEstatus,iNoMovimientos,cEstadoDescripcion,cEstadoNumDes with resume;
			END FOREACH;
		ELSE--- si me mandan un a fecha o un rango de fechas
			FOREACH
				...
				IF cIdentificador = 'AC' THEN  --DSB 25/03/2014
					...
					IF cTipo = 'P' THEN --Archivo de Pagos
						select total_mov into iNoMovimientos from bdiprog:pp_sumario where pp_sumario.nombre_arch = cNombreArchivo;
					ELIF  cTipo = 'C'  THEN--Archivo de Confirmación
						LET cNombre = cNombreArchivo;
						LET cNombre = 'PAGOS' || SUBSTR(cNombreArchivo,5,8) || '.ACOPPEL.'|| SUBSTR(cNombreArchivo,22,2);
						select no_mov into iNoMovimientos from bdiprog:pp_encabezado where nombre_arch = cNombre;
					ELIF  cTipo = 'T'  THEN--Archivo de Cifras de Control
						LET cNombre = 'PAGOS' || SUBSTR(cNombreArchivo,5,8) || '.ACOPPEL.'|| SUBSTR(cNombreArchivo,22,2);
						FOREACH
							Select distinct(status) Into cEstado From bdiprog:pp_detalle Where bdiprog:pp_detalle.nombre_arch = cNombre Group by status	
							LET iContador = iContador + 1;
						END FOREACH;
							LET iNoMovimientos = iContador;
							LET iContador = 0;
					END IF;
				ELIF cIdentificador = 'OB' THEN  --DSB 25/03/2014   -- OTROS BANCOS
					IF cTipo = 'P' THEN
				...
				END IF;
				--selec par aobtener la descripcion del stado-----
				select descripcion,status || '-'|| descripcion into cEstadoDescripcion,cEstadoNumDes from bdiprog:pp_status_afore where status = cEstatus;
				RETURN cCodRet,nvl(cTipoArchivo,''),nvl(cNombreArchivo,''),nvl(dFechaGeneracion,'1900-01-01'),nvl(cEstatus,''),nvl(iNoMovimientos,'0'),nvl(cEstadoDescripcion,''),nvl(cEstadoNumDes,'') with resume;
			END FOREACH;
		END IF;
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Este procedimmiento busca los archivos que se an generado para que se creen reportes, ', 
'retornando datos como el nombre del archivo,fecha de creacion, estado, y numeros de movimientos',
'Solicito : Armando Mercado',
'AUTOR: César Valdéz Figueroa',
'FECHA: 19 de Mayo 2009',
'VERSION: 20090519',
'BD: BDIPROG',
'CAMBIOS: Este Sp se modifico para que cuando reciba un "19" va a retornar todos los archivos que tengan un estado igual a 01 o 09.',
'         tambien s emodifico ya que no se obtenian bien el numero total de movimientos para los archivos de control',
'MODIFICO: César Valdéz Figueroa',
'FECHA: 16/Junio/2009',
'VERSION: 20090616',
'MODIFICO   : Josue Zepeda - 92802036',
'FOLIO      : 1411',
'DESCRIPCION: Se agrega proceso para otros bancos para que tambien se muestren en pantalla',
