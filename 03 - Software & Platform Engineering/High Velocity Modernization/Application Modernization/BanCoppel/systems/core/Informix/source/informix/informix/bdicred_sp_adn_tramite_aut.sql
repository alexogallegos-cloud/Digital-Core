CREATE PROCEDURE "informix".sp_adn_tramite_aut (pEmpresa CHAR (3))	
...
DEFINE dtFechaApertc    DATE;
DEFINE cDescProd     CHAR(50);
DEFINE cStatusSol     CHAR(2);
DEFINE cSucursal     CHAR(4);
DEFINE cNombreSuc     CHAR(100);
DEFINE cNombreCiudad     CHAR(100);
DEFINE cFrecuenciaPago     CHAR(20);

DEFINE dAntiguedad    INTEGER;
DEFINE cMovil     CHAR(13);
DEFINE cDescCompania     CHAR(50);
DEFINE cDescStatus     CHAR(50);
DEFINE cDescRT     CHAR(50);
DEFINE cRFC     CHAR(50);
DEFINE dSaldoUltDisp   DECIMAL(18,2);
DEFINE mMontoLinea   DECIMAL(18,2);
...
END EXCEPTION;
	...
	--SET DEBUG FILE TO '/informix/jesus/RQM10617/sp_adn_tramite_aut.out';
	--TRACE ON;
	...
	LET cNombreArchivo = TRIM('Tramite_Aut_ADN_xcta_Mes_')||TO_CHAR(dtFechaHoy,'%m%y')|| '.txt';
	LET cNombreArchivo1 = TRIM('Tramite_Aut_ADN_xcta_Mes_aux')||TO_CHAR(dtFechaHoy,'%m%y')|| '.txt';
	...
	FOREACH WITH HOLD 
		...
		SELECT num_solicitud,numcte, fecha_insert, status_solicitud,sucursal
		INTO cNumSol, cNumCte,dtFechaApertc, cStatusSol, cSucursal
		...
		and fecha_insert between dTFechaSD and dtFechaFinMes
			...
			SELECT DECODE(frecuencia_pgo,'1','MENSUAL','2','QUINCENAL','3','SEMANAL','MENSUAL'),
			movil_cuenta	,cuenta_nomina	,compania,linea,activacion_cobrada
			INTO  cFrecuenciaPago,cMovil, cCuentaNomina, cCompania, mMontoLinea,cActivacion
			...
			SELECT nombre,estado,ciudad 
				INTO cNombreSuc, cEstado,cCiudad
			...
			SELECT  (SELECT nombre FROM bdicheq:"informix".sc_producto WHERE producto =a.producto)
			INTO cDescProd
			FROM bdicheq:"informix".sc_maechq a WHERE cuenta =cCuentaNomina;
			
			SELECT nombre_carrier
			INTO cDescCompania
			FROM bdinteg:"informix".si_carriers 
			WHERE cve_carrier =cCompania;
	...
	END IF;					   
			...
			SELECT LIMIT 1 (rango_minimo * 12)
			INTO dAntiguedad
			FROM bdisolic:"informix".ss_detalle_scoring a , bdisolic:"informix".ss_scoring_element b
			WHERE a.num_solicitud =cNumSol
			and a.grupo =8
			AND a.grupo = b.grupo
			...
			INTO cCodRet2, cnomcte, iEdad;

		LET cRFC= "";
		
		LET cConsulta = TRIM(NVL(cSucursal,''))||'|'|| TRIM(NVL(cNombreSuc,''))||'|'||TRIM(NVL(cNombreCiudad,''))||'|'||TRIM(NVL(cEstadoDesc,''))||'|'|| TRIM(NVL(cNumCte,''))||'|'|| 	 NVL(cCuentaNomina,'')||'|'|| TRIM(NVL(cDescProd,''))||'|'|| NVL(iEdad,0)||'|'||TRIM(NVL(cFrecuenciaPago,''))||'|'||  TRIM(NVL(cMovil,''))||'|'|| TRIM(NVL(cDescCompania,''))||'|'|| '|'|| TRIM(NVL(cRFC,''))||'|'||NVL(dSaldoUltDisp,0)||'|'|| NVL(dAntiguedad,0)||'|'|| dtFechaApertc||'|'||TRIM(NVL(cDescStatus,''))||'|'||NVL(mMontoLinea,0) ||'|'||'|'||TRIM(NVL(cDescRT,''))||'|'||NVL(dComision,0) ;
    ...
    END FOREACH;
		...
		IF iContador  > 0 THEN 	
			...
			---se ejecuta para ponerle el encabezado 
			LET cEncabezado = 'echo "No. de sucursal'||'|'||'Nombre de la sucursal'||'|'||'Municipio'||'|'||'Estado'||'|'||'No. de cliente BanCoppel'||'|'||'No. de cuenta '||'|'||'Tipo de Cuenta  Ligada'||'|'||'Edad del solicitante'||'|'||'Periodo de pago'||'|'||'Telefono Movil Asociado'||'|'||'Compañia telefonica'||'|'||'RFC empresa'||'|'||'Dispersión inmediata anterior (Salario Neto)'||'|'||'Antigüedad en la empresa (Meses)'||'|'||'Fecha de Solicitud ADN'||'|'||'Estatus solicitud ADN'||'|'||'LDC Autorizada'||'|'||'Causa Rechazo'||'|'||'Comisión por activación'||'|'|| '" > '||TRIM(cruta)|| cNombreArchivo;  
...
CREATE PROCEDURE "informix".sp_muestreo_edoctacorte(pUsuario CHAR(8), pIdFuncion CHAR(10), pFecha DATE, pRegistros INTEGER, pRecuperacion INTEGER)
RETURNING CHAR(5) AS codret,
		...
		CHAR(60) AS descripcion_manterior,
		CHAR(60) AS descripcion_mactual;     
	...
	DEFINE cDescripcionMAnterior  CHAR(60);
	DEFINE cDescripcionMActual    CHAR(60);
	...
	LET dFechaCorte ='';
	LET cNumTarjeta ='';
	LET cDescripcionMAnterior='';
	LET cDescripcionMActual='';

	BEGIN
	
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cNumCredito,cNumProducto, cNumcte, dFechaCorte,cNumTarjeta,cDescripcionMAnterior, cDescripcionMActual;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_muestreo_edoctacorte.out';
		--TRACE ON;
		...
		FOREACH		
			...
			INTO cNumCredito, cNumProducto, cNumcte, dFechaCorte, cNumTarjeta, cDescripcionMAnterior, cDescripcionMActual
			FROM (
				SELECT cre.num_credito, cre.num_producto, cre.numcte, edo.fecha_corte, 
				edo.num_tarjeta, tipoant.descripcion as des_mes_anterior, tipoact.descripcion as des_mes_actual
				...
				LEFT JOIN bdicred:"informix".sd_tipocartera AS tipoact ON tipoact.empresa=cre.empresa AND tipoact.status_cred=edo.estatus_mes_actual
				LEFT JOIN bdicred:"informix".sd_tipocartera AS tipoant ON tipoant.empresa=cre.empresa AND tipoant.status_cred=edo.estatus_mes_anterior
			...
			RETURN cCodRet,cNumCredito,cNumProducto, cNumcte, dFechaCorte,cNumTarjeta,cDescripcionMAnterior, cDescripcionMActual WITH RESUME;
		END FOREACH;
...
'DESCRIPCION: SP que consulta informaciÃ³n de los estados de Cuenta',
'BD: bdicred';
