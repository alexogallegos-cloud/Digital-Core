CREATE PROCEDURE "informix".sp_dgeliminadoctosdigitalizados(pNumcte CHAR(20), pProducto CHAR(4), pCodDocto CHAR(4))
RETURNING CHAR(6) AS CodRet;
	...
	ON EXCEPTION SET iSqlErr, iSamErr, cDesErr
        SET DEBUG FILE TO '/resplogifx/conciliachq/sp_dgeliminadoctosdigitalizados.err';
        TRACE ON;
		LET cCodRet = iSqlErr;
	...
	--- SET DEBUG FILE TO '/resplogifx/conciliachq/sp_dgeliminadoctosdigitalizados.out';
	--- TRACE ON;
    
	IF NVL(pNumcte, "") = "" THEN
		LET cCodRet = "000001"; -- DEBE ENVIAR OBLIGATORIAMENTE EL NUMERO DE CLIENTE.		
	END IF
	
	IF NVL(pNumcte, "") = "" AND NVL(pProducto, "") = "" AND NVL(pCodDocto, "") = "" THEN
		LET cCodRet = "000001"; -- DEBE ENVIAR AL MENOS UN PARAMETRO OBLIGATORIO.		
	END IF
	
	IF cCodRet <> "000000" THEN 
		LET cCodRet = "000001"; --PARAMETROS INVÁLIDOS.
		RETURN cCodRet;
	END IF 
	
    /* ###################################################################################
	-- CONSULTAMOS LOS REGISTROS DE LOS DOCUMENTOS DIGITALIZADOS QUE TIENE EL CLIENTE.
	FOREACH
		SELECT cliente, cuenta, producto, cod_docto
		INTO cCliente, cCuenta, cProducto, cCodDocto
		FROM bdidigital@coppelimg_tcp:"informix".dg_expediente 
		WHERE empresa = empresa
		  AND cliente = DECODE(NVL(pNumcte, ""), "", cliente, pNumcte)
		  AND producto = DECODE(NVL(pProducto, ""), "", producto, pProducto)
		  AND cod_docto = DECODE(NVL(pCodDocto, ""), "", cod_docto, pCodDocto)
		  AND secuencia = secuencia
			
        -- ELIMINAMOS LOS REGISTROS DE LOS DOCUMENTOS DIGITALIZADOS QUE TIENE EL CLIENTE.
        DELETE bdidigital@coppelimg_tcp:"informix".dg_expediente 
        WHERE cliente = cCliente
          AND cuenta = cCuenta
          AND producto = cProducto
          AND cod_docto = cCodDocto;
        
        DELETE bdidigital@coppelimg_tcp:"informix".dg_expediente_img
        WHERE cliente = cCliente
          AND cod_docto = cCodDocto;
        
        DELETE "informix".dg_expediente_envio
        WHERE cliente = cCliente
          AND cuenta = cCuenta
          AND producto = cProducto
          AND cod_docto = cCodDocto;
	...
	IF DBINFO("sqlca.sqlerrd2") = 0 THEN
		LET cCodRet = "000002"; -- CLIENTE NO TIENE IMAGENES DIGITALIZADAS
	END IF
    ################################################################################### */
...
DOCUMENT
'DESCRIPCION: Realiza un rollback en caso que alguna imagen no se halla podido digitalizar.',
'AUTOR: Valentin Lopez',   
'FECHA DE CREACION: 28/08/2013',
'VERSION: 20130822.1530',
'BD: bdidigital';
