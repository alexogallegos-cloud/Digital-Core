CREATE PROCEDURE "informix".sp_bitacoraws_antad( pNumCategoria CHAR(2), pNumConvenio CHAR(3), pId_Sucursal CHAR (4), pFolioSucursal CHAR (16), pFechaPago DATE, pCodigoRespuesta CHAR(5), pConceptoRespuesta CHAR(80), pReferencia CHAR (27), pNumTrama INTEGER)
...
ON EXCEPTION SET iSqlErr
	IF iSqlErr <> 0 THEN
		...
		IF iInserta = 0 THEN
			LET iInserta = 1;
			IF iSqlErr <> -268 THEN
				INSERT INTO bdisac: "informix".sac_pagos_antad (id_sucursal, numcategoria, numconvenio, fechapago, folio_suc, num_trama, referencia, codRet, cod_resp, conceptorespuesta, user_insert, fecha_insert) VALUES (pId_Sucursal, pNumCategoria, pNumConvenio, pFechaPago, pFolioSucursal, pNumTrama, pReferencia, cCodRet, pCodigoRespuesta, pConceptoRespuesta,'informix', current);
			end if;
		END IF;
		RETURN cCodRet;
	END IF;
END EXCEPTION;
	...
	--SET DEBUG FILE TO "/informix/noe/sp_bitacoraws_antad.out";
	--TRACE ON;
...
--Insertar bitacora en bdisac: sac_pagos_antad 
	INSERT INTO bdisac:"informix".sac_pagos_antad(id_sucursal, numcategoria, numconvenio, fechapago, folio_suc, num_trama, referencia, forma_pago, importe, codRet, cod_resp, conceptorespuesta, user_insert, fecha_insert) VALUES (pId_Sucursal, pNumCategoria, pNumConvenio, pFechaPago, pFolioSucursal, pNumTrama, pReferencia, iFormaPago, iImporte, cCodRet, pCodigoRespuesta, pConceptoRespuesta, cUser, current);
...
'DESCRIPCION:  SP para la bitacora de ANTAD que se insertarÃ¡ en la tabla bdisac:sac_pagos_antad',
'FOLIO: ',
'FECHA : 08/05/2018',
'VERSION: 20180508.0957',
'BD: bdisac',
'-----------------------------------------------------------------------------------------------------------------------';

CREATE PROCEDURE "informix".sp_escenariosmsw_antad
	(
...
RETURNING 
   ...
   -- DeclaraciÃ³n de variables 
	...
	DEFINE vCampo1			CHAR(40);
	DEFINE vCampo7			CHAR(40);
   ...
   BEGIN
	...
	--SET DEBUG FILE TO '/informix/noe/sp_escenariosmsw_antad.out';
	--TRACE ON;	
	...
	SET LOCK MODE TO WAIT 3;  
		...
		SELECT campo1, campo7--, campo4, campo21
		INTO vCampo1, vCampo7--, sNum_clave, sClv_aprobacion
		...
		LET sRespInteract = TRIM(vCampo1);
		LET sStatus = TRIM(vCampo7);

		IF sRespInteract = '00000' THEN
			
			SELECT clave, accion INTO cCodigoRespuesta, cAccion FROM bdisac:"informix".sac_antad_escenarios where clave = sStatus;

			IF DBINFO("sqlca.sqlerrd2") = 0 THEN
				LET cCodigoRespuesta = '00003';
			ELSE
				IF cAccion = 1 THEN
					LET cCodigoRespuesta = '00001';
				ELSE
					IF cAccion = 2 THEN
						LET cCodigoRespuesta = '00002';
					ELSE
						IF cAccion = 3 THEN
							LET cCodigoRespuesta = '00002';
						ELSE
							IF cAccion = 4 THEN
								LET cCodigoRespuesta = '00004';
							END IF;
						END IF;
					END IF;
				END IF;
			END IF;	
		ELSE
			--LET cCodigoRespuesta = sRespInteract;
			LET cCodigoRespuesta = '00002';
		END IF;
...
DOCUMENT
'AUTOR: 93440138 - Noe Medina',
'DESCRIPCION: Obtiene descripcion del codigo de respuesta del WS de ANTAD',
'FOLIO: ',
'FECHA: 08-05-2018',
'VERSION: 20180508.1212',
'BD: bdisac',
'-----------------------------------------------------------------------------------------------------------------------';
