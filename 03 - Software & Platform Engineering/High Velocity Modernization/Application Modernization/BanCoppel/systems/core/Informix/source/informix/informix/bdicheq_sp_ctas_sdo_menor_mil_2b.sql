CREATE PROCEDURE "informix".sp_ctas_sdo_menor_mil_2b()
RETURNING CHAR(5);
	...
	SELECT pri_dia_mes - 1 units day, (pri_dia_mes - 3 units month) -  1 units day
      INTO dFecha, dFechaIni
      FROM sc_fechas
     WHERE empresa = '001';
	 
    LET cDia = LPAD(SUBSTR(dFecha,4,2),2,'0');
    LET cDia = cDia;
    LET cAnioMes = SUBSTR(dFecha,7,4) || LPAD(SUBSTR(dFecha,1,2),2,'0');
