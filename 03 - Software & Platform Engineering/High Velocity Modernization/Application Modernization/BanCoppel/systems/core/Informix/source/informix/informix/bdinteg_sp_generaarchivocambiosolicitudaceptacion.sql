CREATE PROCEDURE "informix".sp_generaarchivocambiosolicitudaceptacion(pempresa CHAR(3), pFechaAct DATE)
RETURNING CHAR(6); 

--DEFINICION DE VARIABLES
--CLIENTE
DEFINE vClave CHAR(1); --DEFAULT 'M'
DEFINE vcaja SMALLINT;DEFINE varea CHAR(1); --DEFAULT ''
...
DEFINE vaceptadosupervisadorechazado CHAR(1); --DEFAULT A
...
LET vcaja = 0;LET varea = ''; --DEFAULT BLANCO
...
LET vaceptadosupervisadorechazado = 'A'; --DEFAULT A
...
LET vnumcte = '';
LET vtiendafolioanterior = 0;LET vfolioanterior = 0; --DEFAULT 0
...
--Set debug file to '/tmp/sp_GeneraArchivoCambioSolicitudAceptacion.out';
--trace on;
	...
	IF pFechaAct <> mdy(1,1,1900) OR pFechaAct IS NOT NULL THEN	
		...
		ELSE
			...
			FOREACH
				...
				AND ssa.fecha_entrada = pFechaAct AND ssa.status_solicitud = 'AT'
				...
				IF vnumerosolicituddecredito <> '' OR vnumcte <> '' THEN
					...
					cte.fecha_insert, cte.ejecut_autoriza, TRIM(NVL(cte.string2,0))::INTEGER
					INTO vnombre1, vnombre2, vapell_paterno, vapell_materno, vnumcte, vefectuo, vrfc, 
					vfechamovto, vefectuo, vpersonasvivenendomicilio
					...
					--- AND dir.secuencia = (SELECT NVL(MAX(secuencia), 0) FROM bdinteg:"informix".si_direcciones WHERE numcte = vnumcte AND tipo_dir = '1');
					...
					--- SELECT NVL (MAX(secuencia),0) INTO iSecuencia FROM bdinteg:"informix".si_direcciones WHERE numcte = vnumcte AND  tipo_dir = '2';
					
                    SELECT dir.numerociudad,dir.numerocolonia,dir.numerocalle,dir.numeroextcalle,dir.departamento,dir.puntocardinal,dir.observaciones,
                    dir.entre_calles,dir.unidadhabitac,dir.manzana,dir.otros,dir.andador,dir.etapa,dir.lote,dir.edificio,dir.entrada,TRIM(NVL(tel3.telefono,0)),tel3.extension 
					...
					IF 	vestadocivil = 'C' THEN
						...
						SELECT nombre_empresa INTO vlugartrabajoconyuge 
						FROM bdinteg:"informix".si_ingresos WHERE numcte = vnumcte AND empresa = pempresa
						AND sec_ingreso = (SELECT MAX(sec_ingreso) FROM bdinteg:"informix".si_ingresos WHERE numcte = vnumcte AND empresa = pempresa); 
						
						--OBTIENE LA SECUENCIA(Datos Generales)PARA LA REFERENCIA 1 (CONYUGE)
						SELECT NVL(MAX(secuencia),0) INTO iSecuencia FROM bdinteg:"informix".si_refclientes WHERE empresa = pempresa
						AND numcte = vnumcte AND secuencia < (SELECT NVL(MAX(secuencia), 0) FROM bdinteg:"informix".si_refclientes 
						WHERE empresa = pempresa AND numcte = vnumcte AND secuencia > 0);
						
						SELECT numcte_banco,nombre1,nombre2,apell_paterno,apell_materno
						...
						WHERE empresa = pempresa AND numcte = vnumcte AND secuencia = iSecuencia;
						
						IF NVL(vclienteconyuge, '') = '' THEN
							LET vclienteconyuge = '0';
						END IF;
						
						--OBTIENE LA SECUENCIA(Dirección) PARA REFERENCIA 1 
						SELECT MAX(secuencia) INTO iSecuencia FROM bdinteg:"informix".si_refdirecciones WHERE numcte = vnumcte 
						AND secuencia < (SELECT MAX(secuencia) FROM bdinteg:"informix".si_refdirecciones WHERE numcte = vnumcte);
						
						SELECT numerociudad,numerocolonia,numerocalle,numeroextcalle,departamento,puntocardinal,observaciones,entre_calles,
						unidadhabitac,manzana,otros,andador,etapa,lote,edificio,entrada,TRIM(NVL(telefono1,0)),TRIM(NVL(telefono2,0))						
						...
						FROM bdinteg:"informix".si_refdirecciones WHERE numcte = vnumcte AND numcte_banco <> "" AND secuencia = iSecuencia;
						...
						SELECT NVL(MAX(secuencia),0)  INTO iSecuencia FROM bdinteg:"informix".si_refclientes WHERE empresa = pempresa
						AND numcte = vnumcte AND secuencia > 0;
						
						SELECT numcte_banco,nombre1,nombre2,apell_paterno,apell_materno,parentesco
						INTO vclientereferencia2,vnombreunoreferencia2,vnombredosreferencia2,vapellidopaternoreferencia2,
						vapellidomaternoreferencia2,vclavereferencia2
						FROM bdinteg:"informix".si_refclientes
						WHERE empresa = pempresa AND numcte = vnumcte AND secuencia = iSecuencia;
						...
						SELECT numerociudad,numerocolonia,numerocalle,numeroextcalle,departamento,puntocardinal,observaciones,
						entre_calles,unidadhabitac,manzana,otros,andador,etapa,lote,edificio,entrada,TRIM(NVL(telefono1,0)),TRIM(NVL(telefono2,0))
						INTO vciudadbanco,vcoloniabanco,vcallereferencia2,vcasareferencia2,vdeptoointeriorreferencia2,
						vrumboreferencia2,vcomplementoreferencia2,ventrecallesreferencia2,cflaguht,vuhrmanzana2,vuhrotros2,vuhrandador2,vuhretapa2,vuhrlote2,
						vuhredificio2,vuhrentrada2,vtelefonoreferencia2,vtelefonocelularreferencia2
						FROM bdinteg:"informix".si_refdirecciones WHERE numcte = vnumcte AND tipo_dir = '1' AND secuencia = iSecuencia;
					...
					IF 	vestadocivil <> 'C' THEN
						SELECT MAX(secuencia) INTO iSecuencia FROM bdinteg:"informix".si_refclientes WHERE empresa = pempresa
						AND numcte = vnumcte AND secuencia <> (SELECT MAX(secuencia) 
						FROM bdinteg:"informix".si_refclientes WHERE empresa = pempresa AND numcte = vnumcte AND secuencia > 0);
						
						SELECT numcte_banco,nombre1,nombre2,apell_paterno,apell_materno,parentesco
						INTO vclientereferencia,vnombreunoreferencia,vnombredosreferencia,vapellidopaternoreferencia, vapellidomaternoreferencia,vclavereferencia1
						FROM bdinteg:"informix".si_refclientes
						WHERE empresa = pempresa AND numcte = vnumcte AND secuencia = iSecuencia;
						...
						SELECT numerociudad,numerocolonia,numerocalle,numeroextcalle,departamento,puntocardinal,observaciones,
						entre_calles,unidadhabitac,manzana,otros,andador,etapa,lote,edificio,entrada,TRIM(NVL(telefono1,0)),TRIM(NVL(telefono2,0))
						INTO vciudadbanco,vcoloniabanco,vcallereferencia,vcasareferencia,vdeptoointeriorreferencia,vrumboreferencia,
						...
						entre_calles,unidadhabitac,manzana,otros,andador,etapa,lote,edificio,entrada,TRIM(NVL(telefono1,0)),TRIM(NVL(telefono2,0))
						INTO vciudadbanco,vcoloniabanco,vcallereferencia2,vcasareferencia2,vdeptoointeriorreferencia2,
						vrumboreferencia2,vcomplementoreferencia2,ventrecallesreferencia2,cflaguht,vuhrmanzana2,vuhrotros2,vuhrandador2,vuhretapa2,vuhrlote2,vuhredificio2,vuhrentrada2,
						vtelefonoreferencia2,vtelefonocelularreferencia2
						FROM bdinteg:"informix".si_refdirecciones WHERE numcte = vnumcte AND tipo_dir = '1' AND secuencia = iSecuencia;	
					...
					ELSE
						...
						WHERE num_solicitud = (SELECT NVL(MAX(num_solicitud), '') FROM bdisolic:"informix".ss_solicitud_os WHERE num_solicitud = vnumerosolicituddecredito);
					END IF; 							
					...
					ELSE
						LET cfechadesdecuandovive = YEAR(dFechaAlta) - iAniosHabita; 
					...
					ELSE
						LET cfechaantiguedtrab = YEAR(dFechaAlta) - iAniosHabita;
						LET cfechaantiguedtrab = TRIM(cfechaantiguedtrab)||'/01/01';
					END IF;

					--OBTENER ESCOLARIDAD
					EXECUTE PROCEDURE "informix".sp_ObtieneEscolaridad (vnumcte, '') INTO vCodRetorno,vescolaridad; 
					...
					IF NVL(vfolio, '') = '' THEN
						LET vfolio = '0';
					END IF;
					--SELECT  tiendafolio, folio INTO vtiendafolio2, vfolio2 FROM bdisolic:"informix".ss_osclientesupervisar  WHERE num_solicitud = vnumerosolicituddecredito AND secuencia = (SELECT MAX (secuencia) from bdisolic:"informix".ss_osclientesupervisar WHERE num_solicitud = vnumerosolicituddecredito );
					--LET vfolio = ( vtiendafolio2 || vfolio2 );
					...
					--WHERE num_credito = vnumerosolicituddecredito;
					
					--INGRESO MENSUAL
					--LET vlimitecredito = iLimiteCredito/cValor::INTEGER;
					--IF vlimitecredito > 0.1 AND vingresomensual < 1 THEN
...
--*************************************************************************
--| Procedimiento   : "informix".sp_GeneraArchivoCambioSolicitudAceptacion
--| Versión         : 1.0
--| Creado por      : Martha Aguirre
--| Fecha creacion  : Noviembre de 2008
--| Descripción     : Realiza la extracción de datos de clientes con crédito autorizado.
--*************************************************************************
...
--| Descripción     : Se modifica para la consulta de cambios de solicitud a aceptado dentro del proceso de aceptacion de solicitud.
--*************************************************************************
END PROCEDURE;