CREATE PROCEDURE "informix".ctefisico_pros( pEmpresa CHAR(3),
                                            pFuncion CHAR(1),
                                            pNumcte CHAR(20),
                                            pSucursal CHAR(4),
                                            pEjecutivo CHAR(8),
...
RETURNING CHAR(5),CHAR(20);
    ...
    --SET DEBUG FILE TO '/tmp/bbaez/ctefisico_pros.out';
    --TRACE ON;
	...
	-- Agrega registros en la tabla pr_autorizacion con status PC.
		...
		IF EXISTS (SELECT * FROM "informix".pr_autorizacion WHERE num_solicitud = cNumcte AND fecha_entrada = dFecha AND status_solicitud = 'PC') THEN
	...
	-- Consulta la tabla pr_monitorconcilia para obtener id_empcob por numero de empleado de cobranza y fecha
        ...
        BEGIN
			...
			IF cExisteCliente = '1' THEN
					...
					fecha_alta = dFecha, apell_casada = pApell_casada, distrito = pDistrito, numcte_ref = pNumcte_ref, string1 = "", string2 = pNumhabitantes,
					  ...
					  fecha_insert = dFecha, estado = '0', estado_os = '0', status_numcte_pros = 'PC', numcte = '', tipo_alta = pTipoAlta
					  WHERE numcte_pros = cNumcte;
        ...
        RETURN cCodret, cNumcte;
    ELSE
     -- Se elimina a petición del cliente. Folio: 1468 - 03/12/2014
	 /*
		SELECT 1
          INTO cExiste
	...
	*/
        ...
        UPDATE "informix".pr_cliente
           SET ( ejecutivo, tpo_persona, tipo_cliente, sector, segmento, actividad_esp,
                 grupo, subgrupo, residencia, fecha_alta, apell_casada, distrito, string2, estado, estado_os, numcte ) =
               ( pEjecutivo, pTp_persona, pTp_cliente, pSector, pSegmento, pActividad_esp,
                 pGrupo, pSubgrupo, pResidencia, dFecha, pApell_casada, pDistrito, pNumhabitantes, '0', '0', '')
        WHERE numcte_pros = cNumcte;
