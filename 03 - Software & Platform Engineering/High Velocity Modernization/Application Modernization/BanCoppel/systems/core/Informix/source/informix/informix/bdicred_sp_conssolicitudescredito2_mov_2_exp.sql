...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se crea sp para unificar "sp_conssolicitudescredito2_mov" y "sp_conssolicitudescredito2".',
'AUTOR : Ever Fierro Hernández',
'FECHA : 29/10/2018',
'Folio : 480',
'BD    : bdisolic',
'DESCRIPCION: Se modifa la opcion 0 y opcion 3 para separar las consultas de credito y/o movil "sp_conssolicitudescredito2_mov"',
'AUTOR : Jose Luis Garcia',
'FECHA : 11/01/2019',
'Folio : 480',
'BD    : bdisolic';

CREATE PROCEDURE  "informix".envia_monitorsol_cjunk_ss(o_empresa CHAR(3), 
										  o_sucursal CHAR(20), 
...
--    SET LOCK MODE TO WAIT 3;
   -- Carga la Fecha del Dia
	...
	LET dFechaInicio = vfecha_hoy - 10;
	select num_solicitud INTO sNumSolic from ss_solicitudes where sucursal=o_Sucursal and numcte=o_numcte and num_producto=o_Num_producto;
	...
	IF o_Tipo = 1 THEN
		...
		FOREACH 
			SELECT skip o_solicitudes limit 11 				
				...
				a.monto_solicitado, a.fecha_insert, a.tipo_solicitud
			INTO s_numsol, s_numcte, s_status, sNumProducto, s_linea, s_fechasol, pTipoSol 
			FROM "informix".ss_solicitudes a
			INNER JOIN "informix".ss_autorizacion c on a.sucursal = o_Sucursal and a.tipo_solicitud in ('T','C') AND a.status_solicitud IN ('AT','RT','CC','BC','OS','EE','OA','EA','CE','ST','LC','MC','EC')
			and a.num_solicitud = c.num_solicitud and  c.num_solicitud  = sNumSolic AND c.status_solicitud  = o_status_solicitud and c.fecha_entrada =  (SELECT MAX(fecha_entrada) FROM "informix".ss_autorizacion h
                                                            WHERE c.empresa           = '001'
									   	                    AND c.num_solicitud     = sNumSolic
											                AND c.status_solicitud  = o_status_solicitud
                                                            AND c.fecha_entrada >= dFechaInicio)
			...
			--(a.empresa = c.empresa and a.num_solicitud = c.num_solicitud and a.status_solicitud = c.status_solicitud and fecha_entrada = vFechaMaxEnt
            --WHERE a.sucursal = o_Sucursal			
			--AND a.tipo_solicitud IN ('T','C') 
			--AND a.status_solicitud IN ('AT','RT','CC','BC','OS','EE','OA','EA','CE','ST','LC','MC','EC')			
			
			
			SELECT b.apell_paterno, b.apell_materno, b.Nombre1, b.Nombre2, b.razon_social, b.rfc
			INTO apellidopaterno, apellidomaterno, nombre1, nombre2, r_social, s_rfc
			FROM bdinteg:"informix".si_cliente b 
			WHERE b.numcte = o_numcte;
		  ...
		  SELECT skip o_solicitudes limit 11 				
			...
			INNER JOIN "informix".ss_autorizacion c on (a.empresa = c.empresa and a.num_solicitud = c.num_solicitud and a.status_solicitud = c.status_solicitud and fecha_entrada = 
                                                           (SELECT MAX(fecha_entrada) FROM "informix".ss_autorizacion h
                                                            ...
                                                            AND c.fecha_entrada >= vfecha_hoy - 10)
	...
	ELIF o_Tipo = 4 THEN
		...
		SELECT  skip o_solicitudes limit 11 				
			...
			INTO s_numsol, s_numcte, s_status, sNumProducto, s_linea, s_fechasol, pTipoSol
				--apellidopaterno, apellidomaterno, nombre1, nombre2, r_social, s_rfc,
			FROM "informix".ss_solicitudes a 
			INNER JOIN "informix".ss_autorizacion c on a.sucursal = o_Sucursal and a.tipo_solicitud in ('T','C') 
			and a.num_solicitud = c.num_solicitud and  c.num_solicitud  = sNumSolic AND c.status_solicitud  = o_status_solicitud and c.fecha_entrada =  (SELECT MAX(fecha_entrada) FROM "informix".ss_autorizacion h
                                                            WHERE h.empresa           = '001'
									   	                    AND h.num_solicitud     = sNumSolic
											                AND h.status_solicitud  = o_status_solicitud
                                                            AND h.fecha_entrada >= dFechaInicio)
			ORDER BY a.num_solicitud
			
			--on (a.empresa = c.empresa and a.num_solicitud = c.num_solicitud and a.status_solicitud = c.status_solicitud and fecha_entrada = 
            --                                               (SELECT MAX(fecha_entrada) FROM "informix".ss_autorizacion h
            --                                                WHERE c.empresa           = empresa
			--						   	                    AND c.num_solicitud     = num_solicitud
			--								                AND c.status_solicitud  = status_solicitud)
			...
			--AND a.status_solicitud = o_status_solicitud
			--AND a.tipo_solicitud IN ('T','C') 			
			--ORDER BY a.num_solicitud/*b.Nombre1, b.Nombre2, b.apell_paterno, b.apell_materno, */
			
			
			select apell_paterno, apell_materno, Nombre1, Nombre2, razon_social, rfc
			INTO apellidopaterno, apellidomaterno, nombre1, nombre2, r_social, s_rfc
			from bdinteg:"informix".si_cliente where numcte = o_numcte;
	...
	ELIF o_Tipo = 0 Then		  
			...
			FROM "informix".ss_solicitudes a
			--INNER JOIN bdinteg:"informix".si_cliente b          ON (a.numcte  = b.numcte  )
			WHERE  num_soliciud=sNumSolic--a.numcte = o_numcte
			AND a.tipo_solicitud IN ('T','C') 			
			AND a.status_solicitud IN ('AT','RT','CC','BC','OS','EE','OA','EA','CE','ST','LC','MC','EC')
			ORDER BY a.num_solicitud--b.Nombre1, b.Nombre2, b.apell_paterno, b.apell_materno, 
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Se modifica procedimiento para que no se realice filto por "Sucursal cuando se haga consulta por nombre"',
