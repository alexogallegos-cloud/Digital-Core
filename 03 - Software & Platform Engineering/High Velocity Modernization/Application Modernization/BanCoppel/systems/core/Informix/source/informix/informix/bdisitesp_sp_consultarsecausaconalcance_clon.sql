CREATE PROCEDURE "informix".sp_consultarsecausaconalcance_clon(pEmpresa CHAR(3), pSituacion CHAR(1), pCausa INTEGER, pFlagSoloSituaciones CHAR(1), pAlcance SMALLINT, pRegistros INTEGER, pRecuperacion INTEGER)
        ...
        IF pSituacion is not null AND pSituacion <> ''  THEN
            IF pCausa is not null AND pCausa > 0 THEN
                FOREACH   --SE OBTIENEN LOS DATOS DE UNA SITUACION Y CAUSA EN ESPECIFICO
                    SELECT {+INDEX (se_catsitesp  idx_catsitesp)} SKIP pRegistros FIRST pRecuperacion situacion, causa, descripcion, alcance, despliegue, vigente, cveccasociada
        ...
        ELSE
            IF pFlagSoloSituaciones is not null AND pFlagSoloSituaciones <> '' THEN
                FOREACH   --SE OBTIENEN TODAS LAS SITUACIONES ESPECIALES SOLAMENTE LA SITUACION
                    SELECT {+INDEX (se_catsitesp idx_catsitesp2)} SKIP pRegistros FIRST pRecuperacion DISTINCT situacion
                    INTO v_situacion
                    FROM bdisitesp:se_catsitesp
                    WHERE empresa = pEmpresa
                    AND alcance IN ( pAlcance, 2 )
            ...
            ELSE
                FOREACH   --SE OBTIENEN TODOS LOS DATOS DE LAS SITUACIONES ESPECIALES
                    --create index "informix".idx_catsitesp2 on "informix".se_catsitesp (situacion,alcance) using btree  in datos01 ;
                    SELECT {+INDEX (se_catsitesp idx_catsitesp2)} SKIP pRegistros FIRST pRecuperacion situacion, causa, descripcion, alcance, despliegue, vigente, cveccasociada
                    INTO v_situacion, v_causa, v_cdescripcion, v_alcance, v_despliegue, v_vigente, v_cvecc
                    FROM bdisitesp:se_catsitesp
                    WHERE empresa = pEmpresa
                    AND alcance IN ( pAlcance, 2 ) and situacion is not null
                    ORDER BY situacion, causa
