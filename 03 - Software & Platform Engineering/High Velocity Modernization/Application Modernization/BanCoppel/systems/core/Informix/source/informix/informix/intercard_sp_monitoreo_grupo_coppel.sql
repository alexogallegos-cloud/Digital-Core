CREATE PROCEDURE "informix".sp_monitoreo_grupo_coppel(pFechaInicio DATETIME YEAR TO FRACTION(5), pFechaFin DATETIME YEAR TO FRACTION(5))
    ...
    DEFINE vGrupo VARCHAR(40);
    DEFINE vNumIteracion SMALLINT;
    DEFINE vSumCantidad_12 INTEGER;
    DEFINE vSumCantidad_34 INTEGER;
    ...
    LET vNumIteracion = 0;
    LET vGrupo = '';
    LET vSumCantidad_12 = 0;
    LET vSumCantidad_34 = 0;
        
    --SET DEBUG FILE TO RUTA_ORIGEN || "ejec_sp_monitoreo_grupo_coppel.out";
    --TRACE ON;
	...
	BEGIN 
        ...
        DROP TABLE IF EXISTS tmp_movs_coppel;
        DROP TABLE IF EXISTS tmp_movs_grupo;
        TRUNCATE TABLE intercard:tbl_trxs_grupo_coppel;
        ...
        WHERE fechahorainauth BETWEEN pFechaInicio AND pFechaFin
            AND formato = '0200'
            AND prodind = '02'
            AND idretailer IN ('7565819', '6310437', '7167537', '7926768', '1075847', '1090447','3260825')
        INTO TEMP tmp_movs_coppel WITH NO LOG;

        
        SELECT 
            CASE
                WHEN idretailer IN ('7565819', '6310437', '7167537', '7926768', '1075847') AND codigoiso = '00' THEN  '1'
                WHEN idretailer IN ('7565819', '6310437', '7167537', '7926768', '1075847') AND codigoiso <> '00' THEN '2'
                WHEN idretailer IN ('1090447', '3260825') AND codigoiso = '00'  THEN '3'
                WHEN idretailer IN ('1090447', '3260825') AND codigoiso <> '00' THEN '4'
            END as grupo,
            COUNT(*) as cantidad
            FROM intercard:movimiento
                WHERE fechahorainauth BETWEEN pFechaInicio AND pFechaFin
                ...
                AND idretailer IN ('7565819', '6310437', '7167537', '7926768', '1075847', '1090447','3260825')
        GROUP BY 1
            ORDER BY 1 INTO TEMP tmp_movs_grupo WITH NO LOG;

        SELECT NVL(SUM(cantidad),0) as sum_cantidad_12 
                INTO vSumCantidad_12  
            FROM tmp_movs_grupo
                WHERE grupo IN ('1','2');
                
        SELECT NVL(SUM(cantidad),0) as sum_cantidad_34 
                INTO vSumCantidad_34 
            FROM tmp_movs_grupo
                WHERE grupo IN ('3','4');

        --- Iteracion indispensable para garantizar el mismo numero de registros en la tabla. 
        FOR vNumIteracion = 1 TO 4
            select grupo, cantidad
                INTO vGrupo, vCantidadTrxs
                from tmp_movs_grupo
                    WHERE grupo = vNumIteracion;
                    
                    IF (vNumIteracion = 1 OR vNumIteracion = 2) THEN
                        LET vPorcentaje = (vCantidadTrxs * 100) / vSumCantidad_12;
                    ELIF (vNumIteracion = 3 OR vNumIteracion = 4) THEN
                        LET vPorcentaje = (vCantidadTrxs * 100) / vSumCantidad_34;
                    END IF
                    
            
            IF( vNumIteracion == vGrupo  ) THEN
                INSERT INTO intercard:tbl_trxs_grupo_coppel  (t_grupo, t_cantidad_transacciones, t_porcentaje)
                    VALUES (vGrupo, vCantidadTrxs, vPorcentaje); 
            ELSE
                IF (vNumIteracion = 1 ) THEN
                    LET vGrupo = 2;
                ELIF (vNumIteracion = 2 ) THEN
                    LET vGrupo = 1;
                ELIF (vNumIteracion = 3 ) THEN
                    LET vGrupo = 3;
                ELIF (vNumIteracion = 4 ) THEN
                    LET vGrupo = 4;
                END IF;
                
                INSERT INTO intercard:tbl_trxs_grupo_coppel  (t_grupo, t_cantidad_transacciones, t_porcentaje)
                    VALUES ( vGrupo, 0, 0.00);
            END IF;
            
        END FOR

        LET vExecuteSQL = '';
        LET vExecuteSQL = 'echo "SET ISOLATION TO DIRTY READ; SET LOCK MODE TO WAIT 3; UNLOAD TO '||RUTA_ORIGEN||'trxs_grupo_coppel.txt'||
            ' SELECT t_grupo, t_cantidad_transacciones, t_porcentaje ' ||
            ' FROM intercard:tbl_trxs_grupo_coppel ' ||
            ' ORDER BY 1, 3 DESC; ' ||
            '" >'||RUTA_ORIGEN||'script_trxs_coppel.sql';
        SYSTEM vExecuteSQL;
        
        
        LET vExecuteSQL   = '';
        LET vExecuteSQL   = 'dbaccess intercard '||RUTA_ORIGEN||'script_trxs_coppel.sql';
        SYSTEM vExecuteSQL;
...
---Fecha de creacion: 10 de junio el 2019 14:00
