create procedure "informix".burofisicas_clon_pba()
       returning char(5),
                 char(50);
...
--jom fin
   ...
   define bmotivo integer ; --IPCB Abr15 -para identificar que le falta el segmento de dirección
...
--IPCB ValidaciÃ³n para Clve_obs 'EL'
...
DEFINE vidbaja  char(4);
  ...
  DEFINE vDia1  char(10); --- SOLO PRUEBAS
   ...
   LET vidbaja ="";
   LET bmotivo = 0;  --IPCB Abr15-para identificar que le falta el segmento de dirección
...
BEGIN
   ...
   LET iCP                     = 0;
   LET vsegmento_pe = ''; --MACF
   
 --SET DEBUG FILE TO "/informix/macf/burofisicas_clon.trc";
 --TRACE ON; 
...
--temporal para pruebas unicamente
   --let vfecha_hoy = mdy('03','31','2015');
   --let vfecha_corte = mdy('03','20','2015');
--temporal para pruebas unicamente
...
---CreaciÃ³n de indices por cada una de las validacines de SEPOMEX fmj
begin;
...
--- Genera registro encabezado para CÃ­rculo
       ...
       insert into br_burofisicas_clon values(vnumreg,vheader);    
    END IF;
...
--Se crea tabla temporal con los crÃ©ditos a procesar TDC
    SELECT c.numcte, c.num_producto, c.credito_externo,num_credito,status_cred, c.fecha_apertura,c.campo_trab3, 0 as plazo
      ...
      --AND c.sucursal IN ('0003','0008','0009','0036','0069','0092','0109','0137','0143','0146','0157','0220','0223','0282','0290','0293','0323','0326','0376','0379','0460') --- SOLO TEST MACF
      --AND c.sucursal = '0376'  --- SOLO TEST MACF
      --AND c.num_credito in ('600007691485','600007695015','600007737007')
    ...
    SELECT c.numcte, c.num_producto, mae.credito_externo,c.num_credito,mae.status_cred, mae.fecha_apertura,c.campo_trab3, 0 as plazo
      ...
      --AND c.sucursal IN ('0003','0008','0009','0036','0069','0092','0109','0137','0143','0146','0157','0220','0223','0282','0290','0293','0323','0326','0376','0379','0460') --- SOLO TEST MACF
      --AND c.sucursal = '0376' --- SOLO TEST MACF
      --and c.num_credito in ('600007691485','600007695015','600007737007') --- SOLO TEST MACF
    INTO temp creditos WITH NO LOG;
  
  	
--Se anexa a tabla temporal con los crÃ©ditos a procesar REESTRUCTURAS
    ...
    SELECT c.numcte, c.num_producto, c.credito_externo,c.num_credito,c.status_cred, c.fecha_apertura,c.campo_trab3, c.plazo
     ...
     WHERE c.empresa = "001"
       ...
       AND c.fecha = vfecha_hoy 
       --AND c.sucursal = '0376'  --- SOLO TEST MACF
       --and c.num_credito in ('610000674339','610000867917','610000984902')  --- SOLO TEST MACF 
...
--IPCB 06082014/Integra Cancelaciones aperturadas el mismo mes.
    ...
    SELECT a.numcte, a.num_producto,  '' credito_externo,b.num_credito, a.status_cred, a.fecha_apertura,a.campo_trab3, 0 as plazo
     ...
     WHERE a.status_cred IN ('FF','FI')
       ...
       --AND a.num_credito >= '';         
	     --AND a.sucursal IN ('0003','0008','0009','0036','0069','0092','0109','0137','0143','0146','0157','0220','0223','0282','0290','0293','0323','0326','0376','0379','0460') --- SOLO TEST MACF
	     --AND a.sucursal = '0376'  --- SOLO TEST MACF
	     --and a.num_credito in ('600007691485','600007695015','600007737007') --- SOLO TEST MACF
	...
	SELECT a.numcte, a.num_producto,  '' credito_externo,a.num_credito, status_cred, fecha_apertura,a.campo_trab3, a.plazo
	 ...
	 WHERE a.empresa = '001'
	   ...
	   AND a.num_credito NOT IN (SELECT num_credito FROM bdiburo:br_burofisicas_describe_clon)
       AND a.num_credito NOT IN (SELECT num_credito FROM bdiburo:br_burofisicas_concilia_clon)
       --AND status_cred = 'FF' RQM 09 343-0
...
-- INICIO SEGMENTO PN (Nombre)
    ...
    -- Agrega Apellido Materno
         if vapell_materno is not null and vapell_materno <> '' then
            ...
            let vsegmento1_pn = trim(vsegmento1_pn)||'0016NO PROPORCIONADO';  --MACF
         end if;
...
--IPCB Abr15 - Envio de tramas sin direccion, asignación de blando, para armar la trama
...
--IPCB Abr2015 -- Para integrar la etiqueta de delegación vacia al segmento en blanco
    ...
    -- Agregar origen del domicilio (pais) MACF
...
-- INICIA SEGMENTO PE (EMPLEO DEL CLIENTE) MACF

  --Etiqueta PE  Nombre o Razón Social del empleador
    ...
    END IF;
     ...
     let vsegmento_pe = 'PE' || lpad(length(trim(cnombre_empleador)),2,"0")  || trim(cnombre_empleador) || '1802MX';
     let tb_nombre_empleador = trim(cnombre_empleador);
     let tb_origen_razon_soc = 'MX';

-- TERMINA SEGMENTO PE (EMPLEO DEL CLIENTE) MACF
...
-- INICIA SEGMENTO TL(RESUMEN DE CREDITO)
    ...
    -- Se adelanta la obtenciÃ³n de saldos
...
--IPCB 12mar14 Se cambia la extracciÃ³ del saldo vencido para no duplicar el capital y considerar el iva  
...
--IPCB 26nov14: Agrega validaciÃ³n para  saldo_act si <= 0 entonces monto_pagar	=0 para crÃ©ditos con Baja	 
    ...
    -- Agregar Fecha de Ultimo Pago
         ...
         --if (vfechaultpago   is null) then let vfechaultpago   = date(1); end if;   -- MACF
         --if (vfechaultcompra is null) then let vfechaultcompra = date(1); end if;   -- MACF
    ...
    -- Agregar Saldo Actual
         ...
         if nvl(vsaldo_actual,'') = '' then --- MACF
    ...
    -- Agregar Forma de Pago
       -- UR = consideran crÃ©ditos que no tuvieron movimientos en el 
       --      mes de generaciÃ³n de la cinta (del dÃ­a 1 al Ãºltimo dÃ­a del mes que se reporta)
       -- 00 = se consideran crÃ©ditos que fueron aperturados en el mes de 
       --      generaciÃ³n de la cinta (del dÃ­a 1 al Ãºltimo dÃ­a del mes que se reporta)
       -- 01 = se consideran crÃ©ditos que estÃ¡n al corriente y que hayan tenido movimientos. 
       --      AsÃ­ mismo se incluyen los crÃ©ditos que fueron aperturados en el mes de generaciÃ³n 
       --      de la cinta (del dÃ­a 1 al Ãºltimo dÃ­a del mes que se reporta) y hayan tenido movimientos en ese mismo mes
   ...
   select status_cred, nvl(nvl(monto_vencido + mto_venc_trasp,0) + -- MONTO VENCIDO
    ...
    -- Agergar clave de observaciÃ³n
        ...
        --elif (vstatus_cred = 'FF') then RQM 09 343-0
               ...
               LET tb_clave_obs               = 'RV';
--IPCB 090913: Se agrega la asignaciÃ³n de la clave de observaciÃ³n 'PC'
        elif (vmop >= '02' and vmop <> 'UR') then
               LET vsegmento3_tl = TRIM(vsegmento3_tl)||'3002'||'PC';
               LET tb_clave_obs               = 'PC';
--IPCB27sep2013 Se agrega la asignaciÃ³n de la clave de observaciÃ³n 'EL'
...
--IPCB Marzo2015 --INICIO --Proceso trascodificación platino
        IF cNumProducto = '7000' and (vfecha_apertura >= vfecha_ini  and vfecha_apertura <= vfecha_hoy ) THEN
         let vsegmento3_tl = trim(vsegmento3_tl)||'41'||
             lpad(length(trim(cCredExterno)),2,"0")||trim(cCredExterno);
        ELSE
		 Let cCredExterno = '';
        END IF;
         let tb_num_credito_ext       = trim(cCredExterno);		
--IPCB Marzo2015 --FIN    --Proceso trascodificación platino
    -- NUEVOS CAMPOS
    ...
    --- PLAZO EN MESES (Plazo fijo)   MACF  dato es en días solo para Reestructura
       ...
       IF cNumProducto = '6011' THEN
         ...
         let tb_plazo_meses = cplazo_meses;    
      ELSE
         LET vsegmento3_tl  = TRIM(vsegmento3_tl)||'50040.00';
         let tb_plazo_meses = '0'; 
      END IF;

      --Monto del Crédito en la originación MACF
       IF cNumProducto <> '6011' THEN
          ...
          ELSE
              LET vsegmento3_tl  = TRIM(vsegmento3_tl)||'5109000000000';
              LET tb_monto_originacion = 0;
          END IF;
       ELSE       
          LET vsegmento3_tl  = TRIM(vsegmento3_tl)||'5109'||
...
--IPCB Abr15- Se modifica la validación para integrar a la cinta los registros marcados con CSS en la concilia .

        --- VALIDAR CON RICARDO SI ÉSTA PARTE SEGUIRÁ EXISTIENDO - MACF
       ...
       if bmotivo = 1 then
           INSERT INTO bdiburo:br_burofisicas_concilia_clon VALUES('001',cNumProducto,vnum_credito,'CSS',vfecha_hoy,
                    ...
                    tb_codigo_pais, tb_nombre_empleador, tb_origen_razon_soc, --MACF
                    ...
                    tb_saldo_venc, tb_cuotas_ven, tb_mop,tb_clave_obs, vrea_cal_cuota,vstatus_cred,tb_monto_insoluto,
                    trim(tb_plazo_meses), tb_monto_originacion, --MACF
                    tb_num_credito_ext);
       else
    --Se validan CPs para la cinta de BurÃ³
...
--IPCB 12mar14 - Se integran al insert vstatus_cred,cNumProducto
			insert into br_burofisicas_describe_clon
			   values (tb_num_credito, tb_apell_paterno, tb_apell_materno, tb_nombre1, tb_nombre2, tb_fecha_nac, tb_rfc, tb_nacionalidad, tb_estado_civil, tb_sexo,
					   ...
					   tb_clave_usu, tb_nombre_usu, tb_responsabilidad, tb_tipo_cuenta, tb_tipo_producto, tb_clave_monetaria, tb_num_pagos, tb_frecpago, tb_monto_pagar,
					   tb_fecha_apertura, tb_fecha_ult_pago, tb_fecha_ult_compra, tb_fecha_cierre, tb_fecha_reporte, tb_credito_maximo, tb_saldo_actual, tb_monto_otorgado,
					   tb_saldo_venc, tb_cuotas_ven, tb_mop,tb_clave_obs, vrea_cal_cuota,tb_fecha_vencimiento,tb_monto_insoluto,tb_ultimo_pago,vstatus_cred,cNumProducto,trim(tb_plazo_meses), tb_monto_originacion,tb_num_credito_ext);
...
--                       tb_saldo_venc, tb_cuotas_ven, tb_mop,tb_clave_obs, vrea_cal_cuota);
  ...
  SET LOCK MODE TO WAIT 3;
  BEGIN;  CREATE INDEX "informix".idx_br_burofisicas_clon          ON "informix".br_burofisicas_clon(numreg) ONLINE;  COMMIT;
  BEGIN;  CREATE INDEX "informix".idx_br_burofisicas_describe_clon ON "informix".br_burofisicas_describe_clon(num_credito) ONLINE;  COMMIT;
  BEGIN;  CREATE INDEX "informix".inxburoconcilia_clon             ON "informix".br_burofisicas_concilia_clon(empresa, num_producto, num_credito, motivo, fecha_cinta) ONLINE;  COMMIT;
...
--temporal se inhabilita solo para pruebas
  ...
  drop table sepomex; drop table creditos;     --- Comentar para pruebas solamente MACF
