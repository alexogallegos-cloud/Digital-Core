CREATE PROCEDURE "informix".sp_catalotipoconciliacionccl(pUsuario CHAR(8), pIdFuncion CHAR(10))
		RETURNING CHAR(5) AS codret,
			INTEGER AS cIdConciliacion,
			CHAR(12) AS cDescConciliacion;
			
	DEFINE cCodRet CHAR(5);
	DEFINE iSqlErr INTEGER;
	DEFINE cIdConciliacion INTEGER;
	DEFINE cDescConciliacion CHAR(12);
	DEFINE iNoRegistros INTEGER;
	
	LET cCodRet = '00000';
	LET iSqlErr = 0;
	LET cIdConciliacion = 0;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, cIdConciliacion, cDescConciliacion;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_catalotipoconciliacionccl.out';
		--TRACE ON;
		...
		FOREACH
			SELECT id_conciliacion,desc_conciliacion 
			INTO cIdConciliacion,cDescConciliacion 
			FROM bdicnweb:"informix".sw_ccl_tipoconciliacion
			
			LET iNoRegistros = iNoRegistros + 1;
			RETURN cCodRet, cIdConciliacion, UPPER(cDescConciliacion) WITH RESUME;
		END FOREACH;
...
'DESCRIPCION: SPL que consulta el detalle de la tabla bdicnweb:sw_ccl_tipoconciliacion.',
'BD: bdicnweb';

CREATE PROCEDURE "informix".sp_catalotipoproductoccl(pUsuario CHAR(8), pIdFuncion CHAR(10))
		RETURNING CHAR(5) AS codret,
			INTEGER AS cIdProducto,
			CHAR(25) AS cDescProducto;
			
	DEFINE cCodRet CHAR(5);
	DEFINE iSqlErr INTEGER;
	DEFINE cIdProducto INTEGER;
	DEFINE cDescProducto CHAR(25);
	DEFINE iNoRegistros INTEGER;
	...
	BEGIN
		...
		--SET DEBUG FILE TO '/tmp/mfinis/sp_catalotipoproductoccl.out';
		--TRACE ON;
		...
		FOREACH
			...
			FROM bdicnweb:"informix".sw_ccl_tipoproducto
			
			LET iNoRegistros = iNoRegistros + 1;
			RETURN cCodRet, cIdProducto, UPPER(cDescProducto) WITH RESUME;
		END FOREACH;
...
'MODULO: CONCILIACIONES',
'FUNCIONALIDAD: Conciliación Saldos y Movimientos de Crédito', 
'DESCRIPCION: SPL que consulta el detalle de la tabla bdicnweb:sw_ccl_tipoproducto.',
'BD: bdicnweb';
