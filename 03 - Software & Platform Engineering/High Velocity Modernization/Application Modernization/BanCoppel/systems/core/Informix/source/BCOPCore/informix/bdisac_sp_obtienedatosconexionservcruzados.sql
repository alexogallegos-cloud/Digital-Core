CREATE PROCEDURE "informix".sp_obtienedatosconexionservcruzados()
RETURNING CHAR(5), CHAR(15), CHAR(5), CHAR(5), CHAR;
    ...
    DEFINE cIpServidor      CHAR(15);
    DEFINE cPtoEncriptado   CHAR(5);
    DEFINE cPtoNoEncriptado CHAR(5);
    DEFINE cFlagEncripta    CHAR;

    --SET DEBUG FILE TO "/tmp/conexion.out";
    --TRACE ON;
    ...
    BEGIN
        ON EXCEPTION SET iSqlErr, iIsamErr, cInfoErr
            IF iSqlErr <> 0 THEN
                LET cCodRet = iSqlErr;
                EXECUTE PROCEDURE bdisac:sp_sac_guardaMensajeError(iSqlErr, iIsamErr, cInfoErr, "sp_obtieneDatosConexionServCruzados");
                RETURN cCodRet, "", "", "", "";
            END IF;
        END EXCEPTION;
        ...
        RETURN cCodRet, cIpServidor, cPtoEncriptado, cPtoNoEncriptado, cFlagEncripta ;
    END;
 END PROCEDURE
 DOCUMENT
 'AUTOR: Jose Angel Lopez Adams',
 'DESCRIPCION: Se encarga de obtener los parametros de la conexion del servicio SOA que se comunica con coppel',
 'EJECUTADO O LLAMADO POR: BCOFI0020125.DLL (Pagos Coppel)',
 'FECHA: 20081005',
 'BD:   bdisac';

CREATE PROCEDURE "informix".sp_reportebts_conciliacion ( pdtFechaIni DATE, pdtFechaFin DATE )

RETURNING
	DATE 	  AS Dia,
	CHAR (20) AS Forma_Pago,
	MONEY 	  AS Cargo_Pago_Remesa,
	MONEY 	  AS Saldo_Cuenta_BTS,
...
--****************************************************************************************************
-- DESCRIPCION:  GENERA REPORTE DE CONCILIACION BTS
...
DEFINE vmCargo_Pago_Remesa 	MONEY;
DEFINE vmSaldo_Cuenta_BTS 	MONEY;
DEFINE vsFolio_Operacion 	CHAR (16);
DEFINE vsClave_Envio 		CHAR (11);
DEFINE vsSucursal 			CHAR (4);
DEFINE vsCajero 			CHAR (8);
DEFINE vsImportePago 		MONEY ;
DEFINE vsImportePago_old    MONEY ;
DEFINE vsMontoTotal         CHAR (20);
DEFINE visqlerr 			INTEGER ;

/* INICIALIZACION DE VARIABLES */
LET vdtDia 				= CURRENT;
LET vsForma_Pago 		= '';
LET vmCargo_Pago_Remesa = 0.0;
LET vmSaldo_Cuenta_BTS 	= 0.0;
LET vsFolio_Operacion 	= '';
LET vsClave_Envio 		= '';
LET vsSucursal 			= '';
LET vsCajero 			= '';
...
--SET DEBUG FILE TO "/informix/EPG/sp_reportebts_conciliacion.out";
--	TRACE ON;
...
BEGIN
	ON EXCEPTION SET visqlerr   --cacha el error en caso de que exista y regresa un valor predeterminado
		RETURN '01/01/1900', visqlerr, 0.0, 0.0, '', '', '', '', '';
		DROP TABLE bdisac:servicios;
		DROP TABLE bdisac:movtos_chq;
		DROP TABLE bdisac:movtos_chq_old;
	END EXCEPTION;
	
	IF ((pdtFechaIni IS NULL) OR (pdtFechaFin IS NULL)) THEN
	...
	ELSE
		...
		WHERE fech_alt BETWEEN pdtFechaIni AND pdtFechaFin
			AND transacc IN ('1140', '1110')
			AND folio_suc IN(SELECT folio_suc FROM sac_movimientoshistorial 
							WHERE numcategoria='07' AND numconvenio='004' 
							AND fecha_pago BETWEEN pdtFechaIni AND pdtFechaFin
		...
		LET vsMontoTotal = CAST(vsImportePago + vsImportePago_old AS CHAR (20));
		...
		INTO vdtDia, vsForma_Pago, vmCargo_Pago_Remesa, vmSaldo_Cuenta_BTS, vsFolio_Operacion, vsClave_Envio, vsSucursal, vsCajero	
		...
		WHERE   servicios.numcategoria = '07' AND servicios.numconvenio = '004'
			AND mvs_chq.fech_alt BETWEEN pdtFechaIni AND pdtFechaFin
		...
		FROM bdisac:sac_movimientoshistorial servicios,  bdicheq:sc_movhis_old mvs_chq_old
			WHERE   servicios.numcategoria = '07' AND servicios.numconvenio = '004'
			AND mvs_chq_old.fech_alt BETWEEN pdtFechaIni AND pdtFechaFin
			...
			RETURN 	vdtDia, vsForma_Pago, vmCargo_Pago_Remesa, vmSaldo_Cuenta_BTS, vsFolio_Operacion, vsClave_Envio, vsSucursal, vsCajero, vsMontoTotal  WITH RESUME;	
		END FOREACH;
    END IF;
END
END PROCEDURE
...
'Proyecto: BTS',
'Solicito: ',
'Descripcion: GENERA REPORTE DE TBS DIARIO.',
'Fecha: 2010/11/18',
'Version: 20101118.1500',
'BD: BDISAC',
'AUTOR: Edgar Ivan Rochin Rocha',
'MODIFICACION: Se modifica para que no muestre el abono a la cuenta del cliente.',
'Fecha: 25/05/2011',
'VERSION: 20110525.1747',
'AUTOR: Angel Rene de la Llave',
'MODIFICACION: Se modifica para que contemple la sc_movhis_old ademas de optimizar la busqueda.',
'Fecha: 25/05/2011',
'VERSION: 20110525.1747',
'AUTOR: Eduardo Pineda Guzmán',
'MODIFICACION: Optimizar la busqueda por indices.',
'Fecha: 23/01/2014',
'VERSION: 20110525.1747';
