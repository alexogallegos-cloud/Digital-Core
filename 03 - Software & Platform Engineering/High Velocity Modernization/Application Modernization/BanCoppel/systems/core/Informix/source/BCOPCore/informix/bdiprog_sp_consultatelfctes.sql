CREATE PROCEDURE "informix".sp_consultatelfctes(p_NumCte CHAR(20), p_CveCuenta CHAR(2), p_Registros SMALLINT)
RETURNING
     CHAR(6), ---cod_ret
	 CHAR(20), ---cuenta	 
     CHAR(20), ---desc cuenta
...
--##Creó		 : Walber Castro
--##Fecha        : 30/09/2010
--##Descripcion  : Se crea con el proposito de consultar los telefonos frecuentes
--##Arturo Cruz 03/02/2011: Se modifica valor cve_banco para que sea igual a 201
--##############################################################################
...
BEGIN
   ...
   ON EXCEPTION
	...
	--SET DEBUG FILE TO "/tmp/sp_consultatelfctes.out";
	--TRACE ON;
	...
	IF (p_NumCte <> "" AND p_NumCte IS NOT NULL) AND (p_CveCuenta <> "" AND p_CveCuenta IS NOT NULL)  THEN
 		IF EXISTS (SELECT ct.cuenta FROM bdiprog: pp_ctasterceros ct WHERE ct.num_cte = p_NumCte AND ct.cve_cuenta = p_CveCuenta)  THEN
            IF TRIM(p_CveCuenta) = '05' THEN
				FOREACH
                    SELECT ct.cuenta, ct.descrip_cta, ct.digito_ver, ct.canal_alta, ct.fecha_insert, ct.hora_insert
                    INTO v_CtaDestino,v_DescCta, v_Digito, v_Canal, v_FechaInsert, v_HoraInsert
					...
					ELSE													
						...
						END IF;						
					END IF;
					RETURN v_cod_ret, v_CtaDestino,v_DescCta,v_Digito,v_Inhabil WITH RESUME;
				END FOREACH;	
				LET v_Inhabil = '';
                FOREACH
                    SELECT ct.cuenta, ct.descrip_cta, ct.digito_ver, ct.canal_alta, ct.fecha_insert, ct.hora_insert
