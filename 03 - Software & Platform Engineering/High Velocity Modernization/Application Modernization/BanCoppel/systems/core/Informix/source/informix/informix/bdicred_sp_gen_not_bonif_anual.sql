CREATE PROCEDURE "informix".sp_gen_not_bonif_anual()
RETURNING CHAR(5);
...
DEFINE c_codigo_respuesta       CHAR(5);
DEFINE i_sql_err                SMALLINT;
DEFINE i_sam_err                SMALLINT;
DEFINE c_error_info             CHAR(40);
DEFINE v_nombre                 VARCHAR(150);
DEFINE i_porcetanje             INTEGER;
DEFINE i_porcetanje_push        INTEGER;
DEFINE d_importe_total          DECIMAL(15,2);
DEFINE c_id_plantilla 	        CHAR(12);
DEFINE c_id_msj 			    CHAR(10);
DEFINE c_id_plantilla_push      CHAR(12);
DEFINE c_id_msj_push 		    CHAR(10);
DEFINE d_fecha_hoy              DATE;
DEFINE i_anio_curso             INTEGER;
DEFINE i_num_mes_curso          INTEGER;
DEFINE c_nombre_mes_curso       CHAR(10);
DEFINE d_monto_facturacion      DECIMAL(15,2);
DEFINE c_num_producto           CHAR(4);
DEFINE c_correo                 CHAR(20);
DEFINE c_sexo                   CHAR(1);
DEFINE c_num_credito            CHAR(20);
DEFINE c_num_cte                CHAR(20);
DEFINE c_codigo_respuesta_email CHAR(5);
DEFINE c_codigo_respuesta_push  CHAR(5);
DEFINE d_fecha_anualidad        DATE; 
DEFINE i_anio_anualidad         INTEGER; 
DEFINE i_mes_anualidad          INTEGER;
DEFINE d_comision_anualidad     DECIMAL(15,2);
DEFINE c_cod_anualidad          CHAR(4);
DEFINE b_facturacion            BOOLEAN;
DEFINE b_bonificacion           BOOLEAN;
DEFINE i_meses_para_anualidad   INTEGER;
DEFINE i_cont_commit            INTEGER;
DEFINE i_commit                 INTEGER;
DEFINE i_notifico               INTEGER;
-- *************************************************************************
-- *                        ASIGNACION DE VARIABLES 
-- **************************************************************************
LET c_codigo_respuesta      ="00000";
LET v_nombre                ="";
LET i_porcetanje            =0;
LET i_porcetanje_push       =0;
LET d_importe_total         =0.0;
LET i_cont_commit           =0;
LET c_correo                ='';
LET c_id_msj                ='PORTAL_BPI'; 
LET c_id_msj_push           ='PNS_BEX';

LET d_monto_facturacion     =0;
LET c_num_producto          ="";
LET d_comision_anualidad    =0;
LET d_fecha_hoy             =DATE(1);
LET i_anio_curso            =0;
LET i_num_mes_curso         =0;
LET i_commit                =0;
LET c_num_cte               ="";
LET c_num_credito           ="";
LET v_nombre                ="";
LET d_fecha_anualidad       =DATE(1);
LET i_anio_anualidad        =0;
LET i_mes_anualidad         =0;
LET i_porcetanje            =0;
LET d_importe_total         =0;

    BEGIN 
        ON EXCEPTION SET i_sql_err, i_sam_err, c_error_info
            IF i_sql_err <> 0 THEN
                IF i_sql_err = -535 OR i_cont_commit<>0 THEN
                    COMMIT WORK;
                ELIF i_sql_err=-958 THEN
		            DROP TABLE bdicred:tmpcreditos_bon;
                END IF;
                LET c_codigo_respuesta = "11111";
               
                RETURN c_codigo_respuesta;
            END IF;
        END EXCEPTION;

        --SET DEBUG FILE TO '/home/e97948373/sp_gen_not_bonif_anual.out';
        --TRACE ON; 
        ...
        SELECT fecha_hoy,YEAR(fecha_hoy),MONTH(fecha_hoy) 
        INTO d_fecha_hoy,i_anio_curso,i_num_mes_curso 
        FROM bdicred:sd_fechas 
        WHERE empresa = '001';

        -- SE IDENTIFICA EL NOMBRE DEL MES ACTUAL         
        LET c_nombre_mes_curso = 
        CASE i_num_mes_curso
            WHEN 1 THEN 'enero'
            WHEN 2 THEN 'febrero'
            WHEN 3 THEN 'marzo'
            WHEN 4 THEN 'abril'
        ...
        SELECT 
            cliente AS cliente,credito AS credito,num_producto AS num_producto,nombre AS nombre,fecha_cobro AS fecha_cobro,
            porcentaje AS porcentaje,importe_total AS importe_total,notif AS notificado,monto_facturacion AS monto_facturacion,
            substr(cod_anualidad,1,4) AS cod_anualidad
        FROM bdicred:sd_rpt_bonificacion
        WHERE notif IN (0,1,2)
        INTO TEMP tmpcreditos_bon WITH NO LOG;
        

        FOREACH WITH HOLD
            SELECT rpt.cliente,rpt.credito,rpt.nombre,rpt.fecha_cobro,YEAR(rpt.fecha_cobro),MONTH(rpt.fecha_cobro),
                rpt.porcentaje,rpt.importe_total, rpt.notificado,rpt.monto_facturacion, NVL(t.monto,0),rpt.num_producto 
            INTO c_num_cte, c_num_credito,v_nombre,d_fecha_anualidad,i_anio_anualidad,i_mes_anualidad,i_porcetanje,d_importe_total, i_notifico,d_monto_facturacion,d_comision_anualidad,c_num_producto
            FROM bdicred:tmpcreditos_bon rpt
            INNER JOIN bdicred:sd_tpcomis t
            ON (t.cod_comis = rpt.cod_anualidad
                AND t.empresa = "001") 

                IF i_cont_commit = 0 THEN
                    BEGIN WORK;
                END IF;

                LET b_bonificacion = 'f';
                LET b_facturacion = 'f';
                LET i_meses_para_anualidad = 0;
                IF i_porcetanje < 100 THEN
                    LET b_facturacion = 't';
                ELSE
                    IF i_notifico = 0 OR (i_notifico >= 1 AND i_mes_anualidad = i_num_mes_curso) THEN
                        LET b_bonificacion = 't';
                    END IF;
                END IF;

                    
                IF b_facturacion THEN
                    --CALCULAR LOS MESES FALTANTES
                    LET i_meses_para_anualidad = i_num_mes_curso - i_mes_anualidad;
                    IF i_meses_para_anualidad > 0 THEN
                        LET i_meses_para_anualidad = i_meses_para_anualidad -12;
                    ELIF i_meses_para_anualidad = 0 AND i_anio_anualidad > i_anio_curso THEN
                        LET i_meses_para_anualidad = 12;
                    END IF;
                    LET i_meses_para_anualidad = ABS(i_meses_para_anualidad);
                    --ALIMENTAR VARIABLES DE NOIFICACIONES EMAIL
                    IF c_num_producto='5400' THEN
                        LET c_id_plantilla = 'ML_INF_FA_AT';
                    ELSE
                        LET c_id_plantilla = 'ML_TDC_FA_AT';
                    END IF
                    --ALIMENTAR VARIABLES DE NOIFICACIONES PUSH
                    IF i_porcetanje >= 80 THEN
                        --Notificacion push tipo 1
                        IF c_num_producto='5400' THEN
                            LET c_id_plantilla_push = 'NP_INF_FA_AC';
                        ELSE
                            LET c_id_plantilla_push = 'NP_TDC_FA_AC';
                        END IF;
                        LET i_porcetanje_push = d_monto_facturacion - d_importe_total;
                    ELSE    
                        --Notificacion push tipo 2
                        LET i_porcetanje_push = i_porcetanje;
                        LET c_id_plantilla_push = 'NP_INF_FA_AP';
                    END IF;
                ELIF b_bonificacion THEN
                    --ALIMENTAR VARIABLES DE NOIFICACIONES EMAIL
                    IF c_num_producto='5400' THEN
                        LET c_id_plantilla = 'ML_INF_BO_AN';
                        LET c_id_plantilla_push = 'NP_INF_BO_AN';
                    ELSE
                        LET c_id_plantilla = 'ML_TDC_BO_AN';
                        LET c_id_plantilla_push = 'NP_TDC_BO_AN';
                    END IF;
                    LET d_comision_anualidad = d_monto_facturacion;
                    --ALIMENTAR VARIABLES DE NOIFICACIONES PUSH
                    
                    UPDATE bdicred:sd_gastos_bonificacion
                    SET notif=1
                    WHERE empresa = '001' 
                        AND num_credito = c_num_credito
                        AND fecha_registro = d_fecha_anualidad
                        AND anio_registro IN (i_anio_anualidad, (i_anio_anualidad-1));

                END IF;
                IF b_facturacion OR b_bonificacion THEN

                    SELECT sexo
                    INTO c_sexo
                    FROM bdinteg:si_ctepf
                    WHERE numcte = c_num_cte
                        AND empresa='001';
                                                                                              
                    EXECUTE PROCEDURE bdimnsj:sp_registra_evento(2,c_id_msj, c_id_plantilla,c_num_cte, c_num_credito,'', '1', v_nombre, d_comision_anualidad, d_monto_facturacion, c_nombre_mes_curso, i_porcetanje, i_meses_para_anualidad, '', '', '', c_sexo, c_correo, '', 0, 0, 0, 0, 0,current,current)
                    INTO c_codigo_respuesta_email;
                        
                    --RETURN c_codigo_respuesta_email WITH RESUME;
                    EXECUTE PROCEDURE bdimnsj:sp_registra_evento(2,c_id_msj_push, c_id_plantilla_push,c_num_cte, c_num_credito,'', '1', i_porcetanje_push, '', '', '', '', '', '', '', '', '', '', '', 0, 0, 0, 0, 0,current,current)
                    INTO c_codigo_respuesta_push;
                        
                    --RETURN c_codigo_respuesta_push WITH RESUME;
                END IF;
                    
                DELETE FROM bdicred:sd_rpt_bonificacion
                WHERE cliente = c_num_cte 
                    AND credito = c_num_credito;
                    

                LET i_cont_commit = i_cont_commit + 1;
                IF  i_cont_commit = i_commit THEN
                    COMMIT WORK;
                    LET i_cont_commit = 0;     
                END IF;
...
DOCUMENT 
'----------------------------------------------------------------------------',
'RQM         : RQM 10 1669 INCREMENTALES TDC Notificaciones Automaticas de Bonificacion',
'Descripcion : Generar la notificacion de gastos que han tenido los clientes anualmente de acuerdo a si cumple o no con la bonificacion.',
'Creacion    : Arturo Astorga',
'Fecha       : 25/03/2024',
'BD          : BDICRED',
'Solicito    : Maria de los Angeles Yedra',
