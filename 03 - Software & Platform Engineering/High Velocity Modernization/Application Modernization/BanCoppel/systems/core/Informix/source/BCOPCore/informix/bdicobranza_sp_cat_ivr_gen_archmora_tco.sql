CREATE PROCEDURE "informix".sp_cat_ivr_gen_archmora_tco(pempresa CHAR(3), 
						                                        pfechacorte DATE,
                                                    ptipocobranza char(1))
RETURNING CHAR(6);
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Creado por: Abrham Lopez L Fecha: 16/05/2016. 
-- Descripcion: Proceso para la generación del archivo para campaña administrativa para los clientes con mora 1 en su TDC Oro
-- execute procedure "informix".sp_cat_ivr_gen_archmora_tco('001', '05-10-2016','A');
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
--A.L.L. OBTENEMOS LA RUTA DEL ARCHIVO /resplogifx/archivoscartera/ 
	SELECT TRIM(valor_alfabetico)
        INTO cruta
        FROM bdicobranza:cb_param_campania
        WHERE empresa = pempresa
...
/*	select count(*) into vcount
	...
	where a.empresa = pempresa                  
		...
		CALL bdicobranza:"informix".sp_latinia_contador_cobranza('IVR_MORAO1',vcount,null) RETURNING cCod_ret;
...
/*	select count(*) into vcount
	...
	CALL bdicobranza:"informix".sp_latinia_contador_cobranza('IVR_MORAO2',vcount,null) RETURNING cCod_ret;
