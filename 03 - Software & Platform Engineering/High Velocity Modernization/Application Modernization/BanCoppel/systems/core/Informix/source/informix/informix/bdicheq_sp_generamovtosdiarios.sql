CREATE PROCEDURE "informix".sp_generamovtosdiarios( pCuenta CHAR(20), 
                                                    pSucursal CHAR(4), 
                                                    pMonto MONEY(18,2), 
                                                    pUsuario CHAR(8), 
                                                    pNumRegistro INTEGER)

RETURNING CHAR(5),CHAR(16), CHAR(50), MONEY(18,2), CHAR(1), CHAR(16),CHAR(40);
    ...
    ON EXCEPTION SET vSqlErr
        IF vCodRet != 0 THEN
            LET vCodRet = vSqlErr;
            RETURN vCodRet,cFolio, cTransacc, mMonto, cReversion, cTarjeta, cReferencia;
        END IF;
    END EXCEPTION;
    ...
    --- SET DEBUG FILE TO "/tmp/sp_generamovtosdiarios.out";
    --- TRACE ON;
    ...
    IF EXISTS(SELECT cuenta 
        ...
        FOREACH WITH HOLD
            Select SKIP pNumRegistro FIRST 20
                   NVL(folio_suc, " "),
                   NVL(TRIM(transacc||' '||TRIM(descripcion)), " "),
                   NVL(monto_tot, 0), 
                   NVL(cancelad, " "), 
                   NVL(num_tarjeta, " "), 
                   NVL(TRIM(referencia), " ")						
              INTO cFolio, cTransacc, mMonto, cReversion, cTarjeta, cReferencia
              FROM bdicheq:sc_movdia d
             INNER JOIN Bdinteg: si_transacc t ON t.numero = d.transacc and t.sistema = "01"
             WHERE d.empresa = "001" 
               AND d.cuenta = pCuenta
               AND d.monto_tot = CASE WHEN pMonto = 0 THEN monto_tot  ELSE pMonto END
               AND d.sucursal = CASE WHEN pSucursal = "" THEN sucursal  ELSE pSucursal END
               AND d.usuario = CASE WHEN pUsuario = "" THEN usuario  ELSE pUsuario END
             ORDER BY d.fech_alt DESC, d.num_serial DESC

            IF cFolio = "" Or cFolio IS NULL THEN
                LET vCodRet = "20000";
                RETURN vCodRet,cFolio, cTransacc, mMonto, cReversion, cTarjeta, cReferencia;
            END IF;

            RETURN vCodRet,cFolio, cTransacc, mMonto, cReversion, cTarjeta, cReferencia WITH RESUME;
...
'DESCRIPCION: Procedimiento que obtiene los movimientos diarios de una cuenta, con filtros por sucursal, monto',
' y no.empelado',
'VERSION: 20090414.1748',
'Bd: Bdicheq';
