CREATE PROCEDURE "informix".sp_obtiene_productos_cjunk_club2_web(pEmpresa 		CHAR(3),	-- Codigo de empresa
																...
																pIdentificacion CHAR(1),	-- Indica si el cte digitaliza una identificacion oficial
																pOfertaProdCred CHAR(1))	--Indica si oferta o no productos de credito)
	RETURNING 	CHAR(5), -- Codigo de retorno
				CHAR(4), -- C de producto a ofrecer al cte
	...
	--Modificacion: Se realiza homologacion con fuentes modificados por bancoppel: se agrega validacion para no permitir levantar una nueva solicitud de Prestamo cuando el cliente cuente	
	--              con una solicitud de Prestamo personal o de TDC Bancoppel Visa en estatus ?RT?. Tambien se agrega validacion para no permitir levantar una nueva solicitud de Prestamo
	...
	--Fecha: 01-09-2014
	-------------------------------------------------------------------------------------
	--Peticion: RQM 10 888 Limite de apertura de cuentas de captacion
	--Folio: 381
	--Autor: 97893323 Judith Moreno
	--Fecha: 07/03/2018
	--Modificacion: Se modifica para validar que si el solicitante sobrepasa el limite de cuentas de captacion, dado por el sp_limite_cuentas
	--				no se permita ofertar cuentas de captacion, unicamente inversion creciente y pagara
	--Sustento: basado en el requerimiento 5_RQM 10 888 Limite de apertura
	--Solicita: Abraham Narvaez",
	--BD: bdisolic"
	-------------------------------------------------------------------------------------
	-------------------------------------------------------------------------------------
	--Peticion: RQM 10 960 Tarjeta de Credito Grupo Coppel
	--Autor: 96686359 Gutberto Gomez Guadarama
	--Fecha: 15/01/2019
	--Modificacion: Se modifica para validar que si el solicitante tiene producto 1300 con apertura de dos aÃÂ±os mas para poder ofrecer el producto 8500
	--BD: bdisolic"
	-------------------------------------------------------------------------------------
...
--jom-26/11/2018
	...
	--SET DEBUG FILE TO "/informix/sp_obtiene_productos_cjunk_club2.sql";
	--TRACE ON;
...
---RGH 18122018
		...
		END IF;            
---RGH 18122018
	 select NVL(count(numcte),0) into iTotSolWeb from bdisolic:ss_prospecteo_solicitudes
        where numcte=pNumcte--'000022643' 
            and canal_sol=4
		...
		IF pSucursal <> '8503' AND iTotSolWeb=0 THEN	--- Sucursal WEB, omitir flujo de prospecteo para solicitudes WEB
		   --EJECUTANDO EL SP DE PROSPECTEO--
			FOREACH
				execute procedure bdisolic: sp_obtiene_productos_prospecteo(pEmpresa, pSucursal, pEjecutivo, pPuesto_local, pNumcte, pCoppel, pPrecalCoppel, pPrecalBco, pDigiDomicilio, pIdentificacion, pOfertaProdCred)
...
-- Validacion ife/ine ini
            ...
            IF (select COUNT(numcte) from bdinteg:"informix".si_ctepf where numcte = pNumcte and  codidentifi = 'A') > 0 THEN
...
-- EM 2017/05/25
		...
		---I---RQM 10 960 TDC GC
		--cliente: Se valida la fecha alta de la cuenta de nomina (Producto 1300), de no tener o de ser menor a dos aÃÂ±os no se ofrece producto 8500.
        	SELECT fecha_hoy
			INTO dFechaHoy
		...
		SELECT numcte_pros INTO cCteProsp FROM bdiprospectos:"informix".pr_cliente WHERE empresa = pEmpresa AND numcte = pNumcte;
			...
			--Si bandera_os esta activa se aplica vigencia de la OS calle, si la bandera_os estara inactiva no se aplicara la vigencia de la OS calle 
			IF (SELECT COUNT(num_producto) FROM bdicred:"informix".sd_definicion WHERE bandera_os='1') > 0 THEN		
				FOREACH
		...
		IF NVL(cCteProsp,'') <> '' THEN
			...
			IF NVL(cClave,'') = 'R' THEN
				...
				FOREACH
					...
					IF (SELECT COUNT(*) FROM bdisolic:"informix".ss_productos_ofrecer 
		...
		IF inumpPrestamos > 0 THEN -- existe al menos un credito flexible aun activo sin importar el status FF (liquidado).
			UPDATE "informix".ss_productos_ofrecer SET aplica = 'N' WHERE cliente = pNumcte AND sucursal = pSucursal AND ejecutivo = pEjecutivo AND tp_solicitud_ofr = 'P' AND producto_ofr = '6800';
		END IF;
		...
		FOREACH
			...
			IF cprod_final  = '6400' THEN 
				IF (SELECT COUNT(*) FROM bdisolic:"informix".ss_producto_credcap WHERE num_producto = cprod_final AND meses_alta IS NOT NULL) > 0 THEN
