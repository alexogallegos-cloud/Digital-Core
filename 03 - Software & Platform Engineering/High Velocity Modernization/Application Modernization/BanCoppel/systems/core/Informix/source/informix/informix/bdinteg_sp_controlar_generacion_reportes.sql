CREATE PROCEDURE "informix".sp_controlar_generacion_reportes()
	RETURNING 
	CHAR(6) AS estatus, 
	CHAR(100) AS mensaje;
...
--ASIGNACION DE VARIABLES
	...
	LET cEvento	= 'OBTENCION DE INDICADORES ACTIVOS';
		
--ASIGNACION DE VARIABLES ERROR
	LET cCodRet = '000000';
	LET cVarDataErr = 'PROCESO TERMINADO,REPORTES GENERADOS SATISFACTORIAMENTE';
	
	--SET DEBUG FILE TO '/tmp/josea/reportesdomi/sp_controlar_generacion_reportes.out';
	--TRACE ON;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr, iSamErr, cVarDataErr
			IF iSqlErr <> 0 THEN
				LET cCodRet=iSqlErr;
					...
					VALUES (NVL(dFechaProceso,CURRENT), cProceso, cEvento, cCodRet, cVarDataErr);				
		...
		LET iDia  = DAY(dFechaProceso);
		        
        ---IF FECHAS	
        ---VERIFICAR PERIODO DE GENERACION DE REPORTES (DEL 02 AL 05 DE CADA MES) BRENDA KARELI CAMARGO PRECIADO 08/01/2016		
		IF iDia  NOT IN (2,3,4,5) THEN
			LET cCodRet = '000000';
	        LET cVarDataErr = 'PROCESO TERMINADO, EJECUTADO FUERA DEL PERIODO ESTABLECIDO.';
		   RETURN cCodRet,cVarDataErr;
		ELSE
			--IF EXISTS (SELECT 1 FROM "informix".si_proc_indicadores WHERE estatus_proceso='A' AND tipo = 'RP') THEN
			SELECT count(*) INTO vconta 
			  FROM "informix".si_proc_indicadores WHERE estatus_proceso='A' AND tipo = 'RP';
			IF vconta > 0 THEN   
			   FOREACH WITH HOLD
				  SELECT tipo,identificador,nombre_proceso
				  INTO cTipo,cIdentificador,cNombreProceso
				  ...
				  IF cIdentificador <> '' THEN
					 LET cProceso = 'GENERANDO REPORTES';
					 LET cEvento	= 'LLAMADO DEL SP sp_generar_reporte_domi_pp_tdc_servicios';
					 SET ISOLATION TO DIRTY READ;
					 SET LOCK MODE TO WAIT 3;
					
					 EXECUTE PROCEDURE "informix".sp_generar_reporte_domi_pp_tdc_servicios(dFechaProceso,cIdentificador,cTipo)
					 INTO cCodRetSP,cMensajeRet;
				
					 IF cCodRetSP <> '000000' THEN 
						LET cVarDataErr='PROCESO TERMINADO, UNO O MAS REPORTES NO SE GENERARON CORRECTAMENTE';
						INSERT INTO "informix".si_log_indicadores_sucursal (fecha, proceso, evento, cod_error, mensaje)
						VALUES (dFechaProceso, cProceso, cEvento, cCodRetSP, cNombreProceso);
					 END IF;
						
				  END IF;
			   END FOREACH;
			   
			  LET cEvento	= 'PROCESO TERMINADO';
			  INSERT INTO "informix".si_log_indicadores_sucursal (fecha, proceso, evento, cod_error, mensaje)
			  VALUES (dFechaProceso, cProceso, cEvento, cCodRet, cVarDataErr);

			ELSE		   
			  LET cVarDataErr = 'PROCESO TERMINADO, NO EXISTEN REPORTES ACTIVOS'; --NO HAY RESULTADOS PARA LA CONSULTA
			  INSERT INTO "informix".si_log_indicadores_sucursal (fecha, proceso, evento, cod_error, mensaje)
			  VALUES (dFechaProceso, cProceso, cEvento, cCodRet, cVarDataErr);
		   END IF;
...
'FOLIO: 1511-ImpNumCteConSinDomiciliacion',
'DESCRIPCION: ejecuta el sp_generar_reporte_domi_pp_tdc_servicios para generarar los reportes tomando en cuenta el estatus_proceso=A y tipo=RP  de la tabla si_proc_indicadores ',
'FECHA: 02-12-2015 ',
'SUSTENTO: Se define con Jose Angel LÃ?Â³pez Adams en el requerimiento',
'RQM 10 693 - Analisis tÃ?Â©cnico.pdf  ',
'BD: BDINTEG';

create procedure "informix".consnomb1(pempresa char(3),
                          ppaterno char(26),
                          pmaterno char(26),
	                  prazon char(60),
                          pno_rfc char(13),
...
if prazon is not null and prazon !="" then
   foreach
      select razon_social,numcte,rfc, rfc_alterno
 	 into v_razon_soc,v_numcte,v_rfc, v_rfc_alterno
      	 from si_cliente
      	 where apell_paterno = " " AND apell_materno = " " and razon_social = prazon
      	 order by numcte
   ...
   if pno_rfc is not null and pno_rfc != "" then
      foreach
         select nombre1,nombre2,apell_paterno,apell_materno,pf.numcte,rfc, rfc_alterno
	 ...
	 return v_cod_ret,v_nombre_completo,v_numcte,v_rfc with resume;
 	 ...
 	 return v_cod_ret,v_nombre_completo,v_numcte,v_rfc;
         ...
         foreach
	    select limit 20 nombre1,nombre2,apell_paterno,apell_materno,numcte,rfc, rfc_alterno
	       into v_nombre1,v_nombre2,v_paterno,v_materno,v_numcte,v_rfc, v_rfc_alterno
      	       from si_cliente
...
create procedure "informix".consnomb(pempresa char(3),
                          ppaterno char(26), 
                          pmaterno char(26), 
	                  prazon char(60), 
                          psecuencia smallint) 
...
if prazon is not null and prazon !="" then
	...
	if ppaterno is null or ppaterno="" then
		...
		return v_cod_ret, v_nombre_completo, v_numcte;
           ...
           let pmaterno = trim(pmaterno)||"*"; 
		foreach
			select limit 20 nombre1, nombre2, apell_paterno, apell_materno, 
				numcte
