CREATE PROCEDURE "informix".sp_generarbalanzadiariaconsolidadamn(p_dFecha DATE)
RETURNING CHAR(5) AS retorno, VARCHAR(255) AS mensaje;
	...
	--SET DEBUG FILE TO "sp_generarbalanzadiariaconsolidadamn.out";
	--TRACE ON;
	...
	DELETE FROM bdicont:"informix".bi_balanza_dmn
		...
		IF(v_cProcesoCierre IS NOT NULL) THEN
			...
			IF(v_cProcesoCierre == "000") THEN
				...
				IF(YEAR(p_dFecha) = YEAR(v_dFechaContable) AND MONTH(p_dFecha) = MONTH(v_dFechaContable)) THEN
					...
					INSERT INTO bdicont:"informix".bi_balanza_dmn (fecha_insert, fecha_balanza, ccmayor, ccsub, ccsubsub, ccssubsub, ccsssubsub, sector, tipo_cta, descripcion,
					...
					SUM(CASE WHEN b.naturaleza_cta = "D" THEN a.saldo_inicio_dia ELSE 0 END), 
					SUM(CASE WHEN b.naturaleza_cta = "A" THEN a.saldo_inicio_dia ELSE 0 END),
					SUM(a.cargos_dia), SUM(a.abonos_dia),
					SUM(CASE WHEN b.naturaleza_cta = "D" THEN a.saldo_fin_de_dia ELSE 0 END),
					SUM(CASE WHEN b.naturaleza_cta = "A" THEN a.saldo_fin_de_dia ELSE 0 END)
				...
				FOREACH
					...
					FROM 	bdicont:"informix".bi_balanza_dmn
					...
					SELECT COUNT(*) INTO w_cuantos FROM bdicont:"informix".bi_balanza_dmn WHERE ccmayor = v_mayor AND ccsub = v_cero_s AND ccsubsub = v_cero_ss
					...
					IF bpccmayor > v_cero_may AND bpccsub > v_cero_s AND bpccsubsub > v_cero_ss AND bpccssubsub > v_cero_sss AND bpccsssubsub > v_cero_ssss THEN
						...
						SELECT COUNT(*) INTO w_cuantos FROM bdicont:"informix".bi_balanza_dmn WHERE ccmayor = bpccmayor AND ccsub = bpccsub AND ccsubsub = bpccsubsub
						...
						SELECT COUNT(*) INTO w_cuantos FROM bdicont:"informix".bi_balanza_dmn WHERE ccmayor = bpccmayor AND ccsub = bpccsub AND ccsubsub = v_cero_ss
						AND ccssubsub = v_cero_sss AND ccsssubsub = v_cero_ssss AND sector = v_cero_sect AND tipo_cta IS NOT NULL AND fecha_balanza = p_dFecha;
						...
						SELECT COUNT(*) INTO w_cuantos FROM bdicont:"informix".bi_balanza_dmn WHERE ccmayor = bpccmayor AND ccsub = v_cero_s AND ccsubsub = v_cero_ss
						AND ccssubsub = v_cero_sss AND ccsssubsub = v_cero_ssss AND sector = v_cero_sect AND tipo_cta IS NOT NULL AND fecha_balanza = p_dFecha;
				...
				FOREACH
					...
					UPDATE bdicont:"informix".bi_balanza_dmn
