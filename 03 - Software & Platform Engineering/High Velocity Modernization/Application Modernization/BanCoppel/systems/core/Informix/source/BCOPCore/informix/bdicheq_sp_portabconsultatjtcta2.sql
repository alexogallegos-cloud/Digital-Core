CREATE PROCEDURE "informix".sp_portabconsultatjtcta2(pEmpresa CHAR(3), pTipo CHAR(1), pNumTajCta CHAR(20))
RETURNING
	CHAR(5) AS cCodRet,
	CHAR(20) AS cNumCte,
	...
	ELIF pTipo = '2' THEN 
        ...
        ELSE
            LET cCodRet = '00050';
        END IF;
	ELIF pTipo = '3' THEN --Consulta por NÃÂºmero de Cuenta clabe
				...
				FOREACH
                    SELECT  si_cliente.numcte, si_cliente.apell_paterno, si_cliente.apell_materno, si_cliente.nombre1, si_cliente.nombre2,
                        ...
                        AND folio_solicitud = (SELECT max(folio_solicitud)
                                               ...
                                               WHERE empresa = pEmpresa AND num_cte = cNumCte AND (scmaechq.cuenta_clabe =  solicitudporta.cta_ordenante 
                                                                                                    OR scmaechq.cuenta_clabe = solicitudporta.cta_receptora)
...
END PROCEDURE
DOCUMENT
'Elaboro:Armida Pazos ChÃÂ¡vez',
'Fecha: 20100525',
'Proyecto: Habilitar y Deshabilitar la Portabilidad de NÃÂ³mina.',
'-----------------------------------------------------------------------',
'Folio: 1748',
'Autor: Claudio Almodovar',
'Fecha: 31/08/2015',
'ModificaciÃÂ³n: Se modifica SP para portabilidad de nomina de otros bancos a bancoppel',
