CREATE PROCEDURE "informix".sp_domi_consulta_autorizacionesactivas(p_sNumeroCliente CHAR(20), p_sUserStatus CHAR(8))
	RETURNING CHAR(5) 		AS codRet, 
			  ...
			  CHAR(20) 		AS folioActivacion,--folio activaciÃ³n
			  CHAR(20)		AS numeroTarjetaAbono, --NÃºmero de Tarjeta abono #GET SP
			  ...
			  CHAR(10)		AS fechaUltimoPago, --Ãltima Fecha de pago
			  MONEY(16,2)	AS montoProximoPago, --monto proximo pago
			  MONEY(16,2)	AS montoUltimoPago, -- Ãltimo monto de pago
	...
	--*********************************************************************************************************************************
	--SET DEBUG FILE TO "/tmp/sp_domi_consulta_autorizacionesactivas.out";
	--TRACE ON;
	--*********************************************************************************************************************************
	...
	BEGIN
		ON EXCEPTION SET sql_err
			IF sql_err <> 0 THEN
				...
				VALUES (CURRENT, CURRENT HOUR TO FRACTION, v_sCodRet, '', 'sp_domi_consulta_autorizacionesactivas', trim(p_sNumeroCliente), p_sUserStatus, CURRENT);
				
				RETURN v_sCodRet,v_sCuentaCargo,v_sAliasServicioDomi,v_sFolioActivacion,v_sNumeroTarjetaAbono,v_sNombreProductoAbono,v_sNombreBancoAbono,v_sFechaProximo,v_sFechaUltimoPago,v_mMontoProximoPago,v_mMontoUltimoPago, v_sCategoria, v_sPeriodicidad,v_sTipoDomiciliacion, v_sCuentaAbono, v_sMontoPagoFijo, v_sTipoPago, v_sImpMaximo,  v_sTipoFechaPago, v_sFechaPago, v_generico1, v_generico2, v_generico3, v_generico4;
			END IF;


		END EXCEPTION;
			
		IF NVL(p_sNumeroCliente, '') = '' OR NVL(p_sUserStatus, '') = ''THEN
			LET v_sCodRet = '99932'; --PARAMETROS DE ENTRADA ESTAN EN BLANCO.
			
			EXECUTE PROCEDURE bdidomi:"informix".sp_obtenermensajeerror(v_sCodRet) INTO v_sCodRet2, cMensajeRespuesta;
	
			INSERT INTO bdidomi:"informix".dom_errores (Fecha_Error,Hora_Error,Cod_Error,Nombre_Arch,Sp_Llamado,Mensaje_Error,User_Insert,Fecha_Insert)
			VALUES (CURRENT, CURRENT HOUR TO FRACTION, v_sCodRet, '', 'sp_domi_consulta_autorizacionesactivas', trim(p_sNumeroCliente) || ' - ' || trim(cMensajeRespuesta), p_sUserStatus, CURRENT);
			
			
			RETURN v_sCodRet,v_sCuentaCargo,v_sAliasServicioDomi,v_sFolioActivacion,v_sNumeroTarjetaAbono,v_sNombreProductoAbono,
			v_sNombreBancoAbono,v_sFechaProximo,v_sFechaUltimoPago,v_mMontoProximoPago,v_mMontoUltimoPago, v_sCategoria, v_sPeriodicidad,v_sTipoDomiciliacion, v_sCuentaAbono, v_sMontoPagoFijo, v_sTipoPago, v_sImpMaximo,  v_sTipoFechaPago, v_sFechaPago, v_generico1, v_generico2, v_generico3, v_generico4;
		END IF;	
		
		FOREACH 
			SELECT 
			{+INDEX(dom_prod_permitidos_tc idxdom_prodperm_1), +INDEX(dom_autorizaciones dom_auto_2)} 
			 d_aut.cuenta_cargo --Cuenta cargo(DebitDeviceAccess)
			--domiciliaciones:
			, NVL(d_aut.alias_domi,'') as alias_domi --Alias del servicio a domiciliar.
			, d_aut.folio_activacion --Folio activaciÃ³n
			, d_aut.cuenta --cuenta abono (CreditDevicesAccess)
			, prod_tc.nombre_corto --Nombre corto del producto abono
			, TO_CHAR(f_pago.fecha_prox_pago,'%Y-%m-%d') --Fecha proximo pago
			, TO_CHAR(f_pago.fecha_ult_pago,'%Y-%m-%d') --Ultima Fecha de pago 
			, d_pagos.monto_ultimo_pago --Ãltimo monto de pago
			...
			, d_aut.imp_maximo
			, d_aut.imp_fijo_tc
			, TO_CHAR(f_pago.fecha_pago,'%Y-%m-%d') --Fecha pago
			INTO 
			 v_sCuentaCargo,
			--Domiciliaciones:
			...
			INNER JOIN bdicheq:"informix".sc_maechq sc_ma ON d_aut.cuenta_cargo = sc_ma.cuenta
			...
			WHERE d_aut.num_cte = p_sNumeroCliente 
			AND d_aut.cve_estatus = '01'
			GROUP BY d_aut.cve_estatus,	d_aut.cuenta_cargo, d_aut.alias_domi, d_aut.folio_activacion, d_aut.cuenta , sd_def.nombre_prod,prod_tc.nombre_corto,f_pago.fecha_prox_pago, f_pago.fecha_ult_pago, d_pagos.monto_ultimo_pago, d_pagos.monto_proximo_pago, c_tipo.categoria, f_pago.periodo, d_arch_man.tipo_domi, d_aut.cve_domiciliar_tc , d_aut.imp_maximo, prod_tc.nombre_corto, d_aut.imp_fijo_tc,f_pago.fecha_pago
			...
			INTO v_sCodRet, v_cDataAux, v_cDataAux, v_cDataAux, v_cDataAux, v_sCuentaCargo, v_cDataAux, v_sClabeInterbancaria_ca;
			...
			if v_sCodRet = '00000' THEN
			LET v_iContador = 1;
				RETURN v_sCodRet,v_sCuentaCargo,v_sAliasServicioDomi,v_sFolioActivacion,v_sNumeroTarjetaAbono,v_sNombreProductoAbono,v_sNombreBancoAbono,v_sFechaProximo,v_sFechaUltimoPago,v_mMontoProximoPago,v_mMontoUltimoPago, v_sCategoria, v_sPeriodicidad,v_sTipoDomiciliacion, v_sCuentaAbono, v_sMontoPagoFijo, v_sTipoPago, v_sImpMaximo,  v_sTipoFechaPago, v_sFechaPago, v_generico1, v_generico2, v_generico3, v_generico4 WITH RESUME;
			END IF;
		
		END FOREACH;
		
		IF v_iContador = 0 THEN
			LET v_sCodRet = '99933'; --El cliente no cuenta con domiciliaciones activas.
...
CREATE PROCEDURE "informix".sp_dom_reversa_estatus(p_sFolioActivacion CHAR(20), p_sUserStatus CHAR(8), p_sEstatusDomi CHAR(2), p_sMotivo CHAR(100), p_sFolioCancelacion CHAR(20), p_sFolioActivacionAnterior CHAR(20), p_sGenerico1 CHAR(100), p_sGenerico2 CHAR(100), p_sGenerico3 CHAR(100), p_sGenerico4 CHAR(100)) 
...
--Declaracion de  Variables
	...
	DEFINE dFechaEstatus		DATE;
	DEFINE dFechaInsert			DATE;
	DEFINE cCodret2				CHAR(5);
	DEFINE cMensajeRespuesta 	CHAR (110);
	DEFINE cAliasDomi		 	VARCHAR(50);
	DEFINE cCuenta 				CHAR(20);
	DEFINE cRfc 				CHAR(13);
	DEFINE cNumCte 				CHAR(20);
	DEFINE cCveCanal 			CHAR(2);
	DEFINE mImpMaximo		 	MONEY(16,2);
	DEFINE iNumRechazos		 	INTEGER;
	DEFINE cCveSucursal		 	CHAR(4);
	DEFINE cCveEstatus		 	CHAR(2);
	DEFINE cUserEstatus		 	CHAR(8);
	DEFINE cCveCausa		 	CHAR(2);
	DEFINE cUserInsert		 	CHAR(8);
	DEFINE cCveDomiciliarTc		CHAR(1);
	DEFINE mImpFijoTc		 	MONEY(16,2);
	...
	--*********************************************************************************************************************************
	--	SET DEBUG FILE TO "/tmp/sp_dom_reversa_estatus.out";
	--	TRACE ON;
	--*********************************************************************************************************************************
	...
	BEGIN
		...
		ON EXCEPTION SET sql_err
			IF sql_err <> 0 THEN
				...
				IF iContador = 1 THEN 
					--Rollback al insert en la tabla historica.
					DELETE FROM bdidomi:"informix".dom_autorizaciones_hist
					WHERE id = (SELECT MAX(id) FROM bdidomi:dom_autorizaciones_hist)  
					AND cve_estatus != p_sEstatusDomi;	
				...
				INSERT INTO bdidomi:"informix".dom_errores (Fecha_Error,Hora_Error,Cod_Error,Nombre_Arch,Sp_Llamado,Mensaje_Error,User_Insert,Fecha_Insert)
				VALUES (CURRENT, CURRENT HOUR TO FRACTION, sCodret, '', 'sp_dom_reversa_estatus', TRIM(p_sFolioActivacion), p_sUserStatus, CURRENT);
		...
		--Valida parametros de entrada
		 IF NVL(p_sfolioActivacion,'') = '' OR NVL(p_sUserStatus,'') = '' OR NVL(p_sEstatusDomi,'') = '' THEN
			LET sCodret = '99927'; --Problema con los parametros
			...
			--Insertamos a la tabla bdidomi:dom_errores los datos del error ocurrido.
			INSERT INTO bdidomi:"informix".dom_errores (Fecha_Error,Hora_Error,Cod_Error,Nombre_Arch,Sp_Llamado,Mensaje_Error,User_Insert,Fecha_Insert)
			VALUES (CURRENT, CURRENT HOUR TO FRACTION, sCodret, '', 'sp_dom_reversa_estatus', TRIM(p_sFolioActivacion) || '-' || TRIM(cMensajeRespuesta), p_sUserStatus, CURRENT);
			
			RETURN sCodret, v_generico1, v_generico2, v_generico3, v_generico4;
		END IF;
		
		--Valida si existe la domiciliacion, si no existe, continua el flujo y no hace nada.
		IF EXISTS (SELECT 1 FROM bdidomi:"informix".dom_autorizaciones WHERE folio_activacion = p_sfolioActivacion) THEN
			
			-- Obtiene los datos de la domiciliacion en la tabla bdidomi:dom_autorizaciones.
			SELECT cuenta, rfc, num_cte, cve_canal, imp_maximo, num_rechazos, 
				cve_sucursal, cve_estatus, fecha_estatus, user_estatus, user_insert, 
				cve_domiciliar_tc, imp_fijo_tc, tipo_cuenta_cargo, cuenta_cargo, 
				cve_banco_cargo, alias_domi, cve_causa
			INTO cCuenta, cRfc, cNumCte, cCveCanal, mImpMaximo, iNumRechazos, 
				cCveSucursal, cCveEstatus, dFechaEstatus, cUserEstatus, cUserInsert, cCveDomiciliarTc, mImpFijoTc, cTipoCuentaCargo, cCuentaCargo, 
				cCveBancoCargo, cAliasDomi, cCveCausa
			FROM bdidomi:"informix".dom_autorizaciones
			WHERE folio_activacion = p_sFolioActivacion;
		
			--Inserta a la tabla bdidomi:"informix".dom_autorizaciones_hist
			INSERT INTO bdidomi:"informix".dom_autorizaciones_hist(
				alias_domi, folio_activacion, folio_cancelacion, cuenta, rfc, num_cte, cve_canal, imp_maximo, 
				num_rechazos, cve_sucursal, cve_estatus, fecha_estatus, user_estatus, user_insert, fecha_insert, 
				cve_domiciliar_tc, imp_fijo_tc, tipo_cuenta_cargo, cuenta_cargo, cve_banco_cargo, motivo, cve_causa
			)
			VALUES(cAliasDomi, p_sFolioActivacion, p_sFolioCancelacion, cCuenta, cRfc, cNumCte, cCveCanal, mImpMaximo,
				iNumRechazos, cCveSucursal, cCveEstatus, dFechaEstatus, p_sUserStatus, cUserInsert, dFechaInsert, 
				cCveDomiciliarTc, mImpFijoTc, cTipoCuentaCargo, cCuentaCargo, cCveBancoCargo, p_sMotivo, cCveCausa 
			);
			
			-- Contador para verificar si inserto correctamente o si ocurrio error.
			LET iContador = 1;
			IF (p_sEstatusDomi = '02' AND cCveEstatus = '01') THEN
				LET cCuenta = 'C'||cCuenta; 
			ELIF (p_sEstatusDomi = '01' AND cCveEstatus = '02') THEN
				LET cCuenta = SUBSTR(cCuenta, 2, 16);
			END IF;
			
			--Reversa el estatus de la domiciliacion
			IF (NVL(p_sFolioActivacionAnterior,'') = '') THEN  --Si no se recibe folio de activacion anterior, no se toma en cuenta para actualizar el registro
				UPDATE bdidomi:"informix".dom_autorizaciones 
				SET cuenta = cCuenta, cve_estatus = p_sEstatusDomi, user_estatus = p_sUserStatus, fecha_estatus = dFechaEstatus
				WHERE folio_activacion = p_sFolioActivacion;	
			ELSE --Si existe un folio de activaciÃÂ³n anterior, se actualiza tambien el folio en el registro
				UPDATE bdidomi:"informix".dom_autorizaciones 
				SET cuenta = cCuenta, cve_estatus = p_sEstatusDomi, user_estatus = p_sUserStatus, fecha_estatus = dFechaEstatus, folio_activacion = p_sFolioActivacionAnterior
				WHERE folio_activacion = p_sFolioActivacion;	
			END IF;
...
'DESCRIPCION: Se encarga de reversar el estatus del servicio de Domiciliacion e insertar en una tabla historica',
'FECHA      : 04/02/2022',
'BD         : BDIDOMI';

CREATE PROCEDURE "informix".sp_domi_reversa_cancelacion(p_sFolioCancelacion CHAR(20), p_sFolioActivacion CHAR(20), p_sUserStatus CHAR(8))
	RETURNING CHAR(5)   AS codRet, --Codigo de Retorno
			  CHAR(100)	AS v_generico1,
			  CHAR(100)	AS v_generico2,
			  CHAR(100)	AS v_generico3,
...
--Declaracion de  Variables
	...
	--*********************************************************************************************************************************
	--SET DEBUG FILE TO "/tmp/sp_domi_reversa_cancelacion.out";
	--TRACE ON;
	--*********************************************************************************************************************************
	...
	BEGIN
		...
		ON EXCEPTION SET sql_err
			IF sql_err <> 0 THEN
				...
				INSERT INTO bdidomi:"informix".dom_errores (Fecha_Error, Hora_Error, Cod_Error, Nombre_Arch, Sp_Llamado, Mensaje_Error, User_Insert, Fecha_Insert)
				VALUES (CURRENT, CURRENT HOUR TO FRACTION, sCodret, '', 'sp_domi_reversa_cancelacion', trim(p_sFolioActivacion), p_sUserStatus, CURRENT);
		...
		--Valida parametros de entrada
		 IF NVL(p_sFolioActivacion,'') = '' OR NVL(p_sFolioCancelacion,'') = '' OR NVL(p_sUserStatus,'') = ''THEN
			LET sCodret = '99948'; --Algun parametro de entrada requerido esta en blanco.	
			
			EXECUTE PROCEDURE bdidomi:"informix".sp_obtenermensajeerror(sCodret) INTO cCodret2, cMensajeRespuesta;
	
			INSERT INTO bdidomi:"informix".dom_errores (Fecha_Error,Hora_Error,Cod_Error,Nombre_Arch,Sp_Llamado,Mensaje_Error,User_Insert,Fecha_Insert)
			VALUES (CURRENT, CURRENT HOUR TO FRACTION, sCodret, '', 'sp_domi_reversa_cancelacion', trim(p_sFolioActivacion) || ' - ' || trim(cMensajeRespuesta), p_sUserStatus, CURRENT);
			
			RETURN sCodret, v_generico1, v_generico2, v_generico3, v_generico4;
		END IF;
		
		--Busca la domiciliacion en la tabla bdidomi:dom_cancelaciones.
		IF EXISTS (SELECT 1 FROM bdidomi:"informix".dom_cancelaciones d_canc
						WHERE d_canc.folio_activacion = p_sFolioActivacion AND d_canc.folio_cancelacion = p_sFolioCancelacion) THEN
			...
			INSERT INTO bdidomi:"informix".dom_autorizaciones_hist(
				...
				cve_domiciliar_tc, imp_fijo_tc, tipo_cuenta_cargo, cuenta_cargo, cve_banco_cargo, folio_cancelacion,motivo
			) SELECT d_aut.alias_domi, d_aut.folio_activacion, d_aut.cuenta, d_aut.rfc, d_aut.num_cte, d_aut.cve_canal, d_aut.imp_maximo, d_aut.num_rechazos, 
				d_aut.cve_sucursal, d_aut.cve_estatus, d_aut.fecha_estatus, d_aut.user_estatus, d_aut.cve_causa, d_aut.user_insert, d_aut.fecha_insert, 
				d_aut.cve_domiciliar_tc, d_aut.imp_fijo_tc, d_aut.tipo_cuenta_cargo, d_aut.cuenta_cargo, d_aut.cve_banco_cargo,d_canc.folio_cancelacion, d_canc.motivo
				FROM bdidomi:"informix".dom_cancelaciones d_canc
				INNER JOIN bdidomi:"informix".dom_autorizaciones d_aut ON d_canc.folio_activacion = d_aut.folio_activacion
				WHERE d_canc.folio_activacion = p_sFolioActivacion AND d_canc.folio_cancelacion = p_sFolioCancelacion;
			
			--ELIMINAR EL REGISTRO DE dom_cancelaciones
			DELETE FROM bdidomi:"informix".dom_cancelaciones d_canc
			WHERE d_canc.folio_activacion = p_sFolioActivacion AND d_canc.folio_cancelacion = p_sFolioCancelacion;
			
			--Actualiza la tabla bdidomi:dom_autorizaciones.
			UPDATE bdidomi:"informix".dom_autorizaciones 
			SET (cve_causa, user_insert) = 
			(
				'00', 
				p_sUserStatus
			)
...
DOCUMENT
'AUTOR      		: Aldo Alejandro Sanchez Felix',
'DESCRIPCION		: Se encarga de la reversa de cancelacion del servicio de Domiciliacion',
'FECHA      		: 27/04/2022',
'BD         		: BDIDOMI';

CREATE PROCEDURE "informix".sp_domi_valida_folio(p_sNumcte CHAR(20),  p_sFolioActivacion CHAR(20), p_sUserStatus CHAR(8))
    RETURNING CHAR(5) as codRet;
    ...
    BEGIN
        ...
        ON EXCEPTION SET sql_err
            IF sql_err <> 0 THEN
                ...
                --Insertamos a la tabla bdidomi:dom_errores los datos del error ocurrido.
                INSERT INTO bdidomi:"informix".dom_errores(Fecha_Error,Hora_Error,Cod_Error,Nombre_Arch,Sp_Llamado,Mensaje_Error,User_Insert,Fecha_Insert)
                VALUES (CURRENT, CURRENT HOUR TO FRACTION, cCodret, '', 'sp_domi_valida_folio', trim(p_sFolioActivacion), p_sUserStatus, CURRENT);
        ...
        IF NVL(p_sNumcte, '') = '' OR NVL(p_sFolioActivacion, '') = '' OR NVL(p_sUserStatus, '') = '' THEN
            LET cCodret = '99982'; --PARAMETROS DE ENTRADA ESTAN EN BLANCO

            EXECUTE PROCEDURE bdidomi:"informix".sp_obtenermensajeerror(cCodret) INTO cCodret2, cMensajeRespuesta;

            INSERT INTO bdidomi:"informix".dom_errores (Fecha_Error,Hora_Error,Cod_Error,Nombre_Arch,Sp_Llamado,Mensaje_Error,User_Insert,Fecha_Insert)
            VALUES (CURRENT, CURRENT HOUR TO FRACTION, cCodret, '', 'sp_domi_valida_folio', trim(p_sFolioActivacion) || ' - ' || trim(cMensajeRespuesta), p_sUserStatus, CURRENT);

            RETURN cCodret;
        END IF;

        IF EXISTS (SELECT 1 FROM bdidomi:"informix".dom_autorizaciones where num_cte = p_sNumCte AND folio_activacion = p_sFolioActivacion AND cve_estatus = '01') THEN
            RETURN cCodret;
        END IF;

        LET cCodret = '99983'; --NO EXISTE EL FOLIO DE ACTIVACION 
...
END PROCEDURE
DOCUMENT
'AUTOR      : Alberto Diaz Carbajal',
'DESCRIPCION: Validar que exista el folio de activacion de la domiciliacion activa',
'FECHA      : 17/05/2023',
'BD         : BDIDOMI';
