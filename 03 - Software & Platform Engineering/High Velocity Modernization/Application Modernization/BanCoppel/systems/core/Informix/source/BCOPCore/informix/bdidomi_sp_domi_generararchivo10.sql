CREATE PROCEDURE "informix".sp_domi_generararchivo10(pNombreArchivo CHAR(20),pUser_insert CHAR(8))
	RETURNING CHAR(5);
...
DEFINE dFecha 		 DATE;
DEFINE iConsecutivoD INTEGER;
DEFINE cCodSpFecha   CHAR(5);
DEFINE cPrefijoCLABE CHAR(5);
DEFINE cBandera      CHAR(1);
DEFINE dFechaHabil   DATE;

---- VARIABLES ENCABEZADO PASO-----
...
DEFINE cNum_cta_ord_paso 	CHAR(20);
...
DEFINE cTipo_cta_cargo_cteD   		CHAR(2);
DEFINE cCve_banco_cargo_cteD  		CHAR(3);
DEFINE cCuenta_cargo_cteD     		CHAR(20);
DEFINE cRfc_cargo_cteD 		 		CHAR(13);
DEFINE cNombre_cargo_cteD 	  		CHAR(50);
DEFINE cCuenta_abono_cteD 	  		CHAR(20);
DEFINE cImp_operacion_cteD 	  		CHAR(15);
DEFINE cImp_iva_cteD 		  		CHAR(15);
DEFINE cRef_numerica_cteD 	  		CHAR(7);
DEFINE cRef_leyenda_cteD 	  		CHAR(40);
DEFINE cRef_servicio_cteD 	  		CHAR(40);
DEFINE cRef_titular_serv_cteD 		CHAR(40);
DEFINE cAccion_cteD 		   		CHAR(1);
DEFINE cReintentar_cuenta_cteD 		CHAR(1);
DEFINE cEstatus_cteD 				CHAR(2);
DEFINE cCausa_rechazo_cteD 			CHAR(50);
...
DEFINE cUser_insert_cteD 			CHAR(8);
DEFINE dFecha_insert_cteD 			DATE;
DEFINE cTipo_cta_abono_cteD 		CHAR(2);
DEFINE iContador 					INTEGER;
DEFINE cFechaPresentacionCce		CHAR(8);
DEFINE iAuxValidBin                 INTEGER;
DEFINE iIdUnidadProd                INTEGER;
DEFINE cPrefijoCuentasPropias 		VARCHAR(12);
DEFINE cCurrentPrefijo 				VARCHAR(12);
DEFINE dDateYesterday				DATE;
DEFINE dAux1						DATE;
DEFINE dAux2 						DATE;
...
---- INICIALIZAR VARIABLES ENCABEZADO PASO-----
...
LET cUser_insertE 		 = '';
LET dFecha_insertE		 = CURRENT;

---- INICIALIZAR VARIABLES DETALLE  PASO-----
LET cNombre_archD ='';
...
LET cFolio_suc ='';
LET cUser_insertD ='';
LET dFecha_insertD = CURRENT;

---- INICIALIZAR VARIABLES SUMARIO  PASO -----
LET cNombre_archS ='';
...
LET cTipo_cta_cargo_cteD = '';
LET cCve_banco_cargo_cteD = '';
LET cCuenta_cargo_cteD = '';
LET cRfc_cargo_cteD = '';
LET cNombre_cargo_cteD = '';
LET cCuenta_abono_cteD = '';
LET cImp_operacion_cteD = '';
LET cImp_iva_cteD = '';
LET cRef_numerica_cteD = '';
LET cRef_leyenda_cteD = '';
LET cRef_servicio_cteD = '';
LET cRef_titular_serv_cteD = '';
LET cAccion_cteD = '';
LET cReintentar_cuenta_cteD = '';
LET cEstatus_cteD = '';
LET cCausa_rechazo_cteD = '';
LET cNombre_arch_cce_cteD = '';
LET cFecha_presentacion_cce_cteD = '';
LET cTipo_registro_cce_cteD = '';
LET cNumero_secuencia_cce_cteD = '';
LET cUser_insert_cteD = '';
LET dFecha_insert_cteD = CURRENT;
LET cTipo_cta_abono_cteD = '';
LET iContador = 0;
...
LET cCurrentPrefijo = '';
LET dDateYesterday = CURRENT;
LET dAux1 = CURRENT;
LET dAux2 = CURRENT;

-- rango de errores  01900 -01999

		--SET debug FILE TO "/informix/sp_domi_generararchivo10.out";
		--TRACE ON;
...
Begin
	...
	SELECT fecha_hoy INTO dFecha FROM bdicheq:sc_fechas;
	SELECT TRIM(valor) INTO cPrefijoCLABE FROM Dom_Parametros WHERE cod_param = '05';
	SELECT TRIM(valor) 
	INTO cPrefijoCuentasPropias 
	FROM Dom_Parametros 
	WHERE cod_param = '72';
	--- Se obtienen los registros de la tabla dom_cte_detalle donde accion  = 'A' Y estatus = 'EP'
	FOREACH
		SELECT nombre_arch, fecha_envio, tipo_registro, consecutivo, fecha_cargo, fecha_abono, tipo_cta_cargo, cve_banco_cargo, cuenta_cargo,
			   rfc_cargo, nombre_cargo, cuenta_abono, imp_operacion, imp_iva, ref_numerica, ref_leyenda, ref_servicio, ref_titular_serv,
		       accion, reintentar_cuenta, estatus, causa_rechazo, nombre_arch_cce, fecha_presentacion_cce, tipo_registro_cce, numero_secuencia_cce,
			   user_insert, fecha_insert, tipo_cta_abono
		INTO   cNombre_arch_cteD, dFecha_envio_cteD, cTipo_registro_cteD, cConsecutivo_cteD, cFecha_cargo_cteD, cFecha_abono_cteD,
		       cTipo_cta_cargo_cteD, cCve_banco_cargo_cteD, cCuenta_cargo_cteD, cRfc_cargo_cteD, cNombre_cargo_cteD, cCuenta_abono_cteD,
			   cImp_operacion_cteD, cImp_iva_cteD, cRef_numerica_cteD, cRef_leyenda_cteD, cRef_servicio_cteD, cRef_titular_serv_cteD,
			   cAccion_cteD, cReintentar_cuenta_cteD, cEstatus_cteD, cCausa_rechazo_cteD, cNombre_arch_cce_cteD, cFecha_presentacion_cce_cteD,
			   cTipo_registro_cce_cteD, cNumero_secuencia_cce_cteD, cUser_insert_cteD, dFecha_insert_cteD, cTipo_cta_abono_cteD
		FROM dom_cte_detalle de
		WHERE de.accion = 'A' AND de.estatus = 'EP' AND cve_banco_cargo <> cPrefijoCLABE
        ...
        WHERE tar.num_tarjeta = substr(cCuenta_abono_cteD, 5, 16);

        IF iIdUnidadProd IN (2, 4) THEN
            CONTINUE FOREACH;
        END IF;

		LET cCurrentPrefijo = SUBSTR(cCuenta_abono_cteD, 9, 11);
		IF cCurrentPrefijo <> cPrefijoCuentasPropias THEN
			EXECUTE PROCEDURE bdidomi:"informix".sp_domi_obtener_fecha_valida_ob(
				TO_DATE(cFecha_cargo_cteD, '%Y%m%d')
			)
			INTO dAux1, dAux2, dDateYesterday;
			IF dFecha NOT IN (dFecha_envio_cteD, dDateYesterday) THEN
				CONTINUE FOREACH;
			END IF;
		END IF;
		-- Validar  que el campo reintentar_cuenta = 's'
		LET cMensaje = cReintentar_cuenta_cteD ;
		IF(cReintentar_cuenta_cteD = 'S') THEN  --- Si el campo es una 'S'  se obtiene la cuenta sin importar que exista en la dom_ctas_verificadas
			LET cBandera = 'S';
		ELSE--- Si no,  se valida que la cuenta exista en la dom_ctas_verificadas

			SELECT COUNT(*)
			INTO iContador
			FROM dom_ctas_verificadas
			WHERE cuenta =  cCuenta_cargo_cteD;

			--IF( EXISTS(SELECT cuenta FROM dom_ctas_verificadas WHERE cuenta =  cCuenta_cargo_cteD  )) THEN
			IF iContador >= 1 THEN
				LET cBandera = 'N';
			ELSE
				LET cBandera = 'S';
			END IF;
		END IF;
		---Si la  bandera es  una 'S' se genera el registro del detalle
		IF cBandera = 'S' THEN

			SELECT COUNT(*)
			INTO iContador
			FROM bdinteg:si_bancos
			WHERE flg_domi_r = 1 and banco = cCve_banco_cargo_cteD;

			IF iContador >= 1 THEN

				LET cBandera = 'N';
				--- Generar un registro dentro de la tabla dom_cce_detalle_paso
				LET cNombre_archD = pNombreArchivo;
				LET cFecha_presentacionD = YEAR(dFecha) || LPAD(MONTH(dFecha),2,'0')|| LPAD(DAY(dFecha),2,'0');
				LET cTipo_registro ='02';
				LET cNum_secuenciaD =   LPAD(iConsecutivoD,7,'0');
				LET cCod_operacionD = '10';
				LET cCod_divisaD ='01';
				--- Obtener la fecha  T + 1 (el dia siguiente habil)
				EXECUTE FUNCTION bdinteg:sp_valfecha_banca('001',(dFecha) + 1 ,0)INTO cCodSpFecha,dFechaHabil;
				LET cFecha_trans = year(dFechaHabil) || LPAD(month(dFechaHabil),2,'0')|| LPAD(day(dFechaHabil),2,'0');
				LET cBanco_presentador = TRIM(cPrefijoCLABE);
				LET cBanco_receptor = cCve_banco_cargo_cteD;
				LET cImporte = '000000000000000';
				LET cUso_futuro_ccenD = '                ';
				LET cTipo_operacion = '51';
				LET cFecha_aplica = year(dFechaHabil) || LPAD(month(dFechaHabil),2,'0')|| LPAD(day(dFechaHabil),2,'0');
				LET cFecha_trans = year(dFechaHabil) || LPAD(month(dFechaHabil),2,'0')|| LPAD(day(dFechaHabil),2,'0');
				LET cNum_cta_ord = LPAD(cCuenta_abono_cteD,20,'0');
				---El nombre se obtiene de la  si_cliente
                ...
                WHERE banco.flg_domi_r = '1'
                    AND banco = '137'
                    AND creditodebito = 'c'
                    AND bin = SUBSTR(cCuenta_abono_cteD,5,6);
				
		-- FRG_I ---> Se considera el cTipoCtaAbono = '05' o = '03'
				IF cTipo_cta_abono_cteD = '05'
					OR (cTipo_cta_abono_cteD = '03' AND iAuxValidBin > 0 )
			-- FRG_F ---> Se considera el cTipoCtaAbono = '05' o = '03'
				THEN -- Es tarjeta de credito

					LET cNum_cta_ord_paso = SUBSTR(cNum_cta_ord,5,16);

					SELECT NVL(TRIM(TRIM(nombre1)||' '||TRIM(nombre2)||' '||TRIM(apell_paterno)||' '||TRIM(apell_materno)||' '|| TRIM(razon_social)  ),' '), NVL(rfc ,' ')
					INTO cNombre_ord, cRfc_ord
					FROM bdicred:sd_tarjeta tar
					INNER JOIN bdinteg:si_cliente cte ON tar.numcte = cte.numcte
					WHERE tar.empresa = '001' AND tar.num_tarjeta  = cNum_cta_ord_paso;

			-- FRG_I ---> Se considera el cTipoCtaAbono = '05' o = '03'
					-- LET cTipo_cta_ord = '05';
					LET cTipo_cta_ord = cTipo_cta_abono_cteD;
		-- FRG_F ---> Se considera el cTipoCtaAbono = '05' o = '03'

				ELSE -- Es cuenta CLABE

					LET cNum_cta_ord_paso = SUBSTR(cNum_cta_ord,9,11);

					SELECT NVL(TRIM(TRIM(nombre1)||' '||TRIM(nombre2)||' '||TRIM(apell_paterno)||' '||TRIM(apell_materno)||' '|| TRIM(razon_social)  ),' '), NVL(rfc ,' ')
					INTO cNombre_ord, cRfc_ord
					FROM bdicheq:sc_maechq mae
					INNER JOIN bdinteg:si_cliente cte ON mae.num_cte = cte.numcte
					WHERE mae.empresa = '001' AND mae.cuenta = cNum_cta_ord_paso;
					LET cTipo_cta_ord = '40';
				END IF;
				LET cTipo_cta_rec = cTipo_cta_cargo_cteD;
				LET cNum_cta_rec = LPAD(cCuenta_cargo_cteD,20,'0');
				LET cNombre_rec = cNombre_cargo_cteD;
				LET cRfc_rec = cRfc_cargo_cteD;
				LET cRef_servicio = cRef_servicio_cteD;
				LET cNombre_titular_serv = cRef_titular_serv_cteD;
				LET cImporte_iva = cImp_iva_cteD;
				LET cRef_numerica = lpad(TRIM((cRef_numerica_cteD::integer)::char(7)),7,'0');
				LET cRef_leyenda = 'CUENTA PARA VERIFICAR';
				LET cClave_rastreo = TRIM(pNombreArchivo)||TRIM(cNum_secuenciaD);
				LET cMotivo_dev = '00';
				LET cFecha_pres_ini = cFecha_presentacionD;
				LET cUso_futuro_bancoD ='            ';
				LET cCve_estatus ='00';
				LET cFolio_suc ='                ';
				LET cUser_insertD = pUser_insert;
				LET dFecha_insertD = CURRENT;
				LET iConsecutivoD = iConsecutivoD + 1;

                EXECUTE PROCEDURE sp_reemplazar_n_acentos_ob(cNombre_ord)
		        INTO cNombre_ord;

		        EXECUTE PROCEDURE sp_reemplazar_n_acentos_ob(cNombre_rec)
		        INTO cNombre_rec;

				IF cCurrentPrefijo <> cPrefijoCuentasPropias THEN
					EXECUTE PROCEDURE sp_reemplazar_n_acentos_ob(cNombre_titular_serv)
					INTO cNombre_titular_serv;
				END IF;

				-- Insert a la de detalle paso
				...
				Values (cNombre_archD, cFecha_presentacionD, cTipo_registro, cNum_secuenciaD, cCod_operacionD, cCod_divisaD, cFecha_trans,
						cBanco_presentador, cBanco_receptor, cImporte, cUso_futuro_ccenD, cTipo_operacion, cFecha_aplica, cTipo_cta_ord,
						cNum_cta_ord, cNombre_ord,
						cRfc_ord, cTipo_cta_rec, cNum_cta_rec, cNombre_rec, cRfc_rec, cRef_servicio, cNombre_titular_serv,
				...
				-- Nota.- Se agrega el insert a cuentas verificadas el dia 14/sep/2009, dado que se cambio  la logica y ahora se pueden recibir X cantidad de registros en el archivo 11, no los registros que mandamos en el archivo 10
				-- original, peticion hecha por Jaime Gonzales
				IF ( EXISTS(SELECT cuenta FROM dom_ctas_verificadas WHERE cuenta =  cCuenta_cargo_cteD  )) THEN
				ELSE
					...
					VALUES(cBanco_receptor,cNum_cta_rec,cNombre_archD,cFecha_presentacionD,'01','99',cUser_insertD, dFecha_insertD);
				END IF;
			ELSE

				LET cFechaPresentacionCce = TO_CHAR( dFecha_envio_cteD, '%Y%m%d' );

				UPDATE bdidomi:dom_cte_detalle SET estatus = '02', causa_rechazo = '01',
				nombre_arch_cce = cNombre_archD, fecha_presentacion_cce = cFechaPresentacionCce,
				tipo_registro_cce = '02', numero_secuencia_cce = '0'|| cConsecutivo_cteD
				WHERE nombre_arch = cNombre_arch_cteD
				AND fecha_envio = dFecha_envio_cteD
				AND tipo_registro = cTipo_registro_cteD
				AND consecutivo = cConsecutivo_cteD
				AND cve_banco_cargo = cCve_banco_cargo_cteD
				AND cuenta_cargo = cCuenta_cargo_cteD
				AND accion = 'A' AND estatus = 'EP';
			END IF;
		END IF;
	END FOREACH;
	--- Validar si hubo o no registros en detalle si no pues marcar el error en la de errores de lo contrario insertar detalle y sunmario
	IF iConsecutivoD = 2 THEN--- se debe insertar en las dom_errores
		LET cMensaje = 'EL proceso se ejecuto correctamente y no se genero el archivo ya que no existen cuentas pendientes por verificar';
		LET cCodRet = '01900';
	ELSE--- Si no pues se procede a crear el insert en  encabezado y en sumario
		----- Generar el insert del encabezado
		LET cNombre_archE = pNombreArchivo;
		LET cFecha_presentacionE = YEAR(dFecha) || LPAD(MONTH(dFecha),2,'0')|| LPAD(DAY(dFecha),2,'0');
		LET cTpo_registro = '01';
		LET cNum_secuenciaE = '0000001';
		LET cCod_operacionE = '10';
		LET cCve_banco = TRIM(cPrefijoCLABE);
		LET cSentido = 'E';
		LET cServicio = '2';
		LET cNum_bloque = LPAD(DAY(dFecha),2,'0') ||  LPAD(SUBSTR(pNombreArchivo,16,2),5,'0');
		LET cCod_divisaE = '01';
		LET cCve_rechazo_bl = '00';
		LET cModalidad = '2';
		LET cUso_futuro_ccenE = '                                         ';
		LET cUso_futuro_bancoE ='                                                                                                                                                                                                                                                                                                                                                         ';
		LET cUser_insertE = pUser_insert;
		LET dFecha_insertE = CURRENT;
		...
		----- Generar el insert del sumario
		LET cNombre_archS = pNombreArchivo;
		LET cFecha_presentacionS = YEAR(dFecha) || LPAD(MONTH(dFecha),2,'0')|| LPAD(DAY(dFecha),2,'0');
		LET cTipo_registroS = '09';
		LET cNum_secuenciaS = LPAD(iConsecutivoD,7,'0');
		LET cCod_operacionS = '10';
		LET cNum_bloqueS = LPAD(DAY(dFecha),2,'0') ||  LPAD(SUBSTR(pNombreArchivo,16,2),5,'0');
		LET cNum_operaciones = LPAD(iConsecutivoD - 2,7,'0');
		LET cImp_operaciones = '000000000000000000';
		LET cUso_futuro_ccenS = '                                        ';
		LET cUso_futuro_bancoS = '                                                                                                                                                                                                                                                                                                                                                   ';
		LET cUser_insertS = pUser_insert;
		LET dFecha_insertS = CURRENT;
...
'DESCRIPCION: Este Procediemento se encarga de generar los registros de encabezado, detalle y sumario, tomandolos de las tablas clientes ',
'             e insertandolos en las tablas de paso para posteriormente generar el archivo 10 ya como banco presentador',
'FECHA : Julio de 2009',
'Modificacion: CÃ?Â?Ã?Â©sar ValdÃ?Â?Ã?Â©z Figueroa',
'Descripcion: Adaptar el proceso para permitir la generaciÃ?Â?Ã?Â³n de archivos codigo 10 con tarjetas de crÃ?Â?Ã?Â©dito',
'Fecha: 20 de febrero de 2010',
'Version: 20100220.2300',
'BD: BDIDOMI';

CREATE PROCEDURE "informix".sp_domi_alta_cuentas_registradas(pNumcte CHAR(20), pNumTarjeta CHAR(18), pCveBanco CHAR(3), pAlias CHAR(30), p_sGenerico1 NVARCHAR(254), p_sGenerico2 NVARCHAR(254), p_sGenerico3 NVARCHAR(254), p_sGenerico4 NVARCHAR(254), p_sGenerico5 NVARCHAR(254))
    RETURNING   CHAR(5) AS CodRet;
    --Declaracion de  Variables
    DEFINE sql_err 				INTEGER;			  
	DEFINE v_sCodRet			CHAR(5);
	
	DEFINE v_sNombreTitular		CHAR(50);
    ...
    --***************************************************************************************
    --SET DEBUG FILE TO "/tmp/sp_domi_cuentasregistradas.out";
	--TRACE ON;
	--***************************************************************************************    
    ...
    BEGIN
        ON EXCEPTION SET sql_err
			IF sql_err <> 0 THEN
				...
				VALUES (CURRENT, CURRENT HOUR TO FRACTION, v_sCodRet, '', 'sp_domi_alta_cuentas_registradas', trim(pNumcte), '',CURRENT);
        ...
        --Valida si los parametros requeridos de entrada vienen vacios
		IF NVL(pNumcte, '') = '' OR NVL(pNumTarjeta, '') = '' OR NVL(pCveBanco, '') = '' THEN
			LET v_sCodRet = '88823'; -- PARAMETRO DE ENTRADA REQUERIDO EST?? EN BLANCO.
		
			EXECUTE PROCEDURE bdidomi:"informix".sp_obtenermensajeerror(v_sCodRet) INTO v_sCodRet2, cMensajeRespuesta;
	
			INSERT INTO bdidomi:"informix".dom_errores (Fecha_Error,Hora_Error,Cod_Error,Nombre_Arch,Sp_Llamado,Mensaje_Error,User_Insert,Fecha_Insert)
			VALUES (CURRENT, CURRENT HOUR TO FRACTION, v_sCodRet2, '', 'sp_domi_alta_cuentas_registradas', trim(pNumcte) || ' - ' || trim(cMensajeRespuesta), '', CURRENT);
			
			RETURN v_sCodRet;	
		END IF;	

		-- VALIDAR SI EXISTE CLIENTE.
		IF NOT EXISTS(SELECT * FROM bdinteg:"informix".si_cliente WHERE numcte = pNumcte) THEN
			LET v_sCodRet = '88824'; -- CLIENTE NO EXISTENTE.
        ...
        SELECT UPPER(TRIM(nombre1) || ' ' || TRIM(nombre2) || ' ' || TRIM(apell_paterno) || ' ' || TRIM(apell_materno)) as nombre_titular
        INTO v_sNombreTitular
        FROM bdinteg:"informix".si_cliente
        WHERE numcte = pNumcte;

        --Inserta en la tabla dom_cuentas_ob.
		IF NOT EXISTS (SELECT 1 from bdidomi:"informix".dom_cuentas_ob where num_cliente = pNumcte and num_tarjeta = pNumTarjeta) THEN
			
			INSERT INTO bdidomi:"informix".dom_cuentas_ob(num_cliente, num_tarjeta, cve_banco, nombre_titular, concepto, estatus, fecha_insert, user_insert) 
			VALUES(pNumcte, pNumTarjeta, pCveBanco, v_sNombreTitular, pAlias, '03', CURRENT, 'informix');
        
			RETURN v_sCodRet; 
       
        ELIF EXISTS(SELECT 1 FROM bdidomi:"informix".dom_cuentas_ob WHERE num_cliente = pNumcte AND num_tarjeta = pNumTarjeta AND estatus = '02' OR estatus = '04' OR estatus = '05' ) THEN
        
			UPDATE bdidomi:"informix".dom_cuentas_ob SET estatus = '03' 
			WHERE num_cliente = pNumcte AND num_tarjeta = LTRIM(pNumtarjeta,'0');
		
			RETURN v_sCodRet; 
		
        ELSE 
			
        	LET v_sCodRet = '88832'; -- YA EXISTE UN REGISTRO CON ESOS VALORES.
...
'DESCRIPCION: 	Se encarga de resgistrar las cuentas de debito de otros bancos de un cliente.',
'FECHA: 		23/01/2024',
'BD: 			BDIDOMI';

CREATE PROCEDURE "informix".sp_reemplazar_n_acentos_ob(input_text VARCHAR(255))
    RETURNING VARCHAR(255);

    DEFINE output_text VARCHAR(255);
    DEFINE current_char CHAR(1);
    DEFINE i INTEGER;
    DEFINE valid_chars VARCHAR(100);

    -- Inicializamos la cadena con todos los caracteres válidos del español mexicano
    LET valid_chars = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZáéíóúÁÉÍÓÚñÑ ';

    -- Inicializamos la cadena de salida como vacía
    LET output_text = '';
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
		--***************************************************************************************
	--	SET DEBUG FILE TO "/home/sysdomi/sp_reemplazar_n_acentos_ob.out";
	--	TRACE ON;
	--***************************************************************************************

    -- Recorremos cada carácter de la cadena de entrada
    FOR i = 1 TO LENGTH(input_text)
        -- Obtenemos el carácter actual
        LET current_char = SUBSTRING(input_text FROM i FOR 1);

        -- Si el carácter es una letra con acento, lo reemplazamos

        IF current_char = 'á' THEN
            LET current_char = 'a';
        ELIF  current_char = 'é' THEN
            LET current_char = 'e';
        ELIF  current_char = 'í' THEN
            LET current_char = 'i';
        ELIF  current_char = 'ó' THEN
            LET current_char = 'o';
        ELIF  current_char = 'ú' THEN
            LET current_char = 'u';
        ELIF  current_char = 'Á' THEN
            LET current_char = 'A';
        ELIF  current_char = 'É' THEN
            LET current_char = 'E';
        ELIF  current_char = 'Í' THEN
        ...
        ELIF  current_char = 'Ú' THEN
            LET current_char = 'U';
        ELIF  current_char = 'ñ' THEN
            LET current_char = '#';
        ELIF  current_char = 'Ñ' THEN
            LET current_char = '#';
        -- Si el carácter es un signo de puntuación, lo omitimos
        ELIF  current_char IN ('.', ',', ';', ':', '!', '?', '-', '(', ')') THEN
            CONTINUE;
        -- Si el carácter está en la lista de caracteres válidos, lo mantenemos
        ELIF  instr(valid_chars, current_char) > 0 THEN
            LET current_char = current_char;
        -- Si no es válido, lo reemplazamos con '#'
        ELSE
            LET current_char = '#';
        END IF;

        -- Concatenamos el carácter a la cadena de salida
        LET output_text = output_text || current_char;
    END FOR;
...
'DESCRIPCION: 	Remplazo de acentos por letra correspondiente sin acento, Ñs por # y otros caracteres especiales.',
'FECHA: 		04/11/2024',
'BD: 			BDIDOMI';
