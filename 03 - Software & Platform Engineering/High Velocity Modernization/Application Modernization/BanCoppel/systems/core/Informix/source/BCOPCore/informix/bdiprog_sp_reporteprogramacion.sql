CREATE PROCEDURE "informix".sp_reporteprogramacion(p_tiporeporte CHAR(2),p_numcte CHAR(20),p_cve_pago CHAR(2),p_fecha_inicio DATE,p_fecha_fin DATE)
	...
	--Fecha: 15/09/2010
	--Se agrega parámetro de salida sucursal el cual se compone del numero de sucursal mas la descripción de la sucursal.
	--******************************************************
	...
	LET v_sNumSucursal = '';
	LET v_sDescSucursal = '';
		--SET DEBUG FILE TO "/tmp/SP_ReporteProgramacionuno.out";
		--TRACE ON;
	BEGIN
...
--- Se busca la convinacion de parametros para realizar el reporte
...
--Resultado de puros 000
LET v_dFechaBandera = p_fecha_inicio;
LET v_sCiclo = 'F';
--Busqueda con todos los parametros
IF (v_sBandera = '1111') OR (v_sBandera = '2111') OR (v_sBandera = '3111') OR (v_sBandera = '4111') THEN
	--Sa valida que nos datos de entrada sean diferente a blanco o nulo
	IF(NVL(p_numcte,'') <> '') AND (NVL(p_cve_pago,'') <> '') AND (NVL(p_fecha_inicio,'') <> '') AND (NVL(p_fecha_fin,'') <> '') THEN
		--Se valida que el cliente exista en latabal de clientes
		IF EXISTS(SELECT empresa FROM bdinteg:si_cliente WHERE numcte = p_numcte) THEN
			--Se valida que el cliente tenga pagos pendientes
			IF EXISTS(SELECT descripcion FROM bdiprog:pp_pagoprog WHERE num_cte = p_numcte) THEN
					...
					IF p_fecha_inicio <= p_fecha_fin THEN
						...
						ELSE
							...
							RETURN v_sCodRet,v_sMensajeRet,v_dFechaSoli,v_dFechaApli,v_sSucursal,v_sPeridoDesc,v_sMedioDesc,v_sCuentaO,v_sCuentaD,v_mMonto,v_sPagoDesc,
											v_sAviso,v_sEstadoDesc,v_sNumPago,v_sRechaDesc,v_dFechaCan;
						END IF;
							--Reporte de pagos programados por busqueda de clietne, clave de programacion y rango de fechas
							IF v_sBandera = '1111' THEN
								FOREACH
										SELECT b.fecha_insert,b.fecha_prog,a.cve_programa,a.cve_canal,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.estado,b.consecutivo,a.cve_pagoprog,a.user_insert
										...
										WHERE a.num_cte = p_numcte
										AND b.fecha_prog between p_fecha_inicio and p_fecha_fin
								...
								END FOREACH;
							END IF;
							--Reporte depagos realizados por busqueda de clietne, clave de programacion y rango de fechas
							IF v_sBandera = '2111' THEN
								FOREACH
									SELECT b.fecha_insert,b.fecha_aplic,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.consecutivo,a.cve_pagoprog
									INTO v_dFechaSoli, v_dFechaApli,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sNumPago,v_sNumaviso
									FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
									WHERE a.num_cte = p_numcte
									AND b.fecha_aplic between p_fecha_inicio and p_fecha_fin
									AND a.cve_pago = p_cve_pago
									AND b.estado = '05'
								...
								END FOREACH;
							END IF;
							--Reporte depagos rechazados por busqueda de clietne, clave de programacion y rango de fechas
							IF v_sBandera = '3111' THEN
								FOREACH
									SELECT b.fecha_insert,b.fecha_prog,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.consecutivo,b.cve_rechazo,a.cve_pagoprog
								...
								END FOREACH;
							END IF;
							--Reporte depagos cancelados por busqueda de clietne, clave de programacion y rango de fechas
							IF v_sBandera = '4111' THEN
								FOREACH
									SELECT b.fecha_insert,b.fecha_cancela,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.consecutivo,a.cve_pagoprog
									INTO v_dFechaSoli, v_dFechaCan,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sNumPago,v_sNumaviso
									FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
									WHERE a.num_cte = p_numcte
									AND b.fecha_cancela between p_fecha_inicio and p_fecha_fin
									AND a.cve_pago = p_cve_pago
									AND b.estado = '02'
				...
				END IF;
			ELSE
				--Se le informa que el cliente no tiene pagos programados
				SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet  FROM bdiprog:pp_mensajes WHERE cve_mensaje = '98';
				RETURN v_sCodRet,v_sMensajeRet,v_dFechaSoli,v_dFechaApli,v_sSucursal,v_sPeridoDesc,v_sMedioDesc,v_sCuentaO,v_sCuentaD,v_mMonto,v_sPagoDesc,
						v_sAviso,v_sEstadoDesc,v_sNumPago,v_sRechaDesc,v_dFechaCan;
			END IF;
		ELSE
			--SE informa que el cliente no existe en la tabla de clientes
			SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet  FROM bdiprog:pp_mensajes WHERE cve_mensaje = '04';
			RETURN v_sCodRet,v_sMensajeRet,v_dFechaSoli,v_dFechaApli,v_sSucursal,v_sPeridoDesc,v_sMedioDesc,v_sCuentaO,v_sCuentaD,v_mMonto,v_sPagoDesc,
					v_sAviso,v_sEstadoDesc,v_sNumPago,v_sRechaDesc,v_dFechaCan;
		END IF;
	ELSE
		--SE le informa que el los datos son invalidos
		SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet  FROM bdiprog:pp_mensajes WHERE cve_mensaje = '01';
		RETURN v_sCodRet,v_sMensajeRet,v_dFechaSoli,v_dFechaApli,v_sSucursal,v_sPeridoDesc,v_sMedioDesc,v_sCuentaO,v_sCuentaD,v_mMonto,v_sPagoDesc,
				v_sAviso,v_sEstadoDesc,v_sNumPago,v_sRechaDesc,v_dFechaCan;
	END IF;
END IF;
--Consulta por num empleado y clave de PAGO
IF (v_sBandera = '1110') OR (v_sBandera = '2110') OR (v_sBandera = '3110') OR (v_sBandera = '4110') THEN
	--Sa valida que nos datos de entrada sean diferente a blanco o nulo
	IF(NVL(p_numcte,'') <> '') AND (NVL(p_cve_pago,'') <> '')  THEN
		--Se valida que el cliente exista en latabal de clientes
		IF EXISTS(SELECT empresa FROM bdinteg:si_cliente WHERE numcte = p_numcte) THEN
			--Se valida que el cliente tenga pagos pendientes
			IF EXISTS(SELECT descripcion FROM bdiprog:pp_pagoprog WHERE num_cte = p_numcte) THEN
				...
				IF EXISTS(SELECT descripcion FROM bdiprog:pp_pagoprog WHERE cve_pago = p_cve_pago ) THEN
					--Se valida que existan pagos para ese rango de fechas para ese cliente y para esa clave de pago
						IF EXISTS(SELECT  descripcion  FROM bdiprog:pp_pagoprog  WHERE num_cte = p_numcte  AND cve_pago = p_cve_pago) THEN
							--Reporte de pagos programados por busqueda de clietne, clave de programacion
							IF v_sBandera = '1110' THEN
								FOREACH
									SELECT b.fecha_insert,b.fecha_prog,a.cve_programa,a.cve_canal,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.estado,b.consecutivo,a.cve_pagoprog,a.user_insert
									INTO v_dFechaSoli, v_dFechaApli,v_sPerioPago,v_sMedioSoli,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sEstatus,v_sNumPago,v_sNumaviso,v_sUser
									FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
								...
								END FOREACH;
							END IF;
							--Reporte depagos realizados por busqueda de clietne, clave de programacion
							IF v_sBandera = '2110' THEN
								FOREACH
									SELECT b.fecha_insert,b.fecha_aplic,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.consecutivo
									INTO v_dFechaSoli, v_dFechaApli,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sNumPago
									FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
									WHERE a.num_cte = p_numcte
									AND a.cve_pago = p_cve_pago
									AND b.estado = '05'
								...
								END FOREACH;
							END IF;
							--Reporte depagos rechazados por busqueda de clietne, clave de programacion
							IF v_sBandera = '3110' THEN
								FOREACH
									SELECT b.fecha_insert,b.fecha_prog,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.consecutivo,b.cve_rechazo,a.cve_pagoprog
									INTO v_dFechaSoli, v_dFechaApli,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sNumPago,v_sCausaRe,v_sNumaviso
									FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
								...
								END FOREACH;
							END IF;
							--Reporte depagos cancelados por busqueda de clietne, clave de programacion
							IF v_sBandera = '4110' THEN
								FOREACH
									SELECT b.fecha_insert,b.fecha_cancela,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.consecutivo,a.cve_pagoprog
									INTO v_dFechaSoli, v_dFechaCan,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sNumPago,v_sNumaviso
									FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
							...
							END IF;
						ELSE
							--se informa que no existen pagos programados con los datos proporcionados
							SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet  FROM bdiprog:pp_mensajes WHERE cve_mensaje = '94';
							RETURN v_sCodRet,v_sMensajeRet,v_dFechaSoli,v_dFechaApli,v_sSucursal,v_sPeridoDesc,v_sMedioDesc,v_sCuentaO,v_sCuentaD,v_mMonto,v_sPagoDesc,
								v_sAviso,v_sEstadoDesc,v_sNumPago,v_sRechaDesc,v_dFechaCan;
						END IF;
	...
	END IF;
END IF;
IF (v_sBandera = '1101') OR (v_sBandera = '2101') OR (v_sBandera = '3101') OR (v_sBandera = '4101') THEN
	--Sa valida que nos datos de entrada sean diferente a blanco o nulo
	IF(NVL(p_numcte,'') <> '') AND  (NVL(p_fecha_inicio,'') <> '') AND (NVL(p_fecha_fin,'') <> '') THEN
		--Se valida que el cliente exista en latabal de clientes
		IF EXISTS(SELECT empresa FROM bdinteg:si_cliente WHERE numcte = p_numcte) THEN
			--Se valida que el cliente tenga pagos pendientes
			IF EXISTS(SELECT descripcion FROM bdiprog:pp_pagoprog WHERE num_cte = p_numcte) THEN
				...
				IF p_fecha_inicio <= p_fecha_fin THEN
					...
					ELSE
						SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet  FROM bdiprog:pp_mensajes WHERE cve_mensaje = '94';
						RETURN v_sCodRet,v_sMensajeRet,v_dFechaSoli,v_dFechaApli,v_sSucursal,v_sPeridoDesc,v_sMedioDesc,v_sCuentaO,v_sCuentaD,v_mMonto,v_sPagoDesc,
							v_sAviso,v_sEstadoDesc,v_sNumPago,v_sCausaRe,v_dFechaCan;
					END IF;

						--Reporte de pagos programados por busqueda de clietne y rango de fechas
					IF v_sBandera = '1101' THEN
						FOREACH
							SELECT b.fecha_insert,b.fecha_prog,a.cve_programa,a.cve_canal,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.estado,b.consecutivo,a.cve_pagoprog,a.user_insert
							INTO v_dFechaSoli, v_dFechaApli,v_sPerioPago,v_sMedioSoli,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sEstatus,v_sNumPago,v_sNumaviso,v_sUser
							FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
							...
							--se obtiene las claves nofitica
							SELECT cve_notifica,cve_notifica_emi
							INTO v_scve_notifica,v_scve_notifica_emi
							FROM bdiprog:pp_pagoprog
							WHERE num_cte = p_numcte
						...
						END FOREACH;
					END IF;
					--Reporte de pagos realizados por busqueda de clietne y rango de fechas
					IF v_sBandera = '2101' THEN
						FOREACH
							SELECT b.fecha_insert,b.fecha_aplic,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.consecutivo, a.cve_pagoprog
							INTO v_dFechaSoli, v_dFechaApli,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sNumPago,v_sNumaviso
							FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
						...
						END FOREACH;
					END IF;
					--Reporte depagos rechazados por busqueda de clietne y rango de fechas
					IF v_sBandera = '3101' THEN
						FOREACH
							SELECT b.fecha_insert,b.fecha_prog,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.consecutivo,b.cve_rechazo,a.cve_pagoprog
							INTO v_dFechaSoli, v_dFechaApli,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sNumPago,v_sCausaRe,v_sNumaviso
							FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
						...
						END FOREACH;
					END IF;
					--Reporte depagos cancelados por busqueda de clietne  y rango de fechas
					IF v_sBandera = '4101' THEN
						FOREACH
							SELECT b.fecha_insert,b.fecha_cancela,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.consecutivo,a.cve_pagoprog
							INTO v_dFechaSoli, v_dFechaCan,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sNumPago,v_sNumaviso
							FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
	...
	END IF;
END IF;
IF (v_sBandera = '1100') OR (v_sBandera = '2100') OR (v_sBandera = '3100') OR (v_sBandera = '4100') THEN
	--Sa valida que nos datos de entrada sean diferente a blanco o nulo
	IF(NVL(p_numcte,'') <> '') THEN
		...
		IF EXISTS(SELECT empresa FROM bdinteg:si_cliente WHERE numcte = p_numcte) THEN
			--Se valida que el cliente tenga pagos pendientes
			IF EXISTS(SELECT descripcion FROM bdiprog:pp_pagoprog WHERE num_cte = p_numcte) THEN
					--Reporte de pagos programados por busqueda de clietne
						IF v_sBandera = '1100' THEN
							FOREACH
								SELECT b.fecha_insert,b.fecha_prog,a.cve_programa,a.cve_canal,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.estado,b.consecutivo,a.cve_pagoprog,a.user_insert
								INTO v_dFechaSoli, v_dFechaApli,v_sPerioPago,v_sMedioSoli,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sEstatus,v_sNumPago,v_sNumaviso,v_sUser
								FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
							...
							END FOREACH;
						END IF;
						--Reporte depagos realizados por busqueda de clietne,
						IF v_sBandera = '2100' THEN
							FOREACH
								SELECT b.fecha_insert,b.fecha_aplic,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.consecutivo,a.cve_pagoprog
								INTO v_dFechaSoli, v_dFechaApli,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sNumPago,v_sNumaviso
								FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
							...
							END FOREACH;
						END IF;
						--Reporte depagos rechazados por busqueda de clietne
						IF v_sBandera = '3100' THEN
							FOREACH
								SELECT b.fecha_insert,b.fecha_prog,a.cuenta_origen,a.cuenta_destino,a.importe,a.cve_pago,b.consecutivo,b.cve_rechazo,a.cve_pagoprog
								INTO v_dFechaSoli, v_dFechaApli,v_sCuentaO,v_sCuentaD,v_mMonto,v_sTipoPago,v_sNumPago,v_sCausaRe,v_sNumaviso
								FROM bdiprog:pp_pagoprog AS a INNER JOIN bdiprog:pp_pagospend AS b ON a.cve_pagoprog = b.cve_pagoprog
...
RETURN v_sCodRet,v_sMensajeRet,v_dFechaSoli,v_dFechaApli,v_sSucursal,v_sPeridoDesc,v_sMedioDesc,v_sCuentaO,v_sCuentaD,v_mMonto,v_sPagoDesc,
		v_sAviso,v_sEstadoDesc,v_sNumPago,v_sRechaDesc,v_dFechaCan;
END IF;
--Se le informa que los daatos osn invalidos
ELSE
SELECT cod_ret,desc_mensaje INTO v_sCodRet, v_sMensajeRet  FROM bdiprog:pp_mensajes WHERE cve_mensaje = '127';
RETURN v_sCodRet,v_sMensajeRet,v_dFechaSoli,v_dFechaApli,v_sSucursal,v_sPeridoDesc,v_sMedioDesc,v_sCuentaO,v_sCuentaD,v_mMonto,v_sPagoDesc,
		v_sAviso,v_sEstadoDesc,v_sNumPago,v_sRechaDesc,v_dFechaCan;
END IF;
IF v_sAviso = '' THEN
