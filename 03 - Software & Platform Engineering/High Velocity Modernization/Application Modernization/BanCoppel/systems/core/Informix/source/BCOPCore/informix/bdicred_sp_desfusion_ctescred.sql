CREATE PROCEDURE "informix".sp_desfusion_ctescred(pClienteCorrecto CHAR(20),pClienteInCorrecto CHAR(20),pUsuario CHAR(8))
	RETURNING 
	...
	DEFINE cTipo_ingreso CHAR(1);
	DEFINE iSecuencia2	SMALLINT;
	...
	BEGIN	
		ON EXCEPTION SET iSql_err
			IF iSql_err <> 0 THEN
				IF EXISTS (SELECT tabname FROM sysmaster:systabnames WHERE tabname = 'tabla_tel_temp') THEN
					DROP TABLE  tabla_tel_temp;				END IF;
				IF EXISTS (SELECT tabname FROM sysmaster:systabnames WHERE tabname = 'tabla_mail_temp') THEN
					DROP TABLE  tabla_mail_temp;				END IF;
				LET cCodret = iSql_err;
				RETURN NVL(TRIM(cCodret),''), TRIM(cDescErr);
			END IF;
		END EXCEPTION;
		
		--RUTA Y NOMBRE DONDE SE GENERARÃ LO QUE REALIZÓ EL PROCEDIMIENTO
		--SET DEBUG FILE TO '/informix/rmarquez/rqi64080/sp_desfusion_ctescred.out';
		--TRACE ON;
...
-- ----------------------------------------------------------------------------------------------------------------- --		
		-- CUENTAS DE CREDITO:
		FOREACH
			SELECT NVL(a.num_credito,'') 
			INTO cNum_credito
			FROM bdinteg: "informix".si_fusmaecred a, 
				"informix".sd_maecred b
			WHERE a.numcte = pClienteInCorrecto
			AND a.num_credito = b.num_credito
			
			LET cDescErr = 'si_fusmaecred';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE "informix".sd_maecred SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito;
				
				-- SE VERIFICA SI HAY INFORMACION O NO DE LA CONSULTA
				...
				VALUES('CUENTAS DE CREDITO','si_fusmaecred',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --
		-- AUMENTO LINEA CRED:
		FOREACH
			SELECT {+INDEX (bdinteg:"informix".si_fusbitacora_aumlincred idx_fusbitacora_status)} {+INDEX (bdicred:"informix".sd_bitacora_aumlincred idx_bitacora_status)}  DISTINCT NVL(a.num_solicitud,'') 
			INTO cCuenta
			FROM bdinteg:"informix".si_fusbitacora_aumlincred a, 
		   ...
		   LET cDescErr = 'si_fusbitacora_aumlincred';
			
			IF cCuenta <> '' THEN
				LET cDet_mov = TRIM(cCuenta)||'|'||TRIM(pClienteInCorrecto)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE {+INDEX (bdicred:"informix".sd_bitacora_aumlincred idx_bitacora_status)} bdicred:"informix".sd_bitacora_aumlincred 
				SET numcte = pClienteInCorrecto 
				WHERE num_solicitud = cCuenta 
					AND empresa = '001' 
					AND numcte = pClienteCorrecto 
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('AUMENTO LINEA CRED','si_fusbitacora_aumlincred',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --
		-- TARJETAS CREDITO:
		FOREACH
            SELECT {+INDEX (bdinteg:"informix".si_fustarjetacred pk_fustarjetacred)} NVL(a.num_credito,''),a.secuencia,a.num_tarjeta 
            INTO cNum_credito,iSecuencia,cNum_tarjeta
            FROM bdinteg: "informix".si_fustarjetacred a, 
              "informix".sd_tarjeta b
            WHERE  a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			
			LET cDescErr = 'si_fustarjetacred';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto)||'|'||iSecuencia||'|'||TRIM(cNum_tarjeta);
				
				UPDATE "informix".sd_tarjeta SET numcte = pClienteInCorrecto WHERE num_tarjeta = cNum_tarjeta;
				
				-- SE VERIFÍA SI HAY INFORMACIÓ O NO DE LA CONSULTA
				IF DBINFO('sqlca.sqlerrd2') = 0 THEN		
					LET cCodret = '000001'; 
					RETURN cCodRet, cDescErr;
				END IF;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('TARJETAS CREDITO','si_fustarjetacred',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
		...
		LET cNum_tarjeta = '';
-- ----------------------------------------------------------------------------------------------------------------- --
		-- AUMENTO DE LINEA DE CREDITO 2:
		FOREACH
            SELECT {+INDEX (bdinteg:"informix".si_fusmaecredcont idx_maecontcte)} NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fusmaecredcont a, 
              "informix".sd_maecredcont b
            WHERE  a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			AND a.empresa = '001'
			
			
			LET cDescErr = 'si_fusmaecredcont';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE {+INDEX (bdicred:"informix".sd_maecredcont 189_600)} "informix".sd_maecredcont SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito AND fecha IS NOT NULL AND empresa = '001';
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('SI_FUSMAECREDCONT','si_fusmaecredcont',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --		
		-- INTERCARD: 
		FOREACH
			SELECT {+INDEX (bdinteg:"informix".si_fusintercardtarjeta idx_intnumcte)} NVL(c.num_credito,''),NVL(a.numtarjeta,'')		 
			INTO cNum_credito,cNum_tarjeta
			FROM bdinteg:"informix".si_fusintercardtarjeta a,
			...
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto)||'|'||TRIM(cNum_tarjeta);
				
				UPDATE intercard:"informix".tarjeta SET numcliente = pClienteInCorrecto WHERE numtarjeta = cNum_tarjeta;
...
-- ----------------------------------------------------------------------------------------------------------------- --
		-- ENCABEZADO ESTADOS DE CUENTA CRÉITO:
		FOREACH
            SELECT {+INDEX (bdinteg:"informix".si_fusencabezado_edocta pk_encacred)} NVL(a.num_credito,''),NVL(a.num_tarjeta,'')
            INTO cNum_credito,cNum_tarjeta
            FROM bdinteg: "informix".si_fusencabezado_edocta a, 
               "informix".sd_encabezado_edocta b
            WHERE  a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			AND a.num_tarjeta = b.num_tarjeta
			
			LET cDescErr = 'si_fusencabezado_edocta';			
			
			IF cNum_tarjeta <> '' THEN
				LET cDet_mov = TRIM(cNum_tarjeta)||'|'||TRIM(pClienteCorrecto)||'|'||TRIM(cNum_credito);
				
				UPDATE "informix".sd_encabezado_edocta SET numcte = pClienteInCorrecto WHERE num_tarjeta = cNum_tarjeta;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('SD_ENCABEZADO_EDOCTA','si_fusencabezado_edocta',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
		...
		LET cNum_tarjeta = '';		
-- ----------------------------------------------------------------------------------------------------------------- --				
		-- REESTRUCTURA DE CRÉITO:
		FOREACH
            SELECT {+INDEX (bdinteg:"informix".si_fusmaecredcrd pk_fusmaecredcrd)} NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM  bdinteg: "informix".si_fusmaecredcrd a, 
             "informix".sd_maecredcrd b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			
			LET cDescErr = 'si_fusmaecredcrd';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE "informix".sd_maecredcrd SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('SD_MAECREDCRD','si_fusmaecredcrd',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		-- SD_MAECREDREVCRD:
		FOREACH
            SELECT {+INDEX (bdicred:sd_maecredrevcrd 1346_6799)} NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fusmaecredrevcrd a, 
            "informix".sd_maecredrevcrd b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			AND b.empresa = '001'
			
			LET cDescErr = 'si_fusmaecredrevcrd';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE {+INDEX (bdicred:sd_maecredrevcrd 1346_6799)} "informix".sd_maecredrevcrd SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito AND empresa = '001';
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('SD_MAECREDREVCRD','si_fusmaecredrevcrd',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fusmaecredcontcrd a, 
            "informix".sd_maecredcontcrd b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			
			LET cDescErr = 'si_fusmaecredcontcrd';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE {+INDEX (bdicred:"informix".sd_maecredcontcrd 1344_6755)} "informix".sd_maecredcontcrd SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito AND empresa = '001' AND fecha IS NOT NULL;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('SD_MAECREDCONTCRD','si_fusmaecredcontcrd',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fusseguimientocrd  a, 
             "informix".sd_seguimientocrd b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			AND b.empresa = '001'
			
			LET cDescErr = 'si_fusseguimientocrd';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE "informix".sd_seguimientocrd SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito AND b.empresa = '001';
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('SD_SEGUIMIENTOCRD','si_fusseguimientocrd',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(b.num_credito,'')  --SE VOLTEARON LAS TABLAS YA QUE ESTABA MAL LA CONSULTA
            INTO cNum_credito
            FROM bdinteg:"informix".si_fusencabezado_edoctacrd a,    
             "informix".sd_encabezado_edoctacrd b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			
			LET cDescErr = 'si_fusencabezado_edoctacrd';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE "informix".sd_encabezado_edoctacrd SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('SD_ENCABEZADO_EDOCTACRD','si_fusencabezado_edoctacrd',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		-- GRUPO_CREDITO:
		FOREACH
            SELECT NVL(b.num_credito,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fusgrupo_credito a, 
            "informix".sd_grupo_credito b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
             
			
			LET cDescErr = 'si_fusgrupo_credito';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(pClienteInCorrecto)||'|'||TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
																							
				UPDATE "informix".sd_grupo_credito SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('GRUPO_CREDITO','si_fusgrupo_credito',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
		...
		LET cNum_credito = '';	
-- ----------------------------------------------------------------------------------------------------------------- --				
		-- GRUPO CRÉITO (HISTORIAL):
		FOREACH
            SELECT NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM   bdinteg: "informix".si_fusgrupo_credito_his a, 
             "informix".sd_grupo_credito_his b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			AND b.numcte IS NOT NULL -- SE AGREGA CAMPO PARA QUE TOME INDICE
			
			LET cDescErr = 'si_fusgrupo_credito_his';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(pClienteInCorrecto)||'|'||TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE "informix".sd_grupo_credito_his SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito AND numcte IS NOT NULL; -- SE AGREGA CAMPO PARA QUE TOME INDICE
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('GRUPO_CREDITO_HIS','si_fusgrupo_credito_his',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fusmaecred_vendida a, 
           -- "informix".sd_maecred_vendida b
		    "informix".sd_maecred_vend_total b
            WHERE b.fecha IS NOT NULL
			AND a.num_credito = b.num_credito 
			AND a.numcte = pClienteInCorrecto 
           
			--AND b.numcte IS NOT NULL
			 -- SE AGREGA CAMPO PARA QUE TOME INDICE
			
			LET cDescErr = 'si_fusmaecred_vendida';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(pClienteInCorrecto)||'|'||TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
			
				IF EXISTS (Select num_credito FROM sd_maecred_vendida WHERE num_credito = cNum_credito AND numcte IS NOT NULL AND fecha IS NOT NULL) THEN 
					--UPDATE {+INDEX (bdicred:"informix".sd_maecred_vendida idx_sd_maecred_vendida)} "informix".sd_maecred_vendida SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito AND numcte IS NOT NULL AND fecha IS NOT NULL; -- SE AGREGA CAMPO PARA QUE TOME INDICE
					UPDATE "informix".sd_maecred_vendida SET numcte = pClienteInCorrecto 
							WHERE num_credito = cNum_credito AND numcte IS NOT NULL AND fecha IS NOT NULL; -- SE AGREGA CAMPO PARA QUE TOME INDICE
				END IF;
				
				
				IF EXISTS (Select num_credito FROM sd_maecred_vend_total WHERE fecha IS NOT NULL AND num_credito = cNum_credito AND numcte IS NOT NULL) THEN 
					UPDATE bdicred:"informix".sd_maecred_vend_total SET numcte = pClienteInCorrecto 
							WHERE fecha IS NOT NULL AND num_credito = cNum_credito AND numcte IS NOT NULL;
				END IF;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('CARTERA VENDIDA','si_fusmaecred_vendida',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fusmaecredcont_apoyo a, 
             "informix".sd_maecredcont_apoyo b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			AND b.numcte = pClienteCorrecto
			
			LET cDescErr = 'si_fusmaecredcont_apoyo';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(pClienteInCorrecto)||'|'||TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE "informix".sd_maecredcont_apoyo SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito AND numcte = pClienteCorrecto;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('CREDITO APOYO','si_fusmaecredcont_apoyo',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(b.num_credito,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fusmaecredcrd_vendida a, 
            --"informix".sd_maecredcrd_vendida b
			"informix".sd_maecredcrd_vend_total b
            WHERE b.fecha IS NOT NULL
			AND a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			AND b.numcte IS NOT NULL
			...
			LET cDescErr = 'si_fusmaecredcrd_vendida';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(pClienteInCorrecto)||'|'||TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				--verifica que exista y si existe lo actualiza
				IF EXISTS (select num_credito FROM sd_maecredcrd_vendida WHERE num_credito = cNum_credito AND numcte = pClienteCorrecto) THEN 
					UPDATE "informix".sd_maecredcrd_vendida SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito AND numcte = pClienteCorrecto;
				END IF;
				
				
				--tablas nuevas
				IF EXISTS (select num_credito FROM sd_maecredcrd_vend_total WHERE fecha IS NOT NULL AND num_credito = cNum_credito AND numcte = pClienteCorrecto ) THEN 
					UPDATE "informix".sd_maecredcrd_vend_total SET numcte = pClienteInCorrecto WHERE fecha IS NOT NULL AND num_credito = cNum_credito AND numcte = pClienteCorrecto;
				END IF;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('CRD VENDIDA','si_fusmaecredcrd_vendida',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(a.num_credito,'') 
            INTO cNum_credito
			FROM bdinteg: "informix".si_fusmaecredrev a, 
             "informix".sd_maecredrev  b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			AND b.numcte = pClienteCorrecto
			
			LET cDescErr = 'si_fusmaecredrev';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(pClienteInCorrecto)||'|'||TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE "informix".sd_maecredrev SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito AND numcte = pClienteCorrecto;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('SD_MAECREDREV','si_fusmaecredrev',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fusrep_cart_quebrantar a, 
              bdicobranza:"informix".cb_rep_cart_quebrantar b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			
			LET cDescErr = 'si_fusrep_cart_quebrantar';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE bdicobranza:"informix".cb_rep_cart_quebrantar SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('CART_QUEBRANTAR','si_fusrep_cart_quebrantar',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		-- SOLICITUDES MC:
		FOREACH
            SELECT NVL(a.num_solicitud,'') 
            INTO cNum_sol
            FROM bdinteg: "informix".si_fussolicitudes_mc a, 
             bdisolic:"informix".ss_solicitudes_mc b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_solicitud = b.num_solicitud
			
			LET cDescErr = 'si_fussolicitudes_mc';
			
			IF cNum_sol <> '' THEN
				LET cDet_mov = TRIM(cNum_sol)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE bdisolic:"informix".ss_solicitudes_mc SET numcte = pClienteInCorrecto WHERE num_solicitud = cNum_sol AND numcte = pClienteCorrecto;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('SOLICITUDES_MC','si_fussolicitudes_mc',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(a.num_solicitud,'') 
            INTO cNum_sol
            FROM bdinteg: "informix".si_fussolicitudes_sic  a, 
             bdisolic:"informix".ss_solicitudes_sic b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_solicitud = b.num_solicitud
			AND b.numcte = pClienteCorrecto
			
			LET cDescErr = 'si_fussolicitudes_sic';
			
			IF cNum_sol <> '' THEN
				LET cDet_mov = TRIM(cNum_sol)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE bdisolic:"informix".ss_solicitudes_sic SET numcte = pClienteInCorrecto WHERE num_solicitud = cNum_sol AND numcte = pClienteCorrecto;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('SOLICITUDES_SIC','si_fussolicitudes_sic',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(a.num_solicitud,'') 
            INTO cNum_sol
            FROM bdinteg: "informix".si_fussolicitudes_cac a, 
				bdisolic:"informix".ss_solicitudes_cac b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_solicitud = b.num_solicitud
			AND b.numcte = pClienteCorrecto
			
			LET cDescErr = 'si_fussolicitudes_cac';
			
			IF cNum_sol <> '' THEN
				LET cDet_mov = TRIM(cNum_sol)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE bdisolic:"informix".ss_solicitudes_cac SET numcte = pClienteInCorrecto WHERE num_solicitud = cNum_sol AND numcte = pClienteCorrecto;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('SOLICITUDES_CAC','si_fussolicitudes_cac',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fuscamp_inactiv_nuncas a, 
             "informix".sd_camp_inactiv_nuncas  b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			
			LET cDescErr = 'si_fuscamp_inactiv_nuncas';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE "informix".sd_camp_inactiv_nuncas SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('CAMPAÑS INACTIVAS','si_fuscamp_inactiv_nuncas',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		-- COBRANZA:
		FOREACH
            SELECT NVL(a.numcuenta,'') 
            INTO cNum_credito
            FROM  bdinteg: "informix".si_fuscompac a, 
             bdicobranza:"informix".cb_compac b 
            WHERE a.numcliente = pClienteInCorrecto
            AND a.numcuenta = b.numcuenta
			
			LET cDescErr = 'si_fuscompac';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE bdicobranza:"informix".cb_compac SET numcliente = pClienteInCorrecto WHERE numcuenta = cNum_credito;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('COMPAC COBRANZA','si_fuscompac',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(a.numcuenta,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fuscompac_his a, 
             bdicobranza:"informix".cb_compac_his b
            WHERE a.numcliente = pClienteInCorrecto
            AND a.numcuenta = b.numcuenta
			
			LET cDescErr = 'si_fuscompac_his';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE bdicobranza:"informix".cb_compac_his SET numcliente = pClienteInCorrecto WHERE numcuenta = cNum_credito;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('COMPAC COBRANZA HIS','si_fuscompac_his',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH
            SELECT NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fuscat_directorio_cte a, 
             bdicobranza:"informix".cb_cat_directorio_cte b
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			
			LET cDescErr = 'si_fuscat_directorio_cte';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE bdicobranza:"informix".cb_cat_directorio_cte SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('DIRECTORIO COBRANZA','si_fuscat_directorio_cte',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
		...
		LET cNum_credito = '';					
-- ----------------------------------------------------------------------------------------------------------------- --				
		-- COMPAC COBRANZA ERROR:
		FOREACH
            SELECT {+INDEX (bdicobranza:"informix".cb_compac_error idx_cb_compac_error)} NVL(a.numcuenta,'') 
            INTO cNum_credito
            FROM bdinteg: "informix".si_fuscompac_error a, 
            bdicobranza:"informix".cb_compac_error  b 
            WHERE a.numcliente = pClienteInCorrecto
            AND a.numcuenta = b.numcuenta
			AND b.numcliente = pClienteCorrecto
			
			LET cDescErr = 'si_fuscompac_error';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE {+INDEX (bdicobranza:"informix".cb_compac_error idx_cb_compac_error)} bdicobranza:"informix".cb_compac_error SET numcliente = pClienteInCorrecto WHERE numcuenta = cNum_credito AND numcliente = pClienteCorrecto;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('COMPAC COBRANZA ERROR','si_fuscompac_error',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
		...
		LET cNum_credito = '';	
-- ----------------------------------------------------------------------------------------------------------------- --				
		-- DIRECTORIO COBRANZA HISTORIAL:
		FOREACH
            SELECT DISTINCT NVL(a.num_credito,'') 
            INTO cNum_credito
            FROM  bdinteg: "informix".si_fuscat_directorio_cte_his a, 
             bdicobranza:"informix".cb_cat_directorio_cte_his  b 
            WHERE a.numcte = pClienteInCorrecto
            AND a.num_credito = b.num_credito
			AND b.numcte = pClienteCorrecto
			
			LET cDescErr = 'si_fuscat_directorio_cte_his';
			
			IF cNum_credito <> '' THEN
				LET cDet_mov = TRIM(cNum_credito)||'|'||TRIM(pClienteCorrecto);
				
				UPDATE bdicobranza:"informix".cb_cat_directorio_cte_his SET numcte = pClienteInCorrecto WHERE num_credito = cNum_credito AND numcte = pClienteCorrecto;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('DIRECTORIO COBRANZA HIS','si_fuscat_directorio_cte_his',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --						
		-- TELEFONOS:
		LET cDescErr = 'si_telefonos';
		--NUEVO PROCESO--
		IF EXISTS (SELECT tabname FROM sysmaster:systabnames WHERE tabname = 'tabla_tel_temp') THEN
			DROP TABLE  tabla_tel_temp;		ELSE
			--Crear la tabla temporal
			CREATE TEMP TABLE tabla_tel_temp(empresa CHAR(3) NOT NULL,numcte CHAR(20) NOT NULL, telefono CHAR(13) NOT NULL,
				tipo_tel       	SMALLINT,
				status_tel     	CHAR(1),
				secuencia      	SMALLINT,
				extension      	CHAR(5),
				...
				user_insert    	CHAR(8),
				movil_fijo     	CHAR(1) DEFAULT '0',
				status_stel    	CHAR(1) DEFAULT '',
				verificado     	CHAR(1) DEFAULT NULL,
				marcatel       	CHAR(1) DEFAULT NULL,
				fecha_actualiza	DATE DEFAULT NULL,
				tel_confirmado CHAR(1) DEFAULT '',
				fech_confirmado datetime YEAR to SECOND) WITH NO LOG;
		END IF;
		--Respaldar la informacion de si_telefonos en la tabla temporal
		INSERT INTO tabla_tel_temp(empresa, numcte,telefono,tipo_tel,status_tel,secuencia,extension,carrier,canal,contacto,cofetel,
		...
		--Elimino los registros de la tabla si_telefonos y si_telefonos_actual
		DELETE bdinteg:"informix".si_telefonos WHERE numcte = pClienteCorrecto;
		DELETE bdinteg:"informix".si_telefonos_actual WHERE numcte = pClienteCorrecto;
		
		--Restauro los registros que se pasaron en la tabla si_fustelefonos
		INSERT INTO bdinteg:"informix".si_telefonos (empresa, numcte,telefono,tipo_tel,status_tel,secuencia,extension,carrier,canal,contacto,cofetel,
		fecha_hora,user_insert, movil_fijo,status_stel, verificado, marcatel, fecha_actualiza, tel_confirmado, fech_confirmado)
		SELECT empresa, numcte,telefono,tipo_tel,status_tel,secuencia,extension,carrier,canal,contacto,cofetel,
		fecha_hora,user_insert, movil_fijo,status_stel, verificado, marcatel, fecha_actualiza, tel_confirmado, fech_confirmado
		FROM bdinteg:"informix".si_fustelefonos	
		WHERE numcte = pClienteInCorrecto
		OR numcte = pClienteCorrecto;
		
		IF DBINFO('sqlca.sqlerrd2') > 0 THEN
			FOREACH 
				SELECT secuencia,tipo_tel,telefono
				INTO iSecuencia,cTipo_tel,cTel
				FROM bdinteg:"informix".si_fustelefonos
				WHERE numcte = pClienteInCorrecto
				
				LET cDescErr = 'si_fustelefonos';
				
				LET cDet_mov = TRIM(cTel)||'|'||iSecuencia||'|'||cTipo_tel;
				
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('TELEFONOS','si_fustelefonos',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
			
				LET cDet_mov = "";
			END FOREACH
		END IF;
		
		--Obtengo los registros que fueron guardados en la tabla temporal, para verificar si existe
		--algun registro que se realizara despues de haber fusionado los clientes
		FOREACH
			SELECT NVL(secuencia,0)
			INTO iSecuencia
			FROM tabla_tel_temp
			WHERE fecha_hora::date >= (SELECT fecha_insert FROM bdinteg:"informix".si_fusionaut WHERE cliente_tit = pClienteCorrecto and cliente_tras= pClienteInCorrecto and estatus = 1)
			--Si dicha secuencia es > 0, obtener el registro de la tabla temporal e insertarla en si_telefonos del cliente correcto
			IF iSecuencia > 0 THEN
				INSERT INTO bdinteg:"informix".si_telefonos (empresa, numcte,telefono,tipo_tel,status_tel,secuencia,extension,carrier,canal,contacto,cofetel,
				fecha_hora,user_insert, movil_fijo,status_stel, verificado, marcatel, fecha_actualiza, tel_confirmado, fech_confirmado)
				SELECT empresa,numcte,telefono,tipo_tel,status_tel,(SELECT MAX(secuencia) + 1 from bdinteg:"informix".si_telefonos WHERE numcte = pClienteCorrecto),extension,carrier,canal,contacto,cofetel,
				fecha_hora,user_insert, movil_fijo,status_stel, verificado, marcatel, fecha_actualiza, tel_confirmado, fech_confirmado
				FROM tabla_tel_temp
...
-- ----------------------------------------------------------------------------------------------------------------- --						
		-- CORREOS:		
		LET cDescErr = 'si_correos';
		--INICIO EL PROCESO DE CORREOS--
		IF EXISTS (SELECT tabname FROM sysmaster:systabnames WHERE tabname = 'tabla_mail_temp') THEN
			DROP TABLE  tabla_mail_temp;		ELSE
			--Crear la tabla temporal para correos
			CREATE TEMP TABLE tabla_mail_temp( 
			empresa      	CHAR(3),
			numcte       	CHAR(20),
			...
			fecha_hora   	CHAR(23),
			user_insert  	CHAR(8),
			valida_correo	CHAR(3),
			valido       	CHAR(1),
			fecha_valida datetime YEAR to SECOND) WITH NO LOG;
		END IF;
		--Guardo los datos de la tabla si_correos en una tabla temporal
		INSERT INTO tabla_mail_temp(empresa,numcte,correo_elec,tipo_correo,status_correo,secuencia,canal,fecha_hora, user_insert, valida_correo, valido, fecha_valida)
		...
		--Elimino la tabla si_correos
		DELETE bdinteg:"informix".si_correos WHERE numcte = pClienteCorrecto;
		
		--Restauro los registros de la tabla si_fuscorreos en la tabla si_correos
		INSERT INTO bdinteg:"informix".si_correos (empresa,numcte,correo_elec,tipo_correo,status_correo,secuencia,canal,fecha_hora, user_insert, valida_correo, valido, fecha_valida)
		SELECT empresa,numcte,correo_elec,tipo_correo,status_correo,secuencia,canal,fecha_hora, user_insert, valida_correo, valido, fecha_valida
		FROM bdinteg:"informix".si_fuscorreos	
		WHERE numcte = pClienteInCorrecto
		OR numcte = pClienteCorrecto;
		
		LET cDescErr = 'si_fuscorreos';
		
		IF DBINFO('sqlca.sqlerrd2') > 0 THEN
			FOREACH 
				SELECT secuencia,tipo_correo,correo_elec
				INTO iSecuencia,cTipo_correo,cCorreo
				FROM bdinteg:"informix".si_fuscorreos
				WHERE numcte = pClienteCorrecto
				
				LET cDescErr = 'si_fuscorreos';
				
				LET cDet_mov = TRIM(cCorreo)||'|'||iSecuencia||'|'||cTipo_correo;
				
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('EMAIL','si_fuscorreos',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
		...
		FOREACH
			SELECT NVL(secuencia,0)
			INTO iSecuencia
			FROM tabla_mail_temp
			WHERE TO_DATE(substr(fecha_hora,0,10),'%Y-%m-%d')::DATE >= (SELECT fecha_insert FROM bdinteg:"informix".si_fusionaut WHERE cliente_tit = pClienteCorrecto and cliente_tras= pClienteInCorrecto and estatus = 1)
			--Si dicha secuencia es > 0, obtener el registro de la tabla temporal e insertarla en si_telefonos del cliente correcto
			IF iSecuencia > 0 THEN
				INSERT INTO bdinteg:"informix".si_correos (empresa, numcte, correo_elec, tipo_correo, status_correo, secuencia, canal, fecha_hora, user_insert, valida_correo, valido, fecha_valida)
				SELECT empresa,numcte,correo_elec,tipo_correo,status_correo,(SELECT MAX(secuencia)+1 FROM bdinteg:"informix".si_correos WHERE numcte = pClienteCorrecto ) ,canal,fecha_hora, user_insert, valida_correo, valido, fecha_valida
				FROM tabla_mail_temp
				WHERE secuencia = iSecuencia;
			END IF;
		END FOREACH;
		...
		LET cTipo_correo = 0;
		LET cCorreo = "";
		--FIN DEL PROCESO DE CORREOS--
-- ----------------------------------------------------------------------------------------------------------------- --				
		-- ADICIONAL COPPEL:
		FOREACH
            SELECT {+INDEX (bdinteg:"informix".si_adiccoppel idx_adiccoppel)} a.numctecoppel,a.secuencia,a.tipotar 
            INTO cNum_cte,iSecuencia,cTipotar 
            FROM bdinteg: "informix".si_fusadiccoppel a,
                bdinteg:"informix".si_adiccoppel b
            WHERE a.numcte = pClienteInCorrecto 
            AND a.numctecoppel = b.numctecoppel
			AND b.numcte = pClienteCorrecto
			
			LET cDescErr = 'si_fusadiccoppel';
			
			IF cNum_cte <> '' THEN
				LET cDet_mov = TRIM(pClienteCorrecto)||'|'||iSecuencia||'|'||cTipotar;
				
				UPDATE {+INDEX (bdinteg:"informix".si_adiccoppel idx_adiccoppel)} bdinteg:"informix".si_adiccoppel SET numcte = pClienteInCorrecto WHERE numctecoppel = cNum_cte AND numcte = pClienteCorrecto;
				...
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('ADICIONAL COPPEL','si_fusadiccoppel',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH 
			...
			INTO iSecuencia,cTipo_dir
			FROM bdinteg:"informix".si_fusrefdirecciones
			WHERE numcte = pClienteInCorrecto
			
			LET cDescErr = 'si_fusrefdirecciones';
			
			INSERT INTO bdinteg:"informix".si_refdirecciones (numcte ,secuencia,tipo_dir,calle ,colonia,entre_calles,pais ,estado,ciudad,municipio,cod_postal,apart_postal,
			tipo_telef1,telefono1 ,tipo_telef2 ,telefono2 ,tipo_telef3,telefono3 ,extension ,estado_inegi,municipio_inegi,localidad_inegi ,
			numerociudad ,numeroextcalle,numerointcalle,departamento,numerocalle ,numerocolonia ,puntocardinal,unidadhabitac,manzana ,
			otros,andador,etapa,lote,edificio,entrada,observaciones,numcte_banco,user_insert,fecha_insert,ind_cofeteltel1,ind_cofeteltel2 ,
			ind_cofeteltel3,movil_fijo1 ,status_stel1,movil_fijo2 ,status_stel2 ,movil_fijo3 ,status_stel3)
			...
			LET cDet_mov = TRIM(pClienteCorrecto)||'|'||iSecuencia||'|'||cTipo_dir;
			
			INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
			VALUES('REFERENCIAS DIRECCIONES','si_fusrefdirecciones',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		...
		FOREACH 
			...
			INTO iSecuencia,cNum_sol
			FROM bdinteg:"informix".si_fusrefclientes
			WHERE numcte = pClienteInCorrecto
			
			LET cDescErr = 'si_fusrefclientes';
			
			INSERT INTO bdinteg:"informix".si_refclientes (empresa,num_solicitud,numcte,sucursal,secuencia,apell_paterno,apell_materno,nombre1,nombre2,rfc ,
			fecha_nac,curp,sexo,estado_civil,nacionalidad,no_fm3,codidentifi,numidentifi,pers_domicilio,email,parentesco,
			apellido_cas,numcte_ref,numcte_banco,user_insert,fecha_insert)
			SELECT empresa,num_solicitud,numcte,sucursal,secuencia,apell_paterno,apell_materno,nombre1,nombre2,rfc ,
			fecha_nac,curp,sexo,estado_civil,nacionalidad,no_fm3,codidentifi,numidentifi,pers_domicilio,email,parentesco,
			apellido_cas,numcte_ref,numcte_banco,user_insert,fecha_insert
			FROM bdinteg:"informix".si_fusrefclientes	
			WHERE numcte = pClienteInCorrecto
			...
			LET cDet_mov = TRIM(pClienteCorrecto)||'|'||iSecuencia||'|'||TRIM(cNum_sol);
			
			INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
			VALUES(' REFERENCIAS CLIENTES','si_fusrefclientes',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
-- ----------------------------------------------------------------------------------------------------------------- --				
		-- INGRESOS:
		FOREACH
            SELECT a.sec_ingreso,a.tipo_ingreso, b.numcte,a.fecha_insert
            INTO iSecuencia2,cTipo_ingreso, cNum_cte,cFecha
            FROM bdinteg: "informix".si_fusingresos a,
                bdinteg:"informix".si_ingresos b
            WHERE  a.numcte = pClienteInCorrecto
            AND a.sec_ingreso = b.sec_ingreso
			AND b.numcte = pClienteCorrecto
			AND a.fecha_insert = b.fecha_insert
			
			LET cDescErr = 'si_fusingresos';
			
			IF iSecuencia2 > 0 THEN
							
				LET cDet_mov = TRIM(pClienteCorrecto)||'|'||iSecuencia2||'|'||cTipo_ingreso;
				
				UPDATE bdinteg:"informix".si_ingresos 
				SET numcte = pClienteInCorrecto 
				WHERE sec_ingreso = iSecuencia2
				AND numcte = pClienteCorrecto
				AND fecha_insert = cFecha;				
								
				INSERT INTO bdinteg: "informix".si_logdesfusion (proceso,tabla,cliente_tit,cliente_tras,detalle_mov,fecha_hora,user_insert,fecha_insert)
				VALUES('INGRESOS','si_fusingresos',pClienteCorrecto,pClienteInCorrecto,cDet_mov,CURRENT,pUsuario,CURRENT);
...
'Autor: 94912599 ',
'Fecha: 06/01/2014',
'DescripciÃ³n: Se crea procedimiento para hacer la desfusion de un cliente en cuanto a cuentas de crÃ©dito',
'Sustento: Desfusion de Clientes v1.0.doc',
'Solicita: Armando Morales Barraza',
'BD:BDICRED';

CREATE PROCEDURE "informix".sp_ics_obligacion_cred_crd(p_num_ejecucion INTEGER)
	...
	DEFINE c_dia_inhabil1				DATE;
	DEFINE c_dia_inhabil2				DATE;
	...
	LET c_dia_inhabil1 					= mdy(12,25,year(today));
	LET c_dia_inhabil2 					= mdy(01,01,year(today));
	...
	--SET DEBUG FILE TO "/resplogifx/cobranza/obligacionCRD.out";
   -- TRACE ON;
	...
	BEGIN
		...
		ON EXCEPTION IN (-535)
			  ...
			  LET v_transaccion = 1;
			  INSERT INTO "informix".ics_control_errores(num_credito, numcte, num_producto,descripcion_error, proceso, fecha_insert)
			  VALUES( v_account_number, v_num_cliente, v_acc_customer_id, 'ERROR -535 OBLIGACIONES CRD',v_proceso, CURRENT);
			    COMMIT WORK;
				BEGIN WORK;
		...
		INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (iContador1, horaActual, 'IN_CICLO_OBLIGACION_CRD');
		
		
		BEGIN WORK;
		
			 LET v_transaccion = 1;
			 LET v_proceso = 'Creditos No Revolventes Obligación';
			...
			/*SELECT 
					numcte, num_producto, num_credito, sucursal, status_cred, 
					tasa_interes, bandera_ministra, fecha_apertura, balance, delinquency, principal_delinquent,
					interest_delinquent, collection_chargue, rfc
				/*INTO
					v_num_cliente, v_acc_customer_id, v_account_number, v_cust_branch_id, v_work_type, 
					v_interest_rate, v_payment_frequency_char, v_opening_date
				FROM ics_clientes  WHERE tipo_cred = '2'  AND ics_consecutivo BETWEEN v_valor_inicial AND v_valor_final
				INTO TEMP ics_obligaciones_crd WITH NO LOG;*/
				
			FOREACH WITH HOLD
				SELECT 
					numcte, num_producto, num_credito, sucursal, status_cred, tasa_interes, bandera_ministra, fecha_apertura,
					balance, delinquency, principal_delinquent,interest_delinquent, collection_chargue, rfc, pagos_vencidos
				INTO
					v_num_cliente, v_acc_customer_id, v_account_number, v_cust_branch_id, v_work_type, 
					v_interest_rate, v_payment_frequency_char, v_opening_date,v_balance, v_delinquency, v_principal_delinquent, 
					v_interest_delinquent, v_collection_chargue, v_identity_code, v_overdue_payments
				FROM ics_clientes WHERE tipo_cred = '2'  AND num_credito BETWEEN v_valor_inicial AND v_valor_final
				
				
				
				SELECT activo_ics, fecha_desactivado_ics into v_activo_ics, v_fecha_desactivado_ics FROM ics_maectrl where num_credito = v_account_number;
				...
				SELECT dia_corte, prox_fecha_pago
					INTO v_billing_day, v_payment_date
				FROM sd_maecredanexocrd
				WHERE num_credito = v_account_number;
				
				--Validación para las fechas inhabiles 25 dic y 1 enero
				IF v_billing_day = c_dia_inhabil1 OR v_billing_day = c_dia_inhabil2 THEN
					LET v_cycle_day = v_billing_day -1;
				ELSE
					LET v_cycle_day = v_billing_day;
				END IF;
				
				SELECT ic.dias_atraso, ic.intereses_periodo_ch
					INTO v_days_delinquent, v_interest_balance
				FROM sd_indicador_cred_crd ic
				WHERE ic.num_credito = v_account_number;
				...
				IF v_behaviour_score_obtenida = 0 THEN
					...
					END IF
				END IF
				--Fin ObtenciÃ³n del Behavor_score
				
				--ObtenciÃ³n monto en disputa (aclaraciones)
				...
				--Fechas para las principal_balance


					/*Select * FROM sd_amortiza_creditocrd 
					WHERE num_credito = v_account_number INTO TEMP sd_amortiza_creditocrd WITH NO LOG;*/
				...
				FROM sd_amortiza_creditocrd 
					WHERE num_credito = v_account_number AND capital_status in (1,2,6,7) AND fecha_cuota = v_due_date_0;
				...
				--Se toma la fecha maxima y se le resta un mes
				IF v_due_date_0 > v_due_date_6 THEN 
				
					LET v_due_date_1 = add_months(v_due_date_0, -1);
					
					SELECT capital_debe, capital_pagado
						INTO  v_capital_debe, v_capital_pagado
					FROM sd_amortiza_creditocrd 
						WHERE num_credito = v_account_number AND capital_status in (1,2,6,7) AND fecha_cuota = v_due_date_1;
				...
				ELSE
					LET v_due_date_1= null;
					LET v_principal_delinquency_1 = null;
					LET v_delinquency_1 = null;
				END IF;
				...
				IF v_due_date_1 IS NOT NULL AND v_due_date_1 > v_due_date_6 THEN 
				...
				LET v_fecha_validacion = add_months(v_due_date_0, -2);
					...
					FROM sd_amortiza_creditocrd 
						WHERE num_credito = v_account_number AND capital_status in (1,2,6,7)
						and fecha_cuota = v_fecha_validacion;
				...
				IF v_due_date_2 IS NOT NULL AND  v_due_date_2 > v_due_date_6 THEN
				...
				IF v_delinquency_3 IS NOT NULL AND v_delinquency_3 > v_due_date_6 THEN
				...
				IF v_due_date_4 IS NOT NULL AND  v_due_date_4 > v_due_date_6 THEN
				...
				IF v_principal_delinquency_5 IS NOT NULL THEN
					...
					FROM sd_amortiza_creditocrd 
						WHERE num_credito = v_account_number AND capital_status in (1,2,6,7)
						and fecha_cuota between add_months(v_due_date_0, -6) and v_due_date_6;
				...
				--OJO: Los tamaÃ±os de las variables no son las mismas; sin embargo, el tamaÃ±o real de la variable es de 9:
				...
				VALUES
					...
					--DROP TABLE ics_amortiza_credito_crd;
				...
				IF iContador1 >= 100000 THEN
					SELECT DBINFO("utc_to_datetime", sh_curtime) 
						INTO horaActual 
					FROM sysmaster:sysshmvals;
					INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (iContador1, horaActual, 'CICLO_OBLIGACION_CRD');
					LET iContador1 = 0;
				END IF; 
			...
			SELECT DBINFO("utc_to_datetime", sh_curtime) 
				INTO horaActual 
			FROM sysmaster:sysshmvals;
			INSERT INTO ics_tiempos (num_registro, hora, proceso) VALUES (iContador1, horaActual, 'FIN_CICLO_OBLIGACION_CRD');
