CREATE PROCEDURE "informix".califica_scoring_cjunk_precal_opt( o_empresa       CHAR(3),
													o_numsol   	 	CHAR(20),
	...
	-- Descripcion: Se aï¿½?Â±ade cambio al estatus de RT a CN y causas RCB/RGC a CCB/CGC.
	-- RQM: RQM 09 570 Pop Ups para informar sobre solicitudes no autorizadas.
	-- Solicitï¿½?Â³: Abraham Narvaez.
	------------------------------------------------------------------------------------
	-- *********************************************************************
...
--IPCB junio2017//RECHAZO POR CREDITO BLOQUEADO RCB 
	...
	DEFINE vConsAleat			INTEGER;	DEFINE vflagvig 			INTEGER;	DEFINE vFalloSIC			INTEGER;	
	...
	-- ****************************************************************************
    --SET DEBUG FILE TO "/home/sysifx/LeonardoFigueroa/Motor/incidencia_cjunkopt/califica_scoring_cjunk_precal_opt.out";
	--TRACE ON;
...
--IPCB junio2017//RECHAZO POR CREDITO BLOQUEADO RCB 
	...
	--RQI 21 246  Originaciï¿½?Â³n de solicitudes 24 x 7 INI
	...
	END IF;
	--RQI 21 246  Originaciï¿½?Â³n de solicitudes 24 x 7 FIN
   ...
   END EXCEPTION WITH RESUME;*/
	   ...
	   --SET DEBUG FILE TO '/informix/Fperaza/NuevosCambios/canales/traces/califica_scoring_cjunk'||o_numsol||'.out';
		...
		--IPCB 12Abr21 INI --Credito solicita inactivar validaciï¿½?Â³n de segundo producto para meter Requerimiento. Con esto no se prende la bandera																																		  
	...
	--ini jesus RM 09-279-2
      ...
      END IF;
		...
		--SELECT {+ INDEX (bdisolic:ss_scoring_grupo)} count(*) INTO sNumGpos FROM bdisolic:ss_scoring_grupo WHERE seccion = 2 AND mostrar_pantalla = '1' AND grupo != 16;
		SELECT {+ INDEX (bdisolic:ss_scoring_grupo)} count(*) INTO sNumGpos FROM bdisolic:ss_scoring_grupo WHERE seccion = 2 AND mostrar_pantalla = '1';
        ...
        IF ((sNumPregSol != (sNumGpos - sStatCNIncomAux)) OR (select count(*) from bdisolic:ss_detalle_scoring det 
		                                                      where det.empresa = o_empresa and det.seccion = '2' and det.num_solicitud = o_numsol )  != (sNumGpos - sStatCNIncomAux) )  THEN
			-- Cuestionario de solicitud en sucursal no tiene todas las preguntas, segun lo esperado en central
			IF (NVL((select grupo from bdisolic:ss_detalle_scoring where empresa = o_empresa and seccion = '2' and grupo = 39 and tpo_persona = '01' and num_solicitud = o_numsol),0) != 0) THEN
				-- Validacion provisional si es grupo 39 deje pasar la solicitud
				LET cCausa_sol = 'CPI';
				LET cStatusSol = 'CN';
				...
				IF NVL(cTicket,"") <> "" THEN   -- CON EL TICKET SE VALIDA QUE LA HUELLA NO CORRESPONDA A EMPLEADOS 
					...
					--RQM 10 1177 Se contemplan productos de prestamos y linea de credito diferentes de nomina
						...
						--IF sNum_producto NOT IN ( '6400','7800','8500') or (sNum_producto = '6400' and cEmpresa <> 0) THEN
                            ...
                            --Se aï¿½?Â±ade cambio al estatus de RT a CN y causas RCB/RGC a CCB/CGC.
...
-- Se toma el parametro PUNTUALIDAD PARA RECHAZAR CON EFP (EFICIENCIA FUERA DE POLï¿½?ï¿½?TICAS
   SELECT valor
   INTO cPuntualidadZ
...
--	Se sustituye causa RDO por subcausas para reflejarse en el arbol de solicitudes RQM 04 469
		IF (v_puntualidad = cPuntualidadZ) THEN 
		   LET cCausa_sol = "EFP"; 
		   LET vMensaje='Eficiencia Fuera de Polï¿½?Â­ticas'; 	
...
-- Se agrega validacion para filtrar clientes "Z" FIN 
		...
		IF cNumSolSIC IS NULL THEN  --Valida que no hay sol vigente, por lo que asigna la SIC en base al Aleatorio
			...
			IF cNumSolSIC IS NULL THEN --Valida que no tiene solicitud base, por lo que asigna la SIC en base al Aleatorio
				...
				SELECT {+INDEX("informix".ss_sic_dinamicas idx_ss_sic_dinamicas)} institucion INTO status_consul  FROM "informix".ss_sic_dinamicas 
			...
			ELSE
				...
				LET cConsultaSic = '';
				LET cNumSolSIC = '';
			END IF;		END IF;		--Fin RQM 09 606 consulta sic aleatorio
							...
							ELSE
								IF ( cOrigenSol = '1')  THEN
									...
									IF cCanal = 4 THEN
										...
										END IF;
									ELSE	
										IF EXISTS(SELECT numproducto from bdicred:sd_productos_motor WHERE numproducto=sNum_producto) AND EXISTS(SELECT numsucursal from bdicred:sd_sucursales_motor WHERE numsucursal=v_sucursal) THEN
