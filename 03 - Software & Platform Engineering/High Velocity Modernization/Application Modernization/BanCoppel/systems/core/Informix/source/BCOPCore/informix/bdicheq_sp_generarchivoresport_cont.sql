CREATE PROCEDURE "informix".sp_generarchivoresport_cont(pfecha_reg date,pnombrearchivo CHAR(30))
...
BEGIN
	...
	------  Control de Errores no Controlados
		...
		--SET DEBUG FILE TO "/informix/VILLELA/sp_generarchivoresport_cont.out";
		--TRACE ON;
	...
	"SELECT '02'||CASE WHEN secuencia <= 9 THEN LPAD(cast(secuencia as char(1)),7,'0') WHEN  secuencia >= 10 AND  secuencia < 100 THEN  TRIM(LPAD(cast(secuencia as char(2)),7,'0'))ELSE TRIM(LPAD(cast(secuencia as char(3)),7,'0')) END||'21'||folio_solicitud||fecha_solicitud||nombre_cte||" ||                   
        ...
        LET vfiltra= "sed 's/|$//g;/^$/d' " ||  TRIM(vruta) ||  "Solicitudes.txt " || " > " || TRIM(vruta) || TRIM(pnombrearchivo)||substr(YEAR(pfecha_reg),3,2)||LPAD(MONTH(pfecha_reg),2,0)||LPAD(DAY(pfecha_reg),2,0)||'02'||'.txt';
        LET vsumario = "echo '09"|| vsecuencia || "21" || vRegistros || vsumFuturo || "' >> " || TRIM(vruta) ||  TRIM(pnombrearchivo)||substr(YEAR(pfecha_reg),3,2)||LPAD(MONTH(pfecha_reg),2,0)||LPAD(DAY(pfecha_reg),2,0)||'02'||'.txt';
		...
		LET cArchivresp= TRIM(pnombrearchivo) || substr(YEAR(pfecha_reg),3,2)||LPAD(MONTH(pfecha_reg),2,0)||LPAD(DAY(pfecha_reg),2,0)||'02' || '.txt';
