CREATE PROCEDURE "informix".reveraper(P_EMPRESA CHAR(3),
                                      P_SOLICITUD CHAR(20)) 
...
BEGIN
      ...
      SELECT UNIQUE FOLIO_SUC,usuario INTO V_FOLIO,vusuario
	FROM sd_movdia 
       WHERE num_credito = vnum_credito
	 AND codigo_fun = "002";

      EXECUTE PROCEDURE REVERSA_MOV_DIA(P_EMPRESA, vnum_credito, V_FOLIO)
              INTO      P_ERROR, P_MENSAJE;


      EXECUTE PROCEDURE BDICHEQ:REVERSION(P_EMPRESA, "001", 
					  vusuario, V_FOLIO,"B")
	      INTO P_ERROR;
      IF P_ERROR <> "000" THEN
	...
	RETURN P_ERROR, P_MENSAJE;
      ...
      WHERE  NUM_CREDITO = vnum_credito
      AND    EMPRESA     = P_EMPRESA;
      DELETE FROM SD_CONCEPFINA
      WHERE  NUM_CREDITO = vnum_credito
      AND    EMPRESA     = P_EMPRESA;
      DELETE FROM SD_DETMINIS
      WHERE  NUM_CREDITO = vnum_credito
      AND    EMPRESA     = P_EMPRESA;
      DELETE FROM SD_DETCOMIS_EAT
      WHERE  EMPRESA     = P_EMPRESA
      AND    NUM_CREDITO = vnum_credito;
      DELETE FROM SD_DETCOMIHIPOT
      WHERE  EMPRESA     = P_EMPRESA
      AND    NUM_CREDITO = vnum_credito;
      DELETE FROM SD_DETCOMI
      WHERE  NUM_CREDITO = vnum_credito
      AND    EMPRESA     = P_EMPRESA;
      DELETE FROM SD_FUENTES_X_CRED
      WHERE  NUM_CREDITO = vnum_credito
      AND    EMPRESA     = P_EMPRESA;
      DELETE FROM SD_UNIDADPROD
      ...
      WHERE  NUM_CREDITO = vnum_credito
      AND    EMPRESA     = P_EMPRESA;
      DELETE FROM SD_MAECONTRATO
      WHERE  NUM_CONTRATO = vnum_credito 
      AND    EMPRESA      = P_EMPRESA;
      DELETE FROM SD_MAECRED
      WHERE  NUM_CREDITO = vnum_credito
      AND    EMPRESA     = P_EMPRESA;
 
      --Extrae Numeros de Credito para reversar si es que fueron Renovados o 
      --Reestructurados
      FOREACH SELECT desc1 INTO v_credito_old
		FROM bdisolic:ss_bienes_deudas
	       WHERE num_solicitud = P_SOLICITUD
		 AND cod_concepto = "075" 

		SELECT UNIQUE folio_suc,usuario,sucursal
                   INTO v_folio_old,vusuario,vsucursal
                   FROM sd_movdia
		   WHERE empresa = P_EMPRESA
		         AND num_credito = v_credito_old
		         AND codigo_fun = "046";


		EXECUTE PROCEDURE reversion(P_EMPRESA,vsucursal,vusuario,
					    v_folio_old, "A")
		   INTO  P_ERROR;

		IF P_ERROR <> "000" THEN
			LET P_MENSAJE="REVERSANDO CREDITO RENOVADO";
			RETURN P_ERROR, P_MENSAJE;
		END IF

		DELETE FROM sd_movdia 
		 WHERE folio_suc = v_folio_old
		   AND num_credito = v_credito_old
		   AND empresa = P_EMPRESA;
...
END PROCEDURE
DOCUMENT
'Procedimiento de Reversion de Apertura de Credito',
