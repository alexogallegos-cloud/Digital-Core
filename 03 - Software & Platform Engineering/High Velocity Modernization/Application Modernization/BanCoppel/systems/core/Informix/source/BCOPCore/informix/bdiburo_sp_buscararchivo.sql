CREATE PROCEDURE "informix".sp_buscararchivo(psRuta VARCHAR(100), psNombreArchivo VARCHAR(50))
RETURNING
	CHAR(5), ---cod_ret
...
BEGIN
	...
	IF (psRuta = "") OR (psRuta  IS NULL) THEN		
        LET cCodRet = '00450';
	ELIF (psNombreArchivo = "") OR (psNombreArchivo  IS NULL) THEN		
        LET cCodRet = '00451';
	ELSE
        ...
        LET sCadSql = 'ls ' || TRIM(psRuta) || ' > ' || TRIM(psRuta) || 'buscar.bus';
        SYSTEM sCadSql;

        --- GUARDA EL QUERY DEL LOAD EN EL ARCHIVO   *.SQL
        LET sCadSql = 'echo "LOAD FROM ' || TRIM(psRuta) || 'buscar.bus' || ' INSERT INTO tme_busca_archivo" > '|| TRIM(psRuta) || 'EjecutaScripts_sp_buscararchivo.sql';
        SYSTEM sCadSql;

        --- EJECUTA LAS INSTRUCCIONES QUE ESTAN DENTRO DEL ARCHIVO  *.SQL
        LET sCadSql = 'dbaccess BDIBURO ' || TRIM(psRuta) || 'EjecutaScripts_sp_buscararchivo.sql';
        SYSTEM sCadSql;
...
END PROCEDURE
DOCUMENT
'CREADO POR : Josue Zepeda',
'DESCRIPCION: Procedimiento para buscar un archivo en una ruta proporcionada.',
'FECHA: 30/01/2012',
'VERSION: 20120130.0940',
'BD: BDIBURO';

CREATE PROCEDURE "informix".sp_conciliacion_sics(pcEmpresa CHAR(3), pdFecha DATE, pcInstitucion CHAR(2))
	RETURNING CHAR(6),
			  CHAR(60);              
	-- DEFINICIONES
    ...
    DEFINE cSic            CHAR(10);
    DEFINE dPrecio         DECIMAL(10,2);
    DEFINE iRegs           INTEGER;
    DEFINE vsCodRetorno    CHAR(6);
    DEFINE cMsjArchi       CHAR(60);
    DEFINE cFolioBr        CHAR(9);
    DEFINE cFolioEs        CHAR(9);
    
    ON EXCEPTION SET iSql_Err
		LET cCodRet = iSql_Err;		
		IF cCodRet = '-668' AND cCodRet <> '0' THEN
		   LET cCodRet = '00000';
		   LET cMensaje ='Verifique el archivo';
        ELSE          
           LET cCodRet = iSql_Err;
		   LET cMensaje ='Error en el proceso de CONCILIACION DE SICS';           
		END IF;	        
	...
	--SET DEBUG FILE TO "/resplogifx/archivoscartera/sp_conciliacion_sics.out";
	--TRACE ON;	  
	BEGIN          
        IF  EXISTS(SELECT dbsname, tabname FROM sysmaster:systabnames WHERE tabname = 'tme_folios_sics') THEN 
			DROP TABLE tme_folios_sics;
		END IF;		
				
        IF pcEmpresa = '' OR pcEmpresa IS NULL THEN						
			LET cCodRet = '000001';
			LET cMensaje = 'Empresa Faltante';			
		ELIF pdFecha = '' OR pdFecha IS NULL THEN
			LET cCodRet = '000002';
			LET cMensaje = 'Fecha Faltante';			
        ELSE        
             SET ISOLATION TO DIRTY READ;
             SET LOCK MODE TO WAIT 3;  
             
            SELECT COUNT(institucion) 
            INTO iRegs 
            FROM bdiburo:"informix".br_folios_sics 
            WHERE institucion = pcInstitucion
            AND fecha_conciliacion = pdFecha;
            
            IF iRegs >= 1 THEN
               LET cCodRet = '00000';
               LET cMensaje = 'Ya se corrio el proceso en esta fecha';		        
            ELSE               
                LET cFecha = SUBSTRING(pdFecha FROM 4 FOR 2)||SUBSTRING(pdFecha FROM 1 FOR 2)||SUBSTRING(pdFecha FROM 7 FOR 4);        
                LET cNombreArchivo = 'archivo_conciliación_bc_'||cFecha||'.txt';
                ...
                SELECT TRIM(valor)
                INTO cDirectorio
                FROM  bdicred:"informix".sd_param WHERE cod_param='B05';
				
				-- verifica que exista el archivo en la ruta
                EXECUTE PROCEDURE bdiburo:"informix".sp_buscararchivo(cDirectorio,cNombreArchivo)  INTO vsCodRetorno,cMsjArchi;
                               
                IF cMsjArchi = 'F' THEN          
                     LET cCodRet = vsCodRetorno;
                     LET cMensaje = 'Archivo no existe';                     
                ELSE    
				
				CREATE TABLE tme_folios_sics
                    (
					...
					LET cSQL = '';
					LET cSQL = "chmod 777 "||TRIM(cDirectorio)||TRIM(cNombreArchivo)||" > "||TRIM(cDirectorio)||"archivo_conciliación_derechos.txt";
                    SYSTEM cSQL;
					
					LET cSQL = '';
					LET cSQL = "rm "||TRIM(cDirectorio)||"archivo_conciliación_derechos.txt";
					SYSTEM cSQL;
					
					LET cSQL = '';
					LET cSQL = "cp -p "||TRIM(cDirectorio)||TRIM(cNombreArchivo)||" "|| TRIM(cDirectorio)||"archivo_conciliación_copia.txt";
					SYSTEM cSQL;
					
					LET cSQL = '';
					LET cSQL = "rm "||TRIM(cDirectorio)||TRIM(cNombreArchivo);
					SYSTEM cSQL;
					
					LET cSQL = '';
					LET cSQL = "sed 's/|$//g' " ||TRIM(cDirectorio)||"archivo_conciliación_copia.txt > " ||TRIM(cDirectorio)||TRIM(cNombreArchivo);
					SYSTEM cSQL;
					
					LET cSQL = '';
					LET cSQL = "rm "||TRIM(cDirectorio)||"archivo_conciliación_copia.txt";
					SYSTEM cSQL;

					
                    --Carga el archivo de Respuesta a la tabla.
                    LET cSQL = '';
                    LET cSQL = 'echo "LOAD FROM '''||TRIM(cDirectorio)||TRIM(cNombreArchivo)|| ''' DELIMITER ' || '''|''' ||' INSERT INTO tme_folios_sics" > '||TRIM(cDirectorio)||'query.sql';        
                    LET cSQL = cSQL;
                    SYSTEM cSQL;
                    
                    --Se ejecuta el archivo generado anteriormente.
                    LET cSQL = '';
                    LET cSQL = "dbaccess bdiburo "||TRIM(cDirectorio)||"query.sql";       
                    LET cSQL = cSQL;
                    SYSTEM cSQL;
                    ...
                    IF NOT EXISTS(SELECT folio FROM tme_folios_sics) THEN
                        LET cCodRet = '001';
                        LET cMensaje = 'El archivo conciliación no contiene registros.';                             
                    ELSE                  
                        CREATE INDEX idx_tme_folios_sics ON tme_folios_sics(folio) USING BTREE;
                        
                        SET ISOLATION TO DIRTY READ;
                        SET LOCK MODE TO WAIT 3;
                    
                        FOREACH          
                            SELECT folio,tipo,sic,precio
                            INTO cFolio,cTipo,cSic,dPrecio
                            FROM tme_folios_sics
                            WHERE folio <> ''
                            
                            IF cSic = '1' AND pcInstitucion = 'BC' THEN
                                INSERT INTO bdiburo:"informix".br_folios_sics
                                (institucion,folio,fecha_consulta,tipo_respuesta,precio_unitario,status,fecha_conciliacion)
                                VALUES ('BC',cFolio,pdFecha,cTipo,dPrecio,'NO',pdFecha);
                            ELIF cSic = '2' AND pcInstitucion = 'CC' THEN
                                 INSERT INTO bdiburo:"informix".br_folios_sics 
                                (institucion,folio,fecha_consulta,tipo_respuesta,precio_unitario,status,fecha_conciliacion)
                                VALUES ('CC',cFolio,pdFecha,cTipo,dPrecio,'NO',pdFecha);
                            END IF;
                        ...
                        FOREACH                   
                            SELECT br.folio
                            INTO cFolioBr
                            FROM bdiburo:"informix".br_folios_sics br
                            INNER JOIN bdiburo:"informix".br_es es ON es.institucion= br.institucion  
                            AND es.es03 = br.folio
                            WHERE br.fecha_consulta = pdFecha
                            AND br.institucion = pcInstitucion             
                            
                            IF NVL(cFolioBr,'') <> '' THEN             
                                UPDATE bdiburo:"informix".br_folios_sics SET status = 'OK' 
                                WHERE institucion = pcInstitucion AND fecha_consulta = pdFecha AND folio = cFolioBr;      
                            END IF;            
                        END FOREACH;  

                        LET cCodRet = '00000';
                        LET cMensaje ='El proceso CONCILIACION DE SICS se ejecutó correctamente';
                    END IF;
                END IF; 
            END IF; 
        END IF;
...
'FECHA MODIFICACION: 30 Enero del 2012',
'DESCRIPCION: se crea sp_conciliacion_sics para que migre datos de un archivo del servidor a tabla br_folios_sics',
'VERSION: 20120130.1740',
'BD: BDIBURO';

CREATE PROCEDURE "informix".sp_validacarga_sics()
RETURNING CHAR(6);


DEFINE cCod_Ret       CHAR(6);
DEFINE isqlerr        INTEGER ;
DEFINE cFechaConcil   CHAR(10);
DEFINE cErrorSPconcil CHAR(6);
DEFINE cMsjSpConcil   CHAR(60);
...
BEGIN
    ...
    END  EXCEPTION;
     ...
     --SET DEBUG FILE TO '/tmp/sp_validacarga_sics.out';
     --TRACE ON;
    ...
    IF	NVL(cFechaConcil,'') <> '' THEN          
         
		 LET cFecha = SUBSTRING(cFechaConcil FROM 3 FOR 2)||SUBSTRING(cFechaConcil FROM 1 FOR 2)||SUBSTRING(cFechaConcil FROM 5 FOR 4);        

         IF NOT EXISTS (SELECT institucion FROM bdiburo:"informix".br_folios_sics WHERE fecha_conciliacion = cFecha AND institucion='BC') THEN
            EXECUTE PROCEDURE bdiburo:"informix".sp_conciliacion_sics('001',cFecha,'BC')  INTO cErrorSPconcil,cMsjSpConcil;           
         END IF;
         
         IF NOT EXISTS (SELECT institucion FROM bdiburo:"informix".br_folios_sics WHERE fecha_conciliacion = cFecha AND institucion='CC') THEN
            EXECUTE PROCEDURE bdiburo:"informix".sp_conciliacion_sics('001',cFecha,'CC')  INTO cErrorSPconcil,cMsjSpConcil;           
         END IF;    
        LET cCod_Ret = '000000';                             
    END IF;
    RETURN cCod_Ret;           
...
DOCUMENT 
'MODIFICO: Josue Zepeda',
'FECHA MODIFICACION: 25 Enero del 2012',
'DESCRIPCION: Se implementan validacion para ejecutar sp_conciliacion_sics',
'VERSION: 20120125.1200',
'BD: BDIBURO',
'MODIFICO: Jose Angel Gaxiola Gaxiola',
'FECHA MODIFICACION: 30 Marzo del 2012',
'DESCRIPCION: Se cambio el parametro "002" del campo "cod_param" por el valor "B01" ya que con ese parametro realizaba una consulta incorrecta',
'VERSION: 20120330.',
'BD: BDIBURO';

CREATE PROCEDURE "informix".sp_consulta_conciliacion_sics(pcMes CHAR(2), pcAnio CHAR(4))
   RETURNING CHAR(6), CHAR(50), INTEGER, CHAR(12), INTEGER, INTEGER, INTEGER, INTEGER, CHAR(12), INTEGER, INTEGER, INTEGER;
...
DEFINE iFolioBC     INTEGER;
DEFINE cPrecioBC    CHAR(12);
DEFINE iConsultaHITBC 	INTEGER;
DEFINE iConsultaNoHITBC INTEGER;
DEFINE iFolioNoESBC     INTEGER;

DEFINE iFolioCC     INTEGER;
DEFINE cPrecioCC    CHAR(12);
DEFINE iConsultaHITCC 	INTEGER;
DEFINE iConsultaNoHITCC INTEGER;
DEFINE iFolioNoESCC     INTEGER;

    ON EXCEPTION SET iSql_Err
		LET cCodRet = iSql_Err;
		RETURN cCodRet,cMensaje,iFolioBC,cPrecioBC,iConsultaHITBC,iConsultaNoHITBC,iFolioNoESBC,iFolioCC,cPrecioCC,iConsultaHITCC,iConsultaNoHITCC,iFolioNoESCC;
	END EXCEPTION;
...
LET cPrecioBC = '';
LET iConsultaHITBC = 0;
LET iConsultaNoHITBC = 0;
LET iFolioNoESBC  = 0;

LET iFolioCC = 0;
LET cPrecioCC = '';
LET iConsultaHITCC = 0;
LET iConsultaNoHITCC = 0;
LET iFolioNoESCC = 0;


BEGIN

--SET DEBUG FILE TO "/tmp/JA/sp_consulta_conciliacion_sics.out";
--TRACE ON;

--Validar si se tomaron los parametros de entrada.

IF( pcMes == "" OR pcMes IS NULL ) THEN
	LET cCodRet = "000001";
	LET cMensaje = 'No se recibiò el paràmetro MES'; 
	
ELIF( pcAnio == "" OR pcAnio IS NULL ) THEN
	LET cCodRet = "000002";
	LET cMensaje = 'No se recibiò el parametro AÑO'; 
	
ELIF( pcMes < 1 OR pcMes > 12  ) THEN
	LET cCodRet = "000003";
	LET cMensaje = 'El Mes no es Valido'; 
	
ELSE
    
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	SELECT COUNT(Folio),NVL (SUM (Precio_unitario),0)
	INTO iFolioBC, cPrecioBC
	FROM bdiburo:"informix".br_folios_sics 
	WHERE YEAR(fecha_conciliacion)= pcAnio  
	AND MONTH(fecha_conciliacion) = pcMes AND institucion='BC' AND status = 'OK';
	...
	FROM bdiburo:"informix".br_folios_sics 
	WHERE YEAR(fecha_conciliacion)= pcAnio  
	AND MONTH(fecha_conciliacion) = pcMes AND institucion='BC';
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;	
	SELECT COUNT (Folio) INTO iConsultaHITBC
	FROM bdiburo:"informix".br_folios_sics 
	WHERE institucion='BC' AND tipo_respuesta=1
	AND YEAR(fecha_conciliacion)= pcAnio AND MONTH(fecha_conciliacion) = pcMes;
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	SELECT COUNT(Folio) INTO iConsultaNoHITBC
	FROM bdiburo:"informix".br_folios_sics 
	WHERE institucion='BC' AND tipo_respuesta=0
	AND YEAR(fecha_conciliacion)= pcAnio AND MONTH(fecha_conciliacion) = pcMes;
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	SELECT COUNT (Folio) INTO iFolioNoESBC
	FROM bdiburo:"informix".br_folios_sics 
	WHERE institucion='BC' AND status='NO'
	AND YEAR(fecha_conciliacion)= pcAnio AND MONTH(fecha_conciliacion) = pcMes;
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	SELECT COUNT(Folio),NVL (SUM (Precio_unitario),0)
	INTO iFolioCC , cPrecioCC
	FROM bdiburo:"informix".br_folios_sics 
	WHERE YEAR(fecha_conciliacion)= pcAnio  
	AND MONTH(fecha_conciliacion) = pcMes AND institucion='CC' AND status = 'OK'; 
	...
	FROM bdiburo:"informix".br_folios_sics 
	WHERE YEAR(fecha_conciliacion)= pcAnio  
	AND MONTH(fecha_conciliacion) = pcMes AND institucion='CC';
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	SELECT COUNT (Folio) INTO iConsultaHITCC
	FROM bdiburo:"informix".br_folios_sics 
	WHERE institucion='CC' AND tipo_respuesta=1
	AND YEAR(fecha_conciliacion)= pcAnio AND MONTH(fecha_conciliacion) = pcMes;
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	SELECT COUNT (Folio) INTO iConsultaNoHITCC
	FROM bdiburo:"informix".br_folios_sics 
	WHERE institucion='CC' AND tipo_respuesta=0
	AND YEAR(fecha_conciliacion)= pcAnio AND MONTH(fecha_conciliacion) = pcMes;
	
	SET ISOLATION TO DIRTY READ;
	SET LOCK MODE TO WAIT 3;
	SELECT COUNT (Folio) INTO iFolioNoESCC
	FROM  bdiburo:"informix".br_folios_sics 
	WHERE institucion='CC' AND status='NO'
	AND YEAR(fecha_conciliacion)= pcAnio AND MONTH(fecha_conciliacion) = pcMes;
	
	IF  iFolioBC = 0 AND iFolioCC = 0  THEN
		LET cCodRet = "000004";
		LET cMensaje = 'No Hay informaciòn con los parametros recibidos'; 
	ELSE
		LET cMensaje = 'Ejecuciòn Correcta';
	END IF
	--LET iFolioBC = iFolioBC+iConsultaHITBC+iConsultaNoHITBC;
	--LET iFolioCC = iFolioCC+iConsultaHITCC+iConsultaNoHITCC;
END IF;
RETURN cCodRet,cMensaje,iFolioBC,cPrecioBC,iConsultaHITBC,iConsultaNoHITBC,iFolioNoESBC,iFolioCC,cPrecioCC,iConsultaHITCC,iConsultaNoHITCC,iFolioNoESCC;
...
END PROCEDURE
DOCUMENT
'DESCRIPCION: Obtiene la Informacion de las consultas conciliadas SICs',
'AUTOR : Jose Angel Gaxiola Gaxiola',
'FECHA : 02/02/2012',
'VERSION: 20120202.1530',
'BD: bdiburo',
'SISTEMA : Consulta de Conciliacion',
'AUTOR: Jose Angel Gaxiola Gaxiola',
'MODIFICACION: Se cambia longitud de las variables "cPrecioBC" y "cPrecioCC" a CHAR(12)',
'FECHA: 25/04/2012',
'VERSION: 20120425.1520',
'BD: bdiburo';
