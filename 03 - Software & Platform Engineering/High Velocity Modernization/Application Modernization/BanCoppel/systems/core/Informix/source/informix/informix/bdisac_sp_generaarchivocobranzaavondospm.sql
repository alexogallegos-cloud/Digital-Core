CREATE PROCEDURE "informix".sp_generaarchivocobranzaavondospm(pConvenio CHAR(5))
   -- DEFINICION DE VARIABLES
	...
	--SET DEBUG FILE TO '/RESPALDOSNEW/enrique/sp_generaarchivocobranzaavon.out';
	--TRACE ON;
    ...
    BEGIN
        ...
        ON EXCEPTION SET iSqlErr
            IF iSqlErr <> 0 THEN
				...
				VALUES ('GEN_ARCH_COB_XDIAAVON:  '||cCodRet,today,'1','informix',CURRENT,'1','sp_generaarchivocobranzaavondospm','Genera archivo de cobranza avon dos pm');
            END IF;
        END EXCEPTION;
		...
		/* nombre anterior TRIM("XBCYYMMDDHHmmSS.txt") */
		SELECT TRIM(ruta_archivo_cobranza )|| TRIM("XBPYYMMDDHHmmSS.txt"),cuenta_prestadora
		...
		LET cRutaArchAvon = REPLACE(cRutaArchAvon,'YY',SUBSTRING(LPAD(YEAR(dFecha_Hoy ::DATE),4,'0') FROM 3 FOR 2));
		LET cRutaArchAvon = REPLACE(cRutaArchAvon,'MM',cMes);
		LET cRutaArchAvon = REPLACE(cRutaArchAvon,'DD',cDia);
		LET cRutaArchAvon = REPLACE(cRutaArchAvon,'HH','14');
		LET cRutaArchAvon = REPLACE(cRutaArchAvon,'mm','00');
		LET cRutaArchAvon = REPLACE(cRutaArchAvon,'SS','00');
		
		LET cStmt = 'rm -f ' || cRutaArchAvon;
		SYSTEM cStmt;

		-- Consulta de la TransacciÃÂ³n del Servicio
        ...
        FOREACH
            ...
            SELECT LPAD(DAY(fecha_pago::DATE), 2, '0'), 
             ...
             WHERE numcategoria = cCategoria
              AND numconvenio = cConvenio
              AND fecha_pago = today        
              AND extend (fecha_insert, hour to second) <= '14:00:00'
			...
			--LET cStmt = 'echo "' || cTpoOperacion || cDiaPago || cMesPago || cAnioPago || LPAD(TRIM(cReferencia1), 2, '0') || LPAD(TRIM(cReferencia1), 20, '0') || LPAD(TRIM(cCveBancoppel), 2, '0') || LPAD(iImporte_Pago, 8, '0') || LPAD(cFolio, 16, '0') || '" >> ' || cRutaArchAvon;
			
			--NUEVA LINEA QUE ARMARA EL DETALLE CON LAS REFERENCIAS A 20 POSICIONES y fechas YYYYMMDD SEGUN RQM 10 1385 tambien se cambiara en las lineas C, I, E y T
...
--ACTUALIZA BANDERA CONFIRMACION SUCURSAL
        FOREACH SELECT referencia, folio_suc, fecha_pago 
    		 ...
    		 UPDATE bdisac:sac_movimientos SET flag_confirmacion_sucursal='1'
                WHERE numcategoria = cCategoria
                AND numconvenio = cConvenio
