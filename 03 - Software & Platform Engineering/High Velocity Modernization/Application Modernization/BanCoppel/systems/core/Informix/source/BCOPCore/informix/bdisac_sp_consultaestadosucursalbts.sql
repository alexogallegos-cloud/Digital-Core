CREATE PROCEDURE "informix".sp_consultaestadosucursalbts(pNumSucursal CHAR (4))


RETURNING CHAR (5) AS RetCode, CHAR (5) AS CodEstadoSucursal;
--			
...
--------------------------------------------------------------------------
--	SET DEBUG FILE TO "/home/sysifx/JesusBueno/Appriza/sp_consultaestadosucursalbts.out";
--	TRACE ON;
--------------------------------------------------------------------------
 ...
 BEGIN
   ON EXCEPTION SET iSqlErr
    ...
    ELSE
		...
		WHERE cve_estado = (SELECT cve_estado FROM bdinteg:"informix".si_ptf WHERE tipo != 'C' AND id_ptf = pNumSucursal);
		
		--IF cTransaccInt IS NULL OR cTransServicio IS NULL  OR cApprizaCode IS NULL OR cChannelID IS NULL OR cTypeCode IS NULL OR cCountryCode IS NULL OR cod_estado IS NULL THEN
...
END PROCEDURE
DOCUMENT
'AUTOR : 97832766 - Aaron Lopez',
'DESCRIPCION:  Obtiene el codigo del estado correspondiente a la sucursal del catalogo de estado de BTS',
'FECHA : 17/10/2019',
'BD: bdisac',
'-----------------------------------------------------------------------------------------------------------------------';
