CREATE PROCEDURE "informix".elimina_movshistduplicados_2( pempresa CHAR(3), pFecha DATE )
RETURNING CHAR(5), CHAR(5), CHAR(50), INTEGER, INTEGER;
    ...
    ON EXCEPTION SET sql_err, isam_err, desc_err
        SET DEBUG FILE TO "/resplogifx/conciliachq/elimina_movshistduplicados_2.err";
        TRACE ON;
        IF sql_err <> 0 THEN
...
END EXCEPTION WITH RESUME;
    ...
    --- SET DEBUG FILE TO "/resplogifx/conciliachq/elimina_movshistduplicados_2.out";
    --- TRACE ON;
    ...
    SELECT {+INDEX(bdicheq:"informix".sc_movhis_old idx_movhisnew6_old)} 
     ...
     GROUP BY 1
      INTO TEMP tmp_seriales2 WITH NO LOG;
    CREATE INDEX idx_tmpser2 ON tmp_seriales2(cuantos) USING BTREE FILLFACTOR 99;
    UPDATE STATISTICS HIGH FOR TABLE tmp_seriales2;
    ...
    SELECT num_serial  
     ...
     WHERE cuantos > 1
      ...
      INTO TEMP tmp_duplicados2 WITH NO LOG;
    CREATE INDEX idx_tmpdup2 ON tmp_duplicados2(num_serial) USING BTREE FILLFACTOR 99;
    UPDATE STATISTICS HIGH FOR TABLE tmp_duplicados2;

    FOREACH WITH HOLD 
        SELECT num_serial
          INTO vSerialDuplicado
          FROM tmp_duplicados2
