CREATE PROCEDURE "informix".sp_genreportecaptacion()
...
END EXCEPTION;
    ...
    --SET DEBUG FILE TO "/home/tmp/jairo/sp_GenReporteCaptacion.out";
    --TRACE ON;
...
/* 	SELECT fecha_hoy, pri_dia_mes, ult_dia_mes 
	...
	IF EXISTS( SELECT dbsname, tabname FROM sysmaster:systabnames WHERE tabname = 'si_tmp_tiemposmov_cap') THEN
        DROP TABLE "informix".si_tmp_tiemposmov_cap;	
    END IF;

	CREATE TABLE bdinteg:"informix".si_tmp_tiemposmov_cap(
	...
	INSERT INTO bdinteg:"informix".si_tmp_tiemposmov_cap (producto,descripcion,total_solicitudes,tiempo_apertura_promedio)
	VALUES('Producto','Descripcion','Total de Solicitudes','Tiempo promedio Apertura');

	FOREACH	
	
		/*
		SELECT DISTINCT(producto), COUNT(*)
		INTO cProducto,iCont1
		FROM bdinteg:"informix".si_bit_tiempoymovimientos
		WHERE vigente = '1'
		AND (DATE(tiempo_pld_ini) >= dDiaMesIni AND DATE(tiempo_pld_fin) <= dDiaMesFin) OR (DATE(tiempo_apertura_ini) >= dDiaMesIni AND DATE(tiempo_apertura_fin) <= dDiaMesFin)
		AND producto <> '9001'
		AND producto <> '6001'
		GROUP BY producto
		*/
		...
		FROM bdinteg:"informix".si_bit_tiempoymovimientos
		WHERE vigente = '1'
		AND (DATE(tiempo_pld_ini) >= dDiaMesIni 
			AND DATE(tiempo_pld_fin) <= dDiaMesFin) OR (DATE(tiempo_apertura_ini) >= dDiaMesIni 
		...
		IF iCont1 > 0 THEN
			...
			IF cProducto = '9000' THEN
				LET cDescripcion = 'CUESTIONARIO PLD';
				SELECT SUM(tiempo_pld_fin - tiempo_pld_ini) / COUNT(*), COUNT(*)
				INTO dPromPcAn,iCont
				FROM bdinteg:"informix".si_bit_tiempoymovimientos
				WHERE producto = cProducto 
				AND vigente = '1'
				AND DATE(tiempo_pld_ini) >= dDiaMesIni
				AND DATE(tiempo_pld_fin) <= dDiaMesFin;
			ELSE
			...
			INSERT INTO bdinteg:"informix".si_tmp_tiemposmov_cap (producto,descripcion,total_solicitudes,tiempo_apertura_promedio)
			VALUES(TRIM(cProducto),TRIM(cDescripcion),iCont,TRIM(sTiempoTotal));	
			
			LET iCont = 0; LET iTotal = 0; LET sTiempo = 0; LET iLongHora = 0; LET iSumaDias = 0; LET cPromPcAn = ''; 
			LET dPromPcAn = ''; LET iSumaDias2 = 0; LET sTiempoTotal = ''; LET dPromPcAnCHAR = '';	
		END IF;		
	END FOREACH;
	...
	IF iCont1 > 0 THEN
		...
		LET cNomArchTxt = 'RPT_TiemposyMovimientos_'||cDia||cMes||cAnio||'_Captacion.txt';
		LET cNomArchEjecSql = 'QueryUNLCap.sql';
			
		LET cSQL = 'echo "UNLOAD TO '|| TRIM(cRutaArchTxt) || TRIM(cNomArchTxt) || ' DELIMITER ' || '''|''' ||' SELECT * FROM bdinteg:si_tmp_tiemposmov_cap" > '|| TRIM(cRutaArchTxt) || TRIM(cNomArchEjecSql);	
		SYSTEM TRIM(cSQL);
		...
		SYSTEM TRIM(cSQL);
	ELSE
		LET sCodRet = '00001'; --No existen registros que procesar, innecesario generar reporte.
	END IF;
	
	DROP TABLE bdinteg:"informix".si_tmp_tiemposmov_cap;
...
'Fecha: 27/12/2017',
'Descripcion: Se crea sp para la generacion de reportes de tiempos y movimientos de las Aperturas de cuentas de Captacion.',
