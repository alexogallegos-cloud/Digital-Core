CREATE PROCEDURE "informix".sp_buscarorigen(pTipoProducto INTEGER)
    ...
    DEFINE id_origen_evento INTEGER;
    DEFINE descripcionOrigenEvento VARCHAR (50);
    DEFINE nombreOrigenEvento VARCHAR (50);
    DEFINE porcentajeMaximo INTEGER;
    DEFINE validacionCargoRecurrente INT;
    DEFINE indicadorRoboIdentidad INT;
	DEFINE indicadorRoboExtravio INT;
    DEFINE activo INT;

    LET iSqlErr =0 ;
    LET id_origen_evento =0;
    LET descripcionOrigenEvento = '';
    LET nombreOrigenEvento ='';
    LET porcentajeMaximo =0;
    LET validacionCargoRecurrente =0;
    LET indicadorRoboIdentidad =0;
	LET indicadorRoboExtravio = 0;
    LET activo =0;
    LET v_cod_ret = "000";
    ...
    BEGIN
        ON EXCEPTION
            ...
            IF iSqlErr <> 0 THEN
                ...
                LET indicadorRoboIdentidad =0;
                LET activo =0;
                RETURN v_cod_ret, id_origen_evento, descripcionOrigenEvento, nombreOrigenEvento, porcentajeMaximo, validacionCargoRecurrente, indicadorRoboIdentidad, indicadorRoboExtravio;
            END IF;
        END EXCEPTION;
        ...
        FOREACH
               /* SELECT a.pky_origen_evento, a.descripcion,           a.nombre,             a.porcentaje_maximo, a.validacion_cargo_recurrente, a.indicador_robo_identidad
                INTO   id_origen_evento,    descripcionOrigenEvento, nombreOrigenEvento,   porcentajeMaximo,    validacionCargoRecurrente,     indicadorRoboIdentidad
                FROM acl_origen_evento a WHERE  a.activo=1*/
				SELECT distinct oe.pky_origen_evento, oe.descripcion, oe.nombre ,oe.porcentaje_maximo,oe.validacion_cargo_recurrente,oe.indicador_robo_identidad,oe.indicador_robo_extravio
				INTO   id_origen_evento,descripcionOrigenEvento,nombreOrigenEvento,porcentajeMaximo,validacionCargoRecurrente,indicadorRoboIdentidad, indicadorRoboExtravio
				FROM acl_regla_negocio rn
				Inner Join acl_tipo_evento te on rn.fky_tipo_evento = te.pky_tipo_evento and te.activo = 1
				Inner Join acl_origen_evento oe on te.fky_origen_evento = oe.pky_origen_evento and oe.activo = 1
				WHERE rn.activo = 1 and fky_tipo_producto = pTipoProducto
				
                RETURN v_cod_ret, id_origen_evento, descripcionOrigenEvento, nombreOrigenEvento, porcentajeMaximo, validacionCargoRecurrente, indicadorRoboIdentidad, indicadorRoboExtravio WITH RESUME;
        END FOREACH;
    END;
END PROCEDURE
DOCUMENT
...
'FECHA: 18/09/2019',
'DESCRIPCION: Se modifica procedimiento para retornar nuevo campo de robo extravio',
'BD    :  bdiaclaracion';

CREATE PROCEDURE "informix".sp_cat_statustarjeta()
    RETURNING CHAR(5) AS codRet,
		CHAR(3) AS codStatus,
		CHAR(30) AS descStatus;
	
	DEFINE cCodRet CHAR(5);
	DEFINE iSqlErr INT;
	DEFINE codStatus CHAR(3);
	DEFINE descStatus CHAR(30);
	...
	BEGIN
		ON EXCEPTION SET iSqlErr
			IF iSqlErr <> 0 THEN
				LET cCodRet = iSqlErr;
				RETURN cCodRet, codStatus, descStatus;
			END IF;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cat_statustarjeta.out';
		--TRACE ON;
		...
		FOREACH
			...
			SELECT codstatustarjeta, descstatustarjeta
			INTO codStatus, descStatus
			FROM intercard:"informix".statustarjeta
			WHERE codstatustarjeta != 'ACT'
			
			RETURN cCodRet, codStatus, descStatus WITH RESUME;
...
'FECHA: 19/09/2019',
'PROYECTO: Aclaraciones',
'FUNCIONALIDAD: Editar Datos de Eventos',
'DESCRIPCION: Spl para consultar catalogo de estatus de tarjetas.',
'BD: bdiaclaracion';

CREATE PROCEDURE "informix".sp_busca_producto_deb_cheq_cliente(p_sNumeroCliente CHAR(20), p_skip INT)
	...
	BEGIN
		...
		FOREACH
			SELECT SKIP p_skip DISTINCT bdicheq:sc_maechq.producto as numeroProducto, bdicheq:sc_producto.nombre AS nombreProducto, cuenta AS cuentaProducto, intercard:tarjetacuenta.numtarjeta AS tarjetaProducto, codstatustarjeta AS estatusTarjeta
			...
			FROM bdicheq:sc_maechq 
					...
					--LEFT JOIN bdicred:sd_tarjeta ON (bdicred:sd_maecred.empresa = bdicred:sd_tarjeta.empresa and bdicred:sd_maecred.num_credito = bdicred:sd_tarjeta.num_credito)
					LEFT JOIN intercard:tarjeta ON (intercard:tarjetacuenta.numtarjeta = intercard:tarjeta.numtarjeta)
			WHERE bdicheq:sc_maechq.num_cte = p_sNumeroCliente 
			AND bdicheq:sc_maechq.status_cta in ('1','3','4','5','6','7')
...
CREATE PROCEDURE "informix".sp_busca_producto_transfer_cliente(p_sNumeroCliente CHAR(20), p_skip INT)
    ...
    DEFINE resultado_clienteTransfer   CHAR(30);
	DEFINE cStatusTarjeta CHAR(3);
	...
	BEGIN
        ...
        ON EXCEPTION
            ...
            IF iSqlErr <> 0 THEN
                ...
                LET resultado_telefonoTransfer='';
                LET resultado_clienteTransfer = '';
                RETURN resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta, resultado_telefonoTransfer, resultado_clienteTransfer,cStatusTarjeta;
            END IF;
        END EXCEPTION;
		...
		FOREACH
	        SELECT SKIP p_skip DISTINCT bdicheq:sc_producto.producto as numeroProducto, bdicheq:sc_producto.nombre AS nombreProducto,
                ...
                bditransfer:tf_maecte.numcte_tf AS clienteTransfer,
				intercard:tarjeta.codstatustarjeta AS estatusTarjeta
				Into resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta, resultado_telefonoTransfer, resultado_clienteTransfer,cStatusTarjeta
              ...
              WHERE bditransfer:tf_maecte.numcte= p_sNumeroCliente
			RETURN resultado_numeroProducto,resultado_nombreProducto, resultado_numeroCuenta, resultado_numeroTarjeta, resultado_telefonoTransfer, resultado_clienteTransfer,cStatusTarjeta WITH RESUME;
		END FOREACH;
	END
END PROCEDURE
DOCUMENT 'AUTOR: Rodolfo Conde Flores',
