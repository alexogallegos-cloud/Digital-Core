CREATE PROCEDURE "informix".sp_ostelgrabaostelefonica( pEmpresa char(3), 
...
-- 27/12/2008
-- Bernardo Carlos Bá¥º Gonzá¬¥z
-- Generació® ¤e Os Telefonica guardando en ss_ostelrefsolicitud, ss_osclientesupervisartel, ss_osteltelefonos y ss_ostelreferencias
---Modificó º Lorenzo Ibarra Garcð¢ ­-Fecha: 26-10-2009
--Se agregaron validaciones para los datos de entrada
--Se modificó °¡ra obtener los datos de las referencias por medio de un ciclo
--Se modificó °¡ra cuando el cliente no viva en unidad habitacional no se ejecuten los querys que obtienen los datos de esta.
---Modificó º JesÃºs Manuel Aguilar Heredia
--Fecha: 03-11-2010
--Se agrega la actualizacion de un campo en la tabla ss_osclientesupervisartel
---Modificó º Antonio Bastiads
--Fecha: 17-12-2010
...
BEGIN
    ...
    IF TRIM(pEmpresa) = '' OR pEmpresa IS NULL OR
        ...
        pReferenciaUno IS NULL OR pReferenciaUno <= 0 THEN
		--OR pReferenciaDos IS NULL OR pReferenciaDos <= 0 THEN 
    ...
    --verificar que al menos se haya recibido un nÃºmero de solicitud
    ...
    if exists (SELECT {+INDEX("informix".ss_solicitudes empsol)} num_solicitud from "informix".ss_solicitudes where empresa = pEmpresa and num_solicitud = pSolicitudUno) then
    ...
    if exists(SELECT {+INDEX("informix".ss_solicitudes empsol)} num_solicitud from "informix".ss_solicitudes where empresa = pEmpresa and num_solicitud = pSolicitudDos) then
    ...
    --obtener nÃºmero de cliente asignado y sucursal donde fueron las solicitudes
    ...
    WHILE bContinuarCiclo = 't'
        ...
        /*select nombreestado
        into vNombreestado
        from bdinteg:si_catestados
        where numeroestado = vEstado;*/
    ...
    --verificar si se debe enviar a supervisió® ¬a solicitud
    if exists(select ostelefonica from bditarjcop:"informix".sucursalescajaunica where empresa = pEmpresa and cvesucursal = cSucursal) then
