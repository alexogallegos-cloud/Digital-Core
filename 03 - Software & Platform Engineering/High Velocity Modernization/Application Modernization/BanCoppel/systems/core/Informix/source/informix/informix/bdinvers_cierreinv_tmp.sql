create procedure "informix".cierreinv_tmp(pempresa char(3))
  ...
  returning char(5);
   ...
   -- set debug file to "cierreinv_tmp.out";
   -- trace on;
   ...
   select fecha_ant, fecha_hoy, pri_dia_mes,
     ...
     into vgfecha_hoy, vgprox_fecha, vgpri_dia_mes,
          vgpri_hab_mes, vgult_dia_mes, vgult_hab_mes
     from sv_fechas
    where empresa = pempresa;
   ...
   -- // VALIDA SE HALLA EFECTUADO RESPALDO DE DATOS
{
   ...
   IF vexiste2 = 0 THEN
      ...
      VALUES
         (pEmpresa, 'CierreInv', vgfecha_hoy, '03', 'I',
          USER, CURRENT, NULL, '000');
   ELSE
      update bdinteg:sx_contproc set
         status_proc = "I"
   ...
   if vcontproc <> vtotsuc or
      vcontproc is null or
      vtotsuc is null then
...
{
   foreach
      select cuenta,secuencia,sucursal,mv.plaza,capital,sdo_ult_corte,
...
}
{
   ...
   if vgfecha_hoy = vgult_hab_mes then
      ...
      foreach
         select cuenta,capital,mv.sucursal,mv.cod_instrum,
   ...
   foreach with hold
      select cuenta,capital,mv.sucursal,mv.cod_instrum,
       ...
       where status_cta in("1","3")
         ...
         and mv.cod_instrum = pr.cod_instrum
         and pr.gpo_instrum = "CD"
         --and cuenta in("30000122838", "30000123346")
       order by cuenta
      ...
      foreach
         select cap_int,inst_vento,importe,sistema,
      ...
      if vtotal_int > 0 then
         ...
         if vinteresneto > 0 then
           ...
           IF vprovision_int = "M" THEN
              SELECT vinteres - importe INTO vinteresneto
                FROM sv_maeinstrucc
               WHERE empresa = pempresa
                 AND cuenta = vcuenta
                 AND cap_int = "I";
      ...
      IF vaplicado ="S" AND vfecha_venc = vgfecha_hoy
         AND (vinstcap ="01" OR vinstint ="01") THEN
