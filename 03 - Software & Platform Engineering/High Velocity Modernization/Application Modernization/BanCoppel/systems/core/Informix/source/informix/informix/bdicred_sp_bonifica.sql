CREATE PROCEDURE "informix".sp_bonifica ()  
	RETURNING
	CHAR(5)  AS COD_RET;      
...
DEFINE dFechaInsertSc	DATE;
DEFINE cProdCredito		CHAR(4);
DEFINE cStatusSolicitud CHAR(2);
DEFINE cIndicador		CHAR(1);
DEFINE cCodRetSP		CHAR(5);
DEFINE cEmpresa			CHAR(3);
DEFINE dCobroTar		DECIMAL(18,2);
DEFINE dCobroIvaTar		DECIMAL(18,2);
DEFINE iContadorConsec	INTEGER;
DEFINE cSecuencia		CHAR(2);
DEFINE iNumSolic		SMALLINT;
DEFINE iContSol			SMALLINT;
DEFINE bbonificacion 	SMALLINT;
--variables para la secuencia
DEFINE cFolio 			CHAR(16);	
DEFINE cHora		    CHAR(2);
DEFINE cMinuto 			CHAR(2); 
DEFINE cSegundo 		CHAR(2);
DEFINE cHoraActual 		CHAR(16);
...
LET cCuenta				= "";
LET cProdTarjeta		= "";
LET dFechaInsertSc		= DATE(1);
LET cProdCredito		= "";
LET cStatusSolicitud	= "";
LET cIndicador 			= "0";
LET cCodRetSP			= "00000";
LET dCobroTar			= 0.00;
LET dCobroIvaTar		= 0.00;
LET iContadorConsec		= 1;
LET cSecuencia			= "";
LET iNumSolic			= 0;
LET iContSol			= 0;
LET bbonificacion		= 0;	

--INICIALIZACIONES DE VARIABLES SECUENCIA
LET cFolio = "";
LET cHora = "";
LET cMinuto = "";
LET cSegundo = "";
LET cHoraActual = CAST(CURRENT HOUR TO FRACTION AS CHAR(16));
LET ventransacc = 0;

BEGIN
    ON EXCEPTION SET iSqlErr, isamErr, descErr
        SET DEBUG FILE TO "/respaldos/sucproductos/sp_bonifica.err";
        TRACE ON;
        IF 	iSqlErr <> 0 THEN
            IF iSqlErr = -110 OR isamErr = -110 THEN
                LET cCodRet = iSqlErr;
                LET cCodRet2 = isamErr;
                LET cCodRet3 = descErr;
                LET cCodRet = "00000";
	...
	--SET DEBUG FILE TO "/tmp/sp_bonifica.out";
	--TRACE ON;
	

	-- FOREACH PARA BUSCAR LOS CLIENTES QUE TENGAN UNA CUENTA TITULAR  tipo_tarjeta = 'T'
	FOREACH WITH HOLD
		SELECT empresa,numcte, cuenta, prodtarjeta, fecha_insert, cobro_tarjeta,iva_cobrotar
		INTO cEmpresa,cNumcte, cCuenta ,cProdTarjeta, dFechaInsertSc, dCobroTar,dCobroIvaTar
		FROM bdicheq:"informix".sc_tarjeta 
		WHERE tipo_tarjeta = 'T' 
		AND bandera_cobro = 1
		AND bandera_bonificacion = 0
		AND cobro_comision='S'
			
        
        BEGIN WORK;
        LET ventransacc = 1;
		
		LET iContadorConsec = iContadorConsec;
		--FOREACH PARA  BUSCAR LOS STATUS DE SOLICITUDES EN STATUS TERMINALES  "'AN', 'CN', 'AP', 'RT'"
		...
		SELECT COUNT(num_solicitud )
				...
				AND numcte = cNumcte
				AND fecha_insert = dFechaInsertSc 
				AND num_producto in(Select distinct (producto_credito) FROM bdicred:"informix".sd_combproductos ) ;		
		LET iContSol = 0;	
		LET bbonificacion = 0;		
		FOREACH 
		
			SELECT num_producto, status_solicitud 
			INTO cProdCredito, cStatusSolicitud 
			FROM bdisolic:"informix".ss_solicitudes 
			WHERE numcte = cNumcte
			AND fecha_insert <= dFechaInsertSc 
			AND status_solicitud IN (SELECT DISTINCT status FROM bdicred:"informix".sd_combproductos WHERE indicador in (2,3)) 
			--AND status_solicitud IN ('AN', 'CN', 'AP', 'RT') 
								
			
			SELECT indicador 
			INTO cIndicador
			FROM bdicred:"informix".sd_combproductos 
			WHERE producto_debito = cProdTarjeta
			AND producto_credito = cProdCredito
			AND status = cStatusSolicitud;
		
			LET iContSol = iContSol+1;
							
			IF cIndicador <> "2"  THEN	
				IF 	iContSol=iNumSolic AND bbonificacion = 0 THEN 
					UPDATE bdicheq:"informix".sc_tarjeta SET bandera_bonificacion = 3 WHERE numcte = cNumcte AND cuenta = cCuenta; 
				END IF;
			ELIF (cIndicador = "2" AND  dCobroTar = 0) THEN 
					UPDATE bdicheq:"informix".sc_tarjeta SET bandera_bonificacion = 1 WHERE numcte = cNumcte AND cuenta = cCuenta; 	
			ELSE 
				IF bbonificacion = 0 THEN
			
					IF iContadorConsec < 10 THEN
						LET	cSecuencia = "0"||CAST(iContadorConsec AS CHAR);
					ELSE
						LET cSecuencia = iContadorConsec;
					END IF;
					
					LET cHoraActual = CAST(CURRENT HOUR TO FRACTION AS CHAR(16));
					LET cHora = SUBSTR( cHoraActual ,0,2);
					LET cMinuto = SUBSTR( cHoraActual ,4,2);
					LET cSegundo = SUBSTR( cHoraActual  ,7,2);
		
					LET cFolio = "bonifica"||cHora||cMinuto||cSegundo||cSecuencia;
					LET cFolio = cFolio;
					
					EXECUTE PROCEDURE bdicheq:"informix".abono_ref ('001','5401',"informix",'0311','',cFolio,cCuenta,0,dCobroTar,dCobroTar,0,0,0,'01','Bonificacion por cobro de  TD','','')
					INTO cCodRetSP;
						IF CAST(cCodRetSP AS INTEGER) <> 0  THEN 
							INSERT INTO "informix".sd_errorbonifica VALUES(cEmpresa,cCuenta,cCodRetSP,today); 
						END IF;
					--Se agrega la bonificacion del iva 
					EXECUTE PROCEDURE bdicheq:"informix".abono_ref ('001','5401',"informix",'0317','',cFolio,cCuenta,0,dCobroIvaTar,dCobroIvaTar,0,0,0,'01','Bonificacion de IVA por cobro de  TD','','')
					INTO cCodRetSP;
						IF CAST(cCodRetSP AS INTEGER) <> 0  THEN 
							INSERT INTO "informix".sd_errorbonifica VALUES(cEmpresa,cCuenta,cCodRetSP,today); 
						END IF;
					...
					LET iContadorConsec = iContadorConsec +1;
					IF iContadorConsec > 99 THEN
						LET iContadorConsec = 1;
					END IF;
				END IF;	
			END IF;
...
'Fecha:12/02/2014',
'Modificación: Se crea sp para que bonifique a cliente exento de comisión',
'Sustento: RQM 10 408-2 Adendum Clientes exentos de cobro de comisión por emisión de TD.pdf  (Pagina 9 a 11)',
'Solicita: Rodolfo Gomez Hernandez',
'Folio: 1609',
'Modifico: 94972834 Felipe de jesus urias rocha',
'Fecha: 03/06/2014',
'modififcacion:se agrega consulta para optener los estatus de la tabla sd_combproductos',
