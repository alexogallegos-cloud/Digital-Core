CREATE PROCEDURE "informix".sp_ctes_tdd_retiros_atm( pNumeroMeses VARCHAR(2), pNumMesAnteriorSdo INTEGER )
...
BEGIN
    ...
    LET TIPO_PLANTILLA = 'ATM_CAPTA';
    LET ID_PLANTILLA = '4';
    LET OCTUBRE = '10';
    
    --SET DEBUG FILE TO RUTA_DESTINO||"sp_ctes_tdd_retiros_atm.out";
    --TRACE ON;
    ...
    FROM bditarjeta:td_parametro
        WHERE clave = 'SDO_TDD_RETIRO_ATM';
    ...
    */   
    ...
    --indice para optimizar la busqueda en si_correos utilizando los campos: tipo_correo, status_correo
    */
    SET ISOLATION TO DIRTY READ;
    SELECT 
    ...
    */
    ...
    SELECT COUNT(*) conteo_total INTO vTotalRegistros FROM info_reporte_trimestral WHERE plantilla = ID_PLANTILLA;
        ...
        IF (vTotalRegistros = 0) THEN
            ...
            LET vsql = '';
            LET vsql = 'echo "UNLOAD TO "'||RUTA_DESTINO||TIPO_PLANTILLA||'"_00.unl SELECT * FROM info_reporte_trimestral WHERE plantilla = "'||ID_PLANTILLA||'";" > '||RUTA_DESTINO||'tdd_retiros_atm.sql';
            SYSTEM vsql;
            
            LET vsql ='';
            LET vsql= 'dbaccess intercard '||RUTA_DESTINO||'tdd_retiros_atm.sql';
            SYSTEM vsql;
            
            LET vsql = '';
            LET vsql ='rm '||RUTA_DESTINO||'tdd_retiros_atm.sql';
            SYSTEM vsql;
        ...
        IF (vTotalRegistros >  0) THEN            
            ...
            WHILE (vTotalRegistros > 0 ) LOOP
                ...
                LET vsql = '';
                LET vsql = 'echo "UNLOAD TO "'||RUTA_DESTINO||TIPO_PLANTILLA||'"_'||vContadorArchivos||'.unl SELECT SKIP '||vNumInicioRegistros||' FIRST '||vRegistrosMaxPorArchivo||' correo_electronico, titular FROM info_reporte_trimestral WHERE plantilla = "'||ID_PLANTILLA||'";" > '||RUTA_DESTINO||'tdd_retiros_atm.sql';
                SYSTEM vsql;
...
-- Modificado: 22.enero.2018 10:09:00am
