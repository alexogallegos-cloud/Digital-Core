CREATE PROCEDURE "informix".sp_mail_preventiva_tc(pempresa char(3))
RETURNING 	
CHAR(06)  AS codigo_retorno,
CHAR(150)  AS mensaje_retorno;
---------------------------------------------------------------------------------------
--  execute PROCEDURE "informix".sp_mail_preventiva_tc('001')			
---DECLARACIONES
...
DEFINE iCount_TC_PREVEN INTEGER; --A.L.L.
...
LET P_MENSAJE           ='El proceso de las campañas EMAILs PREVENTIVAS TDC se realizó correctamente.';
LET cMensajePagoMin		= '';
LET dtFechaHoy			= '';
LET cNumCte				= '';
LET cNumCta				= '';
...
--Obtiene pago mínimo al corte actualizado con pagos
		...
		if dPagoMinimo > 0	then	
            ...
            LET iCount_TC_PREVEN = iCount_TC_PREVEN + 1;
            call bdimnsj:"informix".sp_registra_evento (1, 'TC_PREVEN' , cNumCte, cNumCred,vnumtarjeta, 2,
                           cApellPat,'','','','',dPagoMinimo,dPagoNoGeneraInt,'','','', '', '')RETURNING P_COD_RET;
			call "informix".sp_inserta_info_rep_envios (pempresa,'EMAIL',1017, cNumCred, cNumCte, vnumprod, today, cEmail, '','','') returning P_COD_RET;							   
	...
	--		select  2, 'TC_PREVEN',numcte,current,apell_paterno,day(dtFechaHoy),month(dtFechaHoy),100
			select  2, 'TC_PREVEN',numcte,current,'',0,'',0
    ...
    CALL bdicobranza:"informix".sp_latinia_contador_cobranza('TC_PREVEN',iCuentasProcesadas,iCuentasExcluidasXMail) RETURNING cCodRet;

--Genera cifras de control
    if iCuentasProcesadas > 0 then
       let cMensaje = 'TOTAL Cuentas procesadas campaña EMAILs PREVENTIVA TDC : ' ||iCuentasProcesadas;
       let cMensaje = trim(cMensaje) ||'    EMAILs enviados : ' ||iCount_TC_PREVEN;
