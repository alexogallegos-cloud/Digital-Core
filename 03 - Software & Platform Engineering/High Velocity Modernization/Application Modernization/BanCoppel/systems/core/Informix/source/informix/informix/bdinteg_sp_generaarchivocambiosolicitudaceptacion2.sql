CREATE PROCEDURE "informix".sp_generaarchivocambiosolicitudaceptacion2(pempresa CHAR(3), pFechaAct DATE)
RETURNING CHAR(6); 
--CLIENTE
...
DEFINE vdeptoointeriortrabajo, vdeptointerior, vdeptoointeriorreferencia2, vfolioaut, cNumInterior, vdeptoointeriorreferencia, cFolioSucursal, vdeptoointeriorconyuge CHAR(4);
DEFINE ventrecallesreferencia2, ventrecalles, ventrecallesconyuge, ventrecallesreferencia1, ventrecallestrabajo CHAR(40);
...
DEFINE vcomplementoconyuge, vcomplemento, vcomplementoreferencia2, vcomplementoreferencia, vcomplementotrabajo CHAR(80);
DEFINE vfolioconsulta CHAR(9);
DEFINE dFechaConsBuro, vfechanacimiento, vfechaaltacliente, vFecha_Hoy, vfechamovto, dFechaAlta DATE;
DEFINE vHora DATETIME HOUR TO FRACTION(3);
DEFINE dEvaluacion1, dEvaluacion2 DECIMAL(5,2);
DEFINE vtelefonoreferencia, vtelefonocelularreferencia, vtelefonotrabajoconyuge, vtelefonocelular, vtelefonoreferencia2, vtelefono, vtelefonocelularconyuge, vtelefonotrabajo, vtelefonocelularreferencia2 INT8;
DEFINE vextensiontrabajo, vciudadreferencia, vcoloniaconyuge, vciudadconyuge, iNumerocasaconyuge, vcoloniareferencia, vcallereferencia, iNumerocasaref2, vflagactualizacion, vcalletrabajoconyuge, vciudadreferencia2, iNumerocasaref, vcoloniareferencia2, vcallereferencia2, iRowId, vciudadbanco, vreposicion, iPuntuacion, vefectuo, vciudad, iSecuencia, iValor, iIngreso, vfoliotienda, vEdad, vcolonia, vcalle, vreferencia3, vreferencia2, iNumerocasa, iAniosHabita, vempleadoautorizo, vcoloniabanco, iNumerocasatrabajo, vcalletrabajo, vcoloniatrabajo, iContConsBuro, vfolioanterior, iRefSecuencias, iElementoScoring, iCompromisosSic, iLineaCredReal, iCapRealAbono, iCapMaximaAbono, iCuentaRegistros, iLineaCrediTope, iTopeAbonoCoppel, iCapSistematicabono, vciudadtrabajo, iMontoIngMensual, iEmpleadoSubCob, iElemento, iSqlErr, iEmpleadoGteAutori, vpersonasvivenendomicilio INTEGER;
DEFINE vsSQL LVARCHAR(32000);
DEFINE vingresomensual, vlimitecredito, vpersonastrabajan, vnumerodependientes, vuhrlote2, sFlagCapCobranza, vuhretapa2, vuhrandador2, sFlagTestParametrico, vuhtandador, vcausasituacionespecial, sPropNegocio, sFlagCapHuella, sParAltoRiesgo, sParPrestamo, vuhcentrada, vopcionpuesto, vuhcedificio, vuhredificio2, vSubopcionpuestoext, vNumempleadosext, vOpcionpuestoext, vSubopcionpuesto, vNumempleados, vTiposueldoext, vSistsegsocial, iFlagLineaCredEsp, vuhclote, vuhcetapa, vuhcandador, vuhrentrada, vuhrentrada2, vclaveproducto, vtiendafolioanterior, vuhtentrada, vuhredificio, vuhrlote, vuhcotros, vuhretapa, vsubnegocio, vuhrandador, vuhrotros, vuhcmanzana, vflaguhc, vuhrmanzana, vflaguhr, vnegocio, vuhtedificio, vflaguhy, vflaguht, vuhtmanzana, vuhtlote, vuhtotros, vuhymanzana, vuhyotros, vtiendafolio, vuhtetapa, vuhrotros2, vuhrmanzana2, vuhyandador, vuhyetapa, vflaguhr2, vuhylote, sParCelulares, vuhyedificio, vtiporeposicion, vuhyentrada, vPuestoext, vcaja SMALLINT;
...
LET vaceptadosupervisadorechazado = 'A';
LET vclientenuevo = '';
LET vcreditojoven = '';
LET vlugartrabajo = '';
...
BEGIN
	ON EXCEPTION
		SET iSqlErr
		Set debug file to '/RESPALDOSNEW/sp_GeneraArchivoCambioSolicitudAceptacion2.out';
		trace on;
	...
	IF pFechaAct <> mdy(1,1,1900) OR pFechaAct IS NOT NULL THEN		
		...
		ELSE
			FOREACH
				...
				WHERE sss.num_solicitud = ssa.num_solicitud AND sss.num_producto = '6500' AND ssa.fecha_entrada = pFechaAct AND ssa.status_solicitud = 'AT'
				IF vnumerosolicituddecredito <> '' OR vnumcte <> '' THEN
					...
					SELECT case when bdinteg:"informix".sp_EsNumerico(dir.numerociudad)       = 'V' then dir.numerociudad::integer else 0 end, 
								...
								NVL(TRIM(REPLACE(dir.numerointcalle,'|',' ')), ''), dir.puntocardinal, NVL(TRIM(REPLACE(dir.observaciones,'|',' ')), ''), NVL(TRIM(REPLACE(dir.entre_calles,'|',' ')), ''), 
								case when bdinteg:"informix".sp_EsNumerico(dir.unidadhabitac)  = 'V' then dir.unidadhabitac::smallint else 0 end, 
					...
					INTO vciudad, vcolonia FROM bdinteg:"informix".si_catzonas
					...
					IF NVL(vcolonia, 0) = 0 THEN
						...
						SELECT FIRST 1 numerocoloniacoppel INTO vcolonia FROM bdinteg:"informix".si_catzonas WHERE numerociudad = vciudadbanco AND numerocoloniacoppel <> 0;
					END IF;
					-- DSB 26/07/2012 Inicia Trabajo Clientes
					...
					SELECT case when bdinteg:"informix".sp_EsNumerico(dir.numerociudad)       = 'V' then dir.numerociudad::integer else 0 end, 
								...
								case when bdinteg:"informix".sp_EsNumerico(tel3.extension)      = 'V' then tel3.extension::int8 else 0 end
					...
					IF 	vestadocivil = 'C' THEN
						...
						SELECT case when bdinteg:"informix".sp_EsNumerico(dir2.numerociudad)    = 'V' then dir2.numerociudad::integer else 0 end, 
								...
								NVL(TRIM(REPLACE(dir2.numerointcalle,'|',' ')), ''), dir2.puntocardinal, NVL(TRIM(REPLACE(dir2.observaciones,'|',' ')), ''), NVL(TRIM(REPLACE(dir2.entre_calles,'|',' ')), ''), 
								case when bdinteg:"informix".sp_EsNumerico(dir2.unidadhabitac)  = 'V' then dir2.unidadhabitac::smallint else 0 end, 
					...
					-- DSB 26/07/2012 CONSULTA NUEVO PARAMETRICO
					SELECT MAX(ROWID) INTO iRowId FROM bdisolic:"informix".ss_nuevo_parametrico WHERE num_solicitud = vnumerosolicituddecredito;
					SELECT case when bdinteg:"informix".sp_EsNumerico(ingreso_mensual)    = 'V' then ingreso_mensual::integer else 0 end, 
								...
								case when bdinteg:"informix".sp_EsNumerico(flaglineacreditoesp) = 'V' then flaglineacreditoesp::smallint else 0 end
					...
					END IF;
					SELECT limitecredito INTO vlimitecredito FROM bdisolic:"informix".ss_nuevo_parametrico WHERE ROWID = iRowId;
