CREATE PROCEDURE "informix".sp_generafolionominacre(cID_USUARIOC CHAR(8),
...
BEGIN
	...
	END IF;
        ...
        EXECUTE PROCEDURE bdicheq:"informix".sp_generafolionomina(cNombreUsuario)
		INTO vCodRet,
		     cNumeroFolio;
...
"FECHA: 10/07/2013",
"DESCRIPCION: Genera los folios de los movimientos en credito";

CREATE PROCEDURE "informix".sp_grabarcargosmanualescre(pUsuario CHAR(8), pIdFuncion CHAR(10), pFolioSuc CHAR(16), pNumCliente CHAR(20), pNumCuenta CHAR(20),
					pImporteCargo MONEY(18,2), pTransaccion CHAR(4), pConcepto CHAR(20), pObservaciones CHAR(200),
					pCapitalVigente MONEY(18,2), pCapitalTransitorio MONEY(18,2), pCapitalVencido MONEY(18,2), pCapitalVdoNoExigible MONEY(18,2), pCapitalTotal	MONEY(18,2),
					pInteresVigente MONEY(18,2), pIvaInteresVigente MONEY(18,2), pInteresVencido MONEY(18,2), pIvaInteresVencido MONEY(18,2), pInteresMoratorio MONEY(18,2),
					pIvaInteresMoratorio MONEY(18,2), pCodigo CHAR(2))
	RETURNING CHAR(5) AS codret,
					MONEY(18,2) AS capital_vigente,
					MONEY(18,2) AS capital_transitorio,
					MONEY(18,2) AS capital_vencido,
					MONEY(18,2) AS capital_vdonoexigible,
					MONEY(18,2) AS capital_total,
					MONEY(18,2) AS interes_vigente,
					MONEY(18,2) AS iva_interes_vigente,
					MONEY(18,2) AS interes_vencido,
					MONEY(18,2) AS iva_interes_vencido,
	...
	LET mCapitalTransitorio = 0;
	LET mCapitalVencido = 0;
	LET mCapitalVdoNoExigible = 0;
	LET mCapitalTotal = 0;
	LET mInteresVigente = 0;
	LET mIvaInteresVigente = 0;
	...
	BEGIN
		...
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet, mCapitalVigente, mCapitalTransitorio, mCapitalVencido, mCapitalVdoNoExigible, mCapitalTotal, 
					mInteresVigente, mIvaInteresVigente, mInteresVencido, mIvaInteresVencido, mInteresMoratorio, mIvaInteresMoratorio;
		END EXCEPTION;
		
		IF pUsuario = '' OR pIdFuncion = '' OR pFolioSuc = '' OR pNumCliente = '' OR pNumCuenta = '' OR
					pImporteCargo = '' OR pTransaccion = '' OR pConcepto = '' OR pObservaciones = '' OR
					pCapitalVigente = '' OR pCapitalTransitorio = '' OR pCapitalVencido = '' OR pCapitalVdoNoExigible = '' OR pCapitalTotal	= '' OR
					pInteresVigente = '' OR pIvaInteresVigente = '' OR pInteresVencido = '' OR pIvaInteresVencido = '' OR pInteresMoratorio = '' OR
					pIvaInteresMoratorio = '' OR pCodigo = '' THEN
		...
		-- Validación de la ejecución del SP
		EXECUTE PROCEDURE bdinteg:sp_cnsif_permisosejecutivo(pUsuario, pIdFuncion, pNumCliente, '06', '2') into cCodRet;
		IF cCodRet <> '00000' THEN
			RETURN cCodRet, mCapitalVigente, mCapitalTransitorio, mCapitalVencido, mCapitalVdoNoExigible, mCapitalTotal, 
					mInteresVigente, mIvaInteresVigente, mInteresVencido, mIvaInteresVencido, mInteresMoratorio, mIvaInteresMoratorio;
		END IF;
		
		EXECUTE PROCEDURE bdicred:sp_grabarcargosmanuales(cEmpresa, pUsuario, pFolioSuc, pNumCliente, pNumCuenta, pImporteCargo, pTransaccion, pConcepto, 
								pObservaciones, pCapitalVigente, pCapitalTransitorio, pCapitalVencido, pCapitalVdoNoExigible, pCapitalTotal, pInteresVigente,
								pIvaInteresVigente, pInteresVencido, pIvaInteresVencido, pInteresMoratorio, pIvaInteresMoratorio, pCodigo)
			INTO cCodRet, mCapitalVigente, mCapitalTransitorio, mCapitalVencido, mCapitalVdoNoExigible, mCapitalTotal, 
				mInteresVigente, mIvaInteresVigente, mInteresVencido, mIvaInteresVencido, mInteresMoratorio, mIvaInteresMoratorio;
...
DOCUMENT "AUTOR: Oscar Flores Conde",
"FECHA: 02/07/2013",
"DESCRIPCION: Procedimiento que realiza el cargo a una cuenta de credito";
