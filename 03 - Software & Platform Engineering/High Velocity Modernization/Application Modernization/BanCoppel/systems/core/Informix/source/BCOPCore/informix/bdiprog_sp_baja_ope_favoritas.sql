CREATE PROCEDURE "informix".sp_baja_ope_favoritas(pnumCte CHAR(20), pidOperacion INTEGER)
    RETURNING 	CHAR(5);

    DEFINE sql_err 		INTEGER;
    DEFINE cCod_ret 	CHAR(5);
	DEFINE vCteCount 	VARCHAR(20);
	
	LET cCod_ret = '00000';
	LET vCteCount = ''; --Se inicializa variable por PDRH.
	
	--SET DEBUG FILE TO "/informix/ireb/bdibpi/bex/sp_baja_ope_favoritas.out";
	--TRACE ON;
...
BEGIN
	...
	IF pnumCte <> '' AND pidOperacion <> 0 THEN 
		-- Se consulta por ID y numero de cliente la operación favorita para obtener los datos completos que son necesarios en el "paso1" de la Transferencia o Pago en BPI
		SELECT num_cte
		INTO vCteCount
		...
		LET vCteCount = NVL(vCteCount,'');
		
		IF vCteCount = '' THEN
			LET cCod_ret = '00002';		ELSE
	...
	ELSE
		LET cCod_Ret = '00001'; -- Parametros incorrectos o cliente sin operaciones favoritas.
...
'AUTOR: Aaron Quiñonez Fajardo',
'FECHA: 27/02/2019',
'SE CREA PROCEDIMIENTO PARA DESACTIVAR OPERACIONES FAVORITAS',
'DB: BDIPROG '*/;

CREATE PROCEDURE "informix".sp_consulta_ope_favoritas(pnumCte CHAR(20), pidOperacion INTEGER)
    RETURNING 	CHAR(5) AS codret,
				...
				CHAR (50) AS desc_operacion,
				CHAR(40) AS concepto,
				CHAR(50) As referencia_bene,
				CHAR(1) AS estatus,
				MONEY(16,2) AS importe;

    DEFINE sql_err 		INTEGER;
    DEFINE cCod_ret 	CHAR(5);
	DEFINE vCta_Terc    CHAR(20);
	DEFINE vCve_estado  CHAR(2);
	DEFINE vId_operacion INTEGER;
	DEFINE vNum_cte		CHAR(20);
	DEFINE vCta_origen 	CHAR(20);
	DEFINE vCta_destino CHAR (20);
	DEFINE vTipo_operacion 	CHAR(50);
	DEFINE vDesc_operacion 	CHAR (50);
	DEFINE vConcepto 	CHAR(40);
	DEFINE vReferencia_bene CHAR(50);
	...
	LET vDesc_operacion = '';
	LET vConcepto		= '';
	LET vReferencia_bene = '';
	LET vEstatus 		= '';
	LET vImporte 	    = 0;
	
	--SET DEBUG FILE TO "/informix/ireb/bdibpi/bex/sp_consulta_ope_favorita.out";
	--TRACE ON;
...
BEGIN
	...
	ON EXCEPTION SET sql_err
	  IF sql_err <> 0 THEN
			let cCod_ret = sql_err;
			RETURN cCod_ret, vId_operacion, vNum_cte, vCta_origen, vCta_destino, vTipo_operacion, vDesc_operacion, vConcepto, vReferencia_bene, vEstatus, vImporte;
	  END IF;
	END EXCEPTION;
	...
	IF pnumCte <> '' AND pidOperacion <> 0 THEN -- Consulta por ID, devuelve todos los datos.
		-- Se consulta por ID y numero de cliente la operación favorita para obtener los datos completos que son necesarios en el "paso1" de la Transferencia o Pago en BPI
		SELECT id_operacion, num_cte, cta_origen, cta_destino, tipo_operacion, desc_operacion, concepto, referencia_bene, estatus, importe
		INTO vId_operacion, vNum_cte, vCta_origen, vCta_destino, vTipo_operacion, vDesc_operacion, vConcepto, vReferencia_bene, vEstatus, vImporte
		FROM bdiprog:"informix".pp_registro_favoritos
		WHERE num_cte = pnumCte 
		AND estatus = '1' 
		AND id_operacion = pidOperacion;
		
		LET vNum_cte = NVL(vNum_cte,'');
	...
	ELIF pnumCte <> '' THEN  -- Consulta solo por no. de cliente, devuelve ID y descripcion.
		-- Obtiene el listado de operaciones favoritas
		FOREACH
			SELECT id_operacion, desc_operacion, tipo_operacion, cta_destino
			INTO vId_operacion, vDesc_operacion, vTipo_operacion, vCta_destino
			FROM bdiprog:"informix".pp_registro_favoritos
			WHERE num_cte = pnumCte 
			AND estatus = '1'
		
			IF vTipo_operacion = 'TRANSFERENCIAS_MIS_CUENTAS' OR  vTipo_operacion = 'PAGO_TDC_BANCOPPEL_PROPIA' THEN --Si es Trans. Ctas Propias o Pago TDC Propia
				LET vTipo_operacion = '';
				LET	vCta_destino = '';
				
				RETURN cCod_ret, vId_operacion, vNum_cte, vCta_origen, vCta_destino, vTipo_operacion, vDesc_operacion, vConcepto, vReferencia_bene, vEstatus, vImporte WITH RESUME;
			
			ELIF vTipo_operacion = 'ORDENES_PAGO' THEN -- Si es Orden de Pago
				SELECT cuenta, cve_estado
				INTO vCta_Terc, vCve_estado
				FROM bdiprog:"informix".pp_beneficiariosfrec_bpi						
				WHERE num_cte = pnumCte
				AND cuenta = vCta_destino;
				
				LET vCta_Terc = NVL(vCta_Terc,'');
				
				IF vCta_Terc = '' THEN
					--Cuenta destino(frecuente) no existe como cuenta frecuente
					CONTINUE FOREACH;
				ELIF vCve_estado <> '01' THEN
					--Cuenta destino(frecuente) no se encuentra activa
                        UPDATE bdiprog:"informix".pp_registro_favoritos
						SET estatus = '2'
						WHERE id_operacion = vId_operacion 
						AND num_cte = pnumCte;
		...
		LET vDesc_operacion = NVL(vDesc_operacion,'');
		
		IF vDesc_operacion = '' THEN
			LET cCod_ret = '00002';			RETURN cCod_ret, vId_operacion, vNum_cte, vCta_origen, vCta_destino, vTipo_operacion, vDesc_operacion, vConcepto, vReferencia_bene, vEstatus, vImporte;
		END IF;
...
'FECHA: 02/01/2018',
'SE CREA PROCEDIMIENTO PARA CONSULTAR OPERACIONES FAVORITAS',
'DB: BDIPROG '*/;
