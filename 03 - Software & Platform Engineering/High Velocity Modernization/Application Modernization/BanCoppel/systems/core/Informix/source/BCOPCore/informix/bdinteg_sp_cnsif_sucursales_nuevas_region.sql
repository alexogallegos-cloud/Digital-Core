CREATE PROCEDURE "informix".sp_cnsif_sucursales_nuevas_region(pUsuario CHAR(8), pIdFuncion CHAR(10), pFecha DATE, pRegistros INTEGER, pRecuperacion INTEGER)
RETURNING CHAR(5) AS codret,
        CHAR(4)   AS sucursal,
        CHAR(40)  AS nombre_sucursal,
		INTEGER   AS  numero_region,
		CHAR(2) AS  estado,
		CHAR(3) AS  ciudad,
		INTEGER AS  numero_ciudad,
	...
	DEFINE iNumeroRegion    INTEGER;     
	DEFINE cEstado          CHAR(2);
	DEFINE cCiudad          CHAR(3);
	DEFINE iNumeroCiudad    INTEGER; 
	DEFINE cTpo_sucursal	CHAR(2);
	...
	--SET DEBUG FILE TO '/informix/PRISCILLA/sp_cnsif_sucursales_nuevas_region.out';
	--TRACE ON;

	BEGIN
		ON EXCEPTION SET iSqlErr
			LET cCodRet = iSqlErr;
			RETURN cCodRet,cSucursal,cNombreSucursal, iNumeroRegion, cEstado, cCiudad, iNumeroCiudad, cTpo_sucursal;
		END EXCEPTION;
		
		--SET DEBUG FILE TO '/tmp/mfinis/sp_cnsif_sucursales_nuevas_region.out';
		--TRACE ON;
		...
		FOREACH		
			...
			SELECT SKIP pRegistros FIRST pRecuperacion ptf.id_ptf,suc.nombre,reg.numero_region,ptf.cve_estado,ptf.cve_ciudad,cat.numerociudad, suc.tpo_sucursal
			INTO cSucursal,cNombreSucursal, iNumeroRegion, cEstado, cCiudad, iNumeroCiudad, cTpo_sucursal
			...
			WHERE ptf.id_ptf = suc.sucursal AND  ptf.tipo = suc.tipo
			...
			AND suc.fecha_insert >= pFecha
			AND ptf.tipo = 'S'
			AND tpo_sucursal = 'S'
			ORDER BY ptf.id_ptf

			/*SELECT SKIP pRegistros FIRST pRecuperacion suc.sucursal,suc.nombre,reg.numero_region,suc.estado,suc.ciudad,cat.numerociudad, suc.tpo_sucursal
			INTO cSucursal,cNombreSucursal, iNumeroRegion, cEstado, cCiudad, iNumeroCiudad, cTpo_sucursal
			FROM bdinteg:"informix".si_ciudades ciu,
				 bdinteg:"informix".si_catciudades cat,
				 bdinteg:"informix".si_regiones reg,
			...
			RETURN cCodRet,cSucursal,cNombreSucursal, iNumeroRegion, cEstado, cCiudad, iNumeroCiudad, cTpo_sucursal WITH RESUME;
		END FOREACH;
...
'FECHA: 11/11/2016',
'MODULO: CONSULTAS',
'FUNCIONALIDAD: : Consulta sucursales nuevas para actualizar catalogo en postgres',
'DESCRIPCION: SP que consulta las nuevas sucursales',
'AUTOR: M.D.S. Sandra Cano',
'FECHA: 16/11/2016',
'DESCRIPCION: Modificacion del SPL para filtar registros que son tipo S, Sucursal',
'BD: bdinteg';
