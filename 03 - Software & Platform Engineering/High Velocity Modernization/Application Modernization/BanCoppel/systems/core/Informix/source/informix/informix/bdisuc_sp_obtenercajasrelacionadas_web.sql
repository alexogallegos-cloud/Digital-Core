CREATE PROCEDURE "informix".sp_obtenercajasrelacionadas_web(pInumsucursal VARCHAR(4), pIopcion VARCHAR(1), pItipopaquete VARCHAR(1), pSecuencia VARCHAR(2))
	...
	--TRACE ON;
		...
		IF pIopcion = '1' THEN 
			...
			SELECT fecha_hoy INTO cFecha FROM bdinteg:si_fechas where empresa = '001';
		...
		ELSE  --pIopcion
			...
			ELSE
				...
				FOREACH
					SELECT skip pSecuencia sucursal_relacionada INTO cCaja FROM bdisuc:"informix".tme_sucursalesrelacionadas 
					WHERE sucursal_matriz = pInumsucursal ORDER BY sucursal_relacionada = pInumsucursal
