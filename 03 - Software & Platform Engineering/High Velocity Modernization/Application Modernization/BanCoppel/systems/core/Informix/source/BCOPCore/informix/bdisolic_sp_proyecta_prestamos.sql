CREATE PROCEDURE "informix".sp_proyecta_prestamos( pMonto_Autorizado DECIMAL(18,6),  -- MONTO DEL CREDITO
...
BEGIN
 ...
 -- ***********************************************************************
	...
	-- 000012   NO ES POSIBLE REALIZAR UNA PROYECCIÃÂ¿N CON LAS CONDICIONES INDICADAS. (Este cÃÂ³digo de retorno llega desde el sp_obtiene_aproximacion)
...
--		LET mPlazo = mPlazo * 2;
	...
	SELECT numcte INTO vNumCte FROM bdisolic:"informix".ss_solicitudes WHERE num_solicitud = pNumCred AND canal_sol in (6,7) and status_solicitud != 'AP' ;
...
--JOM	LET mPlazoAux=mPlazo/pFrecuencia;
	...
	IF pTipoRetorno <> 2 THEN
		...
		IF NVL(mPlazo,0) > 0 AND NVL(mCapacidadPres,0) > 0 AND NVL(mMontoAut,0) = 0 THEN
			CALL "informix".sp_obtiene_aproximacion(0,mPlazo,mCapacidadPres,mTasaInt,mTasaIVA,dFechaCouta,mIVA,dLimites,dDiferencia,iTpoPago,pNumCred) RETURNING cCodRet, mMontoAut;
		...
		ELIF NVL(mPlazo,0) > 0 AND NVL(mMontoAut,0) > 0 AND NVL(mCapacidadPres,0) = 0 THEN
			CALL "informix".sp_obtiene_aproximacion(mMontoAut,mPlazo,0,mTasaInt,mTasaIVA,dFechaCouta,mIVA,dLimites,dDiferencia,iTpoPago,pNumCred) RETURNING cCodRet, mCapacidadPres;
			IF cCodRet <> "000000" THEN
					IF cCodRet < 0  THEN
						LET  cCodRet  = "000010";
						RETURN cCodRet, mPeriodo, dFechaCouta, mSdoInicial, mMensualidad, mIntereses, mIvaInt, mCapital, mSdoFinal, sDiasPeriodo, dFechaAper, mPlazoAux;
					ELIF cCodRet = "000012" THEN
						...
						WHILE  sContinua = 0	
							...
							CALL "informix".sp_obtiene_aproximacion(mMontoAut,mPlazoAux,0,mTasaInt,mTasaIVA,dFechaCouta,mIVA,dLimites,dDiferencia,iTpoPago,pNumCred) RETURNING cCodRet, mCapacidadPres;			
							IF cCodRet <> "000000" THEN
								IF cCodRet < 0  THEN
									LET  cCodRet  = "000011";
									RETURN cCodRet, mPeriodo, dFechaCouta, mSdoInicial, mMensualidad, mIntereses, mIvaInt, mCapital, mSdoFinal, sDiasPeriodo, dFechaAper, mPlazoAux;
...
--JOM								LET mPlazoAux=mPlazo/pFrecuencia;								
		...
		ELIF NVL(mCapacidadPres,0) > 0 AND NVL(mMontoAut,0) > 0 AND NVL(pPlazo,0) = 0 THEN
			...
			CALL "informix".sp_obtiene_aproximacion(mMontoAut,mPlazo,mCapacidadPres,mTasaInt,mTasaIVA,dFechaCouta,mIVA,dLimites,dDiferencia,iTpoPago,pNumCred) RETURNING cCodRet, mPlazo;
			IF cCodRet <> "000000" THEN
					IF cCodRet < 0  THEN
						LET  cCodRet  = "000011";
						RETURN cCodRet, mPeriodo, dFechaCouta, mSdoInicial, mMensualidad, mIntereses, mIvaInt, mCapital, mSdoFinal, sDiasPeriodo, dFechaAper, mPlazoAux;
					...
					--se obtiene la fecha de la proxima cuota.
						EXECUTE PROCEDURE "informix".sp_obtienefechapago('001',dFechaCouta,pNumCred)
							...
							IF cCodRet::INTEGER <> 0  THEN	
								LET cCodRet    = "000015";	--Ocurrio un Error al obtener la fecha de pago del crÃÂ©dito para credinomina.
								RETURN cCodRet, mPeriodo, dFechaCouta, mSdoInicial, mMensualidad, mIntereses, mIvaInt, mCapital, mSdoFinal, sDiasPeriodo, dFechaAper, mPlazoAux;
							END IF;		
