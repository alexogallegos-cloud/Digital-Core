CREATE PROCEDURE "informix".sp_calcularvencidodiarioestadocuenta_tdc(pempresa CHAR(3), pfechainicial DATE, pfechafinal DATE, pnumcredito CHAR(12),pUsuario CHAR(8),pFechaBase DATE)

RETURNING CHAR (5), DATE, DECIMAL (16,2);

--VARIABLES
DEFINE cCodRet         CHAR(6);
DEFINE iSqlError       INTEGER;
DEFINE mCapitalVencido DECIMAL (16,2);
DEFINE mAbono          DECIMAL (16,2);
DEFINE mCargo          DECIMAL (16,2);
DEFINE cNumCredito     CHAR(12);
DEFINE cDia			   CHAR(2);
DEFINE cMes 		   CHAR(3);
DEFINE cAnio 		   CHAR(4);
DEFINE cFechaFormato   CHAR(9);
DEFINE dFecha          DATE;
...
lET cAnio	        = "";
LET cFechaFormato   = "";
LET dFecha          = pFechaInicial -1 UNITS DAY;

BEGIN
ON EXCEPTION SET iSqlError
	IF iSqlError <> 0 THEN
		LET cCodRet = iSqlError;
		RETURN cCodRet, dFecha, mCapitalVencido;
	END IF;
END EXCEPTION;
	...
	--SET DEBUG FILE TO '/tmp/jv/sp_calcularvencidodiarioestadocuenta_tdc.out';
	--TRACE ON;

	--VALIDA PARÃMETROS
	IF NVL(pEmpresa,'') = '' OR  NVL(pEmpresa,'') = '' OR NVL(pFechaInicial,date(1)) = '01/01/1900' OR NVL(pFechaFinal,date(1)) = '01/01/1900' OR NVL(pNumCredito,'') = '' OR NVL(pUsuario,'') = '' THEN
	  LET cCodRet = '001';   --ParÃ metros invÃ¡lidos
	  LET dFecha = '01/01/1900';
	  RETURN cCodRet, dFecha, mCapitalVencido;
	END IF;

	DELETE FROM bdicred:"informix".tmpsd_movhiscredito WHERE usuario = pUsuario;
		
		INSERT INTO bdicred:"informix".tmpsd_movhiscredito (fecha_mov, monto, naturaleza, num_credito, usuario)
    ...
    FOREACH
        SELECT num_credito
        INTO cNumCredito
        FROM bdicred:"informix".sd_detalle_edocta a
        WHERE a.num_credito = pNumCredito
		AND fecha_emision = pFechaFinal
		
        WHILE dFecha <= pFechaFinal

            IF  dFecha = pFechaInicial -1 UNITS DAY  THEN   --Para obtener el capital vencido del mes inmediato anterior			
				SELECT  NVL(capital_ven_tc,0)
				INTO  mCapitalVencido
				FROM  bdicred:"informix".sd_encabezado2_edocta
				WHERE num_credito = pNumCredito
				AND fecha_emision  =  pFechaInicial - 1 UNITS DAY;

				IF mCapitalVencido = "" OR mCapitalVencido IS NULL THEN
            ...
            ELSE  --Para obtener el capital vencido de los dias del periodo
                ...
                FROM TABLE(MULTISET(
                    ...
                    FROM bdicred:"informix".tmpsd_movhiscredito
                    WHERE fecha_mov = dFecha
					AND usuario= pUsuario));
            END IF;
						
            LET mCapitalVencido = mCapitalVencido - mAbono  + mCargo;
			
			IF pFechaBase = dFecha THEN
				RETURN cCodRet, dFecha, mCapitalVencido;
				EXIT FOREACH;
			ELSE 
				RETURN cCodRet, dFecha, mCapitalVencido
...
'DESCRIPCION: Se realiza sp para calcular el vencido diario del cliente.',
'FECHA: 13/01/2017',
'BD: bdicred';
