CREATE PROCEDURE "informix".sp_alta_centros800( pCentro CHAR(4), pCancelado CHAR(1))
RETURNING CHAR(5);
    ...
    BEGIN
        ...
        --SET DEBUG FILE TO "/tmp/sp_alta_Centros800.err";
        --TRACE ON;
			IF Sql_Err <> 0 THEN
				LET vCodRet = Sql_Err;
				RETURN vCodRet;
		...
		--SET DEBUG FILE TO "/tmp/sp_alta_Centros800.out";
		--TRACE ON;

		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;
		
	  IF EXISTS (SELECT 1 FROM bdinteg:"informix".si_cargacentros800 WHERE sucursal800 = pCentro) THEN
		
		UPDATE bdinteg:"informix".si_cargacentros800 
		SET cancelado = pCancelado
		WHERE sucursal800 = pCentro;
	
	  ELSE
		
		INSERT INTO bdinteg:"informix".si_cargacentros800 VALUES(pCentro,pCancelado);
...
END PROCEDURE
DOCUMENT
'Actualizacion del estatus cancelado y nuevos registros de sucursales',
'AUTOR : Hever Barraza',
'FECHA : 16/04/19',
'BD    : bdinteg';

CREATE PROCEDURE "informix".sp_alta_puesto_coppel( pEmpresa CHAR(3), pPuesto CHAR(3), pPuestoBanco CHAR(3), pNombramiento CHAR(30))
RETURNING CHAR(5);
    ...
    BEGIN
		...
		ON EXCEPTION SET Sql_Err
        -- SET DEBUG FILE TO "/tmp/sp_alta_puesto_coppel.err";
        -- TRACE ON;
			IF Sql_Err <> 0 THEN
				LET vCodRet = Sql_Err;
				RETURN vCodRet;
		...
		-- SET DEBUG FILE TO "/tmp/sp_alta_puesto_coppel.out";
		-- TRACE ON;

		SET ISOLATION TO DIRTY READ;
		SET LOCK MODE TO WAIT 3;
		
	 IF EXISTS (SELECT 1 FROM bdinteg:"informix".si_puestosrelacion WHERE puesto_coppel = pPuesto) THEN
			
		UPDATE bdinteg:"informix".si_puestosrelacion 
		SET empresa = pEmpresa, puesto_bancoppel = pPuestoBanco, nombramiento = pNombramiento
		WHERE puesto_coppel = pPuesto;
	
	ELSE
	
		INSERT INTO bdinteg:"informix".si_puestosrelacion VALUES(pEmpresa,pPuesto,pPuestoBanco,pNombramiento);
...
END PROCEDURE
DOCUMENT
'Actualizacion de empresa, puesto bancoppel y nombramiento ademas de nuevos registros de puestos coppel',
'AUTOR : Hever Barraza',
'FECHA : 16/04/19',
'BD    : bdinteg';
