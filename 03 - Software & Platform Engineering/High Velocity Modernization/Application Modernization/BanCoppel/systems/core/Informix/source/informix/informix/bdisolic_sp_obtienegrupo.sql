CREATE PROCEDURE "informix".sp_obtienegrupo(pnum_solicitud CHAR(12)) 
RETURNING 
CHAR(6) AS codret,
--CHAR(80) AS Mensaje,
CHAR(2) AS tipogrupo, 
...
SELECT COUNT(*) INTO iBanPreAprobado FROM bdisolic:ss_solicitudes 
WHERE num_solicitud = pnum_solicitud AND user_insert='sys_cred';

IF iBanPreAprobado <> 1 THEN
...
IF NVL(pmeses_historia,0) > 0 THEN 
	EXECUTE PROCEDURE bdinteg:"informix".mesesvalidoscte (vlNumCte) INTO cCodRet1,iMeses;
	 IF NVL(pmeses_historia,0) > iMeses  THEN 
		...
		INSERT INTO "informix".ss_cambio_grupo (empresa ,num_solicitud ,grupo_anterior,grupo_nuevo ,user_insert ,fecha_insert)
		VALUES (pEmpresa,pnum_solicitud,ptipogrupoAux,ptipogrupo,USER,CURRENT);

	END IF;
END IF;
ELSE
  LET ptipogrupo = pgrupo;
END IF;
    --CALL bdisolic:"informix".sp_obtengrupocliente(vlNumCte) RETURNING vlCodigo, vlGpoCliente;

    -- Actualiza informacion de grupo y de tabla para auditoria-cnbv
    UPDATE "informix".ss_revision_determinacion SET grupo = ptipogrupo WHERE empresa = pempresa AND num_solicitud = pnum_solicitud;
    UPDATE "informix".ss_resum_scor_fin SET grupo = ptipogrupo WHERE empresa = pempresa AND num_solicitud = pnum_solicitud;
