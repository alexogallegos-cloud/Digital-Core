CREATE PROCEDURE "informix".sp_consulta_referencias_bpi(pNumCte CHAR(20), pTipoCta CHAR(2), pCveBanco CHAR(3),pReg SMALLINT)
	RETURNING CHAR(5) as codret,CHAR(20) as  alias,CHAR(20) as referencia,CHAR(1) as inhabil, MONEY(16,2) as monto;
	...
	--Fecha: 30/09/2010
	--Modifica: Walber Castro
	--Actividad: Se agrega parametro de salida de monto máximo.
	--Fecha: 18/11/2011
	-- *************************************************
...
BEGIN
	...
	FOREACH
		SELECT SKIP pReg FIRST 10 descrip_cta,cuenta, canal_alta, fecha_insert, hora_insert , NVL(monto_maximo,0)
		INTO v_Alias,v_Referencia, v_Canal, v_FechaInsert, v_HoraInsert, v_MontoMaximo
		...
		--AND (current - ( YEAR(fecha_insert) || '-' || MONTH(fecha_insert) || '-' || DAY(fecha_insert) || ' ' || hora_insert)::DATETIME YEAR TO FRACTION) > '0 00:30:00'
			...
			LET v_Cont = 1;
			RETURN v_CodRet,v_Alias,v_Referencia,v_Inhabil,v_MontoMaximo WITH RESUME;
