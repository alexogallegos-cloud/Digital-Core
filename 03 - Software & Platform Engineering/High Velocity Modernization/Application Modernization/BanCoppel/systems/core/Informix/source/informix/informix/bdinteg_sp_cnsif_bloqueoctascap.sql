CREATE PROCEDURE "informix".sp_cnsif_bloqueoctascap(cID_USUARIOC char(10),cID_FUNCIONC CHAR(10),cNUMCUENTA CHAR(20),pNumRegistro INTEGER,pRecuperacion INTEGER)
...
RETURNING 	CHAR(5)  AS Cod_Retorno,
			...
			INTEGER  AS Opcion,
			CHAR(35) AS Desc_Opcion,
			CHAR(30) AS Area,
			CHAR(20) AS Tipo_Bloqueo;
...
DEFINE cMDescripcion	CHAR(35);
DEFINE iOpcion 			INTEGER;
DEFINE cODescripcion 	CHAR(35);
DEFINE cArea			CHAR(30);
DEFINE cTBloqueo		CHAR(20);
...
ON EXCEPTION SET vsqlerr
	IF vsqlerr != 0 THEN
      ...
      RETURN scod_ret,cMovieminto,cClave,cStatus,mImporte,dFecha, cUsuario, cNombre_usuario, cMotivo, cMDescripcion, iOpcion,cODescripcion,cArea,cTBloqueo;
	END IF;
END EXCEPTION;


	 --SET DEBUG FILE TO "/tmp/CNSIF/sp_cnsif_consultacta.out";
	 --TRACE ON;
...
-- ****************************************************************************
  ...
  SELECT nvl(COUNT(cuenta),0) INTO iexiste FROM bdicheq:sc_histbloq WHERE cuenta = cNUMCUENTA AND empresa = '001';	
  IF iexiste = 0 THEN 
		LET scod_ret = "00039";
		RETURN scod_ret,cMovieminto,cClave,cStatus,mImporte,dFecha, cUsuario, cNombre_usuario, cMotivo, cMDescripcion, iOpcion,cODescripcion,cArea,cTBloqueo;
  END IF;	
  ...
  FOREACH
	SELECT SKIP pNumRegistro FIRST pRecuperacion hb.tipo_mov,hb.clave,hb.status_blo,hb.importe,hb.fecha,hb.usuario,EJ.nombre,hb.motivo,cb.descripcion,
	hb.opcion,ob.descripcion,ab.descripcion,TB.descripcion
	INTO 
	cMovieminto,cClave,cStatus,mImporte,dFecha, cUsuario, cNombre_usuario, cMotivo, cMDescripcion, iOpcion,cODescripcion,cArea,cTBloqueo
	FROM bdicheq:sc_histbloq hb
	LEFT JOIN bdinteg:si_ejecut EJ 
	ON EJ.ejecutivo= hb.usuario    
	LEFT JOIN bdicheq:sc_opcionbloqueo ob  
	ON  ob.opcion= hb.opcion
	LEFT JOIN bdicheq:sc_bloqueo cb 
	ON  cb.codigo=hb.motivo
	LEFT OUTER JOIN bdicheq:sc_areabloqueo ab
    ON hb.cve_area = ab.clave
	LEFT JOIN bdicheq:sc_tipobloqueo TB
	ON hb.cod_tipobloq = TB.codigo
	WHERE hb.cuenta= cNUMCUENTA ORDER BY hb.fecha desc,hb.hora desc

    LET iCont=iCont+1;	  
	RETURN scod_ret,cMovieminto,cClave,cStatus,mImporte,dFecha, cUsuario, cNombre_usuario, cMotivo, cMDescripcion, iOpcion,cODescripcion,cArea,cTBloqueo with resume;
...
"FUNCIONAMIENTO:Este sp realizara la busqueda de bloqueos en la tabla de historial de bloqueos dependiendo del numero de cuenta que se le envie al SP",
"FECHA : 06-01-2012",
"BD    : bdinteg",
"Modifico : Victor Hugo Sánchez",
"MODIFICACION : Se agrega la paginacion",
"VER   : 1.0";
