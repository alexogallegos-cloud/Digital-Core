...
5. **Excepción D04 Sobres Digitales**: los SPs `sp_retiro_sd · sp_crea_sd · sp_abono_sd · sp_edic_sd`
   tienen P95 de 144–208s por **notificaciones síncronas cross-domain a bdimnsj (D09) post-COMMIT**,
   no por latencia de Informix. El target usará notificaciones asíncronas (Kafka/SNS) — su P95
   de operación bancaria será < 2s. **No aplicar SLO-AM-02 de comparación directa** para estos SPs.
   `sp_perso_sd` y `sp_consmov_sd` (P95=202/220s) tienen latencia por lock contention y posible
   full scan en sc_mov_sd — sí deben resolverse antes del parallel-run.
   Ver análisis completo: `knowledge-base/D04-bdicheq/21-observability-runbook.md §Análisis de causa raíz`.
